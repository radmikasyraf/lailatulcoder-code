...
import { assertVoiceBaseUrlNetworkAllowed, isStreamingVoiceModel, isKeytermEcho, resolveVoiceStreamConfig, resolveVoiceTranscriptionConfig, resolveVoiceTransport, transcribeVoiceAudio, } from './voice-transcriber.js';
function createConfig(models) {
    return {
        getAllConfiguredModels: vi.fn().mockReturnValue(models),
    };
}
function createSettings(env = {}, apiKey, allowedInsecureVoiceBaseUrls) {
    return {
        merged: {
            env,
            security: {
...
async function lookupPublicHost() {
...
describe('voice-transcriber', () => {
    ...
    it('resolves a plain voice model id from configured models', () => {
        ...
        expect(resolveVoiceTranscriptionConfig({
            config,
    ...
    it('routes known voice models by model id instead of user protocol', () => {
        ...
        expect(resolveVoiceTransport('qwen3-asr-flash-2026-02-10')).toBe('qwen-asr-chat');
        expect(resolveVoiceTransport('qwen3-asr-flash-realtime')).toBe('qwen-asr-realtime');
        expect(resolveVoiceTransport('qwen3-asr-flash-realtime-2026-02-10')).toBe('qwen-asr-realtime');
        expect(resolveVoiceTransport('fun-asr-realtime')).toBe('dashscope-task-realtime');
        expect(resolveVoiceTransport('fun-asr-flash-8k-realtime')).toBe('dashscope-task-realtime');
        expect(resolveVoiceTransport('paraformer-realtime-v2')).toBe('dashscope-task-realtime');
        expect(resolveVoiceTransport('qwen3-asr-flash-filetrans')).toBe('unsupported');
    });
    it('does not rewrite qwen3-asr-flash to a realtime model', () => {
        expect(isStreamingVoiceModel('qwen3-asr-flash')).toBe(false);
        expect(() => resolveVoiceStreamConfig({
            config: createConfig([
                {
                    id: 'qwen3-asr-flash',
                    label: 'Qwen ASR',
    ...
    it('threads a custom keyterms file term into the realtime keytermsContext', () => {
        const workspaceDir = fs.mkdtempSync(path.join(os.tmpdir(), 'voice-transcriber-keyterms-'));
    ...
    it('threads a custom keyterms file term into the batch keytermsContext', async () => {
        ...
        try {
            ...
            await transcribeVoiceAudio({ data: new Uint8Array([1, 2, 3]), mimeType: 'audio/wav' }, {
                config: createConfig([
                    {
                        id: 'qwen3-asr-flash',
                        label: 'Qwen ASR',
    ...
    it('allows an exact trusted HTTP private voice base URL', async () => {
        ...
        await expect(assertVoiceBaseUrlNetworkAllowed(resolved, async () => ({
            address: '10.23.45.67',
    ...
    it('allows an exactly trusted IPv4-mapped private voice base URL', async () => {
        ...
        await expect(assertVoiceBaseUrlNetworkAllowed(resolved)).resolves.toBeUndefined();
    });
    it('keeps metadata DNS results blocked for a trusted voice URL', async () => {
        ...
        for (const address of [
            ...
            await expect(assertVoiceBaseUrlNetworkAllowed(trusted, async () => ({ address }))).rejects.toThrow('resolved to an address that is always blocked');
        }
    });
    it('rejects loopback DNS results for a trusted voice URL with loopback guidance', async () => {
        ...
        for (const address of [
            ...
            await expect(assertVoiceBaseUrlNetworkAllowed(trusted, async () => ({ address }))).rejects.toThrow(/loopback/);
        }
    });
    it('rejects a multi-record DNS answer when any record is blocked', async () => {
        ...
        await expect(assertVoiceBaseUrlNetworkAllowed(trusted, async () => [
            { address: '8.8.8.8' },
            { address: '169.254.169.254' },
        ])).rejects.toThrow('resolved to an address that is always blocked');
        await expect(assertVoiceBaseUrlNetworkAllowed({
            model: 'qwen3-asr-flash',
            baseUrl: 'http://voice.internal.example/v1',
        }, async () => [{ address: '8.8.8.8' }, { address: '10.23.45.67' }])).rejects.toThrow(/private-network/);
        await expect(assertVoiceBaseUrlNetworkAllowed(trusted, async () => [
            { address: '8.8.8.8' },
            { address: '10.23.45.67' },
        ])).resolves.toBeUndefined();
    ...
    it('names the accepted loopback spellings for non-canonical loopback literals', () => {
        ...
        for (const baseUrl of [
            ...
            expect(() => resolveVoiceTranscriptionConfig({
                ...
                settings: createSettings({}, undefined, [baseUrl]),
                voiceModel: 'qwen3-asr-flash',
            })).toThrow("Voice model 'qwen3-asr-flash' uses a loopback address outside the accepted spellings. To use a local ASR endpoint, set the baseUrl to http://localhost, http://127.0.0.1, or http://[::1].");
        }
    });
    it('names the accepted loopback spellings when a network check sees a loopback literal', async () => {
        for (const baseUrl of [
            ...
            await expect(assertVoiceBaseUrlNetworkAllowed({
                model: 'qwen3-asr-flash',
                baseUrl,
            })).rejects.toThrow(/uses a loopback address outside the accepted spellings/);
        }
    });
    ...
    it('names the exact normalized URL to allowlist in both rejection messages', () => {
        ...
        expect(() => resolveVoiceTranscriptionConfig({
            ...
            settings: createSettings(),
            voiceModel: 'qwen3-asr-flash',
        })).toThrow(/add its exact complete normalized URL \(http:\/\/voice\.region-a\.internal\.example\/v1\) to security\.allowedInsecureVoiceBaseUrls\. This setting is only honored from User, System, or SystemDefaults scope settings; Workspace entries are ignored\./);
        const privateConfig = createConfig([
            {
                id: 'qwen3-asr-flash',
                label: 'Private ASR',
        ...
        expect(() => resolveVoiceTranscriptionConfig({
            ...
            settings: createSettings(),
            voiceModel: 'qwen3-asr-flash',
        })).toThrow(/add its exact complete normalized URL \(https:\/\/10\.0\.0\.5\/v1\) to security\.allowedInsecureVoiceBaseUrls\. This setting is only honored from User, System, or SystemDefaults scope settings; Workspace entries are ignored\./);
    });
    ...
    it('reaches the batch transport for an allowlisted private IP-literal gateway', async () => {
        ...
        const text = await transcribeVoiceAudio({ data: new Uint8Array([1, 2, 3]), mimeType: 'audio/wav' }, {
            config: createConfig([
                {
                    id: 'qwen3-asr-flash',
                    label: 'Private Qwen ASR',
    ...
    it('rejects voice model hosts that resolve to private-network IPs', async () => {
        ...
        await expect(transcribeVoiceAudio({ data: new Uint8Array([1, 2, 3]), mimeType: 'audio/wav' }, {
            config: createConfig([
                {
                    id: 'qwen3-asr-flash',
                    label: 'Qwen ASR',
    ...
    it('canonicalizes expanded IPv6 DNS results before applying network policy', async () => {
        ...
        for (const address of [
            ...
            await expect(assertVoiceBaseUrlNetworkAllowed(config, async () => ({ address }))).rejects.toThrow(/private-network address/);
        }
        for (const address of [
            '64:ff9b::5db8:d822',
            '64:ff9b:2::1',
            ...
            await expect(assertVoiceBaseUrlNetworkAllowed(config, async () => ({ address }))).resolves.toBeUndefined();
        }
    });
    ...
    it('aborts a pending DNS safety lookup', async () => {
        ...
        const check = assertVoiceBaseUrlNetworkAllowed({
            model: 'qwen3-asr-flash',
            baseUrl: 'https://asr.example/v1',
        }, () => new Promise(() => undefined), controller.signal);
    ...
    it('allows explicit loopback voice URLs without DNS lookup', async () => {
        ...
        for (const baseUrl of [
            ...
            await expect(assertVoiceBaseUrlNetworkAllowed({ model: 'qwen3-asr-flash', baseUrl }, lookupHost)).resolves.toBeUndefined();
        }
    ...
    it('keeps terse speech that happens to use a few keyterms', () => {
        expect(isKeytermEcho('commit schema endpoint async', 'commit schema endpoint async await api cli npm grep regex json refactor')).toBe(false);
    });
    it('keeps longer speech that only mentions a small slice of keyterms', () => {
        expect(isKeytermEcho('commit schema endpoint async await api cli npm', [
            'commit',
            'schema',
            'endpoint',
            'async',
    ...
    it('drops a keyterm echo even when user keyterms make the set large', () => {
        ...
        expect(isKeytermEcho('grep regex typescript json oauth subagent worktree endpoint middleware schema', keyterms.join(' '))).toBe(true);
    });
    it('posts audio to chat/completions as input_audio content', async () => {
        ...
        expect(userMsg.content[0].input_audio.data).toMatch(/^data:audio\/wav;base64,/);
    ...
    it('derives input_audio format from the recorder mime type', async () => {
        ...
        await transcribeVoiceAudio({ data: new Uint8Array([1, 2, 3]), mimeType: 'audio/webm;codecs=opus' }, {
            config: createConfig([
                {
                    id: 'qwen3-asr-flash',
                    label: 'Custom ASR',
        ...
        const body = JSON.parse(init.body);
        const userMsg = body.messages.find((m) => m.role === 'user');
        expect(userMsg.content[0].input_audio.data).toMatch(/^data:audio\/webm;codecs=opus;base64,/);
    ...
    it('falls back to wav for octet-stream audio uploads', async () => {
        ...
        const body = JSON.parse(init.body);
        const userMsg = body.messages.find((m) => m.role === 'user');
        expect(userMsg.content[0].input_audio.data).toMatch(/^data:application\/octet-stream;base64,/);
        expect(userMsg.content[0].input_audio.format).toBe('wav');
    });
    ...
    it('redacts and truncates failed batch transcription responses', async () => {
        const fetchFn = vi.fn().mockResolvedValue({
            ...
            text: vi
                .fn()
                .mockResolvedValue(`Authorization: ApiKey sk-route Bearer sk-secret api_key=sk-query secret=sk-secret-token Invalid API key: sk-test ${'x'.repeat(500)}`),
        });
        let error;
        try {
            await transcribeVoiceAudio({ data: new Uint8Array([1, 2, 3]), mimeType: 'audio/wav' }, {
    ...
    it('sends an inference timeout signal and reports timeout clearly', async () => {
        ...
        await expect(transcribeVoiceAudio({ data: new Uint8Array([1, 2, 3]), mimeType: 'audio/wav' }, {
            ...
            lookupHost: lookupPublicHost,
            fetchFn,
        })).rejects.toThrow('Voice transcription timed out after 60s. Check ASR service health and retry.');
    ...
    it('drops an echoed keyterm list instead of inserting it', async () => {
        ...
        const echoed = 'grep regex TypeScript JSON OAuth subagent worktree endpoint middleware schema';
        const fetchFn = vi.fn().mockResolvedValue({
            ok: true,
            json: vi
                .fn()
