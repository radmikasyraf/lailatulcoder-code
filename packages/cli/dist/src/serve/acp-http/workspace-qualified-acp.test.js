...
function makeBridge() {
    return {
        spawnOrAttach: vi.fn(async (req) => ({
            ...
            attached: false,
            clientId: req.workspaceCwd === '/ws-b' ? 'secondary-client' : 'primary-client',
        })),
        getSessionSummary: vi.fn((sessionId) => {
...
async function writeStoredSession(sessionId, cwd, metadata = {}) {
    ...
    await fsp.writeFile(path.join(chatsDir, `${sessionId}.jsonl`), `${records.map((record) => JSON.stringify(record)).join('\n')}\n`, 'utf8');
}
describe('workspace-qualified ACP (/workspaces/:workspace/acp)', () => {
    ...
    let workspaceVoiceConnection;
    let materializeLiveConversationDirectory;
    let activeLiveSessionIds;
    let runtimeDir;
    let previousRuntimeDir;
    beforeEach(async () => {
        previousRuntimeDir = process.env['QWEN_RUNTIME_DIR'];
        runtimeDir = await fsp.mkdtemp(path.join(os.tmpdir(), 'qwen-workspace-qualified-acp-'));
        process.env['QWEN_RUNTIME_DIR'] = runtimeDir;
        setupGithubMock.mockReset();
        setupGithubMock.mockImplementation(async ({ cwd }) => ({
            kind: 'github_setup',
            workspaceCwd: cwd,
            gitRepoRoot: cwd,
            releaseTag: 'v1.2.3',
        ...
        workspaceVoiceConnection = vi.fn((runtime, ws) => {
            ws.send(JSON.stringify({ workspaceCwd: runtime.workspaceCwd }));
            ws.close(1000, 'done');
        });
        materializeLiveConversationDirectory = vi.fn(async (sessionId) => `/live-root/conversation-${sessionId}`);
        ...
        handle = mountAcpHttp(app, primaryBridge, {
            ...
            liveSessionIsolation: {
                ...
                isSessionActive: (sessionId) => activeLiveSessionIds.has(sessionId),
            },
        });
    ...
    async function postInitialize(pathname) {
    ...
    async function postMessage(pathname, connectionId, id) {
        return fetch(`${base}${pathname}`, {
            method: 'POST',
            headers: {
    ...
    async function sendWsRequest(pathname, request) {
    ...
    async function sendWsRequests(pathname, requests) {
        return new Promise((resolve, reject) => {
            const responses = [];
            const ws = new WebSocket(`ws://127.0.0.1:${port}${pathname}`, {
                handshakeTimeout: 2000,
    ...
    async function initializeWs(pathname) {
        return new Promise((resolve, reject) => {
            const ws = new WebSocket(`ws://127.0.0.1:${port}${pathname}`, {
                handshakeTimeout: 2000,
            });
    ...
    it('routes initialize to a trusted secondary workspace by id', async () => {
        ...
        expect(writeStderrLine).toHaveBeenCalledWith(expect.stringContaining('/workspaces/secondary-id/acp connection established'));
    });
    it('uses daemon env for parent-process setup-github without leaking into overlays', async () => {
        ...
        expect(setupGithubMock).toHaveBeenLastCalledWith(expect.objectContaining({
            cwd: '/ws-b',
    ...
    it('keeps empty runtime overlays isolated from daemon env', async () => {
        workspaceRegistry.add(makeRuntime({
            id: 'empty-overlay-id',
            cwd: '/ws-empty',
            primary: false,
            trusted: true,
        ...
        expect(setupGithubMock).toHaveBeenLastCalledWith(expect.objectContaining({ cwd: '/ws-empty', proxy: undefined }));
    });
    it('routes initialize to a trusted secondary workspace by encoded cwd', async () => {
        const res = await postInitialize(`/workspaces/${encodeURIComponent('/ws-b')}/acp`);
        expect(res.status).toBe(200);
        expect(res.headers.get('acp-connection-id')).toBeTruthy();
    });
    it('meters HTTP requests independently across workspace mounts', async () => {
        ...
        const keys = checkRate.mock.calls.map(([key]) => key);
    ...
    it('keeps ACP shell scoped to the workspace-qualified connection', async () => {
        ...
        expect(secondaryBridge.executeShellCommand).toHaveBeenCalledWith('secondary-session', 'pwd', expect.any(AbortSignal), expect.objectContaining({ clientId: 'secondary-client' }));
    ...
    it('shares caller-supplied sessionId admission across primary and qualified mounts', async () => {
        ...
        vi.mocked(primaryBridge.spawnOrAttach).mockImplementationOnce(async (request) => {
            await primaryGate;
            return {
        ...
        await vi.waitFor(() => expect(primaryBridge.spawnOrAttach).toHaveBeenCalledOnce());
        const secondary = await sendWsRequest('/workspaces/secondary-id/acp', {
            jsonrpc: '2.0',
            id: 3,
            method: 'session/new',
    ...
    it('uses the concrete primary bridge generation for restore admission', async () => {
        ...
        await vi.waitFor(() => expect(primaryBridge.loadSession).toHaveBeenCalledOnce());
        ...
        workspaceRegistry.activateReplacement(entry, makeRuntime({
            id: 'primary-id',
            cwd: '/ws',
            primary: true,
            trusted: true,
    ...
    it('rolls back session/new when its generation changes while building the response', async () => {
        ...
        primaryBridge.getSessionContextStatus = vi.fn(async () => {
            await contextGate;
            return { v: 1, sessionId, workspaceCwd: '/ws', state: {} };
        });
        ...
        await vi.waitFor(() => expect(primaryBridge.getSessionContextStatus).toHaveBeenCalledOnce());
        const entry = workspaceRegistry.primaryEntry;
        expect(workspaceRegistry.beginReplacement(entry, 'policy-2')).toBe(true);
        workspaceRegistry.activateReplacement(entry, makeRuntime({
            id: 'primary-id',
    ...
    it('rolls back session/fork through the bridge generation that created it', async () => {
        ...
        await vi.waitFor(() => expect(primaryBridge.branchSession).toHaveBeenCalledOnce());
        const replacementBridge = makeBridge();
        const entry = workspaceRegistry.primaryEntry;
        expect(workspaceRegistry.beginReplacement(entry, 'policy-2')).toBe(true);
        workspaceRegistry.activateReplacement(entry, makeRuntime({
        ...
        expect(primaryBridge.killSession).toHaveBeenCalledWith('forked-primary-session', { requireZeroAttaches: true });
    ...
    it('uses the registry generation guard for qualified ACP mounts', async () => {
        ...
        secondaryBridge.getSessionContextStatus = vi.fn(async () => {
            await contextGate;
            return { v: 1, sessionId, workspaceCwd: '/ws-b', state: {} };
        });
        ...
        await vi.waitFor(() => expect(secondaryBridge.getSessionContextStatus).toHaveBeenCalledOnce());
    ...
    it('updates persisted organization in the selected workspace only', async () => {
        ...
        const secondarySnapshot = await createSessionOrganizationService('/ws-b').readSnapshot();
        const primarySnapshot = await createSessionOrganizationService('/ws').readSnapshot();
    ...
    it('rejects an unknown workspace selector with 400 workspace_mismatch', async () => {
        const res = await postInitialize(`/workspaces/${encodeURIComponent('/does-not-exist')}/acp`);
    ...
    it('rejects workspace-qualified ACP access to the Live runtime', async () => {
        ...
        const restoreSession = async (req) => ({
            sessionId: req.sessionId,
        ...
        const changeSessionCwd = vi.fn(async (sessionId, req) => ({
            sessionId,
        ...
        await expect(sendWsRequest('/workspaces/live-id/acp', {
            jsonrpc: '2.0',
            id: 40,
            method: 'session/load',
            params: { sessionId: 'live-session' },
        })).rejects.toThrow('Unexpected server response: 400');
        await expect(sendWsRequest('/workspaces/%2Flive-root/acp', {
            jsonrpc: '2.0',
            id: 41,
            method: 'session/load',
            params: { sessionId: 'live-session' },
    ...
    it('exposes qualified HTTP and WS routes with one runtime', async () => {
        ...
        const singleHandle = mountAcpHttp(app, primaryBridge, {
            ...
            requestedSessionIdAdmission: createRequestedSessionIdAdmission({
                archiveCoordinator,
                getBridges: () => registry.listManaged().map((runtime) => runtime.bridge),
                getPersistenceTargets: () => registry.listManaged().map((runtime) => ({
                    workspaceCwd: runtime.workspaceCwd,
                    runtimeBaseDir: runtime.sessionRuntimeBaseDir,
                })),
                getBridgeWorkspaceId: (bridge) => registry
                    .listEntries()
                    .find((entry) => entry.current?.runtime.bridge === bridge)
                    ?.workspaceId,
            }),
        });
        const singleServer = await new Promise((resolve) => {
            const listening = app.listen(0, '127.0.0.1', () => resolve(listening));
        });
        singleHandle.attachServer(singleServer);
        const singlePort = singleServer.address().port;
        try {
            const qualified = await fetch(`http://127.0.0.1:${singlePort}/workspaces/primary-id/acp`, {
                method: 'POST',
                headers: { 'content-type': 'application/json' },
                body: INITIALIZE,
            });
            expect(qualified.status).toBe(200);
            const upgradeStatus = await new Promise((resolve) => {
                const ws = new WebSocket(`ws://127.0.0.1:${singlePort}/workspaces/primary-id/acp`);
                ws.on('open', () => {
                    ws.close();
                    resolve(101);
                });
            ...
            const drainingDynamic = await fetch(`http://127.0.0.1:${singlePort}/workspaces/dynamic-id/acp`, {
                method: 'POST',
                headers: { 'content-type': 'application/json' },
                body: INITIALIZE,
            });
            ...
            const dynamic = await fetch(`http://127.0.0.1:${singlePort}/workspaces/dynamic-id/acp`, {
                method: 'POST',
                headers: { 'content-type': 'application/json' },
                body: INITIALIZE,
            });
    ...
    it('routes a WS upgrade + initialize to a trusted secondary workspace', async () => {
        ...
        expect(result.result?.agentCapabilities?._meta?.qwen?.workspaceCwd).toBe('/ws-b');
    });
    it('routes a WS upgrade by encoded workspace cwd', async () => {
        const result = await initializeWs(`/workspaces/${encodeURIComponent('/ws-b')}/acp`);
        expect(result.result?.agentCapabilities?._meta?.qwen?.workspaceCwd).toBe('/ws-b');
    });
    it('routes workspace-qualified Voice WS by id and encoded cwd', async () => {
        const connect = (selector) => new Promise((resolve, reject) => {
            const ws = new WebSocket(`ws://127.0.0.1:${port}/workspaces/${selector}/voice/stream`, { handshakeTimeout: 2000 });
            ws.on('message', (data) => {
                resolve(JSON.parse(data.toString()));
            });
            ws.on('error', reject);
    ...
    it('rejects unknown and untrusted workspace-qualified Voice WS upgrades', async () => {
        const status = (selector) => new Promise((resolve, reject) => {
            const ws = new WebSocket(`ws://127.0.0.1:${port}/workspaces/${selector}/voice/stream`, { handshakeTimeout: 2000 });
            ...
            ws.on('error', (err) => reject(new Error(`unexpected Voice WS error for ${selector}: ${err.message}`)));
        });
    ...
    it('rejects an encoded relative workspace-qualified Voice selector', async () => {
        ...
        const status = await new Promise((resolve, reject) => {
            const ws = new WebSocket(`ws://127.0.0.1:${port}/workspaces/${encodeURIComponent(relativeSelector)}/voice/stream`, { handshakeTimeout: 2000 });
            ws.on('unexpected-response', (_req, response) => {
                resolve(response.statusCode ?? 0);
                ws.terminate();
            });
    ...
    it('rejects a WS upgrade to an untrusted workspace', async () => {
        const status = await new Promise((resolve, reject) => {
            const ws = new WebSocket(`ws://127.0.0.1:${port}/workspaces/untrusted-id/acp`, { handshakeTimeout: 2000 });
            ws.on('unexpected-response', (_req, res) => {
                resolve(res.statusCode ?? 0);
                ws.terminate();
            });
            ...
            ws.on('error', (err) => reject(new Error(`unexpected ACP WS error: ${err.message}`)));
        });
    ...
    it('closes a WS upgraded before dispose without allowing initialize', async () => {
        const ws = new WebSocket(`ws://127.0.0.1:${port}/workspaces/secondary-id/acp`, { handshakeTimeout: 2000 });
        await new Promise((resolve, reject) => {
            ws.once('open', resolve);
            ws.once('error', reject);
        });
        const closed = new Promise((resolve, reject) => {
            const timer = setTimeout(() => reject(new Error('WebSocket stayed open after ACP disposal')), 2000);
            ws.once('close', () => {
                clearTimeout(timer);
                resolve();
            });
    ...
    it('counts and closes an uninitialized workspace WebSocket on removal', async () => {
        ...
        const closed = new Promise((resolve, reject) => {
            const timer = setTimeout(() => reject(new Error('Workspace WebSocket stayed open after removal')), 2000);
            ws.once('close', () => {
                clearTimeout(timer);
                resolve();
            });
    ...
    it('disposes only the target live WebSocket and allows a fresh mount', async () => {
        const connect = (pathname) => new Promise((resolve, reject) => {
            const ws = new WebSocket(`ws://127.0.0.1:${port}${pathname}`, {
                handshakeTimeout: 2000,
            });
            ws.on('open', () => ws.send(INITIALIZE));
        ...
        const secondaryClosed = new Promise((resolve) => {
        ...
        const primaryReply = new Promise((resolve, reject) => {
        ...
        primaryWs.send(JSON.stringify({ jsonrpc: '2.0', id: 2, method: 'unknown/mutation' }));
        ...
        await expect(initializeWs('/workspaces/secondary-id/acp')).resolves.toMatchObject({
            result: {
    ...
    it('drains, rolls back, disposes, and recreates a secondary mount', async () => {
        ...
        expect((await postInitialize('/workspaces/secondary-id/acp')).status).toBe(200);
        expect(workspaceRegistry.beginDrain(secondaryRuntime)).toBe(true);
        handle.beginWorkspaceDrain('secondary-id');
        const registryDraining = await postInitialize('/workspaces/secondary-id/acp');
        ...
        expect(handle
            .getSnapshot()
            .mounts.some((mount) => mount.workspaceId === 'secondary-id')).toBe(false);
        const replacementBridge = makeBridge();
        workspaceRegistry.add(makeRuntime({
            id: 'secondary-id',
            cwd: '/ws-b',
        ...
        const replies = await new Promise((resolve, reject) => {
            ...
            ws.on('message', (data) => {
                ...
                if (message['id'] === 1) {
                    ...
                    ws.send(JSON.stringify({ type: 'cdp_result', requestId: 'unknown' }));
                    ws.send(JSON.stringify({
                        type: 'mcp_message',
                        id: 'unknown',
                        server: 'missing',
    ...
    it('rejects a raw WS upgrade whose selector is a dot-segment (%2e%2e)', async () => {
        ...
        const outcome = await new Promise((resolve, reject) => {
            const socket = createConnection(port, '127.0.0.1', () => {
                socket.write('GET /workspaces/%2e%2e/acp HTTP/1.1\r\n' +
                    `Host: 127.0.0.1:${port}\r\n` +
                    'Upgrade: websocket\r\n' +
                    'Connection: Upgrade\r\n' +
            ...
            socket.on('close', () => resolve(buf.includes('101') ? 'upgraded' : 'closed'));
    ...
    it('rejects a WS upgrade to an unknown workspace selector', async () => {
        const status = await new Promise((resolve, reject) => {
            const ws = new WebSocket(`ws://127.0.0.1:${port}/workspaces/does-not-exist/acp`, { handshakeTimeout: 2000 });
            ws.on('unexpected-response', (_req, res) => {
                resolve(res.statusCode ?? 0);
                ws.terminate();
            });
    ...
    it('sanitizes decoded selectors before logging WS rejection', async () => {
        vi.mocked(writeStderrLine).mockClear();
        await new Promise((resolve, reject) => {
            const ws = new WebSocket(`ws://127.0.0.1:${port}/workspaces/evil%0AFORGED/acp`, { handshakeTimeout: 2000 });
        ...
        expect(writeStderrLine).toHaveBeenCalledWith(expect.stringContaining('workspace-mismatch evil FORGED'));
...
//# sourceMappingURL=workspace-qualified-acp.test.js.map