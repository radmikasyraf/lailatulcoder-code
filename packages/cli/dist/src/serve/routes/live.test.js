...
import { LIVE_HOST_BUNDLE_ID, LIVE_HOST_PROTOCOL_VERSION, } from '../live/types.js';
...
class FakeSocket extends EventEmitter {
    ...
    send(data) {
        ...
        if (message['type'] !== 'host.set_shortcut')
            return;
        queueMicrotask(() => {
...
function connectReady(coordinator) {
    const socket = new FakeSocket();
    coordinator.attachHost(socket, coordinator.daemonInstanceNonce);
    socket.hello();
    return socket;
}
afterEach(() => {
...
describe('Live routes', () => {
    it.each(['/live/start', '/live/new'])('serializes runtime ownership failures from %s without leaking details', async (route) => {
        ...
        registerLiveRoutes(app, {
            ...
            ensureRuntimeReady: async () => {
                throw new ConversationRuntimeOwnershipError('conversation_runtime_in_use', true, {
                    cause: new Error('/private/conversations owner=1234 nonce=secret'),
                });
            },
        });
    ...
    it('starts, mutes, creates a new call, and stops', async () => {
        ...
        const firstCallId = start.body.callId;
