...
const coreMock = vi.hoisted(() => ({
    ...
    modelsConfigErrorMessage: 'Failed loading provider https://user:secret@broken.example/v1',
    debugLogger: {
        debug: vi.fn(),
        error: vi.fn(),
        info: vi.fn(),
...
describe('createWorkspaceProvidersStatusProvider', () => {
    ...
    beforeEach(async () => {
        ...
        process.env['QWEN_CODE_SYSTEM_SETTINGS_PATH'] = path.join(tmpDir, 'system-settings.json');
        process.env['QWEN_CODE_SYSTEM_DEFAULTS_PATH'] = path.join(tmpDir, 'system-defaults.json');
    ...
    it('warns and falls back for an unknown workspace approval mode', async () => {
        ...
        const result = await provider(workspace, false);
        expect(result.approvalMode).toBe('auto');
        expect(coreMock.debugLogger.warn).toHaveBeenCalledWith('[workspace-providers-status] unrecognized approvalMode "auto-edt", falling back to auto');
    });
    it('marks only the model matching persisted model.baseUrl as current', async () => {
        ...
        const first = models.find((m) => m.baseUrl === 'https://api-one.example/v1');
        const second = models.find((m) => m.baseUrl === 'https://api-two.example/v1');
    ...
    it('filters fastOnly and voiceOnly models from the workspace provider catalog', async () => {
        ...
        const result = await provider(workspace, false);
        const modelIds = result.providers.flatMap((p) => p.models.map((m) => m.modelId));
    ...
    it('reports custom providerProtocol models under their resolved auth type', async () => {
        ...
        const result = await provider(workspace, false);
        const openaiProvider = result.providers.find((p) => p.authType === 'openai');
        ...
        expect(result.providers
            .flatMap((p) => p.models)
            .some((m) => m.modelId === 'ignored-model(openai)')).toBe(false);
    });
    ...
    it('does not mark baseUrl variants current when no baseUrl is resolved', async () => {
        ...
        expect(models.find((m) => m.baseUrl === 'https://proxy.example/v1')?.isCurrent).toBe(false);
    });
    it('uses the auth-specific env model when settings.model.name is absent', async () => {
        ...
        expect(result.providers
            .flatMap((p) => p.models)
            .find((m) => m.modelId === 'env-model(openai)')?.isCurrent).toBe(true);
    });
    ...
    it('does not include runtime models in the workspace provider catalog', async () => {
        ...
        expect(result.providers
            .flatMap((p) => p.models)
            .some((m) => m.modelId === 'runtime-only-model(openai)')).toBe(false);
    });
    ...
    async function writeUserSettings(settings) {
        await fs.writeFile(path.join(qwenHome, 'settings.json'), JSON.stringify(settings), 'utf8');
    }
});
...
//# sourceMappingURL=workspace-providers-status.test.js.map