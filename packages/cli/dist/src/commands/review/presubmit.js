...
import { gh, ghApiAll, ghApiAllNested, currentUser, ensureAuthenticated, isOwnerRepo, setGhHost, } from './lib/gh.js';
...
import { LEADING_INVISIBLE_RE, carriedClaimLine, severityOf, } from './lib/inline-counts.js';
import { carriesCommentMarker } from './lib/review-footer.js';
import { LEDGER_ID_READBACK, LEDGER_ID_SHAPE, LEDGER_ID_TOKEN, } from './lib/ledger.js';
/** The carried id this comment's claim line leads with, if any. */
function extractCarriedIds(body) {
...
function isCurrentActionsRunCheck(run) {
    ...
    return [run.details_url, run.html_url].some((url) => typeof url === 'string' && url.includes(runUrlMarker));
}
/**
 ...
 */
export function parseFindingsFile(path) {
    let parsed;
    try {
 ...
 */
export function classifyHeadDrift(reviewedSha, liveHeadSha, compare, findingPaths) {
    ...
    const fileCount = compare !== null && compare.filesTotal >= COMPARE_API_FILES_CAP
        ? `${COMPARE_API_FILES_CAP}+`
        : `${compare?.filesTotal ?? 0}`;
    const detail = compare === null
        ? ''
        : compare.status === 'diverged'
    ...
    return {
        ...
        downgradeReason: `PR head advanced during review: reviewed ${reviewedSha.slice(0, 8)}, ` +
            `PR is now at ${liveHeadSha.slice(0, 8)}${detail}`,
    };
}
export function classifyCi(checkRuns, statuses) {
    const failedCheckNames = [];
    let hasPending = false;
    const relevantCheckRuns = checkRuns.filter((run) => !isCurrentActionsRunCheck(run));
    ...
    for (const run of relevantCheckRuns) {
        if (run.status !== 'completed') {
            if (PENDING_STATES.has(run.status))
                hasPending = true;
            continue;
        }
        if (!run.conclusion || NOT_RUN_CONCLUSIONS.has(run.conclusion))
...
function classifyExistingComments(qwenComments, repliedToIds, newFindings, commitSha, currentUserLogin) {
    const buckets = { stale: [], resolved: [], overlap: [], repost: [], noConflict: [] };
    ...
    const carriedIdsByLocation = new Map();
    for (const f of newFindings) {
        if (f.id === undefined)
            continue;
        const key = `${f.path}:${f.line}`;
        const ids = carriedIdsByLocation.get(key) ?? new Set();
        ids.add(f.id);
        carriedIdsByLocation.set(key, ids);
    }
    // Own-account LailatulCoder comments per location at the current SHA. A count of
    ...
    const ownOverlapCountByLocation = new Map();
    if (currentUserLogin !== '') {
        for (const c of qwenComments) {
            if (c.commit_id === commitSha &&
                ...
                ownOverlapCountByLocation.set(key, (ownOverlapCountByLocation.get(key) ?? 0) + 1);
            }
        }
    }
    for (const c of qwenComments) {
        ...
        if (c.commit_id !== commitSha) {
            ...
            if (wantedIds &&
                currentUserLogin !== '' &&
                (c.user?.login ?? '').toLowerCase() === currentUserLogin.toLowerCase()) {
                const matchedIds = extractCarriedIds(c.body || '').filter((id) => wantedIds.has(id));
                if (matchedIds.length > 0) {
                    buckets.repost.push({ ...summary, matchedIds });
                }
                else if (!ANY_CARRIED_ID.test(c.body || '') &&
                    wantedIds.size === 1 &&
 ...
 */
function isSelfReview(author, me) {
...
async function runPresubmit(args) {
    ...
    try {
        prMeta = JSON.parse(gh('api', `repos/${owner}/${repo}/pulls/${prNumber}`, '--jq', '{author: .user.login, headSha: .head.sha}'));
    }
    catch {
    ...
    if (liveHeadSha && liveHeadSha !== commitSha) {
        try {
            const c = JSON.parse(gh('api', `repos/${owner}/${repo}/compare/${commitSha}...${liveHeadSha}`, 
            ...
            '--jq', '{status, aheadBy: .ahead_by, files: [.files[] | .filename, .previous_filename] | map(select(. != null))}'));
    ...
    const findingsFileInvalid = newFindingsPath !== undefined && newFindings === null;
    const { headDrift, downgradeReason: driftReason } = classifyHeadDrift(commitSha, liveHeadSha, compare, newFindings === null ? null : newFindings.map((f) => f.path));
    // --- CI status ---------------------------------------------------------
    ...
    const checkRuns = ghApiAllNested(`repos/${owner}/${repo}/commits/${commitSha}/check-runs`, 'check_runs');
    ...
    const statuses = ghApiAllNested(`repos/${owner}/${repo}/commits/${commitSha}/status`, 'statuses');
    ...
    // different trust levels. The visible footer is LailatulCoder's own provenance
    ...
    const qwenComments = allComments.filter((c) => /via LailatulCoder Ai \/review/.test(c.body ?? '') ||
        (!c.in_reply_to_id &&
            me !== '' &&
    ...
    const buckets = classifyExistingComments(qwenComments, repliedToIds, newFindings ?? [], commitSha, me);
    // --- Downgrade decisions ----------------------------------------------
    ...
    if (ciStatus.class === 'no_checks' && ciStatus.totalChecks > 0) {
        downgradeReasons.push(`CI did not run: every check was skipped (${ciStatus.skippedCheckNames.join(', ')})`);
    }
    if (driftReason)
        downgradeReasons.push(driftReason);
    if (metaUnavailable) {
        downgradeReasons.push('PR metadata unavailable — could not verify self-PR status or head drift');
    }
    if (findingsFileInvalid) {
        downgradeReasons.push('the --new-findings file was malformed — overlap dedup was disabled and ' +
 ...
 */
async function runAonePresubmit(args) {
    const { pr_number: prNumber, commit_sha: commitSha, owner_repo: ownerRepo, out_path: outPath, } = args;
    ...
    if (!/^[0-9]+$/.test(prNumber) ||
        ...
        Number(prNumber) <= 0) {
        throw new TypeError(`presubmit: pr_number must be a positive integer, got ${JSON.stringify(prNumber)}`);
    }
    ...
    let metaUnavailable = false;
    try {
        ({ author, headSha: liveHeadSha } = mrPresubmitFacts(Number(prNumber), ownerRepo));
    }
    catch {
        metaUnavailable = true;
    }
    ...
    const { headDrift, downgradeReason: driftReason } = classifyHeadDrift(commitSha, liveHeadSha, null, null);
    // --- Downgrade decisions ----------------------------------------------
    const downgradeReasons = [];
    if (isSelfPr)
        downgradeReasons.push('self-PR');
    if (driftReason)
        downgradeReasons.push(driftReason);
    if (metaUnavailable) {
        downgradeReasons.push('MR metadata unavailable — could not verify self-PR status or head drift');
    }
    const noComments = [];
    const result = {
        prNumber,
...
export const presubmitCommand = {
    ...
    builder: (yargs) => yargs
        ...
        type: 'string',
        demandOption: true,
        describe: 'Target coordinate: GitHub "owner/repo" or Aone "group/project"',
    })
        .positional('out_path', {
        type: 'string',
        demandOption: true,
