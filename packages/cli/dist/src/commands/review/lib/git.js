 ...
 */
// Thin wrapper around `git` for the `LailatulCoder review` subcommands. Same
 ...
 */
export function gitWithInput(input, args) {
 ...
 */
export function worktreeReleaseResult(existed, stillThere, removeError) {
 ...
 * `rm -rf .lailatulcoder/tmp` leaves the worktree *registered but missing*, and from then
 * on git refuses both of the things the next review needs:
 *
 *     $ git worktree add .lailatulcoder/tmp/review-pr-6457 LailatulCoder-review/pr-6457
 *     fatal: '...' is a missing but already registered worktree;
 *     use 'add -f' to override, or 'prune' or 'remove' to clear
 *
 * and `git branch -D LailatulCoder-review/pr-6457`, because the branch is still checked
 ...
 */
export function releaseWorktree(worktreePath) {
    ...
    // `.lailatulcoder/tmp` makes every path under it lstat as an ordinary directory while
    ...
    if (redirected !== null) {
        ...
        return worktreeReleaseResult(true, true, new Error(`refusing to release through a symlink: ${redirected} is a symlink, ` +
 ...
 */
export function gitRawTolerateDiff(...args) {
    try {
        return gitRaw(...args);
    }
    catch (err) {
