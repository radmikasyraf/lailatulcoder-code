...
import { parseRemoteUrl, matchRemotes, normalizeSegment, hostsEquivalent, isAoneCanonicalHost, } from './remote-match.js';
...
describe('matchRemotes', () => {
    ...
    it('strips an explicit port from the input host before comparing', () => {
        ...
        expect(matchRemotes(remotes, {
            owner: 'team',
            repo: 'repo',
            host: 'ghe.example.com:8443',
        }).matched).toEqual(['origin']);
    });
    ...
    it('matches a GHE remote only under its own host', () => {
        ...
        expect(matchRemotes(remotes, { owner: 'LailatulCoder', repo: 'lailatul-coder' }).matched).toEqual(['origin']);
    });
    ...
    it('matches an Aone CR-URL web host against a git-host remote', () => {
        ...
        const { matched } = matchRemotes('origin\tgit@gitlab.alibaba-inc.com:maxcompute/odps_src.git (fetch)\n', { owner: 'maxcompute', repo: 'odps_src', host: 'code.alibaba-inc.com' });
        expect(matched).toEqual(['origin']);
    });
    it("a nested-group target does NOT match a different group's same-named repo", () => {
        ...
        const remoteV = 'origin\tgit@gitlab.alibaba-inc.com:groupB/frontend/app.git (fetch)\n';
    ...
    it("a nested-group target matches its own group's remote", () => {
        const remoteV = 'origin\tgit@gitlab.alibaba-inc.com:groupA/frontend/app.git (fetch)\n';
        const { matched } = matchRemotes(remoteV, {
            owner: 'frontend',
            repo: 'app',
            host: 'code.alibaba-inc.com',
    ...
    it('a nested-group target does NOT match a two-segment remote', () => {
        ...
        // same-tail hazard).
        const remoteV = 'origin\tgit@gitlab.alibaba-inc.com:frontend/app.git (fetch)\n';
        const { matched } = matchRemotes(remoteV, {
            owner: 'frontend',
            repo: 'app',
            host: 'code.alibaba-inc.com',
...
describe('hostsEquivalent', () => {
    ...
    it('Aone web and git hosts are one equivalence class', () => {
        expect(hostsEquivalent('code.alibaba-inc.com', 'gitlab.alibaba-inc.com')).toBe(true);
        expect(hostsEquivalent('gitlab.alibaba-inc.com', 'code.alibaba-inc.com')).toBe(true);
    });
    ...
    it('equates the alias across spelling variants (port, dot, case)', () => {
        ...
        expect(hostsEquivalent('code.alibaba-inc.com:443', 'gitlab.alibaba-inc.com')).toBe(true);
        expect(hostsEquivalent('CODE.ALIBABA-INC.COM', 'gitlab.alibaba-inc.com.')).toBe(true);
...
//# sourceMappingURL=remote-match.test.js.map