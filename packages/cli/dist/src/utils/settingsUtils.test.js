...
import { 
// Schema utilities
getSettingDefinition, requiresRestart, getDefaultValue, getRestartRequiredSettings, getEffectiveValue, getAllSettingKeys, getDialogSettingKeys, 
// Business logic utilities
TEST_ONLY, settingExistsInScope, setPendingSettingValue, getRestartRequiredFromModified, getDisplayValue, isDefaultValue, setNestedPropertySafe, setNestedPropertyForce, validateSettingValue, } from './settingsUtils.js';
    ...
    describe('Schema Utilities', () => {
        ...
        describe('validateSettingValue', () => {
            ...
            it('rejects numbers below the configured minimum', () => {
                const definition = getSettingDefinition('numberWithMinimum');
                expect(definition).toBeDefined();
                expect(validateSettingValue(definition, -1)).toBe('Value must be >= 0');
            });
            it('rejects numbers above the configured maximum', () => {
                const definition = getSettingDefinition('numberWithMaximum');
                expect(definition).toBeDefined();
                expect(validateSettingValue(definition, 11)).toBe('Value must be <= 10');
            });
            it('validates integer settings', () => {
                ...
                expect(validateSettingValue(definition, 1.5)).toBe('Value must be an integer');
        ...
        describe('getDialogSettingKeys', () => {
            ...
            it('should handle nested settings display correctly', () => {
                ...
                const valueFromPending = getEffectiveValue(key, updatedPendingSettings, {});
    ...
    describe('Business Logic Utilities', () => {
        ...
        describe('getDisplayValue', () => {
            ...
            it('should display auto output language as following user input', () => {
                ...
                const result = getDisplayValue('general.outputLanguage', makeMockSettings({ general: { outputLanguage: 'auto' } }), makeMockSettings({ general: { outputLanguage: 'auto' } }), new Set());
