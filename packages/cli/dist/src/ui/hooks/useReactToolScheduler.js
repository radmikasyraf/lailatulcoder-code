 ...
 */
import { CoreToolScheduler, compactToolResultDisplayForHistory, convertToFunctionErrorResponse, createDebugLogger, getToolResponseDisplayText, isAnyAutoMemPath, isShellProgressData, ToolErrorType, } from '@lailatul-coder/lailatul-coder-core';
    ...
    const schedule = useCallback((request, signal, modelOverride) => {
        if (!modelOverride?.endsWith('\0')) {
            void scheduler.schedule(request, signal).catch((error) => {
                if (signal.aborted)
                    return;
                debugLogger.error(`Tool scheduling failed: ${error instanceof Error ? error.message : String(error)}`);
            });
            return;
        }
        void (async () => {
