...
import { presubmitCommand, classifyCi, classifyHeadDrift, parseFindingsFile, } from './presubmit.js';
...
describe('classifyCi — a skipped check is not a passing check', () => {
    ...
    it('calls it no_checks when checks exist and NOT ONE of them ran', () => {
        ...
        const got = classifyCi([run('Test (ubuntu-latest)', 'skipped'), run('Lint', 'skipped')], []);
        expect(got.class).toBe('no_checks');
        expect(got.totalChecks).toBe(2);
    });
    it('still fails on a real failure and waits on a real pending', () => {
        expect(classifyCi([run('Test', 'failure'), run('Lint', 'skipped')], []).class).toBe('any_failure');
        expect(classifyCi([run('Test', '', 'in_progress'), run('Lint', 'skipped')], [])
            .class).toBe('all_pending');
    });
    ...
    it('names a completed check that produced NO conclusion, instead of "skipped ()"', () => {
        ...
        const got = classifyCi([run('Ghost Check', '')], []);
    ...
    it('treats startup_failure as a failure, not a silent pass', () => {
        ...
        const got = classifyCi([run('Test', 'success'), run('E2E', 'startup_failure')], []);
    ...
    it('treats waiting and requested as pending, not skipped', () => {
        ...
        expect(classifyCi([run('E2E', null, 'waiting')], []).class).toBe('all_pending');
        expect(classifyCi([run('Lint', null, 'requested')], [])
            .class).toBe('all_pending');
    });
    it('dedupes a matrix job that fails on several platforms', () => {
        ...
        const got = classifyCi([run('Test', 'failure'), run('Test', 'failure'), run('Test', 'failure')], []);
...
describe('classifyCi — a superseded run does not outvote the latest verdict', () => {
    const at = (name, conclusion, completed_at, status = 'completed') => ({ name, status, conclusion, completed_at });
    ...
    it('a flaky job re-run to green is green — the failed attempt is history', () => {
        const got = classifyCi([
            at('Test (ubuntu-latest, Node 22.x)', 'failure', '2026-07-18T10:00:00Z'),
            at('Test (ubuntu-latest, Node 22.x)', 'success', '2026-07-18T11:00:00Z'),
        ], []);
        expect(got.class).toBe('all_pass');
    });
    ...
    it('a name whose ONLY run was cancelled still fails — nothing superseded it', () => {
        const got = classifyCi([at('E2E', 'cancelled', '2026-07-18T10:00:00Z')], []);
        expect(got.class).toBe('any_failure');
    ...
    it('with no timestamps at all, the first-listed run keeps the name — the API lists newest first', () => {
        const got = classifyCi([at('route', 'success', null), at('route', 'cancelled', null)], []);
        expect(got.class).toBe('all_pass');
    });
...
const { ghMock, ghApiMock, ghApiAllMock, ghApiAllNestedMock, currentUserMock, ensureAuthenticatedMock, setGhHostMock, readFileSyncMock, writeFileSyncMock, writeStdoutLineMock, detectPlatformKindMock, mrPresubmitFactsMock, ensureAoneAuthMock, } = vi.hoisted(() => ({
    ghMock: vi.fn(),
    ghApiMock: vi.fn(),
    ghApiAllMock: vi.fn(),
...
describe('presubmitCommand', () => {
    ...
    async function presubmitWithComments(comments, newFindings) {
        ...
        if (!handler)
            throw new Error('presubmit handler missing');
        await handler({
            ...baseArgs,
            'new-findings': '/tmp/findings.json',
        });
        const [, content] = writeFileSyncMock.mock.calls.find(([path]) => path === '/tmp/presubmit.json') ?? [null, null];
    ...
    it('sets downgradeApprove — not just a reason — when every check was skipped', async () => {
        ...
        ghApiAllNestedMock.mockImplementation((path) => path.endsWith('/check-runs')
            ? [
    ...
    it('downgrades the Approve and reports headDrift when the PR advanced mid-review', async () => {
        ...
        expect(result.downgradeApprove).toBe(true);
        expect(result.downgradeReasons.join(' ')).toContain('PR head advanced during review');
    });
    ...
    it('fails closed and caps the Approve when PR metadata cannot be read', async () => {
        ...
        const result = JSON.parse(String(content));
        expect(result.downgradeApprove).toBe(true);
        expect(result.downgradeReasons.join(' ')).toContain('PR metadata unavailable');
    });
    it('counts a renamed file by BOTH its new and previous path in the drift file set', async () => {
        ...
        ghApiMock.mockReturnValue(null);
        readFileSyncMock.mockImplementation((path) => String(path).includes('findings')
    ...
    it('surfaces a malformed findings file (flag + downgrade) even with no drift', async () => {
        ...
        ghMock.mockReturnValue('{"author":"contributor","headSha":"abc123"}');
        ghApiMock.mockReturnValue(null);
        readFileSyncMock.mockImplementation((path) => String(path).includes('findings') ? 'not json at all {' : '[]');
        const handler = presubmitCommand.handler;
        if (!handler)
            throw new Error('presubmit handler missing');
        await handler({
        ...
        expect(result.downgradeApprove).toBe(true);
        expect(result.downgradeReasons.join(' ')).toContain('the --new-findings file was malformed');
    });
        ...
        ghApiAllNestedMock.mockImplementation((path) => path.endsWith('/check-runs')
            ? [
                ...
                },
                {
                    ...
                    status: 'in_progress',
                    conclusion: null,
                    details_url: 'https://github.com/LailatulCoder/lailatul-coder/actions/runs/28788268483/job/85362025778',
                },
            ]
            : []);
        ghApiMock.mockReturnValue(null);
    ...
    describe('existing-comment classification — carried-id re-posts (#9208)', () => {
        ...
        it('marks an id-matched overlap comment as a re-post target', async () => {
            const result = await presubmitWithComments([CARRIED_COMMENT], [{ path: 'src/parse-args.ts', line: 44, id: 'R3-2' }]);
            expect(result.existingComments.byBucket.overlap).toBe(1);
        ...
        it('reports only the matched ids when several findings share the location', async () => {
            const result = await presubmitWithComments([CARRIED_COMMENT], [
        ...
        it("does not treat a different account's colliding id as a re-post", async () => {
            ...
            const result = await presubmitWithComments([{ ...CARRIED_COMMENT, user: { login: 'maintainer-dev' } }], [{ path: 'src/parse-args.ts', line: 44, id: 'R3-2' }]);
            expect(result.existingComments.byBucket.overlap).toBe(1);
        ...
        it('does not exempt a lone comment that only cross-references the id mid-body (#9212)', async () => {
            ...
            const result = await presubmitWithComments([referenced], [{ path: 'src/parse-args.ts', line: 44, id: 'R3-2' }]);
            expect(result.existingComments.byBucket.overlap).toBe(1);
            expect(result.existingComments.byBucket.repost).toBe(0);
        });
        it('does not match an id embedded in a longer hyphen run (#9212)', async () => {
            ...
            const result = await presubmitWithComments([extended], [{ path: 'src/parse-args.ts', line: 44, id: 'R3-2' }]);
            expect(result.existingComments.byBucket.overlap).toBe(1);
            expect(result.existingComments.byBucket.repost).toBe(0);
        });
        it('sees an id wrapped in Markdown emphasis as an id token (#9212)', async () => {
            ...
            const result = await presubmitWithComments([emphasised], [{ path: 'src/parse-args.ts', line: 44, id: 'R3-2' }]);
            expect(result.existingComments.byBucket.overlap).toBe(1);
            expect(result.existingComments.byBucket.repost).toBe(0);
        });
        ...
        it('keeps the strict match when the id-less target is ambiguous (#9208)', async () => {
            ...
            const result = await presubmitWithComments([first, second], [{ path: 'src/parse-args.ts', line: 44, id: 'R3-2' }]);
        ...
        it('matches a prior re-post as the target when it carries the id (#9212)', async () => {
            ...
            const result = await presubmitWithComments([original, priorRepost], [{ path: 'src/parse-args.ts', line: 44, id: 'R1-2' }]);
            expect(result.existingComments.byBucket.overlap).toBe(2);
            expect(result.existingComments.byBucket.repost).toBe(1);
        ...
        it('counts a replied-to original toward the id-less ambiguity decision (#9212)', async () => {
            ...
            const result = await presubmitWithComments([repliedOriginal, maintainerReply, siblingOriginal], [{ path: 'src/parse-args.ts', line: 44, id: 'R2-1' }]);
            expect(result.existingComments.byBucket.resolved).toBe(1);
            expect(result.existingComments.byBucket.overlap).toBe(1);
            expect(result.existingComments.byBucket.repost).toBe(0);
        });
        it('does not match a different carried id', async () => {
            const result = await presubmitWithComments([CARRIED_COMMENT], [{ path: 'src/parse-args.ts', line: 44, id: 'R3-9' }]);
            expect(result.existingComments.byBucket.overlap).toBe(1);
            expect(result.existingComments.byBucket.repost).toBe(0);
        });
        it('does not match an id carried at a different location', async () => {
            const result = await presubmitWithComments([{ ...CARRIED_COMMENT, line: 45 }], [{ path: 'src/parse-args.ts', line: 44, id: 'R3-2' }]);
...
describe('classifyHeadDrift', () => {
    ...
    it('names both SHAs even when the compare detail is unavailable, and fails anchors safe', () => {
        const got = classifyHeadDrift('57a9273ade45a43b9f16ae1f84cc3ba448a87429', '08ede5645612adca7d4193c1503d9c9e0f4387fb', null, []);
        expect(got.headDrift.drifted).toBe(true);
        expect(got.headDrift.compare).toBeNull();
        expect(got.headDrift.anchorsAtRisk).toBe(true);
        expect(got.downgradeReason).toBe('PR head advanced during review: reviewed 57a9273a, PR is now at 08ede564');
    });
    ...
    it('treats a `behind` force-push-to-earlier as drift with anchors at risk, not "+0 unreviewed"', () => {
        ...
        const got = classifyHeadDrift('sha-ahead', 'sha-earlier', { status: 'behind', aheadBy: 0, filesTouched: [], filesTotal: 0 }, ['src/z.ts']);
        expect(got.headDrift.drifted).toBe(true);
        expect(got.headDrift.anchorsAtRisk).toBe(true);
...
describe('presubmitCommand — Aone targets', () => {
    ...
    async function runAonePresubmit() {
        ...
        if (!handler)
            throw new Error('presubmit handler missing');
        await handler(aoneArgs);
        const [, content] = writeFileSyncMock.mock.calls.find(([path]) => path === '/tmp/presubmit-aone.json') ?? [null, null];
    ...
    it('fails CLOSED when mr view throws — caps the Approve and names it', async () => {
        ...
        expect(result.isSelfPr).toBe(false);
        expect(result.downgradeApprove).toBe(true);
        expect(result.downgradeReasons.join(' ')).toContain('MR metadata unavailable');
    ...
    it("fails with the gate's actionable error when whoami throws — no report", async () => {
        ...
        if (!handler)
            throw new Error('presubmit handler missing');
        await expect(handler(aoneArgs)).rejects.toThrow(/a1 CLI not found/);
...
//# sourceMappingURL=presubmit.test.js.map