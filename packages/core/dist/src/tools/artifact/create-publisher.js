 ...
 */
export function createArtifactPublisher(config) {
    const kind = config.getArtifactPublisherKind();
    switch (kind) {
        case 'host':
            return new HostPublisher(config.getArtifactHostConfig() ?? {
                uploadCommand: '',
                urlTemplate: '',
            });
        case 'oss':
            return new OssPublisher(config.getArtifactOssConfig() ?? { bucket: '', endpoint: '' });
        case 'local':
...
//# sourceMappingURL=create-publisher.js.map