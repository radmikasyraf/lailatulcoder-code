...
import { SkillError, SkillErrorCode, parseAllowedToolsField, parseModelField, parsePathsField, parseUserInvocableField, validateSkillName, } from './types.js';
import { parsePriorityField, validateConfig } from './skill-load.js';
import { validateSymlinkTarget } from './symlinkScope.js';
import { SkillActivationRegistry, splitConditionalSkills, } from './skill-activation.js';
import { createDebugLogger } from '../utils/debugLogger.js';
import { normalizeContent } from '../utils/textUtils.js';
import { expandHomeDir } from '../utils/paths.js';
import { QWEN_DIR, SKILL_PROVIDER_CONFIG_DIRS, Storage, } from '../config/storage.js';
...
export function watcherIgnored(filePath, stats) {
...
export class SkillManager {
    ...
    slashReloadSuppressed = false;
    parseErrors = new Map();
    watchers = new Map();
    watchStarted = false;
    refreshTimer = null;
    bundledSkillsDir;
    activationRegistry = null;
    constructor(config) {
        this.config = config;
        ...
        // chunk.
        this.bundledSkillsDir = path.join(resolveBundleDir(import.meta.url), 'bundled');
    }
    /**
    ...
    async notifyChangeListeners() {
        ...
        const withTimeout = (p) => {
            ...
            const timeoutPromise = new Promise((_, reject) => {
                timerId = setTimeout(() => reject(new Error(`listener timeout after ${TIMEOUT_MS}ms`)), TIMEOUT_MS);
        ...
        const results = await Promise.allSettled(Array.from(this.changeListeners).map((listener) => withTimeout(Promise.resolve().then(listener))));
        for (const result of results) {
            if (result.status === 'rejected') {
                debugLogger.warn('Skill change listener threw an error:', result.reason);
            }
        }
    }
    /**
    ...
    async loadSkill(name, level) {
        ...
        if (bundledSkill) {
            ...
            debugLogger.debug(`Skill ${name} not found at any level (checked: project, user, extension, bundled)`);
        }
        return bundledSkill;
    }
    /**
    ...
    async refreshCache() {
        ...
        for (let i = 0; i < settled.length; i++) {
            ...
            if (result.status === 'fulfilled') {
                ...
                debugLogger.warn(`Failed to load ${levels[i]} level skills:`, result.reason);
            }
        }
        ...
        this.activationRegistry = new SkillActivationRegistry(conditional, this.config.getProjectRoot(), (skill, pattern, error) => {
            this.parseErrors.set(`${skill.filePath}#paths[${pattern}]`, new SkillError(`Invalid glob in "paths": ${pattern} — ${error.message}`, SkillErrorCode.INVALID_CONFIG, skill.name));
        });
     ...
     * be observed.
     */
    async matchAndActivateByPath(filePath) {
     ...
     */
    async matchAndActivateByPaths(filePaths) {
        const registry = this.activationRegistry;
        if (!registry || filePaths.length === 0)
            return [];
        const newlyAcrossPaths = new Set();
    ...
    async startWatching() {
        ...
        if (this.config.getBareMode()) {
            debugLogger.info('Bare mode enabled; refreshing skill cache without starting watchers');
            await this.refreshCache();
            return;
        }
    ...
    parseSkillContent(content, filePath, level) {
        try {
            ...
            const argumentHint = typeof frontmatter['argument-hint'] === 'string'
                ...
                : undefined;
            const whenToUse = typeof frontmatter['when_to_use'] === 'string'
            ...
            const priority = parsePriorityField(frontmatter, filePath, (msg) => debugLogger.warn(msg));
            const config = {
                name,
                description,
                allowedTools,
     ...
     */
    parseHookMatcher(matcher) {
        const matcherPattern = matcher['matcher'];
        const hooksRaw = matcher['hooks'];
    ...
    getSkillsBaseDirs(level) {
        switch (level) {
            case 'project':
                return SKILL_PROVIDER_CONFIG_DIRS.map((v) => path.join(this.config.getProjectRoot(), v, SKILLS_CONFIG_DIR));
            case 'user': {
                ...
                const dirs = SKILL_PROVIDER_CONFIG_DIRS.map((v) => path.resolve(v === QWEN_DIR
                ...
                for (const customDir of this.config.getCustomSkillDirs?.() ?? []) {
                    ...
                    if (!path.isAbsolute(homeExpanded)) {
                        debugLogger.warn(`Custom skill directory "${customDir}" is relative; ` +
    ...
    async listSkillsAtLevel(level) {
        ...
        if (level === 'extension') {
            ...
            for (const extension of extensions) {
                extension.skills?.forEach((skill) => {
                    ...
                    const hasInvalidPriority = skill.priority !== undefined &&
                        (typeof skill.priority !== 'number' ||
                            !Number.isFinite(skill.priority));
                    if (hasInvalidPriority) {
                        debugLogger.warn(`Extension "${extension.name}" skill "${skill.name}" has invalid priority (${typeof skill.priority}: ${String(skill.priority)}); treating as 0.`);
                    }
                    skills.push({
                        ...skill,
        ...
        if (level === 'bundled') {
            ...
            if (!fsSync.existsSync(bundledDir)) {
                debugLogger.warn(`Bundled skills directory not found: ${bundledDir}. This may indicate an incomplete installation.`);
                return [];
            }
        ...
        // so the order defines implicit precedence (.lailatulcoder > .agent > .cursor > ...).
        // Load in parallel but fold sequentially to preserve precedence.
        const baseDirs = this.getSkillsBaseDirs(level);
        const perDirSkills = await Promise.all(baseDirs.map((baseDir) => {
        ...
        for (let i = 0; i < baseDirs.length; i++) {
            ...
            for (const skill of perDirSkills[i]) {
                if (seenNames.has(skill.name)) {
                    debugLogger.debug(`Skipping duplicate skill at ${level} level: ${skill.name} from ${baseDir}`);
                    continue;
                }
                seenNames.add(skill.name);
                skills.push(skill);
                ...
                if (isSymlink) {
                    ...
                    if (!check.ok) {
                        if (check.reason === 'not-directory') {
                            ...
                            debugLogger.warn(`Skipping invalid symlink ${entry.name}: ${check.error instanceof Error ? check.error.message : 'Unknown error'}`);
                        }
                        return null;
                    }
                }
