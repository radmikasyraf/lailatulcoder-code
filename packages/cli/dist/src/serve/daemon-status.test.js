...
import { buildDaemonStatusResponse, } from './daemon-status.js';
...
describe('buildDaemonStatusResponse', () => {
    it('uses one logger snapshot and exposes summary/full log diagnostics', async () => {
        ...
        const summary = await buildDaemonStatusResponse('summary', makeOptions({ daemonLog }));
        ...
        getStatus.mockClear();
        const full = await buildDaemonStatusResponse('full', makeOptions({ daemonLog }));
        expect(getStatus).toHaveBeenCalledOnce();
    ...
    it('rolls degraded logger health into a path-free warning', async () => {
        ...
        const response = await buildDaemonStatusResponse('summary', makeOptions({ daemonLog }));
    ...
    it('sums only the children that actually reported, and says how many did', async () => {
        ...
        expect(response.runtime.memory.children.sampled).toBeLessThan(response.runtime.memory.activeAcpChildren);
    });
    it('reports a zero age as zero, and ages a mixed-contract sum by the ones that can', async () => {
        ...
        const fresh = await buildDaemonStatusResponse('summary', withRegistry([freshBridge]));
        ...
        const mixed = await buildDaemonStatusResponse('summary', withRegistry([olderContract, aged]));
    ...
    it('reports the worst child heap peak, not the sum of them', async () => {
        ...
        const withHeap = (rss, peak, live, gc) => ({
            getDaemonStatusSnapshot: () => BASE_BRIDGE_SNAPSHOT,
            isChannelLive: () => true,
            getChildResourceSnapshot: () => ({
                rssBytes: rss,
    ...
    it('counts only the children that reported a heap block', async () => {
        ...
        expect(response.runtime.memory?.children.heap?.unclassifiedSpaceNames).toEqual(['some_future_v8_space']);
    });
    ...
    it('reports pressure figures in both modes, but only observe raises an issue', async () => {
        ...
        const responses = await Promise.all(['off', 'observe'].map((mode) => {
            const options = makeOptions();
            options.opts.daemonMemoryBudget = resolveDaemonMemoryBudget({
                availableMemoryMb: 1,
            });
        ...
        for (const response of responses) {
            ...
            expect(response.runtime.memory?.pressure.availableBytes).toBe(1024 * 1024);
        }
        ...
        const pressureIssues = (r) => r.issues.filter((issue) => issue.code === 'daemon_memory_pressure');
        ...
        expect(pressureIssues(observeResponse)[0].code).toBe('daemon_memory_pressure');
        expect(Object.keys(observeResponse.runtime.memory.pressure).sort()).toEqual([
            'availableBytes',
            'heapLimitBytes',
            'heapRatio',
            'heapUsedBytes',
        ...
        expect(pressureIssues(observeResponse)[0].message).toContain('of available memory');
    ...
    it('raises nothing on a healthy daemon, and a warning once pressure leaves normal', async () => {
        ...
        expect(healthyResponse.issues.filter((issue) => issue.code === 'daemon_memory_pressure')).toHaveLength(0);
        ...
        const softResponse = await buildDaemonStatusResponse('summary', softOptions);
        expect(softResponse.runtime.memory?.pressure.level).toBe('soft');
        expect(softResponse.issues.filter((issue) => issue.code === 'daemon_memory_pressure')).toHaveLength(1);
    });
    ...
    it('converts the budget from megabytes when computing the ratio', async () => {
        ...
        expect(pressure.rssRatio).toBeCloseTo(pressure.rssBytes / (4_096 * 1024 * 1024), 10);
    });
    ...
    it('still takes one snapshot per bridge when a memory budget is resolved', async () => {
        ...
        options.workspaceRegistry = {
            ...
            list: () => runtimes,
            listManaged: () => runtimes,
            listEntries: () => runtimes.map((r) => ({ workspaceCwd: r.workspaceCwd })),
        };
    ...
    it('reports aggregate workspace-attributed ACP diagnostics in full status', async () => {
        ...
        const secondaryDiagnostic = makeAcpDiagnostic('secondary-id', '/work/secondary', false);
        ...
        const response = await buildDaemonStatusResponse('full', makeOptions({
        ...
        expect(response).toMatchObject({
            ...
            issues: expect.arrayContaining([
                expect.objectContaining({
                    ...
                    severity: 'error',
                    message: 'Channel worker is failed (pid=1234, restarts=2, lastExitAt=2026-07-01T01:00:00.000Z, lastRestartAt=2026-07-01T01:00:05.000Z, lastHeartbeatAt=2026-07-01T00:59:50.000Z): ipc failed.',
    ...
    it('reports and diagnoses non-primary channel workers', async () => {
        ...
        const response = await buildDaemonStatusResponse('summary', options);
        expect(response.runtime.channelWorkers).toEqual(options.getChannelWorkerSnapshots());
        expect(response).toMatchObject({
            status: 'error',
            issues: expect.arrayContaining([
                expect.objectContaining({
    ...
    it('omits channelWorkers for single-workspace and empty multi-workspace snapshots', async () => {
        ...
        expect((await buildDaemonStatusResponse('summary', single)).runtime
            .channelWorkers).toBeUndefined();
        ...
        expect((await buildDaemonStatusResponse('summary', multi)).runtime
            .channelWorkers).toBeUndefined();
    });
        ...
        expect(response).toMatchObject({
            ...
            issues: expect.arrayContaining([
                expect.objectContaining({
                    code: 'channel_worker_exited',
                    severity: 'warning',
                    message: 'Channel worker is failed (restarts=1, nextRestartAt=2026-07-01T01:01:00.000Z): restart failed.',
                    section: 'runtime.channelWorker',
                }),
            ]),
        });
        ...
        expect(response).toMatchObject({
            ...
            issues: expect.arrayContaining([
                expect.objectContaining({
                    code: 'channel_worker_partial_connect',
                    severity: 'warning',
                    message: 'Channel worker connected 1/3 channel(s). Failed: feishu, dingtalk.',
                    section: 'runtime.channelWorker',
                }),
            ]),
            runtime: {
    ...
    it('reports MCP budget warning and exhausted issue codes', async () => {
        const warning = await buildDaemonStatusResponse('full', makeOptions({
            mcpStatus: {
                v: 1,
                workspaceCwd: BASE_WORKSPACE,
                initialized: true,
        ...
        const exhausted = await buildDaemonStatusResponse('full', makeOptions({
            mcpStatus: {
                v: 1,
                workspaceCwd: BASE_WORKSPACE,
                initialized: true,
    ...
    it('marks a timed-out full workspace section unavailable', async () => {
        vi.useFakeTimers();
        const pending = buildDaemonStatusResponse('full', makeOptions({
    ...
    it('reports null activity when daemon has never been active', async () => {
        const response = await buildDaemonStatusResponse('summary', makeOptions({ activePromptCount: 0, lastActivityAt: null }));
        expect(response.runtime.activity).toEqual({
            activePrompts: 0,
            pendingPrompts: 0,
            queuedPrompts: 0,
...
function makeAcpDiagnostic(workspaceId, workspaceCwd, primary) {
    return {
...
function makeOptions(input = {}) {
    ...
    const bridge = {
        ...
        getWorkspaceToolsStatus: async () => input.toolsStatus ?? okStatus({ tools: [] }),
        activePromptCount: input.activePromptCount ?? 0,
        pendingPromptTotal: input.pendingPromptTotal,
        lastActivityAt: input.lastActivityAt ?? null,
    };
    const workspace = {
        getWorkspaceMcpStatus: async () => input.mcpStatus ?? okStatus({ servers: [] }),
        ...
        getWorkspaceHooksStatus: async () => input.hooksStatus ?? okStatus({ hooks: [], events: {} }),
        getWorkspaceExtensionsStatus: async () => input.extensionsStatus ?? okStatus({ extensions: [] }),
    };
    return {
        opts: {
            hostname: '127.0.0.1',
        ...
        ...(input.acpSnapshot
            ? {
                acpHandle: {
                    ...
                    getSnapshot: () => input.acpAggregate ?? {
                        ...
                        bufferedConnectionFrames: input.acpSnapshot.bufferedConnectionFrames,
                        ...
                        mounts: [
                            {
                                ...
                                connectionCount: input.acpSnapshot.connectionCount,
                                wsStreams: input.acpSnapshot.wsStreams,
                                preAttachGuardFailures: input.acpSnapshot.preAttachGuardFailures,
                            },
                        ],
                        connections: [],
                    },
        ...
        ...(input.totalAdmissionInFlight === undefined
            ...
            : {
                getTotalSessionAdmissionSnapshot: () => ({
                    liveCount: input.totalAdmissionLiveCount ??
...
//# sourceMappingURL=daemon-status.test.js.map