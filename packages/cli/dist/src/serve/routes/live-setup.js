 ...
 */
import { LiveSetupError, } from '../live/live-setup-controller.js';
...
function parseApiKeyMutation(value) {
    if (typeof value !== 'object' || value === null || Array.isArray(value)) {
        throw new LiveSetupError('apiKey must be an explicit replace or clear operation.', 'invalid_live_api_key', 400);
    }
    const record = value;
...
function parseUpdate(body) {
    ...
    if (body['enabled'] !== undefined) {
        if (typeof body['enabled'] !== 'boolean') {
            throw new LiveSetupError('enabled must be a boolean.', 'invalid_live_enabled', 400);
        }
        update.enabled = body['enabled'];
    }
    if (body['shortcut'] !== undefined) {
        if (typeof body['shortcut'] !== 'string' || body['shortcut'].length > 128) {
            throw new LiveSetupError('shortcut must be an Electron accelerator or an empty string.', 'invalid_live_shortcut', 400);
        }
    ...
    if (Object.keys(update).length === 0) {
        throw new LiveSetupError('At least one Live Voice setting is required.', 'empty_live_setup_update', 400);
    }
    return update;
}
export function registerLiveSetupRoutes(app, deps) {
    ...
    app.post('/live/setup/install', deps.mutate({ strict: true }), async (_req, res) => {
        try {
    ...
    app.post('/live/setup/launch', deps.mutate({ strict: true }), async (_req, res) => {
        try {
...
//# sourceMappingURL=live-setup.js.map