...
import { buildSubagentPlanToolBlockedResult, isPlanLifecycleToolUnavailableInSubagent, } from '../agents/runtime/subagent-plan-tool-policy.js';
...
const enterPlanModeToolSchemaData = {
...
class EnterPlanModeToolInvocation extends BaseToolInvocation {
    ...
    async execute(_signal) {
        if (isPlanLifecycleToolUnavailableInSubagent(ToolNames.ENTER_PLAN_MODE)) {
            return buildSubagentPlanToolBlockedResult(ToolNames.ENTER_PLAN_MODE, 'EnterPlanModeTool', debugLogger);
        }
        ...
        if (this.config.getApprovalMode() === ApprovalMode.YOLO &&
            ...
            debugLogger.info('Blocked model-initiated plan entry from YOLO (userRequested=%s)', this.params.userRequested);
            return {
        ...
        const isAcpMode = this.config.getExperimentalZedIntegration?.() ||
        ...
        try {
            ...
            debugLogger.error(`[EnterPlanModeTool] Failed to set approval mode to plan: ${errorMessage}`);
            return {
        ...
        try {
            ...
            if (!revealedBefore) {
                ...
                if (geminiClient) {
                    try {
                        ...
                        debugLogger.error(`[EnterPlanModeTool] Failed to sync exit_plan_mode tool declaration: ${setErr instanceof Error ? setErr.message : String(setErr)}`);
                    }
                }
            }
        }
            ...
            debugLogger.warn(`[EnterPlanModeTool] Failed to reveal exit_plan_mode: ${error instanceof Error ? error.message : String(error)}`);
        }
        return {
...
export class EnterPlanModeTool extends BaseDeclarativeTool {
    ...
    constructor(config) {
        super(EnterPlanModeTool.Name, ToolDisplayNames.ENTER_PLAN_MODE, enterPlanModeToolDescription, Kind.Think, enterPlanModeToolSchemaData.parametersJsonSchema, true, // isOutputMarkdown
...
//# sourceMappingURL=enterPlanMode.js.map