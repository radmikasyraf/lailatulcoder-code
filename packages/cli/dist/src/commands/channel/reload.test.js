...
const mockDaemonClient = vi.hoisted(() => vi.fn(() => ({ reloadChannelWorker: mockReloadChannelWorker })));
...
describe('channel reload command', () => {
    ...
    it('reloads via the resolved daemon URL/token and prints the snapshot', async () => {
        ...
        const line = mockWriteStdoutLine.mock.calls[0]?.[0];
    ...
    it('reports failures on stderr and exits non-zero', async () => {
        ...
        const line = mockWriteStderrLine.mock.calls[0]?.[0];
    ...
    it('prints partial startup failures from a successful reload', async () => {
        ...
        await runHandler({});
        expect(mockWriteStdoutLine).toHaveBeenCalledWith('[Channel] Startup failure (channel=feishu, phase=connect): connection refused');
    });
    it('prints structured startup failures from a failed reload body', async () => {
        mockReloadChannelWorker.mockRejectedValue(Object.assign(new Error('reload failed'), {
            body: {
        ...
        await runHandler({});
        expect(mockWriteStderrLine).toHaveBeenCalledWith('[Channel] Startup failure (workspace=/work, channel=telegram, phase=connect, code=ECONNREFUSED): connection refused');
    });
});
