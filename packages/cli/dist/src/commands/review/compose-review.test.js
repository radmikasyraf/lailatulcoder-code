...
import { budgetStopEntry, budgetStopEntryZh, roundCapStopEntry, roundCapStopEntryZh, writeBudgetStop, writeRoundCapStop, } from './lib/deadline.js';
import { getGhHost, setGhHost } from './lib/gh.js';
import { BRIEFS } from './lib/agent-briefs.js';
import { LEDGER_MAX_FILE, LEDGER_MAX_ROUND, LEDGER_MAX_VOLUME, parseLedger, serializeLedger, } from './lib/ledger.js';
import { countInlineFindings } from './lib/inline-counts.js';
import { composeReview, isNonDiffDimensionGap, buildLedger, repositoryContextGate, scriptLintGate, testPlanGate, composeReviewCommand, describeChunkGap, verdictLine, } from './compose-review.js';
...
const reviewSettingsMock = vi.hoisted(() => vi.fn(() => ({})));
...
const runComposeReviewCommand = (argv) => Promise.resolve(composeReviewCommand.handler(argv));
const ghMock = vi.hoisted(() => vi.fn((..._args) => ''));
...
function transcript(id, launchPrompt, opts = {}) {
    ...
    writeFileSync(join(dir, 'subagents', 'S1', `agent-${id}.jsonl`), lines.join('\n') + '\n');
}
/**
 ...
 */
function rehomeToPriorSession(planPath, file) {
    mkdirSync(join(dir, 'subagents', 'S0'), { recursive: true });
    const from = join(dir, 'subagents', 'S1', file);
    writeFileSync(join(dir, 'subagents', 'S0', file), readFileSync(from, 'utf8').replaceAll('"sessionId":"S1"', '"sessionId":"S0"'));
...
function recordBuilt(planPath, chunk) {
    const d = promptRecordDir(planPath);
    mkdirSync(d, { recursive: true });
 ...
 */
function recordMatrix(planPath) {
    const d = promptRecordDir(planPath);
    mkdirSync(d, { recursive: true });
 ...
 */
function coveredPlan(step45Keys = ['verify', 'reverse-audit'], planOpts = {}) {
...
function findingsFile(content) {
    const f = join(dir, 'qwen-review-findings.md');
    writeFileSync(f, content);
    return f;
}
const TAGGED = '- **File:** src/pay.ts:42\n' +
...
describe('composeReview — the C/S table', () => {
    ...
    it('includes the injected CLI version without breaking the stable marker', () => {
        ...
        expect(r.body.endsWith(`_— ${MODEL} via LailatulCoder Ai /review (v0.21.2)_`)).toBe(true);
    });
    ...
    it('attribution off: a footer-unsafe modelId composes — nothing renders it', () => {
        const r = composeReview(base({ modelId: 'evil\nvia LailatulCoder Ai /review' }), '0.21.2', false);
        expect(r.body).toBe('No issues found. LGTM! ✅');
    });
    ...
    it('attribution on: a missing modelId is still refused', () => {
        expect(() => composeReview(base({ modelId: '' }), '0.21.2')).toThrow(/modelId is required/);
    });
    it('attribution on: a footer-unsafe modelId is still refused', () => {
        expect(() => composeReview(base({ modelId: 'evil\nmodel' }), '0.21.2')).toThrow(/single line/);
    });
    it('C=0, S≥1 → COMMENT with the no-blockers opener', () => {
        const r = composeReview(base({ suggestionsInline: 2 }));
        expect(r.event).toBe('COMMENT');
        expect(r.body).toBe(`Reviewed — no blockers. Suggestions are inline.\n\n${FOOTER}`);
    });
    ...
    it('refuses a body Critical that renders as nothing', () => {
        ...
        expect(() => composeReview(base({ bodyCriticals: ['**[Critical]**'] }), '0.21.2', false)).toThrow(/renders as nothing/);
        expect(() => composeReview(base({ bodyCriticals: ['**[Critical]**'] }))).toThrow(/renders as nothing/);
    });
    it('refuses a body Critical held up only by a forged footer past the caps', () => {
        ...
        expect(() => composeReview(base({ bodyCriticals: [forged] }))).toThrow(/renders as nothing/);
    });
    it('refuses a cannot-tell entry a forged footer past the caps reduces to nothing', () => {
        ...
        expect(() => composeReview(base({ cannotTellCriticals: [forged] }))).toThrow(/renders as nothing/);
    });
    ...
    it('refuses a cannot-tell entry that is only a forged footer split by a blank line', () => {
        ...
        expect(() => composeReview(base({ cannotTellCriticals: ['_— m\n\nvia LailatulCoder Ai /review (v1)_'] }), '0.21.2', false)).toThrow(/renders as nothing/);
    });
    ...
    it('refuses an entry that strips to an unterminated HTML comment', () => {
        ...
        // both modes.
        expect(() => composeReview(base({ bodyCriticals: ['**[Critical]** <!-- x'] }), '0.21.2', false)).toThrow(/renders as nothing/);
        expect(() => composeReview(base({ bodyCriticals: ['**[Critical]** <!-- x'] }))).toThrow(/renders as nothing/);
    });
    it('refuses an entry whose lines are code-fence delimiters', () => {
        ...
        expect(() => composeReview(base({ bodyCriticals: ['~~~\nconst x = await db.query()\n~~~'] }), '0.21.2', false)).toThrow(/code fence/);
        expect(() => composeReview(base({ bodyCriticals: ['```js\nconst x = 1\n```'] }))).toThrow(/code fence/);
        expect(() => composeReview(base({
...
describe('composeReview — modeled-system defect-layer cap', () => {
    ...
    const IDENTITY = 'You are review agent `reverse-audit` — Reverse audit agent.';
    const ALL = [
        'lexing',
        'expansion',
        'scope-propagation',
    ...
    const walked = (...ids) => ids.map((id) => `Layer walked: ${id} — clear.`).join('\n');
    ...
    const auditor = (id, receipts) => {
        const planPath = join(dir, 'plan.json');
        const brief = briefPath(planPath, 'reverse-audit');
        const launch = 'You are review agent `reverse-audit`.\n' +
            `read_file(file_path="${brief}")\n` +
            `read_file(file_path="${DIFF}")`;
    ...
    const markedPlan = (domains) => coveredPlan(['verify', 'reverse-audit'], {
        repositoryContext: sentinel(domains),
    });
    const compose = (p) => composeReview({
        criticalsInline: 0,
        suggestionsInline: 0,
        planPath: p,
        env: ENV,
    ...
    it('does not count a verifier whose prompt merely mentions reverse-audit', () => {
        ...
        transcript('vr', `You are review agent \`verify\` — Verification agent, ruling on reverse-audit findings.\nread_file(file_path="${DIFF}")`, { toolCalls: 1, range: [0, 100], text: walked(...ALL) });
    ...
    it('does not count an auditor whose diff read misses its baked territory', () => {
        ...
        transcript('ra-off', `${IDENTITY}\nread_file(file_path="${DIFF}", offset=3300, limit=700)`, { toolCalls: 1, range: [0, 50], text: walked(...ALL) });
        expect(compose(p).event).toBe('COMMENT');
    });
...
describe('composeReview — the low-signal Approve disclosure', () => {
    ...
    it('a zero-finding APPROVE over a non-trivial source diff carries the marker — event and body unchanged', () => {
        ...
        expect(verdictLine(r)).toBe('Verdict: Approve — low signal: none of the 3 review agents reported ' +
...
describe('repository context proof boundary', () => {
    ...
    it('caps the unverified-dimension disclosure at five entries', () => {
        ...
        writeFileSync(planPath, JSON.stringify({
            repositoryContext: {
                ...
                requiredConfigurations: [],
                requiredAgents: [],
                unverifiedDimensions: Array.from({ length: 8 }, (_, index) => `dimension ${index}`),
                verificationNotes: [],
            },
        }));
        expect(repositoryContextGate(planPath)).toEqual([
            ...Array.from({ length: 5 }, (_, index) => `\`dimension ${index}\` — the repository context marks this proof boundary as unverified`),
            'and 3 more',
        ]);
    });
    ...
    it('returns nothing for an unreadable plan but fails closed on a malformed context', () => {
        ...
        writeFileSync(malformed, JSON.stringify({ repositoryContext: { version: 1 } }));
        expect(() => repositoryContextGate(malformed)).toThrow('unknown or missing fields');
    });
...
describe('composeReview — event caps (round-7 Critical #2: caps must reach every path)', () => {
    it('a cannot-tell existing Critical caps APPROVE at COMMENT and is serialized (round-7: body said Unresolved while event said APPROVE)', () => {
        const r = composeReview(base({ cannotTellCriticals: ['SKILL.md:35 — full text unfetchable'] }));
        expect(r.event).toBe('COMMENT');
    ...
    it('an unreviewed dimension caps APPROVE at COMMENT (round-7 Critical #3: zero findings + whiffed Security must not LGTM)', () => {
        ...
        expect(r.event).toBe('COMMENT');
        expect(r.body).toContain('Not reviewed: security — the agent returned no evidence of its walk twice.');
        expect(r.body).not.toContain('LGTM');
        expect(r.body).not.toContain('no blockers');
    });
    ...
    it('a budget-stop marker caps APPROVE at COMMENT with nothing relayed by the caller', () => {
        ...
        const r = composeReview(base({ planPath: plan }));
        expect(r.event).toBe('COMMENT');
        expect(r.body).toContain('reverse audit — stopped before round 4 by the review time budget');
        expect(r.body).not.toContain('LGTM');
    ...
    it('a free-form disclosure that mentions the budget still reaches the body', () => {
        ...
        const freeForm = 'security — the review time budget ended the round before the security relaunch returned evidence';
    ...
    it('the marker does not shadow other reverse-audit scopes the caller disclosed', () => {
        ...
        expect(r.body).toContain('Not reviewed: reverse audit — stopped before round 3 by the review time budget.');
        expect(r.body).toContain("Not reviewed: reverse audit — chunk 2's auditor returned nothing substantive twice.");
    ...
    it('a round-1 budget stop stands alone — no rogue-audit gap, no rebuild FIX', () => {
        ...
        expect(r.event).toBe('COMMENT');
        expect(r.body).toContain('Not reviewed: reverse audit — stopped before round 1 by the review time budget.');
    ...
    it('renders the budget stop bilingually on a Han-description PR', () => {
        ...
        const r = composeReview({ planPath: plan, env: ENV, modelId: MODEL });
        expect(r.body).toContain('Not reviewed: reverse audit — stopped before round 4 by the review time budget.');
        expect(r.body).toContain('未审查：反向审计——评审时间预算不足，未能开始第 4 轮。');
    });
    it('a budget stop does not launder a rewritten pre-stop round', () => {
        ...
        const built = 'You are review agent `reverse-audit`.\n' +
            `read_file(file_path="${brief}")\n` +
            `read_file(file_path="${DIFF}")`;
        writeFileSync(join(d, 'reverse-audit.txt'), built);
        transcript('v-ra-rewritten', `Audit the diff for gaps. Your brief: ${brief}. Diff: ${DIFF}.`, { toolCalls: 2, opens: [brief] });
        writeBudgetStop(plan, {
            remainingSeconds: 900,
            reserveSeconds: 3600,
            expectedRoundSeconds: 1800,
        ...
        expect(r.body).toContain('stopped before round 2 by the review time budget');
    ...
    it('an uncoverable chunk caps APPROVE at COMMENT and names the chunk', () => {
        const r = composeReview(base({ uncoverableChunks: ['chunk 5 (src/big.min.js)'] }));
        expect(r.event).toBe('COMMENT');
    ...
    it('a Suggestion-only COMMENT with a cap loses the certifying opener', () => {
        const r = composeReview(base({ suggestionsInline: 1, unreviewedDimensions: ['security'] }));
        expect(r.event).toBe('COMMENT');
        ...
        expect(r.body).toContain('Partially reviewed — gaps disclosed. Suggestions are inline.');
        expect(r.body).not.toContain('no blockers');
    });
});
describe('composeReview — context-unavailable (clause 2)', () => {
    ...
    it('suggestion-only stays non-certifying under clause 2 with no duplicate opener', () => {
        const r = composeReview(base({ suggestionsInline: 2, contextUnavailable: true }));
        expect(r.event).toBe('COMMENT');
        expect(r.body).toContain('Reviewed diff-only');
    ...
    it('discloses coverage gaps before the diff-only warning', () => {
        const r = composeReview(base({ contextUnavailable: true, unreviewedDimensions: ['security'] }));
        expect(r.body.indexOf('Partially reviewed')).toBeLessThan(r.body.indexOf('Reviewed diff-only'));
    });
    it('does not soften a REQUEST_CHANGES', () => {
        const r = composeReview(base({ criticalsInline: 1, contextUnavailable: true }));
        expect(r.event).toBe('REQUEST_CHANGES');
    });
});
describe('composeReview — 422 recovery (round-7 Critical #1 & round-6: verdict never upgrades)', () => {
    it('all Suggestions discarded on resubmit stays COMMENT, never APPROVE (round-6: Suggestion-only flipped to LGTM)', () => {
        ...
        expect(r.body).toContain('2 Suggestion-level finding(s) could not be anchored to a changed line and were dropped; nothing further to act on here.');
    ...
    it('mixed inline/discarded Suggestions carries both sentences', () => {
        const r = composeReview(base({ suggestionsInline: 1, suggestionsDiscarded: 1 }));
        expect(r.body).toContain('Suggestions are inline.');
    ...
    it('a relocated Critical keeps REQUEST_CHANGES with the blocker as the body', () => {
        const r = composeReview(base({ bodyCriticals: ['relocated after 422'], suggestionsInline: 1 }));
        expect(r.event).toBe('REQUEST_CHANGES');
...
describe('composeReview — duplicate-dropped Suggestions (#9204: the body claimed an anchor failure that never happened)', () => {
    it('an all-duplicates run stays COMMENT with the duplicate sentence, never the anchor-failure one', () => {
        ...
        expect(r.event).toBe('COMMENT');
        expect(r.event).not.toBe('APPROVE');
        expect(r.body).toContain('3 Suggestion-level finding(s) this review confirmed are already reported on this PR and are not repeated:');
    ...
    it('mixed inline/duplicate Suggestions carries the inline sentence and the duplicate paragraph', () => {
        ...
        expect(r.event).toBe('COMMENT');
        expect(r.body).toContain('Suggestions are inline.');
        expect(r.body).toContain('1 Suggestion-level finding(s) this review confirmed');
    });
    ...
    it('links bare comment ids in duplicate entries to their GitHub anchors when the plan names the PR', () => {
        ...
        expect(r.event).toBe('COMMENT');
        expect(r.body).toContain('[comment 3788857375](https://github.com/LailatulCoder/lailatul-coder/pull/9204#discussion_r3788857375)');
    });
    ...
    it('refuses a fence delimiter a bare CR hides in the raw entry', () => {
        ...
        for (const field of ['bodyCriticals', 'cannotTellCriticals']) {
            expect(() => composeReview(base({
    ...
    it('attribution off strips markers and forged footers from duplicate entries too', () => {
        ...
        expect(r.body).toContain('- R1-2 loose pins — already reported (comment 42)');
        ...
        const attributed = composeReview(base({
            suggestionsDroppedAsDuplicates: [
                '**[Suggestion]** R1-2 loose pins — already reported (comment 42)',
            ],
        }));
        expect(attributed.body).toContain('- **[Suggestion]** R1-2 loose pins — already reported (comment 42)');
    });
    ...
    it('rejects a non-string entry', () => {
        expect(() => composeReview(base({
            suggestionsDroppedAsDuplicates: [1],
    ...
    it('a Critical beside duplicate drops keeps REQUEST_CHANGES and carries the duplicate account', () => {
        ...
        expect(r.event).toBe('REQUEST_CHANGES');
        expect(r.body).toContain('**[Critical]** whole-PR blocker X');
        expect(r.body).toContain('2 Suggestion-level finding(s) this review confirmed are already reported on this PR and are not repeated:');
        expect(r.body).toContain('- R1-1 pin gap — already reported (comment 3788857375)');
    });
    ...
    it('caps the rendered list at the deferred line cap and keeps the count truthful with an overflow item', () => {
        const entry = (i) => `R1-${i} finding — already reported (comment 378885${String(i).padStart(5, '0')})`;
        ...
        expect(r.body).toContain('25 Suggestion-level finding(s) this review confirmed are already reported on this PR and are not repeated:');
        ...
        const atCap = composeReview(base({ suggestionsDroppedAsDuplicates: dropped.slice(0, 20) }));
        expect(atCap.body).toContain('20 Suggestion-level finding(s) this review confirmed');
...
describe('composeReview — presubmit downgrades', () => {
    it('downgradeApprove turns a clean APPROVE into COMMENT with the downgrade sentence', () => {
        ...
        expect(r.body).toContain('⚠️ Downgraded from Approve to Comment: self-PR; CI still running.');
    });
...
describe('composeReview — stacked states compose, none erased', () => {
    ...
    it('reads as a sentence when no role was briefed at all', () => {
        ...
        const gap = 'every dimension — none of the 12 required agents was launched with a ' +
...
describe('composeReview — RC carries every applicable disclosure (no clause squeezed out)', () => {
    ...
    it('RC + uncoverable chunk alone still discloses the unread scope (was gated on other parts)', () => {
        const r = composeReview(base({ criticalsInline: 1, uncoverableChunks: ['chunk 3 (a.min.js)'] }));
        expect(r.event).toBe('REQUEST_CHANGES');
    ...
    it('RC + cannot-tell existing Critical carries the unresolved disclosure', () => {
        const r = composeReview(base({ criticalsInline: 1, cannotTellCriticals: ['old blocker'] }));
        expect(r.event).toBe('REQUEST_CHANGES');
        expect(r.body).toContain('Unresolved, please confirm:');
    });
...
describe('composeReview — not-reviewed entries that carry their own reason', () => {
    it('renders the entry verbatim instead of appending the whiff sentence (Agent 0 issue-fetch failure)', () => {
        ...
        expect(r.event).toBe('COMMENT');
        expect(r.body).toContain('Not reviewed: security — the agent returned no evidence of its walk twice.');
        expect(r.body).toContain('Not reviewed: issue-fidelity — linked issue #123 could not be fetched.');
...
describe('composeReview — budget-gap disclosures (a channel, never a cap)', () => {
    it('renders disclosed gaps in the body and still approves a clean run', () => {
        ...
        transcript('a1', goodPrompt(1), {
            toolCalls: 3,
            range: [0, 100],
            text: 'No issues found — walked chunk 1 fully.\n' +
        ...
        // the body inert.
        expect(r.body).toContain('Not explored to full depth (tool budget reached): ' +
    ...
    it('drops its mechanical line for a gap the caller promoted — one register, not two', () => {
        ...
        expect(r.body).toContain('Not reviewed: second-order callers of the renamed export — stopped at the agent tool budget.');
...
describe('composeReview — input validation (the producer is a model that omits inapplicable fields)', () => {
    ...
    it('rejects negative, fractional, NaN, and non-number counts with the field name', () => {
        expect(() => composeReview({ criticalsInline: -1, modelId: MODEL })).toThrow(/criticalsInline/);
        expect(() => composeReview({ criticalsInline: 1.5, modelId: MODEL })).toThrow(/criticalsInline/);
        expect(() => composeReview({ suggestionsDiscarded: Number.NaN, modelId: MODEL })).toThrow(/suggestionsDiscarded/);
        expect(() => composeReview({
            suggestionsInline: '2',
            modelId: MODEL,
    ...
    it('accepts the array form of suggestionsDiscarded, counting it by length', () => {
        ...
        expect(composeReview(base({ suggestionsDiscarded: [] })).event).toBe('APPROVE');
        const r = composeReview(base({
    ...
    it('rejects a non-array list field and a missing or blank modelId', () => {
        ...
        expect(() => composeReview({})).toThrow(/modelId/);
    ...
    it('rejects a modelId that would forge the footer it is interpolated into', () => {
        ...
        expect(() => composeReview({ modelId: 'model via LailatulCoder Ai /review x' })).toThrow(/modelId/);
    });
    ...
    it('rejects stringified booleans — "false" is truthy and once flipped events and published false warnings', () => {
        ...
        expect(() => composeReview(base({
            presubmit: { downgradeApprove: 'false' },
        }))).toThrow(/presubmit\.downgradeApprove/);
        expect(() => composeReview(base({ contextUnavailable: 'false' }))).toThrow(/contextUnavailable/);
    });
...
describe('composeReviewCommand handler (the CLI glue)', () => {
    ...
    afterEach(() => {
        if (savedStartupVersion === undefined)
            ...
            process.env['QWEN_CODE_STARTUP_VERSION'] = savedStartupVersion;
    });
    it('reads --input, counts the drafted comments, and writes the result JSON to --out', async () => {
        ...
        expect(written.body.endsWith(`_— ${MODEL} via LailatulCoder Ai /review (v0.21.2)_`)).toBe(true);
    });
    /**
    ...
    async function composeWithRecordedFloor(opts) {
        ...
        try {
            ...
            if (savedSession === undefined)
                ...
                process.env['QWEN_CODE_SESSION_ID'] = savedSession;
            rmSync(dir, { recursive: true, force: true });
        }
        const stderr = writeStderrLine.mock.calls.map((c) => String(c[0]));
        return {
            written,
            stderrHasOverride: stderr.some((l) => l.includes('verbatim record outranks')),
            stderr,
        };
    }
    ...
    it('falls back to --pr when the state carries no usable plan — the R4-1 symmetry', async () => {
        ...
        expect(stderr.some((l) => l.includes('the recorded `--severity-floor` flag'))).toBe(true);
    });
    it('binds a URL-shaped GHE record with the caller identity — full submit symmetry', async () => {
        ...
        try {
            const { written } = await composeWithRecordedFloor({
                stateFloor: 'suggestion',
                argsLine: 'https://ghe.corp.example/LailatulCoder/lailatul-coder/pull/8255 --severity-floor critical',
                noPlan: true,
                pr: 8255,
    ...
    it("refuses a foreign repo's URL record — the handler's --repo is the bar", async () => {
        ...
        for (const repo of ['LailatulCoder/lailatul-coder', undefined]) {
            const { written, stderrHasOverride } = await composeWithRecordedFloor({
                stateFloor: 'suggestion',
                argsLine: 'https://github.com/other/repo/pull/8255 --severity-floor critical',
                noPlan: true,
                pr: 8255,
                repo,
            });
    ...
    it('names the setting as the source when no flag was typed', async () => {
        ...
        expect(written.floorEnforced).toEqual([0]);
        expect(stderr.some((l) => l.includes('setting resolved against the recorded invocation'))).toBe(true);
        expect(stderr.some((l) => l.includes('the recorded `--severity-floor` flag'))).toBe(false);
    });
    ...
    it('prints the convergence paragraph the trim notice points at', async () => {
        ...
        const planPath = join(dir, 'plan.json');
        writeFileSync(planPath, JSON.stringify({ prNumber: 8255 }), 'utf8');
        writeFileSync(inputPath, JSON.stringify({ modelId: MODEL, planPath }), 'utf8');
        writeFileSync(commentsPath, JSON.stringify([
            { path: 'src/a.ts', line: 3, body: '**[Suggestion]** again' },
        ]), 'utf8');
        writeFileSync(join(dir, 'qwen-review-pr-8255-prev-ledger.json'), JSON.stringify({
            v: 1,
            round: 4,
            posted: 9,
            fresh: 9,
        ...
        try {
            ...
            const lines = writeStderrLine.mock.calls.map((c) => String(c[0]));
            expect(lines.some((l) => l.startsWith('CONVERGENCE: Convergence:'))).toBe(true);
        }
        finally {
            rmSync(dir, { recursive: true, force: true });
        }
    ...
    it('states the round volume on stderr, with and without a predecessor', async () => {
        ...
        try {
            ...
            expect(stderr()).toContain('VOLUME: 2 inline comment(s) this round (2 reported for the first time)');
            // With a recorded predecessor the previous round rides along.
            writeStderrLine.mockClear();
            writeFileSync(join(dir, 'qwen-review-pr-8255-prev-ledger.json'), JSON.stringify({ v: 1, round: 4, findings: [], posted: 9 }), 'utf8');
            await runComposeReviewCommand({
                input: inputPath,
                comments: commentsPath,
            });
            expect(stderr()).toContain('VOLUME: 2 inline comment(s) this round (2 reported for the first time) (previous round: 9)');
            ...
            writeStderrLine.mockClear();
            writeFileSync(join(dir, 'qwen-review-pr-8255-prev-ledger.json'), JSON.stringify({ v: 1, round: 5, findings: [], posted: 0 }), 'utf8');
            await runComposeReviewCommand({
                input: inputPath,
                comments: commentsPath,
            });
            expect(stderr()).toContain('VOLUME: 2 inline comment(s) this round (2 reported for the first time) (previous round: 0)');
        }
        finally {
            rmSync(dir, { recursive: true, force: true });
        }
    ...
    it('pins the persisted footer to the inherited startup version, not the resolved one', async () => {
        ...
        try {
            ...
            const written = JSON.parse(readFileSync(outPath, 'utf8'));
            expect(written.body.endsWith(`_— ${MODEL} via LailatulCoder Ai /review (v0.21.1)_`)).toBe(true);
        }
        finally {
            if (inherited === undefined)
                delete process.env['QWEN_CODE_STARTUP_VERSION'];
            else
                process.env['QWEN_CODE_STARTUP_VERSION'] = inherited;
            rmSync(dir, { recursive: true, force: true });
        }
    });
    ...
    it('a drafted inline Critical reaches the verdict line — the report-only hole', async () => {
        ...
        try {
            ...
            expect(written.verdictLine).toContain('a Request changes was NOT available');
        }
        finally {
            rmSync(dir, { recursive: true, force: true });
        }
    ...
    it('accepts the review-payload shape too — the same file submit takes', async () => {
        ...
        try {
            ...
            expect(JSON.parse(readFileSync(outPath, 'utf8'))
                .baseEvent).toBe('REQUEST_CHANGES');
        }
        finally {
            rmSync(dir, { recursive: true, force: true });
        }
    ...
    it('carries duplicate-dropped Suggestions through the --input seam', async () => {
        ...
        try {
            ...
            const written = JSON.parse(readFileSync(outPath, 'utf8'));
            expect(written.body).toContain('1 Suggestion-level finding(s) this review confirmed');
    ...
    ])('refuses a state JSON carrying %s — counts are counted, not typed', async (_, extra) => {
        const dir = mkdtempSync(join(tmpdir(), 'compose-typed-count-'));
        try {
            const inputPath = join(dir, 'compose.json');
            const commentsPath = join(dir, 'comments.json');
            writeFileSync(inputPath, JSON.stringify({ modelId: MODEL, ...extra }), 'utf8');
            writeFileSync(commentsPath, '[]', 'utf8');
            await expect(runComposeReviewCommand({
                input: inputPath,
                comments: commentsPath,
    ...
    ])('refuses %s — omission is the failure mode, not a default', async (_, commentsPath, pattern) => {
    ...
    it('strips a model-supplied `env` — it cannot redirect the transcript lookup', async () => {
        ...
        try {
            ...
            const good = 'You are reviewing chunk 1 of 1.\nread_file(file_path="/d.txt", offset=0, limit=10)';
            const b = {
                agentId: 'f1',
                agentName: 'general-purpose',
                sessionId: 'S1',
            };
            writeFileSync(join(fdir, 'agent-f1.jsonl'), [
                JSON.stringify({
                    ...b,
                    type: 'user',
            ...
            try {
                ...
                if (prevProj === undefined)
                    ...
                    process.env['QWEN_CODE_PROJECT_DIR'] = prevProj;
            }
            const written = JSON.parse(readFileSync(outPath, 'utf8'));
...
describe('coverage is recomputed, never accepted', () => {
    it('does not repeat a disclosure the caller echoed back — one subject, one line', () => {
        ...
        expect(r.body).toContain('a subject only the caller noticed — the auditor returned nothing twice');
    });
    it('says a shared cause once, with every subject on the one sentence', () => {
        ...
        recordBuilt(p, 1);
        recordBuilt(p, 2);
        transcript('a1', `You are reviewing chunk 1 of 2.\nread_file(file_path="${DIFF}", offset=0, limit=100)`, { toolCalls: 2 });
        transcript('a2', `You are reviewing chunk 2 of 2.\nread_file(file_path="${DIFF}", offset=100, limit=100)`, { toolCalls: 2 });
        ...
        expect(r.body).toMatch(new RegExp(`Not reviewed: the entire diff — ${reason}\\.`));
    ...
    it('an all-rewritten roster never claims nothing launched — precise cause, no contradicting aggregate', () => {
        ...
        transcript('a2', `You are reviewing chunk 2 of 2.\nread_file(file_path="${DIFF}", offset=100, limit=100)`, { toolCalls: 2 });
        const r = composeReview({ planPath: p, env: ENV, modelId: MODEL });
        expect(r.body).toMatch(/Not reviewed: the entire diff — launched with a prompt that is not the one the CLI built\./);
    ...
    it('caller echoes of per-role gaps fold into the one grouped sentence — the #7188 shape end to end', () => {
        ...
        expect(r.body).not.toContain(label);
        expect(r.body.split('the whole-diff test-coverage check')).toHaveLength(2);
        expect(r.body.match(/no record shows its brief reaching an agent/g) ?? []).toHaveLength(1);
    });
    ...
    it('opens with the zero-certified warning when every chunk is disclosed — never "Reviewed." above a body that denies it', () => {
        ...
        expect(r.event).toBe('COMMENT');
        expect(r.body).toMatch(/^⚠️ This run could not certify that any of this diff was reviewed\./);
        expect(r.body).toContain('Suggestions are inline.');
    ...
    it('caps when the agents made no tool call — whatever their prose said', () => {
        ...
        expect(r.remediation.join(' ')).toMatch(/idle agents: relaunch each with the same printed prompt/);
    });
    it('quotes a prose agent label — it is the agent’s name, not a claim about the PR', () => {
        ...
        const launch = 'This PR narrows the daemon-marker check from a truthy test to an exact one\n' +
            `read_file(file_path="${brief}")\n` +
        ...
        expect(r.body).toContain('Not reviewed: `"This PR narrows the daemon-marker check from a truthy test…"`');
        expect(r.body).not.toContain('truthy tes...');
        expect(r.body).toContain('启动 prompt 为它指定了 diff 中的行，但它从未打开');
    });
    it('keeps long agent labels distinct when their first word matches', () => {
        ...
        transcript('p2', `Verify the daemon marker rename does not break linux-build behavior\n${DIFF}`);
        const r = composeReview({ planPath: plan(), env: ENV, modelId: MODEL });
    ...
    it('renders prompt-derived labels as inert Markdown', () => {
        transcript('p1', `Fix the "daemon marker" regression for @owner from #123\n${DIFF}`);
        const r = composeReview({ planPath: plan(), env: ENV, modelId: MODEL });
        expect(r.body).toContain('Not reviewed: `"Fix the \\"daemon marker\\" regression for @owner from #123"`');
    });
    it('collapses spaces after removing backticks from agent labels', () => {
        transcript('p1', `Inspect the \`auth\` and \`session\` paths\n${DIFF}`);
        const r = composeReview({ planPath: plan(), env: ENV, modelId: MODEL });
        expect(r.body).toContain('Not reviewed: `"Inspect the auth and session paths"`');
    });
    it('labels an agent by its brief codename wherever it sits in the prompt', () => {
        ...
        transcript('p1', `PR #9045 modifies getAuthTypeFromEnv().\nYou are review agent \`security\` — inspect auth\n${DIFF}`);
        const r = composeReview({ planPath: plan(), env: ENV, modelId: MODEL });
    ...
    it('names a blind launch as itself, not as a whiff', () => {
        ...
        expect(r.remediation.join(' ')).toContain('"${QWEN_CODE_CLI:-qwen}" review agent-prompt');
        ...
        expect(r.body).not.toContain('no agent reported covering');
        expect(r.remediation.join(' ')).toMatch(/chunks nobody read: build each with/);
    });
    ...
    it('rewritten, unread-brief and never-opened gaps each carry their FIX too', () => {
        ...
        transcript('a1', `You are reviewing chunk 1 of 2.\n` +
            `read_file(file_path="${DIFF}", offset=0, limit=100)`, { toolCalls: 3 });
    ...
    it('the handler prints every FIX to stderr, before the verdict, never to stdout', async () => {
        ...
        try {
            ...
            if (prevDir === undefined)
                ...
                process.env['QWEN_CODE_PROJECT_DIR'] = prevDir;
            if (prevSession === undefined)
                delete process.env['QWEN_CODE_SESSION_ID'];
            else
                process.env['QWEN_CODE_SESSION_ID'] = prevSession;
        }
    });
...
describe('the Step 4/5 gate — verify and reverse audit must have run (high effort)', () => {
    it('caps a clean APPROVE to COMMENT when the reverse audit never ran', () => {
        ...
        expect(r.body).toMatch(/reverse audit — no auditor was launched with a prompt this skill builds/);
    });
    it('does not require the reverse audit at medium effort — a by-design Comment cap, no FIX line', () => {
        ...
        expect(r.body).toContain('the balanced (medium) tier skips the second-look pass');
        expect(r.body).not.toMatch(/no auditor was launched with a prompt this skill builds/);
    ...
    it('says one sentence when verify and the reverse audit failed the same way', () => {
        ...
        expect(r.event).toBe('COMMENT');
        expect(r.body).toMatch(/Not reviewed: verification and reverse audit — neither the verifier nor the reverse auditor was launched with a prompt this skill builds/);
    ...
    it('softens an unverified Request changes to Comment — no verifier, no blocker', () => {
        ...
        expect(verdictLine(r)).toBe('Verdict: Comment — a Request changes was NOT available: its blockers ' +
    ...
    it('keeps the presubmit downgrade reasons when the unverified cap also holds', () => {
        ...
        expect(r.event).toBe('COMMENT');
        expect(r.body).toContain('Downgraded from Request changes to Comment: self-PR');
    ...
    it('keeps the presubmit downgrade reasons when the findings-tag cap also holds', () => {
        ...
        expect(r.body).toContain('Downgraded from Request changes to Comment: self-PR');
        expect(verdictLine(r)).toContain('findings were still unverified when the loop ended');
    });
    ...
    it('requires a verifier for a body Critical that is not pre-confirmed', () => {
        ...
        expect(r.cappedBy).toContain('criticals-unverified');
        expect(r.body).toMatch(/verification — the review posts findings/);
        expect(r.body).toContain('**[Critical]** a real blocker that could not be anchored');
    });
...
describe('verdictLine — the terminal verdict, and its dangling colon', () => {
    ...
    it('names a cap that took an Approve away', () => {
        ...
        })).toBe('Verdict: Comment — an Approve was NOT available: a dimension nobody reviewed');
    });
    it('does not leave a dangling colon when a downgrade ALONE took the Approve', () => {
        ...
        expect(out).toBe('Verdict: Comment — an Approve was NOT available: a presubmit check failed');
    ...
    it('lists a cap AND a downgrade together when both took the Approve', () => {
        expect(line({
            ...
            downgraded: true,
            downgradedFrom: 'Approve',
        })).toBe('Verdict: Comment — an Approve was NOT available: part of the diff cannot be read at all; a presubmit check failed');
    });
    ...
    it('is bare for a clean Approve', () => {
        expect(line({ event: 'APPROVE', baseEvent: 'APPROVE' })).toBe('Verdict: Approve');
    });
    it("marks a low-signal Approve, with the run's own numbers", () => {
        ...
        })).toBe('Verdict: Approve — low signal: none of the 11 review agents reported ' +
...
describe('bilingual body — the PR author writes Chinese (prDescriptionHasHan)', () => {
    it('folds the complete Chinese version under the English body, footer outside the fold', () => {
        ...
        expect(r.body.startsWith('Reviewed — no blockers. Suggestions are inline.')).toBe(true);
    ...
    it('translates the disclosures — role phrase and Not-reviewed frame', () => {
        ...
        recordBuilt(p, 2);
        const r = composeReview({ planPath: p, env: ENV, modelId: MODEL });
        expect(r.body).toContain('Not reviewed: the whole-diff test-coverage check');
    ...
    it('keeps the untranslatable unresolved list in the English half; the Chinese half points at it', () => {
        ...
        expect(r.body.match(/old blocker at a\.ts:1 — still reachable\?/g) ?? []).toHaveLength(1);
        expect(r.body).toContain('未决，请确认：共 1 条');
        expect(r.body.indexOf('old blocker at a.ts:1')).toBeLessThan(r.body.indexOf('<details>'));
    });
});
/**
...
describe('bilingual body — recovered from the live PR when the plan omits the flag', () => {
    ...
    it('stays English when the recovered description has no Han', () => {
        const fetch = recordingFetcher('This PR lazy-loads first-use dependencies.');
        const r = composeReview({
            suggestionsInline: 1,
            planPath: namedPlanWithoutFlag(),
            prBodyFetcher: fetch,
    ...
    it('the production reader calls gh pr view with the right args and parses the body', () => {
        ...
        expect(ghMock).toHaveBeenCalledWith('pr', 'view', '7686', '--repo', 'LailatulCoder/lailatul-coder', '--json', 'body');
        expect(r.body).toContain('<details>\n<summary>中文说明</summary>');
    });
...
describe('scriptLintGate — the deterministic gate reads the report', () => {
    ...
    function writeReport(report, name = 'qwen-review-script-lint.json') {
...
describe('composeReview — the script-lint gate wired to the verdict', () => {
    ...
    function gateReadyPlan(step45Keys = ['verify', 'reverse-audit']) {
    ...
    function writeGateReport(report) {
        writeFileSync(join(dir, 'qwen-review-script-lint.json'), JSON.stringify({
            checked: [],
            skipped: [],
            errored: [],
            ok: true,
    ...
    it('a [probe] in a GATE finding text does not erase a model claim’s verification (identity, not count)', () => {
        ...
        expect(r.body).toMatch(/verification — the review posts findings/);
        expect(r.body).toContain('an unanchored blocker the review could not verify');
    });
...
describe('testPlanGate — Test Plan rulings, disclosed but never capping', () => {
    ...
    const writePlan = (over = {}) => {
        const p = join(dir, 'plan.json');
        writeFileSync(p, JSON.stringify({ prNumber: 1, diffPathAbsolute: diffPath, ...over }));
        return p;
    };
    const writeReport = (claims, over = {}, name = 'qwen-review-pr-1-test-plan.json') => writeFileSync(join(dir, name), JSON.stringify({ found: true, claims, diffHash, note: '', ...over }));
    ...
    it('caps notes at five plus a summary line', () => {
        const p = writePlan();
        writeReport(Array.from({ length: 8 }, (_, i) => ({
            kind: 'count',
            text: `${i + 1} passed`,
            verdict: 'differs',
            observed: '999 passed',
...
describe('buildLedger', () => {
    it('gives a text-less finding a locating title instead of an empty one', () => {
        ...
        // one handle there is.
        const l = buildLedger(2, [{ path: 'packages/cli/src/a.ts', line: 42, body: '**[Critical]**' }], ['   ']);
        ...
        expect(buildLedger(2, [{ path: 'a.ts', line: 1, body: '**[Critical]** real claim' }], []).findings[0].title).toBe('real claim');
    });
    it('numbers findings round-scoped, inline first then body Criticals', () => {
        const l = buildLedger(3, [
            {
                path: 'src/a.ts',
                line: 12,
    ...
    it('flags the real file spelled like a stand-in, not the stand-in', () => {
        ...
        const standIn = buildLedger(2, [{ line: 3, body: '**[Suggestion]** arrived without a path' }], []);
        ...
        for (const name of ['(unknown)', '(body)']) {
            const real = buildLedger(2, [{ path: name, line: 3, body: '**[Suggestion]** a real file' }], []);
...
describe('the ledger marker reaches the POSTED body', () => {
    ...
    it('counts the round from the side file pr-context recovered, +1', () => {
        writeFileSync(join(dir, 'qwen-review-pr-8255-prev-ledger.json'), JSON.stringify({ v: 1, round: 4, findings: [] }));
        const r = composeReview({
            planPath: plan(),
            modelId: 'm',
            criticalsInline: 0,
    ...
    it('a provider-qualified identity is what gets certified, verbatim', () => {
        ...
        }, 'unknown', true, 'qwen3.7-max@1a2b3c4d');
    ...
    it('attribution off WITH a runtime identity: the session model stays withheld', () => {
        ...
        }, 'unknown', false, 'the-session-model');
        const ledger = parseLedger(r.body);
        expect(ledger.sha).toBe('deadbeef00112233');
        expect(ledger.model).toBeUndefined();
    ...
    it('the anchor carries the RUNTIME identity — injected at the CLI boundary, superseding the typed id', () => {
        ...
        }, 'unknown', true, 'the-session-model');
        const ledger = parseLedger(r.body);
        expect(ledger.sha).toBe('deadbeef00112233');
        expect(ledger.model).toBe('the-session-model');
    });
    ...
    it('a whitespace-only entry fails the draft instead of vanishing', () => {
        ...
        for (const field of ['bodyCriticals', 'cannotTellCriticals']) {
            for (const entry of ['', ' \n ']) {
                expect(() => composeReview(base({ [field]: [entry] }))).toThrow(/renders as nothing/);
            }
        }
    });
    ...
    it('classifies a budget stop the same whether or not the entry is relayed', () => {
        ...
        // already prove read.
        const composeWith = (dims) => {
            const planPath = coveredPlan(['verify', 'reverse-audit'], {
                prNumber: 8255,
                fetchedSha: 'deadbeef00112233',
            });
            writeBudgetStop(planPath, { remainingSeconds: 10, reserveSeconds: 300, expectedRoundSeconds: 60 }, 3);
    ...
    it('keeps the marker round-trip whole AT the round cap', () => {
        ...
        writeFileSync(join(dir, 'qwen-review-pr-8255-prev-ledger.json'), JSON.stringify({ v: 1, round: LEDGER_MAX_ROUND, findings: [] }));
        const r = composeReview({
            planPath: plan(),
            modelId: 'm',
            criticalsInline: 0,
    ...
    it("sees a debt the deterministic gates push in AFTER the caller's entries", () => {
        ...
        expect(isNonDiffDimensionGap('the executable-script lint — no report')).toBe(false);
        expect(isNonDiffDimensionGap('reverse-audit layer coverage — 2 layers unwalked')).toBe(false);
        // ...and the only entry that IS exempt stays exempt.
        expect(isNonDiffDimensionGap('build-and-test — the integration suite never ran')).toBe(true);
    });
    it('stays tied to the briefs: every readsDiff flag round-trips the exemption', () => {
        ...
        expect(Object.fromEntries(Object.values(BRIEFS).map((b) => [
            b.publicLabel,
            isNonDiffDimensionGap(`${b.publicLabel} — some reason`),
        ]))).toEqual(Object.fromEntries(Object.values(BRIEFS).map((b) => [b.publicLabel, !b.readsDiff])));
        ...
        expect(Object.fromEntries(variants.map((v) => [v, isNonDiffDimensionGap(v)]))).toEqual(Object.fromEntries(variants.map((v) => [v, true])));
...
describe('composeReview — convergence-posture deferrals (typed channel; disclosed, never capping)', () => {
    ...
    it('an APPROVE with deferrals keeps its event, anchor, and honesty', () => {
        ...
        writeFileSync(join(dirname(planPath), 'qwen-review-pr-8255-prev-ledger.json'), JSON.stringify({ v: 1, round: 5, findings: [] }));
        const r = composeReview({
            planPath,
            env: ENV,
            modelId: MODEL,
        ...
        expect(r.body).toContain('- `src/a.ts:42 — [review] tighten the retry backoff`');
        expect(parseLedger(r.body)?.sha).toBe('deadbeef00112233');
    ...
    it('names the round AT the ledger cap — the clause and the marker agree', () => {
        ...
        writeFileSync(join(dirname(planPath), 'qwen-review-pr-8255-prev-ledger.json'), JSON.stringify({ v: 1, round: LEDGER_MAX_ROUND, findings: [] }));
        const r = composeReview({
            planPath,
            env: ENV,
            modelId: MODEL,
        ...
        expect(r.body).toContain(`convergence posture (round ${LEDGER_MAX_ROUND}, not a blocker)`);
    ...
    it('renders the list on COMMENT and REQUEST_CHANGES alike — no event squeezes it out', () => {
        const comment = composeReview(base({
            suggestionsInline: 1,
    ...
    it('deferrals cast no vote on the event — an all-deferred run is not a Suggestion run', () => {
        ...
        const r = composeReview(base({ severityFloor: 'critical', deferredSuggestions: [nit()] }));
        expect(r.baseEvent).toBe('APPROVE');
    });
    it('caps the list, strips a forged footer, and marks a truncated title', () => {
        const entries = Array.from({ length: 23 }, (_, i) => nit({ file: `f${i}.ts`, title: `nit ${i}` }));
        ...
        const r = composeReview(base({ severityFloor: 'critical', deferredSuggestions: entries }));
        ...
        expect(verdictLine(r)).toContain('listed in the body, truncated — the rest are counted in the run report');
    ...
    it('exactly at the line cap, the verdict line does not claim truncation', () => {
        const entries = Array.from({ length: 20 }, (_, i) => nit({ file: `f${i}.ts`, title: `n${i}` }));
        const r = composeReview(base({ severityFloor: 'critical', deferredSuggestions: entries }));
    ...
    it('a deferrals-only APPROVE is not low signal, and the verdict line names the deferrals', () => {
        ...
        expect(verdictLine(r)).toBe('Verdict: Approve — 1 non-Critical finding(s) deferred under the convergence posture (listed in the body)');
    });
    it('deferred findings count toward the verifier-delivery floor — deterministic sources excepted', () => {
        ...
        for (const source of ['build', 'test', 'probe']) {
            ...
            expect(det.body).toContain(`- \`packages/core/src/my-file.ts:42 (+2 locations) — [${source}] mutation survivor\``);
        }
    });
    it('relocates a Critical entry into the body Criticals — never a throw, never deferred', () => {
        ...
        expect(r.body).toContain('**[Critical]** `src/auth.ts:88 — [review] auth bypass` _(relocated from the deferral channel');
    ...
    it('refuses a malformed entry — the channel that un-posts findings is not guessed at', () => {
        ...
        for (const [entry, re] of cases) {
            expect(() => composeReview(base({ deferredSuggestions: [entry] }))).toThrow(re);
        }
        expect(() => composeReview(base({ deferredSuggestions: 'a.ts' }))).toThrow(/deferredSuggestions/);
    });
    it('caps — never refuses — deferrals the posture does not license', () => {
        ...
        const explicitOff = composeReview(base({ severityFloor: 'suggestion', deferredSuggestions: [nit()] }));
        ...
        const round1Auto = composeReview(base({ severityFloor: 'auto', deferredSuggestions: [nit()] }));
        expect(round1Auto.cappedBy).toContain('unlicensed-deferral');
        expect(verdictLine(round1Auto)).toContain('findings were deferred without a posture licence');
        ...
        const noContext = composeReview(base({
            severityFloor: 'auto',
            contextUnavailable: true,
            deferredSuggestions: [nit()],
        }));
    ...
    it('an unrecognised severityFloor is unknown — never a throw', () => {
        ...
        const withList = composeReview(base({ severityFloor: 'blocker', deferredSuggestions: [nit()] }));
        expect(withList.cappedBy).toContain('unlicensed-deferral');
        const inert = composeReview(base({ severityFloor: 'blocker' }));
        expect(inert.event).toBe('APPROVE');
        expect(inert.cappedBy).toEqual([]);
        const cased = composeReview(base({
            severityFloor: ' Critical ',
            deferredSuggestions: [nit()],
        }));
    ...
    it('auto with a recovered previous round licenses the age-rule deferral', () => {
        ...
        writeFileSync(join(dirname(planPath), 'qwen-review-pr-8255-prev-ledger.json'), JSON.stringify({ v: 1, round: 2, findings: [] }));
        const r = composeReview({
            planPath,
            env: ENV,
            modelId: MODEL,
...
describe("composeReview — the composed body fits GitHub's limit", () => {
    ...
    it('trims the deferral display first, discloses the count, and keeps the blockers', () => {
        ...
        expect(r.remediation.some((line) => line.startsWith('body budget:'))).toBe(true);
    });
    it('trims the deferral display ALONE when that is enough — the order is observable', () => {
        ...
        // oracle at all.
        expect(r.body).not.toContain('Chinese translation of this body was dropped');
        ...
        // string in this file.
        expect(verdictLine(r)).toContain('3 non-Critical finding(s) deferred under the convergence posture ' +
    ...
    it('trims the not-reviewed disclosures only after the deferral display', () => {
        ...
        expect(r.body).toContain('the deferred-findings list and the not-reviewed and non-blocking disclosures did not fit');
    ...
    it('truncates as a last resort rather than composing a body GitHub rejects', () => {
        ...
        expect(r.remediation.join('\n')).toContain('so the posted body is truncated');
    });
    ...
    it("spends the copy the author already has before this round's only copy", () => {
        ...
        expect(r.remediation.join('\n')).toContain('repeat the trimmed sections in your terminal summary');
    });
    it('drops the bilingual fold BEFORE it drops any content', () => {
        ...
        const budgetLines = r.remediation.filter((l) => l.startsWith('body budget:'));
    ...
    it('drops sections only after the fold, and says so in both channels', () => {
        ...
        expect(r.body.indexOf('The Chinese translation')).toBeLessThan(r.body.indexOf('This body was trimmed to fit'));
        const budgetLines = r.remediation.filter((l) => l.startsWith('body budget:'));
        expect(budgetLines).toHaveLength(2);
        expect(budgetLines[0]).toContain('repeat the trimmed sections in your terminal summary');
        expect(budgetLines[1]).toContain('the English body is complete apart from the trimmed sections');
    });
    it('a truncated bilingual body discloses its fold too, and calls nothing complete', () => {
        ...
        expect(r.body.indexOf('This review body was TRUNCATED')).toBeLessThan(r.body.indexOf('The Chinese translation'));
        ...
        expect(r.body).toContain('the English text below is truncated as well — see the notice above');
    ...
    it('counts the sections it dropped, not the ranks', () => {
        ...
        const dims = ['security', 'perf', 'a11y', 'i18n'].map((d, i) => `${d} — ${'D'.repeat(700)}${i}`);
        const r = composeReview(base({
            severityFloor: 'critical',
    ...
    it('the unlicensed-deferral disclosure promises no adjacency it cannot keep', () => {
        ...
        expect(r.body).toContain('They are listed in this body when it has room for them, and always ' +
    ...
    it('sees no swallow when an opener sits inside a quoted attribute', () => {
        ...
        const attrOpener = 'add <img onerror="<script>"> guard ' + 'K'.repeat(70_000);
    ...
    it('ranks the plan-gate disclosures with the not-reviewed ones, not with the deferral list', () => {
        ...
        const withContext = (blocker) => composeReview({
            planPath: coveredPlan(['verify', 'reverse-audit'], {
                repositoryContext: {
                    version: 1,
                    provider: 'test',
        ...
        const runs = Array.from({ length: 61 }, (_, i) => 50_000 + i * 250).map((n) => withContext('B'.repeat(n)));
        const survivesRank1 = runs.find((r) => r.bodyTrim.deferralList &&
            !r.bodyTrim.truncated &&
            r.body.includes('Repository proof boundary'));
        const goesWithRank2 = runs.find((r) => r.bodyTrim.deferralList &&
            !r.body.includes('Repository proof boundary'));
        ...
        expect(goesWithRank2.bodyTrim.sections).toBeGreaterThan(survivesRank1.bodyTrim.sections);
    });
    it('puts the truncation notice ABOVE the cut, where nothing can swallow it', () => {
        ...
        expect(r.body.indexOf('was TRUNCATED to fit')).toBeLessThan(r.body.indexOf('blocker:'));
    });
    it('keeps the blocker under an absurd modelId — the footer is bounded', () => {
        ...
        const r = composeReview(base({ modelId: 'm'.repeat(60_000), bodyCriticals: ['C'.repeat(1_000)] }), '0.21.2');
        expect(r.body.length).toBeLessThanOrEqual(LIMIT);
        expect(r.body).toContain('**[Critical]** ');
    ...
    it('holds room for the ledger marker, so the POSTED body still fits', () => {
        ...
        const r = composeReview({
            ...
            criticalsInline: 0,
            suggestionsInline: 0,
            bodyCriticals: Array.from({ length: 40 }, (_, i) => `blocker ${i}: ${'B'.repeat(1_500)}`),
        });
        expect(r.body).toContain('<!-- qwen-review-ledger ');
        expect(r.body.length).toBeLessThanOrEqual(LIMIT);
...
describe('composeReview — the findings file tag check', () => {
    ...
    it('caps a clean Approve at Comment and discloses the surviving tag', () => {
        ...
        expect(r.event).toBe('COMMENT');
        expect(r.cappedBy).toContain('findings-unverified-at-compose');
        expect(r.body).toContain('1 finding(s) still carried the `— [unverified]` tag when the loop ' +
            'ended');
        expect(r.body).toContain('Review incomplete — unverified findings disclosed.');
        // The opener may not certify over a loop that ended mid-verification.
        expect(r.body).not.toContain('no blockers');
        expect(r.remediation.join(' ')).toContain('--role verify');
        expect(verdictLine(r)).toBe('Verdict: Comment — an Approve was NOT available: findings were ' +
    ...
    it('softens a Request changes whose blockers are non-deterministic', () => {
        ...
        const r = composeReview(base({ criticalsInline: 1, findingsPath: findingsFile(TAGGED) }));
        expect(r.baseEvent).toBe('REQUEST_CHANGES');
        expect(r.event).toBe('COMMENT');
        expect(r.cappedBy).toContain('findings-unverified-at-compose');
        expect(r.cappedBy).not.toContain('criticals-unverified');
        expect(verdictLine(r)).toBe('Verdict: Comment — a Request changes was NOT available: findings ' +
    ...
    it('fails CLOSED on a findingsPath that does not read', () => {
        const r = composeReview(base({ findingsPath: join(dir, 'no-such-findings.md') }));
        expect(r.baseEvent).toBe('APPROVE');
        expect(r.event).toBe('COMMENT');
        expect(r.cappedBy).toContain('findings-unverified-at-compose');
    ...
    it('refuses a present findingsPath of the wrong shape', () => {
        expect(() => composeReview(base({ findingsPath: 42 }))).toThrow(/findingsPath must be a non-empty string/);
    });
});
/**
...
describe('composeReview — unresolved-Critical rendering (#8388 readability)', () => {
    ...
    it('collapses entries sharing the exact reason into one group that says it once', () => {
        ...
        expect(r.body).toContain('- **[Critical]** 2 entries — body truncated; status undetermined:\n' +
            '  - comment one (a.ts)\n' +
            '  - comment two (b.ts)');
        // The shared reason renders once, not per entry …
        expect(r.body.match(/body truncated; status undetermined/g)).toHaveLength(1);
        // … and the odd one out keeps its own full line, nothing dropped.
        expect(r.body).toContain('- **[Critical]** unique.ts:9 — full text unfetchable');
    });
    it('links bare comment ids to their GitHub anchors when the plan names the PR', () => {
        ...
        expect(r.body).toContain('[comment 3733696855](https://github.com/LailatulCoder/lailatul-coder/pull/8388#discussion_r3733696855)');
        expect(r.body).toContain('[issue-level comment 5199834809](https://github.com/LailatulCoder/lailatul-coder/pull/8388#issuecomment-5199834809)');
    });
    it('leaves comment ids bare when the plan names no PR', () => {
        const r = composeReview(base({
            cannotTellCriticals: ['comment 3733696855 (a.ts) — body truncated'],
        }));
        expect(r.body).toContain('- **[Critical]** comment 3733696855 (a.ts) — body truncated');
        expect(r.body).not.toContain('discussion_r');
    });
    ...
    it('leaves an already-linked entry untouched — never nests a second link', () => {
        ...
        expect(r.body).toContain('[comment 3733696855](https://github.com/LailatulCoder/lailatul-coder/pull/8388#discussion_r3733696855) — body truncated');
    ...
    it('collapses embedded newlines so a multi-line entry stays one list item', () => {
        ...
        expect(r.body).toContain('- **[Critical]** comment 3733696855 (a.ts) — body truncated see also b.ts');
    });
    it('counts entries, not groups, in the Chinese fold', () => {
        ...
        // list it points at.
        expect(r.body).toContain('未决，请确认：共 3 条（原文未翻译，列表见上方英文部分）。');
    });
    it("anchors comment ids at the plan's GHE host, short ids included", () => {
        ...
        expect(r.body).toContain('[comment 12345](https://ghe.example.com/corp/widgets/pull/12#discussion_r12345)');
    });
    it('leaves short ids bare on github.com — ordinals are not anchors', () => {
        ...
        expect(r.body).toContain('- **[Critical]** comment 12345 (a.ts) — body truncated');
    ...
    it('strips a copied **[Critical]** prefix from a cannot-tell entry', () => {
        ...
        expect(r.body).toContain('- **[Critical]** old blocker (a.ts) — body truncated');
...
describe('composeReview — a resumed run is continuity, not a coverage gap', () => {
    it('stays APPROVE and renders the non-capping continuity note', () => {
        ...
        // as recovered work.
        const input = base({});
        rehomeToPriorSession(input.planPath, 'agent-a1.jsonl');
        const r = composeReview(input);
        expect(r.event).toBe('APPROVE');
...
describe('composeReview — continuity renders on every verdict', () => {
     ...
     */
    function resumedInput(over = {}) {
        const input = base(over);
        const p = input.planPath;
...
describe('floor enforcement — the posture, as code', () => {
    ...
    const sideFile = (round) => writeFileSync(join(dir, 'qwen-review-pr-8255-prev-ledger.json'), JSON.stringify({ v: 1, round, findings: [] }));
    const drafts = () => [
    ...
    const compose = (over = {}) => composeReview({
        planPath: plan(),
        modelId: 'm',
        criticalsInline: 1,
        suggestionsInline: 2,
    ...
    it('an unrecognised present floor fails open — a posting bar in doubt posts', () => {
        ...
        for (const floor of ['blocker', 'Critical-only', 'high']) {
            const r = compose({ severityFloor: floor });
            expect(r.floorEnforced).toEqual([]);
        }
    });
    ...
    it('keeps BOTH channels’ records when they share an anchor — no dedup, no lost finding', () => {
        ...
        expect(r.body).toContain('1 Suggestion(s) were drafted inline past the resolved critical posting floor');
    });
    it('renders the zh disclosure arm beside the en one on a bilingual review', () => {
        ...
        writeFileSync(hanPlan, JSON.stringify({ prNumber: 8255, prDescriptionHasHan: true }));
        const r = compose({
            severityFloor: 'critical',
            criticalsInline: 0,
            suggestionsInline: 1,
        ...
        expect(r.floorEnforced).toEqual([0]);
        expect(r.body).toContain('1 条 Suggestion 在已解析的 critical 发布下限之外被起草为行内评论');
    ...
    it('turns the enforcement note cap-aware when the moved entries overflow the render cap', () => {
        ...
        const r = compose({
            ...
            planPath: (() => {
                ...
                writeFileSync(p, JSON.stringify({ prNumber: 8255, prDescriptionHasHan: true }));
                return p;
            })(),
        });
...
describe('convergence telemetry — volume, carried in the marker', () => {
    ...
    const sideFile = (prev) => writeFileSync(join(dir, 'qwen-review-pr-8255-prev-ledger.json'), JSON.stringify({ v: 1, findings: [], ...prev }));
    const drafts = (n) => Array.from({ length: n }, (_, i) => ({
        path: `f${i}.ts`,
        line: i + 1,
...
describe('convergence diagnosis reaches the POSTED body', () => {
    ...
    it('renders the observation when findings keep returning to one file', () => {
        ...
        expect(r.body).toContain('`src/a.ts` (findings in rounds 2, 4, 2 more now)');
    ...
    it('counts the POST-enforcement set, like every other volume surface', () => {
        ...
        expect(r.floorEnforced).toHaveLength(2);
        expect(r.body).toContain('round 6 posted 1 inline comment(s), 1 of them reported for the first time');
    });
    ...
    it('reaches a REQUEST_CHANGES body — the verdict a diverging loop produces', () => {
        ...
        writeFileSync(join(dirname(planPath), 'qwen-review-pr-8255-prev-ledger.json'), JSON.stringify({
            v: 1,
            round: 4,
            posted: 9,
            fresh: 9,
    ...
    it('keeps a finding whose line number is not an integer', () => {
        ...
        const l = buildLedger(2, [{ path: 'a.ts', line: 12.5, body: '**[Critical]** boom' }], []);
        expect(l.findings).toEqual([
    ...
    it('re-mints a claimed id the serializer would refuse, keeping the finding', () => {
        ...
        for (const claimed of ['R7-1', 'R0-1', `R2-${'9'.repeat(24)}`]) {
            const l = buildLedger(5, [{ path: 'a.ts', line: 1, body: `**[Critical]** ${claimed}: boom` }], []);
            expect(l.findings.map((x) => x.id)).toEqual(['R5-1']);
            const parsed = parseLedger(serializeLedger({ ...l, sha: 'deadbeef00112233' }));
            expect(parsed.findings).toHaveLength(1);
            expect(parsed.dropped).toBeUndefined();
            expect(parsed.sha).toBe('deadbeef00112233');
        }
        ...
        const kept = buildLedger(5, [{ path: 'a.ts', line: 1, body: '**[Critical]** R2-1: still open' }], []);
...
//# sourceMappingURL=compose-review.test.js.map