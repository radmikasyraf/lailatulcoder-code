 ...
 */
import { MAX_WORKSPACE_PATH_LENGTH, translateAndCheckAbsoluteWorkspacePath, } from '@lailatul-coder/acp-bridge/workspacePaths';
import { normalizeSessionIdForLookup } from '../../config/session-id.js';
import { writeStderrLine } from '../../utils/stdioHelpers.js';
export function sendJsonBodyParserError(res, err) {
...
export const CLIENT_ID_HEADER = 'x-LailatulCoder-client-id';
...
const deferredRuntimeTimingKey = Symbol('deferredRuntimeRequestTiming');
export function setDeferredRuntimeRequestTiming(req, timing) {
    req[deferredRuntimeTimingKey] = timing;
}
export function getDeferredRuntimeRequestTiming(req) {
...
export function parseOptionalWorkspaceCwd(body, boundWorkspace, res) {
    ...
    const cwd = translateAndCheckAbsoluteWorkspacePath(hasCwd ? body['cwd'] : boundWorkspace);
    if (cwd === null) {
        res
            .status(400)
...
export function parseClientIdHeader(req, res) {
    const raw = req.get(CLIENT_ID_HEADER);
    if (raw === undefined || raw === '')
        return undefined;
    if (raw.length > MAX_CLIENT_ID_LENGTH || !CLIENT_ID_RE.test(raw)) {
        res.status(400).json({
            error: '`X-LailatulCoder-Client-Id` must be a non-empty token of 128 characters or fewer',
 ...
 * validation failure.
 */
export function validateMcpRuntimeServerName(name, res) {
...
/**
 * Workspace-level mutation routes validate the parsed `X-LailatulCoder-Client-Id`
 ...
 */
export function parseAndValidateWorkspaceClientId(req, res, bridge) {
...
export function createBuildWorkspaceCtx(boundWorkspace) {
...
export function parsePermissionVoteBody(req, res) {
 ...
 */
export function parseMaxQueuedQuery(raw, res) {
    ...
    if (typeof raw !== 'string' || !/^\d+$/.test(raw)) {
        ...
        // `sendBridgeError`.
        writeStderrLine(`LailatulCoder serve: rejected ?maxQueued ${safeLogValue(raw)} ` +
 ...
 */
export function safeLogValue(raw) {
    return JSON.stringify(String(raw)).slice(0, 82);
}
export function parseLastEventId(raw) {
    ...
    if (typeof raw !== 'string' || !/^\d+$/.test(raw)) {
        ...
        if (typeof raw === 'string' && raw.length > 0) {
            writeStderrLine(`LailatulCoder serve: rejected Last-Event-ID ${safeLogValue(raw)} ` +
                `(not a decimal integer)`);
        }
        return undefined;
    }
