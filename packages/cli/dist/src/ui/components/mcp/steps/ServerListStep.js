...
export const ServerListStep = ({ servers, onSelect, }) => {
    ...
    return (_jsxs(Box, { flexDirection: "column", children: [groupedServers.map((group, groupIndex) => (_jsxs(Box, { flexDirection: "column", marginBottom: groupIndex === groupedServers.length - 1 ? 0 : 1, children: [_jsxs(Text, { bold: true, color: theme.text.primary, children: [`  ${group.displayName}`, group.servers[0]?.configPath && (_jsxs(Text, { color: theme.text.secondary, children: [' ', "(", group.servers[0].configPath, ")"] }))] }), _jsx(Box, { flexDirection: "column", children: group.servers.map((server, itemIndex) => {
                            ...
                            const needsAuth = !server.isDisabled &&
                                !awaitingApproval &&
                                !!server.requiresAuth &&
                                server.status !== MCPServerStatus.CONNECTED;
                            const statusColor = server.isDisabled || awaitingApproval || needsAuth
                                ? 'yellow'
                                            ...
                                            : statusColor === 'yellow'
                                                ...
                                                : awaitingApproval
                                                    ...
                                                    : needsAuth
                                                        ...
                                                        : t(server.status)] }), !!server.invalidToolCount && server.invalidToolCount > 0 && (_jsxs(Text, { color: theme.status.warning, children: [' ', t('{{count}} invalid tools', {
                                                count: String(server.invalidToolCount),
                                            })] }))] }, server.name));
                        }) })] }, group.source))), servers.some((s) => s.status === 'disconnected' && !s.isDisabled && !s.approvalState) && (_jsx(Box, { marginTop: 1, children: _jsxs(Text, { color: theme.status.warning, children: [ICON.REFERENCE, " ", t('Run LailatulCoder --debug to see error logs')] }) }))] }));
};
