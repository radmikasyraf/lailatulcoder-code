 ...
 */
import { OutputFormat } from '@lailatul-coder/lailatul-coder-core';
...
export async function validateNonInteractiveAuth(useExternalAuth, nonInteractiveConfig, settings) {
    try {
        ...
        const authType = nonInteractiveConfig
            ...
            .getCurrentAuthType();
        if (!authType) {
            throw new Error('No auth type is selected. Please configure an auth type (e.g. via settings or `--auth-type`) before running in non-interactive mode.');
        }
        ...
        if (outputFormat === OutputFormat.JSON ||
            ...
            if (outputFormat === OutputFormat.JSON) {
                ...
                adapter = new StreamJsonOutputAdapter(nonInteractiveConfig, nonInteractiveConfig.getIncludePartialMessages());
            }
