 ...
 */
export function narrowToDelta(fullBytes, deltaBytes) {
...
export function selectNarrowing(fullBytes, deltaBytes) {
    ...
    if (fullText === null || deltaText === null)
        return null;
    if (fullText.trim() === '' || deltaText.trim() === '')
        return null;
    const full = parseDiff(fullText);
    const delta = parseDiff(deltaText);
    if (full.files.length === 0 || delta.files.length === 0)
        return null;
    /**
    ...
    // full capture.
    const fullRenames = new Map();
    for (const f of full.files) {
        if (f.renameFrom !== undefined)
            fullRenames.set(f.path, f.renameFrom);
    }
 ...
 */
export function assembleSections(selection, paths) {
    ...
    const parts = selected.map(([from, to]) => lines.slice(from - 1, to).join('\n'));
    return Buffer.from(parts.join('\n') + '\n', 'utf8');
}
//# sourceMappingURL=narrow-diff.js.map