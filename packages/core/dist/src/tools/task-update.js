...
import { getAgentName, isTeammate, resolveActiveTeamName, } from '../agents/team/identity.js';
import { sanitizeName } from '../agents/team/teamHelpers.js';
import { getPlanRequiredTeammatePreApprovalMessage, isPlanRequiredTeammatePreApprovalAllowedTool, isPlanRequiredTeammateAwaitingApproval, } from '../agents/runtime/subagent-plan-tool-policy.js';
import { updateTask, deleteTask, assertValidTaskId, getTask, listTasks, TaskOwnershipError, RECIPROCAL_CALLER, } from '../agents/team/tasks.js';
...
class TaskUpdateInvocation extends BaseToolInvocation {
    ...
    async execute() {
        const awaitingPlanApproval = isPlanRequiredTeammateAwaitingApproval(this.config);
        if (awaitingPlanApproval &&
            !isPlanRequiredTeammatePreApprovalAllowedTool(ToolNames.TASK_UPDATE, this.params)) {
            const msg = getPlanRequiredTeammatePreApprovalMessage(ToolNames.TASK_UPDATE);
            return {
                llmContent: msg,
                returnDisplay: msg,
                error: { message: msg },
        ...
        if (awaitingPlanApproval) {
            ...
            if (existing && (existing.status !== 'pending' || existing.owner)) {
                const msg = 'task_update can only claim an unowned pending task while this ' +
        ...
        if (this.params.status === 'deleted') {
            let ok;
            try {
                ok = await deleteTask(teamName, taskId, teammateCallerName !== undefined
        ...
        if (this.params.addBlocks?.includes(taskId) ||
            this.params.addBlockedBy?.includes(taskId)) {
            const msg = `Cannot update task #${taskId}: a task cannot block ` +
        ...
        if (referencedIds.size > 0) {
            ...
            if (missing.length > 0) {
                const ids = missing
                    ...
                    .map((id) => `#${id}`)
                    .join(', ');
                const msg = `Cannot update task #${taskId}: ` +
            ...
            const cycle = await findDependencyCycle(teamName, taskId, this.params.addBlocks ?? [], this.params.addBlockedBy ?? []);
            if (cycle) {
                const msg = `Cannot update task #${taskId}: this would create a ` +
        ...
        // orphaning the task.
        const autoOwner = this.params.status === 'in_progress' && this.params.owner === undefined
            ? getAgentName()
            : undefined;
        const explicitOwner = this.params.owner === undefined
            ? undefined
            : sanitizeName(this.params.owner);
        if (this.params.owner !== undefined &&
            this.params.owner !== '' &&
            !explicitOwner) {
            const msg = `Cannot assign task #${taskId}: owner must include at least one ` +
        ...
        if (this.params.status === 'in_progress' &&
            !this.params.owner &&
            !autoOwner) {
            const msg = `Cannot move task #${taskId} to in_progress without ` +
        ...
        const statusBecameInProgress = this.params.status === 'in_progress' &&
            existing?.status !== 'in_progress';
        const ownerChanged = explicitOwner !== undefined && explicitOwner !== existingOwner;
        const nextBlockedBy = new Set(existing?.blockedBy ?? []);
        for (const blockedBy of this.params.addBlockedBy ?? []) {
            nextBlockedBy.add(blockedBy);
        }
        const shouldDispatchAssignment = nextStatus === 'in_progress' &&
            !!nextOwner &&
            nextOwner !== teammateCallerName &&
            (ownerChanged || statusBecameInProgress);
        if (shouldDispatchAssignment && nextBlockedBy.size > 0) {
            const msg = `Cannot assign task #${taskId} to "${nextOwner}" while it is ` +
        ...
        let task;
        try {
            task = await updateTask(teamName, taskId, {
        ...
        if (teamManager &&
            ...
            if (!dispatched) {
                const msg = `Task #${taskId} was updated (status: ${task.status}, ` +
        ...
        if (this.params.addBlocks?.length && this.params.status !== 'completed') {
            for (const blockedId of this.params.addBlocks) {
                if (blockedId === taskId)
                    continue;
                reciprocalUpdates.push(updateTask(teamName, blockedId, { addBlockedBy: [taskId] }, { callerName: RECIPROCAL_CALLER }));
            }
        }
        if (this.params.addBlockedBy?.length) {
            for (const blockerId of this.params.addBlockedBy) {
                if (blockerId === taskId)
                    continue;
                reciprocalUpdates.push(updateTask(teamName, blockerId, { addBlocks: [taskId] }, { callerName: RECIPROCAL_CALLER }));
            }
        }
        ...
        const llmContent = `Task #${taskId} updated (status: ${task.status}` +
...
export class TaskUpdateTool extends BaseDeclarativeTool {
    ...
    constructor(config) {
        super(TaskUpdateTool.Name, ToolDisplayNames.TASK_UPDATE, 'Update an existing task. Can change status, owner, ' +
            ...
            'as owner if no owner is set.', Kind.Other, {
            type: 'object',
            properties: {
                taskId: {
                    type: 'string',
