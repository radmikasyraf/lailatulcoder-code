...
describe('AgentComposer', () => {
    ...
    beforeEach(() => {
        ...
        vi.mocked(useKeypress).mockImplementation((handler, _options) => {
    ...
    it('does not reset the parent input-buffer state during unmount', () => {
        const { unmount } = render(_jsx(AgentComposer, { agentId: "agent-1" }));
    ...
    const getShiftTabHandler = () => {
        render(_jsx(AgentComposer, { agentId: "agent-1" }));
        return capturedKeypressHandlers[1];
    };
    it('cycles approval mode on Shift+Tab', () => {
        const handler = getShiftTabHandler();
        handler({ name: 'tab', shift: true, ctrl: false });
        expect(setAgentApprovalMode).toHaveBeenCalledWith('agent-1', ApprovalMode.AUTO_EDIT);
    });
    it('does not cycle approval mode on Ctrl+Shift+Tab', () => {
        const handler = getShiftTabHandler();
        handler({ name: 'tab', shift: true, ctrl: true });
        expect(setAgentApprovalMode).not.toHaveBeenCalled();
    });
});
//# sourceMappingURL=AgentComposer.test.js.map