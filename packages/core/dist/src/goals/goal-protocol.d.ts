...
export declare const GOAL_EVIDENCE_CATALOG_EXHAUSTED_REASON = "The current Goal revision exceeded the bounded evidence catalog. Automatic retries cannot recover. Edit or replace the Goal before resuming it.";
export declare const GOAL_CHECKPOINT_REQUEST_TOO_LARGE_REASON = "The current Goal revision exceeded the checkpoint verifier request limit. Automatic retries cannot recover. Edit or replace the Goal before resuming it.";
/**
...
export declare function isGoalLimitKind(value: unknown): value is GoalLimitKind;
/** The limit a `usage_limited` reason denotes, for reasons that denote one. */
export declare function goalLimitKindForReason(reason: string): GoalLimitKind | undefined;
export declare const PAUSED_GOAL_SYSTEM_REMINDER = "<system-reminder>\nThe Goal is paused. Do not continue its objective unless the user resumes it. Treat this message as ordinary conversation.\n</system-reminder>";
...
export type GoalEvidenceProofKind = 'user_input' | 'delivered_output' | 'external_fact';
export declare function isGoalEvidenceProofKind(value: unknown): value is GoalEvidenceProofKind;
...
export declare function goalRequiresExactPermit(snapshot: GoalSnapshotV2): boolean;
...
export declare function isRepeatedBlockerProposal(proposal: GoalTerminalProposal): boolean;
export declare function validateGoalProposalReason(reason: string): string | null;
export type GoalStateCause = 'create' | 'replace' | 'edit' | 'pause' | 'resume' | 'turn_finished' | 'checkpoint' | 'verifier_accept' | 'verifier_reject' | 'complete' | 'blocked' | 'usage_limited' | 'clear' | 'migrated';
