...
import { acquireInheritedLoaderEnvScrub, clearLoaderKeyRejectionReporterIfCurrent, ENV_ACP_REPEATED_TOOL_FAILURE_GUARD, HOME_ENV_BOOTSTRAP_KEYS, INHERITED_LOADER_ENV_KEYS, isHardcodedProjectEnvExclusion, isLoaderEnvKey, PROJECT_ENV_HARDCODED_EXCLUSIONS, reportRejectedLoaderKeys, resetInheritedLoaderEnvScrubForTesting, resetLoaderKeyRejectionReportingForTesting, scrubAndReportInheritedLoaderEnv, scrubInheritedLoaderEnv, setLoaderKeyRejectionReporter, } from './shared-env-keys.js';
describe('PROJECT_ENV_HARDCODED_EXCLUSIONS', () => {
    ...
    it('excludes NODE_TLS_REJECT_UNAUTHORIZED so a project .env cannot disable TLS', () => {
        expect(PROJECT_ENV_HARDCODED_EXCLUSIONS).toContain('NODE_TLS_REJECT_UNAUTHORIZED');
    });
    it('keeps ACP repeated-tool-failure rollout policy operator-owned', () => {
        expect(PROJECT_ENV_HARDCODED_EXCLUSIONS).toContain(ENV_ACP_REPEATED_TOOL_FAILURE_GUARD);
    });
    it('keeps daemon memory scope operator-owned', () => {
        expect(PROJECT_ENV_HARDCODED_EXCLUSIONS).toContain('QWEN_CODE_MEMORY_PROJECT_SCOPE');
    });
    ...
    it('keeps compile-cache keys out of the hardcoded exclusions', () => {
        expect(PROJECT_ENV_HARDCODED_EXCLUSIONS).not.toContain('NODE_COMPILE_CACHE');
        expect(PROJECT_ENV_HARDCODED_EXCLUSIONS).not.toContain('QWEN_CODE_PENDING_COMPILE_CACHE');
    });
    ...
    it('excludes QWEN_SERVE_NEW_FILE_MODE so a project .env cannot widen new-file mode', () => {
        expect(PROJECT_ENV_HARDCODED_EXCLUSIONS).toContain('QWEN_SERVE_NEW_FILE_MODE');
    });
    ...
    it('keeps SSH_ASKPASS_REQUIRE out of the hardcoded exclusions', () => {
        expect(PROJECT_ENV_HARDCODED_EXCLUSIONS).not.toContain('SSH_ASKPASS_REQUIRE');
    ...
    it('excludes the serve CDP adapter command and tunnel switch', () => {
        ...
        expect(PROJECT_ENV_HARDCODED_EXCLUSIONS).toContain('QWEN_SERVE_CDP_TUNNEL_OVER_WS');
    });
...
describe('isHardcodedProjectEnvExclusion', () => {
    ...
    it('matches the newly added hardcoded exclusions case-insensitively', () => {
        ...
        expect(isHardcodedProjectEnvExclusion('qwen_cdp_mcp_command')).toBe(true);
        expect(isHardcodedProjectEnvExclusion('QWEN_SERVE_CDP_TUNNEL_OVER_WS')).toBe(true);
        expect(isHardcodedProjectEnvExclusion('qwen_serve_cdp_tunnel_over_ws')).toBe(true);
    });
    ...
    it('matches numbered GIT_CONFIG_KEY_/VALUE_ pairs by numeric suffix', () => {
        ...
        expect(isHardcodedProjectEnvExclusion('git_config_value_cache')).toBe(false);
...
describe('scrubInheritedLoaderEnv', () => {
    ...
    it('removes every loader-affecting key and keeps the rest', () => {
        ...
        expect(env['QWEN_SERVER_TOKEN']).toBe('leave-secret-scrubbing-to-other-layers');
    });
...
describe('reportRejectedLoaderKeys', () => {
    it('returns every rejected key while warning only once per source and key', () => {
        ...
        try {
            expect(reportRejectedLoaderKeys('/workspace/.env', [
                'NODE_OPTIONS',
                'PATH',
                'LD_PRELOAD',
...
describe('scrubAndReportInheritedLoaderEnv', () => {
    ...
    it('scrubs and reports the removed keys with the boundary labels', () => {
        ...
        const breadcrumb = captureStderr(() => {
            removedKeys = scrubAndReportInheritedLoaderEnv(env, 'qwen serve', 'daemon');
        });
        expect(removedKeys).toEqual(['NODE_OPTIONS', 'LD_PRELOAD']);
        expect(env['HOME']).toBe('/home/user');
        expect(breadcrumb).toContain('qwen serve: scrubbed inherited loader env vars from the daemon ' +
    ...
    it('stays silent when there is nothing to scrub', () => {
        ...
        const breadcrumb = captureStderr(() => {
            expect(scrubAndReportInheritedLoaderEnv(env, 'qwen', 'ACP child')).toEqual([]);
        });
...
describe('acquireInheritedLoaderEnvScrub', () => {
    ...
    it('does not restore loader vars while a second holder is still active', () => {
        ...
        try {
            ...
            const daemonA = acquireInheritedLoaderEnvScrub(process.env, 'qwen serve', 'daemon');
            ...
            const daemonB = acquireInheritedLoaderEnvScrub(process.env, 'qwen serve', 'daemon');
    ...
    it('release is idempotent and a later assignment wins over the restore', () => {
        ...
        const write = vi
            ...
            .mockImplementation(() => true);
        try {
            const handle = acquireInheritedLoaderEnvScrub(process.env, 'qwen serve', 'daemon');
    ...
    it('does not restore a prior cycle snapshot for a key the host removed', () => {
        ...
        const write = vi
            ...
            .mockImplementation(() => true);
        try {
            const firstCycle = acquireInheritedLoaderEnvScrub(process.env, 'qwen serve', 'daemon');
            firstCycle.release();
            expect(process.env['NODE_OPTIONS']).toBe('--cycle-one');
            // The host removes the key entirely between cycles.
            delete process.env['NODE_OPTIONS'];
            const secondCycle = acquireInheritedLoaderEnvScrub(process.env, 'qwen serve', 'daemon');
    ...
    it('does not snapshot loader keys whose value is undefined', () => {
        ...
        const write = vi
            ...
            .mockImplementation(() => true);
        try {
            const handle = acquireInheritedLoaderEnvScrub(env, 'qwen serve', 'daemon');
...
describe('clearLoaderKeyRejectionReporterIfCurrent', () => {
    ...
    it('clears only when the given reporter is still the active one', () => {
        resetLoaderKeyRejectionReportingForTesting();
        const calls = [];
        const reporterA = () => calls.push('A');
        const reporterB = () => calls.push('B');
        const write = vi
            .spyOn(process.stderr, 'write')
            .mockImplementation(() => true);
        try {
...
//# sourceMappingURL=shared-env-keys.test.js.map