...
import { useGitBranchName, BRANCH_POLL_INTERVAL_MS, } from './useGitBranchName.js';
...
const mockResolve = resolveBranchName;
const mockWatch = watchRepoBranch;
...
describe('useGitBranchName', () => {
    ...
    it('refreshes the branch name when the watcher fires', async () => {
        ...
        let fire;
        mockWatch.mockImplementation(async (_cwd, onChange) => {
            fire = onChange;
            return () => { };
        });
        const { result } = renderHook(() => useGitBranchName(CWD));
    ...
    it('disposes immediately if the watcher resolves after unmount', async () => {
        ...
        const dispose = vi.fn();
        let resolveWatch;
        mockWatch.mockImplementation(() => new Promise((resolve) => {
