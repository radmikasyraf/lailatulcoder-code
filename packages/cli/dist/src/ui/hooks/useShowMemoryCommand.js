...
export function createShowMemoryAction(config, settings, addMessage) {
    return async () => {
        ...
        const contextFileNames = Array.isArray(contextFileName)
            ? contextFileName
            : [contextFileName];
        debugLogger.debug(`[DEBUG] Showing memory. Content from config.getUserMemory() (first 200 chars): ${currentMemory.substring(0, 200)}...`);
        ...
        if (currentMemory && currentMemory.trim().length > 0) {
            ...
            addMessage({
                type: MessageType.INFO,
                content: fileCount > 0
                    ? 'Hierarchical memory (LAILATUL.md or other context files) is loaded but content is empty.'
                    : 'No hierarchical memory (LAILATUL.md or other context files) is currently loaded.',
                timestamp: new Date(),
            });
        }
    };
