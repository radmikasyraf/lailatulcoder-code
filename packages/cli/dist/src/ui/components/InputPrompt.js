...
import { ApprovalMode, Storage, createDebugLogger, } from '@lailatul-coder/lailatul-coder-core';
...
import { isLiveAgentPanelVisibleEntry, LIVE_AGENT_PANEL_MAX_ROWS, getLiveAgentPanelVpMaxRows, } from './background-view/liveAgentPanelVisibility.js';
...
import { assertVoiceBaseUrlNetworkAllowed, isKeytermEcho, isStreamingVoiceModel, resolveVoiceStreamConfig, transcribeVoiceAudio, } from '../voice/voice-transcriber.js';
...
import { clearPromptStash, savePromptStash, } from '../../services/prompt-stash.js';
 ...
 */
export function classifyPastedImagePaths(pasted) {
    const tokens = pasted
        .split(/ (?=@?\/|@?[A-Za-z]:\\)/)
...
export function expandPendingPastePlaceholders(value, pendingPastes) {
    if (pendingPastes.size === 0) {
        return value;
    }
    const placeholders = Array.from(pendingPastes.keys()).sort((a, b) => b.length - a.length);
    const escapedPlaceholders = placeholders.map((placeholderValue) => placeholderValue.replace(/[.*+?^${}()|[\]\\]/g, '\\$&'));
    const placeholderRegex = new RegExp(escapedPlaceholders.join('|'), 'g');
    return value.replace(placeholderRegex, (matchedPlaceholder) => pendingPastes.get(matchedPlaceholder) ?? matchedPlaceholder);
}
...
export const InputPrompt = ({ buffer, onSubmit, userMessages, onClearScreen, config, slashCommands, commandContext, recentSlashCommands, placeholder, focus = true, suggestionsWidth, shellModeActive, setShellModeActive, approvalMode, onEscapePromptChange, onToggleShortcuts, showShortcuts, onSuggestionsVisibilityChange, onTabConsumerChange, vimHandleInput, isEmbeddedShellFocused, promptSuggestion, onPromptSuggestionDismiss, clipboardUnavailableShownRef: sessionClipboardUnavailableShownRef, }) => {
    ...
    const mouseInteractionsEnabled = useVirtualViewport(settings.merged.ui?.useTerminalBuffer) &&
    ...
    const { entries: bgEntries, dialogOpen: bgDialogOpen, pillFocused: bgPillFocused, livePanelFocused, livePanelSelectedIndex, } = useBackgroundTaskViewState();
    const { setLivePanelFocused, setLivePanelSelectedIndex, enterDetailFromPanel: enterBgDetailFromPanel, setSelectedIndex: setBgSelectedIndex, setPillFocused: setBgPillFocused, } = useBackgroundTaskViewActions();
    ...
    const getVisibleBgAgents = useCallback(() => panelDisplayOrder(bgEntries.filter((e) => isLiveAgentPanelVisibleEntry(e, Date.now()))).slice(-liveAgentPanelMaxRows), [bgEntries, liveAgentPanelMaxRows]);
    const hasActiveToolConfirmation = useMemo(() => Boolean(uiState.confirmationRequest) ||
        (uiState.pendingGeminiHistoryItems ?? []).some((item) => item.type === 'tool_group' &&
            item.tools.some((tool) => tool.confirmationDetails)), [uiState.confirmationRequest, uiState.pendingGeminiHistoryItems]);
    const [historyRestoredText, setHistoryRestoredText] = useState(null);
    ...
    const clipboardUnavailableShownRef = sessionClipboardUnavailableShownRef ?? localClipboardUnavailableShownRef;
    // Large paste placeholder handling
    const [pendingPastes, setPendingPastes] = useState(new Map());
    // Track active placeholder IDs for each charCount to enable reuse
    const activePlaceholderIds = useRef(new Map());
    // Parse placeholder to extract charCount and ID
    const parsePlaceholder = useCallback((placeholder) => {
    ...
    const freePlaceholderId = useCallback((charCount, id) => {
    ...
    const isHistoryRestoredText = historyRestoredText !== null && buffer.text === historyRestoredText;
    const completion = useCommandCompletion(buffer, config.getTargetDir(), slashCommands, commandContext, reverseSearchActive, config, 
    // Suppress completion for history-restored text until the user edits it.
    !isHistoryRestoredText, recentSlashCommands);
    const showCompletionSuggestions = completion.showSuggestions && !isHistoryRestoredText;
    const categoryTabsVisible = !exportCompletion.suggestionDisplayProps &&
        !commandSearchActive &&
        !reverseSearchActive &&
        !isAttachmentMode &&
    ...
    const midInputGhostTextRef = useRef(null);
    ...
    const commandSearchCompletion = useReverseSearchCompletion(buffer, commandSearchHistory, commandSearchActive);
    ...
    const voiceModel = typeof rawVoiceModel === 'string' && rawVoiceModel.trim().length > 0
        ? rawVoiceModel.trim()
        : undefined;
    const voiceEnabled = settings.merged.general?.voice?.enabled === true && Boolean(voiceModel);
    const voiceMode = settings.merged.general?.voice?.mode === 'tap' ? 'tap' : 'hold';
    ...
    const voiceSubmitRef = useRef(() => { });
    const transcribeVoice = useCallback((audio, { voiceModel }) => transcribeVoiceAudio(audio, { config, settings, voiceModel }), [config, settings]);
    ...
    const voiceRefineEnabled = settings.merged.general?.voice?.refineTranscript !== false &&
        config.getFastModel() != null;
    const refineVoice = useCallback((raw, signal) => refineVoiceTranscript(config, raw, signal), [config]);
    const voiceMicWarnedStatusRef = useRef(null);
        ...
        void Promise.resolve(recorder.microphoneStatus?.())
            ...
            if (status === 'denied') {
                ...
                uiState.historyManager?.addItem({
                    type: 'error',
                    text: t('Microphone access is denied. Enable it for your terminal in System Settings → Privacy & Security → Microphone, then restart voice dictation.'),
                }, Date.now());
            }
                ...
                uiState.historyManager?.addItem({
                    type: 'info',
                    text: t('Voice dictation needs microphone access. macOS will ask the first time you record — approve it, then start again. Your first recording may be empty while the dialog is open.'),
                }, Date.now());
            }
        })
            .catch(() => { });
    ...
    const openVoiceStreamSession = useCallback((callbacks) => {
        if (!voiceModel) {
        ...
        return assertVoiceBaseUrlNetworkAllowed(streamConfig)
            .then(() => openVoiceStreamWithRetry(() => streamConfig.transport === 'qwen-asr-realtime'
    ...
    const nextLargePastePlaceholder = useCallback((charCount) => {
    ...
    const resetHistoryNavRef = useRef(() => { });
    const handleSubmitAndClear = useCallback((submittedValue, deferUntilIdle = false) => {
    ...
    const reportClipboardUnavailable = useCallback(() => {
        ...
        uiState.historyManager?.addItem({
            type: 'error',
            text: t('Clipboard image paste is unavailable because the native clipboard module could not be loaded. Reinstall LailatulCoder Ai or use the npm installation method.'),
        }, Date.now());
    }, [clipboardUnavailableShownRef, uiState.historyManager]);
    // Handle clipboard image pasting with Ctrl+V
    const handleClipboardImage = useCallback(async (validated = false) => {
        try {
            const hasImage = validated || (await clipboardHasImage(reportClipboardUnavailable));
            if (hasImage) {
    ...
    const promotePastedImagePaths = useCallback(async (imagePaths, originalPasted) => {
        ...
        for (const imagePath of imagePaths) {
            ...
            try {
                ...
                const destPath = path.join(clipboardDir, `clipboard-${Date.now()}-${attachments.length}${path.extname(sourcePath)}`);
    ...
    const descendFromComposer = useCallback(() => {
    ...
    const availableSuggestion = followup.state.isVisible || promptSuggestion
    ...
    const handleInput = useCallback((key) => {
            ...
            if (key.name === 'return') {
                if (livePanelSelectedIndex === 0) {
                    ...
                    const entryIdx = entry
                        ? bgEntries.findIndex((e) => e.kind === 'agent' && e.agentId === entry.agentId)
                        : -1;
                    if (entryIdx >= 0) {
        ...
        if (key.paste) {
            ...
            else if (pastedImagePaths.allImages &&
            ...
            else if (charCount > LARGE_PASTE_CHAR_THRESHOLD ||
        ...
        if (keyMatchers[Command.SHOW_MORE_LINES](key) && buffer.text.length > 0) {
            const textToStash = expandPendingPastePlaceholders(buffer.text, pendingPastes);
        ...
        const popQueueIntoInput = () => {
            ...
            if (currentText) {
                const currentCursorOffset = logicalPosToOffset(buffer.lines, buffer.cursor[0], buffer.cursor[1]);
        ...
        if (showCompletionSuggestions) {
            ...
            if (keyMatchers[Command.ACCEPT_SUGGESTION](key) && !key.paste) {
                ...
                const targetIndex = completion.activeSuggestionIndex === -1
                    ? 0
                    : completion.activeSuggestionIndex;
                const accepted = targetIndex >= 0 && targetIndex < completion.suggestions.length
        ...
        if (!shellModeActive) {
            ...
            if (keyMatchers[Command.HISTORY_UP](key)) {
                ...
                const onFirstRow = buffer.visualCursor[0] === 0 && buffer.visualScrollRow === 0;
                if (!onFirstRow) {
                    buffer.move('up');
                    return true;
                }
    ...
    const renderLineWithHighlighting = useCallback((opts) => {
        const { lineText, isOnCursorLine, cursorCol: cursorVisualColAbsolute, showCursor: showCursorOpt, absoluteVisualIndex, buffer: buf, } = opts;
        const mapEntry = buf.visualToLogicalMap[absoluteVisualIndex];
        const [logicalLineIdx, logicalStartCol] = mapEntry;
        const logicalLine = buf.lines[logicalLineIdx] || '';
        const tokens = parseInputForHighlighting(logicalLine, logicalLineIdx, slashCommands);
        ...
        segments.forEach((seg, segIdx) => {
            ...
            if (isOnCursorLine) {
                ...
                if (cursorVisualColAbsolute >= segStart &&
                    cursorVisualColAbsolute < segEnd) {
                    const charToHighlight = cpSlice(seg.text, cursorVisualColAbsolute - segStart, cursorVisualColAbsolute - segStart + 1);
        ...
        if (isOnCursorLine && cursorVisualColAbsolute === cpLen(lineText)) {
            ...
            if (ghostText && showCursorOpt && ghostText.text.length > 0) {
                if (ghostText.showCursorBeforeText) {
                    renderedLine.push(_jsx(Text, { children: renderSoftwareCursor(' ') }, "ghost-cursor"));
                    renderedLine.push(_jsx(Text, { color: theme.text.secondary, children: ghostText.text }, "ghost-rest"));
                }
                else {
                    ...
                    renderedLine.push(_jsx(Text, { children: renderSoftwareCursor(firstChar) }, "ghost-cursor"));
                    if (rest.length > 0) {
                        renderedLine.push(_jsx(Text, { color: theme.text.secondary, children: rest }, "ghost-rest"));
                    }
                }
                renderedLine.push(_jsx(Text, { children: `\u200B` }, "ghost-zwsp"));
            }
            else {
                // Add zero-width space after cursor to prevent Ink from trimming trailing whitespace
                renderedLine.push(_jsx(Text, { children: showCursorOpt ? renderSoftwareCursor(' ') + '\u200B' : ' \u200B' }, `cursor-end-${cursorVisualColAbsolute}`));
            }
        }
        return _jsx(Text, { children: renderedLine });
    }, [slashCommands]);
    ...
    const shouldUseExportSuggestions = !commandSearchActive && !reverseSearchActive && !isHistoryRestoredText;
    const suggestionDisplayProps = shouldUseExportSuggestions && exportCompletion.suggestionDisplayProps
    ...
    const shouldShowSuggestions = (shouldUseExportSuggestions && exportCompletion.shouldShowSuggestions) ||
    ...
    const suggestionsFromExport = shouldUseExportSuggestions && !!exportCompletion.suggestionDisplayProps;
    ...
    const handleSuggestionSelect = useCallback((index) => {
        if (commandSearchActive || reverseSearchActive) {
            ...
            (isCommandSearch ? setCommandSearchActive : setReverseSearchActive)(false);
        }
        else {
            ...
            const accepted = index >= 0 && index < completion.suggestions.length
    ...
    const hasTabConsumer = shouldShowSuggestions ||
    ...
    const voiceStatusLabel = voiceInput.status === 'recording'
        ? t('Voice: recording')
        : voiceInput.status === 'transcribing'
            ...
            : voiceInput.status === 'refining'
                ? t('Voice: refining')
                : undefined;
    const prefixNode = (_jsxs(Text, { color: statusColor ?? theme.text.accent, "aria-label": statusText || undefined, children: [shellModeActive ? (reverseSearchActive ? (_jsxs(Text, { color: theme.text.link, "aria-label": SCREEN_READER_USER_PREFIX, children: ["(r:)", ' '] })) : ('!')) : commandSearchActive ? (_jsx(Text, { color: theme.text.accent, children: "(r:) " })) : approvalModePromptStyle ? (approvalModePromptStyle.prefix) : ('>'), ' '] }));
    ...
    const prefixWidth = shellModeActive
        ...
        : commandSearchActive
            ...
            : approvalMode === ApprovalMode.YOLO
                ? 2 // "* " = 2 chars
                : 2; // "> " = 2 chars
    return (_jsxs(_Fragment, { children: [attachments.length > 0 && (_jsxs(Box, { marginLeft: 2, marginBottom: 0, children: [_jsx(Text, { color: theme.text.secondary, children: t('Attachments: ') }), attachments.map((att, idx) => (_jsxs(Text, { color: isAttachmentMode && idx === selectedAttachmentIndex
                            ? theme.status.success
                            : theme.text.secondary, children: ["[", att.filename, "]", idx < attachments.length - 1 ? ' ' : ''] }, att.id)))] })), _jsx(VoiceIndicator, { status: voiceInput.status, interimText: voiceInput.interimText, audioLevel: voiceInput.audioLevel }), _jsx(BaseTextInput, { buffer: buffer, onSubmit: handleSubmitAndClear, onKeypress: handleInput, showCursor: showCursor, placeholder: availableSuggestion ?? placeholder, prefix: prefixNode, prefixWidth: prefixWidth, borderColor: borderColor, topRightLabel: voiceStatusLabel ?? uiState.sessionName ?? undefined, isActive: !isEmbeddedShellFocused, renderLine: renderLineWithHighlighting, mouseEnabled: mouseInteractionsEnabled }), shouldShowSuggestions && (_jsx(Box, { marginLeft: 2, marginRight: 2, children: _jsx(SuggestionsDisplay, { suggestions: suggestionDisplayProps.suggestions, activeIndex: suggestionDisplayProps.activeIndex, isLoading: suggestionDisplayProps.isLoading, width: suggestionsWidth, scrollOffset: suggestionDisplayProps.scrollOffset, userInput: buffer.text, mode: buffer.text.startsWith('/') &&
                        !reverseSearchActive &&
                        !commandSearchActive
                        ? 'slash'
                        : 'reverse', expandedIndex: expandedSuggestionIndex, mouseEnabled: mouseInteractionsEnabled, activeCategory: suggestionsFromExport ||
                        commandSearchActive ||
                        reverseSearchActive
                        ? undefined
                        : completion.activeCategory, availableCategories: categoryTabsVisible ? completion.availableCategories : undefined, onHoverIndex: suggestionsFromExport ? undefined : handleSuggestionHover, onSelectIndex: suggestionsFromExport ? undefined : handleSuggestionSelect }) })), attachments.length > 0 && !shouldShowSuggestions && (_jsx(Box, { marginLeft: 2, marginRight: 2, children: _jsx(Text, { color: theme.text.secondary, children: isAttachmentMode
