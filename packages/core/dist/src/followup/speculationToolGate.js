...
const WRITE_TOOLS = new Set([ToolNames.EDIT, ToolNames.WRITE_FILE]);
/** Tools that should always stop speculation */
const BOUNDARY_TOOLS = new Set([
    ToolNames.AGENT,
    ToolNames.SKILL,
 ...
 */
export async function evaluateToolCall(toolName, args, overlayFs, approvalMode, cwd) {
 ...
 */
async function resolveReadPaths(args, overlayFs) {
    for (const key of PATH_ARG_KEYS) {
        if (typeof args[key] === 'string') {
            args[key] = overlayFs.resolveReadPath(unescapePath(String(args[key]).trim()));
            return;
        }
    }
}
 ...
 * Mutates the args object in place.
 */
export async function rewritePathArgs(args, overlayFs) {
    for (const key of PATH_ARG_KEYS) {
        if (typeof args[key] === 'string') {
            args[key] = await overlayFs.redirectWrite(unescapePath(String(args[key]).trim()));
            return;
        }
    }
}
//# sourceMappingURL=speculationToolGate.js.map