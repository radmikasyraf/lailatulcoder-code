...
export const AgentTabBar = () => {
    ...
    const { setLivePanelFocused, setPillFocused } = useBackgroundTaskViewActions();
    const { embeddedShellFocused } = useUIState();
    const hasVisibleBgAgentRoster = () => bgEntries.some((e) => isLiveAgentPanelVisibleEntry(e, Date.now()));
    useKeypress((key) => {
    ...
    return (_jsxs(Box, { flexDirection: "row", paddingX: 1, children: [_jsx(Box, { marginRight: 1, children: _jsx(Text, { bold: activeView === 'main', dimColor: !isFocused, backgroundColor: activeView === 'main' ? theme.border.default : undefined, color: activeView === 'main' ? theme.text.primary : theme.text.secondary, children: ' Main ' }) }), _jsx(Text, { dimColor: !isFocused, color: theme.border.default, children: '\u2502' }), [...agents.entries()].map(([agentId, agent]) => {
                const isActive = activeView === agentId;
                const { symbol, color: indicatorColor } = statusIndicator(agent);
                return (_jsxs(Box, { marginLeft: 1, children: [_jsx(Text, { bold: isActive, dimColor: !isFocused, backgroundColor: isActive ? theme.border.default : undefined, color: isActive ? undefined : agent.color || theme.text.secondary, children: ` ${agent.modelId} ` }), _jsx(Text, { dimColor: !isFocused, color: indicatorColor, children: ` ${symbol}` })] }, agentId));
            }), _jsx(Box, { marginLeft: 2, children: _jsx(Text, { color: theme.text.secondary, children: hint }) })] }));
};
//# sourceMappingURL=AgentTabBar.js.map