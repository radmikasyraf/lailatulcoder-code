 ...
 */
import { type GoalEvidenceCatalog, type GoalEvidenceCheckpointWindow, type GoalEvidenceRecord } from './goal-evidence.js';
import { type GoalCheckpointVerifier } from './goal-checkpoint.js';
import { type GoalControlRequest, type GoalSnapshotV2, type GoalStateCause, type GoalStateRecordPayloadV2, type GoalStateResponse, type GoalTerminalProposal, type GoalTurnPermit, type TranscriptCursor } from './goal-protocol.js';
import type { GoalVerifier } from './goal-verifier.js';
import { type GoalRecoveryRecord } from './goal-persistence.js';
export declare const GOAL_RUNTIME_DISPOSED_MESSAGE = "Goal runtime has been disposed";
export declare const STALE_GOAL_TURN_MESSAGE = "Goal turn permit is no longer valid";
export interface GoalJournal {
    getTranscriptCursor(): TranscriptCursor;
    recordGoalState(recordUuid: string, payload: GoalStateRecordPayloadV2): Promise<unknown>;
}
...
export interface GoalRuntime {
    ...
    subscribe(listener: (snapshot: GoalSnapshotV2, cause?: GoalStateCause) => void): () => void;
    restore(records: readonly GoalRecoveryRecord[]): Promise<void>;
    prepareRestore(records: readonly GoalRecoveryRecord[], checkpointWindow?: GoalEvidenceCheckpointWindow): Promise<void>;
    ...
    recordTerminalProposal(permit: GoalTurnPermit, proposal: GoalTerminalProposal): GoalProposalReceipt;
    takePendingTerminalProposal(): GoalPendingProposal | undefined;
    dispose(): void;
}
export declare function createGoalRuntime(options: CreateGoalRuntimeOptions): GoalRuntime & {
    getSnapshotForPermit(permit: GoalTurnPermit): GoalSnapshotV2;
};
