...
import { addDaemonRequestAttribute, captureDaemonTelemetryContext, emitDaemonLog, runWithDaemonTelemetryContext, } from '@lailatul-coder/lailatul-coder-core';
import { mapDomainErrorToErrorKind } from '@lailatul-coder/acp-bridge';
import { writeStderrLine } from '../../utils/stdioHelpers.js';
import { SubscriberLimitExceededError, } from '@lailatul-coder/acp-bridge/eventBus';
import { errorMessage, } from '../server/error-response.js';
import { CLIENT_ID_RE, MAX_CLIENT_ID_LENGTH, parseLastEventId, parseMaxQueuedQuery, } from '../server/request-helpers.js';
...
import { parseVirtualSubagentSessionId, } from '../virtual-subagent-sessions.js';
let activeSseCount = 0;
const SSE_STREAM_ID_HEADER = 'X-LailatulCoder-SSE-Stream-Id';
// Keep in sync with the REST transport's response-header validator.
const SSE_STREAM_ID_RE = /^[0-9a-f]{8}-[0-9a-f]{4}-[1-8][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/i;
...
const SSE_CONNECT_REASON_SET = new Set(SSE_CONNECT_REASONS);
function parseSseConnectReason(raw) {
    return typeof raw === 'string' && SSE_CONNECT_REASON_SET.has(raw)
        ? raw
        : 'unknown';
}
function parsePreviousSseStreamId(raw) {
    return typeof raw === 'string' && SSE_STREAM_ID_RE.test(raw)
        ? raw.toLowerCase()
        : undefined;
}
function parseSseClientId(raw) {
...
function formatSseFrame(event) {
    ...
    const existingMeta = shaped._meta;
    const existingServerTimestamp = existingMeta?.['serverTimestamp'];
    const serverTimestamp = typeof existingServerTimestamp === 'number' &&
    ...
    const idLine = 'id' in shaped && shaped.id !== undefined ? `id: ${shaped.id}\n` : '';
    return `${idLine}event: ${shaped.type}\ndata: ${dataJson}\n\n`;
}
export function registerSseEventsRoutes(app, deps) {
    ...
    app.get('/session/:id/events', async (req, res) => {
        ...
        const clientId = parseSseClientId(req.headers['x-LailatulCoder-client-id']);
        const connectReason = parseSseConnectReason(req.query['connectReason']);
        const previousStreamId = parsePreviousSseStreamId(req.query['previousStreamId']);
        const telemetryContext = captureDaemonTelemetryContext();
        const telemetryBaseAttributes = {
            ...
            ...(previousStreamId
                ? {
                    'lailatul-coder.daemon.sse.client_reported_previous_stream_id': previousStreamId,
                }
                : {}),
        };
        const emitLifecycleLog = (eventName, body, attributes = {}) => {
            void runWithDaemonTelemetryContext(telemetryContext, async () => {
                emitDaemonLog(body, { ...telemetryBaseAttributes, ...attributes }, { eventName });
            }).catch(() => { });
        };
        let slowWarningCount = 0;
        let eventBusEvictionReason;
        const onSubscriberDiagnostic = (diagnostic) => {
            const triggerEventType = boundedDiagnosticString(diagnostic.data.triggerEventType);
            const common = {
            ...
            if (diagnostic.type === 'slow_client_warning') {
                ...
                try {
                    if (daemonLog) {
                        ...
                        writeStderrLine(`LailatulCoder serve: SSE slow client warning ${JSON.stringify(context)}`);
                    }
                    handled = true;
                }
                catch {
                ...
                emitLifecycleLog('lailatul-coder.daemon.sse.slow_client_warning', 'Daemon SSE slow client warning.', {
                    ...telemetryCommon,
                ...
                eventBusEvictionReason = boundedDiagnosticString(diagnostic.data.reason);
                const threshold = diagnostic.data.reason === 'queue_bytes_overflow'
                    ? 'bytes'
                    : 'frames';
                const context = {
                    sessionId: diagnosticSessionId,
                ...
                try {
                    if (daemonLog) {
                        ...
                        writeStderrLine(`LailatulCoder serve: SSE client evicted ${JSON.stringify(context)}`);
                    }
                    handled = true;
                }
                catch {
                ...
                emitLifecycleLog('lailatul-coder.daemon.sse.client_evicted', 'Daemon SSE client evicted.', {
                    ...telemetryCommon,
                    'lailatul-coder.daemon.sse.threshold': threshold,
                    'lailatul-coder.daemon.sse.event_bus_eviction_reason': eventBusEvictionReason,
                    'lailatul-coder.daemon.sse.dropped_after_event_id': diagnostic.data.droppedAfter,
                    ...(diagnostic.data.eventBytes !== undefined
                        ? {
                            'lailatul-coder.daemon.sse.rejected_event_bytes': diagnostic.data.eventBytes,
                        }
                        : {}),
                });
            }
        ...
        const eventEpoch = parseEventEpochHeader(req.headers['x-LailatulCoder-event-epoch']);
        ...
        try {
            ...
            if (err instanceof SubscriberLimitExceededError) {
                writeStderrLine(`LailatulCoder serve: subscriber limit reached for session ${diagnosticSessionId} (limit=${err.limit}); rejecting new SSE client with 429`);
        ...
        let lastEventIdWritten;
        let backpressureCount = 0;
        let maxDrainWaitMs = 0;
        let activeDrainStartedAt;
        ...
        const finalize = () => {
            ...
            if (activeDrainStartedAt !== undefined) {
                maxDrainWaitMs = Math.max(maxDrainWaitMs, performance.now() - activeDrainStartedAt);
            }
            const durationMs = Math.max(0, performance.now() - openedAt);
            const resolvedCloseReason = closeReason ?? terminalCandidate ?? 'client_disconnect';
            const closeAttributes = {
                'lailatul-coder.daemon.sse.duration_ms': durationMs,
                'lailatul-coder.daemon.sse.event_frames_write_settled': eventFramesWriteSettled,
                'lailatul-coder.daemon.sse.backpressure_count': backpressureCount,
                'lailatul-coder.daemon.sse.max_drain_wait_ms': maxDrainWaitMs,
                'lailatul-coder.daemon.sse.max_live_publish_to_write_settled_ms': maxLivePublishToWriteSettledMs,
            ...
            void runWithDaemonTelemetryContext(telemetryContext, async () => {
                ...
                emitDaemonLog('Daemon SSE stream closed.', { ...telemetryBaseAttributes, ...closeAttributes }, { eventName: 'lailatul-coder.daemon.sse.closed' });
            }).catch(() => { });
            try {
        ...
        emitLifecycleLog('lailatul-coder.daemon.sse.opened', 'Daemon SSE stream opened.', {
        ...
        const trackWriterIdle = writerIdleTimeoutMs !== undefined && writerIdleTimeoutMs > 0;
        ...
        if (trackWriterIdle) {
            ...
            const writerIdleTimeoutMsValue = writerIdleTimeoutMs;
            const checkIntervalMs = Math.max(250, Math.min(5_000, Math.floor(writerIdleTimeoutMsValue / 4)));
            idleTimer = setInterval(() => {
                if (res.writableEnded)
                    return;
                const idleForMs = Date.now() - lastWriteAt;
                if (idleForMs < writerIdleTimeoutMsValue)
                    return;
                ...
                // is bounded.
                try {
                    const settled = res.write(formatSseFrame({
                        v: 1,
                        type: 'client_evicted',
                        data: {
                ...
                try {
                    if (daemonLog) {
                        ...
                        writeStderrLine(`LailatulCoder serve: evicting SSE client (session ${diagnosticSessionId}) — ` +
                ...
                emitLifecycleLog('lailatul-coder.daemon.sse.client_evicted', 'Daemon SSE client evicted by writer idle timeout.', {
                    'lailatul-coder.daemon.sse.writer_idle_for_ms': idleForMs,
                    'lailatul-coder.daemon.sse.writer_idle_timeout_ms': writerIdleTimeoutMsValue,
        ...
        res.on('error', (err) => {
            ...
            try {
                if (daemonLog) {
                    ...
                    writeStderrLine(`LailatulCoder serve: SSE socket error (session ${diagnosticSessionId}): ${err.message} ` +
                        `(streamId=${streamId}${clientId ? `, clientId=${clientId}` : ''})`);
                }
            }
            catch {
            ...
            try {
                while (true) {
                    ...
                    if (next.value.type === 'client_evicted') {
                        ...
                        const reason = next.value.data
                            ?.reason;
                        if (typeof reason === 'string') {
                            eventBusEvictionReason = boundedDiagnosticString(reason);
                        }
                    }
                    else if (next.value.type === 'session_died' ||
                    ...
                    if (next.value.type === 'state_resync_required') {
                        ...
                        const gap = typeof data.earliestAvailableId === 'number' &&
                        ...
                        try {
                            if (daemonLog) {
                                daemonLog.warn('SSE ring eviction detected; consumer must call loadSession to recover', context);
                            }
                            else {
                                writeStderrLine(`LailatulCoder serve: SSE ring eviction detected (session ${diagnosticSessionId}): ` +
                                    `lastEventId=${data.lastDeliveredId ?? '?'}, ` +
                                    `earliestInRing=${data.earliestAvailableId ?? '?'}, ` +
                                    `gap=${gap ?? '?'} events, ` +
                        ...
                        emitLifecycleLog('lailatul-coder.daemon.sse.state_resync_required', 'Daemon SSE state resync required.', {
                            ...
                            ...(typeof data.lastDeliveredId === 'number'
                                ? {
                                    'lailatul-coder.daemon.sse.resync_last_delivered_id': data.lastDeliveredId,
                                }
                                : {}),
                            ...(typeof data.earliestAvailableId === 'number'
                                ? {
                                    'lailatul-coder.daemon.sse.resync_earliest_available_id': data.earliestAvailableId,
                                }
                                : {}),
                            ...(gap !== undefined
                    ...
                    const outcome = await writeWithBackpressure(formatSseFrame(next.value));
                    if (outcome === 'closed')
                        break;
                    eventFramesWriteSettled += 1;
                    ...
                    if (liveEvent &&
                        ...
                        maxLivePublishToWriteSettledMs = Math.max(maxLivePublishToWriteSettledMs, Math.max(0, Date.now() - serverTimestamp));
                    }
                ...
                if (!res.writableEnded && closeReason !== 'socket_error') {
                    ...
                    writeStderrLine(`LailatulCoder serve: bridge iterator error (session ${diagnosticSessionId}): ` +
                        `${errorMessage(err)}` +
                        (errorKind ? ` [${errorKind}]` : ''));
                    const outcome = await writeWithBackpressure(formatSseFrame({
                        v: 1,
                        type: 'stream_error',
                        data: {
