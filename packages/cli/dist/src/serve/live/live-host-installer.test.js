...
import { downloadLiveHostRelease, isExpectedLiveHostSignature, LiveHostInstaller, LIVE_HOST_DOWNLOAD_TIMEOUT_MS, LIVE_HOST_MANIFEST_FETCH_TIMEOUT_MS, LIVE_HOST_OSS_BASE_URL, LIVE_HOST_RELEASE_BASE_URL, parseLiveHostReleaseManifest, resolveLiveHostAssetUrls, resolveLiveHostManifestUrls, } from './live-host-installer.js';
...
function manifestForBytes(version, bytes) {
...
describe('LiveHostInstaller', () => {
    it('prefers the OSS mirror and retains the independent GitHub fallback', () => {
        expect(LIVE_HOST_OSS_BASE_URL).toBe('https://lailatul-coder-assets.oss-cn-hangzhou.aliyuncs.com/live-host');
        expect(LIVE_HOST_RELEASE_BASE_URL).toBe('https://github.com/LailatulCoder/lailatul-coder/releases/download/live-host-latest');
        ...
        expect(resolveLiveHostAssetUrls('0.1.0', 'Qwen-Live-Host-arm64.zip')).toEqual([
    ...
    it('falls back with a matching GitHub manifest and asset pair', async () => {
        ...
        const fetchImpl = vi
            ...
            .mockResolvedValueOnce(new Response(githubBytes, {
            headers: { 'content-length': String(githubBytes.byteLength) },
        }));
        const directory = await fsp.mkdtemp(path.join(os.tmpdir(), 'live-host-download-test-'));
        const destination = path.join(directory, 'host.zip');
        try {
            const release = await downloadLiveHostRelease('arm64', destination, () => { }, fetchImpl);
            ...
            expect(fetchImpl.mock.calls.map(([url]) => String(url))).toEqual([
                ...
                resolveLiveHostAssetUrls(ossManifest.version, ossManifest.assets.arm64.name)[0],
                resolveLiveHostManifestUrls()[1],
                resolveLiveHostAssetUrls(githubManifest.version, githubManifest.assets.arm64.name)[1],
            ]);
        }
        finally {
    ...
    it('removes a checksum-invalid OSS archive before falling back', async () => {
        ...
        const fetchImpl = vi
            .fn()
            .mockResolvedValueOnce(Response.json(ossManifest))
            .mockResolvedValueOnce(new Response(corruptOssBytes, {
    ...
    it('reports both source failures and removes the destination', async () => {
        ...
        const destination = path.join(directory, 'host.zip');
        try {
            await expect(downloadLiveHostRelease('arm64', destination, () => { }, fetchImpl)).rejects.toMatchObject({
                message: 'Qwen Live Host download failed. OSS: Live Host manifest download failed (503). GitHub: Live Host manifest download failed (503).',
                errors: [
    ...
    it('accepts only the Qwen Developer ID team', () => {
        expect(isExpectedLiveHostSignature([
            'Authority=Developer ID Application: Alibaba Cloud (Singapore) Private Limited (NF4574S59H)',
            'TeamIdentifier=NF4574S59H',
        ].join('\n'))).toBe(true);
        expect(isExpectedLiveHostSignature('Authority=Developer ID Application: Other\nTeamIdentifier=OTHER12345')).toBe(false);
        expect(isExpectedLiveHostSignature('Signature=adhoc\nTeamIdentifier=NF4574S59H')).toBe(false);
    });
    it('accepts only the fixed compatible release manifest', () => {
        expect(parseLiveHostReleaseManifest(manifest())).toEqual(manifest());
        expect(() => parseLiveHostReleaseManifest({
            ...manifest(),
    ...
    it('coalesces concurrent installs and exposes progress', async () => {
        let finish;
        const installLatest = vi.fn(async (_architecture, onStatus) => {
...
//# sourceMappingURL=live-host-installer.test.js.map