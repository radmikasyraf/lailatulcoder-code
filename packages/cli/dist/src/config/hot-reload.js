...
import { createDebugLogger, ApprovalMode, getMCPServerStatus, } from '@lailatul-coder/lailatul-coder-core';
import { assembleMcpServers } from './mcpServers.js';
import { getPendingGatedMcpServers, getPromptableMcpServers, } from './mcpApprovals.js';
 ...
 */
export function mcpServersEqual(a, b) {
    return equal(a ?? {}, b ?? {});
}
/**
 ...
 */
export function mcpGatingEqual(a, b) {
    const norm = (xs) => [...(xs ?? [])].sort();
    const allowedEqual = (a.allowed === undefined) === (b.allowed === undefined) &&
        equal(norm(a.allowed), norm(b.allowed));
    return (equal(norm(a.excluded), norm(b.excluded)) &&
        allowedEqual &&
 ...
 */
export function recomputeMcpGating(settings, assembled, cwd, bootAllowed, isYolo) {
 ...
 */
export function registerMcpHotReload(watcher, settings, config, topTierMcpServers) {
    debugLogger.debug('registered MCP hot-reload listener on SettingsWatcher');
    return watcher.addChangeListener(async (events) => {
        debugLogger.debug(`settings change fired (${events.length} event(s)): ${events
        ...
        // survive.
        const next = config.getBareMode() || config.isSafeMode()
            ? { ...topTierMcpServers }
            : assembleMcpServers(settings.merged.mcpServers, cwd, topTierMcpServers);
        ...
        const nextGating = config.getBareMode() || config.isSafeMode()
        ...
        debugLogger.debug(`assembled servers: prev=[${Object.keys(prevServers ?? {}).join(', ')}] next=[${Object.keys(next).join(', ')}]`);
        debugLogger.debug(`gating next: excluded=[${(nextGating.excluded ?? []).join(', ')}] allowed=[${(nextGating.allowed ?? []).join(', ')}] pending=[${(nextGating.pending ?? []).join(', ')}]`);
        ...
        if (!serversChanged && !gatingChanged) {
            debugLogger.debug('no MCP-relevant change (servers + gating unchanged) — skipping reconcile');
            return;
        }
        debugLogger.debug(`MCP-relevant change detected (serversChanged=${serversChanged} gatingChanged=${gatingChanged}) — reconciling`);
        ...
        try {
            ...
            const finalStatuses = Object.keys(next)
                ...
                .join(', ');
            debugLogger.debug(`reinitializeMcpServers resolved; final statuses=[${finalStatuses}]`);
        }
        catch (err) {
            // Keep the full stack on the debug channel for diagnosis…
            debugLogger.error(`reinitializeMcpServers threw: ${err instanceof Error ? (err.stack ?? err.message) : String(err)}`);
            ...
            appEvents.emit(AppEvent.LogError, 'Failed to reload MCP server settings; existing MCP state may be unchanged. Run with --debug for details.');
        }
        ...
        if (!isYolo && promptable.length > 0) {
            debugLogger.debug(`gated servers awaiting approval → emitting ${AppEvent.McpPendingApprovalChanged}: [${promptable.join(', ')}]`);
