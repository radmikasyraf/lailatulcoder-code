...
export function getLiveAgentPanelVpMaxRows(terminalHeight) {
    if (!Number.isFinite(terminalHeight) || terminalHeight <= 0) {
        return LIVE_AGENT_PANEL_VP_MIN_ROWS;
    }
    return Math.max(LIVE_AGENT_PANEL_VP_MIN_ROWS, Math.min(LIVE_AGENT_PANEL_VP_MAX_ROWS_CAP, Math.floor(terminalHeight / LIVE_AGENT_PANEL_VP_ROWS_PER_ROW)));
}
export function isLiveAgentPanelVisibleEntry(entry, nowMs) {
    if (entry.kind !== 'agent')
        return false;
 ...
 */
export function getLiveAgentPanelLayoutKey(entries, livePanelFocused) {
...
//# sourceMappingURL=liveAgentPanelVisibility.js.map