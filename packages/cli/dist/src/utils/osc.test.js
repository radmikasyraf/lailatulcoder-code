...
import { detectTerminal, osc, wrapForMultiplexer, oscITerm2Notify, oscKittyNotify, oscGhosttyNotify, sanitizeOscPayload, encodeKittyPayload, OSC, BEL, ST, } from './osc.js';
...
describe('wrapForMultiplexer', () => {
    ...
    it('wraps in DCS passthrough for tmux with ESC doubling', () => {
        ...
        expect(wrapForMultiplexer(seq)).toBe(`\x1bPtmux;\x1b\x1b]9;hello${BEL}\x1b\\`);
    });
...
describe('oscITerm2Notify', () => {
    ...
    it('formats notification with title and message', () => {
        process.env['TERM_PROGRAM'] = 'iTerm.app';
        delete process.env['TERM'];
        const result = oscITerm2Notify('LailatulCoder Ai', 'Hello');
        expect(result).toContain('LailatulCoder Ai:\nHello');
    });
...
describe('encodeKittyPayload', () => {
    it('base64-encodes ASCII text', () => {
        expect(encodeKittyPayload('Title')).toBe(Buffer.from('Title', 'utf8').toString('base64'));
    });
    it('base64-encodes UTF-8 text', () => {
        expect(encodeKittyPayload('你好')).toBe(Buffer.from('你好', 'utf8').toString('base64'));
    });
});
...
//# sourceMappingURL=osc.test.js.map