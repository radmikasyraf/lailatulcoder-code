 ...
 */
/**
 * Converter for Claude Code plugins to LailatulCoder Ai format.
 */
...
import { copyDirectory, isPathWithin, realPathWithin, } from './gemini-converter.js';
...
const claudeBuildInToolsTransform = (tools) => {
...
/**
 * Converts a Claude agent config to LailatulCoder Ai subagent format.
 * @param claudeAgent Claude agent configuration
 * @returns Converted agent config compatible with LailatulCoder Ai SubagentConfig
 */
...
`;
            ...
            debugLogger.warn(`[Claude Converter] Failed to convert agent file ${filePath}: ${error instanceof Error ? error.message : String(error)}`);
        }
    }
}
/**
 ...
 */
export function normalizeClaudeMcpServer(raw) {
    const server = raw;
 ...
 */
function normalizeClaudeMcpServers(servers) {
    const normalized = {};
...
/**
 * Converts a Claude plugin config to LailatulCoder Ai format.
...
export function convertClaudeToQwenConfig(claudeConfig) {
    ...
    if (claudeConfig.mcpServers) {
        if (typeof claudeConfig.mcpServers === 'string') {
            ...
            debugLogger.warn(`[Claude Converter] MCP servers path not yet supported: ${claudeConfig.mcpServers}`);
        }
        else {
    ...
    if (claudeConfig.outputStyles) {
        debugLogger.warn(`[Claude Converter] Output styles are not yet supported in ${claudeConfig.name}`);
    }
...
/**
 * Converts a complete Claude plugin package to LailatulCoder Ai format.
 ...
 */
export async function convertClaudePluginPackage(extensionDir, pluginName, networkPolicy, signal) {
    ...
    if (!realPathWithin(marketplaceJsonPath, extensionDir)) {
        throw new Error(`Marketplace configuration at ${marketplaceJsonPath} resolves through a symlink outside the plugin`);
    }
    ...
    const pluginDir = path.join(extensionDir, `plugin${createHash('sha256').update(`${extensionDir}/${pluginName}`).digest('hex')}`);
    await fs.promises.mkdir(pluginDir, { recursive: true });
    const { pluginSource, externalContent } = await resolvePluginSource(marketplacePlugin, extensionDir, pluginDir, networkPolicy, signal);
    ...
    const pluginJsonSafe = fs.existsSync(pluginJsonPath) &&
    ...
    if (pluginJsonSafe) {
        ...
        if (strict) {
            throw new Error(`Strict mode requires a trusted plugin.json at ${pluginJsonPath}`);
        }
        if (fs.existsSync(pluginJsonPath)) {
            debugLogger.warn(`Ignoring plugin.json at ${pluginJsonPath}; it resolves through a symlink outside the plugin.`);
        }
        mergedConfig = marketplacePlugin;
    }
    const converted = await buildQwenExtensionFromPlugin(pluginSource, mergedConfig);
 ...
 */
export function resolvePluginRelativeFile(pluginSource, relativePath) {
    if (path.isAbsolute(relativePath)) {
        debugLogger.warn(`Ignoring absolute path "${relativePath}" in plugin config; only paths inside the plugin are allowed.`);
        return null;
    }
    const resolved = path.resolve(pluginSource, relativePath);
    const base = path.resolve(pluginSource);
    if (!isPathWithin(resolved, base)) {
        debugLogger.warn(`Ignoring path "${relativePath}" in plugin config; it escapes the plugin directory.`);
        return null;
    }
    ...
    if (fs.existsSync(resolved) && !realPathWithin(resolved, pluginSource)) {
        debugLogger.warn(`Ignoring path "${relativePath}" in plugin config; it resolves through a symlink outside the plugin directory.`);
        return null;
    }
    return resolved;
}
 ...
 * conversion paths.
 */
export async function buildQwenExtensionFromPlugin(pluginSource, mergedConfig) {
    // Resolve MCP servers from a JSON file path if needed.
    if (mergedConfig.mcpServers && typeof mergedConfig.mcpServers === 'string') {
        const mcpServersPath = resolvePluginRelativeFile(pluginSource, mergedConfig.mcpServers);
        if (mcpServersPath && fs.existsSync(mcpServersPath)) {
            try {
                const mcpContent = fs.readFileSync(mcpServersPath, 'utf-8');
                mergedConfig.mcpServers = JSON.parse(mcpContent);
            }
            catch (error) {
                debugLogger.warn(`Failed to parse MCP servers file ${mcpServersPath}: ${error instanceof Error ? error.message : String(error)}`);
            }
        }
    }
    ...
    try {
        ...
        if (mergedConfig.hooks && typeof mergedConfig.hooks === 'string') {
            const hooksPath = resolvePluginRelativeFile(pluginSource, mergedConfig.hooks);
            if (hooksPath && fs.existsSync(hooksPath)) {
                try {
                    ...
                    debugLogger.warn(`Failed to parse hooks file ${hooksPath}: ${error instanceof Error ? error.message : String(error)}`);
                }
            }
        }
...
/**
 * Converts a standalone Claude plugin to LailatulCoder Ai format. A standalone plugin
 ...
 */
export async function convertClaudePluginStandalone(extensionDir) {
    const pluginJsonPath = path.join(extensionDir, '.claude-plugin', 'plugin.json');
    if (!fs.existsSync(pluginJsonPath)) {
    ...
    if (!realPathWithin(pluginJsonPath, extensionDir)) {
        throw new Error(`Plugin configuration at ${pluginJsonPath} resolves through a symlink outside the plugin`);
    }
    const parsedConfig = JSON.parse(fs.readFileSync(pluginJsonPath, 'utf-8'));
    ...
    if (typeof parsedConfig !== 'object' ||
        ...
        throw new Error(`Invalid plugin configuration at ${pluginJsonPath}: expected a JSON object`);
    }
    const mergedConfig = parsedConfig;
        ...
        if (fs.existsSync(mcpJsonPath) &&
            ...
            try {
                ...
                if (parsed?.mcpServers &&
                    ...
                    mergedConfig.mcpServers = parsed.mcpServers;
                }
                else {
                    debugLogger.warn(`.mcp.json at ${mcpJsonPath} has no valid "mcpServers" object; skipping.`);
                }
            }
            catch (error) {
                debugLogger.warn(`Failed to parse .mcp.json at ${mcpJsonPath}: ${error instanceof Error ? error.message : String(error)}`);
            }
        }
            ...
            debugLogger.warn(`Ignoring .mcp.json at ${mcpJsonPath}; it resolves through a symlink outside the plugin.`);
        }
    }
 ...
 */
async function collectResources(resourcePaths, pluginRoot, destDir) {
    ...
    for (const resourcePath of paths) {
        ...
        if (stat.isDirectory()) {
            ...
            const finalDestDir = dirName === destFolderName ? destDir : path.join(destDir, dirName);
            ...
            for (const file of files) {
                ...
                try {
                    ...
                    if (fileLstat.isSymbolicLink() &&
                        ...
                        debugLogger.warn(`Skipping symlink that escapes the plugin: ${srcFile}`);
                        continue;
                    }
 ...
 */
export function mergeClaudeConfigs(marketplacePlugin, pluginConfig) {
 ...
 */
export function isClaudePluginConfig(extensionDir, marketplace) {
    ...
    const marketplaceConfigObj = marketplaceConfig;
    // Must have name and owner
    if (typeof marketplaceConfigObj['name'] !== 'string' ||
    ...
    const marketplacePluginObj = marketplaceConfigObj['plugins'].find((plugin) => plugin.name === marketplace.pluginName);
 ...
 */
async function resolvePluginSource(pluginConfig, marketplaceDir, pluginDir, networkPolicy, signal) {
    signal?.throwIfAborted();
    ...
    if (typeof source === 'string') {
        ...
        if (resolvedSource !== marketplaceBase &&
            !resolvedSource.startsWith(marketplaceBase + path.sep)) {
            throw new Error(`Plugin source "${sanitizeForError(source)}" escapes the marketplace directory`);
        }
        if (!fs.existsSync(sourcePath)) {
            throw new Error(`Plugin source not found at ${sanitizeForError(sourcePath)}`);
        }
        ...
        if (!realPathWithin(sourcePath, marketplaceDir)) {
            throw new Error(`Plugin source "${sanitizeForError(source)}" resolves through a symlink outside the marketplace directory`);
        }
    ...
    if (source.source === 'git-subdir') {
        ...
        if (!source.path || source.path === '.' || path.isAbsolute(source.path)) {
            throw new Error(`Invalid plugin subdirectory "${sanitizeForError(String(source.path))}" for ${sanitizeForError(source.url)}`);
        }
        const subDir = path.resolve(pluginDir, source.path);
        const repoRoot = path.resolve(pluginDir);
        if (!subDir.startsWith(repoRoot + path.sep)) {
            throw new Error(`Plugin subdirectory "${sanitizeForError(source.path)}" escapes the repository root of ${sanitizeForError(source.url)}`);
        }
        if (!fs.existsSync(subDir)) {
            throw new Error(`Plugin subdirectory "${sanitizeForError(source.path)}" not found in ${sanitizeForError(source.url)} (ref: ${sanitizeForError(source.ref ?? source.sha ?? 'HEAD')})`);
        }
        ...
        if (!realPathWithin(subDir, pluginDir)) {
            throw new Error(`Plugin subdirectory "${sanitizeForError(source.path)}" resolves through a symlink outside the repository root of ${sanitizeForError(source.url)}`);
        }
        return { pluginSource: subDir, externalContent: true };
    }
    throw new Error(`Unsupported plugin source type: ${JSON.stringify(source)}`);
}
//# sourceMappingURL=claude-converter.js.map