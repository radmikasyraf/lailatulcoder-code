 ...
 */
import { getAutoSkillCuratorStatus, restoreArchivedAutoSkill, runAutoSkillCurator, setAutoSkillPinned, } from '@lailatul-coder/lailatul-coder-core';
...
function mutationGuard(config) {
    if (config.isSafeMode()) {
        return message(t('Auto-skill curator changes are disabled in safe mode.'), 'error');
    }
    if (!config.isTrustedFolder()) {
        return message(t('Auto-skill curator changes are only available in trusted workspaces. Trust this folder via `/trust` and try again.'), 'error');
    }
    return undefined;
}
...
function formatStatus(status) {
    ...
    const pinned = [...status.active, ...status.stale].filter((entry) => entry.pinned);
    if (pinned.length > 0) {
...
function formatRun(result) {
    ...
    if (result.archived.length > 0) {
        lines.push('', result.dryRun ? t('Archive candidates:') : t('Archived skills:'));
        lines.push(...result.archived.map((name) => `  ${name}`));
    }
    if (result.skippedCollisions.length > 0) {
        lines.push('', t('Skipped archive collisions:'));
        lines.push(...result.skippedCollisions.map((name) => `  ${name} — remove or rename .lailatulcoder/archived-skills/${name} to re-archive`));
    }
...
async function statusAction(context) {
    ...
    if (!config)
        return message(t('Config not loaded.'), 'error');
    try {
        return message(formatStatus(await getAutoSkillCuratorStatus(config.getProjectRoot())));
    }
    catch (error) {
        return message(t('Failed to read auto-skill curator status: {{message}}', {
...
const runCommand = {
    ...
    action: async (context, args) => {
        ...
        try {
            ...
            return message(t('Failed to run auto-skill curator: {{message}}', {
                message: error instanceof Error ? error.message : String(error),
            }), 'error');
        }
    },
...
const restoreCommand = {
    ...
    action: async (context, args) => {
        ...
        try {
            ...
            await refreshSkillCache(config);
            return message(t('Restored auto-skill: {{name}}', { name: directoryName }));
        }
        catch (error) {
            return message(t('Failed to restore auto-skill: {{message}}', {
                message: error instanceof Error ? error.message : String(error),
            }), 'error');
        }
    },
        ...
        action: async (context, args) => {
            ...
            if (!directoryName) {
                return message(pinned
                    ...
                    : t('Usage: /curator unpin <directory>'), 'error');
            }
            const blocked = mutationGuard(config);
            if (blocked)
                return blocked;
            try {
                await setAutoSkillPinned(config.getProjectRoot(), directoryName, pinned);
                return message(pinned
                    ? t('Pinned auto-skill: {{name}}', { name: directoryName })
                    : t('Unpinned auto-skill: {{name}}', { name: directoryName }));
            }
            catch (error) {
                return message(t('Failed to update auto-skill pin: {{message}}', {
                    message: error instanceof Error ? error.message : String(error),
                }), 'error');
            }
        },
...
//# sourceMappingURL=curator-command.js.map