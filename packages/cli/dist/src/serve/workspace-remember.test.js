...
function buildBridgeStub(opts) {
    ...
    const rememberImpl = opts.rememberImpl ??
        (async () => ({
            summary: 'saved',
            filesTouched: ['/mem/project/MEMORY.md'],
            touchedScopes: ['project'],
        }));
    const forgetImpl = opts.forgetImpl ??
        (async () => ({
            summary: 'forgot',
            removedEntries: [
                {
    ...
    const dreamImpl = opts.dreamImpl ??
        (async () => ({
            summary: 'dreamed',
            touchedTopics: ['project'],
            dedupedEntries: 1,
    ...
    return {
        ...
        async runWorkspaceMemoryRemember(req) {
            rememberCalls.push(req);
            return rememberImpl(req);
        },
        async runWorkspaceMemoryForget(req) {
        ...
        async isWorkspaceMemoryRememberAvailable() {
            if (opts.availableImpl)
                return opts.availableImpl();
        ...
        queryWorkspaceStatus: async (_method, idle) => idle(),
...
function buildApp(bridge, auth = {
    tokenConfigured: true,
    requireAuth: false,
}, lane = new WorkspaceRememberTaskLane(bridge), routeOverrides = {}) {
    const app = express();
...
describe('workspace memory remember routes', () => {
    it('routes qualified remember tasks to the selected workspace lane', async () => {
        ...
        const secondaryLane = new WorkspaceRememberTaskLane(secondary, '/work/secondary');
        const app = express();
        app.use(express.json({ limit: '1mb' }));
    ...
    it('queues and completes a hidden workspace remember task', async () => {
        ...
        expect(taskId).toMatch(/^remember-[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/);
    ...
    it('validates content, contextMode, client id, and unknown task ids', async () => {
        ...
        await request(app)
            ...
            .expect(202);
        await waitFor(() => bridge.rememberCalls.length === 1);
        expect(bridge.rememberCalls[0]?.content).toBe('x'.repeat(MAX_REMEMBER_CONTENT_BYTES));
        await request(app)
            .post('/workspace/memory/remember')
    ...
    it('rolls back the enqueue gate and fails queued tasks on disposal', async () => {
        ...
        lane.beginDrain();
        expect(() => lane.enqueue({ content: 'blocked', contextMode: 'workspace' })).toThrow(WorkspaceDrainingError);
        lane.cancelDrain();
    ...
    it('serializes remember, forget, and dream tasks in one lane', async () => {
        const remember = deferred();
        const forget = deferred();
        const dream = deferred();
        const starts = [];
        const bridge = buildBridgeStub({
    ...
    it.each(['remember', 'forget', 'dream'])('rejects untrusted %s task reads before task lookup', async (kind) => {
        const bridge = buildBridgeStub({});
        const app = buildApp(bridge, undefined, undefined, {
            isWorkspaceTrusted: () => false,
        });
        const response = await request(app).get(`/workspace/memory/${kind}/missing`);
        expect(response.status).toBe(403);
        expect(response.body.code).toBe('untrusted_workspace');
    });
    it.each(['remember', 'forget', 'dream'])('rejects closed-generation %s task reads before task lookup', async (kind) => {
        const bridge = buildBridgeStub({});
        const app = buildApp(bridge, undefined, undefined, {
            captureGenerationAssertion: () => () => {
    ...
    it('records bridge failures with stable public error codes', async () => {
        const bridge = buildBridgeStub({
            rememberImpl: vi
                .fn()
                .mockRejectedValueOnce(Object.assign(new Error('agent wrote /tmp/outside'), {
    ...
    it('logs sanitized details for task-lane failures', async () => {
        ...
        const bridge = buildBridgeStub({
            rememberImpl: vi
                .fn()
                .mockRejectedValue(new Error('Authorization: Bearer secret-token-value')),
        });
        const app = buildApp(bridge);
        const post = await request(app)
            .post('/workspace/memory/remember')
        ...
        expect(mockDebugLogger.error).toHaveBeenCalledWith('Workspace memory remember task failed:', expect.objectContaining({
    ...
    it('records forget and dream failures with kind-specific error codes', async () => {
        ...
        expect(mockDebugLogger.error).toHaveBeenCalledWith('Workspace memory forget task failed:', expect.objectContaining({
        ...
        expect(mockDebugLogger.error).toHaveBeenCalledWith('Workspace memory dream task failed:', expect.objectContaining({
...
//# sourceMappingURL=workspace-remember.test.js.map