...
const cwdOption = {
    type: 'string',
    describe: 'Workspace directory the channel runs in (defaults to the current directory)',
    default: '.',
};
export const pairingListCommand = {
    command: 'list <name>',
    ...
    handler: (argv) => {
        ...
        if (pending.length === 0) {
            writeStdoutLine('No pending pairing requests in this workspace (pass --cwd <dir> if the channel runs elsewhere).');
            return;
        }
        writeStdoutLine(`Pending pairing requests for "${argv.name}":\n`);
        for (const req of pending) {
            const ago = Math.round((Date.now() - req.createdAt) / 60000);
            const subject = req.subject.type === 'group'
...
export const pairingApproveCommand = {
    ...
    handler: (argv) => {
        ...
        if (!request) {
            writeStderrLine(`No pending request found for code "${argv.code.toUpperCase()}" in this workspace. It may have expired, or the channel may run in a different workspace (pass --cwd <dir>).`);
            process.exit(1);
            return; // process.exit is mocked in tests; never fall through
        }
        const approved = request.subject.type === 'group'
            ? `group ${request.subject.name} (${request.subject.id})`
            : `${request.subject.name} (${request.subject.id})`;
        writeStdoutLine(`Approved: ${approved} can now use channel "${argv.name}".`);
    },
};
