...
export function registerCapabilitiesRoutes(app, deps) {
    app.get('/capabilities', (_req, res) => {
        ...
        const activePrimary = entries.find((entry) => entry.primary && entry.state === 'active')?.current?.runtime;
        ...
        const envelope = {
            ...
            policy: {
                permission: activePrimary?.bridge.permissionPolicy ?? deps.permissionPolicy,
            },
            limits: {
            ...
            workspaces: entries.map((entry) => ({
                ...
                trusted: entry.state === 'active' && entry.current?.runtime.trusted === true,
