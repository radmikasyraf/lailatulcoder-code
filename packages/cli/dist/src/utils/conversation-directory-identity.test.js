...
import { ConversationDirectoryIdentityError, createConversationRootIdentity, getConversationDirectoryName, inspectConversationDirectoryIdentity, materializeConversationDirectoryIdentity, revalidateConversationRootIdentity, } from './conversation-directory-identity.js';
// fs-interception seam: lets a test commit a rename exactly between two fs
// calls inside the module under test.
vi.mock('node:fs/promises', { spy: true });
const realFsPromises = await vi.importActual('node:fs/promises');
const cleanup = [];
afterEach(async () => {
    await Promise.all(cleanup.splice(0).map((path) => rm(path, { recursive: true, force: true })));
});
async function tempRoot() {
    const base = await mkdtemp(join(realpathSync.native(tmpdir()), 'qwen-conversation-identity-'));
    cleanup.push(base);
...
describe('conversation directory identity', () => {
    it('derives a deterministic case-sensitive child name', () => {
        expect(getConversationDirectoryName('session-a')).toMatch(/^conversation-[a-f0-9]{64}$/);
        expect(getConversationDirectoryName('session-a')).toBe(getConversationDirectoryName('session-a'));
        expect(getConversationDirectoryName('SESSION-A')).not.toBe(getConversationDirectoryName('session-a'));
        expect(() => getConversationDirectoryName('')).toThrowError(ConversationDirectoryIdentityError);
    });
    it('exposes an error cause only when one was provided', () => {
        ...
        const wrapped = new ConversationDirectoryIdentityError('child', 'io_error', cause);
    ...
    it('pins root and direct-child device and inode identity', async () => {
        ...
        const result = await materializeConversationDirectoryIdentity(root, 'session-a');
        ...
        await expect(inspectConversationDirectoryIdentity(root, 'session-a', result.identity)).resolves.toEqual(result.identity);
    });
    it('reports a missing child without guessing an identity', async () => {
        const { root } = await tempRoot();
        await expect(inspectConversationDirectoryIdentity(root, 'missing')).resolves.toBeUndefined();
    });
    it('rejects same-path replacement against an expected identity', async () => {
        const { root } = await tempRoot();
        const original = await materializeConversationDirectoryIdentity(root, 'replace');
        ...
        await expect(inspectConversationDirectoryIdentity(root, 'replace', original.identity)).rejects.toMatchObject({
            scope: 'child',
    ...
    it('does not follow a replacement symlink', async () => {
        ...
        await expect(inspectConversationDirectoryIdentity(root, 'replace')).rejects.toMatchObject({ scope: 'child', reason: 'not_directory' });
    });
    it('rejects permissive existing children without changing their mode', async () => {
        if (process.platform === 'win32')
            return;
        const { root } = await tempRoot();
        const original = await materializeConversationDirectoryIdentity(root, 'permissive');
        await chmod(original.identity.canonicalPath, 0o755);
        await expect(inspectConversationDirectoryIdentity(root, 'permissive')).rejects.toMatchObject({ scope: 'child', reason: 'wrong_mode' });
        expect((await lstat(original.identity.canonicalPath)).mode & 0o777).toBe(0o755);
    });
    it('rejects a replaced root identity', async () => {
        ...
        await expect(revalidateConversationRootIdentity(root)).rejects.toMatchObject({ scope: 'root', reason: 'identity_changed' });
    });
    it('rejects a root swap committed during child inspection', async () => {
        const { root } = await tempRoot();
        const created = await materializeConversationDirectoryIdentity(root, 'swapme');
        ...
        try {
            await expect(inspectConversationDirectoryIdentity(root, 'swapme')).rejects.toMatchObject({ scope: 'root', reason: 'identity_changed' });
        }
        finally {
    ...
    it('degrades instead of failing on a filesystem that reports no inode', async () => {
        ...
        try {
            ...
            await expect(revalidateConversationRootIdentity(root)).resolves.toMatchObject({ inodeVerifiable: false });
            const created = await materializeConversationDirectoryIdentity(root, 'inodeless');
            expect(created.identity.name).toBe(getConversationDirectoryName('inodeless'));
        }
        finally {
            vi.mocked(lstat).mockRestore();
        }
...
//# sourceMappingURL=conversation-directory-identity.test.js.map