...
export const FORK_AGENT = {
    ...
    description: 'Fork yourself — inherits parent conversation context. Selected explicitly via `subagent_type: "fork"`. Runs detached in the background; you are notified when it completes.',
    tools: ['*'],
...
const forkExecutionStorage = new AsyncLocalStorage();
export function runInForkContext(fn) {
    return forkExecutionStorage.run({ marker: true }, fn);
}
export function isInForkExecution() {
 ...
 */
export function resolveForkExecutionAllowedTools(advertisedToolNames, requestedToolNames) {
 ...
 */
export function registerForkDisplayImageForCache(config, advertisedToolNames) {
    if (!advertisedToolNames.includes(ToolNames.DISPLAY_IMAGE))
        return;
    config
        .getToolRegistry()
...
export function buildForkExecutionAllowlist(requestedTools, declaredTools) {
    return (requestedTools ?? declaredTools).filter((toolName) => toolName !== ToolNames.ASK_USER_QUESTION);
}
export function isValidForkToolWildcard(toolName) {
...
export function validateForkToolList(tools) {
    if (!Array.isArray(tools) ||
        tools.some((toolName) => typeof toolName !== 'string' ||
...
export function normalizeForkTurns(forkTurns) {
    return forkTurns === undefined || forkTurns === 'all'
        ? 'all'
        : Number(forkTurns);
}
function isSystemReminderPart(content, partIndex) {
 ...
 */
export function buildFunctionResponseParts(assistantMessage, placeholderOutput) {
    return (assistantMessage.parts?.filter((part) => part.functionCall) ?? []).map((part) => ({
        functionResponse: {
 ...
 */
export function selectForkHistory(history, forkTurns) {
    ...
    if (typeof forkTurns === 'number') {
        ...
        if (realUserTurnIndexes.length === 0) {
            ...
            selected = history.slice(realUserTurnIndexes[Math.max(0, realUserTurnIndexes.length - forkTurns)]);
        }
    }
 ...
 */
export function buildForkedMessages(directive, assistantMessage, executionAllowedTools, promptHint) {
    ...
    const fullAssistantMessage = {
        ...
        parts: (assistantMessage.parts || []).map((part) => part.functionCall
            ? {
                ...part,
                functionCall: {
                    id: part.functionCall.id,
    ...
    const toolResultParts = buildFunctionResponseParts(assistantMessage, FORK_PLACEHOLDER_RESULT);
 ...
 */
export function buildPinnedWorktreeNotice(worktreeCwd) {
    return (`Your working directory is ${worktreeCwd}, a git worktree checked out to the code you have been asked to work on. ` +
...
export function buildChildMessage(directive, executionAllowedTools, promptHint) {
    const executionRestriction = executionAllowedTools === undefined
        ? ''
