...
import { ensureAutoMemoryScaffold, ensureUserAutoMemoryScaffold, } from './store.js';
...
async function writeExtractCursor(projectRoot, cursor) {
    await atomicWriteFile(getAutoMemoryExtractCursorPath(projectRoot), `${JSON.stringify(cursor, null, 2)}\n`, { encoding: 'utf-8' });
}
...
export async function runAutoMemoryExtract(params) {
    ...
    // write access to `~/.lailatulcoder/memories/` should still be able to use
    ...
    try {
        ...
        debugLogger.warn(`User-level auto-memory scaffold failed (non-critical, will skip user-level writes this run): ${error instanceof Error ? error.message : String(error)}`);
    }
    if (!params.config) {
    ...
    const rawOffset = currentCursor.sessionId === params.sessionId
    ...
    const hasNewUserMessages = params.history
        .slice(startOffset)
        .some((m) => m.role === 'user' && partToString(m.parts ?? []).trim().length > 0);
    ...
    if (agentResult.touchedTopics.length > 0) {
        ...
        //   * user-level rebuild is best-effort. A read-only `~/.lailatulcoder/memories/`
        ...
        const projectRebuild = agentResult.touchedProjectScope || !agentResult.touchedUserScope
        ...
        const userRebuild = agentResult.touchedUserScope
            ? rebuildUserAutoMemoryIndex().catch((error) => {
                debugLogger.warn(`Auto-memory user-level index rebuild failed (non-critical, project-level rebuild unaffected): ${error instanceof Error ? error.message : String(error)}`);
            })
            : Promise.resolve();
    ...
    const madeGenuineProgress = agentResult.touchedTopics.length > 0 || agentResult.hasToolActivity;
    const cursor = {
        sessionId: params.sessionId,
