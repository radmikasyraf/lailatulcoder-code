...
const PARAMETER_PATTERN = /<parameter\s+name=["']([^"']+)["']>([\s\S]*?)<\/parameter>/g;
/**
 * Detects whether text contains XML-style tool call patterns.
 */
export function containsXmlToolCalls(text) {
 ...
 */
function stripDelimitingNewlines(value) {
    let result = value;
 ...
 */
function computeInvokeRanges(text) {
    const ranges = [];
    INVOKE_PATTERN.lastIndex = 0;
    let m;
 ...
 */
function positionInsideFence(text, index, invokeRanges) {
    let openFence = null;
    let lineStart = 0;
    for (const line of text.slice(0, index).split('\n')) {
        const lineEnd = lineStart + line.length;
        const insideInvoke = invokeRanges.some(([start, end]) => lineStart >= start && lineEnd <= end);
        lineStart = lineEnd + 1;
        if (insideInvoke)
            continue;
...
export function extractXmlToolCalls(text) {
    ...
    while ((match = INVOKE_PATTERN.exec(text)) !== null) {
        ...
        while ((paramMatch = PARAMETER_PATTERN.exec(paramsBlock)) !== null) {
            ...
            const paramValue = decodeXmlEntities(stripDelimitingNewlines(paramMatch[2]));
 ...
 */
export function tryRecoverXmlToolCalls(text) {
    ...
    const remainingText = text
        .replace(INVOKE_PATTERN, (block, _name, _body, offset) => {
        ...
        .replace(/\n{3,}/g, '\n\n')
        .trim();
    const functionCallParts = extracted.map((toolCall, index) => ({
        functionCall: {
...
//# sourceMappingURL=xml-tool-call-fallback.js.map