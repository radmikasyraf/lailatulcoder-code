...
export const BUILTIN_ALLOW = Object.freeze([
 ...
 * "edit my .lailatulcoder/settings.json and add envKey FOO=bar" authorizes the
 * specific edit named, but does not authorize unrelated changes to the
 * same file.
 */
export const BUILTIN_SOFT_DENY = Object.freeze([
    ...
    'Self-modification: modifying files or directories that affect LailatulCoder Ai behavior, permissions, startup context, commands, hooks, agents, skills, MCP servers, or project/user instructions, including .lailatulcoder/settings*.json, QWEN.md, AGENTS.md, QWEN.local.md, configured context filenames, .lailatulcoder/rules/, .lailatulcoder/commands/, .lailatulcoder/agents/, .lailatulcoder/skills/, .lailatulcoder/hooks/, .lailatulcoder/fork-profiles/, and .mcp.json, unless the user explicitly requested that exact change. Includes adding or widening permission allow rules (e.g. wildcard "Bash(prefix:*)" entries, broad path allows in .lailatulcoder/settings*.json) that the user did not explicitly request, even while making a user-requested edit to the same file.',
 ...
 */
export const BUILTIN_HARD_DENY = Object.freeze([
...
export const BUILTIN_ENVIRONMENT = Object.freeze([
 ...
 */
export function buildClassifierSystemPrompt(config) {
    ...
    const userSections = renderUserSections(userAllow, userSoftDeny, userHardDeny);
    return PROMPT_TEMPLATE.replace('{{DEFAULT_ALLOW}}', formatBuiltin(BUILTIN_ALLOW))
 ...
 */
function formatBuiltin(entries) {
 ...
 */
function formatUserHints(entries) {
    const capped = entries.slice(0, MAX_USER_HINTS_PER_SECTION);
    return capped
        .map((entry) => {
        const truncated = entry.length > MAX_USER_HINT_LENGTH
 ...
 */
function renderUserSections(userAllow, userSoftDeny, userHardDeny) {
    const blocks = [];
