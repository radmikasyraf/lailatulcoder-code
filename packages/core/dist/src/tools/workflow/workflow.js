...
import { DEFAULT_MAX_AGENTS_PER_RUN, DEFAULT_WORKFLOW_SUBAGENT_MAX_TIME_MINUTES, DEFAULT_WORKFLOW_SUBAGENT_MAX_TURNS, MAX_WORKFLOW_AGENTS_ENV, MAX_WORKFLOW_CONCURRENCY_ENV, WORKFLOW_SUBAGENT_MAX_MINUTES_ENV, WORKFLOW_SUBAGENT_MAX_TURNS_ENV, } from '../../agents/runtime/workflow-orchestrator.js';
import { DEFAULT_STALL_MS, MAX_STALL_ATTEMPTS, MAX_WORKFLOW_STALL_MS_ENV, } from '../../agents/runtime/workflow-stall.js';
...
const WORKFLOW_PARAM_SCHEMA = {
    ...
    properties: {
        script: {
            type: 'string',
            description: 'JavaScript source of the workflow. Wrapped as an async IIFE. ' +
                ...
                '(`.lailatulcoder/agents/<name>.md`, project then user then built-in). Unresolved ' +
                ...
                '`<projectRoot>/.lailatulcoder/worktrees/agent-<7hex>`; the worktree is auto-removed ' +
        ...
        scriptPath: {
            type: 'string',
            description: 'Optional. Absolute path to a saved workflow `.js` file to load and ' +
        ...
        resumeFromRunId: {
            type: 'string',
            description: 'Optional. Resume a prior workflow run by id (e.g. wf_abc123…). ' +
    ...
    async execute(signal, updateOutput, _shellExecutionConfig) {
        ...
        try {
            handle = await WorkflowRunner.start({
                ...
                runInBackground,
                onUpdate: !runInBackground && updateOutput
        ...
        if (runInBackground) {
            ...
            const usageBanner = resolveUsageBanner(this.config, handle.registry, handle.budget.total);
            return {
        ...
        if (settlement.ok) {
            ...
            const display = phases || logs || meta
...
function resolveUsageBanner(config, registry, budgetTotal) {
    if (!registry)
        return '';
    if (config.getSkipWorkflowUsageWarning?.())
        return '';
    if (!registry.shouldShowUsageWarning())
        return '';
...
function safeStringifyResult(result) {
    if (result === undefined)
        return '(workflow returned no value)';
...
export class WorkflowTool extends BaseDeclarativeTool {
    ...
    constructor(config, toolOptions = {}) {
        super(ToolNames.WORKFLOW, ToolDisplayNames.WORKFLOW, WORKFLOW_TOOL_DESCRIPTION, Kind.Other, WORKFLOW_PARAM_SCHEMA, 
