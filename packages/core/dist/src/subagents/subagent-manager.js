...
import { BUBBLE_APPROVAL_MODE, SubagentError, SubagentErrorCode, } from './types.js';
...
import { COLOR_VALUES, isColor, isPermissionMode, parseAgentHooks, parseAgentMcpServers, parseMaxTurns, claudePermissionModeToApprovalMode, } from './agent-frontmatter-schema.js';
import { ToolDisplayNamesMigration, ToolNames } from '../tools/tool-names.js';
import { QWEN_DIR, Storage } from '../config/storage.js';
import { hasRebuiltToolRegistry, rebuildToolRegistryOnOverride, } from '../tools/agent/agent.js';
 ...
 */
function isSubagentApprovalMode(mode) {
    return (APPROVAL_MODES.includes(mode) ||
        mode === BUBBLE_APPROVAL_MODE);
}
/** Human-readable list of valid subagent approval modes, for error messages. */
function subagentApprovalModesLabel() {
...
export class SubagentManager {
    ...
    applyBuiltinSettings(config) {
        if (config.name !== 'Explore') {
            return config;
        }
        const configuredModel = this.config.getAgentsSettings().builtin?.exploreModel;
    ...
    resolveModelGrade(grade, agentConfig) {
        const configuredModel = agentConfig.model?.trim();
        if (!agentConfig.isBuiltin &&
            configuredModel &&
    ...
    getAvailableModelGrades() {
        ...
        return new Map(Object.entries(modelGrades).flatMap(([grade, model]) => {
            const normalizedGrade = grade.trim();
            return normalizedGrade !== '' &&
                typeof model === 'string' &&
                model.trim() !== '' &&
                (allowedGrades === undefined ||
                    allowedGrades.some((allowedGrade) => typeof allowedGrade === 'string' &&
                        allowedGrade.trim() === normalizedGrade))
                ? [[normalizedGrade, model.trim()]]
                : [];
        }));
    }
    getBuiltinAgent(name) {
        const config = BuiltinAgentRegistry.getBuiltinAgent(name);
        return config ? this.applyBuiltinSettings(config) : null;
    }
    getBuiltinAgents() {
        return BuiltinAgentRegistry.getBuiltinAgents().map((config) => this.applyBuiltinSettings(config));
    }
    /**
     ...
     */
    async createSubagent(config, options) {
        this.validator.validateOrThrow(config);
        // Prevent creating session-level agents
        if (options.level === 'session') {
            throw new SubagentError(`Cannot create session-level subagent "${config.name}". Session agents are read-only and provided at runtime.`, SubagentErrorCode.INVALID_CONFIG, config.name);
        }
        // Determine file path
        const filePath = options.customPath || this.getSubagentPath(config.name, options.level);
        // Check if file already exists
        if (!options.overwrite) {
            try {
                await fs.access(filePath);
                throw new SubagentError(`Subagent "${config.name}" already exists at ${filePath}`, SubagentErrorCode.ALREADY_EXISTS, config.name);
            }
            catch (error) {
        ...
        try {
            ...
            throw new SubagentError(`Failed to write subagent file: ${error instanceof Error ? error.message : 'Unknown error'}`, SubagentErrorCode.FILE_ERROR, config.name);
        }
    }
    /**
    ...
    async loadSubagent(name, level) {
        ...
        if (level) {
            ...
            if (level === 'session') {
                ...
                return (sessionSubagents.find((agent) => agent.name.toLowerCase() === lowerName) || null);
            }
            return this.findSubagentByNameAtLevel(name, level);
        }
        // Try session level first (highest priority for runtime)
        const sessionSubagents = this.subagentsCache?.get('session') || [];
        const sessionConfig = sessionSubagents.find((agent) => agent.name.toLowerCase() === lowerName);
        if (sessionConfig) {
        ...
        // Try extension level
        const extensionConfig = await this.findSubagentByNameAtLevel(name, 'extension');
        if (extensionConfig) {
     ...
     */
    async updateSubagent(name, updates, level, options) {
        const existing = await this.loadSubagent(name, level);
        if (!existing) {
            throw new SubagentError(`Subagent "${name}" not found`, SubagentErrorCode.NOT_FOUND, name);
        }
        // Prevent updating built-in agents
        if (existing.isBuiltin) {
            throw new SubagentError(`Cannot update built-in subagent "${name}"`, SubagentErrorCode.INVALID_CONFIG, name);
        }
        // Prevent updating session-level agents
        if (existing.level === 'session') {
            throw new SubagentError(`Cannot update session-level subagent "${name}"`, SubagentErrorCode.INVALID_CONFIG, name);
        }
        if (existing.level === 'extension') {
            throw new SubagentError(`Cannot update extension-provided subagent "${name}"`, SubagentErrorCode.INVALID_CONFIG, name);
        }
        ...
        if (!existing.filePath) {
            throw new SubagentError(`Cannot update subagent "${name}": no file path available`, SubagentErrorCode.FILE_ERROR, name);
        }
        ...
        try {
            ...
            throw new SubagentError(`Failed to update subagent file: ${error instanceof Error ? error.message : 'Unknown error'}`, SubagentErrorCode.FILE_ERROR, name);
        }
    }
    /**
     ...
     */
    async deleteSubagent(name, level, extensionName, options) {
        // Check if it's a built-in agent first
        if (BuiltinAgentRegistry.isBuiltinAgent(name)) {
            throw new SubagentError(`Cannot delete built-in subagent "${name}"`, SubagentErrorCode.INVALID_CONFIG, name);
        }
        if (level === 'extension') {
            throw new SubagentError(`Cannot delete subagent "${name}" in extension "${extensionName}", If needed, you can directly uninstall extension.`, SubagentErrorCode.INVALID_CONFIG, name);
        }
        const levelsToCheck = level
            ? [level]
            : ['project', 'user'];
        let deleted = false;
    ...
    async listSubagents(options = {}) {
        ...
        const defaultLevels = this.config.isSafeMode()
            ? ['builtin']
            : ['project', 'user', 'builtin', 'extension'];
        if (this.config.isSafeMode() &&
            options.level &&
            options.level !== 'builtin') {
            debugLogger.debug(`Safe mode: overriding requested level '${options.level}' to 'builtin'`);
        }
        const levelsToCheck = options.level
    ...
    async refreshCache() {
        ...
        const levels = this.config.isSafeMode()
            ? ['builtin']
            : ['project', 'user', 'builtin', 'extension'];
     ...
     * @returns SubagentConfig or null if not found
     */
    async findSubagentByName(name, level) {
    ...
    async parseSubagentFile(filePath, level) {
        ...
        try {
            ...
            throw new SubagentError(`Failed to read subagent file: ${error instanceof Error ? error.message : 'Unknown error'}`, SubagentErrorCode.FILE_ERROR);
        }
     ...
     * @throws SubagentError if parsing fails
     */
    parseSubagentContent(content, filePath, level) {
     ...
     */
    async createAgentHeadless(config, runtimeContext, options) {
        ...
        let unregisterAgentHooks;
        let disposeSubagentRegistry;
        const runCleanup = async () => {
            if (unregisterAgentHooks) {
                try {
                    ...
                    debugLogger.warn(`Subagent "${config.name}": failed to unregister per-agent hooks: ${error instanceof Error ? error.message : String(error)}`);
                }
            }
            if (disposeSubagentRegistry) {
                try {
                    ...
                    debugLogger.warn(`Subagent "${config.name}": failed to stop per-agent ToolRegistry: ${error instanceof Error ? error.message : String(error)}`);
                }
            }
        };
        try {
            const runtimeConfig = await this.convertToRuntimeConfig(config, runtimeContext);
            const promptConfig = {
            ...
            const configuredToolConfig = options?.toolConfigOverride ?? runtimeConfig.toolConfig;
            const toolConfig = {
            ...
            const runtimeView = await this.buildRuntimeContentGeneratorView(config, runtimeContext, modelConfig.model, options?.runtimeAuthOverrides);
            const { context: subagentContext, cleanup } = await this.buildSubagentContextOverride(runtimeContext, config);
            ...
            if (config.hooks && Object.keys(config.hooks).length > 0) {
                if (hookRegistry) {
                    ...
                    unregisterAgentHooks = hookRegistry.addAgentHooks(config.hooks, agentScope);
                }
                else {
                    ...
                    debugLogger.warn(`Subagent "${config.name}" declares hooks but the host has no HookSystem; ignoring per-agent hooks.`);
                }
            }
            try {
                const subagent = await AgentHeadless.create(config.name, subagentContext, promptConfig, modelConfig, runConfig, toolConfig, options?.eventEmitter, options?.hooks, runtimeView);
            ...
            if (error instanceof Error) {
                throw new SubagentError(`Failed to create AgentHeadless: ${error.message}`, SubagentErrorCode.INVALID_CONFIG, config.name);
            }
            throw error;
        }
    }
     ...
     */
    async buildSubagentContextOverride(runtimeContext, config) {
        ...
        const hasAgentMcpServers = config.mcpServers && Object.keys(config.mcpServers).length > 0;
        ...
        if (hasAgentMcpServers && config.mcpServers) {
            ...
            const results = await Promise.allSettled(serverNames.map((name) => subagentRegistry.discoverToolsForServer(name)));
            for (let i = 0; i < results.length; i++) {
                const r = results[i];
                if (r.status === 'rejected') {
                    debugLogger.warn(`Failed to discover per-agent MCP server "${serverNames[i]}" for subagent "${config.name}": ${r.reason instanceof Error ? r.reason.message : String(r.reason)}`);
                }
            }
            return {
     ...
     * view is built here.
     */
    async buildRuntimeContentGeneratorView(config, base, fallbackModelId, runtimeAuthOverrides) {
        const resolvedModel = this.resolveModelOverride(config.model, base);
        const modelId = resolvedModel?.modelId ?? fallbackModelId;
        if (!modelId) {
            return undefined;
        }
        const authType = resolvedModel?.authType ??
            runtimeAuthOverrides?.authType ??
            base.getContentGeneratorConfig().authType;
        const authOverrides = resolvedModel
            ? { authType: authType }
            : {
                ...runtimeAuthOverrides,
                authType: authType,
            };
        const view = await createRuntimeContentGeneratorView(base, base, modelId, authOverrides);
        debugLogger.info(`Created per-agent ContentGenerator for subagent "${config.name}": authType=${authType}, model=${view.contentGeneratorConfig.model}`);
        return view;
    }
    resolveModelOverride(model, runtimeContext) {
     ...
     */
    async convertToRuntimeConfig(config, runtimeContext) {
        const promptConfig = {
            systemPrompt: config.systemPrompt,
        };
        const resolvedModel = this.resolveModelOverride(config.model, runtimeContext);
        const modelConfig = {
        ...
        if ((config.tools && config.tools.length > 0) ||
            ...
            toolConfig = {
                ...
                ...(config.disallowedTools && config.disallowedTools.length > 0
                    ? {
                        disallowedTools: await this.transformToToolNames(config.disallowedTools),
                    }
                    : {}),
            };
        }
     ...
     * @private
     */
    async transformToToolNames(tools) {
        ...
        for (const toolIdentifier of tools) {
            ...
            const exactNameMatch = allTools.find((tool) => tool.name === toolIdentifier);
            if (exactNameMatch) {
            ...
            const displayNameMatch = allTools.find((tool) => tool.displayName === toolIdentifier ||
                tool.displayName ===
                    ToolDisplayNamesMigration[toolIdentifier]);
            if (displayNameMatch) {
            ...
            debugLogger.warn(`Tool "${toolIdentifier}" not found in tool registry, preserving as-is`);
        }
        return result;
    }
    /**
     ...
     */
    async listSubagentsAtLevel(level) {
     ...
     * @returns SubagentConfig or null if not found
     */
    async findSubagentByNameAtLevel(name, level) {
     ...
     */
    async isNameAvailable(name, level) {
        const existing = await this.loadSubagent(name, level);
        if (!existing) {
...
export async function loadSubagentFromDir(baseDir) {
    try {
        const files = await fs.readdir(baseDir);
        const subagents = [];
        for (const file of files) {
            ...
            try {
                ...
                const config = parseSubagentContent(content, filePath, 'extension', new SubagentValidator());
                subagents.push(config);
            }
            catch (error) {
                warnInvalidSubagentFile(filePath, error);
...
function parseSubagentContent(content, filePath, level, validator) {
    try {
        ...
        const tools = Array.isArray(toolsRaw)
            ? toolsRaw.filter((item) => typeof item === 'string')
        ...
        const disallowedTools = Array.isArray(disallowedToolsRaw)
            ? disallowedToolsRaw.filter((item) => typeof item === 'string')
        ...
        const legacyModelConfig = frontmatter['modelConfig'];
        const runConfig = frontmatter['runConfig'];
        ...
        const color = typeof colorRaw === 'string' && (isColor(colorRaw) || colorRaw === 'auto')
            ? colorRaw
            : undefined;
        if (colorRaw !== undefined &&
            color === undefined &&
            typeof colorRaw === 'string') {
            debugLogger.warn(`Agent file ${filePath} has invalid color '${colorRaw}'. Valid options: ${COLOR_VALUES.join(', ')}, auto. Dropping field.`);
        }
        ...
        if (approvalModeRaw !== undefined &&
            ...
            throw new Error(`Invalid "approvalMode" value: expected a string, got ${typeof approvalModeRaw}. Valid values: ${subagentApprovalModesLabel()}`);
        }
        const approvalMode = typeof approvalModeRaw === 'string' && approvalModeRaw !== ''
            ? approvalModeRaw
            : undefined;
        if (approvalMode !== undefined && !isSubagentApprovalMode(approvalMode)) {
            throw new Error(`Invalid "approvalMode" value "${approvalMode}". Valid values: ${subagentApprovalModesLabel()}`);
        }
        ...
        if (backgroundRaw !== undefined &&
            ...
            backgroundRaw !== true &&
            backgroundRaw !== false) {
            debugLogger.warn(`Agent file ${filePath} has invalid background value '${backgroundRaw}'. Must be 'true', 'false', or omitted.`);
        }
        ...
        if (permissionModeRaw !== undefined &&
            ...
            debugLogger.warn(`Agent file ${filePath} has invalid permissionMode '${permissionModeRaw}'. Dropping field.`);
        }
        const bridgedApprovalMode = approvalMode === undefined && permissionMode !== undefined
        ...
        if (frontmatter['maxTurns'] !== undefined && maxTurns === undefined) {
            debugLogger.warn(`Agent file ${filePath} has invalid maxTurns '${frontmatter['maxTurns']}'. Dropping field.`);
        }
        ...
        if (mcpServersRaw !== undefined && mcpServers === undefined) {
            debugLogger.warn(`Agent file ${filePath} has invalid mcpServers (expected an object of server-name → spec). Dropping field.`);
        }
        ...
        if (hooksRaw !== undefined && hooks === undefined) {
            debugLogger.warn(`Agent file ${filePath} has invalid hooks (expected an object of HookEventName → matcher array). Dropping field.`);
        }
        const config = {
            name,
            description,
        ...
        throw new SubagentError(`Failed to parse subagent file: ${error instanceof Error ? error.message : 'Unknown error'}`, SubagentErrorCode.INVALID_CONFIG);
    }
}
/**
 ...
 */
function warnInvalidSubagentFile(filePath, error) {
