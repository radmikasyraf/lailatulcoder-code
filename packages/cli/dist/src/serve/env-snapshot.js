...
import { detectRuntime, formatMemoryUsage, redactProxyCredentials, } from '@lailatul-coder/lailatul-coder-core';
 ...
 */
export function readProxyVar(env, name) {
...
function safeProxyValue(name, raw) {
    if (name === 'NO_PROXY')
        return redactProxyCredentials(raw);
    try {
 ...
 */
export function buildEnvStatusFromProcess(workspaceCwd, acpChannelLive) {
    return buildEnvStatusFromEnv(workspaceCwd, acpChannelLive, snapshotProcessEnv());
}
export function buildEnvStatusFromEnv(workspaceCwd, acpChannelLive, sourceEnv) {
    ...
    const runtimeVersion = runtime === 'bun'
...
export const ENV_SECRET_VARS = SECRET_ENV_VARS;
export const ENV_NONSECRET_VARS = NONSECRET_ENV_VARS;
export const ENV_PROXY_VARS = PROXY_VARS;
