...
export function advertisedMaxPendingPromptsPerSession(value) {
    if (value === undefined)
...
export function advertisedMaxSessions(value) {
    if (value === undefined)
...
export function createServeFeatures(deps) {
    const { opts, boundWorkspace, persistSettingAvailable, sessionArtifactsPersistenceAvailable, sessionGenerationAvailable, workspaceGenerationAvailable, reloadAvailable, channelReloadAvailable, channelControlAvailable, channelManagementAvailable, sessionShellCommandEnabled, multiWorkspaceSessionsEnabled, dynamicWorkspaceRegistrationAvailable, persistentWorkspaceRegistrationAvailable, scratchWorkspaceRegistrationAvailable, realtimeVoiceEnabled, acpHttpEnabled, workspaceRuntimeRemovalAvailable, workspaceTrustHotReloadAvailable, } = deps;
    const getEnv = deps.getEnv ?? (() => deps.env ?? process.env);
    let cachedVoiceTranscriptionAvailable;
    ...
    const getCachedVoiceTranscriptionAvailable = () => {
        cachedVoiceTranscriptionAvailable ??=
            isWorkspaceVoiceTranscriptionAvailable(boundWorkspace, getEnv(), deps.env !== undefined || deps.getEnv !== undefined, deps.isPrimaryWorkspaceTrusted?.() ?? true);
    ...
    return {
        ...
        currentServeFeatures: () => {
            const env = getEnv();
            const currentAcpHttpEnabled = acpHttpEnabled ?? resolveAcpHttpEnabled(env);
            return getAdvertisedServeFeatures(undefined, {
                ...
                scratchWorkspaceRegistrationAvailable: scratchWorkspaceRegistrationAvailable(),
...
function isWorkspaceVoiceTranscriptionAvailable(boundWorkspace, env, skipLoadEnvironment, workspaceTrusted) {
    try {
        return hasConfiguredBatchVoiceTranscriptionModel(loadSettings(boundWorkspace, {
        ...
        writeStderrLine(`LailatulCoder serve: workspace voice transcription capability check failed: ${err instanceof Error ? err.message : String(err)}`);
        return false;
    }
}
//# sourceMappingURL=serve-features.js.map