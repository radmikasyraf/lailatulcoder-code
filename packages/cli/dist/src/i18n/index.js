...
import { SUPPORTED_LANGUAGES, getLanguageNameFromLocale, resolveSupportedLanguage, } from './languages.js';
import { getTranslationModuleExport, isTranslationDict, } from './translationDict.js';
...
// chunk.
const getBuiltinLocalesDir = () => path.join(resolveBundleDir(import.meta.url), 'locales');
const getUserLocalesDir = () => path.join(Storage.getGlobalQwenDir(), 'locales');
/**
 ...
 * @returns The path to ~/.lailatulcoder/locales
 */
...
async function loadTranslationsAsync(lang) {
    ...
    const loadPromise = (async () => {
        ...
        if (fs.existsSync(getUserLocalesDir()) && fs.existsSync(userJsPath)) {
            const userResult = await tryImportTranslations(pathToFileURL(userJsPath).href);
            if (userResult.translations) {
                translationCache[lang] = userResult.translations;
                return userResult.translations;
            }
            writeStderrLine(`Failed to load translations from user directory for ${lang}: ${userResult.error.message}`);
        }
        ...
        if (lastBuiltinError) {
            writeStderrLine(`Failed to load JS translations for ${lang}: ${lastBuiltinError.message}`);
        }
 ...
 */
export function localizeToolDisplayName(displayName) {
 ...
 */
export function resolveLanguageSetting(settingsLanguage) {
