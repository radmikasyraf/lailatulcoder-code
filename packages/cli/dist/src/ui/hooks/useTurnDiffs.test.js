...
function userTurn(id, text, promptId) {
    return {
        id,
        type: 'user',
        text,
...
function fakeDiff(promptId, fileCount) {
    return {
        promptId,
        timestamp: new Date(),
...
function makeService(responder) {
...
describe('useTurnDiffs', () => {
    ...
    it('filters out slash-commands, missing promptId, and empty diffs', async () => {
        const service = makeService(async (id) => {
            if (id === 'p-empty')
                return fakeDiff(id, 0); // empty: should drop
            if (id === 'p-good')
                return fakeDiff(id, 2);
    ...
    it('processes more than TURN_CONCURRENCY (4) turns without dropping any', async () => {
        ...
        const service = makeService(async (id) => fakeDiff(id, 1));
        const history = promptIds.map((id, i) => userTurn(i + 1, `turn ${i + 1}`, id));
        const { result } = renderHook(() => useTurnDiffs(history, service, true));
        await waitFor(() => expect(result.current.loading).toBe(false));
        expect(result.current.turns).toHaveLength(10);
        // All promptIds present, ordering still newest-first.
        expect(result.current.turns.map((t) => t.diff.promptId)).toEqual([...promptIds].reverse());
    });
    it('caps in-flight calls at TURN_CONCURRENCY (concurrent fan-out bounded)', async () => {
        ...
        const history = Array.from({ length: 12 }, (_, i) => userTurn(i + 1, `t${i + 1}`, `p${i + 1}`));
        const { result } = renderHook(() => useTurnDiffs(history, service, true));
        await waitFor(() => expect(result.current.loading).toBe(false));
...
//# sourceMappingURL=useTurnDiffs.test.js.map