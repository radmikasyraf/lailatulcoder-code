...
import { ALL_PROVIDERS, applyProviderInstallPlan, buildInstallPlan, buildProviderTemplate, computeModelListVersion, getDefaultModelIds, PROVIDER_METADATA_NS, providerMatchesCredentials, resolveBaseUrl, resolveMetadataKey, resolveOwnsModel, } from '@lailatul-coder/lailatul-coder-core';
...
function migrateProviderMetadata(settings) {
    const mergedSettings = settings.merged;
    ...
    const migrateKey = (oldKey, newKey) => {
        ...
        for (const [field, value] of Object.entries(entries)) {
            if (value !== undefined) {
                settings.setValue(persistScope, `${PROVIDER_METADATA_NS}.${newKey}.${field}`, value);
            }
        }
    ...
    if (migrated) {
        ...
        console.log('[info] Migrated provider metadata to providerMetadata namespace.');
    }
}
// ---------------------------------------------------------------------------
function computeModelDiff(existingModelIds, newModelIds, currentModel) {
...
function readInstalledOwnedIds(settings, provider) {
...
function getInstalledOwnedModelIds(settings, provider) {
    ...
    return readInstalledOwnedIds(settings, provider).filter((id) => builtinIds.has(id));
}
function findAllPendingUpdates(settings, currentModel) {
    const results = [];
    for (const provider of ALL_PROVIDERS) {
        ...
        const currentVersion = computeModelListVersion(buildProviderTemplate(provider, baseUrl));
        if (metadata.version === currentVersion)
            continue;
        if (metadata.ignoredVersion === currentVersion)
            continue;
        ...
        if (metadata.postponedVersion === currentVersion &&
 ...
 */
export function useProviderUpdates(settings, config, addItem) {
    const [updateRequest, setUpdateRequest] = useState();
    const migrated = useRef(false);
    const executeUpdate = useCallback(async (pending) => {
        try {
            ...
            const customIds = readInstalledOwnedIds(settings, providerCfg).filter((id) => !defaultIds.includes(id));
            const installPlan = buildInstallPlan(providerCfg, {
                baseUrl: resolved,
                apiKey: '',
                modelIds: [...defaultIds, ...customIds],
            });
            installPlan.providerState[`${PROVIDER_METADATA_NS}.${pending.metadataKey}`]['version'] = pending.currentVersion;
            ...
            const updatesActiveProvider = activeConfig?.authType === providerCfg.protocol &&
                providerMatchesCredentials(providerCfg, activeConfig.baseUrl, activeConfig.apiKeyEnvKey);
            ...
            if (activeModel === previousModel) {
                ...
                addItem({
                    type: 'info',
                    text: t('{{plan}} configuration updated successfully. Model switched to "{{model}}".', { plan: displayName, model: activeModel }),
                }, Date.now());
            }
            addItem({
                type: 'info',
                text: t('Tip: Use /model to switch between available {{plan}} models.', { plan: displayName }),
            }, Date.now());
            return true;
        }
        catch (error) {
    ...
    const checkForUpdates = useCallback(() => {
        ...
        if (pendingList.length === 0)
            return;
        const entries = pendingList.map((p) => ({
        ...
        setUpdateRequest({
            ...
            onConfirm: async (choice) => {
                ...
                if (choice === 'update') {
                    ...
                    const persistScope = getPersistScopeForModelSelection(settings);
                    for (const p of pendingList) {
                        settings.setValue(persistScope, `${PROVIDER_METADATA_NS}.${p.metadataKey}.ignoredVersion`, p.currentVersion);
                    }
                }
                    ...
                    try {
                        settings.setValues(pendingList.flatMap((p) => [
                            {
                                scope: persistScope,
...
//# sourceMappingURL=useProviderUpdates.js.map