...
const WORKSPACE = '/w';
const summary = (sessionId, overrides = {}) => ({
    sessionId,
...
const activeGoal = (condition, overrides = {}) => {
...
const goalWithStatus = (condition, status) => {
...
function makeApp(bridge, overrides = {}) {
    const app = express();
...
describe('GET /goals', () => {
    ...
    it('does not return goals after the captured generation closes', async () => {
        let releaseGoal;
        const pendingGoal = new Promise((resolve) => {
            releaseGoal = resolve;
        });
    ...
    it.each(['paused', 'blocked', 'usage_limited'])('lists a %s goal so its controls stay reachable', async (status) => {
    ...
    it('filters out a completed goal', async () => {
        ...
        const res = await request(app).get('/goals');
        expect(res.status).toBe(200);
        expect(res.body.goals.map((goal) => goal.sessionId)).toEqual(['s2']);
    });
    it('drops a session whose probe rejects rather than failing the whole list', async () => {
        ...
        const app = makeApp({
            ...
            getSessionGoal: async (id) => {
                if (id === 'dead')
                    throw new Error('Session not found: dead');
    ...
    it('probes the live sessions concurrently, one call each', async () => {
        const getSessionGoal = vi.fn(async (_sessionId) => noGoal);
        const app = makeApp({
            listWorkspaceSessions: () => [
                summary('s1'),
                summary('s2'),
    ...
    it('keeps a rejection attributed to the session that caused it', async () => {
        ...
        const app = makeApp({
            ...
            getSessionGoal: async (id) => {
                if (id === 'bad')
                    throw new Error('child is wedged');
                return activeGoal(`goal ${id}`);
            },
        });
        const res = await request(app).get('/goals');
        expect(res.status).toBe(200);
        expect(res.body.droppedCount).toBe(1);
        expect(res.body.goals.map((g) => g.sessionId)).toEqual(expect.arrayContaining(['good-1', 'good-2']));
