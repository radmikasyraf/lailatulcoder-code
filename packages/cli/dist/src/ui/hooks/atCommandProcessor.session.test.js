...
import { FileDiscoveryService, StandardFileSystemService, COMMON_IGNORE_PATTERNS, } from '@lailatul-coder/lailatul-coder-core';
...
describe('handleAtCommand @session:', () => {
    ...
    beforeEach(async () => {
        vi.clearAllMocks();
        testRootDir = await fsPromises.mkdtemp(path.join(os.tmpdir(), 'at-session-test-'));
    ...
    it('injects slimmed session text for a valid @session:<uuid>', async () => {
        ...
        expect(result.toolDisplays?.some((d) => d.name === 'Referenced Session')).toBe(true);
    });
    ...
    it.skipIf(process.platform === 'win32')('preserves a literal escaped shell special after parsing', async () => {
        mockFindSessionsByTitle.mockResolvedValue([{ sessionId: UUID }]);
        mockResolve.mockResolvedValue({
    ...
    it('normalizes escaped Windows session titles in prompt text', async () => {
        ...
        try {
            ...
            const firstPart = Array.isArray(result.processedQuery)
                ...
                : undefined;
            expect(firstPart).toEqual({ text: '@session:a&b' });
            expect(mockFindSessionsByTitle).toHaveBeenCalledWith('a&b');
        }
        finally {
    ...
    it('falls back to literal text with an error card when not found', async () => {
        ...
        expect(result.toolDisplays?.some((d) => d.name === 'Referenced Session' && d.status === ToolCallStatus.Error)).toBe(true);
    });
    it('reports an ambiguous title without guessing', async () => {
        ...
        expect(result.shouldProceed).toBe(true);
        expect(JSON.stringify(result.processedQuery)).toContain('@session:Ambiguous');
        const card = result.toolDisplays?.find((d) => d.name === 'Referenced Session');
        expect(card).toBeDefined();
        expect(card.resultDisplay).toContain('ambiguous');
    });
    it('survives a filesystem error during title lookup', async () => {
        mockFindSessionsByTitle.mockRejectedValue(Object.assign(new Error('EACCES'), { code: 'EACCES' }));
        const result = await handleAtCommand({
    ...
    it('survives a load error during session resolve', async () => {
        mockResolve.mockRejectedValue(Object.assign(new Error('corrupted JSONL'), { code: 'EIO' }));
        const result = await handleAtCommand({
            query: `see @session:${UUID} please`,
            config: mockConfig,
            addItem: mockAddItem,
    ...
    it('deduplicates cross-form refs (UUID + title) resolving to the same session', async () => {
        ...
        const cards = result.toolDisplays?.filter((d) => d.name === 'Referenced Session');
        expect(cards).toHaveLength(1);
        expect(mockOnDebugMessage).toHaveBeenCalledWith(expect.stringContaining('already referenced'));
    });
...
//# sourceMappingURL=atCommandProcessor.session.test.js.map