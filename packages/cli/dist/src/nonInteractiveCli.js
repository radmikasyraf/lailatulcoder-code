...
import { executeToolCall, shutdownTelemetry, isTelemetrySdkInitialized, GeminiEventType, FatalInputError, promptIdContext, OutputFormat, InputFormat, LoopType, ToolNames, uiTelemetryService, parseAndFormatApiError, createDebugLogger, detectAutonomousSentinel, detectLoopSentinel, SendMessageType, buildSessionRecoveryPlanFromApiHistory, restoreWorktreeContext, TeamEventType, ApprovalMode, ToolConfirmationOutcome, createDuplicateProviderToolCallResponse, findPlanModeEntryBatchBoundaryIndex, isSystemReminderContent, markDuplicateProviderToolCallResponseSent, findRepeatedDuplicateProviderToolCall, getCachedToolCallFingerprint, isReplayOfHandledToolCall, recordHandledToolCall, isToolCallConcurrencySafe, canonicalToolName, parsePositiveIntegerEnv, partitionByConcurrencySafety, PLAN_MODE_ENTRY_SIBLING_SKIP_MESSAGE, ToolErrorType, finalizeToolResponses, clampInlineMediaPart, formatFullTurnVisionNotice, formatVisionBridgeNotice, getFullTurnVisionModelSelector, hasImageParts, runVisionBridge, shouldRunVisionBridge, splitImageParts, GoalPersistenceUnavailableError, addAgentOutputMessageAttributes, endInteractionSpan, getErrorType, getActiveInteractionSpan, } from '@lailatul-coder/lailatul-coder-core';
import { JsonOutputAdapter } from './nonInteractive/io/JsonOutputAdapter.js';
import { StreamJsonOutputAdapter } from './nonInteractive/io/StreamJsonOutputAdapter.js';
import { handleSlashCommand } from './nonInteractiveCliCommands.js';
import { handleAtCommand } from './ui/hooks/atCommandProcessor.js';
import { AlreadyReportedError, handleError, handleToolError, handleCancellationError, handleMaxTurnsExceededError, handleBudgetExceededError, } from './utils/errors.js';
import { RunBudgetEnforcer } from './utils/runBudget.js';
import { settleChatRecording, subscribeToHeadlessChatRecordingFailures, } from './utils/chat-recording-failure.js';
...
const restoredBackgroundAgentSessions = new WeakMap();
/**
...
function isHeadlessLoopSentinel(prompt) {
    return (detectLoopSentinel(prompt) !== null ||
 ...
 */
const SUPPRESSED_OUTPUT_SUCCESS = "Skipped: this turn's structured_output contract took precedence as the terminal output.";
...
import { normalizePartList, extractPartsFromUserMessage, buildSystemMessage, createToolProgressHandler, createAgentToolProgressHandler, computeUsageFromMetrics, buildInitialSystemReminders, insertAfterFunctionResponses, } from './utils/nonInteractiveHelpers.js';
...
const LOOP_TYPE_LABELS = {
    [LoopType.CONSECUTIVE_IDENTICAL_TOOL_CALLS]: 'the model repeated the same tool call with identical arguments',
    [LoopType.CHANTING_IDENTICAL_SENTENCES]: 'the model repeated the same sentence in its output',
    [LoopType.REPETITIVE_THOUGHTS]: 'the model repeated the same reasoning thought',
    [LoopType.READ_FILE_LOOP]: 'the model spent too many consecutive calls reading files without making progress',
    [LoopType.ACTION_STAGNATION]: 'the model kept calling the same tool without making progress',
    [LoopType.SHELL_COMMAND_STAGNATION]: 'the model repeated similar shell inspection commands without making progress',
    [LoopType.GLOBAL_TOOL_CALL_DUPLICATE]: 'the model repeated the same tool call across the turn, even when not back-to-back',
    [LoopType.ALTERNATING_TOOL_CALL_PATTERN]: 'the model alternated between the same two tool calls in a repeating pattern',
    [LoopType.TURN_TOOL_CALL_CAP]: 'the turn reached the per-turn tool-call limit',
    [LoopType.INVALID_TOOL_PARAMS_STAGNATION]: 'the model repeatedly sent invalid tool parameters without correcting them',
    [LoopType.REPEATED_TOOL_EXECUTION_FAILURE]: 'the same tool execution failure continued after a corrective reminder',
};
function formatLoopDetectedMessage(loopType) {
    ...
    const isAlwaysOn = loopType === LoopType.CONSECUTIVE_IDENTICAL_TOOL_CALLS ||
    ...
    const hint = loopType === LoopType.TURN_TOOL_CALL_CAP
...
function projectLegacyActiveGoal(snapshot) {
...
function formatGoalState(snapshot, operation) {
    const goal = snapshot.goal;
    if (!goal) {
        return operation === 'clear' ? 'Goal cleared.' : 'No Goal is set.';
    }
    const status = goal.status === 'usage_limited' ? 'usage limited' : goal.status;
...
async function claimUserGoalTurn(runtime, turnKey, signal) {
 ...
 */
export function skipHeadlessLoopSentinel(scheduler, job) {
    ...
    if (job.recurring && !job.durable) {
        ...
        debugLogger.debug('skipHeadlessLoopSentinel: cleaning up recurring session loop.md cron in headless mode', { jobId: job.id });
 ...
 */
function partitionHeadlessToolCalls(requests, config) {
    const registry = config.getToolRegistry();
    return partitionByConcurrencySafety(requests, (request) => isToolCallConcurrencySafe(request.name, registry.getTool(canonicalToolName(request.name))?.kind, request.args));
}
/**
...
export async function runNonInteractive(config, settings, input, prompt_id, options = {}) {
    return promptIdContext.run(prompt_id, async () => {
        ...
        const unsubscribeRecordingFailure = ownsAdapter
            ...
            : undefined;
        let chatRecordingSettlement;
        const settleBeforeTerminalOutput = () => {
        ...
        const cleanupReviewWorktrees = (gitTimeout) => cleanupReviewWorktreeLeases({
            sessionId,
            promptId: prompt_id,
            repositoryRoot: config.getProjectRoot(),
            gitTimeout,
        });
        const unregisterReviewWorktreeCleanup = registerCleanup(() => cleanupReviewWorktrees(1_000));
        let turnCount = 0;
        let limitedTurnCount = 0;
        let totalApiDurationMs = 0;
        const startTime = Date.now();
        let activeInteractionPromptId = prompt_id;
        let activeInteractionOwner;
        const selectActiveInteraction = (promptId, startsInteraction = false) => {
        ...
        const captureActiveInteractionOwner = () => {
            activeInteractionOwner ??= getActiveInteractionSpan(activeInteractionPromptId);
            return activeInteractionOwner;
        };
        const endActiveInteraction = (status, metadata = {}) => {
        ...
        const queuedGoalTurns = [];
        let activeGoalTurn;
        let goalRuntimeUnsubscribe;
        const emitGoalSnapshot = (snapshot) => {
            adapter.processEvent({
        ...
        const observeGoalRuntime = (runtime) => {
            goalRuntimeUnsubscribe ??= runtime.subscribe(emitGoalSnapshot);
        };
        const enforceSessionTurnLimit = async (isRuntimeGoalTurn) => {
            if (isRuntimeGoalTurn)
                return;
            limitedTurnCount++;
            const maxSessionTurns = config.getMaxSessionTurns();
            if (maxSessionTurns >= 0 && limitedTurnCount > maxSessionTurns) {
                await failClosedActiveGoalTurn('Headless Goal stopped after the session turn limit');
                await settleBeforeTerminalOutput();
        ...
        const goalHost = {
            startGoalTurn: async (input) => {
                if (queuedGoalTurns.some(({ permit }) => permit.turnId === input.permit.turnId)) {
                    return;
                }
        ...
        let settlingGoalTurn;
        let goalTurnSettlement;
        const failClosedActiveGoalTurn = (reason) => {
        ...
        const finishGoalTurn = async (turn) => {
            const runtime = await config.getGoalRuntimeReady();
            if (!sameGoalPermit(runtime.permitForTurn(turn.turnKey), turn.permit)) {
                return;
            }
        ...
        const routeAbort = async () => {
            ...
            await failClosedActiveGoalTurn(exceeded?.message ?? 'Headless Goal execution was cancelled');
            await settleBeforeTerminalOutput();
            if (exceeded) {
            ...
            if (options.recoverableCancellation === true &&
        ...
        const localQueue = [];
        const sdkOnlyMonitorQueue = [];
        const isCancelledMonitorEvent = (item) => Boolean(item.monitorId &&
            item.sdkNotification?.status === 'running' &&
            config.getMonitorRegistry().get(item.monitorId)?.status ===
                'cancelled');
        const emitNotificationToSdk = (item) => {
        ...
        const flushQueuedNotificationsToSdk = (queue) => {
        ...
        const detachFromManager = (m) => {
            m.setLeaderMessageCallback(null);
            if (approvalListener) {
                m.getEventEmitter().off(TeamEventType.TEAMMATE_APPROVAL_REQUEST, approvalListener);
                approvalListener = null;
            }
        };
        const onTeamManagerChangeHandler = (manager) => {
            ...
            if (manager) {
                ...
                if (options.controlService) {
                    ...
                    approvalListener = (event) => {
                        ...
                        const requiresExplicitHostApproval = confirmationDetails?.type !== 'plan' &&
                        ...
                        if (mode === ApprovalMode.YOLO && !requiresExplicitHostApproval) {
                            ...
                            event
                                ...
                                .catch((err) => {
                                debugLogger.warn('Teammate approval ProceedOnce failed:', err);
                            });
                            return;
                        }
                        ...
                        pendingTeammateMessages.push(`<team_notice>\n${reason}\n</team_notice>`);
        ...
        try {
            ...
            if (options.controlService) {
                config
                    ...
                    .setApprovalRequestCallback((entry, approval, rawArgs, signal) => options.controlService.permission.handleWorkflowApproval(entry.runId, approval, rawArgs, signal));
            ...
            if (resumedSessionData) {
                const restoredSessions = restoredBackgroundAgentSessions.get(config) ?? new Set();
            ...
            const submittedPrompt = typeof userMessageContent === 'string'
                ? userMessageContent
            ...
            if (options.continueInterrupted) {
                ...
                if (reminderParts.length > 0 && initialPartList) {
                    ...
                    const hasSystemReminderPart = continuationParts.some((part) => isSystemReminderContent({ role: 'user', parts: [part] }));
                    if (!hasSystemReminderPart) {
                        initialPartList = insertAfterFunctionResponses(continuationParts, reminderParts);
                    }
                }
            }
            if (!initialPartList) {
                ...
                if (isSlashCommand(input)) {
                    ...
                    switch (slashCommandResult.type) {
                        case 'submit_prompt':
                            ...
                            if (slashCommandResult.modelOverride !== undefined &&
                                isInlineModelOverrideAllowed(config, slashCommandResult.modelOverride)) {
                                inlineModelOverride = slashCommandResult.modelOverride;
                                debugLogger.debug(`[runNonInteractive] inline model override captured: ${inlineModelOverride}`);
                            }
                            else if (slashCommandResult.modelOverride !== undefined) {
                                debugLogger.warn(`[runNonInteractive] ignoring model override '${slashCommandResult.modelOverride}': not a model on the active provider`);
                            }
                            slashHandled = true;
                            break;
                        case 'goal_control': {
                            const { snapshot } = slashCommandResult.response;
                            const shouldRunGoalWorker = snapshot.goal?.status === 'active' &&
                            ...
                            try {
                                ...
                                debugLogger.debug('[runNonInteractive] canonical Goal runtime unavailable; answering goal_control from the degraded snapshot');
                            }
                            emitGoalSnapshot(snapshot);
                            const message = formatGoalState(snapshot, slashCommandResult.operation.kind);
                            if (!shouldRunGoalWorker) {
                                await emitNonInteractiveFinalMessage({
                                    message,
                                    isError: false,
                            ...
                            if (!activeGoalTurn) {
                                throw new FatalInputError('The Goal runtime did not schedule a continuation.');
                            }
            ...
            const withReminder = (existing, text) => {
                const reminderPart = {
            ...
            if (startupNotice) {
                ...
                try {
                    ...
                    if (restored.contextMessage) {
                        initialPartList = withReminder(initialPartList, restored.contextMessage);
            ...
            const recoveredAgentsNotice = resumedSessionData &&
            ...
            let fullTurnRuntimeView;
            const emitVisionNotice = (subtype, notice) => {
                if (outputFormat === OutputFormat.TEXT) {
            ...
            if (inlineModelOverride === undefined &&
                ...
                if (fullTurnModel?.agentCapable) {
                    const fullTurnParts = initialParts.map((part) => clampInlineMediaPart(part));
                    ...
                    if (hasImageParts(fullTurnParts)) {
                        ...
                        emitVisionNotice('vision_routing', formatFullTurnVisionNotice(fullTurnModel));
                    }
                }
                else {
                    try {
                        ...
                        if (bridgeResult.status !== 'skipped' ||
                            ...
                            emitVisionNotice('vision_bridge', formatVisionBridgeNotice(bridgeResult));
                        }
                        initialParts =
                        ...
                        debugLogger.debug(`vision bridge: failed before replacement; falling back to text-only parts error=${error instanceof Error ? error.message : String(error)}`);
                        emitVisionNotice('vision_bridge_failed', 'Vision bridge failed; proceeding without the image(s).');
                        initialParts = splitImageParts(initialParts).nonImageParts;
                    }
                }
            }
            const initialSendType = continueSendType ??
            ...
            if (!activeGoalTurn && initialSendType === SendMessageType.UserQuery) {
                try {
                    ...
                    if (runtime.getSnapshot().goal?.status === 'active') {
                        const permit = await claimUserGoalTurn(runtime, prompt_id, abortController.signal);
                        if (abortController.signal.aborted) {
                            await routeAbort();
                        }
                        if (permit) {
            ...
            let modelOverride = inlineModelOverride ?? fullTurnModelOverride;
            ...
            if (inlineModelOverrideActive) {
                debugLogger.debug(`[runNonInteractive] inline model override active for turn: ${inlineModelOverride}`);
            }
            ...
            // result envelope.
            let structuredSubmission = undefined;
            ...
            const emitStructuredSuccess = async () => {
                const owner = captureActiveInteractionOwner();
                if (owner &&
                    getActiveInteractionSpan(activeInteractionPromptId) === owner) {
                    let responseText;
                    ...
                    if (responseText !== undefined) {
                        addAgentOutputMessageAttributes(config, owner, responseText, 'tool_call');
                    }
                    endActiveInteraction('ok');
                }
                await failClosedActiveGoalTurn('Headless Goal ended with structured output');
                registry.abortAll();
                ...
                while (Date.now() < holdbackDeadline &&
            ...
            const emitLoopDetectedResult = async () => {
                endActiveInteraction('error', {
                    errorMessage: 'loop detected',
                    errorType: 'loop_detected',
                });
                await failClosedActiveGoalTurn('Headless Goal stopped after loop detection');
                registry.abortAll();
                flushQueuedNotificationsToSdk(localQueue);
                finalizeOneShotMonitors();
                if (outputFormat === OutputFormat.TEXT) {
            ...
            const handledToolCallFingerprints = new Map(geminiClient.getHistoryToolCallFingerprints());
            ...
            const duplicateProviderToolCallResponseIds = new Set();
            const processToolCallBatch = async (batchRequests, setModelOverride, runtimeView) => {
                let terminateTurn = false;
                const responseByRequest = new Map();
                const statusByResponse = new Map();
                const structuredOutputActive = config.getJsonSchema() &&
                    batchRequests.some((r) => r.name === ToolNames.STRUCTURED_OUTPUT);
                const getProviderResponseId = (request) => request.providerCallId ??
                    (structuredOutputActive ? undefined : request.callId || undefined);
                const seenBatchCallIds = new Set();
                const duplicateBatchRequests = [];
                const uniqueBatchRequests = batchRequests.filter((request) => {
                    if (request.callId) {
                        if (seenBatchCallIds.has(request.callId)) {
                            ...
                            debugLogger.debug(`Dropping duplicate non-interactive tool callId=${request.callId} name=${request.name}`);
                ...
                const isReplayOfHandledRequest = (request) => {
                    const providerCallId = getProviderResponseId(request);
                    return providerCallId
                        ? isReplayOfHandledToolCall(handledToolCallFingerprints, providerCallId, getCachedToolCallFingerprint(request, request.name, request.args))
                        : false;
                };
                const repeatedDuplicateRequest = findRepeatedDuplicateProviderToolCall([...uniqueBatchRequests, ...duplicateBatchRequests], getProviderResponseId, isReplayOfHandledRequest, duplicateProviderToolCallResponseIds);
                if (repeatedDuplicateRequest) {
                    const providerCallId = repeatedDuplicateRequest.providerCallId ??
                        repeatedDuplicateRequest.callId;
                    debugLogger.debug(`[runNonInteractive] Dropping batch after repeated duplicate provider tool-call id: ${providerCallId} (tool: ${repeatedDuplicateRequest.name})`);
                    return {
                        responseParts: [],
                ...
                for (const requestInfo of uniqueBatchRequests) {
                    ...
                    if (!isReplayOfHandledRequest(requestInfo)) {
                        recordHandledToolCall(handledToolCallFingerprints, providerCallId, getCachedToolCallFingerprint(requestInfo, requestInfo.name, requestInfo.args));
                        executableBatchRequests.push(requestInfo);
                        continue;
                    }
                    markDuplicateProviderToolCallResponseSent(providerCallId, duplicateProviderToolCallResponseIds);
                    const toolResponse = createDuplicateProviderToolCallResponse(requestInfo);
                    debugLogger.debug(`[runNonInteractive] Suppressing duplicate provider tool-call id: ${providerCallId} (tool: ${requestInfo.name})`);
                ...
                if (structuredOutputActive) {
                    requestsToExecute = executableBatchRequests.filter((r) => r.name === ToolNames.STRUCTURED_OUTPUT);
                }
                const planModeEntryBoundaryIndex = findPlanModeEntryBatchBoundaryIndex(requestsToExecute.map((request) => request.name));
                const planModeEntryBoundary = planModeEntryBoundaryIndex === undefined
                    ? undefined
                    : requestsToExecute[planModeEntryBoundaryIndex];
                const executedRequests = new Set(respondedRequests);
                ...
                // so `LailatulCoder -p` and the TUI agree on which tools parallelise — a
                ...
                const toolBatches = partitionHeadlessToolCalls(requestsToExecute, config);
                ...
                const isBudgetExempt = (requestInfo) => requestInfo.name === ToolNames.STRUCTURED_OUTPUT &&
                ...
                const launchToolCall = async (requestInfo) => {
                    debugLogger.debug(`[runNonInteractive] launching tool call ${requestInfo.callId} (${requestInfo.name})`);
                    ...
                    const response = await executeToolCall(config, requestInfo, activeGoalTurn
                        ? AbortSignal.any([
                    ...
                    debugLogger.debug(`[runNonInteractive] tool call ${requestInfo.callId} (${requestInfo.name}) settled${response.error ? ' with error' : ''}`);
                    return response;
                };
                ...
                const finalizeToolCall = (requestInfo, toolResponse) => {
                    ...
                    config
                        .getGeminiClient()
                        .recordCompletedToolCall(requestInfo.name, requestInfo.args);
                ...
                const finalizePlanModeEntrySiblingSkip = (requestInfo) => {
                ...
                const maxToolConcurrency = parsePositiveIntegerEnv(process.env['QWEN_CODE_MAX_TOOL_CONCURRENCY'], 10);
                ...
                for (const batch of toolBatches) {
                    ...
                    if (batch.concurrent && batch.calls.length > 1) {
                        ...
                        const settled = await Promise.allSettled(launched.map((l) => l.promise));
                ...
                const unexecutedCalls = executableBatchRequests.filter((r) => !executedRequests.has(r));
                if (unexecutedCalls.length > 0) {
                    const skippedOutput = suppressedOutputBody(structuredSubmission !== undefined);
                ...
                const finalized = await finalizeToolResponses(config, orderedResponses.map(({ request, response }) => ({
                    callId: request.callId,
                    toolName: request.name,
                    responseParts: response.responseParts,
                    persistedOutputFiles: response.persistedOutputFiles,
                    artifacts: response.artifacts,
                })), new Map(orderedResponses.map(({ request }) => [
                    request.callId,
                    request.prompt_id,
                ])));
                ...
                for (let index = 0; index < orderedResponses.length; index++) {
                    ...
                    chatRecordingService?.recordToolResult?.(finalizedParts, {
                        callId: request.callId,
                        status: statusByResponse.get(response) ??
            ...
            while (true) {
                ...
                selectActiveInteraction(currentPromptId, sendType !== SendMessageType.ToolResult);
                const toolCallRequests = [];
                const apiStartTime = Date.now();
                const responseStream = geminiClient.sendMessageStream(currentMessages[0]?.parts || [], abortController.signal, currentPromptId, {
                    type: sendType,
                    modelOverride,
                    ...(isFirstTurn &&
                ...
                for await (const event of responseStream) {
                    ...
                    if (event.type === GeminiEventType.Content &&
                        ...
                        const remaining = PLAIN_TEXT_PREVIEW_LIMIT - plainTextPreview.length;
                        plainTextPreview += String(event.value).slice(0, remaining);
                    }
                    if (event.type === GeminiEventType.LoopDetected) {
                        if (!loopDetected) {
                            loopDetectedMessage = emitLoopDetectedMessage(config, event.value?.loopType);
                        }
                        loopDetected = true;
                    }
                    if (outputFormat === OutputFormat.TEXT &&
                ...
                if (toolCallRequests.length > 0) {
                    ...
                    const { responseParts: toolResponseParts, repeatedDuplicateProviderToolCall, terminateTurn, } = await processToolCallBatch(toolCallRequests, (override) => {
                        if (inlineModelOverrideActive)
                            return false;
                    ...
                    if (repeatedDuplicateProviderToolCall &&
                        toolResponseParts.length === 0) {
                        loopDetectedMessage = emitLoopDetectedMessage(config, LoopType.GLOBAL_TOOL_CALL_DUPLICATE);
                        return emitLoopDetectedResult();
                    }
                ...
                if (shouldFinalizeTurn) {
                    ...
                    if (teamManager?.hasActiveTeammates()) {
                        ...
                        while (teamManager.hasActiveTeammates() &&
                            ...
                            const waitResult = await teamManager.waitForTeammateActivity(undefined, abortController.signal);
                            ...
                            if (waitResult === 'timeout') {
                                debugLogger.warn('[runNonInteractive] waitForTeammateActivity timed ' +
                    ...
                    const drainBatch = async () => {
                        ...
                        if (splitIdx === 0) {
                            while (splitIdx < localQueue.length &&
                        ...
                        while (true) {
                            ...
                            const itemStream = geminiClient.sendMessageStream(itemMessages[0]?.parts || [], abortController.signal, itemPromptId, {
                            ...
                            if (itemToolCallRequests.length > 0) {
                                ...
                                // `modelOverride`.
                                const { responseParts: itemToolResponseParts, repeatedDuplicateProviderToolCall, } = await processToolCallBatch(itemToolCallRequests, (override) => {
                                    if (itemModelOverride?.endsWith('\0') &&
                    ...
                    if (scheduler) {
                        ...
                        scheduler.setSkipDurableFire((job) => isHeadlessLoopSentinel(job.prompt));
                        // Durable tasks live under ~/.lailatulcoder (user-owned, not in the
                        // working tree), so no folder-trust gate is needed here.
                        await scheduler
                            .enableDurable(config.getSessionId())
                            .catch((err) => {
                            debugLogger.warn(`Durable cron init failed — persistent tasks will not fire in this run: ${err}`);
                        });
                    ...
                    if (config.getJsonSchema()) {
                        ...
                        const previewSuffix = previewSnippet
                            ? ` Output preview (${plainTextPreview.length}${plainTextPreview.length >= PLAIN_TEXT_PREVIEW_LIMIT ? '+' : ''} chars): ${JSON.stringify(previewSnippet)}.`
                            : '';
                        const errorMessage = `Model produced plain text instead of calling the structured_output tool as required by --json-schema after ${turnCount} turn(s).` +
                            previewSuffix;
                        endActiveInteraction('error', {
            ...
            endActiveInteraction(budgetExceeded || !abortController.signal.aborted
                ? 'error'
                : 'cancelled', {
                ...(budgetExceeded
                    ? {
            ...
            await failClosedActiveGoalTurn(error instanceof Error
                ? error.message
            ...
            const recoverableCancellation = !budgetExceeded &&
            ...
            const skipAdapterEmit = outputFormat === OutputFormat.TEXT && isAlreadyReportedError;
            ...
            if (!skipAdapterEmit) {
                ...
                // consumer closes early (`LailatulCoder -p ... | head -n 1` is the common
                ...
                try {
                    ...
                    debugLogger.error(`Failed to emit terminal result envelope: ${emitErr instanceof Error ? emitErr.message : String(emitErr)}`);
                }
            }
            if (budgetExceeded) {
            ...
            await failClosedActiveGoalTurn('Headless Goal host stopped before its permit was released');
            ...
            // run completes — important for callers (e.g. the `LailatulCoder serve`
