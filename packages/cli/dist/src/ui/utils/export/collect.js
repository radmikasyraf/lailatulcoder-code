...
function calculateFileStats(records) {
    ...
    for (const record of records) {
        ...
        const argsFromId = callId && argsIndex.byId.has(callId)
...
function calculateTokenStats(records) {
    ...
    if (lastValidRecord) {
        const percent = (lastValidRecord.totalTokenCount / lastValidRecord.contextWindowSize) *
            100;
        return {
            totalTokens,
...
function createExportSessionConfig(config) {
    const supportedConfig = {
        ...
        getToolRegistry: () => {
            ...
            return {
                getTool: (toolName) => registry?.getTool?.(toolName) ?? null,
            };
        },
    };
    return new Proxy(supportedConfig, {
        get(target, prop, receiver) {
            ...
            throw new Error(`Export session replay config does not implement ${prop}`);
        },
    });
}
/**
...
async function extractMetadata(conversation, config) {
    ...
    let gitRepo;
    if (cwd) {
        const { getGitRepoName } = await import('../../../utils/deferred-core-runtime.js');
...
class ExportSessionContext {
    ...
    flushCurrentMessage() {
        ...
        if (this.currentMessage.type === 'assistant' &&
