...
import { emptyGoalSnapshot, GoalPersistenceUnavailableError, } from '@lailatul-coder/lailatul-coder-core';
...
describe('parseGoalCommand', () => {
    ...
    it.each(['set', 'set   ', 'edit', ' edit\n\t'])('rejects an empty objective for %j', (args) => {
...
describe('goalCommand', () => {
    ...
    ])('uses the canonical runtime for non-interactive /goal %s', async (args, expectedRequest) => {
    ...
    ])('uses the canonical runtime for ACP /goal %s', async (args, request, operation) => {
    ...
    it('awaits runtime readiness and reads authoritative status without dispatch', async () => {
        ...
        expect(getGoalRuntimeReady.mock.invocationCallOrder[0]).toBeLessThan(getSnapshot.mock.invocationCallOrder[0]);
    ...
    it.each(['edit new objective', 'pause', 'resume'])('rejects %j when no Goal exists', async (args) => {
    ...
    it.each(['set Ship it', 'edit Better', 'resume'])('rejects %j in an untrusted workspace before runtime admission', async (args) => {
        const { dispatch, runtime } = makeRuntime(goalSnapshot());
        const { context, getGoalRuntimeReady } = makeContext(runtime, {
            trusted: false,
        });
    ...
    it.each(['', 'clear', 'pause'])('still allows %j in an untrusted workspace', async (args) => {
    ...
    describe('goal persistence unavailable', () => {
        ...
        function unavailableContext() {
            const getGoalRuntimeReady = vi
                .fn()
                .mockRejectedValue(new GoalPersistenceUnavailableError('no transcript'));
            const config = {
                getGoalRuntimeReady,
        ...
        it.each(['', 'clear'])('answers %j with an empty snapshot instead of failing', async (args) => {
        ...
        it.each(['', 'clear'])('fails %j when recovery fails while persisted Goal state may remain', async (args) => {
            const getGoalRuntimeReady = vi
                .fn()
                .mockRejectedValue(new GoalPersistenceUnavailableError('migration write failed'));
            const config = {
                getGoalRuntimeReady,
        ...
        it.each(['Ship it', 'edit revised', 'resume'])('still fails %j, which genuinely needs persistence', async (args) => {
            const result = await goalCommand.action(unavailableContext(), args);
            expect(result).toMatchObject({
                type: 'message',
                messageType: 'error',
        ...
        it('does not report a failed clear dispatch as successful', async () => {
            const snapshot = goalSnapshot();
            const { dispatch, runtime } = makeRuntime(snapshot);
            dispatch.mockRejectedValue(new GoalPersistenceUnavailableError('journal write failed'));
            const { context } = makeContext(runtime);
            const result = await goalCommand.action(context, 'clear');
            expect(result).toEqual({
                type: 'message',
...
//# sourceMappingURL=goalCommand.test.js.map