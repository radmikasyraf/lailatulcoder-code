...
import { SessionIdCaseConflictError, SessionTranscriptChangedError, SessionWriterConflictError, SessionWriterLostError, SessionWriterUnavailableError, } from '@lailatul-coder/lailatul-coder-core';
...
function responseMock() {
    ...
    return { response: response, status, json };
}
describe('sendBridgeError session writer errors', () => {
    it('serializes the structured session-closing code', () => {
        const { response, status, json } = responseMock();
        sendBridgeError(response, new SessionNotFoundError('session-1', 'The session is closing', 'session_closing'));
    ...
    it('maps sealed session maintenance to daemon_draining', () => {
        ...
        expect(json).toHaveBeenCalledWith({
            error: 'The daemon is draining and no longer accepts session maintenance.',
    ...
    ])('maps $kind without exposing diagnostics', ({ error, status: expectedStatus, kind, message }) => {
        const { response, status, json } = responseMock();
    ...
    ])('maps %s to %i', (kind, expectedStatus) => {
