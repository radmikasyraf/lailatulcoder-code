 ...
 */
import { ExtensionManager, type ClaudeMarketplaceConfig, type ExtensionSetting } from '@lailatul-coder/lailatul-coder-core';
import type { Request, Response } from 'express';
import type { AcpSessionBridge } from '../acp-session-bridge.js';
import { type ServeWorkspaceExtensionsStatus } from '@lailatul-coder/acp-bridge/status';
 ...
 */
export declare const EXTENSION_QUEUE_FULL_MESSAGE = "Extension operation queue is full";
...
export type ExtensionOperationStatus = {
    ...
    operationId: string;
    operation: string;
    status: 'queued' | 'running' | 'waiting_for_input' | 'succeeded' | 'succeeded_with_warnings' | 'failed';
...
export interface ExtensionOperationContext {
    ...
    }>(task: (onCommitted: (generation: number) => void) => Promise<T>): Promise<T>;
}
export interface RuntimeReconciliationReservation {
    run<T>(task: () => Promise<T>): Promise<T>;
    release(): void;
}
export type ReserveRuntimeReconciliation = () => RuntimeReconciliationReservation;
...
export interface ExtensionsController {
    ...
    createExtensionManager(workspaceDir?: string, isWorkspaceTrusted?: boolean, interactions?: ExtensionInteractionHandlers): ExtensionManager;
    ...
    updateOperation(operationId: string, patch: Partial<Omit<ExtensionOperationStatus, 'operationId' | 'createdAt'>>): void;
    preparationQueue: FifoTaskQueue;
    acquireOperationSlot(res: Response): (() => void) | undefined;
    validateExtensionMutationClient(req: Request, res: Response, opts?: {
        requireClientId?: boolean;
        bridges?: readonly AcpSessionBridge[];
    }): boolean;
    runQueuedExtensionMutation(operation: string, failureContext: {
        source?: string;
        name?: string;
    }, res: Response, run: (extensionManager: ExtensionManager, signal?: AbortSignal, context?: ExtensionOperationContext, operationId?: string) => Promise<ExtensionMutationEvent>, options?: {
        manager?: ExtensionManager;
        createManager?: (operationId: string) => ExtensionManager;
        onSettled?: (operationId: string) => void;
        refreshRuntimes?: readonly WorkspaceRuntime[] | (() => readonly WorkspaceRuntime[]);
        reserveRuntimeReconciliation?: ReserveRuntimeReconciliation;
        operationBasePath?: string;
        skipRefresh?: boolean;
        deadlineMs?: number;
        onRuntimeReconciled?: (runtime: WorkspaceRuntime, generation: number) => void;
        assertGenerationOpen?: () => void;
    }): void;
}
export declare function createExtensionsController(deps: CreateExtensionsControllerDeps): ExtensionsController;
