...
import { buildLargePDFGuidance, buildPDFTextTooLargeGuidance, estimatePDFTextOutputTokens, extractPDFText, getPDFPageCount, isPdftotextAvailable, parsePDFPageRange, PDF_MAX_PAGES_PER_READ, PDF_TEXT_EXTRACTION_UNAVAILABLE_MESSAGE, PDF_TEXT_RESULT_MAX_TOKENS, renderPDFPagesToImages, shouldRequirePDFPageRange, } from './pdf.js';
...
import { readTextRange, detectLineEndingFromContent, } from './read-text-range.js';
import { DEFAULT_RANGE_READ_BYTES, TEXT_RANGE_FAST_PATH_MAX_SIZE, } from './text-range-constants.js';
import { IMAGE_MAX_SOURCE_BYTES, ImageViewError, renderImageOverview, } from './image-view.js';
...
const PROVIDER_SAFE_IMAGE_MIME_TYPES = new Set(PIPELINE_IMAGE_MIME_TYPES);
...
export async function decodeBufferWithEncodingInfoAsync(full) {
    if (full.length === 0) {
    ...
    if (detected && !isUtf8CompatibleEncoding(detected)) {
        try {
            ...
            debugLogger.warn(`Failed to decode buffer as ${detected}: ${e instanceof Error ? e.message : String(e)}`);
        }
    }
 ...
 */
export function decodeBOMBuffer(buf, bomInfo) {
 ...
 */
export function bomEncodingToName(bomEncoding) {
    switch (bomEncoding) {
        case 'utf8':
            return 'utf-8';
        case 'utf16le':
 ...
 */
export async function readFileWithEncodingInfo(filePath, signal) {
    // Read the file once; detect BOM and decode from the single buffer.
    const full = await fs.promises.readFile(filePath, signal === undefined ? undefined : { signal });
...
export async function readFileWithLineAndLimit(params) {
    ...
    const { content, encoding, bom } = await readFileWithEncodingInfo(filePath, signal);
    signal?.throwIfAborted();
 ...
 */
export async function detectFileEncoding(source) {
    let opened = null;
    try {
        const fh = typeof source === 'string'
        ...
        if (bom)
            return bomEncodingToName(bom.encoding);
        // 2. Validate UTF-8
 ...
 */
const MIME_LITE_MISSING_VIDEO_TYPES = new Map([
    ['.m4v', 'video/x-m4v'],
]);
/**
...
export async function detectFileType(filePath) {
    ...
    const lookedUpMimeType = mime.getType(filePath) ?? MIME_LITE_MISSING_VIDEO_TYPES.get(ext) ?? null;
    if (lookedUpMimeType) {
        ...
        if (isTextMime(lookedUpMimeType)) {
            ...
            debugLogger.debug(`detectFileType: ${filePath} → text (mime-trust: ${lookedUpMimeType})`);
            return 'text';
        }
    }
    ...
    if (KNOWN_TEXT_EXTENSIONS.has(ext)) {
        debugLogger.debug(`detectFileType: ${filePath} → text (extension-override, mime ${lookedUpMimeType ?? 'null'})`);
        return 'text';
    }
    ...
    if (KNOWN_TEXT_BASENAMES.has(path.basename(filePath))) {
        debugLogger.debug(`detectFileType: ${filePath} → text (basename-override, mime ${lookedUpMimeType ?? 'null'})`);
        return 'text';
    }
 ...
 */
export function isCacheableReadResult(result) {
...
export async function processSingleFileContent(filePath, config, optionsOrOffset, legacyLimit, legacyPages) {
    const options = typeof optionsOrOffset === 'object' && optionsOrOffset !== null
        ? optionsOrOffset
        : {
    ...
    const { offset, limit, pages, preserveUnsupportedImage = false, preparePdfForVisionBridge = false, signal, largePdfBehavior = 'error', displayPath = filePath, } = options;
    const rootDirectory = config.getTargetDir();
    const relativePathForDisplay = (path.isAbsolute(displayPath)
    ...
    try {
        ...
        const mediaMimeType = mime.getType(filePath) ??
        ...
        const willRenderPdfImages = fileType === 'pdf' &&
            !!modalities.image &&
        ...
        const renderForBridge = fileType === 'pdf' &&
            !modalities.image &&
            !modalities.pdf &&
        ...
        if (fileType === 'pdf' && normalizedPages !== undefined) {
            ...
            const invalidPagesResult = (message) => ({
                llmContent: message,
            ...
            if (!parsedPageRange) {
                return invalidPagesResult(`Invalid pages parameter: '${normalizedPages}'. Use formats like '5' or '1-10'.`);
            }
            if (parsedPageRange.lastPage === Infinity) {
                return invalidPagesResult(`Open-ended page ranges (e.g. '3-') are not supported; specify an explicit end page within the ${PDF_MAX_PAGES_PER_READ}-page limit (e.g. '3-22').`);
            }
            if (parsedPageRange.lastPage - parsedPageRange.firstPage + 1 >
                PDF_MAX_PAGES_PER_READ) {
                return invalidPagesResult(`Pages range exceeds maximum of ${PDF_MAX_PAGES_PER_READ} pages per request.`);
            }
        ...
        const willExtractPdfText = fileType === 'pdf' && (pageRange !== undefined || !modalities.pdf);
        if (!pageRange &&
            willExtractPdfText &&
        ...
        if (willExtractPdfText && !pageRange) {
            ...
            debugLogger.debug(`PDF full-text fallback gate: file=${relativePathForDisplay}, sizeMB=${fileSizeInMB.toFixed(2)}, pageCount=${pdfPageCount ?? 'unknown'}, required=${requirement.required}, rangeRequired=${rangeRequired}, effectivePageCount=${requirement.effectivePageCount}, hadPdfInfo=${requirement.hadPdfInfo}, behavior=${largePdfBehavior}`);
            if (rangeRequired) {
                ...
                const returnDisplay = largePdfBehavior === 'reference'
        ...
        if (modality && modality !== 'pdf') {
            if (!modalities[modality]) {
                ...
                if (!bridgeWillHandleImage) {
                    ...
                    debugLogger.warn(`Model '${config.getModel()}' does not support ${modality} input. ` +
                ...
                debugLogger.debug(`Preserving unsupported image for vision bridge: ${relativePathForDisplay}`);
            }
        }
        switch (fileType) {
            case 'binary': {
            ...
            case 'text': {
                ...
                const originalLineCount = _meta?.originalLineCount ??
                ...
                if (Number.isFinite(configCharLimit)) {
                    ...
                    for (const line of selectedLines) {
                        ...
                        if (projectedLength <= configCharLimit) {
                            ...
                            const remaining = Math.max(configCharLimit - currentLength - sep, 10);
                ...
                if (_meta?.truncatedByBytes === true) {
                    ...
                    if (!llmContent.endsWith(marker)) {
                        const prefix = Number.isFinite(configCharLimit)
                            ? llmContent.slice(0, Math.max(configCharLimit - marker.length - 1, 0))
                            : llmContent;
                ...
                const contentRangeTruncated = startLine > 0 ||
            ...
            case 'image': {
                if (shouldRenderImageOverview) {
                    try {
                        const view = await renderImageOverview(filePath, signal ?? new AbortController().signal);
                        return {
                            llmContent: [
                                {
                                    text: `Image overview: ${view.outputWidth}x${view.outputHeight}; ` +
                        ...
                        if (error instanceof ImageViewError) {
                            ...
                            if ((error.code === 'decode_failed' ||
                                ...
                                const notice = `Image ${relativePathForDisplay} could not be decoded ` +
                ...
                if (!PROVIDER_SAFE_IMAGE_MIME_TYPES.has(mediaMimeType) &&
                    ...
                    const notice = `Image format ${mediaMimeType} (${relativePathForDisplay}) cannot ` +
            ...
            case 'pdf': {
                ...
                const toImageParts = (images, startPage) => images.map((image, index) => ({
                    inlineData: {
                        data: image.data,
                ...
                if (willRenderPdfImages) {
                    ...
                    const render = await renderPDFPagesToImages(filePath, pageRange ?? { firstPage: 1, lastPage: PDF_MAX_PAGES_PER_READ });
                    ...
                    const renderError = render.success
                        ...
                        : render.error;
                    debugLogger.debug(`PDF image render failed, falling back to text outcome: file=${relativePathForDisplay}, error=${renderError}`);
                }
                ...
                const isSinglePageRead = pageRange
                    ...
                    : pdfPageCount === 1;
                const singlePageTextOverflow = pdfResult.success &&
                ...
                if (renderForBridge && (!pdfResult.success || singlePageTextOverflow)) {
                    ...
                    const requestedLastPage = pageRange?.lastPage ??
                        pdfPageCount ??
                        firstPage + VISION_BRIDGE_MAX_IMAGES - 1;
                    const effectiveRequestedLastPage = pdfPageCount == null
                        ? requestedLastPage
                        : Math.min(requestedLastPage, pdfPageCount);
                    const lastPage = Math.min(effectiveRequestedLastPage, firstPage + VISION_BRIDGE_MAX_IMAGES - 1);
                    const render = lastPage >= firstPage
                    ...
                    if (render.success && render.images.length > 0) {
                        ...
                        if (pdfPageCount != null) {
                            ...
                            const reachedEndOfFile = render.images.length < renderedRequestedPageCount &&
                        ...
                        if (continuation) {
                            const suggestedLast = Math.min((continuation.certainty === 'known'
                                ? continuation.lastPage
                                : continuation.requestedLastPage) ??
                                continuation.firstPage + VISION_BRIDGE_MAX_IMAGES - 1, continuation.firstPage + VISION_BRIDGE_MAX_IMAGES - 1);
                            const omitted = continuation.certainty === 'known'
                            ...
                            const instruction = continuation.certainty === 'known'
                                ? 'Use'
                        ...
                        if (preparePdfForVisionBridge) {
                            let fallback;
                            if (pdfResult.success) {
                                const guidance = buildPDFTextTooLargeGuidance(displayName, estimatedTokens, normalizedPages);
                                fallback = {
                                    llmContent: guidance,
                    ...
                    const renderError = render.success
                        ? 'renderer returned no page images'
                        : render.error;
                    debugLogger.debug(`PDF bridge render failed, falling back to text outcome: file=${relativePathForDisplay}, error=${renderError}`);
                }
                if (pdfResult.success) {
                    // Overflowed text: guidance to narrow the range.
                    const guidance = buildPDFTextTooLargeGuidance(displayName, estimatedTokens, normalizedPages);
                    debugLogger.debug(`PDF text extraction output exceeds token limit: file=${relativePathForDisplay}, pages=${normalizedPages ?? 'all'}, limit=${PDF_TEXT_RESULT_MAX_TOKENS}`);
...
export function getRangeReadByteLimit(config) {
