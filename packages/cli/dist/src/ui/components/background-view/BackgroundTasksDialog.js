...
import { buildBackgroundEntryLabel, isActiveWorkflowStatus, isTerminalWorkflowStatus, MAX_RECENT_ACTIVITIES, } from '@lailatul-coder/lailatul-coder-core';
import { ToolConfirmationMessage } from '../messages/ToolConfirmationMessage.js';
import { WorkflowSaveOverlay } from './workflow-save-overlay.js';
import { formatDuration, formatTokenCount } from '../../utils/formatters.js';
import { escapeAnsiCtrlCodes, getCachedStringWidth, sanitizeMultilineForDisplay, } from '../../utils/textUtils.js';
import { TOOL_DISPLAY_BY_NAME } from '../../utils/tool-display-map.js';
import { compareActiveThenTerminal, entryId, } from '../../hooks/useBackgroundTaskView.js';
import { localizeToolDisplayName, t } from '../../../i18n/index.js';
import { ancestorChain, computeAgentTreeInfo, computeUserBlockingIds, statusGlyph, treeRowPrefix, } from './agent-forest.js';
...
function isStoppableEntry(entry) {
    return (entry.status === 'running' ||
        (entry.kind === 'workflow' && isActiveWorkflowStatus(entry.status)));
}
function workflowPauseHint(entry) {
    if (entry?.kind !== 'workflow' || !entry.isBackgrounded)
        return undefined;
...
function rowLabel(entry, userBlocking) {
    switch (entry.kind) {
        case 'agent': {
        ...
        case 'workflow': {
            ...
            const counts = entry.agentsDispatched > 0
 ...
 */
function formatParentBreadcrumb(entry, lookup) {
    if (entry.parentAgentId == null)
        return undefined;
    ...
    return (_jsxs(Box, { flexDirection: "column", children: [_jsxs(Box, { paddingX: 1, children: [_jsx(Text, { bold: true, children: t('Background tasks') }), _jsxs(Text, { color: theme.text.secondary, children: [" (", entries.length, ")"] })] }), _jsxs(Box, { flexDirection: "column", children: [hiddenAbove > 0 && (_jsx(Box, { paddingX: 1, children: _jsx(Text, { color: theme.text.secondary, children: `  ^ ${t('{{count}} more above', { count: String(hiddenAbove) })}` }) })), visible.map((entry, visibleIdx) => {
                        ...
                        const labelColor = isSelected
                            ...
                            : presentation
                                ...
                                : theme.text.primary;
                        const treePrefix = entry.kind === 'agent'
                            ? treeRowPrefix(entry, treeInfo.get(entry.agentId))
                            : '';
                        return (_jsxs(Box, { flexDirection: "row", paddingX: 1, children: [_jsx(Text, { color: isSelected ? theme.text.accent : undefined, children: isSelected ? '> ' : '  ' }), treePrefix !== '' && (_jsx(Text, { color: theme.text.secondary, children: treePrefix })), _jsx(Text, { color: labelColor, children: escapeAnsiCtrlCodes(rowLabel(entry, entry.kind === 'agent' && blockingIds.has(entry.agentId))) })] }, entryId(entry)));
...
const DetailBody = ({ entry, maxHeight, maxWidth }) => {
    switch (entry.kind) {
        ...
        case 'workflow':
            return (_jsx(WorkflowDetailBody, { entry: entry, maxHeight: maxHeight, maxWidth: maxWidth }));
        case 'dream':
...
const DreamDetailBody = ({ entry, maxHeight, maxWidth }) => {
    ...
    return (_jsxs(MaxSizedBox, { maxHeight: maxHeight, maxWidth: maxWidth, overflowDirection: "bottom", children: [_jsx(Box, { children: _jsx(Text, { bold: true, color: theme.text.accent, children: title }) }), _jsxs(Box, { children: [presentation && (_jsx(Text, { color: presentation.color, children: `${presentation.icon} ${statusVerb(entry.status)} · ` })), _jsx(Text, { color: theme.text.secondary, children: dimSubtitleParts.join(' · ') })] }), entry.sessionCount !== undefined && (_jsxs(Fragment, { children: [_jsx(Box, {}), _jsx(Box, { children: _jsx(Text, { bold: true, dimColor: true, children: t('Sessions reviewing') }) }), _jsx(Box, { children: _jsx(Text, { children: String(entry.sessionCount) }) })] })), entry.progressText && (_jsxs(Fragment, { children: [_jsx(Box, {}), _jsx(Box, { children: _jsx(Text, { bold: true, dimColor: true, children: t('Progress') }) }), _jsx(Box, { children: _jsx(Text, { wrap: "wrap", children: entry.progressText }) })] })), topics.length > 0 && (_jsxs(Fragment, { children: [_jsx(Box, {}), _jsx(Box, { children: _jsx(Text, { bold: true, dimColor: true, children: t('Topics touched ({{count}})', {
...
const AgentDetailBody = ({ entry, maxHeight, maxWidth }) => {
    ...
    const title = escapeAnsiCtrlCodes(`${entry.subagentType ?? 'Agent'} \u203A ${buildBackgroundEntryLabel(entry, { includePrefix: false })}`);
    const presentation = statusPresentation(entry.status);
    const dimSubtitleParts = [elapsedFor(entry)];
    ...
    if ((entry.depth ?? 0) > 0) {
        dimSubtitleParts.push(t('nested \u00B7 level {{level}} of {{max}}', {
    ...
    const activities = (entry.recentActivities ?? []).slice(-MAX_RECENT_ACTIVITIES);
    ...
    const liveActivity = activities.length > 0 ? activities[activities.length - 1] : undefined;
    const historyActivities = activities.slice(0, -1);
    const liveFullLabel = liveActivity
        ? sanitizeMultilineForDisplay(formatActivityLabel(liveActivity.name, liveActivity.description))
        : '';
    const liveLabel = liveFullLabel.length > MAX_LIVE_LABEL_CHARS
        ? `${liveFullLabel.slice(0, MAX_LIVE_LABEL_CHARS)}\u2026`
        : liveFullLabel;
    const wrappedRows = (text) => maxWidth > 0
    ...
    const shownHistory = historyActivities.slice(Math.max(0, historyActivities.length - historyBudget));
    return (_jsxs(MaxSizedBox, { maxHeight: maxHeight, maxWidth: maxWidth, overflowDirection: "bottom", children: [_jsx(Box, { children: _jsx(Text, { bold: true, color: theme.text.accent, children: title }) }), _jsxs(Box, { children: [presentation && (_jsx(Text, { color: presentation.color, children: `${presentation.icon} ${statusVerb(entry.status)} \u00B7 ` })), _jsx(Text, { color: theme.text.secondary, children: dimSubtitleParts.join(' \u00B7 ') })] }), parentLine !== undefined && (_jsxs(Fragment, { children: [_jsx(Box, {}), _jsx(Box, { children: _jsx(Text, { bold: true, dimColor: true, children: t('Parent') }) }), _jsx(Box, { children: _jsx(Text, { color: theme.text.secondary, wrap: "truncate-end", children: `  ${escapeAnsiCtrlCodes(parentLine)}` }) })] })), visibleChildAgents.length > 0 && (_jsxs(Fragment, { children: [_jsx(Box, {}), _jsx(Box, { children: _jsxs(Text, { bold: true, dimColor: true, children: [t('Sub-agents'), _jsx(Text, { color: theme.text.secondary, children: ` (${childAgents.length})` })] }) }), visibleChildAgents.map((child) => (_jsx(Box, { children: _jsx(Text, { color: theme.text.secondary, wrap: "truncate-end", children: `  ${statusGlyph(child.status)} ${escapeAnsiCtrlCodes(`${child.subagentType ?? 'agent'} \u2014 ${child.description}`)} \u00B7 ${elapsedFor(child)}` }) }, child.agentId))), hiddenChildCount > 0 && (_jsx(Box, { children: _jsx(Text, { color: theme.text.secondary, children: `  \u2026 ${t('{{count}} more', { count: String(hiddenChildCount) })}` }) }))] })), activities.length > 0 && (_jsxs(Fragment, { children: [_jsx(Box, {}), _jsx(Box, { children: _jsx(Text, { bold: true, dimColor: true, children: t('Progress') }) }), shownHistory.map((a, i) => {
                        ...
                        const fullLabel = sanitizeMultilineForDisplay(formatActivityLabel(a.name, a.description));
                        const label = truncateToWidth(fullLabel, Math.max(0, maxWidth - getCachedStringWidth(prefix)));
                        return (_jsx(Box, { children: _jsxs(Text, { color: theme.text.secondary, children: [prefix, label] }) }, `${a.at}-${i}`));
                    }), liveActivity && (
                    ...
                    _jsx(Box, { children: _jsx(Text, { color: theme.text.primary, children: `> ${liveLabel}` }) }, "live"))] })), entry.outputFile && (_jsxs(Fragment, { children: [_jsx(Box, {}), _jsx(Box, { children: _jsx(Text, { bold: true, dimColor: true, children: t('Transcript') }) }), _jsx(Box, { children: _jsx(Text, { color: theme.text.secondary, wrap: "wrap", children: `  ${escapeAnsiCtrlCodes(entry.outputFile)}` }) })] })), visiblePromptLines.length > 0 && (_jsxs(Fragment, { children: [_jsx(Box, {}), _jsx(Box, { children: _jsx(Text, { bold: true, dimColor: true, children: t('Prompt') }) }), visiblePromptLines.map((line, i) => (_jsx(Box, { children: _jsx(Text, { wrap: "truncate-end", children: escapeAnsiCtrlCodes(line) || ' ' }) }, `prompt-${i}`)))] })), hasBlockedReason && (_jsxs(Fragment, { children: [_jsx(Box, {}), _jsx(Box, { children: _jsx(Text, { bold: true, color: theme.status.error, children: t('Resume blocked') }) }), _jsx(Box, { children: _jsx(Text, { color: theme.status.error, wrap: "wrap", children: blockedReason }) })] })), hasError && (_jsxs(Fragment, { children: [_jsx(Box, {}), _jsx(Box, { children: _jsx(Text, { bold: true, color: theme.status.error, children: t('Error') }) }), _jsx(Box, { children: _jsx(Text, { color: theme.status.error, wrap: "wrap", children: entry.error }) })] }))] }));
};
const ShellDetailBody = ({ entry, maxHeight, maxWidth }) => {
    ...
    return (_jsxs(MaxSizedBox, { maxHeight: maxHeight, maxWidth: maxWidth, overflowDirection: "bottom", children: [_jsx(Box, { children: _jsx(Text, { bold: true, color: theme.text.accent, children: title }) }), _jsxs(Box, { children: [presentation && (_jsx(Text, { color: presentation.color, children: `${presentation.icon} ${statusVerb(entry.status)} \u00B7 ` })), _jsx(Text, { color: theme.text.secondary, children: dimSubtitleParts.join(' \u00B7 ') })] }), _jsx(Box, {}), _jsx(Box, { children: _jsx(Text, { bold: true, dimColor: true, children: t('Working dir') }) }), _jsx(Box, { children: _jsx(Text, { wrap: "truncate-end", children: entry.cwd }) }), _jsx(Box, {}), _jsx(Box, { children: _jsx(Text, { bold: true, dimColor: true, children: t('Output file') }) }), _jsx(Box, { children: _jsx(Text, { wrap: "truncate-end", children: entry.outputFile }) }), hasError && (_jsxs(Fragment, { children: [_jsx(Box, {}), _jsx(Box, { children: _jsx(Text, { bold: true, color: theme.status.error, children: t('Error') }) }), _jsx(Box, { children: _jsx(Text, { color: theme.status.error, wrap: "wrap", children: entry.error }) })] }))] }));
};
const MonitorDetailBody = ({ entry, maxHeight, maxWidth }) => {
    ...
    return (_jsxs(MaxSizedBox, { maxHeight: maxHeight, maxWidth: maxWidth, overflowDirection: "bottom", children: [_jsx(Box, { children: _jsx(Text, { bold: true, color: theme.text.accent, children: title }) }), _jsxs(Box, { children: [presentation && (_jsx(Text, { color: presentation.color, children: `${presentation.icon} ${statusVerb(entry.status)} · ` })), _jsx(Text, { color: theme.text.secondary, children: dimSubtitleParts.join(' · ') })] }), _jsx(Box, {}), _jsx(Box, { children: _jsx(Text, { bold: true, dimColor: true, children: t('Command') }) }), _jsx(Box, { children: _jsx(Text, { wrap: "truncate-end", children: entry.command }) }), hasError && (_jsxs(Fragment, { children: [_jsx(Box, {}), _jsx(Box, { children: _jsx(Text, { bold: true, color: errorColor, children: errorIsFailure ? t('Error') : t('Stopped because') }) }), _jsx(Box, { children: _jsx(Text, { color: errorColor, wrap: "wrap", children: entry.error }) })] }))] }));
};
...
const WorkflowDetailBody = ({ entry, maxHeight, maxWidth }) => {
    const title = `${t('Workflow')} › ${entry.meta?.name ?? entry.runId}`;
    const presentation = statusPresentation(entry.status);
    const dimSubtitleParts = [elapsedFor(entry)];
    if (entry.agentsDispatched > 0) {
        dimSubtitleParts.push(`${entry.agentsCompleted}/${entry.agentsDispatched} ${t('agents')}`);
    }
    dimSubtitleParts.push(`${entry.phases.length} ${entry.phases.length === 1 ? t('phase') : t('phases')}`);
    ...
    if (entry.tokensSpent > 0 || entry.tokenBudgetTotal !== null) {
        dimSubtitleParts.push(entry.tokenBudgetTotal !== null
    ...
    const logOverflow = Math.max(0, entry.recentLogs.length - MAX_VISIBLE_LOG_LINES);
    const visibleLogs = entry.recentLogs.slice(-MAX_VISIBLE_LOG_LINES);
    const hasError = Boolean(entry.error);
    return (_jsxs(MaxSizedBox, { maxHeight: maxHeight, maxWidth: maxWidth, overflowDirection: "bottom", children: [_jsx(Box, { children: _jsx(Text, { bold: true, color: theme.text.accent, children: title }) }), _jsxs(Box, { children: [presentation && (_jsx(Text, { color: presentation.color, children: `${presentation.icon} ${statusVerb(entry.status)} · ` })), _jsx(Text, { color: theme.text.secondary, children: dimSubtitleParts.join(' · ') })] }), entry.meta?.description && (_jsxs(Fragment, { children: [_jsx(Box, {}), _jsx(Box, { children: _jsx(Text, { wrap: "wrap", children: entry.meta.description }) })] })), (entry.status === 'pausing' || entry.status === 'paused') && (_jsxs(Fragment, { children: [_jsx(Box, {}), _jsx(Box, { children: _jsx(Text, { color: theme.status.warning, children: entry.status === 'pausing'
                                ? t('Pause is cooperative; in-flight work may finish before the workflow is paused. An agent call waiting on a tool approval keeps the run in this state and still counts against the active-time limit until the approval is answered.')
                                : t('Paused: no new agents will start; script code between agent calls keeps running. Press p to resume. /clear, /branch, and switching sessions cancel paused runs.') }) })] })), _jsx(Box, {}), _jsx(Box, { children: _jsx(Text, { bold: true, dimColor: true, children: t('Phases') }) }), entry.phases.length === 0 ? (_jsx(Box, { children: _jsx(Text, { dimColor: true, children: t('(no phase recorded yet)') }) })) : (_jsxs(Fragment, { children: [phaseOverflow > 0 && (_jsx(Box, { children: _jsx(Text, { dimColor: true, children: `+${phaseOverflow} ${t('more above')}` }) })), visiblePhases.map((phaseTitle, i) => {
                        const isCurrent = isActiveWorkflowStatus(entry.status) &&
                        ...
                        const tokenChip = phaseTokens > 0 ? ` · ${formatTokenCount(phaseTokens)}t` : '';
                        return (_jsx(Box, { children: _jsx(Text, { color: isCurrent ? theme.status.success : undefined, children: `  ${marker} ${phaseTitle}${tokenChip}` }) }, `${phaseTitle}-${i}`));
                    }), (entry.perPhaseTokens.get(null) ?? 0) > 0 && (_jsx(Box, { children: _jsx(Text, { dimColor: true, children: `  · ${t('(no phase)')} · ${formatTokenCount(entry.perPhaseTokens.get(null) ?? 0)}t` }) }))] })), entry.recentLogs.length > 0 && (_jsxs(Fragment, { children: [_jsx(Box, {}), _jsx(Box, { children: _jsx(Text, { bold: true, dimColor: true, children: t('Logs') }) }), logOverflow > 0 && (_jsx(Box, { children: _jsx(Text, { dimColor: true, children: `+${logOverflow} ${t('more above')}` }) })), visibleLogs.map((line, i) => (_jsx(Box, { children: _jsx(Text, { wrap: "truncate-end", dimColor: true, children: line }) }, `log-${i}`)))] })), hasError && (_jsxs(Fragment, { children: [_jsx(Box, {}), _jsx(Box, { children: _jsx(Text, { bold: true, color: theme.status.error, children: t('Error') }) }), _jsx(Box, { children: _jsx(Text, { color: theme.status.error, wrap: "wrap", children: entry.error }) })] }))] }));
};
export const BackgroundTasksDialog = ({ availableTerminalHeight, terminalWidth, }) => {
    ...
    const isDetailMode = dialogMode === 'detail' || dialogMode === 'detail-from-panel';
    const { moveSelectionUp, moveSelectionDown, closeDialog, enterDetail, exitDetail, cancelSelected, resumeSelected, toggleSelectedWorkflowPause, setSelectedIndex, } = useBackgroundTaskViewActions();
    const config = useConfig();
    ...
    const [pauseRejected, setPauseRejected] = useState(null);
    useEffect(() => {
        if (!pauseRejected)
            return;
    ...
    const selectedApproval = selectedEntry?.kind === 'agent' && selectedEntry.pendingApprovals?.[0]
        ? {
            kind: 'agent',
    ...
    const approvalUsesQuestionDialog = selectedApproval?.approval.confirmationDetails.type === 'ask_user_question';
    ...
    const approvalConfirmationDetails = selectedApproval
                ...
                onConfirm: async (outcome, payload) => {
                    if (selectedApproval.kind === 'agent') {
                        await config
                            ...
                            .resolvePendingApproval(selectedApproval.ownerId, selectedApproval.approval.callId, outcome, payload);
                        return;
                    }
                    await config
                        .getWorkflowRunRegistry()
                        .resolvePendingApproval(selectedApproval.ownerId, selectedApproval.approval.approvalId, outcome, payload);
                },
            }
        : undefined;
    useEffect(() => {
    ...
    const selectedShouldTick = selectedEntry?.kind === 'workflow'
    ...
    useEffect(() => {
        ...
        if (seen && seen.entryId !== selectedEntryId) {
            ...
            const driftedIndex = entries.findIndex((candidate) => entryId(candidate) === seen.entryId);
            if (driftedIndex >= 0) {
        ...
        const seenWasActive = seen.status === 'running' ||
            seen.status === 'pausing' ||
            seen.status === 'paused';
        const selectedIsTerminal = selectedStatus === 'completed' ||
    ...
    const handleCancelKey = () => {
        ...
        // background entry.
        const isUserBlockingAgent = selectedEntry.kind === 'agent' &&
    ...
    // be saved to `.lailatulcoder/workflows/<name>.js` from the detail view.
    const workflowSaveTarget = config &&
        selectedEntry?.kind === 'workflow' &&
        isTerminalWorkflowStatus(selectedEntry.status) &&
        selectedEntry.script
        ? selectedEntry
    ...
    if (approvalActive) {
        ...
        if (selectedWorkflowPauseHint)
            hints.push(selectedWorkflowPauseHint);
        ...
        if (workflowSaveTarget)
            hints.push('s save');
    }
    return (_jsxs(Box, { flexDirection: "column", borderStyle: "round", borderColor: theme.border.default, marginTop: 1, paddingX: 1, children: [dialogMode === 'list' && (_jsx(Box, { paddingX: 1, children: _jsx(Text, { bold: true, color: theme.text.accent, children: t('Background tasks') }) })), _jsxs(Box, { marginTop: dialogMode === 'list' ? 1 : 0, flexDirection: "column", children: [dialogMode === 'list' ? (_jsx(ListBody, { entries: entries, selectedIndex: selectedIndex, maxRows: listMaxRows })) : selectedEntry ? (_jsx(DetailBody, { entry: selectedEntry, 
                        // Halve the detail body budget when an approval banner is shown
                        // below so both fit; the body self-caps internally anyway.
                        maxHeight: approvalActive
                            ? Math.max(6, Math.floor(detailContentHeight / 2))
                            : detailContentHeight, maxWidth: detailContentWidth })) : (_jsx(Box, { paddingX: 1, children: _jsx(Text, { color: theme.text.secondary, children: t('No entry to show.') }) })), approvalActive && approvalConfirmationDetails && (_jsxs(Box, { flexDirection: "column", marginTop: 1, paddingX: 1, children: [_jsx(Text, { bold: true, color: theme.status.warning, children: selectedApproval?.kind === 'workflow'
                                    ? `[workflow] ${t('needs approval')}`
                                    : t('Background agent needs approval') }), _jsx(ToolConfirmationMessage, { confirmationDetails: approvalConfirmationDetails, config: config, isFocused: approvalActive, contentWidth: detailContentWidth - 2, availableTerminalHeight: Math.max(6, Math.floor(detailContentHeight / 2)), compactMode: true })] }))] }), saveActive && workflowSaveTarget && config ? (_jsx(WorkflowSaveOverlay, { script: workflowSaveTarget.script, initialName: workflowSaveTarget.meta?.name ?? '', config: config, isActive: saveActive, onClose: () => setSaveActive(false) })) : (_jsx(Box, { marginTop: 1, paddingX: 1, children: pauseRejected && pauseRejected.entryKey === selectedEntryKey ? (_jsx(Text, { color: theme.status.warning, children: t('Pause/resume was rejected; the workflow state changed. Try again.') })) : (_jsx(Text, { color: theme.text.secondary, children: hints.join(' \u00B7 ') })) }))] }));
};
