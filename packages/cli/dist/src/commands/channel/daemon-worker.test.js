...
const mockCanonicalizeWorkspace = vi.hoisted(() => vi.fn((p) => p));
...
const mockNextFireTime = vi.hoisted(() => vi.fn(() => new Date('2026-01-01T00:01:00.000Z')));
...
const mockDaemonSessionRoutesPath = vi.hoisted(() => vi.fn(() => '/tmp/qwen/channels/daemon/workspace-hash/routes.json'));
const mockDaemonChannelLoopPath = vi.hoisted(() => vi.fn(() => '/tmp/qwen/channels/daemon/workspace-hash/cron.json'));
const mockDaemonObservedContactsPath = vi.hoisted(() => vi.fn(() => '/tmp/qwen/channels/daemon/workspace-hash/observed-contacts.json'));
const mockDaemonChannelStateDir = vi.hoisted(() => vi.fn((workspace, channelName) => `/tmp/qwen/channels/daemon/${workspace === '/workspace' ? 'workspace-hash' : 'other-hash'}/instances/${channelName}-hash`));
const mockObserveContact = vi.hoisted(() => vi.fn());
const mockListContacts = vi.hoisted(() => vi.fn());
const mockObservedContactStore = vi.hoisted(() => vi.fn(() => ({
    observe: mockObserveContact,
    list: mockListContacts,
})));
const mockLoadSettings = vi.hoisted(() => vi.fn((_cwd, _opts) => ({
    merged: { proxy: 'http://settings-proxy:8080' },
})));
const mockResolveProxyUrl = vi.hoisted(() => vi.fn((_cliProxy, settingsProxy) => settingsProxy));
const mockWriteStderrLine = vi.hoisted(() => vi.fn());
const mockWriteStderrLineSafe = vi.hoisted(() => vi.fn());
const mockWriteStdoutLine = vi.hoisted(() => vi.fn());
const mockSelectFirstModel = vi.hoisted(() => vi.fn((parsed, bridgeLabel) => {
    const models = [
        ...new Set(parsed
    ...
    if (models.length > 1) {
        mockWriteStderrLine(`[Channel] Warning: Multiple models configured (${models.join(', ')}). ` +
            `${bridgeLabel} will use "${models[0]}".`);
    }
    return models[0];
}));
const mockSanitizeLogText = vi.hoisted(() => vi.fn((value) => String(value).replace(/[\r\n]/g, ' ')));
const mockIsChannelProactiveDeliveryError = vi.hoisted(() => vi.fn((error) => typeof error === 'object' &&
    error !== null &&
    error.code ===
        'channel_proactive_delivery_error' &&
    (error.disposition === 'permanent' ||
        error.disposition === 'transient')));
const mockDefaultDaemonClientCapabilities = vi.hoisted(() => vi.fn().mockResolvedValue({
    v: 1,
    mode: 'http-bridge',
    features: [],
    modelServices: [],
...
const mockDefaultDaemonClient = vi.hoisted(() => vi.fn(() => ({
...
const mockChannelLoopStore = vi.hoisted(() => vi.fn(() => ({
...
const mockChannelLoopScheduler = vi.hoisted(() => vi.fn((_options) => ({
    start: mockChannelLoopSchedulerStart,
    stop: mockChannelLoopSchedulerStop,
})));
const mockDaemonChannelBridge = vi.hoisted(() => vi.fn((_options) => ({
...
const mockRouterRestoreRoutes = vi.hoisted(() => vi.fn(() => ({ restored: 1, dropped: 0 })));
const mockRouterDispose = vi.hoisted(() => vi.fn());
const mockSessionRouter = vi.hoisted(() => vi.fn((_bridge, _defaultCwd, _scope, _persistPath) => ({
...
import { createDaemonChannelBridgeFacade, createDaemonSessionFactory, daemonWorkerCommand, runChannelDaemonWorker, } from './daemon-worker.js';
...
beforeEach(() => {
    ...
    mockCreateChannel.mockImplementation((name) => ({
...
describe('createDaemonSessionFactory', () => {
    it('tags created channel sessions without changing loaded sessions', async () => {
        ...
        expect(sdk.DaemonSessionClient.createOrAttach).toHaveBeenCalledWith(sdk.client, {
            workspaceCwd: '/workspace',
            modelServiceId: 'qwen-plus',
            sessionScope: 'thread',
            sourceType: 'channel',
        }, 'qwen-channel-worker');
        expect(sdk.DaemonSessionClient.resume).toHaveBeenCalledWith(sdk.client, 'existing-session', {
            workspaceCwd: '/workspace',
            modelServiceId: 'qwen-plus',
            sessionScope: 'thread',
        }, 'qwen-channel-worker');
...
describe('createDaemonChannelBridgeFacade', () => {
    ...
    it('forwards permission responses when present on bridge', async () => {
        ...
        await expect(facade.respondToPermission?.('req-1', response)).resolves.toBe(true);
...
describe('runChannelDaemonWorker', () => {
    it('wires permanent classifier-session deletion to the worker workspace', async () => {
        ...
        const options = mockDaemonChannelBridge.mock.calls.at(-1)?.[0];
    ...
    it('treats an already-deleted classifier session as deletion success', async () => {
        ...
        const options = mockDaemonChannelBridge.mock.calls.at(-1)?.[0];
        await expect(options.deleteSessionData?.('classifier-session')).resolves.toBeUndefined();
        await handle.close();
    });
    it('propagates per-session daemon deletion errors as a rejection', async () => {
        ...
        const options = mockDaemonChannelBridge.mock.calls.at(-1)?.[0];
        await expect(options.deleteSessionData?.('classifier-session')).rejects.toThrow('storage locked');
        await handle.close();
    });
    it('rejects when the deletion result omits the session entirely', async () => {
        ...
        const options = mockDaemonChannelBridge.mock.calls.at(-1)?.[0];
        await expect(options.deleteSessionData?.('classifier-session')).rejects.toThrow('Session classifier-session was not deleted.');
        await handle.close();
    });
    it('forwards router discard through the daemon bridge facade', async () => {
        ...
        const bridgeFacade = mockSessionRouter.mock.calls[0][0];
        const bindingToken = {};
        expect(bridgeFacade.discardSession).toBeTypeOf('function');
        await bridgeFacade.discardSession?.('orphan-session', bindingToken);
        expect(mockBridgeDiscardSession).toHaveBeenCalledWith('orphan-session', bindingToken);
        expect(mockBridgeDiscardSession.mock.instances[0]).toBe(mockDaemonChannelBridge.mock.results[0].value);
        await handle.close();
    });
    it('starts selected channels through a daemon-backed bridge facade', async () => {
        ...
        expect(mockParseConfiguredChannels).toHaveBeenCalledWith(expect.any(Object), ['telegram'], { defaultCwd: '/workspace' });
        expect(mockDaemonChannelBridge).toHaveBeenCalledWith(expect.objectContaining({
            cwd: '/workspace',
            modelServiceId: 'qwen-plus',
            promptAuthorization: 'worker-prompt-token',
        }));
        ...
        expect(mockCreateChannel).toHaveBeenCalledWith('telegram', parsedTelegram.config, bridgeFacade, expect.objectContaining({
            ...
            stateDir: '/tmp/qwen/channels/daemon/workspace-hash/instances/telegram-hash',
        }));
        expect(mockDaemonChannelStateDir).toHaveBeenCalledWith('/workspace', 'telegram');
        expect(mockDaemonObservedContactsPath).toHaveBeenCalledWith('/workspace');
        expect(mockObservedContactStore).toHaveBeenCalledWith('/tmp/qwen/channels/daemon/workspace-hash/observed-contacts.json');
        const channelOptions = mockCreateChannel.mock.calls[0][3];
        const observation = {
        ...
        expect(mockRegisterPermissionRelay).toHaveBeenCalledWith(bridgeFacade, mockSessionRouter.mock.results[0].value, expect.any(Map));
        expect(mockRegisterBackgroundResponseRelay).toHaveBeenCalledWith(bridgeFacade, mockSessionRouter.mock.results[0].value, expect.any(Map));
        expect(mockResolveProxyUrl).toHaveBeenCalledWith(undefined, 'http://settings-proxy:8080');
        ...
        expect(mockSessionRouter).toHaveBeenCalledWith(expect.any(Object), '/workspace', 'user', '/tmp/qwen/channels/daemon/workspace-hash/routes.json', { recoveryMode: 'lazy' });
        expect(mockRouterRestoreRoutes).toHaveBeenCalledTimes(1);
        expect(mockBridgeLoadSession).not.toHaveBeenCalled();
        expect(mockRouterSetChannelScope.mock.invocationCallOrder[0]).toBeLessThan(mockRouterRestoreRoutes.mock.invocationCallOrder[0]);
        expect(mockRouterRestoreRoutes.mock.invocationCallOrder[0]).toBeLessThan(mockCreateChannel.mock.invocationCallOrder[0]);
        ...
        expect(mockBridgeStop.mock.invocationCallOrder[0]).toBeLessThan(mockRouterDispose.mock.invocationCallOrder[0]);
    ...
    it('starts a workspace-scoped loop runtime for connected channels', async () => {
        ...
        const schedulerOptions = mockChannelLoopScheduler.mock.calls[0][0];
        expect(schedulerOptions.store).toBe(mockChannelLoopStore.mock.results[0].value);
        ...
        const channel = mockCreateChannel.mock.results[0].value;
        expect(channel.connect.mock.invocationCallOrder[0]).toBeLessThan(mockChannelLoopSchedulerStart.mock.invocationCallOrder[0]);
        expect(mockChannelLoopSchedulerStart.mock.invocationCallOrder[0]).toBeLessThan(ready.mock.invocationCallOrder[0]);
        await handle.close();
        expect(mockChannelLoopSchedulerStop).toHaveBeenCalledOnce();
        expect(mockChannelLoopSchedulerStop.mock.invocationCallOrder[0]).toBeLessThan(channel.disconnect.mock.invocationCallOrder[0]);
        expect(channel.disconnect.mock.invocationCallOrder[0]).toBeLessThan(mockBridgeStop.mock.invocationCallOrder[0]);
    });
    it('keeps disconnected channels out of the loop scheduler', async () => {
        ...
        mockCreateChannel.mockImplementation((name) => name === 'first' ? firstChannel : secondChannel);
        const handle = await runChannelDaemonWorker({
            daemonUrl: 'http://127.0.0.1:4170',
            workspace: '/workspace',
            selection: { mode: 'all' },
    ...
    it('disables daemon loop jobs owned by another workspace', async () => {
        ...
        const channel = mockCreateChannel.mock.results[0].value;
        await expect(runner.runLoopPrompt({ id: 'foreign-loop', cwd: '/other' })).rejects.toThrow('outside daemon workspace');
        expect(mockChannelLoopStoreDisable).toHaveBeenCalledWith('foreign-loop');
        expect(mockWriteStderrLine).toHaveBeenCalledWith('[Channel] Disabled loop "foreign-loop": its workspace does not match this daemon worker.');
        expect(channel.runLoopPrompt).not.toHaveBeenCalled();
        mockChannelLoopStoreDisable.mockRejectedValueOnce(new Error('disk full'));
        await expect(runner.runLoopPrompt({ id: 'unwritable-loop', cwd: '/other' })).rejects.toThrow('outside daemon workspace');
        expect(mockWriteStderrLine).toHaveBeenCalledWith('[Channel] Disabled loop "unwritable-loop": its workspace does not match this daemon worker.');
        expect(channel.runLoopPrompt).not.toHaveBeenCalled();
        await expect(runner.runLoopPrompt({ id: 'local-loop', cwd: '/workspace' })).resolves.toBe('done');
        expect(channel.runLoopPrompt).toHaveBeenCalledWith({ id: 'local-loop', cwd: '/workspace' }, undefined);
        await handle.close();
    });
    it('does not create loop runtime when cron is disabled in settings', async () => {
        ...
        expect(mockCreateChannel.mock.calls[0][3]).not.toHaveProperty('loopController');
        await handle.close();
    });
    it('stops the loop scheduler when startup rolls back after connection', async () => {
        ...
        await expect(runChannelDaemonWorker({
            daemonUrl: 'http://127.0.0.1:4170',
            workspace: '/workspace',
            selection: { mode: 'names', names: ['telegram'] },
            loadDaemonSdk: async () => sdk,
    ...
    it('selects all configured channels in one shared router', async () => {
        ...
        expect(mockParseConfiguredChannels).toHaveBeenCalledWith(expect.any(Object), ['telegram', 'feishu'], { defaultCwd: '/workspace' });
        expect(mockSessionRouter).toHaveBeenCalledTimes(1);
        expect(mockRouterSetChannelScope).toHaveBeenCalledWith('telegram', 'thread');
    ...
    it('applies channel approval mode only for webhook-enabled channels', async () => {
        ...
        expect(mockRouterSetChannelApprovalMode).toHaveBeenCalledWith('telegram', 'yolo');
    });
    it('sanitizes channel names before writing connected logs', async () => {
        ...
        expect(mockWriteStdoutLine).toHaveBeenCalledWith('[Channel] Connecting "evil channel"...');
        expect(mockWriteStdoutLine).toHaveBeenCalledWith('[Channel] "evil channel" connected.');
    });
    ...
    it('does not repopulate daemon-private env from worker settings loads', async () => {
        ...
        mockLoadSettings.mockImplementationOnce((_cwd, opts) => {
            if (!opts ||
                typeof opts !== 'object' ||
    ...
    it('waits for each startup failure report before connecting the next channel', async () => {
        ...
        mockCreateChannel
            ...
            connect: vi.fn().mockRejectedValue(Object.assign(new Error('connection refused'), {
                code: 'ECONNREFUSED',
            })),
            disconnect: vi.fn(),
            name: 'telegram',
    ...
    it('converts finite numeric connection error codes to strings', async () => {
        ...
        mockCreateChannel.mockResolvedValueOnce({
            connect: vi.fn().mockRejectedValue(Object.assign(new Error('port unreachable'), {
                code: 443,
            })),
            disconnect: vi.fn(),
            name: 'telegram',
    ...
    it('keeps an acknowledged failure when the next channel connect hangs', async () => {
        ...
        const reportStartup = vi.fn().mockResolvedValue(undefined);
        const secondConnect = vi.fn(() => new Promise(() => {
    ...
    it('reports at most 64 failures and acknowledges one truncation marker', async () => {
        ...
        mockLoadChannelsConfig.mockReturnValueOnce(Object.fromEntries(names.map((name) => [name, { type: 'test' }])));
        mockParseConfiguredChannels.mockResolvedValueOnce(names.map((name) => ({
            name,
            config: {
                type: 'test',
                cwd: '/workspace',
        ...
        mockCreateChannel.mockImplementation(async (name) => ({
        ...
        expect(reportStartup.mock.calls
            .slice(0, 64)
            .map(([message]) => message.type)).toEqual(Array(64).fill('channel_startup_failure'));
    ...
    it('reports requested channels when only some adapters connect', async () => {
        ...
        expect(mockWriteStderrLine).toHaveBeenCalledWith('[Channel] Failed to connect "feishu": feishu boom');
        await handle.close();
    ...
    it('delivers existing text on the matching channel without an agent turn', async () => {
        ...
        expect(deliverProactive).toHaveBeenCalledWith({ channelName: 'telegram', ...deliveryRequest.target }, deliveryRequest.text);
    ...
    it('rejects webhook tasks for channels that are not running', async () => {
        ...
        await expect(handle.runWebhookTask({ ...webhookTask, channelName: 'missing' })).rejects.toThrow('Channel "missing" is not running.');
    });
});
describe('daemonWorkerCommand', () => {
    ...
    it('rejects the legacy static internal sentinel', async () => {
        mockProcessExit();
        const restoreSend = stubProcessSend(vi.fn());
        ...
        try {
            await expect(daemonWorkerCommand.handler({
                channel: ['telegram'],
                _: [],
                $0: 'qwen',
    ...
    it('scrubs daemon connection env before validating channel selection', async () => {
        ...
        vi.stubEnv('QWEN_DAEMON_WORKSPACE', '/workspace');
        try {
            await expect(daemonWorkerCommand.handler({ channel: [' '], _: [], $0: 'qwen' })).rejects.toThrow('process.exit 1');
        }
        finally {
            restoreSend();
        }
        ...
        expect(process.env['QWEN_DAEMON_WORKSPACE']).toBeUndefined();
        expect(process.env['QWEN_CHANNEL_DAEMON_WORKER']).toBeUndefined();
        expect(mockWriteStderrLine).toHaveBeenCalledWith('[Channel] daemon worker failed: --channel requires a non-empty channel name.');
    });
    ...
    it('scrubs inherited loader env vars before starting channels', async () => {
        ...
        vi.stubEnv('npm_config_node-options', '--import file:///other-checkout/hook.mjs');
        try {
            await expect(daemonWorkerCommand.handler({ channel: [' '], _: [], $0: 'qwen' })).rejects.toThrow('process.exit 1');
        }
        finally {
        ...
        // helper exists.
        expect(mockWriteStderrLineSafe).toHaveBeenCalledWith(expect.stringContaining('scrubbed inherited loader env vars'));
    });
    it('scrubs daemon connection env when required env validation fails', async () => {
        ...
        expect(process.env['QWEN_DAEMON_WORKSPACE']).toBeUndefined();
        expect(process.env['QWEN_CHANNEL_DAEMON_WORKER']).toBeUndefined();
        expect(mockWriteStderrLine).toHaveBeenCalledWith('[Channel] daemon worker failed: QWEN_DAEMON_URL is required.');
    });
    it('sends ready from the command handler and exits cleanly on SIGTERM', async () => {
        const exit = mockProcessExitNoThrow();
        const send = vi.fn();
        const restoreSend = stubProcessSend(send);
        vi.stubEnv('QWEN_CHANNEL_DAEMON_WORKER', 'worker-token');
        vi.stubEnv('QWEN_DAEMON_TOKEN', 'daemon-token');
        vi.stubEnv('QWEN_SERVER_TOKEN', 'server-token');
        vi.stubEnv('QWEN_DAEMON_URL', 'http://127.0.0.1:4170');
    ...
    it('waits for the supervisor ACK instead of the process.send callback', async () => {
        ...
        try {
            ...
            await vi.waitFor(() => {
                expect(send).toHaveBeenCalledWith(expect.objectContaining({ type: 'channel_startup_failure' }), expect.any(Function));
            });
            expect(send).not.toHaveBeenCalledWith(expect.objectContaining({ type: 'ready' }));
    ...
    it('aborts startup when parent IPC disconnects while awaiting an ACK', async () => {
        ...
        try {
            ...
            expect(mockWriteStderrLine).toHaveBeenCalledWith('[Channel] daemon worker failed: Daemon worker startup aborted.');
        }
        finally {
            restoreSend();
        }
    ...
    it('sends heartbeat messages while the daemon worker is live', async () => {
        ...
        try {
            ...
            expect(send).toHaveBeenCalledWith(expect.objectContaining({ type: 'heartbeat', pid: process.pid }));
            process.emit('SIGTERM', 'SIGTERM');
            await handler;
            expect(exit).toHaveBeenCalledWith(0);
            send.mockClear();
            await vi.advanceTimersByTimeAsync(15_000);
            expect(send).not.toHaveBeenCalledWith(expect.objectContaining({ type: 'heartbeat' }));
        }
        finally {
            restoreSend();
            vi.useRealTimers();
    ...
    it('clears heartbeat messages when the IPC send channel is closed', async () => {
        ...
        try {
            ...
            await vi.advanceTimersByTimeAsync(15_000);
            expect(send).toHaveBeenCalledWith(expect.objectContaining({ type: 'heartbeat' }));
            send.mockClear();
            await vi.advanceTimersByTimeAsync(15_000);
    ...
    it('honors a shutdown signal received during async setup', async () => {
        ...
        mockBridgeStart.mockImplementationOnce(() => new Promise((resolve) => {
    ...
    it('exits with failure when shutdown fails', async () => {
        ...
        try {
            ...
            await handler;
            expect(exit).toHaveBeenCalledWith(1);
            expect(mockWriteStderrLine).toHaveBeenCalledWith('[Channel] daemon worker failed to shut down after SIGTERM: stop boom');
        }
        finally {
            restoreSend();
        }
    ...
    it('reports delivery success only after the adapter send completes', async () => {
        ...
        const restoreSend = stubProcessSend(send);
        let resolveDelivery;
        const deliverProactive = vi.fn(() => new Promise((resolve) => {
        ...
        try {
            ...
            await vi.waitFor(() => {
                expect(send).toHaveBeenCalledWith(expect.objectContaining({ type: 'ready' }));
            });
            send.mockClear();
            ...
            expect(send).not.toHaveBeenCalledWith(expect.objectContaining({ type: 'channel_delivery_result' }));
            resolveDelivery();
            await vi.waitFor(() => {
                expect(send).toHaveBeenCalledWith({
    ...
    it('rejects webhook IPC messages when the worker webhook queue is full', async () => {
        ...
        const validateWebhookTask = vi.fn();
        const taskResolves = [];
        const runWebhookTask = vi.fn(() => new Promise((resolve) => {
    ...
    it('logs background webhook task failures after acking the IPC message', async () => {
        ...
        try {
            ...
            await vi.waitFor(() => {
                expect(mockWriteStderrLine).toHaveBeenCalledWith('[Channel] webhook task failed ' +
    ...
    it('drains acknowledged webhook tasks before shutting down', async () => {
        ...
        try {
            ...
            process.emit('SIGTERM', 'SIGTERM');
            await vi.waitFor(() => {
                expect(mockWriteStderrLine).toHaveBeenCalledWith('[Channel] shutdown: draining 1 webhook task(s)...');
            });
            expect(disconnect).not.toHaveBeenCalled();
            resolveTask();
            await handler;
...
//# sourceMappingURL=daemon-worker.test.js.map