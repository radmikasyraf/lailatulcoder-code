...
import { compareText, isControlFree, isSafeRepositoryRelativePath, MAX_ARRAY_ITEMS, MAX_IDENTITY_BYTES, MAX_LABEL_LENGTH, MAX_NOTE_LENGTH, MAX_PATH_LENGTH, MAX_TOKEN_LENGTH, validateBoundedString, validateBoundedStringArray, validateRepositoryContext, } from './repository-context.js';
const MANIFEST_PATH = '.lailatulcoder/review-context.json';
/**
...
function validateManifestString(value, field, maxLength) {
 ...
 */
function validateManifestStringArray(value, field, maxLength) {
...
function validateGlob(pattern, field) {
    ...
    if (pattern.length > MAX_PATH_LENGTH ||
        ...
        throw new Error(`repository context manifest ${field} contains unsafe glob`);
    }
    ...
    if (segments.some((segment) => SKIPPED_DIRECTORIES.has(segment.toLowerCase()))) {
        throw new Error(`repository context manifest ${field} enters a skipped directory`);
    }
}
function validateGlobArray(value, field, requireDirectoryPrefix) {
    ...
    for (const pattern of value) {
        ...
        if (requireDirectoryPrefix && /[*?]/.test(pattern.split('/')[0])) {
            throw new Error(`repository context manifest ${field} requires a directory prefix`);
        }
    }
}
function optionalStringArray(rule, field, maxLength) {
...
function parseManifest(content) {
    ...
    if (typeof value !== 'object' ||
        ...
        throw new Error('repository context manifest has unknown or missing fields');
    }
    ...
    const rules = manifest['rules'].map((value, index) => {
        ...
        if (!keys.includes('paths') ||
            ...
            throw new Error(`repository context manifest rules[${index}] has unknown or missing fields`);
        }
        ...
        if (requiredAgents !== undefined) {
            validateManifestStringArray(requiredAgents, `rules[${index}].requiredAgents`, MAX_TOKEN_LENGTH);
            if (requiredAgents.some((role) => !isRepositoryContextRoleId(role))) {
                throw new Error(`repository context manifest rules[${index}].requiredAgents contains an unsupported role`);
            }
        }
        return {
            paths: rule['paths'],
            ...
            recommendedTests: optionalStringArray(rule, 'recommendedTests', MAX_TOKEN_LENGTH),
            requiredConfigurations: optionalStringArray(rule, 'requiredConfigurations', MAX_TOKEN_LENGTH),
            requiredAgents: (requiredAgents ?? []),
            unverifiedDimensions: optionalStringArray(rule, 'unverifiedDimensions', MAX_NOTE_LENGTH),
            verificationNotes: optionalStringArray(rule, 'verificationNotes', MAX_NOTE_LENGTH),
        };
    });
    ...
    const totalPathPatterns = rules.reduce((sum, rule) => sum + rule.paths.length, 0);
...
function isContainedFile(worktree, path) {
    try {
...
function minimalScanRoots(patterns) {
    const roots = sortedUnique(patterns.map(staticDirectoryPrefix));
    return roots.filter((root, index) => !roots.some((candidate, candidateIndex) => candidateIndex !== index && root.startsWith(`${candidate}/`)));
}
function expandRelatedPaths(worktree, patterns, changedPaths) {
    const matches = new Set();
    let candidates = 0;
    const patternLengths = patterns.map((pattern) => pattern.length);
    let matchWork = 0;
    const anyPatternMatches = (path) => {
        for (let index = 0; index < patterns.length; index++) {
            matchWork += patternLengths[index] * path.length;
            if (matchWork > MAX_MATCH_WORK) {
                throw new Error('repository context manifest relatedPaths matching work exceeds limit');
            }
            if (globMatches(patterns[index], path))
                return true;
        }
        return false;
    };
            ...
            if (!stat.isDirectory()) {
                ...
                if (!changedPaths.has(path) &&
                    ...
                    if (matches.size > MAX_ARRAY_ITEMS) {
                        throw new Error('repository context manifest relatedPaths exceeds limit');
                    }
                }
                return;
            }
    ...
    for (const root of minimalScanRoots(patterns))
        visit(root);
 ...
 */
function cappedSortedUnique(values, subject) {
...
export const manifestRepositoryContextProvider = {
    provide(input) {
        ...
        let filterWork = 0;
        const matched = manifest.rules.filter((rule) => input.changedPaths.some((path) => rule.paths.some((pattern) => {
            filterWork += pattern.length * path.length;
            if (filterWork > MAX_MATCH_WORK) {
                throw new Error(`${MANIFEST_PREFIX}paths matching work exceeds limit`);
            }
        ...
        const context = {
            ...
            domains: cappedSortedUnique(matched.flatMap((rule) => rule.domains), 'domains'),
            relatedPaths: expandRelatedPaths(input.worktree, cappedSortedUnique(matched.flatMap((rule) => rule.relatedPaths), 'relatedPaths glob list'), changedPaths),
            recommendedTests: cappedSortedUnique(matched.flatMap((rule) => rule.recommendedTests), 'recommendedTests'),
            requiredConfigurations: cappedSortedUnique(matched.flatMap((rule) => rule.requiredConfigurations), 'requiredConfigurations'),
            requiredAgents: cappedSortedUnique(matched.flatMap((rule) => rule.requiredAgents), 'requiredAgents'),
            unverifiedDimensions: cappedSortedUnique(matched.flatMap((rule) => rule.unverifiedDimensions), 'unverifiedDimensions'),
            verificationNotes: cappedSortedUnique(matched.flatMap((rule) => rule.verificationNotes), 'verificationNotes'),
        };
        return validateRepositoryContext(context);
    },
};
//# sourceMappingURL=manifest-repository-context.js.map