 ...
 */
export function defaultModalities(model) {
 ...
 * in one place.
 */
export function isQwenFamilyWireModel(model) {
    if (!model) {
        return false;
    }
 ...
 */
export function isTieredEffortWireModel(model) {
    if (!model) {
        return false;
    }
    return model.toLowerCase().startsWith('qwen3.8-max');
}
//# sourceMappingURL=modalityDefaults.js.map