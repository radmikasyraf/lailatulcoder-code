...
describe('StreamJsonOutputAdapter', () => {
    ...
    describe('with partial messages enabled', () => {
        ...
        describe('processEvent with stream events', () => {
            ...
            it('emits v2 goal_state before the gated legacy projection', () => {
                ...
                const goalEvents = stdoutWriteSpy.mock.calls
                    ...
                    (message.event?.type === 'goal_state' ||
                        message.event?.type === 'active_goal'));
                expect(goalEvents.map((message) => message.event.type)).toEqual(['goal_state', 'active_goal']);
            ...
            it('does not emit duplicate v2 goal snapshots from overlapping sources', () => {
                ...
                const goalEvents = stdoutWriteSpy.mock.calls
                    .map((call) => JSON.parse(call[0]))
                    .filter((message) => message.event?.type === 'goal_state');
    ...
    describe('with partial messages disabled', () => {
        ...
        it('should not emit active goal stream events', () => {
            ...
            const activeGoalEventCall = stdoutWriteSpy.mock.calls.find((call) => {
                try {
                    const parsed = JSON.parse(call[0]);
                    return (parsed.type === 'stream_event' &&
    ...
    describe('finalizeAssistantMessage', () => {
        ...
        it('should not emit empty assistant message when started but no content processed', () => {
            stdoutWriteSpy.mockClear();
            adapter.finalizeAssistantMessage();
            const assistantCalls = stdoutWriteSpy.mock.calls.filter((call) => {
                try {
                    const parsed = JSON.parse(call[0]);
                    return parsed.type === 'assistant';
                }
    ...
    describe('emitToolProgress', () => {
        ...
        it('should emit shell heartbeats as tool_progress stream events', () => {
            adapter = new StreamJsonOutputAdapter(mockConfig, true);
            stdoutWriteSpy.mockClear();
            adapter.emitToolProgress({ ...mockRequest, name: 'run_shell_command' }, {
                type: 'shell_progress',
                elapsedMs: 10_000,
        ...
        it('should emit multiple tool_progress events for sequential progress updates', () => {
            ...
            const events = stdoutWriteSpy.mock.calls.map((call) => JSON.parse(call[0]).event);
    ...
    describe('content_block event identification', () => {
        ...
        it('should identify content_block events by session_id and index', () => {
            ...
            expect(blockStartCall).toBeDefined();
            const parsed = JSON.parse(blockStartCall[0]);
            expect(parsed.session_id).toBe('test-session-id');
