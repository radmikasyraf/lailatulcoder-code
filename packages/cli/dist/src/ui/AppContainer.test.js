 ...
 */
const { writeTerminalTitleSpy, useWakeRepaintMock, buildWakeRepaintSpy } = vi.hoisted(() => ({
    writeTerminalTitleSpy: vi.fn(),
    useWakeRepaintMock: vi.fn(),
    buildWakeRepaintSpy: vi.fn((deps) => vi.fn(() => deps)),
}));
...
import { AppContainer, dedupeNewestFirst, getSpeculativeToolResult, getNextRenderMode, isInputActiveForState, isRenderModeToggleKey, mergeStartupWarnings, shouldAutoOpenSkillReview, shouldDrainMessageQueue, useQueuedSubmissionDrain, } from './AppContainer.js';
import { formatSessionWindowTitle, writeTerminalTitle, } from './utils/windowTitle.js';
import ansiEscapes from 'ansi-escapes';
import { makeFakeConfig, SendMessageType, } from '@lailatul-coder/lailatul-coder-core';
...
import { CONTEXT_FILES_ANNOUNCEMENT_PREFIX, isContextFilesAnnouncement, } from './utils/commandUtils.js';
...
import { useVimMode, useVimModeActions, useVimModeState, } from './contexts/VimModeContext.js';
...
describe('AppContainer State Management', () => {
    ...
    const mockedUseWorktreeSession = useWorktreeSession;
    const mockedUseVimMode = useVimMode;
    const mockedUseVimModeActions = useVimModeActions;
    const mockedUseVimModeState = useVimModeState;
    ...
    const getGlobalKeypress = () => mockedUseKeypress.mock.calls
        .map((call) => call[0])
        .reverse()
    ...
    describe('speculative tool results', () => {
        it('renders error envelopes as failed tools', () => {
            expect(getSpeculativeToolResult({
    ...
    const rewindUserItem = (id, text, promptId) => ({
        id,
        type: 'user',
        text,
        promptId,
    ...
    const renderRewindHarness = (options = {}) => {
        ...
        vi.spyOn(mockConfig, 'getGeminiClient').mockReturnValue(options.noGeminiClient ? null : geminiClient);
        const rewind = vi.fn();
        if (options.fileRewindError) {
            rewind.mockRejectedValue(options.fileRewindError);
        }
        else {
            rewind.mockResolvedValue(options.fileRewindResult ?? {
        ...
        if (options.contextFilePaths) {
            vi.spyOn(mockConfig, 'getContextFilePaths').mockReturnValue(options.contextFilePaths);
        }
    ...
    const runRewind = async (userItem, option) => {
        await act(async () => {
            await capturedUIActions.handleRewindConfirm(userItem, option);
        });
    };
    describe('worktree branch wiring', () => {
        it('queries the branch from the worktree path during a worktree session', () => {
            ...
            render(_jsx(AppContainer, { config: mockConfig, settings: mockSettings, version: "1.0.0", initializationResult: mockInitResult }));
            expect(mockedUseGitBranchName).toHaveBeenCalledWith('/repo/.qwen/worktrees/feature');
        });
    ...
    describe('Basic Rendering', () => {
        it('continues quitting when cancelling the active request fails', () => {
            ...
            render(_jsx(AppContainer, { config: mockConfig, settings: mockSettings, version: "1.0.0", initializationResult: mockInitResult }));
            const slashCommandActions = mockedUseSlashCommandProcessor.mock.calls.at(-1)?.[12];
        ...
        it('shows recording failures as warnings and unsubscribes on unmount', async () => {
            ...
            vi.spyOn(mockConfig, 'onChatRecordingFailure').mockImplementation((nextListener) => {
    ...
    describe('Context Providers', () => {
        const renderRespondingInput = (slashCommands) => {
        ...
        it('dedupes startup warnings produced during config initialization', () => {
            expect(mergeStartupWarnings(['early warning', 'same warning'], ['same warning', 'late memory warning'])).toEqual(['early warning', 'same warning', 'late memory warning']);
        });
        ...
        it('refreshStatic stays write-free in VP mode for ordinary callers (#8557)', () => {
            ...
            render(_jsx(AppContainer, { config: mockConfig, settings: vpSettings, version: "1.0.0", initializationResult: mockInitResult }));
            ...
            expect(mockStdout.write).not.toHaveBeenCalledWith(ansiEscapes.clearViewport);
        ...
        it('wires the wake repaint (not refreshStatic) into useWakeRepaint', async () => {
            ...
            render(_jsx(AppContainer, { config: mockConfig, settings: vpSettings, version: "1.0.0", initializationResult: mockInitResult, repaintViewport: repaintSpy }));
        ...
        it('defaults to VP mode when useTerminalBuffer is unset', () => {
            ...
            render(_jsx(AppContainer, { config: mockConfig, settings: defaultSettings, version: "1.0.0", initializationResult: mockInitResult }));
        ...
        it('uses the startup VP decision when provided', () => {
            ...
            render(_jsx(AppContainer, { config: mockConfig, settings: legacySettings, version: "1.0.0", initializationResult: mockInitResult, initialUseVirtualViewport: true }));
            expect(capturedUIState.useTerminalBuffer).toBe(true);
        });
        it('uses a disabled startup VP decision over an enabled setting', () => {
            ...
            render(_jsx(AppContainer, { config: mockConfig, settings: vpSettings, version: "1.0.0", initializationResult: mockInitResult, initialUseVirtualViewport: false }));
            expect(capturedUIState.useTerminalBuffer).toBe(false);
        });
        ...
        it('locks terminal buffer mode for the running session', () => {
            ...
            function Wrapper() {
                ...
                return (_jsx(AppContainer, { config: mockConfig, settings: settings, version: "1.0.0", initializationResult: mockInitResult }));
            }
            render(_jsx(Wrapper, {}));
            expect(capturedUIState.useTerminalBuffer).toBe(true);
        ...
        it('does not repaint static history after a resize settles (#8004)', () => {
            ...
            measureElement.mockReturnValue({ width: 80, height: 2 });
            ...
            act(() => {
                columns = 100;
                for (const notify of resizeListeners)
                    notify();
            });
            ...
            vi.useRealTimers();
            measureElement.mockReturnValue(undefined);
        });
        ...
        it('passes a remount-only refresh callback to slash commands', () => {
            let slashRefreshStatic;
            mockedUseSlashCommandProcessor.mockImplementation((_config, _settings, _history, _addItem, _clearItems, _loadHistory, refreshStatic) => {
        ...
        it('binds one Goal host that enqueues, preempts, and cleans up', async () => {
            ...
            const unbind = vi.fn();
            let host;
            vi.spyOn(mockConfig, 'bindGoalTurnHost').mockImplementation((nextHost) => {
                host = nextHost;
                return unbind;
            });
            mockedUseMessageQueue.mockReturnValue({
            ...
            const view = render(_jsx(AppContainer, { config: mockConfig, settings: mockSettings, version: "1.0.0", initializationResult: mockInitResult }));
        ...
        it('holds ordinary input while the Goal is active and drains it once paused', async () => {
            ...
            const view = renderHook(() => useQueuedSubmissionDrain({
                config: mockConfig,
                isConfigInitialized: true,
                streamingState: StreamingState.Idle,
                isProcessing: false,
            ...
            await vi.waitFor(() => {
                ...
                expect(submitQuery).toHaveBeenCalledWith('held user work', SendMessageType.UserQuery, undefined, expect.objectContaining({
        ...
        it('treats paused, blocked and usage_limited Goals as drain-eligible', async () => {
            ...
            for (const status of ['paused', 'blocked', 'usage_limited']) {
                ...
                await vi.waitFor(() => {
                    expect(popNextSubmission).toHaveBeenCalledWith('normal');
                    expect(submitQuery).toHaveBeenCalledWith('ordinary user work', SendMessageType.UserQuery, undefined, expect.objectContaining({
        ...
        it('does not hot-loop a queued submission whose admission keeps failing', async () => {
            ...
            const submitQuery = vi.fn(async (...args) => {
                const metadata = args[3];
                metadata?.onAdmissionFailed?.();
                throw new Error('persistent prepare failure');
            });
            const { rerender } = renderHook(({ pendingSubmissionCount, submissionSettledRevision }) => useQueuedSubmissionDrain({
                config: mockConfig,
                isConfigInitialized: true,
                streamingState: StreamingState.Idle,
                isProcessing: false,
        ...
        it('drains after preprocessing settlement releases the shared lock', async () => {
            ...
            const { rerender } = renderHook(({ submissionSettledRevision }) => useQueuedSubmissionDrain({
                config: mockConfig,
                isConfigInitialized: true,
                streamingState: StreamingState.Idle,
                isProcessing: false,
            ...
            await vi.waitFor(() => {
                expect(submitQuery).toHaveBeenCalledWith('queued during preprocessing', SendMessageType.UserQuery, undefined, expect.objectContaining({
                    userAdmission: {
                        turnKey: 'message-queue:during-preprocessing',
                    },
                }));
        ...
        it('marks Ctrl+Q submissions to wait for the idle boundary', () => {
            ...
            expect(mockQueueMessage).toHaveBeenCalledWith('/btw next turn', true, '/btw next turn');
        ...
        it('submits /btw immediately instead of queueing while responding', () => {
            ...
            expect(mockSubmitQuery).toHaveBeenCalledWith('/btw quick side question', SendMessageType.UserQuery, undefined, expect.objectContaining({
                submittedPrompt: '/btw quick side question',
        ...
        it('queues a responding ?btw submission when concurrent admission fails', () => {
            ...
            const metadata = mockSubmitQuery.mock.calls[0]?.[3];
            metadata?.onAdmissionFailed?.();
            expect(mockQueueMessage).toHaveBeenCalledWith('?btw wait for the tool', true, '?btw wait for the tool');
        });
        it('runs opted-in slash commands outside the active turn while responding', () => {
            const { handleSlashCommand, submitQuery, addMessage } = renderRespondingInput([
                {
                    name: 'settings',
        ...
        it('submits slash commands immediately instead of queueing while idle', () => {
            ...
            expect(mockSubmitQuery).toHaveBeenCalledWith('/model', SendMessageType.UserQuery, undefined, { submittedPrompt: '/model' });
            expect(mockQueueMessage).not.toHaveBeenCalled();
        });
        it('injects a recovered-agent reminder into the next ordinary prompt once', () => {
            ...
            expect(mockQueueMessage).toHaveBeenNthCalledWith(1, '<system-reminder>\nUse list_agents to inspect restored agents.\n' +
                '</system-reminder>\n\ncontinue the review', false, 'continue the review');
            expect(mockQueueMessage).toHaveBeenNthCalledWith(2, 'one more check', false, 'one more check');
        });
        it('preserves unchanged queue provenance across the input clear before submit', () => {
            ...
            expect(mockQueueMessage).toHaveBeenCalledWith(modelText, false, 'review this');
        });
        it('separates edited restores from same-text history resubmissions', () => {
            ...
            expect(mockQueueMessage).toHaveBeenCalledWith(modelText, false, undefined);
            mockQueueMessage.mockClear();
            expect(capturedUIActions.popAllQueuedMessages()).toBe(modelText);
            capturedUIActions.handleFinalSubmit(`${modelText} with edits`, {
                submittedPrompt: `${modelText} with edits`,
            });
            expect(mockQueueMessage).toHaveBeenCalledWith(`${modelText} with edits`, false, undefined);
            mockQueueMessage.mockClear();
            expect(capturedUIActions.popAllQueuedMessages()).toBe(modelText);
            capturedUIActions.handleFinalSubmit(`${modelText} `, {
                submittedPrompt: `${modelText} `,
            ...
            expect(mockQueueMessage).toHaveBeenCalledWith('fresh prompt', false, 'fresh prompt');
            mockQueueMessage.mockClear();
            expect(capturedUIActions.popAllQueuedMessages()).toBe(modelText);
        ...
        it('treats a restored prompt stash as provenance unavailable', () => {
            const stashedText = '<system-reminder>\ngenerated context\n</system-reminder>\n\nuser text';
            const mockQueueMessage = vi.fn();
            const setText = vi.fn();
            ...
            mockedRestorePromptStash.mockImplementation((_targetDir, _currentText, onRestore) => {
            ...
            expect(mockQueueMessage).toHaveBeenCalledWith(stashedText, false, undefined);
        ...
        it('clears restored prompt undo history after a manual clear', () => {
            ...
            expect(addMessage).toHaveBeenCalledWith('fresh prompt', false, 'fresh prompt');
        });
        ...
        it('uses the explicit pre-attachment text as provenance', () => {
            ...
            render(_jsx(AppContainer, { config: mockConfig, settings: mockSettings, version: "1.0.0", initializationResult: mockInitResult }));
            capturedUIActions.handleFinalSubmit('@.qwen/tmp/clipboard.png\n\ndescribe this image', { submittedPrompt: 'describe this image' });
            expect(mockQueueMessage).toHaveBeenCalledWith('@.qwen/tmp/clipboard.png\n\ndescribe this image', false, 'describe this image');
        });
        it('does not consume composer provenance for a programmatic submission', () => {
            ...
            expect(mockQueueMessage).toHaveBeenCalledWith('configured initial prompt', false, undefined);
            expect(setText).toHaveBeenCalledTimes(1);
            capturedUIActions.handleFinalSubmit(stashedText, {
                submittedPrompt: stashedText,
            });
            expect(mockQueueMessage).toHaveBeenLastCalledWith(stashedText, false, undefined);
        ...
        it('omits provenance while Vim mode is enabled', () => {
            ...
            expect(mockQueueMessage).toHaveBeenCalledWith('vim prompt', false, undefined);
        });
        it('keeps Vim-modified composer text ineligible after Vim is disabled', () => {
            ...
            expect(mockQueueMessage).toHaveBeenCalledWith('register contents', false, undefined);
        });
        it.each(['exit', 'quit', ':q', ':q!', ':wq', ':wq!'])('routes bare "%s" to /quit instead of sending as a message', (command) => {
        ...
        it.each(['/quit', '/exit'])('routes "%s" immediately while responding', (command) => {
            const mockHandleSlashCommand = vi.fn();
            const mockQueueMessage = vi.fn();
            mockedUseSlashCommandProcessor.mockReturnValue({
                handleSlashCommand: mockHandleSlashCommand,
    ...
    describe('Cancel Handler (issue #3204)', () => {
        ...
        let capturedOnCancelSubmit = null;
        ...
        const cancelInfoFor = (text, id = 1, submittedPrompt) => ({
            pendingItem: null,
            lastTurnUserItem: {
                id,
                text,
        ...
        const installCancelCapture = (streamReturnValue) => {
            capturedOnCancelSubmit = null;
            mockedUseGeminiStream.mockImplementation((...args) => {
                const candidate = args[ON_CANCEL_SUBMIT_ARG_INDEX];
                if (typeof candidate === 'function') {
                    capturedOnCancelSubmit = candidate;
                }
                return {
                    ...streamReturnValue,
                    streamingResponseLengthRef: { current: 0 },
        ...
        const triggerCancel = (info) => {
            if (!capturedOnCancelSubmit) {
                throw new Error(`onCancelSubmit was not captured at arg index ${ON_CANCEL_SUBMIT_ARG_INDEX} — useGeminiStream signature may have changed`);
            }
        ...
        it('moves queued follow-up messages into an empty buffer on cancel', async () => {
            ...
            expect(mockSetText).not.toHaveBeenCalledWith(expect.stringContaining('the previous prompt'));
        ...
        it('releases queued Goal turn reservations on cancel using goal-turn keys', async () => {
            ...
            triggerCancel();
            expect(removeGoalTurns).toHaveBeenCalledTimes(1);
            await vi.waitFor(() => expect(releaseTurn).toHaveBeenCalledWith('goal-runtime:turn-1'));
        });
    ...
    describe('Settings Integration', () => {
        ...
        it('initializes Markdown render mode from ui.renderMode', () => {
            ...
            render(_jsx(AppContainer, { config: mockConfig, settings: rawSettings, version: "1.0.0", initializationResult: mockInitResult }));
        ...
        it('falls back to rendered Markdown mode for missing or invalid ui.renderMode', () => {
            ...
            render(_jsx(AppContainer, { config: mockConfig, settings: invalidSettings, version: "1.0.0", initializationResult: mockInitResult }));
    ...
    describe('Terminal Title Update Feature', () => {
        ...
        it('should not update terminal title when showStatusInTitle is false', () => {
            ...
            const titleWrites = process.stdout.write.mock.calls.filter((call) => call[0].includes('\x1b]2;'));
        ...
        it('should keep default terminal title when active without a session name', () => {
            ...
            expect(titleWrites[0][0]).toBe(titleEscape(`${ICON.CIRCLE_LEFT_HALF} Qwen - workspace`));
            unmount();
        });
        ...
        it('should keep default terminal title when waiting for confirmation without a session name', () => {
            ...
            const titleWrites = process.stdout.write.mock.calls.filter((call) => call[0].includes('\x1b]2;'));
            expect(titleWrites).toHaveLength(1);
            expect(titleWrites[0][0]).toBe(titleEscape(`${ICON.SPARKLE} Qwen - workspace`));
            unmount();
        });
        it('should pad the terminal title to 80 characters', () => {
            ...
            expect(calledWith).toBe(titleEscape(`${ICON.CIRCLE_LEFT_HALF} Qwen - workspace`));
            unmount();
        });
        ...
        it('should register for recorded session titles and format them in the terminal title', async () => {
            ...
            const setTitleRecordedCallback = vi.fn((callback) => {
            ...
            expect(registeredTitleRecordedCallback).toStrictEqual(expect.any(Function));
            const currentSessionId = mockConfig.getSessionId();
            await act(async () => {
                registeredTitleRecordedCallback('Fix terminal title', 'manual', currentSessionId);
            });
            ...
            expect(formatSessionWindowTitle('Fix terminal title')).toBe('Fix terminal title');
            ...
            expect(formatSessionWindowTitle(null, 'my-project')).toBe('Qwen - my-project');
        ...
        it('should chain with existing titleRecordedCallback from Session (ACP notifications)', async () => {
            ...
            expect(existingCallback).toHaveBeenCalledWith('Test title', 'rename', currentSessionId);
            await act(async () => {
                titleRecordedCallback('Stale title', 'auto', 'old-session-id');
            });
            expect(existingCallback).toHaveBeenLastCalledWith('Stale title', 'auto', 'old-session-id');
            unmount();
        ...
        it('should revert to static title when showStatusInTitle toggles from true to false', () => {
            ...
            expect(writeSpy).toHaveBeenCalledWith(expect.stringContaining(`\x1b]2;${padded}\x07`));
        });
    });
    describe('Terminal Height Calculation', () => {
        const mockedMeasureElement = measureElement;
        const mockedUseTerminalSize = useTerminalSize;
        const makeTodoHistory = (status) => [
            {
                type: 'tool_group',
                id: 1,
                tools: [
        ...
        it('loads a collapsed summary into history on cold-boot resume when collapseOnResume is enabled', async () => {
            ...
            vi.spyOn(mockConfig, 'initialize').mockResolvedValue(undefined);
            vi.spyOn(mockConfig, 'getResumedSessionData').mockReturnValue(resumeSessionData);
            ...
            expect(historyManager.loadHistory).toHaveBeenCalledWith(expect.arrayContaining([
            ...
            expect(historyManager.history
                .slice(0, -1)
                .every((item) => item.display?.suppressOnRestore === true)).toBe(true);
        });
        it('does not remeasure footer height for sticky todo status-only updates', async () => {
            ...
            let view;
            await act(async () => {
                view = render(_jsx(AppContainer, { config: mockConfig, settings: mockSettings, version: "1.0.0", initializationResult: mockInitResult }));
            });
            // Let any pending state updates from useLayoutEffect settle.
            await act(async () => {
                view.rerender(_jsx(AppContainer, { config: mockConfig, settings: mockSettings, version: "1.0.0", initializationResult: mockInitResult }));
            });
    ...
    describe('Model change refreshStatic wiring', () => {
        function captureModelChangeListener(config) {
            ...
            const subs = [];
            const fakeOnModelChange = vi.fn((cb) => {
            ...
            config.onModelChange = fakeOnModelChange;
            return {
        ...
        it('fires refreshStatic only once per real model change (StrictMode-safe)', async () => {
            ...
            trigger.notify('model-b');
            await flushEffects();
            const clearWrites = mockStdout.write.mock.calls.filter(([arg]) => arg === ansiEscapes.clearTerminal);
    ...
    describe('handleRewindConfirm', () => {
        it('skips conversation truncation when both-mode file restore fails', async () => {
            ...
            expect(harness.addItem).toHaveBeenCalledWith(expect.objectContaining({
                type: 'error',
        ...
        it('truncates conversation when both-mode file restore succeeds', async () => {
            ...
            expect(mockClearPendingState).toHaveBeenCalledTimes(1);
            expect(mockClearPendingState.mock.invocationCallOrder[0]).toBeLessThan(harness.loadHistory.mock.invocationCallOrder[0]);
            ...
            expect(harness.rewindRecording).toHaveBeenCalledWith(1, { truncatedCount: 2 }, harness.snapshots.slice(0, 2));
        });
        it('re-arms the latch when rewinding past the context-file announcement', async () => {
            ...
            const announcementsBefore = harness.addItem.mock.calls.filter(([item]) => isContextFilesAnnouncement(item));
            expect(announcementsBefore).toHaveLength(1);
            await runRewind(harness.target, 'both');
            capturedUIActions.handleFinalSubmit('again', {
                submittedPrompt: 'again',
            });
            const announcementsAfter = harness.addItem.mock.calls.filter(([item]) => isContextFilesAnnouncement(item));
        ...
        it('keeps the latch consumed when rewinding to a turn after the announcement', async () => {
            ...
            const announcements = harness.addItem.mock.calls.filter(([item]) => isContextFilesAnnouncement(item));
        ...
        it('rewinds conversation only without restoring files', async () => {
            ...
            expect(harness.addMessage).toHaveBeenCalledWith('second prompt', false, undefined);
    ...
    describe('Skill review auto-open gating (shouldAutoOpenSkillReview)', () => {
        ...
        it('does NOT open while auto-skill is disabled (the turn-off flow)', () => {
            ...
            expect(shouldAutoOpenSkillReview({ ...openable, autoSkillEnabled: false })).toBe(false);
        });
        it('does NOT open over an open /memory dialog', () => {
            expect(shouldAutoOpenSkillReview({ ...openable, isMemoryDialogOpen: true })).toBe(false);
        });
        it('opens again once /memory closes with auto-skill re-enabled', () => {
            ...
            expect(shouldAutoOpenSkillReview({ ...openable, isMemoryDialogOpen: false })).toBe(true);
        });
        it('does NOT reopen a batch the user dismissed with Esc', () => {
            expect(shouldAutoOpenSkillReview({
                ...openable,
        ...
        it('does NOT open while streaming or with no pending skills', () => {
            ...
            expect(shouldAutoOpenSkillReview({ ...openable, pending: null })).toBe(false);
            expect(shouldAutoOpenSkillReview({
                ...openable,
    ...
    describe('context files announcement (#5267)', () => {
        const renderAnnouncementHarness = (contextFilePaths) => {
            const addItem = vi.fn();
            const loadHistory = vi.fn();
            ...
            vi.spyOn(mockConfig, 'getContextFilePaths').mockReturnValue(contextFilePaths);
            const view = render(_jsx(AppContainer, { config: mockConfig, settings: mockSettings, version: "1.0.0", initializationResult: mockInitResult }));
            return { addItem, enqueueMessage, loadHistory, view };
        };
        const announcementCalls = (addItem) => addItem.mock.calls.filter(([item]) => isContextFilesAnnouncement(item));
        it('announces loaded context files above the first real prompt, once', () => {
            ...
            const announcementIndex = addItem.mock.calls.findIndex(([item]) => isContextFilesAnnouncement(item));
            expect(addItem.mock.invocationCallOrder[announcementIndex]).toBeLessThan(enqueueMessage.mock.invocationCallOrder[0]);
            capturedUIActions.handleFinalSubmit('again', {
                submittedPrompt: 'again',
            });
            expect(announcementCalls(addItem)).toHaveLength(1);
        ...
        it('performMemoryRefresh anchors on config.getWorkingDir() and updates contextFilePaths', async () => {
            ...
            vi.spyOn(mockConfig, 'getWorkingDir').mockReturnValue('/custom/workspace');
            ...
            const setContextFilePathsSpy = vi.spyOn(mockConfig, 'setContextFilePaths');
            render(_jsx(AppContainer, { config: mockConfig, settings: mockSettings, version: "1.0.0", initializationResult: mockInitResult }));
            ...
            const performMemoryRefresh = calls[calls.length - 1][11];
            expect(typeof performMemoryRefresh).toBe('function');
            await act(async () => {
                await performMemoryRefresh();
            });
            expect(mockLoadHierarchicalGeminiMemory).toHaveBeenCalledWith('/custom/workspace', expect.anything(), expect.anything(), ['ext-context.md'], true, expect.anything(), ['exclude-rule'], expect.anything());
