...
import { terminalToGrid, pointInViewport, clampToViewport, } from './selection-coords.js';
const sameViewportContent = (previous, current, rect) => {
    if (!previous) {
        return false;
    }
...
const sameViewportRect = (previous, current) => previous !== null &&
    current !== null &&
 ...
 * changes.
 */
export function TextSelectionController(props) {
    const { stdout } = useStdout();
    const selectionRef = useRef(new SelectionState());
    const dragScrollTopRef = useRef(null);
    const baselineScrollTopRef = useRef(0);
    const baselineScrollHeightRef = useRef(0);
    const baselineFrameRef = useRef(null);
    const baselineRectRef = useRef(null);
    const activeRectIndexRef = useRef(null);
    ...
    const pendingInvalidationGenRef = useRef(null);
    ...
    const applyHighlight = useCallback(() => {
        ...
        getBuffer()?.setSelection(normalized && !selection.isBareClick ? normalized : null);
    }, [getBuffer]);
    const getSelectableRects = useCallback(() => {
        const viewportRect = propsRef.current.getViewportRect();
        if (!viewportRect) {
            return [];
        }
        const additionalRects = propsRef.current.getAdditionalSelectableRects?.() ?? [];
    ...
    const recordBaseline = useCallback((rect) => {
    ...
    const mapEvent = useCallback((event) => {
        ...
        const terminalHeight = stdout.rows ?? frameHeight;
        const point = terminalToGrid(event.col, event.row, terminalHeight, frameHeight);
        const row = buffer.frame?.cells[point.y];
        const snappedPoint = point.x > 0 &&
            row?.[point.x]?.value === '' &&
    ...
    const extendSpanDrag = useCallback((point) => {
        ...
        const cursorAfter = point.y > span.ey || (point.y === span.ey && point.x >= span.ex);
    ...
    const extendActiveDrag = useCallback((point) => {
    ...
    const handleMouse = useCallback((event) => {
        ...
        if (event.name === 'left-press') {
            selectionGenerationRef.current += 1;
            if (propsRef.current.hitTestScrollbar({ col: event.col, row: event.row })) {
                clearSelection();
                return;
            }
