...
import { getAvailableCommands, handleSlashCommand, } from './nonInteractiveCliCommands.js';
import { __resetActiveGoalStoreForTests, createGoalRuntime, uiTelemetryService, } from '@lailatul-coder/lailatul-coder-core';
...
describe('handleSlashCommand', () => {
    ...
    let mockSettings;
    let abortController;
    let mockFireUserPromptExpansionEvent;
    const createJournal = () => ({
    ...
    beforeEach(() => {
        ...
        mockGetCommandsForMode.mockImplementation((mode) => filterCommandsForMode(mockGetCommands(), mode));
        mockGetModelInvocableCommands.mockImplementation(() => mockGetCommands().filter((command) => !command.hidden && command.modelInvocable === true));
    ...
    it('should return no_command for non-slash input', async () => {
        const result = await handleSlashCommand('regular text', abortController, mockConfig, mockSettings);
        expect(result.type).toBe('no_command');
    });
    it('should return no_command for unknown slash commands', async () => {
        mockGetCommands.mockReturnValue([]);
        const result = await handleSlashCommand('/unknowncommand', abortController, mockConfig, mockSettings);
        expect(result.type).toBe('no_command');
    });
    it('should return unsupported for built-in commands without non-interactive supportedModes', async () => {
        ...
        const result = await handleSlashCommand('/help', abortController, mockConfig, mockSettings);
    ...
    it('should return unsupported for /help when using default allowed list', async () => {
        ...
        expect(result.type).toBe('unsupported');
        if (result.type === 'unsupported') {
            expect(result.reason).toBe('The command "/help" is not supported in this mode.');
        }
    });
    it('should execute local commands with non_interactive supportedModes', async () => {
        ...
        const result = await handleSlashCommand('/init', abortController, mockConfig, mockSettings);
    ...
    it('should execute /btw with non_interactive supportedModes', async () => {
        ...
        const result = await handleSlashCommand('/btw question', abortController, mockConfig, mockSettings);
        expect(mockBtwCommand.action).toHaveBeenCalledWith(expect.objectContaining({ abortSignal: undefined }), 'question');
        expect(result.type).toBe('message');
        if (result.type === 'message') {
    ...
    it('passes the abort signal to executed commands', async () => {
        ...
        await handleSlashCommand('/advisor check this', abortController, mockConfig, mockSettings);
        expect(mockCommand.action).toHaveBeenCalledWith(expect.objectContaining({ abortSignal: abortController.signal }), 'check this');
    });
    it('returns canonical goal_control for a non-interactive create', async () => {
        mockGetCommands.mockReturnValue([goalCommand]);
        const result = await handleSlashCommand('/goal write a hello world script', abortController, mockConfig, mockSettings);
    ...
    it('returns canonical goal_control for empty non-interactive /goal status', async () => {
        mockGetCommands.mockReturnValue([goalCommand]);
        const result = await handleSlashCommand('/goal', abortController, mockConfig, mockSettings);
        expect(result).toMatchObject({
            type: 'goal_control',
    ...
    it('returns the active v2 snapshot for status after create', async () => {
        mockGetCommands.mockReturnValue([goalCommand]);
        await handleSlashCommand('/goal write a hello world script', abortController, mockConfig, mockSettings);
        const result = await handleSlashCommand('/goal', abortController, mockConfig, mockSettings);
        expect(result).toMatchObject({
            type: 'goal_control',
            operation: { kind: 'status' },
    ...
    it('returns the cleared v2 snapshot for non-interactive /goal clear', async () => {
        mockGetCommands.mockReturnValue([goalCommand]);
        await handleSlashCommand('/goal write a hello world script', abortController, mockConfig, mockSettings);
        const result = await handleSlashCommand('/goal clear', abortController, mockConfig, mockSettings);
        expect(result).toMatchObject({
            type: 'goal_control',
    ...
    it('should execute FILE commands in any mode without explicit supportedModes', async () => {
        ...
        const result = await handleSlashCommand('/custom', abortController, mockConfig, mockSettings);
    ...
    it('passes context-file refresh intent through submit_prompt results', async () => {
        ...
        mockGetCommands.mockReturnValue([mockCommand]);
        const result = await handleSlashCommand('/remember fact', abortController, mockConfig, mockSettings);
        expect(result.type).toBe('submit_prompt');
        if (result.type === 'submit_prompt') {
    ...
    it('records successful SKILL submit_prompt commands in session metrics', async () => {
        ...
        const result = await handleSlashCommand('/review', abortController, mockConfig, mockSettings);
        expect(result.type).toBe('submit_prompt');
        expect(uiTelemetryService.getMetricsForSession('test-session').skills).toEqual({
            totalCalls: 1,
            totalSuccess: 1,
            totalFail: 0,
            byName: {
    ...
    it('records failed SKILL commands when action throws', async () => {
        ...
        mockGetCommands.mockReturnValue([mockSkillCommand]);
        await expect(handleSlashCommand('/review', abortController, mockConfig, mockSettings)).rejects.toThrow('boom');
        expect(uiTelemetryService.getMetricsForSession('test-session').skills).toEqual({
            totalCalls: 1,
            totalSuccess: 0,
            totalFail: 1,
    ...
    it('records SKILL submit_prompt commands as failures when hooks throw', async () => {
        ...
        mockGetCommands.mockReturnValue([mockSkillCommand]);
        await expect(handleSlashCommand('/review', abortController, mockConfig, mockSettings)).rejects.toThrow('hook crash');
        expect(uiTelemetryService.getMetricsForSession('test-session').skills).toEqual({
            totalCalls: 1,
            totalSuccess: 0,
            totalFail: 1,
    ...
    it('should fire UserPromptExpansion hooks for submit_prompt commands', async () => {
        ...
        mockGetCommands.mockReturnValue([mockFileCommand]);
        const result = await handleSlashCommand('/custom with args', abortController, mockConfig, mockSettings);
        expect(result.type).toBe('submit_prompt');
        expect(mockFireUserPromptExpansionEvent).toHaveBeenCalledWith('custom', 'with args', 'Expanded prompt', abortController.signal);
    });
    ...
    it('should return the block reason for blocked model-invocable command execution', async () => {
        ...
        mockGetCommands.mockReturnValue([mockFileCommand]);
        await handleSlashCommand('/custom', abortController, mockConfig, mockSettings);
    ...
    it('should return unsupported for other built-in commands like /quit', async () => {
        ...
        const result = await handleSlashCommand('/quit', abortController, mockConfig, mockSettings);
        expect(result.type).toBe('unsupported');
        if (result.type === 'unsupported') {
    ...
    it('should handle command with no action', async () => {
        ...
        mockGetCommands.mockReturnValue([mockCommand]);
        const result = await handleSlashCommand('/noaction', abortController, mockConfig, mockSettings);
        expect(result.type).toBe('no_command');
    });
    it('should return message when command returns void', async () => {
        ...
        mockGetCommands.mockReturnValue([mockCommand]);
        const result = await handleSlashCommand('/voidcmd', abortController, mockConfig, mockSettings);
        expect(result.type).toBe('message');
        if (result.type === 'message') {
    ...
    describe('disabled slash commands', () => {
        ...
        it('does not expose disabled model-invocable commands through SkillTool', async () => {
            ...
            mockCommandServiceCreate.mockImplementation(async (_loaders, _signal, disabledNames) => {
                const commands = disabledNames?.has('custom') === true
                    ? []
                    : [modelInvocableCommand];
                return {
                    getCommands: () => commands,
                    getCommandsForMode: (mode) => filterCommandsForMode(commands, mode),
                    getModelInvocableCommands: () => commands.filter((command) => command.modelInvocable === true),
                };
            });
            const result = await handleSlashCommand('/custom', abortController, mockConfig, mockSettings);
            expect(result.type).toBe('unsupported');
    ...
    describe('stacked skill invocations', () => {
        const createSkillCommand = (name, body) => ({
            name,
        ...
        it('combines two stacked skills into a single submit_prompt', async () => {
            ...
            const result = await handleSlashCommand('/feat-dev /e2e-testing implement X', abortController, mockConfig, mockSettings);
            expect(result.type).toBe('submit_prompt');
            if (result.type === 'submit_prompt') {
        ...
        it('preserves context-file refresh intent from stacked skills', async () => {
            ...
            const skillB = createSkillCommand('e2e-testing', 'e2e workflow');
            mockGetCommands.mockReturnValue([skillA, skillB]);
            const result = await handleSlashCommand('/remember-skill /e2e-testing implement X', abortController, mockConfig, mockSettings);
            expect(result.type).toBe('submit_prompt');
            if (result.type === 'submit_prompt') {
                expect(result.refreshContextFilesOnWrite).toBe(true);
            }
        ...
        it('calls each skill action once', async () => {
            ...
            mockGetCommands.mockReturnValue([skillA, skillB]);
            await handleSlashCommand('/feat-dev /review do stuff', abortController, mockConfig, mockSettings);
        ...
        it('records successful stacked project auto-skills as used', async () => {
            ...
            mockGetCommands.mockReturnValue([skillA, skillB]);
            const result = await handleSlashCommand('/feat-dev /review do stuff', abortController, mockConfig, mockSettings);
            expect(result.type).toBe('submit_prompt');
        ...
        it('handles stacked skills with no remaining text', async () => {
            ...
            mockGetCommands.mockReturnValue([skillA, skillB]);
            const result = await handleSlashCommand('/feat-dev /bugfix', abortController, mockConfig, mockSettings);
            expect(result.type).toBe('submit_prompt');
            if (result.type === 'submit_prompt') {
                const content = result.content;
                const texts = content.map((c) => c.text);
        ...
        it('falls through to normal dispatch for a single skill', async () => {
            ...
            const result = await handleSlashCommand('/feat-dev build something', abortController, mockConfig, mockSettings);
        ...
        it('skips skills whose action is undefined', async () => {
            ...
            const result = await handleSlashCommand('/feat-dev /no-action /review do it', abortController, mockConfig, mockSettings);
            expect(result.type).toBe('submit_prompt');
            if (result.type === 'submit_prompt') {
                const content = result.content;
                const texts = content.map((c) => c.text);
        ...
        it('excludes non-submit_prompt results from combined content', async () => {
            ...
            const result = await handleSlashCommand('/feat-dev /error-skill /review do it', abortController, mockConfig, mockSettings);
            expect(result.type).toBe('submit_prompt');
            if (result.type === 'submit_prompt') {
                const content = result.content;
                const texts = content.map((c) => c.text);
        ...
        it('records telemetry success=false for non-submit_prompt results', async () => {
            ...
            const result = await handleSlashCommand('/feat-dev /error-skill do it', abortController, mockConfig, mockSettings);
        ...
        it('propagates modelOverride from first submit_prompt skill', async () => {
            ...
            const skillB = createSkillCommand('review', 'b');
            mockGetCommands.mockReturnValue([skillA, skillB]);
            const result = await handleSlashCommand('/feat-dev /review do it', abortController, mockConfig, mockSettings);
            expect(result.type).toBe('submit_prompt');
            if (result.type === 'submit_prompt') {
...
describe('getAvailableCommands', () => {
    let mockConfig;
    let notifyConfigChanged;
    let fireUserPromptExpansionEvent;
    beforeEach(() => {
        vi.clearAllMocks();
        mockGetCommandsForMode.mockImplementation((mode) => filterCommandsForMode(mockGetCommands(), mode));
        mockGetModelInvocableCommands.mockImplementation(() => mockGetCommands().filter((command) => !command.hidden && command.modelInvocable === true));
    ...
    it('includes /export in the default non-interactive command list', async () => {
        ...
        const commands = await getAvailableCommands(mockConfig, new AbortController().signal);
    ...
    it('registers model-invocable commands for ACP command snapshots', async () => {
        ...
        await getAvailableCommands(mockConfig, expiredSnapshotSignal.signal, 'acp', {
            system: { path: '', settings: {} },
            systemDefaults: { path: '', settings: {} },
            user: { path: '', settings: {} },
            workspace: { path: '', settings: {} },
        ...
        const executor = vi.mocked(mockConfig.setModelInvocableCommandsExecutor)
            .mock.calls[0]?.[0];
        await expect(executor?.('expand-prompt', 'with args')).resolves.toBe('expanded prompt');
        ...
        expect(promptCommand.action).toHaveBeenCalledWith(expect.objectContaining({
            executionMode: 'acp',
            invocation: {
...
//# sourceMappingURL=nonInteractiveCliCommands.test.js.map