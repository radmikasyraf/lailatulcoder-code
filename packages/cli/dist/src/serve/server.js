...
import { allowOriginCors, bearerAuth, createMutationGate, hostAllowlist, MutableOriginAllowlist, parseAllowOriginPatterns, } from './auth.js';
import { CredentialStore, listenerIdentityOf, LocalControlService, } from './local-control/index.js';
...
import { canonicalizeWorkspace, createAcpSessionBridge, MAX_SESSION_RESTORE_TIMEOUT_MS, resolveSessionRestoreTimeoutMs, } from './acp-session-bridge.js';
import {} from './types.js';
import { mountWebShellAssets, mountWebShellSpaFallback, } from './web-shell-static.js';
import { mountWorkspaceMemoryRoutes, mountWorkspaceQualifiedMemoryRoutes, } from './workspace-memory.js';
import { mountWorkspaceMemoryRememberRoutes, mountWorkspaceQualifiedMemoryRememberRoutes, WorkspaceRememberTaskLane, } from './workspace-remember.js';
...
import { createUploadConcurrencyGate, registerWorkspaceFileWriteRoutes, registerWorkspaceFileUploadRoutes, registerWorkspaceQualifiedFileWriteRoutes, registerWorkspaceQualifiedFileUploadRoutes, } from './routes/workspace-file-write.js';
...
import { getActiveSseCount, registerSseEventsRoutes, } from './routes/sse-events.js';
...
import { createBuildWorkspaceCtx, parseAndValidateWorkspaceClientId, parseClientIdHeader, safeBody, } from './server/request-helpers.js';
import { daemonInboundTraceIdCaptureMiddleware, daemonTelemetryMiddleware, } from './server/telemetry.js';
...
import { installFinalErrorHandler, installJsonBodyParser, } from './server/error-handlers.js';
import { installRateLimiter } from './server/rate-limiter-setup.js';
import { createServeFeatures } from './server/serve-features.js';
import { deleteDaemonSessionIfOrphan, SessionArchiveCoordinator, } from './server/session-archive.js';
import { installSelfOriginStripMiddleware } from './server/self-origin.js';
import { createSingleWorkspaceRegistry, createWorkspaceSessionOwnerIndex, WorkspaceGenerationClosedError, } from './workspace-registry.js';
...
import { isScratchRootCompatible, } from './managed-scratch-workspace.js';
import { isPortableAbsolutePath, requireTrustedWorkspaceRuntime, resolveRegisteredWorkspaceRuntimeByPathSelector, resolveWorkspaceRuntimeFromParam, sendWorkspaceRuntimeUnavailable, } from './workspace-route-runtime.js';
...
import { registerWorkspaceGitLogRoutes, registerWorkspaceQualifiedGitLogRoutes, } from './routes/workspace-git-log.js';
import { registerWorkspaceGitBranchRoutes, registerWorkspaceQualifiedGitBranchRoutes, } from './routes/workspace-git-branches.js';
...
import { createConversationRuntimeOwnership, } from './conversations/conversation-runtime-ownership.js';
import { getStableLiveDiscoveryBaseDir } from './live/discovery.js';
import { installServeAppLifecycle, } from './serve-app-lifecycle.js';
import { LiveProviderConfigError, readLiveVoiceConfiguration, resolveLiveProviderCredential, } from './live/provider-credentials.js';
export { createDefaultFsAuditEmit, resolveBoundWorkspacesFromIdeEnv, resolveBridgeFsFactory, } from './server/fs-factory.js';
...
export { InvalidCursorError, getWorkspaceSessionInfoForResponse, invalidateWorkspaceSessionListCache, listWorkspaceSessionsForResponse, } from './server/session-list.js';
...
/**
 * Build the Express app for `LailatulCoder serve`. Pure function — no side effects on
...
function createLiveWorkspaceDelegate(getTarget) {
    return new Proxy({}, {
...
export function createServeApp(opts, getPort = () => opts.port, deps = {}) {
    ...
    const scheduledTaskRestoreTimeoutMs = sessionRestoreTimeoutMs + SCHEDULED_TASK_RESTORE_HEADROOM_MS >
    ...
    if ((deps.workspaceTrustHotReloadAvailable === true ||
        ...
        throw new Error('createServeApp: runtime replacement/removal requires deps.getSessionBridges so session-id admission can inspect draining generations.');
    }
    const app = express();
    const serveAppLifecycle = installServeAppLifecycle(app, deps.serveAppLifecycle);
    ...
    if (!injectedWorkspaceRegistry &&
        ...
        process.stderr.write('LailatulCoder serve: createServeApp default fsFactory uses trusted=false ' +
    ...
    const liveVoiceSurfaceAvailable = runtimePlatform === 'darwin' &&
        opts.serveWebShell !== false &&
        typeof deps.webShellDir === 'string' &&
        acpHttpEnabledAtBoot;
    const primaryRuntimeTrustAuthoritative = deps.workspaceTrustHotReloadAvailable === true ||
    ...
    const isPrimaryWorkspaceTrusted = () => {
        if (!primaryRuntimeTrustAuthoritative)
            return true;
        const entry = primaryTrustRegistry?.primaryEntry;
        if (entry) {
            return (entry.state === 'active' && entry.current?.runtime.trusted === true);
        }
        return deps.primaryWorkspaceTrusted ?? false;
    };
    const capturePrimaryGenerationAssertion = () => {
    ...
    const { languageCodes, currentServeFeatures, invalidateServeFeaturesCache } = createServeFeatures({
        ...
        multiWorkspaceSessionsEnabled: () => workspaceRegistry.listEntries().length > 1,
        dynamicWorkspaceRegistrationAvailable: deps.createWorkspaceRuntime !== undefined,
        ...
        scratchWorkspaceRegistrationAvailable: () => deps.createWorkspaceRuntime !== undefined &&
            deps.managedScratchRoot !== undefined &&
            deps.workspaceRuntimeRemoval !== undefined &&
            workspaceRegistry
                .listManaged()
                .filter((runtime) => !isInternalWorkspaceRuntime(runtime))
                .every((runtime) => isScratchRootCompatible(runtime.workspaceCwd, deps.managedScratchRoot.canonicalRoot)),
        realtimeVoiceEnabled: () => app.locals.liveVoiceEnabled ===
            true,
        acpHttpEnabled: acpHttpEnabledAtBoot,
        workspaceRuntimeRemovalAvailable: deps.workspaceRuntimeRemoval !== undefined,
        workspaceTrustHotReloadAvailable: deps.workspaceTrustHotReloadAvailable === true,
        isPrimaryWorkspaceTrusted: () => isPrimaryWorkspaceTrusted(),
        ...(deps.workspaceTrustHotReloadAvailable === true
            ? {
                getEnv: () => {
                    const entry = primaryTrustRegistry?.primaryEntry;
                    const runtimeEnv = entry?.state === 'active'
    ...
    const bridge = injectedWorkspaceRegistry?.primary.bridge ??
        ...
        createAcpSessionBridge({
            sessionAttachmentsRoot: path.join(new Storage(boundWorkspace).getProjectTempDir(), 'attachments'),
    ...
    const conversationRuntimeActivity = deps.liveConversationWorkspace
        ...
        : undefined;
    app.locals.sessionArchiveCoordinator = archiveCoordinator;
    if (conversationRuntimeActivity) {
        app.locals.conversationRuntimeActivity = conversationRuntimeActivity;
    }
    const cleanupSession = (runtime, sessionId) => runWithWorkspaceRuntimeStorage(runtime, () => deleteDaemonSessionIfOrphan({
        sessionId,
    ...
    const workspace = injectedWorkspaceRegistry?.primary.workspaceService ??
        ...
        createDaemonWorkspaceService({
            ...
            contextFilename: deps.contextFilename ?? 'LAILATUL.md',
            statusProvider,
            ...
            persistDisabledSkillsBatch: deps.persistDisabledSkillsBatch ??
                (async () => {
                    throw new Error('setWorkspaceSkillsEnabled requires persistDisabledSkillsBatch in ServeAppDeps');
                }),
    ...
    const requestedSessionIdAdmission = createRequestedSessionIdAdmission({
        archiveCoordinator,
        getBridges: deps.getSessionBridges ??
    ...
    const primaryRuntime = createLiveWorkspaceDelegate(() => workspaceRegistry.primary);
    const primaryRuntimeEffectiveEnv = createLiveWorkspaceDelegate(() => getRuntimeEffectiveEnv(workspaceRegistry.primary.env) ?? daemonEnv);
    ...
    const acquirePrimaryVoiceLease = () => {
        ...
        return runtime
            ...
            : { kind: 'rejected', reason: 'draining' };
    };
    const primaryBoundWorkspace = primaryRuntime.workspaceCwd;
    const primaryBridge = createLiveWorkspaceDelegate(() => workspaceRegistry.primary.bridge);
    const primaryWorkspace = createLiveWorkspaceDelegate(() => workspaceRegistry.primary.workspaceService);
    const primaryRouteFileSystemFactory = createLiveWorkspaceDelegate(() => workspaceRegistry.primary.routeFileSystemFactory);
    const loadLiveSettings = () => loadSettings(primaryBoundWorkspace, {
    ...
    const liveConfigAtBoot = liveSettingsAtBoot
        ...
        : undefined;
    let liveVoiceEnabled = liveVoiceSurfaceAvailable && liveConfigAtBoot?.enabled === true;
    app.locals.liveVoiceEnabled = liveVoiceEnabled;
    app.locals.liveVoiceSurfaceAvailable = liveVoiceSurfaceAvailable;
    const resolveLiveCredential = () => {
        ...
        try {
            ...
            throw new LiveProviderConfigError('Live provider settings could not be loaded.');
        }
        return resolveLiveProviderCredential(settings);
    };
    const liveCoordinator = deps.liveCoordinator ??
            ...
            getProviderReadiness: () => {
                try {
                    ...
                    return {
                        ...
                        message: error instanceof LiveProviderConfigError &&
    ...
    const conversationRuntimeOwnership = deps.liveConversationWorkspace
        ? (deps.conversationRuntimeOwnershipFactory?.(process.pid, liveCoordinator.daemonInstanceNonce, path.resolve(deps.liveDiscoveryStableBaseDir ?? getStableLiveDiscoveryBaseDir())) ??
            createConversationRuntimeOwnership({
                pid: process.pid,
                instanceNonce: liveCoordinator.daemonInstanceNonce,
                stableBaseDir: path.resolve(deps.liveDiscoveryStableBaseDir ?? getStableLiveDiscoveryBaseDir()),
            }))
        : undefined;
    if (conversationRuntimeOwnership) {
        serveAppLifecycle.setOwnership(conversationRuntimeOwnership);
    }
    liveCoordinator.setAppshotReadiness(liveVoiceEnabled && deps.liveConversationWorkspace
        ? {
            state: 'checking',
    ...
    const conversationRuntimeManager = deps.liveConversationWorkspace
        ? new ConversationRuntimeManager({
            ...
            publishRuntime: async (canonicalRoot, validate) => {
                const runtime = await workspaceManagementHandle.publishOwnedRuntime(canonicalRoot, 'live-conversation', async (candidate) => {
        ...
        : undefined;
    let liveBoundRuntime;
    let liveBindingPromise;
    let liveRuntimeBootPromise;
    let liveRuntimeBootResult;
    let liveAppshotChannelPromise;
    let liveCoordinatorSealed = false;
    const clearLiveRuntimeHandlers = (runtime) => {
        const handlers = [
    ...
    const bindLiveRuntimeHandlers = (runtime) => {
        ...
        try {
            setScreenHandler.call(runtime.bridge, ({ callerSessionId }) => liveCoordinator.captureScreenContext(callerSessionId));
            setTaskHandler.call(runtime.bridge, (info) => liveTaskService.handle(info));
            setSpeakHandler.call(runtime.bridge, ({ callerSessionId, message }) => liveSessionCoordinator.speakToUser(callerSessionId, message));
        }
        catch (error) {
    ...
    const ensureLiveConversationRuntime = () => {
        ...
        const pending = (async () => {
            ...
            const notifyRuntimeReady = app.locals.onConversationRuntimeReady;
            ...
            if (liveBindingPromise === pending)
                liveBindingPromise = undefined;
        });
        liveBindingPromise = pending;
        return pending;
    };
    const startConversationRuntimeBoot = () => {
        if (liveRuntimeBootPromise)
            return liveRuntimeBootPromise;
    ...
    const verifyLiveAppshotChannel = () => {
        if (liveAppshotChannelPromise)
            return liveAppshotChannelPromise;
    ...
    const liveSessionCoordinator = deps.liveSessionCoordinator ??
        new LiveSessionCoordinator({
            ...
            interruptTaskWaits: (callerSessionId) => liveTaskService.interruptWait(callerSessionId),
        });
    ...
    const publishLiveVoiceEnabled = async (enabled) => {
        const updateDiscovery = app.locals.setLiveDiscoveryEnabled;
        if (!updateDiscovery)
            return;
        await updateDiscovery(enabled).catch((error) => {
            daemonLog?.warn(`failed to update Live Host discovery: ${error instanceof Error ? error.message : String(error)}`);
        });
    };
    const setLiveVoiceEnabled = async (enabled) => {
        if (enabled && !liveVoiceSurfaceAvailable) {
            throw new Error('Live Voice is available only in WebShell on macOS.');
        }
        if (enabled === liveVoiceEnabled)
            return;
        if (enabled) {
            await ensureConversationRuntimeWithLifecycle();
            liveVoiceEnabled = true;
            app.locals.liveVoiceEnabled = true;
            liveCoordinator.setAppshotReadiness({
                state: 'checking',
                message: 'Checking the dedicated Live Appshot channel.',
            });
        ...
        app.locals.liveVoiceEnabled = false;
        invalidateServeFeaturesCache();
        try {
    ...
    app.locals.setLiveVoiceEnabled = setLiveVoiceEnabled;
    app.locals.liveCoordinator = liveCoordinator;
    app.locals.liveSessionCoordinator = liveSessionCoordinator;
    let liveCoordinatorStopped = false;
    const stopLiveCoordinator = () => {
        liveCoordinatorSealed = true;
        if (liveCoordinatorStopped)
            return;
        liveCoordinatorStopped = true;
        if (liveBoundRuntime)
            clearLiveRuntimeHandlers(liveBoundRuntime);
        liveSessionCoordinator.dispose();
        liveCoordinator.dispose();
    };
    app.locals.stopLiveCoordinator = stopLiveCoordinator;
    app.locals.sealAndWaitLiveCoordinator = async () => {
        stopLiveCoordinator();
        await Promise.all([
    ...
    // Pattern parsing happens in `run-LailatulCoder-serve.ts` for validation; here we
    ...
    const webShellDir = opts.serveWebShell !== false ? deps.webShellDir : undefined;
    ...
    const webShellFrameAncestors = opts.allowOrigins && opts.allowOrigins.length > 0
        ? [...parseAllowOriginPatterns(opts.allowOrigins).origins].filter((o) => o.startsWith('chrome-extension://') ||
    ...
    app.use(daemonTelemetryMiddleware((req) => {
        ...
        const singularCatalogMatch = req.path.match(/^\/workspace\/([^/]+)\/(?:sessions|session-info)(?:\/|$)/);
    ...
    registerDaemonStatusRoutes(app, {
        ...
        qwenCodeVersion: deps.lailatulcoderCodeVersion,
    ...
    if (liveVoiceSurfaceAvailable) {
        registerLiveRoutes(app, {
            ...
            ...(deps.persistSetting
                ? {
                    persistShortcut: async (shortcut) => {
                        const assertGenerationOpen = capturePrimaryGenerationAssertion?.() ?? (() => { });
                        await deps.persistSetting(primaryBoundWorkspace, SettingScope.User, 'experimental.liveVoice.shortcut', shortcut, assertGenerationOpen);
                    },
                }
                : {}),
        });
        const liveHostInstaller = deps.liveHostInstaller ??
            ...
            ...(deps.persistSettings
                ? {
                    persistSettings: async (writes) => {
                        const assertGenerationOpen = capturePrimaryGenerationAssertion?.() ?? (() => { });
                        await deps.persistSettings(primaryBoundWorkspace, writes, assertGenerationOpen);
                    },
                }
                : {}),
        ...
        app.locals.liveSetupController = liveSetupController;
    ...
    const workspaceManagementHandle = registerWorkspaceManagementRoutes(app, {
        ...
        validateWorkspaceRuntimeForPublication: deps.validateWorkspaceRuntimeForPublication,
    ...
    registerSessionRoutes(app, {
        ...
        isLiveSessionActive: (sessionId) => liveCoordinator.isActiveSession(sessionId),
        ...(liveConversationWorkspaceForRoutes
            ? {
                ...
                materializeLiveConversationDirectory: (sessionId) => liveConversationWorkspaceForRoutes.materializeConversationDirectory(sessionId),
            }
            : {}),
    });
    ...
    registerScheduledTasksRoutes(app, {
        ...
        getRuntime: () => workspaceRegistry.primaryEntry.state === 'active'
    ...
    if (deps.manageScheduledTaskSessions) {
        const registerScheduledTaskAuthorizations = (workspaceCwd, tasks) => {
        ...
        const rehydrateWorkspace = (runtime) => {
            void runWithWorkspaceRuntimeStorage(runtime, () => rehydrateScheduledTaskSessions({
                bridge: runtime.bridge,
                boundWorkspace: runtime.workspaceCwd,
                loadTimeoutMs: scheduledTaskRestoreTimeoutMs,
                onTasksRead: (tasks) => registerScheduledTaskAuthorizations(runtime.workspaceCwd, tasks),
                onError: (sessionId, err) => {
                    process.stderr.write(`LailatulCoder serve: failed to rehydrate scheduled-task session ${sessionId}: ${err instanceof Error ? err.message : String(err)}\n`);
                },
                ...
                process.stderr.write(`LailatulCoder serve: unexpected scheduled-task rehydration failure: ${err instanceof Error ? err.message : String(err)}\n`);
            });
        };
        ...
        // run-LailatulCoder-serve.ts can invoke it without threading it back through the
    ...
    acpHandleRef.current = mountAcpHttp(app, primaryBridge, {
        ...
        // `LailatulCoder-bearer.*` subprotocol would be a way around the scoping.
        credentials,
        ...
        ...(liveConversationWorkspaceForRoutes
            ? {
                liveSessionIsolation: {
                    materializeConversationDirectory: (sessionId) => liveConversationWorkspaceForRoutes.materializeConversationDirectory(sessionId),
                    isSessionActive: (sessionId) => liveCoordinator.isActiveSession(sessionId),
                },
            }
            : {}),
                ...
                ? [
                    {
                        ...
                        onConnection: (ws, req) => {
                            ...
                            const header = req.headers['x-LailatulCoder-live-nonce'];
                            liveCoordinator.attachHost(ws, typeof header === 'string' ? header : undefined);
                        },
                    },
                ]
                : []),
    ...
    const localControlService = new LocalControlService({
        ...
        getPort,
        tlsPaths: opts.tlsCert && opts.tlsKey
    ...
    if (!deps.serveAppLifecycle)
        serveAppLifecycle.setAppDrain(async () => {
            ...
            const bridgeDrains = await Promise.allSettled(workspaceRegistry
                .listManaged()
