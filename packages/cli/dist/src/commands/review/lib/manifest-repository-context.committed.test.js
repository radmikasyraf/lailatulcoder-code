...
const relatedPathSentinels = {
    'packages/core/src/config/**': 'packages/core/src/config/config.ts',
    'packages/web-shell/client/adapters/**': 'packages/web-shell/client/adapters/types.ts',
    'packages/web-shell/client/completions/**': 'packages/web-shell/client/completions/slashCompletion.ts',
    'packages/web-shell/client/hooks/**': 'packages/web-shell/client/hooks/useMessages.ts',
};
...
function provideForRepo(changedPaths) {
...
describe('committed review context manifest', () => {
    it('matches the pinned repository review policy', () => {
        ...
        expect(JSON.parse(content)).toEqual(expectedManifest);
    });
    ...
    it('resolves a distinct sentinel for every relatedPaths pattern', () => {
        const patterns = expectedManifest.rules.flatMap((rule) => 'relatedPaths' in rule ? [...rule.relatedPaths] : []);
        expect(Object.keys(relatedPathSentinels).sort()).toEqual([...patterns].sort());
        for (const rule of expectedManifest.rules) {
            if (!('relatedPaths' in rule))
                continue;
    ...
    it('stays under the resolved-file bound when every rule co-matches', () => {
        const probes = expectedManifest.rules.flatMap((rule) => rule.paths.map(probeFor));
        const context = provideForRepo(probes);
        expect(context).not.toBeNull();
        expect(context?.relatedPaths.length).toBeLessThanOrEqual(MAX_ARRAY_ITEMS);
    });
    it.skipIf(!inGitWorktree())('keeps the manifest un-ignored and tracked despite the .qwen/* ignore', () => {
        expect(exitsZero(['check-ignore', '--no-index', '--', MANIFEST_RELATIVE_PATH]), 'manifest must not be ignored by git').toBe(false);
        expect(exitsZero([
            'ls-files',
            '--error-unmatch',
            '--',
            MANIFEST_RELATIVE_PATH,
        ]), 'manifest must be tracked by git').toBe(true);
    });
});
//# sourceMappingURL=manifest-repository-context.committed.test.js.map