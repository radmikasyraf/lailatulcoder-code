...
import { CdpBrowserEmulator } from './cdp-browser-emulator.js';
...
export function attachCdpClient(ws, registry, log) {
    ...
    if (!bridge) {
        log('LailatulCoder serve: /cdp rejected — no extension bridge connected');
        try {
    ...
    if (bridge.cdpBound) {
        log('LailatulCoder serve: /cdp rejected — a puppeteer client is already bound');
        try {
    ...
    const link = new CdpReverseLink((frame) => bridge.send(frame), undefined, log);
    ...
    bridge.routeInbound = (frame) => link.handleInbound(frame);
    ...
    link.onDetach = (reason) => {
        log(`LailatulCoder serve: /cdp tab detached (${reason}); closing puppeteer socket`);
        try {
    ...
    const heartbeat = setInterval(() => {
        if (disposed)
            return;
        if (!heartbeatAlive) {
            log('LailatulCoder serve: /cdp heartbeat missed; closing puppeteer socket');
        ...
        try {
            ...
            log(`LailatulCoder serve: /cdp heartbeat ping failed: ${err instanceof Error ? err.message : String(err)}`);
    ...
    heartbeat.unref?.();
    const dispose = (reason, notifyExtension = true) => {
        if (disposed)
            return;
        disposed = true;
        ...
        if (notifyExtension && registry.getActive() === bridge) {
            try {
                ...
                log(`LailatulCoder serve: /cdp sent release to extension (puppeteer disconnected: ${reason})`);
            }
            catch (err) {
                log(`LailatulCoder serve: /cdp release send failed: ${err instanceof Error ? err.message : String(err)}`);
            }
        }
        link.dispose(reason);
    ...
    link.onAttachFailure = (reason) => {
        log(`LailatulCoder serve: /cdp attach failed (${reason}); closing puppeteer socket`);
    ...
    bridge.onExtensionGone = () => {
        log('LailatulCoder serve: extension /acp dropped; closing puppeteer /cdp socket');
    ...
    ws.on('message', (data) => {
        ...
        void emulator.handleFromClient(frame).catch((err) => {
            log(`LailatulCoder serve: /cdp emulator error: ${err instanceof Error ? err.message : String(err)}`);
        });
    });
    ws.on('close', () => dispose('puppeteer /cdp socket closed'));
    ws.on('error', (err) => {
        log(`LailatulCoder serve: /cdp WS error: ${err instanceof Error ? err.message : String(err)}`);
    ...
    log('LailatulCoder serve: /cdp puppeteer client bound to extension bridge');
}
