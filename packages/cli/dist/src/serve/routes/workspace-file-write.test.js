...
async function makeHarness(opts) {
    const scratch = await fsp.mkdtemp(path.join(os.tmpdir(), `qwen-write-routes-${randomBytes(4).toString('hex')}-`));
    ...
    const app = createServeApp({ ...baseOpts, workspace, token: opts?.token }, undefined, { fsFactory });
 ...
 */
async function sendChunkedUpload(app, targetPath, totalBytes) {
        ...
        return await new Promise((resolvePromise) => {
            ...
            const writeNext = () => {
                while (remaining > 0) {
                    ...
                    remaining -= size;
                    const ok = req.write(size === chunk.length ? chunk : chunk.subarray(0, size));
                    if (!ok) {
...
describe('POST /file/write', () => {
    ...
    it('creates a text file with no-store headers', async () => {
        ...
        expect(await fsp.readFile(path.join(h.workspace, 'a.txt'), 'utf-8')).toBe('hello\n');
    });
    ...
    it('does not overwrite existing files in create mode', async () => {
        ...
        expect(await fsp.readFile(path.join(h.workspace, 'a.txt'), 'utf-8')).toBe('old');
    });
...
describe('POST /file/upload', () => {
    ...
    afterEach(async () => teardown(h));
    const upload = (pathParam) => request(h.app)
        .post('/file/upload')
        .set('Host', loopbackHost())
        .set('Authorization', 'Bearer secret')
    ...
    it('writes bytes atomically and returns the confirmed path, size, hash', async () => {
        ...
        expect(await fsp.readFile(path.join(h.workspace, 'blob.bin'))).toEqual(data);
    });
    it('accepts a zero-byte octet-stream upload', async () => {
        ...
        await expect(fsp.readFile(path.join(h.workspace, 'empty.bin'))).resolves.toEqual(Buffer.alloc(0));
    });
    ...
    it('rejects an oversized declared Content-Length with the upload 413 envelope', async () => {
        ...
        await expect(fsp.stat(path.join(h.workspace, 'big.bin'))).rejects.toMatchObject({ code: 'ENOENT' });
    });
    it('numbers dotfiles as whole names (.env -> .env (1))', async () => {
        ...
        expect(await fsp.readFile(path.join(h.workspace, '.env'), 'utf-8')).toBe('orig');
    });
    it('auto-numbers when the requested name is occupied by a file', async () => {
        ...
        expect(await fsp.readFile(path.join(h.workspace, 'report.pdf'), 'utf-8')).toBe('orig');
        expect(await fsp.readFile(path.join(h.workspace, 'report (1).pdf'), 'utf-8')).toBe('new');
    });
    ...
    it('lands concurrent same-name uploads on distinct candidates', async () => {
        ...
        expect(new Set([first.body.path, second.body.path])).toEqual(new Set(['race.bin', 'race (1).bin']));
        // Each response path holds exactly one of the two bodies — a
        // misrouted write (same bytes twice, or an empty file) fails here.
        const contents = new Set(await Promise.all([first.body.path, second.body.path].map((p) => fsp.readFile(path.join(h.workspace, p), 'utf-8'))));
    ...
    it('numbers instead of writing through an in-workspace symlink', async () => {
        ...
        await fsp.symlink(path.join(h.workspace, 'real.bin'), path.join(h.workspace, 'link.bin'));
        ...
        expect(await fsp.readFile(path.join(h.workspace, 'real.bin'), 'utf-8')).toBe('orig');
    });
    it('numbers instead of materializing a symlink whose target is absent', async () => {
        await fsp.symlink(path.join(h.workspace, 'fresh.bin'), path.join(h.workspace, 'link.bin'));
        const res = await upload('link.bin').send(Buffer.from('new'));
        expect(res.status).toBe(201);
        expect(res.body.path).toBe('link (1).bin');
        // The dangling symlink is untouched and its target was not created.
        expect(await fsp.readlink(path.join(h.workspace, 'link.bin'))).toBe(path.join(h.workspace, 'fresh.bin'));
        await expect(fsp.stat(path.join(h.workspace, 'fresh.bin'))).rejects.toMatchObject({ code: 'ENOENT' });
    });
    ...
    it('creates a missing parent directory and uploads into it', async () => {
        ...
        expect(await fsp.readFile(path.join(h.workspace, 'no/such/dir/a.txt'), 'utf8')).toBe('x');
    });
    ...
    it('rejects a non-directory parent before buffering', async () => {
        ...
        expect(res.status).toBe(400);
        expect(res.body.errorKind).toBe('parse_error');
        expect(await fsp.readFile(path.join(h.workspace, 'file.txt'), 'utf-8')).toBe('x');
    });
    it('rejects a ../ boundary escape before buffering', async () => {
        ...
        await expect(fsp.stat(path.join(h.scratch, 'escape.txt'))).rejects.toMatchObject({ code: 'ENOENT' });
    });
    it.each(['report.', 'report ', 'CON.txt'])('rejects the suspicious basename %s before buffering', async (name) => {
        ...
        await expect(fsp.stat(path.join(h.workspace, name))).rejects.toMatchObject({ code: 'ENOENT' });
    });
    it('rejects a symlinked parent that escapes the workspace', async () => {
        ...
        expect(res.status).toBe(400);
        expect(res.body.errorKind).toBe('symlink_escape');
        await expect(fsp.stat(path.join(outsideDir, 'a.txt'))).rejects.toMatchObject({ code: 'ENOENT' });
    });
    it('preserves a generation-closed error from the parent stat', async () => {
        ...
        const app = createServeApp({ ...baseOpts, workspace: h.workspace, token: 'secret' }, undefined, { fsFactory: statFactory });
    ...
    it('uploads into an existing subdirectory', async () => {
        ...
        expect(await fsp.readFile(path.join(h.workspace, 'sub', 'file.txt'), 'utf-8')).toBe('hi');
    });
    it('uploads into a workspace whose root path trips the suspicious-pattern check', async () => {
        ...
        expect(await fsp.readFile(path.join(h.workspace, 'report.txt'), 'utf-8')).toBe('hi');
    ...
    it.each(['assets/', 'sub/dir/'])('rejects the directory-shaped trailing-slash path %s before buffering', async (pathParam) => {
    ...
    it('rejects a trailing-slash path even when a same-named directory exists', async () => {
        ...
        await expect(fsp.stat(path.join(h.workspace, 'assets (1)'))).rejects.toMatchObject({ code: 'ENOENT' });
    });
    it.each(['notes\u0000.txt', 'foo\u0000/x.txt'])('rejects the NUL-bearing path with parse_error and no denied event', async (pathParam) => {
        const deniedBefore = h.events.filter((e) => e.type === 'fs.denied').length;
        const res = await upload(pathParam).send(Buffer.from('x'));
        expect(res.status).toBe(400);
        expect(res.body.errorKind).toBe('parse_error');
        expect(h.events.filter((e) => e.type === 'fs.denied')).toHaveLength(deniedBefore);
    });
    it('rejects an encoded request body instead of silently decoding it', async () => {
        ...
        await expect(fsp.stat(path.join(h.workspace, 'gz.bin'))).rejects.toMatchObject({ code: 'ENOENT' });
    });
    it('numbers past a symlink cycle occupying the requested name', async () => {
        await fsp.symlink(path.join(h.workspace, 'loop.txt'), path.join(h.workspace, 'loop.txt'));
        const res = await upload('loop.txt').send(Buffer.from('x'));
        expect(res.status).toBe(201);
        expect(res.body.path).toBe('loop (1).txt');
        expect(await fsp.readFile(path.join(h.workspace, 'loop (1).txt'), 'utf-8')).toBe('x');
    });
    it('numbers past a two-hop symlink cycle occupying the requested name', async () => {
        await fsp.symlink(path.join(h.workspace, 'b.txt'), path.join(h.workspace, 'a.txt'));
        await fsp.symlink(path.join(h.workspace, 'a.txt'), path.join(h.workspace, 'b.txt'));
    ...
    it('uploads through a symlinked directory whose target trips the pattern check', async () => {
        ...
        await fsp.symlink(path.join(h.workspace, 'aux'), path.join(h.workspace, 'docs'), 'dir');
        const res = await upload('docs/report.txt').send(Buffer.from('hi'));
        expect(res.status).toBe(201);
        expect(await fsp.readFile(path.join(h.workspace, 'aux', 'report.txt'), 'utf-8')).toBe('hi');
    });
    ...
    it('accepts an upload above the 5 MiB text cap (binary policy at HTTP)', async () => {
        ...
        expect((await fsp.stat(path.join(h.workspace, 'big.bin'))).size).toBe(data.length);
    });
    it('accepts an upload of exactly MAX_UPLOAD_BYTES', async () => {
        ...
        expect(res.status).toBe(201);
        expect(res.body.sizeBytes).toBe(data.length);
        expect((await fsp.stat(path.join(h.workspace, 'exact.bin'))).size).toBe(data.length);
    }, 30_000);
    it('trims a long stem on numbering to stay within the 255-byte cap', async () => {
        ...
        expect(await fsp.readFile(path.join(h.workspace, base), 'utf-8')).toBe('new');
        // The original is untouched.
        expect(await fsp.readFile(path.join(h.workspace, name), 'utf-8')).toBe('orig');
    });
    ...
    it('lands on the 1000th candidate, then 409s when all are occupied', async () => {
        ...
        await Promise.all(Array.from({ length: 999 }, (_, i) => fsp.writeFile(path.join(h.workspace, i === 0 ? 'a.txt' : `a (${i}).txt`), 'x')));
    ...
    it('publishes no file when the client disconnects during admission', async () => {
        ...
        const app = createServeApp({ ...baseOpts, workspace: h.workspace, token: 'secret' }, undefined, { fsFactory: hangingFactory });
        const server = app.listen(0, '127.0.0.1');
        await new Promise((resolve) => server.once('listening', resolve));
        const port = server.address().port;
        try {
            ...
            const followUp = (name) => request(app)
                .post('/file/upload')
                .set('Host', loopbackHost())
                .set('Authorization', 'Bearer secret')
                .set('Content-Type', 'application/octet-stream')
            ...
            await expect(fsp.stat(path.join(h.workspace, 'photo.jpg'))).rejects.toMatchObject({ code: 'ENOENT' });
        }
        finally {
            server.closeAllConnections?.();
            await new Promise((resolve) => server.close(() => resolve()));
    ...
    it('frees gate slots when oversized chunked bodies are rejected after admission', async () => {
        ...
        for (let i = 0; i < 5; i++) {
            const res = await sendChunkedUpload(h.app, `big-${i}.bin`, 50 * 1024 * 1024 + 1);
            expect(res.status).toBe(413);
...
describe('upload concurrency gate', () => {
    it('admits up to the cap and rejects the next until a slot frees', async () => {
        const { createUploadConcurrencyGate } = await import('./workspace-file-write.js');
...
describe('POST /file/upload HTTP concurrency gate (end-to-end)', () => {
    it('holds a slot through a disconnected write, then frees it on completion', async () => {
        const scratch = await fsp.mkdtemp(path.join(os.tmpdir(), `qwen-upload-gate-${randomBytes(4).toString('hex')}-`));
        ...
        const app = createServeApp({ ...baseOpts, workspace, token: 'secret' }, undefined, { fsFactory: hangingFactory });
        const upload = (name) => request(app)
            .post('/file/upload')
            .set('Host', loopbackHost())
            .set('Authorization', 'Bearer secret')
            .set('Content-Type', 'application/octet-stream')
        ...
        try {
            ...
            const firstSettled = inFlight[0].then(() => undefined, () => undefined);
            inFlight[0].abort();
            await firstSettled;
            ...
            const chunked = await sendChunkedUpload(app, 'chunked.bin', 50 * 1024 * 1024 + 1);
...
//# sourceMappingURL=workspace-file-write.test.js.map