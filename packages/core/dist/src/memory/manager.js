...
import { runSkillReviewByAgent, listExistingSkillDirNames, } from './skillReviewAgentPlanner.js';
import { stageSkillDirs, acceptPendingSkill, rejectPendingSkill, } from './pending-skills.js';
...
export class MemoryManager {
    ...
    async runSkillReview(record, params) {
        ...
        try {
            ...
            if (params.confirmBeforePersist && result.touchedSkillFiles.length > 0) {
                const pending = await stageSkillDirs(result.touchedSkillFiles, params.projectRoot, preExistingSkillDirs, record.id);
                this.update(record, {
                    status: 'completed',
                    progressText: pending.length > 0
    ...
    async acceptPendingSkillFromTask(taskId, skillName) {
    ...
    async rejectPendingSkillFromTask(taskId, skillName) {
        await this.resolvePendingSkill(taskId, skillName, 'reject');
    }
    async resolvePendingSkill(taskId, skillName, action) {
        const record = this.tasks.get(taskId);
        if (!record) {
            debugLogger.warn(`Cannot resolve pending skill: no task ${taskId}.`);
            return;
        }
        const target = (record.metadata?.['pendingSkills'] ?? []).find((p) => p.name === skillName);
        if (!target) {
            debugLogger.warn(`Cannot resolve pending skill "${skillName}": not pending on task ${taskId}.`);
            return;
        }
        try {
            if (action === 'accept') {
            ...
            debugLogger.warn(`Failed to ${action} pending skill "${skillName}": ${error instanceof Error ? error.message : String(error)}`);
            throw error;
        }
        ...
        const remaining = (record.metadata?.['pendingSkills'] ?? []).filter((p) => p.name !== skillName);
    ...
    forgetMatches(projectRoot, matches, now, options = {}) {
     ...
     */
    buildAutoMemoryPrompt(memoryDir, indexContent, userSection, teamSection, options) {
        return buildManagedAutoMemoryPrompt(memoryDir, indexContent, userSection, teamSection, options);
    }
    // ─── Dream utilities ──────────────────────────────────────────────────────────
    /**
