...
function normalizeAllowances(entries) {
    return new Map(entries.map(([file, allowance]) => [path.normalize(file), allowance]));
}
const allowedProcessEnvAccesses = normalizeAllowances([
    ...
    [
        ...
        {
            reason: 'Standalone channel spawning keeps a process-environment compatibility fallback.',
            accesses: { whole: 1 },
        },
    ],
    [
        ...
        {
            reason: 'Whether the daemon runs inside a container sandbox is process-scoped: ' +
    ...
    [
        ...
        {
            reason: 'Embedded callers may omit the daemon-level environment argument.',
            accesses: { whole: 1 },
        },
    ],
    [
        ...
        {
            reason: 'Embedded ACP mounts may omit the daemon-level environment argument.',
            accesses: { whole: 1 },
        },
    ],
    [
        ...
        {
            reason: 'The process-global channel supervisor needs a base environment for child workers.',
            accesses: { whole: 1 },
        },
    ],
    [
    ...
    [
        ...
        {
            reason: 'The daemon snapshots its process environment before applying workspace overlays.',
            accesses: { whole: 1 },
        },
    ],
    [
        ...
        {
            reason: 'Fast-path settings initialize daemon bootstrap compatibility state.',
            accesses: {
                'computed:key': 4,
                'key:CLOUD_SHELL': 1,
    ...
    [
        ...
        {
            reason: 'The fast-path entry point initializes process-level daemon defaults.',
            accesses: { whole: 2 },
        },
    ],
    [
    ...
    [
        ...
        {
            reason: 'The serve entry point owns daemon bootstrap, feature flags, child-process defaults, and the launch-env loader scrub.',
            accesses: {
    ...
    [
        ...
        {
            reason: 'Embedded server construction keeps a process-environment compatibility fallback, ' +
    ...
    [
        ...
        {
            reason: 'Embedded feature detection defaults to the daemon process environment.',
            accesses: { whole: 1 },
        },
    ],
    [
    ...
    [
        ...
        {
            reason: 'Live audio diagnostics are enabled and located for the whole daemon process.',
            accesses: {
...
function findProcessEnvAccesses(file, source) {
    ...
    const visit = (node) => {
        const isProcessEnv = (ts.isPropertyAccessExpression(node) &&
        ...
        if (isProcessEnv) {
            ...
            if (ts.isElementAccessExpression(parent) && parent.expression === node) {
                ...
                record(ts.isStringLiteralLike(argument)
                    ? `key:${argument.text}`
    ...
    visit(sourceFile);
    return Object.fromEntries([...accesses].sort(([left], [right]) => left.localeCompare(right)));
}
describe('serve process.env guard', () => {
    it('detects dot and element process.env access expressions', () => {
        expect(findProcessEnvAccesses('example.ts', `
          process.env.DOT;
          process['env']['ELEMENT'];
          process['env'].MIXED;
    ...
    it('detects computed process.env access expressions', () => {
        expect(findProcessEnvAccesses('example.ts', ['process.env[DYNAMIC_VAR];', 'process.env[`TEMPLATE_${key}`];'].join('\n'))).toEqual({
    ...
    it('allows only documented process-scoped process.env expressions', () => {
        ...
        for (const root of scannedRoots) {
            for (const file of listTypeScriptFiles(root)) {
                if (/\.(?:test|spec)\.tsx?$/.test(file))
                    continue;
                const accesses = findProcessEnvAccesses(file, fs.readFileSync(file, 'utf8'));
        ...
        const mismatches = [
            ...
            const sorted = (accesses) => Object.entries(accesses).sort(([left], [right]) => left.localeCompare(right));
        ...
        expect(mismatches, 'process.env access mismatches — update allowedProcessEnvAccesses with a reason and the new access pattern').toEqual([]);
    });
});
//# sourceMappingURL=process-env-guard.test.js.map