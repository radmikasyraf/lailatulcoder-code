...
import { DEFAULT_DASHSCOPE_BASE_URL, resolveRequestTimeout, } from '../core/openaiContentGenerator/constants.js';
import { DASHSCOPE_REGIONAL_HOSTS } from '../core/openaiContentGenerator/provider/dashscope.js';
import { buildRuntimeFetchOptions, preloadRuntimeFetchModule, } from '../utils/runtimeFetchOptions.js';
 ...
 */
export function evaluateWebSearchGate(config) {
    ...
    const models = config.getAllConfiguredModels(resolved.authType ? [resolved.authType] : undefined);
    ...
    const resolvedEntry = config.getResolvedModelConfig(entry.authType, entry.id, entry.registryBaseUrl);
    return {
        ok: true,
        backend: {
            modelId: entry.id,
 ...
 */
const SIDE_REQUEST_INSTRUCTIONS = 'You are a web search agent. Run web searches and, when helpful, open result pages to verify facts. ' +
    ...
    for (const item of items) {
        switch (item.type) {
            ...
            case 'web_extractor_call': {
                ...
                if (item.output) {
                    extractedParts.push((item.goal ? `[Extracted content — goal: ${item.goal}]\n` : '') +
                        item.output);
                }
                break;
            }
...
function formatLlmContent(query, data, partialNote) {
    ...
    const buildBody = (answerText) => {
        ...
        if (opened.length > 0) {
            sections.push('Opened evidence pages (read in full by the search agent):\n' +
        ...
        if (candidates.length > 0) {
            sections.push('Additional search candidates (returned by search, not opened — weaker evidence):\n' +
...
class WebSearchToolInvocation extends BaseToolInvocation {
    ...
    async execute(signal, updateOutput) {
        ...
        if (!gate.ok) {
            return this.errorResult(gate.notice, ToolErrorType.WEB_SEARCH_BACKEND_FAILED);
        }
        ...
        const timedOutResult = () => this.errorResult(`Web search timed out after ${SEARCH_TIMEOUT_MS / 1000}s.`, ToolErrorType.WEB_SEARCH_BACKEND_FAILED);
        const cancelledResult = () => this.errorResult('Web search cancelled.', ToolErrorType.WEB_SEARCH_BACKEND_FAILED);
        ...
        for (let attempt = 1; attempt <= maxAttempts; attempt++) {
            ...
            const terminalFailure = (fallback) => {
                ...
                if (partialItems.length > 0 || partialText.length > 0) {
                    const partial = this.partialResult(partialItems, partialText, startedAt);
                    if (partial)
                        return partial;
                }
            ...
            try {
                ...
                for await (const event of stream) {
                    ...
                    if (streamedChars > MAX_STREAM_CHARS) {
                        this.debugLogger.warn(`[WebSearch] stream exceeded ${MAX_STREAM_CHARS} chars; aborting`);
                        capController.abort();
                        break;
                    }
                }
            ...
            if (streamError !== undefined) {
                ...
                if (typeof status === 'number') {
                    ...
                    this.debugLogger.error(`[WebSearch] ${message}`);
                    return this.errorResult(message, status === 429
                        ? ToolErrorType.WEB_SEARCH_RATE_LIMITED
                        : ToolErrorType.WEB_SEARCH_BACKEND_FAILED);
                }
                return terminalFailure(() => {
                    ...
                    this.debugLogger.error(`[WebSearch] ${message}`);
                    return this.errorResult(message, ToolErrorType.WEB_SEARCH_BACKEND_FAILED);
                });
            }
            if (!finalResponse) {
                // Stream ended (or was capped) without a terminal event.
                return terminalFailure(() => this.errorResult('Web search stream ended without a response.', ToolErrorType.WEB_SEARCH_BACKEND_FAILED));
            }
            ...
            if (status === 'failed') {
                return terminalFailure(() => this.errorResult('Web search backend reported the request as failed.', ToolErrorType.WEB_SEARCH_BACKEND_FAILED));
            }
            if (status === 'cancelled') {
                return terminalFailure(() => this.errorResult('Web search was cancelled by the backend.', ToolErrorType.WEB_SEARCH_BACKEND_FAILED));
            }
            ...
            if (data.searchCallCount === 0) {
                ...
                if (attempt < maxAttempts) {
                    const backoffMs = NO_SEARCH_RETRY_BASE_DELAY_MS +
                        Math.random() * NO_SEARCH_RETRY_JITTER_MS;
                    this.debugLogger.warn(`[WebSearch] no web_search_call in response; retrying in ${Math.round(backoffMs)}ms`);
                    try {
                ...
                return this.errorResult('The search backend did not perform a web search (this can indicate server-side throttling). Try again later.', ToolErrorType.WEB_SEARCH_NO_SEARCH_PERFORMED);
            }
            if (status === 'incomplete' &&
                (data.candidateUrls.length > 0 ||
                    data.openedUrls.length > 0 ||
                    data.answerText.trim())) {
                return this.finishResult(data, startedAt, '[Partial result: the backend reported this response as incomplete — treat it as potentially missing information.]');
            }
            if (data.candidateUrls.length === 0 &&
                data.openedUrls.length === 0 &&
                !data.answerText.trim()) {
                return this.errorResult(`No search results returned for: "${this.params.query}"`, ToolErrorType.WEB_SEARCH_NO_RESULTS);
            }
            return this.finishResult(data, startedAt, undefined);
        }
        // Unreachable: the loop always returns.
        return this.errorResult('Web search failed unexpectedly.', ToolErrorType.WEB_SEARCH_BACKEND_FAILED);
    }
    finishResult(data, startedAt, partialNote) {
        ...
        const returnDisplay = `Did ${searchCount} search${searchCount === 1 ? '' : 'es'} in ${seconds}s` +
    ...
    partialResult(items, partialText, startedAt) {
        ...
        if (data.searchCallCount === 0)
            return null;
        return this.finishResult(data, startedAt, '[Partial result: the search stream ended before completion — treat it as potentially missing information.]');
    }
}
...
export class WebSearchTool extends BaseDeclarativeTool {
    ...
    constructor(config) {
        super(WebSearchTool.Name, ToolDisplayNames.WEB_SEARCH, getWebSearchToolDescription(), Kind.Search, {
            properties: {
                query: {
