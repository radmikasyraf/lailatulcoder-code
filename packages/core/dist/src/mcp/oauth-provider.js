...
import { MCP_OAUTH_CLIENT_NAME, OAUTH_REDIRECT_PORT, OAUTH_REDIRECT_PATH, } from './constants.js';
export const OAUTH_DISPLAY_MESSAGE_EVENT = 'oauth-display-message';
export const OAUTH_AUTH_URL_EVENT = 'oauth-auth-url';
...
let activeCallbackTimeout = null;
...
export class MCPOAuthProvider {
        ...
        return new Promise((resolve, reject) => {
            ...
            <html>
              <body>
                ...
                <p>You can close this window and return to LailatulCoder Ai.</p>
          ...
          `);
            ...
            server.listen({ port: OAUTH_REDIRECT_PORT, host: '127.0.0.1' }, () => {
                debugLogger.debug(`OAuth callback server listening on port ${OAUTH_REDIRECT_PORT}`);
            });
    ...
    async exchangeCodeForToken(config, code, codeVerifier, mcpServerUrl) {
        ...
        try {
            return normalizeTokenResponse(JSON.parse(responseText));
        }
        catch (error) {
        ...
        const displayMessage = (message) => {
            if (events) {
                ...
                if (typeof message === 'string') {
                    ...
                    debugLogger.info(`[${message.key}]${message.params ? ` ${JSON.stringify(message.params)}` : ''}`);
                }
            }
        };
...
✓ No authorization URL; using OAuth discovery`);
        ...
        if (!config.clientId) {
            ...
            if (registrationUrl) {
                const clientRegistration = await this.registerClient(registrationUrl, config);
        ...
        try {
            ...
            if (savedToken && savedToken.token && savedToken.token.accessToken) {
                ...
                debugLogger.error('Token verification failed: token not found or invalid after save');
            }
        }
        catch (saveError) {
