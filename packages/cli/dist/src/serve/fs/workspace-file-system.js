...
import { CursorNotAtLineBoundaryError, LargeNonUtf8TextError, StandardFileSystemService, TextScanBudgetExceededError, decodeBufferWithEncodingInfoAsync, detectLineEnding, encodeTextFileContentAsync, isUtf8CompatibleEncoding, loadIgnoreRules, isWithinRoot, } from '@lailatul-coder/lailatul-coder-core';
import { createAuditPublisher, } from './audit.js';
import { FsError, isFsError, wrapAsFsError, } from './errors.js';
import { assertCursorMatchesFile, decodeTextCursor, encodeTextCursor, } from './text-cursor.js';
import { canonicalizeWorkspaces, hasSuspiciousPathPattern, resolveWithinWorkspace, } from './paths.js';
import { BINARY_PROBE_BYTES, MAX_READ_BYTES, MAX_TEXT_SCAN_BYTES, MAX_UPLOAD_BYTES, assertTrustedForIntent, enforceReadSize, enforceWriteSize, shouldIgnore, } from './policy.js';
 ...
 */
export function resolveNewFileModeBits(policy, umask) {
 ...
 */
export function createWorkspaceFileSystemFactory(deps) {
    ...
    const newFileMode = deps.newFileMode ?? 'owner';
    const forRequest = (ctx) => new WorkspaceFileSystemImpl({
        primaryWorkspace,
        workspaces,
    ...
    return {
        ...
        async writeSameHostToolText(ctx, request) {
            ...
            let resolved;
            try {
                resolved = await resolveWithinWorkspace(request.path, workspaces.map((workspace) => workspace.path), 'write');
                deps.generationGuard?.assertOpen();
            }
            catch (err) {
                if (!(err instanceof FsError && err.kind === 'path_outside_workspace')) {
                    throw recordSameHostToolWriteDenied(audit, ctx, request.path, err);
                }
                try {
                    ...
                    throw recordSameHostToolWriteDenied(audit, ctx, request.path, outsideErr);
                }
            }
            try {
...
async function writeSameHostToolTextOutsideWorkspace(deps) {
    ...
    await deps.pathLocks.runExclusive(target, async () => {
        ...
        const content = meta.bom && deps.request.content.charCodeAt(0) === 0xfeff
...
async function resolveSameHostToolWriteTarget(input) {
    if (!path.isAbsolute(input)) {
        throw new FsError('path_outside_workspace', `same-host external tool write requires an absolute path: ${input}`);
    }
    if (hasSuspiciousPathPattern(input)) {
        throw new FsError('path_outside_workspace', `path contains suspicious pattern: ${input}`);
    }
    let leaf;
    try {
        ...
        if (!parentStat.isDirectory()) {
            throw new FsError('parse_error', `parent path is not a directory: ${parent}`);
        }
        return path.join(parent, path.basename(input));
    }
    if (leaf.isSymbolicLink()) {
        throw new FsError('symlink_escape', `path is a symlink and cannot be overwritten: ${input}`, { hint: 'resolve the target explicitly before writing' });
    }
    if (!leaf.isFile()) {
    ...
    if (!canonicalStat.isFile()) {
        throw new FsError('parse_error', `canonical path is not a regular file: ${canonical}`);
    }
    assertSameFile(leaf, canonicalStat, input, 'write');
    return canonical;
}
function isWorkspaceGenerationClosedError(error) {
    return (error instanceof Error &&
        'code' in error &&
        error.code === 'workspace_generation_closed');
}
function recordSameHostToolWriteDenied(audit, ctx, input, error) {
...
function assertNoNestedWorkspaces(workspaces) {
    for (let i = 0; i < workspaces.length; i++) {
        ...
        for (let j = i + 1; j < workspaces.length; j++) {
            ...
            if (isWithinRoot(a, b) || isWithinRoot(b, a)) {
                throw new Error(`Nested workspace roots are not supported: ${JSON.stringify(a)} and ${JSON.stringify(b)}`);
            }
        }
    }
}
class WorkspaceFileSystemImpl {
    deps;
    constructor(deps) {
        this.deps = deps;
    }
    workspaceForPath(p) {
        ...
        for (const workspace of this.deps.workspaces) {
            if (isWithinRoot(p, workspace.path) &&
    ...
    ignoreVerdict(p, kind = 'file') {
        const workspace = this.workspaceForPath(p);
        if (!workspace)
    ...
    async resolve(input, intent) {
        try {
            ...
            const resolved = await resolveWithinWorkspace(input, this.deps.workspaces.map((workspace) => workspace.path), intent);
            this.deps.generationGuard?.assertOpen();
            return resolved;
        }
        catch (err) {
    ...
    async stat(p) {
        ...
        try {
            ...
            throw this.recordAndWrap(err, 'stat', p);
        }
    }
    async readText(p, opts = {}) {
        ...
        try {
            ...
            if (opts.line !== undefined &&
                ...
                throw new FsError('parse_error', `line must be a positive integer, got ${opts.line}`);
            }
            if (opts.limit !== undefined &&
                (!Number.isSafeInteger(opts.limit) || opts.limit < 1)) {
                throw new FsError('parse_error', `limit must be a positive integer, got ${opts.limit}`);
            }
            ...
            if (opts.cursor !== undefined && opts.line !== undefined) {
                throw new FsError('parse_error', 'cursor and line are mutually exclusive; a cursor already encodes where to resume');
            }
            if (opts.maxBytes !== undefined &&
                (!Number.isSafeInteger(opts.maxBytes) ||
                    opts.maxBytes < 1 ||
                    opts.maxBytes > MAX_READ_BYTES)) {
                throw new FsError('parse_error', `maxBytes must be a positive integer in [1, ${MAX_READ_BYTES}], got ${opts.maxBytes}`);
            }
            const snapshot = await readTextFromResolvedFile(p, opts, this.deps.lowFs);
            this.deps.generationGuard?.assertOpen();
            const ignoreVerdict = this.ignoreVerdict(p, 'file');
            const meta = snapshot.meta;
            if (ignoreVerdict.ignored)
                meta.matchedIgnore = ignoreVerdict.category;
            this.deps.audit.recordAccess(this.deps.ctx, {
                intent: 'read',
                absolute: p,
                durationMs: performance.now() - start,
            ...
            throw this.recordAndWrap(err, 'read', p);
        }
    }
    ...
    async readBytesWindow(p, opts = {}) {
        ...
        try {
            ...
            if (!Number.isSafeInteger(offset) || offset < 0) {
                throw new FsError('parse_error', `offset must be a non-negative integer, got ${offset}`);
            }
            if (!Number.isSafeInteger(maxBytes) ||
                maxBytes < 1 ||
                maxBytes > MAX_READ_BYTES) {
                throw new FsError('parse_error', `maxBytes must be a positive integer in [1, ${MAX_READ_BYTES}], got ${maxBytes}`);
            }
            ...
            try {
                ...
                assertSameFile(st, afterRead, p, 'read');
            ...
            await assertInodeStableAfterRead(p, st.ino);
            this.deps.generationGuard?.assertOpen();
    ...
    async list(p, opts = {}) {
        ...
        try {
            ...
            if (opts.maxEntries !== undefined &&
                ...
                throw new FsError('parse_error', `maxEntries must be a positive integer, got ${opts.maxEntries}`);
            }
            const entries = [];
            ...
            for await (const d of dir) {
                ...
                const verdict = this.ignoreVerdict(childAbs, kind === 'directory' ? 'directory' : 'file');
                if (verdict.ignored && !opts.includeIgnored)
                    continue;
            ...
            this.deps.audit.recordAccess(this.deps.ctx, {
                ...
                truncated: opts.maxEntries !== undefined && entries.length >= opts.maxEntries,
            });
            return entries;
        }
        catch (err) {
            throw this.recordAndWrap(err, 'list', p);
        }
    }
    async glob(pattern, opts = {}) {
        ...
        try {
            ...
            if (pattern.split(/[\\/]/).some((seg) => seg === '..')) {
                throw new FsError('parse_error', `glob pattern may not contain '..' segments: ${pattern}`);
            }
            if (path.isAbsolute(pattern) ||
                /^[A-Za-z]:[\\/]/.test(pattern) ||
                pattern.startsWith('\\\\') ||
                pattern.startsWith('//')) {
                throw new FsError('parse_error', `glob pattern must be workspace-relative: ${pattern}`, { hint: 'pass a relative pattern such as "src/**/*.ts"' });
            }
            const searchRoots = [];
            if (opts.cwd === undefined) {
                ...
                if (directWorkspace && cwd === directWorkspace.path) {
                    ...
                    try {
                        ...
                        const code = err?.code;
                        if (code === 'ENOENT') {
                            throw new FsError('path_not_found', `glob cwd does not exist: ${cwd}`, { cause: err });
                        }
                        throw err;
                    }
                }
                ...
                if (!workspace) {
                    throw new FsError('path_outside_workspace', `glob cwd is outside workspace: ${cwd}`, { hint: 'opts.cwd must be a path obtained from fs.resolve()' });
                }
            ...
            for (const searchRoot of searchRoots) {
                ...
                try {
                    ...
                    if (!rootStat.isDirectory()) {
                        const err = new Error(`glob workspace root is not a directory: ${searchRoot.cwd}`);
                        err.code = 'ENOTDIR';
                        throw err;
                    }
                ...
                for (const hit of matches) {
                    ...
                    const inAnyWorkspace = this.deps.workspaces.some((workspace) => isWithinRoot(canonical, workspace.path));
                    if (!inAnyWorkspace) {
                        escapedCount += 1;
                        continue;
                    }
                    ...
                    const verdict = this.ignoreVerdict(canonical, kind);
                    if (verdict.ignored && !opts.includeIgnored)
                        continue;
            ...
            if (globErrors.length > 0 && successfulGlobRoots === 0) {
                if (globErrors.length === 1)
                    throw globErrors[0];
                throw new AggregateError(globErrors, 'glob failed for all workspace roots');
            }
            if (escapedCount > 0) {
                this.deps.audit.recordDenied(this.deps.ctx, {
                    intent: 'glob',
    ...
    async writeTextAtomic(p, content, opts) {
        ...
        try {
            ...
            const out = await this.deps.pathLocks.runExclusive(p, async () => {
                const existingMeta = opts.mode === 'replace'
                    ? await readExistingTextMeta(p, opts.expectedHash)
                    : undefined;
                if (opts.mode === 'create') {
                    await assertCreateTargetAbsent(p);
                }
                this.deps.generationGuard?.assertOpen();
                ...
                if (verdict.ignored)
                    meta.matchedIgnore = verdict.category;
            ...
            throw this.recordAndWrap(err, 'write', p);
        }
    }
    ...
    async writeText(p, content, opts) {
        ...
        try {
            ...
            // rejected.
            await assertNotSymlinkBeforeWrite(p);
            this.deps.generationGuard?.assertOpen();
    ...
    async editAtomic(p, oldText, newText, opts) {
        const start = performance.now();
        try {
            this.deps.generationGuard?.assertOpen();
            assertTrustedForIntent(this.deps.trusted, 'edit');
            if (!isContentHash(opts.expectedHash)) {
                throw new FsError('parse_error', 'expectedHash must match sha256:<64 lowercase hex chars>');
            }
            if (typeof oldText !== 'string' || oldText.length === 0) {
                throw new FsError('parse_error', `oldText must be a non-empty string for edit on ${p}`);
            }
            ...
            const out = await this.deps.pathLocks.runExclusive(p, async () => {
                ...
                if (snapshot.meta.hash !== opts.expectedHash) {
                    throw new FsError('hash_mismatch', `expected ${opts.expectedHash}, found ${snapshot.meta.hash}`, { hint: 're-read the file and retry with the latest hash' });
                }
                const current = snapshot.content;
                const occurrences = countOccurrences(current, oldText);
                if (occurrences === 0) {
                    const snippet = oldText.length > 80 ? oldText.slice(0, 80) + '...' : oldText;
                    throw new FsError('text_not_found', `oldText not found in ${p}`, {
                        hint: `searched for: ${JSON.stringify(snippet)}`,
                    });
                }
                if (occurrences > 1) {
                    throw new FsError('ambiguous_text_match', `oldText appears ${occurrences} times in ${p}`, {
                        hint: 'pass a larger oldText span that occurs exactly once',
                    });
                }
                const idx = current.indexOf(oldText);
                const next = current.slice(0, idx) +
                    newText +
            ...
            throw this.recordAndWrap(err, 'edit', p);
        }
    }
    async edit(p, oldText, newText, opts) {
        ...
        try {
            ...
            return await this.deps.pathLocks.runExclusive(p, async () => {
                const snapshot = await readTextSnapshotFromResolvedFile(p);
                const current = snapshot.content;
                const idx = current.indexOf(oldText);
                if (idx === -1) {
    ...
    async mkdir(p, opts) {
        ...
        try {
            ...
            assertTrustedForIntent(this.deps.trusted, 'write');
            const out = await this.deps.pathLocks.runExclusive(p, async () => {
                await ensureResolvedDirectory(p, {
     ...
     */
    recordAndWrap(err, intent, input, opts) {
...
function validateWriteTextAtomicOptions(opts) {
    ...
    if (opts.mode !== 'create' && opts.mode !== 'replace') {
        throw new FsError('parse_error', 'mode must be either "create" or "replace" (use writeTextOverwrite() for unconditional overwrites)');
    }
    if (opts.expectedHash !== undefined && !isContentHash(opts.expectedHash)) {
        throw new FsError('parse_error', 'expectedHash must match sha256:<64 lowercase hex chars>');
    }
    if (opts.mode === 'replace' && opts.expectedHash === undefined) {
        throw new FsError('parse_error', 'expectedHash is required when mode is "replace"');
    }
    if (opts.lineEnding !== undefined &&
        opts.lineEnding !== 'lf' &&
        opts.lineEnding !== 'crlf') {
        throw new FsError('parse_error', 'lineEnding must be "lf" or "crlf"');
    }
}
async function readTextFromResolvedFile(p, opts, lowFs) {
    const pre = await fsp.lstat(p);
    ...
    const wantsWindow = opts.limit !== undefined ||
...
async function readTextSnapshotFromResolvedFile(p, opts = {}, knownPre) {
    const pre = knownPre ?? (await fsp.lstat(p));
    if (pre.isSymbolicLink()) {
        throw new FsError('symlink_escape', `path is a symlink: ${p}`, {
            hint: 're-resolve the target file instead of reading through a link',
        });
    ...
    if (pre.size > MAX_READ_BYTES) {
        throw new FsError('file_too_large', `file of ${pre.size} bytes exceeds read cap of ${MAX_READ_BYTES} bytes`, {
            hint: 'use a finite line limit for large UTF-8 text, or readBytes for explicit byte-windowed access',
        });
    }
    const raw = await readStableRegularFileBuffer(p, pre);
    if (looksBinary(raw)) {
    ...
    const sliced = sliceDecodedText(decoded.content, startLineIndex, opts.limit ?? Number.POSITIVE_INFINITY);
    ...
    const pageableLineCount = sliced.originalLineCount - (decoded.content.endsWith('\n') ? 1 : 0);
    ...
    const decodedBytesMatchSource = isUtf8CompatibleEncoding(decoded.encoding) &&
 ...
 */
function assertStreamWindowStable(before, after, p, reason) {
 ...
 */
async function readTextCursorWindowFromResolvedFile(p, pre, opts, lowFs) {
    const cursor = decodeTextCursor(opts.cursor);
    const fh = await fsp.open(p, 'r');
    let opened;
    let afterRead;
    let window;
    ...
    try {
        ...
        assertCursorMatchesFile(cursor, opened, p);
        try {
    ...
    if (post.isSymbolicLink()) {
        throw new FsError('symlink_escape', `path was replaced with a symlink during read: ${p}`, { hint: 'TOCTOU swap detected via post-read lstat' });
    }
    assertSameFile(opened, afterRead, p, 'read');
    assertStreamWindowStable(opened, afterRead, p, 'file changed during read');
    assertSameFile(opened, post, p, 'read');
    ...
    if (window === undefined) {
        throw new FsError('internal_error', `cursor text read returned no result: ${p}`);
    }
    const meta = {
        ...
        truncated: true,
        hasMore: window.nextOffset !== undefined || window.truncatedByBytes === true,
    };
...
async function readLargeTextWindowFromResolvedFile(p, pre, opts, lowFs) {
    const fh = await fsp.open(p, 'r');
    let opened;
    let afterRead;
    let result;
    ...
    if (result === undefined) {
        throw new FsError('internal_error', `large text range read returned no result: ${p}`);
    }
    ...
    if (readMeta?.originalLineCountExact === true &&
        readMeta?.originalLineCount !== undefined) {
        meta.originalLineCount = readMeta.originalLineCount;
    }
    return { content, meta };
}
async function readStableRegularFileBuffer(p, pre) {
    const fh = await fsp.open(p, 'r');
    let opened;
    try {
        opened = await fh.stat();
        ...
        if (opened.size > MAX_READ_BYTES) {
            throw new FsError('file_too_large', `file of ${opened.size} bytes exceeds read cap of ${MAX_READ_BYTES} bytes`, {
                hint: 'use a finite line limit for large UTF-8 text, or readBytes for explicit byte-windowed access',
            });
        }
...
function sliceDecodedText(content, startLine, limit) {
...
function looksBinary(buf) {
    ...
    const hasUnicodeBom = (bomProbe.length >= 4 &&
        ((bomProbe[0] === 0xff &&
            bomProbe[1] === 0xfe &&
            bomProbe[2] === 0x00 &&
            bomProbe[3] === 0x00) ||
...
async function readExistingTextMeta(p, expectedHash) {
    const snapshot = await readTextSnapshotFromResolvedFile(p);
    if (expectedHash !== undefined && snapshot.meta.hash !== expectedHash) {
        throw new FsError('hash_mismatch', `expected ${expectedHash}, found ${snapshot.meta.hash}`, { hint: 're-read the file and retry with the latest hash' });
    }
    return snapshot.meta;
}
function mergeWriteMeta(existing, opts) {
    return {
...
async function atomicWriteTextResolvedFile(input) {
    const buf = await encodeTextFileContentAsync(input.target, input.content, buildWriteMeta(input.meta));
 ...
 */
async function ensureResolvedDirectory(target, opts) {
    const assertRealDirectory = async (p) => {
        const st = await fsp.lstat(p);
        ...
        if (!st.isDirectory()) {
            throw new FsError('parse_error', `path exists and is not a directory: ${p}`);
        }
    };
    ...
    // create through it.
    const assertParentNotSymlink = async (p) => {
        const parent = path.dirname(p);
        const st = await fsp.lstat(parent);
        if (st.isSymbolicLink()) {
            throw new FsError('symlink_escape', `directory parent is a symlink: ${parent}`, { hint: 're-resolve the target after detecting symlink swaps' });
        }
    };
    try {
...
async function atomicPublishResolvedFile(input) {
    ...
    const tmpBaseMaxBytes = tmpNameMaxBytes - 1 - Buffer.byteLength(tmpSuffix, 'utf-8');
    let tmpBase = path.basename(target);
    if (Buffer.byteLength(tmpBase, 'utf-8') > tmpBaseMaxBytes) {
        tmpBase = safeUtf8Truncate(Buffer.from(tmpBase, 'utf-8'), tmpBaseMaxBytes).toString('utf-8');
    }
    ...
    try {
        ...
        await chmodHandleBestEffort(tempHandle, targetState.mode ?? input.newFileModeBits ?? OWNER_ONLY_NEW_FILE_MODE);
...
async function reserveTempFile(tmpPath) {
...
async function writeBufferToTemp(input) {
    ...
    if (st.isSymbolicLink()) {
        throw new FsError('symlink_escape', `temporary path became a symlink: ${input.tmpPath}`, { hint: 'temp-file race detected before final rename' });
    }
    if (!st.isFile()) {
        throw new FsError('parse_error', `temporary path is not a regular file: ${input.tmpPath}`);
    }
    return { sizeBytes: input.buf.length, hash: hashBuffer(input.buf), stat: st };
}
async function assertCreateTargetAbsent(target) {
    try {
        const st = await fsp.lstat(target);
        if (st.isSymbolicLink()) {
            throw new FsError('symlink_escape', `path is a symlink and cannot be created over: ${target}`, { hint: 'remove the symlink or resolve the target explicitly' });
        }
...
async function assertAtomicTargetPrecondition(input) {
    ...
    if (input.mode === 'overwrite') {
        ...
        if (pre.isSymbolicLink()) {
            throw new FsError('symlink_escape', `path is a symlink and cannot be overwritten atomically: ${input.target}`, {
                hint: 're-resolve the target file instead of writing through a link',
            });
        }
        if (!pre.isFile()) {
            throw new FsError('parse_error', `path is not a regular file: ${input.target}`);
        }
    ...
    if (pre.isSymbolicLink()) {
        throw new FsError('symlink_escape', `path is a symlink and cannot be replaced atomically: ${input.target}`, { hint: 're-resolve the target file instead of writing through a link' });
    }
    if (!pre.isFile()) {
        throw new FsError('parse_error', `path is not a regular file: ${input.target}`);
    }
    ...
    if (actual !== input.expectedHash) {
        throw new FsError('hash_mismatch', `expected ${input.expectedHash}, found ${actual}`, { hint: 're-read the file and retry with the latest hash' });
    }
    return { mode: pre.mode & 0o7777 };
}
async function hashRegularFileAtPath(p, pre) {
    const fh = await fsp.open(p, 'r');
    ...
    try {
        ...
        let offset = 0;
        while (offset < opened.size) {
            const read = await fh.read(buf, 0, Math.min(buf.length, opened.size - offset), offset);
            if (read.bytesRead === 0)
                break;
...
function assertSameFile(pre, post, p, intent) {
...
async function syncHandleBestEffort(fh) {
    try {
        await fh.sync();
    }
    catch {
...
async function fsyncParentDirBestEffort(parent) {
    let fh;
    try {
...
async function chmodHandleBestEffort(fh, mode) {
    try {
        await fh.chmod(mode);
    }
    catch {
...
async function assertTempPathMatchesStat(tmpPath, expected) {
    const st = await fsp.lstat(tmpPath);
    if (st.isSymbolicLink()) {
        throw new FsError('symlink_escape', `temporary path is a symlink: ${tmpPath}`, {
            hint: 'temp-file race detected before final rename',
        });
    }
    if (!st.isFile()) {
        throw new FsError('parse_error', `temporary path is not a regular file: ${tmpPath}`);
    }
...
async function publishCreateNoClobber(tmpPath, target) {
    try {
...
async function renameWithRetryLocal(src, dest, retries, delayMs) {
 ...
 */
function safeUtf8Truncate(buf, maxBytes) {
 ...
 * original handle.
 */
async function assertInodeStableAfterRead(p, preIno) {
    const post = await fsp.lstat(p);
    if (post.isSymbolicLink()) {
        throw new FsError('symlink_escape', `path was replaced with a symlink during read: ${p}`, { hint: 'TOCTOU swap detected via post-read lstat' });
    }
    ...
    if (preNum !== 0n && postNum !== 0n && preNum !== postNum) {
        throw new FsError('symlink_escape', `path inode changed during read: ${p}`, { hint: 'TOCTOU swap detected via inode comparison' });
    }
}
/**
 ...
 */
async function assertNotSymlinkBeforeWrite(p) {
    let pre;
    try {
        ...
        const code = err?.code;
        if (code === 'ENOENT')
            return; // ahead-of-create flow
        throw err;
    }
    if (pre.isSymbolicLink()) {
        throw new FsError('symlink_escape', `path was replaced with a symlink before write: ${p}`, {
...
function errorKindForRealpathFailure(err) {
    const code = errorCode(err);
...
function buildWriteMeta(opts) {
    ...
    if (opts.lineEnding)
        meta['lineEnding'] = opts.lineEnding;
...
//# sourceMappingURL=workspace-file-system.js.map