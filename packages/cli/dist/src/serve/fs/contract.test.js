...
import { WorkspaceContext } from '@lailatul-coder/lailatul-coder-core';
 ...
 * unify.
 */
const OUT_OF_WORKSPACE_KINDS = new Set([
...
const CORPUS = [
    ...
    },
    {
        ...
        setup: async (ws) => {
            ...
            await fsp.symlink(path.join(ws, 'real.txt'), path.join(ws, 'alias'), 'file');
        },
        input: () => 'alias',
        expectInside: true,
        intent: 'read',
...
describe('contract: resolveWithinWorkspace ↔ WorkspaceContext', () => {
    let scratch;
    beforeAll(async () => {
        scratch = await fsp.mkdtemp(path.join(os.tmpdir(), `qwen-fs-contract-${randomBytes(4).toString('hex')}-`));
    });
    afterAll(async () => {
        await fsp.rm(scratch, { recursive: true, force: true });
    });
    it.each(CORPUS.map((tc) => [tc.name, tc]))('%s', async (_name, tc) => {
        ...
        if (tc.setup)
            await tc.setup(workspace, scratch);
        ...
        try {
            resolved = await resolveWithinWorkspace(tc.input(workspace, scratch), workspace, tc.intent);
        }
        catch (err) {
            if (!isFsError(err))
                throw err;
