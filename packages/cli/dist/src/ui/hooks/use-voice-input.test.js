...
describe('use-voice-input', () => {
    ...
    it('sanitizes ASR transcripts before inserting and submitting', async () => {
        ...
        await waitFor(() => {
            expect(buffer.insert).toHaveBeenCalledWith('send this\\u001b[8m hidden\\u001b[0m');
            expect(onSubmit).toHaveBeenCalledWith('send this\\u001b[8m hidden\\u001b[0m');
        });
    });
    ...
    it('waits for a pending stream session before finalizing quick streaming input', async () => {
        ...
        const stop = vi
            .fn()
            .mockRejectedValue(new Error('Native audio capture produced empty audio.'));
        const drain = vi
            .fn()
        ...
        await waitFor(() => {
            expect(streamSession.pushAudio).toHaveBeenCalledWith(new Uint8Array([1, 2, 3]));
    ...
    it('refines streamed transcripts before inserting and submitting', async () => {
        ...
        await waitFor(() => {
            expect(streamSession.finish).toHaveBeenCalledTimes(1);
            expect(transcribe).not.toHaveBeenCalled();
            expect(refine).toHaveBeenCalledWith('um streamed text', expect.any(AbortSignal));
            expect(buffer.insert).toHaveBeenCalledWith('streamed text');
    ...
    it('stops the recorder when streaming requires an unsupported backend', async () => {
        ...
        await waitFor(() => {
            ...
            expect(addItem).toHaveBeenCalledWith({
                type: 'error',
                text: 'Voice transcription failed: Streaming voice transcription requires native audio capture. Install/rebuild @lailatul-coder/audio-capture or switch voiceModel to qwen3-asr-flash for batch transcription.',
            }, expect.any(Number));
            expect(result.current.status).toBe('idle');
        });
    });
    ...
    it('refines the transcript before inserting and submitting (tap mode)', async () => {
        ...
        await waitFor(() => {
            expect(refine).toHaveBeenCalledWith('um the diff', expect.any(AbortSignal));
    ...
    it('drops the refined transcript when cancelled mid-refine', async () => {
        ...
        const refine = vi.fn((_raw, signal) => {
    ...
    it('hold mode: refines without submitting', async () => {
        ...
        try {
            ...
            expect(refine).toHaveBeenCalledWith('um hold text', expect.any(AbortSignal));
            expect(buffer.insert).toHaveBeenCalledWith('hold text');
            expect(onSubmit).not.toHaveBeenCalled();
            expect(result.current.status).toBe('idle');
        }
...
//# sourceMappingURL=use-voice-input.test.js.map