...
import { MEMORY_FRONTMATTER_EXAMPLE, TYPES_SECTION_INDIVIDUAL, WHAT_NOT_TO_SAVE_SECTION, } from './prompt.js';
import { AUTO_MEMORY_INDEX_FILENAME, AUTO_MEMORY_PINNED_DIRNAME, getAutoMemoryRoot, getUserAutoMemoryRoot, } from './paths.js';
...
function buildAgentHistory(history) {
    ...
    const toolResponses = buildFunctionResponseParts(last, 'Background extraction started.');
    return [
        ...history,
...
async function buildTopicSummaryBlock(projectRoot) {
    // User-level scan is best-effort: a read failure on `~/.lailatulcoder/memories/`
    ...
    const [projectDocs, userDocs] = await Promise.all([
        ...
        scanUserAutoMemoryTopicDocuments().catch((error) => {
            debugLogger.warn(`User-level auto-memory scan failed; extraction agent will see project-level summaries only: ${error instanceof Error ? error.message : String(error)}`);
            return [];
        }),
    ]);
    const renderDoc = (doc, scope) => {
...
function buildTaskPrompt(projectMemoryRoot, userMemoryRoot, topicSummaries) {
    return [
 ...
 * the agent.
 */
function touchedTopicsFromFilePaths(filePaths, projectRoot) {
    ...
    const isUnderRoot = (canonP, canonRoot) => {
        if (!canonP.startsWith(canonRoot))
            return false;
...
export async function runAutoMemoryExtractionByAgent(config, projectRoot) {
    const cacheSafe = getCacheSafeParams();
    if (!cacheSafe) {
        throw new Error('runAutoMemoryExtractionByAgent: no cache-safe params available; ' +
    ...
    const result = await runForkedAgent({
        ...
        config: scopedConfig,
        taskPrompt: buildTaskPrompt(projectMemoryRoot, userMemoryRoot, topicSummaries),
    ...
    const { topics, touchedProjectScope, touchedUserScope } = touchedTopicsFromFilePaths(result.filesWritten ?? [], projectRoot);
    return {
        touchedTopics: topics,
        touchedProjectScope,
        touchedUserScope,
        hasToolActivity: result.filesTouched.length > 0,
        systemMessage: topics.length > 0
            ? `Managed auto-memory updated: ${topics.map((t) => `${t}.md`).join(', ')}`
            : undefined,
    };
}
//# sourceMappingURL=extractionAgentPlanner.js.map