 ...
 */
import { fetchGitBranches, findGitRoot, gitCheckout, gitCreateBranch, gitPush, gitPull, gitCommit, isValidRefName, isValidCheckoutRef, } from '@lailatul-coder/lailatul-coder-core';
import { safeBody } from '../server/request-helpers.js';
import { requireTrustedWorkspaceRuntime, resolveContainedCwd, resolveContainedCwdOrFail, resolveWorkspaceRuntimeFromParam, sendGenerationClosedError, sendUntrustedWorkspaceResponse, } from '../workspace-route-runtime.js';
...
function sendGitError(res, err, route, sendBridgeError, cwd) {
...
async function handleBranches(res, cwd, sendBridgeError, route, assertGenerationOpen, env) {
    try {
...
async function handleCheckout(req, res, cwd, sendBridgeError, route, env) {
...
async function handleCreateBranch(req, res, cwd, sendBridgeError, route, env) {
    const body = safeBody(req);
    ...
    const startPoint = typeof rawStartPoint === 'string'
...
async function handlePush(req, res, cwd, sendBridgeError, route, env) {
    const body = safeBody(req);
    if (body['setUpstream'] !== undefined &&
...
async function handlePull(req, res, cwd, sendBridgeError, route, env) {
    const body = safeBody(req);
...
async function handleCommit(req, res, cwd, sendBridgeError, route, env) {
    const body = safeBody(req);
...
export function registerWorkspaceGitBranchRoutes(app, deps) {
    app.get('/workspace/git/branches', (_req, res) => {
        if (deps.isWorkspaceTrusted?.() === false) {
            sendUntrustedWorkspaceResponse(res);
            return;
        }
        void handleBranches(res, deps.boundWorkspace, deps.sendBridgeError, 'GET /workspace/git/branches', deps.captureGenerationAssertion?.());
    });
    app.post('/workspace/git/checkout', deps.mutate({ strict: true }), (req, res) => {
        if (deps.isWorkspaceTrusted?.() === false) {
            sendUntrustedWorkspaceResponse(res);
            return;
        }
        ...
        void handleCheckout(req, res, deps.boundWorkspace, deps.sendBridgeError, 'POST /workspace/git/checkout');
    });
    app.post('/workspace/git/branch', deps.mutate({ strict: true }), (req, res) => {
        if (deps.isWorkspaceTrusted?.() === false) {
            sendUntrustedWorkspaceResponse(res);
            return;
        }
        ...
        void handleCreateBranch(req, res, deps.boundWorkspace, deps.sendBridgeError, 'POST /workspace/git/branch');
    });
    app.post('/workspace/git/push', deps.mutate({ strict: true }), (req, res) => {
        ...
        void handlePush(req, res, deps.boundWorkspace, deps.sendBridgeError, 'POST /workspace/git/push');
    });
    app.post('/workspace/git/pull', deps.mutate({ strict: true }), (req, res) => {
        ...
        void handlePull(req, res, deps.boundWorkspace, deps.sendBridgeError, 'POST /workspace/git/pull');
    });
    app.post('/workspace/git/commit', deps.mutate({ strict: true }), (req, res) => {
        if (deps.isWorkspaceTrusted?.() === false) {
            sendUntrustedWorkspaceResponse(res);
            return;
        }
        ...
        void handleCommit(req, res, deps.boundWorkspace, deps.sendBridgeError, 'POST /workspace/git/commit');
    });
}
export function registerWorkspaceQualifiedGitBranchRoutes(app, deps) {
    app.get('/workspaces/:workspace/git/branches', (req, res) => {
        const runtime = resolveTrustedRuntime(deps.workspaceRegistry, req, res);
        if (!runtime)
            return;
        void handleBranches(res, resolveContainedCwd(req, runtime.workspaceCwd), deps.sendBridgeError, 'GET /workspaces/:workspace/git/branches', () => runtime.generationGuard?.assertOpen(), runtime.env.effectiveEnv);
    });
    app.post('/workspaces/:workspace/git/checkout', deps.mutate({ strict: true }), (req, res) => {
        const runtime = resolveTrustedRuntime(deps.workspaceRegistry, req, res);
        if (!runtime)
            return;
        try {
        ...
        void handleCheckout(req, res, cwd, deps.sendBridgeError, 'POST /workspaces/:workspace/git/checkout', runtime.env.effectiveEnv);
    });
    app.post('/workspaces/:workspace/git/branch', deps.mutate({ strict: true }), (req, res) => {
        const runtime = resolveTrustedRuntime(deps.workspaceRegistry, req, res);
        if (!runtime)
            return;
        try {
        ...
        void handleCreateBranch(req, res, cwd, deps.sendBridgeError, 'POST /workspaces/:workspace/git/branch', runtime.env.effectiveEnv);
    });
    app.post('/workspaces/:workspace/git/push', deps.mutate({ strict: true }), (req, res) => {
        const runtime = resolveTrustedRuntime(deps.workspaceRegistry, req, res);
        if (!runtime)
            return;
        try {
        ...
        void handlePush(req, res, cwd, deps.sendBridgeError, 'POST /workspaces/:workspace/git/push', runtime.env.effectiveEnv);
    });
    app.post('/workspaces/:workspace/git/pull', deps.mutate({ strict: true }), (req, res) => {
        const runtime = resolveTrustedRuntime(deps.workspaceRegistry, req, res);
        if (!runtime)
            return;
        try {
        ...
        void handlePull(req, res, cwd, deps.sendBridgeError, 'POST /workspaces/:workspace/git/pull', runtime.env.effectiveEnv);
    });
    app.post('/workspaces/:workspace/git/commit', deps.mutate({ strict: true }), (req, res) => {
        const runtime = resolveTrustedRuntime(deps.workspaceRegistry, req, res);
        if (!runtime)
            return;
        try {
        ...
        void handleCommit(req, res, cwd, deps.sendBridgeError, 'POST /workspaces/:workspace/git/commit', runtime.env.effectiveEnv);
    });
}
//# sourceMappingURL=workspace-git-branches.js.map