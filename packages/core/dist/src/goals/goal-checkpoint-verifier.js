...
export class GoalCheckpointVerifierInputTooLargeError extends Error {
    ...
    constructor(byteLength) {
        super(`Goal checkpoint verifier request exceeds the ${GOAL_CHECKPOINT_VERIFIER_REQUEST_BYTE_LIMIT}-byte limit`);
...
export function parseGoalCheckpointVerifierText(text) {
    let value;
    try {
...
export function validateGoalCheckpointVerifierText(text) {
    try {
...
export function createGoalCheckpointVerifier(config, options = {}) {
    ...
    return async (input, attemptSignal) => {
        ...
        const timer = setTimeout(() => {
            timeoutController.abort(new Error(`Goal checkpoint verifier timed out after ${timeoutMs}ms`));
        }, timeoutMs);
...
function parseClaim(value, index) {
    if (!isRecord(value) ||
        ...
        value['sourceRefs'].some((reference) => typeof reference !== 'string' || reference.length === 0) ||
...
//# sourceMappingURL=goal-checkpoint-verifier.js.map