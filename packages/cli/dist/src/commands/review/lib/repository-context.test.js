...
import { repositoryContextOf, validateRepositoryContext, } from './repository-context.js';
const valid = {
    version: 1,
...
describe('repository context validation', () => {
    ...
    it('fails closed on a present-but-null repositoryContext', () => {
        ...
        expect(() => repositoryContextOf({ repositoryContext: null })).toThrow('repositoryContext must be an object');
    });
    it('rejects unknown or missing fields and versions', () => {
        expect(() => validateRepositoryContext({ ...valid, version: 2 })).toThrow('unsupported repositoryContext version');
        expect(() => validateRepositoryContext({ ...valid, extra: true })).toThrow('unknown or missing fields');
        const { label: _label, ...withoutLabel } = valid;
        expect(() => validateRepositoryContext(withoutLabel)).toThrow('unknown or missing fields');
    });
    ...
    it('requires bounded sorted unique safe tokens and text', () => {
        expect(() => validateRepositoryContext({ ...valid, domains: ['runtime', 'compiler'] })).toThrow('sorted and unique');
        expect(() => validateRepositoryContext({ ...valid, domains: ['runtime', 'runtime'] })).toThrow('sorted and unique');
        expect(() => validateRepositoryContext({ ...valid, provider: '../provider' })).toThrow('provider is invalid');
        ...
        for (const separator of [
            ...
            expect(() => validateRepositoryContext({
                ...valid,
    ...
    it('enforces sorted-and-unique on every array field', () => {
        ...
        for (const [field, [unsorted, duplicated]] of Object.entries(probes)) {
            expect(() => validateRepositoryContext({ ...valid, [field]: unsorted })).toThrow(`${field} must be sorted and unique`);
            expect(() => validateRepositoryContext({ ...valid, [field]: duplicated })).toThrow(`${field} must be sorted and unique`);
        }
    });
    it('accepts every length bound exactly and rejects one past it', () => {
        ...
        expect(() => validateRepositoryContext({ ...valid, provider: 'p'.repeat(65) })).toThrow('provider is invalid');
        expect(() => validateRepositoryContext({ ...valid, label: 'l'.repeat(121) })).toThrow('label is invalid');
        expect(() => validateRepositoryContext({ ...valid, domains: ['t'.repeat(161)] })).toThrow('domains is invalid');
        expect(() => validateRepositoryContext({
            ...valid,
    ...
    it('accepts the item-count bound exactly', () => {
        ...
        const atBound = {
            ...valid,
            domains: Array.from({ length: 256 }, (_, index) => `d-${String(index).padStart(3, '0')}`),
        };
        expect(validateRepositoryContext(atBound)).toEqual(atBound);
    });
    ...
    it('fails closed on non-string and empty values', () => {
        ...
        expect(() => validateRepositoryContext({ ...valid, label: 123 })).toThrow('label is invalid');
        expect(() => validateRepositoryContext({ ...valid, label: '' })).toThrow('label is invalid');
        expect(() => validateRepositoryContext({ ...valid, domains: [123] })).toThrow('domains is invalid');
        expect(() => validateRepositoryContext({ ...valid, domains: [''] })).toThrow('domains is invalid');
        expect(() => validateRepositoryContext({ ...valid, verificationNotes: [null] })).toThrow('verificationNotes is invalid');
    });
    it('rejects unsafe paths and roles that cannot join the initial roster', () => {
        for (const path of [
            ...
            expect(() => validateRepositoryContext({ ...valid, relatedPaths: [path] })).toThrow();
        }
        for (const role of [
            'not-a-role',
            '7',
            ...
            expect(() => validateRepositoryContext({ ...valid, requiredAgents: [role] })).toThrow('unsupported role');
        }
    });
});
//# sourceMappingURL=repository-context.test.js.map