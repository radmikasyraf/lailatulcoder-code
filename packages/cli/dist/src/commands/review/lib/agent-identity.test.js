...
import { labelFromIdentityLine, labelFromLaunchPrompt, } from './agent-identity.js';
describe('labelFromIdentityLine', () => {
    it('parses the role, keeping round and owned-file suffixes distinct', () => {
        expect(labelFromIdentityLine('You are review agent `security` — inspect auth')).toBe('agent security');
        ...
        expect(labelFromIdentityLine('You are review agent `reverse-audit` — Reverse audit agent (round 2).')).toBe('agent reverse-audit (round 2)');
        ...
        expect(labelFromIdentityLine('You are review agent `invariant-a` — Whole-file invariants. Your file: `packages/cli/src/a.ts`.')).toBe('agent invariant-a (packages/cli/src/a.ts)');
        // A chunk role labels as its chunk id, matching coverage's chunk labels.
        expect(labelFromIdentityLine('You are review agent `chunk 3 of 7` — the territory agent for lines 120-260 of the diff.')).toBe('chunk 3');
        ...
        expect(labelFromIdentityLine('You are review agent `invariant-a` — Whole-file invariants (round 2). Your file: `packages/cli/src/a.ts`.')).toBe('agent invariant-a (round 2)');
    });
    it('tolerates a trailing carriage return — CRLF prompts must still parse', () => {
        ...
        expect(labelFromIdentityLine('You are review agent `security` — inspect auth\r')).toBe('agent security');
        expect(labelFromLaunchPrompt('context line\r\nYou are review agent `6c` — Undirected audit.\r\nbody\r\n')).toBe('agent 6c');
    });
    it('returns null for anything that is not an identity line', () => {
        expect(labelFromIdentityLine('PR #9045 modifies getAuthTypeFromEnv().')).toBeNull();
        expect(labelFromIdentityLine('')).toBeNull();
        // A mid-line mention is a quote, not an identity.
        expect(labelFromIdentityLine('as noted, You are review agent `security` was launched earlier')).toBeNull();
    });
});
describe('labelFromLaunchPrompt', () => {
    it('finds the identity line under a launcher-prepended context line', () => {
        ...
        expect(labelFromLaunchPrompt('PR #9045 (fixes issue #9025) modifies getAuthTypeFromEnv().\n\n' +
            'You are review agent `6c` — Agent 6c: Undirected audit.\n' +
            'Read your brief first.')).toBe('agent 6c');
    });
    it("takes the agent's OWN line, which precedes anything quoted below it", () => {
        ...
        expect(labelFromLaunchPrompt('You are review agent `verify` — Verification agent (round 2).\n' +
            'Prior findings:\n' +
            'You are review agent `security` — inspect auth\n')).toBe('agent verify (round 2)');
    });
    it('returns null when no line is an identity line', () => {
        expect(labelFromLaunchPrompt('Security review of the whole diff.')).toBeNull();
    });
    it('differs from the identity-line entry point on a quoted-below prompt', () => {
        ...
        // fail here.
        const quotedBelow = 'Context: the orchestrator rewrote this launch.\n' +
...
//# sourceMappingURL=agent-identity.test.js.map