...
describe('runScratchTree', () => {
    ...
    const run = (label = 'verify--round-1--abc123') => runScratchTree({ worktree, label });
    beforeEach(() => {
    ...
    it('stands up a sibling tree at the commit under review', () => {
        ...
        expect(r.path).toBe(scratchWorktreePath(worktree, 'verify--round-1--abc123'));
        expect(git(r.path, 'rev-parse', 'HEAD')).toBe(headSha);
        expect(readFileSync(join(r.path, 'a.ts'), 'utf8')).toBe('export const x = 1;\n');
    });
    it('refuses while repo-local config defines a content filter — checkouts would execute it', () => {
        ...
        expect(existsSync(scratchWorktreePath(worktree, 'verify--round-1--abc123'))).toBe(false);
    ...
    it("screens ANOTHER worktree's per-worktree config, not just this one's", () => {
        ...
        const common = execFileSync('git', ['rev-parse', '--path-format=absolute', '--git-common-dir'], { cwd: worktree, encoding: 'utf8' }).trim();
        const scratchAdmin = join(common, 'worktrees', basename(first.path), 'config.worktree');
        execFileSync('git', ['config', 'extensions.worktreeConfig', 'true'], {
            cwd: worktree,
        });
        writeFileSync(scratchAdmin, '[filter "planted"]\n\tsmudge = touch /tmp/qwen-should-never-run\n');
        const r = run();
        expect(r.available).toBe(false);
    ...
    it('hands back a PRISTINE tree on reuse — a stale mutant is a wrong verdict', () => {
        ...
        expect(readFileSync(join(second.path, 'a.ts'), 'utf8')).toBe('export const x = 1;\n');
    ...
    it('rebuilds over a leftover directory instead of resetting the PARENT checkout', () => {
        ...
        expect(git(repo, 'rev-parse', '--abbrev-ref', 'HEAD')).toBe('main');
        expect(readFileSync(join(repo, 'a.ts'), 'utf8')).toBe('LOCAL UNCOMMITTED WORK\n');
    });
    ...
    it('replaces a node_modules it did not build rather than trusting it', () => {
        ...
        writeFileSync(join(first.path, 'node_modules', 'planted-stub.js'), 'module.exports = 1;\n');
        const second = run();
        expect(second.reused).toBe(true);
        expect(existsSync(join(second.path, 'node_modules', 'planted-stub.js'))).toBe(false);
    ...
    it('clears IGNORED probe state too — pristine means pristine', () => {
        ...
        writeFileSync(join(first.path, 'fixtures', 'node_modules', 'planted.js'), 'x');
        const second = run();
        expect(second.reused).toBe(true);
    ...
    it('rebuilds when the tree belongs to a DIFFERENT repository', () => {
        ...
        const first = run();
        writeFileSync(join(first.path, '.git'), `gitdir: ${join(other, '.git')}\n`);
        const second = run();
        expect(second.available).toBe(true);
        expect(second.reused).toBe(false);
    ...
    it('rebuilds when the scratch .git borrows a SIBLING worktree’s admin entry', () => {
        ...
        const admin = git(sibling, 'rev-parse', '--path-format=absolute', '--git-dir');
    ...
    it.skipIf(process.platform === 'win32' || process.getuid?.() === 0)('says linking FAILED rather than "no node_modules" when the farm throws', () => {
    ...
    it('leaves the shared review worktree untouched by everything it does', () => {
        ...
        expect(readFileSync(join(worktree, 'a.ts'), 'utf8')).toBe('export const x = 1;\n');
    ...
    it.skipIf(process.platform === 'win32' || process.getuid?.() === 0)('reports UNMEASURED rather than clean when the residue check cannot run', () => {
        ...
        const index = execFileSync('git', ['rev-parse', '--path-format=absolute', '--git-path', 'index'], { cwd: worktree, encoding: 'utf8' }).trim();
    ...
    it.skipIf(process.platform === 'win32' || process.getuid?.() === 0)('keeps the measured residue when the tree could not be created', () => {
        ...
        try {
            ...
            expect(r.note).toContain('Do NOT fall back to probing in the review worktree');
        }
        finally {
    ...
    describe('the command handler', () => {
        ...
        it('refuses a directory --out BEFORE standing anything up', () => {
            ...
            scratchTreeCommand.handler({
                worktree,
                label: 'verify--round-1--out',
                out: outDir,
            });
            expect(process.exitCode).toBe(2);
            expect(existsSync(scratchWorktreePath(worktree, 'verify--round-1--out'))).toBe(false);
        });
        it('prints the report before writing the side file', () => {
            ...
            const printed = writeStdoutLine
                .mock.calls[0][0];
            expect(JSON.parse(printed).available).toBe(true);
            expect(JSON.parse(readFileSync(out, 'utf8')).path).toBe(scratchWorktreePath(worktree, 'verify--round-1--out'));
        });
...
//# sourceMappingURL=scratch-tree.test.js.map