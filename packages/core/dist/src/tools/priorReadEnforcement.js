 ...
 * seen them.
 */
export async function checkPriorRead(cache, filePath, verb, options = {}) {
    let stats;
    try {
        ...
        if (code === 'ENOENT') {
            if (options.expectExisting) {
                ...
                // bytes.
                const raw = `File ${filePath} disappeared after the model read it ` +
        ...
        const raw = `Could not stat ${filePath} to verify prior read (${code ?? 'unknown error'}). ` +
    ...
    if (stats.isDirectory()) {
        ...
        const raw = `${filePath} is a directory. The Edit / WriteFile tools only ` +
    ...
    if (!stats.isFile()) {
        const verbBare = verb === 'editing' ? 'edit' : 'overwrite';
        const raw = `${filePath} is a FIFO / socket / character or block device. ` +
    ...
    if (status.state === 'stale') {
        const raw = `File ${filePath} has been modified since you last read it ` +
    ...
    if (status.state === 'unverifiable') {
        const verbBare = verb === 'editing' ? 'edit' : 'overwrite';
        const raw = `File ${filePath} is on a filesystem that does not provide a ` +
    ...
    if (status.state === 'fresh' &&
        ...
        // correct).
        const verbBare = verb === 'editing' ? 'edit' : 'overwrite';
        const raw = `File ${filePath} is a binary / image / audio / video / PDF / ` +
    ...
    const partialReadGuidance = verb === 'editing'
    ...
    const raw = `File ${filePath} has not been read in this session. ` +
...
//# sourceMappingURL=priorReadEnforcement.js.map