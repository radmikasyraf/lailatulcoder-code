...
export function createHealthRoutes(deps) {
    ...
    // arbitrary IP:port to confirm a `LailatulCoder serve` is listening), so
    ...
    const healthHandler = (req, res) => {
        ...
        try {
            ...
            const workspaceContext = failedWorkspaceId !== undefined
                ...
                : '';
            writeStderrLine(`LailatulCoder serve: /health deep probe failed${workspaceContext}: ${err instanceof Error ? err.message : String(err)}`);
            res
                .status(503)
