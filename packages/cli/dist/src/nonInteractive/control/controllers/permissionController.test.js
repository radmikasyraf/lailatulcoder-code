...
import { InputFormat, ToolConfirmationOutcome, } from '@lailatul-coder/lailatul-coder-core';
import { createMinimalSettings } from '../../../config/settings.js';
import { PermissionController } from './permissionController.js';
function createContext(canUseToolTimeoutMs) {
...
describe('PermissionController', () => {
    it('round-trips workflow approval through can_use_tool with updated input', async () => {
        ...
        const controller = new PermissionController(context, createRegistry(), 'PermissionController');
        ...
        await controller.handleWorkflowApproval('wf_1', approval, { command: 'echo unsafe' }, approvalSignal);
        expect(sendControlRequest).toHaveBeenCalledWith(expect.objectContaining({
        ...
        }), 120_000, expect.any(AbortSignal));
        expect(resolvePendingApproval).toHaveBeenCalledWith('wf_1', 'wfap_1', ToolConfirmationOutcome.ProceedOnce, { updatedInput });
    });
    it('cancels a workflow approval when the host channel times out', async () => {
        ...
        const controller = new PermissionController(context, createRegistry(), 'PermissionController');
        vi.spyOn(controller, 'sendControlRequest').mockRejectedValue(new Error('Control request timeout'));
        await controller.handleWorkflowApproval('wf_2', {
            approvalId: 'wfap_2',
            name: 'write_file',
            confirmationDetails: {
                type: 'info',
        ...
        }, { path: '/tmp/test' }, new AbortController().signal);
        expect(resolvePendingApproval).toHaveBeenCalledWith('wf_2', 'wfap_2', ToolConfirmationOutcome.Cancel);
    });
    it('cancels a workflow approval when the host denies it', async () => {
        ...
        await controller.handleWorkflowApproval('wf_3', {
            approvalId: 'wfap_3',
            name: 'read_file',
            confirmationDetails: {
                type: 'info',
        ...
        expect(resolvePendingApproval).toHaveBeenCalledWith('wf_3', 'wfap_3', ToolConfirmationOutcome.Cancel, undefined);
    });
    it('forwards the host deny message to the workflow subagent', async () => {
        ...
        await controller.handleWorkflowApproval('wf_deny_msg', {
        ...
        expect(resolvePendingApproval).toHaveBeenCalledWith('wf_deny_msg', 'wfap_deny_msg', ToolConfirmationOutcome.Cancel, { cancelMessage: 'Not allowed by policy' });
    });
    it('cancels a pending host request when the workflow approval is cleared', async () => {
        ...
        const controller = new PermissionController(context, createRegistry(), 'PermissionController');
        vi.spyOn(controller, 'sendControlRequest').mockImplementation((_payload, _timeout, signal) => new Promise((_, reject) => {
            signal?.addEventListener('abort', () => reject(new Error('Request aborted')), { once: true });
        }));
        const approvalAbort = new AbortController();
        const request = controller.handleWorkflowApproval('wf_cleared', {
        ...
        }, { path: '/tmp/test' }, approvalAbort.signal);
        approvalAbort.abort();
        await request;
        expect(resolvePendingApproval).toHaveBeenCalledWith('wf_cleared', 'wfap_cleared', ToolConfirmationOutcome.Cancel);
    });
    ...
    it('fails closed without an interactive SDK even in yolo mode', async () => {
        ...
        await vi.waitFor(() => {
            expect(onConfirm).toHaveBeenCalledWith(ToolConfirmationOutcome.Cancel, expect.objectContaining({
    ...
    it('routes ask_user_question answers from updatedInput into the confirmation payload', async () => {
        ...
        await vi.waitFor(() => {
            expect(onConfirm).toHaveBeenCalledWith(ToolConfirmationOutcome.ProceedOnce, expect.objectContaining({ answers }));
        });
    ...
    it('omits answers from the payload when updatedInput has none', async () => {
        ...
        await vi.waitFor(() => {
            expect(onConfirm).toHaveBeenCalledWith(ToolConfirmationOutcome.ProceedOnce, { updatedInput: { command: 'ls -a' } });
        });
    });
    it('does not promote a same-named answers field for non-ask_user_question tools', async () => {
        ...
        await vi.waitFor(() => {
            expect(onConfirm).toHaveBeenCalledWith(ToolConfirmationOutcome.ProceedOnce, { updatedInput: { command: 'ls', answers: { '0': 'leak' } } });
        });
        expect(onConfirm).not.toHaveBeenCalledWith(ToolConfirmationOutcome.ProceedOnce, expect.objectContaining({ answers: expect.anything() }));
    });
    it.each([
    ...
    ])('omits answers from the payload when %s', async (_desc, updatedInput, expectedAnswers) => {
        const context = createContext(120_000);
        const controller = new PermissionController(context, createRegistry(), 'PermissionController');
        vi.spyOn(controller, 'sendControlRequest').mockResolvedValue({
            subtype: 'success',
        ...
        const isPlainObject = updatedInput !== null &&
    ...
    it('calls onConfirm with Cancel when sendControlRequest rejects', async () => {
        const context = createContext(120_000);
        const controller = new PermissionController(context, createRegistry(), 'PermissionController');
        vi.spyOn(controller, 'sendControlRequest').mockRejectedValue(new Error('Request timeout'));
        const onConfirm = vi.fn();
        controller.getToolCallUpdateCallback()([
            {
                status: 'awaiting_approval',
    ...
    it('forwards ask_user_question answers to a teammate approval', async () => {
        ...
        expect(respond).toHaveBeenCalledWith(ToolConfirmationOutcome.ProceedOnce, expect.objectContaining({ answers }));
    });
    it('does not promote a same-named answers field for a non-ask_user_question teammate approval', async () => {
        ...
        expect(respond).not.toHaveBeenCalledWith(ToolConfirmationOutcome.ProceedOnce, expect.objectContaining({ answers: expect.anything() }));
    });
    it('confirms a teammate approval with no payload when updatedInput is absent', async () => {
        ...
        expect(respond).toHaveBeenCalledWith(ToolConfirmationOutcome.ProceedOnce, undefined);
    });
    it('includes teammate Plan shell warnings in permission suggestions', async () => {
        const controller = new PermissionController(createContext(120_000), createRegistry(), 'PermissionController');
        ...
        expect(send).toHaveBeenCalledWith(expect.objectContaining({
            permission_suggestions: expect.arrayContaining([
                expect.objectContaining({
                    type: 'allow',
                    description: expect.stringContaining('Exact one-off approval required'),
                }),
            ]),
        }), undefined, expect.any(AbortSignal));
    });
    it('omits modify suggestions when edit confirmation hides modify actions', () => {
        const controller = new PermissionController(createContext(), createRegistry(), 'PermissionController');
...
//# sourceMappingURL=permissionController.test.js.map