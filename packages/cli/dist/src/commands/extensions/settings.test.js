...
describe('extensions settings command', () => {
    it('should fail if no subcommand is provided', () => {
        const validationParser = yargs([])
            ...
            .fail(false)
            .locale('en');
        expect(() => validationParser.parse('settings')).toThrow('Not enough non-option arguments: got 0, need at least 1');
    });
    it('should register set subcommand', () => {
        const parser = yargs([]).command(settingsCommand).fail(false).locale('en');
        expect(() => parser.parse('settings set')).toThrow('Not enough non-option arguments');
    });
    it('should register list subcommand', () => {
        const parser = yargs([]).command(settingsCommand).fail(false).locale('en');
        expect(() => parser.parse('settings list')).toThrow('Not enough non-option arguments');
    });
    it('should accept set command with name and setting', () => {
        const parser = yargs([]).command(settingsCommand).fail(false).locale('en');
        expect(() => parser.parse('settings set my-extension API_KEY')).not.toThrow();
    });
    it('should accept set command with scope option', () => {
        const parser = yargs([]).command(settingsCommand).fail(false).locale('en');
        expect(() => parser.parse('settings set my-extension API_KEY --scope=workspace')).not.toThrow();
    });
    it('should fail set command with invalid scope', () => {
        const parser = yargs([]).command(settingsCommand).fail(false).locale('en');
        expect(() => parser.parse('settings set my-extension API_KEY --scope=invalid')).toThrow();
    });
...
describe('settings set handler', () => {
    ...
    it('should return early if extension manager is not available', async () => {
        ...
        vi.mocked(getExtensionManager).mockResolvedValueOnce(null);
    ...
    it('should log error if extension is not found', async () => {
        ...
        const parser = yargs([]).command(settingsCommand);
        await parser.parseAsync('settings set my-extension API_KEY');
        expect(mockWriteStdoutLine).toHaveBeenCalledWith('Extension "my-extension" not found.');
        expect(mockUpdateSetting).not.toHaveBeenCalled();
    });
    it('should call updateSetting with correct arguments for user scope', async () => {
        ...
        const parser = yargs([]).command(settingsCommand);
        await parser.parseAsync('settings set my-extension API_KEY');
        expect(mockUpdateSetting).toHaveBeenCalledWith(mockExtension.config, mockExtension.id, 'API_KEY', mockPromptForSetting, 'user');
    });
    it('should call updateSetting with workspace scope when specified', async () => {
        ...
        mockGetLoadedExtensions.mockReturnValueOnce([mockExtension]);
        const parser = yargs([]).command(settingsCommand);
        await parser.parseAsync('settings set my-extension API_KEY --scope=workspace');
        expect(mockUpdateSetting).toHaveBeenCalledWith(mockExtension.config, mockExtension.id, 'API_KEY', mockPromptForSetting, 'workspace');
    });
});
describe('settings list handler', () => {
    ...
    it('should log message if extension has no settings', async () => {
        ...
        const parser = yargs([]).command(settingsCommand);
        await parser.parseAsync('settings list my-extension');
        expect(mockWriteStdoutLine).toHaveBeenCalledWith('Extension "my-extension" has no settings to configure.');
    });
    it('should list settings with their values', async () => {
        ...
        const parser = yargs([]).command(settingsCommand);
        await parser.parseAsync('settings list my-extension');
        expect(mockWriteStdoutLine).toHaveBeenCalledWith('Settings for "my-extension":');
        expect(mockWriteStdoutLine).toHaveBeenCalledWith('\n- API Key (API_KEY)');
        expect(mockWriteStdoutLine).toHaveBeenCalledWith('  Description: Your API key');
        expect(mockWriteStdoutLine).toHaveBeenCalledWith('  Value: my-api-key (user)');
    });
    it('should show workspace scope for workspace-scoped settings', async () => {
        ...
        expect(mockWriteStdoutLine).toHaveBeenCalledWith('  Value: workspace-value (workspace)');
    });
    ...
    it('should show [value stored in keychain] for sensitive settings', async () => {
        ...
        const parser = yargs([]).command(settingsCommand);
        await parser.parseAsync('settings list my-extension');
        expect(mockWriteStdoutLine).toHaveBeenCalledWith('  Value: [value stored in keychain] (user)');
    });
});
