 ...
 * for task prompts.
 */
export function sliceTextByVisualHeight(text, maxHeight, maxWidth, options = {}) {
 ...
 */
export function sanitizeSensitiveText(text, maxLength = 200) {
    let result = text;
 ...
 */
export function sanitizeFilenameForDisplay(name) {
 ...
 */
export function sanitizeMultilineForDisplay(text) {
    return escapeAnsiCtrlCodes(text).replace(MULTILINE_CONTROL_CHARS_REGEX, escapeControlChar);
}
