...
import { TEXT_CACHE_MAX_ENTRIES, __getTextUtilsCacheSizes, clearStringWidthCache, escapeAnsiCtrlCodes, getCachedStringWidth, sanitizeFilenameForDisplay, sanitizeMultilineForDisplay, sanitizeSensitiveText, sliceTextByVisualHeight, toCodePoints, truncateToWidth, } from './textUtils.js';
describe('textUtils', () => {
    ...
    describe('sanitizeFilenameForDisplay', () => {
        it('passes clean filenames through unchanged', () => {
            ...
            expect(sanitizeFilenameForDisplay('packages/cli/src/index.ts')).toBe('packages/cli/src/index.ts');
    ...
    describe('sanitizeMultilineForDisplay', () => {
        it('preserves line structure while escaping other control bytes', () => {
            ...
            expect(sanitizeMultilineForDisplay('a\rb\x07c\x9bd')).toBe('a\\rb\\u0007c\\u009bd');
        });
    ...
    describe('sanitizeSensitiveText', () => {
        ...
        it('should redact OpenAI-style API keys', () => {
            ...
            expect(sanitizeSensitiveText(text)).toBe('Use API key sk-***REDACTED*** for access');
        });
        ...
        it('should redact Bearer tokens', () => {
            ...
            expect(sanitizeSensitiveText(text)).toBe('Authorization: Bearer ***REDACTED***');
        });
        ...
        it('should redact AWS access keys', () => {
            ...
            expect(sanitizeSensitiveText(text)).toBe('AWS_ACCESS_KEY_ID=***REDACTED***');
        });
        ...
        it('should handle custom max length', () => {
            const text = 'This is a test prompt with sk-1234567890abcdefghijklmnopqrstuv';
        ...
        it('should redact multiple sensitive patterns', () => {
            const text = 'api_key=secretkey12345678901234 and password=mypass123 and sk-test123456789012345678901';
    ...
    describe('text cache bounds', () => {
        ...
        it('bounds the code points cache and keeps results correct', () => {
            const probe = (i) => `cache-bound-probe-${i}-é`;
            for (let i = 0; i < TEXT_CACHE_MAX_ENTRIES + 50; i++) {
                toCodePoints(probe(i));
            }
            expect(TEXT_CACHE_MAX_ENTRIES).toBe(500);
            expect(__getTextUtilsCacheSizes().codePoints).toBe(TEXT_CACHE_MAX_ENTRIES);
        ...
        it('bounds the string width cache and keeps widths correct', () => {
            const probe = (i) => `width-bound-probe-${i}-é`;
            for (let i = 0; i < TEXT_CACHE_MAX_ENTRIES + 50; i++) {
                getCachedStringWidth(probe(i));
            }
            expect(__getTextUtilsCacheSizes().stringWidth).toBe(TEXT_CACHE_MAX_ENTRIES);
