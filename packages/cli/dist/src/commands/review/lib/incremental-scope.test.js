...
function selectionOf(fullPaths, deltaPaths) {
    const sel = selectNarrowing(Buffer.from(fullPaths.map(section).join(''), 'utf8'), Buffer.from(deltaPaths.map(section).join(''), 'utf8'));
    if (sel === null)
...
describe('widenScope', () => {
    it('pulls in a still-clean importer, and publishes its section', () => {
        ...
        const selection = selectionOf(['src/changed.ts', 'src/imp.ts', 'src/other.ts'], ['src/changed.ts']);
        const sources = {
    ...
    it('returns exactly the narrowing when nothing imports what moved', () => {
        ...
        const selection = selectionOf(['src/changed.ts', 'src/other.ts'], ['src/changed.ts']);
        const { paths, scope } = widenScope({
            anchor: 'a'.repeat(40),
            selection,
        ...
        expect(scope.interaction).toEqual([]);
        expect(scope.contextFileCount).toBe(1);
        expect(assembleSections(selection, paths)?.toString('utf8')).toBe(assembleSections(selection, selection.touched)?.toString('utf8'));
    });
    it('does not follow a test file into scope', () => {
        ...
        const selection = selectionOf(['src/changed.ts', 'src/changed.test.ts'], ['src/changed.ts']);
        const { paths, scope } = widenScope({
            anchor: 'a'.repeat(40),
            selection,
...
//# sourceMappingURL=incremental-scope.test.js.map