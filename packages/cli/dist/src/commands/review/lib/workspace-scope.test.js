...
const p = (dir, name, deps = [], scripts = ['test']) => ({ dir, name, scripts, deps });
...
describe('resolveTestScope', () => {
    ...
    it('names the broken graph FIRST when caveats compose — nothing is silent', () => {
        ...
        expect(scope.caveat.indexOf('packages/broken')).toBeLessThan(scope.caveat.indexOf('scripts/x.js'));
    });
    ...
    it('counts a RUNNING root suite on both sides of the half cap', () => {
        ...
        expect(scope.caveat).toContain('3 of 5 testable suites (including the root)');
        expect(scope.caveat).toContain('more than half');
    });
    ...
    it('keeps the broken-graph caveat ahead of the half-cap one when both apply', () => {
        ...
        expect(scope.caveat).toContain('packages/broken');
        expect(scope.caveat).toContain('more than half');
        expect(scope.caveat.indexOf('does not parse')).toBeLessThan(scope.caveat.indexOf('more than half'));
    });
...
//# sourceMappingURL=workspace-scope.test.js.map