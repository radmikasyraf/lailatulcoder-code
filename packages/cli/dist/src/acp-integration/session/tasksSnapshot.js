...
function serializeAgentTask(entry, now) {
    return {
        kind: 'agent',
        id: entry.id,
...
function serializeShellTask(entry, now) {
    return {
        kind: 'shell',
        id: entry.id,
        label: entry.command,
...
function serializeMonitorTask(entry, now) {
    return {
        kind: 'monitor',
        id: entry.id,
...
export function buildSessionTasksStatus(sessionId, config, now = Date.now()) {
    const tasks = [
        ...config
...
//# sourceMappingURL=tasksSnapshot.js.map