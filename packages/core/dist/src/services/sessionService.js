...
import { deserializeSnapshots, FILE_HISTORY_DIR, serializeSnapshot, } from './fileHistoryService.js';
...
import { LITE_READ_BUF_SIZE, readLastJsonStringFieldSync, readSessionTitleInfoFromFileSync, } from '../utils/sessionStorageUtils.js';
import { isSessionArtifactRecord, rebuildSessionArtifactSnapshot, remapSessionArtifactPayloadForFork, selectActiveSideArtifactRecordUuids, } from './session-artifact-persistence.js';
import { SessionOrganizationService } from './session-organization-service.js';
import { SessionTranscriptReader, SessionTranscriptTooLargeError, } from './session-transcript-reader.js';
import { SessionWriterLease, SessionWriterUnavailableError, } from './session-writer-lease.js';
import { parseBranchCheckpointPayload, resolveBranchPoints, } from './branch-points.js';
export { buildApiHistoryFromConversation, } from './session-api-history.js';
import { getResumeTokenCounts, } from './session-resume-token-counts.js';
export { getResumePromptTokenCount, getResumeTokenCounts, } from './session-resume-token-counts.js';
...
export class SessionIdCaseConflictError extends Error {
    ...
    constructor(sessionId, candidateSessionId, reason = 'case_conflict') {
        super(reason === 'unreadable_transcript'
...
async function fsyncDirectoryBestEffort(directory) {
    try {
...
function validatedBackupPath(directory, name) {
...
async function copyFileHistoryBackupsToStaging(sourceSessionId, targetDirectory, backupNames, onMissing) {
    const copiedNames = new Set();
    if (backupNames.size === 0)
        return copiedNames;
    const sourceDir = path.join(Storage.getGlobalQwenDir(), FILE_HISTORY_DIR, sourceSessionId);
    ...
    for (const name of backupNames) {
        ...
        let sourceHandle;
        try {
            sourceHandle = await fs.promises.open(source, fs.constants.O_RDONLY |
        ...
        try {
            ...
            try {
                let sourceOffset = 0;
                while (true) {
                    const { bytesRead } = await sourceHandle.read(copyBuffer, 0, copyBuffer.length, sourceOffset);
                    if (bytesRead === 0)
                        break;
                    let written = 0;
 ...
 * File location: ~/.lailatulcoder/tmp/<project_id>/chats/
 */
export class SessionService {
    ...
    constructor(cwd, options = {}) {
        ...
        this.transcriptReader = new SessionTranscriptReader(cwd, undefined, options.runtimeBaseDir);
    }
    ...
    async acquireSessionWriterLease(sessionId, options) {
    ...
    getChatsDirForState(state) {
        return state === 'archived'
            ? this.getArchiveChatsDir()
            : this.getChatsDir();
    }
    getSessionFilePath(sessionId, state) {
        return path.join(this.getChatsDirForState(state), `${sessionId}.jsonl`);
    }
    getWorktreeSessionPathForState(sessionId, state) {
        return path.join(this.getChatsDirForState(state), `${sessionId}.worktree.json`);
    }
    async sessionBelongsToCurrentProject(sessionId, recordCwd, signal) {
        signal?.throwIfAborted();
        if (getProjectHash(recordCwd) === this.projectHash) {
            return true;
        }
        // Worktree sessions record cwd as the worktree path
        // (<repoRoot>/.lailatulcoder/worktrees/<slug>), which has a different project
        ...
        // /repo/.lailatulcoder/worktrees/parent/.lailatulcoder/worktrees/child, the innermost
        // marker gives repoRoot = /repo/.lailatulcoder/worktrees/parent (the workspace).
        const worktreesMarker = `${path.sep}.lailatulcoder${path.sep}worktrees${path.sep}`;
    ...
    getPromptLedgerPathForState(sessionId, state) {
        return path.join(this.getChatsDirForState(state), `${sessionId}.ledger.jsonl`);
    }
    /**
     * Returns the absolute path to the per-session prompt terminal ledger
    ...
    getWorktreeSessionPathForArchiveState(sessionId, state) {
        return this.getWorktreeSessionPathForState(sessionId, state);
    }
    async readProjectSessionHead(sessionId, filePath) {
        try {
            ...
            if (!(await this.sessionBelongsToCurrentProject(sessionId, firstRecord.cwd))) {
                return undefined;
            }
            return firstRecord;
        }
     ...
     */
    async readParentSessionId(sessionId) {
    ...
    async readCreationMetadata(sessionId) {
        return ((await this.readCreationMetadataInternal(sessionId, ['active', 'archived'], false)) ?? {});
    }
    ...
    async readCreationMetadataInternal(sessionId, states, requireCompleteLines) {
        for (const state of states) {
            const filePath = this.getSessionFilePath(sessionId, state);
            try {
                let records;
                if (requireCompleteLines) {
                    const result = await jsonl.readLinesWithIntegrity(filePath, MAX_PROMPT_SCAN_LINES);
                    if (!result.complete)
                        continue;
                    records = result.records;
                }
                else {
                    records = await jsonl.readLines(filePath, MAX_PROMPT_SCAN_LINES);
                }
                if (records.length === 0)
                    continue;
                if (!(await this.sessionBelongsToCurrentProject(records[0].sessionId, records[0].cwd))) {
                    continue;
                }
                ...
                if (error.code === 'ENOENT')
                    continue;
                this.warn(`readCreationMetadata: failed to read ${sessionId}: ${error}`);
            }
        }
        return undefined;
    }
    async getSessionLocation(sessionId) {
        ...
        const [active, archived] = await Promise.all([
            this.readProjectSessionHead(sessionId, this.getSessionFilePath(sessionId, 'active')),
            this.readProjectSessionHead(sessionId, this.getSessionFilePath(sessionId, 'archived')),
        ]);
        if (active && archived)
            return 'conflict';
        if (active)
     ...
     */
    async findSessionIdIgnoringCase(sessionId) {
        ...
        if (readable.length === 1)
            return readable[0].candidateSessionId;
        if (readable.length > 1) {
            ...
            // conflict.
            const aliased = this.resolveAliasedReadableCandidate(readable, candidates);
        ...
        for (const [candidateSessionId, states] of candidates) {
            ...
            if (occupyingSpelling !== undefined)
                break;
        }
        if (occupyingSpelling === undefined)
            return undefined;
        ...
        candidates.size === 1 ? occupyingSpelling : undefined, 'unreadable_transcript');
    }
    /**
     ...
     */
    resolveAliasedReadableCandidate(readable, candidates) {
    ...
    removeWorktreeSidecars(sessionId) {
        for (const state of ['active', 'archived']) {
    ...
    removeFileHistoryBackups(sessionId) {
        fs.rmSync(path.join(Storage.getGlobalQwenDir(), FILE_HISTORY_DIR, sessionId), { recursive: true, force: true });
    }
    async removeSessionOrganization(sessionId) {
        try {
            ...
            this.warn(`removeSession: failed to clear session organization for ${sessionId}: ${error}`);
        }
    }
    async removeSessionOrganizations(sessionIds) {
        ...
        try {
            ...
            this.warn(`removeSessions: failed to clear session organization for ${sessionIds.join(', ')}: ${error}`);
        }
    }
    moveOptionalFile(sourcePath, targetPath) {
     ...
     */
    moveLedgerSidecar(sourcePath, destinationPath) {
        if (!fs.existsSync(sourcePath)) {
            return;
        }
    ...
    sessionFileMoveError(action, error) {
     ...
     */
    extractCreationMetadataFromRecords(records) {
        let parentSessionId;
        let sourceType;
        let sourceId;
    ...
    async countSessionMessages(sessionId) {
        ...
        try {
            ...
            if (firstRecords.length === 0)
                return 0;
            if (!(await this.sessionBelongsToCurrentProject(sessionId, firstRecords[0].cwd))) {
                return 0;
            }
        }
        catch {
    ...
    async listSessions(options = {}) {
        ...
        for (const file of files) {
            ...
            const records = signal
                ? await jsonl.readLines(filePath, MAX_PROMPT_SCAN_LINES, {
                    signal,
                })
                : await jsonl.readLines(filePath, MAX_PROMPT_SCAN_LINES);
            signal?.throwIfAborted();
            if (records.length === 0)
                continue;
            const firstRecord = records[0];
            if (!(await this.sessionBelongsToCurrentProject(firstRecord.sessionId, firstRecord.cwd, signal))) {
                continue;
            }
    ...
    async countSessionsInState(archiveState) {
        const chatsDir = this.getChatsDirForState(archiveState);
        let fileNames;
        try {
        ...
        for (const name of fileNames) {
            ...
            try {
                ...
                const firstRecord = records[0];
                if (!(await this.sessionBelongsToCurrentProject(firstRecord.sessionId, firstRecord.cwd))) {
                    continue;
                }
                count++;
            }
     ...
     */
    reconstructHistory(records, opts) {
        if (records.length === 0)
        ...
        return {
            messages: prepared.records.map((record) => record),
    ...
    async readRestoreProjection(sessionId, options) {
        return this.transcriptReader.readRestoreProjection(sessionId, options, {
            validateFirstRecord: (record) => this.sessionBelongsToCurrentProject(record.sessionId, record.cwd),
        });
    }
    async readLiveRestoreProjection(sessionId, options) {
     ...
     */
    async loadArchivedSession(sessionId, options) {
        if (!SESSION_FILE_PATTERN.test(`${sessionId}.jsonl`)) {
            return undefined;
        }
        ...
        try {
            ...
            if (stats.size > options.maxBytes) {
                throw new SessionTranscriptTooLargeError(sessionId, stats.size, options.maxBytes);
            }
        }
        catch (error) {
    ...
    async loadSessionFromState(sessionId, state, stats) {
        const filePath = this.getSessionFilePath(sessionId, state);
        ...
        if (gaps.length > 0) {
            debugLogger.warn(`loadSession: detected ${gaps.length} unrecoverable history gap(s) ` +
                `for session ${sessionId}: ` +
                gaps
                    .map((g) => `child=${g.childUuid} missingParent=${g.missingParentUuid}`)
                    .join('; '));
        }
        ...
        for (const msg of messages) {
            try {
                ...
                debugLogger.warn(`loadSession: skipping malformed file_history_snapshot: ${e}`);
            }
        }
        const fileHistorySnapshots = fileHistoryAccumulator.finish();
        const activeBranchRecords = includeActiveSideArtifactRecords(records, messages);
        const artifactSnapshot = rebuildSessionArtifactSnapshot(activeBranchRecords, firstRecord.sessionId);
        return {
            conversation,
            filePath,
     ...
     */
    async salvageUsageBestEffort(transcriptPath) {
        try {
            await persistUsageBeforeTranscriptDeletion(transcriptPath);
        }
        catch (error) {
            this.warn(`usage salvage failed for ${transcriptPath}: ${error}; deleting anyway`);
        }
    }
    async removeSessionFiles(sessionId) {
        ...
        try {
            ...
            const archivedPath = this.getSessionFilePath(sessionId, 'archived');
            const archived = await this.readProjectSessionHead(sessionId, archivedPath);
            if (!archived) {
                return false;
            }
            await this.salvageUsageBestEffort(archivedPath);
    ...
    async archiveSessions(sessionIds, options = {}) {
        ...
        for (const sessionId of [...new Set(sessionIds)]) {
            try {
                ...
                const activeSidecar = this.getWorktreeSessionPathForState(sessionId, 'active');
                const archivedSidecar = this.getWorktreeSessionPathForState(sessionId, 'archived');
                const activeLedger = this.getPromptLedgerPathForState(sessionId, 'active');
                const archivedLedger = this.getPromptLedgerPathForState(sessionId, 'archived');
                try {
                    fs.renameSync(sourcePath, targetPath);
                }
                catch (error) {
                ...
                try {
                    ...
                    this.warn(`archiveSessions: failed to move worktree sidecar for ${sessionId} from ${activeSidecar} to ${archivedSidecar}: ${sidecarError}`);
                }
                try {
                    this.moveLedgerSidecar(activeLedger, archivedLedger);
                }
                catch (ledgerError) {
                    this.warn(`archiveSessions: failed to move prompt ledger for ${sessionId} from ${activeLedger} to ${archivedLedger}: ${ledgerError}`);
                }
        ...
        for (const sessionId of [...new Set(sessionIds)]) {
            try {
                ...
                try {
                    ...
                    this.warn(`unarchiveSessions: failed to move worktree sidecar for ${sessionId} from ${archivedSidecar} to ${activeSidecar}: ${sidecarError}`);
                }
                const archivedLedger = this.getPromptLedgerPathForState(sessionId, 'archived');
                const activeLedger = this.getPromptLedgerPathForState(sessionId, 'active');
                try {
                    ...
                    this.warn(`unarchiveSessions: failed to move prompt ledger for ${sessionId} from ${archivedLedger} to ${activeLedger}: ${ledgerError}`);
                }
    ...
    async removeSessions(sessionIds) {
        ...
        const results = await Promise.allSettled(uniqueSessionIds.map((sessionId) => this.removeSessionFiles(sessionId)));
     ...
     */
    async renameSession(sessionId, title, titleSource = 'manual', archiveState = 'active') {
        if (!SESSION_FILE_PATTERN.test(`${sessionId}.jsonl`)) {
            return false;
        }
        ...
        try {
            ...
            if (!(await this.sessionBelongsToCurrentProject(sessionId, records[0].cwd))) {
                return false;
            }
     ...
     * `sessionExistsInAnyState()` for archive-aware lookups.
     */
    async forkSession(sourceSessionId, newSessionId, options = {}) {
        ...
        if (!(await this.sessionBelongsToCurrentProject(sourceSessionId, records[0].cwd))) {
            throw new Error(`Source session does not belong to current project: ${sourceSessionId}`);
        }
        const { messages: currentActiveMessages } = this.reconstructHistory(records);
        let boundedRecords = records;
        let activeMessages = currentActiveMessages;
        if (options.atRecordId !== undefined) {
            const point = resolveBranchPoints(currentActiveMessages).get(options.atRecordId);
            if (!point)
            ...
            const matchingIndex = records.findIndex((record) => record.uuid === options.atRecordId);
            if (matchingIndex < 0) {
                throw new BranchPointInvalidError(options.atRecordId);
            }
        ...
        const preFilterRecords = includeActiveSideArtifactRecords(boundedRecords, activeMessages);
        ...
        // message.
        const sourceRecord = options.source
            ? {
                uuid: randomUUID(),
                parentUuid: null,
            ...
            : undefined;
        let prevUuid = sourceRecord?.uuid ?? null;
        const remappedArtifactIds = new Map();
        ...
        const nearestRetainedPredecessor = new Map();
        {
        ...
        const forked = [
            ...
            ...sourceRecords.map((record) => {
                ...
                let systemPayload = remapSystemPayloadForFork(record, sourceSessionId, newSessionId, remappedArtifactIds);
        ...
        const sourceSession = options.atRecordId === undefined
            ? await this.loadSession(sourceSessionId)
            : undefined;
        const existingSnapshotPromptIds = collectFileHistorySnapshotPromptIds(forked);
        const remappedSnapshots = sourceSession?.fileHistorySnapshots
            ?.map((snapshot) => remapSnapshotPromptId(snapshot, sourceSessionId, newSessionId))
        ...
        const stagedTranscriptPath = path.join(chatsDir, `.${newSessionId}.${operationId}.tmp`);
        const backupRoot = path.join(Storage.getGlobalQwenDir(), FILE_HISTORY_DIR);
        const stagedBackupPath = path.join(backupRoot, `.${newSessionId}.${operationId}.tmp`);
        ...
        try {
            ...
            const stagedHandle = await fs.promises.open(stagedTranscriptPath, 'wx', 0o600);
            try {
            ...
            if (backupNames.size > 0) {
                const copiedBackupNames = await copyFileHistoryBackupsToStaging(sourceSessionId, stagedBackupPath, backupNames, (name) => {
                    this.warn(`branch ${newSessionId} omitted missing file-history backup ${name}; rewind data for that snapshot is unavailable`);
                });
            ...
            try {
                ...
                this.warn(`branch committed session=${newSessionId} but final durability confirmation failed: ${error instanceof Error ? error.message : String(error)}`);
            }
        }
        finally {
            ...
            if (cleanupFailures.length > 0) {
                this.warn(`branch cleanup incomplete session=${newSessionId}: ${cleanupFailures.join('; ')}`);
            }
        }
    ...
    async sessionExists(sessionId, options = {}) {
        ...
        try {
            const records = options.signal
                ? await jsonl.readLines(filePath, 1, options)
                : await jsonl.readLines(filePath, 1);
            options.signal?.throwIfAborted();
            if (records.length === 0) {
                return false;
            }
            const belongsToCurrentProject = await this.sessionBelongsToCurrentProject(sessionId, records[0].cwd, options.signal);
            options.signal?.throwIfAborted();
    ...
    async sessionExistsInAnyState(sessionId) {
        try {
            const location = await this.getSessionLocation(sessionId);
...
function remapSnapshotPromptId(snapshot, sourceSessionId, newSessionId) {
...
function remapFileHistorySnapshotPayload(payload, sourceSessionId, newSessionId) {
    const filePayload = payload;
    if (!Array.isArray(filePayload?.snapshots))
        return payload;
    try {
        const snapshots = deserializeSnapshots(filePayload.snapshots).map((snapshot) => serializeSnapshot(remapSnapshotPromptId(snapshot, sourceSessionId, newSessionId)));
        return { snapshots };
    }
    catch {
        return payload;
...
function remapSystemPayloadForFork(record, sourceSessionId, newSessionId, remappedArtifactIds) {
    if (record.type !== 'system')
        return record.systemPayload;
    if (record.subtype === 'file_history_snapshot') {
        return remapFileHistorySnapshotPayload(record.systemPayload, sourceSessionId, newSessionId);
    }
    if (record.subtype === 'session_artifact_event' ||
        record.subtype === 'session_artifact_snapshot') {
        return remapSessionArtifactPayloadForFork(record.systemPayload, sourceSessionId, newSessionId, remappedArtifactIds);
    }
    return record.systemPayload;
}
function includeActiveSideArtifactRecords(records, activeRecords) {
    const activeByUuid = new Map(activeRecords.map((record) => [record.uuid, record]));
    const artifactUuids = new Set(selectActiveSideArtifactRecordUuids(records, activeRecords.map((record) => record.uuid)));
    const selected = [];
    for (const record of records) {
        ...
        if (artifactUuids.has(record.uuid))
            selected.push(record);
    }
    return selected;
}
function collectFileHistorySnapshotPromptIds(records) {
    const ids = new Set();
    for (const record of records) {
        if (record.type !== 'system' ||
...
function collectReferencedFileHistoryBackupNames(records) {
    const names = new Set();
    for (const record of records) {
        if (record.type !== 'system' ||
            record.subtype !== 'file_history_snapshot' ||
 ...
 */
export function replayUiTelemetryFromConversation(conversation, sessionId) {
    if (sessionId) {
...
export async function computeUniqueBranchTitle(baseName, sessionService) {
    const maxSuffixLen = ' (Branch 1234567890123)'.length;
    const trimmed = baseName
        .trim()
        .slice(0, SESSION_TITLE_MAX_LENGTH - maxSuffixLen);
    const taken = new Set((await sessionService.findSessionTitlesByPrefix(`${trimmed} (Branch`)).map((t) => t.toLowerCase().trim()));
