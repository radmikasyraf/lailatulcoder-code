...
import { assembleSystemPrompt, getCoreSystemPrompt, } from '../../core/prompts.js';
...
import { ArenaSessionStatus, ARENA_MAX_AGENTS, safeAgentId, } from './types.js';
import { AgentStatus, isTerminalStatus, isSettledStatus, isSuccessStatus, } from '../runtime/agent-types.js';
import { logArenaSessionStarted, logArenaAgentCompleted, logArenaSessionEnded, makeArenaSessionStartedEvent, makeArenaAgentCompletedEvent, makeArenaSessionEndedEvent, } from '../../telemetry/index.js';
import { buildFallbackApproachSummary, summarizeUnifiedDiff, } from './diff-summary.js';
...
export class ArenaManager {
    ...
    constructor(config, callbacks = {}) {
        ...
        // Use the user-configured base dir, or default to ~/.lailatulcoder/arena.
        this.arenaBaseDir =
            arenaSettings?.worktreeBaseDir ??
                path.join(Storage.getGlobalQwenDir(), 'arena');
        this.worktreeService = new GitWorktreeService(config.getWorkingDir(), this.arenaBaseDir);
     ...
     */
    getAgentSnapshot(agentId, scrollOffset = 0) {
     ...
     */
    getAgentScrollbackLength(agentId) {
    ...
    async start(options) {
        ...
        this.arenaConfig = {
            ...
            maxRoundsPerAgent: options.maxRoundsPerAgent ?? arenaSettings?.maxRoundsPerAgent,
        ...
        debugLogger.info(`Models: ${options.models.map((m) => m.modelId).join(', ')}`);
        ...
        if (!isRepo) {
            throw new Error('Failed to start arena: current directory is not a git repository.');
        }
        ...
        logArenaSessionStarted(this.config, makeArenaSessionStartedEvent({
        ...
        try {
            ...
            const displayMode = options.displayMode ??
                this.config.getAgentsSettings().displayMode;
            ...
            const worktreeInfo = Array.from(this.agents.values())
                .map((agent, i) => `  ${i + 1}. ${agent.model.modelId} → ${agent.worktree.path}`)
                .join('\n');
            ...
            this.callbacks.onArenaError?.(error instanceof Error ? error : new Error(errorMessage));
            throw error;
        }
    }
    /**
    ...
    async cleanup() {
        ...
        this.emitSessionEnded(this.sessionStatus === ArenaSessionStatus.CANCELLED
            ? 'cancelled'
            : 'discarded');
    ...
    async cleanupRuntime() {
        ...
        debugLogger.info(`Cleaning up Arena runtime (preserving artifacts): ${this.sessionId}`);
        // If no session-ended event was emitted yet, emit before tearing down.
        this.emitSessionEnded(this.sessionStatus === ArenaSessionStatus.CANCELLED
            ? 'cancelled'
            : 'discarded');
     ...
     */
    async applyAgentResult(agentId) {
        ...
        const applyResult = await this.worktreeService.applyWorktreeChanges(agent.worktree.path);
     ...
     */
    emitSessionEnded(status, winnerModelId) {
        if (this.sessionEndedLogged)
            return;
        this.sessionEndedLogged = true;
        const agents = Array.from(this.agents.values());
        logArenaSessionEnded(this.config, makeArenaSessionEndedEvent({
            arena_session_id: this.sessionId ?? '',
            status,
            duration_ms: this.startedAt ? Date.now() - this.startedAt : 0,
            display_backend: this.backend?.type,
            agent_count: agents.length,
            completed_agents: agents.filter((a) => a.status === AgentStatus.COMPLETED).length,
            failed_agents: agents.filter((a) => a.status === AgentStatus.FAILED)
                .length,
            cancelled_agents: agents.filter((a) => a.status === AgentStatus.CANCELLED).length,
    ...
    validateStartOptions(options) {
        ...
        if (uniqueSafeIds.size !== safeIds.length) {
            const collisions = modelIds.filter((id, i) => safeIds.indexOf(safeIds[i]) !== i);
            throw new Error(`Arena model IDs collide after path normalization: ${collisions.join(', ')}. ` +
     ...
     */
    async initializeBackend(displayMode) {
     ...
     */
    async deriveWorktreeDirName(sessionId) {
    ...
    async setupWorktrees() {
        ...
        for (let i = 0; i < this.arenaConfig.models.length; i++) {
            ...
            if (!worktree) {
                throw new Error(`No worktree created for model ${model.modelId} (name: ${worktreeName})`);
            }
    ...
    async runAgents() {
        ...
        if (isInProcess) {
            this.setupInProcessEventBridge(backend);
        }
        else {
            this.startPolling();
        }
        ...
        const allSettled = await this.waitForAllAgentsSettled(timeoutSeconds ? timeoutSeconds * 1000 : undefined);
    ...
    handleAgentExit(agentId, exitCode, _signal) {
        const agent = this.agents.get(agentId);
        if (!agent) {
            return;
        }
        ...
        this.updateAgentStatus(agentId, agent.abortController.signal.aborted
    ...
    buildAgentSpawnConfig(agent) {
                ...
                runtimeConfig: {
                    promptConfig: {
                        ...
                        systemPrompt: assembleSystemPrompt({
                            base: getCoreSystemPrompt(undefined, model.modelId, undefined, 'headless'),
        ...
        debugLogger.info(`[buildAgentSpawnConfig] agentId=${agentId}, command=${spawnConfig.command}, cliEntry=${process.argv[1]}, resolvedEntry=${path.resolve(process.argv[1])}`);
        debugLogger.info(`[buildAgentSpawnConfig] args=${JSON.stringify(spawnConfig.args)}`);
        debugLogger.info(`[buildAgentSpawnConfig] cwd=${spawnConfig.cwd}, env keys=${Object.keys(env).join(',')}`);
        return spawnConfig;
    }
    // ─── Private: Status & Results ─────────────────────────────────
    /** Decide whether a status transition is valid. Returns the new status or null. */
    resolveTransition(current, incoming) {
        if (current === incoming)
            return null;
    ...
    updateAgentStatus(agentId, newStatus, options) {
        const agent = this.agents.get(agentId);
        if (!agent) {
            return;
        }
        ...
        if (this.sessionStatus === ArenaSessionStatus.IDLE) {
            ...
            else if (previousStatus === AgentStatus.RUNNING &&
                newStatus === AgentStatus.IDLE) {
                if (options?.roundCancelledByUser) {
                    this.emitProgress(`Agent ${label} is cancelled by user.`, 'warning');
                }
        ...
        if (isTerminalStatus(newStatus)) {
            ...
            const agentTelemetryStatus = newStatus === AgentStatus.COMPLETED
                ? 'completed'
                : newStatus === AgentStatus.FAILED
                    ? 'failed'
                    : 'cancelled';
            logArenaAgentCompleted(this.config, makeArenaAgentCompletedEvent({
                arena_session_id: this.sessionId ?? '',
     ...
     * `~/.lailatulcoder/worktrees/<sessionId>/`.  The directory contains:
    ...
    getArenaSessionDir() {
        ...
        return GitWorktreeService.getSessionDir(this.worktreeDirName, this.arenaBaseDir);
    }
     ...
     */
    waitForAllAgentsSettled(timeoutMs) {
     ...
     */
    setupInProcessEventBridge(backend) {
        for (const agent of this.agents.values()) {
            ...
            const syncStats = () => {
                const { totalToolCalls, totalDurationMs, ...rest } = interactive.getStats();
            ...
            const applyStatus = (incoming, options) => {
            ...
            const onStatusChange = (event) => {
                ...
                this.flushInProcessStatusFiles().catch((err) => debugLogger.error('Failed to flush in-process status files:', err));
            };
    ...
    async pollAgentStatuses() {
        ...
        for (const agent of this.agents.values()) {
            ...
            try {
                const statusPath = path.join(agentsDir, `${safeAgentId(agent.agentId)}.json`);
                const content = await fs.readFile(statusPath, 'utf-8');
                const statusFile = JSON.parse(content);
                ...
                const resolved = this.resolveTransition(agent.status, statusFile.status);
                if (resolved) {
                ...
                debugLogger.error(`Error reading status for agent ${agent.agentId}:`, error);
            }
        }
     ...
     */
    async writeConsolidatedStatus(agents) {
        const sessionDir = this.getArenaSessionDir();
        ...
        try {
            ...
            try {
                ...
                config = {
                    ...
                    worktreeNames: arenaConfig.models.map((m) => m.displayName || m.modelId),
            ...
            debugLogger.error('Failed to write consolidated status to config.json:', error);
        }
    }
    /**
     * Build an ArenaStatusFile snapshot from in-memory agent state.
     */
    buildStatusFile(agent) {
        return {
            agentId: agent.agentId,
            status: agent.status,
            updatedAt: Date.now(),
     ...
     */
    async flushInProcessStatusFiles() {
        const sessionDir = this.getArenaSessionDir();
        const agentsDir = path.join(sessionDir, 'agents');
        ...
        for (const agent of this.agents.values()) {
            ...
            const filePath = path.join(agentsDir, `${safeAgentId(agent.agentId)}.json`);
    ...
    async sendControlSignal(agentId, type, reason) {
        const agent = this.agents.get(agentId);
        if (!agent) {
            debugLogger.error(`Cannot send control signal: agent ${agentId} not found`);
            return;
        }
        const controlSignal = {
            type,
        ...
        try {
            ...
            await fs.writeFile(controlPath, JSON.stringify(controlSignal, null, 2), 'utf-8');
            debugLogger.info(`Sent ${type} control signal to agent ${agentId}: ${reason}`);
        }
        catch (error) {
            debugLogger.error(`Failed to send control signal to agent ${agentId}:`, error);
        }
    }
    getAgentTranscript(agentId) {
        ...
        const interactive = this.backend.getAgent(agentId);
    ...
    getFinalTextFromTranscript(transcript) {
        if (!transcript)
            return undefined;
    ...
    async addApproachSummaries(summaryInputs) {
        await Promise.all(summaryInputs.map(async (summaryInput) => {
            summaryInput.result.approachSummary =
                await this.generateAgentApproachSummary(summaryInput);
        }));
    }
    async generateAgentApproachSummary(summaryInput) {
        ...
        try {
            ...
            return (parseApproachSummaryResponse(response.text)?.trim() ||
                buildFallbackApproachSummary(result));
        }
        catch (error) {
            debugLogger.error(`Failed to generate Arena approach summary for ${result.agentId}:`, error);
            return buildFallbackApproachSummary(result);
        }
    }
    buildAgentApproachSummaryPrompt({ result: agent, transcript, }) {
        const payload = {
            task: this.arenaConfig?.task ?? '',
            instruction: 'Summarize this Arena agent approach for user comparison. Use git diff as the source of truth for what changed. Use transcript/finalText only to infer intent and architectural decisions. Do not pick a winner. Return only compact JSON: {"summary":"one sentence summary"}.',
            agent: {
                agentId: agent.agentId,
        ...
        for (const agent of this.agents.values()) {
            ...
            if (isSuccessStatus(agent.status)) {
                try {
                    ...
                    result.modifiedFiles = result.diffSummary.files.map((file) => file.path);
                }
                catch (error) {
                    debugLogger.error(`Failed to get diff for agent ${agent.agentId}:`, error);
                }
            }
            result.diffSummary ??= summarizeUnifiedDiff(result.diff);
            result.modifiedFiles ??= result.diffSummary.files.map((file) => file.path);
