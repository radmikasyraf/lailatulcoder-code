...
function debugReportError(message, element) {
    ...
    if (!React.isValidElement(element)) {
        debugLogger.error(message, `Invalid element: '${String(element)}' typeof=${typeof element}`);
        return;
    }
 ...
 * </MaxSizedBox>
 */
export const MaxSizedBox = ({ children, maxWidth, maxHeight, overflowDirection = 'top', additionalHiddenLinesCount = 0, sourceBoundaries, }) => {
    const id = useId();
    ...
    function visitRows(element) {
        ...
        if (element.type === Box) {
            layoutInkElementAsStyledText(element, maxWidth, laidOutStyledText, lineMetadata, `${id}:${rowIndex++}`, () => sourceBoundaries?.[sourceBoundaryIndex++]);
            return;
        }
    ...
    const visibleLines = visibleStyledText.map((line, index) => {
        ...
        return (_jsx(Box, { flexShrink: 0, children: line.length > 0 ? (line.map((segment, segIndex) => (_jsx(Text, { ...segment.props, selectionFlow: metadata.flowKey, selectionBreakAfter: metadata.breakAfter, selectionJoiner: metadata.joiner, children: segment.text }, segIndex)))) : (_jsx(Text, { selectable: false, selectionFlow: metadata.flowKey, selectionBreakAfter: metadata.breakAfter, selectionJoiner: metadata.joiner, children: ' ' })) }, index));
    });
...
function layoutInkElementAsStyledText(element, maxWidth, output, lineMetadata, flowKey, nextSourceBoundary) {
    const pushOutput = (line, breakAfter, joiner) => {
        output.push(line);
    ...
    function addWrappingPartToLines(breakAfter, joiner) {
        let line;
        if (lines.length === 0) {
