 ...
 */
export function removeModelFromProviders(modelProviders, providerProtocol, target) {
    ...
    for (const [key, models] of Object.entries(modelProviders)) {
        ...
        if (protocol !== target.authType)
            continue;
        for (let index = 0; index < models.length; index++) {
            if (models[index].id !== target.modelId)
                continue;
    ...
    if (idOnly)
        return buildRemoval(modelProviders, idOnly.key, idOnly.index);
 ...
 */
export function isActiveModelSelection(activeModelName, activeBaseUrl, target) {
    if (!activeModelName || activeModelName !== target.modelId)
        return false;
...
//# sourceMappingURL=model-providers-edit.js.map