...
import { detectTomlCommands, migrateTomlCommands, generateMigrationPrompt, } from './command-migration-tool.js';
...
describe('command-migration-tool', () => {
    ...
    describe('detectTomlCommands', () => {
        it('should detect TOML files in directory', async () => {
            ...
            await fs.writeFile(path.join(tempDir, 'cmd1.toml'), 'prompt = "test"', 'utf-8');
            await fs.writeFile(path.join(tempDir, 'cmd2.toml'), 'prompt = "test"', 'utf-8');
        ...
        it('should detect TOML files in subdirectories', async () => {
            ...
            await fs.writeFile(path.join(subdir, 'nested.toml'), 'prompt = "test"', 'utf-8');
            const tomlFiles = await detectTomlCommands(tempDir);
...
description = "Test description"`;
        ...
        it('should delete original TOML when deleteOriginal is true', async () => {
            await fs.writeFile(path.join(tempDir, 'delete-me.toml'), 'prompt = "Test"', 'utf-8');
...
description = "false"`;
            ...
            expect(result.success).toBe(true);
            const mdContent = await fs.readFile(path.join(tempDir, 'test.md'), 'utf-8');
        ...
        it('should fail if Markdown file already exists', async () => {
            await fs.writeFile(path.join(tempDir, 'existing.toml'), 'prompt = "Test"', 'utf-8');
            await fs.writeFile(path.join(tempDir, 'existing.md'), 'Already exists', 'utf-8');
            const result = await migrateTomlCommands({
                commandDir: tempDir,
                createBackup: false,
            });
        ...
        it('should handle migration without backup', async () => {
            await fs.writeFile(path.join(tempDir, 'no-backup.toml'), 'prompt = "Test"', 'utf-8');
            const result = await migrateTomlCommands({
                commandDir: tempDir,
                createBackup: false,
                deleteOriginal: false,
...
//# sourceMappingURL=command-migration-tool.test.js.map