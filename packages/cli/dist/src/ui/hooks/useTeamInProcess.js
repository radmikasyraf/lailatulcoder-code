...
import { DISPLAY_MODE, TeamEventType, } from '@lailatul-coder/lailatul-coder-core';
 ...
 */
export function useTeamInProcess(config, actions) {
    const actionsRef = useRef(actions);
    actionsRef.current = actions;
    useEffect(() => {
        if (!config)
            return;
        let detachTeamListeners = null;
         ...
         *  time. */
        const attachSession = (manager) => {
            const emitter = manager.getEventEmitter();
            let colorIndex = 0;
            const nextColor = () => TEAMMATE_COLORS[colorIndex++ % TEAMMATE_COLORS.length];
            /** Resolve the InProcessBackend, or null if not applicable. */
            const getInProcessBackend = () => {
            ...
            if (inProcessBackend) {
                for (const member of teamFile.members) {
                    ...
                    if (interactive) {
                        ...
                        actionsRef.current.registerAgent(member.agentId, interactive, member.name, member.color ?? nextColor(), member.name);
            ...
            const RETRY_MS = 50;
            const onTeammateJoined = (event) => {
                const tryRegister = (retriesLeft) => {
                    ...
                    if (interactive) {
                        ...
                        actionsRef.current.registerAgent(event.agentId, interactive, event.name, event.color ?? nextColor(), event.name);
...
//# sourceMappingURL=useTeamInProcess.js.map