...
import { closeSync, createReadStream, openSync, readSync, statSync, unwatchFile, watchFile, } from 'node:fs';
...
export class RemoteInputWatcher {
    ...
    readNewLines() {
        ...
        if (currentSize < this.bytesRead) {
            debugLogger.debug('RemoteInput: input file shrank, resetting read offset');
            this.bytesRead = 0;
            this.consumedPrefixHash = null;
        }
        else if (this.hasConsumedPrefixChanged()) {
            debugLogger.debug('RemoteInput: input file prefix changed, resetting read offset');
            this.bytesRead = 0;
            this.consumedPrefixHash = null;
        }
        if (currentSize <= this.bytesRead)
            return Promise.resolve();
        const consumeUntil = this.findLastCompleteRecordEnd(this.bytesRead, currentSize);
        ...
        rl.on('line', (line) => {
            ...
            try {
                ...
                if (cmd &&
                    ...
                    debugLogger.debug(`RemoteInput: confirmation_response for ${cmd.request_id} (allowed=${cmd.allowed})`);
                    this.confirmationHandler?.(cmd.request_id, cmd.allowed);
                }
                else if (cmd &&
                    cmd.type === 'submit' &&
                    typeof cmd.text === 'string') {
                    debugLogger.debug(`RemoteInput: queued command: ${cmd.text.slice(0, 50)}...`);
                    this.queue.push(cmd);
                }
                else {
                    debugLogger.warn(`RemoteInput: unknown command type: ${String(cmd?.type)}`);
                }
            }
            catch (_err) {
    ...
    findLastCompleteRecordEnd(start, end) {
        if (end <= start)
            return null;
        let fd = null;
        try {
    ...
    hasConsumedPrefixChanged() {
        if (this.bytesRead === 0) {
            return false;
        }
        if (this.consumedPrefixHash === null) {
            debugLogger.warn('RemoteInput: missing consumed prefix hash, resetting read offset');
            return true;
        }
        const currentHash = this.hashFilePrefix(this.bytesRead);
        if (currentHash === null) {
            debugLogger.warn('RemoteInput: failed to hash consumed prefix, resetting read offset');
            return true;
        }
    ...
    async processQueue() {
        if (this.processing || !this.submitFn || this.queue.length === 0)
            return;
        ...
        try {
            while (this.queue.length > 0 && this.active) {
                ...
                debugLogger.debug(`RemoteInput: submitting: ${cmd.text.slice(0, 50)}...`);
                try {
...
//# sourceMappingURL=RemoteInputWatcher.js.map