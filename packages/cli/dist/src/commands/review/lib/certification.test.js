...
import { chunkOfKey, declaresOwnUncoverable, openedBrief, readBrief, readFindingsPointer, } from './certification.js';
...
describe('declaresOwnUncoverable', () => {
    ...
    it('does not veto a declaration about a DIFFERENT chunk', () => {
        expect(declaresOwnUncoverable(rec({ finalText: declaring }), 4)).toBe(false);
    });
    it('never vetoes a chunk-less record — a quotation is not a declaration', () => {
        ...
        expect(declaresOwnUncoverable(rec({ finalText: declaring }), null)).toBe(false);
    });
    it('vetoes a declaration preceded by prose on an earlier line (pins /m)', () => {
        expect(declaresOwnUncoverable(rec({ finalText: `Read what I could.\n${declaring}` }), 3)).toBe(true);
    });
    it('matches a lowercase marker (pins /i)', () => {
        expect(declaresOwnUncoverable(rec({ finalText: 'uncoverable: chunk 3 — binary payload' }), 3)).toBe(true);
    });
...
describe('openedBrief / readBrief', () => {
    ...
    it('credits a match anywhere in the call list, not just the first call', () => {
        ...
        expect(openedBrief(rec({ successfulCallArgs: [other, arg] }), PLAN, key)).toBe(true);
        expect(readBrief(rec({ successfulReadFileArgs: [other, arg] }), PLAN, key)).toBe(true);
    });
...
describe('readFindingsPointer', () => {
    const pointer = '/tmp/certification-test/plan.json-prompts/round-2.findings.md';
    ...
    it('requires a successful read_file of the exact pointer', () => {
        expect(readFindingsPointer(rec({ successfulReadFileArgs: [arg] }), pointer)).toBe(true);
        // Named by a listing, read by nothing.
        expect(readFindingsPointer(rec({ successfulCallArgs: [arg] }), pointer)).toBe(false);
    });
});
