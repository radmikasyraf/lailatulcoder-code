...
vi.mock('@lailatul-coder/lailatul-coder-core', () => ({
    ...
    Storage: {
        getDebugLogPath: (sessionId) => `/tmp/qwen-debug/${sessionId}.txt`,
    },
...
describe('DebugModeNotification', () => {
    ...
    function renderNotification() {
        ...
        return render(_jsx(ConfigContext.Provider, { value: config, children: _jsx(DebugModeNotification, {}) }));
    }
    ...
    it('does not show a log path when debug file logging is disabled', () => {
        ...
        const { lastFrame } = renderNotification();
        expect(lastFrame()).toContain('Debug mode enabled');
        expect(lastFrame()).toContain('Debug log file disabled by QWEN_DEBUG_LOG_FILE');
        expect(lastFrame()).not.toContain('Logging to:');
    });
});
//# sourceMappingURL=DebugModeNotification.test.js.map