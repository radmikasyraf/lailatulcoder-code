 ...
 * report to ~/.lailatulcoder/startup-perf/ on finalization.
 *
 ...
 * reports. `LailatulCoder serve` has no sandbox child, so it is profiled directly.
...
function snapshotHeapMb() {
    if (!captureHeap)
        return undefined;
    try {
        return (Math.round(process.memoryUsage().heapUsed * HEAP_BYTES_TO_MB * 100) / 100);
    }
    catch {
        return undefined;
    }
}
export function initStartupProfiler() {
 ...
 */
export function recordStartupEvent(name, attrs) {
    if (!enabled || finalized)
        return;
 ...
 */
export function setInteractiveMode(value) {
    if (!enabled)
        return;
    interactiveMode = value;
}
function findCheckpointMs(name) {
...
function computeDerivedPhases() {
    ...
    if (mcpFirst !== undefined)
        out.mcp_first_tool = mcpFirst;
    const mcpSettled = findEventMs('mcp_all_servers_settled');
    if (mcpSettled !== undefined)
        out.mcp_all_settled = mcpSettled;
...
export function getStartupReport() {
    ...
    const lastTimestamp = checkpoints.length > 0
...
export function finalizeStartupProfile(sessionId) {
    if (!enabled || finalized)
        return;
    finalized = true;
    ...
    try {
        const dir = path.join(os.homedir(), '.lailatulcoder', 'startup-perf');
 ...
 */
export function isStartupProfilerEnabled() {
    return enabled;
}
