 ...
 * This file handles the creation and maintenance of ~/.lailatulcoder/output-language.md
...
const LLM_OUTPUT_LANGUAGE_MARKER_PREFIX = 'lailatul-coder:llm-output-language:';
 ...
 */
export function isAutoLanguage(value) {
 ...
 */
export function normalizeOutputLanguage(language) {
    const normalized = language.trim().replace(/_/g, '-').toLowerCase();
    const knownLanguageName = SUPPORTED_LANGUAGES.find((supportedLanguage) => supportedLanguage.fullName.toLowerCase() === normalized);
...
export function resolveOutputLanguage(value) {
    if (isAutoLanguage(value)) {
        throw new Error('resolveOutputLanguage does not accept auto; use resolveOutputLanguageOrPreserveAuto instead.');
    }
 ...
 */
export function resolveOutputLanguageOrPreserveAuto(value) {
    return isAutoLanguage(value)
        ? OUTPUT_LANGUAGE_AUTO
        : resolveOutputLanguage(value);
}
/**
 * Returns the path to the LLM output language rule file (~/.lailatulcoder/output-language.md).
 */
export function getOutputLanguageFilePath() {
...
function parseOutputLanguageFromContent(content) {
    // Primary: machine-readable marker (e.g., <!-- lailatul-coder:llm-output-language: 中文 -->)
    const markerRegex = new RegExp(String.raw `<!--\s*${LLM_OUTPUT_LANGUAGE_MARKER_PREFIX}\s*(.*?)\s*-->`, 'i');
...
function isGeneratedFixedOutputLanguageFileContent(content, language) {
    ...
    return (normalizedContent.includes(`<!-- ${LLM_OUTPUT_LANGUAGE_MARKER_PREFIX} ${sanitizeForMarker(language)} -->`) &&
        headings.length === 5 &&
 ...
 */
export function writeOutputLanguageFile(language, targetPath) {
 ...
 */
export function updateOutputLanguageFile(settingValue, targetPath) {
 ...
 * SettingsDialog.
 */
export function writeOutputLanguageAndRegisterPath(settingValue, config) {
 ...
 */
export function initializeLlmOutputLanguage(outputLanguage) {
    // Check if the file already exists and has valid content
    const currentFileContent = readOutputLanguageFileContent();
    const currentFileLanguage = currentFileContent === null
        ? null
        : parseOutputLanguageFromContent(currentFileContent);
    const shouldMigrateFixedFileToAuto = isAutoLanguage(outputLanguage) &&
        currentFileLanguage !== null &&
        !isAutoLanguage(currentFileLanguage) &&
        currentFileContent !== null &&
        isGeneratedFixedOutputLanguageFileContent(currentFileContent, currentFileLanguage);
