...
import { buildKittyPlaceholder, createRendererChildEnv, encodeKittyVirtualImage, findExecutable, readPngSize, shouldRunThroughShell, } from './mermaidImageRenderer.js';
...
export function wasKittyImageWritten(key) {
    return transmittedKeys.has(key);
}
export function markKittyImageWritten(key) {
...
export function supportsKittyImageProtocol(env = process.env, stdoutIsTTY = process.stdout.isTTY) {
...
export function getTerminalImageRenderSupport(env = process.env, stdoutIsTTY = process.stdout.isTTY) {
...
export function containsCmdShellMetacharacters(filePath) {
    return CMD_SHELL_METACHARACTERS.test(filePath);
}
export function prepareInlineTerminalImage({ data, mimeType, contentWidth, availableTerminalHeight, env = process.env, stdoutIsTTY = process.stdout.isTTY, disabled = false, }) {
...
export function renderTerminalImage({ display, contentWidth, availableTerminalHeight, env = process.env, stdoutIsTTY = process.stdout.isTTY, }) {
    let stat;
    try {
    ...
    const shape = fitImageToTerminal(headerSize, contentWidth, availableTerminalHeight);
    const useKitty = supportsKittyImageProtocol(env, stdoutIsTTY);
    const chafaPath = useKitty ? null : findExecutable('chafa', env);
    const cacheKey = createRenderCacheKey(display.filePath, stat, shape, useKitty, chafaPath);
    const cached = getCachedRenderResult(cacheKey);
    if (cached)
        return cached;
    let png;
...
function getImageFormat(mimeType) {
    const match = /^image\/([a-z0-9][a-z0-9.+-]*)$/.exec(mimeType.trim().toLowerCase());
    return match?.[1] ?? null;
}
    ...
    switch (result.kind) {
        case 'kitty':
            return (Buffer.byteLength(result.sequence, 'utf8') +
                result.placeholder.lines.reduce((total, line) => total + Buffer.byteLength(line, 'utf8'), 0));
        case 'ansi':
...
function fitImageToTerminal(size, contentWidth, availableTerminalHeight) {
    const maxWidthCells = Math.max(1, Math.min(Math.floor(contentWidth), MAX_PREVIEW_WIDTH_CELLS));
    const maxRows = Math.max(1, Math.min(Math.floor(availableTerminalHeight ?? MAX_PREVIEW_ROWS), MAX_PREVIEW_ROWS));
    ...
    const scale = Math.min(1, maxWidthCells / naturalWidthCells, maxRows / naturalRows);
    return {
        widthCells: Math.max(1, Math.min(maxWidthCells, Math.floor(naturalWidthCells * scale))),
...
function renderKitty(png, shape) {
    ...
    return {
        kind: 'kitty',
        sequence: encodeKittyVirtualImage(png, imageId, shape.widthCells, shape.rows),
...
function renderWithChafa(source, shape, env, chafaPath) {
    ...
    try {
        const stdout = execFileSync(chafaPath, [
            '--animate=off',
            '--colors=256',
            '--format=symbols',
            '--symbols=block',
...
//# sourceMappingURL=terminal-image-renderer.js.map