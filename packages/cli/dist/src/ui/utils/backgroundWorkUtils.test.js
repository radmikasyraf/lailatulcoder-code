...
import { hasBlockingBackgroundWork, resetBackgroundStateForSessionSwitch, describeBlockingBackgroundWork, buildBackgroundWorkBlockedMessage, } from './backgroundWorkUtils.js';
...
describe('hasBlockingBackgroundWork', () => {
    ...
    it('returns true when background tasks are still running', () => {
        expect(hasBlockingBackgroundWork(createMockConfig({ hasRunningTasks: true }))).toBe(true);
    });
    it('returns true when monitors are running', () => {
        expect(hasBlockingBackgroundWork(createMockConfig({ runningMonitors: [{ id: 'm1' }] }))).toBe(true);
    });
    it('returns true when shell entries are running', () => {
        expect(hasBlockingBackgroundWork(createMockConfig({ hasRunningEntries: true }))).toBe(true);
    });
    ...
    it('returns true when a workflow is still running', () => {
        expect(hasBlockingBackgroundWork(createMockConfig({ hasRunningWorkflows: true }))).toBe(true);
    });
...
describe('describeBlockingBackgroundWork (#8741)', () => {
    ...
    it('caps label width', () => {
        ...
        expect(describeBlockingBackgroundWork(config)?.lines[0]).toContain(`${'x'.repeat(79)}…`);
    });
    it('caps enumeration at 10 lines plus an overflow tail', () => {
        ...
        const summary = describeBlockingBackgroundWork(createEnumeratingMockConfig({ shells }));
...
describe('buildBackgroundWorkBlockedMessage (#8741)', () => {
    ...
    it('returns the bare base message when nothing is enumerated', () => {
        expect(buildBackgroundWorkBlockedMessage(createEnumeratingMockConfig(), base)).toBe(base);
    });
    ...
    it('points at both surfaces when both kinds block', () => {
        ...
        const message = buildBackgroundWorkBlockedMessage(config, base);
        expect(message).toContain('Use /tasks and /workflows to inspect them, then retry.');
    });
});
//# sourceMappingURL=backgroundWorkUtils.test.js.map