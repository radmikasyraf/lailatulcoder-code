 ...
 * memory scopes — per-project under `~/.lailatulcoder/projects/<hash>/memory/`
 * and user-level under `~/.lailatulcoder/memories/` — that happen to share the
 ...
 * body content.
 */
function formatMemoryManifest(docs) {
    const lines = [];
    const includedDocs = [];
    let bytes = 0;
    for (const doc of docs) {
        ...
        const line = doc.description
            ...
            : `- ${tag}${doc.filePath} (${ts})`;
        const nextBytes = Buffer.byteLength(`${lines.length > 0 ? '\n' : ''}${line}`);
...
export async function selectRelevantAutoMemoryDocumentsByModel(config, query, docs, limit, recentTools = [], callerAbortSignal) {
    ...
    const response = await runSideQuery(config, {
        ...
        validate: (value) => {
            ...
            if (value.selected_memories.some((filePath) => !validFilePaths.has(filePath))) {
...
//# sourceMappingURL=relevanceSelector.js.map