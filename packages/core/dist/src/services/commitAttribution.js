 ...
 */
function canonicaliseForHash(content) {
    ...
    let normalised = content.length > 0 && content.charCodeAt(0) === 0xfeff
        ? content.slice(1)
        : content;
...
const SANITIZED_GENERATOR_NAME = 'lailatul-coderr';
 ...
 * git-notes payload.
 */
function sanitiseCount(v) {
    return typeof v === 'number' && Number.isFinite(v) && v >= 0 ? v : 0;
}
function sanitiseAttribution(v) {
    const obj = (v ?? {});
    return {
 ...
 * for the lailatul-coder CLI; embedders (IDE extensions, SDK consumers) can
...
export class CommitAttributionService {
     ...
     */
    recordEdit(filePath, oldContent, newContent) {
    ...
    validateAgainst(getContent) {
        for (const [key, attr] of this.fileAttributions) {
            ...
            if (current === null)
                continue; // not a divergence signal
            if (computeContentHash(current) !== attr.contentHash) {
                debugLogger.debug(`validateAgainst: dropping stale attribution for ${key} (hash diverged)`);
     ...
     */
    clearAttributions(commitSucceeded = true) {
        if (commitSucceeded) {
     ...
     */
    clearAttributedFiles(committedAbsolutePaths) {
        this.promptCountAtLastCommit = this.promptCount;
     ...
     */
    matchCommittedFiles(relativeFiles, canonicalRepoRoot) {
     ...
     */
    applyCommittedRenames(renamedFiles, canonicalRepoRoot) {
        ...
        for (const [key, attr] of [...this.fileAttributions.entries()]) {
            ...
            if (!renamedRel)
                continue;
            const renamedAbs = realpathOrSelf(path.posix.join(posixRepoRoot, renamedRel));
            if (renamedAbs === key)
                continue;
    ...
    restoreFromSnapshot(snapshot) {
        ...
        const looksLikeSnapshot = isPlainObject(snapshot) &&
            snapshot['type'] === 'attribution-snapshot';
        ...
        this.promptCountAtLastCommit = sanitiseCount(snapshot.promptCountAtLastCommit);
     ...
     */
    generateNotePayload(stagedInfo, baseDir, generatorName) {
        const generator = sanitizeModelName(generatorName ?? SANITIZED_GENERATOR_NAME);
        const files = {};
        ...
        const aiPercent = totalChars > 0 ? Math.round((totalAiChars / totalChars) * 100) : 0;
        // Surface breakdown
 ...
 */
export function computeCharContribution(oldContent, newContent) {
...
//# sourceMappingURL=commitAttribution.js.map