/**
 * @license
 * Copyright 2025 LailatulCoder Team
 * SPDX-License-Identifier: Apache-2.0
 */
import { createRateLimiter } from '../rate-limit.js';
export function installRateLimiter(app, opts, daemonLog, options = {}) {
    if (!opts.rateLimit)
        return undefined;
    const windowMs = opts.rateLimitWindowMs ?? 60_000;
    const rateLimiter = createRateLimiter({
        tiers: {
            prompt: { windowMs, max: opts.rateLimitPrompt ?? 10 },
            mutation: { windowMs, max: opts.rateLimitMutation ?? 30 },
            read: { windowMs, max: opts.rateLimitRead ?? 120 },
        },
        hostname: opts.hostname,
        workspaceQualifiedAcpEnabled: options.workspaceQualifiedAcpEnabled === true,
        onLimitReached: daemonLog
            ? (tier, key, suppressed) => {
                daemonLog.warn(`rate limit hit${suppressed > 0 ? ` (${suppressed} suppressed)` : ''}`, { tier, key: key.slice(0, 64) });
            }
            : undefined,
        onError: daemonLog
            ? (err, path) => {
                daemonLog.warn(`rate limiter error (fail-open): ${err instanceof Error ? err.message : String(err)}`, { path });
            }
            : undefined,
    });
    if (options.mount !== false) {
        app.use(rateLimiter.middleware);
    }
    return rateLimiter;
}
//# sourceMappingURL=rate-limiter-setup.js.map