...
import { type ChannelDeliveryAccepted, type ChannelDeliveryRequest } from '../runtime/channel-delivery-ipc.js';
import { type ChannelAdapterSnapshot, type ChannelStartupFailure } from './channel-worker-startup-ipc.js';
...
export type ChannelWorkerState = 'disabled' | 'starting' | 'running' | 'exited' | 'failed' | 'stopped';
...
export interface ChannelWorkerSupervisor {
    ...
    deliverChannelMessage?(request: ChannelDeliveryRequest): Promise<ChannelDeliveryAccepted>;
...
export type SpawnChannelWorker = (execPath: string, argv: string[], options: {
    cwd: string;
...
export interface WorkerLogStream {
    on(event: 'data', listener: (chunk: Buffer | string | Uint8Array) => void): unknown;
...
export interface CreateChannelWorkerSupervisorOptions {
    ...
    unregisterChannelLoopMcp?: (sessionId: string, ownerId: string) => Promise<void>;
}
export declare function createChannelWorkerSupervisor(opts: CreateChannelWorkerSupervisorOptions): ChannelWorkerSupervisor;
