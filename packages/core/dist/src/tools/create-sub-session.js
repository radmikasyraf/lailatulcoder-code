...
class CreateSubSessionInvocation extends BaseToolInvocation {
    ...
    async execute(signal) {
        ...
        try {
            ...
            const parentWarning = res.parentSessionPersisted === false
            ...
            if (completion === 'sent') {
                ...
                return {
                    llmContent: `Sub-session ${sessionLink} created and the prompt was ` +
            ...
            const body = res.result && res.result.length > 0
                ? res.result
...
export class CreateSubSessionTool extends BaseDeclarativeTool {
    ...
    constructor(config) {
        super(CreateSubSessionTool.Name, ToolDisplayNames.CREATE_SUB_SESSION, 'Spawn a fresh, independent sub-session (its own clean context and ' +
            ...
            'conversation).', Kind.Other, {
            type: 'object',
            properties: {
                prompt: {
                    type: 'string',
                    description: 'The full, self-contained prompt to run in the new sub-session. ' +
                ...
                completion: {
                    ...
                    description: "'first-turn' (default) waits for the sub-session's first turn " +
                        "and returns its result. 'sent' returns immediately after the " +
                        'prompt is dispatched (fire-and-forget).',
                },
                model: {
                    type: 'string',
                    description: 'Optional model service id for the sub-session. Omit to use the ' +
                        'default model.',
                },
                name: {
                    type: 'string',
