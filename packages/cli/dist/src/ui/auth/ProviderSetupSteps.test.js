...
describe('ProviderSetupSteps', () => {
    ...
    const pressLatestKey = (name, sequence = name, overrides = {}) => {
    ...
    const createAdvancedConfigFlow = () => {
        const noop = vi.fn();
        return {
            state: {
                provider: {
    ...
    const createModelIdsFlow = ({ modelIds = 'MiniMax-M3, MiniMax-M2.7', submitModelIds = vi.fn(), } = {}) => {
        const noop = vi.fn();
        return {
            state: {
                provider: {
    ...
    const createCustomModelIdsFlow = ({ modelIds = 'model-a, model-b', submitModelIds = vi.fn(), } = {}) => {
        const noop = vi.fn();
        return {
            state: {
                provider: {
    ...
    it('maps Ctrl+P/N to advanced-config focus navigation', () => {
        ...
        const { unmount } = renderWithProviders(_jsx(ProviderSetupSteps, { flow: flow }));
    ...
    it('renders predefined editable models with a primary free-form input and recommendations', () => {
        ...
        const { lastFrame, unmount } = renderWithProviders(_jsx(ProviderSetupSteps, { flow: flow }));
        const frame = lastFrame() ?? '';
        expect(frame).toContain('Enter model IDs directly');
        expect(frame).toContain('Recommended models');
        expect(frame).toContain('Checked recommended models are applied on submit but not copied into the input.');
        ...
        expect(frame).toContain('↑↓/Tab to switch input, search, and recommendations');
...
//# sourceMappingURL=ProviderSetupSteps.test.js.map