...
import { collectMemoryDiagnostics } from '@lailatul-coder/lailatul-coder-core';
...
describe('doctorCommand', () => {
    ...
    const getMemoryCommand = () => {
        const memoryCommand = doctorCommand.subCommands?.find((command) => command.name === 'memory');
    ...
    function mockMemoryDiagnostics() {
        ...
        vi.mocked(memoryDiagnosticsModule.formatMemoryDiagnostics).mockReturnValue('Memory diagnostics\nRSS: 100.0 MiB\nActive handles: 3');
        vi.mocked(memoryDiagnosticsModule.writeMemoryHeapSnapshot).mockReturnValue('/tmp/lailatul-coder-heap.heapsnapshot');
        vi.mocked(memoryDiagnosticsModule.collectMemoryPressureSamples).mockResolvedValue([
            {
                index: 1,
        ...
        vi.mocked(memoryDiagnosticsModule.formatMemoryPressureSamples).mockReturnValue('Memory pressure samples\nSample count: 1');
        vi.mocked(memoryDiagnosticsModule.isHighHeapPressure).mockReturnValue(false);
    }
    beforeEach(() => {
        ...
        vi.mocked(cpuProfilerModule.stopCpuProfile).mockResolvedValue({
            ok: true,
            filePath: '/tmp/lailatul-coder.cpuprofile',
        });
    ...
    it('should have the correct name and description', () => {
        ...
        expect(doctorCommand.description).toBe('Run installation and environment diagnostics');
    });
    it('should complete memory subcommand names', async () => {
        ...
        await expect(doctorCommand.completion(mockContext, 'mem')).resolves.toEqual(['memory']);
        await expect(doctorCommand.completion(mockContext, 'cpu')).resolves.toEqual(['cpu-profile']);
        await expect(doctorCommand.completion(mockContext, 'roll')).resolves.toEqual(['rollback']);
    ...
    it('should show pending item and then add doctor item in interactive mode', async () => {
        ...
        expect(mockContext.ui.setPendingItem).toHaveBeenCalledWith(expect.objectContaining({ text: 'Running diagnostics...' }));
    ...
    it('should render sampled memory diagnostics in interactive mode', async () => {
        ...
        expect(memoryDiagnosticsModule.collectMemoryPressureSamples).toHaveBeenCalled();
        expect(mockContext.ui.addItem).toHaveBeenCalledWith(expect.objectContaining({
            type: 'info',
    ...
    it('should refuse heap snapshot when heap pressure is already high', async () => {
        ...
        const result = await doctorCommand.action(mockContext, 'memory --snapshot');
        expect(memoryDiagnosticsModule.writeMemoryHeapSnapshot).not.toHaveBeenCalled();
        expect(result).toEqual(expect.objectContaining({
            messageType: 'error',
    ...
    it('should render heap snapshot diagnostics in interactive mode', async () => {
        ...
        expect(memoryDiagnosticsModule.writeMemoryHeapSnapshot).toHaveBeenCalled();
        expect(mockContext.ui.setPendingItem).toHaveBeenCalledWith(expect.objectContaining({
    ...
    it('should render sampled heap snapshot diagnostics in interactive mode', async () => {
        ...
        expect(memoryDiagnosticsModule.getMemoryDiagnostics).toHaveBeenCalledTimes(2);
        expect(memoryDiagnosticsModule.writeMemoryHeapSnapshot).toHaveBeenCalled();
        expect(mockContext.ui.addItem).toHaveBeenCalledWith(expect.objectContaining({
            type: 'info',
            text: expect.stringContaining('Memory pressure samples'),
    ...
    it('should render heap snapshot failures as error items in interactive mode', async () => {
        vi.mocked(memoryDiagnosticsModule.writeMemoryHeapSnapshot).mockImplementation(() => {
    ...
    it('should not write heap snapshot when aborted before the snapshot side effect', async () => {
        ...
        vi.mocked(memoryDiagnosticsModule.formatMemoryDiagnostics).mockImplementation(() => {
    ...
    it('should capture a short memory pressure sample when requested', async () => {
        ...
        expect(memoryDiagnosticsModule.formatMemoryPressureSamples).toHaveBeenCalled();
        expect(result).toEqual({
            type: 'message',
            messageType: 'info',
    ...
    it('should render completed sample diagnostics when aborted after sampling', async () => {
        const abortController = new AbortController();
        vi.mocked(memoryDiagnosticsModule.collectMemoryPressureSamples).mockImplementation(async () => {
            abortController.abort();
            return [
                {
                    index: 1,
    ...
    it('should recheck heap pressure after sampling before writing snapshot', async () => {
        ...
        const result = await doctorCommand.action(mockContext, 'memory --sample --snapshot');
        expect(memoryDiagnosticsModule.getMemoryDiagnostics).toHaveBeenCalledTimes(2);
        expect(memoryDiagnosticsModule.writeMemoryHeapSnapshot).not.toHaveBeenCalled();
        expect(result).toEqual(expect.objectContaining({
            messageType: 'error',
    ...
    it('should return memory diagnostics as JSON for /doctor memory --json', async () => {
        ...
        expect(JSON.parse(result?.type === 'message' ? result.content : '{}')).toMatchObject({
            memoryUsage: {
                heapUsed: 1_000,
            },
            analysis: {
    ...
    it('should return a readable memory diagnostics summary for /doctor memory', async () => {
        ...
        expect(result?.type === 'message' ? result.content : '').toContain('heapUsed');
        expect(result?.type === 'message' ? result.content : '').toContain('v8MallocedMemory: 2.0 KB');
    });
    it('should render small memory values without rounding to zero MiB', async () => {
        const result = await getMemoryCommand().action(mockContext, '');
        expect(result?.type === 'message' ? result.content : '').toContain('heapUsed: 1.0 KB');
        expect(result?.type === 'message' ? result.content : '').not.toContain('heapUsed: 0.0 MB');
    });
    ...
    describe('tool result retention', () => {
        ...
        function contextWithHistory(history, uiHistory = [], options = {}) {
        ...
        it('should include retention stats in the readable report', async () => {
            const result = await getMemoryCommand().action(contextWithHistory(history), '');
            ...
            expect(content).toContain('Oversized also rendered in UI history: 0 item(s)');
            expect(content).toContain('Oversized also in compression input: yes (shared by reference, no extra copy)');
        ...
        it('should detect tool outputs rendered in UI history', async () => {
            ...
            const result = await getMemoryCommand().action(contextWithHistory(history, uiHistory), '');
            const content = result?.type === 'message' ? result.content : '';
            expect(content).toContain('Oversized also rendered in UI history: 1 item(s)');
        });
        it('should canonicalize legacy tool names when resolving budgets', async () => {
            ...
            const result = await getMemoryCommand().action(contextWithHistory(aliasedHistory), '');
            const content = result?.type === 'message' ? result.content : '';
            expect(content).toContain('Oversized results (above tool budget): 1');
        });
        ...
        it('should include retention stats in --json output', async () => {
            const result = await getMemoryCommand().action(contextWithHistory(history), '--json');
            const parsed = JSON.parse(result?.type === 'message' ? result.content : '{}');
            ...
            expect(parsed.toolResultRetention?.['largestResultChars']).toBeGreaterThan(40_000);
        });
        it('should omit toolResultRetention from --json when history is unavailable', async () => {
            const result = await getMemoryCommand().action(createMockCommandContext({
                executionMode: 'non_interactive',
                ui: {
                    addItem: vi.fn(),
                    setPendingItem: vi.fn(),
        ...
        it('should omit the retention section when reading history throws', async () => {
            const result = await getMemoryCommand().action(contextWithHistory(history, [], { historyThrows: true }), '');
            const content = result?.type === 'message' ? result.content : '';
        ...
        it('should render the compliant-session shape without compression advice', async () => {
            ...
            const result = await getMemoryCommand().action(contextWithHistory(compliantHistory), '');
            const content = result?.type === 'message' ? result.content : '';
    ...
    it('should register memory as a real doctor subcommand', () => {
        expect(doctorCommand.subCommands?.map((command) => command.name)).toContain('memory');
        expect(getMemoryCommand().argumentHint).toBe('[--json] [--sample] [--snapshot]');
    });
    ...
    it('should render risk indicators without failing memory diagnostics', async () => {
        ...
        expect(result?.type === 'message' ? result.content : '').toContain('heap-pressure: Heap pressure detected.');
        expect(result?.type === 'message' ? result.content : '').toContain('recommendation: 1 potential leak indicator(s) found.');
        expect(result?.type === 'message' ? result.content : '').not.toContain('recommendation: WARNING:');
    });
    ...
    it('should return an error message when memory diagnostics fail', async () => {
        vi.mocked(collectMemoryDiagnostics).mockRejectedValueOnce(new Error('probe failed'));
        const result = await getMemoryCommand().action(mockContext, '');
        expect(result).toEqual(expect.objectContaining({
            type: 'message',
            messageType: 'error',
    ...
    it('should reject unknown arguments with a usage hint', async () => {
        ...
        expect(result?.type === 'message' ? result.content : '').toContain('/doctor memory [--json] [--sample] [--snapshot]');
    });
    ...
    it('should advertise the memory subcommand on the parent doctor argumentHint', () => {
        expect(doctorCommand.argumentHint).toBe('[memory|cpu-profile|rollback] [--sample] [--snapshot] [--duration]');
    });
    it('should reject non-integer cpu profile durations before recording', async () => {
        ...
        for (const duration of ['1.5', '2s']) {
            ...
            const result = await doctorCommand.action(mockContext, `cpu-profile --duration ${duration}`);
            expect(result).toEqual(expect.objectContaining({
                type: 'message',
                messageType: 'error',
                content: expect.stringContaining('Usage: /doctor cpu-profile [--duration <seconds>]'),
            }));
            expect(result?.type === 'message' ? result.content : '').toContain('Duration must be between 1 and');
