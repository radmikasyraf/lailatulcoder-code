...
import { appendPromptLedgerRecord, readPromptLedgerRecords, } from '@lailatul-coder/acp-bridge/promptLedger';
import { createPromptLedgerSink, readRecentPromptTerminals, readTranscriptTailUuid, reconcileDanglingPromptTerminals, withPromptTerminals, } from './prompt-terminal-ledger.js';
...
function makeFixture() {
    ...
    const transcriptPath = path.join(path.dirname(ledgerPath), `${sessionId}.jsonl`);
    return {
        workspaceDir,
        runtimeBaseDir,
        sessionService,
...
let recordSeq = 0;
function record(fixture, uuid, parentUuid, text) {
...
function recordAt(fixture, uuid, parentUuid, text, atMs) {
    return {
        ...record(fixture, uuid, parentUuid, text),
        timestamp: new Date(atMs).toISOString(),
    };
}
function toolCallRecord(fixture, uuid, parentUuid, callId) {
    return {
...
function systemRecord(fixture, uuid, parentUuid, subtype, systemPayload) {
    return {
        ...record(fixture, uuid, parentUuid, ''),
        type: 'system',
        subtype,
...
function writeTranscript(fixture, records) {
    mkdirSync(path.dirname(fixture.transcriptPath), { recursive: true });
    writeFileSync(fixture.transcriptPath, records.map((entry) => JSON.stringify(entry)).join('\n') + '\n', 'utf8');
}
function writeLedger(fixture, records) {
...
describe('reconcileDanglingPromptTerminals', () => {
    it('marks a transcript-clean dangling prompt completed', async () => {
        ...
        await reconcileDanglingPromptTerminals(fixture.sessionService, fixture.sessionId);
    ...
    it('attributes the outcome when a visible write lands beyond the dispatch marker', async () => {
        ...
        expect(records[1]).toEqual(expect.objectContaining({ promptId: 'p1', terminal: 'completed' }));
    });
...
describe('readRecentPromptTerminals + withPromptTerminals', () => {
    it('returns undefined without ledger evidence', () => {
        const fixture = makeFixture();
        expect(readRecentPromptTerminals(fixture.sessionService, fixture.sessionId)).toBeUndefined();
    });
    ...
    it('returns the trailing terminal records', () => {
        ...
        expect(readRecentPromptTerminals(fixture.sessionService, fixture.sessionId)).toEqual([
            { v: 1, promptId: 'p1', terminal: 'completed', at: 2 },
            {
                v: 1,
                promptId: 'p2',
    ...
    it('reads only the trailing window, not the whole ledger', () => {
        ...
        const terminals = readRecentPromptTerminals(fixture.sessionService, fixture.sessionId);
...
describe('createPromptLedgerSink', () => {
    it('appends through the SessionService path layout', () => {
        const fixture = makeFixture();
        const sink = createPromptLedgerSink(fixture.workspaceDir, fixture.runtimeBaseDir);
...
describe('readTranscriptTailUuid', () => {
    it('returns the last record uuid, degrading on missing or torn evidence', () => {
        ...
        writeFileSync(fixture.transcriptPath, '{"uuid":"u1"}\n{"uuid":"a1","text":"answ', 'utf8');
        expect(readTranscriptTailUuid(fixture.transcriptPath)).toBeUndefined();
    });
});
//# sourceMappingURL=prompt-terminal-ledger.test.js.map