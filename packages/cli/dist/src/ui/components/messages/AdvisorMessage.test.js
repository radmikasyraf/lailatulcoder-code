...
describe('AdvisorMessage', () => {
    it('renders the resolved model in the header and the review body', () => {
        const { lastFrame } = renderWithProviders(_jsx(AdvisorMessage, { text: '## Verdict\nThe approach is sound.', model: "qwen3-max" }));
    ...
    it('lays out content at containerWidth, not terminal width', () => {
        ...
        try {
            const { lastFrame } = renderWithProviders(_jsx(AdvisorMessage, { text: '```\n' + 'x'.repeat(80) + '\n```', model: "m", containerWidth: 30 }));
            const output = lastFrame() ?? '';
...
//# sourceMappingURL=AdvisorMessage.test.js.map