...
import { canonicalizeWorkspace, canonicalizeWorkspaces, MAX_WORKSPACE_PATH_LENGTH, } from '@lailatul-coder/acp-bridge/workspacePaths';
 ...
 */
const ENOENT_TOLERATING_INTENTS = new Set([
    'write',
    'stat',
]);
/**
 ...
 */
export function hasSuspiciousPathPattern(p) {
    ...
    if (process.platform === 'win32' && /~\d+/.test(p))
        return true;
    if (p.startsWith('\\\\?\\') ||
        p.startsWith('\\\\.\\') ||
        p.startsWith('//?/') ||
    ...
    if ((p.startsWith('\\\\') && p.length > 2 && p[2] !== '\\') ||
 ...
 */
const CANONICAL_BOUND_CACHE = new Map();
function canonicalizeBoundWorkspaceCached(boundWorkspace) {
...
function canonicalizeBoundWorkspacesCached(boundWorkspaces) {
    const inputs = typeof boundWorkspaces === 'string' ? [boundWorkspaces] : boundWorkspaces;
    if (inputs.length === 0) {
        throw new FsError('parse_error', 'boundWorkspaces must include at least one workspace root');
    }
    const out = [];
    const seen = new Set();
...
function findContainingWorkspace(absolute, boundWorkspaces) {
    let match;
    for (const workspace of boundWorkspaces) {
        if (isWithinRoot(absolute, workspace) &&
 ...
 */
async function findExistingAncestor(absolute) {
    ...
    for (let i = 0; i < MAX_ANCESTOR_HOPS; i++) {
        ...
        if (stat) {
            ...
            if (tailParts.length > 0 && !stat.isDirectory()) {
                throw new FsError('parse_error', `path component is not a directory: ${absolute}`, {
        ...
        if (parent === current) {
            throw new FsError('path_not_found', `no existing ancestor for ${absolute}`);
        }
        tailParts.unshift(path.basename(current));
        current = parent;
    }
    throw new FsError('path_not_found', `path traversal exceeded ${MAX_ANCESTOR_HOPS} hops while finding ancestor`, {
 ...
 */
export async function resolveWithinWorkspace(input, boundWorkspaces, intent) {
    ...
    let preResolvedCanonical;
    if (boundCanonical === undefined && path.isAbsolute(input)) {
        try {
            preResolvedCanonical = await fsp.realpath(absolute);
            boundCanonical = findContainingWorkspace(preResolvedCanonical, boundCanonicals);
        }
        catch (err) {
            const code = err?.code;
            if (code === 'ENOENT' || code === 'ENOTDIR') {
                try {
                    ...
                    const canonicalCandidate = tail
                        ...
                        : ancestorReal;
                    boundCanonical = findContainingWorkspace(canonicalCandidate, boundCanonicals);
    ...
    if (boundCanonical === undefined) {
        throw new FsError('path_outside_workspace', `path escapes workspace: ${input}`);
    }
    let canonical;
    try {
        ...
        if (code === 'ENOENT' && ENOENT_TOLERATING_INTENTS.has(intent)) {
            ...
            let symlinkResolvedCanonical = null;
            ...
            try {
                ...
                for (let hop = 0; hop < MAX_ANCESTOR_HOPS; hop++) {
                    ...
                    const ino = typeof linkStat.ino === 'bigint'
                        ? linkStat.ino
                        : BigInt(linkStat.ino);
                    if (ino !== 0n && visitedInodes.has(ino)) {
                        throw new FsError('symlink_escape', `symlink cycle detected at ${cursor}`, { hint: 'symlink loops back onto a previously visited inode' });
                    }
                    if (ino !== 0n)
                    ...
                    if (firstHopTarget === null)
                        firstHopTarget = target;
                    cursor = absTarget;
                }
                if (!resolvedFully) {
                    throw new FsError('symlink_escape', `symlink chain exceeded ${MAX_ANCESTOR_HOPS} hops for ${input}`, { hint: 'symlink chain is too long or contains a cycle' });
                }
                ...
                // just verified.
                if (firstHopTarget !== null) {
                    const { ancestor: targetAncestor, tail: targetTail } = await findExistingAncestor(cursor);
                    ...
                    const canonicalTarget = targetTail
                        ...
                        : targetAncestorReal;
                    if (!boundCanonicals.some((root) => isWithinRoot(canonicalTarget, root))) {
                        throw new FsError('symlink_escape', `dangling symlink target escapes workspace: ${input}`, {
            ...
            throw new FsError('symlink_escape', `symlink loop or chain too long for ${input}`, {
                cause: err,
                hint: 'a symlink in the path forms a cycle or exceeds SYMLOOP_MAX',
            });
        }
        else if (code === 'EACCES') {
            throw new FsError('permission_denied', `permission denied resolving ${input}`, { cause: err });
        }
        else {
            throw err;
        }
    ...
    if (!boundCanonicals.some((root) => isWithinRoot(canonical, root))) {
        ...
        throw new FsError(kind, kind === 'symlink_escape'
