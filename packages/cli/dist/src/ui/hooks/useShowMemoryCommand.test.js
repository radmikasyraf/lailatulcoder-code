...
function createMockConfig({ userMemory = '', autoMemoryPrompt = '', fileCount = 0, }) {
    return {
...
describe('createShowMemoryAction', () => {
    ...
    const getCombinedMemoryMessage = () => messages.find((m) => m.type === MessageType.INFO &&
    ...
    it('reports no memory (and emits no combined-content message) when both layers are empty', async () => {
        ...
        expect(messages.some((m) => m.type === MessageType.INFO &&
            m.content.includes('No hierarchical memory'))).toBe(true);
    });
});
//# sourceMappingURL=useShowMemoryCommand.test.js.map