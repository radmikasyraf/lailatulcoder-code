...
import { getAllMCPServerStatuses, getMCPServerStatus, MCPServerStatus, } from './mcp-status.js';
import { getInvocationContext, INVOCATION_CONTEXT_META_KEY, } from '../utils/invocation-context.js';
 ...
 */
function isExecutionTimeoutFailure(error, serverName, signal) {
    ...
    return !(statuses.has(serverName) &&
        statuses.get(serverName) === MCPServerStatus.DISCONNECTED);
}
const PARENT_ABORT_OUTCOME = Symbol('parent_abort_outcome');
function createToolCallAbortError() {
    return Object.assign(new Error('Tool call aborted'), { name: 'AbortError' });
}
function isParentAbortOutcome(value) {
    return (typeof value === 'object' && value !== null && PARENT_ABORT_OUTCOME in value);
}
function createParentAbortRace(signal, forwardAbort) {
    let onAbort;
...
class DiscoveredMCPToolInvocation extends BaseToolInvocation {
    ...
    static UNSAFE_REPLAY_ERROR_MESSAGE = 'MCP tool execution may have completed before the connection failed. Automatic replay was skipped because the call could not be verified as safe to replay. Do not retry automatically; verify the outcome before trying again.';
    constructor(mcpTool, serverName, serverToolName, displayName, registeredToolName, permissionAliases, trust, params = {}, cliConfig, mcpClient, mcpTimeout, mcpToolIdleTimeoutMs, annotations, allowInvocationContext = false, retryCount = 0) {
        super(params);
        ...
        this.allowInvocationContext = allowInvocationContext;
    ...
    async handleReconnectOnError(error, signal, updateOutput) {
        ...
        if (this.retryCount < DiscoveredMCPToolInvocation.MAX_RECONNECT_RETRIES) {
            ...
            if (newTool) {
                const newInvocation = new DiscoveredMCPToolInvocation(newTool['mcpTool'], this.serverName, this.serverToolName, this.displayName, newTool.name, newTool.permissionAliases, newTool.trust, this.params, this.cliConfig, newTool['mcpClient'], this.mcpTimeout, this.mcpToolIdleTimeoutMs, newTool.annotations, newTool['allowInvocationContext'] === true, this.retryCount + 1);
    ...
    canSafelyReplay() {
        ...
        return (this.annotations.idempotentHint === true ||
    ...
    async executeWithDirectClient(signal, updateOutput) {
        ...
        try {
            ...
            return {
                ...
                returnDisplay: getDisplayFromPartsWithPersistedOutput(transformedParts, truncated.persistedOutputFiles),
            ...
            if (idleTimeoutWon ||
                ...
                throw new StructuredToolError(getErrorMessage(error), ToolErrorType.EXECUTION_TIMEOUT);
            }
    ...
    async buildMcpToolError(rawResponseParts, functionCall) {
        ...
        if (imageContent) {
            ...
            errorMessage = `MCP tool '${this.serverToolName}' reported tool error for function call: ${safeJsonStringify(functionCall)} with response: ${getDisplayFromParts(truncatedContent.parts)}`;
        }
        else {
            errorMessage = `MCP tool '${this.serverToolName}' reported tool error for function call: ${safeJsonStringify(functionCall)} with response: ${safeJsonStringify(rawResponseParts)}`;
...
export class DiscoveredMCPTool extends BaseDeclarativeTool {
    ...
    constructor(mcpTool, serverName, serverToolName, description, parameterSchema, trust, nameOverride, cliConfig, mcpClient, mcpTimeout, mcpToolIdleTimeoutMs, annotations, alwaysLoad = false, allowInvocationContext = false) {
        super(nameOverride ??
    ...
    asFullyQualifiedTool() {
        return new DiscoveredMCPTool(this.mcpTool, this.serverName, this.serverToolName, this.description, this.parameterSchema, this.trust, generateValidName(`mcp__${this.serverName}__${this.serverToolName}`), this.cliConfig, this.mcpClient, this.mcpTimeout, this.mcpToolIdleTimeoutMs, this.annotations, this.alwaysLoad, this.allowInvocationContext);
    }
    /**
     ...
     */
    withSessionConfig(trust, alwaysLoad) {
        if (trust === this.trust && alwaysLoad === this.alwaysLoad)
            return this;
        ...
        // session.
        this.name, this.cliConfig, this.mcpClient, this.mcpTimeout, this.mcpToolIdleTimeoutMs, this.annotations, alwaysLoad, this.allowInvocationContext);
    }
    createInvocation(params) {
        return new DiscoveredMCPToolInvocation(this.mcpTool, this.serverName, this.serverToolName, this.displayName, this.name, this.permissionAliases, this.trust, params, this.cliConfig, this.mcpClient, this.mcpTimeout, this.mcpToolIdleTimeoutMs, this.annotations, this.allowInvocationContext);
    }
}
/**
...
function getDisplayFromPartsWithPersistedOutput(parts, persistedOutputFiles) {
    const display = getDisplayFromParts(parts);
    if (!persistedOutputFiles?.length)
        return display;
