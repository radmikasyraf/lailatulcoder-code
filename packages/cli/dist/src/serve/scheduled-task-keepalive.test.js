...
import { Storage, readCronTasks, SessionService, updateCronTasks, } from '@lailatul-coder/lailatul-coder-core';
...
describe('scheduled-task keepalive', () => {
    ...
    it('heartbeats each distinct bound session, skipping unbound tasks', async () => {
        ...
        expect(onTasksRead).toHaveBeenCalledWith(expect.arrayContaining([
    ...
    it('rehydrates in bounded batches, not all sessions at once', async () => {
        ...
        const many = Array.from({ length: 12 }, (_, i) => task({ id: `t${i}`, sessionId: `s${i}` }));
    ...
    it('rolls back when another process binds the task before write', async () => {
        ...
        const raceBridge = {
            ...
            spawnOrAttach: async () => {
                ...
                await updateCronTasks(workspace, (list) => list.map((t) => t.id === 'raced' ? { ...t, sessionId: 'other-sess' } : t));
    ...
    it('a hung spawnOrAttach does not stall subsequent ticks', async () => {
        ...
        const hungBridge = {
            ...bridge,
            spawnOrAttach: () => new Promise((resolve) => {
    ...
    it('waits for close before deleting a late spawned transcript', async () => {
        ...
        await vi.waitFor(() => expect(closeSession).toHaveBeenCalledWith('late-sess'));
    ...
    it('rehydration onTasksRead populates the authorization store for delivery-enabled tasks', async () => {
        ...
        expect(authorizations.consume(workspace, {
            sessionId: 'sess-del',
...
//# sourceMappingURL=scheduled-task-keepalive.test.js.map