...
function setup(options) {
    ...
    const store = {
        ...
        upsert: vi.fn(async (name, request) => {
            ...
            const clientSecret = request.secrets?.clientSecret?.operation === 'clear'
                ? undefined
    ...
    const manager = {
        ...
        state: vi.fn(() => ({
            ...
            selection: names.length > 0 ? { mode: 'names', names: [...names] } : null,
            transition: 'idle',
            workers: names.length > 0
                ? [
                    {
                        enabled: true,
                        state: 'running',
...
function codeOf(result) {
    if ('code' in result)
        return result.code;
    throw new Error(`expected a pairing code, got rejection "${result.rejected}"`);
}
describe('createChannelManagementService', () => {
    ...
    it('does not expose config fields from an unmanaged channel type', async () => {
        ...
        expect(JSON.stringify(result.instances['legacy'])).not.toContain('LEGACY_TOKEN');
    });
    ...
    it('keeps a failed replacement config and reports the instance as error', async () => {
        ...
        manager.reloadWorkspace.mockRejectedValueOnce(new Error('invalid token clientSecret=start-secret'));
        ...
        expect(manager.setChannelEnabled).toHaveBeenCalledWith({ name: 'bot', workspaceCwd: WORKSPACE }, false);
    ...
    it('rejects a config whose effective cwd escapes the selected workspace', async () => {
        ...
        await expect(service.upsert('bot', {
            expectedRevision: 'rev-1',
            config: {
                type: 'dingtalk',
                cwd: '../secondary',
    ...
    it('fails closed for lifecycle and pairing on a legacy cross-workspace config', async () => {
        ...
        await expect(service.setStartup('bot', {
            expectedRevision: 'rev-1',
            enabled: true,
        })).rejects.toMatchObject({ code: 'channel_workspace_mismatch' });
        ...
        await expect(service.remove('bot', { expectedRevision: 'rev-1' })).rejects.toMatchObject({ code: 'channel_workspace_mismatch' });
        ...
        await expect(service.revokePairingApproval('bot', {
            type: 'user',
            id: 'sender-1',
    ...
    it('manages pairing requests and approvals in the selected workspace scope', async () => {
        ...
        const qwenHome = await fs.mkdtemp(path.join(os.tmpdir(), 'channel-management-pairing-'));
    ...
    it('manages group pairing when groupPolicy uses pairing mode', async () => {
        const previousQwenHome = process.env['QWEN_HOME'];
        const qwenHome = await fs.mkdtemp(path.join(os.tmpdir(), 'channel-management-group-pairing-'));
        process.env['QWEN_HOME'] = qwenHome;
        try {
            const { service } = setup({
                snapshot: settingsSnapshot({
            ...
            const pairing = new PairingStore('bot', WORKSPACE);
            const code = codeOf(pairing.createGroupRequest('group-1', 'Release Team', 'sender-1', 'Alice'));
            const secondCode = codeOf(pairing.createGroupRequest('group-2', 'Platform Team', 'sender-2', 'Bob'));
            await expect(service.pairingRequests('bot')).resolves.toEqual({
                requests: [
                    expect.objectContaining({
                        senderId: 'sender-1',
    ...
    it('retains the reload diagnostic when stopping the failed replacement also fails', async () => {
        const { service, manager } = setup({ committedNames: ['bot'] });
        manager.reloadWorkspace.mockRejectedValueOnce(new Error('invalid token clientSecret=start-secret'));
        manager.setChannelEnabled.mockRejectedValueOnce(new Error('stop failed clientSecret=stop-secret'));
        const result = await service.upsert('bot', {
            expectedRevision: 'rev-1',
            config: {
                type: 'dingtalk',
    ...
    it('does not delete config when worker stop is unconfirmed', async () => {
        ...
        vi.mocked(manager.setChannelEnabled).mockRejectedValueOnce(Object.assign(new Error('stop unconfirmed'), {
            code: 'channel_worker_stop_failed',
        }));
        await expect(service.remove('bot', { expectedRevision: 'rev-1' })).rejects.toMatchObject({ code: 'channel_worker_stop_failed' });
        expect(store.remove).not.toHaveBeenCalled();
    ...
    it('rejects stale removal before changing runtime state', async () => {
        ...
        await expect(service.remove('bot', { expectedRevision: 'stale' })).rejects.toMatchObject({ code: 'channel_settings_conflict' });
        expect(store.remove).not.toHaveBeenCalled();
        expect(manager.setChannelEnabled).not.toHaveBeenCalled();
    });
    it('delegates starts and stops to the manager atomic mutation lane', async () => {
        ...
        expect(manager.setChannelEnabled).toHaveBeenNthCalledWith(1, { name: 'bot', workspaceCwd: WORKSPACE }, true);
        await service.stop('second');
        expect(manager.setChannelEnabled).toHaveBeenNthCalledWith(2, { name: 'second', workspaceCwd: WORKSPACE }, false);
        expect(store.upsert).not.toHaveBeenCalled();
        expect(store.remove).not.toHaveBeenCalled();
    });
    ...
    it.each(['all', ' all ', '\tall\n'])('rejects reserved channel name %j before lifecycle mutation', async (name) => {
        const { service, store, manager } = setup({ committedNames: [] });
        await expect(service.remove(name, { expectedRevision: 'rev-1' })).rejects.toMatchObject({
            code: 'invalid_channel_instance_name',
        });
        await expect(service.start(name)).rejects.toMatchObject({
            code: 'invalid_channel_instance_name',
        });
        await expect(service.setStartup(name, {
            expectedRevision: 'rev-1',
            enabled: true,
    ...
    it.each(['constructor', 'toString', '__proto__'])('rejects inherited instance name %s before start or stop reaches the manager', async (name) => {
    ...
    it('rejects an inactive cross-workspace start before lifecycle mutation', async () => {
        const { service, manager } = setup({ committedNames: [] });
        vi.mocked(manager.setChannelEnabled).mockRejectedValueOnce(Object.assign(new Error('owner mismatch'), {
            code: 'channel_runtime_owner_mismatch',
        }));
        await expect(service.start('bot')).rejects.toMatchObject({
            code: 'channel_runtime_owner_mismatch',
        });
        expect(manager.setChannelEnabled).toHaveBeenCalledWith({ name: 'bot', workspaceCwd: WORKSPACE }, true);
        expect(manager.reload).not.toHaveBeenCalled();
        expect(manager.reloadWorkspace).not.toHaveBeenCalled();
    });
    it('serializes lifecycle mutations for one workspace service', async () => {
        const { service, manager } = setup({ committedNames: ['bot'] });
        let finishReload;
        manager.reloadWorkspace.mockImplementationOnce(() => new Promise((resolve) => {
            ...
            workers: [
                {
                    ...
                    adapters: [{ name: 'bot', state: 'connected' }],
                    workspaceId: 'other',
    ...
    it('rejects mutations when two same-name workers run in the same workspace', async () => {
        ...
        vi.mocked(manager.state).mockReturnValue(twoWorkers);
        await expect(service.remove('bot', { expectedRevision: 'rev-1' })).rejects.toMatchObject({ code: 'channel_runtime_owner_mismatch' });
        expect(manager.reloadWorkspace).not.toHaveBeenCalled();
        expect(manager.setChannelEnabled).not.toHaveBeenCalled();
    });
    it('rejects lifecycle operations for a nonexistent channel', async () => {
        ...
        await expect(service.remove('nonexistent', { expectedRevision: 'rev-1' })).rejects.toMatchObject({ code: 'channel_instance_not_found' });
    ...
    it('rejects setStartup for a nonexistent channel', async () => {
        ...
        await expect(service.setStartup('nonexistent', {
            expectedRevision: 'rev-1',
            enabled: true,
    ...
    it('rejects approval of an unknown pairing code', async () => {
        ...
        try {
            ...
            await expect(service.approvePairing('bot', 'ZZZZZZZZ')).rejects.toMatchObject({ code: 'channel_pairing_request_not_found' });
        }
        finally {
            if (previousQwenHome === undefined)
                delete process.env['QWEN_HOME'];
    ...
    it('rejects pairing operations on a channel without pairing mode', async () => {
        for (const config of [
            ...
            await expect(service.approvePairing('bot', 'ABCDEFGH')).rejects.toMatchObject({ code: 'channel_pairing_not_enabled' });
            await expect(service.pairingApprovals('bot')).rejects.toMatchObject({
                code: 'channel_pairing_not_enabled',
            });
            await expect(service.revokePairingApproval('bot', {
...
//# sourceMappingURL=channel-management-service.test.js.map