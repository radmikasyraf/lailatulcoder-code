...
const mockBuildBtwCacheSafeParams = vi.hoisted(() => vi.fn().mockReturnValue({
    generationConfig: {},
...
describe('advisorCommand', () => {
    ...
    describe('interactive mode', () => {
        it('should show pending item, add an advisor review item, then clear pending', async () => {
            ...
            expect(mockContext.ui.setPendingItem.mock.invocationCallOrder[0]).toBeLessThan(mockRunForkedAgent.mock.invocationCallOrder[0]);
        ...
        it('should pass focus into the advisor prompt', async () => {
            ...
            expect(mockRunForkedAgent).toHaveBeenCalledWith(expect.objectContaining({
            ...
            expect(mockRunForkedAgent.mock.calls[0][0].cacheSafeParams).toBe(mockBuildBtwCacheSafeParams.mock.results.at(-1)?.value);
        });
        it('should trim padding around the focus before building the prompt', async () => {
            ...
            expect(mockRunForkedAgent.mock.calls[0][0].userMessage).toContain('check the padding');
        });
        ...
        it('should forward abortSignal to runForkedAgent', async () => {
            ...
            expect(mockRunForkedAgent).toHaveBeenCalledWith(expect.objectContaining({ abortSignal: abortController.signal }));
        });
        ...
        it('should not add items after abort', async () => {
            ...
            expect(abortableContext.ui.setPendingItem).not.toHaveBeenLastCalledWith(null);
        });
...
//# sourceMappingURL=advisor-command.test.js.map