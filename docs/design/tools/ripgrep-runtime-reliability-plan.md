...
当 ripgrep 缺失时，LailatulCoder Ai 已经提供了主要的回退链路：
...
- 不引入 `@vscode/ripgrep`；LailatulCoder Ai 已经随包提供了针对不同平台的二进制文件。
...
本次修改不是为了让搜索“永不失败”，而是为了保证 LailatulCoder Ai 能准确区分以下三种事实：确实没有匹配项、搜索失败且没有可用结果、搜索失败但产生了部分结果。模型只有看到正确的事实，才能决定是缩小搜索范围、改用其他工具，还是继续使用已有的部分结果。
...
**修改目的：** 防止 LailatulCoder Ai 把带有错误信息或异常输出的退出码 1 一律解释成“仓库中不存在该内容”。“没有匹配项”是会影响模型后续推理的强结论，只有在 ripgrep 确实正常表达无匹配时才能使用。
...
**修改前：** 现有 `RipgrepFallbackEvent` 只在启动探测失败、LailatulCoder Ai 从 `RipGrepTool` 切换到 `GrepTool` 时发送。启动后发生的 EAGAIN、超时、超过最大缓冲区、异常退出或 spawn 失败没有专门的结构化指标，因此无法回答“运行时恢复是否真的有用”。
...
- **截断（Truncated）：** LailatulCoder Ai 为了展示，按行数或字符数主动限制了一份原本完整的输出。
