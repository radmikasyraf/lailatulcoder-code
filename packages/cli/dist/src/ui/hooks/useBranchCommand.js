...
import { SessionStartSource, computeUniqueBranchTitle, } from '@lailatul-coder/lailatul-coder-core';
...
const BACKGROUND_WORK_BRANCH_BLOCKED_MESSAGE = "Stop the current session's running background tasks before branching the conversation.";
/**
 ...
 */
export function useBranchCommand(options) {
    const { config, historyManager, startNewSession, clearPendingState, setSessionName, remount, } = options;
    const handleBranch = useCallback(async (name) => {
        if (!config)
            return;
        if (hasBlockingBackgroundWork(config)) {
            historyManager.addItem({
                type: 'error',
                text: buildBackgroundWorkBlockedMessage(config, t(BACKGROUND_WORK_BRANCH_BLOCKED_MESSAGE)),
            }, Date.now());
            return;
        }
        ...
        try {
            ...
            //    the incomplete fork.
            const baseName = name ?? deriveFirstPrompt(provisional.conversation.messages);
            const effectiveTitle = await computeUniqueBranchTitle(baseName, sessionService);
            const titlePersisted = await sessionService.renameSession(newSessionId, effectiveTitle, name ? 'manual' : 'auto');
            if (!titlePersisted) {
            ...
            const collapseOnResume = options.settings.merged.ui?.history?.collapseOnResume ?? false;
            const collapsePreviewCount = options.settings.merged.ui?.history?.collapsePreviewCount ?? 0;
            ...
            historyManager.addItem({
                type: 'info',
                text: t('Branched conversation{{titleInfo}}. You are now in the branch.', { titleInfo }),
            }, Date.now());
            historyManager.addItem({
                type: 'info',
...
//# sourceMappingURL=useBranchCommand.js.map