...
import { logSkillLaunch, recordSkillInvocation, SkillLaunchEvent, } from '../telemetry/index.js';
...
import { buildSkillLlmContent, applySkillAllowedTools, collectAvailableSkillEntries, clearCollectedSkillEntriesCache, } from './skill-utils.js';
/**
...
export class SkillTool extends BaseDeclarativeTool {
    ...
    constructor(config) {
        ...
        super(SkillTool.Name, ToolDisplayNames.SKILL, SKILL_TOOL_DESCRIPTION, // Static; live skill list is injected via system-reminders.
    ...
    async refreshSkills() {
        try {
            ...
            const collected = await collectAvailableSkillEntries(this.skillManager, this.config);
    ...
    validateToolParams(params) {
        ...
        const commandExists = this.modelInvocableCommands.some((cmd) => cmd.name === params.skill);
        if (commandExists)
            return null;
    ...
    createInvocation(params) {
        return new SkillToolInvocation(this.config, this.skillManager, params, (name) => this.loadedSkillNames.add(name), this.config.getModelInvocableCommandsExecutor(), (name) => this.loadedSkillNames.has(name));
    }
...
class SkillToolInvocation extends BaseToolInvocation {
    ...
    promptId = '';
    constructor(config, skillManager, params, onSkillLoaded, commandExecutor = null, isSkillLoaded = () => false) {
        super(params);
        this.config = config;
        this.skillManager = skillManager;
        this.onSkillLoaded = onSkillLoaded;
        this.commandExecutor = commandExecutor;
        this.isSkillLoaded = isSkillLoaded;
    }
    ...
    async recordAutoSkillUsageBestEffort(skill) {
        try {
            await recordAutoSkillUsage(this.config.getProjectRoot(), skill);
        }
        catch (error) {
            debugLogger.warn(`Failed to record auto-skill usage: ${error instanceof Error ? error.message : String(error)}`);
        }
    }
    async execute(_signal, _updateOutput) {
        ...
        if (disabled) {
            ...
            logSkillLaunch(this.config, new SkillLaunchEvent(this.params.skill, false, this.promptId));
        ...
        try {
            ...
            if (!skill) {
                ...
                if (this.commandExecutor) {
                    ...
                    const commandResult = await this.commandExecutor(this.params.skill, this.params.args ?? '');
                    if (commandResult &&
                    ...
                    if (typeof commandResult === 'string') {
                        logSkillLaunch(this.config, new SkillLaunchEvent(this.params.skill, true, this.promptId));
            ...
            applySkillAllowedTools(this.config.getPermissionManager(), skill.allowedTools);
            ...
            if (skill.hooks) {
                ...
                if (hookSystem && sessionId) {
                    ...
                    if (hookCount > 0) {
                        debugLogger.info(`Registered ${hookCount} hooks from skill "${this.params.skill}"`);
                    }
                    else {
