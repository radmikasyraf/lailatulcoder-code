...
import { existsSync, mkdirSync, mkdtempSync, rmSync, symlinkSync, utimesSync, writeFileSync, appendFileSync, } from 'node:fs';
...
import { findingsPointerOf, promptRecordDir, readRecordedPrompts, } from './lib/prompt-record.js';
import { DEADLINE_ENV, RESERVE_ENV, TOOL_CONCURRENCY_ENV, readBudgetStop, writeRoundCapStop, } from './lib/deadline.js';
...
const DRY_EN_PERIOD = 'No new issues were found. Re-walked the retry cap and both changed ' +
...
const DRY_ZH_COMMA = '未发现新问题，重新走查了重连状态机与两个已改导出的全部调用点，' +
    '每个疑点都已在确认清单中。';
// The canonical shape the classifier accepts — the wiring control.
const DRY_CANONICAL = 'No new issues found — re-walked the retry cap and both changed ' +
    "exports' call sites; every gap I checked was already in the confirmed " +
    'list.';
...
describe('issue #9206 — retirement must retire twice-dry chunks, or say why it cannot', () => {
    ...
    function runRound(round) {
        ...
        const out = writeStdoutLine.mock.calls
            .map((c) => String(c[0]))
            .join('\n');
        const err = writeStderrLine.mock.calls
            .map((c) => String(c[0]))
            .join('\n');
     ...
     */
    function auditorTranscript(launchPrompt, finalText, chunk) {
        const id = `aud-${++seq}`;
     ...
     */
    function mentionsChunk(out, id) {
        return (new RegExp(`chunk ${id} — retired`).test(out) ||
            new RegExp(`chunk ${id}[^\\n]*(?:certif|receipt|territory|transcript)`, 'i').test(out) ||
            new RegExp(`(?:certif|receipt|territory|transcript)[^\\n]*chunk ${id}\\b`, 'i').test(out));
    }
    ...
    ])('twice-dry chunks with %s retire at round 3 — or the builder says why certification failed', (_label, receipt) => {
        twiceDry(receipt);
        const r3 = runRound(3);
    ...
    it('the silence is not one round: across rounds 3-5 the twice-dry chunks are retired or diagnosed at least once', () => {
        ...
        for (const round of [3, 4, 5]) {
            ...
            for (const m of out.matchAll(/— chunk (\d+)(?: \(cold check\))? ─/g)) {
                ...
                auditorTranscript(recordOf(round, chunkId), chunkId <= 14 ? DRY_EN_PERIOD : yieldFor(round), chunkId);
            }
...
describe('issue #9206 — Step 9 cleanup must not destroy a non-converged run’s certification history', () => {
    ...
    it('a previous run\u2019s marked stop survives a retry at the same plan path (#9206)', () => {
        ...
        const planPath = join(dir, '.qwen', 'tmp', 'qwen-review-pr-9206-fetch.json');
        writeFileSync(planPath, JSON.stringify({ prNumber: '9206' }));
        const recordDir = promptRecordDir(planPath);
        mkdirSync(recordDir, { recursive: true });
        writeFileSync(join(recordDir, 'reverse-audit--chunk-1--round-1--abc123.txt'), 'a recorded launch prompt — the certification history');
    ...
    it('one unstatable record entry does not veto the previous-run evidence (#9213)', () => {
        ...
        mkdirSync(recordDir, { recursive: true });
        writeFileSync(join(recordDir, 'reverse-audit--chunk-1--round-1--abc123.txt'), 'a recorded launch prompt — the certification history');
        symlinkSync(join(dir, 'does-not-exist'), join(recordDir, 'a-broken-symlink'));
        writeFileSync(planPath, JSON.stringify({ prNumber: '9206' }));
        const fresh = new Date(Date.now() + 60 * 60 * 1000);
        utimesSync(planPath, fresh, fresh);
        runCleanup('pr-9206');
    ...
    it('records NEWER than the plan are this run\u2019s — a single run still sweeps (#9213)', () => {
        ...
        const recordDir = promptRecordDir(planPath);
        mkdirSync(recordDir, { recursive: true });
        writeFileSync(join(recordDir, 'reverse-audit--chunk-1--round-1--abc123.txt'), 'this run\u2019s own record');
        runCleanup('pr-9206');
...
//# sourceMappingURL=issue-9206-repro.test.js.map