...
function getWorktreeDirName(branchName: string): string {
  return branchName
    .replace(/[\\/]+/g, '--')
...
async function ensureWorktreesGitignored(repoRoot: string): Promise<void> {
  const qwenDir = join(repoRoot, '.qwen')
  await mkdir(qwenDir, { recursive: true })
  await writeFile(
    join(qwenDir, '.gitignore'),
...
export function registerWorkspaceCoreHandlers(server: RpcServer, deps: HandlerDeps): void {
  ...
  server.handle(RPC_CHANNELS.workspaces.GET, async () => sessionManager.getWorkspaces())
  ...
  server.handle(RPC_CHANNELS.workspaces.CREATE_PERMANENT_WORKTREE, async (_ctx, workspaceId: string, branchNameInput: string) => {
    ...
    if (sourceWorkspace.remoteServer) {
      throw new Error('Cannot create a local git worktree from a remote workspace.')
    }

    const workspaceConfig = loadWorkspaceConfig(sourceWorkspace.rootPath)
    const sourceDir = workspaceConfig?.defaults?.workingDirectory || sourceWorkspace.rootPath
    const validation = isValidWorkspaceRootPath(sourceDir)
    if (!validation.valid) {
      throw new Error(validation.reason!)
    }
    if (/[\\/]\.qwen[\\/]worktrees[\\/]/.test(sourceDir)) {
      throw new Error('Already inside a git worktree. Create the new worktree from the main checkout.')
    }

    let repoRoot: string
    try {
      repoRoot = await git(sourceDir, ['rev-parse', '--show-toplevel'])
    } catch {
      throw new Error(`Cannot create a worktree: ${sourceDir} is not a git repository.`)
    }

    try {
      await git(repoRoot, ['check-ref-format', `refs/heads/${branchName}`])
    } catch (error) {
      throw new Error(`Invalid branch name: ${getGitErrorMessage(error)}`)
    }

    if (await localBranchExists(repoRoot, branchName)) {
      throw new Error(`Branch ${branchName} already exists. Choose a different branch name.`)
    }

    const worktreesDir = join(repoRoot, '.qwen', WORKTREES_DIR)
    const worktreePath = join(worktreesDir, getWorktreeDirName(branchName))
    if (existsSync(worktreePath)) {
      throw new Error(`Worktree already exists at ${worktreePath}`)
    }

    await ensureWorktreesGitignored(repoRoot).catch((error: unknown) => {
      deps.platform.logger.warn(`Failed to update .qwen/.gitignore for worktrees: ${error}`)
    })
    ...
    try {
      ...
      throw new Error(`Failed to create worktree: ${getGitErrorMessage(error)}`)
    }

    const workspace = addWorkspace({ name: branchName, rootPath: worktreePath })
    setActiveWorkspace(workspace.id)
    deps.platform.logger.info(`Created permanent worktree workspace "${branchName}" at ${worktreePath}`)
    return workspace
  })
  ...
  server.handle(RPC_CHANNELS.window.GET_MODE, () => 'main')
  ...
  server.handle(RPC_CHANNELS.views.SAVE, async (_ctx, workspaceId: string, views: Array<import('@craft-agent/shared/views').ViewConfig>) => {
    const workspace = getWorkspaceByNameOrId(workspaceId)
    if (!workspace) throw new Error('Workspace not found')
