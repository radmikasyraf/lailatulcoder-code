...
vi.mock('node:fs', async (importOriginal) => {
    ...
    return {
        ...actual,
        statSync: ((p, ...rest) => p === '/c/qwen-repro'
            ? { isDirectory: () => true }
            : actual.statSync(p, ...rest)),
    };
});
describe('validateAndCanonicalizeWorkspaceInput inside a POSIX container sandbox (#7139)', () => {
    ...
    it.skipIf(process.platform === 'win32')('boots a Windows-shaped --workspace via its bind-mount location', () => {
        vi.stubEnv('SANDBOX', 'lailatul-coder-sandbox-0');
        _setSandboxMountExistsForTest((p) => p === '/c/qwen-repro');
        expect(validateAndCanonicalizeWorkspaceInput('C:\\qwen-repro')).toBe('/c/qwen-repro');
    });
    it.skipIf(process.platform === 'win32')('still rejects a Windows-shaped --workspace outside a sandbox', () => {
        vi.stubEnv('SANDBOX', '');
        expect(() => validateAndCanonicalizeWorkspaceInput('C:\\qwen-repro')).toThrow('must be an absolute path');
    });
    it('echoes the operator-typed input in the rejection, not the translation result', () => {
        ...
        expect(() => validateAndCanonicalizeWorkspaceInput('relative/path')).toThrow('Invalid --workspace "relative/path": must be an absolute path.');
    });
});
//# sourceMappingURL=run-qwen-serve.sandbox.test.js.map