 ...
 */
async function validateResolvedHost(hostname, allowPrivateNetworkHosts = false) {
...
export class HttpHookRunner {
    ...
    executedOnceHooks = new Set();
    statusMessageCallback;
    constructor(allowedUrls, allowPrivateNetworkHosts = false) {
     ...
     */
    setStatusMessageCallback(callback) {
     ...
     */
    async execute(hookConfig, eventName, input, signal) {
        ...
        if (hookConfig.once) {
            ...
            if (this.executedOnceHooks.has(onceKey)) {
                debugLogger.debug(`Skipping once hook ${hookId} - already executed for ${eventName}`);
                return {
                    hookConfig,
                    eventName,
                    success: true,
        ...
        try {
            ...
            const url = interpolateUrl(hookConfig.url, hookConfig.allowedEnvVars || []);
            ...
            const hostValidation = await validateResolvedHost(parsed.hostname, this.allowPrivateNetworkHosts);
            ...
            const headers = hookConfig.headers
                ? interpolateHeaders(hookConfig.headers, hookConfig.allowedEnvVars || [])
                : {};
            ...
            const { signal: combinedSignal, cleanup } = combineAbortSignals([signal], { timeoutMs: timeout });
            try {
                ...
                if (!response.ok) {
                    debugLogger.warn(`HTTP hook ${hookId} returned non-2xx status ${response.status} (non-blocking)`);
                ...
                debugLogger.debug(`HTTP hook ${hookId} completed successfully in ${duration}ms`);
                return {
                    hookConfig,
                    eventName,
                    success: true,
                ...
                if (fetchError instanceof Error &&
                    ...
                    debugLogger.warn(`HTTP hook ${hookId} timed out or was aborted after ${timeout}ms (non-blocking)`);
                    return {
                        hookConfig,
                        eventName,
                        success: true,
                ...
                debugLogger.warn(`HTTP hook ${hookId} connection failed (non-blocking): ${fetchError instanceof Error ? fetchError.message : String(fetchError)}`);
                return {
                    hookConfig,
                    eventName,
                    success: true,
     ...
     */
    async parseResponse(response, eventName) {
    ...
    truncateOutput(output) {
        ...
        debugLogger.debug(`Output truncated from ${output.length} to ${MAX_OUTPUT_LENGTH} characters`);
        return `${truncated}\n... [truncated, ${output.length - MAX_OUTPUT_LENGTH} more characters]`;
    }
    /**
     * Normalize response JSON into HookOutput format
     */
    normalizeOutput(json, eventName) {
        const output = {};
        // Map standard fields
        ...
        if ('hookSpecificOutput' in json &&
            ...
            if ('additionalContext' in hookOutput &&
                ...
                hookOutput['additionalContext'] = this.truncateOutput(hookOutput['additionalContext']);
            }
     ...
     * Update allowed URLs
     */
    updateAllowedUrls(allowedUrls) {
        // Create new validator with updated patterns
        this.urlValidator = new UrlValidator(allowedUrls, this.allowPrivateNetworkHosts);
    }
}
//# sourceMappingURL=httpHookRunner.js.map