...
import { addChannelMemoryEntries, clearChannelMemory, getChannelMemoryRevision, listChannelMemoryEntries, nextFireTime, readChannelMemory, recordChannelMemoryRecallMetrics, removeChannelMemoryEntries, updateChannelMemoryEntry, } from '@lailatul-coder/lailatul-coder-core';
import { loadSettings } from '../../config/settings.js';
import { scrubAndReportInheritedLoaderEnv } from '../../config/shared-env-keys.js';
import { ChannelLoopScheduler, ChannelLoopStore, DaemonChannelBridge, isChannelProactiveDeliveryError, sanitizeLogText, SessionRouter, } from '@lailatul-coder/channel-base';
import { normalizeServeChannelSelection } from '../../serve/channel-selection.js';
import { CHANNEL_DAEMON_WORKER_SENTINEL, CHANNEL_WORKER_HEARTBEAT_INTERVAL_MS, QWEN_DAEMON_TOKEN_ENV, QWEN_DAEMON_URL_ENV, QWEN_DAEMON_WORKSPACE_ENV, QWEN_SERVER_TOKEN_ENV, } from '../../serve/channel-worker-env.js';
...
import { ChannelDeliveryError, isChannelDeliveryError, isChannelDeliveryMessage, MAX_CHANNEL_DELIVERIES_IN_FLIGHT, } from '../../runtime/channel-delivery-ipc.js';
import { sanitizeWorkerDiagnostic } from '../../serve/channel-worker-diagnostics.js';
import { isChannelStartupReportAckMessage, MAX_CHANNEL_STARTUP_FAILURES, MAX_CHANNEL_STARTUP_FAILURE_CHANNEL_LENGTH, MAX_CHANNEL_STARTUP_FAILURE_CODE_LENGTH, MAX_CHANNEL_STARTUP_FAILURE_MESSAGE_LENGTH, } from '../../serve/channel-worker-startup-ipc.js';
...
import { createChannel, daemonChannelLoopPath, daemonChannelStateDir, daemonObservedContactsPath, daemonSessionRoutesPath, loadChannelsConfig, loadChannelsFromExtensions, parseConfiguredChannels, registerBackgroundResponseRelay, registerPermissionRelay, registerSessionCleanup, registerToolCallDispatch, selectFirstModel, } from './runtime.js';
import { BridgeChannelMemoryIntentClassifier } from './memory-intent-classifier.js';
import { OBSERVED_CONTACT_MAX_FRESH_WITHIN_SECONDS, ObservedChannelContactStore, } from './observed-contact-store.js';
...
export function createDaemonSessionFactory({ client, DaemonSessionClient, clientId, }) {
    return async (req) => {
        const daemonReq = {
        ...
        if (req.sessionId) {
            return await DaemonSessionClient.resume(client, req.sessionId, daemonReq, clientId);
        }
...
export function createDaemonChannelBridgeFacade(bridge, opts) {
    const facade = {
...
async function loadDaemonSdk() {
    return (await import('@lailatul-coder/sdk/daemon'));
}
...
function validateChannelWorkspaces(parsed, daemonWorkspace) {
    for (const { name, config } of parsed) {
        ...
        if (channelWorkspace !== daemonWorkspace) {
            throw new Error(`Channel "${name}" cwd "${channelWorkspace}" must use daemon workspace "${daemonWorkspace}".`);
        }
    }
}
...
function readConnectErrorMessage(error) {
...
export async function runChannelDaemonWorker(opts) {
    validateDaemonWorkerUrl(opts.daemonUrl);
    const startupSignal = opts.startupSignal;
    const sdk = await abortableStartup((opts.loadDaemonSdk ?? loadDaemonSdk)(), startupSignal);
    const client = new sdk.DaemonClient({
        baseUrl: opts.daemonUrl,
        ...(opts.daemonToken ? { token: opts.daemonToken } : {}),
    });
    const capabilities = await abortableStartup(client.capabilities(), startupSignal);
    ...
    if (capabilities.workspaces && capabilities.workspaces.length > 1) {
        ...
        const match = capabilities.workspaces.find((workspace) => canonicalizeWorkspace(workspace.cwd) === requestedWorkspace);
        if (!match) {
            throw new Error(`Worker workspace "${requestedWorkspace}" is not registered on the daemon.`);
        }
        if (!match.trusted) {
            throw new Error(`Worker workspace "${requestedWorkspace}" is not trusted; channels cannot run there.`);
        }
        daemonWorkspace = requestedWorkspace;
    }
    else {
        // Legacy single-workspace daemon: validate against the primary workspace.
        daemonWorkspace = canonicalizeWorkspace(capabilities.workspaceCwd ?? opts.workspace);
        if (requestedWorkspace !== daemonWorkspace) {
            throw new Error(`Daemon workspace "${daemonWorkspace}" does not match worker workspace "${requestedWorkspace}".`);
        }
    }
    ...
    const proxy = resolveProxyUrl(undefined, settings.merged.proxy);
    ...
    const parsed = await abortableStartup(parseConfiguredChannels(channelsConfig, names, {
    ...
    const observedContacts = new ObservedChannelContactStore(daemonObservedContactsPath(daemonWorkspace));
    ...
    const bridge = new DaemonChannelBridge({
        ...
        sessionFactory: createDaemonSessionFactory({
            ...
            clientId: `LailatulCoder-channel-worker:${process.pid}`,
        }),
        ...
        deleteSessionData: async (sessionId) => {
            ...
            if (!result.removed.includes(sessionId) &&
                !result.notFound.includes(sessionId)) {
                const detail = result.errors.find((entry) => entry.sessionId === sessionId)?.error;
    ...
    try {
        ...
        const bridgeFacade = createDaemonChannelBridgeFacade(bridge, {
            exposeShellCommand: capabilities.features.includes(SESSION_SHELL_COMMAND_FEATURE),
        });
        const createdRouter = new SessionRouter(bridgeFacade, daemonWorkspace, 'user', daemonSessionRoutesPath(daemonWorkspace), { recoveryMode: 'lazy' });
        ...
        writeStdoutLine(`[Channel] Restored ${restoredRoutes.restored} dormant route(s)` +
            (restoredRoutes.dropped > 0
                ? `; dropped ${restoredRoutes.dropped} invalid route(s)`
                : ''));
        for (const { name, config } of parsed) {
            throwIfStartupAborted(startupSignal);
            channels.set(name, await abortableStartup(createChannel(name, config, bridgeFacade, {
                ...(proxy ? { proxy } : {}),
                router: createdRouter,
                ...
                memoryIntentClassifier: new BridgeChannelMemoryIntentClassifier(bridgeFacade, config.cwd),
        ...
        for (const [name, channel] of channels) {
            ...
            try {
                ...
                writeStderrLine(`[Channel] Failed to connect "${safeName}": ${safeMessage}`);
                try {
                    channel.disconnect();
                }
                catch {
                ...
                if (connectFailureCount <= MAX_CHANNEL_STARTUP_FAILURES) {
                    const reportMessage = sanitizeWorkerDiagnostic(message, MAX_CHANNEL_STARTUP_FAILURE_MESSAGE_LENGTH, diagnosticRedaction) || 'Channel connection failed.';
                    const reportCode = code
                        ? sanitizeWorkerDiagnostic(code, MAX_CHANNEL_STARTUP_FAILURE_CODE_LENGTH, diagnosticRedaction)
                        : undefined;
                    await abortableStartup(opts.reportStartup?.({
                        type: 'channel_startup_failure',
                        failure: {
                            channel: sanitizeWorkerDiagnostic(name, MAX_CHANNEL_STARTUP_FAILURE_CHANNEL_LENGTH, diagnosticRedaction) || '<unnamed>',
                            phase: 'connect',
        ...
        if (loopStore) {
            const schedulerChannels = new Map();
                ...
                schedulerChannels.set(name, {
                    runLoopPrompt: async (job, options) => {
                        ...
                        if (jobWorkspace !== daemonWorkspace) {
                            ...
                            writeStderrLine(`[Channel] Disabled loop "${sanitizeLogText(job.id, 128)}": its workspace does not match this daemon worker.`);
                            throw new Error(`Loop ${sanitizeLogText(job.id, 128)} is outside daemon workspace and was disabled.`);
                        }
        ...
        return {
            ...
            async deliverChannelMessage(request) {
                ...
                if (!channel || !connected.includes(request.channelName)) {
                    throw new ChannelDeliveryError('channel_worker_unavailable', `Channel "${request.channelName}" is not running.`);
                }
                await channel.deliverProactive({ channelName: request.channelName, ...request.target }, request.text);
            },
...
function assertInternalDaemonWorkerInvocation() {
    ...
    if (!sentinel || sentinel === '1' || typeof process.send !== 'function') {
        scrubDaemonWorkerEnv();
        throw new Error('daemon-worker is an internal LailatulCoder serve command.');
    }
}
...
export const daemonWorkerCommand = {
    command: 'daemon-worker',
    describe: false,
    builder: (yargs) => yargs.option('channel', {
        type: 'string',
        array: true,
    ...
    handler: async (argv) => {
        ...
        let onWorkerMessage;
        const subscribeMessage = (listener) => {
        ...
        try {
            ...
            const { daemonToken, daemonUrl, promptAuthorization, workspace } = readDaemonWorkerEnv();
            ...
            scrubAndReportInheritedLoaderEnv(process.env, 'LailatulCoder channel daemon-worker', 'channel daemon worker');
            const send = process.send;
            channelLoopMcpHost = new ChannelLoopMcpWorkerHost((message, callback) => send.call(process, message, callback ?? (() => { })));
            onWorkerMessage = (message) => {
                if (channelLoopMcpHost?.handleMessage(message))
                    return;
            ...
            const handle = await runChannelDaemonWorker({
                ...
                reportStartup: (message) => reportStartupToSupervisor(message, startupAbortController.signal, subscribeMessage),
                channelLoopMcpHost,
                sendReady: (ready) => {
            ...
            let heartbeatTimer;
            const sendWebhookTaskResult = (id, result) => {
                try {
                    process.send?.({
            ...
            const sendChannelDeliveryResult = (id, result) => {
                try {
                    process.send?.({
            ...
            const activeWebhookTasks = new Map();
            const activeChannelDeliveries = new Map();
            const onMessage = (message) => {
                if (isChannelDeliveryMessage(message)) {
                    ...
                    if (activeChannelDeliveries.size >= MAX_CHANNEL_DELIVERIES_IN_FLIGHT) {
                        sendChannelDeliveryResult(message.id, {
                            ok: false,
                    ...
                    const delivery = handle
                        ...
                        sendChannelDeliveryResult(deliveryId, {
                            ...
                            error: sanitizeWorkerDiagnostic(error instanceof Error ? error.message : String(error), 512, {
                ...
                if (!isChannelWebhookTaskMessage(message))
                    return;
                if (message.expiresAt <= Date.now()) {
                ...
                try {
                    ...
                    sendWebhookTaskResult(message.id, {
                        ...
                        error: sanitizeLogText(err instanceof Error ? err.message : String(err), 512),
                    });
                    return;
                }
                ...
                const taskPromise = handle
                    ...
                    .catch((err) => {
                    const safeMessage = sanitizeLogText(err instanceof Error ? err.message : String(err), 512);
                    writeStderrLine(`[Channel] webhook task failed ` +
            ...
            const shutdown = async (reason) => {
                if (shuttingDown) {
                    ...
                    try {
                        ...
                        if (deliveryCount > 0) {
                            writeStderrLine(`[Channel] shutdown: draining ${deliveryCount} channel delivery task(s)...`);
                        }
                        if (webhookCount > 0) {
                            writeStderrLine(`[Channel] shutdown: draining ${webhookCount} webhook task(s)...`);
                        }
                        ...
                        const safeMessage = sanitizeLogText(err instanceof Error ? err.message : String(err), 512);
                        writeStderrLine(`[Channel] daemon worker failed to shut down after ${safeReason}: ${safeMessage}`);
                    }
                    finally {
                        clearHeartbeat();
...
function classifyWebhookTaskValidationError(error) {
    ...
    if (message === 'Webhook tasks require unattended approval mode.' ||
        message ===
    ...
    if (message.startsWith('Unknown webhook source "') ||
...
function classifyChannelDeliveryError(error) {
