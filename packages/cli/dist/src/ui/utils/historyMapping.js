 ...
 */
import { CompressionStatus, getStartupContextLength, isClearedMediaPlaceholder, isSystemReminderContent, } from '@lailatul-coder/lailatul-coder-core';
 ...
 */
export function isRealUserTurn(item) {
    if (item.type !== 'user' || !item.text)
        return false;
    if (typeof item.sentToModel === 'boolean')
        return item.sentToModel;
...
function isUserTextContent(content) {
    ...
    return content.parts.some((part) => 'text' in part && !!part.text && !isClearedMediaPlaceholder(part.text));
}
/**
 ...
 */
function findLastSuccessfulCompressionIndex(history) {
    return history.findLastIndex((item) => item.type === 'compression' &&
 ...
 */
export function computeApiTruncationIndex(uiHistory, targetUserItemId, apiHistory) {
    const targetIndex = uiHistory.findIndex((item) => item.id === targetUserItemId);
    if (targetIndex === -1)
        return -1;
    const compressionIndex = findLastSuccessfulCompressionIndex(uiHistory);
    if (compressionIndex !== -1 && targetIndex <= compressionIndex)
        return -1;
    // Count how many UI user turns exist before the target
    let uiUserTurnCount = 0;
    for (let i = compressionIndex === -1 ? 0 : compressionIndex + 1; i < targetIndex; i++) {
...
//# sourceMappingURL=historyMapping.js.map