...
describe('ACP dispatch parseOptionalWorkspaceCwd inside a POSIX container sandbox (#7139)', () => {
    ...
    it.skipIf(process.platform === 'win32')('accepts a Windows-shaped cwd and returns its bind-mount location', () => {
        ...
        expect(parseOptionalWorkspaceCwd({ cwd: 'C:\\qwen-repro' }, '/c/qwen-repro')).toBe('/c/qwen-repro');
    });
    it.skipIf(process.platform === 'win32')('still rejects a Windows-shaped cwd outside a sandbox', () => {
        vi.stubEnv('SANDBOX', '');
        expect(() => parseOptionalWorkspaceCwd({ cwd: 'C:\\qwen-repro' }, '/tmp')).toThrow('`cwd` must be an absolute path when provided');
    });
});
//# sourceMappingURL=dispatch.sandbox.test.js.map