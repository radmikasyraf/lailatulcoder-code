 ...
 */
import { createBuildWorkspaceCtx, MAX_SERVER_NAME_LENGTH, } from '../server/request-helpers.js';
...
async function runInCapturedGeneration(capture, task) {
...
async function runInRuntimeGeneration(runtime, task) {
    runtime.generationGuard?.assertOpen();
    const result = await task();
    runtime.generationGuard?.assertOpen();
    return result;
}
export function registerWorkspaceStatusRoutes(app, deps) {
    ...
    app.get('/workspace/mcp', async (_req, res) => {
        try {
            ...
            res
                .status(200)
                .json(await runInCapturedGeneration(deps.captureGenerationAssertion, () => workspace.getWorkspaceMcpStatus(ctx)));
        }
        catch (err) {
    ...
    app.get('/workspace/mcp/:server/tools', async (req, res) => {
        ...
        try {
            res
                .status(200)
                .json(await runInCapturedGeneration(deps.captureGenerationAssertion, () => bridge.getWorkspaceMcpToolsStatus(serverName)));
        }
        catch (err) {
    ...
    app.get('/workspace/mcp/:server/resources', async (req, res) => {
        ...
        try {
            res
                .status(200)
                .json(await runInCapturedGeneration(deps.captureGenerationAssertion, () => bridge.getWorkspaceMcpResourcesStatus(serverName)));
        }
        catch (err) {
    ...
    app.get('/workspace/skills', async (_req, res) => {
        try {
            ...
            res
                .status(200)
                .json(await runInCapturedGeneration(deps.captureGenerationAssertion, () => workspace.getWorkspaceSkillsStatus(ctx)));
        }
        catch (err) {
    ...
    app.post('/workspace/acp/preheat', mutate(), async (req, res) => {
        try {
            ...
            const timeoutMs = typeof timeoutMsRaw === 'string' && timeoutMsRaw.trim()
                ? Number(timeoutMsRaw)
                : undefined;
            ...
            res.status(200).json(await runInCapturedGeneration(deps.captureGenerationAssertion, () => workspace.preheatAcpChild(ctx, {
    ...
    app.get('/workspace/acp/status', async (_req, res) => {
        try {
            ...
            res
                .status(200)
                .json(await runInCapturedGeneration(deps.captureGenerationAssertion, () => workspace.getWorkspaceAcpStatus(ctx)));
        }
        catch (err) {
    ...
    app.get('/workspace/tools', async (_req, res) => {
        try {
            res
                .status(200)
                .json(await runInCapturedGeneration(deps.captureGenerationAssertion, () => bridge.getWorkspaceToolsStatus()));
        }
        catch (err) {
    ...
    app.get('/workspace/providers', async (_req, res) => {
        try {
            ...
            res
                .status(200)
                .json(await runInCapturedGeneration(deps.captureGenerationAssertion, () => workspace.getWorkspaceProvidersStatus(ctx)));
        }
        catch (err) {
...
export function registerWorkspaceQualifiedStatusRoutes(app, deps) {
    ...
    app.get('/workspaces/:workspace/mcp', async (req, res) => {
        ...
        try {
            res
                .status(200)
                .json(await runInRuntimeGeneration(runtime, () => runtime.workspaceService.getWorkspaceMcpStatus(ctx)));
        }
        catch (err) {
    ...
    app.get('/workspaces/:workspace/mcp/:server/tools', async (req, res) => {
        ...
        try {
            res
                .status(200)
                .json(await runInRuntimeGeneration(runtime, () => runtime.bridge.getWorkspaceMcpToolsStatus(serverName)));
        }
        catch (err) {
            sendBridgeError(res, err, { route });
        }
    ...
    app.get('/workspaces/:workspace/mcp/:server/resources', async (req, res) => {
        ...
        try {
            res
                .status(200)
                .json(await runInRuntimeGeneration(runtime, () => runtime.bridge.getWorkspaceMcpResourcesStatus(serverName)));
        }
        catch (err) {
            sendBridgeError(res, err, { route });
        }
    ...
    app.get('/workspaces/:workspace/skills', async (req, res) => {
        ...
        try {
            res
                .status(200)
                .json(await runInRuntimeGeneration(runtime, () => runtime.workspaceService.getWorkspaceSkillsStatus(ctx)));
        }
        catch (err) {
            sendBridgeError(res, err, { route });
        }
    ...
    app.get('/workspaces/:workspace/tools', async (req, res) => {
        ...
        try {
            res
                .status(200)
                .json(await runInRuntimeGeneration(runtime, () => runtime.bridge.getWorkspaceToolsStatus()));
        }
        catch (err) {
            sendBridgeError(res, err, { route });
        }
    ...
    app.get('/workspaces/:workspace/providers', async (req, res) => {
        ...
        try {
            res
                .status(200)
                .json(await runInRuntimeGeneration(runtime, () => runtime.workspaceService.getWorkspaceProvidersStatus(ctx)));
        }
        catch (err) {
            sendBridgeError(res, err, { route });
        }
...
export function registerWorkspaceDiagnosticStatusRoutes(app, deps) {
    ...
    app.get('/workspace/env', async (_req, res) => {
        try {
            ...
            res
                .status(200)
                .json(await runInCapturedGeneration(deps.captureGenerationAssertion, () => workspace.getWorkspaceEnvStatus(ctx)));
        }
        catch (err) {
    ...
    app.get('/workspace/preflight', async (_req, res) => {
        try {
            ...
            res
                .status(200)
                .json(await runInCapturedGeneration(deps.captureGenerationAssertion, () => workspace.getWorkspacePreflightStatus(ctx)));
        }
        catch (err) {
    ...
    app.get('/workspace/hooks', async (_req, res) => {
        try {
            ...
            res
                .status(200)
                .json(await runInCapturedGeneration(deps.captureGenerationAssertion, () => workspace.getWorkspaceHooksStatus(ctx)));
        }
        catch (err) {
...
export function registerWorkspaceQualifiedDiagnosticStatusRoutes(app, deps) {
    const { workspaceRegistry, sendBridgeError } = deps;
            ...
            try {
                ...
                switch (methodName) {
                    case 'getWorkspaceEnvStatus':
                        result = await runInRuntimeGeneration(runtime, () => runtime.workspaceService.getWorkspaceEnvStatus(ctx));
                        break;
                    case 'getWorkspacePreflightStatus':
                        result = await runInRuntimeGeneration(runtime, () => runtime.workspaceService.getWorkspacePreflightStatus(ctx));
                        break;
                    case 'getWorkspaceHooksStatus':
                        result = await runInRuntimeGeneration(runtime, () => runtime.workspaceService.getWorkspaceHooksStatus(ctx));
                        break;
                    default:
                        throw new Error(`Unsupported workspace status method: ${methodName}`);
                }
