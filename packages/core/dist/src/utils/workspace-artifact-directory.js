...
export function isOfficeDocumentExtension(ext) {
    return OFFICE_DOCUMENT_EXTENSIONS.has(ext);
}
export function shouldSkipDirectoryArtifactName(name) {
    return (name.startsWith('.') ||
        name.startsWith('~$') ||
        SKIP_DIRECTORY_ARTIFACT_NAMES.has(name));
}
const WORKTREE_ARTIFACT_PREFIX_RE = /^\.lailatulcoder\/worktrees\/[^/]+\//;
/**
 * Bound-root-canonical paths from a worktree session always start with
 * `.lailatulcoder/worktrees/<slug>/`. That leading `.lailatulcoder` must not itself trip the
 * skip-directory gate for ordinary subdirectories inside the worktree.
 */
export function stripWorktreeArtifactPrefix(workspacePath) {
    return workspacePath.replace(WORKTREE_ARTIFACT_PREFIX_RE, '');
}
export function pathHasSkippedDirectoryComponent(workspacePath) {
    return stripWorktreeArtifactPrefix(workspacePath)
        .split('/')
        .filter(Boolean)
        .some((segment) => shouldSkipDirectoryArtifactName(segment));
}
export async function collectRecordableWorkspaceFiles(absoluteDir, relativeDir, realWorkspace, isRecordable) {
    const files = [];
    const walked = await walkRecordableWorkspaceFiles(absoluteDir, relativeDir, realWorkspace, files, 0, isRecordable);
    return { files, ...walked };
}
async function walkRecordableWorkspaceFiles(absoluteDir, relativeDir, realWorkspace, files, depth, isRecordable) {
    ...
    if (depth > MAX_DIRECTORY_ARTIFACT_DEPTH) {
        return {
            truncated: false,
            depthLimited: await hasRecordableDescendant(absoluteDir, relativeDir, 8, isRecordable),
            unreadable: false,
            skippedUnrecordable: 0,
        };
    }
    ...
    for (const entry of entries) {
        ...
        if (entry.isDirectory()) {
            ...
            const nested = await walkRecordableWorkspaceFiles(absolutePath, relativePath, realWorkspace, files, depth + 1, isRecordable);
...
async function hasRecordableDescendant(absoluteDir, relativeDir, remainingDepth, isRecordable) {
    ...
    for (const entry of entries) {
        ...
        if (entry.isDirectory() && !shouldSkipDirectoryArtifactName(entry.name)) {
            try {
                if (await hasRecordableDescendant(path.join(absoluteDir, entry.name), relativePath, remainingDepth - 1, isRecordable)) {
                    return true;
                }
            }
            catch {
...
//# sourceMappingURL=workspace-artifact-directory.js.map