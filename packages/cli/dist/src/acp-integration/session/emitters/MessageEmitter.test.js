...
import { apiActivityTracker, } from '@lailatul-coder/lailatul-coder-core';
describe('MessageEmitter', () => {
    ...
    describe('emitUsageMetadata', () => {
        it('emits standard usage updates with the latest context occupancy, not a cumulative sum', async () => {
            await emitter.emitUsageMetadata({ promptTokenCount: 100, totalTokenCount: 175 }, '', 20);
            await emitter.emitUsageMetadata({ promptTokenCount: 120, totalTokenCount: 210 }, '', 30);
        ...
        it('does not replace main-session context usage with replay or subagent usage', async () => {
            ...
            expect(sendUpdateSpy.mock.calls
                .map(([update]) => update)
                .filter((update) => update.sessionUpdate === 'usage_update')).toEqual([]);
        ...
        it('keeps private usage metadata when the context window is unresolved', async () => {
            ...
            await new MessageEmitter(ctx).emitUsageMetadata({ promptTokenCount: 75 }, '', 10);
            expect(sendUpdateSpy).toHaveBeenCalledTimes(1);
            expect(sendUpdateSpy).toHaveBeenCalledWith(expect.objectContaining({
                sessionUpdate: 'agent_message_chunk',
...
//# sourceMappingURL=MessageEmitter.test.js.map