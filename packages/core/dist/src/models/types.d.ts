 ...
 */
export type ModelGenerationConfig = Pick<ContentGeneratorConfig, 'samplingParams' | 'timeout' | 'maxRetries' | 'retryInitialDelayMs' | 'retryMaxDelayMs' | 'retryErrorCodes' | 'enableCacheControl' | 'forceGlobalCacheScope' | 'cacheRetention' | 'cacheRetentionByBlock' | 'schemaCompliance' | 'reasoning' | 'customHeaders' | 'extra_body' | 'thinkingMandatory' | 'contextWindowSize' | 'modalities' | 'splitToolMedia' | 'toolResultContentFormat'>;
/**
 * Model configuration for a single model within an authType
 */
export interface ModelConfig {
    /** Unique model ID within authType (e.g., "lailatul-coderr", "gpt-4-turbo") */
    id: string;
