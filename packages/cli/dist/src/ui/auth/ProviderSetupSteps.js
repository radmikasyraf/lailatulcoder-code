...
function formatModelOptionLabel(model) { return model.id; }
...
function ModelIdsStep({ config, flow, }) {
    ...
    const handleFetchModels = useCallback(() => {
        ...
        fetchModelsFromApi(baseUrl, apiKey)
            .then(models => {
                ...
                setTimeout(() => {
                    const allKeys = models.map(m => m.id);
    ...
    useEffect(() => {
        if (flow.state.modelIds !== prevModelIds.current) {
            ...
            console.error("[SYNC] setting empty, modelIds=" + flow.state.modelIds); setCustomModelIdsText("");
        }
    ...
    const handleSubmitModelIds = useCallback(() => {
        flow.submitModelIds({
            primaryModelId: lastFocusedModelRef.current,
    ...
    return (_jsxs(Box, { marginTop: 1, flexDirection: "column", children: [_jsx(Box, { marginTop: 1, children: _jsx(Text, { color: theme.text.secondary, children: defaultIds
                        ...
                        : t('Enter model IDs separated by commas.') }) }), _jsx(Box, { marginTop: 1, children: _jsx(TextInput, { value: "", onChange: flow.changeModelIds, onSubmit: () => flow.submitModelIds({ primaryModelId: lastFocusedModelRef.current }), placeholder: defaultIds || 'model-id-1, model-id-2' }, "model-ids-input") }), _jsx(Box, { marginTop: 1, children: isFetching
                        ? _jsx(Text, { color: theme.text.accent, children: '⏳ Fetching models...' })
                        : _jsxs(Text, { children: [_jsx(Text, { color: theme.text.secondary, children: 'Or press ' }), _jsx(Text, { color: theme.status.success, bold: true, children: 'Ctrl+F' }), _jsx(Text, { color: theme.text.secondary, children: ' to fetch models from API' })] }) }), fetchError && (_jsx(Box, { marginTop: 1, children: _jsx(Text, { color: theme.status.error, children: fetchError }) })), fetchDone && (_jsx(Box, { marginTop: 1, children: _jsx(Text, { color: theme.status.success, children: '✅ Models fetched! Press Enter to continue.' }) })), flow.state.modelIdsError && (_jsx(Box, { marginTop: 1, children: _jsx(Text, { color: theme.status.error, children: flow.state.modelIdsError }) })), _jsx(NAV_HINT_INPUT, {})] }));
}
// ---------------------------------------------------------------------------
