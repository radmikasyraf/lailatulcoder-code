...
import { findProviderById, findProviderByCredentials, findExistingProviderModels, getDefaultModelIds, customProvider, ALIBABA_PROVIDERS, THIRD_PARTY_PROVIDERS, } from '@lailatul-coder/lailatul-coder-core';
...
const VIEW_TITLES = {
    ...
    'alibaba-select': t('LailatulCoder.Ai Authentication'),
...
export function AuthDialog() {
    ...
    const getExistingModelIds = (providerConfig) => {
        const saved = findExistingProviderModels(providerConfig, settings.merged.modelProviders);
        if (!saved)
            return [];
    ...
    const handleProviderSelect = (providerId) => {
        ...
        if (!providerConfig)
            return;
        setupFlow.start(providerConfig, undefined, existingEnv, getExistingModelIds(providerConfig));
    ...
    const handleMainSelect = (value) => {
        ...
        switch (value) {
            ...
            case 'CUSTOM_PROVIDER':
                setupFlow.start(customProvider, undefined, existingEnv, getExistingModelIds(customProvider));
                pushView('provider-setup');
                break;
            default:
                break;
