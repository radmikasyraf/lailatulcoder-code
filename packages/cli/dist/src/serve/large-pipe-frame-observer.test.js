...
import { classifyLargePipeFrame, createLargePipeFrameObserver, LARGE_PIPE_FRAME_EVENT_NAME, LARGE_PIPE_FRAME_THRESHOLD_BYTES, } from './large-pipe-frame-observer.js';
describe('large pipe frame observer', () => {
    ...
    it('classifies qwen/session/loadUpdates responses', () => {
        ...
        expect(context).toMatchObject({
            ...
            updateCount: 2,
            maxRawOutputApproxBytes: Buffer.byteLength(JSON.stringify({ type: 'task_execution' }), 'utf8'),
    ...
    it('handles cyclic object rawOutput without throwing from attribution', () => {
        ...
        expect(() => (context = classifyLargePipeFrame({
            direction: 'outbound',
            bytes: LARGE_PIPE_FRAME_THRESHOLD_BYTES,
            message: {
                jsonrpc: '2.0',
    ...
    it('caps shallow update and content traversal work', () => {
        let nestedContent = { type: 'text', text: 'too-deep' };
    ...
    it('rate limits large-frame logs and reports suppressed samples on the next log', () => {
        ...
        expect(emitTelemetryLog).toHaveBeenCalledWith('Large ACP pipe frame observed.', expect.objectContaining({
            suppressedCount: 1,
            suppressedWindowStartMs: 0,
    ...
    it('logs without a telemetry hook', () => {
        ...
        expect(daemonLog.warn).toHaveBeenCalledWith('large ACP pipe frame observed', expect.objectContaining({
            sourceClass: 'session_update_notification',
        }));
    });
...
//# sourceMappingURL=large-pipe-frame-observer.test.js.map