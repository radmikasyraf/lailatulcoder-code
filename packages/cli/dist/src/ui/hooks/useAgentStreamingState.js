...
import { AgentStatus, AgentEventType, isTerminalStatus, } from '@lailatul-coder/lailatul-coder-core';
 ...
 */
export function useAgentStreamingState(interactiveAgent, events) {
    ...
    const [lastPromptTokenCount, setLastPromptTokenCount] = useState(() => interactiveAgent?.getLastPromptTokenCount() ?? 0);
    ...
    useEffect(() => {
        ...
        const usageHandler = (event) => {
            const count = event?.usage?.promptTokenCount ?? event?.usage?.totalTokenCount;
...
//# sourceMappingURL=useAgentStreamingState.js.map