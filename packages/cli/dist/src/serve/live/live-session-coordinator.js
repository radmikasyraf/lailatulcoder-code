...
import { escapeXml, SessionService, stripTerminalControlSequences, } from '@lailatul-coder/lailatul-coder-core';
import { SessionArchivedError, SessionNotFoundError, } from '@lailatul-coder/acp-bridge/bridgeErrors';
import { buildQwenRealtimeInstructions, openQwenRealtimeSession, QwenRealtimeError, } from './LailatulCoder-realtime-session.js';
import { buildRealtimeStartupContext } from './realtime-startup-context.js';
import { isCompatibleLiveSessionSource, LIVE_SESSION_SOURCE_PREFIX, } from '../conversations/session-source.js';
...
function workerIdFromEvent(event) {
    ...
    if (typeof rawOutput !== 'string')
        return undefined;
    const match = /^\[🧵 ([A-Za-z0-9._:-]{1,8})\]\(LailatulCoder-session:\/\/([A-Za-z0-9._:-]{1,128})\) (?:started|completed(?: \(stopReason: [A-Za-z0-9._:-]{1,64}\))?)$/.exec(rawOutput);
...
export class LiveSessionCoordinator {
        ...
        const routed = runtime.bridge.enqueueMidTurnMessage(locator.sessionId, modelPrompt, undefined, undefined, {
            ...
            onSettledWithoutDrain: () => {
                void runAsNextTurn(false).catch((error) => {
                    if (this.isCurrentSocket(context, generation) &&
                        context.realtime === source) {
                        source.sendBackendContext(`The LailatulCoder Ai agent could not complete the request: ${errorMessage(error)}`);
                    }
                });
            },
        });
    ...
    async rollbackPreparedCoordinator(runtime, session, removeFreshTranscript) {
        ...
        if (removeFreshTranscript) {
            try {
                const transcriptRemoved = await new SessionService(runtime.workspaceCwd).removeSession(session.sessionId);
