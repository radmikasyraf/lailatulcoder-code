...
function createRegistry(conversationRuntime) {
...
function createWorkspace() {
    return {
        ...
        assertExactRoot: vi.fn(async (candidate) => {
...
function createOwnedRuntime(bridge = createBridge()) {
...
describe('ConversationRuntimeManager', () => {
    it('acquires ownership before touching the root or registry', async () => {
        ...
        expect(registry.getManagedEntryByWorkspaceCwd(root.canonicalRoot)).toBeUndefined();
    });
    ...
    it.each(['draining', 'blocked'])('rejects an existing %s entry without publishing a replacement', async (state) => {
        const candidate = createOwnedRuntime();
        const registry = createRegistry(candidate);
    ...
    ])('rejects a publication candidate with invalid $name', async ({ runtime }) => {
        const registry = createRegistry();
        const manager = createManager({
            workspace: createWorkspace(),
            registry,
    ...
    it('publishes with the revalidated canonical root', async () => {
        ...
        await expect(manager.ensure()).resolves.toBe(candidate);
        expect(publishRuntime).toHaveBeenCalledWith(canonicalRoot, expect.any(Function));
    });
...
//# sourceMappingURL=conversation-runtime-manager.test.js.map