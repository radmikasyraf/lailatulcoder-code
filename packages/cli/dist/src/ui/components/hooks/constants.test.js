...
import { HookEventName, HooksConfigSource, hookEventSupportsMatcher, } from '@lailatul-coder/lailatul-coder-core';
vi.mock('../../../i18n/index.js', () => ({
    t: vi.fn((key) => key),
}));
import { getHookExitCodes, getHookShortDescription, getHookDescription, getTranslatedSourceDisplayMap, createEmptyHookEventInfo, DISPLAY_HOOK_EVENTS, supportsMatchers, } from './constants.js';
describe('hooks constants', () => {
    ...
    describe('getHookExitCodes', () => {
        ...
        it('should return empty array for unknown event', () => {
            const exitCodes = getHookExitCodes('unknown_event');
    ...
    describe('getHookShortDescription', () => {
        ...
        it('should return description for SessionDelete', () => {
            expect(getHookShortDescription(HookEventName.SessionDelete)).toBe('After an explicitly selected session is deleted');
        });
        ...
        it('should describe the deleted session id for SessionDelete', () => {
            expect(getHookDescription(HookEventName.SessionDelete)).toContain('deleted_session_id');
        });
        ...
        it('should return description for PermissionDenied', () => {
            ...
            expect(desc).toBe('When a tool call is denied before a permission dialog is displayed');
        });
        it('should return empty string for unknown event', () => {
            const desc = getHookShortDescription('unknown_event');
            expect(desc).toBe('');
        });
    });
    describe('getHookDescription', () => {
        ...
        it('should return empty string for unknown event', () => {
            const desc = getHookDescription('unknown_event');
            expect(desc).toBe('');
        });
    });
    ...
    describe('supportsMatchers', () => {
        ...
        it('returns false for unknown events', () => {
            expect(supportsMatchers('unknown_event')).toBe(false);
        });
    ...
    describe('createEmptyHookEventInfo', () => {
        it('should create empty info for PreToolUse', () => {
            ...
            expect(info.description).toBe('Input to command is JSON of tool call arguments.');
        ...
        it('should create empty info for Stop', () => {
            ...
            expect(info.shortDescription).toBe('Right before LailatulCoder Ai concludes its response');
            expect(info.description).toBe('');
            expect(info.exitCodes).toHaveLength(3);
            expect(info.matcherGroups).toEqual([]);
        });
        it('should create empty info for unknown event', () => {
            const info = createEmptyHookEventInfo('unknown_event');
        ...
        it('should create empty info for PermissionDenied', () => {
            ...
            expect(info.shortDescription).toBe('When a tool call is denied before a permission dialog is displayed');
        ...
        it('should create empty info for TodoCompleted', () => {
            ...
            expect(info.shortDescription).toBe('When a todo item is marked as completed');
