...
export function isRepositoryContextRoleId(value) {
    return REPOSITORY_CONTEXT_ROLES.includes(value);
}
/**
 ...
 */
export const REVERSE_AUDIT_EXAMPLE_RECEIPT = "No issues found — re-walked the reconnect state machine and the two changed exports' call sites; every gap I checked was already in the list";
/**
...
\`\`\`bash
"\${QWEN_CODE_CLI:-LailatulCoder}" review base-tree --plan <the plan report> --worktree <this worktree> \\
  --out <the plan report's directory>/LailatulCoder-review-pr-<n>-base-tree.json
\`\`\`
...
\`\`\`bash
"\${QWEN_CODE_CLI:-LailatulCoder}" review extract-step --workflow <path in the tree being reviewed> \\
...
\`\`\`bash
"\${QWEN_CODE_CLI:-LailatulCoder}" review mock-provider --responder <a module you write> \\
...
"\${QWEN_CODE_CLI:-LailatulCoder}" review drive --cwd <the worktree> --script <what to run> \\
...
export function readsDiff(role) {
    return BRIEFS[role].readsDiff;
}
//# sourceMappingURL=agent-briefs.js.map