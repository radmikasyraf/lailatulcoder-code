...
const mockDaemonClient = vi.hoisted(() => vi.fn(() => ({ setChannelWorkerSelection: mockSetSelection })));
...
async function runHandler(argv) {
    if (!setCommand.handler)
        throw new Error('set handler missing');
    await setCommand.handler({ _: [], $0: 'qwen', ...argv });
}
describe('channel set command', () => {
    ...
    it('trims and deduplicates names while preserving first occurrence order', async () => {
        ...
        expect(mockSetSelection).toHaveBeenCalledWith({ mode: 'names', names: ['discord', 'telegram'] }, { timeoutMs: 1234 });
    ...
    it('rejects all mixed with named channels before loading the SDK', async () => {
        ...
        expect(mockWriteStderrLine).toHaveBeenCalledWith(expect.stringContaining('all'));
    ...
    it('prints remote errors and exits non-zero', async () => {
        ...
        expect(mockWriteStderrLine).toHaveBeenCalledWith(expect.stringContaining('start failed'));
        expect(process.exit).toHaveBeenCalledWith(1);
    });
    it('prints partial startup failures returned by a successful selection', async () => {
        ...
        await runHandler({ names: ['all'] });
        expect(mockWriteStdoutLine).toHaveBeenCalledWith('[Channel] Startup failure (workspace=/work, channel=feishu, phase=connect, code=ECONNREFUSED): connection refused');
    });
    it('prints structured startup failures from a 502 error body', async () => {
        mockSetSelection.mockRejectedValueOnce(Object.assign(new Error('worker failed'), {
            body: {
        ...
        await runHandler({ names: ['telegram'] });
        expect(mockWriteStderrLine).toHaveBeenCalledWith('[Channel] Startup failure (workspace=/work, channel=telegram, phase=connect): invalid token');
        expect(process.exit).toHaveBeenCalledWith(1);
    });
});
