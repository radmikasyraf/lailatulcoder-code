...
class SendMessageInvocation extends BaseToolInvocation {
    ...
    async execute(signal) {
        if (isPlanRequiredTeammateAwaitingApproval(this.config)) {
            const msg = getPlanRequiredTeammatePreApprovalMessage(ToolNames.SEND_MESSAGE);
            return {
                llmContent: msg,
                returnDisplay: msg,
                error: { message: msg },
        ...
        if (this.params.task_id) {
            ...
            if (entry.status === 'completed') {
                const continued = registry.continueResidentAgent(this.params.task_id, this.params.message);
                if (continued) {
                    return {
                ...
                const revived = await this.config.reviveCompletedBackgroundAgent(this.params.task_id, this.params.message);
                if (!revived) {
                    return {
            ...
            if (registry.isFinishing(this.params.task_id) ||
                !registry.queueMessage(this.params.task_id, this.params.message)) {
                const settled = await registry.waitForFinishing(this.params.task_id, signal);
                if (!settled) {
        ...
        if (!teamManager) {
            const msg = 'No active team and no task_id provided. ' +
        ...
        try {
            ...
            await teamManager.sendMessage(to, this.params.message, getAgentName() ?? LEADER_NAME, this.params.summary);
...
export class SendMessageTool extends BaseDeclarativeTool {
    ...
    constructor(config) {
        super(SendMessageTool.Name, ToolDisplayNames.SEND_MESSAGE, 'Send a message to a teammate (use "to") or to a running, paused, or completed background task (use "task_id"); completed tasks are revived. ' +
            ...
            'Your text output is NOT visible to peer teammates — use this tool to communicate.', Kind.Other, {
            type: 'object',
            properties: {
                to: {
                    type: 'string',
                ...
                type: {
                    ...
                    description: 'Structured message type for control flow. ' +
