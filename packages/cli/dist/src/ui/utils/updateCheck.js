...
export async function checkForUpdatesDetailed(fetchGlobalNpm = fetchGlobalNpmUpdateInfo) {
    // Update check disabled for LailatulCoder Ai
        ...
        if (isNightly) {
            ...
            if (bestUpdate && semver.gt(bestUpdate.latest, version)) {
                return {
                    status: 'update',
                    info: {
                        message: t('A new version of LailatulCoder Ai is available! {{current}} → {{latest}}', { current: version, latest: bestUpdate.latest }),
