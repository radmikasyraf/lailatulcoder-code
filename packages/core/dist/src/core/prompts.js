...
function getInteractionModePrompt(mode) {
    switch (mode) {
        case 'headless':
            return {
 ...
 */
function getDefaultCoreIdentitySentence(role) {
    return `You are an AI coding assistant. Follow any identity and personality instructions provided in the system context above. When a user greets you, detect the language and respond accordingly: If the greeting is in English (hi, hey, hello, hi bro, etc.), respond exactly: "Hi! I am LailatulCoder AI, your assistant. How can I help you with your coding, technology, or any other task today!" If the greeting is in Malay (hai, helo, assalamualaikum, etc.), respond exactly: "Hai! Saya LailatulCoder AI, pembantu anda. Bagaimana saya boleh bantu anda berkenaan coding, technology, atau sebarang tugasan lain hari ini?" Your primary goal is to help users safely and efficiently, adhering strictly to the following instructions and utilizing your available tools.`;
}
/**
 ...
 */
function resolveCoreIdentityOverride() {
 ...
 */
export function getCustomSystemPrompt(customInstruction, userMemory, appendInstruction) {
 ...
 */
export function getCoreSystemPrompt(userMemory, model, appendInstruction, interactionMode = 'interactive') {
    // if QWEN_SYSTEM_MD is set (and not 0|false), override system prompt from file
    // default path is .lailatulcoder/system.md (project-level), can be overridden via QWEN_SYSTEM_MD
    ...
    if (systemMdEnabled) {
        ...
        const coreIdentity = resolveCoreIdentityOverride() ??
...
function getToolCallExamples(model) {
    ...
    if (toolCallStyle) {
        switch (toolCallStyle.toLowerCase()) {
            ...
            default:
                debugLogger.warn(`Unknown QWEN_CODE_TOOL_CALL_STYLE value: ${toolCallStyle}. Using model-based detection.`);
                break;
        }
    }
    // Enhanced regex-based model detection
    if (model && model.length < 100) {
        // Match qwen*-coder patterns (e.g., qwen3-coder, qwen2.5-coder, lailatul-coderr)
 ...
 */
export function getPlanModeSystemReminder(planOnly = false) {
 ...
 */
export function getManualPlanExitSystemReminder(currentMode) {
    return `<system-reminder>
 ...
 */
export function getArenaSystemReminder(configFilePath) {
...
const INSIGHT_PROMPTS = {
    analysis: `Analyze this LailatulCoder Ai session and extract structured facets.
...
4. If very short or just warmup, use warmup_minimal for goal_category`,
    impressive_workflows: `Analyze this LailatulCoder Ai usage data and identify what's working well for this user. Use second person ("you").
...
Include 3 impressive workflows.`,
    project_areas: `Analyze this LailatulCoder Ai usage data and identify project areas.

Call respond_in_schema function with A VALID JSON OBJECT as argument:
{
  "areas": [
    {"name": "Area name", "session_count": N, "description": "2-3 sentences about what was worked on and how LailatulCoder Ai was used."}
  ]
}

Include 4-5 areas. Skip internal QC operations.`,
    future_opportunities: `Analyze this LailatulCoder Ai usage data and identify future opportunities.
...
Include 3 opportunities. Think BIG - autonomous workflows, parallel agents, iterating against tests.`,
    friction_points: `Analyze this LailatulCoder Ai usage data and identify friction points for this user. Use second person ("you").
...
Include 3 friction categories with 2 examples each.`,
    memorable_moment: `Analyze this LailatulCoder Ai usage data and find a memorable moment.
...
Find something genuinely interesting or amusing from the session summaries.`,
    improvements: `Analyze this LailatulCoder Ai usage data and suggest improvements.
...
2. **Custom Skills**: Reusable prompts you define as markdown files that run with a single /command.
   - How to use: Create \`.lailatulcoder/skills/commit/SKILL.md\` with instructions. Then type \`/commit\` to run it.
...
IMPORTANT for features_to_try: Pick 2-3 from the QC FEATURES REFERENCE above. Include 2-3 items for each category.`,
    interaction_style: `Analyze this LailatulCoder Ai usage data and describe the user's interaction style.

Call respond_in_schema function with A VALID JSON OBJECT as argument:
{
  "narrative": "2-3 paragraphs analyzing HOW the user interacts with LailatulCoder Ai. Use second person 'you'. Describe patterns: iterate quickly vs detailed upfront specs? Interrupt often or let Qwen run? Include specific examples. Use **bold** for key insights.",
  "key_pattern": "One sentence summary of most distinctive interaction style"
}
`,
    at_a_glance: `You're writing an "At a Glance" summary for a LailatulCoder Ai usage insights report for LailatulCoder Ai users. The goal is to help them understand their usage and improve how they can use Qwen better, especially as models improve.
...
3. **Quick wins to try** - Specific LailatulCoder Ai features they could try from the examples below, or a workflow technique if you think it's really compelling. (Avoid stuff like "Ask Qwen to confirm before taking actions" or "Type out more context up front" which are less compelling.)
