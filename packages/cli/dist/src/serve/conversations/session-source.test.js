...
import { classifyTopLevelConversationSource, isReservedLiveSessionSource, isReservedStandaloneSessionSource, readLoadableConversationSession, readLoadableLiveConversationMetadata, } from './session-source.js';
...
describe('conversation session source classification', () => {
    ...
    it('reserves Live and standalone source strings before full validation', () => {
        expect(isReservedLiveSessionSource({
            sourceType: 'default',
            sourceId: 'realtime_voice:',
        })).toBe(true);
        expect(isReservedStandaloneSessionSource({ sourceType: 'standalone' })).toBe(true);
    });
    it('classifies compatible top-level sources', () => {
        expect(classifyTopLevelConversationSource(records.get(LIVE_ID))).toMatchObject({ kind: 'live', persistence: 'explicit' });
        expect(classifyTopLevelConversationSource(records.get(LEGACY_ID))).toMatchObject({ kind: 'standalone', persistence: 'legacy' });
        expect(classifyTopLevelConversationSource(records.get(EXPLICIT_ID))).toMatchObject({ kind: 'standalone', persistence: 'explicit' });
        expect(classifyTopLevelConversationSource({
            sourceType: 'standalone',
            sourceId: 'unexpected',
        })).toBeUndefined();
    });
    ...
    ])('classifies %s as %s %s', async (sessionId, kind, persistence) => {
        await expect(readLoadableConversationSession(sessionId, store)).resolves.toMatchObject({ kind, persistence });
    });
    it.each([
    ...
    ])('reports the proven parent lineage for %s', async (sessionId, lineage) => {
        await expect(readLoadableConversationSession(sessionId, store)).resolves.toMatchObject({ parentSource: lineage });
    });
    it.each([EXPLICIT_ID, LEGACY_ID, LIVE_ID, EXPLICIT_CHILD_ID])('leaves the parent lineage unproven for %s', async (sessionId) => {
    ...
    it.each([
        ...
        await expect(readLoadableConversationSession(sessionId, store)).resolves.toBeUndefined();
    });
    it('keeps the compatibility adapter limited to Live and legacy projectless sessions', async () => {
        await expect(readLoadableLiveConversationMetadata(LIVE_CHILD_ID, store)).resolves.toEqual(records.get(LIVE_CHILD_ID));
        await expect(readLoadableLiveConversationMetadata(LEGACY_ID, store)).resolves.toEqual(records.get(LEGACY_ID));
        await expect(readLoadableLiveConversationMetadata(LEGACY_CHILD_ID, store)).resolves.toEqual(records.get(LEGACY_CHILD_ID));
        await expect(readLoadableLiveConversationMetadata(EXPLICIT_ID, store)).resolves.toBeUndefined();
        await expect(readLoadableLiveConversationMetadata(EXPLICIT_CHILD_ID, store)).resolves.toBeUndefined();
        await expect(readLoadableLiveConversationMetadata(LEGACY_CHILD_OF_EXPLICIT_ID, store)).resolves.toBeUndefined();
    });
    it('rejects missing and conflicting transcripts without reading metadata', async () => {
        ...
        await expect(readLoadableConversationSession(LEGACY_ID, unavailableStore)).resolves.toBeUndefined();
        await expect(readLoadableConversationSession(EXPLICIT_ID, unavailableStore)).resolves.toBeUndefined();
        expect(reads).toBe(0);
    });
    it('rejects a transcript that disappears while its metadata is read', async () => {
        ...
        await expect(readLoadableConversationSession(LEGACY_ID, disappearingStore)).resolves.toBeUndefined();
    });
    it('rejects a transcript whose creation metadata is unreadable', async () => {
        ...
        await expect(readLoadableConversationSession(LEGACY_ID, unreadableStore)).resolves.toBeUndefined();
        await expect(readLoadableLiveConversationMetadata(LEGACY_ID, unreadableStore)).resolves.toBeUndefined();
    ...
    it('reads archived transcripts with the archived state', async () => {
        ...
        await expect(readLoadableConversationSession(LEGACY_ID, archivedStore)).resolves.toMatchObject({ kind: 'standalone', persistence: 'legacy' });
        expect(states).toEqual(['archived']);
    });
});
//# sourceMappingURL=session-source.test.js.map