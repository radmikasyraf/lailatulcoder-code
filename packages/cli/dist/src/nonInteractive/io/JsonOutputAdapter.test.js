...
describe('JsonOutputAdapter', () => {
    ...
    describe('finalizeAssistantMessage', () => {
        ...
        it('should split different block types into separate assistant messages', () => {
            ...
            const storedMessages = adapter
                .messages;
            const assistantMessages = storedMessages.filter((msg) => {
            ...
            for (const assistant of assistantMessages) {
                const uniqueTypes = new Set(assistant.message.content.map((block) => block.type));
    ...
    describe('emitResult', () => {
        ...
        it('should emit success result as JSON array', () => {
            ...
            const resultMessage = parsed.find((msg) => typeof msg === 'object' &&
                msg !== null &&
                'type' in msg &&
                msg.type === 'result');
    ...
    describe('emitUserMessage', () => {
        it('should add user message to collection', () => {
            ...
            const output = stdoutWriteSpy.mock.calls[0][0];
            const parsed = JSON.parse(output);
            const userMessage = parsed.find((msg) => typeof msg === 'object' &&
                msg !== null &&
                'type' in msg &&
                msg.type === 'user');
    ...
    describe('emitToolResult', () => {
        it('should emit tool result message', () => {
            ...
            const output = stdoutWriteSpy.mock.calls[0][0];
            const parsed = JSON.parse(output);
            const toolResult = parsed.find((msg) => typeof msg === 'object' &&
                msg !== null &&
                'type' in msg &&
                msg.type === 'user' &&
                'message' in msg &&
        ...
        it('serializes a bounded tool result in JSON output', () => {
            ...
            const parsed = JSON.parse(stdoutWriteSpy.mock.calls[0][0]);
            const user = parsed.find((message) => message.type === 'user');
            const content = user.message.content[0].content;
            expect(Buffer.byteLength(JSON.stringify(content), 'utf8')).toBeLessThanOrEqual(HEADLESS_TOOL_RESULT_TEXT_JSON_BYTE_BUDGET);
        ...
        it('retains only the bounded preview while text output stays unchanged', () => {
            ...
            adapter.finalizeAssistantMessage();
            const storedMessages = adapter.messages;
    ...
    describe('emitSystemMessage', () => {
        it('should add system message to collection', () => {
            ...
            const output = stdoutWriteSpy.mock.calls[0][0];
            const parsed = JSON.parse(output);
            const systemMessage = parsed.find((msg) => typeof msg === 'object' &&
                msg !== null &&
                'type' in msg &&
                msg.type === 'system');
    ...
    describe('multiple messages in collection', () => {
        it('should collect all messages and emit as array', () => {
            ...
            const systemMsg = parsed[0];
            const userMsg = parsed[1];
            expect(systemMsg.type).toBe('system');
            expect(userMsg.type).toBe('user');
            expect(parsed.find((msg) => typeof msg === 'object' &&
                msg !== null &&
                'type' in msg &&
                msg.type === 'assistant')).toBeDefined();
            expect(parsed.find((msg) => typeof msg === 'object' &&
                msg !== null &&
                'type' in msg &&
                msg.type === 'result')).toBeDefined();
        });
    });
});
//# sourceMappingURL=JsonOutputAdapter.test.js.map