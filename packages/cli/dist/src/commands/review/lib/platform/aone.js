...
import { a1Json, a1JsonOnce, a1Once, ensureAoneAuthenticated, } from './aone-client.js';
...
function mrView(prNumber, ownerRepo) {
    const view = a1Json('repo', 'mr', 'view', String(prNumber), '--repo', ownerRepo);
    if (!view.mergeRequest) {
        throw new Error(`a1 returned no mergeRequest for #${prNumber} of ${ownerRepo}`);
    }
...
function mrHeadRefSpec(prNumber) {
 ...
 */
export function mrPresubmitFacts(prNumber, ownerRepo) {
    checkOwnerRepo(ownerRepo);
    const view = mrView(prNumber, ownerRepo);
    return {
        author: typeof view.author?.username === 'string'
...
function mrRepoPath(detailUrl) {
    if (!detailUrl)
        return undefined;
    const m = /^[a-z+]+:\/\/[^/]+\/(.+)\/codereview\/\d+(?:$|[/?#])/i.exec(detailUrl.trim());
    return m?.[1]?.toLowerCase();
}
export const aoneReader = {
    kind: 'aone',
    ...
    resolveRepo() {
        ...
        try {
            ...
            const cause = err.message
                ...
                .map((l) => l.trim())
                .find(Boolean) ?? '';
            throw new Error(`cannot resolve the repository: no \`origin\` remote` +
                (cause ? ` (${cause})` : ''));
        }
        ...
        if (!identity) {
            ...
            throw new Error(`cannot parse the origin remote ${JSON.stringify(redactUrl(url))} into group/project`);
        }
        ...
        if (!isAoneHostFamily(identity.host)) {
            throw new Error(`cannot resolve an Aone repository: this clone's origin is on ` +
    ...
    getClosingIssues(prNumber, ownerRepo) {
        ...
        const items = a1Json('repo', 'mr', 'workitem', 'list', '--mr', String(prNumber), '--repo', ownerRepo);
    ...
    getIssue(issueNumber, ownerRepo) {
        checkOwnerRepo(ownerRepo);
        const item = a1Json('project', 'workitem', 'get', String(issueNumber));
    ...
    fetchDiff(prNumber, ownerRepo) {
        ...
        if (!isPlainBranchName(target)) {
            throw new Error(`refusing target branch ${JSON.stringify(target)} from the MR ` +
        ...
        try {
            ...
            try {
                ...
                process.stderr.write(`WARNING: could not fetch origin/${target} — the merge-base is ` +
            ...
            try {
                ...
                base = git('merge-base', `refs/remotes/origin/${target}`, 
                ...
                process.stderr.write(`WARNING: no merge-base with origin/${target} — diffing the head ` +
            ...
            return gitRaw(...PINNED_DIFF_CONFIG, 'diff', ...PINNED_DIFF_FLAGS, 
            // Head side qualified — same shadow class as the merge-base reads.
            `${base}..refs/heads/${ref}`).toString('latin1');
        }
        finally {
            try {
                git('branch', '-D', ref);
    ...
    getCommentBody(kind, id, ownerRepo, prNumber) {
        ...
        if (prNumber === undefined) {
            throw new TypeError('aone comment bodies are addressed per-MR — pass `--pr <mr id>`');
        }
        // Aone has one flat comment collection per MR; the text is in `note`.
        const comments = a1Json('repo', 'mr', 'comment', 'list', '--mr', String(prNumber), '--repo', ownerRepo);
        ...
        if (!found) {
            throw new Error(`comment ${id} not found in MR ${prNumber} of ${ownerRepo}`);
        }
...
export class AonePartialPostError extends Error {
    ...
    summaryPosted;
    ambiguous;
    constructor(message, postedInline, inlineCommentIds, summaryPosted, ambiguous = false) {
        super(message);
        this.postedInline = postedInline;
        this.inlineCommentIds = inlineCommentIds;
        this.summaryPosted = summaryPosted;
...
function createMrComment(prNumber, ownerRepo, message, inline) {
    ...
    const out = a1JsonOnce('repo', 'mr', 'comment', 'create', '--mr', String(prNumber), '--repo', ownerRepo, ...(inline ? ['--file', inline.path, '--line', String(inline.line)] : []), '--message', message);
...
function a1Cause(err) {
    ...
    const cause = (stderr === undefined ? undefined : firstLine(stderr)) ??
 ...
 * the result says so.
 */
export function submitAoneReview(req) {
    ...
    if (liveHead !== '' && liveHead !== req.commitId) {
        throw new Error(`refusing to post: the MR head moved — the review was composed ` +
    ...
    const summaryMessage = req.event === 'REQUEST_CHANGES'
        ? `**Request changes**\n\n${req.body}`
        : req.body;
    const oversized = [
    ...
    if (oversized) {
        throw new Error(`refusing to post: ${oversized.what} is ` +
    ...
    let summaryCommentId;
    try {
        for (const c of req.comments) {
            postedIds.push(createMrComment(req.prNumber, req.ownerRepo, c.body, {
                path: c.path,
                line: c.line,
            }));
        }
        ...
        if (summaryMessage.trim() !== '') {
            summaryCommentId = createMrComment(req.prNumber, req.ownerRepo, summaryMessage);
            summaryPosted = true;
        }
    }
    catch (err) {
        ...
        const ids = postedIds.filter((n) => typeof n === 'number');
        ...
        const summaryFate = !summaryPosted && postedIds.length === req.comments.length
            ? `; the summary did NOT land`
            : '';
        throw new AonePartialPostError(`posting to MR ${req.prNumber} of ${req.ownerRepo} failed after ` +
            `${postedIds.length} of ${req.comments.length} inline comment(s)` +
            `${summaryPosted ? ' and the summary' : ''} landed` +
            `${summaryFate}: ` +
            a1Cause(err), postedIds.length, ids, summaryPosted, true);
    }
    let approved = false;
    let approveError;
    if (req.event === 'APPROVE') {
        try {
            a1Once('repo', 'mr', 'approve', String(req.prNumber), '--repo', req.ownerRepo);
            approved = true;
        }
        catch (err) {
    ...
    return {
        inlineCommentIds: postedIds.filter((n) => typeof n === 'number'),
