...
import type { ApiErrorEvent, ApiCancelEvent, ApiRequestEvent, ApiResponseEvent, FileOperationEvent, IdeConnectionEvent, StartSessionEvent, ToolCallEvent, UserPromptEvent, UserRetryEvent, FlashFallbackEvent, NextSpeakerCheckEvent, LoopDetectedEvent, RepeatedToolFailureGuardEvent, LoopDetectionDisabledEvent, SlashCommandEvent, ConversationFinishedEvent, KittySequenceOverflowEvent, ChatCompressionEvent, ContentRetryEvent, ContentRetryFailureEvent, ProtocolTagSanitizedEvent, ApiRetryEvent, RipgrepFallbackEvent, RipgrepRuntimeRecoveryEvent, ToolOutputTruncatedEvent, ExtensionDisableEvent, ExtensionEnableEvent, ExtensionUninstallEvent, ExtensionUpdateEvent, ExtensionInstallEvent, ModelSlashCommandEvent, SubagentExecutionEvent, MalformedJsonResponseEvent, InvalidChunkEvent, AuthEvent, SkillLaunchEvent, UserFeedbackEvent, ArenaSessionStartedEvent, ArenaAgentCompletedEvent, ArenaSessionEndedEvent, PromptSuggestionEvent, SpeculationEvent, WorkflowKeywordEvent, WorkflowRunEvent, MemoryExtractEvent, MemoryDreamEvent, MemoryRecallEvent, MemoryRecallDeliveryEvent } from './types.js';
 ...
 */
export declare function normalizeToolCallEvent(event: ToolCallEvent): NormalizedToolCallEvent;
export declare function logStartSession(config: Config, event: StartSessionEvent, previousSessionId?: string): void;
...
export declare function logApiRequest(config: Config, event: ApiRequestEvent, sessionId?: string): void;
...
export declare function logRipgrepRuntimeRecovery(config: Config, event: RipgrepRuntimeRecoveryEvent): void;
export declare function logApiError(config: Config, event: ApiErrorEvent, sessionId?: string): void;
export declare function logApiCancel(config: Config, event: ApiCancelEvent): void;
export declare function logApiResponse(config: Config, event: ApiResponseEvent, sessionId?: string): void;
export declare function logLoopDetected(config: Config, event: LoopDetectedEvent, options?: {
    recordToQwenLogger?: boolean;
}): void;
export declare function logRepeatedToolFailureGuard(event: RepeatedToolFailureGuardEvent): void;
...
export declare function logProtocolTagSanitized(config: Config, event: ProtocolTagSanitizedEvent): void;
...
export declare function recordSkillInvocation(config: Config, event: {
    skillName: string;
    success: boolean;
}): void;
...
export declare function logWorkflowKeyword(config: Config, event: WorkflowKeywordEvent): void;
export declare function logWorkflowRun(config: Config, event: WorkflowRunEvent): void;
export declare function logMemoryExtract(config: Config, event: MemoryExtractEvent): void;
export declare function logMemoryDream(config: Config, event: MemoryDreamEvent): void;
export declare function logMemoryRecall(config: Config, event: MemoryRecallEvent): void;
export declare function logMemoryRecallDelivery(config: Config, event: MemoryRecallDeliveryEvent): void;
