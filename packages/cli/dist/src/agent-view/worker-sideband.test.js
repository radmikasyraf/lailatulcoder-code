...
import { AGENT_VIEW_WORKER_ENV_KEYS, createAgentViewWorkerSidebandEnv, isAgentViewWorkerEnv, QWEN_AGENT_VIEW_ACTIVE_CWD, QWEN_AGENT_VIEW_SESSION_ID, QWEN_AGENT_VIEW_SIDEBAND, QWEN_AGENT_VIEW_TOKEN, QWEN_AGENT_VIEW_WORKER, readAgentViewWorkerSidebandEnv, readAgentViewWorkerControlEvents, reportAgentViewWorkerState, resetAgentViewWorkerStateReportForTests, sendAgentViewWorkerEvent, startAgentViewWorkerHeartbeat, } from './worker-sideband.js';
const mockCallAgentViewSupervisor = vi.hoisted(() => vi.fn(async () => ({ accepted: true })));
...
describe('worker sideband env', () => {
    ...
    it('detects worker mode only when explicitly enabled', () => {
        ...
        expect(isAgentViewWorkerEnv({ [QWEN_AGENT_VIEW_WORKER]: 'true' })).toBe(false);
    ...
    it('sends worker events through the configured sideband endpoint', async () => {
        ...
        await expect(sendAgentViewWorkerEvent({
            type: 'ready',
            cwd: '/repo',
            capabilities: ['ready'],
        }, env)).resolves.toEqual({ accepted: true });
        expect(mockCallAgentViewSupervisor).toHaveBeenCalledWith('/tmp/qwen-agent-view.sock', 'workerEvent', {
            type: 'ready',
            cwd: '/repo',
            capabilities: ['ready'],
            sessionId: 'session-1',
    ...
    it('reads worker control events through the configured sideband endpoint', async () => {
        ...
        expect(mockCallAgentViewSupervisor).toHaveBeenCalledWith('/tmp/qwen-agent-view.sock', 'workerControl', {
            sessionId: 'session-1',
            token: 'token-1',
        }, { timeoutMs: 1000 });
    });
    ...
    it('sends same-state reports when details change', async () => {
        ...
        await reportAgentViewWorkerState({ sessionState: 'working', summary: 'Running build' }, env);
        await reportAgentViewWorkerState({ sessionState: 'working', summary: 'Waiting for approval' }, env);
        expect(mockCallAgentViewSupervisor).toHaveBeenCalledTimes(2);
    });
    it('defaults worker state report cwd to the sideband active cwd', async () => {
        ...
        await reportAgentViewWorkerState({ sessionState: 'working' }, env);
        expect(mockCallAgentViewSupervisor).toHaveBeenCalledWith('/tmp/qwen-agent-view.sock', 'workerEvent', expect.objectContaining({
            cwd: '/repo',
        }));
    });
    ...
    it('serializes concurrent state reports before recording dedupe keys', async () => {
        ...
        mockCallAgentViewSupervisor
            ...
            .mockResolvedValueOnce({ accepted: true });
        const first = reportAgentViewWorkerState({ sessionState: 'working' }, env);
        const second = reportAgentViewWorkerState({ sessionState: 'needs_input' }, env);
        await Promise.resolve();
        expect(mockCallAgentViewSupervisor).toHaveBeenCalledTimes(1);
        resolveFirst({ accepted: true });
        await Promise.all([first, second]);
        await reportAgentViewWorkerState({ sessionState: 'working' }, env);
        expect(mockCallAgentViewSupervisor.mock.calls.map((call) => call[2]
            ?.sessionState)).toEqual(['working', 'needs_input', 'working']);
    });
    it('skips worker events, control reads, and heartbeats outside worker mode', async () => {
        await expect(sendAgentViewWorkerEvent({ type: 'heartbeat' }, {})).resolves.toBeUndefined();
    ...
    it('sends one event for concurrent identical state reports', async () => {
        ...
        let release = () => { };
        mockCallAgentViewSupervisor.mockImplementationOnce(() => new Promise((resolve) => {
            release = resolve;
        }));
        const first = reportAgentViewWorkerState({ sessionState: 'working' }, env);
        const second = reportAgentViewWorkerState({ sessionState: 'working' }, env);
    ...
    it('ignores heartbeat send failures', async () => {
        ...
        try {
            ...
            mockCallAgentViewSupervisor.mockRejectedValueOnce(new Error('supervisor unavailable'));
            const heartbeat = startAgentViewWorkerHeartbeat(env, 100);
            await vi.advanceTimersByTimeAsync(100);
            expect(mockCallAgentViewSupervisor).toHaveBeenCalledTimes(1);
            heartbeat?.dispose();
...
//# sourceMappingURL=worker-sideband.test.js.map