 ...
 * in `@lailatul-coder/lailatul-coder-core` so core-side emitters (e.g. the LailatulCoder Auth
 ...
 * label as-is.
 */
import { osc8Hyperlink, sanitizeForOsc, supportsHyperlinks, wrapForMultiplexer, HYPERLINK_ENV_KEYS, } from '@lailatul-coder/lailatul-coder-core';
 ...
 */
export function isSafeOscScheme(url) {
 ...
 */
export function trimTrailingUrlPunctuation(url, nextCharacter = '') {
 ...
 */
export function shouldWrapMarkdownLink(url, canHyperlink) {
 ...
 */
const HOST_LIKE_RE = /\b[a-z0-9](?:[a-z0-9-]{0,62}[a-z0-9])?(?:\.[a-z0-9](?:[a-z0-9-]{0,62}[a-z0-9])?)*\.[a-z]{2,}\b/gi;
...
export function labelMayDeceive(label, url) {
    if (label === url)
        return false;
