 ...
 */
import { InputFormat, ToolConfirmationOutcome, ToolNames, } from '@lailatul-coder/lailatul-coder-core';
...
export class PermissionController extends BaseController {
     ...
     */
    buildAllowConfirmationPayload(toolName, updatedInput) {
        if (!updatedInput ||
            typeof updatedInput !== 'object' ||
            Array.isArray(updatedInput)) {
            return undefined;
        }
        const updatedInputObj = updatedInput;
        const answers = toolName === ToolNames.ASK_USER_QUESTION
     ...
     */
    async handleTeammateApproval(event) {
        try {
            ...
            const response = await this.sendControlRequest({
                ...
                permission_suggestions: buildPermissionSuggestions(event.confirmationDetails),
                blocked_path: null,
            }, undefined, this.context.abortSignal);
            ...
            if (behavior === 'allow') {
                ...
                const respondPayload = this.buildAllowConfirmationPayload(event.toolName, payload['updatedInput']);
                await event.respond(ToolConfirmationOutcome.ProceedOnce, respondPayload);
            }
            else {
                const cancelMessage = typeof payload['message'] === 'string'
                    ? payload['message']
                    : undefined;
                await event.respond(ToolConfirmationOutcome.Cancel, cancelMessage
                    ? { cancelMessage }
                    : undefined);
            }
        }
        catch (error) {
            this.debugLogger.error('[PermissionController] Teammate approval failed:', error);
            ...
            try {
                ...
                this.debugLogger.error('[PermissionController] Teammate approval cancel failed:', cancelError);
            }
        }
    }
    async handleWorkflowApproval(runId, approval, rawArgs, approvalSignal) {
        ...
        try {
            const response = await this.sendControlRequest({
                ...
                input: rawArgs,
                permission_suggestions: buildPermissionSuggestions(approval.confirmationDetails),
                blocked_path: null,
            }, this.context.sdkCanUseToolTimeoutMs ?? DEFAULT_CAN_USE_TOOL_TIMEOUT_MS, signal);
            const payload = (response.response || {});
            const allowed = response.subtype === 'success' &&
                String(payload['behavior'] || '').toLowerCase() === 'allow';
            const confirmationPayload = allowed
                ? this.buildAllowConfirmationPayload(approval.name, payload['updatedInput'])
                : typeof payload['message'] === 'string'
                    ? { cancelMessage: payload['message'] }
                    : undefined;
            await registry.resolvePendingApproval(runId, approval.approvalId, allowed
                ? ToolConfirmationOutcome.ProceedOnce
                : ToolConfirmationOutcome.Cancel, confirmationPayload);
        }
        catch (error) {
            this.debugLogger.error('[PermissionController] Workflow approval failed:', error);
            await registry.resolvePendingApproval(runId, approval.approvalId, ToolConfirmationOutcome.Cancel);
        }
    }
    /**
     ...
     */
    async handleOutgoingPermissionRequest(toolCall, signal) {
        const requiresUserInteraction = toolCall.invocation?.requiresUserInteraction?.() === true;
        const interactionUnavailableMessage = toolCall.request.name === ToolNames.EXIT_PLAN_MODE
        ...
        try {
            ...
            if (response.subtype !== 'success') {
                await toolCall.confirmationDetails.onConfirm(ToolConfirmationOutcome.Cancel, requiresUserInteraction
                    ? {
                        cancelMessage: interactionUnavailableMessage,
                    }
                    : undefined);
            ...
            if (behavior === 'allow') {
                ...
                const confirmationPayload = this.buildAllowConfirmationPayload(toolCall.request.name, payload['updatedInput']);
                if (confirmationPayload) {
                    ...
                    await toolCall.confirmationDetails.onConfirm(ToolConfirmationOutcome.ProceedOnce, confirmationPayload);
                }
                else {
                    await toolCall.confirmationDetails.onConfirm(ToolConfirmationOutcome.ProceedOnce);
                }
