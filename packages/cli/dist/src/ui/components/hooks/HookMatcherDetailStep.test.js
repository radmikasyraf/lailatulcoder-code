...
function makeConfig(command, source = HooksConfigSource.User) {
    return {
...
describe('HookMatcherDetailStep', () => {
    ...
    it('renders the "Event - Matcher: <value>" title', () => {
        ...
        const { lastFrame } = render(_jsx(HookMatcherDetailStep, { hookEvent: hookEvent, matcherGroup: matcherGroup, selectedIndex: 0 }));
        expect(lastFrame()).toContain(`${HookEventName.PreToolUse} - Matcher: Bash`);
    });
    ...
    it('keeps the event description visible on the matcher detail page', () => {
        ...
        const out = render(_jsx(HookMatcherDetailStep, { hookEvent: hookEvent, matcherGroup: matcherGroup, selectedIndex: 0 })).lastFrame() ?? '';
    ...
    it('renders inline exit code descriptions on the matcher detail page', () => {
        ...
        expect(out).toContain('show stderr to user only but continue with tool call');
    });
    ...
    it('renders multiple handler rows with numbering', () => {
        ...
        const { lastFrame } = render(_jsx(HookMatcherDetailStep, { hookEvent: hookEvent, matcherGroup: matcherGroup, selectedIndex: 1 }));
        const out = lastFrame() ?? '';
...
//# sourceMappingURL=HookMatcherDetailStep.test.js.map