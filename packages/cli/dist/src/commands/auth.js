...
const shouldUseColor = () => Boolean(process.stdout.isTTY && !process.env['NO_COLOR']);
const color = (value, code) => shouldUseColor() ? `\x1b[${code}m${value}\x1b[0m` : value;
const cyan = (value) => color(value, '36');
const yellow = (value) => color(value, '33');
export const buildRemovalNotice = () => [
    '',
    yellow(t('⚠  LailatulCoder auth has been removed.')),
    '',
    `  ${cyan(t('Interactive'))}   →  ${t('run LailatulCoder and use /auth to configure providers')}`,
    ...
    `  ${cyan(t('LailatulCoder Auth'))}    →  ${t('run LailatulCoder interactively and use /auth; OAuth cannot be configured with env vars alone')}`,
    `  ${cyan(t('Scripted'))}      →  ${t('edit ~/.lailatulcoder/settings.json, or run LailatulCoder interactively once')}`,
    '',
    ...
    builder: (yargs) => {
        ...
        for (const name of legacySubcommands) {
            y = y.command({
                ...
                describe: false,
                builder: (subYargs) => subYargs.strict(false),
