 ...
 */
export function expressionsOf(...texts) {
    const seen = new Set();
    for (const t of texts) {
        ...
        while (i !== -1) {
            ...
            if (end === -1)
                break; // unterminated — no site to report
 ...
 */
function stripQuoted(line, open) {
    let live = '';
    let quote = open;
...
const HEREDOC_OPENER = /^<<-?\s*(?:'([A-Za-z_][\w-]*)'|"([A-Za-z_][\w-]*)"|([A-Za-z_][\w-]*))/;
 ...
 */
function commandSubstitutionsOf(line) {
    const bodies = [];
...
export function invokedCommandsOf(script) {
    const seen = new Set();
    ...
    const scanLogicalLine = (rawLine, openQuote) => {
        ...
        for (const body of commandSubstitutionsOf(masked))
            scanSegment(body);
    ...
    const heredocQueue = [];
    const terminates = (rawLine, h) => h.dash ? rawLine.trim() === h.word : rawLine === h.word;
    let openQuote = null;
    ...
    for (const rawLine of script.split('\n')) {
        if (heredocQueue.length > 0) {
            ...
            if (terminates(rawLine, heredocQueue[0]))
                heredocQueue.shift();
            continue;
        }
    ...
    if (pending !== null)
        scanLogicalLine(pending, openQuote);
    return [...seen].sort();
}
/**
 ...
 * resolves it.
 */
function mergeEnv(container, scope, env, sources) {
    const raw = container?.env;
 ...
 */
const SCOPE_ORDER = { step: 0, job: 1, workflow: 2 };
function nearestFirst(env, sources) {
    const ordered = {};
    for (const k of Object.keys(env).sort((a, b) => SCOPE_ORDER[sources[a]] - SCOPE_ORDER[sources[b]])) {
        ordered[k] = env[k];
    }
    return ordered;
}
export function runExtractStep(args) {
    ...
    try {
        ...
        throw new Error(`extract-step: cannot read ${args.workflow}: ${err.message}`);
    }
    let doc;
    try {
        doc = parse(text);
        ...
        throw new Error(`extract-step: cannot parse ${args.workflow}: ${err.message}`);
    }
    const job = doc?.jobs?.[args.job];
    if (!job) {
        throw new Error(`extract-step: no job \`${args.job}\` in ${args.workflow} — jobs: ${Object.keys(doc?.jobs ?? {}).join(', ') || '(none)'}`);
    }
    ...
    const byName = steps.flatMap((s, i) => s?.name === args.step || s?.id === args.step ? [i] : []);
    ...
    if (byName.length > 1) {
        throw new Error(`extract-step: step \`${args.step}\` is ambiguous in job \`${args.job}\` — ${byName.length} steps share that name (indices ${byName.join(', ')}); pass the index instead`);
    }
    ...
    if (!step) {
        const named = steps
            ...
            .join('; ');
        throw new Error(`extract-step: no step \`${args.step}\` in job \`${args.job}\` — steps: ${named || '(none)'}`);
    }
    if (typeof step.run !== 'string' || !step.run.trim()) {
        throw new Error(`extract-step: step \`${args.step}\` has no \`run:\` script (a \`uses:\` action cannot be extracted)`);
    }
    ...
    const declaredShell = nearestString(step.shell, jobDefaults.shell, workflowDefaults.shell);
    ...
    const setLine = declaredShell === 'bash'
        ? 'set -eo pipefail'
        : shellCommand === 'bash' || shellCommand === 'sh'
            ? 'set -e'
            : undefined;
    const workingDir = nearestScoped(['step', step['working-directory']], ['job', jobDefaults['working-directory']], ['workflow', workflowDefaults['working-directory']]);
    ...
    const header = [
        ...
        ...commentLines('# extracted verbatim from ', `${args.workflow} — job \`${args.job}\`, step \`${stepLabel}\``),
        ...(shell === shellCommand
            ? []
        ...
        ...(workingDir
            ? commentLines(`# working-directory [${workingDir.scope}] (run FROM here): `, workingDir.value)
            : []),
        ...Object.entries(env).flatMap(([k, v]) => commentLines(`# env [${envSources[k]}] ${k}=`, v)),
        '',
    ].join('\n');
    ...
    return {
        ...
        expressions: expressionsOf(script, ...Object.values(env), workingDirectory ?? '', shell),
        invokes: invokedCommandsOf(script),
        scriptPath: outPath,
    };
}
export const extractStepCommand = {
    command: 'extract-step',
    describe: "Extract one workflow step's run: script, verbatim, into an executable — with its env and ${{ }} stub list as metadata",
    builder: (yargs) => yargs
        .option('workflow', {
        type: 'string',
        demandOption: true,
        ...
        type: 'string',
        demandOption: true,
        describe: 'Step name, id, or 0-based index within the job (an all-digit value is always read as an index)',
    })
        .option('out', {
        type: 'string',
        demandOption: true,
    ...
    handler: (argv) => {
        ...
        try {
            const meta = runExtractStep(argv);
            writeStdoutLine(JSON.stringify(meta, null, 2));
            const inherited = Object.values(meta.envSources).filter((scope) => scope !== 'step').length;
            writeStderrLine(`extract-step: wrote ${meta.scriptPath} (${meta.expressions.length} \${{ }} site(s) to stub, ` +
...
//# sourceMappingURL=extract-step.js.map