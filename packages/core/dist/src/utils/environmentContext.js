 ...
 */
export function formatDateForContext(date = new Date()) {
...
export async function getEnvironmentContext(config) {
    ...
    const context = `
This is the LailatulCoder Ai. We are setting up the context for our chat.
...
function wrapSystemReminder(body) {
    return `${SYSTEM_REMINDER_OPEN}\n${escapeSystemReminderTags(body)}\n${SYSTEM_REMINDER_CLOSE}`;
}
function truncateDeferredToolDescription(description) {
...
function formatDeferredToolLine({ name, description, }) {
    return `- ${JSON.stringify(name)}: ${JSON.stringify(truncateDeferredToolDescription(description))}`;
}
function byName(a, b) {
    return a.name.localeCompare(b.name);
}
function buildDeferredToolsReminderBody(deferredTools, intro) {
    ...
    if (bundledTools.length > 0) {
        bodyParts.push(['### Bundled', ...bundledTools.map(formatDeferredToolLine)].join('\n'));
    }
...
function buildDeferredToolsReminderForSummary(deferredTools, intro) {
    const body = buildDeferredToolsReminderBody(deferredTools, intro);
    return body ? wrapSystemReminder(body) : null;
}
function formatQuotedNameLine(name) {
    return `- ${JSON.stringify(name)}`;
}
function formatAgentAvailabilityLine(agent) {
    return `- ${JSON.stringify(agent.name)}: ${JSON.stringify(truncateDeferredToolDescription(agent.description))}`;
}
export function buildDeferredToolsReminder(toolRegistry) {
    const deferredTools = toolRegistry
        .getDeferredToolSummary()
        .filter((tool) => !toolRegistry.isDeferredToolRevealed(tool.name));
    return buildDeferredToolsReminderForSummary(deferredTools, `The following tools are reachable via \`${ToolNames.TOOL_SEARCH}\`. Call with \`select:<name>\` or a keyword query.`);
}
export function buildAddedMcpToolsReminder(deferredTools) {
    return buildChangedMcpToolsReminder(deferredTools, []);
}
export function buildChangedMcpToolsReminder(addedTools, removedToolNames) {
    ...
    if (removed.length === 0) {
        return buildDeferredToolsReminderForSummary(mcpTools, `The following MCP tools became available after startup and are reachable via \`${ToolNames.TOOL_SEARCH}\`. Call with \`select:<name>\` or a keyword query.`);
    }
    const bodyParts = [
    ...
    if (mcpTools.length > 0) {
        const addedBody = buildDeferredToolsReminderBody(mcpTools, `The following MCP tools are now available and are reachable via \`${ToolNames.TOOL_SEARCH}\`. Call with \`select:<name>\` or a keyword query.`);
        if (addedBody) {
...
export function buildMcpServerInstructionsReminder(toolRegistry) {
    const serverInstructions = Array.from(toolRegistry.getMcpServerInstructions().entries())
    ...
    const bodyParts = [
        ...
        ...serverInstructions.map(([serverName, instructions]) => `### ${serverName}\n${instructions}`),
    ];
    return wrapSystemReminder(bodyParts.join('\n\n'));
}
...
function trimSkillEntriesTowardsBudget(entries) {
 ...
 * no SkillManager.
 */
export async function buildAvailableSkillsReminder(config) {
    ...
    try {
        ...
        debugLogger.warn('buildAvailableSkillsReminder: collectAvailableSkillEntries failed', error);
        return null;
    }
    if (entries.length === 0) {
        return {
            reminder: wrapSystemReminder('No skills are currently available. Skills can be added by creating directories with SKILL.md files or by configuring MCP servers with model-invocable prompts.'),
            renderedEntries: [],
        };
    }
 ...
 */
export function buildAddedSkillsReminder(entries) {
    return buildChangedSkillsReminder(entries, []);
}
export function buildChangedSkillsReminder(addedEntries, removedNames) {
...
export function buildAddedAgentsReminder(agents) {
...
export function buildChangedAgentsReminder(addedAgents, removedAgentNames) {
...
export async function buildStartupContextReminder(config) {
 ...
 */
export async function getInitialChatHistory(config, extraHistory, options = {}) {
    const toolRegistry = config.getToolRegistry();
    await toolRegistry.warmAll();
    const includeDeferredToolsReminder = options.includeDeferredToolsReminder ?? true;
    const includeAvailableSkillsReminder = options.includeAvailableSkillsReminder ?? true;
    ...
    const reminderParts = [
        ...
        .map((text) => ({ text }));
    const prelude = reminderParts.length === 0
        ? []
        : [
            {
                role: 'user',
 ...
 */
export function getStartupContextLength(history, options = {}) {
...
function detectCompressedPrefixLength(history, offset) {
    ...
    if (isPostCompactAttachmentEntry(history[offset + 2])) {
        if (isModelFunctionCallEntry(history[offset + 3]))
            return 4;
        return 3;
    }
    return 2;
}
function isPostCompactAttachmentEntry(content) {
    if (content?.role !== 'user')
        return false;
    const parts = content.parts ?? [];
    return parts.some((part) => typeof part.text === 'string' &&
        (part.text.startsWith('<plan-mode-active>') ||
            part.text.startsWith('<background-tasks>') ||
            part.text.startsWith('The following files were recently accessed before context was compacted.') ||
            part.text.startsWith('Recently accessed file (full current content embedded):') ||
            part.text.startsWith('Recent visual snapshots preserved from before context was compacted')));
}
function isModelFunctionCallEntry(content) {
 ...
 */
export function isSystemReminderContent(content) {
    const parts = content.parts;
...
export function stripSystemReminderBlocks(text) {
    ...
    while (offset < text.length) {
        ...
        const close = text.indexOf(SYSTEM_REMINDER_CLOSE, open + SYSTEM_REMINDER_OPEN.length);
        if (close === -1)
            return out + text.slice(offset, open);
