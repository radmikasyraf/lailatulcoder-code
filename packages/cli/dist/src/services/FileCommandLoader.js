...
import { z } from 'zod';
import { createDebugLogger, EXTENSIONS_CONFIG_FILENAME, Storage, } from '@lailatul-coder/lailatul-coder-core';
import { parseMarkdownCommand, MarkdownCommandDefSchema, } from './markdown-command-parser.js';
...
export class FileCommandLoader {
    ...
    async loadCommands(signal) {
        ...
        for (const dirInfo of commandDirs) {
            try {
                ...
                // Process TOML files
                const tomlCommandPromises = tomlFiles.map((file) => this.parseAndAdaptTomlFile(path.join(dirInfo.path, file), dirInfo.path, dirInfo.extensionName));
                // Process Markdown files
                const mdCommandPromises = mdFiles.map((file) => this.parseAndAdaptMarkdownFile(path.join(dirInfo.path, file), dirInfo.path, dirInfo.extensionName));
                const commands = (await Promise.all([...tomlCommandPromises, ...mdCommandPromises])).filter((cmd) => cmd !== null);
                ...
                if (!isEnoent && !isAbortError) {
                    debugLogger.error(`[FileCommandLoader] Error loading commands from ${dirInfo.path}:`, error);
                }
            }
        }
        return allCommands;
            ...
            if (fsSync.existsSync(configPath)) {
                ...
                if (config.commands) {
                    ...
                    return commandsArray
                        .map((cmdPath) => path.isAbsolute(cmdPath) ? cmdPath : path.join(ext.path, cmdPath))
                        .filter((cmdPath) => {
                        try {
    ...
    async parseAndAdaptTomlFile(filePath, baseDir, extensionName) {
        ...
        try {
            ...
            debugLogger.error(`[FileCommandLoader] Failed to read file ${filePath}:`, error instanceof Error ? error.message : String(error));
            return null;
        }
        let parsed;
        try {
            ...
            debugLogger.error(`[FileCommandLoader] Failed to parse TOML file ${filePath}:`, error instanceof Error ? error.message : String(error));
            return null;
        }
        const validationResult = TomlCommandDefSchema.safeParse(parsed);
        if (!validationResult.success) {
            debugLogger.error(`[FileCommandLoader] Skipping invalid command file: ${filePath}. Validation errors:`, validationResult.error.flatten());
            return null;
        }
        const validDef = validationResult.data;
        // Use factory to create command
        return createSlashCommandFromDefinition(filePath, baseDir, validDef, extensionName, '.toml');
    }
    /**
    ...
    async parseAndAdaptMarkdownFile(filePath, baseDir, extensionName) {
        ...
        try {
            ...
            debugLogger.error(`[FileCommandLoader] Failed to parse Markdown file ${filePath}:`, error instanceof Error ? error.message : String(error));
            return null;
        }
        ...
        const definition = {
            ...
            description: validDef.frontmatter?.description &&
            ...
            disableModelInvocation: validDef.frontmatter?.['disable-model-invocation'],
        };
        // Use factory to create command
