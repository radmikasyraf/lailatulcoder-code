...
describe('pathRulesFor — scoped, or it is noise', () => {
    ...
    it('caps the path list for workflow files too', () => {
        ...
        const many = Array.from({ length: 15 }, (_, i) => `.github/workflows/ci${i}.yml`);
...
describe('pathRulesFor — the Java/JVM rule', () => {
    ...
    it('prescribes a measurement that cannot mutate the tree or run contributor code', () => {
        ...
        expect(out).toMatch(/never `git checkout`, `git stash`, build in place, or run `mvn`\/`gradle`/);
    });
    ...
    it('names production paths before test paths in the heading', () => {
        ...
        const tests = Array.from({ length: 30 }, (_, i) => `src/test/java/com/x/T${i}Test.java`);
    ...
    it.each([
        [
            ...
            Array.from({ length: 11 }, (_, i) => `target/generated-sources/com/x/S${i}.java`),
        ],
        [
            'Maven generated test output',
            Array.from({ length: 11 }, (_, i) => `target/generated-test-sources/test-annotations/com/x/S${i}.java`),
        ],
        [
    ...
    it('treats a source package merely named generated as production', () => {
        ...
        const noise = Array.from({ length: 11 }, (_, i) => `target/generated-sources/com/x/S${i}.java`);
        const out = pathRulesFor([
            ...noise,
    ...
    it('treats Gradle src/integrationTest as out of scope even past the cap', () => {
        ...
        const tests = Array.from({ length: 12 }, (_, i) => `src/integrationTest/java/com/x/T${i}.java`);
        ...
        expect(heading.indexOf('Hot.java')).toBeLessThan(heading.indexOf('T0.java'));
    });
    ...
    it('caps the triggering-path list in the heading', () => {
        ...
        const many = Array.from({ length: 12 }, (_, i) => `src/main/java/com/x/F${i}.java`);
        const out = pathRulesFor(many);
...
describe('pathRulesFor — the consumer-facing contract documentation rule', () => {
    ...
    it('stays silent on the documentation genres that are not contracts', () => {
        ...
        for (const quiet of [
            ...
            expect(pathRulesFor([quiet])).not.toContain('Consumer-facing contract documentation');
        }
    });
    it('takes governance from location, never from a keyword in the name', () => {
        ...
        expect(pathRulesFor(['docs/reference/wire-protocol.md'])).toContain(governed);
        expect(pathRulesFor(['integrations/wire-protocol.md'])).not.toContain(governed);
    ...
    it('applies the genre exclusion in both its directory and filename forms', () => {
        ...
        const governed = 'Consumer-facing contract documentation';
        expect(pathRulesFor(['docs/api/changelog/2026-08.md'])).not.toContain(governed);
    ...
    it('names the three blocker shapes, including the one a diff-scoped read cannot see', () => {
        ...
        expect(out).toContain('the wrong lines are the ones the diff did **not** touch');
...
//# sourceMappingURL=path-rules.test.js.map