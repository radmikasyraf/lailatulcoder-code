...
import { FatalSandboxError, PRIVATE_ACP_CAPABILITY_ENV, QWEN_DIR, } from '@lailatul-coder/lailatul-coder-core';
...
import { getSandboxPassthroughEnvArgs, resolveSeatbeltProfileFile, start_sandbox, } from './sandbox.js';
...
describe('start_sandbox', () => {
    it('passes child environment variables into a container sandbox', async () => {
        ...
        vi.spyOn(fs, 'realpathSync').mockImplementation((filePath) => String(filePath));
        ...
        const result = start_sandbox({ command: 'docker', image: 'example.com/lailatul-coder:latest' }, [], undefined, [process.execPath, '/path/to/cli.js', '--acp'], { [PRIVATE_ACP_CAPABILITY_ENV]: capability });
    ...
    it('checks image presence with an offline inspect that sees digest references', async () => {
        ...
        vi.spyOn(fs, 'realpathSync').mockImplementation((filePath) => String(filePath));
        execSyncMock.mockReturnValue(Buffer.from(''));
        const digestImage = 'ghcr.io/LailatulCoder/lailatul-coder@sha256:0123456789abcdef0123456789abcdef0123456789abcdef0123456789abcdef';
        const imageCheck = Object.assign(new EventEmitter(), {
            stdout: new EventEmitter(),
        });
        const child = new EventEmitter();
        ...
        // sandbox run itself.
        expect((spawnMock.mock.calls[1]?.[1])[0]).toBe('run');
        child.emit('close', 0);
        await expect(result).resolves.toBe(0);
    });
});
describe('resolveSeatbeltProfileFile', () => {
    it('strips the chunks segment from bundled seatbelt profile paths', () => {
        ...
        const chunkUrl = pathToFileURL(path.join(bundleDir, 'chunks', 'sandbox-AAAA.js')).toString();
        expect(resolveSeatbeltProfileFile('permissive-open', chunkUrl)).toBe(path.join(bundleDir, 'sandbox-macos-permissive-open.sb'));
    });
    it('keeps source-mode seatbelt profile paths next to the module', () => {
        const utilsDir = path.resolve(path.sep, 'repo', 'packages', 'cli', 'src', 'utils');
        const sourceUrl = pathToFileURL(path.join(utilsDir, 'sandbox.ts')).toString();
        expect(resolveSeatbeltProfileFile('restrictive-closed', sourceUrl)).toBe(path.join(utilsDir, 'sandbox-macos-restrictive-closed.sb'));
    });
    it('keeps custom seatbelt profiles under project settings', () => {
        const bundleDir = path.resolve(path.sep, 'tmp', 'qwen', 'lib');
        const chunkUrl = pathToFileURL(path.join(bundleDir, 'chunks', 'sandbox-AAAA.js')).toString();
        expect(resolveSeatbeltProfileFile('project-profile', chunkUrl)).toBe(path.join(QWEN_DIR, 'sandbox-macos-project-profile.sb'));
    });
    it('throws missing file errors with the resolved seatbelt profile path', async () => {
        ...
        await expect(start_sandbox({ command: 'sandbox-exec', image: '' })).rejects.toThrow(new FatalSandboxError(`Missing macos seatbelt profile file '${expectedPath}'`));
    });
    it.each([
        ['managed ACP', '1', true],
        ['ordinary', undefined, false],
    ])('handles Electron Node mode for a %s seatbelt re-exec', async (_name, marker, expected) => {
        vi.stubEnv('QWEN_CODE_SCRUB_ELECTRON_RUN_AS_NODE', marker ?? '');
        vi.spyOn(fs, 'existsSync').mockReturnValue(true);
        vi.spyOn(fs, 'mkdirSync').mockReturnValue(undefined);
        vi.spyOn(fs, 'realpathSync').mockImplementation((filePath) => String(filePath) || '/tmp');
        execSyncMock.mockReturnValue(Buffer.from('/tmp/cache\n'));
        const child = new EventEmitter();
        spawnMock.mockReturnValue(child);
        const result = start_sandbox({ command: 'sandbox-exec', image: '' }, [], undefined, [process.execPath, '/path/to/cli.js', '--acp']);
...
describe('getSandboxPassthroughEnvArgs', () => {
    it('passes update relaunch state into container sandboxes', () => {
        expect(getSandboxPassthroughEnvArgs({
...
describe('isContainerPathWithinWorkdir', () => {
    ...
    it('allows paths under the workdir', () => {
        expect(isContainerPathWithinWorkdir('/repo/app', '/repo/app/bin')).toBe(true);
    });
    it('rejects sibling paths with the same prefix', () => {
        expect(isContainerPathWithinWorkdir('/repo/app', '/repo/app-tools/bin')).toBe(false);
    });
    ...
    it('normalizes trailing slashes and case for container paths', () => {
        expect(isContainerPathWithinWorkdir('/C/Repo/App/', '/c/repo/app/bin')).toBe(true);
    });
...
describe('parseSandboxImageName', () => {
    it('uses the image basename and tag for container names', () => {
        expect(parseSandboxImageName('ghcr.io/LailatulCoder/lailatul-coder:0.18.3')).toBe('lailatul-coder-0.18.3');
    });
    it('handles registry ports without treating them as tags', () => {
        expect(parseSandboxImageName('localhost:5000/team/lailatul-coder-sandbox:dev')).toBe('lailatul-coder-sandbox-dev');
    });
    it('handles registry ports when the image is untagged', () => {
        expect(parseSandboxImageName('localhost:5000/team/lailatul-coder-sandbox')).toBe('lailatul-coder-sandbox');
    });
    it('drops digests from generated container names', () => {
        expect(parseSandboxImageName('registry.example.com/team/lailatul-coder-sandbox@sha256:abcdef')).toBe('lailatul-coder-sandbox');
    });
    it('keeps tags when dropping digests from generated container names', () => {
        expect(parseSandboxImageName('registry.example.com/team/lailatul-coder-sandbox:dev@sha256:abcdef')).toBe('lailatul-coder-sandbox-dev');
    });
});
