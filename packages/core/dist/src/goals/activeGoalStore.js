...
export function recordGoalDeferral(sessionId) {
    const current = store.get(sessionId);
    if (!current)
        return undefined;
    const updated = {
...
export function resetGoalDeferrals(sessionId) {
    const current = store.get(sessionId);
    if (!current || current.deferredEvaluations === 0)
        return current;
    const updated = { ...current, deferredEvaluations: 0 };
    store.set(sessionId, updated);
    return updated;
}
/**
