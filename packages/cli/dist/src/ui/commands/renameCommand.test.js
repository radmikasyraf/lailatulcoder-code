...
vi.mock('@lailatul-coder/lailatul-coder-core', async (importOriginal) => {
    ...
    return {
        ...original,
        tryGenerateSessionTitle: (...args) => tryGenerateSessionTitleMock(...args),
    };
});
describe('renameCommand', () => {
    ...
    it('should have the correct name and description', () => {
        ...
        expect(renameCommand.description).toBe('Rename the current conversation. --auto lets the fast model pick a title.');
    });
    ...
    describe('completion', () => {
        const run = (partial) => renameCommand.completion(mockContext, partial);
    ...
    it('should return error when no name is provided and auto-generate fails', async () => {
        ...
        expect(result).toEqual({
            type: 'message',
            messageType: 'error',
            content: 'No conversation to title yet — send at least one message first.',
        });
    ...
    it('passes channel display projections to automatic title generation', async () => {
        ...
        expect(tryGenerateSessionTitleMock).toHaveBeenCalledWith(mockConfig, expect.any(AbortSignal), ['你好']);
    });
    ...
    it('waits for durable title persistence before reporting success', async () => {
        let resolveTitle;
        const recordCustomTitle = vi.fn(() => new Promise((resolve) => {
    ...
    it('does not update the UI when durable title persistence fails', async () => {
        ...
        await expect(renameCommand.action(mockContext, 'failed-title')).resolves.toEqual({
            type: 'message',
            messageType: 'error',
    ...
    it('should fall back to SessionService when ChatRecordingService is unavailable', async () => {
        ...
        const result = await renameCommand.action(mockContext, 'my-feature');
        expect(mockRenameSession).toHaveBeenCalledWith('test-session-id', 'my-feature', 'manual');
        expect(result).toEqual({
            type: 'message',
            messageType: 'info',
            content: 'Session renamed to "my-feature"',
    ...
    describe('bare /rename pipeline', () => {
        ...
        it('records bare /rename success as auto-sourced', async () => {
            ...
            await renameCommand.action(mockContext, '');
            expect(mockRecordCustomTitle).toHaveBeenCalledWith('Refactor auth middleware', 'auto');
        });
        it('surfaces no_fast_model on bare /rename when fast model is unset', async () => {
            ...
            expect(result.content).toMatch(/requires a fast model/);
        });
    });
    describe('--auto flag', () => {
        ...
        it('refuses --auto combined with a positional name', async () => {
            ...
            expect(result).toEqual({
                type: 'message',
                messageType: 'error',
                content: '/rename --auto does not take a name. Use `/rename <name>` to set a name yourself.',
            });
        ...
        it('writes an auto-sourced title on --auto success', async () => {
            ...
            const result = await renameCommand.action(mockContext, '--auto');
            expect(tryGenerateSessionTitleMock).toHaveBeenCalledOnce();
            expect(mockRecordCustomTitle).toHaveBeenCalledWith('Fix login button on mobile', 'auto');
            expect(result).toEqual({
                type: 'message',
                messageType: 'info',
        ...
        it('surfaces model_error reason distinctly', async () => {
            ...
            expect(result.content).toMatch(/rate limit, auth, network error, or unexpected response format/);
        });
        it('rejects unknown flag with sentinel hint', async () => {
            ...
            const result = await renameCommand.action(mockContext, '--my-label-with-dashes');
            expect(result).toMatchObject({ messageType: 'error' });
        ...
        it('falls back to SessionService.renameSession with auto source', async () => {
            ...
            const result = await renameCommand.action(mockContext, '--auto');
            expect(mockRenameSession).toHaveBeenCalledWith('test-session-id', 'Audit auth middleware', 'auto');
            expect(result).toMatchObject({ messageType: 'info' });
        });
    });
});
//# sourceMappingURL=renameCommand.test.js.map