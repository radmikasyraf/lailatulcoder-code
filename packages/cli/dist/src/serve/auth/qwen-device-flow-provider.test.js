...
import { QwenOAuthPollError, } from '@lailatul-coder/lailatul-coder-core';
...
describe('QwenOAuthDeviceFlowProvider.poll() — stderr audit branches', () => {
    ...
    it('logs only err.name + message length on generic Error (raw message is suppressed)', async () => {
        ...
        const longMessage = 'HTTP 502 from qwen IdP: <html><body>Forbidden — request body: device_code=device-code-secret-AAAA1111&code_verifier=pkce-verifier-secret-BBBB2222</body></html>';
        const provider = new QwenOAuthDeviceFlowProvider(fakeClient({
            pollDeviceToken: async () => {
    ...
    it('logs <non-Error throw: typeof> placeholder when a non-Error value is thrown', async () => {
        ...
        const nonErrorThrower = () => {
            const value = 'this is not an Error instance';
            throw value;
        };
        const provider = new QwenOAuthDeviceFlowProvider(fakeClient({
...
//# sourceMappingURL=qwen-device-flow-provider.test.js.map