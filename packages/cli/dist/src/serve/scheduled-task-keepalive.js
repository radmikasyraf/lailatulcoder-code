...
import { readCronTasks, updateCronTasks, getCronFilePath, createDebugLogger, SessionService, Storage, taskHasLegacyCondition, } from '@lailatul-coder/lailatul-coder-core';
...
function collectBoundSessionIds(tasks) {
    const seen = new Set();
    const ids = [];
    for (const task of tasks) {
        const sessionId = task.sessionId;
        if (task.enabled === false || // disabled (e.g. archived) — let it be reaped
 ...
 */
async function bindAndNameSessions(bridge, boundWorkspace, tasks, renamed, spawnTimeoutMs, binding, cleanupSession) {
    const unbound = tasks.filter((t) => !t.sessionId &&
        t.enabled !== false &&
        !taskHasLegacyCondition(t) &&
        !binding.has(t.id));
    const needsName = tasks.filter((t) => t.sessionId &&
        t.enabled !== false &&
        !taskHasLegacyCondition(t) &&
    ...
    for (const task of unbound) {
        ...
        try {
            ...
            rawSpawn
                ...
                if (timedOut) {
                    log.debug('keepalive: late spawn resolved, cleaning up', task.id, sessionId);
            ...
            const { sessionId } = await withTimeout(rawSpawn, spawnTimeoutMs, `spawnOrAttach(${task.id})`).catch((err) => {
            ...
            await updateCronTasks(boundWorkspace, (list) => {
                ...
                if (!list.some((t) => t.id === task.id && !t.sessionId && t.enabled !== false)) {
                    return list;
                }
                const result = list.map((t) => t.id === task.id && !t.sessionId && t.enabled !== false
                    ? { ...t, sessionId }
                    : t);
                matched = true;
                return result;
            ...
            log.debug('keepalive: bound task', task.id, 'to dedicated session', sessionId);
        }
        catch (err) {
...
export function startScheduledTaskKeepalive(opts) {
    ...
    const cleanupSession = opts.cleanupSession ??
        (async (sessionId) => {
    ...
    const tickInRuntime = async () => {
        ...
        for (const sessionId of collectBoundSessionIds(tasks)) {
            try {
                ...
                const metadata = await new SessionService(boundWorkspace).readCreationMetadata(sessionId);
                ...
                try {
                    await withTimeout(resume, reviveTimeoutMs, `resumeSession(${sessionId})`);
                    ...
                    const backoff = Math.min(intervalMs * 2 ** Math.min(failures - 1, 6), MAX_REVIVE_BACKOFF_MS);
                    reviveState.set(sessionId, {
                        failures,
                        nextAttemptAt: Date.now() + backoff,
                    });
                    log.debug('keepalive: failed to revive session', sessionId, `(failure ${failures}, next retry in ${backoff}ms)`, loadErr);
                }
            }
        }
        ...
        await bindAndNameSessions(bridge, boundWorkspace, tasks, renamed, spawnTimeoutMs, binding, cleanupSession);
    };
    const tick = () => opts.runtimeBaseDir === undefined
        ? tickInRuntime()
        : Storage.runWithResolvedRuntimeBaseDir(opts.runtimeBaseDir, tickInRuntime);
    ...
    let bindDebounce;
    const cronFilePath = opts.runtimeBaseDir === undefined
        ? getCronFilePath(boundWorkspace)
        : Storage.runWithResolvedRuntimeBaseDir(opts.runtimeBaseDir, () => getCronFilePath(boundWorkspace));
    ...
    try {
        ...
        fileWatcher = fsSync.watch(cronDir, { persistent: false }, (_event, filename) => {
            // On Linux fs.watch delivers null as filename — treat it as a match
            // (could be our file); non-matching filenames are skipped.
            if (filename !== null && filename !== cronFileName)
                return;
            if (bindDebounce)
                clearTimeout(bindDebounce);
...
export async function rehydrateScheduledTaskSessions(deps) {
    ...
    const worker = async () => {
        for (let sessionId = queue.shift(); sessionId !== undefined; sessionId = queue.shift()) {
            await loadOne(sessionId);
        }
    };
    await Promise.all(Array.from({ length: Math.min(REHYDRATE_MAX_CONCURRENCY, queue.length) }, () => worker()));
    return { loaded, failed };
}
/** Rejects with a clear error if `p` doesn't settle within `ms`. */
function withTimeout(p, ms, label) {
    if (ms > MAX_SESSION_RESTORE_TIMEOUT_MS)
        return p;
...
//# sourceMappingURL=scheduled-task-keepalive.js.map