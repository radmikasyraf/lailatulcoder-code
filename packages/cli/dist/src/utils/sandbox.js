...
import { getUserSettingsDir, SETTINGS_DIRECTORY_NAME, } from '../config/settings.js';
import { promisify } from 'node:util';
import { FatalSandboxError, Storage, isSubpath, resolveBundleDir, } from '@lailatul-coder/lailatul-coder-core';
...
import { CUSTOM_SANDBOX_IMAGE_ENV_VAR, HOST_UPDATE_RELAUNCH_ENV_VAR, SKIP_UPDATE_CHECK_ENV_VAR, } from './processUtils.js';
import { QWEN_CODE_DESKTOP_ENV, QWEN_CODE_SERVE_ENV, } from '../config/acp-channel-fallback.js';
...
function ensureDirectoryAndGetRealPath(dir) {
    if (!fs.existsSync(dir)) {
        fs.mkdirSync(dir, { recursive: true });
    }
    return fs.realpathSync(dir);
}
const LOCAL_DEV_SANDBOX_IMAGE_NAME = 'lailatul-coder-sandbox';
const SANDBOX_NETWORK_NAME = 'lailatul-coder-sandbox';
const SANDBOX_PROXY_NAME = 'lailatul-coder-sandbox-proxy';
...
export function getSandboxPassthroughEnvArgs(env = process.env) {
    return [
        'QWEN_DEBUG_LOG_FILE',
    ...
    ].flatMap((envVar) => env[envVar] === undefined ? [] : ['--env', `${envVar}=${env[envVar]}`]);
}
export function resolveSeatbeltProfileFile(profile, importMetaUrl = import.meta.url) {
    if (!BUILTIN_SEATBELT_PROFILES.includes(profile)) {
        return path.join(SETTINGS_DIRECTORY_NAME, `sandbox-macos-${profile}.sb`);
    }
    return path.join(resolveBundleDir(importMetaUrl), `sandbox-macos-${profile}.sb`);
}
/**
...
async function shouldUseCurrentUserInSandbox() {
    ...
    if (os.platform() === 'linux') {
        const debugEnv = [process.env['DEBUG'], process.env['DEBUG_MODE']].some((v) => v === 'true' || v === '1');
        if (debugEnv) {
            // Use stderr so it doesn't clutter normal STDOUT output (e.g. in `--prompt` runs).
            writeStderrLine('INFO: Using current user UID/GID in Linux sandbox. Set SANDBOX_SET_UID_GID=false to disable.');
        }
        return true;
    }
    return false;
...
function entrypoint(workdir, cliArgs) {
    ...
    const cliCmd = process.env['NODE_ENV'] === 'development'
        ...
        : process.env['DEBUG']
            ? `node --inspect-brk=0.0.0.0:${process.env['DEBUG_PORT'] || '9229'} $(which LailatulCoder)`
            : 'LailatulCoder';
    const args = [...shellCmds, cliCmd, ...quotedCliArgs];
    return ['bash', '-c', args.join(' ')];
}
export async function start_sandbox(config, nodeArgs = [], cliConfig, cliArgs = [], childEnv) {
    ...
    // determine full path for lailatul-coder to distinguish linked vs installed setting
    ...
    const containerWorkdir = getContainerPath(workdir);
    // if BUILD_SANDBOX is set, then call scripts/build_sandbox.js under lailatul-coder repo
    //
    // note this can only be done with binary linked from lailatul-coder repo
    if (process.env['BUILD_SANDBOX']) {
        if (!gcPath.includes('lailatul-coder/packages/')) {
            throw new FatalSandboxError('Cannot build sandbox using installed LailatulCoder Ai binary; ' +
    ...
    if (!(await ensureSandboxImageIsPresent(config.command, image))) {
        const remedy = image === LOCAL_DEV_SANDBOX_IMAGE_NAME
            ? 'Try running `npm run build:all` or `npm run build:sandbox` under the lailatul-coder repo to build it locally, or check the image name and your network connection.'
            : 'Please check the image name, your network connection, or notify lailatul-coder-dev@service.alibaba.com if the issue persists.';
    ...
    // Mount user settings at /home/node/.lailatulcoder and at the canonical host path
    ...
    const userSettingsDirRealPath = ensureDirectoryAndGetRealPath(userSettingsDirOnHost);
    const runtimeBaseDirRealPath = ensureDirectoryAndGetRealPath(runtimeBaseDirOnHost);
    const userSettingsDirInSandbox = getContainerPath(`/home/node/${SETTINGS_DIRECTORY_NAME}`);
    const userSettingsDirContainerPath = getContainerPath(userSettingsDirRealPath);
    const runtimeBaseDirContainerPath = getContainerPath(runtimeBaseDirRealPath);
    const runtimeCoveredByUserSettings = isSubpath(userSettingsDirRealPath, runtimeBaseDirRealPath);
    const userSettingsCoveredByRuntime = isSubpath(runtimeBaseDirRealPath, userSettingsDirRealPath);
    const runtimeSameAsUserSettings = runtimeCoveredByUserSettings && userSettingsCoveredByRuntime;
    args.push('--volume', `${userSettingsDirRealPath}:${userSettingsDirInSandbox}`);
    if ((!userSettingsCoveredByRuntime || runtimeSameAsUserSettings) &&
        userSettingsDirInSandbox !== userSettingsDirContainerPath) {
        args.push('--volume', `${userSettingsDirRealPath}:${userSettingsDirContainerPath}`);
    }
    // Pass QWEN_HOME so the sandboxed CLI resolves the global LailatulCoder dir to the
    // same path the host did, instead of relying on the /home/node/.lailatulcoder mount
    ...
    // from the global LailatulCoder dir; otherwise the existing user-settings mount
    // already covers it.
    if (!runtimeCoveredByUserSettings) {
        args.push('--volume', `${runtimeBaseDirRealPath}:${runtimeBaseDirContainerPath}`);
    }
    ...
    const isIntegrationTest = process.env['QWEN_CODE_INTEGRATION_TEST'] === 'true';
    let containerName;
    if (isIntegrationTest) {
        containerName = `lailatul-coder-integration-test-${randomBytes(4).toString('hex')}`;
    ...
    if (process.env['QWEN_CODE_TEST_VAR']) {
        args.push('--env', `QWEN_CODE_TEST_VAR=${process.env['QWEN_CODE_TEST_VAR']}`);
    }
    args.push(...getSandboxPassthroughEnvArgs());
    if (process.env['QWEN_CODE_MCP_APPROVALS_PATH']) {
        args.push('--env', `QWEN_CODE_MCP_APPROVALS_PATH=${getContainerPath(process.env['QWEN_CODE_MCP_APPROVALS_PATH'])}`);
    }
    ...
    // copy OPENAI_API_KEY and related env vars for LailatulCoder
    ...
    if (virtualEnv &&
        isContainerPathWithinWorkdir(getContainerPath(workdir), getContainerPath(virtualEnv))) {
    ...
    // tests that need to access host's ~/.lailatulcoder (e.g., --resume functionality)
    ...
    if (useCurrentUser) {
        ...
        // allowing tests that need to access host's ~/.lailatulcoder (e.g., --resume) to work.
        ...
        // finally switch to that user to run the LailatulCoder process. This is
        // necessary on Linux to ensure the user exists within the
        // container's /etc/passwd file, which is required by os.userInfo().
        const username = 'LailatulCoder';
    ...
    return new Promise((resolve, reject) => {
        ...
        sandboxProcess?.on('close', (code, signal) => {
            process.stdin.resume();
            if (code !== 0 && code !== null) {
                writeStderrLine(`Sandbox process exited with code: ${code}, signal: ${signal}`);
            }
            resolve(code ?? 1);
        });
    });
...
async function imageExists(sandbox, image) {
    return new Promise((resolve) => {
        ...
        checkProcess.on('error', (err) => {
            writeStderrLine(`Failed to start '${sandbox}' command for image check: ${err.message}`);
            resolve(false);
        });
...
async function pullImage(sandbox, image) {
    ...
    return new Promise((resolve) => {
        ...
        const onError = (err) => {
            writeStderrLine(`Failed to start '${sandbox} pull ${image}' command: ${err.message}`);
            cleanup();
            resolve(false);
        };
        const onClose = (code) => {
            if (code === 0) {
                ...
                writeStderrLine(`Failed to pull image ${image}. '${sandbox} pull ${image}' exited with code ${code}.`);
                if (stderrData.trim()) {
...
async function ensureSandboxImageIsPresent(sandbox, image) {
    ...
    if (await pullImage(sandbox, image)) {
        ...
        if (await imageExists(sandbox, image)) {
            ...
            writeStderrLine(`Sandbox image ${image} still not found after a pull attempt. This might indicate an issue with the image name or registry, or the pull command reported success but failed to make the image available.`);
            return false;
        }
    }
    writeStderrLine(`Failed to obtain sandbox image ${image} after check and pull attempt.`);
