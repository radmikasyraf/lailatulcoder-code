 ...
 */
import { GITHUB_PR_ERROR_MESSAGE_MAX, fetchGitHubPullRequests, createGitHubPullRequest, getDefaultBranch, } from '@lailatul-coder/lailatul-coder-core';
import { safeBody } from '../server/request-helpers.js';
import { requireTrustedWorkspaceRuntime, resolveContainedCwdOrFail, resolveWorkspaceRuntimeFromParam, sendGenerationClosedError, } from '../workspace-route-runtime.js';
import { applyReadHeaders } from './workspace-file-read.js';
const DEFAULT_CACHE_TTL_MS = 60_000;
function sanitizeMessage(message, workspaceCwd) {
    return message.split(workspaceCwd).join('<workspace>');
}
export function registerWorkspaceQualifiedGitHubPrsRoutes(app, deps) {
    const ttlMs = deps.cacheTtlMs ?? DEFAULT_CACHE_TTL_MS;
    const cache = new Map();
    const getPullRequests = (workspaceCwd, env) => {
        const now = Date.now();
        ...
        if (!fresh) {
            ...
            entry.promise.then((result) => {
                if (cache.get(workspaceCwd) !== entry)
                    return;
                if (result.kind === 'ok')
                    entry.settledAt = Date.now();
                else
                    cache.delete(workspaceCwd);
            }, () => {
                if (cache.get(workspaceCwd) === entry)
                    cache.delete(workspaceCwd);
            });
    ...
    app.get('/workspaces/:workspace/github/prs', async (req, res) => {
        ...
        try {
            const result = await getPullRequests(runtime.workspaceCwd, runtime.env.effectiveEnv);
            switch (result.kind) {
                case 'ok':
                    res.status(200).json({
                        v: 1,
                ...
                case 'cli_unavailable':
                    res.status(502).json({
                        error: 'The GitHub CLI (gh) is not installed on the daemon host; install it and run `gh auth login`.',
                ...
                case 'failed': {
                    ...
                    const error = sanitizeMessage(sanitizeMessage(result.message, runtime.workspaceCwd), result.gitRoot).slice(0, GITHUB_PR_ERROR_MESSAGE_MAX);
                    res.status(502).json({
                        error,
                ...
                default:
                    throw new Error(`unexpected fetchGitHubPullRequests result: ${JSON.stringify(result)}`);
            }
        }
        catch (err) {
    ...
    app.post('/workspaces/:workspace/github/prs/create', deps.mutate({ strict: true }), async (req, res) => {
        ...
        for (const field of ['body', 'base', 'head']) {
        ...
        try {
            const result = await createGitHubPullRequest(cwd, {
                title: title.trim(),
                body: prBody,
                base,
                head,
            ...
            switch (result.kind) {
                ...
                case 'failed':
                    ...
                    res.status(502).json({
                        error: sanitizeMessage(sanitizeMessage(result.message, runtime.workspaceCwd), result.gitRoot).slice(0, GITHUB_PR_ERROR_MESSAGE_MAX),
    ...
    app.get('/workspaces/:workspace/github/default-branch', async (req, res) => {
        ...
        try {
            runtime.generationGuard?.assertOpen();
            const branch = await getDefaultBranch(runtime.workspaceCwd, runtime.env.effectiveEnv);
            runtime.generationGuard?.assertOpen();
...
//# sourceMappingURL=workspace-github-prs.js.map