    ...
    async execute(_signal) {
        ...
        if (agentEntry) {
            if (agentEntry.status === 'paused') {
                ...
                if (!abandoned) {
                    return {
                        llmContent: `Error: Background agent "${taskId}" could not be cancelled ` +
                            `from paused state.`,
                ...
                return {
                    llmContent: `Cancelled paused background agent "${taskId}".\n` +
                        `Description: ${desc}`,
            ...
            const desc = agentEntry.description;
            return {
                llmContent: `Cancellation requested for background agent "${taskId}". ` +
        ...
        if (shellEntry) {
            ...
            return {
                llmContent: `Cancellation requested for background shell "${taskId}". ` +
        ...
        if (memoryRecord) {
            if (memoryRecord.taskType !== 'dream') {
                return {
                    llmContent: `Error: Memory task "${taskId}" (${memoryRecord.taskType}) is ` +
            ...
            if (!cancelled) {
                ...
                // scheduleDream.
                return {
                    llmContent: `Error: Dream task "${taskId}" could not be cancelled ` +
            ...
            return {
                llmContent: `Cancellation requested for dream task "${taskId}". ` +
...
function notRunningError(kind, taskId, status) {
    return {
...
export class TaskStopTool extends BaseDeclarativeTool {
    config;
    static Name = ToolNames.TASK_STOP;
    constructor(config) {
        super(TaskStopTool.Name, ToolDisplayNames.TASK_STOP, 'Stop a background task by its ID. Running agents and shells are cancelled; paused recovered agents are abandoned without resuming them.', Kind.Other, {
            type: 'object',
            properties: {
                task_id: {
                    type: 'string',
