 ...
 */
export function createPatchSmart(filename, oldStr, newStr, oldHeader, newHeader) {
    const cleanPatch = Diff.createPatch(filename, oldStr, newStr, oldHeader, newHeader, DEFAULT_DIFF_OPTIONS);
...
function structuredPatchSmart(filename, oldStr, newStr, oldHeader, newHeader) {
    const result = Diff.structuredPatch(filename, filename, oldStr, newStr, oldHeader, newHeader, DEFAULT_DIFF_OPTIONS);
...
export function getDiffStat(fileName, oldStr, aiStr, userStr) {
    ...
    const modelPatch = structuredPatchSmart(fileName, oldStr, aiStr, 'Current', 'Proposed');
    const modelStats = getStats(modelPatch);
    const userStats = aiStr === userStr
        ? {
            addedLines: 0,
            removedLines: 0,
            addedChars: 0,
        ...
        : getStats(structuredPatchSmart(fileName, aiStr, userStr, 'Proposed', 'User'));
    return {
