 ...
 *   non-configurable).
 */
export async function renameWithRetry(src, dest, retries, delayMs, _renameImpl = fs.rename) {
    for (let attempt = 0; attempt <= retries; attempt++) {
        try {
            ...
            const isRetryable = isNodeError(error) &&
...
async function resolveSymlinkChain(filePath) {
    ...
    const err = new Error(`ELOOP: too many levels of symbolic links, resolve '${filePath}'`);
    err.code = 'ELOOP';
    throw err;
}
/**
...
_testFs) {
    ...
    try {
        ...
        await renameWithRetry(tmpPath, targetPath, retries, delayMs, async (src, dest) => {
            options?.assertCanCommit?.();
        ...
        if (isNodeError(error) && error.code === 'EXDEV') {
            try {
                ...
                if (options?.noFollow) {
                    ...
                    const fd = await openImpl(targetPath, fsSync.constants.O_WRONLY |
                        fsSync.constants.O_CREAT |
                        fsSync.constants.O_EXCL, desiredMode ?? 0o666);
                    let writeOk = false;
                    try {
                        try {
                            await fd.writeFile(typeof data === 'string' ? Buffer.from(data, encoding) : data);
                            if (flush)
                                await fd.sync();
                        ...
                        if (!writeOk) {
                            try {
                                ...
                                debugLogger.debug(`orphan unlink failed for ${targetPath}:`, orphanErr);
                            }
                        }
                        throw writeErr;
                    }
 ...
 */
export function renameWithRetrySync(src, dest, retries, delayMs, _renameImpl = fsSync.renameSync) {
    for (let attempt = 0; attempt <= retries; attempt++) {
        try {
        ...
        if (isNodeError(error) && error.code === 'EXDEV') {
            try {
                ...
                if (options?.noFollow) {
                    ...
                    const fd = openImpl(targetPath, fsSync.constants.O_WRONLY |
                        fsSync.constants.O_CREAT |
                        fsSync.constants.O_EXCL, desiredMode ?? 0o666);
                    let writeOk = false;
                    try {
                ...
                throw annotateWriteError(fallbackError, targetPath, 'atomicWriteFileSync');
            }
        }
        throw annotateWriteError(error, targetPath, 'atomicWriteFileSync');
    }
}
//# sourceMappingURL=atomicFileWrite.js.map