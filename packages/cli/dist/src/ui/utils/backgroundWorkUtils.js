 ...
 * an empty list.
 */
export function describeBlockingBackgroundWork(config) {
    const now = Date.now();
    const entries = [];
    for (const entry of config.getBackgroundTaskRegistry().getAll()) {
        if (!entry.isBackgrounded || entry.status !== 'running')
            continue;
        entries.push({
    ...
    const lines = listed.map((entry) => {
        const label = truncateToWidth(stripUnsafeCharacters(entry.label).replace(/[\r\n]+/g, ' '), MAX_BLOCKING_LABEL_WIDTH);
        return `  [${entry.id}] ${label} (${entry.status} ${formatDuration(now - entry.startTime, { hideTrailingZeros: true })})`;
    });
 ...
 */
export function buildBackgroundWorkBlockedMessage(config, baseMessage) {
