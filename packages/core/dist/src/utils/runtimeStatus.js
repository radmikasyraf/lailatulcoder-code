 ...
 *
 * Runtime status sidecar for an active interactive LailatulCoder Ai session.
 *
 ...
 *
 *     "Which LailatulCoder Ai session is the running PID X serving?"
 *
 ...
 * a try/catch.
 */
export async function writeRuntimeStatus(filePath, fields) {
    const payload = {
        ...
        qwen_version: fields.lailatulcoderVersion ?? null,
    };
 ...
 * dataclass.
 *
 * Note: a returned record only proves that *some* LailatulCoder Ai process
 ...
 */
export async function readRuntimeStatus(filePath, options = {}) {
    let raw;
    try {
    ...
    if (qwenVersion !== null && typeof qwenVersion !== 'string')
        return null;
    return {
        schemaVersion,
        pid,
 ...
 *
 * Intentionally **not** called on `/quit` — when the lailatul-coder process
 ...
 */
export async function clearRuntimeStatus(filePath) {
    try {
