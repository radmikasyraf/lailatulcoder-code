...
function reconcileMouseMode(stdout, entry) {
    ...
    if (desired === entry.active)
        return;
    if (entry.active)
        disableMouseEvents(stdout, entry.active);
    if (desired)
        enableMouseEvents(stdout, desired);
 ...
 */
export function useMouseEvents(handler, { isActive, tracking = 'button', bypassVpGate = false }) {
    ...
    // (`LailatulCoder | tee log`) an active, raw-mode-capable surface — e.g. the non-TTY
    ...
    const mouseCallback = useCallback((event) => {
