...
import { getGlobalQwenDirLite, resolveConfigPathLite, } from '../config/storage-paths-lite.js';
...
const FIXED_CTX_SET = new Set(FIXED_CTX_ORDER);
const RESERVED_TRACE_CTX_KEYS = new Set([
    'trace_id',
    'span_id',
]);
const ISSUE_ORDER = [
...
function renderCtx(ctx) {
    ...
    const extraKeys = Object.keys(ctx)
        .filter((key) => !FIXED_CTX_SET.has(key) &&
...
function renderTraceContext(traceContext) {
    return traceContext
...
function formatDaemonLogLine(args) {
...
export function buildDaemonLogLine(args) {
    return formatDaemonLogLine(args);
}
function buildDaemonFileLogLine(args, runId, pid) {
    const callerCtx = args.ctx
        ? Object.fromEntries(Object.entries(args.ctx).filter(([key]) => key !== 'runId' && key !== 'pid'))
        : undefined;
    const traceStr = renderTraceContext(args.traceContext);
...
function getRuntimeBaseDir(runtimeOutputDir, cwd) {
    const envDir = process.env['QWEN_RUNTIME_DIR'];
    if (envDir)
        return resolveConfigPathLite(envDir);
    if (runtimeOutputDir)
        return resolveConfigPathLite(runtimeOutputDir, cwd);
    return getGlobalQwenDirLite();
}
export function resolveDaemonLogBaseDir(runtimeOutputDir, cwd) {
...
function computeWorkspaceHash(boundWorkspace) {
    return crypto
        .createHash('sha256')
...
function resolvePolicy(overrides) {
    ...
    if (policy.maxBytes === 0 ||
        ...
        throw new TypeError('Daemon logger size/count policy values must be positive');
    }
    if (policy.maxRecordBytes > policy.maxBytes) {
        throw new TypeError('Daemon logger maxRecordBytes must not exceed maxBytes');
    }
    if (policy.maxRecordBytes > policy.maxPendingBytes) {
        throw new TypeError('Daemon logger maxRecordBytes must not exceed maxPendingBytes');
    }
    return policy;
}
function statusSnapshot(runId, mode, issues, droppedRecords, droppedBytes) {
    return {
        runId,
        mode,
...
function createStderrOnlyLogger(input) {
    const tee = (level, message, ctx, err) => {
        const traceContext = getActiveTraceContext();
        input.stderr(formatDaemonLogLine({
            level,
            message,
            now: input.now(),
            ctx,
    ...
    return {
        ...
        getStatus: () => statusSnapshot(input.runId, 'stderr-only', input.issues, 0, 0),
        flush: () => Promise.resolve(),
        close: () => Promise.resolve(),
    };
}
function capUtf8Record(record, maxBytes) {
...
async function ensureRealDirectory(fs, dirPath, mutationAllowed = () => true) {
    try {
...
async function activeFileSize(fs, activePath) {
    try {
...
async function lastByteIsNewline(fs, activePath, size) {
    if (size === 0)
        return true;
...
async function scanArchives(fs, archiveDir) {
    ...
    archives.sort((a, b) => a.generation - b.generation || a.name.localeCompare(b.name));
    return archives;
}
function lockRetries(maxRetryTime) {
    if (maxRetryTime <= 0)
        return 0;
    return {
        retries: 4,
...
async function acquireLease(input) {
    ...
    try {
        if (lease.compromised) {
            throw new Error(`Lease was compromised while acquiring ${input.lockPath}`);
        }
...
async function updateLatestBestEffort(fs, daemonDir, targetPath, mutationAllowed) {
...
async function readRecentFallback(fs, runsDir) {
    try {
        const raw = await fs.readFile(nodePath.join(runsDir, 'recent-fallback'), 'utf8');
        const name = raw.trim();
...
async function writeRecentFallback(fs, runsDir, name, mutationAllowed = () => true) {
...
async function listFallbackDirectories(fs, runsDir) {
...
async function fallbackActivityMtime(fs, familyDir) {
    try {
...
async function cleanupFallbackFamilies(input) {
    ...
    try {
        ...
        const retainedInactive = (input.preferredInactive && inactiveNames.has(input.preferredInactive)
            ? input.preferredInactive
            : undefined) ??
            (locator && inactiveNames.has(locator) ? locator : undefined) ??
            inactive
                .slice()
                .sort((a, b) => b.mtimeMs - a.mtimeMs || b.name.localeCompare(a.name))[0]?.name;
        for (const entry of inactive) {
            await entry.lease.release();
            if (entry.name === retainedInactive)
                continue;
            if (!mutationAllowed()) {
                throw new Error('Fallback cleanup lease was compromised');
            }
        ...
        const remaining = [...live, ...(retainedInactive ? [retainedInactive] : [])]
            ...
            .sort();
        const nextLocator = retainedInactive ??
            (locator && remaining.includes(locator) ? locator : remaining.at(-1));
        if (locator !== nextLocator) {
            await writeRecentFallback(input.fs, input.runsDir, nextLocator, mutationAllowed).catch(() => { });
        }
    }
    finally {
...
async function allocateFallbackFamily(input) {
    ...
    let lease;
    const maintenanceAllowsMutation = () => !maintenance.compromised && !maintenance.released;
    try {
...
async function removeFailedFallbackBestEffort(input) {
    ...
    try {
        ...
        if (maintenance.compromised || input.family.lease.compromised)
            return;
        await input.fs
...
async function closeFallbackFamily(input) {
    ...
    try {
        ...
        const mutationAllowed = () => !maintenance.compromised && !input.family.lease.compromised;
        await cleanupFallbackFamilies({
            ...input,
            runsDir,
...
async function cleanupFallbacksBestEffort(input) {
    ...
    try {
        ...
        const mutationAllowed = () => !currentMaintenance.compromised &&
...
function createConcreteLogger(input) {
    ...
    let closePromise;
    const markDropped = (bytes, queueOverflow = false) => {
        droppedRecords += 1;
        droppedBytes += bytes;
        if (queueOverflow) {
            ...
            if (!overflowEpisode) {
                ...
                input.stderr('LailatulCoder serve: daemon log queue limit reached; dropping file copies until capacity recovers');
            }
        }
    };
    const markRotationFailure = (message, error) => {
        input.issues.add('rotation_failed');
        input.state.nextRotationRetryAt =
            input.monotonicNow() + input.policy.rotationRetryIntervalMs;
        input.warnOnce(`rotation:${message}`, `LailatulCoder serve: daemon log rotation failed; ${message}${error
            ? `: ${error instanceof Error ? error.message : String(error)}`
            : ''}`);
    };
    const markRetentionFailure = (error) => {
        input.issues.add('retention_failed');
        input.warnOnce('retention', `LailatulCoder serve: daemon log retention cleanup failed: ${error instanceof Error ? error.message : String(error)}`);
    };
    const fileMutationAllowed = () => !input.state.poisoned &&
        !input.family.lease.compromised &&
        !input.family.lease.released;
    const prepareArchives = async () => {
        if (!input.state.archiveAvailable &&
            input.monotonicNow() < input.state.nextRotationRetryAt) {
            return false;
        }
        try {
            await ensureRealDirectory(input.fs, input.family.archiveDir, fileMutationAllowed);
            if (!fileMutationAllowed())
                return false;
            input.state.archives = await scanArchives(input.fs, input.family.archiveDir);
    ...
    const pruneForRotation = async () => {
        while (input.state.archives.length >= input.policy.maxArchives) {
            if (!fileMutationAllowed())
                return false;
            const oldest = input.state.archives[0];
            try {
                await input.fs.unlink(nodePath.join(input.family.archiveDir, oldest.name));
    ...
    const appendActive = async (record, forceRotation = false) => {
        if (!fileMutationAllowed())
            return 'dropped';
        const bytes = Buffer.byteLength(record, 'utf8');
        const needsRotation = input.state.currentBytes > 0 &&
            (forceRotation ||
        ...
        if (needsRotation) {
            ...
            if (!(await prepareArchives()))
                return 'dropped';
            if (!fileMutationAllowed())
                return 'dropped';
            if (!(await pruneForRotation()))
                return 'dropped';
            ...
            if (!fileMutationAllowed())
                return 'dropped';
            try {
                await input.fs.rename(input.family.activePath, nodePath.join(input.family.archiveDir, archive.name));
            }
            catch (error) {
            ...
            input.state.archives.sort((a, b) => a.generation - b.generation || a.name.localeCompare(b.name));
        ...
        try {
            ...
            input.warnOnce('write-failed', `LailatulCoder serve: daemon log write failed; file logging disabled for this run: ${error instanceof Error ? error.message : String(error)}`);
            return 'unknown';
        }
    };
    const unreportedDropRecord = () => {
        const records = droppedRecords - reportedDroppedRecords;
        const bytes = droppedBytes - reportedDroppedBytes;
        if (records <= 0)
            return undefined;
        return capUtf8Record(buildDaemonFileLogLine({
            level: 'WARN',
            message: 'daemon file log records dropped',
            now: input.now(),
            ctx: { droppedRecords: records, droppedBytes: bytes },
        }, input.runId, input.pid), input.policy.maxRecordBytes);
    };
    const enqueueRecord = (record, callerOriginated, onSettled) => {
        const bytes = Buffer.byteLength(record, 'utf8');
        if (pendingBytes + bytes > input.policy.maxPendingBytes) {
            if (callerOriginated)
                markDropped(bytes, true);
            return false;
        }
        pendingBytes += bytes;
        pending = pending
            ...
            try {
                ...
                if (callerOriginated && outcome === 'dropped')
                    markDropped(bytes);
            }
            catch (error) {
                input.state.poisoned = true;
                input.issues.add('write_failed');
                input.warnOnce('queue-failed', `LailatulCoder serve: daemon log queue failed; file logging disabled for this run: ${error instanceof Error ? error.message : String(error)}`);
            }
            finally {
                onSettled?.(outcome);
            }
    ...
    const enqueueDropSummary = () => {
        if (dropSummaryPending)
            return;
        ...
        if (enqueueRecord(summary, false, (outcome) => {
            if (outcome === 'written') {
    ...
    const submitFileRecord = (record) => {
        if (!acceptingFileRecords)
            return;
        const bytes = Buffer.byteLength(record, 'utf8');
        if (!fileMutationAllowed()) {
            markDropped(bytes);
    ...
    const tee = (level, message, ctx, err) => {
        ...
        submitFileRecord(capUtf8Record(buildDaemonFileLogLine({ level, message, now: timestamp, ctx, err, traceContext }, input.runId, input.pid), input.policy.maxRecordBytes));
    };
    const finalize = async () => {
        ...
        if (input.family.mode === 'fallback') {
            ...
            await input.family.releaseLease().catch((error) => {
                input.warnOnce('release-failed', `LailatulCoder serve: daemon log lease release failed: ${error instanceof Error ? error.message : String(error)}`);
            });
        }
    };
    const logger = {
        ...
        error: (message, err, ctx) => tee('ERROR', message, ctx, err ?? undefined),
        raw: (line, level = 'info') => {
            submitFileRecord(capUtf8Record(buildDaemonFileLogLine({
        ...
        getDaemonId: () => input.daemonId,
        getStatus: () => statusSnapshot(input.runId, input.family.mode, input.issues, droppedRecords, droppedBytes),
        flush: () => pending,
        close: () => {
            if (closePromise)
                return closePromise;
            ...
            const finalizer = finalize().catch((error) => {
                try {
                    input.warnOnce('close-failed', `LailatulCoder serve: daemon log close failed: ${error instanceof Error ? error.message : String(error)}`);
                }
                catch {
            ...
            const deadline = new Promise((resolve) => {
                timeout = setTimeout(() => {
                    try {
                        input.warnOnce('close-timeout', `LailatulCoder serve: daemon log close drain exceeded ${input.policy.closeDrainBudgetMs}ms; ownership will release after pending I/O settles or process exit`);
                    }
                    catch {
                        // close() is non-rejecting even if stderr itself is unavailable.
                    }
...
export async function initDaemonLogger(opts) {
    ...
    try {
        ...
        warnOnce('init', `LailatulCoder serve: daemon log disabled — init failed: ${error instanceof Error ? error.message : String(error)}`);
        return createStderrOnlyLogger({
            runId,
            daemonId: computeDaemonId(pid),
            now,
    ...
    let currentState;
    const onCompromised = (error) => {
        issues.add('lease_compromised');
        if (currentState)
            currentState.poisoned = true;
        warnOnce('lease-compromised', `LailatulCoder serve: daemon log lease compromised; file logging disabled for this run: ${error.message}`);
    };
    const tryFamily = async (family) => {
        ...
        try {
            ...
            if (family.mode === 'fallback' && currentBytes !== 0) {
                throw new Error('New fallback family unexpectedly contains an active log');
            }
            const completeTail = await lastByteIsNewline(fs, family.activePath, currentBytes);
            let archiveAvailable = true;
            let archives = [];
            const familyMutationAllowed = () => !family.lease.compromised && !family.lease.released;
            try {
                ...
                if (familyMutationAllowed()) {
                    ...
                    warnOnce(`archive-init:${family.mode}`, `LailatulCoder serve: daemon log archive unavailable; active file will remain bounded: ${error instanceof Error ? error.message : String(error)}`);
                }
            }
            state = {
                currentBytes,
            ...
            const bootTime = now();
            const plainBoot = capUtf8Record(buildDaemonFileLogLine({
                level: 'INFO',
                message: 'daemon started',
                now: bootTime,
                ctx: bootCtx,
            }, runId, pid), policy.maxRecordBytes);
            const incompleteTailBoot = !completeTail
                ? `\n${capUtf8Record(buildDaemonFileLogLine({
                    level: 'INFO',
                    message: 'daemon started',
                    now: bootTime,
                    ctx: { ...bootCtx, previousTailIncomplete: true },
                }, runId, pid), policy.maxRecordBytes - 1)}`
                : plainBoot;
            const wouldRotate = currentBytes > 0 &&
                currentBytes + Buffer.byteLength(incompleteTailBoot, 'utf8') >
                    policy.maxBytes;
            const bootRecord = !completeTail && !wouldRotate ? incompleteTailBoot : plainBoot;
            const bootOutcome = await concrete.writeInitialRecord(bootRecord, wouldRotate);
            if (bootOutcome !== 'written' ||
                state.poisoned ||
            ...
            if (state.archiveAvailable &&
                ...
                try {
                    while (state.archives.length > policy.maxArchives) {
                        if (family.lease.compromised || family.lease.released)
                            break;
                    ...
                    state.nextRotationRetryAt =
                        monotonicNow() + policy.rotationRetryIntervalMs;
                    warnOnce('startup-retention', `LailatulCoder serve: daemon log retention cleanup failed: ${error instanceof Error ? error.message : String(error)}`);
                }
            }
            return concrete.logger;
        }
        catch (error) {
            warnOnce(`family-init:${family.mode}`, `LailatulCoder serve: daemon ${family.mode} log initialization failed: ${error instanceof Error ? error.message : String(error)}`);
            return undefined;
        }
    };
    try {
        ...
        if (stableLogger) {
            if (!stableLease.compromised && !stableLease.released) {
                await updateLatestBestEffort(fs, daemonDir, stableFamily.activePath, () => !stableLease.compromised && !stableLease.released);
            }
            if (!stableLease.compromised && !stableLease.released) {
                await cleanupFallbacksBestEffort({
                    ...
                    onRetentionError: (error) => {
                        issues.add('retention_failed');
                        warnOnce('fallback-retention', `LailatulCoder serve: daemon fallback log cleanup failed: ${error instanceof Error ? error.message : String(error)}`);
                    },
                });
            }
            return stableLogger;
        ...
        try {
            ...
            warnOnce('stable-release', `LailatulCoder serve: daemon stable log lease release failed; trying fallback: ${error instanceof Error ? error.message : String(error)}`);
        }
    }
    catch (error) {
        if (isErrno(error, 'ELOCKED')) {
            warnOnce('stable-contention', `LailatulCoder serve: daemon stable log is owned by another daemon instance; using a fallback log for runId=${runId}`);
        }
        else {
            issues.add('init_failed');
            warnOnce('stable-lock', `LailatulCoder serve: daemon stable log unavailable; trying a fallback: ${error instanceof Error ? error.message : String(error)}`);
        }
    }
    let fallbackFamily;
    try {
        ...
        try {
            ...
            try {
                if (!activeFallbackFamily.lease.compromised) {
                    await writeRecentFallback(fs, runsDir, nodePath.basename(activeFallbackFamily.familyDir), () => !maintenance.compromised &&
            ...
            warnOnce('fallback-locator', `LailatulCoder serve: daemon fallback locator update failed; use ${activeFallbackFamily.activePath}: ${error instanceof Error ? error.message : String(error)}`);
        }
        return fallbackLogger;
    }
    catch (error) {
        ...
        warnOnce('fallback-init', `LailatulCoder serve: daemon log disabled — fallback init failed: ${error instanceof Error ? error.message : String(error)}`);
        return createStderrOnlyLogger({
            runId,
            daemonId: computeDaemonId(pid),
            now,
