...
// file via `@lailatul-coder/lailatul-coder-core`. The `constants?.X ?? 0` accesses
 ...
 *  load this module transitively via `@lailatul-coder/lailatul-coder-core` without
...
let untrackedOpenFlagsCache;
function getUntrackedOpenFlags() {
...
export async function fetchGitDiff(cwd) {
    ...
    if (!gitRoot)
        return null;
    if (await isInTransientGitState(gitRoot))
        return null;
    ...
    if (untrackedCount > 0) {
        ...
        const lineStats = await mapWithConcurrency(untrackedPaths, MAX_FILES, (relPath) => countUntrackedLines(path.join(gitRoot, relPath)));
        for (const s of lineStats)
            stats.linesAdded += s.added;
 ...
 */
export async function fetchGitDiffHunks(cwd) {
    ...
    const diffOut = await runGit(['--no-optional-locks', 'diff', '--no-ext-diff', '--no-textconv', 'HEAD'], gitRoot);
    if (diffOut == null)
        return new Map();
 ...
 * with no changes.
 */
export async function fetchGitDiffHunksForFile(cwd, filePath, oldPath) {
    const gitRoot = findGitRoot(cwd);
    if (!gitRoot)
        return null;
    ...
    const oldRelPath = oldPath != null ? toRepoRelativePath(gitRoot, oldPath) : null;
    const diffArgs = [
        '--no-optional-locks',
        'diff',
        '--no-ext-diff',
    ...
    if (parsed.size > 0) {
        const [key, hunks] = parsed.entries().next().value;
 ...
 */
function toRepoRelativePath(gitRoot, filePath) {
 ...
 */
async function synthesizeUntrackedHunk(gitRoot, filePath) {
    ...
    try {
        ...
        const truncated = st.size > MAX_DIFF_SIZE_BYTES || lines.length > capped.length;
        if (capped.length === 0)
...
function forEachNumstatEntry(stdout, visit) {
...
export function parseGitNumstat(stdout) {
    ...
    forEachNumstatEntry(stdout, (entry) => {
        commitEntry(entry.path, entry.added, entry.removed, entry.isBinary, entry.oldPath);
    });
    function commitEntry(filePath, fileAdded, fileRemoved, isBinary, oldPath) {
        validFileCount++;
        added += fileAdded;
        removed += fileRemoved;
 ...
 */
export function parseGitDiff(stdout, truncatedPaths) {
 ...
 */
export function unquoteCStylePath(s) {
...
function extractFilePath(lines) {
    ...
    const normalize = (s) => unquoteCStylePath(stripTab(s));
    if (renameTo !== null)
        return normalize(renameTo);
    if (copyTo !== null)
        return normalize(copyTo);
    if (plus !== null) {
...
export function parseShortstat(stdout) {
    const match = stdout.match(/^ ?(\d{1,10}) files? changed(?:, (\d{1,10}) insertions?\(\+\))?(?:, (\d{1,10}) deletions?\(-\))?$/m);
    if (!match)
        return null;
    return {
 ...
 */
export function parseDeletedFromNameStatus(stdout) {
    const tokens = stdout.split('\0');
    if (tokens.length > 0 && tokens[tokens.length - 1] === '')
        tokens.pop();
...
function countNulDelimited(stdout) {
    if (!stdout)
        return 0;
    let count = 0;
...
function splitNulDelimited(stdout) {
    if (!stdout)
        return [];
...
async function isInTransientGitState(gitRoot) {
    ...
    const results = await Promise.all(transientPaths.map((name) => access(path.join(gitDir, name))
        .then(() => true)
        .catch(() => false)));
...
export async function getGitWorkingTreeStatus(cwd) {
    const gitRoot = findGitRoot(cwd);
    if (!gitRoot)
        return null;
    const stdout = await runGit(['--no-optional-locks', 'status', '--porcelain=v1', '--branch', '-z'], gitRoot);
    if (stdout == null)
        return null;
 ...
 * on branch` (unborn).
 */
export function parseStatusBranchLine(line) {
    if (!line.startsWith('## '))
        return { ...NO_BRANCH };
 ...
 * skipped.
 */
export function parseStatusEntries(tokens) {
    let staged = 0;
    let unstaged = 0;
    let untracked = 0;
    let conflicted = 0;
    for (let i = 0; i < tokens.length; i++) {
        ...
        if (x === '!' && y === '!')
            continue; // ignored; not emitted without --ignored
 ...
 */
async function detectGitOperation(gitRoot) {
    const gitDir = await resolveGitDirFromRoot(gitRoot);
    if (!gitDir)
        return undefined;
    const checks = [
...
const LOG_DETAIL_FORMAT = '%H%x00%h%x00%an%x00%ae%x00%at%x00%s%x00%D%x00%P%x00%b';
function parseLogFields(parts) {
    if (parts.length !== 8)
        return null;
    return {
        sha: parts[0],
 ...
 */
export async function fetchGitLog(cwd, options) {
    const gitRoot = findGitRoot(cwd);
    if (!gitRoot)
        return null;
    const limit = Math.min(Math.max(options?.limit ?? DEFAULT_LOG_LIMIT, 1), MAX_LOG_LIMIT);
 ...
 * or git fails.
 */
export async function fetchGitCommitDetail(cwd, sha) {
    ...
    const diffTreeArgs = parents.length > 1
        ? [
            '--no-optional-locks',
            'diff-tree',
            '--no-commit-id',
