...
function formatEntryPath(entryPath) {
    const sanitized = stripAnsiAndControl(entryPath);
    if (sanitized.length <= MAX_REPORTED_ENTRY_PATH_LENGTH)
        return sanitized;
    return `${sanitized.slice(0, MAX_REPORTED_ENTRY_PATH_LENGTH - 3)}...`;
}
export async function assertTarArchiveHasNoLinks(file, signal) {
    const unsupportedLinkPaths = [];
    let unsupportedLinkCount = 0;
    let linkLimitError;
    const onReadEntry = (entry) => {
        if (entry.type === 'SymbolicLink' || entry.type === 'Link') {
            unsupportedLinkCount += 1;
            const unsupportedLinkPath = formatEntryPath(entry.path) || '<sanitized empty path>';
            ...
            if (unsupportedLinkCount > MAX_LINK_ENTRIES &&
                ...
                linkLimitError = new Error(`Tar archive contains more than ${MAX_LINK_ENTRIES} unsupported link entries: ${unsupportedLinkPaths.join(', ')}`);
            }
        }
    };
    ...
    if (linkLimitError)
        throw linkLimitError;
    if (unsupportedLinkCount > 0) {
        const entryLabel = unsupportedLinkCount === 1
            ? 'unsupported link entry'
            : `${unsupportedLinkCount} unsupported link entries`;
        throw new Error(`Tar archive contains ${entryLabel}: ${unsupportedLinkPaths.join(', ')}`);
    }
}
//# sourceMappingURL=archive-safety.js.map