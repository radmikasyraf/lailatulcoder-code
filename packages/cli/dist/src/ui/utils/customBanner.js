...
import { createDebugLogger, stripTerminalControlSequences, TERMINAL_OSC_REGEX, TERMINAL_CSI_REGEX, TERMINAL_SHIFT_DCS_REGEX, } from '@lailatul-coder/lailatul-coder-core';
 ...
 */
export function resolveCustomBanner(settings) {
...
function collectScopedTiers(settings) {
    ...
    for (const file of order) {
        ...
        const considerTier = (tier, label) => {
            if (tier === undefined)
                return undefined;
            const isPathSource = typeof tier === 'object';
            if (isPathSource && !dir) {
                debugLogger.warn(`Ignoring ui.customAsciiArt.${label}: {path} entry has no owning settings file directory to resolve against.`);
                return undefined;
            }
...
function normalizeTiers(value) {
    ...
    if (!value || typeof value !== 'object') {
        debugLogger.warn('Ignoring ui.customAsciiArt: expected a string, {path}, or {small,large} object.');
        return undefined;
    }
    ...
    if (hasPath && hasTierKeys) {
        debugLogger.warn('Ignoring ui.customAsciiArt: object combines `path` with `small` / `large`. Use one shape or the other.');
        return undefined;
    }
    if (hasPath) {
        return { small: value, large: value };
...
function validateSource(source) {
    ...
    debugLogger.warn('Ignoring ui.customAsciiArt tier: expected a string or {path} object.');
    return undefined;
}
...
function readArtFile(absolutePath) {
    ...
    try {
        ...
        try {
            ...
            debugLogger.warn(`Failed to stat ui.customAsciiArt at ${absolutePath}: ${err.message}`);
            return undefined;
        }
        if (!preOpenStat.isFile()) {
            debugLogger.warn(`Ignoring ui.customAsciiArt: ${absolutePath} is not a regular file.`);
            return undefined;
        }
        ...
        const flags = typeof fs.constants.O_NOFOLLOW === 'number'
        ...
        if (stat.size > MAX_FILE_BYTES) {
            debugLogger.warn(`Truncated ui.customAsciiArt at ${absolutePath}: file is ${stat.size} bytes, capped at ${MAX_FILE_BYTES}.`);
        }
        return buffer.toString('utf8');
    }
    catch (err) {
        debugLogger.warn(`Failed to read ui.customAsciiArt at ${absolutePath}: ${err.message}`);
        return undefined;
    }
    finally {
        if (fd !== undefined) {
 ...
 * `@lailatul-coder/lailatul-coder-core`) so the regexes are authored once, but
...
function sanitizeArt(input) {
    ...
    const cappedLines = limitedLines.map((line) => {
        ...
        for (const cp of codePoints) {
            if (getCachedStringWidth(kept + cp) > MAX_ART_COLS)
                break;
            kept += cp;
        }
        return kept;
    ...
    if (truncatedCols) {
        debugLogger.warn(`Truncated ui.customAsciiArt to ${MAX_ART_COLS} columns per line.`);
    }
...
function sanitizeSubtitle(raw) {
    return sanitizeSingleLine(raw, MAX_SUBTITLE_LENGTH, 'ui.customBannerSubtitle');
}
/**
 ...
 * logo column entirely (fall back to the default LailatulCoder logo or no logo)".
 */
export function pickAsciiArtTier(small, large, availableWidth, logoGap, minInfoPanelWidth, measureWidth) {
...
//# sourceMappingURL=customBanner.js.map