...
import { findingsPointerOf, promptRecordDir, recordPrompt, writeFindingsFile, } from './prompt-record.js';
...
describe('scheduleReverseAuditRound — the scheduler on its own', () => {
     ...
     */
    function record(round, chunk, body, digest = 'abc123') {
        const prompt = `reverse-audit ${body}`;
        recordPrompt(plan, `reverse-audit--chunk-${chunk}--round-${round}--${digest}`, prompt);
        return prompt;
    }
    /** Where `record` put a key's file — for tests that backdate one. */
    function recordFile(round, chunk, digest) {
        return join(promptRecordDir(plan), `${encodeURIComponent(`reverse-audit--chunk-${chunk}--round-${round}--${digest}`)}.txt`);
    }
    /**
     ...
     * and nothing else.
     */
    function transcript(launchPrompt, finalText, calls = 1, filePath = diff, offset = 0, limit = 100) {
        const id = `aud-${++seq}`;
        const base = {
            agentId: id,
    ...
    function schedule(round, chunks = [13, 14, 15]) {
    ...
    it('successful calls that never touched the diff are not dry — the two guards are independent', () => {
        ...
        transcript(record(1, 13, 'chunk 13 round 1 territory walk'), DRY);
        transcript(record(2, 13, 'chunk 13 round 2 territory walk'), DRY, 1, join(dir, 'brief.md'));
        const r3 = schedule(3, [13]);
        expect(r3.due).toEqual([13]);
        expect(r3.skipped).toEqual([]);
    });
    ...
    it("records older than the plan are a dead attempt's — the retry still retires", () => {
        ...
        for (const r of [1, 2]) {
            ...
            fresh.push(record(r, 13, `chunk 13 round ${r} territory walk\nwith the retry's grown findings list`));
        }
    ...
    it('an honest short English receipt is dry — structure, not a length floor', () => {
        ...
        const receipt = 'No issues found — re-walked the retry cap and both changed ' +
            "exports' call sites.";
    ...
    it('a bolded Chinese phrase is dry, exactly like the English one', () => {
        const receipt = '**未发现新问题** —— 重新走查了重连状态机与两个已改导出的全部调用点,' +
            '每个疑点都已在确认清单中。';
        transcript(record(1, 13, 'chunk 13 round 1 territory walk'), receipt);
        transcript(record(2, 13, 'chunk 13 round 2 territory walk'), receipt);
        const r3 = schedule(3, [13]);
    ...
    it('a parenthesised scope between phrase and separator is dry', () => {
        ...
        // territory.
        const receipt = 'No new issues found (chunk 13) — re-walked the retry cap and both ' +
    ...
    it('the bare zh stock sentence is not dry, exactly like the English one', () => {
        transcript(record(1, 13, 'chunk 13 round 1 territory walk'), DRY);
        transcript(record(2, 13, 'chunk 13 round 2 territory walk'), '未发现问题。');
        expect(schedule(3, [13]).due).toEqual([13]);
    });
    it('a receipt whose clause names nothing is not dry', () => {
        ...
        transcript(record(1, 13, 'chunk 13 round 1 territory walk'), DRY);
        transcript(record(2, 13, 'chunk 13 round 2 territory walk'), 'No new issues found — all good.');
        expect(schedule(3, [13]).due).toEqual([13]);
    });
    ...
    it('an echoed file line is not a yield — and its prose lead is not the form (#9213)', () => {
        ...
        // filing check.
        const echo = 'The cumulative list already covers **File:** src/pay.ts:42 — not ' +
            're-reporting it.\n\n' +
            DRY;
        for (const r of [1, 2]) {
    ...
    ])('an admission stays marked, however the absence-of-problems phrasing tempts an exception: %s (#9272)', (_label, leaked, failure) => {
    ...
    it('an honest absence-of-problems receipt stays under audit — the accepted residue (#9272)', () => {
        ...
        for (const r of [1, 2]) {
            transcript(record(r, 13, `chunk 13 round ${r} territory walk`), 'No issues found — verified no regressions in the reconnect path ' +
    ...
    it('an echo in a walk verb\u2019s object seat restates the all-clear — the form refuses it (#9272)', () => {
        ...
        for (const r of [1, 2]) {
            transcript(record(r, 13, `chunk 13 round ${r} territory walk`), 'No issues found — re-walked the scheduler and verified no ' +
    ...
    it('a lead filler carrying walk vocabulary still retires — the lead never restates (#9272)', () => {
        ...
        const receipt = 'No issues found after verification — re-walked the parser and both ' +
            'of its call sites.';
        transcript(record(1, 13, 'chunk 13 round 1 territory walk'), receipt);
        transcript(record(2, 13, 'chunk 13 round 2 territory walk'), receipt);
        const r3 = schedule(3, [13]);
    ...
    it('a Chinese receipt separated by a full-width colon is dry', () => {
        ...
        const receipt = '未发现问题：重新走查了重连状态机与两个已改导出的全部调用点,' +
            '每个疑点都已在确认清单中。';
        transcript(record(1, 13, 'chunk 13 round 1 territory walk'), receipt);
        transcript(record(2, 13, 'chunk 13 round 2 territory walk'), receipt);
        const r3 = schedule(3, [13]);
    ...
    it('an English receipt separated by a period is dry (#9206)', () => {
        ...
        const receipt = 'No new issues were found. Re-walked the retry cap and both changed ' +
    ...
    ])('a receipt separated by %s is dry, like the period and full-width comma (#9206)', (_label, receipt) => {
    ...
    it('a hedged receipt is not dry — a clause that CONTRADICTS the phrase proves no walk (#9206)', () => {
        ...
        const hedged = 'No new issues were found, but I could not open the generated files ' +
    ...
    it('a dash-led hedge is not dry either — the polarity guard covers every separator path (#9206)', () => {
        const hedged = 'No new issues were found — but the generated files would not open ' +
    ...
    it('prose on EITHER side of the receipt line is refused alike (#9213)', () => {
        ...
        const receiptLine = 'No new issues found — re-walked the reconnect flow and both ' +
            "changed exports' call sites.";
        const trailing = 'The list already covered the timer path, so I did not re-report it.';
        transcript(record(1, 13, 'chunk 13 round 1 territory walk'), receiptLine + '\n' + trailing);
        transcript(record(2, 13, 'chunk 13 round 2 territory walk'), receiptLine + '\n' + trailing);
        transcript(record(1, 14, 'chunk 14 round 1 territory walk'), trailing + '\n' + receiptLine);
        transcript(record(2, 14, 'chunk 14 round 2 territory walk'), trailing + '\n' + receiptLine);
        const r3 = schedule(3, [13, 14]);
        expect(r3.due).toEqual([13, 14]);
        expect(r3.skipped).toEqual([]);
        expect(r3.diagnostics).toEqual([
    ...
    it('layer receipts above the receipt do not break the form (#9213)', () => {
        ...
        const ret = 'Layer walked: token-layer — comment tokens and globs examined.\n' +
    ...
    it('a quoted layer line is never stripped — fence and blockquote stay, and fail the form (#9259)', () => {
        ...
        const blockquoted = 'No issues found — re-walked the parser and the retry cap call sites.\n' +
            '> Layer walked: lexing';
        for (const r of [1, 2]) {
            transcript(record(r, 13, `chunk 13 round ${r} territory walk`), fenced);
            transcript(record(r, 14, `chunk 14 round ${r} territory walk`), blockquoted);
        }
        const r3 = schedule(3, [13, 14]);
        expect(r3.due).toEqual([13, 14]);
        expect(r3.skipped).toEqual([]);
    ...
    it('a receipt split across two lines is not dry — the matcher is line-bound (#9213)', () => {
        ...
        shapes.forEach(([ret], i) => {
            const chunk = 13 + i;
            transcript(record(1, chunk, `chunk ${chunk} round 1 territory walk`), ret);
            transcript(record(2, chunk, `chunk ${chunk} round 2 territory walk`), ret);
        });
        const r3 = schedule(3, shapes.map((_, i) => 13 + i));
        expect(r3.due).toEqual([13, 14, 15, 16]);
        expect(r3.skipped).toEqual([]);
        expect(r3.diagnostics).toEqual(shapes.map(([, failure], i) => `chunk ${13 + i} — round 1: ${failure}; round 2: ${failure}`));
    });
    ...
    it('a budget-gap line parted by a blank line does not block the receipt (#9213)', () => {
        ...
        const receipt = 'Budget gap: walked the parser only\n\n' +
    ...
    it('quoting the stock phrase to NEGATE it is not a receipt (#9206)', () => {
        ...
        // never the lead.
        const quoted = 'I cannot write "No new issues were found." I could not open the ' +
    ...
    it('a quoted phrase with a marker-free walk clause is refused ONLY by the anchor (#9259)', () => {
        ...
        const quoted = 'I cannot write "No new issues were found." I re-walked the ' +
    ...
    it('a hedged clause carrying a code span is refused by polarity, not saved by the object (#9259)', () => {
        ...
        const hedged = 'No new issues found; I could not open `gen/output.ts` and left ' +
            'it unchecked.';
        transcript(record(1, 13, 'chunk 13 round 1 territory walk'), hedged);
        transcript(record(2, 13, 'chunk 13 round 2 territory walk'), hedged);
        const r3 = schedule(3, [13]);
    ...
    it('a transcript that never reads the diff reads "no read of the diff", even with territory baked (#9259)', () => {
        ...
        for (const r of [1, 2]) {
            const built = record(r, 13, `chunk 13 round ${r} walk — ` +
    ...
    it('a yield suppresses the diagnostic for the chunk it explains (#9259)', () => {
        ...
        transcript(record(1, 13, 'chunk 13 round 1 territory walk'), 'Walked the territory carefully and studied every edge case in it.');
        transcript(record(2, 13, 'chunk 13 round 2 territory walk'), YIELD);
        const r3 = schedule(3, [13]);
        expect(r3.due).toEqual([13]);
        expect(r3.diagnostics).toEqual([]);
    ...
    it('diagnoses a matched transcript whose receipt fell at a named bar (#9206)', () => {
        ...
        const noReceipt = 'Walked the territory carefully and studied every edge case in it.';
    ...
    it('diagnoses a launch that never read the diff (#9206)', () => {
        ...
        for (const r of [1, 2]) {
            transcript(record(r, 13, `chunk 13 round ${r} territory walk`), DRY, 1, join(dir, 'brief.md'));
        }
        const r3 = schedule(3, [13]);
        expect(r3.due).toEqual([13]);
        expect(r3.diagnostics).toEqual([
    ...
    it('diagnoses a receipt whose clause names no walk (#9206)', () => {
        ...
        for (const r of [1, 2]) {
            transcript(record(r, 13, `chunk 13 round ${r} territory walk`), 'No new issues found — all good.');
        }
        const r3 = schedule(3, [13]);
        expect(r3.due).toEqual([13]);
        expect(r3.diagnostics).toEqual([
    ...
    it('diagnoses a clause that names a walk but stays under the substance floor (#9259)', () => {
        ...
        for (const r of [1, 2]) {
            transcript(record(r, 13, `chunk 13 round ${r} territory walk`), 'No issues found — walked lexing');
        }
        const r3 = schedule(3, [13]);
        expect(r3.due).toEqual([13]);
        expect(r3.diagnostics).toEqual([
    ...
    it('diagnoses a lead-side hedge as the lead contradicting the phrase (#9259)', () => {
        ...
        for (const r of [1, 2]) {
            transcript(record(r, 13, `chunk 13 round ${r} territory walk`), 'No issues found but only skimmed. Re-walked the parser.');
        }
        const r3 = schedule(3, [13]);
        expect(r3.due).toEqual([13]);
        expect(r3.diagnostics).toEqual([
    ...
    it('diagnoses a clause-side admission as the clause contradicting the phrase (#9259)', () => {
        for (const r of [1, 2]) {
            transcript(record(r, 13, `chunk 13 round ${r} territory walk`), 'No issues found — re-walked the parser but skipped the generated files');
        }
        const r3 = schedule(3, [13]);
        expect(r3.due).toEqual([13]);
        expect(r3.diagnostics).toEqual([
    ...
    it('diagnoses an ambiguous launch — one transcript matching several records (#9206)', () => {
        ...
        for (const r of [1, 2]) {
            ...
            const b = record(r, 13, `chunk 13 round ${r} rules-corrected rebuild walk`, 'fff999');
            transcript([a, b].join('\n\n'), DRY);
        }
        const r3 = schedule(3, [13]);
        expect(r3.due).toEqual([13]);
    ...
    it("parroting the brief's own example receipt is not dry", () => {
        ...
        transcript(record(1, 13, 'chunk 13 round 1 territory walk'), DRY);
        transcript(record(2, 13, 'chunk 13 round 2 territory walk'), `${REVERSE_AUDIT_EXAMPLE_RECEIPT}.`);
        expect(schedule(3, [13]).due).toEqual([13]);
    });
    it('a case-shifted echo of the example receipt is not dry either (#9213)', () => {
        ...
        transcript(record(1, 13, 'chunk 13 round 1 territory walk'), DRY);
        transcript(record(2, 13, 'chunk 13 round 2 territory walk'), 'No issues found. Re-walked the reconnect state machine and the ' +
    ...
    it('an echo cannot lend the floor its substance — the restatement bar refuses it first (#9272)', () => {
        ...
        for (const r of [1, 2]) {
            transcript(record(r, 13, `chunk 13 round ${r} territory walk`), 'No issues found — re-walked, no issues found, no issues found.');
        }
        const r3 = schedule(3, [13]);
        expect(r3.due).toEqual([13]);
        expect(r3.diagnostics).toEqual([
    ...
    it('a Chinese clause of four ideographs clears the substance floor (#9259)', () => {
        ...
        for (const r of [1, 2]) {
            transcript(record(r, 13, `chunk 13 round ${r} territory walk`), '未发现问题——走查解析');
        }
        const r3 = schedule(3, [13]);
        expect(r3.due).toEqual([]);
        expect(r3.skipped.map((s) => s.chunkId)).toEqual([13]);
    ...
    it('a Chinese clause of three ideographs stays under the floor (#9259)', () => {
        ...
        for (const r of [1, 2]) {
            transcript(record(r, 13, `chunk 13 round ${r} territory walk`), '未发现问题——走查了');
        }
        const r3 = schedule(3, [13]);
        expect(r3.due).toEqual([13]);
        expect(r3.diagnostics).toEqual([
    ...
    it('a quoted marker inside the clause contradicts the phrase (#9213)', () => {
        ...
        const receipt = 'No issues found — the "could not reproduce" note was already ' +
    ...
    it('the conjunction "and/or" is not a path', () => {
        transcript(record(1, 13, 'chunk 13 round 1 territory walk'), DRY);
        transcript(record(2, 13, 'chunk 13 round 2 territory walk'), 'No issues found — and/or cases.');
        expect(schedule(3, [13]).due).toEqual([13]);
    });
    ...
    it('an overlapping read of the baked territory still retires', () => {
        ...
        for (const r of [1, 2]) {
            const built = record(r, 13, `chunk 13 round ${r} walk — ` +
                `read_file(file_path="${diff}", offset=1000, limit=200)`);
            transcript(built, DRY, 1, diff, r === 1 ? 1000 : 1100, r === 1 ? 200 : 50);
        }
        const r3 = schedule(3, [13]);
        expect(r3.due).toEqual([]);
        expect(r3.converged).toBe(true);
    ...
    it('quoting a WHOLE cumulative-list entry is not a yield', () => {
        ...
        const quoted = 'The list already carries this entry, so it is not re-reported:\n' +
            '- **File:** src/pay.ts:42\n' +
            '- **Severity:** Suggestion\n\n' +
            DRY;
        for (const r of [1, 2]) {
    ...
    it('an auditor that SKIPPED the findings read cannot retire the chunk', () => {
        ...
        for (const r of [1, 2]) {
            const findingsFile = writeFindingsFile(plan, `reverse-audit--round-${r}--skip99`, '- **File:** src/pay.ts:42 — the double charge\n' +
                '- **Severity:** Suggestion\n');
            const built = record(r, 13, `chunk 13 round ${r} territory\n` +
            ...
            writeFileSync(join(dir, 'subagents', 'S1', `agent-${id}.jsonl`), [
                JSON.stringify({
                    ...base,
                    type: 'user',
    ...
    function deadTranscript(launchPrompt, narration) {
    ...
    it('a filed YIELD survives a skipped findings read — the bar gates dry only', () => {
        ...
        transcript(record(1, 13, 'chunk 13 round 1 territory walk'), DRY);
        const findingsFile = writeFindingsFile(plan, 'reverse-audit--round-2--yield7', '- **File:** src/pay.ts:42 — the double charge\n' +
            '- **Severity:** Suggestion\n');
    ...
    it('quoting a WHOLE entry from the findings FILE is not a yield (post-#8597 shape)', () => {
        ...
        const quoted = 'The list already carries this entry, so it is not re-reported:\n' +
            ...
            DRY;
        for (const r of [1, 2]) {
            const findingsFile = writeFindingsFile(plan, `reverse-audit--round-${r}--abc123`, '- **File:** src/pay.ts:42 — the double charge\n' +
                '- **Severity:** Suggestion\n');
            const built = record(r, 13, `chunk 13 round ${r} territory\n` +
                `read_file(file_path="${findingsFile}")`);
            transcript(built, quoted);
    ...
    it('a pointer outside the record dir is not followed — degrades to the prompt', () => {
        ...
        writeFileSync(outside, '- **File:** src/pay.ts:42 — the double charge\n' +
            '- **Severity:** Suggestion\n');
        const quoted = 'The list already carries this entry, so it is not re-reported:\n' +
            '- **File:** src/pay.ts:42\n' +
            '- **Severity:** Suggestion\n\n' +
    ...
    it('a missing findings file is not cross-contaminated between chunks of a round', () => {
        ...
        const quoted = 'The list already carries this entry, so it is not re-reported:\n' +
            ...
            DRY;
        for (const r of [1, 2]) {
            const b13 = record(r, 13, `chunk 13 round ${r} territory\n**File:** src/pay.ts:42\n` +
                `read_file(file_path="${missing}")`);
            const b14 = record(r, 14, `chunk 14 round ${r} territory\n` + `read_file(file_path="${missing}")`);
            transcript(b13, DRY);
    ...
    it('two live records for ONE (chunk, round): any yield keeps it hot', () => {
        ...
        transcript(record(1, 13, 'chunk 13 round 1 territory walk'), DRY);
        transcript(record(2, 13, 'chunk 13 round 2 territory walk', 'aaa111'), YIELD);
        transcript(record(2, 13, 'chunk 13 round 2 rules-corrected rebuild walk', 'fff999'), DRY);
        expect(schedule(3, [13]).due).toEqual([13]);
    });
    it('the same-round pair still outranks retirement with the digest order flipped', () => {
        ...
        transcript(record(1, 13, 'chunk 13 round 1 territory walk'), DRY);
        transcript(record(2, 13, 'chunk 13 round 2 territory walk', 'fff999'), YIELD);
        transcript(record(2, 13, 'chunk 13 round 2 rules-corrected rebuild walk', 'aaa111'), DRY);
        expect(schedule(3, [13]).due).toEqual([13]);
    });
...
describe('scheduleReverseAuditRound — a resumed run reads the prior attempt', () => {
    ...
    function record(round, chunk, body) {
        const prompt = `reverse-audit ${body}`;
        recordPrompt(plan, `reverse-audit--chunk-${chunk}--round-${round}--abc123`, prompt);
        return prompt;
    }
    /** A dry-receipt transcript, written into the named session's dir. */
    function transcriptIn(session, launchPrompt) {
        const id = `aud-${++seq}`;
        const base = {
            agentId: id,
            agentName: 'general-purpose',
    ...
    function ledger(...ids) {
        ...
        ids.forEach((id, i) => appendRunSession(plan, { QWEN_CODE_SESSION_ID: id }, i === ids.length - 1 ? nowMs + 1500 : nowMs));
...
//# sourceMappingURL=retirement.test.js.map