...
import { clearReviewWorktreeLeaseIfOwned, createReviewWorktreeLease, readReviewWorktreeLease, reviewLeaseHeldByAnotherSession, reviewLeasePath, } from '../../services/review-worktree-lease.js';
import { sanitizedGitEnv } from './lib/worktree.js';
import { setGhHost } from './lib/gh.js';
import { getPlatformReader } from './lib/platform/registry.js';
import { git, gitOpt, gitProbe as gitExit, gitRaw, refExists, releaseWorktree, } from './lib/git.js';
...
import { appendRunSession, ledgerResumeCount, readResumeMarker, recordResume, recordRestart, RESUME_MAX, } from './lib/run-ledger.js';
import { assessResume, } from './lib/resume.js';
import { hasReviewDeadline, readBudgetStop, clearBudgetStop, clearRoundStamps, } from './lib/deadline.js';
 ...
 */
export function resolveIncrementalAnchor(rawSince, fetchedSha, probe, mergeBase = null, options = {}) {
 ...
 */
const GIT_PSEUDO_REFS = /^(FETCH|ORIG|MERGE|CHERRY_PICK|REVERT|REBASE|BISECT)_HEAD$/i;
function isPlainBranchName(name) {
    return (name.toUpperCase() !== 'HEAD' &&
...
const gitProbe = {
    ...
    fetch: (remote, ref) => gitExit('fetch', remote, '--', `+refs/heads/${ref}:refs/remotes/${remote}/${ref}`).status === 0 && refExists(`refs/remotes/${remote}/${ref}`),
    refExists,
    mergeBase: (a, b) => {
        ...
        const { out, status } = gitExit('-c', 'core.commitGraph=false', 'merge-base', a, b);
        if (status === 0)
            return out;
        if (status === 1)
            return null;
...
function cleanStale(prNumber) {
    ...
    if (existed && !freed) {
        writeStderrLine(`Could not free the stale worktree at ${worktreePath(prNumber)}: ${reason}`);
    }
 ...
 */
function tryResume(args, wt, platform) {
    ...
    const markerResumes = marker.resumes.filter((r) => r.sessionId.toLowerCase() !== currentKey).length;
    ...
    // in the PR.
    const status = gitOpt('-C', wt, 'status', '--porcelain', '--untracked-files=normal');
    ...
    if (!ruling.ok) {
        return {
            ...
            priorFetchedSha: prev !== null && typeof prev.fetchedSha === 'string'
                ? prev.fetchedSha
                : null,
        };
    }
    ...
    const pinnedEffort = prev !== null && typeof prev.effort === 'string' && prev.effort !== ''
        ? prev.effort
        : 'high';
    ...
    writeStderrLine(`Resumed PR #${prNumber} review (resume ${attempt} of ${RESUME_MAX}): ` +
...
async function runFetchPr(args) {
    ...
    if (!/^\d+$/.test(prNumber) || Number(prNumber) <= 0) {
        throw new Error(`fetch-pr: pr_number must be a positive integer, got ${JSON.stringify(prNumber)}`);
    }
    ...
    if (!/^[1-9]\d*$/.test(prNumber)) {
        throw new Error(`pr_number must be a positive integer, got ${JSON.stringify(prNumber)}`);
    }
    const ref = reviewBranch(prNumber);
    ...
    if (!sessionId || !promptId) {
        throw new Error(`fetch-pr: QWEN_CODE_SESSION_ID and QWEN_CODE_PROMPT_ID must both ` +
    ...
    if (reviewLeaseHeldByAnotherSession(holder)) {
        throw new Error(`PR #${prNumber} is already being reviewed by another session ` +
    ...
    try {
        ...
        if (args.resume) {
            ...
            writeStdoutLine(JSON.stringify({ resumed: false, resumeRefused: outcome.reason }));
            writeStderrLine(`Cannot resume PR #${prNumber} (${outcome.reason}); starting a fresh review.`);
        }
        ...
        try {
            git('fetch', remote, `${platform.fetchHeadRefSpec(Number(prNumber))}:${ref}`);
        }
        catch (err) {
        ...
        try {
            ...
            if (!isPlainBranchName(meta.baseRefName)) {
                throw new Error(`refusing base ref ${JSON.stringify(meta.baseRefName)} from the ` +
        ...
        try {
            ({ sha: mergeBaseSha, baseFetchFailed } = resolveMergeBase(remote, meta.baseRefName, 
            // QUALIFIED — the head side is dwim-shadowable exactly like the
            // fetchedSha read above.
            `refs/heads/${ref}`, gitProbe));
        }
        catch (err) {
        ...
        const readRange = (left) => {
            try {
                ...
                return gitRaw(...PINNED_DIFF_CONFIG, 'diff', ...PINNED_DIFF_FLAGS, `${left}..${fetchedSha}`);
            }
            catch (err) {
        ...
        const rawSince = Array.isArray(args.since)
            ? args.since[args.since.length - 1]
            : args.since;
        ...
        const rawSinceModel = Array.isArray(args.sinceModel)
            ...
            : args.sinceModel;
        const sinceModel = typeof rawSinceModel === 'string' ? rawSinceModel : undefined;
        const crossModel = sinceArg !== undefined &&
            sinceArg !== '' &&
        ...
        if (crossModel) {
            ...
            writeStderrLine(`Incremental anchor not used — it was certified by ` +
            ...
            try {
                anchor = resolveIncrementalAnchor(sinceArg, fetchedSha, {
        ...
        if (anchor?.diffBase) {
            ...
            const deltaBytes = anchor.diffBase === mergeBaseSha
                ? fullBytes
            ...
            else if ((selection = selectNarrowing(fullBytes, deltaBytes)) === null) {
            ...
            if (scopedDelta &&
                ...
                try {
                    ...
                    if (publish(fullBytes)) {
                        plan = rescued;
                        scopedDelta = false;
                        writeStderrLine('Retried the partition over the full range, which tiled; the ' +
        ...
        if (anchor) {
            ...
            writeStderrLine(inc.upToDate
                ...
                : inc.effective
                    ...
                    : `Incremental anchor ${inc.since.slice(0, 10)} refused (${inc.reason}); ${diffPath !== null
        ...
        if (resumeRefusal === 'head-moved') {
            ...
            recordRestart(out, `head-moved ${priorFetchedSha?.slice(0, 7) ?? 'unknown'}->${fetchedSha.slice(0, 7)}`);
        }
        ...
        // original cause.
        if (holder === null) {
            tryRemove(() => clearReviewWorktreeLeaseIfOwned(process.cwd(), leaseTarget, {
                sessionId,
                promptId,
            }));
        }
 ...
 * not content.
 */
export function computeDiffStats(diffText) {
    let additions = 0;
    let deletions = 0;
    let changedFiles = 0;
    let inHunk = false;
...
export const fetchPrCommand = {
    ...
    builder: (yargs) => yargs
        ...
        .option('host', {
        type: 'string',
        describe: "The host the target lives on — it selects the platform, i.e. whether the fetch uses pull/<n>/head or refs/merge-requests/<id>/head. An Aone host (*.alibaba-inc.com) selects the Aone backend; omitted: detected from the remote under review, else the clone's origin, else GitHub.",
    })
        ...
        .option('since', {
        type: 'string',
        describe: 'Incremental anchor: the head sha the last clean review round ' +
        ...
        .option('since-model', {
        type: 'string',
        describe: 'WHO certified the --since anchor: the `lastModelId` beside it in ' +
