 ...
 */
export async function readFirstLineNoFollow(filePath) {
    let fh;
    try {
        ...
        fh = await fsPromises.open(filePath, (fs.constants?.O_RDONLY ?? 0) |
...
export const getCachedGitBranch = (cwd) => {
    if (gitBranchCache.has(cwd))
        return gitBranchCache.get(cwd);
 ...
 */
export function getRecentGitStatus(cwd) {
    ...
    try {
        ...
        const statusWithBranch = execSync('git --no-optional-locks status --short --branch', {
            cwd,
            encoding: 'utf8',
        ...
        const branchName = branchDescription
            ...
            .split('...')[0];
        const branch = branchDescription === 'HEAD (no branch)' || !branchName
            ? DETACHED_HEAD_LABEL
            : branchName;
        ...
        debugLogger.warn('Failed to get recent git status for system prompt:', error);
        return null;
    }
}
