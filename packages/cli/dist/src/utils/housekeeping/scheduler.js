...
import { Storage, createDebugLogger, getSubagentsRootDir, resolveOpenAILogDir, } from '@lailatul-coder/lailatul-coder-core';
import { DEFAULT_OPENAI_LOG_RETENTION_DAYS } from '../../config/settingsSchema.js';
import { cleanupOldFileHistoryBackups, cleanupOldOpenAILogs, cleanupOldSubagentTranscripts, getCutoffDate, } from './cleanup.js';
...
// LailatulCoder for a while or every session has been < 10 min — either way we have
...
let started = false;
const nonInteractiveJobs = new Map();
const nonInteractiveQueue = [];
let activeNonInteractiveJob;
let activeNonInteractiveAbortController;
let nonInteractiveWorker;
let nonInteractiveStopping = false;
let nonInteractiveStopPromise;
export function startBackgroundHousekeeping(config, settings) {
    if (started)
        return;
    started = true;
    void scheduleFirstPass(config, settings).catch((err) => {
        ...
        started = false;
        debugLogger.error('scheduleFirstPass failed; chain will retry on next start', err);
    });
}
...
async function getFirstPassDelay(config, settings) {
...
function getSubagentMarkerPath(qwenDir, projectDir) {
...
function getOpenAILogsMarkerPath(qwenDir, logDir) {
...
function getOpenAILogCleanupTarget(config, settings) {
    try {
        const customLogDir = config.getContentGeneratorConfig?.()?.openAILoggingDir ??
            config.getModelsConfig?.()?.getGenerationConfig?.().openAILoggingDir ??
            settings.merged.model?.openAILoggingDir;
        const systemRetention = settings.system?.settings.model?.openAILogRetentionDays;
        ...
        if (customLogDir &&
            ...
            debugLogger.error('workspace-scoped openAILogRetentionDays is unsafe with a custom openAILoggingDir; skipping cleanup');
            return undefined;
        }
        ...
        debugLogger.error('failed to resolve OpenAI log cleanup target; skipping', err);
        return undefined;
    }
}
export function startNonInteractiveOpenAILogHousekeeping(config, settings) {
    if (nonInteractiveStopping)
        return;
    try {
        const target = getOpenAILogCleanupTarget(config, settings);
        if (!target || nonInteractiveJobs.has(target.logDir))
            return;
        const markerPath = getOpenAILogsMarkerPath(Storage.getGlobalQwenDir(), target.logDir);
        const job = {
            target,
            markerPath,
            queued: false,
        ...
        debugLogger.error('failed to start non-interactive OpenAI log cleanup; skipping', err);
    }
}
export function stopNonInteractiveOpenAILogHousekeeping() {
    if (nonInteractiveStopPromise)
...
function enqueueNonInteractiveJob(job) {
...
function startNonInteractiveWorker() {
    if (nonInteractiveWorker || nonInteractiveStopping)
        return;
...
async function drainNonInteractiveQueue() {
    while (!nonInteractiveStopping) {
        ...
        try {
            const result = await runOpenAILogCleanup(job.target, job.markerPath, abortController.signal);
            if (nonInteractiveStopping)
                continue;
            switch (result.status) {
                case 'completed':
                    ...
                    break;
                case 'fresh':
                    scheduleNonInteractiveJob(job, Math.min(RECURRING_INTERVAL_MS, Math.max(NON_INTERACTIVE_LOCK_RETRY_MS, result.retryAfterMs)));
                    break;
                case 'locked':
            ...
            debugLogger.error(`non-interactive OpenAI log cleanup failed for ${job.target.logDir}`, err);
            if (!nonInteractiveStopping) {
...
async function runOpenAILogCleanup(target, markerPath, signal) {
    return runThrottledOnce({
        ...
        debugLogger.debug(`openai-logs: removed=${r.removed} errors=${r.errors} completed=${r.completed}`);
        return r.completed ? undefined : false;
    });
}
function scheduleNonInteractiveJob(job, delayMs) {
    if (nonInteractiveStopping)
        return;
...
async function waitForNonInteractiveWorkerToStop() {
...
async function runPass(config, settings) {
    if (msSinceLastInteraction() < RECENT_INTERACTION_MS) {
        ...
        setTimeout(() => scheduleNextPass(config, settings), STARTUP_DELAY_MS).unref();
        return;
    }
    ...
    setTimeout(() => scheduleNextPass(config, settings), RECURRING_INTERVAL_MS).unref();
}
...
async function runHousekeeping(config, settings) {
    ...
    await runThrottledOnce({
        ...
        debugLogger.debug(`file-history: removed=${r.removed} errors=${r.errors}`);
    });
    ...
    const openaiTarget = getOpenAILogCleanupTarget(config, settings);
    if (openaiTarget) {
        await runOpenAILogCleanup(openaiTarget, getOpenAILogsMarkerPath(qwenDir, openaiTarget.logDir));
    }
}
...
export async function _resetNonInteractiveForTesting() {
    nonInteractiveStopping = true;
    for (const job of nonInteractiveJobs.values()) {
        if (job.timer)
            clearTimeout(job.timer);
