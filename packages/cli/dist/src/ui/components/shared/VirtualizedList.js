 ...
 */
import { useState, useRef, useLayoutEffect, forwardRef, useImperativeHandle, useMemo, useCallback, memo, } from 'react';
...
function firstIndexOfOffsetRun(offsets, index) {
    let i = index;
...
const VirtualizedListItem = memo(({ content, shouldBeStatic, width, containerWidth, itemKey, onHeightChange, }) => {
    const itemRef = useRef(null);
    const { height, hasMeasured } = useBoxMetrics(itemRef);
    ...
    useLayoutEffect(() => {
        ...
        const measuredHeight = itemRef.current?.yogaNode?.getComputedHeight() ?? height;
        if (hasMeasured) {
            onHeightChangeRef.current(itemKey, measuredHeight);
        }
    }, [itemKey, height, hasMeasured, content]);
    return (_jsx(Box, { width: "100%", flexDirection: "column", flexShrink: 0, ref: itemRef, children: shouldBeStatic ? (_jsx(StaticRender, { width: typeof width === 'number' ? width : containerWidth, children: content }, itemKey +
            '-static-' +
            (typeof width === 'number' ? width : containerWidth))) : (content) }));
});
VirtualizedListItem.displayName = 'VirtualizedListItem';
function VirtualizedList(props, ref) {
    const { data, renderItem, estimatedItemHeight, keyExtractor, initialScrollIndex, initialScrollOffsetInIndex, renderStatic, isStaticItem, width, } = props;
    ...
    const { width: measuredContainerWidth, height: measuredContainerHeight } = useBoxMetrics(containerRef);
    ...
    const [fullHeightMeasurementPending, setFullHeightMeasurementPending] = useState(measureAtFullHeight);
    const [previousMeasurementInputs, setPreviousMeasurementInputs] = useState({
        enabled: measureAtFullHeight,
        data,
        renderItem,
    });
    if (previousMeasurementInputs.enabled !== measureAtFullHeight ||
        (measureAtFullHeight &&
    ...
    useLayoutEffect(() => {
        setHeights((prev) => {
            ...
            if (prevKeys.length <= Math.max(8, 2 * data.length))
                return prev;
    ...
    if (target !== undefined && offsets.length > 1) {
        ...
        if (targetChanged || offsetsJustBecameUsable) {
            ...
            const anchoredIndex = firstIndexOfOffsetRun(offsets, Math.max(0, Math.min(data.length - 1, target)));
    ...
    useLayoutEffect(() => {
        ...
        if (shouldAutoScroll &&
            ...
            if (scrollAnchor.index !== newIndex ||
            ...
            if (scrollAnchor.index !== newAnchor.index ||
    ...
    const clampedScrollTop = Math.min(Math.max(0, isStickingToBottom ? maxScroll : actualScrollTop), maxScroll);
    ...
    const startIndex = firstIndexOfOffsetRun(offsets, Math.max(0, findLastLE(offsets, clampedScrollTop) - 1));
    const viewHeightForEndIndex = scrollableContainerHeight > 0 ? scrollableContainerHeight : 50;
    const endIndexOffsetRaw = upperBound(offsets, clampedScrollTop + viewHeightForEndIndex);
    const endIndex = endIndexOffsetRaw >= offsets.length
        ? data.length - 1
        : Math.min(data.length - 1, endIndexOffsetRaw);
    const topSpacerHeight = renderStatic === true ? 0 : (offsets[startIndex] ?? 0);
    const bottomSpacerHeight = renderStatic
        ? 0
        : totalHeight - (offsets[endIndex + 1] ?? totalHeight);
    const isReady = containerHeight > 0 ||
    ...
    if (!isReady) {
        debugLogger.debug(`viewport not ready (containerHeight=${containerHeight}, width=${String(width)}); rendering empty until measurement settles`);
    }
    ...
    const renderedItems = useMemo(() => {
        ...
        for (let i = renderRangeStart; i <= renderRangeEnd; i++) {
            ...
            if (item) {
                ...
                const shouldBeStatic = (renderStatic === true && isOutsideViewport) ||
                ...
                try {
                    ...
                    content = (_jsx(Box, { flexDirection: "column", flexShrink: 0, children: _jsx(Text, { color: "red", children: "[render error]" }) }));
                }
                const key = keyExtractor(item, i);
                items.push(_jsx(VirtualizedListItem, { itemKey: key, content: content, shouldBeStatic: shouldBeStatic, width: width, containerWidth: containerWidth, onHeightChange: onHeightChange }, key));
            }
        }
        return items;
    }, [
    ...
    const hitTestScrollbar = useCallback(({ col, row }) => {
        const geometry = getScrollbarGeometry();
        if (!geometry)
            return false;
        const zeroBasedCol = col - 1;
        const zeroBasedRow = row - 1;
        return (zeroBasedCol === geometry.col &&
            zeroBasedRow >= geometry.top &&
            zeroBasedRow < geometry.top + geometry.height);
    }, [getScrollbarGeometry]);
    const scrollToScrollbarRow = useCallback((row) => {
        const geometry = getScrollbarGeometry();
        if (!geometry)
            return;
        const zeroBasedRow = row - 1;
        const rowInTrack = Math.max(0, Math.min(geometry.height - 1, zeroBasedRow - geometry.top));
    ...
    useImperativeHandle(ref, () => ({
        ...
        getViewportRect: () => rootRef.current ? measureElementPosition(rootRef.current) : null,
    }), [
        offsets,
        scrollAnchor,
        totalHeight,
    ...
    const { isVisible: scrollbarThumbActive, flashScrollbar } = useAnimatedScrollbar();
    ...
    const scrollbarContent = useMemo(() => {
        if (!showScrollbar || scrollableContainerHeight <= 0)
            return null;
        const trackLen = scrollableContainerHeight;
        const thumbLen = Math.max(1, Math.round((trackLen * trackLen) / totalHeight));
        const thumbTop = Math.round((clampedScrollTop / maxScroll) * (trackLen - thumbLen));
        return (_jsx(Box, { width: 1, flexDirection: "column", flexShrink: 0, children: Array.from({ length: trackLen }, (_, i) => {
                ...
                if (!scrollbarThumbActive) {
                    return (_jsx(Text, { selectable: false, children: ' ' }, i));
                }
                return (_jsx(Text, { dimColor: !inThumb, selectable: false, children: inThumb ? '█' : '│' }, i));
            }) }));
    }, [
        showScrollbar,
        scrollableContainerHeight,
    ...
    const rootHeight = props.containerHeight !== undefined
        ? fullHeightMeasurementPending
            ? props.containerHeight
            : Math.min(props.containerHeight, totalHeight)
        : '100%';
    return (_jsxs(Box, { ref: rootRef, width: "100%", height: rootHeight, flexDirection: "row", children: [_jsx(Box, { ref: containerRef, overflowY: "hidden", overflowX: "hidden", flexGrow: 1, flexDirection: "column", children: _jsxs(Box, { flexShrink: 0, width: "100%", flexDirection: "column", marginTop: -clampedScrollTop, children: [_jsx(Box, { height: topSpacerHeight, flexShrink: 0 }), renderedItems, _jsx(Box, { height: bottomSpacerHeight, flexShrink: 0 })] }) }), scrollbarContent] }));
}
