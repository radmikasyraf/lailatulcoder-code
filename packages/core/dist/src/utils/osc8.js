 ...
 */
export function wrapForMultiplexer(sequence) {
    if (process.env['TMUX']) {
 ...
 */
export function sanitizeForOsc(s) {
    return s.replace(
 ...
 */
export function osc8Hyperlink(url, label = url) {
...
function shouldForceHyperlinks(value) {
    if (value.length === 0)
        return true;
...
export function supportsHyperlinks(stream = process.stdout) {
    ...
    if (env['QWEN_DISABLE_HYPERLINKS'] === '1')
        return false;
    ...
    if (env['VTE_VERSION']) {
        ...
        if (version.major > 0 || version.minor >= 50)
            return true;
        return false;
    }
