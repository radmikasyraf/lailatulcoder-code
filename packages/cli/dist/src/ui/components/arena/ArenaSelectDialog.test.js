...
import { AgentStatus, ArenaSessionStatus, } from '@lailatul-coder/lailatul-coder-core';
...
describe('ArenaSelectDialog', () => {
    it('toggles quick preview and detailed diff for the highlighted agent', async () => {
        ...
        const { lastFrame, stdin } = renderWithProviders(_jsx(ArenaSelectDialog, { manager: manager, config: config, addItem: vi.fn(), closeArenaDialog: vi.fn() }));
        stdin.write('p');
        await waitFor(() => {
    ...
    it('closes without applying or cleaning up when Escape is pressed', async () => {
        const { manager, config, closeArenaDialog, applyAgentResult } = createDialogHarness();
        const cleanupArenaRuntime = config.cleanupArenaRuntime;
        const { stdin } = renderWithProviders(_jsx(ArenaSelectDialog, { manager: manager, config: config, addItem: vi.fn(), closeArenaDialog: closeArenaDialog }));
        stdin.write('\x1B');
        await waitFor(() => {
    ...
    it('ignores Enter when the highlighted agent is not selectable', async () => {
        ...
        const { manager, config, closeArenaDialog, applyAgentResult } = createDialogHarness([failedAgent]);
        const cleanupArenaRuntime = config.cleanupArenaRuntime;
        const { stdin } = renderWithProviders(_jsx(ArenaSelectDialog, { manager: manager, config: config, addItem: vi.fn(), closeArenaDialog: closeArenaDialog }));
        stdin.write('\r');
...
function createDialogHarness(agents = [createAgentResult()]) {
    ...
    const manager = {
        ...
        getAgentStates: vi.fn(() => agents.map((agent) => ({
            agentId: agent.agentId,
            model: agent.model,
            status: agent.status,
            stats: agent.stats,
        }))),
        getAgentState: vi.fn((agentId) => agents.find((agent) => agent.agentId === agentId)),
        applyAgentResult,
    };
    const config = {
...
function createAgentResult({ agentId = 'model-1', status = AgentStatus.IDLE, } = {}) {
    return {
        agentId,
...
//# sourceMappingURL=ArenaSelectDialog.test.js.map