 ...
 */
import { ApprovalMode, OutputFormat, ToolErrorType, createDebugLogger, getArenaSystemReminder, getMCPServerStatus, getPlanModeSystemReminder, isShellProgressData, } from '@lailatul-coder/lailatul-coder-core';
...
export function buildInitialSystemReminders(config) {
    const reminders = [];
...
export function insertAfterFunctionResponses(parts, additions) {
    const firstNonFunctionResponse = parts.findIndex((part) => !part.functionResponse);
...
async function loadSlashCommandNames(config) {
    ...
    try {
        const commands = await getAvailableCommands(config, controller.signal, 'non_interactive');
        // Extract command names and sort
        return commands.map((cmd) => cmd.name).sort();
    }
    catch (error) {
        debugLogger.error('[buildSystemMessage] Failed to load slash commands:', error);
        return [];
    }
    finally {
        controller.abort();
 ...
 */
export function createToolProgressHandler(request, adapter) {
    const handler = (_callId, output) => {
 ...
 * @returns An object containing the output update handler
 */
export function createAgentToolProgressHandler(config, agentToolCallId, adapter) {
    ...
    const outputUpdateHandler = (callId, outputChunk) => {
        ...
        if (typeof outputChunk === 'object' &&
            ...
            if (!previous &&
                ...
                adapter.emitUserMessage([{ text: taskDisplay.taskPrompt }], agentToolCallId);
            }
