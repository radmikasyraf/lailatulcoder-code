...
import { ApprovalMode, AuthType, GoalPersistenceUnavailableError, Storage, } from '@lailatul-coder/lailatul-coder-core';
...
function makePromptRequest(text = 'hello') {
    return {
...
describe('Session.pendingWorktreeNotice', () => {
    ...
    beforeEach(() => {
        ...
        mockChat = {
            sendMessageStream: vi
                .fn()
                .mockImplementation(async (_model, args, _promptId) => {
        ...
        mockConfig = {
            ...
            getActiveTodoWorkChainOwner: vi.fn((promptId) => promptId),
    ...
    it('VP3: first prompt prepends pendingWorktreeNotice as a <system-reminder> block', async () => {
        const session = new Session(SESSION_ID, mockConfig, mockClient, mockSettings);
        const notice = '[Resumed] Active worktree: "feat" at /repo/.qwen/worktrees/feat ' +
        ...
        const firstParts = capturedMessages[0];
        const reminderPart = firstParts.find((p) => typeof p.text === 'string' &&
    ...
    it('VP4: second prompt does not re-inject pendingWorktreeNotice', async () => {
        ...
        const secondParts = capturedMessages[1];
        const reminderPart = secondParts.find((p) => typeof p.text === 'string' &&
            p.text.includes('<system-reminder>') &&
            p.text.includes(notice));
    ...
    it('injects a recovered-agents notice into the next prompt once', async () => {
        const session = new Session(SESSION_ID, mockConfig, mockClient, mockSettings);
        const notice = '2 background agents were restored. Use list_agents to inspect them.';
    ...
    it('does not consume a recovered-agents notice on an interrupted-turn continuation', async () => {
        ...
        const continuationParts = capturedMessages[0];
        expect(continuationParts.some((part) => part.text?.includes(notice))).toBe(false);
        expect(session.pendingRecoveredAgentsNotice).toBe(notice);
        // The next ordinary prompt consumes it exactly once.
        vi.mocked(mockChat.getHistory).mockReturnValue([]);
        await session.prompt(makePromptRequest('ordinary prompt'));
        const ordinaryParts = capturedMessages[1];
        expect(ordinaryParts.some((part) => part.text?.includes(notice))).toBe(true);
        expect(session.pendingRecoveredAgentsNotice).toBeNull();
    });
    ...
    it('VP5: continuation keeps functionResponses first, then worktree notice, then reminders', async () => {
        ...
        expect(capturedMessages.length).toBeGreaterThanOrEqual(1);
        const parts = capturedMessages[0];
        // tool_result blocks must stay first (Anthropic-compatible ordering).
        expect(parts[0].functionResponse).toBeDefined();
        const noticeIdx = parts.findIndex((p) => typeof p.text === 'string' && p.text.includes(notice));
        const reminderIdx = parts.findIndex((p) => typeof p.text === 'string' &&
            p.text.includes('<system-reminder>') &&
            !p.text.includes(notice));
...
//# sourceMappingURL=Session.worktree.test.js.map