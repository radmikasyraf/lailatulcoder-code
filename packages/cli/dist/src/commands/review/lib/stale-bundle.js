...
// already stops the review talking to a DIFFERENT program (a bare `LailatulCoder` on
 ...
 */
export const DIGESTED_EXTENSIONS = {
    code: new Set([
        '.ts',
        '.tsx',
        '.mts',
...
function isDigestedFile(kind, name) {
    if (!DIGESTED_EXTENSIONS[kind].has(extname(name)))
        return false;
    if (kind !== 'code')
        return !NOT_BUNDLED_SKILL_FILE.has(name);
 ...
 */
export function reviewSourcesDigest(repoRoot, roots) {
 ...
 */
function measureReviewSources(repoRoot, roots) {
    const state = { incomplete: false, absentRoots: 0 };
    const files = [];
    for (const root of roots)
        files.push(...sourceFilesUnder(root, state, false));
 ...
 */
export function bundleStaleness(stampedDigest, currentDigest) {
    if (!stampedDigest) {
 ...
 */
function* sourceFilesUnder(root, state, listed) {
    const { kind } = root;
    let entries;
    try {
 ...
 */
export function reviewSourceRoots(repoRoot) {
    ...
    return [
        ...
        {
            path: join(repoRoot, 'packages', 'cli', 'src', 'services', 'review-worktree-lease.ts'),
            kind: 'code',
        },
        ...
        {
            path: join(repoRoot, 'packages', 'core', 'src', 'skills', 'bundled', 'review'),
            kind: 'skill',
        },
    ];
}
 ...
 */
export function bundleStalenessNotices(entryPath, brief) {
    ...
    try {
        ...
        // unresolved, so an alias of the bundle (`ln -s dist/cli.js ~/bin/LailatulCoder`)
        ...
        if (stamped && current && measured.partial) {
            ...
            return (`review: could not check whether the bundle is current — ` +
        ...
        if (!stamped && current) {
            return (`review: could not check whether the bundle is current — ${staleness.unmeasured}. ` +
                (brief
 ...
 */
export function staleBundleWarning(s, brief) {
    if (!s.stale)
        return undefined;
    if (brief) {
        return (`review: the bundle these commands run from was NOT built from the review sources in this tree — ` +
            `rebuild with \`npm run bundle\` and start again, or read every result ` +
            `below as being about the older build.`);
    }
    return (`review: the bundle these commands run from was NOT built from the review sources in this tree. ` +
        `Every \`LailatulCoder review …\` step below runs the BUILT bundle, not the working tree, ` +
...
//# sourceMappingURL=stale-bundle.js.map