 ...
 */
import { QWEN_CODE_DESKTOP_ENV, QWEN_CODE_SERVE_ENV, } from './acp-channel-fallback.js';
...
export const ENV_ACP_REPEATED_TOOL_FAILURE_GUARD = 'QWEN_CODE_ACP_REPEATED_TOOL_FAILURE_GUARD';
...
export const PROJECT_ENV_HARDCODED_EXCLUSIONS = [
    ...
    // `cd <untrusted repo> && LailatulCoder serve` into code execution as the daemon
    ...
    // DEV gates the daemon's inherited-loader-env scrub (run-LailatulCoder-serve.ts);
...
const HARDCODED_PROJECT_ENV_EXCLUSIONS = new Set(PROJECT_ENV_HARDCODED_EXCLUSIONS.map((key) => key.toLowerCase()));
...
export function isHardcodedProjectEnvExclusion(key) {
    const lowerKey = key.toLowerCase();
    return (HARDCODED_PROJECT_ENV_EXCLUSIONS.has(lowerKey) ||
        HARDCODED_PROJECT_ENV_EXCLUSION_PATTERNS.some((pattern) => pattern.test(lowerKey)));
}
...
const canonicalLoaderKey = (key) => key.toLowerCase().replace(/_/gu, '-');
const LOADER_ENV_KEYS = new Set(INHERITED_LOADER_ENV_KEYS.map(canonicalLoaderKey));
...
export function isLoaderEnvKey(key) {
    const canonical = canonicalLoaderKey(key);
    return canonical.startsWith('bash-func-') || LOADER_ENV_KEYS.has(canonical);
}
export function scrubInheritedLoaderEnv(env, snapshotInto) {
    const removedKeys = [];
    for (const key of Object.keys(env)) {
        if (isLoaderEnvKey(key)) {
            const value = env[key];
            if (snapshotInto && value !== undefined)
                snapshotInto.set(key, value);
            delete env[key];
...
export function scrubAndReportInheritedLoaderEnv(env, commandLabel, processLabel, snapshotInto) {
    const removedKeys = scrubInheritedLoaderEnv(env, snapshotInto);
    if (removedKeys.length > 0) {
        writeStderrLineSafe(`${commandLabel}: scrubbed inherited loader env vars from the ` +
...
const sharedProcessEnvScrubOriginals = new Map();
...
// correct.
export function acquireInheritedLoaderEnvScrub(env, commandLabel, processLabel) {
    ...
    const removedKeys = scrubAndReportInheritedLoaderEnv(env, commandLabel, processLabel, sharedProcessEnvScrubOriginals);
    let released = false;
    return {
        removedKeys,
        release() {
...
export function resetInheritedLoaderEnvScrubForTesting() {
...
const reportedLoaderKeyRejections = new Map();
let loaderKeyRejectionReporter;
export function setLoaderKeyRejectionReporter(reporter) {
...
export function clearLoaderKeyRejectionReporterIfCurrent(reporter) {
...
export function reportRejectedLoaderKeys(source, candidateKeys) {
    const rejectedKeys = candidateKeys.filter(isLoaderEnvKey);
    const warnedKeys = reportedLoaderKeyRejections.get(source) ?? new Set();
    ...
    if (loaderKeyRejectionReporter) {
        ...
        writeStderrLineSafe(`LailatulCoder: ${source} cannot set loader-affecting env vars; ignored: ` +
            freshKeys.join(', '));
    }
    return rejectedKeys;
}
/** Test-only: forget already-reported loader-key rejections. */
export function resetLoaderKeyRejectionReportingForTesting() {
    reportedLoaderKeyRejections.clear();
}
//# sourceMappingURL=shared-env-keys.js.map