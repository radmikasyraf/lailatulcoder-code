 ...
 */
const TRANSIENT_HTTP_STATUS_PATTERNS = [
...
export function isTransientNetworkError(error) {
    ...
    for (const transientCode of TRANSIENT_ERROR_CODES) {
        if (message.includes(transientCode))
            return true;
    }
 ...
 */
export async function retryWithBackoff(fn, label, opts) {
    ...
    for (let attempt = 0; attempt <= maxRetries; attempt++) {
        try {
            ...
            debugLogger.info(`Transient error on '${label}' (attempt ${attempt + 1}/${maxRetries + 1}), ` +
