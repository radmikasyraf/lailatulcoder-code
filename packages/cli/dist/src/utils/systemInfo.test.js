...
import { getSystemInfo, getExtendedSystemInfo, getNpmVersion, getSandboxEnv, getIdeClientName, } from './systemInfo.js';
import { createMockCommandContext } from '../test-utils/mockCommandContext.js';
import os from 'node:os';
import { IdeClient } from '@lailatul-coder/lailatul-coder-core';
...
describe('systemInfo', () => {
    ...
    describe('getIdeClientName', () => {
        ...
        it('should return empty string when IDE client detection fails', async () => {
            vi.mocked(IdeClient.getInstance).mockRejectedValue(new Error('IDE client error'));
            const ideClient = await getIdeClientName(mockContext);
            expect(ideClient).toBe('');
        });
    });
    ...
    describe('getExtendedSystemInfo', () => {
        it('should include memory usage and base URL', async () => {
            ...
            setExecFileStdout('10.0.0');
            const { AuthType } = await import('@lailatul-coder/lailatul-coder-core');
            // Update the mock context to use OpenAI auth
            mockContext.services.settings.merged.security.auth.selectedType =
                AuthType.USE_OPENAI;
            vi.mocked(mockContext.services.config.getAuthType).mockReturnValue(AuthType.USE_OPENAI);
        ...
        ])('should format LSP status when %s', async (_name, snapshot, expected) => {
            vi.mocked(IdeClient.getInstance).mockResolvedValue({
                getDetectedIdeDisplayName: vi.fn().mockReturnValue(''),
            });
            mockContext.services.config = {
...
//# sourceMappingURL=systemInfo.test.js.map