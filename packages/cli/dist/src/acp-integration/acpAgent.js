 ...
 */
import { APPROVAL_MODE_INFO, APPROVAL_MODES, AuthType, BTW_MAX_INPUT_LENGTH, buildBtwCacheSafeParams, buildBtwPrompt, ALL_PROVIDERS, applyProviderInstallPlan, buildInstallPlan, clearCachedCredentialFile, createDebugLogger, generateSessionRecap, findProviderById, getAllGeminiMdFilenames, getAutoMemoryRoot, getUserAutoMemoryRoot, getDefaultBaseUrlForProtocol, getDefaultModelIds, getScopedEnvContents, QwenOAuth2Event, qwenOAuth2Events, resolveBaseUrl, MCP_BUDGET_WARN_FRACTION, MCPServerConfig, runForkedAgent, SessionIdCaseConflictError, SessionService, SESSION_WRITER_RPC_CODES, SessionWriterUnavailableError, SESSION_TITLE_MAX_LENGTH, Storage, tokenLimit, getMCPDiscoveryState, getMCPServerStatus, initializeTelemetry, preloadContentGenerator, MCPDiscoveryState, MCPServerStatus, McpTransportPool, POOLED_TRANSPORTS_DEFAULT, INVOCATION_CONTEXT_META_KEY, PRIVATE_ACP_CAPABILITY_ENV, PRIVATE_PARENT_CAPABILITY_META_KEY, parseInvocationContext, findExistingProviderModels, ExtensionManager, ExtensionSettingScope, HookEventName, updateSetting, SessionEndReason, WorkspaceMcpBudget, DiscoveredMCPTool, restoreWorktreeContext, uiTelemetryService, McpBudgetWouldExceedError, McpServerSpawnFailedError, InvalidMcpConfigError, isGatedMcpScope, MCPOAuthProvider, MCPOAuthTokenStorage, InvalidSessionTranscriptCursorError, SESSION_TRANSCRIPT_MAX_LIMIT, SESSION_TRANSCRIPT_MAX_PAGE_BYTES, SessionTranscriptReader, SessionTranscriptPageTooLargeError, SessionTranscriptSnapshotUnavailableError, SessionTranscriptTooLargeError, encodeSessionTranscriptCursor, isTurnResultRecordPayload, subagentGenerator, redactUrlCredentials, computeUniqueBranchTitle, BranchPointInvalidError, parseGoalSnapshotV2, parseGoalStateCause, ToolNames, FORK_SUBAGENT_TYPE, runManagedAutoMemoryDream, runManagedRememberByAgent, matchesAnyServerPattern, mcpServerRequiresOAuth, IMAGE_CAPABILITY, registerAcpEventLoopLagGauge, SESSION_ARTIFACT_PERSISTENCE_VERSION, normalizeEventPayload, normalizeSnapshotPayload, startEventLoopLagMonitor, refreshMemoryInstruction, applyReasoningEffort, REASONING_EFFORT_TIERS, addDaemonRequestAttribute, extractDaemonTraceContext, withDaemonSpan, emptyGoalSnapshot, GoalConflictError, GoalInvalidTransitionError, GoalPersistenceUnavailableError, parseGoalControlRequest, ApprovalMode, } from '@lailatul-coder/lailatul-coder-core';
import { randomUUID, timingSafeEqual } from 'node:crypto';
import { performance } from 'node:perf_hooks';
import { AgentSideConnection, RequestError, PROTOCOL_VERSION, } from '@agentclientprotocol/sdk';
...
import { ACP_EVENT_LOOP_STALL_RESTART_MS, CHANNEL_PROMPT_META_KEY, } from '@lailatul-coder/channel-base';
...
import { loadSettings, reloadEnvironment, SettingScope, } from '../config/settings.js';
import { loadSettingsCached } from '../config/settings-cache.js';
import { normalizeSessionIdForLookup, parseCallerSuppliedSessionId, } from '../config/session-id.js';
import { loadMcpApprovals } from '../config/mcpApprovals.js';
import { assembleMcpServers } from '../config/mcpServers.js';
import { recomputeMcpGating } from '../config/hot-reload.js';
import { REDACTED_MCP_SECRET, redactMcpServerSecrets, restoreRedactedMcpSecrets, } from '../config/mcp-server-secrets.js';
import { buildPermissionSettings, normalizePermissionRules, PermissionRulesValidationError, PERMISSION_RULE_TYPES, readPermissionRuleSet, } from '../config/permission-settings.js';
import { createLoadedSettingsAdapter } from '../config/loadedSettingsAdapter.js';
import { isCompatibleLiveSessionSource } from '../serve/conversations/session-source.js';
import { z } from 'zod';
import { buildDisabledSkillNamesProvider, loadCliConfig, SessionIdConflictError, } from '../config/config.js';
...
import { shouldProbeChildHeap, startChildHeapProbe, } from './child-heap-probe.js';
import { getModelConfiguration, } from './model-configuration.js';
import { buildSessionTasksStatus } from './session/tasksSnapshot.js';
import { collectHistoryReplayUpdates, copyCumulativeUsage, createReplayCumulativeUsage, HistoryReplayLimitError, replayTranscriptRecordPage, } from './session/history-replay-page.js';
import { buildAcpModelOptions, getCurrentAcpModelId, parseAcpBaseModelId, sanitizeProviderBaseUrl, } from '../utils/acpModelUtils.js';
import { updateOutputLanguageFile, resolveOutputLanguageOrPreserveAuto, getOutputLanguageFilePath, writeOutputLanguageAndRegisterPath, } from '../utils/languageUtils.js';
...
import { setLanguageAsync, getCurrentLanguage, SUPPORTED_LANGUAGES, } from '../i18n/index.js';
import { isWorkspaceTrusted, isFolderTrustEnabled, loadTrustedFolders, } from '../config/trustedFolders.js';
import { ACP_PREFLIGHT_KINDS, STATUS_SCHEMA_VERSION, SERVE_CONTROL_EXT_METHODS, SERVE_STATUS_EXT_METHODS, mapDomainErrorToErrorKind, IDLE_HOOK_EVENTS, } from '@lailatul-coder/acp-bridge/status';
import { EXTERNAL_TOOL_GUARD_READY_META_KEY, EXTERNAL_TOOL_GUARD_REQUIRED_VALUE, EXTERNAL_TOOL_GUARD_TOKEN_ENV, isValidExternalToolGuardDenialReason, PRIVATE_EXTERNAL_TOOL_GUARD_ENV, PRIVATE_EXTERNAL_TOOL_GUARD_PROVIDER_ENV, SHELL_EXECUTING_TOOL_NAMES, } from '@lailatul-coder/acp-bridge/externalToolGuard';
import { parseSessionSource, SESSION_SOURCE_META_KEY, } from '@lailatul-coder/acp-bridge';
import { ACTIVE_WORK_CLOSE_IF_UNHELD_PARAM, ACTIVE_WORK_HEARTBEAT_META_KEY, ACTIVE_WORK_HEARTBEAT_VERSION, ACTIVE_WORK_HOLD_CATEGORIES, ACTIVE_WORK_LEGACY_HOLD_CATEGORIES, clampActiveWorkIntervalMs, CHANNEL_STARTUP_PROFILE_META_KEY, CHANNEL_STARTUP_PROFILE_VERSION, CLIENT_MCP_OVER_WS_CONFIG_FLAG, DAEMON_CHANNEL_DELIVERY_META_KEY, DAEMON_MODEL_PROMPT_META_KEY, DAEMON_PROMPT_DISPLAY_TEXT_META_KEY, LOAD_REPLAY_BULK_MODE, LOAD_REPLAY_HIDE_INHERITED_META_KEY, LOAD_REPLAY_MAX_BYTES, LOAD_REPLAY_MAX_UPDATES, LOAD_REPLAY_META_KEY, LOAD_REPLAY_MODE_META_KEY, LOAD_REPLAY_PAGE_SIZE_META_KEY, LOAD_REPLAY_VERSION, PROMPT_CANCEL_METHOD, REQUESTED_SESSION_ID_META_KEY, TODO_STOP_GUARD_QUEUE_RELEASE_METHOD, isValidTrustedModelPrompt, WORKTREE_MCP_DEFER_META_KEY, } from '@lailatul-coder/acp-bridge/bridgeTypes';
import { beginAcpBootstrapConfigProfiling, buildAndFreezeAcpStartupProfile, endAcpBootstrapConfigProfiling, markAcpStartup, } from '../utils/acp-startup-profiler.js';
...
import { collectContextData, formatContextUsageText, } from '../ui/commands/contextCommand.js';
...
import { executeGeneration, GENERATION_MAX_PROMPT_BYTES, GENERATION_TIMEOUT_MS, } from './generation.js';
...
const ACP_REASONING_EFFORT_NAMES = {
    low: 'Low',
    medium: 'Medium',
    high: 'High',
    xhigh: 'Extra high',
...
function currentGoalSnapshot(config, runtime) {
    try {
...
function mapGoalControlError(error, config, runtime) {
    ...
    return new RequestError(-32603, error instanceof Error ? error.message : 'Goal persistence failed', {
        errorKind: 'goal_persist_failed',
        current: currentGoalSnapshot(config, runtime),
    });
}
async function dispatchGoalControl(config, request) {
    const requiresTrustedWorkspace = request.action === 'create' ||
...
async function findSettledTurnResult(reader, sessionId, promptId, workspaceCwd) {
    let cursor;
    for (let page = 0; page < TURN_STATUS_SCAN_MAX_PAGES; page++) {
        ...
        cursor = encodeSessionTranscriptCursor(result.nextCursorState, workspaceCwd);
    }
    return undefined;
}
function createAcpSessionProfiler(span, attributePrefix) {
    let failedStage;
    ...
    const recordStage = (stage, start) => {
        const durationMs = Math.round((performance.now() - start) * 100) / 100;
        if (Number.isFinite(durationMs) && durationMs >= 0) {
            setAttribute(`lailatul-coder.daemon.${attributePrefix}.${stage}_ms`, durationMs);
        }
    };
    const recordFailure = (stage) => {
...
function createAcpSessionRestoreProfiler(span) {
...
const SESSION_WRITER_MESSAGES = {
    session_writer_conflict: 'This session is already open in another LailatulCoder process.',
    session_writer_lost: 'Write ownership for this session was lost.',
    session_transcript_changed: 'The session transcript changed outside its active writer.',
    session_writer_unavailable: 'Session write ownership could not be verified.',
};
function getSessionWriterError(error) {
...
function mapSessionWriterRequestError(error) {
...
async function shutdownSessionConfig(config) {
    await config.shutdown({ shutdownTelemetry: false });
    if (config.hasSessionWriteOwnership()) {
        throw new SessionWriterUnavailableError();
    }
}
async function waitForSessionDrain(operation, timeoutMs, kind, displayTimeoutMs) {
    let timer;
    const timeout = new Promise((_, reject) => {
        timer = setTimeout(() => reject(new Error(`Session ${kind} timed out after ${displayTimeoutMs ?? timeoutMs}ms`)), timeoutMs);
        timer.unref();
    });
    try {
...
async function beginSessionCloseAfterCurrentGate(session, timeoutMs) {
    ...
    while (true) {
        ...
        await waitForSessionDrain(session.waitForCloseGateToRelease(), remainingMs, 'close');
    }
}
const logWorkspaceMemoryExtractionError = createWorkspaceMemoryExtractionErrorLogger(debugLogger);
function parseAcpLocalReadRootsEnv(raw = process.env[QWEN_ACP_LOCAL_READ_ROOTS_ENV]) {
    if (!raw)
        return [];
    return raw
...
function defaultAcpOnlyLocalReadRoots() {
    return process.platform === 'win32' ? [] : [POSIX_TMP_LOCAL_READ_ROOT];
}
function buildAcpLocalReadRoots(config) {
    return [
...
function parseSessionArtifactEventPayload(payload, expectedSessionId) {
    ...
    if (!normalized ||
        warnings.length > 0 ||
        normalized.changes.length !== record['changes'].length) {
        throw invalidArtifactPersistPayload();
    }
    return normalized;
}
function parseSessionArtifactSnapshotPayload(payload, expectedSessionId) {
    const record = parseSessionArtifactBasePayload(payload, expectedSessionId);
    const warnings = [];
    const normalized = normalizeSnapshotPayload(record, warnings);
    if (!normalized ||
        warnings.length > 0 ||
        normalized.artifacts.length !== record['artifacts'].length) {
        throw invalidArtifactPersistPayload();
    }
    return normalized;
}
function parseSessionArtifactBasePayload(payload, expectedSessionId) {
    if (!isObjectRecord(payload)) {
        throw invalidArtifactPersistPayload();
    }
    if (payload['v'] !== SESSION_ARTIFACT_PERSISTENCE_VERSION ||
...
function invalidArtifactPersistPayload() {
    return RequestError.invalidParams(undefined, 'Invalid or missing artifact persist payload');
}
function isBulkLoadReplayRequest(params) {
...
function loadRestoreOptions(params) {
...
const RESUME_RESTORE_OPTIONS = {
    replay: { kind: 'none' },
};
function mapSessionRestoreRequestError(error, sessionId) {
    const mappedWriterError = mapSessionWriterRequestError(error);
    if (mappedWriterError !== error)
        return mappedWriterError;
    ...
    if (error instanceof SessionTranscriptPageTooLargeError) {
        addDaemonRequestAttribute('lailatul-coder.daemon.session_restore.envelope_limit_reason', 'bytes');
    ...
    if (error instanceof HistoryReplayLimitError) {
        addDaemonRequestAttribute('lailatul-coder.daemon.session_restore.envelope_limit_reason', error.reason);
        return new RequestError(-32012, error.message, {
            errorKind: 'transcript_page_too_large',
            sessionId,
            reason: error.reason,
...
function validateLoadReplayEnvelope(sessionId, envelope, enforceLimits) {
    if (!enforceLimits)
        return;
    if (envelope.updates.length > LOAD_REPLAY_MAX_UPDATES) {
        throw new HistoryReplayLimitError(sessionId, 'updates', envelope.updates.length, LOAD_REPLAY_MAX_UPDATES);
    }
    const pageBytes = Buffer.byteLength(JSON.stringify(envelope), 'utf8');
    if (pageBytes > LOAD_REPLAY_MAX_BYTES) {
        throw new HistoryReplayLimitError(sessionId, 'bytes', pageBytes, LOAD_REPLAY_MAX_BYTES);
    }
}
function replayGoalBootstrap(projection) {
    if (projection && !('runtime' in projection)) {
        ...
        if (isObjectRecord(replay)) {
            const bootstrap = HistoryReplayer.v2GoalBootstrap(replay['goalState'], replay['goalCause']);
            if (bootstrap)
                return bootstrap;
        }
        const sourceRecord = projection.goalRecords?.find((record) => record.uuid === sourceUuid);
        if (sourceRecord?.subtype === 'goal_state') {
            const payload = isObjectRecord(sourceRecord.systemPayload)
                ? sourceRecord.systemPayload
                : undefined;
            return HistoryReplayer.v2GoalBootstrap(payload?.['snapshot'], payload?.['cause']);
        }
        const active = findGoalToRestore(collectGoalStatusItemsFromRecords(projection.goalRecords ?? []));
        return active
            ? {
                goalStatus: {
    ...
    if (!projection?.replay || !sourceUuid)
        return undefined;
    if (projection.replay.records.some((record) => record.uuid === sourceUuid)) {
        return undefined;
    }
    const goalBootstrapRecords = projection.replay.goalBootstrapRecords ?? [];
    const sourceRecord = goalBootstrapRecords.find((record) => record.uuid === sourceUuid);
    if (sourceRecord?.subtype === 'goal_state') {
        const payload = isObjectRecord(sourceRecord.systemPayload)
            ? sourceRecord.systemPayload
            : undefined;
    ...
    const active = findGoalToRestore(collectGoalStatusItemsFromRecords(goalBootstrapRecords));
    if (!active)
        return undefined;
    return {
        goalStatus: {
...
function replayInitialGoalState(projection) {
    const replay = projection?.replay?.replay;
    if (!isObjectRecord(replay))
        return {};
...
export function selectVisibleHistoryRecords(records, hideInheritedHistory) {
    const sourceBoundary = records.findIndex((record) => record.type === 'system' &&
...
function getLoadReplayPageSize(params) {
    ...
    if (!Number.isSafeInteger(value) ||
        value < 1 ||
        value > SESSION_TRANSCRIPT_MAX_LIMIT) {
        throw RequestError.invalidParams(undefined, `Invalid load replay page size; expected 1..${SESSION_TRANSCRIPT_MAX_LIMIT}`);
    }
    return value;
}
function deriveForkBaseName(name, recording, sessionId) {
...
function truncateForkDirectiveForHistory(directive) {
    return collapseForkDirective(directive, 200);
}
function hasFailedDisplayStatus(display) {
    return (display !== null &&
 ...
 * here, or explicitly waived for non-env-based auth like LailatulCoder-oauth). The
 ...
 */
export const AUTH_PREFLIGHT_ENV_KEYS = {
 ...
 */
export const AUTH_PREFLIGHT_WAIVED_AUTH_TYPES = new Set([
    'LailatulCoder-oauth',
]);
const PROJECT_SKILL_DIRS = ['.lailatulcoder', '.agents'];
...
const QWEN_CORE_SETTING_KEYS = Object.keys(QWEN_CORE_SETTING_DEFINITIONS);
const QWEN_HOOK_EVENTS = Object.values(HookEventName);
const DEFAULT_QWEN_MEMORY_SETTINGS = {
...
function readOptionalString(value, fieldName) {
    ...
    if (typeof value !== 'string') {
        throw RequestError.invalidParams(undefined, `Invalid ${fieldName}: expected string`);
    }
...
function readRequiredString(value, fieldName) {
    ...
    if (!stringValue) {
        throw RequestError.invalidParams(undefined, `Invalid or missing ${fieldName}`);
    }
    return stringValue;
}
...
function readSkillInstallRequest(params) {
    ...
    if (scope !== 'global') {
        throw RequestError.invalidParams(undefined, 'Only global skill installation is supported');
    }
    const description = readOptionalString(input['description'], 'skill.description');
    return {
...
function readSkillSlugRequest(params) {
    const skillParams = toRecord(params['skill']);
    const input = Object.keys(skillParams).length > 0 ? skillParams : params;
    const slug = readRequiredString(input['slug'], 'skill.slug');
    validateSkillSlug(slug);
    const scope = readOptionalString(input['scope'], 'skill.scope') ?? 'global';
    if (scope !== 'global') {
        throw RequestError.invalidParams(undefined, 'Only global skill management is supported');
    }
    return { slug, scope };
}
function readSkillSetEnabledRequest(params) {
    const skillParams = toRecord(params['skill']);
    const input = Object.keys(skillParams).length > 0 ? skillParams : params;
    const slug = readRequiredString(input['slug'], 'skill.slug');
    validateSkillSlug(slug);
    ...
    if (scope !== 'global' && scope !== 'project') {
        throw RequestError.invalidParams(undefined, 'Only global or project skill management is supported');
    }
    if (typeof input['enabled'] !== 'boolean') {
        throw RequestError.invalidParams(undefined, 'Invalid skill.enabled: expected boolean');
    }
    return {
        slug,
        scope,
...
function splitSkillMarkdown(content) {
    const normalized = content.replace(/^\uFEFF/, '').replace(/\r\n?/g, '\n');
    const match = normalized.match(/^---\n([\s\S]*?)\n---(?:\n|$)([\s\S]*)$/);
    if (!match) {
        throw RequestError.invalidParams(undefined, 'Invalid skill file: missing YAML frontmatter');
    }
    return {
        frontmatter: match[1],
        body: match[2],
...
function setSkillFrontmatterEnabled(content, enabled) {
    ...
    const disabledLineIndex = lines.findIndex((line) => /^disable-model-invocation\s*:/.test(line));
    if (enabled) {
...
function assertAllowedSkillSourceUrl(sourceUrl) {
    let parsed;
    try {
        parsed = new URL(sourceUrl);
    }
    catch {
        throw RequestError.invalidParams(undefined, 'Skill sourceUrl must be a valid URL');
    }
    ...
    if (parsed.protocol !== 'https:') {
        throw RequestError.invalidParams(undefined, 'Skill sourceUrl must be an HTTPS URL');
    }
    if (!ALLOWED_SKILL_SOURCE_HOSTS.has(parsed.hostname)) {
        throw RequestError.invalidParams(undefined, 'Skill sourceUrl host is not allowed (only github.com sources are supported)');
    }
}
function parseGitHubBlobSkillUrl(sourceUrl) {
...
function readTarPath(archive, offset) {
...
export async function extractFilesFromTarGz(archiveBytes, directoryPath, 
...
limits = {}) {
    const maxCompressedBytes = limits.maxCompressedBytes ?? MAX_SKILL_DOWNLOAD_BYTES;
    const maxDecompressedBytes = limits.maxDecompressedBytes ?? MAX_SKILL_DECOMPRESSED_BYTES;
    if (archiveBytes.length > maxCompressedBytes) {
        throw RequestError.invalidParams(undefined, 'Skill archive exceeds the maximum allowed size');
    }
    let archive;
    try {
        ...
        Readable.from([Buffer.from(archiveBytes)]), createGunzip(), new Writable({
            write(chunk, _enc, cb) {
                total += chunk.length;
        ...
        if (error instanceof DecompressedSizeExceededError) {
            throw RequestError.invalidParams(undefined, 'Decompressed skill archive exceeds the maximum allowed size');
        }
        throw RequestError.invalidParams(undefined, `Failed to decompress skill archive: ${error instanceof Error ? error.message : String(error)}`);
    }
...
function isAllowedSkillFetchHost(hostname) {
    if (ALLOWED_SKILL_SOURCE_HOSTS.has(hostname))
        return true;
    return ALLOWED_REDIRECT_HOST_SUFFIXES.some((suffix) => hostname.endsWith(suffix));
}
/**
 ...
 */
export async function fetchAllowedGitHub(url, init = {}, maxRedirects = 5) {
    let current = url;
    for (let hop = 0; hop <= maxRedirects; hop += 1) {
        ...
        try {
            ...
            throw RequestError.invalidParams(undefined, 'Skill download redirected to an invalid URL');
        }
        if (next.protocol !== 'https:' || !isAllowedSkillFetchHost(next.hostname)) {
            throw RequestError.invalidParams(undefined, 'Skill download redirected to a disallowed host');
        }
        current = next.toString();
    }
    throw RequestError.invalidParams(undefined, 'Skill download exceeded the maximum number of redirects');
}
...
async function readBodyWithLimit(response, maxBytes) {
    ...
    if (!body) {
        ...
        if (buf.byteLength > maxBytes) {
            throw RequestError.invalidParams(undefined, 'Skill download exceeds the maximum allowed size');
        }
        return buf;
    }
...
async function fetchBytes(url) {
    ...
    if (!response.ok) {
        throw RequestError.invalidParams(undefined, `Failed to download skill (${response.status})`);
    }
...
async function downloadSingleSkillFile(sourceUrl) {
...
async function downloadGitHubSkillDirectoryFromArchive(githubUrl, directoryPath) {
    const archiveUrl = `https://codeload.github.com/${githubUrl.owner}/${githubUrl.repo}/tar.gz/${encodeURIComponent(githubUrl.ref)}`;
    ...
    if (!response.ok) {
        throw RequestError.invalidParams(undefined, `Failed to download GitHub skill archive (${response.status})`);
    }
    ...
    return extractFilesFromTarGz(await readBodyWithLimit(response, MAX_SKILL_DOWNLOAD_BYTES), directoryPath);
}
async function fetchGitHubDirectoryItems(githubUrl, directoryPath) {
    ...
    if (!response.ok) {
        throw RequestError.invalidParams(undefined, `Failed to list GitHub skill files (${response.status})`);
    }
    const data = await response.json();
    if (!Array.isArray(data)) {
        throw RequestError.invalidParams(undefined, 'GitHub skill URL must point to a directory-backed SKILL.md file');
    }
    return data;
}
async function downloadGitHubSkillDirectoryFromApi(githubUrl, directoryPath, relativeRoot = '', 
...
depth = 0, budget = { files: 0, bytes: 0 }) {
    if (depth > MAX_SKILL_API_DIR_DEPTH) {
        throw RequestError.invalidParams(undefined, 'Skill directory nesting exceeds the maximum allowed depth');
    }
    ...
    for (const item of items) {
        ...
        const relativePath = relativeRoot
            ...
            : name;
        if (type === 'dir') {
            files.push(...(await downloadGitHubSkillDirectoryFromApi(githubUrl, itemPath, relativePath, depth + 1, budget)));
            continue;
        }
        if (type !== 'file')
            continue;
        ...
        if (budget.files > MAX_SKILL_API_FILE_COUNT) {
            throw RequestError.invalidParams(undefined, 'Skill directory contains too many files');
        }
        const downloadUrl = readRequiredString(record['download_url'], 'github.download_url');
        ...
        if (budget.bytes > MAX_SKILL_DECOMPRESSED_BYTES) {
            throw RequestError.invalidParams(undefined, 'Skill directory exceeds the maximum allowed size');
        }
        files.push({
            relativePath,
            content,
...
async function downloadGitHubSkillDirectory(githubUrl, directoryPath) {
    const apiFiles = await downloadGitHubSkillDirectoryFromApi(githubUrl, directoryPath).catch((error) => {
        debugLogger.warn('GitHub API directory listing failed, falling back to archive download:', error);
        return null;
    });
    if (apiFiles)
        return apiFiles;
...
async function downloadSkill(sourceUrl) {
    ...
    if (!skillFile) {
        throw RequestError.invalidParams(undefined, 'GitHub skill directory does not contain SKILL.md');
    }
    return {
...
function resolveSkillInstallPath(skillDir, relativePath) {
    ...
    if (target !== root && !target.startsWith(root + path.sep)) {
        throw RequestError.invalidParams(undefined, `Invalid skill file path: ${relativePath}`);
    }
    return target;
}
...
function readPositiveNumber(value, fieldName) {
    ...
    if (typeof value !== 'number' || !Number.isFinite(value) || value <= 0) {
        throw RequestError.invalidParams(undefined, `Invalid ${fieldName}: expected positive number`);
    }
    return value;
}
function readProviderAdvancedConfig(value) {
    if (value === undefined || value === null)
        return undefined;
    const record = toRecord(value);
    if (record['enableThinking'] !== undefined &&
        typeof record['enableThinking'] !== 'boolean') {
        throw RequestError.invalidParams(undefined, 'Invalid advancedConfig.enableThinking: expected boolean');
    }
    const multimodalRecord = toRecord(record['multimodal']);
    const multimodal = {};
    for (const key of ['image', 'video', 'audio', 'pdf']) {
        const flag = multimodalRecord[key];
        if (flag !== undefined) {
            if (typeof flag !== 'boolean') {
                throw RequestError.invalidParams(undefined, `Invalid advancedConfig.multimodal.${key}: expected boolean`);
            }
            multimodal[key] = flag;
        }
    }
    const contextWindowSize = readPositiveNumber(record['contextWindowSize'], 'advancedConfig.contextWindowSize');
    const maxTokens = readPositiveNumber(record['maxTokens'], 'advancedConfig.maxTokens');
...
function resolveProviderDocumentationUrl(config, baseUrl) {
...
function readSettingsEnv(settings, envKey) {
    if (!envKey)
        return undefined;
    const env = toRecord(settings.merged['env']);
    const value = env[envKey];
    return typeof value === 'string' && value.length > 0 ? value : undefined;
}
function resolveProviderEnvKey(config, protocol, baseUrl) {
    try {
...
function readExistingAdvancedConfig(model) {
...
function readExistingProviderConfig(config, settings) {
    const existing = findExistingProviderModels(config, settings.merged['modelProviders']);
    const firstModel = existing?.models[0];
    const protocol = existing?.protocol ?? config.protocol;
    const baseUrl = typeof firstModel?.baseUrl === 'string'
        ? firstModel.baseUrl
        : resolveBaseUrl(config);
    const envKey = typeof firstModel?.envKey === 'string'
        ? firstModel.envKey
...
// (never serialized to the client). Used so `LailatulCoder/providers/connect` can keep
...
function serializeProviderConfig(config, settings) {
    ...
    const defaultBaseUrl = config.baseUrl === undefined
    ...
    return {
        ...
        baseUrlPlaceholder: config.baseUrl === undefined ? defaultBaseUrl : undefined,
...
function readProviderSetupInputs(config, params, resolveExistingApiKey) {
    const protocol = readOptionalString(params['protocol'], 'protocol');
    if (protocol &&
        protocol !== config.protocol &&
        !config.protocolOptions?.includes(protocol)) {
        throw RequestError.invalidParams(undefined, `Invalid protocol for provider "${config.id}"`);
    }
    let baseUrl = resolveBaseUrl(config, readOptionalString(params['baseUrl'], 'baseUrl')).trim();
    if (!baseUrl && config.baseUrl === undefined) {
        baseUrl = getDefaultBaseUrlForProtocol(protocol ?? config.protocol);
    }
    if (!baseUrl) {
        throw RequestError.invalidParams(undefined, `Invalid or missing baseUrl for provider "${config.id}"`);
    }
    ...
    const apiKey = readOptionalString(params['apiKey'], 'apiKey') ??
    ...
    if (resolvedModelIds.length === 0) {
        throw RequestError.invalidParams(undefined, `Invalid or missing modelIds for provider "${config.id}"`);
    }
...
function readProviderConnectScope(value) {
    if (value === undefined)
        return undefined;
    if (value === 'user')
    ...
    throw RequestError.invalidParams(undefined, 'Invalid scope for provider connect');
}
function getNestedSettingValue(source, key) {
    let current = source;
...
function readCoreSettingValues(source) {
    const values = {};
...
export function normalizeCoreSettingValue(key, value) {
    ...
    switch (definition.type) {
        ...
        case 'number':
            ...
            if (definition.min !== undefined && value < definition.min) {
                throw RequestError.invalidParams(undefined, `${key} must be at least ${definition.min}`);
            }
            return value;
        case 'enum': {
            const values = definition.values;
            if (typeof value !== 'string' || !values?.includes(value)) {
                throw RequestError.invalidParams(undefined, `${key} must be one of ${values?.join(', ')}`);
            }
            return value;
        }
        case 'string': {
        ...
        default:
            throw RequestError.invalidParams(undefined, `${key} has an unsupported setting type`);
    }
}
...
function normalizeMcpServerConfig(value) {
    ...
    if (transport !== 'stdio' && transport !== 'http' && transport !== 'sse') {
        throw RequestError.invalidParams(undefined, 'MCP transport must be stdio, http, or sse');
    }
    ...
    if (typeof input['trust'] === 'boolean')
        server.trust = input['trust'];
    ...
    if (transport === 'stdio') {
        ...
        if (typeof command !== 'string' || !command.trim()) {
            throw RequestError.invalidParams(undefined, 'Stdio MCP servers require a command');
        }
    ...
    if (typeof url !== 'string' || !url.trim()) {
        throw RequestError.invalidParams(undefined, `${transport.toUpperCase()} MCP servers require a URL`);
    }
    if (transport === 'http')
        server.httpUrl = url.trim();
    else
        server.url = url.trim();
    server.headers = normalizeStringRecord(input['headers']);
    return server;
}
function toStoredMcpServerConfig(server) {
    const result = {};
    for (const key of [
        'timeout',
        'trust',
...
function toMcpServerConfig(value) {
...
function restoreSecretRecord(incoming, prior) {
    if (!incoming)
        return incoming;
...
function redactHookSecrets(hook) {
    return {
        ...hook,
...
function restoreRedactedHookSecrets(hook, prior) {
    ...
    return {
        ...
        hooks: hook.hooks.map((config, i) => {
            ...
            return {
                ...
                ...(config.headers
                    ? {
                        headers: restoreSecretRecord(config.headers, priorConfig['headers']),
                    }
                    : {}),
            };
        }),
...
function readMcpServers(source, scope) {
    ...
    return Object.entries(servers)
        ...
        try {
            ...
            debugLogger.warn(`Skipping malformed MCP server config [${scope}:${name}]:`, error);
            return undefined;
        }
    })
        .filter((entry) => !!entry);
...
function isHookEvent(value) {
    return (typeof value === 'string' &&
        QWEN_HOOK_EVENTS.includes(value));
}
function normalizeHookConfig(value) {
    const input = toRecord(value);
    const type = input['type'];
    if (type !== 'command' && type !== 'http') {
        throw RequestError.invalidParams(undefined, 'Hook type must be command or http');
    }
    const config = { type };
    if (type === 'command') {
        const command = input['command'];
        if (typeof command !== 'string' || !command.trim()) {
            throw RequestError.invalidParams(undefined, 'Command hooks require a command');
        }
        config.command = command.trim();
        config.env = normalizeStringRecord(input['env']);
        if (typeof input['async'] === 'boolean')
            config.async = input['async'];
        ...
        if (typeof input['once'] === 'boolean')
            config.once = input['once'];
    }
    const timeout = normalizeOptionalNumber(input['timeout']);
    if (timeout !== undefined)
        config.timeout = timeout;
    for (const key of ['name', 'description', 'statusMessage']) {
...
function normalizeHookDefinition(value) {
    const input = toRecord(value);
    const hooks = input['hooks'];
    if (!Array.isArray(hooks) || hooks.length === 0) {
        throw RequestError.invalidParams(undefined, 'Hook definition requires at least one hook');
    }
    const definition = {
...
function readHooks(source, scope, extensionName) {
    ...
    for (const event of QWEN_HOOK_EVENTS) {
        ...
        eventHooks.forEach((hookValue, index) => {
            try {
                ...
                debugLogger.warn(`Skipping malformed hook entry [${scope}:${event}:${index}]:`, error);
            }
        });
    }
    return entries;
}
function toSettingsScope(scope) {
    if (scope === 'workspace')
        return SettingScope.Workspace;
    if (scope === 'user')
        return SettingScope.User;
    throw RequestError.invalidParams(undefined, 'scope must be user or workspace');
}
function readScopeSettings(settings, scope) {
    return settings.forScope(toSettingsScope(scope)).settings;
}
...
async function resolveQwenMemoryPaths(params) {
    const fallbackFilename = getAllGeminiMdFilenames()[0] ?? 'LAILATUL.md';
    const userMemoryFile = await resolvePreferredMemoryFile(Storage.getGlobalQwenDir(), fallbackFilename);
    const projectMemoryFile = await resolvePreferredMemoryFile(params.cwd, fallbackFilename);
 ...
 * over the `LailatulCoder/control/client_mcp/message` ext-method, returning the
...
export async function deliverClientMcpMessage(connection, serverName, message, sessionId) {
    if (!connection) {
        throw new Error(`client MCP server '${serverName}' has no ACP connection yet`);
    }
    const response = await connection.extMethod(SERVE_CONTROL_EXT_METHODS.clientMcpMessage, {
        server: serverName,
        payload: message,
        ...(sessionId ? { sessionId } : {}),
    });
    ...
    if (payload === undefined || payload === null) {
        throw new Error(`client_mcp/message returned no payload for server '${serverName}'`);
    }
    return payload;
}
/**
 ...
 */
export function createManagedExternalToolGuard(connection, options = {
    ...
    return async (context) => {
        ...
        if (!options.externalProviderAttached &&
        ...
        if (!invocation && options.externalProviderAttached) {
            throw new Error('Managed external tool guard requires a runtime invocation context.');
        }
        ...
        if (typeof sessionId !== 'string' || sessionId.length === 0) {
            throw new Error('Managed external tool guard requires a session identity.');
        }
        ...
        const onAbort = () => rejectOnAbort?.(new DOMException('Tool invocation aborted', 'AbortError'));
        ...
        try {
            const response = await Promise.race([
                connection.extMethod(SERVE_CONTROL_EXT_METHODS.externalToolGuardPrepare, {
                    sessionId,
            ...
            if (typeof response['allowed'] !== 'boolean' ||
                ...
                throw new Error('Managed external tool guard returned an invalid reply.');
            }
            if (response['allowed']) {
                if (Object.hasOwn(response, 'reason')) {
                    throw new Error('Managed external tool guard allow reply contains a reason.');
                }
                return { allowed: true };
            }
            ...
            if (!isValidExternalToolGuardDenialReason(reason)) {
                throw new Error('Managed external tool guard denial reason is invalid.');
            }
...
function readRuntimeMcpRequest(params) {
    ...
    if (!isValidServerName(name)) {
        throw RequestError.invalidParams(undefined, 'Server name must be ≤256 chars, alphanumeric + underscore/hyphen, and not a reserved JS property name');
    }
    const originatorClientId = params['originatorClientId'];
    return {
        name,
        runtimeClientId: typeof originatorClientId === 'string' && originatorClientId.length > 0
            ? originatorClientId
            : 'daemon',
    };
}
function readRuntimeMcpAddRequest(params) {
    ...
    const { trust: _trust, authProviderType: _auth, includeTools: _inc, excludeTools: _exc, cwd: _cwd, env: _env, oauth: _oauth, headers: _headers, type: _type, [CLIENT_MCP_OVER_WS_CONFIG_FLAG]: clientMcpOverWs, ...safeConfig } = config;
...
function getRuntimeMcpManager(config) {
    ...
    if (!manager) {
        throw RequestError.internalError(undefined, 'McpClientManager unavailable on this Config');
    }
    return manager;
}
async function addRuntimeMcpServer(manager, request) {
    try {
        return (await manager.addRuntimeMcpServer(request.name, request.config, request.runtimeClientId));
    }
    catch (err) {
...
export async function runAcpAgent(config, settings, argv, options) {
    ...
    const sessionWriterLeaseEnabledAtStartup = typeof config.isSessionWriterLeaseEnabled === 'function'
        ? config.isSessionWriterLeaseEnabled()
        : settings.merged.experimental?.sessionWriterLease === true;
    const privateParentCapability = options === undefined
    ...
    const externalToolGuardProviderAttached = options?.externalToolGuardProviderAttached === true;
    if (externalToolGuardRequired && privateParentCapability === undefined) {
        throw new Error('Required external tool guard is available only to a private managed ACP parent.');
    }
    ...
    let acpConnection;
    const bootstrapClientMcpSender = (serverName, message) => deliverClientMcpMessage(acpConnection, serverName, message);
    ...
    const startupWarnings = typeof config.getWarnings === 'function' ? config.getWarnings() : [];
    ...
    try {
        ...
        const pendingNewSessionRequestIds = new Set();
            ...
            onMessageObserved: ({ direction, bytes, message }) => {
                ...
                if (direction === 'sent' &&
                    ...
                    setImmediate(() => {
                        ...
                        if (!session)
                            return;
                        void preloadContentGenerator(session.getConfig().getContentGenerator()).catch((error) => {
                            debugLogger.debug(`[ACP] Session provider preload failed for ${sessionId}: ${error instanceof Error ? error.message : String(error)}`);
                        });
                    }).unref();
                }
                if (direction !== 'sent' ||
        ...
        connection = new AgentSideConnection((conn) => {
            ...
            const managedToolInvocationGuard = externalToolGuardRequired
                ...
                : undefined;
            agentInstance = new QwenAgent(config, settings, argv, conn, privateParentCapability, sessionWriterLeaseEnabledAtStartup, managedToolInvocationGuard, externalToolGuardProviderAttached);
    ...
    const drainPoolBeforeExit = async (label, strict = false) => {
        if (!agentInstance)
            return;
        try {
    ...
    const fireSessionEndOnce = async (reason, managedConfigs) => {
        if (sessionEndFired)
            return;
    ...
    const shutdownManagedAgent = (reason, label) => {
        if (managedShutdownPromise)
            return managedShutdownPromise;
        const agent = agentInstance;
        if (!agent?.isTrustedManagedParent()) {
            return Promise.reject(new Error('Managed shutdown requires a trusted private parent'));
        }
        ...
        managedShutdownPromise = (async () => {
            ...
            try {
                ...
                const retainedLockPaths = terminal.configs.flatMap((config) => {
                    if (typeof config.getSessionRuntimeBaseDir !== 'function' ||
                        typeof config.getSessionId !== 'function') {
                        return [];
                    }
                    return [
                        path.join(config.getSessionRuntimeBaseDir(), 'tmp', 'session-writer-locks', `${encodeURIComponent(config.getSessionId())}.lock`),
                    ];
                });
                writeStderrLineSafe('LailatulCoder --acp: managed session writer shutdown failed; a writer lock may be retained for safety. ' +
    ...
    try {
        ...
        if (agentInstance?.isTrustedManagedParent()) {
            try {
                await shutdownManagedAgent(SessionEndReason.PromptInputExit, 'ide_close');
            }
            catch (error) {
                process.exitCode = 1;
                throw error;
 ...
 */
function parsePooledTransports(envValue) {
    if (!envValue || !envValue.trim())
        return POOLED_TRANSPORTS_DEFAULT;
    const KNOWN = new Set([
        'stdio',
        'websocket',
        'http',
 ...
 */
function parsePoolDrainMs(envValue) {
    if (!envValue)
        return 30_000;
    ...
    if (!/^\d+$/.test(trimmed)) {
        process.stderr.write(`LailatulCoder serve: QWEN_SERVE_MCP_POOL_DRAIN_MS=${JSON.stringify(envValue)} ` +
 ...
 */
export function createWorkspaceMcpBudget(onEvent) {
    ...
    if (rawBudget !== undefined && rawBudget !== '') {
        ...
        if (/^\d+$/.test(trimmed) && Number.isSafeInteger(parsed) && parsed > 0) {
            ...
            process.stderr.write(`LailatulCoder serve: ignoring invalid QWEN_SERVE_MCP_CLIENT_BUDGET=` +
...
function normalizeAcpSessionListSize(value) {
    if (value == null)
        return undefined;
...
function parseAcpSessionListCursor(value) {
    ...
    if (trimmed === '' ||
        ...
        throw RequestError.invalidParams(undefined, `Invalid cursor: "${value}" is not a valid numeric cursor`);
    }
    return parsedCursor;
}
function isOwnerOnlyDirectory(stats) {
...
class QwenAgent {
    ...
    argv;
    connection;
    expectedPrivateParentCapability;
    sessionWriterLeaseEnabledAtStartup;
    managedToolInvocationGuard;
    externalToolGuardProviderAttached;
    sessions = new Map();
    historyMutationTails = new Map();
    startingSessionIds = new Set();
    activePromptCalls = new Map();
    workspaceMcpDiscoveryConfig;
    workspaceMcpDiscoveryPromise;
    workspaceMcpDiscoveryError;
    workspaceExtensionStatusRefreshPromise;
    pendingMcpAuthentications = new Map();
    mcpAuthenticationResults = new Map();
    generationControllers = new Map();
    workspaceGenerationControllers = new Map();
    transcriptReplayConfigCache = new Map();
    pendingConfigCleanup = new Map();
    initializingConfigs = new Set();
    ...
    // containers.
    childCpuCoreCount = os.availableParallelism?.() ?? os.cpus().length ?? 1;
    prevChildCpu = (() => {
        try {
     ...
     */
    childHeapProbe = shouldProbeChildHeap(process.env) ? startChildHeapProbe() : undefined;
    /**
    ...
    assertManagedSessionAdmission() {
        if (this.expectedPrivateParentCapability !== undefined &&
            !this.isTrustedManagedParent()) {
            throw RequestError.invalidParams(undefined, 'Private ACP parent is not initialized');
        }
        if (this.managedShuttingDown) {
            throw new SessionWriterUnavailableError();
        }
    }
    async runExclusiveHistoryMutation(sessionId, operation, waitTimeoutMs, waitDisplayTimeoutMs) {
        ...
        try {
            if (waitTimeoutMs === undefined) {
                ...
                await waitForSessionDrain(previous, waitTimeoutMs, 'close', waitDisplayTimeoutMs);
            }
        }
        catch (error) {
            release();
    ...
    rejectUnsupportedGuardedHiddenAgent(operation) {
        if (this.managedToolInvocationGuard &&
            this.externalToolGuardProviderAttached) {
            throw RequestError.invalidParams(undefined, `Managed external tool guard v1 does not support ${operation}.`);
        }
    }
    beginManagedShutdown() {
        ...
        const configs = new Set([this.config, ...this.initializingConfigs]);
        for (const session of this.sessions.values()) {
            try {
                session.beginCloseIfAvailable();
            }
            catch (error) {
                debugLogger.debug(`Session ${session.getId()} admission close during managed shutdown failed: ${error instanceof Error ? error.message : String(error)}`);
            }
            void session.cancelPendingPrompt().catch((error) => {
                debugLogger.debug(`Session ${session.getId()} cancel during managed shutdown failed: ${error instanceof Error ? error.message : String(error)}`);
            });
        ...
        const writerShutdown = Promise.allSettled(writerTerminals).then((results) => {
            const failures = results.flatMap((result) => result.status === 'rejected' ? [result.reason] : []);
            if (failures.length > 0) {
                throw new AggregateError(failures, 'Managed session writer shutdown failed');
            }
        });
    ...
    async finishManagedShutdown(configs) {
        ...
        const results = await Promise.allSettled(configs.map((config) => config.shutdown({
    ...
    refreshBootstrapExtensionStatus() {
        ...
        const promise = (async () => {
            ...
            if (errors.length === 1)
                throw errors[0];
            if (errors.length > 1) {
                throw new AggregateError(errors, 'Bootstrap extension status refresh failed');
            }
        })();
    ...
    async reconcileMcpServerAcrossLiveConfigs(serverName, operation) {
        const errors = [];
        ...
        if (errors.length > 0) {
            const details = errors
                ...
                .join('; ');
            throw new AggregateError(errors, `Failed to synchronize MCP server ${JSON.stringify(serverName)}: ${details}`);
        }
    }
    getMcpServerStatus(config, serverName) {
        const manager = config.getToolRegistry()?.getMcpClientManager();
        return (manager?.getServerStatus?.(serverName) ?? getMCPServerStatus(serverName));
    }
    enqueueWorkspaceMcpDiscovery(label, run) {
        ...
        const tracked = previous
            ...
            debugLogger.error(`Workspace MCP ${label} failed: ${err instanceof Error ? err.message : String(err)}`);
        })
            .finally(() => {
    ...
    async createWorkspaceMcpDiscoveryConfig(settings) {
        if (this.workspaceMcpDiscoveryConfig)
            return;
        const cwd = this.config.getTargetDir();
        const config = await this.runWithPinnedRuntimeBaseDir(settings, cwd, () => loadCliConfig(settings.merged, {
            ...this.argv,
        ...
        try {
            ...
            if (manager.getDiscoveryState() === MCPDiscoveryState.NOT_STARTED) {
                throw new Error('MCP discovery did not start. The workspace may not be trusted.');
            }
    ...
    initializeWorkspaceMcpDiscovery() {
    ...
    reloadWorkspaceMcpDiscovery(options = {}) {
        return this.enqueueWorkspaceMcpDiscovery('reload', async () => {
            ...
            for (const config of liveConfigs) {
                try {
                    ...
                    const mcpServers = isBareOrSafe
                        ...
                        : assembleMcpServers(settings.merged.mcpServers, cwd, config.getTopTierMcpServers());
                    const bootAllowed = config.getCliAllowedMcpServerNames();
                    const gating = isBareOrSafe
                        ? { allowed: bootAllowed ? [...bootAllowed] : undefined }
                        : recomputeMcpGating(settings, mcpServers, cwd, bootAllowed, config.getApprovalMode() === ApprovalMode.YOLO);
            ...
            if (syncErrors.length > 0) {
                throw new AggregateError(syncErrors, 'Failed to synchronize MCP settings with live sessions');
            }
            if (options.forceReconnectAll === true ||
                options.forceReconnectWhich !== undefined) {
                await this.forceReconnectWorkspaceMcp(options.forceReconnectWhich);
            }
        });
    }
    async forceReconnectWorkspaceMcp(requestedServerNames) {
        ...
        const selectedServerNames = requestedServerNames
            ? [...new Set(requestedServerNames)].filter((name) => serverNames.has(name))
            : [...serverNames];
        const errors = [];
        for (const serverName of selectedServerNames) {
            try {
                const poolHasEntries = (this.mcpPool?.getSnapshot().byName[serverName]?.entryCount ?? 0) > 0;
                if (this.mcpPool && poolHasEntries) {
                    ...
                    if (failed) {
                        throw new Error(`MCP server ${JSON.stringify(serverName)} failed to reconnect: ${failed.reason ?? 'unknown error'}`);
                    }
                    await Promise.all(this.getLiveMcpConfigs(serverName).map(async (config) => {
                        const geminiClient = config.getGeminiClient?.();
                        if (geminiClient?.isInitialized?.()) {
                            await geminiClient.setTools?.();
                        }
        ...
        if (errors.length > 0) {
            throw new AggregateError(errors, 'Failed to force reconnect one or more MCP servers');
        }
    }
    /**
     ...
     */
    async shutdownMcpPool(timeoutMs = 10_000) {
        if (!this.mcpPool)
            return;
        try {
            const result = await this.mcpPool.drainAll({ force: true, timeoutMs });
            if (result.forced > 0 || result.errors.length > 0) {
                debugLogger.warn(`MCP pool drain: ${result.drained} clean, ${result.forced} timed out, ` +
                    `${result.errors.length} errors`);
                throw new Error(`MCP pool drain incomplete: ${result.forced} forced, ${result.errors.length} errors`);
            }
        }
        catch (err) {
            debugLogger.error(`MCP pool drainAll failed: ${err instanceof Error ? err.message : String(err)}`);
            throw err;
        }
    }
    async removeStoredSessionEntry(sessionId, session, cleanupErrors = [], options = {}) {
        ...
        if (cleanupErrors.length > 0) {
            debugLogger.warn(`Session ${sessionId} closed after ${cleanupErrors.length} cleanup failure(s): ${cleanupErrors
                .map((error) => error instanceof Error ? error.message : String(error))
                .join('; ')}`);
        }
    }
    async withLiveSessionRestore(sessionId, session, options, operation) {
        await session.assertCanStartTurn();
        const config = session.getConfig();
        const releaseGate = session.beginClose();
        try {
            await waitForSessionDrain(session.waitForActiveTurnsToSettle(), SESSION_DRAIN_TIMEOUT_MS, 'restore');
            const recorder = config.getChatRecordingService();
            const readProjection = () => config
                .getSessionService()
    ...
    async cleanupUnstoredConfig(config) {
        const sessionId = config.getSessionId();
        const cleanupKey = this.pendingConfigCleanupKey(config.getSessionRuntimeBaseDir(), sessionId);
        try {
    ...
    async cleanupAfterRequestFailure(error, cleanup, sessionId) {
        try {
            await cleanup();
        }
        catch (cleanupError) {
            debugLogger.warn(`Session cleanup failed while preserving the original request error: ${cleanupError instanceof Error ? cleanupError.message : String(cleanupError)}`);
        }
        throw sessionId
    ...
    async retryPendingConfigCleanup(runtimeBaseDir, requiredSessionId) {
        const resolvedRuntimeBaseDir = path.resolve(runtimeBaseDir);
        const configs = new Set();
        for (const pending of this.pendingConfigCleanup.values()) {
            for (const config of pending) {
                if (path.resolve(config.getSessionRuntimeBaseDir()) ===
        ...
        for (const config of configs) {
            try {
                ...
                if (config.getSessionId() === requiredSessionId)
                    throw error;
                debugLogger.warn(`Deferred Config cleanup retry failed for session ${config.getSessionId()}: ${error instanceof Error ? error.message : String(error)}`);
            }
        }
    }
    async closeStoredSession(sessionId, opts) {
        ...
        try {
            if (opts?.onlyIfUnheld) {
                ...
                await waitForSessionDrain(session.waitForActiveTurnsToSettle(), drainTimeoutMs, 'close');
                const holds = session.collectActiveWorkHolds();
                if (holds.length > 0) {
                    return { closed: false, holds };
                }
            ...
            await waitForSessionDrain((async () => {
                try {
                    await session.cancelPendingPrompt();
                }
                catch (err) {
                    debugLogger.debug(`Session ${sessionId} cancel during close failed: ${err instanceof Error ? err.message : String(err)}`);
                }
                await session.waitForActiveTurnsToSettle();
            })(), conditionalDrainDeadline === undefined
                ? drainTimeoutMs
                : Math.max(1, conditionalDrainDeadline - Date.now()), 'close', 
            ...
            drainTimeoutMs);
            const blockedByHolds = await this.runExclusiveHistoryMutation(sessionId, async () => {
                if (this.sessions.get(sessionId) !== session) {
            ...
            conditionalDrainDeadline === undefined
                ? drainTimeoutMs
                : Math.max(1, conditionalDrainDeadline - Date.now()), drainTimeoutMs);
            if (blockedByHolds)
                return blockedByHolds;
        }
        finally {
            if (!removedFromStore)
                cancelClose();
    ...
    async discardStoredSessionIfCurrent(sessionId, session, opts) {
        if (this.sessions.get(sessionId) !== session) {
            return;
        }
    ...
    async disposeSessions() {
        ...
        this.workspaceGenerationControllers.clear();
        await Promise.allSettled([...this.sessions.entries()].map(([sessionId, session]) => this.discardStoredSessionIfCurrent(sessionId, session, {
            waitForCloseGate: true,
        })));
        await Promise.allSettled([...this.pendingConfigCleanup.values()]
    ...
    constructor(config, settings, argv, connection, expectedPrivateParentCapability, sessionWriterLeaseEnabledAtStartup = false, managedToolInvocationGuard, externalToolGuardProviderAttached = false) {
        ...
        this.managedToolInvocationGuard = managedToolInvocationGuard;
        this.externalToolGuardProviderAttached = externalToolGuardProviderAttached;
        // Pool kill switch via env var so operators can A/B compare or
        // roll back without rebuilding. `run-LailatulCoder-serve.ts` sets this when
        ...
        if (process.env['QWEN_SERVE_NO_MCP_POOL'] === '1') {
            ...
            this.mcpPool = new McpTransportPool(this.config, {
                ...
                // MCP to that bypass.
                pooledTransports: parsePooledTransports(process.env['QWEN_SERVE_MCP_POOL_TRANSPORTS']),
                drainDelayMs: parsePoolDrainMs(process.env['QWEN_SERVE_MCP_POOL_DRAIN_MS']),
                budget: this.workspaceMcpBudget,
            });
        }
    }
    runWithPinnedRuntimeBaseDir(settings, cwd, operation) {
        return runWithAcpRuntimeOutputDir(settings, cwd, operation);
    }
    async assertLiveSessionScope(config, settings, cwd) {
        if (path.resolve(config.storage.getProjectRoot()) !== path.resolve(cwd)) {
            throw RequestError.invalidParams(undefined, 'The live session belongs to another workspace.');
        }
        const requestedRuntimeBaseDir = await this.runWithPinnedRuntimeBaseDir(settings, cwd, () => Storage.getRuntimeBaseDir());
        if (path.resolve(config.getSessionRuntimeBaseDir()) !==
    ...
    broadcastBudgetEvent(event) {
        ...
        for (const sid of sessionIds) {
            void this.connection
                .extNotification('LailatulCoder/notify/session/mcp-budget-event', {
                v: 1,
                sessionId: sid,
                ...
                .catch((err) => {
                debugLogger.debug(`MCP workspace budget event delivery to session ${sid} failed ` +
    ...
    async initialize(args) {
        ...
        if (this.privateParentState === 'rejected') {
            throw RequestError.invalidParams(undefined, 'Invalid private ACP parent capability');
        }
        if (this.privateParentState === 'uninitialized') {
            ...
            if (expectedCapability === undefined) {
                ...
                const suppliedCapability = args._meta?.[PRIVATE_PARENT_CAPABILITY_META_KEY];
                const suppliedBuffer = typeof suppliedCapability === 'string'
        ...
        const profileRequested = requestedProfile !== null &&
        ...
        const activeWorkRequested = requestedActiveWork !== null &&
            typeof requestedActiveWork === 'object' &&
            !Array.isArray(requestedActiveWork) &&
            requestedActiveWork['v'] ===
        ...
        const activeWorkIntervalMs = activeWorkRequested
            ? clampActiveWorkIntervalMs(requestedActiveWork['intervalMs'])
            : undefined;
        const requestedActiveWorkCategories = activeWorkRequested
            ? requestedActiveWork['categories']
            : undefined;
        const activeWorkCategories = activeWorkRequested
            ? Array.isArray(requestedActiveWorkCategories)
                ? ACTIVE_WORK_HOLD_CATEGORIES.filter((category) => requestedActiveWorkCategories.includes(category))
                : ACTIVE_WORK_LEGACY_HOLD_CATEGORIES
            : undefined;
        if (activeWorkIntervalMs !== undefined) {
            this.activeWorkReporter?.dispose();
            this.activeWorkReporter = new ActiveWorkReporter((method, params) => this.connection.extNotification(method, params), () => this.sessions.values(), activeWorkIntervalMs, activeWorkCategories ?? []);
        }
        const responseMeta = {
            ...(this.managedToolInvocationGuard
                ? {
                    [EXTERNAL_TOOL_GUARD_READY_META_KEY]: EXTERNAL_TOOL_GUARD_REQUIRED_VALUE,
                }
                : {}),
    ...
    async authenticate({ methodId }) {
        ...
        if (method === AuthType.NEVER_MATCH_THIS) {
    ...
    reserveStartingSessionId(sessionId) {
        if (this.sessions.has(sessionId) ||
            this.startingSessionIds.has(sessionId)) {
            throw new RequestError(ACP_ERROR_CODES.INVALID_PARAMS, `Session ${sessionId} is already active or starting.`, { errorKind: 'session_id_conflict', sessionId });
        }
    ...
    async newSession(params) {
        ...
        const parsedSessionId = parseCallerSuppliedSessionId(params._meta?.[REQUESTED_SESSION_ID_META_KEY]);
        if (parsedSessionId.kind === 'invalid') {
            throw new RequestError(ACP_ERROR_CODES.INVALID_PARAMS, `\`_meta["${REQUESTED_SESSION_ID_META_KEY}"]\` must be an RFC UUID v1-v5`, { errorKind: 'invalid_session_id', httpStatus: 400 });
        }
        const requestedSessionId = parsedSessionId.kind === 'valid' ? parsedSessionId.sessionId : undefined;
        ...
        try {
            ...
            return await withDaemonSpan('lailatul-coder.daemon.session_start', { 'lailatul-coder.daemon.operation': 'acp_session_new' }, async (span) => {
                ...
                const settings = profiler.timeSync('settings_load', () => loadSettingsCached(cwd));
                this.settings = settings;
                const config = await profiler.time('config_setup', () => this.newSessionConfig(cwd, mcpServers, settings, sessionSource, requestedSessionId, undefined, shouldDeferMcpDiscovery(params)
                    ? { skipMcpDiscovery: true }
                    : undefined));
                let session;
                try {
                    await profiler.time('auth', () => this.ensureAuthenticated(config));
                    profiler.timeSync('file_system_setup', () => this.setupFileSystem(config));
                    session = await profiler.time('session_register', () => this.createAndStoreSession(config, settings));
                }
                catch (error) {
    ...
    async loadSession(params) {
        ...
        const parentContext = extractDaemonTraceContext(params);
        return await withDaemonSpan('lailatul-coder.daemon.session_restore', {
            'lailatul-coder.daemon.operation': 'acp_session_load',
            'lailatul-coder.daemon.session_restore.action': 'load',
            'session.id': sessionId,
        }, async (span) => this.loadSessionWithProfiler(params, sessionId, createAcpSessionRestoreProfiler(span)), parentContext ? { parentContext } : {});
    }
    async loadSessionWithProfiler(params, initialSessionId, profiler) {
        ...
        if (liveSession) {
            const settings = profiler.timeSync('settings_load', () => loadSettingsCached(params.cwd));
            ...
            return profiler.time('live_restore', async () => {
                ...
                return this.withLiveSessionRestore(sessionId, liveSession, restoreOptions, async (config, projection) => {
                    const response = profiler.timeSync('response_build', () => ({
                        modes: this.buildModesData(config),
                        models: this.buildAvailableModels(config),
                        configOptions: this.buildConfigOptions(config),
                    ...
                    const replay = await profiler.time('history_replay', () => collectHistoryReplayUpdates({
                        sessionId,
                        config,
                    ...
                    addDaemonRequestAttribute('lailatul-coder.daemon.session_restore.partial_replay', replay.replayError !== undefined);
                    if (!bulkReplay) {
                        try {
                    ...
                    validateLoadReplayEnvelope(sessionId, envelope, restoreOptions.replay.kind === 'recent');
                    return {
                        ...response,
                        _meta: {
        ...
        try {
            ...
            const settings = profiler.timeSync('settings_load', () => loadSettingsCached(params.cwd));
            const persistedSessionId = await profiler.time('existence_check', () => this.runWithPinnedRuntimeBaseDir(settings, params.cwd, async () => {
                ...
                try {
                    ...
                    if (error instanceof SessionIdCaseConflictError) {
                        ...
                        // occupancy.
                        throw RequestError.internalError({ errorKind: 'session_conflict', sessionId }, error.message);
                    }
                    throw error;
                }
            }));
            ...
            this.settings = settings;
            const config = await profiler.time('config_setup', () => this.newSessionConfig(params.cwd, 
            ...
            params.mcpServers ?? [], settings, sessionSource, sessionId, true, {}, undefined, restoreOptions));
            const projection = config.consumeSessionRestoreProjection?.();
            const suppressRecoveredGoalPresentation = projection?.runtime.goalRecoverySourceUuid !== undefined &&
            ...
            let recoveredGoalPublicationKey;
            let suppressedRecoveredGoalId;
            let streamGoalUpdates = [];
            let response;
            const buildResponse = () => profiler.timeSync('response_build', () => ({
                modes: this.buildModesData(config),
                models: this.buildAvailableModels(config),
                configOptions: this.buildConfigOptions(config),
            ...
            try {
                await profiler.time('auth', () => this.ensureAuthenticated(config));
                profiler.timeSync('file_system_setup', () => this.setupFileSystem(config));
                await profiler.time('session_register', () => this.createAndStoreSession(config, settings, undefined, {
                    enableLiveScreenContext: isCompatibleLiveSessionSource(sessionSource ?? {}),
                    replayHistory: false,
                    prepareBeforeSessionCreate: async () => {
                        if (bulkReplay && projection?.replay) {
                            ...
                            validateLoadReplayEnvelope(sessionId, replayEnvelope, restoreOptions.replay.kind === 'recent');
                        }
                        const goalBootstrap = replayGoalBootstrap(projection);
                        const rendered = await renderPreparedGoalUpdate(() => config.getGoalRuntimePrepared(), {
                            ...
                            if (suppressRecoveredGoalPresentation)
                                throw error;
                            debugLogger.debug(`Failed to render recovered Goal state: ${error instanceof Error ? error.message : String(error)}`);
                            return {
                    ...
                    primeSession: (createdSession) => {
                        profiler.timeSync('runtime_initialize', () => {
                            if (!projection)
                                return;
                            createdSession.primeTurnState(projection.runtime.initialTurn, projection.runtime.backgroundNotificationTaskIds);
                            copyCumulativeUsage(createdSession.cumulativeUsage, replayUsage);
                            createdSession.primeRecoveredGoalPublication(recoveredGoalPublicationKey, suppressedRecoveredGoalId);
                        });
                    },
                    beforeStartPostReplayServices: async (createdSession) => {
                        if (!bulkReplay && projection?.replay) {
                            await profiler.time('history_replay', async () => {
                                ...
                                const hasGoalReplayState = goalBootstrap !== undefined ||
                                    initialGoalState.initialGoalState !== undefined ||
                                    initialGoalState.initialGoalCause !== undefined;
                                if (hasGoalReplayState) {
                                    await createdSession.replayHistory(projection.replay.records, projection.replay.gaps, {
                            ...
                            try {
                                ...
                                debugLogger.debug(`Failed to publish recovered Goal state: ${error instanceof Error ? error.message : String(error)}`);
                            }
                        }
    ...
    async unstable_resumeSession(params) {
        ...
        return await withDaemonSpan('lailatul-coder.daemon.session_restore', {
            ...
            'session.id': sessionId,
        }, async (span) => this.resumeSessionWithProfiler(params, sessionId, createAcpSessionRestoreProfiler(span)), parentContext ? { parentContext } : {});
    }
    async resumeSessionWithProfiler(params, initialSessionId, profiler) {
        ...
        if (liveSession) {
            ...
            return profiler.time('live_restore', async () => {
                await this.assertLiveSessionScope(liveConfig, settings, params.cwd);
                return this.withLiveSessionRestore(sessionId, liveSession, RESUME_RESTORE_OPTIONS, async (config, projection) => profiler.timeSync('response_build', () => ({
                    modes: this.buildModesData(config),
                    models: this.buildAvailableModels(config),
                    configOptions: this.buildConfigOptions(config),
                    ...(projection?.artifactSnapshot
        ...
        try {
            ...
            profiler.setSessionId(sessionId);
            this.settings = settings;
            const config = await profiler.time('config_setup', () => this.newSessionConfig(params.cwd, params.mcpServers ?? [], settings, sessionSource, sessionId, true, {}, undefined, RESUME_RESTORE_OPTIONS));
            const projection = config.consumeSessionRestoreProjection?.();
            let response;
            try {
                await profiler.time('auth', () => this.ensureAuthenticated(config));
     ...
     * requirement.
     */
    async #restoreWorktreeOnResume(config, session) {
        try {
    ...
    async #restoreBackgroundAgentsOnResume(config, session) {
    ...
    async setSessionConfigOption(params) {
        ...
        switch (configId) {
            ...
            case 'reasoning_effort': {
                const modelReasoning = this.getModelReasoningConfiguration(session.getConfig());
                if (modelReasoning) {
                    const effortValues = modelReasoning.toggleOnly
                        ? undefined
                        : modelReasoning.efforts;
                    const selected = value === ACP_REASONING_EFFORT_NONE
                    ...
                    if (!selected) {
                        ...
                        throw RequestError.invalidParams(undefined, `Unknown reasoning effort: ${value}. Choose one of: ${choices.join(', ')}`);
                    }
                ...
                const effort = value === ACP_REASONING_EFFORT_DEFAULT
                    ? undefined
                    : REASONING_EFFORT_TIERS.find((tier) => tier === value);
                if (value !== ACP_REASONING_EFFORT_DEFAULT && effort === undefined) {
                    throw RequestError.invalidParams(undefined, `Unknown reasoning effort: ${value}. Choose one of: ${ACP_REASONING_EFFORT_DEFAULT}, ${REASONING_EFFORT_TIERS.join(', ')}`);
                }
                if (!applyReasoningEffort(session.getConfig(), effort)) {
                    throw RequestError.invalidParams(undefined, 'Reasoning effort cannot be applied while thinking is disabled');
                }
                break;
            }
            default:
    ...
    async prompt(params) {
        ...
        const invocationContext = this.privateParentState === 'trusted' && suppliedContext !== undefined
            ? parseInvocationContext(suppliedContext)
            : undefined;
        if (this.privateParentState === 'trusted' &&
            suppliedContext !== undefined &&
            invocationContext === undefined) {
            throw RequestError.invalidParams(undefined, 'Invalid trusted ACP invocation context');
        }
        const modelPrompt = this.privateParentState === 'trusted' &&
        ...
        if (this.privateParentState === 'trusted' &&
            ...
            throw RequestError.invalidParams(undefined, 'Invalid trusted ACP model prompt');
        }
        let settleCall = () => { };
        const call = {
        ...
        calls.add(call);
        try {
            return await session.prompt(sanitizedParams, invocationContext, call.controller.signal, modelPrompt);
        }
        finally {
            calls.delete(call);
            if (calls.size === 0) {
    ...
    async buildCoreSettings(settings, cwd) {
        ...
        try {
            ...
            debugLogger.warn('Extension loading failed, continuing without extensions:', error);
        }
        const extensionEntries = await Promise.all(extensions.map(async (extension) => {
            const userEnv = await getScopedEnvContents(extension.config, extension.id, ExtensionSettingScope.USER);
            const workspaceEnv = await getScopedEnvContents(extension.config, extension.id, ExtensionSettingScope.WORKSPACE);
        ...
        const activeExtensions = extensions.filter((extension) => extension.isActive);
        const extensionMcpServers = activeExtensions.flatMap((extension) => readMcpServers({ mcpServers: extension.config.mcpServers ?? {} }, 'extension').map((entry) => ({
            ...entry,
            server: {
                ...entry.server,
        ...
        const extensionHooks = activeExtensions.flatMap((extension) => readHooks({ hooks: extension.hooks ?? {} }, 'extension', extension.displayName ?? extension.name));
        ...
        for (const ruleType of PERMISSION_RULE_TYPES) {
            ...
            for (const session of this.sessions.values()) {
                ...
                try {
                    ...
                    debugLogger.warn(`Failed to sync permission rules to a live session: ${error instanceof Error ? error.message : String(error)}`);
                }
            }
        }
    }
    ...
    discoveryState(config) {
        if (this.workspaceMcpDiscoveryPromise &&
    ...
    async buildWorkspaceMcpStatus(config) {
        try {
            ...
            const systemDefaultServers = settings.systemDefaults?.settings.mcpServers ?? {};
            ...
            try {
                ...
                process.stderr.write(`LailatulCoder serve: pool snapshot for workspace MCP status failed: ` +
                    `${err instanceof Error ? err.message : String(err)}\n`);
            }
            ...
            let clientBudget;
            let budgetMode;
            let refusedSet = new Set();
            let budgetCellScope = 'session';
            ...
            if (wsBudget !== undefined) {
                ...
                try {
                    ...
                    process.stderr.write(`LailatulCoder serve: getMcpClientAccounting failed: ` +
                        `${err instanceof Error ? err.message : String(err)}\n`);
                }
            }
            ...
            return {
                ...
                servers: await Promise.all(Object.entries(servers).map(async ([name, server]) => {
                    ...
                    const requiresAuth = rawStatus !== MCPServerStatus.CONNECTED &&
                    ...
                    if (isGatedMcpScope(server.scope)) {
                        const approvalState = approvals.getState(config.getWorkingDir(), name, server);
                    ...
                    const description = server && typeof server === 'object'
                        ? server.description
                        : undefined;
                    const extensionName = server && typeof server === 'object'
                        ? server.extensionName
                        : undefined;
                    ...
                    const transient = config.getTopTierMcpServers?.()?.[name] !== undefined ||
                        config.getRuntimeMcpServers?.()[name] !== undefined;
                    const configOrigin = out.extensionName
                        ? 'extension'
                        : transient
                            ? 'runtime'
                    ...
                    if (server && typeof server === 'object') {
                        ...
                        if (Array.isArray(candidate.args)) {
                            const args = candidate.args.filter((arg) => typeof arg === 'string');
                            if (args.length > 0) {
                    ...
                    if (!disabled) {
                        out.resourceCount = this.resolveServerMcpResources(config, name).length;
                        out.promptCount = this.resolveServerMcpPrompts(config, name).length;
                    }
                    return out;
                })),
                ...
                ...(budgetMode !== undefined
                    ? {
                        ...
                        budgets: this.buildBudgetCells(clientCount ?? 0, clientBudget, budgetMode, Array.from(refusedSet).filter((n) => !config.isMcpServerDisabled(n)).length, budgetCellScope),
                    }
                    : {}),
                ...(this.workspaceMcpDiscoveryError
                    ? {
                        errors: [
                            this.errorCell('mcp', new Error(this.workspaceMcpDiscoveryError)),
                        ],
                    }
                    : {}),
            };
    ...
    async buildManagedWorkspaceMcpStatus() {
        const config = this.getWorkspaceMcpConfig();
        const status = await this.buildWorkspaceMcpStatus(config);
        if (config === this.config)
            return status;
        const runtimeNames = new Set(Object.keys(this.config.getRuntimeMcpServers?.() ?? {}));
        if (runtimeNames.size === 0)
            return status;
        const runtimeStatus = await this.buildWorkspaceMcpStatus(this.config);
        const errors = [
            ...(status.errors ?? []),
            ...(runtimeStatus.errors ?? []).filter((candidate) => !status.errors?.some((existing) => existing.kind === candidate.kind &&
        ...
        return {
            ...
            servers: [
                ...
                ...runtimeStatus.servers.filter((server) => runtimeNames.has(server.name)),
            ],
            ...(errors.length > 0 ? { errors } : {}),
        };
    }
    buildWorkspaceMcpToolsStatus(config, serverName) {
        ...
        try {
            ...
            if (allTools.filter((t) => t instanceof DiscoveredMCPTool && t.serverName === serverName).length === 0) {
                for (const session of this.getActiveSessions()) {
                    ...
                    if (sessionTools.some((t) => t instanceof DiscoveredMCPTool && t.serverName === serverName)) {
            ...
            const tools = allTools
                .filter((tool) => tool instanceof DiscoveredMCPTool && tool.serverName === serverName)
                .map((tool) => {
                const invalidReasons = [];
                if (!tool.name)
                    invalidReasons.push('missing name');
                if (!tool.description)
                    invalidReasons.push('missing description');
    ...
    resolveServerMcpResources(config, serverName) {
        ...
        for (const session of this.getActiveSessions()) {
            try {
                const sessionResources = session
                    .getConfig()
                    .getResourceRegistry?.()
     ...
     */
    buildBudgetCells(liveCount, budget, mode, refusedCount, scope = 'session') {
        ...
        if (refusedCount > 0) {
            ...
            status = 'warning';
            hint = `Live MCP clients are above ${Math.round(MCP_BUDGET_WARN_FRACTION * 100)}% of the configured budget.`;
        }
        const cell = {
            kind: 'mcp_budget',
        ...
        if (budget !== undefined)
            cell.budget = budget;
        if (errorKind)
            cell.errorKind = errorKind;
        if (hint)
            cell.hint = hint;
        return [cell];
    }
    /** Map core `McpBudgetMode` to protocol `ServeMcpBudgetMode`. */
    coerceBudgetMode(mode) {
        return mode;
    }
    errorCell(kind, error, errorKind) {
    ...
    async buildWorkspaceSkillsStatus(config) {
        ...
        try {
            ...
            const disablements = new Map(Array.from(config.getDisabledSkillNames(), (name) => {
            ...
            const skillsByKey = new Map(skills.map((skill) => [
            ...
            for (const extension of config.getExtensions()) {
                ...
                for (const skill of extension.skills ?? []) {
                    ...
                    if (skillsByKey.has(`extension:${extension.name}:${skill.name}`) ||
                        skillsByKey.has(key)) {
                        continue;
                    }
                    skillsByKey.set(key, mapSkillConfigToStatus({
                        ...skill,
                        level: 'extension',
                        extensionName,
                    }, disablements, { disabled: true }));
                }
            }
            return {
                v: STATUS_SCHEMA_VERSION,
                workspaceCwd: this.workspaceCwd(config),
                initialized: true,
                skills: Array.from(skillsByKey.values()).sort((a, b) => a.name.localeCompare(b.name)),
            };
        }
        catch (error) {
            return {
    ...
    buildWorkspaceProvidersStatus(config) {
        try {
            ...
            const currentAcpModelId = hasCurrentModel
                ? getCurrentAcpModelId(modelOptions, currentModelId, currentAuth, activeRuntimeSnapshot
                    ? undefined
    ...
    async buildAcpPreflightCells(config) {
        ...
        try {
            ...
            if (!authType) {
                return this.acpCell('auth', {
                    ...
                    hint: 'Run `LailatulCoder` and complete the auth flow, or set a provider env var.',
                    detail: { source: 'none', hasToken: false },
                });
            }
            const apiKeyVars = AUTH_PREFLIGHT_ENV_KEYS[String(authType)] ?? [];
            const presentVar = apiKeyVars.find((name) => Boolean(process.env[name]));
            ...
            // No env-var registration → either OAuth-style auth (LailatulCoder-oauth) or
    ...
    buildMcpDiscoveryPreflightCell(config) {
        try {
    ...
    async buildSkillsPreflightCell(config) {
    ...
    buildProvidersPreflightCell(config) {
        try {
            const models = config
    ...
    buildToolRegistryPreflightCell(config) {
        try {
            const registry = config.getToolRegistry();
            if (!registry) {
    ...
    buildWorkspaceToolsStatus(config) {
        const workspaceCwd = this.safeWorkspaceCwd(config);
        try {
            const registry = config.getToolRegistry();
            if (!registry) {
    ...
    async refreshLiveSessionMemoryInstructions(logContext) {
        const sessions = [...this.sessions.values()];
        if (sessions.length === 0) {
            return;
        }
        await Promise.all(sessions.map((session) => refreshMemoryInstruction(session.getConfig(), {
    ...
    async buildSessionContextUsageStatus(sessionId, showDetails) {
        const session = this.sessionOrThrow(sessionId);
        const config = session.getConfig();
        let usage;
        try {
    ...
    buildSessionTasksStatus(sessionId) {
        const session = this.sessionOrThrow(sessionId);
        return buildSessionTasksStatus(sessionId, session.getConfig());
    }
    buildSessionLspStatus(sessionId) {
        const session = this.sessionOrThrow(sessionId);
        const config = session.getConfig();
    ...
    buildSessionStatsStatus(sessionId) {
        const session = this.sessionOrThrow(sessionId);
        const config = session.getConfig();
    ...
    buildWorkspaceHooksStatus(config) {
        try {
            const workspaceCwd = this.workspaceCwd(config);
            ...
            const hooks = registryEntries.map((entry) => ({
                kind: 'hook',
    ...
    buildSessionHooksStatus(sessionId) {
        const session = this.sessionOrThrow(sessionId);
        const config = session.getConfig();
        try {
            const workspaceCwd = this.workspaceCwd(config);
            ...
            const hooks = sessionHooks.map((entry) => ({
                kind: 'hook',
                eventName: entry.eventName,
                config: this.serializeHookConfig(entry.config),
                source: 'session',
    ...
    buildWorkspaceExtensionsStatus(config) {
        try {
            const workspaceCwd = this.workspaceCwd(config);
            ...
            const entries = extensions.map((ext) => {
                const capabilities = {
                    ...
                    hookCount: ext.hooks
                        ? Object.values(ext.hooks).reduce((sum, defs) => sum + (defs?.length ?? 0), 0)
                        : 0,
    ...
    async installSkillFromUrl(request) {
        const skillManager = this.config.getSkillManager();
        if (!skillManager) {
            throw RequestError.invalidParams(undefined, 'SkillManager is not available');
        }
        ...
        const parsed = skillManager.parseSkillContent(download.skillContent, skillFile, 'user');
        if (parsed.name !== request.slug) {
            throw RequestError.invalidParams(undefined, `Skill name "${parsed.name}" does not match requested slug "${request.slug}"`);
        }
    ...
    async deleteGlobalSkill(request) {
        const skillManager = this.config.getSkillManager();
        if (!skillManager) {
            throw RequestError.invalidParams(undefined, 'SkillManager is not available');
        }
        const { skillDir, skillFile, content } = await this.readManagedSkillFile(request.slug, 'global', skillManager);
        ...
        // never a filesystem root or the global LailatulCoder dir itself, so a malformed
        ...
        if (!isDedicatedSkillDir ||
            resolvedSkillDir === path.parse(resolvedSkillDir).root ||
            resolvedSkillDir === globalDir) {
            throw RequestError.invalidParams(undefined, `Refusing to delete unexpected skill directory: ${skillDir}`);
        }
        await fs.rm(skillDir, { recursive: true, force: true });
        await skillManager.refreshCache();
        return {
    ...
    async readManagedSkillFile(slug, scope, skillManager, cwd) {
        if (scope === 'global') {
            const qwenSkillDir = resolveManagedSkillDir(path.join(Storage.getGlobalQwenDir(), 'skills'), slug);
        ...
        if (scope === 'project' && cwd?.trim()) {
            const projectSkill = await this.findProjectSkillFileFromCwd(slug, cwd, skillManager);
            if (projectSkill)
                return projectSkill;
        }
        const level = scope === 'project' ? 'project' : 'user';
        const skill = (await skillManager.listSkills({ level })).find((candidate) => candidate.name === slug);
        const skillFile = skill?.filePath;
        if (!skillFile) {
            throw RequestError.invalidParams(undefined, `${scope === 'project' ? 'Project' : 'Global'} skill not found: ${slug}`);
        }
    ...
    async findProjectSkillFileFromCwd(slug, cwd, skillManager) {
        ...
        for (const configDir of PROJECT_SKILL_DIRS) {
            ...
            if (!skillFile)
                continue;
            const content = await fs.readFile(skillFile, 'utf8').catch(() => {
                throw RequestError.invalidParams(undefined, `Project skill not found: ${slug}`);
            });
            return {
                skillDir: path.dirname(skillFile),
                skillFile,
    ...
    async setGlobalSkillEnabled(request, cwd) {
        const skillManager = this.config.getSkillManager();
        if (!skillManager) {
            throw RequestError.invalidParams(undefined, 'SkillManager is not available');
        }
        const { skillFile, content } = await this.readManagedSkillFile(request.slug, request.scope, skillManager, cwd);
        ...
        if (path.basename(skillFile) !== 'SKILL.md') {
            throw RequestError.invalidParams(undefined, `Refusing to write to unexpected skill file: ${skillFile}`);
        }
    ...
    async extMethod(method, params) {
        try {
            ...
            const normalizedParams = typeof rawSessionId === 'string'
                ? {
                    ...params,
                    sessionId: normalizeSessionIdForLookup(rawSessionId),
                }
                : params;
            if (method === SERVE_CONTROL_EXT_METHODS.sessionBackgroundNotification &&
                this.privateParentState !== 'trusted') {
                throw RequestError.invalidParams(undefined, 'Background notifications require a trusted private ACP parent');
            }
    ...
    async extMethodInternal(method, params) {
        ...
        switch (method) {
            case PROMPT_CANCEL_METHOD: {
                ...
                const targetedCalls = new Set(this.activePromptCalls.get(sessionId) ?? []);
            ...
            case TODO_STOP_GUARD_QUEUE_RELEASE_METHOD: {
                ...
                if (typeof promptId !== 'string' || promptId.length === 0) {
                    throw RequestError.invalidParams(undefined, 'Invalid or missing promptId');
                }
                const session = this.sessions.get(sessionId);
                if (!session) {
                    throw RequestError.invalidParams(undefined, `Session not found for id: ${sessionId}`);
            ...
            case 'LailatulCoder/providers/list': {
                return {
                    providers: ALL_PROVIDERS.map((provider) => serializeProviderConfig(provider, this.settings)),
                };
            }
            case 'LailatulCoder/providers/connect': {
                const providerId = readRequiredString(params['providerId'], 'providerId');
                const providerConfig = findProviderById(providerId);
                if (!providerConfig) {
                    throw RequestError.invalidParams(undefined, `Unknown provider: ${providerId}`);
                }
                const inputs = readProviderSetupInputs(providerConfig, params, (protocol, baseUrl) => resolveExistingProviderApiKey(providerConfig, this.settings, protocol, baseUrl));
                ...
                await applyProviderInstallPlan(plan, {
                    settings: adapter,
                    reloadModelProviders: (modelProviders) => this.config.reloadModelProvidersConfig(modelProviders),
                    syncAuthState: (authType, modelId, baseUrl) => this.config
                        .getModelsConfig()
                        .syncAfterAuthRefresh(authType, modelId, baseUrl),
                    refreshAuth: (authType) => this.config.refreshAuth(authType),
                });
                const effectiveModelId = adapter.getValue('model.name') ??
                    plan.modelSelection?.modelId;
                const effectiveBaseUrl = adapter.getValue('model.baseUrl') ??
            ...
            case 'LailatulCoder/skills/install': {
                return this.installSkillFromUrl(readSkillInstallRequest(params));
            }
            case 'LailatulCoder/skills/delete': {
                return this.deleteGlobalSkill(readSkillSlugRequest(params));
            }
            case 'LailatulCoder/skills/setEnabled': {
                return this.setGlobalSkillEnabled(readSkillSetEnabledRequest(params), requestedCwd);
            }
            case 'LailatulCoder/settings/getMemory': {
            ...
            case 'LailatulCoder/settings/setMemory': {
                ...
                for (const key of QWEN_MEMORY_SETTING_KEYS) {
                    ...
                    if (typeof updates[key] !== 'boolean') {
                        throw RequestError.invalidParams(undefined, `Invalid memory setting '${key}': expected boolean`);
                    }
            ...
            case 'LailatulCoder/settings/getPath': {
                return { path: this.settings.user.path };
            }
            case 'LailatulCoder/settings/getMemoryPaths': {
                const projectRoot = typeof params['projectRoot'] === 'string'
                    ? params['projectRoot']
                    : cwd;
                return {
            ...
            case SERVE_STATUS_EXT_METHODS.workspaceMcp:
                return (await this.buildManagedWorkspaceMcpStatus());
            case SERVE_STATUS_EXT_METHODS.workspaceMcpTools: {
                const serverName = params['serverName'];
                if (typeof serverName !== 'string' || serverName.length === 0) {
                    throw RequestError.invalidParams(undefined, 'Invalid or missing serverName');
                }
                return this.buildWorkspaceMcpToolsStatus(this.getWorkspaceMcpConfig(serverName), serverName);
            }
            case SERVE_STATUS_EXT_METHODS.workspaceMcpResources: {
                ...
                return this.buildWorkspaceMcpResourcesStatus(this.getWorkspaceMcpConfig(serverName), serverName);
            }
            case SERVE_STATUS_EXT_METHODS.workspaceSkills:
                return (await this.buildWorkspaceSkillsStatus(this.config));
            case SERVE_STATUS_EXT_METHODS.workspaceTools:
                return this.buildWorkspaceToolsStatus(this.config);
            case SERVE_STATUS_EXT_METHODS.workspaceProviders:
                return this.buildWorkspaceProvidersStatus(this.config);
            case SERVE_STATUS_EXT_METHODS.workspacePreflight:
                return (await this.buildAcpPreflightCells(this.config));
            case SERVE_STATUS_EXT_METHODS.workspaceResource: {
                ...
                const cpuPercent = computeCpuPercent(this.prevChildCpu, cpu, now - this.prevChildCpuAt, this.childCpuCoreCount);
            ...
            case SERVE_STATUS_EXT_METHODS.sessionContextUsage: {
                ...
                return (await this.buildSessionContextUsageStatus(sessionId, params['detail'] === true));
            }
            case SERVE_STATUS_EXT_METHODS.sessionSupportedCommands: {
                ...
                return (await this.buildSessionSupportedCommandsStatus(sessionId));
            }
            case SERVE_STATUS_EXT_METHODS.sessionTasks: {
                ...
                return this.buildSessionTasksStatus(sessionId);
            }
            case SERVE_STATUS_EXT_METHODS.sessionLspStatus: {
                ...
                return this.buildSessionLspStatus(sessionId);
            }
            case SERVE_STATUS_EXT_METHODS.sessionTranscript: {
                ...
                if (rawCursor !== undefined && typeof rawCursor !== 'string') {
                    throw RequestError.invalidParams(undefined, 'Invalid transcript cursor');
                }
                const rawBeforeRecordId = params['beforeRecordId'];
                if (rawBeforeRecordId !== undefined &&
                    (typeof rawBeforeRecordId !== 'string' ||
                        rawBeforeRecordId.length === 0)) {
                    throw RequestError.invalidParams(undefined, 'Invalid transcript record boundary');
                }
                if (rawCursor !== undefined && rawBeforeRecordId !== undefined) {
                    throw RequestError.invalidParams(undefined, 'Transcript cursor and record boundary are mutually exclusive');
                }
                const rawDirection = params['direction'];
                if (rawDirection !== undefined && rawDirection !== 'backward') {
                    throw RequestError.invalidParams(undefined, 'Invalid transcript direction');
                }
                if (rawCursor !== undefined && rawDirection !== undefined) {
                    throw RequestError.invalidParams(undefined, 'Transcript cursor and direction are mutually exclusive');
                }
                if (rawBeforeRecordId !== undefined && rawDirection !== undefined) {
                    throw RequestError.invalidParams(undefined, 'Transcript record boundary and direction are mutually exclusive');
                }
                const rawLimit = params['limit'];
                if (rawLimit !== undefined &&
                    (!Number.isSafeInteger(rawLimit) ||
                        rawLimit < 1 ||
                        rawLimit > SESSION_TRANSCRIPT_MAX_LIMIT)) {
                    throw RequestError.invalidParams(undefined, 'Invalid transcript limit');
                }
                try {
                    ...
                    return await runWithAcpRuntimeOutputDir(settings, cwd, async () => {
                        ...
                        const replay = await replayTranscriptRecordPage({
                            ...
                            page,
                            config,
                            finalizeDangling: !activePromptBeforeRead &&
                                !this.activePromptCalls.has(sessionId),
                            encodeCursor: (state) => encodeSessionTranscriptCursor(state, cwd),
                            logger: debugLogger,
                        });
                        return {
                            v: 1,
                    ...
                    if (error instanceof InvalidSessionTranscriptCursorError ||
                        ...
                        throw new RequestError(-32602, error instanceof Error ? error.message : 'Invalid transcript', {
                            errorKind: error instanceof InvalidSessionTranscriptCursorError
                    ...
                    if (error.code === 'ENOENT') {
                        if (typeof rawCursor === 'string') {
                            throw new RequestError(-32010, `Transcript snapshot is unavailable for session ${sessionId}`, {
                                errorKind: 'transcript_snapshot_unavailable',
                                sessionId,
                            });
                        }
            ...
            case SERVE_STATUS_EXT_METHODS.sessionStats: {
                ...
                return this.buildSessionStatsStatus(sessionId);
            }
            case SERVE_STATUS_EXT_METHODS.sessionRewindSnapshots: {
                ...
                const results = await Promise.all(snapshots
                    .map((s, idx) => ({ s, idx }))
                    .filter(({ s }) => s.promptId.startsWith(prefix) &&
            ...
            case SERVE_STATUS_EXT_METHODS.workspaceHooks:
                return this.buildWorkspaceHooksStatus(this.config);
            case SERVE_STATUS_EXT_METHODS.sessionHooks: {
                const sessionId = params['sessionId'];
                if (typeof sessionId !== 'string' || sessionId.length === 0) {
                    throw RequestError.invalidParams(undefined, 'Invalid or missing sessionId');
                }
                return this.buildSessionHooksStatus(sessionId);
            }
            case SERVE_STATUS_EXT_METHODS.workspaceExtensions:
                return this.buildWorkspaceExtensionsStatus(this.config);
            case SERVE_CONTROL_EXT_METHODS.workspaceMemoryRememberAvailability:
                return {
                    available: !(this.managedToolInvocationGuard &&
                        this.externalToolGuardProviderAttached) && this.config.isManagedMemoryAvailable(),
                };
            case SERVE_CONTROL_EXT_METHODS.workspaceMemoryRemember: {
                this.rejectUnsupportedGuardedHiddenAgent('agent-backed workspace memory remember');
                const content = params['content'];
                if (typeof content !== 'string' || !content.trim()) {
                    throw RequestError.invalidParams(undefined, 'Invalid or missing content');
                }
                if (Buffer.byteLength(content, 'utf8') > MAX_REMEMBER_CONTENT_BYTES) {
                    throw RequestError.invalidParams(undefined, 'Content exceeds maximum size');
                }
                ...
                if (!this.config.isManagedMemoryAvailable()) {
                    throw new RequestError(-32009, 'Managed memory is unavailable for this daemon workspace', { errorKind: 'managed_memory_unavailable' });
                }
                const childSignal = AbortSignal.timeout(WORKSPACE_MEMORY_REMEMBER_CHILD_TIMEOUT_MS);
                ...
                try {
                    ...
                    if (result.filesTouched.length > 0) {
                        await this.refreshLiveSessionMemoryInstructions('workspace memory remember');
                    }
                    return result;
                }
                catch (err) {
                    ...
                    if (childSignal.aborted) {
                        ...
                        throw new RequestError(-32099, 'Workspace memory remember timed out', workspaceMemoryErrorData(timeoutCode, diagnostics));
                    }
                    ...
                    throw new RequestError(-32099, 'Workspace memory remember failed', workspaceMemoryErrorData(code, diagnostics));
                }
            }
            case SERVE_CONTROL_EXT_METHODS.workspaceMemoryForget: {
                const query = params['query'];
                if (typeof query !== 'string' || !query.trim()) {
                    throw RequestError.invalidParams(undefined, 'Invalid or missing query');
                }
                const trimmedQuery = query.trim();
                if (Buffer.byteLength(trimmedQuery, 'utf8') > MAX_REMEMBER_CONTENT_BYTES) {
                    throw RequestError.invalidParams(undefined, 'Query exceeds maximum size');
                }
                if (!this.config.isManagedMemoryAvailable()) {
                    throw new RequestError(-32009, 'Managed memory is unavailable for this daemon workspace', { errorKind: 'managed_memory_unavailable' });
                }
                ...
                try {
                    ...
                    if (childSignal.aborted) {
                        ...
                        throw new RequestError(-32099, 'Workspace memory forget timed out', workspaceMemoryErrorData(timeoutCode, diagnostics));
                    }
                    ...
                    throw new RequestError(-32099, 'Workspace memory forget failed', workspaceMemoryErrorData(code, diagnostics));
                }
            }
            case SERVE_CONTROL_EXT_METHODS.workspaceMemoryDream: {
                this.rejectUnsupportedGuardedHiddenAgent('agent-backed workspace memory dream');
                if (!this.config.isManagedMemoryAvailable()) {
                    throw new RequestError(-32009, 'Managed memory is unavailable for this daemon workspace', { errorKind: 'managed_memory_unavailable' });
                }
                const childSignal = AbortSignal.timeout(WORKSPACE_MEMORY_REMEMBER_CHILD_TIMEOUT_MS);
                ...
                try {
                    projectRoot = this.config.getProjectRoot();
                    const result = await runManagedAutoMemoryDream(projectRoot, new Date(), createHiddenWorkspaceMemoryConfig(this.config), childSignal, {
                        trigger: 'manual',
                        recordMetadata: true,
                    ...
                    if (childSignal.aborted) {
                        ...
                        throw new RequestError(-32099, 'Workspace memory dream timed out', workspaceMemoryErrorData(timeoutCode, diagnostics));
                    }
                    ...
                    throw new RequestError(-32099, 'Workspace memory dream failed', workspaceMemoryErrorData(code, diagnostics));
                }
            }
            case SERVE_CONTROL_EXT_METHODS.workspaceMcpRestart: {
                ...
                if (rawEntryIndex !== undefined && rawEntryIndex !== '*') {
                    if (typeof rawEntryIndex !== 'number' ||
                        !Number.isInteger(rawEntryIndex) ||
                        rawEntryIndex < 0) {
                        throw RequestError.invalidParams(undefined, 'entryIndex must be a non-negative integer or "*"');
                    }
                ...
                if (!Object.prototype.hasOwnProperty.call(servers, serverName)) {
                    ...
                    throw new RequestError(-32004, `MCP server not configured: ${JSON.stringify(serverName)}`, { errorKind: 'mcp_server_not_found', serverName });
                }
                ...
                try {
                    ...
                    if (mergedDisabled !== undefined && !Array.isArray(mergedDisabled)) {
                        process.stderr.write(`LailatulCoder serve: MCP restart for ${JSON.stringify(serverName)}: ` +
                ...
                const poolHasEntries = poolSnapshot !== undefined &&
                ...
                if (this.mcpPool && poolHasEntries) {
                    ...
                    await Promise.all(this.getLiveMcpConfigs(serverName).map(async (liveConfig) => {
                ...
                if (entryIndex !== undefined && entryIndex !== 0) {
                    throw RequestError.invalidParams(undefined, `entryIndex=${entryIndex} requested but pool not active for ` +
                        `${JSON.stringify(serverName)} — legacy single-entry path ` +
                        `only supports entryIndex=0 or undefined`);
                }
                const start = Date.now();
                await this.reconcileMcpServerAcrossLiveConfigs(serverName, 'discover');
                const disconnectedRuntime = this.getLiveMcpConfigs(serverName).find((liveConfig) => this.getMcpServerStatus(liveConfig, serverName) !==
                    MCPServerStatus.CONNECTED);
                if (disconnectedRuntime) {
                    const postStatus = this.getMcpServerStatus(disconnectedRuntime, serverName);
                    throw new RequestError(-32099, `MCP server ${JSON.stringify(serverName)} did not reach a ` +
            ...
            case SERVE_CONTROL_EXT_METHODS.workspaceMcpReload: {
                ...
                if (forceReconnectAll !== undefined &&
                    typeof forceReconnectAll !== 'boolean') {
                    throw RequestError.invalidParams(undefined, 'forceReconnectAll must be a boolean');
                }
                if (forceReconnectWhich !== undefined &&
                    (!Array.isArray(forceReconnectWhich) ||
                        forceReconnectWhich.some((serverName) => typeof serverName !== 'string' || serverName.length === 0))) {
                    throw RequestError.invalidParams(undefined, 'forceReconnectWhich must be an array of server names');
                }
                if (forceReconnectAll === true && forceReconnectWhich !== undefined) {
                    throw RequestError.invalidParams(undefined, 'forceReconnectAll and forceReconnectWhich cannot be used together');
                }
                return this.reloadWorkspaceMcpDiscovery({
                    forceReconnectAll: forceReconnectAll === true,
                    forceReconnectWhich: forceReconnectWhich,
                });
            }
            case SERVE_CONTROL_EXT_METHODS.workspaceMcpManage: {
                ...
                if (action !== 'approve' &&
                    ...
                    action !== 'authenticate' &&
                    action !== 'clear-auth') {
                    throw RequestError.invalidParams(undefined, 'Invalid or missing MCP manage action');
                }
                const config = this.getWorkspaceMcpConfig(serverName);
                const servers = config.getMcpServers() ?? {};
                const server = servers[serverName];
                ...
                if (!toolRegistry) {
                    throw RequestError.internalError(undefined, 'ToolRegistry unavailable on this Config');
                }
                if (action === 'approve') {
                    if (!isGatedMcpScope(server.scope)) {
                        throw RequestError.invalidParams(undefined, `MCP server is not approval-gated: ${serverName}`);
                    }
                    const approvals = loadMcpApprovals();
                    await approvals.setState(config.getWorkingDir(), serverName, server, 'approved');
                ...
                if (action === 'enable') {
                    ...
                    for (const scope of [SettingScope.User, SettingScope.Workspace]) {
                        ...
                        const filtered = currentExcluded.filter((pattern) => pattern !== serverName);
                    ...
                    for (const liveConfig of this.getLiveMcpConfigs(serverName)) {
                        ...
                        const runtimeFiltered = currentExcluded.filter((pattern) => pattern !== serverName);
                ...
                if (action === 'disable') {
                    ...
                    if (server.extensionName) {
                        throw RequestError.invalidParams(undefined, `Cannot disable extension MCP server: ${serverName}`);
                    }
                ...
                if (action === 'clear-auth') {
                    ...
                    await this.reconcileMcpServerAcrossLiveConfigs(serverName, 'disconnect');
                    return { serverName, action, ok: true, changed: true };
                }
                ...
                if (!pending) {
                    ...
                    const started = new Promise((resolve, reject) => {
                        ...
                        startedTimer = setTimeout(() => rejectStarted(new Error(`MCP OAuth authentication did not provide a URL within ${MCP_OAUTH_START_TIMEOUT_MS / 1000} seconds`)), MCP_OAUTH_START_TIMEOUT_MS);
                        startedTimer.unref();
                    });
                    pending = { started };
                    ...
                    void (async () => {
                        try {
                            try {
                                ...
                                rejectStarted(error);
                                debugLogger.warn(`MCP OAuth authentication failed for ${serverName}:`, error);
                                return;
                            }
                            try {
                                await this.reconcileMcpServerAcrossLiveConfigs(serverName, 'discover');
                                ...
                                debugLogger.warn(`MCP OAuth authenticated for ${serverName}, but tool synchronization failed:`, error);
                            }
                        }
                        finally {
            ...
            case SERVE_CONTROL_EXT_METHODS.workspaceGenerationStart: {
                ...
                if (typeof requestId !== 'string' || requestId.length === 0) {
                    throw RequestError.invalidParams(undefined, 'Invalid or missing requestId');
                }
                if (typeof prompt !== 'string' ||
                    !prompt.trim() ||
                    Buffer.byteLength(prompt, 'utf8') > GENERATION_MAX_PROMPT_BYTES ||
                    params['purpose'] !== 'text') {
                    throw RequestError.invalidParams(undefined, 'Invalid workspace generation request');
                }
                if (this.workspaceGenerationControllers.has(requestId)) {
                    throw RequestError.invalidParams(undefined, 'Duplicate workspace generation requestId');
                }
                ...
                try {
                    const emit = async (event) => {
                        await this.connection.extNotification('LailatulCoder/notify/workspace/generation/event', { v: 1, requestId, event });
                    };
                    const result = await executeGeneration(this.config, requestId, prompt.trim(), signal, emit);
            ...
            case SERVE_CONTROL_EXT_METHODS.workspaceAgentGenerate: {
                ...
                if (typeof description !== 'string' ||
                    ...
                    throw RequestError.invalidParams(undefined, 'Invalid or missing description (max 4096 chars)');
                }
                ...
                return (await subagentGenerator(description.trim(), this.config, AbortSignal.timeout(5 * 60_000)));
            }
            ...
            case SERVE_CONTROL_EXT_METHODS.sessionArtifactsPersist: {
                ...
                if (kind !== 'event' && kind !== 'snapshot') {
                    throw RequestError.invalidParams(undefined, 'Invalid or missing artifact persist kind');
                }
                const payload = params['payload'];
                if (!payload || typeof payload !== 'object' || Array.isArray(payload)) {
                    throw RequestError.invalidParams(undefined, 'Invalid or missing artifact persist payload');
                }
                const session = this.sessionOrThrow(sessionId);
                ...
                if (kind === 'event') {
                    await recording.recordSessionArtifactEvent(parseSessionArtifactEventPayload(payload, sessionId));
                }
                else {
                    await recording.recordSessionArtifactSnapshot(parseSessionArtifactSnapshotPayload(payload, sessionId));
                }
                return { sessionId, persisted: true, kind };
            }
            case 'LailatulCoder/session/recordTextElements': {
                const sessionId = params['sessionId'];
                const content = params['content'];
                ...
                if (typeof content !== 'string' || !Array.isArray(textElements)) {
                    throw RequestError.invalidParams(undefined, 'Invalid user text elements payload');
                }
                const session = this.sessionOrThrow(sessionId);
                await session.assertCanStartTurn();
                const recording = session.getConfig().getChatRecordingService();
            ...
            case SERVE_CONTROL_EXT_METHODS.sessionTitle: {
                ...
                if (typeof displayName !== 'string') {
                    throw RequestError.invalidParams(undefined, 'Invalid or missing displayName');
                }
                ...
                const session = this.sessionOrThrow(sessionId);
                const source = titleSource === 'auto' ? 'auto' : 'manual';
                const recording = session.getConfig().getChatRecordingService();
                let ok = false;
                if (recording) {
            ...
            case SERVE_CONTROL_EXT_METHODS.sessionParent: {
                ...
                if (typeof parentSessionId !== 'string' ||
                    ...
                    throw RequestError.invalidParams(undefined, 'Invalid or missing parentSessionId');
                }
                const session = this.sessionOrThrow(sessionId);
                const recording = session.getConfig().getChatRecordingService();
                let ok = false;
            ...
            case SERVE_CONTROL_EXT_METHODS.sessionSource: {
                ...
                if ('error' in source || source.sourceType === undefined) {
                    throw RequestError.invalidParams(undefined, 'error' in source ? source.error : 'Invalid or missing sourceType');
                }
                const session = this.sessionOrThrow(sessionId);
                ...
                let ok = false;
                if (recording) {
                    ok = await recording.recordSessionSource(source.sourceType, source.sourceId);
                }
                return {
                    sessionId,
            ...
            case SERVE_CONTROL_EXT_METHODS.sessionLiveConversation: {
                ...
                if (typeof active !== 'boolean') {
                    throw RequestError.invalidParams(undefined, 'Invalid or missing active state');
                }
            ...
            case SERVE_CONTROL_EXT_METHODS.sessionLiveTranscript: {
                ...
                if (typeof model !== 'string' ||
                    model.length === 0 ||
                    model.length > 256) {
                    throw RequestError.invalidParams(undefined, 'Invalid or missing realtime model');
                }
                if (!Array.isArray(entries) || entries.length > 128) {
                    throw RequestError.invalidParams(undefined, 'Invalid realtime transcript entries');
                }
                const transcript = [];
                let totalLength = 0;
                for (const entry of entries) {
                    if (!entry || typeof entry !== 'object' || Array.isArray(entry)) {
                        throw RequestError.invalidParams(undefined, 'Invalid realtime transcript entry');
                    }
                    ...
                    if (totalLength > 131_072) {
                        throw RequestError.invalidParams(undefined, 'Realtime transcript is too large');
                    }
                    transcript.push({ role, text });
                }
                await this.sessionOrThrow(sessionId).appendLiveConversationTranscript(transcript, model);
            ...
            case SERVE_CONTROL_EXT_METHODS.sessionBackgroundNotification: {
                ...
                if (typeof displayText !== 'string' ||
                    ...
                    throw RequestError.invalidParams(undefined, 'Invalid or missing background notification displayText');
                }
                if (typeof modelText !== 'string' ||
                    modelText.length === 0 ||
                    modelText.length > 32_768) {
                    throw RequestError.invalidParams(undefined, 'Invalid or missing background notification modelText');
                }
                if (typeof taskId !== 'string' ||
                    taskId.length === 0 ||
                    taskId.length > 256) {
                    throw RequestError.invalidParams(undefined, 'Invalid or missing background notification taskId');
                }
                if (status !== 'completed' &&
                    status !== 'failed' &&
                    status !== 'cancelled') {
                    throw RequestError.invalidParams(undefined, 'Invalid background notification status');
                }
                if (kind !== 'agent') {
                    throw RequestError.invalidParams(undefined, 'Invalid background notification kind');
                }
                if (toolUseId !== undefined &&
                    (typeof toolUseId !== 'string' ||
                        toolUseId.length === 0 ||
                        toolUseId.length > 256)) {
                    throw RequestError.invalidParams(undefined, 'Invalid background notification toolUseId');
                }
                const session = this.sessionOrThrow(sessionId);
            ...
            case SERVE_CONTROL_EXT_METHODS.sessionClose: {
                ...
                if (rawDrainTimeoutMs !== undefined &&
                    (typeof rawDrainTimeoutMs !== 'number' ||
                        !Number.isSafeInteger(rawDrainTimeoutMs) ||
                        rawDrainTimeoutMs < 1 ||
                        rawDrainTimeoutMs > 2_147_483_647)) {
                    throw RequestError.invalidParams(undefined, 'Invalid session close drain timeout');
                }
            ...
            case SERVE_CONTROL_EXT_METHODS.sessionCd: {
                ...
                if (typeof targetPath !== 'string' ||
                    ...
                    throw RequestError.invalidParams(undefined, 'Invalid or missing path (must be an absolute path)');
                }
                const managedRelocation = params['managedRelocation'];
                if (managedRelocation !== undefined &&
                    managedRelocation !== 'live-conversation') {
                    throw RequestError.invalidParams(undefined, 'Invalid managed relocation capability');
                }
                ...
                if (managedRelocation === 'live-conversation') {
                    if (this.privateParentState !== 'trusted') {
                        throw RequestError.invalidParams(undefined, 'Live managed relocation requires a trusted private ACP parent');
                    }
                    if (!Array.isArray(allowedRoots) ||
                        ...
                        throw RequestError.invalidParams(undefined, 'Live managed relocation requires one absolute allowed root');
                    }
                    ...
                    try {
                        ...
                        throw new RequestError(-32004, 'Live managed relocation requires an owner-only allowed root', { errorKind: 'containment_violation', path: rootPath });
                    }
                    if (!isOwnerOnlyDirectory(rootBefore) ||
                    ...
                    try {
                        ...
                        throw new RequestError(-32004, 'Live managed relocation requires an owner-only direct child', { errorKind: 'containment_violation', path: canonicalPath });
                    }
                    const relativeTarget = path.relative(canonicalRoot, canonicalPath);
                    if (!isOwnerOnlyDirectory(targetBefore) ||
                    ...
                    try {
                        ...
                        throw new RequestError(-32004, 'Live managed relocation allowed root changed during validation', { errorKind: 'containment_violation', path: canonicalRoot });
                    }
                    if (!isOwnerOnlyDirectory(rootFinal) ||
                ...
                if (Array.isArray(containmentRoots) && containmentRoots.length > 0) {
                    ...
                    if (!contained) {
                        throw new RequestError(-32004, `Path outside allowed roots: ${canonicalPath}`, { errorKind: 'containment_violation', path: canonicalPath });
                    }
                }
                const previousCwd = config.getTargetDir();
                let trustValidated = false;
                const validateTrust = () => {
                    if (managedTrustAllowed ||
                    ...
                    if (trustedFolders.isPathTrusted(canonicalPath) !== true) {
                        throw new RequestError(-32001, `Directory not trusted: ${canonicalPath}`, { errorKind: 'directory_not_trusted', path: canonicalPath });
                    }
                    trustValidated = true;
                };
                ...
                const releaseGate = await beginSessionCloseAfterCurrentGate(session, SESSION_DRAIN_TIMEOUT_MS);
                try {
                    ...
                    await waitForSessionDrain(session.waitForActiveTurnsToSettle(), SESSION_DRAIN_TIMEOUT_MS, 'close');
                    ...
                    const warnings = [];
                    const relocation = await config.relocateWorkingDirectory(canonicalPath, canonicalPath, { skipProcessChdir: true, skipArtifactMigration: true });
                    ...
                    if (relocation.mcpRefreshError) {
                        warnings.push(`MCP refresh failed: ${relocation.mcpRefreshError instanceof Error
                    ...
                    try {
                        await config
                            .getGeminiClient()
                            ?.addWorkingDirectoryChangedContext(settledPreviousCwd, canonicalPath);
                    }
                    catch (error) {
            ...
            case SERVE_CONTROL_EXT_METHODS.sessionApprovalMode: {
                ...
                if (typeof mode !== 'string' ||
                    ...
                    throw RequestError.invalidParams(undefined, `Invalid approval mode; allowed: ${APPROVAL_MODES.join(', ')}`);
                }
                const session = this.sessionOrThrow(sessionId);
                const config = session.getConfig();
            ...
            case SERVE_CONTROL_EXT_METHODS.sessionLanguage: {
                ...
                if (typeof language !== 'string' ||
                    ...
                    throw RequestError.invalidParams(undefined, `Invalid language; must be one of: ${allowedLanguages.join(', ')}`);
                }
                const session = this.sessionOrThrow(sessionId);
                try {
                    ...
                    throw new RequestError(-32603, `Failed to switch UI language: ${err instanceof Error ? err.message : String(err)}`);
                }
                const resolvedLanguage = getCurrentLanguage();
                try {
                    this.settings.setValue(SettingScope.User, 'general.language', language);
                }
                catch (err) {
                ...
                if (syncOutputLanguage) {
                    ...
                    try {
                        writeOutputLanguageAndRegisterPath(settingValue, session.getConfig());
                        fileWriteOk = true;
                    }
                    catch (err) {
                    ...
                    if (fileWriteOk) {
                        try {
                            this.settings.setValue(SettingScope.User, 'general.outputLanguage', settingValue);
                        }
                        catch (err) {
                            debugLogger.warn('Failed to persist output language setting:', err);
                        }
                        const writtenPath = session.getConfig().getOutputLanguageFilePath() ??
                            getOutputLanguageFilePath();
                        const allSessions = [...this.sessions.values()];
                        const results = await Promise.allSettled(allSessions.map(async (s) => {
                            ...
                            try {
                                ...
                                debugLogger.warn(`Failed to write output-language.md for session ${s.getId()} (path=${sessionPath ?? 'global-default'}):`, err);
                            }
                        ...
                        if (failedCount > 0) {
                            debugLogger.warn(`Language refresh failed for ${failedCount}/${results.length} session(s)`);
                        }
            ...
            case SERVE_CONTROL_EXT_METHODS.sessionRecap: {
                ...
                const recap = await generateSessionRecap(config, new AbortController().signal);
                debugLogger.debug(`recap ext-method completed for session=${sessionId} result=${recap ? `len=${recap.length}` : 'null'}`);
            ...
            case SERVE_CONTROL_EXT_METHODS.sessionGenerationStart: {
                ...
                if (typeof sessionId !== 'string' ||
                    ...
                    Buffer.byteLength(prompt, 'utf8') > GENERATION_MAX_PROMPT_BYTES) {
                    throw RequestError.invalidParams(undefined, 'Invalid generation request');
                }
                if (this.generationControllers.has(requestId)) {
                    throw RequestError.invalidParams(undefined, 'Duplicate generation requestId');
                }
                const session = this.sessionOrThrow(sessionId);
                const controller = new AbortController();
                ...
                try {
                    const result = await executeGeneration(session.getConfig(), requestId, prompt, signal, async (event) => {
                        await this.connection.extNotification('LailatulCoder/notify/session/generation/event', { v: 1, sessionId, requestId, event });
                    });
            ...
            case SERVE_CONTROL_EXT_METHODS.sessionGenerationCancel: {
                ...
                if (typeof sessionId !== 'string' || typeof requestId !== 'string') {
                    throw RequestError.invalidParams(undefined, 'Invalid generation cancellation request');
                }
            ...
            case SERVE_CONTROL_EXT_METHODS.sessionBtw: {
                ...
                if (typeof question !== 'string' ||
                    ...
                    throw RequestError.invalidParams(undefined, `Invalid or missing question (max ${BTW_MAX_INPUT_LENGTH} chars)`);
                }
                const session = this.sessionOrThrow(sessionId);
                const config = session.getConfig();
                ...
                try {
                    ...
                    if (childSignal.aborted) {
                        throw RequestError.internalError(undefined, 'Side question timed out after 55s');
                    }
                    throw err;
                }
            ...
            case SERVE_CONTROL_EXT_METHODS.sessionForkAgent: {
                if (this.managedToolInvocationGuard &&
                    this.externalToolGuardProviderAttached) {
                    throw RequestError.invalidParams(undefined, 'Managed external tool guard v1 does not support /fork.');
                }
                const sessionId = params['sessionId'];
                if (typeof sessionId !== 'string' || sessionId.length === 0) {
                    throw RequestError.invalidParams(undefined, 'Invalid or missing sessionId');
                }
                const directive = typeof params['directive'] === 'string' ? params['directive'] : '';
                const trimmed = directive.trim();
                if (!trimmed) {
                    throw RequestError.invalidParams(undefined, 'Invalid or missing directive');
                }
                const session = this.sessionOrThrow(sessionId);
                const config = session.getConfig();
                if (!config.getModel()) {
                ...
                if (!hasHistory) {
                    throw RequestError.invalidParams(undefined, 'Cannot fork before the first conversation turn.');
                }
                const agentTool = config.getToolRegistry().getTool(ToolNames.AGENT);
                if (!agentTool) {
                    throw RequestError.invalidParams(undefined, 'The agent tool is unavailable; cannot fork.');
                }
                ...
                if (hasFailedDisplayStatus(result?.returnDisplay)) {
                    const reason = typeof result.llmContent === 'string' && result.llmContent.trim()
                        ? result.llmContent.trim()
                        : 'the background agent could not be started.';
                    throw RequestError.invalidParams(undefined, `Failed to launch fork: ${reason}`);
                }
                try {
                    config.getGeminiClient().addHistory({
                        ...
                        parts: [
                            {
                                text: `User launched a background fork via /fork. Directive (truncated): ${truncateForkDirectiveForHistory(trimmed)}`,
                            },
                        ],
                    });
                }
            ...
            case SERVE_CONTROL_EXT_METHODS.sessionShellHistory: {
                ...
                if (typeof command !== 'string') {
                    throw RequestError.invalidParams(undefined, 'Invalid or missing command');
                }
                const session = this.sessionOrThrow(sessionId);
                const config = session.getConfig();
                const geminiClient = config.getGeminiClient();
            ...
            case SERVE_CONTROL_EXT_METHODS.sessionTaskCancel: {
                ...
                if (typeof taskId !== 'string' || taskId.length === 0) {
                    throw RequestError.invalidParams(undefined, 'Invalid or missing taskId');
                }
                const taskKind = params['taskKind'];
                if (taskKind !== 'agent' &&
                    taskKind !== 'shell' &&
                    taskKind !== 'monitor') {
                    throw RequestError.invalidParams(undefined, 'taskKind must be "agent", "shell", or "monitor"');
                }
                debugLogger.info(`sessionTaskCancel requested sessionId=${sessionId} taskId=${taskId} taskKind=${taskKind}`);
                const session = this.sessionOrThrow(sessionId);
                const config = session.getConfig();
                switch (taskKind) {
                    case 'agent': {
                        ...
                        if (!task ||
                            ...
                            debugLogger.info(`sessionTaskCancel skipped sessionId=${sessionId} taskId=${taskId} taskKind=${taskKind} reason=${reason} status=${task?.status ?? 'missing'}`);
                        ...
                        debugLogger.info(`sessionTaskCancel completed sessionId=${sessionId} taskId=${taskId} taskKind=${taskKind} status=${task.status}`);
            ...
            case SERVE_CONTROL_EXT_METHODS.sessionGoalClear: {
                ...
                try {
                    ...
                    debugLogger.info(`sessionGoalClear sessionId=${sessionId} cleared=false (goal persistence unavailable)`);
                    return {
                        cleared: false,
                        condition: undefined,
                ...
                const response = goal
                    ...
                    : { snapshot: before };
                debugLogger.info(`sessionGoalClear sessionId=${sessionId} cleared=${!!goal} objective=${goal?.objective ?? '(none)'}`);
                return {
                    cleared: !!goal,
            ...
            case SERVE_CONTROL_EXT_METHODS.sessionGoalControl: {
                ...
                if (!request) {
                    throw RequestError.invalidParams(undefined, 'Invalid or missing Goal control request');
                }
                const session = this.sessionOrThrow(sessionId);
                const response = await dispatchGoalControl(session.getConfig(), request);
            ...
            case SERVE_CONTROL_EXT_METHODS.sessionGoalGet: {
                ...
                let snapshot;
                try {
                    snapshot = (await session.getConfig().getGoalRuntimeReady()).getSnapshot();
                }
                catch (error) {
                    if (!(error instanceof GoalPersistenceUnavailableError))
                        throw error;
                ...
                const activeGoal = snapshot.goal?.status === 'active' ? snapshot.goal : null;
                return {
                    snapshot,
                    active: activeGoal
                        ? {
            ...
            case SERVE_CONTROL_EXT_METHODS.sessionTurnStatus: {
                ...
                return await runWithAcpRuntimeOutputDir(settings, cwd, async () => {
                    ...
                    try {
                        ...
                        const turnResult = await findSettledTurnResult(reader, sessionId, typeof rawPromptId === 'string' ? rawPromptId : undefined, cwd);
                        return {
                            v: 1,
                            sessionId,
                        ...
                        if (error instanceof SessionTranscriptSnapshotUnavailableError &&
                            reader) {
                            try {
                                const transcript = await fs.stat(reader.getSessionFilePath(sessionId));
            ...
            case SERVE_CONTROL_EXT_METHODS.sessionContinue: {
                ...
                debugLogger.info(`sessionContinue sessionId=${sessionId} accepted=${result.accepted} interruption=${result.interruption}`);
                return result;
            }
            case SERVE_CONTROL_EXT_METHODS.workspaceMcpRuntimeAdd: {
                const request = readRuntimeMcpAddRequest(params);
                const result = await addRuntimeMcpServer(getRuntimeMcpManager(this.config), request);
                await Promise.all(this.getActiveSessions().map(async (session) => {
                    ...
                    if (!sessionManager)
                        return;
                    try {
                        await sessionManager.addRuntimeMcpServer(request.name, request.config, request.runtimeClientId);
                    }
                    catch (sessionErr) {
                        debugLogger.warn(`workspaceMcpRuntimeAdd: failed to add runtime MCP server ` +
            ...
            case SERVE_CONTROL_EXT_METHODS.workspaceMcpRuntimeRemove: {
                const request = readRuntimeMcpRequest(params);
                const result = await getRuntimeMcpManager(this.config).removeRuntimeMcpServer(request.name, request.runtimeClientId);
                ...
                await Promise.all(this.getActiveSessions().map(async (session) => {
                    ...
                    if (!sessionManager)
                        return;
                    try {
                        await sessionManager.removeRuntimeMcpServer(request.name, request.runtimeClientId);
                    }
                    catch (sessionErr) {
                        debugLogger.warn(`workspaceMcpRuntimeRemove: failed to remove runtime MCP server ` +
            ...
            case SERVE_CONTROL_EXT_METHODS.sessionMcpRuntimeAdd: {
                ...
                const session = this.sessionOrThrow(sessionId);
                return addRuntimeMcpServer(getRuntimeMcpManager(session.getConfig()), readRuntimeMcpAddRequest(params));
            }
            case SERVE_CONTROL_EXT_METHODS.sessionMcpRuntimeRemove: {
                ...
                const session = this.sessionOrThrow(sessionId);
                const request = readRuntimeMcpRequest(params);
                const result = await getRuntimeMcpManager(session.getConfig()).removeRuntimeMcpServer(request.name, request.runtimeClientId);
                return result;
            }
            case SERVE_CONTROL_EXT_METHODS.workspaceExtensionsRefresh: {
                const sessionId = params['sessionId'];
                const rawRefreshBootstrap = params['refreshBootstrap'];
                if (rawRefreshBootstrap !== undefined &&
                    typeof rawRefreshBootstrap !== 'boolean') {
                    throw RequestError.invalidParams(undefined, 'refreshBootstrap must be a boolean');
                }
                const session = this.sessionOrThrow(sessionId);
                const config = session.getConfig();
                const extensionManager = config.getExtensionManager();
                const errors = [];
                const runRefresh = async (refresh) => {
                    try {
                        await refresh();
                    }
                    catch (error) {
                ...
                if (rawRefreshBootstrap !== false && bootstrapConfig !== config) {
                    await runRefresh(async () => await this.refreshBootstrapExtensionStatus());
                }
                const discoveryConfig = this.workspaceMcpDiscoveryConfig;
                if (discoveryConfig && discoveryConfig !== config) {
                    const discoveryExtensionManager = discoveryConfig.getExtensionManager();
                    await runRefresh(async () => await discoveryExtensionManager.refreshCache());
                    await runRefresh(async () => await discoveryExtensionManager.refreshTools());
                }
                await runRefresh(async () => await config.getGeminiClient()?.refreshSystemInstruction());
                await runRefresh(async () => await session.sendAvailableCommandsUpdate());
                if (errors.length > 0) {
                    const details = errors
                        .map((error) => error instanceof Error ? error.message : String(error))
                        .join('; ');
                    throw new AggregateError(errors, `Extension runtime refresh failed: ${details}`);
                }
                return { ok: true };
            }
            case 'deleteSession': {
            ...
            case SERVE_CONTROL_EXT_METHODS.sessionRewind: {
                ...
                const promptId = typeof rawPromptId === 'string' ? rawPromptId : undefined;
                let turnIndex = params['targetTurnIndex'];
                const resolveFromPromptId = promptId !== undefined &&
                ...
                if (resolveFromPromptId) {
                    ...
                    if (!/^\d+$/.test(suffix)) {
                        throw new RequestError(-32602, 'Invalid promptId: non-numeric turn suffix', { errorKind: 'invalid_rewind_target' });
                    }
                }
                else if (!Number.isInteger(turnIndex) || turnIndex < 0) {
                    throw RequestError.invalidParams(undefined, 'Invalid or missing targetTurnIndex');
                }
                return await this.runExclusiveHistoryMutation(sessionId, async () => {
                    if (resolveFromPromptId) {
                        ...
                        const fhs = session.getConfig().getFileHistoryService();
                        const snapshots = fhs.getSnapshots();
                        const snapshotIdx = snapshots.findIndex((s) => s.promptId === promptId);
                        if (snapshotIdx < 0) {
                            throw new RequestError(-32602, 'Snapshot not found for the given promptId', { errorKind: 'invalid_rewind_target' });
                        }
                    ...
                    let rewindResult;
                    let releaseHistoryMutation;
                    try {
                        rewindResult = session.rewindToTurn(turnIndex, {
                            rewindFiles,
                        });
                    ...
                    try {
                        ...
                        if (rewindFiles && promptId) {
                            ...
                            try {
                                ...
                                debugLogger.error(`[ACP] File-history rewind failed for session=${sessionId} promptId=${promptId}: ${reason}`);
                                filesFailed = [`file-history-rewind: ${reason}`];
                            }
                        }
                        let artifactSnapshot;
                        let artifactSnapshotUnavailable;
                        try {
                            const config = session.getConfig();
                            const recording = config.getChatRecordingService();
                            await recording?.flush();
                            const loadAuthoritative = () => config.getSessionService().loadSession(sessionId);
                            ...
                            debugLogger.warn(`[ACP] Failed to rebuild artifact snapshot after rewind for session=${sessionId}: ${reason}`);
                        }
                        return {
                            success: true,
                            historyBeforeRewind,
            ...
            case 'LailatulCoder/session/loadUpdates': {
                const sessionId = params['sessionId'];
                if (!sessionId || !SESSION_ID_RE.test(sessionId)) {
                    throw RequestError.invalidParams(undefined, 'Invalid or missing sessionId');
                }
                ...
                if (liveSession) {
                    ...
                    await this.assertLiveSessionScope(config, loadSettingsCached(cwd), cwd);
                    const recording = config.getChatRecordingService();
                    const loadAuthoritative = () => config.getSessionService().loadSession(sessionId);
                    ...
                    const settings = loadSettingsCached(cwd);
                    sessionData = await this.runWithPinnedRuntimeBaseDir(settings, cwd, async () => {
                        const sessionService = new SessionService(cwd);
            ...
            case 'restoreSessionHistory': {
                ...
                if (!Array.isArray(history)) {
                    throw RequestError.invalidParams(undefined, 'Invalid or missing history');
                }
                const session = this.sessions.get(sessionId);
                if (!session) {
                    throw RequestError.invalidParams(undefined, `Session not found for id: ${sessionId}`);
            ...
            case SERVE_CONTROL_EXT_METHODS.sessionSideTask: {
                ...
                if (isSideTask && atRecordId !== undefined) {
                    throw RequestError.invalidParams(undefined, 'atRecordId is not supported for side tasks');
                }
                ...
                if (!isSideTask) {
                    if (!sourceSession.isTurnIdle()) {
                        throw new RequestError(-32602, 'Cannot branch while a prompt is running', { errorKind: 'session_busy' });
                    }
                    try {
                        return await this.runExclusiveHistoryMutation(sessionId, async () => {
                            await sourceSession.assertCanStartTurn();
                            const releaseHistoryMutation = sourceSession.beginHistoryMutation();
                            try {
                                ...
                                const fork = () => sessionService.forkSession(sessionId, newSessionId, {
                                    title,
            ...
            case 'LailatulCoder/settings/getCore': {
                const settings = loadSettings(cwd);
                this.settings = settings;
                return this.buildCoreSettings(settings, cwd);
            }
            case 'LailatulCoder/settings/setCoreValue': {
                const key = params['key'];
                if (typeof key !== 'string' ||
                    !QWEN_CORE_SETTING_KEYS.includes(key)) {
                    throw RequestError.invalidParams(undefined, 'Unsupported LailatulCoder setting key');
                }
                const settings = loadSettings(cwd);
                const settingKey = key;
                const normalizedValue = normalizeCoreSettingValue(settingKey, params['value']);
                ...
                if (settingKey === 'general.outputLanguage' &&
            ...
            case 'LailatulCoder/settings/setMcpServer': {
                const name = params['name'];
                if (typeof name !== 'string' || !name.trim()) {
                    throw RequestError.invalidParams(undefined, 'MCP server name is required');
                }
                const settings = loadSettings(cwd);
                ...
                const mcpServers = {
                    ...existingServers,
                    [name.trim()]: toStoredMcpServerConfig(restoreRedactedMcpSecrets(normalizeMcpServerConfig(params['server']), toRecord(existingServers[name.trim()]))),
                };
            ...
            case 'LailatulCoder/settings/removeMcpServer': {
                const name = params['name'];
                if (typeof name !== 'string' || !name.trim()) {
                    throw RequestError.invalidParams(undefined, 'MCP server name is required');
                }
            ...
            case 'LailatulCoder/settings/setHook': {
                ...
                const isReplace = typeof index === 'number' &&
                ...
                const hook = restoreRedactedHookSecrets(incomingHook, isReplace ? toRecord(eventHooks[index]) : {});
                if (isReplace) {
            ...
            case 'LailatulCoder/settings/removeHook': {
                const event = params['event'];
                if (!isHookEvent(event)) {
                    throw RequestError.invalidParams(undefined, 'Invalid hook event');
                }
                ...
                if (index >= eventHooks.length) {
                    throw RequestError.invalidParams(undefined, `Hook index ${index} out of range (event has ${eventHooks.length} hooks)`);
                }
            ...
            case 'LailatulCoder/settings/setExtensionSetting': {
                const extensionId = params['extensionId'];
                const settingKey = params['settingKey'];
                const value = params['value'];
                if (typeof extensionId !== 'string' || !extensionId) {
                    throw RequestError.invalidParams(undefined, 'extensionId is required');
                }
                ...
                const extScope = toSettingsScope(params['scope']) === SettingScope.Workspace
                    ? ExtensionSettingScope.WORKSPACE
                    : ExtensionSettingScope.USER;
                await updateSetting(extension.config, extension.id, settingKey, async () => value, extScope);
            ...
            case 'LailatulCoder/permissions/setRules': {
                ...
                if (ruleType !== 'allow' && ruleType !== 'ask' && ruleType !== 'deny') {
                    throw RequestError.invalidParams(undefined, 'ruleType must be "allow", "ask", or "deny"');
                }
                const settings = this.loadPermissionSettings(cwd);
                const before = readPermissionRuleSet(settings.merged);
                const settingScope = scope === 'workspace' ? SettingScope.Workspace : SettingScope.User;
                const scopeSettings = scope === 'workspace'
            ...
            case SERVE_CONTROL_EXT_METHODS.workspaceReload: {
                ...
                const envChanged = envResult.updatedKeys.length > 0 || envResult.removedKeys.length > 0;
                const sessions = [...this.sessions.entries()];
                const refreshed = [];
                const skipped = [];
                const results = await Promise.allSettled(sessions.map(async ([id, session]) => {
                    if (!session.isIdle()) {
                        skipped.push(id);
                        return;
                    }
                    ...
                    if (providersChanged) {
                        try {
                            config.reloadModelProvidersConfig(newMerged.modelProviders, newMerged.providerProtocol ?? {});
                        }
                        catch (err) {
                            debugLogger.warn(`reload: reloadModelProvidersConfig failed for session ${id}: ${err}`);
                        }
                    }
                    ...
                    if (changed.has('model') &&
                        ...
                        try {
                            ...
                            debugLogger.warn(`reload: switchModel failed for session ${id}: ${err}`);
                        }
                    }
                        ...
                        try {
                            ...
                            debugLogger.warn(`reload: refreshAuth failed for session ${id}: ${err}`);
                        }
                    }
                    if (changed.has('tools')) {
                        const disabled = normalizeDisabledToolList(newMerged.tools?.disabled);
                        config.setDisabledTools(new Set(disabled));
                        const newMode = newMerged.tools?.approvalMode;
                        if (newMode &&
                            APPROVAL_MODES.includes(newMode) &&
                            ...
                            try {
                                ...
                                debugLogger.warn(`reload: setApprovalMode failed for session ${id}: ${err}`);
                            }
                        }
                    }
                    try {
                ...
                for (let i = 0; i < results.length; i++) {
                    if (results[i].status === 'rejected') {
                        ...
                        debugLogger.warn(`Session ${sessions[i][0]} reload failed: ${reason}`);
            ...
            case SERVE_CONTROL_EXT_METHODS.workspaceSkillsRefresh: {
                ...
                if (rawReason !== undefined &&
                    ...
                    rawReason !== 'content' &&
                    rawReason !== 'all') {
                    throw RequestError.invalidParams(undefined, 'reason must be settings, content, or all');
                }
                ...
                const settingsReloadResults = refreshSettings
                    ? await Promise.allSettled(sessions.map((session) => Promise.resolve().then(() => session.reloadSkillSettings())))
                    : undefined;
                let configResults = [];
                if (refreshContent) {
                    const skillManagers = new Set([this.config, ...sessions.map((session) => session.getConfig())]
                        .map((config) => config.getSkillManager())
                        .filter((manager) => manager !== undefined));
                    configResults = await Promise.allSettled([...skillManagers].map((manager) => manager.refreshCache()));
                ...
                for (let i = 0; i < results.length; i++) {
                    if (results[i].status === 'rejected') {
                        const reason = results[i].reason;
                        debugLogger.warn(`Session ${sessions[i].getId()} skill refresh failed: ${reason}`);
                    }
                }
                return {
                    sessionsRefreshed: results.filter((result) => result.status === 'fulfilled').length,
                    sessionsFailed: results.filter((result) => result.status === 'rejected').length,
                    configsRefreshed: configResults.filter((result) => result.status === 'fulfilled').length,
                    configsFailed: configResults.filter((result) => result.status === 'rejected').length,
                    reason,
                };
            }
            default:
     ...
     * parent serve process via the `LailatulCoder/control/client_mcp/message` ext-method;
     ...
     */
    buildClientMcpSender(sessionId) {
        return (serverName, message) => deliverClientMcpMessage(this.connection, serverName, message, sessionId);
    }
    disposeTranscriptReplayConfig(config) {
        try {
            void Promise.resolve(config.getToolRegistry()?.stop()).catch((err) => {
                debugLogger.debug(`Transcript replay config tool registry stop failed: ${err instanceof Error ? err.message : String(err)}`);
            });
        }
        catch (err) {
            debugLogger.debug(`Transcript replay config tool registry stop failed: ${err instanceof Error ? err.message : String(err)}`);
    ...
    disposeTranscriptReplayConfigs() {
    ...
    async getTranscriptReplayConfig(cwd, settings) {
        ...
        const pending = this.newSessionConfig(cwd, [], settings, undefined, undefined, false, {
            skipMcpDiscovery: true,
            skipHooks: true,
            skipSkillManager: true,
            skipFileCheckpointing: true,
        ...
        try {
            ...
            if (current !== entry) {
                ...
                throw new Error('Transcript replay config was invalidated while loading');
            }
            entry.config = config;
    ...
    async newSessionConfig(cwd, mcpServers, settings, sessionSource, sessionId, resume, initializeOptions = {}, chatRecording, restoreOptions) {
        try {
            this.assertManagedSessionAdmission();
            return await this.runWithPinnedRuntimeBaseDir(settings, cwd, async () => {
                await this.retryPendingConfigCleanup(Storage.getRuntimeBaseDir(), sessionId);
                return this.newSessionConfigInRuntimeContext(cwd, mcpServers, settings, sessionSource, sessionId, resume, initializeOptions, chatRecording, restoreOptions);
            });
        }
        catch (error) {
    ...
    async newSessionConfigInRuntimeContext(cwd, mcpServers, settings, sessionSource, sessionId, resume, initializeOptions = {}, chatRecording, restoreOptions) {
        ...
        for (const server of mcpServers) {
            ...
            if (stdioServer) {
                ...
                sessionMcpServers[stdioServer.name] = new MCPServerConfig(stdioServer.command, stdioServer.args, env);
                continue;
            }
            ...
            if (sseServer) {
                ...
                sessionMcpServers[sseServer.name] = new MCPServerConfig(undefined, undefined, undefined, undefined, sseServer.url, undefined, Object.keys(headers).length > 0 ? headers : undefined);
                continue;
            }
            ...
            if (httpServer) {
                ...
                sessionMcpServers[httpServer.name] = new MCPServerConfig(undefined, undefined, undefined, undefined, undefined, httpServer.url, Object.keys(headers).length > 0 ? headers : undefined);
                continue;
            }
        }
        ...
        const config = await loadCliConfig(mergedSettings, argvForSession, cwd, 
        ...
        buildDisabledSkillNamesProvider(settings), sessionMcpServers, 
        ...
        true, this.managedToolInvocationGuard || restoreOptions
            ? {
                ...(this.managedToolInvocationGuard
                ...
                ...(restoreOptions && sessionId
                    ? {
                        sessionRestore: {
                            projectionSource: (restoreSessionId) => new SessionService(cwd, {
        ...
        try {
            ...
            return this.cleanupAfterRequestFailure(error, () => this.cleanupUnstoredConfig(config));
        }
        ...
        if (!initializeOptions.skipFileCheckpointing &&
        ...
        if (typeof this.config.getRuntimeMcpServers === 'function' &&
            typeof config.addRuntimeMcpServer === 'function') {
            const bootstrapRuntimeMcpServers = this.config.getRuntimeMcpServers();
            for (const [runtimeServerName, runtimeServerConfig] of Object.entries(bootstrapRuntimeMcpServers)) {
        ...
        const wiredSessionId = typeof config.getSessionId === 'function'
            ? config.getSessionId()
            : undefined;
        ...
        if (!skipPerSessionBudgetCallback &&
            ...
            config.setMcpBudgetEventCallback((event) => {
                ...
                void this.connection
                    ...
                    .catch((err) => {
                    debugLogger.debug(`MCP budget extNotification dropped ` +
    ...
    async surfaceMcpFailuresWhenReady(config) {
        try {
            ...
            debugLogger.error(`MCP discovery readiness failed: ${err instanceof Error ? err.message : String(err)}`);
            return;
        }
        const failedMcpServers = typeof config.getFailedMcpServerNames === 'function'
    ...
    async ensureAuthenticated(config) {
        ...
        if (!selectedType) {
            throw RequestError.authRequired({ authMethods: pickAuthMethodsForAuthRequired() }, 'Use LailatulCoder Ai CLI to authenticate first.');
        }
        try {
    ...
    async createAndStoreSession(config, settings, sessionData, options = {}) {
        ...
        this.assertManagedSessionAdmission();
        if (this.sessions.has(sessionId)) {
            throw new RequestError(ACP_ERROR_CODES.INVALID_PARAMS, `Session ${sessionId} is already active.`, { errorKind: 'session_id_conflict', sessionId });
        }
        ...
        const session = new Session(sessionId, config, this.connection, settings, (operation) => this.runExclusiveHistoryMutation(sessionId, operation), () => this.activeWorkReporter?.notifyChanged());
        let published = false;
        try {
            ...
            if (options.replayHistory !== false &&
                ...
                await session.replayHistory(sessionData.conversation.messages, sessionData.historyGaps);
                ...
                try {
                    await session.publishRecoveredGoalState(sessionData.conversation.messages);
                }
                catch (error) {
                    debugLogger.debug(`Failed to publish recovered Goal state: ${error instanceof Error ? error.message : String(error)}`);
                }
            ...
            if (!published) {
                try {
                    ...
                    debugLogger.warn(`Failed to dispose unpublished session ${sessionId}: ${disposeError instanceof Error ? disposeError.message : String(disposeError)}`);
                }
                throw error;
            }
            try {
                ...
                await this.removeStoredSessionEntry(sessionId, session, [cleanupError], { shutdownConfig: false });
            }
            throw error;
        }
    }
    buildAvailableModels(config) {
        ...
        const activeRuntimeSnapshot = config.getActiveRuntimeModelSnapshot?.();
        const currentModelId = getCurrentAcpModelId(modelOptions, activeRuntimeSnapshot?.id ?? rawCurrentModelId, activeRuntimeSnapshot?.authType ?? currentAuthType, activeRuntimeSnapshot
            ? undefined
            : config.getCurrentModelRegistryBaseUrl?.());
    ...
    buildConfigOptions(config) {
        ...
        const reasoningEffortConfigOption = modelReasoning
            ? {
                id: 'reasoning_effort',
                ...
                type: 'select',
                currentValue: config.getContentGeneratorConfig().reasoning === false
                    ? ACP_REASONING_EFFORT_NONE
                    : modelReasoning.toggleOnly
                        ? ACP_REASONING_EFFORT_DEFAULT
                        : (modelReasoning.efforts.find((effort) => effort === currentModelEffort) ?? modelReasoning.defaultEffort),
                options: [
                    {
            ...
            : {
                ...
                options: [
                    ...
                    ...REASONING_EFFORT_TIERS.map((effort) => ({
                        value: effort,
                        name: ACP_REASONING_EFFORT_NAMES[effort],
                        description: 'Providers map or clamp the requested tier for the active model',
                    })),
                ],
            };
    ...
    buildSelectableModelOptions(config) {
        const currentAuthType = config.getAuthType();
        return buildAcpModelOptions(config
            .getAllConfiguredModels()
            .filter((model) => model.authType !== AuthType.NEVER_MATCH_THIS ||
            currentAuthType === AuthType.NEVER_MATCH_THIS));
    }
}
