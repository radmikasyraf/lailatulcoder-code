...
describe('built-in channel registry', () => {
    it('keeps an invalid built-in channel running without management metadata', async () => {
        ...
        expect(catalog.filter((entry) => entry.manageable).map((entry) => entry.type)).toEqual(['wecom', 'feishu', 'github', 'gitlab']);
        expect(stderr).toHaveBeenCalledWith(expect.stringContaining('Invalid management metadata in "dingtalk" channel: Channel field "settings" cannot be a required object.'));
    ...
    it('registers a nested property keyed "type" with management intact', async () => {
        ...
        const entry = (await supportedChannelCatalog()).find((candidate) => candidate.type === 'valid-nested-type-key');
        ...
        expect(entry?.fields.find((field) => field.key === 'senderPolicy')).toMatchObject({ default: 'pairing' });
    });
});
//# sourceMappingURL=channel-registry-builtins.test.js.map