 ...
 * them.
 */
export async function preloadRuntimeFetchModule() {
    if (undiciModule)
        return;
...
function requireUndici() {
    if (!undiciModule) {
        throw new Error('undici is not loaded yet; await preloadRuntimeFetchModule() before ' +
...
let resolvedProxyUrlForRuntimeFetch;
/**
 ...
 */
export function setResolvedProxyUrlForRuntimeFetch(proxyUrl) {
 ...
 */
export function getOrCreateMcpDispatcher(insecure = isTlsVerificationDisabled()) {
    if (resolvedProxyUrlForRuntimeFetch) {
        return getOrCreateSharedDispatcher(resolvedProxyUrlForRuntimeFetch, insecure);
    }
...
function buildFetchOptionsWithDispatcher(sdkType, proxyUrl) {
    ...
    try {
        ...
        // Dual logging: debugLogger writes to ~/.lailatulcoder/debug/ (for local debugging),
