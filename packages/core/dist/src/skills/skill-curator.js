...
import { getArchivedSkillsRoot, getProjectSkillsRoot, SKILL_FILE_NAME, } from './skill-paths.js';
 ...
 */
async function readRegularFileNoFollow(filePath, maxBytes) {
    const handle = await fs.open(filePath, noFollowReadFlags());
    try {
        const stat = await handle.stat();
        if (!stat.isFile()) {
            throw new Error(`Auto-skill curator refuses unsafe path ${filePath} (not a regular file).`);
        }
...
function getCuratorPaths(projectRoot) {
...
function parseState(raw, statePath) {
    ...
    for (const [directoryName, value] of Object.entries(rawSkills)) {
        if (!value || typeof value !== 'object' || Array.isArray(value)) {
            throw new Error(`Invalid auto-skill curator record for ${directoryName}.`);
        }
        const record = value;
        ...
        skills[directoryName] = {
            skillName,
            firstSeenAt: requireTimestamp(record['firstSeenAt'], `${directoryName}.firstSeenAt`),
            lastActivityAt: requireTimestamp(record['lastActivityAt'], `${directoryName}.lastActivityAt`),
            useCount: useCount,
            state,
            pinned: pinned ?? false,
            ...(record['lastUsedAt'] !== undefined
                ? {
                    lastUsedAt: requireTimestamp(record['lastUsedAt'], `${directoryName}.lastUsedAt`),
                }
                : {}),
            ...(record['archivedAt'] !== undefined
                ? {
                    archivedAt: requireTimestamp(record['archivedAt'], `${directoryName}.archivedAt`),
                }
                : {}),
        };
    }
...
async function readState(statePath) {
    ...
    // /dev/zero or a path outside .lailatulcoder/) or a FIFO would otherwise be followed,
    ...
    try {
        ...
        ({ content } = await readRegularFileNoFollow(statePath, MAX_STATE_FILE_BYTES));
    }
    catch (error) {
        if (isMissing(error))
            return emptyState();
...
function parseAutoSkillName(content) {
    const match = /^---[ \t]*\r?\n([\s\S]*?)\r?\n---[ \t]*(?:\r?\n|$)/.exec(content);
    if (!match)
        return undefined;
    const frontmatter = parseYaml(match[1]);
    if (frontmatter['source'] !== 'auto-skill')
        return undefined;
...
function isManagedDirectoryName(directoryName) {
    return (directoryName.startsWith(AUTO_SKILL_PREFIX) &&
...
async function readManagedSkill(root, directoryName) {
    ...
    try {
        ...
        const { content, stat: manifestStat } = await readRegularFileNoFollow(manifestPath, MAX_MANIFEST_BYTES);
...
async function scanManagedSkills(root) {
    let entries;
    try {
    ...
    const skills = await Promise.all(entries
        .filter((entry) => entry.isDirectory() && isManagedDirectoryName(entry.name))
        .map((entry) => readManagedSkill(root, entry.name)));
    return skills
        .filter((skill) => skill !== undefined)
        .sort((a, b) => a.directoryName.localeCompare(b.directoryName));
}
async function ensureSafeQwenRoot(paths) {
    await fs.mkdir(paths.lailatulcoderRoot, { recursive: true });
    const stat = await fs.lstat(paths.lailatulcoderRoot);
    if (!stat.isDirectory() || stat.isSymbolicLink()) {
        throw new Error(`Auto-skill curator refuses unsafe path ${paths.lailatulcoderRoot}.`);
    }
}
...
async function withCuratorLock(projectRoot, operation) {
...
function recordForSkill(state, skill, firstSeenAt = skill.modifiedAt) {
...
function lastActivityMs(skill, record, nowMs) {
    const mtimeMs = parseTimestamp(skill.modifiedAt) ?? 0;
    return Math.max(mtimeMs <= nowMs ? mtimeMs : 0, parseTimestamp(record.firstSeenAt) ?? 0, parseTimestamp(record.lastActivityAt) ?? 0, parseTimestamp(record.lastUsedAt) ?? 0);
}
function entryFor(skill, record, state, nowMs) {
    return {
        directoryName: skill.directoryName,
        skillName: skill.skillName,
        state,
        lastActivityAt: new Date(lastActivityMs(skill, record, nowMs)).toISOString(),
...
async function runLocked(paths, state, now) {
    ...
    try {
        for (const scanned of skills) {
            ...
            if (inactivityMs >= AUTO_SKILL_ARCHIVE_AFTER_MS) {
                const current = await readManagedSkill(paths.skillsRoot, scanned.directoryName);
                if (!current)
                    continue;
                const currentInactivityMs = nowMs - lastActivityMs(current, record, nowMs);
...
async function previewRun(projectRoot, now) {
    const paths = getCuratorPaths(projectRoot);
...
export async function runAutoSkillCurator(projectRoot, options = {}) {
    const now = options.now ?? new Date();
    if (options.dryRun)
        return previewRun(projectRoot, now);
    return withCuratorLock(projectRoot, async (paths) => runLocked(paths, await readState(paths.statePath), now));
}
export async function maybeRunAutoSkillCurator(projectRoot, now = new Date()) {
    const paths = getCuratorPaths(projectRoot);
...
export async function recordAutoSkillUsage(projectRoot, skill, now = new Date()) {
    ...
    return withCuratorLock(projectRoot, async (lockedPaths) => {
        const managed = await readManagedSkill(lockedPaths.skillsRoot, directoryName);
...
export async function setAutoSkillPinned(projectRoot, directoryName, pinned, now = new Date()) {
    if (!isManagedDirectoryName(directoryName)) {
        throw new Error(`Managed auto-skill not found: ${JSON.stringify(directoryName)}.`);
    }
...
export async function getAutoSkillCuratorStatus(projectRoot, now = new Date()) {
    const paths = getCuratorPaths(projectRoot);
    ...
    for (const skill of liveSkills) {
        ...
        const inactivityMs = nowMs - lastActivityMs(skill, record, nowMs);
        const effectiveState = record.pinned
            ? record.state === 'stale'
                ? 'stale'
                : 'active'
    ...
    for (const skill of archivedSkills) {
        if (liveNames.has(skill.directoryName))
            continue;
        const record = recordForSkill(state, skill);
...
export async function restoreArchivedAutoSkill(projectRoot, directoryName, now = new Date()) {
    await withCuratorLock(projectRoot, async (paths) => {
        if (!isManagedDirectoryName(directoryName)) {
            throw new Error(`Archived auto-skill not found: ${JSON.stringify(directoryName)}.`);
        }
        ...
        if (!archiveRootStat.isDirectory() || archiveRootStat.isSymbolicLink()) {
            throw new Error(`Auto-skill curator refuses unsafe path ${paths.archiveRoot}.`);
        }
        ...
        if (!archived) {
            ...
            throw new Error(directoryExists
        ...
        try {
            await fs.lstat(destination);
            throw new Error(`Cannot restore ${directoryName}: an active directory already exists.`);
        }
        catch (error) {
            if (!isMissing(error))
                throw error;
        ...
        try {
            ...
            try {
                ...
                throw new Error(`Rollback failed: ${rollbackError instanceof Error ? rollbackError.message : String(rollbackError)}`, { cause: error });
            }
            throw error;
        }
    });
