...
import { clearHeapSnapshotRateLimit, collectMemoryPressureSamples, formatMemoryDiagnostics, formatMemoryPressureSamples, getMemoryDiagnostics, isHighHeapPressure, writeMemoryHeapSnapshot, } from './memoryDiagnostics.js';
describe('memoryDiagnostics', () => {
    ...
    it('surfaces large non-heap memory gaps separately from V8 heap pressure', () => {
        ...
        expect(report).toContain('large tool results, buffers, or native allocations');
    });
    it('writes heap snapshots to a diagnostics directory with stable filenames', () => {
        ...
        expect(writtenPath).toBe(path.join(outputDir, `lailatul-coder-heap-${process.pid}-2026-05-15T12-00-00-000Z.heapsnapshot`));
    });
    it('refuses heap snapshots when estimated heap dump would leave little free disk', () => {
        const outputDir = path.join(os.tmpdir(), `qwen-memory-diagnostics-disk-${process.pid}`);
        fs.rmSync(outputDir, { recursive: true, force: true });
        try {
            expect(() => writeMemoryHeapSnapshot({
                outputDir,
                writeSnapshot: (filePath) => {
                    fs.writeFileSync(filePath, 'snapshot');
                    return filePath;
    ...
    it('rate-limits repeated heap snapshot writes in the same directory', () => {
        const outputDir = path.join(os.tmpdir(), `qwen-memory-diagnostics-rate-limit-${process.pid}`);
        fs.rmSync(outputDir, { recursive: true, force: true });
        try {
            const writeSnapshot = (filePath) => {
                fs.writeFileSync(filePath, 'snapshot');
                return filePath;
            };
    ...
    it('keeps only the newest heap snapshots after writing', () => {
        const outputDir = path.join(os.tmpdir(), `qwen-memory-diagnostics-cleanup-${process.pid}`);
        fs.rmSync(outputDir, { recursive: true, force: true });
        fs.mkdirSync(outputDir, { recursive: true });
        const oldSnapshot = path.join(outputDir, `lailatul-coder-heap-${process.pid}-2026-05-15T11-00-00-000Z.heapsnapshot`);
        const newerSnapshot = path.join(outputDir, `lailatul-coder-heap-${process.pid}-2026-05-15T11-30-00-000Z.heapsnapshot`);
    ...
    it('orders heap snapshot cleanup by modification time across process ids', () => {
        const outputDir = path.join(os.tmpdir(), `qwen-memory-diagnostics-mtime-cleanup-${process.pid}`);
        fs.rmSync(outputDir, { recursive: true, force: true });
        fs.mkdirSync(outputDir, { recursive: true });
        const newerLowPidSnapshot = path.join(outputDir, 'lailatul-coder-heap-9-2026-05-15T12-30-00-000Z.heapsnapshot');
        const olderHighPidSnapshot = path.join(outputDir, 'lailatul-coder-heap-12345-2026-05-15T12-00-00-000Z.heapsnapshot');
        fs.writeFileSync(newerLowPidSnapshot, 'newer');
        fs.writeFileSync(olderHighPidSnapshot, 'older');
        fs.utimesSync(olderHighPidSnapshot, new Date('2026-05-15T12:00:00.000Z'), new Date('2026-05-15T12:00:00.000Z'));
        fs.utimesSync(newerLowPidSnapshot, new Date('2026-05-15T12:30:00.000Z'), new Date('2026-05-15T12:30:00.000Z'));
        const writtenPath = writeMemoryHeapSnapshot({
            outputDir,
    ...
    it('uses filename timestamps to break equal-mtime cleanup ties', () => {
        const outputDir = path.join(os.tmpdir(), `qwen-memory-diagnostics-equal-mtime-cleanup-${process.pid}`);
        fs.rmSync(outputDir, { recursive: true, force: true });
        fs.mkdirSync(outputDir, { recursive: true });
        const olderSnapshot = path.join(outputDir, 'lailatul-coder-heap-1-2026-05-15T11-00-00-000Z.heapsnapshot');
        const newerSnapshot = path.join(outputDir, 'lailatul-coder-heap-1-2026-05-15T12-00-00-000Z.heapsnapshot');
    ...
    it('keeps successful heap snapshot writes when cleanup fails', () => {
        const outputDir = path.join(os.tmpdir(), `qwen-memory-diagnostics-cleanup-fail-${process.pid}`);
        fs.rmSync(outputDir, { recursive: true, force: true });
        fs.mkdirSync(outputDir, { recursive: true });
        const brokenSymlink = path.join(outputDir, 'lailatul-coder-heap-999-2026-05-15T11-00-00-000Z.heapsnapshot');
    ...
    it('removes partial heap snapshot files after a failed write', () => {
        const outputDir = path.join(os.tmpdir(), `qwen-memory-diagnostics-partial-${process.pid}`);
        fs.rmSync(outputDir, { recursive: true, force: true });
        let partialPath = '';
        try {
            expect(() => writeMemoryHeapSnapshot({
    ...
    it('creates heap snapshot directories and files with private permissions', () => {
        const outputDir = path.join(os.tmpdir(), `qwen-memory-diagnostics-private-${process.pid}`);
        fs.rmSync(outputDir, { recursive: true, force: true });
        const writtenPath = writeMemoryHeapSnapshot({
            outputDir,
            now: new Date('2026-05-15T12:00:00.000Z'),
    ...
    it('refuses heap snapshot writes when free disk space cannot be read', async () => {
        const outputDir = path.join(os.tmpdir(), `qwen-memory-diagnostics-statfs-fallback-${process.pid}`);
        fs.rmSync(outputDir, { recursive: true, force: true });
        vi.resetModules();
        ...
        const { writeMemoryHeapSnapshot: writeWithMockedFs } = await import('./memoryDiagnostics.js');
        try {
            expect(() => writeWithMockedFs({
                outputDir,
                now: new Date('2026-05-15T12:00:00.000Z'),
                writeSnapshot: (filePath) => {
                    fs.writeFileSync(filePath, 'snapshot');
    ...
    it('falls back to process heap total when V8 heap statistics are unavailable for snapshot sizing', async () => {
        const outputDir = path.join(os.tmpdir(), `qwen-memory-diagnostics-estimate-fallback-${process.pid}`);
        fs.rmSync(outputDir, { recursive: true, force: true });
        vi.resetModules();
        ...
        const { writeMemoryHeapSnapshot: writeWithMockedV8 } = await import('./memoryDiagnostics.js');
    ...
    it('marks V8 diagnostics as warning when heap statistics are unavailable', () => {
        ...
        expect(report).not.toContain('No immediate memory pressure signals detected.');
    });
...
//# sourceMappingURL=memoryDiagnostics.test.js.map