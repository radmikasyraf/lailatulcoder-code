...
export function normalizeStopHookBlockingCap(value) {
...
function parseStopHookBlockingCapEnv(value) {
    const parsed = Number(value);
    return Number.isInteger(parsed)
        ? normalizeStopHookBlockingCap(parsed)
        : DEFAULT_STOP_HOOK_BLOCK_CAP;
}
export function resolveStopHookBlockingCap(configValue) {
...
export function formatStopHookBlockingCapWarning(hookLabel, cap) {
...
export function appendStopHookBlockingCapWarning(text, warning) {
    if (!warning)
        return text;
    return text ? `${text}\n\n${warning}` : warning;
}
//# sourceMappingURL=stopHookCap.js.map