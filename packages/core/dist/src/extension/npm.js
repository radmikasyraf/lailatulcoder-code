...
function resolveNpmRedirectUrl(currentUrl, location) {
    try {
        return new URL(location, currentUrl);
    }
    catch {
        throw new Error(`Invalid npm redirect URL: ${redactUrlCredentials(location)}`);
    }
}
/**
 ...
 */
export function parseNpmPackageSource(source) {
    // Scoped package: @scope/name[@version]
    // First @ is the scope prefix, last @ (after scope/) is the version delimiter
    const match = source.match(/^(@[^/]+\/[^@]+)(?:@(.+))?$/);
    if (!match) {
        throw new Error(`Invalid scoped npm package source: ${redactUrlCredentials(source)}`);
    }
    return {
        name: match[1],
        version: match[2],
 ...
 */
export function isScopedNpmPackage(source) {
 ...
 */
export function resolveNpmRegistry(scope, registryOverride) {
    if (registryOverride) {
    ...
    for (const npmrcPath of npmrcPaths) {
        try {
            ...
            for (const line of lines) {
                ...
                const scopeMatch = trimmed.match(new RegExp(`^${scope.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')}:registry\\s*=\\s*(.+)`));
 ...
 */
function getNpmAuthToken(registryUrl, ambientTokenRegistryUrl) {
...
function fetchNpmJson(url, authToken, signal, redirectCount = 0, networkPolicy) {
    signal?.throwIfAborted();
    if (redirectCount > NPM_MAX_REDIRECTS) {
        return Promise.reject(new Error('Too many redirects while fetching npm package metadata'));
    }
    const headers = {
    ...
    return resolveNetworkTarget(url, networkPolicy, signal).then((target) => new Promise((resolve, reject) => {
        signal?.throwIfAborted();
        ...
        client
            ...
            if (res.statusCode === 301 || res.statusCode === 302) {
                if (res.headers.location) {
                    ...
                    const redirectToken = redirectUrl.origin === originalOrigin
                        ? authToken
                        : undefined;
                    fetchNpmJson(redirectUrl.toString(), redirectToken, signal, redirectCount + 1, networkPolicy)
                        .then(resolve)
                        .catch(reject);
                    return;
                }
            ...
            if (res.statusCode !== 200) {
                res.resume();
                return reject(new Error(`npm registry request failed with status ${res.statusCode}: ${redactUrlCredentials(url)}`));
            }
            const chunks = [];
            let totalBytes = 0;
            ...
            res.on('data', (chunk) => {
                ...
                if (totalBytes > NPM_METADATA_MAX_BYTES) {
                    ...
                    res.destroy();
                    reject(new Error(`npm package metadata exceeded maximum size of ${NPM_METADATA_MAX_BYTES} bytes`));
                    return;
                }
                chunks.push(chunk);
            });
...
function downloadNpmFileRedirect(url, dest, context, authToken, signal, redirectCount = 0, networkPolicy) {
    signal?.throwIfAborted();
    if (redirectCount > NPM_MAX_REDIRECTS) {
        return Promise.reject(new Error('Too many redirects while downloading npm package'));
    }
    const headers = {};
    if (authToken) {
        headers['Authorization'] = `Bearer ${authToken}`;
        ...
        const req = client
            ...
            if (res.statusCode === 301 || res.statusCode === 302) {
                if (res.headers.location) {
                    ...
                    res.destroy();
                    context.activeResponse = undefined;
                    downloadNpmFileRedirect(redirectUrl.toString(), dest, context, redirectToken, signal, redirectCount + 1, networkPolicy)
                        .then(finish)
                        .catch(fail);
                    return;
                }
            ...
            if (res.statusCode !== 200) {
                res.destroy();
                context.activeResponse = undefined;
                fail(new Error(`Failed to download npm tarball: status ${res.statusCode}`));
                return;
            }
            ...
            res.on('data', (chunk) => {
                ...
                if (bytesWritten > NPM_ARCHIVE_DOWNLOAD_MAX_BYTES) {
                    res.destroy();
                    file.destroy();
                    fail(new Error(`npm extension archive download exceeded maximum size of ${NPM_ARCHIVE_DOWNLOAD_MAX_BYTES} bytes`));
                    return;
                }
            });
            res.on('error', (error) => {
...
function downloadNpmFile(url, dest, authToken, signal, networkPolicy) {
    const context = {
        requestGeneration: 0,
        timedOut: false,
    };
    ...
    return new Promise((resolve, reject) => {
        ...
        const hardDeadline = setTimeout(() => {
            ...
            const error = new Error(`npm tarball download timed out after ${NPM_ARCHIVE_DOWNLOAD_TIMEOUT_MS}ms`);
        ...
        hardDeadline.unref();
        downloadNpmFileRedirect(url, dest, context, authToken, requestSignal, 0, networkPolicy)
            .then(finish)
            .catch(fail);
    });
}
 ...
 */
export async function downloadFromNpmRegistry(installMetadata, destination, signal) {
    const { name, version: requestedVersion } = parseNpmPackageSource(installMetadata.source);
    ...
    debugLogger.debug(`Fetching npm package metadata from ${redactUrlCredentials(metadataUrl)}`);
    const metadata = await fetchNpmJson(metadataUrl, authToken, signal, 0, installMetadata.networkPolicy);
    // Resolve version
    let resolvedVersion;
    ...
    if (!versionData) {
        throw new Error(`Version data for "${resolvedVersion}" not found for package ${name}`);
    }
    const tarballUrl = versionData.dist.tarball;
    debugLogger.debug(`Downloading ${name}@${resolvedVersion} from ${redactUrlCredentials(tarballUrl)}`);
    ...
    await downloadNpmFile(tarballUrl, tarballPath, tarballAuthToken, signal, installMetadata.networkPolicy);
    signal?.throwIfAborted();
    // Extract tarball
    ...
    if (fs.existsSync(packageDir)) {
        ...
        for (const entry of entries) {
            signal?.throwIfAborted();
            await fs.promises.rename(path.join(packageDir, entry), path.join(destination, entry));
        }
        signal?.throwIfAborted();
    ...
    debugLogger.debug(`Successfully extracted ${name}@${resolvedVersion} to ${destination}`);
    return {
 ...
 */
export async function checkNpmUpdate(installMetadata, signal) {
    try {
        ...
        const metadataUrl = `${registryUrl}/${encodedName}`;
        const metadata = await fetchNpmJson(metadataUrl, authToken, signal, 0, installMetadata.networkPolicy);
        const { version: requestedVersion } = parseNpmPackageSource(installMetadata.source);
        ...
        const targetTag = requestedVersion && metadata['dist-tags'][requestedVersion]
            ? requestedVersion
            : 'latest';
        ...
        signal?.throwIfAborted();
        debugLogger.error(`Failed to check npm update for "${redactUrlCredentials(installMetadata.source)}": ${redactUrlCredentials(String(error))}`);
        return ExtensionUpdateState.ERROR;
    }
}
