 ...
 * `.lailatulcoder/skills/learned-skill-<glob>` so these user-initiated learned skills
 ...
 */
export const LEARNED_SKILL_DIR_PREFIX = 'learned-skill-';
...
function isYouTubeVideoUrl(url) {
    ...
    return ['/embed/', '/shorts/', '/live/'].some((prefix) => url.pathname.startsWith(prefix) &&
        Boolean(url.pathname.slice(prefix.length).split('/')[0]));
}
export function parseLearnVideoInput(rawInput) {
    const match = rawInput.trim().match(/^(\S+)(?:\s+([\s\S]*))?$/);
    if (!match)
        return null;
    const [, source, rawFocus] = match;
    const lowerSource = source.toLowerCase();
    const localVideoEntry = [...DIRECT_VIDEO_MIME_TYPES].find(([extension]) => lowerSource.endsWith(extension));
    ...
    const directVideoEntry = [...DIRECT_VIDEO_MIME_TYPES].find(([extension]) => lowerPath.endsWith(extension));
    if (!directVideoEntry)
        return null;
    return {
        source,
...
async function buildSkillContext(projectRoot) {
 ...
 */
export async function buildLearnSkillPrompt(rawInput, projectRoot) {
    const { skillsRoot, existingNames } = await buildSkillContext(projectRoot);
    const existingLine = existingNames.length === 0
        ? ''
...
export async function buildLearnVideoSkillRequest(video, projectRoot, localVideoPart) {
    ...
    const videoPart = video.kind === 'local'
        ? localVideoPart
        : {
            fileData: {
...
//# sourceMappingURL=learn-skill-agent.js.map