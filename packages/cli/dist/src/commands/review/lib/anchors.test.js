...
import { collectNewSideLines, resolveAnchor, resolveAnchors, } from './anchors.js';
/**
...
describe('resolveAnchor', () => {
    ...
    it('does not let a marker guess also be an indentation guess', () => {
        ...
        const ok = resolveAnchor(lines(diff, 'z.ts'), '+    const deep = compute();');
    ...
    it('will not join two lines that are not consecutive in the file', () => {
        ...
        expect(resolveAnchor(hayG, 'const first = 1;\nconst second = 2;').status).toBe('unmatched');
    ...
    it('refuses a snippet quoting a REMOVED line', () => {
        ...
        expect(resolveAnchor(lines(removedOnly, 'x.ts'), 'const removed = true;').status).toBe('unmatched');
    });
...
describe('resolveAnchor — the substring fallback (KB-long lines)', () => {
    ...
    const PARAGRAPH = 'The resolver turns each quoted snippet into a line number, because a ' +
    ...
    it('resolves a mid-line fragment to the containing added line', () => {
        const r = resolveAnchor(hay(), 'a derived number is strictly better evidence than an asserted one');
        expect(r).toMatchObject({
            status: 'resolved',
            line: 2,
            startLine: 2,
    ...
    it('reports the containing line when it is a context line', () => {
        ...
        const r = resolveAnchor(lines(contextDiff, 'SKILL.md'), 'the natural anchor shape here');
        expect(r).toMatchObject({
            status: 'resolved',
            line: 2,
    ...
    it('counts one line once even when the fragment repeats inside it', () => {
        ...
        const r = resolveAnchor(lines(repeatDiff, 'r.md'), 'review the charge(amt) call');
        expect(r).toMatchObject({
            status: 'resolved',
            line: 1,
            matchCount: 1,
    ...
    it('carries the drift measurement like any other tier', () => {
        const r = resolveAnchor(hay(), 'the natural anchor shape here', 9);
    ...
    it('does not let the marker retry also be an indentation guess', () => {
        ...
        const r = resolveAnchor(lines(indented, 'z2.ts'), '+const deep = compute();');
        expect(r.status).toBe('unmatched');
...
describe('resolveAnchors (batch)', () => {
    ...
    it('never resolves to a line outside a hunk — the 422 guarantee', () => {
        ...
        for (const anchor of anchors) {
            ...
            expect(r.status).toBe('resolved');
            const inSomeHunk = file.hunks.some((h) => h.newCount > 0 && r.line >= h.newStart && r.line <= h.newEnd);
            expect(inSomeHunk).toBe(true);
        }
    });
});
//# sourceMappingURL=anchors.test.js.map