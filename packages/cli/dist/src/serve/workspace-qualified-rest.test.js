...
import { hashDaemonWorkspace } from '@lailatul-coder/lailatul-coder-core';
...
import { createWorkspaceGenerationGuard, createWorkspaceRegistry, } from './workspace-registry.js';
...
function makeWorkspaceService(label) {
    return {
        ...
        setWorkspaceSkillsEnabled: vi.fn(async (_ctx, skillNames, enabled) => ({
            enabled,
            activation: 'deferred',
            sessionsRefreshed: 0,
            sessionsFailed: 0,
...
async function makeHarness(opts) {
    const scratch = await fsp.mkdtemp(path.join(os.tmpdir(), 'qwen-workspace-qualified-rest-'));
    const primaryCwd = canonicalizeWorkspace(path.join(scratch, 'primary'));
    const secondaryCwd = canonicalizeWorkspace(path.join(scratch, opts?.secondaryDirName ?? 'secondary'));
                    ...
                    get(target, prop, receiver) {
                        if (prop === 'writeBytesAtomic') {
                            return (p, data) => {
                                ...
                                return opts.primaryWriteHold.hold.then(() => target.writeBytesAtomic(p, data));
                            };
                        }
    ...
    const secondary = {
        ...
        routeFileSystemFactory: opts?.secondaryTrusted === false
            ? untrustedFsFactory
            : secondaryFsFactory,
        clientMcpSenderRegistry: new ClientMcpSenderRegistry(),
        generationGuard: createWorkspaceGenerationGuard(),
    ...
    const app = createServeApp({ ...baseOpts, workspace: primaryCwd, token: opts?.token }, undefined, {
        workspaceRegistry,
...
async function makeWindowsSelectorHarness() {
    const scratch = await fsp.mkdtemp(path.join(os.tmpdir(), 'qwen-workspace-qualified-rest-win-'));
    const primaryCwd = canonicalizeWorkspace(path.join(scratch, 'primary'));
    await fsp.mkdir(primaryCwd, { recursive: true });
    ...
    const persistSetting = vi.fn(async () => { });
    const app = createServeApp({ ...baseOpts, workspace: primaryCwd }, undefined, {
...
describe('workspace-qualified core REST', () => {
    ...
    it('rejects unknown workspace selectors with workspace_mismatch', async () => {
        ...
        try {
            const res = await request(h.app)
                .get(`/workspaces/${encodeURIComponent(path.join(h.scratch, 'nope'))}/file`)
                .query({ path: 'target.txt' })
                .set('Host', host());
            ...
            expect(JSON.stringify(res.body)).not.toContain(path.join(h.scratch, 'nope'));
        }
        finally {
            await fsp.rm(h.scratch, { recursive: true, force: true });
        }
    ...
    it('routes workspace-qualified status reads to the selected workspace', async () => {
        ...
        try {
            ...
            const mcpResources = await request(h.app)
                .get(`/workspaces/${encodeURIComponent(h.secondaryId)}/mcp/docs/resources`)
                .set('Host', host());
    ...
    it('routes workspace-qualified settings and rejects user scope or untrusted access', async () => {
        ...
        try {
            ...
            expect(h.persistSetting).toHaveBeenCalledWith(h.secondaryCwd, expect.any(String), 'general.cleanupPeriodDays', 30, expect.any(Function));
        ...
        try {
            const res = await request(untrusted.app)
                .get(`/workspaces/${encodeURIComponent(untrusted.secondaryId)}/settings`)
                .set('Authorization', 'Bearer secret')
                .set('Host', host());
            expect(res.status).toBe(403);
            expect(res.body.code).toBe('untrusted_workspace');
    ...
    it('does not publish qualified settings from a closed generation', async () => {
        ...
        try {
            ...
            expect(entry.current.runtime.bridge.publishWorkspaceEvent).not.toHaveBeenCalled();
        }
        finally {
            await fsp.rm(h.scratch, { recursive: true, force: true });
        }
    ...
    it('routes workspace-qualified permissions and rejects non-workspace scope', async () => {
        ...
        try {
            ...
            expect(badScope.status).toBe(400);
            expect(badScope.body.code).toBe('global_scope_not_supported_for_workspace_route');
        }
        finally {
            await fsp.rm(h.scratch, { recursive: true, force: true });
        }
        ...
        try {
            const res = await request(untrusted.app)
                .get(`/workspaces/${encodeURIComponent(untrusted.secondaryId)}/permissions`)
                .set('Authorization', 'Bearer secret')
                .set('Host', host());
            expect(res.status).toBe(403);
            expect(res.body.code).toBe('untrusted_workspace');
    ...
    it('routes workspace-qualified file writes and trust-gates untrusted writes', async () => {
        ...
        try {
            ...
            await expect(fsp.readFile(path.join(h.secondaryCwd, 'created.txt'), 'utf8')).resolves.toBe('created');
        }
        finally {
            await fsp.rm(h.scratch, { recursive: true, force: true });
        }
        ...
        try {
            const res = await request(untrusted.app)
                .post(`/workspaces/${encodeURIComponent(untrusted.secondaryId)}/file/write`)
                .set('Authorization', 'Bearer secret')
                .set('Host', host())
    ...
    it('routes workspace-qualified file uploads and never falls back to primary', async () => {
        ...
        try {
            ...
            await expect(fsp.readFile(path.join(h.secondaryCwd, 'blob.bin'))).resolves.toEqual(data);
            await expect(fsp.stat(path.join(h.primaryCwd, 'blob.bin'))).rejects.toMatchObject({ code: 'ENOENT' });
        }
        finally {
            await fsp.rm(h.scratch, { recursive: true, force: true });
        }
        ...
        try {
            const res = await request(untrusted.app)
                .post(`/workspaces/${encodeURIComponent(untrusted.secondaryId)}/file/upload`)
                .set('Authorization', 'Bearer secret')
                .set('Host', host())
                .set('Content-Type', 'application/octet-stream')
            ...
            expect(res.status).toBe(403);
            expect(res.body.code).toBe('untrusted_workspace');
            await expect(fsp.stat(path.join(untrusted.primaryCwd, 'blocked.bin'))).rejects.toMatchObject({ code: 'ENOENT' });
        }
        finally {
            await fsp.rm(untrusted.scratch, { recursive: true, force: true });
        }
    ...
    it('uploads into a workspace whose root path trips the suspicious-pattern check', async () => {
        ...
        try {
            ...
            await expect(fsp.readFile(path.join(h.secondaryCwd, 'report.txt'), 'utf-8')).resolves.toBe('hi');
        }
        finally {
            await fsp.rm(h.scratch, { recursive: true, force: true });
        }
    ...
    it('shares one upload gate between legacy and qualified routes', async () => {
        ...
        try {
            const legacyUpload = (name) => request(h.app)
                .post('/file/upload')
                .set('Authorization', 'Bearer secret')
                .set('Host', host())
                .set('Content-Type', 'application/octet-stream')
    ...
    it('rejects qualified uploads for unknown, draining, and already removed workspaces', async () => {
        ...
        try {
            ...
            await expect(fsp.stat(path.join(h.primaryCwd, 'a.bin'))).rejects.toMatchObject({ code: 'ENOENT' });
            const secondaryRuntime = h.workspaceRegistry.getByWorkspaceId(h.secondaryId);
            ...
            await expect(fsp.stat(path.join(h.primaryCwd, 'b.bin'))).rejects.toMatchObject({ code: 'ENOENT' });
            ...
            await expect(fsp.stat(path.join(h.primaryCwd, 'c.bin'))).rejects.toMatchObject({ code: 'ENOENT' });
        }
        finally {
            await fsp.rm(h.scratch, { recursive: true, force: true });
        }
    ...
    it('routes workspace-qualified MCP control through the selected runtime', async () => {
        ...
        try {
            const restart = await request(h.app)
                .post(`/workspaces/${encodeURIComponent(h.secondaryId)}/mcp/docs/restart`)
                .set('Authorization', 'Bearer secret')
                .set('Host', host())
                .send({});
            ...
            const enable = await request(h.app)
                .post(`/workspaces/${encodeURIComponent(h.secondaryId)}/mcp/docs/enable`)
                .set('Authorization', 'Bearer secret')
            ...
            const remove = await request(h.app)
                .delete(`/workspaces/${encodeURIComponent(h.secondaryId)}/mcp/servers/runtime`)
                .set('Authorization', 'Bearer secret')
                .set('X-Qwen-Client-Id', 'client-1')
                .set('Host', host());
        ...
        try {
            const res = await request(untrusted.app)
                .post(`/workspaces/${encodeURIComponent(untrusted.secondaryId)}/mcp/docs/restart`)
                .set('Authorization', 'Bearer secret')
                .set('Host', host())
                .send({});
            expect(res.status).toBe(403);
    ...
    it('routes workspace-qualified tool toggles and validates mutation bodies', async () => {
        ...
        try {
            const enabled = await request(h.app)
                .post(`/workspaces/${encodeURIComponent(h.secondaryId)}/tools/Bash/enable`)
                .set('Authorization', 'Bearer secret')
                .set('Host', host())
                .send({ enabled: false });
        ...
        try {
            const res = await request(untrusted.app)
                .post(`/workspaces/${encodeURIComponent(untrusted.secondaryId)}/tools/Bash/enable`)
                .set('Authorization', 'Bearer secret')
                .set('Host', host())
                .send({ enabled: false });
            expect(res.status).toBe(403);
    ...
    it('routes workspace-qualified skill toggles and trust-gates writes', async () => {
        ...
        try {
            const res = await request(h.app)
                .post(`/workspaces/${encodeURIComponent(h.secondaryId)}/skills/review/enable`)
                .set('Authorization', 'Bearer secret')
                .set('X-Qwen-Client-Id', 'client-1')
                .set('Host', host())
                .send({ enabled: false });
            ...
            expect(h.secondaryWorkspaceService.setWorkspaceSkillsEnabled).toHaveBeenCalledWith(expect.objectContaining({
                workspaceCwd: h.secondaryCwd,
                originatorClientId: 'client-1',
            }), ['review', 'deploy'], false);
            vi.mocked(h.secondaryWorkspaceService.setWorkspaceSkillEnabled).mockRejectedValueOnce(new WorkspaceSkillNotFoundError('missing'));
            const missing = await request(h.app)
                .post(`/workspaces/${encodeURIComponent(h.secondaryId)}/skills/missing/enable`)
                .set('Authorization', 'Bearer secret')
                .set('Host', host())
                .send({ enabled: false });
            ...
            expect(h.secondaryWorkspaceService.setWorkspaceSkillsEnabled).toHaveBeenCalledTimes(1);
            ...
            expect(h.secondaryWorkspaceService.setWorkspaceSkillsEnabled).toHaveBeenCalledTimes(2);
            vi.mocked(h.secondaryWorkspaceService.setWorkspaceSkillsEnabled).mockRejectedValueOnce(new Error('disk full'));
            ...
            expect(h.secondaryWorkspaceService.setWorkspaceSkillsEnabled).toHaveBeenCalledTimes(3);
        }
        finally {
            await fsp.rm(h.scratch, { recursive: true, force: true });
        }
        ...
        try {
            const res = await request(untrusted.app)
                .post(`/workspaces/${encodeURIComponent(untrusted.secondaryId)}/skills/review/enable`)
                .set('Authorization', 'Bearer secret')
                .set('Host', host())
                .send({ enabled: false });
            expect(res.status).toBe(403);
            ...
            const batch = await request(untrusted.app)
                .post(`/workspaces/${encodeURIComponent(untrusted.secondaryId)}/skills/enable`)
                .set('Authorization', 'Bearer secret')
                .set('Host', host())
                .send({ skillNames: ['review'], enabled: false });
            expect(batch.status).toBe(403);
            expect(batch.body.code).toBe('untrusted_workspace');
            expect(untrusted.secondaryWorkspaceService.setWorkspaceSkillsEnabled).not.toHaveBeenCalled();
        }
        finally {
            await fsp.rm(untrusted.scratch, { recursive: true, force: true });
        }
    ...
    it('routes project agents to the selected workspace', async () => {
        ...
        try {
            ...
            expect(selected.body.agents.map((a) => a.name)).toContain('ws-agent');
            ...
            const update = await request(h.app)
                .post(`/workspaces/${encodeURIComponent(h.secondaryId)}/agents/ws-agent`)
                .set('Authorization', 'Bearer secret')
                .set('Host', host())
            ...
            expect(primary.body.agents.map((a) => a.name)).not.toContain('ws-agent');
            const deleted = await request(h.app)
                .delete(`/workspaces/${encodeURIComponent(h.secondaryId)}/agents/ws-agent`)
                .set('Authorization', 'Bearer secret')
                .set('Host', host());
    ...
    it('rejects global and user scope on workspace-qualified agents routes', async () => {
        ...
        try {
            ...
            expect(create.body.code).toBe('global_scope_not_supported_for_workspace_route');
            const list = await request(h.app)
                .get(`/workspaces/${encodeURIComponent(h.secondaryId)}/agents?scope=global`)
                .set('Authorization', 'Bearer secret')
                .set('Host', host());
            expect(list.status).toBe(400);
            expect(list.body.code).toBe('global_scope_not_supported_for_workspace_route');
            const detail = await request(h.app)
                .get(`/workspaces/${encodeURIComponent(h.secondaryId)}/agents/ws-agent?scope=user`)
                .set('Authorization', 'Bearer secret')
                .set('Host', host());
            expect(detail.status).toBe(400);
            expect(detail.body.code).toBe('global_scope_not_supported_for_workspace_route');
            const update = await request(h.app)
                .post(`/workspaces/${encodeURIComponent(h.secondaryId)}/agents/ws-agent?scope=user`)
                .set('Authorization', 'Bearer secret')
                .set('Host', host())
                .send({ description: 'bad update' });
            expect(update.status).toBe(400);
            expect(update.body.code).toBe('global_scope_not_supported_for_workspace_route');
        }
        finally {
            await fsp.rm(h.scratch, { recursive: true, force: true });
        }
    ...
    it('routes workspace-qualified memory reads and writes to the selected workspace', async () => {
        ...
        try {
            ...
            expect(read.body.files).toEqual(expect.arrayContaining([
    ...
    it('routes managed-memory tasks to the selected workspace bridge', async () => {
        ...
        try {
            const queued = await request(h.app)
                .post(`/workspaces/${encodeURIComponent(h.secondaryId)}/memory/remember`)
                .set('Authorization', 'Bearer secret')
                .set('Host', host())
                .send({ content: 'Secondary only' });
            expect(queued.status).toBe(202);
            await vi.waitFor(() => {
                expect(h.secondaryBridge.runWorkspaceMemoryRemember).toHaveBeenCalledWith({
            ...
            const completed = await request(h.app)
                .get(`/workspaces/${encodeURIComponent(h.secondaryId)}/memory/remember/${queued.body.taskId}`)
                .set('Authorization', 'Bearer secret')
                .set('Host', host());
            ...
            await vi.waitFor(() => {
                expect(h.secondaryBridge.runWorkspaceMemoryForget).toHaveBeenCalledWith({ query: 'Secondary only' });
            });
    ...
    it('rejects managed-memory tasks for an untrusted workspace', async () => {
        ...
        try {
            ...
            expect(res.status).toBe(403);
            expect(res.body.code).toBe('untrusted_workspace');
            expect(h.secondaryBridge.runWorkspaceMemoryRemember).not.toHaveBeenCalled();
            expect(h.primaryBridge.runWorkspaceMemoryRemember).not.toHaveBeenCalled();
            const forget = await request(h.app)
                .post(`/workspaces/${encodeURIComponent(h.secondaryId)}/memory/forget`)
                .set('Authorization', 'Bearer secret')
    ...
    it('shares the singular lane for the primary workspace qualified route', async () => {
        ...
        try {
            ...
            expect(queued.status).toBe(202);
            const taskId = queued.body.taskId;
            await vi.waitFor(() => {
    ...
    it('rejects global and user scope on workspace-qualified memory routes', async () => {
        ...
        try {
            for (const scope of ['global', 'user']) {
                ...
                expect(res.status).toBe(400);
                expect(res.body.code).toBe('global_scope_not_supported_for_workspace_route');
            }
        }
        finally {
            await fsp.rm(h.scratch, { recursive: true, force: true });
    ...
    it('returns typed memory write errors on workspace-qualified routes', async () => {
        const h = await makeHarness({ token: 'secret' });
        try {
            await fsp.writeFile(path.join(h.secondaryCwd, 'QWEN.md'), 'x'.repeat(17 * 1024 * 1024), 'utf8');
            const res = await request(h.app)
                .post(`/workspaces/${encodeURIComponent(h.secondaryId)}/memory`)
                .set('Authorization', 'Bearer secret')
                .set('Host', host())
...
//# sourceMappingURL=workspace-qualified-rest.test.js.map