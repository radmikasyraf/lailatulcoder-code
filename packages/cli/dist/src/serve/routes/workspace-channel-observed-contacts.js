...
import { requireTrustedWorkspaceRuntime, resolveWorkspaceRuntimeWithLiveCompatibilityFromParam, sendConversationRuntimeUnavailable, sendGenerationClosedError, sendUntrustedWorkspaceResponse, } from '../workspace-route-runtime.js';
...
function sendContacts(req, res, workspaceCwd, assertGenerationOpen) {
    ...
    try {
        ...
        if (sendGenerationClosedError(res, error))
            return;
        process.stderr.write('LailatulCoder serve: observed channel contacts unavailable.\n');
        res.status(500).json({
...
export function registerWorkspaceChannelObservedContactRoutes(app, deps) {
    ...
    app.get('/workspaces/:workspace/channel/observed-contacts', async (req, res) => {
        const runtime = resolveWorkspaceRuntimeWithLiveCompatibilityFromParam(deps.workspaceRegistry, req, res);
        ...
        const send = async () => sendContacts(req, res, runtime.workspaceCwd, () => runtime.generationGuard?.assertOpen());
        if (runtime.provenance === 'live-conversation' &&
            deps.conversationRuntimeActivity) {
            try {
