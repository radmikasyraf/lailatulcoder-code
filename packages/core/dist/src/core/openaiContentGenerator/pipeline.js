...
import { applyOfficialOpenAIPromptCaching, isOfficialOpenAIEndpoint, } from './prefix-caching.js';
...
import { isQwenFamilyWireModel, isTieredEffortWireModel, } from '../modalityDefaults.js';
import { DEFAULT_STREAM_IDLE_TIMEOUT_MS, DEFAULT_STREAM_MAX_LIFETIME_MS, MAX_STREAM_GUARD_TIMEOUT_MS, QWEN_STREAM_IDLE_TIMEOUT_MS_ENV, QWEN_STREAM_MAX_LIFETIME_MS_ENV, } from './constants.js';
...
import { reportOpenAiChunk, reportOpenAiRequest, reportOpenAiResponse, } from '../../telemetry/gen-ai-request.js';
...
function isRequiredThinkingError(error) {
    ...
    return (message.includes('enable_thinking') &&
...
export class StreamInactivityTimeoutError extends Error {
    ...
    constructor(idleMs, chunksReceived, streamLifetimeMs) {
        super(`No stream activity for ${idleMs}ms after ${chunksReceived} chunks ` +
...
export class StreamLifetimeExceededError extends Error {
    ...
    constructor(maxLifetimeMs, chunksReceived, streamLifetimeMs) {
        super(`Stream exceeded its ${maxLifetimeMs}ms upstream-wait cap after ` +
...
function isSSECompatibleContentType(contentType) {
    if (!contentType)
        return true; // absence → assume SSE (SDK default)
...
export class NonSSEResponseError extends Error {
    ...
    constructor(contentType, httpStatus, bodyPrefix, requestId) {
        ...
        super(`Streaming request received a non-SSE response ` +
...
function hasProviderOutputBudgetKey(samplingParams) {
    return PROVIDER_OUTPUT_BUDGET_KEYS.some((key) => samplingParams[key] !== undefined);
}
/**
...
function clampProviderOutputBudgetKeys(samplingParams, requestMaxTokens) {
    if (typeof requestMaxTokens !== 'number')
...
function resolveStreamGuardMs(fromConfig, configLabel, envName, defaultMs) {
    ...
    if (typeof fromConfig === 'number') {
        if (Number.isInteger(fromConfig) &&
            fromConfig <= MAX_STREAM_GUARD_TIMEOUT_MS) {
            return fromConfig;
        }
        // eslint-disable-next-line no-console
        console.warn(`[lailatul-coder] Ignoring out-of-range ${configLabel}=${fromConfig} ` +
    ...
    if (trimmed) {
        ...
        // eslint-disable-next-line no-console
        console.warn(`[lailatul-coder] Ignoring invalid ${envName}="${raw}" ` +
...
function resolveStreamIdleTimeoutMs(config) {
    return resolveStreamGuardMs(config.streamIdleTimeoutMs, 'streamIdleTimeoutMs', QWEN_STREAM_IDLE_TIMEOUT_MS_ENV, DEFAULT_STREAM_IDLE_TIMEOUT_MS);
}
function resolveStreamMaxLifetimeMs(config) {
    return resolveStreamGuardMs(config.streamMaxLifetimeMs, 'streamMaxLifetimeMs', QWEN_STREAM_MAX_LIFETIME_MS_ENV, DEFAULT_STREAM_MAX_LIFETIME_MS);
}
/**
...
async function* withStreamGuards(source, idleMs, maxLifetimeMs, abortRequest, parentSignal) {
    ...
    try {
        while (true) {
            const remainingMs = maxLifetimeMs > 0
            ...
            if (remainingMs <= 0) {
                ...
                abortRequest();
                throw new StreamLifetimeExceededError(maxLifetimeMs, chunksReceived, performance.now() - streamStartedAt);
            }
            ...
            const timeout = new Promise((_resolve, reject) => {
                ...
                timer = setTimeout(() => {
                    if (parentSignal?.aborted) {
                        ...
                        abortRequest();
                        reject(new StreamLifetimeExceededError(maxLifetimeMs, chunksReceived, performance.now() - streamStartedAt));
                    }
                    else {
                        abortRequest();
                        reject(new StreamInactivityTimeoutError(idleMs, chunksReceived, performance.now() - streamStartedAt));
                    }
                }, Math.max(wait, 0));
                timer.unref?.();
            });
...
export class ContentGenerationPipeline {
    ...
    requiredThinkingModels = new Set();
    ...
    constructor(config) {
        ...
        this.streamIdleTimeoutMs = resolveStreamIdleTimeoutMs(this.contentGeneratorConfig);
        this.streamMaxLifetimeMs = resolveStreamMaxLifetimeMs(this.contentGeneratorConfig);
    }
    async execute(request, userPromptId) {
        return this.executeWithErrorHandling(request, userPromptId, false, async (openaiRequest, context, telemetryAttempt) => {
            ...
            try {
                ...
                const geminiResponse = OpenAIContentConverter.convertOpenAIResponseToGemini(openaiResponse, context);
    ...
    async executeStream(request, userPromptId) {
        return this.executeWithErrorHandling(request, userPromptId, true, async (openaiRequest, context, telemetryAttempt) => {
            ...
            try {
                ...
                const createPromise = this.client.chat.completions.create(openaiRequest, { signal: perRequestAc.signal });
                // withResponse() is available on APIPromise (the OpenAI SDK's
                // extended Promise). If unavailable (e.g. a mock), fall back.
                if (typeof createPromise
                    .withResponse === 'function') {
                    const { data, response: httpResponse, request_id, } = await createPromise.withResponse();
                    stream = data;
                    ...
                    if (!isSSECompatibleContentType(contentType)) {
                        ...
                        throw new NonSSEResponseError(contentType, httpResponse.status, bodyPrefix, request_id);
                    }
                }
                else {
                    stream =
            ...
            const guarded = idleMs > 0 || maxLifetimeMs > 0
                ? withStreamGuards(stream, idleMs, maxLifetimeMs, () => perRequestAc.abort(), parentSignal)
                : stream;
            ...
            const innerStream = this.processStreamWithLogging(guarded, context, request, userPromptId, telemetryAttempt);
    ...
    async *processStreamWithLogging(stream, context, request, userPromptId, telemetryAttempt) {
        ...
        const logPendingProtocolTagSanitized = (response, sanitization) => {
            if (!sanitization)
                return;
        ...
        try {
            ...
            for await (const chunk of stream) {
                ...
                const response = OpenAIContentConverter.convertOpenAIChunkToGemini(chunk, context);
                ...
                if (pendingFinishProtocolTagSanitized &&
                    pendingFinishResponse &&
                    !response.candidates?.[0]?.finishReason &&
                    response.candidates?.some((candidate) => (candidate.content?.parts?.length ?? 0) > 0)) {
                    throw new InvalidStreamError('Model response continued after a finish reason.', 'PROTOCOL_TAG_LEAK');
                }
                ...
                const shouldYield = this.handleChunkMerging(response, pendingFinishResponse, (mergedResponse) => {
                    pendingFinishResponse = mergedResponse;
                });
                if (shouldYield) {
                    // If we have a pending finish response, yield it instead
                    if (pendingFinishResponse) {
                        logPendingProtocolTagSanitized(pendingFinishResponse, pendingFinishProtocolTagSanitized);
            ...
            if (context.pendingThinkingTagCandidate?.closingTagName &&
    ...
    async buildRequest(request, userPromptId, context, isStreaming) {
        const messages = OpenAIContentConverter.convertGeminiRequestToOpenAI(request, context);
        ...
        if (request.config?.tools && request.config.tools.length > 0) {
            baseRequest.tools =
                await OpenAIContentConverter.convertGeminiToolsToOpenAI(request.config.tools, this.contentGeneratorConfig.schemaCompliance ?? 'auto');
        ...
        let providerRequest = this.config.provider.buildRequest(baseRequest, userPromptId);
        if (this.contentGeneratorConfig.enableCacheControl !== false &&
            isOfficialOpenAIEndpoint(this.contentGeneratorConfig)) {
            providerRequest = applyOfficialOpenAIPromptCaching(providerRequest, this.config.cliConfig.getSessionId?.(), request.promptCacheSharing === true, isInForkExecution() ? undefined : (getCurrentAgentId() ?? undefined));
        }
        ...
        const isDashScope = DashScopeOpenAICompatibleProvider.isDashScopeProvider(this.contentGeneratorConfig);
        ...
        if (thinkingMandatory) {
            ...
            const chatTemplateKwargs = typed['chat_template_kwargs'];
        ...
        if (isDashScope &&
            ...
            debugLogger.debug('DashScope: dropping tool_choice=required while thinking is enabled', { model, reasoningEffort, thinkingBudget, thinkingMandatory });
    ...
    buildResponseFormat(request) {
        ...
        if (!isOfficialOpenAIEndpoint(this.contentGeneratorConfig))
            return {};
    ...
    requiresThinking(model) {
        ...
        return (this.requiredThinkingModels.has(normalizedModel) ||
    ...
    buildGenerateContentConfig(request) {
        ...
        if (configSamplingParams !== undefined) {
            ...
            const maxTokens = reconcileMaxTokens(configSamplingParams.max_tokens, requestMaxTokens) ??
            ...
            return clampProviderOutputBudgetKeys(maxTokens !== undefined
                ? { ...configSamplingParams, max_tokens: maxTokens }
                : { ...configSamplingParams }, requestMaxTokens);
        }
        const params = {
    ...
    async executeWithErrorHandling(request, userPromptId, isStreaming, executor) {
        ...
        const executeAttempt = async () => {
            openaiRequest = await this.buildRequest(request, userPromptId, context, isStreaming);
        ...
        try {
            ...
            if ((wireRequest?.['enable_thinking'] === false ||
    ...
    createRequestContext(request, isStreaming) {
        ...
        const taggedThinkingParser = isStreaming && responseParsingOptions?.taggedThinkingTags
        ...
        return {
            ...
            startTime: Date.now(),
            splitToolMedia: providerOverrides.splitToolMedia ??
                ...
                true,
            toolResultContentFormat: providerOverrides.toolResultContentFormat ??
