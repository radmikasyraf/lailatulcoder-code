...
function footerHint(tab) {
    switch (tab) {
        case EXTENSIONS_TABS.DISCOVER:
            return t('Type to search · Space to toggle · Enter to view · Ctrl+R refresh · Esc to go back');
        case EXTENSIONS_TABS.INSTALLED:
            return t('↑↓ navigate · Space enable/disable · f favorite · Enter details · Esc close');
...
export function ExtensionsManagerDialog({ onClose, config, initialTab, }) {
    const { extensionsUpdateState, confirmUpdateExtensionRequests, settingInputRequests, pluginChoiceRequests, } = useUIState();
    ...
    const hasPendingRequest = !!consentRequest || !!settingRequest || !!pluginChoiceRequest;
    const [activeTab, setActiveTab] = useState(initialTab ?? EXTENSIONS_TABS.INSTALLED);
    ...
    const [tabFooter, setTabFooter] = useState(null);
    ...
    const handleBrowseSource = useCallback((marketplaceName) => {
        setStatus(null);
        setTabLocked(false);
    ...
    if (!config) {
        return (_jsx(Box, { flexDirection: "column", width: boxWidth, children: _jsx(Box, { borderStyle: "single", borderColor: theme.border.default, padding: 1, width: boxWidth, children: _jsx(Text, { color: theme.status.error, children: t('Extensions are not available in this environment.') }) }) }));
    }
    return (_jsxs(Box, { flexDirection: "column", width: boxWidth, children: [consentRequest ? (_jsx(ConsentPrompt, { prompt: consentRequest.prompt, onConfirm: consentRequest.onConfirm, terminalWidth: boxWidth })) : settingRequest ? (_jsx(SettingInputPrompt, { settingName: settingRequest.settingName, settingDescription: settingRequest.settingDescription, sensitive: settingRequest.sensitive, onSubmit: settingRequest.onSubmit, onCancel: settingRequest.onCancel, terminalWidth: boxWidth }, settingRequest.settingName)) : pluginChoiceRequest ? (_jsx(PluginChoicePrompt, { marketplaceName: pluginChoiceRequest.marketplaceName, plugins: pluginChoiceRequest.plugins, onSelect: pluginChoiceRequest.onSelect, onCancel: pluginChoiceRequest.onCancel, terminalWidth: boxWidth }, pluginChoiceRequest.marketplaceName)) : null, _jsxs(Box, { display: hasPendingRequest ? 'none' : 'flex', borderStyle: "single", borderColor: theme.border.default, flexDirection: "column", paddingLeft: 1, paddingRight: 1, width: boxWidth, gap: 1, children: [_jsx(TabBar, { tabs: TABS, activeTab: activeTab, canSwitch: !tabLocked }), _jsxs(Box, { flexDirection: "column", children: [activeTab === EXTENSIONS_TABS.DISCOVER && (_jsx(DiscoverTab, { config: config, isActive: activeTab === EXTENSIONS_TABS.DISCOVER && !hasPendingRequest, onLockChange: handleLockChange, onStatus: setStatus, onInstalled: bumpReload, marketplaceFilter: discoverFilter ?? undefined, reloadSignal: reloadSignal })), activeTab === EXTENSIONS_TABS.INSTALLED && (_jsx(InstalledTab, { config: config, isActive: activeTab === EXTENSIONS_TABS.INSTALLED && !hasPendingRequest, onLockChange: handleLockChange, onStatus: setStatus, extensionsUpdateState: extensionsUpdateState, reloadSignal: reloadSignal })), activeTab === EXTENSIONS_TABS.SOURCES && (_jsx(SourcesTab, { config: config, isActive: activeTab === EXTENSIONS_TABS.SOURCES && !hasPendingRequest, onLockChange: handleLockChange, onStatus: setStatus, onChanged: bumpReload, onBrowse: handleBrowseSource, onFooter: setTabFooter, reloadSignal: reloadSignal }))] }), status && (_jsx(Text, { color: status.type === 'error'
                            ? theme.status.error
                            : status.type === 'warning'
                                ? theme.status.warning
                                : status.type === 'success'
                                    ? theme.status.success
                                    : theme.text.secondary, children: status.text })), _jsx(Text, { color: theme.text.secondary, children: tabFooter ??
                            (tabLocked
