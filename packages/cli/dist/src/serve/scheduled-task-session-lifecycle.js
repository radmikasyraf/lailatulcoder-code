 ...
 */
import { updateCronTasks, } from '@lailatul-coder/lailatul-coder-core';
 ...
 */
export async function disableTasksForSessions(projectRoot, sessionIds) {
 ...
 */
export async function enableTasksForSessions(projectRoot, sessionIds, now = Date.now()) {
    if (sessionIds.length === 0)
        return;
    const targets = new Set(sessionIds);
    await updateCronTasks(projectRoot, (tasks) => {
...
export async function removeTasksForSessions(projectRoot, sessionIds) {
    if (sessionIds.length === 0)
        return;
    const targets = new Set(sessionIds);
    await updateCronTasks(projectRoot, (tasks) => {
        const next = tasks.filter((task) => !isBoundTask(task) || !targets.has(task.sessionId));
        return next.length === tasks.length ? tasks : next;
    });
}
//# sourceMappingURL=scheduled-task-session-lifecycle.js.map