...
//    domain (which only a `.lailatulcoder/review-context.json` matching rule, read from
 ...
 */
function readReverseAuditReturns(planPath, env, diffPath) {
    try {
        ...
        const delivered = (t) => {
            ...
            for (const [key, prompt] of built) {
                ...
                if (prompt.trim() === '')
                    continue;
                if (!wasDeliveredVerbatim(t.launchPrompt, prompt))
                    continue;
                if (matched !== null)
                    return false;
                matched = key;
        ...
        t.returned &&
            t.diffToolCalls > 0 &&
            delivered(t) &&
            openedTheTerritory(t.diffReads, bakedRanges(t.launchPrompt, diffPath)))
            .map((t) => t.finalText);
 ...
 */
export function layerAuditGate(planPath, env = process.env, readReturns = readReverseAuditReturns) {
    if (!planPath)
        return { unreviewed: [] };
    let plan;
    try {
    ...
    let context;
    try {
        context = repositoryContextOf(plan);
    }
    catch {
    ...
    const diffPathValue = plan
        ?.diffPathAbsolute;
    const diffPath = typeof diffPathValue === 'string' && diffPathValue.length > 0
        ? diffPathValue
        : undefined;
    const { corroborated, identityMatched } = readReturns(planPath, env, diffPath);
...
//# sourceMappingURL=layer-audit-gate.js.map