 ...
 */
import { AuthEvent, AuthType, getErrorMessage, logAuth, buildInstallPlan, applyProviderInstallPlan, } from '@lailatul-coder/lailatul-coder-core';
...
export const useAuthCommand = (settings, config, addItem, onAuthChange) => {
    ...
    const { qwenAuthState, cancelQwenAuth } = useQwenAuth(pendingAuthType, isAuthenticating);
    ...
    const handleProviderSubmit = useCallback(async (providerConfig, inputs) => {
        ...
        try {
            ...
            try {
                ...
                console.error('[AUTO-MODEL] plan FULL:', JSON.stringify(plan));
                // Guna inputs.modelIds[0] - model pertama yang user pilih
                const selectedModelId = inputs?.primaryModelId ?? inputs?.modelIds?.[0] ?? plan?.modelSelection?.modelId;
    ...
    const cancelAuthentication = useCallback(() => {
        if (isAuthenticating && pendingAuthType === AuthType.NEVER_MATCH_THIS) {
            cancelQwenAuth();
        }
    ...
    useEffect(() => {
        ...
        if (val && !valid.includes(val)) {
            onAuthError(t('Invalid QWEN_DEFAULT_AUTH_TYPE value: "{{value}}". Valid values are: {{validValues}}', { value: val, validValues: valid.join(', ') }));
        }
    }, [onAuthError]);
    // -- Public interface ----------------------------------------------------
    const state = useMemo(() => ({
