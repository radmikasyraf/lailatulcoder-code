 ...
 */
const userJudgementPrompt = (condition) => `Based on the conversation transcript above, has the following stopping ` +
...
const JUDGE_RESULT_SCHEMA_COVERS_INTERFACE = true;
...
const JUDGE_ERROR_MESSAGE = 'Goal judge unavailable; the automatic /goal loop paused. The goal remains active.';
const JUDGE_REASON_FALLBACK = 'Goal judge unavailable; continue working toward the goal and run `/goal clear` to stop early.';
const UNVERIFIED_TERMINAL_REASON = 'Goal judge terminal evidence was not found in assistant output or tool results.';
...
function reportGoalJudgeFailure(error, stage) {
    void reportError(error, 'Goal judge failed', { stage }, `goal-judge-${stage}`).catch((reportErr) => {
        debugLogger.debug(`Goal judge error reporting failed: ${reportErr instanceof Error ? reportErr.message : String(reportErr)}`);
    });
}
/**
 ...
 */
export async function judgeGoal(config, args) {
    ...
    try {
        ...
        if (!text) {
            ...
            reportGoalJudgeFailure(new Error('Empty judge response'), 'empty-response');
            return judgeErrorResult();
        }
        const parsed = parseJudgeReply(text);
        if (!parsed) {
            debugLogger.debug(`Goal judge reply not parseable as JSON (length=${text.length})`);
            reportGoalJudgeFailure(new Error('Judge response was not parseable as JSON'), 'parse');
            return judgeErrorResult();
        }
        return toJudgeResult(parsed, evidenceSources);
    }
    catch (err) {
        debugLogger.debug(`Goal judge threw: ${err instanceof Error ? err.message : String(err)}`);
 ...
 */
function collectTranscript(config, lastAssistantText) {
    try {
        const client = config.getGeminiClient();
        ...
        const haveLast = lastModelText.includes(lastAssistantText) ||
        ...
        debugLogger.debug(`Goal judge transcript fetch failed: ${err instanceof Error ? err.message : String(err)}`);
        return fallbackTranscript(lastAssistantText);
    }
}
function fallbackTranscript(lastAssistantText) {
...
function capPart(part) {
    ...
    if (part.functionResponse) {
        return {
            ...
            functionResponse: {
                ...
                response: capStructuredValue(part.functionResponse.response),
            },
        };
    }
    if (part.functionCall) {
        return {
            ...
            functionCall: {
                ...part.functionCall,
                args: capStructuredValue(part.functionCall.args),
            },
        };
    }
    return part;
}
function capStructuredValue(value) {
    const serialized = safeStringify(value);
    if (serialized.length <= TRANSCRIPT_PART_CHAR_CAP)
        return value;
    return {
        truncated: true,
...
function collectResponseStrings(value) {
    if (typeof value === 'string')
        return [value];
    if (Array.isArray(value))
        return value.flatMap(collectResponseStrings);
    if (value && typeof value === 'object')
        return Object.values(value).flatMap(collectResponseStrings);
    return [];
}
function lastModelTextOf(transcript) {
 ...
 */
function collectEvidenceSources(transcript) {
    const sources = [];
...
function parseJudgeReply(text) {
    ...
    const impossibleValue = payload.impossible;
...
function hasVerifiableEvidence(evidence, sources) {
...
function unverifiedTerminalReason(judgeReason) {
    return `${UNVERIFIED_TERMINAL_REASON} Judge reason: ${judgeReason}`.slice(0, MAX_REASON_LEN);
}
function toJudgeResult(result, evidenceSources) {
    if (result.ok) {
        if (hasVerifiableEvidence(result.evidence, evidenceSources)) {
            return { kind: 'met', ok: true, reason: result.reason };
        }
        debugLogger.debug(`Goal judge ok=true evidence unverifiable; judge said: ${result.reason}`);
        return {
            kind: 'not_met',
            ok: false,
    ...
    if (result.impossible) {
        ...
        debugLogger.debug(`Goal judge impossible evidence unverifiable; judge said: ${result.reason}`);
