...
import { CHANNEL_STARTUP_PROFILE_VERSION, } from '@lailatul-coder/acp-bridge/bridgeTypes';
...
export function initializeAcpStartupProfiler() {
    if (enabled)
        return;
    enabled = true;
    frozen = false;
...
export function isAcpStartupProfilerEnabled() {
    return enabled;
}
export function markAcpStartup(mark) {
    if (!enabled || frozen || marks[mark] !== undefined)
        return;
    marks[mark] = performance.now();
}
export function beginAcpBootstrapConfigProfiling() {
    if (!enabled || frozen)
        return;
    bootstrapConfigActive = true;
    markAcpStartup('bootstrapConfigInitializationStart');
}
export function endAcpBootstrapConfigProfiling() {
    if (!enabled || frozen)
        return;
    markAcpStartup('bootstrapConfigInitializationEnd');
    bootstrapConfigActive = false;
}
export function recordAcpConfigStartupEvent(name) {
    if (!enabled || frozen || !bootstrapConfigActive)
        return;
...
export function buildAndFreezeAcpStartupProfile() {
    if (!enabled)
        return undefined;
    const phases = {
        processToProfilerReadyMs: marks.profilerReady === undefined
            ? undefined
        ...
        configConstructionMs: duration('configConstructionStart', 'configConstructionEnd'),
        appInitializationMs: duration('appInitializationStart', 'appInitializationEnd'),
        acpImportMs: duration('acpImportStart', 'acpImportEnd'),
        bootstrapConfigInitializationMs: duration('bootstrapConfigInitializationStart', 'bootstrapConfigInitializationEnd'),
        transportSetupMs: duration('transportSetupStart', 'transportSetupEnd'),
        initializeHandlerMs: duration('initializeHandlerStart', 'initializeHandlerEnd'),
    };
    const config = {
        extensionsInitialMs: duration('extensionsInitialStart', 'extensionsInitialEnd'),
        ...
        hierarchicalMemoryMs: duration('hierarchicalMemoryStart', 'hierarchicalMemoryEnd'),
    ...
    const processToResponseMs = marks.responseBuilt === undefined
        ? undefined
    ...
    if (processToResponseMs !== undefined && topLevelSum !== undefined) {
        phases.unattributedMs = roundMs(Math.max(0, processToResponseMs - topLevelSum));
    }
    ...
    if (phases.bootstrapConfigInitializationMs !== undefined &&
        configSum !== undefined) {
        config.otherMs = roundMs(Math.max(0, phases.bootstrapConfigInitializationMs - configSum));
    }
    const complete = processToResponseMs !== undefined &&
    ...
    const profile = {
        ...
        ...(marks.responseBuilt === undefined
            ? {}
            : {
                responseBuiltAtEpochMs: roundMs(performance.timeOrigin + marks.responseBuilt),
            }),
    };
    frozen = true;
    bootstrapConfigActive = false;
...
export function resetAcpStartupProfilerForTesting() {
    enabled = false;
    frozen = false;
    bootstrapConfigActive = false;
    marks = {};
}
//# sourceMappingURL=acp-startup-profiler.js.map