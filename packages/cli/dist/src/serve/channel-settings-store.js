...
function assertSharedField(key, value, previous) {
    const enumValues = {
    ...
    if (key === 'groups') {
        ...
        for (const [groupId, groupConfig] of Object.entries(value)) {
            ...
            for (const [nestedKey, nestedValue] of Object.entries(groupConfig)) {
                ...
                const valid = (nestedKey === 'requireMention' &&
                ...
                if (known &&
                    ...
                    throw invalidConfig(`Channel field "${key}.${groupId}.${nestedKey}" is invalid.`);
                }
                if (!known) {
                    assertPreservedUnknownField(`${key}.${groupId}`, nestedKey, nestedValue, previousGroup);
                }
            }
        }
        return true;
...
function assertManagedConfig(config, previous, fields) {
    ...
    for (const [key, value] of Object.entries(config)) {
        ...
        if (assertSharedField(key, value, Object.hasOwn(previous, key) ? previous[key] : undefined)) {
            continue;
        }
