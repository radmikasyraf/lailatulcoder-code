...
export class DuplicateWorkspaceInputError extends Error {
    constructor(workspace) {
        super(`Duplicate --workspace value resolves to ${JSON.stringify(workspace)}. ` +
...
export class NestedWorkspaceInputError extends Error {
    constructor(parent, child) {
        super(`Nested --workspace values are not supported yet: ` +
...
export class MultipleWorkspaceInputError extends Error {
    constructor() {
        super('Multiple --workspace values are not supported by this single-workspace caller. ' +
            'Pass one --workspace.');
...
function normalizeWorkspaceInputs(workspace) {
...
function isNestedWorkspace(parent, child) {
    return parent !== child && isWithinRoot(child, parent);
}
function rejectDuplicateOrNestedWorkspaceInputs(workspaces) {
    if (workspaces.length <= 1)
        return;
    let canonicalWorkspaces;
    try {
        canonicalWorkspaces = workspaces.map((workspace) => canonicalizeWorkspace(workspace));
    }
    catch {
        return;
    }
...
export function resolveWorkspaceInputs(workspace) {
    const workspaces = normalizeWorkspaceInputs(workspace);
    rejectDuplicateOrNestedWorkspaceInputs(workspaces);
    return workspaces;
}
export function resolveSingleWorkspaceInput(workspace) {
...
//# sourceMappingURL=workspace-inputs.js.map