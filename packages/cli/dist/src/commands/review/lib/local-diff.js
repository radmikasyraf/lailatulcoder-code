...
import { LITERAL_PATHSPECS, NULL_DEVICE, PINNED_DIFF_CONFIG, PINNED_DIFF_FLAGS, } from './diff-flags.js';
import { git, gitRaw, gitRawTolerateDiff, gitWithInput, refExists, } from './git.js';
/**
...
function describeUndiffable(abs, st) {
    if (st.isDirectory()) {
        return ('is a directory (an embedded git repository, most likely) — git cannot ' +
            'diff it as a file');
    }
 ...
 * with `-C <repoRoot>`. `LailatulCoder review capture-local --file src/foo.ts` from
...
function toRepoPathspec(repoRoot, file) {
    ...
    if (escapes) {
        throw new Error(`--file ${file} resolves to ${abs}, which is outside the repository ` +
            `at ${repoRoot}.`);
    }
 ...
 */
export function isBinarySection(section) {
    ...
    return /^(Binary files .* differ|GIT binary patch)$/m.test(section.toString('utf8'));
}
...
function diffUntracked(repoRoot, path) {
    return gitRawTolerateDiff('-C', repoRoot, LITERAL_PATHSPECS, ...PINNED_DIFF_CONFIG, 'diff', '--no-index', ...PINNED_DIFF_FLAGS, '--', NULL_DEVICE, path);
}
/**
...
export function captureLocalDiff(opts) {
    ...
    if (trackedDiff.length > MAX_UNTRACKED_TOTAL_BYTES) {
        skipped.push({
            ...
            reason: `the tracked diff is ${Math.round(trackedDiff.length / 1_000_000)} MB, ` +
    ...
    if (includeUntracked) {
        // The review writes its own scratch files under `.lailatulcoder/tmp` — the args
        // record, the parsed-args verdict, the diff, the plan — *before* this
        // capture runs. In a repo that does not ignore `.lailatulcoder`, `ls-files --others`
        ...
        const candidates = listUntracked(repoRoot, pathspec).filter((p) => !p.startsWith('.lailatulcoder/tmp/') && p !== '.lailatulcoder/tmp');
        if (candidates.length > MAX_UNTRACKED_FILES) {
            ...
            skipped.push({
                ...
                bytes: null,
                reason: `${candidates.length} untracked files exceeds the ` +
            ...
            for (const path of candidates) {
                ...
                if (bytes > MAX_UNTRACKED_BYTES) {
                    skipped.push({
                        path,
                        bytes,
                        reason: `${Math.round(bytes / 1000)} kB exceeds the ${Math.round(MAX_UNTRACKED_BYTES / 1000)} kB untracked-file cap`,
                    });
                    continue;
                }
                if (bytes > budget) {
                    skipped.push({
                        path,
                        bytes,
                        reason: `the untracked capture reached its ` +
                ...
                if (section.length > MAX_UNTRACKED_BYTES) {
                    skipped.push({
                        ...
                        reason: `its diff is ${Math.round(section.length / 1000)} kB, over the ` +
                ...
                if (isBinarySection(section)) {
                    ...
                    skipped.push({
                        path,
                        bytes,
                        reason: 'is a binary file — git emits only a "Binary files differ" ' +
...
//# sourceMappingURL=local-diff.js.map