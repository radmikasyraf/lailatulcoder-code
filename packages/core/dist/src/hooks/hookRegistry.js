...
export class HookRegistry {
    ...
    async reloadConfiguredHooks() {
        ...
        const enabledSnapshot = new Map(previousEntries.map((entry) => [
            this.getHookStateKey(entry),
            entry.enabled,
        ]));
        const agentEntries = this.entries.filter((entry) => entry.agentScope !== undefined);
        try {
        ...
        debugLogger.debug(`Hook registry reloaded with ${this.entries.length} hook entries`);
    }
    /**
     ...
     */
    addAgentHooks(hooks, agentScope) {
        const before = this.entries.length;
        this.processHooksConfiguration(hooks, HooksConfigSource.Session, agentScope);
        const addedCount = this.entries.length - before;
        debugLogger.debug(`Registered ${addedCount} ephemeral hook entries for agent scope "${agentScope}"`);
        return () => {
            const sizeBefore = this.entries.length;
            this.entries = this.entries.filter((entry) => entry.agentScope !== agentScope);
            const removed = sizeBefore - this.entries.length;
            if (removed > 0) {
                debugLogger.debug(`Removed ${removed} ephemeral hook entries for agent scope "${agentScope}"`);
            }
        };
    }
    /**
     ...
     */
    processHooksConfiguration(hooksConfig, source, agentScope) {
        for (const [eventName, definitions] of Object.entries(hooksConfig)) {
            ...
            for (const definition of definitions) {
                this.processHookDefinition(definition, typedEventName, source, agentScope);
            }
        }
    }
    /**
     ...
     */
    processHookDefinition(definition, eventName, source, agentScope) {
        if (!definition ||
    ...
    validateHookConfig(config, eventName, source) {
        ...
        if (config.type === 'http' && !config.url) {
            debugLogger.warn(`HTTP hook ${eventName} from ${source} missing url field`);
            return false;
        }
        if (config.type === 'function' && typeof config.callback !== 'function') {
            debugLogger.warn(`Function hook ${eventName} from ${source} missing or invalid callback`);
            return false;
        }
        if (config.type === 'prompt' && !config.prompt) {
            debugLogger.warn(`Prompt hook ${eventName} from ${source} missing prompt field`);
            return false;
        }
        return true;
    }
