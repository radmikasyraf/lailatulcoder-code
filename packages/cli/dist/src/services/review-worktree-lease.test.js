...
import { cleanupReviewWorktreeLeases, clearReviewWorktreeLease, clearReviewWorktreeLeaseIfOwned, createReviewWorktreeLease, isReviewLeaseFile, readReviewWorktreeLease, reviewLeaseHeldByAnotherSession, reviewLeasePath, } from './review-worktree-lease.js';
const roots = [];
...
describe('review worktree leases', () => {
    it('protects a worktree created after the lease is registered', () => {
        ...
        expect(execFileSync('git', ['-C', root, 'branch', '--list', 'qwen-review/pr-1'], { encoding: 'utf8' }).trim()).toBe('');
        expect(existsSync(join(root, '.qwen', 'tmp', 'qwen-review-lease-pr-1.json'))).toBe(false);
    });
    ...
    it('keeps the lease when fallback pruning fails', () => {
        ...
        expect(existsSync(worktree)).toBe(false);
        expect(existsSync(join(root, '.qwen', 'tmp', 'qwen-review-lease-pr-1.json'))).toBe(true);
    });
    it('removes only worktrees owned by the completed session', () => {
        ...
        expect(execFileSync('git', ['-C', root, 'branch', '--list', 'qwen-review/pr-1'], { encoding: 'utf8' }).trim()).toBe('');
        expect(readFileSync(join(root, '.qwen', 'tmp', 'qwen-review-lease-pr-2.json'), 'utf8')).toContain('session-b');
    });
    ...
    it('lets explicit review cleanup disarm the finalizer', () => {
        ...
        expect(existsSync(worktree)).toBe(true);
        expect(execFileSync('git', ['-C', root, 'branch', '--list', 'qwen-review/pr-1'], { encoding: 'utf8' }).trim()).toContain('qwen-review/pr-1');
    });
});
describe('readReviewWorktreeLease', () => {
    it('returns the lease createReviewWorktreeLease wrote', () => {
        ...
        expect(reviewLeasePath(root, 'pr-1')).toBe(join(root, '.qwen', 'tmp', 'qwen-review-lease-pr-1.json'));
    });
...
describe('lease acquisition is atomic (#9205)', () => {
    ...
    it('refuses to overwrite a lease another session acquired first', () => {
        ...
        expect(() => createReviewWorktreeLease(leaseParams(root, { sessionId: 'session-b', promptId: 'prompt-b' }))).toThrow(/session-a/);
        const lease = readReviewWorktreeLease(root, 'pr-1');
        expect(lease?.sessionId).toBe('session-a');
...
describe('reviewLeaseHeldByAnotherSession', () => {
    ...
    it('lets the owning session pass regardless of prompt', () => {
        ...
        expect(reviewLeaseHeldByAnotherSession({
            ...lease,
...
//# sourceMappingURL=review-worktree-lease.test.js.map