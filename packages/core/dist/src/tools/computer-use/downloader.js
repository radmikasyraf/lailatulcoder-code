...
import { CUA_DRIVER_VERSION, binaryPath, resolveAssetTarget, resolveAssetUrls, resolveChecksumUrls, versionDir, } from './constants.js';
/**
...
export async function findInstalled(home, platform = process.platform, arch = process.arch, version = CUA_DRIVER_VERSION) {
...
async function fetchFirst(urls, fetchImpl, onProgress) {
    ...
    throw new Error(`Computer Use: all download sources failed. Last error: ${lastErr instanceof Error ? lastErr.message : String(lastErr)}`);
}
/**
...
export async function ensureInstalled(opts) {
    ...
    const { res: sumRes } = await fetchFirst(resolveChecksumUrls(env, version), fetchImpl);
    const checksums = parseChecksums(await sumRes.text());
    const expectedSha = checksums.get(target.asset);
    if (!expectedSha) {
        throw new Error(`Computer Use: ${target.asset} missing from checksums.txt.`);
    }
    // 2. Download the asset to a temp file, hashing as we stream.
    const { res } = await fetchFirst(resolveAssetUrls(target.asset, env, version), fetchImpl, onProgress);
    ...
    if (actualSha !== expectedSha) {
        ...
        throw new Error(`Computer Use: checksum mismatch for ${target.asset} ` +
    ...
    if (platform === 'darwin' && target.hasApp) {
        ...
        // relaunch resolves THIS copy under ~/.lailatulcoder — that relaunch is what makes
...
function computerUseTmp(home) {
    ...
    return join(tmpdir(), 'qwen-computer-use-dl', Buffer.from(home).toString('hex').slice(0, 8));
}
...
async function extractZipWindows(zipPath, destDir) {
    ...
    const psCommand = `Expand-Archive -LiteralPath ${psSingleQuote(zipPath)} ` +
    ...
    throw new Error(`Computer Use: failed to unzip ${zipPath} on Windows (${errors.join('; ')}).`);
}
