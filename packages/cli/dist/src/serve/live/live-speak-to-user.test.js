...
import { SPEAK_TO_USER_TOOL_NAME, SpeakToUserTool, } from './live-speak-to-user.js';
describe('SpeakToUserTool', () => {
    it('uses the trusted Live permission path and forwards exact speech', async () => {
        ...
        expect(tool.description).toContain('Automatic backend LailatulCoder Ai text is silent context');
        await expect(invocation.getDefaultPermission()).resolves.toBe('allow');
        await expect(invocation.execute(new AbortController().signal)).resolves.toMatchObject({ returnDisplay: 'Spoke to user' });
        expect(speak).toHaveBeenCalledWith('原样说出这句话。');
    });
});
//# sourceMappingURL=live-speak-to-user.test.js.map