 ...
 */
import { createDebugLogger, ToolNames, FORK_SUBAGENT_TYPE, } from '@lailatul-coder/lailatul-coder-core';
    ...
    action: async (context, args) => {
        ...
        if (ui.isIdleRef?.current === false) {
            return {
                type: 'message',
                messageType: 'error',
                content: t('Cannot fork while a response or tool call is in progress. Wait for it to finish or resolve the pending tool call.'),
            };
        }
        if (!config.getModel()) {
            return {
        ...
        return {
            type: 'message',
            messageType: 'info',
            content: t('Forked into a background agent. It inherits this conversation and runs without blocking — track it in the background tasks panel; it reports back when done.'),
        };
    },
};
//# sourceMappingURL=forkCommand.js.map