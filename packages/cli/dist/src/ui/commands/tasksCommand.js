        ...
        case 'agent': {
            ...
            switch (status) {
                ...
                default: {
                    ...
                    throw new Error(`statusLabel(agent): unknown status: ${JSON.stringify(_exhaustive)}`);
                }
            }
        }
        case 'shell': {
            ...
            switch (status) {
                ...
                default: {
                    const _exhaustive = status;
                    throw new Error(`statusLabel(shell): unknown status: ${JSON.stringify(_exhaustive)}`);
                }
            }
        }
        case 'monitor': {
            ...
            switch (status) {
                ...
                default: {
                    const _exhaustive = status;
                    throw new Error(`statusLabel(monitor): unknown status: ${JSON.stringify(_exhaustive)}`);
                }
            }
        }
        default: {
            const _exhaustive = entry;
            throw new Error(`statusLabel: unknown TaskEntry kind: ${JSON.stringify(_exhaustive)}`);
        }
    }
}
function taskLabel(entry) {
    switch (entry.kind) {
        ...
        default: {
            const _exhaustive = entry;
            throw new Error(`taskLabel: unknown TaskEntry kind: ${JSON.stringify(_exhaustive)}`);
        }
    }
}
function taskId(entry) {
    switch (entry.kind) {
        ...
        default: {
            const _exhaustive = entry;
            throw new Error(`taskId: unknown TaskEntry kind: ${JSON.stringify(_exhaustive)}`);
        }
    }
}
function taskOutputPath(entry) {
    switch (entry.kind) {
        case 'agent':
        case 'shell':
        ...
        default: {
            const _exhaustive = entry;
            throw new Error(`taskOutputPath: unknown TaskEntry kind: ${JSON.stringify(_exhaustive)}`);
        }
    }
}
export const tasksCommand = {
    name: 'tasks',
    get description() {
        return t('List background tasks (text dump — interactive dialog opens via the footer pill)');
    },
    ...
    action: async (context) => {
        ...
        if (context.executionMode === 'interactive') {
            lines.push(t('Tip: focus the Background tasks pill in the footer (use ↓ from an empty composer) and press Enter for the interactive dialog with detail view + live updates.'), '');
        }
        ...
        for (const entry of entries) {
            ...
            const pidPart = (entry.kind === 'shell' || entry.kind === 'monitor') &&
                entry.pid !== undefined
                ? ` pid=${entry.pid}`
                : '';
            lines.push(`[${taskId(entry)}] ${statusLabel(entry)}  ${runtime}${pidPart}  ${taskLabel(entry)}`);
