...
import { DEFAULT_EXCLUDED_ENV_VARS, HOME_ENV_BOOTSTRAP_KEYS, isHardcodedProjectEnvExclusion, isLoaderEnvKey, PROJECT_ENV_HARDCODED_EXCLUSIONS, reportRejectedLoaderKeys, resetLoaderKeyRejectionReportingForTesting, } from './shared-env-keys.js';
...
const RELOAD_EXCLUDED_KEYS_CASEFOLDED = new Set([...RELOAD_EXCLUDED_KEYS].map((key) => key.toLowerCase()));
 ...
 *
 * User-level paths cover the home `.env` and the global LailatulCoder config dir
 * `.env` (which respects `QWEN_HOME`). When `QWEN_HOME` redirects elsewhere,
 * the legacy `<homedir>/.lailatulcoder/.env` is also included so credentials users
...
export function preResolveHomeEnvOverrides() {
    ...
    // `<homedir>/.lailatulcoder`, so path.dirname() recovers `<homedir>`.
 ...
 *
 * Candidates are iterated most-specific-first (`~/.lailatulcoder/.env` before
 ...
 * implementation.
 */
export function findEnvFiles(settings, startDir, userLevelPaths = getUserLevelEnvPaths(), workspaceTrusted) {
    const homeDir = os.homedir();
    ...
    const canUseEnvFile = (filePath) => {
        ...
        const workspaceDir = path.basename(dirPath) === SETTINGS_DIRECTORY_NAME
            ...
            : dirPath;
        const trusted = workspaceTrusted !== undefined && workspaceDir === realStartDir
            ? workspaceTrusted
    ...
    // ~/.lailatulcoder/.env (only when QWEN_HOME redirects), then ~/.env.
    ...
    while (true) {
        if (currentDir === homeDir) {
            ...
            // Workspace step: prefer .lailatulcoder/.env, then plain .env.
...
function canApplyParsedEnvKey(envFile, key, excludedVars, options = {}) {
    ...
    if (isLoaderEnvKey(key))
        return false;
    if (options.reload && isReloadExcludedKey(key))
        return false;
...
export function buildRuntimeEnvironment(settings, startDir = process.cwd(), baseEnv = process.env, workspaceTrusted) {
    const userLevelPaths = getUserLevelEnvPaths();
    const envFilePaths = findEnvFiles(settings, startDir, userLevelPaths, workspaceTrusted);
    ...
    for (const envFile of parsedEnvFiles.files) {
        ...
        reportRejectedLoaderKeys(`.env file ${envFile.path}`, Object.keys(envFile.parsedEnv));
    }
    if (settings.env) {
        const excludedVars = settings?.advanced?.excludedEnvVars || DEFAULT_EXCLUDED_ENV_VARS;
        ...
        reportRejectedLoaderKeys(`settings.env (${startDir})`, Object.keys(settings.env));
    }
...
export function loadEnvironment(settings, startDir = process.cwd()) {
    ...
    for (const envFile of parsedEnvFiles.files) {
        const excludedVars = settings?.advanced?.excludedEnvVars || DEFAULT_EXCLUDED_ENV_VARS;
        // homeScoped: `.env` lives under the user's home LailatulCoder dir or `~/.env` —
        //   only these may set QWEN_HOME / QWEN_RUNTIME_DIR.
        // qwenScoped: any `.env` whose immediate parent is `.lailatulcoder` (including
        //   `<repo>/.lailatulcoder/.env`) — exempt from the user `excludedEnvVars` list.
        for (const key in envFile.parsedEnv) {
 ...
 */
export function reloadEnvironment(settings, workspaceCwd, workspaceTrusted) {
    const userLevelPaths = getUserLevelEnvPaths();
    const envFilePaths = findEnvFiles(settings, workspaceCwd, userLevelPaths, workspaceTrusted);
    const parsedEnvFiles = parseEnvFiles(envFilePaths, userLevelPaths);
    if (process.env['CLOUD_SHELL'] === 'true') {
        setUpCloudShellEnvironmentInEnv(process.env, parsedEnvFiles.files);
    }
    ...
    if (settings.env) {
        ...
        reportRejectedLoaderKeys(`settings.env (${workspaceCwd})`, Object.keys(settings.env));
    }
