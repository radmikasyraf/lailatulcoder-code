...
import { MCPServerStatus, createTransport, ExtensionManager, isGatedMcpScope, runWithTimeout, } from '@lailatul-coder/lailatul-coder-core';
...
async function testMCPConnection(serverName, config) {
    ...
    try {
        ...
        await runWithTimeout(client.connect(transport, { timeout: MCP_CONNECT_TIMEOUT_MS }), MCP_CONNECT_TIMEOUT_MS, `MCP connection for ${serverName}`);
        ...
        return {
            ...
            timedOut: error instanceof Error && error.message.startsWith('Timed out after '),
        };
    }
}
...
export async function listMcpServers() {
    ...
    for (const serverName of serverNames) {
        ...
        // Gated (project `.mcp.json` / workspace `.lailatulcoder/settings.json`) servers that
        ...
        if (isGatedMcpScope(server.scope)) {
            ...
            if (state !== 'approved') {
                const statusText = state === 'rejected' ? 'Rejected' : 'Pending approval';
                writeStdoutLine(`${COLOR_YELLOW}●${RESET_COLOR} ${serverInfo} - ${statusText}`);
                continue;
            }
        }
