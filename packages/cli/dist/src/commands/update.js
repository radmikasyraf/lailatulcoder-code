...
export const updateCommand = {
    ...
    handler: async () => {
        const [{ loadSettings }, { checkForUpdatesDetailed, describeUpdateCheckFailure }, installationInfoModule, standaloneUpdate, stdioHelpers, { updateEventEmitter },] = await Promise.all([
        ...
        if (updateCheck.status === 'up-to-date') {
            writeStdoutLine(t('LailatulCoder Ai {{version}} is up to date!', {
                version: updateCheck.currentVersion,
            }));
            return;
        }
        if (updateCheck.status === 'error') {
            writeStderrLine(t('Failed to check for updates ({{reason}}). Please check your network or registry configuration.', { reason: describeUpdateCheckFailure(updateCheck.error) }));
            process.exitCode = 1;
            return;
        }
        if (updateCheck.status === 'skipped') {
            writeStderrLine(t('Unable to check for updates: {{reason}}', {
        ...
        if (installationInfo.isStandalone && installationInfo.standaloneDir) {
            ...
            try {
                ...
                const result = await performStandaloneUpdate(installationInfo.standaloneDir, info.update.latest);
                if (result === 'done') {
                    writeStdoutLine(t('Update successful! The new version will be used on your next run.'));
                }
                else {
                    writeStdoutLine(t('Update downloaded. It will be applied after you exit this session.'));
                }
            }
            catch (err) {
                writeStderrLine(t('Update failed: {{error}}', {
        ...
        for (const line of formatUpdateInstructions(installationInfo, info.update.latest)) {
