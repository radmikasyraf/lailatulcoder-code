...
const persistedSessions = vi.hoisted(() => new Map());
const persistedSessionOwners = vi.hoisted(() => new Map());
const persistedSessionLoadDelays = vi.hoisted(() => new Map());
const parentSessions = vi.hoisted(() => new Map());
const sessionSources = vi.hoisted(() => new Map());
const removeSessionMock = vi.hoisted(() => vi.fn(async (_sessionId) => true));
const removeSessionRuntimeBaseDirs = vi.hoisted(() => new Array());
    ...
    return {
        ...
        SessionService: class {
            ...
            sessionExists(sessionId) {
                return Promise.resolve(persistedSessions.has(sessionId) &&
            ...
            readCreationMetadata(sessionId) {
                return Promise.resolve(sessionSources.get(sessionId) ?? {
                    ...(parentSessions.has(sessionId)
                        ? { parentSessionId: parentSessions.get(sessionId) }
                        : {}),
                });
            }
            async readCreationMetadataIfReadable(sessionId, _state) {
...
function message(type, text, uuid, timestamp) {
    return {
        uuid,
        parentUuid: null,
        sessionId: 'task-1',
    ...
    return {
        conversation: {
            ...
            messages: [
                ...
                message('assistant', 'first answer', 'assistant-1', '2026-07-30T00:00:02.000Z'),
            ],
        },
    };
}
function persistedWithThought(id) {
    ...
    return {
        conversation: {
            ...
            messages: [
                ...
                {
                    ...message('assistant', '', 'assistant-call', '2026-07-30T00:00:02.000Z'),
                    message: {
                        role: 'model',
                        parts: [
                            {
...
function makeHarness() {
    ...
    const sendPrompt = vi.fn((_sessionId, _request, _signal, context) => {
        context.onPromptAdmitted?.();
        return new Promise(() => undefined);
    });
    const bridge = {
        getSessionSummary(sessionId) {
            if (!resident.has(sessionId))
        ...
        resumeSession: vi.fn(async ({ sessionId }) => {
        ...
        async *subscribeEvents(_sessionId, options) {
    ...
    const registry = {
        ...
        getByWorkspaceId: (workspaceId) => workspaceId === projectRuntime.workspaceId ? projectRuntime : undefined,
        resolveLiveSessionOwner: (sessionId) => resident.has(sessionId)
            ? { kind: 'found', runtime }
            : { kind: 'not_found' },
    };
    const materializeConversationDirectory = vi.fn(async (sessionId) => `/conversations/${sessionId}`);
...
describe('LiveTaskService', () => {
    it('preserves the structured unavailable error for an inactive internal owner', async () => {
        ...
        await expect(harness.service.handle({
    ...
    it('lists existing tasks in the current Codex wire shape without creating one', async () => {
        ...
        expect(listWorkspaceSessionsForResponse).toHaveBeenNthCalledWith(1, harness.bridge, '/conversations', expect.objectContaining({ view: 'organized', group: 'all' }), { runtimeBaseDir: '/runtime/conversations' });
        expect(listWorkspaceSessionsForResponse).toHaveBeenNthCalledWith(2, harness.projectBridge, '/project', expect.objectContaining({ view: 'organized', group: 'all' }), { runtimeBaseDir: '/runtime/project' });
        expect(listWorkspaceSessionsForResponse.mock.calls[1]?.[0]).toBe(harness.projectBridge);
    ...
    it('reads an existing task without opening or replacing it', async () => {
        ...
        expect(result['thread'].status).toEqual({
            type: 'notLoaded',
        });
    ...
    it('reports the later of the live watermark and the persisted transcript timestamp', async () => {
        ...
        expect(read['thread'].updatedAt).toBe(1_785_369_603);
        const wait = await harness.service.handle({
            callerSessionId: 'live-root',
            name: 'wait_threads',
            arguments: { targets: [{ threadId: 'task-1' }], timeoutMs: 0 },
        });
        const poll = wait['polls'][0];
        expect(poll['revision']).toBe(1_785_369_603);
        expect(JSON.parse(Buffer.from(String(poll['cursor']), 'base64url').toString('utf8'))).toMatchObject({ updatedAt: '2026-07-30T00:00:03.000Z' });
    });
    it('returns only final assistant text from read and wait results', async () => {
        ...
        expect(wait['polls'][0]).toMatchObject({
    ...
    it('preserves target order even when task reads resolve out of order', async () => {
        ...
        expect(result['polls'].map((poll) => poll.thread.id)).toEqual(['task-1', 'task-2']);
    });
    it('suppresses previously delivered text and markers for an unchanged cursor', async () => {
        ...
        expect(second['polls'][0]).toMatchObject({
            changed: false,
            latestAssistantMessageId: 'assistant-2',
        ...
        expect(reset['polls'][0]).toMatchObject({
            changed: true,
            cursorReset: true,
    ...
    it('ends wait on new user input without adding a non-Codex result field', async () => {
        ...
        await vi.waitFor(() => expect(harness.bridge.getSessionEventEpoch).toHaveBeenCalled());
...
//# sourceMappingURL=live-task-service.test.js.map