...
import { existsSync, lstatSync, mkdtempSync, readFileSync, realpathSync, mkdirSync, readdirSync, rmSync, statSync, symlinkSync, writeFileSync, } from 'node:fs';
 ...
 */
export function redirectedAncestor(dir, stopAt = process.cwd()) {
    try {
...
export function sanitizedGitEnv() {
    ...
    const drop = new Set([...GIT_ENV_REDIRECTS, ...GIT_ENV_CONFIG, ...GIT_ENV_EXEC].map((k) => k.toLowerCase()));
 ...
 */
export function discardWorktree(cwd, tree) {
    ...
    if (!symlink) {
        ...
        if (sweep.status !== 0) {
            ...
            sweep = spawnSync('git', ['worktree', 'remove', '--force', '--force', tree], { cwd, encoding: 'utf8', env: sanitizedGitEnv() });
        }
    }
 ...
 */
function dropWorktreeRegistration(cwd, tree, adminDir) {
    if (adminDir !== null) {
        try {
    ...
    const common = spawnSync('git', ['rev-parse', '--path-format=absolute', '--git-common-dir'], { cwd, encoding: 'utf8', env: sanitizedGitEnv() });
    if (common.status !== 0 || typeof common.stdout !== 'string')
        return;
 ...
 */
let pipelineExcludesFile = null;
function pipelineExcludeArgs() {
    if (pipelineExcludesFile === null) {
        try {
            ...
            const file = join(mkdtempSync(join(tmpdir(), 'LailatulCoder-review-excludes-')), 'excludes')
 ...
 */
function ignoreSourcesOf(cwd, paths) {
    const r = spawnSync('git', [...pipelineExcludeArgs(), 'check-ignore', '-z', '-v', '--stdin'], {
        cwd,
 ...
 */
function hidesEverything(pattern) {
 ...
 */
function trackedIgnoreSources(cwd, sources) {
    const inside = [...sources].filter((s) => s.length > 0 && !isAbsolute(s) && !s.split('/').some((p) => p === '..'));
    if (inside.length === 0)
        return new Set();
 ...
 */
export function worktreeResidue(cwd, cap = 12) {
    ...
    try {
        if (!lstatSync(join(cwd, '.git')).isFile()) {
            return {
                paths: [],
                total: 0,
                unmeasured: '.git is not a gitfile (a planted repository?) — a repo stood up ' +
    ...
    if (!isWorktree) {
        return {
            paths: [],
            total: 0,
            unmeasured: 'the path is not a git worktree (repository discovery walks up into ' +
    ...
    // from the name.
    const others = spawnSync('git', ['-c', 'core.fsmonitor=', 'ls-files', '--others', '-z'], {
        cwd,
        encoding: 'utf8',
        maxBuffer: 64 * 1024 * 1024,
        env: sanitizedGitEnv(),
    ...
    if (others.error ||
        ...
        const why = others.error
            ? (others.error.code ?? others.error.message)
    ...
    for (const rec of others.stdout.split('\0')) {
        if (rec.length === 0 || seen.has(rec))
            continue;
        if (inPipelineFootprint(rec))
            continue;
        seen.add(rec);
        extras.push(rec);
    }
    if (extras.length > 0) {
        ...
        if (hiddenBy === null) {
            return {
                ...
                total: paths.length,
                unmeasured: "the ignore rules hiding this tree's untracked files could not be " +
                    'attributed, so `git status` cannot be trusted to have seen them',
            };
        }
        const fromTheCommit = trackedIgnoreSources(cwd, new Set([...hiddenBy.values()].map((rule) => rule.source)));
    ...
    const stage = spawnSync('git', ['-c', 'core.fsmonitor=', 'ls-files', '-s', '-z'], {
        cwd,
        encoding: 'utf8',
        maxBuffer: 64 * 1024 * 1024,
        env: sanitizedGitEnv(),
    ...
    if (blind.length > 0) {
        return {
            paths: paths.slice(0, cap),
            total: paths.length,
            unmeasured: 'git status cannot see inside the committed submodule path(s) ' +
                blind.join(', '),
        };
    }
    ...
    if (bits.error || bits.status !== 0 || typeof bits.stdout !== 'string') {
        const why = bits.error
            ? (bits.error.code ?? bits.error.message)
    ...
    if (bits.stdout.split('\0').some((rec) => /^[a-zS]/.test(rec))) {
        return {
            paths: paths.slice(0, cap),
            total: paths.length,
            unmeasured: 'the index carries skip-worktree or assume-unchanged bits, so `git ' +
 ...
 */
export function exposeDependencies(probeTree, dependencyRoot, opts = {}) {
    const done = {
        linked: 0,
        failed: 0,
    ...
    try {
        ...
        if (lstatSync(nm).isDirectory())
            rootNm = realpathSync(nm);
    }
    catch {
    ...
    const containment = {
        rootNm,
        selfLinks: new Set(memberSources.flatMap((m) => (m.source === null ? [] : [m.source]))),
    };
    farmNodeModules(dependencyRoot, probeTree, done, containment, opts.rebuild === true);
    ...
    const owned = new Set([join(probeTree, 'node_modules')]);
    for (const { dir, source } of memberSources) {
        ...
        if (!source || !target) {
            ...
            if (existsSync(join(dependencyRoot, dir)))
                done.failed++;
            continue;
        }
 ...
 * dependency.
 */
function removeUnownedNodeModules(tree, owned, done) {
    ...
    for (const real of inTreeLinks) {
        if (isOwned(owned, real))
            continue;
        if (holdsNodeModules(real, owned))
            done.failed++;
    }
}
/**
 ...
 */
function holdsNodeModules(dir, owned) {
    let entries;
    try {
        entries = readdirSync(dir, { withFileTypes: true });
    }
    ...
    for (const entry of entries) {
        ...
        if (entry.name === '.git')
            continue;
        if (holdsNodeModules(full, owned))
            return true;
    }
    return false;
}
 ...
 */
const FARM_MARKER = '.lailatulcoder-review-farm';
/**
 ...
 */
function farmLinkVerdict(real, sourceNm, containment) {
    if (insideDir(sourceNm, real))
        return 'internal';
    if (containment.rootNm !== null && insideDir(containment.rootNm, real)) {
        return 'internal';
    }
    for (const member of containment.selfLinks) {
        if (insideDir(member, real))
            return 'self';
    }
    return null;
}
function insideDir(base, real) {
 ...
 * `node_modules/.lailatulcoder-review-farm` beside its own module stubs and `git
 ...
 * consult this at all.)
 */
function marksOurFarm(target, sourceDir) {
    try {
        return (readFileSync(join(target, FARM_MARKER), 'utf8').trim() ===
            resolve(sourceDir));
    }
    catch {
        return false;
 ...
 */
function farmNodeModules(sourceDir, targetDir, done, containment, rebuild = false) {
    ...
    for (const entry of entries) {
        ...
        if (entry.name.startsWith('@')) {
            ...
            for (const pkg of pkgs) {
                ...
                try {
                    if (!statSync(scopedSource).isDirectory())
                        continue;
                }
                catch {
                    done.failed++;
 ...
 */
export function worktreeCreateFailureDetail(label, err, sweepStderr) {
    const sweepErr = sweepStderr.trim();
    return (`${label} worktree could not be created: ${err instanceof Error ? err.message : String(err)}` +
