...
import { findLastSafeSplitPoint, splitFencedMarkdown, parseCodeFenceInfo, getEnclosingFenceInfo, } from './markdownUtilities.js';
describe('markdownUtilities', () => {
    ...
    describe('splitFencedMarkdown', () => {
        ...
        it.each([
            ...
            [14, 3],
            [18, 4],
        ])('splitting at %d (a line boundary) starts the tail at line %d', (splitPoint, expectedStartLine) => {
            const content = '```js\naaa\nbbb\nccc\n```';
            const { after } = splitFencedMarkdown(content, splitPoint);
            expect(after.startsWith(`\`\`\`js lailatul-coder:start-line=${expectedStartLine}\n`)).toBe(true);
        });
        it('accumulates the start line when a re-opened tail is split again', () => {
            ...
            const tail = '```python lailatul-coder:start-line=4\nline4\nline5\n\nline6\nline7\n';
