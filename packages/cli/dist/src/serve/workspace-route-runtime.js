...
export function resolveManagedWorkspaceRuntimeByPathSelector(registry, selector) {
    ...
    if (exact && !isInternalWorkspaceRuntime(exact))
        return exact;
    if (path.isAbsolute(selector) && !isUncPath(selector)) {
        try {
            const canonicalSelector = canonicalizeWorkspace(selector);
    ...
    return registry
        .listManaged()
        .find((runtime) => !isInternalWorkspaceRuntime(runtime) &&
...
export function resolveWorkspaceRuntimeWithLiveCompatibilityFromParam(registry, req, res, paramName = 'workspace') {
    if (typeof registry.getManagedEntryByWorkspaceId !== 'function' ||
...
export function sendConversationRuntimeUnavailable(res) {
