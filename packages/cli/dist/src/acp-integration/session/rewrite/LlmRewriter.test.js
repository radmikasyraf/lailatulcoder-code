...
vi.mock('@lailatul-coder/lailatul-coder-core', () => ({
    ...
    runSideQuery: vi.fn(async (_config, options) => {
...
function makeTurn(messages, thoughts = []) {
...
describe('LlmRewriter', () => {
    ...
    describe('contextTurns', () => {
        it('should include last rewrite output by default (contextTurns=1)', async () => {
            ...
            const firstInput = mockGenerateContent.mock.calls[0][0].contents[0].parts[0].text;
            ...
            const secondInput = mockGenerateContent.mock.calls[1][0].contents[0].parts[0].text;
        ...
        function historyOf(rewriter) {
            return rewriter.outputHistory;
        }
        ...
        it('should include last N rewrites when contextTurns=N', async () => {
            ...
            const input = mockGenerateContent.mock.calls[3][0].contents[0].parts[0].text;
        ...
        it('should include all rewrites when contextTurns="all"', async () => {
            ...
            await rewriter.rewrite(makeTurn(['msg2']));
            await rewriter.rewrite(makeTurn(['msg3']));
            const input = mockGenerateContent.mock.calls[2][0].contents[0].parts[0].text;
    ...
    describe('model override', () => {
        it('should use rewriteConfig.model when set', async () => {
            ...
            expect(mockGenerateContent.mock.calls[0][0].model).toBe('custom-rewrite-model');
        });
    ...
    describe('filtering', () => {
        ...
        it('should not accumulate failed rewrites in history', async () => {
            ...
            const input = mockGenerateContent.mock.calls[1][0].contents[0].parts[0].text;
            expect(input).not.toContain('上一轮改写结果');
        });
    });
});
//# sourceMappingURL=LlmRewriter.test.js.map