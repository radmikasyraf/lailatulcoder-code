 ...
 */
import { fetchGitDiff, fetchGitDiffHunksForFile, } from '@lailatul-coder/lailatul-coder-core';
import { requireTrustedWorkspaceRuntime, resolveContainedCwd, resolveWorkspaceRuntimeFromParam, sendUntrustedWorkspaceResponse, } from '../workspace-route-runtime.js';
...
function buildDiffList(workspaceCwd, result) {
    ...
    return {
        ...
        files,
        hiddenCount: Math.max(0, result.stats.filesCount - result.perFileStats.size),
    };
}
...
async function handleDiffList(res, workspaceCwd, sendBridgeError, route, assertGenerationOpen) {
    try {
...
async function handleDiffFile(req, res, workspaceCwd, sendBridgeError, route, assertGenerationOpen) {
    ...
    const oldPath = typeof queryOldPath === 'string' && queryOldPath.length > 0
        ? queryOldPath
        : undefined;
    try {
        assertGenerationOpen?.();
        ...
        // fetch throws.
        applyReadHeaders(res);
        const result = await fetchGitDiffHunksForFile(workspaceCwd, queryPath, oldPath);
        assertGenerationOpen?.();
...
export function registerWorkspaceGitDiffRoutes(app, deps) {
    app.get('/workspace/git/diff', (_req, res) => {
        if (deps.isWorkspaceTrusted?.() === false) {
            sendUntrustedWorkspaceResponse(res);
            return;
        }
        void handleDiffList(res, deps.boundWorkspace, deps.sendBridgeError, 'GET /workspace/git/diff', deps.captureGenerationAssertion?.());
    });
    app.get('/workspace/git/diff/file', (req, res) => {
        ...
        void handleDiffFile(req, res, deps.boundWorkspace, deps.sendBridgeError, 'GET /workspace/git/diff/file', deps.captureGenerationAssertion?.());
    });
}
...
export function registerWorkspaceQualifiedGitDiffRoutes(app, deps) {
    app.get('/workspaces/:workspace/git/diff', (req, res) => {
        const runtime = resolveTrustedRuntime(deps.workspaceRegistry, req, res);
        if (!runtime)
            return;
        void handleDiffList(res, resolveContainedCwd(req, runtime.workspaceCwd), deps.sendBridgeError, 'GET /workspaces/:workspace/git/diff', () => runtime.generationGuard?.assertOpen());
    });
    app.get('/workspaces/:workspace/git/diff/file', (req, res) => {
        const runtime = resolveTrustedRuntime(deps.workspaceRegistry, req, res);
        if (!runtime)
            return;
        void handleDiffFile(req, res, resolveContainedCwd(req, runtime.workspaceCwd), deps.sendBridgeError, 'GET /workspaces/:workspace/git/diff/file', () => runtime.generationGuard?.assertOpen());
    });
}
//# sourceMappingURL=workspace-git-diff.js.map