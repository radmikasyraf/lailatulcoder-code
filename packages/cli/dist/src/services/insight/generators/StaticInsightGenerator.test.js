...
describe('StaticInsightGenerator', () => {
    ...
    it('writes insights under runtime output directory', async () => {
        ...
        generator.dataProcessor = { generateInsights };
        generator.templateRenderer = { renderInsightHTML };
        const projectsDir = path.resolve('workspace', 'project-a', '.qwen', 'projects');
        ...
        expect(generateInsights).toHaveBeenCalledWith(projectsDir, facetsDir, undefined);
        expect(renderInsightHTML).toHaveBeenCalledWith({});
        expect(mockedFs.writeFile).toHaveBeenCalledWith(expectedOutputPath, '<html>ok</html>', 'utf-8');
        expect(outputPath).toBe(expectedOutputPath);
    });
});
//# sourceMappingURL=StaticInsightGenerator.test.js.map