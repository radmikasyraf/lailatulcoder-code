...
import { assessResume } from './resume.js';
...
describe('assessResume — the empty-string shapes, named by the FIRST break', () => {
    ...
    it('an empty recorded effort reads as the default, not as a mismatch', () => {
        ...
        expect(assessResume({ ...prev(), effort: '' }, probes({ requestedEffort: 'high' }))).toEqual({ ok: true });
    });
});
describe('assessResume', () => {
    ...
    it('refuses with effort-mismatch when an explicit effort differs', () => {
        ...
        expect(assessResume({ ...prev(), effort: 'medium' }, probes({ requestedEffort: 'high' }))).toEqual({ ok: false, reason: 'effort-mismatch' });
    });
    it('resumes when the explicit effort matches the recorded one', () => {
        expect(assessResume({ ...prev(), effort: 'medium' }, probes({ requestedEffort: 'medium' }))).toEqual({ ok: true });
    });
    it('reads a plan with no recorded effort as the default high', () => {
        expect(assessResume(prev(), probes({ requestedEffort: 'high' }))).toEqual({
            ok: true,
        });
        expect(assessResume(prev(), probes({ requestedEffort: 'medium' }))).toEqual({ ok: false, reason: 'effort-mismatch' });
    });
    it('reads an invalid recorded effort as the default high', () => {
        ...
        expect(assessResume({ ...prev(), effort: 'turbo' }, probes({ requestedEffort: 'high' }))).toEqual({ ok: true });
        expect(assessResume({ ...prev(), effort: 'turbo' }, probes({ requestedEffort: 'medium' }))).toEqual({ ok: false, reason: 'effort-mismatch' });
    });
    ...
    it('refuses with no-diff-hash on a pre-diffSha256 report', () => {
        expect(assessResume({ ...prev(), diffSha256: undefined }, probes())).toEqual({ ok: false, reason: 'no-diff-hash' });
    });
    ...
    it('refuses with diff-hash-mismatch when the diff bytes changed', () => {
        ...
        expect(assessResume(prev(), probes({ diffSha256OnDisk: 'b'.repeat(64) }))).toEqual({ ok: false, reason: 'diff-hash-mismatch' });
    });
    ...
    it('still resumes one short of the cap', () => {
        expect(assessResume(prev(), probes({ resumeCount: RESUME_MAX - 1 }))).toEqual({ ok: true });
    });
    it('reports the FIRST broken link when several are broken at once', () => {
        ...
        expect(assessResume(prev(), probes({
        ...
        expect(assessResume(prev(), probes({ liveHeadSha: 'moved', resumeCount: 99 }))).toEqual({ ok: false, reason: 'head-moved' });
    });
});
