...
import { createSingleWorkspaceRegistry, createWorkspaceRegistry, } from './workspace-registry.js';
import { resolveContainedCwd, resolveContainedCwdOrFail, resolveRegisteredWorkspaceRuntimeByPathSelector, resolveWorkspaceRuntimeFromParam, resolveWorkspaceRuntimeWithLiveCompatibilityFromParam, } from './workspace-route-runtime.js';
function fakeReq(cwd) {
    return { query: cwd !== undefined ? { cwd } : {} };
}
describe('resolveContainedCwd', () => {
    ...
    it('returns the resolved path for a valid subdirectory', () => {
        ...
        fs.mkdirSync(sub);
        expect(resolveContainedCwd(fakeReq(sub), workspace)).toBe(fs.realpathSync(sub));
    });
    it('accepts the workspace root itself', () => {
        expect(resolveContainedCwd(fakeReq(workspace), workspace)).toBe(fs.realpathSync(workspace));
    });
...
describe('resolveContainedCwdOrFail', () => {
    ...
    it('fails closed when cwd is an array (a duplicated ?cwd= param)', () => {
        expect(resolveContainedCwdOrFail(fakeReq(['/a', '/b']), workspace)).toBeNull();
    });
    ...
    it('returns the resolved path for a valid contained cwd', () => {
        const sub = path.join(workspace, 'sub');
        fs.mkdirSync(sub);
        expect(resolveContainedCwdOrFail(fakeReq(sub), workspace)).toBe(fs.realpathSync(sub));
    });
...
describe('resolveWorkspaceRuntimeFromParam', () => {
    it.each(['ws-live', '/work/conversations'])('treats internal selector %s as an ordinary workspace mismatch', (selector) => {
        ...
        expect(resolveWorkspaceRuntimeFromParam(registry, { params: { workspace: selector } }, response)).toBeNull();
        expect(resolveRegisteredWorkspaceRuntimeByPathSelector(registry, internal.workspaceCwd)).toBeUndefined();
        expect(response.status).toHaveBeenCalledWith(400);
        expect(JSON.stringify(json.mock.calls)).not.toContain(internal.workspaceCwd);
        expect(JSON.stringify(json.mock.calls)).not.toContain(internal.workspaceId);
    });
    it('returns retryable unavailable for a registered transitioning workspace', () => {
        ...
        const response = makeResponse();
        expect(resolveWorkspaceRuntimeFromParam(registry, { params: { workspace: 'ws-primary' } }, response)).toBeNull();
    ...
    it('keeps unknown workspaces distinct from unavailable registrations', () => {
        const registry = createSingleWorkspaceRegistry(makeRuntime());
        const response = makeResponse();
        expect(resolveWorkspaceRuntimeFromParam(registry, { params: { workspace: 'missing' } }, response)).toBeNull();
        expect(response.status).toHaveBeenCalledWith(400);
        expect(response.json).toHaveBeenCalledWith({
...
describe('resolveWorkspaceRuntimeWithLiveCompatibilityFromParam', () => {
    ...
    it.each(['ws-live', '/work/conversations'])('allows the exact internal selector %s only through the explicit seam', (selector) => {
        const { internal, registry } = setup();
        expect(resolveWorkspaceRuntimeWithLiveCompatibilityFromParam(registry, { params: { workspace: selector } }, makeResponse())).toBe(internal);
    });
    it('does not allow a path alias for the internal runtime', () => {
        const { registry } = setup();
        const response = makeResponse();
        expect(resolveWorkspaceRuntimeWithLiveCompatibilityFromParam(registry, {
    ...
    it('returns a sanitized unavailable response for inactive internal state', () => {
        ...
        const response = makeResponse();
        expect(resolveWorkspaceRuntimeWithLiveCompatibilityFromParam(registry, { params: { workspace: internal.workspaceId } }, response)).toBeNull();
        expect(response.status).toHaveBeenCalledWith(503);
        expect(response.json).toHaveBeenCalledWith({
...
//# sourceMappingURL=workspace-route-runtime.test.js.map