 ...
 */
export function sanitizeArtifactTitle(raw) {
 ...
 */
export function validateSelfContained(fragment) {
    if (!fragment.trim()) {
    ...
    const head = fragment
        .replace(/^\s+/, '')
        .replace(/^(?:<!--[\s\S]*?-->\s*)+/, '');
    const wrapperTag = /^(?:<!doctype\b|<html[\s>]|<head[\s>]|<body[\s>])/i.exec(head);
    if (wrapperTag) {
    ...
    const extResource = /\b(?:src|srcset|poster)\s*=\s*["']?\s*(?:https?:)?\/\//i.exec(scan) ??
    ...
    const extScript = /\b(?:fetch|WebSocket|XMLHttpRequest)\s*\(\s*["']\s*(?:https?|wss?):\/\//i.exec(scan) ??
    ...
    const metaRefresh = /<meta\b[^>]*http-equiv\s*=\s*["']?refresh["']?[^>]*\burl\s*=\s*(?:https?:)?\/\//i.exec(scan);
    if (metaRefresh) {
 ...
 */
export function wrapArtifactHtml(bodyFragment, title) {
