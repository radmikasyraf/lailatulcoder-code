...
export function isBinaryContentType(contentType) {
    ...
    if (mt.startsWith('text/'))
        return false;
    if (mt.endsWith('+json') || mt.endsWith('+xml'))
        return false;
    ...
    if (BINARY_APPLICATION_TYPES.has(mt))
        return true;
 ...
 * Content-Type → bin.
 */
export function sniffFileKind(bytes, contentType, contentDisposition, url) {
    const dispositionExt = extensionFromFilename(filenameFromContentDisposition(contentDisposition));
    let urlExt;
    try {
    ...
    if (bytes.length >= 4 &&
        ...
        const refined = zipHint && ['docx', 'xlsx', 'pptx', 'jar'].includes(zipHint)
            ? zipHint
            : 'zip';
        return {
            extension: refined,
 ...
 */
export async function persistBinaryContent(bytes, ext, dir, persistId) {
...
//# sourceMappingURL=binary-content.js.map