...
function hasReachedTotalCap(state) {
    return (state.totalBlock + state.totalUnavailable >=
        AUTO_MODE_DENIAL_LIMITS.maxTotalDenials);
}
/** Freshly-initialised state with all counters zero. */
export function createDenialState() {
    return {
        consecutiveBlock: 0,
...
export function formatDenialStateLog(state) {
...
export function isDenialFallbackReason(reason) {
    return (reason === 'consecutive_block' ||
        reason === 'consecutive_unavailable' ||
        reason === 'total_denial');
}
/** Record a successful (allow) decision. Resets both consecutive counters. */
export function recordAllow(state) {
 ...
 */
export function recordBlock(state) {
    return {
 ...
 * `consecutiveBlock`.
 */
export function recordUnavailable(state) {
    return {
        consecutiveBlock: 0,
...
export function shouldFallback(state) {
    ...
    if (state.consecutiveUnavailable >=
 ...
 */
export function recordFallbackApprove(state) {
    if (hasReachedTotalCap(state)) {
 ...
 */
export function isApproveOutcome(outcome) {
 ...
 */
export function resetDenialState() {
    return createDenialState();
}
