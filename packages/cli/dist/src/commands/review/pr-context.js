...
import { currentUser, ensureAuthenticated, gh, ghApiAll, HOSTNAME_RE, resolveGhHost, setGhHost, } from './lib/gh.js';
import { LEDGER_MAX_FINDINGS, parseLedger, stripLedgerMarker, } from './lib/ledger.js';
 ...
 */
export const SUMMARY_MARKER = '<!-- LailatulCoder-review-suggestion-summary -->';
/**
 ...
 */
export function isLegacySuggestionSummary(body) {
...
function capBody(s, ref) {
    const body = (s ?? '').trim();
    if (body.length <= FULL_BODY_CAP)
        return body;
...
function refRepo(ctx) {
    return {
        or: ctx?.ownerRepo ?? '{owner}/{repo}',
        n: ctx?.prNumber ?? '{n}',
    };
}
function commentBodyCommand(id, kind, ctx) {
...
const CODE_REF_RE = /(?<![\w./@-])[\w./@-]{0,200}[\w-]\.(?:tsx?|jsx?|mjs|cjs|vue|svelte|py|go|rs|java|kt|rb|c|cc|cpp|h|hpp|cs|php|swift|scala|sh|sql|graphql|gql|proto|gradle|ya?ml|json|toml|md)(?::\d+(?:-\d+)?)?\b/g;
const MAX_CODE_REFS = 12;
export function extractCodeRefs(body) {
    const all = [
    ...
    const refs = all.filter((r) => !all.some((other) => other !== r && other.endsWith(`/${r}`)));
 ...
 */
const NEG_WORD = '\\b(?:no|not|zero|without|never|non[- ])|没有|不是|无|未发现|不存在|并非|绝非|(?<!除)非';
...
const ADVERSATIVE = '\\b(?:but|however|although|though)\\b|但是|但|然而|不过|\\s-{1,2}\\s';
...
export function carriesBlockerSignal(body) {
    ...
    return BLOCKER_PATTERNS.some((re) => {
        ...
        while ((hit = m.exec(b)) !== null) {
            ...
            if (!NEGATION.test(b.slice(0, hit.index)))
                return true;
        }
        return false;
    });
 ...
 */
function snippetWithRef(s, max, ref) {
 ...
 */
export function findRootId(startId, byId) {
    const seen = new Set();
    let cur = startId;
    while (true) {
        if (seen.has(cur))
...
/**
 * The exact "no issues found, LGTM" template the LailatulCoder-review pipeline
 ...
 */
export const CANONICAL_LGTM_RE = /^No issues found\.?\s*LGTM!?\s*(?:✅\s*)?(?:_— [^\n]{0,200} via LailatulCoder Ai \/review(?: \(v[^\n]{1,100}\))?_\s*)?$/i;
/**
 ...
 * template the LailatulCoder-review pipeline auto-emits — those carry no review
 ...
 */
export function isReviewWorthShowing(body) {
    const trimmed = (body ?? '').trim();
    if (trimmed.length === 0)
        return false;
    if (CANONICAL_LGTM_RE.test(trimmed))
        return false;
    return true;
}
/**
 ...
 */
export function isBlockerBody(body, author, me) {
    if (carriesBlockerSignal(body))
        return true;
    return (me !== '' &&
 ...
 */
export function anyRootCarriesCriticalMarker(comments) {
    return comments.some((c) => (c.in_reply_to_id === undefined || c.in_reply_to_id === null) &&
 ...
 */
export function classifyInlineThreads(inline, me = '') {
    ...
    for (const c of inline) {
        ...
        if (rootId === c.id)
            continue; // self-reference safety
    ...
    const roots = inline.filter((c) => c.in_reply_to_id === undefined || c.in_reply_to_id === null);
    ...
    const isBlockerRoot = (c) => isBlockerBody(c.body, c.user?.login, me);
    const repliedBlockerRoots = roots.filter((c) => repliesByRoot.has(c.id) && isBlockerRoot(c));
    const openBlockerRoots = roots.filter((c) => !repliesByRoot.has(c.id) && isBlockerRoot(c));
    const repliedRoots = roots.filter((c) => repliesByRoot.has(c.id) && !isBlockerRoot(c));
    const openRoots = roots.filter((c) => !repliesByRoot.has(c.id) && !isBlockerRoot(c));
    return {
        openRoots,
        openBlockerRoots,
        repliedBlockerRoots,
...
function blockerSection(roots, issueBlockers, repliesByRoot, ctx) {
    if (roots.length === 0 && issueBlockers.length === 0)
        return [];
    const out = [
    ...
    for (const root of sortedRoots) {
        ...
        if (replies.length > 0) {
            out.push(...charge([
                ...
                ...replies.map((r) => `- **@${r.user?.login ?? '?'}**: ${snippetWithRef(r.body, 500, pullCommentRef(r.id, ctx))}`),
                '',
            ]));
        }
    }
 ...
 */
export function recoverLedger(reviews, login) {
    ...
    if (me) {
        for (const r of reviews) {
            if (r.user?.login?.toLowerCase() !== me)
                continue;
            if (r.state === 'PENDING')
                continue;
            sawOwnReview = true;
    ...
    for (const r of reviews) {
        ...
        if (foreign && ledger.round > ownMax + FOREIGN_ROUND_HEADROOM)
            continue;
        const at = r.submitted_at ?? '';
        const id = typeof r.id === 'number' ? r.id : 0;
        ...
        if (newer) {
            best = {
                ...
                id,
                ledger,
                commitId: typeof r.commit_id === 'string' && COMMIT_SHA_RE.test(r.commit_id)
                    ? r.commit_id
                    : null,
                foreign,
                author,
    ...
    if (!best)
        return { recovered: null, sawOwnReview };
    ...
    if (me && best.foreign)
        ledger = stripForeignVolume(ledger);
...
export function latestLedger(reviews, login) {
 ...
 */
export function persistedAnchorSha(sideFilePath) {
    try {
        const raw = JSON.parse(readFileSync(sideFilePath, 'utf8'));
 ...
 */
export function persistRecoveredLedger(sideFilePath, recovered, 
...
flags) {
    ...
    if (recovered) {
        try {
            ...
            const exRound = typeof existing?.['round'] === 'number'
                ? existing['round']
                : -1;
            const exId = typeof existing?.['reviewId'] === 'number'
                ? existing['reviewId']
                : -1;
            ...
            if (!identityKnown && existing !== null) {
                ...
                model: _droppedModel, commitId: _droppedCommitId, ...rest } = existing;
            ...
            const recoveredOut = identityKnown
                ? recovered.ledger
                : withoutVolume(recovered.ledger);
            writeAtomic(JSON.stringify({
                ...recoveredOut,
                ...
                foreign: (identityKnown && recovered.foreign) ||
                ...
                merged: (identityKnown && recovered.merged) ||
                    (!recovered.foreign &&
    ...
    try {
        const stale = JSON.parse(readFileSync(sideFilePath, 'utf8'));
 ...
 */
function stripAnchor(ledger) {
    if (ledger.sha === undefined && ledger.model === undefined)
        return ledger;
 ...
 */
export function withoutVolume(record) {
    const out = { ...record };
 ...
 * branch protects.
 */
export function pickVolume(record) {
    const out = {};
    for (const field of VOLUME_FIELDS) {
...
function stripForeignVolume(ledger) {
    return withoutVolume(ledger);
}
/**
 ...
 * diff?" needs to see.
 */
function anchorRuling(ledger, running, code, persistedSha) {
    ...
    if (persistedSha !== null && persistedSha !== ledger.sha) {
        return (`**Do NOT pass any sha as \`--since\`, and do not run git against ` +
    ...
    if (certifierMatchesRound(ledger.model, running)) {
        return (`The reviewed-at sha is the incremental anchor Step 1's ` +
    ...
    const runner = running !== '' ? `\`${code(running)}\`` : 'an unpublished identity';
    return (`**Do NOT pass the reviewed-at sha as \`--since\`, and do not run git ` +
 ...
 * warning as stale).
 */
export function renderLedgerSection(ledger, running, author = null, merged = false, 
/**
...
persistedSha = null) {
    ...
    const cell = (v) => v
        ...
        .replace(/\|/g, '\\|')
        .replace(/[\r\n]+/g, ' ');
    const code = (v) => cell(v).replace(/`/g, "'");
    const rows = ledger.findings.map((f) => `| ${cell(f.id)} | ${f.sev === 'C' ? 'Critical' : 'Suggestion'} | \`${code(f.file)}${f.line ? `:${f.line}` : ''}\` | ${cell(f.title)} |`);
    return [
...
export function buildMarkdown(prNumber, ownerRepo, meta, inline, issue, reviews, prevLedger = null, me = '', 
...
persistedSha = null) {
    const { openRoots, openBlockerRoots, repliedBlockerRoots, repliedRoots, repliesByRoot, } = classifyInlineThreads(inline, me);
    ...
    const ctx = { ownerRepo, prNumber, host };
    ...
    parts.push(`- **Diff:** ${meta.changedFiles} files, +${meta.additions}/-${meta.deletions}`);
    parts.push('');
    parts.push(PREAMBLE);
    parts.push('');
    ...
    parts.push(...blockerSection(allBlockerRoots, blockerIssue, repliesByRoot, ctx));
    ...
    if (prevLedger) {
        parts.push(renderLedgerSection(prevLedger, roundModelIdFrom(process.env), prevLedgerAuthor, prevLedgerMerged, persistedSha));
    }
    ...
    if (meaningfulReviews.length > 0) {
        ...
        parts.push('');
        parts.push('> Bodies are rendered in full: an unmappable or 422-relocated blocker lives ONLY here, and a truncated rendering once hid one from the re-check. A body cut at the cap names the review id to fetch for the rest.');
        parts.push('');
        for (const r of meaningfulReviews) {
            const date = (r.submitted_at ?? '').slice(0, 10);
            const idNote = r.id !== undefined ? ` (review ${r.id})` : '';
            parts.push(`### @${r.user?.login ?? '?'} [${r.state ?? 'COMMENTED'}]${date ? ` ${date}` : ''}${idNote}`);
            parts.push('');
    ...
    if (openRoots.length > 0) {
        ...
        for (const c of openRoots) {
            parts.push(`- \`${c.path ?? '?'}\`:${c.line ?? '?'} by @${c.user?.login ?? '?'} (comment ${c.id}): ${snippetWithRef(c.body, 240, pullCommentRef(c.id, ctx))}`);
        }
        parts.push('');
    }
    ...
    if (repliedRoots.length > 0 || settledIssue.length > 0) {
        ...
        if (repliedRoots.length > 0) {
            ...
            for (const root of sortedRoots) {
                ...
                parts.push('');
                parts.push(`> ${snippetWithRef(root.body, 240, pullCommentRef(root.id, ctx))}`);
                parts.push('');
                if (replies.length > 0) {
                    parts.push('Replies (chronological):');
                    for (const r of replies) {
                        parts.push(`- **@${r.user?.login ?? '?'}**: ${snippetWithRef(r.body, 240, pullCommentRef(r.id, ctx))}`);
                    }
                    parts.push('');
                }
            }
        ...
        if (settledIssue.length > 0) {
            ...
            for (const c of settledIssue) {
                parts.push(`- by @${c.user?.login ?? '?'}: ${snippetWithRef(c.body, 240, issueCommentRef(c.id, ctx))}`);
            }
            parts.push('');
        }
    }
 ...
 */
export function truncatedHeadings(markdown, limit) {
    const out = [];
    const re = /^#{2,3} .*$/gm;
    let m;
    while ((m = re.exec(markdown)) !== null) {
        if (m.index >= limit)
            out.push({ offset: m.index, heading: m[0] });
    }
    return out;
}
async function runPrContext(args) {
    ...
    ensureAuthenticated();
    const meta = JSON.parse(gh('pr', 'view', prNumber, '--repo', ownerRepo, '--json', 'title,body,author,baseRefName,headRefName,headRefOid,additions,deletions,changedFiles,state'));
    ...
    // otherwise miss.
    const inline = ghApiAll(`repos/${owner}/${repo}/pulls/${prNumber}/comments`);
    const allIssue = ghApiAll(`repos/${owner}/${repo}/issues/${prNumber}/comments`);
    ...
    const reviews = ghApiAll(`repos/${owner}/${repo}/pulls/${prNumber}/reviews`);
    ...
    if (reviews.length || inline.length) {
        ...
        if (!identityKnown && anyRootCarriesCriticalMarker(inline)) {
            throw new Error(`cannot determine the reviewing account (${lookupError === null
                ? 'empty login'
                : lookupError instanceof Error
                    ? lookupError.message
                    : String(lookupError)}) while a posted root comment carries a LailatulCoder critical marker — ` +
    ...
    persistRecoveredLedger(join(dirname(out), `LailatulCoder-review-pr-${prNumber}-prev-ledger.json`), prevRecovered, {
        ...
        // flag.)
        noOwnReview: reviews.length > 0 && identityKnown && !recoveryThrew && !sawOwnReview,
    ...
    const persistedSha = persistedAnchorSha(join(dirname(out), `LailatulCoder-review-pr-${prNumber}-prev-ledger.json`));
    ...
    const bakeHost = resolvedHost !== undefined && HOSTNAME_RE.test(resolvedHost)
        ? resolvedHost
        : undefined;
    const md = buildMarkdown(prNumber, ownerRepo, meta, inline, issue, reviews, prevLedger, me, prevLedgerAuthor, prevLedgerMerged, bakeHost, persistedSha);
    mkdirSync(dirname(out), { recursive: true });
    writeFileSync(out, md, 'utf8');
    const meaningfulReviewCount = reviews.filter((r) => isReviewWorthShowing(stripLedgerMarker(r.body ?? ''))).length;
    ...
    const blockerCount = threads.repliedBlockerRoots.length +
        threads.openBlockerRoots.length +
        issue.filter((c) => carriesBlockerSignal(c.body)).length;
    writeStdoutLine(`Wrote PR context to ${out} (${inline.length} inline, ${issue.length} issue comments, ${blockerCount} blocker(s) to re-check, ${meaningfulReviewCount}/${reviews.length} review summaries — review bodies and blocker bodies rendered in full)`);
    ...
    if (md.length > DEFAULT_TRUNCATE_TOOL_OUTPUT_THRESHOLD) {
        writeStdoutLine(`warning: ${out} is ${md.length} chars; read_file returns the first ` +
        ...
        if (cut.length > 0) {
            ...
            writeStdoutLine('  every heading is inside the cut; the loss is in the last section’s body.');
        }
    }
}
export const prContextCommand = {
    command: 'pr-context <pr_number> <owner_repo>',
    describe: 'Fetch PR metadata + existing comments and emit a Markdown context file for review agents',
    builder: (yargs) => yargs
        ...
        .option('host', {
        type: 'string',
        describe: 'GitHub host for this PR (GitHub Enterprise). Routes every gh call in this command via GH_HOST, and is baked into the emitted comment-body refetch commands; omit for github.com.',
    }),
    handler: async (argv) => {
        const host = argv.host;
        setGhHost(host);
        await runPrContext({ ...argv, host });
    },
};
