...
export function skillSettingStrings(settings, scope, key) {
    const value = settings.forScope(scope).settings.skills?.[key];
    return Array.isArray(value)
        ? value.filter((name) => typeof name === 'string')
        : [];
}
export function resolveSkillSettings(settings) {
    const hardDisabled = normalizeSkillNames(settings.merged.skills?.disabled);
    const defaultDisabled = normalizeSkillNames(settings.merged.skills?.defaultDisabled);
    ...
    for (const [scope, label] of lockedScopes) {
        for (const name of normalizeSkillNames(skillSettingStrings(settings, scope, 'disabled'))) {
...
export function updateWorkspaceSkillSettingLists(lists, skillName, enabled, defaultDisabled) {
    const normalizedName = skillName.trim().toLowerCase();
    const hadExplicitEnable = lists.enabled.some((name) => name.trim().toLowerCase() === normalizedName);
    if (enabled) {
        return {
            disabled: updateTarget(lists.disabled, skillName, false),
            enabled: defaultDisabled || hadExplicitEnable
 ...
 */
export function computeWorkspaceSkillListUpdates(workspaceDisabled, lockedNames, workspaceEnabled, toggles) {
    const previousDisabled = workspaceDisabled.filter((name) => !lockedNames.has(name.trim().toLowerCase()));
    ...
    for (const toggle of toggles) {
        if (toggle.wasEnabled === toggle.isEnabled)
            continue;
        next = updateWorkspaceSkillSettingLists(next, toggle.name, toggle.isEnabled, toggle.defaultDisabled);
    }
    return {
