...
import { BackgroundTaskViewProvider, useBackgroundTaskViewActions, useBackgroundTaskViewState, } from '../../contexts/BackgroundTaskViewContext.js';
...
function monitorEntry(overrides = {}) {
    return {
        kind: 'monitor',
        monitorId: 'mon-1',
...
function setup(initial, availableTerminalHeight = 30) {
    const handlers = [];
    ...
    const config = {
        getBackgroundTaskRegistry: () => ({
            ...
            get: (id) => {
                const match = currentEntries.find((e) => e.kind === 'agent' && e.agentId === id);
                return match;
            },
        ...
        getMonitorRegistry: () => ({
            ...
            get: (id) => {
                const match = currentEntries.find((e) => e.kind === 'monitor' && e.monitorId === id);
                return match;
            },
        }),
        getMemoryManager: () => ({
    ...
    function Harness() {
        ...
        return (_jsx(ConfigContext.Provider, { value: config, children: _jsxs(BackgroundTaskViewProvider, { config: config, children: [_jsx(Probe, { entriesSetter: setEntries }), _jsx(BackgroundTasksDialog, { availableTerminalHeight: availableTerminalHeight, terminalWidth: 80 })] }) }));
    }
    function Probe({ entriesSetter, }) {
        handle.current = {
    ...
    const { lastFrame } = render(_jsx(SettingsContext.Provider, { value: { merged: { general: {} } }, children: _jsx(Harness, {}) }));
    return {
        cancel,
        resume,
        abandon,
...
describe('BackgroundTasksDialog', () => {
    ...
    it('re-renders a paused workflow detail on the 1s tick and runs no tick for terminal entries', () => {
        ...
        try {
            ...
            expect(setIntervalSpy.mock.calls.filter((call) => call[1] === 1000)).toHaveLength(0);
    ...
    it.each([
        ['running', 'pause'],
        ['paused', 'resume'],
    ])('uses bare p to %s a workflow from detail mode', (status, action) => {
        ...
        expect(h.lastFrame()).toContain(action === 'pause' ? 'p pause (cooperative)' : 'p resume (cooperative)');
        h.pressKey({ sequence: 'p' });
    ...
    it('does not resume blocked paused tasks and surfaces the blocked reason', () => {
        ...
        expect(detailFrame).toContain('Legacy fork bootstrap transcript is missing.');
    ...
    describe('MonitorDetailBody render branches', () => {
        function openMonitorDetail(monitorOverrides = {}) {
        ...
        it('renders pid when defined, omits when undefined', () => {
            expect(openMonitorDetail({ pid: 4242 })).toContain('pid 4242');
            expect(openMonitorDetail()).not.toContain('pid ');
        });
        it('uses singular "1 event" / plural "N events"', () => {
            const f1 = openMonitorDetail({ eventCount: 1 });
            ...
            const f5 = openMonitorDetail({ eventCount: 5 });
            expect(f5).toContain('5 events');
        });
        it('renders droppedLines only when > 0', () => {
            expect(openMonitorDetail({ droppedLines: 0 })).not.toContain('dropped');
            expect(openMonitorDetail({ droppedLines: 3 })).toContain('3 dropped');
        });
        it('renders exitCode in subtitle when defined', () => {
            expect(openMonitorDetail({
                status: 'completed',
                exitCode: 0,
            })).toContain('exit 0');
            expect(openMonitorDetail({
    ...
    describe('WorkflowDetailBody budget chip (R2 #15)', () => {
        function openWorkflowDetail(entries) {
        ...
        it('renders the M/N token chip and capped per-phase totals', () => {
            const perPhase = new Map([['Plan', 3_500]]);
    ...
    it.each(['pausing', 'paused'])('keeps workflow detail open without save while %s', (status) => {
        const running = workflowEntry({
            status: 'running',
            script: 'await run();',
        });
        ...
        expect(h.lastFrame()).toContain(status === 'pausing' ? 'Pausing' : 'Paused');
        if (status === 'pausing') {
    ...
    it.each(['pausing', 'paused'])('marks the current phase of a %s workflow in detail view', (status) => {
        const h = setup([
            workflowEntry({
                status,
                phases: ['Plan', 'Build'],
    ...
    it.each(['pausing', 'paused'])('allows an active %s workflow to be stopped', (status) => {
        const h = setup([workflowEntry({ status })]);
        h.call(() => h.probe.current.actions.openDialog());
        ...
        expect(h.lastFrame()).toContain('x stop');
        h.pressKey({ sequence: 'x' });
        expect(h.workflowCancel).toHaveBeenCalledWith('wf_test1234', expect.any(Number));
    });
    ...
    it('routes a workflow approval response by approvalId from detail mode', () => {
        ...
        expect(h.workflowResolvePendingApproval).toHaveBeenCalledWith('wf_test1234', 'wfap-42', ToolConfirmationOutcome.Cancel, undefined);
    });
    describe('nested sub-agent display', () => {
        ...
        it('indents a nested child row with ↳ in the list', () => {
            ...
            expect(frame.indexOf('parent work')).toBeLessThan(frame.indexOf('child work'));
        });
        ...
        it('sorts the Sub-agents roster active-first so the cap cannot hide running children', () => {
            ...
            const doneChildren = [1, 2, 3, 4, 5].map((n) => entry({
                id: `done-${n}`,
                agentId: `done-${n}`,
    ...
    describe('subagent observability (issue #6569)', () => {
        ...
        it('keeps the live row visible with a full 10-row history and no transcript (short terminal)', () => {
            ...
            const h = setup([entry({ outputFile: '', recentActivities: activities })], 20);
            h.call(() => h.probe.current.actions.openDialog());
            h.call(() => h.probe.current.actions.enterDetail());
            const frame = (h.lastFrame() ?? '').replace(/\n\s*/g, ' ');
        ...
        it('renders the full transcript path (wraps instead of truncating)', () => {
            const longPath = '/home/runner/.qwen/projects/some-workspace-slug/subagents/2f9c1a7b-1234-4a5b-8c9d-abcdef012345/agent-general-purpose-call-9.jsonl';
...
//# sourceMappingURL=BackgroundTasksDialog.test.js.map