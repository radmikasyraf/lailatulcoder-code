...
import { shouldDisplayGoalStateCause, waitForGoalRuntime, } from './goal-runtime.js';
describe('waitForGoalRuntime', () => {
    it('allows Goal-less sessions when persistence is disabled', async () => {
        ...
        await expect(waitForGoalRuntime({ getGoalRuntimeReady })).resolves.toBeUndefined();
    ...
    it('does not hide malformed or unsupported persisted Goal state', async () => {
        ...
        await expect(waitForGoalRuntime({ getGoalRuntimeReady })).rejects.toBe(failure);
    });
...
//# sourceMappingURL=goal-runtime.test.js.map