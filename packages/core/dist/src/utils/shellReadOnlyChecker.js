...
import { classifyAwkCommandSafety, classifySedCommandSafety, hasShellBraceExpansion, } from './shell-safety-rules.js';
...
const GIT_EXTERNAL_HELPER_OPTION = /(?:^--(?:ext-diff|filters|show-signature|textconv|open-files-in-pager)(?:=|$)|%G[?GKFPST])/;
const SAFE_SED_OPTION = /^(?:-[nErsuz]|--(?:quiet|silent))$/;
const ENV_ASSIGNMENT_REGEX = /^[A-Za-z_][A-Za-z0-9_]*=/;
const MALFORMED_CONTROL_OPERATOR = /(?:^|[({])\s*(?:&&|\|\||\|&|[|;&])|(?:&&|\|\||\|&|[|;&])\s+(?:&&|\|\||\|&|[|;&])|(?!(?:&&|\|\||\|&))[|;&]{2}|[|;&]{3,}|(?:\|&?|&&|\|\|)\s*[)}]*\s*$/;
...
function evaluateSedCommand(tokens) {
    const [, ...rest] = tokens;
    for (const token of rest) {
        if (['-i', '-I'].some((prefix) => token.startsWith(prefix)) ||
            token === '--in-place' ||
...
function evaluateGitRemoteArgs(args) {
    ...
    if (action && !['show', 'get-url'].includes(action))
        return false;
...
function evaluateGitCommand(tokens) {
    ...
    if (options.some((arg) => GIT_EXTERNAL_HELPER_OPTION.test(arg)) ||
        (subcommand === 'grep' && options.some((arg) => arg.startsWith('-O'))))
        return false;
    if (options.some((arg) => /^(?:--help|--version)$/i.test(arg)))
        return false;
...
function evaluateShellSegment(segment) {
    ...
    if (root !== normalizedRoot)
        return false;
    if (/^(awk|find|git|sed)$/.test(normalizedRoot) &&
        args.some((arg) => !arg || arg.includes('\0') || hasShellBraceExpansion(arg)))
        return false;
...
export function isShellCommandReadOnly(command) {
    ...
    if (MALFORMED_CONTROL_OPERATOR.test(command))
        return false;
...
//# sourceMappingURL=shellReadOnlyChecker.js.map