...
export function getCommandSourceBadge(command) {
...
export function getCommandSourceGroup(command) {
    switch (command.source) {
        case 'builtin-command':
...
export function formatSupportedModes(command) {
...
export function getCommandDisplayName(command, options = {}) {
...
export function getCommandSubcommandNames(command) {
    return (command.subCommands
        ?.filter((subCommand) => !subCommand.hidden)
        .map((subCommand) => subCommand.name) ?? []);
}
export function formatCommandSourceLabel(command) {
