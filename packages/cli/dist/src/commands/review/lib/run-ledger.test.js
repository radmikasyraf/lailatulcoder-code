...
import { mkdtempSync, mkdirSync, lstatSync, realpathSync, rmSync, statSync, symlinkSync, writeFileSync, readFileSync, utimesSync, chmodSync, existsSync, } from 'node:fs';
import { tmpdir } from 'node:os';
import { join } from 'node:path';
import { appendRunSession, priorSessionEntries, priorSessionIds, sessionEntryCount, ledgerResumeCount, runSessionsPath, readResumeMarker, recordResume, recordRestart, resumeMarkerPath, RESUME_MAX, currentSessionEntry, } from './run-ledger.js';
let root;
let plan;
const envOf = (sessionId) => ({
 ...
 */
function authorize(sessionId, atMs = Date.now()) {
...
describe('appendRunSession / priorSessionIds', () => {
    ...
    it('drops a traversal id from the MARKER even with a valid fence', () => {
        ...
        const raw = JSON.parse(readFileSync(resumeMarkerPath(plan), 'utf8'));
        raw.resumes.push({
            sessionId: '../../etc',
            atMs: Date.now(),
...
describe('resume marker', () => {
    ...
    it('reads an unknown schemaVersion as the empty history', () => {
        ...
        writeFileSync(resumeMarkerPath(plan), JSON.stringify({ schemaVersion: 2, resumes: [], restarts: [] }));
        expect(readResumeMarker(plan)).toEqual({
            schemaVersion: 1,
            resumes: [],
            restarts: [],
...
describe('the properties the threat model rests on', () => {
    ...
    it('keeps an entry whose plan mtime drifted by a last-place unit', () => {
        ...
        appendRunSession(plan, envOf('S1'));
        const raw = JSON.parse(readFileSync(runSessionsPath(plan), 'utf8'));
        raw[0]['planMtimeMs'] = raw[0]['planMtimeMs'] - 0.001;
        writeFileSync(runSessionsPath(plan), JSON.stringify(raw));
        authorize('S2');
        expect(priorSessionIds(plan, envOf('S2'))).toEqual(['S1']);
    });
    it('still drops an entry written against a genuinely different plan', () => {
        ...
        appendRunSession(plan, envOf('S1'));
        const raw = JSON.parse(readFileSync(runSessionsPath(plan), 'utf8'));
        raw[0]['planMtimeMs'] = raw[0]['planMtimeMs'] - 50;
        writeFileSync(runSessionsPath(plan), JSON.stringify(raw));
        authorize('S2');
        expect(priorSessionIds(plan, envOf('S2'))).toEqual([]);
    });
    it('reads an over-budget ledger as empty, before parsing it', () => {
        ...
        writeFileSync(runSessionsPath(plan), JSON.stringify([
            {
                sessionId: 'S1',
                atMs: Date.now(),
                planMtimeMs: statSync(plan).mtimeMs,
    ...
    it.skipIf(process.platform === 'win32')('writes the marker through a planted symlink without following it', () => {
...
describe('plant shapes — heal what cannot be legitimate, preserve what can', () => {
    ...
    it('a backdated flood of 65 distinct plants cannot evict the genuine entry', () => {
        ...
        expect(currentSessionEntry(plan, envOf('S-genuine'))?.sessionId).toBe('S-genuine');
        expect(sessionEntryCount(plan)).toBe(64);
    });
});
//# sourceMappingURL=run-ledger.test.js.map