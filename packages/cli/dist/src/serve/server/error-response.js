 ...
 */
import { emitDaemonLog, InvalidSessionTranscriptCursorError, recordDaemonBridgeError, recordDaemonError, SessionIdCaseConflictError, SessionTranscriptPageTooLargeError, SessionTranscriptSnapshotUnavailableError, SessionTranscriptTooLargeError, SessionWriterError, TrustGateError, } from '@lailatul-coder/lailatul-coder-core';
import { restoreRetryAfterSeconds } from '@lailatul-coder/acp-bridge/sessionRestoreTimeout';
import { writeStderrLine } from '../../utils/stdioHelpers.js';
import { BranchWhilePromptActiveError, BridgeChannelQuarantinedError, CancelSentinelCollisionError, CdWhilePromptActiveError, InvalidClientIdError, InvalidPermissionOptionError, InvalidRewindTargetError, InvalidSessionMetadataError, InvalidSessionScopeError, McpServerNotFoundError, McpServerRestartFailedError, PermissionForbiddenError, PermissionPolicyNotImplementedError, PromptQueueFullError, RestoreInProgressError, SessionRestoreTimeoutError, SessionArtifactAuthorizationError, SessionArchivedError, SessionArchivingError, SessionBusyError, SessionConflictError, SessionLimitExceededError, SessionNotArchivedError, SessionNotFoundError, SessionShellClientRequiredError, SessionShellDisabledError, WorkspaceInitConflictError, WorkspaceInitPathEscapeError, WorkspaceInitRaceError, WorkspaceInitSymlinkError, WorkspaceMismatchError, WorkspaceDrainingError, TotalSessionLimitExceededError, } from '../acp-session-bridge.js';
...
function bridgeErrorExtraContext(ctx) {
    const extra = {};
...
function recordExpectedBridgeError(err, ctx, daemonLog) {
...
export function sendPermissionVoteError(res, err, ctx, daemonLog) {
 ...
 */
export function sendBridgeError(res, err, ctx, daemonLog) {
    ...
    if (err instanceof WorkspaceMismatchError) {
        ...
        // readability.
        writeStderrLine(`LailatulCoder serve: workspace_mismatch (POST /session): ` +
    ...
    if (err && typeof err === 'object') {
        ...
        if (data && typeof data === 'object') {
            ...
            if (kind === 'session_writer_conflict' ||
    ...
    emitDaemonLog('Daemon bridge error.', {
        ...
        'error.message': (err instanceof Error ? err.message : String(err)).slice(0, 1024),
    });
    if (daemonLog) {
        daemonLog.error(err instanceof Error ? err.message : String(err), err instanceof Error ? err : undefined, {
            ...(ctx?.route ? { route: ctx.route } : {}),
            ...(ctx?.sessionId ? { sessionId: ctx.sessionId } : {}),
            ...extraContext,
        });
        ...
        writeStderrLine(`LailatulCoder serve: bridge error${ctxStr}: ${err instanceof Error ? (err.stack ?? err.message) : String(err)}`);
    }
