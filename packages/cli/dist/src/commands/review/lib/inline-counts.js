...
export function severityOf(c) {
    const body = typeof c?.body === 'string' ? c.body.replace(LEADING_INVISIBLE_RE, '') : '';
    if (body.startsWith(CRITICAL_PREFIX))
        return 'critical';
    if (body.startsWith(SUGGESTION_PREFIX))
        return 'suggestion';
    return null;
}
/**
 ...
 */
export function carriedClaimLine(body) {
 ...
 * neither marker.
 */
export function markerStrippedBody(body) {
...
export function countInlineFindings(comments) {
...
export function stripSeverityPrefix(body) {
    ...
    for (;;) {
        ...
        const prefix = severity === 'critical' ? CRITICAL_PREFIX : SUGGESTION_PREFIX;
        const rest = visible
            .trimStart()
            .slice(prefix.length)
            .replace(/^[ \t]*[:：]?[ \t]*/, '');
        if (rest.replace(ALL_INVISIBLE_RE, '') === '')
            return '';
        current = rest;
    }
}
 ...
 */
export function unmarkedComments(comments) {
    const out = [];
...
//# sourceMappingURL=inline-counts.js.map