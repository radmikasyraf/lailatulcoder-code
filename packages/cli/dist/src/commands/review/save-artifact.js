...
import { buildReport, validateFindings, } from '../../utils/findings.js';
...
// absolute paths that still sit under the main project's `.lailatulcoder/tmp/`.
//
...
// LailatulCoder-home). An ambient variable that is wrong 100% of the time it is
...
function validateVerdict(value) {
    ...
    if (typeof deferredCount !== 'number' ||
        ...
        deferredCount < 0) {
        throw new Error('Composed verdict.deferredCount must be a non-negative integer.');
    }
    ...
    if (!Array.isArray(floorEnforced) ||
        floorEnforced.some((i) => typeof i !== 'number' || !Number.isInteger(i) || i < 0)) {
        throw new Error('Composed verdict.floorEnforced must be an array of non-negative integers.');
    }
    ...
    if (!postedAbsent && postedInline === undefined) {
        throw new Error('Composed verdict.postedInline must be a non-negative integer.');
    }
    ...
    if (!freshAbsent && postedFresh === undefined) {
        throw new Error('Composed verdict.postedFresh must be a non-negative integer.');
    }
    ...
    if (typeof trim['sections'] !== 'number' ||
        !Number.isInteger(trim['sections']) ||
        trim['sections'] < 0 ||
        typeof trim['deferralList'] !== 'boolean' ||
        typeof trim['fold'] !== 'boolean' ||
        typeof trim['truncated'] !== 'boolean') {
        throw new Error('Composed verdict.bodyTrim must carry a non-negative integer `sections` and boolean `deferralList` / `fold` / `truncated`.');
    }
    return {
        bodyTrim: {
...
export const saveArtifactCommand = {
    ...
    builder: (yargs) => yargs
        ...
        type: 'string',
        demandOption: true,
        describe: 'Output path under .lailatulcoder/reviews/',
    })
