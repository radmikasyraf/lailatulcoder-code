...
import { isQwenQuotaExceededError, isQuotaExhaustedError, formatQuotaExhaustedMessage, } from './quotaErrorDetection.js';
...
function defaultShouldRetry(error, extraRetryErrorCodes) {
    ...
    return (classifyRetryError(error, { extraRetryErrorCodes }).kind === 'transport');
}
/**
 ...
 */
export function isTransientCapacityError(error) {
    const status = getErrorStatus(error);
 ...
 */
export function isUnattendedMode() {
...
export async function retryWithBackoff(fn, options) {
    ...
    const cleanOptions = options
        ...
        : {};
    const { maxAttempts, initialDelayMs, maxDelayMs, authType, extraRetryErrorCodes, shouldRetryOnError, shouldRetryOnContent, persistentMode, persistentMaxBackoffMs, persistentCapMs, heartbeatIntervalMs, heartbeatFn, signal, onRetry, } = {
    ...
    while (attempt < maxAttempts) {
        ...
        try {
            const result = await retryContext.run({ attempt: iterationCount, retryTotalDelayMs, requestSetupMs }, () => fn());
            if (shouldRetryOnContent &&
                ...
                debugLogger.warn(`Attempt ${iterationCount}: response rejected by content check. ` +
            ...
            if (authType === AuthType.lailatulcoder_OAUTH && isQwenQuotaExceededError(error)) {
                debugLogger.error('Qwen OAuth quota exceeded, fast-failing', retryDiagnostics, error);
                throw new Error(`Qwen OAuth free tier has been discontinued as of 2026-04-15.\n\n` +
                    `To continue using LailatulCoder Ai, try one of these alternatives:\n` +
            ...
            if (isQuotaExhaustedError(error)) {
                debugLogger.error('Quota exhausted, fast-failing', retryDiagnostics, error);
            ...
            if (shouldPersist) {
                ...
                debugLogger.warn(`[Persistent] Attempt ${reportedAttempt} failed with status ${errorStatus ?? 'unknown'}. ` +
                    `Retrying in ${Math.ceil(delayMs / 1000)}s...`, retryDiagnostics, error);
                ...
                if (!signal?.aborted) {
                    try {
                        ...
                        debugLogger.warn(`onRetry callback threw (swallowed): ${cbError instanceof Error ? cbError.message : String(cbError)}`);
                    }
                }
                ...
                if (retryAfterMs !== null && retryAfterMs > 0) {
                    ...
                    logRetryAtStatusLevel(`Attempt ${attempt} failed with status ${errorStatus ?? 'unknown'}. Retrying after explicit delay of ${retryAfterMs}ms...`, retryDiagnostics, error, errorStatus);
                }
                else {
                    ...
                    currentDelay = Math.min(maxDelayMs, currentDelay * 2);
                    logRetryAttempt(attempt, error, retryDiagnostics, errorStatus, actualDelayMs);
                }
...
function logRetryAttempt(attempt, error, retryDiagnostics, errorStatus, delayMs) {
    const backoff = delayMs !== undefined
        ? `Retrying with backoff in ${Math.ceil(delayMs / 1000)}s...`
