...
import { BTW_MAX_INPUT_LENGTH, buildBtwCacheSafeParams, buildBtwPrompt, runForkedAgent, } from '@lailatul-coder/lailatul-coder-core';
function formatBtwError(error) {
    return t('Failed to answer btw question: {{error}}', {
        error: error instanceof Error ? error.message : String(error || 'Unknown error'),
    });
}
function getBtwCacheSafeParams(context) {
 ...
 */
async function askBtw(context, question, abortSignal) {
    const { config } = context.services;
    if (!config)
...
export const btwCommand = {
    ...
    action: async (context, args) => {
        ...
        if (executionMode !== 'interactive') {
            try {
                const answer = await askBtw(context, question, context.abortSignal ?? new AbortController().signal);
...
//# sourceMappingURL=btwCommand.js.map