 ...
 */
import { HookEventName, DefaultHookOutput, PreToolUseHookOutput, PostToolUseHookOutput, PostToolUseFailureHookOutput, UserPromptExpansionHookOutput, PostToolBatchHookOutput, StopHookOutput, PermissionRequestHookOutput, isToolArtifactLike, } from './types.js';
...
export class HookAggregator {
                ...
                for (const [key, value] of Object.entries(output.hookSpecificOutput)) {
                    if (key === 'artifacts' && Array.isArray(value)) {
                        ...
                        if (validArtifacts.length !== value.length) {
                            debugLogger.warn('Dropped malformed hookSpecificOutput.artifacts entries');
                        }
                    }
                    else if (key === 'artifacts') {
                        debugLogger.warn('Dropped malformed hookSpecificOutput.artifacts; expected array');
                    }
     ...
     */
    mergeTerminalSequences(outputs, merged) {
