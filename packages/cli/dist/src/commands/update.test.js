...
const resolveUpdateCommand = vi.fn((updateCommand, latestVersion) => updateCommand.replace('@latest', `@${latestVersion}`));
const formatUpdateInstructions = vi.fn((installationInfo, latestVersion) => {
...
const resolveLanguageSetting = vi.fn((language) => language || 'auto');
...
vi.mock('../i18n/index.js', () => ({
    ...
    t: (key, params) => key.replace(/\{\{(\w+)\}\}/g, (_, param) => params?.[param] ?? `{{${param}}}`),
}));
...
function settings(enableAutoUpdate) {
    return {
        merged: {
...
describe('update command', () => {
    ...
    it('prints the package-manager update command even when auto-update is disabled', async () => {
        ...
        expect(writeStdoutLine).toHaveBeenCalledWith('Run the following to update:');
        expect(writeStdoutLine).toHaveBeenCalledWith('  npm install -g @lailatul-coder/lailatul-coder@1.2.3');
    });
    it('sets a non-zero exit code when a standalone update fails', async () => {
        ...
        await updateCommand.handler(updateArgs);
        expect(performStandaloneUpdate).toHaveBeenCalledWith('/tmp/lailatul-coder', '1.2.3');
    ...
    it('updates standalone installs even when auto-update is disabled', async () => {
        ...
        expect(getInstallationInfo).toHaveBeenCalledWith(expect.any(String), true);
        expect(performStandaloneUpdate).toHaveBeenCalledWith('/tmp/lailatul-coder', '1.2.3');
        expect(writeStdoutLine).toHaveBeenCalledWith('Update successful! The new version will be used on your next run.');
    });
    it('does not print generic fallback when installation info has updateMessage', async () => {
        ...
        await updateCommand.handler(updateArgs);
        expect(writeStdoutLine).toHaveBeenCalledWith('Running via npx, update not applicable.');
        expect(writeStdoutLine).not.toHaveBeenCalledWith('Manual update required. Please reinstall LailatulCoder Ai.');
    });
    it('prints the update message when no update command is available', async () => {
        ...
        await updateCommand.handler(updateArgs);
        expect(writeStdoutLine).toHaveBeenCalledWith('Running from a local git clone. Please update with "git pull".');
    });
    ...
    it('prints deferred message on standalone update', async () => {
        ...
        await updateCommand.handler(updateArgs);
        expect(writeStdoutLine).toHaveBeenCalledWith('Downloading update...');
        expect(writeStdoutLine).toHaveBeenCalledWith('Update downloaded. It will be applied after you exit this session.');
    });
    it('prints the current version when no update is available', async () => {
        ...
        await updateCommand.handler(updateArgs);
        expect(writeStdoutLine).toHaveBeenCalledWith('LailatulCoder Ai 1.0.0 is up to date!');
    ...
    it('sets a non-zero exit code when the update check fails', async () => {
        ...
        await updateCommand.handler(updateArgs);
        expect(writeStderrLine).toHaveBeenCalledWith('Failed to check for updates (registry error). Please check your network or registry configuration.');
        expect(process.exitCode).toBe(1);
        expect(getInstallationInfo).not.toHaveBeenCalled();
    });
    it('sets a non-zero exit code when the update check is skipped', async () => {
        ...
        await updateCommand.handler(updateArgs);
        expect(writeStderrLine).toHaveBeenCalledWith('Unable to check for updates: development mode');
        expect(process.exitCode).toBe(1);
        expect(getInstallationInfo).not.toHaveBeenCalled();
    });
});
