...
describe('curator command', () => {
    ...
    it('shows status from the bare parent command', async () => {
        ...
        expect(result.content).toContain('auto-skill-old');
    });
    it('runs a non-mutating preview', async () => {
        ...
        const runCommand = curatorCommand.subCommands.find((command) => command.name === 'run');
        ...
        expect(result.content).toContain('Archive candidates:\n  auto-skill-old');
        expect(result.content).toContain('Skipped archive collisions:\n  auto-skill-collision');
    });
    ...
    it('keeps a successful live run successful when cache refresh fails', async () => {
        ...
        const result = await runCommand.action(context, '');
        expect(result).toMatchObject({ type: 'message', messageType: 'info' });
        expect(result.content).toContain('Archived skills:\n  auto-skill-old');
    });
    it('restores an archived directory and refreshes skill discovery', async () => {
        const restoreCommand = curatorCommand.subCommands.find((command) => command.name === 'restore');
    ...
    it('keeps a successful restore successful when cache refresh fails', async () => {
        ...
        expect(mocks.restore).toHaveBeenCalledWith('/project', 'auto-skill-old');
        expect(result).toMatchObject({ type: 'message', messageType: 'info' });
        expect(result.content).toContain('Restored auto-skill: auto-skill-old');
    });
    it('pins and unpins a managed auto-skill', async () => {
        const pinCommand = curatorCommand.subCommands.find((command) => command.name === 'pin');
        const unpinCommand = curatorCommand.subCommands.find((command) => command.name === 'unpin');
        await pinCommand.action(context, 'auto-skill-old');
        await unpinCommand.action(context, 'auto-skill-old');
        expect(mocks.setPinned).toHaveBeenNthCalledWith(1, '/project', 'auto-skill-old', true);
        expect(mocks.setPinned).toHaveBeenNthCalledWith(2, '/project', 'auto-skill-old', false);
    });
    it('reports an error when reading status fails', async () => {
        ...
        expect(result.content).toContain('state unreadable');
    });
    it('reports an error and skips refresh when a live run fails', async () => {
        ...
        const result = await runCommand.action(context, '');
        expect(result).toMatchObject({ type: 'message', messageType: 'error' });
        expect(result.content).toContain('run exploded');
        expect(mocks.refreshCache).not.toHaveBeenCalled();
    });
    it('reports an error and skips refresh when restore fails', async () => {
        ...
        const result = await restoreCommand.action(context, 'auto-skill-old');
        expect(result).toMatchObject({ type: 'message', messageType: 'error' });
        expect(result.content).toContain('restore exploded');
        expect(mocks.refreshCache).not.toHaveBeenCalled();
    });
    it('reports an error when pinning fails', async () => {
        ...
        expect(result).toMatchObject({ type: 'message', messageType: 'error' });
        expect(result.content).toContain('pin exploded');
    });
    ...
    ])('blocks mutations but preserves read-only commands in %s', async (_name, safeMode, trustedFolder) => {
...
//# sourceMappingURL=curator-command.test.js.map