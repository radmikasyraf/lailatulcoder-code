...
import { assembleSections, narrowToDelta, selectNarrowing, } from './narrow-diff.js';
...
const captureBytes = (from, to) => execFileSync('git', [...PINNED_DIFF_CONFIG, 'diff', ...PINNED_DIFF_FLAGS, from, to], { cwd: repo, maxBuffer: 1 << 28, env });
const capture = (from, to) => captureBytes(from, to).toString('utf8');
...
const lines = (n, tag = 'L') => Array.from({ length: n }, (_, i) => `${tag}${i + 1}`).join('\n') + '\n';
/**
 ...
 */
const commitModeChange = (msg, file, exec, files) => {
    for (const [name, body] of Object.entries(files)) {
        writeFileSync(join(repo, name), body);
    }
 ...
 */
const everyLineIsDisplayed = (narrowed, full) => {
...
describe('narrowToDelta on real-git captures', () => {
    it('keeps only the PR hunks the anchor round did not already cover', () => {
        ...
        const narrowed = narrowToDelta(captureBytes(base, head), deltaBytes)?.toString('utf8') ??
            null;
    ...
    it('refuses to narrow a capture that does not round-trip through utf8', () => {
        ...
        expect(narrowToDelta(invalid, Buffer.from('diff --git a/f b/f\n@@ -1,1 +1,1 @@\n+ok\n', 'utf8'))).toBeNull();
    });
    ...
    it('falls back rather than scoping when the captures key a change differently', () => {
        ...
        git('rm', '-q', 'new.ts');
        writeFileSync(join(repo, 'other.ts'), lines(8, 'T').replace('T3\n', 'T3-EDIT\n'));
        git('add', '-A');
    ...
    it('refuses to narrow a rewrite the delta keys as a rename', () => {
        ...
        writeFileSync(join(repo, 'rw-other.ts'), lines(8, 'T').replace('T3\n', 'T3-EDIT\n'));
        git('add', '-A');
    ...
    it('emits the full section whole for a hunk-less delta touch', () => {
        ...
        const full = capture(base, 'HEAD');
        const narrowed = narrowToDelta(captureBytes(base, 'HEAD'), captureBytes(anchor, 'HEAD'))?.toString('utf8') ?? null;
        expect(narrowed).not.toBeNull();
    ...
    it('carries a post-anchor rename whose hunks all miss the full capture', () => {
        ...
        writeFileSync(join(repo, 'rn-new.ts'), lines(20, 'O').replace('O15\n', 'O15-EDIT\n'));
    ...
    it('emits the rename header and matching hunks when one round does both', () => {
        ...
        writeFileSync(join(repo, 're-new.ts'), lines(20, 'O')
            .replace('O3\n', 'O3-EDIT\n')
            .replace('O15\n', 'O15-EDIT\n'));
        git('add', '-A');
    ...
    it('narrows between FILES, never within one — the invariant, stated once', () => {
        ...
        const full = capture(base, head);
        const narrowed = narrowToDelta(captureBytes(base, head), captureBytes(anchor, head)).toString('utf8');
    ...
    it('carries a change the captures display at disjoint positions under disjoint texts', () => {
        ...
        // ledger.
        const runOf = (n) => Array.from({ length: n }, () => 'R').join('\n') + '\n';
    ...
    it('emits a whole-file deletion the delta performs too', () => {
        ...
        git('rm', '-q', 'f.ts');
        writeFileSync(join(repo, 'g.ts'), lines(10, 'G').replace('G2\n', 'G2-EDIT\n').replace('G7\n', 'G7-EDIT\n'));
        git('add', '-A');
    ...
    it('carries a binary-file delta section the full capture also holds', () => {
        ...
        writeFileSync(join(repo, 'bin.dat'), Buffer.from([0x47, 0x49, 0x00, 0x01, 0xff, 0x00]));
        const base = commit('binary base', { 'bin-text.ts': lines(8, 'T') });
        const anchor = commit('binary round 1', {
            'bin-text.ts': lines(8, 'T').replace('T3\n', 'T3-EDIT\n'),
        });
        writeFileSync(join(repo, 'bin.dat'), Buffer.from([0x47, 0x49, 0x00, 0x02, 0xff, 0x00, 0x03]));
        git('add', '-A');
        ...
        expect(deltaBytes.toString('utf8')).toContain('Binary files a/bin.dat and b/bin.dat differ');
        const narrowed = narrowToDelta(captureBytes(base, 'HEAD'), deltaBytes)?.toString('utf8') ??
            null;
        expect(narrowed).not.toBeNull();
    ...
    it('narrows to a subset that still parses as a diff', () => {
        ...
        expect(() => git('apply', '--check', '--reverse', 'narrowed.patch')).not.toThrow();
        expect(everyLineIsDisplayed(narrowed, full)).toBe(true);
...
describe('the two halves narrowToDelta composes', () => {
    ...
    it('selects the touched paths, and only those the full capture carries', () => {
        const selection = selectNarrowing(captureBytes(base, head), captureBytes(anchor, head));
    ...
    it('emits whatever subset it is asked for, out of the full capture', () => {
        ...
        expect(assembleSections(selection, selection.touched)?.toString('utf8')).toBe(narrowToDelta(captureBytes(base, head), captureBytes(anchor, head))?.toString('utf8'));
        ...
        // for.
        const widened = assembleSections(selection, new Set(['a.ts', 'b.ts'])).toString('utf8');
...
//# sourceMappingURL=narrow-diff.integration.test.js.map