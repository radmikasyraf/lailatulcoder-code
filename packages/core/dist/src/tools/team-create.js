...
import { sanitizeName, formatAgentId, createTeamFile, getTeamDir, getTasksDir, tryReclaimStaleTeam, } from '../agents/team/teamHelpers.js';
...
class TeamCreateInvocation extends BaseToolInvocation {
    ...
    async execute() {
        ...
        if (this.config.getArenaManager()) {
            const msg = 'Cannot create a team while an Arena session is active. ' +
        ...
        try {
            ...
            if (isNodeError(err) && err.code === 'EEXIST') {
                ...
                if (!reclaimed) {
                    const msg = `Team "${teamName}" already exists and appears to be ` +
        ...
        await backend.init();
        const manager = new TeamManager(backend, teamFile, this.config.getSubagentManager(), {
        ...
        const llmContent = `Team "${teamName}" created.` +
...
export class TeamCreateTool extends BaseDeclarativeTool {
    ...
    constructor(config) {
        super(TeamCreateTool.Name, ToolDisplayNames.TEAM_CREATE, `# TeamCreate
...
- **Custom agents** defined in \`.lailatulcoder/agents/\` may have their own tool restrictions. Check their descriptions to understand what they can and cannot do.
...
This creates:
- A team file at \`~/.lailatulcoder/teams/{team-name}/config.json\`
- A corresponding task list directory at \`~/.lailatulcoder/tasks/{team-name}/\`
...
- **Team config location**: \`~/.lailatulcoder/teams/{team-name}/config.json\`
...
\`\`\`
Use the Read tool to read ~/.lailatulcoder/teams/{team-name}/config.json
\`\`\`

## Task List Coordination

Teams share a task list that all teammates can access at \`~/.lailatulcoder/tasks/{team-name}/\`.
...
- If you are an agent in the team, the system will automatically send idle notifications to the team lead when you stop.`, Kind.Other, {
            type: 'object',
            properties: {
                team_name: {
                    type: 'string',
