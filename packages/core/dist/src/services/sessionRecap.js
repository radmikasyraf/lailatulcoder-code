...
const RECAP_USER_PROMPT = 'Generate the recap now. Wrap it in <recap>...</recap>. Nothing outside the tags.';
 ...
 */
export async function generateSessionRecap(config, abortSignal) {
    try {
        ...
        if (fullHistory.length < 2) {
            debugLogger.debug(`recap skipped: history too short (${fullHistory.length} messages)`);
            return null;
        }
        ...
        debugLogger.debug(`recap: sending side-query with ${recentHistory.length} messages`);
        ...
        debugLogger.warn(`Recap generation failed: ${err instanceof Error ? err.message : String(err)}`);
        return null;
    }
}
/**
...
//# sourceMappingURL=sessionRecap.js.map