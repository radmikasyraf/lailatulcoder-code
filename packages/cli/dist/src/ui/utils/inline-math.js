...
export function unescapeMarkdownDollars(text) {
    return text.replace(/(\\+)\$/g, (_match, slashes) => `${'\\'.repeat(Math.floor(slashes.length / 2))}$`);
}
export function unescapeMarkdownBeforeMath(text) {
...
function readInlineMathSpan(text, index) {
    ...
    if (content.length > INLINE_MATH_MAX_CHARS ||
        /\s$/.test(content) ||
        /[\w$]/.test(following)) {
        return null;
    }
...
export function findNextInlineMath(text, fromIndex = 0) {
...
export function* mergeInlineMathMatches(text, markupRegex, enableInlineMath = true) {
...
export function findInlineMathExpressions(text) {
    const expressions = [];
...
export function readInlineMathSpanAt(text, index) {
