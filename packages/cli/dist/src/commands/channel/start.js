...
import { AcpBridge, ChannelLoopScheduler, ChannelLoopStore, SessionRouter, } from '@lailatul-coder/channel-base';
...
function writeServiceInfoOrExit(channels, cleanup) {
    try {
        ...
        if (isFileExistsError(err)) {
            writeStderrLine('Error: Channel service was started concurrently. Use "LailatulCoder channel status" to inspect it.');
            process.exit(1);
        }
        throw err;
    }
...
function checkDuplicateInstance() {
    ...
    if (existing) {
        if (existing.owner === 'serve') {
            writeStderrLine(`Error: Channel service is managed by LailatulCoder serve (PID ${existing.pid}, started ${existing.startedAt}).`);
            writeStderrLine('Stop the LailatulCoder serve process to stop managed channels.');
            process.exit(1);
        }
        writeStderrLine(`Error: Channel service is already running (PID ${existing.pid}, started ${existing.startedAt}).`);
        writeStderrLine('Use "LailatulCoder channel stop" to stop it first.');
        process.exit(1);
    }
}
