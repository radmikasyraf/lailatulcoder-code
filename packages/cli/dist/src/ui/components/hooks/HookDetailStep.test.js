...
import { HookEventName, HooksConfigSource, HookType, } from '@lailatul-coder/lailatul-coder-core';
...
function makeConfig(command, source = HooksConfigSource.User, matcher = '*') {
    return {
        config: { command, type: HookType.Command },
        source,
        sourceDisplay: source === HooksConfigSource.User ? 'User Settings' : 'Local Settings',
        matcher,
        enabled: true,
    };
}
function makeHookInfo(groups, opts = {}) {
...
describe('HookDetailStep', () => {
    ...
    it('renders the "Event - Matchers" title', () => {
        ...
        const { lastFrame } = render(_jsx(HookDetailStep, { hook: hook, selectedIndex: 0 }));
        expect(lastFrame()).toContain(`${HookEventName.PreToolUse} - Matchers`);
    });
    it('renders event description when present', () => {
        const hook = makeHookInfo([{ matcher: '*', configs: [makeConfig('/x.sh')] }], { description: 'desc-for-event' });
        const { lastFrame } = render(_jsx(HookDetailStep, { hook: hook, selectedIndex: 0 }));
    ...
    it('renders inline exit code descriptions', () => {
        ...
        const out = render(_jsx(HookDetailStep, { hook: hook, selectedIndex: 0 })).lastFrame() ??
            '';
        expect(out).toContain('Exit code');
        expect(out).toContain('Success');
        expect(out).toContain('Block');
        const hook2 = makeHookInfo([{ matcher: '*', configs: [makeConfig('/x.sh')] }], {
            exitCodes: [{ code: 'Other', description: 'other-desc' }],
        });
        const out2 = render(_jsx(HookDetailStep, { hook: hook2, selectedIndex: 0 })).lastFrame() ??
            '';
        ...
        const hook = makeHookInfo([
            {
                matcher: 'Bash',
                configs: [
                    makeConfig('/very-specific-command.sh', HooksConfigSource.User, 'Bash'),
                ],
            },
        ]);
        const out = render(_jsx(HookDetailStep, { hook: hook, selectedIndex: 0 })).lastFrame() ??
    ...
    it('places the selection arrow on the selected matcher row', () => {
        ...
        const out = render(_jsx(HookDetailStep, { hook: hook, selectedIndex: 1 })).lastFrame() ??
            '';
...
//# sourceMappingURL=HookDetailStep.test.js.map