 ...
 */
// `LailatulCoder review comment-body`: fetch one comment's body. The pr-context file
...
import { COMMENT_KINDS } from './lib/platform/types.js';
import { writeStdoutLine, writeStderrLineSafe, } from '../../utils/stdioHelpers.js';
const COMMENT_KIND_CHOICES = [...COMMENT_KINDS];
export function runCommentBody(args) {
    ...
    if (args.kind === 'review' && args.prNumber === undefined) {
        throw new TypeError('--kind review needs --pr (review bodies are addressed per-PR)');
    }
    ...
    const body = platform.getCommentBody(args.kind, args.id, args.repo, args.prNumber);
    if (args.out !== undefined) {
...
export const commentBodyCommand = {
    command: 'comment-body <id>',
    describe: 'Print one comment body — the fetch a pr-context truncation note names',
    builder: (yargs) => yargs
        .positional('id', {
        type: 'number',
        demandOption: true,
        describe: 'The comment id (a review id, inline-comment id, or issue-comment id)',
    })
        .option('kind', {
        type: 'string',
        choices: COMMENT_KIND_CHOICES,
        demandOption: true,
        describe: 'Which collection the id belongs to: a review summary, an inline (diff) comment, or an issue-level comment',
    })
        .option('pr', {
        type: 'number',
        describe: 'The PR number — required with --kind review (GitHub), and with every kind on Aone (comment bodies are addressed per-MR)',
    })
        .option('repo', {
        type: 'string',
        demandOption: true,
        ...
        .option('out', {
        type: 'string',
        describe: 'Write the body to this file instead of stdout (for tails too long for one shell preview)',
    }),
    handler: (argv) => {
        const id = argv['id'];
        ...
        if (id === undefined ||
            ...
            writeStderrLineSafe(`comment-body: id and --pr must be positive integers, got ${JSON.stringify(argv['id'])} / ${JSON.stringify(argv['pr'])}`);
            process.exitCode = 2;
            return;
        }
        ...
        const kindRaw = argv['kind'];
        const kind = typeof kindRaw === 'string' &&
            COMMENT_KINDS.includes(kindRaw)
            ? kindRaw
            : undefined;
        if (kind === undefined) {
            writeStderrLineSafe(`comment-body: --kind must be a single value of ${COMMENT_KINDS.join('/')}, got ${JSON.stringify(argv['kind'])}`);
            process.exitCode = 2;
            return;
        }
        try {
            ...
            writeStderrLineSafe(`comment-body: ${err.message}`);
