...
import { MCPServerStatus, McpClient, } from './mcp-client.js';
import { defaultPoolEntryOptions, PoolEntry, } from './mcp-pool-entry.js';
import {} from './mcp-pool-events.js';
import { connectionIdOf, isPoolable, mcpTransportOf, parseConnectionId, POOLED_TRANSPORTS_DEFAULT, } from './mcp-pool-key.js';
import { SessionMcpView } from './session-mcp-view.js';
import { discoveryTimeoutFor, runWithTimeout, } from './mcp-discovery-timeout.js';
...
export class McpTransportPool {
    ...
    entries = new Map();
    unpooledIds = new Set();
    spawnInFlight = new Map();
    /** Reverse index for O(refs) `releaseSession`. */
    sessionToEntries = new Map();
    /**
    ...
    hasNameSibling(serverName) {
        ...
        for (const id of this.spawnInFlight.keys()) {
            try {
                if (parseConnectionId(id).serverName === serverName)
                    return true;
            }
            catch {
     ...
     */
    async acquire(serverName, cfg, sessionId, sessionToolRegistry, sessionPromptRegistry, sessionResourceRegistry) {
        if (this.draining) {
            throw new Error(`McpTransportPool is draining; refusing acquire for ${serverName} (session ${sessionId})`);
        }
        ...
        if (id !== undefined) {
            ...
            if (existing && !existing.isTerminated()) {
                ...
                try {
                    const conn = this.attachPooledSession(existing, id, serverName, cfg, sessionId, sessionToolRegistry, sessionPromptRegistry, sessionResourceRegistry);
                    ...
                    if (existing.isTerminated()) {
                        ...
                        debugLogger.warn(`pool self-heal: evicted terminal entry ${id} ` +
                ...
                this.evictEntry(id, existing);
                debugLogger.warn(`pool self-heal: evicted stale terminal entry ${id} ` +
                    `(state='${existing.currentState}', serverName='${serverName}') ` +
        ...
        if (this.opts.budget !== undefined) {
            ...
            if (reservation === 'refused') {
                ...
                throw new BudgetExhaustedError(serverName, this.opts.budget.getBudget() ?? 0, this.opts.budget.getReservedCount());
            }
        ...
        if (!poolable) {
            try {
                return await this.createUnpooledConnection(serverName, cfg, sessionId, sessionToolRegistry, sessionPromptRegistry, sessionResourceRegistry);
            }
            catch (err) {
        ...
        if (!this.entries.has(id) || entry.isTerminated()) {
            this.indexDetach(sessionId, id);
            throw new Error(`PoolEntry ${id} torn down before attach (concurrent release)`);
        }
        try {
            const conn = this.attachPooledSession(entry, id, serverName, cfg, sessionId, sessionToolRegistry, sessionPromptRegistry, sessionResourceRegistry);
            // re-index
     ...
     */
    async restartByName(serverName, opts) {
        // defense-in-depth
        ...
        if (this.draining)
            return [];
        const matching = [...this.entries.values()].filter((e) => e.serverName === serverName &&
     ...
     */
    aggregateStatusByName(serverName) {
        ...
        for (const entry of this.entries.values()) {
            ...
            if (s === MCPServerStatus.CONNECTING)
                sawConnecting = true;
        }
    ...
    async drainAll(opts) {
            ...
            await Promise.race([
                ...
                new Promise((resolve) => {
                    inflightTimer = setTimeout(() => {
                        ...
                        debugLogger.warn(`drainAll: spawnInFlight wait timed out after ${timeoutMs}ms; ` +
                            `${stuckIds.length} spawn(s) still in-flight: ${stuckIds.join(', ')}. Proceeding with drain.`);
                        resolve();
        ...
        const drained = [];
        const errors = [];
        const shutdownPromises = entries.map((entry) => entry
     ...
     */
    attachPooledSession(entry, id, serverName, cfg, sessionId, sessionToolRegistry, sessionPromptRegistry, sessionResourceRegistry) {
        const view = new SessionMcpView(sessionToolRegistry, sessionPromptRegistry, sessionResourceRegistry, sessionId, serverName, cfg);
    ...
    async spawnEntry(serverName, cfg, id) {
        ...
        poisonedToolRegistry(serverName), poisonedPromptRegistry(serverName), this.opts.workspaceContext, this.opts.debugMode, this.opts.sendSdkMcpMessage);
        //
        ...
        const onClosedForThisEntry = (closedId) => {
            this.evictEntry(closedId, entryRef.current);
        };
        const entry = new PoolEntry(id, serverName, entryIndex, cfg, client, this.cliConfig, entryOpts, onClosedForThisEntry, (name) => this.aggregateStatusByName(name));
        ...
        try {
            ...
            const snap = await runWithTimeout((async () => {
            ...
            })(), timeoutMs, `pool spawn for ${id}`);
            if (this.draining) {
                debugLogger.warn(`Spawn for ${id} completed while pool is draining; discarding entry`);
                try {
            ...
            debugLogger.info(`Spawned pool entry ${id} (entryIndex=${entryIndex}, transport=${transport})`);
            return entry;
        }
        catch (err) {
            debugLogger.error(`Failed to spawn pool entry for '${serverName}' ` +
     ...
     * session.
     */
    async createUnpooledConnection(serverName, cfg, sessionId, sessionToolRegistry, sessionPromptRegistry, sessionResourceRegistry) {
        const entryIndex = this.allocateEntryIndex(serverName);
        const id = `${serverName}::unpooled-${entryIndex}`;
        const transport = mcpTransportOf(cfg);
        const entryOpts = this.opts.entryOptions(transport);
        const client = new McpClient(serverName, cfg, sessionToolRegistry, sessionPromptRegistry, this.opts.workspaceContext, this.opts.debugMode, this.opts.sendSdkMcpMessage);
        ...
        // `trust: undefined`).
        const view = new SessionMcpView(sessionToolRegistry, sessionPromptRegistry, sessionResourceRegistry, sessionId, serverName, cfg);
        const entry = new PoolEntry(id, serverName, entryIndex, cfg, client, this.cliConfig, entryOpts, 
        // release the
        ...
        try {
            ...
            })(), timeoutMs, `unpooled spawn for ${id}`);
            ...
            if (this.draining || !this.entries.has(id) || entry.isTerminated()) {
                ...
                this.indexDetach(sessionId, id);
                throw new Error(`McpTransportPool is draining or unpooled ${id} was cancelled`);
            }
            entry.markActive(snap.tools, snap.prompts, snap.resources);
 ...
 */
function poisonedToolRegistry(serverName) {
    return {
        registerTool() {
            throw new Error(`Pool invariant violated: poisoned ToolRegistry for ${serverName} ` +
                'received registerTool. A pool path must use discoverAndReturn, not discover.');
        },
    };
}
function poisonedPromptRegistry(serverName) {
    return {
        registerPrompt() {
            throw new Error(`Pool invariant violated: poisoned PromptRegistry for ${serverName} ` +
...
//# sourceMappingURL=mcp-transport-pool.js.map