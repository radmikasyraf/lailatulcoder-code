 ...
 */
export function shouldHaltOnTurnToolCallCap(totalCalls, maxKeyRepeat, cap, isExplicitCap) {
    if (totalCalls <= cap)
        return false;
...
export class LoopDetectionService {
    ...
    lastShellInspectionKey = null;
    shellInspectionStreak = 0;
    ...
    addAndCheckHeuristicLoops(event) {
    ...
    checkShellCommandStagnation(toolCall) {
        ...
        if (this.shellInspectionStreak >= SHELL_COMMAND_STAGNATION_THRESHOLD) {
            ...
            logLoopDetected(this.config, new LoopDetectedEvent(LoopType.SHELL_COMMAND_STAGNATION, this.promptId));
            return true;
        }
        return false;
    }
    getShellInspectionKey(toolCall) {
    ...
    isGitOverviewInspectionCommand(command) {
        ...
        return segments.every((segment) => {
            const match = /^git(?:\s+(?:-C\s+\S+|--no-pager))*\s+(status|diff|ls-files)\b/i.exec(segment);
            if (!match) {
                return false;
            }
            return (match[1]?.toLowerCase() !== 'diff' ||
                this.isOverviewGitDiff(segment.slice(match[0].length)));
        });
    }
    isOverviewGitDiff(args) {
        ...
        if (pathspecSeparatorIndex !== -1 &&
            pathspecSeparatorIndex < tokens.length - 1) {
            return false;
        }
        return tokens.every((token) => token.startsWith('-') || this.isGitRevisionToken(token));
    }
    isGitRevisionToken(token) {
        return (token === 'HEAD' ||
            token === '@' ||
            /^(?:HEAD|@)(?:[~^]\d*)+$/.test(token) ||
    ...
    checkTurnToolCallCap() {
        ...
        if (!shouldHaltOnTurnToolCallCap(this.turnToolCallTotal, this.capMaxKeyRepeat, this.config.getMaxToolCallsPerTurn(), this.config.isMaxToolCallsPerTurnExplicit())) {
            return false;
        }
        this.lastLoopType = LoopType.TURN_TOOL_CALL_CAP;
        logLoopDetected(this.config, new LoopDetectedEvent(LoopType.TURN_TOOL_CALL_CAP, this.promptId));
        return true;
    }
    /**
     ...
     */
    checkGlobalDuplicate(toolCallKey) {
        ...
        if (count >= GLOBAL_DUPLICATE_THRESHOLD) {
            ...
            logLoopDetected(this.config, new LoopDetectedEvent(LoopType.GLOBAL_TOOL_CALL_DUPLICATE, this.promptId));
            return true;
        }
        return false;
    }
     ...
     * turn is halted.
     */
    checkAlternatingPattern(toolCallKey) {
        ...
        if (a === b)
            return false; // not alternating, same tool
        ...
        logLoopDetected(this.config, new LoopDetectedEvent(LoopType.ALTERNATING_TOOL_CALL_PATTERN, this.promptId));
        return true;
    }
    /**
