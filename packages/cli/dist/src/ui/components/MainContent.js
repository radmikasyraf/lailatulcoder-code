...
export const MainContent = ({ footerRef }) => {
    ...
    const allVirtualItems = useMemo(() => {
        ...
        for (const item of committedByBatchId.values())
            dropped.add(item);
        if (dropped.size === 0)
            return combined;
