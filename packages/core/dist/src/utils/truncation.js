 ...
 */
export const TOOL_OUTPUT_TRUNCATED_PREFIX = 'Tool output was too large and has been truncated';
/**
 ...
 */
export async function truncateAndSaveToFile(content, fileName, projectTempDir, threshold, truncateLines, keep = 'both', previewChars = threshold) {
    ...
    const truncatedContent = keep === 'head'
        ? beginning.join('\n') + sep
        : keep === 'tail'
            ? sep + end.join('\n')
            : beginning.join('\n') + sep + end.join('\n');
 ...
 */
export async function truncateToolOutput(config, toolName, content, limits, promptId) {
    ...
    const result = await truncateAndSaveToFile(content, fileName, config.storage.getProjectTempDir(), threshold, lines, keep, previewChars);
    if (result.outputFile) {
        try {
            logToolOutputTruncated(config, new ToolOutputTruncatedEvent(promptId ?? '', {
                toolName,
 ...
 */
export async function truncateLlmContent(config, toolName, content, limits, promptId) {
    // --- string path ---
    ...
    const parts = (Array.isArray(content) ? content : [content]).map((p) => (typeof p === 'string' ? { text: p } : p));
    ...
    const result = await truncateToolOutput(config, toolName, combined, limits, promptId);
...
export function isAlreadyTruncated(content) {
    return (content.includes('... [CONTENT TRUNCATED] ...') ||
        content.startsWith('<persisted-output>'));
}
function generatePreview(content) {
    let text = content.length <= PREVIEW_SIZE_CHARS
        ? content
        : (() => {
...
export function normalizeToolResultCallId(callId) {
...
export async function persistAndTruncateToolResult(callId, toolName, content, config) {
    const byteSize = Buffer.byteLength(content, 'utf-8');
    // Hard size cap — content already in memory, just skip disk persistence
    if (byteSize > MAX_FILE_SIZE_BYTES) {
        debugLogger.warn(`Tool result for ${toolName} exceeds ${MAX_FILE_SIZE_BYTES} bytes (${byteSize}), skipping disk persistence`);
        return {
    ...
    if (budgetUsed + byteSize > MAX_SESSION_BYTES) {
        debugLogger.warn(`Session tool result budget exhausted (${budgetUsed} + ${byteSize} > ${MAX_SESSION_BYTES}), skipping disk persistence`);
        return {
    ...
    try {
        ...
        try {
            const fallback = await truncateAndSaveToFile(content, `${toolName}_${crypto.randomBytes(6).toString('hex')}`, config.storage.getProjectTempDir(), config.getTruncateToolOutputThreshold(), config.getTruncateToolOutputLines());
