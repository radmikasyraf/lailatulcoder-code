...
import { aggregateModelTokens, buildStatusLinePresetData, buildStatusLinePresetLines, DEFAULT_STATUS_LINE_PRESET_CONFIG, normalizeStatusLinePresetConfig, } from '../statusLinePresets.js';
...
const CONTEXT_PRESET_ITEM_IDS = new Set([
    'context-used',
    'context-remaining',
]);
/**
 ...
 */
export function resolveHideContextIndicator(config, isContextOverLimit = false, keepAutomaticIndicator = false) {
...
function getStatusLineConfig(settings) {
    ...
    if (typeof raw === 'object' &&
        ...
        if (typeof raw.refreshInterval === 'number' &&
...
function buildMetricsPayload(m) {
    const models = {};
 ...
 */
export function useStatusLine(keepAutomaticContextIndicator = false, availableWidth) {
    ...
    const statusLineConfig = statusLineConfigOverride &&
        settingsStatusLineConfig &&
        statusLineConfigOverride.type === settingsStatusLineConfig.type
        ? statusLineConfigOverride
        : settingsStatusLineConfig;
    const statusLineCommand = statusLineConfig?.type === 'command' ? statusLineConfig.command : undefined;
    const statusLinePreset = statusLineConfig?.type === 'preset' ? statusLineConfig : undefined;
    const statusLineSettingsVersion = uiState.statusLineSettingsVersion ?? 0;
    const hasStatusLinePreset = statusLinePreset !== undefined;
    const statusLinePresetUseThemeColors = statusLinePreset?.useThemeColors ?? false;
    const statusLinePresetItemsKey = statusLinePreset?.items.join('\0') ?? '';
    const refreshInterval = statusLineConfig?.type === 'command'
        ? statusLineConfig.refreshInterval
        : undefined;
    const [output, setOutput] = useState([]);
    const [pullRequestNumber, setPullRequestNumber] = useState(undefined);
    ...
    const reasoningEffortKey = reasoningConfig === false ? 'off' : (reasoningConfig?.effort ?? '');
    ...
    const updatePullRequestNumber = useCallback((nextPullRequestNumber) => {
    ...
    const ensurePullRequestNumber = useCallback((preset, currentDir, branch) => {
        ...
        let child;
        try {
            child = exec(PULL_REQUEST_LOOKUP_COMMAND, { cwd: currentDir, timeout: 2000, maxBuffer: 1024 }, (error, stdout) => {
                if (generation !== pullRequestLookupGenerationRef.current ||
            ...
            debugLog.warn('statusline: gh pr view failed:', err.message);
            pullRequestLookupKeyRef.current = undefined;
            updatePullRequestNumber(undefined);
            return;
        }
    ...
    const doUpdate = useCallback(() => {
        ...
        const usedPercentage = contextWindowSize > 0
            ? Math.min(100, Math.max(0, Math.round((stats.lastPromptTokenCount / contextWindowSize) * 1000) / 10))
            : 0;
        const input = {
        ...
        let child;
        try {
            child = exec(cmd, { cwd: cfg.getTargetDir(), timeout: 5000, maxBuffer: 1024 * 10 }, (error, stdout) => {
                if (gen !== generationRef.current)
                    return; // stale
                activeChildRef.current = undefined;
                const nextLines = !error && stdout
                    ? stdout
                        .replace(/\r?\n$/, '')
                        .split(/\r?\n/)
                        .filter(Boolean)
                ...
                setOutput((prev) => {
                    if (prev.length === nextLines.length &&
            ...
            debugLog.error('statusline exec error:', err.message);
            setOutput([]);
            return;
        }
    ...
    useEffect(() => {
        ...
        if (lastPromptTokenCount !== prev.promptTokenCount ||
    ...
    useEffect(() => {
        if (!hasMountedRef.current || !hasStatusLinePreset)
            return;
        scheduleUpdate();
    }, [
        pullRequestNumber,
    ...
    useEffect(() => {
        if (!statusLineCommand || !refreshInterval)
            return;
    ...
    return {
        ...
        respectUserColors: statusLineConfig?.type === 'command' &&
            statusLineConfig.respectUserColors === true,
        hideContextIndicator: resolveHideContextIndicator(statusLineConfig, uiState.sessionStats.lastPromptTokenCount >
            (config.getContentGeneratorConfig()?.contextWindowSize ?? Infinity), keepAutomaticContextIndicator ||
            output.some((line) => wrapAnsi(line, Math.max(1, availableWidth ?? terminalWidth - 4), {
                trim: false,
                hard: true,
            }).split('\n').length > MAX_STATUS_LINES)),
    };
}
//# sourceMappingURL=useStatusLine.js.map