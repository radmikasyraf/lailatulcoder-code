...
import { LOOP_TASK_FILE_MAX_BYTES, readLoopTaskFile, } from './loop-task-file.js';
import { AUTONOMOUS_PREAMBLE, AUTONOMOUS_PREAMBLE_MARKER, CRON_REARM, autonomousTickText, keepAliveRearm, } from './autonomous-loop.js';
...
/**
 * Fire-time resolver for `.lailatulcoder/loop.md`-driven loops.
 *
...
const INTRO = 'The user configured a loop-tasks file. Work through the tasks defined below; these are the instructions for this tick and every subsequent tick (the reminder on later fires refers back to this message).';
...
const SHORT_REMINDER_PREAMBLE = 'Work the tasks from the loop.md contents established earlier in this conversation. If you cannot find them, treat this as a no-op tick.';
/**
 ...
 */
function tickHeading(mode, opts = {}) {
...
export function detectLoopSentinel(prompt) {
...
function absentAutonomousTickText(mode, locations) {
...
function cutToLastNewline(content) {
...
export class LoopTickResolver {
     ...
     *   - under $HOME             → tilde-abbreviated `~/.lailatulcoder/loop.md`;
    ...
    homeLoopLabel() {
        const homeQwenDir = this.deps.homeQwenDir ?? path.join(this.deps.homeDir, '.lailatulcoder');
        ...
        // (`$QWEN_HOME=/x/.lailatulcoder/`) over-counts the separator, and a filesystem-root
     ...
     * claim `.lailatulcoder/loop.md (project)` for an untrusted folder where the project
    ...
    absentLocations(projectChecked) {
        const homeLabel = this.homeLoopLabel();
        return projectChecked
            ? `.lailatulcoder/loop.md (project) or ${homeLabel} (home)`
            : `${homeLabel} (home)`;
    }
    ...
    #noOpTick(modelText, transientError = false) {
        this.#pendingContent = null;
        if (this.#lastContent !== AUTONOMOUS_PREAMBLE_MARKER) {
            this.#lastContent = null;
        }
     ...
     */
    buildTransientErrorTick(mode, projectChecked, code) {
        ...
        `${tickHeading(mode, { unavailable: true })}\nloop.md at ${this.absentLocations(projectChecked)} could not be read this tick (${code}). ${ABSENT_TAIL[mode]}`, 
     ...
     */
    async resolve(mode, allowProjectFileOverride) {
        ...
        const allowProjectFile = allowProjectFileOverride ?? this.deps.allowProjectFile();
        ...
        if (result.status === 'missing') {
            ...
            return this.#autonomousTick(absentAutonomousTickText(mode, this.absentLocations(allowProjectFile)));
        }
...
//# sourceMappingURL=loop-tick-resolver.js.map