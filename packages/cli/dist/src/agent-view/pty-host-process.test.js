...
import { createAgentViewPtyHostServer, connectAgentViewPtyHostProcess, getAgentViewPtyHostSocketPath, INTERNAL_AGENT_VIEW_PTY_HOST_ARG, launchAgentViewPtyHostProcess, runAgentViewPtyHostProcess, } from './pty-host-process.js';
...
describe('Agent View PTY host process server', () => {
    ...
    afterEach(async () => {
        ...
        await Promise.all([...socketDirs].map((dir) => isWindowsPipePath(dir)
            ? Promise.resolve()
    ...
    it('handles resize, logs, and kill requests', async () => {
        ...
        await expect(requestHost(socketPath, 'resize', { columns: 120, rows: 40 })).resolves.toMatchObject({ resized: true });
        await expect(requestHost(socketPath, 'logs')).resolves.toEqual({
            output: '56789',
        });
        await expect(requestHost(socketPath, 'kill', { signal: 'SIGTERM' })).resolves.toMatchObject({ killed: true });
    ...
    it('forwards attach input bytes without UTF-8 re-encoding', async () => {
        ...
        const request = Buffer.from(`${JSON.stringify({ id: 'attach-1', op: 'attachStream' })}\n`, 'utf8');
    ...
    it.skipIf(process.platform === 'win32')('reclaims a stale socket lock left by a dead process', async () => {
        const host = fakeHost();
        const socketPath = shortSocketPath();
    ...
    it.skipIf(process.platform === 'win32')('releases the socket lock on close so the path can be reused', async () => {
        const host = fakeHost();
        const socketPath = shortSocketPath();
    ...
    it('rejects non-positive or non-integer resize dimensions', async () => {
        ...
        servers.push(server);
        await server.listen();
        await expect(requestHost(socketPath, 'resize', { columns: 0, rows: 40 })).rejects.toThrow('columns must be a positive integer');
        await expect(requestHost(socketPath, 'resize', { columns: 120, rows: 2.5 })).rejects.toThrow('rows must be a positive integer');
    ...
    it('rejects unsupported kill signals', async () => {
        ...
        servers.push(server);
        await server.listen();
        await expect(requestHost(socketPath, 'kill', { signal: 'SIGUSR1' })).rejects.toThrow('Agent View PTY host signal is not allowed.');
    ...
    it('requires auth when the host server has a token', async () => {
        ...
        servers.push(server);
        await server.listen();
        await expect(requestHost(socketPath, 'status')).rejects.toThrow('Unauthorized PTY host request.');
        await expect(requestHost(socketPath, 'status', undefined, 'secret')).resolves.toMatchObject({
            workerPid: 1234,
        });
    });
    it('wires the env host token into the entrypoint server', async () => {
        const launchDir = await fs.mkdtemp(path.join(os.tmpdir(), 'qwen-pty-entrypoint-'));
        ...
        try {
            ...
            try {
                ...
                for (let attempt = 0; attempt < 50 && status === undefined; attempt++) {
                    try {
                        status = await requestHost(socketPath, 'status', undefined, 'entry-token');
                    }
                    catch {
    ...
    it('requires auth for attach streams', async () => {
        ...
        rejectedSocket.write(`${JSON.stringify({ id: '1', op: 'attachStream' })}\n`);
        ...
        acceptedSocket.write(`${JSON.stringify({
            id: '2',
            op: 'attachStream',
            authToken: 'secret',
        })}\nhello`);
    ...
    it('returns control-byte-heavy logs through the connected handle', async () => {
        ...
        host.emitData(output);
        const connected = await connectAgentViewPtyHostProcess(createLaunch('session-control-byte-logs'), socketPath);
        await expect(connected.getOutput?.()).resolves.toBe(output);
    });
    it.skipIf(process.platform === 'win32')('restricts Unix socket and parent directory permissions', async () => {
        const socketPath = shortSocketPath();
    ...
    it.skipIf(process.platform === 'win32')('rejects listening on a socket path owned by a live server', async () => {
        const socketPath = shortSocketPath();
    ...
    it.skipIf(process.platform === 'win32')('replaces a stale socket file when listening', async () => {
        const socketPath = shortSocketPath();
        await fs.mkdir(path.dirname(socketPath), { recursive: true });
    ...
    it.skipIf(process.platform === 'win32')('does not unlink a socket path taken over by another live server', async () => {
        const socketPath = shortSocketPath();
    ...
    it.skipIf(process.platform === 'win32')('rejects a symlinked socket parent directory', async () => {
        const realDir = await fs.mkdtemp(path.join(os.tmpdir(), 'qah-real-'));
        const linkDir = path.join(os.tmpdir(), `qah-link-${process.pid}-${Date.now()}`);
    ...
    it('waits for the remote endpoint to close after shutdown', async () => {
        ...
        servers.push(server);
        await server.listen();
        const connected = await connectAgentViewPtyHostProcess(createLaunch('session-1'), socketPath);
        connected.shutdown?.();
    ...
    it('only resolves exited on SIGKILL for a connected (childless) handle', async () => {
        ...
        await server.listen();
        const connected = await connectAgentViewPtyHostProcess(createLaunch('session-1'), socketPath);
        const notSettledWithin = (ms) => Promise.race([
            connected.exited.then(() => true),
    ...
    it('fails fast when connecting with the wrong host token', async () => {
        ...
        servers.push(server);
        await server.listen();
        await expect(connectAgentViewPtyHostProcess(createLaunch('session-1'), socketPath, 'wrong-token')).rejects.toThrow('Unauthorized PTY host request.');
    });
    ...
    it('rejects input written before an attach stream is established', async () => {
        ...
        servers.push(server);
        await server.listen();
        const connected = await connectAgentViewPtyHostProcess(createLaunch('session-early-write'), socketPath);
        expect(() => connected.write(Buffer.from('early'))).toThrow('Agent View PTY host input requires an active attach stream.');
    ...
    it('passes auth tokens through connected host handle operations', async () => {
        ...
        servers.push(server);
        await server.listen();
        const connected = await connectAgentViewPtyHostProcess(createLaunch('session-token-handle'), socketPath, 'secret');
        const data = [];
        const disposable = connected.onData((chunk) => data.push(chunk));
        connected.write(Buffer.from('hello'));
        await waitFor(() => host.input === 'hello');
    ...
    it('computes Unix and Windows host socket paths', () => {
        expect(getAgentViewPtyHostSocketPath('session-1', {
            globalDir: '/tmp/qwen-agent-view-test',
            platform: 'linux',
        })).toBe(path.join('/tmp/qwen-agent-view-test', 'jobs', 'session-1', 'tmp', 'pty-host.sock'));
        expect(getAgentViewPtyHostSocketPath('session-1', {
    ...
    it.skipIf(process.platform === 'win32')('skips an unusable fallback socket directory', async () => {
        const tmpRoot = await fs.mkdtemp(path.join(os.tmpdir(), 'qwen-pty-bad-tmp-'));
        ...
        try {
            ...
            const uid = typeof process.getuid === 'function' ? process.getuid() : 'user';
            expect(path.dirname(fallbackPath)).toBe(path.join('/tmp', `qwen-avp-${uid}`));
        }
        finally {
    ...
    it('rejects oversized PTY host responses', async () => {
        ...
        try {
            await expect(connectAgentViewPtyHostProcess(createLaunch('session-oversized-response'), socketPath, undefined, { readyRetries: 3, requestTimeoutMs: 5000 })).rejects.toThrow('Agent View PTY host response line is too large.');
        }
        finally {
            server.close();
            await removeTestSocket(socketPath);
    ...
    it('fails fast on malformed PTY host responses', async () => {
        ...
        await listenServer(server, socketPath);
        try {
            await expect(connectAgentViewPtyHostProcess(createLaunch('session-malformed-response'), socketPath)).rejects.toMatchObject({
    ...
    it('rejects promptly when the host closes without a response', async () => {
        ...
        await listenServer(server, socketPath);
        try {
            await expect(connectAgentViewPtyHostProcess(createLaunch('session-closed-response'), socketPath)).rejects.toThrow();
        }
        finally {
            server.close();
            await removeTestSocket(socketPath);
    ...
    it('fails quickly when the spawned PTY host exits before ready', async () => {
        ...
        await expect(launched).rejects.toThrow('Agent View PTY host exited before ready (code 1).');
        expect(child.killedWith).toBe('SIGKILL');
    });
    it('fails quickly when the spawned PTY host emits an error before ready', async () => {
        const child = fakeChildProcess(2468);
        const launched = launchAgentViewPtyHostProcess(createLaunch('session-spawn-error'), {
            globalDir: '/tmp/qwen-agent-view-test',
            spawnProcess: () => child,
        });
    ...
    it('passes the launch file, socket path, and token to spawned PTY hosts', async () => {
        const globalDir = await fs.mkdtemp(path.join(os.tmpdir(), 'qwen-pty-spawn-'));
        ...
        try {
            ...
            expect(spawnProcess).toHaveBeenCalledWith([
                ...
                [PTY_HOST_AUTH_TOKEN_ENV]: expect.stringMatching(/^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/),
    ...
    it('asks a spawned host to shut down when the handle is disposed', async () => {
        const globalDir = await fs.mkdtemp(path.join(os.tmpdir(), 'qwen-pty-launch-'));
    ...
    it('asks a spawned host to deliver kill signals before falling back to the child', async () => {
        const globalDir = await fs.mkdtemp(path.join(os.tmpdir(), 'qwen-pty-kill-'));
    ...
    it('reports child exit signals when they are known', async () => {
        const globalDir = await fs.mkdtemp(path.join(os.tmpdir(), 'qwen-pty-signal-'));
...
function fakeHost(maxOutputBytes = 5) {
    let dataCallbacks = [];
    const host = {
        pid: process.pid,
        workerPid: 1234,
...
function createLaunch(sessionId) {
    return {
        schemaVersion: 1,
        sessionId,
        argv: ['qwen'],
...
async function requestHost(socketPath, op, params, authToken) {
    const socket = net.createConnection(socketPath);
...
async function createStatusServer(socketPath, operations = []) {
    if (!isWindowsPipePath(socketPath)) {
        await fs.mkdir(path.dirname(socketPath), { recursive: true });
    }
    const server = net.createServer((socket) => {
...
async function connectOnce(socketPath) {
...
async function readChunk(socket) {
...
async function removeTestSocket(socketPath) {
    if (isWindowsPipePath(socketPath))
        return;
    await fs.rm(path.dirname(socketPath), { recursive: true, force: true });
}
//# sourceMappingURL=pty-host-process.test.js.map