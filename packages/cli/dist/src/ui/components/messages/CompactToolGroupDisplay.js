...
import { ToolDisplayNames } from '@lailatul-coder/lailatul-coder-core';
...
export function getOverallStatus(toolCalls) {
...
function isToolGroupActive(status) {
    return (status === ToolCallStatus.Executing ||
        status === ToolCallStatus.Pending ||
        status === ToolCallStatus.Confirming);
}
function getElapsedTimeReservedWidth(tool, status) {
...
const COLLAPSIBLE_CATEGORIES = new Set([
    'read',
    'search',
    'list',
]);
 ...
 */
export function isCollapsibleTool(toolName) {
...
function categoryShowsDescriptionsInline(toolCalls, category) {
    const sameCategory = toolCalls.filter((tc) => getToolCategory(tc.name) === category);
    if (sameCategory.length > DESCRIPTION_INLINE_LIMIT)
        return false;
    return sameCategory.every((tc) => safeDescription(tc.description) !== undefined);
}
    ...
    for (const status of statuses) {
        for (let index = toolCalls.length - 1; index >= 0; index--) {
            ...
            if (tool.status === status) {
                ...
                const hasCategoryPeers = toolCalls.some((candidate, candidateIndex) => candidateIndex !== index &&
 ...
 */
export function buildToolSummary(toolCalls, isActive) {
...
export function estimateCompactToolGroupHeight(toolCalls, contentWidth) {
    if (toolCalls.length === 0)
        return 0;
    ...
    const summaryWidth = Math.max(1, contentWidth -
...
export const CompactToolGroupDisplay = ({ toolCalls, contentWidth }) => {
    if (toolCalls.length === 0)
        return null;
    const overallStatus = getOverallStatus(toolCalls);
    const activeTool = getActiveTool(toolCalls);
    const isActive = isToolGroupActive(overallStatus);
    const hint = getActiveToolHint(toolCalls);
    return (_jsxs(Box, { flexDirection: "column", width: contentWidth, gap: 0, children: [_jsxs(Box, { flexDirection: "row", children: [_jsx(ToolStatusIndicator, { status: overallStatus, name: activeTool.name }), _jsx(Box, { flexGrow: 1, children: _jsxs(Text, { wrap: "wrap", bold: true, children: [buildToolSummary(toolCalls, isActive), isActive && _jsx(Text, { children: "\u2026" }, "ellipsis")] }) }), _jsx(ToolElapsedTime, { status: overallStatus, executionStartTime: activeTool.executionStartTime, timeoutMs: getShellTimeoutMs(activeTool) })] }), hint && (_jsx(Box, { paddingLeft: STATUS_INDICATOR_WIDTH, children: _jsxs(Text, { dimColor: true, wrap: "truncate-end", children: ["\u23BF ", hint] }) }))] }));
};
