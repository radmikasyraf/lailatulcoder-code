...
import { ToolNames, sanitizeFilenameComponent, } from '@lailatul-coder/lailatul-coder-core';
 ...
 * separate names.
 */
export function serializedArgsNamePath(serializedArgs, path) {
...
function parseTranscript(file, diffPath) {
    ...
    for (const line of lines) {
        ...
        for (const part of parts) {
            ...
            // to open; a tool *result* that quotes it (a grep over `.lailatulcoder/tmp`, this
        ...
        if (parts.some((p) => p.functionResponse !== undefined))
            toolTrafficAfterText = true;
    }
    if (!agentId)
        return null;
    ...
    return {
        ...
        finalText,
        returned: finalText.trim() !== '' && !toolTrafficAfterText && !diedPerSidecar(file),
        mtimeMs,
    };
}
/**
 ...
 */
function diedPerSidecar(transcriptFile) {
 ...
 */
function recordsIn(dir, names, since, diffPath, opts = {}) {
    const out = [];
    for (const name of names) {
        ...
        if (opts.until !== undefined && rec.mtimeMs >= opts.until)
            continue;
 ...
 */
export function priorSessionDirs(planPath, env = process.env) {
    const { projectDir } = transcriptPaths(env);
    const out = [];
    for (const { sessionId, endsAtMs } of priorSessionEntries(planPath, env)) {
        const dir = join(projectDir, 'subagents', sanitizeFilenameComponent(sessionId));
        try {
 ...
 */
export function readRunTranscripts(planPath, since, env = process.env, diffPath, opts = {}) {
