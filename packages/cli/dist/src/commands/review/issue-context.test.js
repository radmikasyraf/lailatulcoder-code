...
const { ghMock, ensureAuthenticatedMock, setGhHostMock, writeStdoutLineMock, writeFileSyncMock, mkdirSyncMock, } = vi.hoisted(() => ({
    ghMock: vi.fn(),
...
describe('runIssueContext', () => {
    ...
    it('fetches each closing issue from its own repository and renders body + comments', () => {
        ...
        ghMock.mockReturnValueOnce(JSON.stringify({
            title: 'the bug',
            body: 'repro steps',
            comments: [
                {
        ...
        expect(ghMock).toHaveBeenNthCalledWith(1, 'pr', 'view', '9077', '--repo', 'LailatulCoder/lailatul-coder', '--json', 'closingIssuesReferences');
        expect(ghMock).toHaveBeenNthCalledWith(2, 'issue', 'view', '9078', '--repo', 'LailatulCoder/lailatul-coder', '--json', 'title,body,comments');
        const written = writeFileSyncMock.mock.calls[0][1];
        expect(mkdirSyncMock).toHaveBeenCalledWith(dirname(resolve('/tmp/issue-context.md')), { recursive: true });
        // The write TARGET, not just the content: a redirected write that still
        // reported the right body used to ship green (#9194).
        expect(writeFileSyncMock.mock.calls[0][0]).toBe(resolve('/tmp/issue-context.md'));
    ...
    it('uses the reference repository, not the PR repo, for cross-repo issues', () => {
        ...
        mockIssue('elsewhere');
        const result = runIssueContext(ARGS);
        expect(ghMock).toHaveBeenNthCalledWith(2, 'issue', 'view', '42', '--repo', 'acme/other', '--json', 'title,body,comments');
        const written = writeFileSyncMock.mock.calls[0][1];
    ...
    it('a failed extra lands in unfetchable too (JSON and file agree)', () => {
        ...
        const written = writeFileSyncMock.mock.calls[0][1];
        expect(written).toContain('## Issue #555 of LailatulCoder/lailatul-coder — could not be fetched');
    });
    ...
    it('the extras section header does not claim NOT-in-closing when discovery failed', () => {
        ...
        const written = writeFileSyncMock.mock.calls[0][1];
        expect(written).toContain('Additionally fetched issues (referenced by the PR context; the closing set could not be checked)');
    ...
    it('fetches --issue extras from the PR repo, marks them as not-closing, and dedups closing numbers', () => {
        ...
        expect(ghMock).toHaveBeenNthCalledWith(3, 'issue', 'view', '555', '--repo', 'LailatulCoder/lailatul-coder', '--json', 'title,body,comments');
        const written = writeFileSyncMock.mock.calls[0][1];
        ...
        expect(written).toContain('Additionally fetched issues (referenced by the PR context, NOT in the closing set)');
        expect(written).toContain('## Issue #555 of LailatulCoder/lailatul-coder: referenced only');
    });
    it('a cross-repo closing number does not shadow a same-numbered extra', () => {
        ...
        expect(ghMock).toHaveBeenNthCalledWith(3, 'issue', 'view', '42', '--repo', 'LailatulCoder/lailatul-coder', '--json', 'title,body,comments');
        const written = writeFileSyncMock.mock.calls[0][1];
    ...
    it('a repo-qualified extra is fetched from its OWN repository', () => {
        ...
        expect(ghMock).toHaveBeenNthCalledWith(2, 'issue', 'view', '7', '--repo', 'acme/widgets', '--json', 'title,body,comments');
        const written = writeFileSyncMock.mock.calls[0][1];
        expect(written).toContain('## Issue #7 of acme/widgets: referenced elsewhere');
    });
    ...
    it('an unreadable issue degrades to an explicit section, not an abort', () => {
        ...
        expect(written).toContain('## Issue #2 of acme/restricted — could not be fetched');
    ...
    it('falls back to the PR repo for a closing ref with no repository payload', () => {
        ...
        runIssueContext(ARGS);
        expect(ghMock).toHaveBeenNthCalledWith(2, 'issue', 'view', '77', '--repo', 'LailatulCoder/lailatul-coder', '--json', 'title,body,comments');
    });
});
describe('issueContextCommand handler', () => {
    ...
    it('a discovery failure degrades into the file (exit 0 with discoveryError)', () => {
        ...
        expect(writeStdoutLineMock).toHaveBeenCalledWith(expect.stringContaining('"discoveryError":"HTTP 500"'));
    });
    it('wires --issue through to the extra fetch', () => {
        ...
        expect(ghMock).toHaveBeenNthCalledWith(2, 'issue', 'view', '555', '--repo', 'LailatulCoder/lailatul-coder', '--json', 'title,body,comments');
        expect(process.exitCode).toBeUndefined();
    });
...
//# sourceMappingURL=issue-context.test.js.map