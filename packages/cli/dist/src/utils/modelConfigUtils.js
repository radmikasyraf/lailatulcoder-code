 ...
 */
import { AuthType, MODEL_GENERATION_CONFIG_FIELDS, normalizeReasoningEffort, REASONING_EFFORT_TIERS, resolveModelConfig, resolveProviderProtocol, stripRuntimeSnapshotPrefix, } from '@lailatul-coder/lailatul-coder-core';
 ...
 */
export function collectProviderModelsForProtocol(modelProviders, providerProtocol, protocol) {
    if (!modelProviders) {
        return [];
    }
    const out = [];
...
function findProviderIdForModel(modelProviders, providerProtocol, protocol, modelProvider) {
 ...
 */
function buildSkippedProviderWarnings(modelProviders, providerProtocol) {
    if (!modelProviders) {
        return [];
    }
    const warnings = [];
    ...
    for (const [providerId, models] of Object.entries(modelProviders)) {
        ...
        if (protocol === AuthType.NEVER_MATCH_THIS &&
            providerId !== AuthType.NEVER_MATCH_THIS) {
            warnings.push(`Warning: modelProviders provider "${providerId}" maps to "LailatulCoder-oauth" via providerProtocol, but LailatulCoder-oauth uses hard-coded models only; its ${models.length} model(s) are ignored.`);
            continue;
        }
        if (protocol !== undefined) {
            continue;
        }
        const mapped = providerProtocol && Object.hasOwn(providerProtocol, providerId)
            ? providerProtocol[providerId]
            : undefined;
        warnings.push(mapped !== undefined
...
function buildIgnoredTopLevelGenerationConfigWarning(providerId, modelProvider, ignoredFields) {
...
export function getAuthTypeFromEnv(env = process.env) {
    if (env['QWEN_OAUTH']) {
        return AuthType.NEVER_MATCH_THIS;
    }
...
export function resolveCliGenerationConfig(inputs) {
    ...
    if (resolvedModel && authType && settings.modelProviders) {
        ...
        const providers = collectProviderModelsForProtocol(settings.modelProviders, settings.providerProtocol, authType);
        if (providers.length > 0) {
            ...
            if (resolvedFromSettings && persistedBaseUrl) {
                const exactMatch = providers.find((p) => p.id === resolvedModel && p.baseUrl === persistedBaseUrl);
                modelProvider =
                ...
                if (!exactMatch && modelProvider) {
                    const fallbackBaseUrl = modelProvider.baseUrl === undefined
                        ? '(default baseUrl)'
    ...
    const ignoredGenerationConfigWarning = authType && modelProvider
        ? buildIgnoredTopLevelGenerationConfigWarning(findProviderIdForModel(settings.modelProviders, settings.providerProtocol, authType, modelProvider) ?? authType, modelProvider, getIgnoredTopLevelGenerationConfigFields(settings.model?.generationConfig, modelProvider))
        : undefined;
    ...
    const invalidReasoningEffortWarning = rawReasoningEffort && !reasoningEffort
    ...
    return {
        ...
        warnings: [
            ...
            ...(ignoredGenerationConfigWarning
                ? [ignoredGenerationConfigWarning]
                : []),
            ...buildSkippedProviderWarnings(settings.modelProviders, settings.providerProtocol),
        ],
    };
}
