...
import { createNonInteractivePromptId, main, registerLspHotReload, setupUnhandledRejectionHandler, validateDnsResolutionOrder, } from './gemini.js';
...
const mockStopNonInteractiveOpenAILogHousekeeping = vi.hoisted(() => vi.fn(async () => { }));
const mockUpdateBeforeRelaunch = vi.hoisted(() => vi.fn());
const mockGetInstallationInfo = vi.hoisted(() => vi.fn());
const mockRegisterSession = vi.hoisted(() => (..._args) => Promise.resolve(true));
...
describe('gemini import boundary', () => {
    it('does not statically import ACP or noninteractive auth branches', () => {
        ...
        expect(source).not.toContain("import { runAcpAgent } from './acp-integration/acpAgent.js'");
        expect(source).not.toContain("import { validateNonInteractiveAuth } from './validateNonInterActiveAuth.js'");
        expect(source).not.toContain("import { initializeApp } from './core/initializer.js'");
        expect(source).toMatch(/await import\(\s*['"]\.\/acp-integration\/acpAgent\.js['"]\s*\)/);
        expect(source).toMatch(/await import\(\s*['"]\.\/validateNonInterActiveAuth\.js['"]\s*\)/);
        expect(source).toMatch(/await import\(\s*['"]\.\/core\/initializer\.js['"]\s*\)/);
    });
});
...
vi.mock('./config/config.js', () => ({
    ...
    isValidSessionId: vi.fn((value) => /^[0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}(-agent-[a-zA-Z0-9_.-]+)?$/i.test(value)),
}));
...
vi.mock('./utils/relaunch.js', () => ({
    ...
    relaunchOnExitCode: vi.fn((fn) => fn()),
}));
...
vi.mock('./startup/startup-prefetch.js', () => ({
    startEarlyStartupPrefetches: (...args) => mockStartEarlyStartupPrefetches(...args),
    startPostRenderPrefetches: (...args) => mockStartPostRenderPrefetches(...args),
}));
vi.mock('./utils/update-relaunch.js', () => ({
    updateBeforeRelaunch: (...args) => mockUpdateBeforeRelaunch(...args),
}));
vi.mock('./utils/installationInfo.js', () => ({
    getInstallationInfo: (...args) => mockGetInstallationInfo(...args),
}));
vi.mock('./acp-integration/acpAgent.js', () => ({
    runAcpAgent: (...args) => mockRunAcpAgent(...args),
}));
vi.mock('./utils/housekeeping/scheduler.js', async (importOriginal) => {
    const actual = await importOriginal();
    return {
        ...actual,
        startNonInteractiveOpenAILogHousekeeping: (...args) => mockStartNonInteractiveOpenAILogHousekeeping(...args),
        stopNonInteractiveOpenAILogHousekeeping: () => mockStopNonInteractiveOpenAILogHousekeeping(),
    };
});
...
function withLspDisabledConfig(config) {
    return {
        isLspEnabled: () => false,
        getLspClient: () => undefined,
        ...config,
...
describe('gemini.tsx main function', () => {
    ...
    let originalEnvQwenSandboxImage;
    ...
    it('relaunches before config load and preserves only private managed ACP activation', async () => {
        ...
        vi.mocked(parseArguments).mockResolvedValue({ acp: true });
        vi.stubEnv('QWEN_CODE_PRIVATE_ACP_CAPABILITY', 'private-capability');
        vi.stubEnv('QWEN_CODE_PRIVATE_EXTERNAL_TOOL_GUARD', EXTERNAL_TOOL_GUARD_REQUIRED_VALUE);
        vi.stubEnv('QWEN_CODE_EXTERNAL_TOOL_GUARD_TOKEN', 'guard-secret');
        vi.stubEnv('QWEN_CODE_NO_RELAUNCH', '');
        const callOrder = [];
        vi.mocked(relaunchAppInChildProcess).mockImplementation(async (_memoryArgs, _extraArgs, options) => {
            ...
            expect(options?.childEnv).toEqual({
                ...
                QWEN_CODE_PRIVATE_EXTERNAL_TOOL_GUARD: EXTERNAL_TOOL_GUARD_REQUIRED_VALUE,
            });
        });
        ...
        expect(relaunchAppInChildProcess).toHaveBeenCalledWith(expect.any(Array), [], expect.objectContaining({
            childEnv: {
                QWEN_CODE_PRIVATE_ACP_CAPABILITY: 'private-capability',
                QWEN_CODE_PRIVATE_EXTERNAL_TOOL_GUARD: EXTERNAL_TOOL_GUARD_REQUIRED_VALUE,
            },
    ...
    it.each([
    ...
    ])('manages Electron bootstrap env %s', async (_name, argv, extraEnv, expectedElectron, expectedMarker) => {
        ...
        vi.mocked(loadSettings).mockImplementation(() => {
            ...
            expect(process.env['QWEN_CODE_SCRUB_ELECTRON_RUN_AS_NODE']).toBe(expectedMarker);
    ...
    it.each([
        ['scrubs for a daemon-spawned child', { acp: true }, '1', true],
        [
        ...
        ['keeps for a non-ACP launch', {}, '', false],
    ])('inherited loader env vars past the ACP handoff (%s)', async (_mode, argv, daemonMarker, expectScrubbed) => {
        ...
        const { parseArguments, loadCliConfig } = await import('./config/config.js');
        const { loadSettings } = await import('./config/settings.js');
        vi.mocked(parseArguments).mockResolvedValue(argv);
        vi.mocked(loadSettings).mockReturnValue({
            errors: [],
        ...
        try {
            await expect(main()).rejects.toThrow('stop after loader env scrub check');
        }
        finally {
            vi.unstubAllEnvs();
        }
    ...
    it('keeps inherited loader env vars present at the relaunch handoff', async () => {
        ...
        try {
            await expect(main()).rejects.toThrow('stop after loader env handoff check');
        }
        finally {
            vi.unstubAllEnvs();
        }
    ...
    it('scrubs inherited loader env vars in the sandboxed ACP stage', async () => {
        ...
        try {
            await expect(main()).rejects.toThrow('stop after sandboxed loader env scrub check');
        }
        finally {
            vi.unstubAllEnvs();
        }
    ...
    it('keeps the external Guard token available to command parsing and scrubs it before settings load', async () => {
        ...
        vi.mocked(parseArguments).mockImplementation(async () => {
            expect(process.env['QWEN_CODE_EXTERNAL_TOOL_GUARD_TOKEN']).toBe('guard-secret');
            return {};
        });
        vi.mocked(loadSettings).mockImplementation(() => {
            expect(process.env['QWEN_CODE_EXTERNAL_TOOL_GUARD_TOKEN']).toBeUndefined();
    ...
    it('should skip full settings discovery in bare mode', async () => {
        ...
        const { loadCliConfig, parseArguments } = await import('./config/config.js');
        const { loadSettings, createMinimalSettings } = await import('./config/settings.js');
        const { loadSandboxConfig } = await import('./config/sandboxConfig.js');
        const { relaunchAppInChildProcess } = await import('./utils/relaunch.js');
        ...
        vi.mocked(createMinimalSettings).mockReturnValue(minimalSettings);
        vi.mocked(loadSandboxConfig).mockResolvedValue(undefined);
        ...
        expect(loadCliConfig).toHaveBeenCalledWith({}, expect.objectContaining({ bare: true }), process.cwd(), undefined, {
            userHooks: undefined,
    ...
    describe('registerLspHotReload', () => {
        it('does not register a watcher when LSP is disabled', () => {
            ...
            registerLspHotReload(withLspDisabledConfig({
        ...
        it('emits an LSP status update when reload is skipped by the config', async () => {
            ...
            expect(reinitializeLsp).toHaveBeenCalledOnce();
            expect(appEvents.emit).not.toHaveBeenCalledWith(AppEvent.LogError, expect.any(String));
            expect(appEvents.emit).toHaveBeenCalledWith(AppEvent.LspStatusChanged);
        });
        it('emits a user-visible error and rejects when reload fails', async () => {
            ...
            await expect(lspConfigWatcherMock.instances[0]?.listener?.({
                path: '/workspace/.lsp.json',
                changeType: 'modified',
            })).rejects.toThrow('invalid lsp json');
            expect(appEvents.emit).toHaveBeenCalledWith(AppEvent.LogError, 'Failed to reload LSP server settings: invalid lsp json. Some LSP servers may have been partially updated. Run with --debug for details.');
        });
        it('emits a user-visible error and rejects when reload has failed servers', async () => {
            ...
            expect(appEvents.emit).toHaveBeenCalledWith(AppEvent.LogError, 'LSP reload partially completed: changed=<none>, failed=clangd. Run with --debug for details.');
            expect(appEvents.emit).toHaveBeenCalledWith(AppEvent.LspStatusChanged);
        });
        it('surfaces invalid config without reinitializing LSP', async () => {
            ...
            expect(appEvents.emit).toHaveBeenCalledWith(AppEvent.LogError, 'Invalid JSON in .lsp.json; existing LSP runtime state is unchanged.');
        });
    });
    it('writes non-interactive warnings discovered during config initialization', async () => {
        ...
        const userStartupWarningsModule = await import('./utils/userStartupWarnings.js');
        ...
        vi.spyOn(userStartupWarningsModule, 'getUserStartupWarnings').mockResolvedValue([]);
        ...
        vi.mocked(loadCliConfig).mockResolvedValue(configStub);
        vi.spyOn(validatorModule, 'validateNonInteractiveAuth').mockResolvedValue(configStub);
        try {
            await main();
        }
        catch (error) {
        ...
        expect(initializerModule.initializeApp).toHaveBeenCalledWith(configStub, expect.any(Object), { deferIdeConnection: false });
        expect(mockStartNonInteractiveOpenAILogHousekeeping).toHaveBeenCalledWith(configStub, expect.any(Object));
        ...
        expect(mockStartNonInteractiveOpenAILogHousekeeping).toHaveBeenCalledTimes(2);
    });
    it('creates non-interactive prompt ids that preserve session correlation', () => {
        expect(createNonInteractivePromptId('test-session-id')).toBe('test-session-id########0');
    });
    const runSandboxRelaunch = async (argv, sessionId = '123e4567-e89b-12d3-a456-426614174000', command = 'sandbox-exec') => {
        const originalArgv = process.argv;
        process.argv = argv;
        const processExitSpy = vi
            .spyOn(process, 'exit')
    ...
    it('passes the outer session ID into the sandbox child process', async () => {
        ...
        const sandboxArgs = await runSandboxRelaunch(['node', 'script.js', '--debug', '-p', 'hello'], sessionId);
    ...
    it('starts a fresh CLI after the host update completes', async () => {
        ...
        expect(mockUpdateBeforeRelaunch).toHaveBeenCalledWith(expect.anything(), expect.any(String), true);
    });
    it('passes host update capability into a container sandbox', async () => {
        ...
        try {
            await runSandboxRelaunch(['node', 'script.js', '--debug', '-p', 'hello'], '', 'docker');
            expect(mockGetInstallationInfo).toHaveBeenCalledWith(expect.any(String), true);
    ...
    it('does not pass an empty session ID into the sandbox child process', async () => {
        const sandboxArgs = await runSandboxRelaunch(['node', 'script.js', '--debug', '-p', 'hello'], '');
    ...
    ])('does not inject sandbox session ID when argv contains %s', async (_flag, argv) => {
        const sandboxArgs = await runSandboxRelaunch(argv);
        expect(sandboxArgs).not.toContain('--sandbox-session-id');
    });
    it('inserts the sandbox session ID before the argument separator', async () => {
        const sessionId = '123e4567-e89b-12d3-a456-426614174000';
        const sandboxArgs = await runSandboxRelaunch(['node', 'script.js', '--debug', '--', '--not-a-cli-flag'], sessionId);
    ...
    it('starts housekeeping and cleans up stream-json success and failure', async () => {
        ...
        vi.spyOn(startupWarningsModule, 'getStartupWarnings').mockResolvedValue([]);
        vi.spyOn(userStartupWarningsModule, 'getUserStartupWarnings').mockResolvedValue([]);
        const validatedConfig = { validated: true };
        ...
        expect(validateAuthSpy).toHaveBeenCalledWith(undefined, configStub, expect.any(Object));
        expect(initializerModule.initializeApp).toHaveBeenCalledWith(configStub, expect.any(Object), { deferIdeConnection: false });
        expect(mockStartNonInteractiveOpenAILogHousekeeping).toHaveBeenCalledWith(validatedConfig, settingsArg);
        const housekeepingCleanup = vi.mocked(cleanupModule.registerCleanup).mock
            .calls[cleanupRegistrationStart]?.[0];
        expect(housekeepingCleanup).toBeTypeOf('function');
        await housekeepingCleanup?.();
...
describe('gemini.tsx main function kitty protocol', () => {
    ...
    it('should call setRawMode and detectAndEnableKittyProtocol when isInteractive is true', async () => {
        ...
        expect(initializeAppSpy).toHaveBeenCalledWith(expect.any(Object), expect.any(Object), {
            deferIdeConnection: true,
        });
        expect(mockStartPostRenderPrefetches).toHaveBeenCalledWith(expect.any(Object), expect.any(Object), {
            connectIde: true,
            initializeTelemetry: true,
        });
        expect(mockStartEarlyStartupPrefetches).toHaveBeenCalledWith(expect.any(Object));
    });
    ...
    it('should not defer IDE connection when Zed integration is enabled', async () => {
        ...
        expect(mockRunAcpAgent).toHaveBeenCalledWith(expect.any(Object), expect.any(Object), expect.any(Object), {
    ...
    function applyInteractiveSigintConfigMocks(loadCliConfig, loadSettings) {
        vi.mocked(loadCliConfig).mockResolvedValue({
            ...sessionRegistryConfigStub,
            isInteractive: () => true,
            getQuestion: () => '',
    ...
    it('exits on interactive SIGINT only after a second press inside the confirm window', async () => {
        ...
        const processOnSpy = vi.spyOn(process, 'on').mockImplementation(((eventName, listener) => {
            if (eventName === 'SIGTERM' || eventName === 'SIGINT') {
                ...
                if (!signalHandlers.has(eventName)) {
        ...
        try {
            ...
            expect(mockWriteStderrLine).toHaveBeenCalledWith('Press Ctrl+C again to exit.');
            ...
            // (#6776).
            expect(processOnSpy.mock.calls.filter(([event]) => event === 'SIGINT').length).toBeGreaterThan(1);
    ...
    it('rejects --json-schema when running in interactive (TUI) mode', async () => {
        ...
        expect(mockWriteStderrLine).toHaveBeenCalledWith(expect.stringContaining('--json-schema is a headless-only flag'));
...
describe('validateDnsResolutionOrder', () => {
    ...
    it('should return the default "ipv4first" and log a warning for an invalid string', () => {
        ...
        expect(mockWriteStderrLine).toHaveBeenCalledWith('Invalid value for dnsResolutionOrder in settings: "invalid-value". Using default "ipv4first".');
    });
});
describe('startInteractiveUI', () => {
    ...
    afterEach(() => {
        ...
        vi.unstubAllEnvs();
        restoreCiEnv();
        const currentExitListeners = process.listeners('exit');
    ...
    it('should not use alternate screen when VP mode is explicitly disabled', async () => {
        ...
        await startInteractiveUI(mockConfig, legacySettings, mockStartupWarnings, mockWorkspaceRoot, mockInitializationResult);
    ...
    it('should not use alternate screen in screen reader mode when VP mode is unset', async () => {
        ...
        await startInteractiveUI(screenReaderConfig, mockSettings, mockStartupWarnings, mockWorkspaceRoot, mockInitializationResult);
        const [, options] = renderSpy.mock.calls[0];
    ...
    it('should not use alternate screen in screen reader mode even when VP mode is explicitly enabled', async () => {
        ...
        await startInteractiveUI(screenReaderConfig, vpSettings, mockStartupWarnings, mockWorkspaceRoot, mockInitializationResult);
        const [, options] = renderSpy.mock.calls[0];
        expect(options).toMatchObject({
            isScreenReaderEnabled: true,
            alternateScreen: false,
    ...
    it('should perform all startup tasks in correct order', async () => {
        ...
        expect(mockStartPostRenderPrefetches).toHaveBeenCalledWith(mockConfig, mockSettings, { connectIde: false, initializeTelemetry: true });
    });
    it('delegates disabled auto-update settings to post-render prefetch', async () => {
        ...
        await startInteractiveUI(mockConfig, settingsWithAutoUpdateDisabled, mockStartupWarnings, mockWorkspaceRoot, mockInitializationResult);
        expect(mockStartPostRenderPrefetches).toHaveBeenCalledWith(mockConfig, settingsWithAutoUpdateDisabled, { connectIde: false, initializeTelemetry: true });
    });
    it('can skip post-render IDE connection after prompt-interactive awaited it', async () => {
        ...
        await startInteractiveUI(promptInteractiveConfig, mockSettings, mockStartupWarnings, mockWorkspaceRoot, mockInitializationResult, { postRenderConnectIde: false });
        expect(mockStartPostRenderPrefetches).toHaveBeenCalledWith(promptInteractiveConfig, mockSettings, { connectIde: false, initializeTelemetry: false });
    });
    ...
    it('disables the Kitty keyboard protocol only after Ink has unmounted', async () => {
        const unmount = vi.fn();
        const { render } = await import('ink');
        vi.mocked(render).mockReturnValue({ unmount });
        const { disableKittyProtocol } = await import('./ui/utils/kittyProtocolDetector.js');
        ...
        expect(vi.mocked(disableKittyProtocol).mock.invocationCallOrder[0]).toBeGreaterThan(unmount.mock.invocationCallOrder[0]);
    });
    ...
    it('does not echo when no render error was stored', async () => {
        ...
        const cleanupFn = vi.mocked(registerCleanup).mock.calls[0]?.[0];
        await cleanupFn?.();
        expect(mockWriteStderrLine).not.toHaveBeenCalledWith(expect.stringContaining('Rendering error'));
    });
    ...
    describe('exit-time resume echo', () => {
        async function runCleanup(config) {
            ...
            vi.mocked(render).mockReturnValue({ unmount });
            mockConsumeLastRenderError.mockReturnValue(undefined);
            await startInteractiveUI(config, mockSettings, mockStartupWarnings, mockWorkspaceRoot, {
                authError: null,
                themeError: null,
                shouldOpenAuthDialog: false,
                geminiMdFileCount: 0,
        ...
        function makeRecordingConfig(sessionId, sessionFile) {
            return {
                ...mockConfig,
        ...
        it.each([
            ...
            try {
                ...
                expect(mockWriteStdoutLine).toHaveBeenCalledWith(expect.stringContaining(`qwen --resume ${sessionId}`));
            }
            finally {
        ...
        it('does not echo when the transcript file is empty', async () => {
            ...
            try {
                await runCleanup(makeRecordingConfig(sessionId, sessionFile));
                expect(mockWriteStdoutLine).not.toHaveBeenCalledWith(expect.stringContaining('qwen --resume'));
            }
            finally {
                rmSync(projectDir, { recursive: true, force: true });
            }
    ...
    describe('periodic memory-pressure check', () => {
        ...
        it('runs performCheck on an interval without any tool calls', async () => {
            ...
            await startInteractiveUI(config, settings, mockStartupWarnings, mockWorkspaceRoot, {
                authError: null,
                themeError: null,
                shouldOpenAuthDialog: false,
                geminiMdFileCount: 0,
