 ...
 */
import { ACP_PRE_ATTACH_MAX_FRAMES_GLOBAL, ACP_PRE_ATTACH_MAX_FRAMES_PER_CONNECTION, ACP_PRE_ATTACH_MAX_FRAMES_PER_STREAM, ACP_PRE_ATTACH_MAX_PAYLOAD_BYTES_GLOBAL, ACP_PRE_ATTACH_MAX_PAYLOAD_BYTES_PER_CONNECTION, } from './acp-http/pre-attach-budget.js';
import { MAX_CHILD_HEAP_MB, MIN_CHILD_HEAP_MB, recommendedChildShareMb, } from '@lailatul-coder/acp-bridge/daemonMemoryBudget';
...
export function toDaemonStatusMemoryLimits(budget, childHeap, journalGrowth) {
    if (!budget)
        return null;
    return {
        enforced: false,
...
export async function buildDaemonStatusResponse(detail, input) {
    ...
    const aggregateRuntimes = input.workspaceRegistry?.listAll?.() ?? workspaceRuntimes;
    const workspaceSnapshots = aggregateRuntimes?.map((runtime) => ({
    ...
    if (memoryBudget) {
        ...
        let heapMajorGcMs = 0;
        const heapUnclassified = new Set();
        for (const runtime of managedRuntimes ?? []) {
            ...
            if (!heap)
                continue;
            heapReported += 1;
            peakOldGenerationBytes = Math.max(peakOldGenerationBytes, heap.peakOldGenerationBytes);
            peakLiveSetBytes = Math.max(peakLiveSetBytes, heap.peakLiveSetBytes);
            peakTotalHeapBytes = Math.max(peakTotalHeapBytes, heap.peakTotalHeapBytes);
    ...
    const queuedPrompts = aggregateRuntimes?.reduce((sum, runtime, index) => sum +
    ...
    if (detail === 'full') {
        full = await buildFullStatus(input, acpAggregate, workspaceSnapshots
    ...
    return {
        ...
        limits: {
            ...
            acpPreAttachMaxFramesPerStream: acpSnapshot !== undefined ? ACP_PRE_ATTACH_MAX_FRAMES_PER_STREAM : null,
            acpPreAttachMaxFramesPerConnection: acpSnapshot !== undefined
                ? ACP_PRE_ATTACH_MAX_FRAMES_PER_CONNECTION
                : null,
            acpPreAttachMaxFramesGlobal: acpSnapshot !== undefined ? ACP_PRE_ATTACH_MAX_FRAMES_GLOBAL : null,
            acpPreAttachMaxPayloadBytesPerConnection: acpSnapshot !== undefined
                ? ACP_PRE_ATTACH_MAX_PAYLOAD_BYTES_PER_CONNECTION
                : null,
            acpPreAttachMaxPayloadBytesGlobal: acpSnapshot !== undefined
                ? ACP_PRE_ATTACH_MAX_PAYLOAD_BYTES_GLOBAL
                : null,
            memory: toDaemonStatusMemoryLimits(memoryBudget, input.getChildHeapPolicySnapshot?.(), bridgeSnapshot.limits.journalGrowth
                ? {
        ...
        runtime: {
            ...
            transport: {
                ...
                acp: {
                    ...
                    preAttach: {
                        bufferedConnectionFrames: acpAggregate?.bufferedConnectionFrames ?? 0,
            ...
            ...(input.getMetricsSeries
                ...
                : {}),
            activity: {
                activePrompts: aggregateRuntimes?.reduce((sum, runtime) => sum + (runtime.bridge.activePromptCount ?? 0), 0) ??
