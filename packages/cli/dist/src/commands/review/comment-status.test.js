...
const { csGhMock, csGhApiAllMock, csCurrentUserMock, csEnsureAuthenticatedMock, csSetGhHostMock, csGitOptMock, csWorktreePathMock, csWriteFileSyncMock, csMkdirSyncMock, csStdoutMock, } = vi.hoisted(() => ({
    csGhMock: vi.fn(),
...
import { buildThreadStatuses, summarizeThreads, commentStatusCommand, } from './comment-status.js';
const noChange = () => ({
    changed: false,
    touchedBy: [],
    touchedByTotal: 0,
...
describe('buildThreadStatuses — thread grouping', () => {
    ...
    it('drops replies whose root was deleted, same as pr-context renders', () => {
        const threads = buildThreadStatuses([comment({ id: 5, in_reply_to_id: 999 })], 'author', noChange);
    ...
    it('marks the attribution-off posted shape as a blocker, same as pr-context', () => {
        ...
        ], 'author', noChange, 'lailatul-coder-ci-bot');
...
describe('buildThreadStatuses — anchor status', () => {
    it('marks a line comment with null line as outdated', () => {
        const [t] = buildThreadStatuses([comment({ id: 1, line: null })], 'author', noChange);
...
describe('buildThreadStatuses — signals', () => {
    it('flags a blocker body and threads the probe result through', () => {
        ...
        const [t] = buildThreadStatuses([comment({ id: 1, body: '🔴 Finding 1 — poll loop wedges (blocker)' })], 'author', probe);
    ...
    it('does not flag a negated blocker mention', () => {
        ...
        const [negated] = buildThreadStatuses([comment({ id: 1, body: 'No blocking issues here, just a nit.' })], 'author', noChange);
        ...
        const [asserted] = buildThreadStatuses([comment({ id: 2, body: 'This is a blocking defect.' })], 'author', noChange);
    ...
    it('never reports authorReplied for a ghost author matching a ghost replier', () => {
        ...
        const [t] = buildThreadStatuses([comment({ id: 1 }), comment({ id: 2, in_reply_to_id: 1, user: null })], '', noChange);
...
describe('summarizeThreads', () => {
    it('counts each status dimension once per thread', () => {
        const probe = (path) => path === 'src/changed.ts'
...
describe('commentStatusCommand handler — identity fail-closed', () => {
    ...
    const runHandler = () => Promise.resolve(commentStatusCommand.handler({
        pr_number: '42',
        owner_repo: 'o/r',
        out: '/tmp/comment-status/report.json',
    }));
    const writtenReport = () => JSON.parse(csWriteFileSyncMock.mock.calls[0]?.[1]);
    beforeEach(() => {
        csGhMock.mockClear();
        csGhMock.mockReturnValue(JSON.stringify({ author: { login: 'author' }, headRefOid: 'headsha' }));
    ...
    it('refuses the report when the login is EMPTY while a critical marker is posted', async () => {
        ...
        await runHandler();
        expect(writtenReport().error).toMatch(/cannot determine the reviewing account/);
    });
...
//# sourceMappingURL=comment-status.test.js.map