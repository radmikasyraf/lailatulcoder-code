 ...
 */
export declare const LIVE_HOST_PROTOCOL_VERSION: 6;
export declare const LIVE_HOST_BUNDLE_ID: "com.alibaba.lailatul-coder.live-host";
export declare const LIVE_INPUT_AUDIO_EPOCH_BYTES = 8;
export type LiveState = 'unavailable' | 'idle' | 'starting' | 'listening' | 'thinking' | 'speaking' | 'stopping' | 'error';
export type LiveBlocker = 'host_missing' | 'host_disconnected' | 'host_version' | 'microphone_permission' | 'accessibility_permission' | 'screen_recording_permission' | 'audio_input' | 'audio_output' | 'global_shortcut' | 'appshot' | 'provider_config' | 'provider_unreachable';
export type LiveRequirementState = 'ready' | 'missing' | 'denied' | 'unavailable' | 'checking';
...
export interface LiveStatus {
    ...
    requirements?: Partial<Record<'host' | 'microphone' | 'accessibility' | 'screenRecording' | 'audioInput' | 'audioOutput' | 'globalShortcut' | 'appshot' | 'provider', LiveRequirementState>>;
    host?: {
        version?: string;
...
export type LiveHostMessage = LiveHostHello | LiveHostAction | LiveHostPong | LiveHostShortcutResult | LiveHostScreenContextResult;
