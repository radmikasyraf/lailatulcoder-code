...
const createMockGetAvailableModelsForAuthType = () => vi.fn((t) => {
...
const renderComponent = (props = {}, contextValue = undefined, settingsValue = undefined) => {
    const defaultProps = {
        onClose: vi.fn(),
    };
    ...
    const mockConfig = {
        ...
        getAllConfiguredModels: vi.fn(() => getFilteredQwenModels().map((m) => ({
            id: m.id,
            label: m.label,
    ...
    const renderResult = render(_jsx(SettingsContext.Provider, { value: mockSettings, children: _jsx(ConfigContext.Provider, { value: mockConfig, children: _jsx(UIStateContext.Provider, { value: { historyManager: mockHistoryManager }, children: _jsx(ModelDialog, { ...combinedProps }) }) }) }));
    return {
        ...renderResult,
        props: combinedProps,
        mockConfig,
...
describe('<ModelDialog />', () => {
    ...
    it('passes all model options to DescriptiveRadioButtonSelect', () => {
        ...
        expect(props.items[0].value).toBe(`${AuthType.QWEN_OAUTH}::${DEFAULT_QWEN_MODEL}`);
        expect(props.showNumbers).toBe(true);
    });
    it('caps visible model options to the available dialog height', () => {
        renderComponent({ availableTerminalHeight: 20 }, {
            getModel: vi.fn(() => 'model-1'),
            getAuthType: vi.fn(() => AuthType.USE_OPENAI),
            getAllConfiguredModels: vi.fn(() => Array.from({ length: 12 }, (_, i) => ({
                id: `model-${i + 1}`,
                label: `Model ${i + 1}`,
                description: '',
    ...
    it('floors visible model options to 1 when the terminal is very short', () => {
        renderComponent({ availableTerminalHeight: 5 }, {
            getModel: vi.fn(() => 'model-1'),
            getAuthType: vi.fn(() => AuthType.USE_OPENAI),
            getAllConfiguredModels: vi.fn(() => Array.from({ length: 12 }, (_, i) => ({
                id: `model-${i + 1}`,
    ...
    it('clamps visible model options to the default max when the terminal is tall', () => {
        renderComponent({ availableTerminalHeight: 100 }, {
            getModel: vi.fn(() => 'model-1'),
            getAuthType: vi.fn(() => AuthType.USE_OPENAI),
            getAllConfiguredModels: vi.fn(() => Array.from({ length: 12 }, (_, i) => ({
                id: `model-${i + 1}`,
    ...
    it('shrinks visible model options to leave room for a displayed error message', async () => {
        ...
        const propsAfterError = mockedSelect.mock.calls[mockedSelect.mock.calls.length - 1][0];
    ...
    it('initializes with the model from ConfigContext', () => {
        ...
        const expectedIndex = qwenModels.findIndex((m) => m.id === DEFAULT_QWEN_MODEL);
    ...
    it('blocks qwen-oauth model selection with an error message (discontinued)', async () => {
        const { props, mockConfig } = renderComponent({}, {
            getAvailableModelsForAuthType: vi.fn((t) => {
                if (t === AuthType.QWEN_OAUTH) {
                    return getFilteredQwenModels().map((m) => ({
                        id: m.id,
                        label: m.label,
    ...
    it('calls config.switchModel and onClose when selecting a non-OAuth model', async () => {
        ...
        const getAvailableModelsForAuthType = vi.fn((t) => {
        ...
        expect(mockSettings.setValue).toHaveBeenCalledWith(SettingScope.User, 'model.name', 'gpt-4');
        ...
        expect(mockSettings.setValue).toHaveBeenCalledWith(SettingScope.User, 'model.baseUrl', '');
        expect(mockSettings.setValue).toHaveBeenCalledWith(SettingScope.User, 'security.auth.selectedType', AuthType.USE_OPENAI);
    ...
    it('persists model.baseUrl alongside model.name when the selected provider has a baseUrl', async () => {
        ...
        await childOnSelect(`${AuthType.USE_OPENAI}::qwen3.7-max\0https://idealab.example.com/v1`);
        expect(switchModel).toHaveBeenCalledWith(AuthType.USE_OPENAI, 'qwen3.7-max', {
            baseUrl: 'https://idealab.example.com/v1',
        });
        expect(mockSettings.setValue).toHaveBeenCalledWith(SettingScope.User, 'model.name', 'qwen3.7-max');
        expect(mockSettings.setValue).toHaveBeenCalledWith(SettingScope.User, 'model.baseUrl', 'https://idealab.example.com/v1');
        expect(props.onClose).toHaveBeenCalledTimes(1);
    });
    ...
    it('hydrates provider API key env from settings.env before switching', async () => {
        ...
        try {
            ...
            expect(switchModel).toHaveBeenCalledWith(AuthType.USE_OPENAI, 'MiniMax-M3', { baseUrl: 'https://api.minimaxi.com/v1' });
        }
        finally {
    ...
    it('stores authType-qualified selectors in fast model mode', async () => {
        ...
        const { props, mockSettings, recordSlashCommand } = renderComponent({ isFastModelMode: true }, {
        ...
        expect(mockSettings.setValue).toHaveBeenCalledWith(SettingScope.User, 'fastModel', 'openai:deepseek-v4-flash');
    ...
    it('stores authType-qualified selectors in vision model mode without switching models', async () => {
        ...
        const { props, mockSettings, recordSlashCommand } = renderComponent({ isVisionModelMode: true }, {
            getAuthType: vi.fn(() => AuthType.USE_ANTHROPIC),
            getModel: vi.fn(() => 'claude-opus-4-7'),
            switchModel,
            getAllConfiguredModels: vi.fn(() => [
            ...
            isCurrentPrimaryModel: (m) => m.id === 'claude-opus-4-7' && m.authType === AuthType.USE_ANTHROPIC,
            setVisionModel,
        });
        const childOnSelect = mockedSelect.mock.calls[0][0].onSelect;
        await childOnSelect(`${AuthType.USE_OPENAI}::qwen-vl-max`);
        expect(mockSettings.setValue).toHaveBeenCalledWith(SettingScope.User, 'visionModel', 'openai:qwen-vl-max');
        ...
        expect(mockSettings.setValue).not.toHaveBeenCalledWith(SettingScope.User, 'model.name', expect.any(String));
        expect(props.onClose).toHaveBeenCalledTimes(1);
    });
    it('stores compaction model selector without switching models', async () => {
        const switchModel = vi.fn();
        const setCompactionModel = vi.fn();
        const { props, mockSettings, recordSlashCommand } = renderComponent({ isCompactionModelMode: true }, {
            getAuthType: vi.fn(() => AuthType.USE_OPENAI),
            getModel: vi.fn(() => 'gpt-4'),
            switchModel,
            getAllConfiguredModels: vi.fn(() => [
            ...
            isCurrentPrimaryModel: (m) => m.id === 'gpt-4' && m.authType === AuthType.USE_OPENAI,
            setCompactionModel,
        });
        const childOnSelect = mockedSelect.mock.calls[0][0].onSelect;
        await childOnSelect(`${AuthType.USE_OPENAI}::compaction-model`);
        expect(mockSettings.setValue).toHaveBeenCalledWith(SettingScope.User, 'compactionModel', 'openai:compaction-model');
    ...
    it('shows only image-generation models and stores the exact provider route', async () => {
        ...
        const { props, mockSettings, getByText, recordSlashCommand } = renderComponent({ isImageModelMode: true }, {
            getAuthType: vi.fn(() => AuthType.USE_OPENAI),
            getAllConfiguredModels: vi.fn(() => [
                {
                    id: 'qwen-plus',
            ...
            resolveImageGenerationModel: vi.fn((selector) => selector === persisted
                ? {
        ...
        await selectProps.onSelect(`${AuthType.USE_OPENAI}::qwen-image-2.0\0${baseUrl}`);
        expect(mockSettings.setValue).toHaveBeenCalledWith(SettingScope.User, 'imageModel', persisted);
    ...
    it('ignores duplicate input while an image model selection is in flight', async () => {
        let resolveSetImageModel;
        const setImageModel = vi.fn(() => new Promise((resolve) => {
            resolveSetImageModel = resolve;
        }));
        const baseUrl = 'https://images.example.com/api/v1';
        const persisted = `openai:qwen-image-2.0\0${baseUrl}`;
        const { props, mockSettings, mockHistoryManager, recordSlashCommand } = renderComponent({ isImageModelMode: true }, {
            getAuthType: vi.fn(() => AuthType.USE_OPENAI),
            getAllConfiguredModels: vi.fn(() => [
                {
                    id: 'qwen-image-2.0',
        ...
        const selection = onSelect(`${AuthType.USE_OPENAI}::qwen-image-2.0\0${baseUrl}`);
    ...
    it('keeps the selected baseUrl for same-provider duplicate vision model ids', async () => {
        ...
        const { props, mockSettings } = renderComponent({ isVisionModelMode: true }, {
            getAuthType: vi.fn(() => AuthType.USE_OPENAI),
            getModel: vi.fn(() => 'qwen3.7-max'),
            switchModel,
            getAllConfiguredModels: vi.fn(() => [
            ...
            isCurrentPrimaryModel: (m) => m.id === 'qwen3.7-max',
            setVisionModel,
        });
        const childOnSelect = mockedSelect.mock.calls[0][0].onSelect;
        await childOnSelect(`${AuthType.USE_OPENAI}::qwen3.7-plus\0${selectedBaseUrl}`);
        expect(mockSettings.setValue).toHaveBeenCalledWith(SettingScope.User, 'visionModel', `openai:qwen3.7-plus\0${selectedBaseUrl}`);
        expect(setVisionModel).toHaveBeenCalledWith(`openai:qwen3.7-plus\0${selectedBaseUrl}`);
        expect(switchModel).not.toHaveBeenCalled();
        expect(props.onClose).toHaveBeenCalledTimes(1);
    });
    it('warns in the history when a pinned vision model is not image-capable', async () => {
        ...
        const setVisionModel = vi.fn();
        const { mockHistoryManager } = renderComponent({ isVisionModelMode: true }, {
            getAuthType: vi.fn(() => AuthType.USE_OPENAI),
            getModel: vi.fn(() => 'qwen3.7-max'),
            getAllConfiguredModels: vi.fn(() => [
                {
    ...
    it('stores the plain model id in voice model mode without switching models', async () => {
        const switchModel = vi.fn();
        const setFastModel = vi.fn();
        const { props, mockSettings, recordSlashCommand } = renderComponent({ isVoiceModelMode: true }, {
            getAuthType: vi.fn(() => AuthType.USE_OPENAI),
            getModel: vi.fn(() => 'qwen3.7-max'),
            switchModel,
            getAllConfiguredModels: vi.fn(() => [
        ...
        expect(mockSettings.setValue).toHaveBeenCalledWith(SettingScope.User, 'voiceModel', 'qwen3-asr-flash');
        expect(switchModel).not.toHaveBeenCalled();
    ...
    it('does not store a voice model without a transcription baseUrl', async () => {
        const switchModel = vi.fn();
        const { props, mockSettings } = renderComponent({ isVoiceModelMode: true }, {
            getAuthType: vi.fn(() => AuthType.USE_OPENAI),
            getModel: vi.fn(() => 'qwen3.7-max'),
            switchModel,
            getAllConfiguredModels: vi.fn(() => [
    ...
    it('highlights the cross-auth row for a bare fast-model setting', () => {
        ...
        render(_jsx(SettingsContext.Provider, { value: mockSettings, children: _jsx(ConfigContext.Provider, { value: {
                    getModel: vi.fn(() => 'claude-opus-4-7'),
                    getAuthType: vi.fn(() => AuthType.USE_ANTHROPIC),
                    ...
                    getUseModelRouter: vi.fn(() => false),
                    getProxy: vi.fn(() => undefined),
                }, children: _jsx(ModelDialog, { onClose: vi.fn(), isFastModelMode: true }) }) }));
        const items = mockedSelect.mock.calls[0][0].items;
        const deepseekIndex = items.findIndex((item) => String(item.value).includes('deepseek-v4-flash'));
    ...
    it('highlights the cross-auth row for a bare vision-model setting', () => {
        ...
        render(_jsx(SettingsContext.Provider, { value: mockSettings, children: _jsx(ConfigContext.Provider, { value: {
                    ...
                    getUseModelRouter: vi.fn(() => false),
                    getProxy: vi.fn(() => undefined),
                }, children: _jsx(ModelDialog, { onClose: vi.fn(), isVisionModelMode: true }) }) }));
        const items = mockedSelect.mock.calls[0][0].items;
        const visionIndex = items.findIndex((item) => String(item.value).includes('qwen-vl-max'));
    ...
    it('highlights the matching baseUrl for duplicate vision-model settings', () => {
        ...
        const items = mockedSelect.mock.calls[0][0].items;
        const visionIndex = items.findIndex((item) => String(item.value).includes('qwen3.7-plus') &&
    ...
    ])('does not report the primary model when closing an auxiliary picker (%j, %s)', (modeProps, keyName) => {
    ...
    it('reports the active runtime model when closing the primary picker', () => {
        ...
        expect(mockHistoryManager.addItem).toHaveBeenCalledWith({ type: 'info', text: 'Kept model as runtime-model' }, expect.any(Number));
    });
    it('does not report the unchanged model when a selection is made', async () => {
        ...
        await childOnSelect(`${AuthType.USE_OPENAI}::gpt-4`);
        expect(props.onClose).toHaveBeenCalledTimes(1);
        expect(mockHistoryManager.addItem).not.toHaveBeenCalledWith(expect.objectContaining({
    ...
    it('records successful model-switch feedback for resumed history', async () => {
        ...
        await act(async () => {
            await mockedSelect.mock.calls[0][0].onSelect(`${AuthType.USE_OPENAI}::gpt-4`);
        });
    ...
    it('remains dismissible after a failed model switch', async () => {
        ...
        expect(props.onClose).toHaveBeenCalledTimes(1);
        expect(mockHistoryManager.addItem).toHaveBeenCalledWith({ type: 'info', text: 'Kept model as gpt-4' }, expect.any(Number));
    });
    it('ignores escape while a model selection is in flight', async () => {
        let resolveSwitch;
        const switchModel = vi.fn(() => new Promise((resolve) => {
        ...
        const selection = mockedSelect.mock.calls[0][0].onSelect(`${AuthType.USE_OPENAI}::gpt-4`);
        const keyPressHandler = mockedUseKeypress.mock.calls[0][0];
        keyPressHandler({
            name: 'escape',
            ctrl: false,
    ...
    it('does not retry or report an unchanged model after persistence fails', async () => {
        ...
        const { props, getByText, mockHistoryManager, recordSlashCommand } = renderComponent({}, {
        ...
        expect(getByText((text) => text.includes('Model switched, but the selection could not be saved.'))).toBeDefined();
        await onSelect(`${AuthType.USE_OPENAI}::gpt-4`);
        mockedUseKeypress.mock.calls[0][0]({
            name: 'escape',
            ctrl: false,
    ...
    it('updates initialIndex when config context changes', () => {
        ...
        const { rerender } = render(_jsx(SettingsContext.Provider, { value: mockSettings, children: _jsx(ConfigContext.Provider, { value: {
                    getModel: mockGetModel,
                ...
                }, children: _jsx(ModelDialog, { onClose: vi.fn() }) }) }));
        ...
        rerender(_jsx(SettingsContext.Provider, { value: mockSettings, children: _jsx(ConfigContext.Provider, { value: newMockConfig, children: _jsx(ModelDialog, { onClose: vi.fn() }) }) }));
        // Should be called at least twice: initial render + re-render after context change
        expect(mockedSelect).toHaveBeenCalledTimes(2);
        // Calculate expected index for DEFAULT_QWEN_MODEL dynamically
        const qwenModels = getFilteredQwenModels();
        const expectedCoderIndex = qwenModels.findIndex((m) => m.id === DEFAULT_QWEN_MODEL);
...
describe('encodeAuxModelSelector', () => {
    it('encodes the "authType::modelId" key, dropping the baseUrl', () => {
        expect(encodeAuxModelSelector('openai::gpt-4o\0https://api.example.com')).toBe('openai:gpt-4o');
    ...
    it('encodes the "$runtime|authType|modelId" key by positional split', () => {
        expect(encodeAuxModelSelector('$runtime|openai|gpt-4o')).toBe('openai:gpt-4o');
    });
