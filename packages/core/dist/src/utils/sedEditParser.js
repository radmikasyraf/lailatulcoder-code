...
export function parseSedEditCommand(command) {
...
function isSafeCombinedFlagArg(arg) {
...
function hasShellVariableReference(value) {
    return (value.includes('`') ||
        /\$(?:[A-Za-z_][A-Za-z0-9_]*|\{|\(|\d|[#?@$!*])/u.test(value));
}
function canCompileSedPattern(sedInfo) {
    try {
        ...
        if (hasReplacementBackrefBeyondCaptures(sedInfo.replacement, countCapturingGroups(jsPattern))) {
            return false;
        }
...
function hasPosixBracketExpression(pattern) {
 ...
 */
function hasLeadingBracketLiteral(pattern) {
    return /\[\^?\]/u.test(pattern);
}
function hasSedJavascriptDivergentEscape(pattern) {
    return /\\[<>dDwWsS]/u.test(pattern);
}
function hasReplacementBackrefBeyondCaptures(replacement, captureCount) {
...
function hasUnsafeQuantifiedGroup(pattern) {
    const groups = [];
    for (let i = 0; i < pattern.length; i++) {
        const char = pattern[i];
        if (char === '\\') {
...
function getRegexQuantifierEnd(pattern, start) {
...
function parseSubstitution(expression) {
    if (/[\r\n]/.test(expression)) {
        return null;
    }
...
function isSupportedFlags(flags) {
    if (!/^[g0-9]*$/.test(flags)) {
        return false;
    }
...
function hasUnsupportedReplacementEscape(replacement) {
    for (let i = 0; i < replacement.length; i++) {
        if (replacement[i] !== '\\') {
            continue;
        }
...
function toJavascriptPattern(sedInfo) {
...
function unescapeSedDelimiter(pattern) {
    let result = '';
    for (let i = 0; i < pattern.length; i++) {
        const char = pattern[i];
        if (char === '\\' && pattern[i + 1] === '/') {
...
function getOccurrence(flags) {
    const match = flags.match(/[1-9][0-9]*/);
    return match ? Number(match[0]) : null;
}
function replaceLine(line, globalRegex, replacement, options) {
    let seen = 0;
    let lastMatchWasNonEmpty = false;
    let lastMatchEnd = -1;
    globalRegex.lastIndex = 0;
    return line.replace(globalRegex, (...args) => {
        ...
        const shouldReplace = options.occurrence === null
...
function buildReplacement(match, captures, replacement) {
    let result = '';
    for (let i = 0; i < replacement.length; i++) {
...
//# sourceMappingURL=sedEditParser.js.map