...
import { translateAndCheckAbsoluteWorkspacePath, MAX_WORKSPACE_PATH_LENGTH, } from '@lailatul-coder/acp-bridge/workspacePaths';
...
import { isPortableAbsolutePath, resolveManagedWorkspaceRuntimeByPathSelector, } from '../workspace-route-runtime.js';
import { normalizeWorkspaceDisplayName, workspaceRegistrationId, WorkspaceDisplayNameValidationError, WorkspaceRegistrationStoreCommittedError, WorkspaceRegistrationStoreLimitError, } from '../workspace-registration-store.js';
import { createManagedScratchDirectory, isScratchRootCompatible, } from '../managed-scratch-workspace.js';
import { NativeDirectoryPickerUnavailableError, pickNativeDirectory, } from '../native-directory-picker.js';
...
export function registerWorkspaceManagementRoutes(app, deps) {
    const { workspaceRegistry, mutate, safeBody, createWorkspaceRuntime, managedScratchRoot, validateWorkspaceRuntimeForPublication, runWorkspaceTrustOperation, workspaceRegistrationStore, getAcpHandle, runtimeRemoval, pickWorkspaceDirectory: pickWorkspaceDirectoryOverride, reservedWorkspaceRoots = [], } = deps;
    const pickWorkspaceDirectory = pickWorkspaceDirectoryOverride ?? pickNativeDirectory;
    ...
    const isReservedWorkspacePath = (candidate) => {
    ...
    const sendSealed = (res) => {
        res.status(503).json({
            error: 'Daemon is shutting down',
            code: 'daemon_shutting_down',
        });
    };
    const attachRegistrationIds = (runtime, registrationIds) => {
    ...
    const restorePersistedDisplayName = async (runtime, canonical) => {
        const snapshot = await workspaceRegistrationStore.read();
        const storedWorkspace = snapshot.workspaces.find((stored) => process.platform === 'win32'
    ...
    const projectedWorkspaceCount = () => {
        ...
        const cwdSet = new Set(workspaceRegistry.listManaged().map((runtime) => runtime.workspaceCwd));
    ...
    const conflictsWithRegisteredWorkspace = (canonical) => workspaceRegistry.listManaged().some((runtime) => {
        if (runtime.workspaceCwd === canonical)
            return false;
        if (isWithinRoot(canonical, runtime.workspaceCwd))
            return true;
        return (runtime.provenance !== 'live-conversation' &&
            isWithinRoot(runtime.workspaceCwd, canonical));
    });
    const assertOwnedRuntimeAdmission = (canonical, provenance) => {
        if (sealed)
        ...
        const nestingConflict = [
            ...
            ...[...inFlight].flatMap(([cwd, operation]) => operation === 'addition' ? [cwd] : []),
        ].some((cwd) => {
            if (cwd === canonical)
                return false;
            if (isWithinRoot(cwd, canonical))
                return true;
    ...
    const publishOwnedRuntime = async (canonicalCwd, provenance, validateBeforePublication) => {
        ...
        try {
            ...
            const publish = async () => {
                ...
                const nestingConflict = workspaceRegistry
                    .listManaged()
                    .some((entry) => {
                    if (entry.workspaceCwd === canonicalCwd)
                        return false;
                    if (isWithinRoot(entry.workspaceCwd, canonicalCwd))
                        return true;
                    return (provenance !== 'live-conversation' &&
                ...
                try {
                    ...
                    try {
                        writeStderrLine(`LailatulCoder serve: workspace runtime adapter notification failed after registry add: ${error instanceof Error ? error.message : String(error)}`);
                    }
                    catch {
    ...
    const createScratchWorkspace = async (res) => {
        ...
        if (workspaceRegistry
            .listManaged()
            .some((runtime) => !isInternalWorkspaceRuntime(runtime) &&
            !isScratchRootCompatible(runtime.workspaceCwd, managedScratchRoot.canonicalRoot)) ||
            [...inFlight].some(([cwd, operation]) => operation === 'addition' &&
        ...
        try {
            ...
            for (const [cwd, operation] of inFlight) {
                if (operation === 'addition' && cwd !== canonical)
                    boundCwds.push(cwd);
            }
            if (boundCwds.some((cwd) => !isScratchRootCompatible(cwd, managedScratchRoot.canonicalRoot) ||
            ...
            if (runtime.workspaceCwd !== canonical ||
                runtime.primary ||
                !runtime.trusted) {
                throw new Error('Scratch runtime violated the managed runtime contract');
            }
            if (sealed) {
                sendSealed(res);
                return;
            ...
            try {
                ...
                try {
                    writeStderrLine(`LailatulCoder serve: workspace runtime adapter notification failed after registry add: ${err instanceof Error ? err.message : String(err)}`);
                }
                catch {
                    // The runtime is registered; diagnostics are best-effort.
                }
            ...
            writeStderrLine(`LailatulCoder serve: scratch workspace registration failed: ${err instanceof Error ? err.message : String(err)}${canonical ? `; retained directory: ${canonical}` : ''}`);
            if (!res.headersSent) {
                res.status(500).json({
    ...
    app.get('/workspace-path-suggestions', async (req, res) => {
    ...
    app.post('/workspace-directory-picker', mutate(), async (req, res) => {
        ...
        try {
            ...
            if (error instanceof NativeDirectoryPickerUnavailableError) {
                writeStderrLine(`LailatulCoder serve: native directory picker unavailable: ${detail}`);
                res.status(501).json({
            ...
            writeStderrLine(`LailatulCoder serve: native directory picker failed: ${detail}`);
            res.status(500).json({
    ...
    app.post('/workspaces', mutate(), (req, res, next) => {
        ...
        if ('kind' in body) {
            if (body['kind'] !== 'scratch' ||
                ...
                res.status(400).json({
                    error: 'Scratch workspace requests must be exactly { kind: "scratch" }',
        ...
        if (hasDisplayName) {
            try {
                ...
                if (!(err instanceof WorkspaceDisplayNameValidationError))
                    throw err;
                res.status(400).json({
                    error: err.message,
        ...
        const existingRuntime = workspaceRegistry.getManagedByWorkspaceCwd(canonical);
        ...
        if (existingRuntime && persist && !existingRuntime.primary) {
            ...
            const nested = conflictsWithRegisteredWorkspace(canonical) ||
                [...inFlight].some(([cwd, operation]) => cwd !== canonical &&
            ...
            try {
                const snapshot = await workspaceRegistrationStore.read();
                const persistedWorkspaces = snapshot.workspaces.filter((stored) => existingRuntime.registrationIds?.includes(workspaceRegistrationId(stored)) === true ||
                ...
                if (alreadyPersisted) {
                    attachRegistrationIds(existingRuntime, persistedWorkspaces.map(workspaceRegistrationId));
                    const storedDisplayName = persistedWorkspaces
                        .map((stored) => snapshot.displayNames?.[workspaceRegistrationId(stored)])
                    ...
                    try {
                        ...
                        added =
                            persistedDisplayName === undefined
                                ...
                                : await workspaceRegistrationStore.add(canonical, persistedDisplayName);
                    }
                    catch (err) {
                        ...
                        added = true;
                        try {
                            writeStderrLine(`LailatulCoder serve: ${err.message}`);
                        }
                        catch {
                ...
                writeStderrLine(`LailatulCoder serve: failed to persist existing workspace registration: ${err instanceof Error ? err.message : String(err)}`);
                res.status(500).json({
        ...
        try {
            ...
            try {
                        ...
                        try {
                            persistedRecordAdded =
                                displayName === undefined
                                    ? await workspaceRegistrationStore.add(canonical)
                                    : await workspaceRegistrationStore.add(canonical, displayName);
                        }
                        catch (err) {
                            if (!(err instanceof WorkspaceRegistrationStoreCommittedError)) {
                                throw err;
                ...
                const requestTrustReconcile = req.app.locals.requestTrustReconcile;
                ...
                if (persistedRecordAdded) {
                    try {
                        ...
                        writeStderrLine(`LailatulCoder serve: failed to roll back workspace persistence after runtime registration failure: ${rollbackErr instanceof Error
                            ? rollbackErr.message
                            : String(rollbackErr)}`);
                    }
                }
            ...
            writeStderrLine(`LailatulCoder serve: POST /workspaces failed for ${canonical}: ${err instanceof Error ? err.message : String(err)}`);
    ...
    const workspaceActivity = (runtime) => {
        const controllerActivity = runtimeRemoval?.getActivity(runtime) ?? {
            pendingSessionStarts: 0,
            channelWorkers: 0,
            voiceSessions: 0,
        };
        const acpActivity = getAcpHandle?.()?.getWorkspaceActivity(runtime.workspaceId) ?? { acpConnections: 0, memoryTasks: 0 };
        return {
    ...
    const isBusy = (activity) => Object.values(activity).some((count) => count > 0);
    const resolveManagedRuntime = (req, res) => {
        const selector = String(req.params['workspace'] ?? '');
        const byId = workspaceRegistry.getManagedByWorkspaceId(selector);
        if (byId && !isInternalWorkspaceRuntime(byId))
            return byId;
        ...
        const runtime = resolveManagedWorkspaceRuntimeByPathSelector(workspaceRegistry, selector);
        if (runtime)
            return runtime;
        res.status(400).json({
    ...
    app.patch('/workspaces/:workspace', mutate({ strict: true }), async (req, res) => {
        const body = safeBody(req);
        const unsupportedField = Object.keys(body).find((field) => field !== 'displayName');
        if (unsupportedField) {
            res.status(400).json({
        ...
        try {
            if (workspaceRegistrationStore &&
                ...
                try {
                    await workspaceRegistrationStore.setDisplayNameByIds(runtime.registrationIds, displayName);
                }
                catch (err) {
                    if (!(err instanceof WorkspaceRegistrationStoreCommittedError)) {
                        throw err;
            ...
            writeStderrLine(`LailatulCoder serve: failed to persist workspace display name: ${err instanceof Error ? err.message : String(err)}`);
            res.status(500).json({
    ...
    app.delete('/workspaces/:workspace', mutate({ strict: true }), async (req, res) => {
        const body = safeBody(req);
        ...
        const logCleanupFailure = (message) => {
            try {
        ...
        const convergeCommittedRemoval = async () => {
            try {
                getAcpHandle?.()?.commitWorkspaceRemoval(runtime.workspaceId);
            }
            catch (err) {
                logCleanupFailure(`LailatulCoder serve: failed to commit workspace ACP removal: ${err instanceof Error ? err.message : String(err)}`);
            }
            await runtimeRemoval
                .disposeRuntime(runtime, 'workspace_removed')
                .catch((err) => {
                logCleanupFailure(`LailatulCoder serve: workspace runtime cleanup failed: ${err instanceof Error ? err.message : String(err)}`);
                try {
                    runtime.bridge.killAllSync();
                }
                catch {
            ...
            try {
                ...
                logCleanupFailure(`LailatulCoder serve: failed to dispose workspace ACP mount: ${err instanceof Error ? err.message : String(err)}`);
            }
            try {
                runtimeRemoval.completeDrain(runtime);
            }
            catch (err) {
                logCleanupFailure(`LailatulCoder serve: failed to complete workspace admission drain: ${err instanceof Error ? err.message : String(err)}`);
            }
            try {
                workspaceRegistry.completeDrain(runtime);
            }
            catch (err) {
                logCleanupFailure(`LailatulCoder serve: failed to complete workspace registry drain: ${err instanceof Error ? err.message : String(err)}`);
            }
            registryDraining = false;
            controllerDraining = false;
            acpDraining = false;
        ...
        try {
            ...
            if (workspaceRegistrationStore) {
                try {
                    ...
                    rollbackDrain();
                    writeStderrLine(`LailatulCoder serve: failed to remove workspace persistence: ${err instanceof Error ? err.message : String(err)}`);
                    res.status(500).json({
            ...
            try {
                ...
                logCleanupFailure(`LailatulCoder serve: failed to commit workspace registry drain: ${err instanceof Error ? err.message : String(err)}`);
            }
            ...
            rollbackDrain();
            writeStderrLine(`LailatulCoder serve: DELETE /workspaces/:workspace failed: ${err instanceof Error ? err.message : String(err)}`);
            if (!res.headersSent) {
                res.status(500).json({
    ...
    const registrationIsActive = (registrationId) => workspaceRegistry.listManaged().some((runtime) => {
    ...
    app.get('/workspace-registrations', async (_req, res) => {
        ...
        try {
            ...
            writeStderrLine(`LailatulCoder serve: failed to read workspace registrations: ${err instanceof Error ? err.message : String(err)}`);
            res.status(500).json({
    ...
    app.delete('/workspace-registrations/:id', mutate({ strict: true }), async (req, res) => {
        if (!workspaceRegistrationStore) {
            res.status(501).json({
                error: 'Persistent workspace registration is not available',
                code: 'persistence_not_available',
        ...
        try {
            ...
            let runtime = workspaceRegistry
                .listManaged()
                .find((candidate) => workspaceRegistrationId(candidate.workspaceCwd) ===
                registrationId ||
            ...
            if (!operationCwd) {
                ...
                try {
                    ...
                    writeStderrLine(`LailatulCoder serve: failed to read workspace registration before forget: ${err instanceof Error ? err.message : String(err)}`);
                    res.status(500).json({
            ...
            const active = !reservedRegistration && registrationIsActive(registrationId);
            let removed;
            try {
            ...
            if (runtime?.registrationIds) {
                runtime.registrationIds = runtime.registrationIds.filter((id) => id !== registrationId);
                workspaceRegistry.syncRuntimeMetadata(runtime);
            }
            res.json({
                removed: true,
            ...
            writeStderrLine(`LailatulCoder serve: failed to forget workspace registration: ${err instanceof Error ? err.message : String(err)}`);
            res.status(500).json({
            ...
            if (ownsInFlight && operationCwd)
                inFlight.delete(operationCwd);
            operationFinished();
        }
    });
    return {
...
//# sourceMappingURL=workspace-management.js.map