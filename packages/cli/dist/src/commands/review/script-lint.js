...
import { readFileSync, writeFileSync, mkdirSync, mkdtempSync, rmSync, lstatSync, realpathSync, openSync, readSync, closeSync, } from 'node:fs';
...
export function toolFor(path, firstLine) {
    ...
    if (/^#!.*\b(sh|bash|dash|ksh)\b/.test(firstLine))
        return 'shellcheck';
    return null;
}
...
function addedRangesFromDiff(diffText) {
    ...
    for (const f of parsed.files) {
        map.set(f.path, f.addedRanges.map((r) => [r.start, r.end]));
    }
    return map;
}
 ...
 */
let hadolintEmptyConfigPath;
function emptyHadolintConfig() {
    if (!hadolintEmptyConfigPath) {
        try {
            const d = mkdtempSync(join(tmpdir(), 'LailatulCoder-review-hadolint-'));
            // Register cleanup RIGHT AFTER mkdtemp, before the write: `cleanup.ts` only
            // sweeps `.lailatulcoder/tmp`, so we remove `d` ourselves at exit — and registering it
 ...
 */
export function buildToolInvocation(tool, absPath) {
...
function parseFindings(tool, raw) {
    ...
    const mk = (line, code, level, message) => {
        const l = Number(line);
    ...
    if (tool === 'shellcheck') {
        ...
        return (Array.isArray(comments) ? comments : [])
            .map((c) => {
            const o = c;
            return mk(o.line, `SC${o.code}`, String(o.level ?? 'warning'), o.message);
        })
            .filter((x) => x !== null);
    }
    if (tool === 'hadolint') {
        return (Array.isArray(json) ? json : [])
            .map((c) => {
            const o = c;
            return mk(o.line, String(o.code ?? 'DL'), String(o.level ?? 'warning'), o.message);
        })
            .filter((x) => x !== null);
    }
...
export function diffHashOf(diffPath) {
...
export function runScriptLint(args, runner = runTool) {
    let plan;
    try {
        plan = JSON.parse(readFileSync(args.plan, 'utf8'));
    }
    catch (err) {
        throw new Error(`script-lint: cannot read the plan ${args.plan}: ${err.message}`);
    }
    ...
    if (diffBuf) {
        try {
            ...
            addedRanges = new Map();
            diffHash = undefined;
            writeStderrLine('WARNING: script-lint could not parse the captured diff; the report is ' +
    ...
    for (const f of files) {
        ...
        const escapes = (p, root) => p !== root && !p.startsWith(root + sep);
        ...
        if (first.kind === 'missing')
            continue; // deleted on the new side — nothing to lint
        ...
        const ranges = addedRanges.get(path) ?? (addedRanges.size > 0 ? [] : hunksOf(f));
...
function buildNote(checked, skipped, errored, deferred, blocking) {
    ...
    const parts = [];
    parts.push(`Linted ${checked.length} file(s); ${blocking} finding(s) on changed lines.`);
    if (errored.length > 0) {
        const tools = [...new Set(errored.map((e) => e.tool))].join(', ');
        parts.push(`${errored.length} file(s) failed to lint — ${tools} errored (fail closed: not clean).`);
    }
    if (skipped.length > 0) {
        ...
        parts.push(`${skipped.length} file(s) not checked (${tools}) — report as unreviewed, not clean.`);
    }
    if (deferred.length > 0) {
        const tools = [...new Set(deferred.map((d) => d.tool))].join(', ');
        parts.push(`${deferred.length} file(s) deferred (${tools} — a tool limitation, disclosed but not blocking).`);
    }
    return parts.join(' ');
}
export const scriptLintCommand = {
    command: 'script-lint',
    describe: 'Run shellcheck/actionlint/hadolint over the executable scripts a diff ' +
...
//# sourceMappingURL=script-lint.js.map