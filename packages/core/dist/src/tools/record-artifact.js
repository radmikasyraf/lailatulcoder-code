...
import { MAX_DIRECTORY_ARTIFACT_DEPTH, collectRecordableWorkspaceFiles, pathHasSkippedDirectoryComponent, } from '../utils/workspace-artifact-directory.js';
import { resolveBoundWorkspaceRoot, toCanonicalWorkspaceArtifactPath, } from '../utils/workspace-artifact-path.js';
...
const WORKSPACE_PATH_HINT = '"workspacePath" must be relative to the current execution directory (for example "report.csv") or an absolute path inside the workspace. Do not add workspace folder prefixes such as "w/agent/".';
class RecordArtifactInvocation extends BaseToolInvocation {
    ...
    async execute(_signal) {
        ...
        if (workspacePathInput) {
            const locator = await resolveWorkspaceArtifactLocator(workspacePathInput, this.config);
            if (!locator.ok) {
                return {
    ...
    async expandDirectoryLocator(locator) {
        ...
        let collected;
        try {
            collected = await collectRecordableWorkspaceFiles(locator.resolvedPath, locator.workspacePath, locator.workspaceRoot, (workspacePath) => isRecordableDerivedChild(path.posix.basename(workspacePath), workspacePath));
        }
        catch (error) {
            const failure = pathInspectFailure(error, locator.resolvedPath, locator.workspacePath, `Failed to record artifact: could not inspect "${locator.workspacePath}" (${error instanceof Error ? error.message : String(error)}).`);
            return {
        ...
        if (metadataExceedsBudget(this.params.metadata)) {
            const message = 'Failed to record artifact: metadata is too large to expand a directory.';
            return {
                llmContent: message,
                returnDisplay: message,
                error: {
...
export class RecordArtifactTool extends BaseDeclarativeTool {
    ...
    constructor(config) {
        super(RecordArtifactTool.Name, ToolDisplayNames.RECORD_ARTIFACT, DESCRIPTION, Kind.Other, {
            type: 'object',
            ...
            required: ['title'],
        }, true, false, true, false, 'artifact url link file image report notebook dashboard');
        this.config = config;
    }
    validateToolParams(params) {
        if (hasLegacyPathField(params)) {
            return ('"path" is not supported; use "workspacePath" for a file in the current execution directory ' +
    ...
    validateToolParamValues(params) {
        ...
        const titleError = validateString(params.title, 'title', ARTIFACT_TITLE_MAX_LENGTH, true);
        if (titleError) {
            return titleError;
        }
        const descriptionError = validateString(params.description, 'description', 1000, false);
        if (descriptionError) {
            return descriptionError;
        }
        const mimeTypeError = validateString(params.mimeType, 'mimeType', 120, false);
        if (mimeTypeError) {
        ...
        if (params.workspacePath) {
            const workspacePathError = validateWorkspacePath(params.workspacePath, this.config.getTargetDir());
...
export function hasControlCharacter(value, allowLineWhitespace = false) {
...
export function hasUnsafeDisplayPayload(value) {
...
function validateWorkspacePath(value, cwd) {
    ...
    if (isAbsoluteWorkspaceInput(trimmed)) {
        ...
        const root = resolveBoundWorkspaceRoot(tryResolveForContainment(path.resolve(cwd)) ?? path.resolve(cwd));
...
function hasLegacyPathField(params) {
    const raw = params;
    return (Object.prototype.hasOwnProperty.call(raw, 'path') &&
        raw.path != null &&
        raw.path !== '');
}
function isRedirectorRoutedPath(value) {
    ...
    const posixDoubleSlash = process.platform !== 'win32' &&
        !value.includes('\\') &&
        /^\/\//.test(value);
    if (!posixDoubleSlash &&
        (/^\\\\[^\\?]+(?:\\|$)/.test(slashes) ||
...
function formatDirectoryExpansion(locator, collected) {
    ...
    if (collected.depthLimited) {
        lines.push(`Skipped files deeper than ${MAX_DIRECTORY_ARTIFACT_DEPTH} directory levels.`);
    }
    if (collected.unreadable) {
        lines.push('Skipped subdirectories that could not be read.');
    }
    if ((collected.skippedUnrecordable ?? 0) > 0) {
        lines.push(`Skipped ${collected.skippedUnrecordable} files whose names cannot be recorded as artifact titles.`);
    }
    return lines.join('\n');
}
...
export function isRecordableDerivedChild(title, workspacePath) {
...
async function resolveWorkspaceArtifactLocator(rawPath, config) {
    ...
    const inspected = await inspectWorkspaceCandidate(first.path, rawPath, cwd, root);
    if (inspected.ok ||
...
function workspacePathCandidate(locator, cwd, root, preservePosixBackslash) {
    if (isRedirectorRoutedPath(locator) || isForeignWindowsAbsolute(locator)) {
        return locatorFailure(ToolErrorType.INVALID_TOOL_PARAMS, WORKSPACE_PATH_HINT);
    }
    if (isAbsoluteWorkspaceInput(locator)) {
        const absolute = path.resolve(locator);
        const comparable = tryResolveForContainment(absolute);
        if (comparable && !isWithinRoot(comparable, root)) {
            return locatorFailure(ToolErrorType.PATH_NOT_IN_WORKSPACE, `Failed to record artifact: "${locator}" is outside the workspace.\n${WORKSPACE_PATH_HINT}`);
        }
        return { ok: true, path: absolute };
    }
    const relative = preservePosixBackslash && process.platform !== 'win32'
        ? locator
        : locator.replace(/\\/g, '/');
...
async function inspectWorkspaceCandidate(candidate, rawPath, cwd, root) {
    if (pathHasRedirectorHop(candidate)) {
        return locatorFailure(ToolErrorType.PATH_NOT_IN_WORKSPACE, `Failed to record artifact: "${rawPath}" resolves outside the workspace.\n${WORKSPACE_PATH_HINT}`);
    }
    try {
        await fs.lstat(candidate);
    }
    catch (error) {
        return pathInspectFailure(error, candidate, rawPath, `Failed to record artifact: could not inspect "${candidate}" (${error instanceof Error ? error.message : String(error)}).`);
    }
    let resolved;
    try {
        ...
        return pathInspectFailure(error, candidate, rawPath, `Failed to record artifact: could not resolve "${rawPath}" (${error instanceof Error ? error.message : String(error)}).`);
    }
    ...
    try {
        ...
        return pathInspectFailure(error, resolved, rawPath, `Failed to record artifact: ${error instanceof Error ? error.message : String(error)}`);
    }
    if (!st.isFile() && !st.isDirectory()) {
        return locatorFailure(ToolErrorType.TARGET_NOT_REGULAR_FILE, `Failed to record artifact: "${resolved}" is not a regular file.\n${WORKSPACE_PATH_HINT}`);
    }
    if (st.isDirectory() && path.resolve(resolved) === path.resolve(cwd)) {
        return locatorFailure(ToolErrorType.TARGET_IS_DIRECTORY, `Failed to record artifact: "${rawPath}" is the workspace root, which cannot be recorded as a directory artifact.`);
    }
    ...
    if (!workspacePath) {
        ...
        return locatorFailure(ToolErrorType.PATH_NOT_IN_WORKSPACE, `Failed to record artifact: "${rawPath}" could not be converted to a workspace-root-relative path.\n${WORKSPACE_PATH_HINT}`);
    }
    const canonicalError = validateString(workspacePath, 'workspacePath', ARTIFACT_WORKSPACE_PATH_MAX_LENGTH, true);
    if (canonicalError) {
        return locatorFailure(ToolErrorType.INVALID_TOOL_PARAMS, canonicalError.includes('exceeds')
...
function pathInspectFailure(error, candidate, rawPath, fallbackMessage) {
    ...
    if (type === ToolErrorType.FILE_NOT_FOUND) {
        return locatorFailure(type, [
            `Failed to record artifact: file not found at "${candidate}".`,
            WORKSPACE_PATH_HINT,
        ].join('\n'));
    }
    if (type === ToolErrorType.PERMISSION_DENIED) {
        return locatorFailure(type, `Failed to record artifact: permission denied for "${rawPath}".\n${WORKSPACE_PATH_HINT}`);
    }
    return locatorFailure(type, fallbackMessage);
}
//# sourceMappingURL=record-artifact.js.map