...
const findNextFence = (content, from) => {
    ...
    let length = 0;
    while (content[index + length] === char)
        length++;
 ...
 */
const getEnclosingFence = (content, indexToTest) => {
    let open = null;
    let searchPos = 0;
    while (searchPos < content.length) {
        const fence = findNextFence(content, searchPos);
        if (!fence || fence.index >= indexToTest)
            break;
        if (!open) {
 ...
 */
const isIndexInsideCodeBlock = (content, index) => getEnclosingFence(content, index) !== null;
/**
 ...
 */
const findEnclosingCodeBlockStart = (content, index) => getEnclosingFence(content, index)?.startIndex ?? -1;
/**
 ...
 */
export const getEnclosingFenceInfo = (content, index) => {
    const fence = getEnclosingFence(content, index);
    return fence ? parseCodeFenceInfo(fence.infoString) : null;
};
export const findLastSafeSplitPoint = (content, idealMaxLength) => {
    ...
    while (searchStartIndex >= 0) {
        ...
        if (potentialSplitPoint <= searchEnd &&
...
const countHeadContentLines = (content, fenceStartIndex, splitPoint) => {
 ...
 */
export const splitFencedMarkdown = (content, splitPoint) => {
