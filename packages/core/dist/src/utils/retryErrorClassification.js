 ...
 */
export function classifyRetryError(error, context = {}) {
    ...
    if (context.authType === AuthType.lailatulcoder_OAUTH &&
    ...
    if (isRateLimitError(error, context.extraRetryErrorCodes)) {
        const kind = details.transport === 'sse'
            ? 'sse-provider'
...
export function getTransportCode(error) {
    let current = error;
...
function getProviderFields(error) {
    ...
    const source = error;
    const rawCode = typeof source.code === 'string' || typeof source.code === 'number'
        ? String(source.code)
        : undefined;
    ...
    const providerCode = (error instanceof Error && rawCode?.startsWith('ERR_')) || isHttpStatusEcho
        ? undefined
        : rawCode;
 ...
 */
export function isFallbackEligible(classification) {
...
//# sourceMappingURL=retryErrorClassification.js.map