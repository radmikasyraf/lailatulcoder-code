...
import { brandSecret, unsafeRevealSecret, DEVICE_FLOW_DEFAULT_INTERVAL_MS, DEVICE_FLOW_MAX_CONCURRENT, DEVICE_FLOW_MAX_EXPIRES_IN_SEC, DEVICE_FLOW_MAX_INTERVAL_MS, DEVICE_FLOW_PERSIST_TIMEOUT_MS, DEVICE_FLOW_POLL_TIMEOUT_MS, DEVICE_FLOW_SLOW_DOWN_BUMP_MS, DEVICE_FLOW_START_TIMEOUT_MS, DeviceFlowPollTimeoutError, DEVICE_FLOW_TERMINAL_GRACE_MS, DeviceFlowRegistry, TooManyActiveDeviceFlowsError, UnsupportedDeviceFlowProviderError, } from './device-flow.js';
...
function buildRegistry(provider) {
    ...
    const registry = new DeviceFlowRegistry({
        ...
        scheduleInterval: env.scheduleInterval,
        clearScheduled: env.clearScheduled,
        clearScheduledInterval: env.clearScheduledInterval,
    });
...
describe('DeviceFlowRegistry — start / public view', () => {
    ...
    it('emits started + returns redacted public view', async () => {
        ...
        expect(JSON.stringify(events[0].emission.data)).not.toContain('idp.example/verify');
    });
    ...
    it('take-over by a different clientId emits a take-over audit (fold-in 6 #6)', async () => {
        ...
        const takeoverAudit = auditLines.find((line) => line['status'] === 'started' &&
            line['clientId'] === 'sdk-client-B' &&
            typeof line['hint'] === 'string' &&
            line['hint'].startsWith('take-over'));
    ...
    it('take-over by the SAME clientId does not emit a take-over audit', async () => {
        ...
        expect(auditLines.some((line) => typeof line['hint'] === 'string' &&
            line['hint'].startsWith('take-over'))).toBe(false);
    });
    it('concurrent start() for the same providerId coalesces — provider.start fires once', async () => {
        ...
        const attachedCount = [first, second, third].filter((r) => r.attached).length;
        expect(attachedCount).toBe(2);
    });
    it('rejects unsupported provider', async () => {
        await expect(registry.start({ providerId: 'unknown-idp' })).rejects.toBeInstanceOf(UnsupportedDeviceFlowProviderError);
    });
    it('caps at DEVICE_FLOW_MAX_CONCURRENT', async () => {
        const providers = new Map();
        for (let i = 0; i < DEVICE_FLOW_MAX_CONCURRENT + 1; i += 1) {
            providers.set(`provider-${i}`, new FakeProvider());
        }
        const env = makeClockAndScheduler();
        const events = makeEventSink();
        ...
        try {
            ...
            await expect(reg.start({
                providerId: `provider-${DEVICE_FLOW_MAX_CONCURRENT}`,
    ...
    it('caps at DEVICE_FLOW_MAX_CONCURRENT under CONCURRENT distinct-provider starts (round-13 #1)', async () => {
        ...
        try {
            const results = await Promise.allSettled(Array.from({ length: DEVICE_FLOW_MAX_CONCURRENT + 1 }, (_, i) => reg.start({
                providerId: `provider-${i}`,
            })));
            ...
            expect(rejected[0].status === 'rejected' ? rejected[0].reason : null).toBeInstanceOf(TooManyActiveDeviceFlowsError);
        }
        finally {
            reg.dispose();
        }
...
describe('DeviceFlowRegistry — polling state machine', () => {
    ...
    it('honors slow_down by bumping intervalMs and emits throttled', async () => {
        ...
        expect(throttled.emission.data.intervalMs).toBe(DEVICE_FLOW_DEFAULT_INTERVAL_MS + DEVICE_FLOW_SLOW_DOWN_BUMP_MS);
        const refreshed = registry.get(started.deviceFlowId);
        expect(refreshed?.intervalMs).toBe(DEVICE_FLOW_DEFAULT_INTERVAL_MS + DEVICE_FLOW_SLOW_DOWN_BUMP_MS);
    ...
    it('emits failed with errorKind on upstream RFC 8628 error', async () => {
        ...
        expect(failed.emission.data.errorKind).toBe('access_denied');
        const refreshed = registry.get(started.deviceFlowId);
...
describe('DeviceFlowRegistry — authoritative timeouts (fold-in 7)', () => {
    it('start() rejects when a non-abortable provider.start() hangs past START_TIMEOUT_MS (#1)', async () => {
        ...
        try {
            ...
            await expect(registry.start({ providerId: 'qwen-oauth' })).resolves.toMatchObject({ attached: false });
        }
        finally {
            registry.dispose();
        }
    ...
    it('poll() that hangs past POLL_TIMEOUT_MS surfaces as upstream_error and aborts the entry signal (follow-up review)', async () => {
        ...
        try {
            ...
            if (failed && failed.emission.type === 'failed') {
                ...
                expect(failed.emission.data.hint).not.toContain('see daemon audit log for details');
            }
            ...
            const auditFailure = auditLines.find((line) => line['status'] === 'failed' && line['errorKind'] === 'upstream_error');
            expect(auditFailure).toBeDefined();
            const auditHint = auditFailure?.['hint'];
            ...
            // reason.
            expect(provider.lastPollSignal?.aborted).toBe(true);
            const reason = provider.lastPollSignal?.reason;
    ...
    it('records lost_late_poll_after_timeout when provider.poll() resolves AFTER the registry race timeout (follow-up review #5)', async () => {
        ...
        const provider = new FakeProvider();
        let resolveLate;
        const latePollPromise = new Promise((resolve) => {
            resolveLate = resolve;
        });
        ...
        try {
            ...
            expect(auditLines.some((line) => line['hint']?.includes('lost_late_poll_after_timeout'))).toBe(false);
            // Now the IdP belatedly responds.
            resolveLate({ kind: 'pending' });
            await flushAsync();
            // The passive observer must have recorded an audit line.
            const lateAudit = auditLines.find((line) => line['hint']?.includes('lost_late_poll_after_timeout'));
            expect(lateAudit).toBeDefined();
            expect(lateAudit?.['errorKind']).toBe('upstream_error');
            expect(lateAudit?.['hint']).toContain('kind=pending');
            expect(lateAudit?.['hint']).toContain(`${DEVICE_FLOW_POLL_TIMEOUT_MS}ms ceiling`);
    ...
    it('records lost_late_poll_after_timeout when provider.poll() REJECTS after the race timeout (qwen-latest review N2)', async () => {
        ...
        const provider = new FakeProvider();
        let rejectLate;
        const latePollPromise = new Promise((_resolve, reject) => {
            rejectLate = reject;
        });
        provider.poll = async () => {
            provider.pollCount += 1;
        ...
        try {
            ...
            expect(lateAudit).toBeDefined();
            expect(lateAudit?.['errorKind']).toBe('upstream_error');
            const auditHint = lateAudit?.['hint'];
    ...
    it('audits a provider-thrown DeviceFlowPollTimeoutError as a real failure (round-6 #4: brand-aware self-filter)', async () => {
        ...
        try {
            ...
            const providerThrown = new DeviceFlowPollTimeoutError(DEVICE_FLOW_POLL_TIMEOUT_MS);
            expect(providerThrown._isRegistryTimeout).toBe(false);
            rejectLate(providerThrown);
            await flushAsync();
            const lateAudits = auditLines.filter((line) => line['hint']?.includes('lost_late_poll_after_timeout'));
    ...
    it('does NOT attach late-poll observer when the provider beats the timeout (round-5 #1: pollTimedOut race)', async () => {
        ...
        try {
            ...
            env.scheduler.flushDue(env.clock.now);
            await flushAsync();
            const spuriousLate = auditLines.find((line) => line['hint']?.includes('lost_late_poll_after_timeout'));
    ...
    it('sanitizes hostile latePollResult.kind in late-observer audit (round-5 #3)', async () => {
        ...
        try {
            ...
            resolveLate({ kind: hostile });
            await flushAsync();
            const lateAudit = auditLines.find((line) => line['hint']?.includes('lost_late_poll_after_timeout'));
            expect(lateAudit).toBeDefined();
            const hint = lateAudit?.['hint'];
    ...
    it('sanitizes rawProviderError before interpolating into the audit hint (round-6 #3)', async () => {
        ...
        provider.poll = async () => {
            provider.pollCount += 1;
            throw new Error(`upstream${U_2028}[serve] FORGED PROVIDER LINE\x1b[31mRED`);
        };
        const built = buildRegistry(provider);
        const { registry, env, auditLines } = built;
        try {
            ...
            env.scheduler.flushDue(env.clock.now);
            await flushAsync();
            const auditLine = auditLines.find((line) => line['hint']?.includes('provider.poll() threw (raw)'));
            expect(auditLine).toBeDefined();
            const hint = auditLine?.['hint'];
    ...
    it('runPollTick catch uses a static SSE hint and preserves raw on the audit (fold-in 9 #1)', async () => {
        ...
        try {
            ...
            env.scheduler.flushDue(env.clock.now);
            await flushAsync();
            const failedEvent = events.find((e) => e.emission.type === 'failed' &&
                e.emission.data.deviceFlowId === view.deviceFlowId);
            expect(failedEvent).toBeDefined();
            const sseHint = failedEvent && failedEvent.emission.type === 'failed'
            ...
            expect(sseHint).toBe('provider.poll() failed; see daemon audit log for details');
            // Audit line still retains the FULL raw detail (including the
            // secret marker) for operator incident response.
            const failedAudit = auditLines.find((line) => line['status'] === 'failed' &&
...
describe('DeviceFlowRegistry — persist failure paths (fold-in 10 #1)', () => {
    ...
    it('persist throws → entry transitions to error/persist_failed + failed event emitted', async () => {
        ...
        try {
            ...
            if (failedEvent && failedEvent.emission.type === 'failed') {
                ...
                expect(failedEvent.emission.data.hint).toContain('credentials could not be written');
            }
            const failedAudit = auditLines.find((line) => line['status'] === 'failed' && line['errorKind'] === 'persist_failed');
            expect(failedAudit).toBeDefined();
        }
        finally {
            registry.dispose();
    ...
    it('persist throws after cancel() → entry transitions to cancelled (not authorized; not persist_failed)', async () => {
        ...
        let rejectPersist;
        const persistPromise = new Promise((_resolve, reject) => {
        ...
        try {
            ...
            const cancelledEvent = events.find((e) => e.emission.type === 'cancelled' &&
                e.emission.data.deviceFlowId === view.deviceFlowId);
            expect(cancelledEvent).toBeDefined();
            expect(cancelledEvent?.clientId).toBe('sdk-canceller');
            // No `failed`/`persist_failed` event leaked through.
            expect(events.some((e) => e.emission.type === 'failed' &&
                e.emission.data.errorKind === 'persist_failed')).toBe(false);
        }
        finally {
            registry.dispose();
        }
    ...
    it('records lost_success_after_timeout when persist resolves AFTER the registry timeout (round-12 #8)', async () => {
        ...
        const provider = new FakeProvider();
        let resolveLate;
        const latePersist = new Promise((resolve) => {
            resolveLate = resolve;
        });
        provider.pollScript = [
            {
        ...
        try {
            ...
            await flushAsync();
            const lostSuccessAudit = auditLines.find((line) => typeof line['hint'] === 'string' &&
                line['hint'].startsWith('lost_success_after_timeout'));
    ...
    it('persist throws past expiresAt → persist_failed (NOT expired_token; fold-in 9 #13)', async () => {
        ...
        try {
            ...
            const failedAudit = auditLines.find((line) => line['status'] === 'failed' &&
                ...
                typeof line['hint'] === 'string' &&
                line['hint'].startsWith('persist_also_failed_past_expiry'));
            expect(failedAudit).toBeDefined();
            // Critical: no `expired_token` classification anywhere.
            expect(events.some((e) => e.emission.type === 'failed' &&
                e.emission.data.errorKind === 'expired_token')).toBe(false);
        }
        finally {
            registry.dispose();
        }
...
describe('DeviceFlowRegistry — cancel', () => {
    it('cancels a pending flow, emits cancelled, idempotent on terminal', async () => {
        ...
        try {
            ...
            expect(events.filter((e) => e.emission.type === 'cancelled')).toHaveLength(1);
        }
        finally {
            registry.dispose();
        }
