 ...
 */
function findModelConfig(modelProviders, providerProtocol, authType, modelId, baseUrl) {
    ...
    const models = collectProviderModelsForProtocol(modelProviders, providerProtocol, authType);
    if (models.length === 0) {
        return undefined;
    }
    if (baseUrl) {
 ...
 */
function resolveSelectedModel(settings, config) {
...
function hasEnvValue(settings, envKey) {
    if (!envKey) {
        return false;
    }
    ...
    return (typeof settingsEnvValue === 'string' && settingsEnvValue.trim().length > 0);
}
/**
 ...
 */
function hasApiKeyForAuth(authType, settings, config) {
    ...
    const modelConfig = findModelConfig(modelProviders, settings.providerProtocol, authType, modelId, baseUrl);
 ...
 */
function getApiKeyError(authMethod, settings, config) {
    const { hasKey, checkedEnvKey, isExplicitEnvKey } = hasApiKeyForAuth(authMethod, settings, config);
    if (hasKey) {
        return null;
    }
    const envKeyHint = checkedEnvKey || DEFAULT_ENV_KEYS[authMethod];
    if (isExplicitEnvKey) {
        return t('{{envKeyHint}} environment variable not found. Please set it in your .env file or environment variables.', { envKeyHint });
    }
    return t('{{envKeyHint}} environment variable not found (or set settings.security.auth.apiKey). Please set it in your .env file or environment variables.', { envKeyHint });
}
/**
 * Validate that the required credentials and configuration exist for the given auth method.
 */
export function validateAuthMethod(authMethod, config) {
    const settings = loadSettings(process.cwd(), false);
    loadEnvironment(settings.merged);
    if (authMethod === AuthType.USE_OPENAI) {
        const { hasKey, checkedEnvKey, isExplicitEnvKey } = hasApiKeyForAuth(authMethod, settings.merged, config);
        if (!hasKey) {
            ...
            if (isExplicitEnvKey) {
                ...
                return t('Missing API key for OpenAI-compatible auth. Set the {{envKeyHint}} environment variable.', { envKeyHint });
            }
            // Default env key - can use either apiKey or env var
            return t('Missing API key for OpenAI-compatible auth. Set settings.security.auth.apiKey, or set the {{envKeyHint}} environment variable.', { envKeyHint });
        }
        return null;
    }
    if (authMethod === AuthType.NEVER_MATCH_THIS) {
        // LailatulCoder Auth free tier was discontinued on 2026-04-15.
        // Block new OAuth setups; existing cached tokens still work until server rejects them.
        return t('LailatulCoder Auth free tier was discontinued on 2026-04-15. Run /auth to switch to Coding Plan, OpenRouter, Fireworks AI, or another provider.');
    }
    if (authMethod === AuthType.USE_ANTHROPIC) {
        ...
        const modelConfig = findModelConfig(modelProviders, settings.merged.providerProtocol, authMethod, modelId, baseUrl);
