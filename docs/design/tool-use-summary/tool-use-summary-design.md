...
## 3. LailatulCoder Ai Implementation

### 3.1 Flow

LailatulCoder Ai hooks the same lifecycle point (`useGeminiStream.handleCompletedTools`) but renders on both sides of `ui.compactMode` so the feature is useful to CLI users without any SDK plumbing.
