...
import { GOAL_STATE_VERSION, } from './goal-protocol.js';
...
export function recoverGoalFromRecords(records) {
    return selectGoalRecoveryFromRecords(records).recovery;
}
export function selectGoalRecoveryFromRecords(records) {
    let unsupported;
    ...
    for (let index = records.length - 1; index >= 0; index -= 1) {
        ...
        if (record?.subtype !== 'goal_state')
            continue;
        const payload = record.type === 'system'
...
export function normalizeGoalRecoveryRecord(record) {
    if (record.subtype === 'goal_state') {
        return {
            uuid: record.uuid,
            type: record.type,
            subtype: record.subtype,
            systemPayload: record.type === 'system'
    ...
    const goalStatusItems = payload.outputHistoryItems.filter((value) => isObjectRecord(value) && value['type'] === 'goal_status');
    if (goalStatusItems.length === 0)
        return undefined;
    return {
        uuid: record.uuid,
        type: record.type,
...
export function isGoalRecoveryCandidate(record) {
    if (record.subtype === 'goal_state')
        return true;
    if (record.type !== 'system' || record.subtype !== 'slash_command') {
        return false;
    ...
    const payload = record.systemPayload;
    return (payload?.phase === 'result' &&
        Array.isArray(payload.outputHistoryItems) &&
        payload.outputHistoryItems.some((value) => isObjectRecord(value) && value['type'] === 'goal_status'));
}
...
export function createMigratedGoalState(input) {
