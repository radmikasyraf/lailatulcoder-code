...
function isDirectoryTreeIgnored(dirPath, dirFilter, memo) {
    const cached = memo.get(dirPath);
    if (cached !== undefined) {
        return cached;
    }
    const separator = dirPath.lastIndexOf('/');
    const ignored = (separator > 0 &&
...
function collectDirectoryRows(options) {
    const relativeToCrawlDir = getPosixRelative(options.cwd, options.crawlDirectory);
    const dirFilter = options.ignore.getDirectoryFilter();
    const rows = [];
    const visit = (dir, relativePath, depth) => {
        const cwdRelative = relativePath === ''
            ? relativeToCrawlDir
...
function buildResultsFromFileSet(files, extraDirectories = []) {
...
async function crawlWithRipgrep(stateKey, crawlDirectory, cwd, options) {
    ...
    const results = buildResultsFromFileSet(fileSet, collectDirectoryRows(options));
    const filteredResults = applyFilters(results, options, relativeToCrawlDir, fileSet);
    const limitedResults = applyMaxFilesLimit(filteredResults, options.maxFiles);
