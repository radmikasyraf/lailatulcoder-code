 ...
 * none of it.
 */
export function collectNewSideLines(diffText, file) {
 ...
 */
function substringResolution(hay, needleLines, claimedLine, midLineOnly = false) {
    ...
    if (collapsed.length === 0)
        return null;
    const sitsInside = (l) => l.text.includes(needle) || normalizeCollapsed(l.text).includes(collapsed);
    ...
    const lineGuess = (l) => midLineOnly && normalizeCollapsed(l.text) === collapsed;
    ...
    const indentationRefusal = hay.some(lineGuess)
        ? {
            status: 'unmatched',
            reason: 'the marker-stripped reading matches a hunk line only after its ' +
    ...
    if (collapsed.length < MIN_SUBSTRING_LENGTH) {
        ...
        if (hay.some((l) => !lineGuess(l) && sitsInside(l))) {
            return {
                status: 'unmatched',
                reason: 'the snippet sits inside a hunk line but is shorter than ' +
    ...
    for (const [exact, norm] of norms) {
        ...
        if (!exact && cands.length > 1) {
            return {
                status: 'unmatched',
                reason: 'the snippet sits inside more than one hunk line only after its ' +
        ...
        if (!best) {
            return {
                status: 'unmatched',
                reason: 'the snippet sits inside more than one hunk line and nothing ' +
 ...
 */
export function resolveAnchor(newSideLines, anchor, claimedLine) {
    ...
    for (const [vi, needle] of variants.entries()) {
        ...
        for (const [exact, norm] of norms) {
            ...
            if (!exact && cands.length > 1) {
                return {
                    status: 'unmatched',
                    reason: 'the snippet matched in more than one place only after its ' +
            ...
            if (!best) {
                ...
                return {
                    status: 'unmatched',
                    reason: 'the snippet appears in more than one place and nothing ' +
    ...
    if (variants.length > 1) {
        const stripped = substringResolution(newSideLines, variants[1], claimedLine, true);
        if (stripped)
            return stripped;
    }
    return {
        status: 'unmatched',
        reason: 'snippet does not appear in any hunk of this file — neither as whole ' +
 ...
 */
export function resolveAnchors(diffText, requests) {
