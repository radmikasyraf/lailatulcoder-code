...
let cwd;
let errs;
function run(out, over = {}) {
    captureLocalCommand.handler({
        out,
        target: 'local',
        untracked: true,
        ...over,
...
describe('capture-local (command boundary)', () => {
    ...
    it('names every skipped file, with its reason, on stderr', () => {
        ...
        const plan = JSON.parse(readFileSync(join(dir, 'plan.json'), 'utf8'));
        expect(plan.skippedFiles.map((s) => s.path)).toEqual([
            'huge.csv',
            'nested/',
        ]);
    });
...
describe('capture-local — the budget context the handler actually passes', () => {
    ...
    it('carries the operator ceiling and the clock into the written plan', () => {
        ...
        try {
            const huge = Array.from({ length: 9000 }, (_, i) => `+const x${i} = ${i};`).join('\n');
            capture({
                diff: Buffer.from([
            ...
            run(capped);
            expect(JSON.parse(readFileSync(capped, 'utf8')).budget.reverseAuditRounds).toBe(3);
        }
        finally {
            settingsMock.mockReturnValue({ merged: {} });
            if (before === undefined)
...
//# sourceMappingURL=capture-local.test.js.map