...
import { activateManagedNpmUpdate, cleanupManagedNpmUpdate, installManagedNpmUpdate, prepareManagedNpmUpdate, } from './managed-npm-update.js';
...
function writeInstallation(prefix, version) {
    const packageRoot = path.join(prefix, 'node_modules', '@lailatul-coder', 'lailatul-coder');
    fs.mkdirSync(packageRoot, { recursive: true });
    fs.writeFileSync(path.join(packageRoot, 'package.json'), JSON.stringify({ name: '@lailatul-coder/lailatul-coder', version }));
    fs.writeFileSync(path.join(packageRoot, 'cli.js'), '');
    fs.writeFileSync(path.join(packageRoot, 'cli-entry.js'), `if (process.argv.includes('--version')) process.stdout.write('${version}\\n'); else await import('./cli.js');\n`);
}
function writeBaseInstallation(root, version = '1.0.0') {
    const packageRoot = path.join(root, 'global', 'node_modules', '@lailatul-coder', 'lailatul-coder');
...
describe('managed npm update', () => {
    it('stages an exact version for one launcher', () => {
        ...
        const update = prepareManagedNpmUpdate('2.0.0', writeBaseInstallation(root), path.join(root, 'updates'));
        ...
        expect(update.versionDir).toBe(path.join(update.launcherRoot, 'versions', '2.0.0'));
    });
    it('activates a verified install without changing running files', async () => {
        ...
        const update = prepareManagedNpmUpdate('2.0.0', bootstrap, path.join(root, 'updates'));
        ...
        expect(JSON.parse(fs.readFileSync(path.join(update.launcherRoot, 'active.json'), 'utf8'))).toMatchObject({
            version: '2.0.0',
    ...
    it('still activates when the lock is compromised during activation', async () => {
        ...
        try {
            await expect(activateManagedNpmUpdate(update, '2.0.0', bootstrap)).resolves.toBeUndefined();
            expect(getOnCompromised()).toBeTypeOf('function');
            expect(JSON.parse(fs.readFileSync(path.join(update.launcherRoot, 'active.json'), 'utf8'))).toMatchObject({ version: '2.0.0' });
        }
        finally {
            lockSpy.mockRestore();
        }
    ...
    it('installs and activates inside the managed worker', async () => {
        ...
        const spawnFn = vi.fn((_command, args, _options) => {
        ...
        await installManagedNpmUpdate('2.0.0', bootstrap, updateRoot, spawnFn);
        ...
        expect(JSON.parse(fs.readFileSync(path.join(updateRoot, createHash('sha256')
            .update(fs.realpathSync(bootstrap))
            .digest('hex')
            .slice(0, 16), 'active.json'), 'utf8'))).toMatchObject({ version: '2.0.0' });
    });
    it.each([
        ...
        if (brokenBundle) {
            fs.writeFileSync(path.join(update.stagingDir, 'node_modules', '@lailatul-coder', 'lailatul-coder', 'cli.js'), 'this is not valid JavaScript !!!\n');
        }
        await expect(activateManagedNpmUpdate(update, '2.0.0', bootstrap)).rejects.toThrow(error);
        expect(fs.existsSync(path.join(update.launcherRoot, 'active.json'))).toBe(false);
    });
    it('does not mask a global install that changes while staging', async () => {
        ...
        await expect(activateManagedNpmUpdate(update, '2.0.0', bootstrap)).rejects.toThrow('changed during update');
    });
    it('replaces a higher pointer from an older global install', async () => {
        ...
        expect(JSON.parse(fs.readFileSync(path.join(newUpdate.launcherRoot, 'active.json'), 'utf8'))).toMatchObject({ version: '2.1.0', baseVersion: '2.0.0' });
    });
    it('keeps the highest concurrently activated version', async () => {
        ...
        const active = JSON.parse(fs.readFileSync(path.join(newer.launcherRoot, 'active.json'), 'utf8'));
        expect(active.version).toBe('3.0.0');
        expect(fs.existsSync(path.join(newer.launcherRoot, 'versions', active.version))).toBe(true);
    });
    it('reuses a valid payload activated concurrently for the same version', async () => {
        ...
        expect(JSON.parse(fs.readFileSync(path.join(first.launcherRoot, 'active.json'), 'utf8'))).toMatchObject({ version: '2.0.0' });
    ...
    it('removes only orphaned managed update artifacts before staging', () => {
        ...
        const staleStagingDir = fs.mkdtempSync(path.join(versionsDir, `.2.1.0-${missingPid}-`));
        const staleActiveFile = path.join(current.launcherRoot, `active.json.${missingPid}`);
        const activeStagingDir = fs.mkdtempSync(path.join(versionsDir, `.2.2.0-${process.pid}-`));
        const activeTemporaryFile = path.join(current.launcherRoot, `active.json.${process.pid}`);
        const versionDir = path.join(versionsDir, '1.0.0');
        const unknownDir = path.join(versionsDir, 'unrelated');
        const nonSemverDir = path.join(versionsDir, `.not-semver-${missingPid}-abc123`);
        const stagingSymlink = path.join(versionsDir, `.2.3.0-${missingPid}-abcdef`);
        ...
        fs.symlinkSync(versionDir, stagingSymlink, process.platform === 'win32' ? 'junction' : 'dir');
    ...
    it('keeps artifacts when process liveness is uncertain', () => {
        ...
        const stagingDir = fs.mkdtempSync(path.join(versionsDir, `.2.1.0-${inaccessiblePid}-`));
        const temporaryActiveFile = path.join(current.launcherRoot, `active.json.${inaccessiblePid}`);
...
//# sourceMappingURL=managed-npm-update.test.js.map