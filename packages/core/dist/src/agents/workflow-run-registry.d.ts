...
export type WorkflowDispatchTraceStatus = 'queued' | 'running' | 'completed' | 'failed' | 'cancelled' | 'cached';
...
type WorkflowEventPayload = (WorkflowEventBase & {
    type: 'phase-started';
    phaseVisitId: string;
    title: string;
}) | (WorkflowEventBase & {
    ...
    type: 'dispatch-queued' | 'dispatch-started' | 'dispatch-completed' | 'dispatch-cancelled' | 'dispatch-cached';
    dispatchId: string;
}) | (WorkflowEventBase & {
 ...
 */
export type WorkflowTaskRegistration = Omit<TaskRegistration<WorkflowTask>, 'currentPhase' | 'phases' | 'phaseVisits' | 'currentPhaseVisitId' | 'dispatches' | 'agentsDispatched' | 'agentsCompleted' | 'recentLogs' | 'events' | 'tokensSpent' | 'tokenBudgetTotal' | 'perPhaseTokens' | 'script' | 'description' | 'pendingApprovals' | 'isBackgrounded'> & {
    description?: string;
    /**
...
export declare class WorkflowRunRegistry {
    ...
    bridgeApprovalEvents(runId: string, emitter: AgentEventEmitter, dispatchId?: string, expectedEntry?: WorkflowTask): () => void;
    ...
    clearPendingApproval(runId: string, subagentId: string, callId: string, at?: number): boolean;
     ...
     */
    onPhaseStarted(runId: string, rawTitle: string, at?: number): void;
    onDispatchQueued(runId: string, event: WorkflowDispatchQueued): void;
    onDispatchStarted(runId: string, dispatchId: string, at?: number): void;
    onDispatchSettled(runId: string, dispatchId: string, error?: string, at?: number, cancelRequested?: boolean): void;
    /** Record one sandbox log line without forcing a TUI redraw per line. */
    onLogAppended(runId: string, line: string, at?: number): void;
    ...
    setLineage(runId: string, sourceRunId: string, startMode: WorkflowRunStartMode): boolean;
