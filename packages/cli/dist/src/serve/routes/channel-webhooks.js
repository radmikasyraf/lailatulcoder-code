...
export function registerChannelWebhookRoutes(app, deps) {
    app.post('/channels/:channelName/webhooks/:source', ...(deps.rateLimiter ? [createWebhookRateLimitMiddleware(deps)] : []), (req, res, next) => {
        ...
        const sourceConfig = sources && Object.hasOwn(sources, source) ? sources[source] : undefined;
        if (!sourceConfig) {
        ...
        if (typeof secret !== 'string' ||
            secret.length === 0 ||
            !matchesWebhookSecret(req.get('x-LailatulCoder-webhook-secret'), secret)) {
            deps.daemonLog?.warn('channel webhook authentication failed', {
                channelName,
                source,
            });
        ...
        if (deps.rateLimiter &&
            !deps.rateLimiter.checkRate(`webhook:${channelName}:${source}`, 'mutation')) {
...
function createWebhookRateLimitMiddleware(deps) {
    return (req, res, next) => {
        if (!deps.rateLimiter) {
            next();
            return;
        }
        if (deps.rateLimiter.checkRate(`webhook:preauth:${readRequestAddress(req)}`, 'mutation')) {
            next();
            return;
        }
        sendWebhookRateLimitExceeded(res);
...
function sendWebhookRateLimitExceeded(res) {
    res.status(429).json({
...
function readRequiredBodyString(body, key, res) {
...
function readPayload(body, res) {
    ...
    if (typeof payload === 'object' &&
        ...
        return Object.fromEntries(Object.entries(payload).filter(([key]) => !PROTOTYPE_POLLUTION_KEYS.has(key)));
    }
    res.status(400).json({
...
function isWithinPayloadDepth(value, maxDepth) {
...
function classifyChannelWebhookEnqueueError(error) {
...
function statusForChannelWebhookEnqueueCode(code) {
    switch (code) {
