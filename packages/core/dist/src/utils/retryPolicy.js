...
export function getRetryDelayMs(options) {
    ...
    const cappedExponentialDelayMs = Math.min(options.initialDelayMs * Math.pow(2, exponent), delayCeilingMs);
    ...
    if (retryAfterMs !== null && retryAfterMs > 0) {
        const retryAfterCapMs = Math.min(options.retryAfterMaxDelayMs ?? options.maxDelayMs, MAX_TIMEOUT_MS);
    ...
    if (jitterRatio <= 0)
        return cappedExponentialDelayMs;
    const random = options.random ?? Math.random;
    const jitter = cappedExponentialDelayMs * jitterRatio * (random() * 2 - 1);
    return Math.min(Math.max(0, cappedExponentialDelayMs + jitter), delayCeilingMs);
}
/**
