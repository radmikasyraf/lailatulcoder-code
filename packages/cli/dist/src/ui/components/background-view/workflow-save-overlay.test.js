...
import { saveWorkflowScript } from '@lailatul-coder/lailatul-coder-core';
...
function setup(props = {}) {
    ...
    const onClose = vi.fn();
    const { lastFrame } = render(_jsx(WorkflowSaveOverlay, { script: "return 1;", config: {}, isActive: true, onClose: onClose, ...props }));
    const press = async (k) => {
        await act(async () => {
    ...
    const type = async (text) => {
        for (const ch of text)
            await press({ name: ch, sequence: ch });
    };
...
describe('WorkflowSaveOverlay', () => {
    ...
    it('saves to project scope on submit, then closes on the next key', async () => {
        ...
        expect(mockedSave).toHaveBeenCalledWith(expect.anything(), expect.objectContaining({
            name: 'flow',
            scope: 'project',
            script: 'return 1;',
            overwrite: false,
    ...
    it('Tab toggles to user scope', async () => {
        ...
        await press({ name: 'return' });
        expect(mockedSave).toHaveBeenCalledWith(expect.anything(), expect.objectContaining({ scope: 'user' }));
    });
    ...
    it('prompts to overwrite on collision and retries with overwrite:true', async () => {
        ...
        expect(mockedSave).toHaveBeenLastCalledWith(expect.anything(), expect.objectContaining({ overwrite: true }));
    });
});
//# sourceMappingURL=workflow-save-overlay.test.js.map