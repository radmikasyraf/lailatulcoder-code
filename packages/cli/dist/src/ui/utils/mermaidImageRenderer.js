...
export function detectTerminalImageProtocol(env = process.env) {
...
export function encodeITerm2InlineImage(png, widthCells, rows) {
    return `\u001b]1337;File=inline=1;width=${widthCells};height=${rows};preserveAspectRatio=1:${png.toString('base64')}\u0007`;
}
export function encodeKittyImage(png, widthCells, rows) {
    return encodeKittyImageCommand(png, `a=T,f=100,c=${widthCells},r=${rows}`);
}
export function encodeKittyVirtualImage(png, imageId, widthCells, rows) {
    return encodeKittyImageCommand(png, `a=T,f=100,i=${imageId},q=2,U=1,c=${widthCells},r=${rows}`);
}
...
export function buildKittyPlaceholder(imageId, widthCells, rows) {
    const clampedRows = Math.min(rows, KITTY_PLACEHOLDER_DIACRITICS.length);
    const clampedWidth = Math.min(widthCells, KITTY_PLACEHOLDER_DIACRITICS.length);
 ...
 */
export function renderMermaidImageSync({ source, contentWidth, availableTerminalHeight, env = process.env, }) {
    ...
    const cacheKey = createCacheKey(source, contentWidth, availableTerminalHeight, protocol ?? `chafa:${chafa}`, mmdc, env);
    ...
    const rendered = cachedPng ?? rememberPng(pngCacheKey, renderPngWithMmdc(source, mmdc, env));
    if (!rendered.ok) {
    ...
    const imageShape = fitImageToTerminal(pngSize, contentWidth, availableTerminalHeight, env);
    if (protocol) {
        const imageId = protocol === 'kitty'
            ? createKittyImageId(rendered.png, imageShape)
            : undefined;
        const sequence = protocol === 'kitty'
            ? encodeKittyVirtualImage(rendered.png, imageId, imageShape.widthCells, imageShape.rows)
            : encodeITerm2InlineImage(rendered.png, imageShape.widthCells, imageShape.rows);
        return remember(cacheKey, {
            kind: 'terminal-image',
            ...
            protocol,
            placeholder: protocol === 'kitty'
                ? buildKittyPlaceholder(imageId, imageShape.widthCells, imageShape.rows)
                : undefined,
        });
    }
    const ansi = renderPngWithChafa(rendered.png, imageShape.widthCells, imageShape.rows, chafa, env);
    if (!ansi.ok) {
        return remember(cacheKey, {
            kind: 'unavailable',
            reason: ansi.error,
...
export async function renderMermaidImageAsync({ source, contentWidth, availableTerminalHeight, env = process.env, signal, }) {
    if (isMermaidImageRenderingDisabled(env)) {
        return unavailableImageRenderingDisabled();
    }
    const imageRendering = env['QWEN_CODE_MERMAID_IMAGE_RENDERING'];
    ...
    if (!rendered) {
        const nextRendered = await renderPngWithMmdcAsync(source, mmdc, env, signal);
    ...
    const ansi = await renderPngWithChafaAsync(rendered.png, imageShape.widthCells, imageShape.rows, chafa, env, signal);
    if (!ansi.ok) {
        if (signal?.aborted) {
            return {
                kind: 'unavailable',
...
function remember(key, result) {
    ...
    while (cachedResults.size > CACHE_LIMIT ||
...
function rememberPng(key, result) {
    ...
    while (cachedPngResults.size > PNG_CACHE_LIMIT ||
    ...
    switch (result.kind) {
        case 'terminal-image':
            return (Buffer.byteLength(result.sequence, 'utf8') +
                (result.placeholder?.lines.reduce((total, line) => total + Buffer.byteLength(line, 'utf8'), 0) ?? 0));
        case 'ansi':
            return result.lines.reduce((total, line) => total + Buffer.byteLength(line, 'utf8'), 0);
        case 'unavailable':
...
function findMmdc(env) {
    ...
    if (mmdc)
        return mmdc;
    if (env['QWEN_CODE_MERMAID_ALLOW_NPX'] === '1' &&
...
export function findExecutable(command, env) {
    ...
    const localRendererDir = normalizeExecutableDir(process.cwd(), 'node_modules', '.bin');
...
function renderPngWithMmdc(source, mmdc, env) {
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'LailatulCoder-mermaid-'));
    ...
    try {
        ...
        const args = mmdc === NPX_MERMAID_CLI
...
async function renderPngWithMmdcAsync(source, mmdc, env, signal) {
    const tempDir = await fs.promises.mkdtemp(path.join(os.tmpdir(), 'LailatulCoder-mermaid-'));
    const inputPath = path.join(tempDir, 'diagram.mmd');
    const outputPath = path.join(tempDir, 'diagram.png');
    const renderWidth = getMermaidRenderWidth(env);
    try {
...
export function shouldRunThroughShell(command) {
...
export function createRendererChildEnv(env) {
...
function getMermaidCellAspectRatio(env) {
    const configuredAspectRatio = Number(env['QWEN_CODE_MERMAID_CELL_ASPECT_RATIO']);
    if (Number.isFinite(configuredAspectRatio) && configuredAspectRatio > 0) {
        return Math.max(0.2, Math.min(configuredAspectRatio, 2));
    }
    return 0.5;
}
function renderPngWithChafa(png, widthCells, rows, chafa, env) {
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'LailatulCoder-mermaid-'));
...
async function renderPngWithChafaAsync(png, widthCells, rows, chafa, env, signal) {
    ...
    try {
        ...
        const result = await runCommand(chafa, [
            '--animate=off',
            '--format=symbols',
            '--symbols=block',
            `--size=${widthCells}x${rows}`,
...
function runCommand(command, args, options) {
    return new Promise((resolve) => {
        ...
        const finish = (result) => {
            ...
            settled = true;
            clearTimeout(timer);
            if (!terminationRequested && killTimer)
...
function appendBoundedRendererOutput(current, chunk) {
    ...
    return {
        text: next.slice(0, MAX_RENDERER_OUTPUT_CHARS - OUTPUT_TRUNCATION_MARKER.length) + OUTPUT_TRUNCATION_MARKER,
        truncated: true,
    };
}
function finalizeBoundedRendererOutput(output) {
    ...
    return (output.text.slice(0, MAX_RENDERER_OUTPUT_CHARS - OUTPUT_TRUNCATION_MARKER.length) + OUTPUT_TRUNCATION_MARKER);
}
function fitImageToTerminal(size, contentWidth, availableTerminalHeight, env = process.env) {
    ...
    const naturalRows = Math.ceil((size.height / size.width) * widthCells * getMermaidCellAspectRatio(env));
...
//# sourceMappingURL=mermaidImageRenderer.js.map