 ...
 */
export function isMetadataAddress(address) {
    const v = isIP(address);
    if (v === 4) {
...
export function ssrfGuardedLookup(hostname, options, callback) {
    ...
    dns.promises
        ...
        if (!first) {
            callback(Object.assign(new Error(`ENOTFOUND ${hostname}`), {
                code: 'ENOTFOUND',
                hostname,
            }), '');
            return;
...
function ssrfError(hostname, address) {
    const err = new Error(`HTTP hook blocked: ${hostname} resolves to ${address} (private/link-local address). Loopback (127.0.0.1, ::1) is allowed for local dev.`);
