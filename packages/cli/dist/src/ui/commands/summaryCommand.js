...
import { getProjectSummaryPrompt, isSubpath, resolvePath, runSideQuery, } from '@lailatul-coder/lailatul-coder-core';
...
export const summaryCommand = {
    name: 'summary',
    get description() {
        return t('Generate a project summary and save it to .lailatulcoder/PROJECT_SUMMARY.md');
    },
    argumentHint: '[path]',
    ...
    action: async (context, args) => {
        ...
        const resolveSummaryTarget = async () => {
            ...
            const defaultSummaryPath = path.join(projectRoot, '.lailatulcoder', 'PROJECT_SUMMARY.md');
            const customPath = args?.trim();
            if (!customPath) {
                // The default target always overwrites: regenerating the summary is
                // the command's purpose, and .lailatulcoder/PROJECT_SUMMARY.md is a generated
                // artifact — not user prose the overwrite guard protects.
                return {
                    summaryPath: defaultSummaryPath,
                    filePathForDisplay: '.lailatulcoder/PROJECT_SUMMARY.md',
            ...
            // spelled default path (`.lailatulcoder/PROJECT_SUMMARY.md` or `.lailatulcoder/`) is
        ...
        const saveSummaryToDisk = async (markdownSummary, target) => {
            ...
            // The default target is the user's own .lailatulcoder/ directory — a symlinked
            // .lailatulcoder/ is a deliberate setup, not an attack vector.
