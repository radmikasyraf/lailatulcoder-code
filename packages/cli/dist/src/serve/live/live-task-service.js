...
import { partToString, stripTerminalControlSequences, } from '@lailatul-coder/lailatul-coder-core';
import { SessionArchivedError, SessionNotFoundError, } from '@lailatul-coder/acp-bridge/bridgeErrors';
...
import { isCompatibleLiveSessionSource, readLoadableLiveConversationMetadata, } from '../conversations/session-source.js';
...
export class LiveTaskService {
    ...
    async readThread(args) {
        ...
        return {
            ...
            thread: {
                ...
                updatedAt: epochSeconds(laterActivityTimestamp(located.summary.updatedAt, located.persisted?.conversation.lastUpdated)),
            },
            page: {
                order: 'newest_first',
                limit: turnLimit,
    ...
    waitSnapshot(target, task) {
        ...
        const cursor = encodeCursor({
            ...
            ...(task.summary.clientCount > 0
                ...
                : {}),
            updatedAt: laterActivityTimestamp(task.summary.updatedAt, task.persisted?.conversation.lastUpdated),
        });
        ...
        const revision = task.summary.clientCount > 0
            ? task.runtime.bridge.getSessionLastEventId(target.threadId)
            : epochSeconds(laterActivityTimestamp(task.summary.updatedAt, task.persisted?.conversation.lastUpdated) ?? task.summary.createdAt);
        return {
            schemaVersion: 1,
            cursor,
            revision,
    ...
    async ensureResident(task) {
        ...
        const metadata = task.runtime.provenance === 'live-conversation'
            ? await readLoadableLiveConversationMetadata(task.summary.sessionId, service)
    ...
    async rollbackFreshSession(runtime, session, projectless) {
        ...
        if (removed) {
            const persistedRemoved = await runWithWorkspaceRuntimeStorage(runtime, () => createWorkspaceRuntimeSessionService(runtime)
    ...
    async locateTask(threadId) {
        ...
        const runtimes = live.kind === 'found'
            ? [live.runtime]
            : (await Promise.all((this.options.workspaceRegistry.listAll?.() ??
