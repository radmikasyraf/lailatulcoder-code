...
import { addMCPStatusChangeListener, MCPServerStatus, removeMCPStatusChangeListener, updateMCPServerStatus, } from './mcp-client.js';
import { connectionIdOf, mcpTransportOf, } from './mcp-pool-key.js';
 ...
 * in the design doc.
 */
export function defaultPoolEntryOptions(transport) {
    // include
...
export class PoolEntry {
    ...
    opts;
    onClosed;
    aggregateStatusByName;
    localStatus = MCPServerStatus.CONNECTING;
    state = 'spawning';
    _generation = 0;
    refs = new Set();
    subscribers = new Map();
    subscriberHandles = new Map();
    toolsSnapshot = [];
    promptsSnapshot = [];
    resourcesSnapshot = [];
    drainTimer;
     ...
     */
    constructor(id, serverName, entryIndex, 
    // `cfg` carries
    ...
    cfg, client, cliConfig, opts, onClosed, aggregateStatusByName) {
        this.id = id;
        ...
        this.aggregateStatusByName = aggregateStatusByName;
        ...
        this.statusChangeListener = (name, status) => {
            ...
            if (status === MCPServerStatus.DISCONNECTED &&
                ...
                // a cause.
                debugLogger.error(`PoolEntry ${this.id} silent transport drop ` +
                ...
                void this.sweepAndDisconnect('silent_drop').then((result) => {
                    ...
                    const partialSignal = result.descendantsFound !== undefined &&
                    ...
                    if (result.pidSweepError !== undefined || partialSignal) {
                        ...
                        // ambiguous.
                        debugLogger.warn(`PoolEntry ${this.id} silent-drop sweep observability: ` +
     ...
     */
    markActive(initialTools, initialPrompts, 
     ...
     */
    attach(sessionId, view, opts) {
        if (this.state === 'closed' || this.state === 'failed') {
            throw new Error(`Cannot attach to PoolEntry ${this.id} in state ${this.state}`);
        }
        ...
        if (this.state === 'active' && opts?.skipReplay !== true) {
            try {
                ...
                debugLogger.error(`Snapshot replay failed for ${sessionId}/${this.serverName}: ${String(err)}`);
                throw err;
            }
        }
     ...
     */
    updateSessionConfig(sessionId, cfg) {
        if (this.isTerminated()) {
            throw new Error(`Cannot update config on PoolEntry ${this.id} in state ${this.state}`);
        }
        const view = this.subscribers.get(sessionId);
        if (!view) {
            throw new Error(`Cannot update config for detached session ${sessionId} on PoolEntry ${this.id}`);
        }
        if (!view.updateConfig(cfg))
            return;
    ...
    detach(sessionId) {
        ...
        if (view) {
            try {
                ...
                debugLogger.error(`View teardown failed for ${sessionId}/${this.serverName}: ${String(err)}`);
            }
        }
        this.subscribers.delete(sessionId);
        this.subscriberHandles.delete(sessionId);
    ...
    startDrainTimer(delayMs) {
        ...
        if (this.firstIdleAt === undefined) {
            ...
            this.maxIdleTimer = setTimeout(() => {
                ...
                if (this.refs.size > 0) {
                    debugLogger.debug(`PoolEntry ${this.id} max-idle reached but ${this.refs.size} ` +
                        `sessions active; deferring close, resetting first-idle window`);
                    this.maxIdleTimer = undefined;
                    this.firstIdleAt = undefined;
                    return;
                }
                debugLogger.warn(`PoolEntry ${this.id} hit MAX_IDLE_MS (${this.opts.maxIdleMs}ms); force-closing`);
     ...
     */
    async sweepAndDisconnect(reason) {
        const result = {};
        try {
            ...
            if (rootPid !== undefined) {
                ...
                if (descendants.length > 0) {
                    ...
                    debugLogger.debug(`Sent SIGTERM to ${result.descendantsSignaled}/${descendants.length} descendants ` +
            ...
            debugLogger.warn(`Descendant pid sweep failed for ${this.id} (${reason}): ${String(err)}. Proceeding with disconnect.`);
        }
        try {
            ...
            debugLogger.error(`client.disconnect failed for ${this.id} (${reason}): ${String(err)}`);
        }
        return result;
    }
    /**
    ...
    async doRestart() {
        if (this.state === 'closed' || this.state === 'failed') {
            throw new Error(`Cannot restart PoolEntry ${this.id} in state ${this.state}`);
        }
        // set
    ...
    async doRestartInner() {
        ...
        try {
            ...
            snap = await runWithTimeout((async () => {
            ...
            })(), timeoutMs, `pool restart for ${this.id}`);
        }
        catch (err) {
            debugLogger.error(`Restart of ${this.id} failed at connect/discover: ${String(err)}. Transitioning to 'failed'.`);
            // the failure
        ...
        if (oldGen + 1 !== this._generation) {
            ...
            throw new Error(`Restart of ${this.id} superseded by newer generation; ` +
        ...
        if (currentState === 'closed' || currentState === 'failed') {
            ...
            await this.sweepAndDisconnect('restart_superseded');
            throw new Error(`Restart of ${this.id} aborted: entry state is ${currentState} ` +
        ...
        for (const [sid, view] of this.subscribers) {
            try {
                ...
                debugLogger.error(`Restart fan-out to view ${sid}/${this.serverName} failed: ${String(err)}`);
            }
        }
        this.localStatus = MCPServerStatus.CONNECTED;
        this.suppressNextStatusEcho = true;
    ...
    emit(event) {
        ...
        for (const listener of listeners) {
            try {
                ...
                debugLogger.error(`PoolEntry listener error for ${this.id} ` +
                    `(event.kind=${event.kind}): ${err instanceof Error ? err.message : String(err)}`);
            }
        }
    }
    internalOn(listener) {
...
class PooledConnectionImpl {
    ...
    get resourcesSnapshot() {
        return this.entry.resourcesSnapshot;
    }
    updateConfig(cfg) {
        if (this.released) {
            throw new Error(`Cannot update config on released MCP connection ${this.id}`);
        }
...
//# sourceMappingURL=mcp-pool-entry.js.map