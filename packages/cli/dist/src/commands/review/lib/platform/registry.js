...
export function isAoneHost(host) {
...
function cwdOriginUrl() {
    return gitOpt('remote', 'get-url', 'origin') ?? undefined;
}
export function detectPlatformKind(hint) {
    ...
    if (hintHost)
        return 'github';
    if (isAoneHost(hostOfRemoteUrl(hint?.remoteUrl)))
        return 'aone';
    ...
    if (isAoneHost(hostOfRemoteUrl(cwdOriginUrl())))
        return 'aone';
    return 'github';
}
export function getPlatformReader(hint) {
