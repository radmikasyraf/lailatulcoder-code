 ...
 */
export const COMMENT_MARKER = '<!-- LailatulCoder-review -->';
/** The marker with the finding's severity — the shape `submit` posts. */
export function commentMarker(severity) {
    return `<!-- LailatulCoder-review ${severity} -->`;
}
/** The trailing shape `submit` posts on attribution-off comments. */
const POSTED_MARKER_RE = /<!-- LailatulCoder-review (?:critical|suggestion) -->$/;
/** Whether the body ends with the posted marker shape. */
export function carriesCommentMarker(body) {
 ...
 * sees.
 */
export function commentMarkerSeverity(body) {
    const m = /<!-- LailatulCoder-review (critical|suggestion) -->$/.exec(body.trimEnd());
 ...
 */
export function swallowsAppendedMarker(body) {
 ...
 */
export function stripCommentMarkerLines(body) {
    if (!body.includes('<!-- LailatulCoder-review'))
        return body;
    return mapLinesAware(body, (line) => /^[ \t]{0,3}(?:>[ \t]*)*<!-- LailatulCoder-review (?:critical|suggestion)? -->[ \t]*\r?$/.test(line)
        ? null
        : line);
}
/**
 ...
 */
const NAMED_ENTITY_DECODES = new Map([
    ['sol', '/'],
    ['num', '#'],
    ['lpar', '('],
    ['rpar', ')'],
 ...
 */
function projectInvisibles(input) {
    let text = '';
    const starts = [];
    const ends = [];
    const push = (chars, start, end) => {
 ...
 * one character.
 */
function stripByProjection(line, re) {
...
const RUN_BREAK_RE = /^(?:#{1,6}(?:[ \t]|$)|[-*+][ \t]|\d{1,9}[.)][ \t]|(?:\*[ \t]*){3,}$|(?:-[ \t]*){3,}$|(?:_[ \t]*){3,}$)/;
/**
 ...
 */
function stripSplitFooterSpans(text) {
    let changed = false;
    const out = [];
    let para = [];
    let paraDepth = 0;
    const flush = () => {
        if (para.length > 1) {
            const joinedStripped = stripFooterSpanInLine(para.map((l) => l.trimEnd()).join(' '));
    ...
    for (const { line, kind, depth, content } of scanLines(text)) {
        if (kind === 'text' &&
            ...
            if (para.length > 0 && depth !== paraDepth)
                flush();
            para.push(line);
            paraDepth = depth;
            continue;
...
export function reviewFooter(modelId, cliVersion) {
    const name = modelId.length <= MODEL_ID_MAX_CHARS
        ? modelId
        : `${modelId.slice(0, MODEL_ID_MAX_CHARS - 1)}…`;
    const version = cliVersion.length <= MODEL_ID_MAX_CHARS
        ? cliVersion
 ...
 */
export const REVIEW_FOOTER_RE = /\s*(?:_— (?:(?! via LailatulCoder Ai \/review)[^\n])* via LailatulCoder Ai \/review(?: \(v[A-Za-z0-9._+-]{0,200}…?\)?)?_?\s*)+$/;
 ...
 */
export function stripReviewFooter(body) {
    const tail = body.slice(-STRIP_TAIL_LIMIT);
    const proj = projectInvisibles(tail);
    if (!proj.text.includes(FOOTER_MARKER))
        return body;
...
const HTML_BLOCK_TAG_NAMES = '(?:address|article|aside|base|basefont|blockquote|body|caption|center|col|colgroup|dd|details|dialog|dir|div|dl|dt|fieldset|figcaption|figure|footer|form|frame|frameset|h[1-6]|head|header|hr|html|iframe|legend|li|link|main|menu|menuitem|nav|noframes|ol|optgroup|option|p|param|section|source|summary|table|tbody|td|tfoot|th|thead|title|tr|track|ul)';
/**
 ...
 */
const HTML_BLOCK_OPEN_RE = new RegExp(`^(?:<[A-Za-z][^>]*>?|</${HTML_BLOCK_TAG_NAMES}[ \\t]*>)[ \\t]*\\r?$`, 'i');
...
function scanLines(body) {
    ...
    for (const line of body.split(/\r\n?|\n/)) {
        ...
        if (html !== null) {
            const closes = html.endRe === null ? line.trim() === '' : html.endRe.test(line);
        ...
        if (open !== null) {
            ...
            if (!(open[1][0] === '`' && content.slice(open[0].length).includes('`'))) {
 ...
 */
function mapLinesAware(body, map) {
    let changed = false;
    const out = [];
    ...
    const collapsible = (from, to) => runHas(drops, from, to) && !runHas(quotedDrops, from, to);
    const nonEmpty = [];
    out.forEach((l, i) => {
        if (l !== '')
            nonEmpty.push(i);
    ...
    for (let k = 0; k + 1 < nonEmpty.length; k += 1) {
        ...
        pieces.push(out[a]);
        pieces.push(collapsible(a + 1, b) && b - a > 2 ? '\n\n' : '\n'.repeat(b - a));
    }
    const last = nonEmpty[nonEmpty.length - 1];
    pieces.push(out[last]);
    pieces.push(collapsible(last + 1, out.length) ? '' : '\n'.repeat(out.length - 1 - last));
    return pieces.join('');
}
 ...
 */
function linkRefDefLines(lines, i) {
    const m = /^[ \t]{0,3}\[[^\n[\]]+\]:[ \t]*(.*)$/.exec(lines[i]);
    if (m === null)
        return 0;
    let rest = m[1];
 ...
 */
export function rendersAsNothing(text) {
    let stripped = text
        .replace(/<!--[\s\S]*?(?:-->|$)/g, '')
        ...
        .replace(/&nbsp;|&ensp;|&emsp;|&thinsp;|&shy;|&zwj;|&zwnj;|&lrm;|&rlm;|&zerowidthspace;|&#0*(?:160|173|819[2-9]|820[0-7]|8288|65279);|&#x0*(?:a0|ad|200[0-9a-f]|206[0-4]|feff);/gi, '')
    ...
    for (let pass = 0; pass < 4; pass += 1) {
        const next = stripped.replace(/<([a-z][a-z0-9-]*)[^<>\n]*?>\s*<\/\1\s*>/gi, '');
        if (next === stripped)
            break;
        stripped = next;
    }
    ...
    for (let i = 0; i < lines.length; i += 1) {
        ...
        if (/^[ \t]{0,3}(`{3,}|~{3,})[ \t]*\r?$/.test(l))
            continue;
        if (FORGED_FOOTER_LINE_RE.test(l))
            continue;
 ...
 */
const FORGED_FOOTER_LINE_RE = /^[ \t]{0,3}(?:>[ \t]*)?_— [^\n]{0,400} via LailatulCoder Ai \/review(?: \(v[^\n)]{0,200}\)?)?_?[ \t]*\r?$/;
export function stripForgedFooterLines(body) {
    if (!body.includes('/review') && !body.includes('&'))
        return body;
    return mapLinesAware(body, (line) => FORGED_FOOTER_LINE_RE.test(projectInvisibles(line).text) ? null : line);
}
/**
...
const PARAGRAPH_MARKER_RE = /^([ \t]{0,3}(?:>[ \t]*)*)(?:(?:\*\*\[Critical\]\*\*|\*\*\[Suggestion\]\*\*)[ \t]*[:：]?[ \t]*)+/;
export function stripParagraphMarkers(body) {
    if (!body.includes('**['))
        return body;
 ...
 */
export function stripForUnattributedPost(body) {
    let current = body;
    for (;;) {
        const next = stripFooterSpans(stripParagraphMarkers(stripCommentMarkerLines(stripSeverityPrefix(stripForgedFooterLines(current)))));
        if (next === current)
            return current;
        current = next;
    }
 ...
 * normalizing to one.
 */
export function isFooterSafeModelId(modelId) {
 ...
 */
export function footerVersion(stamp) {
    return stamp !== undefined && FOOTER_VERSION_RE.test(stamp)
        ? stamp
        : undefined;
}
//# sourceMappingURL=review-footer.js.map