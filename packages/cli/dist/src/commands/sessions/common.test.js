...
const mockedLoadSettings = mockLoadSettings;
const mockedSetRuntimeBaseDir = mockSetRuntimeBaseDir;
describe('common', () => {
    beforeEach(() => {
        vi.resetAllMocks();
    });
    it('should call Storage.setRuntimeBaseDir with correct args', () => {
        ...
        initSessionService();
        expect(mockedSetRuntimeBaseDir).toHaveBeenCalledWith('/custom/runtime', process.cwd());
    });
    it('should pass undefined when runtimeOutputDir is not configured', () => {
        ...
        mockSessionServiceConstructor.mockReturnValue({});
        initSessionService();
        expect(mockedSetRuntimeBaseDir).toHaveBeenCalledWith(undefined, process.cwd());
    });
