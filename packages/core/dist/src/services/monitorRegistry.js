...
import { stripDisplayControlChars, truncateNotificationLabel, } from '../utils/terminalSafe.js';
...
export class MonitorRegistry {
    ...
    emitNotification(entry, eventLine) {
        ...
        xmlParts.push('<kind>monitor</kind>', '<status>running</status>', `<event-count>${entry.eventCount}</event-count>`, `<summary>Monitor "${escapeXml(desc)}" emitted event #${entry.eventCount}.</summary>`, `<result>${escapeXml(safeEventLine)}</result>`, '</task-notification>');
        const meta = {
    ...
    emitTerminalNotification(entry, detail) {
        ...
        xmlParts.push('<kind>monitor</kind>', `<status>${escapeXml(entry.status)}</status>`, `<event-count>${entry.eventCount}</event-count>`, `<summary>Monitor "${escapeXml(desc)}" ${statusText}. Total events: ${entry.eventCount}.${entry.droppedLines > 0 ? ` ${entry.droppedLines} lines dropped due to throttling.` : ''}</summary>`, `<command>${escapeXml(stripDisplayControlChars(entry.command))}</command>`);
        if (detail) {
