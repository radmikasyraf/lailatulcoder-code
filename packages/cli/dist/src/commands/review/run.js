...
export function classifyRunTarget(target) {
    if (!target)
        return { kind: 'local' };
    ...
    if (t.type === 'file') {
        ...
        // (`LailatulCoder-review--composed.json`) no child artifact can ever carry.
 ...
 * the skill's literal `--out .lailatulcoder/tmp/LailatulCoder-review-{target}-composed.json`
 ...
 * share `.lailatulcoder/tmp`, and the pre-pin newest-composed scan captured the
 ...
 */
export function composedNameFor(cls) {
    switch (cls.kind) {
        case 'pr':
            return `LailatulCoder-review-pr-${cls.number}-composed.json`;
        case 'file':
            return `LailatulCoder-review-${cls.base}-composed.json`;
        case 'local':
        default:
            return 'LailatulCoder-review-local-composed.json';
    }
}
export function composedPatternFor(cls) {
    return new RegExp(`^${escapeRe(composedNameFor(cls))}$`);
}
/**
 * The saved report under `.lailatulcoder/reviews/`, pinned as far as its naming
 ...
 * exact.
 */
export function reportPatternFor(cls) {
    switch (cls.kind) {
        case 'pr':
            return new RegExp(`-pr-${cls.number}\\.md$`);
        case 'file':
            return new RegExp(cls.base.toLowerCase().endsWith('.md')
                ? `-${escapeRe(cls.base)}$`
...
const PARENT_SIGNALS = Object.keys(SIGNAL_EXIT_CODES);
...
export function buildReviewPrompt(args) {
    ...
    // and got wrong — `LailatulCoder review run "$TARGET"` with `TARGET` unset — and
    ...
    if (args.target !== undefined) {
        ...
        if (args.target.trim() === '' ||
            ...
            // `LailatulCoder-review--composed.json` and burns a whole child review before
            // reporting "no composed verdict". Refuse it here instead.
            /^[\\/]+$/.test(args.target)) {
            throw new Error(`Invalid review target ${JSON.stringify(args.target)}: expected a single PR number, PR URL, or file path`);
        }
        parts.push(args.target);
    }
    if (args.effort)
        parts.push(`--effort ${args.effort}`);
    if (args.comment)
        parts.push('--comment');
    if (args.resume)
        parts.push('--resume');
 ...
 */
export function newestArtifactSince(dir, pattern, startMs) {
    let best = null;
    let names;
    try {
 ...
 */
export function exitCodeFor(completed, event, failOn) {
    if (!completed)
        return 1;
    if (failOn === 'request-changes' && event === 'REQUEST_CHANGES')
        return 3;
    return 0;
}
function readComposed(path) {
    try {
 ...
 * `"${QWEN_CODE_CLI:-LailatulCoder}" review match-remote`, resolved `LailatulCoder` off PATH,
 ...
 * that test is worse than none: `${QWEN_CODE_CLI:-LailatulCoder}` falls back on empty,
 * but a set-and-unusable path dies on exit 126.
 *
 * cli.ts stamps QWEN_CODE_CLI first-writer-wins, so a `review run` launched
 * from INSIDE a parent LailatulCoder session inherits the parent's entry — and the
 * skill's every `"${QWEN_CODE_CLI:-LailatulCoder}" review …` subcommand then runs the
 ...
 * `isUnusableScriptEntry`, preserving the bare-`LailatulCoder` fallback instead of a
...
async function runReview(args) {
    ...
    // Re-enter THIS build's CLI, not whatever `LailatulCoder` PATH resolves to — the same
    ...
    // every `.lailatulcoder/tmp/LailatulCoder-review-<target>-*` file — including it — before the
    ...
    const captureTimer = setInterval(() => {
        ...
        if (best === null || best.mtime <= capturedMtime)
            return;
    ...
    const timer = setTimeout(() => {
        ...
        writeStderrLineSafe(`review run: timeout after ${args.timeoutMinutes} minutes — terminating the review`);
    ...
    clearInterval(captureTimer);
    for (const signal of PARENT_SIGNALS)
        process.off(signal, onParentSignal);
    ...
    let composedPath = capturedPath;
    let composed = capturedVerdict;
    if (composed === null) {
        const best = newestArtifactSince(REVIEW_TMP_DIR, composedPattern, cutoffMs);
        composedPath = best?.path ?? null;
        composed = composedPath ? readComposed(composedPath) : null;
    }
    const reportPath = newestArtifactSince(REVIEWS_DIR, reportPatternFor(targetClass), cutoffMs)
        ?.path ?? null;
    ...
    try {
        if (args.json) {
            ...
            if (result.reportPath)
                writeStdoutLine(`Report: ${result.reportPath}`);
        }
        else {
            ...
            // has swept `.lailatulcoder/tmp` — the near-miss name is gone. A no-verdict
            // report that does not say which file it was waiting for cannot be
            // diagnosed as a naming drift.
            const detail = composedPath !== null
                ? `a composed verdict was found at ${resolve(composedPath)} but could not be parsed`
                : `no composed verdict was produced (expected ${join(REVIEW_TMP_DIR, composedNameFor(targetClass))})`;
            writeStdoutLine(timedOut
...
export const runCommand = {
    command: 'run [target]',
    describe: 'Run a full /review non-interactively and print the verdict (machine-readable with --json)',
    builder: (yargs) => yargs
        .positional('target', {
        type: 'string',
        describe: 'What to review: a PR number, a PR URL, or a file path; omit to review the local working tree',
    })
        .option('effort', {
        type: 'string',
        choices: [...EFFORT_LEVELS],
        describe: 'The review effort. Defaults to the skill default for the target (high for a PR, medium locally).',
    })
        .option('comment', {
        type: 'boolean',
        default: false,
        describe: 'Authorise posting the review to GitHub (PR targets only) — same meaning as `/review <pr> --comment`',
    })
        .option('resume', {
        type: 'boolean',
        default: false,
        describe: 'Continue an interrupted review of this PR when its on-disk state still matches, instead of starting over (PR targets only) — same meaning as `/review <pr> --resume`. Falls back to a fresh review when nothing can be resumed.',
    })
        .option('json', {
        type: 'boolean',
        default: false,
        ...
        default: 'none',
        describe: 'Exit 3 when the review completes with this outcome — lets CI gate on the verdict without parsing output',
    })
        .option('timeout-minutes', {
        type: 'number',
        default: 120,
        describe: 'Terminate the review after this long without a verdict (exit 1)',
    })
        .option('approval-mode', {
        type: 'string',
        default: 'yolo',
        choices: ['plan', 'default', 'auto-edit', 'auto', 'yolo'],
        describe: 'Approval mode for the child CLI. The default is yolo: headless runs cannot answer ' +
