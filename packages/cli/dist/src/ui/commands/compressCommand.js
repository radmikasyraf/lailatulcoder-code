...
export const compressCommand = {
    ...
    action: async (context) => {
        ...
        const doCompress = async () => {
            ...
            return await geminiClient.tryCompressChat(promptId, true, abortSignal, customInstructions);
        };
        ...
        try {
            if (executionMode === 'interactive') {
                if (truncationNotice) {
                    ui.addItem({ type: MessageType.INFO, text: truncationNotice }, Date.now());
                }
            ...
            if (executionMode === 'interactive') {
                ...
                if (compressed.warning) {
                    ui.addItem({ type: MessageType.INFO, text: `⚠️ ${compressed.warning}` }, Date.now());
                }
                return;
            }
            return {
