...
vi.mock('@lailatul-coder/lailatul-coder-core', () => {
    class Storage {
        ...
        getHistoryFilePath() {
            return path.join('/test/home/', '.qwen', 'tmp', 'mocked_hash', 'shell_history');
        }
    }
    return {
...
const MOCKED_HISTORY_DIR = path.join(MOCKED_HOME_DIR, '.qwen', 'tmp', MOCKED_PROJECT_HASH);
...
describe('useShellHistory', () => {
    ...
    it('should add a command and write to the history file', async () => {
        ...
        await waitFor(() => {
            ...
            expect(mockedFs.writeFile).toHaveBeenCalledWith(MOCKED_HISTORY_FILE, JSON.stringify(['new_command']));
        });
        let command = null;
        act(() => {
            command = result.current.getPreviousCommand();
    ...
    it('should preserve multi-line commands after reloading history', async () => {
        ...
        const { result, unmount } = renderHook(() => useShellHistory(MOCKED_PROJECT_ROOT));
        await waitFor(() => expect(mockedFs.readFile).toHaveBeenCalled());
        act(() => {
        ...
        const { result: reloaded } = renderHook(() => useShellHistory(MOCKED_PROJECT_ROOT));
    ...
    it('should truncate history to MAX_HISTORY_LENGTH (100)', async () => {
        ...
        const writtenContent = mockedFs.writeFile.mock.calls[0][1];
        const writtenHistory = JSON.parse(writtenContent);
