 ...
 */
function mockRes(writeBehavior, writeCallbacks) {
...
describe('SseStream', () => {
    ...
    it('does not retain drain listeners after a synchronous callback error', async () => {
        ...
        const s = new SseStream(res);
        await expect(s.sendSerialized(Buffer.from('{"ok":true}'))).resolves.toBe('failed');
    ...
    it('reports an accepted complete frame as outcome unknown on close', async () => {
        ...
        await vi.waitFor(() => expect(callbacks).toHaveLength(1));
        expect(res.chunks.join('')).toBe('data: {"ok":true}\n\n');
        res.emit('close');
