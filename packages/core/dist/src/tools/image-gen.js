...
class ImageGenInvocation extends BaseToolInvocation {
    ...
    generateImage;
    outputPath;
    constructor(config, generateImage, params) {
        super(params);
        this.config = config;
        this.generateImage = generateImage;
        const sessionDir = Storage.sanitizePlanSessionId(config.getSessionId());
        this.outputPath = path.join(config.getTargetDir(), '.lailatulcoder', 'generated-images', sessionDir, `${randomUUID()}.png`);
    }
    toolLocations() {
    ...
    async execute(signal) {
        const imageConfig = this.config.getImageGenerationConfig();
        if (!imageConfig) {
            return failureResult('Image generation is not configured with a valid endpoint.');
        }
        const apiKey = process.env[imageConfig.apiKeyEnv]?.trim();
        if (!apiKey) {
            return failureResult(`Image generation requires the ${imageConfig.apiKeyEnv} environment variable.`);
        }
        try {
            signal.throwIfAborted();
            const outputDir = path.dirname(this.outputPath);
            Storage.assertPathWithinDirectory(outputDir, this.config.getTargetDir(), 'Generated image path must stay inside the workspace.');
            ...
            return failureResult(error instanceof Error ? error.message : getErrorMessage(error));
        }
    }
}
export class ImageGenTool extends BaseDeclarativeTool {
    ...
    constructor(config, generateImage = generateConfiguredImage) {
        super(ImageGenTool.Name, ToolDisplayNames.IMAGE_GEN, 'Generates a PNG image with the configured image model and saves it as a workspace artifact. Use size in width*height form when the user requests a specific aspect ratio.', Kind.Execute, {
            type: 'object',
            properties: {
                prompt: {
                    type: 'string',
            ...
            required: ['prompt'],
        }, true, false, false, false, 'image generation picture poster illustration');
        this.config = config;
        this.generateImage = generateImage;
    }
