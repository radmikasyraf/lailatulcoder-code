...
export async function refreshExtensionContentRuntime(options) {
    ...
    try {
        ...
        errors.push(error);
        debugLogger.warn('refreshExtensionContentRuntime: refreshCache failed:', error);
    }
    ...
    for (const result of settled) {
        if (result.status === 'rejected') {
            ...
            debugLogger.warn('refreshExtensionContentRuntime: a refresh leg failed:', result.reason);
        }
    }
    if (errors.length > 0) {
...
function summarizeExtensions(extensions) {
    return extensions.reduce((summary, extension) => {
...
function countHooks(extension) {
    return Object.values(extension.hooks ?? {}).reduce((sum, definitions) => sum +
        (definitions ?? []).reduce((innerSum, definition) => innerSum + (definition.hooks?.length ?? 0), 0), 0);
}
...
//# sourceMappingURL=extension-runtime-reload.js.map