...
import { sanitizeFilenameForDisplay, sanitizeMultilineForDisplay, } from '../utils/textUtils.js';
 ...
 * trailing blob).
 */
async function readPreviewChunk(filePath) {
...
export const SkillReviewDialog = ({ skills, onAccept, onReject, onClose, onDismiss, }) => {
    ...
    useEffect(() => {
        ...
        readPreviewChunk(stagedPath)
            ...
            const allLines = sanitizeMultilineForDisplay(text.replace(/\r\n/g, '\n').replace(/\n+$/, '')).split('\n');
    ...
    const turnOff = () => {
        ...
        try {
            ...
            setActionError(`Failed to save setting: ${err instanceof Error ? err.message : String(err)}`);
            return;
        }
    ...
    if (snapshot.length - index > 1) {
        options.push({ label: t('Keep all remaining'), value: 'keepAll', key: 'keepAll' }, {
    ...
    const visiblePreview = preview.status === 'ready' && preview.path !== stagedPath
        ? { status: 'loading' }
        : preview;
    return (_jsxs(Box, { flexDirection: "column", borderStyle: "round", borderColor: theme.status.warning, paddingX: 1, width: "100%", children: [_jsxs(Text, { bold: true, color: theme.text.primary, children: [t('Auto-generated skill — keep it?'), " (", index + 1, "/", snapshot.length, ")"] }), _jsxs(Box, { marginTop: 1, flexDirection: "column", children: [_jsx(Text, { color: theme.text.primary, children: sanitizeFilenameForDisplay(current.name) }), current.description ? (_jsx(Text, { color: theme.text.secondary, children: sanitizeMultilineForDisplay(current.description) })) : null] }), _jsx(Box, { marginTop: 1, flexDirection: "column", children: visiblePreview.status === 'loading' ? (_jsx(Text, { color: theme.text.secondary, children: t('Loading preview…') })) : visiblePreview.status === 'error' ? (_jsx(Text, { color: theme.text.secondary, children: t('Preview unavailable') })) : (_jsxs(_Fragment, { children: [_jsx(MaxSizedBox, { maxHeight: PREVIEW_MAX_HEIGHT, maxWidth: previewWidth, overflowDirection: "bottom", additionalHiddenLinesCount: visiblePreview.hiddenLines, children: visiblePreview.lines.map((line, i) => (_jsx(Box, { children: _jsx(Text, { wrap: "wrap", color: theme.text.secondary, children: line === '' ? ' ' : line }) }, i))) }), visiblePreview.truncated ? (_jsx(Text, { color: theme.text.secondary, children: t('… preview truncated (file too large) …') })) : null] })) }), actionError ? (_jsx(Box, { marginTop: 1, children: _jsx(Text, { color: theme.status.error, children: sanitizeMultilineForDisplay(actionError) }) })) : null, _jsx(Box, { marginTop: 1, children: _jsx(RadioButtonSelect, { items: options, onSelect: handleSelect, isFocused: true }) }), _jsx(Box, { marginTop: 1, children: _jsx(Text, { color: theme.text.secondary, children: t('o open in editor · Esc decide later') }) })] }));
};
//# sourceMappingURL=SkillReviewDialog.js.map