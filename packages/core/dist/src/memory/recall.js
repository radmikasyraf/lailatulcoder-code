...
import { scanAllAutoMemoryTopicDocuments, scanAllUserAutoMemoryTopicDocuments, } from './scan.js';
 ...
 */
const CJK_CLASS = '[\\p{Script=Han}\\p{Script=Hiragana}\\p{Script_Extensions=Katakana}\\p{Script=Hangul}]';
/**
 ...
 */
const RECALL_TOKEN_RUN = new RegExp(`${CJK_CLASS}+|(?!${CJK_CLASS})[\\p{L}\\p{N}](?:(?!${CJK_CLASS})[\\p{L}\\p{M}\\p{N}]){2,}`, 'gu');
...
function tokenize(text) {
    ...
    const addToken = (token) => {
        ...
        if (tailTokens.size > edgeSize) {
            ...
            if (!oldest.done)
                tailTokens.delete(oldest.value);
        }
    };
 ...
 */
function createActiveToolUsageFilter(recentTools) {
    ...
    return (doc) => {
        ...
        if (DURABLE_ACTIVE_TOOL_MEMORY_MARKERS.some((marker) => haystack.includes(marker))) {
            return false;
        }
        return ACTIVE_TOOL_USAGE_MEMORY_MARKERS.some((marker) => haystack.includes(marker));
    };
}
function scoreDocument(queryTokens, doc) {
    ...
    const body = normalizeRecallText(normalizeBody(doc.body).slice(0, MAX_DOC_BODY_CHARS));
    let lexicalScore = 0;
    ...
    const typeBoost = Math.min(queryTokens.filter((token) => TYPE_KEYWORDS[doc.type]?.includes(token))
        .length, 2);
    return lexicalScore + typeBoost;
}
export function selectRelevantAutoMemoryDocuments(query, docs, limit = MAX_RELEVANT_DOCS) {
...
function selectModelCandidateDocuments(query, docs, recentTools, fallbackLimit) {
    const isActiveToolNoise = createActiveToolUsageFilter(recentTools);
    const eligible = docs.filter((doc) => !isActiveToolNoise(doc));
    const lexical = selectRelevantAutoMemoryDocuments(query, eligible, Math.max(MAX_MODEL_CANDIDATE_DOCS - RECENT_MODEL_CANDIDATE_RESERVE, fallbackLimit));
    const modelLexical = lexical.slice(0, MAX_MODEL_CANDIDATE_DOCS - RECENT_MODEL_CANDIDATE_RESERVE);
...
export function buildRelevantAutoMemoryPrompt(docs) {
    if (docs.length === 0) {
        return '';
    }
    return [
...
function filterExcludedAutoMemoryDocuments(docs, excludedFilePaths) {
...
export async function resolveRelevantAutoMemoryPromptForQuery(projectRoot, query, options = {}) {
    const t0 = Date.now();
    // User-level scan is best-effort: a read failure (EACCES, ELOOP) on
    // `~/.lailatulcoder/memories/` must not cancel the project-level scan, otherwise
    ...
    const [projectDocs, userDocs] = await Promise.all([
        ...
        scanAllUserAutoMemoryTopicDocuments().catch((error) => {
            debugLogger.warn(`User-level auto-memory scan failed; project-level recall continues: ${error instanceof Error ? error.message : String(error)}`);
            return [];
        }),
    ]);
    ...
    // there, not enforced.
    const docs = filterExcludedAutoMemoryDocuments([...projectDocs, ...userDocs], options.excludedFilePaths);
    ...
    if (query.trim().length === 0 || docs.length === 0 || limit <= 0) {
        if (options.config && !options.abortSignal?.aborted) {
            logMemoryRecall(options.config, new MemoryRecallEvent({
    ...
    if (options.config) {
        try {
            const candidates = selectModelCandidateDocuments(query, docs, options.recentTools ?? [], limit);
            ...
            const selectedDocs = await selectRelevantAutoMemoryDocumentsByModel(options.config, query, candidates.modelCandidates, limit, options.recentTools ?? [], options.abortSignal);
            const strategy = selectedDocs.length > 0 ? 'model' : 'none';
            ...
            if (error instanceof DOMException && error.name === 'AbortError') {
                if (options.abortSignal?.aborted) {
                    debugLogger.debug('Model-driven auto-memory recall aborted by caller; heuristic result discarded.');
                }
                else {
                    debugLogger.debug('Model-driven auto-memory recall timed out (30 s safety net); heuristic fallback will run.');
                }
            }
            else {
                debugLogger.warn('Model-driven auto-memory recall failed; falling back to heuristic selection.', error);
            }
        }
    }
    ...
    const isActiveToolNoise = createActiveToolUsageFilter(options.recentTools ?? []);
    const selectedDocs = fallbackDocs ??
        selectRelevantAutoMemoryDocuments(query, docs.filter((doc) => !isActiveToolNoise(doc)), limit);
    const strategy = selectedDocs.length > 0 ? 'heuristic' : 'none';
    if (options.config && !options.abortSignal?.aborted) {
        logMemoryRecall(options.config, new MemoryRecallEvent({
            query_length: query.length,
            docs_scanned: docs.length,
...
export async function buildRelevantAutoMemoryPromptForQuery(projectRoot, query, options = {}) {
    const result = await resolveRelevantAutoMemoryPromptForQuery(projectRoot, query, options);
