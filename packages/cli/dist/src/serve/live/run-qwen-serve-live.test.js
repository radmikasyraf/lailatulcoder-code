...
vi.mock('./discovery.js', async (importOriginal) => {
    ...
    trackedWriteLiveDiscoveryFile.mockImplementation(actual.writeLiveDiscoveryFile);
    return {
        ...actual,
...
describe('qwen serve Live Host discovery', () => {
    it('publishes the authenticated listener and removes only its own record', async () => {
        const runtime = await fs.mkdtemp(path.join(os.tmpdir(), 'qwen-serve-live-'));
        ...
        await fs.writeFile(path.join(qwenHome, 'settings.json'), JSON.stringify({
            experimental: {
        ...
        try {
            ...
            const record = JSON.parse(await fs.readFile(discoveryPath, 'utf8'));
            ...
            expect(trackedWriteLiveDiscoveryFile.mock.calls.map(([baseDir]) => path.resolve(String(baseDir)))).toEqual([path.resolve(runtime)]);
            ...
            expect((await capabilities.json()).features).not.toContain('realtime_voice');
        }
        finally {
            await handle?.close();
    ...
    it('publishes a stable Host locator alongside a custom runtime record', async () => {
        const root = await fs.mkdtemp(path.join(os.tmpdir(), 'qwen-serve-live-stable-'));
        ...
        await fs.writeFile(runtimeDiscoveryPath, `${JSON.stringify({
        ...
        try {
            ...
            const runtimeRecord = JSON.parse(await fs.readFile(getLiveDiscoveryPath(runtime), 'utf8'));
            const stableRecord = JSON.parse(await fs.readFile(getLiveDiscoveryPath(stable), 'utf8'));
            expect(runtimeRecord).toMatchObject({
                url: handle.url,
                pid: process.pid,
            });
            expect(stableRecord).toEqual(runtimeRecord);
            const writtenBaseDirs = trackedWriteLiveDiscoveryFile.mock.calls.map(([baseDir]) => path.resolve(String(baseDir)));
            expect(writtenBaseDirs).toEqual([
                path.resolve(runtime),
                path.resolve(stable),
            ]);
            expect(writtenBaseDirs.every((baseDir) => baseDir === path.resolve(root) ||
                baseDir.startsWith(`${path.resolve(root)}${path.sep}`))).toBe(true);
        }
        finally {
            await handle?.close();
            if (previousQwenHome === undefined)
    ...
    it('hands stable discovery ownership to a waiting enabled daemon', async () => {
        const root = await fs.mkdtemp(path.join(os.tmpdir(), 'qwen-serve-live-handoff-'));
        temporaryDirectories.push(root);
        const stable = path.join(root, 'stable-qwen-home');
        const qwenHome = path.join(root, 'settings-home');
        ...
        try {
            ...
            await vi.waitFor(async () => {
                expect(JSON.parse(await fs.readFile(stablePath, 'utf8'))).toMatchObject({ url: second.url });
            }, { timeout: 5_000, interval: 20 });
            expect(trackedWriteLiveDiscoveryFile.mock.calls.every(([baseDir]) => {
                const resolved = path.resolve(String(baseDir));
                return (resolved === path.resolve(root) ||
...
//# sourceMappingURL=run-qwen-serve-live.test.js.map