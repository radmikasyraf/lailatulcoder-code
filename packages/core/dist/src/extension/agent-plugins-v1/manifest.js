...
export function getAgentPluginSchemaStatus(pluginRoot) {
    const manifestPath = path.join(pluginRoot, AGENT_PLUGIN_MANIFEST);
    let resolvedManifestPath;
    try {
        resolvedManifestPath = resolveContainedExistingPath(pluginRoot, manifestPath);
    }
    catch (error) {
    ...
    try {
        if (!fs.statSync(resolvedManifestPath).isFile())
            return 'unrelated';
        value = JSON.parse(fs.readFileSync(resolvedManifestPath, 'utf8'));
    }
    catch {
        return 'unrelated';
    }
...
export function loadAgentPluginManifest(pluginRoot) {
    const manifestPath = path.join(pluginRoot, AGENT_PLUGIN_MANIFEST);
    const resolvedManifestPath = resolveContainedExistingPath(pluginRoot, manifestPath);
    if (!fs.statSync(resolvedManifestPath).isFile()) {
        throw new Error('Agent Plugins root plugin.json must be a regular file.');
    }
    const value = JSON.parse(fs.readFileSync(resolvedManifestPath, 'utf8'));
    if (!isRecord(value)) {
        throw new Error('Agent Plugins root plugin.json must contain an object.');
    }
    for (const field of Object.keys(value)) {
        if (!MANIFEST_FIELDS.has(field)) {
            debugLogger.warn(`Ignoring unknown Agent Plugins manifest field "${field}".`);
        }
    }
    ...
    if (schema !== AGENT_PLUGIN_SCHEMA) {
        throw new Error(schema.startsWith(AGENT_PLUGIN_SCHEMA_PREFIX)
    ...
    if (typeof name !== 'string' || !isValidPluginName(name)) {
        throw new Error('Agent Plugins name must be 1-64 lowercase letters, numbers, dots, or hyphens without leading, trailing, or consecutive separators.');
    }
...
function validateAuthor(value) {
    ...
    for (const [field, fieldValue] of Object.entries(value)) {
        ...
        if (typeof fieldValue !== 'string') {
            throw new Error(`Agent Plugins author field "${field}" must be a string.`);
        }
    }
}
function validateKeywords(value) {
    ...
    if (!Array.isArray(value) || value.some((item) => typeof item !== 'string')) {
        throw new Error('Agent Plugins manifest "keywords" must be an array of strings.');
    }
}
