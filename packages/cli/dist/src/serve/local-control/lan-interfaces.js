...
export function listLanCandidates(interfaces = networkInterfaces()) {
    ...
    for (const [interfaceName, addresses] of Object.entries(interfaces).sort()) {
        if (isSoftwareNetwork(interfaceName))
            continue;
        for (const address of addresses ?? []) {
            if (address.family !== 'IPv4' || address.internal)
                continue;
            if (!isLanIpv4(address.address))
                continue;
...
export class NoLanInterfaceError extends Error {
    ...
    constructor() {
        super('No private IPv4 address is available. Local Control needs the host to ' +
...
export class AmbiguousLanInterfaceError extends Error {
    ...
    constructor(candidates) {
        super('Multiple local networks are available; choose which one to expose: ' +
...
export class UnknownLanInterfaceError extends Error {
    ...
    constructor(requested) {
        super(`${requested} is not a private IPv4 address on this host right now. ` +
 ...
 * commit to an answer.
 */
export function selectLanAddress(preferredAddress, interfaces = networkInterfaces()) {
    ...
    if (preferredAddress !== undefined) {
        ...
        if (!match)
            throw new UnknownLanInterfaceError(preferredAddress);
        return match;
    }
    if (candidates.length > 1)
        throw new AmbiguousLanInterfaceError(candidates);
    return candidates[0];
}
//# sourceMappingURL=lan-interfaces.js.map