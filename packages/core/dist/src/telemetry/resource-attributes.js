 ...
 */
export const RESERVED_RESOURCE_ATTRIBUTE_KEYS = new Set([
    'service.version',
    'session.id',
]);
 ...
 * (`~/.lailatulcoder/log/otel-*.log`), not console — see PR #3986. The SDK emits a
 * single console summary at startup when this list is non-empty so users
 * notice silent drops without scanning the debug log.
 */
export function parseOtelResourceAttributes(raw, warnings) {
    if (!raw)
        return {};
    const out = {};
    for (const pair of raw.split(',')) {
        ...
        if (idx < 0) {
            ...
            warn(`Skipping malformed OTEL_RESOURCE_ATTRIBUTES entry: "${trimmed}" ` +
                `(hint: percent-encode literal commas as %2C)`, warnings);
            continue;
        }
        const rawKey = trimmed.slice(0, idx).trim();
        if (!rawKey)
            continue; // silent skip: "=value" or " =value"
        ...
        try {
            ...
            warn(`Invalid percent-encoding in OTEL_RESOURCE_ATTRIBUTES key "${rawKey}", using raw key`, warnings);
            key = rawKey;
        }
        let value;
        try {
            ...
            warn(`Invalid percent-encoding in OTEL_RESOURCE_ATTRIBUTES for key "${key}", using raw value`, warnings);
            value = valueRaw;
        }
        out[key] = value;
    }
 ...
 */
export function stripReservedResourceAttributes(attrs, source, warnings) {
 ...
 */
export function coerceStringResourceAttributes(raw, warnings) {
    if (raw === null || raw === undefined)
        return {};
    if (typeof raw !== 'object' || Array.isArray(raw)) {
        warn('settings.telemetry.resourceAttributes must be an object; ignoring', warnings);
        return {};
    }
    const out = {};
    for (const [k, v] of Object.entries(raw)) {
        const key = k.trim();
        if (!key) {
            warn('settings.telemetry.resourceAttributes has an empty or whitespace-only key; ignoring', warnings);
            continue;
        }
        if (typeof v === 'string') {
            out[key] = v;
            ...
            warn(`settings.telemetry.resourceAttributes value for "${key}" must be a string (got ${typeof v}); ignoring`, warnings);
        }
    }
    return out;
}
//# sourceMappingURL=resource-attributes.js.map