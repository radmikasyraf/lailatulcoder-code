...
describe('learnCommand', () => {
    ...
    it.each([AuthType.USE_OPENAI, AuthType.QWEN_OAUTH])('submits a native video part through %s', async (authType) => {
        const ctx = createMockCommandContext({
            services: {
                config: {
                    getProjectRoot: () => '/tmp/test-project',
        ...
        const result = await learnCommand.action(ctx, 'https://cdn.example.com/tutorial.mp4 focus on visual verification');
    ...
    it('attaches a local video through the video-specific /learn path', async () => {
        ...
        const result = await learnCommand.action(ctx, './tutorial.mp4 focus on the hover animation');
        expect(mockReadPathFromWorkspace).toHaveBeenCalledWith(expandHomeDir('./tutorial.mp4'), ctx.services.config);
        expect(result).toMatchObject({
            type: 'submit_prompt',
            content: [
    ...
    it('falls back to the text path when a local video path does not resolve', async () => {
        mockReadPathFromWorkspace.mockRejectedValueOnce(new Error('Absolute path is outside of the allowed workspace'));
        const ctx = createVideoCapableContext();
        const result = await learnCommand.action(ctx, '/tmp/tutorial.mp4');
        expect(result).toMatchObject({ type: 'submit_prompt' });
        expect(result.content).toContain('/tmp/tutorial.mp4');
    });
    it('falls back to the text path when prose starts with a video-extension token', async () => {
        mockReadPathFromWorkspace.mockRejectedValueOnce(new Error('Path not found in workspace: demo.mov'));
        const ctx = createVideoCapableContext();
        const result = await learnCommand.action(ctx, 'demo.mov is how we record release walkthroughs');
        expect(result).toMatchObject({ type: 'submit_prompt' });
        expect(result.content).toContain('demo.mov is how we record release walkthroughs');
    });
    it.each([
        ...
        const result = await learnCommand.action(ctx, 'https://cdn.example.com/tutorial.mp4');
        expect(result).toMatchObject({
            type: 'message',
            messageType: 'error',
