...
import { acquireLock, rollbackStandaloneUpdate, ensureBinWrapper, ensurePathInShellRc, cleanupFirstTimeMigrationArtifacts, performStandaloneUpdate, isSafeTarEntryPath, isSafeTarEntry, isSafeTarLinkTarget, } from './standalone-update.js';
describe('standalone-update', () => {
    ...
    describe('rollbackStandaloneUpdate', () => {
        it('returns no-old when .old directory does not exist', () => {
            const standaloneDir = path.join(tempDir, 'lailatul-coder');
            fs.mkdirSync(standaloneDir);
            fs.writeFileSync(path.join(standaloneDir, 'manifest.json'), JSON.stringify({
        ...
        it('swaps current with .old directory on valid rollback', () => {
            ...
            const manifest = JSON.parse(fs.readFileSync(path.join(standaloneDir, 'manifest.json'), 'utf-8'));
            expect(manifest.version).toBe('0.16.2');
            expect(fs.readFileSync(path.join(standaloneDir, 'marker.txt'), 'utf-8')).toBe('old');
        ...
        it('succeeds even with minimal manifest in .old', () => {
            ...
            fs.mkdirSync(standaloneDir);
            fs.mkdirSync(oldDir);
            fs.writeFileSync(path.join(standaloneDir, 'manifest.json'), JSON.stringify({ name: '@lailatul-coder/lailatul-coder', version: '0.17.0' }));
    ...
    describe('ensureBinWrapper', () => {
        it('uses an independent launcher wait budget in the deferred Windows swap script', () => {
            const source = fs.readFileSync(path.resolve('src/utils/standalone-update.ts'), 'utf8');
        ...
        it.skipIf(process.platform === 'win32')('creates a Unix shell wrapper script', () => {
            const libDir = path.join(tempDir, '.local', 'lib');
            const standaloneDir = path.join(libDir, 'lailatul-coder');
        ...
        it.skipIf(process.platform === 'win32')('escapes single quotes in Unix wrapper paths', () => {
            ...
            const wrapperPath = path.join(tempDir, "o'brien", '.local', 'bin', 'qwen');
            const content = fs.readFileSync(wrapperPath, 'utf-8');
            expect(content).toContain("o'\\''brien");
        });
        it.skipIf(process.platform === 'win32')('allows shell metacharacters in single-quoted Unix wrapper paths', () => {
            ...
            const wrapperPath = path.join(tempDir, 'with$dollar', '.local', 'bin', 'qwen');
            const content = fs.readFileSync(wrapperPath, 'utf-8');
            expect(content).toContain('with$dollar');
        });
        it.skipIf(process.platform === 'win32')('does not overwrite existing wrapper', () => {
            const libDir = path.join(tempDir, '.local', 'lib');
            const standaloneDir = path.join(libDir, 'lailatul-coder');
        ...
        it.skipIf(process.platform === 'win32')('throws when wrapper creation fails safety validation', () => {
            const libDir = path.join(tempDir, 'bad\npath', '.local', 'lib');
            const standaloneDir = path.join(libDir, 'lailatul-coder');
            fs.mkdirSync(standaloneDir, { recursive: true });
            expect(() => ensureBinWrapper(standaloneDir, 'linux-x64')).toThrow('Failed to create bin wrapper');
        });
    });
    describe('performStandaloneUpdate', () => {
        it('rejects invalid version format', async () => {
            ...
            await expect(performStandaloneUpdate(standaloneDir, 'not-a-version')).rejects.toThrow('Invalid version format');
        });
        it('rejects directory without manifest as non-managed install', async () => {
            const standaloneDir = path.join(tempDir, 'lailatul-coder');
            fs.mkdirSync(standaloneDir);
            // No manifest.json — could be user data
            await expect(performStandaloneUpdate(standaloneDir, '1.0.0')).rejects.toThrow('not a LailatulCoder Ai standalone install');
        });
        it('rejects unknown target in manifest', async () => {
            ...
            await expect(performStandaloneUpdate(standaloneDir, '1.0.0')).rejects.toThrow('Unknown target');
        });
        it('fails gracefully when another update is in progress', async () => {
            ...
            await expect(performStandaloneUpdate(standaloneDir, '1.0.0')).rejects.toThrow('Another update is already in progress');
            // Clean up lock
        ...
        it('rejects a stale lock when a Windows deferred swap is pending', async () => {
            ...
            await expect(performStandaloneUpdate(standaloneDir, '1.0.0')).rejects.toThrow('A previous update left a pending swap');
    ...
    describe('isSafeTarEntryPath', () => {
        it('allows double dots inside a filename segment', () => {
            expect(isSafeTarEntryPath('lailatul-coder/release..notes.md')).toBe(true);
            expect(isSafeTarEntryPath('lailatul-coder/node/lib/foo..bar')).toBe(true);
            expect(isSafeTarEntryPath('lailatul-coder/.../file.txt')).toBe(true);
            expect(isSafeTarEntryPath('./lailatul-coder/bin/qwen')).toBe(true);
        });
        it('rejects parent-directory segments and absolute paths', () => {
            ...
            expect(isSafeTarEntryPath('C:\\tmp\\lailatul-coder\\manifest.json')).toBe(false);
    ...
    describe('isSafeTarLinkTarget', () => {
        it('allows relative link targets inside the extraction directory', () => {
            ...
            expect(isSafeTarLinkTarget('lailatul-coder/bin/qwen', '../lib/cli.js', dest)).toBe(true);
            expect(isSafeTarLinkTarget('lailatul-coder/bin/qwen', './qwen', dest)).toBe(true);
        });
        it('allows symlink targets in child directories starting with two dots', () => {
            const dest = path.join(tempDir, 'extract');
            expect(isSafeTarLinkTarget('lailatul-coder/bin/qwen', '../..hidden/tool', dest)).toBe(true);
        });
        it('rejects symlink targets outside the extraction directory', () => {
            const dest = path.join(tempDir, 'extract');
            expect(isSafeTarLinkTarget('lailatul-coder/bin/qwen', '../../../etc/passwd', dest)).toBe(false);
            expect(isSafeTarLinkTarget('lailatul-coder/bin/qwen', '/etc/passwd', dest)).toBe(false);
            expect(isSafeTarLinkTarget('lailatul-coder/bin/qwen', 'C:\\Windows\\System32', dest)).toBe(false);
        });
        it('rejects symlink targets outside the archive root that will be installed', () => {
            const dest = path.join(tempDir, 'extract');
            expect(isSafeTarLinkTarget('lailatul-coder/bin/qwen', '../../shared/node', dest)).toBe(false);
            expect(isSafeTarLinkTarget('./lailatul-coder/bin/qwen', '../../shared/node', dest)).toBe(false);
        });
    });
    describe('isSafeTarEntry', () => {
        it('rejects hardlinks outright', () => {
            const dest = path.join(tempDir, 'extract');
            expect(isSafeTarEntry('lailatul-coder/bin/qwen', { type: 'Link', linkpath: '../poc.txt' }, dest)).toBe(false);
        });
        it('allows safe regular entries and safe symlinks', () => {
            const dest = path.join(tempDir, 'extract');
            expect(isSafeTarEntry('lailatul-coder/bin/qwen', { type: 'File' }, dest)).toBe(true);
            expect(isSafeTarEntry('lailatul-coder/lib', { type: 'Directory' }, dest)).toBe(true);
            expect(isSafeTarEntry('lailatul-coder/bin/qwen', { type: 'SymbolicLink', linkpath: '../lib/cli.js' }, dest)).toBe(true);
        });
        it('accepts fs stats entries from tar filter typing', () => {
            const dest = path.join(tempDir, 'extract');
            const filePath = path.join(tempDir, 'entry.txt');
            fs.writeFileSync(filePath, 'entry');
            expect(isSafeTarEntry('lailatul-coder/bin/qwen', fs.statSync(filePath), dest)).toBe(true);
        });
        it('rejects special archive entry types', () => {
            ...
            for (const type of [
                ...
                expect(isSafeTarEntry('lailatul-coder/bin/qwen', { type }, dest)).toBe(false);
            }
        });
    });
    describe('cleanupFirstTimeMigrationArtifacts', () => {
        it.skipIf(process.platform === 'win32')('removes the wrapper and PATH block created during a failed migration', () => {
            ...
            try {
                ...
                expect(fs.existsSync(wrapperPath)).toBe(true);
                expect(fs.readFileSync(bashrc, 'utf-8')).toContain('# LailatulCoder Ai PATH block begin');
                cleanupFirstTimeMigrationArtifacts(artifacts);
                expect(fs.existsSync(wrapperPath)).toBe(false);
                expect(fs.readFileSync(bashrc, 'utf-8')).not.toContain('# LailatulCoder Ai PATH block begin');
            }
            finally {
    ...
    describe('rollbackStandaloneUpdate — concurrent lock protection', () => {
        ...
        it('proceeds when lock has dead PID', () => {
            ...
            fs.mkdirSync(oldDir);
            fs.writeFileSync(path.join(standaloneDir, 'manifest.json'), JSON.stringify({ name: '@lailatul-coder/lailatul-coder', version: '0.17.0' }));
            fs.writeFileSync(path.join(oldDir, 'manifest.json'), JSON.stringify({ name: '@lailatul-coder/lailatul-coder', version: '0.16.0' }));
            fs.writeFileSync(lockPath, '999999999');
            const result = rollbackStandaloneUpdate(standaloneDir);
            expect(result.ok).toBe(true);
        });
    ...
    describe('acquireLock deferred marker handling', () => {
        it('rejects lock takeover while a deferred bat process is alive', () => {
            ...
            expect(() => acquireLock(lockPath, standaloneDir)).toThrow('A previous update is still being applied');
            expect(fs.existsSync(lockPath)).toBe(true);
    ...
    describe.skipIf(process.platform === 'win32')('ensurePathInShellRc', () => {
        it('appends PATH export to zshrc when SHELL is zsh', () => {
            ...
            try {
                ...
                expect(content).toContain('# LailatulCoder Ai PATH block begin');
                expect(content).toContain('# LailatulCoder Ai PATH block end');
        ...
        it('skips if block markers already in rc file', () => {
            const binDir = path.join(tempDir, 'bin');
            const zshrc = path.join(tempDir, '.zshrc');
            fs.writeFileSync(zshrc, `# LailatulCoder Ai PATH block begin\nexport PATH='${binDir}':$PATH\n# LailatulCoder Ai PATH block end\n`);
            const origShell = process.env['SHELL'];
            const origHome = process.env['HOME'];
            process.env['SHELL'] = '/bin/zsh';
            process.env['HOME'] = tempDir;
            try {
                ensurePathInShellRc(binDir);
                const content = fs.readFileSync(zshrc, 'utf-8');
                const matches = content.match(/# LailatulCoder Ai PATH block begin/g);
        ...
        it('skips if legacy marker already in rc file', () => {
            const binDir = path.join(tempDir, 'bin');
            const zshrc = path.join(tempDir, '.zshrc');
            fs.writeFileSync(zshrc, `# Added by LailatulCoder Ai standalone installer\nexport PATH="${binDir}:$PATH"\n`);
            const origShell = process.env['SHELL'];
            const origHome = process.env['HOME'];
            process.env['SHELL'] = '/bin/zsh';
            process.env['HOME'] = tempDir;
            try {
                ...
                expect(matches).toHaveLength(1);
                expect(content).not.toContain('# LailatulCoder Ai PATH block begin');
            }
            finally {
                process.env['SHELL'] = origShell;
                process.env['HOME'] = origHome;
        ...
        it('falls back to .bash_profile for bash when .bashrc is absent', () => {
            ...
            try {
                ensurePathInShellRc(binDir);
                expect(fs.readFileSync(profile, 'utf-8')).toContain('# LailatulCoder Ai PATH block begin');
        ...
        it('allows shell metacharacters in single-quoted PATH entries', () => {
            ...
            try {
                ensurePathInShellRc(binDir);
                expect(fs.readFileSync(zshrc, 'utf-8')).toContain(`export PATH='${binDir}':$PATH`);
            }
            finally {
                process.env['SHELL'] = origShell;
                process.env['HOME'] = origHome;
        ...
        it('rejects binDir with newlines', () => {
            ...
            process.env['HOME'] = tempDir;
            try {
                expect(() => ensurePathInShellRc(binDir)).toThrow('unsafe for shell embedding');
            }
            finally {
                process.env['SHELL'] = origShell;
                process.env['HOME'] = origHome;
        ...
        it('does nothing for unknown shells', () => {
            ...
            try {
                ...
                expect(fs.readdirSync(tempDir).filter((f) => f.startsWith('.'))).toHaveLength(0);
            }
            finally {
                process.env['SHELL'] = origShell;
                process.env['HOME'] = origHome;
...
//# sourceMappingURL=standalone-update.test.js.map