 ...
 */
import { addDaemonRequestAttribute, SessionService, SessionOrganizationError, Storage, readWorktreeSession, } from '@lailatul-coder/lailatul-coder-core';
...
function encodeOrganizedCursor(last, group, archiveState, sourceType, sourceId, emitted = []) {
...
async function loadAllPersistedSummaries(sessionService, archiveState, signal) {
    ...
    do {
        ...
        if (page.items.length > remaining ||
            ...
            writeStderrLine(`LailatulCoder serve: organized session list truncated at ${MAX_ORGANIZED_SESSIONS} sessions`);
            truncated = true;
            break;
        }
...
async function listAllPersistedSummaries(sessionService, workspaceCwd, archiveState, runtimeBaseDir, queryKind, signal) {
    ...
    addDaemonRequestAttribute('lailatul-coder.daemon.session_list.cache_status', lookup.status);
    addDaemonRequestAttribute('lailatul-coder.daemon.session_list.archive_state', archiveState);
    addDaemonRequestAttribute('lailatul-coder.daemon.session_list.query_kind', queryKind);
    if (lookup.cacheAgeMs !== undefined) {
        addDaemonRequestAttribute('lailatul-coder.daemon.session_list.cache_age_ms', lookup.cacheAgeMs);
    }
    const snapshot = await lookup.promise;
    signal?.throwIfAborted();
    addDaemonRequestAttribute('lailatul-coder.daemon.session_list.persisted_sessions', snapshot.sessions.length);
    addDaemonRequestAttribute('lailatul-coder.daemon.session_list.scan_pages', snapshot.scanPages);
    addDaemonRequestAttribute('lailatul-coder.daemon.session_list.truncated', snapshot.truncated);
    if (lookup.status !== 'cache_hit') {
        addDaemonRequestAttribute('lailatul-coder.daemon.session_list.scan_duration_ms', snapshot.scanDurationMs);
    }
    return snapshot;
}
...
async function listOrganizedWorkspaceSessionsForResponse(bridge, workspaceCwd, options, pageSize, readOptions) {
    ...
    const persistedTimeById = new Map(persisted.sessions.map((session) => [
        session.sessionId,
    ...
    if (readOptions.mergeLive !== false && archiveState !== 'archived') {
        try {
            ...
            writeStderrLine(`LailatulCoder serve: organized session list live merge failed; using persisted sessions only: ${error instanceof Error ? error.message : String(error)}`);
        }
    }
    ...
    const afterCursor = cursorKey === undefined
        ...
        !emittedBeforePage.has(session.sessionId) &&
            compareOrganizedCursorKeys(cursorKey, getOrganizedCursorKey(activityTimeById, session)) < 0);
    const page = afterCursor.slice(0, pageSize);
    let nextCursor;
    if (page.length < afterCursor.length) {
        const boundary = getOrganizedCursorKey(activityTimeById, page[page.length - 1]);
        const emitted = nextEmittedSessionIds({
            carried: emittedBeforePage,
            page,
            liveSessionIds,
            listedById: new Map(filtered.map((session) => [session.sessionId, session])),
            persistedTimeById,
            ...
            reenters: (row, persistedTime) => [true, false].some((isPinned) => compareOrganizedCursorKeys(boundary, {
                isPinned,
                activityTime: persistedTime,
                sessionId: row.sessionId,
            }) < 0),
        });
        nextCursor = encodeOrganizedCursor(boundary, group, archiveState, options.sourceType, options.sourceId, emitted);
    }
    return {
        sessions: page,
...
async function listWorkspaceSessionsByMetadataForResponse(bridge, workspaceCwd, options, pageSize, filter, readOptions) {
    ...
    if (readOptions.mergeLive !== false && archiveState !== 'archived') {
        try {
            ...
            readOptions.signal?.throwIfAborted();
            liveMergeFailed = true;
            writeStderrLine(`LailatulCoder serve: session metadata filter live merge failed; using persisted sessions only: ${error instanceof Error ? error.message : String(error)}`);
        }
    }
    ...
    const afterCursor = cursorKey === undefined
        ...
        !emittedBeforePage.has(session.sessionId) &&
            compareLiveSessionCursorKeys(cursorKey, getLiveSessionCursorKey(session)) < 0);
    const page = afterCursor.slice(0, pageSize);
    let nextCursor;
    if (page.length < afterCursor.length) {
        ...
        const emitted = nextEmittedSessionIds({
            ...
            page,
            liveSessionIds,
            listedById: new Map(matches.map((session) => [session.sessionId, session])),
            persistedTimeById,
            reenters: (row, persistedTime) => compareLiveSessionCursorKeys(boundary, {
                activityTime: persistedTime,
                sessionId: row.sessionId,
            }) < 0,
        });
        nextCursor = encodeMetadataSessionCursor(boundary, filter, archiveState, emitted);
    }
    return {
        sessions: page,
        ...(nextCursor !== undefined ? { nextCursor } : {}),
