...
import { classifyAwkCommandSafety, classifySedCommandSafety, hasShellPatternExpansion, } from './shell-safety-rules.js';
// ---------------------------------------------------------------------------
// Constants
// ---------------------------------------------------------------------------
/**
...
async function loadWasmBinary(dynamicImport, fallbackSpecifier) {
    const nativeFs = process.getBuiltinModule?.('fs') ?? fs;
    const moduleFilePath = fileURLToPath(import.meta.url);
    const isBundleMode = !moduleFilePath.includes(path.join('src', '')) &&
        !moduleFilePath.includes(path.join('dist', 'src', ''));
    try {
        if (isBundleMode) {
            // Bundle mode: esbuild replaces `?binary` imports with inline Uint8Array.
            const mod = await dynamicImport();
            const wasmBinary = mod.default;
...
const WRITE_ROOT_COMMAND = /^(chgrp|chmod|chown|cp|install|ln|mkdir|mkfifo|mknod|mv|rename|rm|rmdir|shred|touch|truncate|unlink)$/;
...
const WRITE_GIT_SUBCOMMAND = /^(add|am|checkout|cherry-pick|clean|clone|commit|fetch|gc|init|merge|mv|pull|push|rebase|reset|restore|revert|rm|stash|switch)$/;
/** git remote actions that mutate state. */
const WRITE_GIT_REMOTE_ACTION = /^(add|remove|rm|rename|set-branches|set-head|set-url|update)$/;
const GIT_EXTERNAL_HELPER_OPTION = /^--(?:ext-diff|filters|show-signature|textconv|open-files-in-pager)(?:=|$)/;
const GIT_COMMIT_VALUE_OPTION = /^(?:-[CcFmt]|--(?:author|cleanup|date|file|fixup|message|pathspec-from-file|reedit-message|reuse-message|squash|template|trailer))$/;
/** git branch flags that mutate state. */
const WRITE_GIT_BRANCH_FLAG = /^(?:-[cCdDmMu](?:.|$)|--(?:delete|move|copy|set-upstream(?:-to)?|unset-upstream|create-reflog|edit-description)(?:=|$))/;
const GIT_BRANCH_LIST_FLAG = /^(?:-[alr]|--(?:all|list|remotes|show-current|contains|no-contains|merged|no-merged|points-at))(?:=|$)/;
const BLOCKED_FIND_PREFIXES = ['-fls', '-fprint', '-fprintf'];
const FIND_VALUE_PREDICATE = /^-(?:[ac]?newer|newer[a-z]{2}|[acm](?:min|time)|context|fstype|gid|group|i?(?:lname|name|path|regex)|inum|links|maxdepth|mindepth|path|perm|printf|regextype|samefile|size|type|uid|used|user|wholename|xtype)$/;
const UNIQ_VALUE_OPTIONS = new Set('-f --skip-fields -s --skip-chars -w --check-chars'.split(' '));
/**
...
export async function initParser() {
    ...
    initPromise = (async () => {
        ...
        const { default: ParserClass } = (await import('web-tree-sitter'));
        const treeSitterWasm = await loadWasmBinary(() => import('web-tree-sitter/tree-sitter.wasm?binary'), 'web-tree-sitter/tree-sitter.wasm');
        await ParserClass.init({ wasmBinary: treeSitterWasm });
        const bashWasm = await loadWasmBinary(() => import('tree-sitter-wasms/out/tree-sitter-bash.wasm?binary'), 'tree-sitter-wasms/out/tree-sitter-bash.wasm');
...
const SHELL_EXPANSION_TYPES = new Set('simple_expansion expansion arithmetic_expansion'.split(' '));
const CHILD_STATEMENT = /^(?:pipeline|list|subshell|compound_statement|negated_command)$/;
/** Collect all descendant nodes of given types. */
function collectDescendants(node, types, outermostOnly = false) {
    const result = [];
    const stack = [node];
 ...
 */
function getArgumentNodes(commandNode) {
    const args = [];
...
function hasShellExpansion(node) {
    return (collectDescendants(node, SHELL_EXPANSION_TYPES).length > 0 ||
        (['word', 'concatenation'].includes(node.type) &&
            hasShellPatternExpansion(node.text)));
}
function mergeSafety(...results) {
    if (results.includes('write'))
        return 'write';
    if (results.includes('unknown'))
        return 'unknown';
    return 'read-only';
}
function beforeTerminator(args) {
    const end = args.indexOf('--');
    return args.slice(0, end < 0 ? args.length : end);
}
function hasHelp(args, valueOptions = []) {
    return beforeTerminator(args).some((arg, index, options) => /^(?:--help|--version)$/i.test(arg) &&
        !valueOptions.includes(options[index - 1]));
}
function withoutOptionValues(args, valueOption) {
    const result = [];
    for (let i = 0; i < args.length; i++) {
        result.push(args[i]);
        if (valueOption.test(args[i]))
            i++;
    }
    return result;
}
function evaluateOutputOption(args, long = true, short = true) {
    for (let i = 0; i < args.length; i++) {
        const arg = args[i];
        if (arg === '--')
            break;
        ...
        if (short && arg.startsWith('-o') && arg.length > 2)
            return 'write';
...
function evaluateGitSafety(args) {
    const first = args[0];
    if (!first || first === '--version')
        return 'read-only';
    if (first === '--help')
        return args.length === 1 ? 'read-only' : 'unknown';
    if (first.startsWith('-'))
        return 'unknown';
    const subcommand = first.toLowerCase();
    const rest = args.slice(1);
    const options = beforeTerminator(rest);
    const invokesHelper = options.some((arg) => GIT_EXTERNAL_HELPER_OPTION.test(arg)) ||
    ...
    if (WRITE_GIT_SUBCOMMAND.test(subcommand)) {
        const effectiveArgs = subcommand === 'commit'
        ...
        const dryRun = effectiveOptions.includes('--dry-run') ||
    ...
    if (subcommand === 'blame' &&
        ...
        return 'unknown';
    if (subcommand !== 'branch' && hasHelp(rest))
        return 'unknown';
    if (subcommand === 'remote') {
        const action = rest.find((arg) => !arg.startsWith('-'))?.toLowerCase();
        if (!action)
            return invokesHelper ? 'unknown' : 'read-only';
        if (['show', 'get-url'].includes(action))
            return rest.some((arg) => /^(?:add|remove|rm|rename|set-branches|set-head|set-url|update|prune)$/i.test(arg)) || invokesHelper
                ? 'unknown'
                : 'read-only';
        if (WRITE_GIT_REMOTE_ACTION.test(action))
            return 'write';
        if (action === 'prune')
...
function evaluateFindSafety(args) {
    let result = 'read-only';
    for (let i = 0; i < args.length; i++) {
        const lower = args[i].toLowerCase();
        if (lower === '--')
            return mergeSafety(result, 'unknown');
        if (/^--(?:help|version)$/.test(lower))
            return 'unknown';
...
function evaluateSedSafety(args) {
    return classifySedCommandSafety(args);
}
function evaluateAwkSafety(args) {
    return classifyAwkCommandSafety(args);
}
function evaluateUniqSafety(args) {
    let positional = 0;
    for (let i = 0; i < args.length; i++) {
        const arg = args[i];
        if (arg === '--') {
...
function processSafety(root, args) {
    ...
    if (args.length === 0 ||
        ...
        return 'unknown';
    if (options.some((arg, index) => (/[$`*?()[\]{}]/.test(arg) &&
        (arg.startsWith('-') ||
...
function evaluateSubstitutions(node) {
    const substitutions = collectDescendants(node, new Set(['command_substitution', 'process_substitution']), true);
    if (substitutions.length === 0) {
        for (const expansion of collectDescendants(node, new Set(['expansion']))) {
            for (let i = 0; i < expansion.childCount - 1; i++) {
                if (expansion.child(i)?.type === '@' &&
    ...
    return mergeSafety('unknown', ...substitutions
        .flatMap((substitution) => substitution.namedChildren)
        .map(evaluateStatementSafety));
}
function evaluateCommandSafety(commandNode) {
    ...
    if (!root)
        ...
        result = evaluateFindSafety(args);
    else if (root === 'sed')
        result = evaluateSedSafety(args);
    else if (root === 'awk')
        result = evaluateAwkSafety(args);
    else if (root === 'sort' || root === 'tree') {
        result = evaluateOutputOption(args, root === 'sort') ?? 'read-only';
        if (hasHelp(args, ['-o', '--output']))
            result = 'unknown';
        if (beforeTerminator(args).some((arg) => /^(?:--o|-[^-]+o)/.test(arg) ||
        ...
        const writesFile = args.some((arg, index) => !arg.startsWith('-') || args[index - 1] === '--');
        ...
        (['rg', 'ripgrep'].includes(root) &&
            beforeTerminator(args).some((arg) => /^(?:--(?:hostname-bin|pre)(?:=|$)|--search-zip$|-[^-]*z)/.test(arg)))) {
        result = 'unknown';
    }
    else {
    ...
    if (result === 'write' &&
        ...
        hasHelp(args))
        result = 'unknown';
    const hasEnvironment = commandNode.namedChildren.some((child) => child.type === 'variable_assignment');
    if (root && hasEnvironment)
        result = mergeSafety(result, 'unknown');
    return mergeSafety(result, evaluateRedirectionSafety(commandNode), ...commandNode.namedChildren
        .filter((child) => !child.type.endsWith('_redirect'))
        .map(evaluateSubstitutions));
}
function evaluateRedirectionSafety(node) {
    let result = 'read-only';
    for (const redirect of node.namedChildren) {
        if (!redirect.type.endsWith('_redirect'))
            continue;
        result = mergeSafety(result, evaluateSubstitutions(redirect));
        if (redirect.type !== 'file_redirect')
            continue;
        const operator = redirect.children.find((child) => child.type !== 'file_descriptor');
        if (!operator)
            return 'unknown';
        if (WRITE_REDIRECT_OPERATORS.has(operator.type))
            return 'write';
        if (operator.type === '>&') {
            ...
            if (/^(?:\d+|-)$/.test(target))
                continue;
            result = mergeSafety(result, /[$`*?()[\]{}]/.test(target) ? 'unknown' : 'write');
        }
    }
    return result;
}
function childrenSafety(node, floor = 'read-only') {
    return mergeSafety(floor, ...node.namedChildren.map(evaluateStatementSafety));
}
function evaluateStatementSafety(node) {
    if (node.type === 'command')
        return evaluateCommandSafety(node);
    if (CHILD_STATEMENT.test(node.type))
        return childrenSafety(node);
    if (node.type === 'redirected_statement')
        return mergeSafety(...node.namedChildren
            .filter((child) => !child.type.endsWith('_redirect'))
            .map((child) => evaluateStatementSafety(child)), evaluateRedirectionSafety(node));
    if (/^variable_assignments?$/.test(node.type))
        return mergeSafety(node.parent?.namedChildCount === 1 ? 'read-only' : 'unknown', evaluateSubstitutions(node));
    if (node.type === 'function_definition')
        return 'unknown';
    return childrenSafety(node, 'unknown');
}
function localGitConfigMakesCommandUnsafe(root, cwd) {
    ...
    for (const command of collectDescendants(root, new Set(['command']))) {
        ...
        if (name !== 'git')
            continue;
        const subcommand = stripOuterQuotes(getArgumentNodes(command)[0]?.text ?? '').toLowerCase();
    ...
    if (!usesDiff && !usesStatus)
        return false;
    const risk = getLocalGitConfigRisk(cwd);
    return (usesDiff && risk.diffExternal) || (usesStatus && risk.fsmonitor);
}
function fallbackGitConfigMakesCommandUnsafe(command, cwd) {
    if (/\b(?:cd|pushd)\b[\s\S]*\bgit\b/i.test(command))
        return true;
...
async function classifyInternal(command, cwd) {
    const tree = await parseShellCommand(command);
    try {
        const root = tree.rootNode;
        if (root.namedChildCount === 0 || root.hasError)
            return 'unknown';
        const safety = mergeSafety(...root.namedChildren.map(evaluateStatementSafety));
        if (safety === 'read-only' && command.includes('\\\n')) {
            const normalizedSafety = await classifyInternal(command.replaceAll('\\\n', ''), cwd);
            if (normalizedSafety !== 'read-only')
                return 'unknown';
        }
        if (safety !== 'read-only' || !cwd)
            return safety;
...
export async function classifyShellCommandSafety(command) {
    if (typeof command !== 'string' || !command.trim())
        return 'unknown';
    return classifyInternal(command).catch(() => 'unknown');
}
export async function classifyShellCommandSafetyInDirectory(command, cwd) {
    if (typeof command !== 'string' || !command.trim())
        return 'unknown';
 ...
 */
export async function isShellCommandReadOnlyAST(command) {
    return isShellCommandReadOnlyInternal(command);
}
export async function isShellCommandReadOnlyASTInDirectory(command, cwd) {
    return isShellCommandReadOnlyInternal(command, cwd);
}
async function isShellCommandReadOnlyInternal(command, cwd) {
    if (typeof command !== 'string' || !command.trim())
        return false;
 ...
 */
function extractRuleFromCommand(commandNode) {
 ...
 */
function extractRulesFromStatement(node) {
    switch (node.type) {
        case 'command':
            return [extractRuleFromCommand(node)].filter(Boolean);
        case 'pipeline':
        case 'list':
 ...
 * // → ['docker compose up *']
 */
export async function extractCommandRules(command) {
    if (typeof command !== 'string' || !command.trim())
        return [];
    const tree = await parseShellCommand(command);
    const root = tree.rootNode;
 ...
 * @internal
 */
export function _setParserFailedForTesting() {
    parserInitFailed = true;
    initPromise = null;
    if (parserInstance) {
        parserInstance.delete();
...
//# sourceMappingURL=shellAstParser.js.map