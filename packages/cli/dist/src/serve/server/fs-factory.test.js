...
import { parseNewFileModePolicy, resolveBridgeFsFactory, resolveBoundWorkspacesFromIdeEnv, } from './fs-factory.js';
...
async function mkdirs(scratch, ...names) {
    const out = {};
...
describe('resolveBoundWorkspacesFromIdeEnv', () => {
    it('keeps the selected workspace first for legacy and JSON encoded roots', async () => {
        ...
        expect(resolveBoundWorkspacesFromIdeEnv(dirs.second, [dirs.first, dirs.second].join(path.delimiter))).toEqual([
            realpathSync.native(dirs.second),
            realpathSync.native(dirs.first),
        ]);
        expect(resolveBoundWorkspacesFromIdeEnv(dirs.second, JSON.stringify([dirs.second, withDelimiter]))).toEqual([
            realpathSync.native(dirs.second),
    ...
    it('falls back to the primary workspace for stale or malformed IDE env', async () => {
        ...
        expect(resolveBoundWorkspacesFromIdeEnv(dirs.primary, '[not json')).toEqual([primary]);
        expect(resolveBoundWorkspacesFromIdeEnv(dirs.primary, JSON.stringify([1, 2, 3]))).toEqual([primary]);
        expect(resolveBoundWorkspacesFromIdeEnv(dirs.primary, JSON.stringify(['relative']))).toEqual([primary]);
    });
    it('keeps valid sibling roots when one env workspace fails canonicalization', async () => {
        ...
        try {
            expect(resolveBoundWorkspacesFromIdeEnv(dirs.primary, JSON.stringify([dirs.primary, dirs.blocked, dirs.sibling]))).toEqual([dirs.primary, dirs.sibling]);
    ...
    it('drops relative delimiter entries before canonicalization', async () => {
        ...
        expect(resolveBoundWorkspacesFromIdeEnv(dirs.primary, [dirs.primary, 'relative', dirs.secondary].join(path.delimiter))).toEqual([
    ...
    it('drops env parents without losing sibling roots', async () => {
        ...
        expect(resolveBoundWorkspacesFromIdeEnv(primary, [parent, sibling].join(path.delimiter))).toEqual([realpathSync.native(primary), realpathSync.native(sibling)]);
    });
    it('drops nested non-primary roots', async () => {
        ...
        expect(resolveBoundWorkspacesFromIdeEnv(primary, [primary, parent, child].join(path.delimiter))).toEqual([realpathSync.native(primary), realpathSync.native(parent)]);
    });
});
describe('parseNewFileModePolicy (QWEN_SERVE_NEW_FILE_MODE)', () => {
    ...
    it('defaults to owner when unset or empty', () => {
        ...
        expect(parseNewFileModePolicy({ QWEN_SERVE_NEW_FILE_MODE: '' })).toBe('owner');
        expect(parseNewFileModePolicy({ QWEN_SERVE_NEW_FILE_MODE: '   ' })).toBe('owner');
        expect(mockWriteStderrLine).not.toHaveBeenCalled();
    });
    it('accepts explicit owner spellings', () => {
        expect(parseNewFileModePolicy({ QWEN_SERVE_NEW_FILE_MODE: 'owner' })).toBe('owner');
        expect(parseNewFileModePolicy({ QWEN_SERVE_NEW_FILE_MODE: '0600' })).toBe('owner');
        expect(parseNewFileModePolicy({ QWEN_SERVE_NEW_FILE_MODE: ' OWNER ' })).toBe('owner');
        expect(mockWriteStderrLine).not.toHaveBeenCalled();
    });
    it('accepts system case-insensitively with surrounding whitespace', () => {
        expect(parseNewFileModePolicy({ QWEN_SERVE_NEW_FILE_MODE: 'system' })).toBe('system');
        expect(parseNewFileModePolicy({ QWEN_SERVE_NEW_FILE_MODE: ' System ' })).toBe('system');
        expect(mockWriteStderrLine).not.toHaveBeenCalled();
    });
    it('rejects unknown values with a warning and keeps the 0600 default', () => {
        expect(parseNewFileModePolicy({ QWEN_SERVE_NEW_FILE_MODE: '0644' })).toBe('owner');
        expect(parseNewFileModePolicy({ QWEN_SERVE_NEW_FILE_MODE: 'everyone' })).toBe('owner');
        expect(mockWriteStderrLine).toHaveBeenCalledTimes(2);
        expect(mockWriteStderrLine.mock.calls[0]?.[0]).toContain('QWEN_SERVE_NEW_FILE_MODE');
...
//# sourceMappingURL=fs-factory.test.js.map