...
function makeSettings(workspaceDir, opts = {}) {
    const { keytermsFile, systemKeytermsFile, workspaceKeytermsFile, isTrusted = true, } = opts;
...
describe('buildVoiceKeyterms', () => {
    ...
    describe('custom keyterms file', () => {
        ...
        it('auto-loads .qwen/voice-keyterms.txt and merges with the globals', () => {
            fs.writeFileSync(path.join(qwenDir, 'voice-keyterms.txt'), 'Kubernetes\nGraphQL\n');
        ...
        it('ignores blank lines and whole-line "#" comments', () => {
            fs.writeFileSync(path.join(qwenDir, 'voice-keyterms.txt'), '# project terms\n\n  Kubernetes # container orchestration  \nIssue #42\nPR #5817\nC#\nF#\n   # indented comment\n');
            const terms = buildVoiceKeyterms(makeSettings(workspaceDir));
        ...
        it('honors an explicit absolute keytermsFile over auto-discovery', () => {
            ...
            const terms = buildVoiceKeyterms(makeSettings(workspaceDir, { keytermsFile: explicit }));
        ...
        it('honors a user-scoped absolute keytermsFile outside the workspace', () => {
            const outsideDir = fs.mkdtempSync(path.join(os.tmpdir(), 'voice-keyterms-outside-'));
        ...
        it('honors an explicit relative keytermsFile over auto-discovery', () => {
            ...
            const terms = buildVoiceKeyterms(makeSettings(workspaceDir, { keytermsFile: 'rel.txt' }));
        ...
        it('resolves a relative keytermsFile from the workspace root', () => {
            ...
            const terms = buildVoiceKeyterms(makeSettings(workspaceDir, { keytermsFile: 'terms.txt' }));
        ...
        it('honors a system-scoped keytermsFile setting', () => {
            ...
            const terms = buildVoiceKeyterms(makeSettings(workspaceDir, { systemKeytermsFile: systemFile }));
        ...
        it('does not read a system-scoped absolute keytermsFile outside the workspace', () => {
            ...
            try {
                const terms = buildVoiceKeyterms(makeSettings(workspaceDir, { systemKeytermsFile: secret }));
        ...
        it('dedupes case-insensitively and keeps the global casing', () => {
            fs.writeFileSync(path.join(qwenDir, 'voice-keyterms.txt'), 'typescript\nKubernetes\n');
            const terms = buildVoiceKeyterms(makeSettings(workspaceDir));
            expect(terms).toContain('TypeScript');
            expect(terms).not.toContain('typescript');
            expect(terms.filter((t) => t.toLowerCase() === 'typescript')).toHaveLength(1);
        });
        it('falls back to the globals when the file is missing', () => {
            const terms = buildVoiceKeyterms(makeSettings(workspaceDir, { keytermsFile: 'does-not-exist.txt' }));
            expect(terms).toContain('TypeScript');
            expect(terms).toContain('worktree');
        });
        ...
        it('caps by total byte length for a file of few long terms', () => {
            ...
            const long = Array.from({ length: 40 }, (_, i) => `t${i}_${'x'.repeat(76)}`).join('\n');
        ...
        it('caps non-ASCII keyterms by UTF-8 byte length', () => {
            const cjkTerms = Array.from({ length: 80 }, (_, i) => `术语${i}_${'测'.repeat(10)}`).join('\n');
        ...
        it('skips over a term that exceeds the remaining byte budget', () => {
            fs.writeFileSync(path.join(qwenDir, 'voice-keyterms.txt'), `${'x'.repeat(2200)}\nShortTerm\n`);
            const terms = buildVoiceKeyterms(makeSettings(workspaceDir));
            expect(terms).toContain('ShortTerm');
            expect(terms.join(' ').length).toBeLessThanOrEqual(2000);
        });
        it('does not read a keyterms file in an untrusted workspace', () => {
            fs.writeFileSync(path.join(qwenDir, 'voice-keyterms.txt'), 'ShouldNotLoad\n');
            const terms = buildVoiceKeyterms(makeSettings(workspaceDir, { isTrusted: false }));
        ...
        it('does not follow a symlinked explicit keytermsFile', () => {
            ...
            const terms = buildVoiceKeyterms(makeSettings(workspaceDir, { keytermsFile: link }));
            expect(terms).not.toContain('SECRETKEYMATERIAL');
            expect(terms).toContain('TypeScript'); // globals only
        });
        it('does not load the default file through a symlinked .qwen directory', () => {
            ...
            fs.writeFileSync(path.join(outsideDir, 'voice-keyterms.txt'), 'SECRETKEYMATERIAL\n');
        ...
        it('does not let relative keytermsFile escape the workspace root', () => {
            ...
            try {
                const relativeEscape = path.relative(workspaceDir, path.join(outsideDir, 'secret.txt'));
                const terms = buildVoiceKeyterms(makeSettings(workspaceDir, { keytermsFile: relativeEscape }));
        ...
        it('ignores workspace-scoped keytermsFile settings', () => {
            const secret = path.join(workspaceDir, 'secret.txt');
            fs.writeFileSync(secret, 'SECRETKEYMATERIAL\n');
            const terms = buildVoiceKeyterms(makeSettings(workspaceDir, { workspaceKeytermsFile: secret }));
            expect(terms).not.toContain('SECRETKEYMATERIAL');
            expect(terms).toContain('TypeScript'); // globals only
        });
...
//# sourceMappingURL=voice-keyterms.test.js.map