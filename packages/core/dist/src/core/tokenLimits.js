export const DEFAULT_TOKEN_LIMIT = 200_000; // 200K tokens
export const DEFAULT_OUTPUT_TOKEN_LIMIT = 32_000; // 32K tokens
 ...
 * past it.
 */
export const OUTPUT_TOKEN_CEILING = ESCALATED_MAX_TOKENS;
/**
 ...
 */
export const MIN_CLAMPED_OUTPUT_TOKENS = 4_000;
/**
 ...
 */
export function outputClampMargin(contextWindowSize) {
 ...
 */
export function clampOutputTokensToWindow(outputCeiling, contextWindowSize, promptTokens) {
...
export function parsePositiveIntegerEnvValue(raw) {
    if (raw === undefined)
        return undefined;
...
export function normalize(model) {
    ...
    s = s.replace(/:(?:free|beta|extended|thinking|online|nitro|floor|latest|\d+(?:\.\d+)?(?:x\d+)?b(?:-[\w.]+)*)$/, '');
    s = s.split(':').pop() ?? s;
...
const PATTERNS = [
    ...
    [/^glm-5(\.[01])?(-|$)/, 202_752], // GLM-5 / 5.0 / 5.1: 200K
    [/^glm-(?:[5-9]|\d{2,})/, LIMITS['1m']], // GLM-5.2+, 6.x..9.x, 10.x+: 1M
    [/^glm-/, 202_752], // GLM <=4.x / non-numeric fallback: 200K
    // -------------------
    // MiniMax
    // -------------------
...
const OUTPUT_PATTERNS = [
    ...
    // Anthropic Claude
    [CLAUDE_OPUS_EXTENDED, 128_000], // Opus 4.6-4.8, Opus 5.x: 128K
 ...
 */
export function hasExplicitOutputLimit(model) {
    const norm = normalize(model);
    return OUTPUT_PATTERNS.some(([regex]) => regex.test(norm));
}
export function knownTokenLimit(model, type = 'input') {
 ...
 */
export function tokenLimit(model, type = 'input') {
 ...
 */
export function defaultOutputCeiling(model) {
 ...
 */
export function reconcileMaxTokens(configMaxTokens, requestMaxTokens) {
    if (typeof configMaxTokens === 'number' &&
