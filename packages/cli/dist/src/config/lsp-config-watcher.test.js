...
describe('LspConfigWatcher', () => {
    ...
    it('notifies on create and suppresses formatting-only changes', async () => {
        ...
        fs.writeFileSync(path.join(dir, '.lsp.json'), '{"typescript":{"command":"tsserver"}}');
        await watcher.handleChange();
        expect(listener).toHaveBeenCalledWith({
            path: path.join(dir, '.lsp.json'),
            changeType: 'created',
        });
        expect(debugLoggerMock.info).toHaveBeenCalledWith(`LSP config changed: created ${path.join(dir, '.lsp.json')}`);
        fs.writeFileSync(path.join(dir, '.lsp.json'), JSON.stringify({ typescript: { command: 'tsserver' } }, null, 2));
        await watcher.handleChange();
    ...
    it('reports read errors separately from invalid JSON', async () => {
        ...
        expect(debugLoggerMock.warn).toHaveBeenCalledWith('Failed to read .lsp.json:', expect.objectContaining({ code: 'EISDIR' }));
    });
    ...
    it('allows retrying startWatching after watcher initialization fails', async () => {
        ...
        expect(debugLoggerMock.warn).toHaveBeenCalledWith(`Failed to start LSP config watcher for ${dir}:`, expect.any(Error));
        await watcher.stopWatching();
    ...
    it('ignores filesystem events after stopWatching begins', async () => {
        ...
        const configPath = path.join(dir, '.lsp.json');
        let resolveClose;
        chokidarMock.watcher.close.mockImplementationOnce(() => new Promise((resolve) => {
    ...
    it('unrefs the debounce timer', async () => {
        ...
        expect(onAll).toBeDefined();
        onAll?.('change', configPath);
        const refreshTimer = watcher.refreshTimer;
    ...
    it('waits for an active listener before stopping', async () => {
        ...
        const configPath = path.join(dir, '.lsp.json');
        let resolveListener;
        const listener = vi.fn(() => new Promise((resolve) => {
    ...
    it('times out a hanging listener without throwing', async () => {
        ...
        const dir = makeTempDir();
        const watcher = new LspConfigWatcher(dir);
        watcher.listener = vi.fn(() => new Promise(() => { }));
        ...
        expect(debugLoggerMock.warn).toHaveBeenCalledWith('LSP config change listener error:', expect.any(Error));
    });
    it('swallows listener rejection that arrives after timeout', async () => {
        ...
        const watcher = new LspConfigWatcher(dir);
        let rejectListener;
        watcher.listener = vi.fn(() => new Promise((_resolve, reject) => {
...
//# sourceMappingURL=lsp-config-watcher.test.js.map