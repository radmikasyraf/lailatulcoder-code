...
import { createInitialMessageDisplayState, stepMessageDisplay, MESSAGE_DISPLAY_DEBOUNCE_MS, } from './message-display-buffer.js';
/**
...
export class MessageDisplayDispatcher {
     ...
     */
    constructor(messageBus, signal, warn, nowMs = Date.now()) {
     ...
     */
    addChunk(chunk, nowMs = Date.now()) {
        if (this.finished) {
            return;
        }
        const step = stepMessageDisplay(this.state, chunk, nowMs, MESSAGE_DISPLAY_DEBOUNCE_MS, false);
    ...
    dispatch(displayedText, isFinal) {
        return this.messageBus
            ...
            this.emitWarning(`MessageDisplay hook failed [${this.messageId}]: ${err}`);
        });
    }
    /**
    ...
    drainWithTimeout() {
        ...
        this.drain ??= new Promise((resolve) => {
            const timer = setTimeout(() => {
                this.emitWarning(`MessageDisplay hook [${this.messageId}] still running after ` +
...
//# sourceMappingURL=message-display-dispatcher.js.map