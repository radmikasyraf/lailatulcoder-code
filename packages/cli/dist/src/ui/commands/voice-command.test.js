...
function createSettings(merged, setValue = vi.fn(), options = {}) {
    return {
        merged,
        isTrusted: false,
        workspace: { settings: {} },
...
describe('voice-command', () => {
    ...
    it('prompts for a voice model before enabling', async () => {
        ...
        expect(result).toEqual({
            type: 'message',
            messageType: 'warning',
            content: 'No voice model selected. Run /model --voice to choose one before enabling voice dictation.',
        });
    ...
    it('enables hold-mode voice dictation when a voice model is selected', async () => {
        ...
        const result = await voiceCommand.action(context, '');
        expect(setValue).toHaveBeenCalledWith(SettingScope.User, 'general.voice.mode', 'hold');
        expect(setValue).toHaveBeenCalledWith(SettingScope.User, 'general.voice.enabled', true);
        expect(result).toEqual({
            type: 'message',
            messageType: 'info',
            content: 'Voice dictation enabled (hold mode). Hold Space at an empty prompt to dictate with qwen3-asr-flash.',
        });
    });
    it('toggles voice dictation off when bare /voice is used while enabled', async () => {
        ...
        const context = createMockCommandContext({
            services: {
                settings: createSettings({ general: { voice: { enabled: true, mode: 'tap' } } }, setValue),
            },
        });
        const result = await voiceCommand.action(context, '');
        expect(setValue).toHaveBeenCalledTimes(1);
        expect(setValue).toHaveBeenCalledWith(SettingScope.User, 'general.voice.enabled', false);
        expect(result).toEqual({
            type: 'message',
            messageType: 'info',
    ...
    it('enables tap mode when /voice tap is used', async () => {
        ...
        expect(setValue).toHaveBeenCalledWith(SettingScope.User, 'general.voice.mode', 'tap');
        expect(setValue).toHaveBeenCalledWith(SettingScope.User, 'general.voice.enabled', true);
        expect(result).toEqual({
            type: 'message',
            messageType: 'info',
            content: 'Voice dictation enabled (tap mode). Tap Space at an empty prompt to start, tap again or pause to stop and submit, using qwen3-asr-flash.',
        });
    });
    ...
    it('writes to workspace scope when workspace owns voice enabled', async () => {
        ...
        expect(setValue).toHaveBeenCalledWith(SettingScope.Workspace, 'general.voice.enabled', true);
        expect(setValue).toHaveBeenCalledWith(SettingScope.Workspace, 'general.voice.mode', 'hold');
    });
...
//# sourceMappingURL=voice-command.test.js.map