...
import { resolveAndValidatePath, formatDisplayPath, resolvePath, unescapePath, } from '../utils/paths.js';
import { getErrorMessage } from '../utils/errors.js';
import { runRipgrep } from '../utils/ripgrepUtils.js';
import { SchemaValidator } from '../utils/schemaValidator.js';
import { DEFAULT_FILE_FILTERING_OPTIONS } from '../config/constants.js';
import { createDebugLogger } from '../utils/debugLogger.js';
import { getQwenIgnoreFileNames, QwenIgnoreParser, } from '../utils/qwenIgnoreParser.js';
...
const RIPGREP_INCOMPLETE_NOTICE = 'Search did not complete: the results above may not include all matches.';
...
function getRipgrepJsonPath(match) {
...
const qwenIgnoreCache = new Map();
const RIPGREP_CACHE_MAX = 256;
function trimCache(m) {
    if (m.size <= RIPGREP_CACHE_MAX)
        return;
    const oldest = m.keys().next().value;
    if (oldest !== undefined)
        m.delete(oldest);
}
function toAbsoluteResultPath(filePath, searchPaths, cache) {
...
function isQwenIgnoreFileName(ignoreFileName) {
    return ignoreFileName === '.lailatulcoderignore';
}
/**
 * Test-only: clear ripGrep's module-level discovery caches between cases.
 */
export function _resetRipGrepCachesForTest() {
...
class GrepToolInvocation extends BaseToolInvocation {
    ...
    async getDefaultPermission() {
        ...
        const resolvedPath = resolvePath(this.config.getTargetDir(), this.params.path);
    ...
    async execute(signal) {
        try {
            ...
            if (this.params.path) {
                ...
                const searchDirAbs = resolveAndValidatePath(this.config, this.params.path, { allowFiles: true, allowExternalPaths: true });
            ...
            if (!rawOutput.trim()) {
                if (searchResult.incomplete) {
                    const incompleteMsg = this.buildIncompleteSearchMessage(searchLocationDescription, filterDescription, searchResult.error);
                    return {
            ...
            if (filteringOptions.respectQwenIgnore) {
                allLines = this.filterQwenIgnoredMatches(allLines, searchPaths, resolvedPathCache, filteringOptions.customIgnoreFiles);
            }
            ...
            const resultFilePaths = Array.from(new Set(visibleLines.map((line) => toAbsoluteResultPath(line.filePath, searchPaths, resolvedPathCache))));
    ...
    filterQwenIgnoredMatches(lines, searchPaths, resolvedPathCache, customIgnoreFiles) {
        const parsers = new Map();
        return lines.filter((line) => {
            const absolutePath = toAbsoluteResultPath(line.filePath, searchPaths, resolvedPathCache);
    ...
    async performRipgrepSearch(options) {
        ...
        if (filteringOptions.respectQwenIgnore) {
            ...
            const seenIgnoreFiles = new Set();
            // Pass .lailatulcoderignore last so custom ignore negations cannot override it.
            const nonQwenIgnorePaths = [];
            const qwenIgnorePathsForRipgrep = [];
            const ignoreFileNames = getQwenIgnoreFileNames(filteringOptions.customIgnoreFiles);
            ...
            for (const qwenIgnorePath of [
                ...nonQwenIgnorePaths,
                ...lailatulcoderIgnorePathsForRipgrep,
            ]) {
        ...
        rgArgs.push(...paths);
        const result = await runRipgrep(rgArgs, options.signal, this.config.getUseBuiltinRipgrep());
    ...
    buildIncompleteSearchMessage(searchLocationDescription, filterDescription, error) {
    ...
    logRipgrepRuntimeRecovery(result) {
        ...
        logRipgrepRuntimeRecovery(this.config, new RipgrepRuntimeRecoveryEvent(eventParams));
    }
    getIgnoreRootForSearchPath(searchPath) {
    ...
    getFileFilteringOptions() {
        ...
        return {
            ...
            respectQwenIgnore: options?.respectQwenIgnore ??
                DEFAULT_FILE_FILTERING_OPTIONS.respectQwenIgnore,
            customIgnoreFiles: options?.customIgnoreFiles ??
    ...
    getDescription() {
        ...
        if (this.params.path) {
            const displayPath = formatDisplayPath(this.params.path, this.config.getTargetDir());
