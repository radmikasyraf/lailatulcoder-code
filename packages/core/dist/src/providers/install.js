...
export async function applyProviderInstallPlan(plan, options) {
    const { settings, reloadModelProviders, syncAuthState, refreshAuth, doRefreshAuth = true, } = options;
    ...
    try {
        ...
        for (const [key, value] of Object.entries(plan.env ?? {})) {
            if (DENY_ENV_KEYS.has(key.toUpperCase())) {
                throw new Error(`Install plan must not set reserved environment variable: ${key}`);
            }
        ...
        if (shadowedEnvKeys.length > 0) {
            ...
            console.error(`[auth] Warning: ${shadowedEnvKeys.join(', ')} ${shadowedEnvKeys.length === 1 ? 'is' : 'are'} also set in your shell environment or .env file. ` +
        ...
        for (const patch of plan.modelProviders ?? []) {
            ...
            settings.setValue(`modelProviders.${patch.authType}`, updatedModelProviders[patch.authType] ?? []);
        }
        // Set auth type
        ...
        if (plan.legacyCredentials?.baseUrl != null) {
            settings.setValue('security.auth.baseUrl', plan.legacyCredentials.baseUrl);
        }
        // Model selection
        ...
        if (effectiveModelSelection?.modelId) {
            ...
            const planOffersCurrentModel = typeof currentModelId === 'string' &&
                currentModelId.length > 0 &&
                (plan.modelProviders ?? []).some((patch) => patch.models.some((model) => currentBaseUrl === '' || currentBaseUrl === undefined
                    ? model.id === currentModelId
                    : isSameModelIdentity({ id: currentModelId, baseUrl: currentBaseUrl }, model)));
        ...
        if (effectiveModelSelection?.modelId) {
            ...
            syncAuthState?.(plan.authType, effectiveModelSelection.modelId, effectiveModelSelection.baseUrl);
        }
        ...
        try {
            ...
            console.error('[applyProviderInstallPlan] settings.restore failed during rollback:', restoreErr);
        }
        try {
        ...
        try {
            ...
            // eslint-disable-next-line no-console -- best-effort rollback path
            console.error('[applyProviderInstallPlan] reloadModelProviders failed during rollback:', reloadErr);
        }
