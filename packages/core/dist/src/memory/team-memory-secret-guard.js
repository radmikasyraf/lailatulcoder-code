 ...
 */
export function checkTeamMemorySecrets(filePath, content, projectRoot) {
    ...
    debugLogger.debug(`Blocked team-memory write; matched rules: ${matches
        .map((m) => m.ruleId)
        .join(', ')}`);
    const labels = matches.map((m) => m.label).join(', ');
    return (`Content contains potential secrets (${labels}) and cannot be written to ` +
        `team memory. Team memory is shared with all repository collaborators. ` +
        `Remove the sensitive content and try again.`);
}
//# sourceMappingURL=team-memory-secret-guard.js.map