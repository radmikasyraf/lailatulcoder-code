 ...
 * domain-style segment by convention, but `LailatulCoder` is distinctive enough that
 * we use the shorter bare form `_qwen/…`. Vendor data on standard messages
 * goes under `_meta` keyed by the same name (`_meta: { "LailatulCoder": … }`).
 */
export const QWEN_METHOD_NS = '_qwen/';
/** Key for vendor `_meta` blocks (capabilities + per-message data). */
export const QWEN_META_KEY = 'LailatulCoder';
...
const LOG_SAFE_RE = new RegExp(String.raw `[\x00-\x1f\x7f-\x9f\u200b-\u200f\u2028-\u202e\u2066-\u2069\ufeff]`, 'g');
/**
