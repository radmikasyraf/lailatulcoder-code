...
import { atomicWriteFileSync, FatalConfigError, ideContextStore, } from '@lailatul-coder/lailatul-coder-core';
...
import { loadTrustedFolders, getTrustedFoldersPath, saveTrustedFolders, TrustLevel, isWorkspaceTrusted, getWorkspaceTrustStatus, resetTrustedFoldersForTesting, } from './trustedFolders.js';
...
describe('Trusted Folders Loading', () => {
    ...
    it('setValue should update the user config and save it', () => {
        ...
        expect(atomicWriteFileSync).toHaveBeenCalledWith(getTrustedFoldersPath(), JSON.stringify({ '/new/path': TrustLevel.TRUST_FOLDER }, null, 2), 
    ...
    it('setValue re-reads under the lock and preserves concurrent disk rules', () => {
        ...
        mockFsExistsSync.mockImplementation((p) => p === userPath || p === dirPath);
        fs.readFileSync
            .mockReturnValueOnce('{}')
            .mockReturnValueOnce(JSON.stringify({ '/concurrent/path': TrustLevel.DO_NOT_TRUST }));
        const loadedFolders = loadTrustedFolders();
        loadedFolders.setValue('/new/path', TrustLevel.TRUST_FOLDER);
        ...
        expect(vi.mocked(atomicWriteFileSync).mock.calls[0]?.[1]).toContain('"/concurrent/path": "DO_NOT_TRUST"');
    });
    it('setValue does not mutate memory when the atomic write fails', () => {
        ...
        expect(() => loadedFolders.setValue('/new/path', TrustLevel.TRUST_FOLDER)).toThrow('disk full');
        expect(loadedFolders.user.config).toEqual({});
    });
  ...
  "/keep/path": "TRUST_FOLDER"
    ...
    it('saveTrustedFolders registers a lock-compromised handler', () => {
        ...
        expect(() => options?.onCompromised?.(new Error('lock lost'))).not.toThrow();
    });
  ...
  "/existing/path": "TRUST_FOLDER"
    ...
    it.each(['"hello"', '42', 'true', 'null'])('saveTrustedFolders should reject an existing top-level primitive: %s', (existingContent) => {
        const userPath = getTrustedFoldersPath();
        const dirPath = path.dirname(userPath);
        mockFsExistsSync.mockImplementation((p) => p === userPath || p === dirPath);
        fs.readFileSync.mockImplementation((p) => {
...
describe('isWorkspaceTrusted', () => {
    ...
    it('should match distrust rules through canonical symlink paths', () => {
        ...
        expect(isWorkspaceTrusted(mockSettings, undefined, '/real/project')).toEqual({
            isTrusted: false,
            source: 'file',
        });
    });
...
describe('getWorkspaceTrustStatus', () => {
    ...
    it('uses explicit workspace path instead of process cwd', () => {
        ...
        mockRules['/home/user/projectA'] = TrustLevel.TRUST_FOLDER;
        expect(getWorkspaceTrustStatus(mockSettings, '/home/user/projectA')).toMatchObject({
    ...
    it('reports trust levels inherited from matching parent rules', () => {
        ...
        expect(getWorkspaceTrustStatus(mockSettings, '/home/user/projectA/subdir')).toMatchObject({
            effective: { state: 'trusted', source: 'file' },
            explicitTrustLevel: TrustLevel.TRUST_FOLDER,
        });
        expect(getWorkspaceTrustStatus(mockSettings, '/home/user/projectB/sibling')).toMatchObject({
            effective: { state: 'trusted', source: 'file' },
            explicitTrustLevel: TrustLevel.TRUST_PARENT,
        });
    });
    it('reports disabled folder trust as trusted disabled source', () => {
        expect(getWorkspaceTrustStatus({ security: { folderTrust: { enabled: false } } }, '/home/user/projectA')).toEqual({
            v: 1,
            workspaceCwd: '/home/user/projectA',
    ...
    it('distinguishes unknown from explicit do not trust', () => {
        mockRules['/home/user/untrusted'] = TrustLevel.DO_NOT_TRUST;
        expect(getWorkspaceTrustStatus(mockSettings, '/home/user/untrusted')).toMatchObject({
            effective: { state: 'untrusted', source: 'file' },
            explicitTrustLevel: TrustLevel.DO_NOT_TRUST,
        });
        expect(getWorkspaceTrustStatus(mockSettings, '/home/user/unknown')).toMatchObject({
    ...
    it('reports exact distrust over ancestor trust', () => {
        mockRules['/home/user/projectA'] = TrustLevel.TRUST_FOLDER;
        mockRules['/home/user/projectA/untrusted'] = TrustLevel.DO_NOT_TRUST;
        expect(getWorkspaceTrustStatus(mockSettings, '/home/user/projectA/untrusted')).toMatchObject({
            effective: { state: 'untrusted', source: 'file' },
            explicitTrustLevel: TrustLevel.DO_NOT_TRUST,
        });
    });
    it('does not mutate the cached config when a preview override is passed', () => {
        ...
        expect(loadTrustedFolders().user.config['/home/user/preview']).toBeUndefined();
    });
});
describe('isWorkspaceTrusted with IDE override', () => {
    ...
    it('should not apply IDE trust to an explicit different workspace', () => {
        ...
        vi.spyOn(fs, 'existsSync').mockImplementation((p) => p === getTrustedFoldersPath());
        vi.spyOn(fs, 'readFileSync').mockReturnValue(JSON.stringify({ '/home/user/other': TrustLevel.DO_NOT_TRUST }));
        expect(isWorkspaceTrusted(mockSettings, undefined, '/home/user/other')).toEqual({
            isTrusted: false,
            source: 'file',
        });
    });
