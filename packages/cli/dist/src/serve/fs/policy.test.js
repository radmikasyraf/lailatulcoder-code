...
import { Ignore } from '@lailatul-coder/lailatul-coder-core';
import { MAX_READ_BYTES, MAX_WRITE_BYTES, assertTrustedForIntent, detectBinary, enforceReadBytesSize, enforceReadSize, enforceWriteSize, shouldIgnore, } from './policy.js';
...
describe('shouldIgnore', () => {
    ...
    let workspace;
    beforeEach(async () => {
        scratch = await fsp.mkdtemp(path.join(os.tmpdir(), `qwen-policy-${randomBytes(4).toString('hex')}-`));
    ...
    it('flags non-trailing-slash directory patterns like node_modules', async () => {
        ...
        await fsp.mkdir(dir);
        const resolved = await resolveWithinWorkspace('node_modules', workspace, 'list');
...
describe('assertTrustedForIntent', () => {
    ...
    it('throws untrusted_workspace for edit intent when untrusted', () => {
        expect(() => assertTrustedForIntent(false, 'edit')).toThrowError(/edit operations are forbidden/);
    });
});
...
describe('detectBinary', () => {
    ...
    let workspace;
    beforeEach(async () => {
        scratch = await fsp.mkdtemp(path.join(os.tmpdir(), `qwen-binary-${randomBytes(4).toString('hex')}-`));
        const wsDir = path.join(scratch, 'ws');
        await fsp.mkdir(wsDir, { recursive: true });
        workspace = canonicalizeWorkspace(wsDir);
    });
    ...
    it('reports text files as non-binary', async () => {
        ...
        const resolved = await resolveWithinWorkspace('plain.txt', workspace, 'read');
