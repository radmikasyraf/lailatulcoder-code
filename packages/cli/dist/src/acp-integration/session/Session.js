...
import { AuthType, ApprovalMode, CompressionStatus, detectLoopSentinel, detectAutonomousSentinel, LoopTickResolver, convertToFunctionErrorResponse, convertToFunctionResponse, createDuplicateProviderToolCallResponse, findPlanModeEntryBatchBoundaryIndex, findRepeatedDuplicateProviderToolCall, markDuplicateProviderToolCallResponseSent, PLAN_MODE_ENTRY_SIBLING_SKIP_MESSAGE, createDebugLogger, DiscoveredMCPTool, StreamEventType, ToolConfirmationOutcome, generatePromptSuggestion, logPromptSuggestion, logToolCall, logUserPrompt, PromptSuggestionEvent, getErrorStatus, UserPromptEvent, readManyFiles, getSpecificMimeType, clampInlineMediaPart, Storage, Kind, ToolNames, ToolErrorType, fireNotificationHook, firePermissionRequestHook, firePreToolUseHook, firePostToolUseHook, firePostToolUseFailureHook, buildContextUsage, injectPermissionRulesIfMissing, NotificationType, persistPermissionOutcome, createHookOutput, wrapUserPromptSubmitContext, generateToolUseId, MessageBusType, MessageDisplayDispatcher, getPlanModeSystemReminder, getArenaSystemReminder, getStartupContextLength, isSystemReminderContent, buildSessionRecoveryPlanFromApiHistory, TURN_INTERRUPTION_HISTORY_TAIL_COUNT, evaluatePermissionFlow, buildPermissionCheckContext, evaluateToolInvocationGuard, getEffectivePermissionForConfirmation, needsConfirmation, isPlanModeBlocked, decoratePlanModeShellConfirmation, evaluatePlanModeShellPolicy, validatePlanModeShellApproval, validatePlanModeShellContext, abortGoalForStopHookCap, getStopHookContinuationReason, formatStopHookBlockingCapWarning, applyAutoModeDecision, decorateClassifierUnavailableConfirmation, evaluateAutoMode, getAutoModePermissionDeniedReason, isApproveOutcome, isDenialFallbackReason, MAX_TRANSCRIPT_MESSAGES, formatDenialStateLog, recordAllow, recordFallbackApprove, shouldFallback, shouldClassifyAllShellForAutoMode, finalizeToolResponses, shouldForceAutoModeReviewForAllow, shouldFirePermissionDeniedForAutoMode, shouldRunAutoModeForCall, extractDaemonTraceContext, addAgentInputMessageAttributes, AgentOutputMessageCapture, getActiveInteractionSpan, withInteractionSpan, SessionWriterError, startToolSpan, endToolSpan, addToolArgumentsAttributes, addToolCallResultAttributes, runInToolSpanContext, startToolExecutionSpan, endToolExecutionSpan, isShellProgressData, logConversationFinishedEvent, ConversationFinishedEvent, GLOBAL_DUPLICATE_THRESHOLD, canonicalToolName, getToolCallRepeatKey, shouldHaltOnTurnToolCallCap, logLoopDetected, logRepeatedToolFailureGuard, LoopDetectedEvent, LoopType, RepeatedToolFailureGuardEvent, acquireSleepInhibitor, didWriteProjectContextFile, refreshMemoryAfterManagedWrite, refreshMemoryInstruction, GoalPersistenceUnavailableError, goalTurnContext, sessionIdContext, promptIdContext, todoWorkChainContext, dedupeToolCallsById, getFunctionCallFingerprint, getProviderToolCallId, isReplayOfHandledToolCall, recordHandledToolCall, parsePositiveIntegerEnv, DEFAULT_TOKEN_LIMIT, hasImageParts, normalizeParts, runVisionBridge, bridgeToolResultImages, shouldRunVisionBridge, formatVisionBridgeNotice, formatFullTurnVisionNotice, getFullTurnVisionModelSelector, splitImageParts, approxBase64Bytes, normalizeTurnResultError, TURN_RESULT_CODE_TEXT_TRUNCATED, TURN_RESULT_TEXT_MAX_CHARS, runWithRuntimeContentGenerator, observeToolResultBoundary, toolResultBoundaryArtifact, toolResultPartDiagnosticValues, getInvocationContext, runWithInvocationContext, truncateNotificationLabel, buildBackgroundEntryLabel, collectSessionTurnState, computeInitialTurnFromHistory as computeInitialTurnFromHistoryCore, } from '@lailatul-coder/lailatul-coder-core';
...
import { DAEMON_CHANNEL_DELIVERY_META_KEY, DAEMON_ATTACHMENT_REFERENCES_META_KEY, DAEMON_PROMPT_DISPLAY_TEXT_META_KEY, MID_TURN_QUEUE_DRAIN_METHOD, isValidTrustedModelPrompt, TODO_STOP_GUARD_CONTINUATION_CLAIM_METHOD, } from '@lailatul-coder/acp-bridge/bridgeTypes';
...
import { CAPTURE_SCREEN_CONTEXT_TOOL_NAME, CaptureScreenContextTool, } from '../../serve/live/capture-screen-context.js';
import { createLiveTaskTools, } from '../../serve/live/live-task-tools.js';
import { SPEAK_TO_USER_TOOL_NAME, SpeakToUserTool, } from '../../serve/live/live-speak-to-user.js';
import { LIVE_BACKEND_END_INSTRUCTIONS, LIVE_BACKEND_START_INSTRUCTIONS, } from '../../serve/live/live-backend-instructions.js';
import { readVoiceModel } from '../../services/voice-settings.js';
import { MAX_AUDIO_BYTES, sanitizeVoiceErrorMessage, transcribeVoiceAudio, } from '../../services/voice-transcriber.js';
import { inactiveExtensionSkillRefs, isInactiveExtensionSkill, } from '../extension-skills.js';
import { RequestError } from '@agentclientprotocol/sdk';
import { SettingScope } from '../../config/settings.js';
import { insertAfterFunctionResponses, normalizePartList, } from '../../utils/nonInteractiveHelpers.js';
...
import { getSlashCommandFirstToken, isSlashCommand, } from '../../ui/utils/commandUtils.js';
...
import { ACP_ROUTE_ID_PREFIX, buildAcpModelOptions, getCurrentAcpModelId, parseAcpModelOption, resolveAcpModelOption, } from '../../utils/acpModelUtils.js';
import { classifyApiError } from '../../utils/classify-api-error.js';
import { getPersistScopeForModelSelection } from '../../config/modelProvidersScope.js';
import { writeStderrLine } from '../../utils/stdioHelpers.js';
import { buildExtensionMentionContext, EXTENSION_CONTEXT_BUDGET, matchExtensionByRef, parseExtensionRef, } from '../../utils/extension-mention.js';
import { buildMcpServerContextText, matchMcpServerByRef, parseMcpServerRef, } from '../../utils/mcp-server-mention.js';
...
import { goalPublicationKey, renderPreparedGoalUpdate, } from './recovered-goal-update.js';
import { SubAgentTracker } from './SubAgentTracker.js';
import { buildPermissionRequestContent, interactionMetaFields, requestPermissionWithAbort, resolvePermissionOutcome, toPermissionOptions, } from './permissionUtils.js';
import { MessageRewriteMiddleware, loadRewriteConfig, } from './rewrite/index.js';
import { DaemonTodoStopGuard, } from './daemon-todo-stop-guard.js';
import { createRepeatedToolFailureGuardState, reduceRepeatedToolFailureGuard, REPEATED_TOOL_FAILURE_REMINDER, REPEATED_TOOL_FAILURE_STOP_MESSAGE, parseRepeatedToolFailureGuardMode, } from './repeated-tool-failure-guard.js';
const debugLogger = createDebugLogger('SESSION');
const permissionRequestTails = new WeakMap();
const USER_CANCEL_ABORT_REASON = 'LailatulCoder:user-cancel';
const NEW_PROMPT_ABORT_REASON = 'LailatulCoder:new-prompt';
const SESSION_DISPOSE_ABORT_REASON = 'LailatulCoder:session-dispose';
const DAEMON_RETRY_META_KEY = 'LailatulCoder.daemon.retry';
const DAEMON_CONTINUE_META_KEY = 'LailatulCoder.daemon.continueLastTurn';
const MAX_DAEMON_ATTACHMENT_REFERENCES = 256;
function readDaemonAttachmentReferences(value) {
    ...
    for (const item of value) {
        ...
        if ((reference['type'] !== 'image' && reference['type'] !== 'resource') ||
...
const TODO_STOP_GUARD_PROMPT_BODY_SUFFIX = ' todo item(s) are still pending or in progress. Continue executing the current task now. Do not ask the user whether to continue. If progress requires user input, use the structured question or permission flow. If progress depends on external state, report the blocker explicitly.';
const TODO_STOP_GUARD_FINAL_PROMPT_SUFFIX = ' This is the final automatic continuation. Before ending, either complete/update the todos or report the completed progress and the exact blocker.';
...
function isTodoStopGuardPromptText(text) {
    if (typeof text !== 'string')
        return false;
    if (!text.startsWith(TODO_STOP_GUARD_PROMPT_PREFIX))
        return false;
...
function isCompressionFailureStatus(status) {
    return (status === CompressionStatus.COMPRESSION_FAILED_INFLATED_TOKEN_COUNT ||
...
async function finalizeToolCallPreparations(tracker, includeResolved, streamName) {
    try {
        ...
        debugLogger.warn(`Failed to discard tool preparations for ${streamName}; continuing stream: ${message}`);
    }
}
...
async function claimGoalTurn(runtime, turnKey, signal) {
    ...
    if (signal.aborted)
        return undefined;
    const immediate = runtime.permitForTurn(turnKey) ?? runtime.beginTurn(turnKey);
...
const PERMISSION_CANCEL_SKIP_MESSAGE = 'Skipped because a permission request was cancelled before the user answered; user input is required before continuing.';
const LOOP_DETECTED_SKIP_MESSAGE = 'Skipped because loop detection stopped the current turn before this tool call could run.';
const LOOP_DETECTED_CONTEXT_MESSAGE = 'System: this turn was terminated because the model exceeded tool-call safety limits. Try a different approach on the next turn.';
export const LOOP_DETECTED_TURN_ERROR_MESSAGE = 'Tool-call loop protection stopped this turn. The session is still available; send a more specific instruction to continue.';
const TOOL_EXECUTION_CANCELLED_MESSAGE = 'Tool execution was cancelled.';
const TOOL_POST_EXECUTION_CANCELLED_MESSAGE = 'The tool had already completed; its output was discarded.';
...
function repeatedToolFailureCountBucket(count) {
    if (count === 0)
        return '0';
    if (count <= 2)
        return '1-2';
...
function repeatedToolFailureBatchBucket(count) {
    if (count === 0)
        return '0';
    if (count === 1)
        return '1';
...
function recordRepeatedToolFailureDecision(promptId, mode, previousState, decision, batch) {
    if (mode === 'off' || decision.kind === 'none') {
        return;
    }
    const countState = decision.kind === 'reset' ? previousState : decision.state;
    const telemetryDecision = decision.kind === 'warn'
        ? 'warned'
        : decision.kind === 'stop'
            ? 'stopped'
            : decision.kind;
    const key = decision.kind === 'reset' ? undefined : decision.state.key;
    const matchingToolTypes = new Set(key
        ? batch.observations
            .filter((observation) => !observation.providerDuplicate &&
        ...
        : []);
    const toolType = matchingToolTypes.size === 1 ? [...matchingToolTypes][0] : undefined;
    logRepeatedToolFailureGuard(new RepeatedToolFailureGuardEvent({
        prompt_id: promptId,
        ...
        failure_count_bucket: repeatedToolFailureCountBucket(countState.failureCount),
...
function recordDaemonLoopDetected(config, promptId, loopType, message, loopState, options = {}) {
    if (!loopState.loopDetected) {
        loopState.loopDetected = true;
        loopState.loopType = loopType;
        debugLogger.warn(message);
        try {
            logLoopDetected(config, new LoopDetectedEvent(loopType, promptId), options);
        }
        catch (error) {
            debugLogger.debug('[Session] Failed to record loop detection telemetry', error);
        }
    }
    return true;
}
function createLoopDetectedTurnError(loopState) {
...
function cancelledOrThrowLoopDetected(signal, loopState) {
    if (signal.aborted)
        return 'cancelled';
    throw createLoopDetectedTurnError(loopState);
}
function isLoopDetectedTurnError(error) {
...
function recordDaemonToolCalls(config, promptId, loopState, calls) {
    ...
    if (shouldHaltOnTurnToolCallCap(loopState.totalToolCalls, loopState.maxToolCallKeyRepeat, config.getMaxToolCallsPerTurn(), config.isMaxToolCallsPerTurnExplicit())) {
        return recordDaemonLoopDetected(config, promptId, LoopType.TURN_TOOL_CALL_CAP, `Stopping ACP turn after ${loopState.totalToolCalls} tool calls in one turn.`, loopState);
    }
    ...
    if (!config.getSkipLoopDetection() &&
        loopState.maxToolCallKeyRepeat >= GLOBAL_DUPLICATE_THRESHOLD) {
        return recordDaemonLoopDetected(config, promptId, LoopType.GLOBAL_TOOL_CALL_DUPLICATE, `Stopping ACP turn after the same tool call repeated ${loopState.maxToolCallKeyRepeat} times.`, loopState);
    }
    return false;
}
function recordDaemonInvalidToolParams(config, promptId, loopState, toolName, error) {
    ...
    if (count < DAEMON_INVALID_TOOL_PARAMS_THRESHOLD)
        return false;
    return recordDaemonLoopDetected(config, promptId, LoopType.INVALID_TOOL_PARAMS_STAGNATION, `Stopping ACP turn after repeated tool parameter errors from ${toolName}: ${error.message}`, loopState);
}
...
function buildVoiceTranscriptBlock(modelId, transcript) {
    return [
...
function buildVoiceUnavailableBlock(reason) {
    return (`[Voice bridge could not transcribe attached audio: ${reason}. ` +
        'The audio content is unavailable; do not assume or invent what it says.]');
}
async function withTimeoutSignal(parentSignal, timeoutMs, fn) {
...
function hasInlineAttachmentContentBlock(content) {
    return content.some((part) => part.type === 'image' ||
        part.type === 'audio' ||
        part.type === 'resource');
}
function extractTurnPromptText(content) {
    let hasImage = false;
...
function stripReferencedAttachmentDataParts(parts, content) {
    const inlineDataCounts = new Map();
...
function capMidTurnDrainItems(items, fieldName) {
    if (items.length <= MAX_MID_TURN_DRAIN_ITEMS)
        return items;
    debugLogger.warn(`Mid-turn drain response had ${items.length} ${fieldName}; processing first ${MAX_MID_TURN_DRAIN_ITEMS}`);
    return items.slice(0, MAX_MID_TURN_DRAIN_ITEMS);
}
function getMidTurnItemDisplayTextForLog(displayText) {
...
function getValidMidTurnContentBlocks(content, displayText) {
    if (!Array.isArray(content)) {
        debugLogger.warn(`Dropped invalid mid-turn item: ${getMidTurnItemDisplayTextForLog(displayText)}`);
        return [];
    }
    const validBlocks = content.filter(isContentBlock);
    const invalidBlockCount = content.length - validBlocks.length;
    if (invalidBlockCount > 0) {
        debugLogger.warn(`Dropped ${invalidBlockCount} invalid mid-turn content block(s): ${getMidTurnItemDisplayTextForLog(displayText)}`);
    }
    return validBlocks;
}
function getStructuredMidTurnDisplayText(content, displayText, willPersistReferences) {
    if (typeof displayText === 'string' && displayText.trim().length > 0) {
...
function parseMidTurnDrainResponse(response) {
    ...
    if (Array.isArray(response['items'])) {
        return capMidTurnDrainItems(response['items'], 'item(s)').flatMap((item) => {
            if (!isRecord(item)) {
                return [];
            }
            const content = getValidMidTurnContentBlocks(item['content'], item['displayText']);
            if (content.length === 0)
                return [];
            const attachmentReferences = readDaemonAttachmentReferences(item['attachmentReferences']);
            ...
            const willPersistReferences = attachmentReferences !== undefined &&
                attachmentReferences.length ===
                    content.filter((block) => block.type === 'image' || block.type === 'resource').length;
            return [
                {
                    kind: 'structured',
                    content,
                    displayText: getStructuredMidTurnDisplayText(content, item['displayText'], willPersistReferences),
    ...
    if (!Array.isArray(response['messages'])) {
        debugLogger.warn(`Mid-turn drain response had no recognized 'items' or 'messages' field; keys: ${Object.keys(response).join(', ')}`);
        return [];
    }
...
function isValidMidTurnDrainResponse(response, requireQueuedPromptState) {
    ...
    if (Array.isArray(response['items'])) {
        return response['items'].every((item) => isRecord(item) &&
    ...
    return (Array.isArray(response['messages']) &&
        response['messages'].every((message) => typeof message === 'string' && message.trim().length > 0));
}
class MidTurnDrainTimeoutError extends Error {
    constructor() {
        super(`mid-turn queue drain got no response within ${MID_TURN_QUEUE_DRAIN_TIMEOUT_MS}ms`);
    }
}
class TodoStopGuardClaimTimeoutError extends Error {
    constructor() {
        super(`Todo Stop Guard continuation claim got no response within ${MID_TURN_QUEUE_DRAIN_TIMEOUT_MS}ms`);
    }
}
function beginChannelDeliveryResponseBlock(capture) {
    capture?.agentOutput.beginResponse();
    if (capture?.channelDelivery)
        capture.channelDelivery.finalText = '';
    if (capture?.turnResult)
        capture.turnResult.finalText = '';
    if (!capture?.channelDelivery && !capture?.turnResult)
        return undefined;
    return {
        parts: [],
        chars: 0,
...
function appendChannelDeliveryResponseText(responseBlock, text) {
    if (!responseBlock)
        return;
    if (responseBlock.capChars !== undefined &&
...
function rewindChannelDeliveryResponseBlock(responseBlock, checkpoint) {
    if (!responseBlock)
        return;
    const removed = responseBlock.parts.splice(checkpoint);
    for (const part of removed)
        responseBlock.chars -= part.length;
}
function commitChannelDeliveryResponseBlock(capture, responseBlock, hasFunctionCalls) {
    ...
    if (responseBlock && !hasFunctionCalls) {
        ...
        if (capture?.channelDelivery)
            capture.channelDelivery.finalText = finalText;
        if (capture?.turnResult)
            capture.turnResult.finalText = finalText;
    }
}
function parsePromptChannelDelivery(params) {
    ...
    if (typeof targetRecord['channelName'] !== 'string' ||
...
export function resolveExistingFile(resolved, resolveRealPath = realpathSync, statFile = statSync) {
    try {
...
export function resolveHomeLoopResolverRoots({ homeQwenDir = Storage.getGlobalQwenDir(), homeDir = os.homedir(), qwenHome = process.env['QWEN_HOME'], } = {}) {
    ...
    return {
        homeConfineRoot: (qwenHome ? homeQwenDir : homeDir) || path.dirname(homeQwenDir),
        homeQwenDir,
    };
}
...
export async function fireSessionPermissionDeniedForAutoMode(config, decision, outcome, toolName, toolParams, callId, signal) {
    if (!config.getDisableAllHooks?.() &&
        shouldFirePermissionDeniedForAutoMode(decision, outcome)) {
        try {
            await config
                .getHookSystem?.()
                ?.firePermissionDeniedEvent(toolName, toolParams, callId, getAutoModePermissionDeniedReason(decision), signal, callId);
        }
        catch (hookError) {
            debugLogger.warn(`PermissionDenied hook failed for tool ${callId}: ${hookError instanceof Error ? hookError.message : String(hookError)}`);
        }
    }
}
...
function collectMcpServerMentionRefs(text, mentions) {
    for (const match of text.matchAll(AT_TOKEN_RE)) {
        const pathName = match[1];
        if (!pathName)
            continue;
...
export async function buildAvailableCommandsSnapshot(config, abortSignal = AbortSignal.timeout(10_000), settings) {
    const slashCommands = await getAvailableCommands(config, abortSignal, 'acp', settings);
    ...
    const visibleSlashCommands = slashCommands.filter((cmd) => {
        if (cmd.kind !== CommandKind.SKILL || !cmd.skillDetail)
            return true;
        const skillName = cmd.skillDetail.name.toLowerCase();
        const isInactiveExtensionCommand = cmd.skillDetail.level === 'extension' &&
            isInactiveExtensionSkill({
                name: cmd.skillDetail.name,
                level: 'extension',
                extensionName: 'extensionName' in cmd.skillDetail &&
    ...
    const availableCommands = visibleSlashCommands.map((cmd) => {
        const acceptsInput = cmd.acceptsInput ??
    ...
    try {
        ...
        if (skillManager) {
            const skills = (await skillManager.listSkills()).filter((skill) => !disabledSkillNames.has(skill.name.toLowerCase()) &&
    ...
    const availableSkillDetails = skillDetailsByName.size > 0
...
export class Session {
    ...
    client;
    settings;
    runExclusiveAutomaticHistoryMutation;
    onActiveWorkChanged;
    pendingPrompt = null;
    /**
    ...
    duplicateProviderToolCallResponseIds = new Set();
    ...
    provisionalRelatedAgentCounts = new Map();
    todoStopGuardQueuedPromptPriority = false;
    todoStopGuardQueuedPromptOwnerPromptId;
    todoStopGuardClaimOwnerCounts = new Map();
    todoStopGuardReleasedDuringClaim = new Set();
    ...
    currentAgentNotificationTaskId = null;
    currentShellNotificationActive = false;
    persistedBackgroundNotificationTaskIds = new Set();
    backgroundNotificationAcceptances = new Map();
    activeAgentNotificationAcceptances = new Set();
    goalQueue = [];
    ...
    goalPublicationTail = Promise.resolve();
    ...
    closeGateCompletion = null;
    resolveCloseGate = null;
    ...
    workflowApprovalAbortController = new AbortController();
    ...
    sessionId;
    constructor(id, config, client, settings, runExclusiveAutomaticHistoryMutation = (operation) => operation(), 
    /**
    ...
    onActiveWorkChanged) {
        ...
        const todoStopGuardEnabled = this.settings.merged.experimental?.todoStopGuard === true &&
            !this.config.getBareMode() &&
            !this.config.isSafeMode();
        this.todoStopGuard = new DaemonTodoStopGuard(todoStopGuardEnabled);
        const configuredGuardMode = process.env[ENV_ACP_REPEATED_TOOL_FAILURE_GUARD];
        ...
        if (configuredGuardMode?.trim() && parsedGuardMode === undefined) {
            debugLogger.warn(`${ENV_ACP_REPEATED_TOOL_FAILURE_GUARD} has an invalid value; defaulting to shadow. Expected off, shadow, warn, or enforce.`);
        }
        ...
        this.config
            ...
            .setApprovalRequestCallback((entry, approval, rawArgs, signal) => this.#requestWorkflowApproval(entry.runId, approval, rawArgs, signal));
    }
    #bindGoalRuntime() {
        try {
            ...
            this.goalRuntimeUnsubscribe = runtime.subscribe((snapshot, cause) => {
                ...
                void this.#queueGoalState(snapshot, cause, previousGoal).catch((error) => debugLogger.warn(`Failed to emit ACP Goal state: ${error instanceof Error ? error.message : String(error)}`));
            });
            const host = {
                startGoalTurn: async (input) => {
                    if (this.goalQueue.some(({ permit }) => permit.turnId === input.permit.turnId) ||
     ...
     */
    rebindGoalRuntimeForNewSession() {
     ...
     */
    async publishRecoveredGoalState(replayedRecords) {
        if (this.disposed || this.closing)
            return;
        let runtime;
        try {
    ...
    async renderRecoveredGoalUpdates(replayedRecords) {
        if (this.disposed || this.closing)
            return [];
        const rendered = await renderPreparedGoalUpdate(() => this.config.getGoalRuntimeReady(), {
            ...(replayedRecords ? { replayedRecords } : {}),
            previousGoal: this.lastGoalSnapshot?.goal ?? null,
        });
        if (rendered.publicationKey &&
     ...
     */
    async #supersedeUnrestorableGoal(replayedRecords) {
     ...
     */
    #unrestorableGoalStatus(replayedRecords) {
        if (!replayedRecords?.length)
            return undefined;
        const active = findGoalToRestore(collectGoalStatusItemsFromRecords(replayedRecords));
        if (!active)
            return undefined;
        return {
            kind: 'cleared',
    ...
    async #publishGoalState(snapshot, cause, previousGoal = this.lastGoalSnapshot?.goal ?? null) {
        if (this.#suppressRecoveredGoalUpdate(snapshot))
            return;
    ...
    #queueGoalState(snapshot, cause, previousGoal = this.lastGoalSnapshot?.goal ?? null) {
        const publication = this.goalPublicationTail.then(() => this.#publishGoalState(snapshot, cause, previousGoal));
    ...
    async #drainGoalQueue() {
        if (this.goalQueue.length === 0)
            return;
        await this.runExclusiveAutomaticHistoryMutation(() => this.#drainGoalQueueExclusive());
    }
    async #drainGoalQueueExclusive() {
        ...
        try {
            ...
            }, undefined, undefined, undefined, turn);
        }
        catch (error) {
            ...
            debugLogger.warn(`ACP Goal turn failed: ${error instanceof Error ? error.message : String(error)}`);
        }
        finally {
            await this.#emitGoalEndTurn(result);
            if (this.activeGoalTurn === turn)
                this.activeGoalTurn = undefined;
    ...
    async #settleGoalTurn(turn, result, failed) {
        try {
            ...
            if (!turn.modelStarted) {
                if (turn.controller.signal.reason === USER_CANCEL_ABORT_REASON &&
            ...
            try {
                ...
                debugLogger.warn(`Failed to flush ACP Goal turn: ${error instanceof Error ? error.message : String(error)}`);
            }
            const cancelledByUser = result?.stopReason === 'cancelled' &&
            ...
            const supersededByNewPrompt = turn.controller.signal.reason === NEW_PROMPT_ABORT_REASON;
            const shouldPause = !supersededByNewPrompt &&
                (failed ||
            ...
            try {
                ...
                debugLogger.warn(`Failed to record ACP Goal turn settlement: ${error instanceof Error ? error.message : String(error)}`);
                await runtime.releaseTurn(turn.turnKey);
            }
        }
        catch (error) {
            debugLogger.warn(`Failed to settle ACP Goal turn: ${error instanceof Error ? error.message : String(error)}`);
        }
    }
    /**
    ...
    async #pauseGoalForStopHookCap() {
        try {
            ...
            debugLogger.warn(`Failed to pause the Goal after the Stop hook cap: ${error instanceof Error ? error.message : String(error)}`);
        }
    }
    async #requestWorkflowApproval(runId, approval, rawArgs, approvalSignal) {
        ...
        const content = confirmationDetails.type === 'edit'
            ? [
        ...
        const { title, locations, kind } = this.toolCallEmitter.resolveToolMetadata(approval.name, rawArgs);
        const params = {
            sessionId: this.sessionId,
        ...
        try {
            ...
            const outcome = resolvePermissionOutcome(response, offeredPermissionOptions);
            const resolved = await registry.resolvePendingApproval(runId, approval.approvalId, outcome === ToolConfirmationOutcome.ProceedOnce
                ? outcome
                : ToolConfirmationOutcome.Cancel, undefined);
            await this.#finishWorkflowApprovalToolCall(approval, externalToolCallId, resolved && outcome === ToolConfirmationOutcome.ProceedOnce);
        }
        catch (error) {
            debugLogger.error('Workflow permission request failed:', error);
            await registry.resolvePendingApproval(runId, approval.approvalId, ToolConfirmationOutcome.Cancel);
            await this.#finishWorkflowApprovalToolCall(approval, externalToolCallId, false);
        }
    }
    async #finishWorkflowApprovalToolCall(approval, externalToolCallId, success) {
        try {
            ...
            debugLogger.warn('Failed to finalize workflow approval tool call:', error);
        }
    }
    #prepareTodoStopGuardForPrompt(params) {
        ...
        const drainSupersededAutomaticQueues = this.todoStopGuard.blocksUnrelatedAutomaticTurns ||
    ...
    #clearTodoStopGuardTrustAndDrainAutomaticQueues() {
        ...
        this.todoStopGuard.blockUntilOrdinaryPromptStarts();
        if (preserveQueuedPromptPriority || !shouldDrain)
            return;
    ...
    releaseTodoStopGuardQueuedPromptWait(promptId) {
        const matchesCurrentWait = this.todoStopGuardQueuedPromptPriority &&
    ...
    async #claimTodoStopGuardContinuation(abortSignal) {
        ...
        const ownerPromptId = context?.sessionId === this.sessionId ? context.promptId : undefined;
        if (ownerPromptId) {
            this.todoStopGuardClaimOwnerCounts.set(ownerPromptId, (this.todoStopGuardClaimOwnerCounts.get(ownerPromptId) ?? 0) + 1);
        }
        let timeoutHandle;
        try {
            const claimPromise = this.client.extMethod(TODO_STOP_GUARD_CONTINUATION_CLAIM_METHOD, {
                sessionId: this.sessionId,
                ...(ownerPromptId ? { promptId: ownerPromptId } : {}),
            });
            const timeoutPromise = new Promise((_, reject) => {
                timeoutHandle = setTimeout(() => reject(new TodoStopGuardClaimTimeoutError()), MID_TURN_QUEUE_DRAIN_TIMEOUT_MS);
            });
            ...
            debugLogger.warn(`Todo Stop Guard continuation claim unavailable [session ${this.sessionId}]: ${this.#formatError(error)}`);
            return 'unavailable';
        }
        finally {
    ...
    #captureTodoStopGuardBackgroundBaseline() {
        ...
        const wakeups = this.config.isCronEnabled?.()
            ? (this.config.getCronScheduler?.()?.list?.() ?? []).filter((job) => job.cronExpr === '@wakeup')
            : [];
        return {
            agents: new Set([
            ...
            wakeups: new Set([
                ...
                ...this.cronQueue.flatMap((item) => item.source === 'loop' && item.taskId ? [item.taskId] : []),
            ]),
        };
    }
    ...
    #agentContinuesTodoStopGuardWorkChain(taskId, visiting = new Set()) {
        if (this.relatedAgentIds.has(taskId) ||
            (this.provisionalRelatedAgentCounts.get(taskId) ?? 0) > 0) {
            return true;
        }
        if (this.todoStopGuardBackgroundBaseline.agents.has(taskId))
            return false;
        ...
        if (!task.parentAgentId)
            return true;
        visiting.add(taskId);
        const related = this.#agentContinuesTodoStopGuardWorkChain(task.parentAgentId, visiting);
    ...
    #hasRelevantTodoStopGuardBackgroundInput() {
        if (this.notificationQueue.some((item) => this.#notificationContinuesTodoStopGuardWorkChain(item)) ||
            this.cronQueue.some((item) => this.#cronContinuesTodoStopGuardWorkChain(item))) {
            return true;
        }
        const baseline = this.todoStopGuardBackgroundBaseline;
        const agents = this.config.getBackgroundTaskRegistry?.()?.getAll?.() ?? [];
        if (agents.some((task) => this.#agentContinuesTodoStopGuardWorkChain(task.id) &&
            task.isBackgrounded &&
            (task.status === 'running' ||
                task.status === 'paused' ||
        ...
        const shells = this.config.getBackgroundShellRegistry?.()?.getAll?.() ?? [];
        if (shells.some((task) => !baseline.shells.has(task.id) && task.status === 'running')) {
            return true;
        }
        const monitors = this.config.getMonitorRegistry?.()?.getAll?.() ?? [];
        if (monitors.some((task) => this.#monitorContinuesTodoStopGuardWorkChain(task.id, task.ownerAgentId) && task.status === 'running')) {
            return true;
        }
        if (!this.config.isCronEnabled?.())
            return false;
        const wakeups = this.config.getCronScheduler?.()?.list?.() ?? [];
        return wakeups.some((job) => job.cronExpr === '@wakeup' && !baseline.wakeups.has(job.id));
    }
    /**
     ...
     */
    #registerSubSessionSpawner() {
        this.config.setSubSessionSpawner(async (req) => {
            const resp = await this.client.extMethod(SERVE_CONTROL_EXT_METHODS.createSubSession, {
                prompt: req.prompt,
            ...
            if (typeof resp['sessionId'] !== 'string' || !resp['sessionId']) {
                throw new Error('create_sub_session: bridge returned non-string sessionId');
            }
    ...
    async enableLiveScreenContext() {
        ...
        if (existing && existing !== this.liveScreenContextTool) {
            throw new Error('capture_screen_context is reserved for the trusted Live Appshot channel.');
        }
        if (!this.liveScreenContextTool) {
            const tool = new CaptureScreenContextTool(async () => {
                const response = await this.client.extMethod(SERVE_CONTROL_EXT_METHODS.liveCaptureScreenContext, { callerSessionId: this.sessionId });
            ...
            if (registry.getTool(CAPTURE_SCREEN_CONTEXT_TOOL_NAME) !== tool) {
                throw new Error('capture_screen_context is required for Live Voice but is disabled.');
            }
            this.liveScreenContextTool = tool;
        }
        if (this.liveTaskTools.length === 0) {
            const tools = createLiveTaskTools(async (name, args) => this.client.extMethod(SERVE_CONTROL_EXT_METHODS.liveTaskTool, {
                callerSessionId: this.sessionId,
                name,
                arguments: args,
            }));
            for (const tool of tools) {
                if (registry.getTool(tool.name)) {
                    throw new Error(`${tool.name} is reserved for the trusted Live task channel.`);
                }
            }
            for (const tool of tools)
                registry.registerTool(tool);
            for (const tool of tools) {
                if (registry.getTool(tool.name) !== tool) {
                    throw new Error(`${tool.name} is required for Live Voice but is disabled.`);
                }
            }
            this.liveTaskTools = tools;
        }
        const existingSpeakToUser = registry.getTool(SPEAK_TO_USER_TOOL_NAME);
        if (existingSpeakToUser &&
            existingSpeakToUser !== this.liveSpeakToUserTool) {
            throw new Error('speak_to_user is reserved for the trusted Live speech channel.');
        }
        if (!this.liveSpeakToUserTool) {
            ...
            if (registry.getTool(SPEAK_TO_USER_TOOL_NAME) !== tool) {
                throw new Error('speak_to_user is required for Live Voice but is disabled.');
            }
    ...
    async setLiveConversationActive(active) {
        if (this.liveConversationActive === active)
            return;
        if (active) {
    ...
    async appendLiveConversationTranscript(entries, model) {
        if (this.liveConversationActive !== true) {
            throw RequestError.invalidParams(undefined, 'Live conversation is not active for this session.');
        }
        const recording = this.config.getChatRecordingService();
        if (!recording) {
            throw RequestError.internalError(undefined, 'Chat recording service unavailable');
        }
        await recording.recordRealtimeConversation(entries, model);
        for (const entry of entries) {
            try {
                await this.sendUpdate({
                    sessionUpdate: entry.role === 'user'
                        ? 'user_message_chunk'
                        : 'agent_message_chunk',
                ...
                debugLogger.warn(`Failed to emit persisted realtime transcript: ${this.#formatError(error)}`);
            }
        }
    }
    async #consumeLiveEndInstruction() {
        if (this.liveConversationActive || !this.liveEndInstructionPending)
            return;
        this.liveEndInstructionPending = false;
    ...
    startCronScheduler() {
        ...
        this.#startCronSchedulerInRuntime().catch((error) => {
            debugLogger.warn(`Cron scheduler startup failed [session ${this.sessionId}]: ${error}`);
        });
    }
    getConfig() {
        return this.config;
    ...
    async assertCanStartTurn() {
        ...
        if (this.historyMutationActive) {
            throw RequestError.invalidParams(undefined, 'Session history mutation is in progress');
        }
        try {
    ...
    collectActiveWorkHolds() {
        ...
        const shellActive = this.config.getBackgroundShellRegistry().hasRunningEntries() ||
    ...
    beginClose() {
        if (this.closing) {
            throw RequestError.invalidParams(undefined, 'Session close is already in progress');
        }
        this.closing = true;
        let resolveGate;
    ...
    #deferAutomaticQueueDrainUntilTurnsSettle() {
        ...
        if (completions.length === 0)
            return false;
        if (this.automaticDrainRetry)
            return true;
        const retry = Promise.allSettled(completions).then(() => {
            if (this.automaticDrainRetry !== retry)
                return;
    ...
    installRewriter() {
        ...
        if (rewriteConfig?.enabled) {
            ...
            this.messageRewriter = new MessageRewriteMiddleware(this.config, rewriteConfig, (update) => this.sendUpdate(update));
        }
    }
    /**
     ...
     */
    primeTurnFromHistory(records) {
        const turnState = collectSessionTurnState(records, this.config.getSessionId());
        this.primeTurnState(turnState.initialTurn, turnState.backgroundNotificationTaskIds);
    }
    primeTurnState(initialTurn, backgroundNotificationTaskIds) {
    ...
    async replayHistory(records, gaps, options) {
    ...
    rewindToTurn(targetTurnIndex, opts) {
        if (!Number.isInteger(targetTurnIndex) || targetTurnIndex < 0) {
            throw RequestError.invalidParams(undefined, 'targetTurnIndex must be a non-negative integer');
        }
        if (this.closing || this.#hasActiveTurn()) {
            throw RequestError.invalidParams(undefined, 'Cannot rewind while a prompt is running');
        }
        const chat = this.config.getGeminiClient().getChat();
        const apiHistory = chat.getHistoryShallow();
        const apiTruncateIndex = this.#computeApiTruncationIndexForUserTurn(apiHistory, targetTurnIndex);
        if (apiTruncateIndex < 0) {
            throw RequestError.invalidParams(undefined, 'Cannot rewind to the requested turn. It may have been compressed or does not exist.');
        }
        ...
        this.config
            ...
            ?.rewindRecording(targetTurnIndex, { truncatedCount: Math.max(0, apiHistory.length - apiTruncateIndex) }, survivingSnapshots);
    ...
    restoreHistory(history) {
        if (this.closing || this.#hasActiveTurn()) {
            throw RequestError.invalidParams(undefined, 'Cannot restore history while a prompt is running');
        }
        this.config
            .getGeminiClient()
            .getChat()
    ...
    #isUserTextContent(content) {
        ...
        if (!content.parts || content.parts.length === 0)
            return false;
        const hasFunctionResponse = content.parts.some((part) => 'functionResponse' in part);
        ...
        // is NOT excluded.
        if (isSystemReminderContent(content))
            return false;
        if (content.parts.some((part) => 'text' in part && isTodoStopGuardPromptText(part.text))) {
            return false;
        }
    ...
    async cancelPendingPrompt() {
        ...
        const hadNotification = !!this.notificationAbortController || this.notificationProcessing;
        ...
        if (queuedGoalTurn) {
            try {
                const runtime = this.config.getGoalRuntime();
                if (sameGoalPermit(runtime.permitForTurn(queuedGoalTurn.turnKey), queuedGoalTurn.permit) &&
                    runtime.getSnapshot().goal?.status === 'active') {
                    await runtime.dispatch({
                        action: 'pause',
                ...
                debugLogger.warn(`Failed to pause queued ACP Goal turn: ${error instanceof Error ? error.message : String(error)}`);
            }
        }
    ...
    async prompt(params, invocationContext, admissionCancellation, modelPrompt, scheduledGoalTurn) {
        if (invocationContext !== undefined &&
            invocationContext.sessionId !== this.config.getSessionId()) {
            throw RequestError.invalidParams(undefined, 'Invocation context session does not match the active session');
        }
        const turnRecording = this.#beginTurnRecording(params, invocationContext);
        try {
            const result = await this.#promptWithTurnRecording(params, invocationContext, admissionCancellation, modelPrompt, scheduledGoalTurn, turnRecording);
            this.#settleTurnRecording(result.stopReason === 'cancelled' ? 'cancelled' : 'completed', turnRecording, result);
            return result;
        }
        catch (error) {
            const pendingSend = turnRecording?.abortController;
            const abortReason = pendingSend?.signal.aborted === true
            ...
            const controlledAbort = abortReason === USER_CANCEL_ABORT_REASON ||
    ...
    async #promptWithTurnRecording(params, invocationContext, admissionCancellation, modelPrompt, scheduledGoalTurn, turnRecording) {
        ...
        if (modelPrompt !== undefined && invocationContext === undefined) {
            throw RequestError.invalidParams(undefined, 'Model-only prompt requires trusted invocation context');
        }
        if (modelPrompt !== undefined && !isValidTrustedModelPrompt(modelPrompt)) {
            throw RequestError.invalidParams(undefined, 'Invalid trusted model-only prompt');
        }
        ...
        let reservedGoalRuntime;
        let reservedGoalTurnKey;
        if (!goalTurn) {
            try {
                const runtime = this.config.getGoalRuntime();
        ...
        if (turnRecording)
            turnRecording.abortController = pendingSend;
        ...
        if (reservedGoalRuntime && reservedGoalTurnKey) {
            try {
                const permit = await claimGoalTurn(reservedGoalRuntime, reservedGoalTurnKey, pendingSend.signal);
                if (permit) {
                    const goal = reservedGoalRuntime.getSnapshot().goal;
                    if (goal) {
                        const verifierFeedback = reservedGoalRuntime.getVerifierFeedback(permit);
                        goalTurn = {
                            permit,
                ...
                try {
                    ...
                    debugLogger.warn(`Failed to release Goal reservation after admission failure: ${releaseError instanceof Error
        ...
        const channelPromptTurn = params._meta?.[CHANNEL_PROMPT_META_KEY] === true;
        const recording = this.config.getChatRecordingService();
        const branchCheckpointCursor = scheduledGoalTurn === undefined && !channelPromptTurn
        ...
        this.pendingPromptCompletion = new Promise((resolve) => {
        ...
        if (turnRecording)
            turnRecording.startedAt = Date.now();
        try {
            const result = await this.#executePrompt(params, pendingSend, responseCapture, invocationContext, modelPrompt, 
            ...
            !channelPromptTurn && goalTurn === undefined, goalTurn, channelPromptTurn);
            let branchPoint;
            if (recording && branchCheckpointCursor) {
                try {
                    ...
                    debugLogger.warn('Failed to record branch checkpoint; completing the turn without a branch point', error);
                }
            }
            const completedResult = branchPoint
                ? {
                    ...result,
                    _meta: {
                        ...result._meta,
                        'LailatulCoder.branchPoint': {
            ...
            if (channelDelivery && completedResult.stopReason === 'end_turn') {
                this.#scheduleChannelDelivery({
                    ...
                    text: normalizeChannelDeliveryText(responseCapture.channelDelivery?.finalText ?? ''),
     ...
     * authoritatively. Powers `LailatulCoder/control/session/continue`.
     */
    async continueLastTurn() {
        ...
        const recoveryPlan = buildSessionRecoveryPlanFromApiHistory({
            sessionId: this.sessionId,
            apiHistory: chat.getHistoryTailShallow?.(TURN_INTERRUPTION_HISTORY_TAIL_COUNT) ??
                chat.getHistoryTail(TURN_INTERRUPTION_HISTORY_TAIL_COUNT),
        });
        if (!recoveryPlan.continuation) {
            return { accepted: false, interruption: 'none' };
        }
        const interruption = recoveryPlan.kind === 'interrupted_prompt'
            ? 'interrupted_prompt'
            : 'interrupted_turn';
     ...
     * `LailatulCoder/notify/session/prompt-suggestion` extNotification. Mirrors
    ...
    #maybeEmitFollowupSuggestion(result) {
        if (result.stopReason !== 'end_turn')
            return;
        if (this.todoStopGuard.blocksUnrelatedAutomaticTurns ||
            this.todoStopGuardQueuedPromptPriority) {
            return;
        }
        ...
        if (this.settings.merged.ui?.enableFollowupSuggestions === false)
            return;
        if (this.config.getApprovalMode() === ApprovalMode.PLAN)
            return;
        const chat = this.config.getGeminiClient()?.getChat();
        ...
        const promptId = this.config.getSessionId() + '########' + String(this.turn);
        void (async () => {
            try {
                const conversationHistory = chat.getHistoryTail(40, true);
                const lastEntry = conversationHistory[conversationHistory.length - 1];
                if (!lastEntry || lastEntry.role !== 'model') {
                    debugLogger.debug('Skipping followup suggestion: last history entry is not model');
                    return;
                }
                const r = await generatePromptSuggestion(this.config, conversationHistory, ac.signal, {
                    ...
                    enableCacheSharing: this.settings.merged.ui?.enableCacheSharing !== false,
                });
                if (ac.signal.aborted)
                    return;
                if (r.suggestion) {
                    await this.client.extNotification('LailatulCoder/notify/session/prompt-suggestion', {
                        v: 1,
                        sessionId: this.sessionId,
                    ...
                    logPromptSuggestion(this.config, new PromptSuggestionEvent({
                        outcome: 'suppressed',
    ...
    async #executePrompt(params, pendingSend, responseCapture, invocationContext, modelPrompt, rejectOnLoopDetected = false, goalTurn, channelTurn = false) {
        ...
        const execute = () => runWithInvocationContext(invocationContext, () => sessionIdContext.run(sessionId, () => this.#executePromptInner(params, pendingSend, responseCapture, modelPrompt, rejectOnLoopDetected, goalTurn, channelTurn)));
        return goalTurn
    ...
    async #executePromptInner(params, pendingSend, responseCapture, modelPrompt, rejectOnLoopDetected = false, goalTurn, channelTurn = false) {
        return Storage.runWithRuntimeBaseDir(this.runtimeBaseDir, this.config.getWorkingDir(), async () => {
            await this.assertCanStartTurn();
            if (pendingSend.signal.aborted) {
                return { stopReason: 'cancelled' };
            }
            ...
            const promptMetadata = params
                ._meta;
            const continuesCurrentWorkChain = params.retry === true ||
            ...
            // it, `LailatulCoder review fetch-pr` cannot record its worktree lease and an
            ...
            return await withInteractionSpan(this.config, {
                ...
                const promptDisplayText = typeof promptDisplayTextValue === 'string'
                    ? promptDisplayTextValue
                    : undefined;
                const modelPromptBlocks = modelPrompt === undefined
                    ? params.prompt
                ...
                if (!isRetry && !isContinue && goalTurn?.origin !== 'runtime') {
                    ...
                    if (interactionSpan) {
                        addAgentInputMessageAttributes(this.config, interactionSpan, promptDisplayText ?? promptText);
                    }
                }
                const firstTextBlock = modelPromptBlocks.find((block) => block.type === 'text');
                ...
                if (goalTurn?.origin === 'runtime') {
                    this.config.getChatRecordingService()?.recordGoalRuntimeMessage(modelPromptBlocks
                        .filter((block) => block.type === 'text')
                        .map((block) => ({ text: block.text })), goalTurn.permit);
                }
                else if (isContinue) {
                    const recoveryPlan = buildSessionRecoveryPlanFromApiHistory({
                        sessionId: this.sessionId,
                    ...
                    if (!recoveryPlan.continuation) {
                        ...
                        debugLogger.warn(`[Session] continue ${promptId}: no interrupted turn on re-detection, nothing to continue`);
                        ...
                        logConversationFinishedEvent(this.config, new ConversationFinishedEvent(this.config.getApprovalMode(), 0));
                ...
                if (goalTurn?.origin === 'runtime') {
                    ...
                    const attachmentReferences = readDaemonAttachmentReferences(promptMetadata?.[DAEMON_ATTACHMENT_REFERENCES_META_KEY]);
                ...
                if (isContinue) {
                    ...
                    const shouldRecordSlashCommand = !(resolvedCommandInfo?.kind === CommandKind.BUILT_IN &&
                    ...
                    try {
                        parts = await this.#processSlashCommandResult(slashCommandResult, modelPromptBlocks, pendingSend.signal, onFullTurnModel, shouldRecordSlashCommand);
                    }
                    catch (error) {
                        logConversationFinishedEvent(this.config, new ConversationFinishedEvent(this.config.getApprovalMode(), 0));
                        throw error;
                    ...
                    // file/editor content is appended (issue: ACP + local LailatulCoder).
                    parts = await this.#resolvePrompt(modelPromptBlocks, pendingSend.signal, { promptLast: true, onFullTurnModel });
                }
                ...
                if (!isContinue &&
                    ...
                    if (hookOutput?.isBlockingDecision() ||
                        ...
                        await this.messageEmitter.emitAgentMessage(`✗ **UserPromptSubmit blocked**: ${blockReason}`);
                        return { stopReason: 'end_turn' };
                    }
                ...
                this.config.startActiveTodoWorkChain(promptId, continuesCurrentWorkChain
                ...
                if (systemReminders.length > 0) {
                    ...
                    const alreadyHasReminder = isContinue &&
                        parts.some((part) => isSystemReminderContent({ role: 'user', parts: [part] }));
                ...
                if (this.pendingRecoveredAgentsNotice &&
                    !isContinue &&
                    !isSlashInput) {
                    const noticePart = {
                ...
                const activeTodoReminder = this.config.takeActiveTodoReminder(promptId, true);
                ...
                let turnCount = 0;
                const toolLoopState = createDaemonToolLoopState(channelTurn ? 'off' : this.repeatedToolFailureGuardMode);
                ...
                try {
                    while (nextMessage !== null) {
                        ...
                        const functionCalls = [];
                        const preparationTracker = new ToolCallPreparationTracker(this.toolCallEmitter);
                        let usageMetadata = null;
                        const streamStartTime = Date.now();
                        const messageDisplay = this.#createMessageDisplayDispatcher(pendingSend.signal);
                        ...
                        try {
                            ...
                            const sendResult = await this.#sendMessageStreamWithAutoCompression(promptId, nextMessage?.parts ?? [], pendingSend.signal, { modelOverride: fullTurnModelOverride });
                            if (!sendResult.responseStream) {
                                ...
                                this.#preserveUnsentMessageHistory(nextMessage, isContinue || sendResult.stopReason === 'cancelled');
                            ...
                            try {
                                for await (const resp of responseStream) {
                                    ...
                                    if (resp.type === StreamEventType.CHUNK &&
                                        ...
                                        for (const part of candidate.content?.parts ?? []) {
                                            ...
                                            if (!part.thought) {
                                                ...
                                                appendChannelDeliveryResponseText(channelDeliveryResponseBlock, part.text);
                                                messageDisplay?.addChunk(part.text);
                                            }
                                        }
                                        responseCapture.agentOutput.observeFinishReason(candidate.finishReason);
                                    }
                                    if (resp.type === StreamEventType.CHUNK &&
                                    ...
                                    if (resp.type === StreamEventType.RETRY ||
                                        ...
                                        responseCapture.agentOutput.restartAttempt(resp.type === StreamEventType.RETRY &&
                                            resp.isContinuation === true);
                                        if (resp.type === StreamEventType.MODEL_FALLBACK ||
                                            !resp.isContinuation) {
                                            rewindChannelDeliveryResponseBlock(channelDeliveryResponseBlock, channelDeliveryCheckpoint);
                                        }
                                        await finalizeToolCallPreparations(preparationTracker, true, `main prompt ${resp.type}`);
                                ...
                                await finalizeToolCallPreparations(preparationTracker, streamFailed || pendingSend.signal.aborted, 'main prompt');
                            }
                        }
                        catch (error) {
                            ...
                            const isControlledCancellation = pendingSend.signal.aborted &&
                        ...
                        commitChannelDeliveryResponseBlock(responseCapture, channelDeliveryResponseBlock, functionCalls.length > 0);
                        if (usageMetadata) {
                        ...
                        if (functionCalls.length > 0) {
                            const toolRun = await this.#runWithFullTurnModel(fullTurnModelOverride, () => this.runToolCalls(pendingSend.signal, promptId, functionCalls, toolLoopState, onFullTurnModel));
                            ...
                            const nextAfterTools = await this.#buildNextMessageAfterToolRun(toolRun, pendingSend.signal, promptId, toolLoopState, onFullTurnModel, rejectOnLoopDetected);
                    ...
                    const result = await this.#handleStopHookLoop(pendingSend, promptId, hooksEnabled, messageBus, true, fullTurnModelOverride, responseCapture, rejectOnLoopDetected);
                    ...
                    logConversationFinishedEvent(this.config, new ConversationFinishedEvent(this.config.getApprovalMode(), turnCount));
                    // Remove review worktrees leased during this prompt and not
                    // released by the skill's own cleanup step — a cancelled or
                    // errored /review otherwise leaves `.lailatulcoder/tmp/review-pr-<n>`
                    ...
                    // lease was already cleared by `LailatulCoder review cleanup`.
    ...
    async #handleStopHookLoop(pendingSend, promptId, hooksEnabled, messageBus, allowExternalHooks = true, modelOverride, responseCapture, rejectOnLoopDetected = false) {
        ...
        while (true) {
            ...
            if (this.todoStopGuard.needsStopInspection) {
                ...
                if (drained.parts.length > 0) {
                    if (drained.hasQueuedPrompt) {
                        const claim = await this.#claimTodoStopGuardContinuation(pendingSend.signal);
                        if (claim === 'queued') {
                            this.#preserveUnsentMessageHistory({ role: 'user', parts: drained.parts }, true);
                            return { stopReason: 'end_turn' };
                        }
                    ...
                    const continuation = await this.#runStopContinuation(pendingSend, promptId + '_mid_turn_' + ++midTurnContinuationCount, promptId, drained.parts, false, {
                        onFullTurnModel,
            ...
            let stopHookCount = 1;
            if (allowExternalHooks &&
                hooksEnabled &&
                messageBus &&
                stopHookIterationCount < stopHookBlockingCap &&
                this.config.hasHooksForEvent?.('Stop')) {
                const responseText = this.#getCurrentChat().getLastModelMessageText?.() ||
                    '[no response text]';
                const contextUsage = buildContextUsage(this.config.getContentGeneratorConfig()?.contextWindowSize ??
                    DEFAULT_TOKEN_LIMIT, this.lastPromptTokenCount);
                let response;
                try {
            ...
            const guardDecision = this.todoStopGuard.decide(this.todoStopGuard.needsStopInspection
            ...
            if (externalReason && stopHookIterationCount >= stopHookBlockingCap) {
                const warning = formatStopHookBlockingCapWarning('Stop', stopHookBlockingCap);
                if (!abortGoalForStopHookCap(this.config, this.config.getSessionId(), warning)) {
            ...
            const continueParts = [];
            if (externalReason)
                continueParts.push({ text: externalReason });
            ...
            if (externalReason && stopHookIterationCount > 1 && !guardContinuation) {
                await this.messageEmitter.emitStopHookLoop(stopHookIterationCount, stopHookReasons, stopHookCount);
            }
            const continuation = await this.#runStopContinuation(pendingSend, continuationPromptId, promptId, continueParts, stopHookIterationCount > 1 || (guardContinuation?.attempt ?? 0) > 1, {
                ...
                ...(externalReason && stopHookIterationCount > 1 && guardContinuation
                    ? {
                        onAutomaticContinuationValidated: () => this.messageEmitter.emitStopHookLoop(stopHookIterationCount, stopHookReasons, stopHookCount),
                    }
                    : {}),
                onFullTurnModel,
                getModelOverride: () => modelOverride,
    ...
    async #runStopContinuation(pendingSend, streamPromptId, toolPromptId, parts, skipCompression, options = {}) {
        ...
        const preservePendingMessage = (message) => {
            if (initialSend)
                return;
            const preservedParts = (message.parts ?? []).filter((part) => !('text' in part && isTodoStopGuardPromptText(part.text)));
            this.#preserveUnsentMessageHistory(preservedParts.length > 0
        ...
        while (nextMessage !== null) {
            ...
            let preservePreparedMessageOnSkippedSend = !guardForThisSend &&
                preparedMessage.some((part) => !('text' in part && isTodoStopGuardPromptText(part.text)));
            const externalParts = initialSend ? options.externalParts : undefined;
            const promptIdForSend = guardForThisSend &&
            ...
            try {
                const sendResult = await this.#sendMessageStreamWithAutoCompression(promptIdForSend, nextMessage.parts ?? [], pendingSend.signal, {
                        ...
                        ? async () => {
                            ...
                            if (drained.parts.length > 0) {
                                ...
                                preparedMessage = initialSend
                                    ? drained.parts
                                    : [
                                        ...(nextMessage?.parts ?? []).filter((part) => !('text' in part &&
                            ...
                            if (!drained.reliable) {
                                ...
                                if (!initialSend && nextMessage) {
                                    nextMessage = {
                                        ...nextMessage,
                                        parts: (nextMessage.parts ?? []).filter((part) => !('text' in part &&
                                            isTodoStopGuardPromptText(part.text))),
                                    };
                                }
                            }
                            else if (drained.hasQueuedPrompt) {
                                const probe = await this.#claimTodoStopGuardContinuation(pendingSend.signal);
                                if (probe === 'queued') {
                                    guardForThisSend = undefined;
                                    nextGuardContinuation = undefined;
                                    preserveGuardOnSkippedSend = true;
                            ...
                            if (guardForThisSend) {
                                ...
                                const refreshedDecision = guardForThisSend.toolClosure
                                    ? this.todoStopGuard.decideToolClosure(guardForThisSend.attempt - 1, hasRelevantBackgroundInput)
                                    : this.todoStopGuard.decide(hasRelevantBackgroundInput);
                                if (refreshedDecision.kind !== 'continue' ||
                            ...
                            if (!automaticContinuationValidated &&
                            ...
                            preservePreparedMessageOnSkippedSend = preparedMessage.some((part) => !('text' in part && isTodoStopGuardPromptText(part.text)));
                            return { kind: 'send', message: preparedMessage };
                        }
                        : undefined,
                        ...
                        if (guardForThisSend && compressionFailed) {
                            ...
                            preparedMessage =
                                initialSend && externalParts
                                    ? externalParts
                                    : preparedMessage.filter((part) => !('text' in part &&
                                        isTodoStopGuardPromptText(part.text)));
                            preservePreparedMessageOnSkippedSend = true;
                        }
                        if (!automaticContinuationValidated &&
                ...
                if (!sendResult.responseStream) {
                    ...
                    const preservedParts = (messageForPreservation.parts ?? []).filter((part) => !('text' in part && isTodoStopGuardPromptText(part.text)));
                    this.#preserveUnsentMessageHistory(preservedParts.length > 0
                        ? { ...messageForPreservation, parts: preservedParts }
                        : null, sendResult.stopReason === 'cancelled' ||
                ...
                const responseStream = sendResult.responseStream;
                nextMessage = null;
                channelDeliveryResponseBlock = beginChannelDeliveryResponseBlock(options.responseCapture);
                channelDeliveryCheckpoint =
                    channelDeliveryResponseBlock?.parts.length ?? 0;
                initialSend = false;
                if (guardForThisSend) {
                    const guardCommitted = this.todoStopGuard.commitContinuation(guardForThisSend.attempt);
                    if (guardCommitted) {
                ...
                for await (const response of responseStream) {
                    ...
                    if (response.type === StreamEventType.CHUNK &&
                        ...
                        options.responseCapture?.agentOutput.observeFinishReason(candidate.finishReason);
                    }
                    if (response.type === StreamEventType.CHUNK &&
                    ...
                    if (response.type === StreamEventType.RETRY ||
                        response.type === StreamEventType.MODEL_FALLBACK) {
                        options.responseCapture?.agentOutput.restartAttempt(response.type === StreamEventType.RETRY &&
                            response.isContinuation === true);
                        if (response.type === StreamEventType.MODEL_FALLBACK ||
                            !response.isContinuation) {
                            rewindChannelDeliveryResponseBlock(channelDeliveryResponseBlock, channelDeliveryCheckpoint);
                        }
                        await finalizeToolCallPreparations(preparationTracker, true, `daemon continuation ${response.type}`);
                        functionCalls.length = 0;
                    }
                }
            }
                ...
                if (preservedParts.length > 0 &&
                    (!providerSendChat ||
                        (providerSendChat.getUserContentPushCount?.() ?? 0) <=
                            userContentPushCountBeforeSend)) {
                    this.#preserveUnsentMessageHistory({ ...messageForPreservation, parts: preservedParts }, true);
                }
                const isControlledCancellation = pendingSend.signal.aborted &&
                    (pendingSend.signal.reason === USER_CANCEL_ABORT_REASON ||
                ...
                try {
                    await finalizeToolCallPreparations(preparationTracker, streamFailed || pendingSend.signal.aborted, 'daemon continuation');
                }
                finally {
                    await messageDisplay?.finish();
                }
            }
            commitChannelDeliveryResponseBlock(options.responseCapture, channelDeliveryResponseBlock, functionCalls.length > 0);
            if (usageMetadata) {
                this.#recordPromptTokenCount(usageMetadata);
                const durationMs = Date.now() - streamStartTime;
                await this.messageEmitter.emitUsageMetadata(usageMetadata, '', durationMs);
            ...
            if (functionCalls.length > 0) {
                const toolRun = await this.#runWithFullTurnModel(options.getModelOverride?.(), () => this.runToolCalls(pendingSend.signal, toolPromptId, functionCalls, toolLoopState, options.onFullTurnModel));
                ...
                const nextAfterTools = await this.#buildNextMessageAfterToolRun(toolRun, pendingSend.signal, toolPromptId, toolLoopState, options.onFullTurnModel, options.rejectOnLoopDetected ?? false);
                nextMessage = nextAfterTools.message;
                ...
                if (guardForThisSend && nextMessage) {
                    const nextDecision = this.todoStopGuard.decideToolClosure(guardForThisSend.attempt, this.#hasRelevantTodoStopGuardBackgroundInput());
                    if (nextDecision.kind === 'continue' &&
                    ...
                    else if (nextDecision.kind === 'continue' &&
                        nextDecision.attempt <= guardForThisSend.attempt) {
                        nextGuardContinuation = undefined;
                    }
    ...
    async #emitTodoStopGuardContinuation(state) {
        await this.#emitTodoStopGuardMessageSafely(`[Todo Stop Guard] Automatic continuation ${state.attempt}/${state.maxAttempts} started; ${state.unfinishedCount} todo item(s) remain unfinished.`, state);
    }
    async #emitTodoStopGuardExhausted(state) {
        if (!this.todoStopGuard.markExhaustionReported())
            return;
        await this.#emitTodoStopGuardMessageSafely(`[Todo Stop Guard] Automatic continuation stopped after ${state.maxAttempts} attempts; ${state.unfinishedCount} todo item(s) remain unfinished.`, state);
    }
    async #emitTodoStopGuardMessageSafely(text, state) {
        try {
            ...
            debugLogger.warn(`Failed to emit Todo Stop Guard status: ${this.#formatError(error)}`);
        }
    }
    ...
    #captureTodoPlanRevision(update) {
    ...
    #scheduleChannelDelivery(params) {
        ...
        const timer = setTimeout(() => {
            void this.client
                ...
                .catch((error) => {
                try {
                    debugLogger.warn(`Channel delivery submission failed: ${error instanceof Error ? error.message : String(error)}`);
                }
                catch {
    ...
    #beginTurnRecording(params, invocationContext) {
        if (!invocationContext)
            return null;
        const promptMetadata = params
            ._meta;
        const rawPromptDisplayText = promptMetadata?.[DAEMON_PROMPT_DISPLAY_TEXT_META_KEY];
        ...
        const promptDisplayText = typeof rawPromptDisplayText === 'string' && rawPromptDisplayText !== ''
            ? rawPromptDisplayText
            : undefined;
        const { text, truncated } = truncateTurnText(promptDisplayText ?? extractTurnPromptText(params.prompt));
    ...
    #settleTurnRecording(state, recording, response, error) {
        ...
        const stopReason = response?.stopReason ?? (state === 'cancelled' ? 'cancelled' : undefined);
        const payload = {
        ...
        try {
            ...
            debugLogger.warn(`Failed to record turn result: ${this.#formatError(recordError)}`);
        }
    }
    #getCurrentChat() {
     ...
     */
    #createMessageDisplayDispatcher(signal) {
        const messageBus = this.config.getMessageBus?.();
        if (this.config.getDisableAllHooks?.() ||
            !messageBus ||
        ...
        return new MessageDisplayDispatcher(messageBus, signal, (message) => debugLogger.warn(message));
    }
    /**
    ...
    async #sendMessageStreamWithAutoCompression(promptId, message, abortSignal, options = {}) {
        ...
        if (abortSignal.aborted) {
            debugLogger.debug(`Send aborted after pre-compression preparation for prompt ${promptId}`);
        ...
        if (!options.skipCompression &&
            ...
            try {
                const compressed = await geminiClient.tryCompressChat(promptId, false, abortSignal);
                compressionInfo = compressed;
                this.#recordCompressionTokenCount(compressed);
                compressionFailed = isCompressionFailureStatus(compressed.compressionStatus);
                if (compressed.compressionStatus === CompressionStatus.COMPRESSED) {
                    ...
                    const reasonClause = compressed.triggerReason === 'image_overflow'
                ...
                debugLogger.warn(`Auto-compression failed for prompt ${promptId}; proceeding without compression: ` +
        ...
        if (sessionTokenLimit > 0) {
            ...
            if (lastPromptTokenCount > sessionTokenLimit) {
                debugLogger.warn(`Session token limit exceeded for prompt ${promptId}: ` +
                    `${lastPromptTokenCount} > ${sessionTokenLimit}. Send dropped.`);
                await this.#emitAgentDiagnosticMessageSafely(`Session token limit exceeded: ${lastPromptTokenCount} tokens > ${sessionTokenLimit} limit. ` +
                    'Please start a new session or increase the sessionTokenLimit in your settings.json.', `Failed to emit token limit diagnostic for prompt ${promptId}`);
                return { responseStream: null, stopReason: 'max_tokens' };
            }
        }
        if (compressionDiagnostic) {
            await this.#emitAgentDiagnosticMessageSafely(compressionDiagnostic, `Failed to emit compression notification for prompt ${promptId}`);
        }
        if (abortSignal.aborted) {
            debugLogger.debug(`Send aborted after compression diagnostic for prompt ${promptId}`);
            return { responseStream: null, stopReason: 'cancelled' };
        }
        ...
        if (abortSignal.aborted) {
            debugLogger.debug(`Send aborted after pre-send validation for prompt ${promptId}`);
            return { responseStream: null, stopReason: 'cancelled' };
        }
        const chat = this.#getCurrentChat();
        const model = options.getModelOverride?.() ??
    ...
    #preserveUnsentMessageHistory(message, preserveFullMessage) {
        if (!message)
            return;
        ...
        const functionResponseParts = message.parts?.filter((part) => 'functionResponse' in part && part.functionResponse) ?? [];
        const droppedParts = (message.parts?.length ?? 0) - functionResponseParts.length;
        if (droppedParts > 0) {
            debugLogger.debug(`Dropping ${droppedParts} non-functionResponse part(s) from unsent ACP message after send was skipped.`);
        }
    ...
    async #preserveStoppedToolRun(toolRun, abortSignal) {
        ...
        const midTurnParts = abortSignal.aborted
            ? await this.#buildMidTurnParts(this.#takeRecoveredMidTurnMessages(), abortSignal, { preserveFallbackOnAbort: true })
    ...
    async #buildNextMessageAfterToolRun(toolRun, abortSignal, promptId, toolLoopState, onFullTurnModel, rejectOnLoopDetected = false) {
        if (toolRun.loopDetected) {
        ...
        const previousRepeatedToolFailureState = toolLoopState.repeatedToolFailureState;
        const repeatedToolFailureBatch = toolRun.repeatedToolFailureBatch ?? {
            complete: false,
            observations: [],
        };
        const repeatedToolFailureDecision = reduceRepeatedToolFailureGuard(previousRepeatedToolFailureState, {
        ...
        recordRepeatedToolFailureDecision(promptId, toolLoopState.repeatedToolFailureMode, previousRepeatedToolFailureState, repeatedToolFailureDecision, repeatedToolFailureBatch);
        ...
        if (repeatedToolFailureDecision.kind !== 'none') {
            ...
            debugLogger.debug(`[repeated-tool-failure-guard] mode=${toolLoopState.repeatedToolFailureMode} decision=${repeatedToolFailureDecision.kind} phase=${state.phase} candidate=${state.candidateOrdinal} failures=${state.failureCount} batches=${state.batchCount}`);
        }
        const parts = [
            ...toolRun.parts,
            ...(activeTodoReminder ? [{ text: activeTodoReminder }] : []),
        ...
        if (repeatedToolFailureDecision.kind === 'stop') {
            ...
            await this.messageRewriter?.waitForPendingRewrites();
            recordDaemonLoopDetected(this.config, promptId, LoopType.REPEATED_TOOL_EXECUTION_FAILURE, REPEATED_TOOL_FAILURE_STOP_MESSAGE, toolLoopState, { recordToQwenLogger: false });
            if (!rejectOnLoopDetected) {
                ...
                try {
                    await this.messageEmitter.emitAgentMessage(REPEATED_TOOL_FAILURE_STOP_MESSAGE);
                }
                catch (error) {
                    debugLogger.warn(`Failed to emit repeated tool failure stop message: ${this.#formatError(error)}`);
                }
            }
            return {
                message: null,
    ...
    #recordPromptTokenCount(usageMetadata) {
        this.#syncPromptTokenCountWithCurrentChat();
        const tokenCount = usageMetadata.promptTokenCount ?? usageMetadata.totalTokenCount;
    ...
    #extractCompressionTokenCount(info) {
        if (!info) {
            return null;
        }
    ...
    #syncPromptTokenCountWithCurrentChat() {
        const chat = this.#getCurrentChat();
        if (this.lastPromptTokenCountChat &&
    ...
    async #emitAgentDiagnosticMessageSafely(text, failureContext) {
        try {
    ...
    async #drainMidTurnInput(abortSignal, options = {}) {
        ...
        try {
            ...
            let timeoutHandle;
            const timeoutPromise = new Promise((_, reject) => {
                timeoutHandle = setTimeout(() => reject(new MidTurnDrainTimeoutError()), MID_TURN_QUEUE_DRAIN_TIMEOUT_MS);
            });
            let response;
            try {
            ...
            const reliable = isValidMidTurnDrainResponse(response, options.watchQueuedPrompt === true);
            return {
                parts: await this.#buildMidTurnParts([...recovered, ...parseMidTurnDrainResponse(response)], abortSignal, options),
                hasQueuedPrompt: isRecord(response) && response['hasQueuedPrompt'] === true,
                reliable,
            };
        }
        catch (error) {
            ...
            if (isTimeout) {
                ...
                // rejection.
                if (drainPromise)
                    void this.#recoverLateDrain(drainPromise);
            }
            ...
            const isPermanentError = errorCode === -32601 ||
            ...
            debugLogger.warn(`Mid-turn queue drain ${isPermanentError ? 'permanently ' : ''}unavailable [session ${this.sessionId}]: ${errorMessage}`);
    ...
    #takeRecoveredMidTurnMessages() {
        if (this.midTurnRecoveredMessages.length === 0)
            return [];
    ...
    async #recoverLateDrain(pending) {
        ...
        if (late === expired) {
            debugLogger.warn(`[mid-turn] dropped a drain response that arrived after the ${MID_TURN_QUEUE_RECOVERY_TIMEOUT_MS}ms recovery deadline [session ${this.sessionId}]`);
            return;
        }
        const lateMessages = parseMidTurnDrainResponse(late);
        if (lateMessages.length > 0) {
            debugLogger.debug(`[mid-turn] recovered ${lateMessages.length} message(s) from a timed-out drain [session ${this.sessionId}]`);
    ...
    async #buildMidTurnParts(messages, abortSignal, options = {}) {
        ...
        for (const message of messages) {
            ...
            try {
                rawParts =
                    message.kind === 'text'
                        ...
                        : await withTimeoutSignal(abortSignal, MID_TURN_QUEUE_RESOLVE_TIMEOUT_MS, (signal) => this.#resolvePrompt(message.content, signal, {
                            onFullTurnModel: options.onFullTurnModel,
                        }));
            }
            catch (messageError) {
            ...
            if (message.kind === 'structured' && message.attachmentReferences) {
                const everyAttachmentBlockHasAReference = message.attachmentReferences.length ===
                    message.content.filter((block) => block.type === 'image' || block.type === 'resource').length;
                if (everyAttachmentBlockHasAReference) {
                    recorder?.recordMidTurnUserMessage(stripReferencedAttachmentDataParts(built, message.content), displayText, undefined, message.attachmentReferences);
                }
                else {
    ...
    async #startCronSchedulerIfNeeded() {
        ...
        if (!this.config.isCronEnabled())
            return;
        if (this.cronDisabledByTokenLimit)
            return;
        ...
        // Durable tasks live under ~/.lailatulcoder (user-owned, not in the working
        ...
        try {
            ...
            debugLogger.warn(`Durable cron init failed — persistent tasks will not fire in this session: ${err}`);
        }
    ...
    #startCronSchedulerInRuntime() {
        return Storage.runWithRuntimeBaseDir(this.runtimeBaseDir, this.config.getWorkingDir(), () => this.#startCronSchedulerIfNeeded());
    }
    #stopCronSchedulerInRuntime() {
        Storage.runWithRuntimeBaseDir(this.runtimeBaseDir, this.config.getWorkingDir(), () => this.config.getCronScheduler().stop());
    }
    #enqueueCronPrompt(item) {
        const automaticWorkDeferred = this.todoStopGuard.blocksUnrelatedAutomaticTurns ||
            this.todoStopGuardQueuedPromptPriority;
        if (automaticWorkDeferred) {
            ...
            if (!incomingIsRelated && item.taskId) {
                const duplicateIndex = this.cronQueue.findIndex((queued) => queued.taskId === item.taskId &&
            ...
            const maxBeforeAppend = MAX_DEFERRED_UNRELATED_CRON_QUEUE -
                (shouldAppend && !incomingIsRelated ? 1 : 0);
            let unrelatedCount = this.cronQueue.filter((queued) => !this.#cronContinuesTodoStopGuardWorkChain(queued)).length;
            const evictedTaskIds = [];
            while (unrelatedCount > maxBeforeAppend) {
                const evictedIndex = this.cronQueue.findIndex((queued) => !this.#cronContinuesTodoStopGuardWorkChain(queued));
                if (evictedIndex < 0)
                    break;
            ...
            if (evictedTaskIds.length > 0) {
                debugLogger.warn(`Cron queue overflow while automatic work is deferred: evicted ${evictedTaskIds.length} unrelated task(s): ${evictedTaskIds.join(', ')}`);
            }
            if (!shouldAppend)
                return;
        }
    ...
    async #drainCronQueue() {
        ...
        if (this.notificationProcessing)
            return;
        if (this.#deferAutomaticQueueDrainUntilTurnsSettle())
            return;
        if (this.#nextCronQueueIndex() < 0)
            return;
        await this.runExclusiveAutomaticHistoryMutation(() => this.#drainCronQueueExclusive());
    }
    async #drainCronQueueExclusive() {
        if (this.disposed || this.closing || this.cronProcessing)
            return;
        if (this.pendingPrompt || this.notificationProcessing)
            return;
        if (this.#deferAutomaticQueueDrainUntilTurnsSettle())
            return;
        if (this.#nextCronQueueIndex() < 0)
        ...
        try {
            ...
            debugLogger.warn(`Cron turn rejected [session ${this.sessionId}]: ${error instanceof Error ? error.message : String(error)}`);
            return;
        }
        if (this.disposed ||
            this.closing ||
        ...
        let resolveCompletion;
        this.cronCompletion = new Promise((resolve) => {
            resolveCompletion = resolve;
        });
        try {
    ...
    #nextCronQueueIndex() {
        if (this.cronQueue.length === 0)
            return -1;
        if (this.todoStopGuardQueuedPromptPriority)
            return -1;
        if (!this.todoStopGuard.blocksUnrelatedAutomaticTurns)
            return 0;
        return this.cronQueue.findIndex((item) => this.#cronContinuesTodoStopGuardWorkChain(item));
    }
    #getLoopTickResolver() {
        ...
        if (!this.loopTickResolver || this.loopTickResolverRoot !== root) {
            ...
            // rest of LailatulCoder honors QWEN_HOME for `.lailatulcoder`); reading raw os.homedir() here
            // would always hit the real `~/.lailatulcoder` and ignore a relocated config home.
            ...
            this.loopTickResolver = new LoopTickResolver({
                ...
                homeQwenDir,
                // The project `.lailatulcoder/loop.md` is repo-controlled, so an untrusted folder
                // must not read it and feed it to the model (mirrors getProjectHooks()'s
                // trust gate). The home/global `~/.lailatulcoder/loop.md` is user-owned and stays
    ...
    async #executeCronPrompt(item) {
        ...
        return runWithInvocationContext(undefined, () => sessionIdContext.run(this.config.getSessionId(), () => this.#executeCronPromptInner(item)));
    }
    async #executeCronPromptInner(item) {
        const { prompt } = item;
        return Storage.runWithRuntimeBaseDir(this.runtimeBaseDir, this.config.getWorkingDir(), async () => {
            const ac = new AbortController();
            ...
            await withInteractionSpan(this.config, {
                promptId,
                model: this.config.getModel(),
                messageType: 'cron',
            }, async () => {
                ...
                try {
                    ...
                    if (ac.signal.aborted)
                        return;
                    this.config.startAutomaticActiveTodoWorkChain(promptId, item.todoWorkChainId);
                    ...
                    if (loopMode) {
                        ...
                        try {
                            ...
                            // resolve() reads .lailatulcoder/loop.md (project or home/global); an
                            ...
                            debugLogger.warn(`loop.md sentinel resolution failed (mode=${loopMode}, code=${code}) — check .lailatulcoder/loop.md permissions/IO`, resolveErr);
                            if (loopMode === 'dynamic' &&
                                ...
                                // cycle.
                                loopTick = resolver.buildTransientErrorTick(loopMode, trustedAtResolve, code);
                            }
                            else {
                                ...
                                // label (never a hardcoded `~/.lailatulcoder`) and naming the project
                                ...
                                throw new Error(`loop.md resolution failed (${code}) for ${resolver.absentLocations(trustedAtResolve)}`);
                            }
                        }
                    }
                    ...
                    if (loopTick) {
                        debugLogger.debug(`loop tick: mode=${loopMode ?? autonomousMode} delivery=${loopTick.full
                            ? 'full'
                            : loopTick.transientError
                                ...
                                : loopTick.autonomous
                                    ...
                                    : loopTick.sourceLabel
                                        ? 'reminder'
                                        : 'absent'} source=${loopTick.sourceLabel ?? 'none'} autonomous=${loopTick.autonomous ?? false} transient=${loopTick.transientError ?? false}`);
                    }
                    ...
                    while (nextMessage !== null) {
                        ...
                        let usageMetadata = null;
                        const streamStartTime = Date.now();
                        const sendResult = await this.#sendMessageStreamWithAutoCompression(promptId, nextMessage.parts ?? [], ac.signal);
                        if (!sendResult.responseStream) {
                            this.todoStopGuard.suspend();
                            this.#preserveUnsentMessageHistory(nextMessage, sendResult.stopReason === 'cancelled');
                        ...
                        const responseStream = sendResult.responseStream;
                        const channelDeliveryResponseBlock = beginChannelDeliveryResponseBlock(responseCapture);
                        const channelDeliveryCheckpoint = channelDeliveryResponseBlock?.parts.length ?? 0;
                        ...
                        nextMessage = null;
                        const messageDisplay = this.#createMessageDisplayDispatcher(ac.signal);
                        let streamFailed = false;
                        try {
                            for await (const resp of responseStream) {
                                if (ac.signal.aborted) {
                                ...
                                if (resp.type === StreamEventType.RETRY ||
                                    ...
                                    await finalizeToolCallPreparations(preparationTracker, true, `cron/loop tick ${resp.type}`);
                                    functionCalls.length = 0;
                                }
                            }
                        }
                            ...
                            try {
                                await finalizeToolCallPreparations(preparationTracker, streamFailed || ac.signal.aborted, 'cron/loop tick');
                            }
                            finally {
                        ...
                        if (functionCalls.length > 0) {
                            const toolRun = await this.runToolCalls(ac.signal, promptId, functionCalls, toolLoopState);
                            if (toolRun.stopAfterPermissionCancel || ac.signal.aborted) {
                                this.todoStopGuard.suspend();
                                await this.#preserveStoppedToolRun(toolRun, ac.signal);
                                return;
                            }
                            const nextAfterTools = await this.#buildNextMessageAfterToolRun(toolRun, ac.signal, promptId, toolLoopState);
                            nextMessage = nextAfterTools.message;
                            if (toolRun.loopDetected) {
                                this.todoStopGuard.suspend();
                                await this.#preserveStoppedToolRun(toolRun, ac.signal);
                    ...
                    if (this.todoStopGuard.needsStopInspection) {
                        const guardStop = await this.#handleStopHookLoop(ac, promptId, false, undefined, false, undefined, responseCapture);
                    ...
                    await this.messageEmitter.emitAgentMessage(`[${item.source} error] ${msg}`);
                }
                finally {
    ...
    #stopCronAfterTokenLimit() {
        ...
        void this.#emitAgentDiagnosticMessageSafely('Cron jobs and loop wakeups disabled for the rest of this session due to token limit. Restart the session to re-enable.', 'Failed to emit cron-disabled diagnostic');
    }
    #registerBackgroundNotificationCallbacks() {
        ...
        backgroundRegistry.setNotificationCallback((displayText, modelText, meta) => {
            ...
            this.#enqueueBackgroundNotification({
                ...
                structured: entry
                    ? {
                        description: truncateNotificationLabel(buildBackgroundEntryLabel(entry)),
                    }
                    : undefined,
            });
        });
        ...
        monitorRegistry.setNotificationCallback((displayText, modelText, meta) => {
            ...
            this.#enqueueBackgroundNotification({
                ...
                status: meta.status,
                kind: 'monitor',
                continuesTodoStopGuardWorkChain: this.#monitorContinuesTodoStopGuardWorkChain(meta.monitorId, meta.ownerAgentId),
                toolUseId: meta.toolUseId,
                todoWorkChainId: meta.todoWorkChainId,
                structured: entry
                    ? {
        ...
        this.config
            ...
            void this.client
                .extNotification('LailatulCoder/notify/session/title-update', {
                v: 1,
                sessionId,
                title: customTitle,
                titleSource,
        ...
        if (typeof this.config.onChatRecordingFailure === 'function') {
            this.unsubscribeChatRecordingFailure = this.config.onChatRecordingFailure((event) => this.client.extNotification('LailatulCoder/notify/session/recording-degraded', {
                v: 1,
        ...
        while (this.notificationQueue.length >= MAX_NOTIFICATION_QUEUE) {
            ...
            if (this.todoStopGuard.blocksUnrelatedAutomaticTurns ||
                ...
                evictedIndex = this.notificationQueue.findIndex((queued) => !this.#notificationContinuesTodoStopGuardWorkChain(queued));
                if (evictedIndex < 0 && !incomingIsRelated) {
                    debugLogger.warn(`Notification queue overflow: dropping unrelated task=${item.taskId} kind=${item.kind} while automatic work is deferred`);
                    return;
                }
                if (evictedIndex < 0) {
                    debugLogger.warn(`Notification queue overflow: dropping related task=${item.taskId} kind=${item.kind} because all queued items are related`);
                    return;
                }
            }
            const [evicted] = this.notificationQueue.splice(evictedIndex, 1);
            debugLogger.warn(`Notification queue overflow: evicting task=${evicted?.taskId ?? 'unknown'} kind=${evicted?.kind ?? 'unknown'}`);
        }
        this.notificationQueue.push(item);
        this.#activeWorkChanged();
        void this.#drainNotificationQueue();
    }
    async enqueueBackgroundNotification(item) {
        ...
        if (existing)
            return { accepted: await existing };
        ...
        try {
            ...
            if (this.backgroundNotificationAcceptances.get(item.taskId) === acceptance) {
    ...
    async #persistDaemonBackgroundNotification(item) {
        if (this.disposed || this.closing)
            return false;
        const recording = this.config.getChatRecordingService();
        if (!recording)
            return false;
        try {
            await recording.recordNotificationStrict([{ text: item.modelText }], item.displayText, {
                taskId: item.taskId,
                status: item.status,
                kind: item.kind,
            ...
            debugLogger.warn(`Daemon notification persistence rejected [session ${this.sessionId}, task ${item.taskId}]: ${error instanceof Error ? error.message : String(error)}`);
            return false;
        }
        ...
        if (!this.disposed && !this.closing) {
            this.#enqueueBackgroundNotification({
                ...item,
                continuesTodoStopGuardWorkChain: this.#agentContinuesTodoStopGuardWorkChain(item.taskId),
                persisted: true,
            });
        }
        return true;
    ...
    async #drainNotificationQueue() {
        ...
        if (this.notificationQueue.length === 0)
            return;
        if (this.#nextNotificationQueueIndex() < 0)
            return;
        await this.runExclusiveAutomaticHistoryMutation(() => this.#drainNotificationQueueExclusive());
    }
    async #drainNotificationQueueExclusive() {
        if (this.disposed || this.closing || this.notificationProcessing)
            return;
        ...
        try {
            ...
            debugLogger.warn(`Notification turn rejected [session ${this.sessionId}]: ${error instanceof Error ? error.message : String(error)}`);
            return;
        }
        if (this.disposed ||
            this.closing ||
        ...
        let resolveCompletion;
        this.notificationCompletion = new Promise((resolve) => {
            resolveCompletion = resolve;
        });
        try {
            while (this.notificationQueue.length > 0) {
                ...
                this.#activeWorkChanged();
                try {
                    await runWithInvocationContext(undefined, () => sessionIdContext.run(this.config.getSessionId(), () => this.#executeBackgroundNotificationPromptInner(item)));
                }
                finally {
    ...
    #nextNotificationQueueIndex() {
        ...
        if (!this.todoStopGuard.blocksUnrelatedAutomaticTurns)
            return 0;
        return this.notificationQueue.findIndex((item) => this.#notificationContinuesTodoStopGuardWorkChain(item));
    }
            ...
            try {
                ...
                while (nextMessage !== null) {
                    ...
                    if (!sendResult.responseStream) {
                        this.todoStopGuard.suspend();
                        this.#preserveUnsentMessageHistory(nextMessage, sendResult.stopReason === 'cancelled');
                        await this.#emitBackgroundNotificationEndTurn(sendResult.stopReason);
                        return;
                    }
                    const responseStream = sendResult.responseStream;
                    nextMessage = null;
                    ...
                    try {
                        for await (const resp of responseStream) {
                            ...
                            if (resp.type === StreamEventType.CHUNK &&
                                ...
                                for (const part of candidate.content?.parts ?? []) {
                                    if (!part.text)
                                        continue;
                                    if (part.thought) {
                                        await this.messageEmitter.emitMessage(part.text, 'assistant', true);
                                    }
                                    else {
                            ...
                            if (resp.type === StreamEventType.RETRY ||
                                resp.type === StreamEventType.MODEL_FALLBACK) {
                                await finalizeToolCallPreparations(preparationTracker, true, `background notification ${resp.type}`);
                                functionCalls.length = 0;
                            }
                        }
                    }
                        ...
                        try {
                            await finalizeToolCallPreparations(preparationTracker, streamFailed || ac.signal.aborted, 'background notification');
                        }
                        finally {
                    ...
                    if (responseText.length > 0) {
                        await this.#emitBackgroundNotificationResponse(item, responseText, ac.signal);
                    }
                    if (this.messageRewriter) {
                    ...
                    if (functionCalls.length > 0) {
                        ...
                        if (toolRun.stopAfterPermissionCancel || ac.signal.aborted) {
                            this.todoStopGuard.suspend();
                            await this.#preserveStoppedToolRun(toolRun, ac.signal);
                            await this.#emitBackgroundNotificationEndTurn(getAbortAwareEndTurnStopReason(ac.signal));
                            return;
                        }
                        const nextAfterTools = await this.#buildNextMessageAfterToolRun(toolRun, ac.signal, promptId, toolLoopState);
                        nextMessage = nextAfterTools.message;
                ...
                let stopReason = 'end_turn';
                if (this.todoStopGuard.needsStopInspection) {
                    stopReason = (await this.#handleStopHookLoop(ac, promptId, false, undefined, false)).stopReason;
                }
                await this.#emitBackgroundNotificationEndTurn(ac.signal.aborted ? 'cancelled' : stopReason);
            }
            catch (error) {
                if (ac.signal.aborted) {
                    this.todoStopGuard.suspend();
                ...
                const msg = error instanceof Error ? error.message : String(error);
                try {
                    await this.messageEmitter.emitAgentMessage(`[notification error] ${msg}`);
                }
                catch (emitError) {
                    debugLogger.error('Failed to emit background notification error:', emitError);
                }
                finally {
    ...
    async #emitBackgroundNotificationResponse(item, text, signal) {
        const update = {
            sessionUpdate: 'agent_message_chunk',
            content: { type: 'text', text },
            _meta: {
    ...
    async #emitBackgroundNotificationEndTurn(reason) {
        try {
            ...
            debugLogger.debug(`Background notification end-turn extNotification dropped: ${this.#formatError(error)}`);
        }
    }
    /**
    ...
    async #emitGoalStartTurn() {
        try {
            ...
            debugLogger.debug(`Goal start-turn extNotification dropped: ${this.#formatError(error)}`);
        }
    }
    async #emitGoalEndTurn(result) {
        try {
            await this.client.extNotification('_qwencode/end_turn', {
                sessionId: this.sessionId,
            ...
            debugLogger.debug(`Goal end-turn extNotification dropped: ${this.#formatError(error)}`);
        }
    }
    ...
    async refreshSkillsFromSettings(options = {}) {
        ...
        if (skillManager && (options.notifyConfigChanged ?? true)) {
            try {
                ...
                if (!updateFailed)
                    throw error;
                debugLogger.error('SkillManager refresh failed after command update failure:', error);
            }
        }
        if (updateFailed)
            throw updateError;
    ...
    async sendAvailableCommandsUpdateOrThrow() {
        const { availableCommands, availableSkills, availableSkillDetails } = await buildAvailableCommandsSnapshot(this.config, undefined, this.settings);
        const update = {
    ...
    #requestPermissionQueued(params, signal) {
        const prior = permissionRequestTails.get(this.client) ?? Promise.resolve();
        const transportRequest = prior.then(() => signal.aborted
        ...
        const tail = Promise.race([
            transportRequest.then(() => undefined, () => undefined),
            new Promise((resolve) => {
                if (signal.aborted)
                    return resolve();
        ...
        return requestPermissionWithAbort({ requestPermission: () => transportRequest }, params, signal);
    }
    /**
    ...
    async setMode(params) {
        ...
        if (approvalMode === undefined) {
            throw RequestError.invalidParams(undefined, `Unknown approval mode: ${params.modeId}`);
        }
        ...
        void this.client
            .extNotification('LailatulCoder/notify/session/mode-update', {
            v: 1,
            sessionId: this.sessionId,
     ...
     */
    async setModel(params, options = {}) {
        const rawModelId = params.modelId.trim();
        if (!rawModelId) {
            throw RequestError.invalidParams(undefined, 'modelId cannot be empty');
        }
        const resolvedRoute = resolveAcpModelOption(rawModelId, this.config.getAllConfiguredModels());
        if (!resolvedRoute && rawModelId.startsWith(ACP_ROUTE_ID_PREFIX)) {
            throw RequestError.invalidParams(undefined, `Unknown or stale model route: "${rawModelId}"`);
        }
        ...
        if (!selectedAuthType) {
            throw RequestError.invalidParams(undefined, `authType cannot be determined for modelId "${parsed.modelId}"`);
        }
        const requireCachedCredentials = selectedAuthType !== previousAuthType &&
            selectedAuthType === AuthType.NEVER_MATCH_THIS;
        const switchOptions = resolvedRoute?.baseUrl !== undefined || requireCachedCredentials
            ? {
            ...
            : undefined;
        await this.config.switchModel(selectedAuthType, parsed.modelId, switchOptions);
        ...
        const currentAcpModelId = getCurrentAcpModelId(buildAcpModelOptions(this.config.getAllConfiguredModels()), activeRuntimeSnapshot?.id ?? effectiveModelId, activeRuntimeSnapshot?.authType ?? effectiveAuthType, activeRuntimeSnapshot
            ? undefined
            : resolvedRoute
        ...
        void this.client
            .extNotification('LailatulCoder/notify/session/model-update', {
            v: 1,
            sessionId: this.sessionId,
        ...
        if (options.persistDefault ?? true) {
            ...
            this.settings.setValue(persistScope, 'model.name', resolvedRoute?.isRuntime ? resolvedRoute.modelId : effectiveModelId);
            this.settings.setValue(persistScope, 'model.baseUrl', resolvedRoute && !resolvedRoute.isRuntime
                ? (resolvedRoute.baseUrl ?? '')
                : '');
            this.settings.setValue(persistScope, 'security.auth.selectedType', effectiveAuthType);
        }
        return {
            _meta: {
                qwenModelSwitch: {
    ...
    async sendCurrentModeUpdateNotification() {
        ...
        try {
            await this.client.extNotification('LailatulCoder/notify/session/mode-update', {
                v: 1,
                sessionId: this.sessionId,
                currentModeId: newModeId,
                legacyFrameSent,
    ...
    async runToolCalls(abortSignal, promptId, functionCalls, toolLoopState, onFullTurnModel) {
        ...
        const executionCallIds = new Map(dedupedFunctionCalls.map((functionCall, index) => [
            functionCall,
            functionCall.id ??
        ...
        const queueToolResultRecord = (fc, record) => {
        ...
        const finalizeRunToolResult = async (result) => {
            const orderedRecords = [...pendingToolResultRecords].sort((left, right) => left.ordinal - right.ordinal || left.sequence - right.sequence);
            const repeatedToolFailureBatch = {
                complete: orderedRecords.length === dedupedFunctionCalls.length &&
            ...
            const finalized = await finalizeToolResponses(this.config, orderedRecords.map((record) => ({
                callId: record.callId,
        ...
        const recordSkippedToolCall = async (fc, message = PERMISSION_CANCEL_SKIP_MESSAGE, emitStart = true, errorType) => {
        ...
        // mutation.
        const handledToolCallFingerprints = new Map(this.#getCurrentChat().getHistoryToolCallFingerprints());
        const isReplayOfHandledCall = (fc) => {
            const providerCallId = getProviderToolCallId(fc) ?? fc.id;
            return providerCallId
                ? isReplayOfHandledToolCall(handledToolCallFingerprints, providerCallId, getFunctionCallFingerprint(fc))
                : false;
        };
        const repeatedDuplicateCall = findRepeatedDuplicateProviderToolCall(dedupedFunctionCalls, (fc) => getProviderToolCallId(fc) ?? fc.id, isReplayOfHandledCall, this.duplicateProviderToolCallResponseIds);
        if (repeatedDuplicateCall) {
            const providerCallId = getProviderToolCallId(repeatedDuplicateCall) ??
                repeatedDuplicateCall.id;
            const message = `Stopping ACP turn after repeated duplicate provider tool-call id: ` +
                `${providerCallId} (tool: ${repeatedDuplicateCall.name ?? 'unknown_tool'}).`;
            if (toolLoopState) {
                recordDaemonLoopDetected(this.config, promptId, LoopType.GLOBAL_TOOL_CALL_DUPLICATE, message, toolLoopState);
            }
            else {
                debugLogger.warn(message);
            }
            await Promise.all(dedupedFunctionCalls.map((fc) => recordSkippedToolCall(fc, LOOP_DETECTED_SKIP_MESSAGE, false, ToolErrorType.UNKNOWN)));
        ...
        const pushDuplicateBatch = (fc, request) => {
            const providerCallId = request.providerCallId ?? request.callId;
            markDuplicateProviderToolCallResponseSent(providerCallId, this.duplicateProviderToolCallResponseIds);
            const response = createDuplicateProviderToolCallResponse(request);
            debugLogger.debug(`[Session.runToolCalls] Suppressing duplicate provider tool-call id: ` +
                `${providerCallId} (tool: ${request.name})`);
            batches.push({ kind: 'duplicate', fc, request, response });
        };
        const emitDuplicateBatch = async (batch) => {
            ...
            try {
                ...
                debugLogger.debug('[Session.runToolCalls] Failed to emit duplicate tool update', emitError);
            }
        ...
        for (const fc of dedupedFunctionCalls) {
            ...
            if (providerCallId) {
                ...
                recordHandledToolCall(handledToolCallFingerprints, providerCallId, getFunctionCallFingerprint(fc));
            }
        ...
        const executableCalls = batches.flatMap((batch) => batch.kind === 'execute' ? batch.calls : []);
        ...
        if (recordDaemonToolCalls(this.config, promptId, toolLoopState, executableCalls)) {
            return await finalizeRunToolResult({
                parts: await Promise.all(dedupedFunctionCalls.map((fc) => recordSkippedToolCall(fc, LOOP_DETECTED_SKIP_MESSAGE, false, ToolErrorType.UNKNOWN))),
                stopAfterPermissionCancel: false,
                loopDetected: true,
            });
        }
        const planModeEntryBoundaryIndex = findPlanModeEntryBatchBoundaryIndex(executableCalls.map((call) => call.name));
        const planModeEntryBoundary = planModeEntryBoundaryIndex === undefined
            ? undefined
            : executableCalls[planModeEntryBoundaryIndex];
        const appendSkippedAfter = async (parts, fc, message = PERMISSION_CANCEL_SKIP_MESSAGE, errorType) => {
            const startIndex = dedupedFunctionCalls.indexOf(fc) + 1;
            for (const remainingCall of dedupedFunctionCalls.slice(startIndex)) {
                parts.push(await recordSkippedToolCall(remainingCall, message, true, errorType));
            }
        };
        const memoryWriteCandidates = [];
        const collectMemoryWriteCandidates = (result) => {
            if (result.memoryWriteCandidates) {
                memoryWriteCandidates.push(...result.memoryWriteCandidates);
            }
        };
        const refreshMemoryIfNeeded = async () => {
            ...
            const matchedContextFileWrite = didWriteProjectContextFile(memoryWriteCandidates, this.config.getProjectRoot());
            debugLogger.debug(`ACP session ${this.sessionId} checked marked context-file memory tool batch; matched=${matchedContextFileWrite}`);
            if (!matchedContextFileWrite) {
                return;
            }
            debugLogger.debug(`ACP session ${this.sessionId} refreshing memory after context-file memory write`);
        ...
        const runBounded = async (calls, runAbortSignal, onStopAfterPermissionCancel, shouldSkipUnstarted) => {
            const maxConcurrency = parsePositiveIntegerEnv(process.env['QWEN_CODE_MAX_TOOL_CONCURRENCY'], 10);
            const results = new Array(calls.length);
            const executing = new Set();
            const fillLoopSkippedFrom = async (startIndex) => {
                for (let i = startIndex; i < calls.length; i++) {
                    if (results[i])
                        continue;
                    results[i] = {
                        parts: [
                            await recordSkippedToolCall(calls[i], LOOP_DETECTED_SKIP_MESSAGE, true, ToolErrorType.UNKNOWN),
                        ],
                        stopAfterPermissionCancel: false,
                        loopDetected: true,
                    };
            ...
            for (let i = 0; i < calls.length; i++) {
                ...
                const p = this.runTool(runAbortSignal, promptId, calls[idx], onStopAfterPermissionCancel, toolLoopState, recordSkippedToolCall, queueToolResultRecord, executionCallIds.get(calls[idx]), onFullTurnModel)
                    .then((r) => {
                    results[idx] = r;
                    if (r.loopDetected &&
                        executing.size > 1 &&
                        ...
                        debugLogger.warn(`Loop detection stopped this ACP turn; waiting for ${executing.size - 1} in-flight tool call(s) to settle before returning.`);
                    }
                })
                    .finally(() => {
                    executing.delete(p);
                ...
                if (executing.size >= maxConcurrency) {
                    ...
                    const invalidToolErrorNearThreshold = toolLoopState &&
                        [...toolLoopState.invalidToolParamErrors.values()].some((count) => count >= DAEMON_INVALID_TOOL_PARAMS_THRESHOLD - 1);
        ...
        try {
            for (const batch of batches) {
                ...
                if (planModeEntryBoundary &&
                    !batch.calls.includes(planModeEntryBoundary)) {
                    for (const fc of batch.calls) {
                        parts.push(await recordSkippedToolCall(fc, PLAN_MODE_ENTRY_SIBLING_SKIP_MESSAGE, true, ToolErrorType.EXECUTION_DENIED));
                    }
                    continue;
                }
                if (batch.concurrent && batch.calls.length > 1) {
                    ...
                    let results;
                    try {
                        results = await runBounded(batch.calls, batchAbortController.signal, stopBatchAfterPermissionCancel, () => batchStopAfterPermissionCancel);
                    }
                    finally {
                    ...
                    if (shouldStopForLoop) {
                        await appendSkippedAfter(parts, batch.calls[batch.calls.length - 1], LOOP_DETECTED_SKIP_MESSAGE, ToolErrorType.UNKNOWN);
                        return await finalizeRunToolResult({
                            parts,
                            stopAfterPermissionCancel: false,
                            loopDetected: true,
                    ...
                    for (const fc of batch.calls) {
                        const r = await this.runTool(abortSignal, promptId, fc, undefined, toolLoopState, recordSkippedToolCall, queueToolResultRecord, executionCallIds.get(fc), onFullTurnModel);
                        parts.push(...r.parts);
                        collectMemoryWriteCandidates(r);
                        if (r.loopDetected) {
                            await appendSkippedAfter(parts, fc, LOOP_DETECTED_SKIP_MESSAGE, ToolErrorType.UNKNOWN);
                            return await finalizeRunToolResult({
                                parts,
                                stopAfterPermissionCancel: false,
                                loopDetected: true,
    ...
    async runTool(abortSignal, promptId, fc, onStopAfterPermissionCancel, toolLoopState, recordSkippedToolCall, queueToolResultRecord, generatedCallId, onFullTurnModel) {
            ...
            return {
                parts: [
                    recordSkippedToolCall
                        ? await recordSkippedToolCall(fc, LOOP_DETECTED_SKIP_MESSAGE, false, ToolErrorType.UNKNOWN)
                        : {
                            functionResponse: {
                                id: callId,
                                name: fc.name ?? 'unknown_tool',
        ...
        const errorResponse = (error, toolName, status, errorType) => {
            ...
            try {
                ...
                debugLogger.debug('[Session.runTool] Failed to record terminal tool telemetry', telemetryError);
            }
            return [
                {
                    functionResponse: {
        ...
        const earlyErrorResponse = async (error, toolName = fc.name ?? 'unknown_tool', opts) => {
            ...
            const errorParts = errorResponse(error, toolName, opts.status, opts.errorType);
            if (toolName !== ToolNames.TODO_WRITE) {
                try {
                    ...
                    debugLogger.debug('[Session.runTool] Failed to emit terminal tool update', emitError);
                }
            }
                ...
                observeToolResultBoundary({
                    ...
                    artifacts: [
                        opts.settledMetadata
                            ? toolResultBoundaryArtifact(opts.settledMetadata.persistedOutputFiles, opts.settledMetadata.artifacts)
            ...
            queueToolResultRecord?.(fc, {
                ...
                toolType,
                executionErrorType: executionStatus === 'error'
            ...
            const loopDetected = opts.recordInvalidToolParams === true &&
                ...
                recordDaemonInvalidToolParams(this.config, promptId, toolLoopState, toolName, error);
            return {
                parts: errorParts,
        ...
        const cancelBeforeExecutionIfAborted = (toolName = fc.name ?? 'unknown_tool') => activeToolAbortSignal.aborted
            ? earlyErrorResponse(new Error('Tool call was cancelled before execution.'), toolName, {
                status: 'cancelled',
                errorType: undefined,
                executionStatus: 'not_started',
            })
        ...
        if (!fc.name) {
            return earlyErrorResponse(new Error('Missing function name'), 'unknown_tool', {
                status: 'error',
        ...
        if (!tool) {
            return earlyErrorResponse(new Error(`Tool "${toolName}" not found in registry.`), toolName, {
                status: 'error',
        ...
        const originalPolicyRequestArgs = policyToolName === ToolNames.SHELL || policyToolName === ToolNames.MONITOR
        ...
        try {
            return await runInToolSpanContext(toolSpan, async () => {
                ...
                const isTrustedTodoWriteTool = isTodoWriteTool &&
                ...
                try {
                    ...
                    if (isAgentTool && taskEventEmitter) {
                        ...
                        const subSubAgentTracker = new SubAgentTracker(this, this.client, parentToolCallId, subagentType, () => {
                        ...
                        subAgentCleanupFunctions = subSubAgentTracker.setup(taskEventEmitter, activeToolAbortSignal);
                    }
                    ...
                    const isAskUserQuestionTool = policyToolName === ToolNames.ASK_USER_QUESTION;
                    // ---- L3→L4: Shared permission flow ----
                    let toolParams = invocation.params;
                    const flowResult = isTrustedLiveScreenContextTool || isTrustedLiveTaskTool
                        ? {
                            defaultPermission: 'allow',
                            finalPermission: 'allow',
                            pmForcedAsk: false,
                            pmCtx: buildPermissionCheckContext(policyToolName, toolParams, this.config.getTargetDir(), invocation.permissionAliases),
                            requiresUserInteraction: false,
                            denyMessage: undefined,
                        }
                        : await evaluatePermissionFlow(this.config, invocation, policyToolName, toolParams);
                    const permissionFlowCancellation = cancelBeforeExecutionIfAborted(toolName);
                    if (permissionFlowCancellation)
                        return permissionFlowCancellation;
                    const { finalPermission, pmForcedAsk, pmCtx, denyMessage, requiresUserInteraction, } = flowResult;
                    // ---- L5: ApprovalMode overrides ----
                    approvalMode = this.config.getApprovalMode();
                    const isPlanMode = approvalMode === ApprovalMode.PLAN;
                    const isPlanShellCall = isPlanMode &&
                        (policyToolName === ToolNames.SHELL ||
                            policyToolName === ToolNames.MONITOR);
                    if (finalPermission === 'deny') {
                        return earlyErrorResponse(new Error(denyMessage ?? `Tool "${toolName}" is denied.`), toolName, {
                            status: 'error',
                            errorType: ToolErrorType.EXECUTION_DENIED,
                            executionStatus: 'not_started',
                        });
                    }
                    let planShellAmbientWorkingDirectory;
                    if (isPlanShellCall) {
                    ...
                    const planPolicyCancellation = cancelBeforeExecutionIfAborted(toolName);
                    if (planPolicyCancellation)
                        return planPolicyCancellation;
                    if (planShellDecision.classification !== 'not-applicable') {
                        ...
                        const initialPlanValidationCancellation = cancelBeforeExecutionIfAborted(toolName);
                        if (initialPlanValidationCancellation) {
                            return initialPlanValidationCancellation;
                        }
                        if (initialPlanShellError) {
                            return earlyErrorResponse(new Error(initialPlanShellError), toolName, {
                                status: 'error',
                                errorType: ToolErrorType.EXECUTION_DENIED,
                                executionStatus: 'not_started',
                            });
                    ...
                    if (planShellDecision.classification === 'write') {
                        return earlyErrorResponse(new Error(planShellDecision.writeBlockMessage), toolName, {
                            status: 'error',
                            errorType: ToolErrorType.EXECUTION_DENIED,
                            executionStatus: 'not_started',
                        });
                    }
                    const planShellRequiresConfirmation = planShellDecision.classification === 'unknown';
                    ...
                    const forceAutoReviewForAllow = approvalMode === ApprovalMode.AUTO &&
                        (shouldForceAutoModeReviewForAllow(pmCtx, this.config.getCwd()) ||
                            shouldClassifyAllShellForAutoMode(policyToolName, this.config));
                    const confirmationPermission = getEffectivePermissionForConfirmation(finalPermission, forceAutoReviewForAllow);
                    if (finalPermission === 'allow' && forceAutoReviewForAllow) {
                        debugLogger.info(`Auto mode: L4 allow overridden by protected-write guard for ${policyToolName}`);
                    }
                    ...
                    if (autoModeAllowed && approvalMode === ApprovalMode.AUTO) {
                        this.config.setAutoModeDenialState(recordAllow(this.config.getAutoModeDenialState()));
                    }
                    let wasAutoModeManualFallback = false;
                    let autoModeFallbackMessage;
                    ...
                    if (!autoModeAllowed &&
                        ...
                        const autoModeCancellation = cancelBeforeExecutionIfAborted(toolName);
                        if (autoModeCancellation)
                            return autoModeCancellation;
                        ...
                        // guard.
                        const outcome = applyAutoModeDecision(decision, this.config, denialState);
                        await fireSessionPermissionDeniedForAutoMode(this.config, decision, outcome, policyToolName, toolParams, callId, abortSignal);
                        const permissionDeniedHookCancellation = cancelBeforeExecutionIfAborted(toolName);
                        ...
                        switch (outcome.kind) {
                            case 'approved':
                                ...
                                break;
                            case 'blocked':
                                debugLogger.warn(`Auto mode blocked (${outcome.reason}): tool=${policyToolName}, ` +
                                    formatDenialStateLog(denialState));
                                return earlyErrorResponse(new Error(outcome.errorMessage), toolName, {
                                    status: 'error',
                                    errorType: ToolErrorType.EXECUTION_DENIED,
                                    executionStatus: 'not_started',
                                });
                            case 'fallback':
                                ...
                                if (wasAutoModeManualFallback) {
                                    debugLogger.warn(`Auto mode fallback to manual approval (${outcome.reason}): ` +
                                        formatDenialStateLog(denialState));
                                }
                                break;
                            default: {
                    ...
                    const recordAutoModeFallbackResolution = (outcome) => {
                        ...
                        if (approvalMode === ApprovalMode.AUTO &&
                            ...
                            if (after === before) {
                                debugLogger.warn(`Auto mode denial counters already clear after fallback approval: ` +
                                    formatDenialStateLog(before));
                                return;
                            }
                            debugLogger.warn(`Auto mode denial counters reset after fallback approval: ` +
                                `${formatDenialStateLog(before)} -> ${formatDenialStateLog(after)}`);
                            this.config.setAutoModeDenialState(after);
                        }
                    };
                    if (!autoModeAllowed &&
                        needsConfirmation(planShellRequiresConfirmation ? 'ask' : confirmationPermission, approvalMode, policyToolName, requiresUserInteraction)) {
                        confirmationDetails = await invocation.getConfirmationDetails(activeToolAbortSignal);
                        const confirmationDetailsCancellation = cancelBeforeExecutionIfAborted(toolName);
                        if (confirmationDetailsCancellation) {
                            return confirmationDetailsCancellation;
                        }
                        if (autoModeFallbackMessage) {
                            confirmationDetails = decorateClassifierUnavailableConfirmation(confirmationDetails, autoModeFallbackMessage);
                        }
                        if (planShellDecision.classification !== 'not-applicable') {
                            const preDisplayPlanShellError = await validatePlanModeShellContext({
                                config: this.config,
                                decision: planShellDecision,
                                requestArgs: args,
                                invocationParams: invocation.params,
                            ...
                            const preDisplayValidationCancellation = cancelBeforeExecutionIfAborted(toolName);
                            if (preDisplayValidationCancellation) {
                                return preDisplayValidationCancellation;
                            }
                            if (preDisplayPlanShellError) {
                                return earlyErrorResponse(new Error(preDisplayPlanShellError), toolName, {
                                    status: 'error',
                                    errorType: ToolErrorType.EXECUTION_DENIED,
                                    executionStatus: 'not_started',
                                });
                        ...
                        try {
                            confirmationDetails = decoratePlanModeShellConfirmation(planShellDecision, confirmationDetails);
                        }
                        catch {
                            if (planShellDecision.classification === 'unknown') {
                                return earlyErrorResponse(new Error(planShellDecision.noApprovalMessage), toolName, {
                                    status: 'error',
                                    errorType: ToolErrorType.EXECUTION_DENIED,
                                    executionStatus: 'not_started',
                                });
                        ...
                        if (planShellDecision.classification === 'not-applicable' &&
                            isPlanModeBlocked(isPlanMode, isExitPlanModeTool, isAskUserQuestionTool, confirmationDetails, isEnterPlanModeTool)) {
                            return earlyErrorResponse(new Error(`Plan mode is active. The tool "${toolName}" cannot be executed because it modifies the system. ` +
                                'Please use the exit_plan_mode tool to present your plan and exit plan mode before making changes.'), toolName, {
                                status: 'error',
                                errorType: ToolErrorType.EXECUTION_DENIED,
                                executionStatus: 'not_started',
                            });
                        ...
                        if (hooksEnabled && messageBus) {
                            const hookResult = await firePermissionRequestHook(messageBus, policyToolName, args, String(approvalMode), undefined, activeToolAbortSignal);
                            const permissionHookCancellation = cancelBeforeExecutionIfAborted(toolName);
                            ...
                            if (hookResult.hasDecision &&
                                ...
                                if (hookResult.shouldAllow) {
                                    if (planShellDecision.classification !== 'not-applicable') {
                                        ...
                                        const hookPlanApprovalCancellation = cancelBeforeExecutionIfAborted(toolName);
                                        if (hookPlanApprovalCancellation) {
                                            return hookPlanApprovalCancellation;
                                        }
                                        await confirmationDetails.onConfirm(approval.outcome, approval.payload);
                                        const hookPlanConfirmationCancellation = cancelBeforeExecutionIfAborted(toolName);
                                        ...
                                        if (approval.outcome === ToolConfirmationOutcome.Cancel) {
                                            return earlyErrorResponse(new Error(approval.payload?.cancelMessage ??
                                                planShellDecision.noApprovalMessage), toolName, {
                                                status: 'error',
                                                errorType: ToolErrorType.EXECUTION_DENIED,
                                                executionStatus: 'not_started',
                                            });
                                        ...
                                        const hookConfirmationCancellation = cancelBeforeExecutionIfAborted(toolName);
                                        if (hookConfirmationCancellation) {
                                            return hookConfirmationCancellation;
                                        }
                                        recordAutoModeFallbackResolution(ToolConfirmationOutcome.ProceedOnce);
                                    }
                                }
                                else {
                                    return earlyErrorResponse(new Error(hookResult.denyMessage ||
                                        `Permission denied by hook for "${toolName}"`), toolName, {
                                        status: 'error',
                                        errorType: ToolErrorType.EXECUTION_DENIED,
                                        executionStatus: 'not_started',
                                    });
                        ...
                        if (!requiresUserInteraction &&
                            ...
                            if (planShellDecision.classification !== 'not-applicable') {
                                const finalPreDisplayPlanShellError = await validatePlanModeShellContext({
                                    config: this.config,
                                    decision: planShellDecision,
                                    requestArgs: args,
                                    invocationParams: invocation.params,
                                ...
                                const finalPlanValidationCancellation = cancelBeforeExecutionIfAborted(toolName);
                                if (finalPlanValidationCancellation) {
                                    return finalPlanValidationCancellation;
                                }
                                if (finalPreDisplayPlanShellError) {
                                    return earlyErrorResponse(new Error(finalPreDisplayPlanShellError), toolName, {
                                        status: 'error',
                                        errorType: ToolErrorType.EXECUTION_DENIED,
                                        executionStatus: 'not_started',
                                    });
                            ...
                            const mappedKind = this.toolCallEmitter.mapToolKind(tool.kind, policyToolName);
                            if (hooksEnabled && messageBus) {
                                this.fireNotificationHookWithTerminalSequence(messageBus, `LailatulCoder Ai needs your permission to use ${toolName}`, NotificationType.PermissionPrompt, 'Permission needed');
                            }
                            const permissionOptions = toPermissionOptions(confirmationDetails, pmForcedAsk);
                            const offeredPermissionOptions = permissionOptions.map((option) => ({ ...option }));
                            const params = {
                                sessionId: this.sessionId,
                                options: permissionOptions,
                                toolCall: {
                            ...
                            const stopAfterPermissionCancel = (message) => {
                                onStopAfterPermissionCancel?.();
                                return earlyErrorResponse(new Error(message ?? `Tool "${toolName}" was canceled by the user.`), toolName, {
                                    status: 'cancelled',
                                    errorType: undefined,
                                    executionStatus: 'not_started',
                                    stopAfterPermissionCancel: true,
                            ...
                            let outcome;
                            try {
                                output = (await this.#requestPermissionQueued(params, activeToolAbortSignal));
                                const permissionRequestCancellation = cancelBeforeExecutionIfAborted(toolName);
                                if (permissionRequestCancellation) {
                                    return permissionRequestCancellation;
                                }
                                outcome = resolvePermissionOutcome(output, offeredPermissionOptions);
                            }
                            catch (error) {
                                debugLogger.error(`Permission request failed for tool ${toolName}:`, error);
                                try {
                                    await confirmationDetails.onConfirm(ToolConfirmationOutcome.Cancel);
                                }
                                catch (confirmError) {
                                    debugLogger.error(`Failed to cancel tool ${toolName} after permission request failure:`, confirmError);
                                }
                                ...
                                const permissionFailureMessage = isExitPlanModeTool
                                    ...
                                    : planShellDecision.classification === 'unknown'
                                        ? `Plan mode could not complete approval for this shell command: ${this.#formatError(error)}. The command was not run; Plan mode remains active.`
                                        : `Permission request failed for "${toolName}": ${this.#formatError(error)}`;
                                return earlyErrorResponse(new Error(wasAborted
                                    ? 'Tool call was cancelled before execution.'
                                    : permissionFailureMessage), toolName, {
                            ...
                            if (planShellDecision.classification !== 'not-applicable') {
                                ...
                                const planApprovalCancellation = cancelBeforeExecutionIfAborted(toolName);
                            ...
                            const shouldSwitchToDefault = outcome ===
                            ...
                            try {
                                await confirmationDetails.onConfirm(outcome, confirmationPayload);
                                const confirmationCancellation = cancelBeforeExecutionIfAborted(toolName);
                                ...
                                debugLogger.error(`Failed to confirm cancellation for tool ${toolName}:`, error);
                            ...
                            if (shouldSwitchToDefault) {
                                ...
                                if (modeUpdateCancellation)
                                    return modeUpdateCancellation;
                            }
                            ...
                            if (outcome === ToolConfirmationOutcome.ProceedAlways ||
                                ...
                                await persistPermissionOutcome(outcome, confirmationDetails, this.config.getOnPersistPermissionRule?.(), this.config.getPermissionManager?.(), confirmationPayload);
                                const permissionPersistenceCancellation = cancelBeforeExecutionIfAborted(toolName);
                            ...
                            if (confirmationDetails.type === 'edit' &&
                                ...
                                await this.sendCurrentModeUpdateNotification();
                                const editModeUpdateCancellation = cancelBeforeExecutionIfAborted(toolName);
                            ...
                            switch (outcome) {
                                case ToolConfirmationOutcome.ProceedOnceAndSwitchToDefault:
                                    throw new Error('Switch-to-Default outcome must be normalized before execution.');
                                case ToolConfirmationOutcome.Cancel:
                                    ...
                                    // as an error.
                                    return stopAfterPermissionCancel(confirmationPayload?.cancelMessage);
                    ...
                    if (!didRequestPermission && !isTodoWriteTool) {
                        ...
                        try {
                            ...
                            debugLogger.debug('[Session.runTool] Failed to emit tool start update', emitError);
                        }
                        const startEmissionCancellation = cancelBeforeExecutionIfAborted(toolName);
                        if (startEmissionCancellation)
                            return startEmissionCancellation;
                    }
                    ...
                    if (hooksEnabledForTool && messageBusForTool) {
                        const preHookResult = await firePreToolUseHook(messageBusForTool, policyToolName, args, toolUseId, permissionMode, activeToolAbortSignal, callId);
                        const preHookCancellation = cancelBeforeExecutionIfAborted(toolName);
                        if (preHookCancellation)
                            return preHookCancellation;
                        if (!preHookResult.shouldProceed) {
                            ...
                            try {
                                await this.messageEmitter.emitAgentMessage(`✗ **PreToolUse blocked**: ${toolName} - ${blockReason}`);
                            }
                            catch (emitError) {
                                debugLogger.debug('[Session.runTool] Failed to emit PreToolUse block message', emitError);
                            }
                            const blockMessageCancellation = cancelBeforeExecutionIfAborted(toolName);
                            if (blockMessageCancellation)
                                return blockMessageCancellation;
                    ...
                    if (toolInvocationGuard) {
                        ...
                        const guardDecision = await evaluateToolInvocationGuard(toolInvocationGuard, {
                            callId,
                            toolName: policyToolName,
                        ...
                        if (activeToolAbortSignal.aborted) {
                            return earlyErrorResponse(new Error('Tool invocation was cancelled'), toolName, {
                                status: 'cancelled',
                                errorType: undefined,
                                executionStatus: 'not_started',
                            });
                        ...
                        if (!guardDecision.allowed) {
                            return earlyErrorResponse(new Error(guardDecision.reason), toolName, {
                                status: 'error',
                                errorType: ToolErrorType.EXECUTION_DENIED,
                                executionStatus: 'not_started',
                            });
                    ...
                    const executionBoundaryCancellation = cancelBeforeExecutionIfAborted(toolName);
                    if (executionBoundaryCancellation) {
                        return executionBoundaryCancellation;
                    }
                    const continuedAgentId = toolName === ToolNames.SEND_MESSAGE &&
                        typeof args['task_id'] === 'string' &&
                        args['task_id'].length > 0
                        ? args['task_id']
                        : undefined;
                    const provisionalRelatedAgent = continuedAgentId !== undefined &&
                        !this.relatedAgentIds.has(continuedAgentId);
                    if (provisionalRelatedAgent) {
                        this.provisionalRelatedAgentCounts.set(continuedAgentId, (this.provisionalRelatedAgentCounts.get(continuedAgentId) ?? 0) +
                            1);
                    }
                    let relatedAgentSettled = false;
                    const settleRelatedAgent = (succeeded) => {
                        if (relatedAgentSettled ||
                            !provisionalRelatedAgent ||
                            !continuedAgentId) {
                            return;
                        }
                        relatedAgentSettled = true;
                        const remaining = (this.provisionalRelatedAgentCounts.get(continuedAgentId) ?? 1) -
                            1;
                        if (remaining > 0) {
                            this.provisionalRelatedAgentCounts.set(continuedAgentId, remaining);
                        }
                        else {
                    ...
                    const onToolProgress = (chunk) => {
                        ...
                        void this.sendUpdate({
                            ...
                            debugLogger.debug(`[Session.runTool] heartbeat update failed for ${callId}: ${err}`);
                        });
                    };
                    const heartbeatSpanAttributes = () => heartbeatCount > 0
                        ? {
                            attributes: {
                        ...
                        : undefined;
                    let settledArtifacts;
                    let settledPersistedOutputFiles;
                    const sleepInhibitorHandle = acquireSleepInhibitor(this.config, `LailatulCoder Ai is executing tool ${toolName}`);
                    try {
                        try {
                            addToolArgumentsAttributes(this.config, toolSpan, invocation.params);
                        }
                        catch {
                            debugLogger.debug('[Session.runTool] Failed to record tool arguments telemetry');
                        }
                        ...
                        try {
                            toolResult = await invocation.execute(activeToolAbortSignal, onToolProgress);
                            ...
                            const explicitErrorType = execError?.errorType;
                            const executionTimedOut = explicitErrorType === ToolErrorType.EXECUTION_TIMEOUT;
                            executionStatus =
                            ...
                            endToolExecutionSpan(execSpan, {
                                success: false,
                                error: executionStatus === 'cancelled'
                                    ? 'tool_cancelled'
                                    : executionTimedOut
                                        ? 'tool_timeout'
                                        : 'tool_exception',
                    ...
                    try {
                        observeToolResultBoundary({
                            ...
                            toolName,
                            artifacts: [
                                toolResultBoundaryArtifact(settledPersistedOutputFiles, settledArtifacts),
                            ],
                            values: () => [
                    ...
                    if ((isEnterPlanModeTool || isExitPlanModeTool) &&
                        !toolResult.error &&
                    ...
                    let responseParts = aborted
                        ? convertToFunctionErrorResponse(toolName, callId, TOOL_EXECUTION_CANCELLED_MESSAGE, TOOL_EXECUTION_CANCELLED_MESSAGE)
                        : toolResult.error
                            ? convertToFunctionErrorResponse(toolName, callId, toolResult.llmContent, toolResult.error.message)
                            : convertToFunctionResponse(toolName, callId, toolResult.llmContent);
                    ...
                    if (isTrustedTodoWriteTool && !toolResult.error) {
                        this.todoStopGuard.observeTodoWrite(toolResult.returnDisplay, this.config.getApprovalMode() !== ApprovalMode.PLAN);
                        if (aborted)
                            this.todoStopGuard.suspend();
                    }
                    // Fire PostToolUse hook on successful execution (aligned with core path)
                    if (hooksEnabledForTool &&
                        messageBusForTool &&
                        !toolResult.error &&
                        !aborted &&
                        ...
                        const postHookResult = await firePostToolUseHook(messageBusForTool, policyToolName, args, toolResponse, toolUseId, permissionMode, activeToolAbortSignal, callId);
                        if (activeToolAbortSignal.aborted) {
                            return earlyErrorResponse(new Error(TOOL_POST_EXECUTION_CANCELLED_MESSAGE), toolName, {
                                status: 'cancelled',
                                errorType: undefined,
                                executionStatus,
                                settledMetadata: {
                        ...
                        if (postHookResult.shouldStop) {
                            ...
                            debugLogger.info(`PostToolUse hook requested stop for ${toolName}: ${stopMessage}`);
                            this.todoStopGuard.suspend();
                        ...
                        try {
                            const failureHookResult = await firePostToolUseFailureHook(messageBusForTool, toolUseId, policyToolName, args, toolResult.error?.message ?? TOOL_EXECUTION_CANCELLED_MESSAGE, isInterrupt, permissionMode, activeToolAbortSignal, callId);
                            if (failureHookResult.additionalContext) {
                                debugLogger.debug(`PostToolUseFailure hook additional context for ${toolName}: ${failureHookResult.additionalContext}`);
                            }
                            await this.emitHookArtifactsNotification({
                            ...
                            debugLogger.debug('[Session.runTool] PostToolUseFailure hook failed', hookError);
                        }
                    }
                    ...
                    const visionBridgeNotice = visionBridgeNotices.length > 0
                    ...
                    if (visionBridgeNotice) {
                        try {
                            ...
                            debugLogger.debug('[Session.runTool] Failed to emit vision bridge notice', emitError);
                        }
                    }
                    if (activeToolAbortSignal.aborted &&
                        !(isExecutionTimeout && parentAbortedAtExecutionSettle)) {
                        status = 'cancelled';
                        responseParts = convertToFunctionErrorResponse(toolName, callId, TOOL_POST_EXECUTION_CANCELLED_MESSAGE, TOOL_POST_EXECUTION_CANCELLED_MESSAGE);
                    }
                    terminalStatus = status;
                    const succeeded = status === 'success';
                    const responseError = status === 'error' && toolResult.error
                    ...
                    if (isTodoWriteTool) {
                        const plan = this.planEmitter.extractPlan(toolResult.returnDisplay, succeeded ? args : undefined);
                        ...
                        if (plan &&
                            ...
                            try {
                                ...
                                debugLogger.debug('[Session.runTool] Failed to emit plan update', emitError);
                            }
                        }
                    ...
                    queueToolResultRecord?.(fc, {
                        ...
                        policyToolName,
                        toolType,
                        executionErrorType: executionStatus === 'error' ? executionErrorType : undefined,
                        metadata: {
                            callId,
                            status,
                            executionStatus,
                            ...
                            ...(visionBridgeNotice !== undefined
                                ...
                                : {}),
                            artifacts: settledArtifacts,
                            error: status === 'error' && toolResult.error
                                ? new Error(toolResult.error.message)
                                : undefined,
                    ...
                    if (succeeded && !nestedPermissionCancelled) {
                        const result = responseParts.find((part) => part.functionResponse !== undefined)?.functionResponse?.response;
                        if (result !== undefined) {
                            try {
                                addToolCallResultAttributes(this.config, toolSpan, result);
                            }
                            catch {
                                debugLogger.debug('[Session.runTool] Failed to record tool result telemetry');
                            }
                        }
                    }
                    ...
                    const executionTimeoutException = !executeReturned &&
                    ...
                    if (hooksEnabledForError && messageBusForError) {
                        try {
                            const failureHookResult = await firePostToolUseFailureHook(messageBusForError, toolUseId, policyToolName, args, error.message, isInterrupt, String(approvalMode), activeToolAbortSignal, callId);
                            if (failureHookResult.additionalContext) {
                                debugLogger.debug(`PostToolUseFailure hook additional context for ${toolName}: ${failureHookResult.additionalContext}`);
                            }
                            await this.emitHookArtifactsNotification({
                    ...
                    const explicitErrorType = e?.errorType;
                    const errorType = status === 'cancelled'
                        ? undefined
                        : (explicitErrorType ??
                            (executeReturned
                                ? ToolErrorType.UNHANDLED_EXCEPTION
            ...
            return await earlyErrorResponse(error, toolName, {
                status,
                errorType: status === 'error' ? ToolErrorType.UNHANDLED_EXCEPTION : undefined,
                executionStatus,
            });
        }
        finally {
     ...
     */
    async #processSlashCommandResult(result, originalPrompt, abortSignal, onFullTurnModel, shouldRecordResult) {
        ...
        switch (result.type) {
            case 'submit_prompt':
                ...
                return this.#applyBridgeConversionsIfNeeded(normalizePartList(result.content), abortSignal, onFullTurnModel);
            case 'message': {
            ...
            case 'stream_messages': {
                ...
                for await (const msg of result.messages) {
                    ...
                    await this.messageEmitter.emitSlashCommandOutput((msg.content || '').replace(/\n/g, '  \n'));
            ...
            case 'goal_control':
                if (!result.cause) {
                    await this.#queueGoalState(result.response.snapshot, undefined, this.lastGoalSnapshot?.goal ?? null);
    ...
    options = {}) {
        ...
        const extensionMentions = new Map();
        const mcpServerMentions = new Map();
        const textPathSpecsToRead = new Map();
        const preserveUnsupportedImageForBridge = shouldRunVisionBridge(this.config);
                ...
                case 'text':
                    ...
                    for (const pathSpec of extractAtPathCommands(part.text)) {
                        if (!path.isAbsolute(pathSpec))
                            continue;
                        const resolved = path.resolve(this.config.getProjectRoot(), pathSpec);
                        const canonicalPath = resolveExistingFile(resolved);
                        if (!canonicalPath)
                            continue;
                        const filteringOptions = this.config.getFileFilteringOptions();
                        if (getSpecificMimeType(canonicalPath)?.startsWith('image/') &&
                            this.config
                                .getWorkspaceContext()
        ...
        const extensionParts = await this.#resolveExtensionMentionParts(extensionMentions, abortSignal);
        const mcpServerParts = this.#resolveMcpServerMentionParts(mcpServerMentions);
        const revalidatedTextPaths = [];
        ...
        const candidatePathsToRead = [
            ...
            ...atPathCommandParts.flatMap((part) => {
                ...
                const canonicalPath = resolveExistingFile(resolved);
                return canonicalPath ? [[canonicalPath, fileUri]] : [];
            }),
        ];
        const filteringOptions = candidatePathsToRead.length > 0
            ? this.config.getFileFilteringOptions()
            : undefined;
        const displayPaths = new Map();
        const acceptedFileUris = new Set();
        for (const [textPath, displayPath] of candidatePathsToRead) {
            try {
                if (resolveExistingFile(textPath) !== textPath ||
        ...
        const partsToSend = parts.filter((part) => !('fileData' in part) || acceptedFileUris.has(part.fileData.fileUri));
        if (pathSpecsToRead.length === 0 &&
            embeddedContext.length === 0 &&
            extensionParts.length === 0 &&
            mcpServerParts.length === 0) {
            return this.#applyBridgeConversionsIfNeeded(partsToSend, abortSignal, options.onFullTurnModel);
        }
        if (pathSpecsToRead.length === 0 && embeddedContext.length === 0) {
            return this.#applyBridgeConversionsIfNeeded([...partsToSend, ...extensionParts, ...mcpServerParts], abortSignal, options.onFullTurnModel);
        }
        ...
        // last. Recency-biased providers (e.g. local Ollama LailatulCoder models) otherwise
        ...
        for (const contextPart of embeddedContext) {
            ...
            if ('blob' in contextPart && contextPart.blob) {
                ...
                referenceParts.push(preserveUnsupportedImageForBridge && hasImageParts([inlinePart])
                    ? inlinePart
        ...
        // trims as before.
        const promptText = pathSpecsToRead.length > 0 ? initialQueryText : initialQueryText.trim();
        ...
        const processedQueryParts = options.promptLast
            ? [...referenceParts, promptPart]
            : [promptPart, ...referenceParts];
        return this.#applyBridgeConversionsIfNeeded(processedQueryParts, abortSignal, options.onFullTurnModel);
    }
    async #applyBridgeConversionsIfNeeded(originalParts, abortSignal, onFullTurnModel) {
        const parts = await this.#applyVoiceBridgeIfNeeded(originalParts, abortSignal);
        ...
        if (onFullTurnModel && fullTurnModel?.agentCapable) {
            ...
            const selected = onFullTurnModel(getFullTurnVisionModelSelector(fullTurnModel));
            if (selected) {
                try {
                    await this.messageEmitter.emitAgentMessage(formatFullTurnVisionNotice(fullTurnModel));
                }
                catch (error) {
                    debugLogger.debug(`full-turn vision: failed to emit notice; continuing error=${error instanceof Error ? error.message : String(error)}`);
                }
            }
            return fullTurnParts;
        }
        ...
        try {
            ...
            debugLogger.debug(`vision bridge: failed before replacement; falling back to text-only parts error=${String(error instanceof Error ? error.message : error)}`);
            return splitImageParts(parts).nonImageParts;
        }
        debugLogger.debug(`vision bridge: status=${bridgeResult.status} applied=${bridgeResult.applied} model=${bridgeResult.modelId ?? '(none)'}${bridgeResult.error ? ` error=${bridgeResult.error}` : ''}`);
        if (bridgeResult.status !== 'skipped' || bridgeResult.egressOccurred) {
            try {
                await this.messageEmitter.emitAgentMessage(formatVisionBridgeNotice(bridgeResult));
            }
            catch (error) {
                debugLogger.debug(`vision bridge: failed to emit notice; continuing with bridge result error=${String(error instanceof Error ? error.message : error)}`);
            }
        }
        if (abortSignal.aborted) {
    ...
    async #applyVoiceBridgeIfNeeded(parts, abortSignal) {
        if (!hasAudioParts(parts) ||
            this.config.getEffectiveInputModalities?.().audio === true) {
            return parts;
        }
        const voiceModel = readVoiceModel(this.settings);
        if (!voiceModel) {
            debugLogger.debug('voice bridge: no voice model configured; replacing audio with note');
            return parts.map((part) => isAudioPart(part)
                ? {
        ...
        for (const part of parts) {
            ...
            if (approxBase64Bytes(inlineData.data) > MAX_AUDIO_BYTES) {
                debugLogger.debug('voice bridge: audio too large; replacing audio with note');
            ...
            try {
                ...
                converted.push({
                    text: transcript.length > 0
                        ...
                        : buildVoiceUnavailableBlock('the voice model returned no transcript'),
                });
            }
            catch (error) {
                if (abortSignal.aborted) {
                ...
                debugLogger.debug(`voice bridge: transcription failed; replacing audio with note error=${sanitizeVoiceErrorMessage(String(error instanceof Error ? error.message : error))}`);
                converted.push({
        ...
        if (transcribedCount > 0 || egressCount > 0) {
            try {
                await this.messageEmitter.emitAgentMessage(transcribedCount > 0
                    ? this.#formatVoiceBridgeNotice(voiceModel, transcribedCount)
                    : this.#formatVoiceBridgeEgressNotice(voiceModel, egressCount));
            }
            catch (error) {
                debugLogger.debug(`voice bridge: failed to emit notice; continuing with bridge result error=${String(error instanceof Error ? error.message : error)}`);
            }
        }
        return converted;
    }
    #formatVoiceBridgeNotice(modelId, convertedCount) {
        return `Converted ${convertedCount} audio file(s) to text via ${modelId}. Your audio was sent to that model.`;
    }
    #formatVoiceBridgeEgressNotice(modelId, audioCount) {
    ...
    async #resolveExtensionMentionParts(extensionMentions, abortSignal) {
        ...
        for (const name of extensionMentions.values()) {
            ...
            if (!extension) {
                this.debug(`Extension "${name}" not found among active extensions. ` +
                    `Available: ${activeExtensions.map((e) => e.name).join(', ') || '(none)'}`);
                continue;
            }
            if (resolvedExtensionNames.has(extension.name))
                continue;
    ...
    #resolveMcpServerMentionParts(mcpServerMentions) {
        if (mcpServerMentions.size === 0)
            return [];
        ...
        for (const name of mcpServerMentions.values()) {
            ...
            if (!matched) {
                this.debug(`MCP server "${name}" not found among configured MCP servers. ` +
    ...
    async emitHookArtifactsNotification(args) {
        if (!args.artifacts || args.artifacts.length === 0) {
            return;
        }
        try {
            await this.client.extNotification('LailatulCoder/notify/session/artifact-event', {
                v: 1,
                sessionId: this.sessionId,
                source: 'hook',
            ...
            writeStderrLine(`Hook artifact notification dropped for ${args.toolName ?? args.hookEventName}: ${error instanceof Error ? error.message : String(error)}`);
        }
    }
    /**
     ...
     * debug level.
     */
    fireNotificationHookWithTerminalSequence(messageBus, message, notificationType, title) {
        void fireNotificationHook(messageBus, message, notificationType, title)
            .then((hookResult) => {
            if (!hookResult.terminalSequence)
                return;
            return this.client.extNotification('LailatulCoder/notify/session/terminal-sequence', {
                v: 1,
                sessionId: this.sessionId,
            ...
            .catch((err) => {
            debugLogger.debug(`ACP terminalSequence notification dropped ` +
