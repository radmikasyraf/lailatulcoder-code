...
import { createDebugLogger, escapeXml, SessionService, stripTerminalControlSequences, } from '@lailatul-coder/lailatul-coder-core';
...
const BIDI_CONTROL_MARKS = new RegExp('[\\u061C\\u200E\\u200F\\u202A-\\u202E\\u2066-\\u2069]', 'g');
...
function escapeXmlWithinBudget(value, budget) {
    ...
    while (escapedCodePoints.length > 0 &&
        length + SENT_COMPLETION_RESULT_TRUNCATION_MARKER.length > budget) {
        length -= escapedCodePoints.pop().length;
    }
    return `${escapedCodePoints.join('')}${SENT_COMPLETION_RESULT_TRUNCATION_MARKER.length <= budget - length
...
function buildSentCompletionNotification(sessionId, label, result, stopReason) {
    ...
    const sessionLink = `[🧵 ${sessionId.slice(0, 8)}](LailatulCoder-session://${sessionId})`;
    ...
    const modelTextPrefix = [
        ...
        `<session-link>LailatulCoder-session://${escapeXml(modelSessionId)}</session-link>`,
        '<result>',
    ].join('');
    const modelTextSuffix = '</result></task-notification>';
    const resultBudget = Math.max(0, MAX_SENT_COMPLETION_MODEL_TEXT_CHARS -
        modelTextPrefix.length -
        modelTextSuffix.length);
    const modelText = `${modelTextPrefix}${escapeXmlWithinBudget(safeResult, resultBudget)}${modelTextSuffix}`;
    return {
...
function isBackgroundNotificationForTask(event, taskId) {
    ...
    return (update?._meta?.source === 'background_notification' &&
...
async function awaitRecoveredParentNotification(bridge, sessionId, notification, lastEventId, eventEpoch, stopSignal) {
    ...
    const timer = setTimeout(() => timeoutAc.abort(), RECOVERED_PARENT_NOTIFICATION_TIMEOUT_MS);
    ...
    try {
        for await (const event of bridge.subscribeEvents(sessionId, {
            ...
            else if (observedOwnNotification &&
...
async function waitForSentCompletionRetry(signal, attempt) {
    if (signal.aborted)
        throw signal.reason;
    const delay = Math.min(SENT_COMPLETION_DELIVERY_RETRY_MS * 2 ** attempt, SENT_COMPLETION_DELIVERY_MAX_RETRY_MS);
...
async function awaitSentCompletionAcceptance(bridge, parentSessionId, notification, stopSignal, deadline) {
    ...
    while (!stopSignal.aborted) {
        try {
            const acknowledgement = await bridge.enqueueBackgroundNotification(parentSessionId, notification);
        ...
        if (Date.now() >= deadline) {
            throw new Error(`Parent ${parentSessionId} did not durably accept completion for sub-session ${notification.taskId}.`, lastError === undefined ? undefined : { cause: lastError });
        }
...
async function deliverSentCompletion(bridge, boundWorkspace, parentSessionId, notification, stopSignal, isolatedWorkspace) {
    const deadline = Date.now() + RECOVERED_PARENT_NOTIFICATION_TIMEOUT_MS;
    const initialDelivery = await awaitSentCompletionAcceptance(bridge, parentSessionId, notification, stopSignal, deadline);
    ...
    try {
        ...
        if (isolatedCwd !== undefined) {
            if (restoredParent.hasActivePrompt === true &&
                ...
                throw new Error('Active restored parent is outside its isolated conversation directory.');
            }
            if (restoredParent.hasActivePrompt === true) {
            ...
            if (restoredParent.hasActivePrompt !== true) {
                ...
                if (changed.newCwd !== isolatedCwd) {
                    ...
                    throw new Error('Restored parent workspace directory relocation was rejected.');
                }
        ...
        const recoveredDelivery = await awaitSentCompletionAcceptance(bridge, parentSessionId, notification, stopSignal, deadline);
        ...
        void awaitRecoveredParentNotification(bridge, parentSessionId, notification, lastEventId, eventEpoch, stopSignal).then((continuationCompleted) => {
            if (!continuationCompleted && !stopSignal.aborted) {
                writeStderrLine(`LailatulCoder serve: restored parent ${parentSessionId} accepted completion ` +
            ...
            try {
                if (restoredParent.hasActivePrompt === true) {
                    if (restoredParent.clientId) {
                        await bridge.detachClient(restoredParent.sessionId, restoredParent.clientId);
                    }
                }
                    ...
                    recoveredParentClosed = await bridge.killSession(restoredParent.sessionId, { requireZeroAttaches: true });
                }
            }
            catch {
    ...
    try {
        for await (const e of bridge.subscribeEvents(sessionId, {
            ...
            if (e.type === 'session_update') {
                const d = e.data;
                if (d?.update?.sessionUpdate === 'agent_message_chunk' &&
...
export function createSubSessionLauncher(opts) {
    const { getBridge, boundWorkspace, notifySentCompletion = false, isolatedWorkspace, } = opts;
    ...
    const maxConcurrentTotal = Math.min(opts.maxConcurrentTotal ?? MAX_CONCURRENT_SUB_SESSIONS_TOTAL, MAX_TRACKED_SPAWNED_SESSIONS);
    ...
    const rememberSpawned = (sessionId) => {
    ...
    const launch = async (info) => {
        if (stopAc.signal.aborted) {
            throw new Error('The daemon is shutting down; cannot create a sub-session.');
        }
        ...
        if (spawnedSessionIds.has(info.callerSessionId) ||
            ...
            throw new Error('A sub-session cannot create further sub-sessions (nesting is capped ' +
                'at one level).');
        }
        ...
        if (current >= maxConcurrentPerCaller) {
            throw new Error(`Too many concurrent sub-sessions for this session ` +
        ...
        if (inflightTotal >= maxConcurrentTotal) {
            throw new Error(`Too many concurrent sub-sessions in this workspace ` +
        ...
        try {
            ...
            if (isolatedWorkspace) {
                ...
                if (changed.newCwd !== isolatedCwd) {
                    throw new Error('Sub-session workspace directory relocation was rejected.');
                }
            }
            ...
            const turn = bridge.sendPrompt(sessionId, {
                sessionId,
            ...
            if (info.completion === 'sent') {
                ...
                void (async () => {
                    try {
                        let notification;
                        try {
                            const turnError = turn.then(() => new Promise(() => { }), (err) => Promise.reject(new Error(`sub-session dispatch failed: ${err instanceof Error ? err.message : String(err)}`)));
                            const completion = await Promise.race([
                                awaitFirstTurn(bridge, sessionId, promptId, lastEventId, sentModeDrainTimeoutMs, drainSignal),
                                turnError,
                            ]);
                            if (stopAc.signal.aborted)
                                return;
                            if (completion.stopReason === 'timeout') {
                                writeStderrLine(`LailatulCoder serve: sub-session ${sessionId} drain timed out after ` +
                            ...
                            if (notifySentCompletion) {
                                notification = buildSentCompletionNotification(sessionId, subSessionName(info.name ?? info.prompt), completion.result, completion.stopReason);
                            }
                        }
                        catch (err) {
                            if (stopAc.signal.aborted)
                            ...
                            notification = buildSentCompletionNotification(sessionId, subSessionName(info.name ?? info.prompt), message, 'error');
                        }
                        if (!notification)
                            return;
                        try {
                            await deliverSentCompletion(bridge, boundWorkspace, info.callerSessionId, notification, stopAc.signal, isolatedWorkspace);
                        }
                        catch (notificationError) {
                            if (!stopAc.signal.aborted) {
                                writeStderrLine(`LailatulCoder serve: sub-session ${sessionId} completion could not be returned to parent ${info.callerSessionId}: ${notificationError instanceof Error ? notificationError.message : String(notificationError)}`);
                            }
                        }
                    }
                    finally {
            ...
            try {
                const turnError = turn.then(() => new Promise(() => { }), // never resolves on success
                (err) => Promise.reject(new Error(`sub-session dispatch failed: ${err instanceof Error ? err.message : String(err)}`)));
                const firstTurn = awaitFirstTurn(bridge, sessionId, promptId, lastEventId, firstTurnTimeoutMs, stopAc.signal);
            ...
            if (spawnedSession !== undefined && isolatedWorkspace) {
                ...
                try {
                    if (spawnedSession.attached) {
                        if (spawnedSession.clientId) {
                            await bridge.detachClient(spawnedSession.sessionId, spawnedSession.clientId);
                        }
                    }
                    else {
                    ...
                    log.debug('sub-session: isolated session rollback failed', spawnedSession.sessionId, cleanupError);
                }
                if (sessionClosed) {
                    if (!promptDispatched) {
                        try {
                            const transcriptRemoved = await new SessionService(boundWorkspace).removeSession(spawnedSession.sessionId);
                            if (transcriptRemoved)
                                bridge.markSessionCatalogChanged();
                        }
                        catch (cleanupError) {
                            log.debug('sub-session: isolated transcript cleanup failed', spawnedSession.sessionId, cleanupError);
                        }
                    }
                    try {
                        await isolatedWorkspace.discardEmptyDirectory(spawnedSession.sessionId);
                    }
                    catch (cleanupError) {
                        log.debug('sub-session: isolated workspace cleanup failed', spawnedSession.sessionId, cleanupError);
                    }
                }
            }
                ...
                try {
                    ...
                    log.debug('sub-session: closeSession threw', spawnedSession.sessionId, closeErr);
                }
            }
            writeStderrLine(`LailatulCoder serve: create_sub_session failed: ${err instanceof Error ? err.message : String(err)}`);
...
//# sourceMappingURL=create-sub-session.js.map