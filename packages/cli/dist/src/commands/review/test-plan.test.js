...
describe('extractTestPlanSection', () => {
    ...
    it('accepts the bold form and the Chinese headings', () => {
        expect(extractTestPlanSection('**Test Plan**\n\nran it\n\n**Risk**\n\nlow')
            ?.content).toBe('ran it');
        expect(extractTestPlanSection('## 测试计划\n\n跑了 `npm test`')?.content).toBe('跑了 `npm test`');
    });
    ...
    it('finds a heading whose name is PREFIXED, not anchored', () => {
        ...
        expect(extractTestPlanSection('## Reviewer Test Plan\n\nran it')?.content).toBe('ran it');
        expect(extractTestPlanSection('## Reviewer 测试计划\n\n跑了它')?.content).toBe('跑了它');
        expect(extractTestPlanSection('## Manual QA / Testing\n\nran it')?.content).toBe('ran it');
    });
...
describe('extractClaims', () => {
    it('picks commands and paths out of code spans', () => {
        const claims = extractClaims('Ran `npm run build` and `npm test --workspace=packages/cli`.\nAdded `packages/cli/src/a.test.ts`.');
    ...
    it('reads fenced blocks, stripping prompts and trailing comments', () => {
        const claims = extractClaims('```bash\n$ npm run lint   # should be clean\n```');
    ...
    it('emits ONE count claim for one statement, not one per overlapping pattern', () => {
        const claims = extractClaims('All 471 tests passed.').filter((c) => c.kind === 'count');
    ...
    it('does not extract a Test Files file-count line as a test-count claim', () => {
        ...
        const claims = extractClaims('Test Files  3 passed (3)\n     Tests  157 passed (157)').filter((c) => c.kind === 'count');
        expect(claims).toHaveLength(1);
    ...
    it('does not extract the MIXED file-count shape either', () => {
        ...
        const claims = extractClaims('Test Files  1 failed | 44 passed (45)\n     Tests  2 failed | 1323 passed (1325)').filter((c) => c.kind === 'count');
    ...
    it('does not extract jest Test Suites counts as test counts', () => {
        ...
        const claims = extractClaims('Test Suites: 1 failed, 44 passed, 45 total\nTests:       2 failed, 1323 passed, 1325 total').filter((c) => c.kind === 'count');
        expect(claims.map((c) => c.text)).toEqual(['1323 passed']);
    });
    ...
    it('reads a count stated in the future tense', () => {
        ...
        const claims = extractClaims('expect all four files and 471 tests to pass').filter((c) => c.kind === 'count');
    ...
    it('resolves a repro command path against its leading `cd`', () => {
        ...
        const claims = extractClaims('`cd packages/core && npx vitest run src/telemetry/loggers.test.ts`');
    ...
    it('still extracts a normal cd base as a path claim', () => {
        const claims = extractClaims('`cd packages/core && npx vitest run src/a.test.ts`');
        expect(claims).toContainEqual({ kind: 'path', text: 'packages/core' });
    });
    it('extracts no path from a `cd` shape it cannot resolve', () => {
        // Rather than guess a base and file a wrong note.
        expect(extractClaims('`for d in a b; do cd $d && npx vitest run src/x.test.ts; done`')).toEqual([]);
    });
    it('does not read a Test FILES summary as a test-count claim', () => {
        expect(extractClaims('Test Files  45 passed (45)').filter((c) => c.kind === 'count')).toEqual([]);
        expect(extractClaims('Tests  100 passed').filter((c) => c.kind === 'count')).toHaveLength(1);
    });
    it('does not read prose inside a quoted argument as a path', () => {
        ...
        const claims = extractClaims("`npx vitest run src/a.test.ts -t 'covers write/edit tools'`");
        expect(claims).toContainEqual({ kind: 'path', text: 'src/a.test.ts' });
        expect(claims
            .filter((c) => c.kind === 'path')
            .some((c) => c.text.includes('write/edit'))).toBe(false);
    });
    it('bails on path-rebasing flags like --root, as it does on cd', () => {
        ...
        const claims = extractClaims('`npx vitest run --root ./integration-tests sdk-typescript/perm.test.ts`');
        expect(claims
            .filter((c) => c.kind === 'path')
            .some((c) => c.text.includes('sdk-typescript'))).toBe(false);
    });
    it('bails on the inline --root=./dir form too, not only --root ./dir', () => {
        ...
        const claims = extractClaims('`npx vitest run --root=./integration-tests src/b.test.ts`');
        expect(claims
            .filter((c) => c.kind === 'path')
            .some((c) => c.text.includes('src/b.test.ts'))).toBe(false);
    });
    it('still reads a positional path after an inline --flag=value', () => {
        ...
        const claims = extractClaims('`npx vitest run --reporter=verbose src/a.test.ts`');
        expect(claims).toContainEqual({ kind: 'path', text: 'src/a.test.ts' });
    });
    it('does not read a bare parenthesised number as a count', () => {
        expect(extractClaims('Follows up on (#8176).').filter((c) => c.kind === 'count')).toEqual([]);
    });
    ...
    it('does not extract a gitignored build-output path as a claim', () => {
        ...
        const claims = extractClaims('Run `node packages/cli/dist/index.js --yolo`');
        expect(claims
            .filter((c) => c.kind === 'path')
            .some((c) => c.text.includes('dist/'))).toBe(false);
    });
});
describe('observedTestCounts', () => {
    ...
    it('reads the vitest summary', () => {
        expect(observedTestCounts(report(['\n Tests  472 passed (472)\n']))).toEqual([472]);
    });
    it('reads the jest summary', () => {
        expect(observedTestCounts(report(['Tests:       12 passed, 12 total']))).toEqual([12]);
    });
    ...
    it('reads a summary that also reports failures', () => {
        expect(observedTestCounts(report(['Tests  1 failed | 40 passed (41)']))).toEqual([40]);
    });
    it('reads a three-segment summary (failed | skipped | passed)', () => {
        expect(observedTestCounts(report(['Tests  2 failed | 3 skipped | 40 passed (45)']))).toEqual([40]);
    });
...
describe('npmScriptOf', () => {
    ...
    it('bails on a CHAINED cd instead of joining against the first hop', () => {
        ...
        expect(extractClaims('`cd a && cd packages/b && npx vitest run src/x.test.ts`')).toEqual([]);
    });
...
describe('runTestPlan', () => {
    ...
    const run = (body, files = [], buildTest) => {
        let btPath;
        if (buildTest) {
    ...
    const verdictOf = (claims, text) => claims.find((c) => c.text === text)?.verdict;
    beforeEach(() => {
    ...
    it('distinguishes a failed body fetch from an absent Test Plan', () => {
        const r = runTestPlan({ plan: plan([]), pr: '1', repo: 'o/r', worktree: dir }, () => {
    ...
    describe('path claims', () => {
        it('reproduces a path the diff changes', () => {
            ...
            expect(verdictOf(r.claims, 'packages/cli/src/a.test.ts')).toBe('reproduces');
        });
        it('reproduces a path that exists but the diff does not touch', () => {
            ...
            expect(verdictOf(r.claims, 'packages/core/src/old.test.ts')).toBe('reproduces');
        });
        it('contradicts a path that is in neither the diff nor the tree', () => {
            const r = run('## Test Plan\n\nAdded `packages/cli/src/ghost.test.ts`');
            const claim = r.claims.find((c) => c.text === 'packages/cli/src/ghost.test.ts');
        ...
        it('ignores a line suffix when resolving a path', () => {
            ...
            expect(verdictOf(r.claims, 'packages/cli/src/a.ts:42')).toBe('reproduces');
        });
        ...
        it('does not claim the VALUE of a flag inside a repro command', () => {
            ...
            expect(r.claims.map((c) => c.text)).not.toContain('infra/docker-compose.yml');
        });
        ...
        it('a TRACKED file that exists says it is state at the reviewed commit', () => {
            ...
            expect(claim?.verdict).toBe('reproduces');
            expect(claim?.note).toBe('exists at the reviewed commit (the diff does not change it)');
        });
    ...
    describe('command claims', () => {
        ...
        it('finds a script defined by a workspace package, not just the root', () => {
            ...
            writeFileSync(join(dir, 'packages/cli/package.json'), JSON.stringify({ name: '@x/cli', scripts: { 'test:e2e': 'vitest' } }));
        ...
        it('reproduces a script defined only by a nameless-but-parseable member', () => {
            ...
            writeFileSync(join(dir, 'packages/nameless/package.json'), JSON.stringify({ scripts: { 'test:ghost': 'vitest' } }));
            const r = run('## Test Plan\n\nRan `npm run test:ghost`');
            const claim = r.claims.find((c) => c.text === 'npm run test:ghost');
            expect(claim?.verdict).toBe('reproduces');
        });
        ...
        it('rules unchecked when the workspace globs use a shape the walker does not model', () => {
            ...
            mkdirSync(join(dir, 'packages/cli'), { recursive: true });
            writeFileSync(join(dir, 'packages/cli/package.json'), JSON.stringify({ name: '@x/cli', scripts: { 'test:unit': 'vitest' } }));
        ...
        it('finds a root-only script when the root defines no build/test', () => {
            ...
            mkdirSync(join(dir, 'packages/cli'), { recursive: true });
            writeFileSync(join(dir, 'packages/cli/package.json'), JSON.stringify({ name: '@x/cli', scripts: { build: 'tsc' } }));
            const r = run('## Test Plan\n\nRan `npm run lint`');
...
describe('the CLI option contract', () => {
    ...
    beforeEach(() => {
        ...
        writeFileSync(join(dir, 'diff.txt'), 'diff');
        writeFileSync(join(dir, 'plan.json'), JSON.stringify({ files: [], diffPathAbsolute: join(dir, 'diff.txt') }));
        writeFileSync(join(dir, 'bt.json'), JSON.stringify({
            build: [],
            test: [
                {
                    command: 'npm test',
    ...
    it('parses --build-test into the field runTestPlan actually reads', () => {
        const parsed = testPlanCommand.builder(yargs([])).parseSync([
            '--plan',
            join(dir, 'plan.json'),
            '--pr',
            '8176',
        ...
        const report = runTestPlan(parsed, () => '## Test Plan\n\n471 tests passed');
...
//# sourceMappingURL=test-plan.test.js.map