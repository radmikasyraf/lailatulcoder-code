...
describe('parseOptionalWorkspaceCwd inside a POSIX container sandbox (#7139)', () => {
    ...
    it.skipIf(process.platform === 'win32')('accepts a Windows-shaped cwd and returns its bind-mount location', () => {
        vi.stubEnv('SANDBOX', 'lailatul-coder-sandbox-0');
        _setSandboxMountExistsForTest((p) => p === '/c/qwen-repro');
        const { res, status } = mockRes();
        const cwd = parseOptionalWorkspaceCwd({ cwd: 'C:\\qwen-repro' }, '/c/qwen-repro', res);
        expect(cwd).toBe('/c/qwen-repro');
        expect(status).not.toHaveBeenCalled();
    });
    it.skipIf(process.platform === 'win32')('still rejects a Windows-shaped cwd outside a sandbox', () => {
        vi.stubEnv('SANDBOX', '');
        const { res, status } = mockRes();
        const cwd = parseOptionalWorkspaceCwd({ cwd: 'C:\\qwen-repro' }, '/tmp', res);
        expect(cwd).toBeUndefined();
        expect(status).toHaveBeenCalledWith(400);
    });
    it.skipIf(process.platform === 'win32')('rejects when the translated mount does not exist (no invented paths)', () => {
        vi.stubEnv('SANDBOX', 'lailatul-coder-sandbox-0');
        const { res, status } = mockRes();
        const cwd = parseOptionalWorkspaceCwd({ cwd: 'D:\\never-mounted' }, '/tmp', res);
        expect(cwd).toBeUndefined();
        expect(status).toHaveBeenCalledWith(400);
    });
});
//# sourceMappingURL=request-helpers.sandbox.test.js.map