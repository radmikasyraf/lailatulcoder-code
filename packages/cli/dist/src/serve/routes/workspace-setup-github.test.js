...
import { resetHomeEnvBootstrapForTesting, SETTINGS_DIRECTORY_NAME, } from '../../config/settings.js';
...
async function makeHarness(opts = {}) {
    const scratch = await fsp.mkdtemp(path.join(os.tmpdir(), `qwen-setup-github-route-${randomBytes(4).toString('hex')}-`));
    ...
    const app = createServeApp({ ...baseOpts, workspace, token: opts.token }, undefined, {
        bridge,
        fsFactory,
        ...(opts.hotReload
            ? {
...
function setupResult() {
    return {
        ...
        gitRepoRoot: '/work',
        releaseTag: 'v1.2.3',
        readmeUrl: 'https://github.com/LailatulCoder/lailatul-coder-action/blob/v1.2.3/README.md#quick-start',
...
describe('POST /workspace/setup-github', () => {
    ...
    it('does not publish completion after the runtime generation closes', async () => {
        ...
        setupGithubMocks.setupGithub.mockImplementationOnce(async () => {
            const registry = h.app.locals['workspaceRegistry'];
    ...
    it('returns retryable unavailable when generation closes before directory setup', async () => {
        await teardown(h);
        h = await makeHarness({ token: 'secret', hotReload: true });
        setupGithubMocks.setupGithub.mockImplementationOnce(async (opts) => {
            const registry = h.app.locals['workspaceRegistry'];
            registry.primaryEntry.current?.guard.close();
    ...
    it('rejects untrusted workspace before creating workflow directory', async () => {
        ...
        await expect(fsp.access(path.join(h.workspace, '.github'))).rejects.toBeDefined();
    });
    ...
    it('rejects a symlinked repository root before creating workflow directories', async () => {
        ...
        await expect(fsp.access(path.join(realRoot, '.github'))).rejects.toBeDefined();
    });
    it('does not use workspace proxy settings before trust is established', async () => {
        ...
        await writeJson(path.join(h.workspace, SETTINGS_DIRECTORY_NAME, 'settings.json'), {
    ...
    it('redacts setup-github filesystem paths from client errors', async () => {
        setupGithubMocks.setupGithub.mockRejectedValueOnce(new setupGithubMocks.SetupGithubError('github_setup_invalid_workspace', `${h.workspace}/.github must not be a symlink.`, 400));
        const res = await request(h.app)
            .post('/workspace/setup-github')
            .set('Host', loopbackHost())
            .set('Authorization', 'Bearer secret')
    ...
    it('uses a generic setup-github message for unexpected errors', async () => {
        setupGithubMocks.setupGithub.mockRejectedValueOnce(new Error(`${h.workspace}/internal dependency failed`));
        const res = await request(h.app)
            .post('/workspace/setup-github')
            .set('Host', loopbackHost())
            .set('Authorization', 'Bearer secret')
        ...
        expect(mockWriteStderrLine).toHaveBeenCalledWith(expect.stringContaining('[setup-github] unexpected error:'));
    });
    it('maps release lookup failure to 502', async () => {
        setupGithubMocks.setupGithub.mockRejectedValueOnce(new setupGithubMocks.SetupGithubError('github_release_lookup_failed', 'Unable to look up release', 502));
        const res = await request(h.app)
            .post('/workspace/setup-github')
            .set('Host', loopbackHost())
            .set('Authorization', 'Bearer secret')
    ...
    it('surfaces workflow write failure as partial', async () => {
        ...
        setupGithubMocks.setupGithub.mockRejectedValueOnce(new setupGithubMocks.SetupGithubError('github_workflow_write_failed', 'Unable to write workflow', 500, partial));
        const res = await request(h.app)
            .post('/workspace/setup-github')
            .set('Host', loopbackHost())
            .set('Authorization', 'Bearer secret')
...
//# sourceMappingURL=workspace-setup-github.test.js.map