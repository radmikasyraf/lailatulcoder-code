...
import { coverageFromTranscripts, TranscriptsUnavailableError, } from './lib/coverage.js';
...
function runCheckCoverage(args) {
    ...
    const totalChunks = report.coveredChunks.length +
    ...
    writeStderrLine(`Coverage: ${report.coveredChunks.length}/${totalChunks} chunk(s) reviewed. ` +
    ...
    if (report.blindAgents.length > 0) {
        writeStderrLine(`ERROR: ${report.blindAgents.length} agent(s) were launched with a prompt ` +
            ...
            `Build each prompt with \`"\${QWEN_CODE_CLI:-LailatulCoder}" review agent-prompt ` +
    ...
    if (report.rewrittenPrompts.length > 0) {
        writeStderrLine(`ERROR: ${report.rewrittenPrompts.length} agent(s) were not launched with ` +
            ...
            `of its own. Re-run \`"\${QWEN_CODE_CLI:-LailatulCoder}" review agent-prompt\` and ` +
            `pass its output ` +
    ...
    if (report.driftedLaunches.length > 0) {
        writeStderrLine(`NOTE: ${report.driftedLaunches.length} agent(s) launched with a ` +
    ...
    if (report.missingRoles.length > 0) {
        ...
        `ERROR: required briefs never reached their agents — ` +
            ...
            `\`"\${QWEN_CODE_CLI:-LailatulCoder}" review agent-prompt --plan ${shellQuotePath(args.plan)} --roster\` ` +
    ...
    if (report.unreadBriefs.length > 0) {
        writeStderrLine(`ERROR: ${report.unreadBriefs.length} agent(s) never opened their brief — ` +
    ...
    if (report.unopenedAgents.length > 0) {
        writeStderrLine(`ERROR: ${report.unopenedAgents.length} agent(s) were pointed at diff lines ` +
    ...
    if (report.missingChunks.length > 0) {
        writeStderrLine('NOTE: a chunk counts as read when an agent was pointed at its lines AND ' +
    ...
    if (report.idleAgents.length > 0) {
        writeStderrLine(`ERROR: ${report.idleAgents.length} agent(s) made no tool call — ` +
    ...
    if (report.uncoverableChunks.length > 0) {
        writeStderrLine(`ERROR: ${report.uncoverableChunks.length} chunk(s) were declared ` +
    ...
    if (report.missingChunks.length > 0) {
        writeStderrLine(`ERROR: ${report.missingChunks.length} chunk(s) were not reviewed — ` +
    ...
    if (report.budgetGaps.length > 0) {
        ...
        writeStderrLine(`NOTE: ${total} budget-gap disclosure(s) from ` +
...
export const checkCoverageCommand = {
    command: 'check-coverage',
    describe: "Prove the diff was read, from the harness's own per-agent transcripts " +
