...
import { generateToolUseId, firePreToolUseHook, firePostToolUseHook, firePostToolUseFailureHook, firePostToolBatchHook, fireNotificationHook, firePermissionRequestHook, appendAdditionalContext, } from './toolHookTriggers.js';
import { NotificationType } from '../hooks/types.js';
import { COMBINED_PASS_TOLERANCE_FACTOR, truncateLlmContent, truncateToolOutput, TOOL_OUTPUT_TRUNCATED_PREFIX, } from '../utils/truncation.js';
import { finalizeToolResponses, toolResponseTextLength, } from '../utils/tool-response-finalizer.js';
import { ToolConfirmationOutcome, ApprovalMode, logToolCall, ToolErrorType, ToolCallEvent, InputFormat, Kind, } from '../index.js';
...
import { collectAvailableSkillEntries, renderAvailableSkillsBlock, } from '../tools/skill-utils.js';
import { escapeSystemReminderTags } from '../utils/xml.js';
import { promptIdContext, todoWorkChainContext, } from '../utils/promptIdContext.js';
import { isToolResultBoundaryDiagnosticsEnabled, observeToolResultBoundary, toolResultBoundaryArtifact, toolResultPartDiagnosticValues, } from '../utils/tool-result-boundary-diagnostics.js';
...
import { isAlreadyTruncated, persistAndTruncateToolResult, } from '../utils/truncation.js';
import { injectPermissionRulesIfMissing, persistPermissionOutcome, } from './permission-helpers.js';
import { evaluatePermissionFlow, getEffectivePermissionForConfirmation, needsConfirmation, isPlanModeBlocked, isAutoEditApproved, } from './permissionFlow.js';
import { decoratePlanModeShellConfirmation, evaluatePlanModeShellPolicy, validatePlanModeShellApproval, validatePlanModeShellContext, } from './plan-mode-shell-policy.js';
import { findPlanModeEntryBatchBoundaryIndex, PLAN_MODE_ENTRY_SIBLING_SKIP_MESSAGE, } from './plan-mode-entry-policy.js';
import { applyAutoModeDecision, decorateClassifierUnavailableConfirmation, evaluateAutoMode, getAutoModePermissionDeniedReason, shouldClassifyAllShellForAutoMode, shouldForceAutoModeReviewForAllow, shouldFirePermissionDeniedForAutoMode, shouldRunAutoModeForCall, } from '../permissions/autoMode.js';
import { MAX_TRANSCRIPT_MESSAGES } from '../permissions/classifier-transcript.js';
import { formatDenialStateLog, isApproveOutcome, isDenialFallbackReason, recordAllow, recordFallbackApprove, shouldFallback, } from '../permissions/denialTracking.js';
import { getResponseTextFromParts, TOOL_SUCCEEDED_OUTPUT, } from '../utils/generateContentResponseUtilities.js';
...
import { getPlanRequiredTeammatePreApprovalMessage, isPlanRequiredTeammateAwaitingApproval, isPlanRequiredTeammatePreApprovalAllowedTool, shouldUsePlanOnlyReminderInSubagentContext, } from '../agents/runtime/subagent-plan-tool-policy.js';
import { safeSetStatus } from '../telemetry/tracer.js';
import { TOOL_FAILURE_KIND_ATTRIBUTE, TOOL_FAILURE_KIND_BACKGROUND_AGENT_DENIED, TOOL_FAILURE_KIND_CANCELLED, TOOL_FAILURE_KIND_INVOCATION_GUARD_DENIED, TOOL_FAILURE_KIND_NON_INTERACTIVE_DENIED, TOOL_FAILURE_KIND_PERMISSION_DENIED, TOOL_FAILURE_KIND_PERMISSION_HOOK_DENIED, TOOL_FAILURE_KIND_PLAN_MODE_BLOCKED, TOOL_FAILURE_KIND_POST_HOOK_STOPPED, TOOL_FAILURE_KIND_PRE_HOOK_BLOCKED, TOOL_FAILURE_KIND_TIMEOUT, TOOL_FAILURE_KIND_TOOL_ERROR, TOOL_FAILURE_KIND_TOOL_EXCEPTION, } from '../telemetry/constants.js';
import { SpanStatusCode } from '@opentelemetry/api';
import { startToolSpan, endToolSpan, runInToolSpanContext, startToolExecutionSpan, endToolExecutionSpan, startToolBlockedOnUserSpan, endToolBlockedOnUserSpan, startHookSpan, endHookSpan, addToolArgumentsAttributes, addToolCallResultAttributes, truncateSpanError, } from '../telemetry/index.js';
import { acquireSleepInhibitor } from '../services/sleepInhibitor.js';
import { getRuntimeContentGenerator, runWithRuntimeContentGenerator, } from '../agents/runtime/agent-context.js';
import { isImagePart, normalizeParts, } from '../services/visionBridge/image-part-utils.js';
import { bridgeToolResultImages } from '../services/visionBridge/tool-result-vision-bridge.js';
import { getInvocationContext, runWithInvocationContext, } from '../utils/invocation-context.js';
...
function dedupeRequestsByCallId(requests) {
    const seenCallIds = new Set();
    const deduped = [];
    for (const request of requests) {
        if (request.callId) {
            if (seenCallIds.has(request.callId)) {
                debugLogger.debug(`dedupeRequestsByCallId: dropping duplicate callId=${request.callId} name=${request.name}`);
                continue;
            }
...
function runInRequestGoalContext(request, callback) {
...
function extractTextFromPartListUnion(c) {
    if (typeof c === 'string')
        return c;
    if (Array.isArray(c)) {
...
const TOOL_SPAN_STATUS_INVOCATION_GUARD_DENIED = 'Tool execution blocked by host policy';
...
const TOOL_SPAN_STATUS_PERMISSION_HOOK_DENIED = 'Permission denied by permission_request hook';
const TOOL_SPAN_STATUS_PLAN_MODE_BLOCKED = 'Plan mode blocked a non-read-only tool call';
const TOOL_SPAN_STATUS_NON_INTERACTIVE_DENIED = 'Non-interactive mode declined permission';
const TOOL_SPAN_STATUS_BACKGROUND_AGENT_DENIED = 'Background agent cannot prompt for confirmation';
...
const TOOL_CANCELLED_BEFORE_COMPLETION_MESSAGE = 'User cancelled tool execution.';
const TOOL_CANCELLED_AFTER_COMPLETION_MESSAGE = 'The tool had already completed; its output was discarded.';
/**
 ...
 */
function createToolTimeoutResult(timeoutMs) {
    const display = timeoutMs >= 1000 ? `${Math.round(timeoutMs / 1000)}s` : `${timeoutMs}ms`;
    const message = `Tool execution timed out after ${display}. ` +
...
async function safelyFirePostToolUseFailureHook(messageBus, toolUseId, toolName, toolInput, errorMessage, isInterrupt, permissionMode, tool_call_id) {
    try {
        return await firePostToolUseFailureHook(messageBus, toolUseId, toolName, toolInput, errorMessage, isInterrupt, permissionMode, undefined, tool_call_id);
    }
    catch (error) {
        const message = error instanceof Error ? error.message : String(error);
        debugLogger.warn(`PostToolUseFailure hook failed for ${toolName}: ${message}`);
 ...
 */
const FS_PATH_TOOL_NAMES = new Set([
    ToolNames.READ_FILE,
...
function isFilesystemPathTool(toolName) {
 ...
 */
function joinSearchRootAndGlob(searchRoot, globField) {
 ...
 */
function pushLspPathCandidate(out, v) {
    ...
    if (v.includes('://'))
        return; // non-file URI scheme: ignore
    out.push(v);
}
/**
 ...
 */
export function extractToolFilePaths(toolName, toolInput) {
    ...
    if (!FS_PATH_TOOL_NAMES.has(canonical)) {
        ...
        if (toolInput && typeof toolInput === 'object') {
            ...
            if (typeof obj['file_path'] === 'string' ||
                ...
                debugLogger.debug(`Tool "${toolName}" (canonical "${canonical}") has path-like input fields ` +
    ...
    switch (canonical) {
        case ToolNames.LSP: {
            ...
            if (item && typeof item === 'object') {
                pushLspPathCandidate(out, item['uri']);
            }
            return out;
        }
        case ToolNames.GLOB: {
            ...
            if (typeof patternField === 'string' && patternField.length > 0) {
                push(joinSearchRootAndGlob(typeof pathField === 'string' ? pathField : undefined, patternField));
            }
            return out;
        }
        case ToolNames.GREP: {
            ...
            if (typeof globField === 'string' && globField.length > 0) {
                push(joinSearchRootAndGlob(typeof pathField === 'string' ? pathField : undefined, globField));
            }
            return out;
        }
        case ToolNames.LS:
...
export function convertToFunctionErrorResponse(toolName, callId, llmContent, fallbackError) {
    return convertToFunctionResponse(toolName, callId, llmContent).map((part) => {
        ...
        const error = typeof existingError === 'string' && existingError.trim()
            ? existingError
...
const createErrorResponse = (request, error, errorType, executionStatus, artifacts, resultDisplay) => ({
...
const createCancelledResponse = (request, reason, executionStatus, artifacts, 
...
persistedOutputFiles, visionBridgeNotice) => {
...
function isToolCallResponseInfo(value) {
...
function appendContextToResponsePart(part, additionalContext) {
    if (!part.functionResponse) {
        debugLogger.warn('appendContextToResponsePart: no functionResponse on part, additionalContext dropped');
        return part;
    }
...
function appendContextToToolResponse(response, additionalContext) {
    ...
    const appendedPart = appendContextToResponsePart(responseParts[lastIndex], additionalContext);
...
function withPostToolBatchAdditionalContext(completedCalls, additionalContext) {
    ...
    calls[lastIndex] = {
        ...calls[lastIndex],
        response: appendContextToToolResponse(calls[lastIndex].response, additionalContext),
    };
    return calls;
}
function withPostToolBatchArtifacts(completedCalls, artifacts) {
...
function withPostToolBatchStop(completedCalls, stopReason) {
    ...
    const { executionStatus: _es, ...baseResponse } = createErrorResponse(lastCall.request, new Error(stopReason), ToolErrorType.EXECUTION_DENIED, executionStatus ?? 'not_started');
    const response = executionStatus !== undefined
 ...
 * runs sequentially).
 */
export function isToolCallConcurrencySafe(name, kind, args) {
    // Agent tools spawn independent sub-agents with no shared state.
    if (canonicalToolName(name) === ToolNames.AGENT)
        return true;
...
function isConcurrencySafe(call) {
    return isToolCallConcurrencySafe(call.request.name, call.tool.kind, call.request.args);
}
/**
 ...
 */
export function partitionByConcurrencySafety(items, isSafe) {
...
function producerInputDropsStructuredContent(input) {
    if (input === null || input === undefined || typeof input === 'string') {
        return false;
    }
    const contentToProcess = Array.isArray(input) && input.length === 1 ? input[0] : input;
    if (typeof contentToProcess === 'string')
        return false;
    const hasOnlyKeys = (part, ...keys) => Object.entries(part).every(([key, value]) => value === undefined || keys.includes(key));
    if (Array.isArray(contentToProcess)) {
        return contentToProcess.some((part) => {
            if (typeof part === 'string')
                return false;
            if (part.text !== undefined)
                return !hasOnlyKeys(part, 'text');
            if (part.inlineData !== undefined) {
                return !hasOnlyKeys(part, 'inlineData');
            }
            if (part.fileData !== undefined)
                return !hasOnlyKeys(part, 'fileData');
            return true;
        });
    }
    ...
    if (contentToProcess.inlineData !== undefined ||
...
function producerContentEqual(toolName, callId, input, output) {
    if (producerInputDropsStructuredContent(input))
        return false;
    return isDeepStrictEqual(convertToFunctionResponse(toolName, callId, input ?? ''), output);
}
export class CoreToolScheduler {
    ...
    postToolBatchEnabledForBatch = false;
    postToolBatchSpanCallId;
    postToolBatchConfigWarned = false;
    isScheduling = false;
    validationRetryCounts = new Map();
    autoModeFallbackCallIds = new Set();
    ...
    callIdToPostToolBatchSignal = new Map();
    ...
    bouncedAwaitingApproval = new Set();
    ...
    bouncedToolUseId = new Map();
    runtimeContentGeneratorViews = new Map();
    requestQueue = [];
    constructor(options) {
    ...
    compactResultDisplayForInteractiveHistory(resultDisplay) {
        return typeof this.config.isInteractive === 'function' &&
            this.config.isInteractive()
            ? compactToolResultDisplayForHistory(resultDisplay)
            : resultDisplay;
    }
    async processToolResultImages(responseParts, signal) {
        let modelOverride;
        const notices = [];
        const processedParts = await bridgeToolResultImages({
            ...
            onFullTurnModel: (model) => {
                if (!this.onToolResultFullTurnModel?.(model))
                    return false;
                modelOverride = model;
                return true;
            },
    ...
    setStatusInternal(targetCallId, newStatus, auxiliaryData, executionStatus) {
        this.toolCalls = this.toolCalls.map((currentCall) => {
            ...
            switch (newStatus) {
                ...
                case 'cancelled': {
                    ...
                    const preservedResultDisplay = this.compactResultDisplayForInteractiveHistory(resultDisplay);
                    const errorMessage = `[Operation Cancelled] Reason: ${auxiliaryData}`;
                    const response = isToolCallResponseInfo(auxiliaryData)
                        ? {
                            ...auxiliaryData,
                            executionStatus: auxiliaryData.executionStatus ??
                                executionStatus ??
                                'not_started',
                            resultDisplay: auxiliaryData.resultDisplay ?? preservedResultDisplay,
                        }
                        : {
        ...
        void this.checkAndNotifyCompletion().catch((error) => {
            debugLogger.warn(`setStatusInternal completion notification failed: ${error instanceof Error ? error.message : String(error)}`);
        });
    }
    setArgsInternal(targetCallId, args) {
        ...
        this.toolCalls = this.toolCalls.map((call) => {
            ...
            const invocationOrError = runInRequestGoalContext(call.request, () => this.buildInvocation(call.tool, args, targetCallId, call.request.prompt_id));
            if (invocationOrError instanceof Error) {
                invocationError = invocationOrError;
                const response = createErrorResponse(call.request, invocationOrError, ToolErrorType.INVALID_TOOL_PARAMS, 'not_started');
                return {
        ...
        if (invocationError) {
            ...
            this.notifyToolCallsUpdate();
            void this.checkAndNotifyCompletion().catch((error) => {
                debugLogger.warn(`setArgsInternal completion notification failed: ${error instanceof Error ? error.message : String(error)}`);
            });
            return false;
        }
        return argsUpdated;
    ...
    cancelPreExecutionIfAborted(callId, signal, toolSpan = this.toolSpans.get(callId)) {
        if (!signal.aborted)
            return false;
        this.setStatusInternal(callId, 'cancelled', 'Tool call cancelled by user.', 'not_started');
     ...
     * (#4321 review).
     */
    finalizeToolSpan(callId, force = false) {
        ...
        if (callId === this.postToolBatchSpanCallId && !force)
            return;
     ...
     * trips.
     */
    finalizeBlockedSpan(callId, decision, source) {
     ...
     */
    releaseBatchListenerIfDrained(callId) {
        ...
        for (const id of batch.callIds) {
            if (this.toolSpans.has(id) || this.blockedSpans.has(id))
                return;
        }
        ...
        setTimeout(() => {
            for (const callId of ids) {
                ...
                try {
                    ...
                    debugLogger.warn(`drainSpansForBatch: failed to drain ${callId}: ${e instanceof Error ? e.message : String(e)}`);
                }
            }
        }, 0);
    }
     ...
     */
    postToolUseFailureEndMeta = (r) => r.hookError
     ...
     */
    async withHookSpan(opts, fn, toEndMeta) {
     ...
     */
    buildInvocation(tool, args, callId, promptId) {
        try {
            const invocation = tool.build(structuredClone(args));
            if (callId) {
                const maybeAware = invocation;
     ...
     */
    async getToolNotFoundMessage(unknownToolName, topN = 3) {
     ...
     */
    getMcpToolUnavailableMessage(unknownToolName) {
        ...
        const prefixOf = (server) => sanitizeToolNameForProvider(`mcp__${server}__`);
        ...
        const byPrefixLengthDesc = (a, b) => prefixOf(b).length - prefixOf(a).length;
        ...
        const serverHit = candidates.find((s) => unknownToolName.startsWith(prefixOf(s)));
        if (!serverHit) {
    ...
    schedule(request, signal, runtimeView) {
     ...
     */
    recordRetryableToolError(toolName, errorMessage) {
    ...
    async _schedule(request, signal, runtimeView) {
        if (runtimeView) {
            ...
            try {
                return await runWithRuntimeContentGenerator(runtimeView, () => this._schedule(request, signal));
            }
            catch (error) {
                for (const item of items) {
        ...
        try {
            ...
            const requestsToProcess = dedupeRequestsByCallId(Array.isArray(request) ? request : [request]).map((item) => ({ ...item, args: structuredClone(item.args) }));
            ...
            // derives from it.
            const planModeEntryBoundaryIndex = findPlanModeEntryBatchBoundaryIndex(requestsToProcess.map((item) => canonicalToolName(item.name)));
            ...
            const newToolCalls = [];
            const retryErrorsRecordedInBatch = new Map();
            const recordBatchRetryableToolError = (toolName, errorMessage) => {
            ...
            for (const [requestIndex, reqInfo] of requestsToProcess.entries()) {
                ...
                const recordPrevalidationCancellation = () => {
                    if (!signal.aborted)
                        return false;
                    newToolCalls.push({
                        status: 'cancelled',
                        request: reqInfo,
                        response: createCancelledResponse(reqInfo, 'Tool call cancelled before execution.', 'not_started'),
                ...
                try {
                    if (recordPrevalidationCancellation())
                        continue;
                    if (planModeEntryBoundaryIndex !== undefined &&
                        requestIndex !== planModeEntryBoundaryIndex) {
                        newToolCalls.push({
                            status: 'error',
                            request: reqInfo,
                            response: createErrorResponse(reqInfo, new Error(PLAN_MODE_ENTRY_SIBLING_SKIP_MESSAGE), ToolErrorType.EXECUTION_DENIED, 'not_started'),
                            durationMs: 0,
                        });
                        continue;
                    }
                    ...
                    if (pm && !permissionEnabled) {
                        ...
                        newToolCalls.push({
                            status: 'error',
                            request: reqInfo,
                            response: createErrorResponse(reqInfo, new Error(permissionErrorMessage), ToolErrorType.EXECUTION_DENIED, 'not_started'),
                            durationMs: 0,
                        });
                        continue;
                    }
                    ...
                    const toolInstance = await runInRequestGoalContext(reqInfo, () => this.toolRegistry.ensureTool(canonicalName));
                    resolvedTool = toolInstance;
                    if (recordPrevalidationCancellation())
                        continue;
                    if (!toolInstance) {
                        // Tool is not in registry and not excluded - likely hallucinated or typo
                        const errorMessage = await runInRequestGoalContext(reqInfo, () => this.getToolNotFoundMessage(reqInfo.name));
                        if (recordPrevalidationCancellation())
                            continue;
                        newToolCalls.push({
                            status: 'error',
                            request: reqInfo,
                            response: createErrorResponse(reqInfo, new Error(errorMessage), ToolErrorType.TOOL_NOT_REGISTERED, 'not_started'),
                            durationMs: 0,
                        });
                        continue;
                    }
                    ...
                    if (reqInfo.wasOutputTruncated && toolInstance.kind === Kind.Edit) {
                        const count = recordBatchRetryableToolError(reqInfo.name, TRUNCATION_EDIT_REJECTION);
                        const truncationError = new Error(count >= VALIDATION_RETRY_LOOP_THRESHOLD
                        ...
                        newToolCalls.push({
                            ...
                            request: reqInfo,
                            tool: toolInstance,
                            response: createErrorResponse(reqInfo, truncationError, ToolErrorType.OUTPUT_TRUNCATED, 'not_started'),
                            durationMs: 0,
                        });
                        continue;
                    }
                    const invocationOrError = runInRequestGoalContext(reqInfo, () => this.buildInvocation(toolInstance, reqInfo.args, reqInfo.callId, reqInfo.prompt_id));
                    if (recordPrevalidationCancellation())
                        continue;
                    if (invocationOrError instanceof Error) {
                        ...
                        const count = recordBatchRetryableToolError(reqInfo.name, invocationOrError.message);
                        const finalError = count >= VALIDATION_RETRY_LOOP_THRESHOLD
                        ...
                        newToolCalls.push({
                            ...
                            request: reqInfo,
                            tool: toolInstance,
                            response: createErrorResponse(reqInfo, finalError, ToolErrorType.INVALID_TOOL_PARAMS, 'not_started'),
                            durationMs: 0,
                        });
                        continue;
                    }
                    ...
                    newToolCalls.push({
                        status: 'error',
                        request: reqInfo,
                        response: createErrorResponse(reqInfo, normalizedError, error?.errorType ??
                            ToolErrorType.UNHANDLED_EXCEPTION, 'not_started'),
                        ...(resolvedTool ? { tool: resolvedTool } : {}),
                        durationMs: 0,
                    });
                }
            ...
            const postToolBatchParentCallId = newToolCalls.findLast((toolCall) => toolCall.status === 'validating')?.request.callId;
            ...
            try {
                ...
                if (!this.postToolBatchConfigWarned) {
                    ...
                    debugLogger.warn('PostToolBatch hook detection failed; deferring span as a precaution:', configError);
                }
            }
            this.notifyToolCallsUpdate();
            ...
            for (const toolCall of newToolCalls) {
                ...
                }, toolCall.tool.description, reqInfo.prompt_id);
                ...
                try {
                    if (this.cancelPreExecutionIfAborted(reqInfo.callId, signal, toolSpan)) {
                        continue;
                    }
                    // =================================================================
                    ...
                    const flowResult = await runInRequestGoalContext(reqInfo, () => evaluatePermissionFlow(this.config, invocation, canonicalName, toolParams));
                    if (this.cancelPreExecutionIfAborted(reqInfo.callId, signal, toolSpan)) {
                        continue;
                    }
                    const { defaultPermission, finalPermission, pmForcedAsk, pmCtx, denyMessage, requiresUserInteraction, } = flowResult;
                    ...
                    const isEnterPlanModeTool = canonicalName === ToolNames.ENTER_PLAN_MODE;
                    ...
                    if (finalPermission === 'allow' && forceAutoReviewForAllow) {
                        debugLogger.info(`Auto mode: L4 allow overridden by protected-write guard for ${canonicalName}`);
                    }
                    if (isPlanRequiredTeammateAwaitingApproval(this.config) &&
                        finalPermission !== 'deny') {
                        const isExplicitPreApprovalTool = isPlanRequiredTeammatePreApprovalAllowedTool(canonicalName, toolParams);
                        const canRunBeforeLeaderApproval = isExplicitPreApprovalTool &&
                        ...
                        const message = getPlanRequiredTeammatePreApprovalMessage(canonicalName);
                        this.setStatusInternal(reqInfo.callId, 'error', createErrorResponse(reqInfo, new Error(message), ToolErrorType.EXECUTION_DENIED, 'not_started'));
                        setToolSpanFailure(toolSpan, TOOL_FAILURE_KIND_PLAN_MODE_BLOCKED, TOOL_SPAN_STATUS_PLAN_MODE_BLOCKED);
                        this.finalizeToolSpan(reqInfo.callId);
                        continue;
                    }
                    if (finalPermission === 'deny') {
                        // Hard deny: security violation or PM explicit deny
                        this.setStatusInternal(reqInfo.callId, 'error', createErrorResponse(reqInfo, new Error(denyMessage ?? `Tool "${reqInfo.name}" is denied.`), ToolErrorType.EXECUTION_DENIED, 'not_started'));
                        setToolSpanFailure(toolSpan, TOOL_FAILURE_KIND_PERMISSION_DENIED, TOOL_SPAN_STATUS_PERMISSION_DENIED);
                        this.finalizeToolSpan(reqInfo.callId);
                        continue;
                    }
                    ...
                    const planShellDecision = isPlanShellCall
                        ? await runInRequestGoalContext(reqInfo, () => evaluatePlanModeShellPolicy({
                            config: this.config,
                            toolName: canonicalName,
                    ...
                    const rejectPlanShell = (message) => {
                        this.setStatusInternal(reqInfo.callId, 'error', {
                            ...createErrorResponse(reqInfo, new Error(message), ToolErrorType.EXECUTION_DENIED, 'not_started'),
                    ...
                    if (planShellDecision.classification !== 'not-applicable') {
                        const initialPlanShellError = await runInRequestGoalContext(reqInfo, () => validatePlanModeShellContext({
                            config: this.config,
                    ...
                    if (!requiresUserInteraction &&
                        ...
                        const decision = await runInRequestGoalContext(reqInfo, () => evaluateAutoMode({
                            ctx: pmCtx,
                            pmForcedAsk,
                            toolParams,
                            messages,
                        ...
                        if (!this.config.getDisableAllHooks() &&
                            ...
                            try {
                                await runInRequestGoalContext(reqInfo, () => this.config
                                    .getHookSystem?.()
                                    ?.firePermissionDeniedEvent(canonicalName, toolParams, reqInfo.callId, getAutoModePermissionDeniedReason(decision), signal, reqInfo.callId));
                            }
                            catch (hookError) {
                                debugLogger.warn(`PermissionDenied hook failed for tool ${reqInfo.callId}: ${hookError instanceof Error ? hookError.message : String(hookError)}`);
                            }
                        }
                        if (this.cancelPreExecutionIfAborted(reqInfo.callId, signal, toolSpan)) {
                            continue;
                        ...
                        switch (outcome.kind) {
                            case 'approved':
                                ...
                                continue;
                            case 'blocked':
                                debugLogger.warn(`Auto mode blocked (${outcome.reason}): tool=${canonicalName}, ` +
                                    formatDenialStateLog(denialState));
                                this.setStatusInternal(reqInfo.callId, 'error', createErrorResponse(reqInfo, new Error(outcome.errorMessage), ToolErrorType.EXECUTION_DENIED, 'not_started'));
                                setToolSpanFailure(toolSpan, TOOL_FAILURE_KIND_PERMISSION_DENIED, outcome.errorMessage);
                                this.finalizeToolSpan(reqInfo.callId);
                                continue;
                            case 'fallback':
                    ...
                    if (!needsConfirmation(planShellRequiresConfirmation ? 'ask' : confirmationPermission, approvalMode, canonicalName, requiresUserInteraction)) {
                        this.setToolCallOutcome(reqInfo.callId, ToolConfirmationOutcome.ProceedAlways);
                        this.setStatusInternal(reqInfo.callId, 'scheduled');
                    }
                    else {
                        confirmationDetails = await runInRequestGoalContext(reqInfo, () => invocation.getConfirmationDetails(signal));
                        if (this.cancelPreExecutionIfAborted(reqInfo.callId, signal, toolSpan)) {
                            continue;
                        }
                        ...
                        if (planShellDecision.classification !== 'not-applicable') {
                            const preDisplayPlanShellError = await runInRequestGoalContext(reqInfo, () => validatePlanModeShellContext({
                                config: this.config,
                                decision: planShellDecision,
                                requestArgs: reqInfo.args,
                                invocationParams: invocation.params,
                        ...
                        if (planShellDecision.classification === 'not-applicable' &&
                            ...
                            const isPlanRequiredTeammate = !shouldUsePlanOnlyReminderInSubagentContext() &&
                                !this.config.getSdkMode();
                            const planModeError = new Error(`Tool blocked by plan mode: "${reqInfo.name}" is not a read-only tool. ` +
                            ...
                            this.setStatusInternal(reqInfo.callId, 'error', {
                                ...createErrorResponse(reqInfo, planModeError, ToolErrorType.EXECUTION_DENIED, 'not_started'),
                         ...
                         */
                        const isNonInteractiveDeny = !this.config.isInteractive() &&
                            !this.config.getExperimentalZedIntegration() &&
                            this.config.getInputFormat() !== InputFormat.STREAM_JSON;
                        if (isNonInteractiveDeny) {
                            const errorMessage = planShellDecision.classification === 'unknown'
                            ...
                            this.setStatusInternal(reqInfo.callId, 'error', createErrorResponse(reqInfo, new Error(errorMessage), ToolErrorType.EXECUTION_DENIED, 'not_started'));
                            setToolSpanFailure(toolSpan, TOOL_FAILURE_KIND_NON_INTERACTIVE_DENIED, TOOL_SPAN_STATUS_NON_INTERACTIVE_DENIED);
                            this.finalizeToolSpan(reqInfo.callId);
                            continue;
                        }
                        ...
                        if (hooksEnabled && messageBus) {
                            ...
                            const hookResult = await runInRequestGoalContext(reqInfo, () => firePermissionRequestHook(messageBus, canonicalName, reqInfo.args || {}, permissionMode, undefined, signal));
                            if (this.cancelPreExecutionIfAborted(reqInfo.callId, signal, toolSpan)) {
                                continue;
                            }
                            if (hookResult.hasDecision &&
                                ...
                                if (hookResult.shouldAllow) {
                                    if (planShellDecision.classification !== 'not-applicable') {
                                        const approval = await runInRequestGoalContext(reqInfo, () => validatePlanModeShellApproval({
                                            config: this.config,
                                            decision: planShellDecision,
                                            requestArgs: reqInfo.args,
                                            invocationParams: invocation.params,
                                        ...
                                        if (approval.outcome === ToolConfirmationOutcome.Cancel) {
                                            await runInRequestGoalContext(reqInfo, () => preparedConfirmationDetails.onConfirm(approval.outcome, approval.payload));
                                            if (this.cancelPreExecutionIfAborted(reqInfo.callId, signal, toolSpan)) {
                                                continue;
                                            }
                                            rejectPlanShell(approval.payload?.cancelMessage ??
                                                planShellDecision.noApprovalMessage);
                                            continue;
                                        }
                                        await runInRequestGoalContext(reqInfo, () => preparedConfirmationDetails.onConfirm(approval.outcome, approval.payload));
                                        ...
                                        this.recordAutoModeFallbackResolution(reqInfo.callId, approval.outcome);
                                    ...
                                    if (hookResult.updatedInput &&
                                        ...
                                        if (!this.setArgsInternal(reqInfo.callId, hookResult.updatedInput)) {
                                            continue;
                                        }
                                    }
                                    await runInRequestGoalContext(reqInfo, () => preparedConfirmationDetails.onConfirm(ToolConfirmationOutcome.ProceedOnce));
                                    if (this.cancelPreExecutionIfAborted(reqInfo.callId, signal, toolSpan)) {
                                        continue;
                                    }
                                    this.recordAutoModeFallbackResolution(reqInfo.callId, ToolConfirmationOutcome.ProceedOnce);
                                    ...
                                    const cancelPayload = hookResult.denyMessage
                                        ...
                                        : undefined;
                                    await runInRequestGoalContext(reqInfo, () => preparedConfirmationDetails.onConfirm(ToolConfirmationOutcome.Cancel, cancelPayload));
                                    if (this.cancelPreExecutionIfAborted(reqInfo.callId, signal, toolSpan)) {
                                        continue;
                                    }
                                    this.recordAutoModeFallbackResolution(reqInfo.callId, ToolConfirmationOutcome.Cancel);
                                    ...
                                    this.setStatusInternal(reqInfo.callId, 'error', createErrorResponse(reqInfo, new Error(hookResult.denyMessage ||
                                        `Permission denied by hook for "${reqInfo.name}"`), ToolErrorType.EXECUTION_DENIED, 'not_started'));
                                    setToolSpanFailure(toolSpan, TOOL_FAILURE_KIND_PERMISSION_HOOK_DENIED, TOOL_SPAN_STATUS_PERMISSION_HOOK_DENIED);
                                    this.finalizeToolSpan(reqInfo.callId);
                                }
                                continue;
                            }
                        ...
                        if (this.config.getShouldAvoidPermissionPrompts?.()) {
                            ...
                            this.setStatusInternal(reqInfo.callId, 'error', createErrorResponse(reqInfo, new Error(errorMessage), ToolErrorType.EXECUTION_DENIED, 'not_started'));
                            setToolSpanFailure(toolSpan, TOOL_FAILURE_KIND_BACKGROUND_AGENT_DENIED, TOOL_SPAN_STATUS_BACKGROUND_AGENT_DENIED);
                            this.finalizeToolSpan(reqInfo.callId);
                            continue;
                        }
                        ...
                        if (planShellDecision.classification !== 'not-applicable') {
                            const finalPreDisplayPlanShellError = await runInRequestGoalContext(reqInfo, () => validatePlanModeShellContext({
                                config: this.config,
                                decision: planShellDecision,
                                requestArgs: reqInfo.args,
                                invocationParams: invocation.params,
                        ...
                        const wrappedConfirmationDetails = {
                            ...
                            ...(pmForcedAsk || requiresUserInteraction
                                ...
                                : {}),
                            onConfirm: async (outcome, payload) => runWithInvocationContext(invocationContext, async () => {
                                await runInRequestGoalContext(reqInfo, async () => {
                                    if (planShellDecision.classification !== 'not-applicable') {
                                        if (planShellResponseClaimed)
                                            return;
                                        planShellResponseClaimed = true;
                                        const currentCall = this.toolCalls.find((call) => call.request.callId === reqInfo.callId &&
                                        ...
                                        await this.handleConfirmationResponse(reqInfo.callId, originalOnConfirm, approval.outcome, signal, approval.payload);
                                        return;
                                    }
                                    await this.handleConfirmationResponse(reqInfo.callId, originalOnConfirm, outcome, signal, payload);
                                });
                            }),
                        };
                        ...
                        if (hooksEnabled && messageBus) {
                            fireNotificationHook(messageBus, `LailatulCoder Ai needs your permission to use ${reqInfo.name}`, NotificationType.PermissionPrompt, 'Permission needed').catch((error) => {
                    ...
                    if (signal.aborted) {
                        this.setStatusInternal(reqInfo.callId, 'cancelled', 'Tool call cancelled by user.', 'not_started');
                    ...
                    const explicitErrorType = error?.errorType;
                    this.setStatusInternal(reqInfo.callId, 'error', createErrorResponse(reqInfo, error instanceof Error ? error : new Error(String(error)), explicitErrorType ?? ToolErrorType.UNHANDLED_EXCEPTION, 'not_started'));
                    ...
                    setToolSpanFailure(toolSpan, TOOL_FAILURE_KIND_TOOL_EXCEPTION, error instanceof Error ? error.message : String(error));
                    this.finalizeToolSpan(reqInfo.callId);
                }
            }
            await this.attemptExecutionOfScheduledCalls(signal);
            void this.checkAndNotifyCompletion().catch((error) => {
                debugLogger.warn(`_schedule completion notification failed: ${error instanceof Error ? error.message : String(error)}`);
            });
    ...
    async handleConfirmationResponse(callId, originalOnConfirm, outcome, signal, payload) {
        ...
        if (runtimeView && getRuntimeContentGenerator() !== runtimeView) {
            return runWithRuntimeContentGenerator(runtimeView, () => this.handleConfirmationResponse(callId, originalOnConfirm, outcome, signal, payload));
        }
        ...
        if (goalTurnContext.getStore() !== toolCall.request.goalContext) {
            return runInRequestGoalContext(toolCall.request, () => this.handleConfirmationResponse(callId, originalOnConfirm, outcome, signal, payload));
        }
        try {
            await this._handleConfirmationResponseInner(callId, toolCall, originalOnConfirm, outcome, signal, payload);
        }
        catch (error) {
            ...
            if (aborted) {
                ...
                this.setStatusInternal(callId, 'error', createErrorResponse(toolCall.request, error instanceof Error ? error : new Error(String(error)), ToolErrorType.UNHANDLED_EXCEPTION, 'not_started'));
            }
            ...
            // wenshao Suggestion).
            debugLogger.warn(`handleConfirmationResponse failed for ${callId}: ${error instanceof Error ? error.message : String(error)}`);
            throw error;
        }
    ...
    async _handleConfirmationResponseInner(callId, toolCall, originalOnConfirm, outcome, signal, payload) {
        const shouldSwitchToDefault = outcome === ToolConfirmationOutcome.ProceedOnceAndSwitchToDefault;
        ...
        if (outcome === ToolConfirmationOutcome.Cancel || signal.aborted) {
            ...
            this.finalizeBlockedSpan(callId, explicitCancel ? 'cancel' : 'aborted', explicitCancel ? this.getBlockedSource() : 'system');
            this.finalizeToolSpan(callId);
        }
            ...
            if (waitingToolCall.confirmationDetails.type === 'edit' &&
                ...
                if (!editorType) {
                    ...
                    debugLogger.warn(`ModifyWithEditor requested for ${callId} but no editor available — tool stays in awaiting_approval; user can recover via Cancel/Proceed`);
                ...
                if (!this.bouncedAwaitingApproval.has(callId)) {
                    for (const key of PATH_ARG_KEYS) {
                        if (typeof normalizedArgs[key] === 'string') {
                            normalizedArgs[key] = unescapePath(String(normalizedArgs[key]).trim());
                        }
                    }
                }
                const { updatedParams, updatedDiff } = await modifyWithEditor(normalizedArgs, modifyContext, editorType, signal, this.onEditorClose);
                if (!this.setArgsInternal(callId, updatedParams))
                    return;
                this.setStatusInternal(callId, 'awaiting_approval', {
                    ...waitingToolCall.confirmationDetails,
            ...
            if (payload?.newContent && toolCall) {
                if (!(await this._applyInlineModify(toolCall, payload, signal))) {
                    return;
                }
            }
            ...
            const decision = outcome === ToolConfirmationOutcome.ProceedOnce
                ? 'proceed_once'
                : 'proceed_always';
    ...
    recordAutoModeFallbackResolution(callId, outcome) {
    ...
    async openIdeDiffIfEnabled(confirmationDetails, callId, signal) {
        ...
        try {
            ...
            if (!ideClient.isDiffingEnabled())
                return;
            resolution = await ideClient.openDiff(confirmationDetails.filePath, confirmationDetails.newContent);
        }
        catch (error) {
            if (!signal.aborted) {
                debugLogger.warn(`IDE diff open failed for ${callId}: ${error instanceof Error ? error.message : String(error)}`);
            }
            return;
        }
        ...
        if (resolution.status === 'accepted') {
            ...
            await this.handleConfirmationResponse(callId, confirmationDetails.onConfirm, ToolConfirmationOutcome.ProceedOnce, signal, userEdited ? { newContent: resolution.content } : undefined);
        }
        else {
            await this.handleConfirmationResponse(callId, confirmationDetails.onConfirm, ToolConfirmationOutcome.Cancel, signal);
        }
    }
    /**
    ...
    hasExecutingOrAwaitingApprovalCall() {
     ...
     */
    async runConcurrently(calls, signal) {
    ...
    async executeSingleToolCall(toolCall, signal) {
        if (toolCall.status !== 'scheduled')
            return;
        if (goalTurnContext.getStore() !== toolCall.request.goalContext) {
            return runInRequestGoalContext(toolCall.request, () => this.executeSingleToolCall(toolCall, signal));
        }
        const scheduledCall = toolCall;
        const { callId, name: toolName } = scheduledCall.request;
        const runtimeView = this.runtimeContentGeneratorViews.get(callId);
        if (runtimeView && getRuntimeContentGenerator() !== runtimeView) {
            return runWithRuntimeContentGenerator(runtimeView, () => this.executeSingleToolCall(toolCall, signal));
        }
        ...
        if (!toolSpan) {
            ...
            }, scheduledCall.tool.description, scheduledCall.request.prompt_id);
            this.toolSpans.set(callId, toolSpan);
        }
        try {
            await runInToolSpanContext(toolSpan, () => this._executeToolCallBody(scheduledCall, signal, toolSpan));
        }
        catch (error) {
            this.bouncedAwaitingApproval.delete(callId);
            this.bouncedToolUseId.delete(callId);
            ...
            setToolSpanFailure(toolSpan, TOOL_FAILURE_KIND_TOOL_EXCEPTION, errorMessage);
            this.setStatusInternal(callId, 'error', createErrorResponse(scheduledCall.request, error instanceof Error ? error : new Error(errorMessage), ToolErrorType.UNHANDLED_EXCEPTION, this.toolCalls.find((call) => call.request.callId === callId)
                ?.status === 'executing'
                ? 'error'
                : 'not_started'));
        }
     ...
     */
    canPromptForAskBounce() {
        const isNonInteractive = !this.config.isInteractive() &&
            !this.config.getExperimentalZedIntegration() &&
            this.config.getInputFormat() !== InputFormat.STREAM_JSON;
        if (isNonInteractive) {
            return false;
     ...
     */
    bounceToAwaitingApprovalForAsk(scheduledCall, reason, toolSpan, signal) {
        const { callId, name: toolName } = scheduledCall.request;
        ...
        if (!this.config.getDisableAllHooks() && messageBus) {
            fireNotificationHook(messageBus, `LailatulCoder Ai needs your permission to use ${toolName}`, NotificationType.PermissionPrompt, 'Permission needed').catch((error) => {
                debugLogger.warn(`Permission prompt notification hook failed: ${error instanceof Error ? error.message : String(error)}`);
            });
        }
    }
    safelyAddToolArgumentsAttributes(span, argumentsValue) {
        try {
    ...
    safelyAddToolCallResultAttributes(span, result) {
        try {
    ...
    async _executeToolCallBody(scheduledCall, signal, span) {
        ...
        const observeSyntheticProducer = (response) => {
            try {
                if (producerObserved || !this.shouldObserveProducer(callId))
                    return;
                ...
                observeToolResultBoundary({
                    ...
                    toolName: canonicalName,
                    artifacts: [
                        toolResultBoundaryArtifact(response.persistedOutputFiles ?? [], response.artifacts ?? []),
                    ],
                    values: () => [
        ...
        if (hooksEnabled && messageBus && !isPostAskReexecution) {
            ...
            const preHookResult = await this.withHookSpan({ hookEvent: 'PreToolUse', toolName: canonicalName, toolUseId }, () => firePreToolUseHook(messageBus, canonicalName, toolInput, toolUseId, permissionMode, undefined, // signal
            callId), (r) => r.hookError
                ? {
                    success: false,
                    error: r.hookError,
            ...
            if (!signal.aborted && !preHookResult.shouldProceed) {
                ...
                if (preHookResult.blockType === 'ask' &&
                    !signal.aborted &&
                    ...
                    this.bounceToAwaitingApprovalForAsk(scheduledCall, preHookResult.blockReason, span, signal);
                    return;
                }
                // Hook blocked the execution.
                const blockMessage = preHookResult.blockReason || 'Tool execution blocked by hook';
                const errorResponse = createErrorResponse(scheduledCall.request, new Error(blockMessage), ToolErrorType.EXECUTION_DENIED, 'not_started');
                observeSyntheticProducer(errorResponse);
                this.setStatusInternal(callId, 'error', errorResponse);
                setToolSpanFailure(span, TOOL_FAILURE_KIND_PRE_HOOK_BLOCKED, TOOL_SPAN_STATUS_PRE_HOOK_BLOCKED);
                return;
            }
        }
        ...
        if (toolInvocationGuard) {
            ...
            if (signal.aborted) {
                const cancelledResponse = createCancelledResponse(scheduledCall.request, 'Tool call cancelled before execution.', 'not_started');
            ...
            if (!guardDecision.allowed) {
                const errorResponse = createErrorResponse(scheduledCall.request, new Error(guardDecision.reason), ToolErrorType.EXECUTION_DENIED, 'not_started');
                observeSyntheticProducer(errorResponse);
                this.setStatusInternal(callId, 'error', errorResponse);
                setToolSpanFailure(span, TOOL_FAILURE_KIND_INVOCATION_GUARD_DENIED, TOOL_SPAN_STATUS_INVOCATION_GUARD_DENIED);
                return;
            }
        }
        if (signal.aborted) {
            const currentCall = this.toolCalls.find((call) => call.request.callId === callId);
            if (currentCall &&
                currentCall.status !== 'success' &&
                currentCall.status !== 'error' &&
                currentCall.status !== 'cancelled') {
        ...
        const liveOutputCallback = scheduledCall.tool.canUpdateOutput
            ? (outputChunk) => {
                ...
                const compactOutput = this.compactResultDisplayForInteractiveHistory(outputChunk);
                if (this.outputUpdateHandler) {
                    this.outputUpdateHandler(callId, outputChunk);
                }
        ...
        //
        const sleepInhibitorHandle = acquireSleepInhibitor(this.config, `LailatulCoder Ai is executing tool ${canonicalName}`);
        let removeParentAbortForward;
        let executionStatus = 'not_started';
        let executionSettled = false;
        let execSpan;
        let producerToolResult;
        ...
        try {
            ...
            const toolExecutionTimeoutMs = Math.min(parsePositiveIntegerEnv(process.env['QWEN_CODE_TOOL_EXECUTION_TIMEOUT_MS'], 0), 2_147_483_647);
            ...
            if (toolExecutionTimeoutMs > 0) {
                ...
                if (signal.aborted) {
                    ...
                    removeParentAbortForward = () => signal.removeEventListener('abort', forwardAbort);
                }
            }
            const inheritedTodoWorkChainId = todoWorkChainContext.getStore();
            const todoWorkChainId = this.config.getActiveTodoWorkChainOwner?.(scheduledCall.request.prompt_id, inheritedTodoWorkChainId) ??
            ...
            if (invocation instanceof ShellToolInvocation) {
                ...
                const setPromoteAbortControllerCallback = (ac) => {
                    this.toolCalls = this.toolCalls.map((tc) => tc.request.callId === callId && tc.status === 'executing'
                ...
                const canPromoteForegroundShell = () => {
                    const promotableShells = this.toolCalls.filter((tc) => tc.status === 'executing' &&
                        tc.request.name === ToolNames.SHELL &&
                        tc.promoteAbortController !== undefined);
                    return (promotableShells.length === 1 &&
                        promotableShells[0]?.request.callId === callId);
                };
                this.safelyAddToolArgumentsAttributes(span, invocation.params);
                promise = todoWorkChainContext.run(todoWorkChainId, () => promptIdContext.run(scheduledCall.request.prompt_id, () => {
                    ...
                    return invocation.execute(execSignal, liveOutputCallback, shellExecutionConfig, setPidCallback, setPromoteAbortControllerCallback, canPromoteForegroundShell);
                }));
            }
            else {
                this.safelyAddToolArgumentsAttributes(span, invocation.params);
                promise = todoWorkChainContext.run(todoWorkChainId, () => promptIdContext.run(scheduledCall.request.prompt_id, () => {
                    ...
                    executionStatus = 'error';
                    return invocation.execute(execSignal, liveOutputCallback, shellExecutionConfig);
                }));
            }
            let toolResult;
                ...
                try {
                    toolResult = await new Promise((resolve, reject) => {
                        timeoutTimer = setTimeout(() => {
                            ...
                            debugLogger.warn(`Tool ${canonicalName} (${callId}) timed out after ` +
                            ...
                            timeoutController?.abort(new DOMException(`Tool execution timed out after ${toolExecutionTimeoutMs}ms`, 'TimeoutError'));
            ...
            observeProducerOutput = (response) => {
                try {
                    if (producerObserved || !this.shouldObserveProducer(callId))
                        return;
                    producerObserved = true;
                    let producerInputValues;
                    const getProducerInputValues = () => (producerInputValues ??= [
                    ...
                    let producerOutputValues;
                    const getProducerOutputValues = () => (producerOutputValues ??= [
                        ...toolResultPartDiagnosticValues(response.responseParts),
                        ...(typeof response.resultDisplay === 'string'
                            ? [
                                {
                    ...
                    const inputArtifact = toolResultBoundaryArtifact(producerToolResult?.persistedOutputFiles, producerToolResult?.artifacts);
                    const outputArtifact = toolResultBoundaryArtifact(response.persistedOutputFiles, response.artifacts);
                    let mutated;
                    const isMutated = () => (mutated ??=
                        producerToolResult == null ||
                            !producerContentEqual(toolName, callId, producerToolResult.llmContent, response.responseParts) ||
                            !isDeepStrictEqual(producerToolResult.returnDisplay, response.resultDisplay) ||
            ...
            const isTimeout = toolResult.error?.type === ToolErrorType.EXECUTION_TIMEOUT &&
            ...
            if (aborted) {
                ...
                if (hooksEnabled && messageBus) {
                    const failureHookResult = await this.withHookSpan({
                        ...
                        toolUseId,
                        isInterrupt: true,
                    }, () => safelyFirePostToolUseFailureHook(messageBus, toolUseId, canonicalName, toolInput, cancelMessage, true, this.config.getApprovalMode(), callId), this.postToolUseFailureEndMeta);
                ...
                const cancelledResponse = createCancelledResponse(scheduledCall.request, cancelMessage, executionStatus, failureHookArtifacts, toolResult.persistedOutputFiles);
                observeProducerOutput(cancelledResponse);
                this.setStatusInternal(callId, 'cancelled', cancelledResponse);
                setToolSpanCancelled(span);
                return; // Both code paths should return here
            }
            const cancelAfterPostProcessing = (artifacts, preserved) => {
                if (!signal.aborted || (isTimeout && parentAbortedAtExecutionSettle)) {
                    return false;
                }
                const cancelledResponse = createCancelledResponse(scheduledCall.request, 
                // Reached only after `execute()` settled with a result.
                TOOL_CANCELLED_AFTER_COMPLETION_MESSAGE, executionStatus, artifacts, preserved?.persistedOutputFiles, preserved?.visionBridgeNotice);
                observeProducerOutput(cancelledResponse);
                this.setStatusInternal(callId, 'cancelled', cancelledResponse);
                setToolSpanCancelled(span);
                return true;
            ...
            if (toolResult.error === undefined) {
                ...
                let persistedOutputFiles = toolResult.persistedOutputFiles
                    ...
                    : toolResult.persistedOutputFiles;
                const mergePersistedOutputFiles = (outputFiles) => {
                    if (outputFiles === undefined)
                        return;
                    persistedOutputFiles = Array.from(new Set([...(persistedOutputFiles ?? []), ...outputFiles]));
                };
                let contentLength = typeof content === 'string' ? content.length : undefined;
                ...
                let postToolUseAdditionalContext;
                let postToolUseArtifacts;
                let reminderEnvelope;
                // PostToolUse Hook
                if (hooksEnabled && messageBus) {
                    const toolResponse = {
                        llmContent: content,
                    ...
                    const permissionMode = this.config.getApprovalMode();
                    const postHookResult = await this.withHookSpan({ hookEvent: 'PostToolUse', toolName: canonicalName, toolUseId }, () => firePostToolUseHook(messageBus, canonicalName, toolInput, toolResponse, toolUseId, permissionMode, undefined, // signal
                    callId), (r) => r.hookError
                        ? {
                            success: false,
                            error: r.hookError,
                    ...
                    if (postHookResult.shouldStop) {
                        ...
                        const errorResponse = createErrorResponse(scheduledCall.request, new Error(stopMessage), ToolErrorType.EXECUTION_DENIED, executionStatus);
                        ...
                        this.setStatusInternal(callId, 'error', errorResponse);
                        setToolSpanFailure(span, TOOL_FAILURE_KIND_POST_HOOK_STOPPED, TOOL_SPAN_STATUS_POST_HOOK_STOPPED);
                        return;
                    }
                }
                // Universal post-execution truncation gate — persists oversized
                // tool results to disk before system-reminders are appended.
                const persisted = await this.maybePersistLargeToolResult(callId, toolName, content);
                ...
                const resultPaths = isFilesystemPathTool(toolName) &&
                    Array.isArray(toolResult.resultFilePaths)
                    ? toolResult.resultFilePaths
                    : [];
                const candidatePaths = Array.from(new Set([...inputPaths.map((p) => unescapePath(p)), ...resultPaths]));
                if (candidatePaths.length > 0) {
                    ...
                    for (const candidatePath of candidatePaths) {
                        ...
                        const rulesCtx = await rulesRegistry?.matchAndConsume(candidatePath);
                        if (rulesCtx)
                            reminderBlocks.push(rulesCtx);
                    }
                    ...
                    const activatedSkills = await skillManager?.matchAndActivateByPaths(candidatePaths);
                    if (activatedSkills && activatedSkills.length > 0 && skillManager) {
                        ...
                        if (hasSkillTool) {
                            ...
                            try {
                                const collected = await collectAvailableSkillEntries(skillManager, this.config);
                                const activatedSet = new Set(activatedSkills);
                                activatedEntries = collected.entries.filter((e) => e.level !== undefined && activatedSet.has(e.name));
                            }
                            catch (error) {
                                debugLogger.warn('coreToolScheduler: collectAvailableSkillEntries failed in activation path', error);
                            ...
                            if (activatedEntries.length > 0) {
                                reminderBlocks.push(`The following skill(s) became available via the Skill tool based on the file you just accessed; invoke a skill by passing its name to the Skill tool:\n<available_skills>\n${renderAvailableSkillsBlock(activatedEntries)}\n</available_skills>`);
                                ...
                                this.config.addInlineAnnouncedSkillKeys(activatedEntries.map((e) => `skill:${e.name}`));
                            }
                        }
                    }
                ...
                const perToolLines = perToolMax !== undefined ? Number.POSITIVE_INFINITY : undefined;
                const promptIdForTruncation = scheduledCall.request.prompt_id;
                try {
                    const contentBeforeTruncation = content;
                    const truncated = await truncateLlmContent(this.config, toolName, content, { threshold: perToolMax, lines: perToolLines, keep: perToolKeep }, promptIdForTruncation);
                    content = truncated.content;
                    mergePersistedOutputFiles(truncated.outputFile
                    ...
                    debugLogger.warn(`TRUNCATION failed for ${toolName}: ${truncErr instanceof Error ? truncErr.message : String(truncErr)}`);
                }
                // 2) Append the deferred metadata now that the body is bounded.
                if (postToolUseAdditionalContext) {
                    content = appendAdditionalContext(content, postToolUseAdditionalContext);
                }
                if (reminderEnvelope) {
                ...
                if ((postToolUseAdditionalContext || reminderEnvelope) &&
                    typeof content === 'string' &&
                    !content.startsWith(TOOL_OUTPUT_TRUNCATED_PREFIX)) {
                    const baseThreshold = perToolMax ?? this.config.getTruncateToolOutputThreshold();
                    // Match the first pass's char-only semantics for per-tool budgets;
                    // only the global path keeps a (doubled) line cap.
                    const combinedLines = perToolMax !== undefined
                    ...
                    if (content.length > baseThreshold * COMBINED_PASS_TOLERANCE_FACTOR) {
                        try {
                            ...
                            const recombined = await truncateToolOutput(this.config, toolName, content, {
                                threshold: baseThreshold * COMBINED_PASS_TOLERANCE_FACTOR,
                                lines: combinedLines,
                                keep: perToolKeep,
                            }, promptIdForTruncation);
                            content = recombined.content;
                            mergePersistedOutputFiles(recombined.outputFile
                            ...
                            debugLogger.warn(`TRUNCATION (combined) failed for ${toolName}: ${truncErr instanceof Error
                                ? truncErr.message
                                : String(truncErr)}`);
                        }
                    }
                ...
                const convertedResponse = convertToFunctionResponse(toolName, callId, content);
                const processedImages = await this.processToolResultImages(convertedResponse, signal);
                const response = processedImages.responseParts;
                if (response !== convertedResponse) {
                    contentLength = response.reduce((total, part) => total + extractTextFromPartListUnion(part).length, 0);
                }
                const artifacts = [
                ...
                const successResponse = {
                    ...
                    resultDisplay: this.compactResultDisplayForInteractiveHistory(toolResult.returnDisplay),
                    error: undefined,
                    errorType: undefined,
                    executionStatus,
                    contentLength,
                ...
                // sanitized history.
                if (canonicalName === ToolNames.EXIT_PLAN_MODE &&
                    typeof toolResult.llmContent === 'string' &&
                    PLAN_EXIT_APPROVED_LLM_CONTENT_PREFIXES.some((prefix) => toolResult.llmContent.startsWith(prefix))) {
                    try {
                        ...
                        const redacted = this.config
                            .getGeminiClient?.()
                            ?.getChat()
                            .redactApprovedPlanFromHistory(callId, approvedPlanRedactionText(planPath), savedPlan);
                        ...
                        if (redacted === false) {
                            debugLogger.debug(`Approved-plan redaction left history unchanged for ` +
                                `${callId}: no matching exit_plan_mode call, or its ` +
                                `plan text does not match ${planPath}.`);
                        }
                    }
                    catch (redactErr) {
                        debugLogger.warn(`Skipping approved-plan redaction for ${callId} (plan file ` +
                            `unavailable?): ${redactErr instanceof Error
                                ? redactErr.message
                                : String(redactErr)}`);
                    }
                }
                ...
                const result = response.find((part) => part.functionResponse !== undefined)?.functionResponse?.response;
                if (result !== undefined) {
                ...
                let errorPersistedOutputFiles = toolResult.persistedOutputFiles
                    ? [...toolResult.persistedOutputFiles]
                    : toolResult.persistedOutputFiles;
                let failureHookAdditionalContext;
                let failureHookArtifacts;
                if (hooksEnabled && messageBus) {
                    const failureHookResult = await this.withHookSpan({
                        hookEvent: 'PostToolUseFailure',
                        ...
                        toolUseId,
                        isInterrupt: false,
                    }, () => safelyFirePostToolUseFailureHook(messageBus, toolUseId, canonicalName, toolInput, toolResult.error.message, false, this.config.getApprovalMode(), callId), this.postToolUseFailureEndMeta);
                    // Append additional context from hook if provided
                    if (failureHookResult.additionalContext) {
                        if (isTimeout) {
                ...
                if (isTimeout) {
                    const timeoutContent = await this.maybePersistLargeToolResult(callId, toolName, toolResult.llmContent);
                    let responseParts = convertToFunctionErrorResponse(toolName, callId, timeoutContent.content, operationalErrorMessage);
                    if (failureHookAdditionalContext && responseParts.length > 0) {
                        const lastIndex = responseParts.length - 1;
                        responseParts = [...responseParts];
                        responseParts[lastIndex] = appendContextToResponsePart(responseParts[lastIndex], failureHookAdditionalContext);
                    }
                    const processedImages = await this.processToolResultImages(responseParts, signal);
                    ...
                    const timeoutPersistedOutputFiles = errorPersistedOutputFiles === undefined &&
                    ...
                    setToolSpanFailure(span, TOOL_FAILURE_KIND_TIMEOUT, TOOL_SPAN_STATUS_TOOL_TIMEOUT);
                    return;
                }
                // Truncate oversized error messages (e.g., large stderr)
                const errorGateThreshold = this.config.getTruncateToolOutputThreshold() + GATE_HEADROOM;
                if (errorMessage.length > errorGateThreshold &&
                    !isAlreadyTruncated(errorMessage)) {
                    const persistResult = await persistAndTruncateToolResult(callId, toolName, errorMessage, this.config);
                    errorMessage = persistResult.content;
                    errorPersistedOutputFiles = Array.from(new Set([
                        ...(errorPersistedOutputFiles ?? []),
                        ...(persistResult.outputFile ? [persistResult.outputFile] : []),
                    ]));
                }
                const error = new Error(errorMessage);
                let errorResponse = createErrorResponse(scheduledCall.request, error, executionErrorType ?? ToolErrorType.UNKNOWN, executionStatus, failureHookArtifacts, typeof toolResult.returnDisplay === 'string'
                    ? undefined
                    : this.compactResultDisplayForInteractiveHistory(toolResult.returnDisplay));
                if (errorPersistedOutputFiles !== undefined) {
                    errorResponse.persistedOutputFiles = Array.from(new Set(errorPersistedOutputFiles));
                }
                const imageParts = normalizeParts(toolResult.llmContent).flatMap((part) => {
                    const nestedImages = (part.functionResponse?.parts ?? [])
                        .filter((nested) => isImagePart(nested))
                        .map((nested) => nested);
                ...
                if (imageParts.length > 0) {
                    ...
                    const processedImages = await this.processToolResultImages(imageErrorParts, signal);
                    const bridgedErrorParts = processedImages.responseParts;
                    if (bridgedErrorParts !== imageErrorParts ||
                ...
                if (cancelAfterPostProcessing([...(toolResult.artifacts ?? []), ...(failureHookArtifacts ?? [])], {
                ...
                observeProducerOutput(errorResponse);
                this.setStatusInternal(callId, 'error', errorResponse);
                setToolSpanFailure(span, TOOL_FAILURE_KIND_TOOL_ERROR, TOOL_SPAN_STATUS_TOOL_ERROR);
            }
        }
            ...
            // differ.
            const executionThrew = !executionSettled && executionStatus !== 'not_started';
            const explicitErrorType = executionError?.errorType;
            const executionTimedOut = executionThrew && explicitErrorType === ToolErrorType.EXECUTION_TIMEOUT;
            const aborted = signal.aborted && !executionTimedOut;
            if (executionThrew) {
                executionStatus = aborted ? 'cancelled' : 'error';
                executionSettled = true;
            }
            const exceptionErrorType = explicitErrorType ??
            ...
            if (execSpan) {
                ...
                endToolExecutionSpan(failedExecSpan, {
                    ...
                    cancelled: aborted,
                    executionStatus,
                    errorType: executionStatus === 'error' ? exceptionErrorType : undefined,
                });
            }
            if (aborted) {
                ...
                const cancelledResponse = createCancelledResponse(scheduledCall.request, cancelMessage, executionStatus, failureHookArtifacts);
                observeProducerOutput(cancelledResponse);
                this.setStatusInternal(callId, 'cancelled', cancelledResponse);
                setToolSpanCancelled(span);
                return;
                ...
                if (hooksEnabled && messageBus) {
                    const failureHookResult = await this.withHookSpan({
                        ...
                        toolUseId,
                        isInterrupt: false,
                    }, () => safelyFirePostToolUseFailureHook(messageBus, toolUseId, canonicalName, toolInput, errorMessage, false, this.config.getApprovalMode(), callId), this.postToolUseFailureEndMeta);
                    // Append additional context from hook if provided
                    if (failureHookResult.additionalContext) {
                ...
                if (signal.aborted && !executionTimedOut) {
                    ...
                    executionThrew
                        ? TOOL_CANCELLED_BEFORE_COMPLETION_MESSAGE
                        : TOOL_CANCELLED_AFTER_COMPLETION_MESSAGE, executionStatus, failureHookArtifacts);
                    observeProducerOutput(cancelledResponse);
                    this.setStatusInternal(callId, 'cancelled', cancelledResponse);
                    setToolSpanCancelled(span);
                    return;
                }
                const errorResponse = createErrorResponse(scheduledCall.request, executionError instanceof Error
                    ? new Error(exceptionErrorMessage)
                    : new Error(String(executionError)), exceptionErrorType, executionStatus, failureHookArtifacts);
                observeProducerOutput(errorResponse);
                this.setStatusInternal(callId, 'error', errorResponse);
                setToolSpanFailure(span, TOOL_FAILURE_KIND_TOOL_EXCEPTION, TOOL_SPAN_STATUS_TOOL_EXCEPTION);
            }
        }
        finally {
            removeParentAbortForward?.();
    ...
    async checkAndNotifyCompletion() {
        ...
        if (this.toolCalls.length > 0 && allCallsAreTerminal) {
            ...
            const batchSignal = completedCalls
                .map((call) => this.callIdToPostToolBatchSignal.get(call.request.callId))
            ...
            try {
                ...
                debugLogger.warn(`PostToolBatch hook setup failed: ${error instanceof Error ? error.message : String(error)}`);
            }
            try {
                ...
                if (messageBus) {
                    ...
                    const permissionMode = this.config.getApprovalMode();
                    const fireBatchHook = () => this.withHookSpan({ hookEvent: 'PostToolBatch', toolName: 'batch' }, () => firePostToolBatchHook(messageBus, batchToolCalls, permissionMode, batchSignal), (r) => r.hookError
                        ? {
                            success: false,
                            error: r.hookError,
                            shouldStop: false,
                    ...
                    if (batchHookResult.shouldStop) {
                        const stopMessage = batchHookResult.stopReason ||
                            'Execution stopped by PostToolBatch hook';
                        debugLogger.info(`PostToolBatch hook stopped batch (${completedCalls.length} calls): ${batchHookResult.stopReason || 'no reason given'}`);
                        ...
                        const stoppedSpan = stoppedCall
                            ...
                            : undefined;
                        if (stoppedSpan) {
                            setToolSpanFailure(stoppedSpan, TOOL_FAILURE_KIND_POST_HOOK_STOPPED, stopMessage);
                        }
                        else {
                            ...
                            debugLogger.debug(`PostToolBatch stop: no tool span for stopped call ${stoppedCall?.request.callId}; post_hook_stopped not recorded`);
                        }
                    }
                    completedCalls = withPostToolBatchAdditionalContext(completedCalls, batchHookResult.additionalContext);
                    completedCalls = withPostToolBatchArtifacts(completedCalls, batchHookResult.artifacts);
                }
    ...
    async maybePersistLargeToolResult(callId, toolName, content) {
        if (GATE_EXEMPT_TOOLS.has(canonicalToolName(toolName)))
            return { content };
        const text = extractTextFromPartListUnion(content);
        if (!text || isAlreadyTruncated(text))
            return { content };
        const gateThreshold = this.config.getTruncateToolOutputThreshold() + GATE_HEADROOM;
        if (text.length <= gateThreshold)
            return { content };
        const result = await persistAndTruncateToolResult(callId, toolName, text, this.config);
        if (result.outputFile) {
            debugLogger.debug(`Persisted ${toolName} result (${result.bytesWritten} bytes) to ${result.outputFile}`);
        }
        // Preserve non-text parts (media) when content is Part[]
        if (Array.isArray(content)) {
            const mediaParts = content.filter((p) => p.inlineData ||
                p.fileData);
            const stubPart = { text: result.content };
            return {
    ...
    async applyBatchOutputBudget(completedCalls) {
        ...
        const finalized = await finalizeToolResponses(this.config, completedCalls.map((call) => ({
        ...
        })), new Map(completedCalls.map((call) => [
            call.request.callId,
            call.request.prompt_id,
        ])), observeFinalizerBoundary, observeFinalizerBoundary);
    ...
    recordToolResults(completedCalls) {
        ...
        for (const call of completedCalls) {
            ...
            if (!goalContext) {
                this.chatRecordingService.recordToolResult(call.response.responseParts, result);
            }
            else if (call.request.name === ToolNames.GET_GOAL ||
                call.request.name === ToolNames.UPDATE_GOAL) {
                this.chatRecordingService.recordToolResult(call.response.responseParts, result, { goalContext: { ...goalContext }, provenance: 'goal_runtime' });
            }
            else {
                this.chatRecordingService.recordToolResult(call.response.responseParts, result, { goalContext: { ...goalContext } });
            }
        }
    }
    notifyToolCallsUpdate() {
        ...
        try {
            ...
            debugLogger.error(`Tool call update observer failed: ${error instanceof Error ? error.message : String(error)}`);
        }
    }
    ...
    async autoApproveCompatiblePendingTools(signal, triggeringCallId) {
        ...
        for (const pendingTool of pendingTools) {
            try {
                if (this.cancelPreExecutionIfAborted(pendingTool.request.callId, signal)) {
                    continue;
                }
                // Re-run L3→L4 to see if the tool can now be auto-approved
                const toolParams = pendingTool.invocation.params;
                const flowResult = await runInRequestGoalContext(pendingTool.request, () => evaluatePermissionFlow(this.config, pendingTool.invocation, pendingTool.request.name, toolParams));
                if (this.cancelPreExecutionIfAborted(pendingTool.request.callId, signal)) {
                    continue;
                }
                const { finalPermission, pmForcedAsk, pmCtx, requiresUserInteraction } = flowResult;
                if (requiresUserInteraction) {
                    continue;
                }
                const forceAutoReviewForAllow = this.config.getApprovalMode() === ApprovalMode.AUTO &&
                    (shouldForceAutoModeReviewForAllow(pmCtx, this.config.getCwd()) ||
                        shouldClassifyAllShellForAutoMode(pendingTool.request.name, this.config));
                if (finalPermission === 'allow' && forceAutoReviewForAllow) {
                    debugLogger.info(`Auto mode: pending L4 allow overridden by protected-write guard or classifyAllShell for ${pendingTool.request.name}`);
                    const denialState = this.config.getAutoModeDenialState();
                    const fallback = shouldFallback(denialState);
                    const messages = this.config
                        .getGeminiClient?.()
                        ?.getHistoryTail(MAX_TRANSCRIPT_MESSAGES, false) ?? [];
                    const decision = await runInRequestGoalContext(pendingTool.request, () => evaluateAutoMode({
                        ctx: pmCtx,
                        pmForcedAsk,
                        toolParams,
                        messages,
                    ...
                    if (!this.config.getDisableAllHooks() &&
                        shouldFirePermissionDeniedForAutoMode(decision, outcome)) {
                        try {
                            await runInRequestGoalContext(pendingTool.request, () => this.config
                                .getHookSystem?.()
                                ?.firePermissionDeniedEvent(pendingTool.request.name, toolParams, pendingTool.request.callId, getAutoModePermissionDeniedReason(decision), signal, pendingTool.request.callId));
                        }
                        catch (hookError) {
                            debugLogger.warn(`PermissionDenied hook failed for pending tool ${pendingTool.request.callId}: ${hookError instanceof Error ? hookError.message : String(hookError)}`);
                        }
                    }
                    if (this.cancelPreExecutionIfAborted(pendingTool.request.callId, signal)) {
                        continue;
                    ...
                    switch (outcome.kind) {
                        case 'approved':
                            ...
                            this.finalizeBlockedSpan(pendingTool.request.callId, 'auto_approved', 'auto');
                            break;
                        case 'blocked': {
                            this.setStatusInternal(pendingTool.request.callId, 'error', createErrorResponse(pendingTool.request, new Error(outcome.errorMessage), ToolErrorType.EXECUTION_DENIED, 'not_started'));
                            this.finalizeBlockedSpan(pendingTool.request.callId, 'error', 'auto');
                        ...
                        case 'fallback':
                            if (isDenialFallbackReason(outcome.reason) ||
                                ...
                                if (outcome.message) {
                                    this.setStatusInternal(pendingTool.request.callId, 'awaiting_approval', decorateClassifierUnavailableConfirmation(pendingTool.confirmationDetails, outcome.message));
                                }
                                debugLogger.warn(`Auto mode fallback for pending tool (${outcome.reason}): consecutiveBlock=${denialState.consecutiveBlock}, consecutiveUnavailable=${denialState.consecutiveUnavailable}`);
                            }
                            break;
                        default: {
                            const _exhaustive = outcome;
