...
import { mkdtempSync, mkdirSync, writeFileSync, readFileSync, rmSync, existsSync, symlinkSync, statSync, lstatSync, } from 'node:fs';
import { tmpdir } from 'node:os';
import { join } from 'node:path';
import { pathToFileURL } from 'node:url';
import { runOneMutant, runOneHunkProbe, splitDiffIntoHunks, testEfficacyCommand, } from './test-efficacy.js';
import { isolateHostGitConfig } from './lib/test-utils.js';
const runHandler = testEfficacyCommand.handler;
let repo;
let outside;
let gitIsolation;
...
function commitAll(msg) {
    git(repo, 'add', '-A');
    git(repo, '-c', 'user.email=a@b', '-c', 'user.name=a', 'commit', '-q', '-m', msg);
...
function treeState(wt) {
    return (git(wt, 'status', '--porcelain', '-z') + '|' + git(wt, 'rev-parse', 'HEAD'));
}
/**
...
function scaffoldModifiedPr() {
    ...
    writeFileSync(join(repo, 'report.json'), JSON.stringify({
        files: [
 ...
 */
function installFailingVitest() {
    writeFileSync(vitestScript(), `#!/usr/bin/env node
...
describe('fixture git-config isolation', () => {
    it('spawned git reads the throwaway global config, not the host user config', () => {
        ...
        writeFileSync(join(gitIsolation.home, '.gitconfig'), '[qwen]\n\tisolation = sentinel\n');
        expect(git(repo, 'config', '--global', 'qwen.isolation').trim()).toBe('sentinel');
        expect(process.env['GIT_CONFIG_GLOBAL']).toBe(join(gitIsolation.home, '.gitconfig'));
...
describe('test-efficacy probe isolation (#6832)', () => {
    it('probes in a disposable worktree and never mutates the shared one', async () => {
        ...
        expect(readFileSync(join(wt, 'packages/lib/src/f.ts'), 'utf8')).toBe('export const f = () => 2;\n');
        ...
        expect(out.findings.map((f) => f.file)).toContain('packages/lib/src/f.test.ts');
    ...
    it('a PR-controlled symlink cannot delete outside the tree — by isolation, not the guard', async () => {
        ...
        const out = JSON.parse(readFileSync(join(repo, 'out.json'), 'utf8'));
        const details = out.probed.map((p) => p.detail);
        expect(details.join('\n')).not.toMatch(/refusing to delete through a symlink/);
    ...
    it('probes hunks end-to-end on a diff with NO mutant candidates', async () => {
        ...
        expect(out.findings.filter((f) => f.kind === 'hunk-survived')).toHaveLength(2);
    ...
    it('scores a hunk inconclusive when its OWN collocated test dropped out of the baseline', async () => {
        ...
        // other file passes.
        writeFileSync(join(repo, 'node_modules', 'vitest', 'vitest.mjs'), `#!/usr/bin/env node
import path from 'node:path';
import fs from 'node:fs';
const files = process.argv.slice(2).filter((a) => a.includes('.test.'));
const st = (f) => {
...
`);
        ...
        expect(out.findings.some((f) => f.kind === 'hunk-survived')).toBe(false);
    });
    it('runs a REPLACEMENT mutant end-to-end and reports the survivor', async () => {
        ...
        const out = JSON.parse(readFileSync(join(repo, 'out.json'), 'utf8'));
        const coalesce = out.mutants.probed.find((m) => m.operator === 'coalesce');
        ...
        expect(out.findings.some((f) => f.message.includes('?? fallback'))).toBe(true);
        // The mutation happened only in the disposable tree.
        expect(treeState(wt)).toEqual(before);
    });
    it('runs a deletion mutant end-to-end and reports the survivor', async () => {
        ...
        write('package.json', '{"private":true,"workspaces":["packages/*"]}\n');
        write('packages/lib/src/f.ts', 'export const state = new Map<string, string>();\n' +
            'export function use(k: string) {\n' +
            '  return state.get(k);\n' +
            '}\n');
        const base = commitAll('base');
        const prSource = 'export const state = new Map<string, string>();\n' +
            'export function use(k: string) {\n' +
            '  return state.get(k);\n' +
            '}\n' +
        ...
        const survivor = out.findings.find((f) => f.kind === 'mutant-survived');
        ...
        expect(treeState(wt)).toBe(before);
        expect(readFileSync(join(wt, 'packages/lib/src/f.ts'), 'utf8')).toBe(prSource);
        expect(existsSync(join(repo, 'wt-probe'))).toBe(false);
    });
...
`);
        ...
        expect(out.findings.some((f) => f.kind === 'mutant-survived')).toBe(false);
        expect(treeState(wt)).toBe(before);
        expect(existsSync(join(repo, 'wt-probe'))).toBe(false);
    });
    it('skips the mutants wholesale when the unmutated baseline is not green', async () => {
        ...
        write('packages/lib/src/f.test.ts', 'import { reset } from "./f.js"; import { it, expect } from "vitest"; it("t", () => { reset(); expect(1).toBe(2); });\n');
        commitAll('pr');
        const wt = join(repo, 'wt');
        git(repo, 'worktree', 'add', '-q', '--detach', wt, 'HEAD');
        writeFileSync(join(repo, 'report.json'), JSON.stringify({
        ...
        expect(stdout).toContain('1 mutant(s) skipped: no probe file was green in the unmutated baseline');
    ...
    it('still probes when an UNRELATED probe file is all-skipped (per-file gate)', async () => {
        ...
        write('packages/lib/src/skipped.test.ts', 'import { it } from "vitest"; it.skip("quarantined", () => {});\n');
        commitAll('pr');
        const wt = join(repo, 'wt');
        git(repo, 'worktree', 'add', '-q', '--detach', wt, 'HEAD');
        writeFileSync(join(repo, 'report.json'), JSON.stringify({
    ...
    it('re-classes every survivor and spends nothing when the positive control fails', async () => {
        ...
        const filler = Array.from({ length: 8 }, (_, i) => `const a${i} = ${i};\n`).join('');
        write('package.json', '{"private":true,"workspaces":["packages/*"]}\n');
        write('packages/lib/src/f.ts', 'export const state = new Map<string, string>();\n' +
            filler +
...
`);
        ...
        expect(out.probed.map((p) => p.verdict)).toEqual([
            'inconclusive',
        ]);
...
`);
        ...
        const forF = out.mutants.probed.filter((m) => m.file.endsWith('f.ts'));
        ...
        expect(out.findings.filter((f) => f.kind === 'mutant-survived' && f.file.endsWith('f.ts'))).toEqual([]);
        // The guard is targeted: `g.ts`, whose own test IS green, still gets a
        // real verdict rather than being swept up with it.
        const forG = out.mutants.probed.filter((m) => m.file.endsWith('g.ts'));
        expect(forG.length).toBeGreaterThan(0);
        expect(forG.every((m) => m.verdict !== 'inconclusive')).toBe(true);
    });
    // A symlink is the mechanism, and Windows needs a privilege to create one.
    it.skipIf(process.platform === 'win32')('a control that could not be SET UP leaves the window spendable', async () => {
...
`);
        const suiteRuns = () => existsSync(runsLog)
        ...
        expect(stdoutChunks.join('')).toContain('2 mutant(s) skipped: the remaining budget cannot fit another suite run');
    });
    it('reports mutants skipped for cap when candidates exceed MAX_MUTANTS', async () => {
        ...
        expect(stdoutChunks.join('')).toContain('1 mutant(s) skipped: more candidates than the selection caps (8 total, 3 of them replacements)');
    });
...
`);
    ...
    it('still finds the survivor under hostile user git diff config', async () => {
        ...
        write('package.json', '{"private":true,"workspaces":["packages/*"]}\n');
        write('packages/lib/src/fø.ts', 'export const state = new Map<string, string>();\n');
        const base = commitAll('base');
        write('packages/lib/src/fø.ts', 'export const state = new Map<string, string>();\n' +
            'export function reset() {\n' +
            '  state.clear();\n' +
    ...
    it('never deletes a line that does not hold the selected statement', () => {
        ...
        const got = runOneMutant(repo, { file: 'src/x.ts', line: 1, statement: 'gone.clear();' }, ['src/x.test.ts']);
...
`);
        runOneMutant(repo, { file: 'src/x.ts', line: 1, statement: 'gone.clear();' }, ['src/x.test.ts']);
        // Both plants are real, and they outlive the run that made them.
        expect(readFileSync(join(repo, 'src/other.ts'), 'utf8')).toContain('planted');
        ...
        expect(git(repo, 'status', '--porcelain', '--untracked-files=all')).not.toContain('planted-cache');
        const second = runOneMutant(repo, { file: 'src/x.ts', line: 1, statement: 'gone.clear();' }, ['src/x.test.ts']);
        // ...and the next run opens on neither.
        expect(readFileSync(join(repo, 'src/other.ts'), 'utf8')).toBe('export const clean = true;\n');
    ...
    it('runs tests with dependencies from the source worktree', () => {
        ...
        const sourceDependencyDir = join(dependencyRoot, 'node_modules', 'probe-dependency');
        const sourceScopedDependencyDir = join(dependencyRoot, 'node_modules', '@probe', 'scoped-dependency');
        mkdirSync(sourceVitestDir, { recursive: true });
        mkdirSync(sourceDependencyDir);
        mkdirSync(sourceScopedDependencyDir, { recursive: true });
        writeFileSync(join(sourceVitestDir, 'package.json'), JSON.stringify({ bin: { vitest: './vitest.mjs' } }));
        const sourceBinDir = join(dependencyRoot, 'node_modules', '.bin');
        mkdirSync(sourceBinDir);
        writeFileSync(join(dependencyRoot, 'node_modules', '.package-lock.json'), '{}\n');
        writeFileSync(join(sourceVitestDir, 'vitest.mjs'), `import '${pathToFileURL(join(probeTree, 'src/x.test.mjs')).href}';
process.stdout.write(JSON.stringify({
  testResults: [{
...
`);
        writeFileSync(join(sourceDependencyDir, 'package.json'), JSON.stringify({ type: 'module', exports: './index.mjs' }));
        writeFileSync(join(sourceDependencyDir, 'index.mjs'), 'export default 1;\n');
        writeFileSync(join(sourceScopedDependencyDir, 'package.json'), JSON.stringify({ type: 'module', exports: './index.mjs' }));
        writeFileSync(join(sourceScopedDependencyDir, 'index.mjs'), 'export default 2;\n');
        const brokenLink = join(dependencyRoot, 'node_modules', 'broken-link');
        if (process.platform !== 'win32') {
            symlinkSync(join(dependencyRoot, 'node_modules', 'missing-package'), brokenLink);
        }
        writeFileSync(join(sourceBinDir, 'probe-tool'), 'available');
        writeFileSync(join(probeTree, 'src/x.ts'), 'gone.clear();\n');
        writeFileSync(join(probeTree, 'src/x.test.mjs'), "import fs from 'node:fs'; import value from 'probe-dependency'; import scopedValue from '@probe/scoped-dependency'; if (value !== 1 || scopedValue !== 2 || fs.readFileSync('node_modules/.bin/probe-tool', 'utf8') !== 'available') throw new Error('bad dependency');\n");
        ...
        const result = runOneMutant(probeTree, { file: 'src/x.ts', line: 1, statement: 'gone.clear();' }, ['src/x.test.mjs'], undefined, Date.now, dependencyRoot);
        ...
        expect(existsSync(join(dependencyRoot, 'node_modules', '.vite-probe'))).toBe(false);
        if (process.platform !== 'win32') {
    ...
    it('sweeps a stale REGISTERED probe worktree left by a crashed run', async () => {
        ...
        git(repo, 'worktree', 'add', '-q', '--detach', join(repo, 'wt-probe'), 'HEAD');
...
describe('per-hunk probes against real git', () => {
    ...
    const hunkPatches = () => {
        const diff = git(repo, 'diff', '--no-color', '--src-prefix=a/', '--dst-prefix=b/', base, 'HEAD', '--', FILE);
    ...
    it('produces two patches git accepts, one per change', () => {
        ...
        for (const h of hunks) {
            ...
            expect(() => execFileSync('git', ['apply', '--reverse', '--check', '-'], {
                cwd: repo,
                input: h.patch,
                encoding: 'utf8',
            })).not.toThrow();
    ...
    it('restores a hunk-ADDED file whose parent directory the reverse apply removed', () => {
        ...
        const diff = git(repo, 'diff', '--no-color', '--src-prefix=a/', '--dst-prefix=b/', 'HEAD~1', 'HEAD', '--', 'src/newdir/added.ts');
        const [h] = splitDiffIntoHunks(diff);
        const got = runOneHunkProbe(repo, {
        ...
        expect(readFileSync(join(repo, 'src/newdir/added.ts'), 'utf8')).toBe('export const fresh = 1;\n');
    });
...
//# sourceMappingURL=test-efficacy.integration.test.js.map