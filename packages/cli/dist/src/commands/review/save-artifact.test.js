...
import { buildReport } from '../../utils/findings.js';
...
describe('saveReviewArtifact', () => {
    it('copies canonical findings and the composed verdict into schema v1', () => {
        ...
        expect(readdirSync(join(root, '.qwen/reviews')).filter((name) => name.endsWith('.tmp'))).toEqual([]);
    });
    ...
    it.each(['findings', 'composed', 'report'])('refuses a %s input path that traverses a symlink', (input) => {
        const paths = fixture();
        const outside = mkdtempSync(join(tmpdir(), 'review-artifact-input-'));
        const outsideFile = join(outside, input === 'report' ? 'review.md' : `${input}.json`);
        writeFileSync(outsideFile, input === 'report' ? '# Outside\n' : readFileSync(paths[input], 'utf8'));
        rmSync(paths[input]);
    ...
    it.each(['two', -1, 1.5])('refuses a present deferredCount of the wrong shape (%s)', (bad) => {
    ...
    it.each([
        ...
        ['a negative index', [-1]],
        ['a fraction', [1.5]],
    ])('refuses a present floorEnforced of the wrong shape (%s)', (_label, bad) => {
    ...
    it.each([
    ...
    ])('refuses a present bodyTrim carrying %s', (_label, bad) => {
        ...
        expect(() => saveReviewArtifact({ ...paths, target: 'local', effort: 'medium' })).toThrow(/bodyTrim/);
        expect(existsSync(paths.out)).toBe(false);
    });
    it.each(['sections', 'deferralList', 'fold', 'truncated'])('refuses a present bodyTrim with no `%s` — that shape never shipped', (key) => {
        ...
        const paths = fixture();
        const bodyTrim = { ...verdict.bodyTrim };
        delete bodyTrim[key];
    ...
    it.each([
        ...
        ['a negative', -1],
        ['a fraction', 1.5],
    ])('refuses a present postedInline of the wrong shape (%s)', (_label, bad) => {
        ...
        expect(() => saveReviewArtifact({ ...paths, target: 'local', effort: 'medium' })).toThrow(/postedInline/);
        expect(existsSync(paths.out)).toBe(false);
    });
    ...
    it('persists a recorded zero — a converged round is an observation', () => {
        ...
        saveReviewArtifact({ ...paths, target: 'local', effort: 'medium' });
        expect(JSON.parse(readFileSync(paths.out, 'utf8')).verdict.postedInline).toBe(0);
    });
    ...
    it('PRESERVES an absent postedFresh and refuses a present one of the wrong shape', () => {
        ...
        const paths = fixture();
        saveReviewArtifact({ ...paths, target: 'local', effort: 'medium' });
        expect('postedFresh' in JSON.parse(readFileSync(paths.out, 'utf8')).verdict).toBe(false);
        rmSync(paths.out, { force: true });
        writeJson(paths.composed, { ...verdict, postedFresh: -1 });
        expect(() => saveReviewArtifact({ ...paths, target: 'local', effort: 'medium' })).toThrow(/postedFresh/);
    });
    ...
    it('reads an absent or null bodyTrim as untrimmed — a pre-budget composed file must still save', () => {
        ...
        for (const composed of [preBudget, { ...verdict, bodyTrim: null }]) {
            writeJson(paths.composed, composed);
            saveReviewArtifact({ ...paths, target: 'local', effort: 'medium' });
            expect(JSON.parse(readFileSync(paths.out, 'utf8')).verdict.bodyTrim).toEqual({
                sections: 0,
                deferralList: false,
                fold: false,
                truncated: false,
    ...
    it('reads an absent or null deferredCount as zero — a pre-posture composed file must still save', () => {
        ...
        for (const composed of [prePosture, { ...verdict, deferredCount: null }]) {
            ...
            expect(JSON.parse(readFileSync(paths.out, 'utf8')).verdict.deferredCount).toBe(0);
        }
    });
    it.each(['findings', 'composed', 'report'])('refuses to overwrite the %s input', (input) => {
        const paths = fixture();
        const inputPath = join(root, '.qwen/reviews', input === 'report' ? 'review.md' : `${input}.json`);
        if (input !== 'report') {
    ...
    it.skipIf(!caseInsensitiveFs)('refuses a case-insensitive output alias of the Markdown report', () => {
        const paths = fixture();
...
describe('the CLI option contract', () => {
    ...
    it('parses --workspace-root into the field saveReviewArtifact actually reads', () => {
        fixture();
        const parsed = saveArtifactCommand.builder(yargs([])).parseSync([
            '--findings',
...
//# sourceMappingURL=save-artifact.test.js.map