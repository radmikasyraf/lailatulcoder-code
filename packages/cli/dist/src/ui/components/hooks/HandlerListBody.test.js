...
function commandConfig(command = '/cmd.sh', async = false) {
    return {
...
function functionConfig(overrides = {}) {
    return {
        config: {
...
function promptConfig(overrides = {}) {
    return {
        config: {
...
describe('HandlerListBody', () => {
    ...
    describe('describeHook (rendered as the row label)', () => {
        it('renders the command path for command hooks', () => {
            const { lastFrame } = render(_jsx(HandlerListBody, { configs: [commandConfig('/check.sh')], selectedIndex: 0 }));
            const out = lastFrame() ?? '';
            expect(out).toContain('[command]');
            expect(out).toContain('/check.sh');
        });
        it('marks async command hooks with " async" in the type column', () => {
            const { lastFrame } = render(_jsx(HandlerListBody, { configs: [commandConfig('/bg.sh', true)], selectedIndex: 0 }));
            expect(lastFrame() ?? '').toContain('[command async]');
        });
        it('prefers the http hook name over the URL', () => {
            const { lastFrame } = render(_jsx(HandlerListBody, { configs: [httpConfig({ name: 'webhook-A', url: 'https://x' })], selectedIndex: 0 }));
            const out = lastFrame() ?? '';
        ...
        it('falls back to the http URL when name is missing', () => {
            const { lastFrame } = render(_jsx(HandlerListBody, { configs: [httpConfig({ url: 'https://example.test/hook' })], selectedIndex: 0 }));
            expect(lastFrame() ?? '').toContain('https://example.test/hook');
        });
        it('prefers the function hook name over the id', () => {
            const { lastFrame } = render(_jsx(HandlerListBody, { configs: [functionConfig({ name: 'fn-name', id: 'fn-id' })], selectedIndex: 0 }));
            const out = lastFrame() ?? '';
        ...
        it('falls back to function id, then to "function-hook" placeholder', () => {
            const withId = render(_jsx(HandlerListBody, { configs: [functionConfig({ id: 'only-id' })], selectedIndex: 0 }));
            expect(withId.lastFrame() ?? '').toContain('only-id');
            const noNameNoId = render(_jsx(HandlerListBody, { configs: [
                    {
                        config: {
                            type: HookType.Function,
                            callback: async () => undefined,
        ...
        it('truncates long prompt text with "..." and keeps short prompts intact', () => {
            ...
            const { lastFrame: longFrame } = render(_jsx(HandlerListBody, { configs: [promptConfig({ prompt: long })], selectedIndex: 0 }));
            const longOut = longFrame() ?? '';
            expect(longOut).toContain('a'.repeat(50) + '...');
            expect(longOut).not.toContain('a'.repeat(51));
            const { lastFrame: shortFrame } = render(_jsx(HandlerListBody, { configs: [promptConfig({ prompt: 'fits' })], selectedIndex: 0 }));
        ...
        it('prefers the prompt name over the prompt text', () => {
            const { lastFrame } = render(_jsx(HandlerListBody, { configs: [
    ...
    describe('source column', () => {
        it('appends the extension name for Extensions-source configs', () => {
            ...
            const { lastFrame } = render(_jsx(HandlerListBody, { configs: [config], selectedIndex: 0 }));
            const out = lastFrame() ?? '';
    ...
    it('renders numbered rows and footer hint, places arrow on selected', () => {
        ...
        const { lastFrame } = render(_jsx(HandlerListBody, { configs: configs, selectedIndex: 1 }));
        const out = lastFrame() ?? '';
...
//# sourceMappingURL=HandlerListBody.test.js.map