...
const coreMocks = vi.hoisted(() => ({
    emitDaemonLog: vi.fn(),
    extractDaemonHttpTraceContext: vi.fn(() => undefined),
    extractInboundTraceId: vi.fn(() => undefined),
    hashDaemonWorkspace: vi.fn((workspace) => `hash:${workspace}`),
    ...
    withDaemonRequestSpan: vi.fn((_attrs, fn) => fn({ setAttribute: coreMocks.spanSetAttribute })),
}));
...
import { daemonInboundTraceIdCaptureMiddleware, daemonTelemetryMiddleware, legacySessionTelemetryRoutes, resolveDaemonTelemetryRoute, setDaemonTelemetryWorkspace, } from './telemetry.js';
...
import { getDeferredRuntimeRequestTiming, MAX_CLIENT_ID_LENGTH, setDeferredRuntimeRequestTiming, } from './request-helpers.js';
...
describe('daemonTelemetryMiddleware — recordRequest seam', () => {
    beforeEach(() => {
        vi.clearAllMocks();
        coreMocks.hashDaemonWorkspace.mockImplementation((workspace) => `hash:${workspace}`);
        coreMocks.spanSetAttribute.mockImplementation(() => undefined);
    });
    it('has no deferred timing for ordinary requests', () => {
        expect(getDeferredRuntimeRequestTiming(mockReq('GET', '/health'))).toBe(undefined);
    });
    ...
    it('records the real status code (not just 200) on error responses', () => {
        ...
        mw(mockReq('POST', '/session/abc/prompt'), res, vi.fn());
        res.emit('finish');
    ...
    it('includes deferred runtime wait in the request span', () => {
        ...
        const res = mockRes(200);
        daemonTelemetryMiddleware(() => '/ws')(req, res, vi.fn());
        res.emit('finish');
        expect(coreMocks.withDaemonRequestSpan).toHaveBeenCalledWith(expect.objectContaining({
            startTime: startedAt,
            deferredRuntimeWaitMs: 24.5,
            deferredRuntimePath: 'joined',
        }), expect.any(Function));
        expect(coreMocks.recordDaemonHttpRequest).toHaveBeenCalledWith(expect.any(Number), 'POST /session', 200, 'joined');
    });
    it('links the request span to an inbound traceparent header when extraction succeeds', () => {
        ...
        const res = mockRes(200);
        daemonTelemetryMiddleware(() => '/ws')(mockReq('GET', '/daemon/status', {
        ...
        expect(coreMocks.withDaemonRequestSpan).toHaveBeenCalledWith(expect.objectContaining({ parentContext }), expect.any(Function));
    });
    it('omits the parent context when no valid traceparent header is present', () => {
        const res = mockRes(200);
        daemonTelemetryMiddleware(() => '/ws')(mockReq('GET', '/daemon/status', {}), res, vi.fn());
        res.emit('finish');
        expect(coreMocks.extractDaemonHttpTraceContext).toHaveBeenCalledWith({});
    ...
    it('logs at debug severity when a present traceparent header is rejected', () => {
        const res = mockRes(200);
        daemonTelemetryMiddleware(() => '/ws')(mockReq('GET', '/daemon/status', { traceparent: 'junk-header' }), res, vi.fn());
        res.emit('finish');
        expect(coreMocks.extractDaemonHttpTraceContext).toHaveBeenCalledWith({
            traceparent: 'junk-header',
        });
        expect(coreMocks.emitDaemonLog).toHaveBeenCalledWith('Rejected invalid inbound traceparent header.', {
    ...
    it('truncates the rejected traceparent header value in the breadcrumb', () => {
        ...
        const res = mockRes(200);
        daemonTelemetryMiddleware(() => '/ws')(mockReq('GET', '/daemon/status', { traceparent: longHeader }), res, vi.fn());
        res.emit('finish');
        expect(coreMocks.emitDaemonLog).toHaveBeenCalledWith('Rejected invalid inbound traceparent header.', expect.objectContaining({
    ...
    it('neutralizes control characters in the rejected traceparent breadcrumb', () => {
        ...
        const res = mockRes(200);
        daemonTelemetryMiddleware(() => '/ws')(mockReq('GET', '/daemon/status', { traceparent: forgedHeader }), res, vi.fn());
        res.emit('finish');
    ...
    it('rate-limits the rejected traceparent breadcrumb', () => {
        ...
        for (let i = 0; i < 200; i += 1) {
            const res = mockRes(200);
            mw(mockReq('GET', '/daemon/status', { traceparent: 'junk-header' }), res, vi.fn());
            res.emit('finish');
        }
    ...
    it('fails closed and settles the request when header extraction throws', () => {
        ...
        const res = mockRes(200);
        const next = vi.fn();
        expect(() => daemonTelemetryMiddleware(() => '/ws')(mockReq('GET', '/daemon/status', {
            traceparent: `00-${'3'.repeat(32)}-${'4'.repeat(16)}-01`,
        }), res, next)).not.toThrow();
        res.emit('finish');
    ...
    it('does not log when extraction succeeds, no header, or an array header is sent', () => {
        ...
        daemonTelemetryMiddleware(() => '/ws')(mockReq('GET', '/daemon/status', {}), resB, vi.fn());
        resB.emit('finish');
    ...
    it('fires exactly once even if both finish and close emit', () => {
        ...
        const mw = daemonTelemetryMiddleware(() => '/ws', recordRequest);
        const res = mockRes(200);
        mw(mockReq('GET', '/session/abc/artifacts'), res, vi.fn());
        res.emit('finish');
        res.emit('close');
        expect(recordRequest).toHaveBeenCalledTimes(1);
        expect(coreMocks.recordDaemonHttpRequest).toHaveBeenCalledTimes(1);
    ...
    it('maps plural workspace session listing to the existing route label', () => {
        ...
        const mw = daemonTelemetryMiddleware(() => '/ws', recordRequest);
        const res = mockRes(200);
        mw(mockReq('GET', '/workspaces/ws-secondary/sessions'), res, vi.fn());
        res.emit('finish');
        expect(recordRequest).toHaveBeenCalledTimes(1);
        expect(coreMocks.withDaemonRequestSpan).toHaveBeenCalledWith(expect.objectContaining({
            method: 'GET',
    ...
    it('maps the workspace live-state route to its own low-cardinality label', () => {
        expect(resolveDaemonTelemetryRoute(mockReq('GET', '/workspaces/ws-secondary/sessions/live-state'))).toEqual({ route: 'GET /workspaces/:workspace/sessions/live-state' });
    });
    it('attributes workspace transcript reads to the target workspace and session', () => {
        ...
        const res = mockRes(200);
        mw(mockReq('GET', '/workspaces/ws-secondary/session/session%2F1/transcript'), res, vi.fn());
        res.emit('finish');
        expect(coreMocks.withDaemonRequestSpan).toHaveBeenCalledWith(expect.objectContaining({
            method: 'GET',
    ...
    it('attributes workspace session-info reads to the shared session-info route', () => {
        const mw = daemonTelemetryMiddleware(() => '/ws');
        const res = mockRes(200);
        mw(mockReq('GET', '/workspace/%2Fwork%2Fa/session-info'), res, vi.fn());
        res.emit('finish');
        expect(coreMocks.withDaemonRequestSpan).toHaveBeenCalledWith(expect.objectContaining({
            method: 'GET',
    ...
    it('attributes plural workspace session-info reads to the shared session-info route', () => {
        const mw = daemonTelemetryMiddleware(() => '/workspace/secondary');
        const res = mockRes(200);
        mw(mockReq('GET', '/workspaces/ws-secondary/session-info'), res, vi.fn());
        res.emit('finish');
        expect(coreMocks.withDaemonRequestSpan).toHaveBeenCalledWith(expect.objectContaining({
            method: 'GET',
            route: 'GET /workspace/:id/session-info',
    ...
    it('attributes workspace exports to the target workspace and session', () => {
        const mw = daemonTelemetryMiddleware(() => '/workspace/secondary');
        const res = mockRes(200);
        mw(mockReq('GET', '/workspaces/ws-secondary/session/session%2F1/export'), res, vi.fn());
        res.emit('finish');
        expect(coreMocks.withDaemonRequestSpan).toHaveBeenCalledWith(expect.objectContaining({
            method: 'GET',
    ...
    it('attributes archived workspace exports to the target workspace and session', () => {
        const mw = daemonTelemetryMiddleware(() => '/workspace/secondary');
        const res = mockRes(200);
        mw(mockReq('GET', '/workspaces/ws-secondary/session/session%2F1/archive/export'), res, vi.fn());
        res.emit('finish');
        expect(coreMocks.withDaemonRequestSpan).toHaveBeenCalledWith(expect.objectContaining({
            method: 'GET',
    ...
    it('defers singular owner-routed workspace attribution until the handler selects a runtime', () => {
        ...
        for (const [method, path, route] of [
            ...
            const res = mockRes(200);
            mw(mockReq(method, path), res, vi.fn());
            expect(coreMocks.withDaemonRequestSpan).toHaveBeenLastCalledWith(expect.not.objectContaining({ workspaceHash: expect.anything() }), expect.any(Function));
            setDaemonTelemetryWorkspace(res, '/workspace/secondary');
            res.emit('finish');
            expect(coreMocks.withDaemonRequestSpan).toHaveBeenLastCalledWith(expect.objectContaining({
                method,
                route,
                sessionId: 'secondary-session',
            }), expect.any(Function));
            expect(coreMocks.spanSetAttribute).toHaveBeenLastCalledWith('lailatul-coder.workspace.hash', 'hash:/workspace/secondary');
        }
    });
    it('decodes session ids before span attribution', () => {
        const mw = daemonTelemetryMiddleware(() => '/workspace/primary');
        const res = mockRes(200);
        mw(mockReq('POST', '/session/secondary%2Fsession/rewind'), res, vi.fn());
        res.emit('finish');
        expect(coreMocks.withDaemonRequestSpan).toHaveBeenCalledWith(expect.objectContaining({ sessionId: 'secondary/session' }), expect.any(Function));
    });
    it('keeps malformed session id encodings without throwing', () => {
        const mw = daemonTelemetryMiddleware(() => '/workspace/primary');
        const res = mockRes(200);
        expect(() => {
            mw(mockReq('POST', '/session/bad%ZZ/rewind'), res, vi.fn());
        }).not.toThrow();
        res.emit('finish');
        expect(coreMocks.withDaemonRequestSpan).toHaveBeenCalledWith(expect.objectContaining({ sessionId: 'bad%ZZ' }), expect.any(Function));
    });
    it('normalizes plural workspace agent routes to stable route labels', () => {
        ...
        for (const [method, path, route] of [
            ...
            mw(mockReq(method, path), res, vi.fn());
            res.emit('finish');
            expect(coreMocks.withDaemonRequestSpan).toHaveBeenLastCalledWith(expect.objectContaining({ method, route }), expect.any(Function));
        }
    });
    it('normalizes the plural workspace upload route to a stable route label', () => {
        const mw = daemonTelemetryMiddleware(() => '/ws');
        const res = mockRes(200);
        mw(mockReq('POST', '/workspaces/ws-secondary/file/upload'), res, vi.fn());
        res.emit('finish');
        expect(coreMocks.withDaemonRequestSpan).toHaveBeenLastCalledWith(expect.objectContaining({
            method: 'POST',
    ...
    it('excludes the dashboard status poll (GET /daemon/status) from recordRequest', () => {
        ...
        mw(mockReq('GET', '/daemon/status'), res, vi.fn());
        res.emit('finish');
        expect(recordRequest).not.toHaveBeenCalled();
        expect(coreMocks.recordDaemonHttpRequest).toHaveBeenCalledTimes(1);
    });
    it('keeps heartbeat in OTel HTTP metrics but excludes it from the metrics ring', () => {
        ...
        const mw = daemonTelemetryMiddleware(() => '/ws', recordRequest);
        const res = mockRes(200);
        mw(mockReq('POST', '/session/abc/heartbeat'), res, vi.fn());
        setDaemonTelemetryWorkspace(res, '/ws');
        res.emit('finish');
        expect(coreMocks.recordDaemonHttpRequest).toHaveBeenCalledWith(expect.any(Number), 'POST /session/:id/heartbeat', 200, undefined);
        expect(recordRequest).not.toHaveBeenCalled();
    });
    it('does not record successful SSE connection lifetime as HTTP request latency', () => {
        ...
        const mw = daemonTelemetryMiddleware(() => '/ws', recordRequest);
        const res = mockRes(200);
        mw(mockReq('GET', '/session/abc/events'), res, vi.fn());
        setDaemonTelemetryWorkspace(res, '/ws');
        res.emit('close');
        expect(coreMocks.recordDaemonHttpResponse).toHaveBeenCalledTimes(1);
    ...
    it('records request-scoped generation SSE duration as ordinary HTTP latency', () => {
        ...
        const mw = daemonTelemetryMiddleware(() => '/ws', recordRequest);
        const res = mockRes(200);
        mw(mockReq('POST', '/session/abc/generate'), res, vi.fn());
        setDaemonTelemetryWorkspace(res, '/ws');
        res.emit('finish');
        expect(coreMocks.recordDaemonHttpRequest).toHaveBeenCalledWith(expect.any(Number), 'POST /session/:id/generate', 200, undefined);
        expect(recordRequest).toHaveBeenCalledWith(expect.any(Number), 200);
    });
    it('counts a 200 SSE request that closes before response headers are sent', () => {
        ...
        mw(mockReq('GET', '/session/abc/events'), res, vi.fn());
        res.emit('close');
        expect(coreMocks.recordDaemonHttpRequest).toHaveBeenCalledWith(expect.any(Number), 'GET /session/:id/events', 200, undefined);
        expect(recordRequest).toHaveBeenCalledWith(expect.any(Number), 200);
    });
    it.each([400, 404, 429, 500])('records an SSE handshake failure with status %s as an ordinary request', (statusCode) => {
        const recordRequest = vi.fn();
        const mw = daemonTelemetryMiddleware(() => '/ws', recordRequest);
        const res = mockRes(statusCode);
        mw(mockReq('GET', '/session/abc/events'), res, vi.fn());
        res.emit('finish');
        expect(coreMocks.recordDaemonHttpRequest).toHaveBeenCalledWith(expect.any(Number), 'GET /session/:id/events', statusCode, undefined);
        expect(recordRequest).toHaveBeenCalledWith(expect.any(Number), statusCode);
    });
    ...
    it('settles normally when telemetry is disabled and no span is created', () => {
        const recordRequest = vi.fn();
        coreMocks.withDaemonRequestSpan.mockImplementationOnce((_attrs, fn) => fn(undefined));
        const mw = daemonTelemetryMiddleware(() => '/workspace/primary', recordRequest);
        const res = mockRes(200);
        mw(mockReq('POST', '/session/abc/prompt'), res, vi.fn());
        setDaemonTelemetryWorkspace(res, '/workspace/secondary');
        expect(() => res.emit('finish')).not.toThrow();
        expect(coreMocks.hashDaemonWorkspace).not.toHaveBeenCalled();
        expect(coreMocks.recordDaemonHttpResponse).toHaveBeenCalledWith(undefined, 200);
        expect(coreMocks.recordDaemonHttpRequest).toHaveBeenCalledTimes(1);
        expect(recordRequest).toHaveBeenCalledTimes(1);
    });
    it('resolves workspace hash per request instead of closing over the primary workspace', () => {
        ...
        mw(mockReq('POST', '/session'), firstRes, vi.fn());
        setDaemonTelemetryWorkspace(firstRes, '/workspace/one');
        firstRes.emit('finish');
        const secondRes = mockRes(200);
        mw(mockReq('POST', '/session/abc/prompt'), secondRes, vi.fn());
        setDaemonTelemetryWorkspace(secondRes, '/workspace/two');
        secondRes.emit('finish');
        expect(coreMocks.hashDaemonWorkspace).toHaveBeenNthCalledWith(1, '/workspace/one');
        expect(coreMocks.hashDaemonWorkspace).toHaveBeenNthCalledWith(2, '/workspace/two');
        expect(coreMocks.spanSetAttribute).toHaveBeenNthCalledWith(1, 'lailatul-coder.workspace.hash', 'hash:/workspace/one');
        expect(coreMocks.spanSetAttribute).toHaveBeenNthCalledWith(2, 'lailatul-coder.workspace.hash', 'hash:/workspace/two');
    });
    it('memoizes workspace hashes by resolved workspace cwd', () => {
        ...
        expect(coreMocks.hashDaemonWorkspace).toHaveBeenCalledWith('/workspace/one');
    });
    it('settles a published workspace after its runtime is removed', () => {
        ...
        expect(coreMocks.spanSetAttribute).toHaveBeenCalledWith('lailatul-coder.workspace.hash', 'hash:/workspace/secondary');
    });
    it('uses first-selection-wins and clears deferred context after settlement', () => {
        ...
        expect(coreMocks.spanSetAttribute).toHaveBeenCalledWith('lailatul-coder.workspace.hash', 'hash:/workspace/first');
    });
    it('omits workspace hash when a dynamic target is never resolved', () => {
        const resolveWorkspaceCwd = vi.fn(() => '/workspace/primary');
        const mw = daemonTelemetryMiddleware(resolveWorkspaceCwd);
        const res = mockRes(404);
        mw(mockReq('POST', '/session/missing/prompt'), res, vi.fn());
        res.emit('finish');
        expect(resolveWorkspaceCwd).not.toHaveBeenCalled();
        expect(coreMocks.hashDaemonWorkspace).not.toHaveBeenCalled();
    ...
    it('keeps pre-resolved resolver failures from affecting request settlement', () => {
        ...
        res.emit('finish');
        expect(next).toHaveBeenCalledTimes(1);
        expect(coreMocks.withDaemonRequestSpan).toHaveBeenCalledWith(expect.not.objectContaining({ workspaceHash: expect.anything() }), expect.any(Function));
        expect(coreMocks.recordDaemonHttpRequest).toHaveBeenCalledTimes(1);
    });
    it('keeps late hash and span attribute failures from affecting metrics', () => {
        ...
        mw(mockReq('POST', '/session/abc/prompt'), hashFailureRes, vi.fn());
        ...
        mw(mockReq('POST', '/session/def/prompt'), attributeFailureRes, vi.fn());
    ...
    it('is a safe no-op when workspace selection is published without middleware context', () => {
        const res = mockRes(200);
        expect(() => setDaemonTelemetryWorkspace(res, '/workspace/secondary')).not.toThrow();
        expect(coreMocks.spanSetAttribute).not.toHaveBeenCalled();
    });
    it('continues a dynamic request when its Response cannot store telemetry context', () => {
        ...
        expect(() => mw(mockReq('POST', '/session/abc/prompt'), res, next)).not.toThrow();
        expect(() => setDaemonTelemetryWorkspace(res, '/workspace/secondary')).not.toThrow();
        res.emit('finish');
        expect(next).toHaveBeenCalledTimes(1);
        expect(coreMocks.spanSetAttribute).not.toHaveBeenCalled();
...
describe('legacy session telemetry route catalog', () => {
    it('contains 61 unique routes with the audited 59/2 attribution split', () => {
        const keys = legacySessionTelemetryRoutes.map(({ method, path }) => `${method} ${path}`);
        expect(keys).toHaveLength(61);
        expect(new Set(keys).size).toBe(61);
        expect(legacySessionTelemetryRoutes.filter(({ attribution }) => attribution === 'handler_resolved')).toHaveLength(59);
        expect(legacySessionTelemetryRoutes.filter(({ attribution }) => attribution === 'pre_resolved')).toHaveLength(2);
        expect(legacySessionTelemetryRoutes
            .filter(({ attribution }) => attribution === 'pre_resolved')
            .map(({ method, path }) => `${method} ${path}`)
            .sort()).toEqual(['POST /permission/:requestId', 'POST /session/:id/a2ui-action'].sort());
    ...
    it('matches every catalog entry with its declared canonical attribution', () => {
        for (const entry of legacySessionTelemetryRoutes) {
            const path = entry.path.replace(/:([A-Za-z][A-Za-z0-9_]*)/g, (_match, name) => {
                if (name === 'id')
                    return 'session-1';
                if (name === 'requestId')
                    return 'request-1';
    ...
    ])('matches %s %s with a canonical label', (method, path, route, sessionId) => {
    ...
    it('decodes and validates permission request ids after segment matching', () => {
        expect(resolveDaemonTelemetryRoute(mockReq('POST', '/session/abc/permission/%72eq-1'))).toMatchObject({
            route: 'POST /session/:id/permission/:requestId',
            sessionId: 'abc',
            permissionRequestId: 'req-1',
        });
        expect(resolveDaemonTelemetryRoute(mockReq('POST', '/session/abc/permission/req%2F1'))).not.toHaveProperty('permissionRequestId');
        expect(resolveDaemonTelemetryRoute(mockReq('POST', '/permission/bad%ZZ'))).not.toHaveProperty('permissionRequestId');
        expect(resolveDaemonTelemetryRoute(mockReq('POST', `/permission/${'a'.repeat(MAX_CLIENT_ID_LENGTH + 1)}`))).not.toHaveProperty('permissionRequestId');
    });
    it.each([
...
describe('daemonInboundTraceIdCaptureMiddleware', () => {
    ...
    it('captures the caller trace id from a valid traceparent header', () => {
        ...
        const res = mockRes(200);
        const next = vi.fn();
        daemonInboundTraceIdCaptureMiddleware(mockReq('GET', '/session/abc/prompt', {
            traceparent: `00-${'3'.repeat(32)}-${'4'.repeat(16)}-01`,
        }), res, next);
    ...
    it('leaves the response untouched when no valid header parses', () => {
        const res = mockRes(200);
        const next = vi.fn();
        daemonInboundTraceIdCaptureMiddleware(mockReq('GET', '/session/abc/prompt', { traceparent: 'junk-header' }), res, next);
    ...
    it('still calls next when extraction throws', () => {
        ...
        const res = mockRes(200);
        const next = vi.fn();
        expect(() => daemonInboundTraceIdCaptureMiddleware(mockReq('GET', '/anything', {}), res, next)).not.toThrow();
        expect(getDaemonTelemetryInboundTraceId(res)).toBeUndefined();
        expect(next).toHaveBeenCalledTimes(1);
    });
    it('capturing the trace id does not open the workspace attribution gate', () => {
        ...
        coreMocks.extractInboundTraceId.mockReturnValueOnce('3'.repeat(32));
        const res = mockRes(200);
        daemonInboundTraceIdCaptureMiddleware(mockReq('GET', '/daemon/status', {
            traceparent: `00-${'3'.repeat(32)}-${'4'.repeat(16)}-01`,
        }), res, vi.fn());
        daemonTelemetryMiddleware(() => '/ws')(mockReq('GET', '/daemon/status', {}), res, vi.fn());
        setDaemonTelemetryWorkspace(res, '/ws');
        res.emit('finish');
        expect(coreMocks.spanSetAttribute).not.toHaveBeenCalledWith('lailatul-coder.workspace.hash', expect.anything());
    });
    it('keeps the captured trace id when the telemetry middleware initializes the workspace context', () => {
        coreMocks.extractInboundTraceId.mockReturnValueOnce('3'.repeat(32));
        const res = mockRes(200);
        daemonInboundTraceIdCaptureMiddleware(mockReq('POST', '/session', {
            traceparent: `00-${'3'.repeat(32)}-${'4'.repeat(16)}-01`,
        }), res, vi.fn());
        daemonTelemetryMiddleware(() => '/ws')(mockReq('POST', '/session', {}), res, vi.fn());
