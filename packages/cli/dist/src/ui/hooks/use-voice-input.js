    ...
    const startRecording = useCallback((silenceDetection) => {
        ...
        const startPromise = waitForPreviousStop(stopPromiseRef.current)
            ...
            if (recorder.supportsStreaming?.() === false) {
                throw new Error('Streaming voice transcription requires native audio capture. Install/rebuild @lailatul-coder/audio-capture or switch voiceModel to qwen3-asr-flash for batch transcription.');
            }
