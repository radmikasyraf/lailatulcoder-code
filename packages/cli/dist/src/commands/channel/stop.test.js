...
const mockDaemonClient = vi.hoisted(() => vi.fn(() => ({ stopChannelWorker: mockStopChannelWorker })));
...
async function invokeStop(argv = {}) {
...
describe('stopCommand', () => {
    ...
    it('stops daemon-managed channels remotely without touching the pidfile', async () => {
        ...
        expect(mockWriteStdoutLine).toHaveBeenCalledWith('Daemon-managed channels stopped.');
    });
    it('reports when daemon-managed channels are already stopped', async () => {
        ...
        expect(mockWriteStdoutLine).toHaveBeenCalledWith('Daemon-managed channels are already stopped.');
        expect(mockReadServiceInfo).not.toHaveBeenCalled();
    });
    it('reports remote stop failures without falling through to standalone mode', async () => {
        ...
        vi.spyOn(process, 'exit').mockImplementation((() => undefined));
        await invokeStop({ 'daemon-url': 'http://daemon:9' });
        expect(mockWriteStderrLine).toHaveBeenCalledWith(expect.stringContaining('stop failed'));
        expect(mockReadServiceInfo).not.toHaveBeenCalled();
