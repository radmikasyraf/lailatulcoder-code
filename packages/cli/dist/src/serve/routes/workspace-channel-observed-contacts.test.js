...
function runtime(workspaceId, workspaceCwd, trusted = true) {
    return {
        workspaceId,
        workspaceCwd,
...
describe('workspace observed channel contact routes', () => {
    ...
    beforeEach(async () => {
        ...
        qwenHome = await fsp.mkdtemp(path.join(os.tmpdir(), 'qwen-observed-contact-routes-'));
    ...
    it('returns complete direct users and observed group/topic membership', async () => {
        ...
        new ObservedChannelContactStore(daemonObservedContactsPath(primary.workspaceCwd)).observe('dingtalk-main', {
        ...
        new ObservedChannelContactStore(daemonObservedContactsPath(secondary.workspaceCwd)).observe('dingtalk-main', {
        ...
        const response = await request(app).get('/workspace/channel/observed-contacts');
    ...
    it('selects an exact trusted workspace and returns an empty graph for no file', async () => {
        const primary = runtime('primary', '/work/main');
        const secondary = runtime('secondary', '/work/secondary');
        new ObservedChannelContactStore(daemonObservedContactsPath(secondary.workspaceCwd)).observe('telegram-team', {
        ...
        const selected = await request(app).get('/workspaces/secondary/channel/observed-contacts');
        const empty = await request(app).get('/workspace/channel/observed-contacts');
    ...
    it('fails closed before internal contact reads when the activity gate is absent', async () => {
        ...
        const response = await request(app).get('/workspaces/conversations/channel/observed-contacts');
    ...
    it('validates freshness bounds and query shape', async () => {
        ...
        const valid = await request(app).get('/workspace/channel/observed-contacts?freshWithinSeconds=60');
        const zero = await request(app).get('/workspace/channel/observed-contacts?freshWithinSeconds=0');
        const nonNumeric = await request(app).get('/workspace/channel/observed-contacts?freshWithinSeconds=recent');
        const repeated = await request(app).get('/workspace/channel/observed-contacts?freshWithinSeconds=60&freshWithinSeconds=120');
        const tooLarge = await request(app).get('/workspace/channel/observed-contacts?freshWithinSeconds=31536001');
    ...
    it('defaults freshness to seven days', async () => {
        const primary = runtime('primary', '/work/main');
        new ObservedChannelContactStore(daemonObservedContactsPath(primary.workspaceCwd), { now: () => new Date(Date.now() - 8 * 24 * 60 * 60 * 1000) }).observe('telegram', {
        ...
        const defaultWindow = await request(app).get('/workspace/channel/observed-contacts');
        const widerWindow = await request(app).get('/workspace/channel/observed-contacts?freshWithinSeconds=777600');
    ...
    it('does not fall back for unknown or untrusted workspace selectors', async () => {
        ...
        const unknown = await request(app).get('/workspaces/missing/channel/observed-contacts');
        const denied = await request(app).get('/workspaces/untrusted/channel/observed-contacts');
