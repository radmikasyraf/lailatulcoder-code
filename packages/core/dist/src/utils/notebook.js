...
const MIME_TYPE_RE = /^[A-Za-z0-9!#$&^_.+-]+\/[A-Za-z0-9!#$&^_.+-]+(?:\+[A-Za-z0-9!#$&^_.+-]+)?$/;
...
export function inferNotebookJsonFormat(content) {
...
export function serializeNotebook(notebook, format = { indent: 1, trailingNewline: true }) {
...
export function getCellDisplayId(cell, index) {
    return typeof cell.id === 'string' && cell.id.length > 0
        ? cell.id
        : `cell-${index}`;
}
export function hasStableCellIds(notebook) {
    return notebook.cells.every((cell) => typeof cell.id === 'string' && cell.id.length > 0);
}
export function findCellIndexesByDisplayId(notebook, cellId) {
    const indexes = [];
...
export function isAmbiguousCellId(notebook, cellId) {
    return findCellIndexesByDisplayId(notebook, cellId).length > 1;
}
export function findCellIndex(notebook, cellId) {
    const indexes = findCellIndexesByDisplayId(notebook, cellId);
    return indexes.length === 1 ? indexes[0] : -1;
}
export function getNotebookLanguage(notebook) {
    return (notebook.metadata?.language_info?.name ??
        notebook.metadata?.kernelspec?.language ??
        'python');
}
export function shouldGenerateCellIds(notebook) {
    return ((notebook.nbformat ?? 0) > 4 ||
        ((notebook.nbformat ?? 0) === 4 && (notebook.nbformat_minor ?? 0) >= 5));
}
export function makeCellId(notebook) {
    if (!shouldGenerateCellIds(notebook)) {
        return undefined;
    }
    const existingDisplayIds = new Set(notebook.cells.map((cell, index) => getCellDisplayId(cell, index)));
    let fallbackIndex = 1;
...
export function inferNotebookSourceArrayStyle(notebook) {
    const sourceCell = notebook.cells.find((cell) => cell.source !== undefined);
    return sourceCell ? Array.isArray(sourceCell.source) : true;
}
export function inferInsertedCellSourceArrayStyle(notebook, insertAt) {
...
export function toNotebookSource(source, preferArray) {
    if (!preferArray) {
        return source;
    }
    if (source.length === 0) {
...
export function normalizeEditedCell(cell, finalType) {
    ...
    switch (output.output_type) {
        ...
        case 'display_data': {
            ...
            if (typeof textData === 'string')
                return stripAnsi(textData);
            if (Array.isArray(textData))
                return stripAnsi(textData.join(''));
 ...
 */
export async function readNotebookWithMetadata(filePath) {
    ...
    for (let i = 0; i < notebook.cells.length; i++) {
        ...
        if (totalLength > MAX_NOTEBOOK_OUTPUT_CHARS) {
            isTruncated = true;
            cellTexts.push(`... [${notebook.cells.length - i} remaining cells truncated, total ${notebook.cells.length} cells. Use shell to inspect: cat <path> | jq '.cells[${i}:]']`);
            break;
        }
 ...
 */
export async function readNotebook(filePath) {
