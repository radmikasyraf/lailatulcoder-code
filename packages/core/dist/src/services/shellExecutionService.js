...
import { serializeTerminalToObject, serializeTerminalToText, } from '../utils/terminalSerializer.js';
 ...
 */
export function getShellAbortReasonKind(reason) {
    if (reason !== null && typeof reason === 'object') {
        try {
            ...
            if (Object.prototype.hasOwnProperty.call(reason, 'kind')) {
                ...
                if (kind === 'background' || kind === 'cancel')
                    return kind;
            }
        }
        catch {
...
function applyUtf8Prefix(command, shell) {
 ...
 */
export function isSignalTermination(signal) {
    return signal !== null && signal !== 0;
}
function createPreSpawnAbortedHandle() {
    return {
        pid: undefined,
...
function getMaxBufferedOutputBytes(config) {
    const configured = config.maxBufferedOutputBytes;
    const floored = typeof configured === 'number' && Number.isFinite(configured)
...
function decodeBufferedOutput(finalBuffer) {
    const fallbackEncoding = getCachedEncodingForBuffer(finalBuffer);
    return new TextDecoder(fallbackEncoding).decode(finalBuffer);
}
function appendOutputCaptureLimitNotice(output, didExceedLimit, totalBytesReceived, maxBufferedOutputBytes) {
    if (!didExceedLimit) {
        return output;
    }
    const notice = `[Output exceeded the maximum captured size of ` +
...
const isExpectedPtyReadExitError = (error) => {
...
const isExpectedPtyExitRaceError = (error) => {
    const code = getErrnoCode(error);
...
const replayTerminalOutput = async (output, cols, rows, Terminal) => {
...
const getLastNonEmptyAnsiLineIndex = (output) => {
...
const trimTrailingEmptyAnsiLines = (output) => output.slice(0, getLastNonEmptyAnsiLineIndex(output) + 1);
const areAnsiOutputsEqual = (left, right) => {
...
const createPlainAnsiLine = (text) => [
    {
        text,
        bold: false,
        italic: false,
...
const serializePlainViewportToAnsiOutput = (terminal, unwrapWrappedLines = false) => {
...
const WINDOWS_TASKKILL_OPTIONS = { windowsHide: true };
const windowsKillPid = (pid, tree) => {
    try {
        ...
        killer.on('error', (e) => {
            debugLogger.warn(`windowsKillPid: taskkill failed to launch for pid ${pid}: ${e.message}`);
        });
        ...
        killer.on('exit', (code, signal) => {
            if (code !== 0 || signal) {
                debugLogger.warn(`windowsKillPid: taskkill exited non-zero for pid ${pid}: ${signal ? `signal ${signal}` : `exit ${code}`}`);
            }
        });
    }
    catch (e) {
        debugLogger.warn(`windowsKillPid: taskkill spawn threw for pid ${pid}: ${e instanceof Error ? e.message : String(e)}`);
    }
};
const windowsStrategy = {
    killPty: (pid, pty) => {
        ...
        try {
            ...
            spawnSync(WINDOWS_TASKKILL, ['/f', '/t', '/pid', pid.toString()], WINDOWS_TASKKILL_OPTIONS);
        }
        catch {
            // ignore
        }
...
const getCleanupStrategy = () => os.platform() === 'win32' ? windowsStrategy : posixStrategy;
/**
...
export class ShellExecutionService {
    ...
    static activeChildProcesses = new Set();
    static cleanup() {
     ...
     */
    static async execute(commandToExecute, cwd, onOutputEvent, abortSignal, shouldUseNodePty, shellExecutionConfig, options = {}) {
        if (abortSignal.aborted) {
            return createPreSpawnAbortedHandle();
        }
        if (shouldUseNodePty) {
            let removeAbortListener;
            const ptyResult = Promise.resolve(getPty()).then((value) => ({ kind: 'resolved', value }), (error) => ({ kind: 'rejected', error }));
            ...
            if (ptyInfo) {
                try {
                    ...
                    return this.executeWithPty(commandToExecute, cwd, onOutputEvent, abortSignal, shellExecutionConfig, ptyInfo, Terminal, options.postPromote);
                }
                catch (_e) {
        ...
        return this.childProcessFallback(commandToExecute, cwd, onOutputEvent, abortSignal, options.streamStdout ?? false, getMaxBufferedOutputBytes(shellExecutionConfig), shellExecutionConfig.pager, options.postPromote);
    }
    static childProcessFallback(commandToExecute, cwd, onOutputEvent, abortSignal, streamStdout, maxBufferedOutputBytes, pager, postPromote) {
        try {
            ...
            const result = new Promise((resolve) => {
                ...
                const markOutputCaptureLimitExceeded = () => {
                    ...
                    debugLogger.warn(`Shell output capture exceeded maxBufferedOutputBytes ` +
                        `(${maxBufferedOutputBytes} bytes). Total bytes: ` +
                        `${totalOutputBytes}. Discarding excess.`);
                };
                const captureOutputData = (data) => {
                    ...
                    const captured = data.length > remainingBytes
                ...
                const handleOutput = (data, stream) => {
                    ...
                    if (isStreamingRawContent && sniffedBytes < MAX_SNIFF_SIZE) {
                        const sniffSlice = data.subarray(0, Math.min(data.length, MAX_SNIFF_SIZE - sniffedBytes));
                ...
                const handleExit = (code, signal) => {
                    ...
                    const boundedOutput = appendOutputCaptureLimitNotice(finalStrippedOutput, outputCaptureLimitExceeded, totalOutputBytes, maxBufferedOutputBytes);
                    if (isStreamingRawContent) {
                ...
                const stdoutHandler = (data) => handleOutput(data, 'stdout');
                const stderrHandler = (data) => handleOutput(data, 'stderr');
                ...
                const performBackgroundPromote = () => {
                    if (!child.pid || exited)
                        return;
                    ...
                    if (child.exitCode !== null || child.signalCode !== null) {
                        debugLogger.debug(`Background-promote requested for pid ${child.pid} but child ` +
                    ...
                    if (streamStdout) {
                        debugLogger.warn('Background-promote on a streamStdout=true child_process: ' +
                    ...
                    const { stdout: snapStdout, stderr: snapStderr, finalBuffer, } = cleanup();
                    const separator = snapStdout.endsWith('\n') ? '' : '\n';
                    const combined = snapStdout +
                        (snapStderr ? (snapStdout ? separator : '') + snapStderr : '');
                    const boundedOutput = appendOutputCaptureLimitNotice(stripAnsi(combined).trim(), outputCaptureLimitExceeded, totalOutputBytes, maxBufferedOutputBytes);
                    ...
                    const postPromoteStderrDecoder = postPromote?.onData
                        ? safeDecoder(detectedEncoding)
                        : null;
                    let postPromoteStdoutHandler = null;
                    let postPromoteStderrHandler = null;
                    if (postPromote?.onData) {
                        const onPostData = postPromote.onData;
                        const safeData = (decoder) => (chunk) => {
                            try {
                                onPostData({
                                    type: 'data',
                                ...
                                debugLogger.warn(`postPromote.onData threw: ${cbErr instanceof Error ? cbErr.message : String(cbErr)}`);
                            }
                        };
                        try {
                            ...
                            debugLogger.warn(`re-attaching post-promote data listeners threw: ${e instanceof Error ? e.message : String(e)}`);
                        }
                    }
                    else if (postPromote) {
                        ...
                        try {
                            ...
                            debugLogger.warn(`post-promote stdout/stderr resume() threw: ${e instanceof Error ? e.message : String(e)}`);
                        }
                    }
                    ...
                    const flushPostPromoteDecoders = () => {
                        if (!postPromote?.onData)
                            return;
                        try {
                            if (postPromoteStdoutDecoder) {
                            ...
                            debugLogger.warn(`post-promote decoder flush threw: ${flushErr instanceof Error ? flushErr.message : String(flushErr)}`);
                        }
                    };
                    const firePostSettle = (info) => {
                        if (postPromoteSettleFired)
                            return;
                        ...
                        try {
                            ...
                            debugLogger.warn(`postPromote.onSettle threw: ${cbErr instanceof Error ? cbErr.message : String(cbErr)}`);
                        }
                    };
                    ...
                    if (postPromote) {
                        try {
                            ...
                            debugLogger.warn(`re-attaching post-promote exit/error listeners threw: ${e instanceof Error ? e.message : String(e)}`);
                        }
                    }
                    resolve({
                        rawOutput: finalBuffer,
                ...
                const performCancelKill = async () => {
                    if (!child.pid || exited)
                        return;
                    if (isWindows) {
                        const killer = cpSpawn(WINDOWS_TASKKILL, ['/f', '/t', '/pid', child.pid.toString()], WINDOWS_TASKKILL_OPTIONS);
                        ...
                        const killChildFallback = (why) => {
                            debugLogger.warn(`performCancelKill (child_process): taskkill ${why} for pid ${child.pid}; falling back to child.kill`);
                            if (!exited) {
                                try {
                                    child.kill('SIGKILL');
                                }
                        ...
                        killer.on('error', (e) => killChildFallback(`failed to launch (${e.message})`));
                        killer.on('exit', (code) => {
                            if (code !== 0)
                                killChildFallback(`exited ${code}`);
                        });
                    }
                    else {
                        try {
    ...
    static executeWithPty(commandToExecute, cwd, onOutputEvent, abortSignal, shellExecutionConfig, ptyInfo, Terminal, postPromote) {
        if (!ptyInfo) {
        ...
        try {
            ...
            const args = os.platform() === 'win32' && shell === 'cmd'
            ...
            const result = new Promise((resolve) => {
                ...
                const maxBufferedOutputBytes = getMaxBufferedOutputBytes(shellExecutionConfig);
                let capturedOutputBytes = 0;
                let outputCaptureLimitExceeded = false;
                let outputCaptureLimitWarningEmitted = false;
                let isWriting = false;
                ...
                const renderFn = () => {
                    ...
                    const finalOutput = shellExecutionConfig.disableDynamicLineTrimming
                        ? newOutput
                        : trimmedOutput;
                    const finalOutputComparison = shellExecutionConfig.disableDynamicLineTrimming
                        ? newOutputComparison
                ...
                const disposeForegroundPtyResources = () => {
                    ...
                    try {
                        ...
                        debugLogger.warn(`dataDisposable.dispose() threw during PTY cleanup: ${e instanceof Error ? e.message : String(e)}`);
                    }
                    try {
                        exitDisposable.dispose();
                    }
                    catch (e) {
                        debugLogger.warn(`exitDisposable.dispose() threw during PTY cleanup: ${e instanceof Error ? e.message : String(e)}`);
                    }
                    try {
                        ptyProcess.removeListener('error', ptyErrorHandler);
                    }
                    catch (e) {
                        debugLogger.warn(`ptyProcess.removeListener('error') threw during PTY cleanup: ${e instanceof Error ? e.message : String(e)}`);
                    }
                    try {
                        headlessTerminal.dispose();
                    }
                    catch (e) {
                        debugLogger.warn(`headlessTerminal.dispose() threw during PTY cleanup: ${e instanceof Error ? e.message : String(e)}`);
                    }
                ...
                const exitDisposable = ptyProcess.onExit(({ exitCode, signal }) => {
                    ...
                    const finalize = async () => {
                        ...
                        try {
                            try {
                                ...
                                debugLogger.warn(`Final PTY render threw during cleanup: ${e instanceof Error ? e.message : String(e)}`);
                            }
                            try {
                                if (isStreamingRawContent) {
                                    ...
                                    const decodedOutput = new TextDecoder(finalEncoding).decode(finalBuffer);
                                    fullOutput = await replayTerminalOutput(decodedOutput, cols, rows, Terminal);
                                }
                                else {
                            ...
                            fullOutput = appendOutputCaptureLimitNotice(fullOutput, outputCaptureLimitExceeded, totalBytesReceived, maxBufferedOutputBytes);
                            resolve({
                                rawOutput: finalBuffer,
                                output: fullOutput,
                                exitCode,
                    ...
                    const deadline = new Promise((res) => setTimeout(res, SIGKILL_TIMEOUT_MS));
                    const drain = () => new Promise((res) => setImmediate(res)).then(flushChain);
                    void Promise.race([
                ...
                const performBackgroundPromote = async () => {
                    ...
                    if (!ShellExecutionService.isPtyActive(ptyProcess.pid)) {
                        debugLogger.debug(`Background-promote requested for PTY pid ${ptyProcess.pid} ` +
                    ...
                    try {
                        ...
                        debugLogger.warn(`dataDisposable.dispose() threw during background-promote: ${e instanceof Error ? e.message : String(e)}`);
                    }
                    try {
                        exitDisposable.dispose();
                    }
                    catch (e) {
                        debugLogger.warn(`exitDisposable.dispose() threw during background-promote: ${e instanceof Error ? e.message : String(e)}`);
                    }
                    try {
                        ...
                        debugLogger.warn(`ptyProcess.removeListener('error') threw during background-promote: ${e instanceof Error ? e.message : String(e)}`);
                    }
                    if (renderTimeout) {
                        clearTimeout(renderTimeout);
                        renderTimeout = null;
                    ...
                    let postPromoteSettleFired = false;
                    let postPromoteDataDisposable = null;
                    let postPromoteExitDisposable = null;
                    let postPromoteErrorListener = null;
                    const disposePostPromoteListeners = () => {
                        if (postPromoteDataDisposable) {
                            try {
                                ...
                                debugLogger.warn(`disposing post-promote data listener threw: ${e instanceof Error ? e.message : String(e)}`);
                            }
                        ...
                        if (postPromoteExitDisposable) {
                            try {
                                ...
                                debugLogger.warn(`disposing post-promote exit listener threw: ${e instanceof Error ? e.message : String(e)}`);
                            }
                        ...
                        if (postPromoteErrorListener) {
                            try {
                                ...
                                debugLogger.warn(`removing post-promote error listener threw: ${e instanceof Error ? e.message : String(e)}`);
                            }
                    ...
                    if (postPromote?.onData) {
                        const onPostData = postPromote.onData;
                        try {
                            postPromoteDataDisposable = ptyProcess.onData((data) => {
                                try {
                            ...
                            debugLogger.warn(`re-attaching post-promote data listener threw: ${e instanceof Error ? e.message : String(e)}`);
                        }
                    }
                    if (postPromote) {
                        try {
                            postPromoteExitDisposable = ptyProcess.onExit(({ exitCode, signal, }) => {
                                firePostSettle({
                                    exitCode,
                                    signal: signal === 0 ? null : (signal ?? null),
                                    endTime: Date.now(),
                            ...
                            debugLogger.warn(`re-attaching post-promote exit listener threw: ${e instanceof Error ? e.message : String(e)}`);
                        }
                        try {
                            postPromoteErrorListener = (err) => {
                                if (isExpectedPtyReadExitError(err)) {
                                    return;
                                }
                                firePostSettle({
                            ...
                            debugLogger.warn(`re-attaching post-promote error listener threw: ${e instanceof Error ? e.message : String(e)}`);
                        }
                    }
                    ...
                    const drain = () => new Promise((res) => setImmediate(res)).then(() => processingChain);
                    const winner = await Promise.race([
                        processingChain
                            .then(drain)
                            .then(drain)
                            .catch(() => undefined),
                        new Promise((res) => setTimeout(() => res(TIMEOUT_SENTINEL), PROMOTE_DRAIN_TIMEOUT_MS)),
                    ]);
                    if (winner === TIMEOUT_SENTINEL) {
                        debugLogger.warn(`Background-promote drain hit the ${PROMOTE_DRAIN_TIMEOUT_MS}ms ` +
                    ...
                    try {
                        ...
                        if (isStreamingRawContent) {
                            const finalEncoding = getCachedEncodingForBuffer(finalBuffer);
                            const decodedOutput = new TextDecoder(finalEncoding).decode(finalBuffer);
                            snapshot = await replayTerminalOutput(decodedOutput, cols, rows, Terminal);
                        }
                        else {
                        ...
                        debugLogger.warn(`Background-promote snapshot replay failed: ${serErr instanceof Error ? serErr.message : String(serErr)}. ` +
                    ...
                    snapshot = appendOutputCaptureLimitNotice(snapshot, outputCaptureLimitExceeded, totalBytesReceived, maxBufferedOutputBytes);
                    try {
                        resolve({
                            rawOutput: finalBuffer,
                            output: snapshot,
                        ...
                        try {
                            ...
                            debugLogger.warn(`headlessTerminal.dispose() threw during background-promote cleanup: ${e instanceof Error ? e.message : String(e)}`);
                        }
                    }
                };
                const performCancelKill = async () => {
                    ...
                    if (os.platform() === 'win32') {
                        ...
                        try {
                            const r = spawnSync(WINDOWS_TASKKILL, ['/f', '/t', '/pid', ptyProcess.pid.toString()], WINDOWS_TASKKILL_OPTIONS);
                            if (r.error || (typeof r.status === 'number' && r.status !== 0)) {
                                debugLogger.warn(`performCancelKill: taskkill failed for pid ${ptyProcess.pid}: ${r.error?.message ?? `exit ${r.status}`}`);
                            }
                        }
                        catch (e) {
                            debugLogger.warn(`performCancelKill: taskkill spawn threw for pid ${ptyProcess.pid}: ${e instanceof Error ? e.message : String(e)}`);
                        }
            ...
            if (error.message.includes('posix_spawnp failed')) {
                onOutputEvent({
                    type: 'data',
                    chunk: '[WARNING] PTY execution failed, falling back to child_process. This may be due to sandbox restrictions.\n',
                });
                throw e;
            }
            else {
