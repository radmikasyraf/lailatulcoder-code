...
const { mockWatchers, mockExistsSync, mockMkdirSync, mockWatch } = vi.hoisted(() => {
    ...
    const mockWatch = vi.fn().mockImplementation((dir) => {
        const handlers = {};
        const instance = {
            on: vi
                .fn()
...
function makeLoadedSettings(overrides = {}) {
...
function fireAllEvent(watcherIndex, event, changedPath) {
...
describe('SettingsWatcher', () => {
    ...
    describe('lifecycle', () => {
        it('should create chokidar watchers for user and workspace directories', () => {
            ...
            expect(mockWatch).toHaveBeenCalledWith('/home/user/.qwen', expect.objectContaining({ ignoreInitial: true, depth: 0 }));
            expect(mockWatch).toHaveBeenCalledWith('/project/.qwen', expect.objectContaining({ ignoreInitial: true, depth: 0 }));
        });
        ...
        it('should watch active workspace even when settings file does not exist', () => {
            ...
            const noWorkspaceFileWatcher = new SettingsWatcher(noWorkspaceFileSettings);
        ...
        it('should register error handler on each watcher', () => {
            watcher.startWatching();
            expect(mockWatchers[0].instance.on).toHaveBeenCalledWith('error', expect.any(Function));
        });
        it('should pass ignored filter that rejects special file types', () => {
            ...
            expect(ignoredFn('/some/file', {
                isFile: () => true,
                isDirectory: () => false,
            })).toBe(false);
            expect(ignoredFn('/some/socket', {
                isFile: () => false,
                isDirectory: () => false,
            })).toBe(true);
        });
    ...
    describe('path filtering', () => {
        it('should trigger refresh only for settings.json basename', async () => {
            ...
            vi.mocked(settings.reloadScopeFromDisk).mockImplementation((scope) => {
            ...
            expect(settings.reloadScopeFromDisk).toHaveBeenCalledWith(SettingScope.User);
    ...
    describe('change type classification', () => {
        it('should report created when file appears', async () => {
            ...
            vi.mocked(noFileSettings.reloadScopeFromDisk).mockImplementation((scope) => {
    ...
    describe('listener management', () => {
        ...
        it('should enforce listener timeout', async () => {
            watcher.startWatching();
            const slowListener = vi.fn(() => new Promise((resolve) => {
            ...
            fireAllEvent(0, 'change', '/home/user/.qwen/settings.json');
            await vi.advanceTimersByTimeAsync(SettingsWatcher.DEBOUNCE_MS + SettingsWatcher.LISTENER_TIMEOUT_MS + 100);
    ...
    describe('watcher creation failure', () => {
        it('should continue with remaining scopes when chokidar throws', () => {
            let callCount = 0;
            mockWatch.mockImplementation((dir) => {
                callCount++;
                if (callCount === 1) {
        ...
        it('should continue with bootstrap watcher when target dir is missing', () => {
            ...
            expect(mockWatch).toHaveBeenCalledTimes(2);
            expect(mockWatch).toHaveBeenCalledWith('/home/user', expect.objectContaining({ ignoreInitial: true, depth: 0 }));
            expect(mockWatch).toHaveBeenCalledWith('/project', expect.objectContaining({ ignoreInitial: true, depth: 0 }));
        });
    });
    describe('lazy directory watching', () => {
        ...
        it('should promote to a target watcher when .qwen appears', async () => {
            ...
            vi.mocked(workspaceOnly.reloadScopeFromDisk).mockImplementation((scope) => {
                workspaceOnly.forScope(scope).settings = s({ promoted: true });
            });
            await vi.advanceTimersByTimeAsync(SettingsWatcher.DEBOUNCE_MS + 10);
            expect(workspaceOnly.reloadScopeFromDisk).toHaveBeenCalledWith(SettingScope.User);
            w.stopWatching();
        });
        ...
        it('should not double-promote from a stale bootstrap callback', async () => {
            ...
            await flush();
            expect(mockWatchers).toHaveLength(2);
            const targetWatchers = mockWatchers.filter((m) => m.dir === '/home/user/.qwen');
    ...
    describe('reloadScopeFromDisk failure', () => {
        ...
        it('should log rejected refreshes', async () => {
            ...
            fireAllEvent(0, 'change', '/home/user/.qwen/settings.json');
            await vi.advanceTimersByTimeAsync(SettingsWatcher.DEBOUNCE_MS + 10);
            expect(mockDebugWarn).toHaveBeenCalledWith('Settings watcher refresh error:', error);
        });
    });
    ...
    describe('path resolution', () => {
        it('should use resolved paths from LoadedSettings (supports QWEN_HOME redirect)', () => {
            ...
            w.startWatching();
            expect(mockWatch).toHaveBeenCalledWith('/custom/qwen-home', expect.any(Object));
            w.stopWatching();
        });
    });
...
//# sourceMappingURL=settingsWatcher.test.js.map