...
export const STATUS_LINE_PRESET_ITEMS = [
    ...
    {
        id: 'LailatulCoder-version',
        label: 'LailatulCoder-version',
...
const STATUS_LINE_PRESET_ITEM_ID_SET = new Set(STATUS_LINE_PRESET_ITEM_IDS);
export function orderStatusLinePresetItems(items) {
    const selectedItems = new Set(items.filter((item) => typeof item === 'string' && STATUS_LINE_PRESET_ITEM_ID_SET.has(item)));
    return STATUS_LINE_PRESET_ITEM_IDS.filter((item) => selectedItems.has(item));
}
export const DEFAULT_STATUS_LINE_PRESET_CONFIG = {
    type: 'preset',
    useThemeColors: true,
    items: orderStatusLinePresetItems(STATUS_LINE_PRESET_ITEMS.filter((item) => item.defaultSelected).map((item) => item.id)),
};
export function normalizeStatusLinePresetConfig(raw) {
    ...
    const items = hasItemsArray
        ? orderStatusLinePresetItems(candidate['items'])
        : [];
    return {
        type: 'preset',
        useThemeColors: typeof candidate['useThemeColors'] === 'boolean'
            ? candidate['useThemeColors']
            : true,
        items: hasItemsArray ? items : [...DEFAULT_STATUS_LINE_PRESET_CONFIG.items],
        hideContextIndicator: typeof candidate['hideContextIndicator'] === 'boolean'
...
export function getRunStateLabel(state) {
    switch (state) {
...
export function formatModelWithReasoning(modelDisplayName, reasoning) {
...
export function inferPullRequestNumber(branch) {
    if (!branch) {
        return undefined;
    }
    const match = branch.match(/(?:^|[/_-])(?:pr|pull|pull-request)[/_-]?#?(\d+)(?:$|[/_-])/i);
    return match?.[1];
}
export function buildStatusLinePresetData(params) {
    const usedPercentage = params.contextWindowSize > 0
        ? Math.min(100, Math.max(0, Math.round((params.currentUsage / params.contextWindowSize) * 1000) / 10))
        : 0;
    return {
...
function formatPresetItem(item, data) {
    switch (item) {
        ...
        case 'pull-request-number': {
            const prNumber = data.pullRequestNumber ?? inferPullRequestNumber(data.branch);
            if (prNumber) {
                return `#${prNumber}`;
            }
            return undefined;
...
export function buildStatusLinePresetParts(config, data) {
    const parts = [];
...
export function buildStatusLinePresetLines(config, data) {
    const line = buildStatusLinePresetParts(config, data).join(' \u00b7 ');
    return line ? [line] : [];
}
//# sourceMappingURL=statusLinePresets.js.map
