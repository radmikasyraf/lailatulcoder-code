...
import { AgentViewLaunchConfigError, AgentViewPtyUnavailableError, BoundedOutputRing, checkAgentViewPtyAvailability, launchAgentViewPtyHost, validateAgentViewLaunchConfig, } from './pty-host.js';
...
describe('PTY availability', () => {
    it('reports injected PTY availability', async () => {
        await expect(checkAgentViewPtyAvailability(async () => createFakePty())).resolves.toEqual({
            available: true,
            implementationName: 'injected',
        });
    });
    it('reports missing PTY without throwing', async () => {
        await expect(checkAgentViewPtyAvailability(async () => null)).resolves.toEqual({
            available: false,
            reason: 'missing',
        });
    });
...
describe('launchAgentViewPtyHost', () => {
    it('rejects commands containing empty segments', async () => {
        ...
        await expect(launchAgentViewPtyHost(createLaunch(), {
            pty,
    ...
    it('passes worker env while stripping host-only secrets', async () => {
        ...
        expect(pty.spawnCalls[0]?.options.env).toEqual(expect.objectContaining({
            QWEN_AGENT_VIEW_WORKER: '1',
            QWEN_AGENT_VIEW_AMBIENT_MARKER: 'ambient-value',
            TERM: 'xterm-256color',
        }));
        expect(pty.spawnCalls[0]?.options.env[PTY_HOST_AUTH_TOKEN_ENV]).toBeUndefined();
    ...
    it('strips host-only secrets even when the launch env re-adds them', async () => {
        ...
        expect(pty.spawnCalls[0]?.options.env).toEqual(expect.objectContaining({ QWEN_AGENT_VIEW_WORKER: '1' }));
        for (const key of [
    ...
    it('lets the launch env override inherited process env values', async () => {
        ...
        try {
            await launchAgentViewPtyHost({ ...createLaunch(), env: { [key]: 'from-launch' } }, { pty });
        }
        finally {
    ...
    it('strips an inherited sideband token the launch env does not replace', async () => {
        ...
        expect(pty.spawnCalls[0]?.options.env['QWEN_AGENT_VIEW_TOKEN']).toBeUndefined();
    });
    it.skipIf(process.platform === 'win32')('passes kill signals through to the PTY process', async () => {
        const pty = createFakePty();
        const handle = await launchAgentViewPtyHost(createLaunch(), { pty });
        handle.kill('SIGKILL');
    ...
    it('kills the PTY process when disposed', async () => {
        ...
        const handle = await launchAgentViewPtyHost(createLaunch(), { pty });
        handle.dispose();
        expect(pty.process.killedWith).toBe(process.platform === 'win32' ? undefined : 'SIGTERM');
        expect(pty.process.killCalls).toEqual(process.platform === 'win32' ? [undefined] : ['SIGTERM']);
        await expect(handle.exited).resolves.toEqual({ exitCode: 1 });
    });
    it.skipIf(process.platform === 'win32')('gracefully shuts down the PTY process with SIGTERM', async () => {
        const pty = createFakePty();
        const handle = await launchAgentViewPtyHost(createLaunch(), { pty });
        handle.shutdown?.();
    ...
    it('throws a typed error when PTY is unavailable', async () => {
        await expect(launchAgentViewPtyHost(createLaunch(), { pty: null })).rejects.toBeInstanceOf(AgentViewPtyUnavailableError);
    });
    it('throws a typed error for invalid launch config', async () => {
        await expect(launchAgentViewPtyHost({ ...createLaunch(), terminal: undefined })).rejects.toBeInstanceOf(AgentViewLaunchConfigError);
    });
});
...
class FakePtyProcess {
    ...
    onData(callback) {
        ...
        return {
            dispose: () => {
                this.dataCallbacks = this.dataCallbacks.filter((item) => item !== callback);
            },
        };
    }
    onExit(callback) {
        ...
        return {
            dispose: () => {
                this.exitCallbacks = this.exitCallbacks.filter((item) => item !== callback);
            },
        };
    }
    kill(signal) {
...
//# sourceMappingURL=pty-host.test.js.map