 ...
 */
import { ApprovalMode, checkCommandPermissions, escapeShellArg, getShellConfiguration, ShellExecutionService, isSignalTermination, flatMapTextParts, checkArgumentSafety, } from '@lailatul-coder/lailatul-coder-core';
...
export class ShellProcessor {
    ...
    async processString(prompt, context) {
        ...
        for (const injection of resolvedInjections) {
            ...
            const { allAllowed, disallowedCommands, blockReason, isHardDenial } = await checkCommandPermissions(command, config, sessionShellAllowlist);
        ...
        for (const injection of resolvedInjections) {
            ...
            if (injection.resolvedCommand) {
                ...
                const { result } = await ShellExecutionService.execute(injection.resolvedCommand, config.getTargetDir(), () => { }, new AbortController().signal, config.getShouldUseNodePtyShell(), shellExecutionConfig);
