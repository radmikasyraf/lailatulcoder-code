...
import { GetPromptResultSchema, ListPromptsResultSchema, ListResourcesResultSchema, ListRootsRequestSchema, ReadResourceResultSchema, } from '@modelcontextprotocol/sdk/types.js';
...
import { getOrCreateMcpDispatcher, loadUndici, preloadRuntimeFetchModule, resetDispatcherCache, } from '../utils/runtimeFetchOptions.js';
...
const invocationContextTransports = new WeakSet();
const invocationContextClients = new WeakSet();
function bindInvocationContextPolicy(client, transport) {
...
export function getMcpOAuthDialogInstruction(action, mcpServerName) {
    return [
        `In interactive LailatulCoder Ai sessions, open the /mcp dialog to ${action}`,
...
function getSseOAuth401Message(mcpServerName, tokenState) {
    if (tokenState === 'accepted-token-rejected') {
        return (`Stored OAuth token for SSE server '${mcpServerName}' was rejected. ` +
            getMcpOAuthDialogInstruction('re-authenticate', mcpServerName));
    }
    if (tokenState === 'unusable') {
        return (`Stored OAuth tokens for SSE server '${mcpServerName}' are expired or could not be refreshed. ` +
            getMcpOAuthDialogInstruction('re-authenticate', mcpServerName));
    }
    return (`401 error received for SSE server '${mcpServerName}' without OAuth configuration. ` +
...
function isStreamableHttpGetSseRequest(init) {
 ...
 */
export function createStreamableHttpCompatibilityFetch(mcpServerName, fetchFn = globalThis.fetch.bind(globalThis), mcpServerConfig) {
    return async (input, init) => {
        const requestHeaders = new Headers(init?.headers ?? (input instanceof Request ? input.headers : undefined));
        const stopAgentPluginRedirect = mcpServerConfig?.agentPluginV1 === true &&
        ...
        if (response.status === 401 &&
            ...
            setMcpOAuthRequirement(mcpServerName, mcpServerConfig, response.headers.get('www-authenticate') ?? '');
        }
        if (!isStreamableHttpGetSseRequest(effectiveInit) ||
        ...
        debugLogger.warn(`MCP server '${mcpServerName}' rejected the optional Streamable HTTP ` +
...
const mcpUndiciFetch = (async (input, init) => {
...
let mcpFetchOverrideForTest;
export function _setMcpFetchForTest(fn) {
...
export function _resetMcpFetchDispatcherForTest() {
    resetDispatcherCache();
}
function createMcpStreamableHttpFetch(mcpServerName, mcpServerConfig) {
    return createStreamableHttpCompatibilityFetch(mcpServerName, mcpFetchOverrideForTest ?? mcpUndiciFetch, mcpServerConfig);
}
/**
...
export class McpClient {
    ...
    async connect() {
        ...
        try {
            ...
            this.client.onerror = (error) => {
                ...
                debugLogger.error(`MCP ERROR (${this.serverName}): ${getErrorMessage(error)}`);
            ...
            if (hasNetworkTransport(this.serverConfig) &&
                ...
                if (oauthChallenge.confirmed401) {
                    setMcpOAuthRequirement(this.serverName, this.serverConfig, oauthChallenge.wwwAuthenticate);
                }
                else {
    ...
    async discover(cliConfig) {
        ...
        const { tools, prompts, resources } = await this.discoverAndReturn(cliConfig);
     ...
     */
    async discoverAndReturn(cliConfig, opts) {
        ...
        try {
            ...
            const [prompts, resources, tools] = await Promise.all([
                ...
                discoverTools(this.serverName, this.serverConfig, this.client, cliConfig, { applyConfigFilters: opts?.applyConfigFilters ?? true }),
            ]);
            if (prompts.length === 0 &&
                tools.length === 0 &&
                resources.length === 0) {
    ...
    getTransportPid() {
        ...
        if (!t || typeof t.pid !== 'number' || t.pid <= 0)
            return undefined;
        return t.pid;
    }
    /**
    ...
    async readResource(uri, options) {
        ...
        return this.client.request({ method: 'resources/read', params: { uri } }, ReadResourceResultSchema, options);
    }
    updateStatus(status) {
        this.status = status;
    ...
    async createTransport() {
        return createTransport(this.serverName, this.serverConfig, this.debugMode, this.sendSdkMcpMessage);
    }
}
/**
...
const mcpServerOAuthChallenges = new Map();
const mcpServerOAuthRequirements = new Set();
const mcpServerOAuthProbeCandidates = new Set();
const mcpServerOAuthProbeInFlight = new Map();
const mcpServerOAuthProbeVersions = new Map();
const automaticOAuthInFlight = new Map();
let automaticOAuthQueue = Promise.resolve();
/**
 ...
 */
export function setMCPDiscoveryState(state) {
...
function oauthRecoveryKey(mcpServerName, mcpServerConfig) {
    return `${mcpServerName}\0${mcpServerConfig.httpUrl ?? mcpServerConfig.url ?? ''}`;
}
function supportsMcpOAuth(mcpServerConfig) {
    return (!mcpServerConfig.authProviderType ||
        mcpServerConfig.authProviderType === AuthProviderType.DYNAMIC_DISCOVERY);
}
function getMcpOAuthChallengeFromError(error) {
    const errorString = getErrorMessage(error);
    return {
        confirmed401: getErrorStatus(error) === 401 || /(^|\D)401(\D|$)/.test(errorString),
        wwwAuthenticate: extractWWWAuthenticateHeader(errorString) ?? '',
    };
}
function setMcpOAuthRequirement(mcpServerName, mcpServerConfig, wwwAuthenticate) {
    ...
    mcpServerOAuthChallenges.set(key, wwwAuthenticate || mcpServerOAuthChallenges.get(key) || '');
    mcpServerRequiresOAuth.set(mcpServerName, true);
}
function clearMcpOAuthRequirement(mcpServerName, mcpServerConfig) {
    const key = oauthRecoveryKey(mcpServerName, mcpServerConfig);
    mcpServerOAuthProbeVersions.set(key, (mcpServerOAuthProbeVersions.get(key) ?? 0) + 1);
    mcpServerOAuthProbeCandidates.delete(key);
    mcpServerOAuthRequirements.delete(key);
    mcpServerOAuthChallenges.delete(key);
    const serverPrefix = `${mcpServerName}\0`;
    if (![...mcpServerOAuthRequirements].some((candidate) => candidate.startsWith(serverPrefix))) {
        mcpServerRequiresOAuth.delete(mcpServerName);
    }
}
export async function probeMcpServerForOAuth(mcpServerName, mcpServerConfig, error) {
    ...
    if (error !== undefined) {
        ...
        if (challenge.confirmed401) {
            setMcpOAuthRequirement(mcpServerName, mcpServerConfig, challenge.wwwAuthenticate);
            return true;
        }
    }
    else if (!mcpServerOAuthProbeCandidates.has(key) &&
    ...
    const probe = (async () => {
        try {
            const response = await fetch(mcpServerConfig.httpUrl || mcpServerConfig.url, {
                method: 'HEAD',
                headers: {
            ...
            debugLogger.debug(`Failed to probe MCP server for OAuth challenge: ${getErrorMessage(fetchError)}`);
        }
        mcpServerOAuthProbeCandidates.delete(key);
        return false;
    })().finally(() => {
...
async function handleAutomaticOAuth(mcpServerName, mcpServerConfig, wwwAuthenticate) {
    try {
        ...
        if (resourceMetadataUri) {
            oauthConfig = await retryWithBackoff(() => OAuthUtils.discoverOAuthConfig(resourceMetadataUri), `${mcpServerName}/oauth-discovery`, { maxRetries: 1 });
        }
            ...
            oauthConfig = await retryWithBackoff(() => OAuthUtils.discoverOAuthConfig(baseUrl), `${mcpServerName}/oauth-discovery`, { maxRetries: 1 });
        }
        if (!oauthConfig) {
            debugLogger.error(`Could not configure OAuth for server '${mcpServerName}'. ` +
                getMcpOAuthDialogInstruction('authenticate', mcpServerName));
            return false;
        }
        ...
        debugLogger.info(`Starting OAuth authentication for server '${mcpServerName}'...`);
        const authProvider = new MCPOAuthProvider(new MCPOAuthTokenStorage());
        await authProvider.authenticate(mcpServerName, oauthAuthConfig, serverUrl);
        debugLogger.info(`OAuth authentication successful for server '${mcpServerName}'`);
        return true;
    }
    catch (error) {
        debugLogger.error(`Failed to handle automatic OAuth for server '${mcpServerName}': ${getErrorMessage(error)}`);
        return false;
    }
}
async function withAutomaticOAuthTimeout(recovery, mcpServerName) {
    let timeout;
    const timedOut = new Promise((resolve) => {
        timeout = setTimeout(() => {
            debugLogger.error(`Timed out handling automatic OAuth for server '${mcpServerName}'. ` +
                getMcpOAuthDialogInstruction('authenticate', mcpServerName));
            resolve(false);
...
export async function attemptAutomaticMcpOAuth(mcpServerName, mcpServerConfig, allowBrowserLaunch) {
    ...
    const recovery = automaticOAuthQueue
        .then(() => withAutomaticOAuthTimeout(handleAutomaticOAuth(mcpServerName, mcpServerConfig, mcpServerOAuthChallenges.get(key) ?? ''), mcpServerName))
        .finally(() => {
        automaticOAuthInFlight.delete(key);
    });
    automaticOAuthInFlight.set(key, recovery);
    automaticOAuthQueue = recovery.then(() => undefined, () => undefined);
    return recovery;
}
/**
...
async function createTransportWithOAuth(mcpServerName, mcpServerConfig, accessToken) {
    try {
        ...
        debugLogger.error(`Failed to create OAuth transport for server '${mcpServerName}': ${getErrorMessage(error)}`);
        return null;
    }
}
/**
...
export async function discoverMcpTools(mcpServers, mcpServerCommand, toolRegistry, promptRegistry, debugMode, workspaceContext, cliConfig) {
    ...
    try {
        mcpServers = populateMcpServerCommand(mcpServers, mcpServerCommand, cliConfig.getTargetDir());
...
export function populateMcpServerCommand(mcpServers, mcpServerCommand, cwd) {
    ...
    return Object.fromEntries(Object.entries(result).map(([name, server]) => [
        name,
 ...
 */
export async function connectAndDiscover(mcpServerName, mcpServerConfig, toolRegistry, promptRegistry, debugMode, workspaceContext, cliConfig, sendSdkMcpMessage) {
    ...
    try {
        ...
        const resources = await discoverResources(mcpServerName, mcpClient, cliConfig.getResourceRegistry());
        ...
        debugLogger.error(`Error connecting to MCP server '${mcpServerName}': ${getErrorMessage(error)}`);
        updateMCPServerStatus(mcpServerName, MCPServerStatus.DISCONNECTED);
    }
}
/**
 ...
 */
export async function discoverTools(mcpServerName, mcpServerConfig, mcpClient, cliConfig, opts) {
    try {
        const { mcpToTool } = await import('@google/genai');
        const mcpCallableTool = mcpToTool(mcpClient, {
            timeout: mcpServerConfig.timeout ?? MCP_DEFAULT_TIMEOUT_MSEC,
        });
        const tool = await retryWithBackoff(() => mcpCallableTool.tool(), `${mcpServerName}/tools/list`);
        ...
        try {
            ...
            debugLogger.error(`Failed to fetch tool annotations from MCP server '${mcpServerName}'`);
        }
        ...
        for (const funcDecl of tool.functionDeclarations) {
            try {
                ...
                if (applyConfigFilters &&
                    !isEnabled(funcDecl, mcpServerName, mcpServerConfig)) {
                    continue;
                }
                discoveredTools.push(new DiscoveredMCPTool(mcpCallableTool, mcpServerName, funcDecl.name, funcDecl.description ?? '', funcDecl.parametersJsonSchema ?? { type: 'object', properties: {} }, applyConfigFilters ? mcpServerConfig.trust : undefined, undefined, cliConfig, mcpClient, // raw MCP Client for direct callTool with progress
                mcpTimeout, cliConfig?.getMcpToolIdleTimeoutMs?.(), annotationsMap.get(funcDecl.name), mcpServerConfig.alwaysLoadTools === true, invocationContextClients.has(mcpClient)));
            }
            catch (error) {
                debugLogger.error(`Error discovering tool: '${funcDecl.name}' from MCP server '${mcpServerName}': ${error.message}`);
            }
        }
        ...
        if (!isMethodNotFound(error)) {
            debugLogger.error(`Error discovering tools from ${mcpServerName}: ${getErrorMessage(error)}`);
        }
        return [];
    }
}
 ...
 * silently invisible in lailatul-coder (no `/`-menu entry) while lenient
 ...
 */
export async function listMcpPrompts(mcpServerName, mcpClient) {
    try {
        const response = await retryWithBackoff(() => mcpClient.request({ method: 'prompts/list', params: {} }, ListPromptsResultSchema), `${mcpServerName}/prompts/list`);
        ...
        if (!isMethodNotFound(error)) {
            debugLogger.error(`Error discovering prompts from ${mcpServerName}: ${getErrorMessage(error)}`);
        }
        return [];
    }
}
...
export async function invokeMcpPrompt(mcpServerName, mcpClient, promptName, promptParams) {
    try {
        ...
        if (!isMethodNotFound(error)) {
            debugLogger.error(`Error invoking prompt '${promptName}' from ${mcpServerName} ${promptParams}: ${getErrorMessage(error)}`);
        }
        throw error;
    }
}
 ...
 */
export async function listMcpResources(mcpServerName, mcpClient) {
    try {
        const response = await retryWithBackoff(() => mcpClient.request({ method: 'resources/list', params: {} }, ListResourcesResultSchema), `${mcpServerName}/resources/list`);
        ...
        // servers).
        if (!isMethodNotFound(error)) {
            debugLogger.error(`Error discovering resources from ${mcpServerName}: ${getErrorMessage(error)}`);
        }
        return [];
    }
}
 ...
 */
export async function discoverResources(mcpServerName, mcpClient, resourceRegistry) {
 ...
 */
export async function connectToMcpServer(mcpServerName, mcpServerConfig, debugMode, workspaceContext, sendSdkMcpMessage) {
    clearMcpOAuthRequirement(mcpServerName, mcpServerConfig);
    const mcpClient = new Client({
        name: 'lailatul-coder-mcp-client',
        version: '0.0.1',
    });
    ...
    if (mcpServerConfig.url &&
        ...
        try {
            hadStoredSseOAuthCredentials = Boolean(await tokenStorage.getCredentials(mcpServerName));
        }
        catch (error) {
            debugLogger.warn(`Failed to pre-read stored OAuth credentials for SSE server '${mcpServerName}': ${getErrorMessage(error)}`);
        }
    }
    try {
        ...
        unlistenDirectories?.();
        unlistenDirectories = undefined;
        const requiresOAuth = await probeMcpServerForOAuth(mcpServerName, mcpServerConfig, error);
        if (requiresOAuth) {
            ...
            if (!shouldTriggerOAuth) {
                ...
                try {
                    ...
                    debugLogger.error(`Failed to re-read stored OAuth credentials for SSE server '${mcpServerName}' after 401: ${getErrorMessage(credentialError)}`);
                }
                let tokenState = credentials || hadStoredSseOAuthCredentials ? 'unusable' : 'missing';
                if (credentials) {
                    ...
                    try {
                        ...
                        debugLogger.error(`Failed to validate stored OAuth token for SSE server '${mcpServerName}': ${getErrorMessage(tokenError)}`);
                        tokenState = 'unusable';
                    }
                }
            ...
            const wwwAuthenticate = mcpServerOAuthChallenges.get(oauthRecoveryKey(mcpServerName, mcpServerConfig)) ?? '';
            if (wwwAuthenticate) {
                debugLogger.debug(`Received 401 with www-authenticate header: ${wwwAuthenticate}`);
                ...
                if (oauthSuccess) {
                    ...
                    debugLogger.info(`Retrying connection to '${mcpServerName}' with OAuth token...`);
                        ...
                        if (accessToken) {
                            ...
                            if (oauthTransport) {
                                try {
                                    ...
                                    debugLogger.error(`Failed to connect with OAuth token: ${getErrorMessage(retryError)}`);
                                    throw retryError;
                                }
                            }
                            else {
                                debugLogger.error(`Failed to create OAuth transport for server '${mcpServerName}'`);
                                throw new Error(`Failed to create OAuth transport for server '${mcpServerName}'`);
                            }
                        }
                        else {
                            debugLogger.error(`Failed to get OAuth token for server '${mcpServerName}'`);
                            throw new Error(`Failed to get OAuth token for server '${mcpServerName}'`);
                        }
                    }
                    else {
                        debugLogger.error(`Failed to get credentials for server '${mcpServerName}' after successful OAuth authentication`);
                        throw new Error(`Failed to get credentials for server '${mcpServerName}' after successful OAuth authentication`);
                    }
                }
                else {
                    const oauthMessage = `Failed to handle automatic OAuth for server '${mcpServerName}'. ` +
                        getMcpOAuthDialogInstruction('authenticate', mcpServerName);
                ...
                debugLogger.info(`Attempting OAuth discovery for '${mcpServerName}'...`);
                if (hasNetworkTransport(mcpServerConfig)) {
                    const serverUrl = new URL(mcpServerConfig.httpUrl || mcpServerConfig.url);
                    const baseUrl = `${serverUrl.protocol}//${serverUrl.host}`;
                    let oauthConfig;
                    try {
                        ...
                        const oauthMessage = `OAuth discovery failed for server '${mcpServerName}'. ` +
                            getMcpOAuthDialogInstruction('authenticate', mcpServerName) +
                    ...
                    if (oauthConfig) {
                        debugLogger.info(`Discovered OAuth configuration from base URL for server '${mcpServerName}'`);
                        // Create OAuth configuration for authentication
                        const oauthAuthConfig = {
                            enabled: true,
                            authorizationUrl: oauthConfig.authorizationUrl,
                        ...
                        if (credentials) {
                            ...
                            if (accessToken) {
                                ...
                                if (oauthTransport) {
                                    ...
                                    const oauthMessage = `Failed to create OAuth transport for server '${mcpServerName}'. ` +
                                        getMcpOAuthDialogInstruction('authenticate', mcpServerName);
                                    debugLogger.error(oauthMessage);
                                    throw new Error(oauthMessage);
                                }
                                ...
                                const oauthMessage = `Failed to get OAuth token for server '${mcpServerName}'. ` +
                                    getMcpOAuthDialogInstruction('authenticate', mcpServerName);
                                debugLogger.error(oauthMessage);
                                throw new Error(oauthMessage);
                            }
                            ...
                            const oauthMessage = `Failed to get stored credentials for server '${mcpServerName}'. ` +
                                getMcpOAuthDialogInstruction('authenticate', mcpServerName);
                            debugLogger.error(oauthMessage);
                            throw new Error(oauthMessage);
                        }
                        ...
                        debugLogger.error(`Could not configure OAuth for server '${mcpServerName}'. ` +
                            getMcpOAuthDialogInstruction('authenticate', mcpServerName));
                        throw new Error(`OAuth configuration failed for server '${mcpServerName}'. ` +
                            getMcpOAuthDialogInstruction('authenticate', mcpServerName));
                    }
                }
                else {
                    debugLogger.error(`'${mcpServerName}' requires authentication but no OAuth configuration found`);
                    throw new Error(`MCP server '${mcpServerName}' requires authentication. ` +
                        getMcpOAuthDialogInstruction('authenticate', mcpServerName));
                }
            }
        }
...
/** Visible for Testing */
export async function createTransport(mcpServerName, mcpServerConfig, debugMode, sendSdkMcpMessage) {
    if (isSdkMcpServerConfig(mcpServerConfig)) {
        if (!sendSdkMcpMessage) {
            throw new Error(`SDK MCP server '${mcpServerName}' requires sendSdkMcpMessage callback`);
        }
    ...
    if (hasOAuthConfig && mcpServerConfig.oauth) {
        ...
        if (!accessToken) {
            debugLogger.error(`The MCP server '${mcpServerName}' requires OAuth authentication. ` +
                getMcpOAuthDialogInstruction('authenticate', mcpServerName));
            throw new Error(`The MCP server '${mcpServerName}' requires OAuth authentication. ` +
                getMcpOAuthDialogInstruction('authenticate', mcpServerName));
        }
    }
    else {
        ...
        if (credentials) {
            ...
            if (accessToken) {
                hasOAuthConfig = true;
                debugLogger.debug(`Found stored OAuth token for server '${mcpServerName}'`);
            }
            else {
                debugLogger.warn(`Stored OAuth credentials exist for server '${mcpServerName}' but no valid token could be obtained. Transport will be created without authentication; expect a 401. ` +
                    getMcpOAuthDialogInstruction('re-authenticate', mcpServerName));
            }
        }
    }
    ...
    if (mcpServerConfig.command) {
        ...
        if (mcpServerConfig.cwd && !existsSync(mcpServerConfig.cwd)) {
            throw new Error(`MCP server '${mcpServerName}': configured cwd does not exist: ${mcpServerConfig.cwd}`);
        }
