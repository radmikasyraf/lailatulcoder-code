...
export async function pickNativeDirectory(signal) {
    try {
        if (process.platform === 'darwin') {
            ...
            const { stdout } = await execFileAsync('osascript', ['-l', 'JavaScript', '-e', script], { timeout: PICKER_TIMEOUT_MS, signal });
        ...
        if (process.platform === 'win32') {
            ...
            const { stdout } = await execFileAsync('powershell.exe', ['-NoProfile', '-STA', '-Command', script], { timeout: PICKER_TIMEOUT_MS, signal });
            return stdout.trim() || undefined;
        }
        ...
        throw new NativeDirectoryPickerUnavailableError(error instanceof Error ? error.message : String(error));
    }
    throw new NativeDirectoryPickerUnavailableError(`Native directory picker is not supported on ${process.platform}`);
}
//# sourceMappingURL=native-directory-picker.js.map