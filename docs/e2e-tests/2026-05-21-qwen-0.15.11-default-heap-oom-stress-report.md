# LailatulCoder Ai 0.15.11 默认 Heap OOM 压测报告

日期：2026-05-21

## 测试范围

本报告记录了针对 LailatulCoder Ai `0.15.11` 最新本地构建的一轮默认 heap 压测。
...
每个模型 worker 都复用同一个 session，不断 `--resume`。每一轮要求 LailatulCoder Ai：
...
stdout 是否还会把 LailatulCoder Ai 推入危险内存路径。
...
`942.5 MiB` 到 `1006.6 MiB`。这些数据主要说明：当时 LailatulCoder Ai 的任务期
...
等于 LailatulCoder Ai 已经没有任何 large-output memory 风险。
...
4. 仍然存在一个独立的极端 large-output 问题：`2048 MiB` stdout 附近，LailatulCoder Ai
