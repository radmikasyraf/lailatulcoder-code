 ...
 */
export async function fetchGitBranches(cwd, env) {
    ...
    const [localRaw, remoteRaw, tagsRaw, headRaw, reflogRaw] = await Promise.all([
        ...
        runGit(cwd, ['reflog', 'show', '--format=%gs', `-${MAX_REFLOG_ENTRIES}`], env).catch(() => ''),
    ]);
...
function parseRecentBranches(reflogRaw, currentHead) {
    if (!reflogRaw.trim())
        return [];
    const seen = new Set();
    const result = [];
    for (const line of reflogRaw.trim().split('\n')) {
        ...
        if (!line.startsWith('checkout: moving from '))
            continue;
        ...
        if (branch &&
            ...
            seen.add(branch);
            result.push(branch);
            if (result.length >= MAX_RECENT_BRANCHES)
                break;
        }
    }
    return result;
}
async function getDetachedHead(cwd, env) {
    try {
 ...
 */
export function isValidCheckoutRef(value) {
 ...
 */
export async function gitCheckout(cwd, ref, env) {
    ...
    const isRemoteTracking = await runGit(cwd, ['show-ref', '--verify', '--quiet', `refs/remotes/${ref}`], env)
        .then(() => true)
        .catch(() => false);
    if (isRemoteTracking) {
        const localName = ref.slice(ref.indexOf('/') + 1);
        if (!isValidCheckoutRef(localName)) {
            throw new Error(`invalid local branch name derived from remote ref: ${localName}`);
        }
        const hasLocal = await runGit(cwd, ['show-ref', '--verify', '--quiet', `refs/heads/${localName}`], env)
            .then(() => true)
            .catch(() => false);
        if (hasLocal) {
        ...
        const head = (await runGit(cwd, ['symbolic-ref', '--short', 'HEAD'], env)).trim();
    ...
    const headRaw = await runGit(cwd, ['symbolic-ref', '--short', 'HEAD'], env).catch(() => '');
 ...
 */
export async function gitCreateBranch(cwd, name, startPoint, env) {
    ...
    const originalRef = (await runGit(cwd, ['symbolic-ref', '--quiet', '--short', 'HEAD'], env).catch(() => '')).trim();
    ...
    try {
        ...
        const nowOn = (await runGit(cwd, ['symbolic-ref', '--quiet', '--short', 'HEAD'], env).catch(() => '')).trim();
        if (nowOn === name) {
            if (originalRef) {
                await runGit(cwd, ['checkout', originalRef, '--'], env).catch(() => { });
            }
            else if (originalCommit) {
                await runGit(cwd, ['checkout', '--detach', originalCommit, '--'], env).catch(() => { });
            }
 ...
 */
export async function gitPush(cwd, opts, env) {
    const args = ['push'];
    if (opts?.force)
    ...
    if (opts?.setUpstream) {
        let branch;
        try {
            branch = (await runGit(cwd, ['symbolic-ref', '--short', 'HEAD'], env)).trim();
        }
        catch {
            throw new Error('cannot push with --set-upstream in detached HEAD state; check out a branch first');
        }
        // If the branch already tracks an upstream, push without rewriting it.
        const hasUpstream = await runGit(cwd, ['rev-parse', '--abbrev-ref', `${branch}@{u}`], env).catch(() => '');
        if (hasUpstream.trim()) {
        ...
        let remote = (await runGit(cwd, ['config', `branch.${branch}.pushRemote`], env).catch(() => '')).trim();
        if (!remote) {
            remote = (await runGit(cwd, ['config', 'remote.pushDefault'], env).catch(() => '')).trim();
        }
        if (!remote) {
            remote = (await runGit(cwd, ['config', `branch.${branch}.remote`], env).catch(() => '')).trim();
        }
        if (!remote) {
            const remotes = (await runGit(cwd, ['remote'], env).catch(() => '')).trim();
 ...
 */
export async function gitPull(cwd, opts, env) {
    if (opts?.fetchOnly) {
 ...
 */
export async function gitCommit(cwd, message, opts, env) {
    ...
    let savedIndex = null;
    if (opts?.all) {
        const tree = (await runGit(cwd, ['write-tree'], env).catch(() => '')).trim();
        if (tree) {
            savedIndex = tree;
        }
        else {
            ...
            const unmerged = (await runGit(cwd, ['ls-files', '--unmerged'], env)).trim();
            if (unmerged) {
                throw new Error('cannot stage all changes: unresolved merge conflicts in the index');
            }
            throw new Error('cannot stage all changes: failed to snapshot index (write-tree failed)');
        }
    }
    try {
        if (opts?.all) {
