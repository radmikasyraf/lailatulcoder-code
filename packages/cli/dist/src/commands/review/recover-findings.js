...
import { readRunTranscripts, TranscriptsUnavailableError, } from './lib/transcripts.js';
import { promptRecordDir, readRecordedPrompts, deliveredVerbatimLines, flattenPrompt, promptLines, findingsPointerOf, } from './lib/prompt-record.js';
import { priorSessionIds } from './lib/run-ledger.js';
import { assignedChunk, pointedAt } from './lib/coverage.js';
import { chunkOfKey, declaresOwnUncoverable, openedBrief, readFindingsPointer, } from './lib/certification.js';
 ...
 * them).
 */
function meetsBar(rec, planPath, key, builtPrompt, plan) {
    ...
    if (declaresOwnUncoverable(rec, assignedChunk(rec)))
        return false;
    ...
    if (!openedBrief(rec, planPath, key))
        return false;
    if (/^chunk-\d+$/.test(key)) {
    ...
    const chunklessFloor = pointedAt(builtPrompt, plan).length === 0 || rec.diffToolCalls > 0;
    ...
    if (!readFindingsPointer(rec, pointer))
        return false;
    return chunk !== null ? rec.diffToolCalls > 0 : chunklessFloor;
}
export function recoverFindings(args, env = process.env) {
    ...
    const plan = planRaw;
    const planChunks = {
        chunks: (Array.isArray(plan.chunks) ? plan.chunks : []).filter((c) => typeof c === 'object' &&
            c !== null &&
            typeof c.id === 'number' &&
            typeof c.startLine === 'number' &&
            typeof c.endLine === 'number'),
    };
    const diffPath = typeof plan.diffPathAbsolute === 'string' && plan.diffPathAbsolute !== ''
    ...
    for (const rec of records) {
        ...
        for (const [key, lines] of builtLines) {
            if (deliveredVerbatimLines(launch, lines))
                keys.push(key);
        }
    ...
    const recoveredMtime = new Map();
    for (const [rec, keys] of matchesOf) {
        if (keys.length !== 1)
            continue; // unmatched, or the injectivity refusal
        const key = keys[0];
        if (!meetsBar(rec, planPath, key, built.get(key) ?? '', planChunks)) {
            continue;
        }
        if (rec.finalText.trim() === '')
            continue;
    ...
    try {
        ...
        const code = err?.code;
        if (code !== 'ENOENT') {
            recordDirUnreadable = code ?? err.message;
        }
    }
    for (const name of names) {
        ...
        if (!corroboratedFindings.has(resolve(path)))
            continue;
    ...
    let latestReverseAuditRound = null;
    for (const key of recoveredKeys) {
        if (!key.startsWith('reverse-audit'))
            continue;
        const m = ROUND_IN_KEY_RE.exec(key);
        if (!m)
            continue;
    ...
    const sections = [
        ...
        '',
        'Written by `LailatulCoder review recover-findings` from the harness transcripts',
...
export const recoverFindingsCommand = {
    command: 'recover-findings',
    describe: 'Recover the certified agent results of an interrupted review run from the harness transcripts, for a resumed run to read back',
    builder: (yargs) => yargs
        .option('plan', {
        type: 'string',
        demandOption: true,
        ...
        type: 'string',
        demandOption: true,
        describe: 'Where to write the recovered final texts (Markdown, one section per certified agent)',
    })
        .version(false),
    handler: (argv) => {
        try {
            const result = recoverFindings(argv);
            writeStdoutLine(JSON.stringify(result));
            writeStderrLine(`recover-findings: ${result.recoveredKeys.length} agent result(s) recovered, ` +
                `${result.missingKeys.length} still owed; wrote ${result.out}`);
            if (result.recordDirUnreadable !== null) {
                writeStderrLine(`WARNING: the prompt-record directory could not be read ` +
...
//# sourceMappingURL=recover-findings.js.map