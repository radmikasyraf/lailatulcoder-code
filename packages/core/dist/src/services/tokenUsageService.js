...
function normalizePeriodValue(period, value) {
    ...
    if (!isValid) {
        throw new Error(`Invalid ${period} value "${normalized}". Expected ${period === 'day' ? 'YYYY-MM-DD' : 'YYYY-MM'}.`);
    }
    return normalized;
}
function usageDir() {
...
export function getTokenUsageFilePath(month) {
...
function calculateTotalTokens(event) {
    ...
    return (calculateInputTokens(event) +
        toNonNegativeInteger(event.output_token_count) +
        toNonNegativeInteger(event.thoughts_token_count));
}
export function apiResponseEventToTokenUsageRecord(config, event) {
    const timestamp = event['event.timestamp'] || new Date().toISOString();
    const date = new Date(timestamp);
    const localParts = getLocalDateParts(Number.isNaN(date.getTime()) ? new Date() : date);
    return {
...
async function readRecordsForMonth(month) {
    ...
    if (dropped > 0) {
        debugLogger.warn(`Dropped ${dropped}/${records.length} invalid record(s) from ${filePath}`);
    }
    return valid;
}
function summarizeRecords(period, value, records) {
    ...
    const byAuthType = new Map();
    const byModelAndAuthType = new Map();
    const bySource = new Map();
    const getGroup = (map, key, fields) => {
    ...
    for (const record of records) {
        ...
        addRecordToTotals(getGroup(byModel, record.model, { model: record.model }), record);
        addRecordToTotals(getGroup(byAuthType, record.authType, { authType: record.authType }), record);
        addRecordToTotals(getGroup(byModelAndAuthType, `${record.model}|${record.authType}`, {
            model: record.model,
            authType: record.authType,
        }), record);
        addRecordToTotals(getGroup(bySource, record.source, { source: record.source }), record);
    }
...
export async function recordTokenUsageFromApiResponse(config, event) {
...
const lastLoggedTimeByCode = new Map();
const suppressedCountByCode = new Map();
const TOKEN_USAGE_FAILURE_LOG_COOLDOWN_MS = 60_000;
let _now = () => Date.now();
/** @internal Override the time source for testing cooldown behavior. */
export function __overrideNowForTesting(fn) {
    _now = fn;
}
function logTokenUsageWriteFailure(error) {
    ...
    if (code && code !== 'ENOENT') {
        ...
        if (now - lastTime > TOKEN_USAGE_FAILURE_LOG_COOLDOWN_MS) {
            ...
            console.error(`[token-usage] Write failed (${code}):`, suppressedCount > 0
...
export function resetTokenUsageFailureLogging() {
    lastLoggedTimeByCode.clear();
    suppressedCountByCode.clear();
    _now = () => Date.now();
}
export function recordTokenUsageFromApiResponseBestEffort(config, event) {
    try {
        const record = apiResponseEventToTokenUsageRecord(config, event);
        void jsonl
...
export async function queryTokenUsage(query) {
    const value = normalizePeriodValue(query.period, query.value);
    const month = query.period === 'day' ? value.slice(0, 7) : value;
    const records = (await readRecordsForMonth(month)).filter((record) => query.period === 'day'
...
export function formatTokenUsageSummaryAsCsv(summary) {
    const header = [
        'period',
        'value',
        'group_type',
    ...
    const rows = [
        ...
        ...groupRows(summary.period, summary.value, 'auth_type', summary.byAuthType),
        ...groupRows(summary.period, summary.value, 'model_auth_type', summary.byModelAndAuthType),
...
export function formatTokenUsageSummaryAsJson(summary) {
    return `${JSON.stringify(summary, null, 2)}\n`;
}
export async function exportTokenUsageSummary(options) {
...
//# sourceMappingURL=tokenUsageService.js.map