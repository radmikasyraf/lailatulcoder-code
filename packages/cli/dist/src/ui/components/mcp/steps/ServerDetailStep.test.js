...
describe('ServerDetailStep — View resources action gating', () => {
    ...
    it('shows "View resources" when a handler is wired and the server has resources', () => {
        const { lastFrame } = render(_jsx(ServerDetailStep, { server: server({ resourceCount: 2 }), onViewTools: vi.fn(), onViewResources: vi.fn(), onBack: vi.fn() }));
    ...
    it('hides "View resources" when no handler is wired, even with resources', () => {
        const { lastFrame } = render(_jsx(ServerDetailStep, { server: server({ resourceCount: 2 }), onViewTools: vi.fn(), onBack: vi.fn() }));
        expect(lastFrame()).not.toContain('View resources');
    });
    it('hides "View resources" when the server has no resources', () => {
        const { lastFrame } = render(_jsx(ServerDetailStep, { server: server({ resourceCount: 0 }), onViewTools: vi.fn(), onViewResources: vi.fn(), onBack: vi.fn() }));
        expect(lastFrame()).not.toContain('View resources');
    });
    it('hides "View resources" when the server is disabled', () => {
        const { lastFrame } = render(_jsx(ServerDetailStep, { server: server({ resourceCount: 2, isDisabled: true }), onViewTools: vi.fn(), onViewResources: vi.fn(), onBack: vi.fn() }));
        expect(lastFrame()).not.toContain('View resources');
    });
});
//# sourceMappingURL=ServerDetailStep.test.js.map