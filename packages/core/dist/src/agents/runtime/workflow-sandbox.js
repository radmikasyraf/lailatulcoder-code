...
function findMetaBlockBounds(source) {
    ...
    if (depth !== 0) {
        throw new Error('stripExportMeta: unbalanced braces in export const meta declaration — ' +
 ...
 */
export function extractAndStripMeta(source) {
    const bounds = findMetaBlockBounds(source);
    if (!bounds)
        return { stripped: source, meta: null };
    ...
    try {
        ...
        throw new Error(`extractAndStripMeta: failed to evaluate meta object literal: ${msg}`);
    }
 ...
 */
function rejectThenablesInMeta(value, seen = new WeakSet()) {
    ...
    if (typeof maybeThen === 'function') {
        ...
        throw new Error('extractAndStripMeta: meta values must not be Promises ' +
...
class WallClockWatchdog {
    ...
    pause() {
        if (this.stopped || this.paused || this.timer === undefined)
            return;
        this.paused = true;
        ...
        this.remainingMs = Math.max(0, this.remainingMs - (performance.now() - this.armedAt));
    }
    resume() {
 ...
 * stringification.
 */
function validateArgs(val, depth = 0, seen = new WeakSet()) {
    if (depth > ARGS_MAX_DEPTH) {
        throw new Error(`WorkflowSandbox: args exceeded max nesting depth of ${ARGS_MAX_DEPTH}`);
    }
    if (val === null)
        return;
    const t = typeof val;
    if (t === 'function') {
        throw new Error('WorkflowSandbox: args must be JSON-serializable (functions are not allowed).');
    }
    if (t === 'bigint') {
        throw new Error('WorkflowSandbox: args must be JSON-serializable (BigInt is not allowed — pass as string).');
    }
    if (t !== 'object')
        return;
    const obj = val;
    if (seen.has(obj)) {
        throw new Error('WorkflowSandbox: args must be JSON-serializable (circular reference detected).');
    }
    seen.add(obj);
    if (Array.isArray(val)) {
        for (const item of val)
            validateArgs(item, depth + 1, seen);
    }
    else {
...
export function createWorkflowSandbox(opts) {
    const phases = [];
    const logs = [];
    const emitLog = (line) => {
        try {
    ...
    const safePhase = (title) => {
        if (phases.length < MAX_PHASE_ENTRIES) {
            ...
            phases.push(`[workflow phases truncated at ${MAX_PHASE_ENTRIES} entries]`);
        }
    };
    ...
    const unconsumedRejections = new Map();
    ...
    let mirroredEscapeKeys = new Set();
    ...
    const unconsumedRejectionLine = (rec) => {
    ...
    const bridge = {
        ...
        // PR #4947 R2 T7 (lailatul-coder-ci-bot): host-side log hook for reviveInRealm's
        ...
        logRevivalFailure: (idx, reason) => {
            debugLogger.warn(`Workflow result revival failed at index ${idx}: ${reason}; ` +
        ...
        wfMarkRejectionHandled: (rootId) => {
            const root = unconsumedRoots.get(rootId);
            if (root)
                root.rejectionHandled = true;
        },
        wfIsRootHandled: (rootId) => unconsumedRoots.get(rootId)?.rejectionHandled === true,
        wfMarkAdopted: (rootId) => {
            const root = unconsumedRoots.get(rootId);
            if (root)
                root.adoptedOut = true;
        },
        runId: opts.runId ?? '',
        wfReportUnconsumed: (rootId, isRoot, dispatchFailed, msg) => {
        ...
        isRunAborted: () => opts.abortOnTimeout?.signal.aborted === true,
    ...
    const sandboxGlobals = Object.assign(Object.create(null), { __workflowBridge: bridge });
    ...
    })();`, ctx, { filename: 'workflow-sandbox-init.js' });
    ...
    const flushUnconsumedRejections = async () => {
        unconsumedSettled = true;
        if (nextUnconsumedId === 1)
            return;
    ...
    const adoptionEscapeHook = (reason, _promise) => {
        try {
            if (!reason || typeof reason !== 'object')
                return;
            const marked = reason;
            if (marked.__wfDispatchFailed !== true)
                return;
            if (String(marked.__wfRunId ?? '') !== hookRunId)
                return;
            const rootId = typeof marked.__wfRootId === 'number' ? marked.__wfRootId : undefined;
            if (rootId !== undefined &&
    ...
    return {
            ...
            try {
                ...
                const timeoutPromise = new Promise((_, reject) => {
                    watchdog = new WallClockWatchdog(maxWallClockMs, () => {
                        ...
                        reject(new Error(`Workflow execution exceeded ${maxWallClockMs} ms of active time (paused time is not counted). ` +
                ...
                const aborted = () => opts.abortOnTimeout?.signal.aborted === true;
                ...
                rearmWatchdogOnAbort = () => watchdog?.resume();
                opts.abortOnTimeout?.signal.addEventListener('abort', rearmWatchdogOnAbort, { once: true });
                return await Promise.race([result, timeoutPromise]);
            }
            finally {
                if (rearmWatchdogOnAbort) {
                    opts.abortOnTimeout?.signal.removeEventListener('abort', rearmWatchdogOnAbort);
                }
                stopWatchingState?.();
                watchdog?.stop();
        ...
        getPhases: () => [...phases],
        getLogs: () => [...logs],
        appendLog: (line) => safeLog(line, false),
        getMeta: () => extractedMeta,
    };
}
//# sourceMappingURL=workflow-sandbox.js.map