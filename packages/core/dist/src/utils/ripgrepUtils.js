...
export function _resetRipgrepUtilsCachesForTest() {
 ...
 */
export function getBuiltinRipgrep() {
    ...
    return path.join(__dirname, ...Array(levelsUp).fill('..'), 'vendor', 'ripgrep', `${arch}-${platform}`, binaryName);
}
/**
 ...
 */
export async function resolveRipgrep(useBuiltin = true) {
 ...
 */
export async function ensureRipgrepHealthy(selection) {
    if (cachedHealth &&
    ...
    let probeCode = -1;
    try {
        const { stdout, code } = await execCommand(selection.command, ['--version'], {
    ...
    if (!working) {
        throw new Error(`${selection.command} is not a working ripgrep binary (exit ${probeCode}): ${probeOutput.trim() || '(no output)'}`);
    }
}
export async function ensureMacBinarySigned(selection) {
    ...
    const alreadySigned = inspect.stdout
        ?.split('\n')
...
async function resolveHealthyRipgrep(useBuiltin) {
    ...
    try {
        ...
        debugLogger.warn(`Bundled ripgrep at ${selection.command} is unusable (${error}); trying system rg.`);
        let fallback = null;
        try {
 ...
 * @throws {Error} If an error occurs while resolving the ripgrep binary.
 */
export async function canUseRipgrep(useBuiltin = true) {
...
function isRipgrepThreadEagain(stderr) {
    ...
    const mentionsEagain = lower.includes('resource temporarily unavailable') ||
...
function withSingleRipgrepThread(args) {
    ...
    const threadsIndex = args.findIndex((arg, index) => arg === '--threads' && args[index + 1] === '4');
    if (threadsIndex === -1) {
        return null;
    }
...
async function runRipgrepOnce(selection, args, signal) {
    return new Promise((resolve) => {
        ...
        let child;
        try {
            child = execFile(selection.command, args, {
                ...
                const { failureKind, canceled } = classifyRipgrepError(error, stderrText, signal);
                const incomplete = (shouldDropLastLine(failureKind, canceled) ||
                    failureKind === 'eagain' ||
                    failureKind === 'exit') &&
                ...
                if (failureKind === 'timeout' || failureKind === 'max_buffer') {
                    debugLogger.warn(`ripgrep exited abnormally (signal=${error.signal} code=${error.code}) with stderr:\n${stderrText.trim() || '(empty)'}`);
                }
                settle({
 ...
 */
export async function runRipgrep(args, signal, useBuiltin = true) {
    const selection = await resolveHealthyRipgrep(useBuiltin);
    if (!selection) {
        throw new Error('ripgrep not found.');
    }
    const firstAttempt = await runRipgrepOnce(selection, args, signal);
    if (firstAttempt.failureKind === 'eagain' &&
        ...
        if (retryArgs !== null) {
            ...
            const retryFailureKind = retryAttempt.error === undefined ? 'eagain' : retryAttempt.failureKind;
            if (retryFailureKind !== undefined) {
                retryRecoveryOptions.failureKind = retryFailureKind;
            }
            return toRunResult(retryAttempt, createRecoveryMetadata(selection, retryRecoveryOptions));
        }
    }
    ...
    return toRunResult(firstAttempt, createRecoveryMetadata(selection, recoveryOptions));
}
