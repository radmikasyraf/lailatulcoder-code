...
function claimedExtension(basename) {
...
export function sniffImageFormat(header) {
    const at = (i) => header[i] ?? -1;
    const ascii = (i, s) => [...s].every((ch, k) => at(i + k) === ch.charCodeAt(0));
    if (at(0) === 0x89 &&
        ascii(1, 'PNG') &&
        at(4) === 0x0d &&
        at(5) === 0x0a &&
...
function refusedExtension(basename, ext) {
    return {
        ok: false,
        reason: `${basename}: extension ${JSON.stringify(ext)} is not an allowed ` +
 ...
 */
export function validateAssetContent(basename, header) {
    ...
    if (actual !== claimed) {
        ...
        return {
            ok: false,
            reason: `content is ${actual ?? 'not a recognized image'} but the ` +
 ...
 * collide. */
export function assetsBranch(pr) {
 ...
 */
export function remoteAssetPath(pr, basename, sha256Hex) {
...
export function parseAssetsRepo(value) {
    const v = (value ?? '').trim();
    if (v === '') {
        return {
            error: 'QWEN_REVIEW_ASSETS_REPO is not set. Designate an assets repository ' +
    ...
    const parts = v.split('/');
    const segmentOk = (p) => /^[A-Za-z0-9_.-]+$/.test(p) && p !== '.' && p !== '..';
...
export function validateAssetFile(basename, bytes) {
    const ext = claimedExtension(basename);
    ...
    if (bytes > MAX_ASSET_BYTES) {
        return {
            ok: false,
            reason: `${basename}: ${bytes} bytes exceeds the per-file cap of ` +
 ...
 */
export function validateAssetBatch(files) {
    let total = 0;
