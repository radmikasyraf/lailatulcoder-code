...
import { AGENT_WORKTREE_SLUG_PATTERN, GitWorktreeService, worktreeBranchForSlug, } from './gitWorktreeService.js';
...
/**
 * Removes stale ephemeral worktrees under `<projectRoot>/.lailatulcoder/worktrees/`.
 *
 ...
 */
export async function cleanupStaleAgentWorktrees(projectRoot, options = {}) {
    ...
    for (const entry of entries) {
        ...
        try {
            ...
            if (error.code !== 'ENOENT') {
                debugLogger.warn(`cleanupStaleAgentWorktrees: cannot stat ${worktreePath} — skipping: ${error}`);
            }
            continue;
        }
        if (mtimeMs >= cutoffDate)
            continue;
        ...
        if (!result.success) {
            debugLogger.warn(`Failed to remove stale agent worktree ${worktreePath}: ${result.error}`);
            continue;
        }
        if (result.branchPreserved) {
            ...
            debugLogger.warn(`Removed stale agent worktree ${worktreePath} but kept branch ` +
    ...
    if (removed > 0) {
        debugLogger.debug(`cleanupStaleAgentWorktrees: removed ${removed} stale worktree(s)`);
    }
    return removed;
}
async function hasTrackedChanges(worktreePath) {
    try {
        ...
        debugLogger.warn(`hasTrackedChanges: cannot inspect ${worktreePath} — assuming dirty: ${error}`);
        return true;
    }
}
export const __test__ = { isEphemeralSlug };
//# sourceMappingURL=worktreeCleanup.js.map