...
import { buildAcpModelOptions, getCurrentAcpModelId, isInlineModelOverrideAllowed, parseAcpBaseModelId, parseAcpModelOption, resolveAcpModelOption, sanitizeProviderBaseUrl, } from './acpModelUtils.js';
describe('acpModelUtils', () => {
    it('uses opaque ids only to disambiguate colliding model routes', () => {
        ...
        expect(options.map((option) => option.modelId).join(' ')).not.toContain('secret');
        expect(resolveAcpModelOption(second.modelId, models)).toMatchObject({
            modelId: 'shared-model',
            authType: AuthType.USE_OPENAI,
            baseUrl: 'https://user:secret@two.example/v1?token=value',
        });
        expect(getCurrentAcpModelId(options, 'shared-model', AuthType.USE_OPENAI, 'https://user:secret@two.example/v1?token=value')).toBe(second?.modelId);
        expect(buildAcpModelOptions([...models].reverse()).find((option) => option.model.baseUrl ===
            'https://user:secret@two.example/v1?token=value')?.modelId).toBe(second?.modelId);
    });
    it('keeps opaque ids unique when colliding routes have identical metadata', () => {
        ...
        expect(resolveAcpModelOption(options[0].modelId, models)?.baseUrl).toBe('https://one.example/v1');
        expect(resolveAcpModelOption(options[1].modelId, models)?.baseUrl).toBe('https://two.example/v1');
        const reversed = buildAcpModelOptions([...models].reverse());
        expect(reversed.find((option) => option.model.baseUrl === 'https://one.example/v1')?.modelId).toBe(options[0]?.modelId);
    });
    it('binds opaque ids to credential-free endpoint identity', () => {
        const makeModels = (suffix) => [
            {
                id: 'shared-model',
                label: 'Provider One',
                authType: AuthType.USE_OPENAI,
        ...
        expect(buildAcpModelOptions(makeModels('first')).map((option) => option.modelId)).not.toEqual(buildAcpModelOptions(makeModels('changed')).map((option) => option.modelId));
        ...
        expect(buildAcpModelOptions(withSecrets).map((option) => option.modelId)).toEqual(buildAcpModelOptions(makeModels('first')).map((option) => option.modelId));
    });
    it('rejects colliding routes that differ only by secret URL parts', () => {
        ...
        expect(() => buildAcpModelOptions(models)).toThrow('need distinct names, envKey values, or public endpoints');
    });
    it('keeps resolved defaults separate from the registry route key', () => {
        ...
        expect(resolveAcpModelOption(options[0].modelId, models)).not.toHaveProperty('baseUrl');
        expect(getCurrentAcpModelId(options, 'shared-model', AuthType.USE_OPENAI, null)).toBe(options[0]?.modelId);
        expect(getCurrentAcpModelId(options, 'shared-model', AuthType.USE_OPENAI, 'https://default.example/v1')).toBe(options[1]?.modelId);
        expect(getCurrentAcpModelId(options, 'shared-model', AuthType.USE_OPENAI, 'https://outside.example/v1')).toBe('shared-model');
    });
    ...
    describe('isInlineModelOverrideAllowed', () => {
        const makeConfig = (contentGeneratorConfig, available) => ({
...
//# sourceMappingURL=acpModelUtils.test.js.map