...
import { inertPath, tmpFile, probeWorktreePath, scratchLabel, scratchWorktreePath, scratchWorktreePrefix, worktreePath, PARSE_ARGS_REPORT, } from './paths.js';
...
describe('scratchWorktreePath', () => {
    ...
    it('names the tree after the agent that owns it', () => {
        ...
        expect(scratchWorktreePath(worktree, 'verify--round-2--abc')).toBe(`${worktree}-scratch-verify--round-2--abc`);
        expect(scratchWorktreePath(worktree, 'verify--round-2--abc')).not.toBe(scratchWorktreePath(worktree, 'verify--round-2--def'));
    });
    it('resolves a relative worktree so the tree is a SIBLING, not a child', () => {
        ...
        expect(scratchWorktreePath('.qwen/tmp/review-pr-1', 'verify')).toBe(`${resolve('.qwen/tmp/review-pr-1')}-scratch-verify`);
    });
    ...
    it('caps a long label — the suffix rides on an already-deep path', () => {
        ...
        expect(basename(p).length).toBeLessThanOrEqual('review-pr-1-scratch-'.length + 64);
    });
...
describe('scratchWorktreePrefix', () => {
    it('is the exact infix cleanup sweeps on, not merely a prefix of the path', () => {
        ...
        expect(scratchWorktreePrefix(worktreePath(7))).toBe(`${resolve(worktreePath(7))}-scratch-`);
        expect(scratchWorktreePath(worktreePath(70), 'x').startsWith(scratchWorktreePrefix(worktreePath(7)))).toBe(false);
    });
});
describe('inertPath', () => {
    ...
    it('leaves an ordinary path exactly as it is', () => {
        ...
        expect(inertPath('packages/cli/src/a-b_c.test.ts')).toBe('packages/cli/src/a-b_c.test.ts');
