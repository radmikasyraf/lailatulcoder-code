 ...
 */
export function convertSchema(schema, mode = 'auto') {
    if (mode === 'openapi_30') {
...
function toOpenAPI30(schema) {
    const convert = (obj) => {
        ...
        else if (typeof source['items'] === 'object' &&
            source['items'] !== null) {
 ...
 */
export function relaxSchemaForFunctionCalling(schema) {
    const relax = (obj) => {
        if (typeof obj !== 'object' || obj === null) {
            return obj;
        }
        ...
        const required = Array.isArray(source['required'])
            ? source['required'].filter((r) => typeof r === 'string')
            : [];
        const hasOptionalProperties = typeof properties === 'object' &&
            properties !== null &&
