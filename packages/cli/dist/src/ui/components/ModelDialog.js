...
import { AuthType, ModelSlashCommandEvent, logModelSlashCommand, MAINLINE_CODER_MODEL, isImageCapable, parseVisionModelSetting, resolveModelId, } from '@lailatul-coder/lailatul-coder-core';
 ...
 */
export function encodeAuxModelSelector(selected) {
...
function encodeVisionModelSelector(selected) {
    if (!selected.includes('::')) {
...
function persistAuthTypeSelection(settings, authType, persistScope) {
    const scope = resolvePersistScope(settings, persistScope);
    settings.setValue(scope, 'security.auth.selectedType', authType);
}
function hydrateApiKeyEnvFromSettings(settings, envKey) {
    if (!envKey || process.env[envKey]) {
        return;
    }
    const settingsEnvValue = settings?.merged?.env?.[envKey];
    if (typeof settingsEnvValue === 'string' &&
        settingsEnvValue.trim().length > 0) {
        process.env[envKey] = settingsEnvValue;
    }
}
function handleModelSwitchSuccess({ config, settings, uiState, after, effectiveAuthType, effectiveModelId, effectiveBaseUrl, isRuntime, persistScope, }) {
    persistModelSelection(settings, effectiveModelId, effectiveBaseUrl, persistScope);
    if (effectiveAuthType) {
        persistAuthTypeSelection(settings, effectiveAuthType, persistScope);
    }
    const baseUrl = after?.baseUrl ?? t('(default)');
    const maskedKey = maskApiKey(after?.apiKey);
    const scopeSuffix = persistScope === 'workspace'
        ? t(' (this project)')
        : persistScope === 'user'
            ? t(' (global)')
            : '';
...
export function ModelDialog({ onClose, isFastModelMode, isVoiceModelMode, isVisionModelMode, isCompactionModelMode, isImageModelMode, persistScope, availableTerminalHeight, }) {
    ...
    const availableModelEntries = useMemo(() => {
        ...
        const registryModels = allModels.filter((m) => {
            const imageModelSelector = encodeVisionModelSelector(buildModelSelectionKey(m.authType, m.id, m.baseUrl));
            const isSelectableImageModel = isImageModelMode
                ? m.imageOnly === true &&
                    config?.resolveImageGenerationModel(imageModelSelector) !== undefined
                : m.imageOnly !== true;
            return (!m.isRuntimeModel &&
                (m.authType !== AuthType.NEVER_MATCH_THIS ||
                    authType === AuthType.NEVER_MATCH_THIS) &&
        ...
        // Fixed order: LailatulCoder-oauth first, then others in a stable order
        const authTypeOrder = [
    ...
    const MODEL_OPTIONS = useMemo(() => availableModelEntries.map(({ authType: t2, model, isRuntime, snapshotId }) => {
        ...
        const isQwenOAuth = t2 === AuthType.NEVER_MATCH_THIS;
        const title = (_jsxs(Text, { children: [_jsxs(Text, { bold: true, color: isQwenOAuth
                        ? theme.status.warning
                        : isRuntime
                            ? theme.status.warning
                            : theme.text.accent, children: ["[", t2, "]"] }), _jsx(Text, { children: ` ${model.label}` }), model.id !== model.label && (_jsxs(Text, { color: theme.text.secondary, italic: true, children: [' ', "(", model.id, ")"] })), isRuntime && (_jsx(Text, { color: theme.status.warning, children: " (Runtime)" })), isQwenOAuth && !isRuntime && (_jsxs(Text, { color: theme.status.warning, children: [" (", t('Discontinued'), ")"] }))] }));
    ...
    const modelOptionRowHeight = MODEL_OPTIONS.some(({ description }) => typeof description !== 'string' || description.trim().length > 0)
    ...
    const errorMessageRows = errorMessage
        ...
        : 0;
    const maxModelItemsToShow = availableTerminalHeight === undefined
        ? MAX_MODEL_ITEMS_TO_SHOW
        : Math.max(1, Math.min(MAX_MODEL_ITEMS_TO_SHOW, Math.floor((availableTerminalHeight -
    ...
    const voiceModelSetting = settings?.merged?.voiceModel;
    const visionModelSetting = settings?.merged?.visionModel;
    const imageModelSetting = settings?.merged?.imageModel;
    ...
    const isAuxiliaryModelMode = isFastModelMode ||
        isVoiceModelMode ||
        isVisionModelMode ||
    ...
    const preferredFastModelEntry = isFastModelMode && parsedFastModelSetting
        ...
        : undefined;
    const preferredVoiceModelEntry = isVoiceModelMode && voiceModelSetting
        ? availableModelEntries.find(({ model }) => model.id === voiceModelSetting)
        : undefined;
    ...
    const matchesVisionModelBaseUrl = (model) => !parsedVisionModelValue?.baseUrl ||
        model.baseUrl === parsedVisionModelValue.baseUrl;
    const preferredVisionModelEntry = isVisionModelMode && parsedVisionModelSetting
        ? parsedVisionModelSetting.authType
            ? availableModelEntries.find(({ authType: t2, model }) => t2 === parsedVisionModelSetting.authType &&
                model.id === parsedVisionModelSetting.modelId &&
                matchesVisionModelBaseUrl(model))
            : availableModelEntries.find(({ model }) => model.id === parsedVisionModelSetting.modelId &&
                matchesVisionModelBaseUrl(model))
        : undefined;
    const preferredImageModelEntry = isImageModelMode && parsedImageModelSetting
        ? parsedImageModelSetting.authType
            ? availableModelEntries.find(({ authType: t2, model }) => t2 === parsedImageModelSetting.authType &&
                model.id === parsedImageModelSetting.modelId &&
                (!parsedImageModelValue?.baseUrl ||
                    model.baseUrl === parsedImageModelValue.baseUrl))
            : availableModelEntries.find(({ model }) => model.id === parsedImageModelSetting.modelId &&
                (!parsedImageModelValue?.baseUrl ||
                    model.baseUrl === parsedImageModelValue.baseUrl))
        : undefined;
    ...
    const preferredCompactionModelEntry = isCompactionModelMode && parsedCompactionSetting
        ? parsedCompactionSetting.authType
            ? availableModelEntries.find(({ authType: t2, model }) => t2 === parsedCompactionSetting.authType &&
                model.id === parsedCompactionSetting.modelId)
            : availableModelEntries.find(({ model }) => model.id === parsedCompactionSetting.modelId)
        : undefined;
    const preferredKey = activeRuntimeSnapshot
        ? activeRuntimeSnapshot.id
        : preferredVoiceModelEntry
            ? buildModelSelectionKey(preferredVoiceModelEntry.authType, preferredVoiceModelEntry.model.id, preferredVoiceModelEntry.model.baseUrl)
            : preferredVisionModelEntry
                ? buildModelSelectionKey(preferredVisionModelEntry.authType, preferredVisionModelEntry.model.id, preferredVisionModelEntry.model.baseUrl)
                : preferredCompactionModelEntry
                    ? buildModelSelectionKey(preferredCompactionModelEntry.authType, preferredCompactionModelEntry.model.id, preferredCompactionModelEntry.model.baseUrl)
                    : preferredImageModelEntry
                        ? buildModelSelectionKey(preferredImageModelEntry.authType, preferredImageModelEntry.model.id, preferredImageModelEntry.model.baseUrl)
    ...
    const reportAuxiliaryModelSelection = useCallback((feedbackItem) => {
        uiState?.historyManager.addItem(feedbackItem, Date.now());
        config?.getChatRecordingService?.()?.recordSlashCommand({
            phase: 'result',
            rawCommand: '/model',
    ...
    const closeWithoutSelection = useCallback(() => {
        if (closeLatchRef.current || selectionInFlightRef.current)
            return;
    ...
    const handleSelect = useCallback(async (selected) => {
        if (selectionInFlightRef.current || selectionCommittedRef.current)
            return;
        setErrorMessage(null);
        const selectedEntry = availableModelEntries.find(({ authType: t2, model, isRuntime, snapshotId }) => {
            const value = isRuntime && snapshotId
                ? snapshotId
                : buildModelSelectionKey(t2, model.id, model.baseUrl);
        ...
        if (isVoiceModelMode) {
            ...
            const matchingEntries = availableModelEntries.filter(({ model }) => model.id === voiceModel);
            if (matchingEntries.length > 1) {
                setErrorMessage(t("Voice model '{{model}}' is configured more than once. Remove duplicate model ids before selecting it for voice transcription.", { model: voiceModel }));
                return;
            }
            const scope = resolvePersistScope(settings, persistScope);
        ...
        if (isVisionModelMode) {
            ...
            const visionModelDisplay = parseVisionModelSetting(visionModel)?.selector ?? visionModel;
            ...
            if (selectedEntry &&
                ...
                setErrorMessage(t("'{{model}}' is the current primary model and cannot be used as the vision bridge.", { model: visionModelDisplay }));
                return;
            }
            const scope = resolvePersistScope(settings, persistScope);
            ...
            const visionWarning = selectedEntry && !isImageCapable(selectedEntry.model)
        ...
        if (isImageModelMode) {
            ...
            const imageModelDisplay = parseVisionModelSetting(imageModel)?.selector ?? imageModel;
            if (!config.resolveImageGenerationModel(imageModel)) {
                setErrorMessage(t("'{{model}}' must declare a valid HTTPS baseUrl and credential environment variable.", { model: imageModelDisplay }));
                return;
            }
            const scope = resolvePersistScope(settings, persistScope);
        ...
        // Block selection of discontinued LailatulCoder-oauth models
        ...
        const isQwenOAuthSelection = selected.startsWith(`${AuthType.NEVER_MATCH_THIS}::`) ||
            (selected.startsWith('$runtime|') &&
                selected.split('|')[1] === AuthType.NEVER_MATCH_THIS);
        const isRuntimeOAuthSelection = selected.startsWith(`$runtime|${AuthType.NEVER_MATCH_THIS}|`);
        if (isQwenOAuthSelection && !isRuntimeOAuthSelection) {
            setErrorMessage(t('LailatulCoder Auth free tier was discontinued on 2026-04-15. Please select a model from another provider or run /auth to switch.'));
            return;
        }
        let after;
        ...
        try {
            ...
            setErrorMessage(`${t('Model switched, but the selection could not be saved.')}\n\n${errorMessage}`);
            return;
        }
        closeLatchRef.current = true;
        onClose();
    ...
    return (_jsxs(Box, { borderStyle: "round", borderColor: theme.border.default, flexDirection: "column", padding: 1, width: "100%", children: [_jsx(Text, { bold: true, children: (isVoiceModelMode
                    ...
                    (persistScope === 'workspace'
                        ...
                        : persistScope === 'user'
                            ? t(' (global)')
                            : '') }), !hasModels ? (_jsxs(Box, { marginTop: 1, flexDirection: "column", children: [_jsx(Text, { color: theme.status.warning, children: t('No models available for the current authentication type ({{authType}}).', {
                            authType: authType ? String(authType) : t('(none)'),
                        }) }), _jsx(Box, { marginTop: 1, children: _jsx(Text, { color: theme.text.secondary, children: t('Please configure models in settings.modelProviders or use environment variables.') }) })] })) : (_jsx(Box, { marginTop: 1, children: _jsx(DescriptiveRadioButtonSelect, { items: MODEL_OPTIONS, onSelect: handleSelect, onHighlight: handleHighlight, initialIndex: initialIndex, showNumbers: true, maxItemsToShow: maxModelItemsToShow }) })), highlightedEntry && (_jsxs(Box, { marginTop: 1, flexDirection: "column", children: [_jsx(Box, { borderStyle: "single", borderTop: true, borderBottom: false, borderLeft: false, borderRight: false, borderColor: theme.border.default }), highlightedEntry.authType === AuthType.NEVER_MATCH_THIS &&
                        !highlightedEntry.isRuntime && (_jsx(Box, { marginTop: 1, children: _jsxs(Text, { color: theme.status.warning, children: ["\u26A0 ", t('Discontinued — switch to Coding Plan or API Key')] }) })), _jsx(DetailRow, { label: t('Modality'), value: formatModalities(highlightedEntry.model.modalities) }), _jsx(DetailRow, { label: t('Context Window'), value: formatContextWindow(highlightedEntry.model.contextWindowSize) }), highlightedEntry.authType !== AuthType.NEVER_MATCH_THIS && (_jsxs(_Fragment, { children: [_jsx(DetailRow, { label: "Base URL", value: highlightedEntry.model.baseUrl ?? t('(default)') }), _jsx(DetailRow, { label: "API Key", value: highlightedEntry.model.envKey ?? t('(not set)') })] }))] })), errorMessage && (_jsx(Box, { marginTop: 1, flexDirection: "column", paddingX: 1, children: _jsxs(Text, { color: theme.status.error, wrap: "wrap", children: ["\u2715 ", errorMessage] }) })), _jsx(Box, { marginTop: 1, flexDirection: "column", children: _jsx(Text, { color: theme.text.secondary, children: t('Enter to select, ↑↓ to navigate, Esc to close') }) })] }));
}
