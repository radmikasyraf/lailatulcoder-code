...
async function configureVSCodeStyle(terminalName, appName) {
    ...
    if (!configDir) {
        return {
            success: false,
            message: t('Could not determine {{terminalName}} config path on Windows: APPDATA environment variable is not set.', { terminalName }),
        };
    }
    ...
    try {
        ...
        try {
            ...
            try {
                ...
                if (!Array.isArray(parsedContent)) {
                    return {
                        success: false,
                        message: t('{{terminalName}} keybindings.json exists but is not a valid JSON array. Please fix the file manually or delete it to allow automatic configuration.', { terminalName }) +
                            '\n' +
                ...
                return {
                    success: false,
                    message: t('Failed to parse {{terminalName}} keybindings.json. The file contains invalid JSON. Please fix the file manually or delete it to allow automatic configuration.', { terminalName }) +
                        '\n' +
                        t('File: {{file}}', { file: keybindingsFile }) +
                        '\n' +
        ...
        if (existingShiftEnter || existingCtrlEnter) {
            ...
            return {
                success: false,
                message: t('Existing keybindings detected. Will not modify to avoid conflicts.') +
                    '\n' +
                    messages.join('\n') +
                    '\n' +
        ...
        if (!hasOurShiftEnter || !hasOurCtrlEnter) {
            ...
            return {
                success: true,
                message: t('Added Shift+Enter and Ctrl+Enter keybindings to {{terminalName}}.', {
                    terminalName,
                }) +
                    '\n' +
        ...
        return {
            success: false,
            message: t('Failed to configure {{terminalName}}.', { terminalName }) +
                '\n' +
                t('File: {{file}}', { file: keybindingsFile }) +
                '\n' +
...
export async function terminalSetup() {
    ...
    if (isKittyProtocolEnabled()) {
        return {
            success: true,
            message: t('Your terminal is already configured for an optimal experience with multiline input (Shift+Enter and Ctrl+Enter).'),
        };
    }
    ...
    if (!terminal) {
        return {
            success: false,
            message: t('Could not detect terminal type. Supported terminals: VS Code, Cursor, Windsurf, and Trae.'),
        };
    }
    switch (terminal) {
        case 'vscode':
