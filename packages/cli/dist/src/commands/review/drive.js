 ...
 * `join(tmpdir(), 'LailatulCoder-review-drive-' + server)` normalises the `..` away; and
 ...
 * not.
 */
export function wrapScript(script, sentinelPath, sentinel = DRIVE_SENTINEL) {
...
export function sentinelExitCode(capture, sentinel = DRIVE_SENTINEL) {
    ...
    let last = null;
    for (let m = re.exec(capture); m; m = re.exec(capture))
        last = m;
    return last ? Number(last[1]) : null;
}
/** Trim a capture to the cap, keeping the TAIL — the end is where the result is. */
export function trimCapture(s) {
    if (s.length <= CAPTURE_MAX)
...
export function runDrive(args) {
    ...
    const tmux = (...a) => exec('tmux', ['-L', server, ...a]);
    ...
    const dir = join(tmpdir(), `LailatulCoder-review-drive-${server}`);
    ...
    try {
        ...
        const create = tmux('new-session', '-d', '-c', args.cwd, 'bash', '-lc', `bash ${shellQuote(scriptPath)} > ${shellQuote(logPath)} 2>&1`);
        if (create.status !== 0) {
            return {
                outcome: 'unavailable',
                observed: false,
...
export const driveCommand = {
    command: 'drive',
    describe: 'Start something, wait until it is really up, drive it, and capture what it did — readiness polled rather than slept on, completion proven by a sentinel, cleanup guaranteed',
    builder: (yargs) => yargs
        .option('script', {
        type: 'string',
        demandOption: true,
        ...
        .option('ready', {
        type: 'string',
        describe: 'Command polled until it exits 0 before driving (omit only when nothing needs to come up first)',
    })
        ...
        describe: 'tmux server name — namespaced so runs cannot capture each other',
    })
        .option('out', {
        type: 'string',
