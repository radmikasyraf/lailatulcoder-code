 ...
 */
import { redactUrlCredentials } from '@lailatul-coder/lailatul-coder-core';
...
export async function handleSourcesRemove(args) {
    try {
        ...
        if (!extensionManager.removeSource(args.name)) {
            writeStderrLine(t('Marketplace "{{name}}" not found.', { name: args.name }));
            process.exit(1);
            return;
        }
...
export async function handleSourcesUpdate(args) {
    try {
        ...
        writeStdoutLine(t('{{count}} available extensions', {
...
const addCommand = {
    ...
    builder: (yargs) => yargs.positional('source', {
        describe: t('The marketplace source to add: owner/repo (GitHub), a git or https URL, or a local path.'),
        type: 'string',
        demandOption: true,
    }),
    handler: async (argv) => {
        await handleSourcesAdd({ source: argv['source'] });
    },
};
const removeCommand = {
    command: 'remove <name>',
    ...
    handler: async (argv) => {
        await handleSourcesRemove({ name: argv['name'] });
    },
};
const listCommand = {
    command: 'list',
...
const updateCommand = {
    ...
    handler: async (argv) => {
        await handleSourcesUpdate({ name: argv['name'] });
    },
};
