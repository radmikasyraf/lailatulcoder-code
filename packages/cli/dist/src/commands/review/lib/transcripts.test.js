...
import { chmodSync, mkdtempSync, rmSync, symlinkSync, writeFileSync, mkdirSync, utimesSync, } from 'node:fs';
import { tmpdir } from 'node:os';
import { join } from 'node:path';
import { readTranscripts, readRunTranscripts, priorSessionDirs, wasGivenTheDiff, transcriptDir, TranscriptsUnavailableError, serializedArgsNamePath, } from './transcripts.js';
...
describe('transcriptDir — resolved from the environment only', () => {
    ...
    it('throws when only one key is present', () => {
        expect(() => transcriptDir({ QWEN_CODE_SESSION_ID: 'S1' })).toThrow(TranscriptsUnavailableError);
        expect(() => transcriptDir({ QWEN_CODE_PROJECT_DIR: '/p' })).toThrow(TranscriptsUnavailableError);
    });
});
describe('readTranscripts — defensive parsing', () => {
    it('throws TranscriptsUnavailableError when the session dir is absent', () => {
        ...
        expect(() => readTranscripts(undefined, {
    ...
    it('skips the harness sidecar files beside a transcript', () => {
        ...
        file('agent-a1.jsonl', JSON.stringify({
            agentId: 'a1',
            agentName: 'general-purpose',
            type: 'user',
            message: { role: 'user', parts: [{ text: 'chunk 1 of 1' }] },
        }) + '\n');
        file('agent-a1.meta.json', JSON.stringify({
            agentId: 'a1',
    ...
    it('tolerates a malformed final line — an agent still writing', () => {
        ...
        file('agent-a1.jsonl', JSON.stringify({
            ...
            '\n' +
            '{"type":"assistant","message":{"parts":[{"text":"partial');
        const recs = readTranscripts(undefined, ENV);
        expect(recs).toHaveLength(1);
        expect(recs[0].agentId).toBe('a1');
    ...
    it('counts a diff read via serializedArgsNamePath, not a look-alike', () => {
        ...
        const b = { agentId: 'a1', agentName: 'general-purpose', sessionId: 'S1' };
        const call = (name, args, response = { output: 'ok' }) => [
            {
                ...b,
                type: 'assistant',
        ...
        file('agent-a1.jsonl', [
            ...
            ...call('read_file', { file_path: '/d.txt', offset: 40, limit: 40 }, { error: 'denied' }),
        ]
...
describe('wasGivenTheDiff', () => {
    ...
    it('is true only when the prompt names the diff path', () => {
        expect(wasGivenTheDiff(rec('read_file(file_path="/d.txt")'), '/d.txt')).toBe(true);
    });
    it('is false for a prompt that names only source files', () => {
        expect(wasGivenTheDiff(rec('read_file(file_path="/src/pay.ts")'), '/d.txt')).toBe(false);
    });
...
describe('readRunTranscripts — the run across its sessions', () => {
    ...
    const transcript = (agentId) => JSON.stringify({
        agentId,
        agentName: 'general-purpose',
        type: 'user',
    ...
    function planWithLedger(...sessionIds) {
        ...
        sessionIds.forEach((id, i) => {
            appendRunSession(plan, { QWEN_CODE_SESSION_ID: id }, i === sessionIds.length - 1 ? now + 1500 : now);
        });
        recordResume(plan, ENV, now + 1500);
        return plan;
    }
    function priorFile(session, name, contents) {
    ...
    it('still throws when the CURRENT session dir is absent', () => {
        const plan = planWithLedger('S0', 'S1');
        priorFile('S0', 'agent-a0.jsonl', transcript('a0'));
        expect(() => readRunTranscripts(plan, undefined, {
    ...
    it('refuses a foreign-stamped transcript in the CURRENT directory too', () => {
        ...
        file('agent-planted.jsonl', JSON.stringify({
            agentId: 'planted',
            agentName: 'general-purpose',
            sessionId: 'S0',
            type: 'user',
    ...
    it('looks up the session directory under the SANITIZED id', () => {
        ...
        writeFileSync(join(dir, 'subagents', 'S_dot', 'agent-a9.jsonl'), JSON.stringify({
            agentId: 'a9',
            agentName: 'general-purpose',
            sessionId: 'S.dot',
            type: 'user',
    ...
    it('refuses a foreign-stamped transcript in a PRIOR directory', () => {
        ...
        const plan = planWithLedger('S0', 'S1');
        priorFile('S0', 'agent-planted.jsonl', JSON.stringify({
            agentId: 'planted',
            agentName: 'general-purpose',
            sessionId: 'S9',
            type: 'user',
    ...
    it('excludes a prior transcript written AFTER its attempt ended', () => {
        ...
        utimesSync(join(dir, 'subagents', 'S0', 'agent-late.jsonl'), future, future);
        file('agent-a1.jsonl', transcript('a1'));
        const recs = readRunTranscripts(plan, undefined, ENV);
        expect(recs.map((r) => r.agentId)).toEqual(['a1']);
    });
...
describe('readRunTranscripts — currentDirOptional', () => {
    ...
    it('absorbs a missing CURRENT dir when asked — the pre-launch resume read', () => {
        ...
        writeFileSync(join(dir, 'subagents', 'S0', 'agent-a0.jsonl'), transcript('a0'));
        ...
        expect(() => readRunTranscripts(plan, undefined, env)).toThrow(TranscriptsUnavailableError);
    ...
    it('still throws on a missing ENVIRONMENT even with the option', () => {
        const plan = join(dir, 'qwen-review-pr-7-fetch.json');
        writeFileSync(plan, JSON.stringify({ diffLines: 1, chunks: [] }));
        expect(() => readRunTranscripts(plan, undefined, {}, undefined, {
            currentDirOptional: true,
        })).toThrow(TranscriptsUnavailableError);
    });
});
describe('readRunTranscripts — containment and fault handling', () => {
    ...
    it.skipIf(process.platform === 'win32' || process.getuid?.() === 0)('still throws when the current dir is unreadable, not merely absent', () => {
        ...
        chmodSync(cur, 0o000);
        try {
            expect(() => readRunTranscripts(plan, undefined, ENV, undefined, {
                currentDirOptional: true,
            })).toThrow(TranscriptsUnavailableError);
        }
        finally {
    ...
    it('throws on a missing current dir when the run has no prior evidence', () => {
        ...
        const plan = join(dir, 'qwen-review-pr-7-fetch.json');
        writeFileSync(plan, JSON.stringify({ diffLines: 1, chunks: [] }));
        expect(() => readRunTranscripts(plan, undefined, { QWEN_CODE_PROJECT_DIR: dir, QWEN_CODE_SESSION_ID: 'S-none' }, undefined, { currentDirOptional: true })).toThrow(TranscriptsUnavailableError);
    });
});
describe('the incomplete-transcript shapes the resume path reads', () => {
    const user = (text) => JSON.stringify({
        agentId: 'a1',
        agentName: 'general-purpose',
        sessionId: 'S1',
        type: 'user',
    ...
    const assistant = (parts) => JSON.stringify({
        agentId: 'a1',
        sessionId: 'S1',
        type: 'assistant',
    ...
    it('a non-terminal sidecar status makes the last text progress, not a return', () => {
        ...
        file('agent-a1.jsonl', [user('go'), assistant([{ text: 'Verdict: clean.' }])].join('\n') + '\n');
        file('agent-a1.meta.json', JSON.stringify({ agentId: 'a1', status: 'running' }));
        const recs = readTranscripts(undefined, ENV);
        expect(recs).toHaveLength(1);
    ...
    it('a completed sidecar (or none) leaves content inference standing', () => {
        file('agent-a1.jsonl', [user('go'), assistant([{ text: 'Verdict: clean.' }])].join('\n') + '\n');
        file('agent-a1.meta.json', JSON.stringify({ agentId: 'a1', status: 'completed' }));
...
describe('serializedArgsNamePath — the one needle both halves use', () => {
    ...
    it('matches the path as a whole JSON string value', () => {
        expect(serializedArgsNamePath(JSON.stringify({ absolute_path: brief }), brief)).toBe(true);
    });
    it('does not credit a longer path holding this one as a prefix', () => {
        expect(serializedArgsNamePath(JSON.stringify({ absolute_path: `${brief}.bak` }), brief)).toBe(false);
    });
    it('does not credit a shell command that merely mentions the path', () => {
        ...
        expect(serializedArgsNamePath(JSON.stringify({ command: `rm ${brief}` }), brief)).toBe(false);
    });
});
//# sourceMappingURL=transcripts.test.js.map