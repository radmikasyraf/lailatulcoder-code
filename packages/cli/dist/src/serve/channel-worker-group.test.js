...
function fakeRuntime(cwd, primary, effectiveEnv) {
    return {
        workspaceId: `id:${cwd}`,
        workspaceCwd: cwd,
        primary,
...
function fakeRegistry(runtimes) {
    return {
        primary: runtimes.find((runtime) => runtime.primary),
        list: () => runtimes,
        listManaged: () => runtimes,
        add: vi.fn(),
        getByWorkspaceCwd: (cwd) => runtimes.find((runtime) => runtime.workspaceCwd === cwd),
        getByWorkspaceId: (id) => runtimes.find((runtime) => runtime.workspaceId === id),
        getManagedByWorkspaceCwd: (cwd) => runtimes.find((runtime) => runtime.workspaceCwd === cwd),
        getManagedByWorkspaceId: (id) => runtimes.find((runtime) => runtime.workspaceId === id),
        resolveWorkspaceCwd: (cwd) => cwd === undefined
...
function makeCreateSupervisor(snapshotFor) {
    const recorded = [];
    const createSupervisor = (opts) => {
        const supervisor = {
            start: vi.fn(async () => { }),
            stop: vi.fn(async () => { }),
...
describe('createChannelWorkerGroup', () => {
    it('wires loop MCP to the exact workspace session with owner-safe cleanup', async () => {
        ...
        const { createSupervisor, recorded } = makeCreateSupervisor(() => snapshot({}));
        ...
        const oldSender = vi.fn(async (payload) => payload);
        await recorded[0].opts.registerChannelLoopMcp?.({
            sessionId: 'session-1',
            ownerId: 'worker-old',
            sendMessage: oldSender,
        });
        expect(addSessionRuntimeMcpServer).toHaveBeenCalledWith('session-1', CHANNEL_LOOP_MCP_SERVER_NAME, {
            type: 'sdk',
            __clientMcpOverWs: true,
        }, 'worker-old');
        const reverseSender = clientMcpSenderRegistry.lookup(CHANNEL_LOOP_MCP_SERVER_NAME);
        await expect(reverseSender?.({ jsonrpc: '2.0', id: 1, method: 'tools/list' }, { sessionId: 'session-1' })).resolves.toMatchObject({ method: 'tools/list' });
        expect(oldSender).toHaveBeenCalledOnce();
        const newSender = vi.fn(async () => ({ owner: 'new' }));
        clientMcpSenderRegistry.setSession(CHANNEL_LOOP_MCP_SERVER_NAME, 'session-1', newSender, 'worker-new');
        await recorded[0].opts.unregisterChannelLoopMcp?.('session-1', 'worker-old');
        expect(removeSessionRuntimeMcpServer).not.toHaveBeenCalled();
        await expect(clientMcpSenderRegistry.lookup(CHANNEL_LOOP_MCP_SERVER_NAME)?.({ jsonrpc: '2.0', id: 2, method: 'ping' }, { sessionId: 'session-1' })).resolves.toEqual({ owner: 'new' });
    });
    it('routes delivery to the exact workspace owner', async () => {
        ...
        await expect(group.deliverChannelMessage(deliveryRequest, SECONDARY)).resolves.toEqual({ delivered: true });
        expect(recorded[0].supervisor.deliverChannelMessage).not.toHaveBeenCalled();
        expect(recorded[1].supervisor.deliverChannelMessage).toHaveBeenCalledWith(deliveryRequest);
    });
    it('does not fall back when the selected workspace lacks the channel', async () => {
        ...
        expect(recorded[0].supervisor.deliverChannelMessage).not.toHaveBeenCalled();
        expect(recorded[1].supervisor.deliverChannelMessage).not.toHaveBeenCalled();
    });
    it('rejects delivery while the owning workspace is draining', async () => {
        ...
        await expect(group.deliverChannelMessage(deliveryRequest, SECONDARY)).rejects.toMatchObject({
            code: 'channel_worker_unavailable',
            message: 'Channel worker for channel "b" is unavailable while its workspace is draining.',
        });
        expect(recorded[1].supervisor.deliverChannelMessage).not.toHaveBeenCalled();
    });
    it('routes webhook tasks to the supervisor that owns the channel', async () => {
        ...
        expect(recorded[1].supervisor.enqueueWebhookTask).toHaveBeenCalledWith(webhookTask);
    });
    it('drains and removes only the target workspace worker', async () => {
        ...
        await expect(group.reconcile(groups, { force: true })).rejects.toThrow('cannot change while a workspace is draining');
    ...
    it('falls back to synchronous kill when target worker stop fails', async () => {
        ...
        recorded[1].supervisor.stop.mockRejectedValueOnce(new Error('stop failed'));
    ...
    it('routes --channel all webhook tasks to the primary supervisor', async () => {
        ...
        expect(recorded[0].supervisor.enqueueWebhookTask).toHaveBeenCalledWith(webhookTask);
    });
    ...
    it('throws when a group references an unregistered workspace', () => {
        const registry = fakeRegistry([fakeRuntime(PRIMARY, true)]);
        const { createSupervisor } = makeCreateSupervisor(() => snapshot({}));
        expect(() => createChannelWorkerGroup({
            groups: [
                {
                    workspaceCwd: SECONDARY,
                    selection: { mode: 'names', names: ['b'] },
    ...
    it('removes a restored worker from routing when startup fails', async () => {
        ...
        const registry = fakeRegistry(runtimes);
        const { createSupervisor, recorded } = makeCreateSupervisor(() => snapshot({}));
        const createSupervisorWithRestoreFailure = (opts) => {
        ...
        await group.start();
        await group.removeWorkspace(SECONDARY);
        await expect(group.restoreWorkspace(SECONDARY)).rejects.toThrow('restore failed');
        expect(group.workspaceActivity(SECONDARY)).toBe(0);
        await expect(group.enqueueWebhookTask(webhookTask)).rejects.toMatchObject({
            code: 'channel_worker_unavailable',
        });
    ...
    it('stops a restored worker whose start finishes after the group stops', async () => {
        ...
        const { createSupervisor, recorded } = makeCreateSupervisor(() => snapshot({}));
        let releaseStart;
        const createSupervisorWithPendingRestore = (opts) => {
            const supervisor = createSupervisor(opts);
            if (recorded.length === 3) {
                supervisor.start.mockImplementationOnce(() => new Promise((resolve) => {
        ...
        expect(group.workspaceActivity(SECONDARY)).toBe(0);
        expect(group.snapshots()).not.toEqual(expect.arrayContaining([
    ...
    it('does not start later supervisors when the first start fails', async () => {
        ...
        recorded[0].supervisor.start.mockRejectedValueOnce(new Error('primary failed'));
    ...
    it('does not start later supervisors when stopped during startup', async () => {
        ...
        let releaseStart;
        recorded[0].supervisor.start.mockImplementationOnce(() => new Promise((resolve) => {
            releaseStart = resolve;
        }));
        ...
        await group.stop();
        releaseStart();
        await expect(startPromise).rejects.toThrow('Channel worker group stopped during startup.');
        expect(recorded[1].supervisor.start).not.toHaveBeenCalled();
    });
    it('rolls back already-started supervisors when a later start fails', async () => {
        ...
        recorded[1].supervisor.start.mockRejectedValueOnce(new Error('secondary failed'));
    ...
    it('attempts to stop every supervisor before reporting a stop failure', async () => {
        ...
        recorded[0].supervisor.stop.mockRejectedValueOnce(new Error('primary stop failed'));
    ...
    it('annotates snapshots with workspace metadata and exposes the primary', () => {
        ...
        const { createSupervisor } = makeCreateSupervisor((workspace) => snapshot({
    ...
    it('forwards ready/exit/log callbacks with workspace metadata', () => {
        ...
        expect(onExit).toHaveBeenCalledWith(expect.objectContaining({ workspaceCwd: SECONDARY, state: 'exited' }));
    ...
    it('reports health for running, mixed, and empty worker groups', async () => {
        ...
        const { createSupervisor } = makeCreateSupervisor((workspace) => snapshot({ state: states.get(workspace) }));
        const group = createChannelWorkerGroup({
            groups: [
                { workspaceCwd: PRIMARY, selection: { mode: 'names', names: ['a'] } },
                {
    ...
    it('force-reconciles an unchanged healthy selection', async () => {
        ...
        await group.start();
        await expect(group.reconcile(groups, { force: true })).resolves.toMatchObject({
            changed: true,
        });
        expect(recorded).toHaveLength(2);
        expect(recorded[0].supervisor.stop).toHaveBeenCalledTimes(1);
    ...
    it('does not commit a worker drained while reconcile is starting it', async () => {
        ...
        group.beginWorkspaceDrain(SECONDARY);
        releaseStart();
        await expect(reconciling).rejects.toThrow('Workspace drained during channel worker reconcile.');
    ...
    it('preserves attempted startup failures when rollback restoration also fails', async () => {
        const registry = fakeRegistry([fakeRuntime(PRIMARY, true)]);
        const { createSupervisor, recorded } = makeCreateSupervisor(() => snapshot({}));
        const createSupervisorWithFailure = (opts) => {
            const supervisor = createSupervisor(opts);
            if (opts.selection.mode === 'names' &&
                opts.selection.names.includes('replacement')) {
                supervisor.start.mockRejectedValueOnce(new ChannelWorkerStartupError('replacement failed', {
        ...
        await group.start();
        recorded[0].supervisor.start.mockRejectedValueOnce(new Error('old worker restore failed'));
        const error = await group
            .reconcile([
            {
                workspaceCwd: PRIMARY,
    ...
    it('attempts to restore every stopped worker after a rollback failure', async () => {
        ...
        await group.start();
        recorded[0].supervisor.start.mockRejectedValueOnce(new Error('primary restore failed'));
        await expect(group.reconcile([
            { workspaceCwd: PRIMARY, selection: { mode: 'names', names: ['c'] } },
            {
                workspaceCwd: SECONDARY,
    ...
    it('does not start replacements after an unconfirmed old-worker stop', async () => {
        ...
        recorded[0].supervisor.stop.mockRejectedValueOnce(new ChannelWorkerStopError('still alive'));
        await expect(group.reconcile([
            { workspaceCwd: PRIMARY, selection: { mode: 'names', names: ['c'] } },
        ])).rejects.toMatchObject({
            stopFailed: true,
    ...
    it('force-kills a replacement that is still starting', async () => {
        ...
        let releaseStart;
        recorded[1].supervisor.start.mockImplementationOnce(() => new Promise((resolve) => {
            releaseStart = resolve;
        }));
        await vi.waitFor(() => expect(recorded[1].supervisor.start).toHaveBeenCalledTimes(1));
        group.killAllSync();
    ...
    it('retains an unconfirmed replacement so a later stop can retry it', async () => {
        ...
        recorded[1].supervisor.start.mockRejectedValueOnce(new Error('replacement failed'));
        recorded[1].supervisor.stop.mockRejectedValueOnce(new ChannelWorkerStopError('still alive'));
...
//# sourceMappingURL=channel-worker-group.test.js.map