...
function setup(path, content, hunks) {
    const abs = join(dir, path);
    mkdirSync(join(abs, '..'), { recursive: true });
    writeFileSync(abs, content);
    const planPath = join(dir, 'plan.json');
    writeFileSync(planPath, JSON.stringify({ files: [{ path, kind: 'source', hunks }] }));
...
describe.skipIf(!hasShellcheck)('runScriptLint — shellcheck on a changed line', () => {
...
describe('runScriptLint — graceful degradation and scoping', () => {
    it('defers a workflow to skipped — actionlint source-mapping is not yet supported', () => {
        ...
        const { plan, worktree } = setup('.github/workflows/ci.yml', 'name: CI\non: push\njobs: {}\n', [{ newStart: 1, newEnd: 3 }]);
        const r = runScriptLint({ plan, worktree });
    ...
    it('records a symlinked script as skipped, never dropped (empty ≠ clean)', () => {
        ...
        writeFileSync(planPath, JSON.stringify({ files: [{ path: 'hook.sh', kind: 'source' }] }));
...
//# sourceMappingURL=script-lint.test.js.map