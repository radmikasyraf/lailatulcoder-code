...
import { McpClient, MCPDiscoveryState, MCPServerStatus, attemptAutomaticMcpOAuth, getMCPServerStatus, populateMcpServerCommand, probeMcpServerForOAuth, removeMCPServerStatus, setMCPDiscoveryState, } from './mcp-client.js';
...
import { McpBudgetWouldExceedError, McpServerSpawnFailedError, InvalidMcpConfigError, } from './mcp-errors.js';
 ...
 */
export const MCP_BUDGET_WARN_FRACTION = 0.75;
/**
 ...
 */
export const MCP_BUDGET_REARM_FRACTION = 0.375;
/**
...
export class BudgetExhaustedError extends Error {
     ...
     */
    reservedCount;
    constructor(serverName, budget, reservedCount) {
        super(`MCP client budget exhausted: cannot reserve slot for '${serverName}' ` +
 ...
 */
export function mcpTransportOf(config) {
...
function readBudgetFromEnv() {
    ...
    if (rawBudget !== undefined && rawBudget !== '') {
        ...
        if (/^\d+$/.test(trimmed) && Number.isSafeInteger(parsed) && parsed > 0) {
            ...
            process.stderr.write(`qwen serve: ignoring invalid QWEN_SERVE_MCP_CLIENT_BUDGET=` +
    ...
    if (rawMode === 'enforce' || rawMode === 'warn' || rawMode === 'off') {
        ...
        if (rawMode !== undefined && rawMode !== '') {
            ...
            // typo is visible.
            process.stderr.write(`qwen serve: ignoring invalid QWEN_SERVE_MCP_BUDGET_MODE=` +
    ...
    // breadcrumb pattern.
    if ((budgetMode === 'enforce' || budgetMode === 'warn') &&
        clientBudget === undefined) {
        process.stderr.write(`qwen serve: QWEN_SERVE_MCP_BUDGET_MODE=${budgetMode} requires ` +
...
export class McpClientManager {
    ...
    isReconnecting = new Map();
    serverDiscoveryPromises = new Map();
    /**
     ...
     */
    stopTimedOut = false;
    constructor(config, toolRegistry, options = {}) {
        ...
        // enforcement.
        if ((resolvedMode === 'enforce' || resolvedMode === 'warn') &&
            resolved.clientBudget === undefined) {
            process.stderr.write(`qwen serve: McpClientManager constructed with budgetMode=${resolvedMode} ` +
     ...
     */
    tryReserveSlot(serverName) {
        if (this.reservedSlots.has(serverName))
            return 'already_held';
        if (this.clientBudget === undefined || this.budgetMode === 'off') {
            return 'reserved';
        }
        if (this.budgetMode === 'enforce' &&
    ...
    getMcpClientAccounting() {
        ...
        for (const [name, client] of this.clients) {
            ...
            const transport = cfg ? mcpTransportOf(cfg) : 'unknown';
    ...
    getServerStatus(serverName) {
        return (this.pooledConnections.get(serverName)?.client.getStatus() ??
     ...
     */
    isServerDiscovering(serverName) {
     ...
     */
    refuseAndLog(serverName, serverConfig) {
        ...
        // guard above).
        this.lastRefusedTransports.set(serverName, serverConfig ? mcpTransportOf(serverConfig) : 'unknown');
        ...
        process.stderr.write(`qwen serve: MCP server '${serverName}' refused (budget exhausted, ` +
     ...
     */
    emitBudgetTelemetry(configuredCount) {
        if (this.budgetMode === 'off')
            return;
     ...
     */
    evaluateBudgetState() {
        if (this.budgetMode === 'off' || this.clientBudget === undefined)
            return;
        ...
        if (this.warnArmed && ratio >= MCP_BUDGET_WARN_FRACTION) {
            ...
            debugLogger.info(`MCP budget warning fired (ratio=${ratio.toFixed(2)}, ` +
            ...
            debugLogger.info(`MCP budget warning re-armed (ratio=${ratio.toFixed(2)}, ` +
    ...
    emitRefusedBatchIfAny() {
        ...
        if (this.bulkPassDepth > 0)
            return;
        if (this.pendingRefusalNames.size === 0)
            return;
        if (this.clientBudget === undefined || this.budgetMode !== 'enforce') {
            ...
            debugLogger.warn(`MCP guardrail: dropped ${this.pendingRefusalNames.size} ` +
        ...
        const namesInOrder = this.lastRefusedServerNames.filter((n) => this.pendingRefusalNames.has(n));
        ...
        const refusedServers = namesInOrder.map((name) => ({
            name,
    ...
    emitBudgetEvent(event) {
        ...
        try {
            ...
            debugLogger.debug(`MCP budget event callback threw (kind=${event.kind}): ` +
    ...
    getEffectiveMcpServers() {
        return populateMcpServerCommand(this.cliConfig.getMcpServers() || {}, this.cliConfig.getMcpServerCommand(), this.cliConfig.getTargetDir());
    }
    /**
        ...
        try {
            ...
            const discoveryPromises = Object.entries(servers).map(async ([name, config]) => {
                ...
                const sdkCallback = isSdkMcpServerConfig(config)
                    ...
                    : undefined;
                const client = new McpClient(name, config, this.toolRegistry, this.cliConfig.getPromptRegistry(), this.cliConfig.getWorkspaceContext(), this.cliConfig.getDebugMode(), sdkCallback);
                ...
                try {
                    ...
                    this.connectedConfigKeys.set(name, this.singleSessionConnectedKeyOf(name, config));
                    this.eventEmitter?.emit('mcp-client-update', this.clients);
                }
                catch (error) {
                    // zombie slot leak.
    ...
    async discoverMcpToolsForServerInternal(serverName, cliConfig) {
        const servers = this.getEffectiveMcpServers();
        const serverConfig = servers[serverName];
        if (!serverConfig) {
            return;
        ...
        if (cliConfig.isTrustedFolder?.() === false) {
            debugLogger.debug(`Skipping MCP server in untrusted folder: ${serverName}`);
            return;
        }
        // disabled gate.
        ...
        try {
            ...
            this.connectedConfigKeys.set(serverName, this.singleSessionConnectedKeyOf(serverName, serverConfig));
            // a server that
            ...
            debugLogger.error(`Error during discovery for server '${serverName}': ${getErrorMessage(error)}`);
        }
        finally {
     ...
     */
    discoverAllMcpToolsViaPool(cliConfig) {
        ...
        this.discoveryInFlight = this.runDiscoverAllMcpToolsViaPool(cliConfig).finally(() => {
            this.discoveryInFlight = undefined;
        });
        return this.discoveryInFlight;
    }
    async runDiscoverAllMcpToolsViaPool(cliConfig) {
        if (!this.pool)
            return; // unreachable; caller already gates
        // reset the
            ...
            for (const [name, conn] of [...this.pooledConnections]) {
                ...
                if (desired === undefined || desired !== conn.transportId) {
                    try {
                        ...
                        debugLogger.debug(`Pool release error (ignored): ${getErrorMessage(err)}`);
                    }
                    this.pooledConnections.delete(name);
                }
            }
            const acquirePromises = Object.entries(servers).map(async ([name, config]) => {
                if (cliConfig.isMcpServerDisabled(name)) {
                    debugLogger.debug(`Skipping disabled MCP server (pool mode): ${name}`);
                    return;
                }
                ...
                if (cliConfig.isMcpServerPendingApproval?.(name)) {
                    debugLogger.debug(`Skipping pending-approval MCP server (pool mode): ${name}`);
                    return;
                }
                ...
                if (existingConnection) {
                    try {
                        ...
                        // outcome.
                        debugLogger.error(`Pool metadata refresh failed for ${name}: ${getErrorMessage(err)}`);
                    }
                    return;
                }
                try {
                    const conn = await this.pool.acquire(name, config, sessionId, this.toolRegistry, promptRegistry, resourceRegistry);
                    //
                    ...
                    if (err instanceof BudgetExhaustedError) {
                        debugLogger.debug(`Pool refused acquire for ${name} (budget exhausted, ` +
                        ...
                        debugLogger.error(`Pool acquire failed for ${name}: ${getErrorMessage(err)}`);
                    }
                }
            });
    ...
    async stop() {
        ...
        if (this.discoveryInFlight) {
            debugLogger.debug('stop(): awaiting in-flight pool discovery to drain (5s cap)');
            ...
            try {
                await Promise.race([
                    ...
                    new Promise((resolve) => {
                        graceTimer = setTimeout(() => {
                            ...
                            debugLogger.debug('stop(): in-flight discovery did not settle within 5s grace; proceeding');
                            resolve();
                ...
                debugLogger.debug(`stop(): in-flight discovery rejected (proceeding): ${getErrorMessage(err)}`);
            }
            finally {
                if (graceTimer)
    ...
    async performHealthCheck(serverName) {
        ...
        try {
            ...
            if (status !== MCPServerStatus.CONNECTED) {
                ...
                debugLogger.warn(`Health check failed for server '${serverName}' (${failures}/${this.healthConfig.maxConsecutiveFailures})`);
            ...
            debugLogger.error(`Error during health check for server '${serverName}': ${getErrorMessage(error)}`);
        }
    }
    /**
     ...
     */
    async discoverAllMcpToolsIncremental(cliConfig) {
        ...
        try {
            ...
            for (const [name] of Object.entries(servers)) {
                ...
                if (cliConfig.isMcpServerDisabled(name) ||
                    ...
                    debugLogger.debug(cliConfig.isMcpServerDisabled(name)
            ...
            const discoveryPromises = serversToUpdate.map(async (name) => {
                ...
                try {
                    await this.runWithDiscoveryTimeout(name, serverConfig, () => this.discoverMcpToolsForServer(name, cliConfig));
                    await probeMcpServerForOAuth(name, serverConfig);
                    const authenticated = await attemptAutomaticMcpOAuth(name, serverConfig, cliConfig.isInteractive?.() === true &&
                    ...
                    const actuallyReady = !!client && getMCPServerStatus(name) === MCPServerStatus.CONNECTED;
                    if (actuallyReady) {
                    ...
                    debugLogger.error(`Error during incremental discovery for server '${name}': ${getErrorMessage(error)}`);
                }
            });
            await Promise.all(discoveryPromises);
     ...
     */
    runWithDiscoveryTimeout(serverName, serverConfig, fn) {
                    ...
                    try {
                        ...
                        if (rootPid !== undefined) {
                            ...
                            if (descendants.length > 0) {
                                ...
                                debugLogger.debug(`Sent SIGTERM to ${signaled}/${descendants.length} descendants ` +
                                    `of pid ${rootPid} for timed-out server '${serverName}'`);
                                if (signaled < descendants.length) {
                                    debugLogger.warn(`Partial signal for timed-out server '${serverName}': ` +
                            ...
                            debugLogger.debug(`Skipping descendant pid sweep for timed-out server '${serverName}': transport pid unavailable`);
                        }
                    }
                    catch (err) {
                        debugLogger.warn(`Descendant pid sweep for timed-out server '${serverName}' threw: ${getErrorMessage(err)}. Proceeding with disconnect.`);
                    }
                    try {
                        await client.disconnect();
                    }
                    catch (err) {
                        debugLogger.debug(`Forced disconnect of timed-out server '${serverName}' threw: ${getErrorMessage(err)}`);
                    }
                }
     ...
     */
    static MIN_DISCOVERY_TIMEOUT_MS = 100;
    static MAX_DISCOVERY_TIMEOUT_MS = 300_000;
    discoveryTimeoutFor(serverConfig) {
        const override = serverConfig?.discoveryTimeoutMs;
        if (override !== undefined && Number.isFinite(override)) {
            return Math.max(McpClientManager.MIN_DISCOVERY_TIMEOUT_MS, Math.min(override, McpClientManager.MAX_DISCOVERY_TIMEOUT_MS));
        }
        ...
        const isRemote = !!(serverConfig?.httpUrl ||
            serverConfig?.url ||
            serverConfig?.tcp);
     ...
     */
    singleSessionConnectedKeyOf(serverName, config) {
     ...
     */
    purgeServerRegistries(serverName) {
    ...
    async removeServer(serverName) {
        ...
        if (client) {
            try {
                ...
                debugLogger.error(`Error disconnecting removed server '${serverName}': ${getErrorMessage(error)}`);
            }
            this.clients.delete(serverName);
            this.stopHealthCheck(serverName);
            this.consecutiveFailures.delete(serverName);
    ...
    async readResource(serverName, uri, options) {
        ...
        if (pooled) {
            ...
            if (pooled.client.getStatus() !== MCPServerStatus.CONNECTED) {
                this.pooledConnections.delete(serverName);
                throw new Error(`MCP server '${serverName}' pool entry disconnected; retry after discovery.`);
            }
        ...
        if (!client) {
            ...
            if (reservation === 'refused') {
                ...
                this.emitRefusedBatchIfAny();
                throw new BudgetExhaustedError(serverName, this.clientBudget, this.reservedSlots.size);
            }
                ...
                await Promise.race([
                    ...
                    new Promise((_, reject) => {
                        timeoutId = setTimeout(() => {
                            reject(new Error(`MCP server '${serverName}' lazy connect timed out after ${timeoutMs}ms`));
                        }, timeoutMs);
                    }),
                ]).finally(() => {
                    if (timeoutId)
    ...
    async addRuntimeMcpServer(name, config, originatorClientId) {
        ...
        if (this.cliConfig.isMcpServerDisabled(name)) {
            throw new InvalidMcpConfigError(name, `server '${name}' is in excludedMcpServers and cannot be added at runtime`);
        }
        debugLogger.info(`addRuntimeMcpServer: ${name} (transport=${mcpTransportOf(config)}, client=${originatorClientId})`);
        // Validate config minimally: must have at least one transport field
        const transport = mcpTransportOf(config);
        if (transport === 'unknown') {
            throw new InvalidMcpConfigError(name, 'config must specify at least one of: command, url, httpUrl, tcp');
        }
        ...
        const ifAbsent = config[RUNTIME_MCP_IF_ABSENT_CONFIG_FLAG] === true;
        if (ifAbsent) {
            const existingRuntimeConfig = this.cliConfig.getRuntimeMcpServers()[name];
            const existingIsIfAbsent = existingRuntimeConfig?.[RUNTIME_MCP_IF_ABSENT_CONFIG_FLAG] === true;
        ...
        try {
            ...
            const exitCode = err instanceof Error && 'exitCode' in err
                ? err.exitCode
                : undefined;
