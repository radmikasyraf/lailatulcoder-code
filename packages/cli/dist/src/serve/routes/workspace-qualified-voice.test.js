...
const homes = [];
function runtime(workspaceId, workspaceCwd, opts = {}) {
    return {
        workspaceId,
        workspaceCwd,
        primary: opts.primary === true,
        trusted: opts.trusted !== false,
        env: opts.envMode === 'parent-process'
...
async function createApp(opts = {}) {
    ...
    await Promise.all([primaryCwd, secondaryCwd, untrustedCwd].map((cwd) => fsp.mkdir(cwd, { recursive: true })));
    ...
    const acquireVoiceLease = vi.fn(opts.acquireVoiceLease ??
        (() => ({
            kind: 'admitted',
...
async function enableSecondaryVoice(runtime) {
    await fsp.mkdir(path.join(runtime.workspaceCwd, '.qwen'), {
        recursive: true,
    });
    await fsp.writeFile(path.join(runtime.workspaceCwd, '.qwen', 'settings.json'), JSON.stringify({
...
describe('workspace-qualified Voice routes', () => {
    ...
    it('selects only the trusted target runtime and writes Voice settings in workspace scope', async () => {
        const { app, secondary, persistSetting, invalidateServeFeaturesCache } = await createApp();
        await expect(request(app).get('/workspaces/secondary-id/voice')).resolves.toMatchObject({
            status: 200,
            body: { workspaceCwd: secondary.workspaceCwd },
        });
        await expect(request(app)
            .post('/workspaces/secondary-id/voice')
            .send({ enabled: false })).resolves.toMatchObject({ status: 200 });
        await expect(request(app).get(`/workspaces/${encodeURIComponent(secondary.workspaceCwd)}/voice`)).resolves.toMatchObject({
            status: 200,
            body: { workspaceCwd: secondary.workspaceCwd },
        });
        expect(persistSetting).toHaveBeenCalledWith(secondary.workspaceCwd, SettingScope.Workspace, 'general.voice.enabled', false);
        expect(secondary.bridge.publishWorkspaceEvent).toHaveBeenCalled();
    ...
    it('rejects an unknown selector before reading settings and untrusted targets without fallback', async () => {
        ...
        await expect(request(app).get('/workspaces/missing/voice')).resolves.toMatchObject({
            status: 400,
            body: { code: 'workspace_mismatch' },
        });
        await expect(request(app).get('/workspaces/untrusted-id/voice')).resolves.toMatchObject({
            status: 403,
    ...
    it('rejects the internal runtime by id and cwd without falling back', async () => {
        ...
        registry.add(runtime('live-id', liveCwd, { provenance: 'live-conversation' }));
        for (const selector of ['live-id', encodeURIComponent(liveCwd)]) {
            await expect(request(app).get(`/workspaces/${selector}/voice`)).resolves.toMatchObject({
                status: 400,
                body: { code: 'workspace_mismatch' },
            });
        }
    ...
    it('inherits the parent process environment for parent-process runtimes', async () => {
        ...
        const response = await request(app)
            ...
            .send(Buffer.from([1, 2, 3]));
        expect(response.status).toBe(200);
        expect(transcribe).toHaveBeenCalledWith(expect.objectContaining({ env: undefined }));
    });
    ...
    it('reports workspace draining after the registry drain gate closes', async () => {
        ...
        const { app, secondary, registry, transcribe, acquireVoiceLease } = await createApp({
...
//# sourceMappingURL=workspace-qualified-voice.test.js.map