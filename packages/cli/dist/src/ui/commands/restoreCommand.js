...
async function restoreAction(context, args) {
    ...
    if (!checkpointDir) {
        return {
            type: 'message',
            messageType: 'error',
            content: 'Could not determine the .lailatulcoder directory path.',
        };
    }
    try {
...
export const restoreCommand = (config) => {
    ...
    return {
        name: 'restore',
        get description() {
            return t('Restore a tool call. This will reset the conversation and file history to the state it was in when the tool call was suggested');
        },
