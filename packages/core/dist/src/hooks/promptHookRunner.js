 ...
 */
const LLM_HOOK_SYSTEM_PROMPT = `You are evaluating a hook in LailatulCoder Ai.
...
export class PromptHookRunner {
    ...
    async execute(hookConfig, eventName, input, signal) {
        ...
        if (signal?.aborted) {
            return {
                ...
                success: false,
                outcome: 'cancelled',
                error: new Error(`Prompt hook execution cancelled (aborted): ${hookName}`),
                duration: 0,
            };
        }
        ...
        try {
            ...
            const processedPrompt = this.replaceArgumentsPlaceholder(hookConfig.prompt, jsonInput);
            ...
            const result = await this.executeWithTimeout(processedPrompt, model, timeoutMs, signal);
            const duration = Date.now() - startTime;
            debugLogger.debug(`Prompt hook ${hookName} completed in ${duration}ms with result: ${result.ok}`);
            // Process result
            debugLogger.debug(`Prompt hook result: ok=${result.ok}, reason=${result.reason}`);
            ...
            debugLogger.warn(`Prompt hook ${hookName} failed (non-blocking): ${errorMessage}`);
            return {
                hookConfig,
                eventName,
                success: false,
     ...
     */
    replaceArgumentsPlaceholder(prompt, jsonInput) {
     ...
     */
    isReasoningModel(model, reasoningConfig) {
     ...
     */
    async executeWithTimeout(prompt, model, timeoutMs, signal) {
        ...
        try {
            ...
            if (!generator) {
                const error = new Error('ContentGenerator not available - make sure you are authenticated');
                debugLogger.error('Prompt hook failed: ContentGenerator not available', error);
                throw error;
            }
            ...
            const isReasoningModel = this.isReasoningModel(requestModel, reasoningConfig);
            const requestConfig = {
            ...
            if (finishReason === 'MAX_TOKENS') {
                debugLogger.warn('LLM response truncated at maxOutputTokens — treating as unreliable');
    ...
    parseResponse(text) {
        ...
        try {
            ...
            if (!validated.success) {
                debugLogger.warn(`LLM response validation failed: ${validated.error.message}`);
     ...
     */
    processResult(hookConfig, eventName, response, duration) {
        const hookName = hookConfig.name || 'prompt-hook';
        if (!response.ok) {
            // Blocking decision
 ...
 */
export function createPromptHookRunner(config) {
    return new PromptHookRunner(config);
}
//# sourceMappingURL=promptHookRunner.js.map