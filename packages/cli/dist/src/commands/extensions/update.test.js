...
describe('extensions update command', () => {
    it('should fail if neither name nor --all is provided', () => {
        const validationParser = yargs([])
            ...
            .fail(false)
            .locale('en');
        expect(() => validationParser.parse('update')).toThrow('Either an extension name or --all must be provided');
    });
    it('should fail if both name and --all are provided', () => {
        const validationParser = yargs([])
            ...
            .fail(false)
            .locale('en');
        expect(() => validationParser.parse('update test-extension --all')).toThrow(/Arguments .* are mutually exclusive/);
    });
...
describe('handleUpdate', () => {
    ...
    describe('update by name', () => {
        it('should show message when extension is not found', async () => {
            ...
            expect(mockWriteStdoutLine).toHaveBeenCalledWith('Extension "non-existent-extension" not found.');
        });
        it('should show message when extension has no install metadata', async () => {
            ...
            expect(mockWriteStdoutLine).toHaveBeenCalledWith('Unable to install extension "test-extension" due to missing install metadata');
        });
        it('should show message when extension is already up to date', async () => {
            ...
            mockCheckForExtensionUpdate.mockResolvedValueOnce(ExtensionUpdateState.UP_TO_DATE);
            await handleUpdate({ name: 'test-extension' });
            expect(mockWriteStdoutLine).toHaveBeenCalledWith('Extension "test-extension" is already up to date.');
        });
        it('should update extension when update is available', async () => {
            ...
            mockGetLoadedExtensions.mockReturnValueOnce([mockExtension]);
            mockCheckForExtensionUpdate.mockResolvedValueOnce(ExtensionUpdateState.UPDATE_AVAILABLE);
            ...
            await handleUpdate({ name: 'test-extension' });
            expect(mockUpdateExtension).toHaveBeenCalledWith(mockExtension, ExtensionUpdateState.UPDATE_AVAILABLE, expect.any(Function));
            expect(mockWriteStdoutLine).toHaveBeenCalledWith('Extension "test-extension" successfully updated: 1.0.0 → 2.0.0.');
        });
        it('should surface committed update warnings', async () => {
            ...
            await handleUpdate({ name: 'test-extension' });
            expect(mockWriteStderrLine).toHaveBeenCalledWith('Extension "test-extension" updated with warning extension_settings_legacy_sync_failed: keychain unavailable');
        });
        ...
        it('should handle errors during update', async () => {
            ...
            mockGetLoadedExtensions.mockReturnValueOnce([mockExtension]);
            mockCheckForExtensionUpdate.mockRejectedValueOnce(new Error('Update check failed'));
            await handleUpdate({ name: 'test-extension' });
    ...
    describe('update all', () => {
        it('should show message when no extensions to update', async () => {
            ...
            expect(mockWriteStdoutLine).toHaveBeenCalledWith('No extensions to update.');
        });
        it('should update all extensions with updates available', async () => {
            ...
            await handleUpdate({ all: true });
            expect(mockWriteStdoutLine).toHaveBeenCalledWith('Extension "extension-1" successfully updated: 1.0.0 → 2.0.0.\n' +
        ...
        it('should handle errors during update all', async () => {
            mockCheckForAllExtensionUpdates.mockRejectedValueOnce(new Error('Update all failed'));
            await handleUpdate({ all: true });
