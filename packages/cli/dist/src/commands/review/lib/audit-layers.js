...
export const LAYER_RECEIPT_LINE_RE = /^[ \t]*(?:[-*+]|\d+[.)])?[ \t]*[*_~]{0,3}layer\s+walked[*_~]{0,3}[ \t]*[:：][\s*_~`]*([a-z][a-z0-9-]*)/i;
/**
...
const INVISIBLE_FORMAT = /[\p{Cf}\p{Variation_Selector}\u000B\u034F\u000C\u2028\u2029]/gu; // eslint-disable-line no-control-regex, no-misleading-character-class
/**
...
export function parseLayerReceipts(finalText, taxonomy = SHELL_MODEL_LAYERS) {
    ...
    for (const line of usedLines(finalText)) {
        ...
        if (TRAILING_STITCH.test(line.slice(m[0].length)))
            continue;
 ...
 * matching rule to `.lailatulcoder/review-context.json` that emits this domain when the
