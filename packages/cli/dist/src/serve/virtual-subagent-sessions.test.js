...
import { getSubagentSessionDir, Storage, } from '@lailatul-coder/lailatul-coder-core';
import { createVirtualSubagentSessionId, parseVirtualSubagentSessionId, preferTerminalTaskStatus, VirtualSubagentSessions, } from './virtual-subagent-sessions.js';
const tempDirs = [];
it.each(['running', 'paused'])('keeps a terminal task status over non-terminal %s metrics', (metricsStatus) => {
    expect(preferTerminalTaskStatus(metricsStatus, 'completed')).toBe('completed');
});
afterEach(async () => {
...
function record(uuid, parentUuid, type, text) {
    return {
        uuid,
        parentUuid,
...
function activeTarget(sessions) {
    const targets = sessions.targets;
    const target = targets.values().next().value;
    if (!target)
        throw new Error('Expected an active virtual subagent target');
    return target;
}
describe('VirtualSubagentSessions', () => {
    it('rejects invalid, oversized, or non-round-trippable id parts', () => {
        ...
        expect(() => createVirtualSubagentSessionId('parent-session', '')).toThrow('valid id parts');
        expect(() => createVirtualSubagentSessionId('parent-session', 'a'.repeat(501))).toThrow('valid id parts');
        expect(() => createVirtualSubagentSessionId('parent-session', '界'.repeat(493))).toThrow('exceeds 2000 characters');
        expect(() => createVirtualSubagentSessionId('parent-session', '\ud800')).toThrow('valid id parts');
    });
    it.each([
        ['a'.repeat(500), 696],
        [`${'界'.repeat(492)}aa`, 2_000],
    ])('round-trips an agent id at an accepted length boundary', (agentId, expectedSessionIdLength) => {
    ...
    it.each([
        ...
        const sessionId = createVirtualSubagentSessionId('parent-session', 'agent-1');
        ...
        expect(parseVirtualSubagentSessionId(nonCanonicalSessionId)).toBeUndefined();
    });
    it('rejects a non-canonical agent encoding that decodes as garbage', () => {
        const sessionId = createVirtualSubagentSessionId('parent-session', 'agent-1');
        const [parentPart] = sessionId.slice('subagent.'.length).split('.');
        expect(parseVirtualSubagentSessionId(`subagent.${parentPart}.garbage`)).toBeUndefined();
    });
    it('rejects a parent part outside the strict charset', () => {
        ...
        expect(parseVirtualSubagentSessionId(`subagent.${parentPart}.${agentPart}`)).toBeUndefined();
    });
    it('rejects an oversized parent part above the part length cap', () => {
        const parentPart = Buffer.from('a'.repeat(600), 'utf8').toString('base64url');
        const agentPart = Buffer.from('agent-1', 'utf8').toString('base64url');
        expect(parseVirtualSubagentSessionId(`subagent.${parentPart}.${agentPart}`)).toBeUndefined();
    });
    it('rejects an oversized session id at the parse-side total length cap', () => {
        ...
        const parentPart = Buffer.from('a'.repeat(500), 'utf8').toString('base64url');
        const agentPart = Buffer.from('界'.repeat(500), 'utf8').toString('base64url');
    ...
    it.each(['general-purpose-agent:8', 'general-purpose-agent/8'])('round-trips an existing agent id containing reserved characters: %s', (agentId) => {
        const sessionId = createVirtualSubagentSessionId('parent-session', agentId);
        expect(parseVirtualSubagentSessionId(sessionId)).toEqual({
            parentSessionId: 'parent-session',
            agentId,
    ...
    it.each(['fork-agent-1', 'general-purpose-agent:8'])('resolves an out-of-band task by agent task id: %s', async (taskId) => {
        const runtime = {
        ...
        const resolved = await new VirtualSubagentSessions().resolve(runtime, 'parent-session', taskId);
    ...
    it('resolves, fully loads, and independently streams an agent transcript', async () => {
        ...
        await fs.writeFile(outputFile, `${JSON.stringify(record('one', null, 'user', 'task'))}\n${JSON.stringify({
            ...record('two', 'one', 'assistant', 'first'),
            timestamp: new Date(1_000).toISOString(),
            agentRunId: 'run-one',
            agentRound: 1,
        })}\n`);
        await fs.writeFile(`${outputFile}.stream`, `${JSON.stringify({
            v: 1,
            runId: 'run-one',
            round: 1,
        ...
        const resolved = await sessions.resolve(runtime, 'parent-session', 'call-1');
        ...
        expect(JSON.stringify(loaded?.compactedReplay)).toContain('already streaming');
        expect(JSON.stringify(loaded?.compactedReplay)).not.toContain('completed round duplicate');
        ...
        expect(JSON.stringify(reloaded?.compactedReplay)).toContain('resumed round live');
    ...
    it('releases the subscriber count when the initial refresh fails', async () => {
        ...
        const sessions = new VirtualSubagentSessions();
        const sessionId = createVirtualSubagentSessionId('parent-session', 'agent-error');
    ...
    it('isolates cached targets by workspace runtime', async () => {
        const dir = await fs.mkdtemp(path.join(os.tmpdir(), 'qwen-subagent-'));
        tempDirs.push(dir);
        const makeRuntime = async (workspaceId, text) => {
            const outputFile = path.join(dir, `${workspaceId}.jsonl`);
            await fs.writeFile(outputFile, `${JSON.stringify(record('one', null, 'user', text))}\n`);
            await fs.writeFile(`${outputFile}.stream`, `${JSON.stringify({
                v: 1,
                round: 1,
        ...
        expect(JSON.stringify(second?.compactedReplay)).toContain('second workspace');
        expect(JSON.stringify(second?.compactedReplay)).not.toContain('first workspace');
        expect(JSON.stringify(second?.compactedReplay)).not.toContain('stale stream');
    });
    it('keeps later canonical rounds when one streamed round is reconciled', async () => {
        ...
        const sessions = new VirtualSubagentSessions();
        const resolved = await sessions.resolve(runtime, 'parent-session', 'call-batch');
        ...
        const iterator = stream[Symbol.asyncIterator]();
        expect((await iterator.next()).value?.type).toBe('replay_complete');
        await fs.appendFile(outputFile, `${JSON.stringify({
    ...
    it('does not replay a second load snapshot again on subscribe', async () => {
        ...
        const sessions = new VirtualSubagentSessions();
        const resolved = await sessions.resolve(runtime, 'parent-session', 'call-reload');
        ...
        const loaded = await sessions.load(runtime, resolved.sessionId);
        expect(JSON.stringify(loaded.compactedReplay)).toContain('between snapshots');
        await fs.writeFile(`${outputFile}.stream`, `${JSON.stringify({
            v: 1,
            runId: 'run-reload',
            round: 1,
    ...
    it('keeps task status while supplementing terminal metrics', async () => {
        const runtimeDir = await fs.mkdtemp(path.join(os.tmpdir(), 'qwen-subagent-runtime-'));
        tempDirs.push(runtimeDir);
        const workspaceCwd = path.join(runtimeDir, 'workspace');
        const projectDir = Storage.runWithRuntimeBaseDir(runtimeDir, workspaceCwd, () => new Storage(workspaceCwd).getProjectDir());
        ...
        await fs.writeFile(path.join(projectDir, 'chats', `${parentSessionId}.jsonl`), `${JSON.stringify({
        ...
        const sessions = new VirtualSubagentSessions();
        const resolved = await sessions.resolve(runtime, parentSessionId, toolCallId);
        expect(resolved).toMatchObject({
            status: 'running',
            durationMs: 500,
            totalTokens: 123,
        ...
        await fs.appendFile(outputFile, `${JSON.stringify(record('final', 'child', 'assistant', 'final canonical output'))}\n`);
        expect(await sessions.resolve(runtime, parentSessionId, toolCallId)).toMatchObject({ status: 'completed', totalTokens: 999 });
        await activeTarget(sessions).refreshLive();
        const finalUpdate = await iterator.next();
        expect(JSON.stringify(finalUpdate?.value)).toContain('final canonical output');
        await fs.appendFile(`${outputFile}.stream`, `${JSON.stringify({
            v: 1,
            round: 1,
    ...
    it('keeps terminal legacy sidecar status over the background launch result', async () => {
        ...
        const outputFile = path.join(sessionDir, 'agent-general-purpose-random.jsonl');
        await fs.writeFile(outputFile, `${JSON.stringify({
            ...record('child', null, 'user', prompt),
            sessionId: parentSessionId,
        })}\n`);
        await fs.writeFile(outputFile.replace(/\.jsonl$/, '.meta.json'), JSON.stringify({
        ...
        const resolved = await new VirtualSubagentSessions().resolve(runtime, parentSessionId, toolCallId);
        expect(parseVirtualSubagentSessionId(resolved.sessionId)?.agentId).toBe('general-purpose-random');
        expect(resolved).toMatchObject({
            status: 'completed',
            durationMs: 88_610,
            totalTokens: 1_234,
...
//# sourceMappingURL=virtual-subagent-sessions.test.js.map