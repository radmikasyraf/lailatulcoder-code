 ...
 */
export async function subagentGenerator(userDescription, config, abortSignal) {
    ...
    try {
        return await runSideQuery(config, {
            contents,
            ...
            validate: (response) => !response.name || !response.description || !response.systemPrompt
