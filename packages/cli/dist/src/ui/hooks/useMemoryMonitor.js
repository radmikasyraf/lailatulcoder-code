...
export const MEMORY_WARNING_THRESHOLD = Math.min(7 * 1024 * 1024 * 1024, Math.floor(os.totalmem() * 0.85));
export const MEMORY_UI_COMPACT_THRESHOLD = () => Math.floor(v8.getHeapStatistics().heap_size_limit * 0.65);
...
export const useMemoryMonitor = ({ addItem, compactOldItems, }) => {
    ...
    useEffect(() => {
        ...
        const debugIntervalId = setInterval(() => {
            ...
            if (debugLogger.isEnabled()) {
                debugLogger.debug(`[MEMORY_USAGE] ` +
            ...
            if (compactOldItems &&
                ...
                if (debugLogger.isEnabled()) {
                    debugLogger.debug(`[UI_COMPACT] heapUsed=${heapUsed.toFixed(1)}MB ` +
                ...
                try {
                    ...
                    debugLogger.error(`[UI_COMPACT] compactOldItems failed: ${err instanceof Error ? err.message : String(err)}`);
                }
            }
        ...
        const warningIntervalId = setInterval(() => {
            ...
            if (usage.rss > MEMORY_WARNING_THRESHOLD) {
                debugLogger.warn(`[MEMORY_WARNING] High memory usage detected: ${(usage.rss / (1024 * 1024 * 1024)).toFixed(2)} GB`);
                addItem({
                    type: MessageType.WARNING,
                    text: `High memory usage detected: ${(usage.rss /
