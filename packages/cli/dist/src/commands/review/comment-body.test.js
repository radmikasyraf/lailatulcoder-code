...
const { ghApiMock, ensureAuthenticatedMock, setGhHostMock, writeStdoutLineMock, writeStderrLineSafeMock, writeFileSyncMock, mkdirSyncMock, } = vi.hoisted(() => ({
    ghApiMock: vi.fn(),
...
describe('runCommentBody', () => {
    ...
    it('fetches an inline comment body from the parsed JSON (no --jq newline)', () => {
        ...
        expect(ghApiMock).toHaveBeenCalledWith('repos/LailatulCoder/lailatul-coder/pulls/comments/3773970278');
    ...
    it('returns an empty string for a null body', () => {
        ...
        expect(runCommentBody({ id: 1, kind: 'inline', repo: 'LailatulCoder/lailatul-coder' }).body).toBe('');
    });
    it('fetches an issue comment body', () => {
        ...
        expect(ghApiMock).toHaveBeenCalledWith('repos/LailatulCoder/lailatul-coder/issues/comments/5277891862');
    });
    it('addresses review bodies per-PR and refuses without one', () => {
        expect(() => runCommentBody({ id: 1, kind: 'review', repo: 'LailatulCoder/lailatul-coder' })).toThrow(TypeError);
        ...
        expect(ghApiMock).toHaveBeenCalledWith('repos/LailatulCoder/lailatul-coder/pulls/9073/reviews/99');
    });
    it('writes --out instead of returning the body inline', () => {
        ...
        expect(mkdirSyncMock).toHaveBeenCalledWith(dirname(resolve('/tmp/body.md')), { recursive: true });
        expect(writeFileSyncMock).toHaveBeenCalledWith(resolve('/tmp/body.md'), 'long tail');
...
describe('commentBodyCommand handler', () => {
    ...
    it('--out prints the JSON marker, not the raw body', () => {
        ...
        expect(writeStdoutLineMock).toHaveBeenCalledWith(JSON.stringify({
...
//# sourceMappingURL=comment-body.test.js.map