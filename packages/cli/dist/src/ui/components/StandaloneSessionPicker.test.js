...
function createMockSessionService(sessions = [], hasMore = false) {
    return {
...
describe('SessionPicker', () => {
    ...
    describe('Empty Sessions', () => {
        it('should show sessions with 0 messages', async () => {
            ...
            const { lastFrame } = render(_jsx(KeypressProvider, { kittyProtocolEnabled: false, children: _jsx(SessionPicker, { sessionService: mockService, onSelect: onSelect, onCancel: onCancel }) }));
            await flush();
    ...
    describe('Branch Filtering', () => {
        it('should filter by branch when Ctrl+B is pressed', async () => {
            ...
            const onSelect = vi.fn();
            const onCancel = vi.fn();
            const { lastFrame, stdin } = render(_jsx(KeypressProvider, { kittyProtocolEnabled: false, children: _jsx(SessionPicker, { sessionService: mockService, onSelect: onSelect, onCancel: onCancel, currentBranch: "main" }) }));
            await flush();
    ...
    describe('Keyboard Navigation', () => {
        it('should type j and k into the explicit search buffer', async () => {
            ...
            const onSelect = vi.fn();
            const onCancel = vi.fn();
            const { lastFrame, stdin } = render(_jsx(KeypressProvider, { kittyProtocolEnabled: false, children: _jsx(SessionPicker, { sessionService: mockService, onSelect: onSelect, onCancel: onCancel }) }));
            await flush();
            stdin.write('/');
            await flush();
            stdin.write('j');
        ...
        it('should select session on Enter', async () => {
            ...
            const onSelect = vi.fn();
            const onCancel = vi.fn();
            const { stdin } = render(_jsx(KeypressProvider, { kittyProtocolEnabled: false, children: _jsx(SessionPicker, { sessionService: mockService, onSelect: onSelect, onCancel: onCancel }) }));
            await flush();
    ...
    describe('Display', () => {
        ...
        it('renders the metadata line cleanly when messageCount is undefined', async () => {
            ...
            const mockService = createMockSessionService(sessions);
            const { lastFrame } = render(_jsx(KeypressProvider, { kittyProtocolEnabled: false, children: _jsx(SessionPicker, { sessionService: mockService, onSelect: vi.fn(), onCancel: vi.fn() }) }));
            await flush();
            const output = lastFrame() ?? '';
        ...
        it('should show branch toggle hint when currentBranch is provided', async () => {
            ...
            const onSelect = vi.fn();
            const onCancel = vi.fn();
            const { lastFrame } = render(_jsx(KeypressProvider, { kittyProtocolEnabled: false, children: _jsx(SessionPicker, { sessionService: mockService, onSelect: onSelect, onCancel: onCancel, currentBranch: "main" }) }));
            await flush();
            const output = lastFrame();
    ...
    describe('Pagination', () => {
        it('should load more sessions when scrolling to bottom', async () => {
            const firstPage = Array.from({ length: 5 }, (_, i) => createMockSession({
                sessionId: `session-${i}`,
                prompt: `Session ${i}`,
                messageCount: 1,
                mtime: Date.now() - i * 1000,
            }));
            const secondPage = Array.from({ length: 3 }, (_, i) => createMockSession({
            ...
            const onSelect = vi.fn();
            const onCancel = vi.fn();
            const { unmount } = render(_jsx(KeypressProvider, { kittyProtocolEnabled: false, children: _jsx(SessionPicker, { sessionService: mockService, onSelect: onSelect, onCancel: onCancel }) }));
            await flush();
    ...
    describe('Preview Mode', () => {
        ...
        function renderPicker(children) {
            return render(_jsx(KeypressProvider, { kittyProtocolEnabled: false, children: _jsx(ConfigContext.Provider, { value: PREVIEW_CONFIG_STUB, children: _jsx(SettingsContext.Provider, { value: PREVIEW_SETTINGS_STUB, children: children }) }) }));
        }
        function fakeResumedData(sessionId) {
            return {
                conversation: {
                    sessionId,
                    projectHash: 'h',
        ...
        it('renders tool_group items without crashing (stub Providers mounted)', async () => {
            ...
            const { stdin, lastFrame } = renderPicker(_jsx(SessionPicker, { sessionService: service, onSelect: vi.fn(), onCancel: vi.fn(), enablePreview: true }));
            await act(async () => {
        ...
        it('Enter inside preview fires onSelect with previewed sessionId', async () => {
            ...
            const onSelect = vi.fn();
            const { stdin } = renderPicker(_jsx(SessionPicker, { sessionService: service, onSelect: onSelect, onCancel: vi.fn(), enablePreview: true }));
            await flush();
        ...
        it('without enablePreview, Space is a no-op and footer omits the hint', async () => {
            ...
            service.loadSession.mockResolvedValue(fakeResumedData('s1'));
            const onSelect = vi.fn();
            const { stdin, lastFrame } = renderPicker(_jsx(SessionPicker, { sessionService: service, onSelect: onSelect, onCancel: vi.fn() }));
            await flush();
...
//# sourceMappingURL=StandaloneSessionPicker.test.js.map