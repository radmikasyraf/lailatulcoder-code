 ...
 *
 * All file operations target `~/.lailatulcoder/teams/{team-name}/config.json`.
...
import { TEAMS_DIR, TEAM_CONFIG_FILENAME, TEAMMATE_COLORS, INBOXES_DIR, TASKS_DIR, LEADER_NAME, } from './types.js';
// ─── Path helpers ───────────────────────────────────────────
/**
 * Absolute path to the teams root directory.
 * `~/.lailatulcoder/teams/`
 */
export function getTeamsRootDir() {
    return path.join(Storage.getGlobalQwenDir(), TEAMS_DIR);
}
/**
 * Absolute path to a specific team's directory.
 * `~/.lailatulcoder/teams/{teamName}/`
 */
 ...
 * `~/.lailatulcoder/teams/{teamName}/config.json`
 */
 ...
 * `~/.lailatulcoder/teams/{teamName}/inboxes/`
 */
export function getInboxesDir(teamName) {
    return path.join(getTeamDir(teamName), INBOXES_DIR);
}
/**
 * Absolute path to the tasks directory for a team.
 * `~/.lailatulcoder/tasks/{teamName}/`
 */
export function getTasksDir(teamName) {
 ...
 */
export function generateUniqueTeammateName(baseName, existingMembers) {
    ...
    if (!sanitized) {
        throw new Error(`Teammate name "${baseName}" sanitizes to an empty string. ` +
            `Choose a name with at least one alphanumeric character.`);
    }
    if (sanitized === LEADER_NAME) {
        throw new Error(`"${LEADER_NAME}" is reserved for the team leader. ` +
    ...
    if (existingNames.has(sanitized)) {
        ...
        throw new Error(`A teammate named "${sanitized}" already exists in this team ` +
 ...
 */
export function assignTeammateColor(existingMembers) {
    const usedColors = new Set(existingMembers
        .map((m) => m.color)
 ...
 */
export function clearTeammateColors(members) {
 ...
 * Returns a new members array (does not mutate).
 */
export function setMemberActive(members, agentId, isActive) {
    return members.map((m) => (m.agentId === agentId ? { ...m, isActive } : m));
}
/**
 * Find a member by agent ID.
 */
export function findMemberById(members, agentId) {
 ...
 */
export function classifyShutdownResponse(message) {
    const trimmed = message.trimStart();
    if (/^shutdown_approved\b/i.test(trimmed))
        return 'shutdown_approved';
    if (/^shutdown_rejected\b/i.test(trimmed))
        return 'shutdown_rejected';
    return undefined;
}
// ─── File I/O ───────────────────────────────────────────────
/**
 ...
 */
export async function writeTeamFile(teamName, teamFile) {
    const filePath = getTeamFilePath(teamName);
 ...
 */
export async function createTeamFile(teamName, teamFile) {
    const filePath = getTeamFilePath(teamName);
    await fs.mkdir(path.dirname(filePath), { recursive: true });
 ...
 * for manual recovery.
 */
export async function tryReclaimStaleTeam(teamName) {
    let existing;
    try {
 ...
 */
export async function deleteTeamDirs(teamName) {
...
/**
 * List all team names (directory names under ~/.lailatulcoder/teams/).
