...
export function mountWorkspaceGenerationRoutes(app, deps) {
    app.post('/workspace/generate', deps.mutate(), async (req, res) => {
        ...
        const stream = deps.bridge.generateWorkspaceContent(prompt.trim(), abort.signal, clientId);
        res.status(200);
        ...
        try {
            ...
            if (!abort.signal.aborted && !res.destroyed) {
                writeStderrLine(`LailatulCoder serve: POST /workspace/generate failed: ${error instanceof Error
...
//# sourceMappingURL=workspace-generation.js.map