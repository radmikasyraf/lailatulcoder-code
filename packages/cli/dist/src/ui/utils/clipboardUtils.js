 ...
 */
export function writeOsc52(text) {
    try {
        ...
        if (Buffer.byteLength(text, 'utf-8') > MAX_OSC52_BYTES) {
            debugLogger.warn(`writeOsc52: text too large (${Buffer.byteLength(text, 'utf-8')} bytes), skipping`);
            return false;
        }
        ...
        const stream = process.stderr.isTTY
            ...
            : process.stdout.isTTY
                ...
                : null;
        if (!stream) {
            debugLogger.warn('OSC 52 clipboard requires a TTY; stdout/stderr not connected to terminal');
            return false;
        }
        stream.write(sequence, (err) => {
            if (err)
                debugLogger.warn('writeOsc52: async write failed:', err);
        });
        return true;
    }
    catch (e) {
...
async function getClipboardModule() {
    if (!clipboardModulePromise) {
        ...
        clipboardModulePromise = import(modName).catch(() => {
            debugLogger.error('Failed to load @teddyzhu/clipboard native module. Clipboard image features will be unavailable.');
            return null;
        });
    }
 ...
 */
export function resetLinuxClipboardTool() {
...
function getLinuxClipboardTool() {
    if (linuxClipboardTool !== undefined)
...
async function saveFromCommand(command, args, destination) {
    ...
    let fd;
    try {
        fd = await fs.open(destination, fsConstants.O_WRONLY | fsConstants.O_CREAT | fsConstants.O_EXCL);
    }
    catch {
        return false;
    }
 ...
 */
export async function clipboardHasImage(onUnavailable) {
...
async function saveFileWithWlPaste(tempFilePath) {
    ...
    if (imageTypes.includes('image/bmp')) {
        ...
        const bmpSuccess = await saveFromCommand('wl-paste', ['--no-newline', '--type', 'image/bmp'], bmpPath);
        if (bmpSuccess) {
            try {
                await new Promise((resolve, reject) => {
                    ...
                    child.on('close', (code) => {
                        ...
                        if (code === 0)
                            ...
                            reject(new Error(`python3 exited with code ${code}${stderr ? ': ' + stderr.trim() : ''}`));
                    });
                    child.on('error', (err) => {
                        clearTimeout(timer);
                        reject(err);
                ...
                debugLogger.warn('BMP-to-PNG conversion failed (install python3-pil for BMP support):', err);
                try {
                    await fs.unlink(bmpPath);
                }
                catch {
...
export async function saveClipboardImage(targetDir) {
    try {
        ...
        if (process.platform === 'linux') {
            const pngPath = path.join(tempDir, `clipboard-${timestamp}-${randomUUID()}.png`);
            const tool = getLinuxClipboardTool();
            if (tool === 'wl-paste') {
            ...
            if (tool === 'xclip') {
                if (await saveFileWithXclip(pngPath))
                    return pngPath;
                return null;
            }
            return null;
        ...
        const tempFilePath = path.join(tempDir, `clipboard-${timestamp}-${randomUUID()}.png`);
