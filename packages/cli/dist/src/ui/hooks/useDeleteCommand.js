...
export function useDeleteCommand(options) {
    ...
    const handleDeleteMany = useCallback(async (sessionIds) => {
        if (!config) {
            return;
        }
        if (isDeletingManyRef.current) {
        ...
        try {
            ...
            if (removedCount > 0 && failedCount === 0) {
                ...
                addItem?.({
                    type: 'error',
                    text: t('Deleted {{removed}} session(s); {{failed}} could not be deleted ({{ids}}{{overflow}}){{reason}}.', {
                ...
                addItem?.({
                    type: 'error',
                    text: t('Failed to delete {{failed}} session(s) ({{ids}}{{overflow}}){{reason}}.', {
                        failed: String(failedCount),
                        ids: sampleIds,
                        overflow,
                        reason,
