...
import { collectMemoryPressureSamples, formatMemoryDiagnostics, formatMemoryPressureSamples, getMemoryDiagnostics, isHighHeapPressure, writeMemoryHeapSnapshot, } from '../../utils/memoryDiagnostics.js';
import { isCpuProfileRecording, startCpuProfile, stopCpuProfile, } from '../../utils/cpuProfiler.js';
...
import { collectMemoryDiagnostics, analyzeToolResultRetention, canonicalToolName, COMBINED_PASS_TOLERANCE_FACTOR, createDebugLogger, resolveSlimmingConfig, ToolDisplayNamesMigration, } from '@lailatul-coder/lailatul-coder-core';
...
function getHeapSnapshotSensitiveDataWarning() {
    return t('Heap snapshot may contain prompts, file contents, tool results, and other sensitive data. Do not share it publicly without reviewing it first.');
}
...
export const doctorCommand = {
    ...
    completion: async (_context, partialArg) => {
        ...
        return DOCTOR_SUBCOMMANDS.filter((candidate) => candidate.startsWith(trimmed));
    },
    action: async (context, args) => {
        const executionMode = context.executionMode ?? 'interactive';
        const abortSignal = context.abortSignal;
        const subCommandArgs = args?.trim().toLowerCase().split(/\s+/).filter(Boolean) ?? [];
            ...
            if (shouldWriteHeapSnapshot) {
                ...
                try {
                    ...
                    if (isHighHeapPressure(latestDiagnostics)) {
                        throw new Error(t('Heap snapshot skipped: V8 heap pressure is already high, and writing a synchronous heap snapshot could make the process unresponsive or trigger OOM. Restart LailatulCoder Ai first if it is unstable, or retry before memory pressure reaches the warning threshold.'));
                    }
        ...
        try {
            ...
            return {
                type: 'message',
                messageType: (summary.fail > 0 ? 'error' : 'info'),
...
async function memoryDoctorAction(context, args = '') {
    ...
    const unknown = tokens.filter((token) => token !== '--json' && token !== '--sample' && token !== '--snapshot');
    if (unknown.length > 0) {
        return {
            type: 'message',
            messageType: 'error',
    ...
    if (shouldSampleMemory || shouldWriteHeapSnapshot) {
        return doctorCommand.action?.(context, [MEMORY_SUBCOMMAND, ...tokens.filter((token) => token !== '--json')].join(' '));
    }
    try {
        ...
        return {
            type: 'message',
            messageType: diagnostics.analysis.risks.length > 0
                ? 'warning'
                : 'info',
...
function formatHeapSpaces(spaces) {
    ...
    const lines = top.map((space) => `  - ${space.name}: used ${formatMemoryUsage(space.used)} / size ${formatMemoryUsage(space.size)}`);
...
function formatCoreDiagnostics(diagnostics) {
    const risks = diagnostics.analysis.risks.length > 0
...
function collectToolResultRetention(context) {
    try {
        ...
        const resolveToolBudgetChars = (name) => config?.getToolRegistry?.()?.getTool(canonicalToolName(name))
            ?.maxOutputChars;
        ...
        const displayNameBudgets = new Map();
        ...
        for (const item of context.ui.history ?? []) {
            ...
            for (const tool of item.tools) {
                ...
                const migratedName = ToolDisplayNamesMigration[tool.name] ?? tool.name;
                const budget = displayNameBudgets.get(tool.name) ??
        ...
        debugLogger.warn(`Tool result retention diagnostics unavailable: ${formatErrorMessage(error)}`);
        return null;
    }
}
function formatToolResultRetention(stats) {
    const lines = [
    ...
    if (stats.oversizedResultCount > 0) {
        lines.push('  Hint: oversized tool results stay in context and tax every later turn; /compress can reclaim space.');
    }
    return lines.join('\n');
}
// ---------------------------------------------------------------------------
...
function parseCpuProfileDuration(raw) {
    if (!raw || !/^\d+$/.test(raw)) {
        return NaN;
    }
    return Number(raw);
}
async function cpuProfileDoctorAction(context, args = '') {
    const executionMode = context.executionMode ?? 'interactive';
    const abortSignal = context.abortSignal;
    if (abortSignal?.aborted)
        return;
    ...
    const unknown = tokens.filter((token, idx) => {
        ...
        if (idx > 0 && tokens[idx - 1] === '--duration')
            return false;
        return true;
    });
    if (unknown.length > 0) {
    ...
    if (isCpuProfileRecording()) {
        const errorMsg = process.platform === 'win32'
            ...
            : t('CPU profiling is already in progress. Send SIGUSR1 or wait for it to complete.');
        if (executionMode === 'interactive') {
            context.ui.addItem({ type: 'error', text: errorMsg }, Date.now());
            return;
        }
    ...
    if (!startResult.ok) {
        if (executionMode === 'interactive') {
            context.ui.addItem({ type: 'error', text: startResult.error }, Date.now());
            return;
        }
        return {
            type: 'message',
    ...
    if (!stopResult.ok) {
        ...
        if (alreadyStopped) {
            const infoMsg = process.platform === 'win32'
                ? t('CPU profile was stopped externally. Check ~/.lailatulcoder/cpu-profiles/ for the output.')
                : t('CPU profile was stopped externally (e.g., via SIGUSR1). Check ~/.lailatulcoder/cpu-profiles/ for the output.');
            if (executionMode === 'interactive') {
...
function rollbackDoctorAction(context) {
    ...
    if (process.platform === 'win32') {
        const winMsg = t('Rollback on Windows requires manual intervention. Rename lailatul-coder.old to lailatul-coder in your installation directory.');
        if (context.executionMode === 'interactive') {
    ...
    let messageType;
    if (result.ok) {
        msg = t('Rollback successful. Restart your terminal to use the previous version.');
        messageType = 'info';
    }
    else {
