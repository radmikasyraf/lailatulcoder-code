...
import { ASSET_EXTENSIONS, ASSET_HEADER_BYTES, MAX_ASSET_BYTES, MAX_TOTAL_ASSET_BYTES, assetsBranch, parseAssetsRepo, rawAssetUrl, remoteAssetPath, sniffImageFormat, validateAssetBatch, validateAssetContent, validateAssetFile, } from './assets.js';
...
describe('remoteAssetPath', () => {
    ...
    it('is idempotent for identical content — re-runs dedupe naturally', () => {
        expect(remoteAssetPath(7, 'a.png', sha)).toBe(remoteAssetPath(7, 'a.png', sha));
    });
    it('sanitizes a hostile basename before it reaches a URL or git path', () => {
        expect(remoteAssetPath(7, 'a b/../$(x).png', sha)).toBe(`7-review/${sha.slice(0, 12)}-a_b_.._$_x_.png`.replace('$', '_'));
    });
});
...
describe('validateAssetFile', () => {
    ...
    it('refuses an empty file and an oversized one, naming the cap', () => {
        ...
        if (!big.ok)
            expect(big.reason).toContain(String(MAX_ASSET_BYTES));
    });
});
describe('validateAssetBatch', () => {
    it('refuses a batch whose files individually pass but jointly exceed the cap', () => {
        ...
        expect(r.ok).toBe(false);
        if (!r.ok)
            expect(r.reason).toContain(String(MAX_TOTAL_ASSET_BYTES));
    ...
    it('surfaces a per-file refusal with the file named', () => {
        ...
        expect(r.ok).toBe(false);
        if (!r.ok)
            expect(r.reason).toContain('evil.svg');
    });
});
describe('sniffImageFormat / validateAssetContent', () => {
    ...
    const WEBP = Uint8Array.from([...'RIFF\u0000\u0000\u0000\u0000WEBPVP8 '].map((c) => c.charCodeAt(0)));
    it('recognizes the four admitted signatures', () => {
        ...
        for (const fourcc of ['VP8L', 'VP8X']) {
            expect(sniffImageFormat(Uint8Array.from([...`RIFF\u0000\u0000\u0000\u0000WEBP${fourcc}`].map((c) => c.charCodeAt(0))))).toBe('webp');
        }
    });
    it('returns null for non-images, truncated or near-miss headers, and empty input', () => {
        ...
        expect(sniffImageFormat(Uint8Array.from([...'RIFF\u0000\u0000\u0000\u0000WEBPALPH'].map((c) => c.charCodeAt(0))))).toBeNull();
        // A WEBP marker without the RIFF container prefix is not WEBP either,
        // and an unknown GIF version is not admitted.
        expect(sniffImageFormat(Uint8Array.from([...'XXXX0000WEBP'].map((c) => c.charCodeAt(0))))).toBeNull();
        expect(sniffImageFormat(Uint8Array.from([...'GIF89b'].map((c) => c.charCodeAt(0))))).toBeNull();
    });
    ...
    const sig = (s) => [...s].map((c) => c.charCodeAt(0));
    const corruptedPng = (i, b) => {
    ...
    it.each(ONE_BYTE_OFF)('refuses %s — one byte off a real signature', (_label, header) => {
    ...
    it('refuses a name whose content is a different format — or no image at all', () => {
        ...
        if (!r1.ok)
            expect(r1.reason).toContain('content is not a recognized image but the extension claims png');
        const r2 = validateAssetContent('mislabeled.png', JPEG);
        expect(r2.ok).toBe(false);
        // Direction matters to the operator: what the content IS comes first.
        if (!r2.ok)
            expect(r2.reason).toContain('content is jpeg but the extension claims png');
    });
    it('fails closed on an extension outside the allowlist', () => {
        ...
        if (!proto.ok)
            expect(proto.reason).toContain('is not an allowed');
    });
    ...
    it.each([
        ...
        const reason = `${name}: extension "${ext}" is not an allowed ` +
    ...
    it('admits every allowed extension by content — the two gates agree', () => {
        ...
        for (const ext of ASSET_EXTENSIONS) {
            ...
            expect({
                ext,
                admittedByContent: bytes !== undefined &&
                    validateAssetContent(`probe.${ext}`, bytes.subarray(0, ASSET_HEADER_BYTES)).ok,
