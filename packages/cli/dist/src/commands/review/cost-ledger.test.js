...
import { chmodSync, existsSync, mkdtempSync, mkdirSync, readFileSync, rmSync, symlinkSync, utimesSync, writeFileSync, } from 'node:fs';
import { tmpdir } from 'node:os';
import { join } from 'node:path';
import { computeLedger, renderLedger, costLedgerCommand, } from './cost-ledger.js';
import { appendRunSession, recordResume } from './lib/run-ledger.js';
const SESSION = 'S-ledger';
function event(timestamp, usage, extra = {}) {
...
describe('cost-ledger — the spend, from the records already on disk', () => {
     ...
     */
    function writeMainCall(project) {
        writeFileSync(chatFile(project), event('2026-08-03T10:01:00Z', { input: 500, output: 50 }));
    }
    it('aggregates the main loop and each agent, newest records only', () => {
        const { plan, env, project } = fixture();
        writeFileSync(chatFile(project), [
        ...
        writeFileSync(join(project, 'subagents', SESSION, 'agent-general-purpose-a1.jsonl'), [
    ...
    it('renders a one-line summary a reader can act on', () => {
        const { plan, env, project } = fixture();
        writeFileSync(chatFile(project), event('2026-08-03T10:01:00Z', {
            input: 1_200_000,
            cached: 600_000,
            output: 10_000,
            thoughts: 5_000,
    ...
    it('refuses an empty window even when no agents ran', () => {
        ...
        expect(() => computeLedger(plan, env)).toThrow(/no main-loop usage records at or after the plan/);
    });
    it('throws TranscriptsUnavailable through to the caller when the env is bare', () => {
        const { plan } = fixture();
        expect(() => computeLedger(plan, {})).toThrow(/QWEN_CODE_PROJECT_DIR/);
    });
    it('names a missing plan as the plan, not the usage records', () => {
        const { env } = fixture();
        expect(() => computeLedger('/nonexistent/plan.json', env)).toThrow(/could not read the plan report/);
    });
    it('rejects an existing file that is not the plan report', () => {
        ...
        expect(() => computeLedger(notPlan, env)).toThrow(/not a review plan report/);
    ...
    it('accepts a degraded diff-less plan — its mtime is still the floor', () => {
        ...
        writeFileSync(plan, JSON.stringify({ diffPathAbsolute: null, diffLines: 0, chunks: [] }));
        const start = new Date('2026-08-03T10:00:00Z');
        utimesSync(plan, start, start);
    ...
    it('refuses to render agents-only totals when the chat transcript is missing', () => {
        ...
        writeFileSync(join(project, 'subagents', SESSION, 'agent-role-1.jsonl'), [
            userRecord('You are review agent `1` — dimension 1.'),
            event('2026-08-03T10:06:30Z', { input: 1_000, output: 100 }),
        ].join('\n'));
        expect(() => computeLedger(plan, env)).toThrow(/could not read the chat transcript/);
    });
    it('refuses agents-only totals when the chat file exists but is empty', () => {
        ...
        writeFileSync(chatFile(project), event('2026-08-03T09:00:00Z', { input: 1_000, output: 100 }));
        expect(() => computeLedger(plan, env)).toThrow(/no main-loop usage records at or after the plan/);
    });
    it('loses only the unreadable agent transcript, not the whole ledger', () => {
        ...
        writeFileSync(join(project, 'subagents', SESSION, 'agent-role-ok.jsonl'), [
    ...
    it('ignores the harness sidecar files beside the transcripts', () => {
        ...
        writeFileSync(join(project, 'subagents', SESSION, 'agent-role-1.meta.json'), JSON.stringify({
            agentId: 'role-1',
        ...
        writeFileSync(join(project, 'subagents', SESSION, 'agent-role-1.jsonl.stream'), 'streaming text, not jsonl records');
        const ledger = computeLedger(plan, env);
        expect(ledger.agents).toHaveLength(1);
    ...
    it('surfaces an unreadable subagent directory instead of main-loop-only totals', () => {
        const { plan, env, project } = fixture();
        writeFileSync(chatFile(project), [event('2026-08-03T10:01:00Z', { input: 1_000, output: 100 })].join('\n'));
        ...
        expect(() => computeLedger(plan, env)).toThrow(/could not list the subagent transcripts/);
    });
    ...
    it('rounds 999.5k up to 1.0M, not 1000k', () => {
        const { plan, env, project } = fixture();
        writeFileSync(chatFile(project), event('2026-08-03T10:01:00Z', { input: 999_500, output: 100 }));
        expect(renderLedger(computeLedger(plan, env))).toContain('1.0M input');
    });
    it('renders billions with a B tier, not 1500.0M', () => {
        const { plan, env, project } = fixture();
        writeFileSync(chatFile(project), event('2026-08-03T10:01:00Z', { input: 1_500_000_000, output: 100 }));
    ...
    it('rounds fractional tiers to the nearest unit, not down', () => {
        ...
        expect(renderLedger(computeLedger(plan, env))).toContain('46k input (13% cached)');
    });
    ...
    it('labels a chunk agent from its identity line, not as "agent chunk …"', () => {
        const { plan, env, project } = fixture();
        writeMainCall(project);
        writeFileSync(join(project, 'subagents', SESSION, 'agent-chunk-c3.jsonl'), [
            userRecord('You are review agent `chunk 3 of 5` — the territory agent for ' +
    ...
    it('labels a free-text chunk mention by the file id, not the quoted chunk', () => {
        ...
        writeFileSync(join(project, 'subagents', SESSION, 'agent-general-purpose-b7.jsonl'), [
    ...
    it('falls back to the file id when the prompt names neither role nor chunk', () => {
        const { plan, env, project } = fixture();
        writeMainCall(project);
        writeFileSync(join(project, 'subagents', SESSION, 'agent-general-purpose-z0.jsonl'), [
    ...
    it('distinguishes parallel invariant agents by full path, not basename', () => {
        ...
        for (const [file, owned] of [
            ...
            writeFileSync(join(project, 'subagents', SESSION, file), [
                userRecord('You are review agent `invariant-a` — the hot-path audit.' +
                    ` Your file: \`${owned}\`.`),
                event('2026-08-03T10:06:30Z', { input: 1_000, output: 100 }),
            ].join('\n'));
        }
        const text = renderLedger(computeLedger(plan, env));
        expect(text).toContain('agent invariant-a (packages/cli/src/config/storage.ts):');
        expect(text).toContain('agent invariant-a (packages/core/src/config/storage.ts):');
        expect(text).not.toContain('(×2)');
    });
    it('labels a fork agent from its launch prompt, not the bootstrap before it', () => {
        const { plan, env, project } = fixture();
        writeMainCall(project);
        writeFileSync(join(project, 'subagents', SESSION, 'agent-fork-f1.jsonl'), [
            ...
            JSON.stringify({
                ...
                systemPayload: {
                    ...
                    history: [
                        {
                            text: 'Earlier turn: You are review agent `chunk 1 of 5` — ' +
    ...
    it('orders agents by input, biggest first, regardless of file order', () => {
        ...
        writeFileSync(join(project, 'subagents', SESSION, 'agent-role-a.jsonl'), [
            userRecord('You are review agent `small` — dimension.'),
            event('2026-08-03T10:06:30Z', { input: 1_000, output: 100 }),
        ].join('\n'));
        writeFileSync(join(project, 'subagents', SESSION, 'agent-role-z.jsonl'), [
    ...
    it('floors agent events by timestamp even when the file itself is fresh', () => {
        ...
        const straddling = join(project, 'subagents', SESSION, 'agent-role-first.jsonl');
        writeFileSync(straddling, [
    ...
    it('skips an agent killed before its first response, with no phantom row', () => {
        ...
        writeFileSync(join(project, 'subagents', SESSION, 'agent-role-dead.jsonl'), userRecord('You are review agent `dead` — killed at launch.'));
        writeFileSync(join(project, 'subagents', SESSION, 'agent-role-live.jsonl'), [
    ...
    it('ranks a folded (×N) row by its combined total, not its first member', () => {
        ...
        writeFileSync(join(project, 'subagents', SESSION, 'agent-role-solo.jsonl'), [
        ...
        expect(text.indexOf('agent 2 (×2)')).toBeLessThan(text.indexOf('agent 3: 1 call'));
    });
    it('truncates the agent block past eight rows, keeping the biggest', () => {
        const { plan, env, project } = fixture();
        writeMainCall(project);
        for (let i = 1; i <= 9; i++) {
            writeFileSync(join(project, 'subagents', SESSION, `agent-role-${i}.jsonl`), [
    ...
    it('keeps a reverse-audit chunk auditor apart from the territory finder', () => {
        const { plan, env, project } = fixture();
        writeMainCall(project);
        writeFileSync(join(project, 'subagents', SESSION, 'agent-t3.jsonl'), [
            userRecord('You are review agent `chunk 3 of 5` — the territory agent.\n' +
                'read_file(file_path="/abs/diff.txt", offset=100, limit=50)'),
            event('2026-08-03T10:07:00Z', { input: 40_000, output: 500 }),
        ].join('\n'));
        writeFileSync(join(project, 'subagents', SESSION, 'agent-ra3.jsonl'), [
            userRecord('You are review agent `chunk 3 of 5` — the territory agent.\n' +
    ...
    it('labels an auditor from its own brief line, not a quoted audit path', () => {
        ...
        writeFileSync(join(project, 'subagents', SESSION, 'agent-ra4.jsonl'), [
            userRecord('You are review agent `chunk 5 of 5` — the territory agent.\n' +
                '\n' +
    ...
    it('separates rounds by label while shards of one round still fold', () => {
        const { plan, env, project } = fixture();
        writeMainCall(project);
        const put = (file, identity, input) => writeFileSync(join(project, 'subagents', SESSION, file), [
            userRecord(identity),
            event('2026-08-03T10:07:00Z', { input, output: 100 }),
        ].join('\n'));
        put('agent-ra1.jsonl', 'You are review agent `reverse-audit` — Reverse audit (round 1).', 20_000);
        put('agent-ra2.jsonl', 'You are review agent `reverse-audit` — Reverse audit (round 2).', 21_000);
        put('agent-v1.jsonl', 'You are review agent `verify` — Verification (round 2).', 9_000);
        put('agent-v2.jsonl', 'You are review agent `verify` — Verification (round 2).', 8_000);
        const text = renderLedger(computeLedger(plan, env));
    ...
    it('labels from the FIRST line only — an identity quoted below never wins', () => {
        ...
        const { plan, env, project } = fixture();
        writeMainCall(project);
        writeFileSync(join(project, 'subagents', SESSION, 'agent-q0.jsonl'), [
            userRecord('Context: this launch was rewritten by the orchestrator.\n' +
    ...
    it('reads the round from the identity line, never from folded findings', () => {
        const { plan, env, project } = fixture();
        writeMainCall(project);
        writeFileSync(join(project, 'subagents', SESSION, 'agent-v0.jsonl'), [
            userRecord('You are review agent `verify` — Verification.\n' +
...
describe('cost-ledger command boundary — informational, never a failure', () => {
    ...
    function run(args) {
        ...
        try {
            costLedgerCommand.handler(args);
        }
        finally {
            outSpy.mockRestore();
            errSpy.mockRestore();
    ...
    it('exits 0 and names the plan when the plan is missing', () => {
        ...
        expect(stderr).toContain('could not read the plan report /nonexistent/plan.json');
    });
    it('exits 0 and names a missing chat transcript instead of printing agents-only totals', () => {
        ...
        expect(stderr).toContain(`could not read the chat transcript ${join(project, 'chats', `${SESSION}.jsonl`)}`);
    ...
    it('exits 0 when the terminal writes throw — the reader went away', () => {
        ...
        writeFileSync(join(project, 'chats', `${SESSION}.jsonl`), JSON.stringify({
            type: 'assistant',
            timestamp: '2026-08-03T10:01:00Z',
            usageMetadata: {
                promptTokenCount: 1_000,
...
describe('cost-ledger — a resumed run bills the whole review', () => {
    ...
    function fixture() {
        ...
        mkdirSync(join(project, 'chats'), { recursive: true });
        mkdirSync(join(project, 'subagents', SESSION), { recursive: true });
        writeFileSync(join(project, 'chats', `${SESSION}.jsonl`), event('2026-08-03T10:10:00Z', { input: 500, output: 50 }));
        const plan = join(project, 'plan.json');
        writeFileSync(plan, JSON.stringify({
            diffPathAbsolute: join(project, 'diff.txt'),
            diffLines: 10,
    ...
    function runLedger(plan) {
        appendRunSession(plan, { QWEN_CODE_SESSION_ID: 'S0' }, Date.parse('2026-08-03T10:00:30Z'));
        appendRunSession(plan, { QWEN_CODE_SESSION_ID: SESSION }, Date.parse('2026-08-03T10:09:00Z'));
        recordResume(plan, { QWEN_CODE_SESSION_ID: SESSION }, Date.parse('2026-08-03T10:09:00Z'));
    }
    ...
    it('still refuses an empty CURRENT chat when prior sessions have events', () => {
        ...
        const { plan, project, env } = fixture();
        writeFileSync(join(project, 'chats', `${SESSION}.jsonl`), '');
        writeFileSync(join(project, 'chats', 'S0.jsonl'), event('2026-08-03T10:05:00Z', { input: 1000, output: 100 }));
        runLedger(plan);
    ...
    it('folds TWO prior sessions, each inside its own window', () => {
        ...
        const { plan, project, env } = fixture();
        writeFileSync(join(project, 'chats', 'S0.jsonl'), event('2026-08-03T10:01:00Z', { input: 1000, output: 100 }));
        writeFileSync(join(project, 'chats', 'S0b.jsonl'), event('2026-08-03T10:06:00Z', { input: 200, output: 20 }));
        appendRunSession(plan, { QWEN_CODE_SESSION_ID: 'S0' }, Date.parse('2026-08-03T10:00:30Z'));
        appendRunSession(plan, { QWEN_CODE_SESSION_ID: 'S0b' }, Date.parse('2026-08-03T10:05:00Z'));
        appendRunSession(plan, { QWEN_CODE_SESSION_ID: SESSION }, Date.parse('2026-08-03T10:09:00Z'));
        recordResume(plan, { QWEN_CODE_SESSION_ID: SESSION }, Date.parse('2026-08-03T10:09:00Z'));
        const ledger = computeLedger(plan, env);
    ...
    it('clamps each prior session at ITS OWN successor, and sums both spans', () => {
        ...
        const { plan, project, env } = fixture();
        writeFileSync(join(project, 'chats', 'S0.jsonl'), [
    ...
    it('prefilters prior agent streams against the PRIOR floor, not the current one', () => {
        ...
        writeFileSync(stream, event('2026-08-03T10:02:00Z', { input: 300, output: 30 }));
    ...
    it("folds the interrupted attempt's main loop and agents into the totals", () => {
        const { plan, env, project } = fixture();
        runLedger(plan);
        writeFileSync(join(project, 'chats', 'S0.jsonl'), event('2026-08-03T10:01:00Z', { input: 1_000, output: 100 }));
        mkdirSync(join(project, 'subagents', 'S0'), { recursive: true });
        writeFileSync(join(project, 'subagents', 'S0', 'agent-a0.jsonl'), [
            userRecord('You are review agent `2` — Agent 2: Security.'),
...
describe('cost-ledger — prior-session bounds, faults and wall time', () => {
    ...
    function runLedger(project, resumedAt = '2026-08-03T10:09:00Z') {
        const plan = join(project, 'plan.json');
        appendRunSession(plan, { QWEN_CODE_SESSION_ID: 'S0' }, Date.parse('2026-08-03T10:00:30Z'));
        appendRunSession(plan, { QWEN_CODE_SESSION_ID: SESSION }, Date.parse(resumedAt));
        recordResume(plan, { QWEN_CODE_SESSION_ID: SESSION }, Date.parse(resumedAt));
    }
    ...
    it.skipIf(process.platform === 'win32' || process.getuid?.() === 0)('discloses an unreadable prior agent dir instead of silently flooring it', () => {
        const { plan, env, project } = fixture();
        runLedger(project);
        writeFileSync(join(project, 'chats', 'S0.jsonl'), event('2026-08-03T10:01:00Z', { input: 1_000, output: 100 }));
        const priorDir = join(project, 'subagents', 'S0');
        ...
        try {
            ...
            expect(ledger.priorSessions).toBe(1);
            expect(seen.some((l) => l.includes("prior session's subagent transcripts"))).toBe(true);
        }
        finally {
    ...
    it('never reads a symlinked prior session directory', () => {
        ...
        dirs.push(outside);
        writeFileSync(join(outside, 'agent-foreign.jsonl'), [
            userRecord('You are review agent `2` — Agent 2: Security.'),
    ...
    it('counts a prior session ONCE when it had both chat and agents', () => {
        ...
        writeFileSync(join(project, 'chats', `${SESSION}.jsonl`), event('2026-08-03T10:10:00Z', { input: 100, output: 10 }));
...
//# sourceMappingURL=cost-ledger.test.js.map