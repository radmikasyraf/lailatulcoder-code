...
import { DiscoveredMCPTool, uiTelemetryService, getCoreSystemPrompt, resolveInteractionMode, DEFAULT_TOKEN_LIMIT, ToolNames, buildSkillLlmContent, computeThresholds, formatContextFileDisplayPath, } from '@lailatul-coder/lailatul-coder-core';
 ...
 */
function parseMemoryFiles(memoryContent, workingDir) {
    ...
    while ((match = regex.exec(memoryContent)) !== null) {
        ...
        results.push({
            ...
            path: formatContextFileDisplayPath(path.resolve(workingDir, filePath), workingDir),
...
export async function collectContextData(config, showDetails) {
    ...
    return {
        ...
        skills: showDetails
            ? detailSkills.filter((skill) => !disabledSkillNames.has(skill.name.toLowerCase()))
            : [],
        isEstimated,
        showDetails,
    };
