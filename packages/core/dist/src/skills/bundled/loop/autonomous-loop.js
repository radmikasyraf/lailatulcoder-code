...
export const CRON_REARM = 'The recurring cron fires the next tick automatically — do not call LoopWakeup from this tick.';
export const keepAliveRearm = (sentinel, reason = 'To keep the loop alive') => `You scheduled this tick via LoopWakeup (not a recurring cron). ${reason}, call LoopWakeup again at the end of this turn with prompt set to the literal sentinel \`${sentinel}\` — otherwise the loop ends after this tick.`;
/** Detect whether a scheduled prompt is an autonomous-loop sentinel, and which
 * pacing mode. Parallel to detectLoopSentinel. */
export function detectAutonomousSentinel(prompt) {
...
export function autonomousTickText(mode) {
