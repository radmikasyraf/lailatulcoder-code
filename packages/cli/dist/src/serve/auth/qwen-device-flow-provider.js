 ...
 */
import { cacheQwenCredentials, generatePKCEPair, isDeviceAuthorizationSuccess, isDeviceTokenPending, isDeviceTokenSuccess, QwenOAuth2Client, QwenOAuthPollError, } from '@lailatul-coder/lailatul-coder-core';
import { writeStderrLine } from '../../utils/stdioHelpers.js';
import { brandSecret, sanitizeForStderr, unsafeRevealSecret, UpstreamDeviceFlowError, } from './device-flow.js';
...
function truncateForStderr(detail) {
    if (detail.length <= STDERR_DETAIL_MAX)
        return detail;
    const dropped = detail.length - STDERR_DETAIL_MAX;
    return `${detail.slice(0, STDERR_DETAIL_MAX)}…[+${dropped} bytes truncated]`;
}
/**
 * LailatulCoder-OAuth implementation of `DeviceFlowProvider` for `LailatulCoder serve`.
 *
 ...
 * a URL.
 */
export class QwenOAuthDeviceFlowProvider {
    providerId = 'LailatulCoder-oauth';
    client;
    constructor(client) {
    ...
    async start(opts) {
        ...
        try {
            ...
            // Network / parse / non-2xx errors from the LailatulCoder IdP. Wrap so the
            ...
            writeStderrLine(`[serve] LailatulCoder device-flow start failed (raw): ${truncateForStderr(detail)}`);
            throw new UpstreamDeviceFlowError('LailatulCoder IdP device authorization request failed');
        }
        ...
        if (!isDeviceAuthorizationSuccess(auth)) {
            ...
            // stderr.
            const errorData = auth;
            writeStderrLine(truncateForStderr(`[serve] LailatulCoder device-flow start error envelope (raw): error=${errorData?.error ?? 'unknown'} description=${errorData?.error_description ?? '(none)'}`));
            throw new UpstreamDeviceFlowError('LailatulCoder IdP rejected the device authorization request');
        }
        return {
            ...
            // LailatulCoder IdP doesn't return `interval`; registry falls back to the
            // RFC 8628 default (5s) when this is undefined.
        };
    }
    async poll(state, opts) {
        if (!state.pkceVerifier) {
            // LailatulCoder *requires* PKCE; missing verifier is a programmer error.
            return {
                kind: 'error',
                errorKind: 'invalid_grant',
                hint: 'LailatulCoder device-flow requires a PKCE verifier',
            };
        }
        if (opts.signal.aborted) {
        ...
        try {
            ...
            const errorKind = err instanceof QwenOAuthPollError
            ...
            if (!aborted) {
                ...
                writeStderrLine(`[serve] LailatulCoder device-flow poll failed (errorKind=${errorKind}): ${truncateForStderr(safeDetail)}`);
            }
            return {
                kind: 'error',
                errorKind,
                hint: errorKind === 'upstream_error'
                    ? 'unexpected response from identity provider'
                    : `LailatulCoder IdP returned ${errorKind}`,
            };
        }
        if (isDeviceTokenSuccess(response)) {
            ...
            return {
                ...
                async persist(persistOpts) {
                    ...
                    // The LailatulCoder IdP token response doesn't carry an
 ...
 */
function mapRfc8628OAuthCode(code) {
    switch (code) {
        case 'expired_token':
...
//# sourceMappingURL=LailatulCoder-device-flow-provider.js.map