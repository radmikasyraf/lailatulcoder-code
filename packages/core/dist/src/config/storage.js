...
export const SKILL_PROVIDER_CONFIG_DIRS = ['.lailatulcoder', '.agents'];
...
export class Storage {
    ...
    constructor(targetDir, runtimeBaseDir = Storage.getRuntimeBaseDir()) {
     ...
     *              Pass the project root so that relative values like ".lailatulcoder" resolve
    ...
    static runWithRuntimeBaseDir(dir, cwd, fn) {
        ...
        return Storage.runtimeBaseDirContext.run({ dir: resolved, pinned: false }, fn);
    }
    static runWithResolvedRuntimeBaseDir(dir, fn) {
        ...
        return Storage.runtimeBaseDirContext.run({ dir: path.resolve(dir), pinned: true }, fn);
    }
    ...
    static getGlobalQwenDir() {
        ...
        if (!homeDir) {
            return path.join(os.tmpdir(), '.lailatulcoder');
        }
    ...
    /**
     * Project-level saved-workflow scripts directory: `<targetDir>/.lailatulcoder/workflows`.
    ...
    /**
     * User-level saved-workflow scripts directory: `~/.lailatulcoder/workflows`. User
    ...
    /**
     * Returns the user-level extensions directory (~/.lailatulcoder/extensions/).
     * Extensions installed at user scope are stored here, as opposed to
     * project-level extensions which live in <project>/.lailatulcoder/extensions/.
     */
