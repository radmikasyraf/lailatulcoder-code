...
import { BACKGROUND_WORK_SWITCH_BLOCKED_MESSAGE, useResumeCommand, } from './useResumeCommand.js';
...
const resumeMocks = vi.hoisted(() => {
    ...
    return {
        ...
        resolvePendingLoadSession(value) {
...
vi.mock('@lailatul-coder/lailatul-coder-core', async (importOriginal) => {
    ...
    class SessionService {
        ...
        async loadSession(_sessionId) {
            return (resumeMocks.getPendingLoadSession() ??
                Promise.resolve({
...
describe('useResumeCommand', () => {
    ...
    it('handleResume closes the dialog immediately and restores session state', async () => {
        ...
        expect(config.startNewSession).toHaveBeenCalledWith('session-2', expect.objectContaining({
        ...
        expect(clearPendingState.mock.invocationCallOrder[0]).toBeLessThan(historyManager.loadHistory.mock.invocationCallOrder[0]);
    ...
    it('handleResume routes history replacement through the loadHistory override', async () => {
        ...
        expect(overrideLoadHistory).toHaveBeenCalledWith(expect.arrayContaining([expect.anything()]));
        expect(historyManager.loadHistory).not.toHaveBeenCalled();
        expect(historyManager.clearItems).toHaveBeenCalledTimes(1);
    });
    ...
    it('adds a recovered-background-agents notice when paused agents are restored', async () => {
        ...
        expect(historyManager.addItem).toHaveBeenCalledWith(expect.objectContaining({
            type: 'info',
    ...
    it('blocks resume when the current session still has running background work', async () => {
        ...
        const blockedItem = historyManager.addItem.mock.calls[0]?.[0];
    ...
    it('rolls core back when persisted Goal state is malformed', async () => {
        ...
        expect(config.startNewSession).toHaveBeenNthCalledWith(1, 'new-session-id', expect.any(Object));
        expect(config.startNewSession).toHaveBeenNthCalledWith(2, 'old-session-id', undefined);
        expect(config.loadPausedBackgroundAgents).toHaveBeenCalledWith('old-session-id');
        // UI never swapped.
        expect(startNewSession).not.toHaveBeenCalled();
        expect(historyManager.clearItems).not.toHaveBeenCalled();
        expect(historyManager.loadHistory).not.toHaveBeenCalled();
        ...
        expect(historyManager.addItem).toHaveBeenCalledWith(expect.objectContaining({
            type: 'error',
            text: expect.stringMatching(/Failed to resume session.*unsupported Goal lifecycle record/),
        }), expect.any(Number));
        expect(geminiClient.initialize).not.toHaveBeenCalled();
    });
});
//# sourceMappingURL=useResumeCommand.test.js.map