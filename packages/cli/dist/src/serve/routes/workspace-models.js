...
import { getModelProvidersOwnerScope, getOwnKeyScope, getWritableScopes, } from '../../config/modelProvidersScope.js';
import { getSettingDefinition } from '../../utils/settingsUtils.js';
import { writeStderrLine } from '../../utils/stdioHelpers.js';
import { isActiveModelSelection, removeModelFromProviders, } from '../model-providers-edit.js';
...
function parseTarget(body) {
    ...
    if (authType.length > MAX_MODEL_FIELD_LENGTH ||
 ...
 */
export function registerWorkspaceModelsRoutes(app, deps) {
    const { boundWorkspace, mutate, safeBody, persistSettings, broadcastSettingsChanged, parseAndValidateClientId, } = deps;
    app.delete('/workspace/models', mutate({ strict: true }), async (req, res) => {
        ...
        if (clientId === null)
            return;
        const broadcastWrite = (write) => {
            try {
                broadcastSettingsChanged(write.key, write.value, scopeToWire(write.scope), clientId);
            }
            catch (err) {
                writeStderrLine(`LailatulCoder serve: DELETE /workspace/models broadcast error (key=${write.key}): ${err instanceof Error ? err.message : String(err)}`);
            }
        };
        let writes;
        try {
            ...
            const { next, removed, removedBaseUrl } = removeModelFromProviders(modelProviders, loaded.merged.providerProtocol, parsed);
            if (!removed) {
                res.status(404).json({
            ...
            for (const activeScope of getWritableScopes(loaded)) {
                ...
                if (isActiveModelSelection(scopeModel?.name, scopeModel?.baseUrl, activeTarget)) {
            ...
            const stillConfigured = Object.values(next).some((models) => Array.isArray(models) &&
            ...
            if (sendGenerationClosedError(res, err))
                return;
            writeStderrLine(`LailatulCoder serve: DELETE /workspace/models error (authType=${parsed.authType}, modelId=${parsed.modelId}): ${err instanceof Error ? err.message : String(err)}`);
        ...
        const requiresRestart = writes.some((w) => getSettingDefinition(w.key)?.requiresRestart === true);
        res
            .status(200)
