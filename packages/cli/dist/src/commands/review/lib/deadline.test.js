...
import { BUDGET_STOP_PHRASE, DEADLINE_ENV, RESERVE_ENV, COMPOSE_FLOOR_ENV, DEFAULT_RESERVE_SECONDS, DEFAULT_ROUND_SECONDS, DEFAULT_COMPOSE_FLOOR_SECONDS, DEFAULT_TOOL_CONCURRENCY, TOOL_CONCURRENCY_ENV, budgetStopEntry, budgetStopEntryZh, claimRetirementDegradeNote, clearBudgetStop, clearRoundStamps, expectedAdmissionSeconds, expectedRoundSeconds, readBudgetStop, readBudgetStopUnfenced, readRoundStamps, reverseAuditBudgetExhausted, reverseAuditBudgetMessage, ROUND_CAP_PHRASE, roundCapStopDisclosure, roundCapStopEntry, roundCapStopEntryZh, writeRoundCapStop, stampRound, verifyBudgetExhausted, verifyBudgetMessage, writeBudgetStop, } from './deadline.js';
...
function backdatePlan(p, atMs = PLAN_CAPTURED_MS) {
...
describe('reverseAuditBudgetExhausted — the round must fit, and its tail', () => {
    it('stays silent when no deadline is set — every local run', () => {
        expect(reverseAuditBudgetExhausted({}, DEFAULT_ROUND_SECONDS, NOW_MS)).toBeNull();
        expect(reverseAuditBudgetExhausted({ [DEADLINE_ENV]: '' }, DEFAULT_ROUND_SECONDS, NOW_MS)).toBeNull();
    });
    it('admits a round while round-plus-reserve still fits', () => {
        const env = { [DEADLINE_ENV]: String(NOW_S + REQUIRED + 60) };
        expect(reverseAuditBudgetExhausted(env, DEFAULT_ROUND_SECONDS, NOW_MS)).toBeNull();
    });
    ...
    it('refuses a round the reserve alone would have admitted', () => {
        ...
        const spent = reverseAuditBudgetExhausted(env, DEFAULT_ROUND_SECONDS, NOW_MS);
    ...
    it('honours a reserve override, in both directions', () => {
        ...
        expect(reverseAuditBudgetExhausted(env, DEFAULT_ROUND_SECONDS, NOW_MS)).not.toBeNull();
    });
    ...
    it('fails OPEN on a malformed deadline — the outer kill still bounds the run', () => {
        for (const bad of ['soon', 'NaN', '-5', '0']) {
            expect(reverseAuditBudgetExhausted({ [DEADLINE_ENV]: bad }, DEFAULT_ROUND_SECONDS, NOW_MS)).toBeNull();
        }
    });
...
describe('the round-cost estimate — measured when it can be', () => {
    ...
    it('claimRetirementDegradeNote fails OPEN when the claim write faults — for any uid (#9272)', () => {
        ...
        const p = plan();
        const claim = join(promptRecordDir(p), 'retirement-degrade-note-round-3.json');
    ...
    it('claimRetirementDegradeNote reclaims a non-file occupant and a stale claim (#9272)', () => {
        ...
        writeFileSync(join(dir, 'retirement-degrade-note-round-4.json'), JSON.stringify({ round: 4, atMs: 1 }));
        expect(claimRetirementDegradeNote(p, 4)).toBe(true);
...
describe('the pair admission price — a round launched beside an in-flight round pays for both', () => {
    ...
    it('prices a round with no in-flight predecessor like expectedRoundSeconds', () => {
        const p = plan();
        expect(expectedAdmissionSeconds(p, 1, 6, {}, NOW_MS)).toBe(DEFAULT_ROUND_SECONDS);
        stampRound(p, 1, NOW_MS - 2_400_000); // round 1 returned 40 min ago
        expect(expectedAdmissionSeconds(p, 2, 6, {}, NOW_MS)).toBe(expectedRoundSeconds(p, 2, NOW_MS));
    ...
    it('prices the pair at both members when the pool serializes them', () => {
        ...
        const p = plan();
        stampRound(p, 1, NOW_MS - 30_000);
        expect(expectedAdmissionSeconds(p, 2, 6, {}, NOW_MS)).toBe(2 * DEFAULT_ROUND_SECONDS);
    });
    it('prices the pair at one round when the pool holds both members at once', () => {
        ...
        const p = plan();
        stampRound(p, 1, NOW_MS - 30_000);
        expect(expectedAdmissionSeconds(p, 2, 3, {}, NOW_MS)).toBe(DEFAULT_ROUND_SECONDS);
        expect(expectedAdmissionSeconds(p, 2, 1, {}, NOW_MS)).toBe(DEFAULT_ROUND_SECONDS);
    });
    it('reads the pool from the tool-concurrency env, like the scheduler', () => {
        ...
        // wave.
        expect(expectedAdmissionSeconds(p, 2, 6, { [TOOL_CONCURRENCY_ENV]: '12' }, NOW_MS)).toBe(DEFAULT_ROUND_SECONDS);
        // A 3-slot pool runs a 6-chunk round in two waves and the pair in
        // four — two rounds' price again.
        expect(expectedAdmissionSeconds(p, 2, 6, { [TOOL_CONCURRENCY_ENV]: '3' }, NOW_MS)).toBe(2 * DEFAULT_ROUND_SECONDS);
        // Malformed falls back to the default pool, never to a wedge.
        expect(expectedAdmissionSeconds(p, 2, 6, { [TOOL_CONCURRENCY_ENV]: 'soon' }, NOW_MS)).toBe(Math.ceil((DEFAULT_ROUND_SECONDS * Math.ceil(12 / DEFAULT_TOOL_CONCURRENCY)) /
...
describe('the budget-stop marker — the deterministic half of the disclosure', () => {
    ...
    it('round-trips, entry text and all', () => {
        ...
        expect(stop?.entry).toBe('reverse audit — stopped before round 4 by the review time budget');
    ...
    it('first refusal wins — a later cap write does not flip a same-run budget marker', () => {
        ...
        expect(stop?.entry).toBe('reverse audit — stopped before round 3 by the review time budget');
    });
    ...
    it('the zh entry pairs the en one, for a numbered round and without', () => {
        expect(budgetStopEntryZh(4)).toBe('反向审计——评审时间预算不足，未能开始第 4 轮');
        expect(budgetStopEntryZh(undefined)).toBe('反向审计——评审时间预算不足，未能开始下一轮');
    });
});
describe('the CI wiring contract', () => {
    it('the workflow exports the exact env names the gate reads', () => {
        ...
        const workflow = readFileSync(join(dirname(fileURLToPath(import.meta.url)), '..', '..', '..', '..', '..', '..', '.github', 'workflows', 'lailatul-coder-pr-review.yml'), 'utf8');
        ...
        expect(workflow).toContain(`${DEADLINE_ENV}="$(( $(date +%s) + attempt_timeout ))"`);
...
describe('verifyBudgetExhausted — the compose floor the verifier answers to', () => {
    ...
    it('reports a past deadline as negative remaining under the default floor', () => {
        const spent = verifyBudgetExhausted({ [DEADLINE_ENV]: String(NOW_S - 120) }, NOW_MS);
    ...
    it('a blank or whitespace floor override falls back — only explicit 0 disables', () => {
        ...
        for (const blank of ['', '   ']) {
            const spent = verifyBudgetExhausted({ ...under, [COMPOSE_FLOOR_ENV]: blank }, NOW_MS);
            expect(spent?.composeFloorSeconds).toBe(DEFAULT_COMPOSE_FLOOR_SECONDS);
        }
        expect(verifyBudgetExhausted({ ...under, [COMPOSE_FLOOR_ENV]: '0' }, NOW_MS)).toBeNull();
    });
    ...
    it('honors the env override, including 0 as the disable hatch', () => {
        ...
        expect(verifyBudgetExhausted({ ...near, [COMPOSE_FLOOR_ENV]: '60' }, NOW_MS)).toBeNull();
        // Floor 0 disables the gate entirely — the escape hatch.
        expect(verifyBudgetExhausted({ ...near, [COMPOSE_FLOOR_ENV]: '0' }, NOW_MS)).toBeNull();
        ...
        expect(verifyBudgetExhausted({ ...pastDeadline, [COMPOSE_FLOOR_ENV]: '0' }, NOW_MS)).toBeNull();
        // A garbled override falls back to the default, which 300s fails.
        expect(verifyBudgetExhausted({ ...near, [COMPOSE_FLOOR_ENV]: 'nonsense' }, NOW_MS)).not.toBeNull();
    });
    ...
    it('the refusal message says compose now and keeps unverified findings', () => {
        const spent = verifyBudgetExhausted({ [DEADLINE_ENV]: String(NOW_S + 300) }, NOW_MS);
