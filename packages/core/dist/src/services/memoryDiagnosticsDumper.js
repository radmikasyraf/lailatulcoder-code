...
export class MemoryDiagnosticsDumper {
     ...
     */
    async dump(trigger, recentSamples = []) {
        if (this.dumpCount >= MAX_DUMPS_PER_SESSION) {
            debugLogger.debug(`Skipping dump: session cap reached (${MAX_DUMPS_PER_SESSION})`);
            return undefined;
        }
        const now = Date.now();
        ...
        try {
            ...
            fs.writeFileSync(filePath, JSON.stringify(minimalPayload, null, 2), 'utf8');
            debugLogger.info(`Phase 1 diagnostics written to ${filePath} (trigger=${trigger}, dump #${dumpNumber})`);
            ...
            debugLogger.info(`Phase 2 diagnostics written to ${filePath} (trigger=${trigger}, dump #${dumpNumber})`);
            ...
            debugLogger.error(`Failed to write memory diagnostics: ${getErrorMessage(err)}`);
            return undefined;
        }
    }
...
//# sourceMappingURL=memoryDiagnosticsDumper.js.map