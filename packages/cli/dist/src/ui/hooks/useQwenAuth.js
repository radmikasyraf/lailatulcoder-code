...
export const useQwenAuth = (pendingAuthType, isAuthenticating) => {
    const [qwenAuthState, setQwenAuthState] = useState({
        deviceAuth: null,
        authStatus: 'idle',
        authMessage: null,
    });
    const isQwenAuth = pendingAuthType === AuthType.NEVER_MATCH_THIS;
    // Set up event listeners when authentication starts
    useEffect(() => {
        if (!isQwenAuth || !isAuthenticating) {
            // Reset state when not authenticating or not LailatulCoder auth
            setQwenAuthState({
                deviceAuth: null,
                authStatus: 'idle',
                authMessage: null,
