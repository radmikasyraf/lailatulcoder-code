...
import { ExtensionSettingScope, getScopedEnvContents, promptForSetting, updateSetting, } from '@lailatul-coder/lailatul-coder-core';
...
const setCommand = {
    ...
    handler: async (args) => {
        ...
        await updateSetting(extension.config, extension.id, setting, promptForSetting, scope);
    },
};
const listCommand = {
    command: 'list <name>',
    ...
    handler: async (args) => {
        ...
        if (!extension || !extension.settings || extension.settings.length === 0) {
            writeStdoutLine(t('Extension "{{name}}" has no settings to configure.', { name }));
            return;
        }
        const userSettings = await getScopedEnvContents(extension.config, extension.id, ExtensionSettingScope.USER);
        const workspaceSettings = await getScopedEnvContents(extension.config, extension.id, ExtensionSettingScope.WORKSPACE);
