 ...
 */
export async function firePreToolUseHook(messageBus, toolName, toolInput, toolUseId, permissionMode, signal, tool_call_id) {
    if (!messageBus) {
 ...
 */
export async function firePostToolUseHook(messageBus, toolName, toolInput, toolResponse, toolUseId, permissionMode, signal, tool_call_id) {
    if (!messageBus) {
 ...
 */
export async function firePostToolUseFailureHook(messageBus, toolUseId, toolName, toolInput, errorMessage, isInterrupt, permissionMode, signal, tool_call_id) {
    if (!messageBus) {
        return {};
    }
    try {
 ...
 */
export async function firePostToolBatchHook(messageBus, toolCalls, permissionMode = 'default', signal) {
    if (!messageBus) {
        return { shouldStop: false };
    }
    try {
        const response = await messageBus.request({
            ...
            signal,
        }, MessageBusType.HOOK_EXECUTION_RESPONSE, POST_TOOL_BATCH_HOOK_TIMEOUT_MS, signal);
        if (!response.success || !response.output) {
            const message = response.error?.message ||
                `hook runner returned ${response.success ? 'no output' : 'success: false'} without error detail`;
