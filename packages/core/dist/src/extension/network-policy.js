...
function isBlockedAddress(address, family) {
    ...
    if (family === 6 && !publicIpv6Addresses.check(address, 'ipv6'))
        return true;
    return blockedAddresses.check(address, family === 6 ? 'ipv6' : 'ipv4');
}
export async function resolveNetworkTarget(value, policy, signal) {
    signal?.throwIfAborted();
    ...
    if (url.username || url.password) {
        throw new Error('Public extension network requests must not use credentials.');
    }
    const hostname = stripIpv6Brackets(url.hostname);
    const literalFamily = isIP(hostname);
    const addresses = literalFamily
        ? [{ address: hostname, family: literalFamily }]
        : await waitForAbortable(dns.lookup(hostname, { all: true, verbatim: true }), signal);
    ...
    if (addresses.some(({ address, family }) => isBlockedAddress(address, family))) {
        throw new Error(`Extension network host resolved to a blocked address: ${hostname}`);
    }
    const selected = addresses[0];
    const lookup = (requestedHostname, options, callback) => {
        if (stripIpv6Brackets(requestedHostname) !== hostname) {
            const error = new Error('Pinned extension lookup hostname mismatch');
