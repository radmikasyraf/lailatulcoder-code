 ...
 */
import { createWriteStream, fstatSync, openSync, constants, } from 'node:fs';
 ...
 *
 * Usage:
 *   LailatulCoder --json-fd 3        # JSON events written to fd 3
 *   LailatulCoder --json-file /path  # JSON events written to file/FIFO
 */
export class DualOutputBridge {
    ...
    constructor(config, target, options = {}) {
        this.sessionId = config.getSessionId();
        if ('fd' in target) {
            // Reject stdin/stdout/stderr to prevent corrupting TUI output
            if (target.fd <= 2) {
                throw new Error(`--json-fd ${target.fd}: file descriptors 0 (stdin), 1 (stdout), and 2 (stderr) ` +
            ...
            try {
                ...
                throw new Error(`--json-fd ${target.fd}: file descriptor is not open. ` +
            ...
            try {
                const fd = openSync(target.filePath, constants.O_WRONLY | constants.O_NONBLOCK);
                ...
                if (code === 'ENXIO') {
                    ...
                    try {
                        const fd = openSync(target.filePath, constants.O_RDWR | constants.O_NONBLOCK);
                        this.stream = createWriteStream('', { fd });
                    }
                    catch (retryErr) {
                        if (retryErr.code === 'EACCES') {
                            throw new Error(`--json-file "${target.filePath}": permission denied opening FIFO for read-write. ` +
        ...
        this.stream.on('error', (err) => {
            ...
            if (code === 'EPIPE' || code === 'ERR_STREAM_DESTROYED') {
                ...
                debugLogger.warn('DualOutput: system error on stream, disabling:', err.message);
            }
            else {
        ...
        this.adapter = new StreamJsonOutputAdapter(config, true, // includePartialMessages — always emit streaming events
        this.stream);
    ...
    disableIfBufferOverflowed() {
        if (this.stream.writableLength > MAX_BUFFERED_BYTES) {
            debugLogger.warn('DualOutput: buffered data exceeds limit, disabling (no consumer draining?)');
            this.active = false;
            this.stream.destroy();
        }
    }
    ...
    emitPermissionRequest(requestId, toolName, toolUseId, input, blockedPath = null, permissionSuggestions = null) {
        ...
        if (!this.active)
            return;
        try {
            this.adapter.emitPermissionRequest(requestId, toolName, toolUseId, input, blockedPath, permissionSuggestions);
        }
        catch (err) {
     ...
     */
    emitControlError(requestId, message) {
        if (!this.active)
            return;
        this.disableIfBufferOverflowed();
        if (!this.active)
...
//# sourceMappingURL=DualOutputBridge.js.map