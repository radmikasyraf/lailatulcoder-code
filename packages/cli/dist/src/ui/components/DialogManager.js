...
export const DialogManager = ({ addItem, terminalWidth, }) => {
    ...
    if (uiState.auth.isAuthenticating) {
        ...
        // LailatulCoder Auth remains as a separate flow
        if (uiState.auth.pendingAuthType === AuthType.NEVER_MATCH_THIS) {
            return (_jsx(QwenOAuthProgress, { deviceAuth: uiState.auth.lailatulcoderAuthState.deviceAuth || undefined, authStatus: uiState.auth.lailatulcoderAuthState.authStatus, authMessage: uiState.auth.lailatulcoderAuthState.authMessage, onTimeout: () => {
                    uiActions.auth.onAuthError('LailatulCoder Auth authentication timed out.');
                    uiActions.auth.cancelAuthentication();
                    uiActions.auth.setAuthState(AuthState.Updating);
                }, onCancel: () => {
                    uiActions.auth.cancelAuthentication();
