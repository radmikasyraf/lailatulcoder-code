...
import { createDebugLogger, isSubpath, Storage, } from '@lailatul-coder/lailatul-coder-core';
...
const EXTENSION_FILES = new Set([
    'LailatulCoder-extension.json',
    '.lailatulcoder-extension-install.json',
]);
...
export class ExtensionFileWatcher {
    ...
    getUserExtensionRefreshAction(event, changedPath) {
        ...
        if (!fs.existsSync(path.join(this.extensionsDir, parts[0], 'LailatulCoder-extension.json'))) {
            return 'stale';
        }
