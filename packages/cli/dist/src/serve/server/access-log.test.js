...
const telemetryMocks = vi.hoisted(() => ({
    getDaemonTelemetryInboundTraceId: vi.fn(() => undefined),
}));
...
vi.mock('./telemetry-context.js', () => ({
    getDaemonTelemetryInboundTraceId: telemetryMocks.getDaemonTelemetryInboundTraceId,
}));
...
function harness() {
    ...
    const begin = (input = {}) => {
        if (!middleware)
            throw new Error('Access middleware was not installed');
...
describe('installAccessLogMiddleware', () => {
    ...
    it('routes non-error responses to info and errors to warn', () => {
        ...
        expect(h.logger.info).toHaveBeenCalledWith('request completed', expect.objectContaining({ route: 'GET /success', status: 399 }));
        expect(h.logger.warn).toHaveBeenCalledWith('request completed', expect.objectContaining({ route: 'GET /failure', status: 400 }));
    });
    it('joins the request log line to the caller trace when telemetry is off', () => {
        telemetryMocks.getDaemonTelemetryInboundTraceId.mockReturnValueOnce('3'.repeat(32));
        const h = harness();
        h.begin({ path: '/traced' }).response.emit('finish');
        expect(h.logger.info).toHaveBeenCalledWith('request completed', expect.objectContaining({ traceId: '3'.repeat(32) }));
    });
    it('omits the traceId field when no inbound trace id was captured', () => {
        const h = harness();
        h.begin({ path: '/untraced' }).response.emit('finish');
        const context = vi.mocked(h.logger.info).mock.calls[0]?.[1];
    ...
    it('caps UTF-8 fields, uses the first raw client header, and tolerates clock retreat', () => {
        ...
        const context = vi.mocked(h.logger.info).mock.calls[0]?.[1];
        expect(context).toBeDefined();
        expect(Buffer.byteLength(String(context?.route), 'utf8')).toBeLessThanOrEqual(2 * 1024);
        expect(context?.['routeOriginalBytes']).toBeGreaterThan(2 * 1024);
        expect(Buffer.byteLength(String(context?.sessionId), 'utf8')).toBeLessThanOrEqual(256);
        expect(context?.['sessionIdOriginalBytes']).toBe(300);
        expect(Buffer.byteLength(String(context?.clientId), 'utf8')).toBeLessThanOrEqual(256);
    ...
    it('bounds bursts, emits summaries before individual records, and keeps fixed status buckets', () => {
        ...
        for (const status of [201, 302, 404, 503, 700]) {
            h.begin({ path: `/suppressed/${status}`, status }).response.emit('finish');
        }
    ...
    it('flushes aggregate summaries in root context before the current request', () => {
        ...
        expect(h.logger.warn).toHaveBeenCalledWith('access logs suppressed', expect.objectContaining({ suppressed: 1 }));
        expect(h.logger.info).toHaveBeenLastCalledWith('request completed', expect.objectContaining({ route: 'GET /current' }));
        expect(vi.mocked(h.logger.warn).mock.invocationCallOrder.at(-1)).toBeLessThan(vi.mocked(h.logger.info).mock.invocationCallOrder.at(-1));
    });
