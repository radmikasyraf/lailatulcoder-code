...
import { isQuotaExhaustedError, formatQuotaExhaustedMessage, } from '../utils/quotaErrorDetection.js';
import { getErrorStatus, isAbortError } from '../utils/errors.js';
import { createDebugLogger } from '../utils/debugLogger.js';
import { containsXmlToolCalls, tryRecoverXmlToolCalls, } from './xml-tool-call-fallback.js';
import { parseAndFormatApiError } from '../utils/errorParsing.js';
import { getRateLimitErrorDetails, getRateLimitRetryDelayMs, isRateLimitError, } from '../utils/rateLimit.js';
import { classifyRetryError, isFallbackEligible, } from '../utils/retryErrorClassification.js';
import { clampOutputTokensToWindow, defaultOutputCeiling, DEFAULT_TOKEN_LIMIT, OUTPUT_TOKEN_CEILING, parsePositiveIntegerEnvValue, } from './tokenLimits.js';
...
import { logContentRetry, logContentRetryFailure, logApiRetry, logChatCompression, } from '../telemetry/loggers.js';
import { subagentNameContext } from '../utils/subagentNameContext.js';
import {} from '../services/chatRecordingService.js';
import { ChatCompressionService, computeThresholds, MAX_CONSECUTIVE_FAILURES, } from '../services/chatCompressionService.js';
import { acquireSleepInhibitor } from '../services/sleepInhibitor.js';
import { getFunctionResponseParts, resolveCompactionTuning, resolveSlimmingConfig, slimCompactionInput, } from '../services/compactionInputSlimming.js';
import { InMemoryImagePayloadStore, buildReattachParts, countAllInlineImages, replaceImagePayloadsInPlace, } from '../services/image-payload-references.js';
import { estimateContentTokens, estimatePromptTokens, getUsageOutputTokenCountForPromptEstimate, } from '../services/tokenEstimation.js';
import { microcompactHistory, } from '../services/microcompaction/microcompact.js';
import { ContentRetryEvent, ContentRetryFailureEvent, ApiRetryEvent, makeChatCompressionEvent, } from '../telemetry/types.js';
import { CompressionStatus } from './turn.js';
import { getContextLengthExceededInfo } from '../utils/contextLengthError.js';
import { getStartupContextLength, isSystemReminderContent, } from '../utils/environmentContext.js';
import { getCustomSystemPrompt, getManualPlanExitSystemReminder, } from './prompts.js';
import { RETRYABLE_STREAM_TRANSPORT_CODES } from './stream-transport-retry.js';
import { collectToolCallIdsFromHistory, getFunctionCallFingerprint, normalizeModelToolCallIds, reserveModelToolCallId, } from './toolCallIdUtils.js';
import { getToolCallPreparations, setToolCallPreparations, } from './tool-call-preparation.js';
...
function hasCandidateOutput(response) {
    return Boolean(response.candidates?.some((candidate) => Boolean(candidate.finishReason) ||
...
function hasNonThoughtCandidateParts(response) {
    return Boolean(response.candidates?.some((candidate) => candidate.content?.parts?.some((part) => !part.thought)));
}
function syncFunctionCallsField(response, parts) {
 ...
 */
function canonicalPlanToolName(toolName) {
    if (!toolName)
        return '';
 ...
 * cannot drift.
 */
export function approvedPlanRedactionText(planPath) {
    return (`[Plan approved and saved to ${planPath}. The plan text was ` +
 ...
 */
export function redactApprovedPlansInHistory(history, savedPlanContent, planPath) {
    ...
    for (const entry of history) {
        ...
        for (const part of entry.parts) {
            ...
            if (typeof output === 'string' &&
                PLAN_EXIT_APPROVED_LLM_CONTENT_PREFIXES.some((prefix) => output.startsWith(prefix))) {
                approved.add(fr.id);
            }
        }
    }
    ...
    const out = history.map((entry) => {
        ...
        const parts = entry.parts.map((part) => {
            ...
            if (!fc?.id ||
                ...
                return part;
            if (!approved.has(fc.id))
                return part;
            if ((fc.args ?? {})['plan'] !== savedPlanContent)
                return part;
            entryChanged = true;
            return {
                ...part,
 ...
 */
export function redactStructuredOutputArgsForRecording(part) {
...
function shouldStopAfterHardRescue(shouldForceFromHard, hardLimit, localPromptTokensAfterCompression) {
    return shouldForceFromHard && localPromptTokensAfterCompression >= hardLimit;
}
function getHardRescueFailureMessage(effectiveTokens, hardLimit, compressionInfo, localPromptTokensAfterCompression) {
    const compressionStatus = CompressionStatus[compressionInfo.compressionStatus] ??
        String(compressionInfo.compressionStatus);
    const tokenCount = compressionInfo.compressionStatus === CompressionStatus.COMPRESSED
    ...
    return (`Context is too large to send safely after automatic compression. ` +
 ...
 */
function coerceUsageCount(value, field) {
    if (typeof value === 'number' && Number.isFinite(value) && value >= 0) {
        return value;
    }
    if (value != null && field) {
        debugLogger.warn(`coerceUsageCount: hostile ${field}=${String(value)}, coercing to 0`);
    }
    return 0;
}
...
(function (StreamEventType) {
    ...
    StreamEventType["COMPRESSED"] = "compressed";
    ...
    StreamEventType["MODEL_FALLBACK"] = "model_fallback";
 ...
 */
const RECOVERY_RESUME_INSTRUCTION = 'Resume directly — no apology, no recap of what you were doing. Pick up ' +
...
function isSignificantRecoveryOverlap(overlap) {
    ...
    return (overlapBytes >= RECOVERY_OVERLAP_MIN_BYTES &&
 ...
 */
function startsWithMarkdownStructuralAnchor(text) {
    const trimmed = text.replace(/^\s+/, '');
    return /^(\|[^\n]*\|[^\n]*\||\|[\s\-:]+\||#{1,6} |```|>\s|[-*+] |\d+\. )/.test(trimmed);
}
function findContainedRecoveryPrefixReplayLength(previousText, continuationText) {
    ...
    const previousTail = previousText.length > RECOVERY_CONTAINED_TAIL_LOOKBACK_CHARS
    ...
    const maxPrefix = Math.min(previousTail.length, trimmedContinuation.length, RECOVERY_OVERLAP_MAX_SCAN_CHARS);
 ...
 */
function previousTailContainsAtLineBoundary(previousTail, prefix) {
    let searchFrom = 0;
 ...
 */
function getRecoveryContinuationSuffix(previousText, continuationText) {
    if (previousText.length === 0 || continuationText.length === 0) {
        return continuationText;
    }
    if (previousText.endsWith(continuationText) &&
        isSignificantRecoveryOverlap(continuationText)) {
        return '';
    }
    const maxOverlap = Math.min(previousText.length, continuationText.length, RECOVERY_OVERLAP_MAX_SCAN_CHARS);
    ...
    for (let length = maxOverlap; length > 0; length -= 1) {
        ...
        if (isSignificantRecoveryOverlap(overlap) &&
    ...
    const containedPrefixLength = findContainedRecoveryPrefixReplayLength(previousText, continuationText);
 ...
 * one expression.
 */
function mergeDeliveredPrefix(deliveredText, continuationText) {
    return (deliveredText +
        getRecoveryContinuationSuffix(deliveredText, continuationText));
}
function isPlainTextPart(part) {
 ...
 */
function sanitizeRecoverySuffixTail(tail) {
    if (!tail.includes('</previous_response_suffix>') &&
 ...
 */
function buildRecoveryMessageFromText(lead, previousText) {
    if (previousText.trim().length === 0) {
        return lead;
    }
    const rawTail = previousText.length > OUTPUT_RECOVERY_TAIL_CHARS
...
function buildOutputRecoveryMessage(previousModelTurn) {
    return buildRecoveryMessageFromText(OUTPUT_RECOVERY_MESSAGE, previousModelTurn?.role === 'model'
 ...
 */
function appendRecoveryContinuationParts(previousParts, continuationParts) {
    ...
    if (previousTextIndex >= 0 && continuationTextIndex >= 0) {
        const previousTextPart = mergedParts[previousTextIndex];
        const continuationTextPart = nextParts[continuationTextIndex];
        const suffix = getRecoveryContinuationSuffix(previousTextPart.text, continuationTextPart.text);
        if (suffix.length > 0) {
...
function findLastPlainTextPartIndex(parts) {
...
function degradedPlaceholderError() {
    return new InvalidStreamError('Model response is an upstream fail-fast placeholder.', 'UPSTREAM_DEGRADED_RESPONSE');
}
function isDegradedPlaceholderTurn(content) {
    const parts = content.parts ?? [];
    return (parts.length > 0 &&
        parts.every((part) => part.functionCall === undefined &&
...
async function* rejectDegradedPlaceholderResponse(stream) {
    const pending = [];
    let text = '';
...
function extractCuratedHistory(comprehensiveHistory) {
    ...
    while (i < length) {
        if (comprehensiveHistory[i].role === 'user') {
            ...
            if (isValid) {
                curatedHistory.push(...modelOutput.filter((turn) => !isDegradedPlaceholderTurn(turn)));
            }
        }
    }
    return curatedHistory;
}
function appendCuratedContent(curatedHistory, content) {
...
function copyContentContainer(content) {
    return {
        ...content,
        ...(content.parts ? { parts: content.parts.map(copyPartContainer) } : {}),
    };
}
function copyPartContainer(part) {
...
function stripThoughtPartsFromContent(content) {
    if (!content.parts) {
        return content;
    }
...
function hasLeakedToolCallTags(text) {
    let inString = false;
    let escaped = false;
    for (let i = 0; i < text.length; i++) {
        ...
        if (inString) {
            ...
            if (LEAKED_TOOL_CALL_TAGS.test(text))
                return true;
        }
    }
    return false;
 ...
 */
export const ORPHAN_TOOL_USE_REPAIR_REASON = 'Tool execution result was not recorded — likely interrupted by network ' +
    ...
    while (scanIdx < history.length && history[scanIdx]?.role === 'user') {
        ...
        for (let pIdx = 0; pIdx < parts.length; pIdx++) {
            ...
            if (id) {
                ...
                if (list)
                    list.push({ turnIdx: scanIdx, partIdx: pIdx, part });
                else
...
function applyRepair(history, plan, reason) {
    ...
    const syntheticParts = plan.synthesizeIds.map(([callId, name]) => ({
 ...
 */
export function repairOrphanedToolUseTurns(history, reason = ORPHAN_TOOL_USE_REPAIR_REASON) {
    const injected = [];
    const droppedDuplicates = [];
 ...
 */
const SESSION_START_CONTEXT_SENTINEL_START = '<qwen:session-start-context hidden="true">';
...
export class GeminiChat {
    ...
    lastPromptTokenCountIsEstimated = false;
    /**
    ...
    imagePayloadStore = new InMemoryImagePayloadStore();
    /**
     ...
     */
    userContentPushCount = 0;
    manualPlanExitNoticesEnabled = false;
    /**
    ...
    popPendingPartialAssistantTurn() {
        ...
        if (this.history.length > idx && this.history[idx]?.role === 'model') {
            ...
            debugLogger.warn(`[PARTIAL_POP] Splice skipped: idx=${idx}, ` +
     ...
     */
    constructor(config, generationConfig = {}, history = [], chatRecordingService, telemetryService) {
        this.config = config;
     ...
     */
    getRequestHistory(currentUserContent) {
        const curatedHistory = extractCuratedHistory(this.history);
        const { maxRecentImages, imagePayloadThreshold } = resolveCompactionTuning(this.config.getChatCompression());
        let replaced = [];
        if (countAllInlineImages(curatedHistory) >= imagePayloadThreshold) {
            const skipEntry = currentUserContent
                ...
                : undefined;
            replaced = replaceImagePayloadsInPlace(curatedHistory, this.imagePayloadStore, skipEntry);
        }
        const requestHistory = curatedHistory.map(copyContentContainer);
        const reattachParts = buildReattachParts(replaced, maxRecentImages, requestHistory, this.imagePayloadStore);
    ...
    getRequestHistoryForRoute(currentUserContent, supportedModalities) {
        return slimCompactionInput(this.getRequestHistory(currentUserContent), supportedModalities).slimmedHistory;
    }
    /**
     ...
     * last response size.
     */
    setLastPromptTokenCount(count, isEstimated = false) {
    ...
    promptCountIsEstimateDerived() {
        return (this.lastPromptTokenCount === 0 || this.lastPromptTokenCountIsEstimated);
    }
    /**
     ...
     */
    seedResumeTokenCounts(promptTokenCount, outputTokenCount, isEstimated = false) {
     ...
     */
    async tryCompress(promptId, force = false, signal, options) {
        const originalTokenCountIsEstimated = options?.originalTokenCountOverride === undefined &&
        ...
        const originalTokenCount = originalTokenCountIsEstimated
            ? (options?.precomputedEffectiveTokens ??
                estimateContentTokens(options?.pendingUserMessage
                    ? [...this.getHistoryShallow(true), options.pendingUserMessage]
                    : this.getHistoryShallow(true), resolveSlimmingConfig(this.config.getChatCompression())
                    .imageTokenEstimate))
            : (options?.originalTokenCountOverride ?? this.lastPromptTokenCount);
        debugLogger.debug(`[compaction] token-count provenance: prompt_id=${promptId}, ` +
        ...
        if (info.compressionStatus === CompressionStatus.COMPRESSED && newHistory) {
            ...
            this.setLastPromptTokenCount(info.newTokenCount, info.newTokenCountIsEstimated);
            ...
            if (!force) {
                ...
                if (this.consecutiveFailures >= MAX_CONSECUTIVE_FAILURES) {
                    debugLogger.warn(`[compaction] circuit breaker tripped after ${this.consecutiveFailures} consecutive failures (cheap-gate path); auto-compaction will NOOP until a successful force compaction resets the counter.`);
                }
            }
        }
        return info;
    ...
    compressFast() {
        ...
        const mcResult = microcompactHistory(this.history, null, this.config.getClearContextOnIdle(), {
            force: true,
            preserveReadFileResult: (filePath) => isManagedMemoryPath(filePath, projectRoot, targetDir),
        });
        ...
        debugLogger.debug(`[compaction] fast token-count provenance: ` +
    ...
    setSessionStartContext(extraInstruction) {
    ...
    applySessionStartContext(extraInstruction, _source) {
        const trimmed = extraInstruction.trim();
        if (!trimmed) {
            return;
        }
     ...
     * ```
     */
    async sendMessageStream(model, params, prompt_id, goalContext, options) {
        ...
        const requestModalities = exactRoute?.contentGeneratorConfig.modalities ??
        ...
        let manualPlanExitNoticeVersion;
        let manualPlanExitNoticeText;
        ...
        const cgConfigForThresholds = exactRoute?.contentGeneratorConfig ??
            this.config.getContentGeneratorConfig();
        const parsedEnvMaxTokensForClamp = parsePositiveIntegerEnvValue(process.env['QWEN_CODE_MAX_OUTPUT_TOKENS']);
        const explicitOutputCeiling = params.config?.maxOutputTokens ??
            cgConfigForThresholds?.samplingParams?.max_tokens ??
            parsedEnvMaxTokensForClamp;
        const outputCeiling = explicitOutputCeiling ?? defaultOutputCeiling(model);
        ...
        // estimate.
        const contextWindowForClamp = cgConfigForThresholds?.contextWindowSize ?? DEFAULT_TOKEN_LIMIT;
        ...
        try {
            ...
            if (toolOutputBudget !== undefined &&
                ...
                if (guarded.responseParts !== userContent.parts) {
                    debugLogger.warn(`Tool response send guard reduced an unfinalized batch to ${toolOutputBudget} characters.`);
            ...
            const { hard } = computeThresholds(contextWindowForClamp, this.config.getAutoCompactThreshold());
            const imageTokenEstimate = resolveSlimmingConfig(this.config.getChatCompression()).imageTokenEstimate;
            ...
            // hard-rescue to miss.
            const effectiveTokens = estimatePromptTokens(this.lastPromptTokenCount > 0 ? [] : this.getHistoryShallow(true), userContent, this.lastPromptTokenCount, this.lastOutputTokenCount, imageTokenEstimate);
            const isHardTier = effectiveTokens >= hard;
            const shouldForceFromHard = !exactRoute &&
                isHardTier &&
            ...
            const lastPromptTokenCountWasEstimatedBeforeHardRescue = this.lastPromptTokenCountIsEstimated;
            const hardRescueFailureCountBeforeHardRescue = this.hardRescueFailureCount;
            if (shouldForceFromHard) {
                debugLogger.warn(`[compaction] hard-tier rescue triggered: prompt_id=${prompt_id}, effectiveTokens=${effectiveTokens}, hard=${hard}, hardRescueAttempt=${this.hardRescueFailureCount + 1}, consecutiveFailures=${this.consecutiveFailures}.`);
            }
            else if (isHardTier && !exactRoute) {
                debugLogger.warn(`[compaction] hard-tier rescue skipped after ${this.hardRescueFailureCount} failed attempts; relying on reactive overflow recovery. prompt_id=${prompt_id}, effectiveTokens=${effectiveTokens}, hard=${hard}.`);
            }
            if (exactRoute || (isHardTier && !shouldForceFromHard)) {
                ...
                compressionInfo = await this.tryCompress(prompt_id, shouldForceFromHard, params.config?.abortSignal, {
            ...
            const localPromptTokensAfterCompression = shouldForceFromHard
                ? estimatePromptTokens(this.lastPromptTokenCount > 0 ? [] : this.getHistoryShallow(true), userContent, this.lastPromptTokenCount, this.lastOutputTokenCount, imageTokenEstimate)
                : 0;
            if (shouldStopAfterHardRescue(shouldForceFromHard, hard, localPromptTokensAfterCompression)) {
                const message = getHardRescueFailureMessage(effectiveTokens, hard, compressionInfo, localPromptTokensAfterCompression);
                if (shouldForceFromHard) {
                ...
                if (compressionInfo.compressionStatus === CompressionStatus.COMPRESSED &&
                    ...
                    this.telemetryService?.setLastPromptTokenCount(lastPromptTokenCountBeforeHardRescue);
                }
                const compressionStatus = CompressionStatus[compressionInfo.compressionStatus] ??
                    String(compressionInfo.compressionStatus);
                debugLogger.warn(`[compaction] hard-tier rescue stopped oversized prompt: ` +
            ...
            if (this.manualPlanExitNoticesEnabled) {
                ...
                if (notice) {
                    ...
                    manualPlanExitNoticeText = getManualPlanExitSystemReminder(notice.currentMode);
                    userContent = {
                        ...userContent,
                        parts: [
                            ...(userContent.parts ?? []),
            ...
            if (inlineRepair.injected.length > 0) {
                debugLogger.warn(`[REPAIR] sendMessageStream inline pass synthesized ` +
            ...
            if (inlineRepair.droppedDuplicates.length > 0) {
                debugLogger.warn(`[REPAIR] sendMessageStream inline pass dropped ` +
            ...
            requestContents = this.getRequestHistoryForRoute(currentUserContent, requestModalities);
            ...
            promptTokensForClamp =
                this.lastPromptTokenCount > 0
                    ? estimatePromptTokens([], userContent, this.lastPromptTokenCount, this.lastOutputTokenCount, imageTokenEstimate, 
                    /* conservative= */ true)
                    : effectiveTokens;
            if (this.promptCountIsEstimateDerived()) {
                promptTokensForClamp += ESTIMATE_CLAMP_OVERHEAD_PAD;
                debugLogger.debug(`[clamp] estimate-derived prompt count; padded by ` +
                    `${ESTIMATE_CLAMP_OVERHEAD_PAD}: ` +
                    `promptTokensForClamp=${promptTokensForClamp}, ` +
                    `count=${this.lastPromptTokenCount}`);
            }
            const clampedMaxOutputTokens = clampOutputTokensToWindow(outputCeiling, contextWindowForClamp, promptTokensForClamp);
            params = {
                ...params,
                config: {
                    ...params.config,
            ...
            if (manualPlanExitNoticeVersion !== undefined) {
                this.config.restorePendingManualPlanExitNotice(manualPlanExitNoticeVersion);
            }
            streamDoneResolver();
            throw error;
        }
        ...
        return (async function* () {
            const sleepInhibitorHandle = acquireSleepInhibitor(self.config, 'LailatulCoder Ai is streaming a model response');
            try {
                ...
                const totalInvalidStreamRetryCount = () => transientInvalidStreamRetryCount + protocolTagLeakRetryCount;
                ...
                const cgConfig = exactRoute?.contentGeneratorConfig ??
                ...
                const retryInitialDelayMs = cgConfig?.retryInitialDelayMs ??
                    RATE_LIMIT_RETRY_OPTIONS.initialDelayMs;
                const retryMaxDelayMs = cgConfig?.retryMaxDelayMs ?? RATE_LIMIT_RETRY_OPTIONS.maxDelayMs;
                ...
                const parsedEnvMaxTokens = parsePositiveIntegerEnvValue(process.env['QWEN_CODE_MAX_OUTPUT_TOKENS']);
                const hasUserMaxTokensOverride = (cgConfig?.samplingParams?.max_tokens !== undefined &&
                ...
                const effectiveInitialMaxOutputTokens = params.config?.maxOutputTokens ?? outputCeiling;
                const escalatedLimit = clampOutputTokensToWindow(OUTPUT_TOKEN_CEILING, contextWindowForClamp, promptTokensForClamp);
                const shouldEscalateMaxOutputTokens = effectiveInitialMaxOutputTokens < escalatedLimit;
                 ...
                 */
                const buildAttemptContents = () => transportContinuationPrefix.length > 0
                    ? [
                        ...requestContents,
                        {
                            role: 'model',
                        ...
                        createUserContent([
                            {
                                text: buildRecoveryMessageFromText(TRANSPORT_CONTINUATION_MESSAGE, transportContinuationPrefix),
                            },
                        ]),
                    ]
                    : requestContents;
                ...
                const foldTransportAttemptText = () => {
                    transportContinuationText += getRecoveryContinuationSuffix(transportContinuationText, transportAttemptText);
                    transportAttemptText = '';
                };
                for (;;) {
                    transportAttemptText = '';
                    ...
                    try {
                        ...
                        const stream = await self.makeApiCallAndProcessStream(model, buildAttemptContents(), params, prompt_id, requestOverrides, turnGoalContext, 
                        ...
                        // connections (#7832).
                        const isRetryableStreamTransportError = classification.kind === 'transport' &&
                        ...
                        if (isRetryableStreamTransportError &&
                            ...
                            self.popPendingPartialAssistantTurn();
                            transportStreamRetryCount++;
                            const delayMs = TRANSPORT_STREAM_RETRY_CONFIG.initialDelayMs *
                                transportStreamRetryCount;
                        ...
                        const canContinueAfterTransportCut = isRetryableStreamTransportError &&
                        ...
                        if (canContinueAfterTransportCut) {
                            ...
                            debugLogger.warn('Transport stream continuation scheduled', {
                                ...
                                maxRetries: TRANSPORT_STREAM_RETRY_CONFIG.maxContinuationRetries,
                                retryDelayMs: delayMs,
                                errorKind: classification.kind,
                                transportCode: classification.transportCode,
                        ...
                        if (isRetryableStreamTransportError) {
                            ...
                            debugLogger.warn('Transport stream retry not taken', {
                                ...
                                maxContinuationRetries: TRANSPORT_STREAM_RETRY_CONFIG.maxContinuationRetries,
                                errorKind: classification.kind,
                                transportCode: classification.transportCode,
                            });
                        }
                        ...
                        if (contextOverflow.isExceeded) {
                            if (!exactRoute && !reactiveCompressionAttempted) {
                                ...
                                const reactiveOriginalTokenCount = contextOverflow.actualTokens ??
                                    contextOverflow.limitTokens ??
                                    cgConfig?.contextWindowSize ??
                                    DEFAULT_TOKEN_LIMIT;
                                debugLogger.warn('Context length exceeded; attempting reactive compression.');
                                try {
                                    const reactiveInfo = await self.tryCompress(prompt_id, true, params.config?.abortSignal, {
                                        originalTokenCountOverride: reactiveOriginalTokenCount,
                                        precomputedEffectiveTokens: reactiveOriginalTokenCount,
                                        requestGenerationConfig: params.config,
                                        trigger: 'auto',
                                    });
                                    if (reactiveInfo.compressionStatus ===
                                        ...
                                        if (noticeText &&
                                            !self.history.some((content) => content.parts?.some((part) => part.text?.includes(noticeText)))) {
                                            ...
                                            if (lastContent?.role === 'user') {
                                                ...
                                                self.history.push(createUserContent([{ text: noticeText }]));
                                            }
                                        }
                                        requestContents = self.getRequestHistoryForRoute(currentUserContent, requestModalities);
                                        debugLogger.info(`Reactive compression succeeded: ` +
                                    ...
                                    debugLogger.warn(`Reactive compression did not recover context overflow: ` +
                                    ...
                                    if (isCompressionFailureStatus(reactiveInfo.compressionStatus)) {
                                        ...
                                        if (self.consecutiveFailures >= MAX_CONSECUTIVE_FAILURES) {
                                            debugLogger.warn(`[compaction] circuit breaker tripped after ${self.consecutiveFailures} consecutive failures (reactive overflow path); auto-compaction will NOOP on the cheap-gate until a successful force compaction resets the counter.`);
                                        }
                                    }
                                }
                                ...
                                debugLogger.warn('Reactive compression already attempted; ' +
                        ...
                        const maxInvalidStreamRetries = isInvalidStreamError && error.type === 'PROTOCOL_TAG_LEAK'
                            ? INVALID_STREAM_RETRY_CONFIG.protocolTagLeakMaxRetries
                            : INVALID_STREAM_RETRY_CONFIG.transientMaxRetries;
                        const invalidStreamRetryCount = isInvalidStreamError && error.type === 'PROTOCOL_TAG_LEAK'
                        ...
                        if (isInvalidStreamError &&
                            ...
                            logContentRetry(self.config, new ContentRetryEvent(nextInvalidStreamRetryCount - 1, error.type, delayMs, model));
                            yield { type: StreamEventType.RETRY };
                            await delay(delayMs, params.config?.abortSignal).promise;
                            continue;
                        }
                ...
                const streamWithInvalidStreamRetries = async function* (buildAttempt, retryEvent = {
                    type: StreamEventType.RETRY,
                }) {
                    ...
                    for (;;) {
                        ...
                        try {
                            const stream = await self.makeApiCallAndProcessStream(model, attemptState.requestContents, attemptState.params, prompt_id, requestOverrides, turnGoalContext);
                            for await (const chunk of stream) {
                                yield { type: StreamEventType.CHUNK, value: chunk };
                            }
                            return;
                            ...
                            if (!(error instanceof InvalidStreamError))
                                throw error;
                            const maxContinuationRetries = error.type === 'PROTOCOL_TAG_LEAK'
                                ? INVALID_STREAM_RETRY_CONFIG.protocolTagLeakMaxRetries
                                : INVALID_STREAM_RETRY_CONFIG.transientMaxRetries;
                            const continuationRetryCount = error.type === 'PROTOCOL_TAG_LEAK'
                                ? protocolTagLeakRetryCount
                                : transientRetryCount;
                            ...
                            debugLogger.warn(`Invalid stream [${error.type}] during output continuation ` +
                                `(retry ${nextContinuationRetryCount}/${maxContinuationRetries}). ` +
                                `Waiting ${delayMs / 1000}s before retrying...`);
                            logContentRetry(self.config, new ContentRetryEvent(nextContinuationRetryCount - 1, error.type, delayMs, model));
                            yield retryEvent;
                ...
                if (lastError === null &&
                    ...
                    let recoveryFinishReason = lastFinishReason;
                    let recoveryParams = params;
                    if (shouldEscalateMaxOutputTokens) {
                        debugLogger.info(`Output truncated at ${effectiveInitialMaxOutputTokens} tokens. ` +
                    ...
                    while (recoveryFinishReason === FinishReason.MAX_TOKENS &&
                        ...
                        const hasFunctionCall = lastEntry?.role === 'model' &&
                            lastEntry.parts?.some((p) => p.functionCall) === true;
                        if (hasFunctionCall) {
                            debugLogger.info('Skipping recovery: truncated turn contains functionCall; ' +
                                'deferring to tool scheduler fallback.');
                            break;
                        }
                        recoveryCount++;
                        debugLogger.info(`Output still truncated after max_tokens handling. ` +
                        ...
                        const recoveryImageTokenEstimate = resolveSlimmingConfig(self.config.getChatCompression()).imageTokenEstimate;
                        const countBasedRecoveryEstimate = self.lastPromptTokenCount > 0
                            ? estimatePromptTokens([], recoveryUserContent, self.lastPromptTokenCount, self.lastOutputTokenCount, recoveryImageTokenEstimate, 
                            /* conservative= */ true)
                            : 0;
                        self.history.push(recoveryUserContent);
                        const recoveryContents = self.getRequestHistoryForRoute(currentUserContent, requestModalities);
                        self.history.pop();
                        const walkRecoveryEstimate = estimateContentTokens(recoveryContents, recoveryImageTokenEstimate) + ESTIMATE_CLAMP_OVERHEAD_PAD;
                        const recoveryPromptEstimate = Math.max(countBasedRecoveryEstimate, walkRecoveryEstimate);
                        ...
                        const recoveryCeiling = recoveryParams.config?.maxOutputTokens ?? outputCeiling;
                        const iterationParams = {
                            ...recoveryParams,
                            config: {
                                ...recoveryParams.config,
                                maxOutputTokens: clampOutputTokensToWindow(recoveryCeiling, contextWindowForClamp, recoveryPromptEstimate),
                            },
                        };
                        try {
                            for await (const event of streamWithInvalidStreamRetries(() => {
                            ...
                            debugLogger.warn(`Recovery attempt ${recoveryCount} failed: ${recoveryError}`);
                ...
                if (lastError) {
                    ...
                    const fallbackModels = exactRoute || options?.disableModelFallbacks
                        ? []
                    ...
                    if (fallbackModels.length > 0 &&
                        ...
                        if (isFallbackEligible(currentErrorClassification)) {
                            ...
                            for (const fallbackModelId of fallbackModels) {
                                ...
                                if (fallbackModelId === model ||
                                    fallbackModelId === currentModel) {
                                    debugLogger.warn(`[FALLBACK] Skipping fallback model "${fallbackModelId}": ` +
                                        `same as current model.`);
                                    continue;
                                }
                                // Resolve the fallback model's content generator
                                let fallbackGenerator;
                                let fallbackRetryAuthType;
                                let fallbackRetryErrorCodes;
                                let resolvedFallbackModel;
                                let fallbackModalities;
                                try {
                                    ...
                                    if (isAbortError(resolveError))
                                        throw resolveError;
                                    const resolveErrorMessage = resolveError instanceof Error
                                        ? resolveError.message
                                        : String(resolveError);
                                    debugLogger.warn(`[FALLBACK] Failed to resolve fallback model ` +
                                        `"${fallbackModelId}": ` +
                                        `${resolveErrorMessage}. ` +
                                ...
                                fallbackIndex++;
                                debugLogger.warn(`[FALLBACK] Model "${currentModel}" exhausted retries ` +
                                ...
                                try {
                                    const fallbackRequestContents = self.getRequestHistoryForRoute(currentUserContent, fallbackModalities ?? {});
                                    for await (const event of self.makeFallbackStream(resolvedFallbackModel, fallbackRequestContents, params, prompt_id, fallbackGenerator, fallbackRetryAuthType, fallbackRetryErrorCodes, turnGoalContext)) {
                                        const emittedUserVisibleOutput = event.type !== StreamEventType.CHUNK ||
                                    ...
                                    debugLogger.info(`[FALLBACK] Successfully completed request with ` +
                                    ...
                                    const fallbackClassification = classifyRetryError(fallbackError, {
                                        authType: fallbackRetryAuthType,
                                        extraRetryErrorCodes: fallbackRetryErrorCodes,
                                    });
                                    const canTryNextFallback = isFallbackEligible(fallbackClassification);
                                    debugLogger.warn(`[FALLBACK] Fallback model "${resolvedFallbackModel}" also ` +
                                    ...
                                    if (!canTryNextFallback) {
                                        debugLogger.warn(`[FALLBACK] Error from "${resolvedFallbackModel}" is not ` +
                            ...
                            if (!fallbackSucceeded) {
                                ...
                                debugLogger.warn('[FALLBACK] Fallback chain exhausted without success. ' +
                                    'Throwing last error.');
                            }
                        }
                        else {
                            debugLogger.warn('[FALLBACK] Fallback chain skipped: primary error is not ' +
                                'fallback-eligible ' +
                                `(reason: ${currentErrorClassification.reason}, ` +
                        ...
                        !isUnattendedMode() &&
                        streamYieldedAnyChunk) {
                        debugLogger.warn('[FALLBACK] Fallback chain skipped because the primary model ' +
                ...
                if (self.pendingPartialAssistantRecord) {
                    try {
                        ...
                        debugLogger.error('[PARTIAL_FLUSH] Failed to persist deferred JSONL record: ' +
     ...
     * the retry wiring.
     */
    async makeApiCallAndProcessStream(model, requestContents, params, prompt_id, overrides, goalContext, transportContinuationPrefix) {
        const generator = overrides?.contentGenerator ?? this.config.getContentGenerator();
        const apiCall = () => generator.generateContentStream({
            model,
        ...
        const extraRetryErrorCodes = overrides?.retryErrorCodes ?? cgConfig?.retryErrorCodes;
        ...
        const streamResponse = await retryWithBackoff(apiCall, {
            shouldRetryOnError: (error) => {
                ...
                if (classifyRetryError(error, { extraRetryErrorCodes }).kind ===
                    'transport') {
                    return true;
                }
                return false;
            ...
            ...(persistentMode
                ? {
                    heartbeatFn: (info) => {
                        process.stderr.write(`[lailatul-coder] Waiting for API capacity... attempt ${info.attempt}, retry in ${Math.ceil(info.remainingMs / 1000)}s\n`);
                    },
                }
                : {}),
            onRetry: (info) => {
        ...
        return this.processStreamResponse(model, rejectDegradedPlaceholderResponse(streamResponse), goalContext, transportContinuationPrefix);
    }
    async *makeFallbackStream(model, requestContents, params, prompt_id, contentGenerator, retryAuthType, retryErrorCodes, goalContext) {
        const stream = await this.makeApiCallAndProcessStream(model, requestContents, params, prompt_id, { contentGenerator, retryAuthType, retryErrorCodes }, goalContext);
        for await (const chunk of stream) {
            yield { type: StreamEventType.CHUNK, value: chunk };
        }
    }
     ...
     */
    getHistoryTail(count, curated = false) {
        if (count <= 0)
            return [];
        const history = curated
            ? extractCuratedHistory(this.history)
     ...
     * payload objects.
     */
    getHistoryShallow(curated = false) {
        const history = curated
            ? extractCuratedHistory(this.history)
            : this.history;
     ...
     */
    getHistoryTailShallow(count, curated = false) {
        if (count <= 0)
            return [];
        const history = curated
            ? extractCuratedHistory(this.history)
        ...
        for (const entry of this.history) {
            ...
            for (const part of entry.parts ?? []) {
                ...
                if (functionCall?.id && !fingerprintsById.has(functionCall.id)) {
                    fingerprintsById.set(functionCall.id, getFunctionCallFingerprint(functionCall));
                }
            }
        }
    ...
    addHistory(content) {
        ...
        if (this.pendingPartialAssistantTurnIndex !== null ||
            this.pendingPartialAssistantRecord !== null) {
            debugLogger.error('[INVARIANT_VIOLATION] addHistory called while a partial-push ' +
     ...
     */
    redactApprovedPlanFromHistory(callId, replacement, expectedPlan) {
        for (let i = this.history.length - 1; i >= 0; i--) {
            const entry = this.history[i];
            if (entry?.role !== 'model' || !entry.parts)
                continue;
            const partIdx = entry.parts.findIndex((part) => part.functionCall?.id === callId &&
    ...
    redactApprovedPlansFromLoadedHistory() {
        const hasPlanCall = this.history.some((entry) => entry?.parts?.some((part) => canonicalPlanToolName(part.functionCall?.name) ===
            ToolNames.EXIT_PLAN_MODE));
        if (!hasPlanCall)
            return;
        let planPath;
        ...
        try {
            ...
            debugLogger.debug(`Skipping load-side plan redaction, plan file unavailable: ${err}`);
            return;
        }
        const redacted = redactApprovedPlansInHistory(this.history, savedPlan, planPath);
        if (redacted) {
            ...
            debugLogger.debug(`Load-side plan redaction left history unchanged: no approved ` +
     ...
     */
    async *processStreamResponse(model, streamResponse, goalContext, transportContinuationPrefix) {
        // Collect ALL parts from the model response (including thoughts for recording)
        const allModelParts = [];
        const usedToolCallIds = collectToolCallIdsFromHistory(this.history);
        const rawToolCallIdsInCurrentTurn = new Set();
        const reservedToolCallIds = new Map();
        let usageMetadata;
        let coercedUsage;
        let hasToolCall = false;
        let hasFinishReason = false;
        const protocolTagDetector = new LeadingProtocolTagLeakDetector();
        let pendingProtocolParts = [];
        const takePendingProtocolParts = () => {
        ...
        const isToolResultContinuation = currentUserTurn?.role === 'user' &&
            currentUserTurn.parts?.some((part) => part.functionResponse) === true;
        let deferredFinishReason;
        ...
        try {
            for await (const chunk of streamResponse) {
                ...
                if (preparations.length > 0) {
                    setToolCallPreparations(chunk, preparations.map((preparation) => ({
                        ...preparation,
                        callId: reserveModelToolCallId(preparation.callId, usedToolCallIds, reservedToolCallIds),
                    })));
                }
                ...
                if (isValidResponse(chunk)) {
                    ...
                    if (content?.parts) {
                        ...
                        content.parts = normalizeModelToolCallIds(content.parts, usedToolCallIds, rawToolCallIdsInCurrentTurn, reservedToolCallIds);
                ...
                if (chunk.usageMetadata) {
                    ...
                    const hasUsablePromptTokenCount = typeof usageMetadata.promptTokenCount === 'number' &&
                        Number.isFinite(usageMetadata.promptTokenCount) &&
                        usageMetadata.promptTokenCount >= 0;
                    const hasUsableTotalTokenCount = typeof usageMetadata.totalTokenCount === 'number' &&
                        Number.isFinite(usageMetadata.totalTokenCount) &&
                        usageMetadata.totalTokenCount >= 0;
                    const promptTokenCount = coerceUsageCount(usageMetadata.promptTokenCount, 'promptTokenCount');
                    const totalTokenCount = coerceUsageCount(usageMetadata.totalTokenCount, 'totalTokenCount');
                    const candidatesTokenCount = coerceUsageCount(usageMetadata.candidatesTokenCount, 'candidatesTokenCount');
                    const cachedContentTokenCount = coerceUsageCount(usageMetadata.cachedContentTokenCount, 'cachedContentTokenCount');
                    const thoughtsTokenCount = coerceUsageCount(usageMetadata.thoughtsTokenCount, 'thoughtsTokenCount');
                    ...
                    if (cachedContentTokenCount && this.telemetryService) {
                        this.telemetryService.setLastCachedContentTokenCount(cachedContentTokenCount);
                    }
                }
                if (isToolResultContinuation) {
        ...
        if (streamError === null &&
            ...
            if (protocolTagDetector.leaked) {
                ...
                const parts = normalizeModelToolCallIds(takePendingProtocolParts(), usedToolCallIds, rawToolCallIdsInCurrentTurn, reservedToolCallIds);
                const chunk = {
        ...
        if (streamError === null &&
            ...
            if (recovery.recovered) {
                ...
                const insertAt = Math.min(textIndices[0] ?? 0, consolidatedHistoryParts.length);
                ...
                debugLogger.warn(`XML tool call fallback: recovered ${recovery.functionCallParts.length} tool call(s) [${recovery.functionCallParts.map((p) => p.functionCall?.name).join(', ')}] from plain text content (contentLength=${contentText.length})`);
            }
            else {
                debugLogger.warn(`XML tool call fallback: detected XML tool calls but recovery was rejected (prose ratio too high or no parameterized blocks), contentLength=${contentText.length}`);
            }
        }
        if (streamError === null && protocolTagDetector.leaked && !hasToolCall) {
            throw new InvalidStreamError('Model response started with leaked protocol tags.', 'PROTOCOL_TAG_LEAK');
        }
        ...
        const lacksVisibleToolResultProgress = isToolResultContinuation &&
        ...
        if (streamError === null &&
            ...
            if (lacksVisibleToolResultProgress) {
                throw new InvalidStreamError('Model stream ended after a tool result without visible progress.', deferredFinishReason === FinishReason.MAX_TOKENS
        ...
        const willPersistToHistory = streamError === null ||
            (hasToolCall &&
        ...
        if (streamError === null && transportContinuationPrefix) {
            ...
            if (textIndex < 0) {
                ...
                const remainderPart = consolidatedHistoryParts[textIndex];
        ...
        if (streamError !== null) {
            ...
            if (willPersistToHistory) {
                ...
                debugLogger.warn('[PARTIAL_PUSH] Persisting partial assistant turn for ' +
     ...
     */
    coalesceRecoveryPairs(pairCount) {
        for (let i = 0; i < pairCount; i++) {
            ...
            if (modelContinuation.role !== 'model' ||
                userRecovery.role !== 'user' ||
                precedingModel.role !== 'model') {
                return;
            }
            precedingModel.parts = appendRecoveryContinuationParts(precedingModel.parts, modelContinuation.parts);
