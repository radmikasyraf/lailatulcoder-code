...
const { ghRawMock, ensureAuthenticatedMock, setGhHostMock, writeStdoutLineMock, writeFileSyncMock, mkdirSyncMock, } = vi.hoisted(() => ({
    ghRawMock: vi.fn(),
...
describe('runFetchDiff', () => {
    ...
    it('writes the diff and reports its size', () => {
        ...
        expect(ghRawMock).toHaveBeenCalledWith('pr', 'diff', '8981', '--repo', 'LailatulCoder/lailatul-coder');
        ...
        expect(writeFileSyncMock).toHaveBeenCalledWith(resolve(OUT), 'diff --git a/x b/x\n+one\n+two\n', 'latin1');
...
//# sourceMappingURL=fetch-diff.test.js.map