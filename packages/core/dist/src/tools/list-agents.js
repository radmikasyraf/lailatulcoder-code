...
class ListAgentsInvocation extends BaseToolInvocation {
    ...
    async execute() {
        const agents = this.config
            ...
            status: entry.status,
            can_message: !entry.resumeBlockedReason &&
        ...
        if (agents.length === 0) {
            const message = 'No ordinary background subagents are available in this session. ' +
...
export class ListAgentsTool extends BaseDeclarativeTool {
    ...
    constructor(config) {
        super(ListAgentsTool.Name, ToolDisplayNames.LIST_AGENTS, 'List addressable ordinary background subagents in the current ' +
