...
describe('loadDaemonVoiceContext', () => {
    ...
    it('uses the injected runtime env for voice auth and model config resolution', async () => {
        ...
        const { loadDaemonVoiceContext } = await import('./resolve-voice-config.js');
        ...
        expect(mocks.resolveCliGenerationConfig).toHaveBeenCalledWith(expect.objectContaining({
            selectedAuthType: AuthType.USE_OPENAI,
            env: injectedEnv,
        }));
        expect(mocks.resolveVoiceTranscriptionConfig).toHaveBeenCalledWith(expect.objectContaining({
            env: injectedEnv,
    ...
    it('resolves providerProtocol-mapped custom provider groups for voice', async () => {
        ...
        const modelsArg = mocks.resolveVoiceTranscriptionConfig.mock.calls[0][0]
            .config;
        expect(modelsArg.getAllConfiguredModels().map((model) => model.id)).toContain('qwen3-asr-flash');
    });
...
//# sourceMappingURL=resolve-voice-config.test.js.map