...
import { getCurrentAgentDepth, getCurrentAgentId, getRuntimeContentGenerator, isTopLevelSession, runWithAgentContext, runWithRuntimeContentGenerator, spawnBlockReason, } from './agent-context.js';
import { createDuplicateProviderToolCallResponse, findRepeatedDuplicateProviderToolCall, GeminiEventType, markDuplicateProviderToolCallResponseSent, } from '../../core/turn.js';
import { LoopDetectionService } from '../../services/loopDetectionService.js';
import { CoreToolScheduler, } from '../../core/coreToolScheduler.js';
import { isShellProgressData } from '../../tools/tools.js';
import { getInitialChatHistory } from '../../utils/environmentContext.js';
import { finalizeToolResponses, } from '../../utils/tool-response-finalizer.js';
...
import { dedupeToolCallsById, getFunctionCallFingerprint, getProviderToolCallId, isReplayOfHandledToolCall, recordHandledToolCall, } from '../../core/toolCallIdUtils.js';
...
import { templateString } from './agent-headless.js';
import { getResponseText } from '../../utils/partUtils.js';
import { getThoughtSummary } from '../../utils/thoughtUtils.js';
import { isTeammate, getTeammateContext, runWithTeammateIdentity, } from '../team/identity.js';
import { getLeaderOnlyToolUnavailableMessage, getSubagentPlanToolUnavailableMessage, isLeaderOnlyToolUnavailableInSubagent, isPlanRequiredTeammateContext, isPlanLifecycleToolUnavailableInSubagent, SUBAGENT_PLAN_LIFECYCLE_TOOLS, } from './subagent-plan-tool-policy.js';
const EXECUTION_ALLOWLIST_ERROR_MAX_ITEMS = 8;
const EXECUTION_ALLOWLIST_ERROR_MAX_CHARS = 240;
function summarizeExecutionAllowlist(executionAllowedTools) {
    if (executionAllowedTools.length === 0) {
        return undefined;
    }
    const visibleTools = executionAllowedTools.slice(0, EXECUTION_ALLOWLIST_ERROR_MAX_ITEMS);
 ...
 */
export const EXCLUDED_TOOLS_FOR_SUBAGENTS = new Set([
    ToolNames.AGENT,
 ...
 */
export function extractParentToolNames(generationConfig) {
    return Array.from(new Set(generationConfig?.tools
 ...
 */
const EXCLUDED_TOOLS_FOR_TEAMMATES = new Set([
    ToolNames.AGENT,
    ToolNames.CRON_CREATE,
    ToolNames.CRON_LIST,
    ToolNames.CRON_DELETE,
...
function getExcludedToolsForCurrentContext() {
    if (!isTeammate()) {
...
export class AgentCore {
    ...
    executionAllowedExactTools;
    executionAllowedMcpPatterns;
    executionAllowlistErrorSummary;
    /**
    ...
    toolUsage = new Map();
    constructor(name, runtimeContext, promptConfig, modelConfig, runConfig, toolConfig, eventEmitter, hooks, runtimeView) {
        ...
        if (toolConfig?.executionAllowedTools !== undefined) {
            ...
            this.executionAllowedExactTools = new Set(this.executionAllowedTools.filter((toolName) => !toolName.includes('*')));
            this.executionAllowedMcpPatterns = Object.freeze(this.executionAllowedTools.filter((toolName) => toolName.includes('*')));
            this.executionAllowlistErrorSummary = summarizeExecutionAllowlist(this.executionAllowedTools);
        }
     ...
     */
    async createChat(context, options) {
        if (!this.promptConfig.systemPrompt &&
            !this.promptConfig.renderedSystemPrompt &&
            !this.promptConfig.initialMessages) {
            throw new Error('PromptConfig must have `systemPrompt`, `renderedSystemPrompt`, or `initialMessages` defined.');
        }
        if (this.promptConfig.systemPrompt &&
            this.promptConfig.renderedSystemPrompt) {
            throw new Error('PromptConfig cannot have both `systemPrompt` and `renderedSystemPrompt` defined.');
        }
        ...
        try {
            const chat = new GeminiChat(this.runtimeContext, generationConfig, startHistory);
            ...
            await reportError(error, 'Error initializing chat session.', startHistory, 'startChat');
            return undefined;
        }
    }
    // ─── Tool Preparation ─────────────────────────────────────
    ...
    async prepareTools() {
        ...
        const nestingAllowed = !isTopLevelSession() &&
        ...
        if (this.toolConfig) {
            ...
            if (hasWildcard ||
                ...
                const allowedNames = asStrings.filter((name) => {
                    if (isExcluded(name)) {
                        this.runtimeContext
                            .getDebugLogger()
                            ?.debug(`[prepareTools] Filtered "${name}" from explicit subagent tool list`);
                        return false;
                    }
                    return true;
                });
                toolsList.push(...toolRegistry.getFunctionDeclarationsFiltered(allowedNames));
            }
            ...
            toolsList.push(...onlyInlineDecls.filter((d) => {
                if (isExcluded(d.name)) {
                    this.runtimeContext
                        .getDebugLogger()
                        ?.debug(`[prepareTools] Filtered inline declaration "${d.name}" from subagent tool list`);
                    return false;
                }
                return true;
            }));
        ...
        if (this.toolConfig?.disallowedTools?.length) {
            ...
            return toolsList.filter((t) => {
                if (!t.name)
                    return true;
                return !disallowed.some((pattern) => t.name.startsWith('mcp__')
     ...
     */
    async runReasoningLoop(chat, initialMessages, toolsList, abortController, options) {
        const inner = () => this._runReasoningLoopInner(chat, initialMessages, toolsList, abortController, options);
        return runWithInvocationContext(undefined, () => this.runInAgentFrames(inner));
    }
    /**
     ...
     */
    runInAgentFrames(fn, inheritedView, inheritedAgentId, inheritedTeammateIdentity, inheritedAgentDepth) {
        const runInner = () => subagentNameContext.run(this.name, () => {
            ...
            return inheritedAgentId
                ? runWithAgentContext(inheritedAgentId, runWithView, inheritedAgentDepth)
                : runWithView();
        });
     ...
     */
    withRuntimeView(fn, inheritedView) {
        const view = this.runtimeView ?? inheritedView;
        return view ? runWithRuntimeContentGenerator(view, fn) : fn();
    }
    async _runReasoningLoopInner(chat, initialMessages, toolsList, abortController, options) {
        ...
        const handledToolCallFingerprints = new Map(chat.getHistoryToolCallFingerprints());
        ...
        let stickyMaxOutputTokens;
        const loopDetector = new LoopDetectionService(this.runtimeContext);
        loopDetector.reset(`${this.runtimeContext.getSessionId()}#${this.subagentId}`);
        const checkSubagentLoop = (event) => {
            if (loopDetector.checkAlwaysOnSafeties(event)) {
                return true;
            }
            return (!this.runtimeContext.getSkipLoopDetection() &&
            ...
            try {
                ...
                for await (const streamEvent of responseStream) {
                    ...
                    if (streamEvent.type === 'compressed') {
                        this.runtimeContext
                            .getDebugLogger()
                            .debug(`[AGENT-COMPACT] subagent=${this.subagentId} round=${turnCounter} ` +
                ...
                if (functionCalls.length > 0) {
                    const toolCallResult = await this.processFunctionCalls(functionCalls, roundAbortController, promptId, turnCounter, toolsList, currentResponseId, wasOutputTruncated, handledToolCallFingerprints, duplicateProviderToolCallResponseIds);
                    ...
                    if (immediateExternalInputs.length > 0) {
                        currentMessages = this.externalInputsToContent(immediateExternalInputs);
                        ...
                        const waitResult = await this.waitForExternalInputs(options, abortController, startTime, turnCounter);
    ...
    hasTurnBudgetForAnotherRound(options, turnCounter) {
        return !options?.maxTurns || turnCounter < options.maxTurns;
    }
    getRemainingTimeMs(options, startTime) {
        if (!options?.maxTimeMinutes)
            return undefined;
        return options.maxTimeMinutes * 60 * 1000 - (Date.now() - startTime);
    }
    async waitForExternalInputs(options, abortController, startTime, turnCounter) {
        while (true) {
            ...
            try {
                const inputs = await options.waitForExternalMessages(waitAbortController.signal);
                if (abortController.signal.aborted) {
                    return { inputs: [], terminateMode: AgentTerminateMode.CANCELLED };
                }
                if (timedOut) {
    ...
    // ─── Tool Execution ───────────────────────────────────────
    observeSyntheticToolResultProducer(params) {
        try {
    ...
    emitSyntheticToolError(params) {
        ...
        this.recordToolCallStats(params.name, false, params.durationMs ?? 0, params.errorMessage);
    }
    isToolExecutionAllowed(toolName) {
        ...
        const registeredTool = this.runtimeContext
            .getToolRegistry()
            .getTool(toolName);
        if (typeof registeredTool?.serverName !== 'string' ||
        ...
        if (this.executionAllowedExactTools?.has(serverPattern) ||
        ...
        return this.executionAllowedMcpPatterns.some((pattern) => {
            ...
            return (pattern.startsWith(toolPatternPrefix) &&
     ...
     */
    async processFunctionCalls(functionCalls, abortController, promptId, currentRound, toolsList, responseId, wasOutputTruncated = false, handledToolCallFingerprints = new Map(), duplicateProviderToolCallResponseIds = new Set()) {
        ...
        const callIdByFunctionCall = new Map(uniqueFunctionCalls.map((functionCall, index) => [
            functionCall,
            functionCall.id ??
        ...
        const repeatedDuplicateCall = findRepeatedDuplicateProviderToolCall(uniqueFunctionCalls, (fc) => getProviderToolCallId(fc) ?? fc.id, isReplayOfHandledCall, duplicateProviderToolCallResponseIds);
        if (repeatedDuplicateCall) {
            const providerCallId = getProviderToolCallId(repeatedDuplicateCall) ??
                repeatedDuplicateCall.id;
            this.runtimeContext
                .getDebugLogger()
                ?.debug(`[processFunctionCalls] Dropping batch after repeated duplicate provider tool-call id: ${providerCallId} (tool: ${String(repeatedDuplicateCall.name)}, round: ${currentRound})`);
            return {
        ...
        for (const fc of uniqueFunctionCalls) {
            ...
            if (providerCallId) {
                if (isReplayOfHandledCall(fc)) {
                    ...
                    this.runtimeContext
                        .getDebugLogger()
                        ?.debug(`[processFunctionCalls] Suppressing duplicate provider tool-call id: ${providerCallId} (tool: ${toolName}, round: ${currentRound})`);
                    this.emitSyntheticToolError({
                        callId: eventCallId,
                        name: toolName,
                        args,
        ...
        const executionStartedEmitted = new Set();
            ...
            onAllToolCallsComplete: async (completedCalls) => {
                for (const call of completedCalls) {
                    ...
                    this.eventEmitter?.emit(AgentEventType.TOOL_RESULT, {
                        ...
                        ...(isToolResultBoundaryDiagnosticsEnabled()
                            ? {
                                boundaryArtifact: toolResultBoundaryArtifact(call.response.persistedOutputFiles, call.response.artifacts),
                            }
                            : {}),
                        durationMs: duration,
                        timestamp: Date.now(),
            ...
            onToolCallsUpdate: (calls) => {
                for (const call of calls) {
                    ...
                    if (call.status === 'executing') {
                        ...
                        const isNewPid = pid !== undefined && !pidMap.has(call.request.callId);
                        if (pid !== undefined) {
                            pidMap.set(call.request.callId, pid);
                        }
                        const needsExecutionStartEmit = executing.executionStartTime !== undefined &&
                    ...
                    try {
                        ...
                        this.eventEmitter?.emit(AgentEventType.TOOL_WAITING_APPROVAL, {
                            ...
                            respond: async (outcome, payload) => {
                                ...
                                await this.runInAgentFrames(() => waiting.confirmationDetails.onConfirm(outcome, payload), inheritedView, inheritedAgentId ?? undefined, inheritedTeammateIdentity, inheritedAgentDepth);
                            },
                            timestamp: Date.now(),
                        });
                    }
        ...
        const orderedResponses = uniqueFunctionCalls.flatMap((fc) => {
        ...
        const finalizedResponses = await finalizeToolResponses(this.runtimeContext, orderedResponses, new Map(orderedResponses.map((response) => [response.callId, promptId])));
        const toolResponseParts = finalizedResponses.flatMap((response) => response.responseParts);
    ...
    setPendingApproval(callId, details) {
        this.pendingApprovals.set(callId, details);
    }
    deletePendingApproval(callId) {
    ...
    getToolIsOutputMarkdown(toolName) {
        try {
            const toolRegistry = this.runtimeContext.getToolRegistry();
    ...
    recordToolCallStats(toolName, success, durationMs, errorMessage) {
        ...
        this.stats.recordToolCall(toolName, success, durationMs, this.toolUsage.get(toolName)?.lastError);
    }
    // ─── Private Helpers ──────────────────────────────────────
    /**
    ...
    setupStateListeners() {
        ...
        emitter.on(AgentEventType.ROUND_TEXT, (event) => {
        ...
        emitter.on(AgentEventType.EXTERNAL_MESSAGE, (event) => {
     ...
     */
    buildChatSystemPrompt(context, options) {
        if (!this.promptConfig.systemPrompt) {
            return '';
        }
...
Important Rules:
 ...
 - When the task is complete, return the final result as a normal model response (not a tool call) and stop.`;
     ...
     */
    recordTokenUsage(usage, turnCounter, roundStreamStart) {
        ...
        if (isFinite(inTok) ||
            ...
            isFinite(thoughtTok) ||
            isFinite(cachedTok)) {
            this.stats.recordTokens(isFinite(inTok) ? inTok : 0, isFinite(outTok) ? outTok : 0, isFinite(thoughtTok) ? thoughtTok : 0, isFinite(cachedTok) ? cachedTok : 0, isFinite(totalTok) ? totalTok : 0);
