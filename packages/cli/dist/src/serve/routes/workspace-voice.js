...
function broadcastVoiceWrite(deps, write, clientId) {
    try {
        ...
        writeStderrLine(`LailatulCoder serve: POST /workspace/voice broadcast error (key=${write.key}, scope=${voiceSettingsScopeToWire(write.scope)}): ${err instanceof Error ? err.message : String(err)}`);
    }
}
        ...
        for (const write of writes) {
            try {
                ...
                if (isGenerationClosedError(err))
                    throw err;
                writeStderrLine(`LailatulCoder serve: POST /workspace/voice partial persist error (workspace=${deps.boundWorkspace}, committed=${committed.length}/${writes.length}, failedKey=${write.key}, failedScope=${voiceSettingsScopeToWire(write.scope)}): ${err instanceof Error ? err.message : String(err)}`);
...
async function handleVoiceUpdate(req, res, deps, route) {
    ...
    try {
        ...
        if (sendGenerationClosedError(res, err))
            return;
        writeStderrLine(`LailatulCoder serve: ${route} persist error (workspace=${deps.boundWorkspace}): ${err instanceof Error ? err.message : String(err)}`);
        res.status(500).json({
    ...
    try {
        ...
        if (sendGenerationClosedError(res, err))
            return;
        writeStderrLine(`LailatulCoder serve: ${route} reload error after persist (workspace=${deps.boundWorkspace}): ${err instanceof Error ? err.message : String(err)}`);
        res.status(500).json({
...
async function handleVoiceTranscription(req, res, deps, route) {
    ...
    try {
        ...
        writeStderrLine(`LailatulCoder serve: ${route} error (workspace=${deps.boundWorkspace}): ${message}`);
        res.status(502).json({
