...
const { MessageRewriteMiddleware } = await import('./MessageRewriteMiddleware.js');
function createMiddleware(target = 'all', sendUpdate) {
    const mockSendUpdate = sendUpdate ?? vi.fn().mockResolvedValue(undefined);
    const middleware = new MessageRewriteMiddleware({}, { enabled: true, target, prompt: 'test prompt' }, mockSendUpdate);
...
describe('MessageRewriteMiddleware', () => {
    ...
    describe('interceptUpdate — target filtering', () => {
        it.each(['message', 'all'])('does not carry slash-command metadata into the next %s rewrite', async (target) => {
            ...
            const rewriter = vi.mocked(LlmRewriter).mock.results[0]?.value;
    ...
    describe('waitForPendingRewrites', () => {
        ...
        it('should be safe to call when no rewrites are pending', async () => {
            ...
            await expect(middleware.waitForPendingRewrites()).resolves.toBeUndefined();
        });
    });
    describe('rewrite metadata', () => {
        it('should emit rewritten message with _meta.rewritten=true', async () => {
            ...
            await middleware.flushTurn();
            await middleware.waitForPendingRewrites();
            const rewriteCall = mockSendUpdate.mock.calls.find((call) => call[0]['_meta'] !== undefined);
            expect(rewriteCall).toBeDefined();
            const meta = rewriteCall[0]['_meta'];
        ...
        it('preserves background discrete metadata on rewritten messages', async () => {
            ...
            await middleware.flushTurn();
            await middleware.waitForPendingRewrites();
            const rewriteCall = mockSendUpdate.mock.calls.find((call) => call[0]['_meta']?.['rewritten'] === true);
            expect(rewriteCall).toBeDefined();
            expect(rewriteCall[0]['_meta']).toEqual({
                source: 'background_notification_response',
                qwenDiscreteMessage: true,
                backgroundTask: {
                    taskId: 'monitor-1',
    ...
    describe('timeoutMs config', () => {
        it('should use configured timeoutMs for the rewrite abort signal', async () => {
            ...
            try {
                ...
                const { LlmRewriter } = await import('./LlmRewriter.js');
                LlmRewriter.mockImplementation(() => ({
                    rewrite: vi.fn((_content, signal) => {
        ...
        it('should default to 30s when timeoutMs is not provided', async () => {
            ...
            try {
                ...
                const mockSendUpdate = vi.fn().mockResolvedValue(undefined);
                const middleware = new MessageRewriteMiddleware({}, { enabled: true, target: 'all', prompt: 'test prompt' }, mockSendUpdate);
                await middleware.interceptUpdate({
                    sessionUpdate: 'agent_message_chunk',
                    content: { type: 'text', text: 'content' },
                });
    ...
    describe('waitForPendingRewrites — draining late arrivals', () => {
        it('waits for a rewrite enqueued while it is already draining', async () => {
            ...
            try {
                ...
                const rewriteCalls = mockSendUpdate.mock.calls.filter((call) => call[0]['_meta']?.['rewritten'] === true);
...
//# sourceMappingURL=MessageRewriteMiddleware.test.js.map