 ...
 */
import { HOSTNAME_RE, isOwnerRepo, resolveGhHost, setGhHost, } from './lib/gh.js';
import { getPlatformReader } from './lib/platform/registry.js';
import { writeStdoutLine, writeStderrLineSafe, } from '../../utils/stdioHelpers.js';
export function runMeta(args) {
    ...
    if (args.repo !== undefined) {
        ...
        if ((args.host === undefined || args.host.trim() === '') &&
            ...
            throw new TypeError(`--repo on a ${platform.kind} target needs --host — there is no default host off GitHub`);
        }
        ...
        if (!HOSTNAME_RE.test(host)) {
            throw new Error(`cannot route at the ${args.host !== undefined ? '--host flag' : 'GH_HOST environment'} ${JSON.stringify(host)} — not a hostname the review subcommands accept`);
        }
        ...
        const routed = (platform.kind === 'github'
            ? resolveGhHost(args.host)
            : (args.host ?? '').trim() || undefined) ?? id.host;
        if (!HOSTNAME_RE.test(routed)) {
            throw new Error(`cannot route at the ${args.host !== undefined ? '--host flag' : 'discovered repo-URL host'} ${JSON.stringify(routed)} — not a hostname the review subcommands accept`);
        }
        setGhHost(routed);
    }
    const result = { platform: platform.kind, host, ownerRepo };
...
export const metaCommand = {
    ...
    describe: 'Print the review platform identity facts for this repository (and, with a PR number, its live head SHA and URL) as one JSON object',
    builder: (yargs) => yargs
        .positional('pr_number', {
        type: 'number',
        describe: 'A PR number — adds its live headSha and webUrl to the output',
    })
        .option('repo', {
        type: 'string',
        describe: 'owner/repo — skips the cwd repository resolution (a bare number resolves through the upstream of a fork clone)',
    })
        .option('host', {
        type: 'string',
    ...
    handler: (argv) => {
        ...
        if (prNumber !== undefined &&
            ...
            writeStderrLineSafe(`meta: pr_number must be a positive integer, got ${JSON.stringify(argv['pr_number'])}`);
            process.exitCode = 2;
            return;
        }
        ...
        try {
            ...
            writeStderrLineSafe(`meta: ${err.message}`);
