...
function withWarnings(description, details) {
    const warnings = Array.isArray(details['warnings'])
        ? details['warnings'].filter((warning) => typeof warning === 'string')
        : [];
    return warnings.length > 0
        ? `${warnings.join('\n')}\n${description}`
        : description;
}
export function buildPermissionSuggestions(confirmationDetails) {
            ...
            return [
                {
                    type: 'allow',
                    label: 'Allow Edit',
                    description: withWarnings(`Edit file: ${details['fileName']}`, details),
                },
                {
                    type: 'deny',
                    label: 'Deny',
...
//# sourceMappingURL=permission-suggestions.js.map