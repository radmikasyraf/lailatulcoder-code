...
import { LIVE_HOST_BUNDLE_ID, LIVE_HOST_PROTOCOL_VERSION, LIVE_INPUT_AUDIO_EPOCH_BYTES, } from './types.js';
...
const MAX_HOST_AUDIO_WIRE_BYTES = LIVE_INPUT_AUDIO_EPOCH_BYTES + MAX_HOST_AUDIO_BYTES;
...
function writeLiveHostDiagnostic(event, details) {
...
function parseAction(value) {
    ...
    if (action === 'mute') {
        ...
        if ((inputMuted === undefined && outputMuted === undefined) ||
...
function permissionRequirement(value) {
    if (value === 'granted')
        return 'ready';
    if (value === 'denied')
        return 'denied';
...
function projectStatusForHost(status) {
    return {
        v: status.v,
...
export class LiveHostCoordinator {
    ...
    outputMuted = false;
    lastCallError;
    pendingAppshots = new Map();
    pendingShortcut;
    inactiveWaiters = new Set();
    constructor(options) {
    ...
    setConfiguredShortcut(shortcut) {
    ...
    attachHost(socket, daemonNonce) {
        ...
        if (this.host)
            this.disconnectHost(this.host, 4008, 'Host lease expired.');
        const lease = {
            socket,
            lastPongAt: this.now(),
        ...
        socket.on('message', (data, isBinary) => {
            if (this.host !== lease)
                return;
            if (isBinary)
                this.handleAudioFrame(lease, data);
            else
                this.handleTextFrame(lease, data);
        });
        socket.on('close', () => {
            if (this.host === lease)
                this.detachHost(lease, 'host_disconnected');
    ...
    buildStatus(stopOnReadinessLoss) {
        ...
        if (hello) {
            requirements.microphone = permissionRequirement(hello.permissions.microphone);
            requirements.accessibility = permissionRequirement(hello.permissions.accessibility);
            requirements.screenRecording = permissionRequirement(hello.permissions.screenRecording);
        ...
        const preserveCheckingCall = providerChecking && blocker === undefined && this.call !== undefined;
        if (!available &&
            this.call &&
            stopOnReadinessLoss &&
    ...
    start(mode) {
        ...
        if (!status.available)
            throw new LiveUnavailableError(status);
    ...
    startCall(mode) {
        ...
        this.broadcastState();
        try {
            void Promise.resolve(this.handlers.onStart?.({
                epoch: call.epoch,
                callId: call.callId,
                mode,
            })).catch(() => {
    ...
    setMute(update) {
        ...
        if (update.outputMuted !== undefined) {
            ...
            if (update.outputMuted && this.call)
    ...
    setShortcut(shortcut) {
        ...
        if (this.pendingShortcut) {
            return Promise.reject(new Error('Another Live shortcut update is already in progress.'));
        }
        const host = this.host;
        if (!host?.hello || !this.isLeaseHealthy(host)) {
            return Promise.reject(new Error('LailatulCoder Live Host must be connected to change the shortcut.'));
        }
        const requestId = randomUUID();
        return new Promise((resolve, reject) => {
            const timer = setTimeout(() => {
                this.rejectPendingShortcut(new Error('LailatulCoder Live Host did not confirm the shortcut change.'));
            ...
            if (!this.sendHost({
                ...
                this.rejectPendingShortcut(new Error('LailatulCoder Live Host is unavailable for shortcut changes.'));
            }
        });
    }
    setCallState(epoch, state) {
        if (!this.call || this.call.epoch !== epoch)
            return false;
        if (this.call.state === state)
            return true;
        this.call.state = state;
        this.broadcastState();
        return true;
    }
    setCoordinator(epoch, locator) {
        if (!this.call || this.call.epoch !== epoch)
            return false;
    ...
    captureScreenContext(callerSessionId) {
        ...
        if (!call ||
            ...
            return Promise.reject(new Error('Appshot is available only to the active Live session with a ready Host.'));
        }
        const requestId = randomUUID();
        return new Promise((resolve, reject) => {
            const timer = setTimeout(() => {
            ...
            if (!this.sendHost({
                ...
                this.rejectPendingAppshot(requestId, new Error('Live Host is unavailable for Appshot.'));
            }
        });
    }
    ...
    setAppshotReadiness(readiness) {
        this.appshotReadiness = { ...readiness };
        this.sendState(this.getStatus());
    }
    failCall(epoch, message = 'Live Voice failed.') {
        if (!this.call || this.call.epoch !== epoch)
            return false;
        const call = this.call;
        this.pendingStartMode = undefined;
    ...
    resolveBlocker(provider, appshot, hello) {
        if (provider.state === 'unavailable') {
            return provider.blocker ?? 'provider_config';
        }
        if (!hello) {
            if (this.lastHostFailure === 'host_version')
                return 'host_version';
        ...
        if (!hello.selfChecks.audioInput)
            return 'audio_input';
        if (!hello.selfChecks.audioOutput)
            return 'audio_output';
        if (!hello.selfChecks.globalShortcut)
            return 'global_shortcut';
        if (!hello.selfChecks.appshot)
            return 'appshot';
        if (appshot.state !== 'ready')
            return 'appshot';
        return undefined;
    }
    blockerMessage(blocker, provider, appshot, hello) {
        if ((blocker === 'provider_config' || blocker === 'provider_unreachable') &&
            provider.message) {
        ...
        const messages = {
            host_missing: 'LailatulCoder Live Host is not connected.',
            host_disconnected: 'LailatulCoder Live Host disconnected.',
            host_version: 'LailatulCoder Live Host is not protocol-compatible.',
    ...
    isLeaseHealthy(lease) {
        return (lease.socket.readyState === WebSocket.OPEN &&
            this.now() - lease.lastPongAt <= this.heartbeatTimeoutMs);
    }
    handleTextFrame(lease, data) {
    ...
    handleShortcutResult(message) {
        ...
        if (message.shortcut !== pending.shortcut) {
            pending.reject(new Error('LailatulCoder Live Host returned a mismatched shortcut.'));
            return;
        }
        if (!message.success) {
            pending.reject(new Error(message.error || 'The Live shortcut could not be registered.'));
            return;
        }
    ...
    handleScreenContextResult(message) {
        ...
        if (!call || call.epoch !== pending.epoch) {
            this.rejectPendingAppshot(message.requestId, new Error('The Live call changed before Appshot completed.'));
            return;
        }
    ...
    handleHello(lease, hello) {
        if (hello.protocolVersion !== LIVE_HOST_PROTOCOL_VERSION ||
        ...
        if (!lease.heartbeatTimer) {
            lease.heartbeatTimer = setInterval(() => this.heartbeat(lease), this.heartbeatIntervalMs);
            lease.heartbeatTimer.unref?.();
        }
        try {
            void Promise.resolve(this.handlers.onHostReady?.()).catch(() => undefined);
        }
        catch {
    ...
    handleAudioFrame(lease, data) {
        ...
        const audio = Buffer.isBuffer(data)
            ...
            : Array.isArray(data)
                ? Buffer.concat(data)
                : Buffer.from(data);
        if (audio.byteLength <= LIVE_INPUT_AUDIO_EPOCH_BYTES ||
    ...
    finishCall(call) {
        ...
        this.call = undefined;
        this.notifyInactive();
        this.rejectPendingAppshots(new Error('The Live call ended before Appshot completed.'), call.epoch);
        ++this.nextEpoch;
        try {
            void Promise.resolve(this.handlers.onStop?.({ epoch: call.epoch, callId: call.callId })).catch(() => { });
        }
        catch {
    ...
    beginCallStop(call) {
        if (this.call !== call || call.state === 'stopping')
            return;
        call.state = 'stopping';
        this.sendState(this.buildStatus(false));
        this.clearOutput(call.epoch);
        ...
        void Promise.resolve(result).then((outcome) => this.finishStoppingCall(call, outcome), () => this.failStoppingCall(call, 'Live Voice failed to stop safely.'));
    }
    finishStoppingCall(call, outcome) {
        if (this.call !== call || call.state !== 'stopping')
            return;
        if (outcome?.error) {
    ...
    disconnectHost(lease, code, reason) {
        if (lease.socket.readyState === WebSocket.OPEN) {
            lease.socket.close(code, reason);
        }
        this.detachHost(lease, 'host_disconnected');
    }
    detachHost(lease, failure) {
        if (this.host !== lease)
            return;
        this.host = undefined;
        this.clearLeaseTimers(lease);
        this.lastHostFailure = failure;
        this.rejectPendingAppshots(new Error('LailatulCoder Live Host disconnected.'));
        this.rejectPendingShortcut(new Error('LailatulCoder Live Host disconnected.'));
        this.stopForReadinessLoss();
    }
    rejectPendingShortcut(error) {
        const pending = this.pendingShortcut;
        if (!pending)
            return;
        this.pendingShortcut = undefined;
    ...
    rejectPendingAppshot(requestId, error) {
    ...
    rejectPendingAppshots(error, epoch) {
    ...
    notifyInactive() {
        if (this.call)
            return;
        for (const resolve of this.inactiveWaiters)
            resolve();
        this.inactiveWaiters.clear();
    }
}
//# sourceMappingURL=live-host-coordinator.js.map