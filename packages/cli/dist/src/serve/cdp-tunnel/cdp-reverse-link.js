...
export class CdpReverseLink {
    ...
    forwardToTab = async (method, params) => {
        ...
        return new Promise((resolve, reject) => {
            ...
            try {
                this.log?.(`LailatulCoder serve: /cdp forwarded command id=${id} method=${method} to extension`);
    ...
    handleResult(frame) {
        ...
        if (id === undefined) {
            this.log?.('LailatulCoder serve: /cdp dropped cdp_result with non-numeric id');
            return;
        }
        if (frame.error) {
    ...
    handleEvent(frame) {
        ...
        if (typeof frame.method !== 'string') {
            this.log?.('LailatulCoder serve: /cdp dropped cdp_event with non-string method');
            return;
        }
    ...
    handleAttached(frame) {
        ...
        if (!attach || attach.id !== frame.id) {
            this.log?.(`LailatulCoder serve: /cdp dropped unexpected cdp_attached (id=${String(frame.id)})`);
            return;
        }
        this.pendingAttach = undefined;
    ...
    armProgressLog(id, method) {
        ...
        if (delay >= this.commandTimeoutMs)
            return undefined;
        const timer = setTimeout(() => {
            this.log?.(`LailatulCoder serve: /cdp still waiting for command id=${id} method=${method} after ${delay}ms`);
        }, delay);
        timer.unref?.();
        return timer;
    }
