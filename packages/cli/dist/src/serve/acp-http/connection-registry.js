...
import { ACP_PRE_ATTACH_MAX_FRAMES_PER_CONNECTION, ACP_PRE_ATTACH_MAX_FRAMES_PER_STREAM, ACP_PRE_ATTACH_MAX_PAYLOAD_BYTES_PER_CONNECTION, AcpPreAttachBudget, } from './pre-attach-budget.js';
...
export class AcpConnection {
    ...
    maxPayloadBytesPerConnection;
    onPreAttachGuardFailure;
    connectionId;
    ...
    nextPreparedSequence = 0;
    ownedFrames = 0;
    ownedBytes = 0;
    connOwnedFrames = 0;
    pendingReceipts = new Set();
    sessionOwnershipStates = new Map();
    sessions = new Map();
    /**
     ...
     */
    onSessionGraceExpired;
    lastActiveMs = Date.now();
    idCounter = 0;
    constructor(connectionId, fromLoopback, onAbandonPending, onDetachSession, preAttachBudget = new AcpPreAttachBudget(), onFatalConnection, maxFramesPerConnection = ACP_PRE_ATTACH_MAX_FRAMES_PER_CONNECTION, maxPayloadBytesPerConnection = ACP_PRE_ATTACH_MAX_PAYLOAD_BYTES_PER_CONNECTION, onPreAttachGuardFailure) {
        this.onAbandonPending = onAbandonPending;
        this.onDetachSession = onDetachSession;
        this.preAttachBudget = preAttachBudget;
        this.onFatalConnection = onFatalConnection;
        this.maxFramesPerConnection = maxFramesPerConnection;
        this.maxPayloadBytesPerConnection = maxPayloadBytesPerConnection;
        this.onPreAttachGuardFailure = onPreAttachGuardFailure;
    ...
    captureSessionOwnershipIdentity(sessionId) {
    ...
    canCommitSessionOwnership(sessionId, ownership) {
    ...
    releaseSessionOwnershipIdentity(sessionId, ownership) {
        const state = this.sessionOwnershipStates.get(sessionId);
        if (!state || state.generation < ownership.generation)
            return;
        state.captures -= 1;
        if (state.captures === 0) {
    ...
    markInitialReplayPending(sessionId) {
        this.getOrCreateSession(sessionId).initialReplayPending = true;
    }
    hasInitialReplayPending(sessionId) {
        return this.sessions.get(sessionId)?.initialReplayPending === true;
    }
    markInitialReplayComplete(sessionId) {
        const binding = this.sessions.get(sessionId);
        if (binding)
            binding.initialReplayPending = false;
    }
    getDiagnostic() {
        ...
        for (const binding of this.sessions.values()) {
            ...
            pendingDeliveryFrames += binding.buffer.filter((prepared) => prepared.lease !== undefined &&
                prepared.deliveryAttempt !== undefined).length;
        ...
        return {
            ...
            connectionStreamOpen: this.connStream !== undefined && !this.connStream.isClosed,
            sessionStreams,
            sseStreams,
            wsStreams,
            bufferedConnectionFrames: this.connBuffer.length,
            bufferedSessionFrames,
            pendingDeliveryFrames: pendingDeliveryFrames +
                [...this.pendingDeliveries].filter((prepared) => prepared.lease !== undefined).length,
     ...
     */
    sendSession(sessionId, frame, id) {
        const binding = this.sessions.get(sessionId);
        if (!binding)
        ...
        return this.prepareAndBuffer(binding.buffer, frame, id, undefined, undefined, binding);
    }
    /**
     ...
     * non-resumed stream).
     */
    sendSessionReply(sessionId, frame, anchorId) {
        const binding = this.sessions.get(sessionId);
        if (!binding)
            return Promise.resolve('closed');
        return this.prepareAndBuffer(binding.buffer, frame, undefined, anchorId, undefined, binding, true);
    }
    /**
     ...
     */
    releaseDeferredSessionReplies(sessionId, deliveredId) {
        const binding = this.sessions.get(sessionId);
        if (!binding || !binding.stream || binding.stream.isClosed)
            return;
     ...
     */
    attachSessionStream(sessionId, stream, abort, resumeFromEventId) {
        ...
        if (binding.graceTimer) {
            ...
            writeStderrLine(`LailatulCoder serve: /acp session reclaimed within grace (${logSafe(sessionId)})`);
        }
        ...
        const replayingOnAttach = resumeFromEventId !== undefined || binding.initialReplayPending === true;
        ...
        if (binding.replayPending) {
            ...
            writeStderrLine(`LailatulCoder serve: /acp replay deferral armed (${logSafe(sessionId)}, from id ${resumeFromEventId ?? 'initial'})`);
        }
     ...
     */
    endReplayDeferral(sessionId, lastReplayedId, evictionOccurred = false) {
        const binding = this.sessions.get(sessionId);
        if (!binding)
            return;
     ...
     */
    flushBufferedSessionFrames(sessionId) {
        const binding = this.sessions.get(sessionId);
        if (!binding)
            return;
     ...
     */
    detachSessionStream(sessionId, stream, graceMs) {
        const binding = this.sessions.get(sessionId);
        if (!binding || binding.stream !== stream)
            return;
        ...
        writeStderrLine(`LailatulCoder serve: /acp session stream detached (${logSafe(sessionId)}), ` +
            `grace=${graceMs}ms`);
        ...
        binding.graceTimer = setTimeout(() => {
            ...
            writeStderrLine(`LailatulCoder serve: /acp session grace expired (${logSafe(sessionId)}), ` +
            ...
            try {
                ...
                writeStderrLine(`LailatulCoder serve: /acp teardown failed during grace expiry ` +
                    `(${logSafe(sessionId)}): ` +
            ...
            try {
                ...
                writeStderrLine(`LailatulCoder serve: /acp onSessionGraceExpired failed during grace expiry ` +
                    `(${logSafe(sessionId)}): ` +
                    (err instanceof Error ? err.message : String(err)));
            }
        }, graceMs);
    ...
    destroy(reason) {
        ...
        for (const binding of bindings) {
            try {
                ...
                writeStderrLine(`LailatulCoder serve: /acp teardownBinding(${logSafe(binding.sessionId)}) failed during destroy: ${err instanceof Error ? err.message : String(err)}`);
            }
        }
    ...
    sendLive(stream, frame, id, receipt, binding) {
        let payload;
        try {
    ...
    isCurrentStreamOwner(stream, binding) {
        if (this.destroyed || stream.isClosed)
            return false;
        return binding
    ...
    prepareAndBuffer(buffer, frame, id, anchorId, receipt, binding, requeueOnStreamReplacement = false) {
        if (this.destroyed) {
            receipt?.discarded();
            return Promise.resolve('closed');
        }
        const expectedStream = binding ? binding.stream : this.connStream;
        if ((binding ? binding.ownedFrames : this.connOwnedFrames) >=
        ...
        const deliverImmediately = buffer.length === 0 &&
            liveStream &&
            !liveStream.isClosed &&
            !deliveryDeferred;
        ...
        if (!deliverImmediately) {
            if (this.ownedFrames >= this.maxFramesPerConnection ||
    ...
    sendSerializedLive(stream, payload, id, receipt) {
        const result = stream.sendSerialized(payload, id);
        this.observeLiveReceipt(result, receipt);
        return result;
    }
    observeLiveReceipt(result, receipt) {
    ...
    deliverPrepared(stream, prepared) {
        ...
        void stream.sendSerialized(prepared.payload, prepared.id).then((outcome) => this.settlePreparedAttempt(prepared, attempt, outcome), () => this.settlePreparedAttempt(prepared, attempt, 'failed'));
    }
    requeuePendingSessionReplies(binding, previousStream) {
        ...
        const requeued = [...this.pendingDeliveries].filter((prepared) => prepared.binding === binding &&
            prepared.deliveryStream === previousStream &&
            prepared.requeueOnStreamReplacement);
        for (const prepared of requeued) {
            if (!this.reservePreparedForBuffer(prepared, binding))
                return false;
        }
        for (const prepared of requeued) {
            if (prepared.binding !== binding ||
    ...
    requeuePendingConnectionFrames(previousStream) {
        const requeued = [...this.pendingDeliveries].filter((prepared) => prepared.binding === undefined &&
            prepared.deliveryStream === previousStream &&
            prepared.requeueOnStreamReplacement);
        for (const prepared of requeued) {
            if (!this.reservePreparedForBuffer(prepared))
                return false;
        }
        for (const prepared of requeued) {
    ...
    settlePreparedAttempt(prepared, attempt, outcome) {
        if (prepared.deliveryAttempt !== attempt)
            return;
    ...
    reservePreparedForBuffer(prepared, binding) {
        if (prepared.lease)
            return true;
        if ((binding ? binding.ownedFrames : this.connOwnedFrames) >=
            ACP_PRE_ATTACH_MAX_FRAMES_PER_STREAM) {
    ...
    insertPrepared(buffer, prepared) {
        const index = buffer.findIndex((buffered) => buffered.sequence > prepared.sequence);
        if (index === -1)
            buffer.push(prepared);
        else
            buffer.splice(index, 0, prepared);
    }
    ...
    settlePrepared(prepared, outcome) {
        ...
        if (!prepared.receiptSettled) {
            ...
            if (prepared.receipt) {
                this.invokeReceipt(prepared.receipt, outcome === 'delivered'
                    ? 'delivered'
    ...
    discardPreparedReceipt(prepared) {
        if (prepared.receiptSettled)
            return;
        prepared.receiptSettled = true;
        if (prepared.receipt)
            this.invokeReceipt(prepared.receipt, 'discarded');
    }
    invokeReceipt(receipt, outcome) {
        try {
            ...
            writeStderrLine(`LailatulCoder serve: /acp ${outcome} receipt callback failed`);
        }
    }
    failOwner(binding, message) {
        this.preAttachBudget.recordGuardFailure();
        this.onPreAttachGuardFailure?.();
        writeStderrLine(`LailatulCoder serve: /acp resource guard: ${message}`);
        if (!binding ||
        ...
        try {
            ...
            writeStderrLine(`LailatulCoder serve: /acp resource guard session teardown failed: ${error instanceof Error ? error.message : String(error)}`);
        }
    }
    failSerializationOwner(binding) {
        writeStderrLine('LailatulCoder serve: /acp response serialization failed');
        if (binding)
            this.failOwner(binding, 'ACP response serialization failed');
    }
    failConnection(message, recordFailure = true) {
        if (recordFailure)
            this.preAttachBudget.recordGuardFailure();
        this.onPreAttachGuardFailure?.();
        writeStderrLine(`LailatulCoder serve: /acp resource guard: ${message}`);
    ...
    retireConnection(_message) {
        const reason = { code: 1013, reason: 'Resource limit' };
        if (this.onFatalConnection)
            this.onFatalConnection(this, reason);
        else
            this.destroy(reason);
    }
    /**
     ...
     */
    abandonPendingForSession(sessionId, clientId) {
        for (const [id, req] of this.pending) {
            ...
            if (!cancelled) {
                writeStderrLine(`LailatulCoder serve: /acp MEDIATOR STUCK: abandonPendingForSession(${logSafe(sessionId)}) cancel failed for ${logSafe(req.bridgeRequestId)}`);
            }
        }
    }
}
...
export class ConnectionRegistry {
    ...
    constructor(onAbandonPending, onDetachSession, maxConnections = DEFAULT_MAX_CONNECTIONS, idleTtlMs = 30 * 60_000, preAttachBudget = new AcpPreAttachBudget(), maxFramesPerConnection = ACP_PRE_ATTACH_MAX_FRAMES_PER_CONNECTION, maxPayloadBytesPerConnection = ACP_PRE_ATTACH_MAX_PAYLOAD_BYTES_PER_CONNECTION) {
        this.onAbandonPending = onAbandonPending;
        this.onDetachSession = onDetachSession;
    ...
    create(fromLoopback) {
        ...
        const conn = new AcpConnection(undefined, fromLoopback, this.onAbandonPending, this.onDetachSession, this.preAttachBudget, (failed, reason) => this.deleteExact(failed, reason), this.maxFramesPerConnection, this.maxPayloadBytesPerConnection, () => {
     ...
     */
    findPendingPermission(requestId, sessionId) {
        for (const conn of this.byId.values()) {
    ...
    deleteExact(conn, reason) {
        if (this.byId.get(conn.connectionId) !== conn)
            return false;
    ...
    getSnapshot() {
        const connections = [...this.byId.values()].map((conn) => conn.getDiagnostic());
        return {
            connectionCount: this.byId.size,
            connectionCap: this.maxConnections > 0 && Number.isFinite(this.maxConnections)
                ? this.maxConnections
                : null,
            ...
            pendingClientRequests: sumBy(connections, (conn) => conn.pendingClientRequests),
            bufferedConnectionFrames: sumBy(connections, (conn) => conn.bufferedConnectionFrames),
            bufferedSessionFrames: sumBy(connections, (conn) => conn.bufferedSessionFrames),
            pendingDeliveryFrames: sumBy(connections, (conn) => conn.pendingDeliveryFrames),
            preAttachOwnedFrames: sumBy(connections, (conn) => conn.preAttachOwnedFrames),
            preAttachOwnedBytes: sumBy(connections, (conn) => conn.preAttachOwnedBytes),
    ...
    sweep() {
        ...
        for (const [id, conn] of this.byId) {
            ...
            writeStderrLine(`LailatulCoder serve: /acp reaping idle connection ${id} ` +
