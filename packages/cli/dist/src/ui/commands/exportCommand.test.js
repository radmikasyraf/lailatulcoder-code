...
describe('exportCommand', () => {
    ...
    beforeEach(() => {
        ...
        vi.mocked(toHtml).mockReturnValue('<html><script id="chat-data" type="application/json">{"data": "test"}</script></html>');
        vi.mocked(toJson).mockReturnValue('{"messages":[]}');
        vi.mocked(toJsonl).mockReturnValue('{"type":"session_metadata"}');
        vi.mocked(generateExportFilename).mockImplementation((ext) => `export-2025-01-01T00-00-00-000Z.${ext}`);
    ...
    describe('command structure', () => {
        it('should have correct name and description', () => {
            ...
            expect(exportCommand.description).toBe('Export current session message history to a file');
        });
    ...
    describe('exportMarkdownAction', () => {
        it('should export session to markdown file', async () => {
            ...
            expect(collectSessionData).toHaveBeenCalledWith(mockSessionData.conversation, expect.anything());
            ...
            expect(fs.writeFile).toHaveBeenCalledWith(expect.stringContaining('export-2025-01-01T00-00-00-000Z.md'), '# Test Markdown', { encoding: 'utf-8', mode: 0o600 });
        });
        it('should export markdown to a relative custom directory', async () => {
            ...
            const filepath = path.join(outputDir, 'export-2025-01-01T00-00-00-000Z.md');
            expect(result).toEqual({
                type: 'message',
                messageType: 'info',
                content: expect.stringContaining(path.join('./logs', 'export-2025-01-01T00-00-00-000Z.md')),
            });
        ...
        it('should keep cwd-equivalent directory output concise', async () => {
            ...
            expect(result).toEqual({
                type: 'message',
                messageType: 'info',
                content: 'Session exported to markdown: export-2025-01-01T00-00-00-000Z.md',
            });
            expect(fs.mkdir).not.toHaveBeenCalled();
        });
        ...
        it('should handle session load errors', async () => {
            mockSessionServiceMocks.loadSession.mockRejectedValue(new Error('EIO: failed to read session'));
            const mdCommand = exportCommand.subCommands?.find((c) => c.name === 'md');
            if (!mdCommand?.action) {
                throw new Error('md command not found');
            }
            ...
            expect(result).toEqual({
                type: 'message',
                messageType: 'error',
                content: 'Failed to export session: EIO: failed to read session (markdown)',
            });
        ...
        it('should handle session collection errors', async () => {
            ...
            expect(result).toEqual({
                type: 'message',
                messageType: 'error',
                content: 'Failed to export session: Failed to collect session data (markdown)',
            });
        ...
        it('should handle session normalization errors', async () => {
            ...
            expect(result).toEqual({
                type: 'message',
                messageType: 'error',
                content: 'Failed to export session: Failed to normalize session data (markdown)',
            });
            expect(toMarkdown).not.toHaveBeenCalled();
            expect(fs.writeFile).not.toHaveBeenCalled();
        });
        it('should use project root when working dir is not available', async () => {
            ...
            expect(fs.writeFile).toHaveBeenCalledWith(expect.stringContaining(`${path.sep}test${path.sep}project`), '# Test Markdown', { encoding: 'utf-8', mode: 0o600 });
        });
        ...
        it('should report default directory realpath validation failures', async () => {
            ...
            expect(result).toEqual({
                type: 'message',
                messageType: 'error',
                content: `Export target directory is not accessible (path does not exist; ` +
        ...
        it('should reject symlinked output directories outside the working directory', async () => {
            ...
            const outputDir = path.resolve(mockWorkingDir, './logs');
            vi.mocked(fs.realpath).mockImplementation(async (p) => p.toString() === outputDir
        ...
        it('should explain when no existing export ancestor can be resolved', async () => {
            ...
            expect(result).toEqual({
                type: 'message',
                messageType: 'error',
                content: `Failed to export session: Cannot resolve any existing ancestor ` +
        ...
        it('should allow output directories with names beginning with two dots', async () => {
            ...
            expect(result).toEqual({
                type: 'message',
                messageType: 'info',
                content: expect.stringContaining(path.join('./..backup', 'export-2025-01-01T00-00-00-000Z.md')),
            });
            expect(fs.mkdir).toHaveBeenCalledWith(outputDir, {
                recursive: true,
                mode: 0o700,
        ...
        it('should validate the nearest existing parent for fresh nested directories', async () => {
            ...
            expect(result).toEqual({
                type: 'message',
                messageType: 'info',
                content: expect.stringContaining(path.join('./nonexistent/logs', 'export-2025-01-01T00-00-00-000Z.md')),
            });
            expect(fs.mkdir).toHaveBeenCalledWith(outputDir, {
                recursive: true,
                mode: 0o700,
        ...
        it('should not ignore non-missing realpath errors while checking parents', async () => {
            ...
            expect(result).toEqual({
                type: 'message',
                messageType: 'error',
                content: 'Failed to export session: EACCES: permission denied (markdown)',
            });
            expect(fs.mkdir).not.toHaveBeenCalled();
            expect(fs.writeFile).not.toHaveBeenCalled();
        });
        it('should report custom directory creation failures with path context', async () => {
            ...
            expect(result).toEqual({
                type: 'message',
                messageType: 'error',
                content: `Failed to create export directory "${path.resolve(mockWorkingDir, './logs')}": EACCES: permission denied, mkdir`,
            });
    ...
    describe('exportHtmlAction', () => {
        it('should export session to HTML file', async () => {
            const htmlCommand = exportCommand.subCommands?.find((c) => c.name === 'html');
            ...
            expect(result).toEqual({
                type: 'message',
                messageType: 'info',
                content: expect.stringContaining('export-2025-01-01T00-00-00-000Z.html'),
            });
            expect(mockSessionServiceMocks.loadSession).toHaveBeenCalled();
            expect(collectSessionData).toHaveBeenCalledWith(mockSessionData.conversation, expect.anything());
            expect(normalizeSessionData).toHaveBeenCalled();
            ...
            expect(fs.writeFile).toHaveBeenCalledWith(expect.stringContaining('export-2025-01-01T00-00-00-000Z.html'), expect.stringContaining('{"data": "test"}'), { encoding: 'utf-8', mode: 0o600 });
        });
        it('should export default HTML to a relative custom directory', async () => {
            ...
            const outputDir = path.resolve(mockWorkingDir, './logs');
            const filepath = path.join(outputDir, 'export-2025-01-01T00-00-00-000Z.html');
            expect(result).toEqual({
                type: 'message',
                messageType: 'info',
                content: expect.stringContaining(path.join('./logs', 'export-2025-01-01T00-00-00-000Z.html')),
            });
            expect(fs.mkdir).toHaveBeenCalledWith(outputDir, {
                recursive: true,
                mode: 0o700,
            });
            expect(fs.writeFile).toHaveBeenCalledWith(filepath, expect.stringContaining('{"data": "test"}'), { encoding: 'utf-8', mode: 0o600 });
        });
        it('should return error when config is not available', async () => {
            const contextWithoutConfig = createMockCommandContext({
                services: {
    ...
    describe('exportJsonAction', () => {
        it('should export session to JSON file', async () => {
            const jsonCommand = exportCommand.subCommands?.find((c) => c.name === 'json');
            ...
            expect(result).toEqual({
                type: 'message',
                messageType: 'info',
                content: expect.stringContaining('export-2025-01-01T00-00-00-000Z.json'),
            });
            expect(mockSessionServiceMocks.loadSession).toHaveBeenCalled();
            expect(collectSessionData).toHaveBeenCalledWith(mockSessionData.conversation, expect.anything());
            expect(normalizeSessionData).toHaveBeenCalled();
            ...
            expect(fs.mkdir).not.toHaveBeenCalled();
            expect(fs.writeFile).toHaveBeenCalledWith(expect.stringContaining('export-2025-01-01T00-00-00-000Z.json'), '{"messages":[]}', { encoding: 'utf-8', mode: 0o600 });
        });
        it('should export JSON to a relative custom directory', async () => {
            ...
            const outputDir = path.resolve(mockWorkingDir, './logs');
            const filepath = path.join(outputDir, 'export-2025-01-01T00-00-00-000Z.json');
            expect(result).toEqual({
                type: 'message',
                messageType: 'info',
                content: expect.stringContaining(path.join('./logs', 'export-2025-01-01T00-00-00-000Z.json')),
            });
            expect(fs.mkdir).toHaveBeenCalledWith(outputDir, {
                recursive: true,
                mode: 0o700,
    ...
    describe('exportJsonlAction', () => {
        it('should export session to JSONL file', async () => {
            const jsonlCommand = exportCommand.subCommands?.find((c) => c.name === 'jsonl');
            ...
            expect(result).toEqual({
                type: 'message',
                messageType: 'info',
                content: expect.stringContaining('export-2025-01-01T00-00-00-000Z.jsonl'),
            });
            expect(mockSessionServiceMocks.loadSession).toHaveBeenCalled();
            expect(collectSessionData).toHaveBeenCalledWith(mockSessionData.conversation, expect.anything());
            expect(normalizeSessionData).toHaveBeenCalled();
            ...
            expect(fs.mkdir).not.toHaveBeenCalled();
            expect(fs.writeFile).toHaveBeenCalledWith(expect.stringContaining('export-2025-01-01T00-00-00-000Z.jsonl'), '{"type":"session_metadata"}', { encoding: 'utf-8', mode: 0o600 });
        });
        it('should export JSONL to a relative custom directory', async () => {
            ...
            const outputDir = path.resolve(mockWorkingDir, './logs');
            const filepath = path.join(outputDir, 'export-2025-01-01T00-00-00-000Z.jsonl');
            expect(result).toEqual({
                type: 'message',
                messageType: 'info',
                content: expect.stringContaining(path.join('./logs', 'export-2025-01-01T00-00-00-000Z.jsonl')),
            });
            expect(fs.mkdir).toHaveBeenCalledWith(outputDir, {
                recursive: true,
                mode: 0o700,
            });
            expect(fs.writeFile).toHaveBeenCalledWith(filepath, '{"type":"session_metadata"}', { encoding: 'utf-8', mode: 0o600 });
        });
