 ...
 */
import { FatalSandboxError, stripAnsiAndControl, } from '@lailatul-coder/lailatul-coder-core';
...
export function resetSandboxProbeCacheForTest() {
    probeCache.clear();
}
/**
...
function getSandboxCommand(sandbox) {
    ...
    if (typeof sandbox === 'string' && sandbox) {
        ...
        if (commandExists.sync(sandbox)) {
            ...
            if (failure) {
                throw new FatalSandboxError(`Sandbox command '${sandbox}'${sandboxSource} is installed but cannot run: ${failure}`);
            }
            return sandbox;
        }
        throw new FatalSandboxError(`Missing sandbox command '${sandbox}'${sandboxSource}`);
    }
    ...
    if (sandbox === true) {
        ...
        if (firstFailure) {
            throw new FatalSandboxError(`${enabledLabel} and '${firstFailure.command}' is installed but cannot run: ` +
                `${firstFailure.detail}; start it, try another installed runtime, or ${specifyHint}`);
        }
        throw new FatalSandboxError(`${enabledLabel} but failed to determine command for sandbox; ` +
