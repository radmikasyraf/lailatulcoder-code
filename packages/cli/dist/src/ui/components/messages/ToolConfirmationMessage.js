...
import { IdeClient, ToolConfirmationOutcome, buildHumanReadableRuleLabel, } from '@lailatul-coder/lailatul-coder-core';
...
export const ToolConfirmationMessage = ({ confirmationDetails, config, isFocused = true, availableTerminalHeight, contentWidth, compactMode = false, }) => {
    ...
    const openFullPlanInEditor = () => {
        ...
        const openPlan = async () => {
            const dir = await fs.mkdtemp(path.join(os.tmpdir(), 'LailatulCoder-plan-'));
