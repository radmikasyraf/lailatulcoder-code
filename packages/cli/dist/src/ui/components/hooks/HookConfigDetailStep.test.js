...
describe('HookConfigDetailStep', () => {
    ...
    const createMockHookConfig = (source = HooksConfigSource.User, sourceDisplay = 'User Settings', sourcePath) => ({
        config: {
    ...
    it('should render hook details title', () => {
        ...
        const { lastFrame } = render(_jsx(HookConfigDetailStep, { hookEvent: hookEvent, hookConfig: hookConfig }));
    ...
    it('should render source for Extensions with path', () => {
        const hookEvent = createMockHookEvent();
        const hookConfig = createMockHookConfig(HooksConfigSource.Extensions, 'ralph-wiggum', '/Users/test/.qwen/extensions/ralph-wiggum');
        const { lastFrame } = render(_jsx(HookConfigDetailStep, { hookEvent: hookEvent, hookConfig: hookConfig }));
    ...
    it('should render Extension field for extensions', () => {
        const hookEvent = createMockHookEvent();
        const hookConfig = createMockHookConfig(HooksConfigSource.Extensions, 'ralph-wiggum');
        const { lastFrame } = render(_jsx(HookConfigDetailStep, { hookEvent: hookEvent, hookConfig: hookConfig }));
    ...
    it('should not render Matcher field for non-matcher events', () => {
        ...
        const output = render(_jsx(HookConfigDetailStep, { hookEvent: hookEvent, hookConfig: hookConfig })).lastFrame() ?? '';
...
//# sourceMappingURL=HookConfigDetailStep.test.js.map