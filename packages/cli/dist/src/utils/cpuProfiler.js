 ...
 *
 * Output: ~/.lailatulcoder/cpu-profiles/lailatul-coder-cpu-<pid>-<timestamp>.cpuprofile
 ...
 */
export function initCpuProfiler() {
    if (initialized)
        return;
    initialized = true;
    ...
    registerCleanup(async () => {
        if (state === 'recording') {
            ...
            if (result.ok) {
                process.stderr.write(`[cpu-profiler] Profile written: ${result.filePath}\n`);
            }
        }
    });
...
export async function stopCpuProfile(options) {
    ...
    try {
        if (!session) {
            throw new Error('Inspector session lost unexpectedly during Profiler.stop; the profile data could not be retrieved.');
        }
        const result = (await session.post('Profiler.stop'));
        if (!result.profile) {
            throw new Error('V8 Profiler.stop returned an empty profile; recording may have been interrupted.');
        }
        ...
        const filePath = path.join(outputDir, `lailatul-coder-cpu-${process.pid}-${formatTimestamp(now)}.cpuprofile`);
        try {
 ...
 */
export function isCpuProfileRecording() {
...
export function _resetCpuProfilerForTest() {
    state = 'idle';
    initialized = false;
...
export function clearCpuProfileRateLimit() {
    lastWriteByDir.clear();
}
// ---------------------------------------------------------------------------
// Internal helpers
...
function defaultOutputDir() {
    return path.join(os.homedir(), '.lailatulcoder', 'cpu-profiles');
}
...
export function _setSessionFactoryForTest(factory) {
...
async function getOrCreateSession() {
    ...
    const newSession = new inspectorModule.Session();
    newSession.connect();
    session = newSession;
    return session;
}
...
function handleSigusr1() {
    if (state === 'idle') {
        void startCpuProfile().then((result) => {
            if (result.ok) {
                process.stderr.write(`[cpu-profiler] Recording started (PID ${process.pid}). Send SIGUSR1 again to stop.\n`);
            }
            else {
                process.stderr.write(`[cpu-profiler] Failed to start: ${result.error}\n`);
            }
        ...
        void stopCpuProfile().then((result) => {
            if (result.ok) {
                ...
                process.stderr.write(`[cpu-profiler] Failed to stop: ${result.error}\n`);
            }
        });
    }
    // state === 'stopping': ignore, already in progress
}
function enforceRateLimit(outputDir, now, rateLimitMs) {
    if (rateLimitMs <= 0)
        return;
    ...
    if (lastWriteMs !== undefined && nowMs - lastWriteMs < rateLimitMs) {
        ...
        throw new Error(`CPU profile rate limit: wait ${waitSeconds}s before writing another profile.`);
    }
}
function recordWrite(outputDir, now) {
...
function checkDiskSpace(outputDir) {
    try {
        ...
        if (available - ESTIMATED_PROFILE_BYTES < MIN_FREE_BYTES_AFTER_WRITE) {
            throw new Error('Insufficient free disk space for CPU profile; skipping to avoid filling the disk.');
        }
    }
    catch (error) {
        ...
        process.stderr.write('[cpu-profiler] Disk space check unavailable on this platform; skipping.\n');
    }
}
function cleanupOldProfiles(outputDir, maxProfiles) {
    if (maxProfiles < 1)
        return;
    let profiles;
    try {
        profiles = fs
            ...
            .filter((name) => name.startsWith('lailatul-coder-cpu-') && name.endsWith('.cpuprofile'))
            .map((name) => path.join(outputDir, name))
            .sort((a, b) => {
            try {
                return (fs.lstatSync(b).mtimeMs - fs.lstatSync(a).mtimeMs ||
...
//# sourceMappingURL=cpuProfiler.js.map