...
function filterAlwaysAllowOptions(confirmation, options, forceHideAlwaysAllow = false) {
    ...
    const rejectIndex = visibleOptions.findIndex((option) => option.kind === 'reject_once');
    if (rejectIndex === -1)
        return [...visibleOptions, switchOption];
    return [
...
export function interactionMetaFields(confirmation) {
    return confirmation.type === 'ask_user_question'
        ? {
...
export function buildPermissionRequestContent(confirmation) {
    ...
    const warnings = confirmation.type === 'exec' || confirmation.type === 'edit'
...
export function requestPermissionWithAbort(client, params, signal) {
    if (signal.aborted) {
    ...
    return new Promise((resolve, reject) => {
        ...
        request.then((response) => finish(() => resolve(response)), (error) => finish(() => reject(error)));
    });
}
export function resolvePermissionOutcome(response, offeredOptions) {
    if (response.outcome.outcome === 'cancelled') {
        return ToolConfirmationOutcome.Cancel;
    }
    const optionId = response.outcome.optionId;
    if (!offeredOptions.some((option) => option.optionId === optionId)) {
        throw new Error(`Permission response selected unoffered option: ${optionId}`);
    }
    if (!Object.values(ToolConfirmationOutcome).includes(optionId)) {
