...
import { OutputFormat, FatalInputError, ToolErrorType, } from '@lailatul-coder/lailatul-coder-core';
import { AlreadyReportedError, _resetExitLatchForTest, getErrorMessage, handleError, handleToolError, handleCancellationError, handleMaxTurnsExceededError, } from './errors.js';
...
describe('errors', () => {
    ...
    describe('getErrorMessage', () => {
        ...
        it('should stringify plain objects without message property', () => {
            ...
            expect(getErrorMessage(obj)).toBe('{"code":500,"details":"internal error"}');
        });
    ...
    describe('handleError', () => {
        describe('in text mode', () => {
            ...
            it('should log error message and re-throw', async () => {
                ...
                await expect(handleError(testError, mockConfig)).rejects.toThrow(testError);
                expect(mockWriteStderrLine).toHaveBeenCalledWith('API Error: Test error');
            });
            it('should handle non-Error objects', async () => {
                const testError = 'String error';
                await expect(handleError(testError, mockConfig)).rejects.toThrow(testError);
                expect(mockWriteStderrLine).toHaveBeenCalledWith('API Error: String error');
            });
            it('does not reformat or reprint AlreadyReportedError', async () => {
                ...
                // "[API Error: [API Error: ...]]").
                const reported = new AlreadyReportedError('[API Error: 402 Model X is not available for billing.]');
        ...
        describe('in JSON mode', () => {
            ...
            it('should format error as JSON and exit with default code', async () => {
                const testError = new Error('Test error');
                await expect(handleError(testError, mockConfig)).rejects.toThrow('process.exit called with code: 1');
                expect(mockWriteStderrLine).toHaveBeenCalledWith(JSON.stringify({
                    error: {
                        type: 'Error',
                        message: 'Test error',
                        code: 1,
            ...
            it('does not reformat or reprint AlreadyReportedError in JSON mode', async () => {
                const reported = new AlreadyReportedError('[API Error: 402 Model X is not available for billing.]', 42);
                await expect(handleError(reported, mockConfig)).rejects.toThrow('process.exit called with code: 42');
            ...
            it('should use custom error code when provided', async () => {
                const testError = new Error('Test error');
                await expect(handleError(testError, mockConfig, 42)).rejects.toThrow('process.exit called with code: 42');
                expect(mockWriteStderrLine).toHaveBeenCalledWith(JSON.stringify({
                    error: {
                        type: 'Error',
                        message: 'Test error',
            ...
            it('should extract exitCode from FatalError instances', async () => {
                ...
                await expect(handleError(fatalError, mockConfig)).rejects.toThrow('process.exit called with code: 42');
                expect(mockWriteStderrLine).toHaveBeenCalledWith(JSON.stringify({
                    error: {
            ...
            it('should handle error with code property', async () => {
                ...
                await expect(handleError(errorWithCode, mockConfig)).rejects.toThrow('process.exit called with code: 404');
            });
            it('should handle error with status property', async () => {
                ...
                await expect(handleError(errorWithStatus, mockConfig)).rejects.toThrow('process.exit called with code: 1');
                expect(mockWriteStderrLine).toHaveBeenCalledWith(JSON.stringify({
                    error: {
                        type: 'Error',
    ...
    describe('handleToolError', () => {
        ...
        describe('when debug mode is enabled', () => {
            ...
            describe('in text mode', () => {
                ...
                it('should log error message to stderr and not exit', () => {
                    ...
                    expect(debugLoggerSpy.error).toHaveBeenCalledWith('Error executing tool test-tool: Tool failed');
                ...
                it('should use resultDisplay when provided and not exit', () => {
                    ...
                    expect(debugLoggerSpy.error).toHaveBeenCalledWith('Error executing tool test-tool: Custom display message');
                    expect(processExitSpy).not.toHaveBeenCalled();
                });
            });
            describe('in JSON mode', () => {
                ...
                it('should prefer resultDisplay over error message and not exit', () => {
                    ...
                    // In JSON mode, should not exit (just log to stderr when debug mode is on)
                    expect(debugLoggerSpy.error).toHaveBeenCalledWith('Error executing tool test-tool: Display message');
                    expect(processExitSpy).not.toHaveBeenCalled();
                });
            });
        ...
        describe('permission denied warnings', () => {
            it('should show warning when EXECUTION_DENIED in non-interactive text mode', () => {
                ...
                mockConfig.getOutputFormat.mockReturnValue(OutputFormat.TEXT);
                handleToolError(toolName, toolError, mockConfig, ToolErrorType.EXECUTION_DENIED);
                expect(processStderrWriteSpy).toHaveBeenCalledWith(expect.stringContaining('Warning: Tool "test-tool" requires user approval'));
                expect(processStderrWriteSpy).toHaveBeenCalledWith(expect.stringContaining('use the -y flag (YOLO mode)'));
                expect(processExitSpy).not.toHaveBeenCalled();
            });
            ...
            it('should not show warning for non-EXECUTION_DENIED errors', () => {
                ...
                mockConfig.isInteractive.mockReturnValue(false);
                mockConfig.getOutputFormat.mockReturnValue(OutputFormat.TEXT);
                handleToolError(toolName, toolError, mockConfig, ToolErrorType.FILE_NOT_FOUND);
                expect(processStderrWriteSpy).not.toHaveBeenCalled();
                expect(processExitSpy).not.toHaveBeenCalled();
            });
        });
    ...
    describe('handleCancellationError', () => {
        describe('in text mode', () => {
            ...
            it('should log cancellation message and exit with 130', async () => {
                await expect(handleCancellationError(mockConfig)).rejects.toThrow('process.exit called with code: 130');
                expect(mockWriteStderrLine).toHaveBeenCalledWith('Operation cancelled.');
            });
        });
        describe('in JSON mode', () => {
            beforeEach(() => {
    ...
    describe('handleMaxTurnsExceededError', () => {
        describe('in text mode', () => {
            ...
            it('should log max turns message and exit with 53', async () => {
                await expect(handleMaxTurnsExceededError(mockConfig)).rejects.toThrow('process.exit called with code: 53');
                expect(mockWriteStderrLine).toHaveBeenCalledWith('Reached max session turns for this session. Increase the number of turns by specifying maxSessionTurns in settings.json.');
            });
        });
        describe('in JSON mode', () => {
            beforeEach(() => {
        ...
        describe('with --json-schema active', () => {
            ...
            it('appends a json-schema-specific hint in text mode', async () => {
                mockConfig.getOutputFormat.mockReturnValue(OutputFormat.TEXT);
                await expect(handleMaxTurnsExceededError(mockConfig)).rejects.toThrow('process.exit called with code: 53');
                const written = mockWriteStderrLine.mock.calls[0]?.[0];
    ...
    describe('cleanup-before-exit invariant', () => {
        ...
        it('handleError drains registered cleanups before exit (JSON mode)', async () => {
            ...
            await expect(handleError(new Error('boom'), mockConfig)).rejects.toThrow('process.exit called with code: 1');
