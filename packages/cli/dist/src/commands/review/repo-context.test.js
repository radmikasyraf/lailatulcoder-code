 ...
 */
import { chmodSync, linkSync, readdirSync, mkdirSync, mkdtempSync, readFileSync, realpathSync, rmSync, statSync, symlinkSync, utimesSync, writeFileSync, renameSync, } from 'node:fs';
import { tmpdir } from 'node:os';
import { dirname, join } from 'node:path';
import { execFileSync } from 'node:child_process';
import { afterEach, beforeEach, describe, expect, it, vi } from 'vitest';
import { MAX_IDENTITY_BYTES, } from './lib/repository-context.js';
import { commitPlanPreservingEpoch, repoContextCommand, runRepoContext, } from './repo-context.js';
import { stringifyPlanReport } from './lib/report.js';
import { isolateHostGitConfig } from './lib/test-utils.js';
import { appendRunSession, priorSessionIds, recordResume, } from './lib/run-ledger.js';
const tempRoots = [];
function temp() {
...
function planAt(root, plan) {
    const path = join(root, 'plan.json');
    write(path, `${JSON.stringify(plan)}\n`);
    return path;
}
function writeManifest(worktree, paths = ['src/**']) {
    write(join(worktree, '.qwen', 'review-context.json'), JSON.stringify({
        version: 1,
...
function run(root, worktree, plan, providers) {
...
describe('repo-context providers and trust boundary', () => {
    ...
    it('fails actionably when the worktree vanished mid-review (#9205)', () => {
        ...
        }, [])).toThrow('is missing — recreate the review worktree ' +
    ...
    it.skipIf(process.platform === 'win32')('rethrows ENOTDIR when a regular file is an intermediate path component', () => {
    ...
    it('uses the trusted base manifest for pull-request opt in and opt out', () => {
        ...
        commitAll(worktree);
        expect(readJson(run(join(root, 'base-enabled'), worktree, {
            files: [{ path: 'src/change.ts' }],
            mergeBaseSha: base,
        }).outPath)).toEqual(manifestContext());
        ...
        commitAll(second);
        expect(readJson(run(join(root, 'base-disabled'), second, {
            files: [{ path: 'src/change.ts' }],
    ...
    it('does not let the current tree opt in or opt out of base identity', () => {
        ...
        expect(readJson(run(join(root, 'base-enabled'), worktree, { files: [{ path: 'src/change.ts' }], mergeBaseSha: base }, [enabled]).outPath)).toEqual(context());
        const second = join(root, 'second');
        initGit(second);
    ...
    it.skipIf(process.platform === 'win32')('follows a committed symlink identity like the worktree reader', () => {
        const root = temp();
        const worktree = join(root, 'repository');
        initGit(worktree);
        write(join(worktree, '.qwen', 'manifest.json'), JSON.stringify({
            version: 1,
            label: 'Manifest project',
            rules: [{ paths: ['src/**'], domains: ['runtime'] }],
        }));
        symlinkSync('manifest.json', join(worktree, '.qwen', 'review-context.json'));
        write(join(worktree, 'src', 'change.ts'), 'base\n');
        const base = commitAll(worktree);
        expect(readJson(run(join(root, 'pr'), worktree, {
            files: [{ path: 'src/change.ts' }],
            mergeBaseSha: base,
        }).outPath)).toEqual(manifestContext());
        expect(readJson(run(join(root, 'local'), worktree, {
            files: [{ path: 'src/change.ts' }],
        }).outPath)).toEqual(manifestContext());
    });
    it.skipIf(process.platform === 'win32')('fails closed when a committed identity symlink escapes the tree', () => {
        const root = temp();
        const worktree = join(root, 'repository');
        initGit(worktree);
        write(join(worktree, 'outside.json'), '{}');
        mkdirSync(join(worktree, '.qwen'), { recursive: true });
        symlinkSync('../../outside.json', join(worktree, '.qwen', 'review-context.json'));
        write(join(worktree, 'src', 'change.ts'), 'base\n');
        const base = commitAll(worktree);
        expect(() => run(join(root, 'escape'), worktree, {
            files: [{ path: 'src/change.ts' }],
            mergeBaseSha: base,
        })).toThrow('escapes the worktree');
    });
    it.skipIf(process.platform === 'win32')('fails closed when the base identity symlink chain exceeds the hop ceiling', () => {
        ...
        initGit(worktree);
        mkdirSync(join(worktree, '.qwen'), { recursive: true });
        symlinkSync('loop-b.json', join(worktree, '.qwen', 'review-context.json'));
        symlinkSync('review-context.json', join(worktree, '.qwen', 'loop-b.json'));
        write(join(worktree, 'src', 'change.ts'), 'base\n');
        const base = commitAll(worktree);
        expect(() => run(join(root, 'cycle'), worktree, {
            files: [{ path: 'src/change.ts' }],
            mergeBaseSha: base,
    ...
    it('fails closed for an invalid or unresolvable base', () => {
        ...
        write(join(worktree, 'src', 'change.ts'), 'base\n');
        commitAll(worktree);
        const provider = { provide: () => context() };
        expect(() => run(join(root, 'invalid'), worktree, { files: [{ path: 'src/change.ts' }], mergeBaseSha: 'nope' }, [provider])).toThrow('mergeBaseSha is invalid');
        expect(() => run(join(root, 'stale'), worktree, {
            files: [{ path: 'src/change.ts' }],
            mergeBaseSha: '0'.repeat(40),
        }, [provider])).toThrow('cannot be resolved');
    });
    it('writes null without consulting the worktree when the base never resolved', () => {
        ...
        commitAll(worktree);
        const provide = vi.fn(() => context());
        const first = run(join(root, 'null-base'), worktree, { files: [{ path: 'src/change.ts' }], mergeBaseSha: null }, [{ provide }]);
        expect(provide).not.toHaveBeenCalled();
        ...
        const second = run(join(root, 'null-base-fetch-failed'), worktree, {
            files: [{ path: 'src/change.ts' }],
            mergeBaseSha: null,
            baseFetchFailed: true,
        }, [{ provide }]);
    ...
    it('normalizes identity content identically in local and pull-request modes', () => {
        ...
        const localProvide = vi.fn((input) => {
        ...
        const prProvide = vi.fn((input) => {
            expect(input.readIdentityFile('.review/identity')).toBe('token\ntail');
            return context();
        });
        run(root, repository, { files: [{ path: 'src/change.ts' }], mergeBaseSha: base }, [{ provide: prProvide }]);
    ...
    it.skipIf(isRoot)('fails closed when a present local identity file cannot be read', () => {
        const root = temp();
        const worktree = join(root, 'worktree');
        ...
        try {
            expect(() => run(root, worktree, { files: [{ path: 'src/change.ts' }] }, [
                {
                    provide(input) {
    ...
    it.skipIf(isRoot)('keeps the artifact when the plan write fails', () => {
        ...
        try {
            expect(() => runRepoContext({ plan: planPath, worktree, out: outPath }, [
                { provide: () => context() },
            ])).toThrow();
            expect(readJson(outPath)).toEqual(context());
    ...
    it('supports recorded linked-worktree paths', () => {
        ...
        const { planPath, outPath } = run(root, linked, {
            files: [{ path: 'src/change.ts' }],
            worktreePath: '../linked',
        }, [{ provide }]);
        expect(provide).toHaveBeenCalledOnce();
    ...
    it.skipIf(process.platform === 'win32')('reads a broken trailing-slash identity symlink as absent in both modes', () => {
    ...
    it('rejects identity traversal and symlink escapes', () => {
        ...
        for (const identityPath of ['../outside', '/outside', 'identity']) {
            expect(() => run(join(root, identityPath.replaceAll('/', '_')), worktree, { files: [{ path: 'src/change.ts' }] }, [
                {
                    provide(input) {
    ...
    it('skips unsafe changed paths instead of aborting the step', () => {
        ...
        run(root, worktree, { files: [{ path: '../secret' }, { path: 'src/ok.ts' }] }, [{ provide }]);
        expect(provide).toHaveBeenCalledOnce();
    });
    it('still rejects a corrupted plan whose file paths are not strings', () => {
        ...
        const worktree = join(root, 'worktree');
        mkdirSync(worktree);
        expect(() => run(root, worktree, { files: [{ path: 42 }] })).toThrow('plan.files[0].path is invalid');
    });
    it('rejects plan/out aliases and preserves the plan on artifact failure', () => {
        ...
        expect(() => runRepoContext({ plan: planPath, worktree, out: planPath }, [
            { provide: () => context() },
        ])).toThrow('--out must differ');
        const alias = join(root, 'alias.json');
        linkSync(planPath, alias);
        expect(() => runRepoContext({ plan: planPath, worktree, out: alias }, [
            { provide: () => context() },
        ])).toThrow('--out must differ');
        const before = readFileSync(planPath, 'utf8');
        const outDirectory = join(root, 'out-directory');
        mkdirSync(outDirectory);
        expect(() => runRepoContext({ plan: planPath, worktree, out: outDirectory }, [
            { provide: () => context() },
        ])).toThrow();
        expect(readFileSync(planPath, 'utf8')).toBe(before);
    });
    it('rejects an --out whose canonical identity equals an absent --plan', () => {
        ...
        }, [{ provide: () => context() }])).toThrow('--out must differ');
    });
    ...
    it('declares all required command options and uses the default manifest provider', () => {
        ...
        const built = repoContextCommand.builder({
            option,
        });
        ...
        expect(option.mock.calls.map(([name, config]) => [
            name,
            config.demandOption,
        ])).toEqual([
            ['plan', true],
    ...
    it('preserves plan fields the command does not know across the rewrite', () => {
        ...
        const worktree = join(root, 'worktree');
        mkdirSync(worktree);
        const { planPath } = run(root, worktree, {
            files: [{ path: 'src/change.ts' }],
            effort: 'high',
            customField: 'kept',
        }, [{ provide: () => context() }]);
    ...
    it('rejects a corrupted plan whose files field is not an array', () => {
        ...
        const worktree = join(root, 'worktree');
        mkdirSync(worktree);
        expect(() => run(root, worktree, { files: 'oops' })).toThrow('plan.files must be an array');
    });
    ...
    it('fails closed when the local identity exceeds the size limit', () => {
        ...
        const root = temp();
        const worktree = join(root, 'worktree');
        write(join(worktree, '.qwen', 'review-context.json'), `"${'x'.repeat(MAX_IDENTITY_BYTES)}"`);
        expect(() => run(root, worktree, { files: [{ path: 'src/change.ts' }] })).toThrow('exceeds the size limit');
    });
    // Windows symlink creation needs elevated privileges.
    it.skipIf(process.platform === 'win32')('canonicalizes a symlinked --worktree argument', () => {
    ...
    it('attaches context from a SHA-256 repository', () => {
        ...
        expect(readJson(run(join(root, 'sha256'), worktree, {
            files: [{ path: 'src/change.ts' }],
            mergeBaseSha: base,
        }).outPath)).toEqual(manifestContext());
    });
    it.skipIf(process.platform === 'win32')('reads a `.`-targeted identity symlink as absent in both modes', () => {
...
describe('the plan mtime is the run epoch — enrichment must not advance it', () => {
    ...
    it('preserves the plan mtime across the enrichment rewrite', () => {
        ...
        try {
            ...
            // the one below.
            expect(Math.abs(statSync(planPath).mtimeMs - mtimeBefore)).toBeLessThan(1);
            ...
            expect(JSON.parse(readFileSync(planPath, 'utf8'))['repositoryContext']).toBeUndefined();
    ...
    it('skips the write entirely when the enrichment changes nothing', () => {
        ...
        try {
            ...
            writeFileSync(planPath, stringifyPlanReport(planObj));
            const before = new Date(Date.now() - 3600_000);
            utimesSync(planPath, before, before);
            ...
            runRepoContext({ plan: planPath, worktree, out: join(root, 'ctx.json') }, [same]);
    ...
    it('aborts on a rename-replacement even with the mtime restored', () => {
        ...
        try {
            ...
            expect(() => runRepoContext({ plan: planPath, worktree, out: join(root, 'ctx.json') }, [racer])).toThrow(/changed while repository context was being computed/);
        }
        finally {
            rmSync(root, { recursive: true, force: true });
        }
    ...
    it('preserves a SUB-MILLISECOND plan mtime, and stays silent about it', () => {
        ...
        try {
            ...
            expect(priorSessionIds(planPath, { QWEN_CODE_SESSION_ID: 'S1' })).toEqual(['S0']);
...
describe('commitPlanPreservingEpoch — the epoch never advances, even torn', () => {
    ...
    it('the committed file carries the anchor epoch, atomically', () => {
        ...
        expect(Math.abs(statSync(plan).mtimeMs - anchor.mtimeMs)).toBeLessThanOrEqual(1);
    ...
    it('refuses to overwrite a capture that landed after the anchor', () => {
        ...
        expect(() => commitPlanPreservingEpoch(plan, '{"stale":true}', anchor)).toThrow(/changed while repository context was being committed/);
...
//# sourceMappingURL=repo-context.test.js.map