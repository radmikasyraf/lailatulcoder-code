...
import { formatTokenUsageSummaryAsCsv, formatTokenUsageSummaryAsJson, isSubpath, queryTokenUsage, } from '@lailatul-coder/lailatul-coder-core';
const VALID_EXPORT_FORMATS = new Set(['csv', 'json']);
...
function formatGenerationMetrics(metrics) {
    ...
    const lastTps = last.generationDurationMs > 0
        ? last.outputTokens / (last.generationDurationMs / 1000)
        : undefined;
    const averageTtft = metrics.timedRequests > 0
        ? metrics.totalTtftMs / metrics.timedRequests
        : undefined;
    const sessionTps = metrics.totalGenerationDurationMs > 0
    ...
    return [
        ...
        `${t('Average TTFT')}: ${averageTtft === undefined ? '—' : formatDuration(averageTtft)}`,
        `${t('Session TPS')}: ${sessionTps === undefined ? '—' : `${sessionTps.toFixed(1)} tok/s`}`,
    ];
}
function formatInteger(value) {
    return new Intl.NumberFormat(getCurrentLanguage()).format(value);
}
function formatGroupLines(titleKey, groups) {
    if (groups.length === 0) {
    ...
    return [
        t(titleKey),
        ...groups.map((group) => {
            const label = group.model && group.authType
...
function formatTokenUsageSummary(summary) {
    const label = summary.period === 'day'
...
function asMessage(content, messageType = 'info') {
    return {
        type: 'message',
        messageType,
        content,
...
function addInteractiveMessage(context, content, type = MessageType.INFO) {
    context.ui.addItem({
        type,
        text: content,
    }, Date.now());
}
async function showTokenUsageStats(context, period, args) {
    try {
...
function statsExportPathError() {
    return asMessage(t('Token usage export path must be within the project working directory.'), 'error');
}
function statsExportTargetMissingError(targetPath) {
    return asMessage(t('Export target does not exist: {{path}}', { path: targetPath }), 'error');
}
async function realpathNearestExisting(targetDirectory, cwd) {
    ...
    throw new Error(t('Cannot resolve export path within the working directory.'));
}
async function validateStatsExportExistingParent(cwd, outputDirectory) {
...
async function validateStatsExportDirectory(cwd, outputDirectory) {
...
function validateStatsExportPathShape(targetPath) {
...
async function validateStatsExportFileTarget(targetPath) {
    try {
...
async function validateStatsExportFinalFile(cwd, targetPath) {
    let stats;
    try {
...
function createTemporaryExportPath(outputDirectory, targetPath) {
    return path.join(outputDirectory, `.${path.basename(targetPath)}.${process.pid}.${Date.now()}.${randomUUID()}.tmp`);
}
function isSamePath(leftPath, rightPath) {
    return isSubpath(leftPath, rightPath) && isSubpath(rightPath, leftPath);
}
async function validateStatsExportDirectoryIdentity(cwd, outputDirectory, expectedRealOutputDirectory) {
    const [realCwd, realOutputDirectory] = await Promise.all([
        fs.realpath(cwd),
        fs.realpath(outputDirectory),
    ]);
...
async function writeStatsExportFileAtomically(cwd, outputDirectory, targetPath, content) {
    const initialDirectoryError = await validateStatsExportDirectory(cwd, outputDirectory);
    ...
    const realTargetPath = path.join(realOutputDirectory, path.basename(targetPath));
    if (!isSubpath(realCwd, realTargetPath)) {
        return statsExportPathError();
    }
    for (let attempt = 0; attempt < 10; attempt++) {
        let tempPath = createTemporaryExportPath(realOutputDirectory, targetPath);
        try {
            if (!isSubpath(realCwd, tempPath)) {
                return statsExportPathError();
            }
            const preTempDirectoryError = await validateStatsExportDirectoryIdentity(cwd, outputDirectory, realOutputDirectory);
            if (preTempDirectoryError) {
                return preTempDirectoryError;
            }
            const preTempFileError = await validateStatsExportFileTarget(realTargetPath);
            if (preTempFileError) {
            ...
            const preRenameDirectoryError = await validateStatsExportDirectoryIdentity(cwd, outputDirectory, realOutputDirectory);
            if (preRenameDirectoryError) {
                return preRenameDirectoryError;
            }
            const preRenameFileError = await validateStatsExportFileTarget(realTargetPath);
...
function parseStatsExportArgs(args) {
    ...
    for (let index = 0; index < tokens.length; index++) {
        ...
        if (token === '--format' || token === '-f') {
            ...
            if (!next || !VALID_EXPORT_FORMATS.has(next)) {
        ...
        throw new Error(t('Unexpected argument: {{argument}}', { argument: token }));
    }
    if (!period) {
        throw new Error(t('Usage: /stats export <daily|monthly> [YYYY-MM-DD|YYYY-MM] [--format csv|json] [--output path]'));
    }
    return {
        period,
        value,
...
function getConfigCwd(context) {
    const config = context.services.config;
    return config?.getWorkingDir() || config?.getProjectRoot() || process.cwd();
}
async function writeStatsExport(context, args) {
    try {
        ...
        const defaultFilename = `LailatulCoder-token-usage-${summary.period}-${summary.value}.${parsed.format}`;
        ...
        const parentError = await validateStatsExportExistingParent(cwd, outputDirectory);
        if (parentError) {
            return parentError;
        }
        await fs.mkdir(outputDirectory, { recursive: true });
        const postMkdirError = await validateStatsExportDirectory(cwd, outputDirectory);
        if (postMkdirError) {
        ...
        const writeError = await writeStatsExportFileAtomically(cwd, outputDirectory, targetPath, content);
        if (writeError) {
            return writeError;
        }
        return asMessage(t('Token usage exported to {{format}}: {{path}}', {
            format: parsed.format.toUpperCase(),
            path: path.relative(cwd, targetPath),
        }));
    }
    catch (error) {
        return asMessage(t('Failed to export token usage stats: {{error}}', {
            error: error instanceof Error ? error.message : String(error),
        }), 'error');
    }
}
export const statsCommand = {
    ...
    subCommands: [
            ...
            action: (context) => {
                if (context.executionMode !== 'interactive') {
                    ...
                    for (const [modelName, modelMetrics] of Object.entries(metrics.models)) {
                        lines.push(`${modelName}: ${t('prompt')}=${modelMetrics.tokens.prompt}, ${t('output')}=${modelMetrics.tokens.candidates}, ${t('cached')}=${modelMetrics.tokens.cached}`);
                        ...
                        if (cost != null) {
                            lines.push(`  ${t('Estimated cost: ${{cost}}', { cost: cost.toFixed(4) })}`);
                        }
                    }
                    if (lines.length === 0) {
        ...
        },
        {
            ...
            action: (context) => {
                if (context.executionMode !== 'interactive') {
                    ...
                    const content = toolNames.length > 0
                        ? [
                            t('Tool calls: {{total}} ({{success}} ok, {{fail}} fail)', {
        ...
        },
        {
            ...
            kind: CommandKind.BUILT_IN,
            supportedModes: ['interactive', 'non_interactive', 'acp'],
            action: (context, args) => showTokenUsageStats(context, 'day', args),
        },
        {
            name: 'monthly',
            altNames: ['month'],
            ...
            kind: CommandKind.BUILT_IN,
            supportedModes: ['interactive', 'non_interactive', 'acp'],
            action: (context, args) => showTokenUsageStats(context, 'month', args),
        },
        {
            name: 'export',
            get description() {
            ...
            argumentHint: '<daily|monthly> [date|month] [--format csv|json] [--output path]',
            kind: CommandKind.BUILT_IN,
            supportedModes: ['interactive', 'non_interactive', 'acp'],
            action: async (context, args) => {
                ...
                addInteractiveMessage(context, result.content, result.messageType === 'error' ? MessageType.ERROR : MessageType.INFO);
            },
        },
    ],
};
