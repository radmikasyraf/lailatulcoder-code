...
export const QWEN_DIR = '.lailatulcoder';
 ...
 */
const isDirectoryCache = new Map();
 ...
 */
export function _resetValidatePathCacheForTest() {
 ...
 */
export function tildeifyPath(filePath, homeOverride) {
...
function expandTilde(p) {
    ...
    if (p.startsWith('~\\')) {
        ...
        const expandedPath = path.join(os.homedir(), ...rest.split(/[/\\]+/).filter(Boolean));
...
export function expandHomeDir(p) {
    ...
    if (lowerPath.startsWith(`${userProfilePrefix}/`) ||
        ...
        const hasTrailingSep = rest.endsWith('/') || rest.endsWith('\\');
        const expandedPath = path.join(os.homedir(), ...rest.split(/[/\\]+/).filter(Boolean));
        return path.normalize(hasTrailingSep ? expandedPath + path.sep : expandedPath);
    }
    if (lowerPath.startsWith(userProfilePrefix)) {
 ...
 */
export function shortenPath(filePath, maxLen = 80) {
 ...
 */
export function formatDisplayPath(filePath, rootDirectory, maxLen = 80) {
 ...
 */
export function unescapeShellSpecials(value) {
...
export function isSubpaths(parentPath, childPath) {
 ...
 */
export function realpathNearestExisting(inputPath) {
 ...
 */
export async function realpathNearestExistingAsync(inputPath) {
 ...
 */
export function resolvePath(baseDir = process.cwd(), relativePath) {
 ...
 */
export function validatePath(config, resolvedPath, options = {}) {
    ...
    if (isDirectory === undefined) {
        ...
        if (isDirectoryCache.size >= VALIDATE_PATH_CACHE_MAX) {
            ...
            if (oldest !== undefined)
                isDirectoryCache.delete(oldest);
        }
 ...
 */
export function resolveAndValidatePath(config, relativePath, options = {}) {
