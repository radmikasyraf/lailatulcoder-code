...
import { acquireInheritedLoaderEnvScrub, clearLoaderKeyRejectionReporterIfCurrent, scrubInheritedLoaderEnv, setLoaderKeyRejectionReporter, } from '../config/shared-env-keys.js';
import { DEFAULT_COMPACTED_REPLAY_MAX_BYTES, DEFAULT_MAX_JOURNAL_BYTES, DEFAULT_MAX_JOURNAL_EVENTS, JOURNAL_GROWTH_HARD_CAP_BYTES, normalizeCompactedReplayMaxBytes, normalizeMaxJournalBytes, normalizeMaxJournalEvents, } from '@lailatul-coder/acp-bridge/replayWindowLimits';
...
import { consumeServeFastPathRejectedLoaderKeys, loadServeFastPathSettings, preResolveServeFastPathHomeEnvOverrides, } from './fast-path-settings.js';
import { MAX_REGISTERED_WORKSPACES, resolveWorkspaceInputs, } from './workspace-inputs.js';
import { formatMemoryBudgetStderr, resolveDaemonMemoryBudget, serveJournalGrowthPoolMb, } from '@lailatul-coder/acp-bridge/daemonMemoryBudget';
import { createChildHeapPolicy, } from '@lailatul-coder/acp-bridge/childHeapPolicy';
import { canonicalizeWorkspace, translateAndCheckAbsoluteWorkspacePath, } from '@lailatul-coder/acp-bridge/workspacePaths';
...
// the run-LailatulCoder-serve chunk. The launcher is only needed after listen().
...
import { allowOriginCors, bearerAuth, denyBrowserOriginCors, hostAllowlist, parseAllowOriginPatterns, } from './auth.js';
import { createPermissionAuditPublisher, PermissionAuditRing, } from './permission-audit.js';
import { ClientMcpSenderRegistry } from './acp-http/client-mcp-sender-registry.js';
import { initDaemonLogger, resolveDaemonLogBaseDir, } from './daemon-logger.js';
import { getAdvertisedServeFeatures, getServeProtocolVersions, SERVE_CAPABILITY_REGISTRY, } from './capabilities.js';
import { EXTERNAL_TOOL_GUARD_PROVIDER_ATTACHED_VALUE, EXTERNAL_TOOL_GUARD_REQUIRED_VALUE, EXTERNAL_TOOL_GUARD_TOKEN_ENV, PRIVATE_EXTERNAL_TOOL_GUARD_ENV, PRIVATE_EXTERNAL_TOOL_GUARD_PROVIDER_ENV, } from '@lailatul-coder/acp-bridge/externalToolGuard';
import { CAPABILITIES_SCHEMA_VERSION, } from './types.js';
import { isManagedScratchChild, prepareManagedScratchRoot, } from './managed-scratch-workspace.js';
...
import { allowOriginMode, listenerMaxConnections, parseDaemonStatusDetail, positiveFiniteOrNull, toDaemonStatusMemoryLimits, } from './daemon-status.js';
...
import { ChannelDeliveryError, isChannelDeliveryError, } from '../runtime/channel-delivery-ipc.js';
import { ChannelDeliveryAuthorizationStore } from './channel-delivery-authorization.js';
import { normalizeWorkerDiagnostic, sanitizeWorkerDiagnostic, } from './channel-worker-diagnostics.js';
import { channelSelectionNames } from './channel-selection.js';
import { resolveChannelWorkspaceGroups, } from './channel-workspace-grouping.js';
import {} from './channel-worker-group.js';
...
import { ACCESS_LOG_CONTROLLER_LOCAL, } from './server/access-log.js';
import { setDeferredRuntimeRequestTiming, } from './server/request-helpers.js';
...
const QWEN_SERVE_WRITER_IDLE_TIMEOUT_MS_ENV = 'QWEN_SERVE_WRITER_IDLE_TIMEOUT_MS';
const SHUTDOWN_FORCE_CLOSE_MS = 5_000;
const DAEMON_LOG_FORCED_FLUSH_BUDGET_MS = 250;
const DEFAULT_LIVE_DISCOVERY_RETRY_MS = 5_000;
function channelDeliveryPublicError(code) {
    switch (code) {
...
export function createBoundChannelDeliveryHandler(boundWorkspace, getManager, authorizations, daemonLog, diagnosticRedaction = {
    workerEnv: {},
}) {
    return async (info) => {
        const failed = (code, error, diagnostic) => {
            writeDaemonLifecycleBestEffort(() => {
                if (!daemonLog)
                    return;
                let diagnosticText;
                if (diagnostic !== undefined) {
                    const message = diagnostic instanceof Error
                        ? diagnostic.message
                        : String(diagnostic);
                    const sanitized = sanitizeWorkerDiagnostic(message, 512, diagnosticRedaction);
                    const targetId = normalizeWorkerDiagnostic(info.target.id);
                    diagnosticText = sanitizeLogText(targetId.length > 0
        ...
        if (!authorizations.consume(boundWorkspace, info)) {
            return failed('channel_delivery_invalid', 'Channel delivery is not authorized.');
        }
        ...
        if (!manager) {
            return failed('channel_worker_unavailable', 'Channel worker is not running.');
        }
        try {
...
async function flushDaemonLogBounded(daemonLog, budgetMs) {
    let timer;
    try {
        await Promise.race([
...
function writeDaemonLifecycleBestEffort(write) {
    try {
        write();
    }
    catch {
...
function daemonPipeDirection(direction) {
    switch (direction) {
        case 'sent':
            return 'outbound';
        case 'received':
...
const WORKSPACE_SETTING_SCOPE = 'Workspace';
function isPositiveIntegerMs(value) {
...
function deriveDefaultMaxTotalSessions(maxSessionsPerWorkspace, workspaceCount) {
    if (workspaceCount <= 1)
        return undefined;
...
function assertTimerDelayInRange(name, value) {
    if (value > MAX_TIMEOUT_MS) {
        throw new TypeError(`Invalid ${name}: ${value}. Exceeds maximum JS timer delay of ` +
 ...
 */
function parseDeadlineEnv(envName, raw) {
    if (raw === undefined)
        return undefined;
    ...
    if (!isPositiveIntegerMs(parsed)) {
        throw new Error(`Invalid ${envName}="${raw}": must be a positive integer (milliseconds).`);
    }
    return parsed;
}
...
function hasChromeExtensionOrigin(origins) {
    return (origins?.some((origin) => origin.trim().toLowerCase().startsWith('chrome-extension://')) === true);
}
function createDaemonTelemetryRuntimeConfig(telemetry, cliVersion, daemonSessionId, defaults) {
    return {
        ...
        getTelemetryOtlpEndpoint: () => telemetry.otlpEndpoint ?? defaults.otlpEndpoint,
        ...
        getTelemetryIncludeSensitiveSpanAttributes: () => telemetry.includeSensitiveSpanAttributes ?? false,
        getTelemetryResourceAttributes: () => ({
            'service.instance.id': daemonSessionId,
            ...(telemetry.resourceAttributes ?? {}),
        }),
        getTelemetryMetricsIncludeSessionId: () => telemetry.metrics?.includeSessionId ?? false,
        getTelemetryResourceAttributeWarnings: () => telemetry.resourceAttributeWarnings ?? [],
 ...
 */
export function validatePolicyConfig(policyConfig = {}, onWarning = writeStderrLine) {
    ...
    const validSet = new Set(SERVE_CAPABILITY_REGISTRY.permission_mediation.modes);
    const permissionStrategy = policyConfig.permissionStrategy;
    const consensusQuorum = policyConfig.consensusQuorum;
    if (permissionStrategy !== undefined &&
        (typeof permissionStrategy !== 'string' ||
            !validSet.has(permissionStrategy))) {
        throw new InvalidPolicyConfigError(`LailatulCoder serve: invalid policy.permissionStrategy ` +
            `"${String(permissionStrategy)}"; must be one of ` +
            `${Array.from(validSet).join(', ')}`);
    }
    if (consensusQuorum !== undefined &&
        (typeof consensusQuorum !== 'number' ||
            !Number.isInteger(consensusQuorum) ||
            consensusQuorum < 1)) {
        throw new InvalidPolicyConfigError(`LailatulCoder serve: invalid policy.consensusQuorum ` +
    ...
    const consensusQuorumActive = consensusQuorum !== undefined && permissionStrategy === 'consensus';
    if (consensusQuorum !== undefined && permissionStrategy !== 'consensus') {
        onWarning('LailatulCoder serve: policy.consensusQuorum is set but ' +
            'policy.permissionStrategy is not "consensus"; the override will ' +
            'be ignored.');
    }
    return {
        permissionPolicy: permissionStrategy,
...
function workspaceRuntimeEffectiveEnv(runtime, daemonEnv) {
    if (runtime.env.mode === 'runtime-overlay') {
        return runtime.env.effectiveEnv ?? {};
    }
    return runtime.env.effectiveEnv ?? daemonEnv;
}
export function formatChannelWorkerDaemonUrl(host, port) {
 ...
 */
export function extractContextFilename(value) {
 ...
 */
export function subSessionConcurrencyCapsFromSettings(serve, onWarning = writeStderrLine) {
    const asCap = (key, value) => {
        if (value === undefined)
            return undefined;
        if (typeof value === 'number' && Number.isInteger(value) && value >= 1) {
            return value;
        }
        onWarning(`LailatulCoder serve: ignoring invalid ${key} (${JSON.stringify(value)}); ` +
            `expected a positive integer, falling back to the built-in default.`);
        return undefined;
    };
    const maxConcurrentPerCaller = asCap('maxConcurrentSubSessionsPerCaller', serve.maxConcurrentSubSessionsPerCaller);
    const maxConcurrentTotal = asCap('maxConcurrentSubSessionsTotal', serve.maxConcurrentSubSessionsTotal);
    return {
...
function hasRetryableChannelWorkerShutdownError(error) {
    if (error instanceof AggregateError) {
        return error.errors.some(hasRetryableChannelWorkerShutdownError);
    }
    return (error instanceof Error && retryableChannelWorkerShutdownErrors.has(error));
}
let channelWorkerRuntimePromise;
async function loadChannelWorkerRuntime() {
    channelWorkerRuntimePromise ??= Promise.all([
        ...
        .then(([supervisor, pidfile, channelRuntime, cliEntryPath, workerGroup, workerManager,]) => ({
...
export function createDisabledChannelWorkerSupervisor() {
    const snapshot = {
        enabled: false,
        state: 'disabled',
        channels: [],
...
function channelServicePidfileConflictError(info) {
    const owner = info.owner === 'serve' ? 'LailatulCoder serve' : 'LailatulCoder channel start';
...
function channelServiceStartingConflictError() {
    return Object.assign(new Error('Channel service is already starting. Retry after the current startup finishes.'), { code: 'channel_service_conflict' });
}
function normalizeInstallModelIds(req, provider, getDefaultModelIds) {
    const fromRequest = req.modelIds
        ?.map((id) => id.trim())
        .filter((id) => id.length > 0);
    const modelIds = fromRequest && fromRequest.length > 0
        ? fromRequest
        : getDefaultModelIds(provider);
    return [...new Set(modelIds)];
}
function buildProviderSetupInputs(req, provider, helpers) {
    const protocol = (req.protocol ?? provider.protocol);
    const baseUrl = helpers.resolveBaseUrl(provider, req.baseUrl);
    return {
        ...(provider.protocolOptions ? { protocol } : {}),
        baseUrl,
        apiKey: req.apiKey.trim(),
        modelIds: normalizeInstallModelIds(req, provider, helpers.getDefaultModelIds),
        ...(req.advancedConfig ? { advancedConfig: req.advancedConfig } : {}),
    };
}
function shouldPreheatBridge(deps) {
    if (deps.preheatBridge !== undefined)
        return deps.preheatBridge;
    return process.env['VITEST_WORKER_ID'] === undefined;
}
let coreRuntimePromise;
function loadCoreRuntime() {
    coreRuntimePromise ??= import('./core-runtime.js');
    return coreRuntimePromise;
}
let liveDiscoveryRuntimePromise;
function loadLiveDiscoveryRuntime() {
...
async function resolveDaemonLogBaseDirForRun(input) {
    ...
    if (input.bootSettings?.advanced?.runtimeOutputDir !== undefined) {
        return resolveDaemonLogBaseDir(input.bootSettings.advanced.runtimeOutputDir, input.boundWorkspace);
    }
    if (input.deps.bootSettings !== undefined) {
...
async function loadServeRuntimeModules() {
    const [serverModule, bridgeModule, spawnChannelModule, processRegistryModule, workspaceModule, workspaceTypesModule, daemonStatusProviderModule, workspaceProvidersStatusModule, workspaceSkillsStatusModule, totalSessionAdmissionModule, workspaceRegistryModule, promptLedgerModule,] = await Promise.all([
        import('./server.js'),
    ...
    return {
        ...
        resolveBoundWorkspacesFromIdeEnv: serverModule.resolveBoundWorkspacesFromIdeEnv,
        ...
        createWorkspaceGenerationGuard: workspaceRegistryModule.createWorkspaceGenerationGuard,
        createPromptLedgerSink: promptLedgerModule.createPromptLedgerSink,
    };
}
function advertisedMaxSessions(value) {
    if (value === undefined)
...
function channelIdleTimeoutMs(value) {
    return value !== undefined && Number.isFinite(value) && value > 0
        ? Math.min(value, MAX_TIMEOUT_MS)
        : 0;
}
function sessionIdleTimeoutMs(value) {
...
function createBootstrapCapabilities(input) {
    return {
        ...
        ...(input.lailatulcoderCodeVersion
            ? { qwenCodeVersion: input.lailatulcoderCodeVersion }
            : {}),
        mode: input.opts.mode,
        features: currentServeFeaturesForRunQwenServe(input.opts, input.sessionShellCommandEnabled, input.sessionArtifactsPersistenceAvailable, input.env),
        modelServices: [],
        workspaceCwd: input.boundWorkspace,
        transports: ['rest'],
        policy: { permission: input.permissionPolicy ?? 'first-responder' },
        limits: {
            maxPendingPromptsPerSession: advertisedMaxPendingPromptsPerSession(input.opts.maxPendingPromptsPerSession),
            sessionRestoreTimeoutMs: resolveSessionRestoreTimeoutMs(input.opts),
        },
    };
}
function validateRateLimitOptions(opts) {
    ...
    for (const [name, value] of [
        ...
        if (value !== undefined &&
            (!Number.isFinite(value) || !Number.isInteger(value) || value <= 0)) {
            throw new TypeError(`Invalid ${name}: ${value}. Must be a positive integer.`);
        }
    }
    if (opts.rateLimitWindowMs !== undefined &&
        ...
        throw new TypeError(`Invalid rateLimitWindowMs: ${opts.rateLimitWindowMs}. Must be an integer >= 1000.`);
    }
}
function installSameOriginOriginStrip(app, getPort) {
...
export function createLazyBridgeProxy(getBridge, getStartupError = () => undefined) {
    return new Proxy({}, {
        get(_target, prop) {
            ...
            if (!bridge) {
                ...
                if (startupError) {
                    throw new Error(`Daemon bridge runtime is not available: ${startupError}`);
                }
                throw new Error('Daemon bridge runtime is still starting.');
            }
            const value = Reflect.get(bridge, prop, bridge);
            return typeof value === 'function' ? value.bind(bridge) : value;
        },
    });
}
export function resolveRuntimeStartupTimeoutMs(override) {
...
export async function waitForRuntimeStartingForShutdown(runtimeStarting, daemonLog, timeoutMs = SHUTDOWN_FORCE_CLOSE_MS) {
    if (!runtimeStarting)
        return;
    let timer;
    await Promise.race([
        ...
        new Promise((resolve) => {
            timer = setTimeout(() => {
                daemonLog.warn(`${timeoutMs}ms runtime-startup wait reached during shutdown; continuing listener close`);
                resolve();
            }, timeoutMs);
            timer.unref();
        }),
...
function runtimeStartupEnvelope(runtimeError) {
    return runtimeError
        ? RUNTIME_STARTUP_FAILED_ENVELOPE
        : RUNTIME_STARTUP_STARTING_ENVELOPE;
}
function createBootstrapServeApp(input) {
    const { opts, getPort, boundWorkspace, startup, daemonLog, qwenCodeVersion, sessionShellCommandEnabled, sessionArtifactsPersistenceAvailable, permissionPolicy, multiWorkspaceCapabilitiesRequireRuntime, getRuntimeError, getChannelWorkerSnapshot, getChannelWorkerSnapshots, onHealthServed, } = input;
    const app = express();
    ...
    app.get(BOOTSTRAP_CAPABILITIES_PATH, (_req, res) => {
        ...
        res.status(200).json(createBootstrapCapabilities({
            opts,
            boundWorkspace,
            qwenCodeVersion,
            sessionShellCommandEnabled,
    ...
    app.get(BOOTSTRAP_DAEMON_STATUS_PATH, (req, res) => {
        ...
        const bootstrapJournalGrowthPoolMb = opts.daemonMemoryBudget !== undefined
        ...
        const response = {
            ...
            security: {
                ...
                allowOriginConfigured: opts.allowOrigins !== undefined && opts.allowOrigins.length > 0,
            ...
            limits: {
                ...
                maxPendingPromptsPerSession: advertisedMaxPendingPromptsPerSession(opts.maxPendingPromptsPerSession),
                ...
                compactedReplayMaxBytes: opts.compactedReplayMaxBytes ?? DEFAULT_COMPACTED_REPLAY_MAX_BYTES,
                ...
                memory: toDaemonStatusMemoryLimits(opts.daemonMemoryBudget, undefined, bootstrapJournalGrowthPoolMb > 0
                    ? {
                        poolBytes: bootstrapJournalGrowthPoolMb * 1024 * 1024,
                        hardCapBytes: JOURNAL_GROWTH_HARD_CAP_BYTES,
                        baselineMaxEvents: opts.maxJournalEvents ?? DEFAULT_MAX_JOURNAL_EVENTS,
                        baselineMaxBytes: opts.maxJournalBytes ?? DEFAULT_MAX_JOURNAL_BYTES,
                    }
                    : null),
            },
            capabilities: {
                protocolVersions: getServeProtocolVersions(),
                features: currentServeFeaturesForRunQwenServe(opts, sessionShellCommandEnabled, sessionArtifactsPersistenceAvailable, process.env),
            },
            runtime: {
    ...
    app.use((req, res, next) => {
        const dispatch = async () => {
            ...
            if (!target &&
                ...
                const preAuthExempted = authGate !== undefined &&
                    (await Promise.resolve(options.isPreAuthRequest?.(req)).catch(() => false)) === true;
...
function createDeferredChannelWebhookAuth(resolveSource, runtime, daemonLog) {
    return (req, res, next) => {
        ...
        const secret = readDeferredWebhookSecret(runtime, configSource.workspaceCwd, channelName, source, configSource.env);
        if (!matchesWebhookSecret(req.get('x-LailatulCoder-webhook-secret'), secret)) {
            daemonLog.warn('deferred webhook auth failed', {
                channelName,
                source,
...
function readDeferredWebhookSecret(runtime, workspace, channelName, source, env) {
    try {
        const rawConfig = runtime.loadChannelsConfig(workspace)[channelName];
        if (typeof rawConfig !== 'object' || rawConfig === null) {
            return undefined;
        }
        return runtime.parseChannelWebhookConfig(channelName, rawConfig, env)?.sources[source]?.secret;
    }
    catch (err) {
        const reason = err instanceof Error ? err.message : String(err);
        writeStderrLine(`[webhook-secret] failed to read deferred webhook secret for ${sanitizeLogText(channelName, 128)}/${sanitizeLogText(source, 128)}: ${sanitizeLogText(reason, 512)}`);
        return undefined;
    }
}
 ...
 */
export function validateAndCanonicalizeWorkspaceInput(rawWorkspace) {
    ...
    if (workspace === null) {
        throw new Error(`Invalid --workspace "${rawWorkspace}": must be an absolute path.`);
    }
    try {
        const stats = fs.statSync(workspace);
        if (!stats.isDirectory()) {
            throw new Error(`Invalid --workspace "${workspace}": exists but is not a directory.`);
        }
    }
    catch (err) {
        if (err && typeof err === 'object' && 'code' in err) {
            const code = err.code;
            if (code === 'ENOENT') {
                throw new Error(`Invalid --workspace "${workspace}": directory does not exist.`);
            }
            ...
            if (code === 'EACCES' || code === 'EPERM') {
                throw new Error(`Invalid --workspace "${workspace}": permission denied ` +
...
export async function runQwenServe(optsIn, deps = {}) {
    let daemonLog;
    let owner = 'startup';
    let restoreScrubbedLoaderEnv;
    let installedLoaderRejectionReporter;
    try {
        ...
        if (installedLoaderRejectionReporter) {
            clearLoaderKeyRejectionReporterIfCurrent(installedLoaderRejectionReporter);
        }
        if (daemonLog && owner === 'startup') {
            const startupLog = daemonLog;
            writeDaemonLifecycleBestEffort(() => startupLog.error('daemon startup failed', error instanceof Error ? error : new Error(String(error))));
...
async function runQwenServeImpl(optsIn, deps, loggerLifecycle) {
    ...
    if (optsIn.memoryProjectScope !== undefined &&
        !MEMORY_PROJECT_SCOPES.includes(optsIn.memoryProjectScope)) {
        throw new TypeError(`Invalid memoryProjectScope: ${String(optsIn.memoryProjectScope)}. ` +
            'Must be "git-root" or "workspace".');
    }
    preResolveServeFastPathHomeEnvOverrides();
    const baseEnv = { ...process.env };
    const launchMemoryProjectScopeValue = baseEnv['QWEN_CODE_MEMORY_PROJECT_SCOPE'];
    const launchMemoryProjectScope = launchMemoryProjectScopeValue?.trim()
        ? launchMemoryProjectScopeValue
        : undefined;
    const memoryProjectScopeValue = optsIn.memoryProjectScope ?? launchMemoryProjectScope ?? 'workspace';
    const memoryProjectScopeSource = optsIn.memoryProjectScope !== undefined
        ? 'option'
        : launchMemoryProjectScope !== undefined
            ? 'environment'
            : 'default';
    const resolvedMemoryProjectScope = memoryProjectScopeValue.trim().toLowerCase() === 'workspace'
        ? 'workspace'
        : 'git-root';
    ...
    const daemonRuntimeBaseEnv = Object.freeze(baseEnv);
    ...
    // environment back.
    const loaderEnvScrub = acquireInheritedLoaderEnvScrub(process.env, 'LailatulCoder serve', 'daemon');
    const scrubbedLoaderEnvKeys = loaderEnvScrub.removedKeys;
    const restoreScrubbedLoaderEnv = () => {
    ...
    const sessionShellCommandEnabled = optsIn.enableSessionShell === true && token !== undefined;
    if (optsIn.enableSessionShell === true && token === undefined) {
        writeStderrLine(`LailatulCoder serve: --enable-session-shell ignored because no bearer token ` +
    ...
    const promptDeadlineMs = optsIn.promptDeadlineMs ??
        parseDeadlineEnv(QWEN_SERVE_PROMPT_DEADLINE_MS_ENV, process.env[QWEN_SERVE_PROMPT_DEADLINE_MS_ENV]);
    const writerIdleTimeoutMs = optsIn.writerIdleTimeoutMs ??
        parseDeadlineEnv(QWEN_SERVE_WRITER_IDLE_TIMEOUT_MS_ENV, process.env[QWEN_SERVE_WRITER_IDLE_TIMEOUT_MS_ENV]);
    ...
    const chromeExtensionOriginAllowed = hasChromeExtensionOrigin(optsIn.allowOrigins);
    ...
    const opts = {
        ...
        clientMcpOverWs: optsIn.clientMcpOverWs ??
            (!envFlagDisabled(clientMcpOverWsEnv) &&
                clientMcpOverWsEnv !== undefined),
        cdpTunnelOverWs: optsIn.cdpTunnelOverWs ??
    ...
    let channelRuntime = opts.channelSelection
        ...
        : undefined;
    let channelServicePidfile = deps.channelServicePidfile ?? channelRuntime?.channelServicePidfile;
    const ensureChannelRuntime = async () => {
    ...
    const removeCurrentServePidfile = () => {
    ...
    const reserveChannelServicePidfile = (selection) => {
        if (!channelServicePidfile) {
    ...
    if (!opts.hostname.startsWith('[') && opts.hostname.split(':').length === 2) {
        ...
        throw new Error(`Invalid --hostname "${opts.hostname}": looks like a "host:port" ` +
    ...
    if ((opts.tlsCert && !opts.tlsKey) || (!opts.tlsCert && opts.tlsKey)) {
        throw new Error(`--tls-cert and --tls-key must be provided together (got only ` +
    ...
    if (opts.tlsCert && opts.tlsKey) {
        ...
        try {
            ...
            throw new Error(`Failed to read --tls-cert "${opts.tlsCert}": ` +
        ...
        try {
            ...
            throw new Error(`Failed to read --tls-key "${opts.tlsKey}": ` +
                `${err instanceof Error ? err.message : String(err)}`);
        }
        ...
        try {
            ...
            throw new Error(`--tls-cert "${opts.tlsCert}" is not a valid certificate: ` +
                `${err instanceof Error ? err.message : String(err)}`);
        }
        const now = Date.now();
        if (new Date(x509.validTo).getTime() < now) {
            throw new Error(`--tls-cert "${opts.tlsCert}" expired on ${x509.validTo}. ` +
        ...
        if (new Date(x509.validFrom).getTime() > now) {
            throw new Error(`--tls-cert "${opts.tlsCert}" is not yet valid (validFrom: ` +
    ...
    if (!isLoopbackBind(opts.hostname) && !token) {
        throw new Error(`Refusing to bind ${opts.hostname}:${opts.port} without a bearer token. ` +
    ...
    if (opts.requireAuth && !token) {
        throw new Error(`Refusing to start with --require-auth set but no bearer token ` +
    ...
    if (opts.allowOrigins && opts.allowOrigins.length > 0) {
        ...
        writeStderrLine(`LailatulCoder serve: --allow-origin: ${opts.allowOrigins.join(', ')}` +
            (parsed.allowAny
    ...
    if (opts.allowPrivateAuthBaseUrl) {
        writeStderrLine('LailatulCoder serve: --allow-private-auth-base-url enabled; ' +
    ...
    const validateAndCanonicalizeWorkspace = validateAndCanonicalizeWorkspaceInput;
    ...
    // runtimes.
    const liveConversationWorkspace = deps.liveConversationWorkspace ?? new ConversationWorkspace();
    const isReservedConversationWorkspace = (candidate) => {
    ...
    const reservedRawWorkspace = rawWorkspaces.find((workspace) => isReservedConversationWorkspace(workspace));
    if (reservedRawWorkspace) {
        throw new Error(`Workspace ${JSON.stringify(reservedRawWorkspace)} is reserved for Conversations.`);
    }
    ...
    const seenCanonicalWorkspaces = new Set();
    for (const workspace of workspaceInputs) {
        if (seenCanonicalWorkspaces.has(workspace.cwd)) {
            throw new Error(`Duplicate --workspace value resolves to ${JSON.stringify(workspace.cwd)}.`);
        }
    ...
    for (let i = 0; i < workspaceInputs.length; i++) {
        for (let j = i + 1; j < workspaceInputs.length; j++) {
            ...
            if (firstRel &&
                ...
                throw new Error(`Nested --workspace values are not supported: ` +
    ...
    if (workspaceInputs.length > MAX_REGISTERED_WORKSPACES) {
        throw new Error(`At most ${MAX_REGISTERED_WORKSPACES} --workspace values may be registered.`);
    }
    ...
    if (opts.daemonMemoryBudget.budgetSource === 'flag' ||
    ...
    const journalGrowthPoolMbValue = opts.daemonMemoryBudget !== undefined
        ? serveJournalGrowthPoolMb({
            budget: opts.daemonMemoryBudget,
            maxJournalEvents: opts.maxJournalEvents,
            maxJournalBytes: opts.maxJournalBytes,
        ...
        : 0;
    const journalGrowthPoolBytes = journalGrowthPoolMbValue > 0
    ...
    const registerJournalGrowthSessionLimits = (provider) => {
        journalGrowthSessionLimitProviders.add(provider);
        return () => {
            journalGrowthSessionLimitProviders.delete(provider);
        };
    };
    const reservedStartupWorkspace = workspaceInputs.find((workspace) => isReservedConversationWorkspace(workspace.cwd));
    if (reservedStartupWorkspace) {
        throw new Error(`Workspace ${JSON.stringify(reservedStartupWorkspace.raw)} is reserved for Conversations.`);
    }
    let workspaceRegistrationStore = deps.workspaceRegistrationStore;
    if (workspaceRegistrationStore === undefined &&
        process.env['QWEN_SERVE_NO_PERSISTENT_REGISTRATION'] !== '1') {
        const { WorkspaceRegistrationStore } = await import('./workspace-registration-store.js');
        ...
        try {
            ...
            for (const storedWorkspace of stored.workspaces) {
                ...
                if (isReservedConversationWorkspace(storedWorkspace)) {
                    writeStderrLine(`LailatulCoder serve: skipping persisted workspace registration ${JSON.stringify(storedWorkspace)}: path is reserved for Conversations`);
                    continue;
                }
                let cwd;
                try {
                    ...
                    writeStderrLine(`LailatulCoder serve: skipping persisted workspace registration ${JSON.stringify(storedWorkspace)}: ${err instanceof Error ? err.message : String(err)}`);
                    continue;
                }
                ...
                if (nested) {
                    writeStderrLine(`LailatulCoder serve: skipping persisted workspace registration ${JSON.stringify(storedWorkspace)}: path nests with an explicit or earlier restored workspace`);
                    continue;
                }
                if (workspaceInputs.length >= MAX_REGISTERED_WORKSPACES) {
                    writeStderrLine(`LailatulCoder serve: skipping persisted workspace registration ${JSON.stringify(storedWorkspace)}: workspace limit reached`);
                    continue;
                }
            ...
            writeStderrLine(`LailatulCoder serve: failed to read persisted workspace registrations: ${err instanceof Error ? err.message : String(err)}; continuing with explicit workspaces only`);
        }
    }
    if (workspaceInputs.length > 1 && deps.bridge) {
        throw new Error('Injected bridge dependencies are only supported with a single workspace; ' +
    ...
    try {
        ...
        contextFilenameForInit = extractContextFilename(bootSettings.context?.fileName);
        ...
        writeStderrLine(`LailatulCoder serve: could not read settings for context.fileName / ` +
    ...
    const loaderRejectionReporter = (source, freshKeys) => {
        daemonLog.warn('rejected loader-affecting env keys; they were not applied', {
            source,
    ...
    if (scrubbedLoaderEnvKeys.length > 0) {
        daemonLog.info('scrubbed inherited loader env vars from the daemon process; ' +
            'session subprocesses will not inherit them', { removedKeys: scrubbedLoaderEnvKeys });
    }
    ...
    if (fastPathRejectedLoaderKeys.length > 0) {
        daemonLog.info('rejected loader-affecting env keys during serve fast-path boot; ' +
            'they were not applied to the daemon process', { rejectedKeys: fastPathRejectedLoaderKeys });
    }
    let loggerPublished = false;
    let loggerSignalOwned = false;
    writeStderrLine(`LailatulCoder serve: daemon log → ${daemonLog.getLogPath() || '(disabled)'}`);
    ...
    if (opts.mcpClientBudget !== undefined) {
        if (!Number.isFinite(opts.mcpClientBudget) ||
            !Number.isInteger(opts.mcpClientBudget) ||
            opts.mcpClientBudget <= 0) {
            throw new TypeError(`Invalid mcpClientBudget: ${opts.mcpClientBudget}. Must be a positive integer.`);
        }
    }
    if (opts.mcpBudgetMode === 'enforce' && opts.mcpClientBudget === undefined) {
        throw new Error('mcpBudgetMode="enforce" requires a positive mcpClientBudget. ' +
    ...
    if (opts.promptDeadlineMs !== undefined) {
        if (!isPositiveIntegerMs(opts.promptDeadlineMs)) {
            throw new TypeError(`Invalid promptDeadlineMs: ${opts.promptDeadlineMs}. Must be a positive integer (milliseconds).`);
        }
    ...
    if (opts.maxSessions !== undefined) {
        if (Number.isNaN(opts.maxSessions) || opts.maxSessions < 0) {
            throw new TypeError(`Invalid maxSessions: ${opts.maxSessions}. Must be a number >= 0 ` +
                `(0 / Infinity = unlimited).`);
        }
    }
    if (opts.maxTotalSessions !== undefined) {
        if (!isNonNegativeIntegerOrInfinity(opts.maxTotalSessions)) {
            throw new TypeError(`Invalid maxTotalSessions: ${opts.maxTotalSessions}. Must be a non-negative integer ` +
                `(0 / Infinity = unlimited).`);
        }
    }
    if (opts.maxPendingPromptsPerSession !== undefined) {
        if (!isNonNegativeIntegerOrInfinity(opts.maxPendingPromptsPerSession)) {
            throw new TypeError(`Invalid maxPendingPromptsPerSession: ${opts.maxPendingPromptsPerSession}. Must be a non-negative integer (0 / Infinity = unlimited).`);
        }
    }
    if (opts.eventRingSize !== undefined) {
        if (!Number.isInteger(opts.eventRingSize) ||
    ...
    if (opts.writerIdleTimeoutMs !== undefined) {
        if (!isPositiveIntegerMs(opts.writerIdleTimeoutMs)) {
            throw new TypeError(`Invalid writerIdleTimeoutMs: ${opts.writerIdleTimeoutMs}. Must be a positive integer (milliseconds).`);
        }
    }
    if (opts.channelIdleTimeoutMs !== undefined) {
        if (!Number.isFinite(opts.channelIdleTimeoutMs) ||
            !Number.isInteger(opts.channelIdleTimeoutMs) ||
            opts.channelIdleTimeoutMs < 0) {
            throw new TypeError(`Invalid channelIdleTimeoutMs: ${opts.channelIdleTimeoutMs}. Must be a non-negative integer (milliseconds, 0 = immediate kill).`);
        }
    }
    if (opts.sessionReapIntervalMs !== undefined) {
        if (!isNonNegativeIntegerMs(opts.sessionReapIntervalMs)) {
            throw new TypeError(`Invalid sessionReapIntervalMs: ${opts.sessionReapIntervalMs}. Must be a non-negative integer (milliseconds, 0 = disabled).`);
        }
    }
    if (opts.sessionIdleTimeoutMs !== undefined) {
        if (!isNonNegativeIntegerMs(opts.sessionIdleTimeoutMs)) {
            throw new TypeError(`Invalid sessionIdleTimeoutMs: ${opts.sessionIdleTimeoutMs}. Must be a non-negative integer (milliseconds, 0 = disabled).`);
        }
    }
    if (opts.initializeTimeoutMs !== undefined) {
        if (!isPositiveIntegerMs(opts.initializeTimeoutMs)) {
            throw new TypeError(`Invalid initializeTimeoutMs: ${opts.initializeTimeoutMs}. Must be a positive integer (milliseconds).`);
        }
    ...
    if (opts.permissionResponseTimeoutMs !== undefined) {
        if (!Number.isFinite(opts.permissionResponseTimeoutMs) ||
            !Number.isInteger(opts.permissionResponseTimeoutMs) ||
            opts.permissionResponseTimeoutMs < 0) {
            throw new TypeError(`Invalid permissionResponseTimeoutMs: ${opts.permissionResponseTimeoutMs}. Must be a non-negative integer (milliseconds, 0 = disabled / wait forever).`);
        }
    }
    const rawExternalToolGuard = opts
        .externalToolGuard;
    if (rawExternalToolGuard !== undefined &&
        (typeof rawExternalToolGuard !== 'object' ||
            rawExternalToolGuard === null ||
            rawExternalToolGuard.mode !== 'required')) {
        throw new TypeError("Invalid externalToolGuard: omit it for off mode or set mode to 'required'.");
    }
    opts.maxTotalSessions ??= deriveDefaultMaxTotalSessions(opts.maxSessions, workspaceInputs.length);
    ...
    let externalToolGuardHandler;
    if (opts.externalToolGuard?.mode === 'required') {
        if (deps.bridge) {
            throw new Error('Required external tool guarding cannot be combined with an injected bridge.');
        }
        const { RequiredExternalToolGuard } = await import('./external-tool-guard-provider.js');
        ...
        writeStderrLine('LailatulCoder serve: required external tool guard handshake succeeded.');
    }
    // Keep the guard's core helper imports out of the serve fast-path bundle.
    const { createDaemonToolGuard } = await import('./daemon-git-worktree-guard.js');
    const daemonToolGuardHandler = createDaemonToolGuard(externalToolGuardHandler);
    const childEnvOverrides = {
        QWEN_SERVE_MCP_CLIENT_BUDGET: opts.mcpClientBudget !== undefined
    ...
    let cliVersion;
    const diagnosticSink = (line, level) => daemonLog.raw(line, level);
    ...
    const webShellDir = opts.serveWebShell === false ? undefined : resolveWebShellDir();
    if (opts.serveWebShell !== false) {
        if (!webShellDir) {
            writeStderrLine('LailatulCoder serve: Web Shell assets not found; serving API only. ' +
            ...
            writeStderrLine(`LailatulCoder serve: Web Shell UI served from ${webShellDir}`);
            if (!isLoopbackBind(opts.hostname)) {
                writeStderrLine('LailatulCoder serve: Web Shell UI is served WITHOUT auth on a non-loopback ' +
                ...
                if (!opts.allowOrigins || opts.allowOrigins.length === 0) {
                    writeStderrLine('LailatulCoder serve: without --allow-origin the Web Shell is read-only on a ' +
    ...
    const liveDiscoveryStableBaseDir = path.resolve(deps.liveDiscoveryStableBaseDir ?? path.join(os.homedir(), '.lailatulcoder'));
    ...
    const markServeAppStartupReady = () => {
        if (serveAppStartupSettled)
            return;
        serveAppStartupSettled = true;
        resolveServeAppStartup();
    };
    const markServeAppStartupFailed = (error) => {
        if (serveAppStartupSettled)
            return;
        serveAppStartupSettled = true;
    ...
    let runtimeApp;
    let runtimeAppForCleanup;
    let bridgeRef = deps.bridge;
    ...
    const disposeDaemonEventLoopMonitor = () => {
        ...
        try {
            ...
            daemonLog.warn(`event loop monitor dispose error: ${err instanceof Error ? err.message : String(err)}`);
        }
        try {
            metricsSampler?.dispose();
        }
        catch (err) {
            daemonLog.warn(`metrics sampler dispose error: ${err instanceof Error ? err.message : String(err)}`);
        }
    };
    let channelWorkerManager;
    let channelWorkerManagerStarting;
    let channelControlDraining = false;
    let channelWorkspaceGroups;
    const channelWebhookEnvByWorkspace = new Map();
    let channelWebhookConfigVersion = 0;
    let refreshChannelWebhookConfigs;
    let ensureChannelWorkerManager;
    const getChannelWebhookConfigSources = () => {
        const app = runtimeApp ?? runtimeAppForCleanup;
        const registry = app?.locals?.['workspaceRegistry'];
        return (channelWorkspaceGroups ?? []).map((group) => {
            const env = registry?.getByWorkspaceCwd(group.workspaceCwd)?.env.effectiveEnv ??
    ...
    const resolveChannelWebhookConfigSource = (channelName) => {
        const source = getChannelWebhookConfigSources().find((source) => !source.channelNames || source.channelNames.includes(channelName));
        if (source)
            return source;
    ...
    const getChannelWorkerSnapshot = () => channelWorkerManager?.primarySnapshot() ?? {
        enabled: false,
        state: 'disabled',
        channels: [],
    };
    const getChannelWorkerSnapshots = () => channelWorkerManager?.snapshots() ?? [];
    const getChannelWorkerControl = () => channelWorkerManager?.state() ?? {
        enabled: false,
        selection: null,
        transition: 'idle',
        workers: [],
    };
    const daemonDrainingError = () => Object.assign(new Error('Daemon is shutting down.'), {
        code: 'daemon_draining',
    });
    const setChannelWorkerSelection = async (selection) => {
        if (channelControlDraining)
            throw daemonDrainingError();
        const manager = await ensureChannelWorkerManager?.();
        if (!manager)
            throw new Error('Channel worker manager is unavailable.');
        if (channelControlDraining) {
    ...
    const stopChannelWorker = async () => {
        if (channelControlDraining)
            throw daemonDrainingError();
        const manager = channelWorkerManager ?? (await channelWorkerManagerStarting);
        if (channelControlDraining) {
            await manager?.shutdown().catch(() => undefined);
            throw daemonDrainingError();
        }
    ...
    const reloadChannelWorker = async () => {
        if (channelControlDraining)
            throw daemonDrainingError();
        const manager = channelWorkerManager ?? (await channelWorkerManagerStarting);
        if (channelControlDraining) {
        ...
        if (!manager) {
            return { enabled: false, state: 'disabled', channels: [] };
        }
        try {
    ...
    const isLiveWorker = (snapshot) => snapshot.state === 'running' || snapshot.state === 'starting';
    ...
    const writeChannelWorkerPidfile = () => {
        if (runtimeStartupError !== undefined)
            return;
        if (!channelPidfileReserved || !channelServicePidfile)
            return;
        const snapshots = getChannelWorkerSnapshots();
        const workers = snapshots.map((snapshot) => ({
        ...
        const includeWorkers = channelWorkerPidfileUsesWorkers && workers.length > 0;
        try {
            ...
            daemonLog.warn(`failed to write channel worker pidfile metadata: ${err instanceof Error ? err.message : String(err)}`);
        }
    };
    const handleBridge = deps.bridge ??
        createLazyBridgeProxy(() => bridgeRef, () => runtimeStartupError);
    const shutdownBridges = new WeakSet();
    const disposedRuntimeApps = new WeakSet();
    const stoppedRuntimeAppProducers = new WeakSet();
    const stoppedExtensionReconcilers = new WeakSet();
    const stoppedTrustPolicyMonitors = new WeakSet();
    const stopTrustPolicyMonitor = (app) => {
        if (!app || stoppedTrustPolicyMonitors.has(app))
            return;
        stoppedTrustPolicyMonitors.add(app);
        const stop = app.locals?.['stopTrustPolicyMonitor'];
        try {
            stop?.();
        }
        catch (err) {
            daemonLog.warn(`trust policy monitor dispose error: ${err instanceof Error ? err.message : String(err)}`);
        }
    };
    const stopExtensionReconciler = (app) => {
        if (!app || stoppedExtensionReconcilers.has(app))
            return;
        ...
        try {
            ...
            daemonLog.warn(`extension generation reconciler dispose error: ${err instanceof Error ? err.message : String(err)}`);
        }
    };
    const stopRuntimeAppProducers = (app) => {
        if (!app || stoppedRuntimeAppProducers.has(app))
            return;
        stoppedRuntimeAppProducers.add(app);
        const locals = app.locals;
        const stopSafely = (name, stop) => {
            try {
                stop?.();
            }
            catch (err) {
                daemonLog.warn(`${name} dispose error: ${err instanceof Error ? err.message : String(err)}`);
            }
        };
    ...
    const disposeRuntimeAppResources = (app) => {
        if (!app || disposedRuntimeApps.has(app))
            return;
        ...
        if (deviceFlowRegistry) {
            try {
                ...
                daemonLog.warn(`device-flow registry dispose error: ${err instanceof Error ? err.message : String(err)}`);
            }
        }
        ...
        const localControlService = app.locals?.['localControlService'];
        if (localControlService) {
            void localControlService.dispose().catch((err) => {
                daemonLog.warn(`Local Control dispose error: ${err instanceof Error ? err.message : String(err)}`);
            });
        }
        const acpHandle = app.locals?.['acpHandle'];
        if (acpHandle?.dispose) {
            try {
                acpHandle.dispose();
            }
            catch (err) {
                daemonLog.warn(`ACP handle dispose error: ${err instanceof Error ? err.message : String(err)}`);
            }
        }
        ...
        if (rateLimiter) {
            try {
                ...
                daemonLog.warn(`rate limiter dispose error: ${err instanceof Error ? err.message : String(err)}`);
            }
        }
        disposeDaemonEventLoopMonitor();
    };
    const getRuntimeBridgesForCleanup = () => {
        const appForCleanup = runtimeApp ?? runtimeAppForCleanup;
        const registry = appForCleanup?.locals?.['workspaceRegistry'];
        const bridges = [
            ...(registry
    ...
    const buildRuntime = async () => {
        const [runtime, core, settingsRuntime, resolvedCliVersion, trustPolicy] = await Promise.all([
        ...
        const bootPrimaryTrustDecision = trustPolicy.evaluateDaemonWorkspaceTrust(bootTrustSnapshot, boundWorkspace);
        const trustedWorkspace = deps.trustedWorkspace ?? bootPrimaryTrustDecision.targetTrusted;
        const workspaceTrustHotReloadAvailable = deps.trustedWorkspace === undefined &&
            deps.bridge === undefined &&
            deps.fsFactory === undefined;
        let managedScratchRoot;
        try {
            ...
            managedScratchRoot = prepareManagedScratchRoot(path.join(core.Storage.getGlobalQwenDir(), 'scratch-workspaces'), workspaceInputs.map((workspace) => workspace.cwd));
        }
        catch (err) {
            writeStderrLine(`LailatulCoder serve: managed scratch workspaces are unavailable: ${err instanceof Error ? err.message : String(err)}`);
        }
        let runtimeBootSettings;
        try {
            runtimeBootSettings = settingsRuntime.settings.loadSettings(boundWorkspace, {
            ...
            writeStderrLine(`LailatulCoder serve: could not read full settings for runtime startup ` +
                `(${err instanceof Error ? err.message : String(err)}); falling back to defaults.`);
        }
        if (deps.trustedWorkspace === undefined &&
            runtimeBootSettings &&
            !trustedWorkspace) {
            daemonLog.warn('workspace file writes are disabled because the bound workspace is not trusted', { workspace: boundWorkspace });
        }
        const runtimeEnvSnapshot = runtimeBootSettings
            ? settingsRuntime.environment.buildRuntimeEnvironment(runtimeBootSettings.merged, boundWorkspace, daemonRuntimeBaseEnv, trustedWorkspace)
            : {
                effectiveEnv: { ...daemonRuntimeBaseEnv },
                overlayKeys: Object.freeze([]),
                envFilePaths: Object.freeze([]),
                envFileReadFailed: false,
                envFileReadFailures: Object.freeze([]),
            };
        const resolveSessionRuntimeBaseDir = (workspace, settings, effectiveEnv) => {
            const resolveConfiguredPath = (configuredPath, relativeTo) => {
                const expanded = configuredPath === '~'
                    ? os.homedir()
                    : configuredPath.startsWith('~/') ||
                        configuredPath.startsWith('~\\')
                        ? path.join(os.homedir(), ...configuredPath
                            .slice(2)
                            .split(/[/\\]+/)
                            .filter(Boolean))
                        : configuredPath;
            ...
            return homeDir
                ? path.join(homeDir, '.lailatulcoder')
                : path.join(os.tmpdir(), '.lailatulcoder');
        };
        const logRuntimeEnvFileReadFailures = (workspace, snapshot) => {
        ...
        const primarySessionRuntimeBaseDir = resolveSessionRuntimeBaseDir(boundWorkspace, runtimeBootSettings, runtimeEnvSnapshot.effectiveEnv);
        const sessionAttachmentsRoot = (workspace, runtimeBaseDir) => path.join(new core.Storage(workspace, runtimeBaseDir).getProjectTempDir(), 'attachments');
        const runtimeEffectiveEnv = {
            ...runtimeEnvSnapshot.effectiveEnv,
            QWEN_RUNTIME_DIR: primarySessionRuntimeBaseDir,
        };
        const replaceRuntimeEffectiveEnv = (nextEnv) => {
        ...
        try {
            ...
            if (err instanceof core.FatalConfigError) {
                throw new core.FatalConfigError(`Invalid telemetry configuration: ${err.message}.`);
            }
            throw err;
        }
        ...
        await core.initializeTelemetry(createDaemonTelemetryRuntimeConfig(daemonTelemetrySettings, resolvedCliVersion, `daemon:${process.pid}`, {
        ...
        core.registerDaemonEventLoopLagGauge(() => currentDaemonEventLoopMonitor.snapshot());
        ...
        const recordPipeMessage = (direction, bytes) => {
        ...
        const createRuntimeBridgeTelemetry = (workspaceHash) => {
            ...
            telemetry.metrics = {
                ...
                channelLifecycle(action, expected) {
                    ...
                    core.emitDaemonLog(action === 'spawn'
                        ? 'ACP channel spawned.'
                ...
                tokenUsage(inputTokens, outputTokens, durationMs, apiErrors, apiRetries) {
        ...
        const customIgnoreFiles = runtimeBootSettings?.merged.context?.fileFiltering?.customIgnoreFiles;
        const boundWorkspaces = runtime.resolveBoundWorkspacesFromIdeEnv(boundWorkspace, undefined, (workspace, index) => {
            if (index === 0)
                return true;
            const trustedSecondary = trustPolicy.evaluateDaemonWorkspaceTrust(bootTrustSnapshot, workspace).targetTrusted;
            if (!trustedSecondary) {
                daemonLog.warn('excluding untrusted secondary workspace root from file-system access', { workspace });
            }
        ...
        const runWorkspaceTrustOperation = (operation) => workspaceTrustOperationGate.runExclusive('runtime-topology', operation);
        ...
        const childHeapPolicy = opts.daemonMemoryBudget && !deps.bridge
        ...
        const channelFactory = runtime.createSpawnChannelFactory({
            ...
            pipeHooks: {
                ...
                onMessageObserved: ({ direction, bytes, message }) => observeLargePipeFrame({
        ...
        const workspaceSkillsStatusProvider = runtime.createWorkspaceSkillsStatusProvider({
            workspaceTrusted: trustedWorkspace,
        });
        ...
        const totalSessionAdmission = runtime.createTotalSessionAdmissionController({
            ...
            getBridges: () => runtimeBridges.length > 0
                ? runtimeBridges
                : bridgeRef
                    ? [bridgeRef]
                    : [],
        ...
        const workspaceRegistryForPersistence = { current: undefined };
        const isWorkspaceTrustedForPersistence = (workspace) => workspaceRegistryForPersistence.current?.getByWorkspaceCwd(workspace)
            ?.trusted ??
            (workspaceRegistryForPersistence.current === undefined &&
                workspace === boundWorkspace &&
                trustedWorkspace);
        const loadSettingsForPersistence = (workspace) => {
        ...
        const persistDisabledToolsFn = (workspace, toolName, enabled, assertGenerationOpen) => withSettingsLock(workspace, async () => {
            ...
            const current = Array.isArray(wsDisabled)
                ? wsDisabled.filter((v) => typeof v === 'string')
                : [];
            ...
            if (enabled)
                ...
                next.add(toolName);
            assertGenerationOpen?.();
            fresh.setValue(WORKSPACE_SETTING_SCOPE, 'tools.disabled', [...next].sort(), assertGenerationOpen);
        });
        const persistDisabledSkillsFn = (workspace, skillName, enabled, assertGenerationOpen) => withSettingsLock(workspace, async () => {
            assertGenerationOpen?.();
            const { resolveSkillSettings, skillSettingStrings, updateWorkspaceSkillSettingLists, } = await import('../config/skill-settings.js');
            const fresh = loadSettingsForPersistence(workspace);
            ...
            if (disablement?.reason === 'hard' && disablement.lockedScope) {
                throw new runtime.WorkspaceSkillNotToggleableError(skillName, 'locked', disablement.lockedScope);
            }
            const workspaceDisabled = skillSettingStrings(fresh, WORKSPACE_SETTING_SCOPE, 'disabled');
            const workspaceEnabled = skillSettingStrings(fresh, WORKSPACE_SETTING_SCOPE, 'enabled');
            const next = updateWorkspaceSkillSettingLists({ disabled: workspaceDisabled, enabled: workspaceEnabled }, skillName, enabled, resolved.defaultDisabledNames.has(normalizedName) &&
                !resolved.enabledNames.has(normalizedName));
            const settingsChanges = [];
            if (JSON.stringify(next.disabled) !== JSON.stringify(workspaceDisabled)) {
            ...
            assertGenerationOpen?.();
            fresh.setValues(settingsChanges.map((change) => ({
                scope: WORKSPACE_SETTING_SCOPE,
                ...change,
            })), undefined, assertGenerationOpen);
            return {
                changed: true,
        ...
        const persistDisabledSkillsBatchFn = (workspace, skillNames, enabled, assertGenerationOpen) => withSettingsLock(workspace, async () => {
            assertGenerationOpen?.();
            const { resolveSkillSettings, skillSettingStrings, updateWorkspaceSkillSettingLists, } = await import('../config/skill-settings.js');
            const fresh = loadSettingsForPersistence(workspace);
            const resolved = resolveSkillSettings(fresh);
            const initialDisabled = skillSettingStrings(fresh, WORKSPACE_SETTING_SCOPE, 'disabled');
            const initialEnabled = skillSettingStrings(fresh, WORKSPACE_SETTING_SCOPE, 'enabled');
            ...
            for (const skillName of skillNames) {
                ...
                if (disablement?.reason === 'hard' && disablement.lockedScope) {
                    outcomes.push({
                        skillName,
                        error: new runtime.WorkspaceSkillNotToggleableError(skillName, 'locked', disablement.lockedScope),
                    });
                    continue;
                }
                const updated = updateWorkspaceSkillSettingLists(next, skillName, enabled, resolved.defaultDisabledNames.has(normalizedName) &&
                    !resolved.enabledNames.has(normalizedName));
                const changed = JSON.stringify(updated.disabled) !==
        ...
        const persistSettingFn = (workspace, scope, key, value, assertGenerationOpen) => withSettingsLock(workspace, async () => {
            assertGenerationOpen?.();
            const fresh = loadSettingsForPersistence(workspace);
            assertGenerationOpen?.();
        ...
        const persistSettingsFn = (workspace, writes, assertGenerationOpen) => withSettingsLock(workspace, async () => {
            assertGenerationOpen?.();
            const fresh = loadSettingsForPersistence(workspace);
            const writesByScope = new Map();
            for (const write of writes) {
                writesByScope.set(write.scope, (writesByScope.get(write.scope) ?? 0) + 1);
            }
            ...
            try {
                ...
                const failedWrite = writes.find((write) => !committedScopes.has(write.scope)) ??
                    writes[committed];
                const message = `persistSettings partial failure (workspace=${workspace}, committed=${committed}/${writes.length}, failedKey=${failedWrite?.key ?? '<unknown>'}, failedScope=${failedWrite?.scope ?? '<unknown>'}): ${err instanceof Error ? err.message : String(err)}`;
                writeStderrLine(`LailatulCoder serve: ${message}`);
                throw new runtime.WorkspaceSettingsPartialPersistError(message, writes.filter((write) => committedScopes.has(write.scope)), err);
            }
        });
        ...
        const { createSubSessionLauncher } = await import('./create-sub-session.js');
        ...
        const subSessionLauncher = createSubSessionLauncher({
            getBridge: () => bridgeRef,
            boundWorkspace,
            ...subSessionConcurrencyCapsFromSettings(runtimeBootSettings?.merged.serve ?? {}),
        });
        const bridge = deps.bridge ??
            runtime.createAcpSessionBridge({
                sessionAttachmentsRoot: sessionAttachmentsRoot(boundWorkspace, primarySessionRuntimeBaseDir),
                ...
                onChannelDelivery: createBoundChannelDeliveryHandler(boundWorkspace, () => channelWorkerManager, channelDeliveryAuthorizations, daemonLog, channelDeliveryDiagnosticRedaction),
                ...
                promptLedger: runtime.createPromptLedgerSink(boundWorkspace, primarySessionRuntimeBaseDir),
                sessionShellCommandEnabled,
                childEnvOverrides,
                channelFactory,
                ...
                persistApprovalMode: (workspace, mode) => withSettingsLock(workspace, async () => {
                    primaryGenerationGuard.assertOpen();
                    if (!trustedWorkspace) {
                        throw new Error('Cannot persist approval mode for an untrusted workspace.');
                    }
                    ...
                    primaryGenerationGuard.assertOpen();
                    fresh.setValue(WORKSPACE_SETTING_SCOPE, 'tools.approvalMode', mode, () => primaryGenerationGuard.assertOpen());
                }),
            });
        if (!deps.bridge) {
            bridgeRef = bridge;
        ...
        const workspaceService = runtime.createDaemonWorkspaceService({
            ...
            contextFilename: contextFilenameForInit ?? 'LAILATUL.md',
            statusProvider,
            ...
            reloadDaemonEnv: (workspace, assertGenerationOpen) => withSettingsLock(workspace, async () => {
                assertGenerationOpen?.();
                const fresh = settingsRuntime.settings.loadSettings(workspace, {
                    skipLoadEnvironment: true,
                    skipWorkspaceSettings: !trustedWorkspace,
                ...
                assertGenerationOpen?.();
                const result = settingsRuntime.settings.reloadEnvironment(fresh.merged, workspace, trustedWorkspace);
                let refreshedRuntimeEnv;
                let fallbackReason;
                try {
                    refreshedRuntimeEnv =
                        settingsRuntime.environment.buildRuntimeEnvironment(fresh.merged, workspace, daemonRuntimeBaseEnv, trustedWorkspace);
                }
                catch (err) {
                    fallbackReason = err instanceof Error ? err.message : String(err);
                    daemonLog.warn('failed to rebuild runtime env snapshot after daemon env reload; preserving previous runtime env', {
                ...
                primaryRuntimeEnv.envFileReadFailures.splice(0, primaryRuntimeEnv.envFileReadFailures.length, ...refreshedRuntimeEnv.envFileReadFailures);
                primaryRuntimeEnv.overlayKeys.splice(0, primaryRuntimeEnv.overlayKeys.length, ...refreshedRuntimeEnv.overlayKeys);
                primaryRuntimeEnv.envFilePaths.splice(0, primaryRuntimeEnv.envFilePaths.length, ...refreshedRuntimeEnv.envFilePaths);
                return result;
            }),
        ...
        const createRuntimeEnvMetadata = (workspace, settings, trusted) => {
            const snapshot = settings
                ? settingsRuntime.environment.buildRuntimeEnvironment(settings.merged, workspace, daemonRuntimeBaseEnv, trusted)
                : {
                    effectiveEnv: { ...daemonRuntimeBaseEnv },
                    overlayKeys: Object.freeze([]),
                    envFilePaths: Object.freeze([]),
            ...
            const sessionRuntimeBaseDir = resolveSessionRuntimeBaseDir(workspace, settings, snapshot.effectiveEnv);
            const effectiveEnv = {
        ...
        const removeArrayValue = (values, value) => {
            const index = values.indexOf(value);
            if (index >= 0)
                values.splice(index, 1);
        };
        for (const workspaceInput of workspaceInputs.slice(1)) {
            const secondaryDecision = trustPolicy.evaluateDaemonWorkspaceTrust(bootTrustSnapshot, workspaceInput.cwd);
            ...
            try {
                ...
                writeStderrLine(`LailatulCoder serve: could not read full settings for secondary workspace ` +
            ...
            const secondaryEnv = createRuntimeEnvMetadata(workspaceInput.cwd, secondarySettings, secondaryTrusted);
            const secondaryCustomIgnoreFiles = secondarySettings?.merged.context?.fileFiltering?.customIgnoreFiles;
            const secondaryContextFilename = extractContextFilename(secondarySettings?.merged.context?.fileName) ??
                contextFilenameForInit ??
                'LAILATUL.md';
            const secondaryWorkspaceHash = core.hashDaemonWorkspace(workspaceInput.cwd);
            ...
            const secondarySubSessionLauncher = createSubSessionLauncher({
                ...
                ...subSessionConcurrencyCapsFromSettings(secondarySettings?.merged.serve ?? {}),
            });
            const secondaryBridge = runtime.createAcpSessionBridge({
                sessionAttachmentsRoot: sessionAttachmentsRoot(workspaceInput.cwd, secondaryEnv.sessionRuntimeBaseDir),
                clientMcpSender: secondaryClientMcpSenderRegistry.lookup,
                onCreateSubSession: secondarySubSessionLauncher.launch,
                onChannelDelivery: createBoundChannelDeliveryHandler(workspaceInput.cwd, () => channelWorkerManager, channelDeliveryAuthorizations, daemonLog, channelDeliveryDiagnosticRedaction),
                maxSessions: opts.maxSessions,
                freshSessionAdmission: totalSessionAdmission.admit,
                sessionLifecycle: (event) => {
                ...
                ...(opts.permissionResponseTimeoutMs !== undefined
                    ...
                    : {}),
                boundWorkspace: workspaceInput.cwd,
                promptLedger: runtime.createPromptLedgerSink(workspaceInput.cwd, secondaryEnv.sessionRuntimeBaseDir),
                sessionShellCommandEnabled,
                childEnvOverrides,
                ...
                persistApprovalMode: (workspace, mode) => withSettingsLock(workspace, async () => {
                    ...
                    secondaryGenerationGuard.assertOpen();
                    fresh.setValue(WORKSPACE_SETTING_SCOPE, 'tools.approvalMode', mode, () => secondaryGenerationGuard.assertOpen());
                }),
            });
            ...
            const secondaryWorkspaceService = runtime.createDaemonWorkspaceService({
                ...
                reloadDaemonEnv: (workspace, assertGenerationOpen) => withSettingsLock(workspace, async () => {
                    ...
                    assertGenerationOpen?.();
                    const result = settingsRuntime.settings.reloadEnvironment(fresh.merged, workspace, secondaryTrusted);
                    try {
                        const refreshedRuntimeEnv = settingsRuntime.environment.buildRuntimeEnvironment(fresh.merged, workspace, daemonRuntimeBaseEnv, secondaryTrusted);
                        logRuntimeEnvFileReadFailures(workspace, refreshedRuntimeEnv);
                        secondaryEnv.replace(refreshedRuntimeEnv.effectiveEnv);
                        secondaryEnv.metadata.envFileReadFailed =
                            refreshedRuntimeEnv.envFileReadFailed;
                        secondaryEnv.metadata.envFileReadFailures.splice(0, secondaryEnv.metadata.envFileReadFailures.length, ...refreshedRuntimeEnv.envFileReadFailures);
                        secondaryEnv.metadata.overlayKeys.splice(0, secondaryEnv.metadata.overlayKeys.length, ...refreshedRuntimeEnv.overlayKeys);
                        secondaryEnv.metadata.envFilePaths.splice(0, secondaryEnv.metadata.envFilePaths.length, ...refreshedRuntimeEnv.envFilePaths);
                        delete secondaryEnv.metadata.fallbackReason;
                    }
                    catch (err) {
                        secondaryEnv.metadata.fallbackReason =
                            err instanceof Error ? err.message : String(err);
                        daemonLog.warn('failed to rebuild secondary runtime env snapshot after daemon env reload; preserving previous runtime env', {
                            workspace,
                ...
                queryWorkspaceStatus: (method, idle) => secondaryBridge.queryWorkspaceStatus(method, idle),
                invokeWorkspaceCommand: (method, params, invokeOpts) => secondaryBridge.invokeWorkspaceCommand(method, params, invokeOpts),
                refreshExtensionsForAllSessions: () => secondaryBridge.refreshExtensionsForAllSessions(),
                publishWorkspaceEvent: (event) => secondaryBridge.publishWorkspaceEvent(event),
            });
            ...
            subSessionStoppersByRuntime.set(secondaryRuntime, secondarySubSessionLauncher.stop);
        ...
        core.registerDaemonGaugeCallbacks({
            sessionCount: () => workspaceRegistry
                .listAll()
        ...
        const metricsSamplerTimer = setInterval(() => {
            ...
            try {
                ...
                const cpuPercent = computeCpuPercent(prevCpu, cpu, nowMs - prevCpuAt, cpuCoreCount);
                if (cpu) {
                    prevCpu = cpu;
                    prevCpuAt = nowMs;
                }
                ...
                const primaryRuntimeBridge = primaryEntry.state === 'active'
                ...
                metricsRing.sample(nowMs, {
                    ...
                    activePrompts: workspaceRegistry
                        .listAll()
                        .reduce((sum, item) => sum + (item.bridge.activePromptCount ?? 0), 0),
                    queuedPrompts: workspaceRegistry
                        .listAll()
                        .reduce((sum, item) => sum + (item.bridge.pendingPromptTotal ?? 0), 0),
                    eventLoopLagP99Ms,
                ...
                daemonLog.warn(`metrics sampler tick failed: ${err instanceof Error ? err.message : String(err)}`);
                try {
                    metricsRing.sample(nowMs, {
                        cpuPercent: 0,
                        rssBytes: 0,
        ...
        const createDynamicWorkspaceRuntime = async (cwd, buildOptions) => {
            ...
            if (provenance === 'managed-scratch' &&
                (!managedScratchRoot ||
                    !isManagedScratchChild(cwd, managedScratchRoot.canonicalRoot))) {
                throw new Error('Managed scratch runtime must use an accepted direct child directory');
            }
            if (provenance === 'live-conversation') {
                await liveConversationWorkspace.assertExactRoot(cwd);
            }
            const snapshot = buildOptions?.snapshot ??
                (await trustPolicy.readDaemonTrustPolicySnapshot());
            const decision = buildOptions?.decision ??
                trustPolicy.evaluateDaemonWorkspaceTrust(snapshot, cwd);
            const trusted = provenance === 'managed-scratch' || provenance === 'live-conversation'
                ? true
            ...
            try {
                ...
                writeStderrLine(`LailatulCoder serve: could not read full settings for dynamic workspace ` +
                    `${cwd} (${err instanceof Error ? err.message : String(err)}); ` +
                    `falling back to defaults.`);
            }
            const wsEnv = createRuntimeEnvMetadata(cwd, wsSettings, trusted);
            const wsCustomIgnoreFiles = wsSettings?.merged.context?.fileFiltering?.customIgnoreFiles;
            const wsContextFilename = extractContextFilename(wsSettings?.merged.context?.fileName) ??
                contextFilenameForInit ??
                'LAILATUL.md';
            const wsHash = core.hashDaemonWorkspace(cwd);
            const generationGuard = buildOptions?.generationGuard ??
            ...
            const wsRouteFsFactory = buildOptions?.primary === true && runtimeBoundWorkspaces.length > 1
            ...
            const wsSubSessionLauncher = createSubSessionLauncher({
                ...
                ...(provenance === 'live-conversation'
                    ? {
                        ...
                        isolatedWorkspace: {
                            materializeDirectory: (sessionId) => liveConversationWorkspace.materializeConversationDirectory(sessionId),
                            discardEmptyDirectory: (sessionId) => liveConversationWorkspace.discardEmptyConversationDirectory(sessionId),
                        },
                    }
                    : {}),
                ...subSessionConcurrencyCapsFromSettings(wsSettings?.merged.serve ?? {}),
            });
            let wsBridge;
            try {
                wsBridge = runtime.createAcpSessionBridge({
                    sessionAttachmentsRoot: sessionAttachmentsRoot(cwd, wsEnv.sessionRuntimeBaseDir),
                    clientMcpSender: wsClientMcpRegistry.lookup,
                    onCreateSubSession: wsSubSessionLauncher.launch,
                    onChannelDelivery: createBoundChannelDeliveryHandler(cwd, () => channelWorkerManager, channelDeliveryAuthorizations, daemonLog, channelDeliveryDiagnosticRedaction),
                    maxSessions: opts.maxSessions,
                    freshSessionAdmission: totalSessionAdmission.admit,
                    sessionLifecycle: (event) => {
                        if (event.type === 'registered' && generationGuard.closed)
                            return;
                        sessionOwnerIndex.handleBridgeSessionLifecycle(event);
                    },
                    ...(opts.maxPendingPromptsPerSession !== undefined
                    ...
                    ...(provenance === 'live-conversation'
                        ? {}
                        : {
                            promptLedger: runtime.createPromptLedgerSink(cwd, wsEnv.sessionRuntimeBaseDir),
                        }),
                    sessionShellCommandEnabled,
                    childEnvOverrides,
                    ...
                    persistApprovalMode: (workspace, mode) => withSettingsLock(workspace, async () => {
                        ...
                        generationGuard.assertOpen();
                        fresh.setValue(WORKSPACE_SETTING_SCOPE, 'tools.approvalMode', mode, () => generationGuard.assertOpen());
                    }),
                });
            }
            catch (err) {
            ...
            try {
                wsService = runtime.createDaemonWorkspaceService({
                    ...
                    reloadDaemonEnv: (workspace, assertGenerationOpen) => withSettingsLock(workspace, async () => {
                        ...
                        assertGenerationOpen?.();
                        const result = settingsRuntime.settings.reloadEnvironment(fresh.merged, workspace, trusted);
                        ...
                        try {
                            const refreshedRuntimeEnv = settingsRuntime.environment.buildRuntimeEnvironment(fresh.merged, workspace, daemonRuntimeBaseEnv, trusted);
                            logRuntimeEnvFileReadFailures(workspace, refreshedRuntimeEnv);
                            wsEnv.replace(refreshedRuntimeEnv.effectiveEnv);
                            wsEnv.metadata.envFileReadFailed =
                                refreshedRuntimeEnv.envFileReadFailed;
                            wsEnv.metadata.envFileReadFailures.splice(0, wsEnv.metadata.envFileReadFailures.length, ...refreshedRuntimeEnv.envFileReadFailures);
                            wsEnv.metadata.overlayKeys.splice(0, wsEnv.metadata.overlayKeys.length, ...refreshedRuntimeEnv.overlayKeys);
                            wsEnv.metadata.envFilePaths.splice(0, wsEnv.metadata.envFilePaths.length, ...refreshedRuntimeEnv.envFilePaths);
                            delete wsEnv.metadata.fallbackReason;
                        }
                        catch (err) {
                            wsEnv.metadata.fallbackReason =
                                err instanceof Error ? err.message : String(err);
                            daemonLog.warn('failed to rebuild dynamic runtime env snapshot after daemon env reload; preserving previous runtime env', {
                                workspace,
                    ...
                    queryWorkspaceStatus: (method, idle) => wsBridge.queryWorkspaceStatus(method, idle),
                    invokeWorkspaceCommand: (method, params, invokeOpts) => wsBridge.invokeWorkspaceCommand(method, params, invokeOpts),
                    refreshExtensionsForAllSessions: () => wsBridge.refreshExtensionsForAllSessions(),
                    publishWorkspaceEvent: (event) => {
                        if (buildOptions?.primary === true &&
            ...
            const wsRuntime = {
                ...
                trusted,
                provenance,
                removable: buildOptions?.removable ?? provenance !== 'live-conversation',
                ...
                generationGuard,
                trustMaterialization: buildOptions?.trustMaterialization ??
                    JSON.stringify({
                        trusted,
            ...
            if (provenance === 'existing' && !buildOptions?.snapshot) {
                ...
                if (latest.revision !== snapshot.revision) {
                    ...
                    if ((buildOptions?.validationAttempt ?? 0) >= 2) {
                        throw new Error('Workspace trust policy kept changing during runtime creation.');
                    }
                    const { generationGuard: _staleGuard, ...retryOptions } = buildOptions ?? {};
        ...
        const serveAppForRuntimeLifecycle = { current: undefined };
        const workspaceRuntimeRemoval = {
            async runtimeAdded(runtimeAdded) {
                if (runtimeAdded.provenance === 'live-conversation')
                    return;
                channelWebhookEnvByWorkspace.set(runtimeAdded.workspaceCwd, workspaceRuntimeEffectiveEnv(runtimeAdded, daemonRuntimeBaseEnv));
                channelWebhookConfigVersion += 1;
                refreshChannelWebhookConfigs?.();
                const app = serveAppForRuntimeLifecycle.current ??
                    runtimeApp ??
                    runtimeAppForCleanup;
                const startScheduledTaskKeepaliveForWorkspace = app?.locals?.['startScheduledTaskKeepaliveForWorkspace'];
                startScheduledTaskKeepaliveForWorkspace?.(runtimeAdded);
                if (!channelWorkerManager)
                    return;
                try {
                    if (runtimeAdded.trusted) {
                        await channelWorkerManager.restoreWorkspace(runtimeAdded.workspaceCwd);
                    }
                    await channelWorkerManager.refreshWorkspaces();
                }
                catch (err) {
                    daemonLog.error('workspace channel worker startup error', err instanceof Error ? err : null);
                }
                finally {
                    writeChannelWorkerPidfile();
                }
            },
            beginDrain(runtimeToDrain) {
                if (runtimeToDrain.primary) {
                    if (bridgeRef === runtimeToDrain.bridge)
                        bridgeRef = undefined;
                    invalidatePrimaryServeFeaturesCache();
                }
                ...
                const app = serveAppForRuntimeLifecycle.current ??
                    runtimeApp ??
                    runtimeAppForCleanup;
                const stopScheduledTaskKeepaliveForWorkspace = app?.locals?.['stopScheduledTaskKeepaliveForWorkspace'];
                try {
                    stopScheduledTaskKeepaliveForWorkspace?.(runtimeToDrain.workspaceCwd);
                }
                catch (err) {
                    daemonLog.error('workspace scheduled-task drain error', err instanceof Error ? err : null);
                }
            },
            cancelDrain(runtimeToDrain) {
                ...
                if (runtimeToDrain.provenance !== 'live-conversation') {
                    channelWebhookEnvByWorkspace.set(runtimeToDrain.workspaceCwd, workspaceRuntimeEffectiveEnv(runtimeToDrain, daemonRuntimeBaseEnv));
                    channelWebhookConfigVersion += 1;
                    refreshChannelWebhookConfigs?.();
                }
                const app = serveAppForRuntimeLifecycle.current ??
                ...
                try {
                    ...
                    daemonLog.error('workspace scheduled-task drain rollback error', err instanceof Error ? err : null);
                }
            },
            completeDrain(runtimeToDrain) {
                totalSessionAdmission.completeWorkspaceDrain(runtimeToDrain.workspaceCwd);
                workspaceVoiceCoordinator.completeWorkspaceDrain(runtimeToDrain);
            },
            getActivity(runtimeToDrain) {
                return {
                    pendingSessionStarts: totalSessionAdmission.snapshotForWorkspace(runtimeToDrain.workspaceCwd).inFlight,
                    channelWorkers: runtimeToDrain.provenance === 'live-conversation'
                        ? 0
                        : (channelWorkerManager?.workspaceActivity(runtimeToDrain.workspaceCwd) ?? 0),
                    voiceSessions: workspaceVoiceCoordinator.getWorkspaceActivity(runtimeToDrain),
                };
            },
            disposeRuntime(runtimeToDrain, reason = 'workspace_removed') {
                ...
                const cleanup = (async () => {
                    ...
                    try {
                        await workspaceVoiceCoordinator.disposeRuntime(runtimeToDrain, reason);
                    }
                    catch (err) {
                        daemonLog.error('workspace voice cleanup error', err instanceof Error ? err : null);
                    }
                    if (reason === 'trust_reconfigured' &&
                        workspaceVoiceCoordinator.getWorkspaceActivity(runtimeToDrain) > 0) {
                        containmentErrors.push(new Error('Workspace voice sessions are still active.'));
                    }
                    const stopSubSessions = subSessionStoppersByRuntime.get(runtimeToDrain);
                    try {
                        stopSubSessions?.();
                    }
                    catch {
                    ...
                    if (reason !== 'daemon_shutdown' &&
                        ...
                        await channelWorkerManager
                            ...
                            .catch((err) => {
                            daemonLog.error('workspace channel worker cleanup error', err instanceof Error ? err : null);
                        });
                        try {
                            await channelWorkerManager.refreshWorkspaces();
                        }
                        catch (err) {
                            channelWorkspaceGroups = (channelWorkspaceGroups ?? []).filter((group) => group.workspaceCwd !== runtimeToDrain.workspaceCwd);
                            channelWebhookConfigVersion += 1;
                            refreshChannelWebhookConfigs?.();
                            daemonLog.error('workspace channel worker topology refresh error', err instanceof Error ? err : null);
                        }
                        writeChannelWorkerPidfile();
                        if (reason === 'trust_reconfigured' &&
                            channelWorkerManager.workspaceActivity(runtimeToDrain.workspaceCwd) > 0) {
                            containmentErrors.push(new Error('Workspace channel workers are still active.'));
                        }
                    }
                    if (reason !== 'daemon_shutdown') {
                        const app = serveAppForRuntimeLifecycle.current ??
                            runtimeApp ??
                            runtimeAppForCleanup;
                        const stopWorkspaceGitStateForWorkspace = app?.locals?.['stopWorkspaceGitStateForWorkspace'];
                        const stopScheduledTaskKeepaliveForWorkspace = app?.locals?.['stopScheduledTaskKeepaliveForWorkspace'];
                        try {
                            stopWorkspaceGitStateForWorkspace?.(runtimeToDrain.workspaceCwd);
                        }
                        catch (err) {
                            daemonLog.error('workspace git-state cleanup error', err instanceof Error ? err : null);
                        }
                        try {
                            stopScheduledTaskKeepaliveForWorkspace?.(runtimeToDrain.workspaceCwd);
                        }
                        catch (err) {
                            daemonLog.error('workspace scheduled-task cleanup error', err instanceof Error ? err : null);
                        }
                    }
                    ...
                    try {
                        if (!shutdownBridges.has(runtimeToDrain.bridge)) {
                            try {
                                ...
                                try {
                                    ...
                                    daemonLog.warn('workspace bridge required forceful shutdown', {
                                    ...
                                    throw new AggregateError([shutdownError, killError], 'Workspace bridge shutdown could not be confirmed.');
                                }
                            }
                        }
                        bridgeStopped = true;
                        ...
                        if (bridgeStopped) {
                            ...
                            removeArrayValue(internalRuntimeBridgesForCleanup, runtimeToDrain.bridge);
                            shutdownBridges.add(runtimeToDrain.bridge);
                        }
                    }
                    if (containmentErrors.length > 0) {
                        throw new AggregateError(containmentErrors, 'Workspace runtime containment could not be confirmed.');
                    }
                })();
        ...
        const channelManagementService = (targetRuntime) => {
            const existing = channelManagementServices.get(targetRuntime);
            if (existing)
                return existing;
            const pending = (async () => {
                if (!ensureChannelWorkerManager) {
                    throw Object.assign(new Error('Channel worker manager is unavailable.'), { code: 'channel_worker_unavailable' });
                }
                const [{ createChannelManagementService }, { WorkspaceChannelSettingsStore },] = await Promise.all([
        ...
        const validateWorkspaceRuntimeForPublication = async (runtimeForPublication) => {
            let candidate = runtimeForPublication;
            for (let attempt = 0; attempt < 3; attempt++) {
                const snapshot = await trustPolicy.readDaemonTrustPolicySnapshot();
                const decision = trustPolicy.evaluateDaemonWorkspaceTrust(snapshot, candidate.workspaceCwd);
                ...
                await workspaceRuntimeRemoval.disposeRuntime(candidate, 'trust_reconfigured');
                if (attempt === 2) {
                    throw new Error('Workspace trust policy kept changing before runtime publication.');
                }
                candidate = await createDynamicWorkspaceRuntime(runtimeForPublication.workspaceCwd, {
        ...
        const app = runtime.createServeApp(opts, () => actualPort, {
            ...
            isChannelControlInitializing: () => channelWorkerManagerStarting !== undefined,
            ...
            deliverChannelMessage: async (workspaceCwd, request) => {
                ...
                return channelWorkerManager.deliverChannelMessage(workspaceCwd, request);
            },
            ...
            getPerfSnapshot: () => ({
                ...
                promptQueueWait: {
                    ...
                    meanMs: promptQueueWaitStats.count === 0
                        ? 0
            ...
            recordDaemonRequest: (durationMs, statusCode) => metricsRing.recordRequest(durationMs, statusCode),
            ...
            persistSetting: persistSettingFn,
            persistSettings: persistSettingsFn,
            sessionArtifactsPersistenceAvailable: sessionArtifactsPersistenceAvailableFromSettings(runtimeBootSettings?.merged),
            installAuthProvider: (req, assertGenerationOpen) => withSettingsLock(boundWorkspace, async () => {
                assertGenerationOpen?.();
        ...
        invalidatePrimaryServeFeaturesCache =
            app.locals.invalidateServeFeaturesCache ?? invalidatePrimaryServeFeaturesCache;
        ...
        app.locals.subSessionStoppers = subSessionStoppers;
        subSessionStoppers.push(subSessionLauncher.stop);
        subSessionStoppersByRuntime.set(workspaceRegistry.primary, subSessionLauncher.stop);
        if (workspaceTrustHotReloadAvailable) {
            const [{ createWorkspaceTrustReconciler }, { createDaemonTrustPolicyMonitor },] = await Promise.all([
                import('./workspace-trust-reconciler.js'),
                import('../config/daemon-trust-policy-monitor.js'),
            ]);
            const materializationFor = (entry, snapshot, decision) => {
                const boundWorkspaces = entry.primary && decision.targetTrusted
                    ? runtime.resolveBoundWorkspacesFromIdeEnv(entry.workspaceCwd, undefined, (workspace, index) => index === 0 ||
            ...
            const trustReconciler = createWorkspaceTrustReconciler({
                ...
                materializationKey: ({ entry, snapshot, decision }) => materializationFor(entry, snapshot, decision).key,
                isTrustDecrease: ({ runtime: current, nextMaterialization, decision, }) => {
                    if (current.trusted && !decision.targetTrusted)
                        return true;
                    if (!current.primary || !current.trusted)
                        return false;
                    try {
                        const previous = JSON.parse(current.trustMaterialization ?? '{}');
                        const next = JSON.parse(nextMaterialization);
                        if (!Array.isArray(previous.boundWorkspaces) ||
                            !Array.isArray(next.boundWorkspaces)) {
                            return true;
                        }
                        const nextRoots = new Set(next.boundWorkspaces.filter((value) => typeof value === 'string'));
                        return previous.boundWorkspaces.some((value) => typeof value !== 'string' || !nextRoots.has(value));
                    }
                    catch {
                        return true;
                    }
                },
                buildRuntime: async ({ entry, trusted, snapshot, decision, generationGuard, }) => {
                    const materialized = materializationFor(entry, snapshot, trusted === decision.targetTrusted
                        ? decision
                ...
                disposeRuntime: async (runtimeToDispose, reason) => {
                    const errors = [];
                    try {
                        await workspaceRuntimeRemoval.disposeRuntime(runtimeToDispose, reason);
                    }
                    catch (error) {
                        errors.push(error);
                    }
                    try {
                        ...
                        while ((handle?.getWorkspaceActivity(runtimeToDispose.workspaceId)
                            .memoryTasks ?? 0) > 0 &&
                            Date.now() < deadline) {
                            await new Promise((resolve) => setTimeout(resolve, 25));
                        }
                        if ((handle?.getWorkspaceActivity(runtimeToDispose.workspaceId)
                            .memoryTasks ?? 0) > 0) {
                            throw new Error('Workspace memory tasks did not stop after runtime disposal.');
                        }
                    }
                    catch (error) {
                        errors.push(error);
                    ...
                    if (errors.length === 1)
                        throw errors[0];
                    if (errors.length > 1) {
                        throw new AggregateError(errors, 'Workspace runtime and ACP cleanup failed.');
                    }
                },
                ...
                onError: (entry, error) => {
                    daemonLog.error(`workspace trust reconciliation failed for ${entry.workspaceCwd}`, error instanceof Error ? error : null);
                },
            });
            const trustMonitor = createDaemonTrustPolicyMonitor({
                onSnapshot: (snapshot) => {
                    latestTrustPolicySnapshot = snapshot;
                    return runWorkspaceTrustOperation(() => trustReconciler.reconcile(snapshot));
                },
                onError: (error) => {
                    daemonLog.error('workspace trust policy monitor failed', error instanceof Error ? error : null);
                },
            });
            app.locals.stopTrustPolicyMonitor = () => trustMonitor.stop();
            app.locals.requestTrustReconcile = () => trustMonitor.requestReconcile('manual');
            app.locals.waitForTrustPolicyIdle = () => runWorkspaceTrustOperation(async () => undefined);
            await trustMonitor.start();
        }
        const activePrimaryBridge = workspaceRegistry.primaryEntry.current?.runtime.bridge;
    ...
    const deferredChannelWebhookAuth = deferRuntimeUntilFirstHealth
        ? createDeferredChannelWebhookAuth(resolveChannelWebhookConfigSource, await loadChannelWebhookConfigRuntime(), daemonLog)
        : undefined;
    const app = runtimeApp ??
        createDelegatingServeApp(bootstrapApp, () => runtimeApp, {
            ...
            isPreAuthRequest: webShellMounted
                ? (req) => import('./web-shell-static.js').then((webShellStatic) => webShellStatic.isPreAuthWebShellRequest(req))
                : undefined,
        });
    ...
    // `LailatulCoder serve --hostname [::1]` would pass the loopback/token check
    ...
    if (opts.hostname.startsWith('[')) {
        ...
        if (!opts.hostname.endsWith(']') ||
            inner.length === 0 ||
            inner.includes(']')) {
            throw new Error(`Invalid --hostname "${opts.hostname}": brackets indicate an ` +
    ...
    if (opts.maxConnections !== undefined &&
        ...
        throw new TypeError(`Invalid maxConnections: ${opts.maxConnections}. Must be >= 0 ` +
            `(0 / Infinity = unlimited).`);
    }
    ...
    const resolveChannelWorkspaceGroupsAtListen = () => {
        ...
        const registry = (runtimeApp ?? runtimeAppForCleanup)?.locals?.['workspaceRegistry'];
        const resolveRuntime = (workspaceCwd) => {
            const runtime = registry?.getByWorkspaceCwd(workspaceCwd);
            if (registry && !runtime) {
                throw Object.assign(new Error(`Workspace "${workspaceCwd}" is unavailable.`), { code: 'workspace_unavailable' });
            }
            return runtime;
        };
        const resolveTrusted = (workspaceCwd, primary, runtime) => {
            if (runtime)
                return runtime.trusted;
            ...
            return channelValidationTrustPolicy.evaluateDaemonWorkspaceTrust(channelValidationTrustSnapshot, workspaceCwd).targetTrusted;
        };
        ...
        if (workspaceInputs.length === 1) {
            ...
            if (!trusted) {
                throw Object.assign(new Error(`Primary workspace "${workspace.cwd}" is not trusted; cannot host channels.`), { code: 'untrusted_workspace' });
            }
            const effectiveEnv = runtime
                ? workspaceRuntimeEffectiveEnv(runtime, daemonRuntimeBaseEnv)
                : channelValidationSettingsRuntime.environment.buildRuntimeEnvironment(channelValidationSettingsRuntime.settings.loadSettings(workspace.cwd, {
                    skipLoadEnvironment: true,
                    skipWorkspaceSettings: false,
                    workspaceTrusted: true,
                }).merged, workspace.cwd, daemonRuntimeBaseEnv, trusted).effectiveEnv;
        ...
        const workspaces = workspaceInputs.map((workspace, index) => {
            ...
            const settings = channelValidationSettingsRuntime.settings.loadSettings(workspace.cwd, {
                skipLoadEnvironment: true,
                skipWorkspaceSettings: !trusted,
                workspaceTrusted: trusted,
            });
            settingsByWorkspace.set(workspace.cwd, settings);
            channelWebhookEnvByWorkspace.set(workspace.cwd, runtime
                ? workspaceRuntimeEffectiveEnv(runtime, daemonRuntimeBaseEnv)
                : channelValidationSettingsRuntime.environment.buildRuntimeEnvironment(settings.merged, workspace.cwd, daemonRuntimeBaseEnv, trusted).effectiveEnv);
            return {
    ...
    return await new Promise((resolve, reject) => {
        ...
        const onListening = (error) => {
            ...
            startup.runQwenServeToListenMs = Math.round(performance.now() - runStartedAt);
            ...
            const liveDiscoveryOwners = [];
            const rememberLiveDiscoveryOwner = (owner) => {
                if (liveDiscoveryOwners.some((candidate) => candidate.runtimeBaseDir === owner.runtimeBaseDir &&
            ...
            let liveDiscoveryPublish;
            let liveDiscoveryRetryTimer;
            let liveDiscoveryRetryTask;
            let liveDiscoveryBootRetryApp;
            let liveDiscoveryEnabled = false;
            let liveDiscoveryShuttingDown = false;
            let liveDiscoveryToggle = Promise.resolve();
            let attemptPendingLiveDiscovery;
            ...
            const liveDiscoveryRetryDelayMs = deps.liveDiscoveryRetryDelayMs !== undefined &&
            ...
            const scheduleLiveDiscoveryRetry = () => {
                if (liveDiscoveryShuttingDown ||
            ...
            const cancelLiveDiscoveryRetry = () => {
                if (!liveDiscoveryRetryTimer)
                    return;
                clearTimeout(liveDiscoveryRetryTimer);
                liveDiscoveryRetryTimer = undefined;
            };
            const publishLiveDiscovery = (candidateApp) => {
                if (liveDiscoveryShuttingDown)
                    return Promise.resolve();
                if (!resolveAcpHttpEnabled())
                    return Promise.resolve();
                if (candidateApp.locals?.['liveVoiceEnabled'] !== true)
                    return Promise.resolve();
                liveDiscoveryEnabled = true;
                if (liveDiscoveryPublish)
                    return liveDiscoveryPublish;
                const coordinator = candidateApp.locals?.['liveCoordinator'];
                ...
                const publication = serveAppLifecycle
                    ...
                    .then(async ({ handoffLiveDiscoveryOwner, LiveDiscoveryOwnerActiveError, LiveDiscoveryPublicationError, removeLiveDiscoveryFile, writeLiveDiscoveryFile, }) => {
                    if (liveDiscoveryShuttingDown || !liveDiscoveryEnabled)
                        return;
                    liveDiscoveryBootRetryApp = undefined;
                    ...
                    attemptPendingLiveDiscovery = async () => {
                        ...
                        const rollbackPublished = async () => {
                            for (const owner of published.splice(0)) {
                                try {
                                    ...
                                    daemonLog.warn(`failed to roll back Live Host discovery at ${owner.runtimeBaseDir}: ${cleanupError instanceof Error
                        ...
                        for (const runtimeBaseDir of targets) {
                            ...
                            try {
                                if (runtimeBaseDir !== stableBaseDir) {
                                    await handoffLiveDiscoveryOwner(runtimeBaseDir, record, async () => undefined);
                                }
                                ...
                                if (err instanceof LiveDiscoveryPublicationError &&
                                    err.published) {
                                    published.push({
                                        runtimeBaseDir,
                                        instanceNonce,
                                ...
                                if (err instanceof LiveDiscoveryOwnerActiveError) {
                                    if (!warnedLiveDiscoveryOwners.has(runtimeBaseDir)) {
                                        ...
                                        daemonLog.warn(`failed to publish Live Host discovery at ${runtimeBaseDir}: ${err.message}`);
                                    }
                                    return;
                                }
                                daemonLog.warn(`failed to publish Live Host discovery at ${runtimeBaseDir}: ${err instanceof Error ? err.message : String(err)}`);
                                return;
                            }
                        }
                    ...
                    if (!publicationRetryable || !warnedLiveDiscoveryBootFailure) {
                        ...
                        daemonLog.warn(`failed to publish Live Host discovery: ${err instanceof Error ? err.message : String(err)}`);
                    }
                });
            ...
            const removeLiveDiscoveryOwners = async () => {
                const owners = [...liveDiscoveryOwners];
                if (owners.length === 0)
                    return;
                let removeLiveDiscoveryFile;
                try {
                ...
                for (const owner of owners) {
                    try {
                        ...
                        if (index >= 0)
                            liveDiscoveryOwners.splice(index, 1);
                    }
                    catch (err) {
                        errors.push(err);
                    }
                ...
                if (errors.length > 0) {
                    throw new AggregateError(errors, 'Live Host discovery cleanup is incomplete.');
                }
            };
            const unpublishLiveDiscovery = async () => {
            ...
            const attachLiveDiscoveryControl = (candidateApp) => {
                candidateApp.locals.setLiveDiscoveryEnabled = (enabled) => {
                    const operation = liveDiscoveryToggle.then(() => enabled
                        ? publishLiveDiscovery(candidateApp)
                        : unpublishLiveDiscovery());
                    liveDiscoveryToggle = operation.catch(() => undefined);
                    return operation;
                };
                candidateApp.locals.onConversationRuntimeReady = () => {
                    void publishLiveDiscovery(candidateApp);
                };
            };
            const cleanupLiveDiscovery = async () => {
            ...
            const runtimeStartupTimeoutMs = resolveRuntimeStartupTimeoutMs(deps.runtimeStartupTimeoutMs);
            const clearRuntimeStartupTimer = () => {
                if (!runtimeStartupTimer)
                    return;
                clearTimeout(runtimeStartupTimer);
                runtimeStartupTimer = undefined;
            };
            const clearRuntimeStartFallbackTimer = () => {
                if (!runtimeStartFallbackTimer)
                    return;
                clearTimeout(runtimeStartFallbackTimer);
                runtimeStartFallbackTimer = undefined;
            };
            const clearRuntimeStartAfterHealthTimer = () => {
                if (!runtimeStartAfterHealthTimer)
                    return;
                clearTimeout(runtimeStartAfterHealthTimer);
                runtimeStartAfterHealthTimer = undefined;
            };
            const cancelDeferredRuntimeStartup = () => {
                if (!deferRuntimeUntilFirstHealth ||
                    runtimeStarting ||
                    runtimeStartupSettled)
                    return;
                daemonLog.info('deferred runtime: cancelled, server closed before startup');
                runtimeStartupSettled = true;
                const error = new Error(RUNTIME_STARTUP_CANCELLED_MESSAGE);
                runtimeStartupError = error.message;
                markRuntimeFailed(error);
            };
            const shutdownBridgeAfterFailedStartup = async (bridge) => {
                if (!bridge || deps.bridge)
                    return;
                if (shutdownBridges.has(bridge))
                    return;
                ...
                try {
                    ...
                    daemonLog.error('bridge shutdown after runtime startup error failed', shutdownErr instanceof Error ? shutdownErr : null);
                }
                finally {
                    if (bridgeRef === bridge) {
                        bridgeRef = undefined;
            ...
            const stopChannelWorkerAfterFailedStartup = async () => {
                if (!channelWorkerManager)
                    return true;
                try {
                    ...
                    daemonLog.error('channel worker stop after runtime startup error failed', stopErr instanceof Error ? stopErr : null);
                    return false;
                }
            };
            const failRuntimeStartup = async (err, bridgeForCleanup) => {
                ...
                if (startup.preheat.status === 'scheduled' ||
                    startup.preheat.status === 'running') {
                    startup.preheat.status = 'failed';
                    startup.preheat.error = message;
                }
                writeStderrLine(`LailatulCoder serve: runtime startup failed: ${message}`);
                daemonLog.error('runtime startup failed', error);
                markRuntimeFailed(error);
                if (closeServerAfterChannelWorkerStartupFailure && server.listening) {
                    server.close((closeErr) => {
                        if (closeErr) {
                            daemonLog.error('server close after runtime startup error failed', closeErr);
                        }
                    });
                    server.closeAllConnections();
                }
                const channelWorkerStopped = await stopChannelWorkerAfterFailedStartup();
                disposeDaemonEventLoopMonitor();
                if (channelWorkerStopped)
            ...
            const armRuntimeStartupTimer = () => {
                if (runtimeStartupTimeoutMs <= 0 || runtimeStartupTimer)
                    return;
                runtimeStartupTimer = setTimeout(() => {
                    void failRuntimeStartup(new Error(`Daemon runtime startup timed out after ${runtimeStartupTimeoutMs}ms.`));
                }, runtimeStartupTimeoutMs);
                runtimeStartupTimer.unref();
            };
            const resolveRuntimeChannelGroups = async (channelSelection, candidateApp, operation) => {
                const registry = candidateApp.locals?.['workspaceRegistry'];
                if (!registry) {
                    throw new Error('Workspace registry is not available for channel workers.');
                }
                ...
                if (runtimes.length <= 1 && operation === 'initial') {
                    ...
                    if (!primary.trusted) {
                        throw Object.assign(new Error(`Primary workspace "${primary.workspaceCwd}" is not trusted; cannot host channels.`), { code: 'untrusted_workspace' });
                    }
                    return [
                        {
                ...
                const grouping = resolveChannelWorkspaceGroups({
                    workspaces: runtimes.map((runtime) => {
                        const settings = settingsRuntime.settings.loadSettings(runtime.workspaceCwd, {
                            skipLoadEnvironment: true,
            ...
            ensureChannelWorkerManager = () => {
                if (channelWorkerManager)
                    return Promise.resolve(channelWorkerManager);
                if (channelWorkerManagerStarting)
                    return channelWorkerManagerStarting;
                const starting = (async () => {
                    const candidateApp = runtimeApp ?? runtimeAppForCleanup;
                    const registry = candidateApp?.locals?.['workspaceRegistry'];
                    if (!candidateApp || !registry) {
                        throw new Error('Workspace registry is not available for channels.');
                    }
                    const workerRuntime = await ensureChannelRuntime();
                    const createSupervisor = deps.channelWorkerSupervisorFactory ??
                        workerRuntime.createChannelWorkerSupervisor;
                    const createGroup = (groups) => workerRuntime.createChannelWorkerGroup({
                        groups,
                        registry,
                        createSupervisor,
                        shared: {
                        ...
                        onExit: (snapshot) => {
                            const workspacePrefix = workspaceInputs.length > 1
                                ? `workspace=${snapshot.workspaceCwd}, `
                                : '';
                            daemonLog.warn(`channel worker exited (${workspacePrefix}state=${snapshot.state}, pid=${snapshot.pid ?? 'unknown'}, ` +
                        ...
                        onLog: ({ stream, line, workspaceCwd }) => {
                            const message = workspaceInputs.length > 1
                    ...
                    channelWorkerManager = workerRuntime.createChannelWorkerManager({
                        resolveGroups: (selection, operation) => resolveRuntimeChannelGroups(selection, candidateApp, operation),
                        createGroup,
            ...
            const completeRuntimeStartup = async (candidateApp) => {
                if (runtimeStartupSettled)
                    return;
                runtimeApp = candidateApp;
                attachLiveDiscoveryControl(candidateApp);
                const acpHandle = candidateApp.locals?.['acpHandle'];
            ...
            const startBridgePreheat = (bridge) => {
                startup.preheat.status = 'running';
                ...
                bridge
                    ...
                    startup.preheat.durationMs = Math.round(performance.now() - preheatStartedAt);
                })
                    .catch((err) => {
                    ...
                    startup.preheat.durationMs = Math.round(performance.now() - preheatStartedAt);
                    startup.preheat.error = message;
                    writeStderrLine(`LailatulCoder serve: ACP preheat failed, will retry on first session: ${message}`);
                });
            };
            const startRuntime = () => {
                if (runtimeStarting)
                ...
                runtimeStarting = buildRuntime()
                    ...
                    runtimeAppForCleanup = runtime.app;
                    if (shuttingDown) {
                        await failRuntimeStartup(new Error('Daemon runtime stopped before mounting.'), runtime.bridge);
                        return;
                    }
            ...
            const scheduleRuntimeStartFallback = () => {
                if (shuttingDown || runtimeStarting || runtimeStartFallbackTimer)
                    return;
                daemonLog.info(`deferred runtime: scheduling fallback start in ${FAST_PATH_RUNTIME_START_FALLBACK_MS}ms`);
            ...
            startRuntimeAfterHealth = () => {
                ...
                clearRuntimeStartFallbackTimer();
                daemonLog.info(`deferred runtime: health served, scheduling start in ${FAST_PATH_RUNTIME_START_AFTER_HEALTH_MS}ms`);
            ...
            const onSignal = async (signal) => {
                if (shuttingDown) {
                    ...
                    //
                    // Synchronously SIGKILL every live `LailatulCoder --acp`
                    ...
                    // `LailatulCoder` processes in the operator's `ps` output.
                    ...
                    try {
                        ...
                        daemonLog.error('force-kill error', err instanceof Error ? err : null);
                    }
                    await flushDaemonLogBounded(daemonLog, DAEMON_LOG_FORCED_FLUSH_BUDGET_MS);
                    process.exit(1);
                    return;
                }
                if (!loggerPublished) {
                ...
                try {
                    ...
                    if (hasRetryableChannelWorkerShutdownError(err)) {
                        daemonLog.error('refusing to exit while a channel worker or service lease remains; signal again to retry after the child exits (another signal during that retry forces exit)');
                        return;
                    }
                    await flushDaemonLogBounded(daemonLog, DAEMON_LOG_FORCED_FLUSH_BUDGET_MS);
                    process.exit(1);
            ...
            const handle = {
                ...
                resolvedToken: token,
                runtimeReady,
                getLocalControl: () => (runtimeApp ?? runtimeAppForCleanup)?.locals?.['localControlService'],
                close: () => {
                    ...
                    closePromise = new Promise((res, rej) => {
                        ...
                        const initiallyMountedManagement = initiallyMountedApp?.locals?.['workspaceManagementHandle'];
                        const initiallyMountedSessionMaintenance = initiallyMountedApp
                            ?.locals?.['sessionArchiveCoordinator'];
                        const initiallyMountedConversationActivity = initiallyMountedApp
                            ?.locals?.['conversationRuntimeActivity'];
                        ...
                        const initialManagementWait = initiallyMountedManagement?.sealAndWait?.();
                        const initiallyMountedLive = initiallyMountedApp?.locals;
                        const initialLiveWait = initiallyMountedLive?.sealAndWaitLiveCoordinator?.();
                        const initialSessionMaintenanceWait = initiallyMountedSessionMaintenance?.sealMaintenanceAndWait?.();
                        const initialConversationActivityWait = initiallyMountedConversationActivity?.sealAndWait?.();
                        let processRegistryShutdown;
                        const startProcessRegistryShutdown = () => {
                            processRegistryShutdown ??= managedProcessRegistry
                                ?.shutdown()
                                .then(() => undefined, (error) => error instanceof Error ? error : new Error(String(error)));
                        };
                        ...
                        let channelWorkerShutdownError;
                        const finish = (err) => {
                            if (settled)
                                return;
                            settled = true;
                            const accessLogController = (runtimeApp ?? runtimeAppForCleanup)?.locals?.[ACCESS_LOG_CONTROLLER_LOCAL];
                            accessLogController?.sealAndFlushSuppressed();
                            const preserveSignalHandlers = channelWorkerShutdownError !== undefined &&
                            ...
                            void (coreRuntimePromise
                                ...
                                : Promise.resolve())
                                .catch((telemetryErr) => {
                                writeStderrLine(`LailatulCoder serve: telemetry shutdown error: ${telemetryErr instanceof Error
                                ...
                                let finalErr = err ?? bridgeShutdownError ?? channelWorkerShutdownError;
                                const retryableChannelClose = channelWorkerShutdownError !== undefined &&
                                    channelWorkerManager?.state().enabled === true;
                                ...
                                try {
                                    ...
                                    const normalizedCleanupError = cleanupError instanceof Error
                                        ? cleanupError
                                        : new Error(String(cleanupError));
                                    if (finalErr) {
                                        writeDaemonLifecycleBestEffort(() => {
                                            daemonLog.error('Live Host discovery cleanup failed during shutdown', normalizedCleanupError);
                                        });
                                    }
                                ...
                                if (loggerPublished || loggerSignalOwned) {
                                    ...
                                    clearLoaderKeyRejectionReporterIfCurrent(loaderRejectionReporter);
                        ...
                        void (coreRuntimePromise
                            ...
                            : Promise.resolve()).catch((flushErr) => {
                            daemonLog.warn(`pre-shutdown metrics flush failed: ${flushErr instanceof Error
                                ? flushErr.message
                                : String(flushErr)}`);
                        });
                        Promise.resolve()
                            .then(async () => {
                            await waitForRuntimeStartingForShutdown(runtimeStarting, daemonLog);
                            const appForCleanup = runtimeApp ?? runtimeAppForCleanup;
                            const workspaceManagementHandle = appForCleanup?.locals?.['workspaceManagementHandle'];
                            const sessionMaintenance = appForCleanup?.locals?.['sessionArchiveCoordinator'];
                            const conversationActivity = appForCleanup?.locals?.['conversationRuntimeActivity'];
                            ...
                            const liveLifecycleHandle = appForCleanup?.locals;
                            ...
                            if (conversationActivity !== initiallyMountedConversationActivity) {
                                await conversationActivity?.sealAndWait?.();
                            }
                            stopTrustPolicyMonitor(appForCleanup);
                            const waitForTrustPolicyIdle = appForCleanup?.locals?.['waitForTrustPolicyIdle'];
                            await waitForTrustPolicyIdle?.().catch((err) => {
                                daemonLog.error('workspace trust reconciliation shutdown wait failed', err instanceof Error ? err : null);
                            });
                            startProcessRegistryShutdown();
                            ...
                            if (channelWorkerManager) {
                                await channelWorkerManager.shutdown().catch((err) => {
                                    daemonLog.error('channel worker stop error', err instanceof Error ? err : null);
                            ...
                            const runtimeRemoval = appForCleanup?.locals?.['workspaceRuntimeRemoval'];
                            const workspaceRegistry = appForCleanup?.locals?.['workspaceRegistry'];
                            const managedRuntimeBridges = new Set();
                            if (runtimeRemoval && workspaceRegistry) {
                                ...
                                await Promise.all(managedRuntimes.map((workspaceRuntime) => runtimeRemoval
                                    .disposeRuntime(workspaceRuntime, 'daemon_shutdown')
                                    .catch((err) => {
                                    daemonLog.error('workspace runtime shutdown error', err instanceof Error ? err : null);
                                    bridgeShutdownError =
                                        err instanceof Error ? err : new Error(String(err));
                                    try {
                            ...
                            for (const bridgeForShutdown of getRuntimeBridgesForCleanup()) {
                                if (managedRuntimeBridges.has(bridgeForShutdown))
                                    continue;
                                if (shutdownBridges.has(bridgeForShutdown))
                                    continue;
                                shutdownBridges.add(bridgeForShutdown);
                                await bridgeForShutdown.shutdown().catch((err) => {
                                    shutdownBridges.delete(bridgeForShutdown);
                                    daemonLog.error('bridge shutdown error', err instanceof Error ? err : null);
                                    bridgeShutdownError =
                                        err instanceof Error ? err : new Error(String(err));
                                });
                            }
                            ...
                            if (processRegistryError) {
                                daemonLog.error('ACP process registry shutdown error', processRegistryError);
            ...
            try {
                ...
                const error = err instanceof Error ? err : new Error(String(err));
                markServeAppStartupFailed(error);
                void serveAppLifecycle.close().then(() => reject(error), (closeError) => reject(closeError instanceof Error
            ...
            if (channelWorkspaceGroups) {
                for (const group of channelWorkspaceGroups) {
                    daemonLog.info('channel worker group assigned', {
                        ...
                        channels: group.selection.mode === 'all' ? ['all'] : group.selection.names,
                    });
                }
                if (opts.channelSelection?.mode === 'all') {
                    writeStderrLine('LailatulCoder serve: --channel all is primary-workspace only; non-primary workspace channels are not hosted.');
                }
            }
            writeStdoutLine(`LailatulCoder serve listening on ${url} (mode=${opts.mode}, ` +
                `workspace=${boundWorkspace})`);
            ...
            writeStderrLine(`LailatulCoder serve: bound to workspace ${JSON.stringify(boundWorkspace)}`);
            writeStderrLine(`LailatulCoder serve: startup timing: processToListenMs=${startup.processToListenMs} ` +
                `runQwenServeToListenMs=${startup.runQwenServeToListenMs}`);
            if (!token) {
                writeStderrLine(`LailatulCoder serve: bearer auth disabled (loopback default). Set ${QWEN_SERVER_TOKEN_ENV} to enable.`);
                if (opts.clientMcpOverWs === true) {
                    writeStderrLine(`LailatulCoder serve: client-hosted MCP tools are accepted over the WebSocket without auth. ` +
                ...
                writeStderrLine('LailatulCoder serve: --require-auth enabled (bearer token mandatory ' +
            ...
            if (preparedRuntimeApp && bridgeRef && deps.bridge) {
                ...
                if (opts.channelSelection && !runtimeStartupSettled) {
                    armRuntimeStartupTimer();
                    runtimeStarting = completeRuntimeStartup(preparedRuntimeApp).catch((err) => failRuntimeStartup(err, bridgeRef));
                }
                else {
                    const acpHandle = preparedRuntimeApp.locals?.['acpHandle'];
                    acpHandle?.attachServer?.(server);
                    markServeAppStartupReady();
            ...
            if (deps.resolveOnListen) {
                ...
                void (liveDiscoveryPublish ?? Promise.resolve()).then(() => resolve(handle));
            }
            else {
                void runtimeReady.then(async () => {
                    ...
                    void handle.close().then(() => {
                        ...
                        if (hasRetryableChannelWorkerShutdownError(closeErr)) {
                            writeDaemonLifecycleBestEffort(() => daemonLog.error('runtime startup failed, but LailatulCoder serve remains alive to retain the channel service lease until worker exit is confirmed'));
                            return;
                        }
                        reject(err instanceof Error ? err : new Error(String(err)));
                    });
        ...
        if (tlsOptions) {
            try {
                ...
                reject(new Error(`--tls-cert "${opts.tlsCert}" and --tls-key "${opts.tlsKey}" ` +
        ...
        const tryListen = (attemptPort, attempt) => {
            ...
            const handleError = (err) => {
                ...
                if (err.code === 'EADDRINUSE' &&
                    ...
                    writeStderrLine(`LailatulCoder serve: port ${attemptPort} is in use, trying ${nextPort}...`);
                    tryListen(nextPort, attempt + 1);
                    return;
                }
                if (err.code === 'EADDRINUSE' && attempt > 0) {
                    writeStderrLine(`LailatulCoder serve: all ports ${opts.port}–${attemptPort} are in use`);
                }
                removeCurrentServePidfile();
                markServeAppStartupFailed(err);
                void serveAppLifecycle.close().then(() => reject(err), (closeError) => reject(closeError instanceof Error
...
//# sourceMappingURL=run-LailatulCoder-serve.js.map