...
import { resolvePath } from '@lailatul-coder/lailatul-coder-core';
    ...
    action: async (context, args) => {
        ...
        if (isFolderTrustEnabled(context.services.settings.merged)) {
            ...
            if (trustedFolders.isPathTrusted(realTargetPath) !== true) {
                ...
                if (context.overwriteConfirmed &&
                    ...
                    return {
                        type: 'confirm_action',
                        prompt: `Move this session to ${realTargetPath}? LailatulCoder Ai will be able to read, edit, and execute files there. This folder will be trusted for future sessions.`,
                        originalInvocation: {
                            raw: rawInvocation,
                        },
                    };
        ...
        if (trustedTargetPath) {
            try {
                ...
                warnings.push(`Trust setting update failed: ${error instanceof Error ? error.message : String(error)}`);
            }
        }
        try {
            await config
