 ...
 * `LailatulCoder serve` wires this into `BridgeOptions.statusProvider` so the
...
/**
 * Construct the production `DaemonStatusProvider` for `LailatulCoder serve`.
 ...
 * fine).
 */
export function createDaemonStatusProvider(options = {}) {
    ...
    return {
        ...
        async getDaemonPreflightCells(boundWorkspace) {
 ...
 */
async function buildDaemonPreflightCells(boundWorkspace, env) {
    ...
    const cliEntryCell = () => {
        ...
        return {
            ...
            hint: 'Set QWEN_CLI_ENTRY to the absolute path of the LailatulCoder entry script.',
            locality: 'daemon',
        };
    };
    const workspaceDirCell = async () => {
        try {
    ...
    const slots = [
        ...
        {
            kind: 'git',
            run: () => safeCheck('git', async () => {
                const v = await getGitVersion();
                return v && v !== 'unknown'
                    ? { status: 'ok', detail: { version: v } }
                    : { status: 'warning', hint: 'git not found on PATH.' };
            }),
        },
        {
            kind: 'npm',
            run: () => safeCheck('npm', async () => {
                const v = await getNpmVersion();
                return v && v !== 'unknown'
                    ? { status: 'ok', detail: { version: v } }
                    : { status: 'warning', hint: 'npm not found on PATH.' };
            }),
        },
    ];
    ...
    const settled = await Promise.allSettled(slots.map((s) => Promise.resolve().then(s.run)));
...
async function safeCheck(kind, body) {
    try {
        const r = await body();
        return {
            kind,
...
//# sourceMappingURL=daemon-status-provider.js.map