...
const ghMock = vi.hoisted(() => vi.fn((_payload, ..._rest) => ''));
const ghViewMock = vi.hoisted(() => vi.fn((..._args) => ''));
...
const writeStdoutSpy = vi.hoisted(() => vi.fn((_line) => { }));
const writeStderrSpy = vi.hoisted(() => vi.fn((_line) => { }));
...
// runSubmit directly.
const reviewSettingsMock = vi.hoisted(() => vi.fn(() => ({ attribution: true })));
...
function args(over = {}) {
    return {
        ...
        skillArgs: file(`skill-args-${seq++}.txt`, 'https://github.com/LailatulCoder/lailatul-coder/pull/6771'),
...
describe('authorization — URL-shaped host and repo binding at the submit call site', () => {
    ...
    function recoverFloor(line, opts = {}) {
    ...
    it('recovers the recorded floor only when the record DECIDED one', () => {
        ...
        expect(recoverFloor('123 --comment', { defaultSeverityFloor: 'critical' })).toEqual({ floor: 'critical', source: 'configured' });
        expect(recoverFloor('123 --comment', { defaultSeverityFloor: 'crtical' })).toBeUndefined();
    ...
    it('binds the recovery to the full recorded identity — number, repo, host', () => {
        ...
        expect(recoverFloor('https://github.com/o/r/pull/123 --severity-floor critical', {
            callerRepo: 'o/r',
        })).toEqual({ floor: 'critical', source: 'explicit' });
        expect(recoverFloor('https://github.com/other/repo/pull/123 --severity-floor critical', { callerRepo: 'o/r' })).toBeUndefined();
        ...
        expect(recoverFloor('https://github.com/other/repo/pull/123 --severity-floor critical', {})).toBeUndefined();
        expect(recoverFloor('https://ghe.corp.example/o/r/pull/123 --severity-floor critical', { callerRepo: 'o/r' })).toBeUndefined();
        ...
        expect(recoverFloor('456 --severity-floor critical', {
            planPath: floorPlan({ prNumber: 123 }),
            callerPr: 456,
        })).toEqual({ floor: 'critical', source: 'explicit' });
        expect(recoverFloor('123 --severity-floor critical', {
            planPath: floorPlan({ prNumber: 123 }),
            callerPr: 456,
        })).toBeUndefined();
        ...
        expect(recoverFloor('https://github.com/other/repo/pull/123 --severity-floor critical', { planPath: floorPlan({ prNumber: 123, ownerRepo: 'o/r' }) })).toBeUndefined();
        // Absent record and plan-less caller both fail open, never throw.
        expect(recordedSeverityFloor({
            callerPr: 123,
            skillArgs: join(dir, 'no-such-record.txt'),
        })).toBeUndefined();
        expect(recordedSeverityFloor({ skillArgs: join(dir, 'no-such-record.txt') })).toBeUndefined();
        ...
        expect(recoverFloor('https://github.com/o/r/pull/999 --severity-floor critical', {
            callerRepo: 'o/r',
        })).toBeUndefined();
        // The caller's host recovers a matching Enterprise record…
        expect(recoverFloor('https://ghe.corp.example/o/r/pull/123 --severity-floor critical', { callerRepo: 'o/r', callerHost: 'ghe.corp.example' })).toEqual({ floor: 'critical', source: 'explicit' });
        ...
        expect(recoverFloor('https://code.alibaba-inc.com/o/r/codereview/123 --severity-floor critical', { callerRepo: 'o/r', callerHost: 'gitlab.alibaba-inc.com' })).toEqual({ floor: 'critical', source: 'explicit' });
        // A genuinely different host still recovers nothing.
        expect(recoverFloor('https://code.alibaba-inc.com/o/r/codereview/123 --severity-floor critical', { callerRepo: 'o/r', callerHost: 'github.com' })).toBeUndefined();
        ...
        expect(recoverFloor('https://github.com/other/repo/pull/123 --severity-floor critical', {
        ...
        expect(recoverFloor('https://ghe.corp.example/o/r/pull/123 --severity-floor critical', {
            planPath: floorPlan({
                prNumber: 123,
                ownerRepo: 'o/r',
    ...
    it('ignores the skillArgs seam whenever a session id is present', () => {
        ...
        try {
            expect(recordedSeverityFloor({ callerPr: 123, skillArgs: argsFile })).toBeUndefined();
        }
        finally {
            if (prev === undefined)
                delete process.env['QWEN_CODE_SESSION_ID'];
            else
                process.env['QWEN_CODE_SESSION_ID'] = prev;
        }
    });
    ...
    it('binds the host in both directions, defaulting an absent host to github.com', () => {
        ...
        expect(authFor('https://ghe.corp.example/o/r/pull/123 --comment', {
            host: 'ghe.corp.example',
        }).ok).toBe(true);
    });
    ...
    it('a requested-but-unbindable comment names the missing PR, not a missing flag', () => {
        ...
        expect(bySetting.why).not.toContain('`--comment` was not in the review arguments');
        ...
        expect(neither.why).toContain('`--comment` was not in the review arguments');
    });
    it('a missing args file names the missing invocation, not a missing flag, when the setting authorises', () => {
        ...
        expect(bySetting.ok).toBe(false);
        expect(bySetting.why).toContain('no recorded invocation names a pull request');
        ...
        expect(byFlagExplicit.why).toContain('cannot show that `--comment` was requested');
    });
    it('surfaces the recorded host on the --user-authorized fast path too', () => {
        ...
        const auth = authFor('https://code.alibaba-inc.com/g/p/codereview/123 --comment', { userAuthorized: true, repo: 'g/p' });
        expect(auth.ok).toBe(true);
        expect(auth.recordedHost).toBe('code.alibaba-inc.com');
        // A different-repo recording of the same number binds nothing.
        expect(authFor('https://code.alibaba-inc.com/g/p/codereview/123 --comment', {
            userAuthorized: true,
        }).recordedHost).toBeUndefined();
        ...
        // gate).
        expect(authFor('123 --host code.alibaba-inc.com --comment', {
            userAuthorized: true,
        }).recordedHost).toBe('code.alibaba-inc.com');
        ...
        expect(authFor('123 --host github.com --comment', { userAuthorized: true })
            .recordedHost).toBe('github.com');
        ...
        expect(authFor('https://code.alibaba-inc.com/O/R/codereview/123 --comment', {
            userAuthorized: true,
            repo: 'o/r',
        }).recordedHost).toBe('code.alibaba-inc.com');
        expect(authFor('https://code.alibaba-inc.com/o/r/codereview/123 --comment', {
        ...
        expect(reviewWriteAuthorization({
            userAuthorized: true,
...
describe('the user-authorized fast path binds a recorded Aone target (round-6 witness)', () => {
    ...
    it('posts a recorded Aone target through a1, never gh', () => {
        const skillArgs = file('fast-path-aone.txt', 'https://code.alibaba-inc.com/g/p/codereview/123 --comment\n');
        expect(() => runSubmit(args({ skillArgs, userAuthorized: true, pr: 123, repo: 'g/p' }), 'unknown', { defaultComment: false })).not.toThrow();
...
describe('the user-authorized fast path binds the recorded host cross-session', () => {
    ...
    beforeEach(() => {
        ...
        writeFileSync(siblingFile, 'https://code.alibaba-inc.com/maxcompute/odps_src/codereview/42 --comment\n', 'utf8');
    });
    afterEach(() => {
        rmSync(siblingDir, { recursive: true, force: true });
    });
    it('routes at a1 when a SIBLING session recorded the same PR on Aone', () => {
        expect(() => runSubmit(args({ userAuthorized: true, pr: 42, repo: 'maxcompute/odps_src' }), 'unknown', { defaultComment: false })).not.toThrow();
        expect(process.exitCode).toBeUndefined();
        expect(aoneSubmitMock).toHaveBeenCalledTimes(1);
        expect(aoneSubmitMock.mock.calls[0][0]).toMatchObject({
            prNumber: 42,
    ...
    it('does not bind a sibling recording of a DIFFERENT PR', () => {
        ...
        expect(() => runSubmit(args({ userAuthorized: true, pr: 999, repo: 'maxcompute/odps_src' }), 'unknown', { defaultComment: false })).not.toThrow();
        expect(process.exitCode).toBe(3);
        expect(JSON.parse(writeStdoutSpy.mock.calls.map((c) => String(c[0])).join(''))).toEqual({ posted: false, reason: 'target-platform-unbound' });
    ...
    it('binds the repo too — a different-repo same-number recording supplies nothing', () => {
        ...
        // would post at Aone instead of refusing.
        writeFileSync(siblingFile, 'https://code.alibaba-inc.com/other/repo/codereview/42 --comment\n', 'utf8');
        expect(() => runSubmit(args({ userAuthorized: true, pr: 42, repo: 'maxcompute/odps_src' }), 'unknown', { defaultComment: false })).not.toThrow();
        expect(process.exitCode).toBe(3);
        expect(JSON.parse(writeStdoutSpy.mock.calls.map((c) => String(c[0])).join(''))).toEqual({ posted: false, reason: 'target-platform-unbound' });
        expect(aoneSubmitMock).not.toHaveBeenCalled();
    ...
    it('FAILS CLOSED on a bare-number recording with no host evidence', () => {
        ...
        expect(() => runSubmit(args({ userAuthorized: true, pr: 42, repo: 'maxcompute/odps_src' }), 'unknown', { defaultComment: false })).not.toThrow();
        expect(process.exitCode).toBe(3);
        const out = JSON.parse(writeStdoutSpy.mock.calls.map((c) => String(c[0])).join(''));
    ...
    it('FAILS CLOSED when NO recording exists at all (fast path, no host evidence)', () => {
        ...
        expect(() => runSubmit(args({
            userAuthorized: true,
            skillArgs: join(dir, 'no-recording-anywhere.txt'),
        }), 'unknown', { defaultComment: false })).not.toThrow();
        expect(process.exitCode).toBe(3);
        const out = JSON.parse(writeStdoutSpy.mock.calls.map((c) => String(c[0])).join(''));
        expect(out).toEqual({ posted: false, reason: 'target-platform-unbound' });
        expect(ghMock).not.toHaveBeenCalled();
    ...
    it('a bare-number recording WITH a recorded --host binds the platform', () => {
        ...
        aoneSubmitMock.mockClear();
        writeStdoutSpy.mockClear();
        writeFileSync(siblingFile, '42 --host gitlab.alibaba-inc.com --comment\n', 'utf8');
        expect(() => runSubmit(args({ userAuthorized: true, pr: 42, repo: 'maxcompute/odps_src' }), 'unknown', { defaultComment: false })).not.toThrow();
        expect(process.exitCode).toBeUndefined();
        expect(ghMock).not.toHaveBeenCalled();
        expect(aoneSubmitMock).toHaveBeenCalledTimes(1);
    ...
    it('a codereview-URL recording posts with the ALIASED git host (web vs git name of one platform)', () => {
        ...
        const rec = file('aone-url-slow.txt', 'https://code.alibaba-inc.com/maxcompute/odps_src/codereview/42 --comment');
        expect(() => runSubmit(args({
            skillArgs: rec,
            userAuthorized: false,
            pr: 42,
    ...
    it('a contradicting --host beside a recorded host refuses — the flag does not retarget the recorded review', () => {
        ...
        expect(process.exitCode).toBe(3);
        expect(JSON.parse(writeStdoutSpy.mock.calls.map((c) => String(c[0])).join(''))).toEqual({ posted: false, reason: 'target-platform-conflict' });
        expect(aoneSubmitMock).not.toHaveBeenCalled();
        expect(ghMock).not.toHaveBeenCalled();
    ...
    it('the cross-session scan is last-writer-wins by the FILE mtime, not name order and not the directory mtime', () => {
        ...
        try {
            ...
            expect(() => runSubmit(args({ userAuthorized: true, pr: 7, repo: 'maxcompute/odps_src' }), 'unknown', { defaultComment: false })).not.toThrow();
            expect(process.exitCode).toBe(3);
            expect(JSON.parse(writeStdoutSpy.mock.calls.map((c) => String(c[0])).join(''))).toEqual({ posted: false, reason: 'target-platform-unbound' });
            expect(aoneSubmitMock).not.toHaveBeenCalled();
            expect(ghMock).not.toHaveBeenCalled();
    ...
    it('a HOSTLESS recording read via the --skill-args seam refuses — the cwd probe must not stand in for the record of another cwd', () => {
        ...
        expect(() => runSubmit(args({ skillArgs: rec, pr: 7, repo: 'maxcompute/odps_src' }), 'unknown', { defaultComment: false })).not.toThrow();
        expect(process.exitCode).toBe(3);
        expect(JSON.parse(writeStdoutSpy.mock.calls.map((c) => String(c[0])).join(''))).toEqual({ posted: false, reason: 'target-platform-unbound' });
        expect(writeStderrSpy.mock.calls.some((c) => String(c[0]).includes('--skill-args'))).toBe(true);
        expect(aoneSubmitMock).not.toHaveBeenCalled();
        expect(ghMock).not.toHaveBeenCalled();
    ...
    it('never reads recordings planted OUTSIDE session dirs (worktree vector)', () => {
        ...
        writeFileSync(join(plantedDir, 'qwen-skill-args-review.txt'), 'https://code.alibaba-inc.com/maxcompute/odps_src/codereview/42 --comment\n', 'utf8');
...
describe('the posting gate', () => {
    ...
    it('matches the refusal advice to the refusal class', () => {
        ...
        const advice = () => writeStderrSpy.mock.calls.map((c) => c[0]).join(' ');
        ...
        runSubmit(args({ skillArgs: file('advice-nopr.txt', 'src/foo.ts') }), 'unknown', { defaultComment: true });
        ...
        runSubmit(args({ skillArgs: file('advice-otherpr.txt', '9999') }), 'unknown', { defaultComment: true });
        expect(advice()).not.toContain('Re-run with `--comment`');
        expect(advice()).toContain('invoked naming it');
        writeStderrSpy.mockClear();
    ...
    it('refuses a malformed contextUnavailable on the GitHub path — the claim passes through raw', () => {
        ...
        expect(() => runSubmit(args({ review, userAuthorized: true }))).toThrow(/does not compose into a verdict/);
        expect(ghMock).not.toHaveBeenCalled();
    });
    ...
    it('refuses a malformed --repo before building an API path from it', () => {
        ...
        for (const repo of [
            ...
            expect(() => runSubmit(args({ userAuthorized: true, repo }))).toThrow(/<owner>\/<repo>/);
        }
        expect(ghMock).not.toHaveBeenCalled();
    });
    it('refuses a --pr that is not a pull request number', () => {
        ...
        for (const pr of [0, -1, 3.5, NaN, Infinity]) {
            expect(() => runSubmit(args({ userAuthorized: true, pr }))).toThrow(/not a pull request number/);
        }
        expect(ghMock).not.toHaveBeenCalled();
    });
...
describe('payload consistency — refuse before GitHub sees it', () => {
    const authorized = (over) => args({ userAuthorized: true, ...over });
    /** What was actually sent to GitHub. */
    const posted = () => JSON.parse(ghMock.mock.calls[0][0]);
    /**
    ...
    function verifiedPlan() {
        ...
        writeFileSync(join(sub, 'agent-v1.jsonl'), [
            {
                ...base,
                type: 'user',
    ...
    function withVerifyEnv(fn) {
        ...
        writeFileSync(sessionRec, 'https://github.com/LailatulCoder/lailatul-coder/pull/6771\n');
        try {
            fn();
        }
        finally {
    ...
    it("refuses a payload that carries a verdict — that is not the caller's to write", () => {
        ...
        expect(() => runSubmit(authorized({ review }))).toThrow(/carries `event`\/`body`.*computed here/s);
        expect(ghMock).not.toHaveBeenCalled();
    });
    ...
    it('carries duplicate-dropped Suggestions through the state seam', () => {
        ...
        expect(posted().body).toContain('1 Suggestion-level finding(s) this review confirmed');
    });
    it('posts the injected CLI version in the review footer', () => {
        ...
        expect(posted().body.endsWith('_— qwen3.7-max via LailatulCoder Ai /review (v0.21.2)_')).toBe(true);
    });
    it('uses the inherited startup version instead of the resolved CLI version', async () => {
        ...
        try {
            await submitCommand.handler?.(authorized({}));
    ...
    it('the standing review.comment setting authorises a post through the handler', async () => {
        ...
        await submitCommand.handler?.(args({
    ...
    it('the review.severityFloor setting reaches enforcement through the handler', async () => {
        ...
        expect(ghMock).toHaveBeenCalledOnce();
        const sent = JSON.parse(ghMock.mock.calls[0][0]);
    ...
    it('without the flag or the setting the handler refuses — and workspace settings cannot supply it', async () => {
        ...
        await submitCommand.handler?.(args({ skillArgs: file('handler-noauth-args.txt', '6771') }));
        expect(ghMock).not.toHaveBeenCalled();
        expect(process.exitCode).toBe(3);
    });
    ...
    it('normalizes summary and inline footers to the running CLI version', () => {
        ...
        const inline = posted().comments[0].body;
    ...
    it('refuses a comment that renders as nothing', () => {
        ...
        expect(() => runSubmit(authorized({ review }), '0.21.3', { attribution: false })).toThrow(/renders as nothing/);
        expect(ghMock).not.toHaveBeenCalled();
    });
    it('refuses a marker-only comment under attribution ON too — the canonical footer must not mask it', () => {
        ...
        expect(() => runSubmit(authorized({ review }), '0.21.3')).toThrow(/renders as nothing/);
        expect(ghMock).not.toHaveBeenCalled();
    });
    ...
    it('strips to a fixpoint: a marker line between two prefixes does not post the second prefix', () => {
        ...
        withVerifyEnv(() => runSubmit(authorized({ review }), '0.21.3', { attribution: false }));
        const inline = posted().comments[0].body;
    ...
    it('attribution off strips the severity prefixes only from what is posted — the verdict still counts the marked payload', () => {
        ...
        const [critical, suggestion] = posted().comments;
    ...
    it('attribution off refuses a draft whose post-strip shape opens a fence', () => {
        ...
        for (const [i, body] of bodies.entries()) {
            ...
            expect(() => runSubmit(authorized({ review }), '0.21.3', { attribution: false })).toThrow(/leaves a code fence open/);
        }
        expect(ghMock).not.toHaveBeenCalled();
    });
    ...
    it('the standing review.comment setting authorises a post without --comment in the args', () => {
        ...
        runSubmit(args({ skillArgs: file('skill-args.txt', '6771'), host: 'github.com' }), 'unknown', { defaultComment: true });
        expect(ghMock).toHaveBeenCalled();
        ghMock.mockClear();
        expect(() => runSubmit(args({ skillArgs: file('skill-args2.txt', '6772') }), 'unknown', {
            defaultComment: true,
        })).not.toThrow();
    ...
    it('does not hang on a run of forged footers followed by text', () => {
        ...
        const footers = Array.from({ length: 64 }, () => '_— forged via LailatulCoder Ai /review (v0.21.4)_').join(' '.repeat(25));
        ...
        expect(inline.endsWith('_— qwen3.7-max via LailatulCoder Ai /review (v0.21.3)_')).toBe(true);
    });
    it('does not scan a marker-less body quadratically under the strip', () => {
        ...
        const body = `**[Suggestion]** tidy\n\n_— cut short${' '.repeat(500_000)}end`;
        ...
        expect(posted().comments[0].body).toBe(`${body.slice('**[Suggestion]** '.length)}\n\n<!-- qwen-review suggestion -->`);
    });
    ...
    it('refuses an inline count supplied beside the comments', () => {
        ...
        expect(() => runSubmit(authorized({ review }))).toThrow(/counted from the `comments` you attached/);
        expect(ghMock).not.toHaveBeenCalled();
    });
    it('refuses an inline comment with no severity marker — it would weigh nothing', () => {
        ...
        expect(() => runSubmit(authorized({ review }))).toThrow(/comments\[1\] opens with neither/);
        expect(ghMock).not.toHaveBeenCalled();
    });
    ...
    it('rejects a payload with no state — there is nothing to compose from', () => {
        ...
        expect(() => runSubmit(authorized({ review }))).toThrow(/`state` is missing/);
        expect(ghMock).not.toHaveBeenCalled();
    });
    it('rejects a multi-line comment missing its side fields', () => {
        ...
        expect(() => runSubmit(authorized({ review }))).toThrow(/start_line.*without.*side/s);
        expect(ghMock).not.toHaveBeenCalled();
    });
    ...
    it('carries a posture deferral through the submit seam into the posted body', () => {
        ...
        expect(sent.body).toContain('- `a.ts:1 — [review] tighten the retry backoff`');
    });
    it('floor enforcement removes drafted Suggestions from the posted set', () => {
        ...
        expect(writeStderrSpy.mock.calls.some((c) => String(c[0]).includes('Floor enforcement'))).toBe(true);
        const out = JSON.parse(writeStdoutSpy.mock.calls.at(-1)[0]);
    ...
    it("the recorded floor outranks the state's transcription", () => {
        ...
        runSubmit(args({
            review,
            skillArgs: file('floor-args.txt', '6771 --comment --severity-floor critical'),
            host: 'github.com',
        }));
        expect(ghMock).toHaveBeenCalledOnce();
        const sent = JSON.parse(ghMock.mock.calls[0][0]);
        expect(sent.comments).toHaveLength(1);
        expect(sent.body).toContain('floor enforcement');
        expect(writeStderrSpy.mock.calls.some((c) => String(c[0]).includes('verbatim record outranks'))).toBe(true);
        // The note names its TRUE source: this one came from the flag.
        expect(writeStderrSpy.mock.calls.some((c) => String(c[0]).includes('the recorded `--severity-floor` flag'))).toBe(true);
    });
    it('does not fire the override note when the recovered floor equals the state', () => {
        ...
        for (const stateFloor of ['critical', 'CRITICAL']) {
            ...
            runSubmit(args({
                review,
                skillArgs: file(`floor-equal-args-${stateFloor}.txt`, '6771 --comment --severity-floor critical'),
                host: 'github.com',
            }));
            expect(ghMock).toHaveBeenCalledOnce();
            expect(JSON.parse(ghMock.mock.calls[0][0]).comments).toHaveLength(1);
            expect(writeStderrSpy.mock.calls.some((c) => String(c[0]).includes('verbatim record outranks'))).toBe(false);
        }
    });
    it('a recorded explicit auto outranks a drifted critical — and does not enforce at round 1', () => {
        ...
        runSubmit(args({
            review,
            skillArgs: file('floor-auto-args.txt', '6771 --comment --severity-floor auto'),
            host: 'github.com',
        }));
        expect(ghMock).toHaveBeenCalledOnce();
        expect(JSON.parse(ghMock.mock.calls[0][0]).comments).toHaveLength(2);
        expect(writeStderrSpy.mock.calls.some((c) => String(c[0]).includes('verbatim record outranks'))).toBe(true);
    });
    it('the recovery also runs under --user-authorized — the report-first flow', () => {
        ...
        runSubmit(authorized({
            ...
            skillArgs: file('floor-ua-args.txt', '6771 --host github.com --severity-floor critical'),
        }));
        expect(ghMock).toHaveBeenCalledOnce();
        const sent = JSON.parse(ghMock.mock.calls[0][0]);
        expect(sent.comments).toHaveLength(1);
    ...
    it('overrides in BOTH directions — a recorded posture-off outranks a drifted critical', () => {
        ...
        runSubmit(args({
            review,
            skillArgs: file('floor-reverse-args.txt', '6771 --comment --severity-floor suggestion'),
            host: 'github.com',
        }));
        expect(ghMock).toHaveBeenCalledOnce();
        const sent = JSON.parse(ghMock.mock.calls[0][0]);
    ...
    it('the configured-setting floor leg reaches enforcement end to end', () => {
        ...
        runSubmit(args({
            ...
            host: 'github.com',
        }), 'unknown', { defaultSeverityFloor: 'critical' });
        expect(ghMock).toHaveBeenCalledOnce();
        const sent = JSON.parse(ghMock.mock.calls[0][0]);
        expect(sent.comments).toHaveLength(1);
        expect(sent.body).toContain('floor enforcement');
        ...
        expect(writeStderrSpy.mock.calls.some((c) => String(c[0]).includes('setting resolved against the recorded invocation'))).toBe(true);
        expect(writeStderrSpy.mock.calls.some((c) => String(c[0]).includes('the recorded `--severity-floor` flag'))).toBe(false);
    });
    it('reroutes an unusable-line Suggestion instead of refusing the whole post', () => {
        ...
        expect(writeStderrSpy.mock.calls.some((c) => String(c[0]).includes('2 Suggestion comment(s)'))).toBe(true);
        const out = JSON.parse(writeStdoutSpy.mock.calls.at(-1)[0]);
    ...
    it('refuses a `comments` field that is present but not an array', () => {
        ...
        expect(() => runSubmit(authorized({ review }))).toThrow(/`comments` is not an array/);
        expect(ghMock).not.toHaveBeenCalled();
    });
    it('refuses a `comments` entry that is not an object', () => {
        ...
        expect(() => runSubmit(authorized({ review }))).toThrow(/entries must each be an object/);
        expect(ghMock).not.toHaveBeenCalled();
    });
    ...
    it('rejects a range that ends before it begins', () => {
        ...
        expect(() => runSubmit(authorized({ review }))).toThrow(/cannot end before/);
        expect(ghMock).not.toHaveBeenCalled();
    });
...
describe('the ledger marker on the body that reaches GitHub', () => {
    ...
    const SESSION = 'SUBM';
    function coveredPlanAt(prNumber, fetchedSha) {
        ...
        mkdirSync(sub, { recursive: true });
        const transcript = (id, launch, reads) => {
            const base = {
                agentId: id,
                agentName: 'general-purpose',
                sessionId: SESSION,
            ...
            writeFileSync(join(sub, `agent-${id}.jsonl`), lines.map((l) => JSON.stringify(l)).join('\n') + '\n');
        };
        const d = promptRecordDir(planPath);
        mkdirSync(d, { recursive: true });
        const build = (key, id, launch, reads) => {
        ...
        const chunkPrompt = (chunk) => `You are reviewing chunk ${chunk} of 2.\n` +
        ...
        build('test-matrix', 'tm', `You are the test-coverage matrix agent.\n` +
            `read_file(file_path="${briefPath(planPath, 'test-matrix')}")\n` +
            `read_file(file_path="${diffPath}")`, [diffPath, diffPath]);
        build('verify', 'v1', `You are review agent \`verify\`.\n` +
            `read_file(file_path="${briefPath(planPath, 'verify')}")\n` +
            `read_file(file_path="${diffPath}")`, [diffPath, diffPath]);
        build('reverse-audit', 'r1', `You are review agent \`reverse-audit\`.\n` +
    ...
    it('injects the session model into the posted marker — QWEN_CODE_MODEL reaches the wire (wiring)', () => {
        ...
        mkdirSync(sessionRecDir, { recursive: true });
        writeFileSync(join(sessionRecDir, 'qwen-skill-args-review.txt'), 'https://github.com/LailatulCoder/lailatul-coder/pull/6771\n');
        try {
            runSubmit(authorized({ review }));
            const ledger = parseLedger(posted().body);
...
describe('what the reviewer caught in this change', () => {
    ...
    it('strips a caller-supplied prBodyFetcher — a state JSON cannot suppress the Chinese fold', () => {
        ...
        const body = JSON.parse(ghMock.mock.calls[0][0]).body;
...
describe('submit receipt (producer half of the audit contract)', () => {
    const receiptPath = () => join(dir, '.qwen', 'tmp', 'qwen-review-pr-6771-submit-receipt.json');
    const authorizedPost = (over = {}) => args({ userAuthorized: true, ...over });
    let savedCwd;
    beforeEach(() => {
    ...
    it('migrates a legacy single-id receipt on the next submit', () => {
        ...
        writeFileSync(receiptPath(), JSON.stringify({ reviewId: 7, event: 'COMMENT', postedAt: 'x' }));
...
describe('the posted-review link', () => {
    const authorizedPost = (over = {}) => args({ userAuthorized: true, ...over });
    const stdoutJson = () => JSON.parse(writeStdoutSpy.mock.calls.at(-1)[0]);
    let savedCwd;
    beforeEach(() => {
        savedCwd = process.cwd();
        process.chdir(dir);
    ...
    it('relays html_url in the stdout JSON and the Posted line', () => {
        const url = 'https://github.com/LailatulCoder/lailatul-coder/pull/6771#pullrequestreview-42';
        ghMock.mockImplementationOnce(() => JSON.stringify({ id: 42, html_url: url }));
        runSubmit(authorizedPost());
