...
import { scanImportSpecifiers, resolveSpecifier, dependentsOfChanged, discoverWorkspacePackages, } from './import-graph.js';
describe('scanImportSpecifiers', () => {
    ...
    it('accepts type-only imports — a signature change is an interaction too', () => {
        expect(scanImportSpecifiers("import type { T } from './types.js';")).toEqual(['./types.js']);
    });
});
describe('resolveSpecifier', () => {
    ...
    it('maps the emitted-extension specifier back to its source', () => {
        expect(resolveSpecifier('packages/cli/src/z.ts', './a.js', files)).toBe('packages/cli/src/a.ts');
        expect(resolveSpecifier('packages/cli/src/z.ts', './b.jsx', files)).toBe('packages/cli/src/b.tsx');
    });
    it('prefers the LITERAL file over its extension remap when both exist', () => {
        ...
        expect(resolveSpecifier('packages/cli/src/z.ts', './util.js', mixed)).toBe('packages/cli/src/util.js');
        ...
        expect(resolveSpecifier('packages/cli/src/z.ts', './util.js', new Set(['packages/cli/src/util.ts']))).toBe('packages/cli/src/util.ts');
    });
    it('accepts a package subpath whose directory merely BEGINS with dots', () => {
        ...
        expect(resolveSpecifier('packages/cli/src/z.ts', '@qwen/core/..config/mod.js', dotted, pkgs)).toBe('packages/core/..config/mod.ts');
        // A genuine escape is still refused.
        expect(resolveSpecifier('packages/cli/src/z.ts', '@qwen/core/../outside.js', new Set(['packages/outside.ts', 'outside.ts']), pkgs)).toBeNull();
    });
    it('walks extensions and index forms for extensionless specifiers', () => {
        expect(resolveSpecifier('packages/cli/src/z.ts', './a', files)).toBe('packages/cli/src/a.ts');
        expect(resolveSpecifier('packages/cli/src/z.ts', './dir', files)).toBe('packages/cli/src/dir/index.ts');
    });
    it('resolves workspace-package specifiers, entry and subpath alike', () => {
        expect(resolveSpecifier('packages/cli/src/z.ts', '@qwen/core', files, pkgs)).toBe('packages/core/src/index.ts');
        expect(resolveSpecifier('packages/cli/src/z.ts', '@qwen/core/src/util/x.js', files, pkgs)).toBe('packages/core/src/util/x.ts');
        // A dist deep-import names build output; the src fallback finds the source.
        expect(resolveSpecifier('packages/cli/src/z.ts', '@qwen/core/util/x.js', files, pkgs)).toBe('packages/core/src/util/x.ts');
    });
    it('maps an emitted .js specifier to a .tsx source — the UI layer convention', () => {
        const uiFiles = new Set(['packages/cli/src/ui/App.tsx']);
        expect(resolveSpecifier('packages/cli/src/ui/AppContainer.tsx', './App.js', uiFiles)).toBe('packages/cli/src/ui/App.tsx');
    });
    it('resolves .cjs to .cts — every EXT_MAP row has a resolution pin', () => {
        expect(resolveSpecifier('a.ts', './f.cjs', new Set(['f.cts']))).toBe('f.cts');
    });
    it('dist deep-imports resolve under BOTH emit layouts', () => {
        ...
        expect(resolveSpecifier('a.ts', '@qwen/core/dist/src/utils/x.js', new Set(['packages/core/src/utils/x.ts']), pkgs)).toBe('packages/core/src/utils/x.ts');
        // flat dist/… layout: strip dist, add src.
        expect(resolveSpecifier('a.ts', '@qwen/core/dist/utils/x.js', new Set(['packages/core/src/utils/x.ts']), pkgs)).toBe('packages/core/src/utils/x.ts');
    });
    it('resolves .mjs to .mts and root-package (dir: "") specifiers', () => {
        ...
        expect(resolveSpecifier('a.ts', 'root', rootFiles, rootPkg)).toBe('src/index.ts');
        expect(resolveSpecifier('a.ts', 'root/src/util/x.js', rootFiles, rootPkg)).toBe('src/util/x.ts');
    });
    it('strips the dist/ segment on emitted-tree deep imports', () => {
        ...
        expect(resolveSpecifier('a.ts', '@qwen/core/dist/utils/foo.js', distFiles, distPkgs)).toBe('packages/core/src/utils/foo.ts');
    });
    it('maps an emitted .js to a .jsx source, and normalises package subpaths', () => {
        expect(resolveSpecifier('a.ts', './W.js', new Set(['W.jsx']))).toBe('W.jsx');
        const pkgs = [{ name: '@q/core', dir: 'packages/core' }];
        expect(resolveSpecifier('a.ts', '@q/core/src/util/../x.js', new Set(['packages/core/src/x.ts']), pkgs)).toBe('packages/core/src/x.ts');
        // …and a subpath escaping the package resolves to nothing.
        expect(resolveSpecifier('a.ts', '@q/core/../outside.js', new Set(['outside.ts']), pkgs)).toBeNull();
    });
    it('a directory that merely BEGINS with dots is not a root escape', () => {
        const dotFiles = new Set(['..config/mod.ts']);
        expect(resolveSpecifier('a.ts', './..config/mod.js', dotFiles)).toBe('..config/mod.ts');
        // Separating case for the guard itself: membership CONTAINS the escaping
        // path, so only repoJoin's refusal produces the null.
        expect(resolveSpecifier('a.ts', '../../x', new Set(['../../x.ts']))).toBeNull();
    });
    it('resolves a specifier that already names its source extension', () => {
        ...
        expect(resolveSpecifier('a.ts', './data.json', new Set(['data.json']))).toBe('data.json');
    ...
    it('returns null outside membership, above the root, and for unknown packages', () => {
        expect(resolveSpecifier('packages/cli/src/z.ts', './missing', files)).toBeNull();
...
describe('dependentsOfChanged', () => {
    ...
    const read = (p) => sources.get(p) ?? null;
    it('returns only candidates that import a changed file, with the edges named', () => {
        const changed = new Set(['src/changed.ts']);
        const out = dependentsOfChanged(changed, ['src/caller.ts', 'src/bystander.ts', 'src/changed.ts'], read);
    ...
    it('consults the packages argument for cross-package edges', () => {
        const out = dependentsOfChanged(new Set(['packages/core/src/index.ts']), ['packages/cli/src/z.ts'], () => "import { x } from '@qwen/core';", [{ name: '@qwen/core', dir: 'packages/core' }]);
        expect([...out.entries()]).toEqual([
            ['packages/cli/src/z.ts', ['packages/core/src/index.ts']],
        ]);
    });
    it('an unreadable candidate contributes no edge and no crash', () => {
        const out = dependentsOfChanged(new Set(['src/changed.ts']), ['src/gone.ts'], () => null);
        expect(out.size).toBe(0);
    });
});
describe('discoverWorkspacePackages', () => {
    ...
    const read = (p) => manifests.get(p) ?? null;
    it('maps each file to its nearest manifest, most specific dir first', () => {
        const pkgs = discoverWorkspacePackages(['packages/core/src/a.ts', 'packages/cli/src/deep/b.ts', 'scripts/x.js'], read);
        expect(pkgs).toEqual([
            { name: '@qwen/core', dir: 'packages/core' },
            { name: '@qwen/cli', dir: 'packages/cli' },
            { name: 'root', dir: '' },
    ...
    it('is fail-quiet on malformed or nameless manifests', () => {
        for (const manifest of [
            ...
            const pkgs = discoverWorkspacePackages(['pkg/src/a.ts'], (p) => p === 'pkg/package.json' ? manifest : null);
            expect(pkgs).toEqual([]);
        }
    });
});
