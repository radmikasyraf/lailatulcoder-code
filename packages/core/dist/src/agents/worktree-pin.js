 ...
 *   user-facing reason.
 */
export async function resolveExternalWorktreeDir(config, workingDir, label = 'working_dir') {
    ...
    const wtService = repoRoot === parentCwd ? probe : new GitWorktreeService(repoRoot);
    ...
    if (!(await wtService.isRegisteredLinkedWorktree(pinnedPath))) {
        ...
        return {
            error: `${label} "${resolvedPath}" is not a registered linked worktree of ` +
...
//# sourceMappingURL=worktree-pin.js.map