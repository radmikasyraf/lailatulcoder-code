...
// Mock cleanup.ts to avoid pulling in @lailatul-coder/lailatul-coder-core dependency chain
vi.mock('./cleanup.js', () => ({
    registerCleanup: vi.fn(),
}));
import { _resetCpuProfilerForTest, _setSessionFactoryForTest, clearCpuProfileRateLimit, isCpuProfileRecording, startCpuProfile, stopCpuProfile, } from './cpuProfiler.js';
function createMockSession() {
    ...
    const post = vi.fn().mockImplementation((method) => {
...
describe('cpuProfiler', () => {
    ...
    describe('startCpuProfile', () => {
        it('starts profiling successfully', async () => {
            ...
            expect(mockSession.post).toHaveBeenCalledWith('Profiler.setSamplingInterval', { interval: 1000 });
        ...
        it('accepts custom sampling interval', async () => {
            ...
            expect(mockSession.post).toHaveBeenCalledWith('Profiler.setSamplingInterval', { interval: 500 });
        });
    ...
    describe('old profile cleanup', () => {
        it('removes old profiles beyond max count', async () => {
            ...
            for (let i = 0; i < 5; i++) {
                const name = `lailatul-coder-cpu-99999-2026-05-29T0${i}-00-00-000Z.cpuprofile`;
    ...
    describe('empty profile guard', () => {
        it('returns error when V8 returns empty profile', async () => {
            _resetCpuProfilerForTest();
            const emptyMock = {
                post: vi.fn().mockImplementation((method) => {
                    if (method === 'Profiler.stop') {
...
//# sourceMappingURL=cpuProfiler.test.js.map