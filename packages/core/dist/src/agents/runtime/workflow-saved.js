 ...
 * `.lailatulcoder/workflows/<name>.js` (project) or `~/.lailatulcoder/workflows/<name>.js`
...
export function validateWorkflowName(name) {
    ...
    if (!WORKFLOW_NAME_PATTERN.test(name)) {
        return (`Invalid workflow name "${name}". Use lower-case letters, digits, and ` +
            `hyphens only (must start with a letter, max 41 chars).`);
    }
    return null;
}
/** Both scope directories, project first (higher precedence). */
export function getSavedWorkflowDirs(config) {
    return [
 ...
 * `.lailatulcoder/workflows -> /outside` link into the allowed boundary — letting discovery
...
async function readWorkflowFileSecurely(filePath, config) {
    ...
    const dirs = (await Promise.all(getSavedWorkflowDirs(config).map(async ({ dir }) => {
        // Exclude a symlinked root: realpath(dir) would launder a
        // `.lailatulcoder/workflows -> /outside` link into the allowed boundary, so a
    ...
    if (!inside) {
        throw new Error(`refusing to load a workflow file outside the saved-workflow directories: '${filePath}'.`);
    }
 ...
 */
export async function listSavedWorkflows(config) {
 ...
 * see the same error.
 */
export async function resolveSavedWorkflowScript(nameOrRef, config) {
    if (typeof nameOrRef === 'object' && nameOrRef !== null) {
        const scriptPath = nameOrRef.scriptPath;
        if (typeof scriptPath !== 'string' || scriptPath.length === 0) {
            throw new Error('workflow() expects a workflow name (string) or {scriptPath: string}.');
        }
        let script;
        try {
            ...
            throw new Error(`workflow({scriptPath: '${scriptPath}'}): ` +
    ...
    throw new Error(`workflow('${name}'): no workflow with that name. Available: ` +
        `${available.length > 0 ? available.join(', ') : '(none)'}.`);
}
/**
 * Save a workflow script to `.lailatulcoder/workflows/<name>.js` (project) or
 * `~/.lailatulcoder/workflows/<name>.js` (user). Powers the `/workflows` save dialog.
 *
 ...
 */
export async function saveWorkflowScript(config, opts) {
    const { name, scope, script, overwrite = false } = opts;
    const nameError = validateWorkflowName(name);
    if (nameError)
        return { status: 'invalid-name', error: nameError };
    ...
    // Refuse to write through a symlinked root (e.g. `.lailatulcoder/workflows -> /outside`):
    ...
    if (await isSymlinkedRoot(dir)) {
        throw new Error(`refusing to save into a symlinked saved-workflow directory: '${dir}'.`);
    }
...
//# sourceMappingURL=workflow-saved.js.map