...
import { applyReasoningEffort, normalizeReasoningEffort, REASONING_EFFORT_TIERS, } from '@lailatul-coder/lailatul-coder-core';
...
export const effortCommand = {
    name: 'effort',
    get description() {
        return t('Set how hard reasoning-capable models think ({{tiers}}); mapped and clamped per provider.', { tiers: TIER_LIST });
    },
    ...
    action: async (context, actionArgs) => {
        ...
        if (!args) {
            ...
            return {
                ...
                messageType: 'info',
                content: current
                    ? t('Current reasoning effort: {{current}}\nAvailable: {{tiers}}\nUse "/effort <tier>" to change it.', { current, tiers: TIER_LIST })
                    : t('Reasoning effort: not set (using the model/provider default).\nAvailable: {{tiers}}\nUse "/effort <tier>" to set it.', { tiers: TIER_LIST }),
            };
        }
        ...
        if (!tier) {
            return {
                type: 'message',
                messageType: 'error',
                content: t('Unknown reasoning effort "{{value}}". Choose one of: {{tiers}}.', { value: args, tiers: TIER_LIST }),
            };
        }
        if (!settings) {
            return {
        ...
        settings.setValue(getPersistScopeForModelSelection(settings), 'model.reasoningEffort', tier);
        return {
            type: 'message',
            messageType: 'info',
...
//# sourceMappingURL=effort-command.js.map