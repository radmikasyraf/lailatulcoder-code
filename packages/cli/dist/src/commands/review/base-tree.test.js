...
import { utimesSync, mkdtempSync, mkdirSync, rmSync, writeFileSync, existsSync, } from 'node:fs';
...
describe('runBaseTree', () => {
    ...
    const writePlan = (over = {}) => {
        ...
        writeFileSync(p, JSON.stringify({ mergeBaseSha: baseSha, files: [], ...over }));
        return p;
    };
    const run = (over = {}, build = () => okBuild) => {
    ...
    it('a budget-TRUNCATED build is unavailable but NOT settled — no marker either way', () => {
        ...
        expect(existsSync(join(first.path, '.qwen-review-base-failed'))).toBe(false);
        const second = run({}, build);
    ...
    it('is NOT available when the build handed off without building anything', () => {
        ...
        const r = run({}, () => handoff);
        expect(r.available).toBe(false);
        expect(existsSync(join(baseWorktreePath(worktree), '.qwen-review-base-ok'))).toBe(false);
    });
    ...
    it('refuses an unreadable plan and a missing worktree without throwing', () => {
        expect(runBaseTree({
            ...
            install: false,
            build: () => okBuild,
        }).note).toMatch(/cannot read the plan/);
        expect(run({ worktree: join(repo, 'no-such-tree') }).note).toMatch(/does not exist/);
    });
...
//# sourceMappingURL=base-tree.test.js.map