...
import { loadMcpApprovals, resetMcpApprovalsForTesting, MCP_APPROVALS_FILENAME, } from '../../config/mcpApprovals.js';
describe('useMcpApproval', () => {
    let dir;
    let discoverSpy;
    let approveForSession;
    const makeConfig = (servers, approvalMode = ApprovalMode.DEFAULT) => {
        const pending = new Set(Object.entries(servers)
            .filter(([, c]) => c.scope === 'project')
            .map(([name]) => name));
        approveForSession = vi.fn((name) => pending.delete(name));
    ...
    it('shows env and header key names in the approval summary', () => {
        ...
        const { result } = renderHook(() => useMcpApproval(config));
        expect(result.current.currentMcpApproval?.summary).toBe('node server.js (stdio) [env: LD_PRELOAD, TOKEN; headers: Authorization]');
    });
    it('approve persists, un-gates, reconnects, and advances the queue', async () => {
        const a = { command: 'a', scope: 'project' };
        const config = makeConfig({
            a,
            b: { command: 'b', scope: 'project' },
        });
        const { result } = renderHook(() => useMcpApproval(config));
        await act(async () => result.current.handleMcpApprovalSelect(McpApprovalChoice.APPROVE));
    ...
    it('reject persists rejected, does not reconnect, advances', async () => {
        ...
        const { result } = renderHook(() => useMcpApproval(config));
        await act(async () => result.current.handleMcpApprovalSelect(McpApprovalChoice.REJECT));
    ...
    it('approve-all approves every queued server and closes the dialog', async () => {
        const a = { command: 'a', scope: 'project' };
        const b = { command: 'b', scope: 'project' };
        const config = makeConfig({ a, b });
        const { result } = renderHook(() => useMcpApproval(config));
        await act(async () => result.current.handleMcpApprovalSelect(McpApprovalChoice.APPROVE_ALL));
        expect(stateOf('a', a)).toBe('approved');
    ...
    it('YOLO: dialog stays closed even with pending project-scoped servers', () => {
        const config = makeConfig({ a: { command: 'a', scope: 'project' } }, ApprovalMode.YOLO);
        const { result } = renderHook(() => useMcpApproval(config));
        expect(result.current.isMcpApprovalDialogOpen).toBe(false);
    });
...
//# sourceMappingURL=useMcpApproval.test.js.map