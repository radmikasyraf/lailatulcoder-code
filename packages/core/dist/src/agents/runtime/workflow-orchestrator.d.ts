...
import type { WorkflowAgentOpts, WorkflowAgentResult, WorkflowBudget, WorkflowMeta, WorkflowOrchestratorEmitter } from './workflow-sandbox.js';
 ...
 */
export declare function resolveMaxAgentsPerRun(env?: Record<string, string | undefined>): number;
export declare const MAX_WORKFLOW_CONCURRENCY_ENV = "QWEN_CODE_MAX_WORKFLOW_CONCURRENCY";
/**
 ...
 */
export declare function resolveConcurrencyLimit(env?: Record<string, string | undefined>): number;
/**
 ...
 */
export declare const DEFAULT_WORKFLOW_SUBAGENT_MAX_TURNS = 50;
export declare const DEFAULT_WORKFLOW_SUBAGENT_MAX_TIME_MINUTES = 10;
export declare const WORKFLOW_SUBAGENT_MAX_TURNS_ENV = "QWEN_CODE_WORKFLOW_AGENT_MAX_TURNS";
export declare const WORKFLOW_SUBAGENT_MAX_MINUTES_ENV = "QWEN_CODE_WORKFLOW_AGENT_MAX_MINUTES";
/** 10× the defaults — generous for a legitimate long agent, still bounded. */
export declare const HARD_WORKFLOW_SUBAGENT_MAX_TURNS_CEILING = 500;
export declare const HARD_WORKFLOW_SUBAGENT_MAX_MINUTES_CEILING = 100;
/** Per-attempt turn ceiling for a workflow subagent. */
export declare function resolveSubagentMaxTurns(env?: Record<string, string | undefined>): number;
/** Per-attempt wall-clock ceiling, in minutes, for a workflow subagent. */
export declare function resolveSubagentMaxTimeMinutes(env?: Record<string, string | undefined>): number;
/**
...
export declare class WorkflowExecutionError extends Error {
    ...
    constructor(message: string, phases: string[], logs: string[], 
    /**
     * The extracted meta if it was parsed before the script body threw —
     * null otherwise (no declaration in the source, or malformed meta
     * which itself was the failure). Surfaced so the tool's failure
...
export interface WorkflowRunRequest {
     ...
     */
    resolveSavedWorkflow?: (nameOrRef: string | {
        scriptPath: string;
    }) => Promise<{
        script: string;
        scriptPath?: string;
...
export type WorkflowAgentDispatch = (prompt: string, opts: WorkflowAgentOpts, dispatchId?: string) => Promise<WorkflowAgentResult>;
/**
 ...
 */
export declare function createProductionDispatch(config: Config, signal?: AbortSignal, 
/**
 ...
 */
onTokens?: (outputTokens: number, opts: WorkflowAgentOpts) => void, bridgeApprovalEvents?: (emitter: AgentEventEmitter, dispatchId?: string) => () => void): WorkflowAgentDispatch;
