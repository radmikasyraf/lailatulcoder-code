 ...
 */
export const USER_PROMPT_SUBMIT_CONTEXT_OPEN = '<qwen:user-prompt-submit-context>';
export const USER_PROMPT_SUBMIT_CONTEXT_CLOSE = '</qwen:user-prompt-submit-context>';
...
export function wrapUserPromptSubmitContext(context) {
    return `${USER_PROMPT_SUBMIT_CONTEXT_OPEN}\n${context}\n${USER_PROMPT_SUBMIT_CONTEXT_CLOSE}`;
}
export function isUserPromptSubmitContextPartText(text) {
...
export function projectUserTranscriptForDisplay(record) {
    ...
    const payload = isObjectRecord(record.systemPayload)
        ...
        : undefined;
    const isUserPromptPayload = payload &&
        (typeof payload['hookContext'] === 'string' || hasFinalHookContextPart);
    const displayText = isUserPromptPayload && typeof payload['displayText'] === 'string'
        ? payload['displayText']
        : undefined;
    if (displayText !== undefined) {
        const visibleParts = parts.filter((part) => !isObjectRecord(part) || typeof part['text'] !== 'string');
