...
function configCountFor(hook) {
    return hook.matcherGroups.reduce((sum, group) => sum + group.configs.length, 0);
}
export function HooksListStep({ hooks, selectedIndex, }) {
    ...
    const totalConfigured = hooks.reduce((sum, hook) => sum + configCountFor(hook), 0);
