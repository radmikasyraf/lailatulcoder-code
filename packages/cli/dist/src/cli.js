...
function printMcpHelp() {
    const lines = [
        'Usage: LailatulCoder mcp <command>',
        '',
        'Manage MCP servers',
        '',
        'Commands:',
        ...MCP_COMMANDS.map(([command, description]) => `  LailatulCoder mcp ${command}  ${description}`),
    ];
...
export async function runCliEntry(rawArgv = process.argv.slice(2)) {
    ...
    if (managedUpdateVersion) {
        ...
        const { installManagedNpmUpdate } = await import('./utils/managed-npm-update.js');
    ...
    if (route !== 'serve') {
        // This credential belongs only to `LailatulCoder serve`. Scrub it before any other
 ...
 * children as `"${QWEN_CODE_CLI:-LailatulCoder}"` (see getShellContextEnvVars in core).
 ...
 * launchers), so every skill shell-out resolved `LailatulCoder` off PATH: a different
 * install, silently.
 *
 ...
 * (dev runs execute .ts sources with no built entry; the bare-`LailatulCoder` fallback
 ...
 * subprocesses run `LailatulCoder`.
 *
 ...
 * overwritten. The cost is that a value inherited from a PARENT LailatulCoder session
 ...
 */
export function stampCliEntryEnv(entryPath) {
    ...
    if (existsSync(entry)) {
        try {
            ...
            try {
                ...
                // stamp and subprocesses fall back to `LailatulCoder`, as before this stamp.
            }
        }
