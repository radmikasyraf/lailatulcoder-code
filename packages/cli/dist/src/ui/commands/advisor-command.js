...
import { BTW_MAX_INPUT_LENGTH, buildBtwCacheSafeParams, runForkedAgent, } from '@lailatul-coder/lailatul-coder-core';
...
function buildAdvisorPrompt(focus) {
    return [
        '<system-reminder>',
...
function formatAdvisorReview(value) {
...
function formatAdvisorError(error) {
    return t('Advisor review failed: {{error}}', {
        error: error instanceof Error ? error.message : String(error || 'Unknown error'),
    });
}
async function askAdvisor(context, focus, abortSignal) {
    ...
    const advisorModel = context.services.settings.merged.advisorModel?.trim() || undefined;
...
export const advisorCommand = {
    name: 'advisor',
    get description() {
        return t('Get a second opinion on the current conversation from a reviewer model');
    },
    ...
    action: async (context, args) => {
        ...
        if (turnInFlight) {
            return {
                type: 'message',
                messageType: 'error',
                content: t('Another operation is in progress, wait for it to complete before running /advisor'),
            };
        }
        try {
            ui.setPendingItem({
            ...
            if (abortSignal.aborted)
                return;
            ui.addItem({ type: MessageType.ADVISOR, text: review.text, model: review.model }, Date.now());
        }
        catch (error) {
            if (abortSignal.aborted)
                return;
            ui.addItem({ type: MessageType.ERROR, text: formatAdvisorError(error) }, Date.now());
        }
        finally {
            if (!abortSignal.aborted)
...
//# sourceMappingURL=advisor-command.js.map