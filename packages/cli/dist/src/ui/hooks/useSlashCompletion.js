...
import { getCommandDisplayName, getCommandSourceBadge, } from '../../services/commandMetadata.js';
...
var CommandMatchStrength;
(function (CommandMatchStrength) {
    CommandMatchStrength[CommandMatchStrength["FUZZY"] = 0] = "FUZZY";
    CommandMatchStrength[CommandMatchStrength["SEGMENT_PREFIX"] = 1] = "SEGMENT_PREFIX";
    CommandMatchStrength[CommandMatchStrength["PREFIX"] = 2] = "PREFIX";
    CommandMatchStrength[CommandMatchStrength["EXACT"] = 3] = "EXACT";
})(CommandMatchStrength || (CommandMatchStrength = {}));
const RECENT_DECAY_MS = 10 * 60 * 1000;
function getCompletionPriority(command) {
    return command.completionPriority ?? 0;
}
function isSegmentBoundary(value, start) {
    if (start <= 0) {
        return false;
    }
    return ['-', '_', '/', ' '].includes(value[start - 1] ?? '');
}
function getCommandMatchStrength(matchedValue, query, start) {
...
function compareRankedCommandMatches(left, right) {
...
function getRecentScore(command, recentCommands, now = Date.now()) {
...
function getMatchedAlias(command, matchedValue) {
    return command.altNames?.find((altName) => altName.toLowerCase() === matchedValue.toLowerCase());
}
function createRankedCommandMatch(command, matchedValue, query, result, originalIndex, recentCommands) {
    return {
        command,
...
function toCommandSuggestion(command, matchedAlias, includeAliases = false) {
    return {
            ...
            const performFuzzySearch = async () => {
                ...
                if (partial === '') {
                    ...
                    rankedSuggestions = commandsToSearch
                        ...
                        return [
                            createRankedCommandMatch(cmd, cmd.name, partial, { score: 0, start: 0 }, index, recentCommands),
                        ];
                    })
                        .sort((left, right) => {
                        ...
                        try {
                            ...
                            fzfResults.forEach((result) => {
                                ...
                                if (cmd && cmd.description && originalIndex !== undefined) {
                                    const rankedMatch = createRankedCommandMatch(cmd, result.item, partial, result, originalIndex, recentCommands);
                            ...
                            rankedSuggestions = Array.from(rankedMatches.values()).sort(compareRankedCommandMatches);
                        }
                        catch (error) {
                ...
                if (!signal.aborted) {
                    const finalSuggestions = rankedSuggestions.map((match) => toCommandSuggestion(match.command, match.matchedAlias, partial === ''));
                    setSuggestions(finalSuggestions);
                }
            };
...
export function useSlashCompletion(props) {
    const { enabled, query, slashCommands, commandContext, recentCommands, setSuggestions, setIsLoadingSuggestions, setIsPerfectMatch, } = props;
    ...
    const getPrefixSuggestions = useMemo(() => (commands, partial) => {
        const rankedMatches = commands.flatMap((cmd, index) => {
            if (!cmd.description || cmd.hidden) {
                return [];
            }
            const matchedValues = [cmd.name, ...(cmd.altNames ?? [])].filter((value) => value.toLowerCase().startsWith(partial.toLowerCase()));
            if (matchedValues.length === 0) {
                return [];
            }
            const bestMatch = matchedValues
                .map((matchedValue) => createRankedCommandMatch(cmd, matchedValue, partial, {
                score: matchedValue.toLowerCase() === partial.toLowerCase()
                    ? 100
                    : 80,
                start: 0,
            }, index, recentCommands))
    ...
    const { suggestions: hookSuggestions, isLoading } = useCommandSuggestions(parserResult, commandContext, getFzfForCommands, getPrefixSuggestions, recentCommands);
