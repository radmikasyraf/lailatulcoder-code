...
import { buildAgentTranscriptAttach, getAgentJsonlPath, getAgentMetaPath, getSubagentSessionDir, normalizeResumedAgentDepth, readAgentMeta, patchAgentMeta, attachJsonlTranscriptWriter, } from './agent-transcript.js';
import { buildOrderedUuidChain } from '../utils/conversation-chain.js';
import { buildAvailableSkillsReminder, buildDeferredToolsReminder, buildMcpServerInstructionsReminder, getInitialChatHistory, } from '../utils/environmentContext.js';
...
import { FORK_AGENT, FORK_DEFAULT_MAX_TURNS, FORK_SUBAGENT_TYPE, buildForkExecutionAllowlist, registerForkDisplayImageForCache, resolveForkExecutionAllowedTools, runInForkContext, } from '../tools/agent/fork-subagent.js';
import { MAX_RETAINED_TERMINAL_AGENTS, } from './background-tasks.js';
import { BUBBLE_APPROVAL_MODE } from '../subagents/types.js';
import { EXCLUDED_TOOLS_FOR_SUBAGENTS, extractParentToolNames, } from './runtime/agent-core.js';
...
const CURRENT_FORK_RUNTIME_BLOCKED_REASON = 'Fork background task cannot be safely resumed because the current parent system prompt or tool surface is unavailable.';
...
const MISSING_TRANSCRIPT_BLOCKED_REASON = 'Background task transcript is missing or unreadable.';
const TRANSCRIPT_IDENTITY_BLOCKED_REASON = 'Background task transcript does not match its retained identity.';
const WORKING_DIRECTORY_BLOCKED_REASON = 'Background task working directory does not match the restored session.';
const WORKTREE_ISOLATION_BLOCKED_REASON = 'Background task worktree isolation cannot be reconstructed after session restore.';
const INCOMPATIBLE_ISOLATION_BLOCKED_REASON = 'Background task isolation metadata is incompatible.';
/**
...
function isWhitespaceOnlyAssistant(record) {
    if (record.type !== 'assistant')
        return false;
    if (!record.message?.parts?.length)
        return true;
...
function buildRecoveredModelNotice(count) {
    return (`${count} background agent${count === 1 ? ' was' : 's were'} restored ` +
        `from this session. Use list_agents to inspect ${count === 1 ? 'it' : 'them'} ` +
        'and send_message with a task_id to continue one.');
}
function recoveryTimestamp(meta) {
...
export class BackgroundAgentResumeService {
    ...
    async loadPausedBackgroundAgents(sessionId) {
        ...
        for (const fileName of files) {
            ...
            try {
                ...
                if (!meta ||
                    ...
                    path.basename(getAgentMetaPath(projectDir, sessionId, meta.agentId)) !== fileName ||
                ...
                debugLogger.warn(`[BackgroundAgentResume] Failed to read background agent metadata from ${metaPath}:`, error);
            }
        }
        sidecars.sort((a, b) => {
        ...
        for (const { metaPath, meta } of sidecars) {
            ...
            try {
                ...
                const outputFile = getAgentJsonlPath(projectDir, sessionId, meta.agentId);
                ...
                else if (records.some((record) => typeof record.cwd === 'string' &&
                    path.resolve(record.cwd) !== projectRoot)) {
                    retainedStateBlockedReason = WORKING_DIRECTORY_BLOCKED_REASON;
                }
                const resumeBlockedReason = retainedStateBlockedReason ||
    ...
    async reviveCompletedBackgroundAgent(agentId, initialMessage) {
        ...
        if (!revived) {
            ...
            const preferNonEmpty = (next, fallback) => (next && next.length > 0 ? [...next] : [...fallback]);
            this.restoreCompletedEntry({
                ...completedEntry,
                resumeBlockedReason: failedEntry?.resumeBlockedReason ??
                    completedEntry.resumeBlockedReason,
                pendingMessages: preferNonEmpty(failedEntry?.pendingMessages, completedEntry.pendingMessages),
                recentActivities: preferNonEmpty(failedEntry?.recentActivities, completedEntry.recentActivities),
                pendingApprovals: preferNonEmpty(failedEntry?.pendingApprovals, completedEntry.pendingApprovals),
            });
        }
        return revived;
    }
    async resumeBackgroundAgentInternal(agentId, operation) {
        ...
        const cleanupPreparedRuntime = () => {
            if (runtimeLifecycleOwned || setupCleaned)
                return;
            setupCleaned = true;
            if (cleanupRuntime) {
                cleanupRuntime();
        ...
        try {
            ...
            const approvalOverride = await createApprovalModeOverride(this.config, resolvedApprovalMode, { persistedCliFlags: meta.persistedCliFlags });
            ...
            if (target.isFork) {
                registerForkDisplayImageForCache(activeAgentConfig, currentForkRuntime.toolNames);
            }
            ...
            const bgConfig = Object.create(activeAgentConfig);
            ...
            const forkResumeCapabilityReminder = target.isFork
                ? await this.buildForkResumeCapabilityReminder(this.config, currentForkRuntime.toolNames)
                : undefined;
            const bgEventEmitter = new AgentEventEmitter();
            const launchModel = meta.model ?? meta.persistedCliFlags?.model;
            let subagent;
            if (target.isFork) {
                subagent = await this.createResumedForkSubagent(bgConfig, bgEventEmitter, resumeHistory ?? [], currentForkRuntime, meta.executionAllowedTools);
            }
            else {
                const resumeSubagentConfig = launchModel && meta.persistedCliFlags?.authType
                    ? { ...target.subagentConfig, model: 'inherit' }
                    : target.subagentConfig;
                const result = await this.config
                    .getSubagentManager()
                    .createAgentHeadless(resumeSubagentConfig, bgConfig, {
            ...
            const { jsonlPath, options: transcriptAttachOptions } = buildAgentTranscriptAttach(this.config, meta.agentId, {
            ...
            cleanupJsonl = attachJsonlTranscriptWriter(bgEventEmitter, jsonlPath, transcriptAttachOptions).cleanup;
            ...
            contextState.set('task_prompt', forkResumeCapabilityReminder
            ...
            const cleanupApprovalBridge = shouldBubble
                ...
                : undefined;
            const canStayResident = !target.isFork &&
                meta.isolation !== 'worktree' &&
                (!target.subagentConfig?.hooks ||
                    Object.keys(target.subagentConfig.hooks).length === 0);
            const needsAutoPermissionLease = () => activeAgentConfig.getApprovalMode() === 'auto' &&
            ...
            const runBody = async (turnContextState, turnAbortController, fireStartHook) => {
                ...
                try {
                    while (true) {
                        ...
                        if (finishingInputs) {
                            await subagent.executeExternalInputs(finishingInputs, turnAbortController.signal, { resetStats: false });
            ...
            const runBackgroundTurn = (turnContextState, turnAbortController, fireStartHook) => {
                ...
                const framedRunBody = () => runWithAgentContext(meta.agentId, () => runBody(turnContextState, turnAbortController, fireStartHook), normalizeResumedAgentDepth(meta.depth));
                const invocationRunBody = () => runWithInvocationContext(undefined, framedRunBody);
                return target.isFork
                    ? runInForkContext(invocationRunBody)
                    : invocationRunBody();
            };
            const reportUnexpectedBackgroundError = (error) => {
                debugLogger.warn(`[BackgroundAgentResume] Background agent ${meta.agentId} body raised unexpected rejection: ${error instanceof Error ? error.message : String(error)}`);
            };
            const residentController = {
                continue: (message) => {
                    ...
                    let restarted;
                    try {
                        restarted = registry.restartCompletedAgent(meta.agentId, nextAbortController);
                    }
                    catch (error) {
                        debugLogger.warn(`[BackgroundAgentResume] Could not continue resident background agent ${meta.agentId}: ${error instanceof Error ? error.message : String(error)}`);
                        return false;
                    }
                    if (!restarted ||
                        disposeRequested ||
    ...
    buildRecoveredBackgroundAgentsModelNotice(count) {
    ...
    async resolveCurrentForkRuntime() {
        try {
            const geminiClient = this.config.getGeminiClient();
            const generationConfig = geminiClient?.getChat().getGenerationConfig();
            if (!generationConfig?.systemInstruction) {
                debugLogger.debug('[BackgroundAgentResume] Current fork runtime unavailable (no_system_instruction): parent Gemini client or system instruction is missing.');
                return undefined;
            }
            const advertisedNames = extractParentToolNames(generationConfig);
            if (advertisedNames.length === 0) {
                debugLogger.debug('[BackgroundAgentResume] Current fork runtime unavailable (no_advertised_tools): parent advertises no inheritable tools after subagent exclusions.');
                return undefined;
            }
            const toolRegistry = this.config.getToolRegistry();
            await toolRegistry.warmAll();
            const registeredNames = new Set(toolRegistry
                .getFunctionDeclarationsFiltered(advertisedNames)
                .map((declaration) => declaration.name)
                .filter((name) => Boolean(name)));
            const toolNames = advertisedNames.filter((name) => registeredNames.has(name));
            if (toolNames.length === 0) {
                debugLogger.debug('[BackgroundAgentResume] Current fork runtime unavailable (no_registered_tools): none of the parent advertised tools are present in the live registry.');
                return undefined;
            }
            return {
                systemInstruction: structuredClone(generationConfig.systemInstruction),
                toolNames,
            };
        }
        catch (error) {
            debugLogger.warn(`[BackgroundAgentResume] Failed to reconstruct current fork runtime: ${error instanceof Error ? error.message : String(error)}`);
            return undefined;
        }
    }
    async buildForkResumeCapabilityReminder(agentConfig, toolNames) {
        ...
        try {
            ...
            if (mcpInstructions)
                reminders.push(mcpInstructions);
            if (toolNames.includes(ToolNames.SKILL)) {
                const skills = await buildAvailableSkillsReminder(agentConfig);
                if (skills)
                    reminders.push(skills.reminder);
            }
            const deferredTools = buildDeferredToolsReminder(toolRegistry);
            if (deferredTools)
                reminders.push(deferredTools);
        }
        catch (error) {
            debugLogger.warn(`[BackgroundAgentResume] Failed to build current fork capability reminder: ${error instanceof Error ? error.message : String(error)}`);
        }
        return reminders.join('\n\n');
    }
    async createResumedForkSubagent(agentConfig, eventEmitter, initialMessages, runtime, executionAllowedTools) {
        const promptConfig = {
        ...
        const toolConfig = {
            ...
            executionAllowedTools: resolveForkExecutionAllowedTools(runtime.toolNames, buildForkExecutionAllowlist(executionAllowedTools, runtime.toolNames)),
        };
