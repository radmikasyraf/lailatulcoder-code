...
import { getAllGeminiMdFilenames, Storage, getAutoMemoryRoot, getAutoMemoryProjectStateDir, getUserAutoMemoryRoot, AUTO_MEMORY_INDEX_FILENAME, } from '@lailatul-coder/lailatul-coder-core';
...
export function MemoryDialog({ onClose }) {
    ...
    const readToggle = (value) => !bareMode && !safeMode && (value ?? true);
    const [autoMemoryOn, setAutoMemoryOn] = useState(() => readToggle(loadedSettings.merged.memory?.enableManagedAutoMemory));
    const [autoDreamOn, setAutoDreamOn] = useState(() => readToggle(loadedSettings.merged.memory?.enableManagedAutoDream));
    const [autoSkillOn, setAutoSkillOn] = useState(() => readToggle(loadedSettings.merged.memory?.enableAutoSkill));
    const [autoSkillConfirmOn, setAutoSkillConfirmOn] = useState(() => readToggle(loadedSettings.merged.memory?.autoSkillConfirm ?? true));
    const [lastDreamAt, setLastDreamAt] = useState(null);
    const globalMemoryPath = useMemo(() => path.join(Storage.getGlobalQwenDir(), getAllGeminiMdFilenames()[0] ?? 'LAILATUL.md'), []);
    const projectMemoryPath = useMemo(() => path.join(config.getWorkingDir(), getAllGeminiMdFilenames()[0] ?? 'LAILATUL.md'), [config]);
    ...
    const resolveTargetPath = useCallback(async (item) => {
        ...
        switch (item.value) {
            case 'project':
                return resolvePreferredMemoryFile(config.getWorkingDir(), getAllGeminiMdFilenames()[0] ?? 'LAILATUL.md');
            case 'global':
                return resolvePreferredMemoryFile(Storage.getGlobalQwenDir(), getAllGeminiMdFilenames()[0] ?? 'LAILATUL.md');
            default: {
                const _exhaustive = item.value;
                return _exhaustive;
            }
    ...
    const handleToggleAutoSkill = useCallback(() => {
        ...
        loadedSettings.setValue(SettingScope.Workspace, 'memory.enableAutoSkill', newValue);
    ...
    const handleToggleAutoSkillConfirm = useCallback(() => {
        ...
        loadedSettings.setValue(SettingScope.Workspace, 'memory.autoSkillConfirm', newValue);
    ...
    return (_jsxs(Box, { borderStyle: "round", borderColor: theme.border.default, flexDirection: "column", padding: 1, width: "100%", children: [_jsx(Text, { bold: true, children: t('Memory') }), _jsxs(Box, { marginTop: 1, flexDirection: "column", children: [_jsxs(Text, { color: focusedSection === 'autoMemory'
                            ...
                            })] }), _jsxs(Text, { color: focusedSection === 'autoSkill'
                            ? theme.status.success
                            : theme.text.secondary, children: [focusedSection === 'autoSkill' ? '› ' : '  ', t('Auto-skill: {{status}}', {
                                status: autoSkillOn ? t('on') : t('off'),
                            })] }), _jsxs(Text, { color: focusedSection === 'autoSkillConfirm'
                            ? theme.status.success
                            : theme.text.secondary, children: [focusedSection === 'autoSkillConfirm' ? '› ' : '  ', t('Confirm auto-skills before saving: {{status}}', {
                    ...
                    return (_jsxs(Box, { flexDirection: "row", children: [_jsxs(Text, { color: isSelected ? theme.status.success : undefined, children: [isSelected ? '› ' : '  ', index + 1, ". ", item.label] }), item.description ? (_jsx(Text, { color: theme.text.secondary, children: `  ${item.description}` })) : null] }, `${item.value}-${item.action}`));
