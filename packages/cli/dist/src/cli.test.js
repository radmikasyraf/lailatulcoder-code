...
import { chmodSync, copyFileSync, mkdirSync, mkdtempSync, readFileSync, realpathSync, renameSync, rmSync, statSync, utimesSync, writeFileSync, } from 'node:fs';
...
import { MCP_COMMANDS, TOP_LEVEL_COMMANDS, handleCriticalError, isExpectedPtyRaceError, resolveBootstrapRoute, runCliEntry, runCliEntryPoint, stampCliEntryEnv, } from './cli.js';
...
describe('resolveBootstrapRoute', () => {
    ...
    it('keeps bundled entrypoint paths out of the route detection', async () => {
        ...
        expect(resolveBootstrapRoute(['C:\\repo\\dist\\cli.js', 'mcp', '--help'])).toBe('mcp');
    });
    it('falls back to the default route for normal interactive startup', async () => {
        expect(resolveBootstrapRoute([])).toBe('default');
        expect(resolveBootstrapRoute(['--model', 'gpt-4', 'Hello'])).toBe('default');
        expect(resolveBootstrapRoute(['--safe-mode', 'mcp', 'list'])).toBe('default');
    });
...
describe('runCliEntry', () => {
    const savedEnv = {
        ...
        QWEN_CODE_EXTERNAL_TOOL_GUARD_TOKEN: process.env['QWEN_CODE_EXTERNAL_TOOL_GUARD_TOKEN'],
        QWEN_CODE_MANAGED_NPM_UPDATE_VERSION: process.env['QWEN_CODE_MANAGED_NPM_UPDATE_VERSION'],
    };
    let stdout;
    let stderr;
    let savedExitCode;
...
describe('stampCliEntryEnv', () => {
    ...
    it.skipIf(process.platform === 'win32')('grants the execute bit tsc never emits, so the spawn filter passes the stamp', () => {
    ...
    it('derives the bin entry one level up from the compiled module', () => {
        ...
        expect(new URL('../index.js', 'file:///repo/packages/cli/dist/src/cli.js')
            .pathname).toBe('/repo/packages/cli/dist/index.js');
    });
...
describe('bootstrap import boundaries', () => {
    ...
    it('initializes profilers during bootstrap module evaluation', () => {
        const source = readFileSync('src/cli.ts', 'utf8');
        expect(source).toContain("import { initStartupProfiler } from './utils/startupProfiler.js'");
        expect(source).toContain("import { initCpuProfiler } from './utils/cpuProfiler.js'");
        expect(source.indexOf('initStartupProfiler();')).toBeLessThan(source.indexOf('export async function runCliEntry('));
        expect(source.indexOf('initCpuProfiler();')).toBeLessThan(source.indexOf('export async function runCliEntry('));
    });
    ...
    it('publishes the daemon compile cache without overriding user policy', () => {
        ...
        const unsupportedEntryPath = path.join(tempDir, 'unsupported-cli-entry.mjs');
        const probeEntryPath = path.join(tempDir, 'compile-cache-probe.mjs');
        try {
            copyFileSync('../../scripts/cli-entry.js', entryPath);
            writeFileSync(unsupportedEntryPath, readFileSync('../../scripts/cli-entry.js', 'utf8').replace("const { default: module } = await import('node:module');", 'const module = {};'));
            writeFileSync(probeEntryPath, [
            ...
            writeFileSync(path.join(tempDir, 'cli.js'), [
            ...
            const runEntry = (env, args = ['serve'], selectedEntryPath = entryPath) => JSON.parse(execFileSync(process.execPath, [selectedEntryPath, ...args], {
                encoding: 'utf8',
                env,
            }));
            const canEnableCompileCache = JSON.parse(execFileSync(process.execPath, [probeEntryPath], {
                encoding: 'utf8',
                env: baseEnv,
            }));
    ...
    it.skipIf(process.platform === 'win32')('reloads the CLI through a stable shim after an update', () => {
        ...
        try {
            mkdirSync(oldDir);
            mkdirSync(newDir);
            copyFileSync('../../scripts/cli-entry.js', path.join(oldDir, 'entry.mjs'));
            copyFileSync('../../scripts/cli-entry.js', path.join(newDir, 'entry.mjs'));
            writeFileSync(path.join(oldDir, 'cli.js'), `import { chmodSync, rmSync, writeFileSync } from 'node:fs';\nwriteFileSync(${JSON.stringify(binPath)}, ${JSON.stringify(`#!/bin/sh\nexec "${process.execPath}" "${path.join(newDir, 'entry.mjs')}" "$@"\n`)});\nchmodSync(${JSON.stringify(binPath)}, 0o755);\nrmSync(${JSON.stringify(oldDir)}, { recursive: true, force: true });\nprocess.exit(44);\n`);
            ...
            writeFileSync(path.join(wrongDir, 'qwen'), '#!/bin/sh\necho wrong-launcher\n');
    ...
    it('does not pass the standalone launcher hint to child processes', () => {
        ...
        try {
            copyFileSync('../../scripts/cli-entry.js', entryPath);
            writeFileSync(path.join(tempDir, 'cli.js'), 'process.stdout.write(JSON.stringify({ launcherPath: process.env.QWEN_CODE_LAUNCHER_PATH }));\n');
    ...
    it('ignores malformed relaunch args in the npm bin wrapper', () => {
        const output = execFileSync(process.execPath, ['../../scripts/cli-entry.js', '--version'], {
            encoding: 'utf8',
    ...
    it('resolves and pins managed updates from the configured home', () => {
        ...
        try {
            ...
            const packageRoot = path.join(launcherRoot, 'versions', '2.0.0', 'node_modules', '@lailatul-coder', 'lailatul-coder');
            mkdirSync(packageRoot, { recursive: true });
            writeFileSync(path.join(entryDir, 'cli.js'), "process.stdout.write(JSON.stringify({ build: 'base', pin: process.env.QWEN_CODE_MANAGED_NPM_PIN }));\n");
            writeFileSync(path.join(entryDir, 'package.json'), JSON.stringify({
            ...
            writeFileSync(path.join(packageRoot, 'cli.js'), "process.stdout.write(JSON.stringify({ build: 'managed-2', managed: process.env.QWEN_CODE_MANAGED_NPM_UPDATE, launcher: process.env.QWEN_CODE_CLI, pin: process.env.QWEN_CODE_MANAGED_NPM_PIN, args: process.argv.slice(2) }));\n");
            ...
            writeFileSync(path.join(tempDir, '.qwen', '.env'), '\uFEFFQWEN_HOME: ~\\custom\\qwen\n');
            const childEnv = {
                ...process.env,
                HOME: tempDir,
                USERPROFILE: tempDir,
            ...
            const baseSession = JSON.parse(execFileSync(process.execPath, [entryPath, '--prompt', 'hello'], {
                encoding: 'utf8',
                env: childEnv,
            }));
            expect(baseSession.build).toBe('base');
            writeFileSync(path.join(launcherRoot, 'active.json'), JSON.stringify({
                version: '2.0.0',
                bootstrap: realpathSync(entryPath),
                baseVersion: '1.0.0',
                bootstrapCtimeMs: bootstrapStat.ctimeMs,
            }));
            expect(JSON.parse(execFileSync(process.execPath, [entryPath, '--prompt', 'hello'], {
                encoding: 'utf8',
                env: {
                    ...childEnv,
            ...
            writeFileSync(path.join(tempDir, '.qwen', '.env'), `QWEN_HOME:${qwenHome}\n`);
            expect(JSON.parse(execFileSync(process.execPath, [entryPath, '--prompt', 'hello'], {
                encoding: 'utf8',
                env: childEnv,
            }))).toMatchObject({ build: 'base' });
            writeFileSync(path.join(tempDir, '.qwen', '.env'), `QWEN_HOME:   \nOTHER=${qwenHome}\n`);
            expect(JSON.parse(execFileSync(process.execPath, [entryPath, '--prompt', 'hello'], {
                encoding: 'utf8',
                env: childEnv,
            }))).toMatchObject({ build: 'base' });
            writeFileSync(path.join(tempDir, '.qwen', '.env'), '\uFEFFQWEN_HOME: ~\\custom\\qwen\n');
            const output = execFileSync(process.execPath, [entryPath, '--prompt', 'hello'], {
                encoding: 'utf8',
                env: childEnv,
            });
            const managedSession = JSON.parse(output);
            ...
            renameSync(path.join(qwenHome, 'updates'), path.join(emptyQwenHome, 'updates'));
            const emptyHomeEnv = {
                ...childEnv,
                HOME: '',
                USERPROFILE: '',
    ...
    it('falls through to cli.js when wrapper package.json lookup fails', () => {
        ...
        try {
            mkdirSync(entryDir);
            copyFileSync('../../scripts/cli-entry.js', path.join(entryDir, 'cli-entry.mjs'));
            writeFileSync(path.join(entryDir, 'cli.js'), "process.stdout.write('fallback-cli\\n');\n");
            const env = { ...process.env };
            delete env['CLI_VERSION'];
            const output = execFileSync(process.execPath, [path.join(entryDir, 'cli-entry.mjs'), '--version'], {
                encoding: 'utf8',
                env,
            });
    ...
    it('copies the npm bin wrapper into the package instead of duplicating it', () => {
        ...
        expect(source).toContain("fs.copyFileSync(path.join(__dirname, 'cli-entry.js'), cliEntryPath)");
    ...
    it('keeps bootstrap top-level help commands aligned with config registrations', () => {
        ...
        const bootstrapCommands = new Set(TOP_LEVEL_COMMANDS.map(([command]) => command.split(' ')[0]));
    ...
    it('keeps bootstrap MCP help commands aligned with MCP registrations', () => {
        ...
        const bootstrapCommands = new Set(MCP_COMMANDS.map(([command]) => command.split(' ')[0]));
        expect(registeredIdentifiers).toHaveLength(commandNameByIdentifier.size);
        for (const identifier of registeredIdentifiers) {
            const commandName = commandNameByIdentifier.get(identifier);
            expect(commandName, `missing mapping for ${identifier}`).toBeDefined();
...
describe('bootstrap error handling', () => {
    ...
    it('prints FatalError messages and exits with their code', async () => {
        ...
        await expect(handleCriticalError(new FatalError('fatal boom', 42))).rejects.toThrow('process.exit:42');
    ...
    it('prints FatalError messages in red when color is enabled', async () => {
        delete process.env['NO_COLOR'];
        await expect(handleCriticalError(new FatalError('fatal color', 42))).rejects.toThrow('process.exit:42');
        expect(stderr.join('')).toContain('\x1b[31mfatal color\x1b[0m');
    });
    it('exits AlreadyReportedError without printing another error', async () => {
        await expect(handleCriticalError(new AlreadyReportedError('already printed', 7))).rejects.toThrow('process.exit:7');
        expect(stderr.join('')).toBe('');
    });
    it('prints unexpected errors with the generic critical header', async () => {
        await expect(handleCriticalError(new Error('generic boom'))).rejects.toThrow('process.exit:1');
        const output = stderr.join('');
    ...
    it('recognizes expected PTY race errors', () => {
        expect(isExpectedPtyRaceError(Object.assign(new Error('read EIO'), { code: 'EIO' }))).toBe(true);
        expect(isExpectedPtyRaceError(new Error('read EAGAIN'))).toBe(true);
        expect(isExpectedPtyRaceError(new Error('Cannot resize a pty that has already exited'))).toBe(true);
    ...
    it('reports when the critical error handler itself fails', async () => {
        ...
        await expect(runCliEntryPoint(run, handleError)).rejects.toThrow('process.exit:1');
        const output = stderr.join('');
    ...
    it('wires stampCliEntryEnv into the entry point', () => {
        const source = readFileSync('src/cli.ts', 'utf8');
        const entryPoint = source.slice(source.indexOf('export async function runCliEntryPoint'));
        ...
        expect(entryPoint.indexOf('stampCliEntryEnv()')).toBeLessThan(entryPoint.indexOf('await run()'));
    });
});
