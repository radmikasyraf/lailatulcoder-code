...
function createMockSettings(setValue = vi.fn()) {
    return {
        merged: {},
        user: { settings: {} },
        workspace: { settings: {} },
...
describe('modelCommand', () => {
    ...
    it('should have the correct name and description', () => {
        ...
        expect(modelCommand.description).toBe('Switch the model for this session (--fast for suggestion model, --voice for voice transcription model, --vision for the vision bridge model, --compaction for chat compression model, --image for the image generation model, --project to persist to project settings, --global to persist to user settings, [model-id] to switch immediately, or [model-id] [prompt] to run a one-off prompt on another model; the inline prompt is sent verbatim without @file expansion).');
    });
    ...
    it('should complete compaction-eligible models for --compaction flag', async () => {
        ...
        const result = await modelCommand.completion(mockContext, '--compaction qw');
    ...
    it('should return dialog action for unsupported auth types', async () => {
        const mockConfig = createMockConfig({
            model: 'test-model',
            authType: 'UNSUPPORTED_AUTH_TYPE',
        });
        mockContext.services.config = mockConfig;
        const result = await modelCommand.action(mockContext, '');
        expect(result).toEqual({
    ...
    it('should switch the main model directly in interactive mode when args are provided', async () => {
        ...
        expect(switchModel).toHaveBeenCalledWith(AuthType.QWEN_OAUTH, 'qwen-max', undefined);
        expect(setValue).toHaveBeenCalledWith(expect.any(String), 'model.name', 'qwen-max');
        ...
        expect(setValue).toHaveBeenCalledWith(expect.any(String), 'model.baseUrl', '');
        expect(result).toEqual({
            type: 'message',
            messageType: 'info',
    ...
    it('runs a trailing prompt on the given model inline without switching or persisting', async () => {
        ...
        const result = await modelCommand.action(mockContext, 'qwen-max explain this code');
    ...
    it('rejects an inline prompt when the model id is not available', async () => {
        ...
        const result = await modelCommand.action(mockContext, 'missing-model hello');
        expect(switchModel).not.toHaveBeenCalled();
    ...
    it('rejects an inline prompt that targets a different provider', async () => {
        ...
        const result = await modelCommand.action(mockContext, 'gpt-4(openai) hello');
        expect(switchModel).not.toHaveBeenCalled();
    ...
    it('runs an inline prompt when the model matches the active provider endpoint/credentials', async () => {
        ...
        const result = await modelCommand.action(mockContext, 'shared-id hello there');
        expect(switchModel).not.toHaveBeenCalled();
        expect(result).toEqual({
            type: 'submit_prompt',
            content: 'hello there',
    ...
    it('rejects an inline prompt in ACP mode (no per-turn override pipeline)', async () => {
        ...
        const result = await modelCommand.action(mockContext, 'qwen-max explain this');
        expect(switchModel).not.toHaveBeenCalled();
        expect(result).toMatchObject({ type: 'message', messageType: 'error' });
    });
    it('should not persist the model when direct model validation fails', async () => {
        ...
        expect(result).toEqual({
            type: 'message',
            messageType: 'error',
            content: "Model 'missing-model' is not available for auth type 'qwen-oauth'.\n" +
    ...
    it('should not persist the model when direct model switching fails after validation', async () => {
        ...
        await expect(modelCommand.action(mockContext, 'qwen-max')).rejects.toThrow('Refresh failed');
        expect(switchModel).toHaveBeenCalledWith(AuthType.QWEN_OAUTH, 'qwen-max', undefined);
        expect(setValue).not.toHaveBeenCalled();
    });
    it('should explain how to configure models when direct switching fails', async () => {
        ...
        const result = await modelCommand.action(mockContext, 'definitely-not-a-model');
        expect(switchModel).not.toHaveBeenCalled();
        expect(setValue).not.toHaveBeenCalled();
        expect(result).toEqual({
            type: 'message',
            messageType: 'error',
            content: "Model 'definitely-not-a-model' is not available for auth type 'openai'.\n" +
    ...
    it('should explain when no models are configured for direct switching', async () => {
        ...
        const switchModel = vi
            .fn()
            .mockRejectedValue(new Error("Model 'gpt-4o' not found for authType 'openai'"));
        mockContext = createMockCommandContext({
            invocation: {
                raw: '/model gpt-4o',
                name: 'model',
        ...
        expect(result).toEqual({
            type: 'message',
            messageType: 'error',
            content: "Model 'gpt-4o' is not available for auth type 'openai'.\n" +
    ...
    it('should switch provider-qualified models through switchModel', async () => {
        ...
        const result = await modelCommand.action(mockContext, `gpt-4(${AuthType.USE_OPENAI})`);
        expect(switchModel).toHaveBeenCalledWith(AuthType.USE_OPENAI, 'gpt-4', undefined);
        expect(setValue).toHaveBeenCalledWith(expect.any(String), 'security.auth.selectedType', AuthType.USE_OPENAI);
        expect(setValue).toHaveBeenCalledWith(expect.any(String), 'model.name', 'gpt-4');
        expect(result).toEqual({
            type: 'message',
            messageType: 'info',
            content: 'Model: gpt-4',
    ...
    it('should set fast models configured under another auth type', async () => {
        ...
        const result = await modelCommand.action(mockContext, '--fast deepseek-v4-flash');
        expect(setValue).toHaveBeenCalledWith(expect.any(String), 'fastModel', 'deepseek-v4-flash');
    ...
    it('should set authType-qualified fast model selectors', async () => {
        ...
        mockContext = createMockCommandContext({
            ...
            services: {
                config: {
                    ...
                    getAvailableModelsForAuthType: vi.fn((authType) => authType === AuthType.USE_OPENAI
                        ? [
                            {
                                id: 'deepseek-v4-flash',
                                label: 'deepseek-v4-flash',
        ...
        const result = await modelCommand.action(mockContext, '--fast openai:deepseek-v4-flash');
        expect(setValue).toHaveBeenCalledWith(expect.any(String), 'fastModel', 'openai:deepseek-v4-flash');
    ...
    it('should reject unavailable fast models across all auth types', async () => {
        ...
        const result = await modelCommand.action(mockContext, '--fast missing-model');
        expect(setValue).not.toHaveBeenCalled();
        expect(setFastModel).not.toHaveBeenCalled();
        expect(result).toEqual({
            type: 'message',
            messageType: 'error',
            content: "Fast model 'missing-model' is not configured for any auth type.\n" +
    ...
    it('should reject unavailable authType-qualified fast models', async () => {
        ...
        const result = await modelCommand.action(mockContext, '--fast openai:missing-model');
        expect(setValue).not.toHaveBeenCalled();
        expect(setFastModel).not.toHaveBeenCalled();
        expect(result).toEqual({
            type: 'message',
            messageType: 'error',
            content: "Fast model 'missing-model' is not available for auth type 'openai'.\n" +
                "Available models for 'openai': gpt-4.\n" +
                'Configure models in settings.modelProviders and ensure the required environment variables are set. In interactive mode, run /auth to configure or switch providers, or run /model --fast without a model to choose from configured models.',
        });
    });
        ...
        mockContext = createMockCommandContext({
            ...
            services: {
                config: {
                    ...
                    isCurrentPrimaryModel: (m) => m.id === 'qwen-plus',
                    setVisionModel,
                },
                settings: createMockSettings(setValue),
            },
        });
        const result = await modelCommand.action(mockContext, '--vision qwen-vl-max');
        expect(setValue).toHaveBeenCalledWith(expect.any(String), 'visionModel', 'openai:qwen-vl-max\0https://vision.example.com/v1');
        expect(setVisionModel).toHaveBeenCalledWith('openai:qwen-vl-max\0https://vision.example.com/v1');
        expect(result).toEqual({
            type: 'message',
            messageType: 'info',
        ...
        mockContext = createMockCommandContext({
            ...
            services: {
                config: {
                    ...
                    isCurrentPrimaryModel: (m) => m.id === 'claude-opus-4-7',
                    setVisionModel,
                },
                settings: createMockSettings(setValue),
            },
        });
        const result = await modelCommand.action(mockContext, '--vision openai:qwen-vl-max');
        expect(setValue).toHaveBeenCalledWith(expect.any(String), 'visionModel', 'openai:qwen-vl-max');
    ...
    it('still sets a non-image-capable vision model but warns', async () => {
        ...
        const result = await modelCommand.action(mockContext, '--vision qwen3.7-max');
        // The pin is still honored...
        expect(setValue).toHaveBeenCalledWith(expect.any(String), 'visionModel', 'openai:qwen3.7-max');
    ...
    it('rejects pinning the current primary model as the vision bridge', async () => {
        ...
        const result = await modelCommand.action(mockContext, '--vision qwen-plus');
        const msg = result;
        expect(msg.messageType).toBe('error');
    ...
    it('should reject unavailable vision models across all auth types', async () => {
        ...
        const result = await modelCommand.action(mockContext, '--vision missing-model');
        expect(setVisionModel).not.toHaveBeenCalled();
        expect(setValue).not.toHaveBeenCalled();
        expect(result).toEqual({
            type: 'message',
            messageType: 'error',
            content: "Vision model 'missing-model' is not configured for any auth type.\n" +
    ...
    it('should reject an authType-qualified vision model missing from that provider', async () => {
        ...
        const result = await modelCommand.action(mockContext, '--vision openai:ghost-model');
        expect(setVisionModel).not.toHaveBeenCalled();
        expect(setValue).not.toHaveBeenCalled();
        expect(result).toEqual({
            type: 'message',
            messageType: 'error',
            content: "Vision model 'ghost-model' is not available for auth type 'openai'.\n" +
    ...
    it('should return current vision model outside interactive mode', async () => {
        ...
        expect(result).toEqual({
            type: 'message',
            messageType: 'info',
            content: 'Current vision model: qwen-vl-max (https://vision.example.com/v1)\nUse "/model --vision <model-id>" to set the vision bridge model.',
        });
    });
    it('should show a malformed vision model setting without hiding the empty selector', async () => {
        ...
        expect(result).toEqual({
            type: 'message',
            messageType: 'info',
            content: 'Current vision model: \\0https://vision.example.com/v1\nUse "/model --vision <model-id>" to set the vision bridge model.',
        });
    });
    ...
    it('should return the current image model outside interactive mode', async () => {
        ...
        expect(result).toEqual({
            type: 'message',
            messageType: 'info',
            content: 'Current image model: openai:qwen-image-2.0 (https://images.example.com/api/v1)\nUse "/model --image <model-id>" to set the image generation model.',
        });
    });
    it('should set an imageOnly model and hot-register its tool', async () => {
        ...
        const result = await modelCommand.action(mockContext, '--image qwen-image-2.0');
        const persisted = `openai:qwen-image-2.0\0${baseUrl}`;
        expect(setValue).toHaveBeenCalledWith(expect.any(String), 'imageModel', persisted);
    ...
    it('should reject a chat model from /model --image', async () => {
        ...
        expect(result).toEqual({
            type: 'message',
            messageType: 'error',
            content: expect.stringContaining("Image model 'qwen-plus' is not configured"),
        });
    });
    ...
    it('should return current voice model outside interactive mode', async () => {
        mockContext = createMockCommandContext({
            ...
            services: {
                ...
                settings: {
                    merged: { voiceModel: 'qwen3-asr-flash' },
                },
            },
        });
        const result = await modelCommand.action(mockContext, '--voice');
        expect(result).toEqual({
            type: 'message',
            messageType: 'info',
            content: 'Current voice model: qwen3-asr-flash\nUse "/model --voice <model-id>" to set voice model.',
        });
    });
    it('should set voice model without switching the main model', async () => {
        ...
        expect(setValue).toHaveBeenCalledWith(expect.any(String), 'voiceModel', 'qwen3-asr-flash');
        expect(switchModel).not.toHaveBeenCalled();
        expect(setValue).not.toHaveBeenCalledWith(expect.any(String), 'model.name', expect.any(String));
        expect(setValue).not.toHaveBeenCalledWith(expect.any(String), 'fastModel', expect.any(String));
        expect(result).toEqual({
            type: 'message',
            messageType: 'info',
    ...
    it('should reject unavailable voice models', async () => {
        ...
        const result = await modelCommand.action(mockContext, '--voice missing-model');
        expect(setValue).not.toHaveBeenCalled();
        expect(result).toEqual({
            type: 'message',
            messageType: 'error',
            content: "Voice model 'missing-model' is not configured.\n" +
    ...
    it('should reject voice models that cannot use the transcription endpoint', async () => {
        ...
        const result = await modelCommand.action(mockContext, '--voice qwen3-coder');
        expect(result).toEqual({
            type: 'message',
            messageType: 'error',
            content: "Voice model 'qwen3-coder' cannot be used for transcription. Configure an OpenAI-compatible model with baseUrl in settings.modelProviders.",
        });
        expect(setValue).not.toHaveBeenCalled();
    });
    it('should reject non OpenAI-compatible voice models', async () => {
        ...
        const result = await modelCommand.action(mockContext, '--voice claude-sonnet');
        expect(result).toEqual({
            type: 'message',
            messageType: 'error',
            content: "Voice model 'claude-sonnet' cannot be used for transcription. Configure an OpenAI-compatible model with baseUrl in settings.modelProviders.",
        });
        expect(setValue).not.toHaveBeenCalled();
    });
    it('should reject duplicate voice model ids as ambiguous', async () => {
        ...
        expect(result).toEqual({
            type: 'message',
            messageType: 'error',
            content: "Voice model 'qwen3-asr-flash' is ambiguous. Configure a unique model id before using /model --voice.",
        });
    });
    it('should treat colon-containing voice model values as literal ids', async () => {
        ...
        const result = await modelCommand.action(mockContext, '--voice openai:qwen3-asr-flash');
        expect(setValue).not.toHaveBeenCalled();
        expect(result).toMatchObject({
            type: 'message',
            messageType: 'error',
            content: expect.stringContaining("Voice model 'openai:qwen3-asr-flash' is not configured."),
        });
    });
    it('should not treat model IDs prefixed with --fast as the --fast flag', async () => {
        ...
        expect(switchModel).toHaveBeenCalledWith(AuthType.USE_OPENAI, '--fast-model', undefined);
        expect(setValue).toHaveBeenCalledWith(expect.any(String), 'model.name', '--fast-model');
        expect(result).toEqual({
            type: 'message',
            messageType: 'info',
    ...
    describe('--compaction handler', () => {
        it('should set compaction model', async () => {
            ...
            const result = await modelCommand.action(mockContext, '--compaction compaction-model');
            expect(setValue).toHaveBeenCalledWith(expect.any(String), 'compactionModel', 'compaction-model');
        ...
        it('should reject unavailable compaction models', async () => {
            ...
            const result = await modelCommand.action(mockContext, '--compaction missing-model');
        ...
        it('should show fallback info when compaction model is not set', async () => {
            ...
            expect(result).toEqual({
                type: 'message',
                messageType: 'info',
                content: 'Current compaction model: not set (falls back to the main model)\nUse "/model --compaction <model-id>" to set compaction model, or "/model --compaction clear" to clear the override.',
            });
        });
        it('should clear compaction model override with --compaction clear', async () => {
            ...
            const result = await modelCommand.action(mockContext, '--compaction clear');
            expect(setValue).toHaveBeenCalledWith(expect.any(String), 'compactionModel', undefined);
    ...
    describe('non-interactive mode', () => {
        it('should use interactive-only wording for unavailable direct switches', async () => {
            ...
            expect(result).toEqual({
                type: 'message',
                messageType: 'error',
                content: "Model 'missing-model' is not available for auth type 'openai'.\n" +
                    "Available models for 'openai': gpt-4.\n" +
                    'Configure models in settings.modelProviders and ensure the required environment variables are set. In interactive mode, run /auth to configure or switch providers, or run /model without arguments to choose from configured models.',
            });
        });
    ...
    describe('selector-only model filtering', () => {
        ...
        it('should reject fastOnly models from --vision selection', async () => {
            ...
            const result = await modelCommand.action(mockContext, '--vision fast-model');
            expect(result).toMatchObject({
                type: 'message',
                messageType: 'error',
                content: expect.stringContaining('fast-model'),
        ...
        it('should reject voiceOnly models from --vision selection', async () => {
            ...
            const result = await modelCommand.action(mockContext, '--vision voice-model');
            expect(result).toMatchObject({
                type: 'message',
                messageType: 'error',
                content: expect.stringContaining('voice-model'),
    ...
    describe('scope flags', () => {
        function setupContext() {
            ...
            mockConfig.getAvailableModelsForAuthType = vi.fn().mockReturnValue([]);
            mockConfig.getAllConfiguredModels = vi.fn().mockReturnValue([]);
            mockContext.services.config = mockConfig;
            return mockContext;
        }
        ...
        it('should show scope suffix in fast model confirmation', async () => {
            ...
            const result = await modelCommand.action(ctx, '--project --fast qwen3-coder-flash');
            expect(result).toMatchObject({
                type: 'message',
                content: expect.stringContaining('(this project)'),
            });
            expect(setValue).toHaveBeenCalledWith(SettingScope.Workspace, 'fastModel', 'qwen3-coder-flash');
        });
        it('should persist to global scope with --global', async () => {
            ...
            cfg.setFastModel = vi.fn();
            const result = await modelCommand.action(ctx, '--global --fast qwen3-coder-flash');
            expect(result).toMatchObject({
                type: 'message',
                content: expect.stringContaining('(global)'),
            });
            expect(setValue).toHaveBeenCalledWith(SettingScope.User, 'fastModel', 'qwen3-coder-flash');
        });
        ...
        it('should show scope suffix in main model confirmation', async () => {
            ...
            expect(setValue).toHaveBeenCalledWith(SettingScope.Workspace, 'model.name', 'qwen-max');
        });
...
//# sourceMappingURL=modelCommand.test.js.map