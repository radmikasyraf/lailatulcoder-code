...
function buildAuthProviderDescriptor(provider) {
    const steps = AUTH_PROVIDER_STEPS.filter((step) => shouldShowStep(provider, step));
    return {
        id: provider.id,
...
export function buildAuthProviderCatalog(workspaceCwd) {
    const providers = ALL_PROVIDERS.map(buildAuthProviderDescriptor);
    const providerIdsByGroup = (group) => providers
        ...
        groups: [
            {
                ...
                description: 'Official recommended setup: Coding Plan, Token Plan, or Standard API Key',
...
function parsePositiveBoundedInteger(value, max) {
...
function parseIPv4MappedHexSuffix(suffix) {
...
function parseIPv6FirstHextet(host) {
    const first = host.split(':', 1)[0];
    if (!first || !/^[0-9a-f]{1,4}$/i.test(first))
        return undefined;
...
function parseLegacyIPv4Host(host) {
    ...
    if (maxLastPart === undefined)
        return undefined;
    const parsed = parts.map((part, index) => parseLegacyIPv4Part(part, index === parts.length - 1 ? maxLastPart : 0xff));
    if (parsed.some((part) => part === undefined))
        return undefined;
    const values = parsed;
    const numeric = values.length === 1
        ? values[0]
        : values.length === 2
...
export function isBlockedAuthProviderHost(hostname) {
    ...
    if (ipVersion === 6) {
        if (blocklistHost === '::' || blocklistHost === '::1')
            return true;
...
function parseAuthProviderBaseUrl(value, allowPrivateBaseUrl) {
...
export function parseAuthProviderInstallRequest(body, options) {
    ...
    const baseUrl = parseAuthProviderBaseUrl(body['baseUrl'], options?.allowPrivateBaseUrl === true);
    if (baseUrl === null) {
        return {
            ok: false,
            code: 'invalid_base_url',
            error: '`baseUrl` must be an http(s) URL without credentials or blocked private-network host',
        };
    }
    const modelIds = parseStringArray(body['modelIds']);
    const rawAdvanced = body['advancedConfig'] && typeof body['advancedConfig'] === 'object'
        ? body['advancedConfig']
        : undefined;
    const rawMultimodal = rawAdvanced?.['multimodal'] && typeof rawAdvanced['multimodal'] === 'object'
        ? rawAdvanced['multimodal']
        : undefined;
    const contextWindowSize = parsePositiveBoundedInteger(rawAdvanced?.['contextWindowSize'], 10_000_000);
    const maxTokens = parsePositiveBoundedInteger(rawAdvanced?.['maxTokens'], 10_000_000);
...
//# sourceMappingURL=auth-provider-helpers.js.map