...
import { FileHistoryService } from '@lailatul-coder/lailatul-coder-core';
...
const userTurn = (id, text, promptId) => ({
    id,
    type: 'user',
    text,
    promptId,
...
describe('RewindSelector', () => {
    ...
    it('navigates the pick list with arrow keys and cancels with Escape', () => {
        ...
        const { lastFrame } = render(_jsx(RewindSelector, { history: [userTurn(1, 'first prompt'), userTurn(2, 'second prompt')], onRewind: vi.fn(), onCancel: onCancel, fileCheckpointingEnabled: false, fileHistoryService: fileHistoryService }));
        expect(lastFrame()).toContain('› #2 second prompt');
        pressKey({ name: 'up' });
        expect(lastFrame()).toContain('› #1 first prompt');
        pressKey({ name: 'down' });
    ...
    it('renders restore options with diff stats for the selected turn', async () => {
        ...
        const { lastFrame } = render(_jsx(RewindSelector, { history: [
                userTurn(1, 'first prompt', 'prompt-1'),
                userTurn(2, 'second prompt', 'prompt-2'),
            ], onRewind: vi.fn(), onCancel: vi.fn(), fileCheckpointingEnabled: true, fileHistoryService: fileHistoryService }));
    ...
    it('falls back to conversation-only options when diff stats fail', async () => {
        vi.spyOn(fileHistoryService, 'getDiffStats').mockRejectedValue(new Error('diff unavailable'));
        const { lastFrame } = render(_jsx(RewindSelector, { history: [
                userTurn(1, 'first prompt', 'prompt-1'),
                userTurn(2, 'second prompt', 'prompt-2'),
            ], onRewind: vi.fn(), onCancel: vi.fn(), fileCheckpointingEnabled: true, fileHistoryService: fileHistoryService }));
    ...
    it('does not request diff stats when the selected turn has no prompt id', async () => {
        ...
        const { lastFrame } = render(_jsx(RewindSelector, { history: [userTurn(1, 'first prompt'), userTurn(2, 'second prompt')], onRewind: vi.fn(), onCancel: vi.fn(), fileCheckpointingEnabled: true, fileHistoryService: fileHistoryService }));
        pressKey({ name: 'return' });
        await flush();
    ...
    it('confirms a conversation-only rewind when checkpointing is disabled', () => {
        ...
        const { lastFrame } = render(_jsx(RewindSelector, { history: [userTurn(1, 'first prompt'), userTurn(2, 'second prompt')], onRewind: onRewind, onCancel: vi.fn(), fileCheckpointingEnabled: false, fileHistoryService: fileHistoryService }));
        pressKey({ name: 'return' });
        expect(lastFrame()).toContain('This will remove all conversation after this turn.');
        pressKey({ sequence: 'y' });
        expect(onRewind).toHaveBeenCalledWith(expect.objectContaining({ id: 2 }), 'conversation');
    });
    it.each([
        ['n', { sequence: 'n' }],
        ['Escape', { name: 'escape' }],
    ])('returns to the pick list when legacy confirm receives %s', (_label, key) => {
        const onRewind = vi.fn();
        const { lastFrame } = render(_jsx(RewindSelector, { history: [userTurn(1, 'first prompt'), userTurn(2, 'second prompt')], onRewind: onRewind, onCancel: vi.fn(), fileCheckpointingEnabled: false, fileHistoryService: fileHistoryService }));
        pressKey({ name: 'return' });
        expect(lastFrame()).toContain('This will remove all conversation after this turn.');
        ...
        expect(lastFrame()).toContain('› #2 second prompt');
        expect(lastFrame()).not.toContain('This will remove all conversation after this turn.');
    });
    it('blocks restore-option keypresses while rewind is pending', async () => {
        ...
        const onCancel = vi.fn();
        let resolveRewind;
        const onRewind = vi.fn(() => new Promise((resolve) => {
            resolveRewind = resolve;
        }));
        const { lastFrame } = render(_jsx(RewindSelector, { history: [
                userTurn(1, 'first prompt', 'prompt-1'),
                userTurn(2, 'second prompt', 'prompt-2'),
            ], onRewind: onRewind, onCancel: onCancel, fileCheckpointingEnabled: true, fileHistoryService: fileHistoryService }));
        pressKey({ name: 'return' });
        await flush();
        pressKey({ name: 'return' });
        await flush();
        expect(onRewind).toHaveBeenCalledWith(expect.objectContaining({ id: 2 }), 'both');
