...
import { AUTH_PREFLIGHT_ENV_KEYS, AUTH_PREFLIGHT_WAIVED_AUTH_TYPES, } from './acpAgent.js';
describe('AUTH_PREFLIGHT_ENV_KEYS drift detection', () => {
    it('covers every public AuthType value (either keyed or explicitly waived)', () => {
        const allAuthTypes = Object.values(AuthType);
        const keyed = new Set(Object.keys(AUTH_PREFLIGHT_ENV_KEYS));
        const uncovered = allAuthTypes.filter((authType) => !keyed.has(authType) && !AUTH_PREFLIGHT_WAIVED_AUTH_TYPES.has(authType));
    ...
    it('does not list waived auth types as keyed', () => {
        ...
        const overlap = keyed.filter((k) => AUTH_PREFLIGHT_WAIVED_AUTH_TYPES.has(k));
    ...
    it('every keyed entry has at least one env var candidate', () => {
        ...
        const empty = Object.entries(AUTH_PREFLIGHT_ENV_KEYS).filter(([, vars]) => vars.length === 0);
        expect(empty).toEqual([]);
    });
});
//# sourceMappingURL=authPreflight.test.js.map