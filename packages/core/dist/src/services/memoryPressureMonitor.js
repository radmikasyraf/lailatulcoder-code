...
import { recordMemoryUsage, recordCpuUsage, isPerformanceMonitoringActive, MemoryMetricType, } from '../telemetry/metrics.js';
...
export class RuntimeSampleRing {
    ...
    record(mem) {
        ...
        const cpuPercent = Math.min(100, Math.max(0, ((cpuTotalUs / (elapsed * 1000)) * 100) / getCpuCoreCount()));
...
// Validation
export function validateMemoryPressureConfig(c) {
...
export class MemoryPressureMonitor extends EventEmitter {
    ...
    hasLoggedSamplingError = false;
    activeCleanupAction = 'none';
    lastCleanupAction = 'none';
    queuedCleanupRecommendation;
    lastCleanupTime = 0;
    consecutiveCleanupFailures = 0;
    consecutiveIneffectiveCleanups = 0;
    consecutiveIneffectiveAggressiveCleanups = 0;
    cleanupGeneration = 0;
    effectiveMemoryLimit;
    diagnosticsDumper;
    runtimeSamples = new RuntimeSampleRing();
    constructor(coreConfig, pressureConfig) {
        super();
        ...
        debugLogger.info(`Effective memory limit: ${formatMiB(this.effectiveMemoryLimit)} MiB; ` +
            `V8 heap limit: ${formatMiB(heapSizeLimit)} MiB`);
        if (this.effectiveMemoryLimit <= 0) {
            debugLogger.warn('Effective memory limit is not positive; RSS pressure checks are disabled');
        }
    }
    // Public API
    ...
    performCheck() {
        try {
            ...
            debugLogger.error(`Memory pressure check failed: ${getErrorMessage(err)}`);
        }
    }
    performCheckInternal() {
        ...
        if (recommendation.action === 'none')
            return;
        const now = Date.now();
        const isEscalation = cleanupActionRank(recommendation.action) >
    ...
    readMemoryUsage() {
        try {
            ...
            debugLogger.error(`Failed to read memory usage for pressure check: ${getErrorMessage(err)}`);
            return undefined;
        }
    }
    /**
     ...
     */
    getPressureLevel(memSnapshot) {
        const mem = memSnapshot ?? this.readMemoryUsage();
        if (!mem)
            return 'normal';
        const rssRatio = this.effectiveMemoryLimit > 0 ? mem.rss / this.effectiveMemoryLimit : 0;
        const heapSizeLimit = getHeapStatistics().heap_size_limit;
        const heapRatio = heapSizeLimit > 0 ? mem.heapUsed / heapSizeLimit : 0;
        const ratio = Math.max(rssRatio, heapRatio);
        if (ratio >= this.config.criticalRatio)
            return 'critical';
        if (ratio >= this.config.hardPressureRatio)
            return 'hard';
        if (ratio >= this.config.softPressureRatio)
            return 'soft';
        return 'normal';
    }
    // Cleanup
    recommendCleanup(pressure) {
        switch (pressure) {
            case 'critical':
                return {
                    action: 'aggressive',
                    steps: [
                        ...
                        'compact_history',
                        'clear_file_cache',
                        ...(this.config.enableExplicitGC ? ['trigger_gc'] : []),
                    ],
                };
            case 'hard':
                return {
    ...
    executeCleanup(recommendation) {
        if (this.cleanupInProgress) {
            ...
            if (recommendationRank > activeRank && recommendationRank > queuedRank) {
                ...
                debugLogger.debug(`Queued escalated cleanup "${recommendation.action}" while ` +
        ...
        void this.runCleanupSteps(recommendation.steps, cleanupGeneration)
            ...
            setImmediate(() => {
                ...
                try {
                    ...
                    debugLogger.error(`Cleanup measurement failed: ${getErrorMessage(err)}`);
                }
                finally {
    ...
    finishCleanupAndRunQueued(cleanupGeneration) {
        if (cleanupGeneration !== this.cleanupGeneration) {
            return;
        }
        this.cleanupInProgress = false;
    ...
    getCleanupCooldownMs(action) {
        ...
        const exponent = Math.min(this.consecutiveIneffectiveAggressiveCleanups - 2, 6);
        return baseCooldownMs * 2 ** exponent;
    }
    logCleanupResult(memBefore, memAfter, recommendation) {
        const freed = memBefore - memAfter;
        const freedRatio = memBefore > 0 ? freed / memBefore : 0;
        debugLogger.info(`Cleanup "${recommendation.action}" completed; RSS delta ${freed} bytes ` +
        ...
        if (freedRatio < 0.01) {
            ...
            if (shouldEmitRepeatedDiagnostic(this.consecutiveIneffectiveCleanups)) {
                ...
                debugLogger.warn(`Cleanup "${recommendation.action}" has been ineffective ` +
    ...
    recordCleanupFailure(recommendation, err) {
        ...
        try {
            ...
            debugLogger.error(`Failed to read RSS after cleanup failure: ${getErrorMessage(rssErr)}`);
        }
        this.consecutiveCleanupFailures++;
        debugLogger.error(`Cleanup "${recommendation.action}" failed: ${error}; ` +
    ...
    async runCleanupSteps(steps, cleanupGeneration) {
        ...
        switch (step) {
            ...
            case 'trigger_gc': {
                if (typeof global.gc === 'function') {
                    ...
                    debugLogger.warn('trigger_gc requested but global.gc is not available; ' +
            ...
            case 'compact_history': {
                try {
                    ...
                    if (!client?.isInitialized?.()) {
                        debugLogger.debug('[COMPACT_HISTORY] skipped: client not initialized');
                        break;
                    }
                    ...
                    const result = microcompactHistory(history, Date.now() - 1, {
                        ...settings,
                        toolResultsThresholdMinutes: (settings.toolResultsThresholdMinutes ?? 0) < 0
                    ...
                    if (result.meta) {
                        ...
                        this.coreConfig.getFileReadCache().clear();
                        const m = result.meta;
                        debugLogger.debug(`[COMPACT_HISTORY] cleared ${m.toolsCleared} tool result(s) ` +
                    ...
                    debugLogger.error(`[COMPACT_HISTORY] failed: ${getErrorMessage(err)}`);
                }
                break;
            }
            default:
    ...
    // Memory metrics
    computeEffectiveMemoryLimit() {
        const hostTotal = os.totalmem();
        const cgroupV2Limit = this.readCgroupMemoryLimit('/sys/fs/cgroup/memory.max', hostTotal);
        if (cgroupV2Limit !== undefined) {
            debugLogger.info(`Using cgroup v2 memory limit: ${formatMiB(cgroupV2Limit)} MiB`);
            return cgroupV2Limit;
        }
        const cgroupV1Limit = this.readCgroupMemoryLimit('/sys/fs/cgroup/memory/memory.limit_in_bytes', hostTotal);
        if (cgroupV1Limit !== undefined) {
            debugLogger.info(`Using cgroup v1 memory limit: ${formatMiB(cgroupV1Limit)} MiB`);
            return cgroupV1Limit;
        }
        debugLogger.info(`Using host memory limit: ${formatMiB(hostTotal)} MiB`);
        return hostTotal;
    }
    readCgroupMemoryLimit(filePath, hostTotal) {
        try {
            const raw = readFileSync(filePath, 'utf-8').trim();
            if (raw === 'max')
                return undefined;
            if (!/^-?\d+$/.test(raw)) {
                debugLogger.warn(`Ignoring non-numeric cgroup memory limit from ${filePath}: ${raw}`);
                return undefined;
            }
            const limit = Number(raw);
            if (!Number.isFinite(limit) || limit <= 0) {
                debugLogger.warn(`Ignoring out-of-range cgroup memory limit from ${filePath}: ${raw}`);
                return undefined;
            }
            if (limit < MIN_CGROUP_MEMORY_LIMIT) {
                debugLogger.warn(`Ignoring unrealistically small cgroup memory limit from ` +
                    `${filePath}: ${raw}`);
                return undefined;
            }
            // cgroup v1 represents "unlimited" with huge sentinel values.
            if (!Number.isSafeInteger(limit)) {
                debugLogger.debug(`Ignoring unlimited cgroup memory limit from ${filePath}: ${raw}`);
                return undefined;
            }
            if (hostTotal > 0 && limit > hostTotal) {
                debugLogger.debug(`Ignoring cgroup memory limit above host total from ${filePath}: ` +
                    `${raw}`);
                return undefined;
            }
            return limit;
...
function cleanupActionRank(action) {
    switch (action) {
        case 'aggressive':
            return 3;
        case 'moderate':
...
function shouldEmitRepeatedDiagnostic(count) {
