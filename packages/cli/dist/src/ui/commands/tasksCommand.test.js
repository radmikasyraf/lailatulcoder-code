...
describe('tasksCommand', () => {
    ...
    it('lists running and terminal shell entries with status / runtime / output path', async () => {
        ...
        expect(result.content).toContain('output: /tmp/tasks/sess/shell-bg_done.output');
    });
    it('includes background agent entries alongside shells', async () => {
        ...
        expect(result.content).toContain('researcher: Fix flaky test and send patch');
        expect(result.content).toContain('output: /tmp/tasks/sess/agent_run.jsonl');
        expect(result.content).toContain('[agent_pause] paused (resume blocked): Subagent "researcher" is no longer available.');
    ...
    it('lists monitor entries with eventCount and exit / error suffixes', async () => {
        ...
        expect(result.content).toContain('[mon_auto] completed (Max events reached, 1000 events)');
        ...
        expect(result.content).toContain('[mon_fail] failed: spawn ENOENT (0 events)');
    ...
    it('shows the dialog hint only in interactive mode', async () => {
        ...
        if (!withHint || withHint.type !== 'message')
            throw new Error('no result');
    ...
    it('merges all three kinds and orders them by startTime', async () => {
        ...
        if (!result || result.type !== 'message')
            throw new Error('no result');
        const order = ['bg_early', 'mon_mid', 'a_late'].map((id) => result.content.indexOf(`[${id}]`));
...
//# sourceMappingURL=tasksCommand.test.js.map