...
import { SERVE_STATUS_EXT_METHODS, SERVE_CONTROL_EXT_METHODS, BridgeChannelClosedError, STATUS_SCHEMA_VERSION, createIdleWorkspaceMcpStatus, createIdleWorkspaceSkillsStatus, createIdleWorkspaceProvidersStatus, createIdleWorkspaceExtensionsStatus, createIdleWorkspaceHooksStatus, createIdleEnvStatus, createIdleAcpPreflightCells, mapDomainErrorToErrorKind, } from '@lailatul-coder/acp-bridge/status';
import { WorkspaceInitPathEscapeError, WorkspaceInitSymlinkError, WorkspaceInitConflictError, WorkspaceInitRaceError, McpServerNotFoundError, McpServerRestartFailedError, SessionNotFoundError, } from '@lailatul-coder/acp-bridge/bridgeErrors';
...
import { buildWorkspaceVoiceSettingsWrites, buildWorkspaceVoiceStatus, validateWorkspaceVoiceState, voiceSettingsScopeToWire, WorkspaceVoiceError, } from '../../services/voice-service.js';
import { writeStderrLine, writeStderrLineSafe, } from '../../utils/stdioHelpers.js';
import { deleteWorkspaceSkill, installWorkspaceSkill, WorkspaceSkillManagementError, } from '../workspace-skill-management.js';
import { mapWorkspaceSkillToggleError, WorkspacePermissionRulesSessionRequiredError, WorkspaceSkillNotFoundError, WorkspaceSkillNotToggleableError, WorkspaceSettingsPartialPersistError, } from './types.js';
export { WorkspacePermissionRulesSessionRequiredError, WorkspaceSkillNotFoundError, WorkspaceSkillNotToggleableError, mapWorkspaceSkillToggleError, } from './types.js';
// ---------------------------------------------------------------------------
// Helpers
// ---------------------------------------------------------------------------
...
async function verifyParentPostOpen(target, wsCanonical, _fh) {
    const parentCanonical = await canonicalizeExistingAncestor(path.dirname(target));
    const within = parentCanonical === wsCanonical ||
    ...
    throw new WorkspaceInitSymlinkError(target, 'parent', `Workspace context file ${JSON.stringify(target)}'s parent moved ` +
...
// Factory
// ---------------------------------------------------------------------------
export function createDaemonWorkspaceService(deps) {
    const { boundWorkspace, isWorkspaceTrusted, assertGenerationOpen, contextFilename, statusProvider, workspaceProvidersStatusProvider, workspaceSkillsStatusProvider, isChannelLive, persistDisabledTools, persistDisabledSkills, persistDisabledSkillsBatch, persistSetting, persistSettings, skillInstallEnv, voiceEnv, voiceSettingsScope, preheatAcpChild: preheatAcpChildOnBridge, queryWorkspaceStatus, invokeWorkspaceCommand, refreshExtensionsForAllSessions: refreshExtensionsForAllSessionsOnBridge, publishWorkspaceEvent, } = deps;
    ...
    const readWorkspaceSkillsStatus = async (generation) => {
        let status;
        try {
            status = await queryWorkspaceStatus(SERVE_STATUS_EXT_METHODS.workspaceSkills, () => createIdleWorkspaceSkillsStatus(boundWorkspace));
        }
        catch (err) {
            ...
            writeStderrLine(`LailatulCoder serve: getWorkspaceSkillsStatus query failed: ${err instanceof Error ? err.message : String(err)}`);
        ...
        if (workspaceSkillsStatusProvider) {
            try {
                ...
                writeStderrLine(`LailatulCoder serve: getWorkspaceSkillsStatus local provider failed: ${err instanceof Error ? err.message : String(err)}`);
            }
        }
        return status;
    };
    ...
    const refreshWorkspaceSkillsAfterMutation = async () => {
        invalidateWorkspaceSkillsSnapshot();
        if (!(isChannelLive?.() ?? false))
            return;
        try {
            const refreshed = await invokeWorkspaceCommand(SERVE_CONTROL_EXT_METHODS.workspaceSkillsRefresh, { cwd: boundWorkspace, reason: 'content' });
            ...
            if ((refreshed.configsFailed ?? 0) > 0) {
                writeStderrLine(`LailatulCoder serve: ${refreshed.configsFailed} skill cache refresh(es) failed after mutation`);
            }
        }
        catch (err) {
            if (!(err instanceof SessionNotFoundError) &&
                !(err instanceof BridgeChannelClosedError)) {
                writeStderrLine(`LailatulCoder serve: workspace skill refresh after mutation failed: ${err instanceof Error ? err.message : String(err)}`);
            }
        }
        finally {
            invalidateWorkspaceSkillsSnapshot();
    ...
    return {
        ...
        async getWorkspaceMcpStatus(_ctx) {
            return queryWorkspaceStatus(SERVE_STATUS_EXT_METHODS.workspaceMcp, () => createIdleWorkspaceMcpStatus(boundWorkspace));
        },
        ...
        async getWorkspaceProvidersStatus(_ctx) {
            if (workspaceProvidersStatusProvider) {
                return workspaceProvidersStatusProvider(boundWorkspace, isChannelLive?.() ?? false);
            }
            return queryWorkspaceStatus(SERVE_STATUS_EXT_METHODS.workspaceProviders, () => createIdleWorkspaceProvidersStatus(boundWorkspace));
        },
        async preheatAcpChild(_ctx, opts) {
            ...
            if (!inFlightAcpPreheat) {
                ...
                void promise.then(() => {
                    ...
                    try {
                        writeStderrLineSafe(`LailatulCoder serve: ACP preheat failed: ${err instanceof Error ? err.message : String(err)}`);
                    }
                    finally {
                        if (inFlightAcpPreheat === promise) {
                            inFlightAcpPreheat = undefined;
            ...
            try {
                await withTimeout(sharedPreheat, opts?.timeoutMs ?? 5_000, 'ACP preheat');
            }
            catch (err) {
                if (err instanceof TimeoutError) {
                    writeStderrLineSafe(`LailatulCoder serve: ACP preheat timed out after ${opts?.timeoutMs ?? 5_000}ms`);
                }
            ...
            const live = channelLive();
            if (!live) {
                writeStderrLineSafe('LailatulCoder serve: ACP preheat resolved without a live channel');
            }
            return live
        ...
        async getWorkspaceEnvStatus(_ctx) {
            ...
            try {
                return await statusProvider.getEnvStatus(boundWorkspace, acpChannelLive);
            }
            catch (err) {
                writeStderrLine(`LailatulCoder serve: getEnvStatus failed: ${err instanceof Error ? err.message : String(err)}`);
                return createIdleEnvStatus(boundWorkspace, acpChannelLive);
            }
        },
        async getWorkspacePreflightStatus(_ctx) {
            ...
            if (statusProvider) {
                try {
                    ...
                    writeStderrLine(`LailatulCoder serve: getDaemonPreflightCells failed: ${err instanceof Error ? err.message : String(err)}`);
                }
            }
            ...
            if (acpChannelLive) {
                try {
                    const acpResult = await queryWorkspaceStatus(SERVE_STATUS_EXT_METHODS.workspacePreflight, () => ({ cells: idleCells }));
        ...
        async getWorkspaceHooksStatus(_ctx) {
            return queryWorkspaceStatus(SERVE_STATUS_EXT_METHODS.workspaceHooks, () => createIdleWorkspaceHooksStatus(boundWorkspace));
        },
        async getWorkspaceExtensionsStatus(_ctx) {
            return queryWorkspaceStatus(SERVE_STATUS_EXT_METHODS.workspaceExtensions, () => createIdleWorkspaceExtensionsStatus(boundWorkspace));
        },
        async getWorkspaceTrustStatus(_ctx) {
            return getWorkspaceTrustStatus(loadBoundSettings(true).merged, boundWorkspace);
        },
        ...
        async getWorkspaceVoiceStatus(_ctx) {
            return buildWorkspaceVoiceStatus(boundWorkspace, loadBoundSettings(Boolean(voiceEnv)));
        },
        // -- Mutations --
        ...
        async setWorkspacePermissionRules(ctx, request) {
            ...
            try {
                const result = await invokeWorkspaceCommand('LailatulCoder/permissions/setRules', {
                    cwd: boundWorkspace,
                    scope: request.scope,
        ...
        async setWorkspaceVoiceSettings(ctx, request) {
            ...
            if (!persistSettings && !persistSetting) {
                throw new WorkspaceVoiceError(501, 'not_implemented', 'Workspace voice settings persistence is not available');
            }
            ...
            if (persistSettings) {
                ...
                for (const write of writes) {
                    try {
                        await persistSetting(boundWorkspace, write.scope, write.key, write.value, assertGenerationOpen);
                    }
                    catch (err) {
                        assertActiveGeneration();
                        writeStderrLine(`LailatulCoder serve: workspace voice partial persist error (workspace=${boundWorkspace}, committed=${committed.length}/${writes.length}, failedKey=${write.key}, failedScope=${voiceSettingsScopeToWire(write.scope)}): ${err instanceof Error ? err.message : String(err)}`);
        ...
        async setWorkspaceToolEnabled(ctx, toolName, enabled) {
            assertActiveGeneration();
            await persistDisabledTools(boundWorkspace, toolName, enabled, assertGenerationOpen);
            assertActiveGeneration();
            publishWorkspaceEvent({
                type: 'tool_toggled',
        ...
        async setWorkspaceSkillEnabled(ctx, requestedSkillName, enabled) {
            assertActiveGeneration();
            const normalizedName = requestedSkillName.trim().toLowerCase();
            const status = await getWorkspaceSkillsStatus();
            const skill = status.skills.find((candidate) => candidate.name.trim().toLowerCase() === normalizedName);
            if (!skill)
                throw new WorkspaceSkillNotFoundError(requestedSkillName);
            if (skill.userInvocable === false) {
                throw new WorkspaceSkillNotToggleableError(skill.name, 'not_user_invocable');
            }
            const needsLegacyInactiveCheck = skill.level === 'extension' &&
                skill.status === 'disabled' &&
                skill.disabledReason === undefined;
            const disabledBySettings = needsLegacyInactiveCheck &&
                resolveSkillSettings(loadBoundSettings(true)).disabledNames.has(normalizedName);
            if (skill.level === 'extension' &&
                skill.status === 'disabled' &&
                (skill.disabledReason === 'inactive_extension' ||
                    (skill.disabledReason === undefined && !disabledBySettings))) {
                throw new WorkspaceSkillNotToggleableError(skill.name, 'inactive_extension');
            }
            const persisted = await persistDisabledSkills(boundWorkspace, skill.name, enabled, assertGenerationOpen);
            assertActiveGeneration();
            const channelLive = isChannelLive?.() ?? false;
            let activation = channelLive
                ? 'applied'
                : 'deferred';
            ...
            if (persisted.changed) {
                ...
                if (channelLive) {
                    try {
                        const refreshed = await invokeWorkspaceCommand(SERVE_CONTROL_EXT_METHODS.workspaceSkillsRefresh, { cwd: boundWorkspace, reason: 'settings' });
                        assertActiveGeneration();
                        ...
                        if (err instanceof SessionNotFoundError ||
                            ...
                            activation = 'partial';
                            sessionsFailed = 1;
                            writeStderrLine(`LailatulCoder serve: workspace skill refresh failed: ${err instanceof Error ? err.message : String(err)}`);
                        }
                    }
                    invalidateWorkspaceSkillsSnapshot();
                }
                ...
                const settingsChanges = persisted.settingsChanges ?? [
                    {
                        key: 'skills.disabled',
                        value: persisted.disabled.length > 0 ? persisted.disabled : undefined,
                    },
                ];
        ...
        async setWorkspaceSkillsEnabled(ctx, requestedSkillNames, enabled) {
            ...
            const disabledNames = resolveSkillSettings(loadBoundSettings(true)).disabledNames;
            const targets = [];
            for (const requestedName of requestedSkillNames) {
                ...
                let domainError;
                if (skill.userInvocable === false) {
                    domainError = new WorkspaceSkillNotToggleableError(skill.name, 'not_user_invocable');
                }
                else {
                    const legacyInactive = skill.level === 'extension' &&
                        skill.status === 'disabled' &&
                        skill.disabledReason === undefined &&
                    ...
                    if (skill.level === 'extension' &&
                        ...
                        domainError = new WorkspaceSkillNotToggleableError(skill.name, 'inactive_extension');
                    }
                }
                if (domainError) {
            ...
            const validSkillNames = targets.flatMap((target) => 'skillName' in target ? [target.skillName] : []);
            const persisted = validSkillNames.length > 0
                ? await persistDisabledSkillsBatch(boundWorkspace, validSkillNames, enabled, assertGenerationOpen)
                : { outcomes: [], settingsChanges: [] };
            assertActiveGeneration();
            const persistedByName = new Map(persisted.outcomes.map((outcome) => [
            ...
            for (const target of targets) {
                ...
                const outcome = persistedByName.get(target.skillName.trim().toLowerCase());
                if (!outcome) {
                    throw new Error(`Missing persisted Skill batch outcome: ${target.skillName}`);
                }
                if ('error' in outcome) {
        ...
        async installWorkspaceSkill(_ctx, request) {
            assertActiveGeneration();
            const result = await installWorkspaceSkill(boundWorkspace, request, skillInstallEnv?.['GH_TOKEN'] ?? skillInstallEnv?.['GITHUB_TOKEN'], assertGenerationOpen);
            assertActiveGeneration();
            await refreshWorkspaceSkillsAfterMutation();
            assertActiveGeneration();
            return result;
        },
        async deleteWorkspaceSkill(_ctx, requestedSkillName, scope) {
            assertActiveGeneration();
            const normalizedName = requestedSkillName.trim().toLowerCase();
            const status = await getWorkspaceSkillsStatus();
            const skill = status.skills.find((candidate) => candidate.name.trim().toLowerCase() === normalizedName);
            ...
            if (skill.level !== expectedLevel || !skill.installedPath) {
                throw new WorkspaceSkillManagementError('skill_not_managed', 'Skill is not managed in the requested scope', 409);
            }
            const result = await deleteWorkspaceSkill(boundWorkspace, scope, skill.name, skill.installedPath, assertGenerationOpen);
            assertActiveGeneration();
            await refreshWorkspaceSkillsAfterMutation();
            assertActiveGeneration();
            return result;
        ...
        async initWorkspace(ctx, opts) {
            ...
            const parentCanonical = await canonicalizeExistingAncestor(path.dirname(target));
            const parentWithinWorkspace = parentCanonical === wsCanonical ||
                parentCanonical.startsWith(wsCanonical + path.sep);
            if (!parentWithinWorkspace) {
                throw new WorkspaceInitSymlinkError(target, 'parent', `Configured workspace context filename ${JSON.stringify(filename)} ` +
            ...
            try {
                ...
                if (lst.isSymbolicLink()) {
                    throw new WorkspaceInitSymlinkError(target, 'target', `Workspace context file ${JSON.stringify(target)} is a symlink. ` +
                        `Refusing to follow it for write — replace the symlink with a ` +
                        `regular file (or remove it) before re-running init.`);
                }
                if (!lst.isFile()) {
                    throw new WorkspaceInitSymlinkError(target, 'target', `Workspace context file ${JSON.stringify(target)} is not a regular file ` +
            ...
            if (action === 'created') {
                ...
                try {
                    ...
                    const code = err?.code;
                    if (code === 'EEXIST') {
                        throw new WorkspaceInitRaceError(target, 'eexist', `Workspace context file ${JSON.stringify(target)} appeared ` +
                ...
                try {
                    assertActiveGeneration();
                    overwriteFh = await fs.open(target, fsConstants.O_WRONLY | (fsConstants.O_NOFOLLOW ?? 0));
                }
                catch (err) {
                    const code = err?.code;
                    if (code === 'ELOOP') {
                        throw new WorkspaceInitSymlinkError(target, 'target', `Workspace context file ${JSON.stringify(target)} could not ` +
                    ...
                    if (code === 'ENOENT') {
                        throw new WorkspaceInitRaceError(target, 'enoent', `Workspace context file ${JSON.stringify(target)} was deleted ` +
        ...
        async restartMcpServer(ctx, serverName, opts) {
            ...
            let result;
            try {
                result = await invokeWorkspaceCommand(SERVE_CONTROL_EXT_METHODS.workspaceMcpRestart, params, { timeoutMs: MCP_RESTART_SERVER_DEADLINE_MS });
            }
            catch (err) {
                const data = err?.data;
            ...
            if ('entries' in result) {
                ...
                if (!Array.isArray(result.entries)) {
                    writeStderrLine(`LailatulCoder serve: pool restart response carried 'entries' field ` +
                ...
                for (const entry of entries) {
                    if (typeof entry !== 'object' ||
                        ...
                        writeStderrLine(`LailatulCoder serve: skipping malformed pool restart entry ` +
        ...
        async reload(ctx) {
            ...
            if (deps.reloadDaemonEnv) {
                try {
                    ...
                    writeStderrLine(`LailatulCoder serve: daemon reload failed: ${err instanceof Error ? err.message : String(err)}`);
                }
            }
            assertActiveGeneration();
            ...
            let childError;
            try {
                const childResult = await invokeWorkspaceCommand(SERVE_CONTROL_EXT_METHODS.workspaceReload, { cwd: boundWorkspace }, { timeoutMs: 30_000 });
                childReloaded = true;
                env = childResult.env;
                ...
                if (err instanceof SessionNotFoundError) {
                    ...
                    writeStderrLine(`LailatulCoder serve: reload failed: ${childError}`);
                }
            }
            assertActiveGeneration();
            publishWorkspaceEvent({
        ...
        async refreshExtensionsForAllSessions() {
            ...
            try {
                ...
                writeStderrLine(`LailatulCoder serve: refreshExtensionsForAllSessions failed: ${err instanceof Error ? err.message : String(err)}`);
