...
describe('roundModelIdFrom', () => {
    ...
    it('falls back to the bare id when the runtime publishes no identity', () => {
        expect(roundModelIdFrom({ QWEN_CODE_MODEL: 'qwen3-coder-plus' })).toBe('qwen3-coder-plus');
    });
    ...
    it('is empty only when NEITHER slot names anything', () => {
        ...
        expect(roundModelIdFrom({ QWEN_CODE_MODEL: '  ', QWEN_CODE_MODEL_IDENTITY: '' })).toBe('');
    });
});
describe('certifierMatchesRound', () => {
    ...
    it('ENGAGES on two bare ids — the qualifier is optional, not required', () => {
        ...
        expect(certifierMatchesRound('qwen3-coder-plus', 'qwen3-coder-plus')).toBe(true);
    });
...
//# sourceMappingURL=round-model.test.js.map