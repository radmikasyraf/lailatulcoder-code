...
import { WORKFLOW_SUBAGENT_SYSTEM_PROMPT, WORKFLOW_SUBAGENT_SYSTEM_PROMPT_WITH_SCHEMA, } from './workflow-prompts.js';
import { AgentTerminateMode } from './agent-types.js';
import { attachJsonlTranscriptWriter, buildAgentTranscriptAttach, } from '../agent-transcript.js';
import { AgentEventEmitter, AgentEventType } from './agent-events.js';
import { ToolNames } from '../../tools/tool-names.js';
import { parsePositiveIntegerEnv } from '../../utils/env.js';
import { stripAnsiAndControl } from '../../utils/textUtils.js';
import { GitWorktreeService, generateAgentWorktreeSlug, writeWorktreeSessionMarker, } from '../../services/gitWorktreeService.js';
 ...
 */
export function resolveMaxAgentsPerRun(env = process.env) {
    ...
    if (parsed < 1) {
        debugLogger.warn(`Invalid ${MAX_WORKFLOW_AGENTS_ENV}=${JSON.stringify(raw)}, ` +
            `using default (${DEFAULT_MAX_AGENTS_PER_RUN})`);
        return DEFAULT_MAX_AGENTS_PER_RUN;
    }
    if (parsed > HARD_MAX_AGENTS_PER_RUN_CEILING) {
        debugLogger.warn(`${MAX_WORKFLOW_AGENTS_ENV}=${parsed} exceeds hard ceiling ` +
            `(${HARD_MAX_AGENTS_PER_RUN_CEILING}); clamping.`);
        return HARD_MAX_AGENTS_PER_RUN_CEILING;
    }
    return parsed;
}
export const MAX_WORKFLOW_CONCURRENCY_ENV = 'QWEN_CODE_MAX_WORKFLOW_CONCURRENCY';
/**
 ...
 */
export function resolveConcurrencyLimit(env = process.env) {
    ...
    if (raw !== undefined && raw.trim() !== '') {
        ...
        if (parsed >= 1) {
            if (parsed > HARD_MAX_CONCURRENCY_CEILING) {
                debugLogger.warn(`${MAX_WORKFLOW_CONCURRENCY_ENV}=${parsed} exceeds hard ceiling ` +
                    `(${HARD_MAX_CONCURRENCY_CEILING}); clamping.`);
                return HARD_MAX_CONCURRENCY_CEILING;
            }
            return parsed;
        }
        debugLogger.warn(`Invalid ${MAX_WORKFLOW_CONCURRENCY_ENV}=${JSON.stringify(raw)}, ` +
...
export const WORKFLOW_SUBAGENT_MAX_TURNS_ENV = 'QWEN_CODE_WORKFLOW_AGENT_MAX_TURNS';
export const WORKFLOW_SUBAGENT_MAX_MINUTES_ENV = 'QWEN_CODE_WORKFLOW_AGENT_MAX_MINUTES';
...
function resolveSubagentBound(envName, defaultValue, ceiling, env) {
    ...
    const parsed = parsePositiveIntegerEnv(raw, 0);
    if (parsed < 1) {
        debugLogger.warn(`Invalid ${envName}=${JSON.stringify(raw)}, using default (${defaultValue})`);
        return defaultValue;
    }
    if (parsed > ceiling) {
        debugLogger.warn(`${envName}=${parsed} exceeds hard ceiling (${ceiling}); clamping.`);
        return ceiling;
    }
    return parsed;
}
/** Per-attempt turn ceiling for a workflow subagent. */
export function resolveSubagentMaxTurns(env = process.env) {
    return resolveSubagentBound(WORKFLOW_SUBAGENT_MAX_TURNS_ENV, DEFAULT_WORKFLOW_SUBAGENT_MAX_TURNS, HARD_WORKFLOW_SUBAGENT_MAX_TURNS_CEILING, env);
}
/** Per-attempt wall-clock ceiling, in minutes, for a workflow subagent. */
export function resolveSubagentMaxTimeMinutes(env = process.env) {
    return resolveSubagentBound(WORKFLOW_SUBAGENT_MAX_MINUTES_ENV, DEFAULT_WORKFLOW_SUBAGENT_MAX_TIME_MINUTES, HARD_WORKFLOW_SUBAGENT_MAX_MINUTES_CEILING, env);
}
/**
 ...
 */
const WORKFLOW_SUBAGENT_DISALLOWED_TOOLS = [
 ...
 */
export function createProductionDispatch(config, signal, 
/**
...
onTokens, bridgeApprovalEvents) {
    return async (prompt, opts, dispatchId) => {
        ...
        const stallMs = resolveStallMs(typeof opts.stallMs === 'number' ? opts.stallMs : undefined);
        ...
        let attempt = 0;
        return runStallResilient(async (attemptSignal, emitter) => {
            attempt += 1;
            const cleanupApprovalBridge = bridgeApprovalEvents?.(emitter, dispatchId);
            const cleanupTranscript = attachDispatchTranscript(config, workflowAgentId, prompt, agentIdentity.name, emitter, attempt);
            try {
                return await runSingleDispatch(config, prompt, opts, attemptSignal, emitter, workflowAgentId, agentIdentity, onTokens);
            }
            finally {
                cleanupTranscript();
 ...
 * one.
 */
function attachDispatchTranscript(config, agentId, prompt, 
...
attempt) {
    ...
    try {
        ...
        debugLogger.warn(`[Workflow] failed to attach transcript for ${agentId}: ${error}`);
        return () => { };
    }
}
/**
 ...
 * terminal.
 */
async function runSingleDispatch(config, prompt, opts, attemptSignal, emitter, 
/**
...
agentIdentity, onTokens) {
    ...
    if (opts.agentType === undefined &&
        ...
        const subagent = await AgentHeadless.create(agentIdentity.name, config, {
        ...
        try {
            ...
            await runWithAgentContext(workflowAgentId, () => subagent.execute(ctx, attemptSignal));
        }
        finally {
        ...
        if (mode !== AgentTerminateMode.GOAL) {
            throw new Error(`Workflow subagent ${workflowAgentId} did not complete (terminate mode: ${mode}).`);
        }
        return toModelVisibleSubagentResult(subagent.getFinalText(), mode);
    }
    return runOverridePath(config, ctx, opts, attemptSignal, workflowAgentId, agentIdentity, onTokens, emitter);
}
/**
 ...
 */
async function runOverridePath(config, ctx, opts, signal, workflowAgentId, 
/** The identity the runtime agent runs under — see resolveWorkflowAgentIdentity. */
agentIdentity, 
/**
...
emitter) {
    ...
    if (opts.agentType !== undefined) {
        ...
        if (!resolved) {
            ...
            throw new Error(`agent({agentType}): agent type '${safeAgentType}' not found.`);
        }
        baseConfig = resolved;
    }
    else {
    ...
    if (opts.isolation !== undefined && opts.workingDir !== undefined) {
        throw new Error('agent({workingDir, isolation}): incompatible options. workingDir ' +
    ...
    if (opts.isolation === 'worktree') {
        ...
        effectiveContext = createDirScopedConfigOverride(config, worktreeIsolation.path);
    }
    else if (opts.workingDir !== undefined) {
        if (typeof opts.workingDir !== 'string' ||
            opts.workingDir.trim().length === 0) {
            throw new Error('agent({workingDir}): must be a non-empty string naming an existing git worktree of this repository.');
        }
        ...
        const resolved = await resolveExternalWorktreeDir(config, opts.workingDir, 'workingDir');
        if ('error' in resolved) {
            ...
            throw new Error(`agent({workingDir: ${sanitizeForErrorMessage(JSON.stringify(opts.workingDir))}}): ${sanitizeForErrorMessage(resolved.error)}`);
        }
    ...
    // the just-provisioned worktree on disk under .lailatulcoder/worktrees/. Move
    ...
    try {
        ...
        if (opts.schema !== undefined) {
            effectiveContext = await createSchemaConfigOverride(effectiveContext, opts.schema);
        ...
        const { subagent, dispose } = await subagentMgr.createAgentHeadless(augmented, effectiveContext, {
        ...
        try {
            ...
            try {
                ...
                // establishes the ALS frame that isSubagentLikeExecutionContext() reads,
                // so plan lifecycle tools remain blocked if tool filtering changes.
                await runWithAgentContext(workflowAgentId, () => subagent.execute(ctx, dispatchSignal));
            }
            finally {
                reportTokens(subagent, opts, onTokens);
            }
            if (schemaState) {
                if (schemaState.result !== null) {
                    ...
                    if (worktreeIsolation) {
                        ...
                        if (preserved) {
                            debugLogger.info(`[Workflow] Schema-mode subagent preserved worktree at ` +
                ...
                if (schemaState.attempts > 2) {
                    // Error message verbatim from upstream Claude Code 2.1.168 strings.
                    throw new Error('subagent completed without calling StructuredOutput (after 2 in-conversation nudges).');
                }
                throw new Error('subagent completed without calling structured_output ' +
            ...
            let finalText = toModelVisibleSubagentResult(subagent.getFinalText(), mode);
        ...
        if (worktreeIsolation) {
            try {
                ...
                debugLogger.warn(`Workflow worktree cleanup in fallback finally failed for ` +
...
async function provisionWorkflowWorktree(config) {
    ...
    if (/\.lailatulcoder[\\/]worktrees[\\/]/.test(cwd)) {
        throw new Error(`agent({isolation:'worktree'}): parent is already inside a worktree ` +
    ...
    if (!gitCheck.available) {
        throw new Error(`agent({isolation:'worktree'}): ${gitCheck.error ?? 'git is not available'}.`);
    }
    if (!(await probe.isGitRepository())) {
        throw new Error(`agent({isolation:'worktree'}): ${cwd} is not a git repository.`);
    }
    ...
    try {
        ...
        debugLogger.warn(`[Workflow] hasWorktreeChanges failed at ${projectRoot}: ${error}`);
        parentDirty = true;
    }
    if (parentDirty) {
        throw new Error(`agent({isolation:'worktree'}): parent working tree at ${projectRoot} ` +
    ...
    try {
        ...
        debugLogger.warn(`[Workflow] getCurrentBranch failed at ${projectRoot}: ${error}`);
    }
    ...
    if (!created.success || !created.worktree) {
        throw new Error(`agent({isolation:'worktree'}): failed to create worktree: ` +
            `${created.error ?? 'unknown error'}.`);
    }
    try {
        await writeWorktreeSessionMarker(created.worktree.path, config.getSessionId());
    }
    catch (error) {
        debugLogger.warn(`[Workflow] failed to write session marker at ${created.worktree.path}: ${error}`);
    }
    return {
        slug,
...
function createDirScopedConfigOverride(base, wtPath) {
    ...
    const wtFileService = new FileDiscoveryService(wtPath, base.getFileFilteringOptions().customIgnoreFiles);
...
async function cleanupWorkflowWorktree(isolation) {
    ...
    const [hasChanges, hasUnmerged] = await Promise.all([
        wtService.hasWorktreeChanges(isolation.path).catch((error) => {
            debugLogger.warn(`[Workflow] hasWorktreeChanges failed for ${isolation.path}: ${error}`);
            return true;
        }),
        wtService.hasUnmergedWorktreeCommits(isolation.slug).catch((error) => {
            debugLogger.warn(`[Workflow] hasUnmergedWorktreeCommits failed for ${isolation.slug}: ${error}`);
            return true;
        }),
    ]);
    if (hasChanges || hasUnmerged) {
        debugLogger.info(`[Workflow] Preserving isolation worktree ${isolation.path} ` +
    ...
    try {
        ...
        if (!result.success) {
            debugLogger.warn(`[Workflow] Failed to remove ephemeral worktree ${isolation.path}: ${result.error}`);
            return { path: isolation.path, branch: isolation.branch };
        }
        if (result.branchPreserved) {
            ...
            debugLogger.warn(`[Workflow] Removed worktree directory ${isolation.path} but kept ` +
                `branch ${isolation.branch} (unmerged commits at delete time)`);
            return { branch: isolation.branch };
        }
    }
    catch (error) {
        debugLogger.warn(`[Workflow] Failed to remove ephemeral worktree ${isolation.path}: ${error}`);
        return { path: isolation.path, branch: isolation.branch };
    }
    return null;
}
...
function appendWorktreePreservedSuffix(finalText, preserved) {
    ...
    return (`${finalText}${sep}[worktree directory removed; branch ${preserved.branch} ` +
...
function attachSchemaListeners(emitter, state) {
    ...
    emitter.on(AgentEventType.TOOL_CALL, (evt) => {
        if (evt.name !== targetTool)
            return;
    ...
    emitter.on(AgentEventType.TOOL_RESULT, (evt) => {
        if (evt.name !== targetTool)
            return;
...
export class WorkflowOrchestrator {
    ...
    async run(req) {
        ...
        const rejectThroughPauseGate = (error) => scheduler.waitUntilRunning().then(() => {
            throw error;
        }, () => {
            throw error;
        });
        ...
        const issueDispatchTrace = (prompt, opts, cached = false) => {
        ...
        const countedDispatch = (prompt, opts) => {
            ...
            if (typeof prompt !== 'string' || prompt.length === 0) {
                return rejectThroughPauseGate(new Error('agent() requires a non-empty string prompt.'));
            }
            ...
            if (journal) {
                ...
                if (!hadMiss && replay) {
                    ...
                    if (cached !== undefined) {
                        ...
                        if (respawns && respawns.length > 0) {
                            debugLogger.info(`[Workflow] resume cache hit after ${respawns.length} prior ` +
                        ...
                        return scheduler.waitUntilRunning().then(() => cached.result, () => cached.result);
                    }
                }
                ...
                journal
                    ...
                    .catch((e) => debugLogger.warn(`journal started-append failed: ${e}`));
            }
            ...
            if (budget && budget.total !== null && budget.remaining() <= 0) {
                debugLogger.warn(`[Workflow] budget gate refused dispatch at entry: ` +
                    `runId=${runId} spent=${budget.spent()} total=${budget.total}`);
                return rejectThroughPauseGate(new WorkflowBudgetExceededError(runId, budget.total, budget.spent()));
            }
            // P5 R3 (wenshao #7): agent-count cap runs AFTER the budget gate.
            // See the reordering rationale at the top of countedDispatch.
            agentCount += 1;
            if (agentCount > maxAgents) {
                return rejectThroughPauseGate(new Error(`Workflow exceeded the maximum of ${maxAgents} agent() calls per run.`));
            }
            ...
            return scheduler
                ...
                try {
                    ...
                    if (budget && budget.total !== null && budget.remaining() <= 0) {
                        debugLogger.warn(`[Workflow] budget gate refused dispatch at slot-acquire: ` +
                            `runId=${runId} spent=${budget.spent()} total=${budget.total}`);
                        throw new WorkflowBudgetExceededError(runId, budget.total, budget.spent());
                    }
                    ...
                    if (journal && journalKey !== undefined) {
                        journal
                            ...
                            .catch((e) => debugLogger.warn(`journal result-append failed: ${e}`));
                    }
            ...
            (result) => scheduler.waitUntilRunning().then(() => result, () => result), (error) => {
        ...
        try {
            const result = await dependencyContext.run({ tails: [] }, () => sandbox.run(req.script));
            return {
                runId,
                result,
            ...
            throw new WorkflowExecutionError(extractErrorMessage(err), sandbox.getPhases(), sandbox.getLogs(), sandbox.getMeta());
        }
    }
}
/**
...
kind = 'parallel') {
    ...
    const result = settled.map((r, i) => {
        ...
        if (reason?.name === 'WorkflowBudgetExceededError') {
            ...
            debugLogger.warn(`Workflow thunk at index ${i} rejected: ${String(reason?.message ?? r.reason)}`);
        }
        return null;
    });
    if (budgetDropped > 0) {
        debugLogger.warn(`${kind}: ${budgetDropped} slot${budgetDropped === 1 ? '' : 's'} ` +
...
function makeParallelImpl(signal, dependencyContext) {
    return (thunks) => {
        if (!Array.isArray(thunks)) {
            return Promise.reject(new Error('parallel() expects an array of thunks (functions returning promises).'));
        }
        for (const t of thunks) {
            if (typeof t !== 'function') {
                return Promise.reject(new Error('parallel() expects an array of functions, not values — wrap each ' +
        ...
        return settleToNullArray(branches.map(({ thunk }) => thunk), signal).then((result) => {
            if (parent && branches.length > 0) {
                parent.tails = mergeFanoutTails(parent.tails, inheritedTails, branches.flatMap(({ store }) => store.tails));
            }
            return result;
        });
    };
...
function makePipelineImpl(signal, dependencyContext) {
    return (items, ...stages) => {
        if (!Array.isArray(items)) {
            return Promise.reject(new Error('pipeline() expects an array of items as its first argument.'));
        }
        for (const s of stages) {
            if (typeof s !== 'function') {
                return Promise.reject(new Error('pipeline() stages must be functions: ' +
        ...
        const branches = items.map((item, idx) => {
            ...
            return {
                store,
                thunk: () => dependencyContext.run(store, () => runPipelineChain(item, idx, stages)),
            };
        });
        return settleToNullArray(branches.map(({ thunk }) => thunk), signal, 'pipeline').then((result) => {
            if (parent && branches.length > 0) {
                parent.tails = mergeFanoutTails(parent.tails, inheritedTails, branches.flatMap(({ store }) => store.tails));
            }
            return result;
