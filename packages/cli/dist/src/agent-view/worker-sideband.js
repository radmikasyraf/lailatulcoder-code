...
const lastStateReportKeys = new Map();
const stateReportChains = new Map();
export function createAgentViewWorkerSidebandEnv(config) {
    return {
...
export function isAgentViewWorkerEnv(env = process.env) {
    return env[QWEN_AGENT_VIEW_WORKER] === '1';
}
export function readAgentViewWorkerSidebandEnv(env = process.env) {
...
export async function sendAgentViewWorkerEvent(event, env = process.env) {
...
export async function readAgentViewWorkerControlEvents(env = process.env) {
    const sideband = readAgentViewWorkerSidebandEnv(env);
    if (!sideband)
        return [];
    const result = await callAgentViewSupervisor(sideband.sidebandEndpoint, 'workerControl', {
        sessionId: sideband.sessionId,
        token: sideband.token,
    }, { timeoutMs: 1000 });
    if (!isRecord(result))
...
export async function reportAgentViewWorkerState(report, env = process.env) {
    const sideband = readAgentViewWorkerSidebandEnv(env);
    if (!sideband)
        return;
    const event = {
...
export function startAgentViewWorkerHeartbeat(env = process.env, intervalMs = 15_000) {
    if (!readAgentViewWorkerSidebandEnv(env))
        return undefined;
...
export function resetAgentViewWorkerStateReportForTests() {
    lastStateReportKeys.clear();
    stateReportChains.clear();
}
function isAgentViewWorkerControlEvent(value) {
    if (!isRecord(value) ||
...
function isAgentViewWorkerAnswerOutcome(value) {
    return (value === 'proceed_once' ||
...
//# sourceMappingURL=worker-sideband.js.map