 ...
 */
import { AmbiguousLanInterfaceError, listLanCandidates, NoLanInterfaceError, UnknownLanInterfaceError, } from '../local-control/lan-interfaces.js';
import { listenerIdentityOf } from '../local-control/listener-identity.js';
import { isLoopbackBind } from '../loopback-binds.js';
import { InvalidLocalControlTargetError, } from '../local-control/service.js';
import { requestWasAuthenticated } from '../auth.js';
import { writeStderrLine, writeStdoutLineSafe, } from '../../utils/stdioHelpers.js';
async function withUiData(status) {
    let qrText;
    if (status.url) {
 ...
 */
function requirePrimaryListener(req, res) {
    if (listenerIdentityOf(req).kind === 'primary')
        return true;
    res.status(403).json({
        error: 'Local Control can only be enabled from the machine running the daemon.',
        code: 'local_control_remote_enable_denied',
    });
    return false;
}
export function registerWorkspaceLocalControlRoutes(app, deps) {
    app.get('/workspace/local-control', async (req, res) => {
        res
            .status(200)
            .json(presentStatus(req, await withUiData(deps.service.status())));
    });
    app.post('/workspace/local-control/enable', deps.mutate(), async (req, res) => {
        if (!requirePrimaryListener(req, res))
            return;
        ...
        if (deps.primaryBindHostname !== undefined &&
            !isLoopbackBind(deps.primaryBindHostname)) {
            res.status(409).json({
                error: 'Local Control requires the daemon to be bound to loopback; ' +
        ...
        try {
            const ui = await withUiData(await deps.service.enable({
            ...
            if (!requestWasAuthenticated(req) && ui.url) {
                ...
                // there (#9106).
                writeStdoutLineSafe(`LailatulCoder serve: Local Control pairing URL: ${ui.url}`);
            }
    ...
    app.post('/workspace/local-control/disable', deps.mutate(), async (req, res) => {
        if (listenerIdentityOf(req).kind === 'local-control') {
            queueMicrotask(() => {
                void deps.service.disable().catch((error) => {
                    writeStderrLine(`LailatulCoder serve: Local Control disable failed: ${error instanceof Error ? error.message : String(error)}`);
                });
            });
        ...
        res
            .status(200)
            .json(presentStatus(req, await withUiData(await deps.service.disable())));
    });
}
function sendEnableError(res, error) {
...
//# sourceMappingURL=workspace-local-control.js.map