...
export const SEVERITIES = ['Critical', 'Suggestion', 'Nice to have'];
 ...
 */
export const OUTCOMES = ['fixed', 'skipped', 'no_change_needed'];
/** Where a finding came from — the tag that decides whether it was verified. */
export const SOURCES = ['review', 'build', 'test', 'probe', 'lint'];
/** `shortSummary`, when the caller did not supply one. */
export function compressSummary(summary, max = 60) {
...
function stringArray(o, key, index) {
    const v = o[key];
    ...
    if (!Array.isArray(v) ||
        ...
        fail(index, `"${key}" must be an array of non-empty strings, got ${JSON.stringify(v)}.`);
    }
...
function parseLocations(o, index) {
    ...
    if (raw !== undefined) {
        ...
        return raw.map((l, j) => {
            ...
            if (!file)
                fail(index, `location ${j} is missing a non-empty "file".`);
            if (lo['line'] !== undefined &&
    ...
    if (!file) {
        fail(index, 'needs a non-empty "file" (or a "locations" array for a pattern aggregate).');
    }
    if (o['line'] !== undefined &&
...
export function validateFindings(raw) {
    ...
    const findings = raw.map((r, i) => {
        ...
        if (!id)
            fail(i, 'is missing a non-empty string "id".');
        const severity = oneOf(o['severity'], SEVERITIES);
        if (!severity) {
            fail(i, `has severity ${JSON.stringify(o['severity'])}; expected one of ${SEVERITIES.map((s) => JSON.stringify(s)).join(', ')}.`);
        }
        ...
        const confidence = o['confidence'] === undefined
            ? 'high'
            : oneOf(o['confidence'], CONFIDENCES);
        if (!confidence) {
            fail(i, `has confidence ${JSON.stringify(o['confidence'])}; expected "high" or "low".`);
        }
        const source = o['source'] === undefined
            ? 'review'
            : oneOf(normalizeSource(o['source']), SOURCES);
        if (!source) {
            fail(i, `has source ${JSON.stringify(o['source'])}; expected one of ${SOURCES.map((s) => JSON.stringify(s)).join(', ')}.`);
        }
        const summary = asString(o, 'summary');
        if (!summary)
            fail(i, 'is missing a non-empty string "summary".');
        const failureScenario = asString(o, 'failureScenario') ?? asString(o, 'failure_scenario');
        if (!failureScenario) {
            fail(i, 'is missing a non-empty "failureScenario" — a finding that cannot name ' +
                'its concrete trigger and wrong outcome (or, for a quality finding, its ' +
                'concrete cost) is not a finding.');
        }
        const outcome = o['outcome'] === undefined ? undefined : oneOf(o['outcome'], OUTCOMES);
        if (o['outcome'] !== undefined && !outcome) {
            fail(i, `has outcome ${JSON.stringify(o['outcome'])}; expected one of ${OUTCOMES.map((s) => JSON.stringify(s)).join(', ')}.`);
        }
        const assetFiles = stringArray(o, 'assetFiles', i) ?? stringArray(o, 'asset_files', i);
        ...
        const outcomeNote = asString(o, 'outcomeNote') ?? asString(o, 'outcome_note');
        ...
        const heldRaw = o.heldByMeasurement;
        const heldFile = heldRaw && typeof heldRaw === 'object'
            ? asString(heldRaw, 'file')
            : undefined;
        const shortSummary = asString(o, 'shortSummary') ?? asString(o, 'short_summary');
    ...
    for (const f of findings) {
        if (seen.has(f.id)) {
            throw new Error(`Duplicate finding id ${JSON.stringify(f.id)}. Ids join outcomes and ` +
 ...
 * nor `src/a.test.tsx`.
 */
function namesPath(text, probe) {
    const isNameChar = (c) => c !== undefined && /[A-Za-z0-9._-]/.test(c);
    ...
    const isBoundary = (c) => c === undefined || (!isNameChar(c) && c !== '/');
    let from = 0;
    for (;;) {
        ...
        const extendsName = after !== undefined &&
 ...
 */
export function holdCriticalsFailingOnBase(findings, sharedFailingFiles) {
    const held = [];
    const readjudicated = [];
 ...
 */
export function holdUnwitnessedCriticals(findings) {
    const unwitnessed = [];
    const out = findings.map((f) => {
        if (f.severity !== 'Critical' ||
            f.confidence !== 'high' ||
 ...
 */
function repoRelative(path, workspace) {
    ...
    if (at >= 0 && !workspace)
        return undefined;
    if (!workspace || bare.startsWith(`${workspace}/`))
        return bare;
 ...
 */
export function sharedFailingFilesOf(raw) {
    if (!raw || typeof raw !== 'object')
        return { shared: [], unidentifiable: [] };
    const shared = new Set();
    const netNew = new Set();
    const unidentifiable = new Set();
    const take = (into, v, workspace) => {
        if (!Array.isArray(v))
            return;
        for (const e of v) {
    ...
    if (rows.length > 0) {
        for (const e of rows) {
            ...
            const workspace = typeof command === 'string'
                ? (WORKSPACE_IN_COMMAND_RE.exec(command)?.[1] ?? undefined)
                : undefined;
            take(shared, e.shared, workspace);
            take(netNew, e.netNew, workspace);
        }
    }
...
export function sortFindings(findings) {
    return [...findings].sort((a, b) => {
        ...
        if (bySeverity !== 0)
            return bySeverity;
        const byConfidence = CONFIDENCES.indexOf(a.confidence) - CONFIDENCES.indexOf(b.confidence);
        if (byConfidence !== 0)
            return byConfidence;
...
export function validateOutcomes(raw) {
    if (!Array.isArray(raw)) {
        throw new Error('Outcomes must be a JSON array of {id, outcome} entries.');
    }
    return raw.map((r, i) => {
        if (r === null || typeof r !== 'object' || Array.isArray(r)) {
            throw new Error(`Outcome at index ${i} is ${JSON.stringify(r)}, not an object.`);
        }
        const o = r;
        const id = asString(o, 'id');
        if (!id)
            throw new Error(`Outcome at index ${i} is missing a string "id".`);
        const outcome = oneOf(o['outcome'], OUTCOMES);
        if (!outcome) {
            throw new Error(`Outcome for ${JSON.stringify(id)} is ${JSON.stringify(o['outcome'])}; ` +
 ...
 */
export function applyOutcomes(findings, outcomes) {
    ...
    for (const o of outcomes) {
        ...
        if (seen.has(o.id)) {
            throw new Error(`Outcome for ${JSON.stringify(o.id)} appears twice. One outcome per finding.`);
        }
        seen.add(o.id);
    }
    if (unknown.length > 0) {
        throw new Error(`Outcome(s) for unknown finding id(s): ${unknown.map((u) => JSON.stringify(u)).join(', ')}. ` +
            "The ledger does not match this review's findings.");
    }
    const missing = findings.filter((f) => !seen.has(f.id)).map((f) => f.id);
    if (missing.length > 0) {
        throw new Error(`No outcome recorded for ${missing.length} finding(s): ` +
    ...
    const noteById = new Map(outcomes.filter((o) => o.note).map((o) => [o.id, o.note]));
...
export function buildReport(findings) {
    ...
    const bySeverity = Object.fromEntries(SEVERITIES.map((s) => [s, sorted.filter((f) => f.severity === s).length]));
    const byConfidence = Object.fromEntries(CONFIDENCES.map((c) => [
        c,
        sorted.filter((f) => f.confidence === c).length,
    ]));
    const outcomesRecorded = sorted.length > 0 && sorted.every((f) => f.outcome !== undefined);
    const byOutcome = outcomesRecorded
        ? Object.fromEntries(OUTCOMES.map((o) => [o, sorted.filter((f) => f.outcome === o).length]))
        : undefined;
    return {
        findings: sorted,
        counts: {
 ...
 */
export function anchorRequestsFor(findings) {
    const requests = [];
    ...
    const seen = new Map(findings.map((f) => [f.id, f.id]));
    for (const f of findings) {
        if (!isPostable(f))
            continue;
        ...
        for (const [i, l] of postable.entries()) {
            ...
            if (other !== undefined && other !== f.id) {
                throw new Error(`findings: anchor request id "${id}" is produced twice — findings ` +
 ...
 */
function anchorlessLocationsFor(findings) {
    const skipped = [];
    for (const f of findings) {
        if (!isPostable(f))
            continue;
...
export function renderFindings(report) {
    return report.findings.map((f) => {
        ...
        const where = loc
            ...
            : '(no location)';
        const more = f.locations.length > 1 ? ` (+${f.locations.length - 1} more)` : '';
...
function readJson(path, what) {
    ...
    try {
        ...
        throw new Error(`Could not read the ${what} file at ${JSON.stringify(path)}: ${err instanceof Error ? err.message : String(err)}`);
    }
    try {
        return JSON.parse(text);
    }
    catch (err) {
        throw new Error(`The ${what} file at ${JSON.stringify(path)} is not valid JSON: ${err instanceof Error ? err.message : String(err)}`);
    }
}
export const findingsCommand = {
    command: 'findings',
    describe: "Validate and canonicalize the review's findings into a JSON artifact, and — with --outcomes — record what happened to each one after --fix (refusing any ledger that does not account for every finding)",
    builder: (yargs) => yargs
        .option('input', {
        type: 'string',
        demandOption: true,
        ...
        .option('outcomes', {
        type: 'string',
        describe: 'JSON array of {id, outcome, note?} recording what --fix did to each finding. Must cover every finding.',
    })
        .option('test-delta', {
        type: 'string',
        describe: 'The test-delta artifact. A Critical naming a test file that also failed on the merge base is held back to Suggestion, carrying the measurement that demoted it.',
    })
        .option('to-anchors', {
        type: 'string',
        describe: 'Also write the Step 7 resolver input: one {id, path, anchor, line?} ' +
    ...
    handler: (argv) => {
        const { input, out, outcomes, print, testDelta, toAnchors } = argv;
        ...
        if (toAnchors !== undefined) {
            ...
            if (anchorStat?.isSymbolicLink()) {
                throw new Error(`findings: --to-anchors must not be a symlink (${toAnchors}); ` +
            ...
            if (anchorStat?.isDirectory()) {
                throw new Error(`findings: --to-anchors must not be a directory (${toAnchors}); the anchor artifact is written as a file`);
            }
            const others = [
                ['--input', input],
                ['--out', out],
            ...
            for (const [flag, p] of others) {
                ...
                try {
                    ...
                    if (siblingStat?.isSymbolicLink()) {
                        throw new Error(`findings: ${flag} must not be a dangling symlink (${p}); ` +
                            'it could alias the resolver input, and no path compare would see the collision');
                    }
                }
                if (isSameFile(anchorTarget, sibling)) {
                    throw new Error(`findings: --to-anchors points at the same file as ${flag} (${p}); the resolver input would overwrite it`);
                }
                if (sibling.startsWith(anchorTarget + sep) ||
                    anchorTarget.startsWith(sibling + sep)) {
                    throw new Error(`findings: --to-anchors must not nest inside ${flag} (${p}) or contain it; ` +
        ...
        if (outcomes !== undefined) {
            findings = applyOutcomes(findings, validateOutcomes(readJson(outcomes, 'outcomes')));
        }
        let held = [];
        let readjudicated = [];
        if (testDelta !== undefined) {
            ...
            if (existsSync(resolve(testDelta))) {
                try {
                    ...
                    writeStderrLine(`findings: no holds applied — ${err instanceof Error ? err.message : String(err)}`);
                }
            }
            ...
            for (const f of unidentifiable) {
                writeStderrLine(`findings: ignored the measured file ${f} — it carries a vitest project key and no workspace to resolve it against, so which project it names cannot be established`);
            }
            ({ findings, held, readjudicated } = holdCriticalsFailingOnBase(findings, shared));
        }
        ...
        const anchorRequests = toAnchors !== undefined ? anchorRequestsFor(report.findings) : undefined;
        ...
        if (toAnchors !== undefined && anchorRequests !== undefined) {
            ...
            writeFileSync(anchorTarget, `${JSON.stringify(anchorRequests, null, 2)}\n`, 'utf8');
            writeStderrLine(`findings: wrote ${anchorRequests.length} anchor request(s) for Step 7 to ${anchorTarget}`);
            for (const { id, count, anchored } of anchorlessLocationsFor(report.findings)) {
                writeStderrLine(`findings: ${id} carries ${count} location(s) without an anchor — ` +
        ...
        writeStderrLine(`findings: ${report.counts.total} total — ` +
        ...
        for (const h of held) {
            writeStderrLine(`findings: ${h.id} held back from Critical — test-delta measured ${h.file} as failing on the merge base too`);
        }
        ...
        for (const id of witnessHold.unwitnessed) {
            writeStderrLine(`findings: ${id} filed at low confidence — a confirmed Critical carried neither a witness nor a 'not run' reason (Step 4's witness rule)`);
        }
        ...
        for (const r of readjudicated) {
            writeStderrLine(`findings: ${r.id} left at Critical — it already carries the ${r.file} measurement and was raised again anyway`);
        }
        if (report.counts.byOutcome) {
            const o = report.counts.byOutcome;
            writeStderrLine(`outcomes: ${o['fixed']} fixed, ${o['skipped']} skipped, ` +
