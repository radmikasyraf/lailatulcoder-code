...
import { requireTrustedWorkspaceRuntime, resolveWorkspaceRuntimeFromParam, sendUntrustedWorkspaceResponse, } from '../workspace-route-runtime.js';
export function registerWorkspaceLifecycleRoutes(app, deps) {
    const { boundWorkspace, workspace, mutate, safeBody, sendBridgeError, invalidateServeFeaturesCache, parseAndValidateClientId, } = deps;
...
export function registerWorkspaceQualifiedLifecycleRoutes(app, deps) {
    app.post('/workspaces/:workspace/init', deps.mutate({ strict: true }), async (req, res) => {
    ...
    app.post('/workspaces/:workspace/reload', deps.mutate({ strict: true }), async (req, res) => {
        void req.app.locals
            .requestTrustReconcile?.()
            .catch(() => {
            // The policy monitor reports reconciliation failures separately.
