 ...
 * [100ms, 300s]).
 */
export function discoveryTimeoutFor(cfg) {
    const override = cfg.discoveryTimeoutMs;
    if (override !== undefined && Number.isFinite(override)) {
        return Math.max(MIN_DISCOVERY_TIMEOUT_MS, Math.min(override, MAX_DISCOVERY_TIMEOUT_MS));
    }
 ...
 */
export function runWithTimeout(task, timeoutMs, label) {
    return new Promise((resolve, reject) => {
        const timer = setTimeout(() => {
            reject(new Error(`Timed out after ${timeoutMs}ms: ${label}. The MCP server may be ` +
...
//# sourceMappingURL=mcp-discovery-timeout.js.map