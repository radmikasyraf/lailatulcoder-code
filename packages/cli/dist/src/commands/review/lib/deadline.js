 ...
 */
export function claimRetirementDegradeNote(planPath, round) {
    ...
    try {
        writeFileSync(file, JSON.stringify({ round: round ?? null, atMs: Date.now() }), { flag: 'wx' });
        return true;
    }
    catch (err) {
        if (err.code !== 'EEXIST')
 ...
 */
export function readRoundStamps(planPath) {
    try {
        const raw = readFileSync(join(promptRecordDir(planPath), STAMPS_FILE), 'utf8');
 ...
 */
export function stampRound(planPath, round, nowMs = Date.now()) {
    try {
        const stamps = readRoundStamps(planPath);
        if (round !== undefined && stamps.some((s) => s.round === round))
            return;
 ...
 */
function costliestSpanSeconds(stamps, endMs) {
    if (stamps.length === 0)
        return null;
    let maxSeconds = 0;
    for (let i = 0; i < stamps.length; i++) {
        const end = i + 1 < stamps.length ? stamps[i + 1].atMs : endMs;
        maxSeconds = Math.max(maxSeconds, Math.round((end - stamps[i].atMs) / 1000));
    }
 ...
 */
export function expectedRoundSeconds(planPath, round, nowMs = Date.now()) {
    const stamps = readRoundStamps(planPath).filter((s) => round === undefined || s.round !== round);
 ...
 */
export function expectedAdmissionSeconds(planPath, round, fanOutWidth, env, nowMs = Date.now()) {
    const stamps = readRoundStamps(planPath).filter((s) => round === undefined || s.round !== round);
    const last = stamps.length > 0 ? stamps[stamps.length - 1] : undefined;
    const predecessorInFlight = last !== undefined && nowMs - last.atMs < MIN_OBSERVED_ROUND_SECONDS * 1000;
    if (!predecessorInFlight) {
        return expectedRoundSeconds(planPath, round, nowMs);
    }
    const single = costliestSpanSeconds(stamps.slice(0, -1), last.atMs) ??
        DEFAULT_ROUND_SECONDS;
    const pool = parsePositiveIntegerEnv(env[TOOL_CONCURRENCY_ENV], DEFAULT_TOOL_CONCURRENCY);
 ...
 */
function readDeadlineSeconds(env) {
 ...
 * process it runs in.
 */
export function hasReviewDeadline(env) {
 ...
 */
function readNonNegativeSeconds(env, key, fallback) {
    const raw = env[key];
    if (raw !== undefined && raw.trim() !== '') {
        const v = Number(raw);
 ...
 */
export function reverseAuditBudgetExhausted(env, roundCostSeconds, nowMs = Date.now()) {
    ...
    const reserve = readNonNegativeSeconds(env, RESERVE_ENV, DEFAULT_RESERVE_SECONDS);
    const remainingSeconds = Math.floor(deadline - nowMs / 1000);
    if (remainingSeconds >= reserve + roundCostSeconds)
        return null;
    return {
        remainingSeconds,
 ...
 * planned for.
 */
export function verifyBudgetExhausted(env, nowMs = Date.now()) {
    const deadline = readDeadlineSeconds(env);
    if (deadline === null)
        return null;
    const floor = readNonNegativeSeconds(env, COMPOSE_FLOOR_ENV, DEFAULT_COMPOSE_FLOOR_SECONDS);
 ...
 */
export function verifyBudgetMessage(spent) {
    ...
    return (`VERIFY BUDGET: ${minutesLeft} minute(s) remain before this review's ` +
 ...
 */
export function budgetStopDisclosure(round) {
 ...
 */
export function roundCapStopDisclosure(cap) {
    return {
        subject: 'reverse audit',
...
export function roundCapStopEntry(cap) {
    const d = roundCapStopDisclosure(cap);
    return `${d.subject} — ${d.reason}`;
}
/** The Chinese pair of `roundCapStopEntry`. */
export function roundCapStopEntryZh(cap) {
    const d = roundCapStopDisclosure(cap);
    return `${d.subjectZh}——${d.reasonZh}`;
}
/**
 ...
 */
export function writeRoundCapStop(planPath, cap, round, nowMs = Date.now()) {
    try {
        ...
        if (readBudgetStop(planPath) !== null)
            return;
        const dir = promptRecordDir(planPath);
        mkdirSync(dir, { recursive: true });
        const stop = {
 ...
 */
export function writeBudgetStop(planPath, spent, round, nowMs = Date.now()) {
    try {
        if (readBudgetStop(planPath) !== null)
            return;
        const dir = promptRecordDir(planPath);
 ...
 */
export function readBudgetStopUnfenced(planPath) {
    try {
        const raw = readFileSync(join(promptRecordDir(planPath), STOP_FILE), 'utf8');
        const parsed = JSON.parse(raw);
 ...
 */
export function readBudgetStop(planPath) {
    const stop = readBudgetStopUnfenced(planPath);
    if (stop === null || stop.atMs < runEpochMs(planPath))
        return null;
    return stop;
}
/**
 ...
 * be rid of.
 */
export function clearBudgetStop(planPath) {
    try {
        const stop = readBudgetStopUnfenced(planPath);
        if (stop === null || stop.atMs < runEpochMs(planPath))
            return;
 ...
 */
export function clearRoundStamps(planPath) {
    try {
 ...
 * with one text.
 */
export function reverseAuditBudgetMessage(spent, round) {
    const minutesLeft = Math.max(0, Math.floor(spent.remainingSeconds / 60));
    const reserveMinutes = Math.round(spent.reserveSeconds / 60);
    const roundMinutes = Math.round(spent.expectedRoundSeconds / 60);
    const which = round !== undefined ? `round ${round}` : 'the next round';
    return (`BUDGET: ${minutesLeft} minute(s) remain before this review's deadline — ` +
