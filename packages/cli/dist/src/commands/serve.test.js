...
const mockQr = vi.hoisted(() => ({
    generate: vi.fn((_input, _opts, callback) => callback?.('QR')),
    setErrorLevel: vi.fn(),
}));
...
describe('serve command args', () => {
    ...
    it('parses --permission-response-timeout-ms as a number', () => {
        const parsed = buildParser().parseSync('--permission-response-timeout-ms 60000');
        expect(parsed['permission-response-timeout-ms']).toBe(60000);
    });
    it('parses --compacted-replay-max-bytes as a number', () => {
        const parsed = buildParser().parseSync('--compacted-replay-max-bytes 4194304');
    ...
    it('parses --session-restore-timeout-ms as a number', () => {
        const parsed = buildParser().parseSync('--session-restore-timeout-ms 60000');
    ...
    it('parses required external tool guard options', () => {
        const parsed = buildParser().parseSync('--external-tool-guard-mode required ' +
        ...
        expect(parsed['external-tool-guard-endpoint']).toBe('http://127.0.0.1:8787');
    ...
    it('registers --experimental-lsp as an explicit serve option', () => {
        const options = buildParser().getOptions();
    ...
    it('leaves the journal caps undefined unless the operator pins them', () => {
        ...
        expect(buildParser().parseSync('--max-journal-events 5000')['max-journal-events']).toBe(5000);
        expect(buildParser().parseSync('--max-journal-bytes 1048576')['max-journal-bytes']).toBe(1048576);
    });
    it('rejects valueless journal cap flags instead of silently unpinning', () => {
        ...
        for (const input of [
            ...
            expect(() => buildParser().parseSync(input)).toThrow(/Not enough arguments following: max-journal-(events|bytes)/);
        }
        expect(buildParser().parseSync('--max-journal-events=5000')['max-journal-events']).toBe(5000);
        expect(buildParser().parseSync('--max-journal-bytes=1048576')['max-journal-bytes']).toBe(1048576);
    });
    it('parses --local-control without taking over daemon credentials', () => {
        ...
        expect(buildParser().parseSync('--allow-origin http://localhost:3000')['allow-origin']).toEqual(['http://localhost:3000']);
        expect(buildParser().parseSync('--local-control')['local-control']).toBe(true);
        const composed = buildParser().parseSync('--local-control --token fixed --allow-origin http://localhost:3000 --port 0');
        expect(composed['token']).toBe('fixed');
        expect(composed['allow-origin']).toEqual(['http://localhost:3000']);
        expect(composed['port']).toBe(0);
        expect(() => buildParser().parseSync('--local-control --no-web')).toThrow(/Local Control requires the Web Shell/);
        expect(() => buildParser().parseSync('--local-control --hostname 192.168.1.2')).toThrow(/Local Control requires --hostname 127\.0\.0\.1/);
        expect(() => buildParser().parseSync('--local-control-address 192.168.1.2')).toThrow(/requires --local-control/);
        expect(buildParser().parseSync('--local-control --local-control-address 192.168.1.2')['local-control-address']).toBe('192.168.1.2');
    });
    it('parses repeatable --channel values', () => {
        const parsed = buildParser().parseSync('--channel telegram --channel feishu');
    ...
    it('parses repeatable --workspace values as an array', () => {
        const parsed = buildParser().parseSync('--workspace /tmp/primary --workspace /tmp/secondary');
        expect(parsed['workspace']).toEqual(['/tmp/primary', '/tmp/secondary']);
    });
    it('parses --memory-project-scope and rejects unsupported values', () => {
        expect(buildParser().parseSync('--memory-project-scope workspace')['memory-project-scope']).toBe('workspace');
        expect(buildParser().parseSync('--memory-project-scope git-root')['memory-project-scope']).toBe('git-root');
        expect(() => buildParser().parseSync('--memory-project-scope unsupported')).toThrow(/Invalid values/);
    });
    it('rejects valueless --workspace forms', () => {
        for (const input of [
            ...
            expect(() => buildParser().parseSync(input)).toThrow(/Not enough arguments following: workspace/);
        }
    });
...
describe('serve rate limit env parsing', () => {
    ...
    async function invokeServeHandler() {
        ...
        if (!handler)
            throw new Error('serve handler missing');
    ...
    async function startServeHandlerWithArgs(args) {
        const handler = serveCommand.handler;
        if (!handler)
            throw new Error('serve handler missing');
    ...
    it.each([
        ...
        await expect(invokeServeHandler()).rejects.toThrow('process.exit(1) called');
    ...
    it('passes decimal env values to runQwenServe', async () => {
        ...
        expect(mockRunQwenServe).toHaveBeenCalledWith(expect.objectContaining({
            rateLimit: true,
            rateLimitPrompt: 11,
    ...
    it('forwards pinned journal caps to runQwenServe', async () => {
        ...
        await startServeHandlerWithArgs('--no-web --max-journal-events 5000 --max-journal-bytes 1048576');
        expect(mockRunQwenServe).toHaveBeenCalledWith(expect.objectContaining({
    ...
    it('forwards a single pinned entry cap without the byte cap', async () => {
        ...
        expect(mockRunQwenServe).toHaveBeenCalledWith(expect.objectContaining({ maxJournalEvents: 5000 }));
    ...
    it('forwards a single pinned byte cap without the entry cap', async () => {
        ...
        expect(mockRunQwenServe).toHaveBeenCalledWith(expect.objectContaining({ maxJournalBytes: 1048576 }));
        expect(mockRunQwenServe.mock.calls[0]?.[0]).not.toHaveProperty('maxJournalEvents');
    });
    it('delegates Local Control to the daemon service and prints its pairing URL', async () => {
        ...
        await startServeHandlerWithArgs('--local-control --open --port 443 --tls-cert cert.pem --tls-key key.pem');
        ...
        expect(mockQr.generate).toHaveBeenCalledWith('https://192.168.1.20/#token=pairing', { small: true }, expect.any(Function));
        ...
        await vi.waitFor(() => expect(mockOpenBrowserSecurely).toHaveBeenCalledWith('https://127.0.0.1/'));
    });
    it('forwards --token and --allow-origin through to runQwenServe with --local-control', async () => {
        ...
        await startServeHandlerWithArgs('--local-control --token fixed --allow-origin http://localhost:3000 --port 0');
        const options = mockRunQwenServe.mock.calls[0]?.[0];
        expect(options).toEqual(expect.objectContaining({
            token: 'fixed',
    ...
    it('closes the daemon when pairing output fails', async () => {
        ...
        await expect(handler(argv)).rejects.toThrow('process.exit(1) called');
    ...
    it('passes normalized named channels to runQwenServe', async () => {
        ...
        await startServeHandlerWithArgs('--no-web --channel telegram --channel telegram --channel feishu');
        expect(mockRunQwenServe).toHaveBeenCalledWith(expect.objectContaining({
    ...
    it('passes compacted replay byte cap to runQwenServe', async () => {
        ...
        await startServeHandlerWithArgs('--no-web --compacted-replay-max-bytes 1048576');
        expect(mockRunQwenServe).toHaveBeenCalledWith(expect.objectContaining({
    ...
    it('passes --max-total-sessions to runQwenServe', async () => {
        ...
        expect(mockRunQwenServe).toHaveBeenCalledWith(expect.objectContaining({ maxTotalSessions: 42 }));
    });
    it('passes --memory-project-scope to runQwenServe', async () => {
        ...
        expect(mockRunQwenServe).toHaveBeenCalledWith(expect.objectContaining({ memoryProjectScope: 'git-root' }));
    });
    it('passes required guard config and keeps its token daemon-local', async () => {
        ...
        await startServeHandlerWithArgs('--no-web --external-tool-guard-mode required ' +
            '--external-tool-guard-endpoint http://127.0.0.1:8787 ' +
            '--external-tool-guard-timeout-ms 2500');
        expect(mockRunQwenServe).toHaveBeenCalledWith(expect.objectContaining({
            externalToolGuard: {
    ...
    it('does not pass a provider when mode is off even if config exists', async () => {
        ...
        await startServeHandlerWithArgs('--no-web --external-tool-guard-endpoint http://127.0.0.1:8787');
        expect(mockRunQwenServe.mock.calls[0]?.[0]).not.toHaveProperty('externalToolGuard');
        expect(process.env['QWEN_CODE_EXTERNAL_TOOL_GUARD_TOKEN']).toBeUndefined();
    });
    it('passes --memory-pressure-mode to runQwenServe', async () => {
        ...
        expect(mockRunQwenServe).toHaveBeenCalledWith(expect.objectContaining({ memoryPressureMode: 'off' }));
    });
    it('passes --child-heap-mode to runQwenServe', async () => {
        ...
        expect(mockRunQwenServe).toHaveBeenCalledWith(expect.objectContaining({ childHeapMode: 'off' }));
    });
    it('defaults the child heap mode to observe, and rejects enforce outright', async () => {
        ...
        await startServeHandlerWithArgs('--no-web');
        expect(mockRunQwenServe).toHaveBeenCalledWith(expect.objectContaining({ childHeapMode: 'observe' }));
        ...
        expect(() => buildParser().parseSync('--child-heap-mode enforce')).toThrow(/Invalid values/);
    });
    it('defaults the memory pressure mode to observe', async () => {
        ...
        await startServeHandlerWithArgs('--no-web');
        expect(mockRunQwenServe).toHaveBeenCalledWith(expect.objectContaining({ memoryPressureMode: 'observe' }));
    });
    it('rejects a memory pressure mode outside the choices', () => {
        expect(() => buildParser().parseSync('--memory-pressure-mode enforce')).toThrow(/Invalid values/);
    });
    ...
    it('rejects --channel all mixed with concrete channels', async () => {
        ...
        if (!handler)
            throw new Error('serve handler missing');
        const argv = buildParser().parseSync('--no-web --channel all --channel telegram');
        await expect(handler(argv)).rejects.toThrow('process.exit(1) called');
        expect(mockRunQwenServe).not.toHaveBeenCalled();
    });
});
describe('maybeOpenWebShellBrowser', () => {
    ...
    const firstOpenedUrl = () => String(mockOpenBrowserSecurely.mock.calls[0]?.[0]);
    it('does nothing when --open is false', async () => {
        await maybeOpenWebShellBrowser({ url: 'http://127.0.0.1:4170/', webShellMounted: true }, false);
        expect(mockOpenBrowserSecurely).not.toHaveBeenCalled();
    });
    it('does nothing when the Web Shell is not mounted', async () => {
        await maybeOpenWebShellBrowser({ url: 'http://127.0.0.1:4170/', webShellMounted: false }, true);
        expect(mockOpenBrowserSecurely).not.toHaveBeenCalled();
    });
    it('does nothing when shouldLaunchBrowser() is false', async () => {
        mockShouldLaunchBrowser.mockReturnValue(false);
        await maybeOpenWebShellBrowser({ url: 'http://127.0.0.1:4170/', webShellMounted: true }, true);
        expect(mockOpenBrowserSecurely).not.toHaveBeenCalled();
    });
    it('rewrites a wildcard bind host to loopback', async () => {
        await maybeOpenWebShellBrowser({ url: 'http://0.0.0.0:4170/', webShellMounted: true }, true);
    ...
    it('skips --open when the runtime failed to mount', async () => {
        ...
        expect(mockOpenBrowserSecurely).not.toHaveBeenCalled();
        expect(stderrWrites.join('')).toContain('qwen serve: Web Shell runtime not ready; skipping --open: runtime boom');
    });
    it('swallows openBrowserSecurely failures (never throws)', async () => {
        mockOpenBrowserSecurely.mockRejectedValueOnce(new Error('boom'));
        await expect(maybeOpenWebShellBrowser({ url: 'http://127.0.0.1:4170/', webShellMounted: true }, true)).resolves.toBeUndefined();
    });
});
describe('serve startup import boundary', () => {
    it('reaches listening through the dev entrypoint without loading interactive Ink internals first', async () => {
        const workspace = fs.realpathSync(fs.mkdtempSync(path.join(os.tmpdir(), 'qws-import-boundary-')));
        const qwenHome = fs.realpathSync(fs.mkdtempSync(path.join(os.tmpdir(), 'qws-import-boundary-home-')));
        ...
        const processGroupHasMembers = (pgid) => {
            if (process.platform === 'win32')
                return false;
        ...
        const waitForProcessGroupExit = async (pgid) => {
            if (process.platform === 'win32')
                return;
            for (let attempt = 0; attempt < 20; attempt++) {
                if (!processGroupHasMembers(pgid))
                    return;
                await new Promise((resolve) => setTimeout(resolve, 100));
            }
        ...
        try {
            const reachedListening = await new Promise((resolve, reject) => {
                const timeout = setTimeout(() => {
                    void cleanup();
                    reject(new Error(`serve did not reach listening\nstdout:\n${stdout}\nstderr:\n${stderr}`));
                }, 30_000);
                ...
                child.on('exit', (code, signal) => {
                    if (stdout.includes('qwen serve listening on'))
                        return;
                    clearTimeout(timeout);
                    reject(new Error(`serve exited before listening: code=${code} signal=${signal}\nstdout:\n${stdout}\nstderr:\n${stderr}`));
                });
            });
