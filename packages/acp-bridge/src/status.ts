...
import type { HookEventName } from '@lailatul-coder/lailatul-coder-core';
import { SkillError } from '@lailatul-coder/lailatul-coder-core';
 ...
 * Mirrors `DaemonAgentLevel` in `@lailatul-coder/sdk` so route + SDK
...
export const IDLE_HOOK_EVENTS: Record<HookEventName, ServeHookEventMeta> = {
  ...
  Stop: { description: 'Right before LailatulCoder Ai concludes its response' },
  SubagentStart: {
...
export function mapDomainErrorToErrorKind(
  ...
  // `SkillError` is defined in `@lailatul-coder/lailatul-coder-core/skills`; same
  ...
  // the one resolved here through acp-bridge's `@lailatul-coder/lailatul-coder-core`
  ...
  // `TrustGateError` is defined in `@lailatul-coder/lailatul-coder-core/config`; we
