 ...
 */
import { createDebugLogger, buildSessionRecoveryPlanFromApiHistory, SendMessageType, TURN_INTERRUPTION_HISTORY_TAIL_COUNT, } from '@lailatul-coder/lailatul-coder-core';
...
import { runNonInteractive, TurnInterruptedError, } from '../nonInteractiveCli.js';
...
class Session {
    ...
    monitorStartedQueue = [];
    monitorQueue = [];
    pendingContinueTurn = false;
    continueTurnInProgress = false;
    ...
    constructor(config, initialPrompt, settings = createMinimalSettings()) {
        ...
        this.unsubscribeRecordingFailure = subscribeToHeadlessChatRecordingFailures(config, this.outputAdapter);
     ...
     */
    async requestContinueLastTurn() {
        await this.waitForInitialization();
        if (this.isShuttingDown || this.sessionAbortController.signal.aborted) {
            debugLogger.debug('[Session] continue_last_turn rejected: session is shutting down');
            return { accepted: false, interruption: 'none' };
        }
        const geminiClient = this.config.getGeminiClient();
        if (!geminiClient || !geminiClient.isInitialized()) {
            debugLogger.debug('[Session] continue_last_turn rejected: gemini client is not ready');
            return { accepted: false, interruption: 'none' };
        }
        const chat = geminiClient.getChat();
        const historyTail = chat.getHistoryTailShallow?.(TURN_INTERRUPTION_HISTORY_TAIL_COUNT) ??
        ...
        if (!recoveryPlan.continuation) {
            debugLogger.debug('[Session] continue_last_turn rejected: no interrupted turn');
            return { accepted: false, interruption: 'none' };
        }
        ...
        if (this.pendingContinueTurn || this.continueTurnInProgress) {
            debugLogger.debug('[Session] continue_last_turn rejected: continuation already pending', { kind: recoveryPlan.kind });
    ...
    async processMonitorNotificationBatch(batch) {
        ...
        batch = batch.filter((item) => {
            ...
            return (this.config.getMonitorRegistry().get(item.sdkNotification.task_id)
                ?.status !== 'cancelled');
        });
        if (batch.length === 0) {
            return;
        ...
        for (const item of batch) {
            ...
            this.outputAdapter.emitSystemMessage('task_notification', item.sdkNotification);
        }
        ...
        const turnAbortController = this.startTurn();
        try {
            await runNonInteractive(this.config, this.settings, combinedModelText, promptId, {
                abortController: turnAbortController,
                adapter: this.outputAdapter,
                controlService: this.controlService ?? undefined,
    ...
    async processPendingWork() {
        ...
        while ((this.pendingContinueTurn ||
            ...
            try {
                ...
                debugLogger.error('[Session] Error processing monitor notification batch:', error);
                await this.emitErrorResult(error);
            }
        }
    }
    ...
    async emitErrorResult(error, numTurns = 0, durationMs = 0, apiDurationMs = 0) {
    ...
    async waitForAllPendingWork() {
        ...
        if (this.pendingContinueTurn) {
            this.pendingContinueTurn = false;
            await this.emitErrorResult(new Error('Continuation abandoned: session shut down before it ran'));
        }
    }
    async shutdown() {
