...
description: Use when reproducing an existing Codex or Claude Code feature in LailatulCoder Ai or another agent CLI by choosing a reference agent, capturing HTTP request bodies, prompts, tool/function schemas, terminal output, and then implementing the matching behavior in the target repo.
---
...
Use this skill to turn an observed feature from a reference agent into an implementation task for LailatulCoder Ai. The workflow treats the current session as the outer harness and runs a nested reference agent process as the program under test.
...
3. Inspect the target repo enough to identify the likely module boundaries and LailatulCoder Ai launch command before changing code.
...
6. Implement the smallest compatible behavior in LailatulCoder Ai using its existing patterns.
...
- Do not copy all captured prompt text into LailatulCoder Ai. Convert it into the minimum behavior, schema, or test needed.
...
- If a captured schema reveals a stable public contract, encode that contract as a typed structure or fixture in LailatulCoder Ai.
...
- LailatulCoder Ai contains a focused implementation and at least one verification path.
- Any user-visible command behavior is documented in LailatulCoder Ai if that repo already documents similar features.
