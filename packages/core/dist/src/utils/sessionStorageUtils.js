 ...
 * `~/.lailatulcoder/tmp/<hash>/chats/` (by another local user or an extension with
 ...
 */
function getReadOpenFlags() {
    const constants = fs.constants;
    if (!constants)
        return 0;
    return (constants.O_RDONLY ?? 0) | (constants.O_NOFOLLOW ?? 0);
}
function readLatestTailIfGrown(fd, previousSize, buffer) {
 ...
 */
export function unescapeJsonString(raw) {
    if (!raw.includes('\\'))
        return raw;
    try {
...
function isInlineJsonWhitespace(char) {
    return char === ' ' || char === '\t';
}
function findNextJsonStringField(text, key, searchFrom) {
    const quotedKey = `"${key}"`;
    let keyOffset = text.indexOf(quotedKey, searchFrom);
    fieldSearch: while (keyOffset >= 0) {
        let i = keyOffset + quotedKey.length;
        while (isInlineJsonWhitespace(text[i]))
            i++;
        if (text[i] !== ':') {
 ...
 */
export function extractJsonStringField(text, key) {
 ...
 */
export function extractLastJsonStringFields(text, primaryKey, otherKeys, lineContains) {
    const out = { [primaryKey]: undefined };
    for (const k of otherKeys)
        out[k] = undefined;
    let bestPrimaryValue;
    let bestLineStart = -1;
    let bestLineEnd = -1;
    let bestOffset = -1;
    let searchFrom = 0;
    while (true) {
        ...
        if (match.keyOffset > bestOffset) {
            ...
            bestLineEnd = lineEnd;
            bestPrimaryValue = unescapeJsonString(text.slice(match.valueStart, match.valueEnd));
        }
    }
    if (bestOffset < 0)
        return out;
 ...
 */
export function extractLastJsonStringField(text, key, lineContains) {
    let lastValue;
    let lastOffset = -1;
    let searchFrom = 0;
    while (true) {
        ...
        if (match.keyOffset > lastOffset) {
            lastValue = unescapeJsonString(text.slice(match.valueStart, match.valueEnd));
 ...
 */
export function readLastJsonStringFieldSync(filePath, key, lineContains, scratchBuffer) {
    let fd;
    try {
        ...
        const buffer = scratchBuffer && scratchBuffer.length >= LITE_READ_BUF_SIZE
            ? scratchBuffer
        ...
        if (grownTail !== undefined) {
            const grownHit = extractLastJsonStringField(grownTail.text, key, lineContains);
        ...
        if (headBytes > 0) {
            ...
            const headText = headBytes < fileSize
 ...
 */
export function readLastJsonStringFieldsSync(filePath, primaryKey, otherKeys, lineContains, scratchBuffer) {
    const emptyResult = {};
    emptyResult[primaryKey] = undefined;
    for (const k of otherKeys)
        emptyResult[k] = undefined;
    let fd;
    try {
        const stats = fs.statSync(filePath);
        const fileSize = stats.size;
        ...
        if (tailBytes > 0) {
            const tailText = buffer.toString('utf-8', 0, tailBytes);
            const hit = extractLastJsonStringFields(tailText, primaryKey, otherKeys, lineContains);
            if (hit[primaryKey] !== undefined)
                return hit;
        }
        const grownTail = readLatestTailIfGrown(fd, fileSize, buffer);
        if (grownTail !== undefined) {
            const hit = extractLastJsonStringFields(grownTail.text, primaryKey, otherKeys, lineContains);
            if (hit[primaryKey] !== undefined)
                return hit;
        }
        if (tailOffset === 0)
        ...
        if (headBytes > 0) {
            ...
            const headText = headBytes < fileSize
                ? rawHead.slice(0, rawHead.lastIndexOf('\n') + 1)
                : rawHead;
            const hit = extractLastJsonStringFields(headText, primaryKey, otherKeys, lineContains);
            if (hit[primaryKey] !== undefined)
                return hit;
        }
        return emptyResult;
...
export function readSessionTitleInfoFromFileSync(filePath, scratchBuffer) {
    const hit = readLastJsonStringFieldsSync(filePath, 'customTitle', ['titleSource'], '"subtype":"custom_title"', scratchBuffer);
...
//# sourceMappingURL=sessionStorageUtils.js.map