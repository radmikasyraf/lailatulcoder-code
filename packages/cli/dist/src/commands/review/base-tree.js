...
import { discardWorktree, sanitizedGitEnv, worktreeCreateFailureDetail, } from './lib/worktree.js';
...
export function runBaseTree(args) {
    ...
    try {
        ...
        return unavailable(`cannot read the plan ${args.plan}: ${err.message}`);
    }
    ...
    if (typeof baseSha !== 'string' || !baseSha) {
        ...
        return unavailable('the plan carries no mergeBaseSha, so there is no base commit to build ' +
    ...
    if (plan.baseFetchFailed === true) {
        return unavailable('the base branch could not be fetched, so mergeBaseSha may be stale; an ' +
    ...
    const marker = () => join(tree, '.lailatulcoder-review-base-ok');
    const failedMarker = () => join(tree, '.lailatulcoder-review-base-failed');
    try {
        if (existsSync(tree) &&
    ...
    try {
        if (existsSync(tree) &&
            ...
            return {
                ...
                baseSha,
                build: null,
                note: `the base tree at ${baseSha.slice(0, 9)} already failed to build (an earlier probe measured it); ` +
    ...
    try {
        ...
        if (err.code !== 'EEXIST') {
            return unavailable(`could not take the base-tree build lock: ${err.message}`);
        }
        return unavailable('another probe is building the base tree right now — retry when its ' +
    ...
    function buildBaseTree(baseSha) {
        let sweep;
        try {
            ...
            return unavailable(worktreeCreateFailureDetail('base', e, String(sweep?.stderr ?? '')));
        }
        ...
        if ((build.notBuilt?.length ?? 0) > 0) {
            return {
                ...
                baseSha,
                build,
                note: `the base tree build at ${baseSha.slice(0, 9)} was cut short by the ` +
        ...
        if (!build.ok || build.toolchain !== 'npm' || build.build.length === 0) {
            ...
            return {
                ...
                baseSha,
                build,
                note: `the base tree at ${baseSha.slice(0, 9)} did not build, so nothing can be run ` +
        ...
        return {
            ...
            baseSha,
            build,
            note: `base tree built at ${baseSha.slice(0, 9)} in ${tree}. Run the same input here and in the ` +
                'PR worktree and compare the observed output; a difference is evidence, a ' +
                'reading is not.',
        };
    }
}
export const baseTreeCommand = {
    command: 'base-tree',
    describe: "Build the PR's merge base in a sibling worktree, so a claim about changed " +
