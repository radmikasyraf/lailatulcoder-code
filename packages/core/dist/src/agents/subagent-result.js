...
function isClosingAnalysisTag(tag) {
    return tag.startsWith('</');
}
function isSelfClosingTag(tag) {
    return tag.endsWith('/>');
}
function findAnalysisCloseEnd(text, from) {
...
function stripClosedAnalysisBlocks(text) {
    const tagPattern = /<\/?analysis\b[^>]*>/gi;
    let result = '';
    let index = 0;
    while (true) {
...
function stripVisibleTags(text) {
    return stripClosedAnalysisBlocks(text)
        .replace(/<\/?summary\b[^>]*>/gi, ' ')
        .replace(/ {2,}/g, ' ');
}
function stripAnalysisOutsideSummary(text) {
...
export function toModelVisibleSubagentResult(text, terminateMode = AgentTerminateMode.GOAL) {
    if (terminateMode !== AgentTerminateMode.GOAL) {
        return text;
    }
    return stripVisibleTags(stripAnalysisOutsideSummary(text).trim()).trim();
}
//# sourceMappingURL=subagent-result.js.map