...
describe('TOOL_DISPLAY_BY_NAME', () => {
    ...
    it('has a display name for every enumerated tool (no drift)', () => {
        ...
        for (const key of Object.keys(ToolNames)) {
            const internalName = ToolNames[key];
            expect(TOOL_DISPLAY_BY_NAME[internalName], `no display name for tool "${internalName}"`).toBe(ToolDisplayNames[key]);
            expect(TOOL_DISPLAY_BY_NAME[internalName]).toBeDefined();
        }
    });
});
//# sourceMappingURL=tool-display-map.test.js.map