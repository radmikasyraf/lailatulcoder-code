...
import { AgentStatus, ArenaEventType, AuthType, } from '@lailatul-coder/lailatul-coder-core';
...
describe('arenaCommand agent completion history', () => {
    it('preserves arena result metadata needed by completion cards', async () => {
        ...
        const startCommand = arenaCommand.subCommands?.find((command) => command.name === 'start');
        if (!startCommand?.action) {
            throw new Error('The arena start command must have an action.');
        }
        await startCommand.action(context, '--models model-a,model-b "implement auth"');
        ...
        expect(chatRecorder.recordSlashCommand).toHaveBeenCalledWith(expect.objectContaining({
            outputHistoryItems: [
...
//# sourceMappingURL=arenaCommand.agentComplete.test.js.map