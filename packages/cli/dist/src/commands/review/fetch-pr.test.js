...
import { fetchPrCommand, countDiffChangedLines, computeDiffStats, isEmptyDiff, isCollapsedFromUpstream, resolveIncrementalAnchor, } from './fetch-pr.js';
import { clearReviewWorktreeLease, clearReviewWorktreeLeaseIfOwned, createReviewWorktreeLease, readReviewWorktreeLease, reviewLeaseHeldByAnotherSession, } from '../../services/review-worktree-lease.js';
...
describe('classifyHeavy', () => {
    ...
    it('never flags a test or generated file', () => {
        ...
        expect(classifyHeavy({ ...heavyShape, kind: 'generated' }).heavy).toBe(false);
    });
...
const producerMocks = vi.hoisted(() => ({
    ...
    git: vi.fn(),
    execFileSync: vi.fn(),
    refExists: vi.fn((..._refs) => false),
    releaseWorktree: vi.fn(() => ({ existed: false, freed: true })),
    gitOpt: vi.fn((..._args) => null),
    ...
    gitExit: vi.fn(),
    statSync: vi.fn((path) => String(path).endsWith('-fetch.json') ||
        String(path).endsWith('fetch-report.json')
        ? { mtimeMs: Date.parse('2026-08-13T00:00:00.000Z') }
        : undefined),
    gitRaw: vi.fn((..._args) => Buffer.from('')),
    resolveMergeBase: vi.fn((..._args) => ({
        sha: null,
...
vi.mock('node:fs', async (importOriginal) => {
    ...
    function statSyncThroughMock(path, ...rest) {
...
vi.mock('../../services/review-worktree-lease.js', () => ({
    ...
    readReviewWorktreeLease: vi.fn(() => null),
    reviewLeaseHeldByAnotherSession: vi.fn(() => false),
    reviewLeasePath: (repositoryRoot, target) => `${repositoryRoot}/.qwen/tmp/qwen-review-lease-${target}.json`,
}));
...
vi.mock('./lib/git.js', () => ({
    ...
    gitProbe: (...args) => producerMocks.gitExit(...args),
...
vi.mock('./lib/diff-plan.js', async (importOriginal) => {
    const actual = await importOriginal();
    producerMocks.actualBuildDiffPlan = actual.buildDiffPlan;
...
describe('fetch-pr report assembly', () => {
    ...
    beforeEach(() => {
        ...
        producerMocks.git.mockImplementation((...args) => args[0] === 'rev-parse' ? 'f00df00df00d' : '');
        ...
        producerMocks.gitExit.mockImplementation((...args) => {
        ...
        producerMocks.buildDiffPlan.mockImplementation((...a) => producerMocks.actualBuildDiffPlan(...a));
        ...
        producerMocks.gh.mockReturnValue(JSON.stringify({
            headRefName: 'feat/x',
    ...
    async function reportFor(extraArgs) {
        const handler = fetchPrCommand.handler;
        if (!handler)
            throw new Error('fetch-pr handler missing');
        ...
        const call = producerMocks.writeFileSync.mock.calls.findLast(([path]) => path === '/tmp/fetch-report.json');
        if (!call)
            throw new Error('report was not written');
        return JSON.parse(String(call[1]));
    }
    /** What `publish()` actually wrote to the diff file, or null. */
    function writtenDiff() {
        const call = producerMocks.writeFileSync.mock.calls.findLast(([path]) => String(path).endsWith('diff.txt'));
    ...
    it('refuses a dash-leading baseRefName from the platform metadata', async () => {
        ...
        await expect(reportFor({})).rejects.toThrow(/refusing base ref "--upload-pack=\/tmp\/evil"/);
        const reportCall = producerMocks.writeFileSync.mock.calls.find(([path]) => path === '/tmp/fetch-report.json');
    ...
    it('a TAG-only base ref degrades to the disclosed baseFetchFailed state', async () => {
        ...
        producerMocks.gitOpt.mockImplementation((...args) => args[0] === 'fetch' ? '' : null);
        producerMocks.refExists.mockReturnValue(false);
        ...
        producerMocks.resolveMergeBase.mockImplementation((...args) => {
            const probe = args[3];
    ...
    it('the tracking-ref check is FULLY QUALIFIED (no origin/<name> shadow)', async () => {
        ...
        producerMocks.gitOpt.mockImplementation((...args) => args[0] === 'fetch' ? '' : null);
        const checked = [];
        producerMocks.refExists.mockImplementation((...refs) => {
    ...
    it('the base fetch is an EXPLICIT branch refspec (no tag dwim)', async () => {
        ...
        const fetched = [];
        producerMocks.gitOpt.mockImplementation((...args) => {
            if (args[0] === 'fetch')
                fetched.push(args.slice(1));
    ...
    it('refuses a non-positive pr_number before any side effect', async () => {
        ...
        await expect(reportFor({ pr_number: '0' })).rejects.toThrow(/pr_number must be a positive integer, got "0"/);
    ...
    it('records the round cap its capture wiring writes — huge tier only with a clock (#9256)', async () => {
        ...
        producerMocks.gitRaw.mockReturnValue(Buffer.from(makeDiff('src/huge.ts', 9000)));
    ...
    describe('lease lock', () => {
        ...
        it('refuses with an actionable error when another session holds the lease', async () => {
            ...
            await expect(reportFor({})).rejects.toThrow('PR #42 is already being reviewed by another session ' +
        ...
        it('names the lease file to delete when the holder session is gone', async () => {
            vi.mocked(readReviewWorktreeLease).mockReturnValueOnce(foreignLease);
            vi.mocked(reviewLeaseHeldByAnotherSession).mockReturnValueOnce(true);
            await expect(reportFor({})).rejects.toThrow('qwen-review-lease-pr-42.json');
        });
        it('refuses a malformed pr_number before the gate, matching the lock to the destroyer', async () => {
            ...
            await expect(reportFor({ pr_number: '5/.' })).rejects.toThrow('fetch-pr: pr_number must be a positive integer, got "5/."');
            expect(producerMocks.releaseWorktree).not.toHaveBeenCalled();
            expect(producerMocks.git).not.toHaveBeenCalled();
            expect(producerMocks.gh).not.toHaveBeenCalled();
            expect(vi.mocked(createReviewWorktreeLease)).not.toHaveBeenCalled();
        ...
        it('refuses a zero pr_number the regex disjunct alone accepts', async () => {
            ...
            await expect(reportFor({ pr_number: '0' })).rejects.toThrow('fetch-pr: pr_number must be a positive integer, got "0"');
            expect(producerMocks.releaseWorktree).not.toHaveBeenCalled();
            expect(vi.mocked(createReviewWorktreeLease)).not.toHaveBeenCalled();
            expect(vi.mocked(clearReviewWorktreeLeaseIfOwned)).not.toHaveBeenCalled();
        });
        ...
        it('lets the holding session re-fetch its own lease', async () => {
            ...
            expect(vi.mocked(createReviewWorktreeLease)).toHaveBeenCalledWith(expect.objectContaining({
                sessionId: 'session-self',
                promptId: 'prompt-now',
                target: 'pr-42',
                repositoryRoot: process.cwd(),
        ...
        it('writes the lease before the stale-clean and the first git call', async () => {
            ...
            expect(leaseOrder).toBeLessThan(producerMocks.releaseWorktree.mock.invocationCallOrder[0]);
            expect(leaseOrder).toBeLessThan(producerMocks.git.mock.invocationCallOrder[0]);
            expect(leaseOrder).toBeLessThan(producerMocks.execFileSync.mock.invocationCallOrder[0]);
        });
    });
    ...
    describe('lease rollback on failure', () => {
        it('clears the lease when the PR fetch fails', async () => {
            ...
            await expect(reportFor({})).rejects.toThrow('Failed to fetch PR #42 from remote "origin"');
        ...
        it('clears the lease when the metadata fetch fails', async () => {
            ...
            await expect(reportFor({})).rejects.toThrow('Failed to fetch PR #42 metadata');
            expect(producerMocks.execFileSync).toHaveBeenCalledWith('git', ['branch', '-D', 'qwen-review/pr-42'], 
            ...
            expect(producerMocks.execFileSync.mock.invocationCallOrder[0]).toBeLessThan(vi.mocked(clearReviewWorktreeLeaseIfOwned).mock.invocationCallOrder[0]);
        });
        it('clears the lease when the worktree add fails', async () => {
            producerMocks.git.mockImplementation((...args) => {
                if (args[0] === 'worktree')
                    throw new Error('disk full');
                return args[0] === 'rev-parse' ? 'f00df00d' : '';
            });
            await expect(reportFor({})).rejects.toThrow('Failed to create worktree at');
            expect(vi.mocked(clearReviewWorktreeLeaseIfOwned)).toHaveBeenCalledWith(process.cwd(), 'pr-42', { sessionId: 'session-self', promptId: 'prompt-now' });
        });
    ...
    it('preserves the earliest window opening across drift restarts of the same PR', async () => {
        ...
        producerMocks.readFileSync.mockReturnValue(JSON.stringify({
            prNumber: '42',
    ...
    function servesBothRanges(full = FULL_DIFF, delta = DELTA_DIFF) {
        producerMocks.gitRaw.mockImplementation((...args) => args.includes(`${ANCHOR}..f00df00df00d`)
            ? Buffer.from(delta)
    ...
    function anchorIsValid() {
        producerMocks.gitOpt.mockImplementation((...args) => args[0] === 'cat-file' || args[0] === 'merge-base'
            ? ''
            : args[0] === 'rev-parse'
                ? ANCHOR
                : null);
     ...
     */
    function serveOrphanShape() {
        producerMocks.gitOpt.mockImplementation((...args) => args[0] === 'cat-file' ? '' : args[0] === 'rev-parse' ? ANCHOR : null);
        producerMocks.resolveMergeBase.mockReturnValue({
            sha: BASE,
            baseFetchFailed: false,
        });
    ...
    it('pulls a still-clean importer of a changed file back into the scope', async () => {
        ...
        servesBothRanges();
        producerMocks.lstatSync.mockImplementation((path) => {
            if (String(path).endsWith('b.ts'))
                return { isFile: () => true };
            throw Object.assign(new Error('ENOENT'), { code: 'ENOENT' });
        });
        producerMocks.readFileSync.mockImplementation((path) => {
            if (String(path).endsWith('b.ts'))
                return "import './a.js';\n";
            throw Object.assign(new Error('ENOENT'), { code: 'ENOENT' });
        });
    ...
    it('drops a widening candidate whose real path leaves the worktree', async () => {
        ...
        // really outside.
        producerMocks.realpathSync.mockImplementation((path) => String(path).endsWith('b.ts') ? '/elsewhere/victim.ts' : String(path));
        ...
        const report = await reportFor({ since: ANCHOR });
        const scope = report.incremental
            .scope;
    ...
    it('treats an irregular worktree file as unreadable, never reading it', async () => {
        ...
        // above flip.
        expect(producerMocks.readFileSync.mock.calls.some((c) => String(c[0]).endsWith('b.ts'))).toBe(false);
    });
    it('scopes the plan to a valid anchor and suppresses the full-range flags', async () => {
        ...
        expect(report.diffPathAbsolute).toBe(resolve(report.diffPath));
    ...
    it('refuses an anchor another identity certified, before touching history', async () => {
        ...
        // mismatch, not a pass.
        expect((await reportFor({ since: ANCHOR, sinceModel: undefined })).incremental).toEqual({
            since: ANCHOR,
            effective: false,
            reason: 'cross-model-anchor',
        });
    ...
    it('refuses to scope when the containment oracle was LOST, not absent', async () => {
        ...
        producerMocks.gitRaw.mockImplementation((...args) => {
        ...
        expect(producerMocks.writeStderrLine.mock.calls
            .map((c) => String(c[0]))
            .find((l) => l.includes('refused'))).toContain('capture-failed');
    });
    ...
    it('splits merge-base probe exits — only exit 1 is "no common ancestor"', async () => {
        ...
        const drive = (mergeBase) => {
            producerMocks.gitOpt.mockImplementation((...args) => args[0] === 'fetch' ||
                args[0] === 'cat-file' ||
                args[0] === 'merge-base'
                ? ''
                : args[0] === 'rev-parse'
    ...
    it('keeps upToDate through a partition failure — the stop flow needs no plan', async () => {
        ...
        producerMocks.buildDiffPlan.mockImplementation((text) => {
    ...
    it('reuses the full range when the anchor IS the merge base', async () => {
        ...
        const ranges = producerMocks.gitRaw.mock.calls.filter((c) => c.some((a) => String(a).includes('..f00df00df00d')));
    ...
    it('calls a probe ERROR infrastructure, not a verdict about the anchor', async () => {
        ...
        const spy = vi
            .spyOn(mod, 'gitProbe')
            .mockImplementation((...args) => args[0] === 'merge-base'
            ? { out: null, status: 128 }
            : args[0] === 'rev-parse'
                ? { out: ANCHOR, status: 0 }
                : { out: '', status: 0 });
    ...
    it('splits each probe exit three ways — 0, deterministic, and the surface', async () => {
        ...
        for (const { what, probe, answer, reason } of cases) {
            ...
            const spy = vi
                .spyOn(mod, 'gitProbe')
                .mockImplementation((...args) => args[0] === probe
                ? answer
                : args[0] === 'rev-parse'
                    ? { out: ANCHOR, status: 0 }
                    : { out: '', status: 0 });
    ...
    it("welds Agent 7's --base to the range the published scope came from", async () => {
        ...
        const brief = buildRoleBrief(report, '7', { planPath: '/tmp/plan.json' });
    ...
    it('ignores a value-less --since instead of blaming the anchor', async () => {
        ...
        expect(producerMocks.writeStderrLine.mock.calls
            .map((c) => String(c[0]))
            .some((l) => l.includes('Ignoring --since with no value'))).toBe(true);
    });
    ...
    it('degrades when the diff FILE cannot be written, instead of dying', async () => {
        ...
        servesBothRanges();
        producerMocks.writeFileSync.mockImplementation((path) => {
    ...
    it('refuses a rebased-away anchor end to end, on a full-range plan', async () => {
        ...
        serveOrphanShape();
        producerMocks.gitRaw.mockImplementation((...args) => args.includes(`${BASE}..f00df00df00d`)
    ...
    it('refuses an anchor OLDER than the merge base — scoping wider than the PR is not incremental', async () => {
        ...
        producerMocks.gitOpt.mockImplementation((...args) => args[0] === 'cat-file'
            ? ''
            : args[0] === 'rev-parse'
                ? ANCHOR
    ...
    it('calls a failed rescue WRITE a capture fault, not a tiling one', async () => {
        ...
        const said = producerMocks.writeStderrLine.mock.calls.map((c) => String(c[0]));
    ...
    describe('effort threading', () => {
        ...
        it('recovers the effort parse-args resolved when --effort is not re-threaded', async () => {
            ...
            const traced = producerMocks.writeStderrLine.mock.calls
                .map((c) => String(c[0]))
                .some((l) => l.includes('effort: medium') && l.includes('parse-args report'));
    ...
    describe('the Aone incremental rule (AGit-Flow, D7, #9618)', () => {
        ...
        function serveAone() {
            producerMocks.execFileSync.mockImplementation((cmd, args) => {
                if (cmd !== 'a1')
                    return '';
                if (args[0] === 'repo' && args[1] === 'mr' && args[2] === 'view') {
                    return JSON.stringify({
                        mergeRequest: {
                            ...
                            targetBranch: 'main',
                            detailUrl: 'https://code.alibaba-inc.com/acme/widgets/codereview/42',
                            description: '',
                        },
                    });
                }
        ...
        it('never asks an ancestry question on the Aone platform', async () => {
            ...
            const ancestryCalls = producerMocks.gitOpt.mock.calls.filter((args) => args[0] === 'merge-base' && args[1] === '--is-ancestor');
...
describe('resolveIncrementalAnchor', () => {
    ...
    it('expands an abbreviated anchor to the full sha it scopes from', () => {
        ...
        const r = resolveIncrementalAnchor('abc1234', HEAD, probe({ resolveCommit: () => ANCHOR }));
        expect(r.diffBase).toBe(ANCHOR);
    ...
    it('rules base-untrusted BEFORE the clamp — an unverifiable base cannot be clamped against', () => {
        ...
        const r = resolveIncrementalAnchor(ANCHOR, HEAD, probe({ isAncestor: (a) => a !== 'c'.repeat(40) }), { sha: 'c'.repeat(40), fetchFailed: true });
    ...
    it('compares the RESOLVED sha to the head, not the string it was given', () => {
        ...
        const r = resolveIncrementalAnchor('f00df00', HEAD, probe({ resolveCommit: () => HEAD }));
        expect(r.incremental).toEqual({
            since: 'f00df00',
            effective: true,
            upToDate: true,
    ...
    it('clamps an anchor older than the merge base — wider than the PR is not incremental', () => {
        ...
        expect(resolveIncrementalAnchor(ANCHOR, HEAD, probe(), base).diffBase).toBe(ANCHOR);
    });
    ...
    it('accepts a 64-character SHA-256 anchor', () => {
        ...
        const r = resolveIncrementalAnchor(sha256, HEAD, probe({ resolveCommit: (sha) => sha }));
    ...
    it('rules a rebased-away anchor even when the base fetch failed', () => {
        ...
        const r = resolveIncrementalAnchor(ANCHOR, HEAD, probe({ isAncestor: () => false }), { sha: 'c'.repeat(40), fetchFailed: true });
        expect(r.incremental).toEqual({
            since: ANCHOR,
            effective: false,
            reason: 'not-an-ancestor',
    ...
    it('noAncestry scopes an orphaned anchor the ancestry test refuses', () => {
        ...
        const r = resolveIncrementalAnchor(ANCHOR, HEAD, probe({ isAncestor: () => false }), null, { noAncestry: true });
        expect(r.incremental).toEqual({ since: ANCHOR, effective: true });
        expect(r.diffBase).toBe(ANCHOR);
    });
    it('noAncestry asks NEITHER ancestry question — not the head test, not the clamp', () => {
        ...
        }), { sha: 'c'.repeat(40), fetchFailed: false }, { noAncestry: true });
        expect(r.incremental).toEqual({ since: ANCHOR, effective: true });
        expect(r.diffBase).toBe(ANCHOR);
    ...
    it('noAncestry keeps the existence refusals — a fresh clone cannot diff the orphan', () => {
        ...
        const absent = resolveIncrementalAnchor(ANCHOR, HEAD, probe({ commitExists: () => false, isAncestor: () => false }), null, { noAncestry: true });
        ...
        const unresolvable = resolveIncrementalAnchor(ANCHOR, HEAD, probe({ resolveCommit: () => null, isAncestor: () => false }), null, { noAncestry: true });
    ...
    it('noAncestry still refuses a possibly-stale base — base-untrusted is not an ancestry test', () => {
        ...
        const r = resolveIncrementalAnchor(ANCHOR, HEAD, probe({ isAncestor: () => false }), { sha: 'c'.repeat(40), fetchFailed: true }, { noAncestry: true });
        expect(r.incremental).toEqual({
            since: ANCHOR,
            effective: false,
            reason: 'base-untrusted',
...
describe('isCollapsedFromUpstream', () => {
    ...
    it('does not fire off a base the fetch could not confirm', () => {
        ...
        expect(isCollapsedFromUpstream({ ...collapsing, baseFetchFailed: false })).toBe(true);
        expect(isCollapsedFromUpstream({ ...collapsing, baseFetchFailed: true })).toBe(false);
    });
...
describe('fetch-pr diff identity (diffSha256)', () => {
    ...
    async function reportFor() {
        ...
        const call = producerMocks.writeFileSync.mock.calls.find(([path]) => path === '/tmp/fetch-report.json');
        if (!call)
            throw new Error('report was not written');
        return JSON.parse(String(call[1]));
    }
    it('hashes the captured diff bytes — the resume check compares against this', async () => {
        ...
        vi.mocked(gitRaw).mockImplementation((...args) => args.includes('diff') ? Buffer.from(diff) : Buffer.from(''));
        const report = await reportFor();
        const { createHash } = await import('node:crypto');
        expect(report.diffSha256).toBe(createHash('sha256').update(Buffer.from(diff)).digest('hex'));
    });
    it('hashes the BYTES, not a utf8 decode of them', async () => {
        ...
        vi.mocked(gitRaw).mockImplementation((...args) => args.includes('diff') ? bytes : Buffer.from(''));
        const report = await reportFor();
        const { createHash } = await import('node:crypto');
        expect(report.diffSha256).toBe(createHash('sha256').update(bytes).digest('hex'));
        // The decode-then-hash digest differs; equality above rules it out.
        expect(report.diffSha256).not.toBe(createHash('sha256').update(bytes.toString('utf8')).digest('hex'));
    });
...
describe('fetch-pr run-session ledger wiring', () => {
    ...
    it('appends the session against the plan it just wrote, after the write', async () => {
        ...
        expect(vi.mocked(appendRunSession)).toHaveBeenCalledWith('/tmp/fetch-report.json');
        ...
        const writeIndex = producerMocks.writeFileSync.mock.calls.findIndex(([path]) => path === '/tmp/fetch-report.json');
        ...
        const writeOrder = producerMocks.writeFileSync.mock.invocationCallOrder[writeIndex];
...
// lines).
function resumePlanFields(diffBytes) {
...
describe('fetch-pr --resume', () => {
    ...
    beforeEach(async () => {
        ...
        vi.mocked(gitOpt).mockImplementation((...args) => {
        ...
        vi.mocked(gitRaw).mockImplementation((...args) => args.includes('ls-tree') || args.includes('cat-file')
            ? Buffer.from('')
        ...
        const { priorSessionIds, readResumeMarker, ledgerResumeCount, sessionEntryCount, } = await import('./lib/run-ledger.js');
    ...
    function reportWritten() {
        return producerMocks.writeFileSync.mock.calls.some(([path]) => path === OUT);
    }
    async function stdoutJsonLines() {
    ...
    it('resumes without touching the report when every probe matches', async () => {
        ...
        const { recordResume, appendRunSession } = await import('./lib/run-ledger.js');
    ...
    it('falls through to a fresh fetch when the head moved, and says so', async () => {
        producerMocks.gh.mockImplementation((...args) => {
        ...
        expect(vi.mocked(recordRestart)).toHaveBeenCalledWith(OUT, expect.stringContaining('head-moved'));
    });
    ...
    it('a same-session retry at the cap is the SAME resume in both terms', async () => {
        ...
        const { readResumeMarker, ledgerResumeCount } = await import('./lib/run-ledger.js');
        vi.mocked(ledgerResumeCount).mockImplementation((_p, opts) => opts?.excludeSessionId?.toLowerCase() === 's-test' ? 1 : 2);
        vi.mocked(readResumeMarker).mockReturnValue({
            schemaVersion: 1,
            resumes: [
    ...
    it('refuses on an explicit effort different from the recorded run', async () => {
        producerMocks.readFileSync.mockImplementation((path) => {
            if (path === OUT)
                return prevReport({ effort: 'medium' });
            if (String(path).endsWith('qwen-review-pr-42-diff.txt')) {
                return Buffer.from(DIFF_BYTES);
            }
            throw Object.assign(new Error('ENOENT'), { code: 'ENOENT' });
    ...
    it('falls through when the worktree holds uncommitted changes', async () => {
        ...
        vi.mocked(gitOpt).mockImplementation((...args) => {
            ...
            if (args.includes('merge-base'))
                return 'baseb45eb45e';
            if (args.includes('status'))
                return ' M packages/cli/src/x.ts';
            if (args.includes('ls-files'))
                return 'H f.txt';
            if (args.includes('--git-common-dir'))
                return '/repo/.git';
    ...
    it('keeps a round-cap stop and its stamps on a continuation', async () => {
        ...
        const { readBudgetStop, clearBudgetStop, clearRoundStamps } = await import('./lib/deadline.js');
...
describe('fetch-pr --resume bookkeeping is counted, not merely called', () => {
    ...
    it('writes the resume bookkeeping exactly once, and only on a continuation', async () => {
        const { appendRunSession, recordResume, recordRestart } = await import('./lib/run-ledger.js');
        await run();
    ...
    it('records nothing when the resume is refused', async () => {
        ...
        const { gitOpt } = await import('./lib/git.js');
        vi.mocked(gitOpt).mockImplementation((...args) => args.includes('status') ? '' : 'someothersha');
        await run();
    ...
    it('records a restart ONLY for head movement, not for any refusal', async () => {
        const { recordRestart } = await import('./lib/run-ledger.js');
        const { gitOpt } = await import('./lib/git.js');
        vi.mocked(gitOpt).mockImplementation((...args) => args.includes('status') ? ' M src/x.ts' : 'f00df00df00d');
        await run();
        expect(vi.mocked(recordRestart)).not.toHaveBeenCalled();
    });
    ...
    it('runs the budget hygiene on a continuation, and only there', async () => {
        const { clearRoundStamps, clearBudgetStop, readBudgetStop } = await import('./lib/deadline.js');
        vi.mocked(readBudgetStop).mockReturnValue({
            cause: 'time-budget',
            entry: 'stopped',
            entryZh: '停止',
    ...
    it('keeps a round-cap stop across the resume', async () => {
        ...
        const { clearBudgetStop, readBudgetStop } = await import('./lib/deadline.js');
        vi.mocked(readBudgetStop).mockReturnValue({
            cause: 'round-cap',
            cap: 5,
            entry: 'round cap',
    ...
    it('keeps a round-cap stop AND its stamps across TWO resumes', async () => {
        ...
        // exists for.
        const actualDeadline = await vi.importActual('./lib/deadline.js');
        const realFs = await vi.importActual('node:fs');
        const recordDir = `${OUT.replace(/\.json$/, '')}-prompts`;
        const routeRecordDirToDisk = () => {
            producerMocks.readFileSync.mockImplementation((path) => {
            ...
            producerMocks.writeFileSync.mockImplementation((path, data) => {
                if (String(path).startsWith(recordDir)) {
                    realFs.writeFileSync(String(path), String(data));
                }
            });
            producerMocks.mkdirSync.mockImplementation((dir) => {
        ...
        try {
            ...
            vi.mocked(readBudgetStop).mockImplementation(() => actualDeadline.readBudgetStop(OUT));
            vi.mocked(clearBudgetStop).mockImplementation(() => actualDeadline.clearBudgetStop(OUT));
            vi.mocked(clearRoundStamps).mockImplementation(() => actualDeadline.clearRoundStamps(OUT));
...
//# sourceMappingURL=fetch-pr.test.js.map