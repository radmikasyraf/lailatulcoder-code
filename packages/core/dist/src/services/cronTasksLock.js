 ...
 *
 * Lock file: `~/.lailatulcoder/tmp/<project-hash>/scheduled_tasks.lock` (per-machine
...
export function getLockFilePath(projectRoot) {
    ...
    return path.join(Storage.getGlobalTempDir(), getProjectHash(projectRoot), LOCK_FILENAME);
}
 ...
 */
export async function tryAcquireLock(projectRoot, sessionId, lockId) {
    ...
    for (let attempt = 0; attempt < 2; attempt++) {
        ...
        try {
            const moved = JSON.parse(await fs.readFile(stalePath, 'utf-8'));
 ...
 */
export async function releaseLock(projectRoot, sessionId, lockId) {
    const lockPath = getLockFilePath(projectRoot);
    try {
...
//# sourceMappingURL=cronTasksLock.js.map