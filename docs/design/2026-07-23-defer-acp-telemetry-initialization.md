...
- One-shot bootstrap events emitted before the response, including the initial `lailatul-coder.auth` event and one early `lailatul-coder.config` event, are permanently absent from ACP telemetry rather than merely delayed. This is the accepted cost of moving SDK initialization behind the response; the change does not synthesize or buffer replacement events.
