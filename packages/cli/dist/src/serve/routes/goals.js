...
export function registerGoalsRoutes(app, deps) {
    ...
    app.get('/goals', async (_req, res) => {
        ...
        try {
            ...
            if (dropped.length > 0) {
                writeStderrLine(`LailatulCoder serve: GET /goals could not probe ${dropped.length} of ${sessions.length} session(s): ${dropped.join('; ')}`);
            }
            ...
            if (sendGenerationClosedError(res, err))
                return;
            writeStderrLine(`LailatulCoder serve: GET /goals failed: ${err instanceof Error ? err.message : String(err)}`);
            res.status(500).json({
