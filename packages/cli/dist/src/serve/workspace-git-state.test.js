...
import { getGitWorkingTreeStatus, resolveBranchName, watchRepoBranch, } from '@lailatul-coder/lailatul-coder-core';
...
describe('WorkspaceGitState', () => {
    ...
    it('returns the current branch and publishes only real changes', async () => {
        ...
        await expect(state.getStatus('/workspace', {
            publishWorkspaceEvent,
        })).resolves.toEqual({
            v: 2,
        ...
        onChange?.();
        await vi.waitFor(() => expect(resolveBranchNameMock).toHaveBeenCalledTimes(2));
        expect(publishWorkspaceEvent).not.toHaveBeenCalled();
        onChange?.();
        await vi.waitFor(() => expect(publishWorkspaceEvent).toHaveBeenCalledWith({
    ...
    it('degrades to the branch-only shape when the working-tree summary throws', async () => {
        ...
        getGitWorkingTreeStatusMock.mockRejectedValueOnce(new Error('git exploded'));
        const state = new WorkspaceGitState();
        await expect(state.getStatus('/workspace', {
            publishWorkspaceEvent: vi.fn(),
        }, { wait: true })).resolves.toEqual({
            v: 2,
            workspaceCwd: '/workspace',
            branch: 'main',
        });
    ...
    it('retries after entry creation fails', async () => {
        ...
        await expect(state.getStatus('/retry', bridge)).rejects.toThrow('git unavailable');
    ...
    it('prefers the watcher branch over the summary branch when detached', async () => {
        ...
        await expect(state.getStatus('/workspace', bridge, { wait: true })).resolves.toMatchObject({
            branch: 'a1b2c3d',
            detached: true,
        });
    });
    ...
    it('keeps a replacement entry when the disposed creation later fails', async () => {
        let rejectFirst;
        const firstBranch = new Promise((_resolve, reject) => {
            rejectFirst = reject;
        });
        resolveBranchNameMock
    ...
    it('publishes a follow-up event only when the summary changes', async () => {
        ...
        await vi.waitFor(() => expect(publishWorkspaceEvent).toHaveBeenCalledTimes(1));
        ...
        try {
            ...
            await state.getStatus('/workspace', bridge);
            await vi.waitFor(() => expect(getGitWorkingTreeStatusMock).toHaveBeenCalledTimes(2));
            expect(publishWorkspaceEvent).toHaveBeenCalledTimes(1);
            getGitWorkingTreeStatusMock.mockResolvedValue(summary({ staged: 3 }));
            vi.setSystemTime(Date.now() + 2_000);
            await state.getStatus('/workspace', bridge);
            await vi.waitFor(() => expect(publishWorkspaceEvent).toHaveBeenCalledTimes(2));
    ...
    it('shares one in-flight computation across concurrent fast callers', async () => {
        ...
        await vi.waitFor(() => expect(getGitWorkingTreeStatusMock).toHaveBeenCalledTimes(1));
    ...
    it('wait:true joins an already in-flight fast-path refresh', async () => {
        let resolveGit;
        getGitWorkingTreeStatusMock.mockImplementation(() => new Promise((resolve) => {
            resolveGit = resolve;
        }));
        resolveBranchNameMock.mockResolvedValue('main');
        watchRepoBranchMock.mockResolvedValue(() => { });
    ...
    it('wait:true awaits a fresh computation and bypasses the throttle', async () => {
        ...
        await vi.waitFor(() => expect(getGitWorkingTreeStatusMock).toHaveBeenCalled());
        ...
        await expect(state.getStatus('/workspace', bridge, { wait: true })).resolves.toMatchObject({ ahead: 5 });
    ...
    it('keeps the cached summary and stays silent when a refresh fails', async () => {
        ...
        const state = new WorkspaceGitState();
        const { bridge, publishWorkspaceEvent } = bridgeWith();
        await expect(state.getStatus('/workspace', bridge, { wait: true })).resolves.toMatchObject({ staged: 1 });
        expect(publishWorkspaceEvent).toHaveBeenCalledTimes(1);
        getGitWorkingTreeStatusMock.mockRejectedValueOnce(new Error('git exploded'));
        await expect(state.getStatus('/workspace', bridge, { wait: true })).resolves.toMatchObject({ staged: 1 });
        expect(publishWorkspaceEvent).toHaveBeenCalledTimes(1);
        expect(writeStderrLineSafeMock).toHaveBeenCalledWith(expect.stringContaining('git status failed'));
        state.dispose();
    });
    ...
    it('keeps the cache and resolves wait:true when publishWorkspaceEvent throws', async () => {
        ...
        await expect(state.getStatus('/workspace', bridge, { wait: true })).resolves.toMatchObject({ staged: 2 });
        const fast = await state.getStatus('/workspace', bridge);
        expect(fast.computedAt).toBeDefined();
        state.dispose();
    });
});
//# sourceMappingURL=workspace-git-state.test.js.map