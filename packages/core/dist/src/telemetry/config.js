...
function parseTelemetryUserId(source, value) {
    ...
    if (typeof value !== 'string') {
        throw new FatalConfigError(`Invalid ${source}: must be a string, got ${typeof value}`);
    }
...
export async function resolveTelemetrySettings(options) {
    ...
    const logPrompts = argv.telemetryLogPrompts ??
        ...
        settings.logPrompts;
    const userId = parseTelemetryUserId('QWEN_TELEMETRY_USER_ID', env['QWEN_TELEMETRY_USER_ID']) ?? parseTelemetryUserId('telemetry.userId', settings.userId);
    const includeSensitiveSpanAttributes = parseBooleanEnvFlag(env['QWEN_TELEMETRY_INCLUDE_SENSITIVE_SPAN_ATTRIBUTES']) ??
        settings.includeSensitiveSpanAttributes ??
        false;
    const sensitiveSpanAttributeMaxLength = parseSensitiveSpanAttributeMaxLengthEnvValue('QWEN_TELEMETRY_SENSITIVE_SPAN_ATTRIBUTE_MAX_LENGTH', env['QWEN_TELEMETRY_SENSITIVE_SPAN_ATTRIBUTE_MAX_LENGTH']) ??
    ...
    const otlpTracesEndpoint = env['QWEN_TELEMETRY_OTLP_TRACES_ENDPOINT'] ??
        env['OTEL_EXPORTER_OTLP_TRACES_ENDPOINT'] ??
        settings.otlpTracesEndpoint;
    const otlpLogsEndpoint = env['QWEN_TELEMETRY_OTLP_LOGS_ENDPOINT'] ??
        env['OTEL_EXPORTER_OTLP_LOGS_ENDPOINT'] ??
        settings.otlpLogsEndpoint;
    const otlpMetricsEndpoint = env['QWEN_TELEMETRY_OTLP_METRICS_ENDPOINT'] ??
    ...
    const resourceAttributes = Object.keys(mergedResourceAttrs).length
        ? mergedResourceAttrs
        : undefined;
    const metricsIncludeSessionId = parseBooleanEnvFlag(env['QWEN_TELEMETRY_METRICS_INCLUDE_SESSION_ID']) ??
