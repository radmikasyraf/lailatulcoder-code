...
import { generateSessionRecap } from '@lailatul-coder/lailatul-coder-core';
...
function shouldFireRecap(history) {
    ...
    if (userMessageCount < MIN_USER_MESSAGES_TO_FIRE)
        return false;
    if (lastRecapIndex === -1)
        return true;
    let userSinceLast = 0;
 ...
 */
export function useAwaySummary(options) {
    const { enabled, config, isFocused, isIdle, addItem, history, awayThresholdMinutes, } = options;
    ...
    const thresholdMs = (awayThresholdMinutes && awayThresholdMinutes > 0
    ...
    useEffect(() => {
        ...
        void generateSessionRecap(config, controller.signal)
            .then((recap) => {
            if (controller.signal.aborted || !recap)
                return;
            if (!isIdleRef.current)
                return;
            const item = {
