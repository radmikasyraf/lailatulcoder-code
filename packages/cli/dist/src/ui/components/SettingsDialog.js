...
import { getDialogSettingKeys, setPendingSettingValue, getDisplayValue, saveModifiedSettings, getSettingDefinition, isDefaultValue, requiresRestart, getRestartRequiredFromModified, getDefaultValue, setPendingSettingValueAny, getNestedValue, getEffectiveValue, validateSettingValue, } from '../../utils/settingsUtils.js';
import { isAutoLanguage, writeOutputLanguageAndRegisterPath, } from '../../utils/languageUtils.js';
...
import { isDeletionKey, isPrintableSearchChar, removeLastGrapheme, } from '../hooks/useSessionSearchInput.js';
...
const CONFIG_TAB_ORDER = ['settings', 'status', 'stats'];
...
function ConfigTabBar({ activeTab, focused, }) {
    return (_jsxs(Box, { children: [CONFIG_TAB_ORDER.map((tab) => {
                const isActive = tab === activeTab;
                return (_jsx(Box, { marginRight: 2, children: isActive ? (_jsx(Text, { bold: true, backgroundColor: theme.text.accent, color: theme.background.primary, children: ` ${configTabLabel(tab)} ` })) : (_jsx(Text, { color: theme.text.secondary, children: ` ${configTabLabel(tab)} ` })) }, tab));
            }), _jsx(Text, { color: theme.text.secondary, dimColor: !focused, children: focused ? t('(←/→ to switch, ↓ to return)') : t('(↑ to switch tabs)') })] }));
}
export function SettingsDialog({ settings, onSelect, onRestartRequest, availableTerminalHeight, width, config, }) {
    ...
    const [restartRequiredSettings, setRestartRequiredSettings] = useState(new Set());
            ...
            return {
                ...
                toggle: () => {
                    ...
                    if (!requiresRestart(key)) {
                        ...
                        debugLogger.debug(`[DEBUG SettingsDialog] Saving ${key} immediately with value:`, newValue);
                        ...
                        if (key === 'tools.approvalMode' &&
                            ...
                            try {
                                ...
                                debugLogger.error('Failed to apply approval mode to current session:', error);
                            }
                        }
    ...
    const items = normalizedQuery
        ? allItems.filter((item) => {
            ...
            const scopeMsg = getScopeMessageForSetting(item.value, selectedScope, settings);
    ...
    const commitEdit = (key) => {
        ...
        // Update pending
        setPendingSettings((prev) => parsed === undefined
            ? setPendingSettingValueAny(key, undefined, prev)
            : setPendingSettingValueAny(key, parsed, prev));
        if (!requiresRestart(key)) {
            const immediateSettings = new Set([key]);
            const immediateSettingsObject = parsed === undefined
                ? {}
                : setPendingSettingValueAny(key, parsed, {});
            saveModifiedSettings(immediateSettings, immediateSettingsObject, settings, selectedScope);
    ...
    const activeDescription = activeTab === 'settings' &&
        mode === 'settings' &&
        focusZone === 'list' &&
    ...
    const totalFixedHeight = DIALOG_PADDING +
        TAB_BAR_HEIGHT +
        SEARCH_BOX_HEIGHT +
    ...
    const availableHeightForSettings = Math.max(1, currentAvailableTerminalHeight - totalFixedHeight);
    ...
    useKeypress((key) => {
        ...
        const cycleTab = (direction) => {
            setActiveTab((current) => {
                ...
                const next = (index + direction + CONFIG_TAB_ORDER.length) %
        ...
        const applyRestart = () => {
            ...
            const restartRequiredSet = new Set(getRestartRequiredFromModified(modifiedSettings));
            ...
            if (keyMatchers[Command.SELECTION_UP](key)) {
                ...
                if (currentSetting) {
                    ...
                    if (!requiresRestart(currentSetting.value)) {
                        ...
                        saveModifiedSettings(immediateSettings, immediateSettingsObject, settings, selectedScope);
                        // Special handling for approval mode to apply to current session
                        if (currentSetting.value === 'tools.approvalMode' &&
                            settings.merged.tools?.approvalMode) {
                            try {
                                config?.setApprovalMode(settings.merged.tools.approvalMode);
                            }
                        ...
                        setRestartRequiredSettings((prev) => new Set(prev).add(currentSetting.value));
                    }
                }
            }
    ...
    return (_jsxs(Box, { borderStyle: "round", borderColor: theme.border.default, flexDirection: "column", padding: 1, width: "100%", children: [_jsx(ConfigTabBar, { activeTab: activeTab, focused: focusZone === 'tabs' }), _jsx(Box, { height: 1 }), activeTab !== 'settings' ? (_jsx(Box, { flexDirection: "column", flexGrow: 1, children: activeTab === 'status' ? (systemInfo ? (
                // Outer Box: border (2) + padding (2) = 4 columns of chrome,
                // matching the embedded StatsDialog width below.
                _jsx(AboutBox, { ...systemInfo, width: width ? width - 4 : undefined })) : statusError ? (_jsx(Text, { color: theme.status.error, children: t('Failed to load status. Press r to retry.') })) : (_jsx(Text, { color: theme.text.secondary, children: t('Loading status…') }))) : (
                ...
                , { 
                    ...
                    // stop StatsDialog from closing — redirecting its onClose does).
                    // A second Escape from the tab bar then closes the dialog.
                    onClose: () => setFocusZone('tabs'), isFocused: focusZone === 'list', 
                    // Outer Box: border (2) + padding (2) = 4 columns of chrome.
                    width: width ? width - 4 : undefined, availableHeight: availableTerminalHeight != null
                        ...
                        : undefined })) })) : mode === 'settings' ? (_jsxs(Box, { flexDirection: "column", flexGrow: 1, children: [_jsxs(Box, { borderStyle: "round", borderColor: focusZone === 'search'
                            ? theme.border.focused
                            : theme.border.default, paddingX: 1, width: "100%", children: [_jsx(Text, { color: theme.text.secondary, children: '⌕ ' }), searchQuery ? (_jsx(Text, { color: theme.text.primary, wrap: "truncate", children: searchQuery })) : (_jsx(Text, { color: theme.text.secondary, children: t('Search settings…') }))] }), _jsx(Box, { height: 1 }), showScrollUp && _jsx(Text, { color: theme.text.secondary, children: "\u25B2" }), items.length === 0 && (_jsx(Text, { color: theme.text.secondary, children: t('No settings match your search.') })), visibleItems.map((item, idx) => {
                        const isActive = mode === 'settings' &&
                            focusZone === 'list' &&
                        ...
                        if (editingKey === item.value) {
                            ...
                            const isSubDialogSetting = item.value === 'ui.theme' ||
                            ...
                            const effectiveCurrentValue = currentValue !== undefined && currentValue !== null
                                ? currentValue
                                : defaultValue;
                            if (item.value === 'general.outputLanguage' &&
                                isAutoLanguage(effectiveCurrentValue)) {
                                displayValue = t('Auto (follow user input)');
                            }
                            else if (effectiveCurrentValue !== undefined &&
                        ...
                        return (_jsxs(Box, { flexDirection: "row", alignItems: "center", children: [_jsx(Box, { minWidth: 2, flexShrink: 0, children: _jsx(Text, { color: isActive ? theme.status.success : theme.text.secondary, children: isActive ? ICON.CIRCLE_FILLED : '' }) }), _jsx(Box, { flexGrow: 1, flexShrink: 1, children: _jsxs(Text, { color: isActive ? theme.status.success : theme.text.primary, wrap: "truncate", children: [item.label, scopeMessage && (_jsxs(Text, { color: theme.text.secondary, children: [" ", scopeMessage] }))] }) }), _jsx(Box, { marginLeft: 1, flexShrink: 0, children: _jsx(Text, { color: isActive
                                            ? theme.status.success
                                            : shouldBeGreyedOut
                                                ? theme.text.secondary
                                                : theme.text.primary, wrap: "truncate", children: displayValue }) })] }, item.value));
                    }), showScrollDown && _jsx(Text, { color: theme.text.secondary, children: "\u25BC" })] })) : (_jsx(ScopeSelector, { onSelect: handleScopeSelect, onHighlight: handleScopeHighlight, isFocused: mode === 'scope', initialScope: selectedScope })), activeDescription && mode === 'settings' && (_jsx(Box, { marginTop: 1, children: _jsx(Text, { color: theme.text.secondary, wrap: "truncate-end", italic: true, children: activeDescription }) })), activeTab === 'settings' && (_jsx(Box, { marginTop: activeDescription && mode === 'settings' ? 0 : 1, children: _jsx(Text, { color: theme.text.secondary, wrap: "truncate", children: mode === 'settings'
                        ? t('(Use Enter to select, Tab to configure scope)')
                        : t('(Use Enter to apply scope, Tab to go back)') }) })), activeTab === 'settings' &&
                mode === 'settings' &&
                focusZone === 'list' &&
                showRestartPrompt && (_jsx(Text, { color: theme.status.warning, children: t('To see changes, LailatulCoder Ai must be restarted. Press r to exit and apply changes now.') }))] }));
}
