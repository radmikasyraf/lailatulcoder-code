...
export function isIncompleteMouseSequence(buffer) {
    ...
    if (buffer.startsWith(SGR_EVENT_PREFIX)) {
        ...
        return (!/[mM]/.test(buffer) && buffer.length < MAX_SGR_MOUSE_SEQUENCE_LENGTH);
    }
    // Prefix of the prefix (e.g. "ESC" or "ESC [")
    return true;
}
const ENABLE_SGR_MOUSE = {
    button: '\x1b[?1002h\x1b[?1006h',
    any: '\x1b[?1003h\x1b[?1006h',
};
const DISABLE_SGR_MOUSE = {
    button: '\x1b[?1006l\x1b[?1002l',
    any: '\x1b[?1006l\x1b[?1003l',
};
export function enableMouseEvents(stdout, tracking = 'button') {
    stdout.write(ENABLE_SGR_MOUSE[tracking]);
}
export function disableMouseEvents(stdout, tracking = 'button') {
