...
const getMockContentGeneratorConfig = () => ({
...
const mockConfig = {
    ...
    getModelsConfig: vi.fn(() => ({
        getModelDisplayName: (model) => model === 'main-model' ? 'Main Model' : 'Test Model',
    })),
...
let mockKill;
function setStatusLineConfig(config) {
    mockSettings.merged =
...
describe('useStatusLine', () => {
    beforeEach(() => {
        ...
        vi.mocked(child_process.exec).mockImplementation(((cmd, _opts, cb) => {
            lastExecCommand = cmd;
            execCallback = cb;
            stdinWrittenData = '';
            stdinErrorHandler = undefined;
    ...
    describe('config validation', () => {
        ...
        it.each(['context-used', 'context-remaining'])('hides the footer indicator when an unset preset shows %s', (contextItem) => {
            setStatusLineConfig({
                type: 'preset',
        ...
        it.each(['context-used', 'context-remaining'])('keeps the footer indicator when a preset showing %s sets hideContextIndicator false', (contextItem) => {
            setStatusLineConfig({
                type: 'preset',
                items: ['model', contextItem],
                hideContextIndicator: false,
        ...
        it('keeps an automatic indicator when the rendered footer column clips it', () => {
            mockConfig.getTargetDir.mockReturnValue('/home/runner/actions-runner-19/_work/lailatul-coder/lailatul-coder');
    ...
    describe('preset status line', () => {
        ...
        it('logs and retries pull request lookup failures after state changes', async () => {
            ...
            expect(debugLogMock.warn).toHaveBeenCalledWith('statusline: gh pr view failed:', 'gh not authenticated');
    ...
    describe('stdin JSON input', () => {
        ...
        it('falls back to zero when contextWindowSize is unavailable', () => {
            ...
            mockConfig.getContentGeneratorConfig.mockReturnValue(null);
            setStatusLineConfig({ type: 'command', command: 'cat' });
            renderHook(() => useStatusLine());
            const input = JSON.parse(stdinWrittenData);
    ...
    describe('stdin error handling', () => {
        it('silently handles EPIPE errors', () => {
            ...
            const epipeError = new Error('EPIPE');
    ...
    describe('refreshInterval', () => {
        ...
        it('clears the interval when config is removed', async () => {
            ...
            expect(vi.mocked(child_process.exec).mock.calls.length).toBe(callsAfterRemoval);
        });
        ...
        it('clears the interval on unmount', async () => {
            ...
            expect(vi.mocked(child_process.exec).mock.calls.length).toBe(callsAfterUnmount);
        });
...
//# sourceMappingURL=useStatusLine.test.js.map