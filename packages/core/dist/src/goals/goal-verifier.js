...
export class GoalVerifierInputTooLargeError extends Error {
    ...
    constructor(byteLength) {
        super(`Goal verifier request exceeds the ${GOAL_VERIFIER_REQUEST_BYTE_LIMIT}-byte limit`);
...
export function parseGoalVerifierText(text) {
    let value;
    try {
...
export function validateGoalVerifierText(text) {
    try {
...
export function createGoalVerifier(config, options = {}) {
    ...
    return async (input, attemptSignal) => {
        ...
        const timer = setTimeout(() => {
            timeoutController.abort(new Error(`Goal verifier timed out after ${timeoutMs}ms`));
        }, timeoutMs);
...
//# sourceMappingURL=goal-verifier.js.map