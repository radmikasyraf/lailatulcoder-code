...
export class HookRunner {
    ...
    promptRunner;
    asyncRegistry;
    constructor(allowedHttpUrls, config) {
        this.httpRunner = new HttpHookRunner(allowedHttpUrls, config?.getAllowPrivateNetworkHooks());
     ...
     */
    async executeHook(hookConfig, eventName, input, contextOrSignal) {
        const startTime = Date.now();
        // Extract signal from context or use directly
        const signal = contextOrSignal && 'aborted' in contextOrSignal
            ? contextOrSignal
        ...
        try {
            ...
            if (this.isAsyncHook(hookConfig)) {
                return this.executeAsyncHook(hookConfig, eventName, input, signal);
            }
            ...
            switch (hookConfig.type) {
                ...
                case HookType.Http:
                    return await this.httpRunner.execute(hookConfig, eventName, input, signal);
                case HookType.Function: {
                    // Function hooks accept context, not just signal
                    const functionContext = contextOrSignal && !('aborted' in contextOrSignal)
                        ? contextOrSignal
                        : { signal };
                    return await this.functionRunner.execute(hookConfig, eventName, input, functionContext);
                }
                case HookType.Prompt: {
                    // Prompt hooks require Config for LLM access
                    if (!this.promptRunner) {
                        throw new Error('Prompt hook requires Config to be provided to HookRunner');
                    }
                    return await this.promptRunner.execute(hookConfig, eventName, input, signal);
                }
                default:
     ...
     */
    getShellConfigForHook(hookConfig) {
        const globalConfig = getShellConfiguration();
        // If hook specifies a shell, use it
        if (hookConfig.shell) {
            const shellType = hookConfig.shell === 'powershell' ? 'powershell' : 'bash';
            ...
            return {
                shell: 'bash',
                executable: globalConfig.shell === 'bash' ? globalConfig.executable : 'bash',
                argsPrefix: ['-c'],
            };
        }
     ...
     */
    async executeAsyncHook(hookConfig, eventName, input, signal) {
        ...
        if (!this.asyncRegistry.canAcceptMore()) {
            debugLogger.warn(`Async hook rejected due to concurrency limit: ${hookName}`);
            return {
                hookConfig,
                eventName,
                success: false,
                duration: 0,
                isAsync: true,
                error: new Error('Async hook rejected: too many concurrent async hooks running'),
        ...
        if (!registeredId) {
            debugLogger.warn(`Async hook registration failed due to concurrency limit: ${hookName}`);
            return {
                hookConfig,
                eventName,
                success: false,
        ...
        this.executeCommandHookInBackground(hookConfig, eventName, input, hookId, signal).catch((error) => {
            // This catch handles any unexpected errors that escape the try-catch in executeCommandHookInBackground
            debugLogger.error(`Unexpected error in async hook background execution: ${hookId} (${hookName}): ${error instanceof Error ? error.message : String(error)}`);
            // Ensure the hook is marked as failed in the registry
            try {
                this.asyncRegistry.fail(hookId, error instanceof Error
                    ? error
                    : new Error(`Unexpected error: ${String(error)}`));
            }
            catch (registryError) {
                // Registry operation failed, log but don't throw
                debugLogger.error(`Failed to update async registry for hook ${hookId}: ${registryError}`);
            }
        });
     ...
     */
    async executeCommandHookInBackground(hookConfig, eventName, input, hookId, signal) {
        const hookName = hookConfig.name || hookConfig.command || 'async-hook';
        try {
            debugLogger.debug(`Executing async hook in background: ${hookId}`);
            const result = await this.executeCommandHook(hookConfig, eventName, input, Date.now(), signal);
            ...
            if (result.success) {
                ...
                debugLogger.debug(`Async hook completed successfully: ${hookId} (${hookName})`);
            }
            else {
                const error = result.error || new Error('Unknown error');
                this.asyncRegistry.fail(hookId, error);
                debugLogger.warn(`Async hook failed: ${hookId} (${hookName}): ${error.message}`);
            }
        }
        catch (error) {
            const errorObj = error instanceof Error ? error : new Error(String(error));
            this.asyncRegistry.fail(hookId, errorObj);
            debugLogger.error(`Async hook threw exception: ${hookId} (${hookName}): ${errorObj.message}`);
     ...
     */
    async executeHooksParallel(hookConfigs, eventName, input, onHookStart, onHookEnd, signal, context) {
     ...
     * @param context Optional function hook context (messages, toolUseID)
     */
    async executeHooksSequential(hookConfigs, eventName, input, onHookStart, onHookEnd, signal, context) {
        const results = [];
        ...
        if (hookOutput.hookSpecificOutput) {
            switch (eventName) {
                ...
                case HookEventName.UserPromptExpansion:
                    {
                        const additionalContext = createHookOutput(eventName, hookOutput).getAdditionalContext();
