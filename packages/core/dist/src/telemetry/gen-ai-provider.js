...
function providerFromHostname(hostname) {
    if (isHostOrSubdomain(hostname, 'dashscope.aliyuncs.com') ||
    ...
    if (isHostOrSubdomain(hostname, 'openai.azure.com') ||
        isHostOrSubdomain(hostname, 'services.ai.azure.com')) {
        return 'azure.ai.openai';
    }
    if (isHostOrSubdomain(hostname, 'deepseek.com'))
        return 'deepseek';
    if (isHostOrSubdomain(hostname, 'x.ai'))
        return 'x_ai';
    if (isHostOrSubdomain(hostname, 'mistral.ai'))
        return 'mistral_ai';
    ...
    if (isHostOrSubdomain(hostname, 'modelscope.cn'))
        return 'modelscope';
    if (isHostOrSubdomain(hostname, 'xiaomimimo.com'))
        return 'mimo';
    if (isHostOrSubdomain(hostname, 'openrouter.ai'))
        return 'openrouter';
    if (isHostOrSubdomain(hostname, 'requesty.ai'))
        return 'requesty';
    if (isHostOrSubdomain(hostname, 'anthropic.com'))
        return 'anthropic';
    if (isHostOrSubdomain(hostname, 'openai.com'))
        return 'openai';
    return undefined;
}
...
export function resolveGenAiProviderName(config, dashscopeProxyBaseUrl) {
...
export function resolveGenAiOperationName(authType) {
    return authType === 'gemini' || authType === 'vertex-ai'
        ? 'generate_content'
        : 'chat';
}
export function resolveGenAiOutputType(authType, config) {
    if (authType !== 'gemini' && authType !== 'vertex-ai')
        return undefined;
...
//# sourceMappingURL=gen-ai-provider.js.map