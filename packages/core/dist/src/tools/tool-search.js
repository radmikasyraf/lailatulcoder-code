...
import { getLeaderOnlyToolUnavailableMessage, getSubagentPlanToolUnavailableMessage, isLeaderOnlyToolUnavailableInSubagent, isPlanLifecycleToolUnavailableInSubagent, } from '../agents/runtime/subagent-plan-tool-policy.js';
...
const ACTION_TERM_ALIASES = new Map([
...
class ToolSearchInvocation extends BaseToolInvocation {
    ...
    async execute(_signal) {
        ...
        if (!query) {
            return {
                llmContent: 'Error: query is empty. Use `select:ToolName` or free-text keywords.',
                returnDisplay: 'Empty query',
                error: { message: 'Empty query' },
            };
        }
        const maxResults = clamp(this.params.max_results ?? DEFAULT_MAX_RESULTS, 1, HARD_MAX_RESULTS);
        ...
        if (searchTerms.length === 0) {
            return {
                llmContent: 'Error: no search terms extracted from query. Use `select:ToolName` or include keywords.',
        ...
        for (const tool of candidates) {
            if (!candidateMatchesRequired(tool, requiredTerms))
                continue;
    ...
    async loadAndReturnSchemas(names, truncated = []) {
        ...
        for (const requested of names) {
            ...
            try {
                ...
                process.stderr.write(`[ToolSearch] ensureTool failed for "${canonical}": ${err instanceof Error ? err.message : String(err)}\n`);
            }
            if (!tool) {
                missing.push(requested);
                continue;
        ...
        if (newlyRevealed.length > 0) {
            ...
            if (!geminiClient) {
                ...
                try {
                    ...
                    debugLogger.warn('setTools() failed while revealing deferred tools:', err);
                    process.stderr.write(`[ToolSearch] setTools() failed while revealing deferred tools: ${setToolsError}\n`);
                }
            }
            if (setToolsError) {
        ...
        const schemaBlocks = loaded.map((tool) => `<function>${JSON.stringify(tool.schema).replace(/</g, '\\u003c')}</function>`);
        let llmContent = '';
        ...
        if (blocked.length > 0) {
            const blockedMessages = blocked.map((name) => isLeaderOnlyToolUnavailableInSubagent(name)
        ...
        const displayParts = [];
        if (loaded.length > 0)
            displayParts.push(`Loaded ${loaded.length} tool(s)`);
        if (missing.length > 0)
            displayParts.push(`${missing.length} missing`);
        if (blocked.length > 0)
            displayParts.push(`${blocked.length} unavailable`);
        if (truncated.length > 0)
...
export class ToolSearchTool extends BaseDeclarativeTool {
    config;
    static Name = ToolNames.TOOL_SEARCH;
    constructor(config) {
        super(ToolSearchTool.Name, ToolDisplayNames.TOOL_SEARCH, toolSearchDescription, Kind.Other, {
            type: 'object',
            properties: {
                query: {
                    type: 'string',
...
function normalizeSearchTerm(raw) {
    ...
    const normalized = body.replace(/^[^\p{L}\p{N}_.+#-]+|[^\p{L}\p{N}_.+#-]+$/gu, '');
...
function candidateMatchesRequired(tool, requiredTerms) {
    if (requiredTerms.length === 0)
        return true;
    const nameLower = tool.name.toLowerCase();
    return requiredTerms.every((t) => getSearchTermVariants(t).some((variant) => nameLower.includes(variant)));
}
/**
    ...
    for (const term of terms) {
        ...
        for (const variant of variants) {
            if (nameLower === variant ||
                ...
                nameScore = Math.max(nameScore, isMcp ? SCORE_NAME_EXACT_MCP : SCORE_NAME_EXACT_BUILTIN);
            }
            else if (nameLower.includes(variant)) {
                nameScore = Math.max(nameScore, isMcp ? SCORE_NAME_SUBSTR_MCP : SCORE_NAME_SUBSTR_BUILTIN);
            }
        }
        total += nameScore;
        ...
        if (ACTION_TERM_ALIASES.has(term) &&
            variants
                .filter((variant) => variant !== term)
                .some((variant) => nameLower.includes(variant) || hintParts.some((p) => p === variant))) {
