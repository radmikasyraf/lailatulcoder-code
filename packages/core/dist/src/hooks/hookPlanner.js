...
export function getToolMatcherTargets(toolName) {
...
export function hookEventSupportsMatcher(eventName) {
...
export class HookPlanner {
     ...
     */
    matchesFilePath(matcher, filePath) {
        try {
            const regex = new RegExp(matcher);
            return regex.test(filePath);
        }
        catch (error) {
            debugLogger.warn(`Invalid regex in hook matcher "${matcher}" for file path "${filePath}", falling back to exact match: ${error}`);
    ...
    matchesToolName(matcher, toolName) {
        ...
        try {
            ...
            debugLogger.warn(`Invalid regex in hook matcher "${matcher}" for tool "${toolName}", no alias match and invalid regex: ${error}`);
            return false;
        }
    }
    /**
     ...
     */
    matchesCommandName(matcher, commandName) {
        try {
            // Attempt to treat the matcher as a regular expression.
            const regex = new RegExp(matcher);
            ...
            // If it's not a valid regex, treat it as a literal string for an exact match.
            debugLogger.warn(`Invalid regex in hook matcher "${matcher}" for command "${commandName}", falling back to exact match: ${error}`);
