...
import { preconnectApi, resetPreconnectState, waitForPreconnect, } from './apiPreconnect.js';
...
describe('apiPreconnect', () => {
    ...
    describe('resolvedBaseUrl handling', () => {
        it('should use resolvedBaseUrl when it is a default URL', async () => {
            ...
            expect(mockFetch).toHaveBeenCalledWith('https://api.openai.com/v1', expect.objectContaining({ method: 'HEAD' }));
        });
        ...
        it('should use resolvedBaseUrl when it is a dashscope compatible-mode URL', async () => {
            ...
            await waitForPreconnect();
            expect(mockFetch).toHaveBeenCalledWith('https://dashscope.aliyuncs.com/compatible-mode/v1', expect.objectContaining({ method: 'HEAD' }));
        });
        ...
        it('should accept DashScope regional endpoint (sg-singapore)', async () => {
            preconnectApi('openai', {
                resolvedBaseUrl: 'https://dashscope-intl.aliyuncs.com/compatible-mode/v1',
                proxy: 'http://proxy.example.com:8080',
            });
            await waitForPreconnect();
            expect(mockFetch).toHaveBeenCalledWith('https://dashscope-intl.aliyuncs.com/compatible-mode/v1', expect.objectContaining({ method: 'HEAD' }));
        });
        it('should accept DashScope regional endpoint (us-virginia)', async () => {
            ...
            await waitForPreconnect();
            expect(mockFetch).toHaveBeenCalledWith('https://dashscope-us.aliyuncs.com/compatible-mode/v1', expect.objectContaining({ method: 'HEAD' }));
        });
        it('should accept DashScope regional endpoint (cn-hongkong)', async () => {
            preconnectApi('openai', {
                resolvedBaseUrl: 'https://cn-hongkong.dashscope.aliyuncs.com/compatible-mode/v1',
                proxy: 'http://proxy.example.com:8080',
            });
            await waitForPreconnect();
            expect(mockFetch).toHaveBeenCalledWith('https://cn-hongkong.dashscope.aliyuncs.com/compatible-mode/v1', expect.objectContaining({ method: 'HEAD' }));
        });
        it('should fall back to authType default when resolvedBaseUrl is a non-URL sentinel', async () => {
            ...
            await waitForPreconnect();
            expect(mockFetch).toHaveBeenCalledWith('https://coding.dashscope.aliyuncs.com', expect.objectContaining({ method: 'HEAD' }));
        });
    ...
    describe('preconnect behavior', () => {
        ...
        it('should use default baseUrl for openai', async () => {
            ...
            await waitForPreconnect();
            expect(mockFetch).toHaveBeenCalledWith('https://api.openai.com', expect.objectContaining({ method: 'HEAD' }));
        });
        it('should use default baseUrl for anthropic', async () => {
            ...
            await waitForPreconnect();
            expect(mockFetch).toHaveBeenCalledWith('https://api.anthropic.com', expect.objectContaining({ method: 'HEAD' }));
        });
        ...
        it('should pass configured proxy to shared dispatcher', async () => {
            ...
            await waitForPreconnect();
            expect(mockGetOrCreateSharedDispatcher).toHaveBeenCalledWith('http://proxy.example.com:8080');
        });
        ...
        it('should skip dispatcher creation when no proxy configured', () => {
            ...
            expect(mockDebugLogger.debug).toHaveBeenCalledWith('Skipping preconnect dispatcher: no proxy configured');
        });
        ...
        it('should handle fetch errors gracefully', async () => {
            ...
            // Should not throw
            expect(() => preconnectApi('qwen-oauth', { proxy: 'http://proxy.example.com:8080' })).not.toThrow();
        });
        it('should redact proxy credentials from async fetch errors', async () => {
            mockFetch.mockRejectedValue(new Error('connect ECONNREFUSED token@proxy.local:8080'));
            preconnectApi('qwen-oauth', {
                proxy: 'http://token@proxy.local:8080',
            });
            await waitForPreconnect();
            expect(mockDebugLogger.debug).toHaveBeenCalledWith('Preconnect failed (ignored): Error: connect ECONNREFUSED <redacted>@proxy.local:8080');
            expect(mockDebugLogger.debug).not.toHaveBeenCalledWith(expect.stringContaining('token@'));
        });
        ...
        it('should redact proxy credentials from dispatcher errors', async () => {
            ...
            await waitForPreconnect();
            expect(mockDebugLogger.debug).toHaveBeenCalledWith('Preconnect failed (ignored): Error: connect ECONNREFUSED <redacted>@proxy.local:8080');
            expect(mockDebugLogger.debug).not.toHaveBeenCalledWith(expect.stringContaining('user:pass@'));
        });
...
//# sourceMappingURL=apiPreconnect.test.js.map