...
describe('BuiltinCommandLoader', () => {
    ...
    it('should exclude trust command when folder trust is disabled', async () => {
        mockConfig.getFolderTrust.mockReturnValue(false);
        const loader = new BuiltinCommandLoader(mockConfig);
        const commands = await loader.loadCommands(new AbortController().signal);
        const trustCmd = commands.find((c) => c.name === 'trust');
    ...
    it('should include lsp command only when LSP is enabled', async () => {
        ...
        const disabledCommands = await disabledLoader.loadCommands(new AbortController().signal);
        expect(disabledCommands.find((c) => c.name === 'lsp')).toBeUndefined();
        mockConfig.isLspEnabled.mockReturnValue(true);
        const enabledLoader = new BuiltinCommandLoader(mockConfig);
        const enabledCommands = await enabledLoader.loadCommands(new AbortController().signal);
    ...
    it('should still load all other commands when ideCommand() throws', async () => {
        ...
        const { ideCommand: ideCommandMock } = await import('../ui/commands/ideCommand.js');
        ideCommandMock.mockRejectedValueOnce(new Error('PowerShell not available'));
        const loader = new BuiltinCommandLoader(mockConfig);
        const commands = await loader.loadCommands(new AbortController().signal);
    ...
    it('should always include hooks command regardless of disableAllHooks', async () => {
        ...
        mockConfig.getDisableAllHooks.mockReturnValue(true);
