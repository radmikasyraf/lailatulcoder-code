...
import { AuthType, resolveModelConfig, } from '@lailatul-coder/lailatul-coder-core';
import { getAuthTypeFromEnv, resolveCliGenerationConfig, } from './modelConfigUtils.js';
...
describe('modelConfigUtils', () => {
    ...
    describe('resolveCliGenerationConfig', () => {
        ...
        it('should resolve config from argv with highest precedence', () => {
            ...
            expect(vi.mocked(resolveModelConfig)).toHaveBeenCalledWith(expect.objectContaining({
                cli: {
                    model: 'argv-model',
                    apiKey: 'argv-key',
                    baseUrl: 'https://argv.example.com',
        ...
        it('should resolve OpenAI logging from settings when argv is undefined', () => {
            ...
            expect(result.generationConfig.enableOpenAILogging).toBe(true);
            expect(result.generationConfig.openAILoggingDir).toBe('/settings/log/dir');
        });
        ...
        it('finds a custom-id provider via providerProtocol (honors its envKey/metadata)', () => {
            ...
            expect(vi.mocked(resolveModelConfig)).toHaveBeenCalledWith(expect.objectContaining({ modelProvider }));
        });
        it('warns when a provider id has models but no resolvable protocol', () => {
            ...
            expect(result.warnings.some((w) => w.includes('idealab') && w.includes('providerProtocol'))).toBe(true);
        });
        it('warns when providerProtocol maps to an unknown protocol', () => {
            ...
            expect(result.warnings).toEqual(expect.arrayContaining([
                expect.stringContaining('providerProtocol["idealab"] = "not-a-protocol" is not a known protocol'),
            ]));
        });
        it('warns when a custom provider maps to qwen-oauth', () => {
            ...
            expect(result.warnings).toEqual(expect.arrayContaining([
                expect.stringContaining('modelProviders provider "idealab" maps to "qwen-oauth"'),
            ]));
        });
        ...
        describe('provider disambiguation by settings.model.baseUrl', () => {
            ...
            it('selects the provider matching the persisted baseUrl, not the first id match', () => {
                ...
                expect(vi.mocked(resolveModelConfig)).toHaveBeenCalledWith(expect.objectContaining({ modelProvider: ideaLab }));
            ...
            it('falls back to the first id match when no baseUrl is persisted (backward compat)', () => {
                ...
                expect(vi.mocked(resolveModelConfig)).toHaveBeenCalledWith(expect.objectContaining({ modelProvider: tokenPlan }));
            });
        ...
        it('should warn when top-level generationConfig fields are ignored for a provider model', () => {
            ...
            expect(result.warnings[0]).toContain('model.generationConfig.contextWindowSize');
            expect(result.warnings[0]).toContain('model.generationConfig.extra_body');
            expect(result.warnings[0]).toContain('provider model "qwen3.6-27b" from modelProviders.openai');
            expect(result.warnings[0]).toContain('modelProviders.openai[].generationConfig');
        });
        it('names the custom provider in ignored generationConfig warnings', () => {
            ...
            expect(result.warnings[0]).toContain('provider model "qwen3.6-27b" from modelProviders.idealab');
            expect(result.warnings[0]).toContain('modelProviders.idealab[].generationConfig');
        ...
        it('should append ignored generationConfig warnings to resolver warnings', () => {
            ...
            expect(result.warnings[1]).toContain('model.generationConfig.modalities is ignored');
        ...
        it('Integration: settings.model.name wins over OPENAI_MODEL even when unmatched', () => {
            ...
            vi.mocked(resolveModelConfig).mockImplementation((input) => {
                const model = input?.modelProvider?.id ||
                    input?.cli?.model ||
        ...
        describe('[Regression] model precedence', () => {
            it('[Regression] settings.model.name must NOT be overridden by OPENAI_MODEL', () => {
                ...
                vi.mocked(resolveModelConfig).mockImplementation(() => ({
                    ...
                    sources: {
                        model: { kind: 'settings', path: 'model.name' },
                    },
                    warnings: [],
                }));
                const result = resolveCliGenerationConfig({
            ...
            it('[Regression] argv.model overrides both settings and OPENAI_MODEL', () => {
                ...
                vi.mocked(resolveModelConfig).mockImplementation(() => ({
                    ...
                    sources: { model: { kind: 'cli', detail: '--model' } },
                    warnings: [],
                }));
                const result = resolveCliGenerationConfig({
                    argv,
            ...
            it('[Regression] QWEN_MODEL as fallback when OPENAI_MODEL not set', () => {
                ...
                vi.mocked(resolveModelConfig).mockImplementation(() => ({
                    ...
                    sources: { model: { kind: 'env', envKey: 'QWEN_MODEL' } },
                    warnings: [],
                }));
                resolveCliGenerationConfig({
                    argv: {},
            ...
            it('[Regression] Non-OpenAI auth ignores OPENAI_MODEL', () => {
                ...
                vi.mocked(resolveModelConfig).mockImplementation(() => ({
                    ...
                    sources: {
                        model: { kind: 'env', envKey: 'ANTHROPIC_MODEL' },
                    },
                    warnings: [],
                }));
                const result = resolveCliGenerationConfig({
...
//# sourceMappingURL=modelConfigUtils.test.js.map