...
function buildModelConfigs(config, inputs) {
    ...
    if (config.models && !config.modelsEditable) {
        models = config.models.map((spec) => specToModelConfig(spec, prefix, inputs.baseUrl, envKey));
    }
...
export function resolveMetadataKey(config) {
    ...
    if (config.id.includes('.')) {
        throw new Error(`Provider id must not contain '.' (would corrupt providerMetadata.${config.id} dotted writes): ${config.id}`);
    }
    return config.id;
}
/**
...
export function buildInstallPlan(config, inputs) {
    ...
    const modelSelection = firstModelId === undefined
        ? undefined
        : {
...
export function resolveBaseUrl(config, selectedBaseUrl) {
    ...
    if (Array.isArray(config.baseUrl)) {
        ...
        const match = config.baseUrl.find((opt) => normalizeBaseUrlForMatching(opt.url) === normalizedSelectedBaseUrl);
        if (match)
            return match.url;
 ...
 */
export function findExistingProviderModels(config, modelProviders) {
    ...
    for (const protocol of protocols) {
        ...
        if (!Array.isArray(raw))
            continue;
        const models = raw.filter((m) => isProviderModelConfig(m) && ownsModel(m));
        if (models.length > 0)
    ...
    if (typeof config.envKey === 'string') {
        ...
        for (const proto of protocols) {
            try {
                ...
                console.warn(`[providerMatchesCredentials] envKey(${proto}, ${safeHost}) threw (${err instanceof Error ? err.constructor.name : typeof err}); skipping this protocol`);
            }
        }
    }
