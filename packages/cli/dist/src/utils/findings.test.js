...
import { anchorRequestsFor, applyOutcomes, buildReport, compressSummary, CONFIDENCES, findingsCommand, OUTCOMES, renderFindings, SEVERITIES, sortFindings, SOURCES, validateFindings, validateOutcomes, holdCriticalsFailingOnBase, holdUnwitnessedCriticals, sharedFailingFilesOf, } from './findings.js';
...
const base = {
    ...
    failureScenario: 'A request that fails twice then succeeds leaves `attempts` at 2; the next unrelated request starts at 2 and is rejected after one failure.',
    file: 'src/retry.ts',
    line: 42,
};
...
describe('validateFindings', () => {
    ...
    it('still rejects an unknown source, bracketed or not', () => {
        expect(() => validateFindings([{ ...base, source: '[bogus]' }])).toThrow(/has source "\[bogus\]"; expected one of/);
        expect(() => validateFindings([{ ...base, source: '[]' }])).toThrow(/has source "\[\]"; expected one of/);
    });
    ...
    it('rejects a finding with no failure scenario', () => {
        ...
        expect(() => validateFindings([{ ...base, failureScenario: undefined }])).toThrow(/failureScenario/);
    });
    it.each([
        ['id', { id: '' }],
        ['summary', { summary: '   ' }],
        ...
        expect(() => validateFindings([{ ...base, ...patch }])).toThrow(/Finding at index 0/);
    });
    it('names the index and the field, and does not throw a TypeError on null', () => {
        expect(() => validateFindings([base, null])).toThrow(/Finding at index 1: is null, not an object/);
    });
    it('rejects an unknown severity, listing the ladder', () => {
        expect(() => validateFindings([{ ...base, severity: 'Blocker' }])).toThrow(/"Critical", "Suggestion", "Nice to have"/);
    });
    it('rejects a duplicate id — ids are what outcomes and anchors join on', () => {
        expect(() => validateFindings([base, { ...base, file: 'src/other.ts' }])).toThrow(/Duplicate finding id "f1"/);
    });
    ...
    it('rejects an empty locations array rather than treating it as standalone', () => {
        expect(() => validateFindings([{ ...base, file: undefined, locations: [] }])).toThrow(/non-empty array/);
    });
    it.each(['42', -1, 0, 1.5, Number.NaN, Number.POSITIVE_INFINITY])('rejects an invalid line value %s', (line) => {
        expect(() => validateFindings([{ ...base, line }])).toThrow(/invalid "line"/);
    });
    ...
    it('rejects a top-level input that is not an array', () => {
        expect(() => validateFindings({ findings: [] })).toThrow(/must be a JSON array/);
    });
});
describe('validateFindings — evidence assets', () => {
    ...
    it('rejects a non-array assetFiles, naming the finding and the field', () => {
        expect(() => validateFindings([{ ...base, assetFiles: 'shot.png' }])).toThrow(/Finding at index 0: "assetFiles" must be an array/);
    });
    it('rejects an empty-string entry rather than publishing a nameless file', () => {
        expect(() => validateFindings([{ ...base, assets: ['ok', ''] }])).toThrow(/"assets" must be an array of non-empty strings/);
    });
    it('rejects a whitespace-only entry, matching the sibling asString rule', () => {
        expect(() => validateFindings([{ ...base, assetFiles: ['  '] }])).toThrow(/"assetFiles" must be an array of non-empty strings/);
    });
...
describe('compressSummary', () => {
    it('passes a short summary through unchanged', () => {
        expect(compressSummary('Retry counter never reset')).toBe('Retry counter never reset');
    });
    it('flattens whitespace so a wrapped summary fits one list cell', () => {
        expect(compressSummary('Retry\n  counter   never reset')).toBe('Retry counter never reset');
    });
    it('cuts on a word boundary and stays within the limit', () => {
        const long = 'The retry counter is never reset, so a later unrelated request is refused after a single failure';
...
describe('validateOutcomes', () => {
    it('rejects an outcome word outside the ladder', () => {
        ...
        expect(() => validateOutcomes([{ id: 'f1', outcome: 'wontfix' }])).toThrow(/"fixed", "skipped", "no_change_needed"/);
    });
    it('rejects an entry with no id', () => {
        expect(() => validateOutcomes([{ outcome: 'fixed' }])).toThrow(/index 0 is missing a string "id"/);
    });
});
describe('buildReport', () => {
    it('counts by severity and confidence and reports no outcomes yet', () => {
        const report = buildReport(validateFindings([
            base,
    ...
    it('reports outcomes only once every finding carries one', () => {
        ...
        const half = [{ ...findings[0], outcome: 'fixed' }, findings[1]];
...
describe('renderFindings', () => {
    it('marks low confidence and outcome, and counts extra locations', () => {
        const report = buildReport(applyOutcomes(validateFindings([
            {
                ...base,
                confidence: 'low',
                locations: [
        ...
        expect(renderFindings(report)[0]).toBe('Critical — a.ts:1 (+1 more) — The retry counter is never reset, so the third attempt is… [low confidence] [skipped]');
...
describe('anchorRequestsFor', () => {
    ...
    const noRequestShapes = [
        ['low-confidence', { confidence: 'low' }],
        ['anchorless', { locations: [{ file: 'z.ts', line: 9 }] }],
        ['Nice to have', { severity: 'Nice to have' }],
    ];
    it.each(noRequestShapes)('refuses the collision when the other finding emits no request (%s)', (_shape, over) => {
        expect(() => anchorRequestsFor([
            finding({
                id: 'p1',
                locations: [
...
describe('findings (command boundary)', () => {
    ...
    function run(findings, outcomes) {
    ...
    function runCapturingStderr(argv) {
        let out = '';
        const spy = vi
        ...
        writeFileSync(input, JSON.stringify([
            {
                ...
                id: 'R1-3',
                severity: 'Critical',
                failureScenario: 'packages/cli/src/ui/auth/AuthDialog.test.tsx goes red on this change.',
            },
        ]));
        writeFileSync(delta, JSON.stringify({
            entries: [
                {
    ...
    it('--to-anchors skips a Critical the witness rule demoted to low confidence', () => {
        ...
        const requests = JSON.parse(readFileSync(anchors, 'utf8'));
        expect(requests.map((r) => r.id)).toEqual(['kept']);
    });
    ...
    it('--to-anchors leaves the previous pair untouched when the projection throws', () => {
        ...
        expect(() => findingsCommand.handler({
            input,
            out,
            toAnchors: anchors,
            print: false,
    ...
    it('--to-anchors names the postable locations it cannot project', () => {
        ...
        // is discarded.
        expect(stderr).toContain('anchorless-c carries 1 location(s) without an anchor — ' +
        ...
        expect(stderr).toContain('agg carries 1 location(s) without an anchor — absent from the ' +
    ...
    it('counts the holds and reaches the same severity with and without outcomes', () => {
        ...
        expect(b.findings[0].heldByMeasurement).toEqual(a.findings[0].heldByMeasurement);
    });
    ...
    it('merges a complete ledger and records the outcome counts', () => {
        const report = run([base, { ...base, id: 'f2', severity: 'Suggestion' }], [
            { id: 'f1', outcome: 'fixed' },
    ...
    it('throws rather than writing an artifact for an incomplete ledger', () => {
        expect(() => run([base, { ...base, id: 'f2' }], [{ id: 'f1', outcome: 'fixed' }])).toThrow(/No outcome recorded for 1 finding\(s\)/);
    });
    ...
    it('refuses a --to-anchors that is the same file as another path argument', () => {
        ...
        for (const flag of ['input', 'out', 'outcomes', 'testDelta']) {
            for (const [flagPath, anchorPath] of [
                ...
                expect(() => findingsCommand.handler(argv)).toThrow(/--to-anchors points at the same file/);
            }
        }
    });
    it('refuses a --to-anchors that is a symlink', () => {
        ...
        expect(() => findingsCommand.handler(makeArgv(alias))).toThrow(/--to-anchors must not be a symlink/);
        const dangling = join(dir, 'dangling.json');
        symlinkSync(join(dir, 'nowhere.json'), dangling);
        expect(() => findingsCommand.handler(makeArgv(dangling))).toThrow(/--to-anchors must not be a symlink/);
    });
    ...
    it('refuses a collision spelled through a symlinked directory', () => {
        ...
        expect(() => findingsCommand.handler(makeArgv(join(dir, 'link/findings.json'), join(dir, 'real/findings.json')))).toThrow(/--to-anchors points at the same file/);
        expect(() => findingsCommand.handler(makeArgv(join(dir, 'real/findings.json'), join(dir, 'link/findings.json')))).toThrow(/--to-anchors points at the same file/);
        ...
        expect(() => findingsCommand.handler(makeArgv(join(dir, 'link/shared.json'), join(dir, 'real/shared.json')))).toThrow(/--to-anchors points at the same file/);
        expect(() => findingsCommand.handler(makeArgv(join(dir, 'real/shared.json'), join(dir, 'link/shared.json')))).toThrow(/--to-anchors points at the same file/);
    });
    it('refuses a --to-anchors nested inside a sibling path, or containing one', () => {
        ...
        const makeArgv = (outArg, toAnchors) => ({
            input,
            out: outArg,
            outcomes: undefined,
            testDelta: undefined,
        ...
        expect(() => findingsCommand.handler(makeArgv(out, anchors))).toThrow(/--to-anchors must not nest inside/);
        expect(() => findingsCommand.handler(makeArgv(anchors, out))).toThrow(/--to-anchors must not nest inside/);
    ...
    it('parses --to-anchors into the field the handler actually reads', () => {
        ...
        const parsed = findingsCommand.builder(yargs([]).strict()).parseSync([
            '--input',
            input,
            '--out',
            out,
...
describe('holdUnwitnessedCriticals — the witness rule has a machine half', () => {
    ...
    it('exempts deterministic sources — their witness is constitutive', () => {
        ...
        for (const source of ['build', 'test', 'probe', 'lint']) {
    ...
    it('is idempotent — a demoted finding re-fed is not touched again', () => {
        ...
        // as a blocker.
        expect(holdUnwitnessedCriticals([{ ...critical, severity: 'Suggestion' }])
            .unwitnessed).toEqual([]);
    });
});
describe('holdCriticalsFailingOnBase', () => {
    ...
    const critical = {
        ...
        failureScenario: "packages/cli/src/ui/auth/AuthDialog.test.tsx > 'drives API key provider steps' expects MiniMax visible without scrolling.",
    ...
    it('leaves a Critical alone when the test it names is not shared', () => {
        const { findings, held } = holdCriticalsFailingOnBase([critical], ['packages/cli/src/other/unrelated.test.ts']);
        expect(findings[0].severity).toBe('Critical');
    ...
    it('holds the same finding whether or not the fixer already applied it', () => {
        ...
        const fixed = holdCriticalsFailingOnBase([{ ...critical, outcome: 'fixed' }], shared);
    ...
    it('leaves a re-filed Critical alone once it already carries the measurement', () => {
        ...
        const again = holdCriticalsFailingOnBase([{ ...once, severity: 'Critical' }], shared);
        ...
        const count = (t) => (t.match(/Held back from Critical by measurement:/g) ?? []).length;
        ...
        const { findings } = holdCriticalsFailingOnBase([
            {
                ...
                failureScenario: 'It asserts `visible` where the contract is `enabled`, so a regression that flips enabled ships green.',
                locations: [
                    {
                        file: 'packages/cli/src/ui/auth/AuthDialog.test.tsx',
                        line: 40,
        ...
        const { findings } = holdCriticalsFailingOnBase([
            {
                ...
                suggestedFix: 'Add a case in packages/cli/src/ui/auth/AuthDialog.test.tsx covering the guard.',
            },
        ], shared);
        expect(findings[0].severity).toBe('Critical');
    });
    it('never touches a finding that is not Critical', () => {
        const { findings, held } = holdCriticalsFailingOnBase([{ ...critical, severity: 'Suggestion' }], shared);
        expect(findings[0].severity).toBe('Suggestion');
        expect(findings[0].failureScenario).toBe(critical.failureScenario);
        expect(held).toEqual([]);
    });
        ...
        const { findings, held } = holdCriticalsFailingOnBase([
            {
                ...
                failureScenario: 'third_party/packages/cli/src/ui/auth/AuthDialog.test.tsx is red',
            },
        ], shared);
        expect(findings[0].severity).toBe('Critical');
        expect(held).toEqual([]);
    ...
    it('matches a path that ends a sentence', () => {
        ...
        const { findings } = holdCriticalsFailingOnBase([
            {
                ...critical,
                failureScenario: 'It goes red in packages/cli/src/ui/auth/AuthDialog.test.tsx.',
    ...
    it('does not match a longer extension on the same stem', () => {
        const { findings } = holdCriticalsFailingOnBase([
            {
                ...critical,
                failureScenario: 'packages/cli/src/ui/auth/AuthDialog.test.tsx.snap is stale',
                locations: [{ file: 'packages/cli/src/ui/auth/AuthDialog.tsx' }],
            },
        ], shared);
        expect(findings[0].severity).toBe('Critical');
    ...
    it('requires a boundary after the match, not only before it', () => {
        ...
        const { findings } = holdCriticalsFailingOnBase([
            {
                ...critical,
                failureScenario: 'packages/cli/src/ui/auth/AuthDialog.test.tsx is red',
                locations: [{ file: 'packages/cli/src/ui/auth/AuthDialog.tsx' }],
            },
    ...
    it('requires a path boundary, so a longer directory name is not a match', () => {
        const { findings } = holdCriticalsFailingOnBase([
            {
                ...critical,
                failureScenario: 'vendor/other-src/ui/auth/AuthDialog.test.tsx is red',
                locations: [{ file: 'packages/cli/src/ui/auth/AuthDialog.tsx' }],
            },
        ], shared);
        expect(findings[0].severity).toBe('Critical');
...
describe('sharedFailingFilesOf — cross-workspace identity', () => {
    ...
    it('does not hold a Critical about the OTHER workspace of the same path', () => {
        ...
        expect(holdCriticalsFailingOnBase([critical], shared).findings[0].severity).toBe('Critical');
        // ...while the workspace it WAS measured in is still held.
        const cli = {
            ...critical,
            failureScenario: 'packages/cli/src/utils/errors.test.ts goes red.',
        };
        expect(holdCriticalsFailingOnBase([cli], shared).findings[0].severity).toBe('Suggestion');
    });
    it('drops a file some other command measured as net-new', () => {
        expect(sharedFailingFilesOf({
            entries: [
    ...
    it('holds nothing from an artifact with no entries to qualify against', () => {
        ...
        expect(sharedFailingFilesOf({ shared: ['src/utils/errors.test.ts'] }).shared).toEqual([]);
    });
...
describe('sharedFailingFilesOf', () => {
    it('takes the shared list from the top level and from every entry', () => {
        ...
        }).shared.sort()).toEqual(['src/a.test.ts', 'src/b.test.ts', 'src/c.test.ts']);
    });
