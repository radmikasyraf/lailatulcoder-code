...
import { HistoryReplayer, MISSING_TOOL_RESULT_MESSAGE, } from './history-replayer.js';
import { collectHistoryReplayUpdates, createReplayCumulativeUsage, replayTranscriptRecordPage, } from './history-replay-page.js';
const observeAcpProjectionMock = vi.hoisted(() => vi.fn());
vi.mock('../../utils/tool-result-boundary-diagnostics.js', async (original) => ({
    ...(await original()),
...
function largeToolResultRecord(textParts, resultDisplay) {
    return {
        uuid: 'tool-record',
...
describe('history replay page', () => {
    it('bounds textual tool results collected for bulk replay', async () => {
        ...
        const update = result.updates.find((candidate) => candidate.sessionUpdate === 'tool_call_update');
    ...
    it('bounds multi-block textual tool results in paged replay', async () => {
        const page = recordPage({
            records: [
                largeToolResultRecord(['a'.repeat(300_000), 'b'.repeat(300_000)], 'r'.repeat(600_001)),
            ],
        });
        ...
        expect(jsonBytes(record['content'])).toBeLessThanOrEqual(65_536);
        expect(jsonBytes(record['rawOutput'])).toBeLessThanOrEqual(65_536);
        expect(record['content'].map((block) => block.content.text)).toHaveLength(2);
    });
    it('lifts record timestamps for bulk replay callers', async () => {
        ...
        const projectionCall = observeAcpProjectionMock.mock.calls.find(([, , sessionId]) => sessionId === SESSION_ID);
    ...
    it('attaches the checkpoint only to the final chunk of a multi-chunk Assistant record', async () => {
        ...
        const readBranchRecordId = (update) => {
            const meta = update._meta;
            const transcript = meta && typeof meta['qwenTranscript'] === 'object'
                ? meta['qwenTranscript']
                : undefined;
            const branchRecordId = transcript?.['branchRecordId'];
            return typeof branchRecordId === 'string' ? branchRecordId : undefined;
        };
        const decorated = result.updates.filter((update) => readBranchRecordId(update) !== undefined);
        expect(decorated).toHaveLength(1);
        expect(decorated[0]).toMatchObject({
            sessionUpdate: 'agent_message_chunk',
            content: { type: 'text', text: 'last part' },
        });
        const thoughtChunk = result.updates.find((update) => update.sessionUpdate === 'agent_thought_chunk');
        expect(thoughtChunk).toBeDefined();
        expect(readBranchRecordId(thoughtChunk)).toBeUndefined();
        const firstChunk = result.updates.find((update) => update.sessionUpdate === 'agent_message_chunk' &&
            update.content?.text ===
                'first part');
    ...
    it('fails incrementally before collecting an update above the count limit', async () => {
        await expect(collectHistoryReplayUpdates({
            sessionId: SESSION_ID,
            records: [userRecord()],
            cumulativeUsage: createReplayCumulativeUsage(),
    ...
    it('filters malformed replay state before encoding the next cursor', async () => {
        ...
        expect(logger.warn).toHaveBeenCalledWith(expect.stringContaining('dropped 1 of 2 malformed pending tool calls'));
        expect(encodeCursor).toHaveBeenCalledWith(expect.objectContaining({
            replay: {
                v: 1,
                pendingToolCalls: [
                    {
    ...
    it('drops a malformed goalState from replay state and warns', async () => {
        ...
        expect(logger.warn).toHaveBeenCalledWith('[transcript] replay state dropped a malformed Goal state');
        expect(replayPage).toHaveBeenCalledWith([], {
            pendingToolCalls: [],
            finalizeDangling: true,
            gaps: [],
    ...
    it('drops a malformed goalCause from replay state and warns', async () => {
        ...
        expect(logger.warn).toHaveBeenCalledWith('[transcript] replay state dropped a malformed Goal cause');
        expect(replayPage).toHaveBeenCalledWith([], {
            pendingToolCalls: [],
            finalizeDangling: true,
            gaps: [],
    ...
    it('keeps checkpoint bookkeeping suppressed across a page boundary', async () => {
        ...
        const goal = GOAL_STATE.goal;
        const rejectedGoal = { ...goal, lastReason: 'More work remains' };
        const goalRecord = (uuid, cause, snapshotGoal) => ({
            uuid,
            parentUuid: null,
            sessionId: SESSION_ID,
            timestamp: TIMESTAMP,
        ...
        const secondPage = await replayTranscriptRecordPage({
            ...
            page: recordPage({
                records: [
                    goalRecord('goal-reject-checkpoint', 'verifier_reject', recommittedGoal),
                ],
                replay: nextReplay,
            }),
            encodeCursor: vi.fn(),
    ...
    it.each([undefined, 'backward'])('keeps a dangling tool call in progress while its prompt is active (%s)', async (direction) => {
        const result = await replayTranscriptRecordPage({
            sessionId: SESSION_ID,
            page: recordPage({
    ...
    it('terminates pagination when replay conversion fails', async () => {
        vi.spyOn(HistoryReplayer.prototype, 'replayPage').mockRejectedValueOnce(new Error('replay failed'));
        const encodeCursor = vi.fn(() => 'next-cursor');
        const result = await replayTranscriptRecordPage({
            sessionId: SESSION_ID,
            page: recordPage({
    ...
    it('rejects an unknown replay cursor state version', async () => {
        await expect(replayTranscriptRecordPage({
            sessionId: SESSION_ID,
...
//# sourceMappingURL=history-replay-page.test.js.map