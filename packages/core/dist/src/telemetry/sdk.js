...
let telemetryInitPromise;
let telemetryShutdownPromise;
let activeMetricReader;
 ...
 */
export function refreshSessionContext(sessionId) {
    if (!telemetryInitialized)
        return;
    try {
        setSessionContext(createSessionRootContext(sessionId), sessionId);
    ...
    telemetryShutdownPromise = (async () => {
        ...
        try {
            ...
            sdkShutdown.catch((err) => {
                if (timedOut) {
                    debugLogger.warn('SDK shutdown rejected after timeout:', err instanceof Error ? err.message : err);
                }
...
export async function forceFlushMetrics() {
    if (!telemetryInitialized || !activeMetricReader)
        return;
    const flush = activeMetricReader.forceFlush();
    flush.catch(() => { });
    let timer;
    const timeout = new Promise((_, reject) => {
        timer = setTimeout(() => reject(new Error(`forceFlushMetrics timed out after ${FORCE_FLUSH_TIMEOUT_MS}ms`)), FORCE_FLUSH_TIMEOUT_MS);
        timer.unref?.();
    });
    try {
