...
import { buildClassifierSystemPrompt, STAGE1_SUFFIX, STAGE2_SUFFIX, } from './classifier-prompts/system-prompt.js';
 ...
 */
export async function classifyAction(input) {
    ...
    try {
        contents = buildClassifierContents(input.messages, input.config.getToolRegistry(), { toolName: input.toolName, toolParams: input.toolParams });
        baseSystemPrompt = buildClassifierSystemPrompt(input.config);
    }
    catch (err) {
        return failUnavailable('Classifier prompt construction failed', err, 'fast', overallStart, input.config);
    }
    ...
    let stage1;
    try {
        stage1 = (await runSideQuery(input.config, {
            contents,
        ...
        if (input.signal.aborted)
            throw err;
        return failUnavailable('Classifier stage 1 unavailable', err, 'fast', overallStart, input.config);
    }
    if (!stage1.shouldBlock) {
        ...
        debugLogger.debug(`ALLOW stage=fast tool=${input.toolName} durationMs=${Date.now() - overallStart}`);
        return {
            shouldBlock: false,
            reason: '',
    ...
    let stage2;
    try {
        stage2 = (await runSideQuery(input.config, {
            contents,
        ...
        debugLogger.warn(`Stage 2 review unavailable (durationMs=${Date.now() - overallStart}): ${errMessage(err)}`);
        return {
            shouldBlock: true,
    ...
    debugLogger.debug(`${stage2.shouldBlock ? 'BLOCK' : 'ALLOW'} stage=thinking ` +
...
function resolveClassifierSettings(config) {
    const classifier = config.getAutoModeSettings().classifier;
    return {
        stage1TimeoutMs: resolveTimeoutMs(classifier?.timeouts?.stage1Ms, STAGE1_TIMEOUT_MS),
        stage2TimeoutMs: resolveTimeoutMs(classifier?.timeouts?.stage2Ms, STAGE2_TIMEOUT_MS),
...
function resolveTimeoutMs(value, fallback) {
    if (typeof value === 'number' &&
        ...
        value > 0 &&
        value < 1000) {
        debugLogger.warn(`Classifier timeout ${value}ms below 1000ms floor, using default ${fallback}ms`);
    }
 ...
 */
export function sanitizeClassifierReason(raw) {
    if (!raw)
        return raw;
...
function failUnavailable(baseMessage, err, stage, startedAt, config) {
    ...
    debugLogger.warn(`failUnavailable stage=${stage} durationMs=${Date.now() - startedAt} ` +
