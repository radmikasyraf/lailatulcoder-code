...
import { getCurrentSessionId, getSessionIdFromContext, } from './session-context.js';
...
function validateUrl(url) {
    ...
    try {
        ...
        if (parsed.protocol !== 'http:' && parsed.protocol !== 'https:') {
            diag.error(`OTLP endpoint must use http or https, got ${parsed.protocol}`);
            return undefined;
        }
        if (!parsed.hostname) {
    ...
    onStart(span, parentContext) {
        try {
            ...
            if (typeof existingSessionId === 'string' && existingSessionId)
                return;
            const sessionId = getSessionIdFromContext(parentContext) ??
 ...
 */
export async function startTelemetrySdk(config) {
    ...
    const { 'service.name': _ignoredServiceName, 'service.version': _ignoredServiceVersion, 'session.id': _ignoredSessionId, ...nonReservedUserAttrs } = userAttrs;
    ...
    if (attrWarnings.length > 0) {
        ...
        console.warn(`[lailatul-coder telemetry] ${attrWarnings.length} resource attribute issue(s):`);
    ...
    const hasPerSignalEndpoint = !!config.getTelemetryOtlpTracesEndpoint() ||
    ...
    if (useOtlp) {
        if (otlpProtocol === 'http') {
            const tracesUrl = validateUrl(config.getTelemetryOtlpTracesEndpoint() ??
                (parsedEndpoint
                    ? resolveHttpOtlpUrl(parsedEndpoint, 'traces')
                    : undefined));
            const logsUrl = validateUrl(config.getTelemetryOtlpLogsEndpoint() ??
                (parsedEndpoint
                    ? resolveHttpOtlpUrl(parsedEndpoint, 'logs')
                    : undefined));
            const metricsUrl = validateUrl(config.getTelemetryOtlpMetricsEndpoint() ??
                (parsedEndpoint
                    ? resolveHttpOtlpUrl(parsedEndpoint, 'metrics')
                    : undefined));
            debugLogger.debug(`OTLP HTTP endpoints: traces=${tracesUrl ?? 'none'}, logs=${logsUrl ?? 'none'}, metrics=${metricsUrl ?? 'none'}`);
                ...
                const httpExporters = createHttpExporters({
                    ...
                    logToSpan: {
                        ...
                        ...(config.isInteractive() && {
                            diagnosticsSink: (message) => debugLogger.warn(message),
                        }),
                    },
                });
            ...
            if (!parsedEndpoint) {
                const warning = 'Per-signal OTLP endpoints are only supported with HTTP protocol. ' +
    ...
    function normalizeOtlpPrefix(raw) {
        ...
        try {
            ...
            diag.warn(`Telemetry OTLP endpoint "${raw}" is not a valid URL; instrumentation feedback-loop guard for it is disabled.`);
            return undefined;
        }
    }
    ...
    // end-of-string.
    const matchesOtlpPrefix = (origin, path) => {
        for (const prefix of otlpUrlPrefixes) {
            if (origin !== prefix.origin)
                continue;
            if (prefix.pathname === '')
                return true;
            if (!path.startsWith(prefix.pathname))
                continue;
    ...
    const textMapPropagator = config.getOutboundCorrelationPropagateTraceContext()
        ...
        instrumentations: [
            new HttpInstrumentation({
                ...
                ignoreOutgoingRequestHook: (req) => {
                    ...
                    const portPart = req.port !== undefined && req.port !== null && String(req.port)
                        ? `:${req.port}`
                        : '';
                    ...
                    const path = typeof req.path === 'string' ? stripPathSuffix(req.path) : '';
...
//# sourceMappingURL=sdk-impl.js.map