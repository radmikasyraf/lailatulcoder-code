...
describe('WorkspaceChannelSettingsStore', () => {
    ...
    let workspace;
    let settingsPath;
    let originalQwenHome;
    const writeWorkspaceSettings = (contents) => {
        fs.mkdirSync(path.dirname(settingsPath), { recursive: true });
        fs.writeFileSync(settingsPath, contents);
    };
    const readWorkspaceSettings = () => JSON.parse(stripJsonComments(fs.readFileSync(settingsPath, 'utf8')));
    beforeAll(() => {
        registerPlugin({
  ...
  "serve": { "port": 4123 }
    ...
    it('preserves an existing secret unless replace or clear is explicit', async () => {
        ...
        expect(readWorkspaceSettings()['channels']['bot']).toEqual({
            type: 'management-validation-test',
            clientId: 'client-id',
            senderPolicy: 'pairing',
            clientSecret: '$BOT_TOKEN',
    ...
    it('preserves an existing secret when its operation is omitted', async () => {
        ...
        expect(readWorkspaceSettings()['channels']['bot']?.['clientSecret']).toBe('$BOT_TOKEN');
    });
    it('accepts chat-and-thread session scope', async () => {
        ...
        expect(readWorkspaceSettings()['channels']['bot']?.['sessionScope']).toBe('chat_thread');
    });
  ...
  "channels": { "bot": {
    ...
    "optionalSecret": "old-secret"
        ...
        expect(readWorkspaceSettings()['channels']['bot']?.['optionalSecret']).toBe('new-secret');
        await store.upsert('bot', {
            expectedRevision: replaced.revision,
            config: { type: 'management-validation-test', clientId: 'client-id' },
            secrets: { optionalSecret: { operation: 'clear' } },
        });
        expect(readWorkspaceSettings()['channels']['bot']).not.toHaveProperty('optionalSecret');
    });
    it('rejects blank replacements and secret keys not declared by the plugin', async () => {
        const store = new WorkspaceChannelSettingsStore(workspace);
        const revision = store.snapshot().revision;
        await expect(store.upsert('bot', {
    ...
    it('rejects malformed direct secret updates without writing', async () => {
        ...
        const invalidMaps = [
            ...
            ...['__proto__', 'constructor', 'prototype'].map((key) => Object.fromEntries([[key, { operation: 'preserve' }]])),
        ];
        for (const invalidMap of invalidMaps) {
            ...
            await expect(store.upsert('bot', options)).rejects.toMatchObject({ code: 'channel_settings_invalid_secret' });
            expect(fs.readFileSync(settingsPath, 'utf8')).toBe(before);
        }
    });
    it('rejects channel types without management descriptors', async () => {
        const store = new WorkspaceChannelSettingsStore(workspace);
        await expect(store.upsert('custom', {
            expectedRevision: store.snapshot().revision,
    ...
    "interactiveCards": {
      "questionCard": { "enabled": true, "timeoutMs": 0 }
        ...
        await expect(store.upsert('bot', {
            ...
            message: 'Channel field "interactiveCards.questionCard.timeoutMs" has an invalid value.',
        });
  ...
  "channels": { "bot": {
    ...
    it.each([
        { stored: null, changed: [] },
        { stored: [], changed: null },
    ])('preserves unchanged non-record groups but rejects a changed value', async ({ stored, changed }) => {
        writeWorkspaceSettings(JSON.stringify({
            $version: 4,
            channels: {
                bot: {
                    type: 'management-validation-test',
    ...
    it('rejects unchanged non-record groups containing an unsafe key', async () => {
        const groups = [JSON.parse('{"__proto__":{"polluted":true}}')];
        writeWorkspaceSettings(JSON.stringify({
            $version: 4,
            channels: {
                bot: {
      ...
      "*": { "requireMention": { "__proto__": { "legacy": true } } }
        ...
        await expect(store.upsert('bot', {
            ...
            config: {
                ...
                groups: {
                    '*': {
                        requireMention: JSON.parse('{"__proto__":{"legacy":true}}'),
                    },
                },
            },
            secrets: { clientSecret: { operation: 'preserve' } },
  ...
  "channels": { "bot": {
    ...
    "nested": { "requiredValue": "kept" }
        ...
        expect(readWorkspaceSettings()['channels']['bot']).not.toHaveProperty('nested');
    });
  ...
  "channels": { "bot": {
    ...
    "interactiveCards": {
      ...
      "questionCard": { "enabled": true, "timeoutMs": 270000 }
        ...
        expect(readWorkspaceSettings()['channels']['bot']?.['interactiveCards']).toEqual({ enabled: true, questionCard: { enabled: true } });
    });
  ...
  "channels": { "bot": {
    ...
    "clientSecret": "existing-secret"
        ...
        await expect(store.upsert('bot', {
            ...
            code: 'channel_settings_invalid_config',
            message: 'Channel field "interactiveCards.statusCard.enabled" does not support environment references.',
        });
    });
    ...
    "interactiveCards": {
      ...
      "questionCard": {
        ...
        await expect(store.upsert('bot', {
            ...
            code: 'channel_settings_invalid_config',
            message: 'Channel field "interactiveCards.questionCard.legacyFlag" is not manageable.',
        });
        expect(fs.readFileSync(settingsPath, 'utf8')).toBe(beforeRejectedWrite);
    });
    ...
    "interactiveCards": {
      "questionCard": {
        ...
        await expect(store.upsert('bot', {
            ...
            code: 'channel_settings_invalid_config',
            message: 'Channel field "interactiveCards.questionCard.constructor" is not manageable.',
        });
        expect(fs.readFileSync(settingsPath, 'utf8')).toBe(before);
    });
  ...
  "channels": { "bot": {
    ...
    ])('rejects a stored interactiveCards value with a $label', async ({ legacy }) => {
        writeWorkspaceSettings(`{
  "$version": 4,
  "channels": { "bot": {
    "type": "dingtalk",
    ...
    "interactiveCards": { "legacy": ${JSON.stringify(legacy)} }
        ...
        await expect(store.upsert('bot', {
            ...
            code: 'channel_settings_invalid_config',
            message: 'Channel field "interactiveCards.legacy" cannot use a reserved key.',
        });
        expect(fs.readFileSync(settingsPath, 'utf8')).toBe(before);
    });
    ...
    it.each([
        ...
        },
        {
            ...
            expectedMessage: 'Channel field "interactiveCards.enabled" has an invalid value.',
        },
        {
            label: 'invalid nested enabled flag',
            interactiveCards: { statusCard: { enabled: 'true' } },
            expectedMessage: 'Channel field "interactiveCards.statusCard.enabled" has an invalid value.',
        },
        {
            label: 'non-positive question timeout',
            interactiveCards: { questionCard: { timeoutMs: 0 } },
            expectedMessage: 'Channel field "interactiveCards.questionCard.timeoutMs" has an invalid value.',
        },
        {
            label: 'unknown nested field',
            interactiveCards: { unexpected: true },
            expectedMessage: 'Channel field "interactiveCards.unexpected" is not manageable.',
        },
    ])('rejects DingTalk interactive card configuration with $label', async ({ interactiveCards, expectedMessage }) => {
        writeWorkspaceSettings(`{
  "$version": 4,
  "channels": { "bot": {
    "type": "dingtalk",
  ...
  "channels": { "bot": {
    ...
    it('rejects a stale revision without writing', async () => {
        const store = new WorkspaceChannelSettingsStore(workspace);
        const before = fs.readFileSync(settingsPath, 'utf8');
        await expect(store.remove('bot', { expectedRevision: 'stale' })).rejects.toMatchObject({ code: 'channel_settings_conflict' });
        expect(fs.readFileSync(settingsPath, 'utf8')).toBe(before);
    });
  ...
  "channels": {
    ...
    "array": []
        ...
        expect(new WorkspaceChannelSettingsStore(workspace).snapshot().channels).toEqual({
            valid: { type: 'management-validation-test' },
        });
    });
    it('rejects unsafe channel names without writing', async () => {
        ...
        for (const name of ['__proto__', 'constructor', 'prototype']) {
            await expect(store.upsert(name, {
                expectedRevision: store.snapshot().revision,
                config: {
                    type: 'management-validation-test',
                    clientId: 'client-id',
            ...
            await expect(store.remove(name, { expectedRevision: store.snapshot().revision })).rejects.toMatchObject({ code: 'channel_settings_invalid_name' });
            expect(fs.readFileSync(settingsPath, 'utf8')).toBe(before);
        }
    });
    ...
    it('writes startup names separately while preserving settings and formatting', async () => {
        ...
        expect(fs.readFileSync(settingsPath, 'utf8')).toContain('// Keep this comment and unrelated setting.');
    ...
    it('rejects unsafe startup names without writing', async () => {
        ...
        const before = fs.readFileSync(settingsPath, 'utf8');
        for (const name of ['__proto__', 'constructor', 'prototype']) {
            await expect(store.setStartupNames([name], {
                expectedRevision: store.snapshot().revision,
            })).rejects.toMatchObject({ code: 'channel_settings_invalid_name' });
            expect(fs.readFileSync(settingsPath, 'utf8')).toBe(before);
        }
    ...
    it('rejects stale startup names without changing workspace settings', async () => {
        const store = new WorkspaceChannelSettingsStore(workspace);
        const before = fs.readFileSync(settingsPath, 'utf8');
        await expect(store.setStartupNames(['bot'], { expectedRevision: 'stale' })).rejects.toMatchObject({ code: 'channel_settings_conflict' });
        expect(fs.readFileSync(settingsPath, 'utf8')).toBe(before);
    });
...
//# sourceMappingURL=channel-settings-store.test.js.map