 ...
 */
export function singleTokenCredentials(token) {
    const expected = hashToken(token);
    return {
        isOpen: (listener) => expected === undefined && listener.kind !== 'local-control',
        verify: (candidate, listener) => {
            if (expected === undefined || listener.kind === 'local-control') {
                return false;
            }
            return equalDigest(createHash('sha256').update(candidate, 'utf8').digest(), expected);
        },
    };
}
