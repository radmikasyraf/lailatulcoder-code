...
const mockEnvHttpProxyAgent = vi.hoisted(() => vi.fn((opts) => ({
    proxyUrl: opts.httpProxy,
})));
const mockNormalizeProxyUrl = vi.hoisted(() => vi.fn((url) => url));
const mockStorageGetGlobalQwenDir = vi.hoisted(() => vi.fn(() => '/tmp/qwen-home'));
...
const mockNextFireTime = vi.hoisted(() => vi.fn((cron) => {
    if (cron === '0 0 31 2 *') {
...
const mockAcpBridge = vi.hoisted(() => vi.fn(() => ({
    on: mockBridgeOn,
    start: mockBridgeStart,
    stop: mockBridgeStop,
})));
const mockSanitizeLogText = vi.hoisted(() => vi.fn((text, maxLen) => String(text).slice(0, maxLen).replace(/\n/g, '\\n').replace(/\r/g, ' ')));
...
const mockSessionRouter = vi.hoisted(() => vi.fn(() => ({
...
import { BRIDGE_SESSION_RESTORE_TIMEOUT_MS, resolveExtensionChannelEntrySpecifier, resolveProxy, startCommand, } from './start.js';
const invokeStartHandler = async (args) => {
    const handler = startCommand.handler;
    if (!handler) {
        throw new Error('startCommand handler is missing');
    }
    await handler({ _: [], $0: 'qwen', ...args });
};
...
beforeEach(() => {
    ...
    mockNormalizeProxyUrl.mockImplementation((url) => url);
    mockNextFireTime.mockImplementation((cron) => {
        if (cron === '0 0 31 2 *') {
            throw new Error('No next fire time');
        }
        return new Date('2026-01-01T00:00:00.000Z');
...
describe('resolveProxy', () => {
    it('prefers the CLI proxy over settings and environment proxies', async () => {
        ...
        const proxy = await resolveProxy('http://cli.example.com:8080', 'http://settings.example.com:8080');
    ...
    it('prefers settings.proxy over environment proxies', async () => {
        process.env['HTTPS_PROXY'] = 'http://env.example.com:8080';
        const proxy = await resolveProxy(undefined, 'http://settings.example.com:8080');
...
describe('resolveExtensionChannelEntrySpecifier', () => {
    it('returns a file URL for extension channel entry paths', () => {
        ...
        expect(resolveExtensionChannelEntrySpecifier(extensionPath, entry)).toBe(pathToFileURL(join(extensionPath, entry)).href);
    });
});
describe('startCommand.handler', () => {
    it('refuses to start when channels are managed by qwen serve', async () => {
        ...
        try {
            await expect(invokeStartHandler({ name: 'telegram' })).rejects.toThrow('process.exit: 1');
        }
        finally {
            exitSpy.mockRestore();
        }
    ...
    it('loads settings.merged.proxy when no CLI proxy is provided', async () => {
        ...
        expect(mockCreateChannel).toHaveBeenCalledWith('telegram', mockParsedChannelConfig, expect.any(Object), expect.objectContaining({
            proxy: settingsProxy,
        ...
        const options = mockCreateChannel.mock.calls[0]?.[3];
        const input = {
    ...
    it('uses available env-var resolution for single-channel config', async () => {
        ...
        expect(mockParseChannelConfig).toHaveBeenCalledWith('telegram', channels.telegram, process.cwd(), { resolveEnvVars: 'available' });
    });
    ...
    it('cleans up a single channel when pidfile creation races', async () => {
        ...
        expect(mockWriteStderrLine).toHaveBeenCalledWith(expect.stringContaining('started concurrently'));
    });
    ...
    it('starts a standalone AcpBridge before creating the channel', async () => {
        ...
        expect(mockBridgeStart.mock.invocationCallOrder[0]).toBeLessThan(mockCreateChannel.mock.invocationCallOrder[0]);
        expect(mockSessionRouter).toHaveBeenCalledWith(bridge, mockParsedChannelConfig.cwd, mockParsedChannelConfig.sessionScope, expect.any(String));
        expect(mockCreateChannel).toHaveBeenCalledWith('telegram', mockParsedChannelConfig, bridge, expect.objectContaining({ router }));
    });
    it('removes router sessions when the bridge reports session death', async () => {
        ...
        const sessionDiedListener = mockBridgeOn.mock.calls.find(([eventName]) => eventName === 'sessionDied')?.[1];
        ...
        expect(mockWriteStderrLine).toHaveBeenCalledWith('[Channel] Session dead\\nsession died (boom\\nreason), updating routing state');
    ...
    it('dispatches bridge tool calls to the routed channel', async () => {
        ...
        const toolCallListener = mockBridgeOn.mock.calls.find(([eventName]) => eventName === 'toolCall')?.[1];
    ...
    it('registers session cleanup on the replacement bridge before restoring sessions', async () => {
        ...
        try {
            ...
            const disconnectedListener = mockBridgeOn.mock.calls.find(([eventName]) => eventName === 'disconnected')?.[1];
            ...
            const sessionDiedCalls = mockBridgeOn.mock.calls.filter(([eventName]) => eventName === 'sessionDied');
            expect(sessionDiedCalls).toHaveLength(2);
            const restartedSessionDiedListener = sessionDiedCalls[1][1];
            expect(mockBridgeOn.mock.invocationCallOrder.at(-2)).toBeLessThan(mockRouterRestoreSessions.mock.invocationCallOrder[0]);
            expect(mockChannelConnect.mock.invocationCallOrder[0]).toBeLessThan(mockRouterRestoreSessions.mock.invocationCallOrder[0]);
            restartedSessionDiedListener({ sessionId: 'dead-after-restart' });
            expect(mockRouterHandleSessionDied).toHaveBeenCalledWith('dead-after-restart');
        }
        finally {
    ...
    it('keeps the readiness gate blocked and coalesces replacement disconnects', async () => {
        ...
        try {
            ...
            await new Promise((resolve) => setImmediate(resolve));
            const options = mockCreateChannel.mock.calls[0]?.[3];
            const firstDisconnect = mockBridgeOn.mock.calls.find(([eventName]) => eventName === 'disconnected')?.[1];
            ...
            await vi.advanceTimersByTimeAsync(3000);
            await vi.waitFor(() => expect(mockRouterRestoreSessions).toHaveBeenCalledTimes(1));
            const disconnectListeners = mockBridgeOn.mock.calls.filter(([eventName]) => eventName === 'disconnected');
            expect(disconnectListeners).toHaveLength(2);
            const replacementDisconnect = disconnectListeners[1][1];
            replacementDisconnect();
            replacementDisconnect();
            resolveFirstRestore({ failed: 0, restored: 0 });
            await vi.advanceTimersByTimeAsync(3000);
            await vi.waitFor(() => expect(mockRouterRestoreSessions).toHaveBeenCalledTimes(2));
    ...
    it('stops the replacement bridge when session restore fails', async () => {
        mockChannelConnect.mockResolvedValue(undefined);
        mockRouterRestoreSessions.mockRejectedValueOnce(new Error('restore failed'));
        const channels = { telegram: { type: 'telegram' } };
        mockLoadSettings.mockReturnValue({ merged: { channels } });
        const processOnSpy = vi
            .spyOn(process, 'on')
    ...
    it('times out a wedged session restore and stops the replacement bridge', async () => {
        mockChannelConnect.mockResolvedValue(undefined);
        mockRouterRestoreSessions.mockImplementationOnce(() => new Promise(() => { }));
        const channels = { telegram: { type: 'telegram' } };
        mockLoadSettings.mockReturnValue({ merged: { channels } });
        const processOnSpy = vi
            .spyOn(process, 'on')
        ...
        try {
            ...
            await vi.waitFor(() => expect(mockRemoveServiceInfo).toHaveBeenCalled());
            expect(mockBridgeStop).toHaveBeenCalledOnce();
            expect(mockWriteStderrLine).toHaveBeenCalledWith(expect.stringContaining('Session restore timed out'));
            expect(exitSpy).toHaveBeenCalledWith(1);
        }
        finally {
            processOnSpy.mockRestore();
    ...
    it('starts all channels with one shared bridge and router', async () => {
        ...
        mockLoadSettings.mockReturnValue({ merged: { channels } });
        mockParseChannelConfig.mockImplementation(async (name) => ({
        ...
        expect(mockSessionRouter).toHaveBeenCalledWith(bridge, process.cwd(), 'user', expect.any(String));
        ...
        expect(mockCreateChannel).toHaveBeenNthCalledWith(1, 'first', expect.objectContaining({ cwd: '/tmp/first' }), bridge, expect.objectContaining({ router }));
        expect(mockCreateChannel).toHaveBeenNthCalledWith(2, 'second', expect.objectContaining({ cwd: '/tmp/second' }), bridge, expect.objectContaining({ router }));
    });
    ...
    it('passes channel memory callbacks when starting all channels', async () => {
        ...
        expect(mockCreateChannel).toHaveBeenCalledTimes(2);
        expect(mockCreateChannel).toHaveBeenNthCalledWith(1, 'discord', mockParsedChannelConfig, expect.any(Object), expect.objectContaining({
            channelMemory: {
                readChannelMemory: mockReadChannelMemory,
                getChannelMemoryRevision: mockGetChannelMemoryRevision,
                listChannelMemoryEntries: mockListChannelMemoryEntries,
        ...
        expect(mockCreateChannel).toHaveBeenNthCalledWith(2, 'telegram', mockParsedChannelConfig, expect.any(Object), expect.objectContaining({
            channelMemory: {
                readChannelMemory: mockReadChannelMemory,
                getChannelMemoryRevision: mockGetChannelMemoryRevision,
                listChannelMemoryEntries: mockListChannelMemoryEntries,
    ...
    it('starts the scheduler with connected channels only', async () => {
        ...
        const processOnSpy = vi
            ...
            .mockImplementation(() => process);
        try {
            await expect(invokeStartHandler({})).rejects.toThrow('stop after scheduler setup');
        }
        finally {
            processOnSpy.mockRestore();
        }
    ...
    it('restarts all channels on shared bridge crash before restoring sessions', async () => {
        ...
        try {
            ...
            expect(mockBridgeOn.mock.calls.filter(([eventName]) => eventName === 'toolCall')).toHaveLength(2);
            expect(mockBridgeOn.mock.calls.filter(([eventName]) => eventName === 'sessionDied')).toHaveLength(2);
            expect(mockRouterRestoreSessions).toHaveBeenCalledTimes(1);
        }
        finally {
            processOnSpy.mockRestore();
