...
const SPOOFING_FORMAT_CODE_UNITS = (code) => code === 0x61c ||
    code === 0x200b ||
    code === 0x200e ||
    code === 0x200f ||
    ...
    code === 0xfeff;
export function isControlFree(value) {
...
export function isSafeRepositoryRelativePath(path) {
 ...
 */
export function validateBoundedString(value, field, maxLength, prefix, pattern) {
...
export function validateBoundedStringArray(value, field, maxLength, prefix, pattern) {
...
export function validateRepositoryContext(value) {
    if (typeof value !== 'object' || value === null || Array.isArray(value)) {
        throw new Error('repositoryContext must be an object');
    }
    const context = value;
    const keys = Object.keys(context).sort();
    if (keys.length !== CONTEXT_KEYS.length ||
    ...
    const requireSortedUnique = (values, field) => {
        if (!isSortedUnique(values)) {
            throw new Error(`${prefix}${field} must be sorted and unique`);
        }
    };
    validateBoundedString(context['provider'], 'provider', MAX_PROVIDER_LENGTH, prefix, SAFE_PROVIDER);
    ...
    validateBoundedStringArray(context['relatedPaths'], 'relatedPaths', MAX_PATH_LENGTH, prefix);
    requireSortedUnique(context['relatedPaths'], 'relatedPaths');
    if (context['relatedPaths'].some((path) => !isSafeRepositoryRelativePath(path))) {
    ...
    validateBoundedStringArray(context['requiredAgents'], 'requiredAgents', MAX_TOKEN_LENGTH, prefix);
    requireSortedUnique(context['requiredAgents'], 'requiredAgents');
    if (context['requiredAgents'].some((role) => !isRepositoryContextRoleId(role))) {
        throw new Error('repositoryContext.requiredAgents contains an unsupported role');
    }
    for (const field of ['unverifiedDimensions', 'verificationNotes']) {
        validateBoundedStringArray(context[field], field, MAX_NOTE_LENGTH, prefix);
        requireSortedUnique(context[field], field);
    }
    return context;
}
export function repositoryContextOf(plan) {
    if (plan.repositoryContext === undefined)
        return null;
