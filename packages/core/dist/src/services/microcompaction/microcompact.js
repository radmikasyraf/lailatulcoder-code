...
const CLEARED_MEDIA_PLACEHOLDER_RE = new RegExp(`^${MICROCOMPACT_CLEARED_IMAGE_PREFIX.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')} [^\\]\\r\\n\\t]*\\]$`);
export function isClearedMediaPlaceholder(text) {
 ...
 */
function collectCompactablePartRefs(history, preserveReadFileResult) {
    const tool = [];
    const media = [];
    const nestedMedia = [];
    ...
    const preservedRefs = buildPreservedReadRefs(history, tool, preserveReadFileResult);
    return {
...
function stripNestedMedia(fnResp) {
    ...
    // a lailatul-coder extension — see `coreToolScheduler.createFunctionResponsePart`.
    const { parts: _droppedNested, ...rest } = fnResp;
    return rest;
}
function getPart(history, ref) {
    return history[ref.contentIndex]?.parts?.[ref.partIndex];
}
function getToolOutputChars(part) {
    if (!part ||
...
function normalizePendingContent(pendingContent) {
    if (!pendingContent)
        return [];
    return Array.isArray(pendingContent) ? pendingContent : [pendingContent];
}
function getToolResultsTotalCharsThreshold(settings) {
...
function buildKeepRefs(refs, keepRecent) {
    return new Set(refs.slice(-keepRecent).map(refKey));
}
function buildClearMap(clearRefs) {
...
function getFilePathsForResponse(part, callIdToFilePath) {
...
function buildPreservedReadRefs(history, refs, preserveReadFileResult) {
...
function buildKeptFilePaths(history, refs, keepRefs, callIdToFilePath) {
    const kept = new Set();
    for (const ref of refs) {
        if (!keepRefs.has(refKey(ref)))
            continue;
        const part = getPart(history, ref);
        if (!part || isErrorResponse(part) || isAlreadyCleared(part))
            continue;
...
function planSizeBasedClearing(history, settings, keepRecent, pendingContent, preserveReadFileResult) {
    ...
    const virtualHistory = pending.length > 0 ? [...history, ...pending] : history;
    const { tool } = collectCompactablePartRefs(virtualHistory);
    const charsByRef = new Map();
    let totalChars = 0;
    let pendingChars = 0;
    ...
    const preservedToolRefs = preserveReadFileResult
        ...
        : new Set();
    const compactableToolRefs = tool.filter((ref) => !preservedToolRefs.has(refKey(ref)));
    ...
    const keepToolRefs = buildKeepRefs(compactableToolRefs.filter((ref) => ref.contentIndex < history.length && charsByRef.has(refKey(ref))), keepRecent);
    const clearRefs = [];
    let remainingChars = totalChars;
    for (const ref of compactableToolRefs) {
        if (remainingChars <= lowWatermark)
            break;
 ...
 */
export function microcompactHistory(history, lastApiCompletionTimestamp, settings, opts) {
    const keepRecent = resolveKeepRecent(process.env['QWEN_MC_KEEP_RECENT'], settings.toolResultsNumToKeep);
    let triggerReason;
    let gapMs = 0;
    let tool = [];
    let media = [];
    ...
    let clearRefs = [];
    let toolResultCharsBefore;
    let toolResultCharsAfter;
    let pendingToolResultChars;
    let toolResultsTotalCharsThreshold;
    let toolResultsLowWatermark;
    ...
    if (opts?.force) {
        ...
        const timeTrigger = evaluateTimeBasedTrigger(lastApiCompletionTimestamp, settings);
        if (timeTrigger) {
            triggerReason = 'idle';
    ...
    if (triggerReason === 'force' || triggerReason === 'idle') {
        ({ tool, media, nestedMedia } = collectCompactablePartRefs(history, opts?.preserveReadFileResult));
        ...
        const keepToolRefs = buildKeepRefs(tool.filter((ref) => {
            const part = getPart(history, ref);
        ...
        const sizePlan = planSizeBasedClearing(history, settings, keepRecent, pending, opts?.preserveReadFileResult);
        if (!sizePlan) {
            return { history };
        }
        triggerReason = 'size';
    ...
    if (clearRefs.length > 0) {
        ...
        const keptFilePaths = buildKeptFilePaths(keptPathHistory, keptPathRefs, keepRefs, callIdToFilePath);
    ...
    const mediaKept = triggerReason === 'size'
        ? 0
...
function resolveKeepRecent(envValue, settingsValue) {
