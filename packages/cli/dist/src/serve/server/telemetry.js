...
import { emitDaemonLog, extractDaemonHttpTraceContext, extractInboundTraceId, hashDaemonWorkspace, isTelemetrySdkInitialized, recordDaemonError, recordDaemonHttpRequest, recordDaemonHttpResponse, withDaemonRequestSpan, } from '@lailatul-coder/lailatul-coder-core';
import { sanitizeLogText } from '@lailatul-coder/channel-base';
import { CLIENT_ID_HEADER, CLIENT_ID_RE, getDeferredRuntimeRequestTiming, MAX_CLIENT_ID_LENGTH, } from './request-helpers.js';
import { daemonInboundTraceIdContext, daemonTelemetryResponseContext, } from './telemetry-context.js';
...
function matchLegacySessionTelemetryRoute(method, requestPath) {
    ...
    for (const entry of legacySessionTelemetryRoutes) {
        ...
        const requestId = rawRequestId !== undefined ? decodePathSegment(rawRequestId) : undefined;
        return {
            route: entry.route,
...
export function setDaemonTelemetryWorkspace(res, workspaceCwd) {
    try {
...
export function resolveDaemonTelemetryRoute(req) {
    ...
    const workspaceTranscript = path.match(/^\/workspaces\/[^/]+\/session\/([^/]+)\/transcript$/);
    ...
    const workspaceExport = path.match(/^\/workspaces\/[^/]+\/session\/([^/]+)\/export$/);
    ...
    const workspaceArchivedExport = path.match(/^\/workspaces\/[^/]+\/session\/([^/]+)\/archive\/export$/);
    ...
    if (pluralWorkspacePrefix.test(path)) {
        ...
        if (req.method === 'GET') {
            ...
            if (suffix === '/workspace/file')
                return { route: 'GET /file' };
            if (suffix === '/workspace/file/bytes') {
                return { route: 'GET /file/bytes' };
            }
            if (suffix === '/workspace/stat')
                return { route: 'GET /stat' };
            if (suffix === '/workspace/list')
                return { route: 'GET /list' };
            if (suffix === '/workspace/glob')
                return { route: 'GET /glob' };
        ...
        if (req.method === 'POST') {
            ...
            if (/^\/workspace\/mcp\/[^/]+\/(approve|enable|disable|authenticate|clear-auth)$/.test(suffix)) {
                return {
        ...
        if (/^\/workspace\/session-groups\/[^/]+$/.test(suffix) &&
            req.method === 'PATCH') {
    ...
    const deviceFlowDelete = path.match(/^\/workspace\/auth\/device-flow\/([^/]+)$/);
    ...
    if (path === '/workspace/settings') {
        if (req.method === 'GET')
            return { route: 'GET /workspace/settings' };
        if (req.method === 'POST')
            return { route: 'POST /workspace/settings' };
    }
    if (path === '/workspace/permissions') {
        if (req.method === 'GET')
            return { route: 'GET /workspace/permissions' };
        if (req.method === 'POST')
            return { route: 'POST /workspace/permissions' };
    }
    if (path === '/workspace/trust') {
        if (req.method === 'GET')
            return { route: 'GET /workspace/trust' };
    }
    ...
    if (path === '/workspace/voice') {
        if (req.method === 'GET')
            return { route: 'GET /workspace/voice' };
        if (req.method === 'POST')
            return { route: 'POST /workspace/voice' };
    }
 ...
 */
export function daemonInboundTraceIdCaptureMiddleware(req, res, next) {
    try {
        const inboundTraceId = extractInboundTraceId(req.headers);
        if (inboundTraceId !== undefined) {
            res[daemonInboundTraceIdContext] =
                inboundTraceId;
        }
    }
    catch {
...
export function daemonTelemetryMiddleware(resolveWorkspaceCwd, 
...
recordRequest) {
    const workspaceHashByCwd = new Map();
        ...
        if (isTelemetrySdkInitialized()) {
            ...
            try {
                ...
                if (!parentContext &&
                    ...
                    emitDaemonLog('Rejected invalid inbound traceparent header.', {
                        'http.route': route.route,
                        'http.request.header.traceparent': sanitizeLogText(inboundTraceparent, 128),
                    }, {
            ...
            const finish = () => {
                ...
                try {
                    ...
                    if (context?.workspaceCwd !== undefined) {
                        span?.setAttribute('lailatul-coder.workspace.hash', resolveWorkspaceHash(context.workspaceCwd));
                    }
                }
                catch {
                ...
                const successfulSse = route.route === 'GET /session/:id/events' &&
                    res.statusCode === 200 &&
                    res.headersSent;
                if (!successfulSse) {
                    recordDaemonHttpRequest(durationMs, route.route, res.statusCode, deferredRuntime?.path);
                }
