...
import { createConversationRuntimeOwnership, getConversationRuntimeOwnerPath, } from './conversation-runtime-ownership.js';
import { ConversationRuntimeOwnershipError } from './conversation-runtime-errors.js';
import { getLiveDiscoveryPath, writeLiveDiscoveryFile, } from '../live/discovery.js';
    ...
    return {
        ...
        open: vi.fn(async (...args) => {
            ...
            if (matches &&
                ...
                recordReadFailure.injected = true;
                vi.spyOn(handle, 'readFile').mockRejectedValueOnce(Object.assign(new Error(recordReadFailure.code), {
                    code: recordReadFailure.code,
                }));
            }
            return handle;
...
async function temporaryStableBase() {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), 'qwen-conversation-owner-'));
    temporaryDirectories.push(root);
    return path.join(root, '.qwen');
}
async function readRecord(stableBaseDir) {
    return JSON.parse(await fs.readFile(getConversationRuntimeOwnerPath(stableBaseDir), 'utf8'));
}
function failRecordReadOnce(recordPath, operation, code) {
...
async function writeForeignRecord(stableBaseDir, kind) {
    if (kind === 'ownership') {
...
function waitForWorkerMessage(child) {
    return new Promise((resolve, reject) => {
        ...
        const onExit = (code, signal) => {
            cleanup();
            reject(new Error(`Ownership worker exited before reporting (code=${String(code)}, signal=${String(signal)}).`));
        };
...
describe('Conversation runtime ownership', () => {
    it('arbitrates real child processes and reclaims a crashed owner after grace', async () => {
        ...
        const workerPath = path.join(path.dirname(stableBaseDir), 'owner-worker.mjs');
        const moduleUrl = pathToFileURL(path.resolve('src/serve/conversations/conversation-runtime-ownership.ts')).href;
        await fs.writeFile(workerPath, `import { createConversationRuntimeOwnership } from ${JSON.stringify(moduleUrl)};
...
`, { mode: 0o600 });
        ...
        const contender = spawnWorker('conversation_owner_child_contender_01', 'once');
        ...
        const successor = spawnWorker('conversation_owner_child_successor_01', 'once');
    ...
    it('creates one exact private record and makes acquire/release idempotent', async () => {
        ...
        if (process.platform !== 'win32') {
            const directory = path.dirname(getConversationRuntimeOwnerPath(stableBaseDir));
            expect((await fs.stat(directory)).mode & 0o777).toBe(0o700);
            expect((await fs.stat(getConversationRuntimeOwnerPath(stableBaseDir))).mode &
                0o777).toBe(0o600);
        }
        await expect(ownership.release()).resolves.toBe(true);
        await expect(ownership.release()).resolves.toBe(false);
        await expect(fs.stat(getConversationRuntimeOwnerPath(stableBaseDir))).rejects.toMatchObject({ code: 'ENOENT' });
    });
    ...
    it('fails closed for an active foreign owner and permits a later retry', async () => {
        ...
        expect((await readRecord(stableBaseDir)).instanceNonce).toBe('conversation_owner_nonce_previous_01');
    ...
    it('treats the same pid with a different nonce as active PID reuse', async () => {
        ...
        expect((await readRecord(stableBaseDir)).instanceNonce).toBe('conversation_owner_nonce_previous_03');
    });
    ...
    it.each([
    ...
    ])('recovers after a transient %s record %s failure', async (kind, operation) => {
        const stableBaseDir = await temporaryStableBase();
        ...
        failRecordReadOnce(recordPath, operation, operation === 'open' ? 'EMFILE' : 'EIO');
        await expect(ownership.acquire()).rejects.toMatchObject({
    ...
    it.each(['ownership', 'Live discovery'])('treats an ELOOP opening the %s record as terminal compromise', async (kind) => {
        const stableBaseDir = await temporaryStableBase();
        const recordPath = await writeForeignRecord(stableBaseDir, kind);
        const ownership = createConversationRuntimeOwnership({
            stableBaseDir,
    ...
    it('commits before removing a dead legacy Live owner and waits one grace', async () => {
        ...
        await expect(fs.stat(getLiveDiscoveryPath(stableBaseDir))).rejects.toMatchObject({ code: 'ENOENT' });
        expect(wait).toHaveBeenCalledOnce();
    ...
    it('never overwrites malformed or unknown-version state', async () => {
        ...
        await expect(ownership.acquire()).rejects.toBeInstanceOf(ConversationRuntimeOwnershipError);
        await expect(fs.readFile(ownerPath, 'utf8')).resolves.toBe('{"version":2}');
        await expect(ownership.release()).resolves.toBe(false);
    });
    it.each(['symlink', 'hardlink', 'oversized', 'mode'])('rejects an unsafe %s owner file without replacing it', async (kind) => {
        if (process.platform === 'win32' &&
        ...
        const targetPath = path.join(path.dirname(stableBaseDir), `owner-${kind}`);
        ...
        if (kind === 'oversized')
            expect(stat.size).toBe(4 * 1024 + 1);
        if (kind === 'mode')
    ...
    it('does not recreate a missing ownership directory after acquire', async () => {
        const stableBaseDir = await temporaryStableBase();
        const ownerDirectory = path.dirname(getConversationRuntimeOwnerPath(stableBaseDir));
        const ownership = createConversationRuntimeOwnership({
            stableBaseDir,
            pid: process.pid,
            instanceNonce: currentNonce,
    ...
    it.skipIf(process.platform === 'win32' || process.getuid?.() === 0)('treats transient directory inspection failures as retryable', async () => {
        const stableBaseDir = await temporaryStableBase();
    ...
    it('rejects an ownership directory below a symlinked stable base', async () => {
        ...
        await expect(fs.readdir(path.join(target, 'conversations'))).resolves.toEqual([]);
    });
});
//# sourceMappingURL=conversation-runtime-ownership.test.js.map