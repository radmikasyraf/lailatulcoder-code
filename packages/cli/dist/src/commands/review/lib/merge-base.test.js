...
describe('resolveMergeBase', () => {
    ...
    it('falls back to the local base branch when there is no tracking ref', () => {
        ...
        expect(resolveMergeBase('origin', 'main', 'pr-head', git).sha).toBe('bbb222');
    });
    it('falls through when the tracking ref exists but has no merge-base', () => {
        ...
        expect(resolveMergeBase('origin', 'main', 'pr-head', git).sha).toBe('ccc333');
    });
    ...
    it('propagates a mergeBase throw — a surface failure is not "none"', () => {
        ...
        expect(() => resolveMergeBase('origin', 'main', 'pr-head', git)).toThrow('surface unavailable');
    });
    it('never merge-bases through an origin/<name> shadow tag', () => {
        ...
        expect(resolveMergeBase('origin', 'main', 'pr-head', git).sha).toBe('true-base');
        expect(git.calls).not.toContain('mergeBase origin/main pr-head');
    });
});
//# sourceMappingURL=merge-base.test.js.map