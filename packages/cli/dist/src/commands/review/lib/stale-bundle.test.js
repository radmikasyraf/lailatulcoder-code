...
import { bundleStaleness, bundleStalenessNotices, DIGEST_FILE, reviewSourceRoots, reviewSourcesDigest, staleBundleWarning, } from './stale-bundle.js';
...
const code = (path) => ({ path, kind: 'code' });
const skill = (path) => ({ path, kind: 'skill' });
describe('reviewSourcesDigest', () => {
    ...
    it('does not depend on the order the roots are passed in', () => {
        ...
        writeFileSync(a, 'x');
        writeFileSync(b, 'y');
        expect(reviewSourcesDigest(root, [code(a), code(b)])).toBe(reviewSourcesDigest(root, [code(b), code(a)]));
    });
    ...
    it('yields nothing when there are no sources to hash', () => {
        expect(reviewSourcesDigest(root, [code(join(root, 'nope'))])).toBeUndefined();
    });
...
describe('bundleStaleness', () => {
    ...
    ])('says it could not measure with %s, and does not accuse the build', (_n, stamped, current, reason) => {
...
describe('the bundled skill stops on what this module prints', () => {
    it('the SKILL.md quotes and the live notices are prefixes of each other', () => {
        const skillPath = join(repoRoot, 'packages', 'core', 'src', 'skills', 'bundled', 'review', 'SKILL.md');
        const quotes = [
        ...
        try {
            ...
            const skillDir = join(root, 'packages', 'core', 'src', 'skills', 'bundled', 'review');
            mkdirSync(skillDir, { recursive: true });
            writeFileSync(join(skillDir, 'SKILL.md'), '# skill');
            ...
            writeFileSync(join(distDir, DIGEST_FILE), reviewSourcesDigest(root, reviewSourceRoots(root)));
            ...
            for (const quote of quotes) {
                expect(lines.some((line) => line.startsWith(quote)), `SKILL.md quotes \`${quote}\`, but no notice this module prints begins with it`).toBe(true);
            }
            ...
            for (const line of lines) {
                expect(quotes.some((quote) => line.startsWith(quote)), `this module prints \`${line}\`, but SKILL.md quotes no prefix of it`).toBe(true);
            }
        }
        finally {
...
describe('reviewSourceRoots', () => {
    it('covers the directory, the registration file beside it, the lease and lifted helpers outside it, and the skill', () => {
        ...
        expect(reviewSourceRoots('/w')).toEqual([
            ...
            // see.
            {
                path: join('/w', 'packages', 'cli', 'src', 'services', 'review-worktree-lease.ts'),
                kind: 'code',
            },
            ...
            {
                path: join('/w', 'packages', 'core', 'src', 'skills', 'bundled', 'review'),
                kind: 'skill',
            },
        ]);
    });
});
//# sourceMappingURL=stale-bundle.test.js.map