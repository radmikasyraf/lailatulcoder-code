...
function editFile(path, ctx, add) {
    return [
...
describe('buildPlanReport', () => {
    ...
    it('emits addedRanges only on heavy files', () => {
        const diff = editFile('src/heavy.ts', 3, 900) + makeDiff('src/light.ts', 20);
        const report = buildPlanReport(buildDiffPlan(diff, 400), (p) => (p === 'src/heavy.ts' ? 6000 : 30), {});
...
describe('stringifyPlanReport', () => {
    ...
    it('keeps every range on one line, and stays pageable', () => {
        const report = buildPlanReport(buildDiffPlan(editFile('src/heavy.ts', 3, 900), 400), () => 6000, {});
    ...
    it('never mistakes a path for a range', () => {
        ...
        const parsed = JSON.parse(stringifyPlanReport(report));
