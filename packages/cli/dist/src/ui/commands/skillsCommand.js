...
import { normalizeSkillPriority } from '@lailatul-coder/lailatul-coder-core';
...
export const skillsCommand = {
    ...
    action: async (context) => {
        ...
        const disabled = context.services.config?.getDisabledSkillNames() ?? new Set();
        const userInvocableSkills = skills.filter((skill) => skill.userInvocable !== false);
        const visibleSkills = userInvocableSkills.filter((s) => !disabled.has(s.name.toLowerCase()));
        if (visibleSkills.length === 0) {
            const content = skills.length > 0 && userInvocableSkills.length === 0
                ? t('All skills are marked as non-user-invocable.')
                : userInvocableSkills.length === 0
                    ? t('No skills are currently available.')
                    : t('All available skills are disabled. Edit ~/.lailatulcoder/settings.json or .lailatulcoder/settings.json (skills.disabled) to re-enable.');
            return {
                type: 'message',
                messageType: 'info',
                content,
        ...
        const sortedSkills = [...visibleSkills].sort((a, b) => normalizeSkillPriority(b.priority) -
            normalizeSkillPriority(a.priority) || a.name.localeCompare(b.name));
        const sanitize = (text, max) => {
            const oneLine = text.replace(/[\r\n]+/g, ' ').trim();
            return oneLine.length <= max
                ? oneLine
                : `${oneLine.slice(0, Math.max(0, max - 1))}…`;
        };
        const lines = sortedSkills.map((s) => `  - ${s.name}${s.description ? `  ${sanitize(s.description, 80)}` : ''}` +
