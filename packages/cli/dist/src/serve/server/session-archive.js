...
export async function deleteDaemonSessions(params) {
    const { sessionIds, service, bridge, coordinator, coordinatorLockHeld = false, onError, } = params;
    const uniqueSessionIds = [
        ...new Set(sessionIds.map(normalizeSessionIdForLookup)),
    ];
...
function logSessionArchiveResult(action, result) {
    ...
    writeStderrLine(`LailatulCoder serve: sessions ${action} result ${details}`);
}
...
export function logSessionArchiveWarning(message) {
    writeStderrLine(`LailatulCoder serve: ${sanitizeLogLine(message)}`);
}
...
export async function archiveDaemonSessions(params) {
    const { sessionIds, service, bridge, coordinator, coordinatorLockHeld = false, } = params;
    const uniqueSessionIds = [
        ...new Set(sessionIds.map(normalizeSessionIdForLookup)),
    ];
    if (!coordinatorLockHeld) {
...
export async function unarchiveDaemonSessions(params) {
    const { sessionIds, service, coordinator, coordinatorLockHeld = false, } = params;
    const uniqueSessionIds = [
        ...new Set(sessionIds.map(normalizeSessionIdForLookup)),
    ];
    if (!coordinatorLockHeld) {
