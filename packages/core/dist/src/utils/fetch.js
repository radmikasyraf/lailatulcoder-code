...
export function isConnectionLevelError(error) {
...
export async function fetchWithPolicy(url, options) {
    ...
    const abortError = () => {
        ...
        if (timeoutSignal.aborted) {
            return new FetchError(`Request timed out after ${options.timeoutMs}ms`, 'ETIMEDOUT');
        }
        return undefined;
    };
    let first;
...
async function fetchPolicyAttempt(url, options, signal, timeoutSignal) {
    const wrapAbort = (error) => {
        ...
        if (timeoutSignal.aborted) {
            throw new FetchError(`Request timed out after ${options.timeoutMs}ms`, 'ETIMEDOUT');
        }
        if (error instanceof FetchError)
            throw error;
    ...
    for (let hop = 0; hop <= options.maxRedirects; hop++) {
        ...
        if (REDIRECT_STATUSES.has(response.status)) {
            ...
            try {
                ...
                throw new FetchError(`Redirect response has a malformed Location header: ${location}`);
            }
        ...
        if (Number.isFinite(contentLength) && contentLength > options.maxBytes) {
            await response.body?.cancel().catch(() => { });
            throw new FetchError(`Response too large: ${contentLength} bytes exceeds the ${options.maxBytes}-byte limit`, 'EMSGSIZE');
        }
            ...
            try {
                for (;;) {
                    ...
                    if (total > options.maxBytes) {
                        throw new FetchError(`Response too large: exceeded the ${options.maxBytes}-byte limit while streaming`, 'EMSGSIZE');
                    }
                    chunks.push(value);
                }
            }
    ...
    throw new FetchError(`Too many redirects (exceeded ${options.maxRedirects})`, 'EMAXREDIRECTS');
}
