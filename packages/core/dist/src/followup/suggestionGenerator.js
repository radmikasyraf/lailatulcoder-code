 ...
 */
import { getCacheSafeParams, getCacheSafeParamsSessionId, runForkedAgent, } from '../utils/forkedAgent.js';
...
const SUGGESTION_SCHEMA = {
    ...
    properties: {
        suggestion: {
            type: 'string',
            description: 'The predicted next user input (2-12 words), or empty string if nothing obvious.',
        },
    },
    required: ['suggestion'],
};
 ...
 */
export async function generatePromptSuggestion(config, conversationHistory, abortSignal, options) {
    // Don't suggest in very early conversations
    const modelTurns = conversationHistory.filter((c) => c.role === 'model').length;
    ...
    try {
        ...
        const cacheSafeSessionId = options?.enableCacheSharing
            ...
            : undefined;
        const cacheSafe = cacheSafeSessionId === sessionId ? getCacheSafeParams() : null;
        ...
        const sessionCacheSafe = cacheSafe && cacheSafe.sessionId === config.getSessionId()
            ? cacheSafe
            : null;
        ...
        const cacheSharingState = sessionCacheSafe
            ...
            : cacheSafeSessionId
                ? 'session_mismatch'
                : 'false';
        debugLogger.debug(`Generating suggestion: cacheSharing=${cacheSharingState}, model=${modelOverride || '(default)'}`);
        const raw = sessionCacheSafe
            ? await generateViaForkedQuery(config, sessionCacheSafe, abortSignal, modelOverride)
            : await generateViaBaseLlm(config, conversationHistory, abortSignal, modelOverride);
        ...
        if (filterReason) {
            debugLogger.debug(`Suggestion filtered: reason=${filterReason}, text="${suggestion}"`);
...
async function generateViaForkedQuery(config, cacheSafeParams, abortSignal, modelOverride) {
...
async function generateViaBaseLlm(config, conversationHistory, abortSignal, modelOverride) {
...
function hasSentenceBoundary(suggestion) {
 ...
 */
export function getFilterReason(suggestion) {
    ...
    const hasCJK = /[\u4e00-\u9fff\u3040-\u309f\u30a0-\u30ff\uac00-\ud7af]/.test(suggestion);
    if (!hasCJK) {
        if (wordCount < 2) {
    ...
    if (suggestion.length >= 100)
        return 'too_long';
    if (hasSentenceBoundary(suggestion))
        return 'multiple_sentences';
    if (/[\n*]|\*\*/.test(suggestion))
        return 'has_formatting';
    if (/\bthanks\b|\bthank you\b|\blooks good\b|\bsounds good\b|\bthat works\b|\bthat worked\b|\bthat's all\b|\bnice\b|\bgreat\b|\bperfect\b|\bmakes sense\b|\bawesome\b|\bexcellent\b/.test(lower)) {
        return 'evaluative';
    }
