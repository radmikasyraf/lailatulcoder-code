 ...
 */
import { createDebugLogger, detectRuntime, getAllProviderBaseUrls, getOrCreateSharedDispatcher, preloadRuntimeFetchModule, redactProxyCredentials, } from '@lailatul-coder/lailatul-coder-core';
import { loadUndici } from './load-undici.js';
const debugLogger = createDebugLogger('PRECONNECT');
let preconnectFired = false;
let preconnectInFlight;
/**
 * Default API base URLs by AuthType.
 */
const DEFAULT_BASE_URLS = {
    openai: 'https://api.openai.com',
    'LailatulCoder-oauth': 'https://coding.dashscope.aliyuncs.com',
 ...
 */
function getPreconnectTargetUrl(authType, resolvedBaseUrl) {
    ...
    if (resolvedBaseUrl && /^https?:\/\//i.test(resolvedBaseUrl)) {
        ...
        debugLogger.debug('Skipping preconnect: resolved baseUrl is not a default URL');
        return undefined;
    }
 ...
 *
 * @param authType - Authentication type (openai, LailatulCoder-oauth, anthropic, etc.)
 * @param options - Configuration options
 */
export function preconnectApi(authType, options = {}) {
    if (preconnectFired) {
        return;
    }
    // Check if disabled
 ...
 * @internal
 */
export function resetPreconnectState() {
 ...
 * @internal
 */
export function waitForPreconnect() {
    return preconnectInFlight ?? Promise.resolve();
}
//# sourceMappingURL=apiPreconnect.js.map