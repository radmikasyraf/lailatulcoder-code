...
async function createConversationWorkspace() {
    const home = await mkdtemp(join(realpathSync.native(tmpdir()), 'qwen-live-worker-'));
...
function createBridge(options) {
    ...
    const bridge = {
        ...
        changeSessionCwd: async (sessionId, request) => {
            operations.push(`cwd:${sessionId}`);
            cwdChanges.push({ sessionId, ...request });
            if (options.failCwdChange)
                throw new Error('cwd relocation failed');
            return {
                sessionId,
        ...
        sendPrompt: (sessionId, _request, _signal, context) => {
...
describe('Live worker workspace isolation', () => {
    it('relocates each worker before its first prompt and keeps deletion recoverable', async () => {
        ...
        const launcher = createSubSessionLauncher({
            ...
            boundWorkspace: root,
            isolatedWorkspace: {
                materializeDirectory: (sessionId) => workspace.materializeConversationDirectory(sessionId),
                discardEmptyDirectory: (sessionId) => workspace.discardEmptyConversationDirectory(sessionId),
            },
        });
    ...
    it.each([
    ...
    ])('only discards a failed worker directory after a confirmed kill (attached=$attached, killed=$killSessionResult)', async ({ attached, killSessionResult, expectedOperation, directoryRetained, }) => {
        const { workspace, root } = await createConversationWorkspace();
        ...
        if (directoryRetained) {
            expect((await lstat(fake.cwdChanges[0].path)).isDirectory()).toBe(true);
        }
        else {
...
//# sourceMappingURL=live-worker-workspace.test.js.map