 ...
 */
import { isActiveWorkflowStatus, isTerminalWorkflowStatus, listWorkflowSnapshots, } from '@lailatul-coder/lailatul-coder-core';
 ...
 */
export function snapshotToTask(s) {
    return {
        id: s.runId,
        kind: 'workflow',
        runId: s.runId,
...
function rowLine(entry, now) {
    ...
    const counts = entry.agentsDispatched > 0
        ...
        : '';
    const phaseCount = entry.phases.length > 0
    ...
    const budgetChip = entry.tokensSpent > 0 || entry.tokenBudgetTotal !== null
        ...
        : '';
    const errorTail = entry.status === 'failed' && entry.error
...
function detailLines(entry, now) {
    ...
    lines.push(`  agents      : ${entry.agentsCompleted}/${entry.agentsDispatched}`);
    ...
    lines.push(`  cap         : ${entry.tokenBudgetTotal !== null ? formatTokenCount(entry.tokenBudgetTotal) : '(no cap)'}`);
    if (entry.error) {
    ...
    if (entry.phases.length > 0) {
        ...
        for (const phase of entry.phases) {
            ...
            const chip = phaseTokens > 0 ? ` · ${formatTokenCount(phaseTokens)}t` : '';
    ...
    action: async (context, args) => {
        ...
        if (tokens[0] === 'p') {
            ...
            if (!target) {
                ...
                const snapshot = (await listWorkflowSnapshots(config)).find((s) => s.runId === runId);
                if (snapshot) {
        ...
        if (trimmedArgs.length > 0) {
            ...
            if (!target) {
                ...
                const snapshot = (await listWorkflowSnapshots(config)).find((s) => s.runId === trimmedArgs);
                if (snapshot)
                    target = snapshotToTask(snapshot);
            }
            if (!target) {
        ...
        const lines = [];
        if (context.executionMode === 'interactive') {
            lines.push(t('Tip: use `/workflows p <runId>` or Background tasks + p to cooperatively pause/resume; use `/workflows <runId>` for details.'), '');
        }
        lines.push(`Workflow runs (${active.length + terminal.length} total · ${active.length} active)`, '');
        if (active.length > 0) {
            lines.push('Active');
            for (const entry of active)
                lines.push(rowLine(entry, now));
            lines.push('');
        }
...
//# sourceMappingURL=workflowsCommand.js.map