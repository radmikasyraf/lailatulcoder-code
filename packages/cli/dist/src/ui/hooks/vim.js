...
const vimReducer = (state, action) => {
    switch (action.type) {
        ...
        case 'INCREMENT_COUNT':
            return {
                ...state,
                count: Math.min(state.count * DIGIT_MULTIPLIER + action.digit, MAX_COUNT),
            };
        case 'CLEAR_COUNT':
        ...
        if (linuxWriteCmd) {
            ...
            const child = execFile(bin, args, { timeout: 500 }, (err) => {
                if (err && !writeOsc52(text)) {
                    debugLogger.warn('writeClipboard: cached tool and OSC 52 both failed');
                }
            });
            child.stdin?.end(text);
            child.unref();
...
function findCharInLineReverse(line, char, fromCol) {
    const cps = [...line];
...
export function useVim(buffer, onSubmit) {
    ...
    const getCurrentCount = useCallback(() => stateRef.current.count || DEFAULT_COUNT, []);
    // ── Yank helper ──
    const yankRange = useCallback((startRow, startCol, endRow, endCol, linewise) => {
    ...
    const executeCommand = useCallback((cmdType, count) => {
        switch (cmdType) {
            ...
            case CMD_TYPES.YANK_WORD_FORWARD: {
                ...
                if (nextWord)
                    yankRange(row, col, nextWord[0], nextWord[1], false);
                break;
            }
            case CMD_TYPES.YANK_WORD_BACKWARD: {
                ...
                if (prevWord)
                    yankRange(prevWord[0], prevWord[1], row, col, false);
                break;
            }
            case CMD_TYPES.YANK_WORD_END: {
                ...
                if (wordEnd)
                    yankRange(row, col, wordEnd[0], wordEnd[1] + 1, false);
                break;
            }
            ...
            case CMD_TYPES.DELETE_MOVEMENT.RIGHT: {
                ...
                if (m) {
                    const [row, col] = bufferRef.current.cursor;
                    const lines = bufferRef.current.lines;
                    const { text, linewise } = extractMovementText(lines, row, col, m, count);
                    dispatch({
            ...
            case CMD_TYPES.REPLACE_CHAR: {
                ...
                if (replaceChar != null) {
                    ...
                    if (col + count <= cpLen(line) && col < cpLen(line)) {
                        buffer.replaceRange(row, col, row, col + count, replaceChar.repeat(count));
                    }
                }
                break;
            }
            case CMD_TYPES.TOGGLE_CASE: {
                ...
                if (toggleCount > 0) {
                    const toggled = [...cpSlice(line, startCol, startCol + toggleCount)]
                        ...
                        .join('');
                    buffer.replaceRange(startRow, startCol, startRow, startCol + toggleCount, toggled);
                }
                break;
            }
            case CMD_TYPES.JOIN_LINES: {
                ...
                const endRow = Math.min(row + Math.max(count - 1, 1), lines.length - 1);
                if (row < endRow) {
                    ...
                    buffer.replaceRange(row, 0, endRow, cpLen(lines[endRow] ?? ''), joined);
            ...
            case CMD_TYPES.INDENT_LINE: {
                ...
                const endRow = Math.min(startRow + count - 1, buffer.lines.length - 1);
                ...
                buffer.replaceRange(startRow, 0, endRow, cpLen(lines[endRow] ?? ''), indentedBlock);
                break;
            }
            case CMD_TYPES.OUTDENT_LINE: {
                ...
                buffer.replaceRange(startRow, 0, endRow, cpLen(lines[endRow] ?? ''), outdentedBlock);
                break;
            }
            default:
                return false;
    ...
    function findNextWordCol(lines, row, col, count) {
        let r = row;
        let c = col;
        for (let i = 0; i < count; i++) {
            ...
            while (c < cps.length && /\w/.test(cps[c]))
                c++;
            // Skip whitespace
            while (c < cps.length && /\s/.test(cps[c]))
                c++;
            if (c >= cps.length) {
                // Move to next line
                r++;
    ...
    function findPrevWordCol(lines, row, col, count) {
        let r = row;
        let c = col;
        for (let i = 0; i < count; i++) {
            if (c > 0) {
                ...
                const cps = [...line];
                // Skip whitespace
                while (c > 0 && /\s/.test(cps[c]))
                    c--;
                // Skip word chars
                while (c > 0 && /\w/.test(cps[c - 1]))
                    c--;
            }
            else if (r > 0) {
                r--;
    ...
    function findWordEndCol(lines, row, col, count) {
        let r = row;
        let c = col;
        for (let i = 0; i < count; i++) {
            c++;
            ...
            const cps2 = [...line];
            // Skip whitespace
            while (c < cps2.length && /\s/.test(cps2[c]))
                c++;
            // Move to end of word
            while (c < cps2.length - 1 && /\w/.test(cps2[c + 1]))
                c++;
        }
        return [r, c];
    }
    // ── Shared movement text extraction ──
    function extractMovementText(lines, row, col, movement, count) {
        let text = '';
        switch (movement) {
            case 'h': {
    ...
    const executeFind = useCallback((findType, char, count = 1) => {
        const [startRow, startCol] = buffer.cursor;
        const line = buffer.lines[startRow] ?? '';
        ...
        for (let i = 0; i < count; i++) {
            ...
            switch (findType) {
                ...
                case 'T':
                    targetCol = findCharInLineReverse(line, char, currentCol);
                    if (targetCol >= 0 && targetCol < cpLen(line) - 1)
                        targetCol++;
                    break;
                default:
                    break;
    ...
    const handleCharRead = useCallback((char) => {
    ...
    const handleDeleteMovement = useCallback((movement) => {
        ...
        const [row, col] = bufferRef.current.cursor;
        const lines = bufferRef.current.lines;
        const { text, linewise } = extractMovementText(lines, row, col, movement, count);
        dispatch({
            type: 'SET_YANK_REGISTER',
            text,
            linewise,
    ...
    const handleYankMovement = useCallback((movement) => {
        const count = getCurrentCount();
        const [row, col] = bufferRef.current.cursor;
        const lines = bufferRef.current.lines;
        const { text, linewise } = extractMovementText(lines, row, col, movement, count);
            ...
            switch (normalizedKey.sequence) {
                ...
                case 'W': {
                    ...
                    // Big WORD forward: move to next non-blank after whitespace
                    {
                        ...
                        if (r !== currentRow) {
                            ...
                            if (r > currentRow)
                                buffer.vimMoveDown(r - currentRow);
                            else
                                buffer.vimMoveUp(currentRow - r);
                            buffer.vimMoveToLineStart();
                            buffer.vimMoveRight(c);
                        }
                    }
                ...
                case ',': {
                    if (s.lastFind) {
                        ...
                        executeFind(reverseMap[s.lastFind.type], s.lastFind.char, repeatCount);
                    }
                    dispatch({ type: 'CLEAR_COUNT' });
                    dispatch({ type: 'SET_PENDING_OPERATOR', operator: null });
                    return true;
                ...
                case '~': {
                    ...
                    if (count > 0) {
                        const toggled = [...cpSlice(line, startCol, startCol + count)]
                            .map((ch) => ch === ch.toUpperCase() ? ch.toLowerCase() : ch.toUpperCase())
                            .join('');
                        buffer.replaceRange(startRow, startCol, startRow, startCol + count, toggled);
                    }
                    dispatch({
                        type: 'SET_LAST_COMMAND',
                    ...
                    if (text) {
                        ...
                        if (isLinewise) {
                            ...
                            if (row + 1 >= buffer.lines.length) {
                                ...
                                buffer.replaceRange(lastRow, lastLineLen, lastRow, lastLineLen, '\n' + repeated.replace(/\n$/, ''));
                            ...
                            buffer.replaceRange(row, insertCol, row, insertCol, text.repeat(repeatCount));
                ...
                case 'P': {
                    ...
                    if (text) {
                        ...
                        if (isLinewise) {
                            ...
                            // Paste before cursor
                            buffer.replaceRange(row, col, row, col, text.repeat(repeatCount));
