...
function enumerableValues(value) {
    try {
        ...
        try {
            ...
            return Object.values(descriptors)
                .filter((descriptor) => 'value' in descriptor && descriptor.enumerable === true)
...
function collectStrings(value, seen, depth = 0) {
    ...
    if (value instanceof Error) {
        ...
        strings.push(...collectStrings(safeReadProperty(value, 'cause'), seen, depth + 1));
    }
...
export function getContextLengthExceededInfo(error) {
    ...
    const isExceeded = fragments.some((fragment) => CONTEXT_LENGTH_PATTERNS.some((pattern) => pattern.test(fragment)) &&
