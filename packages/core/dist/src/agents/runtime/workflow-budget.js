 ...
 * warning).
 */
export function resolveMaxTokensPerWorkflow(env = process.env) {
    ...
    if (parsed < 1) {
        debugLogger.warn(`Invalid ${MAX_TOKENS_PER_WORKFLOW_ENV}=${JSON.stringify(raw)}, ` +
            `treating as unset (no cap)`);
        return null;
    }
    if (parsed > HARD_MAX_TOKENS_CEILING) {
        debugLogger.warn(`${MAX_TOKENS_PER_WORKFLOW_ENV}=${parsed} exceeds hard ceiling ` +
 ...
 */
export class WorkflowBudgetImpl {
    total;
    _spent;
    constructor(total) {
        this.total = total;
...
export class WorkflowBudgetExceededError extends Error {
    ...
    constructor(runId, budgetTotal, spent) {
        ...
        // in `countedDispatch`.
        super(`Workflow ${runId} exceeded the token budget ` +
...
//# sourceMappingURL=workflow-budget.js.map