 ...
 */
export function resolveProjectRelativePath(filePath, projectRoot, pathModule = path) {
 ...
 */
export async function resolveSymlinkAwareRelativePaths(filePath, projectRoot, realpath = fsPromises.realpath, pathModule = path) {
    const originalRelative = resolveProjectRelativePath(filePath, projectRoot, pathModule);
    ...
    try {
        ...
        if (realFilePath !== absolutePath) {
            ...
            const realRelative = resolveProjectRelativePath(realFilePath, realProjectRoot, pathModule);
