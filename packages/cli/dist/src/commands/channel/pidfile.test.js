...
vi.mock('node:fs', () => {
    const mock = {
        ...
        writeSync: (fd, data, _position, _encoding) => {
            const fdPath = fsFds.paths[fd];
            if (!fdPath)
                throw new Error('EBADF');
            fsStore[fdPath] = data;
...
import { readServiceInfo, writeServiceInfo, writeServeServiceInfo, reserveServeServiceInfo, removeServiceInfo, removeServeServiceInfo, signalService, waitForExit, } from './pidfile.js';
...
beforeEach(() => {
    for (const k of Object.keys(fsStore))
        delete fsStore[k];
    fsFds.next = 3;
    for (const k of Object.keys(fsFds.paths))
        delete fsFds.paths[Number(k)];
    for (const k of Object.keys(fsFds.flags))
        delete fsFds.flags[Number(k)];
...
describe('writeServiceInfo + readServiceInfo', () => {
    ...
    it('does not let serve metadata updates overwrite corrupt pidfiles', () => {
        ...
        expect(thrown?.message).toBe('Channel service pidfile is owned by another process.');
    ...
    it('reserves serve-owned service info with exclusive create', () => {
        ...
        expect(() => reserveServeServiceInfo({
            channels: ['telegram'],
            servePid: 5678,
        })).toThrow('EEXIST');
        expect(readServiceInfo()).toMatchObject({
