...
import { isSameProcess, readLocalBootId, readPidNamespaceId, readProcStartToken, } from '../utils/process-liveness.js';
 ...
 * colliding path.
 */
export function getSessionRecordPath() {
 ...
 */
export function resetRegisteredRecordPathForTest() {
 ...
 */
export function deriveSessionName(cwd, sessionId) {
    const base = Array.from(path
        .basename(cwd)
        .normalize('NFC')
 ...
 * undiscoverable.
 */
export async function registerSession(fields) {
    ...
    if (process.platform === 'linux' && procStart === null) {
        ...
        if (procStart === null) {
            debugLogger.debug('registerSession: start token unreadable; refusing to write an impersonable record');
            return false;
        }
    }
    ...
    if (process.platform === 'linux' && pidNs === null) {
        ...
        if (pidNs === null) {
            debugLogger.debug('registerSession: namespace id unreadable; refusing to write an unreclaimable record');
            return false;
        }
    }
    const record = {
        ...
        startedAt: Date.now(),
        qwenVersion: fields.lailatulcoderVersion ?? null,
    };
    try {
        ...
        if (existing.status === 'read-error') {
            debugLogger.debug('registerSession: record path read failed transiently; refusing to overwrite an intact record');
            return false;
        }
        if (existing.status === 'unsupported-version') {
            debugLogger.debug('registerSession: record path held by a newer-schema record');
            return false;
        }
        if (existing.status === 'ok' && !matchesLocalIdentity(existing.record)) {
            debugLogger.debug('registerSession: record path held by a foreign-identity record');
            return false;
        }
 ...
 */
export async function patchSessionRecord(patch) {
    try {
        ...
        await atomicWriteJSON(filePath, { ...record, ...patch }, { mode: REGISTRY_FILE_MODE, forceMode: true, noFollow: true });
    }
    catch (error) {
...
function matchesLocalIdentity(record) {
    if (record.pid !== process.pid)
        return false;
    if (record.pidNs !== readPidNamespaceId())
        return false;
    if (record.procStart === null)
        return true;
    const ownBootId = readLocalBootId();
    const recordBootId = bootIdOf(record.procStart);
    if (ownBootId === null)
        return recordBootId === null;
...
async function sweepOrphanedTempFile(filePath, name) {
    if (!TEMP_FILENAME.test(name))
        return;
    try {
        const stat = await fs.stat(filePath);
        if (!stat.isFile())
            return;
        if (Date.now() - stat.mtimeMs < TEMP_MAX_AGE_MS)
            return;
        await fs.unlink(filePath);
    }
    catch {
...
const READ_ERROR = { status: 'read-error' };
