...
describe('workflowsCommand', () => {
    ...
    it('treats pausing and paused workflows as active and explains cooperative controls', async () => {
        ...
        expect(result.content.indexOf('wf_pausing')).toBeLessThan(result.content.indexOf('Recent'));
        expect(result.content.indexOf('wf_paused')).toBeLessThan(result.content.indexOf('Recent'));
        ...
        expect(result.content.indexOf('wf_paused')).toBeLessThan(result.content.indexOf('wf_pausing'));
    ...
    it('reports the terminal status for a retained terminal foreground run', async () => {
        ...
        expect(result).toMatchObject({
            type: 'message',
            messageType: 'error',
            content: 'Workflow wf_fore_done is completed and cannot be paused or resumed.',
        });
    ...
    it('explains that foreground workflows cannot be paused', async () => {
        ...
        expect(result).toMatchObject({
            type: 'message',
            messageType: 'error',
            content: 'Foreground workflow runs cannot be paused or resumed; only background runs support cooperative pause.',
        });
        expect(pauseMock).not.toHaveBeenCalled();
        expect(resumeMock).not.toHaveBeenCalled();
    });
    ...
    ])('reports a state race when a %s workflow cannot be %s', async (status, operation, registryOperation) => {
        const operationMock = registryOperation === 'pause' ? pauseMock : resumeMock;
        operationMock.mockReturnValue(false);
        getMock.mockReturnValue(entry({ runId: 'wf_racing', status, isBackgrounded: true }));
    ...
    ])('rejects p for a %s workflow', async (status, message, messageType) => {
    ...
    it.each(['non_interactive', 'acp'])('does not expose pause control in %s mode', async (executionMode) => {
        const ctx = createMockCommandContext({
            services: {
                config: {
                    getWorkflowRunRegistry: () => ({
        ...
        expect(result).toMatchObject({
            type: 'message',
            messageType: 'error',
            content: 'Workflow pause controls are available only in the interactive TUI.',
        });
        expect(pauseMock).not.toHaveBeenCalled();
        expect(resumeMock).not.toHaveBeenCalled();
    });
    it('detail view: known runId returns full per-field dump', async () => {
        ...
        getMock.mockImplementation((id) => id === 'wf_target' ? detail : undefined);
    ...
    it('P5: detail view renders tokens, cap, and per-phase chips (R1 #7: formatTokenCount)', async () => {
        ...
        getMock.mockImplementation((id) => id === 'wf_detail' ? detail : undefined);
    ...
    it('P5: detail view renders "(no cap)" when uncapped', async () => {
        ...
        getMock.mockImplementation((id) => id === 'wf_uncapped' ? detail : undefined);
    ...
    describe('P7b: persisted snapshots', () => {
        ...
        async function ctxWithSnapshots(snaps, mode = 'interactive') {
            const dir = await fs.mkdtemp(path.join(os.tmpdir(), 'wf-snap-'));
            tmpDirs.push(dir);
            for (const s of snaps) {
                const snap = snapshot(s);
                await fs.writeFile(path.join(dir, `${snap.runId}.json`), JSON.stringify(snap), 'utf8');
            }
        ...
        it('p falls back to a persisted snapshot and reports the terminal status', async () => {
            ...
            expect(result).toMatchObject({
                type: 'message',
                messageType: 'error',
                content: 'Workflow wf_old is completed and cannot be paused or resumed.',
            });
        });
...
//# sourceMappingURL=workflowsCommand.test.js.map