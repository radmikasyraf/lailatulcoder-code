...
import { clearWelcomeBackState, getProjectSummaryInfo, getWelcomeBackState, saveWelcomeBackRestartChoice, } from '@lailatul-coder/lailatul-coder-core';
...
export function useWelcomeBack(config, submitQuery, buffer, settings) {
    ...
    const checkWelcomeBack = useCallback(async () => {
        ...
        try {
            ...
            const isRestartSuppressed = persistedState?.lastChoice === 'restart' &&
    ...
    const handleWelcomeBackSelection = useCallback((choice) => {
        ...
        if (choice === 'restart' && welcomeBackInfo?.summaryFingerprint) {
            void saveWelcomeBackRestartChoice(welcomeBackInfo.summaryFingerprint).catch((error) => {
                config
                    .getDebugLogger()
        ...
        if (choice === 'continue' && welcomeBackInfo?.content) {
            ...
            const contextMessage = `@.lailatulcoder/PROJECT_SUMMARY.md, Based on our previous conversation,Let's continue?`;
