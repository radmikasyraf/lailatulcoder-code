...
import { readCronTasks, updateCronTasks, generateCronTaskId, appendCronRun, taskHasLegacyCondition, parseCron, nextFireTime, nextDurableFireMs, SessionService, Storage, stripTerminalControlSequences, MAX_JOBS, } from '@lailatul-coder/lailatul-coder-core';
...
import { requireTrustedWorkspaceRuntime, resolveWorkspaceRuntimeWithLiveCompatibilityFromParam, sendConversationRuntimeUnavailable, sendGenerationClosedError, } from '../workspace-route-runtime.js';
...
export function scheduledTaskSessionName(label) {
...
function requireOpenGeneration(target, res) {
    try {
...
async function rollbackCronMutation(target, before, after, route) {
    if (!before || !after)
        return;
    await runWithScheduledTaskTarget(target, () => updateCronTasks(target.workspaceCwd, (tasks) => isDeepStrictEqual(tasks, after) ? before : tasks)).catch((error) => {
        writeStderrLine(`LailatulCoder serve: ${route} failed to roll back a stale task mutation: ${error instanceof Error ? error.message : String(error)}`);
    });
}
async function teardownBoundSession(target, sessionId) {
    if (target.cleanupSession) {
        ...
        const removed = await new SessionService(target.workspaceCwd, {
            ...
            .catch(() => false);
        if (removed)
            target.bridge.markSessionCatalogChanged?.();
    }
}
async function runWithScheduledTaskTarget(target, fn) {
    const result = target.runtimeBaseDir === undefined
        ? fn()
        : Storage.runWithResolvedRuntimeBaseDir(target.runtimeBaseDir, fn);
    return (await result);
}
function sendActivityGateError(res, error) {
    if (!error ||
        typeof error !== 'object' ||
...
function toView(task) {
    return {
        id: task.id,
        name: typeof task.name === 'string' && task.name.length > 0 ? task.name : null,
        cron: task.cron,
        prompt: task.prompt,
        ...
        sessionId: typeof task.sessionId === 'string' && task.sessionId.length > 0
            ? task.sessionId
            : null,
...
function registerScheduledTaskCrudRoutes(app, deps) {
    const { prefix, resolveTarget, mutate, safeBody, channelDeliveryAuthorizations, } = deps;
    ...
    const withTarget = (handler) => async (req, res) => {
        ...
        try {
            ...
            if (sendActivityGateError(res, error))
                return;
            throw error;
        }
    };
    // ── List ──────────────────────────────────────────────────────────
    app.get(base, withTarget(async (_req, res, target) => {
        try {
            const tasks = await runWithScheduledTaskTarget(target, () => readCronTasks(target.workspaceCwd));
            if (!requireOpenGeneration(target, res))
                return;
            res.status(200).json({ v: 1, tasks: tasks.map(toView) });
        }
        catch (err) {
            if (sendActivityGateError(res, err))
                return;
            ...
            // behind a silent [].
            writeStderrLine(`LailatulCoder serve: GET ${base} failed: ${err instanceof Error ? err.message : String(err)}`);
            res.status(500).json({
    ...
    // ── Create ────────────────────────────────────────────────────────
    app.post(base, mutate(), withTarget(async (req, res, target) => {
        ...
        if (bridge) {
            ...
            try {
                if ((await runWithScheduledTaskTarget(target, () => readCronTasks(workspaceCwd))).length >= MAX_SCHEDULED_TASKS) {
                    res.status(409).json({
            ...
            if (!requireOpenGeneration(target, res))
                return;
            try {
                const session = await runWithScheduledTaskTarget(target, () => bridge.spawnOrAttach({
                    workspaceCwd,
                    sessionScope: 'thread',
                ...
                try {
                    await runWithScheduledTaskTarget(target, async () => bridge.updateSessionMetadata(boundSessionId, {
                ...
                if (sendGenerationClosedError(res, err))
                    return;
                writeStderrLine(`LailatulCoder serve: POST ${base} failed to create the task's session: ${err instanceof Error ? err.message : String(err)}`);
                res.status(500).json({
        ...
        let rollbackAfter;
        try {
            await runWithScheduledTaskTarget(target, () => updateCronTasks(workspaceCwd, (tasks) => {
            ...
            if (sendGenerationClosedError(res, err))
                return;
            writeStderrLine(`LailatulCoder serve: POST ${base} failed: ${err instanceof Error ? err.message : String(err)}`);
            res.status(500).json({
        ...
        if (rollbackBefore && rollbackAfter) {
            try {
                ...
                await rollbackCronMutation(target, rollbackBefore, rollbackAfter, `POST ${base}`);
                await rollbackSession();
                if (sendGenerationClosedError(res, error))
                    return;
                throw error;
    ...
    app.patch(`${base}/:id`, mutate(), withTarget(async (req, res, target) => {
        const { workspaceCwd, bridge } = target;
        ...
        try {
            await runWithScheduledTaskTarget(target, () => updateCronTasks(workspaceCwd, (tasks) => {
                ...
                if (idx === -1)
                    return tasks; // not found → no write
                found = true;
                ...
                const justReEnabled = current.enabled === false && patch.enabled === true;
                ...
                const becameRecurring = patch.recurring === true && current.recurring !== true;
                const becameOneShot = patch.recurring === false && current.recurring !== false;
            ...
            if (sendGenerationClosedError(res, err))
                return;
            writeStderrLine(`LailatulCoder serve: PATCH ${base}/${id} failed: ${err instanceof Error ? err.message : String(err)}`);
            res.status(500).json({
        ...
        if (rollbackBefore && rollbackAfter) {
            try {
                ...
                await rollbackCronMutation(target, rollbackBefore, rollbackAfter, `PATCH ${base}/${id}`);
                if (sendGenerationClosedError(res, error))
                    return;
                throw error;
            }
        ...
        if (blockedLegacy) {
            res.status(409).json({
                error: 'This task uses the removed isolated run mode with a precondition and can no longer be enabled or run. Recreate it (and call the `create_sub_session` tool from the prompt if you need per-run isolation).',
        ...
        if (blockedByArchive) {
            res.status(409).json({
                error: 'This task was disabled by archiving its session; unarchive the session to re-enable it.',
        ...
        const effectiveLabelChanged = patch.name !== undefined ||
            clearName ||
        ...
        if (clearDelivery && updated.sessionId) {
            channelDeliveryAuthorizations?.revokeScheduledTask(workspaceCwd, updated.sessionId, updated.id);
        }
        res.status(200).json(toView(updated));
    }));
    // ── Delete ────────────────────────────────────────────────────────
    app.delete(`${base}/:id`, mutate(), withTarget(async (req, res, target) => {
        const { workspaceCwd, bridge } = target;
        const id = typeof req.params['id'] === 'string' ? req.params['id'] : '';
        if (id.length === 0) {
            res
        ...
        try {
            ...
            if (sendGenerationClosedError(res, err))
                return;
            writeStderrLine(`LailatulCoder serve: DELETE ${base}/${id} failed: ${err instanceof Error ? err.message : String(err)}`);
            res.status(500).json({
        ...
        if (rollbackBefore && rollbackAfter) {
            try {
                ...
                await rollbackCronMutation(target, rollbackBefore, rollbackAfter, `DELETE ${base}/${id}`);
                if (sendGenerationClosedError(res, error))
                    return;
                throw error;
            }
        ...
        if (boundSessionId && bridge) {
            try {
                await runWithScheduledTaskTarget(target, () => bridge.closeSession(boundSessionId));
            }
            catch (error) {
                if (sendActivityGateError(res, error))
                    return;
        ...
        if (boundSessionId) {
            channelDeliveryAuthorizations?.revokeScheduledTask(workspaceCwd, boundSessionId, id);
        }
    ...
    app.post(`${base}/:id/run`, mutate(), withTarget(async (req, res, target) => {
        ...
        try {
            ...
            if (sendGenerationClosedError(res, err))
                return;
            writeStderrLine(`LailatulCoder serve: POST ${base}/${id}/run failed: ${err instanceof Error ? err.message : String(err)}`);
            res.status(500).json({
        ...
        if (rollbackBefore && rollbackAfter) {
            try {
                ...
                await rollbackCronMutation(target, rollbackBefore, rollbackAfter, `POST ${base}/${id}/run`);
                if (sendGenerationClosedError(res, error))
                    return;
                throw error;
            }
        ...
        if (blockedLegacy) {
            res.status(409).json({
                error: 'This task uses the removed isolated run mode with a precondition and can no longer run. Recreate it (and call the `create_sub_session` tool from the prompt if you need per-run isolation).',
                code: 'task_legacy_unsupported',
            });
            return;
        }
        if (blockedDisabled) {
            res.status(409).json({
                error: 'Cannot run a disabled task; enable it first (unarchive its session if it was archived).',
                code: 'task_disabled',
            });
            return;
        }
 ...
 */
export function registerScheduledTasksRoutes(app, deps) {
    const { boundWorkspace, mutate, safeBody, bridge, channelDeliveryAuthorizations, } = deps;
            ...
            return {
                ...
                ...(runtime
                    ? {
                        ...
                        ...(deps.cleanupSession
                            ? {
                                cleanupSession: (sessionId) => deps.cleanupSession(runtime, sessionId),
                            }
                            : {}),
                    }
                    : {}),
 ...
 */
export function registerWorkspaceQualifiedScheduledTasksRoutes(app, deps) {
    const { workspaceRegistry, mutate, safeBody, manageScheduledTaskSessions, channelDeliveryAuthorizations, cleanupSession, } = deps;
    registerScheduledTaskCrudRoutes(app, {
        prefix: '/workspaces/:workspace',
        resolveTarget: (req, res) => {
            const runtime = resolveWorkspaceRuntimeWithLiveCompatibilityFromParam(workspaceRegistry, req, res);
            if (!runtime)
                return null;
            ...
            if (runtime.provenance === 'live-conversation' &&
                ...
                res.status(400).json({
                    error: 'Generic scheduled tasks cannot create sessions in the Conversations workspace.',
 ...
 */
const REMOVED_TASK_FIELDS = ['runMode', 'condition'];
function findRemovedTaskField(body) {
    return REMOVED_TASK_FIELDS.find((field) => field in body);
}
function removedFieldError(field) {
    return {
