 ...
 */
function classifyEscapeSequence(data, startIdx) {
 ...
 */
function skipTerminalResponse(data, startIdx) {
    if (startIdx >= data.length || data[startIdx] !== 0x1b) {
 ...
 */
function shouldReplayPendingAtStop(pending) {
    if (pending.length === 0) {
        return false;
    }
 ...
 */
export function startEarlyInputCapture() {
    ...
    captureHandler = (data) => {
        ...
        if (inputBuffer.totalBytes >= MAX_BUFFER_SIZE) {
            debugLogger.warn(`Early input capture buffer full (${MAX_BUFFER_SIZE} bytes). Stopping capture; additional keystrokes during startup will be lost.`);
            stopEarlyInputCapture();
            return;
        }
        const dataToFilter = pendingTerminalResponse.length > 0
            ? Buffer.concat([pendingTerminalResponse, data])
            : data;
        pendingTerminalResponse = Buffer.alloc(0);
        // Filter out terminal response sequences (like Kitty protocol responses)
        const { filtered, trailingPartialTerminalResponse } = filterTerminalResponses(dataToFilter);
        ...
        if (filtered.length > 0) {
            ...
            if (newLength > MAX_BUFFER_SIZE) {
                const truncated = filtered.subarray(0, MAX_BUFFER_SIZE - inputBuffer.totalBytes);
                ...
                debugLogger.debug(`Captured ${filtered.length} bytes (total: ${inputBuffer.totalBytes})`);
            }
        }
    };
 ...
 */
export function stopEarlyInputCapture() {
    ...
    debugLogger.debug(`Stopped early input capture: ${inputBuffer.totalBytes} bytes`);
}
/**
 * Get and clear captured input
 * For use by KeypressContext
 */
export function getAndClearCapturedInput() {
 ...
 */
export function stopAndGetCapturedInput() {
    stopEarlyInputCapture();
 ...
 */
export function hasCapturedInput() {
...
//# sourceMappingURL=earlyInputCapture.js.map