...
import { DEFAULT_LSP_COMMAND_CHECK_TIMEOUT_MS, DEFAULT_LSP_MAX_RESTARTS, DEFAULT_LSP_SHUTDOWN_TIMEOUT_MS, DEFAULT_LSP_SOCKET_MAX_RETRY_DELAY_MS, DEFAULT_LSP_SOCKET_RETRY_DELAY_MS, DEFAULT_LSP_STARTUP_TIMEOUT_MS, DEFAULT_LSP_WARMUP_DELAY_MS, } from './constants.js';
...
export class LspServerManager {
    ...
    serverHandles = new Map();
    serverConfigHashes = new Map();
    /** Serializes hot-reload reconcile calls so stop/start operations do not race. */
    reconcileQueue = Promise.resolve();
    stoppingAll = false;
    requireTrustedWorkspace;
    workspaceRoot;
    constructor(config, workspaceContext, fileDiscoveryService, options) {
        this.config = config;
    ...
    setServerConfigs(configs) {
        ...
        debugLogger.info(`Prepared ${configs.length} LSP server config(s): ${formatServerNames(configs.map((config) => config.name))}`);
    }
    /** Drops all prepared handles without attempting process shutdown. */
    clearServerHandles() {
        if (this.serverHandles.size > 0) {
            debugLogger.info(`Clearing ${this.serverHandles.size} LSP server handle(s): ${formatServerNames(Array.from(this.serverHandles.keys()))}`);
        }
        this.serverHandles.clear();
        this.serverConfigHashes.clear();
    }
    ...
    async reconcileServerConfigs(configs) {
    ...
    async doReconcileServerConfigs(configs) {
        debugLogger.info(`Reconciling LSP server configs: desired=${formatServerNames(configs.map((config) => config.name))}`);
        ...
        debugLogger.info(`LSP reconcile result: added=${formatServerNames(result.added)}, removed=${formatServerNames(result.removed)}, restarted=${formatServerNames(result.restarted)}, unchanged=${formatServerNames(result.unchanged)}, failed=${formatServerNames(result.failed)}`);
        return result;
    }
    /**
    ...
    async doStartServer(name, handle) {
        ...
        if ((this.requireTrustedWorkspace || handle.config.trustRequired) &&
            !workspaceTrusted) {
            debugLogger.warn(`LSP server ${name} requires trusted workspace, skipping startup`);
            handle.status = 'FAILED';
            this.serverConfigHashes.delete(name);
            return;
        }
        // Check workspace trust before starting the server
        const trusted = await this.checkWorkspaceTrust(name, handle.config, workspaceTrusted);
        if (!trusted) {
            debugLogger.warn(`Workspace trust check failed, not starting LSP server ${name}`);
            handle.status = 'FAILED';
            this.serverConfigHashes.delete(name);
            return;
        }
        ...
        if (handle.config.command) {
            ...
            if (!this.isPathSafe(handle.config.command, this.workspaceRoot, commandCwd)) {
                debugLogger.warn(`LSP server ${name} command path is unsafe: ${handle.config.command}`);
                handle.status = 'FAILED';
                this.serverConfigHashes.delete(name);
                return;
            }
            if (!(await this.commandExists(handle.config.command, handle.config.env, commandCwd))) {
                debugLogger.warn(`LSP server ${name} command not found: ${handle.config.command}`);
                handle.status = 'FAILED';
                this.serverConfigHashes.delete(name);
                return;
            }
        ...
        try {
            ...
            debugLogger.info(`Starting LSP server ${name}: command=${handle.config.command ?? '<none>'}, transport=${handle.config.transport}, languages=${formatServerNames(handle.config.languages)}`);
            // Create LSP connection
            const connection = await this.createLspConnection(handle.config, startupAbortController.signal);
            ...
            try {
                ...
                await this.raceStartupAbort(this.initializeLspServer(connection, handle.config), startupAbortController.signal, undefined, startupExit?.promise);
            }
            finally {
                startupExit?.dispose();
            }
            ...
            handle.status = 'FAILED';
            handle.error = error;
            const processDiagnostics = typeof error === 'object' && error !== null
                ? error
                    .processDiagnostics
                : undefined;
            ...
            if (handle.processDiagnostics) {
                debugLogger.error(`LSP server ${name} process diagnostics:`, handle.processDiagnostics);
            }
     ...
     */
    async releaseServerResources(name, handle, graceful) {
     ...
     */
    async shutdownConnection(handle) {
        if (!handle.connection) {
            return;
        }
        try {
            ...
            const timeout = handle.config.shutdownTimeout ?? DEFAULT_LSP_SHUTDOWN_TIMEOUT_MS;
            let timerId;
            try {
                await Promise.race([
                    shutdownPromise,
    ...
    attachRestartHandler(name, handle) {
        if (!handle.process) {
            return;
        }
        handle.process.once('exit', (code) => {
            ...
            if (!handle.config.restartOnCrash) {
                debugLogger.warn(`LSP server ${name} exited but restartOnCrash is disabled`);
                handle.status = 'FAILED';
                this.serverConfigHashes.delete(name);
                return;
            }
            ...
            if (maxRestarts <= 0) {
                debugLogger.warn(`LSP server ${name} exited but maxRestarts is ${maxRestarts}`);
                handle.status = 'FAILED';
                this.serverConfigHashes.delete(name);
                return;
            }
            ...
            if (attempts >= maxRestarts) {
                debugLogger.warn(`LSP server ${name} reached max restart attempts (${maxRestarts}), stopping restarts`);
                handle.status = 'FAILED';
                this.serverConfigHashes.delete(name);
                return;
            }
            handle.restartAttempts = attempts + 1;
            debugLogger.warn(`LSP server ${name} exited (code ${code ?? 'unknown'}), restarting (${handle.restartAttempts}/${maxRestarts})`);
            this.enqueueCrashRestart(name, handle);
        });
    }
    createStartupExitWatcher(name, handle) {
        if (!handle.process) {
            return undefined;
        }
        let onExit;
        const promise = new Promise((_, reject) => {
            onExit = (code, signal) => {
                handle.processExitedUnexpectedly = true;
                reject(new Error(`LSP server ${name} exited before initialization completed (code ${code ?? 'unknown'}, signal ${signal ?? 'unknown'})`));
            };
    ...
    enqueueCrashRestart(name, handle) {
        const restart = async () => {
            if (handle.startingPromise) {
                await handle.startingPromise.catch(() => undefined);
            }
    ...
    buildProcessEnv(env) {
        ...
        for (const [key, value] of Object.entries(env)) {
            if (SECURITY_SENSITIVE_ENV_KEYS.has(key.toUpperCase())) {
                debugLogger.warn(`Ignoring security-sensitive LSP server env override: ${key}`);
                continue;
            }
    ...
    buildCommandProbeEnv(env) {
        ...
        for (const [key, value] of Object.entries(env)) {
            ...
            if (normalizedKey === 'PATH' ||
                ...
                debugLogger.warn(`Ignoring security-sensitive LSP command probe env override: ${key}`);
                continue;
            }
            filteredEnv[key] = value;
        }
    ...
    async connectSocketWithRetry(socket, timeoutMs, signal) {
        ...
        while (true) {
            ...
            try {
                return await this.raceStartupAbort(LspConnectionFactory.createSocketConnection(socket, remaining), signal, (connection) => connection.connection.end());
            }
            catch (error) {
                attempt += 1;
                ...
                const delay = Math.min(DEFAULT_LSP_SOCKET_RETRY_DELAY_MS * attempt, DEFAULT_LSP_SOCKET_MAX_RETRY_DELAY_MS);
    ...
    async delayWithAbort(delayMs, signal) {
        if (!signal) {
    ...
    async raceStartupAbort(promise, signal, cleanupAfterAbort, failurePromise) {
        if (!signal) {
            if (!failurePromise) {
                return promise;
            }
     ...
     */
    async createLspConnection(config, signal) {
        const workspaceFolder = config.workspaceFolder ?? this.workspaceRoot;
        const startupTimeout = config.startupTimeout ?? DEFAULT_LSP_STARTUP_TIMEOUT_MS;
        ...
        if (config.transport === 'stdio') {
            ...
            const lspConnection = await LspConnectionFactory.createStdioConnection(config.command, config.args ?? [], { cwd: workspaceFolder, env }, startupTimeout);
            return {
                connection: lspConnection.connection,
                process: lspConnection.process,
                ...
                initialize: async (params) => lspConnection.connection.initialize(params),
            };
        }
            ...
            if (config.command) {
                ...
                process.stderr?.on('data', (chunk) => {
                    processDiagnostics.stderrTail = (processDiagnostics.stderrTail + chunk.toString()).slice(-4000);
                });
                earlyExit = this.watchSocketProcessEarlyExit(process, processDiagnostics);
                await this.waitForSocketProcessSpawn(process, signal);
            }
            try {
                const socketConnection = this.connectSocketWithRetry(config.socket, startupTimeout, signal);
                const lspConnection = process
                    ? await this.waitForSocketConnectionOrProcessExit(socketConnection, earlyExit)
                ...
                if (processDiagnostics &&
                    ...
                    error.processDiagnostics = processDiagnostics;
                }
    ...
    async waitForSocketConnectionOrProcessExit(socketConnection, earlyExit) {
        try {
     ...
     */
    async waitForSocketProcessSpawn(process, signal) {
        this.throwIfStartupAborted(signal);
        await new Promise((resolve, reject) => {
            let settled = false;
            const cleanup = () => {
            ...
            const onError = (error) => settle(() => reject(new Error(`Failed to spawn LSP server: ${error.message}`)));
            const onAbort = () => settle(() => {
                if (process.exitCode === null) {
                    try {
                        ...
                        debugLogger.warn('Error killing aborted LSP socket process:', error);
                    }
                }
                reject(new Error('LSP server startup cancelled'));
            });
    ...
    watchSocketProcessEarlyExit(process, diagnostics) {
        let onExit;
        const promise = new Promise((_, reject) => {
            onExit = (code, signal) => {
                if (diagnostics) {
                ...
                reject(new Error(`LSP server exited before socket connection was ready: code=${code ?? 'unknown'}, signal=${signal ?? 'none'}`));
            };
     ...
     * Initialize LSP server
     */
    async initializeLspServer(connection, config) {
     ...
     */
    async commandExists(command, env, cwd) {
     ...
     */
    isPathSafe(command, workspacePath, cwd) {
        ...
        return (resolvedPath.startsWith(resolvedWorkspacePath + path.sep) ||
    ...
    async checkWorkspaceTrust(serverName, serverConfig, workspaceTrusted) {
        ...
        if (this.requireTrustedWorkspace || serverConfig.trustRequired) {
            debugLogger.warn(`Workspace not trusted, skipping LSP server ${serverName} (${serverConfig.command ?? serverConfig.transport})`);
            return false;
        }
        debugLogger.info(`Untrusted workspace, but LSP server ${serverName} has trustRequired=false, attempting cautious startup`);
        return true;
    }
    /**
...
function formatServerNames(names) {
