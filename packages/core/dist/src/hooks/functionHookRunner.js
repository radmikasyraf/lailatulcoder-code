...
export class FunctionHookRunner {
     ...
     */
    async execute(hookConfig, eventName, input, context) {
        ...
        if (signal?.aborted) {
            return {
                ...
                success: false,
                outcome: 'cancelled',
                error: new Error(`Function hook execution cancelled (aborted): ${hookId}`),
                duration: 0,
            };
        }
        try {
            ...
            const result = await this.executeWithTimeout(hookConfig.callback, input, context, timeout, signal);
            const duration = Date.now() - startTime;
            debugLogger.debug(`Function hook ${hookId} completed successfully in ${duration}ms`);
            // Process the callback result
            const executionResult = this.processHookResult(hookConfig, eventName, result, duration);
            ...
            if (executionResult.success && hookConfig.onHookSuccess) {
                try {
                    ...
                    debugLogger.warn(`onHookSuccess callback failed for ${hookId}: ${error instanceof Error ? error.message : String(error)}`);
                }
            }
     ...
     */
    processHookResult(hookConfig, eventName, result, duration) {
        ...
        const outcome = this.determineOutcome(output);
        return {
            hookConfig,
            eventName,
     ...
     */
    async executeWithTimeout(callback, input, context, timeout, signal) {
        // Validate callback
        ...
        try {
            ...
            const promises = [callback(input, context), timeoutPromise];
            if (signal) {
...
//# sourceMappingURL=functionHookRunner.js.map