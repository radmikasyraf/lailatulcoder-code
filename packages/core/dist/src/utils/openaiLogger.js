...
export function resolveOpenAILogDir(customLogDir, cwd) {
...
function extractSubagentSuffix(promptId) {
...
function promptIdSuffixForFilename(promptId) {
    if (!promptId)
        return undefined;
...
function sessionIdFromPromptId(promptId) {
    if (!promptId)
        return undefined;
    const mainSessionDelimiterIndex = promptId.indexOf(MAIN_SESSION_PROMPT_ID_DELIMITER);
...
function getErrorRequestId(error) {
    const source = error;
    return (getString(source.requestID) ??
...
export class OpenAILogger {
     ...
     */
    async logInteraction(request, response, error, promptId) {
        if (!this.initialized) {
...
//# sourceMappingURL=openaiLogger.js.map