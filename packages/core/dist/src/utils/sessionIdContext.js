 ...
 */
export const sessionIdContext = new AsyncLocalStorage();
/**
 ...
 * all of them.
 */
const projectDirBySession = new Map();
export function registerSessionProjectDir(sessionId, projectDir) {
    if (sessionId && projectDir)
        projectDirBySession.set(sessionId, projectDir);
}
 ...
 */
export function unregisterSessionProjectDir(sessionId) {
 ...
 */
const modelIdentityBySession = new Map();
export function registerSessionModel(sessionId, model, identity) {
    if (!sessionId || !model)
        return;
    ...
    if (identity)
        modelIdentityBySession.set(sessionId, identity);
    else
...
export function getSessionModelIdentity(sessionId) {
 ...
 */
export function unregisterSessionModel(sessionId) {
    modelBySession.delete(sessionId);
    modelIdentityBySession.delete(sessionId);
}
//# sourceMappingURL=sessionIdContext.js.map