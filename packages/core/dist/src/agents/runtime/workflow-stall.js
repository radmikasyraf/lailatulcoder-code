 ...
 */
export function resolveStallMs(perCall, env = process.env) {
 ...
 */
export function attachStallWatchdog(emitter, controller, stallMs) {
    if (stallMs <= 0) {
    ...
    const arm = () => {
        ...
        timer = setTimeout(() => {
            if (disposed || fired)
                return;
            fired = true;
            debugLogger.warn(`[Workflow] agent dispatch stalled — no progress for ${stallMs}ms; aborting.`);
            try {
 ...
 * `stallMs <= 0`.
 */
export async function runStallResilient(attemptFn, opts) {
    ...
    for (;;) {
        ...
        try {
            ...
            if (watchdog.stalled() && attempt < MAX_STALL_ATTEMPTS) {
                debugLogger.warn(`[Workflow] agent "${label ?? 'workflow-agent'}" stalled ` +
                    `(attempt ${attempt}/${MAX_STALL_ATTEMPTS}) — retrying.`);
                continue;
            }
            if (watchdog.stalled()) {
                throw new Error(`agent "${label ?? 'workflow-agent'}" stalled on all ` +
...
//# sourceMappingURL=workflow-stall.js.map