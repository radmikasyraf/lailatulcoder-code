 ...
 */
import { iconvDecode, iconvEncode, iconvEncodingExists, } from './iconvHelper.js';
import { bomEncodingToName, decodeBOMBuffer, detectBOM, isValidUtf8, } from './fileUtils.js';
import { prepareTextFileContent, } from '../services/fileSystemService.js';
...
export function decodeBufferWithEncodingInfo(full) {
    ...
    if (detected && !isUtf8CompatibleEncoding(detected)) {
        try {
            ...
            debugLogger.warn(`Failed to decode buffer as ${detected}: ${error instanceof Error ? error.message : String(error)}`);
        }
    }
    return { content: full.toString('utf8'), encoding: 'utf-8', bom: false };
}
...
//# sourceMappingURL=sync-file-encoding.js.map