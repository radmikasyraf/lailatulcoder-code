...
import { isPathWithin, resolveContainedExistingPath, resolveContainedPotentialPath, resolveExistingPathPrefix, } from './paths.js';
...
export async function loadAgentPluginMcpServers(pluginRoot, pluginDataRoot, options = {}) {
    ...
    try {
        ...
        if (!(await fs.promises.stat(resolvedMcpPath)).isFile()) {
            debugLogger.warn('Agent Plugins mcp.json is not a regular file; disabling MCP.');
            return {};
        }
        ...
        if (error.code === 'ENOENT')
            return {};
        debugLogger.warn(`Disabling Agent Plugins MCP: ${error instanceof Error ? error.message : String(error)}`);
        return {};
    }
    let value;
    try {
    ...
    for (const [name, entry] of Object.entries(value['mcpServers'])) {
        try {
            ...
            debugLogger.warn(`Skipping Agent Plugins MCP server "${name}": ${error instanceof Error ? error.message : String(error)}`);
        }
    }
    if (options.createDataDir &&
        ...
        try {
            ...
            debugLogger.warn(`Failed to create Agent Plugins data directory; disabling stdio MCP servers: ${error instanceof Error ? error.message : String(error)}`);
...
export function validateAgentPluginStdioRuntimePaths(server) {
    if (!server.agentPluginV1 || !server.command)
        return;
...
function normalizeStdioServer(value, pluginRoot, pluginDataRoot) {
    ...
    const expandedArgs = args.map((arg) => expandPluginVariables(arg, rootString, dataString));
    const env = Object.fromEntries(Object.entries(configuredEnv).map(([key, entry]) => [
        key,
        expandPluginVariables(entry, rootString, dataString),
    ]));
    ...
    return {
        ...
        args: expandedArgs,
        env,
        cwd: normalizeCwd(cwdValue, pluginRootResolved, pluginDataResolved, rootString, dataString),
        agentPluginV1: true,
    };
}
function normalizeCommand(command, pluginRoot) {
    const bare = !command.includes('/') &&
        !command.includes('\\') &&
        path.win32.parse(command).root === '';
    if (bare)
        return command;
    if (!command.startsWith('./') || command.includes('\\')) {
        throw new Error('Stdio command must be a bare executable name or a contained ./ path.');
    }
    return resolveContainedPotentialPath(pluginRoot, path.resolve(pluginRoot, command.slice(2)));
}
function normalizeCwd(cwd, pluginRoot, pluginDataRoot, rootString, dataString) {
    if (cwd.includes('\\')) {
    ...
    if (cwd === './' || cwd.startsWith('./')) {
        ...
        throw new Error('Stdio cwd must be a contained ./, ${PLUGIN_ROOT}, or ${PLUGIN_DATA} path.');
    }
    ...
    const resolved = resolveExistingPathPrefix(path.isAbsolute(expanded)
        ? expanded
...
function normalizeHttpServer(value) {
...
function validateHttpUrl(rawUrl) {
    ...
    try {
        ...
        throw new Error(`Invalid Streamable HTTP url: ${error instanceof Error ? error.message : String(error)}`);
    }
    ...
    if (parsed.username || parsed.password || parsed.hash) {
        throw new Error('Streamable HTTP url must not contain user information or a fragment.');
    }
...
function isLoopbackHost(hostname) {
    ...
    return (ipv4.length === 4 &&
        ipv4.every((part, index) => Number.isInteger(part) &&
            part >= 0 &&
            part <= 255 &&
            (index !== 0 || part === 127)));
}
...
function assertOnlyFields(value, fields) {
    ...
    if (unknown)
        throw new Error(`Unknown MCP server field "${unknown}".`);
}
...
function expandPluginVariables(value, pluginRoot, pluginData) {
    return value.replace(/\$\{PLUGIN_ROOT\}|\$\{PLUGIN_DATA\}/g, (match) => match === '${PLUGIN_ROOT}' ? pluginRoot : pluginData);
}
