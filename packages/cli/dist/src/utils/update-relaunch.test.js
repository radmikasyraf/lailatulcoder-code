...
vi.mock('../i18n/index.js', () => ({
    t: (message, params) => params
        ? message.replace(/\{\{(\w+)\}\}/g, (match, name) => name in params ? String(params[name]) : match)
        : message,
}));
...
describe('updateBeforeRelaunch', () => {
    ...
    it('waits for the package-manager update before reporting success', async () => {
        let finishUpdate;
        handleAutoUpdate.mockReturnValue(new Promise((resolve) => {
        ...
        expect(writeStderrLine).not.toHaveBeenCalledWith('Update successful! The new version will be used on your next run.');
        finishUpdate(true);
        await expect(update).resolves.toBe(true);
        expect(writeStderrLine).toHaveBeenCalledWith('Update successful! The new version will be used on your next run.');
    });
    it.each([
        ['explicit update', true, true],
        ['background update-on-exit', false, false],
    ])('reports %s failure and returns %s', async (_source, relaunchOnFailure, expected) => {
        ...
        expect(writeStderrLine).toHaveBeenCalledWith('Automatic update failed. Please try updating manually.');
    });
    it('relaunches the old version when the update check fails', async () => {
        checkForUpdatesDetailed.mockResolvedValue({ status: 'error' });
        await expect(updateBeforeRelaunch(settings, '/repo', true)).resolves.toBe(true);
    ...
    it('waits for a standalone host update before relaunching', async () => {
        ...
        let finishUpdate;
        performStandaloneUpdate.mockReturnValue(new Promise((resolve) => {
            finishUpdate = resolve;
        }));
        const update = updateBeforeRelaunch(settings, '/repo', false);
        await vi.waitFor(() => expect(performStandaloneUpdate).toHaveBeenCalledWith('/qwen', '2.0.0'));
    ...
    it('exits the supervisor for a deferred standalone update', async () => {
        ...
        await expect(updateBeforeRelaunch(settings, '/repo', false)).resolves.toBe(false);
        expect(writeStderrLine).toHaveBeenCalledWith('Update downloaded. It will be applied after you exit this session.');
    });
});
//# sourceMappingURL=update-relaunch.test.js.map