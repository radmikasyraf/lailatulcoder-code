 ...
 * `~/.lailatulcoder/teams/{teamName}/inboxes/{agentName}.json`.
 *
 ...
 */
export function disposeInboxLocks(teamName) {
 ...
 */
export function getInboxPath(teamName, agentName) {
 ...
 * recipient.
 */
export async function writeMessage(teamName, toAgentName, message) {
 ...
 */
export async function consumeUnread(teamName, agentName, type) {
    const inboxPath = getInboxPath(teamName, agentName);
    await ensureInboxFile(inboxPath);
    return withInboxLock(inboxPath, async () => {
        const messages = await readInboxRaw(inboxPath);
        const predicate = (m) => !m.read && (type === undefined || m.type === type);
        const matching = messages.filter(predicate);
        if (matching.length === 0)
            return [];
        const updated = messages.map((m) => predicate(m) ? { ...m, read: true } : m);
 ...
 */
export async function clearInbox(teamName, agentName) {
    const inboxPath = getInboxPath(teamName, agentName);
    try {
 ...
 */
export async function clearAllInboxes(teamName) {
    const dir = getInboxesDir(teamName);
 ...
 */
export async function sendStructuredMessage(teamName, toAgentName, opts) {
