 ...
 */
import { createDebugLogger, appendToLastTextPart, buildSkillLlmContent, applySkillAllowedTools, } from '@lailatul-coder/lailatul-coder-core';
        ...
        try {
            ...
            const cronVisible = allSkills.filter((skill) => {
                if (!cronEnabled &&
                    ...
                    debugLogger.debug(`Hiding skill "${skill.name}" because cron is not enabled`);
                    return false;
                }
                return true;
            });
            ...
            const skills = cronVisible.filter((skill) => !disabled.has(skill.name.toLowerCase()));
            debugLogger.debug(`Loaded ${skills.length} bundled skill(s) as slash commands; ${cronVisible.length - skills.length} hidden by skills.disabled`);
            return skills.map((skill) => ({
                ...
                action: async (context, _args) => {
                    ...
                    const skillPrompt = buildSkillLlmContent(dirname(skill.filePath), body);
                    ...
                    let content;
                    if (rawArgs) {
                        content = appendToLastTextPart([{ text: skillPrompt }], context.invocation.raw +
...
//# sourceMappingURL=BundledSkillLoader.js.map