...
import { getPlugin, registerPlugin, supportedChannelCatalog, } from './channel-registry.js';
function invalidPlugin(type, fields) {
    return {
        channelType: type,
        displayName: type,
        management: { fields },
...
describe('channel registry', () => {
    it('publishes a plugin session-scope descriptor once with its runtime default', async () => {
        ...
        const descriptor = (await supportedChannelCatalog()).find((entry) => entry.type === 'valid-custom-session-scope');
        const scopeFields = descriptor?.fields.filter((field) => field.key === 'sessionScope');
    ...
    it('strips management for an invalid runtime session-scope default', async () => {
        registerPlugin({
            ...
            defaultSessionScope: 'workspace',
            management: { fields: [] },
            createChannel() {
                throw new Error('not used');
            },
        });
        await expect(getPlugin('invalid-session-scope-default')).resolves.toEqual(expect.objectContaining({ management: undefined }));
    });
    it.each([
        {
        ...
        },
        {
            ...
            message: 'Channel field "settings.endpoint" cannot resolve environment references.',
        },
        {
        ...
        },
        {
            ...
            message: 'Channel field "settings" cannot resolve environment references.',
        },
        {
        ...
        },
        {
            ...
            message: 'Channel field "settings.inner.token" cannot declare a nested secret.',
        },
        {
            ...
            message: 'Channel field "settings.inner.__proto__" cannot use a reserved key.',
        },
        {
        ...
        },
        {
            ...
            message: 'Channel field "mode" must declare non-empty string option values.',
        },
        {
        ...
        },
        {
            ...
            message: 'Channel field "retries" must declare a finite exclusiveMinimum.',
        },
        {
            ...
            message: 'Channel field "retentionDays" can only declare exclusiveMinimum on number fields.',
        },
        {
        ...
        },
        {
            ...
            message: 'Channel field "mode" declares a default that is not one of its options.',
        },
        {
    ...
    ])('registers $type without management metadata', async ({ type, fields, message }) => {
        const plugin = invalidPlugin(type, fields);
        const stderr = vi.spyOn(process.stderr, 'write').mockReturnValue(true);
        registerPlugin(plugin);
        expect(stderr).toHaveBeenCalledWith(expect.stringContaining(`Invalid management metadata in "${type}" channel: ${message}`));
        stderr.mockRestore();
        ...
        const entry = (await supportedChannelCatalog()).find((candidate) => candidate.type === type);
    ...
    it('registers a plugin whose management descriptor lacks a fields array without management metadata', async () => {
        ...
        const stderr = vi.spyOn(process.stderr, 'write').mockReturnValue(true);
        registerPlugin(plugin);
        expect(stderr).toHaveBeenCalledWith(expect.stringContaining('Invalid management metadata in "invalid-missing-fields-array" channel: Channel management metadata must declare a fields array.'));
        stderr.mockRestore();
        const registered = await getPlugin('invalid-missing-fields-array');
        expect(registered?.management).toBeUndefined();
        expect(registered).not.toBe(plugin);
        expect(registered?.createChannel).toBe(plugin.createChannel);
        const entry = (await supportedChannelCatalog()).find((candidate) => candidate.type === 'invalid-missing-fields-array');
        expect(entry).toEqual({
    ...
    it('registers a plugin whose validateConfig is not a function without management metadata', async () => {
        ...
        const stderr = vi.spyOn(process.stderr, 'write').mockReturnValue(true);
        registerPlugin(plugin);
        expect(stderr).toHaveBeenCalledWith(expect.stringContaining('Invalid management metadata in "invalid-validate-config" channel: Channel management metadata must declare validateConfig as a synchronous function.'));
        stderr.mockRestore();
        const registered = await getPlugin('invalid-validate-config');
        expect(registered?.management).toBeUndefined();
        expect(registered).not.toBe(plugin);
        expect(registered?.createChannel).toBe(plugin.createChannel);
        const entry = (await supportedChannelCatalog()).find((candidate) => candidate.type === 'invalid-validate-config');
        expect(entry).toEqual({
    ...
    it('registers a plugin whose validateConfig is async without management metadata', async () => {
        ...
        const stderr = vi.spyOn(process.stderr, 'write').mockReturnValue(true);
        registerPlugin(plugin);
        expect(stderr).toHaveBeenCalledWith(expect.stringContaining('Invalid management metadata in "invalid-async-validate-config" channel: Channel management metadata must declare validateConfig as a synchronous function.'));
        stderr.mockRestore();
        const registered = await getPlugin('invalid-async-validate-config');
        expect(registered?.management).toBeUndefined();
        expect(registered).not.toBe(plugin);
        expect(registered?.createChannel).toBe(plugin.createChannel);
        const entry = (await supportedChannelCatalog()).find((candidate) => candidate.type === 'invalid-async-validate-config');
        expect(entry).toEqual({
    ...
    it('registers a null-prototype plugin without management metadata and keeps Object.prototype behavior', async () => {
        ...
        const stderr = vi.spyOn(process.stderr, 'write').mockReturnValue(true);
        registerPlugin(plugin);
        expect(stderr).toHaveBeenCalledWith(expect.stringContaining('Invalid management metadata in "invalid-null-prototype-plugin" channel: Channel field "settings" cannot be a required object.'));
        stderr.mockRestore();
    ...
    it('registers a plugin whose management is a getter-only accessor without management metadata', async () => {
        ...
        const plugin = new GetterManagementPlugin();
        const stderr = vi.spyOn(process.stderr, 'write').mockReturnValue(true);
        registerPlugin(plugin);
        expect(stderr).toHaveBeenCalledWith(expect.stringContaining('Invalid management metadata in "invalid-getter-management" channel: Channel field "settings" cannot be a required object.'));
        stderr.mockRestore();
    ...
    it('registers an object field declaring required: false with management intact', async () => {
        ...
        const stderr = vi.spyOn(process.stderr, 'write').mockReturnValue(true);
        registerPlugin(plugin);
        expect(stderr).not.toHaveBeenCalledWith(expect.stringContaining('Invalid management metadata'));
        stderr.mockRestore();
        ...
        const entry = (await supportedChannelCatalog()).find((candidate) => candidate.type === 'valid-optional-required-object');
        expect(entry?.manageable).toBe(true);
        expect(entry?.fields.find((field) => field.key === 'sessionScope')).toMatchObject({
            default: 'thread',
            options: [
                { value: 'user' },
                { value: 'thread' },
    ...
    it('only marks the manually configurable built-in types as manageable', async () => {
        ...
        const builtinCatalog = catalog.filter((entry) => !entry.type.startsWith('invalid-') && !entry.type.startsWith('valid-'));
        ...
        expect(builtinCatalog
            .filter((entry) => entry.manageable)
            .map((entry) => entry.type)).toEqual(['dingtalk', 'wecom', 'feishu', 'github', 'gitlab']);
        expect(catalog.find((entry) => entry.type === 'dingtalk')?.fields).toContainEqual(expect.objectContaining({
            key: 'clientSecret',
            kind: 'secret',
            required: true,
        }));
        for (const type of ['dingtalk', 'wecom', 'feishu']) {
            const fields = catalog.find((entry) => entry.type === type)?.fields;
            expect(fields
                ?.find((field) => field.key === 'senderPolicy')
                ?.options?.map((option) => option.value)).toEqual(['pairing', 'allowlist', 'open']);
            expect(fields?.find((field) => field.key === 'senderPolicy')).toMatchObject({ default: 'pairing' });
            ...
            expect(fields
                ...
                ?.options?.map((option) => option.value)).toEqual(['disabled', 'pairing', 'allowlist', 'open']);
            expect(fields?.find((field) => field.key === 'sessionScope')).toMatchObject({
                kind: 'enum',
                required: true,
                default: 'user',
                options: [
        ...
        for (const type of ['github', 'gitlab']) {
            const fields = catalog.find((entry) => entry.type === type)?.fields;
            expect(fields?.filter((field) => field.key === 'senderPolicy')).toHaveLength(1);
            expect(fields?.filter((field) => field.key === 'groupPolicy')).toHaveLength(1);
            expect(fields).toContainEqual(expect.objectContaining({
                key: 'groupPolicy',
                kind: 'enum',
                required: true,
            ...
            expect(fields
                ?.find((field) => field.key === 'groupPolicy')
                ?.options?.map((option) => option.value)).toContain('pairing');
            expect(fields).toContainEqual(expect.objectContaining({
                key: 'senderPolicy',
                kind: 'enum',
                required: true,
            ...
            expect(fields?.filter((field) => field.key === 'sessionScope')).toHaveLength(1);
            expect(fields?.find((field) => field.key === 'sessionScope')).toMatchObject({
                kind: 'enum',
                required: true,
                default: 'chat_thread',
        ...
        expect(catalog.find((entry) => entry.type === 'github')?.fields).toContainEqual(expect.objectContaining({
            key: 'sessionScope',
            default: 'chat_thread',
        }));
        expect(catalog.find((entry) => entry.type === 'telegram')?.fields).not.toContainEqual(expect.objectContaining({ key: 'sessionScope' }));
        expect(catalog.find((entry) => entry.type === 'dingtalk')?.fields).toContainEqual(expect.objectContaining({
        ...
        expect(catalog.find((entry) => entry.type === 'gitlab')?.fields).toContainEqual(expect.objectContaining({
            key: 'token',
            kind: 'secret',
            required: true,
        }));
        const githubFields = catalog.find((entry) => entry.type === 'github')?.fields;
        expect(githubFields).toContainEqual(expect.objectContaining({
            key: 'token',
            kind: 'secret',
        }));
        expect(githubFields?.find((field) => field.key === 'token')).not.toHaveProperty('required');
        expect(githubFields).toContainEqual(expect.objectContaining({
            key: 'useLocalGh',
            kind: 'boolean',
        }));
...
//# sourceMappingURL=channel-registry.test.js.map