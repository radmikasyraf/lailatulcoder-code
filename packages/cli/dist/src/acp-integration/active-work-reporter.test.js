...
import { ACTIVE_WORK_HEARTBEAT_VERSION, ACTIVE_WORK_HOLD_CATEGORIES, ACTIVE_WORK_NOTIFICATION_METHOD, } from '@lailatul-coder/acp-bridge/bridgeTypes';
import { ActiveWorkReporter, } from './active-work-reporter.js';
const INTERVAL_MS = 5_000;
function source(sessionId, collect) {
    return { sessionId, collectActiveWorkHolds: collect };
}
function throwingSource(sessionId) {
    return {
        sessionId,
...
describe('ActiveWorkReporter', () => {
    ...
    it('publishes a full snapshot of every source immediately', async () => {
        const reporter = new ActiveWorkReporter(send, () => [
            source('s1', () => [{ category: 'agent', id: 'a1' }]),
            source('s2', () => []),
        ], INTERVAL_MS, ACTIVE_WORK_HOLD_CATEGORIES);
    ...
    it('increases seq monotonically across reports', async () => {
        const reporter = new ActiveWorkReporter(send, () => [], INTERVAL_MS, ACTIVE_WORK_HOLD_CATEGORIES);
        await reporter.flush();
        await reporter.flush();
    ...
    it('filters only the wire snapshot to negotiated categories', async () => {
        ...
        const reporter = new ActiveWorkReporter(send, () => [source('s1', collect)], INTERVAL_MS, ['agent', 'notification']);
        await reporter.flush();
    ...
    describe('when a source throws while collecting', () => {
        it('does not let the failure escape the interval timer', async () => {
            ...
            try {
                const reporter = new ActiveWorkReporter(send, () => [throwingSource('s1')], INTERVAL_MS, ACTIVE_WORK_HOLD_CATEGORIES);
    ...
    it('does not let a failing transport escape either', async () => {
        const reporter = new ActiveWorkReporter(async () => {
            throw new Error('stream closed');
        }, () => [source('s1', () => [])], INTERVAL_MS, ACTIVE_WORK_HOLD_CATEGORIES);
        await expect(reporter.flush()).resolves.toBeUndefined();
        await expect(reporter.flush()).resolves.toBeUndefined();
        reporter.dispose();
    });
    it('stops publishing after dispose', async () => {
        vi.useFakeTimers();
        const reporter = new ActiveWorkReporter(send, () => [source('s1', () => [])], INTERVAL_MS, ACTIVE_WORK_HOLD_CATEGORIES);
...
//# sourceMappingURL=active-work-reporter.test.js.map