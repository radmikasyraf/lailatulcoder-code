...
export class CommandService {
     ...
     */
    static async create(loaders, signal, disabledNames) {
        ...
        if (disabledNames && disabledNames.size > 0) {
            ...
            for (const entry of disabledNames) {
                ...
                if (trimmed)
                    normalizedDisabled.add(trimmed.toLowerCase());
            }
            if (normalizedDisabled.size > 0) {
                for (const [name, cmd] of Array.from(commandMap.entries())) {
                    ...
                    const matchesAlias = (cmd.altNames ?? []).some((a) => normalizedDisabled.has(a.toLowerCase()));
    ...
    getCommandsForMode(mode) {
        return Object.freeze(filterCommandsForMode(this.commands.filter((cmd) => !cmd.hidden && cmd.userInvocable !== false), mode));
    }
    /**
    ...
    getModelInvocableCommands() {
        return this.commands.filter((cmd) => !cmd.hidden && cmd.modelInvocable === true);
    }
}
