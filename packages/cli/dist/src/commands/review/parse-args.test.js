...
import { parseArgsCommand, parseReviewArgs, tokenizeArgs, } from './parse-args.js';
import { reviewCommand } from '../review.js';
import { reviewSourceRoots, reviewSourcesDigest } from './lib/stale-bundle.js';
import { FOREIGN_DIGEST, makeStaleBundleFixture, stampDigest, } from './lib/test-utils.js';
...
vi.mock('node:fs', async (importOriginal) => {
    ...
    const mock = {
        ...real,
        readFileSync: vi.fn((path, ...rest) => path === 0
            ? fsState.stdin
            : real['readFileSync'](path, ...rest)),
...
describe('parseReviewArgs', () => {
    ...
    it('canonicalizes an uppercase scheme/host and drops query and fragment', () => {
        const got = parseReviewArgs('HTTPS://GitHub.com/LailatulCoder/lailatul-coder/pull/42?diff=split#discussion');
        expect(got.target).toEqual({
            type: 'pr-url',
            url: 'https://github.com/LailatulCoder/lailatul-coder/pull/42',
            host: 'github.com',
    ...
    it('a trailing path segment after the number stays a valid URL boundary', () => {
        const got = parseReviewArgs('https://github.com/LailatulCoder/lailatul-coder/pull/42/files');
    ...
    it('an Aone codereview URL is a pr-url target keyed on the global MR id', () => {
        const got = parseReviewArgs('https://code.alibaba-inc.com/maxcompute/odps_src/codereview/29295886');
    ...
    it('an Aone codereview URL with a trailing query still parses', () => {
        const got = parseReviewArgs('https://code.alibaba-inc.com/maxcompute/odps_src/codereview/123?tab=files');
    ...
    it('an Aone codereview URL with a nested group keeps the last two segments', () => {
        const got = parseReviewArgs('https://code.alibaba-inc.com/sub/maxcompute/odps_src/codereview/123');
        expect(got.target).toMatchObject({
            type: 'pr-url',
            owner: 'maxcompute',
            repo: 'odps_src',
        ...
        expect(got.target.url).toBe('https://code.alibaba-inc.com/sub/maxcompute/odps_src/codereview/123');
    });
    it('a two-segment Aone codereview URL carries its exact path too', () => {
        ...
        const got = parseReviewArgs('https://code.alibaba-inc.com/maxcompute/odps_src/codereview/5');
        expect(got.target).toMatchObject({
            type: 'pr-url',
    ...
    it('a codereview URL on a NON-Aone host is refused, not a live target', () => {
        ...
        const got = parseReviewArgs('https://github.com/LailatulCoder/lailatul-coder/codereview/123');
    ...
    it('a /pull/ URL on an AONE host is refused — Aone serves no /pull/ pages', () => {
        ...
        const got = parseReviewArgs('https://code.alibaba-inc.com/maxcompute/odps_src/pull/123');
        expect(got.target).toEqual({ type: 'local' });
        expect(got.warnings[0]).toContain('not a PR/CR URL');
    });
    it('the trailing-dot FQDN spelling of an Aone host is refused too', () => {
        ...
        const got = parseReviewArgs('https://code.alibaba-inc.com./maxcompute/odps_src/pull/5');
        expect(got.target).toEqual({ type: 'local' });
        expect(got.warnings[0]).toContain('not a PR/CR URL');
    });
    it('refuses a junk PR URL instead of guessing (never a file path, never PR 42)', () => {
        const got = parseReviewArgs('https://github.com/LailatulCoder/lailatul-coder/pull/42oops');
        expect(got.target).toEqual({ type: 'local' });
...
describe('parseReviewArgs — --severity-floor (the convergence posture knob)', () => {
    ...
    it('parses both forms case-insensitively; the last valid occurrence wins', () => {
        ...
        expect(parseReviewArgs('6711 --severity-floor critical --severity-floor suggestion')).toMatchObject({ severityFloor: 'suggestion' });
    });
    ...
    it('an invalid value warns naming what is in effect, and never eats the target', () => {
        ...
        expect(got.severityFloor).toBe('auto');
        expect(got.warnings.some((w) => w.includes('Invalid --severity-floor value "criticl"') &&
            w.includes('round-adaptive default'))).toBe(true);
    });
    it('an invalid equals-form value warns instead of vanishing', () => {
        ...
        expect(got.target).toEqual({ type: 'pr-number', number: 6711 });
        expect(got.severityFloor).toBe('auto');
        expect(got.warnings.some((w) => w.includes('Invalid --severity-floor value "critcl"'))).toBe(true);
    });
    it('a sole invalid value becomes the target, and the warning says so', () => {
        ...
        expect(got.warnings.some((w) => w.includes('Invalid --severity-floor value "criticl"') &&
            w.includes('treating it as the review target'))).toBe(true);
    });
    it('an unrelated typo never changes WHICH codebase is reviewed', () => {
        ...
        expect(got.target).toEqual({ type: 'pr-number', number: 6711 });
        expect(got.warnings.some((w) => w.includes('"blocker"') && w.includes('discarded'))).toBe(true);
    });
    it('a typed target outranks a PR-shaped flag value', () => {
        ...
        expect(got.target).toEqual({ type: 'pr-number', number: 6711 });
        expect(got.extraTokens).toEqual([]);
        expect(got.warnings.some((w) => w.includes('"6712"') && w.includes('discarded'))).toBe(true);
    });
    it('two PR-shaped flag values are ambiguous — refused, never first-wins', () => {
        ...
        expect(got.target).toEqual({ type: 'local' });
        expect(got.extraTokens).toEqual([]);
        expect(got.warnings.some((w) => w.includes('Ambiguous target') &&
            w.includes('"6711"') &&
            w.includes('"6712"'))).toBe(true);
    });
    ...
    it('same-id CR URLs from DIFFERENT nested groups are ambiguous', () => {
        ...
        const got = parseReviewArgs('--severity-floor https://code.alibaba-inc.com/groupA/sub/app/codereview/7 ' +
    ...
    it('records the --host flag verbatim for the write gate', () => {
        expect(parseReviewArgs('123 --host gitlab.alibaba-inc.com').host).toBe('gitlab.alibaba-inc.com');
        expect(parseReviewArgs('123 --host=code.alibaba-inc.com').host).toBe('code.alibaba-inc.com');
    ...
    it('the equals form rescues a PR-shaped value exactly as the spaced form does', () => {
        ...
        const mixed = parseReviewArgs('--severity-floor 6711 --effort https://github.com/LailatulCoder/lailatul-coder/pull/6711');
    ...
    it('an explicit auto floor is legal and overrides a configured floor for one run', () => {
        ...
        expect(parseReviewArgs('6711 --severity-floor auto', {
            severityFloor: 'critical',
    ...
    it('a quoted-empty value is consumed as missing on both flags', () => {
        ...
        for (const raw of ['--severity-floor ""', '--effort ""']) {
            const got = parseReviewArgs(raw);
            expect(got.target).toEqual({ type: 'local' });
            expect(got.warnings.some((w) => w.includes('requires a value'))).toBe(true);
        }
        const withTarget = parseReviewArgs('6711 --severity-floor ""');
        expect(withTarget.target).toEqual({ type: 'pr-number', number: 6711 });
        expect(withTarget.warnings.some((w) => w.includes('requires a value'))).toBe(true);
    });
    it('two invalid values are two typos, not a target and a tiebreak', () => {
        ...
        // to stat `blocker`.
        const got = parseReviewArgs('--severity-floor blocker --severity-floor warning');
        expect(got.target).toEqual({ type: 'local' });
        expect(got.extraTokens).toEqual([]);
    ...
    it('flag-final or flag-followed is a missing value, never a consumed flag', () => {
        ...
        expect(got.severityFloor).toBe('auto');
        expect(got.warnings.some((w) => w.includes('--severity-floor requires a value'))).toBe(true);
    });
    it('applies the configured review.severityFloor; an explicit flag still wins', () => {
        expect(parseReviewArgs('6711', { severityFloor: 'critical' })).toMatchObject({
            severityFloor: 'critical',
            severityFloorSource: 'configured',
        });
        expect(parseReviewArgs('6711 --severity-floor suggestion', {
            severityFloor: 'critical',
        })).toMatchObject({
            severityFloor: 'suggestion',
            severityFloorSource: 'explicit',
    ...
    it('an invalid configured floor warns on a PR target instead of dropping silently', () => {
        ...
        expect(got.severityFloor).toBe('auto');
        expect(got.warnings.some((w) => w.includes('Invalid review.severityFloor value "blocker"'))).toBe(true);
    });
});
describe('parseReviewArgs — settings-provided defaults', () => {
    ...
    it('a flag-forced high names the flag in the forcing warning, not the setting', () => {
        ...
        expect(got.effort).toBe('high');
        expect(got.effortSource).toBe('forced-by-comment');
        expect(got.warnings).toContain('`--comment` requires a verified review; running at high effort.');
        expect(got.warnings.some((w) => w.includes('`review.comment` is enabled'))).toBe(false);
    });
    ...
    it('an invalid explicit --effort falls back to the configured default and says so', () => {
        ...
        expect(got.effort).toBe('medium');
        expect(got.effortSource).toBe('configured');
        expect(got.warnings.some((w) => w.includes('using the configured review.effort'))).toBe(true);
    });
    it('an invalid configured effort warns like the flag path instead of dropping silently', () => {
        ...
        expect(got.effort).toBe('high');
        expect(got.effortSource).toBe('default');
        expect(got.warnings).toContain('Invalid review.effort value "hihg" in settings; using the default effort.');
    });
    it('a setting-forced high names the setting in the resolution, not a flag the user never typed', () => {
        ...
        expect(invalidWarning).toContain('the `review.comment` setting forces high effort');
...
describe('parseArgsCommand wiring', () => {
    ...
    it('a flag-first positional is rejected by strict mode before the handler runs (why --stdin exists)', async () => {
        await expect(runCli(['parse-args', '--effort low'])).rejects.toThrow(/Unknown argument/);
    ...
    it('positional and --stdin together are refused, not silently merged', async () => {
        fsState.stdin = '6711';
        await expect(runCli(['parse-args', '6712', '--stdin'])).rejects.toThrow(/not both/);
    });
    it('a raw string smuggled after -- is refused, not a silent local verdict', async () => {
        ...
        await expect(runCli(['parse-args', '--', '--effort low'])).rejects.toThrow(/--stdin/);
        expect(vi.mocked(writeStdoutLine)).not.toHaveBeenCalled();
    });
    ...
    describe('nested under the real review command', () => {
        async function runNested(tokens) {
            await yargs(tokens)
        ...
        it('the post--- smuggle is still refused when nested', async () => {
            await expect(runNested(['review', 'parse-args', '--', '--effort low'])).rejects.toThrow(/--stdin/);
        });
    });
});
/**
...
describe('parseArgsCommand — configured defaults wiring', () => {
    ...
    async function verdictFor(stdin) {
        fsState.stdin = stdin;
    ...
    it('discards an invalid configured effort, warning instead of dropping it silently', async () => {
        ...
        expect(got.effort).toBe('high');
        expect(got.effortSource).toBe('default');
        expect(got.warnings.some((w) => w.includes('Invalid review.effort value "bogus" in settings'))).toBe(true);
    });
    ...
    it('discards an invalid configured severityFloor, warning instead of dropping it silently', async () => {
        ...
        const got = await verdictFor('6711\n');
        expect(got.severityFloor).toBe('auto');
        expect(got.warnings.some((w) => w.includes('Invalid review.severityFloor value "bogus" in settings'))).toBe(true);
    });
...
describe('parse-args warns when the bundle is not built from these sources', () => {
    ...
    let argv1;
    beforeAll(async () => {
        fsReal = (await vi.importActual('node:fs'));
    });
    beforeEach(() => {
    ...
    const stamp = (digest) => stampDigest(fsReal, repo, digest);
    const run = () => {
        const original = process.argv[1];
        process.argv[1] = argv1;
        try {
            parseArgsCommand.handler({
                raw: '8368',
                _: ['review', 'parse-args'],
            });
        }
    ...
    it('warns when the stamp does not match the sources', () => {
        ...
        expect(vi.mocked(writeStderrLineSafe).mock.calls[0]?.[0]).toContain('NOT built from the review sources');
        expect(vi.mocked(writeStderrLineSafe).mock.calls[0]?.[0]).toContain('runs the BUILT bundle, not the working tree');
        ...
        expect(vi.mocked(writeStderrLineSafe).mock.invocationCallOrder[0]).toBeLessThan(vi.mocked(writeStdoutLine).mock.invocationCallOrder[0]);
    });
    ...
    it('says it could not check when sources exist but the stamp does not', () => {
        ...
        run();
        expect(vi.mocked(writeStderrLineSafe).mock.calls[0]?.[0]).toContain('could not check whether the bundle is current');
        ...
        expect(vi.mocked(writeStderrLineSafe).mock.calls[0]?.[0]).toContain('npm run bundle');
    });
    ...
    it.skipIf(process.platform === 'win32' || process.getuid?.() === 0)('says it could not check when a source cannot be read', () => {
        ...
        stamp(FOREIGN_DIGEST);
        const src = join(repo, 'packages', 'cli', 'src', 'commands', 'review', 'drive.ts');
        fsReal.rmSync(src);
        ...
        try {
            ...
            expect(vi.mocked(writeStderrLineSafe).mock.calls[0]?.[0]).toContain('could not check whether the bundle is current');
            expect(vi.mocked(writeStderrLineSafe).mock.calls[0]?.[0]).toContain('a review source could not be read');
        }
        finally {
            fsReal.chmodSync(src, 0o755);
        }
    });
    it.skipIf(process.platform === 'win32' || process.getuid?.() === 0)('says it could not check when the roots cannot even be statted', () => {
    ...
    it('names the cause when the roots hold nothing the digest admits', () => {
        ...
        stamp(FOREIGN_DIGEST);
        const reviewDir = join(repo, 'packages', 'cli', 'src', 'commands', 'review');
        fsReal.rmSync(join(reviewDir, 'drive.ts'));
        fsReal.writeFileSync(join(reviewDir, 'only.test.ts'), 'a test');
        fsReal.rmSync(join(repo, 'packages', 'cli', 'src', 'commands', 'review.ts'));
        fsReal.rmSync(join(repo, 'packages', 'cli', 'src', 'services', 'review-worktree-lease.ts'));
        ...
        run();
        expect(vi.mocked(writeStderrLineSafe).mock.calls[0]?.[0]).toContain('no review sources were found to compare');
    });
    ...
    it('says it could not check when only some of the roots are materialized', () => {
        ...
        run();
        expect(vi.mocked(writeStderrLineSafe).mock.calls[0]?.[0]).toContain('could not check whether the bundle is current');
        expect(vi.mocked(writeStderrLineSafe).mock.calls[0]?.[0]).toContain('only some of the review sources are present');
        expect(vi.mocked(writeStderrLineSafe).mock.calls[0]?.[0]).not.toContain('NOT built from the review sources');
    });
...
describe('--resume', () => {
    ...
    it('is effective on a PR URL target', () => {
        const r = parseReviewArgs('https://github.com/LailatulCoder/lailatul-coder/pull/6711 --resume');
        expect(r.resume).toEqual({ requested: true, effective: true });
    });
