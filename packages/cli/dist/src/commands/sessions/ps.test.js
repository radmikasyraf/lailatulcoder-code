...
vi.mock('@lailatul-coder/lailatul-coder-core', () => ({
    listLiveSessions: (...args) => listLiveSessions(...args),
}));
const stdout = [];
const stderr = [];
...
const { psCommand, formatAge, NAME_COL, PID_COL, AGE_COL } = await import('./ps.js');
...
describe('qwen sessions ps', () => {
    ...
    it('puts every column at its declared offset', async () => {
        ...
        expect(stdout[0]).toBe('NAME'.padEnd(NAME_COL) +
            'PID'.padEnd(PID_COL) +
            'AGE'.padEnd(AGE_COL) +
            'DIRECTORY');
        expect(stdout[1]).toBe('app-ab'.padEnd(NAME_COL) +
            '4242'.padEnd(PID_COL) +
            '5s'.padEnd(AGE_COL) +
            '/w/app');
    ...
    it('declares --json as a boolean that is off by default', async () => {
        const options = {};
        const yargs = {
            option: vi.fn((key, config) => {
                options[key] = config;
                return yargs;
            }),
        };
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        psCommand.builder(yargs);
