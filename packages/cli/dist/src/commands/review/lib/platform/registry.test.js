...
describe('detectPlatformKind', () => {
    ...
    it('detects Aone from an Aone --host (trimmed, port-bearing, cased)', () => {
        ...
        expect(detectPlatformKind({ host: 'GHE.Alibaba-Inc.com:8443' })).toBe('aone');
        expect(detectPlatformKind({ host: ' gitlab.alibaba-inc.com ' })).toBe('aone');
    });
    ...
    it('an explicit non-Aone host/remote beats the cwd probe', () => {
        ...
        execFileSyncMock.mockReturnValue('git@gitlab.alibaba-inc.com:maxcompute/odps_src.git');
        expect(detectPlatformKind({ host: 'github.com' })).toBe('github');
        expect(detectPlatformKind({ remoteUrl: 'git@github.com:LailatulCoder/lailatul-coder.git' })).toBe('github');
    });
...
describe('parseRemoteUrl', () => {
    it('parses the scp-like ssh form', () => {
        expect(parseRemoteUrl('git@gitlab.alibaba-inc.com:maxcompute/odps_src.git')).toEqual({
            host: 'gitlab.alibaba-inc.com',
            owner: 'maxcompute',
            repo: 'odps_src',
    ...
    it('parses a USER-LESS scp-like remote (ssh-config / insteadOf)', () => {
        expect(parseRemoteUrl('gitlab.alibaba-inc.com:maxcompute/odps_src.git')).toEqual({
            host: 'gitlab.alibaba-inc.com',
            owner: 'maxcompute',
            repo: 'odps_src',
            groupPath: 'maxcompute/odps_src',
    ...
    it('parses the https form (with and without .git)', () => {
        expect(parseRemoteUrl('https://gitlab.alibaba-inc.com/maxcompute/odps_src.git')).toEqual({
            host: 'gitlab.alibaba-inc.com',
            owner: 'maxcompute',
            repo: 'odps_src',
            groupPath: 'maxcompute/odps_src',
        });
        expect(parseRemoteUrl('https://gitlab.alibaba-inc.com/maxcompute/odps_src')).toEqual({
            host: 'gitlab.alibaba-inc.com',
            owner: 'maxcompute',
            repo: 'odps_src',
            groupPath: 'maxcompute/odps_src',
    ...
    it('parses ssh:// with a user@ prefix', () => {
        expect(parseRemoteUrl('ssh://git@gitlab.alibaba-inc.com/maxcompute/odps_src.git')).toEqual({
            host: 'gitlab.alibaba-inc.com',
            owner: 'maxcompute',
            repo: 'odps_src',
            groupPath: 'maxcompute/odps_src',
    ...
    it('lowercases the host and keeps the last two path segments (nested groups)', () => {
        expect(parseRemoteUrl('https://GitLab.Alibaba-Inc.com/sub/maxcompute/odps_src')).toEqual({
            host: 'gitlab.alibaba-inc.com',
            owner: 'maxcompute',
            repo: 'odps_src',
    ...
    it('strips a trailing slash and a .git/ suffix', () => {
        ...
        expect(parseRemoteUrl('git@gitlab.alibaba-inc.com:maxcompute/odps_src/')).toEqual({
            host: 'gitlab.alibaba-inc.com',
            owner: 'maxcompute',
            repo: 'odps_src',
            groupPath: 'maxcompute/odps_src',
