...
import { createDebugLogger, GitWorktreeService, readWorktreeSession, readWorktreeSessionMarker, isSessionRuntimeActive, worktreeBranchForSlug, writeWorktreeSession, writeWorktreeSessionMarker, } from '@lailatul-coder/lailatul-coder-core';
...
export async function setupStartupWorktree(rawInput, options) {
    ...
    // worktree creates `<otherRepo>/.lailatulcoder/worktrees/<slug>/`, which is rarely
    // what the user wants and corrupts ownership tracking.
    if (/[\\/]\.lailatulcoder[\\/]worktrees[\\/]/.test(launchCwd)) {
        return {
            ok: false,
    ...
    const service = repoRoot === launchCwd ? probe : new GitWorktreeService(repoRoot);
    ...
    const originalBranch = originalBranchRaw && originalBranchRaw !== DETACHED_HEAD
        ? originalBranchRaw
        : undefined;
    // Re-attach to an existing worktree instead of erroring out. Common
    // case: user did `LailatulCoder --worktree foo` previously, exited with Keep,
    // and now runs `lailatulcoder --resume <sid> --worktree foo` to continue. The
    ...
    if (registered !== null) {
        if (registered.branch !== expectedBranch) {
            ...
            return {
                ok: false,
                error: `--worktree: ${expectedWorktreePath} is already a git worktree, but its branch ` +
        ...
        debugLogger.debug(`setupStartupWorktree: re-attached to existing worktree at ${worktreePath} (branch=${registered.branch})`);
        return {
            ok: true,
            context: {
                worktreePath,
 ...
 */
export async function persistStartupWorktreeSidecar(config, context) {
    ...
    try {
        ...
        debugLogger.warn(`persistStartupWorktreeSidecar: failed to read existing sidecar at ${sidecarPath} — treating as "no previous worktree" and proceeding: ${error}`);
        previous = null;
    }
    ...
    if (context.wasReattached) {
        ...
        if (owner === null || owner === sessionId) {
            ...
            const ownerActive = await isSessionRuntimeActive(owner, [
                ...
                debugLogger.warn(`persistStartupWorktreeSidecar: failed to check owner runtime ${owner}: ${error}`);
                return true;
            });
 ...
 */
export function buildStartupWorktreeNotice(context, override) {
    const verb = context.wasReattached
        ? 'Re-attached to worktree'
        : 'Active worktree';
    const base = `[Startup] ${verb}: "${context.slug}" at ${context.worktreePath} ` +
    ...
    if (override?.overrodeResumedWorktree && override.overriddenSlug) {
        return (`${base}\n` +
            ...
            `That worktree directory was left intact; re-attach with \`LailatulCoder --worktree ${override.overriddenSlug}\` if needed.`);
    }
    return base;
}
//# sourceMappingURL=worktreeStartup.js.map