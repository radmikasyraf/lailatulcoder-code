...
import { buildDiffPlan, chunksCoverDiff, classifyPath, MAX_CHUNK_CHARS, parseDiff, } from './diff-plan.js';
/** Build a synthetic hunk body of `n` added lines. */
function body(n, prefix = '+') {
    return Array.from({ length: n }, (_, i) => `${prefix}line ${i}`);
}
function fileSection(path, hunks) {
    const out = [
...
describe('classifyPath', () => {
    ...
    const kinds = (paths) => Object.fromEntries(paths.map((p) => [p, classifyPath(p)]));
    const all = (paths, kind) => Object.fromEntries(paths.map((p) => [p, kind]));
...
describe('parseDiff', () => {
    ...
    it('labels a deleted file by its old path when +++ is /dev/null', () => {
        ...
        expect(files[0].hunks[0].newEnd).toBeGreaterThanOrEqual(files[0].hunks[0].newStart);
    });
...
describe('planChunks', () => {
    ...
    it('splits an oversized new-file hunk at top-level boundaries', () => {
        ...
        for (const c of plan.chunks)
            expect(c.lines).toBeLessThanOrEqual(200);
        ...
        for (let i = 1; i < fileRanges.length; i++) {
            expect(fileRanges[i].newStart).toBeGreaterThan(fileRanges[i - 1].newStart);
        }
    });
    it('reaches past the budget window for the next safe boundary', () => {
        ...
        const wall = (n) => Array.from({ length: n }, (_, k) => `+  const deep${k} = ${k};`);
        const body = [
            '+function first() {',
        ...
        const lines = diff.split('\n');
        for (const c of plan.chunks.slice(1)) {
            expect(lines[c.startLine - 1]).toMatch(/^\+function (second|third)\(\) \{$/);
        }
    ...
    it('keeps each chunk within the target size when hunks allow it', () => {
        const diff = fileSection('src/a.ts', Array.from({ length: 20 }, (_, i) => [i * 100 + 1, 30])).join('\n');
        const plan = buildDiffPlan(diff, 200);
        for (const c of plan.chunks) {
            if (!c.oversized)
                expect(c.lines).toBeLessThanOrEqual(200);
    ...
    it("attaches a file's header to the chunk owning its first hunk", () => {
        ...
        const bChunk = plan.chunks.find((c) => c.files.some((f) => f.path === 'src/b.ts'));
        expect(lines[bChunk.startLine - 1]).toBe('diff --git a/src/b.ts b/src/b.ts');
    });
    ...
    it('bounds a chunk by characters, not just lines', () => {
        ...
        const hunkBody = Array.from({ length: 300 }, (_, i) => i % 30 === 0 ? `+f${i}()` : `+  ${long}`);
        // Blank line before each `f<N>()` so a safe split point exists.
        for (let i = 0; i < hunkBody.length; i += 30)
            hunkBody[i - 1] &&= '+';
        const diff = [
    ...
    it('chunk ids are 1-based and contiguous', () => {
        const diff = fileSection('src/a.ts', Array.from({ length: 8 }, (_, i) => [i * 100 + 1, 50])).join('\n');
        const plan = buildDiffPlan(diff, 120);
        expect(plan.chunks.map((c) => c.id)).toEqual(plan.chunks.map((_, i) => i + 1));
    });
});
describe('per-kind diff line totals', () => {
    it('splits the diff into source, test, docs, and generated lines', () => {
        ...
        expect(plan.srcDiffLines +
            plan.testDiffLines +
            plan.docsDiffLines +
            plan.generatedDiffLines).toBe(plan.diffLines);
    });
...
describe('real-world shape', () => {
    it('partitions a 5800-line diff into reviewable chunks', () => {
        ...
        const diff = [
            ...fileSection('packages/channels/qqbot/src/QQChannel.ts', Array.from({ length: 40 }, (_, i) => [i * 60 + 1, 42])),
        ...
        for (const c of plan.chunks)
            expect(c.lines).toBeLessThanOrEqual(400 + 42);
    });
});
//# sourceMappingURL=diff-plan.test.js.map