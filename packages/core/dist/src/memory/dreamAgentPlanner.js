...
import { AUTO_MEMORY_INDEX_FILENAME, AUTO_MEMORY_PINNED_DIRNAME, getAutoMemoryRoot, } from './paths.js';
...
export function getTranscriptDir(projectRoot) {
    return path.join(new Storage(projectRoot).getProjectDir(), 'chats');
}
function quoteShellPathWithTrailingSeparator(dirPath) {
    return escapeShellArg(`${dirPath}${path.sep}`, getShellConfiguration().shell);
}
export function buildConsolidationTaskPrompt(memoryRoot, transcriptDir) {
    const quotedTranscriptDir = quoteShellPathWithTrailingSeparator(transcriptDir);
    return [
...
export async function planManagedAutoMemoryDreamByAgent(config, projectRoot, abortSignal, options = {}) {
    ...
    if (result.status === 'cancelled') {
        ...
        throw new Error(result.terminateReason || 'Dream agent cancelled before completion');
    }
    return result;
}
//# sourceMappingURL=dreamAgentPlanner.js.map