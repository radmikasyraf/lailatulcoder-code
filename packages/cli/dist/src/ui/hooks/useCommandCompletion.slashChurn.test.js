...
function makeContext(marker) {
    return { services: { marker } };
}
function useHarness({ ctx, commands }) {
    ...
    const completion = useCommandCompletion(buffer, '/', commands ?? defaultCommands, ctx, false);
...
describe('slash completion during commandContext churn (#9494)', () => {
    ...
    it('argument completion still receives the latest commandContext', async () => {
        const completionSpy = vi.fn().mockResolvedValue(['arg-a', 'arg-b']);
        const markerOf = (callIndex) => completionSpy.mock.calls[callIndex][0].services.marker;
        const commands = [
            createTestCommand({
                name: 'deploy',
...
//# sourceMappingURL=useCommandCompletion.slashChurn.test.js.map