...
import { SendMessageType } from '@lailatul-coder/lailatul-coder-core';
...
function createContinueRequest(requestId = 'req-continue') {
    return {
        type: 'control_request',
...
describe('runNonInteractiveStreamJson', () => {
    ...
    function installContinueDispatch() {
        ...
        mockDispatcher.dispatch.mockImplementation((request) => {
            const work = (async () => {
        ...
        mockDispatcher.getPendingIncomingRequestCount.mockImplementation(() => pendingDispatches.size);
        mockDispatcher.waitForPendingIncomingRequests.mockImplementation(async () => {
            await Promise.allSettled([...pendingDispatches]);
        });
        return { continueResults, getControlContext: () => controlContext };
    }
    function createInitializedGeminiClient(historyTail) {
    ...
    it('writes only warnings produced during deferred initialization to stderr', async () => {
        ...
        await runNonInteractiveStreamJson(config, '');
        expect(stderrWrite).toHaveBeenCalledWith('Warning: emitted during stream-json initialization\n');
    ...
    it('deduplicates continue_last_turn while a continuation is pending or running', async () => {
        ...
        expect(runNonInteractiveMock).toHaveBeenCalledTimes(1);
        expect(runNonInteractiveMock).toHaveBeenCalledWith(config, expect.objectContaining({ merged: expect.any(Object) }), '', expect.stringContaining('test-session'), expect.objectContaining({ continueInterrupted: true }));
    });
    ...
    it('interrupts only the active turn and accepts a later prompt', async () => {
        ...
        const turnControllers = [];
        runNonInteractiveMock.mockImplementation((_config, _settings, _input, _promptId, options) => {
        ...
        mockInputReader.read = async function* () {
            ...
            expect(controlContext?.getActiveTurnAbortSignal?.()).toBe(turnControllers[0]?.signal);
            controlContext?.onInterrupt?.();
    ...
    it('emits a terminal error result when an accepted continuation is abandoned by shutdown', async () => {
        ...
        expect(runNonInteractiveMock).toHaveBeenCalledTimes(1);
        expect(mockOutputAdapter.emitResult).toHaveBeenCalledWith(expect.objectContaining({
            isError: true,
    ...
    it('flushes recording failures before a session-level error result', async () => {
        ...
        expect(mockOutputAdapter.emitMessage).toHaveBeenCalledWith(expect.objectContaining({
    ...
    it('does not emit a second result when a failed continue turn already reported one', async () => {
        ...
        const initRequest = createControlRequest('initialize');
        const continueRequest = createContinueRequest();
        runNonInteractiveMock.mockImplementationOnce(async (...args) => {
    ...
    it('emits a continue_turn_failed diagnostic when a continue turn fails after a result', async () => {
        ...
        expect(mockOutputAdapter.emitSystemMessage).toHaveBeenCalledWith('continue_turn_failed', { error: 'stream collapsed mid-turn' });
    ...
    it('drops a queued running monitor event after cancellation', async () => {
        ...
        runNonInteractiveMock.mockImplementationOnce(async () => {
            monitorCallback?.('Monitor "logs" event #1: ready', '<task-notification>running</task-notification>', {
                monitorId: 'mon_1',
                toolUseId: 'tool_mon_1',
                status: 'running',
            });
        ...
        expect(mockOutputAdapter.emitSystemMessage).not.toHaveBeenCalledWith('task_notification', expect.anything());
    });
    ...
    it('runs final background cleanup after in-flight processing drains on error shutdown', async () => {
        ...
        runNonInteractiveMock.mockImplementationOnce((_config, _settings, _input, _promptId, options) => new Promise((resolve) => {
