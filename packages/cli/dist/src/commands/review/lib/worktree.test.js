...
import { appendFileSync, chmodSync, existsSync, lstatSync, mkdirSync, mkdtempSync, readFileSync, readdirSync, rmSync, symlinkSync, writeFileSync, } from 'node:fs';
import { tmpdir } from 'node:os';
import { dirname, join } from 'node:path';
import { isolateHostGitConfig } from './test-utils.js';
import { discardWorktree, exposeDependencies, sanitizedGitEnv, worktreeCreateFailureDetail, worktreeResidue, } from './worktree.js';
describe('worktreeResidue', () => {
    ...
    const git = (...args) => execFileSync('git', args, { cwd: tree, encoding: 'utf8' }).trim();
    beforeEach(() => {
    ...
    it('hands back names that survive being turned into commands', () => {
        ...
        for (const p of got)
            expect(existsSync(join(tree, p))).toBe(true);
    });
    ...
    it.skipIf(process.platform === 'win32')('does not mistake a filename containing ` -> ` for a rename record', () => {
    ...
    it('keeps the pipeline’s OWN build output out of the ignore-independent view', () => {
        ...
        writeFileSync(join(tree, 'coverage', 'lcov-report', 'index.html'), '<html>');
    ...
    it.skipIf(process.platform === 'win32')('says UNMEASURED, not clean, when the index-bit oracle cannot run', () => {
...
describe('worktreeResidue — the blind sets', () => {
    ...
    it.skipIf(process.platform === 'win32' || process.getuid?.() === 0)('reports UNMEASURED for a gitlink it cannot read, not clean', () => {
        ...
        git(repo, '-c', 'protocol.file.allow=always', 'submodule', 'add', '-q', sub, 'vendor');
    ...
    // POSIX.
    it.skipIf(process.platform !== 'linux')('reports UNMEASURED for a gitlink whose name carries invalid UTF-8 bytes', () => {
        ...
        writeFileSync(Buffer.concat([rawDir, Buffer.from('/probe-cache.txt')]), 'cache\n');
        expect(worktreeResidue(wt).unmeasured).toBeTruthy();
    });
});
describe('worktreeResidue — index bits', () => {
    ...
    it.each(['--skip-worktree', '--assume-unchanged'])('reports UNMEASURED when %s hides a tracked edit', (bit) => {
...
describe('exposeDependencies', () => {
    ...
    it('does not let `workspaces: ["."]` widen the self-link whitelist', () => {
        ...
        writeFileSync(join(root, 'package.json'), JSON.stringify({ name: 'r', workspaces: ['.'] }));
        ...
        expect(existsSync(join(probe, 'node_modules', 'r', 'secret.txt'))).toBe(false);
    });
    it('links top-level and scoped packages, counting what it linked', () => {
        ...
        expect(readdirSync(join(probe, 'node_modules'))
            .filter((e) => !e.startsWith('.'))
            .sort()).toEqual(['@scope', 'plain-pkg']);
        expect(lstatSync(join(probe, 'node_modules', 'plain-pkg')).isSymbolicLink()).toBe(true);
        expect(lstatSync(join(probe, 'node_modules', '@scope', 'inner-pkg')).isSymbolicLink()).toBe(true);
    });
    it('farms a workspace member’s own node_modules, which npm could not hoist', () => {
        ...
        for (const member of ['cli', 'absent']) {
            ...
            writeFileSync(join(root, 'packages', member, 'package.json'), JSON.stringify({ name: `@x/${member}` }));
        ...
        expect(existsSync(join(probe, 'packages', 'cli', 'node_modules', 'nested'))).toBe(true);
    ...
    it('leaves a farm THIS code built untouched, and rebuilds one it did not', () => {
        ...
        expect(existsSync(join(planted, 'node_modules', 'planted-stub'))).toBe(false);
    ...
    it('rebuilds a marked farm when the caller asks it to', () => {
        ...
        writeFileSync(join(root, 'packages', 'cli', 'package.json'), JSON.stringify({ name: '@x/cli' }));
        mkdirSync(join(probe, 'packages', 'cli'), { recursive: true });
        expect(exposeDependencies(probe, root)).toMatchObject({ linked: 2 });
        writeFileSync(join(probe, 'packages', 'cli', 'node_modules', 'planted.js'), 'x');
        expect(exposeDependencies(probe, root, { rebuild: true })).toMatchObject({
            linked: 2,
            alreadyPresent: false,
        });
        expect(existsSync(join(probe, 'packages', 'cli', 'node_modules', 'planted.js'))).toBe(false);
    });
    ...
    it('rebuild removes node_modules the farm does not recreate — planted or linked', () => {
        ...
        symlinkSync(join(root, 'node_modules'), join(probe, 'linked', 'node_modules'));
        expect(exposeDependencies(probe, root, { rebuild: true })).toMatchObject({
            linked: 2,
        });
    ...
    it('skips a stray file under a scope directory, as it does at top level', () => {
        ...
        expect(existsSync(join(probe, 'node_modules', '@scope', 'real-pkg'))).toBe(true);
        expect(existsSync(join(probe, 'node_modules', '@scope', 'notes.md'))).toBe(false);
    });
    ...
    it('still mirrors npm workspace self-links, counting them as such', () => {
        ...
        writeFileSync(join(root, 'packages', 'core', 'package.json'), JSON.stringify({ name: '@x/core' }));
        mkdirSync(join(root, 'node_modules', '@x'), { recursive: true });
        symlinkSync(join(root, 'packages', 'core'), join(root, 'node_modules', '@x', 'core'), 'dir');
        const got = exposeDependencies(probe, root);
        expect(got).toMatchObject({ linked: 1, failed: 0, selfLinked: 1 });
        expect(lstatSync(join(probe, 'node_modules', '@x', 'core')).isSymbolicLink()).toBe(true);
    });
...
describe('worktreeCreateFailureDetail', () => {
    ...
    it('names the add failure, and folds in the sweep stderr that explains it', () => {
        const got = worktreeCreateFailureDetail('probe', new Error("fatal: '/w/wt-probe' already exists"), "fatal: '/w/wt-probe' is not a working tree\n");
        ...
        expect(got).toContain("(stale-tree sweep also reported: fatal: '/w/wt-probe' is not a working tree)");
    });
    it('omits the sweep clause when the sweep said nothing', () => {
        ...
        const got = worktreeCreateFailureDetail('probe', new Error('disk full'), '   \n');
        expect(got).toBe('probe worktree could not be created: disk full');
    });
    it('survives a non-Error throw', () => {
        expect(worktreeCreateFailureDetail('probe', 'boom', '')).toBe('probe worktree could not be created: boom');
    });
});
