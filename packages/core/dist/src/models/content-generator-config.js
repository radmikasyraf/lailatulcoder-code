 ...
 */
export function buildAgentContentGeneratorConfig(base, modelId, authOverrides) {
    ...
    const resolvedModel = modelId
        ? modelsConfig.getResolvedModel(authOverrides.authType, modelId, authOverrides.baseUrl)
        : undefined;
    if (resolvedModel?.imageOnly) {
        throw new Error(`Image-only model '${resolvedModel.id}' cannot be used for content generation`);
    }
    const nextConfig = {
        ...parentConfig,
        model: modelId ?? parentConfig.model,
        authType: authOverrides.authType,
    };
    ...
    nextConfig.apiKey = resolveCredentialField(authOverrides.apiKey, sameProvider ? parentConfig.apiKey : undefined, authOverrides.authType, 'apiKey');
    nextConfig.baseUrl =
        authOverrides.baseUrl ??
            resolveCredentialField(undefined, sameProvider ? parentConfig.baseUrl : undefined, authOverrides.authType, 'baseUrl');
 ...
 */
export async function createRuntimeContentGeneratorView(base, contentGeneratorOwner, modelId, authOverrides) {
    const contentGeneratorConfig = buildAgentContentGeneratorConfig(base, modelId, authOverrides);
    const contentGenerator = await createContentGenerator(contentGeneratorConfig, contentGeneratorOwner);
    return { contentGenerator, contentGeneratorConfig };
}
function applyResolvedModelConfig(targetConfig, resolvedModel, parentConfig, authOverrides) {
    const sameProvider = authOverrides.authType === parentConfig.authType;
    ...
    if (resolvedModel.envKey) {
        ...
        targetConfig.apiKey = resolveCredentialField(authOverrides.apiKey, sameProvider ? parentConfig.apiKey : undefined, authOverrides.authType, 'apiKey');
    ...
    for (const field of MODEL_GENERATION_CONFIG_FIELDS) {
        ...
        if (registryValue !== undefined || field === 'thinkingMandatory') {
            // eslint-disable-next-line @typescript-eslint/no-explicit-any
            targetConfig[field] = registryValue;
        }
    }
}
/**
 ...
 */
export function resolveCredentialField(explicitValue, inheritedValue, authType, field) {
    if (explicitValue)
...
//# sourceMappingURL=content-generator-config.js.map