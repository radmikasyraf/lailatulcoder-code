...
import { Storage, readCronTasks, updateCronTasks, } from '@lailatul-coder/lailatul-coder-core';
...
describe('scheduled-task session lifecycle', () => {
    ...
    const seed = (tasks) => updateCronTasks(workspace, () => tasks);
    const byId = async () => Object.fromEntries((await readCronTasks(workspace)).map((t) => [t.id, t]));
...
//# sourceMappingURL=scheduled-task-session-lifecycle.test.js.map