# LailatulCoder Ai Companion Plugin: Interface Specification

> Last Updated: September 15, 2025

This document defines the contract for building a companion plugin to enable LailatulCoder Ai's IDE mode. For VS Code, these features (native diffing, context awareness) are provided by the official extension ([marketplace](https://marketplace.visualstudio.com/items?itemName=LailatulCoder.lailatul-coder-vscode-ide-companion)). This specification is for contributors who wish to bring similar functionality to other editors like JetBrains IDEs, Sublime Text, etc.

## I. The Communication Interface

LailatulCoder Ai and the IDE plugin communicate through a local communication channel.
...
For LailatulCoder Ai to connect, it needs to discover what port your server is using. The plugin **MUST** facilitate this by creating a "lock file" and setting the port environment variable.
...
**Legacy note:** For extensions older than v0.5.1, LailatulCoder Ai may fall back to reading JSON files in the system temp directory named `lailatul-coder-ide-server-<PID>.json` or `lailatul-coder-ide-server-<PORT>.json`. New integrations should not rely on these legacy files.
