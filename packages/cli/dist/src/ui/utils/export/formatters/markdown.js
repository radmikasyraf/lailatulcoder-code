...
export function toMarkdown(sessionData) {
    ...
    if (metadata?.uniqueFiles && metadata.uniqueFiles.length > 0) {
        lines.push('');
        lines.push('<details>');
        lines.push(`<summary><strong>Unique Files Referenced (${metadata.uniqueFiles.length})</strong></summary>`);
        lines.push('');
...
function formatMessageContent(message) {
    ...
    const processedText = text.replace(/--- Content from referenced files ---\n([\s\S]*?)\n--- End of content ---/g, (match, content) => `\n> **Referenced Files:**\n\n${createCodeBlock(content)}\n`);
