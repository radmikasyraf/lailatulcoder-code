...
import { ARTIFACT_TITLE_MAX_LENGTH, ARTIFACT_WORKSPACE_PATH_MAX_LENGTH, hasControlCharacter, hasUnsafeDisplayPayload, } from './record-artifact.js';
...
const ARTIFACT_KIND_BY_EXTENSION = new Map([
    ['.csv', 'file'],
    ['.htm', 'html'],
    ['.html', 'html'],
    ['.ipynb', 'notebook'],
...
class WriteFileToolInvocation extends BaseToolInvocation {
    ...
    async getConfirmationDetails(_abortSignal) {
        ...
        if (!this.config.getFileReadCacheDisabled()) {
            ...
            // stance accepts.
            const decision = await checkPriorRead(this.config.getFileReadCache(), this.params.file_path, 'overwriting');
            if (!decision.ok) {
        ...
        if (fileExists && !this.config.getFileReadCacheDisabled()) {
            const postDecision = await checkPriorRead(this.config.getFileReadCache(), this.params.file_path, 'overwriting', { expectExisting: true });
            if (!postDecision.ok) {
        ...
        const fileDiff = createPatchSmart(fileName, originalContent, this.params.content, 'Current', 'Proposed');
    ...
    async execute(_abortSignal) {
        ...
        const teamMemoryError = checkTeamMemorySecrets(file_path, content, this.config.getProjectRoot());
        if (teamMemoryError) {
        ...
        if (!this.config.getFileReadCacheDisabled()) {
            const decision = await checkPriorRead(this.config.getFileReadCache(), file_path, 'overwriting');
            if (!decision.ok) {
                return {
        ...
        // exactly that.
        if (fileExists && !this.config.getFileReadCacheDisabled()) {
            const postDecision = await checkPriorRead(this.config.getFileReadCache(), file_path, 'overwriting', { expectExisting: true });
            if (!postDecision.ok) {
        ...
        // unchanged.
        if (!this.config.getFileReadCacheDisabled()) {
            const writeDecision = await checkPriorRead(this.config.getFileReadCache(), file_path, 'overwriting', 
        ...
        try {
            ...
            if (!modified_by_user) {
                CommitAttributionService.getInstance().recordEdit(file_path, fileExists ? originalContent : null, content);
            }
            ...
            const fileDiff = createPatchSmart(fileName, currentContentForDiff, content, 'Original', 'Written');
            ...
            const artifact = buildWorkspaceArtifactMetadata(this.config, file_path, postWriteSizeBytes);
            if (artifact) {
                llmSuccessMessageParts.push(formatRecordArtifactReminder(artifact.workspacePath));
            }
 ...
 */
export function buildRecordArtifactReminder(config, filePath) {
...
function formatRecordArtifactReminder(workspacePath) {
    return (`This file was automatically recorded as a workspace artifact with ` +
        `workspacePath "${workspacePath}". No extra artifact registration step ` +
        `is needed.`);
}
export function buildWorkspaceArtifactMetadata(config, filePath, sizeBytes) {
    ...
    return {
        ...
        mimeType: getSpecificMimeType(recorded.filePath) ??
...
function resolveRecordedWorkspaceFile(config, filePath) {
    ...
    if (!ARTIFACT_KIND_BY_EXTENSION.has(path.extname(resolvedFile).toLowerCase())) {
        return null;
    }
    const workspacePath = toCanonicalWorkspaceArtifactPath(resolvedFile, resolvedRoot);
    if (!workspacePath) {
        return null;
    }
    return { filePath: resolvedFile, workspacePath };
}
function inferWorkspaceArtifactKind(filePath) {
    return (ARTIFACT_KIND_BY_EXTENSION.get(path.extname(filePath).toLowerCase()) ??
        'file');
}
/**
...
export class WriteFileTool extends BaseDeclarativeTool {
    ...
    constructor(config) {
        super(WriteFileTool.Name, ToolDisplayNames.WRITE_FILE, `Writes content to a specified file in the local filesystem. A request to create or generate a file does not establish that the target path is new. Unless the target's absence or current text contents have already been established in this session, you MUST use the ${ToolNames.READ_FILE} tool first; if the file does not exist, then create it. With prior-read enforcement enabled, blind overwrites are rejected. The file_path argument MUST be an absolute path. Always construct it by combining the project root with the file's relative path (e.g. project root '/path/to/project/' + relative 'foo/bar.txt' = '/path/to/project/foo/bar.txt'). If the user provides a relative path, resolve it against the project root first.
...
The user has the ability to modify \`content\`. If modified, this will be stated in the response.`, Kind.Edit, {
    ...
    validateToolParamValues(params) {
        ...
        const teamMemoryError = checkTeamMemorySecrets(filePath, params.content ?? '', this.config.getProjectRoot());
        if (teamMemoryError) {
