...
function request(command, extraArguments = {}) {
    return {
        sessionId: 'session-1',
        promptId: 'prompt-1',
        toolCallId: 'call-1',
...
describe('createDaemonToolGuard', () => {
    ...
    it('allows relocated read-only Git commands', async () => {
        const guard = createDaemonToolGuard();
        await expect(guard(request(`git -C ${outsideRepo} rev-parse HEAD`))).resolves.toEqual({ allowed: true });
    });
    it.each([
        `git -C ${outsideRepo} diff`,
        `git -C ${outsideRepo} log -p`,
        `git -C ${outsideRepo} show --output=${path.join(outsideRepo, 'out.txt')} HEAD`,
    ])('denies relocated Git subcommands that can execute target-repo config or write files', async (command) => {
        const guard = createDaemonToolGuard();
    ...
    ])('denies relocated Git subcommands that can mutate state', async (command) => {
        const guard = createDaemonToolGuard();
        await expect(guard(request(command))).resolves.toMatchObject({
            allowed: false,
        });
    ...
    it('denies dynamic repository relocation for mutating commands', async () => {
        const guard = createDaemonToolGuard();
        await expect(guard(request('git -C "$OTHER_WORKTREE" reset --hard'))).resolves.toMatchObject({
            allowed: false,
    ...
    it.each(['git -C', 'git --git-dir', 'git --work-tree='])('fails closed on a dangling relocation option', async (command) => {
        const guard = createDaemonToolGuard();
        await expect(guard(request(command))).resolves.toMatchObject({
            allowed: false,
        });
    ...
    it('allows mutating Git commands inside the effective working directory', async () => {
        const guard = createDaemonToolGuard();
        await expect(guard(request('git -C nested reset --hard'))).resolves.toEqual({ allowed: true });
    });
    it('resolves relative targets from the explicit shell directory', async () => {
        const guard = createDaemonToolGuard();
        await expect(guard(request('git -C .. reset --hard', { directory: insideNested }))).resolves.toEqual({ allowed: true });
        await expect(guard(request(`git -C ${path.relative(insideNested, outsideRepo)} reset --hard`, {
    ...
    it.each([
        ...
        `pwd\ngit -C ${outsideRepo} reset --hard`,
    ])('denies a relocated mutation inside shell command forms', async (command) => {
        const guard = createDaemonToolGuard();
        await expect(guard(request(command))).resolves.toMatchObject({
            allowed: false,
        });
    ...
    ])('denies a relocated mutation through wrapper invocations %#', async (buildCommand) => {
        const guard = createDaemonToolGuard();
        await expect(guard(request(buildCommand()))).resolves.toMatchObject({
            allowed: false,
        });
    ...
    it.each([
        ...
        () => 'cd - && git reset --hard',
        () => 'popd && git reset --hard',
    ])('denies mutations after a cwd-shifting builtin %#', async (buildCommand) => {
        const guard = createDaemonToolGuard();
        await expect(guard(request(buildCommand()))).resolves.toMatchObject({
            allowed: false,
        });
    ...
    ])('denies relocated mutations hidden behind shell keywords %#', async (buildCommand) => {
        const guard = createDaemonToolGuard();
        await expect(guard(request(buildCommand()))).resolves.toMatchObject({
            allowed: false,
        });
    });
    it.each([() => 'bash -c "$CMD"', () => 'sh -c "$CMD" arg'])('fails closed on undecidable shell payloads %#', async (buildCommand) => {
        const guard = createDaemonToolGuard();
        await expect(guard(request(buildCommand()))).resolves.toMatchObject({
            allowed: false,
    ...
    ])('denies a relocated mutation inside a command substitution %#', async (buildCommand) => {
        const guard = createDaemonToolGuard();
        await expect(guard(request(buildCommand()))).resolves.toMatchObject({
            allowed: false,
        });
    ...
    it('allows command substitutions that stay inside the boundary', async () => {
        ...
        await expect(guard(request('echo $(git rev-parse HEAD)'))).resolves.toEqual({ allowed: true });
        await expect(guard(request('echo $(cd nested && git commit -m x)'))).resolves.toEqual({ allowed: true });
    });
    it('fails closed on an unterminated command substitution', async () => {
        const guard = createDaemonToolGuard();
        await expect(guard(request(`echo $(git -C ${outsideRepo} reset --hard`))).resolves.toMatchObject({ allowed: false });
    });
    it.each([
        () => `bash -c'git -C ${outsideRepo} reset --hard'`,
        () => `bash -lc'git -C ${outsideRepo} reset --hard'`,
    ])('denies relocated mutations fused into the -c flag token %#', async (buildCommand) => {
        const guard = createDaemonToolGuard();
        await expect(guard(request(buildCommand()))).resolves.toMatchObject({
            allowed: false,
        });
    ...
    ])('keeps the entry cwd as the containment basis inside shell wrappers %#', async (buildCommand) => {
        const guard = createDaemonToolGuard();
        await expect(guard(request(buildCommand()))).resolves.toMatchObject({
            allowed: false,
        });
    ...
    it('still allows wrapper payloads that stay inside the entry cwd', async () => {
        const guard = createDaemonToolGuard();
        await expect(guard(request(`cd ${effectiveCwd} && sh -c 'git reset --hard'`))).resolves.toEqual({ allowed: true });
    });
    it.each([
        () => `git -c core.fsmonitor=/tmp/evil.sh -C ${outsideRepo} status`,
        () => `git -c alias.x='!evil' -C ${outsideRepo} status`,
    ])('inspects command-executing -c config even for read-only subcommands %#', async (buildCommand) => {
        const guard = createDaemonToolGuard();
        await expect(guard(request(buildCommand()))).resolves.toMatchObject({
            allowed: false,
            reason: expect.stringContaining('dynamic repository location'),
    ...
    ])('does not let --exec-path/--list-cmds swallow the relocation token %#', async (buildCommand) => {
        const guard = createDaemonToolGuard();
        await expect(guard(request(buildCommand()))).resolves.toMatchObject({
            allowed: false,
        });
    ...
    it('denies a model-supplied directory outside the effective working directory', async () => {
        const guard = createDaemonToolGuard();
        await expect(guard(request('git reset --hard', { directory: outsideRepo }))).resolves.toMatchObject({
            allowed: false,
            reason: expect.stringContaining(outsideRepo),
        });
        await expect(guard(request('git reset --hard', {
            directory: path.relative(effectiveCwd, outsideRepo),
        }))).resolves.toMatchObject({ allowed: false });
        await expect(guard(request('git reset --hard', { directory: insideNested }))).resolves.toEqual({ allowed: true });
    });
    it('keeps subshell cwd shifts from leaking into later commands', async () => {
        const guard = createDaemonToolGuard();
        await expect(guard(request(`sh -c 'cd ${outsideRepo}'; git reset --hard`))).resolves.toEqual({ allowed: true });
        await expect(guard(request(`cd ${effectiveCwd} && git reset --hard`))).resolves.toEqual({ allowed: true });
    });
    it.each([
        () => `git -C \\
...
it -C ${outsideRepo} reset --hard`,
    ])('joins backslash continuations before parsing %#', async (buildCommand) => {
        const guard = createDaemonToolGuard();
        await expect(guard(request(buildCommand()))).resolves.toMatchObject({
            allowed: false,
            reason: expect.stringContaining(outsideRepo),
    ...
    ])('denies repository relocation through environment forms %#', async (buildCommand) => {
        const guard = createDaemonToolGuard();
        await expect(guard(request(buildCommand()))).resolves.toMatchObject({
            allowed: false,
        });
    ...
    ])('resolves relative git-dir and work-tree against the final -C cwd', async (command) => {
        const guard = createDaemonToolGuard();
        await expect(guard(request(command))).resolves.toMatchObject({
            allowed: false,
        });
    ...
    ])('denies mutating subcommands with command-valued -c config', async (command) => {
        const guard = createDaemonToolGuard();
        await expect(guard(request(command))).resolves.toMatchObject({
            allowed: false,
            reason: expect.stringContaining('dynamic repository location'),
    ...
    it('allows harmless -c config on mutations inside the boundary', async () => {
        const guard = createDaemonToolGuard();
        await expect(guard(request('git -c user.name=Qwen commit --allow-empty'))).resolves.toEqual({ allowed: true });
    });
    it('fails closed on commands that cannot be parsed', async () => {
        const guard = createDaemonToolGuard();
        await expect(guard(request('git -C ${UNBALANCED reset --hard'))).resolves.toMatchObject({
            allowed: false,
    ...
    it('follows chained -C targets using Git semantics', async () => {
        const guard = createDaemonToolGuard();
        await expect(guard(request(`git -C nested -C ${outsideRepo} reset --hard`))).resolves.toMatchObject({ allowed: false });
        await expect(guard(request(`git -C ${outsideRepo} -C ${path.relative(outsideRepo, effectiveCwd)} reset --hard`))).resolves.toEqual({ allowed: true });
    });
    it('checks work-tree and git-dir targets independently', async () => {
        const guard = createDaemonToolGuard();
        await expect(guard(request(`git --work-tree=${effectiveCwd} --git-dir=${path.join(outsideRepo, '.git')} reset --hard`))).resolves.toMatchObject({ allowed: false });
    });
    ...
    it('denies relocated mutations whose target does not exist at decision time', async () => {
        ...
        await expect(guard(request('git -C not-created-yet reset --hard'))).resolves.toMatchObject({
            allowed: false,
    ...
    ])('denies a relocation after the command relinks a path %#', async (build) => {
        const guard = createDaemonToolGuard();
    ...
    it('follows gitfile redirects before the containment check', async () => {
        ...
        await writeFile(path.join(localNested, '.git'), `gitdir: ${path.join(outsideRepo, '.git')}\n`);
        const localRequest = (command) => ({
            ...request(command),
            effectiveCwd: localEffectiveCwd,
        });
        const guard = createDaemonToolGuard();
        await expect(guard(localRequest('git --git-dir=nested/.git branch -D topic'))).resolves.toMatchObject({
            allowed: false,
            reason: expect.stringContaining(outsideRepo),
        });
        await expect(guard(localRequest(`GIT_DIR=nested/.git sh -c 'git reset --hard'`))).resolves.toMatchObject({
            allowed: false,
            reason: expect.stringContaining(outsideRepo),
        });
    });
    it('canonicalizes a symlink named .git before stripping the basename', async () => {
        ...
        await symlink(path.join(outsideRepo, '.git'), path.join(localNestedD, '.git'));
        const localRequest = (command) => ({
            ...request(command),
            effectiveCwd: localEffectiveCwd,
        });
        const guard = createDaemonToolGuard();
        await expect(guard(localRequest('git --git-dir=nested/d/.git branch -D topic'))).resolves.toMatchObject({
            allowed: false,
            reason: expect.stringContaining(outsideRepo),
        });
    });
    it('resolves per-worktree admin directories to the linked worktree', async () => {
        ...
        await writeFile(path.join(outsideCheckout, '.git'), `gitdir: ${adminDir}\n`);
        await writeFile(path.join(adminDir, 'gitdir'), `${path.join(outsideCheckout, '.git')}\n`);
        const guard = createDaemonToolGuard();
        await expect(guard(request('git --git-dir=.git/worktrees/wt1 reset --hard'))).resolves.toMatchObject({
            allowed: false,
    ...
    it('allows per-worktree admin directories whose checkout stays inside', async () => {
        ...
        const guard = createDaemonToolGuard();
        await expect(guard(request('git --git-dir=.git/worktrees/wt2 reset --hard'))).resolves.toEqual({ allowed: true });
    });
    it('clamps long paths and strips control characters in denial reasons', async () => {
        ...
        const longDenial = await guard(request(`git -C ${longTarget} reset --hard`));
        expect(longDenial).toMatchObject({ allowed: false });
        const longReason = longDenial.reason;
        ...
        const controlDenial = await guard(request(`git -C '${tabTarget}' reset --hard`));
        expect(controlDenial).toMatchObject({ allowed: false });
        const controlReason = controlDenial.reason;
    ...
    it('denies dynamic relocations even for read-only subcommands', async () => {
        ...
        await expect(guard(request('git -C "$OTHER_WORKTREE" rev-parse'))).resolves.toMatchObject({
            allowed: false,
            reason: expect.stringContaining('dynamic repository location'),
        });
    });
    ...
    ])('fails closed when an unrecognized program may run a relocated Git command %#', async (buildCommand) => {
        const guard = createDaemonToolGuard();
        await expect(guard(request(buildCommand()))).resolves.toMatchObject({
            allowed: false,
    ...
    ])('denies an unrecognized program running Git after a cwd shift %#', async (buildCommand) => {
        const guard = createDaemonToolGuard();
        await expect(guard(request(buildCommand()))).resolves.toMatchObject({
            allowed: false,
        });
    ...
    it('allows an unrecognized program running Git inside the boundary', async () => {
        const guard = createDaemonToolGuard();
        await expect(guard(request('cd nested && nice git status'))).resolves.toEqual({ allowed: true });
    ...
    ])('denies a mutation after an exported Git relocation %#', async (buildCommand) => {
        const guard = createDaemonToolGuard();
        await expect(guard(request(buildCommand()))).resolves.toMatchObject({
            allowed: false,
        });
    ...
    it('leaves unexported and unrelated assignments alone', async () => {
        const guard = createDaemonToolGuard();
        await expect(guard(request('export FOO=bar && git commit -m x'))).resolves.toEqual({ allowed: true });
        await expect(guard(request(`export GIT_WORK_TREE=${insideNested} && git commit -m x`))).resolves.toEqual({ allowed: true });
        // Without `export` (or `set -a`) the assignment stays shell-local and
        // never reaches the git process.
        await expect(guard(request(`GIT_WORK_TREE=${outsideRepo}; echo done`))).resolves.toEqual({ allowed: true });
    });
    it.each([
    ...
    it.each([
        ...
        () => `cd -- ${outsideRepo} && git reset --hard`,
    ])('denies a mutation after an option-carrying cd %#', async (buildCommand) => {
        const guard = createDaemonToolGuard();
        await expect(guard(request(buildCommand()))).resolves.toMatchObject({
            allowed: false,
        });
    ...
    it('keeps an option-carrying cd inside the boundary allowed', async () => {
        const guard = createDaemonToolGuard();
        await expect(guard(request('cd -P nested && git commit -m x'))).resolves.toEqual({ allowed: true });
        await expect(guard(request('cd -- nested && git commit -m x'))).resolves.toEqual({ allowed: true });
    });
    ...
    ])('denies a relocated read-only subcommand carrying a disqualifying flag %#', async (buildCommand) => {
        const guard = createDaemonToolGuard();
        await expect(guard(request(buildCommand()))).resolves.toMatchObject({
            allowed: false,
            reason: expect.stringContaining(outsideRepo),
    ...
    it('applies the built-in policy to the monitor tool', async () => {
        const guard = createDaemonToolGuard();
        const monitorCall = (command) => ({
            ...request(command),
            toolName: ToolNames.MONITOR,
        });
        await expect(guard(monitorCall(`git -C ${outsideRepo} reset --hard`))).resolves.toMatchObject({ allowed: false });
    ...
    it.each([
        ...
        () => `cd ${outsideRepo} && nice GIT reset --hard`,
    ])('denies a relocated mutation concealed in an unrecognized program %#', async (buildCommand) => {
        const guard = createDaemonToolGuard();
        await expect(guard(request(buildCommand()))).resolves.toMatchObject({
            allowed: false,
        });
    ...
    ])('denies a dynamic program running Git after a cwd shift %#', async (buildCommand) => {
        const guard = createDaemonToolGuard();
        await expect(guard(request(buildCommand()))).resolves.toMatchObject({
            allowed: false,
        });
    ...
    it.each([
        ...
        () => `GIT_WORK_TREE+=${outsideRepo} && export GIT_WORK_TREE && git reset --hard`,
    ])('denies a mutation after a deferred or unresolvable export %#', async (buildCommand) => {
        const guard = createDaemonToolGuard();
        await expect(guard(request(buildCommand()))).resolves.toMatchObject({
            allowed: false,
        });
    ...
    it('keeps shell-local assignments shell-local', async () => {
        const guard = createDaemonToolGuard();
        await expect(guard(request(`GIT_WORK_TREE=${outsideRepo}; echo done`))).resolves.toEqual({ allowed: true });
        await expect(guard(request('FOO=bar; export FOO; git commit -m x'))).resolves.toEqual({ allowed: true });
    });
    ...
    ])('denies relocated commands carrying command-executing config %#', async (buildCommand) => {
        const guard = createDaemonToolGuard();
        await expect(guard(request(buildCommand()))).resolves.toMatchObject({
            allowed: false,
        });
    ...
    ])('parses relocations after value-taking global options %#', async (buildCommand) => {
        const guard = createDaemonToolGuard();
        await expect(guard(request(buildCommand()))).resolves.toMatchObject({
            allowed: false,
            reason: expect.stringContaining(outsideRepo),
    ...
    it('denies a relocation into a directory whose .git redirects outside', async () => {
        ...
        await writeFile(path.join(decoy, '.git'), `gitdir: ${path.join(outsideRepo, '.git')}\n`);
        const guard = createDaemonToolGuard();
        await expect(guard(request('git -C gitfile-decoy reset --hard'))).resolves.toMatchObject({ allowed: false });
        await expect(guard(request('cd gitfile-decoy && git commit -m x'))).resolves.toMatchObject({ allowed: false });
    });
    ...
    it('resolves cd -P through symlinks before applying ..', async () => {
        ...
        const guard = createDaemonToolGuard();
        await expect(guard(request('cd -P outward-link/.. && git reset --hard'))).resolves.toMatchObject({ allowed: false });
        ...
        await expect(guard(request('cd outward-link/.. && git commit -m x'))).resolves.toEqual({ allowed: true });
    });
    ...
    it('resolves git -C physically through symlinks', async () => {
        ...
        const guard = createDaemonToolGuard();
        await expect(guard(request('git -C physical-link/.. reset --hard'))).resolves.toMatchObject({ allowed: false });
    ...
    it('keeps ordinary redirects allowed', async () => {
        const guard = createDaemonToolGuard();
        await expect(guard(request('git -C nested status > out.txt 2> err.txt'))).resolves.toEqual({ allowed: true });
    });
    ...
    ])('fails closed when the run redefines its own context %#', async (build) => {
        const guard = createDaemonToolGuard();
        await expect(guard(request(build()))).resolves.toMatchObject({
            allowed: false,
        });
    ...
    it('scopes a parenthesized subshell the way the shell does', async () => {
        ...
        await expect(guard(request(`(cd ${outsideRepo}); git commit -m x`))).resolves.toEqual({ allowed: true });
        // ...but a Git command inside them is still judged against it.
        await expect(guard(request(`(cd ${outsideRepo} && git reset --hard)`))).resolves.toMatchObject({ allowed: false });
    });
    ...
    it('denies a planted gitfile at the session root', async () => {
        ...
        await writeFile(path.join(decoyRoot, '.git'), `gitdir: ${path.join(outsideRepo, '.git')}\n`);
        const guard = createDaemonToolGuard();
        const call = {
    ...
    it('keeps a subshell from leaking its environment outward', async () => {
        const guard = createDaemonToolGuard();
        await expect(guard(request(`(export GIT_WORK_TREE=${outsideRepo}); git commit -m x`))).resolves.toEqual({ allowed: true });
        await expect(guard(request(`(export GIT_WORK_TREE=${outsideRepo}; git reset --hard)`))).resolves.toMatchObject({ allowed: false });
    });
    it("leaves a program's own -C flag alone", async () => {
        ...
        await expect(guard(request(`xargs -I{} git -C ${outsideRepo} reset --hard`))).resolves.toMatchObject({ allowed: false });
    });
    ...
    describe('reported execution directory', () => {
        const call = (command, invocationCwd) => ({
            ...request(command),
            ...(invocationCwd === undefined ? {} : { invocationCwd }),
        });
        it('accepts a directory inside the session', async () => {
            const guard = createDaemonToolGuard();
            await expect(guard(call('git commit -m x', insideNested))).resolves.toEqual({ allowed: true });
        });
        it('fails closed on a directory the daemon cannot place', async () => {
            ...
            await expect(guard(call('git commit -m x', outsideRepo))).resolves.toMatchObject({
                allowed: false,
        ...
        it('contains a sub-agent to an in-project agent worktree', async () => {
            ...
            const agentWorktree = path.join(effectiveCwd, '.qwen', 'worktrees', 'agent-abc1234');
            const sibling = path.join(effectiveCwd, '.qwen', 'worktrees', 'agent-def5678');
            await mkdir(path.join(agentWorktree, 'src'), { recursive: true });
            await mkdir(sibling, { recursive: true });
            await writeFile(path.join(agentWorktree, '.git'), `gitdir: ${path.join(outsideRepo, '.git', 'worktrees', 'agent-abc1234')}\n`);
            await mkdir(path.join(outsideRepo, '.git', 'worktrees', 'agent-abc1234'), { recursive: true });
            await writeFile(path.join(outsideRepo, '.git', 'worktrees', 'agent-abc1234', 'gitdir'), `${path.join(agentWorktree, '.git')}\n`);
            const guard = createDaemonToolGuard();
            // Work inside its own worktree is allowed...
            await expect(guard(call('cd src && git commit -m x', agentWorktree))).resolves.toEqual({ allowed: true });
            // ...reaching into a sibling agent's worktree is not, even though both
            // sit inside the session's directory.
            await expect(guard(call(`git -C ${sibling} reset --hard`, agentWorktree))).resolves.toMatchObject({ allowed: false });
        });
        it('contains a sub-agent to the worktree it reports', async () => {
            ...
            try {
                ...
                await expect(guard(inWorktree('cd src && git commit -m x'))).resolves.toEqual({ allowed: true });
                // ...while reaching back into the parent checkout is not.
                await expect(guard(inWorktree(`git -C ${effectiveCwd} reset --hard`))).resolves.toMatchObject({ allowed: false });
            }
            finally {
    ...
    ])('denies a common-form relocation the parser used to miss %#', async (b) => {
        await mkdir(path.join(plainOutsidePath, '.git'), { recursive: true });
        const guard = createDaemonToolGuard();
        await expect(guard(request(b()))).resolves.toMatchObject({
            allowed: false,
    ...
    ])('carries the caller allexport into a same-shell body %#', async (build) => {
        await mkdir(path.join(plainOutsidePath, '.git'), { recursive: true });
        const guard = createDaemonToolGuard();
        await expect(guard(request(build()))).resolves.toMatchObject({
            allowed: false,
    ...
    ])('shares option and definition state with a same-shell body %#', async (b) => {
        await mkdir(path.join(plainOutsidePath, '.git'), { recursive: true });
        const guard = createDaemonToolGuard();
        await expect(guard(request(b()))).resolves.toMatchObject({
            allowed: false,
    ...
    it('does not import an unexported function into a subprocess', async () => {
        ...
        await expect(guard(request(`f() { cd ${plainOutsidePath}; }; bash -c 'git status'`))).resolves.toEqual({ allowed: true });
    ...
    it('rolls back a pipe subshell fully', async () => {
        ...
        await expect(guard(request(`export GIT_WORK_TREE; GIT_WORK_TREE=${plainOutsidePath} | cat; git status`))).resolves.toEqual({ allowed: true });
    });
    ...
    ])('does not let a stale/incompatible shadow mask a relocation %#', async (b) => {
        await mkdir(path.join(plainOutsidePath, '.git'), { recursive: true });
        const guard = createDaemonToolGuard();
        await expect(guard(request(b()))).resolves.toMatchObject({
            allowed: false,
    ...
    ])('does not let a mis-modelled removal drop a live relocating shadow %#', async (build) => {
        await mkdir(path.join(plainOutsidePath, '.git'), { recursive: true });
        const guard = createDaemonToolGuard();
        await expect(guard(request(build()))).resolves.toMatchObject({
            allowed: false,
    ...
    it('matches the ToolNames constants for shell-executing tools', () => {
        expect(SHELL_EXECUTING_TOOL_NAMES).toEqual(new Set([ToolNames.SHELL, ToolNames.MONITOR]));
    });
    it('short-circuits the external provider after a built-in denial', async () => {
        ...
        await expect(guard(request(`git -C ${outsideRepo} reset --hard`))).resolves.toMatchObject({ allowed: false });
    ...
    it.each([
    ...
    ])('preserves external-provider nested executor restrictions only when configured (%s)', async (toolName) => {
        const call = {
            ...request('pwd'),
            toolName,
            arguments: {},
        ...
        await expect(createDaemonToolGuard(vi.fn().mockResolvedValue({ allowed: true }))(call)).resolves.toMatchObject({
            allowed: false,
    ...
    it('matches the ToolNames constants for nested executor tools', () => {
        ...
        expect(unsupported).toEqual(new Set([
            ToolNames.AGENT,
            ToolNames.WORKFLOW,
            ToolNames.CREATE_SUB_SESSION,
            ToolNames.SEND_MESSAGE,
    ...
    it('fails closed without the trusted effective working directory', async () => {
        ...
        await expect(guard(call)).rejects.toThrow('trusted workspace context');
    });
    it('applies the built-in policy to prompt-less shell checks', async () => {
        const guard = createDaemonToolGuard();
        const allowed = request('pwd');
        delete allowed['promptId'];
        await expect(guard(allowed)).resolves.toEqual({ allowed: true });
        const denied = request(`git -C ${outsideRepo} reset --hard`);
        delete denied['promptId'];
        await expect(guard(denied)).resolves.toMatchObject({ allowed: false });
    });
    it('refuses to consult the external provider without a prompt binding', async () => {
        ...
        await expect(guard(call)).resolves.toMatchObject({
            allowed: false,
...
//# sourceMappingURL=daemon-git-worktree-guard.test.js.map