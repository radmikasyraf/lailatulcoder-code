...
import { useHistory, UI_COMPACT_CLEARED_MESSAGE, UI_COMPACT_CLEARED_IMAGE_MESSAGE, } from './useHistoryManager.js';
...
describe('useHistoryManager', () => {
    ...
    it('should not change history if updateHistoryItem is called with a nonexistent ID', () => {
        ...
        expect(debugLoggerMock.debug).toHaveBeenCalledWith('Skipped history update; item 99999 was not found.');
    });
    ...
    describe('compactOldItems', () => {
        function addThoughts(result, count, baseTimestamp) {
        ...
        it('should keep the most recent 20 thought items and drop older ones', () => {
            ...
            expect(result.current.history[0]).toEqual(expect.objectContaining({ text: 'thought-10' }));
            expect(result.current.history[19]).toEqual(expect.objectContaining({ text: 'thought-29' }));
        });
        ...
        it('should clear string resultDisplay on old tool_group items', () => {
            ...
            const tool = result.current.history[0].tools[0];
            expect(tool.resultDisplay).toBe(UI_COMPACT_CLEARED_MESSAGE);
            // Last 20 (newest) should be untouched
            const recentTool = result.current.history[24].tools[0];
        ...
        it('clears image payloads from old tool results', () => {
            ...
            const oldestTool = result.current.history[0].tools[0];
        ...
        it('clears old assistant image payloads while keeping recent images', () => {
            ...
            expect(oldestItem.type === 'gemini' ? oldestItem.images : undefined).toBeUndefined();
            expect(oldestItem.type === 'gemini' ? oldestItem.omittedImageCount : undefined).toBeUndefined();
            const recentItem = result.current.history[24];
            expect(recentItem.type === 'gemini_content' ? recentItem.images : undefined).toHaveLength(1);
            expect(recentItem.type === 'gemini_content'
        ...
        it('compacts old assistant image overflow markers without payloads', () => {
            ...
            expect(oldestItem.type === 'gemini_content'
        ...
        it('should keep the most recent 20 tool_group items un-compacted', () => {
            ...
            for (let i = 0; i < 10; i++) {
                const tool = result.current.history[i].tools[0];
                expect(tool.resultDisplay).toBe(UI_COMPACT_CLEARED_MESSAGE);
            }
            // Last 20 (newest) should be untouched
            for (let i = 10; i < 30; i++) {
        ...
        it('should handle mixed-type history (interleaved thoughts + tool_groups)', () => {
            ...
            const remainingThoughts = result.current.history.filter((item) => item.type === 'gemini_thought' ||
                item.type === 'gemini_thought_content');
            const remainingToolGroups = result.current.history.filter((item) => item.type === 'tool_group');
            ...
            expect(remainingThoughts[0]).toEqual(expect.objectContaining({ text: 'thought-10' }));
            // First 10 tool_groups should have resultDisplay compacted
            for (let i = 0; i < 10; i++) {
                const tool = remainingToolGroups[i]
                    .tools[0];
                expect(tool.resultDisplay).toBe(UI_COMPACT_CLEARED_MESSAGE);
            }
        ...
        it('should not compact non-compactable types (Retry, Notification)', () => {
            ...
            for (let i = 0; i < 30; i++) {
                act(() => {
                    result.current.addItem({
                        type: nonCompactableTypes[i % nonCompactableTypes.length],
                        text: `item-${i}`,
                    }, ts + i);
                });
            }
        ...
        it('should handle tool group with mixed output (some tools real, some cleared)', () => {
            ...
            for (let i = 0; i < 5; i++) {
                const tools = result.current.history[i].tools;
