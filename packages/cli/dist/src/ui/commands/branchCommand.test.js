...
function makeCtx(overrides = {}) {
    ...
    return {
        ...
        session: { stats: {}, sessionShellAllowlist: new Set() },
    };
}
describe('branchCommand', () => {
    ...
    it('rejects when no conversation exists to branch from', async () => {
        const result = await branchCommand.action(makeCtx({ sessionExists: false }), '');
        expect(result).toMatchObject({ type: 'message', messageType: 'error' });
        expect(result.content).toMatch(/No conversation to branch/);
    });
    it('rejects while streaming or awaiting a tool confirmation', async () => {
        ...
        expect(result).toMatchObject({ type: 'message', messageType: 'error' });
        expect(result.content).toMatch(/in progress/);
    });
...
//# sourceMappingURL=branchCommand.test.js.map