...
function readBaseInstallation(bootstrapPath) {
    const manifest = JSON.parse(fs.readFileSync(path.join(path.dirname(bootstrapPath), 'package.json'), 'utf8'));
...
async function validateInstallation(prefix, version) {
    ...
    if (manifest.name !== PACKAGE_NAME || manifest.version !== version) {
        throw new Error(`Installed package did not match ${PACKAGE_NAME}@${version}`);
    }
...
async function smokeTest(prefix) {
    ...
    await execFileAsync(process.execPath, [path.join(packageDir(prefix), 'cli-entry.js'), '--help'], { encoding: 'utf8', env, timeout: 10_000 });
}
async function readActiveVersion(activeFile, expected) {
    try {
        const active = JSON.parse(await fsPromises.readFile(activeFile, 'utf8'));
...
export function prepareManagedNpmUpdate(version, bootstrapPath = process.env['QWEN_CODE_CLI'], updateRoot = process.env['QWEN_CODE_MANAGED_NPM_ROOT'] ??
    ...
    const stagingDir = fs.mkdtempSync(path.join(versionsDir, `.${version}-${process.pid}-`));
    return {
        stagingDir,
...
export async function installManagedNpmUpdate(version, bootstrapPath = process.env['QWEN_CODE_CLI'], updateRoot = process.env['QWEN_CODE_MANAGED_NPM_ROOT'] ??
    path.join(Storage.getGlobalQwenDir(), 'updates', 'npm'), spawnFn = spawn) {
    ...
    for (const key of ['NPM_CONFIG_USERCONFIG', 'npm_config_userconfig']) {
        ...
        if (configured)
            env[key] = path.resolve(configured);
    }
    try {
...
export async function activateManagedNpmUpdate(update, version, bootstrapPath = process.env['QWEN_CODE_CLI']) {
    assertVersion(version);
    const resolvedBootstrapPath = resolveBootstrapPath(bootstrapPath);
    if (path.join(path.dirname(update.launcherRoot), launcherId(resolvedBootstrapPath)) !== update.launcherRoot) {
    ...
    try {
        const base = readBaseInstallation(resolvedBootstrapPath);
        if (base.version !== update.baseVersion ||
            base.ctimeMs !== update.bootstrapCtimeMs) {
            throw new Error('The base LailatulCoder Ai npm installation changed during update');
        }
        ...
        if (activeVersion && semver.gt(activeVersion, version)) {
            try {
                await validateInstallation(path.join(update.launcherRoot, 'versions', activeVersion), activeVersion);
                await cleanupManagedNpmUpdate(update);
                return;
            }
            catch {
        ...
        try {
            ...
            if (!fs.existsSync(update.versionDir))
                throw error;
        ...
        try {
            await fsPromises.writeFile(temporaryActivePath, JSON.stringify({
                version,
                bootstrap: resolvedBootstrapPath,
                baseVersion: base.version,
                bootstrapCtimeMs: base.ctimeMs,
...
export async function cleanupManagedNpmUpdate(update) {
    await fsPromises
...
//# sourceMappingURL=managed-npm-update.js.map