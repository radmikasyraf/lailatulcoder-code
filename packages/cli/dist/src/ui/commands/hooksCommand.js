...
const listCommand = {
    ...
    action: async (context, _args) => {
        ...
        const addHook = (eventName, matcher, hook) => {
            const matcherKey = supportsMatchers(eventName)
                ? matcher
                : '*';
        ...
        const extractName = (config) => config.name ||
        ...
        for (const hook of sessionHooks) {
            ...
            const config = sessionHook.config;
        ...
        for (const [eventName, matcherMap] of hooksByEvent) {
            output += `### ${eventName}\n\n`;
            const useMatchers = supportsMatchers(eventName);
            if (useMatchers) {
...
export const hooksCommand = {
    name: 'hooks',
    get description() {
        return t('Manage LailatulCoder Ai hooks');
    },
    kind: CommandKind.BUILT_IN,
    supportedModes: ['interactive', 'non_interactive', 'acp'],
