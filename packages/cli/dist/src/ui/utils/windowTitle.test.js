...
import { computeWindowTitle, writeTerminalTitle, formatSessionWindowTitle, titleStatusPrefix, } from './windowTitle.js';
...
describe('writeTerminalTitle', () => {
    ...
    it('should write both common terminal title sequences with 80-char padding', () => {
        ...
        expect(write).toHaveBeenCalledWith(`\x1b]0;${padded}\x07\x1b]2;${padded}\x07`);
    });
    ...
    it('should truncate titles longer than 80 characters', () => {
        ...
        expect(write).toHaveBeenCalledWith(`\x1b]0;${expected}\x07\x1b]2;${expected}\x07`);
    });
...
describe('formatSessionWindowTitle', () => {
    ...
    it('should prepend a status prefix when provided', () => {
        expect(formatSessionWindowTitle('my session', 'proj', '◐ ')).toBe('◐ my session');
    });
    it('should prepend status prefix to the fallback title too', () => {
        expect(formatSessionWindowTitle(null, 'my-project', '✳\uFE0E ')).toBe('✳\uFE0E Qwen - my-project');
    });
    ...
    it('should strip control characters from the status prefix', () => {
        expect(formatSessionWindowTitle('sess', 'proj', '\x07\x1b]2;evil\x07')).toBe(']2;evilsess');
    });
});
describe('titleStatusPrefix', () => {
    ...
    it('should return the awaiting-confirmation symbol while waiting', () => {
        expect(titleStatusPrefix(StreamingState.WaitingForConfirmation)).toBe('✳\uFE0E ');
    });
