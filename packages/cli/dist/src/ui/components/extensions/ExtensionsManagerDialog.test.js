...
const mockExtension = (name, isActive = true) => ({
    id: name,
    name,
    version: '1.0.0',
...
const createManager = (o = {}) => {
    ...
    return {
        ...
        isFavorite: vi.fn((name) => (o.favorites ?? []).includes(name)),
        getExtensionScope: vi.fn((name) => o.scopes?.[name] ?? 'user'),
...
const createConfig = (manager, overrides = {}) => ({
...
const createUIState = (extensionsUpdateState = new Map()) => ({ extensionsUpdateState });
...
const renderDialog = (config, opts = {}) => render(_jsx(SettingsContext.Provider, { value: mockSettings, children: _jsx(ShellFocusContext.Provider, { value: true, children: _jsx(UIStateContext.Provider, { value: opts.uiState ?? createUIState(), children: _jsx(KeypressProvider, { kittyProtocolEnabled: false, children: _jsx(ExtensionsManagerDialog, { onClose: opts.onClose ?? vi.fn(), config: config, initialTab: opts.initialTab }) }) }) }) }));
const stripAnsi = (s) => 
...
const renderWide = (config, columns) => {
    let lastFrame = '';
    ...
    const instance = inkRender(_jsx(SettingsContext.Provider, { value: mockSettings, children: _jsx(ShellFocusContext.Provider, { value: true, children: _jsx(UIStateContext.Provider, { value: createUIState(), children: _jsx(KeypressProvider, { kittyProtocolEnabled: false, children: _jsx(ExtensionsManagerDialog, { onClose: vi.fn(), config: config }) }) }) }) }), {
        stdout: stdout,
        stderr: stderr,
        stdin: stdin,
...
describe('ExtensionsManagerDialog (tabbed)', () => {
    ...
    it('caps its width on a wide terminal so the status column is not clipped', async () => {
        ...
        let unmount;
        try {
            const r = renderWide(createConfig(createManager({
    ...
    it('shows discovered plugins on the Discover tab', async () => {
        ...
        const { lastFrame } = renderDialog(createConfig(createManager({ discovered })), { initialTab: EXTENSIONS_TABS.DISCOVER });
        await waitFor(() => {
    ...
    it('opens a CC-style plugin detail with an inline scope selector on Enter', async () => {
        ...
        const { stdin, lastFrame } = renderDialog(createConfig(createManager({ discovered })), { initialTab: EXTENSIONS_TABS.DISCOVER });
        await waitFor(() => {
    ...
    it('windows a long Discover list with a scroll hint and count header', async () => {
        const discovered = Array.from({ length: 15 }, (_, i) => ({
            marketplaceName: 'mkt',
            name: `plugin-${i}`,
    ...
    it('nests extension-bundled MCP servers under their extension on the Installed tab', async () => {
        ...
        const config = createConfig(createManager({ extensions: [ext, mockExtension('beta', true)] }), {
            mcpServers: {
    ...
    it('disables an extension-bundled MCP server individually with Space', async () => {
        ...
        await waitFor(() => {
            ...
            expect(manager.setMcpServerDisabled).toHaveBeenCalledWith('alpha', 'alpha-mcp', true);
        });
    });
    ...
    it('moves a plugin into the Favorites group after favoriting (regroup/reload)', async () => {
        ...
        manager.toggleFavorite = vi.fn((name) => {
    ...
    it('collapses a long installed-plugins list in the marketplace detail', async () => {
        const installed = Array.from({ length: 8 }, (_, i) => mockExtension(`ext-${i}`, true));
        const manager = createManager({
    ...
    it('switches tabs with the Tab key', async () => {
        const config = createConfig(createManager({ extensions: [mockExtension('alpha', true)] }));
        const { stdin, lastFrame } = renderDialog(config);
