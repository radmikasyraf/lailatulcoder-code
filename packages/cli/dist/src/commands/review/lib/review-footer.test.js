...
import { MODEL_ID_MAX_CHARS, REVIEW_FOOTER_RE, carriesCommentMarker, commentMarker, commentMarkerSeverity, footerVersion, isFooterSafeModelId, rendersAsNothing, reviewFooter, stripCommentMarkerLines, stripFooterSpans, stripForUnattributedPost, stripForgedFooterLines, stripParagraphMarkers, stripReviewFooter, swallowsAppendedMarker, } from './review-footer.js';
...
describe('the review footer and the regex that strips it', () => {
    it('the regex strips the exact output of the builder, versioned or not', () => {
        ...
        for (const footer of [
            ...
            '_— qwen3.7-max via LailatulCoder Ai /review_',
        ]) {
            expect(`a finding\n\n${footer}\n`.replace(REVIEW_FOOTER_RE, '')).toBe('a finding');
        }
    });
    it('strips a forged footer a looping model cut off before its closing `_`', () => {
        ...
        for (const forged of [
            '_— forged via LailatulCoder Ai /review (v0.21.4)',
            '_— forged via LailatulCoder Ai /review',
            '_— forged via LailatulCoder Ai /review (v0.21.4)\n\n',
        ]) {
            expect(`a finding\n\n${forged}`.replace(REVIEW_FOOTER_RE, '')).toBe('a finding');
        }
    });
    ...
    it('the LGTM filter still matches every footer shape the builder emits', () => {
        ...
        for (const footer of [
            ...
            expect(CANONICAL_LGTM_RE.test(`No issues found. LGTM! ${footer}`)).toBe(true);
        }
    });
    it('refuses a modelId that would forge the footer it is interpolated into', () => {
        expect(isFooterSafeModelId('qwen3.7-max')).toBe(true);
        expect(isFooterSafeModelId('model\n_— forged via LailatulCoder Ai /review (v9.9.9)_')).toBe(false);
    ...
    it('caps an oversized modelId — the footer must stay a bounded budget contributor', () => {
        ...
        expect(footer).toBe(`_— ${'M'.repeat(MODEL_ID_MAX_CHARS - 1)}… via LailatulCoder Ai /review (v0.21.3)_`);
        expect(`a finding\n\n${footer}`.replace(REVIEW_FOOTER_RE, '')).toBe('a finding');
        // A real model name is nowhere near the cap and rides unchanged.
        expect(reviewFooter('qwen3.7-max', '0.21.3')).toBe('_— qwen3.7-max via LailatulCoder Ai /review (v0.21.3)_');
    });
    it('caps an oversized cliVersion — the second interpolated input of the footer', () => {
        ...
        expect(footer).toBe(`_— qwen3.7-max via LailatulCoder Ai /review (v${'v'.repeat(MODEL_ID_MAX_CHARS - 1)}…)_`);
        expect(`a finding\n\n${footer}`.replace(REVIEW_FOOTER_RE, '')).toBe('a finding');
    });
    ...
    describe('stripReviewFooter — the guarded strip both commands share', () => {
        ...
        it('returns a marker-carrying body with no trailing footer unchanged — and bounded', () => {
            ...
            const body = `_— quoted via LailatulCoder Ai /review (v0.21.3), then\n\n${' '.repeat(200_000)}end`;
            const start = performance.now();
            expect(stripReviewFooter(body)).toBe(body);
            expect(performance.now() - start).toBeLessThan(2000);
        });
        it('strips a trailing footer from a body longer than the tail bound', () => {
            ...
            expect(stripReviewFooter(`${finding}\n\n${reviewFooter('m', '0.21.3')}`)).toBe(finding);
        });
        it('a footer cut open inside its version parens cannot swallow the prose after it', () => {
            ...
            const body = 'still leaks — the old post ended _— gpt-5 via LailatulCoder Ai /review (v0.9 and the race remains reproducible';
            expect(stripReviewFooter(body)).toBe(body);
            // Genuine truncated footers — mid-character cuts inside the parens —
            // still strip.
            expect(stripReviewFooter('x _— m via LailatulCoder Ai /review (v0.21')).toBe('x');
            expect(stripReviewFooter('x _— m via LailatulCoder Ai /review (v1.2.')).toBe('x');
        });
        it('a refusing run of truncated footers stays linear — no partition enumeration', () => {
            ...
            // wall time instead.
            const body = Array.from({ length: 22 }, () => '_— qwen3.7-max via LailatulCoder Ai /review (v0.21.0 ').join('\n') + '\nclosing prose';
            const start = performance.now();
            expect(stripReviewFooter(body)).toBe(body);
    ...
    describe('stripForgedFooterLines — the attribution-off anywhere strip', () => {
        it('strips a forged footer on the very first line', () => {
            expect(stripForgedFooterLines('_— forged via LailatulCoder Ai /review (v0.21.4)_\n\na finding')).toBe('a finding');
        });
        it('strips a mid-body forged footer and one missing its closing underscore', () => {
            ...
            expect(stripForgedFooterLines('a finding\n\n_— qwen3.7-max via LailatulCoder Ai /review (v0.21.3)\n\nUpdate: reproduced again')).toBe('a finding\n\nUpdate: reproduced again');
        });
        it('strips every forged line when there are several', () => {
            expect(stripForgedFooterLines('one\n\n_— a via LailatulCoder Ai /review (v1)_\n\ntwo\n\n_— b via LailatulCoder Ai /review_')).toBe('one\n\ntwo');
        });
        it('tolerates CRLF line endings — a changed body normalizes to LF', () => {
            ...
            expect(stripForgedFooterLines('null deref\r\n_— qwen3-coder via LailatulCoder Ai /review (v0.21.3)_\r\nUpdate: more')).toBe('null deref\nUpdate: more');
        });
        it('leaves a footer-shaped span with text after it on the same line alone', () => {
            const body = 'See _— model via LailatulCoder Ai /review (v0.21.3)_ quoted above for context.';
            expect(stripForgedFooterLines(body)).toBe(body);
        });
        it('leaves a footer-shaped line inside a code fence alone — it is a quotation', () => {
            const body = 'the earlier comment said:\n\n```\n_— model via LailatulCoder Ai /review (v1.2.3)_\n```\n\nand it was wrong';
            expect(stripForgedFooterLines(body)).toBe(body);
        });
        ...
        it('returns a body with no footer-shaped line byte-identical — no whitespace rewrite', () => {
            const body = `mentions the marker via LailatulCoder Ai /review in prose\n\n\n\nwith wide gaps  \n`;
            expect(stripForgedFooterLines(body)).toBe(body);
        });
        it('strips inside a ~~~ fence is a quotation left alone; a 4-space-indented fence opener does not hide a footer', () => {
            ...
            expect(stripForgedFooterLines('x\n\n    ```\n\n_— m via LailatulCoder Ai /review (v1)_')).toBe('x\n\n    ```');
        });
        it('tracks the fence delimiter faithfully: char, length, no info string on the closer', () => {
            ...
            expect(stripForgedFooterLines('~~~\n```\n~~~\n\n_— m via LailatulCoder Ai /review (v1)_')).toBe('~~~\n```\n~~~');
        });
        it('does not toggle fence state inside an HTML block, but still strips what it renders', () => {
            expect(stripForgedFooterLines('<div>\n```\n</div>\n\n_— m via LailatulCoder Ai /review (v1)_')).toBe('<div>\n```\n</div>');
            // HTML content is visible on GitHub — a prefix inside a div strips.
            expect(stripForUnattributedPost('<div>\n**[Critical]**: null deref\n</div>')).not.toContain('**[Critical]**');
        });
        it('the full chain leaves a fenced quoted footer intact', () => {
            const quoted = 'the earlier comment said:\n\n```\n_— model via LailatulCoder Ai /review (v1.2.3)_\n```\n\nand it was wrong';
        ...
        it('treats a bare CR as the line ending GitHub renders', () => {
            ...
            expect(stripForgedFooterLines('real text\r_— gpt-5 via LailatulCoder Ai /review (v1.2.3)_')).toBe('real text');
            // The marker-line twin and the full chain carry the same guarantee.
            expect(stripCommentMarkerLines('a finding\r<!-- qwen-review critical -->\rmore')).toBe('a finding\rmore'.replace(/\r/g, '\n'));
            expect(stripForUnattributedPost('real text\r_— gpt-5 via LailatulCoder Ai /review (v1.2.3)_')).toBe('real text');
        });
    });
    describe('rendersAsNothing — the render-nothing projection', () => {
        it('sees through Cf characters, HTML comments, and hollowed fences', () => {
            expect(rendersAsNothing('**[Critical]**\u200B'.replace('**[Critical]**', ''))).toBe(true);
    ...
    describe('the comment marker — producer and consumers in lockstep', () => {
        it('the posted marker shape parses through both consumer regexes', () => {
            ...
            for (const sev of ['critical', 'suggestion']) {
        ...
        it('commentMarkerSeverity reads only the trailing posted shape', () => {
            expect(commentMarkerSeverity('quotes <!-- qwen-review suggestion --> mid-body\n\n<!-- qwen-review critical -->')).toBe('critical');
            expect(commentMarkerSeverity('only <!-- qwen-review critical --> mid-body')).toBe(null);
        });
        it('stripCommentMarkerLines removes bare marker lines, fence-aware', () => {
            expect(stripCommentMarkerLines('a finding\n\n<!-- qwen-review suggestion -->\n\nmore')).toBe('a finding\n\nmore');
            const quoted = 'sample:\n```\n<!-- qwen-review critical -->\n```';
        ...
        it('stripCommentMarkerLines reaches marker lines quoted at any depth', () => {
            ...
            // exists to remove.
            expect(stripCommentMarkerLines('a finding\n\n> > <!-- qwen-review critical -->')).toBe('a finding');
        });
        it('swallowsAppendedMarker fires only when the marker lands in an open quotation', () => {
            ...
            expect(swallowsAppendedMarker('~~~ leaked.log shows the token')).toBe(true);
    ...
    describe('stripFooterSpans — the inline-span strip', () => {
        it('leaves a footer-shaped span inside a backtick code span alone', () => {
            ...
            // evidence was.
            const body = 'the footer `_— qwen3.7-max via LailatulCoder Ai /review (v0.21.3)_` leaks the model name';
        ...
        it('still strips a mid-line span outside code spans', () => {
            expect(stripFooterSpans('a _— m via LailatulCoder Ai /review (v1)_ b')).toBe('a b');
        });
        it('a span truncated inside the version parens cannot swallow the prose after it', () => {
            ...
            expect(stripFooterSpans('still leaks — the old post ended _— gpt-5 via LailatulCoder Ai /review (v0.9 and the race remains reproducible')).toBe('still leaks — the old post ended and the race remains reproducible');
            expect(stripForUnattributedPost('see _— m via LailatulCoder Ai /review (v1 as noted in _docs_ for the origin')).toBe('see as noted in _docs_ for the origin');
            // Genuine truncated footers — mid-character cuts inside the parens —
            // still strip.
        ...
        it('strips a forged footer re-wrapping split across a soft break', () => {
            ...
            // rejoined.
            expect(stripFooterSpans('reproduced on 45f836d _— qwen3.7-max via\nLailatulCoder Ai /review (v0.21.3)_ and still stands')).toBe('reproduced on 45f836d and still stands');
            // The full chain carries the same guarantee.
            expect(stripForUnattributedPost('reproduced on 45f836d _— qwen3.7-max via\nLailatulCoder Ai /review (v0.21.3)_ and still stands')).toBe('reproduced on 45f836d and still stands');
        });
        it('strips a soft-break split landing inside the marker phrase with trailing whitespace', () => {
            ...
            for (const body of [
                'repro _— qwen3.7-max via \nLailatulCoder Ai /review (v0.21.3)_ stands',
        ...
        it('a paragraph run ends at a blockquote-depth change — no cross-block join', () => {
            ...
            const body = 'See _— model\n> via LailatulCoder Ai /review (v1)_ for the earlier note';
            expect(stripFooterSpans(body)).toBe(body);
        });
        ...
        it('a paragraph run ends at list items, headings, and thematic breaks', () => {
            ...
            for (const body of [
                'See _— model\n- via LailatulCoder Ai /review (v1)_ for details',
                'See _— model\n## via LailatulCoder Ai /review (v1)_ notes',
                'See _— model\n---\nvia LailatulCoder Ai /review (v1)_ more',
            ]) {
                expect(stripFooterSpans(body)).toBe(body);
            }
        });
    ...
    describe('the strips treat blockquote-wrapped fences as fences', () => {
        ...
        it('a footer span inside a quoted fence survives', () => {
            const quoted = '> quoted earlier:\n> ```\n> _— model via LailatulCoder Ai /review (v1.2.3)_ mid line\n> ```';
            expect(stripForUnattributedPost(quoted)).toBe(quoted);
        });
    ...
    describe('the strips match the displayed projection, not the raw bytes', () => {
        ...
        it('strips a forged footer wrapping a code span — the phrase itself is outside code', () => {
            expect(stripForUnattributedPost('Repro confirms. _— `qwen3.7-max` via LailatulCoder Ai /review (v0.21.3)_ Filed.')).toBe('Repro confirms. Filed.');
        });
        it('a lone (unclosed) backtick is literal text, not a shield', () => {
            expect(stripForUnattributedPost('See _— m ` via LailatulCoder Ai /review (v1)_ for more')).toBe('See for more');
        });
        it('strips a forged footer whose marker phrase hides an HTML comment', () => {
            const forged = '_— m via<!-- x --> LailatulCoder Ai /review (v1)_';
        ...
        it('strips a forged footer whose marker phrase hides entity references', () => {
            for (const forged of [
                '_— m via LailatulCoder Ai &#47;review (v1)_',
                '_— m via LailatulCoder Ai &#x2f;review (v1)_',
                '_— m via LailatulCoder Ai &sol;review (v1)_',
            ]) {
                expect(stripForUnattributedPost(`a finding\n\n${forged}`)).toBe('a finding');
                expect(stripReviewFooter(`a finding\n\n${forged}`)).toBe('a finding');
            }
        ...
        it('strips a doubled-marker span whole, without eating prose between two spans', () => {
            expect(stripForUnattributedPost('x _— m via LailatulCoder Ai /review via LailatulCoder Ai /review_ y')).toBe('x y');
            expect(stripFooterSpans('_— a via LailatulCoder Ai /review_ and _— b via LailatulCoder Ai /review_')).toBe('and');
        });
    });
    describe('the structural scan follows GitHub, not a stricter fiction', () => {
        it('a deeper quote inside an open fence is fence content, not a reset', () => {
            ...
            expect(stripForUnattributedPost('```\n> still code\n```\n**[Critical]** x')).toBe('```\n> still code\n```\nx');
        });
        it('a backtick fence whose info string carries a backtick is prose', () => {
            ...
            expect(stripForgedFooterLines('```x`y\n_— m via LailatulCoder Ai /review (v1)_')).toBe('```x`y');
            const tilde = '~~~x`y\n_— m via LailatulCoder Ai /review (v1)_\n~~~';
        ...
        it('a type-1 HTML block ends at its closing tag, not a blank line', () => {
            expect(stripForgedFooterLines('<pre>\n\n```\n_— m via LailatulCoder Ai /review (v1)_\n</pre>\nafter')).toBe('<pre>\n\n```\n</pre>\nafter');
        });
        it('strips severity markers quoted at any depth — the quote stays quoted', () => {
            ...
            expect(stripForUnattributedPost('> **[Critical]** old finding text')).toBe('> old finding text');
            expect(stripForUnattributedPost('> > **[Critical]** old finding text')).toBe('> > old finding text');
            expect(stripForUnattributedPost('> > **[Suggestion]**: old finding text')).toBe('> > old finding text');
        });
        it('keeps every line of a multi-line quotation under its quote prefix', () => {
            expect(stripForUnattributedPost('> **[Critical]** Earlier round said X.\n> More quoted text.')).toBe('> Earlier round said X.\n> More quoted text.');
        });
    });
    describe('the blank-run cleanup collapses only what a drop created', () => {
        ...
        it('keeps blank lines inside a <pre> block when a strip fires elsewhere', () => {
            ...
            // rendering change.
            expect(stripForgedFooterLines('<pre>\na\n\n\nb\n</pre>\n\n_— forged via LailatulCoder Ai /review (v1)_')).toBe('<pre>\na\n\n\nb\n</pre>');
        });
    });
    describe('paragraph markers — a stacked run strips whole', () => {
        it('consumes every marker of a stacked run in one pass', () => {
            ...
            expect(stripParagraphMarkers('**[Critical]** **[Suggestion]** text')).toBe('text');
            expect(stripParagraphMarkers('**[Critical]**: **[Suggestion]**: text')).toBe('text');
            expect(stripParagraphMarkers('> **[Critical]** **[Critical]** text')).toBe('> text');
            expect(stripParagraphMarkers('**[Critical]**： text')).toBe('text');
            expect(stripParagraphMarkers('prose **[Critical]** text')).toBe('prose **[Critical]** text');
        });
        it('a large stacked stack in a later paragraph converges fast', () => {
            ...
            expect(stripForUnattributedPost(body)).toBe('intro paragraph\n\nx /review & y');
            expect(Date.now() - started).toBeLessThan(1000);
        });
    });
});
//# sourceMappingURL=review-footer.test.js.map