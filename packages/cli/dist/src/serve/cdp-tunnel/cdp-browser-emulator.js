...
const TAB_TARGET_ID = 'LailatulCoder-cdp-tab';
const PAGE_TARGET_ID = 'LailatulCoder-cdp-page';
const TAB_SESSION_ID = 'LailatulCoder-cdp-tab-session';
const PAGE_SESSION_ID = 'LailatulCoder-cdp-page-session';
...
export class CdpBrowserEmulator {
    cb;
    tab;
    attachedPageSessions = new Set();
    nextPageSessionId = 1;
    autoAttachActive = false;
    pageSessionDetached = false;
    droppedTabEvents = 0;
    constructor(cb, tab = {}) {
        this.cb = cb;
        this.tab = tab;
        ...
        if (!sessionId) {
            switch (method) {
                ...
                case 'Target.attachToTarget': {
                    ...
                    const attachedSessionId = `LailatulCoder-cdp-page-session-${this.nextPageSessionId++}`;
                ...
                case 'Target.detachFromTarget': {
                    ...
                    else if (typeof attachedSessionId === 'string' &&
                ...
                default:
                    ...
                    this.cb.log?.(`LailatulCoder serve: /cdp unsupported browser-level CDP method: ${method ?? '(none)'}`);
                    return this.cb.reply({ id, result: {} });
            }
        }
    ...
    emitTabEvent(method, params) {
        ...
        if (!this.autoAttachActive && this.attachedPageSessions.size === 0) {
            ...
            if (this.droppedTabEvents === 1 || this.droppedTabEvents % 100 === 0) {
                this.cb.log?.(`LailatulCoder serve: /cdp tab event "${method}" dropped (${this.droppedTabEvents} total) — no active page session (lazy-attach clients are command-only until Target.setAutoAttach)`);
            }
        }
    }
}
