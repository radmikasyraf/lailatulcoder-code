...
describe('<AutoAcceptIndicator />', () => {
    it('renders DEFAULT mode with pause badge and Ask permissions text', () => {
        const { lastFrame } = render(_jsx(AutoAcceptIndicator, { approvalMode: ApprovalMode.DEFAULT }));
        const frame = lastFrame();
        expect(frame).toContain('⏸');
        expect(frame).toContain('Ask permissions');
    });
    it('renders PLAN mode indicator', () => {
        const { lastFrame } = render(_jsx(AutoAcceptIndicator, { approvalMode: ApprovalMode.PLAN }));
    ...
    it('renders AUTO mode indicator with the localized label', () => {
        const { lastFrame } = render(_jsx(AutoAcceptIndicator, { approvalMode: ApprovalMode.AUTO }));
        const frame = lastFrame();
    ...
    it('renders YOLO mode indicator', () => {
        const { lastFrame } = render(_jsx(AutoAcceptIndicator, { approvalMode: ApprovalMode.YOLO }));
        expect(lastFrame()).toContain('YOLO mode');
    });
});
//# sourceMappingURL=AutoAcceptIndicator.test.js.map