...
import { clearSettingsCacheForTesting, loadSettingsCached, } from './settings-cache.js';
import { resetEnvironmentTrackingForTesting, resetHomeEnvBootstrapForTesting, SettingScope, SETTINGS_VERSION, SETTINGS_VERSION_KEY, } from './settings.js';
...
describe('loadSettingsCached', () => {
    ...
    const workspaceSettingsPath = (ws = workspaceDir) => path.join(ws, '.qwen', 'settings.json');
    ...
    beforeEach(() => {
        tmpRoot = fs.realpathSync(fs.mkdtempSync(path.join(os.tmpdir(), 'settings-cache-test-')));
        ...
        vi.stubEnv('QWEN_CODE_SYSTEM_SETTINGS_PATH', path.join(tmpRoot, 'system', 'settings.json'));
        vi.stubEnv('QWEN_CODE_SYSTEM_DEFAULTS_PATH', path.join(tmpRoot, 'system', 'system-defaults.json'));
        resetModuleState();
    });
    afterEach(() => {
        vi.unstubAllEnvs();
    ...
    it('reloads when the user settings file changes', () => {
        ...
        const first = loadSettingsCached(workspaceDir);
        writeJson(userSettingsPath(), versioned({ model: { name: 'after-edit-longer' } }));
        const second = loadSettingsCached(workspaceDir);
    ...
    it('reloads when a workspace settings file appears and again when it is deleted', () => {
        const first = loadSettingsCached(workspaceDir);
        writeJson(workspaceSettingsPath(), versioned({ model: { name: 'from-workspace' } }));
        const second = loadSettingsCached(workspaceDir);
        expect(second).not.toBe(first);
    ...
    it('reloads when a .env file appears closer to the workspace', () => {
        ...
        fs.writeFileSync(path.join(workspaceDir, '.env'), `${TEST_ENV_PREFIX}CLOSER=yes\n`);
        const second = loadSettingsCached(workspaceDir);
        expect(second).not.toBe(first);
    ...
    it('reloads when QWEN_HOME points at a different directory', () => {
        ...
        writeJson(path.join(otherQwenHome, 'settings.json'), versioned({ model: { name: 'home-two' } }));
    ...
    it('does not cache a load failure and recovers once the file is fixed', () => {
        ...
        expect(() => loadSettingsCached(workspaceDir)).toThrow(/not a valid JSON object/);
    ...
    it('keeps independent entries per workspace directory', () => {
        ...
        writeJson(workspaceSettingsPath(), versioned({ model: { name: 'ws-app' } }));
        writeJson(workspaceSettingsPath(otherWorkspace), versioned({ model: { name: 'ws-other' } }));
        const first = loadSettingsCached(workspaceDir);
...
//# sourceMappingURL=settings-cache.test.js.map