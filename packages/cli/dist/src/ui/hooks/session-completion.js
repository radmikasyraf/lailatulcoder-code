...
import { buildSessionRef, SESSION_MENTION_PREFIX, } from './session-mention-ref.js';
...
export function __resetSessionSuggestionCacheForTest() {
    listingCache.clear();
}
async function listSessionsCached(cwd, nowMs) {
 ...
 */
export async function getSessionSuggestions(cwd, pattern, nowMs = Date.now()) {
    ...
    return items
        ...
        .filter((sug) => needle.length === 0
        ? true
        : `${sug.label} ${sug.description ?? ''}`
            .toLowerCase()
            .includes(needle));
}
//# sourceMappingURL=session-completion.js.map