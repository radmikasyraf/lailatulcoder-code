 ...
 */
import { ALL_PROVIDERS } from '@lailatul-coder/lailatul-coder-core';
...
import { buildAuthProviderCatalog, parseAuthProviderInstallRequest, } from '../server/auth-provider-helpers.js';
 ...
 * `X-LailatulCoder-Client-Id` is a syntactic header, not bound to a server-
...
export function registerWorkspaceAuthRoutes(app, deps) {
    const { mutate, deviceFlowRegistry, getSupportedDeviceFlowProviders, sendBridgeError, boundWorkspace, allowPrivateAuthBaseUrl, installAuthProvider, captureGenerationAssertion, } = deps;
    app.post('/workspace/auth/device-flow', mutate({ strict: true }), async (req, res) => {
        ...
        if (!supportedProviders.includes(providerIdRaw)) {
            res.status(400).json({
    ...
    app.get('/workspace/auth/device-flow/:id', mutate({ strict: true }), async (req, res) => {
        ...
        if (!callerIsDeviceFlowInitiator(view, clientId) && isServeDebugMode()) {
            writeStderrLine(`LailatulCoder serve debug: GET /workspace/auth/device-flow/${id} redacted verification fields — caller-clientId mismatch (initiator=${view.initiatorClientId ?? 'anonymous'}, caller=${clientId ?? 'anonymous'})`);
        }
    ...
    app.post('/workspace/auth/provider', mutate({ strict: true }), async (req, res) => {
        ...
        const knownProvider = ALL_PROVIDERS.find((provider) => provider.id === installRequest.providerId);
        if (!knownProvider) {
            res.status(400).json({
        ...
        if (installRequest.protocol) {
            const allowedProtocols = knownProvider.protocolOptions && knownProvider.protocolOptions.length
