...
describe('workspace agents routes', () => {
    ...
    it('lists built-in agents alongside on-disk project agents', async () => {
        ...
        await fs.writeFile(path.join(projectAgentsDir, 'reviewer.md'), `---\nname: reviewer\ndescription: reviews PRs\n---\nyou are a reviewer agent\n`, 'utf8');
        ...
        const reviewerEntry = res.body.agents.find((a) => a.name === 'reviewer');
    ...
    it('round-trips complete frontmatter metadata through create, list, and detail', async () => {
        ...
        const summary = list.body.agents.find((agent) => agent.name === config.name);
    ...
    it('restores selected MCP server secrets before writing an agent', async () => {
        ...
        const onDisk = await fs.readFile(path.join(workspace, QWEN_DIR, 'agents', 'private-agent.md'), 'utf8');
    ...
    it('keeps the existing structured agent generation route unchanged', async () => {
        ...
        expect(generateWorkspaceAgent).toHaveBeenCalledWith('generate an agent', undefined);
    });
    ...
    it('GET /workspace/agents reflects out-of-band agent file changes', async () => {
        ...
        expect(res.status).toBe(200);
        const before = res.body.agents.map((a) => a.name);
    ...
    it('returns a shadowed user agent when GET specifies global scope', async () => {
        ...
        const res = await request(app).get('/workspace/agents/shadowed-agent?scope=global');
        expect(res.status).toBe(200);
    ...
    it('matches frontmatter name case-insensitively', async () => {
        const projectAgentsDir = path.join(workspace, QWEN_DIR, 'agents');
        await fs.mkdir(projectAgentsDir, { recursive: true });
        await fs.writeFile(path.join(projectAgentsDir, 'casey.md'), `---\nname: CaseInsensitive-Agent\ndescription: case insensitive lookup test\n---\nyou are a test agent\n`, 'utf8');
        const bridge = buildBridgeStub();
        const app = buildApp({ bridge, boundWorkspace: workspace });
        const res = await request(app).get('/workspace/agents/caseinsensitive-agent');
        expect(res.status).toBe(200);
    ...
    it('creates a project-level agent and emits agent_changed', async () => {
        ...
        const onDisk = await fs.readFile(path.join(workspace, QWEN_DIR, 'agents', 'tester.md'), 'utf8');
    ...
    it('rejects agent creation when the runtime closes at commit', async () => {
        ...
        expect(res.status).toBe(503);
        expect(res.body.code).toBe('workspace_runtime_unavailable');
        await expect(fs.access(path.join(workspace, QWEN_DIR, 'agents', 'blocked-agent.md'))).rejects.toMatchObject({ code: 'ENOENT' });
    ...
    it('creates a user-level agent when scope=global', async () => {
        ...
        const onDisk = await fs.readFile(path.join(globalDir, 'agents', 'global-helper.md'), 'utf8');
    ...
    it.each(['general-purpose', 'explore'])('rejects 422 invalid_config when create uses builtin agent name %s', async (name) => {
        const bridge = buildBridgeStub();
        const app = buildApp({ bridge, boundWorkspace: workspace });
        const res = await request(app).post('/workspace/agents').send({
            name,
    ...
    it('trims leading/trailing whitespace on the agent name', async () => {
        ...
        const onDisk = await fs.readFile(path.join(workspace, QWEN_DIR, 'agents', 'trimmed-name.md'), 'utf8');
    ...
    it('rejects 400 invalid_scope on repeated ?scope= query', async () => {
        ...
        const res = await request(app).delete('/workspace/agents/some-name?scope=workspace&scope=global');
        expect(res.status).toBe(400);
        expect(res.body.code).toBe('invalid_scope');
    });
    ...
    it('stamps originatorClientId on agent_changed for known clients (create / update / delete)', async () => {
        ...
        expect(agentEvents.map((e) => e.data.change)).toEqual(['created', 'updated', 'deleted']);
    });
    ...
    it('returns 500 agent_delete_partial when one level unlink silently fails', async () => {
        ...
        try {
            ...
            const events = bridge.events;
            const deletedEvents = events.filter((e) => e.type === 'agent_changed' &&
                e.data.change === 'deleted');
            expect(deletedEvents).toHaveLength(1);
            expect((deletedEvents[0]?.data).level).toBe('project');
    ...
    it('DELETE /workspace/agents/:agentType?scope=workspace removes only the project shadow', async () => {
        ...
        const res = await request(app).delete('/workspace/agents/scoped-delete?scope=workspace');
        expect(res.status).toBe(204);
        // Project file gone; user file still exists.
        await expect(fs.access(path.join(workspace, QWEN_DIR, 'agents', 'scoped-delete.md'))).rejects.toMatchObject({ code: 'ENOENT' });
        await expect(fs.access(path.join(globalDir, 'agents', 'scoped-delete.md'))).resolves.toBeUndefined();
        // Exactly one agent_changed event, at project level.
        const events = bridge.events;
        const deleteEvents = events.filter((e) => e.type === 'agent_changed' &&
            e.data.change === 'deleted');
        expect(deleteEvents).toHaveLength(1);
        expect((deleteEvents[0]?.data).level).toBe('project');
    });
    it('POST /workspace/agents/:agentType?scope=global updates the user shadow', async () => {
        ...
        expect(res.body.agent.level).toBe('user');
        expect(res.body.agent.description).toBe('NEW user-level description (longer than ten)');
        // Project-level definition is untouched.
        const projectFile = await fs.readFile(path.join(workspace, QWEN_DIR, 'agents', 'scoped-update.md'), 'utf8');
...
//# sourceMappingURL=workspace-agents.test.js.map