...
export class LlmRewriter {
    ...
    constructor(config, rewriteConfig) {
        ...
        if (rewriteConfig.promptFile) {
            ...
            if (existsSync(filePath)) {
                ...
                debugLogger.info(`Loaded rewrite prompt from file: ${filePath} (${this.prompt.length} chars)`);
            }
            else {
                debugLogger.warn(`Rewrite prompt file not found: ${filePath}, using default`);
     ...
     */
    async rewrite(turnContent, signal) {
        ...
        if (this.contextTurns > 0 && this.outputHistory.length > 0) {
            const contextSlice = this.contextTurns === Infinity
                ? this.outputHistory
        ...
        if (inputText.length < 10)
            return null;
        debugLogger.info(`[REWRITE INPUT] system_prompt_len=${this.prompt.length} input_len=${inputText.length} context_turns=${this.outputHistory.length}\n` +
            `--- INPUT TEXT ---\n${inputText}\n---`);
        try {
            ...
            debugLogger.info(`[REWRITE OUTPUT] len=${rewritten.length}\n` +
                `--- OUTPUT ---\n${rewritten}\n---`);
            ...
            if (this.contextTurns > 0) {
                ...
                if (Number.isFinite(this.contextTurns) &&
            ...
            debugLogger.warn(`LLM rewrite failed, skipping: ${error instanceof Error ? error.message : String(error)}`);
            return null;
        }
    }
}
//# sourceMappingURL=LlmRewriter.js.map