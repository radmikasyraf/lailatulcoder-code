...
import { WAKEUP_MAX_SECONDS, WAKEUP_MIN_SECONDS, clampWakeupSeconds, } from '../services/cronScheduler.js';
import { getErrorMessage } from '../utils/errors.js';
import { todoWorkChainContext } from '../utils/promptIdContext.js';
function formatRequested(delaySeconds) {
...
class LoopWakeupInvocation extends BaseToolInvocation {
    ...
    getDescription() {
        ...
        const prefix = clamped === roundedDelaySeconds
            ? `${clamped}s`
    ...
    async execute() {
        ...
        try {
            ...
            if (scheduler.disabled) {
                const message = 'Loop wakeups are disabled for the rest of this session ' +
            ...
            const { id, scheduledFor, clampedDelaySeconds, wasClamped, replacedId } = scheduler.scheduleWakeup(this.params.delaySeconds, prompt, todoWorkChainContext.getStore());
            ...
            const llmContent = [
                ...
                .filter(Boolean)
                .join('\n');
            const returnDisplay = `Loop wakeup ${id} scheduled for ${scheduledFor}${reason ? ` — ${reason}` : ''}`;
...
//# sourceMappingURL=loop-wakeup.js.map