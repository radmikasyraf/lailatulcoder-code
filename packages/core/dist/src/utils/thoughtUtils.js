...
export function createOpenAIReasoningThoughtPart(text) {
 ...
 */
export function isOpenAIReasoningThoughtPart(part) {
    return Boolean(part[OPENAI_REASONING_THOUGHT_MARKER]);
}
/**
...
export function getThoughtSummary(response) {
    if (response.candidates && response.candidates.length > 0) {
        const candidate = response.candidates[0];
        if (candidate.content &&
            candidate.content.parts &&
            candidate.content.parts.length > 0) {
            const thoughtParts = candidate.content.parts.filter((part) => part.thought);
