...
async function notifyFileImported(options, notification) {
    try {
        await options?.onFileImported?.(notification);
    }
    catch (error) {
        logger.warn(`onFileImported callback failed for ${notification.filePath}: ${hasMessage(error) ? error.message : 'Unknown error'}`);
    }
}
...
function findCodeRegions(content) {
    ...
    let offset = 0;
    function walk(token, baseOffset) {
        ...
        if ('tokens' in token && token.tokens) {
            ...
            for (const child of token.tokens) {
                ...
                if (childIndexInParent === -1) {
                    logger.error(`Could not find child token in parent raw content. Aborting parsing for this branch. Child raw: "${child.raw}"`);
                    break;
                }
 ...
 */
export async function processImports(content, basePath, importState = {
    processedFiles: new Set(),
    maxDepth: 5,
    currentDepth: 0,
}, projectRoot, importFormat = 'tree', options = {}) {
    if (!projectRoot) {
    ...
    if (importFormat === 'flat') {
        ...
        async function processFlat(fileContent, fileBasePath, filePath, depth) {
            ...
            if (depth >= importState.maxDepth) {
                logger.warn(`Maximum import depth (${importState.maxDepth}) reached at ${normalizedPath}. Stopping import processing.`);
                return;
            }
            ...
            for (let i = imports.length - 1; i >= 0; i--) {
                ...
                if (childSeenAt !== undefined && childSeenAt <= depth + 1)
                    continue;
                try {
    ...
    for (const { start, _end, path: importPath } of importsList) {
        ...
        try {
            ...
            const imported = await processImports(fileContent, path.dirname(fullPath), newImportState, projectRoot, importFormat, options);
