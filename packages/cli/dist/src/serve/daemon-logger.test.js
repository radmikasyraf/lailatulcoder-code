...
import { appendFileSync, mkdtempSync, readFileSync, readdirSync, existsSync, promises as fsPromises, writeFileSync, rmSync, realpathSync, lstatSync, lutimesSync, symlinkSync, utimesSync, } from 'node:fs';
import { describe, it, expect, afterEach, beforeEach, vi } from 'vitest';
import { trace, TraceFlags } from '@opentelemetry/api';
import { buildDaemonLogLine, initDaemonLogger, } from './daemon-logger.js';
const openLoggers = new Set();
const TRACE_ID = '1'.repeat(32);
const SPAN_ID = '2'.repeat(16);
function fakeSpan({ recording = true, traceId = TRACE_ID, spanId = SPAN_ID, traceFlags = TraceFlags.SAMPLED, } = {}) {
    return {
...
describe('buildDaemonLogLine', () => {
    ...
    it('renders ctx fields in fixed order', () => {
        ...
        expect(line).toBe('2026-05-26T03:14:15.926Z [ERROR] [DAEMON] ' +
    ...
    it('appends extra ctx keys sorted lexicographically after fixed keys', () => {
        ...
        expect(line).toBe('2026-05-26T03:14:15.926Z [WARN] [DAEMON] sessionId=s alpha=a zeta=1 note\n');
    });
    it('JSON.stringify-quotes values that contain spaces or =', () => {
        ...
        expect(line).toBe('2026-05-26T03:14:15.926Z [INFO] [DAEMON] eq="a=b" weird="has space" hi\n');
    });
    it('appends error stack as indented continuation lines', () => {
        ...
        expect(line).toBe('2026-05-26T03:14:15.926Z [ERROR] [DAEMON] failed\n' +
            '  Error: boom\n' +
            '      at fn (file.ts:1:1)\n' +
            '      at main (file.ts:2:2)\n');
    });
    it('falls back to err.message when stack missing', () => {
        const err = { name: 'Plain', message: 'no stack' };
        const line = buildDaemonLogLine({
            level: 'ERROR',
            message: 'failed',
            now: FIXED,
...
describe('initDaemonLogger opt-out', () => {
    ...
    afterEach(() => {
        if (originalEnv === undefined)
            delete process.env['QWEN_DAEMON_LOG_FILE'];
        else
            process.env['QWEN_DAEMON_LOG_FILE'] = originalEnv;
    });
...
describe('initDaemonLogger file init', () => {
    ...
    it('rejects a pending-byte budget smaller than one capped record', async () => {
        await expect(initDaemonLogger({
            boundWorkspace: '/workspace/foo',
            baseDir: tmp,
            policy: { maxRecordBytes: 256, maxPendingBytes: 255 },
        })).rejects.toThrow('Daemon logger maxRecordBytes must not exceed maxPendingBytes');
    });
    it('derives daemon-scoped daemon-id and creates the stable log file', async () => {
        ...
        expect(readFileSync(logger.getLogPath(), 'utf8')).toMatch(/\[INFO\] \[DAEMON\] runId=0123456789abcdef0123456789abcdef pid=1234 workspace=\/workspace\/foo workspaceHash=[0-9a-f]{8} daemon started/);
        expect(readFileSync(logger.getLogPath(), 'utf8')).not.toContain('trace_id=');
        expect(getActiveSpan).not.toHaveBeenCalled();
        if (process.platform !== 'win32') {
            expect(lstatSync(path.join(tmp, 'daemon')).mode & 0o777).toBe(0o700);
            expect(lstatSync(path.join(tmp, 'daemon', '.stable-writer.lock')).mode & 0o777).toBe(0o700);
            expect(lstatSync(path.join(tmp, 'daemon', 'archive')).mode & 0o777).toBe(0o700);
...
describe('initDaemonLogger raw', () => {
    ...
    it('appends prefixed line without ambient trace or stderr tee', async () => {
        ...
        expect(content).toMatch(/\[WARN\] \[DAEMON\] runId=[0-9a-f]{32} pid=1 \[serve pid=123 cwd=\/x\] child crashed\n/);
        expect(content).toMatch(/\[INFO\] \[DAEMON\] runId=[0-9a-f]{32} pid=1 \[serve pid=123 cwd=\/x\] another\n/);
...
describe('initDaemonLogger info/warn/error', () => {
    ...
    it('info appends to file and tees to stderr', async () => {
        ...
        await logger.flush();
        const content = readFileSync(logger.getLogPath(), 'utf8');
        expect(content).toMatch(/\[INFO\] \[DAEMON\] runId=[0-9a-f]{32} pid=1 route=GET \/ hello\n/);
    ...
    it('captures one active trace context for stderr and file records', async () => {
        ...
        expect(fileRecord).toContain(`[DAEMON] runId=${logger.getStatus().runId} pid=1 ${tracePrefix} route=`);
    ...
    it.each([
        ...
        expect(readFileSync(logger.getLogPath(), 'utf8')).toContain('[WARN] [DAEMON] runId=');
    });
    it.each([
        [
            'active span lookup',
    ...
    it('error appends err.stack as continuation', async () => {
        ...
        await logger.flush();
        const content = readFileSync(logger.getLogPath(), 'utf8');
        expect(content).toMatch(/\[ERROR\] \[DAEMON\] runId=[0-9a-f]{32} pid=1 route=POST \/x route failed\n {2}Error: boom/);
    });
...
describe('initDaemonLogger latest symlink', () => {
    ...
    it('keeps latest on stable while a concurrent logger uses fallback', async () => {
        ...
        expect(realpathSync(path.join(tmp, 'daemon', 'latest'))).toBe(realpathSync(a.getLogPath()));
        expect(b.getStatus()).toMatchObject({ mode: 'fallback', health: 'ok' });
        expect(b.getLogPath()).toContain(path.join('runs', 'run-22222222222222222222222222222222'));
    });
});
describe('initDaemonLogger bounded storage', () => {
    ...
    it('reuses the stable path across restarts and keeps run identity immutable', async () => {
        ...
        expect(content).toContain('runId=11111111111111111111111111111111 pid=11 first run');
        expect(content).toContain('runId=22222222222222222222222222222222 pid=22 second run');
    ...
    it('rotates before exceeding the active limit and keeps a strict archive cap', async () => {
        ...
        const archives = readdirSync(archiveDir).filter((name) => /^daemon-\d{12}-\d{8}T\d{9}Z-[0-9a-f]{8}\.log$/.test(name));
    ...
    it('throttles failed rotation retries while allowing records that still fit', async () => {
        ...
        const logger = await createLogger({
            ...
            fs: {
                ...
                rename: async (...args) => {
                    renameCalls += 1;
                    if (renameCalls === 1)
                        throw new Error('reader blocks rename');
        ...
        expect(readFileSync(path.join(tmp, 'daemon', 'archive', archive), 'utf8')).toContain('small');
        expect(logger.getStatus()).toMatchObject({
    ...
    it('caps pending payload and reports recovered queue loss before the next record', async () => {
        ...
        expect(stderr.filter((line) => line.includes('queue limit reached'))).toHaveLength(1);
        releaseAppend?.();
        await logger.flush();
        ...
        expect(recoveredLine).toContain(`[trace_id=${TRACE_ID} span_id=${SPAN_ID}]`);
    ...
    it('attempts the final queue-loss summary during close', async () => {
        ...
        releaseAppend?.();
        await closing;
        expect(readFileSync(logger.getLogPath(), 'utf8')).toContain('daemon file log records dropped');
    });
        ...
        const fallback = await createLogger({
            ...
            fs: {
                ...
                appendFile: async (...args) => {
                    appendCalls += 1;
                    if (appendCalls === 1)
                        throw new Error('stable boot failed');
                    return fsPromises.appendFile(...args);
                },
            },
            policy: { stableAcquireBudgetMs: 0 },
    ...
    it('uses fallback when releasing failed stable ownership throws', async () => {
        let appendCalls = 0;
        const stderr = [];
        const acquireLock = vi.fn(async (_target, options) => {
        ...
        expect(stderr).toContainEqual(expect.stringContaining('stable log lease release failed; trying fallback'));
    });
    ...
    it('ignores a released stable lease compromise after switching to fallback', async () => {
        let appendCalls = 0;
        const compromiseByLockPath = new Map();
        const acquireLock = vi.fn(async (_target, options) => {
            const lockOptions = options;
            if (lockOptions.lockfilePath && lockOptions.onCompromised) {
                compromiseByLockPath.set(lockOptions.lockfilePath, lockOptions.onCompromised);
            }
            return async () => {
                if (lockOptions.lockfilePath?.endsWith('.stable-writer.lock')) {
                    lockOptions.onCompromised?.(new Error('stable compromise during release'));
                }
            };
        });
        const logger = await createLogger({
        ...
        compromiseByLockPath.get(stableLockPath)?.(new Error('late stable compromise'));
        logger.raw('fallback-remains-writable');
        await logger.flush();
        expect(readFileSync(logger.getLogPath(), 'utf8')).toContain('fallback-remains-writable');
        expect(logger.getStatus().issues).not.toContain('lease_compromised');
        expect(logger.getStatus().droppedRecords).toBe(0);
        const ownerLockPath = path.join(path.dirname(logger.getLogPath()), '.owner.lock');
        compromiseByLockPath.get(ownerLockPath)?.(new Error('active fallback compromise'));
        logger.raw('must-not-reach-file');
        await logger.flush();
        expect(readFileSync(logger.getLogPath(), 'utf8')).not.toContain('must-not-reach-file');
        expect(logger.getStatus()).toMatchObject({
    ...
    it('does not recreate the active file when the lease is compromised during rotation', async () => {
        ...
        const logger = await createLogger({
            ...
            baseDir: tmp,
            stderr: () => { },
            acquireLock: vi.fn(async (_target, options) => {
                compromise = options
                    .onCompromised;
                return async () => { };
            }),
    ...
    it('returns from close at its deadline without releasing ownership early', async () => {
        ...
        const logger = await createLogger({
            ...
            baseDir: tmp,
            stderr: () => { },
            acquireLock: vi.fn(async () => releaseLease),
            fs: {
                ...fsPromises,
                chmod: async () => { },
                appendFile: async (...args) => {
...
describe('initDaemonLogger fallback retention', () => {
    ...
    it('releases fallback ownership when close cannot acquire maintenance', async () => {
        ...
        const acquireLock = vi.fn(async (_target, options) => {
            ...
            if (lockPath.endsWith('.owner.lock'))
                return ownerRelease;
    ...
    it('keeps all live fallback families and only the latest cleanly closed family', async () => {
        ...
        if (process.platform !== 'win32') {
            expect(lstatSync(path.dirname(first.getLogPath())).mode & 0o777).toBe(0o700);
            expect(lstatSync(path.join(path.dirname(first.getLogPath()), '.owner.lock'))
                .mode & 0o777).toBe(0o700);
            expect(lstatSync(first.getLogPath()).mode & 0o777).toBe(0o600);
        }
        await first.close();
        const runsDir = path.join(tmp, 'daemon', 'runs');
        expect(readdirSync(runsDir)).toEqual(expect.arrayContaining([
            'run-22222222222222222222222222222222',
            'run-33333333333333333333333333333333',
        ]));
        await second.close();
        expect(readdirSync(runsDir).filter((name) => name.startsWith('run-'))).toEqual(['run-33333333333333333333333333333333']);
        expect(readFileSync(path.join(runsDir, 'recent-fallback'), 'utf8')).toBe('run-33333333333333333333333333333333\n');
        await stable.close();
    });
    it('reclaims a stale fallback owner instead of treating it as live', async () => {
        ...
        const runsDir = path.join(tmp, 'daemon', 'runs');
        const staleFamily = path.join(runsDir, 'run-22222222222222222222222222222222');
    ...
    it.skipIf(process.platform === 'win32')('does not follow fallback log symlinks when selecting retained history', async () => {
        const stable = await createLogger({
            boundWorkspace: '/w',
            baseDir: tmp,
            runId: '11111111111111111111111111111111',
        ...
        const runsDir = path.join(tmp, 'daemon', 'runs');
        const symlinkFamily = path.join(runsDir, 'run-22222222222222222222222222222222');
        const regularFamily = path.join(runsDir, 'run-33333333333333333333333333333333');
    ...
    it('refuses a new fallback family when cleanup fails', async () => {
        ...
        const extraDir = path.join(tmp, 'daemon', 'runs', 'run-33333333333333333333333333333333');
        ...
        expect(existsSync(path.join(tmp, 'daemon', 'runs', 'run-44444444444444444444444444444444'))).toBe(false);
        await stable.close();
    });
    ...
    it('does not allocate a family after the maintenance lease is compromised', async () => {
        ...
        const logger = await createLogger({
            ...
            acquireLock: vi.fn(async (_target, options) => {
                ...
                options.onCompromised(new Error('maintenance lost'));
        ...
        expect(readdirSync(path.join(tmp, 'daemon', 'runs')).filter((name) => name.startsWith('run-'))).toEqual([]);
        expect(releaseMaintenance).toHaveBeenCalledOnce();
    });
});
//# sourceMappingURL=daemon-logger.test.js.map