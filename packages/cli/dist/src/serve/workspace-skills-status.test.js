...
import { ENV_CORRUPTED_PATH, ENV_WAS_RECOVERED, getUserSettingsPath, } from '../config/settings.js';
...
describe('createWorkspaceSkillsStatusProvider', () => {
    ...
    it('returns a non-initialized error status (and logs) when enumeration fails', async () => {
        vi.spyOn(SkillManager.prototype, 'listSkills').mockRejectedValueOnce(new Error('boom'));
        const provider = createWorkspaceSkillsStatusProvider();
    ...
    it('marks skills disabled in workspace settings', async () => {
        ...
        const workspace = await fsp.mkdtemp(path.join(os.tmpdir(), 'qwen-skills-disabled-'));
        await fsp.mkdir(path.join(workspace, '.qwen'), { recursive: true });
        await fsp.writeFile(path.join(workspace, '.qwen', 'settings.json'), JSON.stringify({
            skills: {
    ...
    it('marks hard-disabled skills with a hard disable reason', async () => {
        ...
        const workspace = await fsp.mkdtemp(path.join(os.tmpdir(), 'qwen-skills-hard-disabled-'));
        await fsp.mkdir(path.join(workspace, '.qwen'), { recursive: true });
        await fsp.writeFile(path.join(workspace, '.qwen', 'settings.json'), JSON.stringify({
            skills: {
                disabled: ['disabled'],
    ...
    it('resolves disablements in safe mode (status matches execution)', async () => {
        ...
        const workspace = await fsp.mkdtemp(path.join(os.tmpdir(), 'qwen-skills-safe-mode-'));
        await fsp.mkdir(path.join(workspace, '.qwen'), { recursive: true });
        await fsp.writeFile(path.join(workspace, '.qwen', 'settings.json'), JSON.stringify({
            skills: {
                disabled: ['blocked'],
    ...
    it('does not read workspace-level disabled skills when untrusted', async () => {
        ...
        const workspace = await fsp.mkdtemp(path.join(os.tmpdir(), 'qwen-skills-untrusted-'));
        await fsp.mkdir(path.join(workspace, '.qwen'), { recursive: true });
        await fsp.writeFile(path.join(workspace, '.qwen', 'settings.json'), JSON.stringify({ skills: { disabled: ['project-skill'] } }));
        const provider = createWorkspaceSkillsStatusProvider({
    ...
    it('does not consume corruption recovery state while reading disabled skills', async () => {
        vi.spyOn(SkillManager.prototype, 'listSkills').mockResolvedValueOnce([]);
        const qwenHome = await fsp.mkdtemp(path.join(os.tmpdir(), 'qwen-skills-settings-'));
        ...
        try {
            ...
            expect(process.env[ENV_CORRUPTED_PATH]).toBe(`${userSettingsPath}.corrupted`);
    ...
    it('hides skills from disabled levels in workspace settings', async () => {
        ...
        const workspace = await fsp.mkdtemp(path.join(os.tmpdir(), 'qwen-skills-disabled-levels-'));
        await fsp.mkdir(path.join(workspace, '.qwen'), { recursive: true });
        await fsp.writeFile(path.join(workspace, '.qwen', 'settings.json'), JSON.stringify({ skills: { disabledLevels: ['bundled'] } }));
        try {
            const provider = createWorkspaceSkillsStatusProvider();
            const status = await provider(workspace);
            expect(status.initialized).toBe(true);
    ...
    it('ignores disabledLevels in safe mode (matches CLI child session)', async () => {
        const workspace = await fsp.mkdtemp(path.join(os.tmpdir(), 'qwen-skills-safe-levels-'));
        await fsp.mkdir(path.join(workspace, '.qwen'), { recursive: true });
        await fsp.writeFile(path.join(workspace, '.qwen', 'settings.json'), JSON.stringify({ skills: { disabledLevels: ['bundled'] } }));
        const saved = process.env['QWEN_CODE_SAFE_MODE'];
        process.env['QWEN_CODE_SAFE_MODE'] = '1';
...
//# sourceMappingURL=workspace-skills-status.test.js.map