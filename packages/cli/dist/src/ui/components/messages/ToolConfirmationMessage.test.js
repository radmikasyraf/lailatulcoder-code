...
const { launchEditorMock } = vi.hoisted(() => ({
    launchEditorMock: vi.fn((_filePath) => Promise.resolve()),
}));
...
describe('ToolConfirmationMessage', () => {
    ...
    it('should not display urls if prompt and url are the same', () => {
        ...
        const { lastFrame } = renderWithProviders(_jsx(ToolConfirmationMessage, { confirmationDetails: confirmationDetails, config: mockConfig, availableTerminalHeight: 30, contentWidth: 80 }));
    ...
    it('renders plain-text info prompts without interpreting Markdown or links', () => {
        const prompt = 'Save "[visible](https://hidden.example/target) **bold** `code` <u>under</u>"';
        const confirmationDetails = {
            type: 'info',
            title: 'Hook confirmation',
            prompt,
        ...
        const { lastFrame } = renderWithProviders(_jsx(ToolConfirmationMessage, { confirmationDetails: confirmationDetails, config: mockConfig, availableTerminalHeight: 30, contentWidth: 160 }));
    ...
    it('renders every line of a short multiline plain-text info prompt', () => {
        ...
        const { lastFrame } = renderWithProviders(_jsx(ToolConfirmationMessage, { confirmationDetails: confirmationDetails, config: mockConfig, availableTerminalHeight: 10, contentWidth: 98 }));
        expect(lastFrame()).toContain('Save this exact content to the bound Mem0 repository memory?');
        expect(lastFrame()).toContain('"LINK [visible label](https://hidden.example/secret-target) BOLD **bold-value** CODE');
    ...
    it('marks overflow in constrained plain-text info prompts', () => {
        ...
        const { lastFrame } = renderWithProviders(_jsx(ToolConfirmationMessage, { confirmationDetails: confirmationDetails, config: mockConfig, availableTerminalHeight: 12, contentWidth: 80 }));
    ...
    it('renders the complete plain-text info prompt when unconstrained', () => {
        ...
        const { lastFrame } = renderWithProviders(_jsx(ToolConfirmationMessage, { confirmationDetails: confirmationDetails, config: mockConfig, contentWidth: 80 }));
        const frame = lastFrame() ?? '';
        expect(frame).toContain('PROMPT_TOP');
    ...
    it('renders classifier fallback guidance and the Default Mode option', async () => {
        ...
        const confirmationDetails = {
            ...
            autoModeFallback: {
                ...
                message: "Auto Mode couldn't classify this action. Switching to Default Mode is recommended.",
            },
            onConfirm,
        };
        const { lastFrame, stdin } = renderWithProviders(_jsx(ToolConfirmationMessage, { confirmationDetails: confirmationDetails, config: mockConfig, availableTerminalHeight: 12, contentWidth: 80 }));
        const frame = lastFrame() ?? '';
        expect(frame).toContain("Auto Mode couldn't classify this action");
        expect(frame).toContain('Switch to Default Mode and allow once (recommended)');
        expect(frame).not.toContain('Always allow');
        stdin.write('\x1b[B');
        stdin.write('\r');
        await vi.waitFor(() => expect(onConfirm).toHaveBeenCalledWith(ToolConfirmationOutcome.ProceedOnceAndSwitchToDefault));
    });
    ...
    it('clamps the multi-line command body to make room for the warning on a tight compactMode layout', () => {
        ...
        const { lastFrame } = renderWithProviders(_jsx(ToolConfirmationMessage, { confirmationDetails: confirmationDetails, config: mockConfig, availableTerminalHeight: 10, contentWidth: 80, compactMode: true }));
        const frame = lastFrame() ?? '';
    ...
    describe('full-plan viewer (#7001)', () => {
        ...
        const planDetails = (onConfirm = vi.fn()) => ({
            type: 'plan',
            title: 'Would you like to proceed?',
            plan,
            onConfirm,
        ...
        it('shows the open-in-editor hint on plan confirmations', () => {
            ...
            const { lastFrame } = renderWithProviders(_jsx(ToolConfirmationMessage, { confirmationDetails: planDetails(), config: mockConfig, availableTerminalHeight: 30, contentWidth: 80 }));
        ...
        it('`o` writes the FULL plan to a temp file and opens the editor without confirming', async () => {
            launchEditorMock.mockClear();
            const onConfirm = vi.fn();
            const { stdin } = renderWithProviders(_jsx(ToolConfirmationMessage, { confirmationDetails: planDetails(onConfirm), config: mockConfig, availableTerminalHeight: 30, contentWidth: 80 }));
            stdin.write('o');
        ...
        it('Ctrl+O does not open the editor', async () => {
            launchEditorMock.mockClear();
            const { stdin } = renderWithProviders(_jsx(ToolConfirmationMessage, { confirmationDetails: planDetails(), config: mockConfig, availableTerminalHeight: 30, contentWidth: 80 }));
        ...
        it('`o` is inert for non-plan confirmations', async () => {
            ...
            const { stdin, lastFrame } = renderWithProviders(_jsx(ToolConfirmationMessage, { confirmationDetails: confirmationDetails, config: mockConfig, availableTerminalHeight: 30, contentWidth: 80 }));
    ...
    describe('with folder trust', () => {
        ...
        describe.each([
            ...
            it('should show "allow always" when folder is trusted', () => {
                ...
                const { lastFrame } = renderWithProviders(_jsx(ToolConfirmationMessage, { confirmationDetails: details, config: mockConfig, availableTerminalHeight: 30, contentWidth: 80 }));
    ...
    describe('external editor option', () => {
        ...
        it('should show "Modify with external editor" when preferredEditor is set', () => {
            ...
            const { lastFrame } = renderWithProviders(_jsx(ToolConfirmationMessage, { confirmationDetails: editConfirmationDetails, config: mockConfig, availableTerminalHeight: 30, contentWidth: 80 }), {
                settings: {
        ...
        it('should NOT show "Modify with external editor" when hideModify is true', () => {
            ...
            const { lastFrame } = renderWithProviders(_jsx(ToolConfirmationMessage, { confirmationDetails: { ...editConfirmationDetails, hideModify: true }, config: mockConfig, availableTerminalHeight: 30, contentWidth: 80 }), {
                settings: {
                    merged: { general: { preferredEditor: 'vscode' } },
                },
            });
        ...
        it('renders edit warnings and honors hideAlwaysAllow on small terminals', () => {
            ...
            const { lastFrame } = renderWithProviders(_jsx(ToolConfirmationMessage, { confirmationDetails: {
                ...
                }, config: mockConfig, availableTerminalHeight: 10, contentWidth: 50 }));
            const frame = lastFrame() ?? '';
        ...
        it('budgets edit warnings using their rendered inner width', () => {
            ...
            const { lastFrame } = renderWithProviders(_jsx(ToolConfirmationMessage, { confirmationDetails: {
                ...
                }, config: mockConfig, availableTerminalHeight: availableTerminalHeight, contentWidth: 50 }));
            const frame = lastFrame() ?? '';
            expect(frame.split(EOL).length).toBeLessThanOrEqual(availableTerminalHeight);
        ...
        it('budgets compact edit warnings at the wrapping boundary', () => {
            ...
            const { lastFrame } = renderWithProviders(_jsx(Box, { width: 50, children: _jsx(ToolConfirmationMessage, { confirmationDetails: {
                        ...editConfirmationDetails,
                        fileDiff: '@@ -1 +1 @@\n-a\n+b',
                        hideAlwaysAllow: true,
                        hideModify: true,
                        warnings: ['x'.repeat(46), 'y'.repeat(46)],
                    }, config: mockConfig, availableTerminalHeight: availableTerminalHeight, contentWidth: 50, compactMode: true }) }));
            const frame = lastFrame() ?? '';
            expect(frame.split(EOL).length).toBeLessThanOrEqual(availableTerminalHeight);
        ...
        it('resolves ordinary IDE edits but skips confirmations that have no IDE diff', async () => {
            ...
            const ordinary = renderWithProviders(_jsx(ToolConfirmationMessage, { confirmationDetails: {
                    ...editConfirmationDetails,
                    onConfirm: ordinaryOnConfirm,
                }, config: ideConfig, availableTerminalHeight: 30, contentWidth: 80 }));
            await vi.waitFor(() => expect(isDiffingEnabled).toHaveBeenCalled());
            ordinary.stdin.write('\r');
            await vi.waitFor(() => expect(resolveDiffFromCli).toHaveBeenCalledWith('/test.txt', 'accepted'));
            ordinary.unmount();
            resolveDiffFromCli.mockClear();
            isDiffingEnabled.mockClear();
            const skippedOnConfirm = vi.fn().mockResolvedValue(undefined);
            const skipped = renderWithProviders(_jsx(ToolConfirmationMessage, { confirmationDetails: {
                    ...editConfirmationDetails,
            ...
            await vi.waitFor(() => expect(skippedOnConfirm).toHaveBeenCalledWith(ToolConfirmationOutcome.ProceedOnce));
    ...
    describe('compactMode', () => {
        ...
        it('renders the command and exec-specific question for exec confirmations', () => {
            ...
            const { lastFrame } = renderWithProviders(_jsx(ToolConfirmationMessage, { confirmationDetails: confirmationDetails, config: mockConfig, availableTerminalHeight: 30, contentWidth: 80, compactMode: true }));
            const frame = lastFrame() ?? '';
        ...
        it('caps multi-line exec body at 5 lines with overflow indicator', () => {
            ...
            const { lastFrame } = renderWithProviders(_jsx(ToolConfirmationMessage, { confirmationDetails: confirmationDetails, config: mockConfig, availableTerminalHeight: 50, contentWidth: 80, compactMode: true }));
            const frame = lastFrame() ?? '';
