 ...
 */
import { SESSION_TITLE_MAX_LENGTH, tryGenerateSessionTitle, } from '@lailatul-coder/lailatul-coder-core';
 ...
 */
function titleFailureMessage(reason) {
    switch (reason) {
        case 'no_fast_model':
            return t('Auto-generating a title requires a fast model. Configure one with `/model --fast <model>`, or pass a name: `/rename <name>`.');
        case 'empty_history':
            return t('No conversation to title yet — send at least one message first.');
        case 'empty_result':
            return t('The fast model returned no usable title. Try `/rename <name>` to set one yourself.');
        case 'aborted':
            return t('Title generation was cancelled.');
        case 'model_error':
            return t('The fast model could not generate a title (rate limit, auth, network error, or unexpected response format). Check debug log or try again.');
        case 'no_client':
...
export const renameCommand = {
    ...
    get description() {
        return t('Rename the current conversation. --auto lets the fast model pick a title.');
    },
    ...
    completion: async (_context, partialArg) => {
        ...
        if (trimmed && '--auto'.startsWith(trimmed)) {
            return [
                {
                    value: '--auto',
                    description: t('Let the fast model generate a sentence-case title from the conversation so far.'),
                },
            ];
        }
        return null;
    ...
    action: async (context, args) => {
        ...
        if (unknownFlag) {
            return {
                type: 'message',
                messageType: 'error',
                content: t('Unknown flag "{{flag}}". Supported: --auto. To use this as a literal name, run `/rename -- {{flag}}`.', { flag: unknownFlag }),
            };
        }
        let name = positional;
        // Track where the title came from so the session picker can dim
        // auto-generated titles; only explicit user text stays 'manual'.
        const titleSource = name ? 'manual' : 'auto';
        if (auto && positional) {
            return {
                type: 'message',
                messageType: 'error',
                content: t('/rename --auto does not take a name. Use `/rename <name>` to set a name yourself.'),
            };
        }
        if (!name) {
            ...
            let outcome;
            try {
                const userDisplayTexts = config.getChatRecordingService()?.getUserDisplayTextsForTitle?.() ??
                    [];
                const lastDisplayText = userDisplayTexts.at(-1)?.trim();
                const titleDisplayTexts = lastDisplayText?.match(/^\/(?:rename|tag)(?:\s+--auto)?$/i)
                    ? userDisplayTexts.slice(0, -1)
                    : userDisplayTexts;
                outcome = await tryGenerateSessionTitle(config, context.abortSignal ?? new AbortController().signal, titleDisplayTexts);
            }
            finally {
                clearInterval(timer);
        ...
        if (chatRecordingService) {
            const ok = await chatRecordingService.recordCustomTitle(name, titleSource);
            if (!ok) {
                return {
                    type: 'message',
                    messageType: 'error',
            ...
            const success = await sessionService.renameSession(sessionId, name, titleSource);
            if (!success) {
                return {
                    type: 'message',
                    messageType: 'error',
