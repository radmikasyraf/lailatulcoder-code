...
import { writeStdoutLineSafe, writeStderrLineSafe, } from '../../utils/stdioHelpers.js';
import { transcriptPaths, listAgentTranscriptFiles, priorSessionDirs, TranscriptsUnavailableError, textOf, } from './lib/transcripts.js';
 ...
 */
function readUsage(file, floorMs, ceilingMs) {
    ...
    for (const line of raw.split('\n')) {
        ...
        for (const rec of parseLineTolerant(line, file)) {
            ...
            if (!Number.isFinite(tsMs) || tsMs < floorMs)
                continue;
            ...
            if (ceilingMs !== undefined && tsMs >= ceilingMs)
                continue;
            ...
            events.push({
                ...
                output: rawPrompt !== null && rawTotal !== null && rawTotal > rawPrompt
                    ? rawTotal - rawPrompt
                    : candidates,
 ...
 */
const AUDIT_BRIEF_RE = /read_file\(file_path="[^"]*reverse-audit--chunk-(\d+)--round-(\d+)--[0-9a-f][^"]*\.brief\.md"\)/g;
/** A role label out of the launch prompt, else the fallback. */
function labelOf(launch, fallback) {
    ...
    let auditChunk = null;
    for (const m of launch.matchAll(AUDIT_BRIEF_RE))
        auditChunk = m;
    if (auditChunk) {
...
function foldEvents(id, label, events) {
    const s = {
        id,
        label,
        calls: 0,
...
function planFloorMs(planPath) {
    ...
    try {
        ...
        throw new Error(`could not read the plan report ${planPath}: ${err.message}`);
    }
    let parsed;
    try {
    ...
    const plan = parsed !== null && typeof parsed === 'object'
        ? parsed
        : undefined;
    if (typeof plan?.['diffLines'] !== 'number' ||
...
export function computeLedger(planPath, env = process.env) {
    ...
    try {
        ...
        throw new Error(`could not read the chat transcript ${chatFile}: ` +
            `${err.message}`);
    }
    let files;
    try {
        ...
        if (err.code === 'ENOENT') {
            ...
            throw new Error(`could not list the subagent transcripts at ${dir}: ` +
                `${err.message}`);
        }
    }
    const agents = [];
    ...
    const readAgentDir = (agentDir, names, ceilingMs, streamFloorMs) => {
        let streams = 0;
        for (const f of names) {
            ...
            if (mtimeMs < (streamFloorMs ?? floorMs))
                continue;
            let read;
            try {
    ...
    const priorDirs = new Map(priorSessionDirs(planPath, env).map((p) => [p.sessionId, p]));
    for (const entry of priorSessionEntries(planPath, env)) {
        const paths = priorDirs.get(entry.sessionId);
        let contributed = 0;
        let events = [];
        try {
            events = readUsage(paths?.chatFile ??
            ...
            Math.max(planMs, entry.atMs), entry.endsAtMs ?? undefined).events;
        ...
        let priorAgentEvents = [];
        if (paths !== undefined) {
            const before = agentEvents.length;
            try {
                contributed += readAgentDir(paths.dir, listAgentTranscriptFiles(paths.dir), 
                ...
                entry.endsAtMs ?? undefined, Math.max(planMs, entry.atMs));
            }
            catch (err) {
                ...
                if (err?.code !== 'ENOENT') {
                    writeStderrLineSafe(`WARNING: could not list the prior session's subagent transcripts at ` +
                        `${paths.dir} (${err?.code ?? err.message}); ` +
    ...
    if (mainEvents.length === 0) {
        throw new Error(`could not read the chat transcript ${chatFile}: no main-loop usage ` +
    ...
    const currentEvents = [...mainEvents, ...agentEvents].filter((e) => !priorEventSet.has(e));
    const spans = [...priorSpans];
    if (currentEvents.length > 0)
        spans.push(spanOf(currentEvents));
    const wallSeconds = spans.reduce((acc, sp) => acc + Math.max(0, Math.round((sp.lastMs - sp.firstMs) / 1000)), 0);
...
export function renderLedger(ledger) {
    ...
    const cachedPct = t.inputTokens > 0 ? Math.round((t.cachedTokens / t.inputTokens) * 100) : 0;
    const lines = [];
    lines.push(`Cost ledger: ${plural(t.calls, 'model call')} · ` +
    ...
    const m = ledger.main;
    lines.push(`  main loop: ${plural(m.calls, 'call')} · ${human(m.inputTokens)} in · ` +
        `${human(m.outputTokens)} out`);
    if (ledger.priorSessions > 0) {
        lines.push(`  resumed run: totals include ${plural(ledger.priorSessions, 'earlier session')} of this review`);
    }
    if (ledger.missingStreams > 0) {
        lines.push(`  ⚠️ ${plural(ledger.missingStreams, 'stream')} could not be read; this ledger is a floor`);
    }
    if (ledger.agents.length > 0) {
        ...
        for (const r of rows.slice(0, 8)) {
            ...
            lines.push(`    ${label}: ${plural(r.calls, 'call')} · ` +
        ...
        if (rows.length > 8) {
            ...
            lines.push(`    …and ${plural(restAgents, 'more agent')} · ` +
                `${human(restIn)} in combined`);
        }
    }
    return lines.join('\n');
}
function runCostLedger(args) {
    // EPIPE arrives two ways when the reader goes away (`LailatulCoder … | head`, a
    ...
    try {
        ...
        if (args.out !== undefined && args.out.length > 0) {
            ...
            try {
                ...
                writeStderrLineSafe(`cost-ledger: could not write ${args.out} — ${err.message}`);
            }
        }
...
export const costLedgerCommand = {
    command: 'cost-ledger',
    describe: "Aggregate this review's model-call cost from the harness's usage records",
    builder: (yargs) => yargs
        .option('plan', {
        type: 'string',
        demandOption: true,
        describe: 'The plan report from Step 1 — its mtime marks the review start',
    })
        .option('out', {
        type: 'string',
