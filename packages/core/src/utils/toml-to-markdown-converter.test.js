...
import { convertTomlToMarkdown, isTomlFormat, } from './toml-to-markdown-converter.js';
import { parse as parseYaml } from './yaml-parser.js';
function readDescriptionFromFrontmatter(markdown) {
...
description = "Multi-line test"`;
    ...
    it('should throw error for invalid TOML', () => {
        ...
        expect(() => convertTomlToMarkdown(invalidToml)).toThrow('Failed to parse TOML');
    });
    it('should throw error if prompt field is missing', () => {
        const tomlWithoutPrompt = 'description = "No prompt here"';
        expect(() => convertTomlToMarkdown(tomlWithoutPrompt)).toThrow('TOML must contain a "prompt" field');
    });
    it('should handle special characters in description', () => {
        const tomlContent = `prompt = "Test prompt"
description = "Command with: special, characters!"`;
        const result = convertTomlToMarkdown(tomlContent);
        expect(readDescriptionFromFrontmatter(result)).toBe('Command with: special, characters!');
    });
...
//# sourceMappingURL=toml-to-markdown-converter.test.js.map