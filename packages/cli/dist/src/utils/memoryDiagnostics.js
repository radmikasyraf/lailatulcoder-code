...
function defaultHeapSnapshotDir() {
    return path.join(os.homedir(), '.lailatulcoder', 'memory-snapshots');
}
...
function cleanupOldHeapSnapshots(outputDir, maxSnapshots) {
    ...
    const snapshots = fs
        ...
        .filter((name) => name.startsWith('lailatul-coder-heap-') && name.endsWith('.heapsnapshot'))
...
export function writeMemoryHeapSnapshot({ outputDir = defaultHeapSnapshotDir(), now = new Date(), writeSnapshot = v8.writeHeapSnapshot, estimateSnapshotBytes: estimateSnapshotBytesOption = estimateHeapSnapshotBytes, getAvailableBytes: getAvailableBytesOption = getAvailableBytes, minFreeBytesAfterSnapshot = 512 * 1024 * 1024, maxSnapshots = 5, rateLimitMs = 60_000, } = {}) {
    ...
    const filePath = path.join(outputDir, `lailatul-coder-heap-${process.pid}-${formatSnapshotTimestamp(now)}.heapsnapshot`);
    let writtenPath;
    try {
...
function buildMemoryInsights(diagnostics) {
    ...
    if (diagnostics.v8.unavailable) {
        ...
        recommendations.push('Re-run /doctor memory after restarting LailatulCoder Ai; if V8 diagnostics remain unavailable, include this report when filing an issue.');
    }
    if (heapIsHigh) {
        signals.push('V8 heap usage is high; the process is close to its configured heap limit.');
        recommendations.push('If the CLI is sluggish or near OOM, restart LailatulCoder Ai to recover memory, then capture a heap snapshot before the next restart to identify retained objects.');
    }
