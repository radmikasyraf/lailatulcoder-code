...
describe('dreamCommand', () => {
    ...
    it('submits a consolidation prompt in interactive mode without eager metadata write', async () => {
        ...
        const result = await dreamCommand.action?.(context, '');
        const expectedTranscriptDir = path.join(new Storage(projectRoot).getProjectDir(), 'chats');
        expect(result).toEqual({
            type: 'submit_prompt',
            content: 'dream prompt',
            onComplete: expect.any(Function),
        });
        expect(buildConsolidationPrompt).toHaveBeenCalledWith(expect.any(String), expectedTranscriptDir);
...
//# sourceMappingURL=dreamCommand.test.js.map