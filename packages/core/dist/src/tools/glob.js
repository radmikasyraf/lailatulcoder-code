...
import { resolveAndValidatePath, formatDisplayPath, resolvePath, isSubpath, unescapePath, } from '../utils/paths.js';
...
const normalizePathForComparison = (p) => process.platform === 'win32' || process.platform === 'darwin'
    ? p.toLowerCase()
    : p;
/**
...
class GlobToolInvocation extends BaseToolInvocation {
     ...
     */
    async globInDirectory(searchDir, pattern, signal, entryLimit) {
        ...
        // FileDiscoveryService reuses the real .gitignore/.lailatulcoderignore semantics
        ...
        const isTraversalIgnored = (entry) => {
            try {
                ...
                const ignorePath = entry.isDirectory()
                    ? relativePath + '/'
                    : relativePath;
                return this.fileService.shouldIgnoreFile(ignorePath, fileFilteringOptions);
            }
            catch (error) {
                ...
                debugLogger.debug(`traversal ignore check failed for ${entry.fullpath()}: ${getErrorMessage(error)}`);
                return false;
            }
        };
        const isAllowedByFileFilters = (entry) => {
            const relativePath = path.relative(projectRoot, entry.fullpath());
            return (this.fileService.filterFiles([relativePath], fileFilteringOptions)
                .length > 0);
        };
    ...
    async execute(signal) {
        try {
            ...
            if (this.params.path) {
                ...
                const searchDirAbs = resolveAndValidatePath(this.config, this.params.path, { allowExternalPaths: true });
            ...
            for (const searchDir of searchDirs) {
                const remainingEntries = MAX_GLOB_COLLECTED_ENTRIES - allFilteredEntries.length;
                if (remainingEntries <= 0) {
                    hitCollectionLimit = true;
                    break;
                }
                const { entries, hitLimit } = await this.globInDirectory(searchDir, pattern, signal, remainingEntries);
