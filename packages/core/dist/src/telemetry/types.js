...
export class UserPromptEvent {
    ...
    model;
    constructor(prompt_length, prompt_Id, auth_type, prompt, model) {
...
export class ApiCancelEvent {
    ...
    loop_wakeups_cancelled;
    constructor(model, prompt_id, auth_type, loop_wakeups_cancelled) {
...
export class RipgrepRuntimeRecoveryEvent {
    'event.name';
    'event.timestamp';
    selection_mode;
    retry_triggered;
...
(function (LoopType) {
    ...
    LoopType["SHELL_COMMAND_STAGNATION"] = "shell_command_stagnation";
    /** Same (tool, args) pair appears N times across the entire turn, not necessarily consecutively. */
    LoopType["GLOBAL_TOOL_CALL_DUPLICATE"] = "global_tool_call_duplicate";
    /** Two tools alternating in a fixed pattern (A B A B A B ...). */
    LoopType["ALTERNATING_TOOL_CALL_PATTERN"] = "alternating_tool_call_pattern";
    /** Total tool calls in a single turn exceeded the always-on hard cap, regardless of pattern. */
    LoopType["TURN_TOOL_CALL_CAP"] = "turn_tool_call_cap";
    /** The same tool repeatedly failed schema validation with fresh tool-call ids. */
    LoopType["INVALID_TOOL_PARAMS_STAGNATION"] = "invalid_tool_params_stagnation";
    /** The same tool execution failure continued after a corrective reminder. */
    LoopType["REPEATED_TOOL_EXECUTION_FAILURE"] = "repeated_tool_execution_failure";
})(LoopType || (LoopType = {}));
...
export class RepeatedToolFailureGuardEvent {
    'event.name';
    'event.timestamp';
    prompt_id;
    route;
...
export function makeChatCompressionEvent({ tokens_before, tokens_after, compression_input_token_count, compression_output_token_count, cache_sharing_attempted, cache_sharing_used, }) {
    return {
...
export class ProtocolTagSanitizedEvent {
    'event.name';
    'event.timestamp';
    model;
    prompt_id;
 ...
 * prompt_id.
 */
export class ApiRetryEvent {
    'event.name';
    'event.timestamp'; // ISO 8601
    model;
    prompt_id;
    attempt_number; // 1-based monotonic counter (matches ALS retryContext.attempt)
    error_type;
    error_message;
    status_code;
    retry_delay_ms;
...
export class ToolResultPersistedEvent {
    eventName = 'tool_result_persisted';
    'event.timestamp' = new Date().toISOString();
    'event.name';
    tool_name;
    bytes_written;
...
export class SkillLaunchEvent {
    ...
    success;
    prompt_id;
    constructor(skill_name, success, prompt_id = '') {
...
export class WorkflowKeywordEvent {
    'event.name';
    'event.timestamp';
    constructor() {
...
export class MemoryRecallDeliveryEvent {
    'event.name';
    'event.timestamp';
    phase;
    delivery_point;
