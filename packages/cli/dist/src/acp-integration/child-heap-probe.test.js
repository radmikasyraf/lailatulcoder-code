...
function space(name, size, used = size) {
    return {
        space_name: name,
        space_size: size,
...
const heap = (total) => ({ total_heap_size: total });
function snapshotOf(probe) {
    const snapshot = probe.snapshot();
    if (!snapshot)
        throw new Error('expected at least one successful sample');
    return snapshot;
}
...
describe('startChildHeapProbe', () => {
    ...
    it('keeps the last good values when V8 readings throw', () => {
        ...
        const probe = startChildHeapProbe({
            heapSpaceStatistics: () => {
                if (failing)
                    throw new Error('restricted container');
    ...
    it('updates the live set and GC figures from major-GC entries', () => {
        ...
        const gcEntry = (kind, duration) => ({ detail: { kind }, duration });
        const probe = startChildHeapProbe({
            heapSpaceStatistics: () => [space('old_space', 8_000, 7_000)],
    ...
    it('tracks a real allocation upward against the real V8', () => {
        ...
        try {
            ...
            for (let i = 0; i < 12; i++)
                held.push(new Array(2_000_000).fill(i));
...
//# sourceMappingURL=child-heap-probe.test.js.map