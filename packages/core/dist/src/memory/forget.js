...
import { buildAutoMemoryEntrySearchText, getAutoMemoryBodyHeading, parseAutoMemoryEntries, renderAutoMemoryBody, } from './entries.js';
import { rebuildManagedAutoMemoryIndex, rebuildUserAutoMemoryIndex, } from './indexer.js';
import { getAutoMemoryMetadataPath, isAutoMemPath, isUserAutoMemPath, } from './paths.js';
import { scanAutoMemoryTopicDocuments, scanUserAutoMemoryTopicDocuments, } from './scan.js';
import { ensureAutoMemoryScaffold } from './store.js';
const debugLogger = createDebugLogger('MEMORY_FORGET');
const FORGET_SELECTION_RESPONSE_SCHEMA = {
    type: 'object',
    properties: {
...
async function listIndexedForgetCandidates(projectRoot, abortSignal) {
    ...
    for (const { docs, storageScope } of [
        { docs: userDocs, storageScope: 'user' },
        { docs: projectDocs, storageScope: 'project' },
    ]) {
        abortSignal?.throwIfAborted();
        for (const doc of docs) {
            ...
            for (let i = 0; i < entries.length; i++) {
                ...
                const entryId = entries.length === 1 ? doc.relativePath : `${doc.relativePath}:${i}`;
                candidates.push({
...
function buildForgetSelectionPrompt(query, candidates, limit) {
    return [
...
async function selectByModel(candidates, query, config, limit, callerAbortSignal) {
    const response = await runSideQuery(config, {
...
function selectByHeuristic(candidates, query, limit) {
    const normalizedQuery = query.replace(/\s+/g, ' ').trim();
    const queryLower = normalizedQuery.toLowerCase();
    const matches = candidates
        .filter((candidate) => buildAutoMemoryEntrySearchText(candidate).includes(queryLower))
        .slice(0, limit)
        .map(({ topic, summary, filePath, entryIndex }) => ({
        topic,
        summary,
...
export async function selectManagedAutoMemoryForgetCandidates(projectRoot, query, options = {}) {
    options.abortSignal?.throwIfAborted();
    const limit = options.limit ?? 5;
    const candidates = await listIndexedForgetCandidates(projectRoot, options.abortSignal);
    if (candidates.length === 0) {
        return { matches: [], strategy: 'none' };
    }
    if (options.config) {
        try {
            return await selectByModel(candidates, query, options.config, limit, options.abortSignal);
        }
        catch (err) {
            if (options.abortSignal?.aborted)
                throw err;
            debugLogger.warn('Managed auto-memory forget model selection failed; falling back to heuristic:', err);
        }
    }
    options.abortSignal?.throwIfAborted();
    return selectByHeuristic(candidates, query, limit);
}
async function bumpMetadata(projectRoot, now) {
    try {
        const content = await fs.readFile(getAutoMemoryMetadataPath(projectRoot), 'utf-8');
        const metadata = JSON.parse(content);
        metadata.updatedAt = now.toISOString();
        await atomicWriteFile(getAutoMemoryMetadataPath(projectRoot), `${JSON.stringify(metadata, null, 2)}\n`, { encoding: 'utf-8' });
    }
    catch {
        // Best-effort metadata bump.
    }
}
export async function forgetManagedAutoMemoryMatches(projectRoot, matches, now = new Date(), options = {}) {
    options.abortSignal?.throwIfAborted();
    if (matches.length === 0) {
        return {
            query: '',
    ...
    if (matches.some((match) => classifyMemoryScope(match.filePath, projectRoot) === 'project')) {
    ...
    for (const [filePath, fileMatches] of matchesByFile) {
        try {
            ...
            for (const match of fileMatches) {
                if (Number.isInteger(match.entryIndex) &&
                    match.entryIndex >= 0 &&
            ...
            if (matchesByIndex.size > 0) {
                ...
                const remainingBySummary = new Map();
                for (const match of fileMatches) {
                ...
                removedFileEntries = fileMatches.slice(0, allEntries.length - kept.length);
            }
            ...
            if (options.abortSignal?.aborted)
                throw err;
            debugLogger.warn('Managed auto-memory forget skipped file after apply error:', { filePath }, err);
        }
    }
    if (touchedScopes.has('project')) {
        try {
            ...
            if (options.abortSignal?.aborted)
                throw err;
            debugLogger.warn('Managed auto-memory forget failed to rebuild project index:', err);
        }
    }
    if (touchedScopes.has('user')) {
        try {
            ...
            if (options.abortSignal?.aborted)
                throw err;
            debugLogger.warn('Managed auto-memory forget failed to rebuild user index:', err);
        }
    }
    return {
        query: '',
        ...
        systemMessage: removedEntries.length > 0
...
export async function forgetManagedAutoMemoryEntries(projectRoot, query, options = {}, now = new Date()) {
    options.abortSignal?.throwIfAborted();
    ...
    const selection = await selectManagedAutoMemoryForgetCandidates(projectRoot, trimmedQuery, { ...options, limit: Number.MAX_SAFE_INTEGER });
    const result = await forgetManagedAutoMemoryMatches(projectRoot, selection.matches, now, { abortSignal: options.abortSignal });
    return { ...result, query: trimmedQuery };
}
function classifyMemoryScope(filePath, projectRoot) {
...
function sortTouchedScopes(scopes) {
    const unique = new Set(scopes);
    return ['user', 'project'].filter((scope) => unique.has(scope));
}
