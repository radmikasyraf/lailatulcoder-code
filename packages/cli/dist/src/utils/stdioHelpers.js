 ...
 * CLI commands (like `LailatulCoder extensions list`) where the output IS the user-facing
 * result, not debug logging.
 *
 * For debug/diagnostic logging, use `createDebugLogger()` from @lailatul-coder/lailatul-coder-core.
 */
/**
 ...
 */
export const writeStdoutLine = (message) => {
 ...
 * Avoids double newlines if the message already ends with one.
 */
export const writeStderrLine = (message) => {
 ...
 * going away (`LailatulCoder … | head`) must not fail the command.
 */
export const writeStdoutLineSafe = (message) => {
    try {
 ...
 * the reader goes away (`LailatulCoder … | head`) or a daemon redirects its stderr. Most
 ...
 */
export const writeStderrLineSafe = (message) => {
    try {
...
//# sourceMappingURL=stdioHelpers.js.map