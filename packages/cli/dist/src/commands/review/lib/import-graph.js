 ...
 */
export function scanImportSpecifiers(source) {
    const out = [];
    const seen = new Set();
    const push = (spec) => {
...
function candidatesFor(base) {
    ...
    for (const [re, ts] of EXT_MAP) {
        if (re.test(base))
            out.push(base.replace(re, ts));
    }
    if (!/\.[a-z]+$/i.test(base)) {
        for (const ext of EXT_WALK)
            out.push(`${base}${ext}`);
        for (const ext of EXT_WALK)
 ...
 */
export function resolveSpecifier(fromFile, spec, membership, packages = []) {
    ...
    for (const pkg of packages) {
        ...
        if (spec.startsWith(`${pkg.name}/`)) {
            ...
            for (const base2 of [
                ...
                for (const c of candidatesFor(base2))
                    if (membership.has(c))
                        return c;
            }
            return null;
 ...
 */
export function discoverWorkspacePackages(files, readJson) {
    ...
    const out = new Map(); // dir → name, deduped
 ...
 */
export function dependentsOfChanged(changed, candidates, read, packages = []) {
    const out = new Map();
