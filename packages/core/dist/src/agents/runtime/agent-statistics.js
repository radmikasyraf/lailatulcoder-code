...
export class AgentStatistics {
    ...
    formatCompact(taskDesc, now = Date.now()) {
        ...
        const sr = stats.totalToolCalls > 0
            ? (stats.successRate ??
        ...
        if (typeof stats.totalTokens === 'number') {
            ...
            lines.push(`● Tokens: ${stats.totalTokens.toLocaleString()}${parts.length ? ` (${parts.join(', ')})` : ''}`);
        }
        return lines.join('\n');
    }
    formatDetailed(taskDesc, now = Date.now()) {
        ...
        lines.push(`● Duration: ${this.fmtDuration(stats.totalDurationMs)} | ● Rounds: ${stats.rounds}`);
        // Quality indicator
        ...
        if (d < 10_000)
            speed = 'Fast completion - under 10 seconds';
        else if (d < 60_000)
            speed = 'Good speed - under a minute';
        else if (d < 300_000)
            speed = 'Moderate duration - a few minutes';
        lines.push(`● Speed: ${speed}`);
        lines.push(`● Tools: ${stats.totalToolCalls} calls, ${sr.toFixed(1)}% success (${stats.successfulToolCalls} ok, ${stats.failedToolCalls} failed)`);
        if (typeof stats.totalTokens === 'number') {
            const parts = [
                `in ${stats.inputTokens ?? 0}`,
                `out ${stats.outputTokens ?? 0}`,
            ];
            lines.push(`● Tokens: ${stats.totalTokens.toLocaleString()} (${parts.join(', ')})`);
        }
        if (stats.toolUsage && stats.toolUsage.length) {
            ...
            for (const t of sorted) {
                const avg = typeof t.averageDurationMs === 'number'
                    ? `, avg ${this.fmtDuration(Math.round(t.averageDurationMs))}`
                    : '';
                lines.push(` - ${t.name}: ${t.count} calls (${t.success} ok, ${t.failure} fail${avg}${t.lastError ? `, last error: ${t.lastError}` : ''})`);
            }
        }
    ...
    generatePerformanceTips(stats) {
        ...
        // Network failures
        const isNetworkTool = (name) => /web|fetch|search/i.test(name);
        const hadNetworkFailure = (stats.toolUsage || []).some((t) => isNetworkTool(t.name) &&
            t.lastError &&
            /timeout|network/i.test(t.lastError));
        if (hadNetworkFailure)
            tips.push('Network operations had failures - consider increasing timeout or checking connectivity');
        // Slow tools
        const slow = (stats.toolUsage || [])
            .filter((t) => (t.averageDurationMs ?? 0) > 10_000)
            .sort((a, b) => (b.averageDurationMs ?? 0) - (a.averageDurationMs ?? 0));
        if (slow.length)
            tips.push(`Consider optimizing ${slow[0].name} operations (avg ${this.fmtDuration(Math.round(slow[0].averageDurationMs))})`);
        return tips;
    }
}
//# sourceMappingURL=agent-statistics.js.map