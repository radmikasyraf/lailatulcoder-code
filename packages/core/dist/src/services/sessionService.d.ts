...
import { type SelectiveSessionRestoreOptions, type SessionLiveRestoreProjection, type SessionRestoreProjection } from './session-transcript-reader.js';
import { SessionWriterLease, type SessionWriterProcessKind } from './session-writer-lease.js';
export { buildApiHistoryFromConversation, type BuildApiHistoryOptions, } from './session-api-history.js';
import { type ResumeTokenCounts } from './session-resume-token-counts.js';
export { getResumePromptTokenCount, getResumeTokenCounts, type ResumeTokenCounts, } from './session-resume-token-counts.js';
export declare class BranchPointInvalidError extends Error {
...
export declare class SessionIdCaseConflictError extends Error {
    readonly sessionId: string;
    readonly candidateSessionId?: string | undefined;
    readonly reason: 'case_conflict' | 'unreadable_transcript';
    readonly name = "SessionIdCaseConflictError";
    constructor(sessionId: string, candidateSessionId?: string | undefined, reason?: 'case_conflict' | 'unreadable_transcript');
}
/**
...
export declare class SessionService {
    ...
    acquireSessionWriterLease(sessionId: string, options: {
    ...
    getWorktreeSessionPathForArchiveState(sessionId: string, state: SessionArchiveState): string;
    ...
    readCreationMetadataIfReadable(sessionId: string, state: SessionArchiveState): Promise<{
        parentSessionId?: string;
        sourceType?: string;
        sourceId?: string;
    } | undefined>;
    private readCreationMetadataInternal;
    ...
    readRestoreProjection(sessionId: string, options: SelectiveSessionRestoreOptions): Promise<SessionRestoreProjection | undefined>;
    readLiveRestoreProjection(sessionId: string, options: SelectiveSessionRestoreOptions): Promise<SessionLiveRestoreProjection | undefined>;
    /**
     ...
     */
    loadArchivedSession(sessionId: string, options: {
        maxBytes: number;
    ...
    archiveSessions(sessionIds: string[], options?: ArchiveSessionsOptions): Promise<ArchiveSessionsResult>;
    unarchiveSessions(sessionIds: string[], options?: UnarchiveSessionsOptions): Promise<UnarchiveSessionsResult>;
    /**
     ...
     */
    renameSession(sessionId: string, title: string, titleSource?: TitleSource, archiveState?: SessionArchiveState): Promise<boolean>;
    /**
     ...
     * `sessionExistsInAnyState()` for archive-aware lookups.
     */
    forkSession(sourceSessionId: string, newSessionId: string, options?: ForkSessionOptions): Promise<{
        filePath: string;
        copiedCount: number;
    }>;
    /**
     ...
     * `sessionExistsInAnyState()` for archive-aware lookups.
     */
    sessionExists(sessionId: string, options?: {
        signal?: AbortSignal;
    }): Promise<boolean>;
 ...
 */
export declare function replayUiTelemetryFromConversation(conversation: ConversationRecord, sessionId?: string): ResumeTokenCounts | undefined;
export declare function computeUniqueBranchTitle(baseName: string, sessionService: SessionService): Promise<string>;
