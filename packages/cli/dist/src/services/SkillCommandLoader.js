 ...
 */
import { createDebugLogger, appendToLastTextPart, buildSkillLlmContent, applySkillAllowedTools, recordAutoSkillUsage, } from '@lailatul-coder/lailatul-coder-core';
import { dirname } from 'node:path';
import { writeSkillArgs, clearSkillArgs, staleArgsWarning, skillArgsNote, skillArgsPath, } from './skill-args-file.js';
import { CommandKind } from '../ui/commands/types.js';
import { t } from '../i18n/index.js';
const debugLogger = createDebugLogger('SKILL_COMMAND_LOADER');
export async function recordAutoSkillCommandUsage(config, command) {
    ...
    try {
        ...
        debugLogger.warn(`Failed to record auto-skill command usage: ${error instanceof Error ? error.message : String(error)}`);
    }
}
/**
...
export class SkillCommandLoader {
    ...
    async loadCommands(_signal) {
        ...
        try {
            ...
            const disabled = this.config?.getDisabledSkillNames() ?? new Set();
            const visibleSkills = allSkills.filter((skill) => !disabled.has(skill.name.toLowerCase()));
            const nonUserInvocableCount = visibleSkills.filter((skill) => skill.userInvocable === false).length;
            debugLogger.debug(`Loaded ${userSkills.length} user + ${projectSkills.length} project + ${extensionSkills.length} extension skill(s) as slash commands; ${allSkills.length - visibleSkills.length} hidden by skills.disabled; ${nonUserInvocableCount} marked non-user-invocable`);
            return visibleSkills.map((skill) => {
                ...
                return {
                    ...
                    action: async (context, _args) => {
                        ...
                        const body = buildSkillLlmContent(dirname(skill.filePath), skill.body);
                        ...
                        let content;
                        if (rawArgs) {
                            content = appendToLastTextPart([{ text: body }], context.invocation.raw +
...
//# sourceMappingURL=SkillCommandLoader.js.map