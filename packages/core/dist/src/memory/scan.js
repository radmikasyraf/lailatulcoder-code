...
import { AUTO_MEMORY_INDEX_FILENAME, getAutoMemoryRoot, getTeamAutoMemoryRoot, getUserAutoMemoryRoot, } from './paths.js';
...
export function parseAutoMemoryTopicDocument(filePath, content, mtimeMs = 0, relativePath = path.basename(filePath)) {
    ...
    if (!rawType || !AUTO_MEMORY_TYPES.includes(rawType)) {
        return null;
    }
    return {
        type: rawType,
        ...
        title: parseFrontmatterValue(frontmatter, 'name') ??
...
async function scanAutoMemoryDocumentsFromRoot(root, opts = {}) {
    const relativePaths = await listMarkdownFiles(root);
    const docs = await Promise.all(relativePaths.map(async (relativePath) => {
        ...
        try {
            ...
            return parseAutoMemoryTopicDocument(filePath, content, stats.mtimeMs, relativePath);
        }
        catch (error) {
            ...
            debugLogger.debug(`skipping unreadable memory file ${relativePath}`, error);
            return null;
        }
    }));
    const valid = docs
    ...
    const ordered = opts.deterministic
        ? valid.sort((a, b) => a.relativePath < b.relativePath
            ? -1
            : a.relativePath > b.relativePath
                ? 1
                : 0)
        : valid.sort((a, b) => b.mtimeMs - a.mtimeMs || a.filename.localeCompare(b.filename));
    return opts.uncapped ? ordered : ordered.slice(0, MAX_SCANNED_MEMORY_FILES);
}
export async function scanAutoMemoryTopicDocuments(projectRoot) {
    return scanAutoMemoryDocumentsFromRoot(getAutoMemoryRoot(projectRoot));
}
export async function scanAllAutoMemoryTopicDocuments(projectRoot) {
 ...
 */
export async function scanUserAutoMemoryTopicDocuments() {
 ...
 */
export async function scanTeamAutoMemoryTopicDocuments(projectRoot) {
