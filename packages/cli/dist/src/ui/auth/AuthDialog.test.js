...
const createMockUIState = (overrides = {}) => {
    ...
    return {
        ...
        auth: {
            ...
            pendingAuthType: overrides.auth?.pendingAuthType ?? overrides.pendingAuthType,
        },
    };
};
const createMockUIActions = (overrides = {}) => {
    ...
    for (const key of Object.keys(topLevelOverrides)) {
...
const renderAuthDialog = (settings, uiStateOverrides = {}, uiActionsOverrides = {}, configAuthType = undefined, configApiKey = undefined) => {
    ...
    return renderWithProviders(_jsx(UIStateContext.Provider, { value: uiState, children: _jsx(UIActionsContext.Provider, { value: uiActions, children: _jsx(AuthDialog, {}) }) }), { settings, config: mockConfig });
};
/**
 ...
 */
const typeText = async (stdin, text) => {
...
const expectSelectedOption = (frame, label) => {
    expect(frame).toMatch(new RegExp(`›\\s*(?:\\d+\\.\\s*)?${escapeRegExp(label)}`));
};
const waitForSelectedOption = async (lastFrame, label) => {
    await vi.waitFor(() => {
        expectSelectedOption(lastFrame(), label);
    }, { timeout: WAIT_FOR_TIMEOUT });
};
const pressEnterAndWaitFor = async (stdin, lastFrame, expectedText) => {
    stdin.write('\r');
    await vi.waitFor(() => {
        expect(lastFrame()).toContain(expectedText);
    }, { timeout: WAIT_FOR_TIMEOUT });
};
const moveDownAndWaitForSelection = async (stdin, lastFrame, label) => {
    stdin.write('\u001b[B');
    await waitForSelectedOption(lastFrame, label);
};
const navigateToCustomProtocolSelect = async (stdin, lastFrame) => {
    ...
    await pressEnterAndWaitFor(stdin, lastFrame, 'Custom Provider · Step 1/6 · Protocol');
};
const navigateToCustomBaseUrlInput = async (stdin, lastFrame) => {
    await navigateToCustomProtocolSelect(stdin, lastFrame);
    await pressEnterAndWaitFor(stdin, lastFrame, 'Custom Provider · Step 2/6 · Base URL');
};
const navigateToCustomApiKeyInput = async (stdin, lastFrame) => {
    await navigateToCustomBaseUrlInput(stdin, lastFrame);
    await pressEnterAndWaitFor(stdin, lastFrame, 'Custom Provider · Step 3/6 · API Key');
};
const navigateToCustomModelIdInput = async (stdin, lastFrame, apiKey = 'sk-test') => {
    await navigateToCustomApiKeyInput(stdin, lastFrame);
    await typeText(stdin, apiKey);
    await pressEnterAndWaitFor(stdin, lastFrame, 'Custom Provider · Step 4/6 · Model IDs');
};
const navigateToCustomAdvancedConfig = async (stdin, lastFrame, apiKey = 'sk-test', modelIds = 'model-1,model-2') => {
    await navigateToCustomModelIdInput(stdin, lastFrame, apiKey);
    await typeText(stdin, modelIds);
    await pressEnterAndWaitFor(stdin, lastFrame, 'Custom Provider · Step 5/6 · Advanced Config');
};
const isUnreliableTuiInputEnvironment = process.platform === 'win32' || process.env['CI'] === 'true';
...
describe('AuthDialog', { timeout: 15000 }, () => {
    ...
    itWhenTuiInputReliable('should prevent exiting when no auth method is selected and show error message', async () => {
        ...
        const { lastFrame, stdin, unmount } = renderAuthDialog(settings, {}, { closeAuthDialog }, undefined);
        await waitForSelectedOption(lastFrame, 'Alibaba ModelStudio');
    ...
    itWhenTuiInputReliable('should allow exiting when auth method is already selected', async () => {
        ...
        const { stdin, lastFrame, unmount } = renderAuthDialog(settings, {}, { closeAuthDialog }, AuthType.USE_OPENAI);
        await vi.waitFor(() => {
    ...
    itWhenTuiInputReliable('should preserve the selected main entry when returning from each top-level flow', async () => {
        const createSettings = () => new LoadedSettings({
            settings: { ui: { customThemes: {} }, mcpServers: {} },
            originalSettings: { ui: { customThemes: {} }, mcpServers: {} },
            path: '',
        }, {
        ...
        for (const testCase of cases) {
            ...
            await waitForSelectedOption(lastFrame, 'Alibaba ModelStudio');
            while (!lastFrame()?.match(new RegExp(`›\\s*(?:\\d+\\.\\s*)?${escapeRegExp(testCase.label)}`))) {
                stdin.write('\u001b[B');
                await wait();
            }
    ...
    itWhenTuiInputReliable('should go back from Coding Plan region selection to Alibaba ModelStudio', async () => {
        const settings = new LoadedSettings({
            settings: { ui: { customThemes: {} }, mcpServers: {} },
            originalSettings: { ui: { customThemes: {} }, mcpServers: {} },
            path: '',
        ...
        await waitForSelectedOption(lastFrame, 'Alibaba ModelStudio');
        await pressEnterAndWaitFor(stdin, lastFrame, 'Alibaba ModelStudio · Access Method');
    ...
    itWhenTuiInputReliable('should go back from third-party provider API key input to provider list', async () => {
        const settings = new LoadedSettings({
            settings: { ui: { customThemes: {} }, mcpServers: {} },
            originalSettings: { ui: { customThemes: {} }, mcpServers: {} },
            path: '',
        ...
        await waitForSelectedOption(lastFrame, 'Alibaba ModelStudio');
        await moveDownAndWaitForSelection(stdin, lastFrame, 'Third-party Providers');
        await pressEnterAndWaitFor(stdin, lastFrame, 'Third-party Providers · Provider');
        await waitForSelectedOption(lastFrame, 'DeepSeek API Key');
        await pressEnterAndWaitFor(stdin, lastFrame, 'DeepSeek API Key · Step 1/2 · API Key');
        stdin.write('\u001b');
        await vi.waitFor(() => {
            const frame = lastFrame();
    ...
    itWhenTuiInputReliable('should show preset providers in third-party provider options', async () => {
        const settings = new LoadedSettings({
            settings: { ui: { customThemes: {} }, mcpServers: {} },
            originalSettings: { ui: { customThemes: {} }, mcpServers: {} },
            path: '',
    ...
    itWhenTuiInputReliable('drives API key provider steps from endpoint options metadata', async () => {
        const settings = new LoadedSettings({
            settings: { ui: { customThemes: {} }, mcpServers: {} },
            originalSettings: { ui: { customThemes: {} }, mcpServers: {} },
            path: '',
        ...
        await pressEnterAndWaitFor(stdin, lastFrame, 'MiniMax API Key · Step 1/3 · Endpoint');
        await vi.waitFor(() => {
            const frame = lastFrame();
    ...
    itWhenTuiInputReliable('should show Alibaba ModelStudio access methods after selecting Alibaba ModelStudio', async () => {
        const settings = new LoadedSettings({
            settings: { ui: { customThemes: {} }, mcpServers: {} },
            originalSettings: { ui: { customThemes: {} }, mcpServers: {} },
            path: '',
    ...
    itWhenTuiInputReliable('should submit Token Plan through the shared subscription handler', async () => {
        ...
        const { stdin, lastFrame, unmount } = renderAuthDialog(settings, {}, { handleProviderSubmit });
        await waitForSelectedOption(lastFrame, 'Alibaba ModelStudio');
        stdin.write('\r');
        await waitForSelectedOption(lastFrame, 'Coding Plan');
    ...
    itWhenTuiInputReliable('should pre-fill the Model IDs step with previously saved custom model IDs', async () => {
    ...
    itWhenTuiInputReliable('should return from Token Plan API key input to Token Plan selection', async () => {
        const settings = new LoadedSettings({
            settings: { ui: { customThemes: {} }, mcpServers: {} },
            originalSettings: { ui: { customThemes: {} }, mcpServers: {} },
            path: '',
        ...
        stdin.write('\u001b');
        await vi.waitFor(() => {
            expect(lastFrame()).toContain('Alibaba ModelStudio · Step 1/3 · Region');
        }, { timeout: WAIT_FOR_TIMEOUT });
        stdin.write('\u001b');
        await vi.waitFor(() => {
            expect(lastFrame()).toContain('Alibaba ModelStudio');
...
describe('AuthDialog Custom API Key Wizard', { timeout: 15000 }, () => {
    const wait = (ms = 50) => new Promise((resolve) => setTimeout(resolve, ms));
    const createStandardSettings = () => new LoadedSettings({
        settings: { ui: { customThemes: {} }, mcpServers: {} },
        originalSettings: { ui: { customThemes: {} }, mcpServers: {} },
        path: '',
    }, {
    ...
    itWhenTuiInputReliable('navigates to protocol selection when Custom API Key is selected', async () => {
        ...
        const { stdin, lastFrame, unmount } = renderWithProviders(_jsx(UIStateContext.Provider, { value: mockUIState, children: _jsx(UIActionsContext.Provider, { value: mockUIActions, children: _jsx(AuthDialog, {}) }) }), { settings, config: mockConfig });
        await navigateToCustomProtocolSelect(stdin, lastFrame);
        await vi.waitFor(() => {
            const frame = lastFrame();
    ...
    itWhenTuiInputReliable('navigates to base URL input after selecting a protocol', async () => {
        const settings = createStandardSettings();
        const mockUIState = createMockUIState();
        const mockUIActions = createMockUIActions();
        const mockConfig = {
    ...
    itWhenTuiInputReliable('shows review screen with JSON after entering model IDs', async () => {
        const settings = createStandardSettings();
        const mockUIState = createMockUIState();
        const mockUIActions = createMockUIActions();
        const mockConfig = {
        ...
        const { stdin, lastFrame, unmount } = renderWithProviders(_jsx(UIStateContext.Provider, { value: mockUIState, children: _jsx(UIActionsContext.Provider, { value: mockUIActions, children: _jsx(AuthDialog, {}) }) }), { settings, config: mockConfig });
        await navigateToCustomAdvancedConfig(stdin, lastFrame, 'sk-test-key-12345', 'qwen/qwen3-coder,gpt-4.1');
        await pressEnterAndWaitFor(stdin, lastFrame, 'Custom Provider · Step 6/6 · Review');
        await vi.waitFor(() => {
            const frame = lastFrame();
    ...
    itWhenTuiInputReliable('calls handleProviderSubmit on Enter in review view', async () => {
        const settings = createStandardSettings();
        const handleProviderSubmit = vi.fn().mockResolvedValue(undefined);
        const mockUIState = createMockUIState();
        ...
        const { stdin, lastFrame, unmount } = renderWithProviders(_jsx(UIStateContext.Provider, { value: mockUIState, children: _jsx(UIActionsContext.Provider, { value: mockUIActions, children: _jsx(AuthDialog, {}) }) }), { settings, config: mockConfig });
        await navigateToCustomAdvancedConfig(stdin, lastFrame, 'sk-test', 'model-1,model-2');
        await pressEnterAndWaitFor(stdin, lastFrame, 'Custom Provider · Step 6/6 · Review');
        await vi.waitFor(() => {
            const frame = lastFrame();
            expect(frame).toContain('Enter to save');
        ...
        await vi.waitFor(() => {
            expect(handleProviderSubmit).toHaveBeenCalledWith(expect.objectContaining({ id: 'custom-openai-compatible' }), expect.objectContaining({
    ...
    itWhenTuiInputReliable('shows advanced config screen after entering model IDs', async () => {
        const settings = createStandardSettings();
        const mockUIState = createMockUIState();
        const mockUIActions = createMockUIActions();
        const mockConfig = {
        ...
        await vi.waitFor(() => {
            ...
            expect(frame).toContain('Optional: configure advanced generation settings');
    ...
    itWhenTuiInputReliable('passes generationConfig when advanced options are toggled', async () => {
        const settings = createStandardSettings();
        const handleProviderSubmit = vi.fn().mockResolvedValue(undefined);
        const mockUIState = createMockUIState();
        const mockUIActions = createMockUIActions({ handleProviderSubmit });
        ...
        const { stdin, lastFrame, unmount } = renderWithProviders(_jsx(UIStateContext.Provider, { value: mockUIState, children: _jsx(UIActionsContext.Provider, { value: mockUIActions, children: _jsx(AuthDialog, {}) }) }), { settings, config: mockConfig });
        await navigateToCustomAdvancedConfig(stdin, lastFrame, 'sk-test', 'model-1');
        await vi.waitFor(() => {
            const frame = lastFrame();
            expect(frame).toContain('Custom Provider · Step 5/6 · Advanced Config');
        });
