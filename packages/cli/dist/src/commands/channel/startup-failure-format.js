...
import { MAX_CHANNEL_STARTUP_FAILURES, MAX_CHANNEL_STARTUP_FAILURE_CHANNEL_LENGTH, MAX_CHANNEL_STARTUP_FAILURE_CODE_LENGTH, MAX_CHANNEL_STARTUP_FAILURE_MESSAGE_LENGTH, } from '../../serve/channel-worker-startup-ipc.js';
...
export function sanitizeChannelCommandValue(value, maxLength = 512) {
    return sanitizeValue(value, maxLength);
}
export function safeChannelCommandErrorMessage(error) {
    let value;
    try {
...
export function channelStartupFailureBody(error) {
...
export function formatChannelStartupFailures(source, fallbackWorkspaceCwd) {
    ...
    for (const raw of limitedFailures) {
        try {
            ...
            const safeChannel = sanitizeValue(channel, MAX_CHANNEL_STARTUP_FAILURE_CHANNEL_LENGTH);
            const safeMessage = sanitizeValue(message, MAX_CHANNEL_STARTUP_FAILURE_MESSAGE_LENGTH);
            const workspace = sanitizeValue(typeof rawWorkspace === 'string'
                ? rawWorkspace
                : (fallbackWorkspaceCwd ?? ''), 4096);
            const code = typeof rawCode === 'string'
                ? sanitizeValue(rawCode, MAX_CHANNEL_STARTUP_FAILURE_CODE_LENGTH)
                : undefined;
            if (!safeChannel || !safeMessage)
                continue;
            const fields = [
            ...
            lines.push(`[Channel] Startup failure (${fields.join(', ')}): ${safeMessage}`);
        }
        catch {
...
//# sourceMappingURL=startup-failure-format.js.map