...
describe('planCommand', () => {
    ...
    it('should switch to plan mode if not in plan mode', async () => {
        ...
        expect(result).toEqual({
            type: 'message',
            messageType: 'info',
            content: 'Enabled plan mode. The agent will analyze and plan without executing tools.',
        });
    });
    ...
    it('should return already in plan mode if mode is already plan', async () => {
        ...
        (mockContext.services.config?.getApprovalMode).mockReturnValue(ApprovalMode.PLAN);
        const result = await planCommand.action(mockContext, '');
    ...
    it('should exit plan mode when exit argument is passed', async () => {
        ...
        expect(mockContext.services.config?.setApprovalMode).toHaveBeenCalledWith(ApprovalMode.DEFAULT);
        expect(result).toEqual({
            type: 'message',
            messageType: 'info',
    ...
    it('should restore pre-plan mode when executing from plan mode', async () => {
        ...
        (mockContext.services.config?.getApprovalMode).mockReturnValue(ApprovalMode.PLAN);
        (mockContext.services.config?.getPrePlanMode).mockReturnValue(ApprovalMode.AUTO_EDIT);
        const result = await planCommand.action(mockContext, 'exit');
        expect(mockContext.services.config?.setApprovalMode).toHaveBeenCalledWith(ApprovalMode.AUTO_EDIT);
        expect(result).toEqual({
            type: 'message',
            messageType: 'info',
            content: 'Exited plan mode. Previous approval mode restored.',
...
//# sourceMappingURL=planCommand.test.js.map