...
import { formatSourceLabel, formatSourceLabels, getConfigSourceDisplay, } from './sourceLabels.js';
function makeConfig(source, sourceDisplay = '') {
    return {
...
describe('sourceLabels', () => {
    ...
    describe('formatSourceLabel', () => {
        ...
        it('falls back to the raw source string for unknown values', () => {
            expect(formatSourceLabel('mystery')).toBe('mystery');
        });
    });
    ...
    describe('getConfigSourceDisplay', () => {
        it('returns the translated long label for non-extension sources', () => {
            expect(getConfigSourceDisplay(makeConfig(HooksConfigSource.User))).toBe('User Settings');
            expect(getConfigSourceDisplay(makeConfig(HooksConfigSource.Project))).toBe('Local Settings');
            expect(getConfigSourceDisplay(makeConfig(HooksConfigSource.System))).toBe('System Settings');
        });
        it('appends the extension name for Extensions-source configs', () => {
            expect(getConfigSourceDisplay(makeConfig(HooksConfigSource.Extensions, 'my-ext'))).toBe('Extensions (my-ext)');
        });
        it('uses the long "Session (temporary)" label for session-source configs', () => {
            expect(getConfigSourceDisplay(makeConfig(HooksConfigSource.Session))).toBe('Session (temporary)');
        });
        it('falls back to the raw source string for unknown sources', () => {
            const config = makeConfig('mystery');
            expect(getConfigSourceDisplay(config)).toBe('mystery');
        });
    });
});
//# sourceMappingURL=sourceLabels.test.js.map