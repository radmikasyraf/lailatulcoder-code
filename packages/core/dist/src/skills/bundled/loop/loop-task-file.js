 ...
 */
const moduleRealDirCache = new Map();
function resolveRealDir(dir, cache) {
 ...
 */
async function readBoundedTaskFile(filePath) {
    ...
    try {
        ...
        while (total < allocSize) {
            const { bytesRead } = await handle.read(buffer, total, allocSize - total, total);
            if (bytesRead === 0) {
                break;
            }
            total += bytesRead;
...
/**
 * Reads `.lailatulcoder/loop.md`, project before home, byte-capped at 25 KB. A missing,
 ...
 * stat'd BEFORE the blocking open. A symlinked `.lailatulcoder/loop.md` is refused
 ...
 * catch an *ancestor* symlink like a checked-in `.lailatulcoder -> /outside` that a
 ...
 * always the real `~/.lailatulcoder`). It is the user's own dotfile, so a symlink IS
 ...
 * exfiltrated.
 */
export async function readLoopTaskFile({ projectRoot, homeDir, homeQwenDir = path.join(homeDir, '.lailatulcoder'), allowProjectFile = false, realDirCache = moduleRealDirCache, }) {
    if (!allowProjectFile) {
    ...
    const candidates = [
        ...(allowProjectFile
            ? [
                {
                    source: 'project',
                    path: path.join(projectRoot, '.lailatulcoder', 'loop.md'),
                },
            ]
            : []),
    ...
    for (const { source, path: filePath } of candidates) {
        ...
        try {
            if (source === 'project') {
                ...
                // .lailatulcoder/loop.md`), so confinement passes on the same fs and the secret
                ...
                // checked-in `.lailatulcoder -> /outside`); realpath resolves it, so confine the
                ...
                if (!isWithin(realRoot, real)) {
                    debugLogger.debug('skipping project loop.md that escapes the workspace', {
                        filePath,
                        resolved: real,
                    });
                    continue;
                ...
                // ~/.lailatulcoder/loop.md`) and would otherwise be read and fed to the model.
                if (homeStat.nlink > 1) {
                ...
                // otherwise `~/.lailatulcoder/loop.md -> /etc/passwd` (or `-> /dev/...`) would be
                ...
                if (!isWithin(realHome, real)) {
                    debugLogger.debug('skipping home loop.md that escapes the home directory', { filePath, resolved: real });
                    continue;
                }
                buffer = await readBoundedTaskFile(real);
            }
            ...
            // (ENOTDIR, e.g. a stray file where `.lailatulcoder` should be), a symlink loop
            // (ELOOP, e.g. a self-referential `~/.lailatulcoder/loop.md`), or an over-long path
...
//# sourceMappingURL=loop-task-file.js.map