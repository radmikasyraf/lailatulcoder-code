...
function runPlanDiff(args) {
    ...
    try {
        ...
        throw new Error(`Cannot read diff file ${diffPath}: ${err.message}`);
    }
    ...
    if ((args.pr === undefined) !== (args.repo === undefined)) {
        throw new TypeError('--pr and --repo go together — the roster requires the ' +
    ...
    const host = args.host === undefined || args.host === '' ? undefined : args.host.trim();
    if (host !== undefined && !HOSTNAME_RE.test(host)) {
        throw new TypeError(`--host is not a hostname: ${JSON.stringify(args.host)}`);
    }
    ...
    if (plan.diffLines === 0) {
        ...
        // is skipped.
        writeStderrLine(`WARNING: the diff is empty — 0 chunks. Reviewing from this plan would ` +
...
export const planDiffCommand = {
    ...
    builder: (yargs) => yargs
        ...
        .option('pr', {
        type: 'number',
        describe: 'The PR number this diff came from (lightweight cross-repo mode). ' +
        ...
        .option('host', {
        type: 'string',
        describe: "The PR's host (GitHub Enterprise), together with --pr/--repo — " +
    ...
    handler: (argv) => {
        ...
        try {
            ...
            writeStderrLineSafe(`plan-diff: ${err.message}`);
