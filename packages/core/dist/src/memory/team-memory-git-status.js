 ...
 * directory: a `!.lailatulcoder/team-memory/` re-include is a directory-form negation
 ...
 */
function isTeamFileGitIgnored(gitRoot, filePath) {
 ...
 * e.g. a `.lailatulcoder/` directory-form ignore, which git never descends into, so a
 ...
 */
export function getTeamMemoryShareabilityWarning(projectRoot) {
    ...
    if (!gitRoot) {
        ...
        return (`Team memory is enabled, but ${teamRoot} is not inside a git ` +
    ...
    // (e.g. `.lailatulcoder/team-memory/*.md` + `!.lailatulcoder/team-memory/MEMORY.md`) pass while
    ...
    const indexIgnored = isTeamFileGitIgnored(gitRoot, getTeamAutoMemoryIndexPath(projectRoot));
    const topicIgnored = isTeamFileGitIgnored(gitRoot, path.join(teamRoot, 'feedback.md'));
    if (indexIgnored || topicIgnored) {
        debugLogger.debug(`Team memory at ${teamRoot} is git-ignored (index=${indexIgnored}, topic=${topicIgnored}).`);
        return (`Team memory is enabled, but ${teamRoot} is git-ignored, so saved ` +
            `memories are not shared. If your .gitignore excludes '.lailatulcoder/' ` +
            `(directory form), change it to '.lailatulcoder/*' and re-include ` +
            `'.lailatulcoder/team-memory/' (and its contents, e.g. '!.lailatulcoder/team-memory/**').`);
    }
    return null;
}
//# sourceMappingURL=team-memory-git-status.js.map