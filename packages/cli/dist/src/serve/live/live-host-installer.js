...
export const LIVE_HOST_APP_PATH = '/Applications/LailatulCoder Live Host.app';
export const LIVE_HOST_OSS_BASE_URL = 'https://lailatul-coder-assets.oss-cn-hangzhou.aliyuncs.com/live-host';
export const LIVE_HOST_RELEASE_BASE_URL = 'https://github.com/LailatulCoder/lailatul-coder/releases/download/live-host-latest';
export const LIVE_HOST_MANIFEST_NAME = 'LailatulCoder-Live-Host-manifest.json';
export const LIVE_HOST_MANIFEST_FETCH_TIMEOUT_MS = 5 * 60 * 1000;
export const LIVE_HOST_DOWNLOAD_TIMEOUT_MS = 60 * 60 * 1000;
const LIVE_HOST_APP_NAME = 'LailatulCoder Live Host.app';
const MAX_DOWNLOAD_BYTES = 512 * 1024 * 1024;
const COMMAND_TIMEOUT_MS = 2 * 60 * 1000;
const SHA256_PATTERN = /^[a-f0-9]{64}$/;
const VERSION_PATTERN = /^\d+\.\d+\.\d+(?:-[0-9A-Za-z.-]+)?$/;
export function isExpectedLiveHostSignature(output) {
    return (/^Authority=Developer ID Application:/m.test(output) &&
        new RegExp(`^TeamIdentifier=${LIVE_HOST_TEAM_IDENTIFIER}$`, 'm').test(output));
}
function architecture(value) {
    if (value === 'arm64' || value === 'x64')
        return value;
    throw new Error(`LailatulCoder Live Host is unavailable for architecture ${value}.`);
}
function isRecord(value) {
    return typeof value === 'object' && value !== null && !Array.isArray(value);
}
function parseAsset(value, expectedName) {
    if (!isRecord(value))
...
export function parseLiveHostReleaseManifest(value) {
    ...
    return {
        ...
        assets: {
            arm64: parseAsset(value['assets']['arm64'], 'LailatulCoder-Live-Host-arm64.zip'),
            x64: parseAsset(value['assets']['x64'], 'LailatulCoder-Live-Host-x64.zip'),
        },
    };
}
...
async function inspectApp(appPath) {
    ...
    if (!stat.isDirectory() || stat.isSymbolicLink()) {
        throw new Error('LailatulCoder Live Host installation is not a regular app bundle.');
    }
    const bundleId = await readBundleValue(appPath, 'CFBundleIdentifier');
    if (bundleId !== LIVE_HOST_BUNDLE_ID) {
        throw new Error('LailatulCoder Live Host bundle identity is invalid.');
    }
    const version = await readBundleValue(appPath, 'CFBundleShortVersionString');
    if (!VERSION_PATTERN.test(version)) {
        throw new Error('LailatulCoder Live Host version is invalid.');
    }
    ...
    if (!isExpectedLiveHostSignature(signatureOutput)) {
        throw new Error('LailatulCoder Live Host signing identity is invalid.');
    }
...
async function downloadAsset(asset, url, destination, onProgress, fetchImpl) {
    const response = await fetchImpl(url, {
    ...
    await pipeline(Readable.fromWeb(response.body), guard, fs.createWriteStream(destination, { flags: 'wx', mode: 0o600 }));
    if (received !== asset.size || hash.digest('hex') !== asset.sha256) {
        throw new Error('Live Host checksum verification failed.');
    }
}
export async function downloadLiveHostRelease(currentArchitecture, destination, onProgress, fetchImpl = fetch) {
    ...
    for (let index = 0; index < manifestUrls.length; index += 1) {
        try {
            ...
            const assetUrl = resolveLiveHostAssetUrls(manifest.version, asset.name)[index];
            await downloadAsset(asset, assetUrl, destination, onProgress, fetchImpl);
            return { manifest, asset };
        }
        catch (error) {
            errors.push(new Error(`${labels[index]}: ${errorMessage(error)}`, { cause: error }));
        }
    }
    await fsp.rm(destination, { force: true });
    throw new AggregateError(errors, `LailatulCoder Live Host download failed. ${errors
        .map((error) => error.message)
        .join(' ')}`);
}
async function installLatestHost(currentArchitecture, onStatus) {
    const temporaryDirectory = await fsp.mkdtemp(path.join(os.tmpdir(), 'LailatulCoder-live-host-'));
    const archivePath = path.join(temporaryDirectory, 'LailatulCoder-Live-Host.zip');
    const extractedPath = path.join(temporaryDirectory, 'extracted');
    const candidatePath = path.join(extractedPath, LIVE_HOST_APP_NAME);
    const stagingPath = `/Applications/.lailatulcoder Live Host.install-${randomUUID()}.app`;
    const backupPath = `/Applications/.lailatulcoder Live Host.backup-${randomUUID()}.app`;
    let movedExisting = false;
    let installedCandidate = false;
    try {
        onStatus({ state: 'downloading', progress: 0 });
        const { manifest } = await downloadLiveHostRelease(currentArchitecture, archivePath, (progress) => {
...
function errorMessage(error) {
    ...
    return 'LailatulCoder Live Host setup failed.';
}
export class LiveHostInstaller {
    ...
    async refresh() {
        ...
        if (this.platform !== 'darwin') {
            return this.setError('LailatulCoder Live Host is available only on macOS.', false);
        }
    ...
    async launch() {
        ...
        try {
            const installed = await this.inspectInstalled();
            if (!installed)
                return this.setError('LailatulCoder Live Host is not installed.', true);
...
//# sourceMappingURL=live-host-installer.js.map