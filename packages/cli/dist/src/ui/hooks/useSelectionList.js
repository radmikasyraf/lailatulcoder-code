...
const areItemsStructurallyEqual = (a, b) => {
    if (a === b) {
        return true;
    }
...
function selectionListReducer(state, action) {
    switch (action.type) {
        ...
        case 'INITIALIZE': {
            ...
            const activeKey = !initialIndexChanged && state.activeIndex !== state.initialIndex
                ? state.items[state.activeIndex]?.key
                : undefined;
            const targetIndex = computeInitialIndex(initialIndex, items, activeKey);
            const itemsStructurallyEqual = areItemsStructurallyEqual(items, state.items);
 ...
 */
export function useSelectionList({ items, initialIndex = 0, onSelect, onHighlight, isFocused = true, showNumbers = false, disableVimNav = false, }) {
