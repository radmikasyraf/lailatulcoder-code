...
export function encodeTextCursor(state) {
    return Buffer.from(JSON.stringify({ v: CURSOR_VERSION, ...state }), 'utf8').toString('base64url');
}
/**
 ...
 */
export function decodeTextCursor(cursor) {
    if (cursor.length === 0 || cursor.length > MAX_TEXT_CURSOR_CHARS) {
        throw new FsError('parse_error', `cursor must be a non-empty string of at most ${MAX_TEXT_CURSOR_CHARS} characters`, { hint: 'pass a cursor returned by a previous read' });
    }
    let parsed;
    try {
    ...
    return { off: off, size: size, dev, ino };
}
/**
 ...
 */
export function assertCursorMatchesFile(cursor, stats, path) {
    if (String(stats.dev) !== cursor.dev ||
        String(stats.ino) !== cursor.ino ||
        stats.size < cursor.size) {
        throw new FsError('hash_mismatch', `cursor no longer matches the file it was issued for: ${path}`, { hint: 're-read the file from the beginning to get a fresh cursor' });
    }
}
