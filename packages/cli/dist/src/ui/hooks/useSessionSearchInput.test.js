...
describe('isPrintableSearchChar', () => {
    ...
    it('accepts SPACE — caller decides whether to seed it', () => {
        ...
        expect(isPrintableSearchChar(k({ name: 'space', sequence: ' ' }))).toBe(true);
    });
    it('rejects Ctrl-modified keys', () => {
        expect(isPrintableSearchChar(k({ name: 'a', sequence: 'a', ctrl: true }))).toBe(false);
    });
    it('rejects Meta-modified keys', () => {
        expect(isPrintableSearchChar(k({ name: 'a', sequence: 'a', meta: true }))).toBe(false);
    });
    it('rejects bracketed pastes (multi-line content must never seed a query)', () => {
        expect(isPrintableSearchChar(k({ name: 'paste', sequence: 'a', paste: true }))).toBe(false);
    });
    it('rejects multi-character sequences (e.g. CSI escape sequences)', () => {
        expect(isPrintableSearchChar(k({ name: 'up', sequence: '[A' }))).toBe(false);
    });
    it('rejects empty sequences (synthetic / structural keys)', () => {
        expect(isPrintableSearchChar(k({ name: 'return', sequence: '' }))).toBe(false);
    });
    it('rejects control characters below 0x20 (Tab, Enter, Esc)', () => {
        expect(isPrintableSearchChar(k({ name: 'tab', sequence: '\t' }))).toBe(false);
        expect(isPrintableSearchChar(k({ name: 'return', sequence: '\r' }))).toBe(false);
        expect(isPrintableSearchChar(k({ name: 'escape', sequence: '' }))).toBe(false);
    });
    it('rejects C1 control characters', () => {
        expect(isPrintableSearchChar(k({ name: 'csi', sequence: '\u009b' }))).toBe(false);
        expect(isPrintableSearchChar(k({ name: 'nel', sequence: '\u0085' }))).toBe(false);
    });
    it('rejects DEL (0x7F) — Backspace would otherwise slip through', () => {
        expect(isPrintableSearchChar(k({ name: 'backspace', sequence: '' }))).toBe(false);
    });
});
describe('isDeletionKey', () => {
    it('recognises DEL byte (0x7F) as a deletion key', () => {
        const onExitToList = vi.fn();
        const { result } = renderHook(() => useSessionSearchInput({ onExitToList }));
        act(() => {
    ...
    it('does not treat DEL/BS byte as a printable char', () => {
        expect(isPrintableSearchChar(k({ name: '', sequence: '\x7f' }))).toBe(false);
    ...
    it('does not treat Ctrl+H (BS byte with ctrl) as a deletion key', () => {
        ...
        act(() => {
            result.current.handleSearchKey(k({ name: 'h', sequence: '\b', ctrl: true }));
        });
        expect(result.current.searchQuery).toBe('a');
    ...
    it('does not treat Meta+BS byte as a deletion key', () => {
        ...
        expect(result.current.searchQuery).toBe('a');
        act(() => {
            result.current.handleSearchKey(k({ name: '', sequence: '\x7f', meta: true }));
        });
        expect(result.current.searchQuery).toBe('a');
        expect(onExitToList).not.toHaveBeenCalled();
    });
...
describe('useSessionSearchInput', () => {
    ...
    it('starts with an empty query', () => {
        const { result } = renderHook(() => useSessionSearchInput({ onExitToList: vi.fn() }));
        expect(result.current.searchQuery).toBe('');
    });
    ...
    it('Ctrl+U wipes the query and exits — single-stroke equivalent of full Backspace', () => {
        ...
        act(() => {
            result.current.handleSearchKey(k({ name: 'u', sequence: '', ctrl: true }));
        });
        expect(result.current.searchQuery).toBe('');
        expect(onExitToList).toHaveBeenCalledTimes(1);
    });
    it('Ctrl+L wipes the query and exits (alias of Ctrl+U for muscle-memory parity)', () => {
        ...
        act(() => {
            result.current.handleSearchKey(k({ name: 'l', sequence: '', ctrl: true }));
        });
        expect(result.current.searchQuery).toBe('');
        expect(onExitToList).toHaveBeenCalledTimes(1);
    });
    it('silently swallows unrecognized keys (search owns the keyboard)', () => {
        ...
        act(() => {
            result.current.handleSearchKey(k({ name: 'b', sequence: '', ctrl: true }));
        });
        expect(result.current.searchQuery).toBe('a');
        expect(onExitToList).not.toHaveBeenCalled();
    });
...
//# sourceMappingURL=useSessionSearchInput.test.js.map