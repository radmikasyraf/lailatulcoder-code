...
import { collectText, isUsableImagePart, replaceImagesWithText, splitImageParts, } from './image-part-utils.js';
...
const turnImageCounts = new WeakMap();
 ...
 */
export function isImageCapable(model) {
    return (model.isVision === true ||
        (model.modalities ?? defaultModalities(model.id)).image === true);
}
export function isFullTurnVisionCapable(model) {
    return (!model.fastOnly &&
        !model.voiceOnly &&
        !model.imageOnly &&
        model.capabilities?.agent === true &&
        isImageCapable(model));
}
export function getQualifiedVisionModelId(model) {
...
function hasAmbiguousRoute(candidates, selected) {
    return (candidates.filter((candidate) => candidate.id === selected.id &&
        candidate.authType === selected.authType &&
        candidate.baseUrl === selected.baseUrl).length > 1);
}
export function getVisionModelSelector(selection) {
...
export function getFullTurnVisionModelSelector(selection) {
 ...
 */
export function selectVisionBridgeModel(primaryModelId, models, primaryProvider = {}) {
    const candidates = models.filter((m) => m.id !== primaryModelId && isImageCapable(m));
    ...
    if (primaryProvider.baseUrl) {
        const sameEndpointCandidates = candidates.filter((m) => m.baseUrl === primaryProvider.baseUrl);
        const sameEndpoint = sameEndpointCandidates.find((candidate) => !hasAmbiguousRoute(models, candidate));
        return sameEndpoint ? toSelection(sameEndpoint) : undefined;
    }
    if (primaryProvider.authType) {
        const sameAuthCandidates = candidates.filter((m) => m.authType === primaryProvider.authType);
        const sameAuth = sameAuthCandidates.find((candidate) => !hasAmbiguousRoute(models, candidate));
 ...
 * caller's job.
 */
export function shouldRunVisionBridge(config) {
    return (config.getEffectiveInputModalities?.()?.image !== true &&
        config.getDefaultVisionBridgeModel?.() !== undefined);
}
export function isVisionBridgeNoticeDisplay(value) {
...
export function formatVisionBridgeNoticeDisplay(display) {
    return `${display.summary}\n${display.notice}`;
}
/** Build the user-facing, sanitized disclosure for a bridge attempt. */
export function formatVisionBridgeNotice(result) {
...
function buildPdfSourceGuidance(sourceContext) {
...
export function formatFullTurnVisionNotice(selection) {
    ...
    const sourceHint = sourceContext
        ...
        : imageParts.length > 0
            ? `Describe each image separately under these ordered labels: ${imageParts
                .map((part, index) => `${index + 1}. ${JSON.stringify(part.inlineData?.displayName?.trim() || `image ${index + 1}`)}`)
                .join('; ')}.`
            : '';
    const focusHint = intentText.length > 0
...
function inferPdfSourceContext(imageParts) {
    const sources = imageParts.map((part) => {
        const match = part.inlineData?.displayName?.match(/^(.*\.pdf) \(page (\d+)\)$/i);
    ...
    const first = pages[0];
    if (!first ||
        pages.some((source, index) => source.displayName !== first.displayName ||
 ...
 * conversation.
 */
function failure(reason, parts, omittedCount, extra = {}) {
    const { noteReason, ...resultExtra } = extra;
    const note = `[Vision bridge could not interpret the attached image(s): ${noteReason ?? reason}. ` +
...
export async function runVisionBridge(params) {
    ...
    const intent = (intentText ?? collectText(nonImageParts)).slice(0, BRIDGE_INTENT_MAX_CHARS);
    ...
    if (!modelForApi || !modelId) {
        return failure('no image-capable model is available for the vision bridge', parts, omittedCount);
    }
    const modelEndpoint = hostOf(baseUrl);
    if (toConvert.length === 0) {
        return failure(validImages.length > 0
            ? 'image conversion budget was exhausted'
            : 'no usable image could be read', parts, omittedCount, { modelId, ...(modelEndpoint && { modelEndpoint }) });
    }
    ...
    for (let attempt = 1; attempt <= VISION_BRIDGE_MAX_ATTEMPTS; attempt++) {
        ...
        try {
            ...
            if (description.length === 0) {
                ...
                return failure('the vision model returned no description', parts, omittedCount, { modelId, ...egress });
            }
            ...
            debugLogger.debug(`vision bridge transcription via ${modelId} (${description.length} chars)`);
            return {
                applied: true,
                status: 'ok',
                ...
                parts: replaceImagesWithText(parts, buildInterpretationBlock(modelId, description, toConvert.length, omittedCount, resolvedSourceContext)),
            ...
            if (timedOut && attempt < VISION_BRIDGE_MAX_ATTEMPTS) {
                debugLogger.warn(`conversion attempt ${attempt} via ${modelId} timed out after ${timeoutMs}ms; retrying`);
                continue;
            }
...
//# sourceMappingURL=vision-bridge-service.js.map