...
function showFile(baseRef, path) {
    return gitOpt('show', `${baseRef}:${path}`);
}
export function extractCodeReviewSection(content) {
...
function loadCombined(baseRef) {
    const sections = [];
    const loaded = [];
    // 1. LailatulCoder-native rules.
    const qwenRules = showFile(baseRef, '.lailatulcoder/review-rules.md');
    if (qwenRules) {
        sections.push(`### From .lailatulcoder/review-rules.md\n\n${qwenRules.trim()}`);
        loaded.push('.lailatulcoder/review-rules.md');
    }
    ...
    // 4. LAILATUL.md — extract Code Review section only.
    const qwenMd = showFile(baseRef, 'LAILATUL.md');
    if (qwenMd) {
        const section = extractCodeReviewSection(qwenMd);
        if (section) {
            sections.push(`### From LAILATUL.md\n\n${section}`);
            loaded.push('LAILATUL.md');
        }
    }
    return {
...
async function runLoadRules(args) {
    ...
    if (loaded.length === 0) {
        ...
        writeStdoutLine(`Loaded ${loaded.length} rule source(s) from ${baseRef} → ${out}: ${loaded.join(', ')}`);
    }
}
export const loadRulesCommand = {
    command: 'load-rules <base_ref>',
    describe: 'Read project review rules from the base branch (.lailatulcoder/review-rules.md, .github/copilot-instructions.md, AGENTS.md, LAILATUL.md) and write a combined Markdown file',
    builder: (yargs) => yargs
        .positional('base_ref', {
        type: 'string',
        demandOption: true,
        describe: 'Base ref to read rules from — typically the PR base branch (e.g. "origin/main"). Loading from the base branch (not the PR branch) prevents a malicious PR from injecting bypass rules.',
    })
        .option('out', {
        type: 'string',
        demandOption: true,
