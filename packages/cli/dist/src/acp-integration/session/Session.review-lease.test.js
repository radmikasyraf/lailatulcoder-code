...
import { ApprovalMode, AuthType, GoalPersistenceUnavailableError, Storage, promptIdContext, } from '@lailatul-coder/lailatul-coder-core';
...
describe('Session review-worktree lease sweep', () => {
    ...
    it('RL3: the sweep still runs when the model stream throws', async () => {
        mockChat.sendMessageStream.mockRejectedValueOnce(new Error('stream exploded'));
        const session = new Session(SESSION_ID, mockConfig, mockClient, mockSettings);
        await session.prompt(makePromptRequest()).catch(() => { });
        expect(cleanupReviewWorktreeLeases).toHaveBeenCalledTimes(1);
        expect(cleanupReviewWorktreeLeases).toHaveBeenCalledWith(expect.objectContaining({ promptId: `${SESSION_ID}########1` }));
    });
    it('RL4: consecutive prompts sweep under their own prompt IDs', async () => {
        ...
        expect(cleanupReviewWorktreeLeases).toHaveBeenNthCalledWith(1, expect.objectContaining({ promptId: `${SESSION_ID}########1` }));
        expect(cleanupReviewWorktreeLeases).toHaveBeenNthCalledWith(2, expect.objectContaining({ promptId: `${SESSION_ID}########2` }));
    });
});
//# sourceMappingURL=Session.review-lease.test.js.map