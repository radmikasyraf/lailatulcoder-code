 ...
 */
import { AgentEventType, ToolConfirmationOutcome, createDebugLogger, } from '@lailatul-coder/lailatul-coder-core';
...
export class SubAgentTracker {
    ...
    createToolCallHandler(abortSignal) {
        return (...args) => {
            ...
            void this.toolCallEmitter
                ...
                .catch((error) => {
                debugLogger.debug(`Failed to emit subagent tool start for ${event.name}:`, error);
            });
        };
    }
    /**
    ...
    createToolResultHandler(abortSignal) {
        return (...args) => {
            ...
            void this.toolCallEmitter
                ...
                .catch((error) => {
                debugLogger.debug(`Failed to emit subagent tool result for ${event.name}:`, error);
            });
            // Clean up state
    ...
    createApprovalHandler(abortSignal) {
        return async (...args) => {
            ...
            try {
                ...
                const outcome = resolvePermissionOutcome(output, offeredPermissionOptions);
                // Respond to subagent with the outcome
                await event.respond(outcome, {
                    answers: 'answers' in output
                        ? output.answers
                        : undefined,
                });
                ...
                try {
                    ...
                    debugLogger.error(`Failed to cancel subagent tool ${event.name} after permission request failure:`, respondError);
                }
            }
        };
    }
    ...
    createStreamTextHandler(abortSignal) {
        return (...args) => {
            ...
            void this.messageEmitter
                .emitMessage(event.text, 'assistant', event.thought ?? false, undefined, this.subagentMeta)
                .catch((error) => {
