...
import { createDebugLogger, isDebugLogFileEnabled, registerSession, writeRuntimeStatus, } from '@lailatul-coder/lailatul-coder-core';
...
import { disableKittyProtocol, pushKittyProtocolFlags, } from './utils/kittyProtocolDetector.js';
...
import { ErrorBoundary, consumeLastRenderError, } from './components/shared/ErrorBoundary.js';
...
export async function startInteractiveUI(config, settings, startupWarnings, workspaceRoot = process.cwd(), initializationResult, options = {}) {
    ...
    const restoreTerminalRedrawOptimizer = process.stdout.isTTY && !config.getScreenReader()
        ? installTerminalRedrawOptimizer(process.stdout)
        : () => { };
    const restoreSynchronizedOutput = process.stdout.isTTY && !config.getScreenReader()
    ...
    let dualOutputBridge = null;
    const jsonFd = config.getJsonFd?.();
    const jsonFile = config.getJsonFile?.();
    try {
        if (jsonFd != null) {
            dualOutputBridge = new DualOutputBridge(config, { fd: jsonFd }, { version });
        }
        else if (jsonFile != null) {
            dualOutputBridge = new DualOutputBridge(config, { filePath: jsonFile }, { version });
        }
    }
    catch (err) {
        debugLogger.error('Failed to initialize dual output bridge:', err);
        writeStderrLine(`Warning: dual output disabled — ${err instanceof Error ? err.message : String(err)}`);
    }
    ...
    if (inputFile) {
        try {
            ...
            writeStderrLine(`Warning: remote input disabled — ${err instanceof Error ? err.message : String(err)}`);
        }
    }
    ...
    const useVP = shouldUseVirtualViewport(settings.merged.ui?.useTerminalBuffer, config.getScreenReader(), isInteractiveTerminal());
    ...
    const resizeReflow = process.stdout.isTTY && !config.getScreenReader()
    ...
    const AppWrapper = () => {
        ...
        return (_jsx(RemoteInputContext.Provider, { value: remoteInputWatcher, children: _jsx(DualOutputContext.Provider, { value: dualOutputBridge, children: _jsx(SettingsContext.Provider, { value: settings, children: _jsx(KeypressProvider, { kittyProtocolEnabled: kittyProtocolStatus.enabled, config: config, debugKeystrokeLogging: settings.merged.general?.debugKeystrokeLogging, pasteWorkaround: process.platform === 'win32' || nodeMajorVersion < 20, initialCapturedInput: initialCapturedInput, children: _jsx(SessionStatsProvider, { sessionId: config.getSessionId(), children: _jsx(VimModeProvider, { settings: settings, children: _jsx(AgentViewProvider, { config: config, children: _jsx(BackgroundTaskViewProvider, { config: config, children: _jsx(AppContainer, { config: config, settings: settings, startupWarnings: startupWarnings, version: version, initializationResult: initializationResult, initialUseVirtualViewport: useVP, extensionRefreshState: options.extensionRefreshState, repaintViewport: resizeReflow.repaint }) }) }) }) }) }) }) }) }));
    };
    ...
    const appTree = (_jsx(ErrorBoundary, { recordForExitEcho: true, onError: (error, info) => {
            debugLogger.error(`[FATAL_RENDER_ERROR] ${error.message}\n${info.componentStack ?? ''}\n${error.stack ?? ''}`);
        ...
        }, children: _jsx(AppWrapper, {}) }));
    const instance = render(process.env['DEBUG'] ? (_jsx(React.StrictMode, { children: appTree })) : (appTree), {
        exitOnCtrlC: false,
        ...
        // measureElement+useLayoutEffect -- see MainContent.js). But it was
    ...
    startPostRenderPrefetches(config, settings, {
        ...
        initializeTelemetry: options.postRenderInitializeTelemetry ??
        ...
        try {
            if (process.stdout.isTTY && config.getChatRecordingService()) {
                ...
                if (isValidSessionId(sessionId) && (await stat(sessionFile)).size > 0) {
                    writeStdoutLine(`\n${t('To continue this session, run')}\nlailatulcoder --resume ${sessionId}`);
                }
            }
        }
        catch {
    ...
    config.trackSessionRegistration(registerSession({
...
function setWindowTitle(settings, folderName) {
...
//# sourceMappingURL=startInteractiveUI.js.map