...
import { uiTelemetryService, SessionEndReason, ToolNames, persistSessionUsage, createDebugLogger, } from '@lailatul-coder/lailatul-coder-core';
...
export const clearCommand = {
    ...
    action: async (context, _args) => {
        ...
        if (debugLogger.isEnabled()) {
            debugLogger.debug(`[CLEAR_START] Starting clear command, ` +
        ...
        if (config) {
            if (hasBlockingBackgroundWork(config)) {
                const baseMessage = "Stop the current session's running background tasks before starting a new session.";
            ...
            if (geminiClient) {
                context.ui.setDebugMessage(t('Starting a new session, resetting chat, and clearing terminal.'));
        ...
        if (debugLogger.isEnabled()) {
            ...
            debugLogger.debug(`[CLEAR_END] Clear command completed, ` +
