...
import { findItemAtLayoutRow, } from '../../utils/list-mouse.js';
/**
 ...
 */
export function RowMouseController({ containerRef, itemRefs, scrollOffset, isDisabled, onHoverIndex, onSelectIndex, }) {
    const { rows: terminalHeight } = useTerminalSize();
    const resolveIndex = useCallback((event) => {
    ...
    const handleMouse = useCallback((event) => {
        if (event.name !== 'move' && event.name !== 'left-press')
            return;
        const index = resolveIndex(event);
        if (index === null || isDisabled?.(index))
            return;
        if (event.name === 'move') {
            onHoverIndex(index);
        }
...
//# sourceMappingURL=RowMouseController.js.map