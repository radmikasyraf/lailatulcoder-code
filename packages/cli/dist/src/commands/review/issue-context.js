 ...
 */
// `LailatulCoder review issue-context`: fetch a PR's linked-issue evidence in one
...
function renderIssue(issue) {
    ...
    for (const c of issue.comments) {
        lines.push(`**${c.author || 'unknown'}** (${c.createdAt || 'unknown date'}):`, '', c.body.trim() === '' ? '_(empty)_' : c.body, '');
    }
    return lines.join('\n');
}
...
export function runIssueContext(args) {
    ...
    if (!isOwnerRepo(args.repo)) {
        throw new TypeError(`expected owner/repo, got ${JSON.stringify(args.repo)}`);
    }
    ...
    const fetchOne = (n, ownerRepo) => {
        try {
            return { number: n, ownerRepo, issue: platform.getIssue(n, ownerRepo) };
        }
        catch (err) {
            return { number: n, ownerRepo, error: err.message };
        }
    };
    ...
    const pairKey = (ownerRepo, n) => `${ownerRepo.toLowerCase()}#${n}`;
    ...
    for (const extra of args.extraIssues) {
        ...
        if (closingKeys.has(k) || seenExtras.has(k))
            continue;
        seenExtras.add(k);
    ...
    if (discoveryError !== undefined) {
        sections.push('**Closing-issue discovery FAILED** — the linked-issue set could not be fetched:', '', '```', discoveryError, '```', '', 'Treat the closing-issue set as UNKNOWN (not empty): any issues below ' +
            'come from explicit requests only, and issue fidelity must say the ' +
            'closing set could not be checked.', '');
    }
    else if (refs.length === 0) {
        sections.push('**No closing issues are linked to this PR** (the platform returned an empty closing-issue set).', '');
    }
    ...
    if (extraOutcomes.length > 0) {
        ...
        sections.push(discoveryError !== undefined
            ? '## Additionally fetched issues (referenced by the PR context; the closing set could not be checked)'
            : '## Additionally fetched issues (referenced by the PR context, NOT in the closing set)', '', 'These were requested explicitly. Whether the PR must satisfy them is ' +
...
export const issueContextCommand = {
    command: 'issue-context <pr_number>',
    describe: "Fetch a PR's closing issues (title, body, comments — each from its own repository) and render them as one Markdown evidence file",
    builder: (yargs) => yargs
        ...
        .option('host', {
        type: 'string',
        describe: "The host the target lives on. An Aone host (*.alibaba-inc.com) selects the a1 backend; omitted: detected from the clone's origin, else GitHub (GH_HOST, then github.com).",
    })
        .option('issue', {
        type: 'string',
        array: true,
        describe: "Also fetch this issue (repeatable): `123` (the PR's repo) or " +
    ...
    handler: (argv) => {
        ...
        if (prNumber === undefined ||
            ...
            prNumber <= 0 ||
            !extrasValid) {
            writeStderrLineSafe(`issue-context: pr_number must be a positive integer and every --issue must be \`123\` or \`owner/repo#123\`, got ${JSON.stringify(argv['pr_number'])} / ${JSON.stringify(argv['issue'])}`);
            process.exitCode = 2;
            return;
        }
        ...
        try {
            ...
            writeStderrLineSafe(`issue-context: ${err.message}`);
