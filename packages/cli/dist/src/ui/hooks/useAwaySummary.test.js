...
describe('useAwaySummary', () => {
    it('records the auto-fired recap to chatRecordingService so it survives /resume', async () => {
        ...
        const { rerender } = renderHook(({ isFocused }) => useAwaySummary({
            enabled: true,
            config,
            isFocused,
            isIdle: true,
        ...
        expect(addItem).toHaveBeenCalledWith(expect.objectContaining({ type: 'away_recap', text: 'recap text' }), expect.any(Number));
        expect(recordSlashCommand).toHaveBeenCalledWith(expect.objectContaining({
            phase: 'result',
            rawCommand: '/recap',
            outputHistoryItems: [
...
//# sourceMappingURL=useAwaySummary.test.js.map