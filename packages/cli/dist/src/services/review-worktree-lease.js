...
import { LEASE_PREFIX, REVIEW_TMP_DIR, reviewBranch, } from '../commands/review/lib/paths.js';
const GIT_TIMEOUT_MS = 120_000;
const debugLogger = createDebugLogger('REVIEW_WORKTREE_LEASE');
function gitOptions(timeout) {
    return {
        stdio: 'ignore',
        timeout,
 ...
 */
export function isReviewLeaseFile(fileName) {
    if (!fileName.startsWith(LEASE_PREFIX) || !fileName.endsWith('.json')) {
        return false;
    }
    const target = fileName.slice(LEASE_PREFIX.length, fileName.length - '.json'.length);
    return validTarget(target);
}
function leaseDirectory(repositoryRoot) {
    return join(repositoryRoot, REVIEW_TMP_DIR);
}
function leasePath(repositoryRoot, target) {
...
export function reviewLeasePath(repositoryRoot, target) {
    return leasePath(resolve(repositoryRoot), target);
}
export function clearReviewWorktreeLease(repositoryRoot, target) {
 ...
 */
export function clearReviewWorktreeLeaseIfOwned(repositoryRoot, target, owner) {
...
export function createReviewWorktreeLease(params) {
    ...
    try {
        ...
        if (existing && existing.sessionId !== params.sessionId) {
            throw new Error(`review worktree lease for ${params.target} is held by another ` +
...
export function readReviewWorktreeLease(repositoryRoot, target) {
    if (!validTarget(target))
        return null;
 ...
 */
export function reviewLeaseHeldByAnotherSession(lease) {
    if (!lease)
        return false;
    const sessionId = process.env['QWEN_CODE_SESSION_ID']?.trim();
    return !sessionId || lease.sessionId !== sessionId;
}
function removeLeaseWorktree(lease, gitTimeout) {
    ...
    if (worktreeRelative === '' ||
        ...
        debugLogger.debug(`Rejected review lease outside ${REVIEW_TMP_DIR}: ${worktreePath}`);
        return false;
    }
    try {
        execFileSync('git', ['-C', repositoryRoot, 'worktree', 'remove', worktreePath, '--force'], gitOptions(gitTimeout));
    }
    catch (error) {
        debugLogger.debug(`Git failed to remove review worktree ${lease.target}:`, error);
        try {
            rmSync(worktreePath, { recursive: true, force: true });
            execFileSync('git', ['-C', repositoryRoot, 'worktree', 'prune'], gitOptions(gitTimeout));
        }
        catch (fallbackError) {
            debugLogger.debug(`Fallback failed to remove review worktree ${lease.target}:`, fallbackError);
            return false;
        }
    }
    ...
    try {
        ...
        if (error.status !== 1) {
            debugLogger.debug(`Failed to inspect review branch ${lease.branch}:`, error);
            return false;
        }
        branchExists = false;
    }
    if (branchExists) {
        try {
            execFileSync('git', ['-C', repositoryRoot, 'branch', '-D', lease.branch], gitOptions(gitTimeout));
        }
        catch (error) {
            debugLogger.debug(`Failed to delete review branch ${lease.branch}:`, error);
            return false;
        }
    }
...
//# sourceMappingURL=review-worktree-lease.js.map