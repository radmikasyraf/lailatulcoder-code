...
let dir;
let cwd;
const run = (diffPath, out, maxChunkLines = 400) => planDiffCommand.handler({
    diff_path: diffPath,
    out,
    maxChunkLines,
});
...
describe('plan-diff — the round cap the handler actually records', () => {
    ...
    it('records the huge tier only when the environment has a deadline', () => {
        ...
        try {
            ...
            expect(JSON.parse(readFileSync(withClock, 'utf8')).budget.reverseAuditRounds).toBe(3);
        }
        finally {
            if (before === undefined)
                delete process.env[DEADLINE_ENV];
    ...
    it('records the operator ceiling the settings actually carry', () => {
        ...
        run(diffPath, out);
        expect(JSON.parse(readFileSync(out, 'utf8')).budget.reverseAuditRounds).toBe(10);
        settingsMock.mockReturnValue({
            merged: { review: { reverseAuditRounds: 4 } },
        });
        const capped = join(dir, 'ceiling-4.json');
        run(diffPath, capped);
        expect(JSON.parse(readFileSync(capped, 'utf8')).budget.reverseAuditRounds).toBe(4);
        ...
        run(diffPath, raised);
        expect(JSON.parse(readFileSync(raised, 'utf8')).budget.reverseAuditRounds).toBe(10);
    });
    it('reads a malformed deadline as no deadline, exactly as the gates do', () => {
        ...
        try {
            for (const bad of ['', '   ', 'soon', '0', '-1']) {
                ...
                run(diffPath, out);
                expect(JSON.parse(readFileSync(out, 'utf8')).budget.reverseAuditRounds).toBe(5);
            }
        }
        finally {
            if (before === undefined)
...
describe('plan-diff', () => {
    ...
    it('omits host when none is passed, and rejects a non-hostname', () => {
        ...
        expect(JSON.parse(readFileSync(out, 'utf8')).host).not.toBe('ghe.example.com; touch /tmp/pwned');
    });
    ...
    it('cannot decide heaviness without a tree, and says so by omission', () => {
        ...
        run(diffPath, out);
        const plan = JSON.parse(readFileSync(out, 'utf8'));
        expect(plan.files.every((f) => !f.heavy)).toBe(true);
    ...
    it('carries the topology numbers a local review needs', () => {
        const diffPath = join(dir, 'local.diff');
        const out = join(dir, 'plan.json');
        writeFileSync(diffPath, makeDiff('src/a.ts', 10) +
...
//# sourceMappingURL=plan-diff.test.js.map