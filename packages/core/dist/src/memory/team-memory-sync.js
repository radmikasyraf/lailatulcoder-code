 ...
 */
async function tryGit(cwd, args, killSignal = 'SIGTERM') {
    try {
 ...
 */
async function resolvePushTarget(gitRoot, branch) {
    const remote = (await tryGit(gitRoot, ['config', '--get', `branch.${branch}.remote`]))?.trim();
    const mergeRef = (await tryGit(gitRoot, ['config', '--get', `branch.${branch}.merge`]))?.trim();
    if (!remote || !mergeRef) {
        return null;
    }
 ...
 */
export async function syncTeamMemory(projectRoot, opts) {
    const result = {
        committed: false,
        pulled: false,
        pushed: false,
    ...
    const branch = (await tryGit(gitRoot, ['symbolic-ref', '--quiet', '--short', 'HEAD'], 'SIGKILL'))?.trim();
    if (!branch) {
    ...
    const upstream = await tryGit(gitRoot, ['rev-parse', '--abbrev-ref', '--symbolic-full-name', '@{u}'], 'SIGKILL');
    ...
    result.pushed =
        (await tryGit(gitRoot, ['push', pushTarget.remote, `HEAD:${pushTarget.mergeRef}`], 'SIGKILL')) !== null;
    if (!result.pushed) {
        result.skippedReason = 'push-failed';
    }
    return result;
