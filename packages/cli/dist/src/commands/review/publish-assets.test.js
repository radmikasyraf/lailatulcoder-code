...
const ghWithInputMock = vi.hoisted(() => vi.fn((_input, ..._rest) => ''));
const ghWithInputPlainMock = vi.hoisted(() => vi.fn((_input, ..._rest) => ''));
...
const setGhHostMock = vi.hoisted(() => vi.fn((_h) => { }));
const stderrSpy = vi.hoisted(() => vi.fn((_line) => { }));
...
const { runPublishAssets, publishAssetsCommand } = await import('./publish-assets.js');
...
describe('publish-assets', () => {
    ...
    function happyGh() {
        ...
        ghMock.mockImplementation((...args) => args.includes('.object.sha') ? 'headsha1234567890' : '{}');
    ...
    it('refuses without a designated repo — exit 3, nothing written', () => {
        ...
        expect(stdoutSpy).toHaveBeenCalledWith(JSON.stringify({ published: false }));
    });
    it('refuses an unauthorised run — same gate as submit, exit 3', () => {
        ...
        expect(process.exitCode).toBe(3);
        expect(ghWithInputMock).not.toHaveBeenCalled();
        const why = stderrSpy.mock.calls.map((c) => c[0]).join(' ');
    ...
    it('publishes, writes a manifest with commit-pinned URLs', () => {
        ...
        expect(p.url).toBe(`https://github.com/owner/assets/raw/headsha1234567890/${p.remotePath}`);
        expect(p.remotePath).toMatch(/^8346-review\/[0-9a-f]{12}-evidence\.png$/);
        expect(stdoutSpy).toHaveBeenCalledWith(JSON.stringify({ published: true, count: 1 }));
    ...
    it('creates the branch when missing, from the default branch head', () => {
        ...
        run({ files: [pngFile('a.png')] });
        expect(process.exitCode).toBeUndefined();
        const createCall = ghWithInputMock.mock.calls[0];
        expect(JSON.parse(createCall[0])).toEqual({
        ...
        const refCall = ghMock.mock.calls[0];
        expect(refCall[1]).toBe('repos/owner/assets/git/ref/heads/pr-assets/8346-review');
    ...
    it('retries an existing path with its blob sha — idempotent re-run', () => {
        ghMock.mockImplementation((...args) => {
            ...
            if (String(args[1] ?? '').includes('/contents/'))
                return 'blobsha';
            if (args.includes('.object.sha'))
                return 'headsha1234567890';
            return '{}';
        });
        ghWithInputMock
            .mockImplementationOnce(() => {
        ...
        run({ files: [pngFile('a.png')] });
        expect(process.exitCode).toBeUndefined();
        const retry = JSON.parse(ghWithInputMock.mock.calls[1][0]);
    ...
    it('authorises a URL-shaped --comment without binding it to the assets repo', () => {
        ...
        writeFileSync(argsFile, 'https://github.com/reviewed/upstream/pull/8346 --comment\n');
        happyGh();
        run({ files: [pngFile('a.png')] });
        expect(process.exitCode).toBeUndefined();
        expect(ghWithInputMock).toHaveBeenCalled();
    ...
    it('refuses the whole batch when one file fails the CONTENT ruling', () => {
        ...
        writeFileSync(good, Uint8Array.from([0x89, 0x50, 0x4e, 0x47, 0x0d, 0x0a, 0x1a, 0x0a]));
    ...
    it('publishes a webp whose bytes match — the slice covers the WEBP signature', () => {
        ...
        writeFileSync(shot, Uint8Array.from([...'RIFF\u0000\u0000\u0000\u0000WEBPVP8 '].map((c) => c.charCodeAt(0))));
        run({ files: [shot] });
        expect(process.exitCode).toBeUndefined();
        expect(ghWithInputMock).toHaveBeenCalled();
    });
    ...
    it('weaves published URLs into the findings artifact by assetFiles', () => {
        ...
        writeFileSync(findingsIn, JSON.stringify([
            {
                id: 'R1-1',
                severity: 'Critical',
        ...
        const withAssets = report.findings.find((f) => f.id === 'R1-1');
        expect(withAssets.assets).toHaveLength(1);
        expect(withAssets.assets[0]).toMatch(/^https:\/\/github\.com\/owner\/assets\/raw\/headsha1234567890\//);
        // The local paths survive for provenance; the untouched finding is intact.
        expect(withAssets.assetFiles).toEqual([img]);
        const without = report.findings.find((f) => f.id === 'R1-2');
    ...
    it('routes GitHub Enterprise calls through setGhHost before any API call', () => {
        ...
        const firstApiOrder = Math.min(...[...ghMock.mock.invocationCallOrder, Number.POSITIVE_INFINITY], ...[
            ...ghWithInputMock.mock.invocationCallOrder,
            Number.POSITIVE_INFINITY,
        ]);
        expect(setGhHostMock.mock.invocationCallOrder[0]).toBeLessThan(firstApiOrder);
        // And the manifest URLs carry the host.
        const manifest = JSON.parse(readFileSync(join(dir, 'manifest.json'), 'utf8'));
        expect(manifest.published[0].url).toMatch(/^https:\/\/github\.example\.com\//);
    });
...
describe('publish-assets — round-2 review pins', () => {
    ...
    it('an operator-exported GH_HOST routes AND names the URLs — one host, both jobs', () => {
        ...
        ghMock.mockImplementation((...a) => a.includes('.object.sha') ? 'headsha' : '{}');
        ghWithInputMock.mockImplementation(() => '{}');
        ...
        expect(manifest.published[0].url).toMatch(/^https:\/\/ghe\.corp\.example\//);
    });
    ...
    it('a PUT failure naming 422 only in the API path is rethrown', () => {
        ...
        ghMock.mockImplementation((...a) => a.includes('.object.sha') ? 'headsha' : '{}');
        ghWithInputMock.mockImplementation(() => {
            throw new Error('Command failed: gh api -X PUT repos/owner/assets/contents/4220-review/abc-a.png — HTTP 401: Bad credentials');
        });
        expect(() => runPublishAssets({ ...baseArgs(), pr: 4220 })).toThrow(/401/);
...
describe('publish-assets — round-3 self-review pins', () => {
    ...
    it('a branch-create 422 that is NOT already-exists surfaces, never swallowed', () => {
        ...
        ghMock
            ...
            .mockImplementationOnce(() => 'main')
            .mockImplementationOnce(() => 'basesha');
        ghWithInputMock.mockImplementation((_input, ...rest) => {
        ...
        const putCalls = ghWithInputMock.mock.calls.filter((c) => String(c.slice(1).join(' ')).includes('/contents/'));
...
describe('publish-assets — round-4 pins', () => {
    ...
    it('a double-fired branch create ("Reference already exists") is success', () => {
        ...
        ghMock
            ...
            .mockImplementationOnce(() => 'main')
            .mockImplementationOnce(() => 'basesha')
            .mockImplementation((...a) => a.includes('.object.sha') ? 'headsha' : '{}');
        ghWithInputMock.mockImplementation((_i, ...rest) => {
            if (rest.join(' ').includes('git/refs')) {
    ...
    it('an Enterprise-URL authorisation refuses a host-less (github.com) write', () => {
        ...
        writeFileSync(argsFile, 'https://ghe.corp.example/reviewed/upstream/pull/8346 --comment\n');
        runPublishAssets(baseArgs());
        expect(process.exitCode).toBe(3);
        const why = stderrSpy.mock.calls.map((c) => c[0]).join(' ');
...
describe('publish-assets — empty is two different things', () => {
    ...
    it('a findings artifact with no assetFiles is an ordinary no-op, exit 0', () => {
        ...
        expect(process.exitCode).toBeUndefined();
        expect(stdoutSpy).toHaveBeenCalledWith(JSON.stringify({ published: false, count: 0 }));
    });
});
describe('publish-assets — host binds even without --reviewed-repo', () => {
    ...
    it('refuses a host mismatch when the repo binding is absent', () => {
        ...
        writeFileSync(argsFile, 'https://ghe.example.com/reviewed/upstream/pull/8346 --comment\n');
...
//# sourceMappingURL=publish-assets.test.js.map