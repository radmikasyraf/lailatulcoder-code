...
import { AgentViewProvider, useAgentViewActions, useAgentViewState, } from './AgentViewContext.js';
/**
...
describe('AgentViewProvider in-process bridges', () => {
    ...
    it('clears embedded shell focus when switching agent tabs', async () => {
        ...
        function Probe() {
            ...
            return (_jsxs(Text, { children: [state.activeView, ":", String(state.agentShellFocused)] }));
        }
        const { lastFrame } = render(_jsx(AgentViewProvider, { config: config, children: _jsx(Probe, {}) }));
