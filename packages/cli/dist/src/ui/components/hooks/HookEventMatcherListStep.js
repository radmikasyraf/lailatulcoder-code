...
export function HookEventMatcherListStep({ hook, selectedIndex, }) {
    ...
    return (_jsxs(Box, { flexDirection: "column", paddingX: 1, children: [_jsx(HookEventHeader, { title: `${hook.event} - ${t('Matchers')}`, description: hook.description, exitCodes: hook.exitCodes }), hasMatchers ? (_jsxs(_Fragment, { children: [hook.matcherGroups.map((group, index) => {
                    ...
                    }), _jsx(Box, { marginTop: 1, children: _jsx(Text, { color: theme.text.secondary, children: t('Enter to select · Esc to go back') }) })] })) : (_jsxs(_Fragment, { children: [_jsx(Box, { children: _jsx(Text, { color: theme.text.secondary, children: t('No hooks configured for this event.') }) }), _jsx(Box, { marginTop: 1, children: _jsx(Text, { color: theme.text.secondary, children: t('To add hooks, edit settings.json directly or ask LailatulCoder.') }) }), _jsx(Box, { marginTop: 1, children: _jsx(Text, { color: theme.text.secondary, children: t('Esc to go back') }) })] }))] }));
}
