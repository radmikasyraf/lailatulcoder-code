...
import { buildRuntimeFetchOptions, redactProxyError, } from '../../utils/runtimeFetchOptions.js';
import { resolveRequestTimeout } from '../openaiContentGenerator/constants.js';
import { createDebugLogger } from '../../utils/debugLogger.js';
import { runtimeDiagnostics } from '../../utils/runtimeDiagnostics.js';
import { createChildAbortController } from '../../utils/abortController.js';
import { tokenLimit, hasExplicitOutputLimit, defaultOutputCeiling, reconcileMaxTokens, parsePositiveIntegerEnvValue, } from '../tokenLimits.js';
import { setToolCallPreparations } from '../tool-call-preparation.js';
import { reportAnthropicEvent, reportAnthropicFollowingRequest, reportAnthropicRequest, reportAnthropicResponse, } from '../../telemetry/gen-ai-request.js';
 ...
 * https://github.com/LailatulCoder/lailatul-coder/issues/3786
 */
...
function parseClaudeModelVersion(model) {
    ...
    const match = model
        .toLowerCase()
        .match(new RegExp(`claude-(${CLAUDE_MODEL_FAMILIES.join('|')})-(\\d+)(?:[-.](\\d{1,2})(?!\\d))?`));
    if (!match) {
        return null;
    }
    return {
 ...
 */
function anthropicSupportedEffortTiers(model) {
    ...
    const atLeast = (maj, min) => major > maj || (major === maj && minor >= min);
 ...
 */
function resolveEffectiveBaseUrl(contentGeneratorConfig) {
 ...
 */
function isAnthropicNativeBaseUrl(contentGeneratorConfig) {
    try {
        const hostname = new URL(resolveEffectiveBaseUrl(contentGeneratorConfig)).hostname.toLowerCase();
...
export class AnthropicContentGenerator {
    ...
    budgetDropWarned = false;
    temperatureDropWarned = false;
    ...
    async generateContentStream(request) {
        ...
        const perRequestAc = createChildAbortController(request.config?.abortSignal);
        let stream;
        try {
            stream = (await this.client.messages.create(streamingRequest, {
                signal: perRequestAc.signal,
                ...(headers ? { headers } : {}),
            }));
        }
        ...
        const inner = this.processStreamWithEmptyFallback(this.redactStreamErrors(stream), anthropicRequest, perRequestAc.signal, headers, telemetryAttempt);
    ...
    useGlobalCacheScope() {
        ...
        return (isAnthropicNativeBaseUrl(this.contentGeneratorConfig) ||
     ...
     */
    hasGlobalCacheScopeOnWire(req) {
        const isGlobalScope = (block) => {
            if (!block || typeof block !== 'object')
                return false;
            const cc = block.cache_control;
            if (!cc || typeof cc !== 'object')
                return false;
            return cc.scope === 'global';
        };
        if (Array.isArray(req.system)) {
            for (const block of req.system) {
                if (isGlobalScope(block))
                    return true;
            }
        }
        if (Array.isArray(req.tools)) {
            for (const tool of req.tools) {
                if (isGlobalScope(tool))
                    return true;
            }
        }
        return false;
    ...
    async buildRequest(request) {
        ...
        const dropUnsignedAssistantThinking = !isDeepSeek &&
            !!thinking &&
    ...
    buildSamplingParameters(request) {
        ...
        const configMaxTokens = configSamplingParams?.max_tokens;
        const requestMaxTokens = requestConfig.maxOutputTokens;
        const userMaxTokens = reconcileMaxTokens(configMaxTokens, requestMaxTokens) ??
            configMaxTokens ??
            requestMaxTokens;
        ...
        if (userMaxTokens !== undefined && userMaxTokens !== null) {
            ...
            const envMaxTokens = parsePositiveIntegerEnvValue(process.env['QWEN_CODE_MAX_OUTPUT_TOKENS']);
        ...
        if (temperatureDropped && !this.temperatureDropWarned) {
            const userTemp = getParam('temperature', 'temperature');
            if (userTemp !== undefined) {
                debugLogger.warn(`temperature=${userTemp} is not supported by '${this.contentGeneratorConfig.model ?? 'unknown'}' (deprecated on 4.8+); ignoring.`);
            }
    ...
    resolveEffectiveEffort(request) {
        ...
        if (isDeepSeekAnthropicHostname(this.contentGeneratorConfig)) {
            ...
            const mapped = effort === 'xhigh' || effort === 'max' ? 'max' : 'high';
            if (mapped !== effort && !this.effortClampWarned) {
                debugLogger.warn(`reasoning.effort='${effort}' is not accepted by the DeepSeek ` +
        ...
        const supported = anthropicSupportedEffortTiers(this.contentGeneratorConfig.model ?? '');
        const clamped = clampReasoningEffort(effort, supported);
        if (clamped !== effort && !this.effortClampWarned) {
            debugLogger.warn(`reasoning.effort='${effort}' is not supported by '${this.contentGeneratorConfig.model ?? 'unknown'}'; using '${clamped}'.`);
            this.effortClampWarned = true;
        }
        return clamped;
    }
    ...
    modelSupportsAdaptiveThinking() {
        const parsed = parseClaudeModelVersion(this.contentGeneratorConfig.model || '');
        if (!parsed)
            return false;
    ...
    buildThinkingConfig(request, effectiveEffort) {
        ...
        const applyRequestBudgetCap = (budgetTokens) => typeof requestBudgetCap === 'number' && requestBudgetCap > 0
        ...
        if (reasoning?.budget_tokens !== undefined &&
            ...
            debugLogger.warn(`reasoning.budget_tokens=${reasoning.budget_tokens} is ignored on '${this.contentGeneratorConfig.model ?? 'unknown'}' (Opus 4.7+/5.x use adaptive thinking); output_config.effort governs thinking depth instead.`);
     ...
     */
    resolveToolChoice(request, tools) {
    ...
    async *redactStreamErrors(stream) {
        try {
    ...
    async *processStream(stream, telemetryAttempt) {
        let messageId;
        let model;
        let cachedTokens = 0;
        ...
        for await (const event of stream) {
            ...
            switch (event.type) {
                ...
                case 'content_block_start': {
                    ...
                    const id = 'id' in event.content_block ? event.content_block.id : undefined;
                    const name = 'name' in event.content_block
                    ...
                    if (type === 'tool_use' &&
                        ...
                        typeof name === 'string' &&
                        name.length > 0) {
                        const chunk = this.buildGeminiChunk(undefined, messageId, model, undefined, takePendingMessageStartUsage());
                ...
                case 'content_block_delta': {
                    ...
                    if (deltaType === 'text_delta') {
                        ...
                        if (text) {
                            const chunk = this.buildGeminiChunk({ text }, messageId, model, undefined, takePendingMessageStartUsage());
                            collectedResponses.push(chunk);
                            yield chunk;
                        }
                    }
                        ...
                        if (thinking) {
                            const chunk = this.buildGeminiChunk({ text: thinking, thought: true }, messageId, model, undefined, takePendingMessageStartUsage());
                            collectedResponses.push(chunk);
                            yield chunk;
                        }
                    }
                        ...
                        if (signature) {
                            ...
                            const chunk = this.buildGeminiChunk({ thought: true, thoughtSignature: signature }, messageId, model, undefined, takePendingMessageStartUsage());
                            collectedResponses.push(chunk);
                            yield chunk;
                        }
                    }
                ...
                case 'content_block_stop': {
                    ...
                    if (blockState?.type === 'tool_use') {
                        ...
                        }, messageId, model, undefined, takePendingMessageStartUsage());
                        collectedResponses.push(chunk);
                        yield chunk;
                    }
                    blocks.delete(index);
                ...
                case 'message_delta': {
                    ...
                    if (finishReason || event.usage) {
                        messageStartUsagePending = false;
                        const chunk = this.buildGeminiChunk(undefined, messageId, model, finishReason, buildAnthropicUsageMetadata({
                            inputTokens: promptTokens,
                            cacheReadTokens: cachedTokens,
                            cacheCreationTokens,
    ...
    async *processStreamWithEmptyFallback(stream, fallbackRequest, abortSignal, headers, telemetryAttempt) {
        let hasAssistantPayload = false;
        let hasFinishReason = false;
        for await (const chunk of this.processStream(stream, telemetryAttempt)) {
            const candidates = chunk.candidates ?? [];
            hasFinishReason ||= candidates.some((candidate) => candidate.finishReason !== undefined);
            hasAssistantPayload ||= candidates.some((candidate) => candidate.content?.parts?.some((part) => part.text ||
                part.thought ||
        ...
        debugLogger.warn('Anthropic stream ended without assistant payload or finish reason; ' +
        ...
        try {
            ...
            const fallbackAttempt = reportAnthropicFollowingRequest(fallbackRequest, telemetryAttempt);
