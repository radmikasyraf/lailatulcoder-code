...
vi.mock('@lailatul-coder/lailatul-coder-core', () => {
    ...
    return {
        ...
        isWithinRoot: (location, root) => location === root || location.startsWith(`${root}/`),
        stripRuntimeSnapshotPrefix: (value) => value,
    };
});
...
import { BridgeChannelClosedError, } from '@lailatul-coder/acp-bridge/status';
import { resetHomeEnvBootstrapForTesting, SettingScope, SETTINGS_DIRECTORY_NAME, } from '../../../config/settings.js';
import { resetTrustedFoldersForTesting, TRUSTED_FOLDERS_FILENAME, TrustLevel, } from '../../../config/trustedFolders.js';
import { WorkspaceVoiceError } from '../../../services/voice-service.js';
import { WorkspacePermissionRulesSessionRequiredError, WorkspaceSkillNotFoundError, WorkspaceSkillNotToggleableError, WorkspaceSettingsPartialPersistError, } from '../types.js';
// ---------------------------------------------------------------------------
// Helpers
// ---------------------------------------------------------------------------
function makeDeps(overrides = {}) {
    return {
        ...
        queryWorkspaceStatus: vi
            .fn()
            .mockImplementation((_method, idle) => Promise.resolve(idle())),
...
async function withIsolatedQwenHome(fn) {
...
async function withIsolatedWorkspace(fn) {
    ...
    const originalTrustedFoldersPath = process.env['QWEN_CODE_TRUSTED_FOLDERS_PATH'];
    await fs.mkdir(home, { recursive: true });
    await fs.mkdir(workspace, { recursive: true });
    process.env['QWEN_HOME'] = home;
    process.env['QWEN_CODE_TRUSTED_FOLDERS_PATH'] = path.join(home, TRUSTED_FOLDERS_FILENAME);
...
describe('createDaemonWorkspaceService', () => {
    ...
    describe('workspace voice', () => {
        ...
        it('persists voice settings through batch persistence and publishes events', async () => {
            await withIsolatedQwenHome(async () => {
                ...
                const svc = createDaemonWorkspaceService(makeDeps({ persistSettings, publishWorkspaceEvent }));
                const result = await svc.setWorkspaceVoiceSettings(makeCtx({ originatorClientId: 'voice-client' }), { enabled: false, mode: 'tap', language: 'english' });
        ...
        it('rejects invalid voice settings before persisting', async () => {
            await withIsolatedQwenHome(async () => {
                ...
                const publishWorkspaceEvent = vi.fn();
                const svc = createDaemonWorkspaceService(makeDeps({ persistSettings, publishWorkspaceEvent }));
                await expect(svc.setWorkspaceVoiceSettings(makeCtx(), {
                    voiceModel: 'not-configured',
                })).rejects.toMatchObject({ code: 'unknown_voice_model' });
                await expect(svc.setWorkspaceVoiceSettings(makeCtx(), {})).rejects.toMatchObject({ code: 'invalid_voice_update' });
        ...
        it('publishes committed fallback voice writes when a later write fails', async () => {
            await withIsolatedQwenHome(async () => {
                const persistSetting = vi.fn(async (_workspace, _scope, key, _value) => {
                    if (key === 'general.voice.language') {
                        throw new Error('disk full');
                    }
                });
                const publishWorkspaceEvent = vi.fn();
                const svc = createDaemonWorkspaceService(makeDeps({ persistSetting, publishWorkspaceEvent }));
                await expect(svc.setWorkspaceVoiceSettings(makeCtx({ originatorClientId: 'voice-client' }), { mode: 'tap', language: 'english' })).rejects.toMatchObject({
                ...
                expect(mockWriteStderrLine).toHaveBeenCalledWith(expect.stringContaining('partial persist error'));
            });
        });
        it('publishes committed batch voice writes when batch persistence partially fails', async () => {
            ...
            const persistSettings = vi.fn(async (_workspace, writes) => {
                throw new WorkspaceSettingsPartialPersistError('batch failed', [writes[0]], new Error('disk full'));
            });
            const svc = createDaemonWorkspaceService(makeDeps({ persistSettings, publishWorkspaceEvent }));
            await expect(svc.setWorkspaceVoiceSettings(makeCtx({ originatorClientId: 'voice-client' }), { mode: 'tap', language: 'english' })).rejects.toThrow(WorkspaceSettingsPartialPersistError);
    ...
    describe('workspace permissions', () => {
        it('sets permission rules through the ACP command when a session is live', async () => {
            ...
            const publishWorkspaceEvent = vi.fn();
            const svc = createDaemonWorkspaceService(makeDeps({ invokeWorkspaceCommand, publishWorkspaceEvent }));
            const result = await svc.setWorkspacePermissionRules(makeCtx({ originatorClientId: 'perm-client' }), { scope: 'workspace', ruleType: 'allow', rules: ['Shell(*)'] });
            expect(invokeWorkspaceCommand).toHaveBeenCalledWith('qwen/permissions/setRules', {
                cwd: '/workspace',
                scope: 'workspace',
                ruleType: 'allow',
                rules: ['Shell(*)'],
        ...
        it('rejects permission updates when ACP has no live session', async () => {
            await withIsolatedQwenHome(async () => {
                ...
                await expect(svc.setWorkspacePermissionRules(makeCtx({ originatorClientId: 'perm-client' }), { scope: 'user', ruleType: 'deny', rules: ['Shell(rm -rf *)'] })).rejects.toThrow(WorkspacePermissionRulesSessionRequiredError);
        ...
        it('rethrows non-session permission command errors without fallback persistence', async () => {
            ...
            await expect(svc.setWorkspacePermissionRules(makeCtx(), {
                scope: 'workspace',
                ruleType: 'allow',
                rules: ['Shell(*)'],
    ...
    describe('status methods', () => {
        it('getWorkspaceTrustStatus reads current settings and trusted folders', async () => {
            await withIsolatedWorkspace(async ({ home, workspace }) => {
                ...
                const svc = createDaemonWorkspaceService(makeDeps({ boundWorkspace: workspace }));
        ...
        it('getWorkspacePermissionsStatus reads scoped and merged settings', async () => {
            await withIsolatedWorkspace(async ({ home, workspace }) => {
                ...
                await writeJson(path.join(workspace, SETTINGS_DIRECTORY_NAME, 'settings.json'), {
                    permissions: {
                        allow: ['Read(src/**)'],
                        ask: ['Shell(npm *)'],
                    },
        ...
        it('getWorkspaceMcpStatus delegates to queryWorkspaceStatus with correct method', async () => {
            ...
            expect(queryWorkspaceStatus).toHaveBeenCalledWith('qwen/status/workspace/mcp', expect.any(Function));
        });
        ...
        it('does not replay a stale MCP snapshot when the status provider is idle', async () => {
            ...
            const queryWorkspaceStatus = vi
                ...
                .mockImplementationOnce((_m, idle) => Promise.resolve(idle()));
            const svc = createDaemonWorkspaceService(makeDeps({ queryWorkspaceStatus, boundWorkspace: '/ws' }));
            await svc.getWorkspaceMcpStatus(makeCtx());
            const result = await svc.getWorkspaceMcpStatus(makeCtx());
            expect(result.initialized).toBe(false);
        ...
        it('getWorkspaceSkillsStatus delegates with correct method', async () => {
            ...
            expect(queryWorkspaceStatus).toHaveBeenCalledWith('qwen/status/workspace/skills', expect.any(Function));
        });
        ...
        it('getWorkspaceSkillsStatus replays the last live child status when the channel is idle', async () => {
            ...
            const queryWorkspaceStatus = vi
                .fn()
                .mockImplementation((_m, idle) => Promise.resolve(channelLive ? liveStatus : idle()));
            const svc = createDaemonWorkspaceService(makeDeps({ queryWorkspaceStatus, boundWorkspace: '/ws' }));
        ...
        it('does not let a superseded read extend the freshness window', async () => {
            ...
            const queryWorkspaceStatus = async () => (await query());
            const svc = createDaemonWorkspaceService(makeDeps({ queryWorkspaceStatus, boundWorkspace: '/ws' }));
            try {
                const supersededRead = svc.getWorkspaceSkillsStatus(makeCtx());
                svc.invalidateWorkspaceSkillsStatus();
                await expect(svc.getWorkspaceSkillsStatus(makeCtx())).resolves.toEqual(fresh);
                now += 4_000;
                ...
                now += 1_001;
                await expect(svc.getWorkspaceSkillsStatus(makeCtx())).resolves.toEqual(later);
        ...
        it('does not cache a workspace skills query invalidated while in flight', async () => {
            ...
            await expect(svc.getWorkspaceSkillsStatus(makeCtx())).resolves.toEqual(freshStatus);
        ...
        it('getWorkspaceSkillsStatus does not use the daemon-local provider while the child answers', async () => {
            ...
            const svc = createDaemonWorkspaceService(makeDeps({ queryWorkspaceStatus, workspaceSkillsStatusProvider }));
            await svc.getWorkspaceSkillsStatus(makeCtx());
            expect(workspaceSkillsStatusProvider).not.toHaveBeenCalled();
        });
        ...
        it('getWorkspaceProvidersStatus uses daemon-local provider when present', async () => {
            ...
            expect(workspaceProvidersStatusProvider).toHaveBeenCalledWith('/workspace', false);
        ...
        it('getWorkspaceProvidersStatus keeps ACP fallback without daemon-local provider', async () => {
            ...
            expect(queryWorkspaceStatus).toHaveBeenCalledWith('qwen/status/workspace/providers', expect.any(Function));
        });
        it('getWorkspaceEnvStatus uses statusProvider instead of queryWorkspaceStatus', async () => {
            ...
            expect(queryWorkspaceStatus).not.toHaveBeenCalled();
            expect(statusProvider.getEnvStatus).toHaveBeenCalledWith('/workspace', false);
            expect(result.initialized).toBe(true);
        });
        ...
        it('getWorkspacePreflightStatus queries ACP only when channel is live', async () => {
            ...
            expect(queryWorkspaceStatus).toHaveBeenCalledWith('qwen/status/workspace/preflight', expect.any(Function));
        });
    ...
    describe('setWorkspaceToolEnabled', () => {
        it('calls persistDisabledTools with workspace, toolName, and enabled', async () => {
            ...
            expect(persistDisabledTools).toHaveBeenCalledWith('/my/workspace', 'Bash', false, undefined);
        });
        ...
        it('does not publish toggle event when persistDisabledTools rejects', async () => {
            ...
            const publishWorkspaceEvent = vi.fn();
            const svc = createDaemonWorkspaceService(makeDeps({ persistDisabledTools, publishWorkspaceEvent }));
            await expect(svc.setWorkspaceToolEnabled(makeCtx(), 'Bash', false)).rejects.toThrow('disk full');
            expect(publishWorkspaceEvent).not.toHaveBeenCalled();
        });
        it('does not publish after persistence closes the runtime generation', async () => {
            ...
            await expect(svc.setWorkspaceToolEnabled(makeCtx(), 'Bash', false)).rejects.toThrow('generation closed');
            expect(publishWorkspaceEvent).not.toHaveBeenCalled();
        });
    });
    describe('setWorkspaceSkillEnabled', () => {
        const skillStatus = (overrides = {}) => ({
            kind: 'skill',
            status: 'ok',
            name: 'review',
            description: 'Review changed code',
        ...
        const statusQuery = (skill = skillStatus()) => vi.fn().mockResolvedValue({
            v: 1,
            workspaceCwd: '/workspace',
            initialized: true,
            skills: [skill],
        ...
        it('uses the canonical skill name and refreshes every active session', async () => {
            ...
            const result = await svc.setWorkspaceSkillEnabled(makeCtx({ originatorClientId: 'client-1' }), 'ReViEw', false);
            expect(persistDisabledSkills).toHaveBeenCalledWith('/workspace', 'review', false, undefined);
            expect(invalidate).toHaveBeenCalledWith('/workspace');
            expect(invokeWorkspaceCommand).toHaveBeenCalledWith('qwen/control/workspace/skills/refresh', { cwd: '/workspace', reason: 'settings' });
            ...
            expect(publishWorkspaceEvent).toHaveBeenCalledWith(skillToggleSettingsChanged({
                key: 'skills.disabled',
                value: ['review'],
        ...
        it('shares mutation ids within a request and renews them across requests', async () => {
            ...
            await svc.setWorkspaceSkillEnabled(makeCtx(), 'review', true);
            const mutationIds = publishWorkspaceEvent.mock.calls.map(([event]) => event.data.mutation.id);
        ...
        it('does not retain a status snapshot read while a settings refresh is in flight', async () => {
            ...
            const invokeWorkspaceCommand = vi.fn(() => refresh.promise);
            const svc = createDaemonWorkspaceService(makeDeps({
                queryWorkspaceStatus,
                persistDisabledSkills: vi.fn().mockResolvedValue({
                    changed: true,
            ...
            await vi.waitFor(() => expect(invokeWorkspaceCommand).toHaveBeenCalledOnce());
            await expect(svc.getWorkspaceSkillsStatus(makeCtx())).resolves.toEqual(oldStatus);
            refresh.resolve({ sessionsRefreshed: 1, sessionsFailed: 0 });
            await toggle;
            await expect(svc.getWorkspaceSkillsStatus(makeCtx())).resolves.toEqual(newStatus);
        ...
        it('publishes an explicit enabled override for a default-disabled skill', async () => {
            ...
            await expect(svc.setWorkspaceSkillEnabled(makeCtx(), 'review', true)).resolves.toMatchObject({
                skillName: 'review',
                enabled: true,
                activation: 'applied',
            });
        ...
        it('reports partial activation when a session refresh fails', async () => {
            ...
            await expect(svc.setWorkspaceSkillEnabled(makeCtx(), 'review', false)).resolves.toMatchObject({
                activation: 'partial',
                sessionsRefreshed: 1,
                sessionsFailed: 1,
            });
        ...
        it('defers refresh when no child exists or the child closes mid-refresh', async () => {
            ...
            const noChild = createDaemonWorkspaceService(makeDeps({
                queryWorkspaceStatus: statusQuery(),
                persistDisabledSkills: vi.fn().mockResolvedValue({
                    changed: true,
                    disabled: ['review'],
            ...
            await expect(noChild.setWorkspaceSkillEnabled(makeCtx(), 'review', false)).resolves.toMatchObject({
                activation: 'deferred',
                sessionsRefreshed: 0,
                sessionsFailed: 0,
            });
            expect(noChildCommand).not.toHaveBeenCalled();
            const closedChild = createDaemonWorkspaceService(makeDeps({
                queryWorkspaceStatus: statusQuery(),
                persistDisabledSkills: vi.fn().mockResolvedValue({
                    changed: true,
                    disabled: ['review'],
                ...
                invokeWorkspaceCommand: vi
                    .fn()
                    .mockRejectedValue(new BridgeChannelClosedError('mid-request (workspace status)')),
                isChannelLive: () => true,
            }));
            await expect(closedChild.setWorkspaceSkillEnabled(makeCtx(), 'review', false)).resolves.toMatchObject({
                activation: 'deferred',
                sessionsRefreshed: 0,
                sessionsFailed: 0,
            });
        ...
        it('does not refresh or publish an idempotent mutation', async () => {
            ...
            await expect(svc.setWorkspaceSkillEnabled(makeCtx(), 'review', false)).resolves.toMatchObject({ changed: false, activation: 'applied' });
        ...
        it('rejects unknown, hidden, and inactive extension skills before persisting', async () => {
            ...
            const unknown = createDaemonWorkspaceService(makeDeps({
                queryWorkspaceStatus: statusQuery(),
                persistDisabledSkills,
            }));
            await expect(unknown.setWorkspaceSkillEnabled(makeCtx(), 'missing', false)).rejects.toBeInstanceOf(WorkspaceSkillNotFoundError);
            const hidden = createDaemonWorkspaceService(makeDeps({
                queryWorkspaceStatus: statusQuery(skillStatus({ userInvocable: false })),
                persistDisabledSkills,
            }));
            await expect(hidden.setWorkspaceSkillEnabled(makeCtx(), 'review', false)).rejects.toMatchObject({
                reason: 'not_user_invocable',
            });
            const inactive = createDaemonWorkspaceService(makeDeps({
                queryWorkspaceStatus: statusQuery(skillStatus({
                    status: 'disabled',
            ...
            await expect(inactive.setWorkspaceSkillEnabled(makeCtx(), 'review', true)).rejects.toMatchObject({
        ...
        it('rejects a legacy inactive extension skill with no disabledReason and not disabled by settings', async () => {
            await withIsolatedWorkspace(async ({ workspace }) => {
                ...
                await expect(svc.setWorkspaceSkillEnabled(makeCtx(), 'review', true)).rejects.toMatchObject({
                    reason: 'inactive_extension',
                });
                expect(persistDisabledSkills).not.toHaveBeenCalled();
            });
        ...
        it('allows toggling a legacy extension skill that is disabled by settings', async () => {
            await withIsolatedWorkspace(async ({ workspace }) => {
                await writeJson(path.join(workspace, SETTINGS_DIRECTORY_NAME, 'settings.json'), { skills: { disabled: ['review'] } });
                const persistDisabledSkills = vi.fn().mockResolvedValue({
                    changed: true,
                    disabled: [],
                });
                ...
                expect(persistDisabledSkills).toHaveBeenCalledWith(workspace, 'review', true, undefined);
            });
        });
        it('does not refresh or publish when persistence fails', async () => {
            ...
            await expect(svc.setWorkspaceSkillEnabled(makeCtx(), 'review', false)).rejects.toThrow('disk full');
            expect(invokeWorkspaceCommand).not.toHaveBeenCalled();
            expect(publishWorkspaceEvent).not.toHaveBeenCalled();
        });
    });
    describe('setWorkspaceSkillsEnabled', () => {
        ...
        it('accepts enabling a Skill before installation as an idempotent result', async () => {
            ...
            await expect(svc.setWorkspaceSkillsEnabled(makeCtx(), ['future-skill'], true)).resolves.toMatchObject({
                results: [{ skillName: 'future-skill', enabled: true, changed: false }],
                errors: [],
            });
            expect(persistDisabledSkillsBatch).toHaveBeenCalledWith('/workspace', ['future-skill'], true, undefined);
        });
        it('persists and refreshes once while preserving ordered target outcomes', async () => {
            ...
            const persistDisabledSkillsBatch = vi.fn().mockResolvedValue({
                outcomes: [
                    ...
                    {
                        skillName: 'locked',
                        error: new WorkspaceSkillNotToggleableError('locked', 'locked', 'user'),
                    },
            ...
            const result = await svc.setWorkspaceSkillsEnabled(makeCtx({ originatorClientId: 'client-1' }), ['Review', 'missing', 'hidden', 'inactive', 'locked', 'deploy'], false);
            expect(queryWorkspaceStatus).toHaveBeenCalledOnce();
            expect(persistDisabledSkillsBatch).toHaveBeenCalledOnce();
            expect(persistDisabledSkillsBatch).toHaveBeenCalledWith('/workspace', ['review', 'missing', 'locked', 'deploy'], false, undefined);
        ...
        it('orders results and errors by request targets, not persist outcomes', async () => {
            ...
            const result = await svc.setWorkspaceSkillsEnabled(makeCtx(), ['review', 'locked', 'missing', 'deploy'], false);
        ...
        it('fails the whole batch when persistence fails unexpectedly', async () => {
            ...
            await expect(svc.setWorkspaceSkillsEnabled(makeCtx(), ['review'], false)).rejects.toThrow('disk full');
            expect(invokeWorkspaceCommand).not.toHaveBeenCalled();
            expect(publishWorkspaceEvent).not.toHaveBeenCalled();
        });
        it('fails the whole batch when a persisted outcome is missing for a valid target', async () => {
            ...
            await expect(svc.setWorkspaceSkillsEnabled(makeCtx(), ['review', 'deploy'], false)).rejects.toThrow('Missing persisted Skill batch outcome: deploy');
            expect(invokeWorkspaceCommand).not.toHaveBeenCalled();
            expect(publishWorkspaceEvent).not.toHaveBeenCalled();
        });
        it('returns validation errors without persisting when no target is valid', async () => {
            ...
            await expect(svc.setWorkspaceSkillsEnabled(makeCtx(), ['hidden', 'inactive'], false)).resolves.toMatchObject({
                activation: 'applied',
                sessionsRefreshed: 0,
                sessionsFailed: 0,
                results: [],
        ...
        it('rejects a legacy inactive extension skill like the single-toggle path', async () => {
            await withIsolatedWorkspace(async ({ workspace }) => {
                ...
                await expect(svc.setWorkspaceSkillsEnabled(makeCtx(), ['legacy-inactive'], false)).resolves.toMatchObject({
                    results: [],
                    errors: [
                        {
        ...
        it('allows batch-toggling a legacy extension skill disabled by settings', async () => {
            await withIsolatedWorkspace(async ({ workspace }) => {
                await writeJson(path.join(workspace, SETTINGS_DIRECTORY_NAME, 'settings.json'), { skills: { disabled: ['legacy-inactive'] } });
                const persistDisabledSkillsBatch = vi.fn().mockResolvedValue({
                ...
                await expect(svc.setWorkspaceSkillsEnabled(makeCtx(), ['legacy-inactive'], true)).resolves.toMatchObject({
                    enabled: true,
                    results: [
                ...
                expect(persistDisabledSkillsBatch).toHaveBeenCalledWith(workspace, ['legacy-inactive'], true, undefined);
            });
        });
        it('passes enabled:true through to persistence, results, and events', async () => {
            ...
            const result = await svc.setWorkspaceSkillsEnabled(makeCtx({ originatorClientId: 'client-1' }), ['review'], true);
            expect(persistDisabledSkillsBatch).toHaveBeenCalledWith('/workspace', ['review'], true, undefined);
            expect(result).toMatchObject({
                enabled: true,
        ...
        it('publishes one settings_changed event per settingsChanges entry in order', async () => {
            ...
            await svc.setWorkspaceSkillsEnabled(makeCtx({ originatorClientId: 'client-1' }), ['review'], true);
            expect(publishWorkspaceEvent).toHaveBeenCalledTimes(2);
            expect(publishWorkspaceEvent).toHaveBeenNthCalledWith(1, skillToggleSettingsChanged({
                key: 'skills.disabled',
                value: undefined,
                skills: [{ name: 'review', enabled: true }],
                activation: 'deferred',
            ...
            expect(publishWorkspaceEvent).toHaveBeenNthCalledWith(2, skillToggleSettingsChanged({
                key: 'skills.enabled',
                value: ['review'],
                skills: [{ name: 'review', enabled: true }],
                activation: 'deferred',
        ...
        it('reports partial activation when the shared batch refresh fails', async () => {
            const publishWorkspaceEvent = vi.fn();
            const failedSessions = createDaemonWorkspaceService(makeDeps({
                queryWorkspaceStatus: vi.fn().mockResolvedValue({
                    v: 1,
                    workspaceCwd: '/workspace',
                    initialized: true,
            ...
            await expect(failedSessions.setWorkspaceSkillsEnabled(makeCtx(), ['review'], false)).resolves.toMatchObject({
                activation: 'partial',
                sessionsRefreshed: 1,
                sessionsFailed: 1,
            });
            ...
            const unexpectedError = createDaemonWorkspaceService(makeDeps({
                queryWorkspaceStatus: vi.fn().mockResolvedValue({
                    v: 1,
                    workspaceCwd: '/workspace',
                    initialized: true,
            ...
            await expect(unexpectedError.setWorkspaceSkillsEnabled(makeCtx(), ['review'], false)).resolves.toMatchObject({
                activation: 'partial',
                sessionsRefreshed: 0,
                sessionsFailed: 1,
            });
        ...
        it('defers batch refresh when sessions or the channel disappear', async () => {
            const missingSession = createDaemonWorkspaceService(makeDeps({
                queryWorkspaceStatus: vi.fn().mockResolvedValue({
                    v: 1,
                    workspaceCwd: '/workspace',
                    initialized: true,
            ...
            await expect(missingSession.setWorkspaceSkillsEnabled(makeCtx(), ['review'], false)).resolves.toMatchObject({
                activation: 'deferred',
                sessionsRefreshed: 0,
                sessionsFailed: 0,
            });
            const closedChannel = createDaemonWorkspaceService(makeDeps({
                queryWorkspaceStatus: vi.fn().mockResolvedValue({
                    v: 1,
                    workspaceCwd: '/workspace',
                    initialized: true,
                ...
                invokeWorkspaceCommand: vi
                    .fn()
                    .mockRejectedValue(new BridgeChannelClosedError('mid-request (batch toggle)')),
                isChannelLive: () => true,
            }));
            await expect(closedChannel.setWorkspaceSkillsEnabled(makeCtx(), ['review'], false)).resolves.toMatchObject({
                activation: 'deferred',
                sessionsRefreshed: 0,
                sessionsFailed: 0,
            });
            const deadChannelCommand = vi.fn();
            const deadChannel = createDaemonWorkspaceService(makeDeps({
                queryWorkspaceStatus: vi.fn().mockResolvedValue({
                    v: 1,
                    workspaceCwd: '/workspace',
                    initialized: true,
            ...
            await expect(deadChannel.setWorkspaceSkillsEnabled(makeCtx(), ['review'], false)).resolves.toMatchObject({
                activation: 'deferred',
                sessionsRefreshed: 0,
                sessionsFailed: 0,
            });
        ...
        it('does not refresh or publish when every batch target is unchanged', async () => {
            ...
            await expect(svc.setWorkspaceSkillsEnabled(makeCtx(), ['review', 'deploy'], false)).resolves.toMatchObject({
                activation: 'applied',
                sessionsRefreshed: 0,
            });
            expect(invokeWorkspaceCommand).not.toHaveBeenCalled();
        ...
        it('drops the cached skill snapshot after a changed batch like the single-toggle path', async () => {
            ...
            await expect(svc.getWorkspaceSkillsStatus(makeCtx())).resolves.toEqual(beforeStatus);
            await svc.setWorkspaceSkillsEnabled(makeCtx(), ['review'], false);
            await expect(svc.getWorkspaceSkillsStatus(makeCtx())).resolves.toEqual(afterStatus);
            expect(queryWorkspaceStatus).toHaveBeenCalledTimes(2);
        });
        it('does not retain a status snapshot read while a batch settings refresh is in flight', async () => {
            ...
            const toggle = svc.setWorkspaceSkillsEnabled(makeCtx(), ['review'], false);
            await vi.waitFor(() => expect(invokeWorkspaceCommand).toHaveBeenCalledOnce());
            await expect(svc.getWorkspaceSkillsStatus(makeCtx())).resolves.toEqual(beforeStatus);
            refresh.resolve({ sessionsRefreshed: 1, sessionsFailed: 0 });
            await toggle;
    ...
    describe('requestWorkspaceTrustChange', () => {
        it('publishes trust_change_requested with originatorClientId', async () => {
            const publishWorkspaceEvent = vi.fn();
            const svc = createDaemonWorkspaceService(makeDeps({ boundWorkspace: '/my/workspace', publishWorkspaceEvent }));
            const result = await svc.requestWorkspaceTrustChange(makeCtx({ originatorClientId: 'c-42' }), { desiredState: 'untrusted', reason: 'remote user request' });
            expect(publishWorkspaceEvent).toHaveBeenCalledWith({
    ...
    describe('refreshExtensionsForAllSessions', () => {
        it('delegates to the all-session refresh callback', async () => {
            ...
            const svc = createDaemonWorkspaceService(makeDeps({ invokeWorkspaceCommand, refreshExtensionsForAllSessions }));
        ...
        it('returns a failed result when the refresh callback rejects', async () => {
            ...
            const svc = createDaemonWorkspaceService(makeDeps({ refreshExtensionsForAllSessions }));
            await expect(svc.refreshExtensionsForAllSessions()).resolves.toEqual({
                refreshed: 0,
                failed: 1,
            });
    ...
    describe('preheatAcpChild', () => {
        it('returns the current ACP channel status', async () => {
            const svc = createDaemonWorkspaceService(makeDeps({ isChannelLive: () => true }));
        ...
        it('returns ready without preheating when the channel is already live', async () => {
            ...
            const svc = createDaemonWorkspaceService(makeDeps({ isChannelLive: () => true, preheatAcpChild }));
        ...
        it('returns an error when ACP preheat is unavailable', async () => {
            const svc = createDaemonWorkspaceService(makeDeps({ isChannelLive: () => false }));
        ...
        it('preheats the ACP child when the channel is not live', async () => {
            ...
            const svc = createDaemonWorkspaceService(makeDeps({ isChannelLive: () => live, preheatAcpChild }));
            const result = await svc.preheatAcpChild(makeCtx());
            expect(result).toMatchObject({ ready: true, channelLive: true });
            expect(preheatAcpChild).toHaveBeenCalledOnce();
        });
        it('returns timeout when preheat does not finish before the deadline', async () => {
            const preheatAcpChild = vi.fn(() => new Promise(() => undefined));
            const svc = createDaemonWorkspaceService(makeDeps({ isChannelLive: () => false, preheatAcpChild }));
        ...
        it('returns an error when preheat resolves without a live channel', async () => {
            ...
            await expect(svc.preheatAcpChild(makeCtx(), { timeoutMs: 1000 })).resolves.toMatchObject({
                ready: false,
                channelLive: false,
                reason: 'error',
                error: 'ACP preheat did not produce a live channel',
            });
            expect(mockWriteStderrLine).toHaveBeenCalledWith('qwen serve: ACP preheat resolved without a live channel');
        });
        it('returns error when preheat fails before the deadline', async () => {
            ...
            expect(mockWriteStderrLine).toHaveBeenCalledWith('qwen serve: ACP preheat failed: preheat failed');
        });
        it('sanitizes a synchronous bridge failure', async () => {
            ...
            expect(mockWriteStderrLine).toHaveBeenCalledWith('qwen serve: ACP preheat failed: child command contained a secret');
        });
    });
    describe('restartMcpServer', () => {
        it('calls invokeWorkspaceCommand with correct method and params', async () => {
            ...
            expect(invokeWorkspaceCommand).toHaveBeenCalledWith('qwen/control/workspace/mcp/restart', { serverName: 'myServer' }, { timeoutMs: 300_000 });
        });
        it('passes entryIndex when provided', async () => {
            ...
            expect(invokeWorkspaceCommand).toHaveBeenCalledWith('qwen/control/workspace/mcp/restart', { serverName: 'poolServer', entryIndex: 3 }, { timeoutMs: 300_000 });
        });
        ...
        it('publishes mcp_server_restart_refused event when restarted is false', async () => {
            ...
            const invokeWorkspaceCommand = vi.fn().mockResolvedValue(invokeResult);
            const svc = createDaemonWorkspaceService(makeDeps({ invokeWorkspaceCommand, publishWorkspaceEvent }));
            await svc.restartMcpServer(makeCtx({ originatorClientId: 'c-1' }), 'blocked');
        ...
        it('translates mcp_server_not_found errorKind into McpServerNotFoundError', async () => {
            ...
            const svc = createDaemonWorkspaceService(makeDeps({ invokeWorkspaceCommand }));
            await expect(svc.restartMcpServer(makeCtx(), 'ghost')).rejects.toThrow(/ghost/);
        });
        it('translates mcp_restart_failed errorKind into McpServerRestartFailedError', async () => {
            ...
            const invokeWorkspaceCommand = vi.fn().mockRejectedValue(err);
            const svc = createDaemonWorkspaceService(makeDeps({ invokeWorkspaceCommand }));
            await expect(svc.restartMcpServer(makeCtx(), 'broken')).rejects.toThrow(/broken/);
        });
        it('re-throws non-errorKind errors without translation', async () => {
            const err = new Error('generic boom');
            const invokeWorkspaceCommand = vi.fn().mockRejectedValue(err);
            const svc = createDaemonWorkspaceService(makeDeps({ invokeWorkspaceCommand }));
            await expect(svc.restartMcpServer(makeCtx(), 'srv')).rejects.toThrow('generic boom');
        });
        it('lets SessionNotFoundError pass through for 404 mapping', async () => {
            const err = new SessionNotFoundError('some-session-id');
            const invokeWorkspaceCommand = vi.fn().mockRejectedValue(err);
            const svc = createDaemonWorkspaceService(makeDeps({ invokeWorkspaceCommand }));
            await expect(svc.restartMcpServer(makeCtx(), 'my-mcp-server')).rejects.toThrow(SessionNotFoundError);
        });
        it('fans out per-entry events in pool-mode', async () => {
            ...
            const invokeWorkspaceCommand = vi.fn().mockResolvedValue(invokeResult);
            const svc = createDaemonWorkspaceService(makeDeps({ invokeWorkspaceCommand, publishWorkspaceEvent }));
            await svc.restartMcpServer(makeCtx({ originatorClientId: 'c-pool' }), 'pool-srv');
            expect(publishWorkspaceEvent).toHaveBeenCalledTimes(2);
            expect(publishWorkspaceEvent).toHaveBeenCalledWith(expect.objectContaining({
                type: 'mcp_server_restarted',
    ...
    describe('reload', () => {
        it('stops before child reload when the runtime generation closes', async () => {
            ...
            const reloadDaemonEnv = vi.fn(async (_workspace, commitGuard) => {
                commitGuard?.();
            ...
            expect(reloadDaemonEnv).toHaveBeenCalledWith('/workspace', assertGenerationOpen);
            expect(invokeWorkspaceCommand).not.toHaveBeenCalled();
            expect(publishWorkspaceEvent).not.toHaveBeenCalled();
        });
    });
    describe('initWorkspace', () => {
        ...
        it('rechecks the runtime generation before creating the context file', async () => {
            ...
            await expect(svc.initWorkspace(makeCtx({ workspaceCwd: tmpDir }), {})).rejects.toThrow('generation closed');
        ...
        it('throws when file has content and force is not set', async () => {
            ...
            await expect(svc.initWorkspace(makeCtx(), {})).rejects.toThrow(/already exists/);
        });
        ...
        it('throws for escaping filename', async () => {
            ...
            await expect(svc.initWorkspace(makeCtx(), {})).rejects.toThrow(/resolves outside/);
        });
        ...
        it('throws when target is a non-regular file', async () => {
            ...
            try {
                await expect(svc.initWorkspace(makeCtx(), {})).rejects.toThrow(/not a regular file/);
            }
            finally {
        ...
        it('throws WorkspaceInitRaceError when fs.open hits EEXIST', async () => {
            ...
            vi.spyOn(fs, 'open').mockImplementation(async (filePath, flags) => {
                ...
                return origOpen(filePath, flags);
            });
            try {
                await expect(svc.initWorkspace(makeCtx(), {})).rejects.toThrow(/appeared.*between/);
            }
            finally {
                vi.restoreAllMocks();
            }
        ...
        it('throws WorkspaceInitSymlinkError when overwrite open hits ELOOP', async () => {
            ...
            try {
                await expect(svc.initWorkspace(makeCtx(), { force: true })).rejects.toThrow(/O_NOFOLLOW.*ELOOP|symlink/i);
            }
            finally {
                vi.restoreAllMocks();
            }
        ...
        it('throws WorkspaceInitRaceError when overwrite open hits ENOENT', async () => {
            ...
            try {
                await expect(svc.initWorkspace(makeCtx(), { force: true })).rejects.toThrow(/deleted.*between|concurrent/i);
            }
            finally {
                vi.restoreAllMocks();
            }
        ...
        it('parent symlink outside workspace is rejected', async () => {
            ...
            await expect(svc.initWorkspace(makeCtx(), {})).rejects.toThrow(/parent.*resolves outside|parent.*workspace/i);
        });
    });
});
