...
// OAuth Endpoints
const QWEN_OAUTH_BASE_URL = 'https://chat.lailatulcoder.ai';
...
function createTokenRefreshNetworkError(error, timedOut) {
    ...
    return new Error(`${prefix}: ${formatFetchErrorForUser(error, {
...
export class QwenOAuthPollError extends Error {
    ...
    description;
    constructor(opts) {
        super(`Device token poll failed: ${opts.oauthError ?? 'Unknown error'} - ${opts.description ?? '(no description)'}`);
...
export class QwenOAuth2Client {
    ...
    async getAccessToken() {
        try {
            ...
            debugLogger.warn('Failed to get access token from shared manager:', error);
    ...
    async refreshAccessToken() {
        ...
        try {
            ...
            try {
                ...
                throw new Error(`Qwen OAuth refresh returned invalid JSON: ${responseText || '(empty response body)'}`);
            }
            // Check if the response indicates success
        ...
        if (error instanceof TokenManagerError) {
            switch (error.type) {
                case TokenError.NO_REFRESH_TOKEN:
                    debugLogger.debug('No refresh token available, proceeding with device flow');
                    break;
                case TokenError.REFRESH_FAILED:
                    debugLogger.debug('Token refresh failed, proceeding with device flow');
                    break;
                case TokenError.NETWORK_ERROR:
                    debugLogger.warn('Network error during token refresh, trying device flow');
                    break;
                default:
                    debugLogger.warn('Token manager error:', error.message);
            }
        }
        if (options?.requireCachedCredentials) {
            throw new Error('Qwen OAuth credentials expired. Please use /auth to re-authenticate with qwen-oauth.');
        }
 ...
 */
export function showFallbackMessage(verificationUriComplete, out = process.stderr) {
    ...
    // Build content lines
    const instructionLines = wrapText('Please visit the following URL in your browser to authorize:', contentWidth);
    ...
    out.write('| ' + waitingLine + ' '.repeat(contentWidth - waitingLine.length) + ' |\n');
    out.write(emptyLine + '\n');
...
async function authWithQwenDeviceFlow(client, config) {
    ...
    const emitAuthProgress = (status, message) => {
        ...
        for (let attempt = 0; attempt < maxAttempts; attempt++) {
            ...
            try {
                ...
                if (isDeviceTokenSuccess(tokenResponse)) {
                    ...
                    debugLogger.debug('Authentication successful! Access token obtained.');
                    return { success: true };
                }
...
export async function cacheQwenCredentials(credentials, opts) {
    ...
    try {
        ...
        try {
            ...
            try {
                ...
                if (process.platform !== 'win32') {
                    throw new Error(`cacheQwenCredentials: refusing to publish credentials — chmod 0o${QWEN_CREDENTIAL_FILE_MODE.toString(8)} on temp file failed: ${chmodErr instanceof Error ? chmodErr.message : String(chmodErr)}`);
                }
                ...
                debugLogger.warn(`cacheQwenCredentials: chmod 0o${QWEN_CREDENTIAL_FILE_MODE.toString(8)} on Windows temp file ${tempPath} failed; relying on NTFS ACL: ${chmodErr instanceof Error ? chmodErr.message : String(chmodErr)}`);
            }
        ...
        try {
            ...
            debugLogger.warn(`cacheQwenCredentials: SharedTokenManager.clearCache failed; in-process callers may serve stale credentials until the next mtime poll: ${clearErr instanceof Error ? clearErr.message : String(clearErr)}`);
        }
    }
    catch (error) {
...
export async function clearQwenCredentials() {
    try {
        ...
        debugLogger.warn('Warning: Failed to clear cached Qwen credentials:', error);
    }
    finally {
