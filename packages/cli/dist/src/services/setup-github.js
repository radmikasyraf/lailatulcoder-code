...
import { getGitHubRepoInfoAsync, getGitRepoRootAsync, getLatestGitHubRelease, isGitHubRepositoryAsync, } from '../utils/gitUtils.js';
...
export const GITHUB_WORKFLOW_PATHS = [
    'LailatulCoder-dispatch/LailatulCoder-dispatch.yml',
    'LailatulCoder-assistant/LailatulCoder-invoke.yml',
    'issue-triage/LailatulCoder-triage.yml',
    'issue-triage/LailatulCoder-scheduled-triage.yml',
    'pr-review/LailatulCoder-review.yml',
];
const GITIGNORE_ENTRIES = ['.lailatulcoder/', 'gha-creds-*.json'];
...
export async function setupGithub(options = {}) {
    ...
    if (!(await isGitHubRepositoryAsync({ cwd }))) {
        throw new SetupGithubError('github_repository_not_found', 'Unable to determine the GitHub repository. /setup-github must be run from a git repository.', 400);
    }
    let gitRepoRoot;
    try {
    ...
    if (options.workspaceRoot) {
        ...
        if (gitRootReal !== workspaceReal) {
            throw new SetupGithubError('github_git_root_mismatch', 'The Git repository root must match the daemon workspace root.', 400);
        }
    }
    ...
    try {
        ...
        writeStderrLine(`LailatulCoder setup-github: failed to determine latest lailatul-coder-action release: ${error instanceof Error ? error.message : String(error)}`);
        debugLogger.debug('Failed to determine latest lailatul-coder-action release:', error);
        throw new SetupGithubError('github_release_lookup_failed', 'Unable to determine the latest lailatul-coder-action release on GitHub.', 502);
    }
    ...
    try {
        ...
        for (const workflow of downloads) {
            const relativePath = path.posix.join('.github', 'workflows', path.posix.basename(workflow.sourcePath));
            try {
                const write = await fileOps.writeTextFile(gitRepoRoot, relativePath, workflow.content);
                ...
                if (isWorkspaceGenerationClosed(error))
                    throw error;
                result.partial = true;
                result.workflows.push({
                    sourcePath: workflow.sourcePath,
                ...
                throw new SetupGithubError('github_workflow_write_failed', `Unable to write ${relativePath}.`, 500, result);
            }
        }
    }
    catch (error) {
        ...
        if (error instanceof SetupGithubError)
            throw error;
        throw new SetupGithubError('github_workflow_write_failed', 'Unable to create .github/workflows.', 500, result);
    }
    return result;
}
export async function updateGitignore(gitRepoRoot, fileOps = nodeFileOps) {
    try {
        const existingContent = await fileOps.readTextFile(gitRepoRoot, '.gitignore');
        ...
        const missingEntries = GITIGNORE_ENTRIES.filter((entry) => !existingContent
            .split(/\r?\n/)
...
async function downloadWorkflows(options) {
    ...
    try {
        ...
        const dispatcher = options.proxy
            ...
            : undefined;
        return await Promise.all(GITHUB_WORKFLOW_PATHS.map(async (workflow) => {
        ...
        throw new SetupGithubError('github_workflow_download_failed', `Unable to download lailatul-coder-action workflows from GitHub. ${message}`, 502);
    }
}
async function readResponseTextWithLimit(response, sourcePath) {
    ...
    if (contentLength !== null) {
        ...
        if (Number.isFinite(parsedLength) &&
            ...
            throw new Error(`${sourcePath} exceeds download limit of ${MAX_WORKFLOW_DOWNLOAD_BYTES} bytes`);
        }
    }
    if (!response.body)
        return '';
