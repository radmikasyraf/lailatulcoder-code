...
import { buildShellExecWarnings, getCommandRoot, getShellConfiguration, hasUnsafeMonitorBackgroundOperator, normalizeMonitorCommand as normalizeMonitorShellCommand, splitCommands, } from '../utils/shell-utils.js';
import { createDebugLogger } from '../utils/debugLogger.js';
import { isSubpaths } from '../utils/paths.js';
import { getMonitorOutputPath, } from '../services/monitorRegistry.js';
import { MAX_CONCURRENT_MONITORS } from '../services/monitorRegistry.js';
import { extractCommandRules, isShellCommandReadOnlyASTInDirectory, } from '../utils/shellAstParser.js';
 ...
 */
export function sanitizeMonitorLine(line) {
    let cleaned = '';
    for (let i = 0; i < line.length; i++) {
        ...
        if (code < 0x20)
            continue; // C0 controls (NUL, BEL, ESC, etc.)
...
class MonitorToolInvocation extends BaseToolInvocation {
    ...
    async getDefaultPermission() {
        ...
        try {
            const isReadOnly = await isShellCommandReadOnlyASTInDirectory(command, cwd);
            if (isReadOnly) {
                return 'allow';
            }
        }
    ...
    async getConfirmationDetails(_abortSignal) {
        ...
        for (const sub of subCommands) {
            ...
            try {
                ...
                debugLogger.warn('AST read-only check failed for monitor sub-command, falling back to ask:', e);
            }
            if (isReadOnly) {
                continue;
            }
        ...
        try {
            ...
            permissionRules = [...new Set(allRules)].map((rule) => `Monitor(${rule})`);
        }
        catch (e) {
        ...
        // trigger the warning.
        const warnings = buildShellExecWarnings(normalized.safetyCommand, this.params.command);
    ...
    async execute(_signal) {
        ...
        if (normalized.strippedTrailingAmp) {
            debugLogger.warn('Stripped trailing & from monitor command — monitor lifecycle handles backgrounding');
        }
        ...
        const maxEvents = Math.min(this.params.max_events ?? DEFAULT_MAX_EVENTS, MAX_MAX_EVENTS);
        const idleTimeoutMs = Math.min(this.params.idle_timeout_ms ?? DEFAULT_IDLE_TIMEOUT_MS, MAX_IDLE_TIMEOUT_MS);
        ...
        const registration = {
            ...
            outputFile: getMonitorOutputPath(this.config.storage.getProjectDir(), this.config.getSessionId(), monitorId),
        ...
        let earlySpawnError;
        const captureEarlySpawnError = (err) => {
            earlySpawnError ??= err;
        };
        ...
        const throttledEmit = (line) => {
            ...
            if (elapsed < 0) {
                ...
                debugLogger.warn(`Monitor ${monitorId}: clock moved backwards by ${-elapsed}ms, resetting refill timestamp`);
                lastRefill = now;
            }
            else {
        ...
        const killChildProcessGroup = () => {
            ...
            if (process.platform === 'win32') {
                const tk = spawn('taskkill', ['/pid', child.pid.toString(), '/f', '/t'], { stdio: 'ignore' });
                tk.on('error', (err) => debugLogger.warn(`Monitor taskkill failed for pid ${child.pid}: ${getErrorMessage(err)}`));
            }
            else {
                try {
                    ...
                    debugLogger.warn(`Monitor ${monitorId} SIGTERM failed (pid=${child.pid}): ${getErrorMessage(err)}`);
                }
                setTimeout(() => {
                    if (!exited && child.pid) {
                        try {
                            ...
                            debugLogger.warn(`Monitor ${monitorId} SIGKILL escalation failed (pid=${child.pid}): ${getErrorMessage(err)}`);
                        }
                    }
                }, 200).unref?.();
            }
        ...
        const processLines = (buffer, data) => {
            ...
            for (const line of lines) {
                ...
                if (trimmed.length === 0)
                    continue;
                const truncated = trimmed.length > PARTIAL_LINE_BUFFER_CAP
        ...
        const cleanup = () => {
            ...
            if (registration.droppedLines > 0) {
                debugLogger.info(`Monitor ${monitorId} dropped ${registration.droppedLines} lines due to throttling`);
            }
        };
        let exitResult = null;
        const settleFromExit = (code, sig) => {
            if (registration.status !== 'running')
...
export class MonitorTool extends BaseDeclarativeTool {
    ...
    constructor(config) {
        super(MonitorTool.Name, ToolDisplayNames.MONITOR, 'Starts a long-running shell command and streams its stdout/stderr as event notifications back to you.\n\n' +
            'Use this tool for:\n' +
            ...
            '- Commands with no output (use run_shell_command with is_background: true instead)', Kind.Execute, {
            type: 'object',
            properties: {
                command: {
                    type: 'string',
    ...
    validateToolParamValues(params) {
        ...
        if (params.max_events !== undefined) {
            if (typeof params.max_events !== 'number' ||
        ...
        if (params.idle_timeout_ms !== undefined) {
            if (typeof params.idle_timeout_ms !== 'number' ||
    ...
    toAutoClassifierInput(params) {
        ...
        if (params.directory)
            out['directory'] = params.directory;
        return out;
    }
}
