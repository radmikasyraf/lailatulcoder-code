 ...
 * mutating.
 */
export function omitSkillDetailsForSdkSurface(event) {
    ...
    if (!meta || !('availableSkillDetails' in meta))
        return event;
    const trimmedMeta = {};
    for (const [key, value] of Object.entries(meta)) {
        if (key !== 'availableSkillDetails')
    ...
    if (flat)
        return { ...event, data: nextCandidate };
 ...
 */
export function omitSkillDetailsFromReplayArrays(session) {
    if (!session.compactedReplay && !session.liveJournal)
        return session;
    return {
        ...session,
        ...(session.compactedReplay
            ? {
                compactedReplay: session.compactedReplay.map(omitSkillDetailsForSdkSurface),
            }
            : {}),
        ...(session.liveJournal
            ? {
...
//# sourceMappingURL=skill-details-redaction.js.map