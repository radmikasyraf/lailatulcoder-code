...
export class SchemaValidator {
    ...
    static validate(schema, data) {
        ...
        if (!valid && validate.errors) {
            ...
            fixBooleanValues(data, anySchema, anySchema);
            ...
            fixStringValues(data, anySchema, anySchema);
            ...
            fixStringifiedJsonValues(data, anySchema, anySchema);
            ...
            fixNumericValues(data, anySchema);
            valid = validate(data);
            if (!valid && validate.errors) {
 ...
 */
function resolvePropSchema(propSchema, rootSchema) {
    if (!propSchema)
        return undefined;
 ...
 */
function resolveArrayElementSchema(parentSchema, rootSchema, index) {
 ...
 */
function getCompositionVariants(schema, rootSchema) {
    const variants = [];
            ...
            for (const item of val) {
                if (typeof item === 'object' && item != null && !Array.isArray(item)) {
                    ...
                    const resolved = rootSchema
                        ? resolvePropSchema(item, rootSchema)
                        : item;
                    if (resolved)
 ...
 */
function schemaHasObjectShape(schema, rootSchema) {
    if (schema['properties'] || schema['additionalProperties'])
        return true;
    for (const variant of getCompositionVariants(schema, rootSchema)) {
        if (variant['properties'] || variant['additionalProperties'])
            return true;
    }
    return false;
}
...
function getEffectiveProperties(schema, rootSchema) {
 ...
 */
function getAcceptedTypes(propSchema, rootSchema, depth = 0, 
        ...
        for (const keyword of ['anyOf', 'oneOf', 'allOf']) {
            ...
            if (Array.isArray(variants)) {
                for (const variant of variants) {
                    const nested = getAcceptedTypes(variant, rootSchema, depth + 1, cache);
                    if (nested) {
 ...
 */
function valueToSchemaType(value) {
    if (value === null)
        return 'null';
    ...
    if (typeof value === 'number') {
        if (!Number.isFinite(value))
            return null; // NaN/Infinity — don't coerce
 ...
 */
function typeIsAccepted(valueType, accepted) {
    if (accepted.has(valueType))
        return true;
    // integer is a subtype of number
    if (valueType === 'integer' && accepted.has('number'))
        return true;
    return false;
}
/**
 ...
 */
function fixStringifiedJsonValuesInArray(array, itemsSchema, root, key) {
                ...
                if (elAccepted && !elAccepted.has('string')) {
                    try {
                        ...
                        if (elAccepted.has(parsedType)) {
                            debugLogger.debug(`coercion: ${key}[${i}] = ${String(item)} → ${JSON.stringify(parsed)} (accepted: ${[...elAccepted].join('|')})`);
                            array[i] = parsed;
                        }
                    }
                    catch {
    ...
    if (!resolvedItems)
        return;
    const innerItems = resolvedItems['items'];
        ...
        if ((trimmed.startsWith('[') && trimmed.endsWith(']')) ||
            ...
            try {
                ...
                if (itemsAccepted.has(parsedType)) {
                    debugLogger.debug(`coercion: ${key}[${i}] = ${String(item)} → ${JSON.stringify(parsed)} (accepted: ${[...itemsAccepted].join('|')})`);
                    array[i] = parsed;
                }
            }
            catch {
 ...
 * are also repaired.
 */
function fixStringifiedJsonValues(data, schema, rootSchema, depth = 0) {
    ...
    if (!properties && !schema['additionalProperties'])
        return;
    for (const key of Object.keys(data)) {
        ...
        const resolved = resolvePropSchema(propSchema, root);
        if (!resolved)
            continue;
        // Recurse into nested objects
        if (typeof value === 'object' && value !== null && !Array.isArray(value)) {
            if (schemaHasObjectShape(resolved, root)) {
                fixStringifiedJsonValues(value, resolved, root, depth + 1);
            }
            continue;
        }
        // Recurse into arrays
                ...
                for (let i = 0; i < value.length; i++) {
                    ...
                    if (typeof item === 'object' &&
                        ...
                        if (schemaHasObjectShape(elSchema, root)) {
                            fixStringifiedJsonValues(item, elSchema, root, depth + 1);
                        }
                    }
                    else if (Array.isArray(item)) {
                        if (elSchema['type'] === 'array' ||
                            elSchema['items'] ||
                    ...
                    if (schemaHasObjectShape(resolvedItems, root)) {
                        ...
                        for (let i = 0; i < value.length; i++) {
                            ...
                            if (typeof item === 'object' &&
                                item !== null &&
                                !Array.isArray(item)) {
                                fixStringifiedJsonValues(item, resolvedItems, root, depth + 1);
                            }
                            else if (typeof item === 'string' &&
                                itemsAccepted &&
                                ...
                                if ((trimmed.startsWith('{') && trimmed.endsWith('}')) ||
                                    ...
                                    try {
                                        const parsed = JSON.parse(trimmed);
                                        if (itemsAccepted.has(Array.isArray(parsed) ? 'array' : typeof parsed)) {
                                            debugLogger.debug(`coercion: ${key}[${i}] = ${String(item)} → ${JSON.stringify(parsed)} (accepted: ${[...itemsAccepted].join('|')})`);
                                            value[i] = parsed;
                                        }
                                    }
                        ...
                        for (const subArray of value) {
                            if (Array.isArray(subArray)) {
                                fixStringifiedJsonValuesInArray(subArray, resolvedItems, root, key);
                            }
                        }
                    }
                    else {
        ...
        if ((trimmed.startsWith('[') && trimmed.endsWith(']')) ||
            ...
            try {
                ...
                if (accepted.has(parsedType)) {
                    debugLogger.debug(`coercion: ${key} = ${String(value)} → ${JSON.stringify(parsed)} (accepted: ${[...accepted].join('|')})`);
                    data[key] = parsed;
                }
            }
            catch {
...
function fixNumericValues(data, schema) {
    ...
    for (const key of Object.keys(data)) {
        ...
        if (wantsInteger && !wantsNumber && num % 1 !== 0)
            continue;
 ...
 */
function fixBooleanValues(data, schema, rootSchema, depth = 0) {
    ...
    for (const key of Object.keys(data)) {
        ...
        const resolved = propSchema
            ? resolvePropSchema(propSchema, root)
            : undefined;
        if (typeof value === 'object' && value !== null) {
            if (Array.isArray(value)) {
                if (Array.isArray(resolved?.['prefixItems'])) {
                    ...
                    for (let i = 0; i < value.length; i++) {
                        ...
                        if (typeof item === 'object' &&
                            ...
                            !Array.isArray(item)) {
                            if (schemaHasObjectShape(elSchema, root)) {
                                fixBooleanValues(item, elSchema, root, depth + 1);
                            }
                        }
                        else if (typeof item === 'string') {
                            const elAccepted = getAcceptedTypes(elSchema, root);
                            ...
                            if (elAccepted?.has('boolean') && !elAccepted.has('string')) {
                                ...
                                if (lower === 'true' || lower === 'false') {
                                    debugLogger.debug(`coercion: ${key}[${i}] = ${JSON.stringify(item)} → ${lower === 'true'} (accepted: ${[...elAccepted].join('|')})`);
                                    value[i] = lower === 'true';
                                }
                            }
                        }
                        ...
                        if (schemaHasObjectShape(resolvedItems, root)) {
                            ...
                            for (const item of value) {
                                if (typeof item === 'object' &&
                                    item !== null &&
                                    !Array.isArray(item)) {
                                    fixBooleanValues(item, resolvedItems, root, depth + 1);
                                }
                            }
                        }
                        else {
                            ...
                            if (itemsAccepted?.has('boolean') &&
                                ...
                                for (let i = 0; i < value.length; i++) {
                                    ...
                                    if (typeof item === 'string') {
                                        const lower = item.toLowerCase();
                                        if (lower === 'true' || lower === 'false') {
                                            debugLogger.debug(`coercion: ${key}[${i}] = ${JSON.stringify(item)} → ${lower === 'true'} (accepted: ${[...itemsAccepted].join('|')})`);
                                            value[i] = lower === 'true';
                                        }
                                    }
                                }
                ...
                if (resolved && schemaHasObjectShape(resolved, root)) {
                    fixBooleanValues(value, resolved, root, depth + 1);
                }
            }
            continue;
        }
        if (typeof value === 'string') {
            ...
            if (accepted?.has('boolean') && !accepted.has('string')) {
                debugLogger.debug(`coercion: ${key} = ${JSON.stringify(value)} → ${lower === 'true'} (accepted: ${[...accepted].join('|')})`);
                data[key] = lower === 'true';
            }
        }
    }
 ...
 */
function fixStringValues(data, schema, rootSchema, depth = 0) {
    // Guard against stack overflow from deeply nested data.
    if (depth > 64)
        return;
    const root = rootSchema ?? schema;
    ...
    for (const key of Object.keys(data)) {
        ...
        if (typeof value === 'object' && value !== null) {
            if (Array.isArray(value)) {
                if (Array.isArray(resolved['prefixItems'])) {
                    ...
                    for (let i = 0; i < value.length; i++) {
                        ...
                        if (typeof item === 'object' &&
                            ...
                            !Array.isArray(item)) {
                            if (schemaHasObjectShape(elSchema, root)) {
                                fixStringValues(item, elSchema, root, depth + 1);
                            }
                        }
                        else if (item !== null &&
                            item !== undefined &&
                            ...
                            if (elAccepted?.has('string')) {
                                ...
                                if (currentType && !typeIsAccepted(currentType, elAccepted)) {
                                    coerced ??= [...value];
                                    debugLogger.debug(`coercion: ${key}[${i}] = ${String(item)} → ${JSON.stringify(String(item))} (accepted: ${[...elAccepted].join('|')})`);
                            ...
                            if (schemaHasObjectShape(resolvedItems, root)) {
                                ...
                                for (const item of value) {
                                    if (typeof item === 'object' &&
                                        item !== null &&
                                        !Array.isArray(item)) {
                                        fixStringValues(item, resolvedItems, root, depth + 1);
                                    }
                                }
                            }
                            else {
                                    ...
                                    for (let i = 0; i < value.length; i++) {
                                        ...
                                        if (item !== null &&
                                            ...
                                            if (currentType &&
                                                ...
                                                coerced ??= [...value];
                                                debugLogger.debug(`coercion: ${key}[${i}] = ${String(item)} → ${JSON.stringify(String(item))} (accepted: ${[...itemsAccepted].join('|')})`);
                                                coerced[i] = String(item);
                                            }
                                        }
                                    }
                ...
                if (schemaHasObjectShape(resolved, root)) {
                    fixStringValues(value, resolved, root, depth + 1);
                }
            }
            continue;
        }
        ...
        if (typeof value === 'number' ||
            ...
            if (accepted?.has('string')) {
                ...
                if (currentType && !typeIsAccepted(currentType, accepted)) {
                    debugLogger.debug(`coercion: ${key} = ${String(value)} → ${JSON.stringify(String(value))} (accepted: ${[...accepted].join('|')})`);
