...
export async function collectMemoryDiagnostics(options = {}) {
    ...
    const [openFileDescriptors, smapsRollup, heapSpaceStatistics, processTree] = await Promise.all([
        optionalProbe('openFileDescriptors', options.openFileDescriptors ?? countOpenFileDescriptors),
        optionalProbe('smapsRollup', options.smapsRollup ?? readProcSmapsRollup),
        optionalSyncProbe('heapSpaceStatistics', options.heapSpaceStatistics ?? (() => v8.getHeapSpaceStatistics())),
        optionalProbe('processTree', options.processTree ?? (() => collectProcessTreeMemoryUsage(platform))),
    ]);
    ...
    const diagnostics = {
        ...
        qwenVersion: options.lailatulcoderVersion,
        uptimeSeconds,
        memoryUsage,
        ...
        processTree,
        activeHandles: getProcessInternalCount('activeHandles', '_getActiveHandles', options.activeHandles),
        activeRequests: getProcessInternalCount('activeRequests', '_getActiveRequests', options.activeRequests),
        openFileDescriptors,
        smapsRollup,
        platform,
...
function normalizeMaxRSSBytes(maxRSSKiB) {
...
function getProcessInternalCount(name, internalMethod, probe) {
    try {
        if (probe) {
            return probe();
        }
        ...
        if (!Array.isArray(result)) {
            logProbeFailure(name, new Error(`${internalMethod} returned a non-array result`));
            return 0;
        }
...
async function collectProcessTreeMemoryUsage(platform) {
    if (platform === 'win32') {
...
async function optionalProbe(name, probe) {
    try {
        return await probe();
    }
    catch (error) {
...
async function optionalSyncProbe(name, probe) {
    try {
        return probe();
    }
    catch (error) {
...
function analyzeMemoryDiagnostics(diagnostics) {
    ...
    if (nativeMemory >= NATIVE_MEMORY_PRESSURE_MIN_BYTES &&
    ...
    if (diagnostics.memoryUsage.heapUsed > 0 &&
