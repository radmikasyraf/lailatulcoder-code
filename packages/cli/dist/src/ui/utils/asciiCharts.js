...
function computeThresholds(data) {
    ...
    if (values.length === 0)
        return [1, 2, 4, 8];
    const p = (pct) => values[Math.floor(values.length * pct)] || values[values.length - 1];
    return [p(0.25), p(0.5), p(0.75), p(0.9)];
}
export function buildHeatmapData(data, weeks = 52, monthOffset = 0) {
    const now = new Date();
    ...
    for (const cm of colMondays) {
        ...
        if (nextMonth !== undefined && nextMonth - cm.col <= 1)
            continue;
...
export function buildBrailleLineChart(data, chartWidth, chartHeight = 8) {
    if (data.length === 0)
        return null;
    ...
    for (let y = 0; y < pixelH; y++) {
        pixels.push(new Array(pixelW).fill(false));
    }
    const mappedPoints = sorted.map((p, i) => {
        const px = sorted.length === 1
            ? Math.floor(pixelW / 2)
...
//# sourceMappingURL=asciiCharts.js.map