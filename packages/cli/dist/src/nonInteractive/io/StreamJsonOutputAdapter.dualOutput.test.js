...
describe('StreamJsonOutputAdapter — dual-output extensions', () => {
    ...
    describe('custom outputStream injection', () => {
        it('writes to the injected stream instead of stdout', () => {
            ...
            const adapter = new StreamJsonOutputAdapter(mockConfig, false, customStream);
    ...
    describe('emitPermissionRequest / emitControlResponse', () => {
        it('emits a control_request with subtype can_use_tool', () => {
            ...
            adapter.emitPermissionRequest('req-1', 'run_shell_command', 'tool-use-1', { command: 'ls' }, '/etc/passwd');
        ...
        it('emits supplied warning-aware permission suggestions', () => {
            ...
            adapter.emitPermissionRequest('req-warning', 'run_shell_command', 'tool-warning', { command: "python -c 'print(1)'" }, null, suggestions);
            const parsed = JSON.parse(stdoutWriteSpy.mock.calls[0][0]);
...
//# sourceMappingURL=StreamJsonOutputAdapter.dualOutput.test.js.map