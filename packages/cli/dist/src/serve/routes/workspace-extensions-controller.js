...
import { ExtensionManager, redactUrlCredentials, stripAnsiAndControl, } from '@lailatul-coder/lailatul-coder-core';
import { loadSettings } from '../../config/settings.js';
import { getWorkspaceTrustStatus } from '../../config/trustedFolders.js';
import { detectSystemLanguage, resolveLanguageSetting, } from '../../i18n/index.js';
...
const sanitizeDaemonMessage = (message) => redactUrlCredentials(stripAnsiAndControl(message));
export const redactExtensionDisplaySource = (source) => {
    const redacted = redactUrlCredentials(source);
    if (redacted.startsWith('upload:'))
        return redacted;
...
const resolveExtensionLocale = (workspaceDir, workspaceTrusted) => {
    const configuredLanguage = loadSettings(workspaceDir, workspaceTrusted === undefined
        ? true
        : {
...
export function createExtensionsController(deps) {
    ...
    const createExtensionManager = (workspaceDir = boundWorkspace, trustedOverride, interactions) => {
        ...
        return new ExtensionManager({
            ...
            requestSetting: interactions?.requestSetting ??
                (async (setting) => {
                    throw new Error(`Extension setting "${setting.envVar}" requires interactive configuration and is not supported over the daemon install endpoint.`);
                }),
            requestChoicePlugin: interactions?.requestChoicePlugin ??
                (async () => {
                    throw new Error('Marketplace plugin selection is not supported over the daemon install endpoint. Specify a plugin name in the source.');
                }),
        });
    };
    const validateExtensionMutationClient = (req, res, opts = {}) => {
        const clientId = parseAndValidateWorkspaceClientId(req, res, opts.bridges ?? bridge);
        if (clientId === null)
            return false;
        if (clientId === undefined && opts.requireClientId !== false) {
            res.status(400).json({
                error: 'Missing X-LailatulCoder-Client-Id header',
    ...
    const isTerminalExtensionOperation = (operation) => operation.status !== 'queued' &&
        operation.status !== 'running' &&
        operation.status !== 'waiting_for_input';
    const redactExtensionOperationResult = (event) => ({
        ...event,
        ...(event.source
            ? { source: redactExtensionDisplaySource(event.source) }
            : {}),
    });
    const bridgeMutationEvent = (event) => {
    ...
    const pruneExtensionOperations = () => {
        const terminalCount = () => [...extensionOperations.values()].filter(isTerminalExtensionOperation)
            .length;
    ...
    const rememberExtensionOperation = (operation) => {
        extensionOperations.set(operation.operationId, operation);
        pruneExtensionOperations();
    };
    const updateExtensionOperation = (operationId, patch) => {
    ...
    const refreshExtensionsForAllSessions = async () => {
        const queueAbort = new AbortController();
        let releaseCommitLane;
        const refresh = commitQueue.runUntilReleased(async (release) => {
        ...
        try {
            return await Promise.race([
                ...
                new Promise((_resolve, reject) => {
                    timer = setTimeout(() => {
                        const error = new Error(`extension refresh timed out after ${EXTENSION_REFRESH_TIMEOUT_MS}ms`);
                        releaseCommitLane?.();
    ...
    const runQueuedExtensionMutation = (operation, failureContext, res, run, options = {}) => {
        const assertGenerationOpen = options.assertGenerationOpen ?? deps.captureGenerationAssertion?.();
        ...
        const operationBasePath = options.operationBasePath ?? '/workspace/extensions/operations';
        try {
            res
                .status(202)
        ...
        void (async () => {
            ...
            const runReconciliation = async (task) => {
            ...
            try {
                ...
                const extensionManager = options.manager ??
                ...
                const startDeadline = () => {
                    ...
                    if (options.deadlineMs) {
                        deadline = setTimeout(() => {
                            const error = new Error(`Extension ${operation} exceeded its ${options.deadlineMs}ms preparation deadline.`);
                ...
                const context = {
                    prepare: async (task) => {
                        ...
                        try {
                            const prepared = await preparationQueue.run(async () => {
                                try {
                                    assertGenerationOpen?.();
                    ...
                    commit: async (task) => {
                        ...
                        const result = await commitQueue.runUntilReleased(async (release) => {
                            assertGenerationOpen?.();
                ...
                const event = await run(extensionManager, deadlineController.signal, context, operationId);
                mutationEvent = event;
                if (deadline)
                ...
                if (options.skipRefresh || event.updated === false) {
                    ...
                    reconciliationReservation = undefined;
                    updateExtensionOperation(operationId, {
                        status: commitWarnings.length > 0
                ...
                if (committedGeneration === undefined) {
                    committedGeneration = (await extensionManager.getExtensionStoreSnapshot()).generation;
                    reconciliationReservation ??=
                        options.reserveRuntimeReconciliation?.();
                }
                updateExtensionOperation(operationId, {
                ...
                const refreshTargets = typeof options.refreshRuntimes === 'function'
                    ? options.refreshRuntimes()
                    : options.refreshRuntimes;
                if (refreshTargets) {
                    const results = await runReconciliation(async () => await Promise.all(refreshTargets.map(async (runtime) => {
                        ...
                        try {
                            ...
                            try {
                                return {
                                    status: 'fulfilled',
                                    result: await runtime.bridge.refreshExtensionsForAllSessions(bridgeMutationEvent(event)),
                    ...
                    for (let index = 0; index < results.length; index += 1) {
                        ...
                        if (settled.status === 'fulfilled') {
                            ...
                            failed += 1;
                            const message = sanitizeDaemonMessage(settled.reason instanceof Error
                    ...
                    try {
                        const { result, elapsedMs } = await runReconciliation(async () => {
                            ...
                            const startedAt = Date.now();
                            try {
                                const result = await bridge.refreshExtensionsForAllSessions(bridgeMutationEvent(event));
                        ...
                        writeStderrLine(`LailatulCoder serve: [${boundWorkspace}] extensions ${operation}: refreshed ${result.refreshed} session(s), ${result.failed} failed`);
                    }
                    catch (refreshErr) {
                        const message = sanitizeDaemonMessage(refreshErr instanceof Error
                            ? refreshErr.message
                            : String(refreshErr));
                        updateExtensionOperation(operationId, {
                        ...
                        try {
                            ...
                            writeStderrLine(`LailatulCoder serve: [${boundWorkspace}] extensions ${operation}: failed to broadcast refresh failure: ${sanitizeDaemonMessage(broadcastErr instanceof Error
                                ? broadcastErr.message
                                : String(broadcastErr))}`);
                        }
                        writeStderrLine(`LailatulCoder serve: [${boundWorkspace}] extensions ${operation}: mutation succeeded but refresh failed: ${message}`);
                    }
                }
            }
            catch (err) {
                const message = sanitizeDaemonMessage(err instanceof Error ? err.message : String(err));
                const code = err &&
                    typeof err === 'object' &&
                ...
                if (committedGeneration !== undefined) {
                    extensionsStatusCache = undefined;
                    const error = `Commit succeeded but post-commit work failed: ${message}`.slice(0, 500);
                    const warnings = [
                        ...commitWarnings,
                        {
                            workspaceCwd: boundWorkspace,
                    ...
                    try {
                        ...
                        warnings.push({
                            ...
                            error: sanitizeDaemonMessage(invalidationError instanceof Error
                                ? invalidationError.message
                                : String(invalidationError)).slice(0, 500),
                        });
                    }
                    updateExtensionOperation(operationId, {
                        status: 'succeeded_with_warnings',
                    ...
                    try {
                        writeStderrLine(`LailatulCoder serve: [${boundWorkspace}] extensions ${operation}: ${error}`);
                    }
                    catch {
                ...
                try {
                    ...
                    writeStderrLine(`LailatulCoder serve: [${boundWorkspace}] extensions ${operation}: failed to broadcast failure: ${sanitizeDaemonMessage(broadcastErr instanceof Error
                        ? broadcastErr.message
                        : String(broadcastErr))}`);
                }
                try {
                    writeStderrLine(`LailatulCoder serve: [${boundWorkspace}] extensions ${operation}: background task failed: ${message}`);
                }
                catch {
                    // Keep queued background work from surfacing as unhandledRejection.
                }
            ...
            return {
                ...
                ...(ext.installMetadata?.type === 'snapshot'
                    ...
                    : ext.installMetadata?.credentialPersistence === 'stored'
                        ...
                        : {}),
                updateState: ext.installMetadata?.type === 'snapshot'
                    ? 'not updatable'
                    : ext.installMetadata
                        ? 'unknown'
                        : 'not updatable',
    ...
    return {
        ...
        getActiveOperations: () => [...extensionOperations.values()].filter((operation) => !isTerminalExtensionOperation(operation)),
...
//# sourceMappingURL=workspace-extensions-controller.js.map