...
import { observeToolResultBoundary, toolResultBoundaryArtifact, toolResultPartDiagnosticValues, } from './tool-result-boundary-diagnostics.js';
import { normalizeToolResultCallId, persistAndTruncateToolResult, } from './truncation.js';
const debugLogger = createDebugLogger('TOOL_RESPONSE_FINALIZER');
const associatedFinalizerResponses = new WeakSet();
...
function associateFinalizerEntries(entries, indexes) {
    for (const index of indexes) {
        associatedFinalizerResponses.add(entries[index].responseParts);
    }
}
function observeFinalizerEntries(config, stage, entries, mutatedEntryIndexes, promptIds, entryIndexes) {
    for (let index = 0; index < entries.length; index++) {
        if (entryIndexes && !entryIndexes.has(index))
            continue;
        ...
        try {
            observeToolResultBoundary({
                ...
                artifacts: [
                    toolResultBoundaryArtifact(entry.persistedOutputFiles, entry.artifacts),
                ],
...
function collectTextSlots(entries, includeTopLevelText = true, excludeBudgetExemptOutput = true) {
    const slots = [];
        ...
        for (let partIndex = 0; partIndex < parts.length; partIndex++) {
            ...
            if (typeof output === 'string') {
                const protectedPrefix = excludeBudgetExemptOutput
                    ? getPlanModeLifecyclePrefix(part.functionResponse?.name ?? entry.toolName, output)
                    : undefined;
...
function allocateTextBudget(lengths, budget) {
...
function sliceStartWithoutBrokenSurrogate(text, length) {
...
function sliceEndWithoutBrokenSurrogate(text, length) {
...
function fitText(text, maxChars, persistedOutputFiles) {
    if (text.length <= maxChars)
        return text;
    if (maxChars <= 0)
        return '';
    const header = persistedOutputFiles && persistedOutputFiles.length > 0
    ...
    if (previewBudget <= marker.length) {
        return `${header}${separator}${sliceStartWithoutBrokenSurrogate(text, previewBudget)}`;
    }
    const contentBudget = previewBudget - marker.length;
    const headBudget = Math.floor(contentBudget / 5);
    const tailBudget = contentBudget - headBudget;
    return `${header}${separator}${sliceStartWithoutBrokenSurrogate(text, headBudget)}${marker}${sliceEndWithoutBrokenSurrogate(text, tailBudget)}`;
}
function replaceTextSlots(entries, slots, allocations) {
    ...
    for (let index = 0; index < slots.length; index++) {
        ...
        if (slot.text.length <= allocations[index])
            continue;
        const entry = result[slot.entryIndex];
        const part = entry.responseParts[slot.partIndex];
        const replacement = fitText(slot.text, allocations[index], entry.persistedOutputFiles);
        if (slot.field === 'text') {
...
export function toolResponseTextLength(parts) {
    return collectTextSlots([{ callId: '', toolName: '', responseParts: parts }], true, false).reduce((total, slot) => total + slot.text.length, 0);
}
export function enforceFunctionResponseBudget(entries, budget) {
    ...
    if (total <= budget)
        return entries;
    return replaceTextSlots(entries, slots, allocateTextBudget(slots.map((slot) => slot.text.length), budget));
}
export async function finalizeToolResponses(config, entries, promptIds, observeBoundary = true, associateBoundary = false) {
    ...
    const associatedEntryIndexes = observeBoundary
        ...
        : new Set();
    const observationIndexes = (mutatedEntryIndexes) => new Set(entries.flatMap((_, index) => mutatedEntryIndexes.has(index) || !associatedEntryIndexes.has(index)
        ? [index]
        : []));
    const observeUnchangedEntries = () => {
        ...
        observeFinalizerEntries(config, 'finalizer_input', entries, unchanged, promptIds, indexes);
        observeFinalizerEntries(config, 'finalizer_output', entries, unchanged, promptIds, indexes);
    };
    const budget = config.getToolOutputBatchBudget?.() ?? Number.POSITIVE_INFINITY;
    if (!Number.isFinite(budget) || budget <= 0) {
    ...
    const allocations = allocateTextBudget(slots.map((slot) => slot.text.length), budget);
    const entriesToPersist = new Set();
    for (let index = 0; index < slots.length; index++) {
    ...
    if (observeBoundary)
        observeFinalizerEntries(config, 'finalizer_input', entries, entriesToPersist, promptIds, indexes);
    const withPersistence = [...entries];
    const normalizedCallIds = entries.map((entry) => normalizeToolResultCallId(entry.callId));
    const normalizedCallIdCounts = new Map();
    for (const callId of normalizedCallIds) {
        if (!callId)
            continue;
        normalizedCallIdCounts.set(callId, (normalizedCallIdCounts.get(callId) ?? 0) + 1);
    }
    const reservedCallIds = new Set(normalizedCallIds.filter((callId) => !!callId));
    const usedCallIds = new Set();
    for (const entryIndex of entriesToPersist) {
        const entry = withPersistence[entryIndex];
        if (entry.persistedOutputFiles !== undefined)
            continue;
        const content = slots
        ...
        try {
            ...
            if (normalizedCallId) {
                if (normalizedCallIdCounts.get(normalizedCallId) === 1 &&
            ...
            const persisted = await persistAndTruncateToolResult(persistenceCallId, entry.toolName, content, config);
    ...
    if (observeBoundary)
        observeFinalizerEntries(config, 'finalizer_output', finalized, entriesToPersist, promptIds, indexes);
    if (shouldAssociateBoundary) {
        associateFinalizerEntries(finalized, new Set(finalized.keys()));
    }
    const finalizedTotal = collectTextSlots(finalized).reduce((sum, slot) => sum + slot.text.length, 0);
    debugLogger.info(`Tool response budget (${budget} chars): reduced ${entriesToPersist.size} result(s) from ${total} to ${finalizedTotal} chars.`);
    return finalized;
}
//# sourceMappingURL=tool-response-finalizer.js.map