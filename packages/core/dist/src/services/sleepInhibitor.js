...
function isHeadlessSshSession(env) {
 ...
 */
function sanitizeInhibitorReason(reason) {
...
export class SleepInhibitor {
    activeCount = 0;
    child;
    spawnFailedForCurrentRun = false;
    exitedWhileActiveLogged = false;
    platform;
    env;
    spawn;
    logger;
    noAskPasswordSupported;
    probing = false;
    constructor(config = {}) {
    ...
    acquire(reason = 'LailatulCoder Ai is processing a request') {
        this.activeCount += 1;
        ...
        try {
            ...
            child.once('exit', (code, signal) => {
                ...
                if (this.activeCount > 0 &&
                    ...
                    this.logger.debug(`Sleep inhibitor exited while active: code=${String(code)} signal=${String(signal)}`);
                }
            });
        }
        catch (error) {
            this.spawnFailedForCurrentRun = true;
            this.logger.debug(`Failed to spawn sleep inhibitor: ${error instanceof Error ? error.message : String(error)}`);
        }
    }
    /**
    ...
    stop() {
        ...
        try {
            ...
            this.logger.warn(`Failed to stop sleep inhibitor: ${error instanceof Error ? error.message : String(error)}`);
        }
    }
    getCommand(reason) {
        switch (this.platform) {
            ...
            case 'linux': {
                ...
                args.push('--what=sleep', '--who=LailatulCoder Ai', `--why=${sanitizeInhibitorReason(reason)}`, '--mode=block', 'sleep', 'infinity');
...
export function acquireSleepInhibitor(config, reason) {
    if (config.getPreventSystemSleepEnabled?.() !== true) {
        return NOOP_HANDLE;
    }
    return sleepInhibitor.acquire(reason);
}
//# sourceMappingURL=sleepInhibitor.js.map