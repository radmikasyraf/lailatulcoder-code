 ...
 */
import { createBuildWorkspaceCtx, parseAndValidateWorkspaceClientId, } from '../server/request-helpers.js';
import { requireTrustedWorkspaceRuntime, resolveWorkspaceRuntimeFromParam, } from '../workspace-route-runtime.js';
import { MAX_WORKSPACE_SKILL_NAME_LENGTH, WorkspaceSkillManagementError, validateWorkspaceSkillName, } from '../workspace-skill-management.js';
const MAX_WORKSPACE_SKILL_BATCH_SIZE = 100;
function rejectSkillNameTooLong(skillName, res) {
...
function parseEnabledFlag(body, res) {
...
function parseSkillToggleRequest(req, res, safeBody) {
...
function parseSkillInstallRequest(req, res, safeBody) {
    const body = safeBody(req);
    ...
    if (rejectSkillNameTooLong(name.trim(), res))
        return undefined;
...
function sendSkillManagementError(res, error) {
...
export function registerWorkspaceSkillsRoutes(app, deps) {
    const buildWorkspaceCtx = createBuildWorkspaceCtx(deps.workspaceRuntime.workspaceCwd);
    const route = 'POST /workspace/skills/:name/enable';
    const batchRoute = 'POST /workspace/skills/enable';
    app.post('/workspace/skills/install', deps.mutate({ strict: true }), async (req, res) => {
        if (!requireTrustedWorkspaceRuntime(deps.workspaceRuntime, res))
            return;
        ...
        try {
            const result = await deps.workspaceRuntime.workspaceService.installWorkspaceSkill(buildWorkspaceCtx(installRoute, clientId), input);
    ...
    app.delete('/workspace/skills/:name', deps.mutate({ strict: true }), async (req, res) => {
        if (!requireTrustedWorkspaceRuntime(deps.workspaceRuntime, res))
            return;
        const rawSkillName = req.params['name'];
        const scope = parseDeleteScope(req, res);
        if (!rawSkillName || !scope)
            return;
        let skillName;
        try {
        ...
        try {
            const result = await deps.workspaceRuntime.workspaceService.deleteWorkspaceSkill(buildWorkspaceCtx(deleteRoute, clientId), skillName, scope);
            res.status(200).json(result);
        }
        catch (err) {
            if (!sendSkillManagementError(res, err))
    ...
    app.post('/workspace/skills/enable', deps.mutate({ strict: true }), async (req, res) => {
        if (!requireTrustedWorkspaceRuntime(deps.workspaceRuntime, res))
            return;
        ...
        if (clientId === null)
            return;
        try {
            const result = await deps.workspaceRuntime.workspaceService.setWorkspaceSkillsEnabled(buildWorkspaceCtx(batchRoute, clientId), input.skillNames, input.enabled);
            res.status(200).json(result);
        }
        catch (err) {
    ...
    app.post('/workspace/skills/:name/enable', deps.mutate({ strict: true }), async (req, res) => {
        if (!requireTrustedWorkspaceRuntime(deps.workspaceRuntime, res))
            return;
        ...
        if (clientId === null)
            return;
        try {
            const result = await deps.workspaceRuntime.workspaceService.setWorkspaceSkillEnabled(buildWorkspaceCtx(route, clientId), input.skillName, input.enabled);
            res.status(200).json(result);
        }
        catch (err) {
...
export function registerWorkspaceQualifiedSkillsRoutes(app, deps) {
    const route = 'POST /workspaces/:workspace/skills/:name/enable';
    const batchRoute = 'POST /workspaces/:workspace/skills/enable';
    app.post('/workspaces/:workspace/skills/install', deps.mutate({ strict: true }), async (req, res) => {
        ...
        try {
            const result = await runtime.workspaceService.installWorkspaceSkill(createBuildWorkspaceCtx(runtime.workspaceCwd)(installRoute, clientId), input);
            res.status(200).json(result);
        }
        catch (err) {
            if (!sendSkillManagementError(res, err))
    ...
    app.delete('/workspaces/:workspace/skills/:name', deps.mutate({ strict: true }), async (req, res) => {
        const runtime = resolveWorkspaceRuntimeFromParam(deps.workspaceRegistry, req, res);
        if (!runtime || !requireTrustedWorkspaceRuntime(runtime, res))
            return;
        const rawSkillName = req.params['name'];
        ...
        try {
            const result = await runtime.workspaceService.deleteWorkspaceSkill(createBuildWorkspaceCtx(runtime.workspaceCwd)(deleteRoute, clientId), skillName, scope);
            res.status(200).json(result);
        }
        catch (err) {
            if (!sendSkillManagementError(res, err))
    ...
    app.post('/workspaces/:workspace/skills/enable', deps.mutate({ strict: true }), async (req, res) => {
        const runtime = resolveWorkspaceRuntimeFromParam(deps.workspaceRegistry, req, res);
        if (!runtime || !requireTrustedWorkspaceRuntime(runtime, res))
            return;
        const input = parseSkillBatchToggleRequest(req, res, deps.safeBody);
        ...
        if (clientId === null)
            return;
        try {
            const result = await runtime.workspaceService.setWorkspaceSkillsEnabled(createBuildWorkspaceCtx(runtime.workspaceCwd)(batchRoute, clientId), input.skillNames, input.enabled);
            res.status(200).json(result);
        }
        catch (err) {
            deps.sendBridgeError(res, err, { route: batchRoute });
    ...
    app.post('/workspaces/:workspace/skills/:name/enable', deps.mutate({ strict: true }), async (req, res) => {
        const runtime = resolveWorkspaceRuntimeFromParam(deps.workspaceRegistry, req, res);
        if (!runtime || !requireTrustedWorkspaceRuntime(runtime, res))
            return;
        const input = parseSkillToggleRequest(req, res, deps.safeBody);
        ...
        if (clientId === null)
            return;
        try {
            const result = await runtime.workspaceService.setWorkspaceSkillEnabled(createBuildWorkspaceCtx(runtime.workspaceCwd)(route, clientId), input.skillName, input.enabled);
            res.status(200).json(result);
        }
        catch (err) {
            deps.sendBridgeError(res, err, { route });
...
//# sourceMappingURL=workspace-skills.js.map