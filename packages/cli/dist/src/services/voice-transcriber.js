...
function normalizeBaseUrl(baseUrl, modelName) {
    ...
    if (url.username || url.password) {
        throw new Error(`Voice model '${modelName}' baseUrl must not contain embedded credentials.`);
    }
...
function isInsecureVoiceBaseUrlAllowed(settings, normalizedBaseUrl) {
    ...
    return (Array.isArray(allowed) &&
        allowed.some((candidate) => typeof candidate === 'string' &&
...
export async function assertVoiceBaseUrlNetworkAllowed(voiceConfig, lookupHost, abortSignal) {
    ...
    if (isIP(hostname) !== 0) {
        if (isAlwaysBlockedVoiceAddress(hostname) ||
            (!voiceConfig.allowInsecureBaseUrl && isPrivateNetworkIp(hostname))) {
            throw new Error(isLoopbackVoiceAddress(hostname)
    ...
    if (records.some((record) => isAlwaysBlockedVoiceAddress(record.address) ||
        (!voiceConfig.allowInsecureBaseUrl &&
            isPrivateNetworkIp(record.address)))) {
        throw new Error(records.some((record) => isLoopbackVoiceAddress(record.address))
...
export function resolveVoiceTranscriptionConfig({ config, settings, voiceModel, env, }) {
    ...
    const allowInsecureBaseUrl = isInsecureVoiceBaseUrlAllowed(settings, normalizedBaseUrl);
    if (parsedBaseUrl.protocol !== 'http:' &&
        parsedBaseUrl.protocol !== 'https:') {
        throw new Error(`Voice model '${voiceModel}' must use an http or https baseUrl.`);
    }
    if (!isLocalhost && isAlwaysBlockedVoiceAddress(parsedBaseUrl.hostname)) {
        throw new Error(isLoopbackVoiceAddress(parsedBaseUrl.hostname)
