...
export function getStickyTodosLayoutKey(todos, width, maxVisibleItems) {
    ...
    const orderedOpenTodos = getOrderedStickyTodos(todos).filter((todo) => todo.status !== 'completed');
...
// over-allocate.
function getStickyTodoVpCap(terminalHeight) {
    if (!Number.isFinite(terminalHeight) || terminalHeight <= 0) {
        return STICKY_TODO_VP_MIN_VISIBLE_ITEMS;
    }
    return Math.max(STICKY_TODO_VP_MIN_VISIBLE_ITEMS, Math.min(STICKY_TODO_VP_MAX_VISIBLE_ITEMS, Math.floor(terminalHeight / STICKY_TODO_VP_ROWS_PER_VISIBLE_ITEM)));
}
...
export function getStickyTodoMaxVisibleItemsForMode(terminalHeight, useTerminalBuffer) {
