 ...
 */
import { getErrorMessage, getMCPServerPrompts, } from '@lailatul-coder/lailatul-coder-core';
...
export class McpPromptLoader {
    ...
    loadCommands(_signal) {
        ...
        for (const serverName in mcpServers) {
            ...
            for (const prompt of prompts) {
                ...
                const description = prompt.description || `Invoke prompt ${prompt.name}`;
                const newPromptCommand = {
                    ...
                    completion: async (commandContext, partialArg) => {
                        ...
                        const argsString = indexOfFirstSpace === 0
                            ? ''
                        ...
                        // mirroring parseArgs.
                        const positionalTokens = argsString
                            .replace(namedArgRegex, ' ')
                            .match(/(?:"(?:\\.|[^"\\])*"|[^ ]+)/g) || [];
                        const positionalCount = positionalTokens.filter((token) => !token.startsWith('--')).length;
                        ...
                        for (const arg of prompt.arguments) {
                            if (providedArgNames.has(arg.name))
                                continue;
                            const flag = `--${arg.name}="`;
                            if (arg.required) {
                        ...
                        const unusedArguments = unusedRequired.length > 0
                            ? partialArg.length > 0
    ...
    parseArgs(userArgs, promptArgs) {
        ...
        if (unfilledArgs.length === 1 && positionalArgs.length > 0) {
            ...
            for (let i = 0; i < unfilledArgs.length && i < positionalArgs.length; i++) {
                promptInputs[unfilledArgs[i].name] = positionalArgs[i];
            }
        }
        const missingRequired = promptArgs.filter((arg) => arg.required && !Object.hasOwn(promptInputs, arg.name));
