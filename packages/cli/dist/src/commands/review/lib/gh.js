 ...
 */
// Thin wrapper around the GitHub CLI (`gh`) for the `LailatulCoder review`
...
const TRANSIENT_RE = /HTTP 5\d{2}|server is currently unavailable|service unavailable|bad gateway|internal server error/i;
    ...
    for (let attempt = 0;; attempt++) {
        try {
            ...
            if (attempt < MAX_RETRIES && isTransientGhError(err)) {
                ...
                process.stderr.write(`gh transient error (attempt ${attempt + 1}/${MAX_RETRIES}), retrying in ${delay}ms…\n`);
                sleepSync(delay);
                continue;
            }
            throw err;
 ...
 */
export function isOwnerRepo(repo) {
 ...
 */
export function setGhHost(host) {
    ...
    if (!HOSTNAME_RE.test(trimmed)) {
        throw new TypeError(`--host must be a hostname (optionally :port), got ${JSON.stringify(host)}`);
    }
    ghHost = trimmed;
}
/**
 ...
 */
export function resolveGhHost(flagHost) {
    return ((flagHost === undefined || flagHost === '' ? undefined : flagHost.trim()) ??
 ...
 */
export function ghWithInputRetried(input, ...args) {
 ...
 */
export function ghWithInput(input, ...args) {
 ...
 * throws.
 */
export function ghApiAllNested(path, key) {
 ...
 */
export function parseNdjson(out, opts = {}) {
...
export function ensureAuthenticated() {
    for (let attempt = 0;; attempt++) {
        try {
            ...
            throw new Error('gh CLI is not authenticated. Run `gh auth login` and retry.');
        }
    }
}
