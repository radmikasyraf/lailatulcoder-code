...
import { IdeClient, AuthType, createDebugLogger, formatMemoryUsage, } from '@lailatul-coder/lailatul-coder-core';
...
function probeVersion(binary) {
    return new Promise((resolve) => {
        execFile(binary, ['--version'], { timeout: VERSION_PROBE_TIMEOUT_MS, encoding: 'utf-8' }, (err, stdout) => {
            if (err) {
                resolve('unknown');
                return;
            }
 ...
 * For bug reports, removes 'LailatulCoder-' or 'lailatul-coder-' prefixes from sandbox names.
 *
 * @param stripPrefix - Whether to strip 'LailatulCoder-' prefix (used for bug reports)
 */
export function getSandboxEnv(stripPrefix = false) {
    ...
    // For bug reports, remove LailatulCoder- prefix
    if (stripPrefix) {
        return sandbox.replace(/^LailatulCoder-(?:code-)?/, '');
    }
    return sandbox;
}
/**
...
export async function getExtendedSystemInfo(context) {
    ...
    const contentGeneratorConfig = baseInfo.selectedAuthType === AuthType.USE_OPENAI ||
...
function getLspStatus(context) {
    try {
        ...
        if (context.services.config?.getDebugMode?.()) {
            debugLogger.debug('Unable to read LSP status snapshot for /status:', error);
        }
        return undefined;
    }
}
