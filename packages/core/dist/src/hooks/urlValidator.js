...
export class UrlValidator {
     ...
     */
    constructor(allowedPatterns = [], allowPrivateNetworkHosts = false) {
        this.allowedPatterns = allowedPatterns;
        this.allowPrivateNetworkHosts = allowPrivateNetworkHosts;
        this.compiledPatterns = allowedPatterns.map((pattern) => this.compilePattern(pattern));
    }
    /**
        ...
        try {
            ...
            if (this.isIpAddress(hostname)) {
                ...
                if (isMetadataAddress(cleanHostname)) {
                    debugLogger.debug(`URL blocked: IP ${hostname} is a cloud metadata endpoint`);
                    return true;
                }
 ...
 */
export function createUrlValidator(allowedUrls, allowPrivateNetworkHosts) {
