...
import { requiredAgents, reviewMode, isTerritoryFanOut, hasExecutableScript, } from './roster.js';
...
const keys = (plan) => requiredAgents(plan).map((a) => a.key);
...
describe('requiredAgents — Step 3A', () => {
    ...
    it('drops the adversarial personas when the plan records medium effort, but keeps every other dimension', () => {
        ...
        expect(med).toEqual(expect.arrayContaining(['0', '1a', '2', '3a', '3b', '3c', '4', '5', '7']));
        // High, and the default (no effort recorded), still demand them.
        expect(keys({ ...PR, effort: 'high' })).toEqual(expect.arrayContaining(['6a', '6b', '6c']));
    ...
    it('does not let repository context override the effort, topology, or mode gates', () => {
        ...
        expect(keys({ ...PR, repositoryContext: context(['6a']) }).filter((role) => role === '6a')).toHaveLength(1);
        ...
        expect(keys({ ...PR, repositoryContext: context(['test-matrix']) })).not.toContain('test-matrix');
    ...
    it('fails closed on a present-but-invalid repository context', () => {
        ...
        expect(() => keys({ ...PR, repositoryContext: future })).toThrow('unsupported repositoryContext version');
    });
...
describe('requiredAgents — Step 3B', () => {
    ...
    it('demands one agent per territory, plus the ones no territory can see', () => {
        expect(keys(BIG)).toEqual(expect.arrayContaining([
            'chunk-1',
            'chunk-2',
            'chunk-3',
            'test-matrix',
    ...
    it('demands three invariant agents per heavily-rewritten file', () => {
        ...
        expect(keys(heavy)).toEqual(expect.arrayContaining([
...
//# sourceMappingURL=roster.test.js.map