...
function toSoxError(error) {
    if (error.code === 'ENOENT') {
        return new Error('SoX is not installed or not on PATH. Install SoX and try again.');
    }
    return error;
}
function formatSoxExitError(code, stderr) {
    const detail = stderr.trim();
    return new Error(`Voice recorder failed with exit code ${code ?? 'unknown'}${detail ? `: ${detail}` : ''}.`);
}
class SoxRecorder {
    child = null;
    tmpDir = null;
    ...
    async start(options = {}) {
        ...
        child.stderr?.on('data', (chunk) => {
            if (this.stderr.length < MAX_STDERR_LENGTH) {
                this.stderr = (this.stderr + chunk.toString()).slice(0, MAX_STDERR_LENGTH);
            }
        });
...
export function createSoxRecorder() {
    return new SoxRecorder();
}
//# sourceMappingURL=sox-recorder.js.map