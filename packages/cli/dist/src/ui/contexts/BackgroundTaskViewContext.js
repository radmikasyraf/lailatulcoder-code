...
// ─── Context ────────────────────────────────────────────────
export const BackgroundTaskViewStateContext = createContext(null);
export const BackgroundTaskViewActionsContext = createContext(null);
...
// ─── Hooks ──────────────────────────────────────────────────
export function useBackgroundTaskViewState() {
    return useContext(BackgroundTaskViewStateContext) ?? DEFAULT_STATE;
}
export function useBackgroundTaskViewActions() {
    return useContext(BackgroundTaskViewActionsContext) ?? DEFAULT_ACTIONS;
}
export function BackgroundTaskViewProvider({ config, children, }) {
    ...
    const setLivePanelFocused = useCallback((focused) => {
        setLivePanelFocusedRaw(focused);
        if (focused)
            setLivePanelSelectedIndex(0);
    }, []);
    ...
    const cancelSelected = useCallback(() => {
        ...
        switch (target.kind) {
            ...
            case 'dream': {
                ...
                if (!ok) {
                    debugLogger.warn(`cancelSelected: dream task ${target.dreamId} could not be cancelled ` +
            ...
            default: {
                ...
                throw new Error(`cancelSelected: unknown DialogEntry kind: ${JSON.stringify(_exhaustive)}`);
            }
        }
    ...
    const toggleSelectedWorkflowPause = useCallback(() => {
        ...
        if (target.status === 'running')
            return registry.pause(target.runId);
        if (target.status === 'paused')
            return registry.resume(target.runId);
    ...
    return (_jsx(BackgroundTaskViewStateContext.Provider, { value: state, children: _jsx(BackgroundTaskViewActionsContext.Provider, { value: actions, children: children }) }));
}
//# sourceMappingURL=BackgroundTaskViewContext.js.map