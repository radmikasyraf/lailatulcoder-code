 ...
 */
/**
 * Device-flow authorization registry for `LailatulCoder serve`. The registry
...
//   \ufeff:         BOM / ZWNBSP
String.raw `[\x00-\x1f\x7f-\x9f\u200b-\u200f\u2028-\u202e\u2066-\u2069\ufeff]`, 'g');
export function sanitizeForStderr(value) {
...
export const DEVICE_FLOW_SUPPORTED_PROVIDERS = ['LailatulCoder-oauth'];
/**
 ...
 */
const SECRET_BRAND = Symbol('DeviceFlowSecret');
const SECRETS = new WeakMap();
export function brandSecret(value) {
 ...
 */
export function unsafeRevealSecret(secret) {
    ...
    if (value === undefined) {
        ...
        throw new Error('unsafeRevealSecret: argument is not a BrandedSecret (was never registered, or its WeakMap binding was lost via serialization)');
    }
    return value;
}
/**
...
export class UnsupportedDeviceFlowProviderError extends Error {
    ...
    constructor(providerId) {
        super(`Unsupported device-flow provider (internal: declared but not registered): ${providerId}`);
        this.name = 'UnsupportedDeviceFlowProviderError';
    }
}
export class TooManyActiveDeviceFlowsError extends Error {
    code = 'too_many_active_flows';
    constructor() {
        super(`Too many active device-flow attempts. Cancel one of the existing ` +
 ...
 */
function makeRegistryPollTimeoutError(timeoutMs) {
 ...
 */
const DEVICE_FLOW_REGISTRY_LOCAL = 'deviceFlowRegistry';
export function setDeviceFlowRegistry(app, registry) {
    app.locals[DEVICE_FLOW_REGISTRY_LOCAL] = registry;
}
export function getDeviceFlowRegistry(app) {
    return app.locals[DEVICE_FLOW_REGISTRY_LOCAL];
}
/**
...
export class DeviceFlowRegistry {
    ...
    constructor(deps) {
        ...
        this.sweeperHandle = this.scheduleInterval(DEVICE_FLOW_SWEEP_INTERVAL_MS, () => this.sweep());
        if (this.sweeperHandle &&
            typeof this.sweeperHandle.unref === 'function') {
    ...
    async doStart(params, provider) {
        ...
        try {
            startResult = await new Promise((resolve, reject) => {
                startTimer = this.schedule(DEVICE_FLOW_START_TIMEOUT_MS, () => {
                    ...
                    reject(new UpstreamDeviceFlowError('device-flow start timeout (upstream IdP unresponsive)'));
                });
                provider
        ...
        // first poll fires.
        const expiresInSec = Number.isFinite(startResult.expiresIn) && startResult.expiresIn > 0
            ? Math.min(DEVICE_FLOW_MAX_EXPIRES_IN_SEC, Math.max(DEVICE_FLOW_MIN_EXPIRES_IN_SEC, startResult.expiresIn))
            : 600;
        ...
        const intervalSec = Number.isFinite(startResult.interval) &&
            startResult.interval > 0
            ? startResult.interval
            : DEVICE_FLOW_DEFAULT_INTERVAL_MS / 1000;
        const intervalMs = Math.min(DEVICE_FLOW_MAX_INTERVAL_MS, Math.max(1_000, intervalSec * 1000));
        const entry = {
     ...
     * can map it to 404.
     */
    cancel(deviceFlowId, cancellerClientId) {
        const entry = this.byId.get(deviceFlowId);
        if (!entry)
            return undefined;
    ...
    schedulePoll(entry, provider) {
        ...
        if (entry.pollHandle &&
            typeof entry.pollHandle.unref === 'function') {
            entry.pollHandle.unref();
        }
    }
    async runPollTick(entry, provider) {
        if (entry.status !== 'pending')
            return;
        if (this.disposed)
            return;
        ...
        try {
            result = await new Promise((resolve, reject) => {
                pollTimer = this.schedule(DEVICE_FLOW_POLL_TIMEOUT_MS, () => {
                    ...
                    const timeoutError = makeRegistryPollTimeoutError(DEVICE_FLOW_POLL_TIMEOUT_MS);
                    try {
        ...
        if (pollTimedOut && providerPollPromise !== undefined) {
            ...
            void tracked
                ...
                audit?.record({
                    ...
                    hint: latePollResult.kind === 'error'
                ...
                if (lateErr instanceof DeviceFlowPollTimeoutError &&
                ...
                const safeDetail = lateErr instanceof Error
        ...
        switch (result.kind) {
            ...
            case 'slow_down':
                ...
                entry.intervalMs = Math.min(DEVICE_FLOW_MAX_INTERVAL_MS, entry.intervalMs + DEVICE_FLOW_SLOW_DOWN_BUMP_MS);
                this.deps.events.publish({
                    type: 'throttled',
                    data: {
                        deviceFlowId: entry.deviceFlowId,
            ...
            case 'success': {
                ...
                if (persistError === undefined) {
                    try {
                        ...
                        if (persistTimer !== undefined)
                            this.clearScheduled(persistTimer);
                ...
                if (persistError) {
                    ...
                    else if (this.transitionTerminal(entry, 'error', 'persist_failed')) {
     ...
     */
    recordTakeover(entry, takeoverClientId) {
        if (!takeoverClientId)
            return;
        if (takeoverClientId === entry.initiatorClientId)
            return;
        if (takeoverClientId === entry.lastOriginatorClientId)
            return;
     ...
     */
    transitionTerminal(entry, status, errorKind) {
        if (entry.status !== 'pending')
            return false;
        entry.status = status;
        if (errorKind)
...
function toPublicView(entry) {
    ...
    if (entry.errorKind)
        base.errorKind = entry.errorKind;
    if (entry.hint)
        base.hint = entry.hint;
    if (entry.lastPolledAt !== undefined)
        base.lastPolledAt = entry.lastPolledAt;
    if (entry.status === 'pending') {
