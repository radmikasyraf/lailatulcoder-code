 ...
 */
export function measureElementPosition(node) {
    const { yogaNode } = node;
    if (!yogaNode) {
 ...
 */
export function measureFrameHeight(node) {
    let root = node;
    let current = node;
    while (current) {
        root = current;
 ...
 */
export function layoutRowForEvent(node, terminalRow1Based, terminalHeight) {
    const anchor = frameAnchor(terminalHeight, measureFrameHeight(node));
    return terminalRowToLayoutRow(terminalRow1Based, anchor);
}
//# sourceMappingURL=measure-element-position.js.map