...
import { requireTrustedWorkspaceRuntime, resolveWorkspaceRuntimeWithLiveCompatibilityFromParam, sendConversationRuntimeUnavailable, } from '../workspace-route-runtime.js';
...
function isPortableInstanceName(name, allowReservedAll = false) {
    if (allowReservedAll && name.trim() === 'all')
        return true;
    const baseName = name.split('.', 1)[0].trimEnd();
    return (isWellFormed(name) &&
        name.trim().length > 0 &&
        name !== '.' &&
        name !== '..' &&
        name.trim() !== 'all' &&
...
function parseInstanceName(req, res, allowReservedAll = false) {
...
function parseRevision(body, res) {
...
function parseStartup(body, res) {
    const revision = parseRevision(body, res);
    if (!revision)
        return undefined;
...
function parsePairingCode(body, res) {
...
function parsePairingApprovalSubject(body, res) {
...
async function resolveTarget(req, res, resolveRuntime, resolveService, activity) {
    ...
    if (runtime.provenance === 'live-conversation' && req.method !== 'GET') {
        res.status(400).json({
            error: 'Channel configuration and lifecycle operations are unavailable in the Conversations workspace.',
    ...
    try {
        const run = (operation) => runtime.provenance === 'live-conversation'
...
export function registerWorkspaceChannelManagementRoutes(app, deps) {
    const primary = () => deps.primaryRuntime;
    const qualified = (req, res) => resolveWorkspaceRuntimeWithLiveCompatibilityFromParam(deps.workspaceRegistry, req, res);
    const register = (prefix, resolveRuntime) => {
        ...
        const target = (req, res) => resolveTarget(req, res, resolveRuntime, deps.resolveService, deps.conversationRuntimeActivity);
        const validateClient = (req, res, runtime) => deps.parseAndValidateClientId(req, res, runtime) !== null;
        app.get(`${prefix}/channel-types`, async (req, res) => {
            ...
            try {
                noStore(res);
                const catalog = runtime.provenance === 'live-conversation' &&
                    deps.conversationRuntimeActivity
                    ? await deps.conversationRuntimeActivity.run(() => supportedChannelCatalog())
        ...
        app.get(`${prefix}/channels`, async (req, res) => {
            ...
            if (!resolved || !validateClient(req, res, resolved.runtime))
                return;
            try {
                noStore(res);
        ...
        app.get(`${prefix}/channels/:name/pairing-requests`, pairingRead, async (req, res) => {
            const resolved = await target(req, res);
            if (!resolved || !validateClient(req, res, resolved.runtime))
                return;
            ...
            try {
                ...
                res
                    .status(200)
                    .json(await resolved.run(() => resolved.service.pairingRequests(name)));
            }
            catch (error) {
                sendManagementError(res, error);
            }
        });
        app.post(`${prefix}/channels/:name/pairing-requests/approve`, pairingApprove, async (req, res) => {
            const resolved = await target(req, res);
            if (!resolved || !validateClient(req, res, resolved.runtime))
                return;
            const name = parseInstanceName(req, res);
            ...
            try {
                ...
                res
                    .status(200)
                    .json(await resolved.run(() => resolved.service.approvePairing(name, code)));
            }
            catch (error) {
                sendManagementError(res, error);
            }
        });
        app.get(`${prefix}/channels/:name/pairing-approvals`, pairingApprovalsRead, async (req, res) => {
            const resolved = await target(req, res);
            if (!resolved || !validateClient(req, res, resolved.runtime))
                return;
            const name = parseInstanceName(req, res);
            ...
            try {
                ...
                res
                    .status(200)
                    .json(await resolved.run(() => resolved.service.pairingApprovals(name)));
            }
            catch (error) {
                sendManagementError(res, error);
            }
        });
        app.delete(`${prefix}/channels/:name/pairing-approvals`, pairingRevoke, async (req, res) => {
            const resolved = await target(req, res);
            if (!resolved || !validateClient(req, res, resolved.runtime))
                return;
            const name = parseInstanceName(req, res);
            ...
            try {
                ...
                res
                    .status(200)
                    .json(await resolved.run(() => resolved.service.revokePairingApproval(name, subject)));
            }
            catch (error) {
                sendManagementError(res, error);
            }
        ...
        app.put(`${prefix}/channels/:name`, upsert, async (req, res) => {
            ...
            try {
                ...
                res
                    .status(200)
                    .json(await resolved.run(() => resolved.service.upsert(name, request)));
            }
            catch (error) {
                sendManagementError(res, error);
            }
        ...
        app.delete(`${prefix}/channels/:name`, remove, async (req, res) => {
            ...
            try {
                ...
                res
                    .status(200)
                    .json(await resolved.run(() => resolved.service.remove(name, request)));
            }
            catch (error) {
                sendManagementError(res, error);
            }
        ...
        app.put(`${prefix}/channels/:name/startup`, startup, async (req, res) => {
            ...
            try {
                ...
                res
                    .status(200)
                    .json(await resolved.run(() => resolved.service.setStartup(name, request)));
            }
            catch (error) {
                sendManagementError(res, error);
            }
        ...
        const action = (operation, middleware) => {
            app.post(`${prefix}/channels/:name/${operation}`, middleware, async (req, res) => {
                const resolved = await target(req, res);
                if (!resolved || !validateClient(req, res, resolved.runtime))
                    return;
                const name = parseInstanceName(req, res);
                ...
                try {
                    ...
                    res
                        .status(200)
                        .json(await resolved.run(() => resolved.service[operation](name)));
                }
                catch (error) {
                    sendManagementError(res, error);
                }
...
//# sourceMappingURL=workspace-channel-management.js.map