 ...
 */
import { loadUsageHistory, aggregateUsage, getTimeRangeBounds, } from '@lailatul-coder/lailatul-coder-core';
function calculateStreaks(dates) {
    ...
    for (let i = 1; i < parsed.length; i++) {
        const diff = Math.round((parsed[i].getTime() - parsed[i - 1].getTime()) / (1000 * 60 * 60 * 24));
        if (diff === 1) {
            currentStreak++;
...
function buildHeatmap(records, start, end) {
    const heatmap = {};
    for (const r of records) {
        if (r.timestamp < start.getTime() || r.timestamp > end.getTime())
            continue;
        if (!r.models)
            continue;
...
function buildTokensPerDay(records, start, end) {
    ...
    for (const r of records) {
        ...
        for (const [model, m] of Object.entries(r.models)) {
            ...
            const tokens = m.totalTokens || m.inputTokens + m.outputTokens + m.thoughtsTokens;
...
export function getPreviousRangeBounds(range) {
    if (range === 'all')
        return null;
    const now = new Date();
    switch (range) {
...
export function computeDelta(current, previous) {
    ...
    const currentCacheRate = currentInputTokens > 0
        ? (currentCachedTokens / currentInputTokens) * 100
        : null;
    const previousCacheRate = previousInputTokens > 0
        ? (previousCachedTokens / previousInputTokens) * 100
        : null;
    const cacheRateDelta = currentCacheRate !== null && previousCacheRate !== null
        ? currentCacheRate - previousCacheRate
        : null;
    const currentToolSuccess = current.tools.totalCalls > 0
        ? (current.tools.totalSuccess / current.tools.totalCalls) * 100
        : null;
    const previousToolSuccess = previous.tools.totalCalls > 0
        ? (previous.tools.totalSuccess / previous.tools.totalCalls) * 100
        : null;
    const toolSuccessDelta = currentToolSuccess !== null && previousToolSuccess !== null
        ? currentToolSuccess - previousToolSuccess
        : null;
    const currentAvgLatency = current.totalRequests > 0
        ? current.totalLatencyMs / current.totalRequests
        : null;
    const previousAvgLatency = previous.totalRequests > 0
        ? previous.totalLatencyMs / previous.totalRequests
        : null;
    const avgLatencyDelta = currentAvgLatency !== null && previousAvgLatency !== null
...
export async function loadStatsData(range, currentSession) {
    ...
    if (prevBounds) {
        const prevFiltered = records.filter((r) => r.timestamp >= prevBounds.start.getTime() &&
    ...
    const toolSuccessRate = report.tools.totalCalls > 0
        ? (report.tools.totalSuccess / report.tools.totalCalls) * 100
        : 0;
    const avgLatencyMs = report.totalRequests > 0
    ...
    const toolLeaderboard = report.tools.topTools
        .slice(0, 8)
        .map((t) => ({
        name: t.name,
        count: t.count,
...
//# sourceMappingURL=statsDataService.js.map