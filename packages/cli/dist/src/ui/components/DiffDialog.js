...
export function DiffDialog({ history, cwd, fileHistoryService, fileCheckpointingEnabled, onClose, }) {
    const current = useDiffData(cwd);
    const { turns, loading: turnsLoading } = useTurnDiffs(history, fileHistoryService, fileCheckpointingEnabled);
    const sources = useMemo(() => {
        const list = [{ kind: 'current', label: t('Current') }];
    ...
    const safeSourceIndex = Math.min(sourceIndex, Math.max(0, sources.length - 1));
    ...
    const stats = useMemo(() => {
        if (!activeSource)
            return { filesCount: 0, linesAdded: 0, linesRemoved: 0 };
    ...
    const handleKeypress = useCallback((key) => {
        ...
        if (name === 'down' || name === 'j') {
            setFileIndex((i) => Math.min(Math.max(0, filesLenRef.current - 1), i + 1));
            return;
        }
        if (name === 'return') {
            ...
            if (sel.oversized) {
                setKeyHintRef.current(t('Oversized file — diff omitted. Use `git diff` to inspect.'));
                return;
            }
            if (!sel.hasHunks) {
    ...
    const headerTitle = activeSource?.kind === 'turn'
        ? t('Turn {{n}}', { n: String(activeSource.entry.turnIndex) })
        : t('Working tree vs HEAD');
    const headerSubtitle = activeSource?.kind === 'turn' && activeSource.entry.promptPreview
        ? `“${activeSource.entry.promptPreview}”`
        : activeSource?.kind === 'current'
            ? t('(git diff HEAD)')
            : '';
    const loadingNow = (activeSource?.kind === 'current' && current.loading) ||
    ...
    const hiddenFileCount = activeSource?.kind === 'current'
    ...
    return (_jsxs(Box, { flexDirection: "column", borderStyle: "round", borderColor: theme.border.default, paddingX: 1, width: dialogWidth, children: [_jsxs(Box, { flexDirection: "row", justifyContent: "space-between", children: [_jsxs(Text, { bold: true, color: theme.text.primary, children: ["/diff \u00B7 ", headerTitle, headerSubtitle ? (_jsxs(Text, { color: theme.text.secondary, children: [" ", headerSubtitle] })) : null] }), _jsxs(Text, { color: theme.text.secondary, children: [stats.filesCount, " ", stats.filesCount === 1 ? t('file') : t('files'), stats.linesAdded > 0 ? (_jsxs(Text, { color: theme.status.success, children: [" +", stats.linesAdded] })) : null, stats.linesRemoved > 0 ? (_jsxs(Text, { color: theme.status.error, children: [" -", stats.linesRemoved] })) : null] })] }), _jsx(SourceSwitcher, { sources: sources, sourceIndex: safeSourceIndex }), _jsx(Box, { marginTop: 1, flexDirection: "column", children: loadingNow ? (_jsx(Text, { color: theme.text.secondary, children: t('Loading diff…') })) : !activeSource || files.length === 0 ? (_jsx(Text, { color: theme.text.secondary, children: emptyMessage(activeSource, current.result, fileCheckpointingEnabled) })) : viewMode === 'list' ? (_jsxs(_Fragment, { children: [_jsx(FileList, { files: files, selectedIndex: safeFileIndex, contentWidth: dialogWidth - 4 }), hiddenFileCount > 0 ? (_jsxs(Text, { color: theme.text.secondary, children: [' ', hiddenIsUpperBound
                                    ...
                                    : t('…and {{n}} more (showing first {{shown}})', {
                                        n: String(hiddenFileCount),
                                        shown: String(files.length),
                                    })] })) : null] })) : selectedFile ? (_jsx(FileDetail, { file: selectedFile, activeSource: activeSource, currentHunks: current.hunks, availableHeight: detailHeight, contentWidth: dialogWidth - 4 })) : null }), _jsx(Box, { marginTop: 1, children: _jsx(Text, { color: keyHint ? theme.status.warning : theme.text.secondary, children: keyHint && viewMode === 'list'
                        ? keyHint
                        : viewMode === 'list'
                            ? sources.length > 1
...
function SourceSwitcher({ sources, sourceIndex, }) {
    if (sources.length <= 1)
        return null;
    return (_jsxs(Box, { marginTop: 1, flexDirection: "row", children: [sourceIndex > 0 ? (_jsx(Text, { color: theme.text.secondary, children: "\u25C0 " })) : (_jsx(Text, { children: " " })), sources.map((s, i) => {
                const selected = i === sourceIndex;
                return (_jsxs(Text, { bold: selected, color: selected ? theme.text.accent : theme.text.secondary, children: [i > 0 ? ' · ' : '', s.label] }, `${s.kind}:${i}`));
            }), sourceIndex < sources.length - 1 ? (_jsx(Text, { color: theme.text.secondary, children: " \u25B6" })) : null] }));
}
function FileList({ files, selectedIndex, contentWidth, }) {
    const { startIndex, endIndex } = useVisibleWindow(files.length, selectedIndex, MAX_VISIBLE_FILES);
    ...
    return (_jsxs(Box, { flexDirection: "column", children: [aboveCount > 0 ? (_jsxs(Text, { color: theme.text.secondary, children: [' ', "\u2191 ", aboveCount, " ", aboveCount === 1 ? t('more file') : t('more files')] })) : null, visible.map((f, idx) => (_jsx(FileRow, { file: f, selected: startIndex + idx === selectedIndex, maxPathChars: maxPathChars }, f.path))), belowCount > 0 ? (_jsxs(Text, { color: theme.text.secondary, children: [' ', "\u2193 ", belowCount, " ", belowCount === 1 ? t('more file') : t('more files')] })) : null] }));
}
function FileRow({ file, selected, maxPathChars, }) {
    ...
    return (_jsxs(Box, { flexDirection: "row", children: [_jsx(Text, { color: selected ? theme.text.accent : theme.text.primary, bold: selected, children: pointer }), renameFits ? (_jsxs(Text, { color: theme.text.secondary, children: [truncatePathStart(file.oldDisplayPath, pathBudget), " \u2192", ' '] })) : null, _jsx(Text, { color: selected ? theme.text.accent : theme.text.primary, bold: selected, children: path }), _jsxs(Text, { color: theme.text.secondary, children: [tag, " "] }), file.isBinary ? (_jsx(Text, { color: theme.text.secondary, italic: true, children: t('binary') })) : (_jsxs(_Fragment, { children: [file.added > 0 ? (_jsxs(Text, { color: theme.status.success, children: ["+", file.added] })) : null, file.added > 0 && file.removed > 0 ? _jsx(Text, { children: " " }) : null, file.removed > 0 ? (_jsxs(Text, { color: theme.status.error, children: ["-", file.removed] })) : null] }))] }));
}
function FileDetail({ file, activeSource, currentHunks, availableHeight, contentWidth, }) {
    const diffText = useMemo(() => {
        ...
        const entry = activeSource.entry.diff.files.find((f) => f.filePath === file.path);
        if (!entry)
            return '';
        return hunksToUnifiedDiff(file.path, entry.hunks);
    }, [file, activeSource, currentHunks]);
    if (file.isBinary) {
        return (_jsx(Text, { color: theme.text.secondary, children: t('Binary file — no diff.') }));
    }
    if (file.oversized) {
        return (_jsx(Text, { color: theme.text.secondary, children: t('Oversized file — diff omitted. Use `git diff` to inspect.') }));
    }
    if (!diffText) {
        return (_jsx(Text, { color: theme.text.secondary, children: t('No hunks available for {{path}}.', { path: file.displayPath }) }));
    }
    return (_jsxs(Box, { flexDirection: "column", children: [_jsx(Text, { bold: true, color: theme.text.primary, children: truncatePathStart(file.displayPath, contentWidth) }), _jsx(Box, { marginTop: 1, children: _jsx(DiffRenderer, { diffContent: diffText, filename: file.displayPath, availableTerminalHeight: availableHeight, contentWidth: contentWidth }) })] }));
}
/**
 ...
 */
function truncatePathStart(path, maxChars) {
    if (maxChars <= 0)
        return '';
...
function useVisibleWindow(total, selectedIndex, windowSize) {
    if (total <= windowSize)
...
function currentToFiles(result, hunks) {
    if (!result)
        return [];
...
function perFileToUnified(path, s, hunks) {
...
function turnToFiles(diff) {
    return diff.files.map(turnFileToUnified);
}
function turnFileToUnified(f) {
    return {
        path: f.filePath,
...
function hunksToUnifiedDiff(filePath, hunks) {
    ...
    const lines = [`--- a/${safePath}`, `+++ b/${safePath}`];
...
function emptyMessage(activeSource, currentResult, fileCheckpointingEnabled) {
    if (!activeSource) {
        return fileCheckpointingEnabled
            ? t('No diff data yet.')
            : t('Per-turn diffs are unavailable because file checkpointing is disabled.');
    }
    if (activeSource.kind === 'current') {
        if (!currentResult) {
            return t('No diff available. Either this is not a git repository, HEAD is missing, or a merge/rebase/cherry-pick/revert is in progress.');
        }
        ...
        if (currentResult.stats.filesCount > 0) {
            return t('{{count}} files changed but the diff is too large to list per-file (+{{added}} / -{{removed}}). Use `git diff` for details.', {
...
//# sourceMappingURL=DiffDialog.js.map