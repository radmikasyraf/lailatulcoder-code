...
import { AskUserQuestionDialog, computeHeaderCap, } from './AskUserQuestionDialog.js';
import { ToolConfirmationOutcome } from '@lailatul-coder/lailatul-coder-core';
import { renderWithProviders } from '../../../test-utils/render.js';
import stripAnsi from 'strip-ansi';
const wait = (ms = 50) => new Promise((resolve) => setTimeout(resolve, ms));
const writeText = async (stdin, text) => {
    stdin.write(text);
    await wait();
};
const clean = (value) => stripAnsi(value ?? '');
const waitForFrame = async (predicate, options = {}) => {
...
const createSingleQuestion = (overrides = {}) => ({
...
const createConfirmationDetails = (overrides = {}) => ({
...
describe('computeHeaderCap', () => {
    ...
    it('is maximal — one more cell of cap would overflow the budget', () => {
        ...
        const usedAt = (c) => widths.reduce((sum, w) => sum + Math.min(w, c), 0);
...
describe('<AskUserQuestionDialog />', () => {
    describe('rendering', () => {
        it('renders single question with options', () => {
            ...
            const { lastFrame } = renderWithProviders(_jsx(AskUserQuestionDialog, { confirmationDetails: details, availableWidth: 80, onConfirm: onConfirm }));
        ...
        it('shows an over-length header in full when the tab row has room', () => {
            ...
            const { lastFrame } = renderWithProviders(_jsx(AskUserQuestionDialog, { confirmationDetails: details, availableWidth: 100, onConfirm: vi.fn() }));
            const output = lastFrame();
        ...
        it('clips an over-length header and keeps the row within width when space is tight', () => {
            ...
            const { lastFrame } = renderWithProviders(_jsx(AskUserQuestionDialog, { confirmationDetails: details, availableWidth: 28, onConfirm: vi.fn() }));
            const output = lastFrame();
        ...
        it('keeps a long header full when a short neighbor leaves room', () => {
            ...
            const { lastFrame } = renderWithProviders(_jsx(AskUserQuestionDialog, { confirmationDetails: details, availableWidth: 50, onConfirm: vi.fn() }));
        ...
        it('clips every header when four long tabs must share a tight row', () => {
            ...
            const { lastFrame } = renderWithProviders(_jsx(AskUserQuestionDialog, { confirmationDetails: details, availableWidth: availableWidth, onConfirm: vi.fn() }));
            const output = lastFrame();
    ...
    describe('single-select interaction', () => {
        it('selects an option with Enter and submits immediately for single question', async () => {
            const onConfirm = vi.fn();
            const details = createConfirmationDetails();
            const { stdin, unmount } = renderWithProviders(_jsx(AskUserQuestionDialog, { confirmationDetails: details, availableWidth: 80, onConfirm: onConfirm }));
            await wait();
            // Press Enter to select the first option (Red)
            stdin.write('\r');
            await wait();
            expect(onConfirm).toHaveBeenCalledWith(ToolConfirmationOutcome.ProceedOnce, { answers: { 0: 'Red' } });
            unmount();
        });
        it('auto-submits when pressing a number key for a predefined option', async () => {
            ...
            stdin.write('2');
            await wait();
            expect(onConfirm).toHaveBeenCalledWith(ToolConfirmationOutcome.ProceedOnce, { answers: { 0: 'Blue' } });
            unmount();
        });
        ...
        it('navigates with selection shortcuts when custom input is not focused', async () => {
            const onConfirm = vi.fn();
            const details = createConfirmationDetails();
            const { stdin, lastFrame, unmount } = renderWithProviders(_jsx(AskUserQuestionDialog, { confirmationDetails: details, availableWidth: 80, onConfirm: onConfirm }));
            await wait();
    ...
    describe('multi-select interaction', () => {
        ...
        it('auto-selects custom input and submits on Enter after typing', async () => {
            ...
            stdin.write('\r');
            await wait();
            expect(onConfirm).toHaveBeenCalledWith(ToolConfirmationOutcome.ProceedOnce, { answers: { 0: 'My custom answer' } });
            unmount();
        }, 10000);
        ...
        it('submits predefined options together with custom input', async () => {
            ...
            stdin.write('\r');
            await wait();
            expect(onConfirm).toHaveBeenCalledWith(ToolConfirmationOutcome.ProceedOnce, { answers: { 0: expect.stringContaining('Red') } });
            expect(onConfirm).toHaveBeenCalledWith(ToolConfirmationOutcome.ProceedOnce, { answers: { 0: expect.stringContaining('Purple') } });
            unmount();
        }, 10000);
    });
    describe('multiple questions', () => {
        it.skipIf(process.platform === 'win32')('does not auto-submit when pressing number key on Submit tab', async () => {
            const onConfirm = vi.fn();
            const details = createConfirmationDetails({
                questions: [
                    createSingleQuestion({ header: 'Q1' }),
    ...
    describe('focus behavior', () => {
        it('does not respond to keys when isFocused is false', async () => {
            const onConfirm = vi.fn();
            const details = createConfirmationDetails();
            const { stdin, unmount } = renderWithProviders(_jsx(AskUserQuestionDialog, { confirmationDetails: details, availableWidth: 80, isFocused: false, onConfirm: onConfirm }));
            await wait();
            stdin.write('\r'); // Enter
            await wait();
            stdin.write('\u001B'); // Escape
...
//# sourceMappingURL=AskUserQuestionDialog.test.js.map