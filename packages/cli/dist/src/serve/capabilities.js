 ...
 */
export const SERVE_PROTOCOL_VERSION = 'v1';
...
export const SERVE_CAPABILITY_REGISTRY = {
    ...
    // hierarchical LAILATUL.md state and accepts append/replace writes scoped
    // to either the bound workspace or the global ~/.lailatulcoder directory.
    ...
    // `LAILATUL.md` (or whatever `getCurrentGeminiMdFilename()` returns) at
    ...
    // X-LailatulCoder-Client-Id when calling the route.
 ...
 */
export const CONDITIONAL_SERVE_FEATURES = new Map([
    ...
    [
        ...
        (toggles) => typeof toggles.promptDeadlineMs === 'number' &&
            toggles.promptDeadlineMs > 0,
    ],
    [
        'writer_idle_timeout',
        (toggles) => typeof toggles.writerIdleTimeoutMs === 'number' &&
    ...
    [
        ...
        (toggles) => toggles.acpHttpEnabled === true &&
    ...
    [
        'realtime_voice',
        (toggles) => toggles.acpHttpEnabled === true && toggles.realtimeVoiceEnabled === true,
    ],
]);
export const SERVE_FEATURES = Object.freeze(Object.keys(SERVE_CAPABILITY_REGISTRY));
...
function isFeatureAvailableInProtocol(feature, protocolVersion) {
    return (serveProtocolVersionIndex(SERVE_CAPABILITY_REGISTRY[feature].since) <=
        serveProtocolVersionIndex(protocolVersion));
}
export function getRegisteredServeFeatures() {
    return [...SERVE_FEATURES];
}
export function getAdvertisedServeFeatures(protocolVersion = SERVE_PROTOCOL_VERSION, toggles = {}) {
    return SERVE_FEATURES.filter((feature) => {
        if (!isFeatureAvailableInProtocol(feature, protocolVersion))
            return false;
        ...
        if (predicate !== undefined)
            return predicate(toggles);
        return true;
    });
}
export function getServeFeatures() {
