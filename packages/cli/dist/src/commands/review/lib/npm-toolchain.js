...
import { BUILD_MIN_FREE_BYTES, INSTALL_MIN_FREE_BYTES, freeDiskBytes, gib, } from './disk.js';
import { affectedWorkspaces, buildSetFor, hasUnmodeledWorkspaceGlob, readRootPackage, readWorkspaceGlobs, readWorkspacePackages, reverseDependencyClosure, scriptFansOut, } from './workspaces.js';
import { resolveTestScope } from './workspace-scope.js';
 ...
 */
export function unresolvedWorkspaceDeps(output, packages) {
...
const succeeded = (r) => r.exitCode === 0;
/**
 ...
 */
function installFailureFraming(install) {
    if (!install || install.exitCode === 0)
        return '';
    return (`\`${install.command}\` exited ${install.exitCode} but left a usable ` +
 ...
 */
function resumeNpmToolchain(args, previous) {
    const { root, exec } = args;
    const perCommandMs = args.timeout * 1000;
    const callBudgetMs = (args.budget ?? Math.max(args.timeout, DEFAULT_WHOLE_CALL_BUDGET_S)) * 1000;
    ...
    if (previous.toolchain !== 'npm') {
        return withNote('Nothing to resume: the run being continued did not scope an npm ' +
    ...
    if (previous.notBuilt?.length) {
        ...
        return withNote(`Cannot resume the suites: the run being continued left ` +
    ...
    if (work.length === 0) {
        ...
        const endedEarly = neverTested &&
            (previous.buildOnly === true ||
                previous.ok === false ||
                previous.endedBeforeTests === true);
        return withNote(endedEarly
    ...
    for (let i = 0; i < work.length; i++) {
        ...
        if (remaining < BUDGET_MIN_ATTEMPT_MS) {
            ...
            for (const left of work.slice(i)) {
                if (left.dir !== null)
                    stillPending.push(left.dir);
                else
                    unattemptedRetries.push(left.command);
            }
            break;
        }
    ...
    if (prevScope) {
        ...
        // PR-authored name.
        const liveCaveat = prevScope.liveCaveat !== undefined
        ...
        if (outstanding.length > 0) {
            ...
            liveSegments.push(`a --resume call ran ${ranDirs.length + replaced.size} more ` +
        ...
        const provisionalOnly = stillClamped.filter((c) => !outstanding.includes(c));
        if (provisionalOnly.length > 0) {
            liveSegments.push(`a --resume call left ${provisionalOnly.length} command(s) ` +
                `provisional (killed on a budget-shortened deadline): ` +
                `${provisionalOnly.join(', ')}`);
        }
        const caveat = liveSegments.join('; ');
        testScope = {
            workspaces: [...prevScope.workspaces, ...ranDirs].filter((d) => !stillPending.includes(d) || ranSet.has(d)),
    ...
    const merged = {
        ...
        // quotes.
        note: resumedNote(previous, mergedTest, timedOut, [...stillPending, ...unattemptedRetries], stillClamped) + (testScope?.caveat ? ` Caveat: ${testScope.caveat}.` : ''),
        ...(testScope ? { testScope } : {}),
    };
    return merged;
}
/** The merged report's one-line story, rewritten for the continued run. */
function resumedNote(previous, test, timedOut, stillPending, stillClamped) {
    ...
    if (failures.length > 0) {
        parts.push(`${failures.length} command(s) failed. Correlate each error with the ` +
            `diff: a failure in a file the PR changed is a Critical; one in a file ` +
            `it did not touch is pre-existing.`);
    }
    if (timedOut.length > 0) {
        parts.push(`${timedOut.length} command(s) ran out of time — infrastructure, not a ` +
            `defect in the diff.`);
    }
    ...
    if (stillPending.length > 0) {
        parts.push(`The budget was spent with ${stillPending.length} command(s) still to ` +
    ...
    if (provisionalOnly.length > 0) {
        parts.push(`${provisionalOnly.length} command(s) are still provisional — killed ` +
    ...
    if (stillPending.length === 0 && stillClamped.length === 0) {
        parts.push(failures.length === 0 && timedOut.length === 0
            ? 'Every suite in scope has now run, and everything passed.'
            : 'Every suite in scope has now run.');
    }
    return parts.join(' ');
}
function runNpmToolchain(args) {
    if (args.previous)
        return resumeNpmToolchain(args, args.previous);
    ...
    const deadlineSecs = (r) => Math.round((r.deadlineMs ?? perCommandMs) / 1000);
    ...
    if (unmodeled ||
        ...
        return unsupportedReport(unmodeled
    ...
    let testScope = singleRoot || args.buildOnly
        ? undefined
        : resolveTestScope({
            changed,
            globs,
    ...
    if (unmapped.length > 0) {
        return unsupportedReport(`The diff touches ${unmapped.join(', ')}, which the workspace globs map to no ` +
    ...
    const installComplete = () => existsSync(join(root, 'node_modules', '.package-lock.json'));
    ...
    if (args.install && !npmLock && !existsSync(join(root, 'node_modules'))) {
        ...
        return unsupportedReport(altLock
    ...
    if (args.install && npmLock && !installComplete()) {
        ...
        const install = exec(installCmd, root, Math.min(perCommandMs, remainingMs()));
        results.install = install;
        if (install.timedOut)
            results.timedOut.push(install.command);
    ...
    const rootBuildRuns = !!rootPkg?.scripts.includes('build') &&
    ...
    for (let attempt = 0; attempt <= 3; attempt++) {
        ...
        for (const dir of set) {
            ...
            if (remainingMs() < BUDGET_MIN_ATTEMPT_MS) {
                ...
                notBuilt.push(...set.filter((d) => !built.has(d) && byDir.get(d)?.scripts.includes('build')));
                break;
            }
            const r = exec(buildCommand(dir), root, Math.min(perCommandMs, remainingMs()));
            results.build.push(r);
            if (r.timedOut)
                results.timedOut.push(r.command);
            if (r.exitCode !== 0) {
                failure = r;
                break;
            }
        ...
        if (missing.length === 0 || failure.timedOut || attempt === 3) {
            ...
            results.buildSet = (rootBuildSkipped ? set.filter((d) => d !== '.') : set).filter((d) => !notBuilt.includes(d));
    ...
    if (notBuilt.length > 0)
        results.notBuilt = [...notBuilt].sort();
    ...
    const runnable = (dir) => dir === '.' ? rootHasTest : !!byDir.get(dir)?.scripts.includes('test');
    ...
    const untestable = notBuilt.length > 0
    ...
    if (testScope)
        results.testScope = testScope;
    if (!results.note) {
        const failed = [...results.build, ...results.test].filter((r) => r.exitCode !== 0);
        ...
        if (results.ok) {
            ...
            if (testScope?.caveat)
                testsClause += ` Caveat: ${testScope.caveat}.`;
            ...
            const builtWorkspaces = results.buildSet.filter((d) => singleRoot || d !== '.').length;
            const rootSuffix = !singleRoot && results.buildSet.includes('.') && !rootBuildSkipped
                ? ' (plus the root package)'
                : '';
            results.note =
                `Built ${builtWorkspaces} of ${packages.length} workspaces${rootSuffix} (the ${affected.length} the ` +
                    `diff changes, plus what they compile against${widened.size
...
export const npmToolchainAdapter = {
    ...
    applies: (root) => {
        const globs = readWorkspaceGlobs(root);
        if (globs.length > 0) {
            return (!hasUnmodeledWorkspaceGlob(globs) &&
...
//# sourceMappingURL=npm-toolchain.js.map