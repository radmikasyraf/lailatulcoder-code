...
function createConfig(opts) {
    const sessionService = {
        ...
        removeSessions: opts.removeSessions ??
            vi.fn().mockResolvedValue({ removed: [], notFound: [], errors: [] }),
    };
    const hookSystem = {
        fireSessionDeleteEvent: opts.fireSessionDeleteEvent ?? vi.fn().mockResolvedValue(undefined),
    };
...
describe('useDeleteCommand', () => {
    ...
    describe('handleDeleteMany', () => {
        it('removes sessions and reports the count on success', async () => {
            ...
            const addItem = vi.fn();
            const { result } = renderHook(() => useDeleteCommand({ config, addItem }));
            act(() => {
                result.current.openDeleteDialog();
            });
            await act(async () => {
            ...
            const [item] = addItem.mock.calls.at(-1);
        ...
        it('emits a progress toast before awaiting the batch', async () => {
            ...
            let resolveRemove = () => { };
            const removeSessions = vi.fn(() => new Promise((resolve) => {
            ...
            const [progress] = addItem.mock.calls[0];
        ...
        it('surfaces an info toast when the active session is stripped', async () => {
            ...
            const texts = addItem.mock.calls.map((c) => c[0].text);
            expect(texts.some((t) => /current active session skipped/i.test(t))).toBe(true);
        });
        ...
        it('drops a re-entrant call while a batch is in flight and tells the user', async () => {
            ...
            expect(removeSessions).toHaveBeenCalledTimes(1);
            const [busyItem] = addItem.mock.calls.at(-1);
        ...
        it('fires once for each successfully deleted session', async () => {
            ...
            const { result } = renderHook(() => useDeleteCommand({ config, addItem: vi.fn() }));
            await act(async () => {
        ...
        it('reports batch deletion success when SessionDelete hooks reject', async () => {
            ...
            expect(debugLogger.warn).toHaveBeenNthCalledWith(1, 'SessionDelete hook failed for a: failed');
            expect(debugLogger.warn).toHaveBeenNthCalledWith(2, 'SessionDelete hook failed for b: failed');
            const [item] = addItem.mock.calls.at(-1);
    ...
    describe('handleDelete', () => {
        it('fires after a successful deletion', async () => {
            ...
            expect(hookSystem.fireSessionDeleteEvent).toHaveBeenCalledWith('deleted-id');
        });
        ...
        it('reports deletion success when the SessionDelete hook rejects', async () => {
            ...
            expect(hookSystem.fireSessionDeleteEvent).toHaveBeenCalledWith('deleted-id');
            expect(debugLogger.warn).toHaveBeenCalledWith('SessionDelete hook failed for deleted-id: failed');
        ...
        it('reports deletion success when the SessionDelete hook rejects with undefined', async () => {
            ...
            expect(debugLogger.warn).toHaveBeenCalledWith('SessionDelete hook failed for deleted-id: undefined');
            expect(addItem).toHaveBeenCalledWith(expect.objectContaining({
                type: 'info',
                text: 'Session deleted successfully.',
            }), expect.any(Number));
...
//# sourceMappingURL=useDeleteCommand.test.js.map