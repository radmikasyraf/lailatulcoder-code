...
import { estimateContentChars, resolveCompactionTuning, resolveSlimmingConfig, slimCompactionInput, } from './compactionInputSlimming.js';
import { CHARS_PER_TOKEN, estimateContentTokens, estimatePromptTokens, } from './tokenEstimation.js';
import { buildStateReminderParts, composePostCompactHistory, countToolResponseImages, postProcessSummary, stripAnalysisBlock, } from './postCompactAttachments.js';
 ...
 * (https://github.com/LailatulCoder/lailatul-coder/issues/7960). Floored at 1 so
 ...
 */
export function computeCompactionOutputBudget(estimatedInputTokens, contextLimit) {
...
const COMPRESSION_REQUEST_DIRECTIVE = 'First, reason in your <analysis> block. Then, produce the <state_snapshot> XML.';
/**
...
function estimateSummaryOutputTokens(summary, imageTokenEstimate) {
    const genericEstimate = estimateContentTokens([{ role: 'model', parts: [{ text: summary }] }], imageTokenEstimate);
    ...
    const cjkAwareEstimate = Math.ceil(nonCjkCharCount / CHARS_PER_TOKEN) +
 ...
 * Pure function — no I/O, no shared state — safe to call repeatedly.
 */
export function computeThresholds(window, pct) {
    const effectivePct = Math.min(1, Math.max(0, pct !== undefined && Number.isFinite(pct) ? pct : DEFAULT_PCT));
...
function supportsCompressionCacheSharing(config) {
...
export class ChatCompressionService {
    async compress(chat, opts) {
        const { promptId, force, config, consecutiveFailures, originalTokenCount, trigger, signal, } = opts;
        ...
        let triggerReason = compactTrigger === 'manual' ? 'manual' : 'token_limit';
        ...
        const contextLimit = contentGeneratorConfig.contextWindowSize ?? DEFAULT_TOKEN_LIMIT;
        ...
        if (!force) {
            ...
            const { auto } = computeThresholds(contextLimit, config.getAutoCompactThreshold());
            ...
            const effectiveTokens = opts.precomputedEffectiveTokens !== undefined
                ? opts.precomputedEffectiveTokens
                : pendingUserMessage
                    ? estimatePromptTokens(chat.getHistoryShallow(true), pendingUserMessage, originalTokenCount, 
                    ...
                    0, slimmingConfig.imageTokenEstimate)
                    : originalTokenCount;
            if (effectiveTokens < auto) {
                ...
                const screenshotOverflow = tuning.enableScreenshotTrigger &&
                    countToolResponseImages(chat.getHistoryShallow(true)) >=
                        tuning.screenshotTriggerThreshold;
                if (!screenshotOverflow) {
                    debugLogger.debug(`[compaction] cheap-gate NOOP: effectiveTokens=${effectiveTokens}, ` +
        ...
        if (hookSystem) {
            const preCompactTrigger = compactTrigger === 'manual'
                ? PreCompactTrigger.Manual
                : PreCompactTrigger.Auto;
            try {
                const result = await hookSystem.firePreCompactEvent(preCompactTrigger, opts.customInstructions ?? '', signal);
        ...
        const pendingToolResult = opts.pendingUserMessage?.parts?.some((part) => !!part.functionResponse);
        ...
        const pendingToolResultTokenCount = pendingToolResult
            ? estimateContentTokens([opts.pendingUserMessage], slimmingConfig.imageTokenEstimate)
            : 0;
        ...
        const systemInstruction = buildCompressionSystemPrompt(opts.customInstructions, hookExtraInstructions);
        ...
        let effectiveCompactionModel = config.getCompactionModel?.() ?? config.getModel();
        ...
        let cachedColdInputEstimate;
        const getColdInputEstimate = () => (cachedColdInputEstimate ??=
            estimateContentTokens(getColdInput().slimmedHistory, slimmingConfig.imageTokenEstimate) + Math.ceil(systemInstruction.length / CHARS_PER_TOKEN));
        ...
        if (effectiveCompactionModel !== config.getModel()) {
            ...
            if (resolved) {
                ...
                const slimmedTokenEstimate = getColdInputEstimate() + COMPACT_MAX_OUTPUT_TOKENS;
        ...
        const runColdCompression = () => {
            ...
            if (slim.stats.imagesStripped > 0 || slim.stats.documentsStripped > 0) {
                config
                    .getDebugLogger()
                    .debug(`[chat-compression] slimmed ${slim.stats.imagesStripped} image(s) ` +
            ...
            coldOutputBudget = computeCompactionOutputBudget(getColdInputEstimate() +
                Math.ceil(COMPRESSION_REQUEST_DIRECTIVE.length / CHARS_PER_TOKEN), budgetWindow);
            if (coldOutputBudget < COMPACT_MAX_OUTPUT_TOKENS) {
                config
                    .getDebugLogger()
                    .debug(`[chat-compression] output budget clamped to ${coldOutputBudget} ` +
            ...
            return runSideQuery(config, {
                ...
                // See https://github.com/LailatulCoder/lailatul-coder/issues/5956
                ...
                // session. See https://github.com/LailatulCoder/lailatul-coder/issues/5861.
                stream: true,
        ...
        const sharedRequestText = `${systemInstruction}\n\n` +
            'Do not call tools; tool execution is disabled for this request. ' +
            COMPRESSION_REQUEST_DIRECTIVE;
        const sharedPromptTokenCount = opts.precomputedEffectiveTokens ??
            originalTokenCount + (chat.getLastOutputTokenCount?.() ?? 0);
        const sharedDirectiveTokenCount = Math.ceil(sharedRequestText.length / CHARS_PER_TOKEN);
        const usesMainModel = effectiveCompactionModel === config.getModel();
        const providerSupportsCacheSharing = supportsCompressionCacheSharing(config);
        ...
        const sharedRequestFits = sharedPromptTokenCount +
            sharedDirectiveTokenCount +
            COMPACT_MAX_OUTPUT_TOKENS <=
            contextLimit;
        const canShareCache = usesMainModel &&
        ...
        if (canShareCache) {
            try {
                ...
                if (!sharedResult.hadToolCall && hasStateSnapshot(sharedResult.text)) {
                    ...
                    config
                        .getDebugLogger()
                        .debug(`[chat-compression] cache-sharing request succeeded; ` +
                    ...
                    config
                        .getDebugLogger()
                        .warn(`[chat-compression] cache-sharing response was unusable ` +
                ...
                config
                    .getDebugLogger()
                    .warn(`[chat-compression] cache-sharing request failed; falling back ` +
        ...
        const isSummaryEmpty = !summary || stripAnalysisBlock(summary).trim().length === 0;
        ...
        if (compressionOutputTokenCount === undefined &&
        ...
        if (compressionOutputTokenCount === undefined && !isSummaryEmpty) {
            ...
            compressionOutputTokenCount = estimateSummaryOutputTokens(summary, slimmingConfig.imageTokenEstimate);
            config
                .getDebugLogger()
                .warn(`[chat-compression] compression side-query omitted usage metadata; ` +
        ...
        const truncationThreshold = outputCountIsEstimated && coldOutputBudget > 1
            ? COMPACT_MAX_OUTPUT_TOKENS
            : coldOutputBudget;
        if (!usedCacheSharing &&
            !isSummaryEmpty &&
            ...
            config
                .getDebugLogger()
                .warn(`[chat-compression] summary output reached the truncation ` +
            ...
            return {
                ...
                info: {
                    ...
                    compressionStatus: CompressionStatus.COMPRESSION_FAILED_OUTPUT_TRUNCATED,
                },
            };
        }
        ...
        if (!usedCacheSharing &&
            ...
            config
                .getDebugLogger()
                .warn(`[chat-compression] summary lacks a closed <state_snapshot> while ` +
        ...
        if (!isSummaryEmpty) {
            ...
            const historyForCompose = compactTrigger === 'manual' &&
            ...
            try {
                extraHistory = await composePostCompactHistory(historyForCompose, summary, {
                ...
                const fcParts = trailingFc?.role === 'model'
            ...
            if (!usedCacheSharing &&
                ...
                const compressedHistoryTokenCount = Math.max(0, compressionInputTokenCount - 1000 - pendingToolResultTokenCount);
                newTokenCount = Math.max(0, originalTokenCount -
                ...
                const restorationChars = extraHistory
                    ...
                    .reduce((acc, c) => acc + estimateContentChars(c, slimmingConfig.imageTokenEstimate), 0);
                newTokenCount += Math.ceil(restorationChars / CHARS_PER_TOKEN);
            }
            else {
                const estimatedOriginalVisibleTokenCount = estimateContentTokens(curatedHistory, slimmingConfig.imageTokenEstimate);
                const estimatedNewVisibleTokenCount = estimateContentTokens(extraHistory, slimmingConfig.imageTokenEstimate);
                if (estimatedOriginalVisibleTokenCount > 0 &&
                    estimatedNewVisibleTokenCount > 0) {
                    const estimatedNonVisibleTokenCount = Math.max(0, originalTokenCount - estimatedOriginalVisibleTokenCount);
                    ...
                    config
                        .getDebugLogger()
                        .debug(`[chat-compression] ${usedCacheSharing
                        ? 'cache-sharing token accounting'
                        : 'usage metadata missing'}; estimated ` +
        ...
        if (isSummaryEmpty) {
            ...
            try {
                const postCompactTrigger = compactTrigger === 'manual'
                ...
                await config
                    .getHookSystem()
                    ?.firePostCompactEvent(postCompactTrigger, stripAnalysisBlock(summary), signal);
            }
            catch (err) {
