 ...
 */
import { AuthType, buildLearnSkillPrompt, buildLearnVideoSkillRequest, expandHomeDir, parseLearnVideoInput, readPathFromWorkspace, } from '@lailatul-coder/lailatul-coder-core';
        ...
        if (video) {
            if (video.kind === 'youtube') {
                return {
                    type: 'message',
                    messageType: 'error',
                    content: t('YouTube page URLs cannot be sent as native video input. Download the video into your workspace and pass the local video file path to /learn.'),
                };
            }
            const authType = config.getContentGeneratorConfig()?.authType;
            const supportsVideoTransport = authType === AuthType.USE_OPENAI || authType === AuthType.NEVER_MATCH_THIS;
            if (config.getEffectiveInputModalities().video !== true ||
                !supportsVideoTransport) {
                return {
                    type: 'message',
                    messageType: 'error',
                    content: t('The current model or provider does not support native video input for /learn. Switch to a video-capable model on an OpenAI-compatible provider and try again.'),
                };
            }
            let localVideoPart;
            if (video.kind === 'local') {
                let parts;
                try {
                    parts = await readPathFromWorkspace(expandHomeDir(video.source), config);
                }
                catch {
                ...
                localVideoPart = parts.find((part) => typeof part !== 'string' &&
                ...
                // single video.
                if (!localVideoPart) {
                    const inlineParts = parts.filter((part) => typeof part !== 'string' && part.inlineData != null);
            ...
            return {
                type: 'submit_prompt',
                content: await buildLearnVideoSkillRequest(video, projectRoot, localVideoPart),
            };
        }
        return {
            type: 'submit_prompt',
