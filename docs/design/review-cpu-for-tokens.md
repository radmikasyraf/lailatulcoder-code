...
| -------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Step 1 remote matching** (pr-url: parse `git remote -v`, match host + owner/repo by exact segments; bare pr-number: pick the remote whose URL is the derived owner/repo) | **Sink.** Deterministic parsing with two shipped bug classes: a substring match once matched `shao/lailatul-coder` against a `wenshao/lailatul-coder` remote (reviewed one repo, posted to another), and a guessed owner/repo once stopped a review before it read any code. Prose rules with bug histories are exactly the class DESIGN.md moves into code. |
...
shapes, `.git` suffix, case-insensitivity, the `shao/lailatul-coder` vs
`wenshao/lailatul-coder` regression row, fork layout (origin = fork, upstream =
