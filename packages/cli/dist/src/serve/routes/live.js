...
export function registerLiveRoutes(app, deps) {
    ...
    app.post('/live/mute', deps.mutate(), (req, res) => {
        ...
        if ((inputMuted === undefined && outputMuted === undefined) ||
            ...
            res.status(400).json({
                error: 'At least one of `inputMuted` or `outputMuted` must be a boolean.',
                code: 'invalid_live_mute',
            });
            return;
        }
        res.status(200).json(deps.coordinator.setMute({
            ...(inputMuted !== undefined ? { inputMuted } : {}),
            ...(outputMuted !== undefined ? { outputMuted } : {}),
        }));
    });
    app.post('/live/shortcut', deps.mutate({ strict: true }), async (req, res) => {
        const shortcut = safeBody(req)['shortcut'];
        if (typeof shortcut !== 'string' || shortcut.trim().length > 128) {
            res.status(400).json({
                error: '`shortcut` must be an Electron accelerator or an empty string.',
