...
export function seedParseArgs(dir, effort) {
    mkdirSync(join(dir, dirname(PARSE_ARGS_REPORT)), { recursive: true });
    writeFileSync(join(dir, PARSE_ARGS_REPORT), JSON.stringify({ effort, effortSource: 'flag' }), 'utf8');
}
/**
 ...
 */
export function makeDiff(path, n) {
    const body = [];
    while (body.length < n) {
 ...
 */
export function makeStaleBundleFixture(fs, prefix) {
    ...
    fs.writeFileSync(join(services, 'review-worktree-lease.ts'), 'leases the review worktree');
    ...
    const skillDir = join(repo, 'packages', 'core', 'src', 'skills', 'bundled', 'review');
...
export function stampDigest(fs, repo, digest) {
