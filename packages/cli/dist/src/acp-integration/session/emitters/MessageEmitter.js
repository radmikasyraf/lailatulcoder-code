...
import { createTranscriptMessageUpdate, createTranscriptUsageUpdate, } from '@lailatul-coder/acp-bridge/transcriptReplay';
import { apiActivityTracker, getActiveGoal, projectGoalStateToLegacy, } from '@lailatul-coder/lailatul-coder-core';
 ...
 */
export function buildGoalStatusUpdate(status) {
    return {
 ...
 */
export function buildGoalStateUpdate(snapshot, cause, previousGoal = null) {
...
export class MessageEmitter extends BaseEmitter {
    ...
    async emitGoalStatus(status, goalState) {
    ...
    async emitGoalState(snapshot, cause, previousGoal = null) {
    ...
    async emitUsageMetadata(usageMetadata, text = '', durationMs, subagentMeta) {
        ...
        // standard usage_update frame rather than LailatulCoder's private `_meta.usage`.
        ...
        const used = usageMetadata.promptTokenCount ?? usageMetadata.totalTokenCount;
