...
export class WorkspaceContext {
    ...
    addDirectory(directory, basePath = process.cwd()) {
        try {
            const resolved = WorkspaceContext.resolveAndValidateDir(directory, basePath);
    ...
    static resolveAndValidateDir(directory, basePath = process.cwd()) {
    ...
    static resolveRootDirectories(directory, additionalDirectories = []) {
        ...
        for (const additionalDirectory of additionalDirectories) {
            directories.add(WorkspaceContext.resolveAndValidateDir(additionalDirectory));
        }
        return {
            directories,
    ...
    applyRootDirectories(resolved) {
        ...
        const directoriesChanged = newDirectories.size !== this.directories.size ||
            ![...newDirectories].every((d) => this.directories.has(d));
        const initialDirectoriesChanged = newInitialDirectories.size !== this.initialDirectories.size ||
 ...
 */
export function resolveWorkspacePath(pathToCheck) {
    try {
...
function resolveMissingPath(pathToCheck) {
    const missingTail = [];
...
function isResolvableMissingPathError(error) {
 ...
 */
function isFileSymlink(filePath) {
    try {
