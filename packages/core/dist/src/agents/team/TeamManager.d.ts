...
import type { TeamFile, SwarmTask } from './types.js';
...
export declare class TeamManager {
    ...
    constructor(backend: Backend, teamFile: TeamFile, subagentManager?: SubagentManager | null, options?: {
        maxTeammates?: number;
    });
    /**
     ...
     */
    sendMessage(toName: string, message: string, from?: string, summary?: string, automatic?: boolean): Promise<void>;
    /**
     ...
     */
    getLeaderMessages(): Promise<Array<{
        from: string;
        text: string;
        timestamp: string;
    }>>;
     ...
     */
    setLeaderMessageCallback(cb: ((message: string, display: string) => void) | null): void;
    requestPlanApproval(request: TeamPlanApprovalRequest): Promise<TeamPlanApprovalDecision>;
    resolvePlanApprovalRequest(requestId: string, decision: TeamPlanApprovalDecision): void;
    /**
     ...
     */
    formatLeaderEnvelope(messages: ReadonlyArray<{
        from: string;
        text: string;
    }>): string[];
    /**
     ...
     * can diverge.
     */
    formatLeaderDisplay(messages: ReadonlyArray<{
        from: string;
        summary?: string;
    }>): string;
    /**
     ...
     */
    waitForTeammateActivity(timeoutMs?: number, signal?: AbortSignal): Promise<'message' | 'terminated' | 'timeout' | 'aborted'>;
    /**
     ...
     */
    dispatchAssignedTask(task: SwarmTask): Promise<boolean>;
    /**
