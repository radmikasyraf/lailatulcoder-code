...
import { checkForUpdates, checkForUpdatesDetailed, classifyUpdateCheckError, fetchGlobalNpmUpdateInfo, FETCH_TIMEOUT_MS, runGlobalNpm, UpdateCheckTimeoutError, } from './updateCheck.js';
...
describe('checkForUpdates', () => {
    ...
    it('checks npm updates in the global npm context', async () => {
        ...
        await expect(fetchGlobalNpmUpdateInfo('@lailatul-coder/lailatul-coder', '1.0.0', 'latest', run)).resolves.toMatchObject({ current: '1.0.0', latest: '1.1.0' });
        const npmArgs = [
            'view',
        ...
        expect(run).toHaveBeenCalledWith(process.execPath, [expect.stringMatching(/npm-cli\.js$/), ...npmArgs], expect.objectContaining({
    ...
    it('always resolves the update check via npm view, regardless of installation type (#7515)', async () => {
        ...
        await expect(checkForUpdatesDetailed(fetchGlobalNpm)).resolves.toMatchObject({
            status: 'update',
            info: { update: { current: '1.0.0', latest: '1.1.0' } },
        });
        expect(fetchGlobalNpm).toHaveBeenCalledWith('@lailatul-coder/lailatul-coder', '1.0.0', 'latest');
    });
    it('runs the Windows npm CLI through Node without a shell', async () => {
        ...
        const resolveNpmCliPath = vi
            .fn()
            .mockReturnValue('C:\\Program Files\\nodejs\\node_modules\\npm\\bin\\npm-cli.js');
        await runGlobalNpm(['view', '@lailatul-coder/lailatul-coder'], run, 'win32', 'C:\\Program Files\\nodejs\\node.exe', resolveNpmCliPath);
        expect(run).toHaveBeenCalledWith('C:\\Program Files\\nodejs\\node.exe', [
            'C:\\Program Files\\nodejs\\node_modules\\npm\\bin\\npm-cli.js',
            'view',
            '@lailatul-coder/lailatul-coder',
        ], expect.not.objectContaining({ shell: true }));
        expect(resolveNpmCliPath).toHaveBeenCalledWith('C:\\Program Files\\nodejs\\node.exe', 'win32');
    });
    it('does not fall back when the global npm query fails', async () => {
        const run = vi.fn().mockRejectedValue(new Error('npm view failed'));
        await expect(fetchGlobalNpmUpdateInfo('@lailatul-coder/lailatul-coder', '1.0.0', 'latest', run)).rejects.toThrow('npm view failed');
    });
    it('treats an empty dist-tag response as no update', async () => {
        const run = vi.fn().mockResolvedValue({ stdout: '\n', stderr: '' });
        await expect(fetchGlobalNpmUpdateInfo('@lailatul-coder/lailatul-coder', '1.0.0', 'nightly', run)).resolves.toMatchObject({ current: '1.0.0', latest: '1.0.0' });
    });
    ...
    it.each([
        ...
        await expect(fetchGlobalNpmUpdateInfo('@lailatul-coder/lailatul-coder', '1.0.0', 'latest', run)).rejects.toThrow('Invalid npm latest version response');
    });
    it('should pass the exact package name and version to fetchGlobalNpm', async () => {
        ...
        expect(fetchGlobalNpm).toHaveBeenCalledWith('test-package', '1.0.0', 'latest');
    });
    ...
    describe('nightly updates', () => {
        it('should notify for a newer nightly version when current is nightly', async () => {
            ...
            const fetchGlobalNpm = vi
                .fn()
                .mockImplementation(async (name, current, distTag) => ({
    ...
    describe('fetchInfo timeout (#6857)', () => {
        ...
        it('surfaces a timeout when only the nightly dist-tag stalls', async () => {
            ...
            const fetchGlobalNpm = vi
                .fn()
                .mockImplementation(async (name, current, distTag) => distTag === 'nightly'
                ? new Promise(() => { }) // never resolves
                : { current, latest: '1.0.0', type: 'latest', name });
            const resultPromise = checkForUpdatesDetailed(fetchGlobalNpm);
            await vi.advanceTimersByTimeAsync(FETCH_TIMEOUT_MS + 1);
            const result = await resultPromise;
            expect(result.status).toBe('error');
...
describe('classifyUpdateCheckError', () => {
    it('classifies UpdateCheckTimeoutError as timeout', () => {
        expect(classifyUpdateCheckError(new UpdateCheckTimeoutError(5000))).toBe('timeout');
    });
    ...
    it('classifies network codes embedded in the message as offline', () => {
        ...
        expect(classifyUpdateCheckError(new Error('npm error code ENOTFOUND\nnpm error network'))).toBe('offline');
    });
    it('classifies network codes from the error cause as offline', () => {
        const cause = new Error('getaddrinfo failed');
        cause.code = 'ENOTFOUND';
        expect(classifyUpdateCheckError(new TypeError('fetch failed', { cause }))).toBe('offline');
    });
    it('classifies other errors as registry', () => {
        expect(classifyUpdateCheckError(new Error('404 Not Found'))).toBe('registry');
