 ...
 */
const INTERNAL_SESSION_ID_REGEX = /^[0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}(-agent-[a-zA-Z0-9_.-]+)?$/i;
...
const CALLER_SUPPLIED_SESSION_ID_REGEX = /^[0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/i;
 ...
 */
export function normalizeSessionIdForLookup(value) {
    return CALLER_SUPPLIED_SESSION_ID_REGEX.test(value)
        ? value.toLowerCase()
        : value;
}
export function parseCallerSuppliedSessionId(value) {
