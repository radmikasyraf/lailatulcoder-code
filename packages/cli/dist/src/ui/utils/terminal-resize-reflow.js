...
import { countOccurrences, createEraseLinesPattern, ERASE_LINE, } from './terminalRedrawOptimizer.js';
...
function greedyRows(charWidths, columns) {
    const rows = [];
    let current = [];
    let used = 0;
    const flush = () => {
...
function reflowModel(model, columns) {
    ...
    if (model.trailingNewline && lines[lines.length - 1] === '')
        lines.pop();
    let total = 0;
 ...
 */
export function buildWakeRepaint(deps) {
    return () => {
        if (deps.isVP) {
 ...
 */
export function installTerminalResizeReflow(stdout, options = {}) {
    ...
    const modelFrame = (content, bypassMin = false) => {
    ...
    const reflowWrite = function (chunk, encodingOrCallback, callback) {
            ...
            if (match) {
                ...
                if (isVP && Date.now() < clearUntil) {
                    ...
                    if (count < target) {
                        ...
                        const prefix = chunk
                            .slice(0, match.index)
                            .replace(CURSOR_DOWN_PATTERN, (_m, n) => `${ESC}${Number(n) + delta}B`);
    ...
    return {
        ...
        repaint: () => {
            ...
            originalWrite.call(stdout, model.columns === columns && model.content
                ? CLEAR_VIEWPORT + model.content
                : CLEAR_VIEWPORT);
        },
    };
}
//# sourceMappingURL=terminal-resize-reflow.js.map