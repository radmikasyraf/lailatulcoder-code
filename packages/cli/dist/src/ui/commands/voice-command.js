...
import { getVoiceSettingsScope, isVoiceEnabled, readVoiceMode, readVoiceModel, } from '../../services/voice-settings.js';
    ...
    action: (context, args) => {
        ...
        if (command === 'status') {
            ...
            return {
                type: 'message',
                messageType: 'info',
                content: t('Voice dictation: {{status}} (mode: {{mode}}, {{modelText}}).', {
                    status,
        ...
        if (command === 'off') {
            settings.setValue(getVoiceSettingsScope(settings), 'general.voice.enabled', false);
            return {
                type: 'message',
                messageType: 'info',
        ...
        if (!voiceModel) {
            return {
                type: 'message',
                messageType: 'warning',
                content: t('No voice model selected. Run /model --voice to choose one before enabling voice dictation.'),
            };
        }
        ...
        return {
            ...
            messageType: 'info',
            content: mode === 'tap'
                ? t('Voice dictation enabled (tap mode). Tap Space at an empty prompt to start, tap again or pause to stop and submit, using {{voiceModel}}.', { voiceModel })
                : t('Voice dictation enabled (hold mode). Hold Space at an empty prompt to dictate with {{voiceModel}}.', { voiceModel }),
        };
    },
};
