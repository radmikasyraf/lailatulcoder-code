...
import { parseInstallSource, redactUrlCredentials, getErrorMessage, SettingScope, ExtensionNotUpdatableError, isSupportedArchiveUrl, } from '@lailatul-coder/lailatul-coder-core';
...
const EXTENSION_INTERACTIVE_PREPARE_DEADLINE_MS = EXTENSION_PREPARE_DEADLINE_MS + EXTENSION_INTERACTION_DEADLINE_MS;
...
const parseExtensionArchiveFilename = (value) => {
    const invalidCharacter = typeof value === 'string' &&
    ...
    if (lower.endsWith('.tar.gz'))
        return { filename: value, suffix: '.tar.gz' };
    if (lower.endsWith('.zip'))
        return { filename: value, suffix: '.zip' };
    return null;
};
// Runs before the body parser so invalid uploads are rejected without
// buffering up to the full archive limit.
const extensionArchiveUploadMetadata = (req, res, next) => {
...
const parseExtensionScope = (body, res) => {
...
const parseExtensionBatchNames = (req, res, safeBody) => {
...
const parseExtensionRegistryUrl = (value, res) => {
    let parsed;
    try {
...
const parsePotentialSourceUrl = (source) => {
...
const parseExtensionInstallSource = (source, persistence, res) => {
    ...
    const hasUserInfo = !!parsed.username || !!parsed.password || authority?.includes('@') === true;
    if (!hasUserInfo) {
        if (persistence !== undefined) {
            res.status(400).json({
                error: '`credentialPersistence` requires source URL credentials',
    ...
    const invalidText = (value, maxBytes) => Buffer.byteLength(value, 'utf8') > maxBytes || hasControlCharacter(value);
...
const validateExtensionSourceMetadata = (installMetadata) => {
...
const findLoadedExtension = (extensionManager, extensionName) => {
    const requested = extensionName.toLowerCase();
    const extensions = extensionManager.getLoadedExtensions();
    const byName = extensions.find((extension) => extension.name.toLowerCase() === requested);
    if (byName)
        return byName;
    if (!extensionName.includes('://') && !extensionName.includes('@')) {
        return undefined;
    }
    return extensions.find((extension) => extension.installMetadata?.source?.toLowerCase() === requested);
};
export function registerWorkspaceExtensionRoutes(app, deps) {
    const { boundWorkspace, bridge, workspace, mutate, safeBody, sendBridgeError, workspaceRegistry, } = deps;
    const maxExtensionOperationHistory = deps.maxExtensionOperationHistory;
    const controllerDeps = (ws, wsBridge, wsService) => {
        const isWorkspaceTrusted = ws === boundWorkspace
    ...
    const primaryController = createExtensionsController(controllerDeps(boundWorkspace, bridge, workspace));
    const pendingExtensionInteractions = new Map();
    const supersededInstallOperations = new Set();
    const cancelPendingExtensionInteraction = (operationId, reason) => {
    ...
    const supersedeActiveInstallOperations = (controller, currentOperationId) => {
        for (const operation of controller.getActiveOperations()) {
            ...
            cancelPendingExtensionInteraction(operation.operationId, 'Extension installation cancelled by a new install request');
        }
    };
    const waitForExtensionInteraction = (controller, operationId, interaction) => {
        if (supersededInstallOperations.delete(operationId)) {
            return Promise.reject(new Error('Extension installation superseded by a new install request'));
        }
    ...
    const extensionInteractionHandlers = (controller, operationId) => ({
        requestSetting: (setting) => waitForExtensionInteraction(controller, operationId, {
            kind: 'setting',
            setting: {
                name: setting.name,
        ...
        requestChoicePlugin: (marketplace) => {
            if (marketplace.plugins.length === 0) {
                return Promise.reject(new Error(`Marketplace "${marketplace.name}" has no plugins`));
            }
            return waitForExtensionInteraction(controller, operationId, {
                ...
                plugins: marketplace.plugins.map((plugin) => ({
                    ...
                    source: redactExtensionDisplaySource(typeof plugin.source === 'string'
                        ? plugin.source
    ...
    const reserveRuntimeReconciliation = () => {
        let provideTask;
        ...
        return {
            run: async (run) => {
                if (used)
                    throw new Error('Runtime reconciliation already released');
                used = true;
                provideTask(deps.conversationRuntimeActivity
    ...
    const appliedGenerationByWorkspaceId = new Map();
    const onRuntimeReconciled = (runtime, generation) => {
        appliedGenerationByWorkspaceId.set(runtime.workspaceId, generation);
    };
    const globalReconciliationOptions = () => workspaceRegistry
        ? {
        ...
        : {};
    const workspaceReconciliationOptions = () => workspaceRegistry
        ? {
        ...
        : {};
    const mutationClientBridges = (runtimes) => (typeof runtimes === 'function'
        ? runtimes()
        : (runtimes ?? workspaceRegistry?.listAll()))?.map((runtime) => runtime.bridge) ?? [bridge];
    if (workspaceRegistry) {
        let observedGeneration;
        let reconciling = false;
        const reconcileExternalGeneration = async () => {
            if (reconciling)
                return;
            reconciling = true;
            try {
                const manager = primaryController.createExtensionManager(boundWorkspace, true);
                const generation = (await manager.getExtensionStoreSnapshot())
                    .generation;
                const pendingRuntimes = workspaceRegistry
                    .listAll()
                    .filter((runtime) => (appliedGenerationByWorkspaceId.get(runtime.workspaceId) ?? 0) !==
                    generation);
                ...
                if (runtimes.length === 0)
                    return;
                const reconcile = () => runtimeReconciliationQueue.run(async () => await Promise.allSettled(runtimes.map(async (runtime) => {
                    runtime.workspaceService.invalidateWorkspaceSkillsStatus();
                    try {
                        const result = await runtime.bridge.refreshExtensionsForAllSessions();
                        if (result.failed > 0) {
                            throw new Error(`${result.failed} extension session refresh(es) failed`);
                        }
                    }
                    finally {
                        runtime.workspaceService.invalidateWorkspaceSkillsStatus();
                ...
                results.forEach((result, index) => {
                    if (result.status === 'fulfilled') {
                        ...
                        writeStderrLine(`LailatulCoder serve: extension generation reconciliation failed for workspace ${runtimes[index].workspaceId}: ${redactUrlCredentials(result.reason instanceof Error
                            ? result.reason.message
                            : String(result.reason))}`);
                    }
                });
                if (runtimes.length === pendingRuntimes.length &&
                    results.every((result) => result.status === 'fulfilled')) {
                    observedGeneration = generation;
                }
            }
            catch (error) {
                writeStderrLine(`LailatulCoder serve: extension generation reconciliation failed: ${redactUrlCredentials(error instanceof Error ? error.message : String(error))}`);
            }
            finally {
                reconciling = false;
            }
        };
        const generationPoller = setInterval(() => void reconcileExternalGeneration(), 30_000);
        generationPoller.unref();
        app.locals.stopExtensionGenerationReconciler = () => clearInterval(generationPoller);
    }
    const registerFor = (base, resolve) => {
        ...
        app.get(`${base}/operations/:operationId`, async (req, res) => {
            ...
            try {
                ...
                if (base === '/workspace/extensions' &&
                    operation.status === 'succeeded_with_warnings') {
                    const warningError = operation.warnings?.find((warning) => warning.code === undefined)
                        ?.error ??
        ...
        app.post(`${base}/operations/:operationId/interactions/:interactionId`, mutate({ strict: true }), async (req, res) => {
            const ctrl = resolve(req, res, false);
            if (!ctrl)
                return;
            try {
                ...
                if (pending.interaction.kind === 'marketplace_plugin') {
                    ...
                    if (typeof pluginName !== 'string' ||
                        !pending.interaction.plugins.some((plugin) => plugin.name === pluginName)) {
                        res
                            .status(400)
        ...
        app.post(`${base}/install-archive`, mutate({ strict: true }), extensionArchiveUploadMetadata, extensionArchiveBodyParser, async (req, res) => {
            ...
            try {
                ...
                const archiveName = parseExtensionArchiveFilename(req.query['filename']);
                if (!req.is('application/octet-stream')) {
                    res.status(415).json({
                        error: 'Extension archive uploads require application/octet-stream',
                    });
                    return;
                }
                ...
                let archive = req.body;
                const source = `upload:v1:${crypto.randomUUID()}:${archiveName.filename}`;
                ctrl.runQueuedExtensionMutation('install', { source }, res, async (extensionManager, _signal, context, operationId) => {
                    let uploadDir;
                    const sanitizeUploadError = (error) => {
                        const sanitized = new Error(uploadDir
                            ? getErrorMessage(error).replaceAll(uploadDir, '<extension-upload-dir>')
                    ...
                    let failure;
                    try {
                        uploadDir = await fs.mkdtemp(path.join(os.tmpdir(), 'LailatulCoder-extension-upload-'));
                        const archivePath = path.join(uploadDir, `extension${archiveName.suffix}`);
                        ...
                        try {
                            const committed = await context.commit(async (onCommitted) => await extensionManager.commitPreparedExtension(prepared, onCommitted));
                            result = {
                                status: 'installed',
                                source,
                    ...
                    createManager: (operationId) => ctrl.createExtensionManager(undefined, undefined, extensionInteractionHandlers(ctrl, operationId)),
                    onSettled: (operationId) => {
                        supersededInstallOperations.delete(operationId);
                        cancelPendingExtensionInteraction(operationId, 'Extension operation ended');
                    },
        ...
        app.post(`${base}/install`, mutate({ strict: true }), async (req, res) => {
            ...
            try {
                ...
                const autoUpdateValue = typeof autoUpdate === 'boolean' ? autoUpdate : undefined;
                const allowPreReleaseValue = typeof allowPreRelease === 'boolean' ? allowPreRelease : undefined;
                const registryValue = typeof registry === 'string' ? registry : undefined;
                ...
                const parsedSource = parseExtensionInstallSource(source, credentialPersistence, res);
                if (!parsedSource)
                    return;
                ...
                const localSource = /^[A-Za-z]:[\\/]/.test(sourceValue) ||
                ...
                if (localSource) {
                    try {
                        ...
                        if (metadata.type !== 'git' &&
                            ...
                            metadata.type !== 'npm') {
                            res.status(400).json({
                                error: 'Only GitHub, Git, and npm extension installs are supported over the daemon endpoint.',
                            });
                            return;
                        }
                    }
                    catch (error) {
                        const message = error instanceof Error ? error.message : 'Invalid install source';
                        res.status(400).json({
                            error: redactUrlCredentials(message.replaceAll(sourceValue, redactExtensionDisplaySource(sourceValue))),
                        });
                        return;
                    }
                }
                const npmSource = sourceValue.startsWith('@');
                const remoteSource = npmSource ||
                    sourceValue.startsWith('https://') ||
                    /^[^/\s]+\/[^/\s]+/.test(sourceValue);
                if (!localSource && !remoteSource) {
                    res.status(400).json({
                        error: `Install source not found: ${redactExtensionDisplaySource(sourceValue)}`,
                    });
                    return;
                }
                ...
                ctrl.runQueuedExtensionMutation('install', gitCredential?.persistence === 'one_time'
                    ? {}
                    : { source: sourceValue }, res, async (extensionManager, _signal, context, operationId) => {
                    const prepared = await context.prepare(async (signal) => {
                        ...
                        if (installMetadata.type !== 'git' &&
                            ...
                            throw new Error('Only GitHub, Git, and npm extension installs are supported over the daemon endpoint.');
                        }
                        if (gitCredential &&
                        ...
                        if (installMetadata.type !== 'npm' && registryValue) {
                            throw new Error('--registry is only applicable for npm extensions.');
                        }
        ...
        app.post(`${base}/check-updates`, mutate({ strict: true }), async (req, res) => {
            const ctrl = resolve(req, res, true);
            if (!ctrl)
                return;
            let timer;
            let releaseOperationSlot;
            try {
                if (!ctrl.validateExtensionMutationClient(req, res, {
                    requireClientId: false,
                })) {
                ...
                if (!releaseOperationSlot)
                    return;
                const extensionManager = ctrl.createExtensionManager();
                const updateStates = Object.create(null);
                const deadline = new AbortController();
                timer = setTimeout(() => {
                    const error = new Error('Extension update check exceeded its preparation deadline.');
                    error.code = 'extension_prepare_timeout';
                    deadline.abort(error);
                }, EXTENSION_UPDATE_CHECK_DEADLINE_MS);
                timer.unref();
                let rejectRefreshOnAbort;
                try {
                    await Promise.race([
                        extensionManager.refreshCache(),
                        new Promise((_resolve, reject) => {
                            rejectRefreshOnAbort = () => reject(deadline.signal.reason);
                            deadline.signal.addEventListener('abort', rejectRefreshOnAbort, { once: true });
                        }),
                    ]);
                }
                finally {
                    if (rejectRefreshOnAbort) {
                        deadline.signal.removeEventListener('abort', rejectRefreshOnAbort);
                    }
                }
                await extensionManager.checkForAllExtensionUpdates((name, state) => {
                    updateStates[name] = state;
                }, deadline.signal, async (task) => await ctrl.preparationQueue.run(task, {
        ...
        app.post(`${base}/:name/enable`, mutate({ strict: true }), async (req, res) => {
            const ctrl = resolve(req, res, true);
            if (!ctrl)
                return;
            try {
                ...
                if (scope === null)
                    return;
                ctrl.runQueuedExtensionMutation('enable', { name }, res, async (extensionManager, _signal, context) => {
                    const extension = findLoadedExtension(extensionManager, name);
                    if (!extension) {
                        throw new Error(`Extension "${name}" not found`);
                    }
                    await context.commit(async (onCommitted) => await extensionManager.enableExtension(extension.name, scope, ctrl.boundWorkspace, onCommitted));
        ...
        app.post(`${base}/:name/disable`, mutate({ strict: true }), async (req, res) => {
            const ctrl = resolve(req, res, true);
            if (!ctrl)
                return;
            try {
                ...
                if (scope === null)
                    return;
                ctrl.runQueuedExtensionMutation('disable', { name }, res, async (extensionManager, _signal, context) => {
                    const extension = findLoadedExtension(extensionManager, name);
                    if (!extension) {
                        throw new Error(`Extension "${name}" not found`);
                    }
                    await context.commit(async (onCommitted) => await extensionManager.disableExtension(extension.name, scope, ctrl.boundWorkspace, onCommitted));
        ...
        app.post(`${base}/:name/update`, mutate({ strict: true }), async (req, res) => {
            const ctrl = resolve(req, res, true);
            if (!ctrl)
                return;
            try {
                ...
                ctrl.runQueuedExtensionMutation('update', { name }, res, async (extensionManager, _signal, context) => {
                    const extension = findLoadedExtension(extensionManager, name);
                    if (!extension) {
                        throw new Error(`Extension "${name}" not found`);
                    }
                    let preparedResult;
                    try {
                        preparedResult = await context.prepare(async (signal) => await extensionManager.prepareExtensionUpdate({
                            extension,
                            signal,
                        }));
                    }
                    catch (error) {
                        const wrapped = new Error(`Update check failed for extension "${extension.name}": ${error instanceof Error ? error.message : String(error)}`, { cause: error });
                        if (error &&
                            typeof error === 'object' &&
                            typeof error.code === 'string') {
                    ...
                    try {
                        const committed = await context.commit(async (onCommitted) => await extensionManager.commitPreparedExtension(preparedResult.prepared, onCommitted));
                        return {
                            status: 'updated',
                            name: extension.name,
                            version: committed.version,
                        ...
                        await extensionManager.disposePreparedExtension(preparedResult.prepared);
                    }
                }, {
                    createManager: (operationId) => ctrl.createExtensionManager(undefined, undefined, extensionInteractionHandlers(ctrl, operationId)),
                    onSettled: (operationId) => {
        ...
        app.delete(`${base}/:name`, mutate({ strict: true }), async (req, res) => {
            ...
            try {
                ...
                ctrl.runQueuedExtensionMutation('uninstall', { name }, res, async (extensionManager, _signal, context) => {
                    const extension = findLoadedExtension(extensionManager, name);
                    if (!extension) {
                        throw new Error(`Extension "${name}" not found`);
                    }
                    await context.commit(async (onCommitted) => await extensionManager.uninstallExtension(extension.name, false, ctrl.boundWorkspace, onCommitted));
    ...
    const extensionById = (manager, extensionId) => manager
        .getLoadedExtensions()
        .find((extension) => extension.id === extensionId);
    const parseExtensionId = (req, res) => {
    ...
    const parseActivationState = (req, res) => {
    ...
    const parseWorkspaceBatchActivationState = (req, res) => {
        const state = safeBody(req)['state'];
    ...
    const sendOperation = (req, res, route, manager, operation, failureContext, run, options = {}) => {
        if (!primaryController.validateExtensionMutationClient(req, res, {
            requireClientId: false,
            bridges: mutationClientBridges(options.refreshRuntimes),
        })) {
            return;
        }
        primaryController.runQueuedExtensionMutation(operation, failureContext, res, run, {
            manager,
            ...
            res.status(200).json({
                ...
                extensions: manager.getLoadedExtensions().map((extension) => {
                    ...
                    return {
                        ...
                        workspaceOverrideCount: Object.values(policy?.workspaceOverrides ?? {}).filter((activation) => activation !== 'inherit').length,
                    };
                }),
            });
        }
    ...
    app.put('/extensions/activation', mutate({ strict: true }), (req, res) => {
        ...
        if (!state)
            return;
        const manager = primaryController.createExtensionManager(boundWorkspace, true);
        sendOperation(req, res, 'PUT /extensions/activation', manager, 'set_default_activation_batch', {}, async (extensionManager, _signal, context) => {
            await context.commit(async (onCommitted) => await extensionManager.setExtensionDefaultActivations(names, state, onCommitted));
            return {
                status: 'updated',
    ...
    app.put('/extensions/:extensionId/activation', mutate({ strict: true }), async (req, res) => {
        ...
        if (!state)
            return;
        const manager = primaryController.createExtensionManager(boundWorkspace, true);
        sendOperation(req, res, 'PUT /extensions/:extensionId/activation', manager, 'activation', { name: extensionId }, async (extensionManager, _signal, context) => {
            const extension = extensionById(extensionManager, extensionId);
            if (!extension)
                throw new Error(`Extension "${extensionId}" not found`);
            await context.commit(async (onCommitted) => await extensionManager.setExtensionDefaultActivation(extensionId, state, onCommitted));
            return {
    ...
    app.post('/extensions/install', mutate({ strict: true }), (req, res) => {
        ...
        else if (activationRecord['scope'] === 'workspace' &&
            typeof activationRecord['workspaceId'] === 'string' &&
            workspaceRegistry) {
            const runtime = workspaceRegistry.getByWorkspaceId(activationRecord['workspaceId']);
            if (!runtime) {
                res.status(400).json({
        ...
        const manager = primaryController.createExtensionManager(boundWorkspace, true);
        sendOperation(req, res, 'POST /extensions/install', manager, 'install', gitCredential?.persistence === 'one_time' ? {} : { source: sourceValue }, async (extensionManager, _signal, context) => {
            const prepared = await context.prepare(async (signal) => {
                const metadata = await parseInstallSource(sourceValue, {
                    networkPolicy: 'public',
                });
    ...
    app.post('/extensions/check-updates', mutate({ strict: true }), (req, res) => {
        const manager = primaryController.createExtensionManager(boundWorkspace, true);
        sendOperation(req, res, 'POST /extensions/check-updates', manager, 'check-updates', {}, async (extensionManager, signal, context) => {
            const states = Object.create(null);
            await extensionManager.checkForAllExtensionUpdates((name, state) => {
                states[name] = state;
            }, signal, async (task) => await context.prepare(async () => await task()));
    ...
    app.post('/extensions/:extensionId/update', mutate({ strict: true }), (req, res) => {
        const extensionId = parseExtensionId(req, res);
        if (!extensionId)
            return;
        const manager = primaryController.createExtensionManager(boundWorkspace, true);
        sendOperation(req, res, 'POST /extensions/:extensionId/update', manager, 'update', { name: extensionId }, async (extensionManager, _signal, context) => {
            const extension = extensionById(extensionManager, extensionId);
            if (!extension)
                throw new Error(`Extension "${extensionId}" not found`);
            ...
            const preparedResult = await context.prepare(async (signal) => await extensionManager.prepareExtensionUpdate({
                extension,
                signal,
            }));
            if (preparedResult.upToDate) {
    ...
    app.delete('/extensions/:extensionId', mutate({ strict: true }), async (req, res) => {
        const extensionId = parseExtensionId(req, res);
        if (!extensionId)
            return;
        ...
        try {
            ...
            sendOperation(req, res, route, manager, 'uninstall', { name: policy.name }, async (extensionManager, _signal, context) => {
                await context.commit(async (onCommitted) => await extensionManager.uninstallExtensionById(extensionId, false, undefined, onCommitted));
    ...
    if (workspaceRegistry) {
        ...
        app.get('/workspaces/:workspace/extensions', async (req, res) => {
            ...
            if (!runtime)
                return;
            try {
                const manager = primaryController.createExtensionManager(runtime.workspaceCwd, runtime.trusted);
                const snapshot = await manager.refreshCacheWithSnapshot();
                runtime.generationGuard?.assertOpen();
                const extensions = manager.getLoadedExtensions().map((extension) => {
                    const activation = manager.getExtensionActivationFromSnapshot(extension.id, snapshot, runtime.workspaceCwd);
                    return {
                        extensionId: extension.id,
                        name: extension.name,
                        version: extension.version,
                        defaultActivation: activation.default,
                        workspaceActivation: activation.workspace === 'inherit' ? null : activation.workspace,
                ...
                res.status(200).json({
                    ...
                    appliedGeneration: appliedGenerationByWorkspaceId.get(runtime.workspaceId) ?? 0,
                    extensions,
                });
            }
            catch (error) {
        ...
        app.put('/workspaces/:workspace/extensions/activation', mutate({ strict: true }), (req, res) => {
            const runtime = resolveWorkspaceRuntimeFromParam(registry, req, res);
            ...
            if (!state)
                return;
            const manager = primaryController.createExtensionManager(runtime.workspaceCwd, true);
            sendOperation(req, res, 'PUT /workspaces/:workspace/extensions/activation', manager, 'set_workspace_activation_batch', {}, async (extensionManager, _signal, context) => {
                const snapshot = await context.commit(async (onCommitted) => await extensionManager.setExtensionWorkspaceActivations(names, runtime.workspaceCwd, state, onCommitted));
                return {
                    status: 'updated',
                    ...
                    results: names.map((name) => ({
                        ...
                        effectiveActivation: extensionManager.getExtensionActivationForNameFromSnapshot(name, snapshot, runtime.workspaceCwd).effective,
                    })),
                };
            }, {
        ...
        app.put('/workspaces/:workspace/extensions/:extensionId/activation', mutate({ strict: true }), (req, res) => {
            const runtime = resolveWorkspaceRuntimeFromParam(registry, req, res);
            if (!runtime || !requireTrustedWorkspaceRuntime(runtime, res))
                return;
            const extensionId = parseExtensionId(req, res);
            ...
            if (!state)
                return;
            const manager = primaryController.createExtensionManager(runtime.workspaceCwd, true);
            sendOperation(req, res, 'PUT /workspaces/:workspace/extensions/:extensionId/activation', manager, 'activation', { name: extensionId }, async (extensionManager, _signal, context) => {
                const extension = extensionById(extensionManager, extensionId);
                if (!extension) {
                    throw new Error(`Extension "${extensionId}" not found`);
                }
                await context.commit(async (onCommitted) => await extensionManager.setExtensionWorkspaceActivation(extensionId, runtime.workspaceCwd, state, onCommitted));
                return {
                    status: state === 'enabled' ? 'enabled' : 'disabled',
                    name: extension.name,
                };
        ...
        app.delete('/workspaces/:workspace/extensions/:extensionId/activation', mutate({ strict: true }), (req, res) => {
            const runtime = resolveWorkspaceRuntimeFromParam(registry, req, res);
            if (!runtime || !requireTrustedWorkspaceRuntime(runtime, res))
                return;
            const extensionId = parseExtensionId(req, res);
            if (!extensionId)
                return;
            const manager = primaryController.createExtensionManager(runtime.workspaceCwd, true);
            sendOperation(req, res, 'DELETE /workspaces/:workspace/extensions/:extensionId/activation', manager, 'activation', { name: extensionId }, async (extensionManager, _signal, context) => {
                const extension = extensionById(extensionManager, extensionId);
                if (!extension) {
                    throw new Error(`Extension "${extensionId}" not found`);
                }
                const snapshot = await context.commit(async (onCommitted) => await extensionManager.clearExtensionWorkspaceActivation(extensionId, runtime.workspaceCwd, onCommitted));
                const activation = extensionManager.getExtensionActivationFromSnapshot(extensionId, snapshot, runtime.workspaceCwd);
        ...
        app.post('/workspaces/:workspace/extensions/refresh', mutate({ strict: true }), (req, res) => {
            const runtime = resolveWorkspaceRuntimeFromParam(registry, req, res);
            if (!runtime || !requireTrustedWorkspaceRuntime(runtime, res))
                return;
            const manager = primaryController.createExtensionManager(runtime.workspaceCwd, true);
            sendOperation(req, res, 'POST /workspaces/:workspace/extensions/refresh', manager, 'refresh', {}, async () => ({ status: 'refreshed' }), {
                refreshRuntimes: [runtime],
                assertGenerationOpen: () => runtime.generationGuard?.assertOpen(),
            });
        });
...
//# sourceMappingURL=workspace-extensions.js.map