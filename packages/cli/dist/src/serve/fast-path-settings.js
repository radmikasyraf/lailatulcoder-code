...
import { DEFAULT_EXCLUDED_ENV_VARS, HOME_ENV_BOOTSTRAP_KEYS, isHardcodedProjectEnvExclusion, isLoaderEnvKey, reportRejectedLoaderKeys, } from '../config/shared-env-keys.js';
import { getGlobalQwenDirLite, getSystemDefaultsPath, getSystemSettingsPath, SETTINGS_DIRECTORY_NAME, } from '../config/storage-paths-lite.js';
import { getPathComparisonVariants } from '../config/path-comparison.js';
import { buildTrustPrecedenceRules, resolveTrustDecision, } from '../config/trust-precedence.js';
...
let cachedTrustedFoldersPath;
let cachedTrustedFolderRules;
function getTrustedFoldersPathFastPath() {
    return (process.env['QWEN_CODE_TRUSTED_FOLDERS_PATH'] ??
        path.join(getGlobalQwenDirLite(), 'trustedFolders.json'));
}
function getUserLevelEnvPathsFastPath() {
...
export function preResolveServeFastPathHomeEnvOverrides() {
...
function readHomeEnvIntoFastPath(file) {
...
function getHomeEnvFallbackVarsFastPath() {
    const globalQwenDir = getGlobalQwenDirLite();
...
function findEnvFilesFastPath(settings, startDir, userLevelPaths = getUserLevelEnvPathsFastPath()) {
    const homeDir = os.homedir();
    ...
    const globalQwenDir = getGlobalQwenDirLite();
    const legacyQwenDir = path.normalize(path.join(homeDir, SETTINGS_DIRECTORY_NAME));
    const hasCustomConfigDir = path.normalize(globalQwenDir) !== legacyQwenDir;
    const found = [];
    const seen = new Set();
    const canUseEnvFile = (filePath) => {
        const normalized = path.normalize(filePath);
        if (userLevelPaths.has(normalized))
            return true;
        const dirPath = path.dirname(normalized);
        const workspaceDir = path.basename(dirPath) === SETTINGS_DIRECTORY_NAME
    ...
    while (true) {
        if (currentDir === homeDir) {
            ...
            const qwenEnvPath = path.join(currentDir, SETTINGS_DIRECTORY_NAME, '.env');
...
function setUpCloudShellEnvironmentFromFilesFastPath(envFilePaths) {
...
let serveFastPathRejectedLoaderKeys = [];
export function loadServeFastPathEnvironment(settings, startDir = process.cwd()) {
    ...
    if (settings.env) {
        ...
        rejectedLoaderKeys.push(...reportRejectedLoaderKeys(`settings.env (${startDir})`, Object.keys(settings.env)));
    }
...
export function consumeServeFastPathRejectedLoaderKeys() {
    const keys = serveFastPathRejectedLoaderKeys;
    serveFastPathRejectedLoaderKeys = [];
    return keys;
}
function readTrustedFolderRulesFastPath() {
    const trustedFoldersPath = getTrustedFoldersPathFastPath();
    if (cachedTrustedFolderRules &&
    ...
    let parsed;
    try {
        parsed = JSON.parse(stripJsonComments(fs.readFileSync(trustedFoldersPath, 'utf8')));
    }
    catch (err) {
        throw new Error(`Failed to read serve fast path trusted folders from ${trustedFoldersPath}: ${err instanceof Error ? err.message : String(err)}`);
    }
    if (!isPlainObject(parsed)) {
        throw new Error(`Serve fast path trusted folders file ${trustedFoldersPath} must be a JSON object.`);
    }
    const out = {};
...
function buildTrustedFolderRules(trustedFolders) {
    return buildTrustPrecedenceRules(Object.entries(trustedFolders).map(([rulePath, trustLevel]) => ({
        path: rulePath,
        trustLevel,
    })));
}
function isPathTrustedFastPath(location) {
    const rules = readTrustedFolderRulesFastPath();
    return resolveTrustDecision(rules, getPathComparisonVariants(location));
}
function isWorkspaceTrustedFastPath(settings, realWorkspaceDir) {
    if (settings.security?.folderTrust?.enabled !== true) {
        return true;
    }
    return isPathTrustedFastPath(realWorkspaceDir);
}
function readSettingsSummary(filePath) {
    ...
    try {
        ...
        throw new Error(`Failed to read serve fast path settings from ${filePath}: ${err instanceof Error ? err.message : String(err)}`);
    }
    if (!isPlainObject(parsed)) {
        throw new Error(`Serve fast path settings file ${filePath} must be a JSON object.`);
    }
    return pickFastPathSettings(parsed);
}
function shouldUseLegacyFastPathKeys(value) {
...
function pickFastPathSettings(value) {
    const out = {};
    ...
    if (isPlainObject(general)) {
        ...
        if (chatRecording !== undefined && typeof chatRecording !== 'boolean') {
            throw new Error('Serve fast path settings general.chatRecording must be a boolean.');
        }
    ...
    if (isPlainObject(advanced)) {
        ...
        if (excludedEnvVars !== undefined && !isStringArray(excludedEnvVars)) {
            throw new Error('Serve fast path settings advanced.excludedEnvVars must be a string array.');
        }
        ...
        if (runtimeOutputDir !== undefined &&
            ...
            throw new Error('Serve fast path settings advanced.runtimeOutputDir must be a string.');
        }
        if (runtimeOutputDir !== undefined) {
    ...
    if (isPlainObject(security)) {
        ...
        if (isPlainObject(folderTrust)) {
            ...
            if (enabled !== undefined && typeof enabled !== 'boolean') {
                throw new Error('Serve fast path settings security.folderTrust.enabled must be a boolean.');
            }
    ...
    if (useLegacyKeys &&
        ...
        if (legacyFolderTrust !== undefined &&
            ...
            throw new Error('Serve fast path settings folderTrust must be a boolean.');
        }
        if (legacyFolderTrust !== undefined) {
    ...
    if (isPlainObject(tools)) {
        ...
        if (typeof approvalMode === 'string') {
            pickedTools.approvalMode = approvalMode;
        }
...
function mergeFastPathSettings(...sources) {
    const merged = {};
...
export function loadServeFastPathSettings(workspaceDir) {
    ...
    const user = readSettingsSummary(path.join(getGlobalQwenDirLite(), 'settings.json'));
    const initialTrustCheckSettings = mergeFastPathSettings(system, user);
    const isTrusted = isWorkspaceTrustedFastPath(initialTrustCheckSettings, realWorkspaceDir) ??
        true;
    ...
    const workspaceSettingsPath = path.join(realWorkspaceDir, SETTINGS_DIRECTORY_NAME, 'settings.json');
    ...
    return resolveEnvVarsInObject(merged, getHomeEnvFallbackVarsFastPath());
}
...
//# sourceMappingURL=fast-path-settings.js.map