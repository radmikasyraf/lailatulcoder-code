...
export class CursorNotAtLineBoundaryError extends Error {
    ...
    constructor(startOffset, maxSnapBytes) {
        super(`Byte offset ${startOffset} is not the start of a line, and no line break was found within ${maxSnapBytes} bytes after it. Resume from a cursor this reader returned.`);
...
export class TextScanBudgetExceededError extends Error {
    ...
    constructor(scannedBytes, maxScanBytes) {
        super(`Locating the requested line window would read more than ${maxScanBytes} bytes (line offsets are resolved by scanning from the start of the file). Use a byte-offset read to reach this part of the file.`);
...
export async function readTextRange(request) {
    ...
    return readLargeUtf8Range(request.path, request, maxOutputBytes, maxScanBytes, stats.size);
}
/**
 ...
 * never closes it.
 */
export async function readTextRangeFromHandle(fileHandle, request) {
    request.signal?.throwIfAborted();
    return readLargeUtf8Range(fileHandle, request, normalizeMaxBytes(request.maxOutputBytes), request.maxScanBytes, request.fileSize);
}
/**
 ...
 */
export async function readTextCursorWindowFromHandle(fileHandle, request) {
    request.signal?.throwIfAborted();
    ...
    let firstChunk = true;
    let reachedEof = true;
    for await (const raw of chunksFromHandle(fileHandle, startOffset, request.fileSize, request.signal)) {
        request.signal?.throwIfAborted();
        ...
        if (contentBytes + pendingSeparator + Buffer.byteLength(pendingLine, 'utf8') >
            maxOutputBytes) {
            if (lines.length === 0) {
...
async function snapToLineStart(fileHandle, request, allowEof = false) {
    ...
    if (bytesRead === 1 && previous[0] === LINE_FEED)
        return startOffset;
    let scanned = 0;
    const snapEnd = Math.min(fileSize, startOffset + maxSnapBytes);
    for await (const chunk of chunksFromHandle(fileHandle, startOffset, snapEnd, signal)) {
        const index = chunk.indexOf(LINE_FEED);
        if (index !== -1)
            return startOffset + scanned + index + 1;
...
async function readLargeUtf8Range(source, request, maxOutputBytes, maxScanBytes, sourceSize) {
    ...
    let pathStream;
    let chunks;
    const sourceEnd = Math.min(sourceSize ?? Number.POSITIVE_INFINITY, maxScanBytes);
    ...
    const budgetExhausted = !stoppedEarly &&
...
export function detectLineEndingFromContent(content) {
