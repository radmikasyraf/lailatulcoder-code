...
import { parseDurationSeconds, validateMaxToolCalls, validateMaxWallTimeSetting, } from '../utils/runBudget.js';
...
function resolveLocaleForExtensions(settings) {
    const envLang = process.env['QWEN_CODE_LANG'];
    if (envLang)
        return envLang;
    const settingsLang = settings.general?.language;
...
export async function parseArguments() {
    ...
    if (rawArgv.length > 0 &&
        (rawArgv[0].endsWith('/dist/LailatulCoder-cli/cli.js') ||
    ...
    const yargsInstance = yargs(rawArgv)
        ...
        .command('$0 [query..]', 'Launch LailatulCoder Ai CLI', (yargsInstance) => yargsInstance
        .positional('query', {
        ...
        .option('worktree', {
        type: 'string',
        description: 'Start the session inside a git worktree at <repoRoot>/.lailatulcoder/worktrees/<slug>/. ' +
        ...
        if (argv['jsonSchema']) {
            ...
            const hasPositionalQuery = Array.isArray(query)
                ? query.length > 0
                : !!query;
            // Allow stdin piping (`echo "..." | LailatulCoder --json-schema ...`):
        ...
        // Register `LailatulCoder serve` (Stage 1 daemon)
...
export async function loadHierarchicalGeminiMemory(currentWorkingDirectory, includeDirectoriesToReadGemini = [], fileService, extensionContextFilePaths = [], folderTrust, memoryImportFormat = 'tree', contextRuleExcludes = [], options = {}) {
    ...
    return loadServerHierarchicalMemory(effectiveCwd, includeDirectoriesToReadGemini, fileService, extensionContextFilePaths, folderTrust, memoryImportFormat, contextRuleExcludes, options);
}
/**
 ...
 */
function resolveWebSearchSettings(settings) {
    ...
    const enabled = envEnabled !== undefined ? isTruthy(envEnabled) : webSearch?.enabled;
    ...
    const webExtractor = envExtractor !== undefined
...
function resolveMaxWallTimeSeconds(argv, settings) {
    if (argv.maxWallTime !== undefined && argv.maxWallTime !== null) {
        try {
            ...
            throw new Error(`--max-wall-time: ${err.message}`);
        }
    }
    ...
    if (typeof fromSettings === 'number') {
        try {
            ...
            throw new Error(`settings.json: ${err.message}`);
        }
    }
    return -1;
}
 ...
 */
function resolveMaxSubagentDepth(argv, settings) {
    const value = argv.maxSubagentDepth;
    if (value !== undefined && value !== null) {
        if (!Number.isInteger(value) ||
            value < 1 ||
            value > MAX_SUBAGENT_DEPTH_LIMIT) {
            throw new Error(`--max-subagent-depth must be an integer between 1 and ${MAX_SUBAGENT_DEPTH_LIMIT}; got ${value}.`);
        }
        return value;
    }
 ...
 */
export function buildDisabledSkillNamesProvider(loadedSettings) {
...
hostPolicy) {
    ...
    // Pass cwd so that relative paths like ".lailatulcoder" resolve per-project.
    ...
    const fileService = new FileDiscoveryService(cwd, settings.context?.fileFiltering?.customIgnoreFiles);
    const includeDirectories = (bareMode || safeMode ? [] : (settings.context?.includeDirectories ?? []))
        .map(resolvePath)
    ...
    if (safeMode && argv.coreTools && argv.coreTools.length > 0) {
        writeStderrLine('⚠ Safe mode: --core-tools flag is ignored (settings-sourced core tools are also disabled).\n');
    }
    ...
    const disabledTools = bareMode || safeMode
        ? []
        : normalizeDisabledToolList(settings.tools?.disabled);
    const visibleTools = bareMode || safeMode
        ? []
    ...
    const sandboxConfig = await loadSandboxConfig(bareMode || safeMode ? {} : settings, argv);
    const screenReader = argv.screenReader !== undefined
        ? argv.screenReader
        : (settings.ui?.accessibility?.screenReader ?? false);
    let sessionId;
    let sessionData;
    let sessionRestoreProjection;
    const sessionRestoreProjectionSource = hostPolicy?.sessionRestore?.projectionSource;
    ...
    if (argv.continue || argv.resume) {
        ...
        if (argv.resume) {
            ...
            if (sessionRestoreProjectionSource) {
                if (!deferProjectionUntilWriterLease && !argv.forkSession) {
                    addDaemonRequestAttribute('lailatul-coder.daemon.session_restore.projection_acquisition', 'preloaded');
            ...
            if (!sessionRestoreProjectionSource && !sessionData) {
                const message = `No saved session found with ID ${argv.resume}. Run \`lailatulcoder --resume\` without an ID to choose from existing sessions.`;
        ...
        try {
            ...
            debugLogger.debug(`Session id occupancy check failed for ${argv['sessionId']}: ${error}`);
            occupied = true;
        }
        if (occupied) {
    ...
    const boundSessionRestoreProjectionSource = sessionRestoreProjectionSource && restoreSessionId
    ...
    const topTierMcpServers = sessionMcpServers || cliMcpServers
    ...
    const mcpServers = bareMode || safeMode
        ? { ...topTierMcpServers }
    ...
    const pendingMcpServers = bareMode || safeMode || approvalMode === ApprovalMode.YOLO
        ? undefined
    ...
    const configParams = {
        ...
        targetDir: cwd,
        includeDirectories,
        loadMemoryFromIncludeDirectories: bareMode || safeMode
        ...
        coreTools: bareMode || safeMode
            ? undefined
            : argv.coreTools || settings.tools?.core || undefined,
        allowedTools: bareMode || safeMode
        ...
        disabledSkillNamesProvider: bareMode || safeMode ? undefined : disabledSkillNamesProvider,
        terminalImageRenderSupportProvider: interactive
            ? async () => {
                const { getTerminalImageRenderSupport } = await import('../ui/utils/terminal-image-renderer.js');
                return getTerminalImageRenderSupport();
            }
            : undefined,
        disabledSkillLevels: bareMode || safeMode || !Array.isArray(settings.skills?.disabledLevels)
            ? undefined
            : settings.skills.disabledLevels.filter(isSkillLevel),
        customSkillDirs: bareMode || safeMode
            ? undefined
        ...
        toolSearchThreshold: bareMode || safeMode ? 0 : settings.tools?.toolSearch?.threshold,
        // New unified permissions (PermissionManager source of truth).
        permissions: {
            allow: mergedAllow.length > 0 ? mergedAllow : undefined,
            ask: mergedAsk.length > 0 ? mergedAsk : undefined,
            deny: mergedDeny.length > 0 ? mergedDeny : undefined,
            autoMode: bareMode || safeMode ? undefined : settings.permissions?.autoMode,
        },
        ...
        toolDiscoveryCommand: bareMode || safeMode ? undefined : settings.tools?.discoveryCommand,
        toolCallCommand: bareMode || safeMode ? undefined : settings.tools?.callCommand,
        mcpServerCommand: bareMode || safeMode ? undefined : settings.mcp?.serverCommand,
        ...
        // before deferred init are an accepted startup-latency tradeoff. `LailatulCoder -i
        ...
        usageStatisticsEnabled: parseBooleanEnvFlag(process.env['QWEN_USAGE_STATISTICS_ENABLED']) ??
        ...
        computerUseEnabled: (() => {
            ...
            if (tunnelOn && settings.tools?.computerUse?.enabled === true) {
                writeStderrLine('LailatulCoder serve: ignoring tools.computerUse.enabled=true — the CDP ' +
        ...
        bareMode,
        safeMode,
        allowedHttpHookUrls: bareMode || safeMode
            ? []
            : (settings.security?.allowedHttpHookUrls ?? []),
        allowPrivateNetworkHooks: bareMode || safeMode
            ? false
        ...
        enableManagedAutoMemory: bareMode || safeMode
            ? false
            : (settings.memory?.enableManagedAutoMemory ?? true),
        enableManagedAutoDream: bareMode || safeMode
            ? false
            : (settings.memory?.enableManagedAutoDream ?? true),
        enableTeamMemory: bareMode || safeMode
            ? false
            : (settings.memory?.enableTeamMemory ?? false),
        enableTeamMemorySync: bareMode || safeMode
            ? false
            : (settings.memory?.enableTeamMemorySync ?? false),
        enableAutoSkill: bareMode || safeMode
            ? false
            : (settings.memory?.enableAutoSkill ?? false),
        autoSkillConfirm: bareMode || safeMode
            ? false
        ...
        webSearch: bareMode || safeMode ? undefined : resolveWebSearchSettings(settings),
        ...
        modelFallbacks: resolveModelFallbacks(argv.fallbackModel, settings.modelFallbacks),
        // Use separated hooks if provided, otherwise fall back to merged hooks
        userHooks: bareMode || safeMode
            ? undefined
        ...
        stopHookBlockingCap: bareMode || safeMode ? undefined : settings.stopHookBlockingCap,
