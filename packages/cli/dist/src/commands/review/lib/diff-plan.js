...
import { unquoteCStylePath } from '@lailatul-coder/lailatul-coder-core';
const TEST_RE = /(^|\/)(__tests__|__snapshots__|__mocks__|tests?|spec|integration-tests|e2e)\/|\.(test|spec)\.[cm]?[jt]sx?$|_test\.(go|py|rb)$|(^|\/)test_[^/]+\.py$|(^|\/)src\/test\//;
const GENERATED_RE = /(^|\/)(package-lock\.json|yarn\.lock|pnpm-lock\.yaml|bun\.lock(b)?|Cargo\.lock|go\.sum|poetry\.lock|Gemfile\.lock|composer\.lock|NOTICES\.txt)$|\.snap$|\.min\.(js|css)$|(^|\/)(dist|build|vendor|node_modules)\//;
/**
...
const DOCS_RE = new RegExp(`(^|/)(docs|doc|documentation|website)/.*${DOCS_EXT}` + `|^[^/]+${DOCS_EXT}`);
/**
 ...
 */
export function chunkIdsProblem(ids) {
...
export function parseDiff(diffText) {
    ...
    const noteAdded = (f, line) => {
        ...
        if (last && last.end === line - 1)
            ...
            f.addedRanges.push({ start: line, end: line });
    };
...
function isSafeSplitPoint(lines, n) {
    ...
    if (!isNewSide(cur) || !isNewSide(prev))
        return false;
    if (cur === '')
        return false; // an empty line is not a declaration
...
function splitUnit(unit, lines, prefix, maxChunkLines, bodyStart) {
    ...
    const bigEnough = (s, e) => e - s + 1 >= MIN_SPLIT_SEGMENT ||
        charsIn(prefix, s, e) >= MAX_CHUNK_CHARS / 2;
    if (!over(unit.start, unit.end))
        return [unit];
    ...
    while (over(segStart, unit.end)) {
        ...
        for (let n = upper; n > bodyStart; n--) {
            if (!isSafeSplitPoint(lines, n))
                continue;
            if (over(segStart, n - 1))
                continue;
            if (!bigEnough(segStart, n - 1))
                continue;
        ...
        if (cut < 0)
            break; // genuinely no safe boundary left — keep the rest intact
 ...
 */
export function planChunks(files, lines, maxChunkLines = DEFAULT_MAX_CHUNK_LINES) {
    ...
    for (const f of files) {
        ...
        f.hunks.forEach((h, idx) => {
            ...
            units.push(...splitUnit(unit, lines, prefix, maxChunkLines, h.diffStart + 1));
        });
    }
    const chunks = [];
    let cur = null;
    const flush = () => {
        if (cur) {
            ...
            for (let n = cur.startLine; n <= cur.endLine; n++) {
                if (lines[n - 1].length > widest)
                    widest = lines[n - 1].length;
            }
    ...
    for (const u of units) {
        ...
        if (!cur) {
            cur = {
                ...
                oversized: uLines > maxChunkLines ||
...
export function buildDiffPlan(diffText, maxChunkLines = DEFAULT_MAX_CHUNK_LINES) {
    const { files, diffLines } = parseDiff(diffText);
    const lines = diffText.split('\n');
    if (lines.length > 0 && lines[lines.length - 1] === '')
        lines.pop();
    const linesOf = (kind) => files
    ...
    if (!chunksCoverDiff(chunks, diffLines)) {
        throw new Error(`diff-plan: chunks do not tile the diff (${chunks.length} chunks over ` +
 ...
 */
export function chunksCoverDiff(chunks, diffLines) {
    if (diffLines === 0)
        return chunks.length === 0;
    const sorted = [...chunks].sort((a, b) => a.startLine - b.startLine);
    let expected = 1;
    for (const c of sorted) {
        if (c.startLine !== expected)
            return false;
...
//# sourceMappingURL=diff-plan.js.map