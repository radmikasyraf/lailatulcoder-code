...
import { isBrowserCommandBlocked, shouldAttemptBrowserLaunch, } from './browser.js';
...
function validateUrl(url, { allowFile = false, allowedFilePaths } = {}) {
    ...
    if (!allowedProtocols.includes(parsedUrl.protocol)) {
        throw new Error(`Unsafe protocol: ${parsedUrl.protocol}. Only ${allowedProtocols.join(', ')} are allowed.`);
    }
 ...
 */
export async function openBrowserSecurely(url, browserOptions = {}) {
    ...
    const browserCommand = platformName === 'win32' ? undefined : buildBrowserCommand(browserEnv, url);
    if (browserCommand) {
        try {
            ...
            console.warn(`Failed to open BROWSER command ${browserCommand.command}: ${formatLaunchError(_error)}. Falling back to the platform browser opener.`);
            /* eslint-enable no-console */
        }
    }
    if (!shouldAttemptBrowserLaunch({ ignoreBrowserBlocklist: true })) {
        /* eslint-disable no-console */
        console.warn(`Browser launch is not available in this environment. Please open this URL manually: ${url}`);
        /* eslint-enable no-console */
        return;
    }
    {
