...
export function parseLastEventId(raw, logPrefix = '') {
    if (typeof raw !== 'string' || !/^\d+$/.test(raw)) {
        if (typeof raw === 'string' && raw.length > 0) {
            writeStderrLine(`LailatulCoder serve: ${logPrefix}rejected Last-Event-ID ${safeLogValue(raw)} ` +
...
/**
 * Parse an `X-LailatulCoder-Event-Epoch` request header into an epoch token for the
 ...
 * @param logPrefix distinguishes the surface in logs, e.g. `'/acp '` vs `''`.
 */
export function parseEventEpochHeader(raw, logPrefix = '') {
    if (typeof raw !== 'string' || raw.length === 0)
        return undefined;
    if (raw.length > 64 || !/^[\w-]+$/.test(raw)) {
        writeStderrLine(`LailatulCoder serve: ${logPrefix}rejected X-LailatulCoder-Event-Epoch ` +
            `${safeLogValue(raw)} (expected [A-Za-z0-9_-]{1,64})`);
        return undefined;
    }
    return raw;
}
//# sourceMappingURL=sse-last-event-id.js.map