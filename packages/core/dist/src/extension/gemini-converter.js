 ...
 */
/**
 * Converter for Gemini extensions to LailatulCoder Ai format.
 */
...
export function convertGeminiToQwenConfig(extensionDir) {
    ...
    if (!realPathWithin(configFilePath, extensionDir)) {
        throw new Error(`Gemini extension config at ${configFilePath} resolves through a symlink outside the extension`);
    }
    ...
    const settings = geminiConfig.settings;
 ...
 */
export function realPathWithin(target, root) {
    try {
 ...
 */
export async function copyDirectory(source, destination, confineRoot) {
        ...
        if (entry.isDirectory()) {
            ...
            try {
                ...
                if (!isPathWithin(realPath, root)) {
                    debugLogger.warn(`Skipping symlink that escapes the package: ${sourcePath} -> ${realPath}`);
                    continue;
                }
 ...
 */
async function convertCommandsDirectory(commandsDir) {
    ...
    for (const relativeFile of tomlFiles) {
        ...
        try {
            ...
            debugLogger.warn(`Warning: Failed to convert command file ${relativeFile}: ${error instanceof Error ? error.message : String(error)}`);
 ...
 */
export function isGeminiExtensionConfig(extensionDir) {
    const configFilePath = path.join(extensionDir, 'gemini-extension.json');
