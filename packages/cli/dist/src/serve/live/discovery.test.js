...
import { getLiveDiscoveryPath, handoffLiveDiscoveryOwner, LiveDiscoveryOwnerActiveError, LiveDiscoveryStateError, removeLiveDiscoveryFile, writeLiveDiscoveryFile, } from './discovery.js';
...
function record(instanceNonce, pid = process.pid) {
    return {
...
describe('Live discovery file', () => {
    it('publishes an atomic mode-0600 record below the runtime directory', async () => {
        ...
        expect(JSON.parse(await fs.readFile(writtenPath, 'utf8'))).toEqual(expected);
        expect((await fs.stat(writtenPath)).mode & 0o777).toBe(0o600);
        expect((await fs.readdir(path.dirname(writtenPath))).filter((name) => name.endsWith('.tmp'))).toEqual([]);
    });
    it('safely creates a missing nested runtime directory tree', async () => {
        ...
        await expect(writeLiveDiscoveryFile(runtime, expected)).resolves.toBe(getLiveDiscoveryPath(runtime));
        await expect(fs.readFile(getLiveDiscoveryPath(runtime), 'utf8')).resolves.toContain(expected.instanceNonce);
        if (process.platform !== 'win32') {
            expect((await fs.stat(path.dirname(getLiveDiscoveryPath(runtime)))).mode &
                0o777).toBe(0o700);
        }
    });
    it('rejects a symlinked runtime base without publishing through it', async () => {
        ...
        await expect(writeLiveDiscoveryFile(runtimeBaseDir, record('daemon_instance_nonce_symlink_01'))).rejects.toBeInstanceOf(LiveDiscoveryStateError);
    ...
    it('rejects an unsafe discovery lock shape without replacing it', async () => {
        ...
        await expect(writeLiveDiscoveryFile(runtime, record('daemon_instance_nonce_unsafe_lock_01'))).rejects.toBeInstanceOf(LiveDiscoveryStateError);
    ...
    it('only removes a record owned by the matching daemon pid and nonce', async () => {
        ...
        await expect(removeLiveDiscoveryFile(runtime, {
            pid: current.pid,
        ...
        await expect(fs.readFile(getLiveDiscoveryPath(runtime), 'utf8')).resolves.toContain(current.instanceNonce);
    ...
    it('allows only one concurrent healthy owner', async () => {
        ...
        const published = JSON.parse(await fs.readFile(getLiveDiscoveryPath(runtime), 'utf8'));
        expect([first.instanceNonce, second.instanceNonce]).toContain(published.instanceNonce);
    });
    it('does not publish over a stale foreign owner without a handoff', async () => {
        ...
        await expect(writeLiveDiscoveryFile(runtime, replacement, {
            isProcessAlive: (pid) => {
                expect(pid).toBe(previous.pid);
                return false;
            },
        })).rejects.toBeInstanceOf(LiveDiscoveryStateError);
        expect(JSON.parse(await fs.readFile(getLiveDiscoveryPath(runtime), 'utf8'))).toEqual(previous);
    });
    it('does not publish over a stale legacy-protocol owner without a handoff', async () => {
        ...
        expect(JSON.parse(await fs.readFile(discoveryPath, 'utf8'))).toEqual(previous);
        expect((await fs.stat(discoveryPath)).mode & 0o777).toBe(0o600);
    });
    it.each([
        ['current', LIVE_HOST_PROTOCOL_VERSION, 999_996],
        ['legacy', 2, 999_995],
    ])('reclaims a valid stale %s-protocol owner during handoff', async (_label, protocolVersion, pid) => {
        const runtime = await temporaryRuntime();
        const discoveryPath = getLiveDiscoveryPath(runtime);
        const previous = {
        ...
        await expect(handoffLiveDiscoveryOwner(runtime, replacement, async () => {
            committed = true;
        }, {
            wait,
            handoffGraceMs: 37,
    ...
    it('does not replace a live owner only because its protocol is legacy', async () => {
        ...
        await expect(writeLiveDiscoveryFile(runtime, record('daemon_instance_nonce_current_02'), {
            isProcessAlive: (pid) => {
                expect(pid).toBe(previous.pid);
                return true;
            },
    ...
    it('does not overwrite an invalid existing record', async () => {
        ...
        await expect(writeLiveDiscoveryFile(runtime, record('daemon_instance_nonce_0007'))).rejects.toThrow('Existing Live discovery record is invalid.');
        await expect(fs.readFile(discoveryPath, 'utf8')).resolves.toBe('{broken');
    });
    it('rejects non-loopback URLs and malformed nonces', async () => {
        const runtime = await temporaryRuntime();
        await expect(writeLiveDiscoveryFile(runtime, {
            ...record('daemon_instance_nonce_0005'),
            url: 'https://example.com',
        })).rejects.toThrow(/loopback/);
        await expect(writeLiveDiscoveryFile(runtime, record('short'))).rejects.toThrow(/invalid/);
    });
