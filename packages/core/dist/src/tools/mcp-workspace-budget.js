...
import { MCP_BUDGET_REARM_FRACTION, MCP_BUDGET_WARN_FRACTION, } from './mcp-client-manager.js';
...
export class WorkspaceMcpBudget {
    ...
    pendingRefusalTransports = new Map();
    /**
     ...
     */
    recordRefusal(serverName, transport) {
        if (this.mode !== 'enforce')
            return;
        this.pendingRefusalNames.add(serverName);
        this.pendingRefusalTransports.set(serverName, transport);
        debugLogger.warn(`MCP server '${serverName}' refused at pool acquire ` +
    ...
    endBulkPass() {
        if (this.bulkPassDepth === 0) {
            ...
            debugLogger.warn('WorkspaceMcpBudget.endBulkPass called without matching beginBulkPass; ignoring');
            return;
        }
        this.bulkPassDepth -= 1;
        if (this.bulkPassDepth > 0)
    ...
    flushRefusedBatch() {
        ...
        if (this.onEvent) {
            try {
                ...
                debugLogger.error(`WorkspaceMcpBudget refused_batch event delivery failed: ${err instanceof Error ? err.message : String(err)}`);
            }
        }
    }
    /**
     ...
     */
    evaluateState() {
        if (this.mode === 'off' || this.clientBudget === undefined)
            return;
        ...
        if (this.warnArmed && ratio >= MCP_BUDGET_WARN_FRACTION) {
            ...
            debugLogger.info(`MCP workspace budget warning fired (ratio=${ratio.toFixed(2)}, ` +
                `reservedCount=${this.reservedSlots.size}, ` +
            ...
            if (this.onEvent) {
                try {
                    ...
                    debugLogger.error(`WorkspaceMcpBudget budget_warning event delivery failed: ${err instanceof Error ? err.message : String(err)}`);
                }
            }
        }
        else if (!this.warnArmed && ratio < MCP_BUDGET_REARM_FRACTION) {
            this.warnArmed = true;
            debugLogger.info(`MCP workspace budget warning re-armed (ratio=${ratio.toFixed(2)}, ` +
                `budget=${this.clientBudget}; next 75% crossing will fire)`);
        }
    }
}
//# sourceMappingURL=mcp-workspace-budget.js.map