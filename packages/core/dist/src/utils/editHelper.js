...
function normalizeBasicCharacters(text) {
    if (text === '') {
        return text;
    }
    let normalized = '';
 ...
 */
function seekSequenceWithTransform(lines, pattern, transform) {
    if (pattern.length === 0) {
        return 0;
    }
 ...
 */
function sliceFromLines(text, offsets, lines, startLine, lineCount, includeTrailingNewline) {
    if (lineCount === 0) {
...
function findLineBasedMatch(haystack, needle) {
    ...
    if (matchIndex !== null) {
        return {
            slice: sliceFromLines(haystack, offsets, lines, matchIndex, patternLines.length, endsWithNewline),
...
/* Slice discovery                                                           */
/* -------------------------------------------------------------------------- */
function findMatchedSlice(haystack, needle) {
    if (needle === '') {
        return null;
    }
...
function adjustNewStringForTrailingLine(newString, removedTrailingLine) {
 ...
 */
export function normalizeEditStrings(fileContent, oldString, newString) {
    ...
    if (canonicalOriginal !== null) {
        return {
            ...
            newString: adjustNewStringForTrailingLine(newString, canonicalOriginal.removedTrailingFinalEmptyLine),
        };
    }
    return {
        oldString,
 ...
 */
export function maybeAugmentOldStringForDeletion(fileContent, oldString, newString) {
    if (fileContent === null ||
        oldString === '' ||
        newString !== '' ||
        oldString.endsWith('\n')) {
 ...
 */
export function extractEditSnippet(oldContent, newContent) {
