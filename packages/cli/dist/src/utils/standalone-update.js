...
const OSS_BASE = 'https://lailatul-coder-assets.oss-cn-hangzhou.aliyuncs.com/releases/lailatul-coder';
...
async function tryFetch(url, timeoutMs = FETCH_TIMEOUT_MS) {
    try {
...
async function downloadWithFallback(versionPath, filename, timeoutMs = FETCH_TIMEOUT_MS) {
    const ossUrl = `${OSS_BASE}/${versionPath}/${filename}`;
    const ossResult = await tryFetch(ossUrl, timeoutMs);
    if (ossResult.response)
    ...
    throw new Error(`Failed to download ${filename}: OSS (${ossResult.error?.message ?? 'unknown'}), GitHub (${ghResult.error?.message ?? 'unknown'})`);
}
async function verifyChecksum(actualHash, filename, versionPath) {
    ...
    if (sigResponse) {
        ...
        throw new Error('SHA256SUMS.sig not found and QWEN_REQUIRE_SIGNATURE=1 is set');
    }
    else {
        debugLogger.warn('SHA256SUMS.sig not available — update integrity relies on SHA256 checksum only. ' +
...
async function downloadToFile(versionPath, filename, destPath) {
    const response = await downloadWithFallback(versionPath, filename, ARCHIVE_TIMEOUT_MS);
...
export function isSafeTarLinkTarget(entryPath, linkPath, resolvedDest) {
    ...
    const linkTarget = path.resolve(resolvedDest, path.posix.dirname(normalizedEntryPath), linkPath);
...
export function isSafeTarEntry(entryPath, entry, resolvedDest) {
    if (!isSafeTarEntryPath(entryPath))
        return false;
...
function validateExtractedPaths(resolvedDest, options = {}) {
    ...
    for (const entry of entries) {
        ...
        const fullPath = path.join(String(entry.parentPath || entry.path), entry.name);
        let resolved;
        try {
            ...
            throw new Error(`Invalid archive entry: ${entry.name} could not be resolved (${detail})`);
        }
        if (!isPathInside(resolvedDest, resolved)) {
            fs.rmSync(resolvedDest, { recursive: true, force: true });
            throw new Error(`Path traversal detected in archive: ${entry.name} resolves to ${resolved}`);
        }
    }
}
export function isSafeTarEntryPath(entryPath) {
...
function spawnAndCapture(command, args, timeoutMs) {
    return new Promise((resolve, reject) => {
        let settled = false;
        const child = execFile(command, args, { timeout: timeoutMs }, (err, out, stderr) => {
            if (settled)
                return;
            settled = true;
            if (err &&
 ...
 */
async function smokeTest(newInstallDir, target) {
    ...
    const { exitCode, stdout, stderr } = await spawnAndCapture(nodeBin, [cliBin, '--version'], 10_000);
    if (exitCode !== 0) {
        const detail = stderr.trim() ? `: ${stderr.trim()}` : '';
        throw new Error(`Smoke test failed: new binary exited with code ${exitCode}${detail}`);
    }
    const version = stdout.trim();
    if (!SEMVER_RE.test(version)) {
        throw new Error(`Smoke test failed: unexpected version output "${version}"`);
    }
...
function checkDeferredSwap(standaloneDir) {
    const deferredMarker = `${standaloneDir}.deferred`;
    if (fs.existsSync(deferredMarker)) {
        try {
            const batPid = parseInt(fs.readFileSync(deferredMarker, 'utf-8').trim(), 10);
            if (!Number.isNaN(batPid) && isProcessAlive(batPid)) {
                throw new Error('A previous update is still being applied. Please wait a moment and try again.');
            }
        }
        catch (readErr) {
    ...
    if (fs.existsSync(`${standaloneDir}.new`)) {
        throw new Error(`A previous update left a pending swap at ${standaloneDir}.new. ` +
            'If no LailatulCoder-update.bat process is running, remove the pending swap and .lailatulcoder-update.lock, then try again.');
    }
}
export function acquireLock(lockPath, standaloneDir) {
    try {
        ...
        if (fastPathErr instanceof Error &&
...
function cleanupEmptyStandaloneDir(standaloneDir) {
...
function assertSafeForSingleQuotedShellPath(p, context) {
    if (UNSAFE_SHELL_CHARS.test(p)) {
        throw new Error(`${context} contains characters unsafe for shell embedding: ${p}`);
    }
}
function shellQuoteForSh(p) {
    return `'${p.replace(/'/g, "'\\''")}'`;
}
function shellQuoteForFish(p) {
...
function atomicReplace(standaloneDir, newDir, lockPath) {
    ...
    if (os.platform() === 'win32') {
        ...
        if (UNSAFE_CMD_CHARS.test(standaloneDir) ||
            ...
            throw new Error('Installation path contains characters unsafe for deferred update script');
        }
        ...
        const deferredMarker = `${standaloneDir}.deferred`;
        const logFile = path.join(path.dirname(standaloneDir), 'LailatulCoder-update.log');
        ...
        // 3. Log success/failure to LailatulCoder-update.log for post-mortem (the bat
        //    runs with stdio:ignore — the log is the only diagnostic surface).
        // 4. Delete the .deferred marker so future `LailatulCoder update` calls know the
        ...
        const scriptPath = path.join(path.dirname(standaloneDir), 'LailatulCoder-update.bat');
        ...
        // Write .deferred marker with the bat script PID so future `LailatulCoder update`
        ...
        try {
            ...
            try {
                ...
                const detail = `Standalone update failed AND rollback failed.\n` +
                    `Original error: ${promoteErr.message}\n` +
...
/**
 * Ensures ~/.local/bin/LailatulCoder exists and points to the standalone install.
 * Required for npm→standalone migration so the new binary is on PATH.
 */
export function ensureBinWrapper(standaloneDir, target) {
    const binDir = path.join(path.dirname(standaloneDir), '..', 'bin');
    const artifacts = { wrapperCreated: false };
    try {
        fs.mkdirSync(binDir, { recursive: true });
        if (target.startsWith('win')) {
            if (UNSAFE_CMD_CHARS.test(standaloneDir)) {
                throw new Error('standaloneDir contains characters unsafe for cmd.exe wrapper');
            }
            const wrapperPath = path.join(binDir, 'LailatulCoder.cmd');
            artifacts.wrapperPath = wrapperPath;
            const content = `@echo off\r\ncall "${standaloneDir}\\bin\\LailatulCoder.cmd" %*\r\n`;
            try {
                ...
                artifacts.wrapperNeedsAttention = !existingWrapperMatches(wrapperPath, standaloneDir);
            }
        }
        else {
            assertSafeForSingleQuotedShellPath(standaloneDir, 'standaloneDir');
            const wrapperPath = path.join(binDir, 'LailatulCoder');
            artifacts.wrapperPath = wrapperPath;
            // Match install-LailatulCoder-standalone.sh's write_unix_wrapper:
            // uses /usr/bin/env sh for portability, and single-quoted paths.
            const quotedQwenBin = shellQuoteForSh(path.join(standaloneDir, 'bin', 'LailatulCoder'));
...
function existingWrapperMatches(wrapperPath, standaloneDir) {
    try {
 ...
 * Mirrors the logic in install-LailatulCoder-standalone.sh maybe_update_shell_path.
 */
export function ensurePathInShellRc(binDir) {
    ...
    if (shell.endsWith('/zsh')) {
        ...
        // Match install-LailatulCoder-standalone.sh's maybe_update_shell_path logic:
    ...
    if (!rcFile)
        return { blockAdded: false };
    try {
        const content = fs.existsSync(rcFile)
            ? fs.readFileSync(rcFile, 'utf-8')
            : '';
        // Use begin/end block markers matching install-LailatulCoder-standalone.sh's
...
function surfaceBinWrapperWarnings(artifacts) {
    if (!artifacts)
        return;
    if (artifacts.wrapperNeedsAttention && artifacts.wrapperPath) {
        const message = `Update completed, but the existing wrapper at ${artifacts.wrapperPath} ` +
    ...
    if (artifacts.shellPathUpdate?.error && artifacts.shellPathUpdate.rcFile) {
        const message = `Update completed, but ${artifacts.shellPathUpdate.rcFile} could not be updated. ` +
            'Add ~/.local/bin to PATH manually.';
        debugLogger.warn(message);
        updateEventEmitter.emit('update-info', { message });
    }
}
function cleanupShellPathBlock(rcFile) {
    try {
...
export function cleanupFirstTimeMigrationArtifacts(artifacts) {
    if (!artifacts)
        return;
    ...
    if (artifacts.shellPathUpdate?.blockAdded &&
...
export async function performStandaloneUpdate(standaloneDir, newVersion) {
    ...
    if (fs.existsSync(manifestPath)) {
        ...
        // Directory exists but has no manifest — not a managed LailatulCoder install.
        // Refuse to overwrite to avoid data loss.
        throw new Error(`${standaloneDir} exists but is not a LailatulCoder Ai standalone install. Remove it manually to proceed.`);
    }
    else {
    ...
    // misleading error.
    const lockPath = path.join(parentDir, '.lailatulcoder-update.lock');
    ...
    try {
        ...
        const archiveHash = await downloadToFile(versionPath, filename, archivePath);
        ...
        if (!fs.existsSync(path.join(newInstallDir, 'manifest.json'))) {
            throw new Error('Extracted archive does not contain expected lailatul-coder directory');
        }
        ...
        if (fs.existsSync(oldDir)) {
            if (isFirstTimeMigration) {
                ...
                try {
                    ...
                    fs.writeFileSync(path.join(oldDir, '.lailatulcoder-rollback-info.json'), JSON.stringify(rollbackInfo, null, 2));
                }
                catch {
 ...
 */
export function rollbackStandaloneUpdate(standaloneDir) {
    const lockPath = path.join(path.dirname(standaloneDir), '.lailatulcoder-update.lock');
    try {
        ...
        if (!Number.isNaN(pid) && isProcessAlive(pid)) {
            return {
                ...
                detail: 'An auto-update is currently in progress. Wait for it to finish before rolling back.',
            };
        }
    }
    catch {
    ...
    try {
        ...
        if (!fs.existsSync(standaloneDir) && fs.existsSync(failedDir)) {
            try {
                ...
                return {
                    ok: false,
                    reason: 'rename-failed',
                    detail: `Filesystem error: ${err.message}. Current installation was restored automatically.`,
                };
            }
            catch {
        ...
        return {
            ok: false,
            reason: 'rename-failed',
            detail: `Filesystem error: ${err.message}. Manual recovery: mv "${oldDir}" "${standaloneDir}"`,
        };
    }
}
//# sourceMappingURL=standalone-update.js.map