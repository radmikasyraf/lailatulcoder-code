...
const { ghMock, ghApiAllMock, currentUserMock, ensureAuthenticatedMock, setGhHostMock, writeFileSyncMock, rmSyncMock, mkdirSyncMock, } = vi.hoisted(() => ({
    ghMock: vi.fn(),
    ghApiAllMock: vi.fn(),
...
import { prContextCommand, anyRootCarriesCriticalMarker, isLegacySuggestionSummary, isReviewWorthShowing, SUMMARY_MARKER, truncatedHeadings, buildMarkdown, carriesBlockerSignal, extractCodeRefs, classifyInlineThreads, fullBody, fullCommentBody, latestLedger, recoverLedger, renderLedgerSection, FOREIGN_ROUND_HEADROOM, } from './pr-context.js';
import { serializeLedger, LEDGER_MAX_FINDINGS, } from './lib/ledger.js';
...
describe('isLegacySuggestionSummary', () => {
    ...
    it('does not match an ordinary comment', () => {
        ...
        expect(isLegacySuggestionSummary('mentions qwen-review-suggestion-summary')).toBe(false);
    });
...
describe('buildMarkdown section order', () => {
    ...
    it('feeds buildMarkdown\u2019s ledger section the RUNNING identity', () => {
        ...
        try {
            expect(buildMarkdown('1', 'o/r', meta, [], [], [], ledger)).toContain('the same-model contract HOLDS');
            process.env['QWEN_CODE_MODEL_IDENTITY'] = 'other-model@9f8e7d6c';
            expect(buildMarkdown('1', 'o/r', meta, [], [], [], ledger)).toContain('Do NOT pass the reviewed-at sha');
        }
        finally {
            if (prev === undefined)
                delete process.env['QWEN_CODE_MODEL_IDENTITY'];
            else
                process.env['QWEN_CODE_MODEL_IDENTITY'] = prev;
        }
    });
...
describe('isReviewWorthShowing', () => {
    ...
    const VERSIONED_FOOTER = '_— qwen3.8-max-preview via LailatulCoder Ai /review (v0.21.2)_';
    it('filters the exact canonical LGTM template, with or without either footer', () => {
        ...
        expect(isReviewWorthShowing(`No issues found. LGTM! ✅\n\n${LEGACY_FOOTER}`)).toBe(false);
        expect(isReviewWorthShowing(`No issues found. LGTM! ✅\n\n${VERSIONED_FOOTER}`)).toBe(false);
    ...
    it('shows a body that OPENS with the template but carries more (a relocated blocker once hid behind a prefix match)', () => {
        expect(isReviewWorthShowing('No issues found. LGTM! ✅\n\n**[Critical]** relocated blocker: the cache is never invalidated')).toBe(true);
    });
    it('shows ordinary review bodies', () => {
        expect(isReviewWorthShowing('Downgraded from Approve: self-PR.')).toBe(true);
    });
});
describe('buildMarkdown — review bodies and replied Criticals', () => {
    ...
    it('renders review bodies in full, not 240-char snippets (a body-only blocker lives only here)', () => {
        ...
        const md = buildMarkdown('1', 'o/r', meta, [], [], [
            {
                id: 7,
                user: { login: 'rev' },
...
describe('buildMarkdown — truncation refs are copy-runnable with real coordinates', () => {
    ...
    it('a cut open-root snippet and a cut issue comment name their exact fetch (no {owner}/{n} placeholders)', () => {
        ...
        const md = buildMarkdown('6711', 'LailatulCoder/lailatul-coder', meta, inline, issue, []);
        ...
        expect(md).toContain('"${QWEN_CODE_CLI:-qwen}" review comment-body 21 --kind inline --repo LailatulCoder/lailatul-coder');
        expect(md).toContain('comment-body 31 --kind issue --repo LailatulCoder/lailatul-coder');
        expect(md).not.toContain('{owner}');
    });
    it('a capped review body names the filled-in review fetch', () => {
        const md = buildMarkdown('6711', 'LailatulCoder/lailatul-coder', meta, [], [], [
            {
                id: 7,
                user: { login: 'rev' },
                state: 'CHANGES_REQUESTED',
        ...
        expect(md).toContain('comment-body 7 --kind review --pr 6711 --repo LailatulCoder/lailatul-coder');
    });
...
describe('buildMarkdown — a markerless maintainer blocker must not render as an endorsement (PR #6486 regression)', () => {
    const realBody = readFileSync(join(dirname(fileURLToPath(import.meta.url)), '__fixtures__', 'pr-6486-comment-4942713150.md'), 'utf8');
    const meta = {
    ...
    const render = () => buildMarkdown('6486', 'LailatulCoder/lailatul-coder', meta, [], [
    ...
    it('does not file it under "do NOT re-report"', () => {
        ...
        expect(alreadyDiscussed === -1 || blocker < alreadyDiscussed, 'the blocker is rendered inside "Already discussed — do NOT re-report"').toBe(true);
    });
    ...
    it('does not promote the triage bot saying there are NO blockers', () => {
        ...
        const md = buildMarkdown('6486', 'LailatulCoder/lailatul-coder', meta, [], [
...
describe('extractCodeRefs', () => {
    it('pulls the locations a blocker points at, with line numbers', () => {
        expect(extractCodeRefs("`text-buffer.ts:2663` still binds `Ctrl+F → move('right')`, and the " +
            'handler in `AppContainer.tsx` is an independent subscriber.')).toEqual(['text-buffer.ts:2663', 'AppContainer.tsx']);
    });
    it('keeps full paths and line ranges', () => {
        expect(extractCodeRefs('see packages/cli/src/ui/x.ts:10-20 and lib/y.go:3')).toEqual(['packages/cli/src/ui/x.ts:10-20', 'lib/y.go:3']);
    });
    ...
    it('collapses a bare filename into the full path naming the same location', () => {
        ...
        expect(extractCodeRefs('`text-buffer.ts:2663` still binds it; remove it at ' +
            '`packages/cli/src/ui/components/shared/text-buffer.ts:2663`.')).toEqual(['packages/cli/src/ui/components/shared/text-buffer.ts:2663']);
    ...
    it('drops paths that escape the worktree — the read list is a trusted directive', () => {
        ...
        expect(extractCodeRefs('read `../../../../etc/passwd.sh` and `src/ok.ts:5`')).toEqual(['src/ok.ts:5']);
...
describe('carriesBlockerSignal', () => {
    it('recognises a blocker that never uses the [Critical] marker', () => {
        ...
        expect(carriesBlockerSignal('### 🔴 Finding 1 — Ctrl+F dual-fires: it toggles the model **and** moves the cursor (blocker)')).toBe(true);
        expect(carriesBlockerSignal('This is still reproducible at HEAD.')).toBe(true);
        expect(carriesBlockerSignal('Must fix before merge: auth bypass.')).toBe(true);
        expect(carriesBlockerSignal('这个问题是阻塞项，合并前必须修复。')).toBe(true);
    });
    ...
    it('reads only rendered text — an HTML comment carries no signal', () => {
        ...
        expect(carriesBlockerSignal('<!-- x --> this is still reproducible')).toBe(true);
    });
    it('is not fooled by a signal sitting inside its own negation', () => {
        ...
        expect(carriesBlockerSignal('No critical blockers in the parser. The cache path, though, is a blocker.')).toBe(true);
    });
    it('recognises the words people actually write, not the nouns we imagined', () => {
        ...
        expect(carriesBlockerSignal('## E2E verification — 86/90 checks pass, 1 blocking gap')).toBe(true);
        expect(carriesBlockerSignal('### 🔴 Blocking: a committed policy never lands')).toBe(true);
        expect(carriesBlockerSignal('### 🔴 阻塞问题：策略没有到达运行中的 agent')).toBe(true);
    });
    ...
    it('does not promote a severity emoji on a list of repairs', () => {
        ...
        expect(carriesBlockerSignal('### 🔴 Critical fixes\nAddressed all 3.')).toBe(false);
    });
    it('resets a negation at an adversative, but not at a bare comma', () => {
        ...
        expect(carriesBlockerSignal('No other concerns, but auth is a blocker')).toBe(true);
        expect(carriesBlockerSignal('没有其他问题，但这是阻塞问题')).toBe(true);
        // Coordinated negated list — the `No` distributes across the commas.
        expect(carriesBlockerSignal('No blocking, must-fix, or critical issues.')).toBe(false);
    ...
    it('resets a negation at a space-surrounded hyphen, not at must-fix', () => {
        ...
        expect(carriesBlockerSignal('No blockers - auth is still broken and is a blocker')).toBe(true);
        expect(carriesBlockerSignal('No issues -- the cache is a blocker')).toBe(true);
    ...
    it('breaks the negation window at a semicolon or colon (new clause)', () => {
        ...
        expect(carriesBlockerSignal('No blockers; the cache path is a blocker')).toBe(true);
        expect(carriesBlockerSignal('No blockers: the cache path is a blocker')).toBe(true);
    ...
    it('does not promote ordinary chatter', () => {
        ...
        expect(carriesBlockerSignal('Addressed all 3 findings, thanks!')).toBe(false);
        expect(carriesBlockerSignal('**[Suggestion]** rename this helper')).toBe(false);
...
describe('blockerSection — both channels, and the budget', () => {
    ...
    it('degrades a body past the budget to a snippet that names its fetch', () => {
        ...
        const md = buildMarkdown('6486', 'LailatulCoder/lailatul-coder', meta, [], [big(1), big(2), big(3)], []);
    ...
    it('charges its own headings and reference lists against the budget', () => {
        ...
        const md = buildMarkdown('1', 'o/r', meta, [], [withRefs(1), withRefs(2), withRefs(3), withRefs(4)], []);
        const section = md.slice(md.indexOf('## Blockers to re-check'), md.indexOf('## Description'));
...
describe('classifyInlineThreads', () => {
    ...
    it('promotes an attribution-off marker body through buildMarkdown only with the reviewing identity', () => {
        ...
        const withIdentity = buildMarkdown('1', 'o/r', meta, [markerComment], [], [], null, 'review-bot');
        const section = withIdentity.indexOf('## Blockers to re-check');
        expect(section).toBeGreaterThanOrEqual(0);
        expect(withIdentity.indexOf('the guard checks the wrong variable')).toBeGreaterThan(section);
        const withoutIdentity = buildMarkdown('1', 'o/r', meta, [markerComment], [], [], null, '');
...
describe('anyRootCarriesCriticalMarker', () => {
    it('fires only on a critical marker carried by a ROOT comment', () => {
        expect(anyRootCarriesCriticalMarker([
            { body: 'x\n\n<!-- qwen-review critical -->' },
        ])).toBe(true);
...
describe('prContextCommand builder', () => {
    it('registers --host so Enterprise routing is a flag, not a prose instruction', () => {
        ...
        prContextCommand.builder(stub);
        expect(opts).toContain('host');
    });
});
describe('latestLedger — the split trust surface', () => {
    const marker = (round) => `LGTM <!-- qwen-review-ledger {"v":1,"round":${round},"findings":[{"id":"R${round}-1","sev":"C","file":"a.ts","title":"t"}]} -->`;
    const review = (login, at, body) => ({
        id: 1,
        user: { login },
        submitted_at: at,
        body,
    ...
    it('reports an own-account ledger as not foreign', () => {
        const found = latestLedger([review('bot', '2026-01-03T00:00:00Z', marker(3))], 'bot');
    ...
    it('drops the anchor from ANOTHER account, keeping the work list', () => {
        ...
        const foreign = latestLedger([review('ci-bot', '2026-01-01T00:00:00Z', serializeLedger(anchored))], 'maintainer');
    ...
    it('carries the anchor through intact for the OWN account', () => {
        ...
        const own = latestLedger([
            review('bot', '2026-01-01T00:00:00Z', `LGTM ${serializeLedger(anchored)}`),
        ], 'bot');
    ...
    it('distinguishes "no own review" from "own review without a parseable ledger"', () => {
        ...
        const damaged = recoverLedger([review('bot', '2026-01-01T00:00:00Z', 'edited body, marker gone')], 'bot');
        ...
        // not leftovers.
        const foreignOnly = recoverLedger([review('stranger', '2026-01-01T00:00:00Z', marker(3))], 'bot');
        ...
        const cased = recoverLedger([review('Bot', '2026-01-01T00:00:00Z', marker(2))], 'bot');
    ...
    it('treats an unknown login as foreign — an anchor needs a proven owner', () => {
        const found = latestLedger([review('bot', '2026-01-01T00:00:00Z', serializeLedger(anchored))], null);
        expect(found?.foreign).toBe(true);
    ...
    it("drops a finding that squats a FUTURE round's id prefix", () => {
        ...
        const squatting = 'LGTM <!-- qwen-review-ledger {"v":1,"round":3,"findings":[' +
            ...
            ']} -->';
        const found = latestLedger([review('stranger', '2026-01-09T00:00:00Z', squatting)], 'bot');
    ...
    it("drops the volume telemetry from another account's marker", () => {
        ...
        const foreign = 'x <!-- qwen-review-ledger {"v":1,"round":9,"findings":[' +
            '{"id":"R9-1","sev":"S","file":"src/auth.ts","title":"planted"}' +
            '],"posted":1,"prevPosted":1,"floor":"c"} -->';
        const found = latestLedger([review('stranger', '2026-01-09T00:00:00Z', foreign)], 'bot');
        expect(found?.foreign).toBe(true);
    ...
    it('keeps the volume when the identity lookup is what failed', () => {
        ...
        const own = 'x <!-- qwen-review-ledger {"v":1,"round":9,"findings":[],' +
            '"posted":4,"prevPosted":2,"fresh":3,"floor":"c",' +
            '"sha":"deadbeef00112233"} -->';
        const anonymous = latestLedger([review('maintainer', '2026-01-09T00:00:00Z', own)], null);
    ...
    it("restores this account's own volume when it restores its own findings", () => {
        ...
        const own = 'x <!-- qwen-review-ledger {"v":1,"round":8,"findings":[' +
        ...
        const foreign = 'y <!-- qwen-review-ledger {"v":1,"round":8,"findings":[' +
    ...
    it('restores an own TRUE-ZERO volume even with nothing to merge', () => {
        ...
        // trend for that round with a good count in hand.
        const own = 'LGTM <!-- qwen-review-ledger {"v":1,"round":8,"findings":[],' +
    ...
    it('will not pair own counts with a round the own marker does not describe', () => {
        ...
        const own = 'LGTM <!-- qwen-review-ledger {"v":1,"round":7,"findings":[],' +
            '"posted":0,"fresh":0,"floor":"o"} -->';
        const foreign = 'y <!-- qwen-review-ledger {"v":1,"round":8,"findings":[' +
            '{"id":"R8-1","sev":"S","file":"b.ts","title":"theirs"}' +
            '],"posted":99,"fresh":97,"floor":"c"} -->';
    ...
    it('merges a foreign winner OVER the own findings — displacement is dead', () => {
        ...
        const own = 'LGTM <!-- qwen-review-ledger {"v":1,"round":7,"findings":[' +
            ...
            ']} -->';
        const emptyForeign = 'x <!-- qwen-review-ledger {"v":1,"round":8,"findings":[]} -->';
        ...
        const doctored = 'x <!-- qwen-review-ledger {"v":1,"round":8,"findings":[' +
        ...
        // authoritative.
        const tampered = 'x <!-- qwen-review-ledger {"v":1,"round":8,"findings":[' +
        ...
        const emptyOwn = 'LGTM <!-- qwen-review-ledger {"v":1,"round":7,"findings":[]} -->';
    ...
    it('the merge cap trims FOREIGN entries first — own-first is load-bearing', () => {
        ...
        const ownFindings = Array.from({ length: LEDGER_MAX_FINDINGS }, (_, i) => `{"id":"R7-${i + 1}","sev":"S","file":"a.ts","title":"own ${i + 1}"}`).join(',');
        const foreignFindings = Array.from({ length: 5 }, (_, i) => `{"id":"R8-${i + 1}","sev":"S","file":"b.ts","title":"theirs ${i + 1}"}`).join(',');
        ...
        const atCap = recoverLedger([
            review('maintainer', '2026-01-01T00:00:00Z', `x <!-- qwen-review-ledger {"v":1,"round":7,"findings":[${ownFindings}],"dropped":3} -->`),
            review('stranger', '2026-01-09T00:00:00Z', `x <!-- qwen-review-ledger {"v":1,"round":8,"findings":[${foreignFindings}],"dropped":2} -->`),
        ], 'maintainer').recovered;
        // Every own id survives the cap…
        const ids = atCap?.ledger.findings.map((f) => f.id) ?? [];
        expect(ids.filter((id) => id.startsWith('R7-'))).toHaveLength(LEDGER_MAX_FINDINGS);
    ...
    it('does not adopt a foreign round implausibly far past our own', () => {
        ...
        const atBound = latestLedger([
            review('maintainer', '2026-01-05T00:00:00Z', marker(8)),
            review('ci-bot', '2026-01-09T00:00:00Z', marker(8 + FOREIGN_ROUND_HEADROOM)),
        ], 'maintainer');
        expect(atBound?.ledger.round).toBe(8 + FOREIGN_ROUND_HEADROOM);
        // …and first refused:
        const pastBound = latestLedger([
            review('maintainer', '2026-01-05T00:00:00Z', marker(8)),
            review('ci-bot', '2026-01-09T00:00:00Z', marker(8 + FOREIGN_ROUND_HEADROOM + 1)),
        ], 'maintainer');
    ...
    it('bounds foreign rounds from zero when this account never posted', () => {
        ...
        const atBound = latestLedger([
            review('ci-bot', '2026-01-02T00:00:00Z', marker(FOREIGN_ROUND_HEADROOM)),
        ], 'maintainer');
        expect(atBound?.ledger.round).toBe(FOREIGN_ROUND_HEADROOM);
        expect(latestLedger([
            review('ci-bot', '2026-01-02T00:00:00Z', marker(FOREIGN_ROUND_HEADROOM + 1)),
        ], 'maintainer')).toBeNull();
    });
    ...
    it('refuses an out-of-range round from any account', () => {
        ...
        expect(latestLedger([review('stranger', '2026-01-09T00:00:00Z', huge)], 'bot')).toBeNull();
    ...
    it('yields nothing with no marker, or a malformed one', () => {
        expect(latestLedger([review('bot', '2026-01-01', 'plain body')], 'bot')).toBeNull();
        expect(latestLedger([review('bot', '2026-01-01', '<!-- qwen-review-ledger nope -->')], 'bot')).toBeNull();
    });
});
describe('renderLedgerSection', () => {
    /** Live cell separators, counted the way markdown reads them. */
    const liveSeparators = (row) => {
        let n = 0;
    ...
    it('a MERGED list is mixed provenance — never all THEIR claims', () => {
        ...
        const mergedSection = renderLedgerSection({
            ...
            dropped: 2,
        }, 'm', 'lailatul-coder-ci-bot', true);
        expect(mergedSection).toContain("MERGED over this account's own latest findings");
        expect(mergedSection).toContain('entries this account certified are its own claims');
    ...
    it('says so when the ledger is PARTIAL, and stays silent when it is not', () => {
        ...
        expect(renderLedgerSection({
            v: 1,
            round: 3,
            findings: [{ id: 'R3-1', sev: 'C', file: 'a.ts', title: 't' }],
        }, 'm')).not.toContain('PARTIAL');
    });
    it('names the reviewed-at sha when the ledger carries one, and stays silent when not', () => {
        ...
        expect(anchored).toContain('pass it as `--since <sha> --since-model <model>`');
        ...
        expect(anchored).toContain("when Step 1's recovered-anchor check rules a re-run admissible");
    ...
    it('refuses when the side file holds a DIFFERENT anchor than the one recovered', () => {
        ...
        const diverged = renderLedgerSection(recovered, 'model-a@aaaaaaaa', null, false, 'ffff1111ffff1111');
        ...
        expect(renderLedgerSection(recovered, 'model-a@aaaaaaaa', null, false, 'aaaa2222aaaa2222')).toContain('the same-model contract HOLDS');
        // …and so does a side file that holds no anchor to disagree with.
        expect(renderLedgerSection(recovered, 'model-a@aaaaaaaa', null, false, null)).toContain('the same-model contract HOLDS');
    });
    it('RULES the same-model gate here instead of asking the model to compare', () => {
        ...
        const otherProvider = renderLedgerSection(ledger('m@9f8e7d6c'), 'm@1a2b3c4d');
        ...
        expect(renderLedgerSection(ledger('m'), 'm@1a2b3c4d')).toContain('Do NOT pass');
        expect(renderLedgerSection(ledger('m@1a2b3c4d'), 'm')).toContain('Do NOT pass');
...
describe('ledger marker vs the canonical-LGTM filter', () => {
    it('a marker-carrying canonical LGTM is still filtered out', () => {
        ...
        const marker = '<!-- qwen-review-ledger {"v":1,"round":2,"findings":[]} -->';
        const md = buildMarkdown('1', 'o/r', { title: 't', body: '', state: 'OPEN' }, [], [], [
            {
                id: 1,
                user: { login: 'bot' },
...
describe('prContextCommand handler — identity fail-closed', () => {
    ...
    const run = (out) => Promise.resolve(prContextCommand.handler({
        pr_number: '42',
        owner_repo: 'o/r',
        out,
    }));
    const withMarkerPosted = () => {
        ghApiAllMock.mockImplementation((path) => path.includes('/pulls/') && path.endsWith('/comments')
            ? [MARKER_COMMENT]
            : []);
    };
    it('refuses the context when identity fails while a severity marker is posted', async () => {
        ...
        await expect(run(join(outDir, 'context.md'))).rejects.toThrow(/cannot determine the reviewing account/);
    });
    ...
    it('proceeds best-effort when the login is empty and nothing needs it', async () => {
        ...
        await run(out);
        const written = writeFileSyncMock.mock.calls[0]?.[1];
    ...
    it('promotes the marker comment into the re-check section when identity resolves', async () => {
        ...
        const section = md.indexOf('## Blockers to re-check');
        expect(section).toBeGreaterThanOrEqual(0);
        expect(md.indexOf('the guard checks the wrong variable')).toBeGreaterThan(section);
    });
});
describe('buildMarkdown host baking', () => {
    ...
    it('bakes --host into the emitted refetch command when a host is set', () => {
        const md = buildMarkdown('6711', 'o/r', meta, [], [], [longReview], null, undefined, null, false, 'ghe.example.com');
        expect(md).toContain('comment-body 7 --kind review --pr 6711 --repo o/r --host ghe.example.com');
    });
    it('keeps the author and host slots apart — both are strings, tsc cannot', () => {
        ...
        const md = buildMarkdown('6711', 'o/r', meta, [], [], [longReview], ledger, '', 'lailatul-coder-ci-bot', false, 'ghe.example.com');
    ...
    it('bakes --host for inline and issue kinds too, not just reviews', () => {
        ...
        const md = buildMarkdown('6711', 'o/r', meta, inline, issue, [], null, undefined, null, false, 'ghe.example.com');
        expect(md).toContain('comment-body 21 --kind inline --repo o/r --host ghe.example.com');
        expect(md).toContain('comment-body 31 --kind issue --repo o/r --host ghe.example.com');
    });
});
describe('runPrContext identity failure (handler level)', () => {
    ...
    it('never deletes the side file over a failed identity lookup', async () => {
        ...
        await prContextCommand.handler({
            _: [],
            $0: 'qwen',
            pr_number: '6711',
            owner_repo: 'o/r',
    ...
    const run = async () => prContextCommand.handler({
        _: [],
        $0: 'qwen',
        pr_number: '6711',
        owner_repo: 'o/r',
    ...
    const contextWrite = () => writeFileSyncMock.mock.calls.find((c) => c[0] === '/tmp/ctx.md')?.[1] ?? '';
    it('recovery SURVIVES the identity throw — isolation, not just non-deletion', async () => {
        ...
        expect(rmSyncMock.mock.calls.some((c) => String(c[0]).endsWith('prev-ledger.json'))).toBe(false);
    ...
    it('wires the foreign marker through to the rendered context and the side file', async () => {
        ...
        const sideWrite = writeFileSyncMock.mock.calls.find((c) => String(c[0]).includes('prev-ledger.json'));
        ...
        const ownSideWrite = writeFileSyncMock.mock.calls.find((c) => String(c[0]).includes('prev-ledger.json'));
...
describe('runPrContext host baking (handler level)', () => {
    ...
    async function runHandler(extra) {
        await prContextCommand.handler({
            _: [],
            $0: 'qwen',
            pr_number: '6711',
    ...
    it('bakes --host into the emitted refetch commands when passed', async () => {
        ...
        expect(written).toContain('comment-body 7 --kind review --pr 6711 --repo o/r --host ghe.example.com');
...
//# sourceMappingURL=pr-context.test.js.map