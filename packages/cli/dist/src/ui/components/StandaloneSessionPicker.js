...
import { getGitBranch, SessionService, } from '@lailatul-coder/lailatul-coder-core';
...
function StandalonePickerScreen({ sessionService, onSelect, onCancel, currentBranch, initialSessions, }) {
    const { exit } = useApp();
    ...
    return (_jsx(ConfigContext.Provider, { value: PREVIEW_CONFIG_STUB, children: _jsx(SettingsContext.Provider, { value: PREVIEW_SETTINGS_STUB, children: _jsx(SessionPicker, { sessionService: sessionService, onSelect: (id) => {
                    onSelect(id);
                    handleExit();
                }, onCancel: () => {
                    onCancel();
                    handleExit();
                }, currentBranch: currentBranch, centerSelection: true, initialSessions: initialSessions, enablePreview: true }) }) }));
}
/**
 ...
 */
export async function showResumeSessionPicker(cwd = process.cwd(), initialSessions) {
    const sessionService = new SessionService(cwd);
    const hasSession = await sessionService.loadLastSession();
    if (!hasSession) {
        writeStdoutLine('No sessions found. Start a new session with `LailatulCoder`.');
        return undefined;
    }
    ...
    return new Promise((resolve) => {
        let selectedId;
        const { unmount, waitUntilExit } = render(_jsx(KeypressProvider, { kittyProtocolEnabled: false, pasteWorkaround: process.platform === 'win32' ||
                parseInt(process.versions.node.split('.')[0], 10) < 20, children: _jsx(StandalonePickerScreen, { sessionService: sessionService, onSelect: (id) => {
                    selectedId = id;
                }, onCancel: () => {
                    selectedId = undefined;
                }, currentBranch: getGitBranch(cwd), initialSessions: initialSessions }) }), {
            exitOnCtrlC: false,
        });
        waitUntilExit().then(() => {
            unmount();
