 ...
 */
function collectFailedCallIds(history) {
 ...
 */
export function extractRecentFilePaths(history, maxFiles) {
    if (maxFiles <= 0)
        return [];
    ...
    for (let i = history.length - 1; i >= 0; i--) {
        ...
        for (let j = parts.length - 1; j >= 0; j--) {
            ...
            if (!call || !FILE_TOUCHING_TOOLS.has(call.name ?? ''))
                continue;
            // Skip paths whose tool call failed (denied / errored).
            if (call.id && failedCallIds.has(call.id))
                continue;
 ...
 * stayed green.
 */
function imagePartsInContentReverse(content) {
    const result = [];
    const parts = content.parts ?? [];
    for (let j = parts.length - 1; j >= 0; j--) {
        const part = parts[j];
 ...
 */
export function extractRecentImages(history, maxImages) {
    if (maxImages <= 0)
        return [];
    const collected = [];
 ...
 */
export function countToolResponseImages(history) {
    let count = 0;
    for (const content of history) {
        for (const part of content.parts ?? []) {
            const nested = getFunctionResponseParts(part);
 ...
 */
export async function readFileSizeAdaptive(filePath, maxTokens, signal) {
    ...
    const sample = buffer.subarray(0, Math.min(buffer.length, BINARY_DETECT_SAMPLE));
    let nonPrintable = 0;
    for (const byte of sample) {
        const printable = (byte >= 0x20 && byte <= 0x7e) || // ASCII printable
            byte === 0x09 || // tab
            byte === 0x0a || // LF
            byte === 0x0d || // CR
 ...
 * to references.
 */
export async function buildFileRestorationBlocks(filePaths, signal) {
    const references = [];
    const embeds = [];
    let usedChars = 0;
    ...
    for (const filePath of filePaths) {
        if (signal?.aborted)
            break;
        const result = await readFileSizeAdaptive(filePath, POST_COMPACT_MAX_TOKENS_PER_FILE, signal);
        if (result.kind === 'missing' || result.kind === 'binary')
            continue;
 ...
 */
export function buildImageRestorationBlock(images) {
    if (images.length === 0)
        return null;
    const lines = [
    ...
    for (const img of images) {
        if (img.sourceToolName) {
            ...
            lines.push(`- turn ${img.turnIndex}: ${img.sourceToolName} args=${argsStr}`);
        }
        else {
 ...
 */
const RESUME_TRAILER = 'Resume the prior task using the summary above. Continue from the last in-flight step; do not acknowledge the summary, do not re-introduce, do not greet the user again.';
/**
 ...
 */
export function stripAnalysisBlock(rawSummary) {
...
export function postProcessSummary(rawSummary) {
 ...
 */
function trailingFunctionCallContent(history) {
    const last = history[history.length - 1];
    if (!last || last.role !== 'model')
        return undefined;
    if (!last.parts?.some((p) => !!p.functionCall))
        return undefined;
    return last;
}
/**
...
const PLAN_MODE_REMINDER_TEXT = '<plan-mode-active>\n' +
...
function buildSubagentSnapshotPart(snaps) {
    ...
    const lines = shown.map((s) => {
        ...
        const truncated = flattened.length > MAX_SUBAGENT_DESC_CHARS
    ...
    if (overflow > 0) {
        lines.push(`- (… and ${overflow} older task${overflow === 1 ? '' : 's'} not shown)`);
    }
    return {
...
export async function composePostCompactHistory(history, summary, options = {}) {
    const { workspaceRoot, signal, maxFiles = POST_COMPACT_MAX_FILES_TO_RESTORE, maxImages = POST_COMPACT_MAX_IMAGES_TO_RESTORE, planModeActive, runningSubagents, } = options;
    // Workspace-boundary filter on the extracted file paths (Finding 4).
    const filePaths = extractRecentFilePaths(history, maxFiles).filter((p) => isInsideWorkspace(p, workspaceRoot));
    ...
    for (const block of fileBlocks) {
        for (const part of block.parts ?? [])
            postAckParts.push(part);
    }
    if (imageBlock) {
        for (const part of imageBlock.parts ?? [])
            postAckParts.push(part);
    }
...
//# sourceMappingURL=postCompactAttachments.js.map