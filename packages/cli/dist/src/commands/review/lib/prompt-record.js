 ...
 */
export function runEpochMs(planPath) {
    try {
 ...
 */
export function promptRecordDir(planPath) {
 ...
 */
const fileFor = (key) => `${encodeURIComponent(key)}.txt`;
...
export function recordedPromptPath(planPath, key) {
    return join(promptRecordDir(planPath), fileFor(key));
}
export function briefPath(planPath, key) {
...
export function findingsFilePath(planPath, key) {
    return join(promptRecordDir(planPath), `${encodeURIComponent(key)}.findings.md`);
}
/**
 ...
 */
export function findingsPointerOf(prompt) {
 ...
 */
export function writeFindingsFile(planPath, key, content) {
    ...
    try {
        ...
        // of it on EPIPE (`LailatulCoder … | head`).
        writeStderrLineSafe(`agent-prompt: failed to write findings file ${p}: ` +
            `${err.message} — inlining the list instead`);
        return null;
    }
    return p;
}
 ...
 */
export function writeBrief(planPath, key, brief) {
    ...
    if (hadRules && !brief.includes(RULES_MARKER)) {
        throw new Error(`agent-prompt: rebuilding "${key}" without --rules would overwrite a ` +
...
export function recordPrompt(planPath, key, prompt) {
    try {
 ...
 * live ones.
 */
export function readRecordedPrompts(planPath, sinceMs) {
    const out = new Map();
    const dir = promptRecordDir(planPath);
    let names;
    try {
    ...
    for (const name of names) {
        ...
        try {
            ...
            if (sinceMs !== undefined && statSync(file).mtimeMs < sinceMs)
                continue;
 ...
 */
export function wasDeliveredVerbatim(launchPrompt, built) {
 ...
 */
export function deliveredVerbatim(flattenedLaunch, built) {
 ...
 */
export function deliveredVerbatimLines(flattenedLaunch, builtLines) {
