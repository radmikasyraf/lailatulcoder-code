...
describe('useBranchCommand', () => {
    ...
    let getGoalRuntimeReady;
    let startNewSessionUI;
    let clearPendingState;
    let findSessionTitlesByPrefix;
    let clearItems;
    let loadHistory;
    let setSessionName;
    let remount;
    ...
    it('refuses to branch while background work is running', async () => {
        ...
        const blockedItem = addItem.mock.calls[0]?.[0];
    ...
    it('clears terminal background state after the branch initializes', async () => {
        ...
        expect(clearPendingState.mock.invocationCallOrder[0]).toBeLessThan(loadHistory.mock.invocationCallOrder[0]);
        expect(startNewSessionUI.mock.invocationCallOrder[0]).toBeLessThan(backgroundTaskRegistry.reset.mock.invocationCallOrder[0]);
    });
    ...
    it('starts the forked recorder from the post-title JSONL tail', async () => {
        ...
        expect(startNewSessionConfig).toHaveBeenCalledWith(expect.any(String), titled);
    });
    ...
    it('rolls core back when the fork contains malformed Goal state', async () => {
        getGoalRuntimeReady.mockRejectedValueOnce(new Error('unsupported Goal lifecycle record'));
        const { result } = renderHook(() => useBranchCommand(makeOptions()));
        await act(async () => {
            await result.current.handleBranch('my-branch');
        });
    ...
    it('records the user-provided name with a (Branch) suffix', async () => {
        ...
        expect(renameSession).toHaveBeenCalledWith(expect.any(String), 'my-branch (Branch)', 'manual');
    ...
    it('bumps to (Branch N) when the default suffix is already taken', async () => {
        ...
        expect(renameSession).toHaveBeenCalledWith(expect.any(String), 'my-branch (Branch 2)', 'manual');
    ...
    it('does ONE prefix scan even when many (Branch N) slots are taken', async () => {
        ...
        expect(renameSession).toHaveBeenCalledWith(expect.any(String), 'my-branch (Branch 5)', 'manual');
    });
    it('derives the base title from the first user ChatRecord when no name is given', async () => {
        ...
        expect(renameSession).toHaveBeenCalledWith(expect.any(String), 'help me fix the login bug (Branch)', 'auto');
    });
    it('falls back to "Branched conversation (Branch)" when the transcript has no user records', async () => {
        ...
        expect(renameSession).toHaveBeenCalledWith(expect.any(String), 'Branched conversation (Branch)', 'auto');
    });
    it('skips synthetic user-role records (cron, notification, etc.) and picks the first real prompt', async () => {
        ...
        expect(renameSession).toHaveBeenCalledWith(expect.any(String), 'what does this codebase do (Branch)', 'auto');
    });
    ...
    ])('removes the fork and leaves the parent active when title persistence %s', async (_label, arrange) => {
        arrange();
        const { result } = renderHook(() => useBranchCommand(makeOptions()));
        await act(async () => {
            await result.current.handleBranch('x');
    ...
    it('rolls core back to the parent session when getGeminiClient().initialize() rejects after swap', async () => {
        ...
        loadSession.mockImplementation(async (sid) => sid === oldSessionId ? parentResumed : forkResumed);
        const initialize = vi
            .fn()
        ...
        expect(startNewSessionConfig).toHaveBeenNthCalledWith(1, expect.not.stringMatching(oldSessionId), forkResumed);
        expect(startNewSessionConfig).toHaveBeenNthCalledWith(2, oldSessionId, parentResumed);
    ...
    it('still surfaces the error and leaves core on the parent when rollback re-init also throws', async () => {
        ...
        expect(startNewSessionConfig).toHaveBeenNthCalledWith(2, oldSessionId, expect.any(Object));
        expect(removeSession).toHaveBeenCalledTimes(1);
        expect(debugWarn).toHaveBeenCalledWith(expect.stringContaining('Rollback after failed /branch init failed'));
    ...
    it('does not roll core back to parent when a post-UI-swap step throws', async () => {
        ...
        expect(startNewSessionConfig).not.toHaveBeenCalledWith(oldSessionId, expect.anything());
        // The user still sees the failure surfaced as an error item.
        expect(addItem).toHaveBeenCalledWith(expect.objectContaining({
            type: 'error',
            text: expect.stringMatching(/Failed to branch conversation.*remount boom/),
        }), expect.any(Number));
    });
    ...
    it('applies collapse policy when collapseOnResume is true', async () => {
        ...
        expect(loadedItems).toEqual(expect.arrayContaining([
...
//# sourceMappingURL=useBranchCommand.test.js.map