...
export function normalizeParts(input) {
    ...
    if (Array.isArray(input)) {
        return input.map((part) => typeof part === 'string' ? { text: part } : part);
    }
    return [input];
}
/**
 ...
 */
export function hasImageParts(input) {
 ...
 */
export function splitImageParts(input) {
    const imageParts = [];
    const nonImageParts = [];
 ...
 */
export function replaceImagesWithText(input, text) {
    const result = [];
    let replaced = false;
    for (const part of normalizeParts(input)) {
        if (isImagePart(part)) {
 ...
 */
export function isUsableImagePart(part) {
    const data = part.inlineData?.data;
...
//# sourceMappingURL=image-part-utils.js.map