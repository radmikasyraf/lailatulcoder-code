...
export function parseCron(cronExpr) {
    ...
    if (parts.length !== 5) {
        throw new Error(`Cron expression must have exactly 5 fields, got ${parts.length}: "${cronExpr}"`);
    }
 ...
 */
export function matches(cronExpr, date) {
 ...
 */
export function nextFireTime(cronExpr, after) {
    const fields = parseCron(cronExpr);
