 ...
 */
import { parseRules, parseRule, matchesRule, resolveToolName, splitCompoundCommand, SHELL_TOOL_NAMES, toolMatchesRuleToolName, } from './rule-parser.js';
import { extractShellOperationsAcrossCommand } from './shell-semantics.js';
import { isShellCommandReadOnlyAST, isShellCommandReadOnlyASTInDirectory, } from '../utils/shellAstParser.js';
import { normalizeMonitorCommand } from '../utils/shell-utils.js';
import { createDebugLogger } from '../utils/debugLogger.js';
import { findDangerousAllowRules, isDangerousAllowRule, } from './dangerousRules.js';
...
export class PermissionManager {
    ...
    async evaluate(ctx) {
        ...
        // `deny: ["Write(.lailatulcoder/settings.json)"]` would miss
        ...
        if (command !== undefined) {
            ...
            if (subCommands.length > 1) {
                ...
                if (bashDecision === 'default' &&
                    SHELL_TOOL_NAMES.has(toolName) &&
                    command !== undefined) {
                    bashDecision = await this.resolveDefaultPermission(command, ctx.cwd ?? this.config.getCwd?.());
                }
            }
        }
        else {
     ...
     */
    evaluateSingle(ctx) {
        const { toolName, toolAliases, command, cwd, filePath, domain, specifier, toolParams, } = ctx;
        ...
        if (SHELL_TOOL_NAMES.has(toolName) && command !== undefined) {
            ...
            // `bash -lc 'echo > .lailatulcoder/settings.json'` still surfaces the inner
            ...
            const virtualDecision = this.evaluateShellVirtualOps(extractShellOperationsAcrossCommand(command, cwd), pathCtx);
            if (virtualDecision !== 'default' &&
     ...
     */
    evaluateShellVirtualOps(ops, pathCtx) {
        if (ops.length === 0)
            return 'default';
        let worst = 'default';
        for (const op of ops) {
            ...
            if (op.cwdUnknown &&
                ...
                debugLogger.info(`PermissionManager: cwdUnknown escalation to 'ask' for virtualTool=${op.virtualTool} filePath=${op.filePath}`);
                opDecision = 'ask';
            }
    ...
    hasDenyOrAskRuleForTool(toolName) {
        return [
            ...this.sessionRules.ask,
            ...this.persistentRules.ask,
            ...this.sessionRules.deny,
            ...this.persistentRules.deny,
        ].some((rule) => !rule.invalid && toolMatchesRuleToolName(rule.toolName, toolName));
    }
    /**
    ...
    async evaluateCompoundCommand(ctx, subCommands) {
        ...
        const changesDirectory = subCommands.some((command) => /^\s*(?:cd|pushd)(?:\s|$)/.test(command));
        for (const subCmd of subCommands) {
            ...
            const decision = rawDecision === 'default'
                ? await this.resolveDefaultPermission(changesDirectory ? ctx.command : subCmd, ctx.cwd ?? this.config.getCwd?.())
                : rawDecision;
     ...
     */
    async resolveDefaultPermission(command, cwd) {
        try {
    ...
    normalizePermissionContext(ctx) {
     ...
     */
    async isCommandAllowed(command, cwd) {
    ...
    hasRelevantRules(ctx) {
        ...
        // rules like `Write(.lailatulcoder/settings.json)` are recognised as relevant
        // for `cd .lailatulcoder && bash -lc 'echo > settings.json'`.
        if (SHELL_TOOL_NAMES.has(toolName) && command !== undefined) {
            ...
            if (ops.some((op) => {
                ...
                return (restrictiveRules.some((rule) => matchesRule(rule, ...opMatchArgs, undefined, undefined, 'canonical')) ||
                    allowRules.some((rule) => matchesRule(rule, ...opMatchArgs, undefined)));
            })) {
                return true;
            }
        }
        ...
        return (restrictiveRules.some((rule) => matchesRule(rule, ...matchArgs, 'canonical')) || allowRules.some((rule) => matchesRule(rule, ...matchArgs)));
    }
    /**
    ...
    hasMatchingAskRule(ctx) {
        ...
        if (SHELL_TOOL_NAMES.has(toolName) && command !== undefined) {
            ...
            if (ops.some((op) => {
                ...
                return askRules.some((rule) => matchesRule(rule, ...opMatchArgs, undefined, undefined, 'canonical'));
            })) {
                return true;
            }
        }
        ...
        return askRules.some((rule) => matchesRule(rule, ...matchArgs, 'canonical'));
    }
    hasAskRuleForTool(toolName) {
        return [...this.sessionRules.ask, ...this.persistentRules.ask].some((rule) => !rule.invalid && toolMatchesRuleToolName(rule.toolName, toolName));
    }
    // ---------------------------------------------------------------------------
     ...
     */
    addSessionAllowRule(raw) {
        if (raw && raw.trim()) {
            const rule = parseRule(raw);
            if (rule.invalid) {
                debugLogger.warn(`Ignoring malformed allow rule (unbalanced parentheses): ${rule.raw}`);
                return;
            }
            ...
            if (this.strippedAllowRules && isDangerousAllowRule(rule)) {
                ...
                const exists = this.strippedAllowRules.session.some((r) => r.raw === rule.raw);
                if (!exists) {
                    this.strippedAllowRules.session.push(rule);
                }
                debugLogger.info(`Stashed newly added dangerous allow rule while in AUTO mode: ${rule.raw}`);
                return;
            }
     ...
     */
    addSessionDenyRule(raw) {
        if (raw && raw.trim()) {
            const rule = parseRule(raw);
            if (rule.invalid) {
                debugLogger.warn(`Ignoring malformed deny rule (unbalanced parentheses): ${rule.raw}`);
                return;
            }
     ...
     */
    addSessionAskRule(raw) {
        if (raw && raw.trim()) {
            const rule = parseRule(raw);
            if (rule.invalid) {
                debugLogger.warn(`Ignoring malformed ask rule (unbalanced parentheses): ${rule.raw}`);
                return;
            }
     ...
     */
    addPersistentRule(raw, type) {
        const rule = parseRule(raw);
        if (rule.invalid) {
            debugLogger.warn(`Ignoring malformed ${type} rule (unbalanced parentheses): ${rule.raw}`);
            return rule;
        }
        ...
        if (type === 'allow' &&
            ...
            const exists = this.strippedAllowRules.persistent.some((r) => r.raw === rule.raw);
            if (!exists) {
                this.strippedAllowRules.persistent.push(rule);
            }
            debugLogger.info(`Stashed newly added dangerous persistent allow rule while in AUTO mode: ${rule.raw}`);
            return rule;
        }
    ...
    listRules() {
        const result = [];
        const addRules = (rules, type, scope) => {
    ...
    stripDangerousRulesForAutoMode() {
        ...
        const persistentDangerous = findDangerousAllowRules(this.persistentRules.allow);
        ...
        this.persistentRules.allow = this.persistentRules.allow.filter((r) => !persistentDangerousSet.has(r));
        this.sessionRules.allow = this.sessionRules.allow.filter((r) => !sessionDangerousSet.has(r));
        this.strippedAllowRules = {
