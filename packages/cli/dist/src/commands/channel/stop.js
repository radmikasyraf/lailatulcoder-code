...
import { readServiceInfo, signalService, waitForExit, removeServiceInfo, } from './pidfile.js';
...
export const stopCommand = {
    ...
    handler: async (argv) => {
        if (argv['daemon-url']) {
            ...
            try {
                const sdk = (await import('@lailatul-coder/sdk/daemon'));
                const client = new sdk.DaemonClient({
                    baseUrl: argv['daemon-url'],
                    ...(token ? { token } : {}),
                });
                const result = await client.stopChannelWorker(argv.timeout !== undefined ? { timeoutMs: argv.timeout } : undefined);
                writeStdoutLine(result.changed
                    ? 'Daemon-managed channels stopped.'
                    : 'Daemon-managed channels are already stopped.');
                process.exit(0);
            }
            catch (error) {
                writeStderrLine(`Failed to stop daemon-managed channels: ${error instanceof Error ? error.message : String(error)}`);
                process.exit(1);
            }
            return;
        }
        ...
        if (info.owner === 'serve') {
            writeStderrLine(`Channel service is managed by LailatulCoder serve (PID ${info.pid}). Stop LailatulCoder serve to stop channels.`);
            process.exit(1);
        }
        writeStdoutLine(`Stopping channel service (PID ${info.pid})...`);
        if (!signalService(info.pid, 'SIGTERM')) {
            writeStderrLine('Failed to send signal — process may have already exited.');
            removeServiceInfo();
            process.exit(0);
        }
        ...
        if (exited) {
            ...
            writeStderrLine('Service did not exit within 5 seconds. Sending SIGKILL...');
