...
import { AGENT_PLUGIN_MANIFEST, getAgentPluginSchemaStatus, loadAgentPluginManifest, loadAgentPluginMcpServers, loadAgentPluginSkills, } from './agent-plugins-v1/index.js';
import { resolveContainedExistingPath } from './agent-plugins-v1/paths.js';
import { prepareStoredGitCredential, prepareStoredGitCredentialDeletion, removeGitCredentialSelector, resolveStoredGitCredential, writeGitCredentialSelector, } from './extension-git-credentials.js';
...
export class ExtensionManager {
    ...
    getExtensionActivationFromSnapshot(extensionId, snapshot, workspacePath = this.workspaceDir) {
        ...
        return this.getExtensionActivationForIdentityFromSnapshot({ id: extension.id, name: extension.name }, snapshot, workspacePath);
    }
    getExtensionActivationForIdentityFromSnapshot(identity, snapshot, workspacePath = this.workspaceDir) {
        const activation = this.extensionStore.getActivation(snapshot, identity.id, identity.name, workspacePath);
        if (this.enabledExtensionNamesOverride.length === 0) {
            return activation;
        }
        return {
    ...
    getExtensionActivationForNameFromSnapshot(name, snapshot, workspacePath = this.workspaceDir) {
        const entry = Object.entries(snapshot.extensions).find(([, policy]) => policy.name.toLowerCase() === name.toLowerCase());
        if (!entry) {
            const cliOverride = this.enabledExtensionNamesOverride.length > 0;
            return {
                default: 'enabled',
                workspace: 'inherit',
                effective: cliOverride && !this.isEnabled(name) ? 'disabled' : 'enabled',
                source: cliOverride ? 'cli_override' : 'default',
            };
        }
        const [id, policy] = entry;
        return this.getExtensionActivationForIdentityFromSnapshot({ id, name: policy.name }, snapshot, workspacePath);
    }
    ...
    async setExtensionDefaultActivations(names, activation, onCommitted) {
        ...
        try {
            ...
            const snapshot = await this.extensionStore.setDefaultActivations(identities, activation);
            onCommitted?.(snapshot.generation);
            this.applyStoreActivation(snapshot);
            const warning = await this.refreshToolsAfterActivation(`${identities.length} extensions`);
            return warning ? { ...snapshot, warnings: [warning] } : snapshot;
        }
        finally {
            endMutation();
    ...
    async setExtensionWorkspaceActivations(names, workspacePath, activation, onCommitted) {
        ...
        try {
            ...
            if (activation === 'inherit') {
                const outcome = await this.extensionStore.clearWorkspaceActivations(identities, workspacePath);
                snapshot = outcome.snapshot;
                updated = outcome.updated;
            }
            else {
                snapshot = await this.extensionStore.setWorkspaceActivations(identities, workspacePath, activation);
            }
            if (!updated) {
                this.applyStoreActivation(snapshot);
    ...
    resolveBatchExtensionIdentities(names) {
        const loadedByName = new Map(this.getLoadedExtensions().map((extension) => [
    ...
    async refreshCacheWithSnapshot(options) {
        ...
        const dirFingerprintBeforeLoad = requestedNames.length === 0 ? this.extensionDirFingerprint() : undefined;
        ...
        if (dirFingerprintBeforeLoad !== undefined) {
            this.lastSourceFingerprint = this.sourceFingerprint(dirFingerprintBeforeLoad);
        }
        return snapshot;
    }
    static stampPath(target, followSymlinks = true) {
        try {
    ...
    extensionDirFingerprint() {
        ...
        for (const entry of entries) {
            ...
            const effectiveRoot = installMetadata?.type === 'link' &&
                typeof installMetadata.source === 'string' &&
                installMetadata.source.length > 0
                ? installMetadata.source
                : extensionRoot;
            const manifestName = getAgentPluginSchemaStatus(effectiveRoot) === 'unrelated'
            ...
            if (manifestName === AGENT_PLUGIN_MANIFEST) {
                try {
                    manifestPath = resolveContainedExistingPath(effectiveRoot, manifestPath);
                }
                catch {
                    followManifestSymlink = false;
                }
            }
            const stamp = ExtensionManager.stampPath(manifestPath, followManifestSymlink);
     ...
     */
    extensionStoreFingerprint() {
        return [
            `enablement:${ExtensionManager.stampPath(this.configFilePath)}`,
            `state:${ExtensionManager.stampPath(path.join(this.extensionStore.storeDir, 'state.json'))}`,
        ].join('|');
    }
    sourceFingerprint(dirFingerprint) {
    ...
    async loadExtension(context, options = {}) {
        ...
        try {
            ...
            if (loadedManifest.format === 'agent-plugins-v1') {
                config = {
                    ...config,
                    mcpServers: await loadAgentPluginMcpServers(effectiveExtensionPath, this.extensionStore.agentPluginDataRoot(extensionId), { createDataDir: true }),
                };
            }
            const extension = {
                id: extensionId,
    ...
    loadExtensionManifest(context) {
        ...
        if (agentPluginStatus !== 'unrelated') {
            try {
                ...
                throw new Error(`Failed to load Agent Plugins manifest from ${path.join(extensionDir, 'plugin.json')}: ${getErrorMessage(error)}`);
            }
        }
    ...
    async prepareExtensionInstall(options) {
        return (await this.installExtensionInternal({ ...options.installMetadata }, options.requestConsent, options.requestSetting, options.cwd, undefined, options.initialActivation, options.signal, true, false, options.localSourcePath, options.gitCredential));
    }
    async prepareExtensionUpdateFromState(extension, signal) {
        ...
        return (await this.installExtensionInternal({ ...installMetadata }, undefined, undefined, undefined, previousConfig, { scope: 'user' }, signal, true, false, undefined, gitCredential));
    }
    ...
    async installExtensionInternal(installMetadata, requestConsent, requestSetting, cwd, previousExtensionConfig, initialActivation, signal, prepareOnly, emitMutation, localSourcePathOverride, gitCredential) {
        ...
        const remoteGitInstall = installMetadata.type === 'git' ||
        ...
        let extension;
        const redactedInstallSource = gitCredential?.persistence === 'one_time'
        ...
        try {
            ...
            if (installMetadata.type === 'git' ||
                ...
                if (gitCredential) {
                    ...
                    installMetadata.gitCommit = await cloneFromGit(installMetadata, tempDir, signal, gitCredential, gitCredential.persistence === 'one_time');
                }
                else {
                    try {
                        ...
                        installMetadata.gitCommit = await cloneFromGit(installMetadata, tempDir, signal);
                ...
                isSupportedArchivePath(localSourcePathOverride ?? installMetadata.source)) {
                tempDir = await ExtensionStorage.createTmpDir();
                await extractArchiveFile(localSourcePathOverride ?? installMetadata.source, tempDir, signal);
                localSourcePath = tempDir;
            }
            else if (installMetadata.type === 'local' ||
            ...
            try {
                ...
                const { extensionDir, originSource, externalContent } = await convertCompatibleExtension(sourceBeforeConversion, installMetadata.pluginName, installMetadata.networkPolicy, signal);
                signal?.throwIfAborted();
                ...
                else if (gitCredential?.persistence === 'one_time' &&
                ...
                if (isAgentPlugin) {
                    newExtensionConfig = {
                        ...newExtensionConfig,
                        mcpServers: await loadAgentPluginMcpServers(localSourcePath, this.extensionStore.agentPluginDataRoot(extensionId)),
                    };
                }
                ...
                if (gitCredential?.persistence === 'stored') {
                    if (gitCredential.selector) {
                        await writeGitCredentialSelector(stagingPath, gitCredential.selector);
                    }
                    else {
                        preparedGitCredential = await prepareStoredGitCredential(stagingPath, gitCredential);
                    }
                }
                if (isUpdate) {
                ...
                const configHooksPath = typeof newExtensionConfig.hooks === 'string'
                    ...
                    : null;
                const usesPluginVariables = originSource === 'Claude' || originSource === 'Qoder';
                if (usesPluginVariables &&
                ...
                if (isUpdate) {
                    ...
                    logExtensionInstallEvent(telemetryConfig, new ExtensionInstallEvent(newExtensionConfig.name, newExtensionConfig.version, redactedInstallSource, 'success'));
                }
                if (this.extensionCache)
                    this.applyStoreActivation(snapshot);
                await this.refreshTools().catch((error) => {
                ...
                if (!ownershipTransferred && preparedGitCredential) {
                    await preparedGitCredential.discard().catch((error) => {
                        debugLogger.warn(`Failed to discard prepared extension Git credentials: ${getErrorMessage(error)}`);
                    });
                }
            ...
            if (isUpdate) {
                ...
                logExtensionInstallEvent(telemetryConfig, new ExtensionInstallEvent(newExtensionConfig?.name ?? '', newExtensionConfig?.version ?? '', redactedInstallSource, 'error'));
            }
            throw error;
        }
        finally {
    ...
    async cleanupPreparedExtension(prepared) {
        ...
        const settingsCleanup = !prepared.settingsActivated && prepared.discardSettings
            ...
            : [];
        const credentialCleanup = !prepared.gitCredentialActivated && prepared.discardGitCredential
        ...
        const credentialErrors = credentialCleanup.flatMap((result) => result.status === 'rejected' ? [result.reason] : []);
    ...
    async uninstallExtension(extensionIdentifier, isUpdate, cwd, onCommitted) {
        ...
        try {
            ...
            return await this.uninstallExtensionPolicy({ id: extension.id, name: extension.name }, extension.installMetadata?.type === 'link'
                ...
                : extension.path, isUpdate, telemetryConfig, onCommitted, extension.installMetadata?.credentialPersistence === 'stored');
        }
        finally {
            endMutation();
        }
    ...
    async uninstallExtensionById(extensionId, isUpdate, cwd, onCommitted) {
        ...
        try {
            ...
            if (!policy || policy.declarationOnly)
                return snapshot;
            const extension = this.getLoadedExtensions().find((candidate) => candidate.id === extensionId);
            const destinationDirectory = extension && extension.installMetadata?.type !== 'link'
                ? extension.path
                : path.join(this.configDir, policy.name);
            const installMetadata = extension?.installMetadata ??
                this.loadInstallMetadata(destinationDirectory);
            return await this.uninstallExtensionPolicy({ id: extensionId, name: policy.name }, destinationDirectory, isUpdate, getTelemetryConfig(cwd ?? this.workspaceDir, this.telemetrySettings), onCommitted, installMetadata?.credentialPersistence === 'stored');
        }
        finally {
            endMutation();
        }
    }
    async uninstallExtensionPolicy(identity, destinationDirectory, isUpdate, telemetryConfig, onCommitted, hasStoredGitCredential = false) {
        let deleteGitCredential;
...
export async function copyExtension(source, destination, options = {}) {
    ...
    await fs.promises.cp(copySource, destination, {
        ...
        filter: async (src) => {
            try {
                if (options.excludeRootGitDirectory &&
                ...
                if (options.skipSymlinks && stats.isSymbolicLink())
                    return false;
