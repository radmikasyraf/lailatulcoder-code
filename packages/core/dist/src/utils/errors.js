...
function describeErrorCause(cause) {
    if (cause == null)
        return undefined;
    return (describeCodedCause(cause, 0, new Set()) ??
...
function describeCodedCause(cause, depth, visited) {
    ...
    const nested = describeCodedCause(cause.cause, depth + 1, visited);
    if (nested) {
        return nested;
    }
...
export function getErrorMessage(error) {
    if (error instanceof Error) {
        ...
        if (detail && detail !== error.message) {
            return truncateStringifiedErrorMessage(`${error.message} (cause: ${detail})`);
        }
    ...
    if (error !== null && typeof error === 'object' && !Array.isArray(error)) {
        ...
        if (typeof message === 'string' && message.trim()) {
            ...
            const result = detail && detail !== message
 ...
 * LailatulCoder/lailatul-coder#4103.
 */
