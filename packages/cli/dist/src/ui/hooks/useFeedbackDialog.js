...
import { createDebugLogger, logUserFeedback, UserFeedbackEvent, isNodeError, Storage, } from '@lailatul-coder/lailatul-coder-core';
 ...
 */
const lastMessageIsAIResponse = (history) => history.length > 0 && history[history.length - 1].type === MessageType.GEMINI;
/**
...
const getFeedbackLastShownTimestampFromFile = () => {
    try {
        ...
        if (isNodeError(error) && error.code !== 'ENOENT') {
            debugLogger.warn('Failed to read feedbackLastShownTimestamp from settings file:', error);
        }
    }
    return 0;
};
 ...
 */
const shouldShowFeedbackBasedOnFatigue = () => {
 ...
 */
const meetsMinimumSessionRequirements = (sessionStats) => {
    const toolCallsCount = sessionStats.metrics.tools.totalCalls;
    const userMessagesCount = sessionStats.promptCount;
    return (toolCallsCount > MIN_TOOL_CALLS || userMessagesCount > MIN_USER_MESSAGES);
};
export const useFeedbackDialog = ({ config, settings, streamingState, history, sessionStats, }) => {
    ...
    const submitFeedback = useCallback((rating) => {
        ...
        if (rating >= FEEDBACK_OPTIONS.GOOD && rating <= FEEDBACK_OPTIONS.FINE) {
            const feedbackEvent = new UserFeedbackEvent(sessionStats.sessionId, rating, config.getModel(), config.getApprovalMode());
    ...
    useEffect(() => {
        const checkAndShowFeedback = () => {
            if (streamingState === StreamingState.Idle && history.length > 0) {
                ...
                // 1. LailatulCoder logger is enabled (required for feedback submission)
...
//# sourceMappingURL=useFeedbackDialog.js.map