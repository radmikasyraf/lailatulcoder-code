...
import { startEarlyStartupPrefetches, startPostRenderPrefetches, } from './startup-prefetch.js';
...
const mockGetIdeClientInstance = vi.hoisted(() => vi.fn().mockResolvedValue({ disconnect: mockDisconnectIde }));
...
vi.mock('@lailatul-coder/lailatul-coder-core', () => ({
    ...
    initializeTelemetry: (...args) => mockInitializeTelemetry(...args),
}));
vi.mock('../utils/apiPreconnect.js', () => ({
    preconnectApi: (...args) => mockPreconnectApi(...args),
}));
vi.mock('../utils/startupProfiler.js', () => ({
    recordStartupEvent: (...args) => mockRecordStartupEvent(...args),
}));
vi.mock('../ui/utils/updateCheck.js', async (importOriginal) => ({
    ...(await importOriginal()),
    checkForUpdatesDetailed: (...args) => mockCheckForUpdatesDetailed(...args),
}));
vi.mock('../utils/processUtils.js', () => ({
    ...
    requestUpdateOnExit: (...args) => mockRequestUpdateOnExit(...args),
}));
vi.mock('../utils/handleAutoUpdate.js', () => ({
    handleAutoUpdate: (...args) => mockHandleAutoUpdate(...args),
}));
...
vi.mock('../utils/updateEventEmitter.js', () => ({
    updateEventEmitter: {
        emit: (...args) => mockUpdateEventEmit(...args),
    },
}));
vi.mock('../i18n/index.js', () => ({
    t: (key, params) => params
        ? key.replace(/\{\{(\w+)\}\}/g, (match, name) => name in params ? String(params[name]) : match)
        : key,
}));
vi.mock('../core/initializer.js', () => ({
    connectIdeForStartup: (...args) => mockConnectIdeForStartup(...args),
}));
vi.mock('../utils/housekeeping/scheduler.js', () => ({
    startBackgroundHousekeeping: (...args) => mockStartBackgroundHousekeeping(...args),
}));
...
function makeSettings(enableAutoUpdate = undefined) {
    return {
        merged: {
...
describe('startupPrefetch', () => {
    ...
    it('does not record an unbalanced lifecycle event for API preconnect', () => {
        const config = makeConfig();
        startEarlyStartupPrefetches(config);
        expect(mockRecordStartupEvent).not.toHaveBeenCalledWith('startup_prefetch_started', { name: 'api_preconnect' });
    });
    ...
    it('records completed profiler lifecycle events for successful deferred tasks', async () => {
        ...
        expect(mockRecordStartupEvent).toHaveBeenCalledWith('startup_prefetch_started', { name: 'update_check' });
        expect(mockRecordStartupEvent).toHaveBeenCalledWith('startup_prefetch_completed', { name: 'update_check' });
    });
    ...
    it('starts post-render tasks without awaiting completion', async () => {
        ...
        expect(mockStartBackgroundHousekeeping).toHaveBeenCalledWith(config, expect.any(Object));
    });
    ...
    it('reports a timeout-specific reason when the update check times out', async () => {
        const config = makeConfig();
        const { UpdateCheckTimeoutError, FETCH_TIMEOUT_MS } = await import('../ui/utils/updateCheck.js');
        mockCheckForUpdatesDetailed.mockResolvedValue({
            status: 'error',
    ...
    it('reports an offline-specific reason when the registry is unreachable', async () => {
        ...
        await vi.dynamicImportSettled();
        expect(mockUpdateEventEmit).toHaveBeenCalledWith('update-failed', {
            message: 'Update check skipped (registry unreachable) — run /update to retry.',
            severity: 'warning',
        });
    });
    ...
    it('does not connect IDE when IDE mode is disabled', async () => {
        const config = makeConfig({ getIdeMode: () => false });
        const { statuses, stop } = captureIdeConnectionStatuses();
        try {
            startPostRenderPrefetches(config, makeSettings(), { connectIde: true });
            await vi.dynamicImportSettled();
    ...
    it('fails deferred IDE connection when the startup connect hangs', async () => {
        ...
        try {
            ...
            expect(mockRecordStartupEvent).toHaveBeenCalledWith('startup_prefetch_failed', { name: 'ide_connect' });
            expect(mockWarn).toHaveBeenCalledWith('ide_connect failed:', expect.objectContaining({
                message: 'ide_connect timed out after 15000ms',
            }));
    ...
    it('logs the underlying IDE connection error after timeout', async () => {
        ...
        const config = makeConfig();
        let rejectConnect;
        const delayedFailure = new Promise((_, reject) => {
        ...
        expect(mockDebug).toHaveBeenCalledWith('ide_connect underlying error after timeout:', underlyingError);
    });
    ...
    it('swallows telemetry initialization failures', async () => {
        ...
        expect(() => startPostRenderPrefetches(config, makeSettings(), {
            initializeTelemetry: true,
        })).not.toThrow();
        await vi.dynamicImportSettled();
    ...
    it('swallows deferred task failures', async () => {
        ...
        expect(() => startPostRenderPrefetches(config, makeSettings())).not.toThrow();
        await vi.dynamicImportSettled();
...
//# sourceMappingURL=startup-prefetch.test.js.map