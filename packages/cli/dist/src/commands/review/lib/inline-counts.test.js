...
import { carriedClaimLine, countInlineFindings, severityOf, stripSeverityPrefix, unmarkedComments, } from './inline-counts.js';
describe('stripSeverityPrefix — the attribution-off posted shape', () => {
    ...
    it('leaves an unmarked body alone', () => {
        ...
        expect(stripSeverityPrefix('see **[Critical]** above')).toBe('see **[Critical]** above');
    });
    it('strips stacked markers iteratively — a looping model drafts them', () => {
        expect(stripSeverityPrefix('**[Critical]** **[Suggestion]** broken')).toBe('broken');
        expect(stripSeverityPrefix('**[Critical]****[Critical]****[Critical]** x')).toBe('x');
    });
    it('skips render-nothing residue between stacked markers', () => {
        ...
        expect(stripSeverityPrefix('**[Critical]**<!-- x -->**[Suggestion]** text')).toBe('text');
        expect(stripSeverityPrefix('**[Critical]**\u200B**[Suggestion]** text')).toBe('text');
...
describe('carriedClaimLine — the shared readback strip', () => {
    it('reads the claim through every shape the classifier admits', () => {
        ...
        expect(carriedClaimLine('\u200B**[Critical]** R1-3: zwsp residue')).toBe('R1-3: zwsp residue');
        expect(carriedClaimLine('<!-- x -->**[Suggestion]** the claim')).toBe('the claim');
        // Residue BETWEEN the marker and the carried id.
        expect(carriedClaimLine('**[Critical]** <!-- x --> R1-2: the claim')).toBe('R1-2: the claim');
        // The full-width colon the prefix strip admits ('[:：]') — the
        // ASCII-only separator nulled the readback on this shape.
        expect(carriedClaimLine('**[Critical]**：R2-3: the claim')).toBe('R2-3: the claim');
        // The ASCII colon and the plain shapes keep their existing readback.
        expect(carriedClaimLine('**[Critical]**: R4-1: the claim')).toBe('R4-1: the claim');
...
describe('severityOf — one acceptance set with the strip', () => {
    it('classifies through the leading residue the strip skips', () => {
        ...
        expect(severityOf({ body: '<!-- x -->**[Critical]** text' })).toBe('critical');
        expect(severityOf({ body: '\u200B**[Suggestion]** text' })).toBe('suggestion');
        expect(countInlineFindings([{ body: '<!-- x -->**[Critical]** text' }])).toEqual({ criticalsInline: 1, suggestionsInline: 0 });
        expect(unmarkedComments([{ body: '<!-- x -->**[Critical]** text' }])).toEqual([]);
    });
...
//# sourceMappingURL=inline-counts.test.js.map