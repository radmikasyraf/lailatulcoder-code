...
import { estimateTextTokens, estimateTextTokenUnits, TOKEN_ESTIMATE_UNITS_PER_TOKEN, } from '../../utils/request-tokenizer/textTokenizer.js';
import { parseTaggedThinkingText } from './taggedThinkingParser.js';
import { convertSchema, relaxSchemaForFunctionCalling, } from '../../utils/schemaConverter.js';
...
function normalizeStreamingTextDelta(rawDelta, state) {
    ...
    if (state.cumulativeMode) {
        ...
        if (state.emittedText.startsWith(rawDelta)) {
            debugLogger.debug(`normalizeStreamingTextDelta: cumulative rewind suppression (emitted=${state.emittedText.length}b, chunk=${rawDelta.length}b)`);
            return '';
        }
        debugLogger.debug('normalizeStreamingTextDelta: exiting cumulative mode (chunk does not match prior accumulated text)');
    ...
    if (rawDelta.length > state.emittedText.length &&
        ...
        // case).
        const baselineFrozenAtCap = baselineLen >= CUMULATIVE_DETECTION_WINDOW_BYTES &&
        ...
        if (rawDelta.length > sliceFrom) {
            ...
            debugLogger.debug(`normalizeStreamingTextDelta: entered cumulative mode (prefix overlap, baseline=${baselineLen}b sliceFrom=${sliceFrom}b -> curr=${rawDelta.length}b)`);
            return suffix;
        }
    ...
    if (rawDelta === state.emittedText) {
        if (rawDelta.length >= CUMULATIVE_DELTA_EXACT_REPEAT_MIN_LENGTH) {
            state.cumulativeMode = true;
            debugLogger.debug(`normalizeStreamingTextDelta: entered cumulative mode (exact repeat, ${rawDelta.length}b)`);
            return '';
        }
 ...
 */
export function convertGeminiToolParametersToOpenAI(parameters) {
 ...
 */
export async function convertGeminiToolsToOpenAI(geminiTools, schemaCompliance = 'auto') {
    const openAITools = [];
            ...
            for (const func of actualTool.functionDeclarations) {
                if (func.name) {
                    ...
                    if (func.parametersJsonSchema) {
                        ...
                        parameters = convertGeminiToolParametersToOpenAI(func.parameters);
                    }
                    if (parameters) {
 ...
 */
export function convertGeminiRequestToOpenAI(request, requestContext, options = { cleanOrphanToolCalls: true }) {
    let messages = [];
 ...
 */
export function convertGeminiResponseToOpenAI(response, requestContext) {
...
function addSystemInstructionMessage(request, messages) {
    if (!request.config?.systemInstruction)
        return;
    const systemText = extractTextFromContentUnion(request.config.systemInstruction);
    if (systemText) {
        messages.push({
            role: 'system',
            content: systemText,
...
function processContent(content, messages, requestContext) {
    ...
    const emittedFunctionResponseIds = new Set();
    ...
    for (const part of parts) {
        ...
        if ('functionCall' in part && part.functionCall && role === 'assistant') {
            ...
            if (callId) {
                if (emittedFunctionCallIds.has(callId)) {
                    debugLogger.debug(`Dropping duplicate functionCall id=${callId} while converting content`);
                    continue;
                }
        ...
        if (part.functionResponse && role === 'user') {
            ...
            const toolMessage = createToolMessage(part.functionResponse, requestContext);
            if (toolMessage) {
                ...
                if (requestContext.splitToolMedia &&
                ...
                if (requestContext.toolResultContentFormat === 'string' &&
                    Array.isArray(toolMessage.content)) {
                    const toolContent = toolMessage.content;
                    if (toolContent.every((cp) => cp?.type === 'text')) {
...
function createMediaContentPart(part, requestContext) {
    ...
    if (part.inlineData?.mimeType && part.inlineData?.data) {
        ...
        if (mediaType === 'image') {
            if (!modalities.image) {
                return unsupportedModalityPlaceholder('image', displayName, requestContext);
            }
        ...
        if (mimeType === 'application/pdf') {
            if (!modalities.pdf) {
                return unsupportedModalityPlaceholder('pdf', displayName, requestContext);
            }
        ...
        if (mediaType === 'audio') {
            if (!modalities.audio) {
                return unsupportedModalityPlaceholder('audio', displayName, requestContext);
            }
        ...
        if (mediaType === 'video') {
            if (!modalities.video) {
                return unsupportedModalityPlaceholder('video', displayName, requestContext);
            }
            return {
                type: 'video_url',
                video_url: {
    ...
    if (part.fileData?.mimeType && part.fileData?.fileUri) {
        ...
        if (mediaType === 'image') {
            if (!modalities.image) {
                return unsupportedModalityPlaceholder('image', filename, requestContext);
            }
            return {
                type: 'image_url',
        ...
        if (mediaType === 'video') {
            if (!modalities.video) {
                return unsupportedModalityPlaceholder('video', filename, requestContext);
            }
            return {
                type: 'video_url',
                video_url: {
...
function unsupportedModalityPlaceholder(modality, displayName, requestContext) {
    debugLogger.warn(`Model '${requestContext.model}' does not support ${modality} input. ` +
...
const LEADING_CLOSING_THINKING_TAG_PATTERN = /^[^\S\r\n]*<\/think(?:ing)?[^\S\r\n]*>/i;
const LEADING_THINKING_TAG_PATTERN = /^\s*<\/?think(?:ing)?\s*>/i;
const STANDALONE_CLOSING_THINKING_TAG_PATTERN = /^\s*<\/(think|thinking)\s*>\s*$/i;
const STANDALONE_OPENING_THINKING_TAG_PATTERN = /^\s*<(think|thinking)\s*>\s*$/i;
...
function classifyContentOnlyThinkingTagPrefix(text, streamFinished) {
    ...
    if (!candidate)
        return 'clean';
    const consumeTag = (value, closing) => {
        ...
        if (!canBeStandaloneThinkingTagPrefix(value))
            return undefined;
        if (!value || value === '<')
            return null;
...
function throwProtocolTagLeak(requestContext) {
    ...
    throw new InvalidStreamError('Model response leaked thinking tags.', 'PROTOCOL_TAG_LEAK');
}
/**
 * Convert OpenAI response to Gemini format.
 */
export function convertOpenAIResponseToGemini(openaiResponse, requestContext) {
        ...
        response.candidates = [
            {
                ...
                finishReason: mapOpenAIFinishReasonToGemini(choice.finish_reason || 'stop'),
                index: 0,
                safetyRatings: [],
            },
        ];
    ...
    if (openaiResponse.usage) {
        ...
        const cachedTokens = usage.prompt_tokens_details?.cached_tokens ??
            ...
            0;
        const cachedInputTokensReported = typeof usage.prompt_tokens_details?.cached_tokens === 'number' ||
        ...
        if (thinkingTokens == null) {
            ...
            if (thinkingTokens > 0) {
                debugLogger.debug(`convertOpenAIResponseToGemini: reasoning_tokens absent; estimated ${thinkingTokens} from text`);
            }
        }
 ...
 */
export function convertOpenAIChunkToGemini(chunk, requestContext) {
    const choice = chunk.choices?.[0];
    const response = new GenerateContentResponse();
    const preparations = [];
    const toolCallParser = requestContext.toolCallParser;
    if (!toolCallParser) {
        throw new Error('convertOpenAIChunkToGemini requires requestContext.toolCallParser — attach a fresh StreamingToolCallParser at stream start.');
    }
    if (choice) {
        let parts = [];
        let contentParts = [];
        ...
        if (typeof choice.delta?.content === 'string') {
            const normalizedContent = normalizeStreamingTextDelta(choice.delta.content, (requestContext.textDeltaState ??= {
                emittedText: '',
                emittedLength: 0,
            ...
            if (normalizedContent || choice.finish_reason) {
                contentParts = convertOpenAITextToParts(normalizedContent, requestContext, Boolean(choice.finish_reason));
            }
        }
        ...
        if (hasThoughtPart(contentParts)) {
            ...
            debugLogger.debug('convertOpenAIChunkToGemini: tagged thinking content emitted a thought; dropping buffered reasoning');
            if (requestContext.pendingContentParts?.length) {
                debugLogger.debug(`convertOpenAIChunkToGemini: flushing ${requestContext.pendingContentParts.length} buffered content part(s) before tagged content`);
        ...
        if (reasoningText &&
            ...
            const normalizedReasoningText = normalizeStreamingTextDelta(reasoningText, reasoningDeltaState);
            ...
            else if (normalizedReasoningText &&
                !requestContext.hasTaggedThinkingThought) {
                requestContext.pendingReasoningText =
                    (requestContext.pendingReasoningText ?? '') + normalizedReasoningText;
                debugLogger.debug(`convertOpenAIChunkToGemini: buffered reasoning text (${requestContext.pendingReasoningText.length} chars) for tagged stream`);
            }
        }
        if (requestContext.responseParsingOptions?.taggedThinkingTags &&
            !requestContext.hasTaggedThinkingThought &&
            ...
            debugLogger.debug(`convertOpenAIChunkToGemini: buffered ${contentParts.length} content part(s) behind pending reasoning`);
            contentParts = [];
        }
        if (choice.finish_reason &&
            ...
            !requestContext.hasTaggedThinkingThought &&
            requestContext.pendingReasoningText) {
            debugLogger.debug('convertOpenAIChunkToGemini: flushing buffered reasoning for tagged stream with no tagged thought');
            parts.push(createOpenAIReasoningThoughtPart(requestContext.pendingReasoningText));
            requestContext.pendingReasoningText = undefined;
        }
        if (choice.finish_reason && requestContext.pendingContentParts?.length) {
            debugLogger.debug(`convertOpenAIChunkToGemini: flushing ${requestContext.pendingContentParts.length} buffered content part(s) on stream finish`);
            parts.push(...requestContext.pendingContentParts);
            requestContext.pendingContentParts = undefined;
        }
        ...
        if (choice.delta?.tool_calls) {
            for (const toolCall of choice.delta.tool_calls) {
                ...
                const parseResult = toolCall.function?.arguments
                    ? toolCallParser.addChunk(index, toolCall.function.arguments, toolCall.id, toolCall.function.name)
                    : toolCallParser.addChunk(index, '', // Empty chunk for metadata-only updates
                    toolCall.id, toolCall.function?.name);
                const { id: callId, name: toolName } = toolCallParser.getToolCallMeta(parseResult.actualIndex ?? index);
                if (callId && toolName) {
        ...
        const getVisibleText = (part) => part.thought !== true && typeof part.text === 'string' ? part.text : '';
        let visibleText = parts.map(getVisibleText).join('');
        const pendingTagCandidate = requestContext.pendingThinkingTagCandidate;
        const replayedTagPrefix = !pendingTagCandidate?.closingTagName &&
            /\S/.test(pendingTagCandidate?.text ?? '') &&
            pendingTagCandidate?.text === visibleText;
        const replayedClosingTag = STANDALONE_CLOSING_THINKING_TAG_PATTERN.exec(visibleText)?.[1]?.toLowerCase();
        if (replayedTagPrefix ||
        ...
        const detectContentOnlyThinkingTagLeaks = requestContext.responseParsingOptions?.contentOnlyThinkingTagLeaks ===
            true;
        const contentOnlyThinkingState = hasStructuredReasoning ||
            requestContext.hasVisibleContent === true ||
            !detectContentOnlyThinkingTagLeaks
            ? 'clean'
            : classifyContentOnlyThinkingTagPrefix(combinedCandidateText, Boolean(choice.finish_reason));
        const canStartTagCandidate = requestContext.hasVisibleContent !== true &&
        ...
        if (pendingTagCandidate || canStartTagCandidate) {
            const closingTag = STANDALONE_CLOSING_THINKING_TAG_PATTERN.exec(combinedCandidateText)?.[1]?.toLowerCase();
            const closingTagName = closingTag === 'think' || closingTag === 'thinking'
                ? closingTag
                : undefined;
            const openingTag = STANDALONE_OPENING_THINKING_TAG_PATTERN.test(combinedCandidateText);
            const isPossibleTag = canBeStandaloneThinkingTagPrefix(combinedCandidateText) ||
                contentOnlyThinkingState === 'pending' ||
                contentOnlyThinkingState === 'suspicious';
            const finishedWhitespaceCandidate = Boolean(choice.finish_reason) &&
                !closingTagName &&
            ...
            const confirmedOpeningTagCandidate = LEADING_THINKING_TAG_PATTERN.test(combinedCandidateText) &&
                !combinedCandidateText.trimStart().startsWith('</');
            const releaseContentOnlyCandidate = contentOnlyThinkingState === 'pending' &&
            ...
            if (finishedWhitespaceCandidate || releaseContentOnlyCandidate) {
                ...
                if (!confirmedOpeningTagCandidate &&
                    !closingTagName &&
                    combinedCandidateText.trimStart().length >
                        MAX_THINKING_TAG_CANDIDATE_LENGTH) {
                    throwProtocolTagLeak(requestContext);
        ...
        const leakedThinkingTag = requestContext.hasStructuredReasoningContent === true &&
            ((requestContext.hasVisibleContent !== true &&
        ...
        if (choice.finish_reason &&
            requestContext.pendingThinkingTagCandidate?.closingTagName) {
            if (requestContext.hasThinkingTagInReasoning === true ||
        ...
        const shouldHoldParts = !choice.finish_reason &&
            (toolCallWithoutName ||
                requestContext.hasThinkingTagInReasoning === true ||
                requestContext.pendingThinkingTagCandidate !== undefined);
        if (shouldHoldParts) {
        ...
        if (effectiveFinishReason) {
            candidate.finishReason = mapOpenAIFinishReasonToGemini(effectiveFinishReason);
        }
    ...
    if (chunk.usage) {
        ...
        const totalTokens = usage.total_tokens || 0;
        const providerReasoningTokens = usage.completion_tokens_details?.reasoning_tokens;
        const estimatedThinkingTokens = Math.ceil((requestContext.reasoningDeltaState?.emittedTokenUnits ?? 0) /
            TOKEN_ESTIMATE_UNITS_PER_TOKEN);
        const thinkingTokens = providerReasoningTokens ??
            (completionTokens > 0
                ? Math.min(estimatedThinkingTokens, completionTokens)
                : estimatedThinkingTokens);
        if (providerReasoningTokens == null && estimatedThinkingTokens > 0) {
            debugLogger.debug(`convertOpenAIChunkToGemini: reasoning_tokens absent; estimated ${thinkingTokens} from streamed text`);
        }
        // Support both formats: prompt_tokens_details.cached_tokens (OpenAI standard)
        // and cached_tokens (some models return it at top level)
        const extendedUsage = usage;
...
function mapOpenAIFinishReasonToGemini(openaiReason) {
    if (!openaiReason)
...
function mapGeminiFinishReasonToOpenAI(geminiReason) {
    if (!geminiReason) {
        return 'stop';
    }
    switch (geminiReason) {
...
function cleanOrphanedToolCalls(messages) {
    ...
    const validToolResponseIndexesByAssistant = new Map();
    const splitMediaIndexesByAssistant = new Map();
    const emittedWithAssistant = new Set();
    const survivingToolCallIds = new Set();
    for (let index = 0; index < messages.length; index += 1) {
        const message = messages[index];
        if (hasToolCalls(message)) {
            const candidateToolCalls = [];
            const candidateToolCallIds = new Set();
            ...
            const adjacentToolResponseIds = new Set();
            ...
            for (let nextIndex = index + 1; nextIndex < messages.length; nextIndex += 1) {
                ...
                if (nextMessage.role === 'tool' && 'tool_call_id' in nextMessage) {
                    ...
                    if (candidateToolCallIds.has(nextMessage.tool_call_id) &&
            ...
            const validToolCalls = candidateToolCalls.filter((toolCall) => adjacentToolResponseIds.has(toolCall.id));
    ...
    for (let index = 0; index < messages.length; index += 1) {
        ...
        if (hasToolCalls(message)) {
            ...
            if (validToolCalls.length > 0) {
                ...
                for (const toolResponseIndex of validToolResponseIndexesByAssistant.get(index) ?? []) {
                ...
                debugLogger.debug(`cleanOrphanedToolCalls: dropping assistant with ${message.tool_calls.length} orphaned tool call(s) and no text/reasoning content`);
            }
        }
        else if (message.role === 'tool' && 'tool_call_id' in message) {
            debugLogger.debug(`cleanOrphanedToolCalls: dropping orphaned tool response ${message.tool_call_id || '<empty>'}`);
        }
        else if (isSplitToolMediaMessage(message)) {
            debugLogger.debug('cleanOrphanedToolCalls: dropping orphaned split tool media message');
        }
        else {
