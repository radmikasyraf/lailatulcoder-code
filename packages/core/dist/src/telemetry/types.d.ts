...
export declare class UserPromptEvent implements BaseTelemetryEvent {
    ...
    model?: string;
    constructor(prompt_length: number, prompt_Id: string, auth_type?: string, prompt?: string, model?: string);
}
...
export declare class ApiRequestEvent implements BaseTelemetryEvent {
    ...
    constructor(model: string, prompt_id: string, request_text?: string, subagent_name?: string);
}
...
export declare class ApiCancelEvent implements BaseTelemetryEvent {
    ...
    constructor(model: string, prompt_id: string, auth_type?: string, loop_wakeups_cancelled?: number);
}
export declare class ApiResponseEvent implements BaseTelemetryEvent {
     ...
     */
    subagent_name?: string;
    constructor(response_id: string, model: string, duration_ms: number, prompt_id: string, auth_type?: string, usage_data?: GenerateContentResponseUsageMetadata, response_text?: string, subagent_name?: string, ttft_ms?: number);
}
...
export type RepeatedToolFailureGuardTelemetryMode = 'shadow' | 'warn' | 'enforce';
export type RepeatedToolFailureGuardTelemetryPhase = 'idle' | 'tracking' | 'warned' | 'latched';
export type RepeatedToolFailureGuardTelemetryDecision = 'reset' | 'tracked' | 'would_warn' | 'warned' | 'would_stop' | 'stopped';
export type RepeatedToolFailureGuardCountBucket = '0' | '1-2' | '3-4' | '5-7' | '8+';
export type RepeatedToolFailureGuardBatchBucket = '0' | '1' | '2' | '3+';
export type RepeatedToolFailureGuardResetReason = 'success' | 'cancelled' | 'not_started' | 'post_execution_failure' | 'unknown' | 'mixed' | 'incomplete' | 'external_input' | 'queued_prompt' | 'unreliable_input' | 'contract_violation';
export declare class RepeatedToolFailureGuardEvent implements BaseTelemetryEvent {
    ...
    tool_type?: 'native' | 'mcp';
    constructor(params: Omit<RepeatedToolFailureGuardEvent, 'event.name' | 'event.timestamp'>);
}
...
export declare function makeChatCompressionEvent({ tokens_before, tokens_after, compression_input_token_count, compression_output_token_count, cache_sharing_attempted, cache_sharing_used, }: Omit<ChatCompressionEvent, CommonFields>): ChatCompressionEvent;
...
export declare class HookCallEvent implements BaseTelemetryEvent {
    ...
    success: boolean;
    error?: string;
    constructor(hookEventName: string, hookType: HookTelemetryType, hookName: string, hookInput: Record<string, unknown>, durationMs: number, success: boolean, hookOutput?: Record<string, unknown>, exitCode?: number, stdout?: string, stderr?: string, error?: string);
}
...
export declare class MemoryExtractEvent implements BaseTelemetryEvent {
    ...
    skipped_reason?: 'already_running' | 'queued' | 'memory_tool' | 'memory_pressure';
...
export type MemoryRecallDiscardReason = 'no_safe_delivery_point' | 'new_query' | 'reset' | 'abort' | 'shutdown' | 'no_relevant_results'
