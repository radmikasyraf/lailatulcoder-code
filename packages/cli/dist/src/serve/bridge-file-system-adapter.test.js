...
import { createWorkspaceFileSystemFactory, } from './fs/workspace-file-system.js';
describe('createBridgeFileSystemAdapter', () => {
    ...
    beforeEach(async () => {
        ...
        tmpDir = await fsp.realpath(await fsp.mkdtemp(path.join(os.tmpdir(), 'bridge-fs-adapter-')));
        outsideDir = await fsp.realpath(await fsp.mkdtemp(path.join(os.tmpdir(), 'bridge-fs-outside-')));
        auditEmits = [];
    });
    afterEach(async () => {
        await Promise.all([
    ...
    describe('writeText (trusted workspace)', () => {
        it('writes content to disk through the PR 18 layer', async () => {
            const adapter = createBridgeFileSystemAdapter(buildFactory({ trusted: true }));
    ...
    describe('writeText (untrusted workspace)', () => {
        it('rejects with FsError when trust gate is closed', async () => {
            const adapter = createBridgeFileSystemAdapter(buildFactory({ trusted: false }));
            ...
            await expect(fsp.stat(path.join(tmpDir, 'denied.txt'))).rejects.toThrow(/ENOENT/);
        });
    ...
    describe('readText', () => {
        ...
        it('forwards line/limit window to PR 18', async () => {
            ...
            await fsp.writeFile(target, Array.from({ length: 10 }, (_, i) => `line${i + 1}`).join('\n') + '\n', 'utf8');
            const adapter = createBridgeFileSystemAdapter(buildFactory({ trusted: true }));
            const response = await adapter.readText({
                path: target,
                sessionId: 'sess:test',
        ...
        it('propagates symlink_escape from wfs.resolve when target is a symlink to outside', async () => {
            ...
            try {
                ...
                expect(['symlink_escape', 'path_outside_workspace']).toContain(err.kind);
            }
            finally {
    ...
    describe('same-host built-in tool writes', () => {
        ...
        it('writes an external file only when the adapter opts in and provenance is valid', async () => {
            const adapter = createBridgeFileSystemAdapter(buildFactory({ trusted: true }), { allowSameHostToolWritesOutsideWorkspace: true });
        ...
        it.each([
            ...
            const adapter = createBridgeFileSystemAdapter(buildFactory({ trusted: true }), enabled === undefined
                ? undefined
        ...
        it('rejects external writes from an untrusted runtime with one denied audit', async () => {
            const adapter = createBridgeFileSystemAdapter(buildFactory({ trusted: false }), { allowSameHostToolWritesOutsideWorkspace: true });
        ...
        it('rejects a Unix socket as an external text target', async () => {
            if (process.platform === 'win32')
                return;
            const socketDir = await fsp.mkdtemp(path.join(os.tmpdir(), 'bridge-fs-socket-'));
        ...
        it('canonicalizes a symlinked parent for a new external file', async () => {
            const alias = path.join(path.dirname(outsideDir), `${path.basename(outsideDir)}-alias`);
            ...
            try {
                ...
                expect(await fsp.readFile(path.join(outsideDir, 'through-alias.txt'), 'utf8')).toBe('through-alias');
            }
            finally {
        ...
        it('strips the ACP BOM marker before re-encoding non-UTF-8 content', async () => {
            ...
            expect(bytes.subarray(0, 4)).toEqual(Buffer.from([0xff, 0xfe, 0x68, 0x00]));
        ...
        it('preserves CRLF and writes exactly one UTF-8 BOM', async () => {
            ...
            expect(await fsp.readFile(target)).toEqual(Buffer.concat([
...
//# sourceMappingURL=bridge-file-system-adapter.test.js.map