 ...
 * under the user runtime dir (~/.lailatulcoder/tmp/<project-hash>/) and survive
 * restarts.
 */
...
import { addCronTask, CRON_TASKS_DISPLAY_PATH, appendCronRun, generateCronTaskId, getCronFilePath, readCronTasks, removeCronTasks, taskHasLegacyCondition, taskHasLegacyRunMode, updateCronTasks, } from './cronTasksFile.js';
...
const DEFAULT_RECURRING_MAX_AGE_MS = DEFAULT_RECURRING_MAX_AGE_DAYS * 24 * 60 * 60 * 1000;
...
export function normalizeRecurringMaxAge(value, fallback) {
    if (value === 0)
        return Infinity;
...
function isCronSlotVisibleToTick(slotMinuteMs, currentMs, jitterMs) {
    ...
    return (Math.abs(currentMinute.getTime() - slotMinuteMs) <=
...
export function clampWakeupSeconds(delaySeconds) {
    if (!Number.isFinite(delaySeconds))
        return WAKEUP_DEFAULT_SECONDS;
    return Math.min(WAKEUP_MAX_SECONDS, Math.max(WAKEUP_MIN_SECONDS, Math.round(delaySeconds)));
}
/**
...
export class CronScheduler {
    ...
    // headless run can't expand a `.lailatulcoder/loop.md` sentinel, so it marks such
    ...
    warnedLegacyConditionIds = new Set();
    ...
    warnedLegacyRunModeIds = new Set();
    ...
    firePersistPending = new Map();
    markFirePersistPending(ids) {
        for (const id of ids) {
            this.firePersistPending.set(id, (this.firePersistPending.get(id) ?? 0) + 1);
        }
    }
    clearFirePersistPending(ids) {
        for (const id of ids) {
            ...
            if (next > 0)
                this.firePersistPending.set(id, next);
            else
                this.firePersistPending.delete(id);
        }
    }
    fileWatcher = null;
    lockProbeTimer = null;
    ...
    constructor(projectRoot = null, recurringMaxAgeMs = DEFAULT_RECURRING_MAX_AGE_MS) {
        ...
        this.recurringMaxAgeMs = normalizeRecurringMaxAge(recurringMaxAgeMs, DEFAULT_RECURRING_MAX_AGE_MS);
    }
    /**
    ...
    create(cronExpr, prompt, recurring) {
        ...
        if (process.env['QWEN_CODE_TEST_CRON_FAST'] === '1' && !job.durable) {
            ...
            debugLogger.debug(`Test seam: auto-fire scheduled for job ${id} in ${delayMs}ms`);
        }
        return job;
    }
    /**
    ...
    scheduleWakeup(delaySeconds, prompt, todoWorkChainId) {
        ...
        if (this._disabled) {
            throw new Error('Cannot schedule a loop wakeup: the scheduler is disabled for this ' +
        ...
        const roundedDelaySeconds = Number.isFinite(delaySeconds)
            ...
            : delaySeconds;
        const wasClamped = !Number.isFinite(delaySeconds) ||
        ...
        if (fireAtMs > this.wakeupChainStartedAt + WAKEUP_CHAIN_MAX_AGE_MS) {
            throw new Error('Loop wakeup chain exceeded the 24h session limit. ' +
        ...
        debugLogger.debug(`Wakeup ${id} scheduled for ${new Date(fireAtMs).toISOString()} ` +
    ...
    cancelAllWakeups() {
        ...
        this.wakeups.clear();
        if (count > 0)
            debugLogger.debug(`Cancelled ${count} wakeup(s)`);
        return count;
    }
    /**
     * Creates a durable cron job: registered like any other job, and
     * persisted under ~/.lailatulcoder (per-project) so it survives restarts.
    ...
    async enableDurable(sessionId) {
        ...
        try {
            ...
            const acquired = await tryAcquireLock(projectRoot, sessionId, this.lockId);
            if (generation !== this.durableGeneration) {
    ...
    releaseLateAcquisition(acquired, projectRoot, sessionId) {
        if (!acquired)
            return;
        if (this.durableEnabled && this.sessionId === sessionId)
            return;
        this.pendingRelease = releaseLock(projectRoot, sessionId, this.lockId);
    }
    async loadFileTasks(handleMissed) {
        ...
        try {
            ...
            if (this.jobs.size > 0) {
                ...
                console.warn('CronScheduler: durable tasks reload failed; keeping the previous ' +
        ...
        const tasks = read.filter((t) => {
            if (!hasParseableCron(t) || t.enabled === false)
                return false;
            if (taskHasLegacyCondition(t)) {
                if (!this.warnedLegacyConditionIds.has(t.id)) {
                    ...
                    console.warn(`CronScheduler: scheduled task ${t.id} carries a legacy precondition ` +
            ...
            if (taskHasLegacyRunMode(t) && !this.warnedLegacyRunModeIds.has(t.id)) {
                ...
                console.warn(`CronScheduler: scheduled task ${t.id} was created with the removed ` +
        ...
        // responsibility and skipped.
        {
            for (const t of tasks) {
                ...
                if (this.pendingRemoval.has(t.id))
                    continue;
                const responsibleForMissed = typeof t.sessionId === 'string' && t.sessionId.length > 0
                    ? t.sessionId === this.sessionId
                    : handleMissed;
                if (!responsibleForMissed)
                    continue;
                ...
                if (this.firePersistPending.has(t.id))
                    continue;
                ...
                if (!t.recurring) {
                    ...
                    console.warn(`Durable cron task ${t.id} (created ${new Date(t.createdAt).toISOString()}) is past the recurring max age at load; ` +
        ...
        for (const task of tasks) {
            if (this.pendingRemoval.has(task.id))
                continue;
            const existing = this.jobs.get(task.id);
            if (!existing && durableJobCount >= MAX_JOBS) {
                debugLogger.warn(`Durable task ${task.id} skipped — durable cap (${MAX_JOBS}) reached.`);
                continue;
            }
        ...
        if (finalTasks.length > 0) {
            this.fireOrBuffer({
                kind: 'final',
                jobs: finalTasks.map((t) => durableTaskToJob(t, this.recurringMaxAgeMs)),
            });
        }
    }
    /**
        ...
        switch (pending.kind) {
            case 'missed': {
                ...
                const runnable = pending.tasks.filter((t) => {
                    ...
                    if (job.durable && this.skipDurableFire?.(job)) {
                        debugLogger.debug(`Skipping durable job ${t.id} (missed): consumer cannot run it`);
                        skipped.push(t.id);
                        return false;
                    }
                    return true;
                ...
                if (runnable.length > 0) {
                    ...
                    const carrier = durableTaskToJob(runnable[0], this.recurringMaxAgeMs);
                    onFire({
                        ...carrier,
            ...
            case 'catch-up': {
                ...
                for (const id of pending.ids) {
                    ...
                    // owning session.
                    if (job.durable && this.skipDurableFire?.(job)) {
                        debugLogger.debug(`Skipping durable job ${job.id} (catch-up): consumer cannot run it`);
                        continue;
                    }
                    onFire(job);
                    fired.push(id);
            ...
            case 'final': {
                ...
                for (const job of pending.jobs) {
                    ...
                    if (job.durable && this.skipDurableFire?.(job)) {
                        debugLogger.debug(`Skipping durable job ${job.id} (final): consumer cannot run it`);
    ...
    persistCatchUpStamps(ids) {
        if (!this.projectRoot || ids.length === 0)
            return;
        ...
        this.trackPersist(updateCronTasks(this.projectRoot, (tasks) => {
            let changed = false;
     ...
     */
    trackPersist(write) {
        const settled = write.then(() => { }, (err) => {
            debugLogger.warn(`Durable cron persist failed — disk state is stale and the task may fire again in a later session: ${err}`);
        });
    ...
    startFileWatcher(projectRoot) {
        ...
        try {
            ...
            this.fileWatcher = fsSync.watch(dir, { persistent: false }, (_event, filename) => {
            ...
            this.fileWatcher.on('error', (err) => {
                debugLogger.warn(`Tasks-file watcher error — durable task changes from other sessions may not be picked up until restart: ${err}`);
            });
        }
        catch {
    ...
    stop() {
        ...
        for (const timer of this.testFireTimers.values())
            clearTimeout(timer);
        ...
        if (this.durableEnabled) {
            ...
            if (this.isOwner && this.projectRoot && this.sessionId) {
                ...
                this.pendingRelease = this.pendingPersist.then(() => releaseLock(projectRoot, sessionId, this.lockId));
            }
            this.isOwner = false;
            this.durableEnabled = false;
            ...
            for (const pending of this.pendingFires) {
                if (pending.kind === 'missed') {
                    for (const t of pending.tasks)
                        this.pendingRemoval.delete(t.id);
                }
                else if (pending.kind === 'final') {
                    for (const j of pending.jobs)
                        this.pendingRemoval.delete(j.id);
                }
            }
    ...
    tick(now) {
        ...
        if (this.jobs.size === 0 && this.wakeups.size === 0)
            return;
        const currentDate = now ?? new Date();
        const currentMs = currentDate.getTime();
        const firedAt = new Map(); // durable recurring fires
        const removedIds = []; // durable one-shot fires / expiries
        for (const job of this.jobs.values()) {
            ...
            if (job.durable && !this.#shouldFireDurable(job))
                continue;
            ...
            if (job.durable && this.skipDurableFire?.(job)) {
                debugLogger.debug(`Skipping durable job ${job.id} (tick): consumer cannot run it`);
                continue;
            }
            const result = this.processJob(job, currentDate, currentMs);
            if (!job.durable || result === 'none')
                continue;
        ...
        if (this.projectRoot && (firedAt.size > 0 || removedIds.length > 0)) {
            ...
            this.markFirePersistPending(guarded);
            this.trackPersist(updateCronTasks(this.projectRoot, (tasks) => tasks
    ...
    getExitSummary() {
        ...
        if (sessionJobs.length === 0 && wakeups.length === 0)
            return null;
        ...
        for (const wakeup of wakeups) {
            lines.push(`  - [${wakeup.id}] wakeup at ${new Date(wakeup.fireAtMs).toISOString()}: ${truncatePrompt(wakeup.prompt)}`);
        }
        return lines.join('\n');
    }
    /**
 ...
 */
export function buildMissedCronNotification(missed) {
    const plural = missed.length > 1;
    const header = `The following one-shot scheduled task${plural ? 's were' : ' was'} missed while LailatulCoder Ai was not running. ` +
...
const nextDurableFireCache = new Map();
const NEXT_DURABLE_FIRE_CACHE_MAX = 512;
export function nextDurableFireMs(task) {
