...
import { AuthType, qwenOAuth2Events, QwenOAuth2Event, } from '@lailatul-coder/lailatul-coder-core';
...
describe('useQwenAuth', () => {
    ...
    it('should initialize with default state when not Qwen auth', () => {
        const { result } = renderHook(() => useQwenAuth(AuthType.USE_GEMINI, false));
    ...
    it('should initialize with default state when Qwen auth but not authenticating', () => {
        const { result } = renderHook(() => useQwenAuth(AuthType.QWEN_OAUTH, false));
        expect(result.current.qwenAuthState).toEqual({
            deviceAuth: null,
            authStatus: 'idle',
            authMessage: null,
    ...
    it('should set up event listeners when Qwen auth and authenticating', () => {
        ...
        expect(mockQwenOAuth2Events.on).toHaveBeenCalledWith(QwenOAuth2Event.AuthUri, expect.any(Function));
        expect(mockQwenOAuth2Events.on).toHaveBeenCalledWith(QwenOAuth2Event.AuthProgress, expect.any(Function));
    });
    ...
    it('should handle auth progress event - success', () => {
        ...
        expect(result.current.qwenAuthState.authMessage).toBe('Authentication successful!');
    });
    it('should handle auth progress event - error', () => {
        ...
        expect(result.current.qwenAuthState.authMessage).toBe('Authentication failed');
    });
    it('should handle auth progress event - polling', () => {
        ...
        expect(result.current.qwenAuthState.authStatus).toBe('polling');
        expect(result.current.qwenAuthState.authMessage).toBe('Waiting for user authorization...');
    });
    it('should handle auth progress event - rate_limit', () => {
        ...
        expect(result.current.qwenAuthState.authMessage).toBe('Too many requests. The server is rate limiting our requests. Please select a different authentication method or try again later.');
    });
    ...
    it('should clean up event listeners when auth type changes', () => {
        const { rerender } = renderHook(({ pendingAuthType, isAuthenticating }) => useQwenAuth(pendingAuthType, isAuthenticating), {
            initialProps: {
        ...
        expect(mockQwenOAuth2Events.off).toHaveBeenCalledWith(QwenOAuth2Event.AuthUri, expect.any(Function));
        expect(mockQwenOAuth2Events.off).toHaveBeenCalledWith(QwenOAuth2Event.AuthProgress, expect.any(Function));
    });
    it('should clean up event listeners when authentication stops', () => {
        const { rerender } = renderHook(({ isAuthenticating }) => useQwenAuth(AuthType.QWEN_OAUTH, isAuthenticating), { initialProps: { isAuthenticating: true } });
        // Stop authentication
    ...
    it('should clean up event listeners on unmount', () => {
        const { unmount } = renderHook(() => useQwenAuth(AuthType.QWEN_OAUTH, true));
        unmount();
        expect(mockQwenOAuth2Events.off).toHaveBeenCalledWith(QwenOAuth2Event.AuthUri, expect.any(Function));
        expect(mockQwenOAuth2Events.off).toHaveBeenCalledWith(QwenOAuth2Event.AuthProgress, expect.any(Function));
    });
    it('should reset state when switching from Qwen auth to another auth type', () => {
        ...
        const { result, rerender } = renderHook(({ pendingAuthType, isAuthenticating }) => useQwenAuth(pendingAuthType, isAuthenticating), {
            initialProps: {
                pendingAuthType: AuthType.QWEN_OAUTH,
                isAuthenticating: true,
            },
    ...
    it('should reset state when authentication stops', () => {
        ...
        const { result, rerender } = renderHook(({ isAuthenticating }) => useQwenAuth(AuthType.QWEN_OAUTH, isAuthenticating), { initialProps: { isAuthenticating: true } });
        // Simulate device auth
        act(() => {
            handleDeviceAuth(mockDeviceAuth);
        });
    ...
    it('should handle different auth types correctly', () => {
        ...
        const { result: qwenResult } = renderHook(() => useQwenAuth(AuthType.QWEN_OAUTH, true));
        ...
        const { result: geminiResult } = renderHook(() => useQwenAuth(AuthType.USE_GEMINI, true));
        expect(geminiResult.current.qwenAuthState.authStatus).toBe('idle');
        const { result: oauthResult } = renderHook(() => useQwenAuth(AuthType.USE_OPENAI, true));
...
//# sourceMappingURL=useQwenAuth.test.js.map