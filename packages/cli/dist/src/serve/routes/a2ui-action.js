 ...
 *  2. fallback: `mcpServers` in the workspace `.lailatulcoder/settings.json` (when the
...
export async function findFromSettingsFile(workspaceCwd) {
    try {
        const raw = await fsp.readFile(path.join(workspaceCwd, '.lailatulcoder', 'settings.json'), 'utf8');
...
export async function callA2uiAction(cfg, args, env) {
    ...
    const client = new Client({ name: 'LailatulCoder-serve-a2ui', version: '0.0.1' });
    try {
