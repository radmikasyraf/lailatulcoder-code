...
import { registerWorkspaceGitDiffRoutes, registerWorkspaceQualifiedGitDiffRoutes, } from './workspace-git-diff.js';
...
function runtime(workspaceId, workspaceCwd, trusted) {
    return {
        workspaceId,
        workspaceCwd,
...
describe('workspace Git diff routes', () => {
    ...
    it('returns single-file hunks for the bound workspace', async () => {
        ...
        const response = await request(app).get('/workspace/git/diff/file?path=src/a.ts');
        expect(response.status).toBe(200);
        ...
        expect(fetchGitDiffHunksForFileMock).toHaveBeenCalledWith('/work/main', 'src/a.ts', undefined);
    });
    it('forwards the oldPath query to fetchGitDiffHunksForFile', async () => {
        ...
        const response = await request(app).get('/workspace/git/diff/file?path=src/new.ts&oldPath=src/old.ts');
        expect(response.status).toBe(200);
        ...
        expect(fetchGitDiffHunksForFileMock).toHaveBeenCalledWith('/work/main', 'src/new.ts', 'src/old.ts');
    });
    ...
    it('surfaces the truncated flag when the diff was capped', async () => {
        ...
        const response = await request(app).get('/workspace/git/diff/file?path=big.txt');
        expect(response.status).toBe(200);
    ...
    it('uses the selected trusted workspace runtime for the file route', async () => {
        ...
        const response = await request(app).get('/workspaces/secondary/git/diff/file?path=b.ts');
        expect(response.status).toBe(200);
        expect(fetchGitDiffHunksForFileMock).toHaveBeenCalledWith('/work/secondary', 'b.ts', undefined);
    });
    ...
    it('rejects an untrusted workspace on the single-file endpoint too', async () => {
        ...
        const response = await request(app).get('/workspaces/untrusted/git/diff/file?path=a.ts');
        expect(response.status).toBe(403);
        expect(response.body.code).toBe('untrusted_workspace');
        expect(fetchGitDiffHunksForFileMock).not.toHaveBeenCalled();
    });
...
//# sourceMappingURL=workspace-git-diff.test.js.map