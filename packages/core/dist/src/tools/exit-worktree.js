...
import { GitWorktreeService, readWorktreeSessionMarker, worktreeBranchForSlug, } from '../services/gitWorktreeService.js';
import { readWorktreeSession, clearWorktreeSession, isSessionRuntimeActive, } from '../services/worktreeSessionService.js';
...
class ExitWorktreeInvocation extends BaseToolInvocation {
    ...
    async getConfirmationDetails(_abortSignal) {
        ...
        const command = `git worktree remove ${worktreePath}` + ` && git branch -d ${branch}`;
        const details = {
            type: 'exec',
    ...
    async execute(_signal) {
        ...
        // make exit_worktree look at `<subdir>/.lailatulcoder/worktrees/<slug>`,
        ...
        let service = projectRoot === cwd ? probe : new GitWorktreeService(projectRoot);
        ...
        try {
            ...
            return errorResult(`Cannot access worktree at ${worktreePath} (${error instanceof Error ? error.message : String(error)}).`);
        }
        if (!exists) {
        ...
        if (!exists) {
            return errorResult(`Worktree "${this.params.name}" not found at ${worktreePath}.`);
        }
        if (this.params.action === 'keep') {
            ...
            const output = {
                ...
                message: `Kept worktree "${this.params.name}" at ${worktreePath}. ` +
        ...
        //    `.lailatulcoder/worktrees/` and call `exit_worktree` with a name
        ...
        if (owner !== null && owner !== currentSessionId) {
            ...
            const currentSessionOwnsPath = currentWorktreeSession?.slug === this.params.name &&
                samePath(currentWorktreeSession.worktreePath, worktreePath);
            const ownerActive = await isSessionRuntimeActive(owner, currentWorktreeSession
                ? [
                    currentWorktreeSession.originalCwd,
                    currentWorktreeSession.worktreePath,
                ]
                : [ownerProjectRoot, worktreePath]).catch((error) => {
                debugLogger.warn(`exit_worktree: failed to check owner runtime ${owner}: ${error}`);
                return true;
            });
            if (ownerActive ||
                (!currentSessionOwnsPath && !currentSessionInWorktree)) {
                return errorResult(`Refusing to remove worktree "${this.params.name}" — it was ` +
                    `created by a different session (owner=${owner}). Resume the ` +
                    `owning session to drop it, or remove it manually with ` +
                    `\`git worktree remove ${worktreePath}\`.`);
            }
            debugLogger.warn(`exit_worktree: allowing session ${currentSessionId} to remove ` +
                `${worktreePath}; stale marker owner=${owner}`);
        }
        if (owner === null) {
            debugLogger.warn(`exit_worktree: worktree ${worktreePath} has no session marker; ` +
        ...
        if (!this.params.discard_changes) {
            ...
            if (counts === null) {
                ...
                return errorResult(`Cannot inspect worktree "${this.params.name}" — git status failed against ${worktreePath}. ` +
            ...
            if (total > 0) {
                return errorResult(`Refusing to remove worktree "${this.params.name}" — it has ` +
        ...
        try {
            ...
            debugLogger.warn(`exit_worktree: hasUnmergedWorktreeCommits failed for ${branch}: ${error}`);
        }
        if (hasUnmerged) {
            return errorResult(`Refusing to remove worktree "${this.params.name}" — its branch ` +
        ...
        if (result.branchPreserved) {
            ...
            const output = {
                ...
                worktreePath,
                worktreeBranch: branch,
                message: `Removed worktree directory "${this.params.name}" but kept branch ${branch} ` +
        ...
        debugLogger.debug(`Removed user worktree: ${worktreePath} (branch=${branch})`);
        await this.maybeClearWorktreeSession();
        const output = {
            action: 'remove',
            worktreePath,
    ...
    async maybeClearWorktreeSession() {
        try {
            ...
            debugLogger.warn(`exit_worktree: failed to clear WorktreeSession sidecar: ${error}`);
        }
    }
    async readCurrentWorktreeSession() {
        try {
            const sessionPath = this.config
                .getSessionService()
                .getWorktreeSessionPath(this.config.getSessionId());
            ...
            debugLogger.warn(`exit_worktree: failed to read current WorktreeSession sidecar: ${error}`);
            return null;
        }
    }
    async resolveCurrentSessionWorktree() {
        ...
        try {
            ...
            debugLogger.warn(`exit_worktree: cannot stat sidecar worktree ${session.worktreePath}: ${error}`);
            return null;
        }
        return {
            service,
...
async function resolveManagedWorktreeFromCwd(cwd, slug) {
    let cursor = path.resolve(cwd);
    while (true) {
        if (path.basename(cursor) === slug &&
            path.basename(path.dirname(cursor)) === 'worktrees' &&
            path.basename(path.dirname(path.dirname(cursor))) === '.lailatulcoder') {
...
export class ExitWorktreeTool extends BaseDeclarativeTool {
    config;
    static Name = ToolNames.EXIT_WORKTREE;
    constructor(config) {
        super(ExitWorktreeTool.Name, ToolDisplayNames.EXIT_WORKTREE, exitWorktreeDescription, Kind.Other, {
            type: 'object',
            properties: {
                name: {
                    type: 'string',
    ...
    validateToolParams(params) {
        ...
        if (params.discard_changes !== undefined &&
