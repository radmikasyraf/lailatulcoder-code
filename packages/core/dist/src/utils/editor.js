...
export function isValidEditorType(editor) {
    return [
        'vscode',
        'vscodium',
        'windsurf',
 ...
 */
export function getEditorExecutable(editorType) {
 ...
 */
export function getExternalEditorCommand(editorType, filePath) {
...
export function getDiffCommand(oldPath, newPath, editor) {
    ...
    switch (editor) {
        ...
        case 'emacs': {
            ...
            const toElispString = (p) => p.replace(/\\/g, '\\\\').replace(/"/g, '\\"');
            return {
                command: 'emacs',
                args: [
                    '--eval',
