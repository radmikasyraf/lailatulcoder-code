 ...
 */
import { OutputFormat, JsonFormatter, parseAndFormatApiError, FatalTurnLimitedError, FatalCancellationError, FatalBudgetExceededError, ToolErrorType, createDebugLogger, } from '@lailatul-coder/lailatul-coder-core';
...
export function _resetExitLatchForTest() {
    exiting = false;
}
/**
 ...
 */
export async function handleError(error, config, customErrorCode) {
...
export function handleToolError(toolName, toolError, config, errorCode, resultDisplay) {
    ...
    if (isExecutionDenied && isNonInteractive && isTextMode) {
        const warningMessage = `Warning: Tool "${toolName}" requires user approval but cannot execute in non-interactive mode.\n` +
            ...
            `Example: LailatulCoder -p 'your prompt' -y\n\n`;
 ...
 */
export async function handleCancellationError(config) {
 ...
 */
export async function handleMaxTurnsExceededError(config) {
 ...
 */
export async function handleBudgetExceededError(config, exceeded) {
