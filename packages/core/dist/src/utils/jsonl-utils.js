 ...
 */
function recoverObjectsFromLine(line) {
    const out = [];
    let depth = 0;
    let inString = false;
    let escape = false;
    ...
    return {
        records: out,
        complete: complete && out.length > 0 && depth === 0 && !inString && start === -1,
    };
}
export function _recoverObjectsFromLine(line) {
    return recoverObjectsFromLine(line).records;
}
/**
 ...
 */
function parseLineTolerantWithIntegrity(line, filePath) {
    try {
        const parsed = JSON.parse(line);
        if (parsed !== null &&
            typeof parsed === 'object' &&
            !Array.isArray(parsed)) {
            return { records: [parsed], complete: true };
        }
        debugLogger.warn(`Skipping non-object JSONL value in ${filePath}`);
        return { records: [], complete: false };
    }
    catch {
        const recovered = recoverObjectsFromLine(line);
        if (recovered.records.length === 0) {
            debugLogger.warn(`Failed to parse line in ${filePath}`);
        }
        else {
            debugLogger.warn(`Recovered ${recovered.records.length} record(s) from malformed line in ${filePath}`);
        }
        return recovered;
    }
}
export function parseLineTolerant(line, filePath) {
    return parseLineTolerantWithIntegrity(line, filePath).records;
}
...
async function readLinesWithIntegrityInternal(filePath, count, options = {}, budget = 'records') {
    let fileStream;
    let rl;
    try {
        ...
        for await (const line of rl) {
            if ((budget === 'records' && results.length >= count) ||
            ...
            if (trimmed.length === 0)
                continue;
            scannedLines++;
            const parsed = parseLineTolerantWithIntegrity(trimmed, filePath);
        ...
        if (error.code !== 'ENOENT') {
            debugLogger.error(`Error reading up to ${count} ${budget} from ${filePath}:`, error);
        }
        return { records: [], complete: false };
    }
    finally {
...
export async function readLines(filePath, count, options = {}) {
    // The slice preserves this reader's record-budget contract: at most `count`
    // records even when a glued line recovers several.
    return (await readLinesWithIntegrityInternal(filePath, count, options)).records.slice(0, count);
}
/**
 ...
 */
export async function readLinesWithIntegrity(filePath, count, options = {}) {
    return readLinesWithIntegrityInternal(filePath, count, options, 'lines');
}
/**
 * Reads all lines from a JSONL file.
 * Returns an array of parsed objects.
 */
export async function read(filePath, options = {}) {
    let fileStream;
    let rl;
    try {
 ...
 */
export function _resetEnsuredDirsCacheForTest() {
    ensuredDirs.clear();
}
/**
 ...
 */
export async function writeLine(filePath, data) {
 ...
 * strings.
 */
export function writeLineSync(filePath, data) {
    const line = `${JSON.stringify(data)}\n`;
