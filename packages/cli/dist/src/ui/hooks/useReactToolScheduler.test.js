...
import { mapToDisplay, useReactToolScheduler, } from './useReactToolScheduler.js';
...
const makeCompleted = (status, displayName, responseMedia = []) => ({
    status,
...
const makeSuccess = (displayName, responseMedia = []) => makeCompleted('success', displayName, responseMedia);
describe('mapToDisplay — detailedDisplay (§4.9 live path)', () => {
    ...
    it.each(['success', 'error', 'cancelled'])('extracts nested inline images from %s tool response parts', (status) => {
        const group = mapToDisplay(makeCompleted(status, 'Read File', [
            {
                inlineData: {
                    data: 'dG9vbC1pbWFnZQ==',
                    mimeType: 'image/png',
    ...
    it('caps tool images and reports the overflow count', () => {
        ...
        const tool = mapToDisplay(makeCompleted('success', 'Read File', images))
            .tools[0];
        expect(tool.images).toEqual(images
...
describe('useReactToolScheduler', () => {
    it('handles a queued tool cancellation at the fire-and-forget boundary', async () => {
        ...
        const { result } = renderHook(() => useReactToolScheduler(vi.fn(), { getToolRegistry: () => ({}) }, () => undefined, vi.fn()));
        act(() => {
            result.current[1]({
                callId: 'queued-call',
                name: 'read_file',
