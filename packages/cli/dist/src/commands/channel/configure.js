...
import { loadAccount, saveAccount, clearAccount, DEFAULT_BASE_URL, } from '@lailatul-coder/channel-weixin/accounts';
import { startLogin, waitForLogin } from '@lailatul-coder/channel-weixin/login';
export const configureWeixinCommand = {
    ...
    handler: async (argv) => {
        ...
        if (existing) {
            writeStdoutLine(`Existing WeChat credentials found (saved ${existing.savedAt}).`);
        ...
        try {
            ...
            if (result.connected && result.token) {
                ...
                writeStdoutLine('\n' + result.message);
                writeStdoutLine('Credentials saved. You can now start a weixin channel with:');
                writeStdoutLine('  LailatulCoder channel start <name>');
            }
            else {
                writeStderrLine('\n' + result.message);
                process.exit(1);
            ...
            writeStderrLine(`Login failed: ${err instanceof Error ? err.message : String(err)}`);
            process.exit(1);
        }
    },
};
