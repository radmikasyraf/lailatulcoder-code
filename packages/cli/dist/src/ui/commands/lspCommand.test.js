...
const createConfig = ({ enabled = true, client, }) => ({
...
describe('lspCommand', () => {
    ...
    it('returns guidance when LSP is enabled but no client is connected', async () => {
        ...
        const context = createMockCommandContext({
            executionMode: 'non_interactive',
            services: {
                config: createConfig({ client: undefined }),
            },
        });
        const result = await lspCommand.action(context, '');
        expect(result).toMatchObject({
    ...
    it('renders mixed server status without emoji in non-interactive mode', async () => {
        ...
        expect(result.content).toContain('| pyright | `pyright-langserver` | python | FAILED - startup failed |');
    ...
    it('returns server status as a message in ACP mode', async () => {
        ...
        expect(result).toMatchObject({
            type: 'message',
            messageType: 'info',
            content: expect.stringContaining('| tsserver | `typescript-language-server` | typescript | READY |'),
        });
        expect(context.ui.addItem).not.toHaveBeenCalled();
    });
...
//# sourceMappingURL=lspCommand.test.js.map