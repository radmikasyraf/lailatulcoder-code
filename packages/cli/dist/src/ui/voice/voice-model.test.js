...
import { isSelectableVoiceModel, isTranscribableVoiceModel, } from './voice-model.js';
...
describe('voice model guards', () => {
    ...
    it('isTranscribableVoiceModel rejects runtime models and empty baseUrls', () => {
        expect(isTranscribableVoiceModel(model({ isRuntimeModel: true }))).toBe(false);
    ...
    it('isSelectableVoiceModel accepts ids with a real ASR transport', () => {
        ...
        expect(isSelectableVoiceModel(model({ id: 'qwen3-asr-flash-realtime' }))).toBe(true);
    });
    it('isSelectableVoiceModel rejects ids with no ASR transport', () => {
        ...
        expect(isSelectableVoiceModel(model({ id: 'qwen3-asr-flash-filetrans' }))).toBe(false);
    });
});
//# sourceMappingURL=voice-model.test.js.map