 ...
 */
import { DEFAULT_IMAGE_TOKEN_ESTIMATE, TOKEN_TO_CHAR_RATIO, estimateContentChars, } from './compactionInputSlimming.js';
/**
 ...
 */
export function estimateContentTokens(contents, imageTokenEstimate = DEFAULT_IMAGE_TOKEN_ESTIMATE) {
    let totalChars = 0;
...
export function estimatePromptTokens(history, userMessage, lastPromptTokenCount, lastOutputTokenCount = 0, imageTokenEstimate = DEFAULT_IMAGE_TOKEN_ESTIMATE, conservative = false) {
    if (lastPromptTokenCount > 0) {
        const newContentTokens = estimateContentTokens([userMessage], imageTokenEstimate);
...
export function getUsageOutputTokenCountForPromptEstimate(usage) {
