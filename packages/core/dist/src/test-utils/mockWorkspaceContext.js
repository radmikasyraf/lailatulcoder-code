...
import { isPathWithinRoot, resolveWorkspacePath, } from '../utils/workspaceContext.js';
/**
...
export function createMockWorkspaceContext(rootDir, additionalDirs = []) {
    ...
    const mockWorkspaceContext = {
        ...
        isPathWithinWorkspace: vi.fn().mockImplementation((path) => {
            try {
                const canonicalPath = canonicalizeForContainment(path);
                return allDirs.some((dir) => {
                    try {
                        return isPathWithinRoot(canonicalPath, canonicalizeForContainment(dir));
                    }
                    catch {
                        return false;
                    }
