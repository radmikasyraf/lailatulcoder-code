    ...
    const persistChanges = useCallback(async () => {
        if (!settings.isTrusted) {
            addItem({
                ...
                text: t('Workspace is untrusted; workspace settings are ignored by the merged config. Run /trust first to persist skills changes here, or edit ~/.lailatulcoder/settings.json directly to manage skills at user scope.'),
            }, Date.now());
            return 'untrusted';
        }
