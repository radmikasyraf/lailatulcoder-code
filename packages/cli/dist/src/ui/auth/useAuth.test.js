...
import { AuthType, deepseekProvider, openRouterProvider, tokenPlanProvider, customProvider, generateCustomEnvKey as generateCustomApiKeyEnvKey, getDefaultModelIds, resolveBaseUrl, } from '@lailatul-coder/lailatul-coder-core';
import { useAuthCommand, normalizeCustomModelIds, maskApiKey, } from './useAuth.js';
...
describe('useAuthCommand', () => {
    ...
    it('exposes closeAuthDialog that flips isAuthDialogOpen to false', () => {
        ...
        const addItem = vi.fn();
        const { result } = renderHook(() => useAuthCommand(settings, config, addItem));
        act(() => {
    ...
    it('configures DeepSeek via the unified provider submit', async () => {
        ...
        expect(settings.setValue).toHaveBeenCalledWith('user', 'env.DEEPSEEK_API_KEY', 'sk-deepseek');
        expect(settings.setValue).toHaveBeenCalledWith('user', 'security.auth.selectedType', 'openai');
        expect(settings.setValue).toHaveBeenCalledWith('user', 'model.name', 'deepseek-v4-flash');
    ...
    it('configures OpenRouter via the unified provider submit', async () => {
        ...
        expect(settings.setValue).toHaveBeenCalledWith('user', 'env.OPENROUTER_API_KEY', 'sk-or-v1-key');
        expect(settings.setValue).toHaveBeenCalledWith('user', 'security.auth.selectedType', 'openai');
        expect(settings.setValue).toHaveBeenCalledWith('user', 'model.name', 'z-ai/glm-4.5-air:free');
        expect(config.refreshAuth).toHaveBeenCalledWith(AuthType.USE_OPENAI);
    });
    it('configures Token Plan with the independent Token Plan endpoint', async () => {
        ...
        expect(settings.setValue).toHaveBeenCalledWith('user', 'env.BAILIAN_TOKEN_PLAN_API_KEY', 'sk-token-plan');
        expect(config.refreshAuth).toHaveBeenCalledWith(AuthType.USE_OPENAI);
    });
    it('configures Custom API Key via the provider install plan flow', async () => {
        const envKey = generateCustomApiKeyEnvKey(AuthType.USE_OPENAI, 'https://api.example.com/v1');
        const settings = createSettings();
        const config = createConfig();
        const addItem = vi.fn();
        const { result } = renderHook(() => useAuthCommand(settings, config, addItem));
        ...
        expect(settings.setValue).toHaveBeenCalledWith('user', `env.${envKey}`, 'sk-custom');
        expect(settings.setValue).toHaveBeenCalledWith('user', 'security.auth.selectedType', AuthType.USE_OPENAI);
        expect(settings.setValue).toHaveBeenCalledWith('user', 'model.name', 'custom-model');
        expect(config.refreshAuth).toHaveBeenCalledWith(AuthType.USE_OPENAI);
    });
    ...
    it('surfaces install-plan rejection as an auth error and records telemetry', async () => {
        ...
        expect(result.current.authError).toEqual(expect.stringContaining('refreshAuth rejected'));
        expect(result.current.isAuthDialogOpen).toBe(true);
        expect(result.current.isAuthenticating).toBe(false);
...
describe('generateCustomApiKeyEnvKey', () => {
    it('generates deterministic URL-based env key', () => {
        const key = generateCustomApiKeyEnvKey(AuthType.USE_OPENAI, 'https://api.openai.com/v1');
        expect(key).toMatch(/^QWEN_CUSTOM_API_KEY_[A-Z0-9_]+$/);
        const key2 = generateCustomApiKeyEnvKey(AuthType.USE_OPENAI, 'https://api.openai.com/v1');
        expect(key).toBe(key2);
    });
    it('produces different keys for different protocols', () => {
        const key1 = generateCustomApiKeyEnvKey(AuthType.USE_OPENAI, 'https://api.example.com/v1');
        const key2 = generateCustomApiKeyEnvKey(AuthType.USE_ANTHROPIC, 'https://api.example.com/v1');
        expect(key1).not.toBe(key2);
    });
    it('produces different keys for different base URLs', () => {
        const key1 = generateCustomApiKeyEnvKey(AuthType.USE_OPENAI, 'https://api.openai.com/v1');
        const key2 = generateCustomApiKeyEnvKey(AuthType.USE_OPENAI, 'http://localhost:11434/v1');
        expect(key1).not.toBe(key2);
    });
    it('produces equal keys for URLs that differ only in trailing slash', () => {
        const key1 = generateCustomApiKeyEnvKey(AuthType.USE_OPENAI, 'https://openrouter.ai/api/v1/');
        const key2 = generateCustomApiKeyEnvKey(AuthType.USE_OPENAI, 'https://openrouter.ai/api/v1');
...
describe('normalizeCustomModelIds', () => {
    ...
    it('deduplicates while preserving order', () => {
        const result = normalizeCustomModelIds('qwen/qwen3-coder,openai/gpt-4.1,qwen/qwen3-coder');
        expect(result).toEqual(['qwen/qwen3-coder', 'openai/gpt-4.1']);
    });
