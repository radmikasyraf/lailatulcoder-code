...
import { AgentViewAttachLeaseManager, MAX_AGENT_VIEW_ATTACH_LEASE_TTL_MS, } from './attach-lease.js';
describe('AgentViewAttachLeaseManager', () => {
    ...
    it('uses a random lease id when no id factory is provided', () => {
        ...
        if (result.ok) {
            expect(result.lease.leaseId).toMatch(/^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/);
        }
    });
    ...
    it('rejects a second acquire while a lease is active', () => {
        ...
        expect(manager.acquire('session-1', {
            leaseId: 'lease-2',
        })).toEqual({
            ok: false,
    ...
    it('rejects non-positive, oversized, and non-finite ttls', () => {
        const manager = new AgentViewAttachLeaseManager();
        expect(() => manager.acquire('session-1', { ttlMs: 0 })).toThrow('Attach lease ttlMs must be positive.');
        expect(() => manager.acquire('session-1', { ttlMs: Number.NaN })).toThrow(RangeError);
        expect(() => manager.acquire('session-1', {
            ttlMs: MAX_AGENT_VIEW_ATTACH_LEASE_TTL_MS + 1,
        })).toThrow('Attach lease ttlMs must not exceed');
        expect(() => manager.acquire('')).toThrow('Agent View session id is required.');
        expect(manager.get('session-1')).toBeUndefined();
    });
    it('honors per-call ttls and expires lazily through get and release', () => {
        ...
        clock.advance(1500);
        expect(manager.get('session-1')).toMatchObject({ leaseId: 'lease-1' });
        expect(manager.heartbeat('session-1', 'lease-1', { ttlMs: 3000 })).toMatchObject({ expiresAt: '2026-07-17T00:00:04.500Z' });
        clock.advance(3000);
        expect(manager.get('session-1')).toBeUndefined();
    ...
    it('expires corrupted leases with invalid timestamps', () => {
        ...
        manager.acquire('session-1');
        manager.leases.get('session-1').expiresAt = 'not-a-date';
...
//# sourceMappingURL=attach-lease.test.js.map