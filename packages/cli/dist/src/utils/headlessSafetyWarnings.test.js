...
import { HEADLESS_YOLO_NO_SANDBOX_WARNING, getHeadlessYoloSafetyWarning, } from './headlessSafetyWarnings.js';
function makeConfig(approvalMode, sandbox) {
    return {
...
describe('getHeadlessYoloSafetyWarning', () => {
    it('warns when approval mode is YOLO and no sandbox is configured', () => {
        ...
        expect(getHeadlessYoloSafetyWarning(cfg, {})).toBe(HEADLESS_YOLO_NO_SANDBOX_WARNING);
    });
    ...
    it('does not warn when SANDBOX env is set to the value the sandbox transport actually writes', () => {
        const cfg = makeConfig(ApprovalMode.YOLO, undefined);
        // macOS seatbelt
        expect(getHeadlessYoloSafetyWarning(cfg, { SANDBOX: 'sandbox-exec' })).toBeNull();
        // Docker / Podman container name
        expect(getHeadlessYoloSafetyWarning(cfg, { SANDBOX: 'lailatul-coder-sandbox' })).toBeNull();
    ...
    it('warns when SANDBOX env is unset or empty string', () => {
        const cfg = makeConfig(ApprovalMode.YOLO, undefined);
        expect(getHeadlessYoloSafetyWarning(cfg, {})).toBe(HEADLESS_YOLO_NO_SANDBOX_WARNING);
        expect(getHeadlessYoloSafetyWarning(cfg, { SANDBOX: '' })).toBe(HEADLESS_YOLO_NO_SANDBOX_WARNING);
    });
    it('respects the explicit suppression env var when set to 1 or true', () => {
        const cfg = makeConfig(ApprovalMode.YOLO, undefined);
        expect(getHeadlessYoloSafetyWarning(cfg, {
...
//# sourceMappingURL=headlessSafetyWarnings.test.js.map