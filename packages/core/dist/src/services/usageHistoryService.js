...
export function persistSessionUsage(params) {
    ...
    const record = metricsToUsageRecord(sessionId, project, startTime.getTime(), endTime.getTime(), metrics);
    jsonl.writeLineSync(getUsageHistoryPath(), record);
}
export function metricsToUsageRecord(sessionId, project, startTime, endTime, metrics) {
    const models = {};
        ...
        ...(metrics.skills
            ? {
                skills: {
                    ...
                    byName: Object.fromEntries(Object.entries(metrics.skills.byName).map(([name, s]) => [
                        name,
...
function summarizeTranscript(records) {
    ...
    return {
        sessionId,
        record: metricsToUsageRecord(sessionId, project, startTime, endTime, telemetry.getMetrics()),
    };
}
/**
 ...
 * written.
 */
export async function persistUsageBeforeTranscriptDeletion(transcriptPath) {
    try {
        const records = await jsonl.read(transcriptPath);
        const summarized = summarizeTranscript(records);
        if (!summarized?.record)
            return false;
        const usagePath = getUsageHistoryPath();
        try {
            if (fs.existsSync(usagePath)) {
                const existing = await jsonl.read(usagePath);
            ...
            debugLogger.debug(`persistUsageBeforeTranscriptDeletion: cannot read history: ${e}`);
        }
...
async function rebuildFromSessionJsonl(options = {}) {
    ...
    try {
        ...
        debugLogger.debug(`rebuildFromSessionJsonl: cannot access projectsDir: ${e}`);
        return [];
    }
    const results = [];
    ...
    for (const projDir of projectDirs) {
        ...
        try {
            ...
            debugLogger.debug(`rebuildFromSessionJsonl: cannot read chatsDir ${chatsDir}: ${e}`);
            continue;
        }
            ...
            try {
                ...
                if (sinceMs !== undefined) {
                    ...
                    try {
                        ...
                        debugLogger.debug(`rebuildFromSessionJsonl: cannot stat ${filePath}: ${e}`);
                        continue;
                    }
                    if (mtimeMs < sinceMs)
                        continue;
                ...
                if (skipSessionIds && skipSessionIds.size > 0) {
                    ...
                    if (skipSessionIds.has(fileSessionId))
                        continue;
                }
                ...
                if (!summarized)
                    continue;
                if (seenSessionIds.has(summarized.sessionId))
                    continue;
                ...
                debugLogger.debug(`rebuildFromSessionJsonl: failed to process ${file}: ${e}`);
                continue;
            }
        }
    }
    ...
    // `persist: false` so opening the dashboard never writes to `~/.lailatulcoder`.
...
function dedupBySessionId(records) {
    ...
    if (map.size < records.length) {
        debugLogger.debug(`dedupBySessionId: removed ${records.length - map.size} duplicate record(s)`);
    }
    return [...map.values()];
}
export async function loadUsageHistory(skipSessionInRebuild, options) {
    try {
        const records = await jsonl.read(getUsageHistoryPath());
        const filtered = records.filter((r) => r.version === 1);
        if (filtered.length > 0)
            return dedupBySessionId(filtered);
    }
    catch (e) {
        debugLogger.debug(`loadUsageHistory: failed to read usage file: ${e}`);
    }
    return dedupBySessionId(await rebuildFromSessionJsonl({
...
export async function loadUsageHistoryWithLive(options) {
    ...
    try {
        ...
        debugLogger.debug(`loadUsageHistoryWithLive: failed to read usage file: ${e}`);
    }
...
export function aggregateUsage(records, range) {
    ...
    for (const r of filtered) {
        if (!r.models || !r.tools?.byName || !r.files)
            continue;
...
//# sourceMappingURL=usageHistoryService.js.map