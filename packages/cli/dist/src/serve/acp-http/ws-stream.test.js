...
describe('WsStream', () => {
    ...
    it('reports queued sends as closed when the stream is already closed', async () => {
        const stream = new WsStream(ws);
        stream.close();
        await expect(stream.sendSerialized(Buffer.from('{"ok":true}'))).resolves.toBe('closed');
        expect(ws.sent).toEqual([]);
    });
    it('close() marks an active accepted send outcome unknown', async () => {
        const controlled = new ControlledWebSocket();
        const stream = new WsStream(controlled);
    ...
    it('closes after a synchronous send failure when stderr is unavailable', async () => {
        ...
        const stream = new WsStream(ws, onClose);
        try {
            await expect(stream.sendSerialized(Buffer.from('{"fail":true}'))).resolves.toBe('failed');
            expect(stream.isClosed).toBe(true);
            expect(onClose).toHaveBeenCalledTimes(1);
            await expect(stream.sendSerialized(Buffer.from('{"after":"failure"}'))).resolves.toBe('closed');
        }
        finally {
