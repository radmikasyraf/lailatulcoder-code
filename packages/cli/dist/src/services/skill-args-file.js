...
import { mkdirSync, openSync, writeSync, closeSync, rmSync, lstatSync, existsSync, constants, } from 'node:fs';
...
export const SKILL_ARGS_DIR = join('.lailatulcoder', 'tmp');
...
export function currentSessionId() {
    return (sessionIdContext.getStore()?.trim() ||
 ...
 * always `LailatulCoder-skill-args-<skill>.txt` — a stable name the skill prompt and the
 ...
 */
export function skillArgsDir(sessionId = currentSessionId()) {
    return sessionId
 ...
 */
export function skillArgsPath(skillName, sessionId = currentSessionId()) {
    return join(skillArgsDir(sessionId), `LailatulCoder-skill-args-${safe(skillName)}.txt`);
}
/**
 ...
 */
export function writeSkillArgs(skillName, args) {
    ...
    try {
        ...
        // into `victim/LailatulCoder-skill-args-review.txt`, truncating an arbitrary
        ...
        if (lstatSync(dir).isSymbolicLink()) {
            debugLogger.warn(`Skill args directory ${dir} is a symlink; refusing to write through it.`);
            return null;
        }
        if (lstatSync(path, { throwIfNoEntry: false })?.isSymbolicLink()) {
            debugLogger.warn(`Skill args file ${path} is a symlink; refusing to write through it.`);
            return null;
        }
        ...
        debugLogger.warn(`Could not write skill args to ${path}: ${err.message}`);
        return null;
    }
}
/**
 ...
 * were. Never throws.
 */
export function clearSkillArgs(skillName) {
    const path = skillArgsPath(skillName);
    try {
        ...
        debugLogger.warn(`Could not clear skill args for ${skillName}: ${err.message}`);
        return false;
    }
 ...
 */
export function staleArgsWarning() {
 ...
 */
export function skillArgsNote(path, args) {
...
//# sourceMappingURL=skill-args-file.js.map