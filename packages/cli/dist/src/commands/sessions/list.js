...
function outputHuman(items) {
    ...
    const PROMPT_COL = Math.max(20, termWidth - SESSION_COL - TIME_COL - TITLE_COL - BRANCH_COL - 4);
    const header = padDisplay('SESSION ID', SESSION_COL) +
        ' ' +
    ...
    for (const item of items) {
        const sessionId = truncate(sanitize(String(item.sessionId ?? '')), SESSION_COL);
        const time = truncate(sanitize(formatTime(item.startTime)), TIME_COL);
        const sanitizedPrompt = sanitize(item.prompt ?? '');
        const title = truncate(item.customTitle != null ? sanitize(item.customTitle) : sanitizedPrompt, TITLE_COL);
        const branch = truncate(item.gitBranch != null ? sanitize(item.gitBranch) : '-', BRANCH_COL);
        const prompt = truncate(sanitizedPrompt, PROMPT_COL);
        writeStdoutLine(`${padDisplay(sessionId, SESSION_COL)} ${padDisplay(time, TIME_COL)} ${padDisplay(title, TITLE_COL)} ${padDisplay(branch, BRANCH_COL)} ${prompt}`);
    }
}
...
export async function handleList(argv) {
    ...
    try {
        ...
        writeStderrLine(`Error: failed to initialize session service: ${formatError(err)}`);
        process.exit(1);
        return;
    }
    let result;
    ...
    if (argv.json) {
        ...
        // stream, keeping pipelines like `LailatulCoder sessions list --json | jq …` safe.
        ...
        if (result.items.length > 0 && result.hasMore) {
            writeStdoutLine(`Showing ${result.items.length} sessions. Use --limit to show more.`);
        }
    }
}
