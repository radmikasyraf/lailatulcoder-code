...
import { registerWorkspaceGitLogRoutes, registerWorkspaceQualifiedGitLogRoutes, } from './workspace-git-log.js';
...
describe('workspace Git log routes', () => {
    ...
    it('returns commit detail for a valid sha', async () => {
        ...
        const response = await request(app).get(`/workspace/git/log/commit?sha=${ENTRY.shortSha}`);
        expect(response.status).toBe(200);
        expect(response.body).toMatchObject({
            available: true,
            sha: ENTRY.sha,
        ...
        expect(fetchGitCommitDetailMock).toHaveBeenCalledWith('/work/main', ENTRY.shortSha);
    });
    ...
    it('rejects a malformed (non-hex) sha with 400, distinct from a valid miss', async () => {
        ...
        const response = await request(app).get('/workspace/git/log/commit?sha=not-hex');
    ...
    it('reports available=false for a valid sha with no matching commit', async () => {
        ...
        const response = await request(app).get('/workspace/git/log/commit?sha=deadbee');
        expect(response.status).toBe(200);
    ...
    it('returns a structured error when fetching commit detail throws', async () => {
        ...
        const response = await request(app).get('/workspace/git/log/commit?sha=abcdef1');
        expect(response.status).toBe(500);
        expect(response.body).toMatchObject({ error: 'boom' });
    });
    it('rejects an untrusted workspace on the qualified routes', async () => {
        ...
        const detail = await request(app).get('/workspaces/untrusted/git/log/commit?sha=abcdef1');
    ...
    it('uses the selected trusted workspace for qualified commit detail', async () => {
        ...
        const response = await request(app).get('/workspaces/secondary/git/log/commit?sha=abcdef1');
        expect(response.status).toBe(200);
        expect(fetchGitCommitDetailMock).toHaveBeenCalledWith('/work/secondary', 'abcdef1');
    });
});
//# sourceMappingURL=workspace-git-log.test.js.map