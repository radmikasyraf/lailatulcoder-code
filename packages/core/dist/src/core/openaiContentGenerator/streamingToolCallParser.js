...
export class StreamingToolCallParser {
    ...
    namelessToolCallIndices = new Set();
    ...
    addChunk(index, chunk, id, name) {
        ...
        const isNameOnlyDelta = Boolean(validName &&
            chunk.length === 0 &&
        ...
        if (isKnownId && currentDepth === 0) {
            if (currentBuffer.trim()) {
                try {
                    ...
                    debugLogger.debug(`Ignoring replay chunk for completed toolCall id=${id}`);
                    return { actualIndex, complete: false };
                }
                catch {
    ...
    hasInvalidToolCallArguments() {
        for (const [index, buffer] of this.buffers.entries()) {
            if (!this.toolCallMeta.get(index)?.name || buffer.length === 0)
                continue;
            try {
            ...
            if (meta?.name) {
                ...
                if (buffer.trim()) {
                    ...
                    if (typeof args !== 'object' ||
                        args === null ||
                        Array.isArray(args)) {
                        debugLogger.debug(`Collapsing non-object arguments for tool call ${meta.name} (id=${meta.id}) at index ${index}; buffer likely polluted by a misrouted fragment`);
                        args = {};
                    }
                }
                else {
                    debugLogger.debug(`Emitting no-argument tool call ${meta.name} (id=${meta.id}) at index ${index} with empty buffer`);
                }
                completed.push({
                    id: meta.id,
                    name: meta.name,
...
//# sourceMappingURL=streamingToolCallParser.js.map