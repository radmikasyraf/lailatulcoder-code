...
import { deleteWorkspaceSkill, installWorkspaceSkill, } from './workspace-skill-management.js';
...
describe('workspace Skill management', () => {
    it('installs folder files into the workspace and deletes them', async () => {
        ...
        await fs.writeFile(path.join(source, 'SKILL.md'), skillMarkdown('demo-skill'));
        await fs.writeFile(path.join(source, 'references', 'example.md'), 'example');
        const result = await installWorkspaceSkill(workspace, {
            name: 'demo-skill',
            scope: 'workspace',
            source: { type: 'folder', path: source },
        });
        expect(result.installedPath).toBe(path.join(workspace, '.qwen', 'skills', 'demo-skill', 'SKILL.md'));
        expect(await fs.readFile(path.join(workspace, '.qwen', 'skills', 'demo-skill', 'references', 'example.md'), 'utf8')).toBe('example');
        await deleteWorkspaceSkill(workspace, 'workspace', 'demo-skill', result.installedPath);
        await expect(fs.access(path.dirname(result.installedPath))).rejects.toThrow();
    });
    it('does not create Skill directories after the generation closes', async () => {
        ...
        await fs.writeFile(path.join(source, 'SKILL.md'), skillMarkdown('demo-skill'));
        let checks = 0;
        await expect(installWorkspaceSkill(workspace, {
            name: 'demo-skill',
            scope: 'workspace',
            source: { type: 'folder', path: source },
        }, undefined, () => {
    ...
    it('installs a ZIP into the global Skill directory', async () => {
        ...
        expect(result.installedPath).toBe(path.join(globalDirectory, 'skills', 'zip-skill', 'SKILL.md'));
        await expect(fs.readFile(result.installedPath, 'utf8')).resolves.toContain('name: zip-skill');
    });
    it('installs a Skill from a GitHub SKILL.md URL', async () => {
        ...
        await expect(fs.readFile(result.installedPath, 'utf8')).resolves.toContain('name: github-skill');
        expect(fetch).toHaveBeenCalledTimes(2);
        expect(fetch).toHaveBeenNthCalledWith(1, 'https://api.github.com/repos/owner/repo/contents?ref=main', expect.objectContaining({
    ...
    ])('explains GitHub Skill file HTTP %i failures', async (status, expectedMessage) => {
        const workspace = await temporaryDirectory('qwen-skill-workspace-');
        vi.stubGlobal('fetch', vi
            .fn()
            .mockResolvedValueOnce(new Response(JSON.stringify([
    ...
    it.each(['--upload-pack', 'main%20--upload-pack'])('rejects unsafe GitHub ref %s before making a request', async (ref) => {
        const workspace = await temporaryDirectory('qwen-skill-workspace-');
    ...
    it('keeps the existing Skill when replacement validation fails', async () => {
        ...
        await fs.writeFile(path.join(source, 'SKILL.md'), skillMarkdown('stable-skill'));
        await fs.writeFile(path.join(invalidSource, 'SKILL.md'), skillMarkdown('different-name'));
        const validRequest = {
            name: 'stable-skill',
            scope: 'workspace',
            source: { type: 'folder', path: source },
        ...
        await expect(fs.readFile(installed.installedPath, 'utf8')).resolves.toContain('name: stable-skill');
    });
    it('keeps a committed replacement when backup cleanup fails', async () => {
        ...
        await fs.writeFile(path.join(source, 'SKILL.md'), skillMarkdown('stable-skill'));
        await fs.writeFile(path.join(replacement, 'SKILL.md'), `${skillMarkdown('stable-skill')}Replacement instructions.`);
        const request = {
            name: 'stable-skill',
            scope: 'workspace',
            source: { type: 'folder', path: source },
        ...
        await expect(fs.readFile(result.installedPath, 'utf8')).resolves.toContain('Replacement instructions.');
        await expect(fs.readdir(path.join(workspace, '.qwen', 'skills'))).resolves.toEqual(['stable-skill']);
    });
    it('preserves the install error when staging cleanup fails', async () => {
        const workspace = await temporaryDirectory('qwen-skill-workspace-');
        const source = await temporaryDirectory('qwen-skill-source-');
        await fs.writeFile(path.join(source, 'SKILL.md'), skillMarkdown('different-name'));
        vi.spyOn(fs, 'rm').mockRejectedValueOnce(new Error('cleanup failed'));
        await expect(installWorkspaceSkill(workspace, {
            name: 'expected-name',
            scope: 'workspace',
        ...
        await expect(fs.readdir(path.join(workspace, '.qwen', 'skills'))).resolves.toEqual([]);
    });
    it('removes legacy install artifacts before installing', async () => {
        ...
        const staleBackup = path.join(path.dirname(baseDir), '.skills-stable-skill.backup-stale');
        await fs.mkdir(legacyBackup, { recursive: true });
        await fs.mkdir(staleBackup, { recursive: true });
        await fs.writeFile(path.join(legacyBackup, 'SKILL.md'), skillMarkdown('stable-skill'));
        await fs.writeFile(path.join(source, 'SKILL.md'), skillMarkdown('stable-skill'));
    ...
    it('restores the existing Skill when committing a replacement fails', async () => {
        ...
        await expect(fs.readFile(installed.installedPath, 'utf8')).resolves.not.toContain('Replacement instructions.');
    });
...
//# sourceMappingURL=workspace-skill-management.test.js.map