...
export class ModelsConfig {
    ...
    constructor(options = {}) {
        this.modelRegistry = new ModelRegistry(options.modelProvidersConfig, options.providerProtocolConfig);
        ...
        if (this.currentAuthType && initialModelId) {
            const initialModel = this.modelRegistry.getModel(this.currentAuthType, initialModelId, options.initialRegistryBaseUrl);
            if (initialModel) {
    ...
    getAllConfiguredModels(authTypes) {
        ...
        if (uniqueAuthTypes.includes(AuthType.lailatulcoder_OAUTH)) {
            orderedAuthTypes.push(AuthType.lailatulcoder_OAUTH);
        }
    ...
    getModelDisplayName(modelId) {
        if (!this.currentAuthType)
            return modelId;
        const resolved = (this.currentRegistryBaseUrl !== undefined
            ? this.modelRegistry.getModel(this.currentAuthType, modelId, this.currentRegistryBaseUrl)
            : undefined) ??
    ...
    async setModel(newModel, metadata) {
        ...
        if (this.currentAuthType === AuthType.lailatulcoder_OAUTH &&
            ...
            if (this.onModelChange) {
                await this.onModelChange(AuthType.lailatulcoder_OAUTH, false);
            }
            return;
        }
    ...
    async switchModel(authType, modelId, options) {
        ...
        const rollbackSnapshot = this.createStateSnapshotForRollback();
        if (authType === AuthType.lailatulcoder_OAUTH && options?.requireCachedCredentials) {
            this.requireCachedQwenCredentialsOnce = true;
        }
        try {
            const isAuthTypeChange = authType !== this.currentAuthType;
            this.currentAuthType = authType;
            const model = this.modelRegistry.getModel(authType, modelId, options?.baseUrl);
            if (!model) {
                throw new Error(`Model '${modelId}' not found for authType '${authType}'`);
            }
            if (model.imageOnly) {
                throw new Error(`Image-only model '${modelId}' cannot be used as the primary model`);
            }
            const previousModelId = rollbackSnapshot.generationConfig.model || '';
            const previousModel = !isAuthTypeChange && previousModelId
                ? (this.modelRegistry.getModel(authType, previousModelId, rollbackSnapshot.currentRegistryBaseUrl) ?? this.modelRegistry.getModel(authType, previousModelId))
                : undefined;
            const canReusePreviousApiKey = authType !== AuthType.lailatulcoder_OAUTH &&
                !isAuthTypeChange &&
    ...
    applyResolvedModelDefaults(model) {
        ...
        if (model.envKey !== undefined) {
            ...
            if (apiKey) {
                ...
                debugLogger.debug(`No API key found for model "${model.id}": ` +
    ...
    syncAfterAuthRefresh(authType, modelId, providerBaseUrlOverride) {
        ...
        const providerBaseUrl = providerBaseUrlOverride ??
        ...
        if (resolved) {
            ...
            const shouldPreserveResolvedBaseUrl = isUnchanged &&
                !!this._generationConfig.baseUrl &&
    ...
    reloadModelProvidersConfig(modelProvidersConfig, providerProtocolConfig) {
        this.modelRegistry.reloadModels(modelProvidersConfig, providerProtocolConfig);
    }
}
