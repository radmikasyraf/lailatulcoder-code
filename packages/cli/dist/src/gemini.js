 ...
 */
import { AuthType, InputFormat, isDebugLogFileEnabled, isDebugLoggingDegraded, isBareMode, logUserPrompt, QWEN_CODE_SIMPLE_ENV_VAR, Storage, SessionService, setStartupEventSink, createDebugLogger, persistSessionUsage, PRIVATE_ACP_CAPABILITY_ENV, uiTelemetryService, } from '@lailatul-coder/lailatul-coder-core';
...
import { buildDisabledSkillNamesProvider, loadCliConfig, parseArguments, } from './config/config.js';
import { ENV_CORRUPTED_PATH, ENV_WAS_RECOVERED, createMinimalSettings, getSettingsWarnings, loadSettings, preResolveHomeEnvOverrides, } from './config/settings.js';
...
import { setupStartupWorktree, persistStartupWorktreeSidecar, buildStartupWorktreeNotice, } from './startup/worktreeStartup.js';
...
import { profileCheckpoint, recordStartupEvent, setInteractiveMode, finalizeStartupProfile, isStartupProfilerEnabled, } from './utils/startupProfiler.js';
import { isAcpStartupProfilerEnabled, markAcpStartup, recordAcpConfigStartupEvent, } from './utils/acp-startup-profiler.js';
...
import { CUSTOM_SANDBOX_IMAGE_ENV_VAR, HOST_UPDATE_RELAUNCH_ENV_VAR, UPDATE_COMPLETE_EXIT_CODE, } from './utils/processUtils.js';
...
export function validateDnsResolutionOrder(order) {
    ...
    writeStderrLine(`Invalid value for dnsResolutionOrder in settings: "${order}". Using default "${defaultValue}".`);
    return defaultValue;
}
function getNodeMemoryArgs(isDebugMode) {
    ...
    if (targetMaxOldSpaceSizeInMB > currentMaxOldSpaceSizeMb) {
        if (isDebugMode) {
            writeStderrLine(`Need to relaunch with more memory: ${targetMaxOldSpaceSizeInMB.toFixed(2)} MB`);
        }
...
import { handleUncaughtException, isExpectedPtyRaceError, } from './utils/uncaught-exception-handler.js';
let uncaughtExceptionHandler;
export function setupUncaughtExceptionHandler(config) {
...
function installInteractiveSignalHandlers(wasRaw) {
...
export async function main() {
    ...
    if (startupProfilerEnabled || acpStartupProfilerEnabled) {
        setStartupEventSink((name, attrs) => {
            if (startupProfilerEnabled)
                recordStartupEvent(name, attrs);
            if (acpStartupProfilerEnabled)
                recordAcpConfigStartupEvent(name);
        });
    }
    ...
    const privateExternalToolGuard = process.env[PRIVATE_EXTERNAL_TOOL_GUARD_ENV] ===
    ...
    const privateAcpChildEnv = isAcpMode && privateAcpParentCapability !== undefined
        ? {
            ...
            ...(privateExternalToolGuard
                ? {
                    ...
                    ...(privateExternalToolGuardProvider
                        ? {
                            [PRIVATE_EXTERNAL_TOOL_GUARD_PROVIDER_ENV]: privateExternalToolGuardProvider,
                        }
                        : {}),
                }
                : {}),
    ...
    if (settings.corruptedPath) {
        writeStderrLine('Warning: Settings file had invalid JSON and was reset. ' +
    ...
    if (argv.listExtensions) {
        ...
        const { handleList: handleListExtensions } = await import('./commands/extensions/list.js');
        await handleListExtensions();
        process.exit(0);
    }
    // Check for invalid input combinations early to prevent crashes
    if (argv.promptInteractive && !process.stdin.isTTY) {
        writeStderrLine('Error: The --prompt-interactive flag cannot be used when input is piped from stdin.');
        process.exit(1);
    }
    const isDebugMode = cliConfig.isDebugMode(argv);
    dns.setDefaultResultOrder(validateDnsResolutionOrder(settings.merged.advanced?.dnsResolutionOrder));
    const { themeManager, AUTO_THEME_NAME } = await import('./ui/themes/theme-manager.js');
    ...
    if (!process.env['SANDBOX']) {
        ...
        const onUpdateRelaunch = async (relaunchOnFailure) => {
            await initializeI18n(resolveLanguageSetting(settings.merged.general?.language));
            const { updateBeforeRelaunch } = await import('./utils/update-relaunch.js');
            const shouldRelaunch = await updateBeforeRelaunch(settings, updateProjectRoot, relaunchOnFailure);
            return shouldRelaunch ? UPDATE_COMPLETE_EXIT_CODE : 0;
        };
        const sandboxConfig = await loadSandboxConfig(settings.merged, argv);
        const customSandboxImage = argv.sandboxImage ??
        ...
        if (sandboxConfig &&
            ...
            process.env[HOST_UPDATE_RELAUNCH_ENV_VAR] = String(Boolean(hostInstallationInfo.updateCommand ||
        ...
        if (sandboxConfig) {
            ...
            const injectSandboxSessionIdIntoArgs = (args, sessionId) => {
                const separatorIndex = args.indexOf('--');
                const cliArgs = separatorIndex < 0 ? args : args.slice(0, separatorIndex);
                const hasArg = (names) => cliArgs.some((arg) => names.some((name) => arg === name || arg.startsWith(`${name}=`)));
                if (hasArg(['--session-id', '--sandbox-session-id']) ||
                    hasArg(['--continue', '-c']) ||
                    hasArg(['--resume', '-r'])) {
                    return args;
                }
            ...
            const sandboxArgs = sessionId
                ? injectSandboxSessionIdIntoArgs(injectStdinIntoArgs(process.argv, stdinData), sessionId)
                : injectStdinIntoArgs(process.argv, stdinData);
            await relaunchOnExitCode(() => start_sandbox(sandboxConfig, memoryArgs, partialConfig, sandboxArgs, privateAcpChildEnv), {
                onUpdateRelaunch,
            });
            process.exit(0);
        }
    ...
    if (isAcpMode && process.env[QWEN_CODE_SERVE_ENV] === '1') {
        ...
        scrubAndReportInheritedLoaderEnv(process.env, 'LailatulCoder', 'ACP child');
    }
    ...
    if (argv.worktree !== undefined) {
        ...
        const looksLikeInlineJson = (v) => {
            const t = v.trim();
            return t.startsWith('{') || t.startsWith('[');
        };
        const resolveIfPath = (v) => {
        ...
        if (Array.isArray(argv.includeDirectories)) {
            argv.includeDirectories = argv.includeDirectories.map((d) => typeof d === 'string' && d.length > 0
    ...
    let startupWorktreeContext = null;
    if (argv.worktree !== undefined && (argv.acp || argv.experimentalAcp)) {
        writeStderrLine('--worktree cannot be combined with --acp / --experimental-acp. ' +
    ...
    if (argv.resume !== undefined) {
        ...
        if (argv.resume === '') {
            ...
            const { showResumeSessionPicker } = await import('./ui/components/StandaloneSessionPicker.js');
            ...
            if (matches.length === 1) {
                ...
                writeStderrLine(`Multiple sessions found with title "${argv.resume}". Please select one:`);
                const { showResumeSessionPicker } = await import('./ui/components/StandaloneSessionPicker.js');
                resolvedSessionId = await showResumeSessionPicker(process.cwd(), matches);
            }
    ...
    // to run LailatulCoder Ai. It is now safe to perform expensive initialization that
    ...
    }
    {
        ...
        const config = await loadCliConfig(settings.merged, argv.acp || argv.experimentalAcp
            ? { ...argv, chatRecording: false }
            : argv, process.cwd(), argv.extensions, 
        // Pass separated hooks for proper source attribution
        {
            userHooks: settings.getUserHooks(),
            projectHooks: settings.getProjectHooks(),
        }, buildDisabledSkillNamesProvider(settings), undefined, settingsWatcher);
        markAcpStartup('configConstructionEnd');
        profileCheckpoint('after_load_cli_config');
        const nonInteractiveHousekeeping = !config.isInteractive() || config.getExperimentalZedIntegration()
            ? await import('./utils/housekeeping/scheduler.js')
            : undefined;
        if (nonInteractiveHousekeeping) {
            registerCleanup(() => nonInteractiveHousekeeping.stopNonInteractiveOpenAILogHousekeeping());
        }
        ...
        if (settingsWatcher) {
            const disposeMcpHotReload = registerMcpHotReload(settingsWatcher, settings, config, config.getTopTierMcpServers());
        ...
        const extensionFileWatcher = isBareMode(argv.bare) || config.isSafeMode()
            ? undefined
            : new ExtensionFileWatcher(config, undefined, extensionRefreshState);
        extensionFileWatcher?.startWatching();
        if (extensionFileWatcher) {
            const restartExtensionWatcher = () => extensionFileWatcher.restartWatching();
            extensionRefreshState.on(AppEvent.ExtensionsReloaded, restartExtensionWatcher);
            registerCleanup(() => {
                extensionRefreshState.off(AppEvent.ExtensionsReloaded, restartExtensionWatcher);
        ...
        if (startupWorktreeContext) {
            config.setPendingStartupWorktreeNotice(buildStartupWorktreeNotice(startupWorktreeContext));
            try {
                const startupWorktreePersist = await persistStartupWorktreeSidecar(config, startupWorktreeContext);
                if (startupWorktreePersist.overrodeResumedWorktree) {
                    writeStderrLine(`--worktree overrode the resumed session's previous worktree ` +
                ...
                config.setPendingStartupWorktreeNotice(buildStartupWorktreeNotice(startupWorktreeContext, startupWorktreePersist));
            }
            catch (error) {
                debugLogger.warn(`--worktree sidecar persist failed (non-fatal, notice preserved): ${error instanceof Error ? error.message : String(error)}`);
            }
        }
        ...
        let themeAutoDetectionComplete;
        if (config.isInteractive()) {
            registerCleanup(installInteractiveSignalHandlers(wasRaw));
        }
        if (config.isInteractive() && !wasRaw && process.stdin.isTTY) {
            const { startEarlyInputCapture, stopAndGetCapturedInput } = await import('./utils/earlyInputCapture.js');
        ...
        if (config.isInteractive()) {
            const { setMaxSizedBoxDebugging } = await import('./ui/components/shared/MaxSizedBox.js');
        ...
        const deferIdeConnection = config.isInteractive() &&
        ...
        const startupWarnings = [
            ...new Set([
                ...(config.isSafeMode()
                    ? [
                        '⚠ SAFE MODE — all customizations disabled (hooks, extensions, skills, MCP servers, LAILATUL.md). Restart without --safe-mode to resume normal operation.',
                    ]
                    : []),
        ...
        if (config.isInteractive()) {
            ...
            // can't catch this case (`LailatulCoder --json-schema '...'` on a TTY with
            ...
            if (config.getJsonSchema?.()) {
                writeStderrLine('Error: --json-schema is a headless-only flag. Provide a one-shot prompt via -p / --prompt or pipe one in via stdin.');
        ...
        if (inputFormat !== InputFormat.STREAM_JSON) {
            ...
            for (const warning of config.getWarnings()) {
                if (emittedStartupWarnings.has(warning))
                    continue;
            ...
            if (failedMcpServers.length > 0) {
                writeStderrLine(`Warning: MCP server(s) failed to start: ${failedMcpServers.join(', ')}. ` +
        ...
        const { validateNonInteractiveAuth } = await import('./validateNonInterActiveAuth.js');
        const nonInteractiveConfig = await validateNonInteractiveAuth(settings.merged.security?.auth?.useExternal, config, settings);
        const prompt_id = createNonInteractivePromptId(config.getSessionId());
        if (inputFormat === InputFormat.STREAM_JSON) {
            const trimmedInput = (input ?? '').trim();
            const { runNonInteractiveStreamJson } = await import('./nonInteractive/session.js');
            nonInteractiveHousekeeping?.startNonInteractiveOpenAILogHousekeeping(nonInteractiveConfig, settings);
            try {
                await runNonInteractiveStreamJson(nonInteractiveConfig, trimmedInput.length > 0 ? trimmedInput : '', settings);
            }
            finally {
                await runExitCleanup();
            }
        ...
        if (!input) {
            writeStderrLine(`No input provided via stdin. Input can be provided by piping data into gemini or using the --prompt option.`);
            process.exit(1);
        }
        ...
        let exitCode;
        try {
            exitCode = await runNonInteractive(nonInteractiveConfig, settings, input, prompt_id);
        }
        finally {
...
export function createNonInteractivePromptId(sessionId) {
    return `${sessionId}########0`;
}
/**
        ...
        try {
            ...
            if (result) {
                ...
                if (failedServers.length > 0) {
                    ...
                    const message = `LSP reload partially completed: changed=${formatRuntimeReloadNames(changedServers)}, failed=${formatRuntimeReloadNames(failedServers)}. Run with --debug for details.`;
                ...
                debugLogger.info('Skipped LSP server settings reload because LSP is disabled or no client is available');
            }
            appEvents.emit(AppEvent.LspStatusChanged);
        }
        catch (error) {
...
function formatRuntimeReloadNames(names) {
