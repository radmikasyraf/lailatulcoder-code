...
(function (MessageSenderType) {
    ...
    MessageSenderType["MODEL_SWITCH"] = "model_switch";
...
export class Logger {
    ...
    lastLoggedUserEntry = null; // Tracks the most recently persisted USER entry for cancel-undo (mirrors claude-code's lastAddedEntry).
    ...
    async _readLogFile() {
        ...
        try {
            ...
            if (!Array.isArray(parsedLogs)) {
                this.debugLogger.debug(`Log file at ${this.logFilePath} is not a valid JSON array. Starting with empty logs.`);
            ...
            if (error instanceof SyntaxError) {
                this.debugLogger.debug(`Invalid JSON in log file ${this.logFilePath}. Backing up and starting fresh.`, error);
                await this._backupCorruptedLogFile('invalid_json');
                return [];
            }
            this.debugLogger.debug(`Failed to read or parse log file ${this.logFilePath}:`, error);
            throw error;
        }
    }
    ...
    async initialize() {
        ...
        this.lailatulcoderDir = this.storage.getProjectTempDir();
        this.logFilePath = path.join(this.lailatulcoderDir, LOG_FILE_NAME);
        try {
            await fs.mkdir(this.lailatulcoderDir, { recursive: true });
    ...
    async _updateLogFile(entryToAppend) {
        if (!this.logFilePath) {
            this.debugLogger.debug('Log file path not set. Cannot persist log entry.');
        ...
        try {
            ...
            this.debugLogger.debug('Critical error reading log file before append:', readError);
            throw readError;
        }
        ...
        if (entryExists) {
            this.debugLogger.debug(`Duplicate log entry detected and skipped: session ${entryToAppend.sessionId}, messageId ${entryToAppend.messageId}`);
        ...
        try {
            await atomicWriteFile(this.logFilePath, JSON.stringify(currentLogsOnDisk, null, 2), { encoding: 'utf-8' });
    ...
    async logMessage(type, message) {
        if (!this.initialized || this.sessionId === undefined) {
            this.debugLogger.debug('Logger not initialized or session ID missing. Cannot log message.');
            return;
        }
        ...
        try {
            const writtenEntry = await this.serialize(() => this._updateLogFile(newEntryObject));
            if (writtenEntry) {
    ...
    async removeLastUserMessage() {
        ...
        if (!target)
            return false;
        this.lastLoggedUserEntry = null;
        const matchesTarget = (e) => e.sessionId === target.sessionId &&
        ...
        return this.serialize(async () => {
            ...
            try {
                ...
                this.debugLogger.debug('Failed to read log file while undoing last user entry:', error);
                restoreOptimistic();
                return false;
            }
            ...
            try {
                await atomicWriteFile(logFilePath, JSON.stringify(currentLogsOnDisk, null, 2), { encoding: 'utf-8' });
                this.logs = currentLogsOnDisk;
                ...
                this.debugLogger.debug('Failed to write log file while undoing last user entry:', error);
                restoreOptimistic();
                return false;
            }
        });
    ...
    async _getCheckpointPath(tag) {
        ...
        const oldPath = path.join(this.lailatulcoderDir, `checkpoint-${tag}.json`);
        try {
    ...
    async saveCheckpoint(conversation, tag) {
        if (!this.initialized) {
            this.debugLogger.error('Logger not initialized or checkpoint file path not set. Cannot save a checkpoint.');
            return;
        }
    ...
    async loadCheckpoint(tag) {
        if (!this.initialized) {
            this.debugLogger.error('Logger not initialized or checkpoint file path not set. Cannot load checkpoint.');
            return [];
        }
        ...
        try {
            ...
            if (!Array.isArray(parsedContent)) {
                this.debugLogger.warn(`Checkpoint file at ${path} is not a valid JSON array. Returning empty checkpoint.`);
                return [];
            }
            ...
            this.debugLogger.error(`Failed to read or parse checkpoint file ${path}:`, error);
            return [];
        }
    }
    async deleteCheckpoint(tag) {
        if (!this.initialized || !this.lailatulcoderDir) {
        ...
        try {
            ...
            const nodeError = error;
            if (nodeError.code !== 'ENOENT') {
                this.debugLogger.error(`Failed to delete checkpoint file ${newPath}:`, error);
        ...
        if (newPath !== oldPath) {
            try {
                ...
                const nodeError = error;
                if (nodeError.code !== 'ENOENT') {
                    this.debugLogger.error(`Failed to delete checkpoint file ${oldPath}:`, error);
                    throw error; // Rethrow unexpected errors
                }
                // It's okay if it doesn't exist.
            }
    ...
    async checkpointExists(tag) {
        ...
        try {
            ...
            this.debugLogger.error(`Failed to check checkpoint existence for ${filePath ?? `path for tag "${tag}"`}:`, error);
            throw error;
        }
    }
    close() {
