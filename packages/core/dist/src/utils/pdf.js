...
export const PDF_TEXT_EXTRACTION_UNAVAILABLE_MESSAGE = 'pdftotext is not installed. Install poppler-utils to enable PDF text extraction (e.g. `apt-get install poppler-utils` or `brew install poppler`).';
export const PDF_RENDER_UNAVAILABLE_MESSAGE = 'pdftoppm is not installed. Install poppler-utils to enable PDF page rendering (e.g. `apt-get install poppler-utils` or `brew install poppler`).';
/**
...
export function shouldRequirePDFPageRange(pageCount, sizeBytes) {
...
export function estimatePDFTextOutputTokens(text) {
    return Math.ceil(estimateTextTokens(text) +
        PDF_TEXT_RESULT_WRAPPER_TOKEN_CHARS / PDF_TEXT_RESULT_CHARS_PER_TOKEN);
}
export function buildLargePDFGuidance(displayName, requirement) {
...
export function buildPDFTextTooLargeGuidance(displayName, estimatedTokens, pagesUsed) {
    ...
    if (pageRange) {
        const suggestedEnd = Math.floor((pageRange.firstPage + pageRange.lastPage) / 2);
    ...
    return new Promise((resolve) => {
        execFile(command, args, { encoding: 'utf8', ...options }, (error, stdout, stderr) => {
            if (error) {
                ...
                const timedOut = !maxBufferExceeded &&
...
export function parsePDFPageRange(pages) {
    ...
    const inRange = (n) => Number.isFinite(n) && n >= 1 && n <= MAX_PDF_PAGE_NUMBER;
    ...
    if (range) {
        ...
        if (!inRange(first) || !inRange(last) || last < first)
            return null;
 ...
 */
export function resetPdftotextCache() {
 ...
 */
export function resetPdftoppmCache() {
 ...
 */
export async function renderPDFPagesToImages(filePath, options) {
    ...
    try {
        ...
        if (timedOut) {
            return {
                success: false,
                error: `pdftoppm timed out after ${Math.round(PDF_RENDER_TIMEOUT_MS / 1000)}s. The PDF may be unusually large or complex; try the 'pages' parameter to narrow the range.`,
            };
        }
        if (code !== 0) {
            if (/password/i.test(stderr)) {
