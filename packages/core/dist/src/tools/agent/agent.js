...
import { EXCLUDED_TOOLS_FOR_SUBAGENTS, extractParentToolNames, } from '../../agents/runtime/agent-core.js';
import { ToolConfirmationOutcome } from '../tools.js';
import { BUBBLE_APPROVAL_MODE } from '../../subagents/types.js';
import { AgentTerminateMode } from '../../agents/runtime/agent-types.js';
import { AgentHeadless, ContextState, } from '../../agents/runtime/agent-headless.js';
import { FORK_AGENT, FORK_DEFAULT_MAX_TURNS, FORK_SUBAGENT_TYPE, FORK_PLACEHOLDER_RESULT, buildForkExecutionAllowlist, buildForkedMessages, buildChildMessage, buildPinnedWorktreeNotice, buildWorktreeNotice, normalizeForkTurns, registerForkDisplayImageForCache, resolveForkExecutionAllowedTools, runInForkContext, selectForkHistory, validateForkToolList, } from './fork-subagent.js';
import { loadForkProfile, validateForkProfileName, } from './fork-profile.js';
import { generateAgentWorktreeSlug, GitWorktreeService, writeWorktreeSessionMarker, } from '../../services/gitWorktreeService.js';
...
import { childLaunchDepth, getCurrentAgentId, isTopLevelSession, runWithAgentContext, spawnBlockReason, } from '../../agents/runtime/agent-context.js';
import { trace, context as otelContext } from '@opentelemetry/api';
import { endSubagentSpan, runInSubagentSpanContext, startSubagentSpan, } from '../../telemetry/index.js';
import { AgentEventEmitter, AgentEventType, } from '../../agents/runtime/agent-events.js';
import { BuiltinAgentRegistry, DEFAULT_BUILTIN_SUBAGENT_TYPE, } from '../../subagents/builtin-agents.js';
...
import { ApprovalMode, Config, normalizeMaxSubagentDepth, validateMaxSessionTurns, } from '../../config/config.js';
...
import { buildAgentTranscriptAttach, getAgentMetaPath, attachJsonlTranscriptWriter, patchAgentMeta, writeAgentMeta, } from '../../agents/agent-transcript.js';
...
const resolvedForkProfiles = new WeakMap();
const FORK_PROFILE_SAFE_MODE_ERROR = 'Parameter "fork_profile" is unavailable in safe mode because project profiles are local customizations.';
const FORK_PROFILE_BARE_MODE_ERROR = 'Parameter "fork_profile" is unavailable in bare mode because project profiles are local customizations.';
function getForkProfileModeError(config) {
    if (config.getBareMode())
        return FORK_PROFILE_BARE_MODE_ERROR;
...
const TEAM_AGENT_NAME_PROPERTY = {
    type: 'string',
    description: 'When provided, spawn as a named teammate via the active team ' +
...
const TEAM_AGENT_PLAN_REQUIRED_PROPERTY = {
    type: 'boolean',
    description: 'When true, the named teammate starts in plan mode and must call ' +
...
const TEAM_AGENT_READ_ONLY_PROPERTY = {
    type: 'boolean',
    description: 'When true, the named teammate can only inspect the checkout and use ' +
 ...
 */
export function resolveSubagentApprovalMode(parentApprovalMode, agentApprovalMode, isTrustedFolder) {
    ...
    if (parentApprovalMode === ApprovalMode.YOLO ||
 ...
 */
function permissionModeToApprovalMode(mode) {
    switch (mode) {
 ...
 */
export const TOOL_REGISTRY_REBUILT = Symbol.for('lailatul-coder:tool-registry-rebuilt');
/**
 ...
 */
export function hasRebuiltToolRegistry(config) {
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    return config[TOOL_REGISTRY_REBUILT] === true;
}
/**
 ...
 */
export async function rebuildToolRegistryOnOverride(override, base) {
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    const ov = override;
...
function applyPersistedCliFlagOverrides(override, flags) {
    if (!flags)
        return;
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    const ov = override;
...
function capturePersistedCliFlags(config, resolvedApprovalMode, modelOverride, runtimeAuthOverrides) {
 ...
 */
export async function createApprovalModeOverride(base, mode, options = {}) {
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    ...
    override.setApprovalMode = (nextMode, setOptions) => {
        if (base.getApprovalMode() !== ApprovalMode.AUTO) {
            Config.prototype.setApprovalMode.call(override, nextMode, setOptions);
            return;
        }
        const hadOwnPermissionManager = Object.prototype.hasOwnProperty.call(override, 'permissionManager');
    ...
    const cleanup = () => {
        if (override.getApprovalMode() === ApprovalMode.AUTO &&
...
export class AgentTool extends BaseDeclarativeTool {
    ...
    subagentManager;
    availableSubagents = BuiltinAgentRegistry.getBuiltinAgents();
    ...
    constructor(config) {
        ...
        const initialSchema = {
            ...
            properties: {
                ...
                fork_profile: {
                    ...
                    minLength: 2,
                    maxLength: 50,
                    description: 'Only valid with subagent_type "fork". Loads a project profile from .lailatulcoder/fork-profiles/<name>.md and applies its tools and optional promptHint. Cannot be combined with fork_tools.',
                },
                run_in_background: {
                    type: 'boolean',
                    default: true,
                ...
                isolation: {
                    type: 'string',
                    enum: ['worktree'],
                    description: "Isolation mode. 'worktree' creates a temporary git worktree under <projectRoot>/.lailatulcoder/worktrees/agent-<7hex> so the agent works on an isolated copy of the repo. The worktree is auto-removed if the agent makes no changes; otherwise the worktree path and branch are returned in the result.",
                },
                working_dir: {
                    type: 'string',
        ...
        super(AgentTool.Name, ToolDisplayNames.AGENT, 'Launch a new agent to handle complex, multi-step tasks autonomously.\n\nThe Agent tool launches specialized agents (subprocesses) that autonomously handle complex tasks. Each agent type has specific capabilities and tools available to it.\n\nAvailable agent types and the tools they have access to:\n', Kind.Agent, initialSchema, true, // isOutputMarkdown
        true);
        this.config = config;
...
`;
        ...
        if (schema.properties) {
            ...
            if (availableGrades.length > 0) {
                schema.properties.model = {
                    ...
                    description: 'User-defined model grade for this subagent. Custom agents with an explicit model keep their configured model. Omit it to use the agent default.',
                };
            }
            else {
    ...
    validateToolParams(params) {
        ...
        if (params.subagent_type !== undefined) {
            ...
            if (lowerType !== FORK_SUBAGENT_TYPE) {
                const subagentExists = this.availableSubagents.some((subagent) => subagent.name.toLowerCase() === lowerType);
                if (!subagentExists) {
        ...
        if (params.fork_turns !== undefined) {
            if (typeof params.fork_turns !== 'string' ||
    ...
    createInvocation(params) {
        ...
        const forkProfile = invocationParams.fork_profile !== undefined
            ? loadForkProfile(this.config.getProjectRoot(), invocationParams.fork_profile)
            : undefined;
        if (forkProfile) {
        ...
        return new AgentToolInvocation(this.config, this.subagentManager, invocationParams, forkProfile);
    }
...
function deriveSubagentExceptionMetadata(error, signalAborted) {
    return {
        status: signalAborted ? 'aborted' : 'failed',
        error: error instanceof Error ? error.message : String(error),
        errorType: error instanceof Error ? error.constructor.name : 'NonErrorThrown',
...
class AgentToolInvocation extends BaseToolInvocation {
    ...
    currentToolCalls = [];
    callId;
    constructor(config, subagentManager, params, forkProfile) {
        super(params);
        this.config = config;
        this.subagentManager = subagentManager;
        this.forkProfile = forkProfile;
    }
     ...
     */
    updateDisplay(updates, updateOutput) {
    ...
    registerOwnedMonitorNotifications(agentId, enqueue, wake) {
        ...
        monitorRegistry.setAgentNotificationCallback(agentId, (_displayText, modelText) => void enqueue({ kind: 'notification', text: modelText }));
     ...
     */
    setupEventListeners(updateOutput) {
        let pendingConfirmationCallId;
        ...
        this.eventEmitter.on(AgentEventType.TOOL_CALL, (...args) => {
            const event = args[0];
            const newToolCall = {
                callId: event.callId,
                name: event.name,
        ...
        this.eventEmitter.on(AgentEventType.TOOL_RESULT, (...args) => {
            const event = args[0];
            const toolCallIndex = this.currentToolCalls.findIndex((call) => call.callId === event.callId);
            if (toolCallIndex >= 0) {
                ...
                const clearPending = pendingConfirmationCallId === event.callId
        ...
        this.eventEmitter.on(AgentEventType.USAGE_METADATA, (...args) => {
            const event = args[0];
        ...
        this.eventEmitter.on(AgentEventType.TOOL_WAITING_APPROVAL, (...args) => {
            const event = args[0];
            const idx = this.currentToolCalls.findIndex((c) => c.callId === event.callId);
            if (idx >= 0) {
            ...
            const details = {
                ...
                onConfirm: async (outcome, payload) => {
                    ...
                    if (proceedOutcomes.has(outcome)) {
                        const idx2 = this.currentToolCalls.findIndex((c) => c.callId === event.callId);
                        if (idx2 >= 0) {
                        ...
                        this.updateDisplay({ pendingConfirmation: undefined }, updateOutput);
                    }
     ...
     */
    async createForkSubagent(agentConfig, eventEmitter = this.eventEmitter) {
        const geminiClient = this.config.getGeminiClient();
        ...
        const requestedExecutionAllowedTools = requestedTools === undefined
            ? undefined
            : resolveForkExecutionAllowedTools(parentToolNames, buildForkExecutionAllowlist(requestedTools, []));
        ...
        if (geminiClient) {
            ...
            if (forkTurns === 'all') {
                rawHistory = selectForkHistory(geminiClient.getHistoryShallow?.(true) ??
                    geminiClient.getHistory(true), forkTurns);
            }
            else {
                const comprehensiveHistory = geminiClient.getHistoryShallow?.() ?? geminiClient.getHistory();
                const startupContext = comprehensiveHistory.slice(0, getStartupContextLength(comprehensiveHistory));
                rawHistory = [
                    ...
                    geminiClient.getHistoryForForkWindow?.() ??
                        geminiClient.getHistory(), forkTurns),
                ];
            }
        }
        ...
        if (rawHistory.length > 0) {
            ...
            if (lastMessage.role === 'model') {
                const forkedMessages = buildForkedMessages(this.params.prompt, lastMessage, requestedExecutionAllowedTools, profilePromptHint);
        ...
        if (!taskPrompt) {
            taskPrompt = buildChildMessage(this.params.prompt, requestedExecutionAllowedTools, profilePromptHint);
        }
        ...
        if (generationConfig?.systemInstruction) {
            ...
            const declaredExecutionToolNames = parentToolNames.length > 0
                ? parentToolNames
                : agentConfig
                    .getToolRegistry()
                    .getAllToolNames()
            ...
            promptConfig = {
                renderedSystemPrompt: generationConfig.systemInstruction,
                initialMessages,
            };
            toolConfig = {
                tools: parentToolNames.length > 0 ? parentToolNames : ['*'],
                executionAllowedTools: resolveForkExecutionAllowedTools(parentToolNames, buildForkExecutionAllowlist(requestedTools, declaredExecutionToolNames)),
            };
        }
        else {
            ...
            toolConfig = {
                tools: ['*'],
                executionAllowedTools: resolveForkExecutionAllowedTools(parentToolNames, buildForkExecutionAllowlist(requestedTools, registeredToolNames)),
            };
        }
        const subagent = await AgentHeadless.create(FORK_AGENT.name, agentConfig, promptConfig, {}, { max_turns: FORK_DEFAULT_MAX_TURNS }, toolConfig, eventEmitter);
    ...
    async runSubagentStopHookLoop(subagent, opts) {
        ...
        if (!hookSystem)
            return undefined;
        const effectiveTranscriptPath = transcriptPath ?? this.config.getTranscriptPath();
        let stopHookActive = false;
        const maxIterations = this.config.getStopHookBlockingCap();
        for (let i = 0; i < maxIterations; i++) {
            try {
                const stopHookOutput = await hookSystem.fireSubagentStopEvent(agentId, agentType, effectiveTranscriptPath, subagent.getFinalText(), stopHookActive, resolvedMode, signal);
                ...
                debugLogger.warn(`[Agent] SubagentStop hook failed, allowing stop: ${hookError}`);
                return undefined;
            }
        }
        return undefined;
     ...
     * #3731 Phase 3.
     */
    async runWithSubagentSpan(spec, signal, body) {
        const invokerSpanContext = spec.invocationKind === 'foreground'
            ? undefined
        ...
        try {
            ...
            if (!recordedMetadata) {
                debugLogger.warn(`runWithSubagentSpan: body did not call recordOutcome for ${spec.subagentName}/${spec.agentId} — defaulting span status to failed (wiring bug)`);
            }
            endSubagentSpan(span, recordedMetadata ?? {
                status: 'failed',
     ...
     */
    buildSubagentSpanSpec(hookOpts, subagentConfig, invocationKind) {
        return {
     ...
     */
    async runSubagentWithHooks(subagent, contextState, opts) {
        ...
        try {
            if (hookSystem) {
                try {
                    const startHookOutput = await hookSystem.fireSubagentStartEvent(agentId, agentType, resolvedMode, signal);
                    ...
                    debugLogger.warn(`[Agent] SubagentStart hook failed, continuing execution: ${hookError}`);
                }
            }
            ...
            const finalText = appendStopHookBlockingCapWarning(toModelVisibleSubagentResult(subagentRawText, terminateMode), stopHookWarning);
            ...
            opts.recordSpanOutcome?.(deriveSubagentOutcomeMetadata({
                terminateMode,
                signalAborted: signal?.aborted ?? false,
                resultSummaryPresent: Boolean(subagentRawText && subagentRawText.length > 0),
            }));
            if (signal?.aborted) {
                this.updateDisplay({
                    status: 'cancelled',
            ...
            opts.recordSpanOutcome?.(deriveSubagentExceptionMetadata(error, signal?.aborted ?? false));
            const errorMessage = error instanceof Error ? error.message : String(error);
            debugLogger.error(`[AgentTool] Error inside subagent background task: ${errorMessage}`);
            this.updateDisplay({
                status: 'failed',
     ...
     * shape in lockstep.
     */
    buildSpawnBlockedResult(llmContent, terminateReason) {
        return {
            llmContent,
            ...
            returnDisplay: {
                type: 'task_execution',
                subagentName: this.params.subagent_type ?? DEFAULT_BUILTIN_SUBAGENT_TYPE,
    ...
    async execute(signal, updateOutput) {
        if (this.params.plan_mode_required === true) {
            if (!this.params.name ||
                ...
                const msg = 'plan_mode_required can only be used when spawning a named teammate from the team leader.';
                return {
                    llmContent: msg,
                    returnDisplay: msg,
                    error: { message: msg },
            ...
            if (!this.config.getTeamManager()) {
                const msg = 'plan_mode_required requires an active team. Use TeamCreate first.';
                return {
                    llmContent: msg,
                    returnDisplay: msg,
                    error: { message: msg },
        ...
        if (this.params.read_only === true) {
            if (!this.params.name ||
                this.params.name.trim() === '' ||
                isSubagentLikeExecutionContext()) {
                const msg = 'read_only can only be used when spawning a named teammate from the team leader.';
                return {
                    llmContent: msg,
                    returnDisplay: msg,
                    error: { message: msg },
        ...
        if (this.params.name && !isTeammate() && isTopLevelSession()) {
            if (!this.config.getTeamManager()) {
                debugLogger.debug(`[AgentTool] Ignoring teammate name "${this.params.name}" because no team is active.`);
            }
            else if (this.params.model !== undefined) {
                return this.buildSpawnBlockedResult('Error: "model" is not supported for a named teammate.', 'model is incompatible with a named teammate');
            }
            else if (this.params.isolation !== undefined) {
                return this.buildSpawnBlockedResult('Error: "isolation" cannot be used for a named teammate. Create a leader-owned worktree first, then pass it with "working_dir".', 'isolation is incompatible with a named teammate');
            }
                ...
                return this.buildSpawnBlockedResult('Error: "run_in_background" cannot be false for a named teammate: teammates always run concurrently and report through team messaging. Omit "run_in_background" when spawning a teammate, or omit "name" and keep run_in_background: false to run a regular agent in the foreground and get its result inline.', 'run_in_background: false is incompatible with a named teammate');
            }
            else {
            ...
            debugLogger.debug(`[AgentTool] Ignoring teammate name "${this.params.name}" from a nested sub-agent; spawning a regular sub-agent instead.`);
        }
        // ─── Spawn guards ─────────────────────────────────────────────
        ...
        if (spawnBlocked !== null) {
            debugLogger.debug(`[AgentTool] Spawn blocked (${spawnBlocked}): childLevel=${childLaunchDepth() + 1} max=${maxSubagentDepth} type=${this.params.subagent_type ?? DEFAULT_BUILTIN_SUBAGENT_TYPE}`);
            switch (spawnBlocked) {
                case 'depth':
                    return this.buildSpawnBlockedResult(`Error: sub-agent nesting depth limit reached ` +
                        `(max ${maxSubagentDepth} level${maxSubagentDepth === 1 ? '' : 's'}). ` +
                        `Complete this task directly with your own tools instead of ` +
                        `spawning another sub-agent.`, `Nesting depth limit reached (max ${maxSubagentDepth})`);
                case 'teammate':
                    return this.buildSpawnBlockedResult('Error: Teammates cannot spawn sub-agents. Complete this task directly with your own tools, or coordinate through send_message.', 'Sub-agent spawning is not allowed from a teammate context');
                case 'fork':
                    return this.buildSpawnBlockedResult('Error: Cannot spawn sub-agents from within a fork. Please execute tasks directly.', 'Sub-agent spawning is not allowed inside a fork');
                default: {
                    const exhaustive = spawnBlocked;
        ...
        const cleanupWorktreeIsolation = async () => {
            ...
            // all.
            if (isolation.externallyManaged)
                return {};
            ...
            const [hasChanges, hasUnmerged] = await Promise.all([
                wtService.hasWorktreeChanges(isolation.path).catch((error) => {
                    debugLogger.warn(`[Agent] hasWorktreeChanges failed for ${isolation.path}: ${error}`);
                    // Fail-closed: assume changes exist so we preserve.
                    return true;
                }),
                wtService.hasUnmergedWorktreeCommits(isolation.slug).catch((error) => {
                    debugLogger.warn(`[Agent] hasUnmergedWorktreeCommits failed for ${isolation.slug}: ${error}`);
                    // Fail-closed: assume uncovered work exists so we preserve.
                    return true;
                }),
            ]);
            if (hasChanges || hasUnmerged) {
                debugLogger.info(`[Agent] Preserving isolation worktree ${isolation.path} ` +
            ...
            try {
                ...
                if (!result.success) {
                    ...
                    debugLogger.warn(`[Agent] Failed to remove ephemeral worktree ${isolation.path}: ${result.error}`);
                    return {
                        preservedPath: isolation.path,
                        preservedBranch: isolation.branch,
                    };
                ...
                if (result.branchPreserved) {
                    ...
                    debugLogger.warn(`[Agent] Removed worktree directory ${isolation.path} but kept ` +
                ...
                debugLogger.warn(`[Agent] Failed to remove ephemeral worktree ${isolation.path}: ${error}`);
                return {
                    preservedPath: isolation.path,
                    preservedBranch: isolation.branch,
                };
        ...
        let subagentModelId;
        let subagentRuntimeAuthOverrides;
        ...
        try {
            ...
            if (isForkRequested && !isTopLevelSession()) {
                debugLogger.debug('[AgentTool] Fork request rejected because forks do not nest.');
                return this.buildSpawnBlockedResult('Error: subagent_type "fork" is not supported from within a sub-agent. Complete this task directly with your own tools instead of requesting a nested fork.', 'Nested forks are not supported');
            }
            if (this.params.run_in_background === true && !isTopLevelSession()) {
                debugLogger.debug('[AgentTool] Explicit background request rejected because background agents do not nest.');
                return this.buildSpawnBlockedResult('Error: run_in_background: true is not supported from within a sub-agent. Run this agent in the foreground by omitting run_in_background or setting it to false.', 'Background execution is not supported from a nested sub-agent');
            }
            const isFork = isForkRequested;
            if (isFork) {
                debugLogger.debug(`[AgentTool] Fork request accepted with inherited context (${this.config.isInteractive() ? 'interactive' : 'headless'}).`);
            }
            ...
            if (isFork) {
                ...
                const loadedConfig = await this.subagentManager.loadSubagent(effectiveSubagentType);
                if (!loadedConfig) {
            ...
            const model = this.subagentManager.resolveModelGrade(this.params.model, subagentConfig);
            ...
            const backgroundRequested = isFork && !this.config.isInteractive()
                ? true
            ...
            if (this.params.working_dir !== undefined && shouldRunInBackground) {
                ...
                return this.buildSpawnBlockedResult('Error: "working_dir" cannot be used with a background agent — the caller owns the worktree and could remove it while the detached agent is still running there. Run this agent in the foreground, or drop "working_dir".', 'working_dir is incompatible with a background agent');
            }
            if (backgroundRequested && !shouldRunInBackground) {
                debugLogger.debug(`[AgentTool] Background request downgraded to a foreground run for a nested sub-agent (type=${subagentConfig.name}).`);
            }
            if (shouldRunInBackground) {
                ...
                const resolvedSubagentModel = resolveModelId(subagentConfig.model, buildModelIdContext(this.config));
                subagentModelId = resolvedSubagentModel?.modelId;
                subagentModelId ??= this.config.getModel();
                const parentContentGeneratorConfig = this.config.getContentGeneratorConfig();
                const authType = resolvedSubagentModel?.authType ??
                ...
                if (!backgroundSlotReservation) {
                    ...
                    backgroundSlotReservation = await registry.waitForBackgroundSlot(signal, subagentModelId);
                }
                this.updateDisplay({
                    status: 'running',
            ...
            const failWorktreeProvisioning = (reason) => {
            ...
            if (this.params.working_dir !== undefined) {
                ...
                const resolved = await resolveExternalWorktreeDir(this.config, this.params.working_dir);
                ...
                // running inside a worktree (cwd contains `.lailatulcoder/worktrees/`),
                ...
                if (/\.lailatulcoder[\\/]worktrees[\\/]/.test(cwd)) {
                    return failWorktreeProvisioning(`Failed to set up worktree isolation: parent is already inside ` +
                ...
                if (!gitCheck.available) {
                    return failWorktreeProvisioning(`Failed to set up worktree isolation: ${gitCheck.error ?? 'git is not available'}`);
                }
                if (!(await probe.isGitRepository())) {
                    return failWorktreeProvisioning(`Failed to set up worktree isolation: ${cwd} is not a git repository.`);
                }
                // Anchor the worktree at the repo top-level so monorepo subdir
                // launches still gather worktrees under `<repoRoot>/.lailatulcoder/...`,
                ...
                try {
                    ...
                    debugLogger.warn(`[Agent] hasWorktreeChanges failed at ${projectRoot}: ${error}`);
                    // Fail-closed: assume dirty so we refuse rather than
                    // silently launch a subagent against a possibly-stale tree.
                    parentDirty = true;
                }
                if (parentDirty) {
                    return failWorktreeProvisioning(`Failed to set up worktree isolation: parent working tree at ` +
                ...
                try {
                    ...
                    // the fallback path.
                    debugLogger.warn(`[Agent] getCurrentBranch failed at ${projectRoot}: ${error}`);
                }
                ...
                if (!created.success || !created.worktree) {
                    return failWorktreeProvisioning(`Failed to create isolation worktree: ${created.error ?? 'unknown error'}`);
                }
                worktreeIsolation = {
                    slug,
                ...
                try {
                    await writeWorktreeSessionMarker(created.worktree.path, this.config.getSessionId());
                }
                catch (error) {
                    debugLogger.warn(`[Agent] failed to write session marker at ${created.worktree.path}: ${error}`);
                }
            }
            // Resolve the subagent's permission mode before creating it
            const resolvedMode = resolveSubagentApprovalMode(this.config.getApprovalMode(), subagentConfig.approvalMode, this.config.isTrustedFolder());
            ...
            const { config: agentConfig, cleanup } = await createApprovalModeOverride(this.config, resolvedApprovalMode);
            ...
            if (worktreeIsolation) {
                ...
                const wtFileService = new FileDiscoveryService(wtPath, this.config.getFileFilteringOptions().customIgnoreFiles);
            ...
            const shouldBubble = Boolean(shouldRunInBackground &&
            ...
            const subagentRuntimeConfig = shouldRunInBackground
                ? Object.create(agentConfig)
                : agentConfig;
            if (shouldRunInBackground) {
                subagentRuntimeConfig.getShouldAvoidPermissionPrompts = () => !shouldBubble;
            }
            ...
            let subagentDispose;
            if (isFork) {
                const fork = await this.createForkSubagent(subagentRuntimeConfig, backgroundEventEmitter);
                ...
                const result = await this.subagentManager.createAgentHeadless(subagentConfig, subagentRuntimeConfig, {
            ...
            if (worktreeIsolation) {
                ...
                const notice = worktreeIsolation.externallyManaged
                    ...
                    : buildWorktreeNotice(this.config.getTargetDir(), worktreeIsolation.path);
                taskPrompt = `${notice}\n\n${taskPrompt}`;
            }
            ...
            if (shouldRunInBackground) {
                ...
                if (hookSystem) {
                    try {
                        const startHookOutput = await hookSystem.fireSubagentStartEvent(hookOpts.agentId, hookOpts.agentType, resolvedMode, signal);
                        const additionalContext = startHookOutput?.getAdditionalContext();
                        if (additionalContext) {
                            contextState.set('hook_context', additionalContext);
                        }
                ...
                const { jsonlPath, options: transcriptAttachOptions } = buildAgentTranscriptAttach(this.config, hookOpts.agentId, {
                ...
                const metaPath = getAgentMetaPath(projectDir, sessionId, hookOpts.agentId);
                try {
                    ...
                    if (hookSystem && subagentStartHookCompleted) {
                        try {
                            await hookSystem.fireSubagentStopEvent(hookOpts.agentId, hookOpts.agentType, jsonlPath, bgSubagent.getFinalText(), false, resolvedMode, signal);
                        }
                        catch (hookError) {
                            debugLogger.warn(`[Agent] SubagentStop hook after background registration failure failed: ${hookError}`);
                        }
                    }
                    let wtSuffix = '';
                    try {
                        ...
                        debugLogger.warn(`[Agent] Worktree cleanup after background registration failure failed: ${cleanupError}`);
                    }
                    this.updateDisplay({
                        status: 'failed',
                    ...
                    void agentConfig
                        ...
                        .stop()
                        .catch((stopError) => {
                        debugLogger.warn(`[Agent] ToolRegistry stop after background registration failure failed: ${stopError}`);
                    });
                ...
                const { cleanup: cleanupJsonl } = attachJsonlTranscriptWriter(bgEventEmitter, jsonlPath, transcriptAttachOptions);
                writeAgentMeta(metaPath, {
                    ...
                    ...(isFork &&
                        ...
                        : {}),
                    persistedCliFlags: capturePersistedCliFlags(this.config, resolvedApprovalMode, bgSubagent.getCore().modelConfig.model, bgSubagent.getCore().runtimeView?.contentGeneratorConfig ??
                ...
                const cleanupApprovalBridge = shouldBubble
                    ...
                    : undefined;
                const cleanupOwnedMonitorNotifications = this.registerOwnedMonitorNotifications(hookOpts.agentId, (input) => registry.queueExternalInput(hookOpts.agentId, input), () => registry.wakeExternalInputWaiters(hookOpts.agentId));
                // Wire external message drain so SendMessage and owned Monitor
                // notifications can inject inputs between tool rounds.
                bgSubagent.setExternalMessageProvider(() => registry.drainMessages(hookOpts.agentId));
                bgSubagent.setExternalMessageWaiter?.((waitSignal) => registry.waitForMessages(hookOpts.agentId, waitSignal));
                bgSubagent.setExternalMessageWaitPredicate?.(() => this.config.getMonitorRegistry().hasRunningForOwner(hookOpts.agentId));
                ...
                const canStayResident = !isFork &&
                    this.params.isolation !== 'worktree' &&
                    (!subagentConfig.hooks ||
                        Object.keys(subagentConfig.hooks).length === 0);
                const needsAutoPermissionLease = () => agentConfig.getApprovalMode() === ApprovalMode.AUTO &&
                    this.config.getApprovalMode() !== ApprovalMode.AUTO;
                let runtimeDisposed = false;
                let disposeRequested = false;
                let turnRunning = false;
                let currentAbortController = bgAbortController;
                let currentTurnPromise;
                ...
                const cleanupRuntime = () => {
                    ...
                    registry.unregisterResidentAgent(hookOpts.agentId, residentController);
                ...
                // spawn nested forks.
                const bgBody = async (turnContextState, turnAbortController, recordSpanOutcome, fireStartHook) => {
                    ...
                    try {
                        while (true) {
                            if (shouldFireStartHook && hookSystem) {
                                try {
                                    const startHookOutput = await hookSystem.fireSubagentStartEvent(hookOpts.agentId, hookOpts.agentType, resolvedMode, turnAbortController.signal);
                                    const additionalContext = startHookOutput?.getAdditionalContext();
                                    if (additionalContext) {
                                        ...
                                        turnContextState.set('task_prompt', `${String(turnContextState.get('task_prompt'))}\n\n${additionalContext}`);
                                    }
                                }
                                catch (hookError) {
                                    debugLogger.warn(`[Agent] SubagentStart hook failed, continuing execution: ${hookError}`);
                            ...
                            if (finishingInputs) {
                                await bgSubagent.executeExternalInputs(finishingInputs, turnAbortController.signal, { resetStats: false });
                                finishingInputs = undefined;
                            }
                            else {
                                await bgSubagent.execute(turnContextState, turnAbortController.signal);
                            }
                            let stopHookWarning;
                            ...
                            const recordTerminalOutcome = () => recordSpanOutcome(deriveSubagentOutcomeMetadata({
                                terminateMode,
                            ...
                            const wtSuffix = formatWorktreeSuffix(hadWorktreeIsolation ? await cleanupWorktreeIsolation() : {});
                            const modelVisibleText = toModelVisibleSubagentResult(subagentRawText, terminateMode);
                            const finalText = appendStopHookBlockingCapWarning(terminateMode === AgentTerminateMode.GOAL
                                ...
                                : modelVisibleText, stopHookWarning) + wtSuffix;
                            ...
                            if (terminateMode === AgentTerminateMode.GOAL) {
                                ...
                                // wenshao @ #4410.
                                registry.finalizeCancelled(hookOpts.agentId, finalText, completionStats);
                                persistBackgroundCancellation(metaPath, registry.get(hookOpts.agentId)?.persistedCancellationStatus ??
                                    'cancelled');
                            }
                            else {
                                ...
                                patchAgentMeta(metaPath, {
                                    status: 'failed',
                                    lastUpdatedAt: new Date().toISOString(),
                                    lastError: finalText || `Agent terminated with mode: ${terminateMode}`,
                                });
                            }
                            break;
                        }
                        ...
                        recordSpanOutcome(deriveSubagentExceptionMetadata(error, turnAbortController.signal.aborted));
                        const baseErrorMsg = error instanceof Error ? error.message : String(error);
                        debugLogger.error(`[Agent] Background agent failed: ${baseErrorMsg}`);
                        ...
                        if (turnAbortController.signal.aborted) {
                            registry.finalizeCancelled(hookOpts.agentId, errorMsg, getCompletionStats());
                            persistBackgroundCancellation(metaPath, registry.get(hookOpts.agentId)?.persistedCancellationStatus ??
                                'cancelled');
                        }
                        else {
                ...
                const runBackgroundTurn = (turnContextState, turnAbortController, fireStartHook) => {
                    const framedBgBody = () => this.runWithSubagentSpan(this.buildSubagentSpanSpec(hookOpts, subagentConfig, isFork ? 'fork' : 'background'), turnAbortController.signal, (recordOutcome) => runWithAgentContext(hookOpts.agentId, () => bgBody(turnContextState, turnAbortController, recordOutcome, fireStartHook), launchDepth));
                    return isFork ? runInForkContext(framedBgBody) : framedBgBody();
                };
                const reportUnexpectedBackgroundError = (err) => {
                    debugLogger.warn(`[Agent] background subagent ${hookOpts.agentId} body raised unexpected rejection: ${err instanceof Error ? err.message : String(err)}`);
                };
                const residentController = {
                    continue: (message) => {
                        ...
                        let restarted;
                        try {
                            restarted = registry.restartCompletedAgent(hookOpts.agentId, nextAbortController);
                        }
                        catch (error) {
                            debugLogger.warn(`[Agent] Could not continue resident background agent ${hookOpts.agentId}: ${error instanceof Error ? error.message : String(error)}`);
                            return false;
                        }
                        if (!restarted ||
                            disposeRequested ||
                        ...
                        currentTurnPromise = previousTurn
                            ...
                            if (disposeRequested || runtimeDisposed)
                                return;
                            await runBackgroundTurn(nextContextState, nextAbortController, true);
                        });
                ...
                currentTurnPromise = runBackgroundTurn(contextState, bgAbortController, false);
                currentTurnPromise.catch(reportUnexpectedBackgroundError);
                this.updateDisplay({ status: 'background' }, updateOutput);
                return {
            ...
            if (isFork) {
                ...
                subagent.setExternalMessageWaiter?.((waitSignal) => forkMonitorInputs.wait(waitSignal));
                subagent.setExternalMessageWaitPredicate?.(() => this.config.getMonitorRegistry().hasRunningForOwner(hookOpts.agentId));
                const cleanupOwnedMonitorNotifications = this.registerOwnedMonitorNotifications(hookOpts.agentId, forkMonitorInputs.enqueue, forkMonitorInputs.wake);
                ...
                const runFramedFork = () => this.runWithSubagentSpan(this.buildSubagentSpanSpec(hookOpts, subagentConfig, 'fork'), 
                ...
                // load-bearing path.
                undefined, (recordSpanOutcome) => runWithAgentContext(hookOpts.agentId, async () => {
                    try {
                ...
                runInForkContext(runFramedFork).catch((err) => debugLogger.warn(`[Agent] fork subagent ${hookOpts.agentId} body raised unexpected rejection: ${err instanceof Error ? err.message : String(err)}`));
                return {
            ...
            const runFramed = () => this.runWithSubagentSpan(this.buildSubagentSpanSpec(hookOpts, subagentConfig, 'foreground'), fgAbortController.signal, (recordSpanOutcome) => runWithAgentContext(hookOpts.agentId, () => this.runSubagentWithHooks(subagent, contextState, {
                ...fgHookOpts,
                recordSpanOutcome,
            })));
            ...
            const { jsonlPath: fgJsonlPath, options: fgTranscriptAttachOptions } = buildAgentTranscriptAttach(this.config, hookOpts.agentId, {
                agentName: subagentConfig.name,
                agentColor: subagentConfig.color,
            ...
            const fgMetaPath = getAgentMetaPath(fgProjectDir, fgSessionId, hookOpts.agentId);
            ...
            let cleanupFgJsonl;
            const cleanupOwnedMonitorNotifications = this.registerOwnedMonitorNotifications(hookOpts.agentId, (input) => registry.queueExternalInput(hookOpts.agentId, input), () => registry.wakeExternalInputWaiters(hookOpts.agentId));
            subagent.setExternalMessageProvider?.(() => registry.drainMessages(hookOpts.agentId));
            subagent.setExternalMessageWaiter?.((waitSignal) => registry.waitForMessages(hookOpts.agentId, waitSignal));
            subagent.setExternalMessageWaitPredicate?.(() => this.config.getMonitorRegistry().hasRunningForOwner(hookOpts.agentId));
            ...
            try {
                ({ cleanup: cleanupFgJsonl } = attachJsonlTranscriptWriter(this.eventEmitter, fgJsonlPath, fgTranscriptAttachOptions));
                ...
                writeAgentMeta(fgMetaPath, {
                    ...
                    ...(isFork &&
                        ...
                        : {}),
                    persistedCliFlags: capturePersistedCliFlags(this.config, resolvedApprovalMode, subagent.getCore().modelConfig.model),
                    subagentName: subagentConfig.name,
                    agentColor: subagentConfig.color,
                    resumeCount: 0,
                    // Persisted so resume restores the original nesting level; see
                ...
                const terminateMode = subagent.getTerminateMode();
                const finalText = appendStopHookBlockingCapWarning(toModelVisibleSubagentResult(subagent.getFinalText(), terminateMode), stopHookWarning);
                ...
                const fgTerminalStatus = fgTerminateMode === AgentTerminateMode.GOAL
                    ? 'completed'
            ...
            if (worktreeIsolation) {
                try {
                    ...
                    debugLogger.warn(`[AgentTool] Worktree cleanup after error failed: ${cleanupError}`);
                }
            }
            ...
            try {
                ...
                debugLogger.warn(`[AgentTool] restoreParentPM after error failed: ${restoreError}`);
            }
            const errorDisplay = {
                ...this.currentDisplay,
                status: 'failed',
     ...
     */
    async executeTeammate(name, signal, updateOutput) {
        ...
        try {
            ...
            if (this.params.working_dir) {
                const resolved = await resolveExternalWorktreeDir(this.config, this.params.working_dir);
                if ('error' in resolved) {
                    return this.buildSpawnBlockedResult(`Error: ${resolved.error}`, 'Invalid teammate working_dir');
                }
                cwd = resolved.path;
            }
            if (signal?.aborted) {
            ...
            const msg = `Teammate "${name}" is now running concurrently.` +
            ...
            const errorMessage = error instanceof Error ? error.message : String(error);
            debugLogger.error(`[AgentTool] Failed to spawn teammate: ${errorMessage}`);
            return {
