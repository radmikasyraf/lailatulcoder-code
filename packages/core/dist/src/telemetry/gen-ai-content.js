...
function messages(values, convertPart, roleMap) {
    ...
    for (const value of items) {
        ...
        const rawParts = content === undefined || content === null
            ? []
        ...
        for (const rawPart of rawParts) {
            ...
            if (currentParts.length > 0 && nextRole !== currentRole)
                flush();
        ...
        if (Array.isArray(message['tool_calls'])) {
            if (currentParts.length > 0 && currentRole !== role)
                flush();
            currentRole = role;
...
function normalizeSchema(value) {
    ...
    function visit(item) {
        ...
        if (items !== undefined) {
            ...
            if (schemas.length === 0 || !schemas.every(visitSchema))
                return false;
        }
...
function openAiTools(value) {
    ...
    for (const item of value) {
        ...
        const entry = type === 'function' && fn
            ? definition('function', fn['name'], fn['description'], fn['parameters'])
...
function anthropicTools(value) {
    ...
    for (const item of value) {
        const tool = record(item);
        const entry = tool
            ? definition('function', tool['name'], tool['description'], tool['input_schema'])
            : undefined;
        if (!entry)
            return undefined;
        result.push(compactUndefined(entry));
    ...
    for (const wrapper of value) {
        ...
        for (const item of declarations) {
            ...
            const hasJsonSchema = tool
                ...
                : false;
            const parameters = hasParameters && hasJsonSchema
                ? undefined
                : hasJsonSchema
...
export function extractOpenAiContent(request) {
    const value = request;
    return {
        inputMessages: Object.hasOwn(value, 'messages')
            ? messages(value['messages'], openAiPart, (role) => role === 'function' ? 'tool' : role)
            : undefined,
...
export function extractAnthropicContent(request) {
    const value = request;
    return {
        inputMessages: Object.hasOwn(value, 'messages')
...
export function extractGeminiContent(request) {
    const value = request;
    const config = record(value['config']);
    return {
        inputMessages: Object.hasOwn(value, 'contents')
            ? messages(value['contents'], geminiPart, (role) => role === 'model' ? 'assistant' : role)
            : undefined,
        systemInstructions: config && Object.hasOwn(config, 'systemInstruction')
            ? systemParts(config['systemInstruction'], geminiPart)
            : undefined,
        toolDefinitions: config && Object.hasOwn(config, 'tools')
            ? geminiTools(config['tools'])
            : undefined,
    };
}
export function stringifyGenAiJson(value, maxLength, requireObject = false) {
    const converted = jsonValue(value);
    if (converted === undefined ||
        (requireObject &&
            (converted === null ||
...
export class GenAiOutputAccumulator {
    ...
    recordOpenAiResponse(response) {
        ...
        for (const [position, rawChoice] of choices.entries()) {
            ...
            const index = typeof choice['index'] === 'number' &&
            ...
            if (message && this.enabled) {
                ...
                if (converted?.[0])
                    this.setComplete(index, converted[0]);
                else
                    this.markOverflow();
            }
        ...
        for (const [position, rawChoice] of choices.entries()) {
            ...
            if (Array.isArray(toolCalls)) {
                for (const [toolPosition, rawToolCall] of toolCalls.entries()) {
                    ...
                    const toolIndex = typeof toolCall['index'] === 'number' &&
                    ...
                    if (argumentsFragment !== undefined) {
                        ...
                        if (!this.reserve(argumentsFragment.length))
                            continue;
                        part.arguments = next;
                    }
    ...
    recordAnthropicEvent(event) {
        ...
        if (type === 'content_block_start') {
            ...
            if (blockType === 'text') {
                this.append(candidate, `block:${index}`, 'text', string(block?.['text']));
            }
            else if (blockType === 'thinking') {
                this.append(candidate, `block:${index}`, 'reasoning', string(block?.['thinking']) ?? string(block?.['data']));
            }
                ...
                if (!this.reserve(JSON.stringify({ type: 'tool_call' }).length))
                    return;
                const id = string(block?.['id']);
                const name = string(block?.['name']);
                if (!this.reserve((id?.length ?? 0) + (name?.length ?? 0)))
                    return;
                ...
                const initialArguments = input === undefined ? undefined : JSON.stringify(input);
            ...
            if (deltaType === 'text_delta') {
                this.append(candidate, `block:${index}`, 'text', string(delta?.['text']));
            }
            else if (deltaType === 'thinking_delta' ||
                deltaType === 'signature_delta') {
                if (deltaType === 'thinking_delta') {
                    this.append(candidate, `block:${index}`, 'reasoning', string(delta?.['thinking']));
                }
            }
    ...
    recordGeminiResponse(response) {
        ...
        for (const [position, rawCandidate] of candidates.entries()) {
            ...
            if (content && this.enabled) {
                const converted = messages([content], geminiPart, (role) => role === 'model' ? 'assistant' : role);
                if (converted?.[0])
                    this.setComplete(index, converted[0]);
                else
                    this.markOverflow();
            ...
            if (finishReason)
                outputCandidate.finishReason = finishReason;
        }
    }
    recordGeminiChunk(chunk) {
        ...
        for (const [position, rawCandidate] of candidates.entries()) {
            ...
            const index = typeof value['index'] === 'number' &&
            ...
            if (role)
                candidate.role = role === 'model' ? 'assistant' : role;
    ...
    finalize(success) {
        ...
        const output = [];
        for (const [, candidate] of [...this.candidates.entries()].sort(([left], [right]) => left - right)) {
            const finishReason = candidate.finishReason;
            if (!finishReason) {
                debugLogger.debug('Omitting GenAI output messages because a candidate has no finish reason');
                return undefined;
            }
            const parts = [];
            for (const part of candidate.parts.values()) {
                ...
                if (part.type === 'tool_call') {
                    if (!part.name)
                        return undefined;
                    const argumentsValue = part.arguments === undefined
                        ? undefined
        ...
        for (const [partIndex, value] of parts.entries()) {
            ...
            if (type === 'text' || type === 'reasoning') {
                ...
                candidate.parts.set(`part:${partIndex}`, {
                    ...
                    arguments: part['arguments'] === undefined
                        ? undefined
    ...
    append(candidate, key, type, fragment) {
        if (fragment === undefined || !this.enabled || this.overflow)
            return;
        ...
        if (!this.reserve(fragment.length + newPartLength))
            return;
        if (part) {
    ...
    reserve(length) {
        ...
        if (this.estimatedLength <= this.maxLength)
            return true;
        this.markOverflow();
        return false;
    }
...
//# sourceMappingURL=gen-ai-content.js.map