...
import { fetchGitDiff } from '@lailatul-coder/lailatul-coder-core';
...
function makeContextWithCwd(cwd = '/tmp/repo') {
...
function makeInteractiveContext(cwd = '/tmp/repo') {
    return createMockCommandContext({
...
describe('diffCommand', () => {
    ...
    it('reports when not in a git repo or transient state', async () => {
        ...
        expect(result.content).toMatch(/not a git repository|merge|rebase/i);
    });
    it('reports clean working tree when stats show zero changes', async () => {
        ...
        const result = await diffCommand.action(mockContext, '');
        expect(result.content).toMatch(/Clean working tree/i);
    });
...
describe('renderDiffModelText filename sanitization', () => {
    ...
    async function renderText(perFileStats) {
        if (!diffCommand.action)
            throw new Error('Command has no action');
        mockFetchGitDiff.mockResolvedValue({
    ...
    it('escapes raw ANSI escape sequences embedded in tracked filenames', async () => {
        ...
        const content = await renderText(new Map([[evil, { added: 1, removed: 0, isBinary: false }]]));
    ...
    it('escapes ANSI sequences in untracked / binary / deleted suffix rows', async () => {
        ...
        const content = await renderText(new Map([
