...
import { selectLanAddress } from './lan-interfaces.js';
...
export class InvalidLocalControlTargetError extends Error {
    code = 'invalid_local_control_target';
    constructor() {
 ...
 * the way through — all of it compensation for one fact: `LailatulCoder serve` fixed
...
export class LocalControlService {
    ...
    async #enable(options) {
        ...
        const server = tls
            ? createSecureServer({ cert: readFileSync(tls.cert), key: readFileSync(tls.key) }, this.#deps.app)
        ...
        server.on('error', (error) => {
            writeStderrLine(`LailatulCoder serve: Local Control listener error: ${error.message}`);
        });
        this.#server = server;
        this.#token = token;
...
function buildPairedUrl(scheme, authority, secret, target) {
