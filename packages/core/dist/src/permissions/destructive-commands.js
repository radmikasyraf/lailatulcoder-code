 ...
 */
const DESTRUCTIVE_GIT_PATTERNS = Object.freeze([
 ...
 */
const IAC_DESTROY_PATTERNS = Object.freeze([
 ...
 */
const DISCARD_KEYWORDS = Object.freeze([
    /\bdiscard\b/i,
    /\bthrow\s+away\b/i,
    /\bwipe\b/i,
    /\bclean\s+up\b/i,
 ...
 */
export function userMentionsDiscard(userPrompt) {
...
function stripShellQuotes(command) {
    return command.replace(/(?:^|\s)(?:bash|sh|zsh|fish|dash|ksh)\s+-[a-zA-Z]*c\s+(?:"([^"]*)"|'([^']*)')/g, (_match, dq, sq) => ' ' + (dq ?? sq ?? ''));
}
/**
...
const sessionCommitShas = new Set();
/**
 ...
 */
export function registerSessionCommit(sha) {
 ...
 */
export function isAmendOfSessionCommit(cwd) {
    if (sessionCommitShas.size === 0)
        return false;
    try {
 ...
 */
export function clearSessionCommits() {
 ...
 */
export function isDestructiveCommand(command, userPrompt, cwd = process.cwd()) {
    ...
    if (GIT_AMEND_PATTERN.test(expanded) && !isAmendOfSessionCommit(cwd)) {
        return {
            blocked: true,
            reason: 'Blocked "git commit --amend": the target commit was not made by the agent in this session. To proceed, use manual approval.',
        };
    }
...
function userMentionsStack(userPrompt, toolName) {
    if (!userPrompt)
        return false;
    const lower = userPrompt.toLowerCase();
    return (lower.includes(toolName) &&
