...
vi.mock('../../i18n/index.js', () => ({
    ...
    getLanguageNameFromLocale: vi.fn((locale) => {
        const map = {
            zh: 'Chinese',
            en: 'English',
            ru: 'Russian',
...
import { initializeLlmOutputLanguage, writeOutputLanguageFile, } from '../../utils/languageUtils.js';
describe('languageCommand', () => {
    ...
    beforeEach(() => {
        ...
        vi.mocked(i18n.t).mockImplementation((key, params) => {
            ...
            return Object.entries(params).reduce((result, [name, value]) => result.replace(`{{${name}}}`, value ?? ''), key);
        });
        // Reset fs mocks
    ...
    describe('/language ui subcommand', () => {
        ...
        it('should persist setting to user scope', async () => {
            ...
            'general.language', 'en', undefined, { throwOnWriteFailure: true });
        });
        it('persists to user scope with --global', async () => {
            ...
            expect(i18n.setLanguageAsync).toHaveBeenCalledWith('en');
            expect(mockContext.services.settings.setValue).toHaveBeenCalledWith('user', 'general.language', 'en', undefined, { throwOnWriteFailure: true });
        });
        it('persists to workspace scope with --project when trusted', async () => {
            if (!languageCommand.action) {
                throw new Error('The language command must have an action.');
            }
            mockContext.services.settings.isTrusted =
                true;
            await languageCommand.action(mockContext, 'ui zh --project');
            expect(i18n.setLanguageAsync).toHaveBeenCalledWith('zh');
            expect(mockContext.services.settings.setValue).toHaveBeenCalledWith('workspace', 'general.language', 'zh', undefined, { throwOnWriteFailure: true });
        });
        it('rejects --project in an untrusted workspace without persisting', async () => {
            ...
            mockContext.services.settings.isTrusted =
                false;
            const result = await languageCommand.action(mockContext, 'ui en --project');
            expect(i18n.setLanguageAsync).not.toHaveBeenCalled();
        ...
        it('rejects using both --project and --global together', async () => {
            ...
            const result = await languageCommand.action(mockContext, 'ui en --project --global');
            expect(i18n.setLanguageAsync).not.toHaveBeenCalled();
            expect(mockContext.services.settings.setValue).not.toHaveBeenCalled();
            expect(result).toMatchObject({
                messageType: 'error',
    ...
    describe('/language output subcommand', () => {
        ...
        it('should save LLM output language setting', async () => {
            ...
            expect(mockContext.services.settings?.setValue).toHaveBeenCalledWith(expect.anything(), // SettingScope.User
            'general.outputLanguage', 'Chinese');
            // Verify path registration on first-time creation (getOutputLanguageFilePath returned undefined)
            expect(mockContext.services.config['setOutputLanguageFilePath']).toHaveBeenCalledWith(expect.any(String));
            expect(result).toEqual({
                type: 'message',
                messageType: 'info',
        ...
        it('should preserve auto through the full output command path', async () => {
            ...
            expect(mockContext.services.settings?.setValue).toHaveBeenCalledWith(expect.anything(), 'general.outputLanguage', 'auto');
            const writtenContent = vi.mocked(fs.writeFileSync).mock
                .calls[0][1];
            expect(writtenContent).toContain('# Output language preference: auto');
            expect(writtenContent).toContain("Respond in the same language as the user's input.");
        ...
        it('should apply the new output language to the running session without requiring a restart', async () => {
            ...
            const instructionOrder = refreshSystemInstruction.mock.invocationCallOrder[0];
        ...
        it('should still report success when applying to the running session fails', async () => {
            ...
            expect(mockContext.services.settings?.setValue).toHaveBeenCalledWith(expect.anything(), 'general.outputLanguage', 'Korean');
            expect(result).toEqual({
                type: 'message',
                messageType: 'info',
                content: expect.stringContaining('LLM output language set to'),
        ...
        it('should normalize locale code "ru" to "Russian"', async () => {
            ...
            expect(mockContext.services.settings?.setValue).toHaveBeenCalledWith(expect.anything(), 'general.outputLanguage', 'Russian');
        });
        it('should normalize output language name "english" to "English"', async () => {
            ...
            expect(mockContext.services.settings?.setValue).toHaveBeenCalledWith(expect.anything(), 'general.outputLanguage', 'English');
        });
        it('should normalize locale code "de" to "German"', async () => {
            ...
            // Verify setting was saved with normalized value
            expect(mockContext.services.settings?.setValue).toHaveBeenCalledWith(expect.anything(), 'general.outputLanguage', 'German');
        });
        it('should save the setting and report success on a valid argument', async () => {
            if (!languageCommand.action) {
                throw new Error('The language command must have an action.');
            }
            const result = await languageCommand.action(mockContext, 'output Spanish');
            // Verify setting was saved
            expect(mockContext.services.settings?.setValue).toHaveBeenCalledWith(expect.anything(), 'general.outputLanguage', 'Spanish');
    ...
    describe('output subcommand object', () => {
        ...
        it('should have action that saves setting', async () => {
            ...
            // Verify the setting was persisted.
            expect(mockContext.services.settings?.setValue).toHaveBeenCalledWith(expect.anything(), 'general.outputLanguage', 'French');
            expect(result).toEqual({
                type: 'message',
                messageType: 'info',
                content: expect.stringContaining('LLM output language set to'),
    ...
    describe('nested ui language subcommands', () => {
        ...
        const deDESubcommand = uiSubcommand?.subCommands?.find((c) => c.name === 'de-DE');
        ...
        const jaJPSubcommand = uiSubcommand?.subCommands?.find((c) => c.name === 'ja-JP');
        ...
        it('zh-CN action should accept --global and persist to user scope', async () => {
            ...
            expect(i18n.setLanguageAsync).toHaveBeenCalledWith('zh');
            expect(mockContext.services.settings.setValue).toHaveBeenCalledWith('user', 'general.language', 'zh', undefined, { throwOnWriteFailure: true });
            expect(result).toEqual({
                type: 'message',
                messageType: 'info',
                content: expect.stringContaining('UI language changed'),
        ...
        it('zh-CN action should reject using both --project and --global together', async () => {
            ...
            const result = await zhCNSubcommand.action(mockContext, '--project --global');
            expect(i18n.setLanguageAsync).not.toHaveBeenCalled();
            expect(mockContext.services.settings.setValue).not.toHaveBeenCalled();
            expect(result).toMatchObject({
                messageType: 'error',
    ...
    describe('initializeLlmOutputLanguage', () => {
        ...
        it('should create file when it does not exist', () => {
            ...
            expect(fs.writeFileSync).toHaveBeenCalledWith(expect.stringContaining('output-language.md'), expect.stringContaining("Respond in the same language as the user's input."), 'utf-8');
            expect(i18n.detectSystemLanguage).not.toHaveBeenCalled();
        });
        it('should NOT overwrite existing file when content matches resolved language', () => {
            vi.mocked(fs.existsSync).mockReturnValue(true);
            vi.mocked(i18n.detectSystemLanguage).mockReturnValue('en');
            vi.mocked(fs.readFileSync).mockReturnValue(`# Output language preference: English
<!-- lailatul-coder:llm-output-language: English -->
`);
            initializeLlmOutputLanguage();
...
`);
        ...
        it('should normalize Russian locale and create Russian rule file', () => {
            ...
            expect(fs.writeFileSync).toHaveBeenCalledWith(expect.stringContaining('output-language.md'), expect.stringContaining('Russian'), 'utf-8');
        });
        it('should normalize German locale and create German rule file', () => {
            vi.mocked(fs.existsSync).mockReturnValue(false);
            initializeLlmOutputLanguage('de');
            expect(fs.writeFileSync).toHaveBeenCalledWith(expect.stringContaining('output-language.md'), expect.stringContaining('German'), 'utf-8');
        });
        it('should normalize Japanese locale and create Japanese rule file', () => {
            vi.mocked(fs.existsSync).mockReturnValue(false);
            initializeLlmOutputLanguage('ja');
            expect(fs.writeFileSync).toHaveBeenCalledWith(expect.stringContaining('output-language.md'), expect.stringContaining('Japanese'), 'utf-8');
        });
        it('should normalize Portuguese locale and create Portuguese rule file', () => {
            vi.mocked(fs.existsSync).mockReturnValue(false);
            initializeLlmOutputLanguage('pt');
            expect(fs.writeFileSync).toHaveBeenCalledWith(expect.stringContaining('output-language.md'), expect.stringContaining('Portuguese'), 'utf-8');
        });
    });
});
