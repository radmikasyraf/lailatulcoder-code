...
import { WorkspaceRegistrationStore, WorkspaceRegistrationStoreError, WorkspaceDisplayNameValidationError, getWorkspaceRegistrationStorePath, normalizeWorkspaceDisplayName, workspaceRegistrationId, workspaceRegistrationScopeHash, } from './workspace-registration-store.js';
const cleanup = [];
...
describe('WorkspaceRegistrationStore', () => {
    it('uses a full stable scope hash and returns an empty missing store', async () => {
        ...
        expect(getWorkspaceRegistrationStorePath('/work/primary', home)).toBe(path.join(home, 'daemon', 'workspaces', `${hash}.json`));
        await expect(new WorkspaceRegistrationStore('/work/primary', home).read()).resolves.toEqual({
            schemaVersion: 1,
    ...
    it('sets and clears display names for every matching registration id', async () => {
        ...
        await expect(store.setDisplayNameByIds([firstId, secondId, 'missing'], 'Shared')).resolves.toBe(2);
        await expect(store.read()).resolves.toMatchObject({
            displayNames: {
                [firstId]: 'Shared',
                [secondId]: 'Shared',
        ...
        await expect(store.setDisplayNameByIds([firstId, secondId])).resolves.toBe(2);
        await expect(store.read()).resolves.toMatchObject({
        ...
        await expect(store.setDisplayNameByIds(['missing'], 'Ignored')).resolves.toBe(0);
        await expect(store.read()).resolves.not.toHaveProperty('displayNames');
    });
    it('normalizes empty display names and rejects invalid values', () => {
        ...
        expect(normalizeWorkspaceDisplayName('x'.repeat(256))).toBe('x'.repeat(256));
        expect(normalizeWorkspaceDisplayName(`${'x'.repeat(256)} `)).toBe('x'.repeat(256));
        for (const invalid of [
            ...
            expect(() => normalizeWorkspaceDisplayName(invalid)).toThrow(WorkspaceDisplayNameValidationError);
        }
    });
    ...
    it.skipIf(process.platform !== 'win32')('deduplicates workspace paths case-insensitively on Windows', async () => {
        const home = await tempHome();
    ...
    it('removes multiple raw and canonical registration identities atomically', async () => {
        ...
        await expect(store.removeByIds([
    ...
    it('rejects invalid primary and primary-as-secondary inputs', async () => {
        const home = await tempHome();
        expect(() => new WorkspaceRegistrationStore('relative', home)).toThrow(/primaryWorkspace must be an absolute path/);
        const store = new WorkspaceRegistrationStore('/work/primary', home);
        await expect(store.add('/work/primary')).rejects.toThrow(/Primary workspace cannot be stored/);
    ...
    it('refuses a malformed store without overwriting it', async () => {
        ...
        await expect(store.add('/work/secondary')).rejects.toBeInstanceOf(WorkspaceRegistrationStoreError);
    ...
    it('refuses a primary-scope mismatch without overwriting it', async () => {
        ...
        await expect(store.add('/work/other')).rejects.toThrow(/primary does not match/);
    ...
    ])('rejects displayNames with %s', async (_description, displayNames, error) => {
        const home = await tempHome();
        const store = new WorkspaceRegistrationStore('/work/primary', home);
        await fs.mkdir(path.dirname(store.filePath), { recursive: true });
        await fs.writeFile(store.filePath, JSON.stringify({
    ...
    it('rejects additions after reaching the secondary workspace limit', async () => {
        ...
        const store = new WorkspaceRegistrationStore('/work/primary', home);
        await fs.mkdir(path.dirname(store.filePath), { recursive: true });
        const workspaces = Array.from({ length: 24 }, (_, index) => `/work/secondary-${index}`);
        await fs.writeFile(store.filePath, JSON.stringify({
            schemaVersion: 1,
            primaryWorkspace: '/work/primary',
            workspaces,
        }));
        await expect(store.add('/work/overflow')).rejects.toThrow(/limit of 24 reached/);
        await expect(store.read()).resolves.toMatchObject({ workspaces });
    });
    ...
    it('reports lock release failure after a committed write', async () => {
        ...
        try {
            ...
            const store = new storeModule.WorkspaceRegistrationStore('/work/primary', home);
            await fs.mkdir(path.dirname(store.filePath), { recursive: true });
            await fs.writeFile(store.filePath, JSON.stringify({
                schemaVersion: 1,
                primaryWorkspace: '/work/primary',
                workspaces: ['/work/secondary'],
                displayNames: {
                    [storeModule.workspaceRegistrationId('/work/secondary')]: 'Secondary',
                },
            }));
            await expect(store.removeByIds([
                storeModule.workspaceRegistrationId('/work/secondary'),
            ])).rejects.toBeInstanceOf(storeModule.WorkspaceRegistrationStoreCommittedError);
            expect(JSON.parse(await fs.readFile(store.filePath, 'utf8'))).toMatchObject({ workspaces: [] });
            expect(JSON.parse(await fs.readFile(store.filePath, 'utf8'))).not.toHaveProperty('displayNames');
        }
        finally {
...
//# sourceMappingURL=workspace-registration-store.test.js.map