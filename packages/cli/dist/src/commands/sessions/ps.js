 ...
 */
import { listLiveSessions, } from '@lailatul-coder/lailatul-coder-core';
import stringWidth from 'string-width';
import { sanitizeTerminalText, truncateToWidth, } from '../../ui/utils/textUtils.js';
...
function outputHuman(records, now) {
    writeStdoutLine(padDisplay('NAME', NAME_COL) +
        padDisplay('PID', PID_COL) +
        padDisplay('AGE', AGE_COL) +
        'DIRECTORY');
    for (const record of records) {
        writeStdoutLine(padDisplay(truncateToWidth(sanitize(record.name), NAME_COL - 2), NAME_COL) +
            padDisplay(String(record.pid), PID_COL) +
            padDisplay(formatAge(now - record.startedAt), AGE_COL) +
            sanitize(record.cwd));
    }
}
async function handlePs(argv) {
