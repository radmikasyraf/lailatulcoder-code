...
import { FatalConfigError, getErrorMessage, Storage, createDebugLogger, stripRuntimeSnapshotPrefix, } from '@lailatul-coder/lailatul-coder-core';
...
// from `~/.env` or `~/.lailatulcoder/.env` by `preResolveHomeEnvOverrides()` in
...
export function getSettingsWarnings(loadedSettings) {
    ...
    if (workspaceFile.rawJson !== undefined &&
        workspaceFile.originalSettings.security?.allowPrivateNetworkHooks !==
            undefined) {
        warningSet.add(`Warning: security.allowPrivateNetworkHooks in workspace settings (${workspaceFile.path}) is ignored. This setting is only honored from User, System, or SystemDefaults scope settings.`);
    }
    if (workspaceFile.rawJson !== undefined &&
        workspaceFile.originalSettings.security?.allowedInsecureVoiceBaseUrls !==
            undefined) {
        warningSet.add(`Warning: security.allowedInsecureVoiceBaseUrls in workspace settings (${workspaceFile.path}) is ignored. This setting is only honored from User, System, or SystemDefaults scope settings.`);
    }
    return [...warningSet];
}
/**
 ...
 */
function stripWorkspaceSecurityBypasses(settings) {
    if (settings.security?.allowPrivateNetworkHooks === undefined &&
        settings.security?.allowedInsecureVoiceBaseUrls === undefined) {
        return settings;
    }
    const { allowPrivateNetworkHooks: _privateHooks, allowedInsecureVoiceBaseUrls: _insecureVoice, ...restSecurity } = settings.security;
...
export function createMinimalSettings() {
    ...
    return new LoadedSettings(emptySettingsFile, emptySettingsFile, emptySettingsFile, emptySettingsFile, false, new Set(), [], undefined, false);
}
/**
...
export function loadSettings(workspaceDir = process.cwd(), consumeCorruptionEnvVars = true) {
    ...
    const homeEnvFallback = getHomeEnvFallbackVars((message) => debugLogger.warn(message));
