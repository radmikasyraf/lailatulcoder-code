...
import { recordAutoSkillCommandUsage, SkillCommandLoader, } from './SkillCommandLoader.js';
import { skillArgsPath } from './skill-args-file.js';
import { mkdtempSync, rmSync, readFileSync } from 'node:fs';
import { tmpdir } from 'node:os';
import { join } from 'node:path';
import { CommandKind } from '../ui/commands/types.js';
import { buildSkillLlmContent, } from '@lailatul-coder/lailatul-coder-core';
...
describe('SkillCommandLoader', () => {
    ...
    it('should return empty array in bare mode', async () => {
        mockConfig.getBareMode.mockReturnValue(true);
    ...
    it('should propagate argumentHint from skills to slash commands', async () => {
        ...
        mockSkillManager.listSkills.mockImplementation(({ level }) => Promise.resolve(level === 'user' ? [skill] : []));
        const loader = new SkillCommandLoader(mockConfig);
    ...
    it('should load project skill with sourceLabel "Project"', async () => {
        ...
        mockSkillManager.listSkills.mockImplementation(({ level }) => Promise.resolve(level === 'project' ? [skill] : []));
        const loader = new SkillCommandLoader(mockConfig);
        const commands = await loader.loadCommands(signal);
    ...
    it('keeps usage recording best-effort when persistence fails', async () => {
        ...
        await expect(recordAutoSkillCommandUsage(mockConfig, {
            name: 'my-skill',
            description: 'My skill',
            kind: CommandKind.SKILL,
            skillDetail: {
    ...
    it('should submit skill body as prompt', async () => {
        ...
        const loader = new SkillCommandLoader(mockConfig);
        const commands = await loader.loadCommands(signal);
        const result = await commands[0].action({ invocation: { raw: '/my-skill', args: '' } }, '');
    ...
    it('should append raw invocation when args are provided', async () => {
        ...
        try {
            ...
            const loader = new SkillCommandLoader(mockConfig);
            const commands = await loader.loadCommands(signal);
            const result = (await commands[0].action({ invocation: { raw: '/my-skill hello', args: 'hello' } }, 'hello'));
    ...
    describe('extension skills', () => {
        it('should be modelInvocable when description is present', async () => {
            ...
            mockSkillManager.listSkills.mockImplementation(({ level }) => Promise.resolve(level === 'extension' ? [skill] : []));
            const loader = new SkillCommandLoader(mockConfig);
            const commands = await loader.loadCommands(signal);
            expect(commands[0].modelInvocable).toBe(true);
    ...
    describe('skills.disabled filter', () => {
        it('omits disabled skills (case-insensitive) from the command list', async () => {
            ...
            mockConfig.getDisabledSkillNames.mockReturnValue(new Set(['hideme']));
            const loader = new SkillCommandLoader(mockConfig);
            const commands = await loader.loadCommands(signal);
        ...
        it('reflects provider mutations on each load (live read)', async () => {
            ...
            mockSkillManager.listSkills.mockImplementation(({ level }) => level === 'user'
                ? Promise.resolve([makeSkill({ name: 'foo', level: 'user' })])
                : Promise.resolve([]));
            let disabled = new Set();
            mockConfig.getDisabledSkillNames.mockImplementation(() => disabled);
            const loader = new SkillCommandLoader(mockConfig);
...
//# sourceMappingURL=SkillCommandLoader.test.js.map