...
export const lspCommand = {
    ...
    action: async (context, _args) => {
        ...
        if (!config.isLspEnabled()) {
            return emitMessage(context, 'info', t('LSP is not enabled. Start LailatulCoder Ai with `--experimental-lsp` to enable LSP support.'));
        }
        const client = config.getLspClient();
        if (!client) {
            return emitMessage(context, 'info', t('LSP is enabled but no client is connected. Check debug logs under `${QWEN_RUNTIME_DIR:-~/.lailatulcoder}/debug/` or see the LSP troubleshooting docs.'));
        }
