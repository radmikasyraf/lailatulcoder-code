...
import { hasUnmodeledWorkspaceGlob, readWorkspaceGlobs, readWorkspacePackages, } from './lib/workspaces.js';
/**
 ...
 */
const PLAN_NAME_RE = /(test\s*plan|\btesting\b|how\s+(?:has\s+this|to)\s+(?:been\s+)?test(?:ed)?|测试计划|测试方案|测试步骤)/i;
/**
 ...
 */
export function extractTestPlanSection(body) {
    ...
    for (let i = 0; i < lines.length; i++) {
        ...
        if (!name || !PLAN_NAME_RE.test(name))
            continue;
        ...
        for (let j = i + 1; j < lines.length; j++) {
            if (!fenced[j]) {
                ...
                if (next && next[1].length <= level)
                    break;
                if (!hash && (next || BOLD_LINE_RE.test(lines[j])))
                    break;
            }
            out.push(lines[j]);
        }
...
const RUNNER_RE = /^(npm|npx|yarn|pnpm|bun|make|node|go|cargo|python3?|pytest)\b/;
 ...
 */
function maskFileCounts(section) {
    let out = '';
    let cursor = 0;
...
function codeSpans(section) {
    ...
    const add = (line) => {
        ...
        if (/^(?:diff --git |\+\+\+ |--- |@@ |index )/.test(line.trim()))
            return;
    ...
    const fence = /(```|~~~)[^\n]*\n([\s\S]*?)\1/g;
    let m;
    while ((m = fence.exec(section)))
        m[2].split('\n').forEach(add);
    const inline = /`([^`\n]+)`/g;
    const outsideFences = section.replace(fence, ' ');
    while ((m = inline.exec(outsideFences)))
        add(m[1]);
    return spans;
}
/**
 ...
 */
export function extractClaims(section) {
    const claims = [];
    const seen = new Set();
    ...
    const isExcludedPath = (bare) => bare.startsWith('.lailatulcoder/') ||
    ...
    for (const span of codeSpans(section)) {
        ...
        if (/^(?:diff --git|---|\+\+\+|@@)\s/.test(span))
            continue;
        if (RUNNER_RE.test(span))
            push('command', span);
...
export function observedTestCounts(report) {
    if (!report)
        return [];
    const counts = [];
...
function normalizeClaimPath(text) {
 ...
 */
function isGitIgnoredCached(worktree, path) {
...
function rulePath(text, worktree, changed) {
    ...
    // commit BY CONSTRUCTION, the same reasoning that excludes `.lailatulcoder/` paths.
    if (ignored) {
        return {
            kind: 'path',
            text,
...
export function npmScriptOf(command) {
    ...
    const alias = /^(?:npm|pnpm|yarn)\s+(test|start|stop|restart)(?=\s|$)/.exec(command);
    return alias ? alias[1] : null;
}
function ruleCommand(text, worktree, buildTest) {
    ...
    const ran = matches.find((c) => !c.timedOut && c.exitCode !== 0) ??
    ...
    for (const dir of skipped) {
        try {
            const pkg = JSON.parse(readFileSync(join(worktree, dir, 'package.json'), 'utf8'));
    ...
    if (hasUnmodeledWorkspaceGlob(readWorkspaceGlobs(worktree))) {
        return {
            ...
            text,
            verdict: 'unchecked',
            note: 'the workspace globs use a shape this check does not model, so the ' +
    ...
    if (unreadable.length > 0) {
        return {
            ...
            text,
            verdict: 'unchecked',
            note: `${unreadable.join(', ')} ${unreadable.length === 1 ? 'has' : 'have'} a ` +
...
function ruleCount(text, observed) {
...
function fetchPrBody(ownerRepo, prNumber) {
    return gh('pr', 'view', prNumber, '--repo', ownerRepo, '--json', 'body', '--jq', '.body');
}
export function runTestPlan(args, fetchBody = fetchPrBody) {
    let plan;
    try {
        plan = JSON.parse(readFileSync(args.plan, 'utf8'));
    }
    catch (err) {
        throw new Error(`test-plan: cannot read the plan ${args.plan}: ${err.message}`);
    }
    ...
    try {
        ...
        return {
            ...
            claims: [],
            diffHash,
            note: `the PR description could not be fetched (${err.message.split('\n')[0]}); no Test Plan was checked`,
        };
    }
    ...
    const changed = new Set((plan.files ?? []).map((f) => normalize(String(f.path))));
    let buildTest = null;
    if (args.buildTest) {
        try {
            buildTest = JSON.parse(readFileSync(args.buildTest, 'utf8'));
        }
        catch {
    ...
    const claims = extractClaims(section.content).map((c) => {
        if (c.kind === 'path')
            return rulePath(c.text, worktree, changed);
        if (c.kind === 'command')
            return ruleCommand(c.text, worktree, buildTest);
...
export const testPlanCommand = {
    command: 'test-plan',
    describe: "Rule on the PR Test Plan's checkable claims (paths, npm scripts, test counts) against the reviewed tree",
    builder: (yargs) => yargs
        .option('plan', {
        type: 'string',
        demandOption: true,
        ...
        .option('build-test', {
        type: 'string',
        describe: "Agent 7's build-test report; supplies the observed test counts and exit codes",
    })
