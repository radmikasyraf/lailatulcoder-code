 ...
 */
import { APPROVAL_MODES, BTW_MAX_INPUT_LENGTH, createDebugLogger, GROUP_COLOR_OPTIONS, Storage, SessionService, SessionIdCaseConflictError, SessionOrganizationError, SESSION_WRITER_RPC_CODES, BuiltinAgentRegistry, SubagentError, WorkspaceMemoryFileTooLargeError, WorkspaceMemoryWriteTimeoutError, writeWorkspaceContextFile, IMAGE_CAPABILITY, } from '@lailatul-coder/lailatul-coder-core';
...
import { CancelSentinelCollisionError, InvalidPermissionOptionError, PermissionForbiddenError, PermissionPolicyNotImplementedError, SessionArchivingError, SessionConflictError, } from '../acp-session-bridge.js';
import { FsError } from '../fs/errors.js';
import { TooManyActiveDeviceFlowsError, UnsupportedDeviceFlowProviderError, UpstreamDeviceFlowError, } from '../auth/device-flow.js';
import { REQUESTED_SESSION_ID_META_KEY, } from '@lailatul-coder/acp-bridge/bridgeTypes';
...
import { translateAndCheckAbsoluteWorkspacePath, canonicalizeWorkspace, } from '@lailatul-coder/acp-bridge/workspacePaths';
import { SessionNotFoundError, SessionShellClientRequiredError, SessionShellDisabledError, WorkspaceMismatchError, } from '@lailatul-coder/acp-bridge/bridgeErrors';
import { SessionArtifactAuthorizationError, SessionArtifactValidationError, } from '@lailatul-coder/acp-bridge/sessionArtifacts';
import { writeStderrLine } from '../../utils/stdioHelpers.js';
import { MAX_WORKSPACE_PATH_LENGTH } from '../fs/paths.js';
import { MAX_READ_BYTES, MAX_TEXT_CURSOR_CHARS, } from '../fs/index.js';
import { isPermissionRuleType, normalizePermissionRules, PermissionRulesValidationError, readPermissionRuleSet, } from '../../config/permission-settings.js';
...
import { createSetupGithubFileOps, resolveSetupGithubProxy, sanitizeSetupGithubMessage, sanitizeSetupGithubResult, setupGithubEventData, } from '../routes/workspace-setup-github.js';
...
import { publicErrorMessage, publicErrorStatus, } from '../workspace-remember.js';
import { extractRememberErrorCode } from '../../runtime/workspace-remember-errors.js';
import { RequestedSessionIdAdmissionError, } from '../session-id-admission.js';
...
import { createDaemonSubagentManager, toSummary as agentToSummary, toDetail as agentToDetail, } from '../workspace-agents.js';
import { InvalidCursorError, invalidateWorkspaceSessionListCache, listWorkspaceSessionsForResponse, } from '../server.js';
import { createSessionOrganizationService } from '../session-organization-helpers.js';
import { archiveDaemonSessions, assertSessionLoadable, deleteDaemonSessionIfOrphan, deleteDaemonSessions, DaemonDrainingError, logSessionArchiveWarning, SessionArchiveCoordinator, unarchiveDaemonSessions, } from '../server/session-archive.js';
import { WorkspacePermissionRulesSessionRequiredError, WorkspaceSettingsPartialPersistError, } from '../workspace-service/types.js';
import { QWEN_META_KEY, QWEN_METHOD_NS, RPC, error, isNotification, isObject, isRequest, isResponse, logSafe, notification, request, success, } from './json-rpc.js';
function errMsg(err) {
...
const SESSION_WRITER_RPC_ERRORS = {
    session_writer_conflict: {
        ...
        message: 'This session is already open in another LailatulCoder process.',
    },
    session_writer_lost: {
...
function sessionWriterRpcError(err) {
...
const TRUSTED_WORKSPACE_METHODS = new Set([
    `${QWEN_METHOD_NS}workspace/mcp`,
    `${QWEN_METHOD_NS}workspace/skills`,
    `${QWEN_METHOD_NS}workspace/providers`,
    `${QWEN_METHOD_NS}workspace/env`,
...
const WORKSPACE_GENERATION_MUTATION_METHODS = new Set([
    'session/new',
    'session/load',
    'session/resume',
    'session/fork',
...
function advertisedQwenVendorMethods(sessionShellCommandEnabled) {
    return ALL_QWEN_VENDOR_METHODS.filter((method) => sessionShellCommandEnabled || method !== SESSION_SHELL_METHOD);
}
/**
 ...
 */
const CONN_ROUTED_METHODS = new Set([
    'authenticate',
    'session/new',
    'session/load',
    'session/resume',
 ...
 */
const HTTP_ACP_CONFIG_OPTION_IDS = ['model', 'mode'];
...
class RequestedSessionIdNotHonoredError extends Error {
    ...
    constructor(requestedSessionId, actualSessionId) {
        super(`The ACP agent returned session "${actualSessionId}" instead of requested session "${requestedSessionId}".`);
...
function parseOptionalSafeIntegerInRange(value, min, max) {
    if (value === undefined)
        return undefined;
    if (typeof value !== 'number' ||
        !Number.isSafeInteger(value) ||
...
export function parseOptionalWorkspaceCwd(params, boundWorkspace) {
    if (!('cwd' in params) || params['cwd'] === undefined)
    ...
    if (cwd.length > MAX_WORKSPACE_PATH_LENGTH) {
        throw new AcpParamError(`\`cwd\` exceeds the ${MAX_WORKSPACE_PATH_LENGTH}-character limit`);
    }
...
function validatePrompt(params) {
    const prompt = params['prompt'];
    if (!Array.isArray(prompt) || prompt.length === 0) {
        throw new AcpParamError('`prompt` is required and must be a non-empty array of content blocks');
    }
    if (!prompt.every((b) => typeof b === 'object' && b !== null && !Array.isArray(b))) {
        throw new AcpParamError('each `prompt` element must be an object');
    }
}
function parsePermissionResponse(params) {
    ...
    const cleanOutcome = outcome['outcome'] === 'cancelled'
        ? { outcome: 'cancelled' }
        : { outcome: 'selected', optionId: outcome['optionId'] };
    const response = { outcome: cleanOutcome };
    ...
    if (answers !== undefined) {
        if (isObject(answers) &&
            ...
            writeStderrLine('LailatulCoder serve: /acp session/permission dropping invalid `answers` (expected an object map of string values)');
        }
    }
...
function pickSessionArtifactInput(params) {
    const { title, kind, storage, description, workspacePath, managedId, url, mimeType, sizeBytes, metadata, retention, clientRetained, } = params;
    return {
        title,
        kind,
        storage,
...
export class AcpDispatcher {
    ...
    getSessionRuntimeContext;
    agentManager;
    constructor(bridge, boundWorkspace, getEnv, workspace, workspaceRememberLane, requestedSessionIdAdmission, fsFactory, deviceFlowRegistry, sessionShellCommandEnabled = false, registry, archiveCoordinator = new SessionArchiveCoordinator(), isWorkspaceTrusted = () => true, captureGenerationAssertion = () => undefined, liveSessionIsolation, sessionRuntimeBaseDir = Storage.getRuntimeBaseDir(), getSessionRuntimeContext = () => ({
        bridge,
        sessionRuntimeBaseDir,
    })) {
        this.bridge = bridge;
        ...
        this.requestedSessionIdAdmission = requestedSessionIdAdmission;
        ...
        this.captureGenerationAssertion = captureGenerationAssertion;
        this.liveSessionIsolation = liveSessionIsolation;
        this.sessionRuntimeBaseDir = sessionRuntimeBaseDir;
        this.getSessionRuntimeContext = getSessionRuntimeContext;
        this.agentManager = createDaemonSubagentManager(boundWorkspace);
    }
    invalidateSessionLists(archiveStates) {
    ...
    invalidateSessionListsAndMarkCatalog(archiveStates) {
        this.invalidateSessionLists(archiveStates);
        this.bridge.markSessionCatalogChanged();
    }
    async runWithSessionListInvalidation(archiveStates, mutation) {
        try {
    ...
    removeOrphanSession(sessionId, removePersistedSession = false, runtime = this.getSessionRuntimeContext()) {
        ...
        return cleanup.catch((err) => {
            writeStderrLine(`LailatulCoder serve: /acp orphan killSession(${logSafe(sessionId)}) failed: ${logSafe(errMsg(err))}`);
            return undefined;
        });
    }
    ownershipReceipt(conn, sessionId, clientId, attached, runtime, options = {}) {
        let settled = false;
        const ownershipIdentity = conn.captureSessionOwnershipIdentity(sessionId);
        const releaseIdentity = () => conn.releaseSessionOwnershipIdentity(sessionId, ownershipIdentity);
        ...
        const rollback = () => {
            ...
            if (attached) {
                ...
                try {
                    void this.removeOrphanSession(sessionId, options.removePersisted === true, runtime);
                }
                catch {
                    // Best-effort rollback; teardown must continue settling other receipts.
                }
    ...
    parseBoundWorkspaceParam(params) {
        const rawWorkspace = typeof params['workspaceCwd'] === 'string'
            ? params['workspaceCwd']
            : undefined;
        if (rawWorkspace === undefined) {
            return this.boundWorkspace;
        }
        const requestedWorkspace = canonicalizeWorkspace(parseOptionalWorkspaceCwd({ cwd: rawWorkspace }, this.boundWorkspace));
        if (requestedWorkspace !== this.boundWorkspace) {
            throw new WorkspaceMismatchError(this.boundWorkspace, requestedWorkspace);
        }
        return requestedWorkspace;
    }
    parseSessionWorkspaceCwd(params) {
        const requestedWorkspace = parseOptionalWorkspaceCwd(params, this.boundWorkspace);
    ...
    parseSessionIds(params) {
        const sessionIds = params['sessionIds'];
        if (!Array.isArray(sessionIds) ||
            sessionIds.length === 0 ||
            sessionIds.length > 100 ||
            !sessionIds.every((s) => typeof s === 'string')) {
            throw new AcpParamError('`sessionIds` must be non-empty string array (max 100)');
        }
        return [...new Set(sessionIds)];
    }
    rejectActiveLiveSessionMutation(conn, id, sessionIds) {
        const activeSessionId = sessionIds.find((sessionId) => this.liveSessionIsolation?.isSessionActive?.(sessionId));
        if (!activeSessionId)
            return false;
        if (id !== undefined) {
            conn.sendConn(error(id, RPC.INVALID_REQUEST, 'An active Live Voice session cannot be closed, deleted, or archived. Stop or replace the Live call first.', {
     ...
     */
    sessionCtx(conn, sessionId, fromLoopback) {
        const clientId = conn.sessions.get(sessionId)?.clientId;
        if (!clientId) {
            throw new Error(`no bridge-stamped clientId for session ${sessionId} (ownership invariant violated)`);
        }
     ...
     */
    async configOptionsFor(sessionId, bridge = this.bridge) {
        try {
            const ctx = (await bridge.getSessionContextStatus(sessionId));
            const co = ctx?.state?.configOptions;
            return Array.isArray(co)
                ? co.filter((option) => isObject(option) &&
                    HTTP_ACP_CONFIG_OPTION_IDS.includes(option['id']))
                : undefined;
        }
        catch (err) {
            writeStderrLine(`LailatulCoder serve: /acp configOptionsFor(${logSafe(sessionId)}) failed: ${logSafe(errMsg(err))}`);
            return undefined;
        }
    }
    /**
    ...
    extractModelState(configOptions) {
        if (!configOptions)
            return undefined;
        const modelOpt = configOptions.find((o) => typeof o === 'object' &&
            o !== null &&
            o['category'] === 'model');
        if (!modelOpt)
            return undefined;
    ...
    extractModeState(configOptions) {
        if (!configOptions)
            return undefined;
        const modeOpt = configOptions.find((o) => typeof o === 'object' &&
            o !== null &&
            o['category'] === 'mode');
        if (!modeOpt)
            return undefined;
     ...
     */
    cancelAbandonedPermission(req, clientId) {
        try {
            this.bridge.respondToSessionPermission(req.sessionId, req.bridgeRequestId, { outcome: { outcome: 'cancelled' } }, clientId !== undefined ? { clientId } : undefined);
            return true;
        }
        catch (err) {
            ...
            if (/not found|unknown session/i.test(msg))
                return true;
            writeStderrLine(`LailatulCoder serve: /acp cancelAbandonedPermission(${logSafe(req.sessionId)}) failed: ${logSafe(msg)}`);
            return false;
        }
    }
    /**
     ...
     */
    buildInitializeResult(connectionId, requestedVersion) {
        const requested = typeof requestedVersion === 'number' && Number.isFinite(requestedVersion)
            ? requestedVersion
            ...
            agentCapabilities: {
                ...
                _meta: {
                    [QWEN_META_KEY]: {
                        connectionId,
                        workspaceCwd: this.boundWorkspace,
                        methods: advertisedQwenVendorMethods(this.sessionShellCommandEnabled),
                    },
     ...
     */
    requireOwned(conn, sessionId, id) {
        ...
        if (id === undefined) {
            ...
            writeStderrLine(`LailatulCoder serve: /acp notification for unowned session ${logSafe(sessionId)} (dropped)`);
            return false;
        }
        conn.sendConn(error(id, RPC.INVALID_PARAMS, `Session ${sessionId} is not owned by this connection`));
        return false;
    }
    async withMutableOwned(conn, sessionId, id, fn) {
     ...
     */
    dropOwnPendingPermission(conn, requestId) {
     ...
     */
    async handle(conn, msg, sessionHeader, reqLoopback) {
        return Storage.runWithResolvedRuntimeBaseDir(this.sessionRuntimeBaseDir, () => this.handleInRuntime(conn, msg, sessionHeader, reqLoopback));
    }
    async handleInRuntime(conn, msg, sessionHeader, reqLoopback) {
        ...
        if (isResponse(msg)) {
            try {
                ...
                writeStderrLine(`LailatulCoder serve: /acp response handling error: ${logSafe(errMsg(err))}`);
            }
            return;
        }
        ...
        const params = {
            ...(isObject(msg.params) ? msg.params : {}),
        };
        ...
        const generationScoped = TRUSTED_WORKSPACE_METHODS.has(method) ||
        ...
        if (TRUSTED_WORKSPACE_METHODS.has(method) && !this.isWorkspaceTrusted()) {
            if (id !== undefined) {
                conn.sendConn(error(id, -32003, 'Workspace is not trusted.', {
        ...
        if (normalizedSessionHeader &&
            typeof params['sessionId'] === 'string' &&
            params['sessionId'] !== normalizedSessionHeader) {
            if (id !== undefined) {
                conn.sendConn(error(id, RPC.INVALID_PARAMS, 'Acp-Session-Id header does not match params.sessionId'));
            }
            return;
        }
        try {
            switch (method) {
                ...
                case 'session/new': {
                    if (id === undefined) {
                        writeStderrLine('LailatulCoder serve: /acp session/new notification rejected');
                        return;
                    }
                    const meta = isObject(params['_meta']) ? params['_meta'] : undefined;
                    const parsedSessionId = parseCallerSuppliedSessionId(meta?.[REQUESTED_SESSION_ID_META_KEY]);
                    if (parsedSessionId.kind === 'invalid') {
                        throw new InvalidRequestedSessionIdError(`\`_meta["${REQUESTED_SESSION_ID_META_KEY}"]\` must be an RFC UUID v1-v5`);
                    }
                    const requestedSessionId = parsedSessionId.kind === 'valid'
                        ? parsedSessionId.sessionId
                        : undefined;
                    const cwd = this.parseSessionWorkspaceCwd(params);
                    if (this.liveSessionIsolation) {
                        if (id !== undefined) {
                            conn.sendConn(error(id, RPC.INVALID_PARAMS, 'Sessions in the Conversations workspace can only be created by Live Voice.', { errorKind: 'live_session_creation_reserved' }));
                        }
                        return;
                    }
                    const sessionRuntime = this.getSessionRuntimeContext();
                    if (isReservedStandaloneSessionSource({
                        sourceType: typeof params['sourceType'] === 'string'
                            ? params['sourceType']
                            : undefined,
                    })) {
                        conn.sendConn(error(id, RPC.INVALID_PARAMS, 'The requested session source is reserved for daemon-owned standalone sessions.', { errorKind: 'reserved_session_source' }));
                        return;
                    }
                    ...
                    if (isReservedLiveSessionSource(source)) {
                        if (id !== undefined) {
                            conn.sendConn(error(id, RPC.INVALID_PARAMS, 'The requested session source is reserved for daemon-owned Live Voice sessions.'));
                        }
                        return;
                    }
                    const reservation = requestedSessionId
                        ? await this.requestedSessionIdAdmission.reserveCreate(requestedSessionId, {
                    ...
                    try {
                        ...
                        const ownership = this.ownershipReceipt(conn, session.sessionId, session.clientId, session.attached, sessionRuntime, { removePersisted: true });
                        try {
                            assertGenerationOpen?.();
                            if (requestedSessionId !== undefined &&
                                session.sessionId !== requestedSessionId) {
                                throw new RequestedSessionIdNotHonoredError(requestedSessionId, session.sessionId);
                            }
                            if (conn.destroyed) {
                                ownership.discarded();
                                return;
                            }
                            const configOptions = await this.configOptionsFor(session.sessionId, sessionRuntime.bridge);
                            assertGenerationOpen?.();
                            if (conn.destroyed) {
                                ownership.discarded();
                                return;
                ...
                case 'session/resume': {
                    if (id === undefined) {
                        writeStderrLine(`LailatulCoder serve: /acp ${method} notification rejected`);
                        return;
                    }
                    const sessionId = normalizeSessionIdForLookup(String(params['sessionId'] ?? ''));
                    if (!sessionId) {
                        if (id !== undefined) {
                            conn.sendConn(error(id, RPC.INVALID_PARAMS, '`sessionId` is required'));
                        }
                        return;
                    }
                    ...
                    if (conn.closingSessions.has(sessionId)) {
                        if (id !== undefined) {
                            ...
                            conn.sendConn(error(id, RPC.INTERNAL_ERROR, `session ${sessionId} is being closed; retry`));
                        }
                        return;
                    }
                    const cwd = this.parseSessionWorkspaceCwd(params);
                    ...
                    try {
                        ...
                        const restored = await this.archiveCoordinator.runSharedMany([sessionId], async () => {
                            assertGenerationOpen?.();
                            ...
                            await assertSessionLoadable(cwd, storageSessionId, sessionRuntime.sessionRuntimeBaseDir);
                            ...
                            const metadata = this.liveSessionIsolation
                                ? await readLoadableLiveConversationMetadata(storageSessionId, sessionService)
                            ...
                            const liveConversationCwd = this.liveSessionIsolation
                                ? await this.liveSessionIsolation.materializeConversationDirectory(sessionId)
                                : undefined;
                            assertGenerationOpen?.();
                            const session = method === 'session/load'
                            ...
                            if (session.hasActivePrompt) {
                                ...
                                try {
                                    if (session.clientId) {
                                        await sessionRuntime.bridge.detachClient(session.sessionId, session.clientId);
                                    }
                                }
                                catch {
                            ...
                            try {
                                const changed = await sessionRuntime.bridge.changeSessionCwd(sessionId, {
                                ...
                                try {
                                    if (session.attached && session.clientId) {
                                        ...
                                        await sessionRuntime.bridge.killSession(session.sessionId, {
                        ...
                        const initialReplayOnDelivery = method === 'session/load' && !conn.ownsSession(sessionId);
                        const ownership = this.ownershipReceipt(conn, sessionId, restored.clientId, restored.attached, sessionRuntime, { initialReplayPending: initialReplayOnDelivery });
                        try {
                            assertGenerationOpen?.();
                            // ACP standard: load/resume response includes configOptions + models + modes
                            const loadConfigOptions = await this.configOptionsFor(sessionId, sessionRuntime.bridge);
                            ...
                            const loadQwenMeta = isObject(loadMeta?.[QWEN_META_KEY])
                                ...
                                : undefined;
                            const replayStatus = method === 'session/load' && restored.partial === true
                                ? {
                                    partial: true,
                            ...
                            const replayArmRaced = !conn.destroyed &&
                                !closeRaced &&
                                method === 'session/load' &&
                            ...
                            if (conn.destroyed || closeRaced || replayArmRaced) {
                                ...
                                if ((closeRaced || replayArmRaced) && !conn.destroyed) {
                                    conn.sendConn(error(id, RPC.INTERNAL_ERROR, `session ${sessionId} was closed during load; retry`));
                                }
                                return;
                            }
                            this.replyOwnership(conn, id, {
                ...
                case 'session/list': {
                    ...
                    if (group !== undefined && view !== 'organized') {
                        throw new AcpParamError('`group` requires `view` to be "organized"');
                    }
                    const meta = isObject(params['_meta']) ? params['_meta'] : undefined;
                    const metaSize = typeof meta?.['size'] === 'number'
                        ? meta['size']
                        : undefined;
                    const rawArchiveState = typeof params['archiveState'] === 'string'
                        ? params['archiveState']
                    ...
                    if (rawArchiveState !== undefined) {
                        if (rawArchiveState !== 'active' &&
                            rawArchiveState !== 'archived') {
                            throw new AcpParamError('`archiveState` must be "active" or "archived"');
                        }
                    ...
                    if (parentSessionId !== undefined) {
                        if (parentSessionId.length === 0) {
                            throw new AcpParamError('`parentSessionId` must be a non-empty string');
                        }
                        if (view === 'organized') {
                            throw new AcpParamError('`parentSessionId` is not supported with `view` "organized"');
                        }
                    }
                    const parsedSource = parseSessionSource(params['sourceType'], params['sourceId']);
                    ...
                    try {
                        ...
                        result = await listWorkspaceSessionsForResponse(this.bridge, workspaceCwd, {
                            cursor,
                            size: metaSize,
                            archiveState,
                            view,
                ...
                case 'session/close': {
                    ...
                    const closeLocalSessionStream = () => {
                        try {
                            ...
                            writeStderrLine(`LailatulCoder serve: /acp session/close local teardown failed (${logSafe(sessionId)}): ${logSafe(errMsg(teardownErr))}`);
                        }
                    };
                    const closeSession = async () => {
                        closeStarted = true;
                        await this.bridge.closeSession(sessionId, this.sessionCtx(conn, sessionId, loopback));
                    };
                    try {
                        try {
                            await this.archiveCoordinator.runExclusiveMany([sessionId], closeSession);
                        }
                        catch (err) {
                            ...
                            if (err instanceof SessionArchivingError &&
                                ...
                                await this.archiveCoordinator.runSharedMany([sessionId], closeSession);
                            }
                            else {
                                throw err;
                            }
                ...
                case 'session/fork': {
                    if (id === undefined) {
                        writeStderrLine('LailatulCoder serve: /acp session/fork notification rejected');
                        return;
                    }
                    if (this.liveSessionIsolation) {
                        if (id !== undefined) {
                    ...
                    await this.withMutableOwned(conn, sessionId, id, async () => {
                        ...
                        assertGenerationOpen?.();
                        const result = (await sessionRuntime.bridge.branchSession(sessionId, {
                            name: typeof params['name'] === 'string'
                                ? params['name']
                                : undefined,
                        }, ctx));
                        const ownership = this.ownershipReceipt(conn, result.sessionId, result.clientId, result.attached, sessionRuntime, { removePersisted: true });
                        try {
                            assertGenerationOpen?.();
                            if (conn.destroyed) {
                                ownership.discarded();
                            ...
                            const configOptions = await this.configOptionsFor(result.sessionId, sessionRuntime.bridge);
                            assertGenerationOpen?.();
                            const models = this.extractModelState(configOptions);
                            const modes = this.extractModeState(configOptions);
                            this.replyOwnership(conn, id, {
                ...
                case 'session/cancel': {
                    ...
                    await this.withMutableOwned(conn, sessionId, id, async () => {
                        ...
                        { ...params, sessionId }, this.sessionCtx(conn, sessionId, loopback));
                ...
                case 'session/permission': {
                    ...
                    if (!requestId) {
                        ...
                        writeStderrLine(`LailatulCoder serve: /acp session/permission rejected: \`requestId\` is required`);
                        if (id !== undefined) {
                            conn.sendConn(error(id, RPC.INVALID_PARAMS, '`requestId` is required', {
                                httpStatus: 400,
                                requestId,
                            }));
                        }
                    ...
                    try {
                        ...
                        if (err instanceof AcpParamError) {
                            writeStderrLine(`LailatulCoder serve: /acp session/permission invalid params (requestId ${logSafe(requestId)}): ${logSafe(err.message)}`);
                            if (id !== undefined) {
                    ...
                    const sessionIdParam = typeof params['sessionId'] === 'string' &&
                    ...
                    if (sessionIdParam &&
                        ...
                        writeStderrLine(`LailatulCoder serve: /acp session/permission vote rejected: requestId ${logSafe(requestId)} does not belong to session ${logSafe(sessionIdParam)}`);
                        if (id !== undefined) {
                            conn.sendConn(error(id, RPC.INVALID_PARAMS, 'requestId does not belong to the specified session', { httpStatus: 409, sessionId: sessionIdParam, requestId }));
                        }
                        return;
                    }
                    ...
                    if (this.registry && !pendingRef) {
                        writeStderrLine(`LailatulCoder serve: /acp session/permission vote dropped: no pending request for requestId ${logSafe(requestId)}`);
                        if (id !== undefined) {
                            conn.sendConn(error(id, RPC.INVALID_PARAMS, 'No pending permission request', {
                                httpStatus: 404,
                                requestId,
                            }));
                        }
                    ...
                    // this connection).
                    if (!conn.ownsSession(sessionId)) {
                        writeStderrLine(`LailatulCoder serve: /acp session/permission vote rejected: session ${logSafe(sessionId)} not owned by this connection (requestId ${logSafe(requestId)})`);
                        if (id !== undefined) {
                            conn.sendConn(error(id, RPC.INVALID_PARAMS, 'Session not owned by this connection', { httpStatus: 403, sessionId, requestId }));
                        }
                        return;
                    }
                    let accepted;
                    try {
                        accepted = this.bridge.respondToSessionPermission(sessionId, requestId, response, this.sessionCtx(conn, sessionId, loopback));
                    }
                    catch (err) {
                        ...
                        if (err instanceof InvalidPermissionOptionError) {
                            writeStderrLine(`LailatulCoder serve: /acp session/permission invalid option (${logSafe(sessionId)}, requestId ${logSafe(requestId)}): ${logSafe(err.message)}`);
                            if (id !== undefined) {
                                conn.sendConn(error(id, RPC.INVALID_PARAMS, err.message, {
                                    httpStatus: 400,
                        ...
                        if (err instanceof PermissionForbiddenError) {
                            writeStderrLine(`LailatulCoder serve: /acp session/permission forbidden (${logSafe(sessionId)}, requestId ${logSafe(requestId)}): ${logSafe(err.message)}`);
                            if (id !== undefined) {
                                conn.sendConn(error(id, RPC.INVALID_PARAMS, err.message, {
                                    httpStatus: 403,
                        ...
                        if (err instanceof PermissionPolicyNotImplementedError) {
                            ...
                            writeStderrLine(`LailatulCoder serve: /acp session/permission policy not implemented (${logSafe(sessionId)}, requestId ${logSafe(requestId)}): ${logSafe(err.message)}`);
                            if (id !== undefined) {
                                conn.sendConn(error(id, RPC.INTERNAL_ERROR, err.message, {
                                    httpStatus: 501,
                        ...
                        if (err instanceof CancelSentinelCollisionError) {
                            ...
                            writeStderrLine(`LailatulCoder serve: /acp session/permission cancel-sentinel collision (${logSafe(sessionId)}, requestId ${logSafe(requestId)}): ${logSafe(err.message)}`);
                            if (id !== undefined) {
                                conn.sendConn(error(id, RPC.INTERNAL_ERROR, err.message, {
                                    httpStatus: 500,
                        ...
                        // error for the wire.
                        writeStderrLine(`LailatulCoder serve: /acp session/permission vote failed (${logSafe(sessionId)}, requestId ${logSafe(requestId)}): ${logSafe(errMsg(err))}`);
                        const cancelled = this.cancelAbandonedPermission({ sessionId, bridgeRequestId: requestId }, pendingRef?.conn.sessions.get(sessionId)?.clientId);
                    ...
                    if (!accepted) {
                        ...
                        writeStderrLine(`LailatulCoder serve: /acp session/permission vote not accepted by bridge (${logSafe(sessionId)}, requestId ${logSafe(requestId)})`);
                        if (id !== undefined) {
                            conn.sendConn(error(id, RPC.INVALID_PARAMS, 'Permission vote not accepted (request already resolved)', {
                                httpStatus: 409,
                                sessionId,
                                requestId,
                            }));
                    ...
                    writeStderrLine(`LailatulCoder serve: /acp session/permission vote accepted (${logSafe(sessionId)}, requestId ${logSafe(requestId)}, connection ${logSafe(conn.connectionId.slice(0, 8))})`);
                    this.replyConn(conn, id, {});
                    return;
                }
                ...
                case 'session/set_config_option': {
                    ...
                    await this.withMutableOwned(conn, sessionId, id, async () => {
                        ...
                        if (typeof rawValue !== 'string' || rawValue.length === 0) {
                            if (id !== undefined) {
                                this.replySession(conn, sessionId, id, undefined, error(id, RPC.INVALID_PARAMS, '`value` must be a non-empty string'));
                            }
                            return;
                        }
                        if (configId === 'model') {
                            await this.bridge.setSessionModel(sessionId, { modelId: rawValue }, ctx);
                        }
                        else if (configId === 'mode') {
                            if (!APPROVAL_MODES.includes(rawValue)) {
                                if (id !== undefined) {
                                    this.replySession(conn, sessionId, id, undefined, error(id, RPC.INVALID_PARAMS, `invalid mode "${rawValue}" (expected one of: ${APPROVAL_MODES.join(', ')})`));
                                }
                                return;
                            }
                            await this.bridge.setSessionApprovalMode(sessionId, rawValue, { persist: params['persist'] === true }, ctx);
                        }
                        else {
                            // Ids advertised by raw-state surfaces but unroutable here
                            // (e.g. reasoning_effort) must fail loud, not fall into mode.
                            if (id !== undefined) {
                                this.replySession(conn, sessionId, id, undefined, error(id, RPC.INVALID_PARAMS, `ConfigId not supported by this transport: ${configId} (supported: ${HTTP_ACP_CONFIG_OPTION_IDS.join(', ')})`));
                            }
                            return;
                        }
                ...
                case 'session/set_mode': {
                    ...
                    await this.withMutableOwned(conn, sessionId, id, async () => {
                        ...
                        if (!modeId || !APPROVAL_MODES.includes(modeId)) {
                            if (id !== undefined) {
                                this.replySession(conn, sessionId, id, undefined, error(id, RPC.INVALID_PARAMS, `invalid modeId "${modeId}" (expected one of: ${APPROVAL_MODES.join(', ')})`));
                            }
                            return;
                        }
                        const ctx = this.sessionCtx(conn, sessionId, loopback);
                        await this.bridge.setSessionApprovalMode(sessionId, modeId, { persist: false }, ctx);
                        this.replySession(conn, sessionId, id, {});
                    });
                    return;
                }
                ...
                case 'session/set_model': {
                    ...
                    await this.withMutableOwned(conn, sessionId, id, async () => {
                        ...
                        if (!modelId) {
                            if (id !== undefined) {
                                this.replySession(conn, sessionId, id, undefined, error(id, RPC.INVALID_PARAMS, '`modelId` is required'));
                            }
                            return;
                        }
                        const ctx = this.sessionCtx(conn, sessionId, loopback);
                        await this.bridge.setSessionModel(sessionId, { modelId, sessionId }, ctx);
                        this.replySession(conn, sessionId, id, {});
                    });
                    return;
                }
                case `${QWEN_METHOD_NS}session/heartbeat`: {
                    ...
                    if (!this.requireOwned(conn, sessionId, id))
                        return;
                    const result = this.bridge.recordHeartbeat(sessionId, this.sessionCtx(conn, sessionId, loopback));
                ...
                case `${QWEN_METHOD_NS}session/context`: {
                    ...
                    if (!this.requireOwned(conn, sessionId, id))
                        return;
                    this.replyConn(conn, id, await this.bridge.getSessionContextStatus(sessionId));
                    return;
                }
                case `${QWEN_METHOD_NS}session/supported_commands`: {
                    ...
                    if (!this.requireOwned(conn, sessionId, id))
                        return;
                    this.replyConn(conn, id, await this.bridge.getSessionSupportedCommandsStatus(sessionId));
                    return;
                }
                case `${QWEN_METHOD_NS}session/update_metadata`: {
                    ...
                    await this.withMutableOwned(conn, sessionId, id, async () => {
                        ...
                        let result;
                        try {
                            result = this.bridge.updateSessionMetadata(sessionId, metadata, this.sessionCtx(conn, sessionId, loopback));
                        }
                        finally {
                            this.invalidateSessionLists(['active']);
                        }
                ...
                case `${QWEN_METHOD_NS}session/update_organization`: {
                    ...
                    if ('color' in params &&
                        ...
                        (typeof params['color'] !== 'string' ||
                            !GROUP_COLOR_OPTIONS.includes(params['color']))) {
                        throw new AcpParamError('`color` must be a supported color or null');
                    }
                    await this.archiveCoordinator.runSharedMany([sessionId], async () => {
                        ...
                        const organization = await createSessionOrganizationService(this.boundWorkspace).updateSessionOrganization(sessionId, {
                ...
                case `${QWEN_METHOD_NS}workspace/session_groups/list`: {
                    const workspaceCwd = this.parseBoundWorkspaceParam(params);
                    const groups = await createSessionOrganizationService(workspaceCwd).listGroups();
                    this.replyConn(conn, id, groups);
                    return;
                }
                case `${QWEN_METHOD_NS}workspace/session_groups/create`: {
                    const workspaceCwd = this.parseBoundWorkspaceParam(params);
                    const group = await createSessionOrganizationService(workspaceCwd).createGroup({
                        name: params['name'],
                        color: params['color'],
                    });
                    this.invalidateSessionListsAndMarkCatalog(['active', 'archived']);
                ...
                case `${QWEN_METHOD_NS}workspace/session_groups/update`: {
                    ...
                    const group = await createSessionOrganizationService(workspaceCwd).updateGroup(groupId, {
                ...
                case `${QWEN_METHOD_NS}workspace/session_groups/delete`: {
                    ...
                    const deleted = await createSessionOrganizationService(workspaceCwd).deleteGroup(groupId);
                ...
                case `${QWEN_METHOD_NS}workspace/mcp`:
                    {
                        const result = await this.workspace.getWorkspaceMcpStatus(this.wsCtx(conn, method));
                        assertGenerationOpen?.();
                        this.replyConn(conn, id, result);
                    }
                    return;
                case `${QWEN_METHOD_NS}workspace/skills`:
                    {
                        const result = await this.workspace.getWorkspaceSkillsStatus(this.wsCtx(conn, method));
                        assertGenerationOpen?.();
                        this.replyConn(conn, id, result);
                    }
                    return;
                case `${QWEN_METHOD_NS}workspace/providers`:
                    {
                        const result = await this.workspace.getWorkspaceProvidersStatus(this.wsCtx(conn, method));
                        assertGenerationOpen?.();
                        this.replyConn(conn, id, result);
                    }
                    return;
                case `${QWEN_METHOD_NS}workspace/env`:
                    {
                        const result = await this.workspace.getWorkspaceEnvStatus(this.wsCtx(conn, method));
                        assertGenerationOpen?.();
                        this.replyConn(conn, id, result);
                    }
                    return;
                case `${QWEN_METHOD_NS}workspace/preflight`:
                    {
                        const result = await this.workspace.getWorkspacePreflightStatus(this.wsCtx(conn, method));
                        assertGenerationOpen?.();
                        this.replyConn(conn, id, result);
                    }
                    return;
                case `${QWEN_METHOD_NS}workspace/init`: {
                    ...
                    if (rawForce !== undefined && typeof rawForce !== 'boolean') {
                        if (id !== undefined) {
                            conn.sendConn(error(id, RPC.INVALID_PARAMS, '`force` must be a boolean when provided'));
                        }
                        return;
                    }
                    const force = rawForce === true;
                    const result = await this.workspace.initWorkspace(this.wsCtx(conn, method), { force });
                    assertGenerationOpen?.();
                    this.replyConn(conn, id, result);
                    return;
                }
                case `${QWEN_METHOD_NS}workspace/trust`: {
                    const result = await this.workspace.getWorkspaceTrustStatus(this.wsCtx(conn, method));
                    this.replyConn(conn, id, result);
                    return;
                }
                case `${QWEN_METHOD_NS}workspace/trust/request`: {
                    ...
                    if (desiredState !== 'trusted' && desiredState !== 'untrusted') {
                        if (id !== undefined) {
                            conn.sendConn(error(id, RPC.INVALID_PARAMS, '`desiredState` must be "trusted" or "untrusted"'));
                        }
                        return;
                    }
                    ...
                    if (reason !== undefined &&
                        ...
                        if (id !== undefined) {
                            conn.sendConn(error(id, RPC.INVALID_PARAMS, `\`reason\` must be a string up to ${MAX_TRUST_REASON_LENGTH} chars`));
                        }
                        return;
                    }
                    ...
                    if (!status.folderTrustEnabled) {
                        if (id !== undefined) {
                            conn.sendConn(error(id, RPC.INVALID_REQUEST, 'Folder trust is disabled for this workspace'));
                        }
                        return;
                    }
                    assertGenerationOpen?.();
                ...
                case `${QWEN_METHOD_NS}workspace/permissions`: {
                    const result = await this.workspace.getWorkspacePermissionsStatus(this.wsCtx(conn, method));
                    assertGenerationOpen?.();
                    this.replyConn(conn, id, result);
                    return;
                }
                case `${QWEN_METHOD_NS}workspace/permissions/set`: {
                    ...
                    if (scope !== 'user' && scope !== 'workspace') {
                        if (id !== undefined) {
                            conn.sendConn(error(id, RPC.INVALID_PARAMS, '`scope` must be "user" or "workspace"'));
                        }
                        return;
                    }
                    ...
                    if (!isPermissionRuleType(ruleType)) {
                        if (id !== undefined) {
                            conn.sendConn(error(id, RPC.INVALID_PARAMS, '`ruleType` must be "allow", "ask", or "deny"'));
                        }
                        return;
                    }
                    let rules;
                    ...
                    let result;
                    try {
                        result = await this.workspace.setWorkspacePermissionRules(this.wsCtx(conn, method), { scope, ruleType, rules });
                    }
                    catch (err) {
                ...
                case `${QWEN_METHOD_NS}workspace/voice`: {
                    const result = await this.workspace.getWorkspaceVoiceStatus(this.wsCtx(conn, method));
                    assertGenerationOpen?.();
                    this.replyConn(conn, id, result);
                    return;
                }
                case `${QWEN_METHOD_NS}workspace/voice/set`: {
                    ...
                    const result = await this.workspace.setWorkspaceVoiceSettings(this.wsCtx(conn, method), update);
                    assertGenerationOpen?.();
                    this.replyConn(conn, id, result);
                    return;
                }
                case `${QWEN_METHOD_NS}workspace/setup-github`: {
                    if (params['consent'] !== true) {
                        if (id !== undefined) {
                            conn.sendConn(error(id, RPC.INVALID_PARAMS, '`consent` must be true', {
                    ...
                    if (!this.fsFactory) {
                        if (id !== undefined) {
                            conn.sendConn(error(id, RPC.INTERNAL_ERROR, 'File system not configured', {
                    ...
                    try {
                        const fileOps = createSetupGithubFileOps(this.fsFactory, `ACP ${method}`, conn.clientId, assertGenerationOpen);
                        fileOps.assertCanWrite();
                        const result = await setupGithub({
                            cwd: this.boundWorkspace,
                            workspaceRoot: this.boundWorkspace,
                            proxy: resolveSetupGithubProxy(this.boundWorkspace, this.getEnv(), true),
                        ...
                        if (err instanceof SetupGithubError && id !== undefined) {
                            conn.sendConn(error(id, err.status >= 500 ? RPC.INTERNAL_ERROR : RPC.INVALID_PARAMS, sanitizeSetupGithubMessage(err.message, this.boundWorkspace), {
                                errorKind: err.code,
                                ...(err.partial
                                    ? {
                                        partial: true,
                                        result: err.partialResult
                                            ? sanitizeSetupGithubResult(err.partialResult, this.boundWorkspace)
                                            : null,
                                    }
                                    : {}),
                            }));
                ...
                case `${QWEN_METHOD_NS}workspace/set_tool_enabled`: {
                    ...
                    if (!toolName || toolName.length > MAX_NAME_LENGTH) {
                        if (id !== undefined) {
                            conn.sendConn(error(id, RPC.INVALID_PARAMS, `\`toolName\` is required and must be ≤ ${MAX_NAME_LENGTH} chars`));
                        }
                        return;
                    }
                    const result = await this.workspace.setWorkspaceToolEnabled(this.wsCtx(conn, method), toolName, params['enabled'] === true);
                    assertGenerationOpen?.();
                    this.replyConn(conn, id, result);
                    return;
                }
                case `${QWEN_METHOD_NS}workspace/restart_mcp_server`: {
                    ...
                    if (!serverName || serverName.length > MAX_NAME_LENGTH) {
                        if (id !== undefined) {
                            conn.sendConn(error(id, RPC.INVALID_PARAMS, `\`serverName\` is required and must be ≤ ${MAX_NAME_LENGTH} chars`));
                        }
                        return;
                    }
                    ...
                    if (rawIdx !== undefined &&
                        (typeof rawIdx !== 'number' ||
                            ...
                            rawIdx < 0)) {
                        if (id !== undefined) {
                            conn.sendConn(error(id, RPC.INVALID_PARAMS, '`entryIndex` must be a non-negative integer'));
                        }
                        return;
                    }
                    const result = await this.workspace.restartMcpServer(this.wsCtx(conn, method), serverName, rawIdx !== undefined ? { entryIndex: rawIdx } : undefined);
                    assertGenerationOpen?.();
                    this.replyConn(conn, id, result);
                    return;
                }
                ...
                case `${QWEN_METHOD_NS}session/recap`: {
                    const sessionId = String(params['sessionId'] ?? '');
                    await this.withMutableOwned(conn, sessionId, id, async () => {
                        const result = await this.bridge.generateSessionRecap(sessionId, this.sessionCtx(conn, sessionId, loopback));
                        this.replyConn(conn, id, result);
                    });
                    return;
                }
                case `${QWEN_METHOD_NS}session/btw`: {
                    ...
                    await this.withMutableOwned(conn, sessionId, id, async () => {
                        ...
                        if (typeof rawQ !== 'string' ||
                            ...
                            if (id !== undefined)
                                conn.sendConn(error(id, RPC.INVALID_PARAMS, `\`question\` required, non-empty, max ${BTW_MAX_INPUT_LENGTH} chars`));
                            return;
                        }
                        const result = await this.bridge.generateSessionBtw(sessionId, rawQ.trim(), undefined, this.sessionCtx(conn, sessionId, loopback));
                        this.replyConn(conn, id, result);
                    });
                    return;
                }
                case `${QWEN_METHOD_NS}session/shell`: {
                    ...
                    await this.withMutableOwned(conn, sessionId, id, async () => {
                        ...
                        if (!clientId) {
                            if (id !== undefined) {
                                conn.sendConn(rpcErrorFrame(id, new SessionShellClientRequiredError()));
                            }
                            return;
                        }
                        ...
                        if (typeof rawCmd !== 'string' || rawCmd.trim().length === 0) {
                            if (id !== undefined)
                                conn.sendConn(error(id, RPC.INVALID_PARAMS, '`command` required and must be non-empty'));
                            return;
                        }
                        ...
                        writeStderrLine(`LailatulCoder serve: /acp session/shell session=${logSessionId} client=${logClientId} cmd=${logCommand}`);
                        const result = await this.bridge.executeShellCommand(sessionId, rawCmd, binding.abort.signal, this.sessionCtx(conn, sessionId, loopback));
                        this.replyConn(conn, id, result);
                    });
                    return;
                }
                ...
                case `${QWEN_METHOD_NS}session/context_usage`: {
                    ...
                    if (!this.requireOwned(conn, sessionId, id))
                        return;
                    const result = await this.bridge.getSessionContextUsageStatus(sessionId, { detail: params['detail'] === true });
                    this.replyConn(conn, id, result);
                    return;
                }
                ...
                case `${QWEN_METHOD_NS}session/artifacts`: {
                    ...
                    if (!this.requireOwned(conn, sessionId, id))
                        return;
                    const result = await this.bridge.getSessionArtifacts(sessionId, this.sessionCtx(conn, sessionId, loopback));
                    this.replyConn(conn, id, result);
                    return;
                }
                case `${QWEN_METHOD_NS}session/artifacts/add`: {
                    const sessionId = String(params['sessionId'] ?? '');
                    await this.withMutableOwned(conn, sessionId, id, async () => {
                        const result = await this.bridge.addSessionArtifact(sessionId, pickSessionArtifactInput(params), this.sessionCtx(conn, sessionId, loopback));
                        this.replyConn(conn, id, result);
                    });
                    return;
                }
                case `${QWEN_METHOD_NS}session/artifacts/remove`: {
                    ...
                    await this.withMutableOwned(conn, sessionId, id, async () => {
                        ...
                        if (!artifactId) {
                            if (id !== undefined) {
                                conn.sendConn(error(id, RPC.INVALID_PARAMS, '`artifactId` is required'));
                            }
                            return;
                        }
                        const result = await this.bridge.removeSessionArtifact(sessionId, artifactId, this.sessionCtx(conn, sessionId, loopback));
                        this.replyConn(conn, id, result);
                    });
                    return;
                }
                case `${QWEN_METHOD_NS}workspace/memory`: {
                    const result = await collectWorkspaceMemoryStatus(this.boundWorkspace);
                    assertGenerationOpen?.();
                    this.replyConn(conn, id, result);
                    return;
                }
                case `${QWEN_METHOD_NS}workspace/memory/write`: {
                    ...
                    if (typeof content !== 'string') {
                        if (id !== undefined)
                            conn.sendConn(error(id, RPC.INVALID_PARAMS, '`content` required, must be string'));
                        return;
                    }
                    if (Buffer.byteLength(content, 'utf8') > 1024 * 1024) {
                        if (id !== undefined)
                            conn.sendConn(error(id, RPC.INVALID_PARAMS, '`content` exceeds 1MB limit'));
                        return;
                    }
                    ...
                    if (rawScope !== undefined &&
                        ...
                        rawScope !== 'global') {
                        if (id !== undefined)
                            conn.sendConn(error(id, RPC.INVALID_PARAMS, '`scope` must be "workspace" or "global"'));
                        return;
                    }
                    ...
                    if (rawMode !== undefined &&
                        ...
                        rawMode !== 'replace') {
                        if (id !== undefined)
                            conn.sendConn(error(id, RPC.INVALID_PARAMS, '`mode` must be "append" or "replace"'));
                        return;
                    }
                    const mode = rawMode ?? 'append';
                    writeStderrLine(`LailatulCoder serve: /acp workspace/memory/write scope=${scope} mode=${mode} client=${conn.clientId?.slice(0, 8)} bytes=${Buffer.byteLength(content, 'utf8')}`);
                ...
                case `${QWEN_METHOD_NS}workspace/memory/remember`: {
                    ...
                    if (typeof content !== 'string' || !content.trim()) {
                        if (id !== undefined) {
                            conn.sendConn(error(id, RPC.INVALID_PARAMS, '`content` must be a non-empty string'));
                        }
                        return;
                    }
                    if (Buffer.byteLength(content, 'utf8') > MAX_REMEMBER_CONTENT_BYTES) {
                        if (id !== undefined) {
                            conn.sendConn(error(id, RPC.INVALID_PARAMS, `\`content\` exceeds the ${MAX_REMEMBER_CONTENT_BYTES}-byte limit`));
                        }
                        return;
                    }
                    ...
                    if (rawContextMode !== 'workspace' && rawContextMode !== 'clean') {
                        if (id !== undefined) {
                            conn.sendConn(error(id, RPC.INVALID_PARAMS, '`contextMode` must be "workspace", "clean", or omitted'));
                        }
                        return;
                    }
                    try {
                        const available = await this.bridge.isWorkspaceMemoryRememberAvailable();
                        assertGenerationOpen?.();
                        if (!available) {
                            if (id !== undefined) {
                                conn.sendConn(error(id, -32009, 'Managed memory is unavailable for this daemon workspace', {
                        ...
                        if (id !== undefined) {
                            conn.sendConn(error(id, -32099, publicErrorMessage(code, 'remember'), {
                                errorKind: code,
                                httpStatus: publicErrorStatus(code),
                            }));
                        }
                        else {
                            debugLogger.warn('workspace memory remember notification failed:', err);
                        }
                    }
                    return;
                }
                case `${QWEN_METHOD_NS}workspace/memory/remember/get`: {
                    ...
                    const task = this.workspaceRememberLane.get(taskId, conn.clientId, 'remember');
                    if (!task) {
                        if (id !== undefined) {
                            conn.sendConn(error(id, -32004, 'Workspace memory remember task not found', {
                ...
                case `${QWEN_METHOD_NS}workspace/memory/forget`: {
                    ...
                    if (!trimmedQuery) {
                        if (id !== undefined) {
                            conn.sendConn(error(id, RPC.INVALID_PARAMS, '`query` must be a non-empty string'));
                        }
                        return;
                    }
                    if (Buffer.byteLength(trimmedQuery, 'utf8') > MAX_REMEMBER_CONTENT_BYTES) {
                        if (id !== undefined) {
                            conn.sendConn(error(id, RPC.INVALID_PARAMS, `\`query\` exceeds the ${MAX_REMEMBER_CONTENT_BYTES}-byte limit`));
                        }
                        return;
                    }
                    try {
                        ...
                        if (id !== undefined) {
                            conn.sendConn(error(id, -32099, publicErrorMessage(code, 'forget'), {
                                errorKind: code,
                                httpStatus: publicErrorStatus(code),
                            }));
                        }
                        else {
                            debugLogger.warn('workspace memory forget notification failed:', err);
                        }
                    }
                    return;
                }
                case `${QWEN_METHOD_NS}workspace/memory/forget/get`: {
                    ...
                    const task = this.workspaceRememberLane.get(taskId, conn.clientId, 'forget');
                    if (!task) {
                        if (id !== undefined) {
                            conn.sendConn(error(id, -32004, 'Workspace memory forget task not found', {
                ...
                case `${QWEN_METHOD_NS}workspace/memory/dream`: {
                    try {
                        ...
                        if (id !== undefined) {
                            conn.sendConn(error(id, -32099, publicErrorMessage(code, 'dream'), {
                                errorKind: code,
                                httpStatus: publicErrorStatus(code),
                            }));
                        }
                        else {
                            debugLogger.warn('workspace memory dream notification failed:', err);
                        }
                    }
                    return;
                }
                case `${QWEN_METHOD_NS}workspace/memory/dream/get`: {
                    ...
                    const task = this.workspaceRememberLane.get(taskId, conn.clientId, 'dream');
                    if (!task) {
                        if (id !== undefined) {
                            conn.sendConn(error(id, -32004, 'Workspace memory dream task not found', {
                ...
                case `${QWEN_METHOD_NS}file/read`: {
                    ...
                    const maxBytes = parseOptionalSafeIntegerInRange(params['maxBytes'], 1, MAX_READ_BYTES);
                    if (maxBytes === null) {
                        if (id !== undefined)
                            conn.sendConn(error(id, RPC.INVALID_PARAMS, `\`maxBytes\` must be a positive integer in [1, ${MAX_READ_BYTES}]`));
                        return;
                    }
                    const line = parseOptionalSafeIntegerInRange(params['line'], 1, Number.MAX_SAFE_INTEGER);
                    if (line === null) {
                        if (id !== undefined)
                            conn.sendConn(error(id, RPC.INVALID_PARAMS, '`line` must be a positive integer'));
                        return;
                    }
                    const limit = parseOptionalSafeIntegerInRange(params['limit'], 1, MAX_FILE_LINE_LIMIT);
                    if (limit === null) {
                        if (id !== undefined)
                            conn.sendConn(error(id, RPC.INVALID_PARAMS, `\`limit\` must be a positive integer in [1, ${MAX_FILE_LINE_LIMIT}]`));
                        return;
                    }
                    ...
                    if (rawCursor !== undefined &&
                        ...
                        if (id !== undefined)
                            conn.sendConn(error(id, RPC.INVALID_PARAMS, `\`cursor\` must be a non-empty string of at most ${MAX_TEXT_CURSOR_CHARS} characters`));
                        return;
                    }
                ...
                case `${QWEN_METHOD_NS}file/read_bytes`: {
                    ...
                    const offset = parseOptionalSafeIntegerInRange(params['offset'], 0, Number.MAX_SAFE_INTEGER);
                    if (offset === null) {
                        if (id !== undefined)
                            conn.sendConn(error(id, RPC.INVALID_PARAMS, '`offset` must be a non-negative safe integer'));
                        return;
                    }
                    const maxBytes = parseOptionalSafeIntegerInRange(params['maxBytes'], 1, MAX_READ_BYTES);
                    if (maxBytes === null) {
                    ...
                    this.replyConn(conn, id, { path: p, ...buf });
                    return;
                }
                case `${QWEN_METHOD_NS}file/stat`: {
                    ...
                    this.replyConn(conn, id, { path: p, ...result });
                    return;
                }
                ...
                case `${QWEN_METHOD_NS}file/glob`: {
                    ...
                    if (!pattern) {
                        if (id !== undefined)
                            conn.sendConn(error(id, RPC.INVALID_PARAMS, '`pattern` required'));
                        return;
                    }
                    if (!this.fsFactory) {
                        if (id !== undefined)
                    ...
                    const maxResults = parseOptionalPositiveInteger(params['maxResults'], DEFAULT_FILE_GLOB_MAX_RESULTS, MAX_FILE_GLOB_MAX_RESULTS);
                    if (maxResults === null) {
                        if (id !== undefined)
                            conn.sendConn(error(id, RPC.INVALID_PARAMS, '`maxResults` must be an integer between 1 and 50000'));
                        return;
                    }
                ...
                case `${QWEN_METHOD_NS}file/write`: {
                    ...
                    if (typeof params['content'] !== 'string') {
                        if (id !== undefined)
                            conn.sendConn(error(id, RPC.INVALID_PARAMS, '`content` must be string'));
                        return;
                    }
                    if (!this.fsFactory) {
                        if (id !== undefined)
                    ...
                    if (Buffer.byteLength(params['content'], 'utf8') >
                        10 * 1024 * 1024) {
                        if (id !== undefined)
                            conn.sendConn(error(id, RPC.INVALID_PARAMS, 'content exceeds 10MB limit'));
                        return;
                    }
                    await fs.writeTextOverwrite(resolved, params['content']);
                    assertGenerationOpen?.();
                ...
                case `${QWEN_METHOD_NS}file/edit`: {
                    ...
                    if (typeof params['oldText'] !== 'string' ||
                        ...
                        if (id !== undefined)
                            conn.sendConn(error(id, RPC.INVALID_PARAMS, '`oldText` and `newText` must be strings'));
                        return;
                    }
                    if (!this.fsFactory) {
                        if (id !== undefined)
                    ...
                    const resolved = await fs.resolve(p, 'write');
                    const result = await fs.edit(resolved, params['oldText'], params['newText']);
                    assertGenerationOpen?.();
                    this.replyConn(conn, id, { ok: true, path: p, ...result });
                    return;
                }
                ...
                case `${QWEN_METHOD_NS}workspace/auth/device_flow/start`: {
                    if (!this.deviceFlowRegistry) {
                        if (id !== undefined)
                            conn.sendConn(error(id, RPC.INTERNAL_ERROR, 'Device flow not configured'));
                        return;
                    }
                    const providerId = String(params['providerId'] ?? '');
                    if (!providerId) {
                        if (id !== undefined)
                            conn.sendConn(error(id, RPC.INVALID_PARAMS, '`providerId` required'));
                        return;
                    }
                    ...
                    const gated = view.initiatorClientId === conn.clientId
                        ? view
                        : {
                    ...
                    this.replyConn(conn, id, { view: gated, attached });
                    return;
                }
                case `${QWEN_METHOD_NS}workspace/auth/device_flow/get`: {
                    ...
                    if (!view) {
                        if (id !== undefined)
                            conn.sendConn(error(id, RPC.INVALID_PARAMS, `Device flow "${flowId}" not found`));
                        return;
                    }
                    const gated = view.initiatorClientId === conn.clientId
                        ? view
                ...
                case `${QWEN_METHOD_NS}workspace/auth/device_flow/cancel`: {
                    ...
                    if (flowView &&
                        ...
                        if (id !== undefined)
                            conn.sendConn(error(id, RPC.INVALID_PARAMS, 'Only the flow initiator can cancel'));
                        return;
                    }
                    const cancelResult = this.deviceFlowRegistry.cancel(flowId, conn.clientId);
                    if (!cancelResult) {
                        if (id !== undefined)
                            conn.sendConn(error(id, RPC.INVALID_PARAMS, `Device flow "${flowId}" not found`));
                        return;
                ...
                case `${QWEN_METHOD_NS}workspace/mcp/tools`: {
                    ...
                    if (!serverName) {
                        if (id !== undefined)
                            conn.sendConn(error(id, RPC.INVALID_PARAMS, '`serverName` required'));
                        return;
                    }
                    const result = await this.bridge.getWorkspaceMcpToolsStatus(serverName);
                    assertGenerationOpen?.();
                    this.replyConn(conn, id, result);
                    return;
                }
                case `${QWEN_METHOD_NS}workspace/mcp/resources`: {
                    ...
                    const result = await this.bridge.getWorkspaceMcpResourcesStatus(serverName);
                    assertGenerationOpen?.();
                    this.replyConn(conn, id, result);
                    return;
                }
                case `${QWEN_METHOD_NS}workspace/mcp/servers/add`: {
                    ...
                    if (!name || name.length > MAX_NAME_LENGTH) {
                        if (id !== undefined)
                            conn.sendConn(error(id, RPC.INVALID_PARAMS, `\`name\` required, max ${MAX_NAME_LENGTH} chars`));
                        return;
                    }
                    const config = params['config'];
                    if (!config || typeof config !== 'object' || Array.isArray(config)) {
                        if (id !== undefined)
                            conn.sendConn(error(id, RPC.INVALID_PARAMS, '`config` required, must be object'));
                        return;
                    }
                    const result = await this.bridge.addRuntimeMcpServer(name, config, conn.clientId);
                    assertGenerationOpen?.();
                    this.replyConn(conn, id, result);
                    return;
                }
                case `${QWEN_METHOD_NS}workspace/mcp/servers/remove`: {
                    ...
                    const result = await this.bridge.removeRuntimeMcpServer(name, conn.clientId);
                    assertGenerationOpen?.();
                    this.replyConn(conn, id, result);
                    return;
                }
                case `${QWEN_METHOD_NS}sessions/delete`: {
                    ...
                    if (this.rejectActiveLiveSessionMutation(conn, id, ids))
                        return;
                    const svc = new SessionService(this.boundWorkspace);
                    const result = await this.runWithSessionListInvalidation(['active', 'archived'], () => deleteDaemonSessions({
                        sessionIds: ids,
                        service: svc,
                        bridge: this.bridge,
                        coordinator: this.archiveCoordinator,
                        onError: ({ phase, sessionId, error }) => {
                            ...
                            writeStderrLine(`LailatulCoder serve: /acp sessions/delete ${phase}Session(${safeSessionId}) failed: ${safeMessage}`);
                        },
                    }));
                    this.replyConn(conn, id, result);
                    return;
                ...
                case `${QWEN_METHOD_NS}sessions/archive`: {
                    ...
                    const result = await this.runWithSessionListInvalidation(['active', 'archived'], () => archiveDaemonSessions({
                        sessionIds: ids,
                        service: svc,
                        bridge: this.bridge,
                        coordinator: this.archiveCoordinator,
                ...
                case `${QWEN_METHOD_NS}sessions/unarchive`: {
                    ...
                    const result = await this.runWithSessionListInvalidation(['active', 'archived'], () => unarchiveDaemonSessions({
                        sessionIds: ids,
                        service: svc,
                        coordinator: this.archiveCoordinator,
                    }));
                ...
                case `${QWEN_METHOD_NS}workspace/agents/get`: {
                    ...
                    if (!agentType) {
                        if (id !== undefined)
                            conn.sendConn(error(id, RPC.INVALID_PARAMS, '`agentType` required'));
                        return;
                    }
                    ...
                    if (!config) {
                        if (id !== undefined)
                            conn.sendConn(error(id, RPC.INVALID_PARAMS, `Agent "${agentType}" not found`));
                        return;
                    }
                    this.replyConn(conn, id, agentToDetail(config));
                    return;
                }
                case `${QWEN_METHOD_NS}workspace/agents/create`: {
                    ...
                    const level = scope === 'workspace' ? 'project' : 'user';
                    if (BuiltinAgentRegistry.isBuiltinAgent(name)) {
                        if (id !== undefined)
                            conn.sendConn(error(id, RPC.INVALID_PARAMS, `Cannot shadow built-in agent "${name}"`));
                        return;
                    }
                    const collision = await this.agentManager.loadSubagent(name, level);
                    if (collision) {
                        if (id !== undefined)
                            conn.sendConn(error(id, RPC.INVALID_PARAMS, `Agent "${name}" already exists`));
                        return;
                    }
                    assertGenerationOpen?.();
                ...
                case `${QWEN_METHOD_NS}workspace/agents/update`: {
                    ...
                    if (typeof params['description'] === 'string') {
                        if (Buffer.byteLength(params['description'], 'utf8') > MAX_FIELD_BYTES) {
                            if (id !== undefined)
                                conn.sendConn(error(id, RPC.INVALID_PARAMS, '`description` exceeds 256KB limit'));
                            return;
                        }
                        updates['description'] = params['description'];
                    }
                    if (typeof params['systemPrompt'] === 'string') {
                        if (Buffer.byteLength(params['systemPrompt'], 'utf8') >
                            MAX_FIELD_BYTES) {
                            if (id !== undefined)
                                conn.sendConn(error(id, RPC.INVALID_PARAMS, '`systemPrompt` exceeds 256KB limit'));
                            return;
                        }
                    ...
                    if (Array.isArray(params['tools'])) {
                        if (params['tools'].length > 256) {
                            if (id !== undefined)
                                conn.sendConn(error(id, RPC.INVALID_PARAMS, '`tools` exceeds 256-entry limit'));
                            return;
                        }
                        if (!params['tools'].every((t) => typeof t === 'string' && t.length <= 256)) {
                            if (id !== undefined)
                                conn.sendConn(error(id, RPC.INVALID_PARAMS, '`tools` elements must be strings ≤256 chars'));
                            return;
                        }
                    ...
                    if (Object.keys(updates).length === 0) {
                        if (id !== undefined)
                            conn.sendConn(error(id, RPC.INVALID_PARAMS, 'at least one updatable field required'));
                        return;
                    }
                    assertGenerationOpen?.();
                    await this.agentManager.updateSubagent(agentType, updates, existing.level, { assertCanCommit: assertGenerationOpen });
                    assertGenerationOpen?.();
                    const updated = await this.agentManager.loadSubagent(agentType, existing.level);
                    assertGenerationOpen?.();
                    this.replyConn(conn, id, {
                        ok: true,
                ...
                case `${QWEN_METHOD_NS}workspace/agents/delete`: {
                    ...
                    const level = scope === 'workspace'
                        ? 'project'
                        : scope === 'global'
                            ? 'user'
                            : undefined;
                    const existing = await this.agentManager.loadSubagent(agentType, level);
                    if (!existing) {
                        if (id !== undefined)
                            conn.sendConn(error(id, RPC.INVALID_PARAMS, `Agent "${agentType}" not found`));
                        return;
                    ...
                    assertGenerationOpen?.();
                    await this.agentManager.deleteSubagent(agentType, existing.level, undefined, { assertCanCommit: assertGenerationOpen });
                    assertGenerationOpen?.();
                    this.replyConn(conn, id, { ok: true });
                    try {
                        this.bridge.publishWorkspaceEvent({
                ...
                default:
                    if (id !== undefined) {
                        conn.sendConn(error(id, RPC.METHOD_NOT_FOUND, `Unknown method: ${method}`));
                    }
                    return;
            }
        }
            ...
            writeStderrLine(`LailatulCoder serve: /acp dispatch error (${logSafe(method)}): ${logSafe(errMsg(err))}`);
            if (id !== undefined) {
                const { code, message, data } = toRpcError(err);
     ...
     * write). The `/acp` GET route advertises it as `X-LailatulCoder-Event-Epoch`
     ...
     */
    async pumpSessionEvents(conn, sessionId, signal, lastEventId, epoch) {
        try {
            ...
            if (conn.hasInitialReplayPending(sessionId)) {
                ...
                if (snapshot) {
                    if (snapshot.degraded) {
                        ...
                        writeStderrLine(`LailatulCoder serve: /acp initial replay used a DEGRADED snapshot (compaction failure) session=${logSafe(sessionId)}; replay may be incomplete`);
                    }
                    ...
                    writeStderrLine(`LailatulCoder serve: /acp initial replay skipped (no snapshot) session=${logSafe(sessionId)}`);
            ...
            for await (const event of iterable) {
                ...
                if (event.type === 'replay_complete') {
                    ...
                    const replayedCount = event.data
                        ?.replayedCount;
                    writeStderrLine(`LailatulCoder serve: /acp replay complete (${logSafe(sessionId)}) ` +
            ...
            if (!signal.aborted) {
                ...
                conn.flushBufferedSessionFrames(sessionId);
                conn.sendSession(sessionId, notification(`${QWEN_METHOD_NS}notify`, {
                    kind: 'stream_error',
                    error: errMsg(err),
                }));
            }
    ...
    translateEvent(conn, sessionId, event) {
        switch (event.type) {
            case 'session_update': {
                ...
                conn.sendSession(sessionId, notification('session/update', event.data), event.id);
                return;
            }
            case 'permission_request': {
                ...
                if (!binding?.stream || binding.stream.isClosed) {
                    ...
                    if (binding?.graceTimer) {
                        writeStderrLine(`LailatulCoder serve: /acp permission cancel during reconnect grace ` +
                            `(${logSafe(sessionId)}); vote not deferred (see §1.7 follow-up)`);
                    }
                    const cancelled = this.cancelAbandonedPermission({ sessionId, bridgeRequestId: data.requestId }, 
                    ...
                    if (!cancelled) {
                        writeStderrLine(`LailatulCoder serve: /acp permission cancel FAILED for ${logSafe(sessionId)} (mediator stuck; no automatic recovery)`);
                    }
                    return;
                }
                ...
                void binding.stream.send(request(id, 'session/request_permission', {
            ...
            default: {
                ...
                // model_switched / approval_mode_changed / … → opaque LailatulCoder notify.
     ...
     */
    resolveClientResponse(conn, msg, fromLoopback) {
        ...
        if (pendingConn !== conn && !conn.ownsSession(pending.sessionId)) {
            ...
            writeStderrLine(`LailatulCoder serve: /acp permission vote dropped: responding connection ${logSafe(conn.connectionId.slice(0, 8))} does not own session ${logSafe(pending.sessionId)} (requestId ${logSafe(pending.bridgeRequestId)})`);
            return;
        }
        ...
        try {
            ...
            const vote = 'error' in msg
                ? { outcome: { outcome: 'cancelled' } }
                : parsePermissionResponse(isObject(msg.result)
                    ? msg.result
                    : {});
            this.bridge.respondToSessionPermission(pending.sessionId, pending.bridgeRequestId, vote, this.sessionCtx(conn, pending.sessionId, fromLoopback));
            this.dropResolvedPermission(pendingConn, id);
        }
        catch (err) {
            writeStderrLine(`LailatulCoder serve: /acp permission vote failed (${logSafe(pending.sessionId)}): ${logSafe(errMsg(err))}`);
            ...
            const cancelled = this.cancelAbandonedPermission(pending, pendingConn.sessions.get(pending.sessionId)?.clientId);
            if (cancelled)
                this.dropResolvedPermission(pendingConn, id);
        }
    }
    async handlePrompt(conn, sessionId, id, params, fromLoopback) {
        ...
        binding.promptAbort = abort;
        try {
            const result = await this.bridge.sendPrompt(sessionId, 
            ...
            params, abort.signal, this.sessionCtx(conn, sessionId, fromLoopback));
            if (id !== undefined)
                this.replySession(conn, sessionId, id, result);
        }
        catch (err) {
            const { code, message, data } = toRpcError(err);
            if (id !== undefined) {
                this.replySession(conn, sessionId, id, undefined, error(id, code, message, data));
            }
            else {
                // Notification-form prompt (no id): no response frame to send, so a
                // failure would vanish silently — log it for the operator.
                writeStderrLine(`LailatulCoder serve: /acp prompt error (${logSafe(sessionId)}, notification): ${logSafe(errMsg(err))}`);
            }
        }
        finally {
            if (binding.promptAbort === abort)
                binding.promptAbort = undefined;
        }
    }
    replyConn(conn, id, result, receipt) {
        if (id === undefined) {
            receipt?.discarded();
        ...
        void delivery.then((outcome) => {
            if (outcome === 'failed' && !conn.destroyed) {
                void conn.sendConn(error(id, RPC.INTERNAL_ERROR, 'Response delivery failed'));
            }
        }, () => undefined);
        return delivery;
    }
    replyOwnership(conn, id, result, receipt) {
        void this.replyConn(conn, id, result, receipt);
    }
    replySession(conn, sessionId, id, result, errorFrame) {
        if (id === undefined)
            return;
        ...
        if (conn.sessions.has(sessionId)) {
            ...
            try {
                ...
                anchorId = undefined;
                writeStderrLine(`LailatulCoder serve: /acp replySession(${logSafe(sessionId)}) ` +
            ...
            void delivery.then((outcome) => {
                if ((outcome === 'failed' || outcome === 'closed') &&
                    !conn.destroyed) {
                    void conn.sendConn(error(id, RPC.INTERNAL_ERROR, 'Response delivery failed'));
                }
            }, () => undefined);
            ...
            // session teardown.
            writeStderrLine(`LailatulCoder serve: /acp replySession(${logSafe(sessionId)}) binding gone mid-flight, ` +
