 ...
 */
// `LailatulCoder review fetch-diff`: write a PR's full unified diff to a file. This
...
export function runFetchDiff(args) {
...
export const fetchDiffCommand = {
    ...
    handler: (argv) => {
        ...
        if (prNumber === undefined ||
            ...
            prNumber <= 0) {
            writeStderrLineSafe(`fetch-diff: pr_number must be a positive integer, got ${JSON.stringify(argv['pr_number'])}`);
            process.exitCode = 2;
            return;
        }
        ...
        try {
            ...
            writeStderrLineSafe(`fetch-diff: ${err.message}`);
            process.exitCode = err instanceof TypeError ? 2 : 1;
        }
    },
};
//# sourceMappingURL=fetch-diff.js.map