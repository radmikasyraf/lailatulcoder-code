...
describe('summaryCommand custom export path', () => {
    ...
    const run = async (args) => (await summaryCommand.action?.(makeContext(projectRoot), args));
    ...
    it('overwrites a hand-written file at the default path', async () => {
        ...
        await fs.writeFile(path.join(qwenDir, 'PROJECT_SUMMARY.md'), 'hand-written notes', 'utf8');
        const result = await run('');
        expect(result).toMatchObject({ type: 'message', messageType: 'info' });
        const written = await fs.readFile(path.join(qwenDir, 'PROJECT_SUMMARY.md'), 'utf8');
        expect(written).toContain('SUMMARY BODY');
    ...
    it('treats a relative path with a trailing separator as a directory', async () => {
        ...
        expect(await fileExists(path.join(projectRoot, 'docs', 'PROJECT_SUMMARY.md'))).toBe(true);
    ...
    it('appends the default filename for an existing directory', async () => {
        ...
        expect(await fileExists(path.join(projectRoot, 'existingdir', 'PROJECT_SUMMARY.md'))).toBe(true);
    ...
    it.skipIf(process.platform === 'win32')('allows a symlink that resolves inside the project root', async () => {
        await fs.mkdir(path.join(projectRoot, 'real-dir'));
        await fs.symlink(path.join(projectRoot, 'real-dir'), path.join(projectRoot, 'internal-link'));
        const result = await run('internal-link/summary.md');
        expect(result).toMatchObject({ type: 'message', messageType: 'info' });
        expect(await fileExists(path.join(projectRoot, 'real-dir', 'summary.md'))).toBe(true);
    });
    it.skipIf(process.platform === 'win32')('rejects a path that escapes the project root via a symlink', async () => {
        const outside = await fs.mkdtemp(path.join(os.tmpdir(), 'summary-outside-'));
        try {
    ...
    it.skipIf(process.platform === 'win32')('rejects a broken symlink whose target is outside the project root', async () => {
        const outsideTarget = path.join(os.tmpdir(), `summary-broken-${Date.now()}`, 'leak.md');
    ...
    it.skipIf(process.platform === 'win32')('rejects a multi-link symlink chain that escapes the project root', async () => {
        const outsideTarget = path.join(os.tmpdir(), `summary-chain-${Date.now()}`, 'evil.md');
    ...
    it.skipIf(process.platform === 'win32')('rejects a symlink with a relative target escaping the project root', async () => {
        const outsideDir = await fs.mkdtemp(path.join(os.tmpdir(), 'summary-outside-'));
        try {
            // Git stores symlink targets verbatim, so a committed
            // `ln -s ../outside/leak.md` recreates a relative target on checkout.
            await fs.symlink(path.join('..', path.basename(outsideDir), 'leak.md'), path.join(projectRoot, 'rel-link'));
    ...
    it.skipIf(process.platform === 'win32')('rejects a directory whose appended default filename is a symlink escaping the project root', async () => {
        const outside = await fs.mkdtemp(path.join(os.tmpdir(), 'summary-outside-'));
        try {
            const docsDir = path.join(projectRoot, 'docs');
            await fs.mkdir(docsDir);
            await fs.symlink(path.join(outside, 'evil-target.md'), path.join(docsDir, 'PROJECT_SUMMARY.md'));
            const result = await run('docs');
            expect(result).toMatchObject({
                type: 'message',
                messageType: 'error',
            ...
            expect(result.content).toContain('within the project root');
            expect(await fileExists(path.join(outside, 'evil-target.md'))).toBe(false);
            expect(runSideQuery).not.toHaveBeenCalled();
        }
        finally {
            await fs.rm(outside, { recursive: true, force: true });
    ...
    it.skipIf(process.platform === 'win32')('rejects a symlink cycle', async () => {
    ...
    it('refuses to overwrite a file that merely mentions Summary Metadata in prose', async () => {
        ...
        await fs.writeFile(target, 'The summary file ends with a `## Summary Metadata` footer.\n', 'utf8');
        const result = await run('DESIGN.md');
        expect(result).toMatchObject({ type: 'message', messageType: 'error' });
        expect(result.content).toContain('already exists');
        expect(await fs.readFile(target, 'utf8')).toContain('`## Summary Metadata`');
        expect(runSideQuery).not.toHaveBeenCalled();
    });
    ...
    it('overwrites a previously generated summary', async () => {
        ...
        await fs.writeFile(target, 'old body\n\n---\n\n## Summary Metadata\n**Update time**: old\n', 'utf8');
    ...
    it.skipIf(process.platform === 'win32')('creates a custom-path summary with mode 0o600', async () => {
    ...
    it.skipIf(process.platform === 'win32')('allows the default target when .qwen is a symlink', async () => {
        const outside = await fs.mkdtemp(path.join(os.tmpdir(), 'summary-shared-'));
        try {
            ...
            expect(await fileExists(path.join(outside, 'PROJECT_SUMMARY.md'))).toBe(true);
        }
        finally {
            await fs.rm(outside, { recursive: true, force: true });
        }
    ...
    it('rejects a non-summary file created during generation', async () => {
        vi.mocked(runSideQuery).mockImplementationOnce(async () => {
            await fs.writeFile(path.join(projectRoot, 'race.md'), 'precious content', 'utf8');
            return { text: 'SUMMARY BODY' };
        });
        const result = await run('race.md');
        expect(result).toMatchObject({ type: 'message', messageType: 'error' });
        expect(result.content).toContain('already exists');
        expect(await fs.readFile(path.join(projectRoot, 'race.md'), 'utf8')).toBe('precious content');
    });
    it.skipIf(process.platform === 'win32')('rejects a symlink planted at the target during generation', async () => {
        const outside = await fs.mkdtemp(path.join(os.tmpdir(), 'summary-toctou-'));
        try {
            vi.mocked(runSideQuery).mockImplementationOnce(async () => {
    ...
    it.skipIf(process.platform === 'win32')('applies 0o700 to .qwen/ when spelled explicitly', async () => {
        const result = await run('.qwen/');
        expect(result).toMatchObject({ type: 'message', messageType: 'info' });
        const stat = await fs.stat(path.join(projectRoot, '.qwen'));
        expect(stat.mode & 0o777).toBe(0o700);
    });
    it.skipIf(process.platform === 'win32')('preserves existing file permissions on regeneration', async () => {
        const target = path.join(projectRoot, 'summary.md');
        await fs.writeFile(target, 'old body\n\n---\n\n## Summary Metadata\n**Update time**: old\n', 'utf8');
    ...
    it('refuses to overwrite a file that embeds a full footer mid-document', async () => {
        ...
        await fs.writeFile(target, 'intro\n\n---\n\n## Summary Metadata\n**Update time**: old\n\ntrailing prose\n', 'utf8');
    ...
    it('detects the footer in a file larger than the tail window', async () => {
        ...
        await fs.writeFile(target, `${padding}\n\n---\n\n## Summary Metadata\n**Update time**: old\n`, 'utf8');
    ...
    it('returns empty content in interactive mode errors to avoid double rendering', async () => {
        ...
        const result = (await summaryCommand.action?.(context, '../outside/leak.md'));
        expect(result).toMatchObject({ type: 'message', messageType: 'error' });
        expect(result.content).toBe('');
        expect(vi.mocked(context.ui.addItem)).toHaveBeenCalledWith(expect.objectContaining({ type: 'error' }), expect.any(Number));
    });
});
//# sourceMappingURL=summaryCommand.test.js.map