 ...
 */
let cachedBootId;
export function readLocalBootId() {
 ...
 * share one `~/.lailatulcoder` (host + devcontainer with a mounted home, sibling
 ...
 */
export function readPidNamespaceId() {
    if (process.platform !== 'linux')
        return null;
    try {
 ...
 */
export function isSameProcess(pid, procStart) {
    if (!isPidAlive(pid))
        return false;
    if (procStart == null)
        return true;
