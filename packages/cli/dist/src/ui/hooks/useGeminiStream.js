...
import { runOutsideAgentContext, GeminiEventType as ServerGeminiEventType, SendMessageType, createDebugLogger, ToolNames, getErrorMessage, isNodeError, MessageSenderType, logUserPrompt, logUserRetry, UnauthorizedError, UserPromptEvent, UserRetryEvent, logConversationFinishedEvent, ConversationFinishedEvent, ApprovalMode, parseAndFormatApiError, promptIdContext, ToolConfirmationOutcome, logApiCancel, ApiCancelEvent, detectAutonomousSentinel, isSupportedImageMimeType, getUnsupportedImageFormatWarning, runVisionBridge, shouldRunVisionBridge, formatVisionBridgeNotice, formatFullTurnVisionNotice, getFullTurnVisionModelSelector, hasImageParts, clampInlineMediaPart, splitImageParts, generateToolUseSummary, goalRequiresExactPermit, createDuplicateProviderToolCallResponse, markDuplicateProviderToolCallResponseSent, findRepeatedDuplicateProviderToolCall, getCachedToolCallFingerprint, isReplayOfHandledToolCall, recordHandledToolCall, AutonomousLoopTickResolver, didWriteProjectContextFile, refreshMemoryAfterManagedWrite, refreshMemoryInstruction, finalizeToolResponses, endInteractionSpan, getActiveInteractionSpan, } from '@lailatul-coder/lailatul-coder-core';
...
import { handleAtCommand, resolveAtCommandQuery, } from './atCommandProcessor.js';
import { findLastSafeSplitPoint, splitFencedMarkdown, getEnclosingFenceInfo, } from '../utils/markdownUtilities.js';
...
import { getInlineImageData, MAX_INLINE_IMAGES_PER_ITEM, } from '../utils/inline-image-parts.js';
...
function applyModelOverride(modelOverrideRef, inlineActiveRef, value, isInline) {
    ...
    debugLogger.debug(`model override ${value === undefined ? 'cleared' : `set to ${value}`} (inline=${isInline})`);
}
...
const MID_TURN_AT_COMMAND_RESOLVE_TIMEOUT_MESSAGE = 'Mid-turn @ command resolution timed out';
function sameGoalPermit(left, right) {
    return (left.goalId === right.goalId &&
        left.revision === right.revision &&
        left.turnId === right.turnId);
}
function sharedGoalPermit(contexts) {
...
async function resolveWithAbort(signal, run) {
    let onAbort;
...
function clampLoadingThoughtDescription(description) {
...
terminalWidthRef, midTurnRestoreRef, goalQueueRef) => {
    const [initError, setInitError] = useState(null);
    const abortControllerRef = useRef(null);
    const activeInteractionPromptIdRef = useRef(undefined);
    const activeInteractionOwnerRef = useRef(undefined);
    const activeGoalTurnRef = useRef(null);
    const activeGoalAdmissionRef = useRef(null);
    const goalTurnBindingsRef = useRef(new Map());
    const bindGoalTurn = useCallback((permit, turnKey, origin, controller = new AbortController()) => {
    ...
    const releaseGoalTurn = useCallback((binding) => {
        ...
        if (activeGoalAdmissionRef.current?.controller === binding.controller &&
            activeGoalAdmissionRef.current.turnKey === binding.turnKey) {
            activeGoalAdmissionRef.current = null;
        }
    }, []);
    const failClosedGoalTurn = useCallback(async (binding, reason) => {
    ...
    const releaseUndeliveredGoalTurn = useCallback(async (turnKey) => {
        if (!turnKey)
            return;
        try {
            const runtime = await config.getGoalRuntimeReady();
            ...
            debugLogger.warn(`Failed to release undelivered Goal turn ${turnKey}`, error);
        }
    }, [config]);
    ...
    const setSubmissionInFlight = useCallback((inFlight) => {
    ...
    const commitItem = useCallback((item, userMessageTimestamp) => {
    ...
    const submissionActivitiesByGenerationRef = useRef(new Map());
    const settleSubmissionStateIfIdle = useCallback(() => {
        const currentGeneration = submissionLeaseGenerationRef.current;
        if ((submissionActivitiesByGenerationRef.current.get(currentGeneration) ??
            0) === 0 &&
    ...
    const retainSubmissionActivity = useCallback((generation) => {
        submissionActivitiesByGenerationRef.current.set(generation, (submissionActivitiesByGenerationRef.current.get(generation) ?? 0) + 1);
        return () => {
            const remainingActivities = Math.max(0, (submissionActivitiesByGenerationRef.current.get(generation) ?? 1) -
                1);
            if (remainingActivities === 0) {
                submissionActivitiesByGenerationRef.current.delete(generation);
            }
            else {
                submissionActivitiesByGenerationRef.current.set(generation, remainingActivities);
            }
    ...
    const [pendingAssistantItems, pendingAssistantItemsRef, setPendingAssistantItems,] = useStateAndRef([]);
    const commitPendingAssistantItems = useCallback((userMessageTimestamp) => {
    ...
    const commitItemInOrder = useCallback((item, userMessageTimestamp) => {
    ...
    const stagePendingAssistantItem = useCallback(() => {
    ...
    const inlineModelOverrideActiveRef = useRef(false);
    const canUseToolResultFullTurnModel = useCallback((model) => {
        const current = modelOverrideRef.current;
        return (!inlineModelOverrideActiveRef.current &&
    ...
    const handledToolCallFingerprintsRef = useRef(new Map());
    ...
    const duplicateProviderToolCallResponseIdsRef = useRef(new Set());
    const pendingDuplicateToolResponsesRef = useRef([]);
    const interactionOwnersByToolCallIdRef = useRef(new Map());
    const continuationOwnersByToolCallIdRef = useRef(new Map());
    const detachedToolContinuationAbortControllersRef = useRef(new Set());
    const pendingCompletedToolBatchesRef = useRef([]);
    const handleCompletedToolsRef = useRef(async () => { });
    const immediateDuplicateToolResponsesRef = useRef(null);
    ...
    const toolBatchIdByCallIdRef = useRef(new Map());
    ...
    const registerToolBatch = useCallback((requests) => {
    ...
    const getToolBatchId = useCallback((callId) => toolBatchIdByCallIdRef.current.get(callId), []);
    const [toolCalls, scheduleToolCalls, markToolsAsSubmitted] = useReactToolScheduler(async (completedToolCallsFromScheduler) => {
        ...
        if (completedToolCallsFromScheduler.length > 0) {
            ...
            const batchId = getToolBatchId(completedToolCallsFromScheduler[0].request.callId);
            try {
                ...
                for (const tc of completedToolCallsFromScheduler) {
                    if (toolBatchIdByCallIdRef.current.get(tc.request.callId) ===
                        batchId) {
    ...
    }, config, getPreferredEditor, onEditorClose, canUseToolResultFullTurnModel);
    const pendingToolCallGroupDisplay = useMemo(() => {
        if (!toolCalls.length)
            return undefined;
        const group = mapTrackedToolCallsToDisplay(toolCalls, config.getProjectRoot());
    ...
    const cancelOngoingRequest = useCallback(() => {
        ...
        const cancellationEvent = new ApiCancelEvent(modelOverrideRef.current ?? config.getModel(), prompt_id, config.getContentGeneratorConfig()?.authType, loopWakeupsCancelled > 0 ? loopWakeupsCancelled : undefined);
    ...
    const applyVisionBridgeIfNeeded = useCallback(async (parts, timestamp, signal) => {
        ...
        if (fullTurnModel?.agentCapable) {
            const fullTurnParts = (Array.isArray(parts) ? parts : [parts]).map((part) => typeof part === 'string'
                ? { text: part }
                : clampInlineMediaPart(part));
            if (!hasImageParts(fullTurnParts)) {
                return { parts: fullTurnParts, shouldProceed: true };
            }
            applyModelOverride(modelOverrideRef, inlineModelOverrideActiveRef, getFullTurnVisionModelSelector(fullTurnModel), false);
            addItem({
        ...
        debugLogger.debug(`vision bridge: status=${bridgeResult.status} applied=${bridgeResult.applied} model=${bridgeResult.modelId ?? '(none)'}`);
        ...
        if (bridgeResult.status !== 'skipped' || bridgeResult.egressOccurred) {
            addItem({
                type: bridgeResult.status === 'failed'
                    ? MessageType.ERROR
    ...
    const prepareQueryForGemini = useCallback(async (query, userMessageTimestamp, abortSignal, prompt_id, submitType, submittedPrompt, preserveTurnOwnership) => {
        ...
        if (typeof query === 'string') {
            ...
            if (submitType === SendMessageType.Teammate) {
                onDebugMessage(`Received teammate message (${trimmedQuery.length} chars)`);
                return { queryToSend: trimmedQuery, shouldProceed: true };
            }
            ...
            if (slashCommandResult) {
                switch (slashCommandResult.type) {
                    ...
                    case 'submit_prompt': {
                        ...
                        refreshContextFilesOnWriteRef.current = Boolean(slashCommandResult.refreshContextFilesOnWrite);
                        ...
                        if (slashCommandResult.modelOverride) {
                            if (isInlineModelOverrideAllowed(config, slashCommandResult.modelOverride)) {
                                applyModelOverride(modelOverrideRef, inlineModelOverrideActiveRef, slashCommandResult.modelOverride, true);
                            }
                            else {
                                debugLogger.warn(`ignoring model override '${slashCommandResult.modelOverride}': not a model on the active provider`);
                            }
                        }
                        const bridgeResult = await applyVisionBridgeIfNeeded(localQueryToSendToGemini, userMessageTimestamp, abortSignal);
    ...
    const handleContentEvent = useCallback((eventValue, currentGeminiMessageBuffer, userMessageTimestamp, startAsContinuation = false) => {
        if (turnCancelledRef.current) {
        ...
        if (pendingHistoryItemRef.current?.type !== 'gemini' &&
            ...
            setPendingHistoryItem(startAsContinuation
        ...
        let nextPendingType = pendingHistoryItemRef.current?.type === 'gemini_content'
            ? 'gemini_content'
        ...
        while (newGeminiMessageBuffer.length > STREAM_PENDING_ITEM_MAX_CHARS) {
            const splitPoint = findLastSafeSplitPoint(newGeminiMessageBuffer, STREAM_PENDING_ITEM_MAX_CHARS);
            const safeSplitPoint = splitPoint > 0 && splitPoint < newGeminiMessageBuffer.length
                ? splitPoint
            ...
            const { before: beforeText, after: afterText } = splitFencedMarkdown(newGeminiMessageBuffer, safeSplitPoint);
            commitItemInOrder({
        ...
        const viewportRows = (availableTerminalHeightRef?.current ?? 0) > 0
        ...
        const commitWidth = (terminalWidthRef?.current ?? 0) > 0
            ? terminalWidthRef.current
            : terminalWidth;
        const commitRowBudget = Math.max(4, viewportRows - STREAM_PENDING_COMMIT_RESERVE_ROWS);
        const tableClampRows = Math.max(2, viewportRows - 3);
        while (true) {
            const bufferLines = newGeminiMessageBuffer.split('\n');
            const { keptLines, clipped } = fitPendingSlice(bufferLines, commitWidth, commitRowBudget, tableClampRows);
            if (!clipped)
                break;
            ...
            if (boundaryLine < 0) {
                ...
                const capIndex = charIndexAfterLine(newGeminiMessageBuffer, keptLines);
                const fenceInfo = capIndex > 0
            ...
            const splitPoint = findLastSafeSplitPoint(newGeminiMessageBuffer, target);
            if (splitPoint <= 0 || splitPoint >= newGeminiMessageBuffer.length) {
                break;
            }
            // Repair fences when the split lands inside a code block so the tail
            // does not render as prose (see splitFencedMarkdown).
            const { before: beforeText, after: afterText } = splitFencedMarkdown(newGeminiMessageBuffer, splitPoint);
            commitItemInOrder({
                type: nextPendingType,
                text: beforeText,
            }, userMessageTimestamp);
    ...
    const mergeThought = useCallback((incoming) => {
        setThought((prev) => {
            ...
            const description = clampLoadingThoughtDescription(`${prev.description ?? ''}${incomingDescription ?? ''}`);
            if (debugLogger.isEnabled()) {
    ...
    const handleThoughtEvent = useCallback((eventValue, currentThoughtBuffer, userMessageTimestamp) => {
        ...
        // a collapsible block.
        let pendingThoughtType = startingNewThought
            ? 'gemini_thought'
            : pendingThoughtItemRef.current?.type === 'gemini_thought_content'
                ? 'gemini_thought_content'
                : 'gemini_thought';
        const getThoughtDurationMs = () => thoughtStartTimeRef.current
            ? Date.now() - thoughtStartTimeRef.current
            : 0;
        const buildThoughtItem = (type, text) => type === 'gemini_thought'
            ? {
                type,
                text,
        ...
        let splitPoint = findLastSafeSplitPoint(newThoughtBuffer, STREAM_PENDING_ITEM_MAX_CHARS);
        while (newThoughtBuffer.length > STREAM_PENDING_ITEM_MAX_CHARS) {
            const safeSplitPoint = splitPoint > 0 && splitPoint < newThoughtBuffer.length
                ? splitPoint
                : STREAM_PENDING_ITEM_MAX_CHARS;
            // Repair fences when the split lands inside a code block so the tail
            // does not render as prose (see splitFencedMarkdown).
            const { before: beforeText, after: afterText } = splitFencedMarkdown(newThoughtBuffer, safeSplitPoint);
            addItem(buildThoughtItem(pendingThoughtType, beforeText), userMessageTimestamp);
            pendingThoughtType = 'gemini_thought_content';
            newThoughtBuffer = afterText;
            splitPoint = findLastSafeSplitPoint(newThoughtBuffer, STREAM_PENDING_ITEM_MAX_CHARS);
        }
        setPendingThoughtItem(buildThoughtItem(pendingThoughtType, newThoughtBuffer));
        return newThoughtBuffer;
    ...
    const commitPendingThought = useCallback((userMessageTimestamp) => {
    ...
    const handleErrorEvent = useCallback((eventValue, userMessageTimestamp, submitType) => {
        ...
        if (!isShowingAutoRetry) {
            const retryHint = submitType !== SendMessageType.Goal
    ...
    const autonomousLoopTickResolverRef = useRef(null);
    ...
    const processGeminiStreamEvents = useCallback(async (stream, userMessageTimestamp, signal, submitType, turnAdmission, promptId, trackInteractionOwner = true, toolContinuationOwner) => {
        let geminiMessageBuffer = '';
        let thoughtBuffer = '';
        let scheduledToolContinuation = false;
        let assistantOutputStarted = pendingHistoryItemRef.current?.type === 'gemini' ||
            pendingHistoryItemRef.current?.type === 'gemini_content';
        let assistantInlineImageCount = [
            ...pendingAssistantItemsRef.current,
            pendingHistoryItemRef.current,
        ].reduce((count, item) => item?.type === 'gemini' || item?.type === 'gemini_content'
        ...
        const flushBufferedStreamEvents = () => {
            ...
            while (bufferedEvents.length > 0) {
                ...
                if (nextEvent.kind === 'content') {
                    ...
                    geminiMessageBuffer = handleContentEvent(contentParts.join(''), geminiMessageBuffer, userMessageTimestamp, assistantOutputStarted);
                ...
                if (nextEvent.kind === 'image') {
                    ...
                    turnSawContentEventRef.current = true;
                    const pendingItem = pendingHistoryItemRef.current;
                    const isOverflowOnlyItem = (pendingItem?.type === 'gemini' ||
                        pendingItem?.type === 'gemini_content') &&
                        pendingItem.text.length === 0 &&
                        !pendingItem.images?.length &&
                        Boolean(pendingItem.omittedImageCount);
                    const shouldDisplayImage = assistantInlineImageCount < MAX_INLINE_IMAGES_PER_ITEM;
                ...
                }, thoughtBuffer, userMessageTimestamp);
            }
        };
                ...
                switch (event.type) {
                    ...
                    case ServerGeminiEventType.ToolCallRequest:
                        ...
                        if (event.value.goalContext && turnAdmission) {
                            bindGoalTurn(event.value.goalContext, turnAdmission.turnKey, turnAdmission.origin, turnAdmission.controller);
                        }
        ...
        if (toolCallRequests.length > 0 &&
            ...
            const executableToolCallRequests = [];
            ...
            const handledToolCallFingerprints = new Map(geminiClient ? geminiClient.getHistoryToolCallFingerprints() : []);
            for (const [providerCallId, fingerprint,] of handledToolCallFingerprintsRef.current) {
                if (!handledToolCallFingerprints.has(providerCallId)) {
                    handledToolCallFingerprints.set(providerCallId, fingerprint);
                }
            }
            const isReplayOfHandledRequest = (request) => request.providerCallId
                ? isReplayOfHandledToolCall(handledToolCallFingerprints, request.providerCallId, getCachedToolCallFingerprint(request, request.name, request.args))
                : false;
            const repeatedDuplicateRequest = findRepeatedDuplicateProviderToolCall(toolCallRequests, (request) => request.providerCallId, isReplayOfHandledRequest, duplicateProviderToolCallResponseIdsRef.current);
            if (repeatedDuplicateRequest?.providerCallId) {
                debugLogger.debug(`[processGeminiStreamEvents] Dropping batch after repeated duplicate provider tool-call id: ${repeatedDuplicateRequest.providerCallId} (tool: ${repeatedDuplicateRequest.name})`);
                loopDetectedRef.current = true;
                return {
            ...
            for (const request of toolCallRequests) {
                ...
                if (isReplayOfHandledRequest(request)) {
                    markDuplicateProviderToolCallResponseSent(providerCallId, duplicateProviderToolCallResponseIdsRef.current);
                    const response = createDuplicateProviderToolCallResponse(request);
                    debugLogger.debug(`[processGeminiStreamEvents] Suppressing duplicate provider tool-call id: ${providerCallId} (tool: ${request.name})`);
                    dualOutput?.emitToolResult(request, response);
                    duplicateResponses.push({ request, response });
                    duplicatePromptId ??= request.prompt_id;
                    continue;
                }
                const requestFingerprint = getCachedToolCallFingerprint(request, request.name, request.args);
                recordHandledToolCall(handledToolCallFingerprints, providerCallId, requestFingerprint);
                recordHandledToolCall(handledToolCallFingerprintsRef.current, providerCallId, requestFingerprint);
                executableToolCallRequests.push(request);
            }
            if (duplicateResponses.length > 0) {
                if (executableToolCallRequests.length > 0) {
                    pendingDuplicateToolResponsesRef.current.push({
                        executableCallIds: new Set(executableToolCallRequests.map((request) => request.callId)),
                        promptId: duplicatePromptId ?? executableToolCallRequests[0]?.prompt_id,
            ...
            if (executableToolCallRequests.length > 0) {
                if (toolContinuationOwner) {
                    for (const request of executableToolCallRequests) {
                        continuationOwnersByToolCallIdRef.current.set(request.callId, toolContinuationOwner);
                    }
                }
                if (streamInteractionOwner) {
                    for (const request of executableToolCallRequests) {
                        interactionOwnersByToolCallIdRef.current.set(request.callId, streamInteractionOwner);
                    }
                }
                scheduledToolContinuation = true;
                registerToolBatch(executableToolCallRequests);
                scheduleToolCalls(executableToolCallRequests, signal, modelOverrideRef.current);
            }
        }
        return {
            status: StreamProcessingStatus.Completed,
    ...
    const resolveSteeredMessages = useCallback(async (messages, signal) => {
        ...
        for (let index = 0; index < messages.length; index += 1) {
            ...
            const sideEffects = [];
            let resolvedQuery = [{ text: message }];
            if (isAtCommand(message)) {
                ...
                try {
                    const atCommandResult = await resolveWithAbort(atCommandSignal, () => resolveAtCommandQuery({
                        query: message,
                        config,
                        onDebugMessage,
                    ...
                    const shouldSkipMessage = !atCommandResult.shouldProceed &&
                    ...
                    if (atCommandResult.shouldProceed &&
                        ...
                        const showToolDisplays = () => addItem({ type: 'tool_group', tools: toolDisplays }, timestamp + index);
                        if (shouldSkipMessage)
                            showToolDisplays();
                        else
                            sideEffects.push(showToolDisplays);
                    }
                    if (atCommandResult.recording) {
                        const recordAtCommand = () => config.getChatRecordingService?.()?.recordAtCommand?.({
                        ...
                        if (shouldSkipMessage)
                            ...
                            sideEffects.push(recordAtCommand);
                    }
                    if (shouldSkipMessage)
                        continue;
                }
                    ...
                    onDebugMessage(`Failed to resolve mid-turn @ command: ${errorMessage}`);
                    if (!signal.aborted) {
                        addItem({
            ...
            const bridgeResult = await applyVisionBridgeIfNeeded(resolvedQuery, timestamp + index, signal);
            if (!bridgeResult.shouldProceed) {
                if (signal.aborted) {
                    restoreMessages.push(...messages.slice(index + 1));
                    break;
            ...
            const messageParts = normalizePartList(bridgeResult.parts ?? resolvedQuery);
            const formatCheck = checkImageFormatsSupport(messageParts);
            if (formatCheck.hasUnsupportedFormats) {
                sideEffects.push(() => addItem({
                    type: MessageType.INFO,
    ...
    const resolveDrainedSteerMessages = useCallback(async (messages, signal) => {
        try {
            ...
            onDebugMessage(`Failed to prepare steer input: ${getErrorMessage(error)}`);
            return undefined;
        }
    }, [midTurnRestoreRef, onDebugMessage, resolveSteeredMessages]);
    const drainSteerAtBoundary = useCallback(async (signal) => {
        const messages = midTurnDrainRef?.current?.(false, Boolean(activeGoalAdmissionRef.current)) ?? [];
    ...
    const submitQuery = useCallback(async (query, submitType = SendMessageType.UserQuery, prompt_id, metadata) => {
        ...
        const releaseSubmissionLease = () => {
            if (!ownsSubmissionLease)
                return;
            ownsSubmissionLease = false;
            if (submissionLeaseGeneration !== submissionLeaseGenerationRef.current) {
                return;
            }
            setSubmissionInFlight(false);
        };
        const isTurnContinuation = submitType === SendMessageType.ToolResult ||
            submitType === SendMessageType.Steer;
        const submittedPrompt = submitType === SendMessageType.UserQuery
        ...
        if (!isTurnContinuation &&
            ...
            const droppingInlineOverrideOnRetry = submitType === SendMessageType.Retry &&
        ...
        const isDetachedToolContinuation = inheritedToolContinuationOwner?.survivesGenerationChange === true;
        const detachedAbortController = inheritedToolContinuationOwner?.detachedAbortController ??
            (allowConcurrentBtwDuringResponse ? abortController : undefined);
        const foregroundAbortController = allowConcurrentBtwDuringResponse || isDetachedToolContinuation
            ? undefined
            : abortController;
        let keepToolContinuationAbortController = false;
        if (detachedAbortController) {
            detachedToolContinuationAbortControllersRef.current.add(detachedAbortController);
        }
        ...
        const releaseSubmissionActivity = retainSubmissionActivity(submissionGeneration);
        const submission = promptIdContext.run(prompt_id, async () => {
                ...
                preparedQuery =
                    submitType === SendMessageType.Goal
                        ...
                        : submitType === SendMessageType.Retry
                            ...
                            : await prepareQueryForGemini(query, userMessageTimestamp, abortSignal, prompt_id, submitType, submittedPrompt, allowConcurrentBtwDuringResponse ||
            ...
            const { queryToSend, shouldProceed, scheduledToolCallId } = preparedQuery;
            ...
            if (submitType === SendMessageType.UserQuery) {
                if (metadata?.userAdmission) {
                    const goal = metadata.userAdmission.goal ??
            ...
            let goalBinding = metadata?.goalBinding ??
                (goal
                    ? bindGoalTurn(goal.permit, goal.turnKey, submitType === SendMessageType.UserQuery ? 'user' : 'runtime')
                    : undefined);
            const turnKey = goalBinding?.turnKey ?? userAdmission?.turnKey;
            const turnController = goalBinding?.controller ??
            ...
            const processingSignal = processingSignals.length > 1
                ? AbortSignal.any(processingSignals)
                : abortSignal;
            const toolContinuationOwner = {
                promptId: prompt_id,
                signal: processingSignal,
                survivesGenerationChange: allowConcurrentBtwDuringResponse ||
                    inheritedToolContinuationOwner?.survivesGenerationChange === true,
                detachedAbortController,
                foregroundAbortController,
            };
            const turnAdmission = turnKey && turnController
                ? {
                    turnKey,
                    controller: turnController,
                    origin: goalBinding?.origin ?? 'user',
                }
                : undefined;
            if (turnAdmission &&
                !goalBinding &&
                ...
                !activeGoalAdmissionRef.current) {
                try {
                    if (config.getGoalRuntime().getSnapshot().goal?.status === 'active') {
            ...
            if (submitType === SendMessageType.UserQuery ||
                ...
                if (typeof queryToSend === 'string') {
                    logUserPrompt(config, new UserPromptEvent(queryToSend.length, prompt_id, config.getContentGeneratorConfig()?.authType, queryToSend, modelOverrideRef.current ?? config.getModel()));
                }
            ...
            try {
                ...
                const providerSignal = inheritedToolContinuationOwner
                    ? processingSignal
                    : abortSignal;
                const stream = geminiClient.sendMessageStream(finalQueryToSend, providerSignal, prompt_id, {
                    ...sendOptions,
                    ...(goalBinding
                        ? {
                            ...
                            getQueuedGoalTurnKey: () => goalQueueRef?.current?.peekNextUserBatchKey(true),
                        }
                        : userAdmission
                            ? {
                ...
                const processingResult = await processGeminiStreamEvents(stream, userMessageTimestamp, processingSignal, submitType, turnAdmission, prompt_id, !allowConcurrentBtwDuringResponse, toolContinuationOwner);
                if (!goalBinding &&
                    turnAdmission &&
                ...
                const immediateDuplicateToolResponses = immediateDuplicateToolResponsesRef.current;
                if (immediateDuplicateToolResponses) {
                    immediateDuplicateToolResponsesRef.current = null;
                    const finalized = await finalizeToolResponses(config, immediateDuplicateToolResponses.responses.map(({ request, response }) => ({
                        callId: request.callId,
                        toolName: request.name,
                        responseParts: response.responseParts,
                        persistedOutputFiles: response.persistedOutputFiles,
                        artifacts: response.artifacts,
                    })), new Map(immediateDuplicateToolResponses.responses.flatMap(({ request }) => {
                        const promptId = request.prompt_id ??
                            immediateDuplicateToolResponses.promptId;
                        return promptId
                            ? [[request.callId, promptId]]
                            : [];
                    })));
                    const responseParts = finalized.flatMap((entry) => entry.responseParts);
                    immediateDuplicateToolResponses.responses.forEach(({ request, response }, index) => {
                        const goalContext = request.goalContext;
                        config.getChatRecordingService?.()?.recordToolResult?.(finalized[index].responseParts, {
                            callId: request.callId,
                    ...
                    await submitQuery(responseParts, SendMessageType.ToolResult, immediateDuplicateToolResponses.promptId, { goalBinding });
                    if (goalBinding &&
                ...
                submitPromptOnCompleteRef.current = null;
                activeModelStreamsRef.current = Math.max(0, activeModelStreamsRef.current - 1);
                const shouldDrainCompletedToolBatches = activeModelStreamsRef.current === 0;
                if (goalBinding) {
                    let retainGoalBinding = keepGoalBinding && !goalBinding.controller.signal.aborted;
                    ...
                    if (!retainGoalBinding) {
                        await failClosedGoalTurn(goalBinding, 'Goal turn ended without a valid continuation');
                    }
                }
                if (turnAdmission &&
                    !goalBinding &&
                ...
                if (shouldDrainCompletedToolBatches) {
                    const pendingCompletedToolBatches = pendingCompletedToolBatchesRef.current.splice(0);
                    const pendingCompletedTools = new Map();
        ...
        return submission.finally(() => {
            ...
            if (detachedAbortController && !keepToolContinuationAbortController) {
                detachedToolContinuationAbortControllersRef.current.delete(detachedAbortController);
            }
            if (foregroundAbortController &&
                !keepToolContinuationAbortController &&
    ...
    const preemptGoalTurn = useCallback((reason) => {
    ...
    const handleApprovalModeChange = useCallback(async (newApprovalMode) => {
        ...
        if (newApprovalMode === ApprovalMode.YOLO ||
            ...
            let awaitingApprovalCalls = toolCalls.filter((call) => {
    ...
    const handleCompletedTools = useCallback(async (completedToolCallsFromScheduler) => {
        ...
        if (dedupedCallIds.length > 0) {
            ...
            const detachedAbortControllers = new Set();
            const foregroundAbortControllers = new Set();
            for (const callId of dedupedCallIds) {
                const continuationOwner = continuationOwnersByToolCallIdRef.current.get(callId);
                if (continuationOwner?.detachedAbortController) {
                    detachedAbortControllers.add(continuationOwner.detachedAbortController);
                }
                if (continuationOwner?.foregroundAbortController) {
                    foregroundAbortControllers.add(continuationOwner.foregroundAbortController);
                }
            ...
            for (const controller of detachedAbortControllers) {
                ...
                if (!isStillOwned) {
                    detachedToolContinuationAbortControllersRef.current.delete(controller);
                }
            }
        ...
        if (activeModelStreamsRef.current > 0) {
            const deferredTools = completedAndReadyToSubmitTools.filter((toolCall) => !historyCallIdsWithResponse.has(toolCall.request.callId));
        ...
        const continuationOwner = completedAndReadyToSubmitTools
            .filter((toolCall) => !historyCallIdsWithResponse.has(toolCall.request.callId))
            .map((toolCall) => continuationOwnersByToolCallIdRef.current.get(toolCall.request.callId))
            .find((owner) => owner !== undefined);
        const continuationWasCancelled = () => continuationOwner
        ...
        let geminiTools = completedAndReadyToSubmitTools.filter((t) => !t.request.isClientInitiated &&
            !historyCallIdsWithResponse.has(t.request.callId));
        const terminalPromptId = completedAndReadyToSubmitTools.find((toolCall) => !toolCall.request.isClientInitiated)?.request.prompt_id;
        const ownerForToolCall = (toolCall) => interactionOwnersByToolCallIdRef.current.get(toolCall.request.callId) ??
            (activeInteractionPromptIdRef.current === toolCall.request.prompt_id
                ? activeInteractionOwnerRef.current
                : undefined);
        const liveOwnerForToolCall = (toolCall) => {
        ...
        const liveActiveInteractionOwner = activeInteractionPromptIdRef.current &&
            activeInteractionOwnerRef.current &&
            getActiveInteractionSpan(activeInteractionPromptIdRef.current) ===
                activeInteractionOwnerRef.current
            ? activeInteractionOwnerRef.current
            : undefined;
        const ownerToolCall = (liveActiveInteractionOwner
            ? geminiTools.find((toolCall) => liveOwnerForToolCall(toolCall) === liveActiveInteractionOwner)
            : undefined) ??
            geminiTools.find((toolCall) => liveOwnerForToolCall(toolCall)) ??
            geminiTools[0] ??
            completedAndReadyToSubmitTools.find((toolCall) => !toolCall.request.isClientInitiated);
        ...
        const secondaryTools = interactionOwner
            ? geminiTools.filter((toolCall) => ownerForToolCall(toolCall) !== interactionOwner)
            : [];
        for (const toolCall of secondaryTools) {
            ...
            if (secondaryOwner && toolCall.request.prompt_id) {
                secondaryInteractionOwners.set(secondaryOwner, toolCall.request.prompt_id);
            }
        ...
        if (secondaryTools.length > 0) {
            const secondaryCallIds = new Set(secondaryTools.map((toolCall) => toolCall.request.callId));
            markToolsAsSubmitted([...secondaryCallIds]);
            geminiTools = geminiTools.filter((toolCall) => !secondaryCallIds.has(toolCall.request.callId));
        }
        for (const toolCall of completedAndReadyToSubmitTools) {
            interactionOwnersByToolCallIdRef.current.delete(toolCall.request.callId);
            continuationOwnersByToolCallIdRef.current.delete(toolCall.request.callId);
        }
        ...
        let promptId = ownerToolCall?.request.prompt_id ?? continuationOwner?.promptId;
        const endToolInteraction = (status, errorMessage, errorType) => {
            if (continuationOwner?.detachedAbortController) {
                detachedToolContinuationAbortControllersRef.current.delete(continuationOwner.detachedAbortController);
            }
            if (continuationOwner?.foregroundAbortController &&
        ...
        let toolGoalPermit;
        const toolGoalContexts = geminiTools.map((toolCall) => toolCall.request.goalContext);
        try {
            ...
            endToolInteraction('error', 'invalid Goal tool context', 'continuation_goal_context_invalid');
            return;
        }
        if (!toolGoalPermit && toolGoalContexts.length > 0) {
            ...
            if (active && activeGoalPermitValid) {
                ...
                endToolInteraction('error', 'missing Goal tool context', 'continuation_goal_context_missing');
                return;
            }
        }
        let toolGoalBinding;
        if (toolGoalPermit) {
            ...
            if (existing && !sameGoalPermit(existing.permit, toolGoalPermit)) {
                ...
                endToolInteraction('error', 'stale Goal tool context', 'continuation_goal_context_stale');
                return;
            }
            toolGoalBinding =
                existing ??
                    bindGoalTurn(toolGoalPermit, `goal-runtime:${toolGoalPermit.turnId}`, 'runtime');
        }
        const memoryWriteCandidates = completedAndReadyToSubmitTools.map((toolCall) => ({
            toolName: toolCall.request.name,
            args: toolCall.request.args,
            status: toolCall.status,
        }));
        const didRefreshManagedMemory = await refreshMemoryAfterManagedWrite(config, memoryWriteCandidates, { logContext: 'interactive memory tool batch' });
        if (refreshContextFilesOnWriteRef.current) {
            const matchedContextFileWrite = didWriteProjectContextFile(memoryWriteCandidates, config.getProjectRoot());
            debugLogger.debug(`Checked marked context-file memory tool batch; matched=${matchedContextFileWrite}`);
            if (matchedContextFileWrite) {
                debugLogger.debug('Refreshing memory after context-file memory write');
        ...
        const completedCallIds = new Set(geminiTools.map((toolCall) => toolCall.request.callId));
        const secondaryCallIds = new Set(secondaryTools.map((toolCall) => toolCall.request.callId));
        const readyDuplicateBatches = [];
        pendingDuplicateToolResponsesRef.current =
            pendingDuplicateToolResponsesRef.current.filter((batch) => {
                const isReady = [...batch.executableCallIds].some((callId) => completedCallIds.has(callId));
                if (isReady) {
                    readyDuplicateBatches.push(batch);
                }
                const belongsToSecondaryOwner = [...batch.executableCallIds].some((callId) => secondaryCallIds.has(callId));
        ...
        const pendingDuplicateResponses = readyDuplicateBatches
            ...
            .filter(({ request }) => !interactionOwner ||
            !request.prompt_id ||
        ...
        if (geminiTools.length === 0 && pendingDuplicateResponses.length === 0) {
            ...
            if (toolGoalBinding) {
                await failClosedGoalTurn(toolGoalBinding, 'Goal tool continuation ended without a result');
            }
            if (completedAndReadyToSubmitTools.length > 0 &&
                completedAndReadyToSubmitTools.every((toolCall) => toolCall.status === 'cancelled')) {
                endToolInteraction('cancelled');
            }
            else if (completedAndReadyToSubmitTools.some((toolCall) => toolCall.status === 'error')) {
                endToolInteraction('error', 'tool continuation ended with an error', 'continuation_tool_error');
            }
            else {
        ...
        const executableQueues = new Map();
        for (const toolCall of geminiTools) {
        ...
        for (const batch of readyDuplicateBatches) {
            for (const callId of batch.callOrder) {
                ...
                if (duplicate)
                    orderedResponses.push(duplicate);
            }
        }
        ...
        const finalizedResponses = await finalizeToolResponses(config, orderedResponses.map(({ request, response }) => ({
            callId: request.callId,
            toolName: request.name,
            responseParts: response.responseParts,
            persistedOutputFiles: response.persistedOutputFiles,
            artifacts: response.artifacts,
        })), new Map(orderedResponses.flatMap(({ request }) => request.prompt_id
            ? [[request.callId, request.prompt_id]]
            : [])));
        const responsesToSend = finalizedResponses.flatMap((entry) => entry.responseParts);
        orderedResponses.forEach(({ request, response, status }, index) => {
            const goalContext = request.goalContext;
            config.getChatRecordingService?.()?.recordToolResult?.(finalizedResponses[index].responseParts, {
                callId: request.callId,
                status,
                resultDisplay: response.resultDisplay,
                persistedOutputFiles: finalizedResponses[index].persistedOutputFiles,
        ...
        if (continuationWasCancelled()) {
            markToolsAsSubmitted(geminiTools.map((toolCall) => toolCall.request.callId));
            if (toolGoalBinding) {
                await failClosedGoalTurn(toolGoalBinding, 'Goal tool continuation was cancelled');
            }
            endToolInteraction('cancelled');
            return;
        }
        ...
        for (const toolCall of geminiTools) {
            if ('modelOverride' in toolCall.response) {
                if (inlineModelOverrideActiveRef.current ||
                    ...
                    debugLogger.debug(`skill-tool model override (${String(toolCall.response.modelOverride)}) blocked: ${inlineModelOverrideActiveRef.current
                        ? 'inline override active'
                        : 'full-turn override active'}`);
                }
                else {
                    applyModelOverride(modelOverrideRef, inlineModelOverrideActiveRef, toolCall.response.modelOverride, false);
                }
            }
        }
        ...
        markToolsAsSubmitted(callIdsToMarkAsSubmitted);
        const terminatesGoalTurn = geminiTools.some((toolCall) => toolCall.response.terminateTurn === true);
        if (terminatesGoalTurn && toolGoalBinding) {
            ...
            try {
                ...
                await failClosedGoalTurn(toolGoalBinding, `Goal turn could not finish: ${errorMessage}`);
            }
            finally {
            ...
            if (goalFinishFailed) {
                endToolInteraction('error', 'Goal tool continuation could not finish', 'continuation_goal_finish_failed');
            }
            else {
                endToolInteraction('ok');
            }
        ...
        if (modelSwitchedFromQuotaError) {
            if (toolGoalBinding) {
                await failClosedGoalTurn(toolGoalBinding, 'Goal tool continuation stopped after a model switch');
            }
            endToolInteraction('cancelled');
            return;
        }
        const backgroundTaskRegistry = config.getBackgroundTaskRegistry();
        const backgroundLaunchExhaustedCapacity = backgroundTaskRegistry.getMaxConcurrentBackgroundAgents() === 1 &&
            !backgroundTaskRegistry.canStartBackgroundAgent() &&
            geminiTools.some((toolCall) => {
                const display = toolCall.response.resultDisplay;
                return (toolCall.request.name === ToolNames.AGENT &&
        ...
        if (backgroundLaunchExhaustedCapacity) {
            geminiClient?.addHistory({ role: 'user', parts: responsesToSend });
            if (toolGoalBinding) {
                await failClosedGoalTurn(toolGoalBinding, 'Goal tool continuation stopped: background capacity exhausted');
            }
            endToolInteraction('error', 'tool continuation capacity exhausted', 'continuation_capacity_exhausted');
            return;
        }
        ...
        const drained = continuationOwner?.survivesGenerationChange ||
            continuationWasCancelled()
            ? []
            : (midTurnDrainRef?.current?.(false, Boolean(activeGoalAdmissionRef.current)) ?? []);
        let drainedSteer;
        if (drained.length > 0) {
            const midTurnAbort = abortControllerRef.current ?? new AbortController();
            ...
            try {
                drainedSteer = await resolveDrainedSteerMessages(drained, midTurnAbort.signal);
                if (drainedSteer) {
        ...
        if (toolGoalBinding?.controller.signal.aborted) {
            drainedSteer?.restore();
            await failClosedGoalTurn(toolGoalBinding, 'Goal tool continuation was preempted');
            endToolInteraction('cancelled');
            return;
        }
        await submitQuery(responsesToSend, SendMessageType.ToolResult, promptId, {
            ...
            onAdmissionFailed: () => {
                endToolInteraction('error', 'tool continuation admission failed', 'continuation_admission_failed');
            },
            onDeliveryFailed: () => {
                drainedSteer?.restore();
                endToolInteraction('error', 'tool continuation delivery failed', 'continuation_delivery_failed');
            },
    ...
    const goalQueuePendingCount = goalQueueRef?.current?.getPendingSubmissionCount?.() ?? 0;
    const claimSystemGoalTurn = useCallback(() => {
        ...
        try {
            goalOwnsTurn = goalRequiresExactPermit(config.getGoalRuntime().getSnapshot());
        }
        catch {
            goalOwnsTurn = false;
        }
    ...
    useEffect(() => {
        ...
        void (async () => {
            try {
                ...
                // lives under ~/.lailatulcoder, not the working tree, so it's user-owned
    ...
    useEffect(() => {
        ...
        const handleManagerChange = (manager) => {
            ...
            if (manager) {
                manager.setLeaderMessageCallback((modelText, display) => {
        ...
        if (streamingState === StreamingState.Idle &&
            ...
            runOutsideAgentContext(() => {
                ...
                for (const entry of batch) {
                    if (!entry.displayed) {
                        addItem({ type: 'notification', text: entry.display }, Date.now());
