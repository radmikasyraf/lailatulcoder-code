 ...
 */
import { registerGoalHook, setGoalTerminalObserver, setLastGoalTerminal, unregisterGoalHook, } from '@lailatul-coder/lailatul-coder-core';
import { isGoalStatusKind, isTerminalGoalStatusKind, MessageType, } from '../types.js';
 ...
 */
export function findGoalToRestore(history) {
 ...
 */
function findSetAtOfRun(history, startIndex) {
    ...
    for (let i = startIndex - 1; i >= 0; i--) {
        ...
        if (goal.kind !== 'set' && goal.kind !== 'checking')
            return undefined;
        if (goal.condition !== condition)
            return undefined;
        if (goal.setAt !== undefined)
            return goal.setAt;
    }
    return undefined;
}
 ...
 */
export function findLastTerminalGoal(history) {
    for (let i = history.length - 1; i >= 0; i--) {
        const item = history[i];
        if (item?.type !== MessageType.GOAL_STATUS)
            continue;
        const goal = item;
        if (!isTerminalGoalStatusKind(goal.kind))
            continue;
        return {
            kind: goal.kind,
            condition: goal.condition,
 ...
 */
export function isTranscriptItemRecord(item) {
 ...
 */
export function parseGoalStatusItem(item) {
    if (!isTranscriptItemRecord(item))
        return null;
    if (item['type'] !== MessageType.GOAL_STATUS)
        return null;
    const kind = item['kind'];
    const condition = item['condition'];
    if (!isGoalStatusKind(kind) || typeof condition !== 'string')
        return null;
    ...
    const lastReason = typeof item['lastReason'] === 'string' ? item['lastReason'] : undefined;
    return {
 ...
 */
export function collectGoalStatusItemsFromRecords(records) {
    const items = [];
    for (const record of records) {
        ...
        const raws = payload.outputHistoryItems;
...
export function goalTerminalEventToHistoryItem(event) {
    return {
        type: MessageType.GOAL_STATUS,
        kind: event.kind,
...
export function recordGoalStatusItem(config, item, rawCommand = '/goal') {
    try {
        ...
        if (!recording) {
            ...
            writeStderrLineSafe(`LailatulCoder: no chat recording service; goal_status (kind=${item.kind}) will not survive a resume.`);
            return;
        }
        ...
        writeStderrLineSafe(`LailatulCoder: failed to record goal_status (kind=${item.kind}): ${error}`);
    }
}
 ...
 */
export function goalRestoreBlockedBy(config) {
    if (!config.isTrustedFolder())
        return 'untrusted-folder';
 ...
 */
export function goalConditionBlockedBy(condition) {
 ...
 */
export function restoreGoalFromHistory(history, config, addItem) {
    ...
    if (goalConditionBlockedBy(restorable.condition)) {
        writeStderrLineSafe(`LailatulCoder: refusing to restore a goal for session ${sessionId}: the condition is empty.`);
        unregisterGoalHook(config, sessionId);
...
//# sourceMappingURL=restoreGoal.js.map