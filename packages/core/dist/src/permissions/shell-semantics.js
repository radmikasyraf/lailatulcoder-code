...
function trimShellSyntax(token) {
    let start = 0;
...
function isShellAbsolutePath(p) {
 ...
 * reads/writes.
 */
function isNetworkPseudoDevice(target) {
 ...
 */
function extractRedirects(tokens, cwd) {
    const readFiles = [];
    const writeFiles = [];
 ...
 */
function getPositionalArgs(args, flagsWithValue = new Set()) {
    const positional = [];
    let skipNext = false;
...
function getFlagValue(args, shortName, longName) {
...
function targetDirectoryPath(args, cwd) {
    const target = getFlagValue(args, '-t', '--target-directory');
    if (!target || !looksLikePath(target))
        return undefined;
    return resolvePath(target, cwd);
}
function targetDirectoryWrites(targetDir, sources) {
    return sources.map((source) => ({
        virtualTool: 'write_file',
        filePath: nodePath.posix.join(targetDir, nodePath.posix.basename(source.replace(/\\/g, '/'))),
    }));
}
function writeOpForFlag(args, cwd, shortName, longName) {
    const target = getFlagValue(args, shortName, longName);
    if (!target || !looksLikePath(target))
        return undefined;
    return { virtualTool: 'write_file', filePath: resolvePath(target, cwd) };
}
function hasCombinedShortFlag(arg, flag) {
    return (arg.startsWith('-') && !arg.startsWith('--') && arg.slice(1).includes(flag));
}
function isAttachedShortFlagValue(arg, flag) {
...
function readOps(args, cwd, flagsWithValue) {
...
function listOps(args, cwd, flagsWithValue) {
...
const COMMANDS = {
    ...
    tail: (a, d) => readOps(a, d, new Set(['-n', '-c', '-s', '--lines', '--bytes', '--sleep-interval'])),
    less: (a, d) => readOps(a, d, new Set(['-b', '-h', '-j', '-p', '-x', '-y', '-z', '--shift', '--tabs'])),
    more: (a, d) => readOps(a, d),
    most: (a, d) => readOps(a, d),
    wc: (a, d) => readOps(a, d),
    file: (a, d) => readOps(a, d, new Set([
        '-m',
        '-e',
        '-F',
        '-P',
    ...
    stat: (a, d) => readOps(a, d, new Set(['-c', '-f', '--format', '--printf', '--file-system'])),
    readlink: (a, d) => readOps(a, d, new Set([
        '-e',
        '-f',
        '-m',
        '-q',
    ...
    realpath: (a, d) => readOps(a, d, new Set([
        '--relative-to',
        '--relative-base',
        '-e',
        '-m',
    ...
    sdiff: (a, d) => readOps(a, d, new Set(['-o', '-w', '-W', '-s', '-i', '-b', '-B', '-E', '-H'])),
    cmp: (a, d) => readOps(a, d, new Set([
        '-i',
        '-l',
        '-n',
        '-s',
    ...
    sum: (a, d) => readOps(a, d),
    strings: (a, d) => readOps(a, d, new Set([
        '-n',
        '-t',
        '-e',
        '-o',
    ...
    hexdump: (a, d) => readOps(a, d, new Set([
        '-n',
        '-s',
        '-l',
        '-C',
    ...
    paste: (a, d) => readOps(a, d, new Set(['-d', '-s', '--delimiters', '--serial'])),
    join: (a, d) => readOps(a, d, new Set([
        '-t',
        '-1',
        '-2',
        '-j',
    ...
    column: (a, d) => readOps(a, d, new Set([
        '-t',
        '-s',
        '-n',
        '-c',
    ...
    fold: (a, d) => readOps(a, d, new Set(['-w', '-b', '-s', '--width', '--bytes', '--spaces'])),
    expand: (a, d) => readOps(a, d, new Set(['-t', '--tabs', '--initial'])),
    unexpand: (a, d) => readOps(a, d, new Set(['-t', '-a', '--tabs', '--all', '--first-only'])),
    base64: (a, d) => readOps(a, d, new Set(['-d', '-i', '-w', '--decode', '--ignore-garbage', '--wrap'])),
    base32: (a, d) => readOps(a, d, new Set(['-d', '-i', '-w', '--decode', '--ignore-garbage', '--wrap'])),
    tr: (a, d) => readOps(a, d),
    // ── Grep / search commands ────────────────────────────────────────────────
    grep: (args, cwd) => {
        const hasPatternFlag = args.some((a) => a === '-e' || a === '-f' || a.startsWith('-e') || a.startsWith('-f'));
        const isRecursive = args.some((a) => ['-r', '-R', '--recursive', '--dereference-recursive'].includes(a));
        ...
        const positional = getPositionalArgs(args, flagsWithValue).filter(looksLikePath);
    ...
    egrep: (a, d) => COMMANDS['grep'](a, d),
    fgrep: (a, d) => COMMANDS['grep'](a, d),
    zgrep: (a, d) => COMMANDS['grep'](a, d),
    bzgrep: (a, d) => COMMANDS['grep'](a, d),
    rg: (args, cwd) => {
    ...
    tree: (args, cwd) => listOps(args, cwd, new Set([
        '-L',
        '-P',
        '-I',
        '-o',
    ...
    du: (args, cwd) => listOps(args, cwd, new Set([
        '-d',
        '--max-depth',
        '--threshold',
        '-t',
    ...
    touch: (args, cwd) => getPositionalArgs(args, new Set(['-t', '-r', '--reference', '--date', '-d', '--time']))
        .filter(looksLikePath)
        .map((p) => ({
        virtualTool: 'write_file',
        filePath: resolvePath(p, cwd),
    })),
    mkdir: (args, cwd) => getPositionalArgs(args, new Set(['-m', '--mode', '-Z', '--context']))
        .filter(looksLikePath)
        .map((p) => ({
        virtualTool: 'write_file',
        filePath: resolvePath(p, cwd),
    })),
    mkfifo: (args, cwd) => getPositionalArgs(args, new Set(['-m', '--mode', '-Z']))
        .filter(looksLikePath)
        .map((p) => ({
        virtualTool: 'write_file',
        filePath: resolvePath(p, cwd),
    })),
    tee: (args, cwd) => getPositionalArgs(args)
        .filter(looksLikePath)
        .map((p) => ({
        virtualTool: 'write_file',
        filePath: resolvePath(p, cwd),
    ...
    rsync: (args, cwd) => {
        const positional = getPositionalArgs(args, new Set([
            '-e',
            '--rsh',
            '--rsync-path',
            '--backup-dir',
    ...
    ln: (args, cwd) => {
        ...
        const targetDir = targetDirectoryPath(args, cwd);
        const positional = getPositionalArgs(args, new Set(['-S', '--suffix', '-t', '--target-directory', '-b', '--backup'])).filter(looksLikePath);
        if (targetDir && positional.length > 0) {
            return [
                ...positional.map((p) => ({
                    virtualTool: 'read_file',
    ...
    rm: (args, cwd) => getPositionalArgs(args, new Set(['--interactive']))
        .filter(looksLikePath)
        .map((p) => ({
        virtualTool: 'edit',
        filePath: resolvePath(p, cwd),
    })),
    rmdir: (args, cwd) => getPositionalArgs(args, new Set(['--ignore-fail-on-non-empty', '-p', '--parents']))
        .filter(looksLikePath)
        .map((p) => ({
        virtualTool: 'edit',
        filePath: resolvePath(p, cwd),
    })),
    unlink: (args, cwd) => getPositionalArgs(args)
        .filter(looksLikePath)
        .map((p) => ({
        virtualTool: 'edit',
        filePath: resolvePath(p, cwd),
    })),
    shred: (args, cwd) => getPositionalArgs(args, new Set(['-n', '--iterations', '-s', '--size', '--random-source']))
        .filter(looksLikePath)
        .map((p) => ({
        virtualTool: 'edit',
        filePath: resolvePath(p, cwd),
    })),
    truncate: (args, cwd) => getPositionalArgs(args, new Set([
        '-s',
        '--size',
        '-r',
        '--reference',
    ...
    chmod: (args, cwd) => {
        ...
        const positional = getPositionalArgs(args, new Set(['-f', '--reference', '--from']));
        return positional
            .slice(1)
            .filter(looksLikePath)
            .map((p) => ({
    ...
    chown: (args, cwd) => {
        ...
        const positional = getPositionalArgs(args, new Set(['--from', '--reference']));
        return positional
            .slice(1)
            .filter(looksLikePath)
            .map((p) => ({
    ...
    patch: (args, cwd) => {
        ...
        const outputOps = output && looksLikePath(output)
            ? [
                {
                    virtualTool: 'edit',
        ...
        return [
            ...outputOps,
            ...getPositionalArgs(args, new Set(['-i', '--input', '-d', '--directory', '-o', '--output']))
                .filter(looksLikePath)
                .map((p) => ({
                virtualTool: 'edit',
                filePath: resolvePath(p, cwd),
    ...
    perl: (args, cwd) => {
        const hasInPlace = args.some((a) => a === '-i' ||
            a.startsWith('-i') ||
            a === '--in-place' ||
            a.startsWith('--in-place=') ||
            hasCombinedShortFlag(a, 'i'));
        const hasExplicitScript = args.some((a) => a === '-e' ||
            a === '-f' ||
            a.startsWith('-e') ||
            hasCombinedShortFlag(a, 'e'));
        const positional = getPositionalArgs(args, new Set(['-e', '-f', '-I', '-M', '-m', '-0'])).filter(looksLikePath);
        const files = hasExplicitScript ? positional : positional.slice(1);
        const tool = hasInPlace ? 'edit' : 'read_file';
        return files.map((p) => ({
            virtualTool: tool,
            filePath: resolvePath(p, cwd),
        }));
    ...
    curl: (args, cwd) => {
        ...
        return [
            ...
            ...getPositionalArgs(args, flagsWithValue)
                .filter((p) => p.includes('://') || /^https?:\/\//.test(p) || /^ftp:\/\//.test(p))
                .flatMap((url) => {
                const op = webOp(url);
                return op ? [op] : [];
            }),
 ...
 */
const PREFIX_COMMAND_FLAGS_WITH_VALUE = new Map([
    [
        'sudo',
        new Set([
            '-u',
 ...
 */
export function extractShellOperations(simpleCommand, cwd) {
    ...
    const { readFiles: redirectReads, writeFiles: redirectWrites } = extractRedirects(tokens, cwd);
...
function isDynamicShellPath(word) {
    return word.includes('$') || word.includes('`');
}
function resolveCdTargetCwd(command, cwd, cwdUnknown) {
    ...
    let targetIndex = 1;
    while (targetIndex < words.length &&
 ...
 *     "cd .lailatulcoder && bash -lc 'echo {} > settings.json'",
 *     '/repo',
 *   )
 *   // → [{ virtualTool: 'write_file', filePath: '/repo/.lailatulcoder/settings.json' }]
 */
export function extractShellOperationsAcrossCommand(command, cwd) {
    return walkCompoundCommand(command, cwd, 0, false);
}
function extractFindExecOps(args, cwd) {
    const ops = [];
    for (let i = 0; i < args.length; i++) {
        const marker = args[i];
        if (marker !== '-exec' &&
...
function getHeredocDelimiters(line) {
    const delimiters = [];
    let inSingle = false;
    let inDouble = false;
    let escaped = false;
...
function walkCompoundCommand(command, cwd, depth, initialCwdUnknown) {
    ...
    for (const { command: sub, terminator } of subCommands) {
        ...
        if (depth < MAX_SHELL_UNWRAP_DEPTH) {
            ...
            if (subUnwrapped !== sub) {
                ops.push(...walkCompoundCommand(subUnwrapped, effectiveCwd, depth + 1, cwdUnknown));
                continue;
            }
        }
        else if (stripShellWrapper(sub) !== sub) {
            shellSemanticsDebugLogger.warn(`Shell wrapper unwrap depth limit reached (${MAX_SHELL_UNWRAP_DEPTH}); analysing remaining command as-is.`);
        }
...
function hasAbsolutePathTokenForOperation(command, cwd, filePath) {
    for (const token of tokenize(command)) {
        const redirectTarget = token.match(/^(?:1>>|1>|>>|>|2>>|2>|&>>|&>|<)(.+)$/)?.[1];
...
function markCwdUnknownOps(ops, command, cwd) {
    return ops.map((op) => {
        if (!op.filePath)
            return op;
        return {
            ...op,
            cwdUnknown: true,
            pathMayDependOnCwd: !hasAbsolutePathTokenForOperation(command, cwd, op.filePath),
        };
    });
}
//# sourceMappingURL=shell-semantics.js.map