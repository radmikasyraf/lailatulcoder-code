...
import { useExtensionUpdates, useSettingInputRequests, useConfirmUpdateRequests, usePluginChoiceRequests, } from './useExtensionUpdates.js';
import { QWEN_DIR, ExtensionUpdateState, } from '@lailatul-coder/lailatul-coder-core';
...
function createMockExtension(overrides = {}) {
    return {
...
function createMockExtensionManager(extensions, checkCallback, updateResult) {
    return {
        getLoadedExtensions: vi.fn(() => extensions),
        checkForAllExtensionUpdates: vi.fn(async (callback) => {
            if (checkCallback) {
...
describe('useConfirmUpdateRequests', () => {
    it('should add a confirmation request', () => {
        ...
        expect(result.current.confirmUpdateExtensionRequests[0].prompt).toBe('Test prompt');
    });
    ...
    it('should handle multiple confirmation requests', () => {
        ...
        expect(result.current.confirmUpdateExtensionRequests).toHaveLength(1);
        expect(result.current.confirmUpdateExtensionRequests[0].prompt).toBe('Prompt 2');
...
describe('useSettingInputRequests', () => {
    it('should add a setting input request', () => {
        ...
        expect(result.current.settingInputRequests[0].settingDescription).toBe('Enter your API key');
...
describe('useExtensionUpdates', () => {
    ...
    it('should check for updates and log a message if an update is available', async () => {
        ...
        const addItem = vi.fn();
        const cwd = '/test/cwd';
        const extensionManager = createMockExtensionManager([extension], async (callback) => {
    ...
    it('should check for updates and automatically update if autoUpdate is true', async () => {
        ...
        renderHook(() => useExtensionUpdates(extensionManager, addItem, tempHomeDir));
        await waitFor(() => {
            expect(addItem).toHaveBeenCalledWith({
                type: MessageType.INFO,
    ...
    it('should batch update notifications for multiple extensions with autoUpdate: false', async () => {
        ...
        const addItem = vi.fn();
        const cwd = '/test/cwd';
        const extensionManager = createMockExtensionManager([extension1, extension2], async (callback) => {
            callback('test-extension-1', ExtensionUpdateState.UPDATE_AVAILABLE);
...
describe('usePluginChoiceRequests', () => {
    it('should add a plugin choice request', () => {
        ...
        expect(result.current.pluginChoiceRequests[0].marketplaceName).toBe('test-marketplace');
    ...
    it('should handle multiple plugin choice requests', () => {
        ...
        expect(result.current.pluginChoiceRequests).toHaveLength(1);
        expect(result.current.pluginChoiceRequests[0].marketplaceName).toBe('marketplace-2');
