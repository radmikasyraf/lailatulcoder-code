...
import { channelLoopPath, daemonChannelLoopPath, daemonChannelStateDir, daemonObservedContactsPath, daemonSessionRoutesPath, parseConfiguredChannels, registerBackgroundResponseRelay, registerPermissionRelay, registerSessionCleanup, sessionsPath, } from './runtime.js';
vi.mock('@lailatul-coder/lailatul-coder-core', () => ({
    Storage: { getGlobalQwenDir: () => '/tmp/qwen' },
    hashDaemonWorkspace: (workspace) => workspace === '/workspace' ? 'workspace-hash' : 'other-hash',
}));
...
vi.mock('./channel-registry.js', () => ({
    getPlugin: async (type) => type === 'telegram'
...
it('isolates daemon route stores by workspace hash', () => {
    expect(daemonSessionRoutesPath('/workspace')).toBe(path.join('/tmp/qwen', 'channels', 'daemon', 'workspace-hash', 'routes.json'));
    expect(daemonSessionRoutesPath('/other')).toBe(path.join('/tmp/qwen', 'channels', 'daemon', 'other-hash', 'routes.json'));
    expect(daemonSessionRoutesPath('/workspace')).not.toBe(sessionsPath());
});
it('isolates observed contact stores beside daemon routes', () => {
    expect(daemonObservedContactsPath('/workspace')).toBe(path.join('/tmp/qwen', 'channels', 'daemon', 'workspace-hash', 'observed-contacts.json'));
    expect(daemonObservedContactsPath('/other')).toBe(path.join('/tmp/qwen', 'channels', 'daemon', 'other-hash', 'observed-contacts.json'));
    expect(daemonObservedContactsPath('/workspace')).not.toBe(sessionsPath());
});
it('isolates daemon loop stores by workspace hash', () => {
    expect(daemonChannelLoopPath('/workspace')).toBe(path.join('/tmp/qwen', 'channels', 'daemon', 'workspace-hash', 'cron.json'));
    expect(daemonChannelLoopPath('/other')).toBe(path.join('/tmp/qwen', 'channels', 'daemon', 'other-hash', 'cron.json'));
...
it('isolates daemon channel state by workspace and safe instance key', () => {
    ...
    expect(stateDir.replaceAll(path.sep, '/')).toMatch(/^\/tmp\/qwen\/channels\/daemon\/workspace-hash\/instances\/team_\.\._bot-[0-9a-f]{16}$/);
...
describe('parseConfiguredChannels', () => {
    ...
    it('throws a clear error when a selected channel is missing config', async () => {
        await expect(parseConfiguredChannels({}, ['telegram'], { defaultCwd: '/workspace' })).rejects.toThrow('Error in channel "telegram": channel is not configured. Add a "telegram" entry under "channels" in settings.json.');
    });
    ...
    it('rejects unresolved credential env vars', async () => {
        await expect(parseConfiguredChannels({
            telegram: {
                type: 'telegram',
                token: '$TOKEN_LITERAL_VALUE',
            },
        }, ['telegram'], { defaultCwd: '/workspace' })).rejects.toThrow('Error in channel "telegram": Environment variable TOKEN_LITERAL_VALUE is not set (referenced as $TOKEN_LITERAL_VALUE). Set the variable or remove the $ prefix to use a literal value.');
    });
...
describe('registerPermissionRelay', () => {
    ...
    it('cancels permission requests when no route exists', async () => {
        ...
        try {
            registerPermissionRelay(bridge, router, new Map());
            ...
            await vi.waitFor(() => expect(bridge.respondToPermission).toHaveBeenCalledWith('req-1', {
                outcome: { outcome: 'cancelled' },
            }));
            expect(stderr.mock.calls.join('')).toContain('No route for session missing-session; cancelling permission req-1');
        }
        finally {
            stderr.mockRestore();
        }
    ...
    it('does not crash cancelling permission requests without a responder', () => {
        const stderr = vi
            ...
            .mockImplementation(() => true);
        const bridge = createBridge();
        delete bridge.respondToPermission;
        const router = { getTarget: vi.fn() };
        try {
            registerPermissionRelay(bridge, router, new Map());
            expect(() => bridge.emit('permissionRequest', {
                requestId: 'req-1',
                sessionId: 'missing-session',
                request: {
                    toolCall: {
    ...
    it('cancels permission requests when channel dispatch fails', async () => {
        ...
        registerPermissionRelay(bridge, router, new Map([['telegram', channel]]));
        bridge.emit('permissionRequest', {
            requestId: 'req-1',
            sessionId: 'session-1',
            request: {
    ...
    it('logs before cancelling permission requests with no channel', async () => {
        ...
        try {
            ...
            expect(stderr.mock.calls.join('')).toContain('No channel "telegram" for session session-1; cancelling permission req-1');
        }
        finally {
            stderr.mockRestore();
        }
    ...
    it('broadcasts resolved permission requests to channels', () => {
        ...
        registerPermissionRelay(bridge, { getTarget: vi.fn() }, new Map([['telegram', channel]]));
...
describe('registerBackgroundResponseRelay', () => {
    it('routes the final background response without joining the active prompt', async () => {
        ...
        registerBackgroundResponseRelay(bridge, router, new Map([['telegram', channel]]));
        bridge.emit('backgroundResponse', 'session-1', 'Background final answer.');
        await vi.waitFor(() => {
            expect(channel.dispatchBackgroundResponse).toHaveBeenCalledWith('session-1', 'Background final answer.');
        });
    });
    it('logs when no route exists for the background response', async () => {
        ...
        const router = { getTarget: vi.fn() };
        try {
            registerBackgroundResponseRelay(bridge, router, new Map());
            bridge.emit('backgroundResponse', 'session-1', 'Background final answer.');
            await vi.waitFor(() => {
                expect(stderr.mock.calls.join('')).toContain('No route for background response from session session-1');
            });
        }
        finally {
            stderr.mockRestore();
    ...
    it('logs when the channel is not found for the background response', async () => {
        ...
        try {
            ...
            bridge.emit('backgroundResponse', 'session-1', 'Background final answer.');
            await vi.waitFor(() => {
                expect(stderr.mock.calls.join('')).toContain('No channel "telegram" for background response from session session-1');
            });
        }
        finally {
            stderr.mockRestore();
    ...
    it('logs when dispatchBackgroundResponse rejects', async () => {
        ...
        try {
            ...
            bridge.emit('backgroundResponse', 'session-1', 'Background final answer.');
            await vi.waitFor(() => {
                expect(stderr.mock.calls.join('')).toContain('Background response relay failed for session session-1');
            });
        }
        finally {
            stderr.mockRestore();
...
describe('registerSessionCleanup', () => {
    it('updates routing state when no channel matches the dead session', () => {
        ...
        registerSessionCleanup(bridge, router, new Map());
