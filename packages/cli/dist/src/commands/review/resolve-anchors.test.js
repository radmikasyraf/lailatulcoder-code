...
describe('validateRequests', () => {
    ...
    it('rejects duplicate ids rather than resolving them into a wrong answer', () => {
        ...
        expect(() => validateRequests([ok, { ...ok, anchor: 'other();' }])).toThrow(/Duplicate finding id\(s\): f1/);
    });
    it('rejects a missing or empty anchor', () => {
        expect(() => validateRequests([{ id: 'f1', path: 'src/a.ts' }])).toThrow(/"anchor"/);
        expect(() => validateRequests([{ ...ok, anchor: '' }])).toThrow(/"anchor"/);
    });
    it.each(['42', -1, 0, 1.5, Number.NaN, Number.POSITIVE_INFINITY])('rejects an invalid claimed line %s', (line) => {
        ...
        // there.
        expect(() => validateRequests([{ ...ok, line }])).toThrow(/invalid "line"/);
    });
...
describe('resolve-anchors (command boundary)', () => {
    ...
    function runWithStderr(findings) {
        let stderr = '';
        const spy = vi
    ...
    it('routes each finding to its own file in a multi-file diff', () => {
        ...
        expect(report.resolved.map((r) => [r.id, r.line])).toEqual([
            ['a', 12],
            ['b', 101],
        ]);
    ...
    it('fails on a diff path that does not exist', () => {
        const input = join(dir, 'in.json');
        writeFileSync(input, '[]');
        expect(() => resolveAnchorsCommand.handler({
...
//# sourceMappingURL=resolve-anchors.test.js.map