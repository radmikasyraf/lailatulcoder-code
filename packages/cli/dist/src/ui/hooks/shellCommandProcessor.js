...
import { compactToolResultDisplayForHistory, createDebugLogger, isSignalTermination, isBinary, ShellExecutionService, } from '@lailatul-coder/lailatul-coder-core';
 ...
 */
export const useShellCommandProcessor = (addItemToHistory, setPendingHistoryItem, onExec, onDebugMessage, config, geminiClient, setShellInputFocused, terminalWidth, terminalHeight) => {
    const [activeShellPtyId, setActiveShellPtyId] = useState(null);
    const handleShellCommand = useCallback((rawQuery, abortSignal) => {
        ...
        const executeCommand = async (resolve) => {
            ...
            try {
                    ...
                    if (shouldUpdate ||
                        ...
                        setPendingHistoryItem((prevItem) => {
                            if (prevItem?.type === 'tool_group') {
                                return {
                                    ...prevItem,
                                    tools: prevItem.tools.map((tool) => tool.callId === callId
                                        ? {
                                            ...tool,
                                            resultDisplay: typeof currentDisplayOutput === 'string'
                                                ? compactToolResultDisplayForHistory(currentDisplayOutput)
                ...
                }, abortSignal, config.getShouldUseNodePtyShell(), shellExecutionConfig);
                ...
                if (pid) {
                    ...
                    setPendingHistoryItem((prevItem) => {
                        if (prevItem?.type === 'tool_group') {
                            return {
                                ...prevItem,
                                tools: prevItem.tools.map((tool) => tool.callId === callId ? { ...tool, ptyId: pid } : tool),
                            };
                        }
                        return prevItem;
                    });
