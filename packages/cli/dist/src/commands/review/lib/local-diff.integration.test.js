...
import { captureLocalDiff, isBinarySection, MAX_UNTRACKED_BYTES, MAX_UNTRACKED_FILES, MAX_UNTRACKED_TOTAL_BYTES, } from './local-diff.js';
...
describe('captureLocalDiff — untracked files', () => {
    ...
    it('includes a brand-new file that `git diff HEAD` cannot see', () => {
        write('src/payment.ts', 'export function pay(amt: number) {\n  return amt;\n}\n');
    ...
    it('never claims to have reviewed a directory-shaped untracked entry', () => {
        ...
        for (const s of res.skipped)
            expect(s.reason).toContain('directory');
    ...
    it('records a file git could not diff, and keeps the others', () => {
        ...
        for (const p of res.untracked)
            expect(res.text).toContain(`+++ b/${p}`);
    ...
    it('reports an oversized tracked diff instead of inlining it', () => {
        ...
        const big = 'export const x = "' + 'y'.repeat(MAX_UNTRACKED_TOTAL_BYTES) + '";\n';
        write('huge.ts', big);
        git('add', 'huge.ts');
        const res = capture();
    ...
    it('refuses a `file` outside the repository', () => {
        expect(() => capture({ file: '../../etc/passwd' })).toThrow(/outside the repository/);
    });
...
//# sourceMappingURL=local-diff.integration.test.js.map