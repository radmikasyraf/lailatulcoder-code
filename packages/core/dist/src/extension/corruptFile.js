...
export function quarantineCorruptFile(filePath) {
    ...
    try {
        ...
        process.stderr.write(`[warn] Corrupt extension state file ${filePath} moved aside to ${quarantinePath}\n`);
        debugLogger.warn(`Corrupt file ${filePath} could not be parsed; moved aside to ${quarantinePath}.`);
    }
    catch (error) {
        process.stderr.write(`[warn] Corrupt extension state file ${filePath} could not be moved aside: ${error instanceof Error ? error.message : String(error)}\n`);
        debugLogger.warn(`Corrupt file ${filePath} could not be parsed and could not be moved aside: ${error instanceof Error ? error.message : String(error)}`);
    }
}
//# sourceMappingURL=corruptFile.js.map