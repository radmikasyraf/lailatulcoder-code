...
import { createDebugLogger, unescapePath, escapePath, getExternalEditorCommand, } from '@lailatul-coder/lailatul-coder-core';
...
export function useTextBuffer({ initialText = '', initialCursorOffset = 0, viewport, stdin, setRawMode, onChange, isValidPath, shellModeActive = false, preferredEditor, }) {
    ...
    const setText = useCallback((newText, options) => {
        dispatch({
            type: 'set_text',
            payload: newText,
    ...
    const openInExternalEditor = useCallback(async (opts = {}) => {
        ...
        let filePath;
        try {
            tmpDir = fs.mkdtempSync(pathMod.join(os.tmpdir(), 'LailatulCoder-edit-'));
