...
import { __resetActiveGoalStoreForTests, getActiveGoal, getLastGoalTerminal, notifyGoalTerminal, setActiveGoal, setGoalTerminalObserver, } from '@lailatul-coder/lailatul-coder-core';
import { collectGoalStatusItemsFromRecords, findGoalToRestore, findLastTerminalGoal, goalTerminalEventToHistoryItem, parseGoalStatusItem, recordGoalStatusItem, restoreGoalFromHistory, } from './restoreGoal.js';
const goalItem = (overrides) => ({
    id: 1,
    type: 'goal_status',
    kind: 'set',
...
const userItem = (text = 'hi') => ({ id: 2, type: 'user', text });
...
describe('restoreGoalFromHistory', () => {
    ...
    it('restores an active goal and re-registers the hook', () => {
        ...
        const result = restoreGoalFromHistory([goalItem({ kind: 'set', condition: 'write hello' })], cfg);
    ...
    it('skips restore when workspace is no longer trusted and clears stale in-memory goal', () => {
        ...
        const result = restoreGoalFromHistory([goalItem({ kind: 'set', condition: 'do x' })], cfg);
    ...
    it('restores the terminal observer when an active goal is restored', () => {
        ...
        const addItem = vi.fn();
        const result = restoreGoalFromHistory([goalItem({ kind: 'checking', condition: 'do x' })], cfg, addItem);
    ...
    it.each([
        ['an active goal is restored', 'checking'],
        ['there is no goal to restore', 'achieved'],
    ])('tears down an existing terminal observer when %s and no addItem is given', (_label, kind) => {
        ...
        restoreGoalFromHistory([goalItem({ kind, condition: 'do x' })], makeConfig());
        notifyGoalTerminal('sess-1', {
            kind: 'achieved',
            condition: 'do x',
            iterations: 1,
...
const slashCommandRecord = (outputHistoryItems, phase = 'result') => ({
    uuid: 'rec-1',
    parentUuid: null,
    sessionId: 'sess-1',
...
describe('parseGoalStatusItem', () => {
    ...
    it('returns null when condition is missing or not a string', () => {
        expect(parseGoalStatusItem({ type: 'goal_status', kind: 'set' })).toBeNull();
        expect(parseGoalStatusItem({ type: 'goal_status', kind: 'set', condition: 7 })).toBeNull();
    });
...
describe('collectGoalStatusItemsFromRecords', () => {
    ...
    it('ignores invocation-phase records', () => {
        expect(collectGoalStatusItemsFromRecords([
            slashCommandRecord([{ type: 'goal_status', kind: 'set', condition: 'goal A' }], 'invocation'),
        ])).toEqual([]);
    });
    it('ignores non-slash_command system records and other record types', () => {
        ...
        const user = { ...slashCommandRecord([]), type: 'user' };
...
describe('goalTerminalEventToHistoryItem', () => {
    ...
    it('prefers lastReason over systemMessage when both are present', () => {
        ...
        expect(goalTerminalEventToHistoryItem({
            ...
            systemMessage: 'Goal max iterations reached; cleared.',
        }).lastReason).toBe('two tests still fail');
    });
});
...
describe('restoreGoalFromHistory carries the original start time', () => {
    ...
    it('restores setAt from the set card rather than restarting the clock', () => {
        const cfg = makeConfig();
        restoreGoalFromHistory([goalItem({ kind: 'set', condition: 'do x', setAt: 1000 })], cfg);
    ...
    it('ignores a non-positive setAt from a corrupted transcript', () => {
        const cfg = makeConfig();
        const now = Date.now();
        restoreGoalFromHistory([goalItem({ kind: 'set', condition: 'do x', setAt: 0 })], cfg);
...
describe('restoreGoalFromHistory refuses an empty condition', () => {
    ...
    it('does not register a hook for a blank condition', () => {
        ...
        const cfg = makeConfig();
        expect(restoreGoalFromHistory([goalItem({ kind: 'set', condition: '' })], cfg)).toEqual({ restored: false, blockedBy: 'condition-invalid' });
        expect(getActiveGoal('sess-1')).toBeUndefined();
    });
});
describe('recordGoalStatusItem', () => {
    ...
    it('warns when there is no chat recording service to persist the card', () => {
        ...
        recordGoalStatusItem(makeConfig({
            getChatRecordingService: vi.fn().mockReturnValue(undefined),
        }), item);
        expect(stderr).toHaveBeenCalledWith(expect.stringContaining('no chat recording service'));
    });
    it('warns but does not throw when the recording write fails', () => {
        ...
        expect(stderr).toHaveBeenCalledWith(expect.stringContaining('failed to record goal_status'));
    });
});
//# sourceMappingURL=restoreGoal.test.js.map