...
import { openQwenRealtimeSession } from './LailatulCoder-realtime-session.js';
...
export class LiveSetupController {
    ...
    async retryInstall() {
        if (!this.deps.getEnabled()) {
            throw new LiveSetupError('Enable Live Voice before installing LailatulCoder Live Host.', 'live_setup_disabled', 409);
        }
    ...
    async launchHost() {
        if (!this.deps.getEnabled()) {
            throw new LiveSetupError('Enable Live Voice before launching LailatulCoder Live Host.', 'live_setup_disabled', 409);
        }
