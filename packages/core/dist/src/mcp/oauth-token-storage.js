...
function warnPlaintextTokenStorage(tokenFile) {
    if (didWarnPlaintextTokenStorage) {
        return;
    }
    didWarnPlaintextTokenStorage = true;
    const message = `MCP OAuth tokens are stored unencrypted at ${tokenFile}. ` +
...
export class MCPOAuthTokenStorage {
    ...
    async getAllCredentials() {
        ...
        try {
            ...
            if (error.code !== 'ENOENT') {
                debugLogger.error(`Failed to load MCP OAuth tokens: ${getErrorMessage(error)}`);
            }
        }
        return tokenMap;
    }
    ...
    async setCredentials(credentials) {
        ...
        try {
            ...
            debugLogger.error(`Failed to save MCP OAuth token: ${getErrorMessage(error)}`);
            throw error;
        }
    }
    /**
        ...
        if (tokens.delete(serverName)) {
            ...
            try {
                if (tokenArray.length === 0) {
                    ...
                    await atomicWriteFile(tokenFile, JSON.stringify(tokenArray, null, 2), { mode: 0o600, forceMode: true, noFollow: true });
                    warnPlaintextTokenStorage(tokenFile);
                }
            }
            catch (error) {
                debugLogger.error(`Failed to remove MCP OAuth token: ${getErrorMessage(error)}`);
            }
        }
    }
    /**
    ...
    async clearAll() {
        ...
        try {
            ...
            if (error.code !== 'ENOENT') {
                debugLogger.error(`Failed to clear MCP OAuth tokens: ${getErrorMessage(error)}`);
            }
        }
    }
}
