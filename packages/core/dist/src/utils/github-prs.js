...
const GH_PR_LIST_FIELDS = 'number,title,url,author,headRefName,isDraft,reviewDecision,statusCheckRollup,updatedAt';
...
function aggregateChecks(rollup) {
    ...
    for (const entry of rollup) {
        if (entry.__typename === 'StatusContext') {
            ...
            if (state && FAILING_STATUS_CONTEXT_STATES.has(state))
                return 'failing';
            if (state === 'SUCCESS')
                sawPassing = true;
            else
            ...
            if (conclusion) {
                if (FAILING_CHECK_RUN_CONCLUSIONS.has(conclusion))
                    return 'failing';
                sawPassing = true;
            }
            else {
...
function runGhPrList(gitRoot, env) {
...
function ghErrorMessage(error, commandLabel = 'gh pr list', timeoutMs = GH_TIMEOUT_MS) {
 ...
 */
export async function fetchGitHubPullRequests(cwd, env) {
...
function runGhPrCreate(gitRoot, opts, env) {
 ...
 */
export async function createGitHubPullRequest(cwd, opts, env) {
    const gitRoot = findGitRoot(cwd);
    if (!gitRoot)
        return { kind: 'not_a_repo' };
    let stdout;
 ...
 * "origin/main").
 */
export async function getDefaultBranch(cwd, env) {
    const gitRoot = findGitRoot(cwd);
    if (!gitRoot)
        return null;
    try {
...
//# sourceMappingURL=github-prs.js.map