...
const mocks = vi.hoisted(() => ({
    ...
    reviewLeaseHeldByAnotherSession: vi.fn((_lease) => false),
    ...
    releaseWorktree: vi.fn((_path) => ({
        existed: false,
        freed: false,
        reason: undefined,
    })),
    ghApiAll: vi.fn((_path) => []),
    currentUser: vi.fn(() => 'reviewer'),
    setGhHost: vi.fn(),
    getGhHost: vi.fn(() => undefined),
}));
...
vi.mock('../../services/review-worktree-lease.js', () => ({
    ...
    isReviewLeaseFile: (fileName) => /^qwen-review-lease-pr-\d+\.json$/.test(fileName),
}));
vi.mock('./lib/git.js', () => ({
...
vi.mock('./lib/paths.js', () => ({
    worktreePath: (prNumber) => `/repo/.qwen/tmp/review-pr-${prNumber}`,
    probeWorktreePath: (path) => `${path}-probe`,
    baseWorktreePath: (path) => `${path}-base`,
    scratchWorktreePrefix: (path) => `${path}-scratch-`,
    reviewBranch: (prNumber) => `qwen-review/pr-${prNumber}`,
    LEASE_PREFIX: 'qwen-review-lease-',
    REVIEW_TMP_DIR: '/repo/.qwen/tmp',
    tmpFile: (target, suffix) => `/repo/.qwen/tmp/qwen-review-${target}-${suffix}`,
    tmpPrefix: (target) => `qwen-review-${target}-`,
}));
import { findUnsanctionedIssueComments, findUnsanctionedReviews, runCleanup, } from './cleanup.js';
describe('runCleanup', () => {
    beforeEach(() => {
        vi.clearAllMocks();
        ...
        mocks.readdirSync.mockImplementation((_path) => []);
    ...
    it('keeps the lease when branch deletion fails', () => {
        ...
        runCleanup('pr-123');
        expect(mocks.execFileSync).toHaveBeenCalledWith('git', ['branch', '-D', 'qwen-review/pr-123'], 
        ...
        expect(mocks.writeStderrLine).toHaveBeenCalledWith(expect.stringContaining('Failed to delete branch qwen-review/pr-123'));
    ...
    it('clears the lease when cleanup succeeds', () => {
        ...
        runCleanup('pr-123');
        expect(mocks.clearReviewWorktreeLease).toHaveBeenCalledWith(process.cwd(), 'pr-123');
    });
    it('clears the lease when only a side file fails to delete', () => {
        ...
        runCleanup('pr-123');
        expect(mocks.writeStderrLine).toHaveBeenCalledWith(expect.stringContaining('Failed to remove'));
        expect(mocks.clearReviewWorktreeLease).toHaveBeenCalledWith(process.cwd(), 'pr-123');
    });
    it('skips the whole target when another session holds the lease (#9205)', () => {
        ...
        mocks.reviewLeaseHeldByAnotherSession.mockImplementationOnce((l) => l === lease);
        ...
        expect(mocks.readReviewWorktreeLease).toHaveBeenCalledWith(process.cwd(), 'pr-123');
        expect(mocks.releaseWorktree).not.toHaveBeenCalled();
        expect(mocks.execFileSync).not.toHaveBeenCalled();
        expect(mocks.rmSync).not.toHaveBeenCalled();
        expect(mocks.ghApiAll).not.toHaveBeenCalled();
        expect(mocks.clearReviewWorktreeLease).not.toHaveBeenCalled();
        expect(mocks.writeStdoutLine).toHaveBeenCalledWith(expect.stringContaining('skipped cleanup for "pr-123"'));
        expect(mocks.writeStdoutLine).toHaveBeenCalledWith(expect.stringContaining('session-a'));
        ...
        expect(mocks.writeStdoutLine).toHaveBeenCalledWith(expect.stringContaining('qwen-review-lease-pr-123.json'));
    });
    ...
    it('re-checks the lease after the network-bound audit and skips if a session moved in during it (#9205)', () => {
        ...
        expect(mocks.readReviewWorktreeLease).toHaveBeenNthCalledWith(1, process.cwd(), 'pr-123');
        expect(mocks.readReviewWorktreeLease).toHaveBeenNthCalledWith(2, process.cwd(), 'pr-123');
        ...
        const auditNoteIndex = mocks.writeStderrLine.mock.calls.findIndex((c) => String(c[0]).includes('bypass audit skipped'));
        expect(auditNoteIndex).toBeGreaterThanOrEqual(0);
        expect(mocks.readReviewWorktreeLease.mock.invocationCallOrder[1]).toBeGreaterThan(mocks.writeStderrLine.mock.invocationCallOrder[auditNoteIndex]);
        // Nothing of B's may be touched.
        expect(mocks.releaseWorktree).not.toHaveBeenCalled();
        expect(mocks.execFileSync).not.toHaveBeenCalled();
        expect(mocks.rmSync).not.toHaveBeenCalled();
        expect(mocks.clearReviewWorktreeLease).not.toHaveBeenCalled();
        expect(mocks.writeStdoutLine).toHaveBeenCalledWith(expect.stringContaining('acquired the lease'));
    });
    ...
    it('unlinks a dangling symlink at a family path, which releaseWorktree cannot see', () => {
        ...
        runCleanup('pr-123');
        expect(mocks.rmSync).toHaveBeenCalledWith('/repo/.qwen/tmp/review-pr-123-scratch-verify--round-1--aaa', { force: true });
        expect(mocks.writeStdoutLine).toHaveBeenCalledWith(expect.stringContaining('Removed scratch worktree link'));
    });
    it('unlinks a symlink at the three NAMED family paths instead of releasing what it points at', () => {
        ...
        expect(mocks.rmSync).toHaveBeenCalledWith('/repo/.qwen/tmp/review-pr-123-probe', { force: true });
        expect(mocks.rmSync).toHaveBeenCalledWith('/repo/.qwen/tmp/review-pr-123-base', { force: true });
        expect(mocks.writeStdoutLine).toHaveBeenCalledWith(expect.stringContaining('Removed worktree link'));
        expect(mocks.writeStdoutLine).toHaveBeenCalledWith(expect.stringContaining('Removed probe worktree link'));
        expect(mocks.writeStdoutLine).toHaveBeenCalledWith(expect.stringContaining('Removed base worktree link'));
        ...
        expect(mocks.execFileSync).toHaveBeenCalledWith('git', ['worktree', 'prune'], expect.anything());
    });
    it('does not announce a clean sweep when it could not list the family', () => {
        ...
        runCleanup('pr-123');
        expect(mocks.writeStderrLine).toHaveBeenCalledWith(expect.stringContaining('for scratch worktrees'));
        expect(mocks.writeStdoutLine).not.toHaveBeenCalledWith(expect.stringContaining('Nothing to clean'));
        expect(mocks.clearReviewWorktreeLease).not.toHaveBeenCalled();
    });
    it('refuses to clean anything when the temp dir hangs off a symlink', () => {
        ...
        runCleanup('pr-123');
        expect(mocks.writeStderrLine).toHaveBeenCalledWith(expect.stringContaining('Refusing to clean'));
        expect(mocks.rmSync).not.toHaveBeenCalled();
        expect(mocks.releaseWorktree).not.toHaveBeenCalled();
        expect(mocks.clearReviewWorktreeLease).not.toHaveBeenCalled();
    });
    it('sweeps a stale base-tree build lock left by a killed builder', () => {
        ...
        mocks.execFileSync.mockReturnValue(Buffer.from(''));
        runCleanup('pr-123');
        expect(mocks.rmSync).toHaveBeenCalledWith('/repo/.qwen/tmp/review-pr-123-base.lock', { recursive: true, force: true });
    });
    it('never sweeps lease files, even for a target whose name collides with the lease prefix (#9205)', () => {
        ...
        runCleanup('lease');
        expect(mocks.rmSync).not.toHaveBeenCalledWith(join('/repo/.qwen/tmp', 'qwen-review-lease-pr-123.json'), expect.anything());
        expect(mocks.writeStdoutLine.mock.calls.map((c) => String(c[0])).join('\n')).not.toContain('qwen-review-lease-pr-123.json');
    });
    it('sweeps the side files of a lease-named target that share the lease prefix', () => {
        ...
        expect(mocks.rmSync).not.toHaveBeenCalledWith(join('/repo/.qwen/tmp', 'qwen-review-lease-pr-999.json'), expect.anything());
    });
    it('still sweeps side files that match the target prefix', () => {
        ...
        expect(mocks.writeStdoutLine).toHaveBeenCalledWith(`Removed temp file: ${sideFile}`);
    });
    it('keeps the record directory of a NON-CONVERGED reverse audit (#9206)', () => {
        ...
        expect(removed).not.toContain('/repo/.qwen/tmp/qwen-review-pr-123-fetch-prompts');
        expect(mocks.writeStdoutLine).toHaveBeenCalledWith(expect.stringContaining('Kept /repo/.qwen/tmp/qwen-review-pr-123-fetch-prompts'));
    });
    it('keeps the record directory whose records predate the plan — a killed loop leaves no marker (#9206)', () => {
        ...
        mocks.execFileSync.mockReturnValue(Buffer.from(''));
        mocks.existsSync.mockReturnValue(true);
        mocks.readdirSync.mockImplementation((p) => p === '/repo/.qwen/tmp'
        ...
        mocks.statSync.mockImplementation((p) => ({
            mtimeMs: p.endsWith('.json') ? planNow : Date.parse('2020-01-01'),
        }));
        runCleanup('pr-123');
        expect(mocks.rmSync).not.toHaveBeenCalledWith('/repo/.qwen/tmp/qwen-review-pr-123-fetch-prompts', expect.anything());
        expect(mocks.writeStdoutLine).toHaveBeenCalledWith(expect.stringContaining('Kept /repo/.qwen/tmp/qwen-review-pr-123-fetch-prompts'));
    });
    it('keeps the record directory whose plan is already gone — a second cleanup keeps what the first kept (#9213)', () => {
        ...
        mocks.execFileSync.mockReturnValue(Buffer.from(''));
        mocks.existsSync.mockImplementation((p) => p === '/repo/.qwen/tmp');
    ...
    it('still sweeps the record directory once the loop converged (#9206)', () => {
        ...
        runCleanup('pr-123');
        expect(mocks.rmSync).toHaveBeenCalledWith('/repo/.qwen/tmp/qwen-review-pr-123-fetch-prompts', { recursive: true, force: true });
    });
});
...
describe('findUnsanctionedReviews', () => {
    ...
    it('excludes every receipt-vouched review id, not just the last', () => {
        ...
        const got = findUnsanctionedReviews([review({ id: 1 }), review({ id: 2 }), review({ id: 3 })], 'reviewer', since, new Set([2, 3]));
        expect(got.map((r) => r.id)).toEqual([1]);
    });
});
describe('runCleanup — bypass-write audit', () => {
    ...
    it('flags reviewer issue comments posted inside the window', () => {
        ...
        runCleanup('pr-123');
        expect(mocks.readFileSync).toHaveBeenCalledWith('/repo/.qwen/tmp/qwen-review-pr-123-fetch.json', 'utf8');
        expect(mocks.setGhHost).toHaveBeenCalledWith('ghe.example.com');
        expect(mocks.ghApiAll).toHaveBeenCalledWith(expect.stringContaining('repos/acme/widgets/issues/123/comments'));
    ...
    it('skips the audit without gh calls when the fetch report is absent or pre-fetchedAt, and names the skip', () => {
        ...
        mocks.readFileSync.mockReturnValue(JSON.stringify({ prNumber: '123', ownerRepo: 'acme/widgets' }));
    ...
    it('reaches back past the recorded opening by the clock-skew allowance', () => {
        ...
        mocks.readFileSync.mockReturnValue(fetchReport);
        mocks.ghApiAll.mockImplementation((path) => path.includes('/issues/')
            ? [
                {
                    id: 11,
                    user: { login: 'reviewer' },
        ...
        expect(String(mocks.ghApiAll.mock.calls.find(([p]) => String(p).includes('/issues/'))[0])).toContain(encodeURIComponent('2026-07-24T07:58:00.000Z'));
    });
    ...
    it('renders the edited-comment warning with id, timestamp and URL through runCleanup', () => {
        ...
        const warnings = mocks.writeStdoutLine.mock.calls
            .map((c) => String(c[0]))
            .filter((l) => l.startsWith('warning:'));
        expect(warnings.join('\n')).toContain('edited comment 21 at 2026-07-24T09:10:00Z — https://ghe.example.com/acme/widgets/pull/123#c21');
    });
    it('flags an in-window review with no receipt, and spares the receipt-vouched one', () => {
        ...
        mocks.ghApiAll.mockImplementation((path) => path.includes('/reviews')
            ? [
                {
                    id: 500,
                    user: { login: 'reviewer' },
