...
const AUTO_MODE_FIRST_TIME_MESSAGE_FALLBACK = 'Auto mode enabled.\n' +
...
export function useAutoAcceptIndicator({ config, settings, addItem, onApprovalModeChange, shouldBlockTab, disabled, }) {
    ...
    useKeypress((key) => {
        ...
        if (isShiftTab || isWindowsTab) {
            ...
            try {
                ...
                if (nextApprovalMode === ApprovalMode.AUTO &&
...
export function emitAutoModeEntryNotices(opts) {
    ...
    if (!acknowledged) {
        addItem({ type: MessageType.INFO, text: getAutoModeFirstTimeMessage() }, now);
        if (settings) {
            try {
