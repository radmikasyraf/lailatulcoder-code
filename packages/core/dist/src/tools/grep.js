...
class GrepToolInvocation extends BaseToolInvocation {
    ...
    async execute(signal) {
        try {
            ...
            const matchesByFile = matchesToInclude.reduce((acc, match) => {
            ...
            const appendChunk = (chunk, match) => {
                if (Number.isFinite(charLimit) &&
                    grepOutput.length + chunk.length > charLimit) {
                    grepOutput += chunk.slice(0, Math.max(charLimit - grepOutput.length, 0));
                    grepOutput += '...';
                    if (match)
            ...
            const resultFilePaths = Array.from(new Set(visibleMatches
    ...
    parseGrepOutput(output, basePath) {
        ...
        if (!output)
            return results;
        const pushMatch = (filePathRaw, lineNumberStr, lineContent) => {
        ...
        if (output.includes('\0')) {
            ...
            while (index < output.length) {
                ...
                if (nextNewline !== -1 && nextNewline < pathEnd) {
                    debugLogger.debug(`Skipping unframed grep output line: ${output.substring(index, nextNewline)}`);
                    index = nextNewline + 1;
                    continue;
                }
                ...
                if (separator !== -1) {
                    pushMatch(filePathRaw, rest.substring(0, separator), rest.substring(separator + 1));
                }
                else {
                    debugLogger.debug(`Skipping malformed grep --null record for ${filePathRaw}`);
                }
                index = lineEnd + 1;
            }
            return results;
