...
import { createWorkspaceSessionOwnerIndex, createWorkspaceGenerationGuard, createWorkspaceRegistry, createSingleWorkspaceRegistry, } from './workspace-registry.js';
function bridgeWithSummary(getSessionSummary) {
    return { getSessionSummary };
}
function makeRuntime(workspaceCwd, overrides = {}) {
...
describe('createSingleWorkspaceRegistry', () => {
    ...
    it('keeps a failed replacement as a queryable blocked entry', () => {
        ...
        const recovered = registry.activateReplacement(entry, replacement, 'policy-3');
...
describe('createWorkspaceRegistry', () => {
    it('hides internal runtimes from ordinary selectors and preserves owner routing', () => {
        ...
        const internal = makeRuntime('/work/conversations', {
            ...
            removable: false,
            bridge: bridgeWithSummary((sessionId) => ({
                sessionId,
        ...
        expect(registry.getEntryByWorkspaceId(internal.workspaceId)).toBeUndefined();
        expect(registry.resolveWorkspaceCwd(internal.workspaceCwd)).toBeUndefined();
        expect(registry.listAll()).toEqual([primary, internal]);
        expect(registry.getManagedByWorkspaceId(internal.workspaceId)).toBe(internal);
        ...
        expect(() => registry.activateReplacement(entry, makeRuntime(internal.workspaceCwd, {
            workspaceId: internal.workspaceId,
        }), 'policy-2')).toThrow(/identity does not match/);
    });
    it('does not fall back to an ordinary duplicate while an indexed internal owner is unavailable', () => {
        ...
        expect(registry.beginReplacement(registry.getManagedEntryByWorkspaceId(internal.workspaceId), 'policy-2')).toBe(true);
    ...
    it('keeps runtime order frozen and uses the marked primary runtime', () => {
        ...
        expect(() => registry.list().push(primary)).toThrow(TypeError);
    ...
    it('rejects invalid registry construction', () => {
        ...
        expect(() => createWorkspaceRegistry([])).toThrow(/at least one workspace runtime/);
        expect(() => createWorkspaceRegistry([makeRuntime('/work/no-primary')])).toThrow(/exactly one primary workspace runtime/);
        expect(() => createWorkspaceRegistry([primary, otherPrimary])).toThrow(/exactly one primary workspace runtime/);
    ...
    it('uses the session owner index before scanning runtime bridges', () => {
        ...
        const secondarySummary = vi.fn((sessionId) => ({
            sessionId,
            workspaceCwd: '/work/secondary',
        }));
        const primary = makeRuntime('/work/primary', {
    ...
    it('drops stale indexed owners without scanning unrelated runtimes', () => {
        const primarySummary = vi.fn((sessionId) => ({
            sessionId,
            workspaceCwd: '/work/primary',
        }));
    ...
    it('hides draining runtimes, rolls back, and releases cwd ownership on completion', () => {
        ...
        expect(registry.getManagedByWorkspaceId(secondary.workspaceId)).toBe(secondary);
    ...
    it('closes the generation when removal commits and cannot reactivate it', () => {
        ...
        expect(generationGuard.closed).toBe(true);
        expect(() => generationGuard.assertOpen()).toThrow('Workspace runtime generation is no longer active.');
        registry.cancelDrain(secondary);
...
//# sourceMappingURL=workspace-registry.test.js.map