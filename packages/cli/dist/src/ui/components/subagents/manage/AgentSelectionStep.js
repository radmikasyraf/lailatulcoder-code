...
export const AgentSelectionStep = ({ availableAgents, onAgentSelect, }) => {
    ...
    const renderAgentItem = (agent, index, isSelected) => {
        ...
        return (_jsxs(Box, { alignItems: "center", children: [_jsx(Box, { minWidth: 2, flexShrink: 0, children: _jsx(Text, { color: isSelected ? theme.text.accent : theme.text.primary, children: isSelected ? ICON.CIRCLE_FILLED : ' ' }) }), _jsxs(Text, { color: textColor, wrap: "truncate", children: [agent.name, agent.isBuiltin && (_jsxs(Text, { color: isSelected ? theme.text.accent : theme.text.secondary, children: [' ', t('(built-in)')] })), agent.level === 'user' && projectNames.has(agent.name) && (_jsxs(Text, { color: isSelected ? theme.status.warning : theme.text.secondary, children: [' ', t('(overridden by project level agent)')] }))] })] }, `${agent.name}-${agent.level}`));
    };
