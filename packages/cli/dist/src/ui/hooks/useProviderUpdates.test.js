...
import { AuthType, CODING_PLAN_CHINA_BASE_URL, CODING_PLAN_ENV_KEY, CODING_PLAN_GLOBAL_BASE_URL, codingPlanProvider, TOKEN_PLAN_BASE_URL, TOKEN_PLAN_ENV_KEY, tokenPlanProvider, buildProviderTemplate, computeModelListVersion, PROVIDER_METADATA_NS, } from '@lailatul-coder/lailatul-coder-core';
...
const chinaTemplate = buildProviderTemplate(codingPlanProvider, CODING_PLAN_CHINA_BASE_URL);
const chinaVersion = computeModelListVersion(chinaTemplate);
const tokenTemplate = buildProviderTemplate(tokenPlanProvider, TOKEN_PLAN_BASE_URL);
...
describe('useProviderUpdates', () => {
    ...
    it('does not show update prompt when no version is stored', () => {
        const { result } = renderHook(() => useProviderUpdates(mockSettings, mockConfig, mockAddItem));
        expect(result.current.providerUpdateRequest).toBeUndefined();
    });
    it('does not show update prompt when versions match', () => {
        mockSettings.merged[PROVIDER_METADATA_NS][METADATA_KEY] = {
            baseUrl: CODING_PLAN_CHINA_BASE_URL,
    ...
    it('uses the stored non-default base URL when versions match', () => {
        const globalTemplate = buildProviderTemplate(codingPlanProvider, CODING_PLAN_GLOBAL_BASE_URL);
        mockSettings.merged[PROVIDER_METADATA_NS][METADATA_KEY] = {
    ...
    it('persists the template version and preserves custom models', async () => {
        ...
        expect(reloaded[AuthType.USE_OPENAI]).toEqual(expect.arrayContaining([
            expect.objectContaining({ id: 'my-custom-model' }),
        ]));
        expect(mockSettings.setValue).toHaveBeenCalledWith(expect.anything(), `${PROVIDER_METADATA_NS}.${METADATA_KEY}.version`, chinaVersion);
    });
    it('executes update when user confirms with "update"', async () => {
        ...
        expect(mockSettings.setValue).toHaveBeenCalledWith(expect.anything(), `${PROVIDER_METADATA_NS}.${METADATA_KEY}.baseUrl`, CODING_PLAN_CHINA_BASE_URL);
        expect(mockConfig.reloadModelProvidersConfig).toHaveBeenCalled();
        ...
        expect(mockSettings.setValue).not.toHaveBeenCalledWith(expect.anything(), 'security.auth.selectedType', expect.anything());
    });
    it('preserves the stored global base URL when updating', async () => {
        ...
        await result.current.providerUpdateRequest.onConfirm('update');
        expect(mockSettings.setValue).toHaveBeenCalledWith(expect.anything(), `${PROVIDER_METADATA_NS}.${METADATA_KEY}.baseUrl`, CODING_PLAN_GLOBAL_BASE_URL);
        expect(mockSettings.setValue).toHaveBeenCalledWith(expect.anything(), `${PROVIDER_METADATA_NS}.${METADATA_KEY}.version`, globalVersion);
    });
    it('updates both provider metadata keys from a batched prompt', async () => {
        const metadataNs = mockSettings.merged[PROVIDER_METADATA_NS];
        ...
        await result.current.providerUpdateRequest.onConfirm('update');
        expect(mockSettings.setValue).toHaveBeenCalledWith(expect.anything(), `${PROVIDER_METADATA_NS}.${METADATA_KEY}.version`, chinaVersion);
        expect(mockSettings.setValue).toHaveBeenCalledWith(expect.anything(), `${PROVIDER_METADATA_NS}.${TOKEN_METADATA_KEY}.version`, tokenVersion);
    });
    it.each([
        {
    ...
    ])('does not change auth when updating an inactive provider $name', async ({ activeConfig }) => {
    ...
    it('does not refresh auth before auth initialization completes', async () => {
        mockConfig.getContentGeneratorConfig.mockReturnValue(undefined);
        mockSettings.merged[PROVIDER_METADATA_NS][METADATA_KEY] = {
            baseUrl: CODING_PLAN_CHINA_BASE_URL,
            version: 'old-version-hash',
        };
    ...
    it('does not overwrite existing env key with empty value', async () => {
        ...
        const envCalls = mockSettings.setValue.mock.calls.filter((call) => typeof call[1] === 'string' && call[1].startsWith('env.'));
    ...
    it('leaves the model selection alone when the previous model is gone', async () => {
        ...
        expect(mockModelsConfig.syncAfterAuthRefresh).not.toHaveBeenCalled();
        expect(mockSettings.setValue).not.toHaveBeenCalledWith(expect.anything(), 'model.name', expect.anything());
        expect(mockSettings.setValue).not.toHaveBeenCalledWith(expect.anything(), 'model.baseUrl', expect.anything());
    ...
    it('persists ignoredVersion when user chooses "skip"', async () => {
        ...
        expect(mockSettings.setValue).toHaveBeenCalledWith(expect.anything(), `${PROVIDER_METADATA_NS}.${METADATA_KEY}.ignoredVersion`, chinaVersion);
        expect(mockConfig.reloadModelProvidersConfig).not.toHaveBeenCalled();
    });
    ...
    it('skip persists ignoredVersion for all providers in batch', async () => {
        ...
        expect(mockSettings.setValue).toHaveBeenCalledWith(expect.anything(), `${PROVIDER_METADATA_NS}.${METADATA_KEY}.ignoredVersion`, chinaVersion);
        expect(mockSettings.setValue).toHaveBeenCalledWith(expect.anything(), `${PROVIDER_METADATA_NS}.${TOKEN_METADATA_KEY}.ignoredVersion`, tokenVersion);
    });
...
//# sourceMappingURL=useProviderUpdates.test.js.map