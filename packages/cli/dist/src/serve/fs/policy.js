...
import { isBinaryFile } from '@lailatul-coder/lailatul-coder-core';
 ...
 */
export function shouldIgnore(absolute, boundWorkspace, ignore, kind = 'file') {
    const rel = path.relative(boundWorkspace, absolute);
    ...
    if (kind === 'directory') {
        ...
        if (dirFilter(withSlash))
            return { ignored: true, category: 'directory' };
        if (fileFilter(rel))
            return { ignored: true, category: 'file' };
        return { ignored: false };
    }
    if (fileFilter(rel))
        return { ignored: true, category: 'file' };
 ...
 */
export function assertTrustedForIntent(trusted, intent) {
    if (trusted)
        return;
    switch (intent) {
        case 'read':
        ...
        case 'write':
        case 'edit':
            throw new FsError('untrusted_workspace', `workspace is not trusted; ${intent} operations are forbidden`, {
                hint: 'mark the folder as trusted in the daemon configuration to allow writes',
            });
        default: {
            const _exhaustive = intent;
            throw new FsError('untrusted_workspace', `workspace is not trusted; intent "${String(_exhaustive)}" is not classified`, {
 ...
 */
export function enforceReadSize(fileBytes, maxBytes = MAX_READ_BYTES) {
 ...
 */
export function enforceWriteSize(bytes, maxBytes = MAX_WRITE_BYTES) {
    if (bytes > maxBytes) {
 ...
 */
export function enforceReadBytesSize(fileBytes) {
    if (fileBytes > MAX_READ_BYTES) {
        throw new FsError('file_too_large', `file of ${fileBytes} bytes exceeds read limit of ${MAX_READ_BYTES} bytes`, {
 ...
 */
export async function detectBinary(filePath) {
