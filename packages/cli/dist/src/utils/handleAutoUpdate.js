...
import { getHomebrewLatestVersion, getInstallationInfo, PackageManager, resolveUpdateCommand, } from './installationInfo.js';
...
const UPDATE_SUCCESS_MESSAGE = 'Update successful! The new version will be used on your next run.';
const UPDATE_FAILED_MESSAGE = 'Automatic update failed. Please try updating manually.';
export async function handleAutoUpdate(info, settings, projectRoot, spawnFn = spawnWrapper) {
    if (!info) {
        return;
    }
    ...
    if (installationInfo.packageManager === PackageManager.HOMEBREW) {
        ...
        const brewLatestVersion = semver.coerce((await getHomebrewLatestVersion()) ?? undefined)?.version;
    ...
    if (installationInfo.isStandalone &&
        ...
        performStandaloneUpdate(installationInfo.standaloneDir, info.update.latest)
            ...
            updateEventEmitter.emit('update-failed', {
                message: t('Automatic update failed: {{error}}. Re-run the installer to update manually.', { error: err.message }),
            });
        });
        return;
    }
    ...
    const updateCommand = resolveUpdateCommand(installationInfo.updateCommand, info.update.latest);
    const platform = os.platform();
    const isWindows = platform === 'win32';
    const isManagedNpmUpdate = installationInfo.packageManager === PackageManager.NPM;
    ...
    return new Promise((resolve) => {
        ...
        updateProcess.once('close', (code) => {
            finish(code === 0
                ? undefined
                : new Error(`Command failed: ${updateCommand}; stderr: ${errorOutput.trim()}`));
        });
...
export function setUpdateHandler(addItem, setUpdateInfo, isIdleRef = { current: true }) {
    let successfullyInstalled = false;
    const pendingNotifications = [];
    const addItemOrDefer = (item) => {
    ...
    const handleUpdateFailed = (data) => {
        ...
        addItemOrDefer({
            ...
            type: data?.severity === 'warning' ? MessageType.WARNING : MessageType.ERROR,
