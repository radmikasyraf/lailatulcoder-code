...
const mockUninstallExtension = vi.hoisted(() => vi.fn().mockResolvedValue({ warnings: [] }));
...
describe('extensions uninstall command', () => {
    ...
    it('prints committed uninstall warnings', async () => {
        ...
        expect(mockWriteStdoutLine).toHaveBeenCalledWith('Extension "test-extension" successfully uninstalled.');
        expect(mockWriteStderrLine).toHaveBeenCalledWith('extension_preferences_cleanup_failed: cleanup failed');
    });
});
