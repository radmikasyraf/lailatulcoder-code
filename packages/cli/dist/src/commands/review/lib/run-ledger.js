...
import { atomicWriteFileSync, sanitizeFilenameComponent, } from '@lailatul-coder/lailatul-coder-core';
...
function ledgerOccupant(path) {
    ...
    if (st.size > MAX_LEDGER_BYTES)
        return { kind: 'plant', shape: 'oversize' };
    try {
 ...
 */
function readSessions(planPath) {
    return parseSessions(readLedgerFile(runSessionsPath(planPath)), planPath, planMtimeMs(planPath));
}
/**
 ...
 */
function parseSessions(raw, planPath, planMtime) {
    try {
        if (raw === null)
            return [];
        ...
        const kept = parsed.filter((e) => typeof e === 'object' &&
            e !== null &&
            ...
            planMtime !== null &&
            Math.abs(e.planMtimeMs - planMtime) <=
 ...
 */
export function appendRunSession(planPath, env = process.env, nowMs = Date.now()) {
    try {
        ...
        if (occ.kind === 'refused')
            return;
        const entries = parseSessions(occ.kind === 'ok' ? occ.text : null, planPath, mtime);
 ...
 */
export function sessionEntryCount(planPath, opts = {}) {
    const entries = readSessions(planPath);
    if (opts.excludeSessionId === undefined)
 ...
 */
export function ledgerResumeCount(planPath, opts = {}) {
 ...
 */
export function priorSessionIds(planPath, env = process.env) {
 ...
 */
export function currentSessionEntry(planPath, env = process.env) {
    const raw = env['QWEN_CODE_SESSION_ID']?.trim();
    if (!raw)
        return null;
    const current = sessionPathKey(raw);
    return (readSessions(planPath).find((e) => sessionPathKey(e.sessionId) === current) ?? null);
}
/**
 ...
 */
function resumeAuthorized(planPath, env = process.env) {
    const raw = env['QWEN_CODE_SESSION_ID']?.trim();
    if (!raw)
        return false;
    const current = sessionPathKey(raw);
    return readResumeMarker(planPath).resumes.some((r) => sessionPathKey(r.sessionId) === current);
}
/**
 ...
 */
export function priorSessionEntries(planPath, env = process.env) {
    ...
    if (!resumeAuthorized(planPath, env))
        return [];
    ...
    const ownIdx = current === undefined
        ? -1
...
export function resumeMarkerPath(planPath) {
 ...
 */
export function readResumeMarker(planPath) {
    return parseMarker(readLedgerFile(resumeMarkerPath(planPath)), planPath, planMtimeMs(planPath));
}
/**
 ...
 */
function parseMarker(text, planPath, planMtime) {
    try {
        if (text === null)
            return emptyMarker();
        ...
        const ceiling = runCeilingMs();
        const raw = parsed;
        const seenResume = new Set();
        const seenRestart = new Set();
...
function writeMarker(planPath, marker) {
    try {
        const dir = promptRecordDir(planPath);
        mkdirSync(dir, { recursive: true });
 ...
 */
export function recordResume(planPath, env = process.env, nowMs = Date.now()) {
    const id = env['QWEN_CODE_SESSION_ID']?.trim();
    if (!id || !SESSION_ID_RE.test(id))
        return;
    const mtime = planMtimeMs(planPath);
    ...
    if (occ.kind === 'refused')
        return;
    const marker = parseMarker(occ.kind === 'ok' ? occ.text : null, planPath, mtime);
    if (marker.resumes.some((r) => sessionPathKey(r.sessionId) === sessionPathKey(id)))
        return;
 ...
 */
export function recordRestart(planPath, reason, nowMs = Date.now()) {
    const mtime = planMtimeMs(planPath);
    if (mtime === null)
        return;
    ...
    if (occ.kind === 'refused')
        return;
    const marker = parseMarker(occ.kind === 'ok' ? occ.text : null, planPath, mtime);
    if (marker.restarts.some((r) => r.reason === reason))
        return;
    healPlant(markerPath, occ);
