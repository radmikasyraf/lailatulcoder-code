...
import { AUTH_ENV_MAPPINGS, DEFAULT_MODELS, QWEN_OAUTH_ALLOWED_MODELS, MODEL_GENERATION_CONFIG_FIELDS, } from './constants.js';
 ...
 */
function applyTimeoutEnvOverride(env, generationConfig, sources, modelProvider) {
    if (modelProvider?.generationConfig?.timeout !== undefined)
        return;
...
export function resolveModelConfig(input) {
    ...
    const generationConfig = resolveGenerationConfig(settings?.generationConfig, modelProvider?.generationConfig, authType, modelProvider?.id ?? modelResult.value, sources);
 ...
 */
function resolveQwenOAuthConfig(input, warnings) {
    const { cli, settings, proxy, modelProvider } = input;
    const sources = {};
    // Qwen OAuth only allows specific models
    const allowedModels = new Set(QWEN_OAUTH_ALLOWED_MODELS);
    ...
    if (requestedModel && allowedModels.has(requestedModel)) {
        ...
        if (requestedModel) {
            const isVisionModel = requestedModel.includes('vl') || requestedModel.includes('vision');
            const extraMessage = isVisionModel
                ? ` Note: vision-model has been removed since coder-model now supports vision capabilities.`
                : '';
            warnings.push(`Warning: Unsupported Qwen OAuth model '${requestedModel}', falling back to '${DEFAULT_QWEN_MODEL}'.${extraMessage}`);
        }
    ...
    const generationConfig = resolveGenerationConfig(settings?.generationConfig, modelProvider?.generationConfig, AuthType.lailatulcoder_OAUTH, resolvedModel, sources);
    // ---- Env override: QWEN_CODE_API_TIMEOUT_MS ----
    applyTimeoutEnvOverride(input.env, generationConfig, sources, modelProvider);
    const config = {
        authType: AuthType.lailatulcoder_OAUTH,
        model: resolvedModel,
