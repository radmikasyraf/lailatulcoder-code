...
import { containsUnsafeExternalToolGuardControlCharacter, EXTERNAL_TOOL_GUARD_TOKEN_ENV, isValidExternalToolGuardDenialReason, } from '@lailatul-coder/acp-bridge/externalToolGuard';
export const EXTERNAL_TOOL_GUARD_PROTOCOL_VERSION = 1;
...
function normalizeEndpoint(raw) {
    if (typeof raw !== 'string') {
        throw new TypeError('Invalid external tool guard endpoint: expected an absolute loopback HTTP(S) origin.');
    }
    let endpoint;
    try {
    ...
    if ((endpoint.protocol !== 'http:' && endpoint.protocol !== 'https:') ||
        ...
        endpoint.search !== '' ||
        endpoint.hash !== '') {
        throw new TypeError('Invalid external tool guard endpoint: expected an origin-only loopback HTTP(S) URL.');
    }
    return endpoint;
}
function normalizeToken(raw) {
    if (typeof raw !== 'string' ||
        ...
        throw new TypeError(`Invalid ${EXTERNAL_TOOL_GUARD_TOKEN_ENV}: expected a non-blank token without control characters.`);
    }
...
function normalizeTimeout(timeoutMs) {
    ...
    if (typeof value !== 'number' ||
        ...
        throw new TypeError(`Invalid external tool guard timeout: expected an integer in [${MIN_EXTERNAL_TOOL_GUARD_TIMEOUT_MS}, ${MAX_EXTERNAL_TOOL_GUARD_TIMEOUT_MS}] ms.`);
    }
    return value;
}
...
function validatePrepareResponse(value, requestId) {
    if (!isRecord(value) ||
        !hasExactKeys(value, ['protocolVersion', 'requestId', 'allowed'], ['reason']) ||
        value['protocolVersion'] !== EXTERNAL_TOOL_GUARD_PROTOCOL_VERSION ||
    ...
    if (!isValidExternalToolGuardDenialReason(reason)) {
        throw new Error('External tool guard denial response contains an unsafe reason.');
    }
    return { allowed: false, reason };
}
/**
 * Small direct HTTP(S) client. It intentionally does not use global fetch:
 * LailatulCoder's model proxy may install a process-global dispatcher, while this
...
export class RequiredExternalToolGuard {
    ...
    request(pathname, value) {
        ...
        try {
            ...
            return Promise.reject(new Error('External tool guard request is not JSON serializable.'));
        }
        if (body.byteLength > MAX_REQUEST_BYTES) {
            return Promise.reject(new Error('External tool guard request exceeds the size limit.'));
        }
            ...
            const request = transport.request({
                ...
                response.on('data', (chunk) => {
                    ...
                    if (received > MAX_RESPONSE_BYTES) {
                        finish(new Error('External tool guard response exceeds the size limit.'));
                        response.destroy();
                        return;
                    }
                    chunks.push(bytes);
                ...
                response.on('end', () => {
                    if (response.statusCode !== 200) {
                        finish(new Error(`External tool guard returned HTTP ${response.statusCode ?? 'unknown'}.`));
                        return;
                    }
                    const contentType = response.headers['content-type'];
                    if (typeof contentType !== 'string' ||
                        !/^application\/json(?:\s*;|$)/iu.test(contentType)) {
                        finish(new Error('External tool guard returned a non-JSON content type.'));
                        return;
                    }
                    try {
...
//# sourceMappingURL=external-tool-guard-provider.js.map