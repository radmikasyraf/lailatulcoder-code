...
import { MAX_RETAINED_TOOL_RESULT_DISPLAY_CHARS, } from '@lailatul-coder/lailatul-coder-core';
...
describe('useShellCommandProcessor', () => {
    ...
    beforeEach(() => {
        ...
        mockGeminiClient = { addHistory: vi.fn() };
    ...
    const renderProcessorHook = () => renderHook(() => useShellCommandProcessor(addItemToHistoryMock, setPendingHistoryItemMock, onExecMock, onDebugMessageMock, mockConfig, mockGeminiClient, setShellInputFocusedMock));
    ...
    it('should initiate command execution and set pending state', async () => {
        ...
        expect(mockShellExecutionService).toHaveBeenCalledWith(wrappedCommand, '/test/dir', expect.any(Function), expect.any(Object), false, expect.any(Object));
    ...
    it('compacts large shell output for UI history without changing model history behavior', async () => {
        ...
        const { result } = renderProcessorHook();
        act(() => {
            result.current.handleShellCommand('generate-large-output', new AbortController().signal);
        });
        const execPromise = onExecMock.mock.calls[0][0];
        act(() => {
        ...
        const finalDisplay = finalHistoryItem.tools[0].resultDisplay;
        expect(finalDisplay.length).toBeLessThanOrEqual(MAX_RETAINED_TOOL_RESULT_DISPLAY_CHARS);
        expect(finalDisplay).toContain('head-');
        expect(finalDisplay).toContain('-tail');
        expect(finalDisplay).toContain('truncated from');
        const modelHistoryText = vi.mocked(mockGeminiClient.addHistory).mock.calls[0][0].parts[0].text;
    ...
    it('preserves unmatched surrogate code units in truncated shell model history', async () => {
        ...
        const { result } = renderProcessorHook();
        act(() => {
            result.current.handleShellCommand('surrogate-output', new AbortController().signal);
        });
        const execPromise = onExecMock.mock.calls[0][0];
        act(() => {
    ...
    it('should treat PTY clean-exit signal 0 as a successful command', async () => {
        const { result } = renderProcessorHook();
        act(() => {
            result.current.handleShellCommand('pty-clean-exit', new AbortController().signal);
        });
        const execPromise = onExecMock.mock.calls[0][0];
        act(() => {
        ...
        expect(finalHistoryItem.tools[0].resultDisplay).not.toContain('terminated by signal');
    });
    describe('UI Streaming and Throttling', () => {
        ...
        it('should throttle pending UI updates for text streams (non-interactive)', async () => {
            ...
            const wrappedCommand = `{ stream; }; __code=$?; pwd > "${path.join(os.tmpdir(), 'shell_pwd_abcdef.tmp')}"; exit $__code`;
            expect(mockShellExecutionService).toHaveBeenCalledWith(wrappedCommand, '/test/dir', expect.any(Function), expect.any(Object), false, // enableInteractiveShell
            expect.any(Object));
            ...
            if (!streamUpdateFn || typeof streamUpdateFn !== 'function') {
                throw new Error('setPendingHistoryItem was not called with a stream updater function');
            }
        ...
        it('should show binary progress messages correctly', async () => {
            ...
            expect(stateAfterBinaryDetected).toEqual(expect.objectContaining({
                tools: [
                    expect.objectContaining({
            ...
            expect(stateAfterProgress).toEqual(expect.objectContaining({
                tools: [
                    expect.objectContaining({
    ...
    it('should not wrap the command on Windows', async () => {
        ...
        expect(mockShellExecutionService).toHaveBeenCalledWith('dir', '/test/dir', expect.any(Function), expect.any(Object), false, expect.any(Object));
    });
    ...
    describe('ActiveShellPtyId management', () => {
        ...
        it('should reset activeShellPtyId to null if execution promise rejects', async () => {
            let rejectResultPromise;
            mockShellExecutionService.mockImplementation(() => Promise.resolve({
                pid: 1234_5,
