...
const { ghMock, ghWithInputMock, setGhHostMock, getPlatformReaderMock, gitOptMock, authMock, submitAoneMock, composeMock, stdoutMock, stderrMock, } = vi.hoisted(() => ({
    ghMock: vi.fn(),
...
import { AonePartialPostError, } from './lib/platform/aone.js';
let tmp;
let savedGhHost;
let seq = 0;
...
describe('submit posts an authorised Aone target through a1', () => {
    ...
    it('posts the findings via submitAoneReview, never gh', () => {
        expect(() => runSubmit(base(), 'unknown', { defaultComment: false })).not.toThrow();
        ...
        expect(composeMock.mock.calls[0][0]['contextUnavailable']).toBe(true);
        const req = submitAoneMock.mock.calls[0][0];
        ...
        expect(stderrMock).toHaveBeenCalledWith(expect.stringContaining('no native request-changes state'));
        expect(stderrMock).toHaveBeenCalledWith(expect.stringContaining('1 inline Critical(s) block the merge'));
        ...
        expect(stderrMock).toHaveBeenCalledWith(expect.stringContaining("block the merge while their discussions stay unresolved. They are NOT marked as AI comments — `a1 repo mr comment create` cannot set the flag — so they join the generic discussion gate only; a repo's dedicated ai_comment merge gate does not track them"));
    });
    it('an UNAUTHORISED Aone run takes the normal auth-refusal path first', () => {
        ...
        expect(() => runSubmit(base({ userAuthorized: false }), 'unknown', {
    ...
    it('a padded Aone --host still routes to a1 (the flag is trimmed)', () => {
        ...
        expect(() => runSubmit(base({ host: ' gitlab.alibaba-inc.com ' }), 'unknown', {
            defaultComment: false,
        })).not.toThrow();
        expect(submitAoneMock).toHaveBeenCalledTimes(1);
        expect(ghWithInputMock).not.toHaveBeenCalled();
    ...
    it('an explicit wildcard-family --host (a GHE host) routes to gh, not a1, and binds gh at that host', () => {
        ...
        ghWithInputMock.mockReturnValue('');
        expect(() => runSubmit(base({ host: 'ghe.alibaba-inc.com' }), 'unknown', {
            defaultComment: false,
        })).not.toThrow();
        expect(submitAoneMock).not.toHaveBeenCalled();
        expect(ghWithInputMock).toHaveBeenCalledTimes(1);
    ...
    it('an explicit --host that CONTRADICTS the recorded host refuses — in BOTH directions', () => {
        ...
        expect(() => runSubmit(base({ host: 'github.com' }), 'unknown', {
            defaultComment: false,
        })).not.toThrow();
        expect(process.exitCode).toBe(3);
        ...
        expect(submitAoneMock).not.toHaveBeenCalled();
        expect(ghWithInputMock).not.toHaveBeenCalled();
        expect(stderrMock).toHaveBeenCalledWith(expect.stringContaining('contradicts the host the recorded review'));
    ...
    it('a RECORDED non-Aone host is not vetoed by an Aone cwd probe', () => {
        ...
        expect(composeMock.mock.calls[0][0]['contextUnavailable']).toBeUndefined();
    });
    it('the FAST path with no recording at all refuses — the cwd probe must not guess the platform', () => {
        ...
        expect(stderrMock).toHaveBeenCalledWith(expect.stringContaining('no recorded review names this target'));
        expect(submitAoneMock).not.toHaveBeenCalled();
        expect(ghWithInputMock).not.toHaveBeenCalled();
    });
    ...
    it('a HOSTLESS recording from the --skill-args override refuses — the submission cwd must not stand in for the record of another cwd', () => {
        ...
        expect(stderrMock).toHaveBeenCalledWith(expect.stringContaining('--skill-args'));
        expect(submitAoneMock).not.toHaveBeenCalled();
        expect(ghWithInputMock).not.toHaveBeenCalled();
        ...
        process.exitCode = undefined;
        stdoutMock.mockClear();
        expect(() => runSubmit(base({ userAuthorized: false, host: 'gitlab.alibaba-inc.com' }), 'unknown', { defaultComment: false })).not.toThrow();
        expect(process.exitCode).toBeUndefined();
        expect(submitAoneMock).toHaveBeenCalledTimes(1);
        expect(ghWithInputMock).not.toHaveBeenCalled();
    });
    ...
    it('dry-run validates and composes but never calls a1', () => {
        expect(() => runSubmit(base({ dryRun: true }), 'unknown', { defaultComment: false })).not.toThrow();
        expect(submitAoneMock).not.toHaveBeenCalled();
        expect(ghWithInputMock).not.toHaveBeenCalled();
        const out = postedJson();
        expect(out.posted).toBe(false);
    ...
    it('a mid-batch a1 failure exits 3, warns against a re-run, and carries the structured counts', () => {
        ...
        submitAoneMock.mockImplementation(() => {
            throw new AonePartialPostError('boom after 1 of 3 landed', 1, [11], false);
        });
        expect(() => runSubmit(base(), 'unknown', { defaultComment: false })).not.toThrow();
        expect(process.exitCode).toBe(3);
        expect(postedJson()).toEqual({
        ...
        expect(stderrMock).toHaveBeenCalledWith(expect.stringContaining('do NOT re-run submit'));
        expect(ghWithInputMock).not.toHaveBeenCalled();
    });
    it('an AMBIGUOUS failure warns against a re-run even when the count is zero', () => {
        ...
        submitAoneMock.mockImplementation(() => {
            throw new AonePartialPostError('first create died mid-flight', 0, [], false, true);
        });
        expect(() => runSubmit(base(), 'unknown', { defaultComment: false })).not.toThrow();
        expect(process.exitCode).toBe(3);
        expect(postedJson()).toEqual({
    ...
    it('an UNEXPECTED pre-write error rethrows — gh parity, retryable, nothing landed', () => {
        ...
        expect(() => runSubmit(base(), 'unknown', { defaultComment: false })).toThrow(/token expired/);
        expect(submitAoneMock).toHaveBeenCalledTimes(1);
        expect(ghWithInputMock).not.toHaveBeenCalled();
    });
    it('an APPROVE runs the native approval and reports it', () => {
        ...
        expect(() => runSubmit(base({ review: writeReview(CLEAN_REVIEW) }), 'unknown', {
            defaultComment: false,
        })).not.toThrow();
        const req = submitAoneMock.mock.calls[0][0];
    ...
    it('an approve failure keeps the post but names the missing command', () => {
        ...
        // prevent.
        expect(stderrMock).toHaveBeenCalledWith(expect.stringContaining('a1 repo mr approve 1 --repo maxcompute/odps_src'));
        expect(stderrMock).toHaveBeenCalledWith(expect.stringContaining('ask the USER to run that command'));
    });
    it('a head that moved DURING posting discloses the orphaned pins', () => {
        ...
        expect(() => runSubmit(base(), 'unknown', { defaultComment: false })).not.toThrow();
        expect(postedJson().posted).toBe(true);
        expect(stderrMock).toHaveBeenCalledWith(expect.stringContaining('MR head MOVED during posting'));
    });
    it('an attribution-OFF Aone post strips the severity prefix and appends the invisible marker', () => {
        ...
        expect(stderrMock).toHaveBeenCalledWith(expect.stringContaining('1 inline Critical(s) block the merge'));
        expect(stderrMock).toHaveBeenCalledWith(expect.stringContaining('ai_comment merge gate does not track them'));
    });
    ...
    it('a REQUEST_CHANGES with zero inline Criticals says nothing mechanically blocks', () => {
        ...
        expect(() => runSubmit(base({ review: writeReview(suggestionOnly) }), 'unknown', {
            defaultComment: false,
        })).not.toThrow();
        expect(stderrMock).toHaveBeenCalledWith(expect.stringContaining('NO inline Critical discussions'));
        // The gate disclosure fires only when inline Criticals actually
        // posted — the zero-Critical shape discloses no gate at all.
        expect(stderrMock).not.toHaveBeenCalledWith(expect.stringContaining('does not track them'));
    });
});
//# sourceMappingURL=submit-aone.test.js.map