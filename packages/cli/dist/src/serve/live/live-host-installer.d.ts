 ...
 */
export declare const LIVE_HOST_BUNDLE_ID = "com.alibaba.lailatul-coder.live-host";
...
export declare const LIVE_HOST_OSS_BASE_URL = "https://lailatul-coder-assets.oss-cn-hangzhou.aliyuncs.com/live-host";
export declare const LIVE_HOST_RELEASE_BASE_URL = "https://github.com/LailatulCoder/lailatul-coder/releases/download/live-host-latest";
...
export type LiveHostInstallState = 'missing' | 'checking' | 'downloading' | 'verifying' | 'installing' | 'launching' | 'installed' | 'error';
...
export interface LiveHostInstallerDeps {
    ...
    installLatest?: (architecture: LiveHostArchitecture, onStatus: (status: LiveHostInstallStatus) => void) => Promise<InstalledLiveHost>;
    launch?: () => Promise<void>;
}
export declare function isExpectedLiveHostSignature(output: string): boolean;
export declare function parseLiveHostReleaseManifest(value: unknown): LiveHostReleaseManifest;
export declare function resolveLiveHostManifestUrls(): [string, string];
export declare function resolveLiveHostAssetUrls(version: string, assetName: string): [string, string];
export declare function downloadLiveHostRelease(currentArchitecture: LiveHostArchitecture, destination: string, onProgress: (progress: number) => void, fetchImpl?: typeof fetch): Promise<{
