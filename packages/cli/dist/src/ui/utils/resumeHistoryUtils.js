...
import { getToolResponseDisplayText, isGoalCheckpointBookkeepingRecord, parseGoalStateRecordPayloadV2, projectUserTranscriptForDisplay, } from '@lailatul-coder/lailatul-coder-core';
...
import { formatHistoryGapNotice, indexGapsByChild, } from './history-gap-notice.js';
import { shouldDisplayGoalStateCause } from './goal-runtime.js';
import { collectInlineImages, extractInlineContentRuns, } from './inline-image-parts.js';
/**
 ...
 */
function createHistoryGapItem(gap) {
    return {
        ...
        if (record.type === 'system') {
            ...
            if (record.subtype === 'slash_command') {
                ...
                if (payload.phase === 'invocation' &&
                    ...
                    const sentToModel = typeof payload.sentToModel === 'boolean'
                        ? payload.sentToModel
                        : undefined;
                    items.push({
                        type: 'user',
        ...
        switch (record.type) {
            case 'user': {
                ...
                if (record.subtype === 'notification' || record.subtype === 'cron') {
                    const payload = record.systemPayload;
                    const fallback = record.subtype === 'cron'
                        ? 'Cron job fired'
                        : 'Background agent completed';
                    const text = payload?.displayText ||
                        extractTextFromParts(record.message?.parts) ||
                        fallback;
                ...
                if (record.subtype === 'mid_turn_user_message') {
                    const payload = record.systemPayload;
                    const hasAttachmentReferences = Array.isArray(payload?.attachmentReferences) &&
            ...
            case 'tool_result': {
                ...
                if (record.toolCallResult) {
                    ...
                    if (toolCall) {
                        const responseParts = record.toolCallResult.responseParts ??
                        ...
                        const { images, omittedImageCount } = collectInlineImages(responseParts);
                        if (images.length > 0) {
...
export function buildResumedHistoryItems(sessionData, config, baseTimestamp = Date.now()) {
    ...
    const historyItems = convertToHistoryItems(sessionData.conversation, config, sessionData.historyGaps);
 ...
 */
function applyResumeDisplayPolicy(items) {
 ...
 */
function createHistoryCollapseSummaryItem(messageCount) {
    const n = String(messageCount);
    return {
        type: MessageType.INFO,
        text: t('History collapsed: {{n}} messages hidden. Use /history expand-now to show.', { n }),
 ...
 */
export function stripSuppressOnRestore(item) {
    if (!item.display?.suppressOnRestore)
        return item;
 ...
 */
export function expandCollapsedHistory(items) {
    return items
 ...
 */
export function applyCollapsePolicyAndSummary(rawItems, collapseOnResume, collapsePreviewCount = 0) {
    if (!collapseOnResume)
        return rawItems;
    if (collapsePreviewCount === -1)
        return rawItems;
