...
import { createGitHubPullRequest, fetchGitHubPullRequests, getDefaultBranch, } from '@lailatul-coder/lailatul-coder-core';
...
const passthroughMutate = () => ((_req, _res, next) => next());
...
describe('workspace GitHub PR routes', () => {
    ...
    it('lists pull requests for the selected trusted workspace', async () => {
        ...
        expect(fetchGitHubPullRequestsMock).toHaveBeenCalledWith('/work/secondary', undefined);
    });
    ...
    it('returns runtime-unavailable on default-branch when the generation is closed', async () => {
        ...
        const response = await request(app).get('/workspaces/primary/github/default-branch');
    ...
    it('creates a pull request and returns 201 with url and number', async () => {
        ...
        expect(createGitHubPullRequestMock).toHaveBeenCalledWith('/work/main', {
            title: 'Add a thing',
            body: undefined,
            base: undefined,
            head: undefined,
...
//# sourceMappingURL=workspace-github-prs.test.js.map