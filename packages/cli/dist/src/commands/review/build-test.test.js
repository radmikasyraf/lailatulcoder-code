...
import { run, runBuildTest, trimOutput, unresolvedWorkspaceDeps, buildRunEnv, } from './build-test.js';
import { npmToolchainAdapter, unresolvedWorkspaceDeps as toolchainUnresolvedWorkspaceDeps, } from './lib/npm-toolchain.js';
...
describe('unresolvedWorkspaceDeps', () => {
    ...
    it('finds the workspace package a TS2307 names', () => {
        const out = "src/a.ts(23,8): error TS2307: Cannot find module '@x/webui' or its " +
    ...
    it("reads a bundler's wording too", () => {
        expect(unresolvedWorkspaceDeps('✘ [ERROR] Could not resolve "@x/webui"', PKGS)).toEqual(['@x/webui']);
    });
    ...
    it('returns nothing for output with no unresolved module at all', () => {
        expect(unresolvedWorkspaceDeps('src/a.ts(1,1): error TS2345: nope', PKGS)).toEqual([]);
    });
});
...
describe('run (capture-time failing-file measurement)', () => {
    it.skipIf(process.platform === 'win32')('records failing files the trim then drops from the report', () => {
        ...
        const r = run(`printf '%s\\n' "${'p'.repeat(4_000)}" "${failLines}" ` +
            `"${'x'.repeat(20_000)}" "Tests 2 failed | 5 passed"`, process.cwd(), 30_000);
    ...
    it.skipIf(process.platform === 'win32')('omits the field for a command that named no test file', () => {
        ...
        const r = run("printf 'added 2054 packages in 24s\\n'", process.cwd(), 30_000);
...
describe('runBuildTest', () => {
    ...
    it('treats a package.json with no build role as no npm project at all', () => {
        ...
        expect(rep).toEqual({
            ...
            ok: true,
            timedOut: [],
            note: 'No npm package here to scope (no workspaces, and the root has no build/test ' +
    ...
    it('surfaces the declared-but-empty workspaces note when no adapter applies', () => {
        writeFileSync(join(root, 'package.json'), JSON.stringify({ name: 'r', workspaces: ['packages/*'] }));
        writePlan(['src/a.ts']);
        const rep = runBuildTest({
            plan: planPath,
            worktree: root,
        ...
        expect(rep.note).toContain('declares npm workspaces, but none resolve to a package');
    });
    ...
    it('reports `unsupported` — not a false "nothing to build" — for an unmodeled glob', () => {
        ...
        writeFileSync(join(root, 'packages', 'a', 'package.json'), JSON.stringify({ name: '@x/a', scripts: { build: 'exit 0' } }));
        ...
        expect(rep.note).toContain('uses a workspace glob shape this command does not model');
    ...
    it('says no tests ran for a single-package repo whose root defines only a build script', () => {
        ...
        writeFileSync(join(root, 'package.json'), JSON.stringify({ name: 'solo', scripts: { build: 'exit 0' } }));
        writePlan(['src/index.ts']);
        const calls = [];
        const rep = runBuildTest({
            plan: planPath,
    ...
    it('is `unsupported` for a single-package repo with no build/test script', () => {
        writeFileSync(join(root, 'package.json'), JSON.stringify({ name: 'solo', scripts: { lint: 'exit 0' } }));
        writePlan(['src/index.ts']);
        const rep = runBuildTest({
            plan: planPath,
            worktree: root,
    ...
    it('reorders when two affected packages have an undeclared source-reach', () => {
        ...
        expect(rep.ok).toBe(true);
        expect(rep.buildSet.indexOf('packages/zzz')).toBeLessThan(rep.buildSet.indexOf('packages/aaa'));
    });
    ...
    const wsOf = (command) => /--workspace="([^"]+)"/.exec(command)?.[1] ?? '';
    const okExec = (command) => ({ command, exitCode: 0, seconds: 1, timedOut: false, output: '' });
    it('rescues the runner summary from a trimmed middle', () => {
        ...
        expect(trimOutput('h\n' + 'x'.repeat(3000) + `\n${colored}\n` + 'y'.repeat(9000))).toContain(colored);
    });
    ...
    it('buildOnly builds the same set but runs NO tests', () => {
        ...
        expect(buildOnly.build.map((b) => b.command)).toEqual(withTests.build.map((b) => b.command));
    ...
    it('discloses a budget-stopped single-root suite instead of claiming no test script', () => {
        ...
        expect(calls).toEqual(['npm run build']);
        expect(rep.test).toEqual([]);
        expect(rep.note).toContain('whole-call budget was spent before any suite could run');
    ...
    it('records a caveat — and still runs the closure — when it covers more than half the workspaces', () => {
        ...
        expect(rep.testScope?.caveat).toContain('3 of 5 testable suites (including the root)');
    ...
    it('throws a descriptive error for a plan that is valid JSON but not an object', () => {
        ...
        expect(() => runBuildTest({ plan: bad, worktree: root, timeout: 5, install: false })).toThrow(/not a JSON object/);
            ...
            exec: (command, cwd, _timeoutMs) => {
                ...
                if (command.startsWith('npm ci')) {
                    ...
                    return {
                        ...
                        seconds: 190,
                        timedOut: false,
                        output: "client/components/ChatEditor.tsx(21,10): error TS2300: Duplicate identifier 'useWebShellPortalRoot'.",
                    };
                }
                return {
                    command,
    ...
    it('frames a TEST timeout as infrastructure, not a defect to correlate', () => {
        ...
        const rep = runBuildTest({
            ...
            timeout: 60,
            install: false,
            exec: (command) => command.startsWith('npm test')
    ...
    describe('--resume: the ceiling is per call, not per run', () => {
        ...
        it('a COMPLETED zero-suite run is not "ended before its test phase"', () => {
            ...
            writeFileSync(outPath, JSON.stringify({ ...base, endedBeforeTests: true }));
        ...
        it('a continuation that runs suites drops the stale endedBeforeTests stamp', () => {
            ...
            writeFileSync(outPath, JSON.stringify({
                ...
                timedOut: [],
                note: 'the whole-call budget (16s) was spent with 3 suite(s) still ' +
        ...
        it('retires the superseded budget-stop caveat and keeps live limitations', () => {
            ...
            threePackages();
            const outPath = join(root, 'report.json');
            const liveSegment = '10 changed file(s) sit in negated workspaces (e.g. pkg/x) — excluded';
            // As the producer writes it: `caveat` is the joined prose, `liveCaveat`
            // the scope's own half without the machine clause.
            const report = (caveat, liveCaveat) => ({
                toolchain: 'npm',
                run: runId(),
                affected: ['packages/core'],
                buildSet: ['packages/core', 'packages/a', 'packages/b'],
            ...
            writeFileSync(outPath, JSON.stringify(report(`${liveSegment}; the whole-call budget (61s) was spent with 2 ` +
                `suite(s) still to run — not run: packages/a, packages/b`, liveSegment)));
            const kept = resume();
            expect(kept.testScope?.caveat).toBe(liveSegment);
            expect(kept.testScope?.caveat).not.toContain('still to run');
            writeFileSync(outPath, JSON.stringify(report('the whole-call budget (61s) was spent with 2 suite(s) still ' +
        ...
        it('retires its own clause whole across a SECOND resume', () => {
            ...
            writeFileSync(outPath, JSON.stringify({
                ...
                testScope: {
                    workspaces: ['packages/core'],
                    notRun: ['packages/a', 'packages/b'],
                    caveat: 'the whole-call budget (61s) was spent with 2 suite(s) still ' +
                        'to run — not run: packages/a, packages/b',
                    liveCaveat: '',
                },
            }));
        ...
        it('cannot be talked out of a LIVE limitation by a PR-authored name', () => {
            ...
            threePackages();
            const outPath = join(root, 'report.json');
            const live = '2 changed file(s) could not be mapped to a workspace (e.g. ' +
        ...
        it('carries the install-failure framing through the merge', () => {
            ...
            expect(rep.note).toContain('never as a Critical, and never against this PR');
            expect(rep.note).toContain('Continued from a previous build-test call');
        });
        ...
        it("refuses to continue another run's report — identity, not just shape", () => {
            ...
            writeFileSync(outPath, JSON.stringify({ ...base, run: { root: '/somewhere/else' } }));
            ...
            // run's.
            writeFileSync(outPath, JSON.stringify({ ...base, run: { ...runId(), sha: 'aaaa1111' } }));
        ...
        it('stamps the run identity a future resume will verify', () => {
            ...
            writeFileSync(shaPlan, JSON.stringify({
                diffPathAbsolute: '/dev/null',
...
//# sourceMappingURL=build-test.test.js.map