...
import { findCellIndex, getCellDisplayId, getNotebookLanguage, hasStableCellIds, inferInsertedCellSourceArrayStyle, inferNotebookJsonFormat, isAmbiguousCellId, makeCellId, normalizeEditedCell, normalizeSource, parseNotebook, serializeNotebook, toNotebookSource, } from '../utils/notebook.js';
...
function requireNotebookSource(source, mode) {
    ...
    if (typeof source !== 'string') {
        throw new NotebookEditError(`new_source is required when edit_mode is "${mode}".`, ToolErrorType.INVALID_TOOL_PARAMS);
    }
    return source;
}
...
function resolveTargetIndex(notebook, cellId, mode) {
    if (!cellId) {
        ...
        throw new NotebookEditError('cell_id is required for replace and delete operations.', ToolErrorType.INVALID_TOOL_PARAMS);
    }
    if (isAmbiguousCellId(notebook, cellId)) {
        throw new NotebookEditError(`Cell ID "${cellId}" is ambiguous in the rendered notebook. Re-read the notebook and target a stable real cell ID before editing.`, ToolErrorType.INVALID_TOOL_PARAMS);
    }
    const index = findCellIndex(notebook, cellId);
    if (index === -1) {
        throw new NotebookEditError(`Cell with ID "${cellId}" not found in notebook.`, ToolErrorType.NOTEBOOK_CELL_NOT_FOUND);
    }
    return index;
}
...
export function applyNotebookEdit(rawContent, params) {
    let notebook;
    try {
        notebook = parseNotebook(rawContent);
    }
    catch (error) {
        throw new NotebookEditError(error instanceof Error ? error.message : String(error), ToolErrorType.NOTEBOOK_INVALID_JSON);
    }
    ...
    switch (mode) {
        case 'insert': {
            ...
            const newCell = createNotebookCell(notebook, cellType, source, inferInsertedCellSourceArrayStyle(notebook, insertAt));
        ...
        case 'replace': {
            ...
            if (!target) {
                throw new NotebookEditError(`Cell index ${targetIndex} is out of range.`, ToolErrorType.NOTEBOOK_CELL_NOT_FOUND);
            }
        ...
        default:
            throw new NotebookEditError(`Unsupported notebook edit mode: ${mode}`, ToolErrorType.INVALID_TOOL_PARAMS);
    }
}
...
class NotebookEditInvocation extends BaseToolInvocation {
    ...
    getDescription() {
        const relativePath = makeRelative(this.params.notebook_path, this.config.getTargetDir());
    ...
    async getConfirmationDetails(abortSignal) {
        ...
        const fileDiff = createPatchSmart(fileName, prepared.originalContent, prepared.updatedContent, 'Current', 'Proposed');
    ...
    async prepareEdit(abortSignal) {
        const preDecision = await checkPriorNotebookRead(this.config, this.params.notebook_path);
        if (!preDecision.ok) {
        ...
        try {
            ...
            const code = error?.code;
            if (code === 'ENOENT') {
                throw new StructuredToolError(`Notebook file not found: ${this.params.notebook_path}`, ToolErrorType.FILE_NOT_FOUND);
            }
            throw new StructuredToolError(`Error reading notebook: ${error instanceof Error ? error.message : String(error)}`, ToolErrorType.READ_CONTENT_FAILURE);
        }
        const postDecision = await checkPriorNotebookRead(this.config, this.params.notebook_path, { expectExisting: true });
        if (!postDecision.ok) {
        ...
        try {
            if (this.modifyMetadata?.modifiedNotebookContent !== undefined) {
                return {
                    ...prepareModifiedNotebookContent(originalContent, this.modifyMetadata.modifiedNotebookContent, this.params),
                    originalContent,
                    bom,
                    encoding,
                    lineEnding,
    ...
    async execute(signal) {
        ...
        try {
            ...
            const errorType = error instanceof StructuredToolError
                ? error.errorType
        ...
        // .lailatulcoder/team-memory/ could otherwise carry credentials past the guard
        // that write-file.ts/edit.ts enforce. Block before any disk side effects.
        const teamMemoryError = checkTeamMemorySecrets(this.params.notebook_path, prepared.updatedContent, this.config.getProjectRoot());
        if (teamMemoryError) {
            return {
        ...
        try {
            ...
            const writeDecision = await checkPriorNotebookRead(this.config, this.params.notebook_path, { expectExisting: true });
            if (!writeDecision.ok) {
                return {
            ...
            if (!this.modifyMetadata?.modifiedByUser) {
                CommitAttributionService.getInstance().recordEdit(this.params.notebook_path, prepared.originalContent, prepared.updatedContent);
            }
            try {
                ...
                debugLogger.warn(`[NotebookEdit] post-write cache update failed for ${this.params.notebook_path}: ${error instanceof Error ? error.message : String(error)}`);
            }
            const fileName = path.basename(this.params.notebook_path);
            const fileDiff = createPatchSmart(fileName, prepared.originalContent, prepared.updatedContent, 'Current', 'Proposed');
            const diffStat = getDiffStat(fileName, prepared.originalContent, this.modifyMetadata?.aiProposedContent ?? prepared.updatedContent, prepared.updatedContent);
            logFileOperation(this.config, new FileOperationEvent(NotebookEditTool.Name, FileOperation.UPDATE, prepared.updatedContent.split('\n').length, getSpecificMimeType(this.params.notebook_path), '.ipynb', prepared.language));
            const displayResult = {
                fileDiff,
                fileName,
                originalContent: prepared.originalContent,
            ...
            const llmContent = this.modifyMetadata?.modifiedNotebookContent !== undefined
                ...
                : `Notebook ${this.params.notebook_path} has been updated. ${prepared.mode} cell ${prepared.editedCellId}.${prepared.mode === 'delete'
                    ? ''
...
export class NotebookEditTool extends BaseDeclarativeTool {
    ...
    modifyMetadataByParams = new WeakMap();
    modifyMetadataByKey = new Map();
    constructor(config) {
        super(NotebookEditTool.Name, ToolDisplayNames.NOTEBOOK_EDIT, `Edits a Jupyter notebook (.ipynb) safely at the cell level. Use this instead of ${ToolNames.EDIT} or ${ToolNames.WRITE_FILE} for notebook cells. Supports replacing, inserting, and deleting cells. Always read the notebook first with ${ToolNames.READ_FILE}; then use the cell IDs shown in that output.`, Kind.Edit, {
            properties: {
                notebook_path: {
    ...
    validateToolParamValues(params) {
        ...
        if (typeof params.new_source === 'string') {
            const teamMemoryError = checkTeamMemorySecrets(params.notebook_path, params.new_source, this.config.getProjectRoot());
            if (teamMemoryError) {
    ...
    createInvocation(params) {
        return new NotebookEditInvocation(this.config, params, this.consumeModifyMetadata(params));
    }
    ...
    getModifyContext(_abortSignal) {
        const currentContentSnapshots = new Map();
        const readCurrentContentSnapshot = async (params) => {
        ...
        return {
            getFilePath: (params) => params.notebook_path,
            getCurrentContent: async (params) => readCurrentContentSnapshot(params),
            ...
            createUpdatedParams: (oldContent, modifiedProposedContent, originalParams) => {
                ...
                if (aiProposedContent === undefined) {
                    try {
                        aiProposedContent = applyNotebookEdit(oldContent, originalParams).updatedContent;
                    }
                    catch {
