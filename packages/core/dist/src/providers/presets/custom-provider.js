 ...
 * Old 6-char keys persist in settings.json (and ~/.lailatulcoder/env-equivalent
...
function normalizeEnvSegment(value) {
    ...
    for (let i = 0; i < upper.length; i++) {
        ...
        const isAlphaNum = (code >= 65 /* A */ && code <= 90) /* Z */ ||
            (code >= 48 /* 0 */ && code <= 57); /* 9 */
        if (isAlphaNum) {
            result += upper[i];
    ...
    while (start < end && result.charCodeAt(start) === 95 /* _ */)
        start++;
    while (end > start && result.charCodeAt(end - 1) === 95 /* _ */)
        end--;
...
export function generateCustomEnvKey(protocol, baseUrl) {
    ...
    const suffix = createHash('sha256')
        ...
        .slice(0, 12)
        .toUpperCase();
    return `${CUSTOM_API_KEY_ENV_PREFIX}${normalizeEnvSegment(protocol)}_${normalizeEnvSegment(baseUrl)}_${suffix}`;
}
