 ...
 */
import { ExtensionManager, redactUrlCredentials, getExtensionDisplayName, getExtensionDescription, } from '@lailatul-coder/lailatul-coder-core';
...
export function resolveExtensionCommandScope(scope) {
    if (!scope) {
        return SettingScope.User;
    }
    const normalized = scope.toLowerCase();
    const matched = EXTENSION_COMMAND_SCOPES.find((candidate) => candidate.toLowerCase() === normalized);
    if (matched) {
        return matched;
    }
...
export function extensionToOutputString(extension, extensionManager, workspaceDir, inline = false) {
