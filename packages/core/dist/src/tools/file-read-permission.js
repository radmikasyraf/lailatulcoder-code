...
export function getFileReadDefaultPermission(config, requestedPath) {
    ...
    const allowedRoots = [
        ...
        // Approved plans are persisted here (default ~/.lailatulcoder/plans, outside
    ...
    if (workspaceContext.isPathWithinWorkspace(filePath) ||
        ...
        // (per-project + user-level under ~/.lailatulcoder/memories/) — never the
        // broad getMemoryBaseDir() — to avoid exposing sensitive ~/.lailatulcoder
        ...
        // under a repo-tracked `.lailatulcoder/`, so canonicalizing it would let a checked-in
        // `.lailatulcoder -> /outside` relocate the allowed root — see
...
//# sourceMappingURL=file-read-permission.js.map