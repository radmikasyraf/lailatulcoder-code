...
const { ghMock, ensureAuthenticatedMock, setGhHostMock, writeStdoutLineMock } = vi.hoisted(() => ({
    ghMock: vi.fn(),
...
vi.mock('./lib/platform/registry.js', async (importOriginal) => {
    ...
    return {
        ...actual,
        getPlatformReader: (hint) => readerKindMock() === 'aone'
            ? {
                kind: 'aone',
                ...
                fetchDiff: () => '',
                getCommentBody: () => '',
                fetchHeadRefSpec: (n) => `refs/merge-requests/${n}/head`,
                getFetchMeta: () => {
                    throw new Error('not used');
                },
            }
...
let savedGhHost;
function saveAndClearGhHost() {
    savedGhHost = process.env['GH_HOST'];
    delete process.env['GH_HOST'];
}
function restoreGhHost() {
...
describe('runMeta', () => {
    ...
    it('resolves the cwd repository (upstream in a fork clone) with host from the URL', () => {
        ghMock.mockReturnValue('{"owner":{"login":"LailatulCoder"},"name":"lailatul-coder","url":"https://github.com/LailatulCoder/lailatul-coder"}');
        const result = runMeta({});
        expect(ghMock).toHaveBeenCalledWith('repo', 'view', '--json', 'owner,name,url,parent');
    ...
    it('resolves a fork clone to its PARENT, where the PR actually lives', () => {
        ...
        // parent.url.
        ghMock.mockReturnValue('{"owner":{"login":"contributor"},"name":"lailatul-coder","url":"https://github.com/contributor/lailatul-coder","parent":{"owner":{"login":"LailatulCoder"},"name":"lailatul-coder"}}');
        const result = runMeta({});
        expect(result.ownerRepo).toBe('LailatulCoder/lailatul-coder');
        expect(result.host).toBe('github.com');
    });
    it('keeps an explicit port in the derived host', () => {
        ghMock.mockReturnValue('{"owner":{"login":"o"},"name":"r","url":"https://ghe.example.com:8443/o/r"}');
        expect(runMeta({}).host).toBe('ghe.example.com:8443');
    });
    it('aligns gh routing with the discovered host before the PR call', () => {
        ghMock
            .mockReturnValueOnce('{"owner":{"login":"o"},"name":"r","url":"https://ghe.example.com/o/r"}')
    ...
    it('an explicit --host flag wins over the discovered host in the cwd branch', () => {
        ghMock
            .mockReturnValueOnce('{"owner":{"login":"o"},"name":"r","url":"https://github.com/o/r"}')
            .mockReturnValueOnce('{"headRefOid":"abc","url":"u"}');
    ...
    it('an operator-exported GH_HOST keeps precedence over the discovered host', () => {
        ...
        ghMock.mockReturnValue('{"owner":{"login":"o"},"name":"r","url":"https://other.example.com/o/r"}');
        const result = runMeta({});
        expect(setGhHostMock).toHaveBeenLastCalledWith('ghe.example.com');
    ...
    it('explicit-branch: an unroutable GH_HOST env value throws naming the env (exit-1 class)', () => {
        ...
        expect(() => runMeta({ prNumber: 1, repo: 'o/r' })).toThrow(/GH_HOST environment.*my_ghe/);
        expect(() => runMeta({ prNumber: 1, repo: 'o/r' })).not.toThrow(TypeError);
    });
    it('adds headSha and webUrl when a PR number is given', () => {
        ghMock.mockReturnValue('{"headRefOid":"2d71a0f851c8c18462cc85b60d90973e132274d8","url":"https://github.com/LailatulCoder/lailatul-coder/pull/8981"}');
        const result = runMeta({ prNumber: 8981, repo: 'LailatulCoder/lailatul-coder' });
        expect(ghMock).toHaveBeenCalledWith('pr', 'view', '8981', '--repo', 'LailatulCoder/lailatul-coder', '--json', 'headRefOid,url');
    ...
    it('labels an explicit --host (port survives), and an empty flag falls through', () => {
        ghMock.mockReturnValue('{"headRefOid":"abc","url":"u"}');
        expect(runMeta({ prNumber: 1, repo: 'o/r', host: 'ghe.example.com:8443' }).host).toBe('ghe.example.com:8443');
        expect(runMeta({ prNumber: 1, repo: 'o/r', host: '' }).host).toBe('github.com');
    });
    ...
    it('rejects a malformed --repo before any gh call, with or without a number', () => {
        expect(() => runMeta({ prNumber: 1, repo: '../escape' })).toThrow(TypeError);
    ...
    describe('no-default-host guard on a non-GitHub platform', () => {
        ...
        it('throws a usage error when --repo is given without --host', () => {
            expect(() => runMeta({ prNumber: 1, repo: 'g/p' })).toThrow(/--repo on a aone target needs --host/);
        ...
        it('an EMPTY-STRING --host is a missing flag (env must not bypass)', () => {
            ...
            process.env['GH_HOST'] = 'ghe.example.com';
            expect(() => runMeta({ prNumber: 1, repo: 'g/p', host: '' })).toThrow(/--repo on a aone target needs --host/);
        });
...
describe('metaCommand handler', () => {
    ...
    it('prints the result as one JSON object', () => {
        ghMock.mockReturnValue('{"owner":{"login":"LailatulCoder"},"name":"lailatul-coder","url":"https://github.com/LailatulCoder/lailatul-coder"}');
        metaCommand.handler({ _: [], $0: 'qwen' });
        expect(process.exitCode).toBeUndefined();
        expect(setGhHostMock).toHaveBeenCalledWith(undefined);
        expect(writeStdoutLineMock).toHaveBeenCalledWith('{"platform":"github","host":"github.com","ownerRepo":"LailatulCoder/lailatul-coder"}');
    });
