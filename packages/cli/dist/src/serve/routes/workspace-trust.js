 ...
 */
import { FatalConfigError } from '@lailatul-coder/lailatul-coder-core';
import { MAX_TRUST_REASON_LENGTH } from '../validation-limits.js';
import { writeStderrLine } from '../../utils/stdioHelpers.js';
import { resolveWorkspaceEntryFromParam, resolveWorkspaceRuntimeFromParam, sendGenerationClosedError, } from '../workspace-route-runtime.js';
import { parseAndValidateWorkspaceClientId } from '../server/request-helpers.js';
import { evaluateDaemonWorkspaceTrust, readDaemonTrustPolicySnapshot, } from '../../config/daemon-trust-policy.js';
function unavailableRuntime(res) {
...
function trustStatusV2(entry, snapshot) {
    ...
    const reconciliationState = entry.state === 'transitioning' ||
...
function sendTrustError(res, route, err) {
    if (sendGenerationClosedError(res, err))
        return;
    writeStderrLine(`LailatulCoder serve: ${route} error: ${err instanceof Error ? err.message : String(err)}`);
...
export function registerWorkspaceTrustRoutes(app, deps) {
    const { boundWorkspace, workspace, mutate, safeBody, parseAndValidateClientId, workspaceRegistry, workspaceTrustHotReloadAvailable, getWorkspaceTrustPolicySnapshot = readDaemonTrustPolicySnapshot, } = deps;
    app.get('/workspace/trust', async (req, res) => {
        try {
            if (req.query['statusVersion'] === '2' &&
                workspaceRegistry &&
                workspaceTrustHotReloadAvailable === true) {
                res
                    .status(200)
                    .json(trustStatusV2(workspaceRegistry.primaryEntry, await getWorkspaceTrustPolicySnapshot()));
                return;
            }
    ...
    app.post('/workspace/trust/request', mutate({ strict: true }), async (req, res) => {
...
export function registerWorkspaceQualifiedTrustRoutes(app, deps) {
    const { workspaceRegistry, mutate, safeBody } = deps;
    const getWorkspaceTrustPolicySnapshot = deps.getWorkspaceTrustPolicySnapshot ?? readDaemonTrustPolicySnapshot;
    ...
    app.post('/workspaces/:workspace/trust/request', mutate({ strict: true }), async (req, res) => {
        const entry = resolveWorkspaceEntryFromParam(workspaceRegistry, req, res);
        if (!entry)
            return;
        ...
        try {
            const status = await runtime.workspaceService.getWorkspaceTrustStatus(ctx);
            if (!status.folderTrustEnabled) {
                res.status(409).json({
                    error: 'Folder trust is disabled for this workspace',
                    code: 'folder_trust_disabled',
            ...
            const result = await runtime.workspaceService.requestWorkspaceTrustChange(ctx, {
                desiredState,
                ...(reason !== undefined ? { reason } : {}),
            });
            res.status(202).json(result);
