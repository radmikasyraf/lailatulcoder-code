...
function findGitBashPath() {
    ...
    const commonPaths = [
        ...
        path.join(process.env['ProgramFiles'] || path.join('C:', 'Program Files'), 'Git', 'bin', 'bash.exe'),
...
export function getShellConfiguration() {
    if (isWindows()) {
        ...
        const isGitBash = msystem?.startsWith('MINGW') ||
 ...
 */
const PARAMETER_EXPANSION_COMMAND = /^("?)\$(?:\{([A-Za-z_][A-Za-z0-9_]*)(?:(:?-)([^}]*))?\}|([A-Za-z_][A-Za-z0-9_]*))\1$/;
function resolveLeadingParameterExpansion(command) {
    let rest = command;
    ...
    while (true) {
        ...
        if (!t || !isEnvAssignmentToken(t.token))
            break;
        rest = t.rest;
    }
    ...
    const useDefault = op === undefined ? false : op === ':-' ? !value : value === undefined;
...
export function getCommandRoot(command) {
    ...
    try {
        const tokens = parse(trimmedCommand).filter((token) => typeof token === 'string');
        let idx = 0;
...
export function stripShellWrapper(command) {
    ...
    while (true) {
        ...
        if (isMonitorCommandMarker(wrapperToken.token, token.token)) {
            ...
            if (!commandToken)
                return trimmed;
            const { value: innerCommand, quote } = stripSymmetricQuotes(commandToken.token);
...
const SELF_KILL_PROCESS_PATTERN = /(^|[^a-z0-9_-])(node(?:\.exe)?|qwen(?:-code)?(?:\.exe)?)(?=$|[^a-z0-9_-])/i;
const QWEN_PROCESS_PATTERN = /(^|[^a-z0-9_-])qwen(?:-code)?(?:\.exe)?(?=$|[^a-z0-9_-])/i;
...
const COMMAND_OPTIONS_WITH_VALUES = new Set();
...
export const SHELL_SELF_KILL_REJECTION = 'Blocked: this command may terminate the running lailatul-coder process because it targets all node/lailatul-coder processes. Use task_stop for managed background shells, or kill a specific PID instead.';
function parseShellSegment(segment) {
    try {
...
function shortOptionBundleHasFlag(token, flag) {
    return (token.startsWith('-') &&
        !token.startsWith('--') &&
        token.length > 2 &&
        token.slice(1).toLowerCase().includes(flag.toLowerCase().slice(1)));
}
function matchesSelfProcessPattern(value) {
    return SELF_KILL_PROCESS_PATTERN.test(value);
}
function matchesQwenProcessPattern(value) {
    return QWEN_PROCESS_PATTERN.test(value);
}
function isBroadNodeFullPattern(value) {
...
function consumeOptionsWithValues(tokens, startIndex, optionsWithValues) {
    ...
    while (index < tokens.length) {
        ...
        if ((optionsWithValues.has(normalized) ||
            [...optionsWithValues].some((option) => shortOptionBundleHasFlag(token, option))) &&
...
function unwrapExecutionPrefixes(tokens) {
    let index = 0;
    ...
    while (index < tokens.length) {
        ...
        if (root === 'sudo') {
            index = consumeOptionsWithValues(tokens, index + 1, SUDO_OPTIONS_WITH_VALUES);
            continue;
        }
        if (root === 'command') {
            index = consumeOptionsWithValues(tokens, index + 1, COMMAND_OPTIONS_WITH_VALUES);
            continue;
        }
        if (root === 'env') {
            index = consumeOptionsWithValues(tokens, index + 1, ENV_OPTIONS_WITH_VALUES);
            while (index < tokens.length &&
...
function getCommandSegments(command, depth = 0) {
    const segments = [];
...
function commandArguments(tokens, optionsWithValues) {
    const args = [];
...
function taskkillTargetsSelf(tokens) {
    for (let index = 1; index < tokens.length; index++) {
        const token = tokens[index];
...
function killallTargetsSelf(tokens) {
    return commandArguments(tokens, KILLALL_OPTIONS_WITH_VALUES).some((arg) => matchesSelfProcessPattern(arg));
}
function pkillTargetsSelf(tokens) {
    ...
    for (let index = 1; index < tokens.length; index++) {
        ...
        if (isOptionToken(token)) {
            if (PKILL_OPTIONS_WITH_VALUES.has(normalized) &&
                !optionHasInlineValue(token)) {
                index++;
            }
            continue;
    ...
    if (usesFullCommandLine) {
        return args.some((arg) => matchesQwenProcessPattern(arg) || isBroadNodeFullPattern(arg));
    }
    return args.some((arg) => matchesSelfProcessPattern(arg));
}
function pgrepTargetsSelf(tokens) {
    return pkillTargetsSelf(['pkill', ...tokens.slice(1)]);
}
function pgrepSubstitutionArgs(segment) {
    let quote = '';
    let escaped = false;
    const args = [];
...
function killCommandTargetsSelf(segment) {
...
function xargsInvokesKill(segment) {
    if (!segment) {
        return false;
    }
    ...
    const commandTokens = unwrapExecutionPrefixes(commandArguments(tokens, XARGS_OPTIONS_WITH_VALUES));
    const command = commandTokens.find((token) => !isOptionToken(token));
    return normalizeExecutableName(command ?? '') === 'kill';
}
export function detectSelfKillCommand(command) {
 ...
 * at the end at all.
 */
export function stripTrailingBackgroundAmp(command) {
...
export function hasNonFinalTopLevelBackgroundOperator(command) {
    let quote = '';
    let escaped = false;
    let inBackticks = false;
    let commandSubstitutionDepth = 0;
    const previousNonWhitespace = (index) => {
...
function parseMonitorShellWrapper(command) {
    const trimmed = command.trim();
    let rest = trimmed;
    ...
    while (true) {
        ...
        if (isMonitorCommandMarker(wrapperToken.token, token.token)) {
            ...
            const { value: innerCommand, quote: innerQuote } = stripSymmetricQuotes(commandToken.token);
            return {
                wrapperTokens,
                innerCommand,
                innerQuote,
...
export function normalizeMonitorCommand(command) {
    const { wrapperTokens, innerCommand, innerQuote, innerArgsSuffix } = parseMonitorShellWrapper(command);
    const leadingEnvTokens = wrapperTokens?.filter((token) => isEnvAssignmentToken(token)) ?? [];
    const analysisCommand = stripTrailingBackgroundAmp(innerCommand);
    const rawInnerArgsSuffix = innerArgsSuffix?.trim() ?? '';
    const normalizedInnerArgsSuffix = stripTrailingBackgroundAmp(rawInnerArgsSuffix);
    ...
    const safetyCommand = wrapperTokens && safetyParts.length > 0
...
export function hasUnsafeMonitorBackgroundOperator(command) {
    const { innerCommand, innerArgsSuffix } = parseMonitorShellWrapper(command);
    return (hasNonFinalTopLevelBackgroundOperator(innerCommand) ||
 ...
 */
export const COMMAND_SUBSTITUTION_WARNING = 'Contains command substitution ($(...), backticks, <(...), >(...), or ${parameter@P}).';
/**
 ...
 */
export function hasShellSubstitution(rawCommand) {
    if (typeof rawCommand !== 'string' || rawCommand.length === 0)
        return false;
    if (detectCommandSubstitution(rawCommand))
        return true;
 ...
 */
export function buildShellExecWarnings(strippedCommand, rawCommand) {
    ...
    if (hasShellSubstitution(rawCommand) ||
 ...
 */
export async function checkCommandPermissions(command, config, sessionAllowlist) {
    // Disallow command substitution for security.
    if (detectCommandSubstitution(command)) {
        return {
            allAllowed: false,
            disallowedCommands: [command],
            blockReason: 'Command substitution using $(), `` ` ``, <(), >(), or ${parameter@P} is not allowed for security reasons',
            isHardDenial: true,
        };
    }
    ...
    if (pm) {
        ...
        for (const cmd of commandsToValidate) {
            ...
            if (sessionAllowlist) {
                ...
                const isSessionAllowed = doesToolInvocationMatch('run_shell_command', invocation, [...sessionAllowlist].flatMap((c) => SHELL_TOOL_NAMES.map((name) => `${name}(${c})`)));
                if (isSessionAllowed)
                    continue;
            }
...
export async function isCommandAllowed(command, config) {
...
export function isCommandNeedsPermission(command) {
 ...
 */
export function checkArgumentSafety(args) {
...
export function shouldDefaultToNodePty() {
