...
import { LONG_TASK_NOTIFICATION_THRESHOLD_SECONDS, useAttentionNotifications, } from './useAttentionNotifications.js';
vi.mock('../../services/notificationService.js', () => ({
    sendNotification: vi.fn(),
}));
const { sendNotification: mockedSendNotification } = await import('../../services/notificationService.js');
const mockTerminal = {
    notifyITerm2: vi.fn(),
    notifyKitty: vi.fn(),
    notifyGhostty: vi.fn(),
...
const mockSettingsTaskCompleteOnly = {
    merged: {
        general: {
            terminalBell: true,
...
describe('useAttentionNotifications', () => {
    ...
    const render = (props) => renderHook(({ hookProps }) => useAttentionNotifications(hookProps), {
        initialProps: {
            hookProps: {
                isFocused: true,
    ...
    it('notifies when tool approval is required while unfocused', () => {
        ...
        expect(mockedSendNotification).toHaveBeenCalledWith(expect.objectContaining({ title: 'LailatulCoder Ai' }), mockTerminal, true);
    });
    ...
    it('includes tool name in approval notification message', () => {
        ...
        expect(mockedSendNotification).toHaveBeenCalledTimes(1);
        expect(mockedSendNotification).toHaveBeenCalledWith(expect.objectContaining({
...
//# sourceMappingURL=useAttentionNotifications.test.js.map