 ...
 * the pinned asset once into `~/.lailatulcoder/computer-use/`, preferring a
 ...
 */
export const OSS_MIRROR_BASE = 'https://lailatul-coder-assets.oss-cn-hangzhou.aliyuncs.com/computer-use';
/** GitHub release download base for the pinned tag (fallback source). */
export const GITHUB_RELEASE_BASE = 'https://github.com/trycua/cua/releases/download';
/**
 ...
 */
export function resolveAssetTarget(platform = process.platform, arch = process.arch, version = CUA_DRIVER_VERSION) {
    const v = version;
    if (platform === 'darwin') {
    ...
    if (platform === 'linux') {
        if (arch !== 'x64') {
            throw new Error(`Computer Use: unsupported Linux arch '${arch}' (only x64).`);
        }
 ...
 */
export function resolveAssetUrls(asset, env = process.env, version = CUA_DRIVER_VERSION) {
    const urls = [];
...
export function resolveChecksumUrls(env = process.env, version = CUA_DRIVER_VERSION) {
 ...
 */
export function resolveMaxImageDimension(settingValue, env = process.env) {
...
export function computerUseRoot(home = homedir()) {
    return join(home, '.lailatulcoder', 'computer-use');
}
/** Directory a given version's assets extract into. */
export function versionDir(home = homedir(), version = CUA_DRIVER_VERSION) {
 ...
 */
export function binaryPath(home = homedir(), platform = process.platform, arch = process.arch, version = CUA_DRIVER_VERSION) {
    const target = resolveAssetTarget(platform, arch, version);
    return join(versionDir(home, version), target.extractDir, target.binaryRelPath);
}
/**
 ...
 */
export function approvalKey(version = CUA_DRIVER_VERSION) {
