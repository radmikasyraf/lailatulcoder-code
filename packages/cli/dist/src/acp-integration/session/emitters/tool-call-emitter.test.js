...
describe('ToolCallEmitter', () => {
    ...
    describe('tool preparation lifecycle', () => {
        ...
        it('emits a protocol-valid discarded preparation terminal update', async () => {
            await emitter.emitPreparationDiscarded('call-1', 'mcp__filesystem__read_file');
            expect(sendUpdateSpy).toHaveBeenCalledWith({
                sessionUpdate: 'tool_call_update',
                toolCallId: 'call-1',
                status: 'failed',
        ...
        it.each(['result', 'error'])('clears prepared state after terminal %s', async (terminal) => {
            const preparation = {
                callId: 'call-1',
                toolName: 'read_file',
                args: {},
    ...
    describe('emitResult', () => {
        ...
        it('keeps the vision bridge disclosure in ACP content on failure', async () => {
            const resultDisplay = {
                ...
                notice: 'Vision bridge (qwen3-vl-plus) failed after sending images to dashscope.aliyuncs.com.',
            };
            await emitter.emitResult({
                toolName: 'read_file',
        ...
        it('emits structured artifacts without a wire trust marker', async () => {
            ...
            expect((sendUpdateSpy.mock.calls[0]?.[0])._meta).not.toHaveProperty('artifactsTrustedPublisher');
        });
        ...
        describe('TodoWriteTool handling', () => {
            ...
            it('should not publish rejected TodoWrite arguments as a plan', async () => {
                await emitter.emitResult({
                    ...
                    success: true,
                    message: [],
                    resultDisplay: 'Error writing todos: dependency graph contains a cycle',
                    args: {
                        todos: [
                            {
                                id: 'a',
    ...
    describe('mapToolKind', () => {
        ...
        it('should map enter_plan_mode tool to switch_mode kind', () => {
            expect(emitter.mapToolKind(Kind.Think, 'enter_plan_mode')).toBe('switch_mode');
        });
    ...
    describe('resolveToolMetadata', () => {
        it('uses persisted metadata without a Config in direct replay', () => {
            ...
            expect(configless.resolveToolMetadata('read_file', {
    ...
    describe('resolveToolProvenance (#4175 F4 prereq, chiga0 #19 P0)', () => {
        ...
        it('classifies mcp__<server>__<tool> as mcp with serverId', () => {
            const out = ToolCallEmitter.resolveToolProvenance('mcp__filesystem__read');
        ...
        it('preserves underscores in the tool segment', () => {
            ...
            const out = ToolCallEmitter.resolveToolProvenance('mcp__playwright__take_screenshot');
    ...
    describe('provenance stamping on emit (#4175 F4 prereq)', () => {
        ...
        it('stamps provenance:subagent (no serverId) when subagentMeta present', async () => {
            await emitter.emitStart({
                ...
                callId: 'call-sub',
                args: {},
                subagentMeta: { agentType: 'researcher' },
            });
            const call = sendUpdateSpy.mock.calls[0][0];
...
//# sourceMappingURL=tool-call-emitter.test.js.map