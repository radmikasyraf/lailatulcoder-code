 ...
 * of the LailatulCoder Ai application.
 */
export class BuiltinCommandLoader {
    ...
    async loadCommands(_signal) {
        ...
        let resolvedIdeCommand = null;
        try {
            resolvedIdeCommand = await ideCommand();
        }
        catch (error) {
            builtinDebugLogger.warn('Failed to load IDE command:', error instanceof Error ? error.message : String(error));
        }
