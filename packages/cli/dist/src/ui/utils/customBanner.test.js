...
function makeSettings(opts) {
    ...
    return {
        ...
        workspace: opts.workspaceUi
            ? file({ ui: opts.workspaceUi }, opts.workspacePath ?? '/repo/.qwen/settings.json')
            : empty,
...
describe('resolveCustomBanner', () => {
    ...
    it('caps art at 200 lines × 200 cols', () => {
        ...
        const out1 = resolveCustomBanner(makeSettings({ userUi: { customAsciiArt: tooManyLines } }));
        expect(out1.asciiArt.small?.split('\n').length).toBe(200);
        const tooWide = 'a'.repeat(300);
        const out2 = resolveCustomBanner(makeSettings({ userUi: { customAsciiArt: tooWide } }));
    ...
    it('falls back when the {path} target is missing', () => {
        const out = resolveCustomBanner(makeSettings({
            userUi: {
                customAsciiArt: { path: 'missing.txt' },
            },
            userPath: path.join(tmpDir, 'settings.json'),
        }));
        expect(out.asciiArt.small).toBeUndefined();
    ...
    it('caps art width by terminal cells, not UTF-16 length (CJK fullwidth)', () => {
        ...
        const out = resolveCustomBanner(makeSettings({ userUi: { customAsciiArt: fullwidth } }));
    ...
    it('treats whitespace-only inline art as empty', () => {
        const out = resolveCustomBanner(makeSettings({ userUi: { customAsciiArt: '   \n   ' } }));
        expect(out.asciiArt.small).toBeUndefined();
    });
    ...
    it('combines tiers across scopes via deep-merge (workspace.large + user.small)', () => {
        const out = resolveCustomBanner(makeSettings({
            userUi: {
                customAsciiArt: { small: 'user-small' },
            },
            workspaceUi: {
                customAsciiArt: { large: 'workspace-large' },
            },
        }));
    ...
    it('treats empty title as undefined (falls back to default)', () => {
        const out = resolveCustomBanner(makeSettings({ userUi: { customBannerTitle: '   ' } }));
        expect(out.title).toBeUndefined();
    });
    ...
    it('treats empty subtitle as undefined (header keeps the blank spacer row)', () => {
        const out = resolveCustomBanner(makeSettings({ userUi: { customBannerSubtitle: '   ' } }));
        expect(out.subtitle).toBeUndefined();
    });
...
describe('pickAsciiArtTier', () => {
    ...
    it('prefers large when it fits', () => {
        expect(pickAsciiArtTier('small', 'BIGGER', 100, 2, 40, measure)).toBe('BIGGER');
    });
    it('falls back to small when large is too wide', () => {
        expect(pickAsciiArtTier('sml', 'a'.repeat(200), 60, 2, 40, measure)).toBe('sml');
    });
    it('returns undefined when neither tier fits', () => {
        expect(pickAsciiArtTier('a'.repeat(80), 'a'.repeat(120), 50, 2, 40, measure)).toBeUndefined();
    });
    it('skips missing tiers', () => {
        expect(pickAsciiArtTier(undefined, 'fits', 100, 2, 40, measure)).toBe('fits');
        expect(pickAsciiArtTier('fits', undefined, 100, 2, 40, measure)).toBe('fits');
        expect(pickAsciiArtTier(undefined, undefined, 100, 2, 40, measure)).toBeUndefined();
    });
});
//# sourceMappingURL=customBanner.test.js.map