...
export const goalCommand = {
    ...
    action: async (context, args) => {
        ...
        if (operation.kind === 'error')
            return errorMessage(operation.message);
        // Starting or re-driving an autonomous Goal ingests workspace context
        // (LAILATUL.md, files) without per-tool confirmation, so it requires a trusted
        ...
        const requiresTrustedFolder = operation.kind === 'set' ||
            operation.kind === 'edit' ||
        ...
        try {
            ...
            if (operation.kind === 'set') {
                ...
                return goalControl(operation, await runtime.dispatch(request), request.action);
            }
            if (!current) {
...
//# sourceMappingURL=goalCommand.js.map