 ...
 */
export function chunkOfKey(key) {
 ...
 */
export function declaresOwnUncoverable(rec, chunk) {
    if (chunk === null)
        return false;
 ...
 */
function argsNameExactPath(args, path) {
 ...
 */
export function openedBrief(rec, planPath, key) {
 ...
 */
export function readBrief(rec, planPath, key) {
    return argsNameExactPath(rec.successfulReadFileArgs, briefPath(planPath, key));
}
/**
 ...
 * an open.
 */
export function readFindingsPointer(rec, pointer) {
    if (pointer === null)
        return true;
