 ...
 */
import { createDebugLogger, IdeClient, initializeTelemetry, } from '@lailatul-coder/lailatul-coder-core';
import { preconnectApi } from '../utils/apiPreconnect.js';
import { AppEvent, appEvents } from '../utils/events.js';
import { recordStartupEvent } from '../utils/startupProfiler.js';
import { CUSTOM_SANDBOX_IMAGE_ENV_VAR, HOST_UPDATE_RELAUNCH_ENV_VAR, SKIP_UPDATE_CHECK_ENV_VAR, requestUpdateOnExit, } from '../utils/processUtils.js';
 ...
 */
function runDeferredTask(name, task) {
 ...
 */
export function startEarlyStartupPrefetches(config) {
    if (earlyStarted.has(config))
        return;
    ...
    try {
        ...
        debugLogger.debug(`Preconnect skipped due to error getting authType: ${error}`);
    }
}
/**
 ...
 */
export function startPostRenderPrefetches(config, settings, options = {}) {
    if (postRenderStarted.has(config))
        return;
    ...
    if (settings.merged.general?.enableAutoUpdate !== false &&
        ...
        runDeferredTask('update_check', async () => {
            const [{ checkForUpdatesDetailed, describeUpdateCheckFailure }, { handleAutoUpdate }, { getInstallationInfo }, { updateEventEmitter }, { t },] = await Promise.all([
            ...
            const updateCheckSkippedMessage = (error) => t('Update check skipped ({{reason}}) — run /update to retry.', {
                ...
                if (result.status === 'update') {
                    ...
                    if (hostUpdateRelaunch === 'true') {
                        updateEventEmitter.emit('update-info', {
                            message: `${result.info.message}\n${t('Run /update to install the update on the host.')}`,
                        });
                        return;
                    }
                    if (hostUpdateRelaunch === 'false') {
                        updateEventEmitter.emit('update-info', {
                            message: `${result.info.message}\n${t('Update LailatulCoder Ai on the host, then restart the sandbox.')}`,
                        });
                        return;
                    }
                    ...
                    if (installationInfo.updateCommand ||
                        ...
                        if (requestUpdateOnExit()) {
                            updateEventEmitter.emit('update-info', {
                                message: `${result.info.message}\n${t('The update will be installed after you exit this session.')}`,
                            });
                        }
                        else {
                            updateEventEmitter.emit('update-info', {
                                message: `${result.info.message}\n${t('Run /update to install the update.')}`,
                            });
                        }
                    }
                    else {
    ...
    if (options.connectIde && config.getIdeMode()) {
        runDeferredTask('ide_connect', async () => {
            ...
            try {
                ...
                await withTimeout(connectIdeForStartup(config), 'ide_connect', DEFERRED_IDE_CONNECT_TIMEOUT_MS, disconnectIdeAfterDeferredTimeout);
                appEvents.emit(AppEvent.StartupIdeConnectionStatusChanged, {
                    state: 'connected',
                });
            }
    ...
    if (config.isInteractive()) {
        runDeferredTask('background_housekeeping', async () => {
            const { startBackgroundHousekeeping } = await import('../utils/housekeeping/scheduler.js');
            startBackgroundHousekeeping(config, settings);
        });
    }
}
//# sourceMappingURL=startup-prefetch.js.map