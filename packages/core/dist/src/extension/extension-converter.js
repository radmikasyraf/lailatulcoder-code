...
import { convertClaudePluginPackage, convertClaudePluginStandalone, } from './claude-converter.js';
import { convertQoderPlugin, QODER_PLUGIN_MANIFEST, } from './qoder-converter.js';
import { AGENT_PLUGIN_MANIFEST, AGENT_PLUGIN_SCHEMA, getAgentPluginSchemaStatus, } from './agent-plugins-v1/manifest.js';
...
export async function convertCompatibleExtension(extensionDir, pluginName, networkPolicy, signal) {
    ...
    const configFilePath = path.join(extensionDir, SUPPORTED_EXTENSION_MANIFESTS[0]);
    if (agentPluginStatus === 'unsupported') {
        throw new Error(`Unsupported Agent Plugins schema. Supported schema: "${AGENT_PLUGIN_SCHEMA}".`);
    }
        ...
        const converted = await convertClaudePluginPackage(extensionDir, pluginName, networkPolicy, signal);
    ...
    else if (fs.existsSync(path.join(extensionDir, SUPPORTED_EXTENSION_MANIFESTS[3]))) {
...
//# sourceMappingURL=extension-converter.js.map