...
const TRANSCRIPT_READ_FLAGS = nodeConstants.O_RDONLY | TRANSCRIPT_NO_FOLLOW_FLAG | TRANSCRIPT_NONBLOCK_FLAG;
const TRANSCRIPT_APPEND_FLAGS = nodeConstants.O_APPEND |
...
export class SessionWriterConflictError extends SessionWriterError {
    name = 'SessionWriterConflictError';
...
export class SessionWriterLostError extends SessionWriterError {
    name = 'SessionWriterLostError';
    rpcCode = SESSION_WRITER_RPC_CODES.session_writer_lost;
...
export class SessionTranscriptChangedError extends SessionWriterError {
    name = 'SessionTranscriptChangedError';
    rpcCode = SESSION_WRITER_RPC_CODES.session_transcript_changed;
...
export class SessionWriterUnavailableError extends SessionWriterError {
    ...
    rpcCode = SESSION_WRITER_RPC_CODES.session_writer_unavailable;
    errorKind = 'session_writer_unavailable';
    httpStatus = 503;
    constructor(options) {
        super(options?.message ?? 'Session write ownership could not be verified.', options);
    }
}
export class SessionTranscriptIdentityUnavailableError extends SessionWriterUnavailableError {
    name = 'SessionTranscriptIdentityUnavailableError';
    constructor(cause) {
        super({
            message: 'Session transcript identity could not be verified on this filesystem.',
            cause: cause ??
                new Error('The session transcript filesystem does not provide a verifiable inode identity (ino=0).'),
        });
    }
}
...
async function execFileText(file, args, env) {
...
async function readProcessStartIdentity(pid) {
    ...
    if (process.platform === 'darwin') {
        const startedAt = await execFileText('/bin/ps', ['-o', 'lstart=', '-p', String(pid)], { ...process.env, LC_ALL: 'C', LANG: 'C', TZ: 'UTC' });
...
function hasValidOwnerFields(record) {
...
function isSealedTranscriptProof(value) {
    if (!value || typeof value !== 'object' || Array.isArray(value))
        return false;
    const proof = value;
    return (typeof proof['relative_path'] === 'string' &&
...
function isActiveLockRecord(record) {
    return (record.schema_version === LEGACY_LOCK_SCHEMA_VERSION ||
        record.state === 'active');
}
async function lockStateForRecord(record, raw) {
    if (record.hostname !== os.hostname())
        return { kind: 'live', record, raw };
    if (!isProcessAlive(record.pid))
        return { kind: 'stale', record, raw };
    if (!record.process_start_identity)
        return { kind: 'live', record, raw };
...
function transcriptFingerprint(stat) {
    return {
        dev: stat.dev,
        ino: stat.ino,
        mode: stat.mode,
...
function assertVerifiableTranscriptIdentity(fingerprint) {
 ...
 */
async function assertTranscriptFilesystemProvidesIdentity(transcriptPath) {
...
function sameFileSecurityMetadata(left, right) {
    return (left.mode === right.mode &&
        left.uid === right.uid &&
        left.gid === right.gid &&
        left.nlink === right.nlink);
}
function sameHardTranscriptState(left, right) {
...
function sameTranscriptState(left, right) {
    if (left.exists !== right.exists)
        return false;
    if (!left.exists || !right.exists)
        return true;
    return (sameHardTranscriptState(left, right) &&
...
function transcriptStateFromStat(stat) {
...
function transcriptStateChangedFields(left, right) {
    if (left.exists !== right.exists)
        return ['exists'];
    if (!left.exists || !right.exists)
        return [];
...
function transcriptHashesEqual(left, right) {
    return left.copy().digest().equals(right.copy().digest());
}
async function assertTranscriptPathMissing(filePath) {
    try {
...
async function getOpenTranscriptState(filePath, handle, invalidPathIsChange) {
    let handleStat;
    try {
        handleStat = await handle.stat();
    }
    catch (error) {
        if (invalidPathIsChange &&
            error.code === 'ENOENT') {
    ...
    if (handleStat.size > 0) {
        ...
        const { bytesRead } = await handle.read(lastByte, 0, 1, handleStat.size - 1);
...
async function inspectTranscriptPath(filePath, invalidPathIsChange) {
    let stat;
    try {
...
async function openTranscriptForRead(filePath, expectedState) {
    const pathState = await inspectTranscriptPath(filePath, expectedState !== undefined);
...
async function openTranscriptForAppend(filePath, expectedState) {
...
async function getTranscriptState(filePath, expectedState) {
    let handle;
    try {
        handle = await openTranscriptForRead(filePath, expectedState);
        if (!handle)
            return { exists: false, byteLength: 0 };
        return await getOpenTranscriptState(filePath, handle, expectedState !== undefined);
    }
    catch (error) {
        if (error instanceof SessionWriterError)
            throw error;
...
async function captureOpenTranscriptSnapshot(filePath, handle, expectedState, shouldAbort) {
    let buffer;
    for (let attempt = 1; attempt <= TRANSCRIPT_SNAPSHOT_ATTEMPTS; attempt++) {
        if (shouldAbort())
            throw new SessionWriterLostError();
        const beforeState = await getOpenTranscriptState(filePath, handle, expectedState !== undefined);
        if (expectedState && !sameHardTranscriptState(beforeState, expectedState)) {
            throw new SessionTranscriptChangedError();
        }
        const bufferBytes = Math.min(TRANSCRIPT_HASH_BUFFER_BYTES, beforeState.byteLength);
        ...
        while (position < beforeState.byteLength) {
            if (shouldAbort())
                throw new SessionWriterLostError();
            const length = Math.min(buffer.byteLength, beforeState.byteLength - position);
            if (length === 0)
                throw new SessionWriterUnavailableError();
            ...
            while (chunkBytesRead < length) {
                if (shouldAbort())
                    throw new SessionWriterLostError();
                const { bytesRead } = await handle.read(buffer, chunkBytesRead, length - chunkBytesRead, position + chunkBytesRead);
                if (bytesRead === 0)
                    throw new SessionTranscriptChangedError();
        ...
        if (shouldAbort())
            throw new SessionWriterLostError();
        const afterState = await getOpenTranscriptState(filePath, handle, expectedState !== undefined);
        ...
        debugLogger.debug(`Session transcript snapshot retry attempt=${attempt} ` +
...
async function captureTranscriptSnapshot(filePath, expectedState, shouldAbort) {
    let handle;
    try {
        handle = await openTranscriptForRead(filePath, expectedState);
        if (!handle) {
            const missingState = { exists: false, byteLength: 0 };
            if (expectedState &&
        ...
        return await captureOpenTranscriptSnapshot(filePath, handle, expectedState, shouldAbort);
    }
    catch (error) {
        if (error instanceof SessionWriterError)
            throw error;
...
function getTranscriptRelativePath(runtimeBaseDir, transcriptPath) {
...
async function openTranscriptProof(filePath) {
    let handle;
    try {
        try {
        ...
        if (beforeStat.size > 0) {
            const lastByte = Buffer.allocUnsafe(1);
            const { bytesRead } = await handle.read(lastByte, 0, 1, beforeStat.size - 1);
            if (bytesRead !== 1 || lastByte[0] !== 0x0a) {
                throw new SessionTranscriptChangedError();
            }
        }
        ...
        while (position < beforeStat.size) {
            const { bytesRead } = await handle.read(buffer, 0, Math.min(buffer.byteLength, beforeStat.size - position), position);
            if (bytesRead === 0)
                throw new SessionTranscriptChangedError();
...
async function validateOpenTranscriptProof(filePath, proof) {
...
async function closeTranscriptProof(proof) {
    await proof.handle?.close().catch(() => { });
}
function assertSealedProofMatches(sealed, relativePath, proof) {
    const transcript = sealed.transcript;
    if (transcript.relative_path !== relativePath ||
...
async function restoreMovedLock(movedPath, lockPath) {
    try {
...
async function installLockRecord(lockPath, record) {
...
async function acquireReclaimGuard(lockPath, staleOwnerId, record, inspect) {
    ...
    for (let attempt = 0; attempt < ACQUIRE_ATTEMPTS; attempt++) {
        if (await installLockRecord(guardPath, record))
            return guardPath;
...
async function removeExactRecord(candidatePath, expectedRaw) {
    let raw;
    try {
...
async function inspectExactRecord(candidatePath, expectedRaw) {
    try {
...
async function removeTransitionClaimAfterFailure(lockPath, expectedPrimaryRaw, claimPath, claimRaw) {
    const { state: primaryState } = await inspectExactRecord(lockPath, expectedPrimaryRaw);
    if (primaryState !== 'exact') {
        throw new SessionWriterUnavailableError({
            cause: new Error('Session writer primary was not restored; transition claim retained'),
        });
    }
    await removeExactRecord(claimPath, claimRaw);
}
async function releaseTransitionClaim(claimPath, claimRaw) {
    let raw;
    try {
...
async function inspectClaimedPrimary(lockPath, sourceRaw, sessionId) {
    try {
        ...
        return {
            state: record?.schema_version === LOCK_SCHEMA_VERSION &&
                record.state === 'active' &&
...
async function assertExactTransitionClaim(claimPath, claimRaw) {
...
async function waitForClaimedPrimaryCandidate(attempt) {
    if (attempt >= CLAIMED_PRIMARY_WAIT_ATTEMPTS) {
        throw new SessionWriterUnavailableError({
            cause: new Error('Session writer primary candidate did not release the claimed path'),
        });
    }
    await delay(MALFORMED_RETRY_DELAY_MS);
}
async function linkClaimedPrimary(lockPath, sourcePath, sourceRaw, sessionId, claimPath, claimRaw) {
...
async function removeClaimedPrimary(lockPath, sourceRaw, sessionId, claimPath, claimRaw) {
    let candidateWaitAttempts = 0;
    for (;;) {
        await assertExactTransitionClaim(claimPath, claimRaw);
        const { state, cause } = await inspectClaimedPrimary(lockPath, sourceRaw, sessionId);
        if (state === 'missing')
            return;
        if (state === 'candidate') {
            await waitForClaimedPrimaryCandidate(++candidateWaitAttempts);
        ...
        try {
            ...
            const { state: afterState } = await inspectClaimedPrimary(lockPath, sourceRaw, sessionId);
            if (afterState === 'missing')
                return;
...
async function transitionExactPrimary(lockPath, expectedRaw, replacementPath, replacementRaw, retiredPath, sessionId, claimPath, claimRaw) {
    try {
    ...
    try {
        await linkClaimedPrimary(lockPath, replacementPath, replacementRaw, sessionId, claimPath, claimRaw);
    }
    catch (error) {
        try {
            await linkClaimedPrimary(lockPath, retiredPath, expectedRaw, sessionId, claimPath, claimRaw);
            await removeExactRecord(retiredPath, expectedRaw);
        }
        catch (restoreError) {
            throw new SessionWriterUnavailableError({
                cause: new AggregateError([error, restoreError], 'Session writer primary transition could not be restored'),
            });
        }
        throw new SessionWriterUnavailableError({
            cause: error instanceof Error ? error : undefined,
...
async function rollbackExactTransition(lockPath, replacementRaw, retiredPath, expectedRaw, sessionId, claimPath, claimRaw) {
    await removeClaimedPrimary(lockPath, replacementRaw, sessionId, claimPath, claimRaw);
...
export function getSessionWriterLockPath(runtimeBaseDir, sessionId) {
    return path.join(runtimeBaseDir, 'tmp', 'session-writer-locks', `${encodeURIComponent(sessionId)}.lock`);
}
export class SessionWriterLease {
    ...
    retiredPath;
    claimPath;
    constructor(lockPath, lockRecord, options) {
    ...
    get transcriptExistedAtAcquire() {
    ...
    static async acquire(options) {
        try {
            ...
            const lockPath = getSessionWriterLockPath(path.resolve(options.runtimeBaseDir), options.sessionId);
            const errorKind = error instanceof SessionWriterError ? error.errorKind : 'unknown';
            debugLogger.debug(`Session writer lease acquisition failed stage=acquire errorKind=${errorKind} ` +
    ...
    static async acquireInternal(options) {
        const normalizedOptions = {
            ...options,
            runtimeBaseDir: path.resolve(options.runtimeBaseDir),
            transcriptPath: path.resolve(options.transcriptPath),
        };
        const lockPath = getSessionWriterLockPath(normalizedOptions.runtimeBaseDir, normalizedOptions.sessionId);
        ...
        try {
            ...
            if (!lockDirStat.isDirectory() || lockDirStat.isSymbolicLink()) {
                throw new SessionWriterUnavailableError({
                    cause: new Error('Session writer lock directory is not a regular directory'),
                });
            }
        }
        catch (error) {
        ...
        const lockRecord = {
            ...
            qwen_version: normalizedOptions.lailatulcoderVersion ?? null,
        };
        ...
        for (let attempt = 0; attempt < ACQUIRE_ATTEMPTS; attempt++) {
            ...
            if (await installLockRecord(lockPath, lockRecord)) {
                ...
                return SessionWriterLease.finishAcquisition(lockPath, lockRecord, normalizedOptions);
            }
            const state = await SessionWriterLease.inspectExistingLock(lockPath, normalizedOptions.sessionId);
            if (state.kind === 'missing')
                continue;
            if (state.kind === 'live')
            ...
            if (state.kind === 'sealed') {
                ...
                return SessionWriterLease.takeOverSealed(lockPath, state, lockRecord, normalizedOptions);
            }
            if (normalizedOptions.reclaimPolicy === 'never') {
                throw new SessionWriterConflictError();
            }
            const staleOwnerId = state.record.owner_id;
            const reclaimPath = await acquireReclaimGuard(lockPath, staleOwnerId, lockRecord, (candidatePath, sessionId) => SessionWriterLease.inspectExistingLock(candidatePath, sessionId));
            let primaryInstalled = false;
            let staleMoved = false;
            const stalePath = `${lockPath}.stale.${process.pid}.${randomUUID()}`;
            try {
                await assertPathMissing(claimPath);
                const currentState = await SessionWriterLease.inspectExistingLock(lockPath, normalizedOptions.sessionId);
                ...
                staleMoved = true;
                const movedState = await SessionWriterLease.inspectExistingLock(stalePath, normalizedOptions.sessionId);
                if (movedState.kind !== 'stale' ||
                ...
                await assertPathMissing(claimPath);
                const finishingLease = SessionWriterLease.finishAcquisition(lockPath, lockRecord, normalizedOptions);
    ...
    static async takeOverSealed(lockPath, observed, lockRecord, options) {
        const relativePath = getTranscriptRelativePath(options.runtimeBaseDir, options.transcriptPath);
        const proof = await openTranscriptProof(options.transcriptPath);
        const claimPath = `${lockPath}.claim`;
        const claimRaw = JSON.stringify(lockRecord);
        const retiredPath = `${lockPath}.sealed.${encodeURIComponent(observed.record.owner_id)}.${encodeURIComponent(lockRecord.owner_id)}`;
        ...
        try {
            ...
            claimAcquired = true;
            const current = await SessionWriterLease.inspectExistingLock(lockPath, options.sessionId);
            ...
            await transitionExactPrimary(lockPath, observed.raw, claimPath, claimRaw, retiredPath, options.sessionId, claimPath, claimRaw);
            transitionCommitted = true;
            await validateOpenTranscriptProof(options.transcriptPath, proof);
            const lease = await SessionWriterLease.finishAcquisition(lockPath, lockRecord, options, proof.state);
            await releaseTransitionClaim(claimPath, claimRaw);
            claimAcquired = false;
            await removeExactRecord(retiredPath, observed.raw).catch((error) => {
                debugLogger.debug(`Session writer sealed lock cleanup failed path=${JSON.stringify(retiredPath)} ` +
                    `error=${describeDiagnosticError(error)}`);
            });
            debugLogger.info(`Certified session writer handoff accepted sessionId=${JSON.stringify(options.sessionId)} ` +
            ...
            if (claimState === 'unknown') {
                cleanupFailures.push(new SessionWriterUnavailableError({
                    cause: new Error('Session writer transition claim ownership is unreadable'),
                }));
            }
            if (transitionCommitted && claimState === 'exact') {
                try {
                    await rollbackExactTransition(lockPath, claimRaw, retiredPath, observed.raw, options.sessionId, claimPath, claimRaw);
                }
                catch (cleanupError) {
            ...
            if (claimState === 'exact') {
                try {
                    if (transitionStarted) {
                        await removeTransitionClaimAfterFailure(lockPath, observed.raw, claimPath, claimRaw);
                    }
                    else {
                        await releaseTransitionClaim(claimPath, claimRaw);
                    }
            ...
            if (cleanupFailures.length > 0) {
                throw new SessionWriterUnavailableError({
                    cause: new AggregateError([error, ...cleanupFailures], 'Certified session writer takeover cleanup failed'),
                });
            }
            if (error instanceof SessionWriterError)
                throw error;
    ...
    static async finishAcquisition(lockPath, lockRecord, options, requiredTranscriptState) {
        const lease = new SessionWriterLease(lockPath, lockRecord, options);
        try {
            options.onOwnershipAcquired?.(lease);
            const snapshot = await captureTranscriptSnapshot(options.transcriptPath, undefined, () => lease.released);
            if (!snapshot.state.exists) {
                await assertTranscriptFilesystemProvidesIdentity(options.transcriptPath);
            }
            await lease.readOwnedLock();
            lease.expectedTranscriptState = snapshot.state;
            lease.expectedTranscriptHasher = snapshot.hasher;
            if (requiredTranscriptState &&
                !sameTranscriptState(lease.expectedTranscriptState, requiredTranscriptState)) {
                throw new SessionTranscriptChangedError();
            }
            return lease;
        }
            ...
            try {
                ...
                throw new SessionWriterUnavailableError({
                    cause: new AggregateError([error, releaseError], 'Session writer acquisition cleanup failed'),
                });
            }
            throw error;
        }
    }
    static async inspectExistingLock(lockPath, expectedSessionId) {
    ...
    async assertOwnedAndUnchangedOnce() {
        ...
        if (!sameHardTranscriptState(transcriptState, expectedState)) {
            debugLogger.debug(`Session transcript hard state changed changedFields=${transcriptStateChangedFields(expectedState, transcriptState).join(',')}`);
            throw new SessionTranscriptChangedError();
        }
        await this.reconcileTranscriptMetadata(transcriptState);
    }
    appendJsonLine(value) {
        return this.runExclusive(() => this.appendJsonLineOnce(value));
    }
    async appendJsonLineOnce(value) {
        let serialized;
        try {
        ...
        try {
            ...
            handle = await openTranscriptForAppend(this.transcriptPath, expectedBefore);
            let beforeState = await getOpenTranscriptState(this.transcriptPath, handle, true);
            ...
            const afterState = transcriptStateFromStat(afterStat);
            if (afterState.byteLength !== nextByteLength ||
                !sameFileIdentity(afterState.fingerprint, beforeState.fingerprint) ||
                !sameFileSecurityMetadata(afterState.fingerprint, beforeState.fingerprint)) {
                throw new SessionTranscriptChangedError();
            }
            await handle.close();
            handle = undefined;
            const transcriptState = await getTranscriptState(this.transcriptPath, afterState);
            if (!transcriptState.exists ||
                transcriptState.byteLength !== nextByteLength ||
                !sameFileIdentity(transcriptState.fingerprint, afterState.fingerprint) ||
                !sameFileSecurityMetadata(transcriptState.fingerprint, afterState.fingerprint)) {
                throw new SessionTranscriptChangedError();
            }
            let committedState = transcriptState;
            let committedHasher = candidateHasher;
            let appendReconciliation;
            if (!sameTranscriptState(transcriptState, afterState)) {
                const changedFields = transcriptStateChangedFields(afterState, transcriptState);
                const startedAt = Date.now();
                await this.readOwnedLock();
                const snapshot = await captureTranscriptSnapshot(this.transcriptPath, afterState, () => this.released);
                if (!transcriptHashesEqual(snapshot.hasher, candidateHasher)) {
                    debugLogger.debug(`Session transcript content changed after append metadata signal ` +
            ...
            if (appendReconciliation) {
                debugLogger.debug(`Session transcript append metadata reconciled path=slow ` +
    ...
    async reconcileTranscriptMetadata(observedState, handle) {
        const expectedState = this.expectedTranscriptState;
        const expectedHasher = this.expectedTranscriptHasher;
        if (!expectedState ||
            !expectedHasher ||
        ...
        const changedFields = transcriptStateChangedFields(expectedState, observedState);
        const startedAt = Date.now();
        await this.readOwnedLock();
        const snapshot = handle
            ? await captureOpenTranscriptSnapshot(this.transcriptPath, handle, expectedState, () => this.released)
            : await captureTranscriptSnapshot(this.transcriptPath, expectedState, () => this.released);
        if (!transcriptHashesEqual(snapshot.hasher, expectedHasher)) {
            debugLogger.debug(`Session transcript content changed after metadata signal ` +
                `path=slow changedFields=${changedFields.join(',')} ` +
                `attempts=${snapshot.attempts} durationMs=${Date.now() - startedAt}`);
            throw new SessionTranscriptChangedError();
        }
        ...
        debugLogger.debug(`Session transcript metadata reconciled path=slow ` +
    ...
    async releaseOnce() {
        ...
        try {
            ...
            await fs.unlink(this.retiredPath).catch((error) => {
                debugLogger.debug(`Session writer retired lock cleanup failed path=${JSON.stringify(this.retiredPath)} ` +
                    `error=${describeDiagnosticError(error)}`);
            });
        }
        catch (error) {
    ...
    async sealForHandoffOnce() {
        ...
        const relativePath = getTranscriptRelativePath(this.runtimeBaseDir, this.transcriptPath);
        ...
        const sealedCandidatePath = `${this.lockPath}.sealed-candidate.${encodeURIComponent(this.ownerId)}`;
        const handoffRetiredPath = `${this.lockPath}.handoff.${encodeURIComponent(this.ownerId)}`;
        let claimAcquired = false;
        let transitionStarted = false;
        let transitionCommitted = false;
        try {
            ...
            transitionStarted = true;
            await transitionExactPrimary(this.lockPath, this.lockRecordRaw, sealedCandidatePath, sealedRaw, handoffRetiredPath, this.sessionId, this.claimPath, this.lockRecordRaw);
            transitionCommitted = true;
            await validateOpenTranscriptProof(this.transcriptPath, proof);
            await releaseTransitionClaim(this.claimPath, this.lockRecordRaw);
            claimAcquired = false;
            this.released = true;
            await removeExactRecord(handoffRetiredPath, this.lockRecordRaw).catch((error) => {
                debugLogger.debug(`Session writer handoff retired cleanup failed path=${JSON.stringify(handoffRetiredPath)} ` +
                    `error=${describeDiagnosticError(error)}`);
            });
            await removeExactRecord(sealedCandidatePath, sealedRaw).catch((error) => {
                debugLogger.debug(`Session writer handoff candidate cleanup failed path=${JSON.stringify(sealedCandidatePath)} ` +
                    `error=${describeDiagnosticError(error)}`);
            });
            debugLogger.info(`Session writer handoff sealed sessionId=${JSON.stringify(this.sessionId)} ` +
            ...
            if (transitionCommitted && claimState === 'exact') {
                try {
                    await rollbackExactTransition(this.lockPath, sealedRaw, handoffRetiredPath, this.lockRecordRaw, this.sessionId, this.claimPath, this.lockRecordRaw);
                }
                catch (cleanupError) {
                    cleanupFailures.push(cleanupError);
                }
            ...
            if (claimState === 'exact') {
                try {
                    if (transitionStarted) {
                        await removeTransitionClaimAfterFailure(this.lockPath, this.lockRecordRaw, this.claimPath, this.lockRecordRaw);
                    }
                    else {
                        await releaseTransitionClaim(this.claimPath, this.lockRecordRaw);
                    }
            ...
            if ((await inspectExactRecord(this.lockPath, this.lockRecordRaw)).state !==
                'exact') {
                this.released = true;
            }
            if (cleanupFailures.length > 0) {
                throw new SessionWriterUnavailableError({
                    cause: new AggregateError([error, ...cleanupFailures], 'Session writer handoff cleanup failed'),
                });
            }
            if (error instanceof SessionWriterError)
                throw error;
    ...
    async inspectReleasePath(candidatePath) {
        try {
            const stat = await fs.lstat(candidatePath);
            if (!stat.isFile() || stat.isSymbolicLink())
                return 'other';
...
//# sourceMappingURL=session-writer-lease.js.map