 ...
 */
async function fetchUrl(url, headers, networkPolicy) {
    ...
    const hardDeadline = setTimeout(() => {
        deadlineController.abort(new Error(`Marketplace request timed out after ${MARKETPLACE_FETCH_TIMEOUT_MS}ms`));
        req?.destroy();
        finish?.(null);
    ...
    let target;
    try {
        target = await resolveNetworkTarget(url, networkPolicy, deadlineController.signal);
        deadlineController.signal.throwIfAborted();
    }
    catch (error) {
        clearTimeout(hardDeadline);
        debugLogger.debug(`Failed to resolve marketplace network target: ${redactUrlCredentials(getErrorMessage(error))}`);
        return null;
    }
    return new Promise((resolve) => {
        ...
        try {
            req = client.get(url, {
                ...
                ...(target.lookup
                    ? { lookup: target.lookup, agent: false }
                    : {}),
            }, onResponse);
        }
        catch {
 ...
 */
async function fetchGitHubMarketplaceConfig(owner, repo, networkPolicy) {
 ...
 */
export async function loadMarketplaceConfigFromSource(source, networkPolicy) {
    ...
    if (lowerTrimmed.startsWith('http://') ||
        ...
        try {
            ...
            const ghConfig = await fetchGitHubMarketplaceConfig(owner, repo, networkPolicy);
            if (ghConfig) {
                return ghConfig;
            }
        }
        ...
        const content = await fetchUrl(trimmed, { 'User-Agent': 'lailatul-coder' }, networkPolicy);
        if (!content) {
            return null;
        }
        try {
    ...
    if (lowerTrimmed.startsWith('git@') || lowerTrimmed.startsWith('sso://')) {
        ...
        if (sshMatch) {
            return fetchGitHubMarketplaceConfig(sshMatch[1], sshMatch[2], networkPolicy);
        }
        try {
            const { owner, repo } = parseGitHubRepoForReleases(trimmed);
...
export async function parseInstallSource(source, options = {}) {
    ...
    if (isLocalPath) {
        ...
        try {
            ...
            marketplaceConfig = await fetchGitHubMarketplaceConfig(owner, repoName, options.networkPolicy);
        }
        catch {
