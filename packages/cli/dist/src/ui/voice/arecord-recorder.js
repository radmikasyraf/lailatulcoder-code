...
function toArecordError(error) {
    if (error.code === 'ENOENT') {
        return new Error('arecord is not installed or not on PATH. Install alsa-utils and try again.');
    }
    return error;
}
class ArecordRecorder {
    ...
    async start(_options = {}) {
        this.tmpDir = await mkdtemp(path.join(tmpdir(), 'LailatulCoder-voice-'));
        ...
        try {
            await new Promise((resolve, reject) => {
                ...
                child.once('close', (code) => {
                    ...
                    reject(new Error(`arecord could not open an audio device (exit code ${code ?? 'unknown'})${detail ? `: ${detail}` : ''}.`));
                });
            });
        }
        catch (error) {
    ...
    async stop() {
        ...
        try {
            ...
            const closeResult = this.closeResult ?? (closePromise ? await closePromise : null);
            if (closeResult &&
                closeResult.code !== 0 &&
                closeResult.signal !== 'SIGINT') {
                throw new Error(`Voice recorder failed with exit code ${closeResult.code ?? 'unknown'}.`);
            }
...
export function createArecordRecorder() {
    return new ArecordRecorder();
}
//# sourceMappingURL=arecord-recorder.js.map