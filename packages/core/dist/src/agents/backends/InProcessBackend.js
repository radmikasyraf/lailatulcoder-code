...
export class InProcessBackend {
    ...
    agentApprovalCleanups = new Map();
    agentOrder = [];
    activeAgentId = null;
    exitCallback = null;
    autoApprovalOverrideCount = 0;
    ...
    async spawnAgent(config) {
        ...
        const runWithContext = (fn) => runInContext ? runInContext(fn) : fn();
        ...
        const perAgent = await runWithContext(() => createPerAgentConfig(this.runtimeContext, config.agentId, config.cwd, inProcessConfig.runtimeConfig.modelConfig.model, inProcessConfig.authOverrides, inProcessConfig.approvalMode, {
        ...
        this.runtimeContext
            .getMonitorRegistry()
            .setAgentNotificationCallback(config.agentId, (_displayText, modelText) => interactive.enqueueMessage(modelText));
    ...
    releaseAgentResources(agentId, registry = this.agentRegistries.get(agentId)) {
 ...
 */
async function createPerAgentConfig(base, agentId, cwd, modelId, authOverrides, approvalMode, approvalModeHooks) {
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    let override = Object.create(base);
    let dedicatedContentGenerator;
    let runtimeView;
    let cleanup = () => { };
    if (approvalMode !== undefined) {
        const handle = createApprovalModeConfigOverride(base, approvalMode, approvalModeHooks);
    ...
    try {
        ...
        const agentFileService = new FileDiscoveryService(cwd, base.getFileFilteringOptions().customIgnoreFiles);
        ...
        if (agentRegistry) {
            void agentRegistry.stop().catch((stopError) => {
                debugLogger.error('Failed to stop partially created agent tool registry:', stopError);
            });
        }
        throw error;
    }
}
function createApprovalModeConfigOverride(base, mode, hooks) {
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    ...
    override.setApprovalMode = (nextMode, options) => {
        const beforeMode = override.getApprovalMode();
        ...
        try {
            Config.prototype.setApprovalMode.call(override, nextMode, options);
        }
        finally {
        ...
        const afterMode = override.getApprovalMode();
        if (beforeMode !== ApprovalMode.AUTO && afterMode === ApprovalMode.AUTO) {
            acquireAutoOverride();
        }
        else if (beforeMode === ApprovalMode.AUTO &&
    ...
    if (initialMode === ApprovalMode.AUTO &&
        base.getApprovalMode() !== ApprovalMode.AUTO) {
        acquireAutoOverride();
    }
    return { config: override, cleanup };
}
function getTrustedInitialApprovalMode(base, mode) {
    if (!base.isTrustedFolder() &&
...
function createRunInContext(inProcessConfig) {
    const identity = inProcessConfig.teammateIdentity;
    if (!identity) {
        return undefined;
    }
    return (fn) => runWithTeammateIdentity(identity, fn);
}
