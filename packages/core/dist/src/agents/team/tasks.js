 ...
 * `~/.lailatulcoder/tasks/{teamName}/{id}.json`.
...
function assertMetadataWithinLimit(metadata) {
    ...
    if (size > MAX_METADATA_BYTES) {
        throw new Error(`Task metadata is too large (${size} bytes; max ${MAX_METADATA_BYTES}). ` +
...
export function assertValidTaskId(taskId) {
    if (!/^[1-9]\d*$/.test(taskId)) {
        throw new Error(`Invalid task ID "${taskId}". Task IDs must be positive integers.`);
    }
}
/** Path to a single task file. */
export function getTaskPath(teamName, taskId) {
 ...
 */
export function onTasksUpdated(listener) {
...
export function notifyTasksUpdated(teamName) {
 ...
 */
export async function createTask(teamName, opts) {
    ...
    for (let attempt = 0; attempt < MAX_RETRIES; attempt++) {
        ...
        try {
            ...
            const handle = await fs.open(taskPath, fsConstants.O_WRONLY | fsConstants.O_CREAT | fsConstants.O_EXCL);
            await handle.close();
        }
        catch (err) {
    ...
    throw new Error(`Failed to create task after ${MAX_RETRIES} attempts (ID contention).`);
}
/**
 * Read a single task by ID.
 * Returns undefined if the task doesn't exist.
 */
export async function getTask(teamName, taskId) {
 ...
 */
export async function updateTask(teamName, taskId, updates, opts) {
    const taskPath = getTaskPath(teamName, taskId);
    return withTaskFileLock(taskPath, async () => {
        let raw;
        try {
        ...
        if (opts?.callerName !== undefined &&
            ...
            const restrictsOwnership = updates.status !== undefined ||
 ...
 * unclaimable forever.
 */
export async function deleteTask(teamName, taskId, opts) {
    const taskPath = getTaskPath(teamName, taskId);
    ...
    const dependentIds = await withTaskFileLock(taskPath, async () => {
        let task;
        try {
            const raw = await fs.readFile(taskPath, 'utf-8');
            task = JSON.parse(raw);
        ...
        const deps = new Set([...task.blocks, ...task.blockedBy]);
        deps.delete(taskId);
        try {
    ...
    const results = await Promise.allSettled(Array.from(dependentIds).map((depId) => removeEdgesReferencing(teamName, depId, taskId)));
...
async function removeEdgesReferencing(teamName, targetId, referencedId) {
    ...
    await withTaskFileLock(depPath, async () => {
        try {
            ...
            if (task.blocks.length === beforeBlocks &&
 ...
 */
export async function listTasks(teamName, filters) {
    const dir = getTasksDir(teamName);
    let entries;
    try {
    ...
    for (let i = 0; i < jsonEntries.length; i += MAX_PARALLEL_TASK_READS) {
        ...
        const batchReads = await Promise.all(batch.map(async (entry) => {
            ...
            try {
                ...
                try {
                    ...
                    const renameMsg = renameErr instanceof Error
                        ? renameErr.message
                        : String(renameErr);
    ...
    const tasks = reads.filter((t) => t !== undefined);
    ...
    return tasks.filter((t) => {
        ...
        if (filters.blockedBy !== undefined &&
 ...
 */
export async function resetTaskList(teamName) {
    const dir = getTasksDir(teamName);
...
async function unblockDependents(teamName, completedId, dependentIds) {
    ...
    const results = await Promise.allSettled(dependentIds.map(async (depId) => {
 ...
 */
export async function blockTask(teamName, fromId, toId) {
 ...
 * or not found.
 */
export async function claimTask(teamName, taskId, agentId, opts) {
    ...
    if (opts?.checkAgentBusy) {
        return getAgentClaimLock(agentId).runExclusive(() => claimTaskLocked(teamName, taskId, agentId, opts, true));
    }
 ...
 */
export async function releaseOwnedTask(teamName, taskId, expectedOwner) {
    const taskPath = getTaskPath(teamName, taskId);
    return withTaskFileLock(taskPath, async () => {
        let raw;
        try {
 ...
 */
export async function unassignTeammateTasks(teamName, agentId) {
    ...
    const owned = inProgress.filter((task) => task.owner === agentId || task.owner === bareName);
    ...
    const results = await Promise.allSettled(owned.map((task) => releaseOwnedTask(teamName, task.id, task.owner)));
    const failed = results.filter((r) => r.status === 'rejected');
    if (failed.length > 0) {
        debug.warn(`unassignTeammateTasks: ${failed.length}/${owned.length} task(s) failed to unassign for ${agentId}`);
    }
