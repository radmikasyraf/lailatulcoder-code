...
import { createLazyBridgeProxy, extractContextFilename, formatChannelWorkerDaemonUrl, InvalidPolicyConfigError, createDisabledChannelWorkerSupervisor, createBoundChannelDeliveryHandler, resolveRuntimeStartupTimeoutMs, runQwenServe, subSessionConcurrencyCapsFromSettings, validatePolicyConfig, waitForRuntimeStartingForShutdown, } from './run-qwen-serve.js';
...
import { journalGrowthPoolMb, resolveDaemonMemoryBudget, } from '@lailatul-coder/acp-bridge/daemonMemoryBudget';
import { canonicalizeWorkspace } from '@lailatul-coder/acp-bridge/workspacePaths';
import { DEFAULT_MAX_JOURNAL_BYTES, DEFAULT_MAX_JOURNAL_EVENTS, JOURNAL_GROWTH_HARD_CAP_BYTES, } from '@lailatul-coder/acp-bridge/replayWindowLimits';
...
const isolatedTestRuntimeDir = fs.realpathSync(fs.mkdtempSync(path.join(os.tmpdir(), 'qws-run-serve-tests-')));
...
describe('createBoundChannelDeliveryHandler', () => {
    ...
    it('routes only to the workspace captured by the bridge', async () => {
        ...
        const handler = createBoundChannelDeliveryHandler('/canonical', () => ({ deliverChannelMessage }), authorizations);
        await expect(handler({ ...info, workspaceCwd: '/forged' })).resolves.toEqual({ status: 'delivered' });
    ...
    it('does not lazily start a missing manager and returns a clear failure', async () => {
        ...
        const handler = createBoundChannelDeliveryHandler('/canonical', getManager, authorizations, { warn });
    ...
    it('rejects an unauthorized or changed target before worker IPC', async () => {
        ...
        await expect(handler({ ...info, target: { ...info.target, id: 'user-2' } })).resolves.toEqual({
            status: 'failed',
            code: 'channel_delivery_invalid',
            error: 'Channel delivery is not authorized.',
        });
    ...
    it('consumes prompt authorization before skipping an empty final', async () => {
        ...
        const handler = createBoundChannelDeliveryHandler('/canonical', getManager, authorizations);
    ...
    it('logs a sanitized diagnostic for unexpected delivery failures', async () => {
        ...
        const handler = createBoundChannelDeliveryHandler('/canonical', () => ({ deliverChannelMessage }), authorizations, { warn }, {
        ...
        try {
            ...
            expect(JSON.stringify({ result, calls: warn.mock.calls })).not.toMatch(/worker-secret|daemon-secret|user-1/u);
        }
        finally {
            vi.unstubAllEnvs();
        }
    ...
    it('sanitizes typed delivery failures before returning them', async () => {
        ...
        const deliverChannelMessage = vi
            .fn()
            .mockRejectedValue(new ChannelDeliveryError('channel_worker_unavailable', 'Channel worker is not running. abcdworkerxyz daemon-secret /canonical'));
        const warn = vi.fn();
        const authorizations = new ChannelDeliveryAuthorizationStore();
        authorizations.authorizePrompt('/canonical', {
        ...
        try {
            ...
            expect(warn).toHaveBeenCalledWith('channel delivery failed', {
                ...
                code: 'channel_worker_unavailable',
                diagnostic: 'Channel <redacted> is not running. <redacted> <redacted> /canonical',
            });
            expect(JSON.stringify({ result, calls: warn.mock.calls })).not.toMatch(/abcdworkerxyz|abcd|xyz|daemon-secret/u);
        }
        finally {
            vi.unstubAllEnvs();
        }
...
function writeWebShellFixture(workspaceDir) {
    const shellDir = path.join(workspaceDir, 'web-shell');
    fs.mkdirSync(path.join(shellDir, 'assets'), { recursive: true });
    fs.writeFileSync(path.join(shellDir, 'index.html'), '<!doctype html><body><div id="root"></div></body>');
    vi.spyOn(webShellResolver, 'resolveWebShellDir').mockReturnValue(shellDir);
    return shellDir;
}
async function startDeferredDaemon(workspace, overrides = {}) {
...
vi.mock('@lailatul-coder/acp-bridge/spawnChannel', async (importOriginal) => {
    ...
    return {
        ...actual,
        createSpawnChannelFactory: vi.fn((options = {}) => {
...
vi.mock('./channel-worker-manager.js', async (importOriginal) => {
    ...
    return {
        ...actual,
        createChannelWorkerManager: (...args) => {
...
describe('workspace skill settings persistence', () => {
    ...
    it('canonicalizes, deduplicates, preserves orphans, serializes updates, and enforces user locks', async () => {
        workspace = fs.realpathSync(fs.mkdtempSync(path.join(os.tmpdir(), 'qws-skill-settings-')));
        qwenHome = fs.realpathSync(fs.mkdtempSync(path.join(os.tmpdir(), 'qws-skill-home-')));
        previousQwenHome = process.env['QWEN_HOME'];
        process.env['QWEN_HOME'] = qwenHome;
        settingsRuntime.resetHomeEnvBootstrapForTesting();
        fs.mkdirSync(path.join(workspace, '.qwen'), { recursive: true });
        fs.writeFileSync(path.join(workspace, '.qwen', 'settings.json'), JSON.stringify({
        ...
        await expect(persistDisabledSkills(workspace, 'inherited-opt-in', true)).resolves.toEqual({
            changed: false,
            disabled: ['orphan', ' ReViEw ', 'review'],
        });
        await expect(persistDisabledSkills(workspace, 'review', false)).resolves.toEqual({
            changed: true,
        ...
        await expect(persistDisabledSkills(workspace, 'review', true)).resolves.toMatchObject({ changed: true });
        await expect(persistDisabledSkills(workspace, 'opt-in-skill', true)).resolves.toEqual({
            changed: true,
            disabled: ['orphan', 'alpha', 'beta'],
            settingsChanges: [{ key: 'skills.enabled', value: ['opt-in-skill'] }],
        });
        const saved = JSON.parse(fs.readFileSync(path.join(workspace, '.qwen', 'settings.json'), 'utf8'));
        expect(saved.skills.disabled).toEqual(['orphan', 'alpha', 'beta']);
        expect(saved.skills.enabled).toEqual(['opt-in-skill']);
        await expect(persistDisabledSkills(workspace, 'locked-skill', true)).rejects.toMatchObject({ reason: 'locked', lockedScope: 'user' });
    });
    it('produces both skills.disabled and skills.enabled changes when enabling a workspace-hard-disabled default-disabled skill', async () => {
        workspace = fs.realpathSync(fs.mkdtempSync(path.join(os.tmpdir(), 'qws-skill-dual-')));
        qwenHome = fs.realpathSync(fs.mkdtempSync(path.join(os.tmpdir(), 'qws-skill-dual-home-')));
        previousQwenHome = process.env['QWEN_HOME'];
        process.env['QWEN_HOME'] = qwenHome;
        settingsRuntime.resetHomeEnvBootstrapForTesting();
        fs.mkdirSync(path.join(workspace, '.qwen'), { recursive: true });
        ...
        await expect(persistDisabledSkills(workspace, 'dual-skill', true)).resolves.toEqual({
            changed: true,
            disabled: [],
            settingsChanges: [
        ...
        const setValue = vi.spyOn(settingsRuntime.LoadedSettings.prototype, 'setValue');
        const setValues = vi.spyOn(settingsRuntime.LoadedSettings.prototype, 'setValues');
    ...
    it('persists a Skill batch with one settings write and per-target lock outcomes', async () => {
        workspace = fs.realpathSync(fs.mkdtempSync(path.join(os.tmpdir(), 'qws-skill-batch-')));
        qwenHome = fs.realpathSync(fs.mkdtempSync(path.join(os.tmpdir(), 'qws-skill-batch-home-')));
        previousQwenHome = process.env['QWEN_HOME'];
        process.env['QWEN_HOME'] = qwenHome;
        settingsRuntime.resetHomeEnvBootstrapForTesting();
        fs.mkdirSync(path.join(workspace, '.qwen'), { recursive: true });
        fs.writeFileSync(path.join(workspace, '.qwen', 'settings.json'), JSON.stringify({ skills: { disabled: ['orphan'] } }));
        fs.writeFileSync(path.join(qwenHome, 'settings.json'), JSON.stringify({
            skills: {
                disabled: ['locked-skill'],
        ...
        const setValues = vi.spyOn(settingsRuntime.LoadedSettings.prototype, 'setValues');
        const result = await persistDisabledSkillsBatch(workspace, ['review', 'alpha', 'locked-skill'], false);
        ...
        const noopResult = await persistDisabledSkillsBatch(workspace, ['review'], false);
        expect(noopResult.outcomes).toEqual([
            { skillName: 'review', changed: false },
        ]);
        expect(noopResult.settingsChanges).toEqual([]);
        expect(setValues).toHaveBeenCalledOnce();
        const savedAfterDisable = JSON.parse(fs.readFileSync(path.join(workspace, '.qwen', 'settings.json'), 'utf8'));
        ...
        const savedUser = JSON.parse(fs.readFileSync(path.join(qwenHome, 'settings.json'), 'utf8'));
        expect(savedUser.skills.disabled).toEqual(['locked-skill']);
        expect(savedUser.skills.enabled).toBeUndefined();
        const preinstallNoop = await persistDisabledSkillsBatch(workspace, ['future-skill'], true);
        expect(preinstallNoop.outcomes).toEqual([
            { skillName: 'future-skill', changed: false },
        ]);
        expect(preinstallNoop.settingsChanges).toEqual([]);
        expect(setValues).toHaveBeenCalledOnce();
        const preinstallEnable = await persistDisabledSkillsBatch(workspace, ['orphan'], true);
        ...
        const enableResult = await persistDisabledSkillsBatch(workspace, ['opt-in'], true);
        ...
        const savedAfterEnable = JSON.parse(fs.readFileSync(path.join(workspace, '.qwen', 'settings.json'), 'utf8'));
        ...
        await expect(persistDisabledSkillsBatch(workspace, ['guarded-too'], false, blockingGuard)).rejects.toThrow('generation closed');
...
describe('extractContextFilename (#4297 fold-in 7 P2-1 helper)', () => {
    ...
    it('returns the first non-empty string when given an array', () => {
        expect(extractContextFilename(['AGENTS.md', 'BACKUP.md'])).toBe('AGENTS.md');
        // Skips empty and whitespace entries to find the first valid name.
        expect(extractContextFilename(['', '  ', 'PRIMARY.md', 'OTHER.md'])).toBe('PRIMARY.md');
    ...
    it('returns undefined when the array has no string entries', () => {
        ...
        expect(extractContextFilename([null, undefined, 42, { a: 1 }])).toBeUndefined();
    });
...
describe('subSessionConcurrencyCapsFromSettings', () => {
    it('passes through positive integer caps', () => {
        expect(subSessionConcurrencyCapsFromSettings({
    ...
    it('warns naming a present-but-invalid cap', () => {
        const onWarning = vi.fn();
        expect(subSessionConcurrencyCapsFromSettings({ maxConcurrentSubSessionsTotal: '50' }, onWarning)).toEqual({});
        expect(onWarning).toHaveBeenCalledTimes(1);
        expect(onWarning.mock.calls[0][0]).toContain('maxConcurrentSubSessionsTotal');
...
describe('formatChannelWorkerDaemonUrl', () => {
    it.each(['', '0.0.0.0', '::', '[::]'])('uses loopback when the daemon binds wildcard host %j', (host) => {
        expect(formatChannelWorkerDaemonUrl(host, 4170)).toBe('http://127.0.0.1:4170');
    });
    ...
    it('preserves and accepts concrete IPv4 loopback hosts in 127/8', () => {
        expect(formatChannelWorkerDaemonUrl('127.0.0.2', 4170)).toBe('http://127.0.0.2:4170');
...
describe('validatePolicyConfig (#4335 boot validation)', () => {
    ...
    it.each([['first-responder'], ['designated'], ['consensus'], ['local-only']])('accepts the %s permissionStrategy literal', (literal) => {
    ...
    it('throws InvalidPolicyConfigError for an unknown permissionStrategy', () => {
        expect(() => validatePolicyConfig({ permissionStrategy: 'bogus' })).toThrow(InvalidPolicyConfigError);
        expect(() => validatePolicyConfig({ permissionStrategy: 'bogus' })).toThrow(/invalid policy.permissionStrategy/);
    });
    it.each([0, -1, 1.5, Number.NaN])('throws InvalidPolicyConfigError for non-positive-integer consensusQuorum (%s)', (badValue) => {
    ...
    it('warns AND drops consensusQuorum when strategy is not consensus (#4335 / 3273077270)', () => {
        ...
        const warnings = [];
        const onWarning = vi.fn((m) => warnings.push(m));
    ...
    it('does not warn when consensusQuorum is set with consensus strategy', () => {
        const onWarning = vi.fn();
        validatePolicyConfig({ permissionStrategy: 'consensus', consensusQuorum: 2 }, onWarning);
        expect(onWarning).not.toHaveBeenCalled();
    });
    it('error messages name the field that failed (operator-debugging signal)', () => {
        expect(() => validatePolicyConfig({ permissionStrategy: 'oops' })).toThrow(/permissionStrategy/);
        expect(() => validatePolicyConfig({ consensusQuorum: 0 })).toThrow(/consensusQuorum/);
    });
});
/**
...
describe('runQwenServe daemon logger wiring', () => {
    ...
    it('creates a daemon log file at boot and flushes on shutdown', async () => {
        ...
        try {
            ...
            const logContent = fs.readFileSync(path.join(daemonDir, 'daemon.log'), 'utf8');
            ...
            expect(logContent).toContain(`workspace=${fs.realpathSync.native(workspace)}`);
            ...
            await Promise.all(Array.from({ length: 70 }, (_, index) => fetch(`${handle.url}/missing-${index}`)));
...
describe('runQwenServe telemetry validation', () => {
    let tmpDir;
    const originalSensitiveSpanAttributeMaxLengthEnv = process.env['QWEN_TELEMETRY_SENSITIVE_SPAN_ATTRIBUTE_MAX_LENGTH'];
    afterEach(() => {
        vi.restoreAllMocks();
    ...
    it('accepts multiple explicit workspace inputs and advertises workspaces', async () => {
        ...
        const createBridge = vi
            ...
            bridge.shutdown = vi.fn(() => new Promise((resolve) => {
        ...
        try {
            ...
            for (const [index, [bridgeOptions],] of createBridge.mock.calls.entries()) {
                const target = path.join(tmpDir, `static-runtime-external-${index}.txt`);
            ...
            closing ??= handle.close();
            await vi.waitFor(() => expect(shutdownResolvers).toHaveLength(2));
            for (const resolve of shutdownResolvers)
                resolve();
            await closing;
        }
    ...
    it('keeps external built-in writes disabled for an injected primary filesystem factory', async () => {
        tmpDir = fs.realpathSync(fs.mkdtempSync(path.join(os.tmpdir(), 'qws-injected-fs-')));
        ...
        try {
            await expect(createBridge.mock.calls[0]?.[0].fileSystem.writeText({
    ...
    it('invalidates primary voice capabilities when its workspace service publishes settings changes', async () => {
        tmpDir = fs.realpathSync(fs.mkdtempSync(path.join(os.tmpdir(), 'qws-voice-capability-')));
        const workspace = path.join(tmpDir, 'workspace');
        fs.mkdirSync(workspace);
        vi.spyOn(qwenCore, 'resolveTelemetrySettings').mockResolvedValue({
            enabled: false,
        ...
        vi.spyOn(acpBridge, 'createAcpSessionBridge').mockImplementation(() => makeRuntimeBridge());
        const originalCreateWorkspaceService = workspaceServiceRuntime.createDaemonWorkspaceService;
        let publishWorkspaceEvent;
        vi.spyOn(workspaceServiceRuntime, 'createDaemonWorkspaceService').mockImplementation((deps) => {
        ...
        try {
            const before = (await (await fetch(`${handle.url}/capabilities`)).json());
            ...
            const after = (await (await fetch(`${handle.url}/capabilities`)).json());
    ...
    it('adds, advertises, and hot-removes a dynamic workspace runtime', async () => {
        ...
        tmpDir = fs.realpathSync(fs.mkdtempSync(path.join(os.tmpdir(), 'qws-hot-remove-')));
        const primary = path.join(tmpDir, 'primary');
        const secondary = path.join(tmpDir, 'secondary');
        fs.mkdirSync(primary);
        fs.mkdirSync(secondary);
        ...
        vi.spyOn(settingsRuntime, 'loadSettings').mockImplementation((workspace) => ({
            merged: typeof workspace === 'string' &&
        ...
        try {
            ...
            expect(createBridge.mock.calls[0]?.[0].onChannelDelivery).toBeTypeOf('function');
            expect(createBridge.mock.calls[1]?.[0].onChannelDelivery).toBeTypeOf('function');
            ...
            expect(createBridge.mock.calls[1]?.[0].journalGrowthPoolBytes).toEqual(expect.any(Number));
            expect(createBridge.mock.calls[1]?.[0].journalGrowthPoolBytes).toBe(createBridge.mock.calls[0]?.[0].journalGrowthPoolBytes);
            ...
            // bridges at once.
            expect(createBridge.mock.calls[0]?.[0].journalGrowthSessionLimits).toBeTypeOf('function');
            expect(createBridge.mock.calls[1]?.[0].journalGrowthSessionLimits).toBe(createBridge.mock.calls[0]?.[0].journalGrowthSessionLimits);
            expect(createBridge.mock.calls[0]?.[0].registerJournalGrowthSessionLimits).toBeTypeOf('function');
            expect(createBridge.mock.calls[1]?.[0].registerJournalGrowthSessionLimits).toBe(createBridge.mock.calls[0]?.[0].registerJournalGrowthSessionLimits);
            expect(createBridge.mock.calls[1]?.[0]).not.toHaveProperty('permissionConsensusQuorum');
            const firstDynamicFileSystem = createBridge.mock.calls[1]?.[0].fileSystem;
            const firstDynamicTarget = path.join(tmpDir, 'dynamic-runtime-external.txt');
            ...
            expect(fs.readFileSync(firstDynamicTarget, 'utf8')).toBe('first-generation');
            const before = (await (await fetch(`${handle.url}/capabilities`, { headers })).json());
            expect(before.features).toContain('workspace_runtime_removal');
            const removable = before.workspaces.find((workspace) => workspace.cwd === canonicalizeWorkspace(secondary));
            expect(removable).toMatchObject({ removable: true });
            const removed = await fetch(`${handle.url}/workspaces/${encodeURIComponent(removable.id)}`, {
                method: 'DELETE',
                headers,
            ...
            expect(removeByIds).toHaveBeenCalledWith(expect.arrayContaining([
            ...
            await expect(firstDynamicFileSystem.writeText({
            ...
            expect((after.workspaces ?? []).some((workspace) => workspace.id === removable.id)).toBe(false);
            ...
            const secondDynamicTarget = path.join(tmpDir, 'dynamic-runtime-readded.txt');
            ...
            expect(fs.readFileSync(secondDynamicTarget, 'utf8')).toBe('second-generation');
            for (const [options] of createBridge.mock.calls) {
                expect(options).toMatchObject({
                    delegateReadTextFileToClient: false,
                    sessionRestoreTimeoutMs: 90_000,
            ...
            let releaseRemoval;
            removeByIds.mockImplementationOnce(() => new Promise((resolve) => {
                releaseRemoval = resolve;
            }));
            const pendingRemoval = fetch(`${handle.url}/workspaces/${encodeURIComponent(removable.id)}`, {
                method: 'DELETE',
                headers,
                body: JSON.stringify({ force: true }),
            });
    ...
    it('kills a half-built dynamic bridge when async construction cleanup fails', async () => {
        tmpDir = fs.realpathSync(fs.mkdtempSync(path.join(os.tmpdir(), 'qws-runtime-failure-')));
        const primary = path.join(tmpDir, 'primary');
        const secondary = path.join(tmpDir, 'secondary');
        fs.mkdirSync(primary);
        fs.mkdirSync(secondary);
        ...
        vi.mocked(failedBridge.shutdown).mockRejectedValue(new Error('async cleanup failed'));
        vi.spyOn(acpBridge, 'createAcpSessionBridge')
            .mockReturnValueOnce(primaryBridge)
            .mockReturnValueOnce(failedBridge);
        const originalCreateWorkspaceService = workspaceServiceRuntime.createDaemonWorkspaceService;
        const createWorkspaceService = vi.spyOn(workspaceServiceRuntime, 'createDaemonWorkspaceService');
    ...
    it('uses the daemon-wide policy and limits when constructing workspace bridges', async () => {
        ...
        const createBridge = vi
            .spyOn(acpBridge, 'createAcpSessionBridge')
            .mockReturnValueOnce(primaryBridge)
            .mockReturnValueOnce(secondaryBridge);
        const resolveBridgeFsFactory = vi.spyOn(serverModule, 'resolveBridgeFsFactory');
        const createWorkspaceService = vi.spyOn(workspaceServiceRuntime, 'createDaemonWorkspaceService');
        vi.spyOn(qwenCore, 'resolveTelemetrySettings').mockResolvedValue({
            enabled: false,
            sensitiveSpanAttributeMaxLength: 1024 * 1024,
        ...
        vi.spyOn(settingsRuntime, 'loadSettings').mockImplementation((workspace) => {
            const workspaceCwd = typeof workspace === 'string' ? canonicalizeWorkspace(workspace) : '';
            return {
                merged: workspaceCwd === secondaryCwd
                    ? {
                        policy: {
                            permissionStrategy: 'consensus',
                            consensusQuorum: 2,
        ...
        try {
            ...
            expect(createBridge.mock.calls[1]?.[0]).not.toHaveProperty('permissionConsensusQuorum');
            expect(resolveBridgeFsFactory.mock.calls.find(([input]) => input.boundWorkspaces.length === 1 &&
                input.boundWorkspaces[0] === secondaryCwd)?.[0]).toMatchObject({ customIgnoreFiles: ['.secondaryignore'] });
            expect(createWorkspaceService.mock.calls.find(([input]) => input.boundWorkspace === secondaryCwd)?.[0]).toMatchObject({ contextFilename: 'SECONDARY.md' });
        }
        finally {
            await handle.close();
        }
    ...
    it('does not validate policy settings for untrusted secondary workspaces', async () => {
        ...
        const createBridge = vi
            .spyOn(acpBridge, 'createAcpSessionBridge')
            .mockReturnValueOnce(makeRuntimeBridge())
            .mockReturnValueOnce(makeRuntimeBridge());
        vi.spyOn(qwenCore, 'resolveTelemetrySettings').mockResolvedValue({
            enabled: false,
            sensitiveSpanAttributeMaxLength: 1024 * 1024,
        ...
        vi.spyOn(trustedFoldersRuntime, 'getWorkspaceTrustStatus').mockImplementation((_settings, workspace) => ({
            effective: {
                state: canonicalizeWorkspace(workspace) === secondaryCwd
                    ? 'untrusted'
                    : 'trusted',
            },
        }));
    ...
    it('uses a daemon-scoped telemetry service instance id', async () => {
        ...
        try {
            const runtimeConfig = initializeTelemetry.mock.calls[0]?.[0];
...
describe('runQwenServe permissionResponseTimeoutMs validation', () => {
    ...
    it('rejects a non-finite permissionResponseTimeoutMs', async () => {
        ...
        try {
            ...
            }, { bridge: fakeBridge })).rejects.toThrow(/permissionResponseTimeoutMs/);
            const log = fs.readFileSync(path.join(tmpDir, 'debug', 'daemon', 'daemon.log'), 'utf8');
    ...
    it('preserves the startup error and releases the log lease when stderr fails', async () => {
        ...
        expect(fs.existsSync(path.join(tmpDir, 'debug', 'daemon', '.stable-writer.lock'))).toBe(false);
    });
});
/**
...
describe('runQwenServe memory budget', () => {
    ...
    function makeTmpDir() {
        const dir = fs.realpathSync(fs.mkdtempSync(path.join(os.tmpdir(), 'qws-mem-')));
        tmpDirs.push(dir);
        return dir;
    }
    it('reports the resolved budget over HTTP without sizing any child', async () => {
        ...
        try {
            ...
            if (memory?.childHeap?.perChildCeilingMb === null) {
                ...
                expect(memory?.childHeap?.maxConcurrentChildren ?? 0).toBeGreaterThan(0);
                expect((memory?.childHeap?.maxConcurrentChildren ?? 0) *
                    (memory?.childHeap?.perChildCeilingMb ?? 0)).toBeLessThanOrEqual(memory?.modeled.childPoolMb ?? 0);
            }
            ...
            expect(memory?.effectiveBudgetMb).toBeLessThanOrEqual(memory?.availableMemoryMb ?? 0);
            // Modeled pools stay non-negative and inside the budget they come from.
            expect(memory?.modeled.rootReserveMb).toBeLessThanOrEqual(memory?.effectiveBudgetMb ?? 0);
            expect(memory?.modeled.childPoolMb).toBeGreaterThanOrEqual(0);
            expect(memory?.modeled.childPoolMb).toBeLessThan(memory?.effectiveBudgetMb ?? 0);
            ...
            expect(runtimeMemory?.modeled.recommendedShareAtRegisteredMb).toBeGreaterThan(0);
            ...
            expect(pressure?.availableBytes).toBe((memory?.availableMemoryMb ?? 0) * 1024 * 1024);
            expect(pressure?.ratio).toBe(Math.max(pressure?.rssRatio ?? 0, pressure?.heapRatio ?? 0));
            ...
            expect(children?.sampled).toBeLessThanOrEqual(runtimeMemory?.activeAcpChildren ?? 0);
    ...
    it('derives an adaptive journal growth pool into every bridge', async () => {
        ...
        try {
            ...
            // available memory.
            const expectedPoolBytes = journalGrowthPoolMb(resolveDaemonMemoryBudget({ budgetMb: 4096 })) *
                1024 *
                1024;
            for (const [options] of createBridge.mock.calls) {
    ...
    it('disables adaptive journal growth on a host too small for the budget', async () => {
        ...
        try {
            ...
            for (const [options] of createBridge.mock.calls) {
                ...
                expect(options).not.toHaveProperty('registerJournalGrowthSessionLimits');
            }
        }
        finally {
            createBridge.mockRestore();
    ...
    it('wires every bridge to one shared daemon-wide growth-pool view', async () => {
        ...
        try {
            ...
            for (const [options] of createBridge.mock.calls) {
                ...
                expect(typeof options.registerJournalGrowthSessionLimits).toBe('function');
            }
            ...
            const views = createBridge.mock.calls.map(([options]) => options.journalGrowthSessionLimits);
            const unregisters = createBridge.mock.calls.map(([options], index) => options.registerJournalGrowthSessionLimits?.(() => [
...
describe('runQwenServe initializeTimeoutMs validation', () => {
    ...
    it('rejects a non-positive initializeTimeoutMs', async () => {
        ...
        try {
            ...
            }, { bridge: fakeBridge })).rejects.toThrow(/initializeTimeoutMs/);
        }
        finally {
            delete process.env['QWEN_RUNTIME_DIR'];
            if (origEnv !== undefined) {
...
describe('runQwenServe TLS (--tls-cert / --tls-key)', () => {
    ...
    it.each([
        ...
        tmpDir = fs.realpathSync(fs.mkdtempSync(path.join(os.tmpdir(), 'qws-tls-')));
        const origEnv = process.env['QWEN_RUNTIME_DIR'];
        process.env['QWEN_RUNTIME_DIR'] = tmpDir;
        try {
            await expect(runQwenServe({
                ...
                maxSessions: 1,
                ...tlsOpts,
            }, { bridge: minimalBridge() })).rejects.toThrow(/--tls-cert and --tls-key must be provided together/);
        }
        finally {
            delete process.env['QWEN_RUNTIME_DIR'];
            if (origEnv !== undefined) {
    ...
    it('rejects an unreadable cert file', async () => {
        ...
        try {
            ...
            }, { bridge: minimalBridge() })).rejects.toThrow(/Failed to read --tls-cert/);
        }
        finally {
            delete process.env['QWEN_RUNTIME_DIR'];
            if (origEnv !== undefined) {
    ...
    it('rejects an unreadable key file', async () => {
        ...
        try {
            ...
            }, { bridge: minimalBridge() })).rejects.toThrow(/Failed to read --tls-key/);
        }
        finally {
            delete process.env['QWEN_RUNTIME_DIR'];
            if (origEnv !== undefined) {
    ...
    it('rejects an expired certificate at boot', async () => {
        ...
        try {
            await expect(runQwenServe({
                ...
                tlsCert: certPath,
                tlsKey: keyPath,
            }, { bridge: minimalBridge() })).rejects.toThrow(/expired on/);
        }
        finally {
            delete process.env['QWEN_RUNTIME_DIR'];
            if (origEnv !== undefined) {
    ...
    it('rejects an unparseable certificate at boot', async () => {
        ...
        try {
            await expect(runQwenServe({
                ...
                tlsCert: certPath,
                tlsKey: keyPath,
            }, { bridge: minimalBridge() })).rejects.toThrow(/is not a valid certificate/);
        }
        finally {
            delete process.env['QWEN_RUNTIME_DIR'];
            if (origEnv !== undefined) {
    ...
    it('rejects a cert/key mismatch at boot', async () => {
        ...
        try {
            await expect(runQwenServe({
                ...
                tlsCert: certPath,
                tlsKey: keyPath,
            }, { bridge: minimalBridge() })).rejects.toThrow(/could not be loaded/);
        }
        finally {
            delete process.env['QWEN_RUNTIME_DIR'];
            if (origEnv !== undefined) {
    ...
    it('serves over https when both cert and key are valid', async () => {
        ...
        vi.spyOn(qwenCore, 'resolveTelemetrySettings').mockReturnValue(new Promise((resolve) => {
        ...
        vi.spyOn(acpBridge, 'createAcpSessionBridge').mockReturnValue(bridge);
        const handle = await runQwenServe({
            port: 0,
            hostname: '127.0.0.1',
            mode: 'http-bridge',
        ...
        try {
            ...
            const statusCode = await new Promise((resolve, reject) => {
                const req = https.get(`${handle.url}/health`, { rejectUnauthorized: false }, (res) => {
                    res.resume();
...
describe('runQwenServe pre-listen bridge option validation', () => {
    ...
    ])('rejects invalid %s=%s before printing the listening line', async (optionName, value, message) => {
        tmpDir = fs.realpathSync(fs.mkdtempSync(path.join(os.tmpdir(), 'qws-bridge-opt-')));
        const stdoutWrites = [];
    ...
    it.each([
        ...
        tmpDir = fs.realpathSync(fs.mkdtempSync(path.join(os.tmpdir(), 'qws-rate-opt-')));
        const stdoutWrites = [];
        vi.spyOn(process.stdout, 'write').mockImplementation((chunk) => {
            stdoutWrites.push(String(chunk));
            return true;
    ...
    it.each(['root', 'child', 'missing-child', 'alias'])('rejects an explicit Conversations reserved %s before listening', async (candidateKind) => {
        tmpDir = fs.realpathSync(fs.mkdtempSync(path.join(os.tmpdir(), 'qws-reserved-workspace-')));
        ...
        fs.mkdirSync(child);
        const missingChild = path.join(liveConversationWorkspace.rootPath, 'missing-session');
        const alias = path.join(tmpDir, 'conversation-alias');
        fs.symlinkSync(liveConversationWorkspace.rootPath, alias, process.platform === 'win32' ? 'junction' : 'dir');
        const workspace = candidateKind === 'root'
        ...
        await expect(runQwenServe({
            ...
            mode: 'http-bridge',
            workspace,
        }, { liveConversationWorkspace })).rejects.toThrow(/reserved for Conversations/);
        expect(stdoutWrites.join('')).not.toContain('qwen serve listening on');
    });
    it('rejects an unknown embedded external Tool Guard mode before listening', async () => {
        tmpDir = fs.realpathSync(fs.mkdtempSync(path.join(os.tmpdir(), 'qws-guard-opt-')));
        const stdoutWrites = [];
        vi.spyOn(process.stdout, 'write').mockImplementation((chunk) => {
            stdoutWrites.push(String(chunk));
            return true;
    ...
    it('rejects unsafe required provider configuration before listening', async () => {
        tmpDir = fs.realpathSync(fs.mkdtempSync(path.join(os.tmpdir(), 'qws-guard-config-')));
        const stdoutWrites = [];
        vi.spyOn(process.stdout, 'write').mockImplementation((chunk) => {
            stdoutWrites.push(String(chunk));
            return true;
...
describe('runQwenServe session reaper timeout validation', () => {
    ...
    async function runWithReaperOption(optionName, value) {
    ...
    ])('rejects invalid %s=%s', async (optionName, value) => {
        await expect(runWithReaperOption(optionName, value)).rejects.toThrow(optionName);
    });
    it.each([
        ['sessionReapIntervalMs', 0],
        ['sessionIdleTimeoutMs', 0],
    ])('keeps %s=0 as the disabled sentinel', async (optionName, value) => {
...
describe('runQwenServe runtime startup failures', () => {
    ...
    it('keeps the primary bridge reference from the reconciled startup generation', async () => {
        tmpDir = fs.realpathSync(fs.mkdtempSync(path.join(os.tmpdir(), 'qws-runtime-trust-race-')));
        const bootSnapshot = {
        ...
        vi.spyOn(acpBridge, 'createAcpSessionBridge')
            .mockReturnValueOnce(bootBridge)
            .mockReturnValueOnce(reconciledBridge);
        const handle = await runQwenServe({
            port: 0,
            hostname: '127.0.0.1',
            mode: 'http-bridge',
        ...
        try {
            ...
            expect(loadSettings).toHaveBeenCalledWith(tmpDir, expect.objectContaining({ workspaceTrusted: true }));
        }
        finally {
            await handle.close();
        }
    ...
    it('does not expose the disposed primary bridge after reconciliation blocks', async () => {
        tmpDir = fs.realpathSync(fs.mkdtempSync(path.join(os.tmpdir(), 'qws-runtime-trust-blocked-')));
        ...
        vi.spyOn(trustPolicyRuntime, 'readDaemonTrustPolicySnapshot').mockImplementation(async () => currentSnapshot);
        const bootBridge = makeRuntimeBridge();
        vi.spyOn(acpBridge, 'createAcpSessionBridge')
            .mockReturnValueOnce(bootBridge)
            .mockImplementationOnce(() => {
        ...
        try {
            ...
            await vi.waitFor(() => expect(bootBridge.shutdown).toHaveBeenCalledTimes(1));
            expect(() => handle.bridge.preheat()).toThrow('Daemon bridge runtime is still starting.');
            expect(bootBridge.preheat).not.toHaveBeenCalled();
        }
        finally {
    ...
    it('disposes ACP routing when runtime containment fails', async () => {
        tmpDir = fs.realpathSync(fs.mkdtempSync(path.join(os.tmpdir(), 'qws-runtime-acp-cleanup-')));
        const trustedSnapshot = {
            revision: 'trusted',
            folderTrustEnabled: false,
            ideTrust: undefined,
        ...
        vi.spyOn(acpBridge, 'createAcpSessionBridge').mockReturnValue(bootBridge);
    ...
    it('closes the listener before rejecting when resolveOnListen is false and runtime startup fails', async () => {
        tmpDir = fs.realpathSync(fs.mkdtempSync(path.join(os.tmpdir(), 'qws-runtime-fail-close-')));
    ...
    it.each([
        ...
        ['true', true],
        ['anything', true],
    ])('normalizes browser MCP env flag %j', async (raw, shouldEnable) => {
        const features = await readBrowserMcpFeatureFlagsForEnv(raw);
        if (shouldEnable) {
            expect(features).toEqual(expect.arrayContaining(['client_mcp_over_ws', 'cdp_tunnel_over_ws']));
        }
        else {
    ...
    it('does not advertise browser automation MCP without an active CDP tunnel', async () => {
        const features = await readBrowserMcpFeatureFlagsForEnv(undefined, 'https://example.com', '/opt/qwen-cdp-mcp-adapter');
        expect(features).not.toContain('browser_automation_mcp');
    });
    it('does not enable browser automation MCP on bearer-protected endpoints', () => {
        expect(isBrowserAutomationMcpAvailable({
            cdpTunnelOverWs: true,
            token: 'secret-token',
        }, {})).toBe(false);
    });
    it('forwards auto-enabled CDP tunnel state to the ACP child env', async () => {
        tmpDir = fs.realpathSync(fs.mkdtempSync(path.join(os.tmpdir(), 'qws-runtime-child-env-')));
        const originalClientMcpOverWs = process.env['QWEN_SERVE_CLIENT_MCP_OVER_WS'];
        const originalCdpTunnelOverWs = process.env['QWEN_SERVE_CDP_TUNNEL_OVER_WS'];
        delete process.env['QWEN_SERVE_CLIENT_MCP_OVER_WS'];
        delete process.env['QWEN_SERVE_CDP_TUNNEL_OVER_WS'];
        ...
        try {
            await handle.runtimeReady;
            const bridgeOptions = createBridge.mock.calls[0]?.[0];
            ...
            expect(bridgeOptions?.childEnvOverrides).toHaveProperty('QWEN_CODE_PRIVATE_EXTERNAL_TOOL_GUARD_PROVIDER', undefined);
            ...
            const daemonGuard = bridgeOptions?.externalToolGuard;
            await expect(daemonGuard({
                sessionId: 'session-1',
                promptId: 'prompt-1',
                toolCallId: 'call-1',
    ...
    it('forwards the provider-attached marker when a real provider handshakes', async () => {
        tmpDir = fs.realpathSync(fs.mkdtempSync(path.join(os.tmpdir(), 'qws-guard-provider-')));
    ...
    it.each([
    ...
    ])('%s', async (_name, launchScope, optionScope, expectedScope) => {
        tmpDir = fs.realpathSync(fs.mkdtempSync(path.join(os.tmpdir(), 'qws-memory-project-scope-')));
        const primary = path.join(tmpDir, 'primary');
        const secondary = path.join(tmpDir, 'secondary');
        fs.mkdirSync(primary);
        fs.mkdirSync(secondary);
        ...
        vi.spyOn(serverModule, 'createServeApp').mockImplementation((_opts, _getPort, deps) => {
        ...
        try {
            ...
            for (const runtime of workspaceRegistry?.list() ?? []) {
                expect(runtime.env.effectiveEnv?.['QWEN_CODE_MEMORY_PROJECT_SCOPE']).toBe(expectedScope);
            }
    ...
    it('rebuilds runtime env from the immutable daemon base after workspace reload', async () => {
        tmpDir = fs.realpathSync(fs.mkdtempSync(path.join(os.tmpdir(), 'qws-runtime-env-reload-')));
        ...
        const buildRuntimeEnvironment = vi.spyOn(environmentRuntime, 'buildRuntimeEnvironment');
        let workspace;
        ...
        vi.spyOn(serverModule, 'createServeApp').mockImplementation((_opts, _getPort, deps) => {
            ...
            primaryRuntimeEnv = deps?.primaryRuntimeEnv;
    ...
    it('preserves previous runtime env and marks fallback when reload env rebuild fails', async () => {
        tmpDir = fs.realpathSync(fs.mkdtempSync(path.join(os.tmpdir(), 'qws-runtime-env-fallback-')));
        vi.spyOn(qwenCore, 'resolveTelemetrySettings').mockResolvedValue({
            enabled: false,
            sensitiveSpanAttributeMaxLength: 1024 * 1024,
        });
        ...
        const buildRuntimeEnvironmentActual = environmentRuntime.buildRuntimeEnvironment;
        let failReloadBuild = false;
        vi.spyOn(environmentRuntime, 'buildRuntimeEnvironment').mockImplementation((...args) => {
            if (failReloadBuild) {
        ...
        try {
            ...
            expect(primaryRuntimeEnv.effectiveEnv).toBe(capturedRuntimeEnv);
            expect(capturedRuntimeEnv['QWEN_TEST_RUNTIME_VALUE']).toBe('boot');
            expect(primaryRuntimeEnv.fallbackReason).toBe('runtime env rebuild failed');
            ...
            expect(log).toContain('failed to rebuild runtime env snapshot after daemon env reload; preserving previous runtime env');
        }
        finally {
            if (!closed) {
                await handle.close();
    ...
    it('updates secondary runtime env metadata in place after workspace reload', async () => {
        tmpDir = fs.realpathSync(fs.mkdtempSync(path.join(os.tmpdir(), 'qws-secondary-env-reload-')));
        const primary = path.join(tmpDir, 'primary');
        const secondary = path.join(tmpDir, 'secondary');
        const originalRuntimeDir = process.env['QWEN_RUNTIME_DIR'];
        delete process.env['QWEN_RUNTIME_DIR'];
        ...
        let runtimeMounted = false;
        vi.spyOn(settingsRuntime, 'loadSettings').mockImplementation((...args) => {
        ...
        try {
            ...
            const pinnedRuntimeBaseDir = path.join(secondary, '.secondary-runtime-boot');
            expect(secondaryRuntime.sessionRuntimeBaseDir).toBe(pinnedRuntimeBaseDir);
    ...
    it('restores persisted workspaces through the normal secondary runtime path', async () => {
        tmpDir = fs.realpathSync(fs.mkdtempSync(path.join(os.tmpdir(), 'qws-restored-workspace-')));
        const primary = path.join(tmpDir, 'primary');
        ...
        const reservedConversationChild = path.join(liveConversationWorkspace.rootPath, 'legacy-child');
        const missingReservedConversationChild = path.join(liveConversationWorkspace.rootPath, 'missing-legacy-child');
        fs.mkdirSync(primary);
        ...
        const restoredSecondaryAlias = path.join(tmpDir, 'restored-secondary-alias');
        fs.symlinkSync(restoredSecondary, restoredSecondaryAlias, process.platform === 'win32' ? 'junction' : 'dir');
        ...
        const createBridge = vi
            ...
            .mockImplementation(() => makeRuntimeBridge());
        let restoredCwds = [];
        let restoredDisplayNames = [];
        let restoredRemovable = [];
        let restoredRegistrationIds = [];
        let advertisedMaxTotalSessions;
        vi.spyOn(serverModule, 'createServeApp').mockImplementation((opts, _getPort, deps) => {
            restoredCwds =
        ...
        const store = {
            read: vi.fn().mockResolvedValue({
                ...
                displayNames: {
                    [workspaceRegistrationId(canonicalExplicitSecondary)]: 'Explicit workspace',
                    [workspaceRegistrationId(restoredSecondaryAlias)]: 'Restored workspace',
                    [workspaceRegistrationId(canonicalRestoredSecondary)]: 'Later alias name',
                },
            }),
        };
        const handle = await runQwenServe({
        ...
        try {
            ...
            expect(stderrWrite.mock.calls.some(([message]) => String(message).includes(`skipping persisted workspace registration ${JSON.stringify(missingPersistedWorkspace)}`))).toBe(true);
            expect(stderrWrite.mock.calls.some(([message]) => String(message).includes('path nests with an explicit'))).toBe(true);
            expect(stderrWrite.mock.calls.filter(([message]) => String(message).includes('path is reserved for Conversations'))).toHaveLength(3);
        }
        finally {
            await handle.close();
        }
    ...
    it('continues with explicit workspaces when the registration store read fails', async () => {
        tmpDir = fs.realpathSync(fs.mkdtempSync(path.join(os.tmpdir(), 'qws-restored-read-error-')));
        const primary = path.join(tmpDir, 'primary');
        fs.mkdirSync(primary);
        const canonicalPrimary = canonicalizeWorkspace(primary);
        const stderrWrite = vi
        ...
        try {
            ...
            expect(stderrWrite.mock.calls.some(([message]) => String(message).includes('failed to read persisted workspace registrations'))).toBe(true);
        }
        finally {
            await handle.close();
        }
    ...
    it('skips persisted workspaces after the runtime limit is reached', async () => {
        tmpDir = fs.realpathSync(fs.mkdtempSync(path.join(os.tmpdir(), 'qws-restored-limit-')));
        ...
        const canonicalExplicit = explicitWorkspaces.map((workspace) => canonicalizeWorkspace(workspace));
        ...
        try {
            ...
            expect(stderrWrite.mock.calls.some(([message]) => String(message).includes('workspace limit reached'))).toBe(true);
        }
        finally {
            await handle.close();
        }
    ...
    it('filters secondary workspace roots before constructing the bridge filesystem', async () => {
        tmpDir = fs.realpathSync(fs.mkdtempSync(path.join(os.tmpdir(), 'qws-runtime-roots-')));
        const primary = path.join(tmpDir, 'primary');
        ...
        const roots = [primary, trustedSecondary, untrustedSecondary].map((root) => canonicalizeWorkspace(root));
        const bridgeFsBoundWorkspaces = [];
        vi.spyOn(qwenCore, 'resolveTelemetrySettings').mockResolvedValue({
            enabled: false,
            sensitiveSpanAttributeMaxLength: 1024 * 1024,
        });
        ...
        vi.spyOn(trustPolicyRuntime, 'evaluateDaemonWorkspaceTrust').mockImplementation((_snapshot, workspace) => ({
            state: workspace === roots[2] ? 'untrusted' : 'trusted',
            targetTrusted: workspace !== roots[2],
            source: 'file',
            explicitTrustLevel: null,
        }));
        vi.spyOn(serverModule, 'resolveBoundWorkspacesFromIdeEnv').mockImplementation((_primary, _ide, includeWorkspace) => includeWorkspace === undefined ? roots : roots.filter(includeWorkspace));
        vi.spyOn(serverModule, 'resolveBridgeFsFactory').mockImplementation((input) => {
    ...
    it('keeps trusted child roots when an untrusted parent is filtered out', async () => {
        ...
        const roots = [primary, untrustedParent, trustedChild].map((root) => canonicalizeWorkspace(root));
        const originalIdeWorkspacePath = process.env['QWEN_CODE_IDE_WORKSPACE_PATH'];
    ...
    it('shares one path lock registry across bridge and REST filesystem factories', async () => {
        ...
        fs.mkdirSync(primary);
        fs.mkdirSync(secondary);
        const roots = [primary, secondary].map((root) => canonicalizeWorkspace(root));
        const pathLocks = [];
        vi.spyOn(qwenCore, 'resolveTelemetrySettings').mockResolvedValue({
            enabled: false,
            sensitiveSpanAttributeMaxLength: 1024 * 1024,
    ...
    it('excludes secondary workspace roots when runtime trust settings are unavailable', async () => {
        ...
        try {
            ...
            expect(trustedFoldersRuntime.getWorkspaceTrustStatus).not.toHaveBeenCalled();
        }
        finally {
            await handle.close();
        }
    ...
    it('keeps browser MCP features disabled for non-extension origins when the env flag is unset', async () => {
        const features = await readBrowserMcpFeatureFlagsForEnv(undefined, 'https://example.com');
        expect(features).not.toContain('client_mcp_over_ws');
        expect(features).not.toContain('cdp_tunnel_over_ws');
    });
    it('bounds shutdown waiting when runtime startup never settles', async () => {
        const daemonLog = { warn: vi.fn() };
        await expect(waitForRuntimeStartingForShutdown(new Promise(() => { }), daemonLog, 1)).resolves.toBeUndefined();
        expect(daemonLog.warn).toHaveBeenCalledWith('1ms runtime-startup wait reached during shutdown; continuing listener close');
    });
    it('proxies bridge access only after the runtime bridge is ready', async () => {
        const holder = {};
        let runtimeStartupError;
        const proxy = createLazyBridgeProxy(() => holder.bridge, () => runtimeStartupError);
        expect(() => proxy.getDaemonStatusSnapshot()).toThrow('Daemon bridge runtime is still starting.');
        runtimeStartupError = 'runtime boom';
        expect(() => proxy.getDaemonStatusSnapshot()).toThrow('Daemon bridge runtime is not available: runtime boom');
        const getDaemonStatusSnapshot = vi.fn(function () {
        ...
        holder.bridge = { getDaemonStatusSnapshot };
    ...
    ])('resolves QWEN_SERVE_RUNTIME_STARTUP_TIMEOUT_MS=%s to %s', (envValue, expected) => {
    ...
    it('returns bootstrap 503 for unknown routes while runtime is still starting', async () => {
        tmpDir = fs.realpathSync(fs.mkdtempSync(path.join(os.tmpdir(), 'qws-runtime-starting-route-')));
        let resolveTelemetry;
        const telemetryPromise = new Promise((resolve) => {
            resolveTelemetry = resolve;
        });
        vi.spyOn(qwenCore, 'resolveTelemetrySettings').mockReturnValue(telemetryPromise);
        const bridge = makeRuntimeBridge();
        vi.spyOn(acpBridge, 'createAcpSessionBridge').mockReturnValue(bridge);
        const handle = await runQwenServe({
            port: 0,
    ...
    it('returns bootstrap 503 for multi-workspace capabilities until runtime routes mount', async () => {
        tmpDir = fs.realpathSync(fs.mkdtempSync(path.join(os.tmpdir(), 'qws-runtime-starting-caps-')));
        const primary = path.join(tmpDir, 'primary');
        const secondary = path.join(tmpDir, 'secondary');
        fs.mkdirSync(primary);
        fs.mkdirSync(secondary);
    ...
    it('keeps health responsive before starting deferred runtime work', async () => {
        tmpDir = fs.realpathSync(fs.mkdtempSync(path.join(os.tmpdir(), 'qws-health-first-')));
        const logBaseDir = path.join(tmpDir, 'debug');
        ...
        try {
            ...
            expect(logContent).toContain('deferred runtime: health timer fired, starting');
        }
        finally {
            if (!closed) {
                await handle.close();
    ...
    it('returns retryable bootstrap deep health while starting deferred runtime', async () => {
        tmpDir = fs.realpathSync(fs.mkdtempSync(path.join(os.tmpdir(), 'qws-health-deep-first-')));
        vi.spyOn(qwenCore, 'resolveTelemetrySettings').mockResolvedValue({
            enabled: false,
            sensitiveSpanAttributeMaxLength: 1024 * 1024,
        });
    ...
    it('starts deferred runtime once for duplicate health probes', async () => {
        tmpDir = fs.realpathSync(fs.mkdtempSync(path.join(os.tmpdir(), 'qws-health-dedupe-')));
        vi.spyOn(qwenCore, 'resolveTelemetrySettings').mockResolvedValue({
            enabled: false,
            sensitiveSpanAttributeMaxLength: 1024 * 1024,
        });
    ...
    it('starts deferred runtime for the first runtime route and serves that request', async () => {
        tmpDir = fs.realpathSync(fs.mkdtempSync(path.join(os.tmpdir(), 'qws-runtime-route-start-')));
        let resolveTelemetry;
        const telemetryPromise = new Promise((resolve) => {
            resolveTelemetry = resolve;
        });
        ...
        let resolveSecondSessionRequest;
        const secondSessionRequest = new Promise((resolve) => {
            resolveSecondSessionRequest = resolve;
        });
        const observeSessionRequest = (req) => {
            if (req.method !== 'POST' || req.url !== '/session')
                return;
            sessionRequestCount += 1;
            if (sessionRequestCount === 2)
                resolveSecondSessionRequest?.();
        };
        handle.server.on('request', observeSessionRequest);
        try {
            expect(createBridge).not.toHaveBeenCalled();
            const firstResponse = fetch(`${handle.url}/session`, { method: 'POST' });
            await vi.waitFor(() => expect(resolveTelemetrySettings).toHaveBeenCalledOnce());
    ...
    it('rejects unauthenticated deferred runtime routes before starting runtime', async () => {
        tmpDir = fs.realpathSync(fs.mkdtempSync(path.join(os.tmpdir(), 'qws-runtime-route-auth-')));
        const bridge = makeRuntimeBridge();
        const createBridge = vi
            .spyOn(acpBridge, 'createAcpSessionBridge')
            .mockReturnValue(bridge);
    ...
    it('serves Web Shell document navigations during the deferred runtime window', async () => {
        tmpDir = fs.realpathSync(fs.mkdtempSync(path.join(os.tmpdir(), 'qws-deferred-webshell-')));
    ...
    it('serves the Web Shell root during the deferred runtime window', async () => {
        tmpDir = fs.realpathSync(fs.mkdtempSync(path.join(os.tmpdir(), 'qws-deferred-root-')));
        writeWebShellFixture(tmpDir);
        const { handle, createBridge } = await startDeferredDaemon(tmpDir);
        try {
    ...
    it('serves the // root alias during the deferred window like the warm app', async () => {
        tmpDir = fs.realpathSync(fs.mkdtempSync(path.join(os.tmpdir(), 'qws-deferred-root-alias-')));
        writeWebShellFixture(tmpDir);
        const { handle, createBridge } = await startDeferredDaemon(tmpDir);
        try {
    ...
    it('degrades to the bearer gate when the pre-auth predicate rejects', async () => {
        tmpDir = fs.realpathSync(fs.mkdtempSync(path.join(os.tmpdir(), 'qws-deferred-predicate-fail-')));
        writeWebShellFixture(tmpDir);
        const { handle, createBridge } = await startDeferredDaemon(tmpDir);
    ...
    it('serves Web Shell assets during the deferred runtime window', async () => {
        tmpDir = fs.realpathSync(fs.mkdtempSync(path.join(os.tmpdir(), 'qws-deferred-assets-')));
        const shellDir = writeWebShellFixture(tmpDir);
        fs.writeFileSync(path.join(shellDir, 'assets', 'fixture.js'), 'console.log("fixture");');
        const { handle, createBridge } = await startDeferredDaemon(tmpDir);
        try {
    ...
    it('answers bare /assets during the deferred window like the warm app', async () => {
        tmpDir = fs.realpathSync(fs.mkdtempSync(path.join(os.tmpdir(), 'qws-deferred-assets-bare-')));
        writeWebShellFixture(tmpDir);
        const { handle, createBridge } = await startDeferredDaemon(tmpDir);
        try {
    ...
    it('serves trailing-slash and case-variant session deep links during the deferred window', async () => {
        tmpDir = fs.realpathSync(fs.mkdtempSync(path.join(os.tmpdir(), 'qws-deferred-shapes-')));
        writeWebShellFixture(tmpDir);
        const { handle, createBridge } = await startDeferredDaemon(tmpDir);
        try {
    ...
    it('serves query-carrying session deep links during the deferred window', async () => {
        tmpDir = fs.realpathSync(fs.mkdtempSync(path.join(os.tmpdir(), 'qws-deferred-query-')));
        writeWebShellFixture(tmpDir);
        const { handle, createBridge } = await startDeferredDaemon(tmpDir);
        try {
    ...
    it('keeps JSON and API-subpath requests gated during the deferred runtime window', async () => {
        tmpDir = fs.realpathSync(fs.mkdtempSync(path.join(os.tmpdir(), 'qws-deferred-webshell-gate-')));
        writeWebShellFixture(tmpDir);
        const { handle, createBridge } = await startDeferredDaemon(tmpDir);
        try {
    ...
    it('keeps session document navigations gated during the deferred window with --no-web', async () => {
        tmpDir = fs.realpathSync(fs.mkdtempSync(path.join(os.tmpdir(), 'qws-deferred-noweb-')));
        const { handle, createBridge } = await startDeferredDaemon(tmpDir, {
    ...
    it('answers pre-auth Web Shell navigations with the failure envelope when the deferred runtime fails', async () => {
        tmpDir = fs.realpathSync(fs.mkdtempSync(path.join(os.tmpdir(), 'qws-deferred-webshell-fail-')));
        writeWebShellFixture(tmpDir);
        vi.spyOn(qwenCore, 'resolveTelemetrySettings').mockResolvedValue({
            enabled: false,
            sensitiveSpanAttributeMaxLength: 1024 * 1024,
    ...
    it('starts deferred runtime for webhook routes without bearer auth', async () => {
        tmpDir = fs.realpathSync(fs.mkdtempSync(path.join(os.tmpdir(), 'qws-runtime-webhook-start-')));
        const previousQwenHome = process.env['QWEN_HOME'];
        const previousWebhookSecret = process.env['QWEN_DEFERRED_WEBHOOK_SECRET'];
        const tempHome = fs.mkdtempSync(path.join(os.tmpdir(), 'qws-runtime-webhook-home-'));
        ...
        vi.spyOn(environmentRuntime, 'buildRuntimeEnvironment').mockImplementation((_settings, _workspace, baseEnv) => ({
        ...
        try {
            expect(createBridge).not.toHaveBeenCalled();
            const res = await fetch(`${handle.url}/channels/dingtalk-main/webhooks/github%20ci`, {
                method: 'POST',
                headers: {
                    'content-type': 'application/json',
    ...
    it('rejects bad-secret deferred webhook routes before starting runtime', async () => {
        tmpDir = fs.realpathSync(fs.mkdtempSync(path.join(os.tmpdir(), 'qws-runtime-webhook-auth-')));
        const logBaseDir = path.join(tmpDir, 'debug');
        const previousQwenHome = process.env['QWEN_HOME'];
        const tempHome = fs.mkdtempSync(path.join(os.tmpdir(), 'qws-runtime-webhook-home-'));
        process.env['QWEN_HOME'] = tempHome;
        ...
        let closed = false;
        try {
            const res = await fetch(`${handle.url}/channels/dingtalk-main/webhooks/github-ci`, {
                method: 'POST',
                headers: {
                    'content-type': 'application/json',
            ...
            await handle.close();
            closed = true;
            const log = fs.readFileSync(path.join(logBaseDir, 'daemon', 'daemon.log'), 'utf8');
    ...
    it('logs deferred webhook secret lookup failures before starting runtime', async () => {
        tmpDir = fs.realpathSync(fs.mkdtempSync(path.join(os.tmpdir(), 'qws-runtime-webhook-log-')));
        const previousQwenHome = process.env['QWEN_HOME'];
        ...
        try {
            const res = await fetch(`${handle.url}/channels/dingtalk-main/webhooks/github%0Aci`, {
                method: 'POST',
                headers: {
                    'content-type': 'application/json',
            ...
            expect(await res.json()).toEqual({ error: 'Invalid webhook secret' });
            expect(createBridge).not.toHaveBeenCalled();
            expect(stderrWrites.join('')).toContain('[webhook-secret] failed to read deferred webhook secret for dingtalk-main/github\\nci:');
            expect(stderrWrites.join('')).not.toContain('github\nci');
            expect(stderrWrites.join('')).toContain('webhooks.sources.github\\nci.secretEnv');
        }
        finally {
            await handle.close();
            fs.rmSync(tempHome, { recursive: true, force: true });
    ...
    it('allows deferred runtime CORS preflight without auth or runtime startup', async () => {
        tmpDir = fs.realpathSync(fs.mkdtempSync(path.join(os.tmpdir(), 'qws-runtime-preflight-')));
        const bridge = makeRuntimeBridge();
        const createBridge = vi
            .spyOn(acpBridge, 'createAcpSessionBridge')
            .mockReturnValue(bridge);
    ...
    it('does not start deferred runtime for unsupported bootstrap route methods', async () => {
        tmpDir = fs.realpathSync(fs.mkdtempSync(path.join(os.tmpdir(), 'qws-bootstrap-method-')));
        const bridge = makeRuntimeBridge();
        const createBridge = vi
            .spyOn(acpBridge, 'createAcpSessionBridge')
            .mockReturnValue(bridge);
    ...
    it('serves trailing-slash bootstrap health without waiting for deferred runtime', async () => {
        tmpDir = fs.realpathSync(fs.mkdtempSync(path.join(os.tmpdir(), 'qws-bootstrap-trailing-')));
        let resolveTelemetry;
        const telemetryPromise = new Promise((resolve) => {
            resolveTelemetry = resolve;
        });
        ...
        try {
            const res = await Promise.race([
                fetch(`${handle.url}/health/`),
                new Promise((_, reject) => setTimeout(() => reject(new Error('Trailing-slash health timed out')), 200)),
            ]);
            expect(res.status).toBe(200);
    ...
    it('reports deferred runtime startup failure for the triggering runtime route', async () => {
        tmpDir = fs.realpathSync(fs.mkdtempSync(path.join(os.tmpdir(), 'qws-runtime-route-fail-')));
        vi.spyOn(qwenCore, 'resolveTelemetrySettings').mockResolvedValue({
            enabled: false,
            sensitiveSpanAttributeMaxLength: 1024 * 1024,
        });
    ...
    it('starts deferred runtime on fallback when no health probe arrives', async () => {
        tmpDir = fs.realpathSync(fs.mkdtempSync(path.join(os.tmpdir(), 'qws-health-fallback-')));
        const bridge = makeRuntimeBridge();
        const createBridge = vi
            .spyOn(acpBridge, 'createAcpSessionBridge')
            .mockReturnValue(bridge);
    ...
    it('does not start deferred runtime after close before first health', async () => {
        tmpDir = fs.realpathSync(fs.mkdtempSync(path.join(os.tmpdir(), 'qws-health-close-')));
        const logBaseDir = path.join(tmpDir, 'debug');
        const bridge = makeRuntimeBridge();
        const createBridge = vi
            .spyOn(acpBridge, 'createAcpSessionBridge')
        ...
        expect(createBridge).not.toHaveBeenCalled();
        await expect(handle.runtimeReady).rejects.toThrow(RUNTIME_STARTUP_CANCELLED_MESSAGE);
        const daemonDir = path.join(logBaseDir, 'daemon');
        const [logFile] = fs
            .readdirSync(daemonDir)
            .filter((fileName) => fileName.endsWith('.log'));
        expect(logFile).toBeDefined();
        const logContent = fs.readFileSync(path.join(daemonDir, logFile), 'utf8');
        expect(logContent).toContain('deferred runtime: cancelled, server closed before startup');
    });
    it('does not start deferred runtime after close following first health', async () => {
        tmpDir = fs.realpathSync(fs.mkdtempSync(path.join(os.tmpdir(), 'qws-health-close-after-')));
        vi.spyOn(qwenCore, 'resolveTelemetrySettings').mockResolvedValue({
            enabled: false,
            sensitiveSpanAttributeMaxLength: 1024 * 1024,
        });
    ...
    it('stops the deferred runtime extension reconciler during close', async () => {
        tmpDir = fs.realpathSync(fs.mkdtempSync(path.join(os.tmpdir(), 'qws-health-reconciler-close-')));
        vi.spyOn(qwenCore, 'resolveTelemetrySettings').mockResolvedValue({
            enabled: false,
            sensitiveSpanAttributeMaxLength: 1024 * 1024,
        });
        ...
        expect(stopExtensionGenerationReconciler.mock.invocationCallOrder[0]).toBeLessThan(vi.mocked(bridge.shutdown).mock.invocationCallOrder[0]);
    });
    it('seals and drains admitted session maintenance before bridge shutdown', async () => {
        tmpDir = fs.realpathSync(fs.mkdtempSync(path.join(os.tmpdir(), 'qws-maintenance-drain-')));
        vi.spyOn(qwenCore, 'resolveTelemetrySettings').mockResolvedValue({
            enabled: false,
            sensitiveSpanAttributeMaxLength: 1024 * 1024,
        });
    ...
    it('propagates an admitted session maintenance drain failure', async () => {
        tmpDir = fs.realpathSync(fs.mkdtempSync(path.join(os.tmpdir(), 'qws-maintenance-failure-')));
        vi.spyOn(qwenCore, 'resolveTelemetrySettings').mockResolvedValue({
            enabled: false,
            sensitiveSpanAttributeMaxLength: 1024 * 1024,
        });
    ...
    it('does not cancel deferred runtime once startup is already running', async () => {
        tmpDir = fs.realpathSync(fs.mkdtempSync(path.join(os.tmpdir(), 'qws-health-close-running-')));
        let resolveTelemetry;
        const telemetryPromise = new Promise((resolve) => {
            resolveTelemetry = resolve;
        });
        ...
        expect(healthRes.status).toBe(200);
        expect(await healthRes.json()).toEqual({ status: 'ok' });
        await vi.waitFor(() => expect(resolveTelemetrySettings).toHaveBeenCalledTimes(1), { timeout: 500 });
        ...
        await closePromise;
        expect(createBridge).toHaveBeenCalledTimes(1);
        await expect(handle.runtimeReady).rejects.toThrow('Daemon runtime stopped before mounting.');
    });
    it('disposes a deferred runtime app that finishes after the shutdown wait', async () => {
        tmpDir = fs.realpathSync(fs.mkdtempSync(path.join(os.tmpdir(), 'qws-health-close-late-app-')));
        let resolveTelemetry;
        const telemetryPromise = new Promise((resolve) => {
            resolveTelemetry = resolve;
        });
        ...
        await vi.waitFor(() => expect(stopExtensionGenerationReconciler).toHaveBeenCalledOnce(), { timeout: 1_000 });
    ...
    it('does not retry deferred runtime after startup failure and later health probe', async () => {
        tmpDir = fs.realpathSync(fs.mkdtempSync(path.join(os.tmpdir(), 'qws-health-fail-once-')));
        vi.spyOn(qwenCore, 'resolveTelemetrySettings').mockResolvedValue({
            enabled: false,
            sensitiveSpanAttributeMaxLength: 1024 * 1024,
        });
    ...
    it('flushes runtime startup failures to the daemon log when closing', async () => {
        tmpDir = fs.realpathSync(fs.mkdtempSync(path.join(os.tmpdir(), 'qws-runtime-fail-log-')));
        const originalRuntimeDir = process.env['QWEN_RUNTIME_DIR'];
        process.env['QWEN_RUNTIME_DIR'] = tmpDir;
        vi.spyOn(acpBridge, 'createAcpSessionBridge').mockImplementation(() => {
            throw new Error('runtime boom');
    ...
    it('does not block shutdown on pending metrics flush', async () => {
        tmpDir = fs.realpathSync(fs.mkdtempSync(path.join(os.tmpdir(), 'qws-runtime-flush-pending-')));
        const forceFlushMetrics = vi.spyOn(qwenCore, 'forceFlushMetrics');
        forceFlushMetrics.mockReturnValue(new Promise(() => { }));
        const bridge = {
            spawnOrAttach: vi.fn(),
            shutdown: vi.fn().mockResolvedValue(undefined),
            killAllSync: vi.fn(),
    ...
    it('accumulates prompt queue wait stats in daemon status perf data', async () => {
        tmpDir = fs.realpathSync(fs.mkdtempSync(path.join(os.tmpdir(), 'qws-runtime-queue-wait-')));
        vi.spyOn(qwenCore, 'resolveTelemetrySettings').mockResolvedValue({
            enabled: false,
            sensitiveSpanAttributeMaxLength: 1024 * 1024,
        });
        ...
        vi.spyOn(qwenCore, 'createDaemonBridgeTelemetry').mockReturnValue(telemetry);
        const recordPromptQueueWait = vi.spyOn(qwenCore, 'recordDaemonPromptQueueWait');
        const bridge = makeRuntimeBridge();
        vi.spyOn(acpBridge, 'createAcpSessionBridge').mockReturnValue(bridge);
        const handle = await runQwenServe({
            port: 0,
    ...
    it('fails runtimeReady and health when runtime startup times out', async () => {
        tmpDir = fs.realpathSync(fs.mkdtempSync(path.join(os.tmpdir(), 'qws-runtime-timeout-')));
        let resolveTelemetry;
        const telemetryPromise = new Promise((resolve) => {
            resolveTelemetry = resolve;
        });
        ...
        try {
            await expect(handle.runtimeReady).rejects.toThrow('Daemon runtime startup timed out after 1ms.');
            const healthRes = await fetch(`${handle.url}/health`);
            ...
            expect(() => handle.bridge.getDaemonStatusSnapshot()).toThrow('Daemon bridge runtime is not available: Daemon runtime startup timed out after 1ms.');
            resolveTelemetry?.({
                enabled: false,
                sensitiveSpanAttributeMaxLength: 1024 * 1024,
            });
    ...
    it('reports bootstrap status and capabilities when fast path resolves on listen', async () => {
        ...
        try {
            ...
            const sameOriginRes = await fetch(`${handle.url}/daemon/status?detail=full`, {
                headers: { Origin: handle.url },
            });
            expect(sameOriginRes.status).toBe(200);
    ...
    it('reports the journal growth pool on bootstrap daemon status', async () => {
        ...
        const constrainedSpy = vi
            .spyOn(process, 'constrainedMemory')
            .mockReturnValue(0);
        tmpDir = fs.realpathSync(fs.mkdtempSync(path.join(os.tmpdir(), 'qws-bootstrap-growth-')));
        vi.spyOn(acpBridge, 'createAcpSessionBridge').mockImplementation(() => {
            throw new Error('runtime boom');
        });
        try {
            ...
            try {
                ...
                expect(body.limits.memory.journalGrowth).toEqual({
                    poolBytes: journalGrowthPoolMb(resolveDaemonMemoryBudget({ availableMemoryMb: 8 * 1024 })) *
                        1024 *
                        1024,
    ...
    it('shuts down a bridge when runtime mounting fails after bridge creation', async () => {
        tmpDir = fs.realpathSync(fs.mkdtempSync(path.join(os.tmpdir(), 'qws-runtime-partial-fail-')));
        const bridge = {
            spawnOrAttach: vi.fn(),
            shutdown: vi.fn().mockResolvedValue(undefined),
            killAllSync: vi.fn(),
    ...
    it('cleans up runtime locals when closed immediately after listening', async () => {
        tmpDir = fs.realpathSync(fs.mkdtempSync(path.join(os.tmpdir(), 'qws-runtime-close-')));
        const bridge = {
            spawnOrAttach: vi.fn(),
            shutdown: vi.fn().mockResolvedValue(undefined),
            killAllSync: vi.fn(),
    ...
    it('disposes the daemon event loop monitor when closed after listening', async () => {
        tmpDir = fs.realpathSync(fs.mkdtempSync(path.join(os.tmpdir(), 'qws-runtime-monitor-close-')));
        const bridge = {
            spawnOrAttach: vi.fn(),
            shutdown: vi.fn().mockResolvedValue(undefined),
            killAllSync: vi.fn(),
...
describe('runQwenServe Web Shell signals on RunHandle', () => {
    ...
    it('passes --experimental-lsp to spawned ACP children only when opted in', async () => {
        ...
        expect(mockCreateSpawnChannelFactoryOptions.at(-1)).not.toHaveProperty('extraArgs');
    ...
    it('scrubs loader env vars from the daemon process and the session-child base env', async () => {
        ...
        try {
            ...
            try {
                ...
                expect(stderrWrites.join('')).toContain('scrubbed inherited loader env vars');
                expect(stderrWrites.join('')).toContain('NODE_OPTIONS');
                expect(stderrWrites.join('')).toContain('NODE_PATH');
                const sourceEnv = mockCreateSpawnChannelFactoryOptions.at(-1)?.['sourceEnv'];
            ...
            expect(process.env['NODE_OPTIONS']).toBe('--import file:///other-checkout/register.mjs');
    ...
    it('keeps loader vars in the session-child base env under the dev harness', async () => {
        ...
        try {
            ...
            try {
                const sourceEnv = mockCreateSpawnChannelFactoryOptions.at(-1)?.['sourceEnv'];
                expect(sourceEnv?.['NODE_OPTIONS']).toBe('--import file:///other-checkout/register.mjs');
            }
            finally {
                await handle.close();
            }
    ...
    it('persists serve fast-path loader key rejections in the daemon log', async () => {
        ...
        tmpDir = fs.realpathSync(fs.mkdtempSync(path.join(os.tmpdir(), 'qws-ws-')));
        process.env['QWEN_RUNTIME_DIR'] = tmpDir;
        fs.writeFileSync(path.join(tmpDir, '.env'), 'NODE_OPTIONS=--max-old-space-size=8192\n');
        const stderrWrite = vi
            .spyOn(process.stderr, 'write')
            .mockImplementation(() => true);
        try {
            ...
            try {
                ...
                await vi.waitFor(() => {
                    const content = fs.readFileSync(logPath, 'utf8');
                    expect(content).toContain('rejected loader-affecting env keys during serve fast-path boot');
                    expect(content).toContain('NODE_OPTIONS');
                }, { timeout: 7_000, interval: 50 });
            }
            finally {
    ...
    it('persists post-boot loader key rejections in the daemon log', async () => {
        ...
        try {
            ...
            try {
                ...
                await vi.waitFor(() => {
                    const content = fs.readFileSync(logPath, 'utf8');
                    expect(content).toContain('rejected loader-affecting env keys; they were not applied');
                    expect(content).toContain('NODE_OPTIONS');
                }, { timeout: 7_000, interval: 50 });
            }
            finally {
    ...
    it('falls back to stderr for loader key rejections after close', async () => {
        ...
        try {
            ...
            const warnings = stderrWrites.filter((chunk) => chunk.includes('cannot set loader-affecting env vars') &&
    ...
    it('restores the launch environment when startup fails after the scrub', async () => {
        ...
        try {
            await expect(bootHandle({ serveWebShell: false })).rejects.toThrow(/QWEN_SERVE_PROMPT_DEADLINE_MS/u);
            expect(process.env['NODE_OPTIONS']).toBe('--import file:///other-checkout/register.mjs');
        }
        finally {
            if (previousNodeOptions === undefined) {
    ...
    it('wires the pipe message observer without changing existing pipe stats', async () => {
        ...
        try {
            await handle.runtimeReady;
            const pipeHooks = mockCreateSpawnChannelFactoryOptions.at(-1)?.['pipeHooks'];
...
describe('runQwenServe channel worker supervisor', () => {
    ...
    function makeFakeBridge(onShutdown) {
        return {
            spawnOrAttach: vi.fn(),
    ...
    function makeReadyWorkerFactory(worker) {
        return vi.fn((opts) => {
    ...
    it('rejects webhook tasks when the channel worker is disabled', async () => {
        ...
        await expect(supervisor.enqueueWebhookTask({
    ...
    it('forwards webhook tasks through the channel worker group', async () => {
        tmpDir = fs.realpathSync(fs.mkdtempSync(path.join(os.tmpdir(), 'qws-channel-webhook-')));
        ...
        try {
            await handle.runtimeReady;
            expect(capturedDeps?.enqueueChannelWebhookTask).toEqual(expect.any(Function));
            await expect(capturedDeps.enqueueChannelWebhookTask(task)).resolves.toEqual({ accepted: true });
            ...
            await expect(capturedDeps.deliverChannelMessage(tmpDir, delivery)).resolves.toEqual({ delivered: true });
    ...
    it('keeps webhook enqueue available when no worker is selected', async () => {
        tmpDir = fs.realpathSync(fs.mkdtempSync(path.join(os.tmpdir(), 'qws-channel-webhook-disabled-')));
        const originalCreateServeApp = serverModule.createServeApp;
        let capturedDeps;
        vi.spyOn(serverModule, 'createServeApp').mockImplementation((...args) => {
            capturedDeps = args[2];
        ...
        try {
            await handle.runtimeReady;
            expect(capturedDeps?.enqueueChannelWebhookTask).toEqual(expect.any(Function));
            await expect(capturedDeps.enqueueChannelWebhookTask({
                channelName: 'telegram',
                source: 'github-ci',
                eventType: 'check_failed',
                targetRef: 'default',
            ...
            await expect(capturedDeps.deliverChannelMessage(tmpDir, {
                deliveryId: 'delivery-1',
                channelName: 'telegram',
                target: { type: 'user', id: 'user-1' },
                text: 'CI failed',
    ...
    it('enables, queries, idempotently reapplies, and stops channels after boot', async () => {
        tmpDir = fs.realpathSync(fs.mkdtempSync(path.join(os.tmpdir(), 'qws-channel-runtime-control-')));
        fs.mkdirSync(path.join(tmpDir, '.qwen'));
        fs.writeFileSync(path.join(tmpDir, '.qwen', 'settings.json'), JSON.stringify({ channels: { telegram: { type: 'telegram' } } }));
        const worker = makeWorker({
            enabled: true,
            state: 'running',
            pid: 1234,
        ...
        try {
            ...
            const stoppedFeatures = (await stoppedCaps.json());
    ...
    it('single-flights concurrent first PUTs through one manager and worker', async () => {
        tmpDir = fs.realpathSync(fs.mkdtempSync(path.join(os.tmpdir(), 'qws-channel-runtime-race-')));
        fs.mkdirSync(path.join(tmpDir, '.qwen'));
        fs.writeFileSync(path.join(tmpDir, '.qwen', 'settings.json'), JSON.stringify({ channels: { telegram: { type: 'telegram' } } }));
        const worker = makeWorker({
            enabled: true,
    ...
    it('orders DELETE behind a first PUT that is still creating the manager', async () => {
        tmpDir = fs.realpathSync(fs.mkdtempSync(path.join(os.tmpdir(), 'qws-channel-runtime-fifo-')));
        fs.mkdirSync(path.join(tmpDir, '.qwen'));
        fs.writeFileSync(path.join(tmpDir, '.qwen', 'settings.json'), JSON.stringify({ channels: { telegram: { type: 'telegram' } } }));
        let capturedDeps;
        const originalCreateServeApp = serverModule.createServeApp;
    ...
    it('rejects channel mutations once shutdown starts before manager creation', async () => {
        tmpDir = fs.realpathSync(fs.mkdtempSync(path.join(os.tmpdir(), 'qws-channel-runtime-drain-')));
        let capturedDeps;
        const originalCreateServeApp = serverModule.createServeApp;
        vi.spyOn(serverModule, 'createServeApp').mockImplementation((...args) => {
            capturedDeps = args[2];
        ...
        const workerFactory = vi.fn(() => makeWorker({
            enabled: true,
            state: 'running',
            channels: ['telegram'],
        }));
        ...
        await expect(capturedDeps.setChannelWorkerSelection({ mode: 'all' })).rejects.toMatchObject({ code: 'daemon_draining' });
    ...
    it('rejects an unknown runtime selection before reserving or starting', async () => {
        tmpDir = fs.realpathSync(fs.mkdtempSync(path.join(os.tmpdir(), 'qws-channel-runtime-unknown-')));
        const workerFactory = vi.fn(() => makeWorker({
            enabled: true,
            state: 'running',
            channels: ['missing'],
    ...
    it('reports a standalone service conflict on the first runtime PUT', async () => {
        tmpDir = fs.realpathSync(fs.mkdtempSync(path.join(os.tmpdir(), 'qws-channel-runtime-conflict-')));
        fs.mkdirSync(path.join(tmpDir, '.qwen'));
        fs.writeFileSync(path.join(tmpDir, '.qwen', 'settings.json'), JSON.stringify({ channels: { telegram: { type: 'telegram' } } }));
        const workerFactory = vi.fn(() => makeWorker({
            enabled: true,
    ...
    it('closes the listener when worker startup fails after resolveOnListen', async () => {
        tmpDir = fs.realpathSync(fs.mkdtempSync(path.join(os.tmpdir(), 'qws-channel-worker-fail-')));
        const worker = makeWorker({
            enabled: true,
            state: 'failed',
            channels: ['telegram'],
        ...
        vi.spyOn(serverModule, 'createServeApp').mockImplementation((...args) => {
            ...
            const acpHandle = app.locals['acpHandle'];
            if (acpHandle)
                acpHandle.attachServer = attachServer;
            return app;
        });
        const handle = await runQwenServe({
            port: 0,
    ...
    it('reloads through a forced settings reconcile', async () => {
        tmpDir = fs.realpathSync(fs.mkdtempSync(path.join(os.tmpdir(), 'qws-channel-worker-reload-')));
        fs.mkdirSync(path.join(tmpDir, '.qwen'));
        fs.writeFileSync(path.join(tmpDir, '.qwen', 'settings.json'), JSON.stringify({ channels: { telegram: { type: 'telegram' } } }));
        const worker = makeWorker({
            enabled: true,
    ...
    it('rejects ambiguous multi-workspace channel ownership before exposing a handle', async () => {
        tmpDir = fs.realpathSync(fs.mkdtempSync(path.join(os.tmpdir(), 'qws-channel-worker-plan-')));
        const primary = path.join(tmpDir, 'primary');
        const secondary = path.join(tmpDir, 'secondary');
        fs.mkdirSync(primary);
        fs.mkdirSync(secondary);
        ...
        vi.spyOn(acpBridge, 'createAcpSessionBridge').mockImplementation(() => makeFakeBridge());
        const supervisorFactory = vi.fn(() => makeWorker({
            enabled: true,
            state: 'running',
            channels: ['telegram'],
        }));
    ...
    it('records a secondary-only worker added to a primary-only daemon', async () => {
        tmpDir = fs.realpathSync(fs.mkdtempSync(path.join(os.tmpdir(), 'qws-dynamic-worker-pidfile-')));
        const primary = path.join(tmpDir, 'primary');
        const secondary = path.join(tmpDir, 'secondary');
        fs.mkdirSync(primary);
        fs.mkdirSync(secondary);
        ...
        vi.spyOn(settingsRuntime, 'loadSettings').mockImplementation((workspace) => ({
            merged: {
                channels: canonicalizeWorkspace(String(workspace)) === secondaryCwd
                    ? { feishu: { type: 'feishu' } }
        ...
        try {
            ...
            expect(workerFactory).toHaveBeenCalledWith(expect.objectContaining({ workspace: secondaryCwd }));
    ...
    it('orchestrates, persists, and hot-removes distinct workspace workers', async () => {
        ...
        tmpDir = fs.realpathSync(fs.mkdtempSync(path.join(os.tmpdir(), 'qws-channel-worker-groups-')));
        const primary = path.join(tmpDir, 'primary');
        const secondary = path.join(tmpDir, 'secondary');
        fs.mkdirSync(primary);
        fs.mkdirSync(secondary);
        ...
        fs.writeFileSync(path.join(secondary, '.qwen', 'settings.json'), JSON.stringify({ channels: { feishu: secondaryChannelConfig } }));
        vi.spyOn(qwenCore, 'resolveTelemetrySettings').mockResolvedValue({
            enabled: false,
            sensitiveSpanAttributeMaxLength: 1024 * 1024,
        });
        vi.spyOn(settingsRuntime, 'loadSettings').mockImplementation((workspace) => {
            ...
            return {
                merged: {
                    channels: workspaceCwd === secondaryCwd
                        ? { feishu: secondaryChannelConfig }
                        : { telegram: { type: 'telegram' } },
                },
            };
        });
        vi.spyOn(environmentRuntime, 'buildRuntimeEnvironment').mockImplementation((_settings, workspace, baseEnv) => ({
            effectiveEnv: Object.freeze({
                ...baseEnv,
                QWEN_SHARED_WEBHOOK_SECRET: canonicalizeWorkspace(workspace ?? process.cwd()) === secondaryCwd
                    ? 'secondary-secret'
                    : 'primary-secret',
            }),
        ...
        const workerSupervisors = new Map();
        const webhookEnqueues = new Map();
        const supervisorFactory = vi.fn((options) => {
            const pid = options.workspace === primaryCwd ? 1234 : 5678;
            const channels = options.selection.mode === 'names' ? options.selection.names : [];
            ...
            const supervisor = {
                start: vi.fn(async () => {
                    const capabilitiesResponse = await fetch(`${options.daemonUrl}/capabilities`, {
            ...
            workerSupervisors.set(options.workspace, supervisor);
            return supervisor;
        });
        const pidfile = makePidfileDeps();
        const removeByIds = vi.fn().mockResolvedValue(1);
        ...
        try {
            const crossWorkspaceSecretResponse = await fetch(`${handle.url}/channels/feishu/webhooks/github-ci`, {
                method: 'POST',
                headers: {
                    'content-type': 'application/json',
            ...
            const webhookResponse = await fetch(`${handle.url}/channels/feishu/webhooks/github-ci`, {
                method: 'POST',
                headers: {
            ...
            const capabilities = (await (await fetch(`${handle.url}/capabilities`, {
                headers: { Authorization: 'Bearer worker-remove-token' },
            })).json());
            const secondaryRuntime = capabilities.workspaces.find((workspace) => workspace.cwd === secondaryCwd);
            expect(secondaryRuntime).toMatchObject({ removable: true });
            const removalUrl = `${handle.url}/workspaces/${encodeURIComponent(secondaryRuntime.id)}`;
            ...
            const removedWebhook = await fetch(`${handle.url}/channels/feishu/webhooks/github-ci`, {
                method: 'POST',
                headers: {
                    Authorization: 'Bearer worker-remove-token',
                    'content-type': 'application/json',
            ...
            expect(pidfile.writeServeServiceInfo).toHaveBeenLastCalledWith(expect.objectContaining({
                workerPid: 1234,
                workers: [
                    expect.objectContaining({
                        workspaceCwd: primaryCwd,
            ...
            expect(pidfile.writeServeServiceInfo.mock.calls
                .at(-1)?.[0]
                .workers?.find((worker) => worker.workspaceCwd === secondaryCwd)).toBeUndefined();
            ...
            const readdedWebhook = await fetch(`${handle.url}/channels/feishu/webhooks/github-ci`, {
                method: 'POST',
                headers: {
                    Authorization: 'Bearer worker-remove-token',
                    'content-type': 'application/json',
            ...
            expect(webhookEnqueues.get(secondaryCwd)).toHaveBeenCalledWith(expect.objectContaining({
                channelName: 'feishu',
                title: 'CI failed again',
            }));
            snapshots.set(secondaryCwd, failedSecondary);
            workerOptions.get(secondaryCwd).onExit?.(failedSecondary);
            const failedWorkerPidfile = pidfile.writeServeServiceInfo.mock.calls.at(-1)?.[0];
            expect(failedWorkerPidfile?.workers?.find((worker) => worker.workspaceCwd === secondaryCwd)).toMatchObject({
                workspaceCwd: secondaryCwd,
                channels: ['feishu'],
            });
            expect(failedWorkerPidfile?.workers?.find((worker) => worker.workspaceCwd === secondaryCwd)?.workerPid).toBeUndefined();
    ...
    it('starts the channel worker after runtime mount and stops it before bridge shutdown', async () => {
        tmpDir = fs.realpathSync(fs.mkdtempSync(path.join(os.tmpdir(), 'qws-channel-worker-')));
        const order = [];
        ...
        const processRegistry = mockCreateSpawnChannelFactoryOptions.at(-1)?.['processRegistry'];
        const shutdownProcessRegistry = processRegistry.shutdown.bind(processRegistry);
    ...
    it('force-kills channel worker, bridge, and pidfile on a second shutdown signal', async () => {
        tmpDir = fs.realpathSync(fs.mkdtempSync(path.join(os.tmpdir(), 'qws-channel-worker-force-')));
        let finishBridgeShutdown;
        const bridge = makeFakeBridge();
        vi.mocked(bridge.shutdown).mockImplementation(() => new Promise((resolve) => {
    ...
    it('retries graceful shutdown after an unconfirmed channel worker exit', async () => {
        tmpDir = fs.realpathSync(fs.mkdtempSync(path.join(os.tmpdir(), 'qws-channel-worker-stuck-')));
        const bridge = makeFakeBridge();
        const worker = makeWorker({
            enabled: true,
            state: 'failed',
        ...
        worker.stop
            .mockRejectedValueOnce(new Error('Channel worker did not exit after SIGKILL.'))
    ...
    it('exits on the first signal when only ACP process shutdown fails', async () => {
        tmpDir = fs.realpathSync(fs.mkdtempSync(path.join(os.tmpdir(), 'qws-channel-acp-error-')));
        const bridge = makeFakeBridge();
        const worker = makeWorker({
            enabled: true,
            state: 'running',
        ...
        const processRegistry = mockCreateSpawnChannelFactoryOptions.at(-1)?.['processRegistry'];
        vi.spyOn(processRegistry, 'shutdown').mockRejectedValue(new Error('ACP process shutdown failed'));
    ...
    it('retries only the channel worker error when ACP process shutdown also fails', async () => {
        tmpDir = fs.realpathSync(fs.mkdtempSync(path.join(os.tmpdir(), 'qws-channel-combined-error-')));
        const bridge = makeFakeBridge();
        const worker = makeWorker({
            enabled: true,
            state: 'failed',
    ...
    it('keeps the channel lease when lifecycle aggregation wraps the retryable worker error', async () => {
        tmpDir = fs.realpathSync(fs.mkdtempSync(path.join(os.tmpdir(), 'qws-channel-aggregate-error-')));
        const worker = makeWorker({
            enabled: true,
            state: 'failed',
            pid: 1234,
    ...
    it('bounds the logger flush before allowing a retryable close to reject', async () => {
        tmpDir = fs.realpathSync(fs.mkdtempSync(path.join(os.tmpdir(), 'qws-channel-log-stuck-')));
        const bridge = makeFakeBridge();
        const worker = makeWorker({
            enabled: true,
            state: 'failed',
        ...
        try {
            ...
            await response.text();
            closeOutcome = handle.close().then(() => ({ kind: 'resolved' }), (error) => ({ kind: 'rejected', error }));
            let timeout;
            ...
            if (firstOutcome.kind === 'rejected') {
                expect(firstOutcome.error).toEqual(expect.objectContaining({
    ...
    it('force-stops the bridge while retrying a failed channel teardown', async () => {
        tmpDir = fs.realpathSync(fs.mkdtempSync(path.join(os.tmpdir(), 'qws-channel-bridge-stuck-')));
        const bridge = makeFakeBridge();
        ...
        await expect(handle.close()).rejects.toThrow('Channel worker did not exit after SIGKILL.');
        expect(worker.stop).toHaveBeenCalledTimes(1);
        expect(bridge.shutdown).toHaveBeenCalledTimes(1);
    ...
    it('removes serve-owned pidfile through the legacy fallback cleanup path', async () => {
        tmpDir = fs.realpathSync(fs.mkdtempSync(path.join(os.tmpdir(), 'qws-channel-worker-fallback-')));
        const worker = makeWorker({
            enabled: true,
            state: 'running',
            pid: 1234,
        ...
        const pidfile = makePidfileDeps();
        delete pidfile.removeServeServiceInfo;
    ...
    it('keeps serve running when worker pidfile metadata cannot be written', async () => {
        tmpDir = fs.realpathSync(fs.mkdtempSync(path.join(os.tmpdir(), 'qws-channel-worker-pidfile-')));
        const worker = makeWorker({
            enabled: true,
            state: 'running',
            pid: 1234,
    ...
    it('updates the serve-owned pidfile when a restarted worker becomes ready', async () => {
        tmpDir = fs.realpathSync(fs.mkdtempSync(path.join(os.tmpdir(), 'qws-channel-worker-ready-')));
        const worker = makeWorker({
            enabled: true,
            state: 'running',
            pid: 5678,
    ...
    it('forwards channel worker log and exit details into the daemon log', async () => {
        tmpDir = fs.realpathSync(fs.mkdtempSync(path.join(os.tmpdir(), 'qws-channel-worker-log-')));
        const originalRuntimeDir = process.env['QWEN_RUNTIME_DIR'];
        process.env['QWEN_RUNTIME_DIR'] = tmpDir;
        const worker = makeWorker({
            enabled: true,
        ...
        try {
            ...
            const logContent = fs
                ...
                .join('\n');
            expect(logContent).toContain('channel worker stderr: adapter failed with <redacted>');
            expect(logContent).toContain('channel worker exited (state=exited, pid=1234, code=1, signal=null, error=ipc failed, restartCount=2, nextRestartAt=2026-07-01T01:00:05.000Z, staleHeartbeatAt=2026-07-01T01:00:00.000Z)');
    ...
    it('passes a loopback daemon URL to workers when serve binds a wildcard host', async () => {
        tmpDir = fs.realpathSync(fs.mkdtempSync(path.join(os.tmpdir(), 'qws-channel-worker-loopback-')));
        const worker = makeWorker({
            enabled: true,
            state: 'running',
            pid: 1234,
    ...
    it('does not write a worker pidfile after runtime startup already timed out', async () => {
        tmpDir = fs.realpathSync(fs.mkdtempSync(path.join(os.tmpdir(), 'qws-channel-worker-timeout-')));
        let releaseStart;
        const worker = makeWorker({
            enabled: true,
            state: 'running',
        ...
        const handle = await runQwenServe({
            ...
            bridge: makeFakeBridge(),
            channelWorkerSupervisorFactory: vi.fn((opts) => {
                worker.start.mockImplementation(() => new Promise((resolve) => {
                    releaseStart = () => {
                        opts.onReady?.(worker.snapshot());
                        resolve();
                    };
        ...
        try {
            await expect(Promise.race([
                handle.runtimeReady,
                new Promise((_, reject) => setTimeout(() => reject(new Error('runtimeReady did not settle')), 1000)),
    ...
    it('reports a warning when the ready channel worker exits', async () => {
        tmpDir = fs.realpathSync(fs.mkdtempSync(path.join(os.tmpdir(), 'qws-channel-worker-status-')));
        const snapshot = {
            enabled: true,
            state: 'running',
            pid: 1234,
        ...
        let onExit;
        const channelWorkerSupervisorFactory = vi.fn((opts) => {
            onExit = opts.onExit;
            return worker;
        });
        const pidfile = makePidfileDeps();
        ...
        try {
            ...
            const res = await fetch(`${handle.url}/daemon/status`);
            const body = await res.json();
            expect(pidfile.removeServeServiceInfo).not.toHaveBeenCalledWith(process.pid);
            const lastPidfileWrite = pidfile.writeServeServiceInfo.mock.calls.at(-1)?.[0];
    ...
    it('drains the runtime when channel grouping rejects startup after listen', async () => {
        tmpDir = fs.realpathSync(fs.mkdtempSync(path.join(os.tmpdir(), 'qws-channel-untrusted-')));
        const bridge = makeFakeBridge();
        const pidfile = makePidfileDeps();
        await expect(runQwenServe({
            port: 0,
    ...
    it('preserves the startup reason when channel cleanup also fails', async () => {
        tmpDir = fs.realpathSync(fs.mkdtempSync(path.join(os.tmpdir(), 'qws-channel-startup-cleanup-')));
        ...
        try {
            ...
            expect(error.message).toContain('is not trusted; cannot host channels');
    ...
    it('keeps the serve owner alive when failed startup cannot confirm worker exit', async () => {
        tmpDir = fs.realpathSync(fs.mkdtempSync(path.join(os.tmpdir(), 'qws-channel-worker-retained-')));
        const worker = makeWorker({
            enabled: true,
            state: 'failed',
            pid: 1234,
        ...
        worker.stop.mockRejectedValue(new Error('Channel worker did not exit after SIGKILL.'));
        const pidfile = makePidfileDeps();
        const exitSpy = vi
            .spyOn(process, 'exit')
            .mockImplementation((() => undefined));
    ...
    it('refuses to start when another channel service is already running', async () => {
        tmpDir = fs.realpathSync(fs.mkdtempSync(path.join(os.tmpdir(), 'qws-channel-worker-busy-')));
        const workerFactory = vi.fn(() => makeWorker({
            enabled: true,
            state: 'running',
            pid: 1234,
    ...
    it('retries channel pidfile reservation after an EEXIST stale file cleanup', async () => {
        tmpDir = fs.realpathSync(fs.mkdtempSync(path.join(os.tmpdir(), 'qws-channel-worker-stale-')));
        const worker = makeWorker({
            enabled: true,
            state: 'running',
            pid: 1234,
    ...
    it('removes the channel pidfile reservation when listener startup fails', async () => {
        tmpDir = fs.realpathSync(fs.mkdtempSync(path.join(os.tmpdir(), 'qws-channel-worker-listen-')));
        const worker = makeWorker({
            enabled: true,
            state: 'running',
            pid: 1234,
    ...
    it('retries the next port on EADDRINUSE and succeeds', async () => {
        tmpDir = fs.realpathSync(fs.mkdtempSync(path.join(os.tmpdir(), 'qws-port-retry-')));
        const portsAttempted = [];
        const stderrWrites = [];
        const stderrSpy = vi
            .spyOn(process.stderr, 'write')
            .mockImplementation((chunk) => {
        ...
        try {
            ...
            expect(new URL(handle.url).port).toBe(String(handle.server.address().port));
            expect(stderrWrites.some((w) => w.includes('port 4170 is in use, trying 4171'))).toBe(true);
        }
        finally {
            await handle.close();
        }
    ...
    it('does not retry on non-EADDRINUSE listen errors', async () => {
        tmpDir = fs.realpathSync(fs.mkdtempSync(path.join(os.tmpdir(), 'qws-port-no-retry-')));
        const portsAttempted = [];
    ...
    it('rejects after exhausting all port retry attempts', async () => {
        tmpDir = fs.realpathSync(fs.mkdtempSync(path.join(os.tmpdir(), 'qws-port-exhaust-')));
        const portsAttempted = [];
        const stderrWrites = [];
        const stderrSpy = vi
            .spyOn(process.stderr, 'write')
        ...
        stderrSpy.mockRestore();
        expect(portsAttempted).toEqual(Array.from({ length: 10 }, (_, i) => 4170 + i));
        expect(stderrWrites.some((w) => w.includes('all ports 4170–4179 are in use'))).toBe(true);
    });
    it('does not retry EADDRINUSE when port is 0 (ephemeral)', async () => {
        tmpDir = fs.realpathSync(fs.mkdtempSync(path.join(os.tmpdir(), 'qws-port0-no-retry-')));
        const portsAttempted = [];
        const listenError = new Error('address in use');
        listenError.code = 'EADDRINUSE';
        vi.spyOn(serverModule, 'createServeApp').mockReturnValue({
    ...
    it('does not remove the channel pidfile reservation for handled uncaught exceptions', async () => {
        tmpDir = fs.realpathSync(fs.mkdtempSync(path.join(os.tmpdir(), 'qws-channel-worker-crash-')));
        const worker = makeWorker({
            enabled: true,
            state: 'running',
            pid: 1234,
        ...
        const pidfile = makePidfileDeps();
        const existingMonitorListeners = new Set(process.rawListeners('uncaughtExceptionMonitor'));
        ...
        try {
            ...
            const monitorListeners = process.rawListeners('uncaughtExceptionMonitor');
            const newMonitorListeners = monitorListeners.filter((listener) => !existingMonitorListeners.has(listener));
    ...
    it('preserves the channel pidfile reservation until an unhandled-exit worker is confirmed gone', async () => {
        tmpDir = fs.realpathSync(fs.mkdtempSync(path.join(os.tmpdir(), 'qws-channel-worker-unhandled-')));
        const worker = makeWorker({
            enabled: true,
            state: 'running',
            pid: 1234,
...
describe('runQwenServe startup observability', () => {
    it("names every pre-auth surface in the --allow-origin '*' warning", async () => {
        ...
        // message.
        tmpDir = fs.realpathSync(fs.mkdtempSync(path.join(os.tmpdir(), 'qws-allow-origin-')));
        const stderrWrites = [];
        const spy = vi
            .spyOn(process.stderr, 'write')
            .mockImplementation((chunk) => {
    ...
    async function readStartup(handle) {
        const res = await fetch(`${handle.url}/daemon/status`, {
    ...
    async function waitForPreheatStatus(handle, status) {
        await handle.runtimeReady;
        for (let i = 0; i < 20; i++) {
            const startup = await readStartup(handle);
            if (startup?.preheat?.status === status)
                return startup.preheat;
            await new Promise((resolve) => setTimeout(resolve, 10));
        }
        throw new Error(`preheat status did not become ${status}`);
    }
    function installInternalBridge(preheat) {
        const bridge = makeFakeBridge();
    ...
    it('keeps the stdout listening contract and exposes startup timing on stderr and status', async () => {
        tmpDir = fs.realpathSync(fs.mkdtempSync(path.join(os.tmpdir(), 'qws-startup-')));
        const stderrWrites = [];
        const stdoutWrites = [];
        vi.spyOn(process.stderr, 'write').mockImplementation((chunk) => {
            stderrWrites.push(String(chunk));
        ...
        try {
            expect(stdoutWrites).toEqual(expect.arrayContaining([
                expect.stringMatching(/^qwen serve listening on http:\/\/127\.0\.0\.1:\d+ \(mode=http-bridge, workspace=/),
            ]));
            expect(stderrWrites.join('')).toMatch(/qwen serve: startup timing: processToListenMs=\d+ runQwenServeToListenMs=\d+/);
    ...
    it('uses boot runtimeOutputDir for daemon logs', async () => {
        const originalRuntimeDir = process.env['QWEN_RUNTIME_DIR'];
        delete process.env['QWEN_RUNTIME_DIR'];
        tmpDir = fs.realpathSync(fs.mkdtempSync(path.join(os.tmpdir(), 'qws-startup-runtime-dir-')));
        const boundWorkspace = canonicalizeWorkspace(tmpDir);
        const stderrWrites = [];
        vi.spyOn(process.stderr, 'write').mockImplementation((chunk) => {
            stderrWrites.push(String(chunk));
        ...
        try {
            ...
            const expectedDaemonDir = path.join(boundWorkspace, '.qwen-runtime', 'debug', 'daemon');
            expect(stderrWrites.join('')).toContain(`qwen serve: daemon log → ${expectedDaemonDir}`);
    ...
    it('uses explicit daemonLogBaseDir when provided by an embedder', async () => {
        tmpDir = fs.realpathSync(fs.mkdtempSync(path.join(os.tmpdir(), 'qws-startup-log-dep-')));
    ...
    it('preserves Storage runtime base dir for default exported callers', async () => {
        ...
        tmpDir = fs.realpathSync(fs.mkdtempSync(path.join(os.tmpdir(), 'qws-startup-storage-dir-')));
        fs.mkdirSync(path.join(tmpDir, '.qwen'));
        fs.writeFileSync(path.join(tmpDir, '.qwen', 'settings.json'), JSON.stringify({
    ...
    it('tracks preheat running and succeeded states for an internally-created bridge', async () => {
        tmpDir = fs.realpathSync(fs.mkdtempSync(path.join(os.tmpdir(), 'qws-startup-preheat-')));
        let resolvePreheat;
        const preheatPromise = new Promise((resolve) => {
    ...
    it('tracks preheat failed state and error message for an internally-created bridge', async () => {
        tmpDir = fs.realpathSync(fs.mkdtempSync(path.join(os.tmpdir(), 'qws-startup-preheat-')));
        const bridge = installInternalBridge(() => Promise.reject(new Error('preheat boom')));
        const handle = await runQwenServe({
            port: 0,
            hostname: '127.0.0.1',
            mode: 'http-bridge',
...
//# sourceMappingURL=run-qwen-serve.test.js.map