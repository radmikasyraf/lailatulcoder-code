 ...
 * individual fields.
 */
import type { ServeWorkspaceMcpStatus, ServeWorkspaceSkillsStatus, ServeWorkspaceProvidersStatus, ServeWorkspaceExtensionsStatus, ServeWorkspaceHooksStatus, ServeWorkspaceEnvStatus, ServeWorkspacePreflightStatus, DaemonStatusProvider } from '@lailatul-coder/acp-bridge';
import type { WorkspaceTrustStatus } from '../../config/trustedFolders.js';
import type { PermissionRuleType, PermissionSettingsScope, QwenPermissionSettings } from '../../config/permission-settings.js';
import type { SettingScope, EnvReloadResult, LoadedSettings } from '../../config/settings.js';
...
import type { WorkspaceSkillInstallRequest, WorkspaceSkillMutationResult, WorkspaceSkillScope } from '../workspace-skill-management.js';
export type { WorkspaceSkillInstallRequest, WorkspaceSkillMutationResult, WorkspaceSkillScope, } from '../workspace-skill-management.js';
/**
 ...
 */
export type InvokeWorkspaceCommandFn = <T>(method: string, params?: Record<string, unknown>, opts?: {
    timeoutMs?: number;
}) => Promise<T>;
...
export interface DaemonWorkspaceService {
    ...
    getWorkspaceMcpStatus(ctx: WorkspaceRequestContext): Promise<ServeWorkspaceMcpStatus>;
    /** Skill status for the bound workspace. */
    getWorkspaceSkillsStatus(ctx: WorkspaceRequestContext): Promise<ServeWorkspaceSkillsStatus>;
    /** Model-provider status for the bound workspace. */
    getWorkspaceProvidersStatus(ctx: WorkspaceRequestContext): Promise<ServeWorkspaceProvidersStatus>;
    /** Environment snapshot for the bound workspace. */
    getWorkspaceEnvStatus(ctx: WorkspaceRequestContext): Promise<ServeWorkspaceEnvStatus>;
    /** Preflight diagnostics for the bound workspace. */
    getWorkspacePreflightStatus(ctx: WorkspaceRequestContext): Promise<ServeWorkspacePreflightStatus>;
    /** Hook configuration status for the bound workspace. */
    getWorkspaceHooksStatus(ctx: WorkspaceRequestContext): Promise<ServeWorkspaceHooksStatus>;
    /** Installed extension status for the bound workspace. */
    getWorkspaceExtensionsStatus(ctx: WorkspaceRequestContext): Promise<ServeWorkspaceExtensionsStatus>;
    /** Trust status for the bound workspace. */
    getWorkspaceTrustStatus(ctx: WorkspaceRequestContext): Promise<WorkspaceTrustStatus>;
    /** Permission settings for the bound workspace. */
    getWorkspacePermissionsStatus(ctx: WorkspaceRequestContext): Promise<QwenPermissionSettings>;
    /** Start the ACP child/channel without creating a user-visible session. */
    preheatAcpChild(ctx: WorkspaceRequestContext, opts?: {
        timeoutMs?: number;
    }): Promise<WorkspaceAcpPreheatResult>;
    /** Current ACP child/channel liveness for the bound workspace. */
    getWorkspaceAcpStatus(ctx: WorkspaceRequestContext): Promise<WorkspaceAcpStatusResult>;
    /** Voice settings and selectable transcription models for the workspace. */
    getWorkspaceVoiceStatus(ctx: WorkspaceRequestContext): Promise<WorkspaceVoiceStatus>;
    /** Request that the local operator change workspace trust. */
    requestWorkspaceTrustChange(ctx: WorkspaceRequestContext, request: WorkspaceTrustChangeRequest): Promise<WorkspaceTrustChangeResult>;
    /** Replace one permission rule list. */
    setWorkspacePermissionRules(ctx: WorkspaceRequestContext, request: WorkspacePermissionRulesUpdate): Promise<QwenPermissionSettings>;
    /** Persist workspace voice settings. */
    setWorkspaceVoiceSettings(ctx: WorkspaceRequestContext, request: WorkspaceVoiceSettingsUpdate): Promise<WorkspaceVoiceStatus>;
    /** Toggle a tool enabled/disabled in workspace settings. */
    setWorkspaceToolEnabled(ctx: WorkspaceRequestContext, toolName: string, enabled: boolean): Promise<{
        toolName: string;
        enabled: boolean;
    }>;
    /** Toggle a skill in the workspace skill settings. */
    setWorkspaceSkillEnabled(ctx: WorkspaceRequestContext, skillName: string, enabled: boolean): Promise<WorkspaceSkillToggleResult>;
    /** Toggle multiple skills with one settings write and one session refresh. */
    setWorkspaceSkillsEnabled(ctx: WorkspaceRequestContext, skillNames: readonly string[], enabled: boolean): Promise<WorkspaceSkillBatchToggleResult>;
    /** Install a project- or user-level Skill from a bounded package. */
    installWorkspaceSkill(ctx: WorkspaceRequestContext, request: WorkspaceSkillInstallRequest): Promise<WorkspaceSkillMutationResult>;
    /** Delete a managed project- or user-level Skill. */
    deleteWorkspaceSkill(ctx: WorkspaceRequestContext, skillName: string, scope: WorkspaceSkillScope): Promise<WorkspaceSkillMutationResult>;
    /** Scaffold (init) a QWEN.md file in the workspace. */
    initWorkspace(ctx: WorkspaceRequestContext, opts: {
        force?: boolean;
    }): Promise<{
        path: string;
    ...
    restartMcpServer(ctx: WorkspaceRequestContext, serverName: string, opts?: {
        entryIndex?: number;
...
export declare class WorkspaceSettingsPartialPersistError extends Error {
    readonly committedWrites: WorkspaceSettingsWrite[];
    readonly cause: unknown;
    constructor(message: string, committedWrites: WorkspaceSettingsWrite[], cause: unknown);
}
...
export declare class WorkspacePermissionRulesSessionRequiredError extends Error {
    constructor();
}
...
export type WorkspaceSkillToggleErrorCode = 'skill_not_found' | 'skill_not_toggleable' | 'skill_inactive_extension';
...
export type PersistDisabledSkillsBatchOutcome = {
    ...
    skillName: string;
    error: WorkspaceSkillNotToggleableError;
};
...
export type WorkspaceSkillNotToggleableReason = 'not_user_invocable' | 'inactive_extension' | 'locked';
export declare class WorkspaceSkillNotFoundError extends Error {
    readonly skillName: string;
    constructor(skillName: string);
}
export declare class WorkspaceSkillNotToggleableError extends Error {
    readonly skillName: string;
    readonly reason: WorkspaceSkillNotToggleableReason;
    readonly lockedScope?: "system" | "user" | "systemDefaults" | undefined;
    constructor(skillName: string, reason: WorkspaceSkillNotToggleableReason, lockedScope?: "system" | "user" | "systemDefaults" | undefined);
}
export declare function mapWorkspaceSkillToggleError(error: unknown): WorkspaceSkillToggleError | undefined;
...
export interface DaemonWorkspaceServiceDeps {
    ...
    persistDisabledTools: (workspace: string, toolName: string, enabled: boolean, assertGenerationOpen?: () => void) => Promise<void>;
    /** Persist a skill enable/disable change to workspace settings. */
    persistDisabledSkills: (workspace: string, skillName: string, enabled: boolean, assertGenerationOpen?: () => void) => Promise<PersistDisabledSkillResult>;
    /** Persist multiple skill changes under one settings lock. */
    persistDisabledSkillsBatch: (workspace: string, skillNames: readonly string[], enabled: boolean, assertGenerationOpen?: () => void) => Promise<PersistDisabledSkillsBatchResult>;
    persistSetting?: (workspace: string, scope: SettingScope, key: string, value: unknown, assertGenerationOpen?: () => void) => Promise<void | LoadedSettings>;
    persistSettings?: (workspace: string, writes: WorkspaceSettingsWrite[], assertGenerationOpen?: () => void) => Promise<void>;
    ...
    reloadDaemonEnv?: (workspace: string, assertGenerationOpen?: () => void) => Promise<EnvReloadResult>;
