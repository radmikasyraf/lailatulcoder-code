...
import { ArenaEventType, ArenaSessionStatus, DISPLAY_MODE, } from '@lailatul-coder/lailatul-coder-core';
 ...
 */
export function useArenaInProcess(config, actions) {
    const actionsRef = useRef(actions);
    actionsRef.current = actions;
    useEffect(() => {
        if (!config)
            return;
        let detachArenaListeners = null;
        ...
        const attachSession = (manager) => {
            ...
            if (inProcessBackend) {
                for (const agentState of manager.getAgentStates()) {
                    ...
                    if (interactive) {
                        actionsRef.current.registerAgent(agentState.agentId, interactive, agentState.model.modelId, nextColor(), agentState.model.displayName);
                    }
                }
            }
            ...
            const onAgentStart = (event) => {
                const tryRegister = (retriesLeft) => {
                    ...
                    if (interactive) {
                        actionsRef.current.registerAgent(event.agentId, interactive, event.model.modelId, nextColor(), event.model.displayName);
                        return;
                    }
                    if (retriesLeft > 0) {
...
//# sourceMappingURL=useArenaInProcess.js.map