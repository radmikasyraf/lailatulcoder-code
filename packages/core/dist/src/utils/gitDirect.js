...
export function isValidRefName(name) {
    ...
    if (name.includes('@{'))
        return false;
    if (INVALID_REF_CHARS.test(name))
        return false;
 ...
 */
export function clearGitDirCache() {
...
async function hasGitStore(dir) {
 ...
 */
async function isRealGitDir(gitDir) {
    if (await hasGitStore(gitDir))
        return true;
...
async function resolveTrustedGitDir(cwd) {
    const gitDir = await resolveGitDir(cwd);
    if (!gitDir)
        return null;
    return (await isRealGitDir(gitDir)) ? gitDir : null;
}
async function getCachedGitDir(cwd) {
 ...
 */
export async function readGitHead(gitDir) {
 ...
 */
export async function resolveBranchName(cwd) {
...
const repoBranchWatches = new Map();
/** Close every shared reflog watcher and drop the entries. */
function closeAllRepoBranchWatches() {
 ...
 */
export async function watchRepoBranch(cwd, onChange) {
    const gitDir = await getCachedGitDir(cwd);
    if (!gitDir)
        return () => { };
    ...
    if (!entry) {
        ...
        if (existing) {
            ...
            let watcher;
            try {
                watcher = fs.watch(logsHeadPath, (eventType) => {
            ...
            watcher.on('error', (err) => {
                debug.warn(`reflog watcher error for ${gitDir}; branch will no longer auto-update: ${err?.message}`);
...
//# sourceMappingURL=gitDirect.js.map