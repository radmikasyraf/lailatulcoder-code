...
import { collectContextData, formatContextUsageText, } from './contextCommand.js';
...
const { mockGetLastPromptTokenCount, mockGetLastCachedContentTokenCount } = vi.hoisted(() => ({
...
function makeMockConfig(contextWindowSize = 32_000) {
    return {
...
describe('collectContextData (contextCommand)', () => {
    let getFunctionDeclarationsSpy;
    let mockConfig;
    beforeEach(() => {
    ...
    it('excludes deferred-but-not-revealed tools from the per-tool breakdown (#4508)', async () => {
        const isDeferredAndHidden = vi
            .fn()
            .mockImplementation((name) => name === 'web_fetch' || name === 'mcp__server__tool');
        const hiddenBuiltin = {
            name: 'web_fetch',
        ...
        const config = {
            ...
            getToolRegistry: vi.fn().mockReturnValue({
                ...
                isDeferredAndHidden: vi
                    .fn()
                    .mockImplementation((name) => name === 'monitor'),
            }),
    ...
    it('shortens home-dir memory marker paths to ~ in the breakdown', async () => {
        ...
        const memory = `--- Context from: ${markerPath} ---\n` +
            `global rules\n` +
    ...
    it('renders project-local markers as relative paths when workingDir != cwd', async () => {
        ...
        const workingDir = path.join(os.tmpdir(), 'context-session-dir');
        const memory = `--- Context from: QWEN.md ---\n` +
            `project rules\n` +
...
//# sourceMappingURL=contextCommand.test.js.map