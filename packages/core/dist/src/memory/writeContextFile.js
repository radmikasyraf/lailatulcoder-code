...
export class WorkspaceMemoryWriteTimeoutError extends Error {
    ...
    constructor(filePath, timeoutMs) {
        super(`Workspace memory write at ${filePath} did not acquire the per-file ` +
 ...
 * `~/.lailatulcoder/` directory. Used by the `qwen serve` daemon's
 ...
 */
export async function writeWorkspaceContextFile(options) {
    if (!path.isAbsolute(options.projectRoot)) {
        throw new Error(`writeWorkspaceContextFile: projectRoot must be absolute, got "${options.projectRoot}"`);
    }
    ...
    // composite.
    try {
        return await getFileLock(filePath).runExclusive(async () => await runWrite(filePath, options));
    }
    catch (err) {
        ...
        if (err === E_TIMEOUT) {
            throw new WorkspaceMemoryWriteTimeoutError(filePath, FILE_LOCK_TIMEOUT_MS);
        }
        throw err;
    }
}
async function runWrite(filePath, options) {
...
function resolveContextFilePath(scope, projectRoot) {
 ...
 * `~/.lailatulcoder/QWEN.md` may have been edited externally). 16 MB sits
...
export class WorkspaceMemoryFileTooLargeError extends Error {
    ...
    bytes;
    limit;
    constructor(filePath, bytes, limit) {
        super(`Existing memory file at ${filePath} is ${bytes} bytes, exceeds ` +
...
async function composeAppendedContent(filePath, newContent) {
    ...
    try {
        ...
        if (stat.size > MAX_EXISTING_FILE_BYTES) {
            throw new WorkspaceMemoryFileTooLargeError(filePath, stat.size, MAX_EXISTING_FILE_BYTES);
        }
 ...
 */
function findNextTopLevelHeading(text, start) {
    let inFence = false;
    let lineStart = start;
...
function trimNewlines(s) {
    let start = 0;
    let end = s.length;
    while (start < end && s.charCodeAt(start) === 0x0a)
        start++;
    while (end > start && s.charCodeAt(end - 1) === 0x0a)
        end--;
    return start === 0 && end === s.length ? s : s.slice(start, end);
}
//# sourceMappingURL=writeContextFile.js.map