...
import { RUNTIME_MCP_IF_ABSENT_CONFIG_FLAG, Storage, } from '@lailatul-coder/lailatul-coder-core';
...
import { isPortableAbsolutePath, resolveManagedWorkspaceRuntimeByPathSelector, resolveWorkspaceEntryFromParam, sendWorkspaceRuntimeUnavailable, } from '../workspace-route-runtime.js';
import { ConnectionRegistry, } from './connection-registry.js';
import { ACP_PRE_ATTACH_MAX_FRAMES_GLOBAL, ACP_PRE_ATTACH_MAX_PAYLOAD_BYTES_GLOBAL, AcpPreAttachBudget, } from './pre-attach-budget.js';
import { SseStream } from './sse-stream.js';
import { WsStream } from './ws-stream.js';
import { SessionArchiveCoordinator } from '../server/session-archive.js';
import { RPC, error as rpcError, isNotification, isRequest, isResponse, logSafe, parseInbound, } from './json-rpc.js';
import { parseEventEpochHeader, parseLastEventId, } from '../sse-last-event-id.js';
...
const CDP_PATH = '/cdp';
function isActiveDrainCorrelation(registry, conn, message) {
 ...
 */
function rawRequestPathname(reqUrl) {
    const target = reqUrl ?? '/';
 ...
 */
function pluralWorkspaceRawSelector(rawPath, suffix) {
    let p = rawPath;
    if (p.endsWith(`${suffix}/`)) {
        p = p.slice(0, -1);
    }
    if (!p.startsWith(PLURAL_WS_PREFIX) ||
        !p.endsWith(suffix) ||
 ...
 */
const CDP_BRIDGE_CLIENT_NAME = 'LailatulCoder-cdp-bridge';
...
function formatCdpEndpointHost(hostname) {
...
function isAcpChannelUnavailable(err) {
    return (err?.data?.errorKind ===
...
function buildChromeDevToolsMcpRuntimeConfig(localPort, hostname, env) {
    ...
    if (!command) {
        writeStderrLine(`LailatulCoder serve: set ${QWEN_CDP_MCP_COMMAND_ENV} to enable browser automation MCP (no adapter is bundled)`);
        return undefined;
    }
    return {
        command,
 ...
 * `LailatulCoder-bearer.<base64url(token)>`. Kept in sync with the encoder in
 * `packages/web-shell/client/voice/useVoiceCapture.ts`.
 */
export const WS_BEARER_SUBPROTOCOL_PREFIX = 'LailatulCoder-bearer.';
/**
 ...
 * the `LailatulCoder-bearer.*` subprotocol (browser clients). Returns `undefined` when
 * neither is present or parseable.
 */
function extractUpgradeBearer(req) {
    ...
    if (offered) {
        for (const raw of offered.split(',')) {
            ...
            if (!entry.startsWith(WS_BEARER_SUBPROTOCOL_PREFIX))
                continue;
...
function isSameLoopbackOrigin(origin, localPort) {
    if (!localPort)
        return false;
...
function workspaceRateLimitKey(mount, clientKey) {
 ...
 */
export function mountAcpHttp(app, bridge, opts) {
    const enabled = opts.enabled ?? resolveAcpHttpEnabled();
    if (!enabled)
        return undefined;
    const daemonEnv = opts.daemonEnv ?? process.env;
    const primarySessionRuntimeBaseDir = opts.workspaceRegistry?.primary.sessionRuntimeBaseDir ??
        Storage.getRuntimeBaseDir();
    const archiveCoordinator = opts.archiveCoordinator ?? new SessionArchiveCoordinator();
    const requestedSessionIdAdmission = opts.requestedSessionIdAdmission;
    const getPrimaryEnv = () => opts.workspaceRegistry
    ...
    const rejectIfUnavailable = (mount, res) => {
        if (rejectIfDisposed(res))
            return true;
        if (!mount.draining)
            return false;
    ...
    const registry = new ConnectionRegistry((req, clientId) => {
        ...
        if (!dispatcherRef.current) {
            writeStderrLine('LailatulCoder serve: /acp abandonPending called before dispatcher initialized (skipped)');
            return false;
        }
    ...
    (sessionId, clientId) => {
        void bridge.detachClient(sessionId, clientId).catch((err) => {
            writeStderrLine(`LailatulCoder serve: /acp detachClient(${sessionId}) failed: ${err instanceof Error ? err.message : String(err)}`);
        });
    }, opts.maxConnections, undefined, preAttachBudget);
    let cdpMcpRegistered = false;
    let cdpMcpRegistering;
    let cdpMcpTerminalSkipLogged = false;
    async function removeChromeDevToolsMcp(originatorClientId) {
        if (!cdpMcpRegistered)
            return;
        cdpMcpRegistered = false;
        try {
            await bridge.removeRuntimeMcpServer(CHROME_DEVTOOLS_MCP_SERVER_NAME, originatorClientId);
        }
        catch (err) {
            writeStderrLine(`LailatulCoder serve: failed to remove ${CHROME_DEVTOOLS_MCP_SERVER_NAME} runtime MCP: ${err instanceof Error ? err.message : String(err)}`);
        }
    }
    function removeChromeDevToolsMcpIfUnused(originatorClientId) {
        void (async () => {
            if (opts.cdpTunnelRegistry?.hasActive())
                return;
            if (cdpMcpRegistering)
                await cdpMcpRegistering;
            if (opts.cdpTunnelRegistry?.hasActive())
                return;
            await removeChromeDevToolsMcp(originatorClientId);
        })().catch((err) => {
            writeStderrLine(`LailatulCoder serve: failed to clean up ${CHROME_DEVTOOLS_MCP_SERVER_NAME} runtime MCP: ${err instanceof Error ? err.message : String(err)}`);
        });
    }
    function ensureChromeDevToolsMcpRegistered(localPort, originatorClientId) {
        if (opts.token) {
            if (!cdpMcpTerminalSkipLogged) {
                writeStderrLine(`LailatulCoder serve: ${CHROME_DEVTOOLS_MCP_SERVER_NAME} runtime MCP skipped because /cdp requires bearer auth`);
        ...
        const runtimeConfig = buildChromeDevToolsMcpRuntimeConfig(localPort, opts.hostname, daemonEnv);
        if (!runtimeConfig) {
            cdpMcpTerminalSkipLogged = true;
            return;
        }
        cdpMcpRegistering = (async () => {
            try {
                ...
                for (let attempt = 0;; attempt++) {
                    try {
                        result = await bridge.addRuntimeMcpServer(CHROME_DEVTOOLS_MCP_SERVER_NAME, runtimeConfig, originatorClientId);
                        break;
                    }
                    catch (err) {
                        if (!isAcpChannelUnavailable(err) ||
                ...
                if (result.skipped) {
                    writeStderrLine(`LailatulCoder serve: ${CHROME_DEVTOOLS_MCP_SERVER_NAME} runtime MCP skipped: ${result.reason ?? 'unknown'}`);
                    return;
                }
                if (result.shadowedSettings) {
                    await bridge
                        .removeRuntimeMcpServer(CHROME_DEVTOOLS_MCP_SERVER_NAME, originatorClientId)
                        .catch(() => { });
                    cdpMcpTerminalSkipLogged = true;
                    writeStderrLine(`LailatulCoder serve: ${CHROME_DEVTOOLS_MCP_SERVER_NAME} runtime MCP skipped because settings already define it`);
                    return;
                }
                ...
                writeStderrLine(`LailatulCoder serve: failed to add ${CHROME_DEVTOOLS_MCP_SERVER_NAME} runtime MCP: ${err instanceof Error ? err.message : String(err)}`);
            }
            finally {
    ...
    const dispatcher = new AcpDispatcher(bridge, opts.boundWorkspace, getPrimaryEnv, opts.workspace, opts.workspaceRememberLane, requestedSessionIdAdmission, opts.fsFactory, opts.deviceFlowRegistry, opts.sessionShellCommandEnabled === true, registry, archiveCoordinator, opts.isPrimaryWorkspaceTrusted ??
        (() => {
    ...
    }, undefined, primarySessionRuntimeBaseDir, () => {
    ...
    const handleAcpDelete = (mount, req, res) => {
        if (rejectIfUnavailable(mount, res))
            return;
        ...
        if (existed) {
            writeStderrLine(`LailatulCoder serve: ${mount.routeLabel} connection deleted ${connectionId.slice(0, 8)} (remaining=${mount.registry.size})`);
        }
        res.status(202).end();
    };
    // ── POST /acp ──────────────────────────────────────────────────────
    const handleAcpPost = async (mount, req, res) => {
        if (rejectIfDisposed(res))
            return;
        ...
        if (!parsed.ok) {
            writeStderrLine(`LailatulCoder serve: ${mount.routeLabel} malformed request from ${req.socket?.remoteAddress}: ${parsed.error.error.message}`);
            res.status(400).json(parsed.error);
            return;
        }
        const message = parsed.message;
        const cleanupMessage = (isRequest(message) || isNotification(message)) &&
            message.method === 'session/cancel';
        ...
        if (isRequest(message) && message.method === 'initialize') {
            ...
            if (!conn) {
                ...
                writeStderrLine(`LailatulCoder serve: ${mount.routeLabel} connection cap reached (max=${mount.registry.connectionCap}), rejecting initialize`);
                res.setHeader('Retry-After', '5');
                res
                    .status(503)
                    .json(rpcError(message.id, RPC.INTERNAL_ERROR, 'Too many ACP connections; retry later'));
                return;
            }
            const requestedVersion = message.params &&
            ...
            res.status(200).json({
                ...
                jsonrpc: '2.0',
                id: message.id,
                result: mount.dispatcher.buildInitializeResult(conn.connectionId, requestedVersion),
            });
            writeStderrLine(`LailatulCoder serve: ${mount.routeLabel} connection established ${conn.connectionId.slice(0, 8)} ` +
        ...
        if (!connHeader) {
            res
                .status(400)
                .json(rpcError(isRequest(message) ? message.id : null, RPC.INVALID_REQUEST, 'Missing Acp-Connection-Id'));
            return;
        }
        ...
        if (!conn) {
            res
                .status(404)
                .json(rpcError(isRequest(message) ? message.id : null, RPC.INVALID_REQUEST, 'Unknown Acp-Connection-Id'));
            return;
        }
        if (mount.draining) {
        ...
        if (opts.checkRate && isRequest(message)) {
            ...
            if (!WS_EXEMPT_METHODS.has(m)) {
                const tier = m === 'session/prompt' || m === '_qwen/session/prompt'
                    ? 'prompt'
                    : WS_READ_METHODS.has(m)
                        ? 'read'
                        : 'mutation';
                const httpKey = (req.socket?.remoteAddress ?? 'http-unknown').replace(/^::ffff:/, '');
        ...
        await mount.dispatcher
            ...
            .catch((err) => {
            writeStderrLine(`LailatulCoder serve: ${mount.routeLabel} handle error: ${err instanceof Error ? err.message : String(err)}`);
        });
    };
    app.post(path, (req, res) => handleAcpPost(primaryMount, req, res));
    // ── GET /acp (SSE) ─────────────────────────────────────────────────
    const handleAcpGet = (mount, req, res) => {
        if (rejectIfUnavailable(mount, res))
            return;
        ...
        if (!sessionId) {
            ...
            const stream = new SseStream(res, () => {
                writeStderrLine(`LailatulCoder serve: ${mount.routeLabel} connection stream closed (${connId.slice(0, 8)})`);
                ...
                const reapConnIfDead = () => {
                    if (mount.registry.get(connId) === conn &&
                        conn.connStream === stream &&
                        conn.connGraceExpired &&
                        !conn.hasLiveSessionStream() &&
                        !conn.hasRecoverableSession()) {
                        writeStderrLine(`LailatulCoder serve: ${mount.routeLabel} reaping connection ${connId.slice(0, 8)} (conn stream gone, no live session stream)`);
        ...
        const lastEventId = parseLastEventId(headerOf(req, 'last-event-id'), '/acp ');
        ...
        const eventEpoch = parseEventEpochHeader(headerOf(req, 'x-LailatulCoder-event-epoch'), '/acp ');
        ...
        if (busEpoch !== undefined) {
            res.setHeader('X-LailatulCoder-Event-Epoch', busEpoch);
        }
        ...
        const onPumpSettled = () => {
            if (stream.isClosed) {
                ...
                writeStderrLine(`LailatulCoder serve: ${mount.routeLabel} session stream pump ended while open ` +
                ...
                writeStderrLine(`LailatulCoder serve: ${mount.routeLabel} session stream pump settled for a superseded ` +
                    `stream (${logSafe(sessionId)}) — no-op`);
            }
        };
        void mount.dispatcher
            .pumpSessionEvents(conn, sessionId, ac.signal, lastEventId, eventEpoch)
            .then(onPumpSettled, (err) => {
            writeStderrLine(`LailatulCoder serve: ${mount.routeLabel} event pump error (${logSafe(sessionId)}, lastEventId=${lastEventId ?? 'none'}): ${logSafe(err instanceof Error ? err.message : String(err))}`);
            onPumpSettled();
        });
    };
    ...
    const createSecondaryAcpMount = (rt) => {
        const secondaryDispatcherRef = {};
        const secondaryRegistry = new ConnectionRegistry((req, clientId) => {
            if (!secondaryDispatcherRef.current)
                return false;
            return secondaryDispatcherRef.current.cancelAbandonedPermission(req, clientId);
        }, (sessionId, clientId) => {
            void rt.bridge
                .detachClient(sessionId, clientId)
                .catch((err) => {
                writeStderrLine(`LailatulCoder serve: /workspaces/${rt.workspaceId}/acp detachClient(${sessionId}) failed: ${err instanceof Error ? err.message : String(err)}`);
            });
        }, opts.maxConnections, undefined, preAttachBudget);
        const workspaceRememberLane = new WorkspaceRememberTaskLane(rt.bridge, rt.workspaceCwd);
        const registeredGeneration = opts.workspaceRegistry?.getEntryByWorkspaceId(rt.workspaceId)?.current;
        const generationGuard = registeredGeneration?.runtime === rt
            ? registeredGeneration.guard
            : rt.generationGuard;
        const secondaryDispatcher = new AcpDispatcher(rt.bridge, rt.workspaceCwd, () => runtimeEffectiveEnv(rt, daemonEnv), rt.workspaceService, workspaceRememberLane, requestedSessionIdAdmission, rt.routeFileSystemFactory, 
        ...
        opts.deviceFlowRegistry, opts.sessionShellCommandEnabled === true, secondaryRegistry, archiveCoordinator, () => rt.trusted, () => (generationGuard ? () => generationGuard.assertOpen() : undefined), rt.provenance === 'live-conversation'
            ? opts.liveSessionIsolation
            : undefined, rt.sessionRuntimeBaseDir, () => ({
            bridge: rt.bridge,
        ...
        return {
            ...
            clientMcpProviderFactory: opts.clientMcpOverWs === true
                ? (connectionId) => createClientMcpServerProvider(rt.clientMcpSenderRegistry, rt.bridge, connectionId)
                : undefined,
        };
    };
    const secondaryMounts = new Map();
    const drainingWorkspaceIds = new Set();
    const primaryWorkspaceId = opts.workspaceRegistry?.primaryEntry.workspaceId;
    const mountForWorkspace = (workspaceId) => workspaceId === primaryWorkspaceId
        ? primaryMount
        : secondaryMounts.get(workspaceId);
    const getOrCreateSecondaryMount = (rt) => {
    ...
    const resolvePluralMount = (req, res) => {
    ...
    const upgradeCredentials = opts.credentials ?? singleTokenCredentials(opts.token);
    function setupWebSocket(httpServer) {
        if (disposed || upgradeServers.has(httpServer))
            return;
        if (wss) {
        ...
        wss = new WebSocketServer({
            ...
            // `LailatulCoder-bearer.*` subprotocol (see extractUpgradeBearer). Never echo that
            ...
            // marker (`LailatulCoder-ws`) alongside the bearer one precisely so there is always
        ...
        upgradeListener = (req, socket, head) => {
            ...
            const logReject = (reason) => {
                writeStderrLine(`LailatulCoder serve: WebSocket upgrade rejected (${reason}) from ${rawAddr}`);
            };
            ...
            const isCdpPath = opts.cdpTunnelOverWs === true &&
                opts.cdpTunnelRegistry !== undefined &&
                rawPath === CDP_PATH;
            const extraRoute = opts.extraWsRoutes?.find((route) => route.path === rawPath);
            ...
            if (upgradeListenerIdentity.kind === 'local-control') {
                ...
                const browserHost = upgradeListenerIdentity.origin === undefined
                    ? undefined
            ...
            if (origin) {
                try {
                    ...
                    const isAllowlistedOrigin = opts.allowedOrigins !== undefined &&
                        opts.allowedOrigins.origins.has(origin.toLowerCase());
                    const isListenerOrigin = upgradeListenerIdentity.kind === 'local-control' &&
            ...
            if (!upgradeCredentials.isOpen(upgradeListenerIdentity)) {
                ...
                // `LailatulCoder-bearer.*` subprotocol (browsers, which can't set Authorization
            ...
            if (isPluralAcpShape) {
                ...
                const rt = wsRegistry.getManagedByWorkspaceId(selector) ??
                    (isPortableAbsolutePath(selector)
                        ? resolveManagedWorkspaceRuntimeByPathSelector(wsRegistry, selector)
                        : undefined);
                if (!rt) {
            ...
            wss.handleUpgrade(req, socket, head, (ws) => {
                ...
                const initTimer = setTimeout(() => {
                    if (!initialized) {
                        writeStderrLine(`LailatulCoder serve: ${activeMount.routeLabel} WS initialize timeout (30s) from ${rawAddr}`);
                ...
                const wsKey = rawAddr.startsWith('::ffff:')
                    ...
                    : rawAddr;
                ws.on('error', (err) => {
                    writeStderrLine(`LailatulCoder serve: ${activeMount.routeLabel} WS error: ${err instanceof Error ? err.message : String(err)}`);
                });
                ...
                ws.on('close', () => {
                    ...
                    if (cdpBridgeUnregister) {
                        ...
                        activeMount.removeChromeDevToolsMcpIfUnused(connRef?.connectionId ?? 'cdp-bridge');
                    }
                });
                ws.on('message', (rawData) => {
                    messageQueue = messageQueue
                        ...
                        .catch((err) => {
                        writeStderrLine(`LailatulCoder serve: ${activeMount.routeLabel} WS message handler error: ${err instanceof Error ? err.message : String(err)}`);
                    });
                });
                async function handleWsMessage(rawData) {
                    ...
                    try {
                        ...
                        ws.send(JSON.stringify(rpcError(null, RPC.PARSE_ERROR, 'Parse error')));
                        return;
                    }
                    ...
                    const frameType = parsed !== null && typeof parsed === 'object'
                        ? parsed.type
                        : undefined;
                    if (activeMount.draining &&
                        ...
                        const result = await clientMcp.handleFrame(parsed);
                        if (result.kind === 'message_resolved')
                            return;
                    }
                    if (activeMount.draining &&
                        !(connRef !== undefined &&
                        ...
                        if (opts.cdpTunnelOverWs === true &&
                            cdpEndpoint !== undefined &&
                            parsed !== null &&
                            typeof parsed === 'object' &&
                            isCdpInboundFrameType(frameType) &&
                            cdpEndpoint.routeInbound(parsed)) {
                            return;
                        }
                        ws.send(JSON.stringify(rpcError(isRequest(parsed) ? parsed.id : null, RPC.INTERNAL_ERROR, 'Workspace runtime is being removed', {
                            code: 'workspace_draining',
                    ...
                    if (opts.clientMcpOverWs === true &&
                        parsed !== null &&
                        typeof parsed === 'object' &&
                        ClientMcpWsConnection.isClientMcpFrameType(parsed.type)) {
                        ...
                        if (opts.checkRate) {
                            const tier = frameType === 'mcp_message' ? 'read' : 'mutation';
                            if (!opts.checkRate(workspaceRateLimitKey(activeMount, wsKey), tier)) {
                        ...
                        if (!clientMcp) {
                            ...
                            const provider = activeMount.clientMcpProviderFactory
                                ? activeMount.clientMcpProviderFactory(connRef?.connectionId ?? 'ws-unknown')
                                : opts.clientMcpProvider;
                            clientMcp = new ClientMcpWsConnection((frame) => safeWsSend(ws, JSON.stringify(frame), 'client-MCP'), provider);
                        }
                        const sendClientMcpAck = (result) => {
                        ...
                        if (dispatchOffQueue) {
                            ...
                            if (clientMcpInflightDispatch >= MAX_INFLIGHT_MCP_DISPATCH) {
                                writeStderrLine(`LailatulCoder serve: ${activeMount.routeLabel} client-MCP inflight cap hit (${MAX_INFLIGHT_MCP_DISPATCH}); rejecting ${String(frameType)} frame`);
                                safeWsSend(ws, JSON.stringify({
                                    type: 'mcp_error',
                        ...
                        const handleP = clientMcp
                            .handleFrame(parsed)
                            .then(sendClientMcpAck, (err) => {
                            const message = err instanceof Error ? err.message : String(err);
                            writeStderrLine(`LailatulCoder serve: ${activeMount.routeLabel} client-mcp frame error: ${message}`);
                    ...
                    if (opts.cdpTunnelOverWs === true &&
                        ...
                        parsed !== null &&
                        typeof parsed === 'object' &&
                        isCdpInboundFrameType(parsed.type)) {
                        cdpEndpoint.routeInbound(parsed);
                        return;
                    }
                    ...
                    if (!initialized) {
                        if (!isRequest(message) || message.method !== 'initialize') {
                            ws.send(JSON.stringify(rpcError(isRequest(message) ? message.id : null, RPC.INVALID_REQUEST, 'First message must be initialize')));
                            ws.close(1002, 'Protocol error');
                            return;
                        }
                        const conn = activeMount.registry.create(fromLoopback);
                        if (!conn) {
                            ws.send(JSON.stringify(rpcError(message.id, RPC.INTERNAL_ERROR, 'Too many connections')));
                        ...
                        const stream = new WsStream(ws, () => {
                            writeStderrLine(`LailatulCoder serve: ${activeMount.routeLabel} WS closed (${conn.connectionId.slice(0, 8)})`);
                        ...
                        ws.send(JSON.stringify({
                            jsonrpc: '2.0',
                            id: message.id,
                            result: activeMount.dispatcher.buildInitializeResult(conn.connectionId, requestedVersion),
                        }));
                        initialized = true;
                        activeMount.pendingWebSockets.delete(ws);
                        clearTimeout(initTimer);
                        connRef = conn;
                        writeStderrLine(`LailatulCoder serve: ${activeMount.routeLabel} WS established ${conn.connectionId.slice(0, 8)} (loopback=${fromLoopback}, active=${activeMount.registry.size})`);
                        ...
                        const clientName = message.params &&
                            typeof message.params === 'object' &&
                            !Array.isArray(message.params)
                            ? message.params['clientInfo']?.name
                            : undefined;
                        if (activeMount.primary &&
                            opts.cdpTunnelOverWs === true &&
                            opts.cdpTunnelRegistry !== undefined &&
                            clientName === CDP_BRIDGE_CLIENT_NAME) {
                            cdpEndpoint = {
                                connectionId: conn.connectionId,
                                send: (frame) => safeWsSend(ws, JSON.stringify(frame), 'CDP'),
                                routeInbound: () => false,
                            };
                            cdpBridgeUnregister =
                                opts.cdpTunnelRegistry.register(cdpEndpoint);
                            activeMount.ensureChromeDevToolsMcpRegistered(localPort, conn.connectionId);
                            writeStderrLine(`LailatulCoder serve: ${activeMount.routeLabel} connection ${conn.connectionId.slice(0, 8)} registered as CDP bridge`);
                        }
                        return;
                    }
                    // Subsequent messages
                    ...
                    if (!conn || conn.destroyed) {
                        ws.send(JSON.stringify(rpcError(null, RPC.INTERNAL_ERROR, 'Connection lost')));
                    ...
                    if (isRequest(message) &&
                        message.params &&
                        typeof message.params === 'object') {
                        const rawSid = message.params['sessionId'];
                        const sid = typeof rawSid === 'string'
                        ...
                        if (typeof sid === 'string' && conn.ownsSession(sid)) {
                            ...
                            if (binding &&
                                ...
                                void activeMount.dispatcher
                                    ...
                                    .then(cleanupSession, (err) => {
                                    writeStderrLine(`LailatulCoder serve: ${activeMount.routeLabel} WS pump error (${sid}): ${err instanceof Error ? err.message : String(err)}`);
                                    cleanupSession();
                                });
                            }
                        }
                        ...
                        if (WS_EXEMPT_METHODS.has(m)) {
                            ...
                            const tier = m === 'session/prompt' || m === '_qwen/session/prompt'
                                ...
                                : WS_READ_METHODS.has(m)
                                    ...
                                    : 'mutation';
                            if (!opts.checkRate(workspaceRateLimitKey(activeMount, wsKey), tier)) {
                                ws.send(JSON.stringify(rpcError(message.id, RPC.INTERNAL_ERROR, 'Rate limit exceeded')));
                                return;
                            }
                        }
                    }
                    ...
                    const isPrompt = isRequest(message) &&
                        (message.method === 'session/prompt' ||
                            message.method === '_qwen/session/prompt');
                    const dispatchP = activeMount.dispatcher
                        .handle(conn, message, undefined, fromLoopback)
                        .catch((err) => {
                        writeStderrLine(`LailatulCoder serve: ${activeMount.routeLabel} WS handle error: ${err instanceof Error ? err.message : String(err)}`);
                    });
                    if (!isPrompt)
                        await dispatchP;
                }
        ...
        httpServer.on('upgrade', upgradeListener);
        writeStderrLine(`LailatulCoder serve: /acp WebSocket transport enabled on ${path}`);
    }
    return {
        dispose: () => {
            if (disposed)
        ...
        getWorkspaceActivity: (workspaceId) => {
            const mount = mountForWorkspace(workspaceId);
            return {
                acpConnections: (mount?.registry.size ?? 0) + (mount?.pendingWebSockets.size ?? 0),
        ...
        ensureWorkspaceRememberLane: (workspaceId) => {
            ...
            if (existing)
                return existing.workspaceRememberLane;
        ...
        getWorkspaceRememberLane: (workspaceId) => mountForWorkspace(workspaceId)?.workspaceRememberLane,
        ...
        getSnapshot: () => {
            ...
            return {
                connectionCount: perMount.reduce((n, m) => n + m.snap.connectionCount, 0),
                connectionStreams: perMount.reduce((n, m) => n + m.snap.connectionStreams, 0),
                ...
                pendingClientRequests: perMount.reduce((n, m) => n + m.snap.pendingClientRequests, 0),
                bufferedConnectionFrames: perMount.reduce((n, m) => n + m.snap.bufferedConnectionFrames, 0),
                bufferedSessionFrames: perMount.reduce((n, m) => n + m.snap.bufferedSessionFrames, 0),
                ...
                connections: perMount.flatMap((mount) => mount.snap.connections.map((connection) => ({
                    ...connection,
        ...
        detachServer(server) {
            if (!upgradeServers.delete(server))
                return;
            if (upgradeListener)
                server.removeListener('upgrade', upgradeListener);
