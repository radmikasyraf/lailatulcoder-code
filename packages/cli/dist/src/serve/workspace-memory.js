...
import { Storage, WorkspaceMemoryFileTooLargeError, WorkspaceMemoryWriteTimeoutError, getAllGeminiMdFilenames, writeWorkspaceContextFile, } from '@lailatul-coder/lailatul-coder-core';
import { writeStderrLine } from '../utils/stdioHelpers.js';
import { isServeDebugMode } from './debug-mode.js';
import { createIdleWorkspaceMemoryStatus, STATUS_SCHEMA_VERSION, } from '@lailatul-coder/acp-bridge/status';
...
function captureOpenGeneration(deps, res) {
...
function sendWorkspaceMemoryWriteError(res, err, options) {
    ...
    if (err instanceof WorkspaceMemoryWriteTimeoutError) {
        writeStderrLine(`LailatulCoder serve: ${route} timeout — file lock at ` +
    ...
    if (err instanceof WorkspaceMemoryFileTooLargeError) {
        writeStderrLine(`LailatulCoder serve: ${route} refused — existing file ` +
    ...
    writeStderrLine(`LailatulCoder serve: ${route} failed (scope=${scope} mode=${mode}): ${err instanceof Error ? (err.stack ?? err.message) : String(err)}`);
...
export function mountWorkspaceMemoryRoutes(app, deps) {
    app.get('/workspace/memory', async (_req, res) => {
        ...
        try {
            ...
            writeStderrLine(`LailatulCoder serve: GET /workspace/memory failed: ${err instanceof Error ? (err.stack ?? err.message) : String(err)}`);
            res.status(500).json({
...
export function mountWorkspaceQualifiedMemoryRoutes(app, deps) {
    app.get('/workspaces/:workspace/memory', async (req, res) => {
        ...
        try {
            ...
            if (sendGenerationClosedError(res, err))
                return;
            writeStderrLine(`LailatulCoder serve: GET /workspaces/:workspace/memory failed: ${err instanceof Error ? (err.stack ?? err.message) : String(err)}`);
            res.status(500).json({
                error: 'Failed to discover workspace memory',
                code: 'memory_discovery_failed',
            });
    ...
    app.post('/workspaces/:workspace/memory', deps.mutate({ strict: true }), async (req, res) => {
        const runtime = resolveWorkspaceRuntimeFromParam(deps.workspaceRegistry, req, res);
        if (!runtime || !requireTrustedWorkspaceRuntime(runtime, res))
            return;
        ...
        if (body['scope'] !== 'workspace') {
            res.status(400).json({
                error: 'workspace-qualified memory routes only support "workspace" scope',
...
/**
 * Filesystem-only discovery of explicit `LAILATUL.md` / `AGENTS.md`
 * files reachable from the daemon's bound workspace plus the user's
 * `~/.lailatulcoder/` global directory.
 *
 ...
 * based rules (`.lailatulcoder/rules/`) are also out of scope for v1.
 */
export async function collectWorkspaceMemoryStatus(boundWorkspace) {
...
/**
 * Stat each known memory filename (`LAILATUL.md`, `AGENTS.md`) at the
