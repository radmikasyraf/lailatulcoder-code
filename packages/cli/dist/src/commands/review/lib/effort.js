 ...
 */
export function resolveEffort(explicit) {
    if (explicit && EFFORT_LEVELS.has(explicit))
        return explicit;
    try {
        const parsed = JSON.parse(readFileSync(PARSE_ARGS_REPORT, 'utf8'));
 ...
 */
export function planEffortField(explicit) {
    ...
    writeStderrLine(`effort: ${effort} (from ${explicit && EFFORT_LEVELS.has(explicit) ? '--effort' : 'parse-args report'})`);
    return { effort };
}
