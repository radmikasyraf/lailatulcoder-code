...
import { mkdtempSync, realpathSync, rmSync, existsSync, writeFileSync, mkdirSync, symlinkSync, } from 'node:fs';
...
describe('releaseWorktree', () => {
    ...
    it('removes an unregistered non-empty leftover git no longer tracks', () => {
        ...
        expect(() => git('worktree', 'add', '-q', 'wt', '-b', 'topic')).not.toThrow();
    });
    it('frees a path whose directory was deleted by hand', () => {
        ...
        expect(() => git('worktree', 'add', 'wt', 'topic')).toThrow(/missing but already registered/);
    ...
    it('unlocks the branch a phantom worktree still holds checked out', () => {
        ...
        rmSync(join(repo, 'wt'), { recursive: true, force: true });
        expect(() => git('branch', '-D', 'qwen-review/pr-1')).toThrow(/used by worktree|checked out/);
    ...
    it('unlinks a symlink at the path instead of releasing the worktree it points at', () => {
        ...
        expect(git('worktree', 'list')).toContain(join(realpathSync(repo), 'victim'));
    ...
    it('refuses to release through an ANCESTOR symlink, which lstat cannot see', () => {
        ...
        expect(git('worktree', 'list')).toContain(join(realpathSync(repo), 'real', 'victim'));
    ...
    it('unlinks a DANGLING symlink, which existsSync cannot see', () => {
        ...
        expect(() => git('worktree', 'add', '-q', 'wt-link', '-b', 'topic2')).not.toThrow();
    });
});
describe('gitRawTolerateDiff', () => {
    it('returns the diff when git exits 1 because the inputs differ', () => {
        ...
        const out = gitRawTolerateDiff('-C', repo, 'diff', '--no-index', '--', NULL_DEVICE, 'new.ts');
    ...
    it('throws when git exits 1 with NO output — that is a failure, not a diff', () => {
        ...
        expect(() => gitRawTolerateDiff('-C', repo, 'diff', '--no-index', '--', NULL_DEVICE, 'subdir')).toThrow();
    });
});
describe('gitProbe — the exit status the anchor taxonomy rests on', () => {
    ...
    it('reports 0, the predicate NO, and an error apart from each other', () => {
        ...
        try {
            ...
            // definitive no.
            expect(gitProbe('-C', repo, 'cat-file', '-e', '0'.repeat(40)).status).toBe(1);
            ...
            expect(gitProbe('-C', repo, 'cat-file', '-e', '0000000').status).toBe(128);
            ...
            expect(gitProbe('-C', repo, 'merge-base', '--is-ancestor', head, head).status).toBe(0);
            expect(gitProbe('-C', repo, 'merge-base', '--is-ancestor', head, newer).status).toBe(0);
            // 1: no — the newer commit is not an ancestor of the older.
            expect(gitProbe('-C', repo, 'merge-base', '--is-ancestor', newer, head).status).toBe(1);
            ...
            expect(gitProbe('-C', repo, 'merge-base', '--is-ancestor', '0'.repeat(40), head).status).toBe(128);
            ...
            expect(gitProbe('-C', repo, 'rev-parse', `${head}^{commit}`).out).toBe(head);
