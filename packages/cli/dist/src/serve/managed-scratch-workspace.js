...
export function isManagedScratchChild(canonicalCwd, canonicalRoot) {
    return (isSamePath(dirname(canonicalCwd), canonicalRoot) &&
 ...
 */
export function isScratchRootCompatible(canonicalCwd, canonicalRoot) {
    if (isSamePath(canonicalCwd, canonicalRoot))
        return false;
    if (isManagedScratchChild(canonicalCwd, canonicalRoot))
        return true;
    return (!isWithinRoot(canonicalCwd, canonicalRoot) &&
...
function validateRootStats(stats) {
    ...
    if (process.platform !== 'win32' && (stats.mode & 0o077) !== 0) {
        throw new Error('Managed scratch root must be accessible only to its owner');
    }
}
/**
 ...
 */
export function prepareManagedScratchRoot(root, startupWorkspaceCwds) {
    ...
    if (startupWorkspaceCwds.some((cwd) => !isScratchRootCompatible(cwd, canonicalRoot))) {
...
async function revalidateManagedScratchRoot(root) {
...
export async function createManagedScratchDirectory(root) {
...
//# sourceMappingURL=managed-scratch-workspace.js.map