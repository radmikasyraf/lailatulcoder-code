...
import { INLINE_LAYER_WALKED_RE, LAYER_RECEIPT_LINE_RE, } from './audit-layers.js';
import { deliveredVerbatimLines, findingsPointerOf, flattenPrompt, promptLines, promptRecordDir, readRecordedPrompts, } from './prompt-record.js';
 ...
 * never consulted.
 */
export function bakedRanges(prompt, diffPath) {
    const out = [];
    if (diffPath === undefined)
        return out;
    for (const m of prompt.matchAll(/read_file\(\s*file_path="([^"]*)",\s*offset=(\d+),\s*limit=(\d+)/gi)) {
        if (m[1] !== diffPath)
            continue;
...
const DRY_RECEIPT_ZH = '|未发现(?:新的?)?(?:问题|发现)' +
    '|无新的?(?:问题|发现)' +
    '|没有(?:发现)?(?:新的?)?问题';
/**
 ...
 */
const DRY_RECEIPT_PHRASE = '(?:' + DRY_RECEIPT_EN + '[ \\w()]{0,32}' + DRY_RECEIPT_ZH + ')';
/**
 ...
 */
const DRY_RECEIPT_RE = new RegExp('^[ \\t]*[*_~]*' +
    DRY_RECEIPT_PHRASE +
    DRY_RECEIPT_TAIL +
    '(?:[—–]+|[:：.,;。，；]|--+|-+[ \\t])[ \\t]*' +
    '([^\\n]*)', 'i');
 ...
 */
const EXAMPLE_RECEIPT_CLAUSE = (DRY_RECEIPT_RE.exec(REVERSE_AUDIT_EXAMPLE_RECEIPT)?.[1] ?? '').trim();
 ...
 */
const NEGATION_MARKER_RE = /\bnot\b|n['’]t\b|\bnever\b|\bno\b|\bcannot\b|\bunable\b|\bfail(?:ed|ing|s)?\b|\bskip(?:ped|ping|s)?\b|\bskim(?:med|ming|s)?\b|\bun(?:checked|tested|verified|read|opened|examined)\b|未|没|无法|跳过|不(?!过)/i;
/**
 ...
 */
const WALK_VERB_SRC = '\\bwalk|\\bverif|\\btrace|\\bexamin|走查|核对|复核|核查|复查|重走';
 ...
 */
function namesAnObject(clause) {
    return (/`[^`]+`/.test(clause) ||
        /\w[\w.-]+\/[\w.$-]+\/\w/.test(clause) ||
        /\w[\w.-]+\/[\w$-]+\.\w+/.test(clause));
}
function namesTheWalk(clause) {
 ...
 */
function substantiveClause(clause) {
    const c = clause.replace(/\s+/g, ' ').trim();
    if (c.length === 0)
        return false;
    if (EXAMPLE_RECEIPT_CLAUSE_LC.length > 0 &&
        c.toLowerCase().includes(EXAMPLE_RECEIPT_CLAUSE_LC)) {
        return false;
    }
    const stripped = c.replace(SATURATED_CLAUSE_RE, ' ').trim();
    if (namesAnObject(stripped))
        return true;
    if ((stripped.match(CJK_RE) ?? []).length >= 4)
        return true;
 ...
 */
function findingsListFor(prompt, recordDir, memo) {
 ...
 * way (#9213).
 */
function stripLayerReceiptLines(finalText) {
    const kept = [];
    let inFence = false;
 ...
 */
function classifyReturn(rec, territory, findingsList, findingsRead) {
    ...
    if (SEVERITY_LINE_RE.test(text)) {
        ...
        for (const m of text.matchAll(FILE_LINE_RE)) {
            const file = (m[1] ?? '').trim();
            if (file === '' || /^N\/A\b/i.test(file))
                continue;
            if (findingsList.includes(`**File:** ${file}`))
                continue;
    ...
    const cutAt = Math.min(inlineGap?.index ?? clause.length, inlineLayer?.index ?? clause.length);
    ...
    if (rec.successfulToolCalls === 0)
        return unknown('no successful tool calls');
    if (rec.diffToolCalls === 0)
        return unknown('no read of the diff');
    if (!openedTheTerritory(rec.diffReads, territory))
        return unknown('territory read missing');
    if (receipt === null)
        return unknown('receipt not matched');
    ...
    // with a live finding.
    if (!findingsRead)
        return unknown('findings list unread');
 ...
 */
export function openedTheTerritory(diffReads, territory) {
    if (territory.length === 0)
        return true;
    return diffReads.some(([s, e]) => territory.some(([ts, te]) => s <= te && ts <= e));
}
/**
...
function mergeOutcomes(outcomes) {
    if (outcomes.includes('yielded'))
        return 'yielded';
    if (outcomes.includes('dry'))
        return 'dry';
    return 'unknown';
}
/**
 ...
 * not a schedule.
 */
export function scheduleReverseAuditRound(planPath, chunkIds, round, env = process.env, diffPath) {
    ...
    const matchesByRecord = records.map((rec) => candidates
        .filter((c) => deliveredVerbatimLines(c.flat, rec.lines))
        .map((c) => c.transcript));
    const recordsPerTranscript = new Map();
    ...
    matchesByRecord.forEach((matches, i) => {
        ...
        classifyReturn(t, records[i].territory, records[i].findings, readFindingsPointer(t, records[i].pointer)));
        ...
        const failures = [];
        if (matches.length === 0)
            failures.push('no matching transcript');
    ...
    for (const chunkId of chunkIds) {
        ...
        const retired = lastTwo.length === 2 && lastTwo.every((a) => a.outcome === 'dry');
        if (!retired) {
            ...
            const certifiable = lastTwo.length === 2 && lastTwo.every((a) => a.outcome !== 'yielded');
            if (certifiable) {
