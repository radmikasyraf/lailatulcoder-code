...
describe('SystemController', () => {
    describe('initialize timeout validation', () => {
        it('accepts valid timeout within bounds', async () => {
            ...
            const controller = new SystemController(context, createRegistry(), 'SystemController');
    ...
    describe('continue_last_turn', () => {
        it('delegates to the session callback and merges its payload', async () => {
            ...
            const controller = new SystemController(createContext({ onContinueLastTurn }), createRegistry(), 'SystemController');
            const result = await controller.handleRequest({ subtype: 'continue_last_turn' }, 'continue-1');
        ...
        it('fails loudly when no session callback is registered', async () => {
            const controller = new SystemController(createContext(), createRegistry(), 'SystemController');
            await expect(controller.handleRequest({ subtype: 'continue_last_turn' }, 'continue-2')).rejects.toThrow(/was not registered on ControlContext/);
        });
    });
    describe('set_effort', () => {
        it('sets effort and returns applied=true when read-back matches', async () => {
            const context = createContext();
            context.config.getReasoningEffort.mockReturnValue('high');
            const controller = new SystemController(context, createRegistry(), 'SystemController');
            const result = await controller.handleRequest({ subtype: 'set_effort', effort: 'high' }, 'effort-1');
        ...
        it('returns applied=false when thinking is disabled (read-back mismatch)', async () => {
            const context = createContext();
            context.config.getReasoningEffort.mockReturnValue('medium');
            const controller = new SystemController(context, createRegistry(), 'SystemController');
            const result = await controller.handleRequest({ subtype: 'set_effort', effort: 'high' }, 'effort-2');
            expect(result).toEqual({
                subtype: 'set_effort',
                effort: 'high',
                applied: false,
        ...
        it('returns the higher-priority wire override', async () => {
            const context = createContext();
            context.config.getReasoningEffort.mockReturnValue('high');
            context.config.getReasoningEffortOverride.mockReturnValue({
                source: 'samplingParams',
                field: 'enable_thinking',
            });
            const controller = new SystemController(context, createRegistry(), 'SystemController');
            const result = await controller.handleRequest({ subtype: 'set_effort', effort: 'high' }, 'effort-override');
            expect(result).toEqual({
                subtype: 'set_effort',
                effort: 'high',
                applied: false,
        ...
        it('rejects invalid effort value', async () => {
            const controller = new SystemController(createContext(), createRegistry(), 'SystemController');
            await expect(controller.handleRequest({ subtype: 'set_effort', effort: 'banana' }, 'effort-3')).rejects.toThrow('Invalid effort value');
        });
        it('rejects empty effort string', async () => {
            const controller = new SystemController(createContext(), createRegistry(), 'SystemController');
            await expect(controller.handleRequest({ subtype: 'set_effort', effort: '  ' }, 'effort-4')).rejects.toThrow('Invalid effort specified');
        });
    });
    describe('get_available_models', () => {
        it('returns models without exposing baseUrl or envKey', async () => {
            const context = createContext();
            context.config.getAvailableModels.mockReturnValue([
                {
                    id: 'qwen-max',
                    label: 'Qwen Max',
            ...
            const controller = new SystemController(context, createRegistry(), 'SystemController');
            const result = await controller.handleRequest({ subtype: 'get_available_models' }, 'models-1');
            expect(result).toEqual({
        ...
        it('returns empty models list when none available', async () => {
            const controller = new SystemController(createContext(), createRegistry(), 'SystemController');
            const result = await controller.handleRequest({ subtype: 'get_available_models' }, 'models-2');
            expect(result).toEqual({
                subtype: 'get_available_models',
                models: [],
            });
    ...
    describe('get_usage_info', () => {
        it('returns dashboard with subtype when range is provided', async () => {
            const controller = new SystemController(createContext(), createRegistry(), 'SystemController');
            const result = await controller.handleRequest({ subtype: 'get_usage_info', range: 'week' }, 'usage-1');
        ...
        it('returns dashboard without range filter', async () => {
            const controller = new SystemController(createContext(), createRegistry(), 'SystemController');
            const result = await controller.handleRequest({ subtype: 'get_usage_info' }, 'usage-2');
            expect(result).toHaveProperty('subtype', 'get_usage_info');
            expect(result).toHaveProperty('generatedAt');
        });
    });
    describe('initialize with effort', () => {
        it('sets effort during initialize when provided', async () => {
            ...
            context.config.getReasoningEffort.mockReturnValue('high');
            const controller = new SystemController(context, createRegistry(), 'SystemController');
            const result = await controller.handleRequest({ subtype: 'initialize', effort: 'high' }, 'init-effort-1');
            expect(context.config.setReasoningEffort).toHaveBeenCalledWith('high');
        ...
        it('warns when effort not applied during initialize (thinking disabled)', async () => {
            ...
            context.config.getReasoningEffort.mockReturnValue('medium');
            const controller = new SystemController(context, createRegistry(), 'SystemController');
            const result = await controller.handleRequest({ subtype: 'initialize', effort: 'high' }, 'init-effort-2');
            expect(context.config.setReasoningEffort).toHaveBeenCalledWith('high');
            ...
            expect(mockDebugLogger.warn).toHaveBeenCalledWith("[SystemController] Effort 'high' was not applied (thinking may be disabled)");
        });
        it('reports the higher-priority override during initialize', async () => {
            ...
            const controller = new SystemController(context, createRegistry(), 'SystemController');
            const result = await controller.handleRequest({ subtype: 'initialize', effort: 'high' }, 'init-effort-override');
            expect(result).toHaveProperty('effort_status', {
                effort: 'high',
                applied: false,
                override: {
            ...
            expect(mockDebugLogger.warn).toHaveBeenCalledWith("[SystemController] Effort 'high' was not applied (samplingParams.enable_thinking takes precedence)");
        });
        it('joins every held cause in the effort_status reason during initialize', async () => {
            ...
            const controller = new SystemController(context, createRegistry(), 'SystemController');
            const result = await controller.handleRequest({ subtype: 'initialize', effort: 'high' }, 'init-effort-both-causes');
            expect(result).toHaveProperty('effort_status', {
                effort: 'high',
                applied: false,
                override: {
            ...
            expect(mockDebugLogger.warn).toHaveBeenCalledWith("[SystemController] Effort 'high' was not applied (thinking may be disabled; extra_body.thinking_budget takes precedence)");
        });
        it('rejects invalid effort during initialize', async () => {
            const controller = new SystemController(createContext(), createRegistry(), 'SystemController');
            await expect(controller.handleRequest({ subtype: 'initialize', effort: 'banana' }, 'init-effort-3')).rejects.toThrow('Invalid effort value');
        });
    });
});
