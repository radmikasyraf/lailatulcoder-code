 ...
 */
export function resolveProviderProtocol(providerId, providerProtocol) {
    const explicit = providerProtocol && Object.hasOwn(providerProtocol, providerId)
 ...
 */
export function modelRegistryKey(id, baseUrl) {
...
export class ModelRegistry {
    ...
    getDefaultBaseUrl(authType) {
        switch (authType) {
            case AuthType.lailatulcoder_OAUTH:
    ...
    constructor(modelProvidersConfig, providerProtocolConfig) {
        ...
        this.registerAuthTypeModels(AuthType.lailatulcoder_OAUTH, QWEN_OAUTH_MODELS);
    ...
    registerProvidersConfig(modelProvidersConfig) {
        ...
        for (const [providerId, models] of Object.entries(modelProvidersConfig)) {
            const protocol = resolveProviderProtocol(providerId, this.providerProtocolConfig);
            if (!protocol) {
                ...
                const mapped = Object.hasOwn(this.providerProtocolConfig, providerId)
                    ...
                    : undefined;
                const message = mapped !== undefined
            ...
            if (protocol === AuthType.lailatulcoder_OAUTH) {
                continue;
            }
    ...
    registerAuthTypeModels(authType, models, providerId) {
        ...
        if (!Array.isArray(models)) {
            debugLogger.warn(`modelProviders for provider "${providerId ?? authType}" is not an array; ` +
        ...
        // (id + baseUrl) key wins.
        const modelMap = this.modelsByAuthType.get(authType) ??
            new Map();
        const providerLabel = providerId && providerId !== authType
            ? ` (provider "${providerId}")`
            : '';
        for (const config of models) {
            const key = modelRegistryKey(config.id, config.baseUrl);
            if (modelMap.has(key)) {
                debugLogger.warn(`Duplicate model id "${config.id}"${config.baseUrl ? ` with baseUrl "${config.baseUrl}"` : ''} for protocol "${authType}"${providerLabel}. Using the first registered config.`);
                continue;
            }
    ...
    validateModelConfig(config, authType) {
        ...
        if (selectorOnlyCount > 1) {
            debugLogger.warn(`Model "${config.id}" in authType "${authType}" has multiple selector-only flags. It will be unreachable in at least one model selector.`);
        }
    }
    /**
