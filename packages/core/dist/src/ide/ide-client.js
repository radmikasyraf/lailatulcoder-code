...
export class IdeClient {
    ...
    ideProcessInfo;
    connectionConfig;
    workspaceRejectedPorts = new Set();
    authToken;
    ...
    async connect() {
        if (!this.currentIde) {
            this.setState(IDEConnectionStatus.Disconnected, `IDE integration is not supported in your current environment. To use this feature, run LailatulCoder Ai in one of these supported IDEs: VS Code or VS Code forks`, false);
            return;
        }
        ...
        if (this.workspaceRejectedPorts.size > 0) {
            this.setState(IDEConnectionStatus.Disconnected, `Found IDE companion extension, but its workspace does not match the current directory. Run LailatulCoder Ai from the workspace open in your IDE, or switch the IDE to this project.`, true);
            return;
        }
        ...
        const promise = new Promise((resolve, reject) => {
            ...
            this.client
                ...
                if (parsedResultData.isError) {
                    ...
                    debugLogger.debug(`Request for openDiff ${filePath} failed with isError:`, errorMessage);
    ...
    async closeDiff(filePath, options) {
        try {
            ...
            if (resultData.isError) {
                ...
                debugLogger.debug(`Request for closeDiff ${filePath} failed with isError:`, errorMessage);
                return undefined;
            }
            const textPart = resultData.content.find((part) => part.type === 'text');
            if (textPart?.text) {
                try {
                    ...
                    debugLogger.debug(`Invalid JSON in closeDiff response for ${filePath}:`, textPart.text);
                }
            }
        }
        catch (err) {
    ...
    async discoverTools() {
        ...
        try {
            ...
            if (this.availableTools.length > 0) {
                debugLogger.debug(`Discovered ${this.availableTools.length} tools from IDE: ${this.availableTools.join(', ')}`);
            }
            else {
                debugLogger.debug('IDE supports tool discovery, but no tools are available.');
            }
        }
        catch (error) {
    ...
    static validateWorkspacePath(ideWorkspacePath, cwd) {
        ...
        if (!isWithinWorkspace) {
            return {
                isValid: false,
                error: `Directory mismatch. LailatulCoder Ai is running in a different location than the open workspace in the IDE. Please run the CLI from one of the following directories: ${ideWorkspacePaths.join(', ')}`,
            };
        }
        return { isValid: true };
    }
    static parseWorkspacePathEnv(value) {
        if (value.trimStart().startsWith('[')) {
            try {
                const parsed = JSON.parse(value);
                if (Array.isArray(parsed) &&
                    parsed.every((item) => typeof item === 'string')) {
                    return parsed.filter((item) => item.length > 0 && path.isAbsolute(item));
                }
            }
            catch {
    ...
    static matchesCurrentWorkspace(config, cwd) {
        return (config.workspacePath === undefined ||
        ...
        if (argsStr) {
            try {
                ...
                if (Array.isArray(parsedArgs)) {
                    ...
                    debugLogger.error('QWEN_CODE_IDE_SERVER_STDIO_ARGS must be a JSON array string.');
                }
            }
            catch (e) {
                debugLogger.error('Failed to parse QWEN_CODE_IDE_SERVER_STDIO_ARGS:', e);
            }
        }
        return { command, args };
    }
    async getConnectionConfigFromFile() {
        ...
        if (portFromEnv) {
            try {
                ...
                debugLogger.debug(`Ignoring IDE env lock file: workspace "${config.workspacePath}" does not match cwd "${cwd}".`);
            }
            catch (_) {
    ...
    async getLegacyConnectionConfig(portFromEnv) {
        if (this.ideProcessInfo) {
            try {
                const portFile = path.join(os.tmpdir(), `lailatul-coder-ide-server-${this.ideProcessInfo.pid}.json`);
                const portFileContents = await fs.promises.readFile(portFile, 'utf8');
                return {
                    config: JSON.parse(portFileContents),
                    source: 'pid',
                };
            }
            catch (_) {
                // For older/newer extension versions, the file name matches the pattern
                // /^lailatul-coder-ide-server-${pid}-\d+\.json$/. If multiple IDE
        ...
        if (portFromEnv) {
            try {
                const portFile = path.join(os.tmpdir(), `lailatul-coder-ide-server-${portFromEnv}.json`);
                const portFileContents = await fs.promises.readFile(portFile, 'utf8');
                return {
                    config: JSON.parse(portFileContents),
                    source: 'env-port',
    ...
    async getAllConnectionConfigs(ideDir) {
        ...
        const parsedLockFiles = fileContents.filter((lockFile) => lockFile !== undefined);
        const activeLockFiles = await Promise.all(parsedLockFiles.map(async (lockFile) => ({
            lockFile,
            isStale: await this.cleanupStaleLockFile(lockFile),
        })));
        const staleCount = activeLockFiles.filter(({ isStale }) => isStale).length;
        if (staleCount > 0) {
            debugLogger.debug(`[cleanupStaleLockFiles] Cleaned up ${staleCount} stale lock file(s)`);
        }
    ...
    async cleanupStaleLockFile({ file, fullPath, parsed, }) {
        try {
            if (parsed.ppid) {
                try {
                    ...
                    debugLogger.debug(`[cleanupStaleLockFiles] Removing lock file "${file}" - ppid ${parsed.ppid} no longer exists`);
            ...
            if (parsed.workspacePath) {
                ...
                debugLogger.debug(`[cleanupStaleLockFiles] Removing lock file "${file}" - workspace doesn't exist`);
                await fs.promises.unlink(fullPath);
                return true;
            }
            return false;
            ...
            debugLogger.debug(`[cleanupStaleLockFiles] Error checking lock file "${file}":`, error);
            return false;
        }
    }
    async tryFallbackPorts() {
        ...
        const configs = await this.getAllConnectionConfigs(Storage.getGlobalIdeDir());
        ...
        for (const config of configs) {
            ...
            else if (IdeClient.validateWorkspacePath(config.workspacePath, cwd).isValid) {
    ...
    createProxyAwareFetch(ideHost) {
        ...
        const undiciReady = loadUndici().then(({ EnvHttpProxyAgent, fetch: undiciFetch }) => ({
    ...
    async establishHttpConnection(port) {
        ...
        if (ideHost === CONTAINER_HOST) {
            debugLogger.debug(`Connection to ${LOCAL_HOST}:${port} failed, retrying with ${CONTAINER_HOST}`);
            return this.tryHttpConnect(port, CONTAINER_HOST);
        }
        return false;
    }
    async tryHttpConnect(port, host) {
        let transport;
        try {
            debugLogger.debug(`Attempting to connect to IDE via HTTP at ${host}:${port}`);
            this.client = new Client({
                name: 'streamable-http-client',
                // TODO(#3487): use the CLI version here.
                version: '1.0.0',
            });
            transport = new StreamableHTTPClientTransport(new URL(`http://${host}:${port}/mcp`), {
 ...
 * @internal
 */
export function _resetCachedIdeServerHost() {
 ...
 */
async function resolveIdeServerHost() {
    ...
    debugLogger.debug('Container detected, but host.docker.internal is NOT reachable, falling back to 127.0.0.1');
    return LOCAL_HOST;
}
export async function getIdeServerHost() {
    if (!hostPromise) {
