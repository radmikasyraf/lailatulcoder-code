...
import { fetchWithPolicy, isConnectionLevelError, isPrivateHost, } from '../utils/fetch.js';
import { MAX_SESSION_BYTES } from '../utils/truncation.js';
import { formatByteSize, isBinaryContentType, looksLikeText, persistBinaryContent, sniffFileKind, } from '../utils/binary-content.js';
...
export function sideQueryTimeoutMs() {
    const value = parsePositiveIntegerEnv(process.env['QWEN_WEB_FETCH_PROCESSING_TIMEOUT_MS'], SIDE_QUERY_TIMEOUT_MS);
...
let cachesByStorage = new WeakMap();
 ...
 */
export function rewriteGitHubBlobUrl(url) {
    try {
...
- prompt: "${this.params.prompt}"`;
    ...
    buildMetadataHeader(entry) {
        const urlLine = entry.finalUrl !== this.params.url
...
Status: ${entry.status} ${entry.statusText || 'OK'} | Content-Type: ${entry.contentType || 'unknown'} | Size: ${entry.byteLength.toLocaleString('en-US')} bytes`;
    ...
    async fetchAndProcess(signal) {
        ...
        this.debugLogger.debug(`[WebFetchTool] Fetching ${fetchUrl} (Accept: ${acceptHeader})`);
        const policyOptions = {
        ...
        try {
            ...
            if (upgradedFrom && isConnectionLevelError(error)) {
                this.debugLogger.debug(`[WebFetchTool] https upgrade failed (${error.message}); retrying original ${upgradedFrom}`);
        ...
        const sniff = sniffFileKind(response.body, contentType, response.contentDisposition, response.finalUrl);
        ...
        let isBinary = isBinaryContentType(contentType) ||
            sniff.magicMatched ||
            (!contentType &&
        ...
        if (isBinary) {
            ...
            if (budgetUsed + response.body.length > MAX_SESSION_BYTES) {
                throw new Error(`Fetched ${response.body.length} bytes of binary content (${contentType || 'unknown content type'}) but the session's tool-result disk budget is exhausted (${budgetUsed} bytes used of ${MAX_SESSION_BYTES}).`);
            }
            this.config.trackToolResultBytes(response.body.length);
            const persistId = `webfetch-${Date.now()}-${Math.random().toString(36).slice(2, 8)}`;
            const persisted = await persistBinaryContent(response.body, sniff.extension, this.config.storage.getToolResultsDir(), persistId);
            if ('error' in persisted) {
                this.config.trackToolResultBytes(-response.body.length);
                this.debugLogger.error(`[WebFetchTool] Failed to persist binary content: ${persisted.error}`);
                ...
                throw new Error(`Fetched ${response.body.length} bytes of binary content (${contentType || 'unknown content type'}) but failed to save it: ${persisted.error}`);
            }
            ...
            if (sniff.extension === 'pdf' && persistedPath) {
                ...
                if (pdfText.success && pdfText.text.trim()) {
                    ...
                    this.debugLogger.debug(`[WebFetchTool] PDF text extraction failed: ${pdfText.error}`);
                }
            }
        }
            ...
            try {
                ...
                this.debugLogger.error(`[WebFetchTool] HTML conversion failed, using raw text`, error);
        ...
        this.debugLogger.debug(`[WebFetchTool] network_ms=${networkMs} extract_ms=${Date.now() - extractStart}`);
        const entry = {
            fetchedAt: Date.now(),
    ...
    async executeDirectFetch(signal) {
        ...
        const elapsedSuffix = () => ` in ${((Date.now() - startMs) / 1000).toFixed(1)}s`;
        try {
...
---`;
            ...
            try {
                ...
                this.debugLogger.debug(`[WebFetchTool] side_query_ms=${Date.now() - sideQueryStart}`);
            }
            catch (error) {
                ...
                this.debugLogger.error(`[WebFetchTool] side_query_ms=${Date.now() - sideQueryStart} (failed); returning raw content`, error);
