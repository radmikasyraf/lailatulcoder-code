...
export const SESSION_ARTIFACT_PERSISTENCE_VERSION = 2;
...
const SECRET_TOKEN_VALUE_PATTERN = /(?:^|\s)(?:bearer\s+\S{8,}|sk-[A-Za-z0-9_-]{12,}|(?:gh[pousr]|github_pat)_[A-Za-z0-9_/-]{12,}|[a-f0-9]{40,}|[A-Za-z0-9+/]{48,}={0,2})(?:$|\s)/i;
export function isSessionArtifactRecord(record) {
    return isTranscriptArtifactRecord(record);
}
export function selectActiveSideArtifactRecordUuids(records, activeRecordUuids) {
    const activeUuids = new Set(activeRecordUuids);
    const firstActiveUuid = activeRecordUuids[0];
    const firstActiveIndex = firstActiveUuid === undefined
        ? -1
        : records.findIndex((record) => record.uuid === firstActiveUuid);
    const nextActiveUuidByIndex = new Map();
    const nextBlockingUuidByIndex = new Map();
    let nextActiveUuid;
    ...
    for (let index = records.length - 1; index >= 0; index--) {
        ...
        else if (!isSessionArtifactRecord(record) &&
            !(record.type === 'system' && record.subtype === 'custom_title')) {
            nextBlockingUuid = record.uuid;
        }
    }
    const selected = [];
    const includedSideArtifactUuids = new Set();
    ...
    for (let index = 0; index < records.length; index++) {
        ...
        const isInActiveSegment = !nextBlockingUuidByIndex.has(index) &&
            (nextUuid !== undefined
...
export function stableSessionArtifactId(sessionId, identityKey) {
    return createHash('sha256')
        .update(`${sessionId}:${identityKey}`)
        .digest('hex')
        .slice(0, 16);
}
export function sessionArtifactIdentityKey(artifact) {
    if (artifact.workspacePath)
        return `workspace:${artifact.workspacePath}`;
    if (artifact.managedId)
        return `managed:${artifact.managedId}`;
    if (artifact.url)
...
export class SessionArtifactSnapshotAccumulator {
    ...
    stickyEphemeralIds = new Set();
    markerArtifacts = new Map();
    warnings = [];
    sequence = 0;
    ...
    add(record) {
        ...
        if (record.subtype === 'session_artifact_snapshot') {
            const payload = normalizeSnapshotPayload(record.systemPayload, this.warnings);
            if (!payload)
                return;
            this.sawRecord = true;
            ...
            for (const id of payload.tombstonedIds ?? [])
            ...
            for (const artifact of payload.artifacts) {
                if (artifact.retention === 'ephemeral')
                    continue;
        ...
        if (payload.sequence <= this.lastSnapshotSequence) {
            this.warnings.push(`skipped stale event sequence ${payload.sequence} at or before snapshot sequence ${this.lastSnapshotSequence}`);
            return;
        }
        this.sequence = Math.max(this.sequence, payload.sequence);
    ...
    finish() {
        if (!this.sawRecord || !this.sessionId)
            return undefined;
        return {
...
export function rebuildSessionArtifactSnapshot(records, fallbackSessionId) {
    const accumulator = new SessionArtifactSnapshotAccumulator(fallbackSessionId);
    for (const record of records)
        accumulator.add(record);
    return accumulator.finish();
}
export function remapSessionArtifactPayloadForFork(payload, sourceSessionId, newSessionId, remappedArtifactIds = new Map()) {
    const snapshot = normalizeSnapshotPayload(payload, []);
    if (snapshot) {
        const { markerArtifacts: snapshotMarkerArtifacts, ...snapshotWithoutMarkers } = snapshot;
        const artifacts = [];
        for (const artifact of snapshot.artifacts) {
            const remapped = remapForkSafeSessionArtifactForFork(artifact, sourceSessionId, newSessionId);
            if (!remapped) {
        ...
        return {
            ...
            tombstonedIds: (snapshot.tombstonedIds ?? []).map((artifactId) => remapArtifactIdForFork(artifactId, sourceSessionId, newSessionId, remappedArtifactIds)),
            stickyEphemeralIds: (snapshot.stickyEphemeralIds ?? []).map((artifactId) => remapArtifactIdForFork(artifactId, sourceSessionId, newSessionId, remappedArtifactIds)),
        };
    }
    ...
    return {
        ...
        changes: event.changes
            .map((change) => {
            if (change.artifact) {
                const artifact = remapForkSafeSessionArtifactForFork(change.artifact, sourceSessionId, newSessionId);
                if (!artifact) {
                    if (change.action !== 'removed')
                        return undefined;
                    seedForkArtifactIdRemap(change.artifact, newSessionId, remappedArtifactIds);
                    const { artifact: _unsafeArtifact, ...changeWithoutArtifact } = change;
                    return {
                        ...changeWithoutArtifact,
                        artifactId: remapArtifactIdForFork(change.artifactId, sourceSessionId, newSessionId, remappedArtifactIds),
                    };
                }
...
function remapForkSafeSessionArtifactForFork(artifact, sourceSessionId, newSessionId) {
    if (!isForkSafeArtifact(artifact))
        return undefined;
...
function seedForkArtifactIdRemap(artifact, newSessionId, remappedArtifactIds) {
    ...
    if (!identityKey)
        return;
    remappedArtifactIds.set(artifact.id, stableSessionArtifactId(newSessionId, identityKey));
}
...
function remapArtifactIdForFork(artifactId, sourceSessionId, newSessionId, remappedArtifactIds) {
    return (remappedArtifactIds.get(artifactId) ??
        stableSessionArtifactId(newSessionId, `fork:${sourceSessionId}:${artifactId}`));
}
function remapSessionArtifactForFork(artifact, sourceSessionId, newSessionId) {
    ...
    const id = identityKey
        ...
        : stableSessionArtifactId(newSessionId, `fork:${sourceSessionId}:${artifact.id}`);
    const next = {
        ...artifact,
        id,
...
function isSecretLikeText(value) {
    ...
    return /(?:^|[-_.])(token|secret|password|passwd|pwd|cookie|authorization|credential|signature|sig|api[-_]?key|access[-_]?key)(?:$|[-_.=&#])/i.test(normalized);
}
...
function isSecretLikeMetadataValue(value) {
    return /^(?:bearer\s+\S{8,}|sk-[A-Za-z0-9_-]{12,}|(?:gh[pousr]|github_pat)_[A-Za-z0-9_/-]{12,})$/i.test(value.trim());
}
export function normalizeSnapshotPayload(value, warnings) {
    if (!isRecord(value)) {
        warnings.push('skipped malformed snapshot record');
        return undefined;
    }
    if (value['v'] !== SESSION_ARTIFACT_PERSISTENCE_VERSION) {
        warnings.push(`skipped v${String(value['v'])} snapshot record (expected v${SESSION_ARTIFACT_PERSISTENCE_VERSION})`);
        return undefined;
    }
    ...
    if (rawArtifacts.length > MAX_PERSISTED_ARTIFACTS) {
        warnings.push(`snapshot artifact list truncated to ${MAX_PERSISTED_ARTIFACTS}`);
    }
    ...
    if (rawMarkerArtifacts.length > MAX_PERSISTED_MARKER_ARTIFACTS) {
        warnings.push(`snapshot marker artifact list truncated to ${MAX_PERSISTED_MARKER_ARTIFACTS}`);
    }
    ...
    return {
        ...
        recordedAt: getString(value, 'recordedAt', MAX_PERSISTED_TIMESTAMP_CHARS) ??
            new Date(0).toISOString(),
        artifacts,
        ...(markerArtifacts.length > 0 ? { markerArtifacts } : {}),
        tombstonedIds: getStringArray(value, 'tombstonedIds', MAX_PERSISTED_IDS),
        stickyEphemeralIds: getStringArray(value, 'stickyEphemeralIds', MAX_PERSISTED_IDS),
    };
}
export function normalizeEventPayload(value, warnings) {
    if (!isRecord(value)) {
        warnings.push('skipped malformed event record');
        return undefined;
    }
    if (value['v'] !== SESSION_ARTIFACT_PERSISTENCE_VERSION) {
        warnings.push(`skipped v${String(value['v'])} event record (expected v${SESSION_ARTIFACT_PERSISTENCE_VERSION})`);
        return undefined;
    }
    ...
    if (rawChanges.length > MAX_PERSISTED_EVENT_CHANGES) {
        warnings.push(`event change list truncated to ${MAX_PERSISTED_EVENT_CHANGES}`);
    }
    return {
        v: SESSION_ARTIFACT_PERSISTENCE_VERSION,
        sessionId,
...
function normalizePersistedArtifact(value, warnings) {
    ...
    const storage = normalizeLiteral(value['storage'], ['workspace', 'external_url', 'managed', 'published']);
    const source = normalizeLiteral(value['source'], ['tool', 'hook', 'client']);
    ...
    const description = getString(value, 'description', MAX_PERSISTED_DESCRIPTION_CHARS);
    ...
    const persistedAt = getString(value, 'persistedAt', MAX_PERSISTED_TIMESTAMP_CHARS);
    const expiresAt = getString(value, 'expiresAt', MAX_PERSISTED_TIMESTAMP_CHARS);
    ...
    const hookEventName = getString(value, 'hookEventName', MAX_PERSISTED_FIELD_CHARS);
    ...
    return {
        ...
        createdAt: getString(value, 'createdAt', MAX_PERSISTED_TIMESTAMP_CHARS) ??
            new Date(0).toISOString(),
        updatedAt: getString(value, 'updatedAt', MAX_PERSISTED_TIMESTAMP_CHARS) ??
            new Date(0).toISOString(),
...
function normalizeContentRef(value) {
    if (!isRecord(value) || value['kind'] !== 'managed_copy')
        return undefined;
    const contentId = getString(value, 'contentId', MAX_PERSISTED_FIELD_CHARS);
    const sha256 = getString(value, 'sha256', 64);
    const sizeBytes = getNonNegativeInteger(value, 'sizeBytes');
    const createdAt = getString(value, 'createdAt', MAX_PERSISTED_TIMESTAMP_CHARS);
    if (!contentId ||
...
export function isPrototypeMetadataKey(key) {
    return key === '__proto__' || key === 'constructor' || key === 'prototype';
}
export function isReservedWorkspaceMetadataKey(key) {
    return (key === WORKSPACE_CONTENT_SHA256_METADATA_KEY ||
...
function hasUnsafeDisplayPayload(value) {
    return (/<\s*\/?[a-z!]|&(?:#[0-9]+|#x[0-9a-f]+|[a-z][a-z0-9]+);|javascript\s*:|data\s*:\s*(?:text\/(?:html|javascript)|application\/javascript|image\/svg\+xml)/i.test(value) || /(?:^|[\s"'`<])on[a-z][a-z0-9-]*\s*=/i.test(value));
}
export function metadataBudgetBytes(metadata, budget = 'persisted') {
    if (budget === 'user') {
        return Buffer.byteLength(JSON.stringify(metadata), 'utf8');
    }
    const userMetadata = Object.fromEntries(Object.entries(metadata).filter(([key, value]) => !isWorkspaceContentMetadataEntry(key, value)));
    return Buffer.byteLength(JSON.stringify(userMetadata), 'utf8');
}
export function isWorkspaceContentMetadataEntry(key, value) {
...
function getStringArray(record, key, maxItems = MAX_PERSISTED_IDS, maxLength = MAX_PERSISTED_ID_CHARS) {
    ...
    if (!Array.isArray(value))
        return undefined;
    const items = value
        .filter((item) => typeof item === 'string' && item.length <= maxLength)
        .slice(-maxItems);
...
//# sourceMappingURL=session-artifact-persistence.js.map