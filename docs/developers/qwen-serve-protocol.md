...
Stage 1 of the [lailatul-coder daemon design](https://github.com/LailatulCoder/lailatul-coder/issues/3803). All routes live under the daemon's base URL (default `http://127.0.0.1:4170`).
...
`mcp_guardrail_events` (issue [#4175](https://github.com/LailatulCoder/lailatul-coder/issues/4175) PR 14b) advertises the typed SSE push events that surface MCP budget state crossings without a poll loop. Two frame types arrive on `GET /session/:id/events`:
...
**MCP client guardrails (issue [#4175](https://github.com/LailatulCoder/lailatul-coder/issues/4175)).** Current daemons extend the payload with four additive fields and a capability-scoped budget cell:
...
transport to read credentials that another local LailatulCoder Ai process may have
