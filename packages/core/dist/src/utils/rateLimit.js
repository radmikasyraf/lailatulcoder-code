 ...
 */
export function isRateLimitError(error, extraCodes) {
    const code = getErrorCode(error);
    if (code === null)
        return false;
    if (RATE_LIMIT_ERROR_CODES.has(code))
        return true;
    if (extraCodes && extraCodes.includes(code))
        return true;
    return false;
}
/**
 ...
 */
export function getRateLimitErrorDetails(error) {
    const statusCode = getErrorStatus(error);
    const payload = getProviderErrorPayload(error);
    const message = getRawErrorMessage(error);
    const transport = message?.includes('event:error') || message?.includes('HTTP_STATUS/')
        ? 'sse'
 ...
 */
export function getRateLimitRetryDelayMs(attempt, options) {
...
function getProviderErrorPayload(error) {
    for (const payload of getJsonPayloads(error)) {
        ...
        const nested = typeof nestedError === 'object' && nestedError !== null
            ? nestedError
            : undefined;
...
function getJsonPayloads(error) {
    ...
    if (jsonStart !== -1 && jsonEnd > jsonStart) {
        try {
            payloads.push(JSON.parse(message.slice(jsonStart, jsonEnd + 1)));
        }
        catch {
