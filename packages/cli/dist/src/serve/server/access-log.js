...
function countSuppressed(counts, status) {
    ...
    if (status >= 200 && status < 300)
        ...
        counts.statusOther += 1;
}
function firstRawHeader(rawHeaders, targetName) {
    for (let i = 0; i + 1 < rawHeaders.length; i += 2) {
        if (rawHeaders[i]?.toLowerCase() === targetName)
...
export function installAccessLogMiddleware(app, daemonLog, monotonicNow = () => performance.now()) {
    let sealed = false;
    ...
    const refill = () => {
        ...
        tokens = Math.min(ACCESS_LOG_BURST, tokens + ((now - refillBaseline) / 1_000) * ACCESS_LOG_REFILL_PER_SECOND);
        refillBaseline = now;
    };
    const flushSuppressed = () => {
        if (!daemonLog || suppressed.suppressed === 0)
            return false;
        ...
        res.on('finish', () => {
            try {
                ...
                const sessionId = sessionMatch?.[1]
                    ...
                    : undefined;
                const rawClientId = firstRawHeader(req.rawHeaders, 'x-LailatulCoder-client-id');
