...
const SETTINGS_SELECTOR_FILENAME = '.lailatulcoder-extension-settings.json';
...
function getSettingsSelectorPath(extensionName, envFilePathOverride) {
    return path.join(envFilePathOverride
        ...
        : new ExtensionStorage(extensionName).getExtensionDir(), SETTINGS_SELECTOR_FILENAME);
}
async function readSettingsSelector(extensionName) {
    ...
    if (!parsed ||
        ...
        parsed.version !== 1 ||
        !('backend' in parsed) ||
        !Object.values(TokenStorageType).includes(parsed.backend) ||
        !('bundleKey' in parsed) ||
...
function createSelectedStorage(serviceName, backend) {
    return backend === TokenStorageType.KEYCHAIN
        ? new KeychainTokenStorage(serviceName)
        : new FileTokenStorage(serviceName);
}
async function deleteSettingsSnapshot(selector, serviceName) {
...
function parseSensitiveSettingsBundle(content) {
...
export function validateExtensionSettingEnvVars(settings) {
    if (settings?.some((setting) => !ENV_VAR_NAME_PATTERN.test(setting.envVar))) {
        throw new Error('Extension setting "envVar" must be a valid environment variable name.');
    }
}
...
const getKeychainStorageName = (extensionName, extensionId, scope) => {
    const base = `LailatulCoder Ai Extensions ${extensionName} ${extensionId}`;
...
export async function maybePromptForSettings(extensionConfig, extensionId, requestSetting, previousExtensionConfig, previousSettings, envFilePathOverride, deferKeychainMutations = false) {
    ...
    const envFilePath = envFilePathOverride ?? getEnvFilePath(extensionName, scope);
    const selectorPath = getSettingsSelectorPath(extensionName, envFilePathOverride);
    const keychain = new HybridTokenStorage(getKeychainStorageName(extensionName, extensionId, scope));
    ...
    if (!settings || settings.length === 0) {
        ...
        return await applyOrDeferKeychainMutations(keychainMutations, deferKeychainMutations, previousSelector, undefined, serviceName);
    }
    ...
    for (const removedSensitiveSetting of settingsChanges.removeSensitive) {
        keychainMutations.push(async () => await keychain.deleteSecret(removedSensitiveSetting.envVar));
    }
    ...
    for (const setting of settings) {
        ...
        if (setting.sensitive) {
            ...
            keychainMutations.push(async () => await keychain.setSecret(setting.envVar, value));
        }
        else {
    ...
    if (Object.keys(sensitiveSettings).length > 0) {
        ...
        try {
            ...
            try {
                ...
                throw new AggregateError([error, cleanupError], 'Failed to stage extension settings and clean its secret snapshot.', { cause: error });
            }
            throw error;
        }
    }
    ...
    return await applyOrDeferKeychainMutations(keychainMutations, deferKeychainMutations, previousSelector, newSelector, serviceName);
}
async function applyOrDeferKeychainMutations(mutations, defer, previousSelector, newSelector, serviceName) {
...
export async function getScopedEnvContents(extensionConfig, extensionId, scope) {
    ...
    const keychain = new HybridTokenStorage(getKeychainStorageName(extensionName, extensionId, scope));
    const selector = scope === ExtensionSettingScope.USER
        ? await readSettingsSelector(extensionName)
        : undefined;
    const settingsStorage = selector
        ? createSelectedStorage(getKeychainStorageName(extensionName, extensionId, scope), selector.backend)
        : keychain;
    ...
    if (extensionConfig.settings) {
        ...
        for (const setting of extensionConfig.settings) {
            ...
            const override = selector
                ? await settingsStorage.getSecret(`${selector.bundleKey}:override:${setting.envVar}`)
                : null;
...
export async function updateSetting(extensionConfig, extensionId, settingKey, requestSetting, scope) {
    ...
    if (settingToUpdate.sensitive) {
        ...
        const settingsStorage = selector
            ...
            : keychain;
        if (selector) {
            await settingsStorage.setSecret(`${selector.bundleKey}:override:${settingToUpdate.envVar}`, newValue);
            try {
                await keychain.setSecret(settingToUpdate.envVar, newValue);
            }
            catch (error) {
                debugLogger.warn(`Failed to synchronize legacy extension setting "${settingToUpdate.envVar}": ${error instanceof Error ? error.message : String(error)}`);
            }
        }
        else {
...
async function clearKeychainSettings(keychain) {
