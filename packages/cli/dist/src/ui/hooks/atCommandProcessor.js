...
import { getErrorMessage, isNodeError, Storage, isSubpath, unescapePath, readManyFiles, shouldRunVisionBridge, emptyMcpResourceText, formatMcpResourceContents, summarizeMcpResource, SessionService, SessionReferenceService, } from '@lailatul-coder/lailatul-coder-core';
import { ToolCallStatus } from '../types.js';
import { matchMcpServerPrefix } from './mcpResourceRef.js';
import { parseExtensionRef, matchExtensionByRef, buildExtensionRef, } from './extension-mention-ref.js';
import { parseSessionRef, buildSessionRef } from './session-mention-ref.js';
import { buildExtensionMentionContext, EXTENSION_CONTEXT_BUDGET, getSanitizedExtensionDisplayName, } from '../../utils/extension-mention.js';
import { buildMcpServerContextText, buildMcpServerRef, matchMcpServerByRef, parseMcpServerRef, } from '../../utils/mcp-server-mention.js';
/**
...
export function extractAtPathCommands(query) {
    return parseAllAtCommands(query).flatMap((part) => part.type === 'atPath' && part.content !== '@'
 ...
 */
export async function resolveAtCommandQuery({ query, config, onDebugMessage, messageId: userMessageTimestamp, signal, }) {
    ...
    const displayPathsByCanonicalPath = new Map();
    const ignoredByReason = {
        git: [],
        LailatulCoder: [],
        both: [],
    };
    const getIgnoreReason = (candidate) => {
    ...
    for (const atPathPart of atPathCommandParts) {
        ...
        if (extRef) {
            ...
            onDebugMessage(`Extension "${extRef.name}" not found among active extensions. ` +
        ...
        if (sessionRef) {
            if (!sessionMentions.some((m) => (m.ref.id ?? m.ref.title) === (sessionRef.id ?? sessionRef.title))) {
                sessionMentions.push({ originalAtPath, ref: sessionRef });
            }
            atPathToResolvedSpecMap.set(originalAtPath, buildSessionRef(sessionRef.id ?? sessionRef.title));
            continue;
        }
        ...
        if (mcpServerRef) {
            const matched = matchMcpServerByRef(mcpServerRef.name, config.getMcpServers() || {});
            if (matched) {
                ...
                atPathToResolvedSpecMap.set(originalAtPath, buildMcpServerRef(matched.serverName));
                continue;
            }
            onDebugMessage(`MCP server "${mcpServerRef.name}" not found among configured MCP servers. ` +
        ...
        if (!isSubpath(configuredProjectTempDir, absolutePathName) &&
        ...
        if (ignoredReason) {
            ...
            const reasonText = reason === 'both'
                ? 'ignored by both git and LailatulCoder'
                : reason === 'git'
                    ? 'git-ignored'
                    : 'LailatulCoder-ignored';
        ...
        for (const dir of config.getWorkspaceContext().getDirectories()) {
            try {
                ...
                if (!isSubpath(configuredProjectTempDir, canonicalPath) &&
                ...
                if (canonicalIgnoreReason) {
                    ...
                    const reasonText = canonicalIgnoreReason === 'both'
                        ? 'ignored by both git and LailatulCoder'
                ...
                const canonicalDisplays = displayPathsByCanonicalPath.get(canonicalPath) ?? new Set();
                ...
                if (isNodeError(error) && error.code === 'ENOENT') {
                    ...
                    onDebugMessage(`Error stating path ${pathName}: ${getErrorMessage(error)}. Path ${pathName} will be skipped.`);
                }
            }
    ...
    const totalIgnored = ignoredByReason['git'].length +
        ignoredByReason['LailatulCoder'].length +
    ...
    if (totalIgnored > 0) {
        ...
        if (ignoredByReason['LailatulCoder'].length) {
            messages.push(`LailatulCoder-ignored: ${ignoredByReason['LailatulCoder'].join(', ')}`);
        }
    ...
    const resourceReads = await Promise.allSettled(mcpResourceRefs.map((ref) => config
        .getToolRegistry()
    ...
    for (let i = 0; i < mcpResourceRefs.length; i++) {
        ...
        if (outcome.status === 'rejected') {
            onDebugMessage(`Failed to read MCP resource ${label}: ${getErrorMessage(outcome.reason)}`);
    ...
    for (let i = 0; i < sessionMentions.length; i++) {
        ...
        if (!sessionId && ref.title) {
            let matches = [];
            try {
                matches = await new SessionService(config.getProjectRoot()).findSessionsByTitle(ref.title);
            }
            catch (error) {
        ...
        if (resolvedSessionIds.has(sessionId)) {
            onDebugMessage(`Session reference "@${originalAtPath.substring(1)}" resolves to session ${sessionId}, which was already referenced; skipping duplicate.`);
            continue;
        }
        resolvedSessionIds.add(sessionId);
        let resolved;
        try {
            resolved = await new SessionReferenceService(config.getProjectRoot()).resolve(sessionId, ref.title ? { title: ref.title } : {});
        }
        catch (error) {
    ...
    const scopedMentionOrder = new Map(atPathCommandParts.map((part, index) => [part.content, index]));
    scopedMentionEntries.sort((a, b) => (scopedMentionOrder.get(a.originalAtPath) ?? Number.MAX_SAFE_INTEGER) -
    ...
    const scopedMentionDisplays = scopedMentionEntries.map((entry) => entry.display);
    ...
    const revalidatedDisplayPaths = new Map();
    const validatedPathIdentities = new Map();
    const pruneSkippedPath = (approvedPath) => {
        const displayPath = displayPaths.get(approvedPath);
        const displayLabels = displayPathsByCanonicalPath.get(approvedPath) ??
    ...
    for (const approvedPath of pathSpecsToRead) {
        try {
            ...
            if (currentPath === approvedPath &&
                ...
                const displayPath = displayPaths.get(approvedPath);
                if (displayPath)
                    revalidatedDisplayPaths.set(currentPath, displayPath);
                ...
                onDebugMessage(`Path ${approvedPath} failed revalidation and will be skipped.`);
            }
        }
        catch {
            pruneSkippedPath(approvedPath);
            onDebugMessage(`Path ${approvedPath} changed before it could be read and will be skipped.`);
        }
    }
    ...
    if (revalidatedPathSpecs.length > 0) {
        try {
            ...
            const errorMessage = typeof errorToolCallDisplay.resultDisplay === 'string'
...
export async function handleAtCommand(params) {
