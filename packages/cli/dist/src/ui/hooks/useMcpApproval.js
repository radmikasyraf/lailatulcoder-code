...
function sourceLabel(scope) {
    switch (scope) {
        case 'workspace':
            return '.lailatulcoder/settings.json';
        case 'project':
        default:
            return '.mcp.json';
    }
...
function summarize(config) {
    ...
    if (config.httpUrl) {
        ...
        summary =
            `${config.command} ${config.args?.join(' ') ?? ''} (stdio)`.replace(/\s+\(/, ' (');
    }
    else {
 ...
 * `.mcp.json` and workspace `.lailatulcoder/settings.json` (issue #4615). On mount it
 ...
 */
export const useMcpApproval = (config) => {
    ...
    const reconnect = useCallback((name) => {
    ...
    const handleMcpApprovalSelect = useCallback(async (choice) => {
        const approvals = loadMcpApprovals();
        const root = config.getWorkingDir();
        ...
        if (choice === McpApprovalChoice.APPROVE_ALL) {
            for (const server of queue) {
                await approvals.setState(root, server.name, server.config, 'approved');
        ...
        if (choice === McpApprovalChoice.APPROVE) {
            await approvals.setState(root, current.name, current.config, 'approved');
            reconnect(current.name);
        }
        else {
            await approvals.setState(root, current.name, current.config, 'rejected');
        }
...
//# sourceMappingURL=useMcpApproval.js.map