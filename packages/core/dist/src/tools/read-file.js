...
import { processSingleFileContent, getSpecificMimeType, isCacheableReadResult, } from '../utils/fileUtils.js';
...
import { formatVisionBridgeNotice, runVisionBridge, shouldRunVisionBridge, } from '../services/visionBridge/vision-bridge-service.js';
import { hasImageParts, normalizeParts, splitImageParts, } from '../services/visionBridge/image-part-utils.js';
...
class ReadFileToolInvocation extends BaseToolInvocation {
    ...
    async execute(signal) {
        ...
        const isFullRead = this.params.offset === undefined &&
        ...
        let result = await processSingleFileContent(this.params.file_path, this.config, {
        ...
        logFileOperation(this.config, new FileOperationEvent(ReadFileTool.Name, FileOperation.READ, lines, mimetype, path.extname(this.params.file_path), programming_language));
        return {
            llmContent,
            returnDisplay: this.toToolResultDisplay(result),
        };
    }
    async transcribePdfCandidate(result, signal) {
        ...
        try {
            ...
            if (bridgeResult.status === 'ok' &&
                ...
                if (bridgeResult.convertedCount !== imageParts.length ||
                    bridgeResult.omittedCount !== 0) {
                    debugLogger.debug('pdf vision bridge omitted candidate pages');
                    return this.restorePdfFallback(result, `${notice} The transcription was discarded because the bridge did not transcribe every rendered PDF page.`);
                }
                const bridgedParts = normalizeParts(bridgeResult.parts);
                if (bridgedParts.some((part) => part.inlineData != null || part.fileData != null)) {
                    debugLogger.debug('pdf vision bridge returned media data');
                    return this.restorePdfFallback(result, `${notice} The transcription was discarded because the bridge returned an unsafe media payload.`);
                }
                return {
                    ...result,
            ...
            return this.restorePdfFallback(result, bridgeResult.status === 'ok'
            ...
            signal.throwIfAborted();
            debugLogger.debug(`pdf vision bridge failed before replacement: ${String(error instanceof Error ? error.message : error)}`);
            return this.restorePdfFallback(result, 'Vision bridge failed before producing a transcription.');
        }
    }
    restorePdfFallback(result, notice) {
        const candidate = result.pdfVisionBridgeCandidate;
        if (!candidate)
            return result;
    ...
    formatPdfBridgeRange(candidate, action) {
    ...
    toToolResultDisplay(result, fallback = '') {
        const summary = result.returnDisplay || fallback;
        if (!result.pdfVisionBridgeNotice)
            return summary;
        const display = {
     ...
     */
    unchangedResult(absPath) {
        const relativePath = shortenPath(makeRelative(absPath, this.config.getTargetDir()));
        const llmContent = `[File ${relativePath} unchanged since last read in this session — ` +
