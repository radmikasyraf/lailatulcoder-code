...
export declare abstract class DeclarativeTool<TParams extends object, TResult extends ToolResult> implements ToolBuilder<TParams, TResult> {
     ...
     */
    toAutoClassifierInput(_params: TParams): Record<string, unknown> | string | undefined;
    /**
...
export type ToolArtifactKind = 'file' | 'link' | 'html' | 'image' | 'video' | 'audio' | 'pdf' | 'notebook' | 'document' | 'other';
...
export type ToolArtifactStorage = 'workspace' | 'external_url' | 'managed' | 'published';
...
export declare function isShellProgressData(display: unknown): display is ShellProgressData;
export declare const MAX_TERMINAL_IMAGE_BYTES: number;
export interface TerminalImageDisplay {
    type: 'terminal_image';
    filePath: string;
    mimeType: 'image/png';
}
export declare function isTerminalImageDisplay(display: unknown): display is TerminalImageDisplay;
export type ToolResultDisplay = string | FileDiff | TodoResultDisplay | PlanResultDisplay | AgentResultDisplay | TeamResultDisplay | TaskListResultDisplay | AnsiOutputDisplay | McpToolProgressData | VisionBridgeNoticeDisplay | ShellProgressData | TerminalImageDisplay;
 ...
 */
export declare const CONCURRENCY_SAFE_KINDS: ReadonlySet<Kind>;
