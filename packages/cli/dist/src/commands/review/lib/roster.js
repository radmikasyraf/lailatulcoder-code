...
export function isPositivePrNumber(value) {
 ...
 * `LailatulCoder review script-lint` step and the gate that checks its output share, so they
 * cannot disagree about what counts as an executable script.
 */
export function hasExecutableScript(plan) {
    const files = Array.isArray(plan.files) ? plan.files : [];
    ...
    return files.some((f) => {
        if (typeof f?.path !== 'string' || pathTool(f.path) === null)
            return false;
        if (!trustsFileLines)
            return true;
...
export function requiredAgents(plan) {
    ...
    if (mode !== 'diff-only') {
        ...
        // `LailatulCoder review script-lint` deterministically and `compose-review` reads its
    ...
    for (const role of repositoryContext?.requiredAgents ?? []) {
        if (!contextRoleRunsInThisReview(role, plan, mode))
            continue;
 ...
 */
function contextRoleRunsInThisReview(role, plan, mode) {
