...
function takeUtf8Prefix(text, byteLimit) {
    let bytes = 0;
    let result = '';
...
function takeUtf8Suffix(text, byteLimit) {
...
export function truncateRealtimeTextToTokenBudget(text, budgetTokens) {
    ...
    while (contentBytes > 0) {
        ...
        const removedBytes = Math.max(0, totalBytes -
...
function buildCurrentThreadSection(records) {
    const turns = [];
    let currentUser = [];
    ...
    for (const [index, turn] of [...turns].reverse().entries()) {
        ...
        const turnText = truncateRealtimeTextToTokenBudget(turnLines.join('\n'), Math.min(REALTIME_TURN_TOKEN_BUDGET, remainingBudget));
...
async function loadRecentThreads(workspaceRegistry) {
    try {
        const pages = await Promise.all(workspaceRegistry.listAll().map(async (runtime) => {
        ...
        debugLogger.warn('Failed to load Live startup tasks from the session catalog:', error);
        return [];
    }
}
function groupRoot(cwd) {
    return findGitRoot(cwd) ?? cwd;
}
function buildRecentWorkSection(currentCwd, recentThreads) {
    ...
    for (const thread of recentThreads) {
        ...
        if (entries)
            ...
            grouped.set(root, [thread]);
    }
    const currentGroup = groupRoot(currentCwd);
    const groups = [...grouped.entries()].sort(([leftRoot, left], [rightRoot, right]) => {
        const byCurrent = Number(leftRoot !== currentGroup) - Number(rightRoot !== currentGroup);
        if (byCurrent !== 0)
            return byCurrent;
    ...
    for (const [root, unsorted] of groups.slice(0, MAX_RECENT_WORK_GROUPS)) {
        const entries = [...unsorted].sort((left, right) => right.updatedAt - left.updatedAt);
        ...
        if (latest.gitBranch)
            lines.push(`Latest branch: ${latest.gitBranch}`);
        lines.push('', 'User asks:');
        const seen = new Set();
        const maxAsks = root === currentGroup ? MAX_CURRENT_CWD_ASKS : MAX_OTHER_CWD_ASKS;
        for (const entry of entries) {
            const ask = entry.firstUserMessage.trim().split(/\s+/).join(' ');
            const dedupeKey = `${entry.cwd}:${ask}`;
            if (!ask || seen.has(dedupeKey))
                continue;
            seen.add(dedupeKey);
            const boundedAsk = [...ask].length > MAX_ASK_CHARS
                ? `${[...ask].slice(0, MAX_ASK_CHARS - 3).join('')}...`
                : ask;
            lines.push(`- ${entry.cwd}: ${boundedAsk}`);
            if (seen.size === maxAsks)
                break;
        }
        if (lines.length > 5)
...
function collectTreeLines(directory, depth, lines) {
    ...
    try {
        entries = readdirSync(directory, { withFileTypes: true })
            ...
            .sort((left, right) => {
            const byDirectory = Number(!left.isDirectory()) - Number(!right.isDirectory());
    ...
    if (entries.length > DIR_ENTRY_LIMIT) {
        lines.push(`${'  '.repeat(depth)}- ... ${entries.length - DIR_ENTRY_LIMIT} more entries`);
    }
}
...
function buildWorkspaceSection(currentCwd, userRoot) {
    const gitRoot = findGitRoot(currentCwd) ?? undefined;
    const cwdTree = renderTree(currentCwd);
    const gitRootTree = gitRoot && gitRoot !== currentCwd ? renderTree(gitRoot) : undefined;
    const userRootTree = userRoot && userRoot !== currentCwd && userRoot !== gitRoot
        ? renderTree(userRoot)
        : undefined;
    if (!cwdTree && !gitRoot && !userRootTree)
        return undefined;
    const lines = [
    ...
    if (gitRootTree)
        lines.push('', 'Git root tree:', ...gitRootTree);
    if (userRootTree)
        lines.push('', 'User root tree:', ...userRootTree);
    return lines.join('\n');
}
...
export async function buildRealtimeStartupContext(options) {
    const current = await createWorkspaceRuntimeSessionService(options.runtime)
        .loadSession(options.sessionId)
        .catch(() => undefined);
    const currentThread = buildCurrentThreadSection(current?.conversation.messages ?? []);
    const recentWork = buildRecentWorkSection(options.currentCwd, await loadRecentThreads(options.workspaceRegistry));
    const workspace = buildWorkspaceSection(options.currentCwd, options.userRoot ?? homedir());
    if (!currentThread && !recentWork && !workspace)
        return undefined;
    const parts = [STARTUP_CONTEXT_HEADER];
    const sections = [
        formatSection('Current Thread', currentThread, CURRENT_THREAD_SECTION_TOKEN_BUDGET),
        formatSection('Recent Work', recentWork, RECENT_WORK_SECTION_TOKEN_BUDGET),
        formatSection('Machine / Workspace Map', workspace, WORKSPACE_SECTION_TOKEN_BUDGET),
        formatSection('Notes', 'Built at realtime startup from the current thread history, local thread metadata, and a bounded local workspace scan. This excludes repo memory instructions, AGENTS files, project-doc prompt blends, and memory summaries.', NOTES_SECTION_TOKEN_BUDGET),
    ];
...
//# sourceMappingURL=realtime-startup-context.js.map