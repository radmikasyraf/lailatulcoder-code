...
import { getMCPServerStatus, removeMCPServerStatus, addMCPStatusChangeListener, removeMCPStatusChangeListener, mcpServerRequiresOAuth, MCPServerStatus, DiscoveredMCPTool, MCPOAuthTokenStorage, createDebugLogger, matchesAnyServerPattern, } from '@lailatul-coder/lailatul-coder-core';
 ...
 */
export const McpServerActionsView = ({ config, serverName, isActive, onStatus, onReload, onExit, }) => {
    const [sub, setSub] = useState('detail');
    const [server, setServer] = useState(null);
    const [selectedTool, setSelectedTool] = useState(null);
    const [selectedResource, setSelectedResource] = useState(null);
    const [loading, setLoading] = useState(true);
    const buildServer = useCallback(async () => {
        ...
        const serverTools = (toolRegistry?.getAllTools() || []).filter((tool) => tool instanceof DiscoveredMCPTool && tool.serverName === serverName);
        const promptRegistry = config.getPromptRegistry();
        const serverPrompts = (promptRegistry?.getAllPrompts() || []).filter((p) => 'serverName' in p && p.serverName === serverName);
        const serverResources = config.getResourceRegistry()?.getResourcesByServer(serverName) || [];
        let source = 'user';
        ...
        try {
            const credentials = await new MCPOAuthTokenStorage().getCredentials(serverName);
    ...
    const freshen = useCallback((info) => {
        if (!info)
            return info;
    ...
    useEffect(() => {
        const listener = (name, status) => {
            if (name !== serverName || status === undefined)
                return;
            setServer((prev) => prev
                ? {
                    ...prev,
            ...
            if (server.isDisabled) {
                ...
                for (const scope of [SettingScope.User, SettingScope.Workspace]) {
                    ...
                    if (excluded.includes(serverName)) {
                        settings.setValue(scope, 'mcp.excluded', excluded.filter((n) => n !== serverName));
                    }
                }
                const runtimeExcluded = config.getExcludedMcpServers() || [];
                config.setExcludedMcpServers(runtimeExcluded.filter((n) => n !== serverName));
                ...
                const scope = server.source === 'project'
    ...
    if (sub === 'authenticate') {
        return (_jsx(AuthenticateStep, { server: server, isActive: isActive, onBack: () => {
                setSub('detail');
                void reload();
            } }));
    }
    if (sub === 'tool-detail') {
        return (_jsx(ToolDetailStep, { tool: selectedTool, isActive: isActive, onBack: () => setSub('tools') }));
    }
    if (sub === 'tools') {
        return (_jsx(ToolListStep, { tools: getServerTools(), serverName: serverName, isActive: isActive, onSelect: (tool) => {
                setSelectedTool(tool);
                setSub('tool-detail');
            }, onBack: () => setSub('detail') }));
    }
    if (sub === 'resource-detail') {
        return (_jsx(ResourceDetailStep, { resource: selectedResource, isActive: isActive, onBack: () => setSub('resources') }));
    }
    if (sub === 'resources') {
        return (_jsx(ResourceListStep, { resources: getServerResources(), serverName: serverName, isActive: isActive, onSelect: (resource) => {
                setSelectedResource(resource);
                setSub('resource-detail');
            }, onBack: () => setSub('detail') }));
    }
    return (_jsx(ServerDetailStep, { server: server, isActive: isActive, onViewTools: () => setSub('tools'), onViewResources: () => setSub('resources'), onReconnect: () => void handleReconnect(), onDisable: () => void handleToggleDisable(), onAuthenticate: () => setSub('authenticate'), onClearAuth: () => void handleClearAuth(), onBack: onExit }));
};
//# sourceMappingURL=McpServerActionsView.js.map