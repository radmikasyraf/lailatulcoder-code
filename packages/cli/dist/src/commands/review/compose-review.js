...
import { coverageFromTranscripts, verificationGaps, TranscriptsUnavailableError, } from './lib/coverage.js';
import { compressSummary, SEVERITIES, SOURCES, } from '../../utils/findings.js';
import { BRIEFS } from './lib/agent-briefs.js';
import { budgetStopDisclosure, budgetStopEntry, budgetStopEntryZh, roundCapStopEntry, roundCapStopEntryZh, roundCapStopDisclosure, readBudgetStop, } from './lib/deadline.js';
import { LARGE_REVERSE_AUDIT_ROUNDS } from './lib/budget.js';
import { shellQuotePath } from './lib/shell-quote.js';
import { HOSTNAME_RE, gh, getGhHost, isOwnerRepo, resolveGhHost, setGhHost, } from './lib/gh.js';
import { isPositivePrNumber, hasExecutableScript, requiredAgents, reviewMode, } from './lib/roster.js';
...
import { LEDGER_BODY_FILE, LEDGER_ID_READBACK, LEDGER_MAX_ID, isLedgerFinding, isStandInName, normalizeLedgerFinding, LEDGER_MAX_BYTES, LEDGER_MAX_ROUND, LEDGER_UNKNOWN_FILE, serializeLedger, volumeOf, } from './lib/ledger.js';
import { mdField } from './lib/md-field.js';
import { diagnoseConvergence, isFreshDraft, renderConvergenceDiagnosis, } from './lib/convergence.js';
import { CRITICAL_PREFIX, LEADING_INVISIBLE_RE, SUGGESTION_PREFIX, carriedClaimLine, countInlineFindings, markerStrippedBody, severityOf, stripSeverityPrefix, unmarkedComments, } from './lib/inline-counts.js';
import { MODEL_ID_MAX_CHARS, footerVersion, isFooterSafeModelId, rendersAsNothing, reviewFooter, stripCommentMarkerLines, stripFooterSpans, stripForUnattributedPost, stripReviewFooter, } from './lib/review-footer.js';
...
export function renderDeferredEntry(entry) {
    const loc = entry.line !== undefined ? `${entry.file}:${entry.line}` : entry.file;
    const agg = entry.locations && entry.locations > 0
 ...
 */
function boundDeferredLine(rendered) {
...
function toDeferredEntries(value) {
    if (value === undefined || value === null)
        return [];
    if (!Array.isArray(value)) {
        throw new TypeError(`compose-review: deferredSuggestions must be an array of {file, line?, source, severity, title, locations?} entries, got ${JSON.stringify(value)}`);
    }
    return value.map((raw, i) => {
        if (typeof raw !== 'object' || raw === null || Array.isArray(raw)) {
            throw new TypeError(`compose-review: deferredSuggestions[${i}] must be an object {file, line?, source, severity, title, locations?} — a free-text entry is not accepted, the channel is typed`);
        }
        const o = raw;
        ...
        if (file === '' || title === '') {
            throw new TypeError(`compose-review: deferredSuggestions[${i}] needs a non-empty file and title`);
        }
        if (typeof source !== 'string' || !SOURCES.includes(source)) {
            throw new TypeError(`compose-review: deferredSuggestions[${i}].source must be one of ${SOURCES.join('|')}, got ${JSON.stringify(source)}`);
        }
        if (typeof severity !== 'string' ||
            !SEVERITIES.includes(severity)) {
            throw new TypeError(`compose-review: deferredSuggestions[${i}].severity must be one of ${SEVERITIES.join('|')}, got ${JSON.stringify(severity)}`);
        }
        if (severity === 'Nice to have') {
            throw new TypeError(`compose-review: deferredSuggestions[${i}] is a Nice to have — terminal-only findings are never deferred to the PR; drop it from the state`);
        }
        if (line !== undefined &&
            line !== null &&
            (typeof line !== 'number' || !Number.isInteger(line) || line < 1)) {
            throw new TypeError(`compose-review: deferredSuggestions[${i}].line must be a positive integer when present`);
        }
        if (locations !== undefined &&
            ...
            (typeof locations !== 'number' ||
                ...
                locations < 0)) {
            throw new TypeError(`compose-review: deferredSuggestions[${i}].locations must be a non-negative integer when present`);
        }
        return {
            file,
 ...
 */
function splitDeferralChannel(raw) {
    ...
    return {
        ...
        relocated: relocatedEntries.map((e) => `${mdField(boundDeferredLine(renderDeferredEntry(e)))} _(relocated from the deferral channel — a Critical is never deferred, it posts)_`),
        relocatedDeterministic: relocatedEntries.filter((e) => DETERMINISTIC_SOURCES.has(e.source)).length,
    };
}
/**
 ...
 * exists to prevent.
 */
export function normalizeSeverityFloor(value) {
 ...
 */
export function criticalFloorKind(severityFloor, contextUnavailable, prevRound) {
    ...
    const floor = raw === 'critical' || raw === 'suggestion' || raw === 'auto'
        ? raw
        : absent
            ? 'auto'
            : undefined;
 ...
 */
export function criticalFloorInEffect(severityFloor, contextUnavailable, prevRound) {
    return (floorResolvesCritical(normalizeSeverityFloor(severityFloor), contextUnavailable, prevRound) !== undefined);
}
/** Does this resolved floor mean `critical` for the round after `prevRound`? */
function floorResolvesCritical(floor, contextUnavailable, prevRound) {
...
function severityFloorKnown(severityFloor) {
    const raw = normalizeSeverityFloor(severityFloor);
 ...
 */
export function floorEnforcedReroute(severityFloor, contextUnavailable, prevRound, drafted) {
    ...
    drafted.forEach((c, i) => {
        ...
        if (claim !== null && DETERMINISTIC_TAG_RE.test(claim))
            return;
        const file = typeof c.path === 'string' && c.path.trim() !== '' ? c.path : null;
        if (file === null)
            return;
        ...
        const title = collapseToLine(stripReviewFooter(markerStrippedBody(body) ?? ''));
        indices.push(i);
        entries.push({
            file,
 ...
 */
function canonicalDimensionHead(s) {
    return (s
        .toLowerCase()
...
function squashedDimensionHead(s) {
 ...
 */
const NON_DIFF_DIMENSION_HEADS = new Set(Object.values(BRIEFS)
...
const NON_DIFF_DIMENSION_HEADS_SQUASHED = new Set([...NON_DIFF_DIMENSION_HEADS].map((h) => h.replace(/-/g, '')));
/**
 ...
 */
export function isNonDiffDimensionGap(entry) {
 ...
 */
function planPrIdentity(plan) {
    if (typeof plan !== 'object' || plan === null)
        return null;
    const p = plan;
    const ownerRepo = typeof p.ownerRepo === 'string' && isOwnerRepo(p.ownerRepo)
        ? p.ownerRepo
        : null;
    ...
    const host = typeof p.host === 'string' && HOSTNAME_RE.test(p.host) ? p.host : null;
    return ownerRepo && prNumber ? { ownerRepo, prNumber, host } : null;
}
function prIdentityFromPlan(planPath) {
    if (!planPath)
        return null;
    try {
 ...
 */
function linkifyCommentRefs(text, pr) {
    if (!pr || text.includes(']('))
        return text;
    ...
    const commentRef = host === 'github.com'
    ...
    return text.replace(commentRef, (_m, issueLevel, id) => issueLevel || issueLevelEntry
 ...
 */
function formatCannotTell(cannotTell, pr, attribution) {
    const parsed = cannotTell.map((raw) => {
        ...
        const unmarked = severityOf({ body: source }) === null
            ? source
    ...
    for (const p of parsed) {
        ...
        const group = { reason: p.reason, heads: [p.head] };
        groups.push(group);
        if (p.reason !== null)
            byReason.set(p.reason, group);
    }
    const lines = [];
    ...
    for (const { reason, heads } of groups) {
        if (heads.length === 1) {
            lines.push(`- ${marker}${heads[0]}${reason === null ? '' : ` — ${reason}`}`);
        }
        else {
            lines.push(`- ${marker}${heads.length} entries — ${reason}:`, ...heads.map((head) => `  - ${head}`));
        }
    }
    return {
...
function toCount(value, field) {
    ...
    if (typeof value !== 'number' || !Number.isInteger(value) || value < 0) {
        throw new TypeError(`compose-review: ${field} must be a non-negative integer, got ${JSON.stringify(value)}`);
    }
    return value;
}
function toStringList(value, field) {
    ...
    if (!Array.isArray(value) || value.some((v) => typeof v !== 'string')) {
        throw new TypeError(`compose-review: ${field} must be an array of strings, got ${JSON.stringify(value)}`);
    }
 ...
 */
function strippedList(input, key, attribution) {
    return toStringList(input[key], key)
        .map((entry) => stripReviewFooter(attribution ? entry : stripForUnattributedPost(entry)))
...
function toBool(value, field) {
    ...
    if (typeof value !== 'boolean') {
        throw new TypeError(`compose-review: ${field} must be a boolean, got ${JSON.stringify(value)}`);
    }
    return value;
}
export function composeReview(input, cliVersion = 'unknown', attribution = true, 
/**
...
runtimeModelId) {
    ...
    const reroute = floorEnforcedReroute(input.severityFloor, input.contextUnavailable === true, prevRound, Array.isArray(input.draftedComments) ? input.draftedComments : []);
    ...
    const floorKind = criticalFloorKind(input.severityFloor, input.contextUnavailable === true, prevRound);
    let effective = input;
    if (reroute.indices.length > 0) {
        const drop = new Set(reroute.indices);
        effective = {
            ...input,
            draftedComments: (input.draftedComments ?? []).filter((_, i) => !drop.has(i)),
            ...
            ...(() => {
                const seam = input.suggestionsInline;
                const counted = typeof seam === 'number'
                    ? seam
                    : Array.isArray(seam)
                        ? seam.length
                        : undefined;
    ...
    const result = composeReviewBody(effective, cliVersion, attribution, prevRound, reroute, {
        prev: {
    ...
    const marker = ledgerMarkerFor(effective, result.cappedBy, result.scopeUnproven ?? true, result.dimensionGapsAreDepthOnly ?? false, attribution, runtimeModelId, prevRound, postedInline, result.postedFresh, prevFacts.posted, floorKind, severityFloorKnown(input.severityFloor), {
 ...
 */
function prevLedgerFacts(planPath) {
    try {
        if (!planPath)
            return EMPTY_PREV_FACTS;
        const plan = JSON.parse(readFileSync(planPath, 'utf8'));
        const pr = plan?.prNumber;
        if (!isPositivePrNumber(pr))
            return EMPTY_PREV_FACTS;
        const prev = JSON.parse(readFileSync(join(dirname(planPath), `LailatulCoder-review-pr-${pr}-prev-ledger.json`), 'utf8'));
        const round = Number.isInteger(prev.round) && prev.round > 0 ? prev.round : 0;
        ...
        const findings = rawFindings
            .filter((f) => isLedgerFinding(f, round))
        ...
        return {
            ...
            ...(() => {
                const f = round === 0 || posted === undefined
                    ? undefined
                    : volumeOf(prev.fresh);
                return f === undefined || f > posted ? {} : { fresh: f };
            })(),
        };
    }
    catch {
 ...
 */
function ledgerMarkerFor(input, cappedBy, scopeUnproven, dimensionGapsAreDepthOnly, attribution, runtimeModelId, prevRound, postedInline, freshInline, prevPostedInline, floorKind, floorKnown, carriedWorkList) {
    try {
        if (!input.planPath)
            return null;
        ...
        const failClosed = scopeUnproven ||
            !dimensionGapsAreDepthOnly ||
            cappedBy.some((cap) => cap !== 'unreviewed-dimension');
        const shaCandidate = !failClosed && typeof plan.fetchedSha === 'string'
            ? plan.fetchedSha
            : undefined;
        ...
        const runtime = typeof runtimeModelId === 'string' ? runtimeModelId.trim() : '';
        const declared = typeof input.modelId === 'string' ? input.modelId.trim() : '';
        ...
        const roundStart = typeof plan.reviewModelId === 'string' ? plan.reviewModelId.trim() : '';
        ...
        // channel empty — a deferred `LailatulCoder review submit` run from a terminal
        ...
        const identityDrifted = roundStart !== '' && (runtime === '' || roundStart !== runtime);
        const model = attribution && certifying !== '' && !identityDrifted
            ? certifying
            : undefined;
        return serializeLedger({
            ...
            Math.min(prevRound + 1, LEDGER_MAX_ROUND), (input.draftedComments ?? []), [
 ...
 */
function ingestEntryList(value, field) {
    ...
    const raw = toStringList(value, field).map((entry) => entry.replace(/\r\n?/g, '\n'));
    for (const entry of raw) {
        if (entry
            .split('\n')
            .some((line) => ENTRY_FENCE_DELIMITER_RE.test(line.trim()))) {
            throw new Error(`compose-review: ${field === 'bodyCriticals' ? 'a body Critical' : 'a cannot-tell entry'} quotes a code fence its one-line render cannot carry — redraft ` +
...
function composeReviewBody(input, cliVersion, attribution, prevRound, reroute = {
    indices: [],
    entries: [],
}, 
/**
...
convergence = null) {
    ...
    const carriedIds = convergence
        ? new Set(convergence.prev.findings
            .map((f) => f?.id)
            .filter((id) => typeof id === 'string'))
        : undefined;
    const postedFresh = volumeOf(draftedFindingsOf(input.draftedComments).filter((d) => isFreshDraft(d, Math.min(prevRound + 1, LEDGER_MAX_ROUND), carriedIds, convergence?.prev.complete === true)).length) ?? 0;
    ...
    const suggestionsInline = toCount(input.suggestionsInline, 'suggestionsInline');
    ...
    for (const entry of bodyCriticals) {
        if (rendersAsNothing(stripReviewFooter(stripForUnattributedPost(entry)))) {
            throw new Error('compose-review: a body Critical renders as nothing (marker-only, ' +
                'empty comment, or otherwise invisible) — redraft it with the ' +
                "finding's description");
        }
    }
    const suggestionsDiscarded = toCount(input.suggestionsDiscarded, 'suggestionsDiscarded');
    const suggestionsDroppedAsDuplicates = strippedList(input, 'suggestionsDroppedAsDuplicates', attribution);
    ...
    const { deferred: modelDeferred, relocated: relocatedCriticals, relocatedDeterministic, } = splitDeferralChannel(input.deferredSuggestions);
    ...
    const floorKnown = floorRaw === 'critical' || floorRaw === 'suggestion' || floorRaw === 'auto';
    const floorAbsent = !floorKnown;
    const severityFloor = floorKnown
        ? floorRaw
        : 'auto';
    const cannotTell = ingestEntryList(input.cannotTellCriticals, 'cannotTellCriticals');
    ...
    for (const entry of cannotTell) {
        if (rendersAsNothing(stripReviewFooter(stripForUnattributedPost(entry)))) {
            throw new Error('compose-review: a cannot-tell entry renders as nothing ' +
                '(marker-only, empty comment, or otherwise invisible) — ' +
                "redraft it with the finding's description");
        }
    }
    const uncoverable = toStringList(input.uncoverableChunks, 'uncoverableChunks');
    const unreviewed = toStringList(input.unreviewedDimensions, 'unreviewedDimensions');
        ...
        if (stop !== null) {
            canonicalStopEntries =
                stop.cause === 'round-cap'
                    ? new Set([
                        roundCapStopEntry(typeof stop.cap === 'number'
                            ? stop.cap
                            : LARGE_REVERSE_AUDIT_ROUNDS),
                        roundCapStopEntryZh(typeof stop.cap === 'number'
                            ? stop.cap
                            : LARGE_REVERSE_AUDIT_ROUNDS),
                    ])
                    : new Set([
            ...
            budgetEntry = isRoundCap
                ? roundCapStopDisclosure(typeof stop.cap === 'number'
                    ? stop.cap
                    : LARGE_REVERSE_AUDIT_ROUNDS)
    ...
    // it reads the report the orchestrator's `LailatulCoder review script-lint` step wrote
    ...
    const ownBodyCriticals = modelBodyCriticals.slice(0, modelBodyCriticals.length - relocatedCount);
    const nonDeterministicBodyCriticals = ownBodyCriticals.filter((x) => !DETERMINISTIC_TAG_RE.test(x)).length +
    ...
    if (!input.planPath) {
        coverageEntries.push({
            subject: 'coverage',
            reason: 'no plan was given, so this run cannot show that any of the diff ' +
                'was read',
            subjectZh: '覆盖情况',
        ...
        try {
            ...
            for (const id of cov.missingChunks)
            ...
            for (const id of cov.uncoverableChunks) {
                ...
                const already = uncoverable.some((e) => e === prefix || e.startsWith(`${prefix} `));
                if (!already)
                    uncoverable.push(prefix);
            }
            ...
            if (cov.idleAgents.length > 0) {
                remediation.push('idle agents: relaunch each with the same printed prompt — it already ' +
            ...
            // this line: the line lands in the posted body, and `LailatulCoder review
            // agent-prompt` is not something a PR author can run.
            for (const label of cov.blindAgents) {
                coverageEntries.push({
                    subject: label,
                    publicSubject: publicAgentSubject(label),
                    reason: 'launched with a prompt that never named the diff file, so it ' +
                        'could not have read it',
                    reasonZh: '启动 prompt 从未提到 diff 文件，它不可能读过 diff',
                });
            }
            if (cov.blindAgents.length > 0) {
                remediation.push('blind agents: rebuild each prompt with `"${QWEN_CODE_CLI:-LailatulCoder}" ' +
            ...
            for (const label of cov.unopenedAgents) {
                coverageEntries.push({
                    subject: label,
                    publicSubject: publicAgentSubject(label),
                    reason: 'pointed at diff lines it never opened: it made tool calls, but ' +
                        'none of them read the diff',
                    reasonZh: '启动 prompt 为它指定了 diff 中的行，但它从未打开：有工具调用，' +
                        '却没有一次读取 diff',
                });
            }
            if (cov.unopenedAgents.length > 0) {
                remediation.push('agents that never opened the diff: relaunch each with the same ' +
            ...
            if (cov.rewrittenPrompts.length > 0) {
                remediation.push('rewritten launches: re-run `"${QWEN_CODE_CLI:-LailatulCoder}" review ' +
            ...
            if (cov.missingRoles.length > 0) {
                remediation.push('missing briefs: build every required prompt in one call — ' +
                    `\`"\${QWEN_CODE_CLI:-LailatulCoder}" review agent-prompt --plan ${planRef} ` +
            ...
            if (cov.unreadBriefs.length > 0) {
                remediation.push('unread briefs: relaunch each agent with the same printed prompt — ' +
            ...
            const why = err instanceof TranscriptsUnavailableError
                ? `could not read the agents' transcripts (${err.message})`
                : `the plan could not be used (${err.message})`;
            const whyZh = err instanceof TranscriptsUnavailableError
                ? `无法读取 agent 的运行记录（${err.message}）`
                : `plan 无法使用（${err.message}）`;
            coverageEntries.push({
                subject: 'coverage',
        ...
        try {
            ...
            const findingsToVerify = criticalsInline +
                suggestionsInline +
                nonDeterministicBodyCriticals +
                deferredSuggestions.filter((e) => !DETERMINISTIC_SOURCES.has(e.source))
                    .length;
            const verification = verificationGaps(input.planPath, { postsFindings: findingsToVerify > 0 }, input.env);
            ...
            coverageEntries.push({
                ...
                reason: `could not check that Step 4 and Step 5 ran ` +
                    `(${err.message})`,
                subjectZh: '验证',
                reasonZh: `无法检查步骤 4 与步骤 5 是否运行（${err.message}）`,
            });
    ...
    if (findingsPath !== undefined && findingsPath !== null) {
        if (typeof findingsPath !== 'string' || findingsPath.trim() === '') {
            throw new TypeError(`compose-review: findingsPath must be a non-empty string, got ${JSON.stringify(findingsPath)}`);
        }
        try {
            const findingsContent = readFileSync(findingsPath, 'utf8');
            unverifiedTagCount = (findingsContent.match(UNVERIFIED_FINDING_TAG_RE) ?? []).length;
            findingsUnverifiedAtCompose = unverifiedTagCount > 0;
            if (findingsUnverifiedAtCompose) {
                remediation.push('findings still tagged `— [unverified]`: relaunch the verifier ' +
            ...
            remediation.push('findings file not readable: pass the cumulative reverse-audit ' +
                "findings file — the one every round's `--findings` received — " +
                'as `findingsPath` in the state JSON, and run compose-review ' +
                'again');
        }
    }
    const contextUnavailable = toBool(input.contextUnavailable, 'contextUnavailable');
    ...
    const unlicensedDeferral = deferredSuggestions.length === 0
        ? null
        : floorAbsent
    ...
    if (typeof presubmitRaw !== 'object' || Array.isArray(presubmitRaw)) {
        throw new TypeError(`compose-review: presubmit must be an object, got ${JSON.stringify(presubmitRaw)}`);
    }
    const presubmitObj = presubmitRaw;
    const downgradeApprove = toBool(presubmitObj['downgradeApprove'], 'presubmit.downgradeApprove');
    const downgradeRequestChanges = toBool(presubmitObj['downgradeRequestChanges'], 'presubmit.downgradeRequestChanges');
    const downgradeReasons = toStringList(presubmitObj['downgradeReasons'], 'presubmit.downgradeReasons');
    const modelId = input.modelId;
    let footer = '';
    if (attribution) {
        if (typeof modelId !== 'string' || modelId.trim() === '') {
            throw new TypeError('compose-review: modelId is required (the public footer names the reviewing model)');
        }
        if (!isFooterSafeModelId(modelId)) {
            throw new TypeError('compose-review: modelId is interpolated into the public footer ' +
        ...
        if (modelId.length > MODEL_ID_MAX_CHARS) {
            remediation.push(`body budget: modelId was ${modelId.length} characters and was ` +
        ...
        if (cliVersion.length > MODEL_ID_MAX_CHARS) {
            remediation.push(`body budget: cliVersion was ${cliVersion.length} characters and ` +
    ...
    if (cannotTell.length > 0)
        cappedBy.push('cannot-tell-existing-critical');
    if (missingReceipts.length > 0)
        cappedBy.push('chunk-nobody-read');
    ...
    if (contextUnavailable)
        cappedBy.push('context-unavailable');
    if (unlicensedDeferral !== null)
        cappedBy.push('unlicensed-deferral');
    if (criticalsUnverified)
        cappedBy.push('criticals-unverified');
    if (findingsUnverifiedAtCompose) {
    ...
    const scopeUnproven = missingReceipts.length > 0 ||
    ...
    if ((event === 'APPROVE' || (baseEvent === 'APPROVE' && event === 'COMMENT')) &&
        downgradeApprove) {
        event = 'COMMENT';
        downgraded = true;
    ...
    const bodyBudget = BODY_MAX_CHARS -
        BODY_SAFETY_MARGIN -
     ...
     */
    const trimNote = (ranks, sections, cut) => {
     ...
     */
    const noteTrimmedRanks = (droppedRanks) => {
        if (droppedRanks.length === 0)
            return;
        remediation.push(`body budget: ${droppedRanks
    ...
    const render = (parts, sep) => {
         ...
         */
        const foldNote = (sections, cut) => !hadFold
            ? []
            : [
                {
                    keep: 1,
                    en: `⚠️ The Chinese translation of this body was dropped to fit ` +
                        `GitHub's ${BODY_MAX_CHARS}-character review limit; the ` +
                        `English text below is ${cut
                    ...
                    zh: `⚠️ 为适配 GitHub ${BODY_MAX_CHARS} 字符的评审正文上限，` +
                        `本正文的中文翻译已被丢弃。`,
                },
            ];
        const noteFoldDropped = (sections, cut) => {
            if (!hadFold)
                return;
            bodyTrim.fold = true;
            remediation.push(`body budget: the bilingual fold was dropped to fit GitHub's ` +
        ...
        const hardNote = {
            keep: 1,
            en: `⚠️ This review body was TRUNCATED to fit GitHub's ` +
            ...
            zh: `⚠️ 为适配 GitHub ${BODY_MAX_CHARS} 字符的评审正文上限，本正文已被截断。` +
        ...
        noteTrimmedRanks(droppedRanks);
        remediation.push(`body budget: read the complete blockers in the terminal report and the ` +
    ...
    if (missingReceipts.length > 0) {
        ...
        // to prevent.
        remediation.push('chunks nobody read: build each with `"${QWEN_CODE_CLI:-LailatulCoder}" review ' +
        ...
        const unexplainedReceipts = missingReceipts.filter((id) => !disclosedSubjects.has(`chunk ${id}`));
    ...
    if (uncoverable.length > 0) {
        ...
        const bareGap = bareIds.length > 0 ? describeChunkGap(bareIds, plannedChunks) : null;
    ...
    for (const d of unreviewed) {
        if (seenCaller.has(d))
            continue; // a caller pasting itself twice
        seenCaller.add(d);
        ...
        // dedups).
        const echoesCoverage = covEntries.some((e) => d === e.subject ||
    ...
    const budgetGapItems = [];
    for (const g of budgetGapNotes) {
        for (const gap of g.gaps)
            budgetGapItems.push({ agent: g.agent, gap });
    }
    const keptBudgetGaps = budgetGapItems.filter((it) => !unreviewed.some((d) => d.includes(it.gap)));
    if (keptBudgetGaps.length > 0) {
        ...
        const enList = shown
            .map((it) => `${publicAgentSubject(it.agent) ?? it.agent}: ${mdField(it.gap)}`)
    ...
    const byReason = new Map();
    const reasonZhOf = new Map();
    for (const e of covEntries) {
        if (seenSubjects.has(e.subject))
            continue;
    ...
    for (const [reason, entries] of byReason) {
        ...
        const gap = chunkIds.length > 0 ? describeChunkGap(chunkIds, plannedChunks) : null;
        const shown = [
            ...(gap ? [gap.phrase] : []),
            ...[...named].map(([subject, { count }]) => count > 1 ? `${subject} (×${count})` : subject),
        ];
        const shownZh = [
            ...(gap ? [gap.phraseZh] : []),
            ...[...named.values()].map(({ zh, count }) => count > 1 ? `${zh}（×${count}）` : zh),
        ];
    ...
    const bodyCriticalBlock = bodyCriticals
    ...
    const duplicatesMore = suggestionsDroppedAsDuplicates.length - duplicatesShown.length;
    const duplicatesBlock = suggestionsDroppedAsDuplicates.length === 0
        ? []
        : [
            {
                en: `${suggestionsDroppedAsDuplicates.length} Suggestion-level ` +
                    ...
                    (duplicatesMore > 0
                        ...
                        : ''),
                zh: `本轮确认的 ${suggestionsDroppedAsDuplicates.length} 条建议级发现已在 PR ` +
    ...
    const disclosedChunkIds = new Set();
    for (const e of coverageEntries) {
        const m = /^chunk (\d+)$/.exec(e.subject);
        if (m)
            disclosedChunkIds.add(Number(m[1]));
    }
    const nothingCertified = coverageEntries.some((e) => e.subject === 'coverage') ||
        (plannedChunks.length > 0 &&
            coveredChunks.every((id) => disclosedChunkIds.has(id)));
    const hasCoverageGaps = unreviewed.length + coverageEntries.length > 0 ||
        missingReceipts.length > 0 ||
        uncoverable.length > 0;
    const coverageOpener = nothingCertified
        ? {
            keep: 1,
    ...
    const testPlanBlock = testPlanNotes.length
        ? [
            {
                trim: 2,
    ...
    const unverifiedTagsBlock = !findingsUnverifiedAtCompose
        ? []
    ...
    const repositoryContextBlock = repositoryContextNotes.length
        ? [
            {
                trim: 2,
    ...
    const unlicensedDeferralBlock = unlicensedDeferral === null
        ? []
        : [
            {
                keep: 1,
    ...
    const enforcedShown = Math.min(reroute.entries.length, MAX_DEFERRED_SUGGESTION_LINES);
    const enforcedOverflow = reroute.entries.length - enforcedShown;
    const floorEnforcedNote = reroute.entries.length > 0
        ? [
            {
        ...
        : [];
    const deferredSuggestionsBlock = deferredSuggestions.length
        ? [
            ...floorEnforcedNote,
            {
    ...
    const notReviewedForBody = notReviewedParts.map((p) => ({
        ...p,
        trim: 2,
    }));
    ...
    const convergenceBlock = convergenceNote
    ...
    const continuityBlock = recoveredFromPriorAttempt
        ? [
            {
    ...
    if (suggestionsDiscarded > 0) {
        ...
        clauses.push({
            keep: 1,
            en: `${suggestionsDiscarded} Suggestion-level finding(s) could not be ` +
                `anchored to a changed line and were dropped; nothing further to act ` +
                `on here.`,
            zh: `${suggestionsDiscarded} 条建议级发现无法锚定到改动行，已丢弃；` +
                `此处无需进一步处理。`,
        });
    }
            ...
            ? [
                {
                    ...
                    keep: openerParts.reduce((lowest, c) => Math.min(lowest, c.keep ?? 3), 3),
 ...
 */
function publicAgentSubject(label) {
 ...
 */
export function describeChunkGap(ids, planned) {
...
const fetchPrBodyViaGh = (ownerRepo, prNumber) => {
    const json = gh('pr', 'view', prNumber, '--repo', ownerRepo, '--json', 'body');
    return JSON.parse(json).body ?? '';
};
export function repositoryContextGate(planPath) {
    let plan;
    try {
    ...
    const disclosed = dimensions
        ...
        .map((dimension) => `${mdField(dimension)} — the repository context marks this proof boundary as unverified`);
 ...
 */
export function scriptLintGate(planPath) {
    const criticals = [];
    const unreviewed = [];
    ...
    try {
        ...
        unreviewed.push('the executable-script lint — could not read the plan to check the gate');
    ...
    const reportPath = join(dirname(planPath), scriptLintReportName(plan.prNumber));
    let report;
    try {
        ...
        // `LailatulCoder review script-lint` on EVERY same-repo review, unconditionally. So a
        ...
        if (owed) {
            unreviewed.push('the executable-script lint — `LailatulCoder review script-lint` produced no report');
        }
        return { criticals, unreviewed, disclosed };
    }
    ...
    if (!planDiffHash || report.diffHash !== planDiffHash) {
        unreviewed.push('the executable-script lint — the report is stale or its diff could not be verified; re-run `LailatulCoder review script-lint`');
        return { criticals, unreviewed, disclosed };
    }
    ...
    for (const file of report.checked ?? []) {
        for (const f of file.findings ?? []) {
            if (f.inDiff && f.level !== 'style') {
                criticals.push(`${mdField(file.path)}:${f.line} ${f.code} — ${mdField(f.message)} [lint]`);
            }
        }
    }
    ...
    for (const s of report.skipped ?? []) {
        unreviewed.push(`the executable-script lint — ${mdField(s.path)}: ${s.reason ?? `${s.tool} unavailable`}`);
    }
    for (const e of report.errored ?? []) {
        unreviewed.push(`the executable-script lint — ${e.tool} errored on ${mdField(e.path)}`);
    }
    ...
    for (const d of report.deferred ?? []) {
        disclosed.push(`the executable-script lint — ${mdField(d.path)}: ${d.reason ?? `${d.tool} deferred`}`);
    }
    return { criticals, unreviewed, disclosed };
}
/**
...
function scriptLintReportName(pr) {
    return isPositivePrNumber(pr)
        ? `LailatulCoder-review-pr-${pr}-script-lint.json`
        : 'LailatulCoder-review-script-lint.json';
}
/**
 ...
 */
export function testPlanGate(planPath) {
    const notes = [];
    let plan;
    try {
        plan = JSON.parse(readFileSync(planPath, 'utf8'));
    ...
    let report;
    try {
        report = JSON.parse(readFileSync(join(dirname(planPath), `LailatulCoder-review-pr-${pr}-test-plan.json`), 'utf8'));
    }
    catch {
        return { notes };
    }
    ...
    for (const claim of report.claims ?? []) {
        if (claim.verdict === 'contradicted') {
            notes.push(`${mdField(claim.text)} — ${mdField(claim.observed ?? 'not reproduced')}`);
        }
        else if (claim.verdict === 'differs') {
            notes.push(`${mdField(claim.text)} — this review observed ${mdField(claim.observed ?? 'a different result')}`);
        }
    }
...
function bilingualFromPlan(planPath, fetchPrBody = fetchPrBodyViaGh) {
    ...
    const han = plan?.prDescriptionHasHan;
 ...
 */
function readDraftedComments(path) {
    let raw;
    try {
        raw = readFileSync(path, 'utf8');
    }
    catch (err) {
        throw new Error(`compose-review: cannot read the comments file ${path}: ` +
            `${err.message}. Pass the drafted inline comments — the ` +
    ...
    try {
        ...
        throw new Error(`compose-review: the comments file ${path} is not JSON: ${err.message}`);
    }
    const comments = Array.isArray(parsed)
        ? parsed
        : parsed?.comments;
    if (!Array.isArray(comments)) {
        throw new Error(`compose-review: the comments file ${path} must be a JSON array of ` +
            `comment objects, or a review payload with a \`comments\` array.`);
    }
    const unmarked = unmarkedComments(comments);
    if (unmarked.length > 0) {
        throw new Error(`compose-review: comments[${unmarked.join(', ')}] in ${path} open with ` +
...
export const composeReviewCommand = {
    ...
    describe: 'Compute the review event and body from the drafted comments and run states (the Step 7 invariant, as code); reads the state JSON from --input or stdin',
    builder: (yargs) => yargs
        .option('input', {
        type: 'string',
        ...
        type: 'string',
        demandOption: true,
        describe: 'Path to the drafted inline comments JSON (the review payload, or ' +
        ...
        .option('host', {
        type: 'string',
        describe: 'GitHub Enterprise host (routes gh via GH_HOST) — needed only when ' +
            'the bilingual body-language recovery has to fetch the PR description',
    })
        .option('pr', {
        type: 'number',
        describe: 'The PR this compose is for — the recorded-floor recovery binds ' +
            'the record to it first, exactly as submit binds its own --pr. ' +
            'Pass it on every PR review.',
    })
        .option('repo', {
        type: 'string',
        describe: '<owner>/<repo> under review — the URL-shaped record bar binds ' +
            'it first, exactly as submit binds its own --repo. Pass it on ' +
            'every PR review.',
    })
        .option('skill-args', {
        type: 'string',
        describe: "Path to the CLI-written record of the review's invocation " +
    ...
    handler: async (argv) => {
        const { input, comments, out, host, pr, repo, skillArgs } = argv;
        ...
        if (!comments) {
            throw new Error('compose-review: --comments is required — the inline counts are ' +
        ...
        if (parsed.criticalsInline !== undefined ||
            parsed.suggestionsInline !== undefined) {
            throw new Error('compose-review: `criticalsInline` / `suggestionsInline` are counted ' +
        ...
        const recovered = recordedSeverityFloor({
            ...
            callerPr: typeof pr === 'number' && Number.isSafeInteger(pr) && pr > 0
                ? pr
                : undefined,
            callerRepo: typeof repo === 'string' && isOwnerRepo(repo) ? repo : undefined,
        ...
        footerVersion(process.env['QWEN_CODE_STARTUP_VERSION']) ??
            (await getCliVersion()), operatorReviewSettings().attribution, 
        ...
        const json = JSON.stringify({ ...result, verdictLine: verdictLine(result) }, null, 2);
        if (out) {
        ...
        writeStderrLine(`VOLUME: ${result.postedInline} inline comment(s) this round` +
 ...
 */
function ledgerClaimLine(body) {
    const claim = carriedClaimLine(typeof body === 'string' ? body : '');
    return claim === null
        ? ''
        : stripFooterSpans(stripCommentMarkerLines(claim)).replace(LEADING_INVISIBLE_RE, '');
}
/**
 ...
 */
function draftedFindingsOf(drafted) {
 ...
 */
export function buildLedger(round, drafted, bodyCriticals, 
/**
...
carriedWorkList) {
     ...
     */
    const locatable = (title, where) => title || `(comment carried no text — see the posted finding at ${where})`;
    for (const c of drafted) {
        ...
        findings.push({
            ...
            ...(typeof c.line === 'number' && Number.isInteger(c.line)
                ? { line: c.line }
                : {}),
            title: locatable(title, `${file}${typeof c.line === 'number' ? `:${c.line}` : ''}`),
        });
    }
    for (const b of bodyCriticals) {
        ...
        const { id: carried, title } = readClaim(stripForUnattributedPost(b).replace(LEADING_INVISIBLE_RE, ''));
        findings.push({
            id: idFor(carried),
            sev: 'C',
...
export function verdictLine(r) {
    ...
    const why = {
        ...
        'findings-unverified-at-compose': 'findings were still unverified when the loop ended',
    };
    ...
    if (r.baseEvent === 'REQUEST_CHANGES' &&
        ...
        if (r.downgraded)
            reasons.push('a presubmit check failed');
    ...
    if (r.deferredCount > 0) {
        ...
        const where = r.bodyTrim.deferralList
            ...
            : `listed in the body${r.deferredCount > MAX_DEFERRED_SUGGESTION_LINES
...
//# sourceMappingURL=compose-review.js.map