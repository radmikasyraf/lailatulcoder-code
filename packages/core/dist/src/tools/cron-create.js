...
function recurringExpiryBlurb(maxAgeDays) {
    if (!Number.isFinite(maxAgeDays)) {
        return ('Recurring tasks never auto-expire in this configuration — they keep ' +
    ...
    return (`Recurring tasks auto-expire after ${span} — they fire one final time, ` +
    ...
    async execute() {
        ...
        try {
            ...
            const where = durable
                ...
                : 'Session-only (not written to disk, dies when LailatulCoder Ai exits)';
...
export class CronCreateTool extends BaseDeclarativeTool {
    ...
    constructor(config) {
        ...
        super(CronCreateTool.Name, ToolDisplayNames.CRON_CREATE, 'Schedule a prompt to be enqueued at a future time. Use for both recurring schedules and one-shot reminders.\n\n' +
            ...
            '## Durability\n\n' +
            'By default (durable: false) the job lives only in this LailatulCoder Ai session — nothing is written to disk, and the job is gone when LailatulCoder Ai exits. ' +
            ...
            'Returns a job ID you can pass to CronDelete.', Kind.Other, {
            type: 'object',
            properties: {
                cron: {
                    type: 'string',
                ...
                recurring: {
                    type: 'boolean',
                    description: `true (default) = fire on every cron match until deleted${Number.isFinite(maxAgeDays)
                ...
                durable: {
                    type: 'boolean',
                    description: `true = persist to ${CRON_TASKS_DISPLAY_PATH} and survive restarts. false (default) = in-memory only, dies when LailatulCoder Ai exits. Use true only when the user asks the task to survive across sessions.`,
                },
            },
...
//# sourceMappingURL=cron-create.js.map