...
import { GitWorktreeService, writeWorktreeSessionMarker, } from '../services/gitWorktreeService.js';
...
const enterWorktreeDescription = `Creates an isolated git worktree at \`<projectRoot>/.lailatulcoder/worktrees/<slug>\` and returns its absolute path so subsequent file edits, shell commands, and other tools can operate inside it.
...
class EnterWorktreeInvocation extends BaseToolInvocation {
    ...
    async execute(_signal) {
        ...
        // already inside `.lailatulcoder/worktrees/<slug>/`, a fresh worktree would
        // be provisioned at `<repo>/.lailatulcoder/worktrees/<new>/` — but the
        ...
        // `.lailatulcoder/worktrees` somewhere in cwd qualifies. We also forbid
        ...
        if (/\.lailatulcoder[\\/]worktrees[\\/]/.test(cwd)) {
            const reason = 'Already inside a git worktree. Call exit_worktree first, ' +
        ...
        // monorepo subdirectory would scatter `.lailatulcoder/worktrees/` under each
        ...
        // `<repoRoot>/.lailatulcoder/worktrees/`, regardless of which subdirectory
        ...
        const requested = this.params.name && this.params.name.length > 0
            ? this.params.name
            : undefined;
        ...
        try {
            ...
            debugLogger.warn(`enter_worktree: getCurrentBranch failed at ${projectRoot}: ${error}`);
        }
        ...
        try {
            ...
            debugLogger.warn(`enter_worktree: getCurrentCommitHash failed at ${projectRoot}: ${error}`);
        }
        ...
        try {
            await writeWorktreeSessionMarker(result.worktree.path, this.config.getSessionId());
        }
        catch (error) {
            debugLogger.warn(`enter_worktree: failed to write session marker at ${result.worktree.path}: ${error}`);
        }
        ...
        try {
            await writeWorktreeSession(this.config
                .getSessionService()
            ...
            debugLogger.warn(`enter_worktree: failed to write WorktreeSession sidecar: ${error}`);
        }
        const output = {
            worktreePath: result.worktree.path,
            worktreeBranch: result.worktree.branch,
            message: `Created worktree "${slug}" at ${result.worktree.path} on branch ${result.worktree.branch}. ` +
                `Use this absolute path for all subsequent file operations until you call ${ToolNames.EXIT_WORKTREE}.`,
        };
        debugLogger.debug(`Created user worktree: ${output.worktreePath} (branch=${output.worktreeBranch})`);
        return {
...
export class EnterWorktreeTool extends BaseDeclarativeTool {
    ...
    constructor(config) {
        super(EnterWorktreeTool.Name, ToolDisplayNames.ENTER_WORKTREE, enterWorktreeDescription, Kind.Other, {
            type: 'object',
            properties: {
                name: {
                    type: 'string',
