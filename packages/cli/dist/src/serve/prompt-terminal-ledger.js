...
import { buildApiHistoryFromConversation, detectTurnInterruption, SessionService, TURN_INTERRUPTION_HISTORY_TAIL_COUNT, } from '@lailatul-coder/lailatul-coder-core';
import { appendPromptLedgerRecord, danglingInFlightPromptIds, isPromptLedgerTerminalRecord, readPromptLedgerRecords, recentPromptTerminalRecords, } from '@lailatul-coder/acp-bridge/promptLedger';
/**
 ...
 */
export function createPromptLedgerSink(workspaceCwd, sessionRuntimeBaseDir) {
    const sessionService = new SessionService(workspaceCwd, {
        runtimeBaseDir: sessionRuntimeBaseDir,
    });
    return {
        appendSync(sessionId, record) {
            appendPromptLedgerRecord(sessionService.getPromptLedgerPath(sessionId), record);
        },
        transcriptTailUuid(sessionId) {
            return readTranscriptTailUuid(sessionService.getSessionTranscriptPath(sessionId));
        },
    };
}
/**
 ...
 */
export function readTranscriptTailUuid(transcriptPath) {
    let contents;
    try {
 ...
 */
export async function reconcileDanglingPromptTerminals(sessionService, sessionId) {
    ...
    // settled in_flight.
    const settledPromptIds = new Set(records.filter(isPromptLedgerTerminalRecord).map((r) => r.promptId));
    let targetAdmission;
    for (const record of records) {
        if (!isPromptLedgerTerminalRecord(record) &&
    ...
    if (admissionMarker !== undefined) {
        markerIndex = messages.findIndex((record) => record.uuid === admissionMarker);
        let wroteAfterMarker = false;
        for (let i = markerIndex + 1; i < messages.length; i++) {
            const record = messages[i];
            if (record === undefined || record.type === 'system')
                continue;
        ...
        if (markerIndex < 0 || !wroteAfterMarker)
            return;
    }
    ...
    for (let idx = 0; idx < messages.length; idx++) {
        ...
        if (record.type === 'system') {
            if (isCompressionResetRecord(record)) {
                ...
                // to the wall clock.
                const afterAdmission = admissionMarker !== undefined
                    ? idx > markerIndex
    ...
    for (const record of records) {
        if (isPromptLedgerTerminalRecord(record) && record.promptId !== target) {
            ...
            if (record.code === 'prompt_deadline_exceeded')
                return;
            lastOtherTerminalAt = Math.max(lastOtherTerminalAt, record.at);
        }
    }
    if (compressedAfterAdmission ||
    ...
    const interrupted = verdict.kind !== 'none' || tailHoldsAnyFunctionCall(historyTail);
    ...
    if (refetch.length !== snapshotLength)
        return;
 ...
 */
function isCompressionResetRecord(record) {
    if (record.type !== 'system' || record.subtype !== 'chat_compression') {
        return false;
    }
    return Boolean(record.systemPayload
        ?.compressedHistory);
}
/**
 ...
 */
function tailHoldsAnyFunctionCall(history) {
 ...
 */
export function readRecentPromptTerminals(sessionService, sessionId) {
    try {
        const terminals = recentPromptTerminalRecords(readPromptLedgerRecords(sessionService.getPromptLedgerPath(sessionId), {
 ...
 */
export function withPromptTerminals(session, terminals) {
    if (terminals === undefined || terminals.length === 0)
        return session;
    return { ...session, promptTerminals: [...terminals] };
}
//# sourceMappingURL=prompt-terminal-ledger.js.map