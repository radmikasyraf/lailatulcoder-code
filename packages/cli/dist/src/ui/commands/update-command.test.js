...
function context(executionMode, enableAutoUpdate) {
...
describe('updateCommand', () => {
    ...
    it('hands an interactive update off to the parent process', async () => {
        ...
        expect(commandContext.services.settings.merged.general?.enableAutoUpdate).toBeUndefined();
    ...
    it('returns manual instructions in interactive mode when auto-update is disabled', async () => {
        ...
        expect(relaunchForUpdate).not.toHaveBeenCalled();
        expect(commandContext.services.settings.merged.general?.enableAutoUpdate).toBe(false);
    });
    it('does not update standalone installs in interactive mode when auto-update is disabled', async () => {
        ...
        const result = await updateCommand.action(context('interactive', false), '');
        expect(result).toEqual({
            type: 'message',
            messageType: 'info',
    ...
    it('does not mutate enableAutoUpdate when relaunching throws', async () => {
        ...
        await expect(updateCommand.action(commandContext, '')).rejects.toThrow('spawn failed');
        expect(commandContext.services.settings.merged.general?.enableAutoUpdate).toBeUndefined();
    });
    ...
    it('returns an error when the update check fails', async () => {
        ...
        expect(result).toEqual({
            type: 'message',
            messageType: 'error',
            content: 'Failed to check for updates (registry error). Please check your network or registry configuration.',
        });
...
//# sourceMappingURL=update-command.test.js.map