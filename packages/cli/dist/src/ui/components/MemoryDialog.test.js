...
import { AUTO_MEMORY_INDEX_FILENAME, clearAutoMemoryRootCache, getAutoMemoryRoot, } from '@lailatul-coder/lailatul-coder-core';
...
const folderOpenCommandByPlatform = {
    darwin: 'open',
    win32: 'explorer',
};
function createMockSpawnChild(event = 'spawn', error) {
    const child = new EventEmitter();
    child.unref = vi.fn();
    queueMicrotask(() => {
...
function expectedFolderOpenCommand(platform = process.platform) {
...
describe('MemoryDialog', () => {
    ...
    beforeEach(() => {
        ...
        vi.stubEnv('QWEN_CODE_MEMORY_BASE_DIR', path.join(os.homedir(), '.qwen-memory-test'));
    ...
    it('opens managed memory folders instead of launching an editor', async () => {
        ...
        render(_jsx(MemoryDialog, { onClose: onClose }));
        ...
        expect(mockedSpawn).toHaveBeenCalledWith(expectedFolderOpenCommand(), [path.join(os.homedir(), '.qwen-memory-test', 'memories')], expect.objectContaining({ detached: true, stdio: 'ignore' }));
        expect(mockedFs.mkdir).toHaveBeenCalledWith(path.join(os.homedir(), '.qwen-memory-test', 'memories'), { recursive: true });
        expect((mockedSpawn.mock.results[0]?.value).unref).toHaveBeenCalled();
        expect(launchEditor).not.toHaveBeenCalled();
        expect(onClose).toHaveBeenCalled();
    });
    it.each(['darwin', 'win32'])('opens managed memory folders on %s without display variables', async (platform) => {
        ...
        expect(mockedSpawn).toHaveBeenCalledWith(expectedFolderOpenCommand(platform), [path.join(os.homedir(), '.qwen-memory-test', 'memories')], expect.objectContaining({ detached: true, stdio: 'ignore' }));
        expect(launchEditor).not.toHaveBeenCalled();
        expect(onClose).toHaveBeenCalled();
    });
    it('opens the project managed memory folder after moving selection down', async () => {
        ...
        const launchEditor = vi.fn();
        mockedUseLaunchEditor.mockReturnValue(launchEditor);
        const { lastFrame } = render(_jsx(MemoryDialog, { onClose: onClose }));
        expect(lastFrame()).toContain('› 1. User memory');
        act(() => {
            mockedUseKeypress.mock.calls.at(-1)[0]({ name: 'down' });
        });
        expect(lastFrame()).toContain('› 2. Project memory');
        await act(async () => {
            mockedUseKeypress.mock.calls.at(-1)[0]({ name: 'return' });
            await Promise.resolve();
            await Promise.resolve();
        });
        expect(mockedSpawn).toHaveBeenCalledWith(expectedFolderOpenCommand(), [getAutoMemoryRoot('/tmp/project')], expect.objectContaining({ detached: true, stdio: 'ignore' }));
        expect(launchEditor).not.toHaveBeenCalled();
        expect(onClose).toHaveBeenCalled();
    });
    it('opens the second visible item with its numeric shortcut', async () => {
        ...
        const keypressHandler = mockedUseKeypress.mock.calls[0][0];
        await act(async () => {
            keypressHandler({ name: '2', sequence: '2' });
            await Promise.resolve();
            await Promise.resolve();
        });
        expect(mockedSpawn).toHaveBeenCalledWith(expectedFolderOpenCommand(), [getAutoMemoryRoot('/tmp/project')], expect.objectContaining({ detached: true, stdio: 'ignore' }));
    ...
    it('opens the managed memory index in an editor on headless Linux', async () => {
        ...
        expect(mockedFs.access).toHaveBeenCalledWith(path.join(os.homedir(), '.qwen-memory-test', 'memories', AUTO_MEMORY_INDEX_FILENAME));
        expect(launchEditor).toHaveBeenCalledWith(path.join(os.homedir(), '.qwen-memory-test', 'memories', AUTO_MEMORY_INDEX_FILENAME));
        expect(onClose).toHaveBeenCalled();
    });
    it('shows an error when opening a managed memory folder fails', async () => {
        const onClose = vi.fn();
        mockedSpawn.mockImplementationOnce(() => createMockSpawnChild('error', new Error('ENOENT')));
        const { lastFrame } = render(_jsx(MemoryDialog, { onClose: onClose }));
        const keypressHandler = mockedUseKeypress.mock.calls[0][0];
        await act(async () => {
            keypressHandler({ name: 'return' });
    ...
    it('toggles Auto-skill on Enter and persists to workspace settings', () => {
        ...
        expect(setValue).toHaveBeenCalledWith(expect.anything(), 'memory.enableAutoSkill', true);
    ...
    it('toggles Auto-skill back OFF on Enter (re-disable path)', () => {
        ...
        expect(lastFrame()).toContain('› Auto-skill: on');
        pressKey({ name: 'return' });
        expect(setValue).toHaveBeenCalledWith(expect.anything(), 'memory.enableAutoSkill', false);
    ...
    it('toggles autoSkillConfirm on Enter and persists to workspace settings', () => {
        ...
        // Enter toggles
        pressKey({ name: 'return' });
        expect(setValue).toHaveBeenCalledWith(expect.anything(), 'memory.autoSkillConfirm', false);
    ...
    it('keeps QWEN.md editor entries when managed memory is unavailable', async () => {
        ...
        const { lastFrame } = render(_jsx(MemoryDialog, { onClose: vi.fn() }));
        expect(lastFrame()).toContain('› 1. User memory');
        expect(lastFrame()).toContain(`Saved in ~${path.sep}.qwen${path.sep}QWEN.md`);
        expect(lastFrame()).toContain('2. Project memory');
        ...
        expect(launchEditor).toHaveBeenCalledWith(expect.stringMatching(/[/\\]\.qwen[/\\]QWEN\.md$/));
        expect(mockedSpawn).not.toHaveBeenCalled();
    });
    it('opens the project QWEN.md editor entry when managed memory is unavailable', async () => {
        ...
        render(_jsx(MemoryDialog, { onClose: vi.fn() }));
        const keypressHandler = mockedUseKeypress.mock.calls[0][0];
        await act(async () => {
            keypressHandler({ name: '2', sequence: '2' });
            await Promise.resolve();
        ...
        expect(launchEditor).toHaveBeenCalledWith(path.join('/tmp/project', 'QWEN.md'));
        expect(mockedSpawn).not.toHaveBeenCalled();
    });
    it('reflects the persisted value when the dialog is reopened (remounted)', () => {
        ...
        mockedUseSettings.mockReturnValue({ setValue, merged });
        const pressKey = (key) => {
            const keypressHandler = mockedUseKeypress.mock.calls[mockedUseKeypress.mock.calls.length - 1][0];
            act(() => {
                keypressHandler(key);
        ...
        const first = render(_jsx(MemoryDialog, { onClose: vi.fn() }));
        ...
        const second = render(_jsx(MemoryDialog, { onClose: vi.fn() }));
