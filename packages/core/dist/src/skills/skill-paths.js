...
export const PROJECT_SKILLS_RELATIVE_DIR = path.join('.lailatulcoder', 'skills');
export const ARCHIVED_SKILLS_RELATIVE_DIR = path.join('.lailatulcoder', 'archived-skills');
export const SKILL_FILE_NAME = 'SKILL.md';
export function getProjectSkillsRoot(projectRoot) {
    return path.join(projectRoot, PROJECT_SKILLS_RELATIVE_DIR);
}
export function getArchivedSkillsRoot(projectRoot) {
    return path.join(projectRoot, ARCHIVED_SKILLS_RELATIVE_DIR);
}
export const PENDING_SKILLS_RELATIVE_DIR = path.join('.lailatulcoder', 'pending-skills');
/**
 * Staging root for auto-skills awaiting user confirmation. Deliberately a
 * SIBLING of `.lailatulcoder/skills/` so `loadSkillsFromDir` never discovers
 * unconfirmed skills (it scans the skills root only).
 */
export function getPendingSkillsRoot(projectRoot) {
    return path.join(projectRoot, PENDING_SKILLS_RELATIVE_DIR);
}
export function isProjectSkillPath(filePath, projectRoot) {
...
export function assertProjectSkillPath(targetPath, projectRoot) {
    if (!isProjectSkillPath(targetPath, projectRoot)) {
        throw new Error(`Skills writes are restricted to ${getProjectSkillsRoot(projectRoot)}. ` +
 ...
 */
export async function assertRealProjectSkillPath(targetPath, projectRoot) {
    ...
    for (;;) {
        try {
            ...
            if (real !== realSkillsRoot &&
                !real.startsWith(realSkillsRoot + path.sep)) {
                throw new Error(`Skills write blocked: symlink traversal detected — resolved path "${real}" ` +
            ...
            if (err.code === 'ENOENT') {
                ...
                try {
                    ...
                    if (lstatResult.isSymbolicLink()) {
                        throw new Error(`Skills write blocked: dangling symlink detected at "${check}".`);
                    }
                }
                catch (lstatErr) {
