...
export async function runThrottledOnce(opts, task) {
    const minIntervalMs = opts.minIntervalMs ?? ONE_DAY_MS;
    const staleLockMs = opts.staleLockMs ?? STALE_LOCK_MS;
    // First-ever housekeeping pass may run before ~/.lailatulcoder/ exists. mode 0o700
    // matches the rest of the codebase's convention for ~/.lailatulcoder/ subdirs
    ...
    const firstFreshForMs = await markerFreshForMs(opts.markerPath, minIntervalMs, opts.name);
    ...
    try {
        ...
        const secondFreshForMs = await markerFreshForMs(opts.markerPath, minIntervalMs, opts.name);
        ...
        try {
            ...
            if (taskCompleted) {
                try {
                    ...
                    debugLogger.debug(`${opts.name}: marker write failed (cleanup succeeded)`, err);
                }
            }
        }
...
async function markerFreshForMs(markerPath, minIntervalMs, name) {
    try {
...
//# sourceMappingURL=throttledOnce.js.map