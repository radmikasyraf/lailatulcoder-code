...
function parts(record) {
    ...
    return (record.message?.parts ?? []).filter((part) => part !== null && typeof part === 'object');
}
...
function functionResponses(record) {
    return parts(record).flatMap((part) => {
...
function hasVisibleText(record) {
    return parts(record).some((part) => part.thought !== true &&
        typeof part.text === 'string' &&
        part.text.trim().length > 0);
}
function uniqueNameMatch(pending, matchingIndexes) {
    ...
    const freshIndexes = matchingIndexes.filter((candidateIndex) => pending[candidateIndex]?.carriedFromPrefix !== true);
    return freshIndexes.length === 1 ? freshIndexes[0] : -1;
}
function closeToolCall(pending, response) {
    let index = -1;
    if (response.id !== undefined) {
        index = pending.findIndex((call) => call.id === response.id);
        if (index < 0 && response.name !== undefined) {
            const matchingIndexes = pending.flatMap((call, candidateIndex) => call.id === undefined && call.name === response.name
                ? [candidateIndex]
                : []);
        ...
        const matchingIndexes = pending.flatMap((call, candidateIndex) => call.name === response.name ? [candidateIndex] : []);
        index = uniqueNameMatch(pending, matchingIndexes);
    }
    if (index < 0)
        return false;
...
function resolveCompletedTurnBranchCandidateInRange(input) {
    const { activeChain, startIndex, endIndex, startExclusiveRecordUuid, pendingCallsAtStart, } = input;
    ...
    const pendingCalls = pendingCallsAtStart.map((call) => ({
        ...call,
    ...
    for (let index = startIndex + 1; index <= endIndex; index++) {
        ...
        for (const response of responses) {
            if (!closeToolCall(pendingCalls, response))
                return undefined;
        }
    }
    if (pendingCalls.some((call) => !call.carriedFromPrefix))
        return undefined;
    let assistantRecordUuid;
    for (let index = lastToolResultIndex + 1; index <= endIndex; index++) {
        ...
        if (assistantRecordUuid !== undefined)
            return undefined;
...
export function updatePendingBranchToolCalls(pendingCalls, record) {
    pendingCalls.push(...functionCalls(record));
    for (const response of functionResponses(record)) {
        closeToolCall(pendingCalls, response);
    }
}
export function collectPendingBranchToolCalls(records) {
    const pendingCalls = [];
...
export function parseBranchCheckpointPayload(value) {
...
export function resolveBranchPoints(activeChain) {
    ...
    for (let index = 0; index < activeChain.length; index++) {
        ...
        if (!payload)
            continue;
        const startIndex = payload.startExclusiveRecordUuid === null
            ? -1
            : (recordIndexes.get(payload.startExclusiveRecordUuid) ?? -1);
        if (payload.startExclusiveRecordUuid !== null &&
        ...
        if (startIndex >= 0)
            boundaryIndexes.add(startIndex);
    }
    const pendingCallsAtBoundary = new Map();
    const pendingCalls = [];
    for (let index = 0; index < activeChain.length; index++) {
        const record = activeChain[index];
        pendingCalls.push(...functionCalls(record));
        ...
        if (boundaryIndexes.has(index)) {
            pendingCallsAtBoundary.set(index, pendingCalls.map((call) => ({ ...call })));
        }
    }
    const checkpointByAssistantUuid = new Map();
    for (const { checkpoint, checkpointIndex, startIndex, payload, } of checkpoints) {
        const candidate = resolveCompletedTurnBranchCandidateInRange({
            ...
            pendingCallsAtStart: startIndex < 0 ? [] : (pendingCallsAtBoundary.get(startIndex) ?? []),
        });
        if (!candidate ||
            candidate.assistantRecordUuid !== payload.assistantRecordUuid) {
            continue;
        }
        const previousCheckpoint = checkpointByAssistantUuid.get(candidate.assistantRecordUuid);
        ...
        checkpointByAssistantUuid.set(candidate.assistantRecordUuid, checkpoint.uuid);
    }
    return points;
}
//# sourceMappingURL=branch-points.js.map