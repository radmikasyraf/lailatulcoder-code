 ...
 */
// Centralised path constants and helpers for the `LailatulCoder review` subcommands.
 ...
 */
export function assertWritableOutPath(out) {
    if (out.trim() === '') {
...
export const REVIEW_TMP_DIR = join('.lailatulcoder', 'tmp');
export const REVIEWS_DIR = join('.lailatulcoder', 'reviews');
export const REVIEW_CACHE_DIR = join('.lailatulcoder', 'review-cache');
/**
 ...
 */
export const LEASE_PREFIX = 'LailatulCoder-review-lease-';
/**
 * Where the skill tees `LailatulCoder review parse-args`'s verdict (SKILL Step 0). A fixed,
 ...
 */
export const PARSE_ARGS_REPORT = join(REVIEW_TMP_DIR, 'LailatulCoder-review-parse-args.json');
/** Worktree path for a given PR review session. */
export function worktreePath(prNumber) {
 ...
 */
export function probeWorktreePath(worktree) {
 ...
 */
export function baseWorktreePath(worktree) {
 ...
 */
export function scratchWorktreePath(worktree, label) {
    ...
    if (!safe) {
        throw new TypeError(`scratch label ${JSON.stringify(label)} keeps no path-safe character ` +
 ...
 */
export function scratchWorktreePrefix(worktree) {
 ...
 */
export function scratchLabel(label) {
    return (label
...
export function reviewBranch(prNumber) {
    return `LailatulCoder-review/pr-${prNumber}`;
}
/**
 * Per-target side-file path (review JSON, PR context, presubmit report).
 *
 * Files live under `.lailatulcoder/tmp/` rather than the OS temp dir so the path is
...
export function tmpFile(target, suffix) {
    return join(REVIEW_TMP_DIR, `LailatulCoder-review-${safeTarget(target)}-${suffix}`);
}
/** Filename prefix used by `tmpFile`; useful for cleanup globbing. */
export function tmpPrefix(target) {
    return `LailatulCoder-review-${safeTarget(target)}-`;
}
/**
