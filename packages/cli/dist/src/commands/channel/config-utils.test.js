...
describe('resolveEnvVars', () => {
    ...
    it('throws when referenced env var is not set', () => {
        expect(() => resolveEnvVars(`$${ENV_KEY}`)).toThrow(`Environment variable ${ENV_KEY} is not set`);
    });
...
describe('parseChannelConfig', () => {
    it('throws when type is missing', async () => {
        await expect(parseChannelConfig('bot', {})).rejects.toThrow('missing required field "type"');
    });
    it('throws for unsupported channel type', async () => {
        await expect(parseChannelConfig('bot', { type: 'slack' })).rejects.toThrow('"slack" is not supported');
    });
    it('throws when plugin-required fields are missing', async () => {
        await expect(parseChannelConfig('bot', { type: 'telegram' })).rejects.toThrow('requires "token"');
    });
    ...
    it('throws a clear error when token is not a string', async () => {
        await expect(parseChannelConfig('bot', { type: 'telegram', token: 123 })).rejects.toThrow('Channel "bot" field "token" must be a string.');
    });
    it('throws a clear error when dingtalk credentials are not strings', async () => {
        await expect(parseChannelConfig('bot', {
            type: 'dingtalk',
            clientId: 123,
            clientSecret: 'secret',
    ...
    it('does not resolve plugin fields twice when declarations overlap', async () => {
        ...
        }, process.cwd(), { resolveEnvVars: 'available' });
    ...
    it('rejects available env vars when explicitly empty', async () => {
        ...
        }, process.cwd(), { resolveEnvVars: 'available' })).rejects.toThrow('Environment variable TEST_EMPTY_SECRET is empty (referenced as $TEST_EMPTY_SECRET)');
    ...
    it('rejects available env vars when unset', async () => {
        ...
        }, process.cwd(), { resolveEnvVars: 'available' })).rejects.toThrow('Environment variable TEST_MISSING_SECRET is not set (referenced as $TEST_MISSING_SECRET). Set the variable or remove the $ prefix to use a literal value.');
    });
    it('rejects unavailable required credential env vars', async () => {
        ...
        }, process.cwd(), { resolveEnvVars: 'available' })).rejects.toThrow('Environment variable TEST_MISSING_TOKEN is not set (referenced as $TEST_MISSING_TOKEN). Set the variable or remove the $ prefix to use a literal value.');
    });
    ...
    it('rejects an unknown approvalMode', async () => {
        await expect(parseChannelConfig('bot', {
            type: 'bare',
            approvalMode: 'YOLO',
        })).rejects.toThrow('Channel "bot" field "approvalMode" must be one of: plan, default, auto-edit, auto, yolo.');
    });
    ...
    it('rejects a non-object identity', async () => {
        await expect(parseChannelConfig('bot', { type: 'bare', identity: 'ops' })).rejects.toThrow('Channel "bot" field "identity" must be an object.');
    });
    it('rejects a non-string identity field', async () => {
        await expect(parseChannelConfig('bot', { type: 'bare', identity: { id: 123 } })).rejects.toThrow('Channel "bot" field "identity.id" must be a string.');
    });
    it('rejects a non-object memoryScope', async () => {
        await expect(parseChannelConfig('bot', { type: 'bare', memoryScope: ['ops'] })).rejects.toThrow('Channel "bot" field "memoryScope" must be an object.');
    });
    it('rejects an unknown memoryScope.mode', async () => {
        await expect(parseChannelConfig('bot', {
            type: 'bare',
            memoryScope: { mode: 'full' },
        })).rejects.toThrow('Channel "bot" field "memoryScope.mode" must be "metadata-only".');
    });
    ...
    it('rejects non-env webhook secretEnv values', async () => {
        await expect(parseChannelConfig('dingtalk-main', {
            type: 'bare',
            token: 'token',
            webhooks: {
                sources: {
        ...
        })).rejects.toThrow('Channel "dingtalk-main" field "webhooks.sources.github-ci.secretEnv" must be an environment variable name or $-prefixed reference.');
    });
    it('resolves existing uppercase webhook secretEnv names without underscores', async () => {
        ...
        try {
            ...
            expect(config['webhooks']?.sources['github-ci']?.secret).toBe('secret-from-env');
        }
        finally {
            delete process.env['MYSECRET'];
        }
    ...
    it('rejects webhook secretEnv refs when the environment variable is unset', async () => {
        ...
        })).rejects.toThrow('Channel "dingtalk-main" field "webhooks.sources.custom.secretEnv" references an unset environment variable.');
    });
    it('rejects webhook secretEnv refs when the environment variable is empty', async () => {
        ...
        try {
            ...
            })).rejects.toThrow('Channel "dingtalk-main" field "webhooks.sources.custom.secretEnv" references an empty environment variable.');
        }
        finally {
    ...
    it('rejects webhook targets without chatId or senderId', async () => {
        ...
        })).rejects.toThrow('Channel "dingtalk-main" field "webhooks.sources.custom.targets.default.senderId" must be a string.');
    });
    it('rejects webhook sources with non-string secretEnv', async () => {
        ...
        })).rejects.toThrow('Channel "dingtalk-main" field "webhooks.sources.custom.secretEnv" must be a string.');
    });
    it('rejects webhook sources with non-string secret', async () => {
        ...
        })).rejects.toThrow('Channel "dingtalk-main" field "webhooks.sources.custom.secret" must be a string.');
    });
    it('rejects webhook sources without a secret', async () => {
        ...
        })).rejects.toThrow('Channel "dingtalk-main" field "webhooks.sources.custom" must define exactly one of "secret" or "secretEnv".');
    });
