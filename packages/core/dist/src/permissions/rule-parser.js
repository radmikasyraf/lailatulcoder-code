...
import { normalizeMcpToolName, sanitizeToolNameForProvider, } from '../utils/tool-name-utils.js';
 ...
 */
export function getSpecifierKind(canonicalToolName) {
 ...
 */
export function toolMatchesRuleToolName(ruleToolName, contextToolName) {
...
export function parseRule(raw) {
        ...
        for (const part of parts) {
            ...
            if (colonIdx > 0) {
                ...
                if (key && /^[a-zA-Z_][a-zA-Z0-9_]*$/.test(key)) {
                    if (valuePattern === '') {
                        debugLogger.warn(`Empty valuePattern in rule "${trimmed}": key="${key}" will only match empty strings. Use "*" to match any value.`);
                    }
                    matchers.push({ key, valuePattern });
                    continue;
                }
                else if (key && !/^[a-zA-Z_][a-zA-Z0-9_]*$/.test(key)) {
                    debugLogger.warn(`Invalid key "${key}" in rule "${trimmed}": keys must match /^[a-zA-Z_][a-zA-Z0-9_]*$/. Hyphens and dots are not supported.`);
                }
            }
    ...
    else if (specifierKind !== 'literal' &&
        rawSpecifier.includes(':') &&
        !rawSpecifier.startsWith('domain:')) {
        debugLogger.warn(`key:value syntax is only supported for literal-specifier tools (got ${specifierKind} for "${canonicalName}")`);
    }
    return {
        raw: trimmed,
        toolName: canonicalName,
...
export function parseRules(raws) {
    return raws
        ...
        .map((r) => {
        if (r.invalid) {
            debugLogger.warn(`Ignoring malformed rule (unbalanced parentheses): ${r.raw}`);
        }
        return r;
    });
}
 ...
 */
const CANONICAL_TO_RULE_DISPLAY = {
    // Read meta-category
    read_file: 'Read',
    zoom_image: 'Read',
    grep_search: 'Read',
 ...
 */
export function getRuleDisplayName(canonicalToolName) {
 ...
 */
export function buildPermissionRules(ctx) {
            ...
            if (ctx.toolParams) {
                for (const key of stableParamKeys) {
                    ...
                    if (typeof v === 'string' || typeof v === 'number') {
                        ...
                        if (ctx.specifier && String(v) === ctx.specifier)
                            continue;
                        parts.push(`${key}:${v}`);
                    }
                }
 ...
 *
 * `//Users/mochi/.lailatulcoder/**` → `/Users/mochi/.lailatulcoder/`
 * `/src/**`                → `src/`
 */
function cleanPathSpecifier(specifier) {
 ...
 *
 * Examples:
 *   `["Read(//Users/mochi/.lailatulcoder/**)"]`  → `"read files in /Users/mochi/.lailatulcoder/"`
 ...
 */
export function buildHumanReadableRuleLabel(rules) {
    if (!rules.length)
        return '';
    const parts = [];
    for (const rule of rules) {
        ...
        const canonicalName = Object.entries(CANONICAL_TO_RULE_DISPLAY).find(([, v]) => v === displayName)?.[0];
 ...
 */
function precedingBackslashCount(command, index) {
    let count = 0;
 ...
 * `rm`.
 */
function isAsyncOperator(command, index) {
    if (command[index + 1] === '>') {
        return false;
    }
 ...
 */
export function splitCompoundCommandSegments(command) {
    const segments = [];
    let inSingle = false;
    let inDouble = false;
    let escaped = false;
...
export function splitCompoundCommand(command) {
    const commands = splitCompoundCommandSegments(command).map((segment) => segment.command);
 ...
 */
function matchesParamValuePattern(pattern, value) {
 ...
 */
function evaluateParamMatchers(matchers, toolParams, ruleRaw) {
    if (!toolParams) {
    ...
    for (const matcher of matchers) {
        if (!Object.hasOwn(toolParams, matcher.key)) {
            debugLogger.debug(`Param matcher failed: rule="${ruleRaw}" key=${matcher.key} expected="${matcher.valuePattern}" actual=missing`);
            return false;
        }
        const actualValue = toolParams[matcher.key];
        if (actualValue === undefined || actualValue === null) {
            debugLogger.debug(`Param matcher failed: rule="${ruleRaw}" key=${matcher.key} expected="${matcher.valuePattern}" actual=null/undefined`);
            return false;
        }
        if (typeof actualValue !== 'string' && typeof actualValue !== 'number') {
            debugLogger.debug(`Param matcher skipped: rule="${ruleRaw}" key=${matcher.key} value is ${typeof actualValue}`);
            return false;
        }
        const actualStr = String(actualValue);
        if (!matchesParamValuePattern(matcher.valuePattern, actualStr)) {
            debugLogger.debug(`Param matcher failed: rule="${ruleRaw}" key=${matcher.key} expected="${matcher.valuePattern}" actual="${actualStr}"`);
            return false;
        }
    }
    return true;
...
function stripLeadingVariableAssignments(command) {
    ...
    try {
        ...
        while (firstCommandToken < tokens.length &&
 ...
 */
export function resolvePathPattern(specifier, projectRoot, cwd) {
 ...
 */
export function matchesPathPattern(specifier, filePath, projectRoot, cwd, matchMode = 'lexical') {
    const resolvedPattern = resolvePathPattern(specifier, projectRoot, cwd);
    const patterns = matchMode === 'canonical'
    ...
    const paths = matchMode === 'canonical'
        ? getCanonicalPathMatchCandidates(filePath, cwd)
        : [toPosixPath(filePath)];
    return paths.some((candidate) => matchers.some((isMatch) => isMatch(candidate)));
}
function getCanonicalPatternCandidates(resolvedPattern) {
    ...
    if (canonicalBase !== undefined) {
        candidates.add(`${toPosixPath(canonicalBase)}${resolvedPattern.slice(base.length)}`);
    }
    return [...candidates];
}
function getCanonicalPathMatchCandidates(filePath, cwd) {
 ...
 */
export function matchesDomainPattern(specifier, domain) {
 ...
 */
export function matchesMcpPattern(pattern, toolName) {
    if (pattern === toolName) {
        return true;
    }
 ...
 */
export function matchesRule(rule, toolName, command, filePath, domain, pathContext, specifier, toolParams, toolAliases, pathMatchMode = 'lexical') {
    ...
    // ── MCP tool matching ────────────────────────────────────────────────
    if (rule.toolName.startsWith('mcp__') ||
        canonicalCtxToolName.startsWith('mcp__')) {
        const matchesLegacyExactName = !rule.toolName.endsWith('*') &&
            rule.toolName.split('__').length >= 3 &&
            (toolAliases ?? []).some((alias) => rule.toolName === resolveToolName(alias));
        const matchesMcpName = matchesMcpPattern(rule.toolName, canonicalCtxToolName) ||
        ...
        if (rule.specifier) {
            debugLogger.debug(`MCP rule "${rule.raw}" has specifier "${rule.specifier}" — MCP tool names already encode server identity; specifier not supported`);
            return false;
        }
    ...
    if (rule.specifier) {
        switch (kind) {
            ...
            case 'path': {
                ...
                specifierMatched = matchesPathPattern(rule.specifier, filePath, ctx.projectRoot, ctx.cwd, pathMatchMode);
                break;
            }
            case 'domain': {
                if (domain === undefined) {
