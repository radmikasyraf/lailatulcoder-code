...
import { isNodeError, resolveBundleDir } from '@lailatul-coder/lailatul-coder-core';
...
async function handleNew(args) {
    try {
        if (args.template) {
            ...
            writeStdoutLine(`Successfully created new extension from template "${args.template}" at ${args.path}.`);
        }
        else {
            ...
            await writeFile(join(args.path, 'LailatulCoder-extension.json'), JSON.stringify(manifest, null, 2));
            writeStdoutLine(`Successfully created new extension at ${args.path}.`);
        }
        writeStdoutLine(`You can install this using "LailatulCoder extensions link ${args.path}" to test it out.`);
    }
    catch (error) {
...
async function getBoilerplateChoices() {
    ...
    try {
        ...
        if (!isMissing) {
            writeStderrLine(`Warning: failed to read extension templates: ${getErrorMessage(e)}`);
        }
