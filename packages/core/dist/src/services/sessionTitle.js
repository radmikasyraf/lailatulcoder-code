...
import { getStartupContextLength, stripSystemReminderBlocks, } from '../utils/environmentContext.js';
...
const TITLE_USER_PROMPT = 'Generate the session title now. Populate the schema with a single short title string.';
const TITLE_SCHEMA = {
    type: 'object',
    properties: {
        title: {
...
const LEADING_PAIRED_BRACKETS_RE = /^\s*[「『【〈《][^」』】〉》]*[」』】〉》]\s*/;
const TRAILING_PAIRED_BRACKETS_RE = /\s*[「『【〈《][^」』】〉》]*[」』】〉》]\s*$/;
/**
 ...
 */
export async function tryGenerateSessionTitle(config, abortSignal, userDisplayTexts = []) {
    try {
        const model = config.getFastModel();
        if (!model)
            return { ok: false, reason: 'no_fast_model' };
        ...
        const hasDisplayProjection = userDisplayTexts.some((displayText) => displayText !== undefined);
        const dialog = hasDisplayProjection
            ? userDisplayTexts.flatMap((displayText) => displayText ? [{ role: 'user', parts: [{ text: displayText }] }] : [])
            : filterToDialog(fullHistory);
        const recentHistory = takeRecentDialog(dialog, RECENT_MESSAGE_WINDOW);
        if (recentHistory.length === 0) {
            return { ok: false, reason: 'empty_history' };
        }
        const conversationText = flattenToTail(recentHistory, MAX_CONVERSATION_CHARS);
        ...
        debugLogger.warn(`Session title generation failed: ${err instanceof Error ? err.message : String(err)}`);
        if (abortSignal.aborted)
            return { ok: false, reason: 'aborted' };
 ...
 */
function filterToDialog(history) {
    const out = [];
 ...
 * user message.
 */
function takeRecentDialog(history, windowSize) {
 ...
 */
function flattenToTail(history, maxChars) {
    const lines = [];
