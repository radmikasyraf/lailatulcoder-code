...
import { layoutRowForEvent, measureElementPosition, } from '../utils/measure-element-position.js';
// Mock child components
vi.mock('./messages/ToolGroupMessage.js', () => ({
    ToolGroupMessage: vi.fn(() => _jsx("div", {})),
}));
vi.mock('./TerminalImage.js', () => ({
    TerminalImage: ({ image }) => (_jsxs(Text, { children: ["MockTerminalImage:", image.mimeType] })),
}));
...
describe('<HistoryItemDisplay />', () => {
    ...
    it('renders assistant replies with a leading spacer row', () => {
        ...
        const { lastFrame } = renderWithProviders(_jsx(HistoryItemDisplay, { item: item, terminalWidth: 100, isPending: false }));
        const output = lastFrame() ?? '';
        expect(output.startsWith('\n')).toBe(true);
        expect(output).toContain('◆\uFE0E Hello');
    });
    it.each(['gemini', 'gemini_content'])('passes images from a %s history item to the assistant renderer', (type) => {
        const item = {
            id: 1,
            type,
            text: '',
    ...
    it('renders v2 goal_state history items through the lifecycle card', () => {
        ...
        const { lastFrame } = renderWithProviders(_jsx(HistoryItemDisplay, { item: item, terminalWidth: 80, isPending: false }));
        const output = lastFrame();
    ...
    it('renders SkillStatsDisplay for "skill_stats" type', () => {
        ...
        const { lastFrame } = renderWithProviders(_jsx(SessionStatsProvider, { children: _jsx(HistoryItemDisplay, { ...baseItem, item: item }) }));
        expect(lastFrame()).toContain('No skill calls have been made in this session.');
    });
    it('renders SessionSummaryDisplay for "quit" type', () => {
        ...
        const { lastFrame } = renderWithProviders(_jsx(ConfigContext.Provider, { value: mockConfig, children: _jsx(SessionStatsProvider, { children: _jsx(HistoryItemDisplay, { ...baseItem, item: item }) }) }));
    ...
    it('renders tool_use_summary as a dim badge line in full mode', () => {
        ...
        const { lastFrame } = renderWithProviders(_jsx(HistoryItemDisplay, { ...baseItem, item: item, isPending: false, terminalWidth: 80 }));
    ...
    it('renders committed thinking expanded when ThoughtExpandedProvider is true', () => {
        ...
        const { lastFrame } = renderWithProviders(_jsx(ThoughtExpandedProvider, { value: {
                allExpanded: true,
                expandedHeadIds: new Set(),
                toggle: () => { },
            }, children: _jsx(HistoryItemDisplay, { item: item, terminalWidth: 100, isPending: false }) }));
        const output = lastFrame() ?? '';
        expect(output).toContain('Thought for');
    ...
    it('fullDetail forces a committed thought expanded even when context/prop are collapsed', () => {
        ...
        const { lastFrame } = renderWithProviders(_jsx(HistoryItemDisplay, { item: item, terminalWidth: 100, isPending: false, fullDetail: true }));
        const output = lastFrame() ?? '';
        expect(output).toContain('Thought for');
        expect(output).toContain('Inspecting the repository');
    });
    it('forwards fullDetail to ToolGroupMessage for tool_group items', () => {
        ...
        renderWithProviders(_jsx(HistoryItemDisplay, { item: item, terminalWidth: 80, isPending: false, fullDetail: true }));
        const passedProps = vi.mocked(ToolGroupMessage).mock.calls[0][0];
    ...
    describe('showTimestamps', () => {
        ...
        const makeTimestampSettings = () => new LoadedSettings({ path: '', settings: {}, originalSettings: {} }, { path: '', settings: {}, originalSettings: {} }, {
            path: '',
        ...
        it('does not render timestamp when showTimestamps is disabled', () => {
            const { lastFrame } = renderWithProviders(_jsx(HistoryItemDisplay, { ...baseItem, item: timestampItem, isPending: false }));
            expect(lastFrame()).not.toMatch(/\[\d{2}:\d{2}:\d{2}\]/);
        });
        it('renders [HH:MM:SS] timestamp when showTimestamps is enabled', () => {
            const { lastFrame } = renderWithProviders(_jsx(HistoryItemDisplay, { ...baseItem, item: timestampItem, isPending: false }), { settings: makeTimestampSettings() });
            expect(lastFrame()).toMatch(/\[\d{2}:\d{2}:\d{2}\]/);
        });
        it('renders timestamp even when isPending is true (streaming)', () => {
            const { lastFrame } = renderWithProviders(_jsx(HistoryItemDisplay, { ...baseItem, item: timestampItem, isPending: true }), { settings: makeTimestampSettings() });
            expect(lastFrame()).toMatch(/\[\d{2}:\d{2}:\d{2}\]/);
        });
        it('does not render timestamp when timestamp field is missing', () => {
            ...
            const { lastFrame } = renderWithProviders(_jsx(HistoryItemDisplay, { ...baseItem, item: noTimestampItem, isPending: false }), { settings: makeTimestampSettings() });
            expect(lastFrame()).not.toMatch(/\[\d{2}:\d{2}:\d{2}\]/);
        });
    });
    describe('thinking-block mouse tracking is VP-gated (non-VP scroll fix)', () => {
        ...
        const settingsWithVp = (enabled) => new LoadedSettings({ path: '', settings: {}, originalSettings: {} }, { path: '', settings: {}, originalSettings: {} }, {
            path: '',
            settings: { ui: { useTerminalBuffer: enabled } },
            originalSettings: {},
        }, { path: '', settings: {}, originalSettings: {} }, true, new Set());
        const mouseEvent = (name, col) => ({
            name,
            col,
            row: 1,
            shift: false,
        ...
        const renderThoughtWithToggle = (toggle) => {
            ...
            renderWithProviders(_jsx(ThoughtExpandedProvider, { value: {
                    allExpanded: false,
                    expandedHeadIds: new Set(),
                    toggle,
                }, children: _jsx(HistoryItemDisplay, { item: thoughtItem, terminalWidth: 100, isPending: false }) }));
        ...
        it('subscribes the click handler without bypassVpGate (stays VP-gated)', () => {
            ...
            renderWithProviders(_jsx(HistoryItemDisplay, { item: thoughtItem, terminalWidth: 100, isPending: false }));
        ...
        it('shows the click hint when raw settings are unset but startup VP is enabled', () => {
            const { lastFrame } = renderWithProviders(_jsx(VirtualViewportContext.Provider, { value: true, children: _jsx(HistoryItemDisplay, { item: thoughtItem, terminalWidth: 100, isPending: false }) }));
        ...
        it('hides the click hint when startup VP overrides an enabled setting', () => {
            const { lastFrame } = renderWithProviders(_jsx(VirtualViewportContext.Provider, { value: false, children: _jsx(HistoryItemDisplay, { item: thoughtItem, terminalWidth: 100, isPending: false }) }), { settings: settingsWithVp(true) });
        ...
        it('hides the click hint when ui.mouseTracking is false despite VP being on', () => {
            const settingsNoMouse = new LoadedSettings({ path: '', settings: {}, originalSettings: {} }, { path: '', settings: {}, originalSettings: {} }, {
                path: '',
                settings: { ui: { useTerminalBuffer: true, mouseTracking: false } },
                originalSettings: {},
            }, { path: '', settings: {}, originalSettings: {} }, true, new Set());
            const { lastFrame } = renderWithProviders(_jsx(HistoryItemDisplay, { item: thoughtItem, terminalWidth: 100, isPending: false }), { settings: settingsNoMouse });
            expect(lastFrame()).not.toContain(`click or ${toggleKeyHint} to expand`);
        });
    });
});
