...
import { currentUser, ensureAuthenticated, gh, ghApiAll, setGhHost, } from './lib/gh.js';
import { gitOpt } from './lib/git.js';
import { worktreePath } from './lib/paths.js';
import { anyRootCarriesCriticalMarker, isBlockerBody, findRootId, } from './pr-context.js';
 ...
 * of the same data.
 */
export function buildThreadStatuses(comments, prAuthor, probe, me = '') {
    const byId = new Map();
    ...
    for (const root of comments) {
        ...
        threads.push({
            ...
            anchor: {
                ...
                outdated: !isFileLevel && (root.line === null || root.line === undefined),
                isFileLevel,
                commitId: anchorSha,
            },
            code: {
...
export function summarizeThreads(threads) {
    return {
        ...
        changedSinceComment: threads.filter((t) => t.code.changedSinceComment === true).length,
        changeUnknown: threads.filter((t) => t.code.changedSinceComment === 'unknown').length,
 ...
 */
export function makeGitProbe(worktree) {
    ...
    return (path, sinceSha) => {
        ...
        if (ancestor === undefined) {
            ancestor =
                gitOpt('-C', worktree, 'merge-base', '--is-ancestor', sinceSha, 'HEAD') !== null;
        ...
        if (!ancestor) {
            ...
            // unrelated files.
            const log = gitOpt('-C', worktree, 'log', '--format=%h', `${sinceSha}..HEAD`, '--', `:(top,literal)${path}`);
            if (log === null) {
                result = { changed: 'unknown', touchedBy: [], touchedByTotal: 0 };
            }
            else {
...
async function runCommentStatus(args) {
    ...
    try {
        ...
        const meta = JSON.parse(gh('pr', 'view', prNumber, '--repo', ownerRepo, '--json', 'author,headRefOid'));
        const prAuthor = meta.author?.login ?? '';
        const liveHeadBefore = meta.headRefOid ?? '';
        const comments = ghApiAll(`repos/${owner}/${repo}/pulls/${prNumber}/comments`);
        ...
        try {
            liveHeadAfter =
                JSON.parse(gh('pr', 'view', prNumber, '--repo', ownerRepo, '--json', 'headRefOid')).headRefOid ?? liveHeadBefore;
        }
        catch {
        ...
        const headMovedDuringFetch = liveHeadBefore !== '' &&
            liveHeadAfter !== '' &&
            liveHeadBefore !== liveHeadAfter;
        const worktreeStale = !worktreeMissing &&
            liveHeadAfter !== '' &&
        ...
        const threads = buildThreadStatuses(comments, prAuthor, makeGitProbe(worktree), me);
        if (worktreeStale) {
        ...
        writeStdoutLine(`Wrote comment-status report to ${out} (${comments.length} inline comments in ${summary.threads} threads: ` +
        ...
        if (worktreeMissing) {
            writeStdoutLine(`warning: no worktree at ${worktree} — run \`LailatulCoder review fetch-pr\` first. ` +
        ...
        if (headMovedDuringFetch) {
            writeStdoutLine(`warning: PR head moved during the comments fetch (${liveHeadBefore.slice(0, 8)} → ${liveHeadAfter.slice(0, 8)}) — ` +
                `anchor facts may be mixed across commits. Re-run after the push settles.`);
        }
        else if (worktreeStale) {
            writeStdoutLine(`warning: worktree HEAD ${(worktreeHeadSha ?? '').slice(0, 8)} != live PR head ${liveHeadAfter.slice(0, 8)} — ` +
        ...
        if (json.length > DEFAULT_TRUNCATE_TOOL_OUTPUT_THRESHOLD) {
            writeStdoutLine(`warning: ${out} is ${json.length} chars; read_file returns the first ` +
        ...
        writeStdoutLine(`warning: comment-status failed: ${msg}. It is an index, not the ` +
...
export const commentStatusCommand = {
...
//# sourceMappingURL=comment-status.js.map