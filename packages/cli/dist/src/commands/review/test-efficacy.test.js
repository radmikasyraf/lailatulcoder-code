...
import { replacementMutantsOf, splitDiffIntoHunks, selectHunkProbes, MAX_HUNK_PROBES, isWorkspaceMember, planTestEfficacy, classifyProbeRun, classifyMutantRun, safeRmWithin, selectMutants, parseAddedLines, hasCollocatedNewTest, collocatedProbe, collocatedNotGreenDetail, runnerFailureReason, heldForRedCollocatedTest, fitsAnotherMutantRun, probeCleanupFailureDetail, findVitestBin, MAX_MUTANTS, runControlMutant, runOneMutant, runOneHunkProbe, committedSymlinkProbes, } from './test-efficacy.js';
...
describe('isWorkspaceMember', () => {
    it('places the integration-tests directory outside every workspace', () => {
        ...
        expect(isWorkspaceMember('integration-tests/interactive/model-toggle-hotkey.test.ts', GLOBS)).toBe(false);
    });
    it('places a package test inside one', () => {
        expect(isWorkspaceMember('packages/cli/src/config/keyBindings.test.ts', GLOBS)).toBe(true);
        expect(isWorkspaceMember('packages/channels/base/src/x.test.ts', GLOBS)).toBe(true);
    });
    it('honours a negated glob', () => {
        expect(isWorkspaceMember('packages/desktop/src/a.test.ts', GLOBS)).toBe(false);
    });
    it('honours workspace-glob ORDER — a positive after a negation re-includes', () => {
        ...
        expect(isWorkspaceMember('packages/desktop/src/a.test.ts', globs)).toBe(true);
        const reordered = ['packages/*', 'packages/desktop', '!packages/desktop'];
        expect(isWorkspaceMember('packages/desktop/src/a.test.ts', reordered)).toBe(false);
    });
...
describe('planTestEfficacy', () => {
    ...
    it('probes nothing on a source-only diff (no tests to run)', () => {
        ...
        const plan = planTestEfficacy([{ path: 'packages/cli/src/a.ts', kind: 'source' }], GLOBS);
    ...
    it('probes nothing on a test-only diff', () => {
        ...
        const plan = planTestEfficacy([{ path: 'packages/cli/src/a.test.ts', kind: 'test' }], GLOBS);
        expect(plan.probes).toEqual([]);
        expect(plan.revert).toEqual([]);
    });
});
describe('findVitestBin', () => {
    it('names the search root when vitest cannot be resolved', () => {
        ...
        const vitestNotInstalled = () => {
            const err = new Error("Cannot find module 'vitest/package.json'");
            err.code = 'MODULE_NOT_FOUND';
            throw err;
        };
        expect(() => findVitestBin(worktree, vitestNotInstalled)).toThrow(`vitest not found searching up from ${worktree}`);
    });
    ...
    it('keeps the real error when vitest is present but hides its package.json', () => {
        ...
        writeFileSync(join(vitestDir, 'package.json'), JSON.stringify({ name: 'vitest', exports: { '.': './index.js' } }));
...
describe('runControlMutant', () => {
    ...
    it('returns null — never injects through — a probe file that is a symlink', () => {
        ...
        try {
            ...
            expect(readFileSync(join(outside, 'shared.test.ts'), 'utf8')).toBe(original);
        }
        finally {
            rmSync(dir, { recursive: true, force: true });
...
describe('probe write sites refuse a symlinked target', () => {
    ...
    it('runOneMutant reports inconclusive and leaves the target untouched', () => {
        ...
        try {
            const r = runOneMutant(dir, { file: 'target.ts', line: 1, statement: 'line one' }, []);
    ...
    it('refuses a target reached through a SYMLINKED ANCESTOR too — a leaf check cannot see it', () => {
        ...
        try {
            ...
            const m = runOneMutant(dir, { file: 'src/lib.ts', line: 1, statement: 'line one' }, []);
            expect(m.verdict).toBe('inconclusive');
            expect(m.detail).toContain('symlink');
            const h = runOneHunkProbe(dir, {
                file: 'src/lib.ts',
                index: 0,
                header: '@@ -1,2 +1,2 @@',
                startLine: 1,
            ...
            expect(readFileSync(join(outside, 'src', 'lib.ts'), 'utf8')).toBe(original);
        }
        finally {
            rmSync(dir, { recursive: true, force: true });
            rmSync(outside, { recursive: true, force: true });
...
describe('probeCleanupFailureDetail', () => {
    ...
    it('keeps the exception reason — the rmSync error that explains the survival', () => {
        const got = probeCleanupFailureDetail('/w/wt-probe', new Error("EBUSY: resource busy, rmdir '/w/wt-probe'"), "fatal: '/w/wt-probe' is not a working tree\n");
    ...
    it("falls back to git's refusal when rmSync itself did not throw", () => {
        const got = probeCleanupFailureDetail('/w/wt-probe', undefined, "fatal: '/w/wt-probe' contains modified files\n");
        expect(got).toBe("could not remove probe worktree /w/wt-probe: fatal: '/w/wt-probe' contains modified files");
    });
    it('says only what it knows when neither had anything to say', () => {
        // No dangling ": " — the bare path is the honest message here.
        expect(probeCleanupFailureDetail('/w/wt-probe', undefined, '  \n')).toBe('could not remove probe worktree /w/wt-probe');
    });
});
describe('restoreProbeTreeTracked, through runOneMutant', () => {
    it('refuses to run when the tree carries NO .git at all', () => {
        ...
        try {
            ...
            const r = runOneMutant(dir, { file: 'a.ts', line: 1, statement: 'gone.clear();' }, ['a.test.ts']);
            expect(r.verdict).toBe('inconclusive');
...
describe('safeRmWithin', () => {
    ...
    it.skipIf(process.platform === 'win32')('treats a backslash in a POSIX name as a NAME, not as a separator', () => {
...
describe('classifyProbeRun', () => {
    ...
    const reasonOf = (r) => r.verdict === 'inconclusive' ? r.reason : undefined;
    ...
    it('tags a run that produced no parseable output as no-output', () => {
        const got = classifyProbeRun(1, 'boom', ['packages/lib/src/a.test.ts'], 'x');
    ...
    it('tags a file absent from the results as not-in-results', () => {
        ...
        // error.
        const got = classifyProbeRun(1, json({
            testResults: [
                {
    ...
    it('tags an all-skipped file as all-skipped', () => {
        const got = classifyProbeRun(0, json({
            testResults: [
                {
    ...
    it('keeps POSIX matching case-sensitive', () => {
        ...
        try {
            ...
            const got = only(classifyProbeRun(1, json({
                testResults: [
                    {
    ...
    it('is inconclusive on unparseable output, and says why', () => {
        const got = only(classifyProbeRun(1, 'ELIFECYCLE npm ERR!', ['a.test.ts'], 'ENOENT: vitest'));
        expect(got.verdict).toBe('inconclusive');
    ...
    it('does not call an all-skipped file INERT', () => {
        ...
        const got = only(classifyProbeRun(0, json({
            testResults: [
                {
                    name: '/w/a.test.ts',
                    assertionResults: [{ status: 'skipped' }, { status: 'skipped' }],
...
describe('selectMutants', () => {
    ...
    it('caps at MAX_MUTANTS, preferring files that also have new tests', () => {
        ...
        expect(got.slice(0, 5).map((c) => c.file)).toEqual(Array(5).fill('src/tested.ts'));
...
describe('hasCollocatedNewTest', () => {
    it('pairs file.ts with its collocated file.test.ts / file.spec.ts', () => {
        expect(hasCollocatedNewTest('packages/cli/src/x.ts', [
            'packages/cli/src/x.test.ts',
        ])).toBe(true);
        expect(hasCollocatedNewTest('packages/cli/src/x.ts', [
            'packages/cli/src/x.spec.ts',
        ])).toBe(true);
        expect(hasCollocatedNewTest('packages/cli/src/Comp.tsx', [
    ...
    it('does not pair across directories or by basename suffix', () => {
        ...
        expect(hasCollocatedNewTest('packages/cli/src/y.ts', [
...
describe('collocatedProbe', () => {
    it('returns the collocated test path when one is in the probe set', () => {
        expect(collocatedProbe('packages/cli/src/x.ts', [
            'packages/cli/src/other.test.ts',
            'packages/cli/src/x.test.ts',
        ])).toBe('packages/cli/src/x.test.ts');
        expect(collocatedProbe('packages/cli/src/x.ts', ['packages/cli/src/x.spec.ts'])).toBe('packages/cli/src/x.spec.ts');
    });
...
describe('runnerFailureReason', () => {
    ...
    it('calls a suite killed at the deadline one that did not survive', () => {
        const r = spawnSync(process.execPath, ['-e', 'setTimeout(() => {}, 5000)'], { timeout: 300, encoding: 'utf8' });
...
describe('collocatedNotGreenDetail', () => {
    const perFile = [
        { file: 'packages/cli/src/red.test.ts', verdict: 'gated' },
        {
    ...
    it('says the test was red when the baseline collected it and it failed', () => {
        ...
        const detail = collocatedNotGreenDetail('mutant', 'packages/cli/src/red.test.ts', perFile);
        expect(detail).toContain('was RED there');
        expect(detail).not.toContain('compile or import error');
    });
    it.each(['not-run', 'runner-died', 'control-failed'])('refuses to explain %s, which a baseline entry cannot carry', (reason) => {
        ...
        const detail = collocatedNotGreenDetail('mutant', 'packages/cli/src/x.test.ts', [
            {
    ...
    it('refuses to explain a probe the baseline reported GREEN', () => {
        ...
        const detail = collocatedNotGreenDetail('mutant', 'packages/cli/src/green.test.ts', [{ file: 'packages/cli/src/green.test.ts', verdict: 'inert' }]);
    ...
    it('says the baseline never reported a probe it has no entry for', () => {
        ...
        const detail = collocatedNotGreenDetail('mutant', 'packages/cli/src/absent.test.ts', perFile);
    ...
    ])('names %s as the reason rather than guessing one', (reason, phrase, alsoPhrase) => {
        const detail = collocatedNotGreenDetail('mutant', 'packages/cli/src/x.test.ts', [
            {
                file: 'packages/cli/src/x.test.ts',
                verdict: 'inconclusive',
    ...
    it('names the probe and what the passing probes cannot show, per kind', () => {
        expect(collocatedNotGreenDetail('mutant', 'packages/cli/src/red.test.ts', perFile)).toBe("this mutant's collocated test packages/cli/src/red.test.ts did not run green in the unmutated baseline — it was RED there, so the remaining probes passing cannot show the statement is uncovered");
        expect(collocatedNotGreenDetail('hunk', 'packages/cli/src/red.test.ts', perFile)).toContain('cannot show the hunk is uncovered');
    });
});
describe('heldForRedCollocatedTest — the one decision both loops make', () => {
    ...
    it('holds when the collocated test was not green, and explains why', () => {
        const detail = heldForRedCollocatedTest('mutant', 'packages/cli/src/x.ts', probes, ['packages/cli/src/ok.test.ts'], perFile);
        expect(detail).toContain('was RED there');
        expect(detail).toContain('packages/cli/src/x.test.ts');
    });
    it('does not hold when the collocated test was green', () => {
        expect(heldForRedCollocatedTest('hunk', 'packages/cli/src/ok.ts', probes, ['packages/cli/src/ok.test.ts'], perFile)).toBeUndefined();
    });
    it('does not hold when the file has no collocated probe at all', () => {
        ...
        expect(heldForRedCollocatedTest('mutant', 'packages/cli/src/untested.ts', probes, [], perFile)).toBeUndefined();
    });
});
describe('classifyMutantRun', () => {
    ...
    const perFile = (exit, json, probes) => classifyProbeRun(exit, JSON.stringify(json), probes);
    it('SURVIVED when every affected test still passes', () => {
        const got = classifyMutantRun(perFile(0, {
            testResults: [
    ...
    it('KILLED when any assertion fails — the deletion was caught', () => {
        const got = classifyMutantRun(perFile(1, {
            testResults: [
                { name: '/w/a.test.ts', assertionResults: [{ status: 'passed' }] },
    ...
    it('INCONCLUSIVE when the mutant breaks the compile, never killed', () => {
        ...
        const got = classifyMutantRun(perFile(1, { testResults: [] }, ['a.test.ts']));
...
describe('splitDiffIntoHunks', () => {
    ...
    it('returns nothing for a diff with no hunks (a binary file)', () => {
        expect(splitDiffIntoHunks('diff --git a/i.png b/i.png\nBinary files a/i.png and b/i.png differ\n')).toEqual([]);
...
describe('selectHunkProbes', () => {
    const diffOf = (...hunks) => [
        ...
        '--- a/f',
        '+++ b/f',
        ...hunks.map(([start, len]) => `@@ -${start},${len} +${start},${len} @@\n x`),
        '',
    ].join('\n');
    const file = (over = {}) => ({
        file: 'src/a.ts',
    ...
    it('COUNTS what the cap drops rather than losing it', () => {
        ...
        const many = Array.from({ length: MAX_HUNK_PROBES + 3 }, (_, i) => file({ file: `src/f${i}.ts`, diff: diffOf([1, 1]) }));
...
describe('selectMutants — replacement operators', () => {
    const fileOf = (content, addedLines) => ({
        file: 'src/m.ts',
        content,
        addedLines,
        hasNewTests: false,
    ...
    it('spends the cap on deletion mutants BEFORE replacement ones', () => {
        const { selected, skippedForCap } = selectMutants([fileOf('if (a !== b) go();\nstate.clear();\n', [1, 2])], 1);
        expect(selected).toHaveLength(1);
    ...
    it('caps replacements at their sub-cap and counts what it drops', () => {
        ...
        const many = Array.from({ length: 6 }, (_, i) => fileOf(`if (a${i} !== b${i}) go();\n`, [1])).map((f, i) => ({ ...f, file: `src/g${i}.ts` }));
...
describe('replacementMutantsOf', () => {
    const same = (line) => replacementMutantsOf(line, line.trim());
    ...
    it('drops a `+ UPPER_CONST` reserve term', () => {
        expect(same('  if (estimate + COMPACT_MAX_OUTPUT_TOKENS > window) {')).toEqual({
            operator: 'term-drop',
    ...
    it('yields NOTHING when the raw line and the code view disagree', () => {
        ...
        expect(replacementMutantsOf("  if (s === ')') { fire(); }", "if (s === '') { fire(); }")).toBeNull();
    });
...
//# sourceMappingURL=test-efficacy.test.js.map