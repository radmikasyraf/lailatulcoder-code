...
import { AgentStatus, isTerminalStatus, ApprovalMode, APPROVAL_MODES, } from '@lailatul-coder/lailatul-coder-core';
...
// ─── Component ──────────────────────────────────────────────
export const AgentComposer = ({ agentId }) => {
    const { agents, agentTabBarFocused, agentShellFocused, agentApprovalModes } = useAgentViewState();
    const { setAgentInputBufferText, setAgentTabBarFocused, setAgentApprovalMode, } = useAgentViewActions();
    ...
    const { status, streamingState, isInputActive, elapsedTime, lastPromptTokenCount, } = useAgentStreamingState(interactiveAgent);
    ...
    useKeypress((key) => {
        ...
        isActive: streamingState === StreamingState.Responding && !agentShellFocused,
    });
    // ── Shift+Tab to cycle this agent's approval mode ──
    const agentApprovalMode = agentApprovalModes.get(agentId) ?? ApprovalMode.DEFAULT;
    useKeypress((key) => {
        const isShiftTab = key.shift && key.name === 'tab' && !key.ctrl;
        const isWindowsTab = process.platform === 'win32' &&
            key.name === 'tab' &&
            !key.ctrl &&
            !key.meta;
    ...
    const statusLabel = useMemo(() => {
        switch (status) {
            ...
            case AgentStatus.FAILED:
                return {
                    text: t('Failed: {{error}}', {
                        error: interactiveAgent?.getError() ??
    ...
    const inputBorderColor = !isInputActive || agentTabBarFocused
        ? theme.border.default
        : (statusColor ?? theme.border.focused);
    const prefixNode = (_jsxs(Text, { color: statusColor ?? theme.text.accent, children: [approvalModePromptStyle.prefix, ' '] }));
    const prefixWidth = 2; // "> " or "* " = 2 chars
    return (_jsx(StreamingContext.Provider, { value: streamingState, children: _jsxs(Box, { flexDirection: "column", marginTop: 1, children: [_jsx(LoadingIndicator, { currentLoadingPhrase: streamingState === StreamingState.Responding
                        ? t('Thinking…')
                        : undefined, elapsedTime: elapsedTime }), statusLabel && (_jsx(Box, { marginLeft: 2, children: _jsx(Text, { color: statusLabel.color, children: statusLabel.text }) })), _jsx(QueuedMessageDisplay, { messageQueue: messageQueue }), _jsx(BaseTextInput, { buffer: buffer, onSubmit: handleSubmit, onKeypress: handleKeypress, showCursor: isInputActive && !agentTabBarFocused, placeholder: '  ' + t('Send a message to this agent'), prefix: prefixNode, prefixWidth: prefixWidth, borderColor: inputBorderColor, isActive: isInputActive && !agentShellFocused }), _jsx(AgentFooter, { approvalMode: agentApprovalMode, promptTokenCount: lastPromptTokenCount, contextWindowSize: config.getContentGeneratorConfig()?.contextWindowSize, terminalWidth: terminalWidth })] }) }));
};
//# sourceMappingURL=AgentComposer.js.map