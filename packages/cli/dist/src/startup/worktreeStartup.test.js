...
import { setupStartupWorktree, buildStartupWorktreeNotice, persistStartupWorktreeSidecar, } from './worktreeStartup.js';
import { readWorktreeSessionMarker, SessionService, Storage, writeRuntimeStatus, writeWorktreeSessionMarker, } from '@lailatul-coder/lailatul-coder-core';
...
describe('setupStartupWorktree', () => {
    ...
    it('creates a worktree with an auto-generated slug for bare --worktree', async () => {
        ...
        if (res.ok) {
            ...
            expect(res.context.worktreePath).toContain(path.join('.qwen', 'worktrees', res.context.slug));
    ...
    it('creates a worktree with an explicit slug', async () => {
        ...
        if (res.ok) {
            ...
            expect(res.context.worktreePath).toBe(path.join(tempRepo, '.qwen', 'worktrees', 'my-feature'));
        }
    });
    it('rejects invalid slug characters before any git operation', async () => {
        ...
        expect(res.ok).toBe(false);
        if (!res.ok) {
            expect(res.error.toLowerCase()).toMatch(/letters|hyphens|invalid|may only/);
        }
    ...
    it('rejects full GitHub PR URLs when origin remote is missing', async () => {
        tempRepo = await makeTempRepo();
        process.chdir(tempRepo);
        const res = await setupStartupWorktree('https://github.com/LailatulCoder/lailatul-coder/pull/4174');
        expect(res).not.toBeNull();
        expect(res.ok).toBe(false);
        if (!res.ok) {
    ...
    it('creates a pr-<N> worktree from FETCH_HEAD when fetch succeeds (local fake remote)', async () => {
        ...
        const upstream = await fs.mkdtemp(path.join(os.tmpdir(), 'qwen-wt-pr-upstream-'));
        ...
        try {
            ...
            if (res.ok) {
                ...
                expect(res.context.worktreePath).toBe(path.join(tempRepo, '.qwen', 'worktrees', 'pr-42'));
                // The PR file lives inside the worktree (proving FETCH_HEAD was
                // the base, not main).
                const prFile = await fs.readFile(path.join(res.context.worktreePath, 'pr-file.txt'), 'utf8');
                ...
                const expectedSha = (await exec('git', ['rev-parse', 'refs/pull/42/head'], {
                    cwd: upstreamResolved,
                })).stdout.trim();
                expect(res.context.originalHeadCommit).toBe(expectedSha);
                // Sanity: must NOT equal the parent repo's main HEAD.
                const parentHead = (await exec('git', ['rev-parse', 'HEAD'], { cwd: tempRepo })).stdout.trim();
    ...
    it('refuses to re-attach when an existing dir occupies the slot on a different branch', async () => {
        ...
        const slotPath = path.join(tempRepo, '.qwen', 'worktrees', 'plain-dir-conflict');
        ...
        if (!res.ok) {
            ...
            expect(res.error.toLowerCase()).toMatch(/already exists|registered git worktree|expected/);
        }
        // Unexpected file survived.
        const survived = await fs.readFile(path.join(slotPath, 'unexpected-content.txt'), 'utf8');
    ...
    it('refuses nested worktree creation from inside .qwen/worktrees/', async () => {
        ...
        expect(res.ok).toBe(false);
        if (!res.ok) {
            expect(res.error.toLowerCase()).toMatch(/nested|inside another worktree/);
        }
    });
});
describe('persistStartupWorktreeSidecar', () => {
    ...
    function makeConfig(targetDir, sessionId) {
    ...
    it('adopts a stale marker when re-attaching to an inactive owner', async () => {
        ...
        await writeRuntimeStatus(new Storage(setup.context.worktreePath).getRuntimeStatusPath('old-session'), {
            sessionId: 'old-session',
            workDir: setup.context.worktreePath,
            pid: 2147483647,
        });
        await persistStartupWorktreeSidecar(makeConfig(setup.context.worktreePath, 'new-session'), { ...setup.context, wasReattached: true });
        expect(await readWorktreeSessionMarker(setup.context.worktreePath)).toBe('new-session');
    });
    it('keeps the marker when the owner runtime is still active', async () => {
        ...
        await persistStartupWorktreeSidecar(makeConfig(setup.context.worktreePath, 'new-session'), { ...setup.context, wasReattached: true });
        expect(await readWorktreeSessionMarker(setup.context.worktreePath)).toBe('old-session');
    });
    it('finds an active owner under a repo-subdir relative runtime dir', async () => {
        ...
        if (!setup?.ok)
            return;
        await writeWorktreeSessionMarker(setup.context.worktreePath, 'old-session');
        await writeRuntimeStatus(new Storage(packageDir).getRuntimeStatusPath('old-session'), {
            sessionId: 'old-session',
            workDir: packageDir,
            pid: process.pid,
        });
...
//# sourceMappingURL=worktreeStartup.test.js.map