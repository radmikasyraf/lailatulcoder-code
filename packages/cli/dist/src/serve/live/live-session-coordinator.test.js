...
import { LIVE_SESSION_SOURCE_PREFIX, LiveSessionCoordinator, } from './live-session-coordinator.js';
import { QwenRealtimeError, } from './qwen-realtime-session.js';
const readPersistedParentSessionId = vi.hoisted(() => vi.fn());
const buildRealtimeStartupContext = vi.hoisted(() => vi.fn(async () => '<startup_context>test context</startup_context>'));
...
function makeHarness(options = {}) {
    ...
    const bridge = {
        ...
        changeSessionCwd: vi.fn(async (sessionId, request) => ({
            sessionId,
        ...
        enqueueMidTurnMessage: vi.fn((_sessionId, _message, _context, _messageId, enqueueOptions) => {
        ...
        async *subscribeEvents(_sessionId, request) {
            const subscriber = { queue: [] };
        ...
        sendPrompt: vi.fn(async (sessionId, request, _signal, context) => {
            promptRequests.push({
                sessionId,
    ...
    const openRealtimeSession = vi.fn(async (_config, nextCallbacks) => {
    ...
    const coordinator = new LiveSessionCoordinator({
        ...
        openRealtimeSession: openRealtimeSession,
        materializeConversationDirectory: vi.fn(async (sessionId) => '/conversations/conversation-' + sessionId),
    ...
    const finishTurn = async (index, events) => {
        const turn = pendingTurns[index];
        if (!turn)
            throw new Error('No pending turn at index ' + index);
    ...
    return {
        ...
        get callbacks() {
            if (!callbacks)
                throw new Error('Realtime callbacks are unavailable.');
            return callbacks;
        },
        finishTurn,
    };
...
describe('LiveSessionCoordinator', () => {
    it('attaches Realtime to a persistent projectless Live session before direct conversation', async () => {
        ...
        expect(harness.bridge.updateSessionMetadata).toHaveBeenCalledWith('live-new', { displayName: 'Voice chat' });
        ...
        expect(harness.host.sendOutputAudio).toHaveBeenCalledWith(1, new Uint8Array([1, 0]));
    ...
    it('rolls back a fresh coordinator session and marks the catalog when the setup aborts before admission', async () => {
        ...
        expect(harness.host.failCall).toHaveBeenCalledWith(1, expect.stringContaining('Live call ended.'));
    ...
    it('releases completed input and delegation tracking during a long call', async () => {
        ...
        const active = harness.coordinator.active;
    ...
    it('ignores completion from an interrupted response after its replacement starts', async () => {
        ...
        const stateUpdatesBeforeStaleCompletion = harness.host.setCallState.mock.calls.length;
        harness.callbacks.onResponseDone?.({
            callEpoch: 1,
            responseId: 'response-first',
            inputItemId: 'input-first',
        ...
        expect(harness.host.setCallState).toHaveBeenCalledTimes(stateUpdatesBeforeStaleCompletion);
        harness.callbacks.onOutputAudioDelta?.({
            callEpoch: 1,
            responseId: 'response-second',
        ...
        expect(harness.host.setCallState).toHaveBeenLastCalledWith(1, 'listening');
        await expect(harness.coordinator.stop({ epoch: 1, callId: 'call-1' })).resolves.toBeUndefined();
    });
    ...
    it('uses the attached Live session and sends the exact delegation envelope', async () => {
        ...
        await harness.finishTurn(0, [{ type: 'message', text: '检查完成。' }]);
        await waitFor(() => expect(harness.realtime.completeHandoff).toHaveBeenCalledOnce());
    ...
    it('persists direct dialogue before admitting a following backend handoff', async () => {
        ...
        await waitFor(() => expect(harness.pendingTurns).toHaveLength(1));
        expect(harness.bridge.appendSessionLiveTranscript).toHaveBeenCalledWith('live-new', [
            { role: 'user', text: '先聊一下' },
            { role: 'assistant', text: '好的。' },
        ], 'qwen3.5-omni-plus-realtime');
        expect(vi.mocked(harness.bridge.appendSessionLiveTranscript).mock
            .invocationCallOrder[0]).toBeLessThan(vi.mocked(harness.bridge.sendPrompt).mock.invocationCallOrder[0] ?? 0);
        await harness.finishTurn(0, [{ type: 'message', text: '检查完成。' }]);
    });
    it('returns completed Agent messages at message boundaries before completing the handoff', async () => {
        ...
        await waitFor(() => expect(harness.realtime.sendHandoffUpdate).toHaveBeenCalledTimes(2));
        ...
        expect(harness.realtime.sendHandoffUpdate.mock.invocationCallOrder[1]).toBeLessThan(harness.realtime.completeHandoff.mock.invocationCallOrder[0] ?? 0);
    });
    it('completes the handoff when the backend turn ends without ordinary text', async () => {
        ...
        await waitFor(() => expect(harness.pendingTurns).toHaveLength(1));
        await harness.finishTurn(0, []);
        await waitFor(() => expect(harness.realtime.completeHandoff).toHaveBeenCalledWith({
            callEpoch: 1,
            callId: 'handoff-1',
        }));
    ...
    it('routes a second handoff into the active backend turn', async () => {
        ...
        await waitFor(() => expect(harness.bridge.enqueueMidTurnMessage).toHaveBeenCalledWith('live-new', '<realtime_delegation>\n  <input>只检查测试目录</input>\n  <transcript_delta>user: 只检查测试目录</transcript_delta>\n</realtime_delegation>', undefined, undefined, expect.objectContaining({
            queueOnly: true,
    ...
    it('starts the steering request as the next turn on the same session if the first turn just settled', async () => {
        ...
        expect(harness.bridge.enqueueMidTurnMessage).toHaveBeenCalledWith('live-new', expect.stringContaining('第二步'), undefined, undefined, expect.objectContaining({
            queueOnly: true,
            onSettledWithoutDrain: expect.any(Function),
        }));
        await harness.finishTurn(1, [{ type: 'message', text: '第二步完成。' }]);
        await waitFor(() => expect(harness.realtime.sendBackendContext).toHaveBeenCalledWith('第二步完成。'));
    });
    it('starts an accepted steering request as the next turn when it misses the final drain', async () => {
        ...
        await waitFor(() => expect(harness.bridge.enqueueMidTurnMessage).toHaveBeenCalledOnce());
    ...
    it('streams backend message deltas into silent context on the Codex cadence', async () => {
        ...
        await waitFor(() => expect(harness.realtime.sendHandoffUpdate).toHaveBeenCalledWith({
            callEpoch: 1,
            callId: 'handoff-1',
            output: '增量进度',
        }));
    ...
    it('routes backend speak_to_user to the owning Realtime conversation', async () => {
        ...
        expect(harness.realtime.speakToUser).toHaveBeenCalledWith('正在检查，请稍等。');
        await expect(harness.coordinator.speakToUser('another-session', '错误路由')).rejects.toThrow(/No active Live conversation owns/u);
    });
    ...
    it('tracks a task session only from a completed built-in create_sub_session result', async () => {
        ...
        await waitFor(() => expect(harness.host.setWorkers).toHaveBeenCalledWith(1, [
            {
                workspaceCwd: '/conversations',
                workspaceId: 'conversations-workspace',
                sessionId: 'worker-1',
    ...
    it('keeps Live usable while approved and denied tool permissions resolve', async () => {
        ...
        await waitFor(() => expect(harness.host.setPendingPermission).toHaveBeenLastCalledWith(1, true));
        harness.publish({
        ...
        await waitFor(() => expect(harness.host.setPendingPermission).toHaveBeenLastCalledWith(1, false));
        harness.publish({
            type: 'permission_request',
            data: {
        ...
        await Promise.resolve();
        expect(harness.host.setPendingPermission).toHaveBeenLastCalledWith(1, false);
        ...
        expect(harness.host.sendOutputAudio).toHaveBeenLastCalledWith(1, new Uint8Array([7, 0]));
    });
    ...
    it('reports a bounded stop failure when the provider final never arrives', async () => {
        ...
        await expect(harness.coordinator.stop({ epoch: 1, callId: 'call-1' })).resolves.toEqual({
    ...
    it('stops immediately while backend work continues and persists the final realtime tail', async () => {
        ...
        await waitFor(() => expect(harness.pendingTurns).toHaveLength(1));
        await expect(harness.coordinator.stop({ epoch: 1, callId: 'call-1' })).resolves.toBeUndefined();
        expect(harness.bridge.appendSessionLiveTranscript).toHaveBeenCalledWith('live-new', [{ role: 'user', text: '先停下语音' }], 'qwen3.5-omni-plus-realtime');
    ...
    it('reports provider configuration failures without retrying', async () => {
        const harness = makeHarness({
            providerError: new QwenRealtimeError('Invalid API key.', 'invalid_api_key', true, { kind: 'configuration' }),
        });
        await harness.coordinator.start({
            epoch: 1,
            callId: 'call-1',
        ...
        expect(harness.openRealtimeSession).toHaveBeenCalledOnce();
        expect(harness.host.failCall).toHaveBeenCalledWith(1, 'Live Voice failed to start: Invalid API key.');
...
//# sourceMappingURL=live-session-coordinator.test.js.map