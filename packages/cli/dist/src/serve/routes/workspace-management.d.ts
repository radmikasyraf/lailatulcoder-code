...
export interface WorkspaceManagementRouteDeps {
    ...
    pickWorkspaceDirectory?: (signal?: AbortSignal) => Promise<string | undefined>;
...
export interface WorkspaceRuntimeRemovalController {
    ...
    disposeRuntime(runtime: WorkspaceRuntime, reason?: 'daemon_shutdown' | 'workspace_removed' | 'trust_reconfigured'): Promise<void>;
}
export interface WorkspaceManagementHandle {
    sealAndWait(): Promise<void>;
    publishOwnedRuntime(canonicalCwd: string, provenance: Exclude<WorkspaceRuntimeProvenance, 'existing'>, validateBeforePublication: (runtime: WorkspaceRuntime) => void | Promise<void>): Promise<WorkspaceRuntime>;
}
