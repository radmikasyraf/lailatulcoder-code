...
import { FsError, MAX_READ_BYTES, MAX_TEXT_CURSOR_CHARS, canonicalizeWorkspace, isFsError, } from '../fs/index.js';
import { getWorkspaceRouteContext, resolveWorkspaceRuntimeFromParam, sendGenerationClosedError, setWorkspaceRouteContext, } from '../workspace-route-runtime.js';
/**
 ...
 */
export function applyReadHeaders(res) {
 ...
 */
export function sendFsError(res, err, route) {
    ...
    writeStderrLine(`LailatulCoder serve: ${route} unexpected error: ${err instanceof Error ? err.message : String(err)}`);
    res.status(500).json({
 ...
 */
function requireStringQuery(res, raw, name, route) {
    if (typeof raw !== 'string' || raw.length === 0) {
        ...
        writeStderrLine(`LailatulCoder serve: ${route} rejected request missing required ?${name}`);
        return null;
    }
    return raw;
}
...
async function handleGetFile(req, res, deps) {
...
async function handleGetFileBytes(req, res, deps) {
    ...
    if (queryPath === null)
        return;
    const offset = parseIntInRange(req.query['offset'], 0, Number.MAX_SAFE_INTEGER);
    if (offset === null) {
        applyReadHeaders(res);
        res.status(400).json({
            errorKind: 'parse_error',
...
async function handleGetStat(req, res, deps) {
...
async function handleGetList(req, res, deps) {
...
async function handleGetGlob(req, res, deps) {
    ...
    if (clientId === null)
        return;
    const pattern = requireStringQuery(res, req.query['pattern'], 'pattern', ROUTE);
    if (pattern === null)
        return;
    const includeIgnored = parseBoolFlag(req.query['includeIgnored']);
    const maxResults = parseIntInRange(req.query['maxResults'], 1, MAX_GLOB_MAX_RESULTS);
    if (maxResults === null) {
        applyReadHeaders(res);
        res.status(400).json({
            errorKind: 'parse_error',
    ...
    const cwdString = typeof cwdRaw === 'string' && cwdRaw.length > 0 ? cwdRaw : undefined;
    const fs = factory.forRequest({
        originatorClientId: clientId ?? undefined,
        route: ROUTE,
    });
    ...
    try {
        ...
        // `/stat`.
        const relMatches = trimmed.map((m) => workspaceRelative(req, m));
        applyReadHeaders(res);
        res.status(200).json({
            kind: 'glob',
            pattern,
 ...
 */
export function workspaceRelative(req, resolved) {
    const boundWorkspace = getWorkspaceRouteContext(req)?.runtime.workspaceCwd ??
        req.app.locals.boundWorkspace;
    if (!boundWorkspace) {
    ...
    if (isOutsideWorkspaceRelative(rel)) {
        try {
            ...
            throw new FsError('path_outside_workspace', 'path resolved outside workspace');
        }
    }
    if (isOutsideWorkspaceRelative(rel)) {
        throw new FsError('path_outside_workspace', 'path resolved outside workspace');
...
function isOutsideWorkspaceRelative(rel) {
    return (rel === '..' || rel.startsWith(`..${path.sep}`) || path.isAbsolute(rel));
}
export function registerWorkspaceFileReadRoutes(app, deps) {
...
export function registerWorkspaceQualifiedFileReadRoutes(app, deps) {
...
//# sourceMappingURL=workspace-file-read.js.map