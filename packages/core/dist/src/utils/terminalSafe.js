 ...
 * `@lailatul-coder/lailatul-coder-core` so the CLI sanitizer can re-use them when
 ...
 */
export function stripTerminalControlSequences(s) {
    return (s
 ...
 */
export function isBidiControlChar(code) {
 ...
 * same defense.
 */
export function stripDisplayControlChars(text) {
    let out = '';
 ...
 */
export function truncateNotificationLabel(label) {
    ...
    return (codePoints.slice(0, NOTIFICATION_LABEL_MAX_LENGTH - 3).join('') + '...');
}
//# sourceMappingURL=terminalSafe.js.map