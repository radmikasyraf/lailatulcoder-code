 ...
 */
export function isSyntheticHistoryItem(item) {
    switch (item.type) {
 ...
 */
export function itemsAfterAreOnlySynthetic(history, fromIndex) {
    for (let i = fromIndex + 1; i < history.length; i++) {
        if (!isSyntheticHistoryItem(history[i]))
            return false;
    }
    return true;
}
/** Index of the last `user` (real prompt) item, or -1. */
export function findLastUserItemIndex(history) {
...
export function realUserPromptTexts(history) {
    return history
 ...
 * HistoryItemDisplay).
 */
export function buildThoughtHeadIdMap(items) {
    const map = new Map();
    for (let i = 0; i < items.length; i++) {
        ...
        for (let j = i + 1; j < items.length; j++) {
            const next = items[j];
            if (next.type !== 'gemini_thought_content')
                break;
            map.set(next, headId);
        }
    }
