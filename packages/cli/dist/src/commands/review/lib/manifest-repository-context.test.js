...
import { manifestRepositoryContextProvider, MAX_GLOB_CANDIDATES, MAX_MATCH_WORK, } from './manifest-repository-context.js';
...
function provide(worktree, changedPaths, content) {
...
describe('manifest repository context provider', () => {
    ...
    it('returns null without a manifest or matching rule', () => {
        ...
        expect(provide(worktree, ['src/change.ts'], manifest({ rules: [{ paths: ['docs/**'] }] }))).toBeNull();
    });
    ...
    it('fails closed when the rule count exceeds the parse bound', () => {
        ...
        expect(() => provide(temp(), ['src/change.ts'], manifest({ rules: Array.from({ length: 129 }, () => ({ paths: [] })) }))).toThrow('rules is invalid');
    });
    it('fails closed when the total paths globs outgrow the matching bound', () => {
        ...
        expect(() => provide(worktree, ['src/change.ts'], manifest({ rules }))).toThrow('paths exceeds limit');
    });
    it('fails closed when merged fields or glob lists outgrow the wire bound', () => {
        ...
        expect(() => provide(worktree, ['src/change.ts'], manifest({
            rules: [
                {
                    paths: ['src/**'],
                    domains: Array.from({ length: 256 }, (_, index) => `domain-a-${String(index).padStart(3, '0')}`),
                },
                {
                    paths: ['src/**'],
                    domains: Array.from({ length: 256 }, (_, index) => `domain-b-${String(index).padStart(3, '0')}`),
                },
            ],
        ...
        expect(() => provide(worktree, ['src/change.ts'], manifest({
            rules: [
                {
                    paths: ['src/**'],
                    relatedPaths: Array.from({ length: 256 }, (_, index) => `p-a/${index}.ts`),
                },
                {
                    paths: ['src/**'],
                    relatedPaths: Array.from({ length: 256 }, (_, index) => `p-b/${index}.ts`),
                },
            ],
    ...
    it('rejects globs that enter dependency or build-output trees', () => {
        ...
        const worktree = temp();
        const content = (paths, relatedPaths) => manifest({ rules: [{ paths, relatedPaths }] });
        expect(() => provide(worktree, ['src/change.ts'], content(['src/**'], ['coverage/**']))).toThrow('relatedPaths enters a skipped directory');
        expect(() => provide(worktree, ['src/change.ts'], content(['src/**'], ['src/dist/**']))).toThrow('relatedPaths enters a skipped directory');
        expect(() => provide(worktree, ['src/change.ts'], content(['node_modules/vendor/**/*.ts'], ['src/**']))).toThrow('paths enters a skipped directory');
    });
    it('excludes related file and directory symlink escapes', () => {
        ...
        const context = provide(worktree, ['src/change.ts'], manifest({
    ...
    it.skipIf(process.platform === 'win32')('skips related files with POSIX-legal unsafe name bytes', () => {
    ...
    it.each([
        ...
        for (let index = 0; index < MAX_GLOB_CANDIDATES; index++) {
            writeFileSync(join(skipped, `${String(index).padStart(6, '0')}.js`), '');
        }
        expect(provide(worktree, ['src/change.ts'], manifest({
            rules: [{ paths: ['src/**'], relatedPaths: ['src/**'] }],
        }))?.relatedPaths).toEqual(['src/keep.ts']);
    }, 30_000);
    ...
    it('bounds candidate scanning even when matches are later excluded', () => {
        ...
        const source = join(worktree, 'src');
        mkdirSync(source);
        const changedPaths = Array.from({ length: MAX_GLOB_CANDIDATES + 1 }, (_, index) => {
            const name = `${String(index).padStart(6, '0')}.ts`;
            writeFileSync(join(source, name), '');
            return `src/${name}`;
        });
        expect(() => provide(worktree, changedPaths, manifest({
            rules: [{ paths: ['src/**'], relatedPaths: ['src/**'] }],
    ...
    it('expands nested related globs without double-counting the subsumed root', () => {
        ...
        }))?.relatedPaths).toEqual(['docs/a.ts', 'docs/api/b.ts']);
    });
    it('counts a subsumed subtree against the scan bound only once', () => {
        ...
        }))?.relatedPaths).toEqual(['docs/a.ts']);
    }, 30_000);
    ...
    it('deduplicates related patterns before applying the merge bound', () => {
        ...
        expect(provide(worktree, ['src/change.ts'], manifest({ rules }))?.relatedPaths).toHaveLength(9);
    });
    it('fails closed when cumulative matching work exceeds the budget', () => {
        ...
        for (let index = 0; index < 60; index++) {
            writeFileSync(join(directory, `leaf-${String(index).padStart(4, '0')}.ts`), '');
        }
        const stem = `x/${'a/'.repeat(198)}*/`;
        expect(() => provide(worktree, ['x/change.ts'], manifest({
            rules: [
                {
                    paths: ['x/**'],
                    relatedPaths: Array.from({ length: 128 }, (_, index) => `${stem}b${index}`),
                },
            ],
    ...
    it('expands static related paths as their own files', () => {
        ...
        expect(provide(worktree, ['src/main.ts'], manifest({
            rules: [{ paths: ['src/**'], relatedPaths: ['src/main.test.ts'] }],
        }))?.relatedPaths).toEqual(['src/main.test.ts']);
    });
    it('resolves a top-level static related entry as itself', () => {
        ...
        }))?.relatedPaths).toEqual(['package.json']);
    });
    it('uses case-sensitive UTF-16 matching for rules and related expansion', () => {
        ...
        expect(provide(worktree, ['src/X.TS'], manifest({ rules: [{ paths: ['src/*.ts'] }] }))).toBeNull();
        expect(provide(worktree, ['src/😀.ts'], manifest({ rules: [{ paths: ['src/?.ts'] }] }))).toBeNull();
        expect(provide(worktree, ['src/😀.ts'], manifest({ rules: [{ paths: ['src/??.ts'] }] }))?.label).toBe('Example repository');
    });
    it('matches multi-star segments in polynomial time', () => {
        ...
        expect(provide(worktree, ['src/change.ts'], content)?.relatedPaths).toEqual(['src/ababab']);
    }, 10_000);
    ...
    it('fails closed before parsing an oversized manifest', () => {
        ...
        // rejection.
        expect(() => provide(temp(), ['src/change.ts'], 'x'.repeat(MAX_IDENTITY_BYTES + 1))).toThrow('exceeds the size limit');
    });
    ...
    it('enforces the skip set case-insensitively', () => {
        ...
        // enforcement sites.
        const worktree = temp();
        expect(() => provide(worktree, ['src/change.ts'], manifest({ rules: [{ paths: ['NODE_MODULES/**'] }] }))).toThrow('paths enters a skipped directory');
        expect(() => provide(worktree, ['src/change.ts'], manifest({
    ...
    it('skips a static related entry that is itself a symlink', () => {
        ...
        }))?.relatedPaths).toEqual([]);
    });
    it.skipIf(process.platform === 'win32')('excludes files reached through a symlinked interior scan-root component', () => {
    ...
    it('still scans names the skip set deliberately excludes', () => {
        ...
        }))?.relatedPaths).toEqual(['build/gen.ts']);
    });
    ...
    it('keeps sibling scan roots when one string-prefixes the other', () => {
        ...
        }))?.relatedPaths).toEqual(['sr/a.ts', 'src/b.ts']);
    });
    it('fails closed when rule-filter matching work exceeds the budget', () => {
        ...
        const changed = Array.from({ length: 1024 }, (_, index) => `src/${'c'.repeat(240)}${String(index).padStart(4, '0')}.ts`);
        const paths = Array.from({ length: 128 }, (_, index) => `src/${'p'.repeat(240)}${String(index).padStart(3, '0')}/**`);
        expect(() => provide(temp(), changed, manifest({ rules: [{ paths }] }))).toThrow('paths matching work exceeds limit');
    }, 30_000);
    it('accepts matching work sitting exactly at the budget', () => {
        ...
        for (let index = 0; index < fileCount; index++) {
            writeFileSync(join(source, `${'f'.repeat(53)}${String(index).padStart(4, '0')}.ts`), '');
        }
        const patterns = Array.from({ length: 128 }, (_, index) => `src/**/${'m'.repeat(118)}${String(index).padStart(3, '0')}`);
        const pathLength = `src/${'f'.repeat(53)}0000.ts`.length;
        expect(fileCount * patterns.length * pathLength * patterns[0].length).toBe(MAX_MATCH_WORK);
        expect(provide(worktree, ['src/change.ts'], manifest({
    ...
    it('expands related globs whose static prefix contains ?', () => {
        ...
        }))?.relatedPaths).toEqual(['src/a1c/x.ts', 'src/a2c/y.ts']);
    });
    ...
    it.skipIf(isRoot)('fails closed when a scanned directory cannot be read', () => {
...
//# sourceMappingURL=manifest-repository-context.test.js.map