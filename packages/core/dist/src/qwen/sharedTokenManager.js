...
import { isErrorResponse, CredentialsClearRequiredError, } from './qwenOAuth2.js';
...
(function (TokenError) {
    TokenError["REFRESH_FAILED"] = "REFRESH_FAILED";
    TokenError["NO_REFRESH_TOKEN"] = "NO_REFRESH_TOKEN";
    TokenError["LOCK_TIMEOUT"] = "LOCK_TIMEOUT";
    TokenError["FILE_ACCESS_ERROR"] = "FILE_ACCESS_ERROR";
    TokenError["NETWORK_ERROR"] = "NETWORK_ERROR";
...
export class SharedTokenManager {
    ...
    async getValidCredentials(qwenClient, forceRefresh = false) {
        try {
            ...
            throw new TokenManagerError(TokenError.REFRESH_FAILED, `Failed to get valid credentials: ${error instanceof Error ? error.message : String(error)}`, error);
        }
    }
    /**
    ...
    withTimeout(promise, timeoutMs, operationType = 'Operation') {
        ...
        return Promise.race([
            ...
            new Promise((_, reject) => {
                timeoutId = setTimeout(() => reject(new Error(`${operationType} timed out after ${timeoutMs}ms`)), timeoutMs);
            }),
        ]);
    }
    /**
    ...
    async forceFileCheck(qwenClient) {
        try {
            ...
            if (error instanceof Error &&
                ...
                throw new TokenManagerError(TokenError.FILE_ACCESS_ERROR, `Failed to access credentials file during refresh: ${error.message}`, error);
            }
            // For missing files (ENOENT), just reset file modification time
            this.memoryCache.fileModTime = 0;
        }
    ...
    async reloadCredentialsFromFile(qwenClient) {
        try {
            ...
            if (error instanceof Error &&
                ...
                debugLogger.warn(`Failed to validate credentials file: ${error.message}`);
            }
    ...
    async performTokenRefresh(qwenClient, forceRefresh = false) {
        ...
        try {
            ...
            if (!currentCredentials.refresh_token) {
                throw new TokenManagerError(TokenError.NO_REFRESH_TOKEN, 'No refresh token available for token refresh');
            }
            ...
            if (lockAcquisitionTime > 5000) {
                ...
                debugLogger.warn(`Token refresh lock acquisition took ${lockAcquisitionTime}ms`);
            }
            ...
            if (totalOperationTime > 10000) {
                ...
                debugLogger.warn(`Token refresh operation took ${totalOperationTime}ms`);
            }
            ...
            if (!tokenData.access_token) {
                throw new TokenManagerError(TokenError.REFRESH_FAILED, 'Failed to refresh access token: no token returned');
            }
            ...
            if (error instanceof CredentialsClearRequiredError) {
                debugLogger.debug('SharedTokenManager: Clearing memory cache due to credentials clear requirement');
                ...
                this.refreshPromise = null;
                throw new TokenManagerError(TokenError.REFRESH_FAILED, error.message, error);
            }
            if (error instanceof TokenManagerError) {
                throw error;
            }
            ...
            if (error instanceof Error &&
                ...
                throw new TokenManagerError(TokenError.NETWORK_ERROR, `Network error during token refresh: ${error.message}`, error);
            }
            throw new TokenManagerError(TokenError.REFRESH_FAILED, `Unexpected error during token refresh: ${error instanceof Error ? error.message : String(error)}`, error);
        }
        finally {
    ...
    async saveCredentialsToFile(credentials) {
        ...
        try {
            await this.withTimeout(fs.mkdir(dirPath, { recursive: true, mode: 0o700 }), 5000, 'File operation');
        }
        catch (error) {
            throw new TokenManagerError(TokenError.FILE_ACCESS_ERROR, `Failed to create credentials directory: ${error instanceof Error ? error.message : String(error)}`, error);
        }
        ...
        try {
            ...
            throw new TokenManagerError(TokenError.FILE_ACCESS_ERROR, `Failed to write credentials file: ${error instanceof Error ? error.message : String(error)}`, error);
        }
    }
    /**
    ...
    async acquireLock(lockPath) {
                    ...
                    try {
                        ...
                        if (lockAge > LOCK_TIMEOUT_MS) {
                            ...
                            try {
                                ...
                                debugLogger.warn(`Removed stale lock file: ${lockPath} (age: ${lockAge}ms)`);
                                continue; // Retry lock acquisition immediately
                            }
                            catch (renameError) {
                                // Lock might have been removed by another process, continue trying
                                debugLogger.warn(`Failed to remove stale lock file ${lockPath}: ${renameError instanceof Error ? renameError.message : String(renameError)}`);
                        ...
                        debugLogger.warn(`Failed to stat lock file ${lockPath}: ${statError instanceof Error ? statError.message : String(statError)}`);
                    }
        ...
        throw new TokenManagerError(TokenError.LOCK_TIMEOUT, 'Failed to acquire file lock for token refresh: timeout exceeded');
    }
    /**
    ...
    async releaseLock(lockPath) {
        try {
            ...
            if (error.code !== 'ENOENT') {
                debugLogger.warn(`Failed to release lock file ${lockPath}: ${error instanceof Error ? error.message : String(error)}`);
            }
        }
    }
    /**
