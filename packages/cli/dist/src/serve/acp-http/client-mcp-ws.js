 ...
 */
import { ClientMcpRegistrar, } from '@lailatul-coder/lailatul-coder-core';
...
export class ClientMcpWsConnection {
    ...
    registrar;
    disposed = false;
    constructor(sendFrame, provider) {
    ...
    static isClientMcpFrameType(type) {
        return (type === CLIENT_MCP_FRAME_TYPES.register ||
            type === CLIENT_MCP_FRAME_TYPES.message ||
            type === CLIENT_MCP_FRAME_TYPES.unregister);
    }
    async handleRegister(server) {
        if (!isValidServerName(server)) {
            return {
                kind: 'error',
                code: 'invalid_server_name',
                message: 'server must be ≤256 chars, alphanumeric + underscore/hyphen, and not a reserved JS property name',
            };
        }
        ...
        if (!this.provider) {
            return {
                kind: 'error',
                code: 'not_wired',
                message: 'client_mcp_over_ws is advertised but no McpClientManager provider is wired into this daemon process',
            };
        }
        ...
        try {
            const { toolCount } = await this.provider.registerClientMcpServer(server, this.registrar.sendSdkMcpMessage);
    ...
    handleMessage(id, payload) {
        ...
        const resolved = this.registrar.resolveMessage(id, payload);
        return resolved
     ...
     */
    async dispose(reason = 'client MCP WS connection closed') {
        if (this.disposed)
            return;
        this.disposed = true;
        ...
        if (this.provider) {
            await Promise.allSettled(servers.map((server) => this.provider.unregisterClientMcpServer(server)));
        }
    }
}
//# sourceMappingURL=client-mcp-ws.js.map