...
import { addConfigToMatcherGroup, getAllConfigs, normalizeMatcher, } from './matcherGrouping.js';
function emptyHookInfo() {
    return {
...
describe('addConfigToMatcherGroup', () => {
    ...
    it('keeps non-matcher events in original handler order', () => {
        const info = emptyHookInfo();
        addConfigToMatcherGroup(info, 'A', undefined, {
            config: { type: HookType.Command, command: '/first.sh' },
            source: HooksConfigSource.User,
            sourceDisplay: 'User Settings',
            enabled: true,
        }, false);
        addConfigToMatcherGroup(info, 'B', undefined, {
        ...
        expect(info.matcherGroups).toHaveLength(1);
        expect(info.matcherGroups[0].matcher).toBe('*');
        expect(getAllConfigs(info).map((config) => config.config.type === HookType.Command ? config.config.command : '')).toEqual(['/first.sh', '/second.sh', '/third.sh']);
    });
});
//# sourceMappingURL=matcherGrouping.test.js.map