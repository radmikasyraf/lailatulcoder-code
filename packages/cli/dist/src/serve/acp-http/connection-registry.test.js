...
class FakeStream {
    ...
    async sendSerialized(payload, id) {
...
class PendingSessionStream {
    ...
    completeAll(result) {
        for (const settle of this.settles.splice(0))
            settle(result);
    }
    close() {
        this.isClosed = true;
...
describe('ConnectionRegistry.getSnapshot', () => {
    it('counts SSE streams and redacts full connection ids', () => {
        ...
        try {
            ...
            conn.attachSessionStream('sess-1', new FakeStream('sse'), new AbortController());
    ...
    it('non-resume attach flushes all pre-attach buffered frames WITH their bus id', () => {
        ...
        try {
            ...
            const binding = conn.attachSessionStream('sess-1', stream, new AbortController());
            ...
            expect(binding
                .lastFlushedEventId).toBeUndefined();
        }
        finally {
            registry.dispose();
        }
    ...
    it('keeps session frames buffered when only the connection SSE stream is live', async () => {
        ...
        const registry = new ConnectionRegistry(undefined, undefined, 2, 30 * 60_000, budget);
        try {
            const conn = registry.create(true);
            if (!conn)
                return;
    ...
    it('retires the exact session instead of silently evicting a pre-attach frame', async () => {
        ...
        try {
            ...
            await expect(conn.sendSession('sess-1', { chunk: 256 }, 256)).resolves.toBe('failed');
    ...
    it('discards every provisional receipt when overflow retires a connection', async () => {
        ...
        try {
            ...
            await expect(conn.sendConn({ reply: 256 }, { delivered, discarded })).resolves.toBe('failed');
    ...
    it('bounds retained serialized payload bytes before stream attachment', async () => {
        const registry = new ConnectionRegistry(undefined, undefined, 2, 30 * 60_000, undefined, 1024, 1024);
        try {
            const conn = registry.create(true);
            if (!conn)
                return;
            await expect(conn.sendConn({ value: 'x'.repeat(2048) })).resolves.toBe('failed');
            expect(conn.destroyed).toBe(true);
        }
        finally {
            registry.dispose();
    ...
    it('enforces the connection frame cap across distinct session streams', async () => {
        const registry = new ConnectionRegistry(undefined, undefined, 2, 30 * 60_000, undefined, 2);
        try {
            const conn = registry.create(true);
            if (!conn)
                return;
            ...
            await expect(conn.sendSession('sess-3', { overflow: true })).resolves.toBe('failed');
            expect(registry.get(conn.connectionId)).toBeUndefined();
        }
        finally {
            registry.dispose();
    ...
    it('accepts the exact byte limit and rejects the next byte', async () => {
        ...
        const registry = new ConnectionRegistry(undefined, undefined, 2, 30 * 60_000, undefined, 1024, exactBytes);
        try {
            const conn = registry.create(true);
            if (!conn)
                return;
    ...
    it.each([
        ['BigInt', { value: 1n }],
        ['undefined', undefined],
    ])('retires the exact buffered owner for an unserializable %s frame', async (_, frame) => {
        const registry = new ConnectionRegistry();
        try {
            const conn = registry.create(true);
            if (!conn)
    ...
    it('shares the daemon budget across workspace registries', async () => {
        ...
        try {
            ...
            expect(budget.snapshot().usedFrames).toBe(2);
            await expect(secondaryConn.sendConn({ source: 'overflow' })).resolves.toBe('failed');
    ...
    it('requeues a connection reply when its live stream closes before delivery', async () => {
        ...
        try {
            ...
            await vi.waitFor(() => expect(registry.getSnapshot().bufferedConnectionFrames).toBe(1));
            expect(budget.snapshot()).toMatchObject({
                usedFrames: 1,
    ...
    it('discards a live provisional receipt when teardown beats delivery', async () => {
        ...
        try {
            ...
            const delivered = vi.fn();
            const discarded = vi.fn();
            const send = conn.sendConn({ sessionId: 'provisional' }, { delivered, discarded });
            registry.delete(conn.connectionId);
    ...
    it('lets an ambiguous delivery settle before destroy discards receipts', async () => {
        ...
        try {
            ...
            const send = conn.sendConn({ sessionId: 'provisional' }, { delivered, discarded, outcomeUnknown });
            registry.delete(conn.connectionId);
            stream.complete('outcome_unknown');
            await expect(send).resolves.toBe('outcome_unknown');
    ...
    it('preserves an active WebSocket receipt when close makes delivery ambiguous', async () => {
        ...
        try {
            ...
            conn.attachConnStream(new WsStream(socket));
            const delivered = vi.fn();
            const discarded = vi.fn();
            const outcomeUnknown = vi.fn();
            const send = conn.sendConn({ sessionId: 'provisional' }, { delivered, discarded, outcomeUnknown });
    ...
    it('preserves an active WebSocket receipt when peer loss reaches the send callback first', async () => {
        ...
        try {
            ...
            if (!conn)
                return;
            const socket = new PendingWebSocket();
            conn.attachConnStream(new WsStream(socket, () => registry.delete(conn.connectionId)));
            const delivered = vi.fn();
            const discarded = vi.fn();
            const outcomeUnknown = vi.fn();
            const send = conn.sendConn({ sessionId: 'provisional' }, { delivered, discarded, outcomeUnknown });
    ...
    ])('contains a %s connection response serialization failure to one frame', async (mode, frame) => {
        const registry = new ConnectionRegistry();
        try {
            const conn = registry.create(true);
            if (!conn)
    ...
    it('settles delivery when a delivered receipt callback throws', async () => {
        ...
        try {
            ...
            if (!conn)
                return;
            const delivery = conn.sendConn({ reply: true }, {
                delivered: () => {
    ...
    it('detachSessionStream is a no-op for a stale stream after reclaim (identity guard)', () => {
        ...
        vi.useFakeTimers();
        const detached = [];
        const registry = new ConnectionRegistry(undefined, (sid) => detached.push(sid));
        try {
            const conn = registry.create(true);
            if (!conn)
                return;
    ...
    it('attachSessionStream within the grace window reclaims (cancels the pending teardown)', () => {
        ...
        try {
            ...
            conn.ownSession('sess-1');
            const s1 = new FakeStream('sse');
            const binding = conn.attachSessionStream('sess-1', s1, new AbortController());
            const promptAbort = new AbortController();
            binding.promptAbort = promptAbort;
            conn.detachSessionStream('sess-1', s1, 10_000);
    ...
    it('logs a stderr breadcrumb when a resume arms replay deferral, and stays silent on a fresh connect', () => {
        ...
        try {
            ...
            conn.attachSessionStream('sess-1', new FakeStream('sse'), new AbortController());
            expect(stderr.mock.calls.some((c) => String(c[0]).includes('replay deferral armed'))).toBe(false);
            // Resumptive attach (cursor present): arming the deferral logs once with
            // the session id and the cursor it resumed from.
            conn.attachSessionStream('sess-1', new FakeStream('sse'), new AbortController(), 5);
    ...
    it('holds a deferred reply until the pump delivers through its anchor (MsyIt) — a result produced during a slow replay lands after live tail content, not at replay_complete', () => {
        ...
        try {
            ...
            expect(conn.sessions.get('sess-1')?.replayPending).toBe(false);
            const hasReply = () => s.sent.some((f) => f.message.promptResult);
    ...
    it('finds pending permissions across connections', () => {
        ...
        try {
            ...
            expect(registry.findPendingPermission('perm-1', 'wrong-session')).toBeUndefined();
...
//# sourceMappingURL=connection-registry.test.js.map