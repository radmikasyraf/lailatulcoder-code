...
import { ToolErrorType, MCPServerStatus, getMCPServerStatus, OutputFormat, } from '@lailatul-coder/lailatul-coder-core';
import { normalizePartList, extractPartsFromUserMessage, computeUsageFromMetrics, buildSystemMessage, createToolProgressHandler, createAgentToolProgressHandler, functionResponsePartsToString, insertAfterFunctionResponses, } from './nonInteractiveHelpers.js';
// Mock dependencies
vi.mock('../nonInteractiveCliCommands.js', () => ({
    getAvailableCommands: vi
        .fn()
        .mockImplementation(async (_config, _signal, mode = 'acp') => {
        const allCommands = [
            {
                name: 'help',
        ...
        return allCommands.filter((cmd) => cmd.supportedModes.includes(mode));
    }),
}));
...
describe('createToolProgressHandler', () => {
    ...
    it('should call emitToolProgress with request and McpToolProgressData', () => {
        ...
        expect(mockAdapter.emitToolProgress).toHaveBeenCalledWith(mockRequest, progressData);
    });
    it('should not call emitToolProgress for non-McpToolProgressData output', () => {
        const mockAdapter = {
            emitToolProgress: vi.fn(),
        };
        const { handler } = createToolProgressHandler({ ...mockRequest, name: 'test_tool' }, mockAdapter);
    ...
    it('should forward shell heartbeats as tool progress', () => {
        ...
        expect(mockAdapter.emitToolProgress).toHaveBeenCalledWith(shellRequest, heartbeat);
    });
    it('should forward multiple progress updates', () => {
        ...
        expect(mockAdapter.emitToolProgress).toHaveBeenNthCalledWith(1, browserRequest, progress1);
        expect(mockAdapter.emitToolProgress).toHaveBeenNthCalledWith(2, browserRequest, progress2);
        expect(mockAdapter.emitToolProgress).toHaveBeenNthCalledWith(3, browserRequest, progress3);
    });
});
describe('createAgentToolProgressHandler', () => {
    ...
    it('should create handler that processes task tool calls', () => {
        const { handler } = createAgentToolProgressHandler(mockConfig, 'parent-tool-id', mockAdapter);
        const taskDisplay = {
            type: 'task_execution',
    ...
    it('should work with adapter that does not support subagent APIs', () => {
        ...
        const { handler } = createAgentToolProgressHandler(mockConfig, 'parent-tool-id', limitedAdapter);
        const taskDisplay = {
            type: 'task_execution',
            subagentName: 'test-agent',
            taskDescription: 'Test task',
