...
import { skillArgsPath, currentSessionId, SKILL_ARGS_DIR, } from '../../../services/skill-args-file.js';
 ...
 *
 * The scan is HARDENED — the store lives under `.lailatulcoder/tmp/`, which also
 ...
 */
function lookupRecordedHost(req) {
    const bindHost = (raw) => {
    ...
    try {
        for (const entry of readdirSync(SKILL_ARGS_DIR, { withFileTypes: true })) {
            // Session directories ONLY — `.lailatulcoder/tmp/` also holds review
            ...
            if (!/^s-/.test(entry.name))
                continue;
            candidatePaths.add(join(SKILL_ARGS_DIR, entry.name, 'LailatulCoder-skill-args-review.txt'));
        }
        candidatePaths.add(join(SKILL_ARGS_DIR, 'LailatulCoder-skill-args-review.txt'));
    }
    catch {
    ...
    for (const path of candidatePaths) {
        try {
            ...
            if (st.isSymbolicLink() || st.size > RECORDED_ARGS_MAX_BYTES)
                continue;
...
export function reviewWriteAuthorization(req) {
    ...
    const skillArgsOverride = currentSessionId() === '' && req.skillArgs ? req.skillArgs : undefined;
    ...
    if (!verdict.comment.effective) {
        ...
        const commentRequested = verdict.comment.requested || req.defaultComment === true;
        return {
            ok: false,
...
export function recordedSeverityFloor(opts) {
    ...
    try {
        if (opts.planPath) {
            const plan = JSON.parse(readFileSync(opts.planPath, 'utf8'));
    ...
    const path = currentSessionId() === '' && opts.skillArgs
        ? opts.skillArgs
        : defaultSkillArgsPath();
    try {
        ...
        if (t.type === 'pr-number') {
            ...
            if (!hostsEquivalent(t.host.toLowerCase(), host))
                return undefined;
        }
        else {
            return undefined;
        }
        if (verdict.severityFloorSource === 'default')
            return undefined;
        return {
