...
const storeMutexes = new Map();
function getStoreMutex(storeDir) {
...
function normalizeRulePath(workspacePath) {
...
function validLegacyProjection(projection) {
    if (!projection ||
    ...
    return Object.entries(projection).every(([name, config]) => {
        ...
        return (!!config &&
            ...
            config.overrides.every((rule) => typeof rule === 'string'));
    });
}
function findLegacyRules(projection, name) {
    const normalizedName = name.toLowerCase();
    return (Object.entries(projection).find(([candidate]) => candidate.toLowerCase() === normalizedName)?.[1].overrides ?? []);
}
...
function setLegacyPathActivation(policy, scopePath, activation) {
    ...
    const nextRule = Override.fromInput(activation === 'disabled' ? `!${canonicalScope}` : canonicalScope, true);
    const rules = (policy.legacyPathRules ?? []).filter((rule) => {
        const existing = Override.fromFileRule(rule);
        return (!existing.conflictsWith(nextRule) &&
...
function clearWorkspaceActivation(policy, workspacePath, canonicalWorkspace = canonicalizeWorkspacePath(workspacePath)) {
    ...
    const legacyMatches = (policy.legacyPathRules ?? []).some((rule) => {
        ...
        return legacyCandidates.some((candidate) => override.matchesPath(candidate));
    });
    if (legacyMatches) {
...
function parseState(content, statePath) {
    ...
    try {
        ...
        throw new ExtensionStoreCorruptError(`Extension store state is corrupt at ${statePath}.`, { cause: error });
    }
    const candidate = value;
    const validPolicy = (extensionId, policy) => {
        ...
        return (typeof parsed.name === 'string' &&
            ...
            Object.values(parsed.workspaceOverrides).every((activation) => activation === 'enabled' ||
                activation === 'disabled' ||
                activation === 'inherit') &&
    ...
    if (!candidate ||
        ...
        !Object.entries(candidate.extensions).every(([id, policy]) => validPolicy(id, policy))) {
        throw new ExtensionStoreCorruptError(`Extension store state has an invalid schema at ${statePath}.`);
    }
    return value;
}
export class ExtensionStore {
    ...
    agentPluginDataRoot(extensionId) {
        if (!/^[a-f0-9]{64}$/.test(extensionId)) {
            throw new Error(`Invalid extension id "${extensionId}".`);
        }
        return path.join(this.storeDir, 'plugin-data', 'agent-plugins', extensionId);
    }
    async ensureInitialized(extensions) {
        return await this.withLock(async () => await this.ensureInitializedUnlocked(extensions));
    }
    async readConsistent(readArtifacts) {
    ...
    async ensureInitializedUnlocked(extensions) {
        ...
        for (const identity of extensions) {
            ...
            if (existingIdentity &&
                ...
                throw new ExtensionConflictError(`Extension name "${identity.name}" conflicts with loaded extension "${existingIdentity.name}".`);
            }
        ...
        if (existing) {
            let changed = false;
            const renamedPolicyNames = new Set();
            const importUnmappedLegacy = existing.legacyProjectionHash === projectionHash(legacy);
            ...
            for (const identity of extensions) {
                ...
                if (directPolicy) {
                    if (directPolicy.name !== identity.name) {
                        const nameOwner = Object.entries(existing.extensions).find(([id, policy]) => id !== identity.id &&
                            policy.name.toLowerCase() === identity.name.toLowerCase());
                        if (nameOwner && !nameOwner[1].declarationOnly) {
                            throw new ExtensionConflictError(`Extension name "${identity.name}" conflicts with an installed extension.`);
                        }
                ...
                const staleEntry = Object.entries(existing.extensions).find(([id, policy]) => !loadedIds.has(id) &&
                    policy.name.toLowerCase() === identity.name.toLowerCase());
                if (staleEntry) {
            ...
            if (existing.legacyProjectionHash !== projectionHash(legacy)) {
                ...
                if (!legacyProjectionIsNewer) {
                    ...
                    for (const identity of extensions) {
                        assertIdentity(identity);
                        if (existing.extensions[identity.id])
                            continue;
                        const rules = findLegacyRules(existing.legacyProjectionRemainder ?? {}, identity.name);
                    ...
                    const identities = new Map(Object.entries(existing.extensions).map(([id, policy]) => [
                        id,
                        { id, name: policy.name },
                    ]));
                    for (const identity of extensions)
                    ...
                    for (const identity of identities.values()) {
                        ...
                        const { rules, activationChanged } = this.importLegacyProjection(findLegacyRules(legacy, identity.name), existingPolicy);
                        if (existingPolicy) {
                            const previousRules = existingPolicy.legacyPathRules ?? [];
                            const policyChanged = activationChanged ||
            ...
            if (renamedPolicyNames.size > 0) {
                remainderSource = Object.fromEntries(Object.entries(remainderSource).filter(([name]) => !renamedPolicyNames.has(name.toLowerCase())));
            }
            changed =
    ...
    async commitArtifact(input) {
        ...
        return await this.withLock(async () => {
            const snapshot = (await this.readSnapshotUnlocked()) ?? this.emptySnapshot();
            const transactionId = crypto.randomUUID();
            const transactionsDir = path.join(this.storeDir, 'transactions');
            const backupDirectory = path.join(this.storeDir, 'rollback', transactionId);
            const journalPath = path.join(transactionsDir, `${transactionId}.json`);
            const currentPolicy = snapshot.extensions[input.identity.id];
            const destinationDirectory = input.operation !== 'install' && currentPolicy?.artifactDirectory
            ...
            if (input.operation === 'install' && destinationExists) {
                throw new ExtensionConflictError(`Extension "${input.identity.name}" is installed.`);
            }
            if (input.operation === 'update' && !destinationExists) {
                throw new ExtensionConflictError(`Extension "${input.identity.name}" is not installed.`);
            }
            if (input.operation === 'install' && !input.initialActivation) {
                throw new Error('Install requires an initial activation.');
            }
            const nameConflict = Object.entries(snapshot.extensions).find(([extensionId, policy]) => extensionId !== input.identity.id &&
                policy.name.toLowerCase() === input.identity.name.toLowerCase());
            const currentPolicyIsAdoptable = input.operation === 'install' &&
                !!currentPolicy &&
            ...
            const nameConflictIsAdoptable = input.operation === 'install' &&
                !currentPolicy &&
                !!nameConflict &&
            ...
            if (input.operation !== 'uninstall' &&
                ...
                throw new ExtensionConflictError(`Extension name "${input.identity.name}" conflicts with an installed extension.`);
            }
            if (currentPolicy &&
                currentPolicy.name.toLowerCase() !== input.identity.name.toLowerCase()) {
                throw new ExtensionConflictError(`Extension id belongs to "${currentPolicy.name}", not "${input.identity.name}".`);
            }
            if (input.operation === 'uninstall' && !currentPolicy) {
                if (!destinationExists)
                    return snapshot;
                throw new ExtensionConflictError(`Extension "${input.identity.name}" has no matching policy.`);
            }
            ...
            if (input.operation === 'update' &&
                ...
                throw new ExtensionConflictError(`Extension "${input.identity.name}" changed while its update was being prepared.`);
            }
            const targetSnapshot = structuredClone(snapshot);
            if (input.operation === 'install') {
                const declarationEntry = currentPolicyIsAdoptable
                    ? [input.identity.id, currentPolicy]
                    : nameConflictIsAdoptable
                        ? nameConflict
                        : undefined;
                if (declarationEntry) {
                    ...
                    const rules = findLegacyRules(snapshot.legacyProjectionRemainder ?? {}, input.identity.name);
                    targetSnapshot.extensions[input.identity.id] = {
                        ...
                        workspaceOverrides: initial.scope === 'workspace'
                            ? {
                ...
                this.updateLegacyProjectionRemainder(targetSnapshot, targetSnapshot.legacyProjectionRemainder ?? {});
            }
                ...
                if (policy && policy.name !== input.identity.name) {
                    throw new ExtensionConflictError(`Extension update changed name from "${policy.name}" to "${input.identity.name}".`);
                }
                targetSnapshot.extensions[input.identity.id] = policy;
                delete targetSnapshot.extensions[input.identity.id]
            ...
            targetSnapshot.legacyProjectionHash = projectionHash(this.buildLegacyProjection(targetSnapshot));
            const journal = {
                version: 1,
                transactionId,
            ...
            try {
                ...
                if (input.operation !== 'uninstall') {
                    await renameWithRetry(input.stagingDirectory, destinationDirectory, 3, 50);
                }
                ...
                if (!stateCommitted) {
                    try {
                        ...
                        throw new AggregateError([error, rollbackError], 'Extension transaction failed and rollback recovery did not complete.', { cause: error });
                    }
                }
                throw error;
            }
    ...
    getActivation(snapshot, extensionId, extensionName, workspacePath) {
        ...
        for (const rule of policy.legacyPathRules ?? []) {
            const override = Override.fromFileRule(rule);
            if (!legacyCandidates.some((candidate) => override.matchesPath(candidate))) {
                continue;
            }
    ...
    async setDefaultActivation(identity, activation) {
        return await this.mutate(identity, (policy) => {
            policy.defaultActivation = activation;
        });
    }
    async setDefaultActivations(identities, activation) {
        const outcome = await this.mutateMany(identities, (policy) => {
            policy.defaultActivation = activation;
        });
        return outcome.snapshot;
    }
    async setActivationScope(identity, activation) {
        return await this.mutate(identity, (policy) => {
    ...
    async setWorkspaceActivation(identity, workspacePath, activation) {
        return await this.mutate(identity, (policy) => {
            policy.workspaceOverrides[canonicalizeWorkspacePath(workspacePath)] =
                activation;
        });
    }
    async setWorkspaceActivations(identities, workspacePath, activation) {
        const canonicalWorkspace = canonicalizeWorkspacePath(workspacePath);
        const outcome = await this.mutateMany(identities, (policy) => {
    ...
    async clearWorkspaceActivation(identity, workspacePath) {
        return await this.mutate(identity, (policy) => {
            clearWorkspaceActivation(policy, workspacePath);
        });
    }
    async clearWorkspaceActivations(identities, workspacePath) {
        const canonicalWorkspace = canonicalizeWorkspacePath(workspacePath);
        return await this.mutateMany(identities, (policy) => {
            clearWorkspaceActivation(policy, workspacePath, canonicalWorkspace);
        }, false);
    }
    async setLegacyPathActivation(identity, scopePath, activation) {
        return await this.mutate(identity, (policy) => {
            setLegacyPathActivation(policy, scopePath, activation);
        });
    }
    async mutate(identity, update) {
        assertIdentity(identity);
        return await this.withLock(async () => {
            const snapshot = (await this.readSnapshotUnlocked()) ?? this.emptySnapshot();
            const policy = snapshot.extensions[identity.id];
            if (!policy) {
                throw new ExtensionConflictError(`Extension "${identity.name}" is not installed.`);
            }
            if (policy.declarationOnly) {
                throw new ExtensionConflictError(`Extension "${identity.name}" is not installed.`);
            }
            if (policy.name !== identity.name) {
                throw new Error(`Extension id ${identity.id} belongs to "${policy.name}", not "${identity.name}".`);
            }
            update(policy);
    ...
    async mutateMany(identities, update, declareUnknown = true) {
        ...
        return await this.withLock(async () => {
            ...
            if (existing &&
                ...
                if (await this.legacyProjectionIsNewerThanState()) {
                    for (const policy of Object.values(snapshot.extensions)) {
                        const { rules } = this.importLegacyProjection(findLegacyRules(legacy, policy.name), policy);
                        if (rules.length > 0) {
                            policy.legacyPathRules = [...rules];
                        }
                        else {
            ...
            for (const identity of identities) {
                ...
                if (!policy) {
                    const existingByName = Object.values(snapshot.extensions).find((existing) => existing.name.toLowerCase() === identity.name.toLowerCase());
                    if (existingByName) {
                ...
                if (policy.name.toLowerCase() !== identity.name.toLowerCase()) {
                    throw new ExtensionConflictError(`Extension id ${identity.id} belongs to "${policy.name}", not "${identity.name}".`);
                }
    ...
    async extensionArtifactExists(policy) {
    ...
    async readLegacyProjection() {
        try {
            const projection = JSON.parse(await fsp.readFile(this.enablementPath, 'utf8'));
            if (!validLegacyProjection(projection)) {
                throw new ExtensionStoreCorruptError(`Extension enablement projection has an invalid schema at ${this.enablementPath}.`);
            }
            return projection;
        }
        catch (error) {
            ...
            if (error instanceof SyntaxError) {
                throw new ExtensionStoreCorruptError(`Extension enablement projection is corrupt at ${this.enablementPath}.`, { cause: error });
            }
            throw error;
        }
    }
    buildLegacyProjection(snapshot) {
        const projection = Object.create(null);
        const representedNames = new Set(Object.values(snapshot.extensions).map((policy) => policy.name.toLowerCase()));
        for (const [name, config] of Object.entries(snapshot.legacyProjectionRemainder ?? {})) {
            if (representedNames.has(name.toLowerCase()))
                continue;
        ...
        for (const policy of Object.values(snapshot.extensions)) {
            ...
            for (const [workspacePath, activation] of Object.entries(policy.workspaceOverrides)) {
                const effective = activation === 'inherit' ? policy.defaultActivation : activation;
                overrides.push(Override.fromInput(effective === 'disabled' ? `!${workspacePath}` : workspacePath, false).output());
            }
            if (overrides.length > 0)
                projection[policy.name] = { overrides };
        }
        return projection;
    }
    updateLegacyProjectionRemainder(snapshot, legacy) {
        const representedNames = new Set(Object.values(snapshot.extensions).map((policy) => policy.name.toLowerCase()));
        const remainder = Object.create(null);
    ...
    importLegacyProjection(incomingRules, policy) {
        if (!policy)
            return { rules: [...incomingRules], activationChanged: false };
        ...
        for (const [workspacePath, activation] of Object.entries(policy.workspaceOverrides)) {
            const effective = activation === 'inherit' ? policy.defaultActivation : activation;
            generatedRules.push({
                rule: Override.fromInput(effective === 'disabled' ? `!${workspacePath}` : workspacePath, false),
                workspacePath,
            });
        }
        ...
        const rules = incomingRules.filter((rule) => {
            ...
            const exactIndex = generatedRules.findIndex((generated, index) => !consumed.has(index) && generated.rule.isEqualTo(incoming));
            if (exactIndex >= 0) {
                consumed.add(exactIndex);
                return false;
            }
            const oppositeIndex = generatedRules.findIndex((generated, index) => !consumed.has(index) &&
    ...
    async writeSnapshotUnlocked(snapshot) {
        await this.prepareDirectories();
        const projection = this.buildLegacyProjection(snapshot);
        snapshot.legacyProjectionHash = projectionHash(projection);
        try {
            const previous = parseState(await fsp.readFile(this.statePath, 'utf8'), this.statePath);
    ...
    async writeLegacyProjectionUnlocked(snapshot, projection = this.buildLegacyProjection(snapshot)) {
    ...
    async legacyProjectionIsNewerThanState() {
        try {
            ...
            if (projection.mtimeNs === state.mtimeNs) {
                throw new ExtensionStoreCorruptError(`Extension store state and projection disagree at the same timestamp in ${this.storeDir}.`);
            }
    ...
    async prepareDirectories() {
        ...
        const privateDirectories = [
            this.storeDir,
            ...['staging', 'rollback', 'transactions'].map((directory) => path.join(this.storeDir, directory)),
        ];
        await Promise.all(privateDirectories.slice(1).map((directory) => fsp.mkdir(directory, {
            recursive: true,
            mode: 0o700,
        })));
        await Promise.all(privateDirectories.map((directory) => fsp.chmod(directory, 0o700)));
    ...
    assertArtifactDestination(destinationDirectory) {
        const extensionsRoot = path.resolve(this.extensionsDir);
        const destination = path.resolve(destinationDirectory);
        if (path.dirname(destination) !== extensionsRoot ||
    ...
    async recoverCorruptStateUnlocked() {
        ...
        for (const name of await fsp.readdir(transactionsDir)) {
            if (!name.endsWith('.json'))
                continue;
            const journal = await this.readRecoverableJournalUnlocked(path.join(transactionsDir, name));
            if (journal?.phase === 'state_committed') {
                candidates.push({
        ...
        let backupError;
        try {
            const previous = parseState(await fsp.readFile(this.previousStatePath, 'utf8'), this.previousStatePath);
            candidates.push({
                snapshot: previous,
        ...
        const latest = candidates.sort((left, right) => right.recoveryGeneration - left.recoveryGeneration)[0];
        if (latest) {
            const recovered = {
                ...latest.snapshot,
        ...
        throw new ExtensionStoreCorruptError(`Extension store state and recovery data are corrupt at ${this.storeDir}.`, { cause: backupError });
    }
    async readJournalUnlocked(journalPath) {
        try {
            ...
            if (journal.version !== 1 ||
                ...
                !['prepared', 'artifact_swapped', 'state_committed'].includes(journal.phase) ||
            ...
            journal.targetSnapshot = parseState(JSON.stringify(journal.targetSnapshot), journalPath);
            ...
            if (error instanceof ExtensionStoreCorruptError)
                throw error;
            throw new ExtensionStoreCorruptError(`Extension transaction journal is corrupt at ${journalPath}.`, { cause: error });
        }
    }
    async readRecoverableJournalUnlocked(journalPath) {
        try {
            ...
            try {
                ...
                throw new ExtensionStoreCorruptError(`Extension transaction journal is corrupt and could not be quarantined at ${journalPath}.`, {
                    cause: new AggregateError([error, quarantineError]),
                });
            }
            debugLogger.warn(`Quarantined corrupt extension transaction journal at ${quarantinePath}:`, error);
            return undefined;
        }
    }
    assertRecoveredJournalPaths(journal, journalPath) {
        const extensionsRoot = path.resolve(this.extensionsDir);
        const rollbackRoot = path.resolve(this.storeDir, 'rollback');
        const stagingRoot = path.resolve(this.storeDir, 'staging');
        const transactionsRoot = path.resolve(this.storeDir, 'transactions');
        if (path.dirname(path.resolve(journalPath)) !== transactionsRoot ||
            ...
            throw new UnsafeRecoveredJournalError(`Extension transaction ${journal.transactionId} contains unsafe paths.`);
        }
    }
    async rollbackJournal(journal) {
        ...
        if (hasBackup) {
            await renameWithRetry(journal.backupDirectory, journal.destinationDirectory, 3, 50);
        }
    ...
    async cleanupCommittedJournal(journal, journalPath) {
