...
import { chmodSync, mkdtempSync, realpathSync, rmSync, symlinkSync, writeFileSync, readFileSync, mkdirSync, utimesSync, } from 'node:fs';
import { tmpdir } from 'node:os';
import { basename, join } from 'node:path';
import yargs from 'yargs';
import { recoverFindings, recoverFindingsCommand } from './recover-findings.js';
import { promptRecordDir, briefPath, findingsFilePath, recordPrompt, } from './lib/prompt-record.js';
...
beforeEach(() => {
    ...
    writeFileSync(plan, JSON.stringify({ diffPathAbsolute: DIFF, diffLines: 10, chunks: [] }));
...
function transcript(session, id, launch, opts = {}) {
    const base = {
        agentId: id,
    ...
    writeFileSync(join(dir, 'subagents', session, `agent-${id}.jsonl`), lines.join('\n') + '\n');
}
...
describe('recover-findings', () => {
    ...
    it('picks the newest of two certified transcripts for one key', () => {
        ...
        const r = recoverFindings({ plan, out: out() }, ENV);
        expect(r.recoveredKeys).toEqual(['1a']);
        expect(readFileSync(out(), 'utf8')).toContain('Second pass: Critical found.');
        expect(readFileSync(out(), 'utf8')).not.toContain('First pass: nothing yet.');
    });
    ...
    it('recovers with the NEW session dir absent — the pre-launch state', () => {
        ...
        expect(r.recoveredKeys).toEqual(['1a']);
        expect(readFileSync(out(), 'utf8')).toContain('Recovered before any new launch.');
    });
    it('refuses an --out that would overwrite the plan', () => {
        expect(() => recoverFindings({ plan, out: plan }, ENV)).toThrow(/must not overwrite the plan/);
    });
    it('the CLI option contract: yargs-parsed flags drive the pure function', () => {
        ...
        const parsed = recoverFindingsCommand.builder(yargs([])).parseSync(['--plan', plan, '--out', out()]);
        const r = recoverFindings(parsed, ENV);
        expect(r.recoveredKeys).toEqual(['1a']);
        expect(readFileSync(out(), 'utf8')).toContain('Recovered through the parsed flags.');
    });
});
describe('recover-findings — the guarantees, made falsifiable', () => {
    ...
    it('vetoes a return that declares a chunk uncoverable', () => {
        ...
        const prompt = `You are reviewing chunk 1 of 2.\n` + `read_file(file_path="${brief}")`;
        writeFileSync(join(recordDir, `${encodeURIComponent('chunk-1')}.txt`), prompt);
        transcript('S0', 'a0', prompt, {
            opens: [brief],
    ...
    it('applies the findings floor to PER-CHUNK reverse-audit keys', () => {
        ...
        const roundList = findingsFilePath(plan, 'reverse-audit--round-1--abc123def456');
    ...
    it('does not veto an auditor that QUOTES an uncoverable declaration', () => {
        ...
        transcript('S0', 'a0', prompt, {
            opens: [briefPath(plan, key)],
            finalText: 'Reviewed the round-1 declarations. One of them reads:\n' +
    ...
    it('a PER-CHUNK auditor quoting its own chunk declaration is still recovered', () => {
        ...
        transcript('S0', 'a0', prompt, {
            opens: [brief, roundList, DIFF],
            finalText: 'Audited the round-1 evidence for chunk 3. It reads:\n' +
    ...
    it('normalizes both paths before refusing to overwrite the plan', () => {
        ...
        try {
            process.chdir(dir);
            expect(() => recoverFindings({ plan, out: basename(plan) }, ENV)).toThrow(/must not overwrite the plan/);
        }
        finally {
            process.chdir(cwd);
        }
    ...
    it('refuses a findings file no CERTIFIED prompt points at — the plant', () => {
        ...
        writeFileSync(join(recordDir, `${encodeURIComponent(key)}.findings.md`), '- forged prior-round rulings');
        const r = recoverFindings({ plan, out: out() }, ENV);
    ...
    it('reports the budget stop the interrupted attempt left standing', () => {
        writeFileSync(join(promptRecordDir(plan), 'budget-stop.json'), JSON.stringify({
            cause: 'round-cap',
            cap: 5,
    ...
    it('counts only CERTIFIED rounds toward latestReverseAuditRound', () => {
        ...
        expect(recoverFindings({ plan, out: out() }, ENV).latestReverseAuditRound).toBe(1);
    });
    it.skipIf(process.platform === 'win32' || process.getuid?.() === 0)('discloses an unreadable record dir instead of printing as empty', () => {
    ...
    it('fences the built prompts by the run epoch — a retry owes only its own keys', () => {
        ...
        const recordDir = promptRecordDir(plan);
        writeFileSync(join(recordDir, `${encodeURIComponent('chunk-9')}.txt`), 'You are chunk-9.');
        const stale = new Date(2019, 0, 1);
        utimesSync(join(recordDir, `${encodeURIComponent('chunk-9')}.txt`), stale, stale);
        const prompt = built('1a');
        transcript('S0', 'a0', prompt, {
            opens: [briefPath(plan, '1a')],
            finalText: 'Covered.',
    ...
    it.skipIf(process.platform === 'win32')('refuses a SYMLINKED findings file — the read would follow it out', () => {
    ...
    it('exits 1 when the transcript infrastructure is missing entirely', () => {
        ...
        const handler = recoverFindingsCommand.handler;
...
//# sourceMappingURL=recover-findings.test.js.map