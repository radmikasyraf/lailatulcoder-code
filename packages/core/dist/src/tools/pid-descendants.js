 ...
 */
export async function listDescendantPids(rootPid) {
    ...
    try {
        ...
        debugLogger.warn(`listDescendantPids(${rootPid}) failed: ${String(err instanceof Error ? err.message : err)}. Returning empty — orphans will be OS-reaped.`);
        return [];
    }
}
async function listDescendantPidsUnix(root) {
    let tree;
    try {
        tree = await snapshotProcessTreeUnix();
    }
    catch (err) {
        debugLogger.warn(`Unix snapshot via 'ps -A' failed (${err instanceof Error ? err.message : String(err)}); falling back to per-pid pgrep BFS`);
    }
    if (tree) {
...
async function snapshotProcessTreeUnix() {
    // `ps -A -o pid=,ppid=`
    ...
    if (parsedRows === 0) {
        ...
        throw new Error(`'ps -A -o pid=,ppid=' returned no parseable rows (stdout length=${stdout.length})`);
    }
    return childrenByPpid;
}
async function listDescendantPidsUnixPgrepFallback(root) {
    const all = [];
    const queue = [
        { pid: root, depth: 0 },
    ];
...
async function listDescendantPidsWin(root) {
    let tree;
    try {
        tree = await snapshotProcessTreeWin();
    }
    catch (err) {
        debugLogger.warn(`Windows snapshot via Get-CimInstance failed (${err instanceof Error ? err.message : String(err)}); falling back to per-pid filter BFS`);
    }
    if (tree) {
        return walkDescendants(tree, root);
    }
...
async function snapshotProcessTreeWin() {
    ...
    // locales / PS versions.
    const script = 'Get-CimInstance -ClassName Win32_Process ' +
        '| Select-Object ProcessId,ParentProcessId ' +
        '| ConvertTo-Csv -NoTypeInformation -Delimiter ","';
    const { stdout } = await execFileAsync('powershell.exe', ['-NoProfile', '-NonInteractive', '-Command', script], { timeout: QUERY_TIMEOUT_MS, maxBuffer: SNAPSHOT_MAXBUFFER_BYTES });
    const childrenByPpid = new Map();
    let parsedRows = 0;
    ...
    if (parsedRows === 0) {
        throw new Error(`Get-CimInstance snapshot returned no parseable rows (stdout length=${stdout.length})`);
    }
    return childrenByPpid;
}
async function listDescendantPidsWinPerPidFallback(root) {
    const all = [];
    const queue = [
        { pid: root, depth: 0 },
    ];
    while (queue.length && all.length < MAX_DESCENDANTS) {
        ...
        try {
            ...
            const { stdout } = await execFileAsync('powershell.exe', ['-NoProfile', '-NonInteractive', '-Command', script], { timeout: QUERY_TIMEOUT_MS });
            children = stdout
                .split(/\r?\n/)
                .map((s) => Number.parseInt(s.trim(), 10))
                .filter((n) => Number.isFinite(n) && n > 0);
            ...
            debugLogger.warn(`Windows pid descendant query failed for ${pid}: ${String(err instanceof Error ? err.message : err)}`);
            continue;
        }
        for (const child of children) {
            if (all.length >= MAX_DESCENDANTS)
 ...
 */
function walkDescendants(tree, root) {
    const all = [];
 ...
 */
export function sigtermPids(pids) {
    let signaled = 0;
    for (const pid of pids) {
        try {
            ...
            if (err &&
                ...
                'code' in err &&
                err.code !== 'ESRCH') {
                debugLogger.debug(`SIGTERM ${pid} failed: ${String(err instanceof Error ? err.message : err)}`);
            }
        }
    }
    return signaled;
}
//# sourceMappingURL=pid-descendants.js.map