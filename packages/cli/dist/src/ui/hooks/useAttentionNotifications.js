...
import { fireNotificationHook, NotificationType, } from '@lailatul-coder/lailatul-coder-core';
...
export const useAttentionNotifications = ({ isFocused, streamingState, elapsedTime, settings, config, terminal, pendingToolCalls, }) => {
    ...
    // behavior. See #6898.
    const notificationMode = settings?.merged?.general
        ...
        if (streamingState === StreamingState.Idle) {
            ...
            if (config && !idleNotificationSentRef.current) {
                ...
                if (hooksEnabled && messageBus) {
                    fireNotificationHook(messageBus, 'LailatulCoder Ai is waiting for your input', NotificationType.IdlePrompt, 'Waiting for input')
                        .then((hookResult) => {
