...
export function worktreeBranchForSlug(slug) {
 ...
 * `.lailatulcoder/worktrees/.gitignore`) so it cannot leak into commits.
 */
export const WORKTREE_SESSION_FILE = '.lailatulcoder-session';
/** Writes the owning session id into the worktree's session marker. */
export async function writeWorktreeSessionMarker(worktreePath, sessionId) {
    await fs.writeFile(path.join(worktreePath, WORKTREE_SESSION_FILE), sessionId, 'utf8');
 ...
 */
export async function readWorktreeSessionMarker(worktreePath) {
    ...
    try {
        ...
        if (error.code !== 'ENOENT') {
            debugLogger.warn(`readWorktreeSessionMarker: cannot read ${markerPath}: ${error}`);
        }
        return null;
    }
}
 ...
 * Changing this value affects the on-disk layout (`~/.lailatulcoder/<WORKTREES_DIR>/`)
...
export const AGENT_WORKTREE_SLUG_PATTERN = new RegExp(`^${AGENT_WORKTREE_PREFIX}-[0-9a-f]{${AGENT_WORKTREE_HEX_LENGTH}}$`);
/**
 * Generates a fresh ephemeral-agent slug. Centralised so the format
 * stays in lock-step with {@link AGENT_WORKTREE_SLUG_PATTERN}.
 */
export function generateAgentWorktreeSlug() {
...
export class GitWorktreeService {
    ...
    getGit() {
        this.gitPromise ??= loadSimpleGit().then(({ simpleGit }) => simpleGit(this.sourceRepoPath));
    ...
    static getWorktreesDir(sessionId, customBaseDir) {
        return path.join(GitWorktreeService.getSessionDir(sessionId, customBaseDir), WORKTREES_DIR);
    }
    /**
     ...
     * scatter `.lailatulcoder/worktrees/` under each subdirectory instead of
    ...
    async getRepoTopLevel() {
        try {
            ...
            debugLogger.warn(`getRepoTopLevel failed at ${this.sourceRepoPath}: ${error}`);
            return null;
        }
    }
    /**
    ...
    async getMainWorktreePath() {
        try {
            ...
            for (const line of lines.slice(1)) {
                ...
                if (attr === '')
                    break; // blank line ends the first record
                if (!isWorktreeListPorcelainAttribute(attr))
                    return null;
            }
            ...
            const ourCommonDir = await realpathOr(path.resolve(this.sourceRepoPath, ourRaw.trim()));
            const anchorCommonDir = await realpathOr(path.resolve(mainPath, anchorRaw.trim()));
            return ourCommonDir === anchorCommonDir ? mainPath : null;
        }
        catch (error) {
            debugLogger.warn(`getMainWorktreePath failed at ${this.sourceRepoPath}: ${error}`);
            return null;
        }
    }
    /**
    ...
    async createWorktree(sessionId, name, baseBranch) {
        try {
            const worktreesDir = GitWorktreeService.getWorktreesDir(sessionId, this.customBaseDir);
    ...
    async setupWorktrees(config) {
        ...
        const sessionDir = GitWorktreeService.getSessionDir(config.sessionId, this.customBaseDir);
        ...
        for (const name of config.worktreeNames) {
            const createResult = await this.createWorktree(config.sessionId, name, config.baseBranch);
    ...
    async cleanupSession(sessionId) {
        ...
        for (const worktree of worktrees) {
            ...
            if (removeResult.success) {
                ...
                result.errors.push(removeResult.error || `Failed to remove ${worktree.name}`);
                result.success = false;
            }
        }
        // Remove session directory
        const sessionDir = GitWorktreeService.getSessionDir(sessionId, this.customBaseDir);
        try {
            await fs.rm(sessionDir, { recursive: true, force: true });
        }
        catch (error) {
            result.errors.push(`Failed to remove session directory: ${error instanceof Error ? error.message : 'Unknown error'}`);
        }
    ...
    async getWorktreeDiff(worktreePath, baseBranch) {
        try {
            ...
            const base = (await this.resolveBaseline(worktreeGit)) ??
                baseBranch ??
                (await this.getCurrentBranch());
            return await this.withStagedChanges(worktreeGit, () => worktreeGit.diff(['--binary', '--cached', base]));
        }
        catch (error) {
    ...
    async applyWorktreeChanges(worktreePath, targetPath) {
        ...
        try {
            ...
            const patch = await this.withStagedChanges(worktreeGit, () => worktreeGit.diff(['--binary', '--cached', base]));
            if (!patch.trim()) {
                return { success: true };
            }
            const patchFile = path.join(this.getBaseDirForInstance(), `.worktree-apply-${Date.now()}-${Math.random().toString(16).slice(2)}.patch`);
    ...
    // `<projectRoot>/.lailatulcoder/worktrees/<slug>` rather than under the
    // session-scoped Arena baseDir.
    // ──────────────────────────────────────────────────────────────────────
    /**
     * Returns the directory holding all general-purpose worktrees for this
     * repo: `<projectRoot>/.lailatulcoder/worktrees`.
     */
    getUserWorktreesDir() {
        return path.join(this.sourceRepoPath, '.lailatulcoder', WORKTREES_DIR);
    }
    /**
     ...
     * and `<repoRoot>/.lailatulcoder/worktrees/foo` already exists on disk, we
     ...
     *    <repo>/.lailatulcoder/worktrees/foo`) git walks up to the outer `.git`
    ...
    async getRegisteredWorktreeBranch(worktreePath) {
        ...
        try {
            ...
            debugLogger.debug(`getRegisteredWorktreeBranch: probe at ${worktreePath} failed: ${error}`);
            return null;
        }
        if (probeCommonDir !== ourCommonDir) {
            debugLogger.debug(`getRegisteredWorktreeBranch: ${worktreePath} belongs to a different repo (common-dir=${probeCommonDir}, expected ${ourCommonDir})`);
            return null;
        }
    ...
    async isRegisteredLinkedWorktree(worktreePath) {
        ...
        try {
            ...
            const ourCommonDir = path.resolve(this.sourceRepoPath, (await (await this.getGit()).raw(['rev-parse', '--git-common-dir'])).trim());
            ...
            for (const name of entryNames) {
                ...
                let pointer;
                try {
                    pointer = (await fs.readFile(path.join(entry, 'gitdir'), 'utf8')).trim();
                }
                catch {
            ...
            debugLogger.debug(`isRegisteredLinkedWorktree: probe at ${worktreePath} failed: ${error}`);
            return false;
        }
    }
    /**
    ...
    async fetchPullRequestRef(prNumber, options) {
        ...
        try {
            ...
            await execFileAsync('git', ['fetch', '--end-of-options', 'origin', refspec], {
            ...
            if (err.signal === 'SIGTERM') {
                return {
                    success: false,
                    error: `Failed to fetch PR #${prNumber}: timed out after ${Math.round(timeoutMs / 1000)}s. ` +
            ...
            if (lower.includes('does not appear to be a git repository') ||
                ...
                return {
                    success: false,
                    error: `--worktree=#${prNumber} requires an "origin" remote that points at GitHub. ` +
                        `Add one with \`git remote add origin <url>\` and retry.`,
                };
            }
            if (lower.includes('no such ref') ||
                lower.includes("couldn't find remote ref") ||
                lower.includes("couldn't find remote ref pull/")) {
                return {
                    success: false,
                    error: `Failed to fetch PR #${prNumber}: the PR does not exist on origin, ` +
            ...
            debugLogger.warn(`fetchPullRequestRef: git fetch pull/${prNumber}/head failed: ${error}`);
            return {
                success: false,
     ...
     */
    static validateUserWorktreeSlug(slug) {
        ...
        if (slug.startsWith(`${AGENT_WORKTREE_PREFIX}-`)) {
            ...
            if (!AGENT_WORKTREE_SLUG_PATTERN.test(slug)) {
                return (`Worktree name must not start with "${AGENT_WORKTREE_PREFIX}-": that prefix ` +
    ...
    /**
     * Creates a general-purpose worktree at `<projectRoot>/.lailatulcoder/worktrees/<slug>`
    ...
    async createUserWorktree(slug, baseBranch, options) {
        ...
        if (validationError) {
            debugLogger.warn(`createUserWorktree: invalid slug ${slug}: ${validationError}`);
        ...
        try {
            ...
            // touches an existing user-managed `.lailatulcoder/.gitignore`.
            ...
            if (branchExists) {
                const error = `Cannot create worktree "${slug}": branch ${branchName} already exists. ` +
            ...
            await this.configureHooksPath(worktreePath).catch((error) => {
                debugLogger.warn(`createUserWorktree: failed to configure core.hooksPath for ${slug}: ${error}`);
            });
            ...
            if (symlinkPaths.length > 0) {
                await this.symlinkConfiguredDirectories(worktreePath, symlinkPaths).catch((error) => {
                    debugLogger.warn(`createUserWorktree: symlinkConfiguredDirectories failed for ${slug}: ${error}`);
                });
            }
            const worktree = {
                id: slug,
    ...
    async configureHooksPath(worktreePath) {
        ...
        try {
            ...
            if (!(isNodeError(error) && error.code === 'ENOENT')) {
                debugLogger.warn(`configureHooksPath: cannot stat ${huskyPath}: ${error}`);
            }
        }
        ...
        if (!hooksPath) {
            try {
                ...
                if (!(isNodeError(error) && error.code === 'ENOENT')) {
                    debugLogger.warn(`configureHooksPath: cannot resolve git common hooks dir: ${error}`);
                }
            }
        }
        if (!hooksPath)
        ...
        if (existing === '') {
            ...
            debugLogger.debug(`configureHooksPath: preserving existing core.hooksPath=${existing} ` +
    ...
    async symlinkConfiguredDirectories(worktreePath, configured) {
        ...
        try {
            ...
            // opt-in symlink step.
            debugLogger.warn(`symlinkConfiguredDirectories: cannot realpath sourceRepoPath "${this.sourceRepoPath}", skipping all entries`);
            return;
        }
        const gitDirAbs = path.join(repoRootAbs, '.git');
        const qwenDirAbs = path.join(repoRootAbs, '.lailatulcoder');
        ...
        for (const raw of configured) {
            if (typeof raw !== 'string' || raw.length === 0) {
                debugLogger.warn(`symlinkConfiguredDirectories: skipping non-string / empty entry: ${JSON.stringify(raw)}`);
                continue;
            }
            // Reject absolute paths and any traversal-prone form. Resolve first
            // to catch `./foo/../../etc` style escapes that look relative.
            if (path.isAbsolute(raw)) {
                debugLogger.warn(`symlinkConfiguredDirectories: refusing absolute path "${raw}"`);
                continue;
            }
            ...
            if (raw.split(/[\\/]/).includes('..')) {
                debugLogger.warn(`symlinkConfiguredDirectories: refusing path "${raw}" — contains '..' segment`);
                continue;
            }
            ...
            if (sourceAbs === repoRootAbs) {
                ...
                debugLogger.warn(`symlinkConfiguredDirectories: refusing empty / repo-root path "${raw}"`);
                continue;
            }
            if (!isWithinRoot(sourceAbs, repoRootAbs)) {
                debugLogger.warn(`symlinkConfiguredDirectories: refusing path "${raw}" — resolves outside repo root (${sourceAbs} vs ${repoRootAbs})`);
                continue;
            }
            ...
            // common-dir, and a symlink would shadow it). The whole `.lailatulcoder`
            // tree is also off-limits: linking `.lailatulcoder` (parent) would
            // recursively pull `.lailatulcoder/worktrees` into the new worktree,
            // recreating the loop; linking `.lailatulcoder/worktrees` directly
            // creates the same loop more obviously; and `.lailatulcoder/projects`
            // / `.lailatulcoder/tmp` are CLI metadata users have no legitimate
            ...
            if (isWithinRoot(sourceAbs, gitDirAbs)) {
                debugLogger.warn(`symlinkConfiguredDirectories: refusing git-internal path "${raw}"`);
                continue;
            }
            if (isWithinRoot(sourceAbs, qwenDirAbs)) {
                debugLogger.warn(`symlinkConfiguredDirectories: refusing path "${raw}" — ` +
                    `the .lailatulcoder tree is CLI-managed; symlinking any of it could ` +
            ...
            try {
                ...
                if (isNodeError(error) && error.code === 'ENOENT') {
                    debugLogger.debug(`symlinkConfiguredDirectories: source missing, skipping: ${sourceAbs}`);
                }
                else {
                    debugLogger.warn(`symlinkConfiguredDirectories: cannot stat ${sourceAbs}: ${error}`);
                }
                continue;
            }
            ...
            // into `.git`, an outside dir, or `.lailatulcoder`. Without this step a
            ...
            try {
                ...
                debugLogger.warn(`symlinkConfiguredDirectories: cannot realpath source "${sourceAbs}": ${error}`);
                continue;
            }
            if (!isWithinRoot(realSource, repoRootAbs)) {
                debugLogger.warn(`symlinkConfiguredDirectories: refusing path "${raw}" — real source ${realSource} escapes repo root ${repoRootAbs}`);
                continue;
            }
            if (isWithinRoot(realSource, gitDirAbs)) {
                debugLogger.warn(`symlinkConfiguredDirectories: refusing path "${raw}" — real source ${realSource} resolves inside .git`);
                continue;
            }
            if (isWithinRoot(realSource, qwenDirAbs)) {
                debugLogger.warn(`symlinkConfiguredDirectories: refusing path "${raw}" — real source ${realSource} resolves inside .lailatulcoder`);
                continue;
            }
            ...
            try {
                ...
                debugLogger.warn(`symlinkConfiguredDirectories: cannot mkdir parent of ${destAbs}: ${error}`);
                continue;
            }
            ...
            try {
                ...
                debugLogger.warn(`symlinkConfiguredDirectories: cannot realpath dest parent for "${raw}" (${path.dirname(destAbs)}): ${error}`);
                continue;
            }
            if (!isWithinRoot(realDestParent, realWorktreePath)) {
                debugLogger.warn(`symlinkConfiguredDirectories: refusing path "${raw}" — dest parent ${realDestParent} escapes worktree root ${realWorktreePath} (committed-symlink chain)`);
                continue;
            }
            ...
            try {
                ...
                debugLogger.debug(`symlinkConfiguredDirectories: linked ${destAbs} → ${realSource} (${symlinkType})`);
            }
            catch (error) {
                if (isNodeError(error) && error.code === 'EEXIST') {
                    debugLogger.debug(`symlinkConfiguredDirectories: destination exists, skipping: ${destAbs}`);
                }
                else {
                    debugLogger.warn(`symlinkConfiguredDirectories: failed to link ${destAbs} → ${realSource}: ${error}`);
                }
            }
        }
    }
    ...
    /**
     * Ensures `<projectRoot>/.lailatulcoder/.gitignore` ignores the worktrees
    ...
    async ensureWorktreesGitignored() {
        try {
            const qwenDir = path.join(this.sourceRepoPath, '.lailatulcoder');
            ...
            try {
                await fs.writeFile(gitignorePath, `# Auto-generated by lailatul-coder.\n${WORKTREES_DIR}/\n`, { encoding: 'utf8', flag: 'wx' });
            }
            catch (error) {
                if (isNodeError(error) && error.code === 'EEXIST') {
            ...
            debugLogger.warn(`ensureWorktreesGitignored failed (non-fatal): ${error}`);
        }
    }
    /**
    ...
    async removeUserWorktree(slug, options = {}) {
        ...
        try {
            ...
            debugLogger.warn(`removeUserWorktree: safe branch delete failed for ${branchName}: ${error}`);
        }
        if (options.forceDeleteBranch) {
            try {
                ...
                debugLogger.warn(`removeUserWorktree: force branch delete failed for ${branchName}: ${error}`);
            }
        }
    ...
    async hasUnmergedWorktreeCommits(slug) {
        ...
        try {
            ...
            debugLogger.warn(`hasUnmergedWorktreeCommits failed for slug ${slug}: ${error}`);
            return true;
        }
    }
    /**
    ...
    async countWorktreeChanges(worktreePath) {
        try {
            ...
            const tracked = status.staged.length +
...
//# sourceMappingURL=gitWorktreeService.js.map