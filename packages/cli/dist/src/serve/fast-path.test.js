...
import { consumeServeFastPathRejectedLoaderKeys, loadServeFastPathEnvironment, loadServeFastPathSettings, preResolveServeFastPathHomeEnvOverrides, resetServeFastPathHomeEnvBootstrapForTesting, } from './fast-path-settings.js';
import { resetLoaderKeyRejectionReportingForTesting } from '../config/shared-env-keys.js';
import { getGlobalQwenDirLite, SETTINGS_DIRECTORY_NAME, } from '../config/storage-paths-lite.js';
...
const originalSystemSettingsPath = process.env['QWEN_CODE_SYSTEM_SETTINGS_PATH'];
const originalSystemDefaultsPath = process.env['QWEN_CODE_SYSTEM_DEFAULTS_PATH'];
...
const originalNodeDisableCompileCache = process.env['NODE_DISABLE_COMPILE_CACHE'];
const originalPendingCompileCache = process.env['QWEN_CODE_PENDING_COMPILE_CACHE'];
...
function useTempQwenHome() {
    tempQwenHome = realpathSync(mkdtempSync(join(os.tmpdir(), 'qws-fast-path-home-')));
    ...
    process.env['QWEN_CODE_SYSTEM_SETTINGS_PATH'] = join(tempQwenHome, 'system-settings.json');
    process.env['QWEN_CODE_SYSTEM_DEFAULTS_PATH'] = join(tempQwenHome, 'system-defaults.json');
    return tempQwenHome;
}
...
describe('CLI entry import boundary', () => {
    ...
    it('keeps headless yolo warning helper free of runtime core imports', () => {
        ...
        expect(helperSource).not.toMatch(/import\s+(?!type\b)[^;]*from ['"]@lailatul-coder\/lailatul-coder-core['"]/);
    });
    ...
    it('keeps runQwenServe from statically loading the full server and ACP runtime', () => {
        ...
        expect(runServeSource).not.toMatch(/from ['"]@lailatul-coder\/acp-bridge\/bridge['"]/);
        expect(runServeSource).not.toMatch(/from ['"]@lailatul-coder\/acp-bridge\/spawnChannel['"]/);
    ...
    it('keeps request helpers from value-importing the ACP compatibility shim', () => {
        ...
        expect(requestHelpersSource).toContain("import type { AcpSessionBridge } from '@lailatul-coder/acp-bridge/bridgeTypes';");
        ...
        expect(requestHelpersSource).toMatch(/import \{[^}]*\bMAX_WORKSPACE_PATH_LENGTH\b[^}]*\} from '@lailatul-coder\/acp-bridge\/workspacePaths';/);
    ...
    it('keeps the runQwenServe static source graph free of ACP runtime modules', () => {
        const graph = collectStaticSourceGraph(resolve(cliPackageRoot, 'src/serve/run-qwen-serve.ts'));
...
describe('serve fast path argument parsing', () => {
    ...
    it('parses valid memory project scopes and falls back for invalid values', () => {
        expect(parseServeFastPathArgs(['serve', '--memory-project-scope', 'workspace'])).toMatchObject({
            kind: 'serve',
            options: { memoryProjectScope: 'workspace' },
        });
        expect(parseServeFastPathArgs(['serve', '--memory-project-scope=git-root'])).toMatchObject({
            kind: 'serve',
    ...
    it('parses --memory-pressure-mode and falls back on an unknown value', () => {
        ...
        expect(parseServeFastPathArgs(['serve', '--memory-pressure-mode', 'enforce'])).toEqual({ kind: 'fallback' });
    });
    it('parses --child-heap-mode and falls back on an unknown value', () => {
        ...
        expect(parseServeFastPathArgs(['serve', '--child-heap-mode', 'enforce'])).toEqual({ kind: 'fallback' });
    });
    ...
    it('prints a breadcrumb when settings bootstrap falls back to the full CLI', async () => {
        ...
        expect(stderrWrites.join('')).toContain('qwen serve: fast-path bootstrap failed, falling back to full startup:');
    });
    it.each([
        [
    ...
    it.each([
        ...
        expect(stderrWrites.join('')).not.toContain('must be an integer in [1024, 1048576]');
    });
...
describe('serve fast path environment bootstrap', () => {
    ...
    it('matches Storage.getGlobalQwenDir path resolution', () => {
        ...
        tempQwenHome = realpathSync(mkdtempSync(join(os.tmpdir(), 'qws-fast-path-storage-home-')));
    ...
    it('validates rate limit env after settings bootstrap enables rate limiting', async () => {
        ...
        mkdirSync(join(tempWorkspace, '.qwen'));
        writeFileSync(join(tempWorkspace, '.qwen', 'settings.json'), JSON.stringify({
            env: {
                QWEN_SERVE_RATE_LIMIT: '1',
        ...
        expect(stderrWrites.join('')).toContain('qwen serve: --rate-limit-prompt must be a positive integer.');
        expect(process.exit).toHaveBeenCalledWith(1);
    });
    it('exits when runQwenServe fails after settings bootstrap succeeds', async () => {
        useTempQwenHome();
        vi.spyOn(runQwenServeModule, 'runQwenServe').mockRejectedValue(new Error('listen boom'));
        const stderrWrites = [];
        vi.spyOn(process.stderr, 'write').mockImplementation((chunk) => {
            stderrWrites.push(String(chunk));
            return true;
    ...
    it('keeps headless yolo warning best-effort after listening', async () => {
        ...
        const qwenHome = useTempQwenHome();
        writeFileSync(join(qwenHome, 'settings.json'), JSON.stringify({ tools: { approvalMode: 'yolo', sandbox: false } }));
    ...
    it.each([
        ...
        const qwenHome = useTempQwenHome();
        writeFileSync(join(qwenHome, 'settings.json'), JSON.stringify(settingsJson));
        await expect(tryRunServeFastPath(['serve', '--port', '0', '--no-open', '--no-web'])).resolves.toBe(false);
    });
    it('loads QWEN_SERVER_TOKEN from the workspace .env before the daemon starts', async () => {
        ...
        mkdirSync(join(tempWorkspace, '.qwen'));
        writeFileSync(join(tempWorkspace, '.qwen', '.env'), 'QWEN_SERVER_TOKEN=from-workspace-env\n');
        process.chdir(tempWorkspace);
    ...
    it('preserves workspace .env compile cache over the pending default', async () => {
        ...
        useTempQwenHome();
        tempWorkspace = realpathSync(mkdtempSync(join(os.tmpdir(), 'qws-fast-path-compile-cache-')));
        mkdirSync(join(tempWorkspace, '.qwen'));
        writeFileSync(join(tempWorkspace, '.qwen', '.env'), 'NODE_COMPILE_CACHE=/tmp/operator-cache\n');
        await bootstrapServeFastPathEnvironment(tempWorkspace);
    ...
    it('loads .env from --workspace even when launched from another directory', async () => {
        ...
        mkdirSync(join(tempWorkspace, '.qwen'));
        writeFileSync(join(tempWorkspace, '.qwen', '.env'), 'QWEN_SERVER_TOKEN=from-explicit-workspace-env\n');
        process.chdir(tempLaunchCwd);
        await bootstrapServeFastPathEnvironment(tempWorkspace);
        expect(process.env['QWEN_SERVER_TOKEN']).toBe('from-explicit-workspace-env');
    });
    it('loads home .env after workspace .env for daemon boot-time keys', async () => {
        ...
        writeFileSync(join(tempWorkspace, '.env'), 'QWEN_SERVE_RATE_LIMIT_PROMPT=123\n');
    ...
    it('applies legacy excludedProjectEnvVars before loading workspace .env', async () => {
        delete process.env['QWEN_SERVER_TOKEN'];
        const qwenHome = useTempQwenHome();
        writeFileSync(join(qwenHome, 'settings.json'), JSON.stringify({ excludedProjectEnvVars: ['QWEN_SERVER_TOKEN'] }));
        tempWorkspace = realpathSync(mkdtempSync(join(os.tmpdir(), 'qws-fast-path-legacy-env-')));
        writeFileSync(join(tempWorkspace, '.env'), 'QWEN_SERVER_TOKEN=from-workspace-env\n');
        process.chdir(tempWorkspace);
        await bootstrapServeFastPathEnvironment(tempWorkspace);
    ...
    it('loads QWEN_SERVER_TOKEN from workspace settings.env without the full settings loader', async () => {
        ...
        process.chdir(tempWorkspace);
        await bootstrapServeFastPathEnvironment(tempWorkspace);
        expect(process.env['QWEN_SERVER_TOKEN']).toBe('from-workspace-settings-env');
    });
    it('pre-resolves home env overrides in the same order as the full loader', () => {
        ...
        tempQwenHome = realpathSync(mkdtempSync(join(os.tmpdir(), 'qws-fast-path-discovered-home-')));
        mkdirSync(join(tempLaunchCwd, '.qwen'), { recursive: true });
        writeFileSync(join(tempLaunchCwd, '.qwen', '.env'), `QWEN_HOME=${tempQwenHome}\n`);
        writeFileSync(join(tempLaunchCwd, '.env'), [
            'QWEN_RUNTIME_DIR=from-home-env',
            'QWEN_CODE_SERVE=1',
            'QWEN_CODE_DESKTOP=1',
        ].join('\n'));
        writeFileSync(join(tempQwenHome, '.env'), [
        ...
        expect(process.env['QWEN_CODE_MCP_APPROVALS_PATH']).toBe('from-discovered-home');
        expect(process.env['QWEN_CODE_TRUSTED_FOLDERS_PATH']).toBe('from-discovered-trust');
    ...
    it('still pre-resolves missing home-scoped keys when QWEN_HOME and runtime are already set', () => {
        ...
        writeFileSync(join(qwenHome, '.env'), 'QWEN_CODE_TRUSTED_FOLDERS_PATH=from-existing-home\n');
        preResolveServeFastPathHomeEnvOverrides();
        expect(process.env['QWEN_CODE_TRUSTED_FOLDERS_PATH']).toBe('from-existing-home');
    });
    it('applies legacy settings keys consumed by the serve fast path', () => {
        ...
        writeFileSync(join(qwenHome, 'settings.json'), JSON.stringify({
            approvalMode: 'yolo',
    ...
    it('matches the full settings loader for fields consumed before listen', async () => {
        ...
        writeFileSync(process.env['QWEN_CODE_SYSTEM_DEFAULTS_PATH'], JSON.stringify(versioned({
            env: {
        ...
        writeFileSync(join(qwenHome, 'settings.json'), JSON.stringify(versioned({
            env: {
        ...
        writeFileSync(join(tempWorkspace, '.qwen', 'settings.json'), JSON.stringify(versioned({
            env: {
        ...
        writeFileSync(process.env['QWEN_CODE_SYSTEM_SETTINGS_PATH'], JSON.stringify(versioned({
            env: {
    ...
    it('uses trusted-folders path from home .env before loading workspace env', async () => {
        ...
        writeFileSync(join(qwenHome, '.env'), `QWEN_CODE_TRUSTED_FOLDERS_PATH=${customTrustedFoldersPath}\n`);
        writeFileSync(join(qwenHome, 'settings.json'), JSON.stringify({ security: { folderTrust: { enabled: true } } }));
        writeFileSync(customTrustedFoldersPath, JSON.stringify({ [tempWorkspace]: TrustLevel.DO_NOT_TRUST }));
        writeFileSync(join(tempWorkspace, '.env'), 'QWEN_SERVER_TOKEN=from-untrusted-workspace-env\n');
        await bootstrapServeFastPathEnvironment(tempWorkspace);
        expect(process.env['QWEN_CODE_TRUSTED_FOLDERS_PATH']).toBe(customTrustedFoldersPath);
        expect(process.env['QWEN_SERVER_TOKEN']).toBeUndefined();
    });
    it('uses legacy folderTrust before loading workspace env', async () => {
        ...
        writeFileSync(join(qwenHome, 'settings.json'), JSON.stringify({ folderTrust: true }));
        process.env['QWEN_CODE_TRUSTED_FOLDERS_PATH'] = join(qwenHome, 'trustedFolders.json');
        writeFileSync(process.env['QWEN_CODE_TRUSTED_FOLDERS_PATH'], JSON.stringify({ [tempWorkspace]: TrustLevel.DO_NOT_TRUST }));
        writeFileSync(join(tempWorkspace, '.env'), 'QWEN_SERVER_TOKEN=from-untrusted-workspace-env\n');
        await bootstrapServeFastPathEnvironment(tempWorkspace);
        expect(process.env['QWEN_SERVER_TOKEN']).toBeUndefined();
    });
    it('caches trusted folders during a single fast-path bootstrap', () => {
        ...
        writeFileSync(join(qwenHome, 'settings.json'), JSON.stringify({ security: { folderTrust: { enabled: true } } }));
        process.env['QWEN_CODE_TRUSTED_FOLDERS_PATH'] = join(qwenHome, 'trustedFolders.json');
        writeFileSync(process.env['QWEN_CODE_TRUSTED_FOLDERS_PATH'], JSON.stringify({ [tempWorkspace]: TrustLevel.TRUST_FOLDER }));
    ...
    it('never applies loader-affecting keys from .env files or settings.env', () => {
        ...
        tempWorkspace = realpathSync(mkdtempSync(join(os.tmpdir(), 'qws-fast-path-loader-env-')));
        try {
            writeFileSync(join(tempWorkspace, '.env'), [
            ...
            writeFileSync(join(tempWorkspace, SETTINGS_DIRECTORY_NAME, '.env'), [
    ...
    it('never applies loader-affecting keys from user-level .env files either', () => {
        const trackedKeys = ['NODE_OPTIONS', 'npm_config_node_options'];
        const previous = {};
        for (const key of trackedKeys) {
            previous[key] = process.env[key];
            delete process.env[key];
        ...
        const qwenHome = useTempQwenHome();
        tempWorkspace = realpathSync(mkdtempSync(join(os.tmpdir(), 'qws-fast-path-loader-home-')));
        writeFileSync(join(qwenHome, '.env'), [
            'NODE_OPTIONS=--import file:///workspace-a/harness.mjs',
            'npm_config_node_options=--import file:///workspace-a/hook.mjs',
    ...
    it('never applies QWEN_CLI_ENTRY from a project .env on the fast path', () => {
        useTempQwenHome();
        const trackedKeys = ['QWEN_CLI_ENTRY', 'qwen_cli_entry'];
        const previous = {};
        for (const key of trackedKeys) {
            previous[key] = process.env[key];
            delete process.env[key];
        }
        tempWorkspace = realpathSync(mkdtempSync(join(os.tmpdir(), 'qws-fast-path-cli-entry-')));
        writeFileSync(join(tempWorkspace, '.env'), [
    ...
    it('never applies a case-variant hardcoded exclusion from settings.env on the fast path', () => {
        ...
        tempWorkspace = realpathSync(mkdtempSync(join(os.tmpdir(), 'qws-fast-path-settings-hardcoded-')));
        try {
            loadServeFastPathEnvironment({
                env: {
                    qwen_cli_entry: '/workspace-a/evil-entry-lower.js',
    ...
    it('warns when loader-affecting keys are rejected on the fast path', () => {
        ...
        useTempQwenHome();
        const trackedKeys = ['NODE_OPTIONS', 'LD_PRELOAD'];
        const previous = {};
        for (const key of trackedKeys) {
            previous[key] = process.env[key];
            delete process.env[key];
        }
        tempWorkspace = realpathSync(mkdtempSync(join(os.tmpdir(), 'qws-fast-path-loader-warn-')));
        writeFileSync(join(tempWorkspace, '.env'), ['NODE_OPTIONS=--max-old-space-size=8192', ''].join('\n'));
        const stderrWrites = [];
        ...
        try {
            loadServeFastPathEnvironment({ env: { LD_PRELOAD: '/workspace-a/hijack.so' } }, tempWorkspace);
        }
        finally {
    ...
    it('resets the rejected-loader-key stash after one consume', () => {
        useTempQwenHome();
        const trackedKeys = ['NODE_OPTIONS'];
        const previous = {};
        for (const key of trackedKeys) {
            previous[key] = process.env[key];
            delete process.env[key];
        }
        tempWorkspace = realpathSync(mkdtempSync(join(os.tmpdir(), 'qws-fast-path-loader-consume-')));
        writeFileSync(join(tempWorkspace, '.env'), 'NODE_OPTIONS=--max-old-space-size=8192\n');
        try {
            loadServeFastPathEnvironment({}, tempWorkspace);
            expect(consumeServeFastPathRejectedLoaderKeys()).toContain('NODE_OPTIONS');
    ...
    it('accumulates rejected loader keys across loads without duplicates', () => {
        ...
        const firstWorkspace = realpathSync(mkdtempSync(join(os.tmpdir(), 'qws-fast-path-loader-accum-1-')));
        const secondWorkspace = realpathSync(mkdtempSync(join(os.tmpdir(), 'qws-fast-path-loader-accum-2-')));
        writeFileSync(join(firstWorkspace, '.env'), 'NODE_OPTIONS=--max-old-space-size=8192\n');
        ...
        try {
            ...
            loadServeFastPathEnvironment({ env: { LD_PRELOAD: '/hijack.so', NODE_OPTIONS: '--inspect' } }, secondWorkspace);
    ...
    it('does not load env from an explicitly untrusted nested workspace', async () => {
        ...
        writeFileSync(join(qwenHome, 'settings.json'), JSON.stringify({ security: { folderTrust: { enabled: true } } }));
        process.env['QWEN_CODE_TRUSTED_FOLDERS_PATH'] = join(qwenHome, 'trustedFolders.json');
        writeFileSync(process.env['QWEN_CODE_TRUSTED_FOLDERS_PATH'], JSON.stringify({
    ...
    it('does not load env from a descendant of an explicitly untrusted workspace', async () => {
        delete process.env['QWEN_SERVER_TOKEN'];
        const qwenHome = useTempQwenHome();
        tempWorkspace = realpathSync(mkdtempSync(join(os.tmpdir(), 'qws-fast-path-trust-descendant-')));
        ...
        writeFileSync(join(childWorkspace, '.env'), 'QWEN_SERVER_TOKEN=from-untrusted-descendant-env\n');
    ...
    it('allows a trusted child rule to override an untrusted parent', async () => {
        delete process.env['QWEN_SERVER_TOKEN'];
        const qwenHome = useTempQwenHome();
        tempWorkspace = realpathSync(mkdtempSync(join(os.tmpdir(), 'qws-fast-path-trust-opt-in-')));
        ...
        writeFileSync(join(trustedWorkspace, '.env'), 'QWEN_SERVER_TOKEN=from-trusted-child-env\n');
        await bootstrapServeFastPathEnvironment(subDir);
    ...
    it('does not load a distrusted parent .env for a trusted child workspace', async () => {
        delete process.env['QWEN_SERVER_TOKEN'];
        const qwenHome = useTempQwenHome();
        tempWorkspace = realpathSync(mkdtempSync(join(os.tmpdir(), 'qws-fast-path-trusted-child-')));
        const childWorkspace = join(tempWorkspace, 'child');
        mkdirSync(childWorkspace);
        writeFileSync(join(tempWorkspace, '.env'), 'QWEN_SERVER_TOKEN=from-distrusted-parent-env\n');
        writeFileSync(join(qwenHome, 'settings.json'), JSON.stringify({ security: { folderTrust: { enabled: true } } }));
        process.env['QWEN_CODE_TRUSTED_FOLDERS_PATH'] = join(qwenHome, 'trustedFolders.json');
        writeFileSync(process.env['QWEN_CODE_TRUSTED_FOLDERS_PATH'], JSON.stringify({
            [tempWorkspace]: TrustLevel.DO_NOT_TRUST,
    ...
    it('expands home .env fallback placeholders in workspace settings.env', async () => {
        ...
        delete process.env['FAST_PATH_REFERENCED_TOKEN'];
        const qwenHome = useTempQwenHome();
        writeFileSync(join(qwenHome, '.env'), 'FAST_PATH_REFERENCED_TOKEN=from-home-env\n');
        tempWorkspace = realpathSync(mkdtempSync(join(os.tmpdir(), 'qws-fast-path-settings-env-')));
        mkdirSync(join(tempWorkspace, '.qwen'));
        writeFileSync(join(tempWorkspace, '.qwen', 'settings.json'), JSON.stringify({
            env: { QWEN_SERVER_TOKEN: '${FAST_PATH_REFERENCED_TOKEN}' },
    ...
    it.each([
        ...
        mkdirSync(join(tempWorkspace, '.qwen'));
        writeFileSync(join(tempWorkspace, '.qwen', 'settings.json'), settingsJson);
        process.chdir(tempWorkspace);
    ...
    it('checks trust against the canonical explicit workspace path', async () => {
        ...
        writeFileSync(join(tempWorkspace, '.env'), 'QWEN_SERVER_TOKEN=from-symlinked-untrusted-workspace-env\n');
        process.chdir(tempLaunchCwd);
