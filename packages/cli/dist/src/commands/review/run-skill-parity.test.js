...
const SKILL_PATH = join(repoRoot, 'packages/core/src/skills/bundled/review/SKILL.md');
/** The `{target}` token, rendered per class exactly as the skill defines it. */
const TARGETS = {
    pr: { cls: { kind: 'pr', number: '9014' }, token: 'pr-9014' },
    file: { cls: { kind: 'file', base: 'foo.ts' }, token: 'foo.ts' },
    local: { cls: { kind: 'local' }, token: 'local' },
};
describe('run pins match the bundled skill templates', () => {
    ...
    itWithSkill('composedNameFor renders Step 6’s --out template', () => {
        ...
        const m = /--out\s+\.qwen\/tmp\/(qwen-review-\{target\}-composed\.json)(?=\s|$)/.exec(skill);
    ...
    itWithSkill('reportPatternFor accepts Step 8’s report stems', () => {
        ...
        const stems = [
            ...skill.matchAll(/`\.qwen\/reviews\/<YYYY-MM-DD>-<HHMMSS>-([^`]+)\.md`/g),
        ].map((s) => s[1]);
        // A miss here means Step 8 no longer lists those stems: update
        // reportPatternFor and this oracle together to the new template.
        expect(stems).toEqual(expect.arrayContaining(['local', 'pr-<number>', '<filename>']));
        const render = (stem) => `2026-08-13-101010-${stem}.md`
            .replace('pr-<number>', 'pr-9014')
            .replace('<filename>', 'foo.ts');
        expect(reportPatternFor(TARGETS.pr.cls).test(render('pr-<number>'))).toBe(true);
        expect(reportPatternFor(TARGETS.file.cls).test(render('<filename>'))).toBe(true);
        expect(reportPatternFor(TARGETS.local.cls).test(render('local'))).toBe(true);
        ...
        expect(reportPatternFor(TARGETS.local.cls).test(render('pr-<number>'))).toBe(false);
        expect(reportPatternFor(TARGETS.file.cls).test(render('local'))).toBe(false);
    });
});
//# sourceMappingURL=run-skill-parity.test.js.map