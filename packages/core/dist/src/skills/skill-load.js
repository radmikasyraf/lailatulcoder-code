import { parseAllowedToolsField, parseModelField, parsePathsField, parseUserInvocableField, validateSkillName, } from './types.js';
...
export function parseSkillContent(content, filePath) {
    ...
    const disableModelInvocation = disableModelInvocationRaw === true || disableModelInvocationRaw === 'true'
        ? true
        : undefined;
 ...
 */
export function parsePriorityField(frontmatter, filePath, 
...
warn = (message) => debugLogger.warn(message)) {
    ...
    if (typeof raw !== 'number' || !Number.isFinite(raw)) {
        warn(`Ignoring invalid priority value in ${filePath}: expected a finite number.`);
        return undefined;
    }
    return raw;
}
 ...
 */
export function normalizeSkillPriority(priority) {
