...
import { collectSessionPathInfo, formatSessionPathInfo, } from './sessionPaths.js';
describe('sessionPaths', () => {
    ...
    it('collects current session paths and latest matching OpenAI log', async () => {
        ...
        await fs.writeFile(path.join(openAILogDir, 'openai-2026-01-01T00-00-00-000Z-old.json'), JSON.stringify({ context: { sessionId } }), 'utf-8');
        const latestOpenAILog = path.join(openAILogDir, 'openai-2026-01-02T00-00-00-000Z-new.json');
        await fs.writeFile(latestOpenAILog, JSON.stringify({ context: { sessionId } }), 'utf-8');
    ...
    it('matches OpenAI logs by promptId when sessionId is absent', async () => {
        ...
        await fs.mkdir(openAILogDir, { recursive: true });
        const latestOpenAILog = path.join(openAILogDir, 'openai-2026-01-02T00-00-00-000Z-new.json');
        await fs.writeFile(latestOpenAILog, JSON.stringify({ context: { promptId: sessionId } }), 'utf-8');
        const context = createMockCommandContext({
            services: {
                config: {
                    getSessionId: vi.fn().mockReturnValue(sessionId),
    ...
    it('limits OpenAI log JSON scans to recent files', async () => {
        ...
        await fs.mkdir(openAILogDir, { recursive: true });
        for (let i = 0; i <= 100; i++) {
            await fs.writeFile(path.join(openAILogDir, `openai-2026-01-01T00-00-00-000Z-${String(i).padStart(3, '0')}.json`), JSON.stringify({
    ...
    it('keeps session output when the OpenAI log directory is unreadable', async () => {
        const sessionId = '2a25a035-da35-4722-850e-b8aa074bd244';
        const openAILogDir = path.join(tmpDir, 'openai-logs');
        vi.spyOn(fs, 'readdir').mockRejectedValue(Object.assign(new Error('denied'), { code: 'EACCES' }));
        const context = createMockCommandContext({
            services: {
                config: {
                    getSessionId: vi.fn().mockReturnValue(sessionId),
...
//# sourceMappingURL=sessionPaths.test.js.map