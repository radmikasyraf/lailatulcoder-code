...
import { ASSET_HEADER_BYTES, assetsBranch, parseAssetsRepo, rawAssetUrl, remoteAssetPath, validateAssetBatch, validateAssetContent, } from './lib/assets.js';
import { validateFindings, buildReport, } from '../../utils/findings.js';
/** The Contents-API dance for one file: create, or update when it exists. */
function putContent(repo, branch, remotePath, contentBase64, pr) {
    ...
    try {
        ...
        let existing;
        try {
            existing = gh('api', `${api}?ref=${encodeURIComponent(branch)}`, '--jq', '.sha').trim();
        }
        catch {
        ...
        ghWithInputRetried(payload(existing), 'api', '-X', 'PUT', api, '--input', '-');
    }
}
...
function ensureBranch(repo, branch) {
    ...
    try {
        baseSha = gh('api', `repos/${repo}/git/ref/heads/${defaultBranch.trim()}`, '--jq', '.object.sha');
    }
    catch (err) {
        ...
        const msg = err instanceof Error ? err.message : String(err);
        if (/HTTP 404|Not Found/i.test(msg)) {
            throw new Error(`assets repository ${repo} appears to be empty (its default branch ` +
    ...
    if (base === '' || base === 'null') {
        throw new Error(`could not resolve the head of ${repo}@${defaultBranch.trim()} — the ` +
            'ref lookup returned nothing to branch from.');
    }
    try {
        ghWithInputRetried(JSON.stringify({ ref: `refs/heads/${branch}`, sha: base }), 'api', '-X', 'POST', `repos/${repo}/git/refs`, '--input', '-');
    }
    catch (err) {
...
export function runPublishAssets(args) {
    ...
    if (args.host !== undefined) {
        try {
            ...
            refuse(`${err instanceof Error ? err.message : String(err)} (from --host)`);
            return;
        }
    ...
    if (!auth.ok) {
        refuse(`not authorised — ${auth.why}. Evidence images are published only for ` +
    ...
    if (effectiveHost) {
        try {
            ...
            refuse(`${err instanceof Error ? err.message : String(err)} (from ${args.host !== undefined
                ? '--host'
    ...
    if (args.findings) {
        ...
        try {
            ...
            findings = validateFindings(Array.isArray(artifact) ? artifact : artifact.findings);
        }
        catch (err) {
            refuse(`cannot use the findings artifact at ${JSON.stringify(args.findings)}: ${err instanceof Error ? err.message : String(err)}`);
            return;
        }
        for (const f of findings) {
            for (const a of f.assetFiles ?? [])
                fileList.push(a);
        }
    }
    ...
    if (unique.length === 0) {
        ...
        // repair.
        if (args.findings) {
            writeStderrLine('publish-assets: no finding carries assetFiles — nothing to publish.');
    ...
    for (const file of unique) {
        ...
        try {
            ...
            refuse(`cannot read ${JSON.stringify(file)}: ${err instanceof Error ? err.message : String(err)}`);
            return;
        }
    }
    ...
    for (const st of stats) {
        ...
        try {
            ...
            refuse(`cannot read ${JSON.stringify(st.file)}: ${err instanceof Error ? err.message : String(err)}`);
            return;
        }
        ...
        const contentRuling = validateAssetContent(st.basename, content.subarray(0, ASSET_HEADER_BYTES));
        if (!contentRuling.ok) {
    ...
    const headSha = gh('api', `repos/${repo}/git/ref/heads/${branch}`, '--jq', '.object.sha').trim();
    const published = prepared.map((p) => ({
        file: p.file,
    ...
    if (findings && !args.findingsOut) {
        writeStderrLine('publish-assets: --findings given without --findings-out — the URLs are ' +
    ...
    writeStderrLine(`publish-assets: ${published.length} file(s) → ${repo}@${branch} ` +
        `(commit ${headSha.slice(0, 9)})`);
    writeStdoutLine(JSON.stringify({ published: true, count: published.length }));
}
export const publishAssetsCommand = {
    command: 'publish-assets',
    describe: 'Publish review evidence images to the user-designated assets repository (QWEN_REVIEW_ASSETS_REPO) and emit commit-pinned URLs — gated on the same authorisation as submit',
    builder: (yargs) => yargs
        .option('pr', {
        type: 'number',
        demandOption: true,
        ...
        type: 'string',
        describe: 'The owner/repo the reviewed PR lives in — strengthens the authorisation binding for URL-shaped review arguments; omit to bind by PR number alone',
    })
        .option('files', {
        type: 'string',
        array: true,
        ...
        .option('findings', {
        type: 'string',
        describe: 'Findings artifact whose per-finding assetFiles should be published',
    })
        .option('findings-out', {
        type: 'string',
        implies: 'findings',
        describe: 'Where to write the findings artifact with published URLs woven into each finding (requires --findings)',
    })
        .option('out', {
        type: 'string',
        demandOption: true,
        ...
        type: 'boolean',
        default: false,
        describe: 'Pass ONLY when the user asked, in a message they typed this session, for this review to be published',
    })
        .option('skill-args', {
        type: 'string',
        describe: 'Test seam: path to the recorded review arguments (ignored when a session id is present)',
    }),
    handler: (argv) => {
        runPublishAssets({
            pr: argv['pr'],
            reviewedRepo: argv['reviewed-repo'],
            files: argv['files'],
            findings: argv['findings'],
            findingsOut: argv['findings-out'],
            out: argv['out'],
            host: argv['host'],
            userAuthorized: Boolean(argv['user-authorized']),
            skillArgs: argv['skill-args'],
...
//# sourceMappingURL=publish-assets.js.map