...
import { convertCompatibleExtension, SUPPORTED_EXTENSION_MANIFESTS, } from './extension-converter.js';
import { AGENT_PLUGIN_MANIFEST, getAgentPluginSchemaStatus, } from './agent-plugins-v1/manifest.js';
...
import { ExtensionCredentialUnavailableError, resolveStoredGitCredential, } from './extension-git-credentials.js';
const debugLogger = createDebugLogger('EXT_GITHUB');
const SUPPORTED_ARCHIVE_EXTENSIONS = ['.tar.gz', '.zip'];
const ARCHIVE_DOWNLOAD_TIMEOUT_MS = 120_000;
const ARCHIVE_DOWNLOAD_MAX_BYTES = 100 * 1024 * 1024;
const MINIMUM_PINNED_GIT_VERSION = { major: 2, minor: 37 };
function getSupportedArchiveExtensionFromPathname(pathname) {
    const normalizedPathname = pathname.toLowerCase();
    return SUPPORTED_ARCHIVE_EXTENSIONS.find((extension) => normalizedPathname.endsWith(extension));
}
function getSupportedArchiveExtension(url) {
    let pathname;
    try {
...
export function isSupportedArchivePath(source) {
    return getSupportedArchiveExtensionFromPathname(source) !== undefined;
}
export function isSupportedArchiveUrl(source) {
    let parsedUrl;
    try {
...
function createRedactedErrorCause(error, message) {
    if (!(error instanceof Error)) {
        return new Error(message);
    }
    const cause = Object.create(Object.getPrototypeOf(error));
...
function getGitHubCredential(source) {
...
async function assertPinnedGitSupported() {
    const { simpleGit } = await loadSimpleGit();
    const version = await simpleGit().version();
    if (version.major < MINIMUM_PINNED_GIT_VERSION.major ||
        ...
        const detectedVersion = [version.major, version.minor, version.patch]
            ...
            .join('.');
        throw new Error(`Public extension Git installs require Git 2.37 or newer for secure DNS pinning; found Git ${detectedVersion}. Upgrade Git, or install the extension from a local path or archive instead.`);
    }
}
function createPinnedGitConfig(curlResolve) {
    return [
...
function restrictGitEnvironment(git, networkPolicy, authentication) {
    if (networkPolicy !== 'public' && !authentication)
        return git;
    const environment = {
    ...
    if (authentication) {
        const value = Buffer.from(`${authentication.credential.username}:${authentication.credential.password}`, 'utf8').toString('base64');
 ...
 */
export async function cloneFromGit(installMetadata, destination, signal, credential, hideSource = false) {
    ...
    try {
        ...
        if (installMetadata.networkPolicy === 'public') {
            ...
            const networkTarget = await resolveNetworkTarget(installMetadata.source, installMetadata.networkPolicy, signal);
            networkConfig = networkTarget.curlResolve
                ? createPinnedGitConfig(networkTarget.curlResolve)
                : [];
        }
        const effectiveCredential = credential ?? getGitHubCredential(installMetadata.source);
        const git = restrictGitEnvironment(simpleGit(destination, {
            ...(signal ? { abort: signal } : {}),
            ...(networkConfig.length > 0
                ...
                : {}),
        }), installMetadata.networkPolicy, effectiveCredential
        ...
        throw new Error(`Failed to clone Git repository from ${redactedSource} ${redactedErrorMessage}`, {
...
export function parseGitHubRepoForReleases(source) {
    ...
    if (parts?.length !== 2 || parsedUrl?.host !== 'github.com') {
        throw new Error(`Invalid GitHub repository source: ${redactUrlCredentials(source)}. Expected "owner/repo" or a github repo uri.`);
    }
    const owner = parts[0];
...
async function fetchReleaseFromGithub(owner, repo, ref, signal, networkPolicy) {
    const endpoint = ref ? `releases/tags/${ref}` : 'releases/latest';
    const url = `https://api.github.com/repos/${owner}/${repo}/${endpoint}`;
    return await fetchJson(url, signal, networkPolicy);
}
export async function checkForExtensionUpdate(extension, extensionManager, signal) {
    signal?.throwIfAborted();
    ...
    if (installMetadata?.type === 'local') {
        ...
        try {
            ...
            if (isSupportedArchivePath(installMetadata.source)) {
                tempDir = await fs.promises.mkdtemp(path.join(os.tmpdir(), 'extension-archive-update-'));
                signal?.throwIfAborted();
            ...
            if (tempDir !== undefined || installMetadata.originSource === 'Qoder') {
                ...
                const converted = await convertCompatibleExtension(sourceBeforeConversion, installMetadata.pluginName, installMetadata.networkPolicy, signal);
            ...
            signal?.throwIfAborted();
            debugLogger.error(`Failed to check for update for local extension "${extension.name}". Could not load extension from source path: ${redactUrlCredentials(installMetadata.source)}. Error: ${redactUrlCredentials(getErrorMessage(e))}`);
            return ExtensionUpdateState.NOT_UPDATABLE;
        }
        finally {
            if (tempDir) {
        ...
        if (!latestConfig) {
            debugLogger.error(`Failed to check for update for local extension "${extension.name}". Could not load extension from source path: ${redactUrlCredentials(installMetadata.source)}`);
            return ExtensionUpdateState.NOT_UPDATABLE;
        }
    ...
    if (installMetadata?.type === 'archive-url') {
        ...
        try {
            ...
            const converted = await convertCompatibleExtension(tempDir, installMetadata.pluginName, installMetadata.networkPolicy, signal);
            ...
            if (!latestConfig) {
                debugLogger.error(`Failed to check for update for archive URL extension "${extension.name}". Could not load extension from source URL: ${redactUrlCredentials(installMetadata.source)}`);
            ...
            signal?.throwIfAborted();
            debugLogger.error(`Failed to check for update for archive URL extension "${extension.name}" from ${redactUrlCredentials(installMetadata.source)}: ${redactUrlCredentials(getErrorMessage(error))}`);
            return ExtensionUpdateState.ERROR;
        }
        finally {
            if (tempDir) {
    ...
    try {
        if (installMetadata.type === 'git') {
            const storedCredential = installMetadata.credentialPersistence === 'stored'
            ...
            if (installMetadata.gitCommit) {
                ...
                const localGit = simpleGit(extension.path, signal ? { abort: signal } : undefined);
            ...
            if (installMetadata.networkPolicy === 'public') {
                ...
                const remoteTarget = await resolveNetworkTarget(parsedRemote, installMetadata.networkPolicy, signal);
                networkConfig = remoteTarget.curlResolve
                    ? createPinnedGitConfig(remoteTarget.curlResolve)
                    : [];
            }
            signal?.throwIfAborted();
            const effectiveCredential = storedCredential ?? getGitHubCredential(remoteUrl);
            const git = restrictGitEnvironment(simpleGit(extension.path, {
                ...(signal ? { abort: signal } : {}),
                ...(networkConfig.length > 0
                    ? {
                        config: networkConfig,
            ...
            const peeledLine = remoteLines.find((line) => line.split('\t')[1]?.endsWith('^{}'));
            ...
            const releaseData = await fetchReleaseFromGithub(owner, repo, installMetadata.ref, signal, installMetadata.networkPolicy);
        ...
        if (error instanceof ExtensionCredentialUnavailableError)
            throw error;
        signal?.throwIfAborted();
        debugLogger.error(`Failed to check for updates for extension "${redactUrlCredentials(installMetadata.source)}": ${redactUrlCredentials(getErrorMessage(error))}`);
        return ExtensionUpdateState.ERROR;
    }
}
export async function downloadFromGitHubRelease(installMetadata, destination, signal) {
    const { source, ref } = installMetadata;
    const { owner, repo } = parseGitHubRepoForReleases(source);
    const releaseData = await fetchReleaseFromGithub(owner, repo, ref, signal, installMetadata.networkPolicy);
    if (!releaseData) {
    ...
    try {
        ...
        throw new Error(`Failed to download release from ${redactUrlCredentials(installMetadata.source)}: ${redactUrlCredentials(getErrorMessage(error))}`);
    }
    signal?.throwIfAborted();
...
export async function downloadFromArchiveUrl(installMetadata, destination, signal) {
    const archiveExtension = getSupportedArchiveExtension(installMetadata.source);
    if (!archiveExtension) {
        throw new Error(`Unsupported archive URL for extension install: ${redactUrlCredentials(installMetadata.source)}`);
    }
    const archiveName = path.basename(new URL(installMetadata.source).pathname) ||
    ...
    try {
        ...
        signal?.throwIfAborted();
        throw new Error(`Failed to download archive from ${redactUrlCredentials(installMetadata.source)}: ${redactUrlCredentials(getErrorMessage(error))}`);
    }
    signal?.throwIfAborted();
    await extractArchiveFile(downloadedAssetPath, destination, signal);
    signal?.throwIfAborted();
...
export async function extractArchiveFile(archivePath, destination, signal) {
    signal?.throwIfAborted();
    if (!isSupportedArchivePath(archivePath)) {
        throw new Error(`Unsupported archive file for extension install: ${redactUrlCredentials(archivePath)}`);
    }
    try {
        ...
        signal?.throwIfAborted();
        throw new Error('Extension archive could not be extracted. Make sure it is a valid ' +
...
async function fetchJson(url, signal, networkPolicy) {
    const timeoutError = new Error('Timed out fetching GitHub API response');
    const timeoutController = new AbortController();
    const hardDeadline = setTimeout(() => timeoutController.abort(timeoutError), ARCHIVE_DOWNLOAD_TIMEOUT_MS);
    hardDeadline.unref();
        ...
        try {
            req = https.get(url, {
                ...
                if (res.statusCode !== 200) {
                    res.resume();
                    return fail(new Error(`Request failed with status code ${res.statusCode}`));
                }
                const chunks = [];
...
async function downloadFile(url, dest, options = { includeGitHubToken: false }, redirectCount = 0, signal) {
    if (redirectCount > 10) {
    ...
    return new Promise((resolve, reject) => {
        ...
        const req = https
            ...
            if (res.statusCode === 301 ||
                ...
                const redirectOptions = redirectHost === parsedUrl.host
                    ? options
                    : { ...options, includeGitHubToken: false };
                cleanup();
                downloadFile(redirectUrl.toString(), dest, redirectOptions, redirectCount + 1, signal)
                    .then(finish)
                    .catch(fail);
                return;
            }
            ...
            res.on('data', (chunk) => {
                ...
                if (bytesWritten > ARCHIVE_DOWNLOAD_MAX_BYTES) {
                    res.destroy();
                    file.destroy();
                    fail(new Error(`Extension archive download exceeded maximum size of ${ARCHIVE_DOWNLOAD_MAX_BYTES} bytes`));
                }
            });
            res.on('error', (error) => {
                file.destroy();
...
export async function extractFile(file, dest, signal) {
    signal?.throwIfAborted();
...
async function flattenSingleExtensionDirectory(destination, archivePath) {
    ...
    for (const file of extractedDirFiles) {
        ...
        if (fs.existsSync(destinationPath)) {
            throw new Error(`Extension archive cannot be flattened because "${file}" exists at both the archive root and inside "${lonelyDir.name}".`);
        }
    }
    for (const file of extractedDirFiles) {
        const destinationPath = path.join(destination, file);
...
function getSupportedManifestList() {
    return [
        ...SUPPORTED_EXTENSION_MANIFESTS,
        `${AGENT_PLUGIN_MANIFEST} (Agent Plugins)`,
    ].join(', ');
}
function hasSupportedExtensionSourceManifest(rootPath) {
    return (getAgentPluginSchemaStatus(rootPath) !== 'unrelated' ||
        SUPPORTED_EXTENSION_MANIFESTS.some((manifestPath) => fs.existsSync(path.join(rootPath, manifestPath))));
}
function assertExtractedArchiveContainsExtensionSource(destination) {
    if (hasSupportedExtensionSourceManifest(destination)) {
        return;
    }
    throw new Error('Extension archive is missing a supported extension manifest. ' +
        `Expected one of: ${getSupportedManifestList()} at the archive root, ` +
        'or inside a single top-level extension directory.');
}
function getContainedArchiveName(destination, archivePath) {
