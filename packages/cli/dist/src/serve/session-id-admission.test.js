    ...
    return {
        ...
        SessionService: class {
            ...
            async findSessionIdIgnoringCase(sessionId) {
                return (await sessionServiceMock.exists(this.cwd, this.options.runtimeBaseDir, sessionId))
                    ? sessionId
                    : undefined;
            }
            getWorktreeSessionPathForArchiveState(sessionId, state) {
                return sessionServiceMock.sidecarPath(this.cwd, this.options.runtimeBaseDir, sessionId, state);
            }
        },
    };
});
...
const tempDirs = [];
function fakeBridge(liveSessionIds = []) {
...
describe('RequestedSessionIdAdmission', () => {
    beforeEach(() => {
        ...
        sessionServiceMock.sidecarPath.mockReturnValue(path.join(os.tmpdir(), `missing-${crypto.randomUUID()}`));
    });
    afterEach(async () => {
    ...
    it('claims synchronously before the persisted scan finishes', async () => {
        ...
        await expect(admission.reserveCreate(SESSION_ID, {
            bridge,
            workspaceCwd: '/one',
    ...
    it('checks every live bridge and every registered persistence target', async () => {
        ...
        await expect(diskAdmission.reserveCreate(SESSION_ID, {
            bridge: current,
            workspaceCwd: '/one',
        })).rejects.toMatchObject({
            code: 'session_id_conflict',
        ...
        expect(sessionServiceMock.exists).toHaveBeenCalledWith('/two', '/runtime-two', SESSION_ID);
    });
    it.each(['active', 'archived'])('treats an %s worktree sidecar as persisted history', async (persistedState) => {
        const bridge = fakeBridge();
        const admission = createRequestedSessionIdAdmission({
            archiveCoordinator: new SessionArchiveCoordinator(),
            getBridges: () => [bridge],
        ...
        const tempDir = await fsp.mkdtemp(path.join(os.tmpdir(), 'requested-session-id-'));
        tempDirs.push(tempDir);
        const persistedSidecar = path.join(tempDir, `${persistedState}.worktree.json`);
        await fsp.writeFile(persistedSidecar, '{}');
        sessionServiceMock.sidecarPath.mockImplementation((_cwd, _runtimeBaseDir, _sessionId, state) => state === persistedState
            ? persistedSidecar
    ...
    it('treats a case-only transcript conflict as persisted occupancy', async () => {
        ...
        sessionServiceMock.exists.mockRejectedValueOnce(new SessionIdCaseConflictError(SESSION_ID));
        await expect(admission.reserveCreate(SESSION_ID, {
            bridge,
            workspaceCwd: '/one',
        })).rejects.toMatchObject({
    ...
    it('shares restore claims only on the same bridge generation', () => {
        ...
        expect(() => admission.reserveRestore(SESSION_ID, {
            bridge: secondBridge,
            workspaceCwd: '/two',
    ...
    it('treats mixed-case caller UUIDs as the same restore claim', () => {
        ...
        expect(() => admission.reserveRestore(SESSION_ID.toUpperCase(), {
            bridge: secondBridge,
            workspaceCwd: '/two',
        })).toThrowError(RequestedSessionIdAdmissionError);
        first.release();
    ...
    it('names the live foreign owner workspace id on restore conflicts', () => {
        ...
        const admission = createRequestedSessionIdAdmission({
            ...
            getPersistenceTargets: () => [],
            getBridgeWorkspaceId: (bridge) => bridge === liveBridge ? 'live-workspace' : undefined,
        });
        let error;
        try {
    ...
    it('returns retryable unavailable for a non-ENOENT sidecar error', async () => {
        ...
        const tempDir = await fsp.mkdtemp(path.join(os.tmpdir(), 'requested-session-id-error-'));
        tempDirs.push(tempDir);
        const nonDirectory = path.join(tempDir, 'not-a-directory');
        await fsp.writeFile(nonDirectory, 'file');
        sessionServiceMock.sidecarPath.mockReturnValue(path.join(nonDirectory, 'sidecar.json'));
        await expect(admission.reserveCreate(SESSION_ID, {
            bridge,
            workspaceCwd: '/one',
        })).rejects.toMatchObject({
...
//# sourceMappingURL=session-id-admission.test.js.map