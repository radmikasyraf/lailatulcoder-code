...
**Goal:** Make `open-computer-use` a zero-config built-in capability in lailatul-coder. 9 computer-use tools appear in the deferred tool list as `computer_use__click`, `computer_use__type_text`, etc. First invocation transparently installs the upstream npm binary, walks the user through macOS Accessibility / Screen Recording permissions if needed, and forwards the call to the upstream MCP server.

**Architecture:** Thin shell over upstream `npx -y open-computer-use mcp`. We do NOT bundle the binary; upstream's `npx` cache + `.app` bundle handles distribution and macOS TCC. 9 tools are registered as parameterized `ComputerUseTool` instances (one per tool name) backed by a singleton `ComputerUseClient` that owns a long-running MCP stdio child process. Bootstrap state machine layers on top: standard lailatul-coder tool permission (existing) → first-time install confirm → optional macOS permission guide.

**Tech Stack:** TypeScript, vitest, `@modelcontextprotocol/sdk` (already a lailatul-coder dep), `node:child_process`, `node:fs/promises`.
...
Expected: PASS (if there's no test file, run `npm run build -- --filter @lailatul-coder/lailatul-coder-core` to typecheck)
...
describe('computer-use schemas', () => {
  ...
  it('each tool name matches the upstream convention (no computer_use__ prefix)', () => {
    ...
    // The computer_use__ prefix lives on the lailatul-coder-facing wrapper.
...
Create `packages/core/src/tools/computer-use/schemas.ts`. The schemas below are MVP — they reflect upstream's tool surface and parameter naming. The `sync-computer-use-schemas.ts` script (Task 13) will regenerate this file from a live upstream snapshot in CI before each lailatul-coder release.
 ...
 * the `computer_use__` prefix is added by the lailatul-coder-facing wrapper in
...
Run: `npm run build -- --filter @lailatul-coder/lailatul-coder-core --filter @lailatul-coder/lailatul-coder`
Expected: PASS
 ...
 * stays alive until `stop()` or lailatul-coder exits. State (element_index
...
npm run build -- --filter @lailatul-coder/lailatul-coder-core
```
...
- [ ] **Step 2: Build lailatul-coder**

Run: `npm run build`
Expected: PASS.

- [ ] **Step 3: Launch lailatul-coder and test discovery**

Start lailatul-coder, then ask the model: _"Use the ToolSearch tool with query 'click computer use' to find any desktop automation tools available."_
...
Note: the actual "ask user a question mid-execution" mechanic in lailatul-coder uses the existing tool-confirmation framework. **IMPLEMENTER**: before writing this task's implementation, grep for `shouldConfirmExecute` in `packages/core/src/tools/` to see how `shell.ts` / similar do confirmation. This task assumes that mechanic is available; if it isn't, swap in `process.stderr.write` + read from `process.stdin` for the install confirm (acceptable v0 UX).
 ...
 * lailatul-coder's BaseDeclarativeTool exposes a `shouldConfirmExecute()`
...
export interface BootstrapDeps {
   ...
   * true if approved. Implementation may use the lailatul-coder confirm
...
function defaultDeps(): BootstrapDeps {
  return {
    ...
    promptInstallApproval: async (spec) => {
      ...
      // lailatul-coder's standard confirm pathway when wired in.
      ...
      // lailatul-coder confirm system. For headless / SDK contexts the
...
### Task 11: Wire the real `promptInstallApproval` to lailatul-coder's confirm system
...
Launch lailatul-coder and ask a computer-use question. Confirm the install prompt appears in the chosen UX (confirm dialog or stderr) and that approving it persists state correctly.

- [ ] **Step 3: Commit**

```bash
git add -A
git commit -m "feat(computer-use): wire install approval to lailatul-coder confirm UX"
```
...
```bash
npm run build
# launch lailatul-coder, ask a computer-use question
```
...
Runs as part of lailatul-coder release prep. Spawns `npx -y open-computer-use@<pin> mcp`, sends `tools/list`, regenerates `schemas.ts`.
...
- Idle timeout for the MCP server process (resource savings; v0 keeps it alive until lailatul-coder exits).
