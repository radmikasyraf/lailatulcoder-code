 ...
 */
export function buildWebShellCsp(frameAncestors = []) {
 ...
 * gate in `run-LailatulCoder-serve.ts` — can apply the same discriminator.
 */
 ...
 */
export function isPreAuthWebShellRequest(req) {
...
function createSendIndex(webShellDir, frameAncestors = []) {
    ...
    return (res) => {
        ...
        // installed LailatulCoder via nvm.
        res.sendFile(indexPath, { cacheControl: false, dotfiles: 'allow' }, (err) => {
            if (!err)
                return;
            ...
            // deploy).
            writeStderrLine(`LailatulCoder serve: Web Shell index send failed: ${err instanceof Error ? err.message : String(err)}`);
            if (!res.headersSent) {
 ...
 */
export function mountWebShellAssets(app, webShellDir, frameAncestors = []) {
    const sendIndex = createSendIndex(webShellDir, frameAncestors);
    app.use('/assets', express.static(path.join(webShellDir, 'assets'), {
        index: false,
        immutable: true,
        maxAge: '1y',
    }));
    ...
    app.use('/assets', (req, res) => {
        ...
        if (isServeDebugMode()) {
            writeStderrLine(`LailatulCoder serve: Web Shell asset not found: ${req.originalUrl}`);
        }
 ...
 */
export function mountWebShellSpaFallback(app, webShellDir, frameAncestors = []) {
    const sendIndex = createSendIndex(webShellDir, frameAncestors);
    app.use((req, res, next) => {
        ...
        if (isServeDebugMode()) {
            writeStderrLine(`LailatulCoder serve: Web Shell SPA fallback served for ${req.method} ${req.originalUrl}`);
        }
        sendIndex(res);
    });
}
//# sourceMappingURL=web-shell-static.js.map