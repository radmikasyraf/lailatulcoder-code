...
const { buildReviewPrompt, newestArtifactSince, exitCodeFor, killProcessGroup, runCommand, classifyRunTarget, composedPatternFor, reportPatternFor, } = await import('./run.js');
...
describe('buildReviewPrompt', () => {
    ...
    it('threads target, effort, and --comment through verbatim', () => {
        expect(buildReviewPrompt({ target: '7724', effort: 'high', comment: true })).toBe('/review 7724 --effort high --comment');
    });
    it('omits what was not asked for', () => {
        expect(buildReviewPrompt({ effort: 'medium' })).toBe('/review --effort medium');
    });
    ...
    it('threads --resume through, after the other flags', () => {
        expect(buildReviewPrompt({ target: '7724', resume: true })).toBe('/review 7724 --resume');
        expect(buildReviewPrompt({ target: '7724', resume: false })).toBe('/review 7724');
    });
    it('rejects a target that would re-tokenize into extra args', () => {
        ...
        expect(() => buildReviewPrompt({ target: '123 --comment' })).toThrow(/Invalid review target/);
        expect(() => buildReviewPrompt({ target: '--comment' })).toThrow(/Invalid review target/);
    });
    it('rejects a separators-only or empty target', () => {
        ...
        for (const target of ['/', '//', '\\']) {
            expect(() => buildReviewPrompt({ target })).toThrow(/Invalid review target/);
        }
    ...
    it('keeps accepting separator-bearing paths — the guard is not over-broad', () => {
        ...
        expect(buildReviewPrompt({ target: '/repo/src/foo.ts' })).toBe('/review /repo/src/foo.ts');
    ...
    it('rejects a target carrying quote characters', () => {
        ...
        expect(() => buildReviewPrompt({ target: "src/it's-a-file.ts" })).toThrow(/Invalid review target/);
        expect(() => buildReviewPrompt({ target: 'src/"quoted".ts' })).toThrow(/Invalid review target/);
    });
});
describe('newestArtifactSince', () => {
    ...
    it('ignores artifacts older than the run', () => {
        ...
        expect(newestArtifactSince(dir, /^qwen-review-.*composed\.json$/, start)).toBeNull();
    });
    it('returns the newest matching artifact from this run, mtime attached', () => {
        ...
        const best = newestArtifactSince(dir, /^qwen-review-.*composed\.json$/, start);
    ...
    it('returns null when the directory does not exist', () => {
        expect(newestArtifactSince(join(dir, 'absent'), /composed/, Date.now())).toBeNull();
    });
});
describe('target-pinned artifact patterns', () => {
    it('classifies exactly as the child parser does — no second classifier', () => {
        ...
        expect(classifyRunTarget('https://github.com/o/r/pull/9014/files')).toEqual({ kind: 'pr', number: '9014' });
...
describe('killProcessGroup', () => {
    ...
    it('kills the process tree via taskkill on Windows', () => {
        ...
        killProcessGroup(12345, 'SIGTERM');
        expect(execFileSyncMock).toHaveBeenCalledWith('taskkill', ['/pid', '12345', '/T', '/F'], { stdio: 'ignore' });
...
describe('review run (handler)', () => {
    ...
    function armChild(exit, composed) {
        spawnMock.mockImplementation(() => {
            ...
            setImmediate(() => {
                if (composed) {
                    ...
                    writeFileSync(join(REVIEW_TMP_DIR, 'qwen-review-local-composed.json'), JSON.stringify(composed), 'utf8');
    ...
    it('captures the verdict before Step 9 cleanup sweeps it', async () => {
        ...
        spawnMock.mockImplementation(() => {
            ...
            mkdirSync(REVIEW_TMP_DIR, { recursive: true });
            writeFileSync(join(REVIEW_TMP_DIR, 'qwen-review-local-composed.json'), JSON.stringify({ event: 'APPROVE', verdictLine: 'Verdict: Approve' }), 'utf8');
            child = new FakeChild();
            return child;
        });
    ...
    it('ignores a concurrent run’s other-PR verdict and captures its own', async () => {
        ...
        spawnMock.mockImplementation(() => {
            ...
            mkdirSync(REVIEW_TMP_DIR, { recursive: true });
            writeFileSync(join(REVIEW_TMP_DIR, 'qwen-review-pr-9013-composed.json'), JSON.stringify({ event: 'APPROVE', verdictLine: 'Verdict: Approve' }), 'utf8');
            ...
            utimesSync(join(REVIEW_TMP_DIR, 'qwen-review-pr-9013-composed.json'), Date.now() / 1000 + 60, Date.now() / 1000 + 60);
            // The neighbour's saved report exists too — and is newer than this
            // run's own by the time the scan runs.
            mkdirSync(REVIEWS_DIR, { recursive: true });
            writeFileSync(join(REVIEWS_DIR, '2026-08-13-071500-pr-9014.md'), '# own report', 'utf8');
            writeFileSync(join(REVIEWS_DIR, '2026-08-13-162336-pr-9013.md'), '# neighbour report', 'utf8');
            // Force the neighbour's report strictly newer, so an unpinned
            // newest-.md scan would deterministically pick the wrong one.
            utimesSync(join(REVIEWS_DIR, '2026-08-13-162336-pr-9013.md'), Date.now() / 1000 + 60, Date.now() / 1000 + 60);
            child = new FakeChild();
            return child;
        });
        ...
        writeFileSync(join(REVIEW_TMP_DIR, 'qwen-review-pr-9014-composed.json'), JSON.stringify({
            event: 'COMMENT',
            verdictLine: 'Verdict: Comment',
        }), 'utf8');
        await vi.advanceTimersByTimeAsync(1_000);
    ...
    it('republishes the newest recompose, not the first snapshot', async () => {
        ...
        spawnMock.mockImplementation(() => {
            ...
            writeFileSync(path, JSON.stringify({ event: 'APPROVE', verdictLine: 'Verdict: Approve' }), 'utf8');
    ...
    it('names the artifact it waited for when no verdict appears', async () => {
        ...
        expect(outs.join('')).toContain('no composed verdict was produced (expected');
        ...
        const result = JSON.parse(outs.join(''));
        expect(result.completed).toBe(false);
        expect(result.expectedComposedName).toBe('qwen-review-pr-9014-composed.json');
    });
    it('closes the child stdin so piped input cannot defeat slash detection', async () => {
        ...
        await runHandler();
        const [, argvUsed, opts] = spawnMock.mock.calls[0];
    ...
    it('passes the approval mode through to the child CLI', async () => {
        ...
        const [, argvUsed] = spawnMock.mock.calls[0];
    ...
    describe('child env: QWEN_CODE_CLI version skew', () => {
        ...
        it.skipIf(process.platform === 'win32')('keeps the bare-`qwen` fallback when a shell could not exec this build', async () => {
        ...
        it.skipIf(process.platform === 'win32')('writes the fallback for a tsx dev entry — a 0644 .ts is not execable', async () => {
        ...
        it('preserves an inherited entry that IS this build', async () => {
            ...
            process.env['QWEN_CODE_CLI'] = process.argv[1];
            armChild(0, { event: 'COMMENT', verdictLine: 'Verdict: Comment' });
            await runHandler();
        ...
        it('preserves an inherited entry that is a symlink to this build', async () => {
            ...
            symlinkSync(process.argv[1], shim);
    ...
    it('forwards a parent signal to the child group and exits 128+signum', async () => {
        ...
        const registered = onSpy.mock.calls
            .map(([sig]) => sig)
            .filter((sig) => ['SIGHUP', 'SIGINT', 'SIGTERM'].includes(sig));
        expect(registered).toEqual(expect.arrayContaining(['SIGHUP', 'SIGINT', 'SIGTERM']));
        const handler = onSpy.mock.calls.find(([sig]) => sig === 'SIGTERM')?.[1];
        handler('SIGHUP');
        handler('SIGINT');
        handler('SIGTERM');
    ...
    it('distinguishes a corrupt composed artifact from a missing one', async () => {
        spawnMock.mockImplementation(() => {
            ...
            setImmediate(() => {
                mkdirSync(REVIEW_TMP_DIR, { recursive: true });
                writeFileSync(join(REVIEW_TMP_DIR, 'qwen-review-local-composed.json'), '{truncated', 'utf8');
                child.emit('close', 0);
            });
            return child;
        });
