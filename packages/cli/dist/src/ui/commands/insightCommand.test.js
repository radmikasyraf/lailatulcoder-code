...
describe('insightCommand', () => {
    ...
    it('uses runtime base dir to locate projects directory', async () => {
        ...
        expect(mockOpenBrowserSecurely).toHaveBeenCalledWith(pathToFileURL(path.resolve('runtime-output', 'insights', 'insight-2026-03-05.html')).href, {
            allowFile: true,
            allowedFilePaths: [
    ...
    it('shows the generated file path before attempting to open the browser', async () => {
        ...
        const messages = vi
            ...
            .mock.calls.map(([item]) => 'text' in item && typeof item.text === 'string' ? item.text : '');
        expect(messages).toContain(`Insights generated at: ${path.resolve('runtime-output', 'insights', 'insight-2026-03-05.html')}. If the browser does not open automatically, open this file manually.`);
        expect(messages.some((message) => message.includes('Opening insights'))).toBe(false);
    });
