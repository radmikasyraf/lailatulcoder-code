...
This monorepo contains two main packages: `@lailatul-coder/lailatul-coder` and `@lailatul-coder/lailatul-coder-core`.

## `@lailatul-coder/lailatul-coder`

This is the main package for LailatulCoder Ai. It is responsible for the user interface, command parsing, and all other user-facing functionality.

When this package is published, it is bundled into a single executable file. This bundle includes all of the package's dependencies, including `@lailatul-coder/lailatul-coder-core`. This means that whether a user installs the package with `npm install -g @lailatul-coder/lailatul-coder` or runs it directly with `npx @lailatul-coder/lailatul-coder`, they are using this single, self-contained executable.
...
Releases are managed through the [release.yml](https://github.com/LailatulCoder/lailatul-coder/actions/workflows/release.yml) GitHub Actions workflow. To perform a manual release for a patch or hotfix:
...
# Preview
npm install -g @lailatul-coder/lailatul-coder@preview

# Nightly
npm install -g @lailatul-coder/lailatul-coder@nightly
```
...
- `npx -y @lailatul-coder/lailatul-coder@latest --version` to validate the push worked as expected if you were not doing a rc or dev tag
- `npx -y @lailatul-coder/lailatul-coder@<release tag> --version` to validate the tag pushed appropriately
- _This is destructive locally_ `npm uninstall @lailatul-coder/lailatul-coder && npm uninstall -g @lailatul-coder/lailatul-coder && npm cache clean --force &&  npm install @lailatul-coder/lailatul-coder@<version>`
...
1.  Go to the [Actions tab](https://github.com/LailatulCoder/lailatul-coder/actions/workflows/release.yml) of the repository.
...
You can then inspect the generated tarballs to ensure that they contain the correct files and that the `package.json` files have been updated correctly. The tarballs will be created in the root of each package's directory (e.g., `packages/cli/lailatul-coder-0.1.6.tgz`).
...
- **Simplified Script Execution**: You can run scripts in any package from the root of the project using the `--workspace` flag. For example, to run the `build` script in the `cli` package, you can run `npm run build --workspace @lailatul-coder/lailatul-coder`.
