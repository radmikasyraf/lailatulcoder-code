...
import { listSavedWorkflows, } from '@lailatul-coder/lailatul-coder-core';
...
describe('SavedWorkflowLoader', () => {
    ...
    it('builds one tool-dispatch command per discovered workflow', async () => {
        ...
        const cmds = await new SavedWorkflowLoader(makeConfig()).loadCommands(signal);
    ...
    it('action dispatches the workflow tool with the scriptPath', async () => {
        ...
        const [cmd] = await new SavedWorkflowLoader(makeConfig()).loadCommands(signal);
    ...
    it('returns [] when workflows are disabled (tool not registered)', async () => {
        listMock.mockResolvedValue([entry()]);
        const cmds = await new SavedWorkflowLoader(makeConfig({ isWorkflowsEnabled: () => false })).loadCommands(signal);
        expect(cmds).toEqual([]);
        expect(listMock).not.toHaveBeenCalled();
    });
    it('returns [] in bare mode', async () => {
        listMock.mockResolvedValue([entry()]);
        const cmds = await new SavedWorkflowLoader(makeConfig({ getBareMode: () => true })).loadCommands(signal);
        expect(cmds).toEqual([]);
        expect(listMock).not.toHaveBeenCalled();
    });
    it('returns [] when folder trust is enabled but the folder is untrusted', async () => {
        listMock.mockResolvedValue([entry()]);
        const cmds = await new SavedWorkflowLoader(makeConfig({
    ...
    it('returns [] when the signal aborts during enumeration', async () => {
        ...
        const cmds = await new SavedWorkflowLoader(makeConfig()).loadCommands(aborted);
        expect(cmds).toEqual([]);
    });
});
//# sourceMappingURL=saved-workflow-loader.test.js.map