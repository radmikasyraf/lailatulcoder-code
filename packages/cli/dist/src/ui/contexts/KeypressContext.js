...
const KITTY_KEYPAD_PRINTABLE_KEYCODE_TO_CHAR = {
    57399: '0',
    57400: '1',
    57401: '2',
    57402: '3',
...
const KITTY_KEYPAD_FUNCTIONAL_KEYCODE_TO_NAME = {
    57417: 'left',
    57418: 'right',
    57419: 'up',
    57420: 'down',
...
export function KeypressProvider({ children, kittyProtocolEnabled, pasteWorkaround = false, config, debugKeystrokeLogging, initialCapturedInput, }) {
    const { stdin, setRawMode } = useStdin();
    const subscribers = useRef(new Set()).current;
    const mouseSubscribers = useRef(new Set()).current;
    ...
    const subscribeMouse = useCallback((handler) => {
        mouseSubscribers.add(handler);
    }, [mouseSubscribers]);
    const unsubscribeMouse = useCallback((handler) => {
    ...
    useEffect(() => {
                ...
                if (kittySequenceBufferRef.current) {
                    ...
                    if (kittySequenceBufferRef.current) {
                        if (debugKeystrokeLogging) {
                            debugLogger.debug('[DEBUG] Kitty buffer timeout, clearing:', kittySequenceBufferRef.current);
                        }
                        kittySequenceBufferRef.current = '';
                    }
                }
        ...
        const onPasteIdleTimeout = () => {
            ...
            if (idleFor >= PASTE_IDLE_TIMEOUT_MS) {
                ...
                pasteIdleTimeout = setTimeout(onPasteIdleTimeout, PASTE_IDLE_TIMEOUT_MS - idleFor);
            }
        };
        ...
        const createPrintableKey = (char) => {
            const printableName = char === ' '
                ? 'space'
                : /^[A-Za-z]$/.test(char)
                    ? char.toLowerCase()
                    : char;
        ...
        const parseKittyPrefix = (buffer) => {
            ...
            const csiUPrefix = new RegExp(`^${ESC}\\[(\\d+)(?::\\d+)*(?:;(\\d+)(?::\\d+)*)?(?:;(\\d+))?([u~])`);
        ...
        const getCompleteCsiSequenceLength = (buffer) => {
        ...
        const parsePlainTextPrefix = (buffer) => {
        ...
        const handleKeypress = async (_, key) => {
            ...
            const isCtrlCKey = (key.ctrl && key.name === 'c') ||
            ...
            if (isCtrlCKey) {
                ...
                if (kittySequenceBufferRef.current && debugKeystrokeLogging) {
                    debugLogger.debug('[DEBUG] Kitty buffer cleared on Ctrl+C:', kittySequenceBufferRef.current);
                }
            ...
            const isModifyOtherKeysSequence = key.sequence?.startsWith(`${ESC}[27`) ?? false;
            if (kittyProtocolEnabled ||
                ...
                if (kittySequenceBufferRef.current ||
                    ...
                    while (kittySequenceBufferRef.current) {
                        ...
                        if (parsed) {
                            if (debugKeystrokeLogging) {
                                ...
                                if (kittySequenceBufferRef.current.length > parsed.length) {
                                    debugLogger.debug('[DEBUG] Kitty sequence parsed successfully (prefix):', parsedSequence);
                                }
                                else {
                        ...
                        const completeUnsupportedCsiLength = getCompleteCsiSequenceLength(kittySequenceBufferRef.current);
                        if (completeUnsupportedCsiLength) {
                            if (debugKeystrokeLogging) {
                                debugLogger.debug('[DEBUG] Dropping unsupported complete CSI sequence:', kittySequenceBufferRef.current.slice(0, completeUnsupportedCsiLength));
                            }
                        ...
                        if (plainTextPrefix) {
                            if (debugKeystrokeLogging) {
                                debugLogger.debug('[DEBUG] Recovered plain text after kitty sequence:', plainTextPrefix.key.sequence);
                            }
                        ...
                        if (nextStart > 0) {
                            if (debugKeystrokeLogging) {
                                debugLogger.debug('[DEBUG] Skipping incomplete/invalid CSI prefix:', kittySequenceBufferRef.current.slice(0, nextStart));
                            }
                    ...
                    if (config?.getDebugMode() || debugKeystrokeLogging) {
                        const codes = Array.from(kittySequenceBufferRef.current).map((ch) => ch.charCodeAt(0));
                    ...
                    if (kittySequenceBufferRef.current.length > MAX_KITTY_SEQUENCE_LENGTH) {
                        if (debugKeystrokeLogging) {
                            debugLogger.debug('[DEBUG] Kitty buffer overflow, clearing:', kittySequenceBufferRef.current);
                        }
                        if (config) {
                            const event = new KittySequenceOverflowEvent(kittySequenceBufferRef.current.length, kittySequenceBufferRef.current);
            ...
            const composedKeyName = Object.hasOwn(OPTION_COMPOSED_GLYPHS, key.sequence)
        ...
        const shouldFlushRawDataAsPaste = (data) => {
        ...
        function partialMarkerTailLength(chunk, marker, minLen = 2) {
        ...
        const startRawPasteIdleTimeout = () => {
            clearRawPasteIdleTimeout();
            rawPasteIdleTimeout = setTimeout(forceFlushRawPaste, PASTE_IDLE_TIMEOUT_MS);
        };
        const broadcastPasteFromRaw = (content) => {
        ...
        const handleStdinData = (data) => {
            ...
            const buf = rawStdinTail.length > 0 ? Buffer.concat([rawStdinTail, raw]) : raw;
            rawStdinTail = Buffer.alloc(0);
            let cursor = 0;
            while (cursor < buf.length) {
                if (rawPasteAccumulating) {
                    ...
                    if (suffixIdx === -1) {
                        ...
                        const tailLen = partialMarkerTailLength(remaining, pasteModeSuffixBuf, 1);
                    ...
                    if (prefixIdx === -1) {
                        ...
                        if (remaining.length > 0) {
                            ...
                            const tailLen = partialMarkerTailLength(remaining, pasteModePrefixBuf);
                            if (tailLen > 0) {
                                const safe = remaining.subarray(0, remaining.length - tailLen);
                                if (safe.length > 0)
                                    keypressStream.write(safe);
                                rawStdinTail = Buffer.from(remaining.subarray(remaining.length - tailLen));
                            }
                            else {
    ...
    return (_jsx(KeypressContext.Provider, { value: {
            subscribe,
            unsubscribe,
            subscribeMouse,
            unsubscribeMouse,
