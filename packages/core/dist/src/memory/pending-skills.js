...
import { getProjectSkillsRoot, getPendingSkillsRoot, isProjectSkillPath, SKILL_FILE_NAME, } from '../skills/skill-paths.js';
 ...
 * exists on disk.
 */
export async function stageSkillDirs(touchedFiles, projectRoot, preExistingDirNames = new Set(), taskId = '') {
    const skillsRoot = getProjectSkillsRoot(projectRoot);
    const pendingRoot = getPendingSkillsRoot(projectRoot);
    const seen = new Set();
    const result = [];
    for (const file of touchedFiles) {
        if (!isProjectSkillPath(file, projectRoot))
            continue;
        if (path.basename(file) !== SKILL_FILE_NAME)
            continue;
        const skillDir = path.dirname(path.resolve(projectRoot, file));
        if (path.dirname(skillDir) !== path.resolve(skillsRoot))
        ...
        if (preExistingDirNames.has(dirName)) {
            ...
            debugLogger.debug(`Not staging "${dirName}": pre-existing skill edited in place.`);
            continue;
        }
        ...
        try {
            ...
            debugLogger.debug(`Not staging "${dirName}": ${err instanceof Error ? err.message : String(err)}`);
            continue;
        }
 ...
 */
export async function acceptPendingSkill(pending) {
    ...
    try {
        ...
        try {
            ...
            debugLogger.debug(`Accept no-op "${pending.name}": already in the skills library.`);
            return;
        }
        catch {
            throw new Error(`Cannot accept "${pending.name}": staged directory is missing and it is not in the skills root.`);
        }
    }
...
export async function rejectPendingSkill(pending) {
    const stagedDir = path.dirname(pending.stagedManifestPath);
    await fs.rm(stagedDir, { recursive: true, force: true });
    debugLogger.debug(`Discarded staged skill "${pending.name}".`);
}
//# sourceMappingURL=pending-skills.js.map