 ...
 */
const SED_ADDRESS = /^\s*(?:(?:\d+|\$)(?:\s*,\s*(?:\d+|\$))?|\/(?:\\[\s\S]|[^/\\])*\/)?\s*/;
const SED_ADDRESS_AT = /\s*(?:(?:\d+|\$)(?:\s*,\s*(?:\d+|\$))?|\/(?:\\[\s\S]|[^/\\])*\/)?\s*/y;
...
const AWK_STATIC_WRITE = /^\s*(?:print|printf)\b(?!\s*\()(?:(?:"(?:\\[\s\S]|[^"\\])*")|[^">|])*>>?\s*"[^"]*"\s*$/;
...
function classifySingleSedCommandSafety(script) {
    ...
    if (command === 's') {
        ...
        if (/[eErRwW]/.test(flags))
            return 'unknown';
        if (!SAFE_SUBSTITUTION_FLAGS.test(flags))
            return 'unknown';
        return compatibilityUnknown ? 'unknown' : 'read-only';
    }
    if (/[;\n{}]/.test(script.slice(commandOffset + 1)))
        return 'unknown';
    if (!SAFE_SED_COMMAND.test(command))
        return 'unknown';
    return compatibilityUnknown ? 'unknown' : 'read-only';
}
...
export function classifySedScriptSafety(script) {
    ...
    while (start < script.length) {
        ...
        if (/[eErR]/.test(command))
            return 'unknown';
        if (command !== 's' && !SAFE_SED_COMMAND.test(command))
            return 'unknown';
        let separator;
        if (command === 's') {
            const delimiter = script[commandOffset + 1];
        ...
        if (current === 'unknown')
            result = 'unknown';
        if (separator === script.length)
            return result;
        start = separator + 1;
    }
    return result;
}
export function classifySedCommandSafety(args) {
    const terminator = args.indexOf('--');
    const options = args.slice(0, terminator < 0 ? args.length : terminator);
    if (options.some((arg, index) => /^(?:--help|--version)$/i.test(arg) &&
    ...
    for (let i = 0; i < args.length; i++) {
        ...
        if (arg === '--')
            break;
        if (SED_VALUE_OPTIONS.includes(arg))
            i++;
        else if (/^-[nErsuz]*e.+/.test(arg))
            continue;
        else if (/^(?:-[nErsuz]*[iI]|--in-place(?:=|$))/.test(arg))
            return 'write';
    }
    const scripts = [];
    const scriptArguments = new Set();
    for (let i = 0; i < args.length; i++) {
        const arg = args[i];
        if (arg === '--') {
            if (scripts.length === 0) {
        ...
        if (/^(?:-l|--line-length)$/.test(arg) && !args[++i])
            return 'unknown';
        if (/^(?:-f|--file(?:=|$))/.test(arg))
            return 'unknown';
...
export function classifyAwkScriptSafety(script) {
    ...
    if (!ambiguousSlash &&
        ...
        return 'write';
    if (unsupportedAt || AWK_UNKNOWN_OPERATION.test(script))
        return 'unknown';
    return AWK_PRINT.test(script) && /[>|]/.test(script)
        ? 'unknown'
        : 'read-only';
}
export function classifyAwkCommandSafety(args) {
    let programIndex = -1;
    for (let i = 0; i < args.length; i++) {
        const arg = args[i];
        if (arg === '--') {
...
export function hasShellBraceExpansion(text) {
    let braceDepth = 0;
...
export function hasShellPatternExpansion(text) {
    return /[[*?]/.test(text) || hasShellBraceExpansion(text);
}
//# sourceMappingURL=shell-safety-rules.js.map