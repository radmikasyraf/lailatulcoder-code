...
import { createDebugLogger, ideContextStore, Storage, } from '@lailatul-coder/lailatul-coder-core';
import { findEnvFiles, preResolveHomeEnvOverrides } from './environment.js';
import { getSystemDefaultsPath, getSystemSettingsPath, getUserSettingsPath, loadSettings, } from './settings.js';
/**
 * Process-level cache for `loadSettings()`, keyed by workspace directory.
 *
 * The ACP child under `LailatulCoder serve` is long-lived and serves many sessions
 ...
 */
function settingsFileSigs(workspaceDir) {
    const filePaths = [
 ...
 */
function envFileSigs(settings, workspaceDir) {
    return findEnvFiles(settings.merged, workspaceDir).map((filePath) => ({
        filePath,
        sig: statSig(filePath),
    }));
}
function sameFileSigs(a, b) {
    if (a.length !== b.length) {
        return false;
    }
    return a.every((sig, i) => sig.filePath === b[i].filePath && sig.sig === b[i].sig);
}
function isEntryFresh(key, entry) {
    const fp = entry.fingerprint;
    return (ideContextStore.get()?.workspaceState?.isTrusted === fp.ideTrust &&
 ...
 */
export function loadSettingsCached(workspaceDir) {
    ...
    if (entry) {
        ...
        try {
            ...
            debugLogger.warn(`fingerprint check failed for ${key}; reloading:`, error);
        }
        if (isFresh) {
...
export function clearSettingsCacheForTesting() {
    cache.clear();
}
