...
const INSTALL_REASON = 'This downloads the Computer Use driver (~20MB, signed + notarized) into ~/.lailatulcoder/computer-use/ the first time. ' +
...
export function isHighRiskCall(upstreamName, params) {
    if (HIGH_RISK_TOOLS.has(upstreamName))
        return true;
    return (upstreamName === 'page' &&
        HIGH_RISK_PAGE_ACTIONS.has(params['action']));
}
class ComputerUseInvocation extends BaseToolInvocation {
    ...
    async getConfirmationDetails(_abortSignal) {
        ...
        const installApproved = await isPackageSpecApproved(homedir(), approvalKey());
        const onConfirm = async (outcome, _payload) => {
    ...
    async execute(signal, updateOutput) {
        ...
        client.setMaxImageDimension(resolveMaxImageDimension(this.config?.getComputerUseMaxImageDimension()));
        ...
        const llmContent = buildLlmContent(mcpResult.content, this.upstreamName, mcpResult.structuredContent);
...
export class ComputerUseTool extends BaseDeclarativeTool {
    ...
    constructor(upstreamName, schema, config) {
        ...
        super(qwenName, qwenName, // displayName == name; no MCP branding in UI
        schema.description, Kind.Other, schema.parameterSchema, true, // isOutputMarkdown — many results are JSON-ish text or screenshots
    ...
    validateToolParams(params) {
        const coerced = coerceTypes(params, this.parameterSchema);
 ...
 */
export function coerceTypes(params, schema) {
    ...
    for (const [key, value] of Object.entries(result)) {
        ...
        if ((fieldType === 'integer' || fieldType === 'number') &&
            ...
            const matchesType = fieldType === 'integer'
                ? /^-?\d+$/.test(trimmed)
                : /^-?\d+(\.\d+)?$/.test(trimmed);
            if (matchesType) {
 ...
 */
export function buildLlmContent(content, toolName, structuredContent) {
    const parts = [];
 ...
 */
export function buildDisplayText(content) {
    return content
 ...
 */
export function stringifyStructured(structured) {
