 ...
 * suppressed.
 */
function isRestartRequiredKey(changedPath) {
    ...
    for (let i = parts.length; i > 0; i--) {
        ...
        if (definition)
            return definition.requiresRestart === true;
    }
    return false;
}
/**
 ...
 * in-memory state.
 *
 * Uses chokidar to monitor the `.lailatulcoder` directory (depth: 0) with strict
 ...
 *
 * The watcher never creates `.lailatulcoder` itself. When the directory is missing at
 * startup it bootstrap-watches the parent (depth: 0, `.lailatulcoder`-only filter) and
 * promotes to watching `.lailatulcoder` once it appears — so a `settings.json` added
...
export class SettingsWatcher {
     ...
     * for the missing `.lailatulcoder` dir to appear; `target` watches `.lailatulcoder` itself.
     */
    watchStage = new Map();
    /**
     ...
     */
    watchGeneration = new Map();
    changeListeners = new Set();
    refreshTimer = null;
    pendingScopeChanges = new Set();
    processing = false;
    started = false;
    ...
    startWatching() {
        ...
        for (const { scope, settingsPath } of this.getScopePaths()) {
            ...
            // Watch `.lailatulcoder` directly when it already exists; otherwise bootstrap on
            // the parent and promote once `.lailatulcoder` appears. We never create the
    ...
    /**
     * Watches the resolved `.lailatulcoder` directory for changes to `settings.json`.
     * If `.lailatulcoder` itself is removed, demotes back to a parent bootstrap watcher
     * so a later re-creation is still caught.
     */
    watchTargetDir(scope, settingsPath) {
        const dir = path.dirname(settingsPath);
        ...
        try {
            const watcher = watchFs(dir, {
                ...
                if (this.watchGeneration.get(scope) !== gen)
                    return;
                // The `.lailatulcoder` directory itself was removed — demote so we can catch
                ...
                if (path.basename(changedPath) !== targetBasename)
                    return;
            ...
            debugLogger.warn(`Failed to start settings watcher for ${scope} (${dir}):`, error);
        }
    }
    /**
     * Bootstrap watcher: monitors the parent directory (depth 0) with a strict
     * predicate that only allows the `.lailatulcoder` entry through, so unrelated
     * top-level churn is suppressed and the project tree is never recursed.
     * Promotes to a target watcher once `.lailatulcoder` appears.
     */
    watchParentForDir(scope, settingsPath) {
        const dir = path.dirname(settingsPath);
        ...
        try {
            ...
            debugLogger.warn(`Failed to start settings bootstrap watcher for ${scope} (${parentDir}):`, error);
            return;
        }
        // Close the TOCTOU gap: `.lailatulcoder` may have been created between the
        // existence check and the watcher arming (bootstrap uses ignoreInitial).
        if (fs.existsSync(dir)) {
            void this.promoteScope(scope, settingsPath);
        }
    }
    /** Swaps a scope's bootstrap watcher for a target watcher on `.lailatulcoder`. */
    async promoteScope(scope, settingsPath) {
        if (this.watchStage.get(scope) !== 'bootstrap')
            return;
        await this.replaceWatcher(scope);
        if (!this.started)
            return;
        this.watchTargetDir(scope, settingsPath);
        // Pick up a settings.json that already exists inside the new `.lailatulcoder`.
        this.scheduleRefresh(scope);
    }
    /** Swaps a scope's target watcher back to a parent bootstrap watcher. */
    async demoteScope(scope, settingsPath) {
        if (this.watchStage.get(scope) !== 'target')
            return;
        await this.replaceWatcher(scope);
        if (!this.started)
            return;
    ...
    async handleChange(changedScopes) {
        const events = [];
        for (const scope of changedScopes) {
            ...
            const hasHotReloadableChange = changedKeys.some((key) => !isRestartRequiredKey(key));
            ...
            events.push({
                scope,
                path: file.path,
                changeType: !existedBefore && existsNow
                    ? 'created'
    ...
    async notifyListeners(events) {
        const TIMEOUT_MS = SettingsWatcher.LISTENER_TIMEOUT_MS;
        const withTimeout = (p) => {
            let timerId;
            const timeoutPromise = new Promise((_, reject) => {
                timerId = setTimeout(() => reject(new Error(`settings change listener timeout after ${TIMEOUT_MS}ms`)), TIMEOUT_MS);
        ...
        const results = await Promise.allSettled(Array.from(this.changeListeners).map((listener) => withTimeout(Promise.resolve().then(() => listener(events)))));
...
//# sourceMappingURL=settingsWatcher.js.map