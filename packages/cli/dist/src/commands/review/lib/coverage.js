...
import { readRunTranscripts, wasGivenTheDiff, TranscriptsUnavailableError, } from './transcripts.js';
import { readRecordedPrompts, wasDeliveredVerbatim, briefPath, findingsPointerOf, findingsFilePath, recordedPromptPath, } from './prompt-record.js';
import { declaresOwnUncoverable, openedBrief, readFindingsPointer, } from './certification.js';
import { requiredAgents, } from './roster.js';
...
export function assignedChunk(rec) {
 ...
 */
export function pointedAt(prompt, plan) {
    const out = [];
    ...
    for (const m of prompt.matchAll(re)) {
        ...
        if (limit > 0)
            out.push([offset + 1, offset + limit]);
    }
    if (out.length > 0)
        return out;
...
function publicRoleLabelZh(req) {
    if (req.role === 'chunk')
        return undefined;
 ...
 */
export function coverageFromTranscripts(planPath, env = process.env) {
    ...
    const allRecords = readRunTranscripts(planPath, mtimeMs, env, plan.diffPathAbsolute, { currentDirOptional: true });
    ...
    const disclose = (subject, reason, pub) => {
        disclosures.push({
            subject,
            reason,
    ...
    const chunkSatisfied = (c, self, only = () => true) => {
        const b = builtOf(`chunk-${c}`);
        if (b === undefined)
            return false;
        return records.some((r) => r !== self &&
            only(r) &&
    ...
    const keySatisfied = (rec, only = () => true) => {
        for (const key of built.keys()) {
            const b = builtOf(key);
            if (b === undefined)
                continue;
            if (!wasDeliveredVerbatim(rec.launchPrompt, b))
                continue;
            if (records.some((r) => r !== rec &&
                only(r) &&
     ...
     */
    const supersededByCurrent = (rec, chunk) => {
        const current = (r) => r.fromPriorSession !== true;
        return chunk !== null
    ...
    const gapsSuperseded = (rec, chunk) => {
        if (chunk !== null) {
            const b = builtOf(`chunk-${chunk}`);
            if (b === undefined)
                return false;
            return records.some((r) => r !== rec &&
    ...
    for (const rec of records) {
        ...
        if (chunk !== null) {
            ...
            if (b === undefined) {
                ...
                if (!nothingBuiltAtAll && !superseded(rec, chunk)) {
                    rewrittenPrompts.push(disclose(name, 'ran on a prompt the run wrote itself (none was built for this ' +
                        'chunk), so the brief with its method and rules never reached it', {
                        reasonZh: '运行在这次 run 自行编写的 prompt 上（该 chunk 从未构建过 ' +
                ...
                if (openedBriefOf(rec, `chunk-${chunk}`) && rec.diffToolCalls > 0) {
                    if (!superseded(rec, chunk)) {
                        driftedLaunches.push(`${name} — launched with a near-verbatim prompt; its brief was ` +
                    ...
                    rewrittenThisRecord = true;
                    if (!superseded(rec, chunk)) {
                        rewrittenPrompts.push(disclose(name, 'launched with a prompt that is not the one the CLI built', { reasonZh: '启动时使用的 prompt 不是 CLI 构建的那一份' }));
                    }
                }
            }
        }
        ...
        if (chunk !== null && declaresOwnUncoverable(rec, chunk)) {
            ...
            if (!chunkSatisfied(chunk, rec, (r) => !declaresOwnUncoverable(r, chunk))) {
    ...
    if (nobodyBuiltAnything) {
        ...
        missingRoles.push(disclose('every dimension', `none of the ${roster.length} required agents is on record as ` +
            ...
            subjectZh: '所有维度',
            reasonZh: `${roster.length} 个必需 agent 中没有任何一个有记录表明是用本 ` +
    ...
    const openedOfReq = buildable.map((req, i) => candidatesOf[i].filter((r) => openedBriefOf(r, req.key)));
    const matchedRec = new Map();
    ...
    for (const [rec, i] of matchedRec)
        assignment.set(i, rec);
    ...
    for (const req of roster) {
        ...
        if (b === undefined) {
            if (!nobodyBuiltAnything) {
                missingRoles.push(disclose(roleLabel(req), 'no record shows its brief reaching an agent, so this dimension ' +
                    'was reviewed, if at all, from a prompt the run wrote for itself', {
                    subject: publicRoleLabel(req),
                    subjectZh: publicRoleLabelZh(req),
                    reasonZh: '没有记录表明它的 brief 到达过任何 agent，这个维度即便被审查' +
        ...
        if (pick === undefined) {
            ...
            if (!anyMatch) {
                ...
                const rescue = records.find((r) => !matchedRec.has(r) &&
                    !rescued.has(r) &&
                ...
                if (rescue !== undefined) {
                    ...
                    if (req.role !== 'chunk') {
                        driftedLaunches.push(`${roleLabel(req)} — no launch matched its block verbatim, ` +
            ...
            missingRoles.push(disclose(roleLabel(req), anyMatch
        ...
        if (!openedBrief(pick, planPath, req.key)) {
            ...
            unreadBriefs.push(disclose(roleLabel(req), `never opened its brief (${brief}), so it reviewed without the ` +
                'instructions it was launched to follow', {
                subject: publicRoleLabel(req),
                reason: 'never opened its brief, so it reviewed without the ' +
                    'instructions it was launched to follow',
                subjectZh: publicRoleLabelZh(req),
    ...
    const missingChunks = planned.filter((id) => !covered.has(id) && !uncoverable.has(id));
    ...
    const recoveredAgents = records.filter((r) => r.fromPriorSession &&
        certifies(r) &&
        ...
        !supersededByCurrent(r, assignedChunk(r))).length;
    return {
        ok: blindAgents.length === 0 &&
            idleAgents.length === 0 &&
 ...
 */
const rebuildFix = (role, noun) => `build the prompt with \`"\${QWEN_CODE_CLI:-LailatulCoder}" review agent-prompt ` +
...
const REVERSE_AUDIT_GAP = {
    ...
    'not-built': {
        gap: 'no auditor was launched with a prompt this skill builds — the pass ' +
            'that hunts what the rest of the review missed ran, if at all, without ' +
            'the method its brief carries',
        gapZh: '没有审计 agent 是用本 skill 构建的 prompt 启动的——负责搜寻评审其余部分' +
    ...
    'not-launched': {
        gap: 'its prompt was built, but no agent was launched with it — the pass ' +
            'that hunts what the rest of the review missed ran, if at all, without ' +
            'the method its brief carries, and cannot be certified',
        gapZh: '它的 prompt 已构建，但没有 agent 用它启动——负责搜寻评审其余部分遗漏' +
    ...
    rewritten: {
        gap: 'an auditor ran and opened its brief, but no agent was launched with the ' +
        ...
        gapZh: '有审计 agent 运行并打开了自己的 brief，但没有 agent 是用 CLI 构建的 ' +
            'prompt 启动的——启动 prompt 是手写的，agent 实际被要求做的并不是本 ' +
            'skill 所认证的内容',
        fix: rebuildFix('reverse-audit', 'round'),
    },
    'brief-unread': {
        gap: 'it was launched with the built prompt but never opened its brief, so it ' +
            'audited without the gaps-only method and the finding format it was ' +
            'launched to follow',
        gapZh: '它用构建的 prompt 启动，却从未打开自己的 brief，审计时缺失了只报缺口的' +
            '方法和它本应遵循的发现格式',
        fix: 'relaunch with the same printed prompt — the agent must OPEN the brief ' +
            'file the prompt names; that read is the receipt',
    },
    'findings-unread': {
        gap: 'it was launched with the built prompt and opened its brief, but never ' +
            'read the findings file the prompt points at, so it audited without ' +
            'the confirmed list it was launched against',
        gapZh: '它用构建的 prompt 启动并打开了自己的 brief，却从未读取 prompt 所指向的 ' +
...
const VERIFY_GAP = {
    ...
    'not-built': {
        gap: 'the review posts findings, but no verifier was launched with a prompt ' +
            'this skill builds — they were ruled on, if at all, without the verdict ' +
            'bar its brief carries',
        gapZh: '本次评审发布了发现，但没有验证 agent 是用本 skill 构建的 prompt 启动的' +
            '——这些发现即便被裁定过，也缺失了 brief 承载的裁定标准',
        fix: rebuildFix('verify', 'shard'),
    },
    'not-launched': {
        gap: 'its prompt was built, but no agent was launched with it, so the posted ' +
            'findings cannot be counted as verified',
        gapZh: '它的 prompt 已构建，但没有 agent 用它启动，发布的发现不能算作已验证',
        fix: rebuildFix('verify', 'shard'),
    },
    rewritten: {
        gap: 'a verifier ran and opened its brief, but no agent was launched with the ' +
            'prompt the CLI built — the launch was written by hand, and the posted ' +
            'findings cannot be counted as verified against it',
        gapZh: '有验证 agent 运行并打开了自己的 brief，但没有 agent 是用 CLI 构建的 ' +
    ...
    'brief-unread': {
        ...
        gapZh: '它用构建的 prompt 启动，却从未打开自己的 brief，裁定发现时缺失了它本应' +
            '使用的裁定标准',
        fix: 'relaunch with the same printed prompt — the agent must OPEN the brief ' +
            'file the prompt names; that read is the receipt',
    },
 ...
 */
const COMBINED_STEP45_GAP = {
    'not-built': {
        en: 'neither the verifier nor the reverse auditor was launched with a prompt ' +
        ...
        zh: '验证 agent 与反向审计 agent 都没有用本 skill 构建的 prompt 启动——发布的' +
            '发现即便被裁定过、评审其余部分遗漏的问题即便被搜寻过，也都缺失了本 ' +
            'skill 用以认证的 brief',
    },
    'not-launched': {
        en: 'both prompts were built, but no agent was launched with either — the ' +
        ...
        zh: '两份 prompt 都已构建，但都没有 agent 用它们启动——发布的发现不能算作已' +
            '验证，搜寻评审遗漏问题的工序也无法作证',
    },
    rewritten: {
        en: 'each ran and opened its brief, but neither was launched with the prompt ' +
        ...
        zh: '两者都运行并打开了各自的 brief，但都不是用 CLI 构建的 prompt 启动的——' +
            '启动 prompt 是手写的，发布的发现不能算作已验证，agent 实际被要求做的也' +
            '不是本 skill 所认证的内容',
    },
    'brief-unread': {
        en: 'each was launched with its built prompt and never opened its brief, so ' +
        ...
        zh: '两者都用构建的 prompt 启动，却都从未打开自己的 brief——发现的裁定缺失了' +
            '裁定标准，审计也缺失了它本应遵循的只报缺口的方法',
    },
    'findings-unread': {
        en: 'each was launched with its built prompt and opened its brief, but never ' +
        ...
        zh: '两者都用构建的 prompt 启动并打开了各自的 brief，却都未读取 prompt 所指向' +
 ...
 */
export function verificationGaps(planPath, opts, env = process.env) {
    const { plan, mtimeMs } = readPlan(planPath);
    ...
    const records = liveRecords(readRunTranscripts(planPath, mtimeMs, env, plan.diffPathAbsolute, {
    ...
    const deliveryOf = (key) => {
        ...
        const opened = (r) => openedBrief(r, planPath, key);
        const findingsPointer = findingsPointerOf(b);
        const readTheFindings = (r) => readFindingsPointer(r, findingsPointer);
        const gotTheBuiltPrompt = records.filter((r) => wasDeliveredVerbatim(r.launchPrompt, b));
        if (gotTheBuiltPrompt.some((r) => opened(r) && readTheFindings(r))) {
            return 'ok';
        }
        if (gotTheBuiltPrompt.some(opened))
            return 'findings-unread';
        if (gotTheBuiltPrompt.length > 0)
            return 'brief-unread';
     ...
     * verdict stricter.
     */
    const currentDigestKeys = (planPath, keys) => {
    ...
    const reverseKeys = [...built.keys()].filter((k) => k === 'reverse-audit' || k.startsWith('reverse-audit--'));
    ...
    if (reverseGap) {
        ...
        remediation.push(`reverse audit: ${REVERSE_AUDIT_GAP[reverse].fix.replace('--plan <plan>', () => `--plan ${shellQuotePath(planPath)}`)}`);
    }
    ...
    if (opts.postsFindings) {
        ...
        const verifyKeys = [...built.keys()].filter((k) => k === 'verify' || k.startsWith('verify--'));
        verify = bestDelivery(currentDigestKeys(planPath, verifyKeys));
        if (verify !== 'ok') {
            unverifiedFindings = true;
            remediation.push(`verification: ${VERIFY_GAP[verify].fix.replace('--plan <plan>', 
    ...
    if (balancedMedium) {
        gaps.push({
            subject: 'reverse audit',
            reason: 'not run — the balanced (medium) tier skips the second-look pass, so ' +
