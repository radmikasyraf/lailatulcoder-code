...
import { DuplicateWorkspaceInputError, MissingWorkspaceInputError, MultipleWorkspaceInputError, NestedWorkspaceInputError, resolveSingleWorkspaceInput, resolveWorkspaceInputs, } from './workspace-inputs.js';
let scratch;
...
describe('resolveSingleWorkspaceInput', () => {
    ...
    it('rejects an explicit empty workspace array', () => {
        expect(() => resolveSingleWorkspaceInput([])).toThrow(MissingWorkspaceInputError);
    });
    it('rejects duplicate canonical explicit workspaces', () => {
        const root = makeScratch();
        const workspace = fs.realpathSync(path.join(root));
        expect(() => resolveSingleWorkspaceInput([workspace, workspace])).toThrow(DuplicateWorkspaceInputError);
    });
    it('rejects nested explicit workspaces in either order', () => {
        ...
        expect(() => resolveSingleWorkspaceInput([parent, child])).toThrow(NestedWorkspaceInputError);
        expect(() => resolveSingleWorkspaceInput([child, parent])).toThrow(NestedWorkspaceInputError);
        expect(() => resolveSingleWorkspaceInput([parent, dotPrefixedChild])).toThrow(NestedWorkspaceInputError);
    });
    it('rejects distinct non-nested explicit workspaces for single-workspace callers', () => {
        ...
        expect(() => resolveSingleWorkspaceInput([primary, secondary])).toThrow(MultipleWorkspaceInputError);
    });
    it('lets single-workspace callers reject multiple inputs even if early canonicalization fails', async () => {
        const canonicalizationError = Object.assign(new Error('permission denied'), { code: 'EACCES' });
        vi.resetModules();
        ...
        try {
            const { MultipleWorkspaceInputError, resolveSingleWorkspaceInput } = await import('./workspace-inputs.js');
            expect(() => resolveSingleWorkspaceInput(['/inaccessible', '/other'])).toThrow(MultipleWorkspaceInputError);
        }
        finally {
...
describe('resolveWorkspaceInputs', () => {
    ...
    it('still rejects duplicate and nested explicit workspaces', () => {
        ...
        const child = path.join(parent, 'child');
        fs.mkdirSync(child, { recursive: true });
        expect(() => resolveWorkspaceInputs([parent, parent])).toThrow(DuplicateWorkspaceInputError);
        expect(() => resolveWorkspaceInputs([parent, child])).toThrow(NestedWorkspaceInputError);
    });
});
//# sourceMappingURL=workspace-inputs.test.js.map