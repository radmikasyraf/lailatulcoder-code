...
import { accessSync, closeSync, constants as fsConstants, lstatSync, openSync, statSync, } from 'node:fs';
...
import { BoundedOutputRing, launchAgentViewPtyHost, } from './pty-host.js';
...
export const INTERNAL_AGENT_VIEW_PTY_HOST_ARG = '--internal-agent-view-pty-host';
...
const ALLOWED_KILL_SIGNALS = new Set([
    'SIGINT',
    'SIGKILL',
    'SIGTERM',
]);
...
export async function launchAgentViewPtyHostProcess(launch, options = {}) {
    ...
    const child = (options.spawnProcess ?? defaultSpawnPtyHost)([INTERNAL_AGENT_VIEW_PTY_HOST_ARG, launchPath, socketPath], { [PTY_HOST_AUTH_TOKEN_ENV]: authToken }, stderrLogPath);
    child.unref?.();
    let status;
    try {
...
export async function connectAgentViewPtyHostProcess(launch, socketPath, authToken, options = {}) {
    const status = await waitForPtyHost(socketPath, options.readyRetries ?? CONNECT_HOST_READY_RETRIES, authToken, {
        requestTimeoutMs: options.requestTimeoutMs ?? HOST_READY_REQUEST_TIMEOUT_MS,
    });
    return createRemotePtyHostHandle({
        socketPath,
        launch,
...
function createRemotePtyHostHandle({ socketPath, launch, authToken, pid, workerPid, child, }) {
    ...
    return {
        ...
        write(data) {
            if (!attachSocket) {
                throw new Error('Agent View PTY host input requires an active attach stream.');
            }
        ...
        onData(callback) {
            ...
            const onData = (textChunk) => {
                ...
                buffer += textChunk;
                if (Buffer.byteLength(buffer, 'utf8') > MAX_PTY_HOST_RESPONSE_LINE_BYTES) {
                    socket.destroy(new AgentViewPtyHostProtocolError('Agent View PTY host response line is too large.'));
                    return;
                }
...
function createChildExitTracker(child) {
    let resolveExit = () => { };
...
function createRemoteExitTracker(socketPath, authToken) {
    let settled = false;
...
export async function runAgentViewPtyHostProcess({ launchPath, socketPath, authToken, loadPty, }) {
...
export function getAgentViewPtyHostSocketPath(sessionId, options = {}) {
    const platform = options.platform ?? process.platform;
    const digest = shortHash(`${options.globalDir ?? ''}:${sessionId}:pty-host`);
    if (platform === 'win32') {
        return `\\\\.\\pipe\\LailatulCoder-agent-pty-${digest}`;
    }
    ...
    const fallbackCandidates = [
        path.join(os.tmpdir(), `LailatulCoder-avp-${uid}`, `${digest}.sock`),
        path.join('/tmp', `LailatulCoder-avp-${uid}`, `${digest}.sock`),
    ];
    const fallback = fallbackCandidates.find((item) => Buffer.byteLength(item) < UNIX_SOCKET_PATH_LIMIT &&
        canPrepareSocketDirectory(path.dirname(item)));
    const lengthFallback = fallbackCandidates.find((item) => Buffer.byteLength(item) < UNIX_SOCKET_PATH_LIMIT);
    if (fallback) {
        return fallback;
    }
    if (lengthFallback) {
        ...
        throw new Error('Agent View PTY host socket fallback directories are not writable.');
    }
    throw new Error('Agent View PTY host socket path is too long.');
}
async function callAgentViewPtyHost(socketPath, authToken, op, params, timeoutMs) {
    ...
    throw new AgentViewPtyHostRequestError(response.error.code, response.error.message);
}
...
async function requestAgentViewPtyHost(socketPath, request, options = {}) {
    return new Promise((resolve, reject) => {
        const socket = net.createConnection(socketPath);
        let buffer = '';
        let settled = false;
        const timeout = setTimeout(() => {
            finish(undefined, new Error('Timed out waiting for Agent View PTY host.'));
            socket.destroy();
        ...
        socket.on('data', (chunk) => {
            buffer += chunk;
            if (Buffer.byteLength(buffer, 'utf8') > MAX_PTY_HOST_RESPONSE_LINE_BYTES) {
                finish(undefined, new AgentViewPtyHostProtocolError('Agent View PTY host response line is too large.'));
                socket.destroy();
                return;
            }
            const newline = buffer.indexOf('\n');
...
export function createAgentViewPtyHostServer(host, socketPath, options = {}) {
    const attachState = {
    ...
    const server = net.createServer((socket) => {
        ...
        socket.on('data', (chunk) => {
            ...
            socket.pause();
            void respondToHostLine(host, line, socket, attachState, leftover, options.authToken, options.shutdownGraceMs).catch(() => {
                socket.destroy();
            });
        });
    });
            ...
            try {
                ...
                if (!isWindowsPipePath(socketPath)) {
                    ...
                    if (lockContent !== String(process.pid)) {
                        ...
                        const displaced = new Error(`Agent View PTY host socket is already in use: ${socketPath}`);
...
async function respondToHostLine(host, line, socket, attachState, leftover = Buffer.alloc(0), authToken, shutdownGraceMs) {
    const request = parseHostRequest(line);
    if (!request) {
        socket.end(`${JSON.stringify(errorResponse('', 'invalid_json', 'Invalid JSON.'))}\n`);
        return;
    }
    if (authToken && !isValidAuthToken(request.authToken, authToken)) {
        socket.end(`${JSON.stringify(errorResponse(request.id, 'unauthorized', 'Unauthorized PTY host request.'))}\n`);
        return;
    }
    if (request.op === 'attachStream') {
        if (attachState.activeAttachSocket?.destroyed === false) {
            socket.end(`${JSON.stringify(errorResponse(request.id, 'already_attached', 'Agent View PTY host already has an attached stream.'))}\n`);
            return;
        }
    ...
    try {
        ...
        socket.end(`${JSON.stringify(errorResponse(request.id, 'internal_error', error instanceof Error ? error.message : 'PTY host request failed.'))}\n`);
    }
}
async function handleHostRequest(host, request, shutdownGraceMs) {
    switch (request.op) {
        case 'status':
            return {
                pid: process.pid,
        ...
        default: {
            const unknownOperation = request.op;
            throw new Error(`Unsupported PTY host operation: ${unknownOperation}`);
        }
    }
}
async function shutdownHost(host, graceMs = SHUTDOWN_GRACE_MS) {
    if (host.shutdown) {
        await host.shutdown();
    }
    else {
...
async function waitForSpawnedPtyHost(socketPath, child, authToken) {
    return new Promise((resolve, reject) => {
        let settled = false;
        ...
        const onExit = (code, signal) => {
            ...
            finishReject(new Error(`Agent View PTY host exited before ready (${suffix}).`));
        };
        const onError = (error) => {
            finishReject(error);
        };
        ...
        }).then((status) => finishResolve(status), (error) => {
...
async function waitForPtyHost(socketPath, retries = HOST_READY_RETRIES, authToken, options = {}) {
    ...
    const deadlineMs = Date.now() + retries * (HOST_READY_DELAY_MS + requestTimeoutMs);
    for (let attempt = 0; attempt < retries; attempt++) {
        if (options.signal?.aborted || Date.now() >= deadlineMs)
            break;
        try {
            const result = await callAgentViewPtyHost(socketPath, authToken, 'status', undefined, requestTimeoutMs);
            ...
            if (error instanceof AgentViewPtyHostProtocolError ||
...
function defaultSpawnPtyHost(args, env, stderrLogPath) {
...
async function withHostStderrTail(error, stderrLogPath) {
...
function parseHostRequest(line) {
    try {
...
function parseHostResponse(line) {
    let parsed;
    try {
        parsed = JSON.parse(line);
    }
    catch (error) {
        throw new AgentViewPtyHostProtocolError(error instanceof Error ? error.message : 'Invalid PTY host response.');
    }
    if (!isRecord(parsed) || typeof parsed['id'] !== 'string') {
        throw new AgentViewPtyHostProtocolError('Invalid Agent View PTY host response.');
    }
    if (parsed['ok'] === true) {
...
function isHostOperation(value) {
    return HOST_OPERATIONS.includes(value);
}
...
async function acquireSocketPathLock(socketPath) {
    ...
    const busy = new Error(`Agent View PTY host socket is already in use: ${socketPath}`);
...
async function prepareSocketPath(socketPath) {
    ...
    if (await canConnect(socketPath)) {
        const error = new Error(`Agent View PTY host socket is already in use: ${socketPath}`);
        error.code = 'EADDRINUSE';
        throw error;
    }
    await removeSocketPath(socketPath);
}
async function ensurePrivateSocketDirectory(socketDir) {
...
function canPrepareSocketDirectory(socketDir) {
    try {
...
async function removeOwnedSocketPath(socketPath) {
    if (isWindowsPipePath(socketPath))
        return;
    if (!(await socketPathExists(socketPath)))
        return;
...
function signalParam(params) {
    return killSignalValue(params?.['signal']);
}
function killSignalValue(value) {
    if (value === undefined || value === '')
        return undefined;
    if (typeof value === 'string' &&
        ALLOWED_KILL_SIGNALS.has(value)) {
        return value;
    }
    throw new Error('Agent View PTY host signal is not allowed.');
}
function isValidAuthToken(provided, expected) {
    if (!provided)
        return false;
...
//# sourceMappingURL=pty-host-process.js.map