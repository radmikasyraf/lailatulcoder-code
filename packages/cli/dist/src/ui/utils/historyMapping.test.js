...
import { CompressionStatus, SYSTEM_REMINDER_OPEN, SYSTEM_REMINDER_CLOSE, } from '@lailatul-coder/lailatul-coder-core';
// ---------------------------------------------------------------------------
// Helpers
// ---------------------------------------------------------------------------
...
function compressionItem(id, compressionStatus = CompressionStatus.COMPRESSED, compressionKind = 'summarize') {
    return {
        type: 'compression',
        id,
        compression: {
...
describe('computeApiTruncationIndex', () => {
    ...
    describe('with microcompaction media-clear placeholders', () => {
        ...
        function clearedMediaContent(mime = 'image/png') {
            return {
                role: 'user',
                parts: [{ text: `[Old inline media cleared: ${mime}]` }],
            };
        }
        ...
        it('still counts a genuine prompt that merely begins with the placeholder prefix', () => {
            ...
            const prefixPromptText = '[Old inline media cleared: image/png] why is this in my history?';
            const prefixPrompt = {
                role: 'user',
                parts: [{ text: prefixPromptText }],
            };
            const ui = [
        ...
        it('pins the exact-match collision corner as a loud block, not silent loss', () => {
            ...
            const collidingPrompt = {
                role: 'user',
                parts: [{ text: exactPlaceholderText }],
            };
            const ui = [
