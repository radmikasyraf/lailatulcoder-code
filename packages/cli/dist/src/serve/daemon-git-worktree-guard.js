...
import { GitWorktreeService, isWithinRoot, realpathNearestExistingAsync, splitCommands, } from '@lailatul-coder/lailatul-coder-core';
import { EXTERNAL_TOOL_GUARD_MAX_DENIAL_REASON_CHARS, SHELL_EXECUTING_TOOL_NAMES as SHELL_EXECUTING_TOOLS, } from '@lailatul-coder/acp-bridge/externalToolGuard';
...
const DYNAMIC_RELOCATION_DENIAL = 'Daemon shell guard denied a mutating Git command with a dynamic repository location.';
const UNPARSEABLE_COMMAND_DENIAL = 'Daemon shell guard denied a shell command that could not be parsed before execution.';
const UNRESOLVED_TARGET_DENIAL_PREFIX = 'Daemon shell guard denied a mutating Git command with an unresolvable repository location: ';
const OUTSIDE_TARGET_DENIAL_PREFIX = 'Daemon shell guard denied a mutating Git command outside the session working directory: ';
const UNDECIDABLE_PAYLOAD_DENIAL = 'Daemon shell guard denied a shell command whose payload could not be resolved before execution.';
const UNRECOGNIZED_PROGRAM_DENIAL = 'Daemon shell guard denied a shell command that may run a relocated Git command through an unrecognized program.';
const SHADOW_REMOVAL_DENIAL = 'Daemon shell guard denied a shell command that removes a tracked shell definition in a way it cannot model.';
const PROMPTLESS_PROVIDER_DENIAL = 'Managed external tool guard cannot consult an external provider without an active prompt binding.';
const MAX_PAYLOAD_RECURSION_DEPTH = 3;
const UNVERIFIABLE_SCOPE_DENIAL_PREFIX = 'Daemon shell guard could not establish the execution directory of this call: ';
...
const SUBSHELL_OPENING_OPERATORS = new Set([
    '(',
    '<(',
    '>(',
]);
...
function tokenizeSegment(segment, startDepth) {
    ...
    for (let index = 0; index < parsed.length; index++) {
        ...
        if (op === 'glob') {
            ...
            const pattern = 'pattern' in token && typeof token.pattern === 'string'
                ? token.pattern
                : '';
...
function expandShellLocals(token, shellLocals) {
    if (!token.dynamic || shellLocals.size === 0)
        return token;
    let resolved = true;
    const text = token.text.replace(/\$\{?([A-Za-z_][A-Za-z0-9_]*)\}?/g, (match, name) => {
...
function hasGitRelocationMarker(tokens) {
    return tokens.some((token) => {
        if (token.text === '-C' || token.text.startsWith('-C'))
            return true;
        if (/^--(?:git-dir|work-tree)(?:=|$)/.test(token.text))
            return true;
...
const TEXT_RELOCATION_MARKER_WITHOUT_C_PATTERN = /(^|\s)(--git-dir=?|--work-tree=?|-execdir)|(^|[\s;&|(){}])(cd|pushd)([\s;&|]|$)|(^|[\s;&|(){}])(GIT_DIR|GIT_WORK_TREE|GIT_COMMON_DIR|GIT_INDEX_FILE|GIT_EXEC_PATH|PATH)\+?=/;
const TEXT_RELOCATION_MARKER_PATTERN = /(^|\s)(-C|--git-dir=?|--work-tree=?|-execdir)|(^|[\s;&|(){}])(cd|pushd)([\s;&|]|$)|(^|[\s;&|(){}])(GIT_DIR|GIT_WORK_TREE|GIT_COMMON_DIR|GIT_INDEX_FILE|GIT_EXEC_PATH|PATH)\+?=/;
...
function recordEnvAssignment(token, state) {
    ...
    if (!GIT_DIR_ENV_KEYS.has(key) && !GIT_WORK_TREE_ENV_KEYS.has(key))
        return;
    ...
    while (index < run.length) {
        ...
        if (token.text === '-S' || token.text === '--split-string') {
            ...
            if (payloadToken === undefined)
                return { next: run.length };
            // Mirrors the `-c` payload rule: a payload the daemon cannot read is
            // undecidable, not absent.
            if (payloadToken.dynamic)
                return { next: run.length, undecidable: true };
...
function consumeSudoWrapper(run, start, state) {
    ...
    while (index < run.length) {
        ...
        if (SUDO_CHROOT_FLAGS.has(token.text) ||
...
function consumeShellWrapper(run, start) {
    ...
    while (index < run.length) {
        ...
        if (shellBundleRequestsCommand(token.text)) {
            ...
            let toSkip = shellBundleValueFlagsBeforeCommand(token.text) +
...
function findChdirTarget(run, start, variant) {
    let index = start;
    while (index < run.length) {
        const token = run[index];
        if (token.text === '--')
...
function requestsAllExport(run, start) {
    for (let index = start; index < run.length; index++) {
        ...
        if (token.dynamic)
            return true;
        if (text === '-o' || text === '--') {
            if (run[index + 1]?.text === 'allexport')
                return true;
            continue;
        }
        if (text === 'allexport' || text === '--allexport')
            return true;
        if (/^-[a-zA-Z]*a/.test(text))
            return true;
    }
    return false;
}
...
function disablesAllExport(run, start) {
    for (let index = start; index < run.length; index++) {
        ...
        if (text === '+o' && run[index + 1]?.text === 'allexport')
            return true;
        if (/^\+[a-zA-Z]*a/.test(text))
            return true;
    }
...
function readAliasDefinitions(run) {
    const body = runFromProgramWord(run);
    if (body.length === 0 || executableBaseName(body[0]) !== 'alias')
        return [];
    const definitions = [];
...
function analyzeRun(run) {
    ...
    while (index < run.length) {
        ...
        if (program === 'command' || program === 'builtin') {
            index++;
            while (index < run.length && run[index].text.startsWith('-'))
                index++;
            continue;
        }
        ...
        if (program === 'eval') {
            ...
            if (payloadTokens.some((payloadToken) => payloadToken.dynamic || payloadToken.ambiguousFd)) {
                return { kind: 'undecidable' };
            }
            return {
                kind: 'payload',
...
function readGitInvocation(tokens) {
    ...
    const pushRelocation = (kind, value, emptyIsNoop) => {
        ...
        if (kind === 'cwd')
            cwdTargets.push(value);
        else if (kind === 'git-dir')
            gitDirTargets.push(value);
        else
            workTreeTargets.push(value);
        return true;
    };
    let index = 1;
    while (index < tokens.length) {
        ...
        if (token.text === '-C') {
            ...
            if (!pushRelocation('cwd', tokens[index + 1], true))
                break;
            index += 2;
            continue;
        }
        if (token.text === '--git-dir' || token.text === '--work-tree') {
            ...
            if (!pushRelocation(kind, tokens[index + 1], false))
                break;
            index += 2;
            continue;
        }
        if (token.text.length > 2 && token.text.startsWith('-C')) {
            if (!pushRelocation('cwd', { text: token.text.slice(2), dynamic: false }, false)) {
                break;
            }
            index++;
            continue;
        }
        if (token.text.startsWith('--git-dir=') ||
        ...
        if (token.text === '-c' || token.text === '--config-env') {
            ...
            if (value.dynamic)
                ...
                recordConfigAssignment(value.text);
            index += 2;
            continue;
        }
...
async function resolveGitDirRepository(canonicalGitDir) {
    ...
    for (let depth = 0; depth < 3; depth++) {
        ...
        if (stats.isFile()) {
            ...
            if (!match)
                throw new Error('unrecognized gitfile');
        ...
        if (/[/\\]\.git[/\\]worktrees[/\\][^/\\]+$/.test(current)) {
            ...
            if (!worktreeGitPointer)
                throw new Error('empty worktree gitdir file');
...
async function resolveDiscoveredRepository(startDirectory, boundary) {
    ...
    for (let depth = 0; depth < 64; depth++) {
        ...
        if (exists) {
            return resolveGitDirRepository(await realpathNearestExistingAsync(candidate));
        }
        if (current === boundary)
            return undefined;
...
async function evaluateGitInvocation(invocation, state, basisCwd, entryCwd, context) {
    ...
    if (RELOCATED_READ_ONLY_GIT_SUBCOMMANDS.has(invocation.subcommand ?? '') &&
    ...
    for (const relocation of [
        ...
        if (relocation.kind === 'cwd')
            cwdRelocations.push(relocation);
        else
            repositoryRelocations.push(relocation);
    }
    ...
    const relocated = basisCwd === undefined ||
        basisCwd !== entryCwd ||
    ...
    for (const relocation of cwdRelocations) {
        if (gitCwd === undefined && !path.isAbsolute(relocation.target))
            break;
...
async function denyOutsideDiscoveredRepository(startDirectory, context) {
    ...
    let discovered;
    try {
        discovered = await resolveDiscoveredRepository(canonicalStart, context.canonicalEffectiveCwd);
    }
    catch {
...
async function evaluateUnrecognizedRun(run, state, basisCwd, context, relink) {
    ...
    const gitIndex = run.findIndex((token) => executableBaseName(token) === 'git');
    ...
    if ((!ownsCFlag && hasGitRelocationMarker(run)) ||
        run.some((token) => (ownsCFlag
...
function readTopLevelSeparators(command) {
    ...
    for (let index = 0; index < command.length; index++) {
        ...
        if (single || double || backtick || substitution > 0)
            continue;
        if (character === '&' && next === '&') {
            separators.push('&&');
            index++;
...
async function evaluateCommandWithCwd(command, startCwd, entryCwd, context, depth, scope = { relink: { targets: [], gitDir: false } }) {
    ...
    const shellLocals = scope.locals ?? new Map();
    ...
    const definedBodies = scope.definedBodies ?? new Map();
    ...
    const exportedNames = scope.exportedNames ?? new Set();
    ...
    const gitShapedNames = scope.gitShapedNames ?? new Set();
    const exportedFunctions = scope.exportedFunctions ?? new Set();
    ...
    const activeContext = () => exported.relocations.length > 0 || exported.unresolved
        ? {
    ...
    const restoreShellState = (snapshot) => {
        ...
        shellLocals.clear();
        for (const [key, token] of snapshot.locals)
    ...
    const invokeDefinedBody = async (programToken, run) => {
        ...
        const programIndex = run.findIndex((token) => !token.redirect && !token.ambiguousFd && token.text === programToken);
        if (defined.alias) {
        ...
        const bodyContext = prefix.relocations.length > 0 || prefix.unresolved
            ...
            : base;
        const nested = await evaluateCommandWithCwd(replay, trackedCwd, entryCwd, bodyContext, depth + 1, {
            relink: scope.relink,
            locals: shellLocals,
            exportedNames,
            allExport,
    ...
    const isPipeComponent = (index) => separatorsMatch
    ...
    for (const [segmentIndex, segment] of segments.entries()) {
        ...
        const functionHeader = /^\s*([A-Za-z_][A-Za-z0-9_]*)\s*\(\s*\)/.exec(segment) ??
        ...
        for (const body of substitutions) {
            ...
            const nested = await evaluateCommandWithCwd(body, trackedCwd, entryCwd, activeContext(), depth + 1, 
        ...
        for (const { tokens: run, depth: runDepth } of runs) {
            ...
            let removalStart = 0;
            const skipRedirectOperands = () => {
                while (removalStart < run.length &&
            ...
            while (removalStart < run.length &&
                ...
                if (definedBodies.has(run[removalStart].text))
                    break;
            ...
            const shadowedBuiltin = !hasCommandPrefix &&
                removalProgram !== undefined &&
                definedBodies.has(removalProgram);
            const isRemoval = !shadowedBuiltin &&
                (removalProgram === 'unset' ||
                    removalProgram === 'unalias' ||
                    (removalProgram === 'export' &&
                        removalTokens.some((token) => /^-[A-Za-z]*n/.test(token.text))));
            if (isRemoval) {
                const clearsAll = removalTokens.some((token) => /^-[A-Za-z]*a/.test(token.text));
                const touchesShadow = (name) => definedBodies.has(name) ||
                    gitShapedNames.has(name) ||
                    exportedFunctions.has(name);
                const anyShadow = definedBodies.size > 0 ||
                    gitShapedNames.size > 0 ||
                    exportedFunctions.size > 0;
                const retractsShadow = (clearsAll && anyShadow) ||
                    removalTokens
                        .slice(1)
                        .some((token) => !token.text.startsWith('-') &&
            ...
            if (invoked !== undefined &&
                ...
                if (denial)
                    return { denial, cwdAfter: trackedCwd };
                continue;
            }
            const analysis = analyzeRun(run);
            switch (analysis.kind) {
                case 'cd': {
                    const target = analysis.target === undefined
                        ? undefined
                ...
                case 'payload': {
                    ...
                    const nested = await evaluateCommandWithCwd(analysis.payload, trackedCwd, entryCwd, ambient, depth + 1, {
                        relink: scope.relink,
                        ...
                        ...(analysis.propagatesCwd
                            ...
                            : analysis.importsExportedFunctions
                                ? {
                                    ...
                                    definedBodies: new Map([...definedBodies].filter(([name]) => exportedFunctions.has(name) &&
                                        !envUnsetRemovesFunction(name, analysis.state))),
                                    exportedFunctions: new Set([...exportedFunctions].filter((name) => !envUnsetRemovesFunction(name, analysis.state))),
                                }
                                : {}),
                    });
                    if (nested.denial) {
                ...
                case 'git': {
                    ...
                    ].map((target) => trackedCwd === undefined
                        ? target.text
                        : path.resolve(trackedCwd, target.text));
                    if (trackedCwd !== undefined && trackedCwd !== entryCwd) {
                        resolvedRelocations.push(trackedCwd);
                    }
                    if (scope.relink.gitDir ||
                        resolvedRelocations.some((target) => relinkedTargets.some((relinked) => target === relinked ||
                    ...
                    const denial = await evaluateGitInvocation(invocation, analysis.state, trackedCwd, entryCwd, activeContext());
                    if (denial)
                        return { denial, cwdAfter: trackedCwd };
                    break;
                }
                case 'dynamic-program': {
                    const inherited = activeContext();
                    const expanded = analysis.rest.map((token) => expandShellLocals(token, shellLocals));
                    ...
                    for (const operand of expanded) {
                        if (operand.text.startsWith('-') || operand.dynamic)
                            continue;
                    ...
                    if (analysis.state.unresolved ||
                        ...
                        expanded.some((token) => TEXT_RELOCATION_MARKER_PATTERN.test(token.text))) {
                        return { denial: denyDynamicRelocation(), cwdAfter: trackedCwd };
                    }
                    ...
                    if (trackedCwd === undefined ||
                        !isWithinRoot(await realpathNearestExistingAsync(trackedCwd), inherited.canonicalEffectiveCwd)) {
                        return { denial: denyDynamicRelocation(), cwdAfter: trackedCwd };
                    }
                    const denial = await evaluateUnrecognizedRun(expanded, analysis.state, trackedCwd, inherited, scope.relink);
                    if (denial)
                        return { denial, cwdAfter: trackedCwd };
                    break;
                }
                ...
                case 'export': {
                    ...
                    for (const operand of analysis.operands) {
                        if (leadingEnvAssignmentKey(operand.text) !== null)
                            continue;
                        if (operand.dynamic) {
                        ...
                        if (pending)
                            recordEnvAssignment(pending, exported);
                        if (GIT_DIR_ENV_KEYS.has(operand.text) ||
                    ...
                    const denial = await evaluateUnrecognizedRun(run, analysis.state, trackedCwd, activeContext(), scope.relink);
                    if (denial)
                        return { denial, cwdAfter: trackedCwd };
                    break;
                }
                ...
                case 'other': {
                    ...
                    if (programToken !== undefined && gitShapedNames.has(programToken)) {
                        ...
                        ], analysis.state, trackedCwd, activeContext(), scope.relink);
                        if (denial)
                            return { denial, cwdAfter: trackedCwd };
                        break;
                    }
                    ...
                    if (run.some((t) => PATH_EXTRACTING_PROGRAMS.has(executableBaseName(t)))) {
                        ...
                        if (trackedCwd !== undefined)
                            relinkedTargets.push(trackedCwd);
                    }
                    if (run.some((t) => PATH_RELINKING_PROGRAMS.has(executableBaseName(t)))) {
        ...
        if (pipeComponent) {
            ...
            definedBodies.clear();
            for (const [k, v] of definedBodiesBefore)
                definedBodies.set(k, v);
            gitShapedNames.clear();
            for (const k of gitShapedNamesBefore)
                gitShapedNames.add(k);
            exportedNames.clear();
            for (const k of exportedNamesBefore)
                exportedNames.add(k);
            exportedFunctions.clear();
            for (const k of exportedFunctionsBefore)
                exportedFunctions.add(k);
            shellLocals.clear();
            for (const [k, v] of shellLocalsBefore)
                shellLocals.set(k, v);
            exported.relocations.length = 0;
...
async function evaluateBuiltInGuard(request) {
    if (!SHELL_EXECUTING_TOOLS.has(request.toolName))
        return { allowed: true };
    ...
    if (typeof reportedCwd === 'string' && reportedCwd.length > 0) {
        ...
        if (isWithinRoot(canonicalReported, sessionCwd)) {
            ...
            // `<projectRoot>/.lailatulcoder/worktrees/`, which is inside the session — so
            ...
            if (canonicalReported !== sessionCwd) {
                let discovered;
                try {
                    discovered = await resolveDiscoveredRepository(canonicalReported, sessionCwd);
                }
                catch {
            ...
            const ownedWorktrees = await realpathNearestExistingAsync(GitWorktreeService.getWorktreesDir(request.sessionId));
    ...
    if (typeof startDirectoryValue === 'string') {
        startDirectory = await realpathNearestExistingAsync(path.resolve(canonicalEffectiveCwd, startDirectoryValue));
        if (!isWithinRoot(startDirectory, canonicalEffectiveCwd)) {
            return denyTarget(OUTSIDE_TARGET_DENIAL_PREFIX, startDirectory);
        }
    }
    const { denial } = await evaluateCommandWithCwd(command, startDirectory, startDirectory, {
...
export function createDaemonToolGuard(externalGuard) {
    return async (request) => {
        ...
        if (!builtInDecision.allowed || !externalGuard)
...
//# sourceMappingURL=daemon-git-worktree-guard.js.map