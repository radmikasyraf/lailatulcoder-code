...
import { SettingScope, MCPServerStatus, getMCPServerStatus, removeMCPServerStatus, addMCPStatusChangeListener, removeMCPStatusChangeListener, mcpServerRequiresOAuth, MCPOAuthTokenStorage, createDebugLogger, matchesAnyServerPattern, } from '@lailatul-coder/lailatul-coder-core';
import { loadSettings, SettingScope as CliSettingScope, } from '../../../../config/settings.js';
...
function groupFor(isActive, isFavorite, scope) {
    if (!isActive)
        return 'disabled';
    if (isFavorite)
        return 'favorites';
...
export const InstalledTab = ({ config, isActive, onLockChange, onStatus, extensionsUpdateState, reloadSignal, }) => {
    ...
    const load = useCallback(async () => {
        ...
        try {
            ...
            const pluginItems = extensions.map((ext) => {
                const isFavorite = favorites.has(ext.name);
                const scope = scopes[ext.name] ?? 'user';
                return {
                    kind: 'plugin',
                    key: `plugin:${ext.name}`,
                    name: ext.name,
            ...
            const hasAnyMcp = Object.keys(mcpServers).length > 0 ||
            ...
            const buildMcpInfo = (name, serverConfig, scope, isDisabled) => {
            ...
            for (const item of pluginItems) {
                ...
                for (const name of Object.keys(ext.mcpServers ?? {})) {
                    ...
                    children.push({
                        ...
                        name,
                        mcp: buildMcpInfo(name, merged ?? ext.mcpServers[name], 'extension', isDisabled),
                        isActive: !isDisabled,
                        isFavorite: false,
                        group: item.group,
                ...
                if (children.length)
                    childMcpItems.set(ext.name, children);
            }
    ...
    useEffect(() => {
        const listener = (serverName, status) => {
            ...
            setItems((prev) => prev.map((it) => it.kind === 'mcp' && it.mcp.name === serverName
                ? {
                    ...it,
                    mcp: {
                        ...it.mcp,
    ...
    useEffect(() => {
        ...
        const idx = displayRows.findIndex((r) => r.type === 'item' && r.item.key === selectedItem?.key);
        if (idx < 0)
            return;
    ...
    useEffect(() => {
        if (view === 'list' || loading)
            return;
        const matches = selectedItem && (view === 'mcp-detail') === (selectedItem.kind === 'mcp');
        if (!matches)
            goToList();
    }, [view, loading, selectedItem, goToList]);
    const isParentExtensionActive = useCallback((item) => items.some((p) => p.kind === 'plugin' && p.name === item.parentExtension && p.isActive), [items]);
    const enterDetail = useCallback((item) => {
    ...
    const togglePlugin = useCallback(async (item) => {
        if (!extensionManager || mutatingRef.current)
            return;
        const scope = item.scope === 'user' ? SettingScope.User : SettingScope.Workspace;
    ...
    const toggleMcp = useCallback(async (item) => {
        ...
        if (item.parentExtension) {
            ...
            if (item.isActive) {
                ...
                try {
                    extensionManager.setMcpServerDisabled(item.parentExtension, item.name, true);
        ...
        const clearBundledDisable = Boolean(item.parentExtension) && !item.isActive;
        ...
        try {
            if (clearBundledDisable && extensionManager && item.parentExtension) {
                extensionManager.setMcpServerDisabled(item.parentExtension, item.name, false);
            }
            const settings = loadSettings();
            const targetScope = item.mcp.scope === 'project'
                ? CliSettingScope.Workspace
                : CliSettingScope.User;
            if (item.isActive) {
                // Disable: add to excluded + disconnect.
                const excluded = settings.forScope(targetScope).settings.mcp?.excluded ?? [];
                ...
                for (const scope of [
                    ...
                    if (excluded.includes(item.name)) {
                        settings.setValue(scope, 'mcp.excluded', excluded.filter((n) => n !== item.name));
                    }
                }
                const runtimeExcluded = config.getExcludedMcpServers() ?? [];
                config.setExcludedMcpServers(runtimeExcluded.filter((n) => n !== item.name));
    ...
    if (view === 'plugin-detail' && selectedItem?.kind === 'plugin') {
        return (_jsx(ExtensionActionsView, { config: config, extension: selectedItem.extension, isActive: isActive, updateState: extensionsUpdateState.get(selectedItem.name), onStatus: onStatus, onReload: load, onExit: goToList }));
    }
    if (view === 'mcp-detail' && selectedItem?.kind === 'mcp') {
        return (_jsx(McpServerActionsView, { config: config, serverName: selectedItem.mcp.name, isActive: isActive, onStatus: onStatus, onReload: load, onExit: goToList }));
    }
    if (items.length === 0) {
        return (_jsxs(Box, { flexDirection: "column", children: [_jsx(Text, { color: theme.text.secondary, children: t('No plugins or MCP servers installed.') }), _jsx(Text, { color: theme.text.secondary, children: t('Use the Discover tab to find and install plugins.') })] }));
    }
    ...
    const visibleRows = displayRows.slice(scrollOffset, scrollOffset + visibleCount);
    while (visibleRows[0]?.type === 'gap')
        visibleRows.shift();
    const hasAbove = scrollOffset > 0;
    const hasBelow = scrollOffset + visibleCount < displayRows.length;
    return (_jsxs(Box, { flexDirection: "column", children: [hasAbove ? (_jsx(Text, { color: theme.text.secondary, children: t('↑ more above') })) : null, visibleRows.map((row, i) => {
                if (row.type === 'gap') {
                    return _jsx(Box, { height: 1 }, `gap-${scrollOffset + i}`);
                }
                if (row.type === 'header') {
                    return (_jsxs(Text, { color: theme.text.accent, bold: true, children: [groupLabel(row.group), " (", row.count, ")"] }, `header-${row.group}`));
                }
                const item = row.item;
                ...
                return (_jsxs(Box, { children: [_jsx(Box, { minWidth: 2, flexShrink: 0, children: _jsx(Text, { color: isSelected ? theme.text.accent : theme.text.primary, children: marker }) }), _jsxs(Box, { flexGrow: 1, children: [_jsxs(Text, { color: isSelected ? theme.text.accent : theme.text.primary, children: [isChild ? '  └ ' : '', item.name] }), item.isFavorite ? (_jsxs(Text, { color: theme.status.warning, children: [" ", ICON.STAR] })) : null] }), _jsxs(Text, { color: isSelected ? theme.text.accent : theme.text.secondary, children: [kindBadge, ' '] }), _jsxs(Text, { color: isSelected ? theme.text.accent : statusColor, children: ["(", statusLabel, ")"] })] }, item.key));
            }), hasBelow ? (_jsx(Text, { color: theme.text.secondary, children: t('↓ more below') })) : null] }));
};
//# sourceMappingURL=InstalledTab.js.map