...
import { countOccurrences, extractEditSnippet, maybeAugmentOldStringForDeletion, normalizeEditStrings, } from '../utils/editHelper.js';
...
class EditToolInvocation {
    ...
    async calculateEdit(params) {
        ...
        if (!this.config.getFileReadCacheDisabled()) {
            const decision = await checkPriorRead(this.config.getFileReadCache(), params.file_path, 'editing');
            if (!decision.ok) {
                return {
                    currentContent: null,
                    newContent: '',
        ...
        if (fileExists && !this.config.getFileReadCacheDisabled()) {
            const postDecision = await checkPriorRead(this.config.getFileReadCache(), params.file_path, 'editing', { expectExisting: true });
            if (!postDecision.ok) {
        ...
        const normalizedStrings = normalizeEditStrings(currentContent, finalOldString, finalNewString);
        ...
        if (finalOldString === '' && !fileExists) {
            ...
            finalOldString = maybeAugmentOldStringForDeletion(currentContent, finalOldString, finalNewString);
        ...
        if (!error) {
            const teamMemoryError = checkTeamMemorySecrets(params.file_path, newContent, this.config.getProjectRoot());
            if (teamMemoryError) {
                ...
                const preExisting = currentContent !== null &&
                    checkTeamMemorySecrets(params.file_path, currentContent, this.config.getProjectRoot()) !== null;
    ...
    async getConfirmationDetails(abortSignal) {
        ...
        const fileDiff = createPatchSmart(fileName, editData.currentContent ?? '', editData.newContent, 'Current', 'Proposed');
    ...
    async execute(signal) {
        ...
        try {
            ...
            if (!this.config.getFileReadCacheDisabled()) {
                const writeDecision = await checkPriorRead(this.config.getFileReadCache(), this.params.file_path, 'editing', 
            ...
            if (!this.params.modified_by_user) {
                CommitAttributionService.getInstance().recordEdit(this.params.file_path, editData.currentContent, editData.newContent);
            }
            ...
            const snippetResult = extractEditSnippet(editData.currentContent, editData.newContent);
            if (snippetResult) {
...
**Multiple replacements:** Set \`replace_all\` to true when you want to replace every occurrence that matches \`old_string\`.`, Kind.Edit, {
            properties: {
                file_path: {
    ...
    validateToolParamValues(params) {
        ...
        const teamMemoryError = checkTeamMemorySecrets(params.file_path, params.new_string ?? '', this.config.getProjectRoot());
        if (teamMemoryError) {
