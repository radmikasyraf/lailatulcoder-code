...
export function jsonStringPayloadByteLength(value, stopAfterBytes = Number.POSITIVE_INFINITY) {
    let bytes = 0;
...
export function jsonStringJsonByteLength(value) {
...
export function truncateJsonStringPayload(value, originalPayloadBytes, payloadBudget, marker) {
    if (originalPayloadBytes <= payloadBudget)
        return value;
    ...
    return (copyString(value.slice(0, headEnd)) +
        marker +
        copyString(value.slice(tailStart)));
}
export function projectJsonStringToByteBudget(value, jsonByteBudget, marker) {
    const payloadBudget = Math.max(0, jsonByteBudget - JSON_STRING_DELIMITER_BYTES);
    const payloadBytes = jsonStringPayloadByteLength(value, payloadBudget);
    if (payloadBytes <= payloadBudget)
        return value;
    const projected = truncateJsonStringPayload(value, payloadBytes, payloadBudget, marker);
    if (Buffer.byteLength(JSON.stringify(projected), 'utf8') <= jsonByteBudget) {
        return projected;
    }
    const fallback = copyString(marker.slice(0, selectPrefix(marker, payloadBudget)));
    return Buffer.byteLength(JSON.stringify(fallback), 'utf8') <= jsonByteBudget
        ? fallback
        : '';
}
//# sourceMappingURL=json-string-byte-projection.js.map