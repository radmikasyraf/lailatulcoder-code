...
import { collectText, isImagePart, normalizeParts, } from './image-part-utils.js';
import { formatFullTurnVisionNotice, formatVisionBridgeNotice, getFullTurnVisionModelSelector, runVisionBridge, shouldRunVisionBridge, } from './vision-bridge-service.js';
const debugLogger = createDebugLogger('TOOL_RESULT_VISION_BRIDGE');
function getNestedParts(part) {
...
function appendResponseText(response, text) {
    const next = { ...(response ?? {}) };
...
function unavailableNote(part, cancelled) {
...
function replaceNestedImages(part, replacement) {
    const nestedParts = getNestedParts(part);
    if (!nestedParts?.some(isImagePart))
        return part;
...
function clampNestedImages(part) {
    const nestedParts = getNestedParts(part);
    if (!nestedParts?.some(isImagePart))
        return part;
    return {
        ...
        functionResponse: {
            ...
            parts: nestedParts.map((nested) => isImagePart(nested) ? clampInlineMediaPart(nested) : nested),
        },
    };
}
async function bridgeFunctionResponse(part, config, signal, onVisionBridgeNotice) {
    const nestedParts = getNestedParts(part);
    if (!nestedParts)
        return part;
    ...
    try {
        ...
        debugLogger.warn(`vision bridge failed before replacing tool images: ${error instanceof Error ? error.message : String(error)}`);
        onVisionBridgeNotice?.('Vision bridge failed unexpectedly. The image may have been sent to the configured vision model and was not interpreted.');
        replacement = unavailableNote(part, signal.aborted);
    }
    return replaceNestedImages(part, replacement);
}
/** Remove tool-result images from speculative work without making a side query. */
export function stripToolResultImages(responseParts) {
 ...
 * config.
 */
export async function bridgeToolResultImages({ config, responseParts, signal, onFullTurnModel, onVisionBridgeNotice, }) {
    if (!responseParts.some((part) => getNestedParts(part)?.some(isImagePart)) ||
    ...
    if (fullTurnModel?.agentCapable && onFullTurnModel) {
        ...
        const hasUsableImage = fullTurnParts.some((part) => getNestedParts(part)?.some(isImagePart));
        if (!hasUsableImage)
    ...
    for (const part of responseParts) {
        bridged.push(await bridgeFunctionResponse(part, config, signal, onVisionBridgeNotice));
    }
    return bridged;
}
//# sourceMappingURL=tool-result-vision-bridge.js.map