...
import { DEFAULT_QWEN_CUSTOM_IGNORE_FILE_NAMES, DEFAULT_SENSITIVE_SPAN_ATTRIBUTE_MAX_LENGTH, SENSITIVE_SPAN_ATTRIBUTE_MAX_LENGTH_LIMIT, } from '@lailatul-coder/lailatul-coder-core';
import { getSettingsSchema, MergeStrategy, } from './settingsSchema.js';
import { MAX_CONCURRENT_SUB_SESSIONS_PER_CALLER, MAX_CONCURRENT_SUB_SESSIONS_TOTAL, MAX_TRACKED_SPAWNED_SESSIONS, } from '../serve/create-sub-session.js';
    ...
    describe('getSettingsSchema', () => {
        ...
        it('should have accessibility nested properties', () => {
            ...
            expect(getSettingsSchema().ui?.properties?.accessibility.properties
                ?.enableLoadingPhrases.type).toBe('boolean');
        });
        it('should have fileFiltering nested properties', () => {
            expect(getSettingsSchema().context.properties.fileFiltering.properties
                ?.respectGitIgnore).toBeDefined();
            expect(getSettingsSchema().context.properties.fileFiltering.properties
                ?.respectQwenIgnore).toBeDefined();
            expect(getSettingsSchema().context.properties.fileFiltering.properties
                ?.customIgnoreFiles).toBeDefined();
            expect(getSettingsSchema().context.properties.fileFiltering.properties
                ?.customIgnoreFiles?.type).toBe('array');
            expect(getSettingsSchema().context.properties.fileFiltering.properties
                ?.customIgnoreFiles?.default).toEqual([...DEFAULT_QWEN_CUSTOM_IGNORE_FILE_NAMES]);
            expect(getSettingsSchema().context.properties.fileFiltering.properties
                ?.customIgnoreFiles?.showInDialog).toBe(false);
            expect(getSettingsSchema().context.properties.fileFiltering.properties
                ?.enableRecursiveFileSearch).toBeDefined();
        });
        it('should keep the ACP session writer lease opt-in', () => {
            expect(getSettingsSchema().experimental.properties.sessionWriterLease).toMatchObject({
                type: 'boolean',
                default: false,
        ...
        it('should expose cumulative tool result threshold in clearContextOnIdle', () => {
            const threshold = getSettingsSchema().context.properties.clearContextOnIdle.properties
        ...
        it('should have sandboxImage setting under tools', () => {
            ...
            expect(getSettingsSchema().tools.properties.sandboxImage.type).toBe('string');
            expect(getSettingsSchema().tools.properties.sandboxImage.default).toBe(undefined);
        });
        it('should define tools.sandbox schema override as boolean or string', () => {
            expect(getSettingsSchema().tools.properties.sandbox.jsonSchemaOverride).toEqual({
        ...
        it('should define the built-in Explore model setting', () => {
            const exploreModel = getSettingsSchema().agents.properties.builtin.properties.exploreModel;
        ...
        it('should define stopHookBlockingCap schema override as a positive integer', () => {
            expect(getSettingsSchema().stopHookBlockingCap.jsonSchemaOverride).toEqual({
                type: 'integer',
                minimum: 1,
                default: 8,
            });
        ...
        it('should define telemetry sensitiveSpanAttributeMaxLength as a positive integer', () => {
            ...
            expect(telemetrySchema.properties?.sensitiveSpanAttributeMaxLength).toEqual({
        ...
        it('should define telemetry userId as a privacy-sensitive string', () => {
            ...
            expect(telemetrySchema.properties?.userId).toEqual({
                description: 'Stable end-user identifier written to GenAI spans as gen_ai.user.id for ARMS session analysis. This value is linkable personal data: prefer a pseudonymous ID, and configure it only when one process represents one user.',
                type: 'string',
            });
        });
        it('should have voice dictation settings under general', () => {
            const voice = getSettingsSchema().general.properties.voice.properties ?? {};
            ...
            expect(voice.mode.options?.map((o) => o.value)).toEqual(['hold', 'tap']);
        ...
        it('marks MCP reconcile inputs as hot-reloadable but startup-only MCP keys as restart-required (sub-task 3)', () => {
            ...
            expect(getSettingsSchema().mcp.properties.allowed.requiresRestart).toBe(false);
            expect(getSettingsSchema().mcp.properties.excluded.requiresRestart).toBe(false);
            ...
            expect(getSettingsSchema().mcp.properties.serverCommand.requiresRestart).toBe(true);
        ...
        it('defines disabled skill levels as a restart-required union setting', () => {
            const disabledLevels = getSettingsSchema().skills.properties.disabledLevels;
        ...
        it('keeps Session Workflow opt-in without requiring a restart', () => {
            expect(getSettingsSchema().experimental.properties.sessionWorkflow).toMatchObject({
                type: 'boolean',
                default: false,
        ...
        it('should have showInDialog property configured', () => {
            ...
            expect(getSettingsSchema().ui.properties.hideWindowTitle.showInDialog).toBe(false);
            expect(getSettingsSchema().ui.properties.hideTips.showInDialog).toBe(true);
            expect(getSettingsSchema().ui.properties.showResponseTokensPerSecond
            ...
            expect(getSettingsSchema().ui.properties.accessibility.showInDialog).toBe(false);
            expect(getSettingsSchema().context.properties.fileFiltering.showInDialog).toBe(false);
            expect(getSettingsSchema().general.properties.preferredEditor.showInDialog).toBe(true);
        ...
        it('should have useTerminalBuffer in ui settings', () => {
            const useTerminalBuffer = getSettingsSchema().ui.properties.useTerminalBuffer;
        ...
        it('should expose response tokens/sec as an opt-in UI setting', () => {
            const responseTokensPerSecond = getSettingsSchema().ui.properties.showResponseTokensPerSecond;
        ...
        it('should pin serve sub-session caps to the runtime defaults', () => {
            ...
            expect(serve.maxConcurrentSubSessionsPerCaller.default).toBe(MAX_CONCURRENT_SUB_SESSIONS_PER_CALLER);
            expect(serve.maxConcurrentSubSessionsTotal.default).toBe(MAX_CONCURRENT_SUB_SESSIONS_TOTAL);
            ...
            expect(serve.maxConcurrentSubSessionsTotal.maximum).toBe(MAX_TRACKED_SPAWNED_SESSIONS);
        });
        ...
        it('should define context.fileName schema override as string or string array', () => {
            expect(getSettingsSchema().context?.properties.fileName.jsonSchemaOverride).toEqual({
                anyOf: [
                    { type: 'string' },
        ...
        it('should have loadFromIncludeDirectories setting in schema', () => {
            expect(getSettingsSchema().context?.properties.loadFromIncludeDirectories).toBeDefined();
            expect(getSettingsSchema().context?.properties.loadFromIncludeDirectories.type).toBe('boolean');
            expect(getSettingsSchema().context?.properties.loadFromIncludeDirectories
        ...
        it('should define advanced.dnsResolutionOrder as ipv4first or verbatim', () => {
            const dnsResolutionOrder = getSettingsSchema().advanced.properties.dnsResolutionOrder;
