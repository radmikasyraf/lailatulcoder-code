...
import { MAX_INLINE_ANGLES, MIN_INLINE_ANGLES, VERIFY_SHARD, budgetGapDisclosures, stripBudgetGapLines, launchToolBudget, reverseAuditRoundCap, reverseAuditRoundTier, cappedRoundTier, reviewBudget, } from './budget.js';
const budget = (srcDiffLines, diffLines = srcDiffLines) => reviewBudget({ srcDiffLines, diffLines });
describe('reviewBudget — inline angles scale with what there is to see', () => {
    ...
    it('still earns angles on a large all-prose diff, at a coarser rate', () => {
        ...
        // equivalent would.
        expect(budget(0, 2000).inlineAngles).toBeLessThanOrEqual(budget(2000, 2000).inlineAngles);
    });
});
...
describe('reviewBudget — domain specialists', () => {
    ...
    it('shed on a huge non-source diff — the gate keys on effective, not src', () => {
        ...
        expect(reviewBudget({ srcDiffLines: 100, diffLines: 30_000 }).specialistCap).toBe(0);
    });
});
...
describe('reviewBudget — the agent tool budget', () => {
    it('floors at 30 on a small diff', () => {
        expect(reviewBudget({ srcDiffLines: 40, diffLines: 60 }).agentToolBudget).toBe(32);
        expect(reviewBudget({ srcDiffLines: 0, diffLines: 0 }).agentToolBudget).toBe(30);
    });
    it('earns a call per twenty effective lines', () => {
        expect(reviewBudget({ srcDiffLines: 300, diffLines: 400 }).agentToolBudget).toBe(45);
    });
    it('caps at 60 — a wanderer must not out-earn the ceiling', () => {
        expect(reviewBudget({ srcDiffLines: 5000, diffLines: 6000 }).agentToolBudget).toBe(60);
    });
    it('a large all-prose diff earns budget at the coarse effective rate', () => {
        ...
        expect(reviewBudget({ srcDiffLines: 0, diffLines: 3200 }).agentToolBudget).toBe(50);
    });
});
...
describe('budgetGapDisclosures — the one parser of the disclosure format', () => {
    it('parses plain fixed-format lines', () => {
        expect(budgetGapDisclosures('No issues found — walked it all.\n' +
            'Budget gap: callers of parseArgs outside packages/cli\n' +
            'Budget gap: the removed retry path')).toEqual([
    ...
    it('does not read a QUOTATION of the format as a use of it', () => {
        ...
        expect(budgetGapDisclosures('> Budget gap: the removed retry path in fetch-pr')).toEqual([]);
        expect(budgetGapDisclosures('```\nBudget gap: inside a fence\n```\nafter the fence')).toEqual([]);
        expect(budgetGapDisclosures('- `Budget gap: <the check>`, which check-coverage parses out of the transcripts')).toEqual([]);
    });
    it('requires the gap on the SAME line as its marker', () => {
        ...
        expect(budgetGapDisclosures('Budget gap:\nNo further checks were cut short.')).toEqual([]);
        expect(budgetGapDisclosures('**Budget gap:**\n- item one\n- item two')).toEqual([]);
    });
    ...
    it('strips full-width parens too — a CJK IME wraps the same no-answer', () => {
        ...
        expect(budgetGapDisclosures('Budget gap: （无法验证 Windows 矩阵的集成测试）')).toEqual(['（无法验证 Windows 矩阵的集成测试）']);
    });
    ...
    it('keeps a REAL Chinese gap — 无法 is a prefix of the token, not the token', () => {
        ...
        expect(budgetGapDisclosures('Budget gap: 无 — 除 Windows 矩阵外均已完成')).toEqual(['无 — 除 Windows 矩阵外均已完成']);
    ...
    it('folds duplicate disclosures into one gap', () => {
        ...
        expect(budgetGapDisclosures('Budget gap: second-order callers\n' +
            'more prose\n' +
            'Budget gap: Second-order callers')).toEqual(['second-order callers']);
        // … whether or not the restatement wraps the gap in parentheses …
        expect(budgetGapDisclosures('Budget gap: auth flow untested\n' + 'Budget gap: (auth flow untested)')).toEqual(['auth flow untested']);
    ...
    it('sanitizes and caps what will reach a terminal and the posted body', () => {
        ...
        const laundered = budgetGapDisclosures('Budget gap: first\u2028second \u009b\u202epart')[0];
        ...
        const many = budgetGapDisclosures(Array.from({ length: 20 }, (_, i) => `Budget gap: check ${i}`).join('\n'));
    ...
    it('stays linear on pathological inputs', () => {
        ...
        expect(budgetGapDisclosures(`${'\t'.repeat(4000)}- Budget gap: the check`)).toEqual(['the check']);
    });
});
describe('stripBudgetGapLines — the receipt judged without its disclosures', () => {
    it('removes exactly the disclosure lines and keeps everything else', () => {
        expect(stripBudgetGapLines('No new issues found — re-walked the territory.\n' +
...
describe('reviewBudget — the reverse-audit round cap', () => {
    it('gives each topology its own cap: ten on 3A, five on 3B, three when huge (with a clock)', () => {
        ...
        expect(reviewBudget({ srcDiffLines: 100, diffLines: 100 }).reverseAuditRounds).toBe(10);
        expect(reviewBudget({ srcDiffLines: 2999, diffLines: 2999 }).reverseAuditRounds).toBe(5);
        expect(reviewBudget({ srcDiffLines: 3000, diffLines: 3000 }, { hasDeadline: true }).reverseAuditRounds).toBe(3);
        expect(reviewBudget({ srcDiffLines: 10_000, diffLines: 12_000 }, { hasDeadline: true }).reverseAuditRounds).toBe(3);
    });
    it('switches tiers on the topology gate, not on a second set of numbers', () => {
        ...
        expect(reviewBudget({ srcDiffLines: 500, diffLines: 3200 }).reverseAuditRounds).toBe(10);
        expect(reviewBudget({ srcDiffLines: 501, diffLines: 3200 }).reverseAuditRounds).toBe(5);
        expect(reviewBudget({ srcDiffLines: 500, diffLines: 3201 }).reverseAuditRounds).toBe(5);
        // …and the huge floor wins over the topology gate: a 3B diff at 3000
        // effective lines caps at three, not five.
        expect(reviewBudget({ srcDiffLines: 3000, diffLines: 3200 }, { hasDeadline: true }).reverseAuditRounds).toBe(3);
    });
    it('keys on effective lines, not raw source — a huge lockfile diff caps too', () => {
        ...
        expect(reviewBudget({ srcDiffLines: 100, diffLines: 30_000 }, { hasDeadline: true }).reverseAuditRounds).toBe(3);
        expect(reviewBudget({ srcDiffLines: 100, diffLines: 30_000 }).sweep).toBe(true);
    });
    it('never drops below the convergence minimum', () => {
        for (const n of [0, 1, 50, 3000, 100_000]) {
            expect(reviewBudget({ srcDiffLines: n, diffLines: n }).reverseAuditRounds).toBeGreaterThanOrEqual(3);
        }
    });
});
describe('reverseAuditRoundCap — the one reader of the plan field', () => {
    ...
    it('passes a value the plan owns through, at every tier', () => {
        expect(reverseAuditRoundCap({ ...SMALL, budget: { reverseAuditRounds: 10 } }, true)).toBe(10);
        expect(reverseAuditRoundCap({ ...LARGE, budget: { reverseAuditRounds: 5 } }, true)).toBe(5);
        expect(reverseAuditRoundCap({ ...HUGE, budget: { reverseAuditRounds: 3 } }, true)).toBe(3);
        // Below its tier but in band is honoured — nothing here inflates a cap
        // the plan itself wrote smaller.
        expect(reverseAuditRoundCap({ ...SMALL, budget: { reverseAuditRounds: 5 } }, true)).toBe(5);
    });
    it('honours a stored value in the INTERIOR of the tier-relative band', () => {
        ...
        expect(reverseAuditRoundCap({ ...SMALL, budget: { reverseAuditRounds: 7 } }, true)).toBe(7);
        expect(reverseAuditRoundCap({ ...LARGE, budget: { reverseAuditRounds: 4 } }, true)).toBe(4);
        ...
        expect(reverseAuditRoundCap({ ...HUGE, budget: { reverseAuditRounds: 4 } }, true)).toBe(3);
    });
    it('clamps to the plan’s own tier, so a hand edit cannot cross topologies', () => {
        ...
        // comparison.
        expect(reverseAuditRoundCap({ ...HUGE, budget: { reverseAuditRounds: 10 } }, true)).toBe(3);
        expect(reverseAuditRoundCap({ ...LARGE, budget: { reverseAuditRounds: 10 } }, true)).toBe(5);
        expect(reverseAuditRoundCap({ ...SMALL, budget: { reverseAuditRounds: 11 } }, true)).toBe(10);
    });
    it('reads absent, out-of-band and garbled values as the tier', () => {
        ...
        for (const bad of [0, 1, 2, 2.5, '1', null]) {
            expect(reverseAuditRoundCap({ ...SMALL, budget: { reverseAuditRounds: bad } }, true)).toBe(10);
            expect(reverseAuditRoundCap({ ...HUGE, budget: { reverseAuditRounds: bad } }, true)).toBe(3);
        }
    ...
    it('rejects an in-band NON-INTEGER — the only guard that can catch it', () => {
        ...
        expect(reverseAuditRoundCap({ ...SMALL, budget: { reverseAuditRounds: 3.5 } }, true)).toBe(10);
        expect(reverseAuditRoundCap({ ...LARGE, budget: { reverseAuditRounds: 4.5 } }, true)).toBe(5);
    });
    it('honours a legacy in-band value instead of migrating it to the tier', () => {
        ...
        expect(reverseAuditRoundCap({ ...SMALL, budget: { reverseAuditRounds: 5 } }, true)).toBe(5);
        expect(reverseAuditRoundCap({ ...SMALL, budget: { reverseAuditRounds: 3 } }, true)).toBe(3);
    });
    it('treats a COERCIBLE garbage size as unsized, not as a zero-line diff', () => {
        ...
        for (const bad of [null, '', false, [], -5, '1', '3000']) {
            expect(reverseAuditRoundCap({ srcDiffLines: bad, diffLines: bad }, true)).toBe(5);
            // …including when only ONE of the pair is garbage.
            expect(reverseAuditRoundCap({ srcDiffLines: 100, diffLines: bad }, true)).toBe(5);
            expect(reverseAuditRoundCap({ srcDiffLines: bad, diffLines: 100 }, true)).toBe(5);
        }
    ...
    it('does not let reviewBudget RECORD a tier for a size it never received', () => {
        ...
        for (const bad of [Number.NaN, -5, Number.POSITIVE_INFINITY]) {
            expect(reviewBudget({ srcDiffLines: bad, diffLines: bad }).reverseAuditRounds).toBe(5);
        }
        expect(reviewBudget({}).reverseAuditRounds).toBe(5);
        // …while a genuinely empty diff still records the small tier.
        expect(reviewBudget({ srcDiffLines: 0, diffLines: 0 }).reverseAuditRounds).toBe(10);
    });
    it('reads a plan with no usable size as the large tier — today’s value', () => {
        ...
        expect(reverseAuditRoundCap({ budget: { reverseAuditRounds: 10 } }, true)).toBe(5);
        expect(reverseAuditRoundCap({
            srcDiffLines: 'x',
            diffLines: 10,
            budget: { reverseAuditRounds: 10 },
...
describe('reviewBudget — the budget survives the trip through the plan', () => {
    it('agentToolBudget is an enumerable field of the returned object', () => {
        ...
        expect(JSON.parse(JSON.stringify(b))['agentToolBudget']).toBe(30);
    });
});
describe('cappedRoundTier — the operator ceiling may only lower a tier', () => {
    ...
    it('ignores a ceiling that is not a whole number', () => {
        for (const bad of [3.5, Number.NaN, Number.POSITIVE_INFINITY]) {
            expect(cappedRoundTier(SMALL, bad, true)).toBe(10);
        }
    ...
    it('lowers what reviewBudget RECORDS, so every reader sees one number', () => {
        ...
        const b = reviewBudget({ srcDiffLines: 100, diffLines: 100 }, { operatorRoundCap: 4 });
    ...
    it('does not let the ceiling touch any other budget field', () => {
        ...
        const capped = reviewBudget({ srcDiffLines: 900, diffLines: 900 }, { operatorRoundCap: 3 });
...
describe('the huge reduction applies only where there is a wall to fit inside', () => {
    ...
    it('a huge diff with no deadline is just a large 3B diff', () => {
        ...
        expect(reviewBudget(HUGE, { hasDeadline: false }).reverseAuditRounds).toBe(5);
