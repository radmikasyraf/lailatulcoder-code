...
//
// This repo runs `LailatulCoder-autofix.yml`, which posts to pull requests. The skill already
...
// rules (`.lailatulcoder/review-rules.md` and friends), never a replacement: a project that
...
const GITHUB_ACTIONS = {
    ...
    matches: (p) => /^\.github\/(workflows\/.+\.ya?ml|actions\/.+\/action\.ya?ml|(?:actions\/(?:[^/]+\/)+|scripts\/(?:[^/]+\/)*)[^/]+\.(?:[cm]?[jt]sx?|py|sh|bash|zsh|ksh|dash|ps1|bat|cmd|rb|pl))$/i.test(p),
 ...
 * hypothetical one.
 */
const NON_CONTRACT_GENRE = 'designs?|plans?|rfcs?|proposals?|adrs?|changelogs?|releases?';
 ...
 */
const NON_CONTRACT_STEM = new RegExp(`(?:^|[-_. ])(?:${NON_CONTRACT_GENRE})(?:[-_. ]|$)`, 'i');
/**
 ...
 * enumerated at all, and the reason it is exact-match: `LailatulCoder` excludes an
 * agent-context file without touching `LailatulCoder-serve-protocol`.
 */
const NON_CONTRACT_FILE = /^(?:contributing|roadmap|code[-_]of[-_]conduct|license|licence|authors|governance|support|LailatulCoder|agents|claude)$/i;
/**
 ...
 */
const NOT_OURS = /(?:^|\/)(?:vendor|vendors|third[-_]?party|node_modules|fixtures|__fixtures__)\//i;
const CONTRACT_DOCS = {
    ...
    // checklist anyway has `.lailatulcoder/review-rules.md`.
    //
    ...
    matches: (p) => {
        if (!/\.(?:mdx?|markdown|mdown)$/i.test(p))
            return false;
        if (NOT_OURS.test(p))
            return false;
        if (NON_CONTRACT_DIR.test(p))
            return false;
        ...
        if (NON_CONTRACT_FILE.test(stem))
            return false;
        if (NON_CONTRACT_STEM.test(stem))
            return false;
        return (
...
function isOutOfScope(p) {
    return (/src\/(test|integrationTest|integTest|androidTest|testFixtures)\//i.test(p) ||
        /(?:^|\/)(?:package-info|module-info)\.java$/i.test(p) ||
        /(?:^|\/)(?:target|build)\/(?:generated-sources|generated-test-sources|generated)\//i.test(p));
}
/**
 ...
 * for.
 */
function describePaths(rule, which) {
    const CAP = 10;
 ...
 */
export function pathRulesFor(paths) {
    ...
    for (const r of hit) {
        ...
        parts.push(`### ${r.title} — ${describePaths(r, which)}`, '', r.checklist, '');
    }
    return parts.join('\n').trimEnd();
}
