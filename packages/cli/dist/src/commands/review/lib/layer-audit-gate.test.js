...
describe('layerAuditGate', () => {
    ...
    const writePlan = (plan) => writeFileSync(planPath(), JSON.stringify(plan));
    const env = {};
    // Receipts covering exactly the named layers, one return each.
    const returns = (...covered) => covered.map((id) => `Layer walked: ${id} — examined.`);
    ...
    const reader = (corroborated, identityMatched = corroborated.length) => () => ({ corroborated, identityMatched });
    beforeEach(() => {
        dir = mkdtempSync(join(tmpdir(), 'layer-gate-'));
    });
    afterEach(() => rmSync(dir, { recursive: true, force: true }));
    it('is inert with no plan path', () => {
        expect(layerAuditGate(undefined, env, reader(returns())).unreviewed).toEqual([]);
    });
    it('is inert on a plan with no repository context', () => {
        writePlan({ srcDiffLines: 10 });
        expect(layerAuditGate(planPath(), env, reader(returns('lexing'))).unreviewed).toEqual([]);
    });
    ...
    it('owes an entry per unwalked layer once the diff is marked a modeled system', () => {
        ...
        const out = layerAuditGate(planPath(), env, reader(returns('lexing', 'expansion'))).unreviewed;
    ...
    it('fails open (default reader) when the transcript dir is missing — finding 1', () => {
        ...
        writePlan({ repositoryContext: context([MODELED_SYSTEM_DOMAIN]) });
        expect(() => layerAuditGate(planPath(), {})).not.toThrow();
        expect(layerAuditGate(planPath(), {}).unreviewed).toEqual([]);
    });
    it('owes nothing when every layer was walked', () => {
        writePlan({ repositoryContext: context([MODELED_SYSTEM_DOMAIN]) });
        const out = layerAuditGate(planPath(), env, reader(returns('lexing', 'expansion', 'scope-propagation', 'resolution-order', 'inheritance', 'toctou'))).unreviewed;
        expect(out).toEqual([]);
    });
    ...
    it('fails open on an unreadable plan', () => {
        expect(layerAuditGate(join(dir, 'nope.json'), env, reader(returns('lexing')))
            .unreviewed).toEqual([]);
    });
});
describe('the real reader on a resumed run — prior-session auditors count', () => {
    ...
    function auditorTranscript(session, covered) {
        ...
        const launch = 'You are review agent `reverse-audit` — hunt the gaps.\n' +
    ...
    it('refuses a receipt whose auditor only NAMED the brief, never read it', () => {
        ...
        writeFileSync(f, readFileSync(f, 'utf8').replace('"functionCall":{"name":"read_file","args":{"file_path":' +
    ...
    it('a transcript matching TWO reverse-audit records delivers neither', () => {
        ...
        recordPrompt(plan, 'reverse-audit--chunk-9', `read_file(file_path="${diff}", offset=0, limit=100)`);
        const out = layerAuditGate(plan, ENV()).unreviewed;
        expect(out).toHaveLength(6);
    });
    it('refuses a STALE record a dead attempt left beside the plan', () => {
        ...
        const rec = join(promptRecordDir(plan), `${encodeURIComponent('reverse-audit')}.txt`);
    ...
    it('refuses a delivered auditor whose reads were OFF its territory', () => {
        ...
        writeFileSync(f, readFileSync(f, 'utf8').replaceAll('"args":{"file_path":' +
            JSON.stringify(diff) +
            ',"offset":0,"limit":100}', '"args":{"file_path":' +
            JSON.stringify(diff) +
    ...
    it('ignores receipts from a delivered agent WITHOUT the identity line', () => {
        ...
        const key = 'verify';
        const brief = briefPath(plan, key);
        const launch = 'You are review agent `verify` — rule on the findings.\n' +
            `read_file(file_path="${brief}")\n` +
            `read_file(file_path="${diff}", offset=0, limit=100)`;
        mkdirSync(promptRecordDir(plan), { recursive: true });
        ...
        writeFileSync(join(dir, 'subagents', 'S1', 'agent-v-1.jsonl'), [
            JSON.stringify({
                ...base,
                type: 'user',
                message: { role: 'user', parts: [{ text: launch }] },
            ...
            JSON.stringify({
                ...
                message: {
                    ...
                    parts: [
                        {
                            text: LAYERS.map((id) => `Layer walked: ${id} — examined.`).join('\n'),
                        },
                    ],
                },
            }),
    ...
    it('refuses an identity MENTION that is not the identity line', () => {
        ...
        const key = 'verify';
        const brief = briefPath(plan, key);
        const launch = 'You are review agent `verify` — after the reverse-audit pass, rule.\n' +
            `read_file(file_path="${brief}")\n` +
            `read_file(file_path="${diff}", offset=0, limit=100)`;
        mkdirSync(promptRecordDir(plan), { recursive: true });
        writeFileSync(brief, 'The verify brief.');
...
//# sourceMappingURL=layer-audit-gate.test.js.map