...
import { AuthType, isImageCapable, parseVisionModelSetting, resolveModelId, } from '@lailatul-coder/lailatul-coder-core';
import { SettingScope } from '../../config/settings.js';
import { isInlineModelOverrideAllowed, parseAcpModelOption, } from '../../utils/acpModelUtils.js';
import { formatUnsupportedVoiceModelMessage, isSelectableVoiceModel, } from '../voice/voice-model.js';
const MAIN_MODEL_CONFIGURATION_HINT = 'Configure models in settings.modelProviders and ensure the required environment variables are set. In interactive mode, run /auth to configure or switch providers, or run /model without arguments to choose from configured models.';
const FAST_MODEL_CONFIGURATION_HINT = 'Configure models in settings.modelProviders and ensure the required environment variables are set. In interactive mode, run /auth to configure or switch providers, or run /model --fast without a model to choose from configured models.';
const VISION_MODEL_CONFIGURATION_HINT = 'Configure an image-capable model in settings.modelProviders and ensure the required environment variables are set. Run /model --vision <model-id> to set it, or leave it unset to auto-pick a same-provider vision model.';
const COMPACTION_MODEL_CONFIGURATION_HINT = 'Configure models in settings.modelProviders and ensure the required environment variables are set. In interactive mode, run /auth to configure or switch providers, or run /model --compaction without a model to choose from configured models.';
const IMAGE_MODEL_CONFIGURATION_HINT = 'Configure a model with imageOnly: true, baseUrl, and envKey in settings.modelProviders. Run /model --image <model-id> to select it.';
...
const MODEL_PICKER_ONLY_PATTERN = new RegExp(`^(?:\\s*--${MODEL_PICKER_FLAG_PATTERN})*\\s*$`);
export function isPickerOnlyModelInvocation(args) {
...
function persistSetting(settings, path, value, scopeOverride) {
...
async function switchMainModel(config, settings, currentAuthType, modelArg, scopeOverride) {
    ...
    if (parsed.authType) {
        await config.switchModel(parsed.authType, parsed.modelId, parsed.authType !== currentAuthType &&
            parsed.authType === AuthType.NEVER_MATCH_THIS
            ? { requireCachedCredentials: true }
            : undefined);
        persistSetting(settings, 'security.auth.selectedType', parsed.authType, scopeOverride);
...
function formatUnavailableModelMessage(kind, modelName, authType, availableModels) {
    const availableModelIds = Array.from(new Set(availableModels.map((model) => model.id)));
    const availableModelsLine = availableModelIds.length === 0
        ? `No models are configured for auth type '${authType}'.`
        : `Available models for '${authType}': ${availableModelIds.join(', ')}.`;
    const hint = kind === 'Fast model'
...
function formatUnavailableFastModelMessage(modelName, availableModels) {
    return formatUnavailableAuxModelMessage('Fast model', modelName, availableModels, FAST_MODEL_CONFIGURATION_HINT);
}
function formatUnavailableVisionModelMessage(modelName, availableModels) {
    return formatUnavailableAuxModelMessage('Vision model', modelName, availableModels, VISION_MODEL_CONFIGURATION_HINT);
}
function formatUnavailableCompactionModelMessage(modelName, availableModels) {
    return formatUnavailableAuxModelMessage('Compaction model', modelName, availableModels, COMPACTION_MODEL_CONFIGURATION_HINT);
}
function formatUnavailableImageModelMessage(modelName, availableModels) {
    return formatUnavailableAuxModelMessage('Image model', modelName, availableModels, IMAGE_MODEL_CONFIGURATION_HINT);
}
function formatAmbiguousVisionModelMessage(modelName, matchingModels) {
    const endpoints = matchingModels
        ...
        .join(', ');
    const qualifiedSelectors = Array.from(new Set(matchingModels
        .map((model) => model.authType ? `${model.authType}:${model.id}` : undefined)
        .filter((selector) => selector !== undefined)));
    const scriptedHint = qualifiedSelectors.length > 1
        ? `\n${t('For scripts, pass an auth-qualified selector such as {{selector}}.', {
...
function formatNonVisionModelWarning(modelName) {
    return t("⚠ '{{model}}' is not a known image-capable model; the vision bridge may fail on images.", { model: modelName });
}
function formatUnavailableVoiceModelMessage(modelName, availableModels) {
    ...
    return (t("Voice model '{{modelName}}' is not configured.", { modelName }) +
        '\n' +
        `${availableModelsLine}\n` +
        t('Configure a unique model id in settings.modelProviders or run /model --voice to select an available model.'));
}
// Get an array of the available model IDs as strings, filtered by mode
function getAvailableModelIds(context, mode = 'main') {
    ...
    const availableModels = models.filter((m) => {
        ...
        if (mode === 'fast')
            return !m.voiceOnly && !m.imageOnly && !m.visionOnly;
        if (mode === 'voice')
            return !m.fastOnly && !m.imageOnly && !m.visionOnly;
...
export const modelCommand = {
    ...
    get description() {
        return t('Switch the model for this session (--fast for suggestion model, --voice for voice transcription model, --vision for the vision bridge model, --compaction for chat compression model, --image for the image generation model, --project to persist to project settings, --global to persist to user settings, [model-id] to switch immediately, or [model-id] [prompt] to run a one-off prompt on another model; the inline prompt is sent verbatim without @file expansion).');
    },
    ...
    completion: async (context, partialArg) => {
        if (partialArg) {
            const flagCompletions = [
                {
                    value: '--fast',
                    description: t('Set a lighter model for prompt suggestions and speculative execution'),
                },
                {
                    value: '--voice',
                ...
                {
                    value: '--vision',
                    description: t('Set the image-capable model used to transcribe images for a text-only main model'),
                },
                {
                    value: '--compaction',
                    description: t('Set the model used for chat compression (auto-compaction)'),
                },
                {
                    value: '--image',
                ...
                {
                    value: '--project',
                    description: t('Persist the model selection to the project settings (workspace scope)'),
                },
                {
                    value: '--global',
                    description: t('Persist the model selection to the user settings (global scope)'),
                },
            ...
            if (trimmed) {
                ...
                const modelPrefix = trimmed
                    ...
                    .replace(/(?:^|\s)--global(?:\s|$)/, ' ')
                    .trim();
                if (/(?:^|\s)--fast(?:\s|$)/.test(trimmed))
                    mode = 'fast';
                else if (/(?:^|\s)--voice(?:\s|$)/.test(trimmed))
                    mode = 'voice';
                else if (/(?:^|\s)--vision(?:\s|$)/.test(trimmed))
                    mode = 'vision';
                else if (/(?:^|\s)--compaction(?:\s|$)/.test(trimmed))
                    mode = 'compaction';
                else if (/(?:^|\s)--image(?:\s|$)/.test(trimmed))
                    mode = 'image';
                return getAvailableModelIds(context, mode).filter((id) => id.startsWith(modelPrefix));
            }
            return null;
        }
        else {
    ...
    action: async (context, actionArgs) => {
        const { services } = context;
        ...
        const { scopeOverride, remaining: args, hasProject, hasGlobal, } = parseScopeFlags(rawArgs);
        // Reject mutually exclusive scope flags
        if (hasProject && hasGlobal) {
            return {
                type: 'message',
                messageType: 'error',
                content: t('Cannot use both --project and --global. Choose one scope flag.'),
            };
        }
        ...
        const scopeSuffix = scopeOverride === SettingScope.Workspace
            ? t(' (this project)')
            : scopeOverride === SettingScope.User
                ? t(' (global)')
                : '';
        const isVoiceModelCommand = args === '--voice' || args.startsWith('--voice ');
        if (isVoiceModelCommand) {
            const modelName = args.replace('--voice', '').trim();
            if (!modelName) {
                if (context.executionMode !== 'interactive') {
                    const voiceModel = context.services.settings?.merged?.voiceModel?.trim() ||
                        t('not set');
                    return {
                        type: 'message',
                        messageType: 'info',
                        content: t('Current voice model: {{voiceModel}}\nUse "/model --voice <model-id>" to set voice model.', { voiceModel }),
                    };
                }
                return {
                    type: 'dialog',
            ...
            if (matches.length === 0) {
                return {
                    type: 'message',
                    messageType: 'error',
                    content: formatUnavailableVoiceModelMessage(modelName, availableModels),
                };
            }
            if (matches.length > 1) {
                return {
                    type: 'message',
                    messageType: 'error',
                    content: t("Voice model '{{modelName}}' is ambiguous. Configure a unique model id before using /model --voice.", { modelName }),
                };
            }
        ...
        if (isFastModelCommand) {
            ...
            const availableModels = (selector.authType
                ...
                : config.getAllConfiguredModels()).filter((m) => !m.voiceOnly && !m.imageOnly && !m.visionOnly);
            if (!availableModels.some((model) => model.id === selector.modelId)) {
                return {
                    type: 'message',
                    messageType: 'error',
                    content: selector.authType
                        ? formatUnavailableModelMessage('Fast model', selector.modelId, selector.authType, availableModels)
        ...
        if (isVisionModelCommand) {
            ...
            if (!modelName) {
                ...
                if (context.executionMode !== 'interactive') {
                    const visionModel = context.services.settings?.merged?.visionModel?.trim();
                    return {
                        type: 'message',
                        messageType: 'info',
                        content: t('Current vision model: {{visionModel}}\nUse "/model --vision <model-id>" to set the vision bridge model.', {
            ...
            const availableModels = (selector.authType
                ? config.getAvailableModelsForAuthType(selector.authType)
                : config.getAllConfiguredModels()).filter((m) => !m.fastOnly && !m.voiceOnly && !m.imageOnly);
            const matchingModels = availableModels.filter((model) => model.id === selector.modelId);
            ...
            if (!matched) {
                return {
                    ...
                    messageType: 'error',
                    content: selector.authType
                        ? formatUnavailableModelMessage('Vision model', selector.modelId, selector.authType, availableModels)
            ...
            if (config.isCurrentPrimaryModel(matched)) {
                return {
                    type: 'message',
                    messageType: 'error',
                    content: t("'{{model}}' is the current primary model and cannot be used as the vision bridge. Choose a different image-capable model.", { model: modelName }),
                };
            }
            ...
            return {
                type: 'message',
                messageType: 'info',
                content: t('Vision Model') + ': ' + modelName + scopeSuffix + visionWarning,
            };
        }
        ...
        if (isCompactionModelCommand) {
            ...
            if (!modelName || modelName === 'clear') {
                ...
                if (context.executionMode !== 'interactive') {
                    const compactionModel = context.services.settings?.merged?.compactionModel?.trim() ||
                        t('not set (falls back to the main model)');
                    return {
                        type: 'message',
                        messageType: 'info',
                        content: t('Current compaction model: {{compactionModel}}\nUse "/model --compaction <model-id>" to set compaction model, or "/model --compaction clear" to clear the override.', { compactionModel }),
                    };
                }
                return {
                    type: 'dialog',
            ...
            const availableModels = (selector.authType
                ? config.getAvailableModelsForAuthType(selector.authType)
                : config.getAllConfiguredModels()).filter((m) => !m.fastOnly && !m.voiceOnly && !m.imageOnly && !m.visionOnly);
            if (!availableModels.some((model) => model.id === selector.modelId)) {
                return {
                    type: 'message',
                    messageType: 'error',
                    content: selector.authType
                        ? formatUnavailableModelMessage('Compaction model', selector.modelId, selector.authType, availableModels)
                        : formatUnavailableCompactionModelMessage(modelName, availableModels),
                };
            }
        ...
        if (isImageModelCommand) {
            ...
            if (!modelName) {
                if (context.executionMode !== 'interactive') {
                    const imageModel = context.services.settings?.merged?.imageModel?.trim();
                    return {
                        type: 'message',
                        messageType: 'info',
                        content: t('Current image model: {{imageModel}}\nUse "/model --image <model-id>" to set the image generation model.', {
            ...
            const availableModels = (selector.authType
                ? config.getAvailableModelsForAuthType(selector.authType)
                : config.getAllConfiguredModels()).filter((model) => model.imageOnly === true && !model.fastOnly && !model.voiceOnly);
            const matchingModels = availableModels.filter((model) => model.id === selector.modelId);
            if (matchingModels.length > 1) {
                return {
                    type: 'message',
                    messageType: 'error',
                    content: t("Image model '{{modelName}}' matches multiple configured endpoints. Run /model --image without an argument and choose the exact endpoint.", { modelName }),
                };
            }
            const matched = matchingModels[0];
            if (!matched) {
                return {
                    ...
                    messageType: 'error',
                    content: selector.authType
                        ? formatUnavailableModelMessage('Image model', selector.modelId, selector.authType, availableModels)
            ...
            if (!config.resolveImageGenerationModel(imageModel)) {
                return {
                    type: 'message',
                    messageType: 'error',
                    content: t("Image model '{{modelName}}' must declare a valid HTTPS baseUrl and credential environment variable.", { modelName }),
                };
            }
        ...
        if (modelName) {
            ...
            if (!availableModels.some((model) => model.id === parsed.modelId)) {
                return {
                    type: 'message',
                    messageType: 'error',
                    content: formatUnavailableModelMessage('Model', parsed.modelId, targetAuthType, availableModels),
                };
            }
            if (inlinePrompt) {
                ...
                if (context.executionMode === 'acp') {
                    return {
                        type: 'message',
                        messageType: 'error',
                        content: t("Inline one-shot override isn't supported in this mode — run '/model {{model}}' first, then send your prompt.", { model: modelName }),
                    };
                }
                ...
                if (scopeOverride) {
                    ...
                    return {
                        type: 'message',
                        messageType: 'error',
                        content: t("Cannot combine {{flag}} with an inline prompt. Run '/model {{flag}} {{model}}' first, then send your prompt.", { flag: scopeFlag }),
                    };
                }
                ...
                if (!sameAuthType ||
                    ...
                    return {
                        type: 'message',
                        messageType: 'error',
                        content: t("Inline one-shot override can't switch providers. '{{model}}' belongs to a different provider — run '/model {{model}}' first, then send your prompt.", { model: modelName }),
                    };
                }
                return {
                    type: 'submit_prompt',
            ...
            const effectiveModelName = await switchMainModel(config, settings, authType, modelName, scopeOverride);
            return {
                type: 'message',
                messageType: 'info',
        ...
        if (context.executionMode !== 'interactive') {
            ...
            return {
                type: 'message',
                messageType: 'info',
                content: t('Current model: {{model}}\nUse "/model <model-id>" to switch models, "/model --fast <model-id>" to set the fast model, "/model --project <model-id>" to persist to project settings, or "/model --global <model-id>" to persist to user settings.', { model: currentModel }),
            };
        }
        return {
            type: 'dialog',
