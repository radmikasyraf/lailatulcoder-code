...
import { MODELED_SYSTEM_DOMAIN, SHELL_MODEL_LAYERS, inferLayersFromProse, layerCoverage, owedLayerDimensions, parseLayerReceipts, renderShellLayerBriefList, uncoveredLayers, } from './audit-layers.js';
describe('audit-layers taxonomy', () => {
    ...
    it('renders the brief layer list from the taxonomy — one source, no drift', () => {
        ...
        expect(parseLayerReceipts(`Layer walked: ${SHELL_MODEL_LAYERS[0].id}`).size).toBe(1);
...
describe('parseLayerReceipts', () => {
    ...
    it('returns empty when the marker is absent', () => {
        expect(parseLayerReceipts('No issues found — re-read the diff.').size).toBe(0);
    });
    ...
    it('does not read a marker inside an inline code span or an indented code block', () => {
        ...
        expect(parseLayerReceipts('- `Layer walked: scope-propagation — quoted`').size).toBe(0);
        expect(parseLayerReceipts('    Layer walked: inheritance — 4-space code block')
            .size).toBe(0);
        expect(parseLayerReceipts('\tLayer walked: lexing — tab code block').size).toBe(0);
    });
    it('tracks fences the CommonMark way — a quoted marker survives no divergence', () => {
        ...
        expect(parseLayerReceipts(['```', '~~~', 'Layer walked: toctou — quoted', '~~~', '```'].join('\n')).size).toBe(0);
        // (2) a list-item fence must open (GitHub renders the marker as quoted code):
        expect(parseLayerReceipts(['The form:', '- ```', '  Layer walked: toctou — quoted', '  ```'].join('\n')).size).toBe(0);
        ...
        expect([
            ...parseLayerReceipts(['```', 'quoted', '```', 'Layer walked: scope-propagation — real'].join('\n')),
    ...
    it('defers quotation to the authority — constructs a hand-rolled scanner missed', () => {
        ...
        expect(parseLayerReceipts(['<div>', 'Layer walked: toctou — quoted', '</div>'].join('\n')).size).toBe(0);
        expect(parseLayerReceipts('\tLayer walked: lexing — quoted').size).toBe(0);
        expect(parseLayerReceipts('> > Layer walked: expansion — quoted').size).toBe(0);
    ...
    it('does not credit a marker hidden in an INLINE construct — the R4-1 residual, closed', () => {
        ...
        for (const q of hidden)
            expect(parseLayerReceipts(q).size).toBe(0);
    ...
    it('folds invisible format characters out of the entity-decoded prose view', () => {
        ...
        for (const cp of ['&#8232;', '&#8233;', '&#11;', '&#65279;']) {
            expect([
                ...parseLayerReceipts(`Layer walked: lexing — ok\nlayer${cp}walked: toctou`),
            ]).toEqual(['lexing']);
        }
        ...
        expect([
            ...parseLayerReceipts('Layer walked: lexing — real\nLay&#10;er walked: toctou'),
        ]).toEqual(['lexing']);
    ...
    it('requires the colon — a colon-less shape is not a receipt', () => {
        ...
        expect(parseLayerReceipts('Layer walked scope-propagation — no colon').size).toBe(0);
    });
...
describe('layerCoverage', () => {
    ...
    it('a fully-receipted run owes nothing', () => {
        const full = SHELL_MODEL_LAYERS.map((l) => `Layer walked: ${l.id} — examined, clear.`);
    ...
    it('keyword fallback estimates coverage on marker-less (baseline) transcripts', () => {
        ...
        expect(layerCoverage(baseline).uncovered.length).toBe(SHELL_MODEL_LAYERS.length);
...
describe('inferLayersFromProse', () => {
    it('is signal-specific, not a catch-all', () => {
        ...
        expect(inferLayersFromProse('No issues found — re-read the whole diff.').size).toBe(0);
        ...
        expect(inferLayersFromProse('Reviewed the changed files and the diff thoroughly.').size).toBe(0);
    });
    ...
    it('shares the receipt parser quotation view — an inline-code signal is dropped', () => {
        ...
        expect(inferLayersFromProse('the guard mishandles set -a expansion').size).toBeGreaterThan(0);
        expect(inferLayersFromProse('the guard mishandles `set -a` expansion').size).toBe(0);
    });
});
describe('owedLayerDimensions', () => {
    it('turns each unwalked layer into a self-explained cap entry', () => {
        ...
        for (const e of owed)
            expect(e).toMatch(/^reverse-audit layer coverage — the .+ was never walked$/);
        ...
        for (const e of owed)
            expect(e.startsWith('reverse audit — ')).toBe(false);
    });
    it('owes nothing when every layer was walked', () => {
        const full = SHELL_MODEL_LAYERS.map((l) => `Layer walked: ${l.id} — clear.`);
...
//# sourceMappingURL=audit-layers.test.js.map