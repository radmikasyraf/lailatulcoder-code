...
const { mockObserveBoundary } = vi.hoisted(() => ({
    mockObserveBoundary: vi.fn((_observation) => true),
}));
vi.mock('@lailatul-coder/lailatul-coder-core', async (importOriginal) => ({
    ...(await importOriginal()),
    observeToolResultBoundary: mockObserveBoundary,
}));
import { associateAcpToolResultArtifact, observeAcpToolResultProjection, observeAcpToolResultWire, observeHeadlessJsonToolResultWire, observeHeadlessToolResultProjection, observeHeadlessToolResultWire, } from './tool-result-boundary-diagnostics.js';
function acpUpdate(text, toolCallId = 'call-secret') {
    return {
...
function headlessMessage(content, toolCallId = 'call-secret') {
    return {
        type: 'user',
...
describe('CLI tool-result boundary diagnostics', () => {
    ...
    it('correlates ACP projection with the exact direct NDJSON writer frame', () => {
        ...
        expect(mockObserveBoundary).toHaveBeenNthCalledWith(1, expect.objectContaining({
        ...
        expect(mockObserveBoundary).toHaveBeenNthCalledWith(2, expect.objectContaining({
        ...
        expect(typeof projectionValues === 'function'
            ? projectionValues()
            : projectionValues).toEqual([
            { representation: 'acp_content', value: 'original' },
            { representation: 'acp_raw_output', value: 'original' },
        ]);
        expect(mockObserveBoundary).toHaveBeenLastCalledWith(expect.objectContaining({
            stage: 'acp_wire',
            sessionId: 'session-secret',
            toolCallId: 'call-secret',
            mutated: true,
        ...
        expect(typeof wireValues === 'function' ? wireValues() : wireValues).toEqual([
    ...
    it('correlates the delivered replay clone inside a bulk response', () => {
        ...
        observeAcpToolResultProjection(input, projected, 'session-secret', delivered);
        observeAcpToolResultWire({
            jsonrpc: '2.0',
            id: 1,
            result: {
    ...
    it('preserves per-call artifact summaries in bulk ACP responses', () => {
        ...
        observeAcpToolResultProjection(firstInput, firstProjected, 'session-secret', firstDelivered);
        associateAcpToolResultArtifact(secondInput, {
            state: 'none',
            kinds: ['image'],
        });
        observeAcpToolResultProjection(secondInput, secondProjected, 'session-secret', secondDelivered);
        observeAcpToolResultWire({
            jsonrpc: '2.0',
            id: 1,
            result: {
    ...
    it('correlates Headless projection with its writer frame', () => {
        ...
        observeHeadlessToolResultProjection(message, 'original', 'projected', 'call-secret', { state: 'reusable', kinds: ['file'] });
        const frame = '汉😀';
    ...
    it('logs one exact wire event for a batch JSON frame', () => {
        ...
        observeHeadlessToolResultProjection(first, 'first-original', 'first-projected', 'first-call', { state: 'reusable', kinds: ['file'] });
        observeHeadlessToolResultProjection(second, 'second-original', 'second-projected', 'second-call', { state: 'none', kinds: ['image'] });
        observeHeadlessJsonToolResultWire([headlessMessage('uncorrelated'), first, second], 'x'.repeat(999));
        expect(mockObserveBoundary).toHaveBeenLastCalledWith(expect.objectContaining({
            stage: 'headless_wire',
            toolCallId: undefined,
            toolCallIds: ['first-call', 'second-call'],
    ...
    it('does not retain objects when neither projection event is eligible', () => {
        ...
        expect(mockObserveBoundary).toHaveBeenNthCalledWith(1, expect.objectContaining({ mutated: false }));
        expect(mockObserveBoundary).toHaveBeenNthCalledWith(2, expect.objectContaining({ mutated: false }));
    ...
    it('correlates eligible unmutated ACP updates with the writer', () => {
        ...
        mockObserveBoundary.mockClear();
        observeAcpToolResultWire({ method: 'session/update', params: { update } }, 20);
        expect(mockObserveBoundary).toHaveBeenCalledWith(expect.objectContaining({
            stage: 'acp_wire',
            sessionId: 'session-secret',
            mutated: false,
            wireUtf8Bytes: 21,
    ...
    it('swallows observer failures at projection boundaries', () => {
        ...
        const message = headlessMessage('projected');
        expect(() => observeHeadlessToolResultProjection(message, 'original', 'projected', 'call-secret', { state: 'undecided', kinds: [] })).not.toThrow();
        expect(() => observeAcpToolResultProjection(acpUpdate('original'), acpUpdate('projected'), 'session-secret')).not.toThrow();
    });
    it('swallows observer failures at writer boundaries', () => {
        ...
        observeHeadlessToolResultProjection(headless, 'original', 'projected', 'call-secret', { state: 'undecided', kinds: [] });
        mockObserveBoundary.mockImplementation(() => {
            throw new Error('diagnostic failure');
        });
        expect(() => observeAcpToolResultWire({
            method: 'session/update',
            params: { sessionId: 'session-secret', update: acpOutput },
        }, 10)).not.toThrow();
        expect(() => observeHeadlessToolResultWire(headless, 'frame')).not.toThrow();
        expect(() => observeHeadlessJsonToolResultWire([headless], 'frame')).not.toThrow();
    });
});
//# sourceMappingURL=tool-result-boundary-diagnostics.test.js.map