...
vi.mock('./LoadingIndicator.js', () => ({
    LoadingIndicator: ({ currentLoadingPhrase, candidatesTokens, taskStartTokens, taskStartStreamingChars, showResponseTokensPerSecond, }) => (_jsxs(Text, { children: ["LoadingIndicator", currentLoadingPhrase ? `: ${currentLoadingPhrase}` : '', typeof candidatesTokens === 'number' ? `: ${candidatesTokens}` : '', typeof taskStartTokens === 'number' ? `: start ${taskStartTokens}` : '', typeof taskStartStreamingChars === 'number'
...
const renderComposer = (uiState, config = createMockConfig(), uiActions = createMockUIActions()) => render(_jsx(ConfigContext.Provider, { value: config, children: _jsx(UIStateContext.Provider, { value: uiState, children: _jsx(UIActionsContext.Provider, { value: uiActions, children: _jsx(Composer, {}) }) }) }));
