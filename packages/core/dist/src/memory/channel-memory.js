...
import { createChannelMemoryEntry, MAX_CHANNEL_MEMORY_ENTRIES_PER_REQUEST, normalizeChannelMemoryText, parseChannelMemoryDocument, parseLegacyChannelMemory, renderChannelMemoryRecall, serializeChannelMemoryDocument, } from './channel-memory-document.js';
...
function assertNoChannelMemorySecrets(content) {
...
async function cleanupLegacyAfterCommit(legacyPath, expectedBytes) {
    try {
        const currentBytes = await fs.readFile(legacyPath);
        if (legacyHash(currentBytes) !== legacyHash(expectedBytes) ||
...
function getChannelMemoryDirectory(target) {
    return path.join(Storage.getGlobalQwenDir(), 'channels', 'memory', safeChannelName(target.channelName), hashedThreadPath(target));
}
export function getChannelMemoryFilePath(target) {
    return path.join(getChannelMemoryDirectory(target), CHANNEL_MEMORY_FILE_NAME);
}
export function getLegacyChannelMemoryFilePath(target) {
    return path.join(getChannelMemoryDirectory(target), LEGACY_CHANNEL_MEMORY_FILE_NAME);
}
async function serializeMutation(directory, task) {
...
function verifyDualFileState(document, legacyBytes) {
...
async function loadChannelMemory(target) {
    ...
    const jsonBytes = initialJsonBytes ??
    ...
    if (jsonBytes !== undefined) {
        ...
        const document = parseChannelMemoryDocument(new TextDecoder('utf-8', { fatal: true }).decode(jsonBytes));
        verifyDualFileState(document, legacyBytes);
        return {
            document,
            legacyBytes,
            legacyHasEntries: legacyBytes !== undefined &&
...
async function writeChannelMemory(filePath, serialized) {
...
async function mutateChannelMemory(target, apply) {
    const filePath = getChannelMemoryFilePath(target);
    ...
    return serializeMutation(directory, async () => {
        ...
        try {
            ...
            const mutation = apply(loaded.document, loaded.document.entries.length > 0 || loaded.legacyHasEntries);
            if (!mutation.changed) {
...
export async function listChannelMemoryEntries(target) {
...
export async function getChannelMemoryRevision(target) {
...
export async function addChannelMemoryEntries(target, texts, createdBy) {
    ...
    const filePath = getChannelMemoryFilePath(target);
    return mutateChannelMemory(target, (document) => {
        const entriesByNormalizedText = new Map(document.entries.map((entry) => [
...
export async function updateChannelMemoryEntry(target, mutation) {
...
export async function removeChannelMemoryEntries(target, mutation) {
    const filePath = getChannelMemoryFilePath(target);
    return mutateChannelMemory(target, (document) => {
        ...
        const removed = document.entries.filter((entry) => requestedIds.has(entry.id));
        if (removed.length === 0) {
            return { changed: false, result: { changed: false, filePath, removed } };
        }
        document.entries = document.entries.filter((entry) => !requestedIds.has(entry.id));
        return { changed: true, result: { changed: true, filePath, removed } };
    });
}
export async function readChannelMemory(target) {
    return renderChannelMemoryRecall(await listChannelMemoryEntries(target));
}
export async function appendChannelMemory(target, text) {
    const { changed, filePath } = await addChannelMemoryEntries(target, [text]);
    return { changed, filePath };
}
export async function clearChannelMemory(target) {
    const filePath = getChannelMemoryFilePath(target);
    return mutateChannelMemory(target, (document, sourceHasEntries) => {
        if (!sourceHasEntries) {
            return { changed: false, result: { changed: false, filePath } };
        }
        document.entries = [];
...
//# sourceMappingURL=channel-memory.js.map