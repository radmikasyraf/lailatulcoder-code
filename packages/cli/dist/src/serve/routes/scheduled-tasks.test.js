...
import { SessionService, Storage, getCronFilePath, readCronTasks, } from '@lailatul-coder/lailatul-coder-core';
import { registerScheduledTasksRoutes, registerWorkspaceQualifiedScheduledTasksRoutes, scheduledTaskSessionName, } from './scheduled-tasks.js';
...
async function makeHarness(runtimeTrusted, generationGuard) {
    ...
    const getRuntime = runtimeTrusted === undefined
        ? undefined
        : () => ({
            workspaceId: 'primary',
    ...
    const cleanupSession = vi.fn(async (_runtime, sessionId) => {
...
function closeGenerationDuringCronCommit() {
    let open = true;
    let checks = 0;
    return {
        get closed() {
...
describe('scheduled-tasks routes', () => {
    ...
    it('returns 503 and removes the new session when generation closes after spawn', async () => {
        ...
        expect(h.cleanupSession).toHaveBeenCalledWith(expect.objectContaining({ workspaceCwd: h.workspace }), 'sess-1');
        expect(h.bridge.closed).toEqual(['sess-1']);
        await expect(fsp.readFile(getCronFilePath(h.workspace), 'utf8')).rejects.toThrow();
    });
    ...
    const create = (body) => request(h.app).post('/scheduled-tasks').send(body);
    ...
    it('creates and persists a task with channel delivery', async () => {
        ...
        expect(h.channelDeliveryAuthorizations.consume(h.workspace, {
    ...
    it('updates delivery via PATCH (sole field)', async () => {
        ...
        const newDelivery = {
            kind: 'channel',
            target: { channelName: 'feishu', type: 'chat', id: 'chat-2' },
        };
    ...
    it('adds delivery to a task that had none via PATCH', async () => {
        ...
        const delivery = {
            kind: 'channel',
            target: { channelName: 'feishu', type: 'user', id: 'user-9' },
        };
        const patch = await request(h.app)
            .patch(`/scheduled-tasks/${id}`)
            .send({ delivery });
    ...
    it('returns 500 when the tasks file is corrupt', async () => {
        ...
        expect(res.body.error).toBe('Failed to read scheduled tasks (the tasks file may be corrupt)');
    ...
    it('records a manual run: advances lastFiredAt and appends a manual run', async () => {
        const created = await create({ cron: '0 9 * * *', prompt: 'p' });
        const id = created.body.id;
        const before = created.body.lastFiredAt;
    ...
    it('404s a manual run for an unknown task', async () => {
        const res = await request(h.app).post('/scheduled-tasks/does-not-exist/run');
        expect(res.status).toBe(404);
        expect(res.body.code).toBe('task_not_found');
    });
    ...
    it('reports a legacy guarded task (still-on-disk `condition`) as disabled on GET, fail-closed', async () => {
        ...
        const res = await request(h.app).get('/scheduled-tasks');
        expect(res.status).toBe(200);
        const legacy = res.body.tasks.find((t) => t.id === 'legacy-guard');
        ...
        const ok = res.body.tasks.find((t) => t.id === normal.body.id);
    ...
    it('refuses a manual run for a legacy guarded task (409 task_legacy_unsupported, no record)', async () => {
        ...
        const list = await request(h.app).get('/scheduled-tasks');
        const t = list.body.tasks.find((x) => x.id === 'legacy-run');
    ...
    it('refuses to enable a legacy guarded task via PATCH (409 task_legacy_unsupported)', async () => {
        ...
        const list = await request(h.app).get('/scheduled-tasks');
        const t = list.body.tasks.find((x) => x.id === 'legacy-enable');
    ...
    it('revokes channel delivery when a manual run consumes a one-shot task', async () => {
        ...
        const res = await request(h.app).post(`/scheduled-tasks/${created.body.id}/run`);
        expect(res.status).toBe(200);
        const firedAt = res.body.lastFiredAt + 60_000;
        expect(h.channelDeliveryAuthorizations.consume(h.workspace, {
...
function makeStubRegistry(runtimes) {
    const asRuntime = (r) => r;
    ...
    return {
        ...
        getEntryByWorkspaceId: (id) => entries.find((entry) => entry.workspaceId === id),
        getEntryByWorkspaceCwd: (cwd) => entries.find((entry) => entry.workspaceCwd === cwd),
        getManagedEntryByWorkspaceId: (id) => entries.find((entry) => entry.workspaceId === id),
        getManagedEntryByWorkspaceCwd: (cwd) => entries.find((entry) => entry.workspaceCwd === cwd),
        getByWorkspaceId: (id) => {
...
async function makeQualifiedHarness() {
    const scratch = await fsp.mkdtemp(path.join(os.tmpdir(), 'sched-wsq-'));
    Storage.setRuntimeBaseDir(scratch);
    const mkRuntime = async (name, trusted) => {
...
describe('workspace-qualified scheduled-tasks routes', () => {
    ...
    const qualified = (id) => `/workspaces/${id}/scheduled-tasks`;
    const cronFilePath = (runtime) => Storage.runWithResolvedRuntimeBaseDir(runtime.sessionRuntimeBaseDir, () => getCronFilePath(runtime.workspaceCwd));
    it('creates a task in the targeted workspace, isolated from the primary', async () => {
        ...
        const secList = await request(h.app).get(qualified(h.secondary.workspaceId));
    ...
    it('writes to the targeted workspace’s own cron file on disk', async () => {
        await request(h.app)
            .post(qualified(h.secondary.workspaceId))
            .send({ cron: '0 9 * * *', prompt: 'p' });
        const onDisk = JSON.parse(await fsp.readFile(cronFilePath(h.secondary), 'utf-8'));
        expect(onDisk).toHaveLength(1);
        // Neither the primary runtime nor the process-global fallback was touched.
        await expect(fsp.readFile(cronFilePath(h.primary), 'utf-8')).rejects.toThrow();
        await expect(fsp.readFile(getCronFilePath(h.secondary.workspaceCwd), 'utf-8')).rejects.toThrow();
    });
    ...
    it('holds the Conversations activity lease for the whole delete handler', async () => {
        ...
        const originalClose = h.secondary.bridge.closeSession.bind(h.secondary.bridge);
        vi.spyOn(h.secondary.bridge, 'closeSession').mockImplementation(async (sessionId) => {
...
//# sourceMappingURL=scheduled-tasks.test.js.map