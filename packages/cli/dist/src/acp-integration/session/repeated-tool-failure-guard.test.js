 ...
 */
import { ToolErrorType, } from '@lailatul-coder/lailatul-coder-core';
...
function reduce(state, observations, options = {}) {
...
describe('repeated tool failure guard', () => {
    ...
    it('requires eight failures across at least two batches before warning', () => {
        const first = reduce(createRepeatedToolFailureGuardState(), Array.from({ length: 8 }, (_, index) => observation({ callId: `call-${index}` })));
    ...
    it('stops only after the next complete matching batch', () => {
        const first = reduce(createRepeatedToolFailureGuardState(), Array.from({ length: 4 }, (_, index) => observation({ callId: `first-${index}` })));
        const warned = reduce(first.state, Array.from({ length: 4 }, (_, index) => observation({ callId: `second-${index}` })));
        ...
        expect(reduce(stopped.state, [observation({ callId: 'ignored' })]).kind).toBe('none');
    });
    it.each([
        ['shadow', 'would_warn', 'would_stop'],
        ['warn', 'warn', 'would_stop'],
        ['enforce', 'warn', 'stop'],
    ])('applies %s mode without changing detection semantics', (mode, warningKind, stopKind) => {
        const first = reduce(createRepeatedToolFailureGuardState(), Array.from({ length: 4 }, (_, index) => observation({ callId: `first-${index}` })), { mode });
        const warning = reduce(first.state, Array.from({ length: 4 }, (_, index) => observation({ callId: `second-${index}` })), { mode });
        const stop = reduce(warning.state, [observation({ callId: 'post-warning' })], { mode });
    ...
    ])('resets a streak for a %s outcome', (_label, terminalStatus, executionStatus) => {
    ...
    it('keeps counting a failure key when other tools succeed in the same batch', () => {
        const failingShell = (callId) => observation({
            callId,
            policyToolName: 'run_shell_command',
            executionErrorType: ToolErrorType.EXECUTION_FAILED,
        });
        const first = reduce(createRepeatedToolFailureGuardState(), Array.from({ length: 4 }, (_, index) => failingShell(`first-${index}`)));
    ...
    it('preserves a failure streak across successful batches from other tools', () => {
        ...
        const second = reduce(unrelatedSuccess.state, Array.from({ length: 4 }, (_, index) => failingShell(`second-${index}`)));
    ...
    ])('resets for boundary condition %s', (options, reason) => {
        const tracked = reduce(createRepeatedToolFailureGuardState(), [
            observation(),
        ]);
    ...
    it('downgrades enforcement after an unknown execution contract', () => {
        ...
        const first = reduce(invalid.state, Array.from({ length: 4 }, (_, index) => observation({ callId: `first-${index}` })));
        const warning = reduce(first.state, Array.from({ length: 4 }, (_, index) => observation({ callId: `second-${index}` })));
    ...
    it.each([
        [
            'success/error',
            { terminalStatus: 'success', executionStatus: 'error' },
            'contract_violation',
        ],
        [
            'success/cancelled',
    ...
    ])('downgrades enforcement for %s', (_label, overrides, reason) => {
        const result = reduce(createRepeatedToolFailureGuardState(), [
    ...
    it.each([
    ...
    ])('classifies mixed ineligible batches deterministically when %s', (_label, order) => {
        const tracked = reduce(createRepeatedToolFailureGuardState(), [
            observation(),
        ]);
        const byOutcome = {
        ...
        const result = reduce(tracked.state, order.map((outcome) => byOutcome[outcome]));
        expect(result).toMatchObject({
            kind: 'reset',
            reason: 'cancelled',
            state: { enforcementDisabled: true },
    ...
    it.each(['error', 'success'])('fails open when a future execution status reaches a terminal %s observation', (terminalStatus) => {
        const result = reduce(createRepeatedToolFailureGuardState(), [
            observation({
                terminalStatus,
    ...
    it('uses fixed privacy-safe reminder and stop text', () => {
        expect(REPEATED_TOOL_FAILURE_REMINDER).toBe('System: the same tool execution has failed repeatedly for the same classified reason. Do not repeat the same approach. Inspect the returned result, change the approach or required preconditions, or explain the blocker.');
        expect(REPEATED_TOOL_FAILURE_STOP_MESSAGE).toBe('Automatic continuation stopped because the same tool execution failure continued after a corrective reminder. New user input is required to continue.');
    });
});
//# sourceMappingURL=repeated-tool-failure-guard.test.js.map