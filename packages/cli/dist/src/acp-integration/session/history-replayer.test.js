...
describe('HistoryReplayer', () => {
    let mockContext;
    let sendUpdateSpy;
    let setActiveRecordIdSpy;
    let sentUpdateContexts;
    let replayer;
    beforeEach(() => {
        let activeRecordId = null;
        let activeRecordTimestamp;
        sentUpdateContexts = [];
        sendUpdateSpy = vi.fn().mockResolvedValue(undefined);
        setActiveRecordIdSpy = vi.fn((id, timestamp) => {
            activeRecordId = id;
    ...
    const toEpochMs = (ts) => new Date(ts).getTime();
    const replayMeta = (record, extra = {}) => ({
        ...extra,
        timestamp: toEpochMs(record.timestamp),
        qwenTranscript: { sourceRecordIds: [record.uuid] },
    });
    const sentUpdates = () => sendUpdateSpy.mock.calls.map((call) => call[0]);
    ...
    describe('function call replay', () => {
        ...
        it('should fail dangling function calls after replay completes', async () => {
            ...
            expect(setActiveRecordIdSpy).toHaveBeenCalledWith(record.uuid, record.timestamp);
        ...
        it('should match a pending function call with its result on the next page', async () => {
            ...
            sendUpdateSpy.mockClear();
            const resultRecord = createToolResultRecord('read_file', 'File contents here');
        ...
        it('should expose pending function calls when replayPage throws mid-page', async () => {
            ...
            sendUpdateSpy.mockImplementation(async (update) => {
                if (update['sessionUpdate'] === 'agent_message_chunk') {
                    throw new Error('replay failed');
                }
            });
            await expect(replayer.replayPage([record], { finalizeDangling: false })).rejects.toThrow('replay failed');
        ...
        it('should fail dangling calls before rethrowing replay errors', async () => {
            ...
            await expect(replayer.replay([danglingRecord, failingRecord])).rejects.toThrow('replay failed');
            const updates = sentUpdates();
            expect(updates.map((update) => update['sessionUpdate'])).toEqual([
                'tool_call',
                'user_message_chunk',
        ...
        it('should throw dangling errors and continue failing later dangling calls', async () => {
            ...
            await expect(replayer.replay([record])).rejects.toThrow('first synthetic failure failed');
            const updates = sentUpdates();
            expect(updates.map((update) => update['sessionUpdate'])).toEqual([
                'tool_call',
                'tool_call',
        ...
        it('should aggregate replay and dangling cleanup errors', async () => {
            ...
            expect(caughtError.message).toBe('Replay and dangling-cleanup both failed');
            expect(caughtError.errors.map((error) => error instanceof Error ? error.message : String(error))).toEqual(['replay failed', 'dangling cleanup failed']);
        });
    ...
    describe('tool result replay', () => {
        ...
        it('should replay structured artifacts persisted by the recorder', async () => {
            ...
            try {
                ...
                const jsonl = readFileSync(join(projectDir, 'chats', `${sessionId}.jsonl`), 'utf8');
                expect(jsonl).not.toContain('/private/tool-result.txt');
                const storedRecord = JSON.parse(jsonl.trim());
    ...
    describe('goal card replay', () => {
        const goalRecord = (...outputHistoryItems) => ({
            uuid: 'goal-uuid',
            parentUuid: null,
            sessionId: 'test-session',
            timestamp: new Date().toISOString(),
        ...
        const goalStatuses = () => sentUpdates()
            .map((u) => u['_meta'])
        ...
        it('still replays non-goal output items as agent text', async () => {
            await replayer.replay([
                goalRecord({ type: 'goal_status', kind: 'set', condition: 'ship it' }, { type: 'assistant', text: 'hello' }),
            ]);
        ...
        it('survives a broken stderr instead of abandoning the transcript', async () => {
            ...
            record.systemPayload['outputHistoryItems'] = [
    ...
    describe('qwen.session.recordId stamping', () => {
        it('should stamp persisted updates with activeRecordId for user messages', async () => {
            ...
            expect(setActiveRecordIdSpy).toHaveBeenCalledWith(userRecord.uuid, userRecord.timestamp);
        });
        it('should stamp persisted updates with activeRecordId for assistant messages', async () => {
            ...
            expect(setActiveRecordIdSpy).toHaveBeenCalledWith(assistantRecord.uuid, assistantRecord.timestamp);
        });
    });
});
//# sourceMappingURL=history-replayer.test.js.map