...
import { createWorkspaceTrustReconciler, } from './workspace-trust-reconciler.js';
...
function policy(revision, rules) {
    return {
        revision,
...
describe('workspace trust reconciler', () => {
    it.each(['managed-scratch', 'live-conversation'])('does not downgrade a %s runtime through folder trust policy', async (provenance) => {
        ...
        expect(registry.getManagedByWorkspaceCwd(scratch.workspaceCwd)).toBe(scratch);
        expect(scratch.trusted).toBe(true);
        expect(registry.getManagedEntryByWorkspaceCwd(scratch.workspaceCwd)
            ?.appliedRevision).toBe('two');
    ...
    it('closes and starts draining every trust decrease before disposing any runtime', async () => {
        ...
        const drained = [];
        const drainRuntime = vi.fn(async (runtime) => {
            expect(registry.list()).toEqual([]);
            expect(registry.listEntries().every((entry) => entry.current?.guard.closed)).toBe(true);
        ...
        const reconciler = createWorkspaceTrustReconciler({
            registry,
            readLatestSnapshot: async () => nextPolicy,
            buildRuntime: async ({ entry, trusted, generationGuard }) => makeRuntime(entry.workspaceCwd, {
    ...
    it('applies a newly observed revision to every workspace before returning', async () => {
        ...
        expect(registry.listEntries().map((entry) => entry.appliedRevision)).toEqual(['three', 'three']);
    });
    it('keeps an untrusted fallback active when a grant build fails', async () => {
        ...
        const reconciler = createWorkspaceTrustReconciler({
            ...
            buildRuntime: async ({ entry, trusted, generationGuard }) => {
                if (trusted)
                    throw new Error('trusted build failed');
    ...
    it('disposes an untrusted fallback when the latest policy cannot be read', async () => {
        ...
        expect(disposeRuntime).toHaveBeenCalledTimes(2);
        expect(registry.primaryEntry.state).toBe('blocked');
        expect(registry.primaryEntry.applyError).toContain('Trusted runtime and untrusted fallback both failed to build.');
    });
    it('blocks only the affected entry when containment fails', async () => {
        ...
        const reconciler = createWorkspaceTrustReconciler({
            ...
            disposeRuntime: async (runtime) => {
                if (runtime.workspaceCwd === '/secondary')
    ...
    it('retries containment before recovering a blocked entry', async () => {
        ...
        const buildRuntime = vi.fn(async ({ entry, trusted, generationGuard, }) => makeRuntime(entry.workspaceCwd, {
            workspaceId: entry.workspaceId,
            primary: entry.primary,
            trusted,
            trustMaterialization: String(trusted),
    ...
    it('does not repeat containment after the previous runtime was disposed', async () => {
        ...
        const buildRuntime = vi
            ...
            .mockImplementation(async ({ entry, trusted, generationGuard }) => makeRuntime(entry.workspaceCwd, {
            workspaceId: entry.workspaceId,
            primary: entry.primary,
            trusted,
            trustMaterialization: String(trusted),
...
//# sourceMappingURL=workspace-trust-reconciler.test.js.map