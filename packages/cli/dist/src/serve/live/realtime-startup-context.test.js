...
import { buildRealtimeStartupContext, truncateRealtimeTextToTokenBudget, } from './realtime-startup-context.js';
const sessionData = vi.hoisted(() => new Map());
const sessionLists = vi.hoisted(() => new Map());
...
function record(type, text, index, cwd) {
    return {
        uuid: `record-${index}`,
...
describe('buildRealtimeStartupContext', () => {
    it('copies the Codex current-thread, recent-work, workspace, and notes sections', async () => {
        ...
        const runtime = { workspaceCwd: repo };
    ...
    it('omits startup context when every Codex source is empty', async () => {
        ...
        mkdirSync(home);
        const runtime = { workspaceCwd: root };
        const workspaceRegistry = {
            list: () => [runtime],
            listAll: () => [runtime],
        };
        await expect(buildRealtimeStartupContext({
            runtime,
            workspaceRegistry,
            sessionId: 'missing',
            currentCwd,
    ...
    it('reads the current and recent catalogs from each runtime base', async () => {
        ...
        expect(sessionServiceConstructions).toEqual(expect.arrayContaining([
            {
...
//# sourceMappingURL=realtime-startup-context.test.js.map