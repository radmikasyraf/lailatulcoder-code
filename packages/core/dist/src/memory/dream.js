...
async function runDreamByAgent(projectRoot, config, abortSignal, options = {}) {
    const result = await planManagedAutoMemoryDreamByAgent(config, projectRoot, abortSignal, { suppressChatRecording: options.suppressChatRecording });
...
export async function runManagedAutoMemoryDream(projectRoot, now = new Date(), config, abortSignal, options = {}) {
    await ensureAutoMemoryScaffold(projectRoot, now);
    const t0 = Date.now();
    if (!config) {
        throw new Error('Managed auto-memory dream requires config for forked-agent execution.');
    }
    ...
    logMemoryDream(config, new MemoryDreamEvent({
...
async function updateDreamMetadataResult(projectRoot, now, touchedTopics, sessionId) {
 ...
 */
export async function writeDreamManualRunToMetadata(projectRoot, sessionId, now = new Date()) {
