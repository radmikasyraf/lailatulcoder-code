...
const { a1JsonMock, a1JsonOnceMock, a1OnceMock, ensureAuthMock, gitMock, gitRawMock, } = vi.hoisted(() => ({
    a1JsonMock: vi.fn(),
...
import { AonePartialPostError, aoneReader, mrPresubmitFacts, parseRemoteUrl, submitAoneReview, } from './aone.js';
...
describe('parseRemoteUrl hardening', () => {
    it('discards an explicit port instead of folding it into the path', () => {
        expect(parseRemoteUrl('https://gitlab.alibaba-inc.com:8443/solo')).toBeNull();
        expect(parseRemoteUrl('https://gitlab.alibaba-inc.com:8443/g/p.git')).toEqual({
    ...
    it('strips a query string / fragment (credential-bearing channel)', () => {
        expect(parseRemoteUrl('https://h.example/g/p?private_token=SECRET')).toEqual({ host: 'h.example', owner: 'g', repo: 'p', groupPath: 'g/p' });
    ...
    it('consumes multi-@ userinfo whole (no cleartext residue)', () => {
        ...
        expect(parseRemoteUrl('https://ci-user:SECRET1@SECRET2@code.alibaba-inc.com/g/p')).toEqual({
            host: 'code.alibaba-inc.com',
            owner: 'g',
            repo: 'p',
            groupPath: 'g/p',
        });
        expect(parseRemoteUrl('https://ci-user:S1@S2@S3@code.alibaba-inc.com/g/p')).toEqual({
            host: 'code.alibaba-inc.com',
            owner: 'g',
            repo: 'p',
            groupPath: 'g/p',
    ...
    it('agrees with GIT on a `:`/`/`-bearing scp userinfo (fails closed)', () => {
        ...
        expect(parseRemoteUrl('ci-user:/token-with-slash@code.alibaba-inc.com:g/p.git')).toBeNull();
        // The plain token-bearing scp shape too: git reads host `oauth2`, so
        // the residue fails closed here as well.
        expect(parseRemoteUrl('oauth2:SECRET@code.alibaba-inc.com:g/p.git')).toBeNull();
    ...
    it('fails closed on a single-segment multi-@ origin without leaking', () => {
        ...
        expect(parseRemoteUrl('https://ci-user:SECRET1@SECRET2@code.alibaba-inc.com/solo')).toBeNull();
    });
    it('fails closed when a `/`-bearing secret leaves the @ in path territory', () => {
        ...
        expect(parseRemoteUrl('https://user:sec/ret@code.example.com/g/p')).toBeNull();
        expect(parseRemoteUrl('https://user:S1/S2@host.example:8443/project')).toBeNull();
        expect(parseRemoteUrl('https://gitlab.alibaba-inc.com/x//y@g/p')).toBeNull();
    });
    it('keeps a query-borne @ from fabricating coordinates', () => {
        ...
        expect(parseRemoteUrl('https://gitlab.alibaba-inc.com/group/proj?private_token=ab@cd:8443/x/y')).toEqual({
            host: 'gitlab.alibaba-inc.com',
            owner: 'group',
            repo: 'proj',
        ...
        expect(parseRemoteUrl('git@gitlab.alibaba-inc.com:group/proj?private_token=ab@cd:8443/x/y')).toEqual({
            host: 'gitlab.alibaba-inc.com',
            owner: 'group',
            repo: 'proj',
            groupPath: 'group/proj',
        ...
        // into a host swap.
        expect(parseRemoteUrl('https://gitlab.alibaba-inc.com/junk@gitlab.alibaba-inc.com:g/p')).toBeNull();
    });
});
describe('aoneReader.resolveRepo', () => {
    ...
    it("quotes git's real error line, not the execFileSync preamble", () => {
        gitMock.mockImplementation(() => {
            throw new Error('Command failed: git remote get-url origin\n' +
                "error: No such remote 'origin'\n");
        });
        expect(() => aoneReader.resolveRepo()).toThrow(/no `origin` remote \(error: No such remote 'origin'\)/);
    });
    ...
    it('refuses an origin OUTSIDE the Aone host family (round-12 witness)', () => {
        ...
        expect(() => aoneReader.resolveRepo()).toThrow(/not the Aone host family — run from inside an Aone clone/);
    });
...
describe('aoneReader.getCommentBody', () => {
    ...
    it('throws on a missing id — not an empty string', () => {
        ...
        expect(() => aoneReader.getCommentBody('inline', 99, 'g/p', 5)).toThrow(/comment 99 not found in MR 5/);
    });
    it('requires --pr for every kind (Aone addresses comments per-MR)', () => {
        expect(() => aoneReader.getCommentBody('inline', 1, 'g/p', undefined)).toThrow(/pass `--pr <mr id>`/);
    });
});
...
describe('aoneReader.getFetchMeta / fetchHeadRefSpec', () => {
    ...
    it('uses the merge-requests refspec with the global id', () => {
        expect(aoneReader.fetchHeadRefSpec(29295886)).toBe('refs/merge-requests/29295886/head');
    });
    it('throws when mr view returns no mergeRequest', () => {
        a1JsonMock.mockReturnValue({});
        expect(() => aoneReader.getFetchMeta(7, 'g/p')).toThrow(/no mergeRequest for #7/);
    });
});
describe("mrPresubmitFacts (the presubmit gate's Aone seam)", () => {
    ...
    it('reads the author account and the live head from ONE mr view fetch', () => {
        ...
        expect(a1JsonMock).toHaveBeenCalledWith('repo', 'mr', 'view', '29295886', '--repo', 'maxcompute/odps_src');
    });
...
describe('aoneReader.fetchDiff', () => {
    ...
    it('fetches the MR ref, merge-bases, and diffs via gitRaw (byte-faithful)', () => {
        ...
        expect(gitRawMock).toHaveBeenCalledWith(...PINNED_DIFF_CONFIG, 'diff', ...PINNED_DIFF_FLAGS, expect.stringMatching(/^base-sha\.\.refs\/heads\/__qwen-review-diff-7-\d+$/));
        ...
        expect(gitMock).toHaveBeenCalledWith('fetch', 'origin', expect.stringMatching(/^\+refs\/merge-requests\/7\/head:__qwen-review-diff-7-\d+$/));
        ...
        expect(gitMock).toHaveBeenCalledWith('fetch', 'origin', '+refs/heads/master:refs/remotes/origin/master');
        // The throwaway ref is cleaned up.
        expect(gitMock).toHaveBeenCalledWith('branch', '-D', expect.stringMatching(refRe));
    });
    it('refuses a dash-leading target branch from the MR metadata', () => {
        ...
        expect(() => aoneReader.fetchDiff(7, 'g/p')).toThrow(/refusing target branch "--upload-pack=\/tmp\/evil"/);
        expect(gitMock).not.toHaveBeenCalledWith('fetch', 'origin', expect.stringContaining('--upload-pack'));
    ...
    it('refuses anything that is not a plain branch name (allowlist)', () => {
        ...
        for (const target of [
            ...
            expect(() => aoneReader.fetchDiff(7, 'g/p')).toThrow(/not a plain branch name/);
        }
        expect(gitRawMock).not.toHaveBeenCalled();
    });
    it('falls back to the head first-parent when merge-base fails', () => {
        ...
        expect(gitRawMock).toHaveBeenCalledWith(...PINNED_DIFF_CONFIG, 'diff', ...PINNED_DIFF_FLAGS, expect.stringMatching(/^refs\/heads\/__qwen-review-diff-7-\d+~1\.\.refs\/heads\/__qwen-review-diff-7-\d+$/));
        ...
        expect(stderrCalls.some((c) => String(c[0]).includes('no merge-base with origin/master'))).toBe(true);
        expect(stderrCalls.some((c) => String(c[0]).includes("a multi-commit MR's diff may be incomplete"))).toBe(true);
    });
    it('refuses to diff from a clone of a DIFFERENT repo', () => {
        ...
        expect(() => aoneReader.fetchDiff(7, 'g/p')).toThrow(/not g\/p — run from inside a clone of the target repo/);
        expect(gitRawMock).not.toHaveBeenCalled();
    });
    ...
    it('accepts the web/git host alias as the origin', () => {
        ...
        gitMock.mockImplementation((...args) => {
            if (args[0] === 'merge-base')
                return 'base-sha';
            if (args[0] === 'remote')
                return 'https://code.alibaba-inc.com/g/p.git';
            return '';
        });
        gitRawMock.mockReturnValue(Buffer.from('d', 'latin1'));
    ...
    it('refuses a different NESTED group via the MR detailUrl full path', () => {
        ...
        expect(() => aoneReader.fetchDiff(7, 'sub/app')).toThrow(/not sub\/app — run from inside a clone of the target repo/);
        expect(gitRawMock).not.toHaveBeenCalled();
    });
    ...
    it('merge-bases against the QUALIFIED tracking ref (no shadow tag)', () => {
        ...
        gitRawMock.mockReturnValue(Buffer.from('d', 'latin1'));
        aoneReader.fetchDiff(7, 'g/p');
        expect(gitMock).toHaveBeenCalledWith('merge-base', 'refs/remotes/origin/master', expect.stringMatching(/^refs\/heads\/__qwen-review-diff-7-\d+$/));
...
describe('submitAoneReview (the a1 write path)', () => {
    function mrView(head) {
        ...
        // could not express.
        a1JsonMock.mockImplementation((...args) => {
    ...
    it('posts inline first, summary last — one comment create per finding', () => {
        ...
        const calls = a1JsonOnceMock.mock.calls.map((c) => c);
        ...
        expect(a1JsonMock).toHaveBeenCalledWith('repo', 'mr', 'view', '7', '--repo', 'g/p');
    ...
    it('APPROVE runs the native approve AFTER the summary lands', () => {
        ...
        expect(a1OnceMock).toHaveBeenCalledWith('repo', 'mr', 'approve', '7', '--repo', 'g/p');
        ...
        expect(a1OnceMock.mock.invocationCallOrder[0]).toBeGreaterThan(Math.max(...a1JsonOnceMock.mock.invocationCallOrder));
    ...
    it('refuses BEFORE writing when the head drifted', () => {
        expect(() => submitAoneReview(req({ commitId: 'stale-sha' }))).toThrow(/the MR head moved/);
    ...
    it('refuses WHOLE, before any write, when a message overruns the a1 argv limit', () => {
        ...
        expect(caught.message).toContain('over the 131072-byte single-argument limit');
        ...
        expect(caught.message).toContain('the USER can post them by hand');
    ...
    it('guards an oversized INLINE comment too, naming it', () => {
        ...
        let caught;
        try {
            submitAoneReview(req({ comments: [{ path: 'big.ts', line: 1, body: huge }] }));
        }
        catch (err) {
            caught = err;
        }
        expect(caught.message).toContain('inline comment 1');
        expect(a1JsonOnceMock).not.toHaveBeenCalled();
    });
    ...
    it('an empty-stderr failure reports EXIT FACTS, never the argv-bearing message', () => {
        ...
        a1JsonOnceMock.mockImplementationOnce(() => {
            throw Object.assign(new Error('Command failed: a1 repo mr comment create --message the body text\nmore body'), { status: undefined, signal: 'SIGTERM' });
        });
        let caught;
        try {
            submitAoneReview(req({ comments: [{ path: 'a.ts', line: 3, body: 'b' }] }));
        }
        catch (err) {
            caught = err;
        }
    ...
    it('an empty-body REQUEST_CHANGES still posts the blocking header', () => {
        ...
        const result = submitAoneReview(req({ event: 'REQUEST_CHANGES', body: '' }));
        expect(result.summaryPosted).toBe(true);
        const calls = a1JsonOnceMock.mock.calls.map((c) => c);
    ...
    it('reports a1 stderr as the cause, not a line of the comment body', () => {
        ...
        a1JsonOnceMock.mockImplementationOnce(() => {
            const err = Object.assign(new Error('Command failed: a1 repo mr comment create --message line one\nline two of the body'), { stderr: 'HTTP 422: real a1 error\n' });
            throw err;
        });
        let caught;
        try {
...
//# sourceMappingURL=aone.test.js.map