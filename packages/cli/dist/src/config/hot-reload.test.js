...
import { registerMcpHotReload, mcpServersEqual, mcpGatingEqual, } from './hot-reload.js';
import { loadMcpApprovals, resetMcpApprovalsForTesting, } from './mcpApprovals.js';
...
describe('mcpServersEqual', () => {
    ...
    it('detects a nested config field change', () => {
        expect(mcpServersEqual({ x: { command: 'a' } }, { x: { command: 'b' } })).toBe(false);
    });
    ...
    it('treats args array reorder as NOT equal (arg order is semantic)', () => {
        expect(mcpServersEqual({ x: { command: 'c', args: ['--a', '--b'] } }, { x: { command: 'c', args: ['--b', '--a'] } })).toBe(false);
    });
});
describe('mcpGatingEqual', () => {
    it('is order-insensitive across the three lists', () => {
        expect(mcpGatingEqual({ allowed: ['a', 'b'] }, { allowed: ['b', 'a'] })).toBe(true);
    });
    it('treats undefined and [] as equal', () => {
        expect(mcpGatingEqual({ excluded: undefined }, { excluded: [] })).toBe(true);
    });
    it('detects a member added to any list', () => {
        expect(mcpGatingEqual({ pending: ['a'] }, { pending: ['a', 'b'] })).toBe(false);
...
function makeFakeConfig(cwd, state) {
    const reinitializeMcpServers = vi.fn(async () => { });
    const setExcludedMcpServers = vi.fn((v) => {
        state.gating.excluded = v;
    });
    const setAllowedMcpServers = vi.fn((v) => {
        state.gating.allowed = v;
    });
    const setPendingMcpServers = vi.fn((v) => {
...
describe('registerMcpHotReload', () => {
    ...
    beforeEach(() => {
        ...
        process.env['QWEN_CODE_MCP_APPROVALS_PATH'] = path.join(cwd, 'mcpApprovals.json');
        resetMcpApprovalsForTesting();
        unsubscribe = vi.fn();
        watcher = {
            addChangeListener: vi.fn((l) => {
                listener = l;
                return unsubscribe;
            }),
        };
    ...
    it('returns the watcher unsubscribe fn', () => {
        ...
        const dispose = registerMcpHotReload(watcher, settings, fc.config, undefined);
    ...
    it('reconciles on an admission-list-only change (mcp.excluded), servers unchanged', async () => {
        ...
        expect(fc.setExcludedMcpServers.mock.invocationCallOrder[0]).toBeLessThan(fc.reinitializeMcpServers.mock.invocationCallOrder[0]);
    });
    ...
    it('surfaces a user-visible LogError when reconcile throws', async () => {
        const fc = makeFakeConfig(cwd, { settingsMcp: {}, gating: {} });
        fc.reinitializeMcpServers.mockRejectedValueOnce(new Error('reconcile boom'));
        registerMcpHotReload(watcher, settings, fc.config, undefined);
        const spy = vi.fn();
        ...
        try {
            ...
            expect(String(spy.mock.calls[0][0])).toMatch(/Failed to reload MCP server settings/);
        }
        finally {
    ...
    it('re-emits when an edit invalidates a previously rejected gated server', async () => {
        ...
        await loadMcpApprovals().setState(cwd, 'ws', { command: 'ws', scope: 'workspace' }, 'rejected');
        registerMcpHotReload(watcher, settings, fc.config, undefined);
        const spy = vi.fn();
        appEvents.on(AppEvent.McpPendingApprovalChanged, spy);
        try {
    ...
    it('does NOT re-emit for an unrelated edit while a server stays rejected', async () => {
        const ws = { command: 'ws', scope: 'workspace' };
        const fc = makeFakeConfig(cwd, {
            settingsMcp: { ws },
            gating: { pending: ['ws'] },
        });
