...
const defaultRunCommand = async (command, args, signal) => {
...
export function tokenizeCommand(command) {
    ...
    if (quote)
        throw new Error('Unterminated quote in uploadCommand.');
    if (started)
        tokens.push(current);
    return tokens;
}
...
function normalizeKeyPrefix(raw) {
    ...
    if (!prefix) {
        throw new Error('artifact.host.keyPrefix must not be empty or "/" after stripping slashes.');
    }
    if (/[#?%\s]/.test(prefix)) {
        throw new Error('artifact.host.keyPrefix must not contain #, ?, %, or whitespace.');
    }
    return prefix;
}
/**
...
export class HostPublisher {
    ...
    async publish(input, signal) {
        ...
        if (!uploadCommand) {
            throw new Error('artifact.host.uploadCommand is not configured (set it to e.g. "aws s3 cp {file} s3://bucket/{key}").');
        }
        if (!urlTemplate) {
            throw new Error('artifact.host.urlTemplate is not configured (set it to e.g. "https://bucket.example.com/{key}").');
        }
        if (!uploadCommand.includes('{file}')) {
            throw new Error('artifact.host.uploadCommand must include the {file} placeholder (the local HTML path to upload).');
        }
        if (!uploadCommand.includes('{key}')) {
            throw new Error('artifact.host.uploadCommand must include the {key} placeholder so the upload destination matches the returned URL.');
        }
        if (!urlTemplate.includes('{key}')) {
            throw new Error('artifact.host.urlTemplate must include the {key} placeholder (the remote object key).');
        }
        if (urlTemplate.includes('{file}')) {
            throw new Error('artifact.host.urlTemplate must not include {file}; only {key} is supported.');
        }
        ...
        try {
            const argv = tokenizeCommand(uploadCommand).map((t) => substitute(t, file, key));
...
//# sourceMappingURL=host-publisher.js.map