...
import { assertExactConversationRoot, ConversationWorkspace, getConversationRootPath, revalidateConversationRoot, } from './conversation-workspace.js';
...
vi.mock('../../utils/conversation-directory-identity.js', async (importOriginal) => {
    const actual = await importOriginal();
    return {
        ...actual,
        inspectConversationDirectoryIdentity: async (...args) => {
            const identity = await actual.inspectConversationDirectoryIdentity(...args);
...
async function tempHome() {
    const home = await mkdtemp(join(realpathSync.native(tmpdir()), 'qwen-live-home-'));
    cleanup.push(home);
    return home;
}
describe('Live conversation workspace root', () => {
    ...
    it('rejects symlink, non-directory, permissive, and foreign-owned roots', async () => {
        ...
        await expect(new ConversationWorkspace({ homeDir: fileHome }).getRoot()).rejects.toThrow(/non-symlink/);
        ...
        await expect(new ConversationWorkspace({ homeDir: permissiveHome }).getRoot()).rejects.toThrow(/only to its owner/);
        ...
        try {
            await expect(new ConversationWorkspace({ homeDir: ownerHome }).getRoot()).rejects.toThrow(/owned by the daemon user/);
        }
        finally {
    ...
    it('preserves Live filesystem errors while standalone keeps root scope', async () => {
        ...
        await rename(standaloneRoot.configuredRoot, `${standaloneRoot.configuredRoot}-old`);
        await mkdir(standaloneRoot.configuredRoot, { mode: 0o700 });
        await expect(standaloneWorkspace.inspectStandaloneDirectory('standalone')).rejects.toMatchObject({
    ...
    it('accepts only the exact configured or canonical root identity', async () => {
        ...
        expect(await workspace.assertExactRoot(identity.configuredRoot)).toBe(identity);
        expect(await assertExactConversationRoot(identity, identity.canonicalRoot)).toBe(identity);
    ...
    it('rejects a replaced conversation child symlink', async () => {
        ...
        await expect(workspace.materializeConversationDirectory('replace')).rejects.toThrow(/non-symlink/);
    });
    it('discards only an empty expected conversation child', async () => {
        ...
        const occupied = await workspace.materializeConversationDirectory('occupied');
        await writeFile(join(occupied, 'keep.txt'), 'keep');
        await expect(workspace.discardEmptyConversationDirectory('empty')).resolves.toBe(true);
        await expect(lstat(empty)).rejects.toMatchObject({ code: 'ENOENT' });
        await expect(workspace.discardEmptyConversationDirectory('empty')).resolves.toBe(false);
        await expect(workspace.discardEmptyConversationDirectory('occupied')).resolves.toBe(false);
    ...
    it('prepares only a new or reusable empty standalone child', async () => {
        ...
        await expect(workspace.prepareStandaloneDirectory('standalone')).rejects.toMatchObject({
            name: 'ConversationDirectoryIdentityError',
            scope: 'child',
            reason: 'not_empty',
        });
        expect((await lstat(created.identity.canonicalPath)).isDirectory()).toBe(true);
    });
    ...
    it('sanitizes standalone child filesystem errors', async () => {
        ...
        try {
            ...
            expect(error.message).not.toContain(prepared.identity.canonicalPath);
            expect(JSON.stringify(error)).not.toContain(prepared.identity.canonicalPath);
        }
        finally {
    ...
    it('inspects, creates, and rejects replaced standalone child identities', async () => {
        const home = await tempHome();
        const workspace = new ConversationWorkspace({ homeDir: home });
        await expect(workspace.inspectStandaloneDirectory('standalone')).resolves.toEqual({ status: 'missing' });
        const created = await workspace.ensureStandaloneDirectory('standalone');
        expect(created.status).toBe('created');
        if (created.status !== 'created')
            throw new Error('expected creation');
        await expect(workspace.inspectStandaloneDirectory('standalone', created.identity)).resolves.toMatchObject({ status: 'ready' });
        ...
        const compromised = await workspace.inspectStandaloneDirectory('standalone', created.identity);
        expect(compromised.status).toBe('compromised');
        if (compromised.status !== 'compromised') {
            throw new Error('expected compromised');
        }
        expect(compromised.error).toBeInstanceOf(ConversationDirectoryIdentityError);
    ...
    it('reports recreated only when a known identity vanished', async () => {
        ...
        await rename(prepared.identity.canonicalPath, `${prepared.identity.canonicalPath}.preserved`);
        const ensured = await workspace.ensureStandaloneDirectory('standalone', prepared.identity);
        expect(ensured.status).toBe('recreated');
        if (ensured.status !== 'recreated')
            throw new Error('expected recreate');
        expect(ensured.identity.canonicalPath).toBe(prepared.identity.canonicalPath);
    ...
    it('propagates a raced compromised inspection verbatim from the ensure race', async () => {
        ...
        const workspace = new ConversationWorkspace({ homeDir: home });
        const prepared = await workspace.prepareStandaloneDirectory('standalone');
        const racedError = new ConversationDirectoryIdentityError('child', 'unexpected_identity');
        const inspect = vi.spyOn(workspace, 'inspectStandaloneDirectory');
        inspect
    ...
    it('propagates a raced compromised verdict when no identity is expected', async () => {
        ...
        const racedError = new ConversationDirectoryIdentityError('child', 'wrong_mode');
        const inspect = vi.spyOn(workspace, 'inspectStandaloneDirectory');
        inspect
            .mockResolvedValueOnce({ status: 'missing' })
            .mockResolvedValueOnce({ status: 'compromised', error: racedError });
...
//# sourceMappingURL=conversation-workspace.test.js.map