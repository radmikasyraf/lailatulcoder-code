...
import { observeToolResultBoundary, toolResultBoundaryArtifact, toolResultPartDiagnosticValues, } from '../utils/tool-result-boundary-diagnostics.js';
...
import { SessionTranscriptChangedError, SessionWriterLostError, SessionWriterUnavailableError, } from './session-writer-lease.js';
import { collectPendingBranchToolCalls, resolveCompletedTurnBranchCandidateFromRecords, updatePendingBranchToolCalls, } from './branch-points.js';
...
function copyGoalContext(goalContext) {
    return {
...
export const TURN_RESULT_CODE_TEXT_TRUNCATED = 'RESULT_TEXT_TRUNCATED';
function readTurnResultErrorField(error, field) {
...
function truncateTurnResultErrorField(value, maxChars) {
    return value.length > maxChars
        ? { value: value.slice(0, maxChars), truncated: true }
        : { value, truncated: false };
}
export function normalizeTurnResultError(error) {
    const rawMessage = readTurnResultErrorField(error, 'message');
    let message = typeof rawMessage === 'string' && rawMessage.length > 0
        ? rawMessage
        : undefined;
    ...
    const boundedMessage = truncateTurnResultErrorField(message ?? 'Unknown error', TURN_RESULT_ERROR_MESSAGE_MAX_CHARS);
    const rawCode = readTurnResultErrorField(error, 'code') ??
        readTurnResultErrorField(error, 'rpcCode');
    const code = typeof rawCode === 'string' && rawCode.length > 0
        ? rawCode
        : typeof rawCode === 'number'
            ? String(rawCode)
            : undefined;
    const boundedCode = code === undefined
        ? undefined
...
export function isTurnResultRecordPayload(value) {
    ...
    if (typeof payload['promptId'] !== 'string' ||
        ...
        !['completed', 'cancelled', 'error'].includes(payload['state']) ||
        typeof payload['endedAt'] !== 'number' ||
        !Number.isFinite(payload['endedAt'])) {
        return false;
    }
    const optionalString = (field, maxChars) => {
        const fieldValue = payload[field];
        return (fieldValue === undefined ||
            (typeof fieldValue === 'string' &&
                (maxChars === undefined || fieldValue.length <= maxChars)));
    };
    const optionalBoolean = (field) => payload[field] === undefined || typeof payload[field] === 'boolean';
    const optionalTimestamp = (field) => payload[field] === undefined ||
        (typeof payload[field] === 'number' && Number.isFinite(payload[field]));
    if (!optionalString('stopReason', TURN_RESULT_IDENTIFIER_MAX_CHARS) ||
...
export class ChatRecordingService {
    ...
    activeBranchBaseUuid = null;
    /** Unclosed tool calls at the current active tail. */
    pendingBranchToolCalls = [];
    config;
    /**
    ...
    constructor(config, onWriteFailure, writerLeaseRequired = config.isSessionWriterLeaseEnabled?.() ??
        ...
        if (writerLeaseRequired) {
            ...
            if (restoreState) {
                ...
                this.restoreSessionState(resumed
                    ? {
                        conversation: resumed.conversation ?? { messages: [] },
                        lastCompletedUuid: resumed.lastCompletedUuid,
                    }
                    : undefined, resumed ? this.readPersistedTitleInfo() : undefined);
            }
        }
    }
    readPersistedTitleInfo() {
        try {
            return this.config
                .getSessionService()
    ...
    ensureConversationFile() {
        ...
        const conversationFile = path.join(this.ensureChatsDir(), `${this.getSessionId()}.jsonl`);
        try {
    ...
    restoreSessionState(sessionData, persistedTitleInfo) {
        ...
        for (const record of sessionData.conversation.messages) {
            if (record.type === 'user' && record.subtype === undefined) {
                this.trackUserDisplayTextForTitle(record.systemPayload
                    ?.displayText);
            }
            ...
            if (record.subtype === 'custom_title') {
                ...
                this.currentParentSessionId = record.systemPayload?.parentSessionId;
            }
    ...
    trackUserDisplayTextForTitle(displayText) {
    ...
    activate(lease, sessionData, persistedTitleInfo, restoreState) {
        if (!this.writerLeaseRequired ||
            this.state !== 'inactive' ||
    ...
    enterWriteFailure(cause, sessionId, operation = 'append') {
        ...
        if (!this.integrityFailure &&
            ...
            debugLogger.error(`Session writer failure sessionId=${sessionId} operation=${operation} errorKind=${failure.errorKind}`);
        }
    ...
    updateActiveBranch(record) {
        const currentTail = this.activeBranchRecords.at(-1)?.uuid ?? null;
        if (record.parentUuid !== currentTail) {
            const parentIndex = record.parentUuid === null
                ? -1
                : this.activeBranchRecords.findIndex((candidate) => candidate.uuid === record.parentUuid);
            this.activeBranchRecords =
                parentIndex < 0
                    ? []
            ...
            this.pendingBranchToolCalls = collectPendingBranchToolCalls(this.activeBranchRecords);
        }
        this.activeBranchRecords.push(record);
        updatePendingBranchToolCalls(this.pendingBranchToolCalls, record);
    }
    enqueueRecordWrite(record, legacyConversationFile, updateActiveTail = true) {
        ...
        this.operationTail = pendingWrite.then(() => undefined, () => undefined);
        return pendingWrite;
    }
    /**
    ...
    async appendRecordStrict(record, options) {
        ...
        const pendingWrite = this.enqueueRecordWrite(record, legacyConversationFile, updateActiveTail);
    ...
    releaseTopologyFence(fence) {
        if (this.topologyFence !== fence)
            return;
        ...
        for (const intent of fence.buffered) {
            ...
            if (intent.resolve && intent.reject) {
                void this.appendRecordStrict(intent.record, intent.options).then(intent.resolve, intent.reject);
            }
            else {
    ...
    async readActiveTranscriptChain() {
        await this.flush();
        ...
        if (!session) {
            throw new Error(`Unable to load active transcript for session ${sessionId}`);
        }
        return session.conversation.messages;
    }
    async runWithWriteBarrier(operation) {
        if (this.writeFailure)
            throw this.writeFailure;
        if (!this.acceptingWrites || this.state !== 'active') {
            throw new SessionWriterUnavailableError();
        ...
        const pending = this.operationTail.then(async () => {
            ...
            try {
                ...
                if (error instanceof SessionWriterLostError ||
                    ...
                    throw this.enterWriteFailure(error, this.getSessionId(), 'read_barrier');
                }
                throw error;
            }
        });
    ...
    async recordGoalState(recordUuid, payload) {
        const record = {
            ...this.createBaseRecord('system'),
            uuid: recordUuid,
            type: 'system',
    ...
    getUserDisplayTextsForTitle() {
        return this.userDisplayTextsForTitle;
    }
    recordGoalRuntimeMessage(message, goalContext) {
        try {
            const record = {
                ...this.createBaseRecord('user'),
     ...
     */
    recordMidTurnUserMessage(message, displayText, goalContext, attachmentReferences) {
        try {
            const record = {
                ...this.createBaseRecord('user'),
     ...
     */
    recordCronPrompt(message, displayText, goalContext) {
        this.recordNotificationLike(message, 'cron', displayText, undefined, goalContext);
    }
    /**
     ...
     */
    recordNotification(message, displayText, backgroundTask, goalContext) {
        this.recordNotificationLike(message, 'notification', displayText, backgroundTask, goalContext);
    }
    /**
     ...
     */
    async recordNotificationStrict(message, displayText, backgroundTask) {
        await this.appendRecordStrict(this.createNotificationRecord(message, 'notification', displayText, backgroundTask));
    }
    recordNotificationLike(message, subtype, displayText, backgroundTask, goalContext) {
        try {
            const record = this.createNotificationRecord(message, subtype, displayText, backgroundTask, goalContext);
            this.appendRecord(record);
        }
        catch (error) {
    ...
    createNotificationRecord(message, subtype, displayText, backgroundTask, goalContext) {
        return {
            ...this.createBaseRecord('user'),
            subtype,
            provenance: 'system',
    ...
    async recordRealtimeConversation(entries, model) {
    ...
    maybeTriggerAutoTitle() {
        ...
        void (async () => {
            try {
                const outcome = await tryGenerateSessionTitle(this.config, controller.signal, this.userDisplayTextsForTitle);
                if (!outcome.ok)
                    return;
     ...
     */
    recordToolResult(message, toolCallResult, options) {
        try {
            const persistedOutputFiles = toolCallResult?.persistedOutputFiles;
            const artifacts = [
                toolResultBoundaryArtifact(persistedOutputFiles, toolCallResult?.artifacts),
            ];
            ...
            if (toolCallResult) {
                ...
                recordingToolCallResult = sanitizeToolCallResultForRecording(recordableToolCallResult);
            ...
            let displayMutated;
            const mutated = () => (displayMutated ??= !isDeepStrictEqual(inputDisplay, outputDisplay));
    ...
    rewindRecording(targetTurnIndex, payload, survivingFileHistorySnapshots) {
        try {
            ...
            const projectionStart = Math.max(0, this.turnParentUuids.length - this.userDisplayTextsForTitle.length);
            this.userDisplayTextsForTitle.splice(Math.max(0, targetTurnIndex - projectionStart));
    ...
    async recordUserTextElements(payload) {
        const record = {
            ...this.createBaseRecord('system'),
            type: 'system',
     ...
     */
    recordTurnResult(payload) {
        if (!isTurnResultRecordPayload(payload)) {
            debugLogger.error('Skipping turn result record that violates the bounded contract:', payload);
            return;
        }
        try {
            const record = {
