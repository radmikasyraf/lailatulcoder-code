...
function makeFixture(loadOverrides = {}) {
...
function writeTranscript(fixture, records) {
    const transcriptPath = path.join(path.dirname(fixture.ledgerPath), `${fixture.sessionId}.jsonl`);
    mkdirSync(path.dirname(transcriptPath), { recursive: true });
    writeFileSync(transcriptPath, records.map((record) => JSON.stringify(record)).join('\n') + '\n', 'utf8');
}
function chatRecord(fixture, uuid, parentUuid, text) {
...
function bridgeWithColdLoad(sessionId, workspaceCwd, loadOverrides) {
    const restored = {
        sessionId,
    ...
    return {
        ...
        getSessionSummary: vi.fn((requestedId) => {
...
//# sourceMappingURL=session-prompt-terminals.test.js.map