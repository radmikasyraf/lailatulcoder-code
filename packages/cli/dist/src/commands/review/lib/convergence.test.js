...
import { diagnoseConvergence, renderConvergenceDiagnosis, MAX_RENDERED_CLUSTERS, } from './convergence.js';
import { LEDGER_MAX_ROUND } from './ledger.js';
const f = (id, file) => ({
    id,
    sev: 'S',
    file,
...
const d = (file, carriedId) => carriedId === undefined ? { file } : { file, carriedId };
describe('diagnoseConvergence — the trigger table', () => {
    ...
    it('measures the trend on new findings, not on the round total', () => {
        ...
        const carried = Array.from({ length: 30 }, (_, i) => d(`old${i}.ts`, `R2-${i + 1}`));
        const standing = Array.from({ length: 30 }, (_, i) => f(`R2-${i + 1}`, `old${i}.ts`));
        expect(diagnoseConvergence({
            round: 4,
            posted: 31,
    ...
    it('carries the merged qualifier through, and drops the depth key with it', () => {
        ...
        const planted = Array.from({ length: 50 }, (_, i) => f(`R${i + 1}-1`, 'src/planted.ts'));
        const r = diagnoseConvergence({
            round: 6,
            posted: 2,
            prev: {
...
describe('renderConvergenceDiagnosis — what the author reads', () => {
    ...
    it('states the measured facts before the reading of them', () => {
        ...
        expect(r.en).toContain('round 6 posted 4 inline comment(s), 2 of them reported for the first time');
    ...
    it('says the observation withheld nothing — scoped to the observation', () => {
        ...
        const r = renderConvergenceDiagnosis(base);
        expect(r.en).toContain('nothing was withheld from this review because of this observation');
    ...
    it('discloses a work list that was truncated or came from elsewhere', () => {
        ...
        expect(r.en).toContain("the previous round's work list was truncated to fit the marker");
    ...
    it('qualifies each reading by the evidence that reading rests on', () => {
        ...
        expect(volumeOnly.en).toContain("the previous round's work list was truncated to fit the marker");
