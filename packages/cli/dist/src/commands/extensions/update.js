...
import { checkForExtensionUpdate, ExtensionUpdateState, } from '@lailatul-coder/lailatul-coder-core';
import { getExtensionManager } from './utils.js';
import { t } from '../../i18n/index.js';
const updateOutput = (info) => t('Extension "{{name}}" successfully updated: {{oldVersion}} → {{newVersion}}.', {
    name: info.name,
    oldVersion: info.originalVersion,
    newVersion: info.updatedVersion,
});
const updateWarningOutput = (info) => (info.warnings ?? [])
    .map((warning) => t('Extension "{{name}}" updated with warning {{code}}: {{error}}', {
    name: info.name,
    code: warning.code,
    error: warning.error,
}))
...
export async function handleUpdate(args) {
    ...
    if (args.name) {
        try {
            ...
            if (!extension) {
                writeStdoutLine(t('Extension "{{name}}" not found.', { name: args.name }));
                return;
            }
            if (!extension.installMetadata) {
                writeStdoutLine(t('Unable to install extension "{{name}}" due to missing install metadata', { name: args.name }));
                return;
            }
            const updateState = await checkForExtensionUpdate(extension, extensionManager);
            if (updateState !== ExtensionUpdateState.UPDATE_AVAILABLE) {
                writeStdoutLine(t('Extension "{{name}}" is already up to date.', { name: args.name }));
                return;
            }
            // TODO(chrstnb): we should list extensions if the requested extension is not installed.
            const updatedExtensionInfo = (await extensionManager.updateExtension(extension, updateState, () => { }));
            if (updatedExtensionInfo.originalVersion !==
                updatedExtensionInfo.updatedVersion) {
                writeStdoutLine(t('Extension "{{name}}" successfully updated: {{oldVersion}} → {{newVersion}}.', {
                    name: args.name,
    ...
    if (args.all) {
        try {
            ...
            let updateInfos = await extensionManager.updateAllUpdatableExtensions(extensionState, () => { });
...
export const updateCommand = {
    ...
    describe: t('Updates all extensions or a named extension to the latest version.'),
    builder: (yargs) => yargs
        .positional('name', {
        ...
        if (!argv.all && !argv.name) {
            throw new Error(t('Either an extension name or --all must be provided'));
        }
        return true;
    }),
    handler: async (argv) => {
