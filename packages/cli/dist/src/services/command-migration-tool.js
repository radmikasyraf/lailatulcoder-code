...
import { convertTomlToMarkdown } from '@lailatul-coder/lailatul-coder-core';
 ...
 */
export async function detectTomlCommands(commandDir) {
    try {
 ...
 */
export async function migrateTomlCommands(options) {
    ...
    for (const relativeFile of tomlFiles) {
        ...
        try {
            ...
            try {
                ...
                throw new Error(t('Markdown file already exists: {{filename}}', {
 ...
 */
export function generateMigrationPrompt(tomlFiles) {
    if (tomlFiles.length === 0) {
        return '';
    }
    const count = tomlFiles.length;
    const moreCount = tomlFiles.length - 3;
    const fileList = tomlFiles.length <= 5
...
${t('You can migrate these files automatically using:')}
  lailatul-coder migrate-commands
...
//# sourceMappingURL=command-migration-tool.js.map