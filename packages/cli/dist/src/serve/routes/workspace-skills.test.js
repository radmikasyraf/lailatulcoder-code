...
function createHarness() {
    ...
    const setWorkspaceSkillEnabled = vi.fn(async (_ctx, skillName, enabled) => ({
    ...
    const setWorkspaceSkillsEnabled = vi.fn(async (_ctx, skillNames, enabled) => ({
        enabled,
        activation: 'applied',
        sessionsRefreshed: 1,
        sessionsFailed: 0,
...
describe('workspace Skill management routes', () => {
    it('forwards an install request to the workspace service', async () => {
        ...
        expect(harness.installWorkspaceSkill).toHaveBeenCalledWith(expect.objectContaining({
            workspaceCwd: '/workspace',
    ...
    it('forwards delete scope and rejects invalid scopes', async () => {
        const harness = createHarness();
        const response = await request(harness.app).delete('/workspace/skills/demo-skill?scope=global');
        const invalid = await request(harness.app).delete('/workspace/skills/demo-skill?scope=extension');
        expect(response.status).toBe(200);
        expect(harness.deleteWorkspaceSkill).toHaveBeenCalledWith(expect.objectContaining({ originatorClientId: 'client-1' }), 'demo-skill', 'global');
        expect(invalid.status).toBe(400);
        expect(invalid.body.code).toBe('invalid_skill_scope');
    });
    it('returns structured management errors', async () => {
        const harness = createHarness();
        harness.installWorkspaceSkill.mockRejectedValueOnce(new WorkspaceSkillManagementError('skill_manifest_missing', 'Skill package must contain a root SKILL.md'));
        const response = await request(harness.app)
            .post('/workspace/skills/install')
            .send({
            name: 'demo-skill',
    ...
    it('rejects an invalid delete name before calling the service', async () => {
        const harness = createHarness();
        const response = await request(harness.app).delete('/workspace/skills/invalid%20name?scope=workspace');
        expect(response.status).toBe(400);
        expect(response.body.code).toBe('invalid_skill_name');
    ...
    it('toggles a deduplicated Skill batch and returns per-target outcomes', async () => {
        ...
        expect(harness.setWorkspaceSkillsEnabled).toHaveBeenCalledWith(expect.objectContaining({
            route: 'POST /workspace/skills/enable',
            originatorClientId: 'client-1',
        }), ['Review', 'missing', 'locked'], false);
    });
    ...
    it('fails the whole batch when the workspace generation closes', async () => {
        const harness = createHarness();
        harness.setWorkspaceSkillsEnabled.mockRejectedValueOnce(Object.assign(new Error('closed'), {
...
//# sourceMappingURL=workspace-skills.test.js.map