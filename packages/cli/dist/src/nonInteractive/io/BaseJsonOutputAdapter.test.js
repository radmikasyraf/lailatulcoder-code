...
import { HEADLESS_TOOL_RESULT_TEXT_JSON_BYTE_BUDGET, HEADLESS_TOOL_RESULT_TEXT_TRUNCATION_MARKER, } from './headless-tool-result-text-projection.js';
/**
...
describe('BaseJsonOutputAdapter', () => {
    ...
    describe('appendThinking', () => {
        ...
        it('should preserve whitespace in thinking content', () => {
            const state = adapter.exposeCreateMessageState();
            adapter.startAssistantMessage();
            adapter.exposeAppendThinking(state, '', 'The user just said "Hello"', null);
            expect(state.blocks).toHaveLength(1);
            expect(state.blocks[0]).toMatchObject({
                type: 'thinking',
        ...
        it('should preserve whitespace when appending multiple thinking fragments', () => {
            ...
            adapter.exposeAppendThinking(state, '', '. This is a simple greeting', null);
            expect(state.blocks).toHaveLength(1);
            const block = state.blocks[0];
            // Verify the complete text with all spaces preserved
            expect(block.thinking).toBe('The user just said "Hello". This is a simple greeting');
    ...
    describe('emitToolResult', () => {
        ...
        const persistedOutputCases = [
            ['unhandled', undefined],
            ['handled without artifact', []],
            ['reusable artifact', ['/tmp/private-output.txt']],
        ];
        it.each(persistedOutputCases)('bounds %s tool display without mutating or serializing persistence metadata', (_name, persistedOutputFiles) => {
            ...
            expect(Buffer.byteLength(JSON.stringify(block.content), 'utf8')).toBeLessThanOrEqual(HEADLESS_TOOL_RESULT_TEXT_JSON_BYTE_BUDGET);
            expect(block.content).toContain(HEADLESS_TOOL_RESULT_TEXT_TRUNCATION_MARKER);
    ...
    describe('buildResultMessage', () => {
        ...
        it('includes structured_result and JSON-stringifies result when structuredResult is provided', () => {
            ...
            if (!result.is_error) {
                ...
                expect(result
                    .structured_result).toEqual(payload);
            }
        });
        it('omits structured_result when structuredResult is undefined (back-compat)', () => {
            ...
            const result = adapter.exposeBuildResultMessage(options);
            expect(result
                .structured_result).toBeUndefined();
        });
        ...
        describe('toolResultContent', () => {
            ...
            it('includes the vision bridge disclosure with tool content', () => {
                const response = {
                    ...
                    resultDisplay: {
                        ...
                        notice: 'Converted 4 images via qwen3-vl-plus (dashscope.aliyuncs.com).',
                    },
                    responseParts: [
                        {
                            functionResponse: {
                ...
                expect(toolResultContent(response)).toBe('Transcribed PDF pages 20-23; remaining pages 24-25\n' +
            ...
            it('includes a tool-result vision notice without replacing content', () => {
                const response = {
                    ...
                    visionBridgeNotice: 'Converted 1 image via qwen3-vl-plus (dashscope.aliyuncs.com).',
                    responseParts: [
                        {
                            functionResponse: {
                ...
                expect(toolResultContent(response)).toBe('Converted 1 image via qwen3-vl-plus (dashscope.aliyuncs.com).\n' +
                    'captured screen');
            });
            it('prefers a top-level tool error over content with a vision bridge disclosure', () => {
                ...
                expect(toolResultContent(response)).toBe('Failed to read PDF after rendering pages 20-23\n' +
            ...
            it('prefers detailed response error over the operational summary', () => {
                ...
                expect(toolResultContent(response)).toBe('Command timed out.\npartial output');
            });
