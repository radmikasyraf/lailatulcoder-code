...
import { AGENT_CONTEXT_FILENAME, DEFAULT_CONTEXT_FILENAME, Storage, setGeminiMdFilename, } from '@lailatul-coder/lailatul-coder-core';
...
function resetContextFilenames() {
...
describe('workspace memory routes', () => {
    ...
    describe('GET /workspace/memory', () => {
        ...
        it('reports workspace and global QWEN.md files with byte counts', async () => {
            ...
            expect(res.body.totalBytes).toBe(Buffer.byteLength(wsContent) + Buffer.byteLength(globalContent));
    ...
    describe('POST /workspace/memory', () => {
        it('appends to workspace QWEN.md and emits memory_changed', async () => {
            ...
            const written = await fs.readFile(path.join(workspace, 'QWEN.md'), 'utf8');
        ...
        it('writes to the global ~/.qwen directory when scope=global', async () => {
            ...
            const written = await fs.readFile(path.join(globalDir, 'QWEN.md'), 'utf8');
        ...
        it('omits errorMessage + filePath in 500/413 responses unless QWEN_SERVE_DEBUG is on', async () => {
            ...
            try {
                ...
                if (prevDebug === undefined)
                    ...
                    process.env['QWEN_SERVE_DEBUG'] = prevDebug;
...
//# sourceMappingURL=workspace-memory.test.js.map