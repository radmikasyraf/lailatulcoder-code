...
describe('createNativeAudioRecorder', () => {
    it('records mono 16k audio through the native capture backend', async () => {
        const backend = {
            ...
            microphoneAuthorizationStatus: vi.fn(() => 'unknown'),
        };
    ...
    it('rejects concurrent starts while the backend is loading', async () => {
        ...
        let resolveBackend = () => { };
        const loadBackend = vi.fn(() => new Promise((resolve) => {
    ...
    it('explains mirror registry installs when the native package is missing', async () => {
        const recorder = createNativeAudioRecorder({
            loadBackend: () => {
                throw new Error("Cannot find package '@lailatul-coder/audio-capture' imported from /qwen/dist/cli.js");
            },
        });
    ...
    it.each(['ERR_MODULE_NOT_FOUND', 'MODULE_NOT_FOUND'])('explains mirror registry installs for %s native package errors', async (code) => {
        const error = Object.assign(new Error('missing @lailatul-coder/audio-capture dependency'), { code });
        const recorder = createNativeAudioRecorder({
            loadBackend: () => {
                throw error;
            },
    ...
    it('does not rewrite wrapped native addon load failures as missing packages', async () => {
        const loadError = new Error("Native audio capture addon could not be loaded. Reinstall @lailatul-coder/audio-capture, or use the SoX fallback. (Cannot find module 'node-gyp-build')");
        const recorder = createNativeAudioRecorder({
            loadBackend: () => {
                throw loadError;
            },
    ...
    it('does not explain native start failures as missing packages', async () => {
        const startError = new Error("Cannot find package '@lailatul-coder/audio-capture' while starting");
        const backend = {
            startRecording: vi.fn(() => {
                throw startError;
            }),
...
//# sourceMappingURL=native-audio-recorder.test.js.map