 ...
 */
import { fetchGitLog, fetchGitCommitDetail, MAX_LOG_LIMIT, DEFAULT_LOG_LIMIT, } from '@lailatul-coder/lailatul-coder-core';
import { requireTrustedWorkspaceRuntime, resolveContainedCwd, resolveWorkspaceRuntimeFromParam, } from '../workspace-route-runtime.js';
import { applyReadHeaders } from './workspace-file-read.js';
function buildLogList(workspaceCwd, result) {
    if (!result) {
        return {
            v: 1,
            workspaceCwd,
...
function buildCommitDetail(workspaceCwd, result) {
    if (!result) {
...
function parsePagination(req) {
    ...
    const limit = Math.min(Math.max(Number.isNaN(parsedLimit) ? DEFAULT_LOG_LIMIT : parsedLimit, 1), MAX_LOG_LIMIT);
    const skip = Math.max(Number.isNaN(parsedSkip) ? 0 : parsedSkip, 0);
    return { limit, skip };
}
async function handleLogList(req, res, workspaceCwd, sendBridgeError, route) {
    try {
...
async function handleCommitDetail(req, res, workspaceCwd, sendBridgeError, route) {
...
export function registerWorkspaceGitLogRoutes(app, deps) {
    app.get('/workspace/git/log', (req, res) => {
        void handleLogList(req, res, deps.boundWorkspace, deps.sendBridgeError, 'GET /workspace/git/log');
    });
    app.get('/workspace/git/log/commit', (req, res) => {
        void handleCommitDetail(req, res, deps.boundWorkspace, deps.sendBridgeError, 'GET /workspace/git/log/commit');
    });
}
...
export function registerWorkspaceQualifiedGitLogRoutes(app, deps) {
    app.get('/workspaces/:workspace/git/log', (req, res) => {
        ...
        if (!runtime)
            return;
        void handleLogList(req, res, resolveContainedCwd(req, runtime.workspaceCwd), deps.sendBridgeError, 'GET /workspaces/:workspace/git/log');
    });
    app.get('/workspaces/:workspace/git/log/commit', (req, res) => {
        const runtime = resolveTrustedRuntime(deps.workspaceRegistry, req, res);
        if (!runtime)
            return;
        void handleCommitDetail(req, res, resolveContainedCwd(req, runtime.workspaceCwd), deps.sendBridgeError, 'GET /workspaces/:workspace/git/log/commit');
    });
}
//# sourceMappingURL=workspace-git-log.js.map