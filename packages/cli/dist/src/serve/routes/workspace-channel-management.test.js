...
function mount(secondaryTrusted = true) {
    ...
    const mutate = (_opts) => (req, res, next) => {
...
describe('workspace Channel management routes', () => {
    it('lists catalog and sanitized instances without mutation auth', async () => {
        ...
        const qualifiedCatalog = await request(app).get('/workspaces/secondary/channel-types');
        expect(qualifiedCatalog.status).toBe(200);
        expect(qualifiedCatalog.headers['cache-control']).toBe('no-store');
        expect(qualifiedCatalog.body).toEqual(expect.arrayContaining([
            expect.objectContaining({ type: 'dingtalk', manageable: true }),
        ]));
    ...
    it('advertises the exact nested object descriptor shape on the wire', async () => {
        ...
        const response = await request(app).get('/workspace/channel-types');
        expect(response.status).toBe(200);
        const dingtalk = response.body.find((entry) => entry.type === 'dingtalk');
    ...
    it('routes strict CRUD and lifecycle mutations to the primary service', async () => {
        ...
        await auth(request(app)
            ...
            .send({ expectedRevision: 'r2' })).expect(200);
        await auth(request(app)
            .put('/workspace/channels/bot/startup')
            .send({ expectedRevision: 'r2', enabled: true })).expect(200);
        ...
        await auth(request(app).post('/workspace/channels/bot/restart')).expect(200);
    ...
    it('uses only the exact trusted secondary service', async () => {
        ...
        await auth(request(app)
            ...
            .send({ expectedRevision: 'r1', enabled: false })).expect(200);
    ...
    it('rejects qualified channel mutations for the Conversations runtime', async () => {
        ...
        const resolveService = vi.fn((target) => target === live ? liveService : service());
        const app = express();
        app.use(express.json());
        registerWorkspaceChannelManagementRoutes(app, {
            primaryRuntime: primary,
    ...
    it('fails closed before internal channel reads when the activity gate is absent', async () => {
        ...
        const response = await request(app).get('/workspaces/conversations/channels');
    ...
    it('fails closed for an untrusted secondary workspace', async () => {
        ...
        const approvals = await auth(request(app).get('/workspaces/secondary/channels/bot/pairing-approvals'));
    ...
    it('manages pairing requests and approvals in the selected workspace', async () => {
        ...
        await request(app)
            ...
            .expect(401);
        const pairingRequests = await auth(request(app).get('/workspaces/secondary/channels/bot/pairing-requests'));
        const approval = await auth(request(app)
        ...
        await request(app)
            ...
            .expect(401);
        const approvals = await auth(request(app).get('/workspaces/secondary/channels/bot/pairing-approvals'));
        const revocation = await auth(request(app)
            .delete('/workspaces/secondary/channels/bot/pairing-approvals')
            .send({ senderId: 'sender-1' }));
        ...
        expect(secondaryService.approvePairing).toHaveBeenCalledWith('bot', 'ABCDEFGH');
        ...
        const primaryPairingRequests = await auth(request(app).get('/workspace/channels/bot/pairing-requests'));
    ...
    it('returns 404 when a pairing approval no longer exists', async () => {
        const { app, primaryService } = mount();
        vi.mocked(primaryService.revokePairingApproval).mockRejectedValueOnce(Object.assign(new Error('Pairing approval was not found.'), {
    ...
    it('rejects requests with an invalid client ID', async () => {
        const { app, primaryService } = mount();
        const invalidClient = (test) => test
            .set('Authorization', 'Bearer secret')
            .set('X-Qwen-Client-Id', 'invalid');
        const list = await invalidClient(request(app).get('/workspace/channels'));
        const upsert = await invalidClient(request(app)
            .put('/workspace/channels/bot')
            .send({ expectedRevision: 'r1', config: { type: 'dingtalk' } }));
        const remove = await invalidClient(request(app)
            .delete('/workspace/channels/bot')
            .send({ expectedRevision: 'r1' }));
        const start = await invalidClient(request(app).post('/workspace/channels/bot/start'));
        const stop = await invalidClient(request(app).post('/workspace/channels/bot/stop'));
        const restart = await invalidClient(request(app).post('/workspace/channels/bot/restart'));
        const startup = await invalidClient(request(app)
            .put('/workspace/channels/bot/startup')
            .send({ expectedRevision: 'r1', enabled: true }));
        const approve = await invalidClient(request(app)
            .post('/workspace/channels/bot/pairing-requests/approve')
            .send({ code: 'ABCDEFGH' }));
        const pairingRequests = await invalidClient(request(app).get('/workspace/channels/bot/pairing-requests'));
        const pairingApprovals = await invalidClient(request(app).get('/workspace/channels/bot/pairing-approvals'));
        const revokePairingApproval = await invalidClient(request(app)
            .delete('/workspace/channels/bot/pairing-approvals')
            .send({ senderId: 'sender-1' }));
    ...
    it('rejects malformed names, revisions, secrets, and pairing codes', async () => {
        const { app, primaryService } = mount();
        vi.mocked(primaryService.upsert).mockRejectedValueOnce(Object.assign(new Error('Channel name is not allowed.'), {
            code: 'channel_settings_invalid_name',
        }));
        const invalidName = await auth(request(app).post('/workspace/channels/a%2Fb/start'));
        const unsafeName = await auth(request(app)
            .put('/workspace/channels/prototype')
            .send({
            expectedRevision: 'r1',
            config: { type: 'dingtalk' },
        }));
        const invalidRevision = await auth(request(app)
            .delete('/workspace/channels/bot')
            .send({ expectedRevision: '' }));
        const invalidSecret = await auth(request(app)
            .put('/workspace/channels/bot')
            .send({
            expectedRevision: 'r1',
            config: { type: 'dingtalk' },
        ...
        const invalidPairing = await auth(request(app)
            .post('/workspace/channels/bot/pairing-requests/approve')
            .send({ code: 'short' }));
        const invalidSenderId = await auth(request(app)
            .delete('/workspace/channels/bot/pairing-approvals')
            .send({ senderId: '' }));
        const invalidGroupId = await auth(request(app)
            .delete('/workspace/channels/bot/pairing-approvals')
            .send({ groupId: '' }));
        const ambiguousSubject = await auth(request(app)
            .delete('/workspace/channels/bot/pairing-approvals')
            .send({ senderId: 'sender-1', groupId: 'group-1' }));
        const missingSubject = await auth(request(app).delete('/workspace/channels/bot/pairing-approvals').send({}));
        ...
        expect(invalidRevision.body.code).toBe('invalid_channel_management_request');
    ...
    it('sends only the name error when both name and body are invalid', async () => {
        const { app, primaryService } = mount();
        const approve = await auth(request(app)
            .post('/workspace/channels/a%2Fb/pairing-requests/approve')
            .send({ code: 'short' }));
        const revoke = await auth(request(app)
            .delete('/workspace/channels/a%2Fb/pairing-approvals')
            .send({ senderId: '' }));
        const upsert = await auth(request(app)
            .put('/workspace/channels/a%2Fb')
            .send({ expectedRevision: '' }));
        const remove = await auth(request(app)
            .delete('/workspace/channels/a%2Fb')
            .send({ expectedRevision: '' }));
        const startup = await auth(request(app)
...
//# sourceMappingURL=workspace-channel-management.test.js.map