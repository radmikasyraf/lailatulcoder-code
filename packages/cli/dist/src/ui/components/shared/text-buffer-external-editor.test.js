...
vi.mock('node:os', async (importOriginal) => {
    ...
    return {
        ...actual,
        default: { ...actual['default'], tmpdir: () => '/tmp' },
        tmpdir: () => '/tmp',
    };
});
...
describe('openInExternalEditor', () => {
    beforeEach(() => {
        vi.restoreAllMocks();
        fs.mkdtempSync.mockReturnValue('/tmp/qwen-edit-mock');
        fs.writeFileSync.mockImplementation(() => { });
        fs.readFileSync.mockReturnValue('edited text');
    ...
    it('should create temp file in private mkdtemp directory', async () => {
        ...
        expect(fs.mkdtempSync).toHaveBeenCalledWith(expect.stringContaining('qwen-edit-'));
    ...
    it('should write temp file with mode 0o600', async () => {
        ...
        const writeOpts = fs.writeFileSync.mock.calls[0]?.[2];
    ...
    it('should fall back to env/default when no preferredEditor', async () => {
        ...
        expect(mockSpawnSync).toHaveBeenCalledWith(expect.any(String), expect.arrayContaining([expect.stringMatching(/\.txt$/)]), expect.objectContaining({ stdio: 'inherit' }));
    });
    it('should use getExternalEditorCommand when preferredEditor is set', async () => {
        ...
        expect(mockGetExternalEditorCommand).toHaveBeenCalledWith('vscode', expect.stringMatching(/\.txt$/));
        expect(mockSpawnSync).toHaveBeenCalledWith('code', expect.arrayContaining(['--wait']), expect.objectContaining({ shell: false }));
    });
    it('should quote args when needsShell is true', async () => {
        ...
        const spawnCmd = mockSpawnSync.mock.calls[0]?.[0];
        expect(spawnCmd).toBe('"code.cmd"');
        const spawnArgs = mockSpawnSync.mock.calls[0]?.[1];
        for (const arg of spawnArgs) {
            expect(arg).toMatch(/^".*"$/);
        }
        expect(mockSpawnSync.mock.calls[0]?.[2]).toEqual(expect.objectContaining({ shell: true }));
    });
    ...
    it('should respect VISUAL > EDITOR > default fallback order', async () => {
        ...
        try {
            ...
            expect(mockSpawnSync).toHaveBeenCalledWith('my-visual-editor', expect.any(Array), expect.any(Object));
        }
        finally {
    ...
    it('should fall back to EDITOR when VISUAL is not set', async () => {
        ...
        try {
            ...
            expect(mockSpawnSync).toHaveBeenCalledWith('my-editor', expect.any(Array), expect.any(Object));
        }
        finally {
            if (origVISUAL === undefined)
                delete process.env['VISUAL'];
    ...
    it('should detect .cmd/.bat in env-var fallback and enable shell mode on Windows', async () => {
        ...
        try {
            ...
            expect(mockSpawnSync).toHaveBeenCalledWith('"code.cmd"', expect.arrayContaining([expect.stringMatching(/^".*"$/)]), expect.objectContaining({ shell: true }));
        }
        finally {
    ...
    it('should pass timeout to spawnSync', async () => {
        ...
        expect(mockSpawnSync).toHaveBeenCalledWith(expect.any(String), expect.any(Array), expect.objectContaining({ timeout: 30 * 60 * 1000 }));
    });
    ...
    it('should not create undo snapshot when editor fails — undo is no-op', async () => {
        mockSpawnSync.mockReturnValue({ status: 1, error: null });
        fs.readFileSync.mockReturnValue('should not see this');
        const { result } = renderHook(() => useTextBuffer({
            initialText: 'original',
            viewport,
            isValidPath: () => false,
    ...
    it('should skip undo snapshot when content is unchanged', async () => {
        fs.readFileSync.mockReturnValue('unchanged');
        const { result } = renderHook(() => useTextBuffer({
    ...
    it('should quote editorCmd and args with shell:true when shell mode is enabled', async () => {
        ...
        try {
            ...
            expect(mockSpawnSync).toHaveBeenCalledWith('"C:\\Program Files\\Code\\code.cmd"', expect.arrayContaining([expect.stringMatching(/^".*"$/)]), expect.objectContaining({ shell: true }));
        }
        finally {
            Object.defineProperty(process, 'platform', { value: origPlatform });
            if (origVISUAL === undefined)
    ...
    it('should use opts.editor when provided', async () => {
        ...
        expect(mockGetExternalEditorCommand).not.toHaveBeenCalled();
        expect(mockSpawnSync).toHaveBeenCalledWith('nano', expect.arrayContaining([expect.stringMatching(/\.txt$/)]), expect.objectContaining({ stdio: 'inherit', shell: false }));
    });
    it('should enable shell mode for .cmd opts.editor on Windows', async () => {
        ...
        try {
            ...
            expect(mockSpawnSync).toHaveBeenCalledWith('"C:\\editor.cmd"', expect.arrayContaining([expect.stringMatching(/^".*"$/)]), expect.objectContaining({ shell: true }));
        }
        finally {
            Object.defineProperty(process, 'platform', { value: origPlatform });
        }
    ...
    ])('should reject opts.editor with unsafe "$char" on Windows', async ({ editor }) => {
        const origPlatform = process.platform;
        Object.defineProperty(process, 'platform', { value: 'win32' });
        try {
            const { result } = renderHook(() => useTextBuffer({
    ...
    it('should fall back to vi when VISUAL and EDITOR are unset on non-Windows', async () => {
        ...
        try {
            ...
            expect(mockSpawnSync).toHaveBeenCalledWith('vi', expect.arrayContaining([expect.stringMatching(/\.txt$/)]), expect.objectContaining({ stdio: 'inherit' }));
        }
        finally {
            Object.defineProperty(process, 'platform', { value: origPlatform });
            if (origVISUAL === undefined)
    ...
    it('should fall back to notepad when VISUAL and EDITOR are unset on Windows', async () => {
        ...
        try {
            ...
            expect(mockSpawnSync).toHaveBeenCalledWith('notepad', expect.arrayContaining([expect.stringMatching(/\.txt$/)]), expect.objectContaining({ stdio: 'inherit' }));
        }
        finally {
            Object.defineProperty(process, 'platform', { value: origPlatform });
            if (origVISUAL === undefined)
    ...
    ])('should reject env-var .cmd with unsafe "$char" on Windows', async ({ env }) => {
        const origPlatform = process.platform;
        const origVISUAL = process.env['VISUAL'];
        Object.defineProperty(process, 'platform', { value: 'win32' });
...
//# sourceMappingURL=text-buffer-external-editor.test.js.map