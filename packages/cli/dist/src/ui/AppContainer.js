...
import { IdeClient, ideContextStore, createDebugLogger, getErrorMessage, getAllGeminiMdFilenames, ShellExecutionService, Storage, createInstructionsLoadedCallback, SessionEndReason, generatePromptSuggestion, logPromptSuggestion, PromptSuggestionEvent, logSpeculation, SpeculationEvent, logWorkflowKeyword, WorkflowKeywordEvent, startSpeculation, acceptSpeculation, abortSpeculation, IDLE_SPECULATION, ApprovalMode, ConditionalRulesRegistry, MCPDiscoveryState, ToolConfirmationOutcome, ToolNames, SendMessageType, clearWorktreeSession, restoreWorktreeContext, GitWorktreeService, readWorktreeSessionMarker, isSessionRuntimeActive, } from '@lailatul-coder/lailatul-coder-core';
import { applyCollapsePolicyAndSummary, buildResumedHistoryItems, expandCollapsedHistory, } from './utils/resumeHistoryUtils.js';
import { buildWakeRepaint } from './utils/terminal-resize-reflow.js';
import { loadLowlight } from './utils/lowlightLoader.js';
import { getStickyTodos, getStickyTodoMaxVisibleItemsForMode, getStickyTodosLayoutKey, getStickyTodosRenderKey, } from './utils/todoSnapshot.js';
import { loadHierarchicalGeminiMemory } from '../config/config.js';
import { profileCheckpoint, finalizeStartupProfile, } from '../utils/startupProfiler.js';
...
import { isInteractiveTerminal, shouldUseVirtualViewport, } from './utils/terminal-buffer.js';
...
import { computeApiTruncationIndex, isRealUserTurn, } from './utils/historyMapping.js';
import { waitForGoalRuntime } from './utils/goal-runtime.js';
import { useVimModeState, useVimModeActions, } from './contexts/VimModeContext.js';
...
import { formatSessionWindowTitle, titleStatusPrefix, writeTerminalTitle, } from './utils/windowTitle.js';
import { clearScreen } from '../utils/stdioHelpers.js';
import { useTextBuffer } from './components/shared/text-buffer.js';
import { useLogger } from './hooks/useLogger.js';
import { useGeminiStream, } from './hooks/useGeminiStream.js';
import { useVim } from './hooks/vim.js';
import { CONTEXT_FILES_ANNOUNCEMENT_PREFIX, consumesContextAnnouncementLatch, isBtwCommand, isContextFilesAnnouncement, isSlashCommand, } from './utils/commandUtils.js';
import { detectWorkflowKeyword, buildWorkflowSteeringNotice, } from './utils/workflow-keyword.js';
...
import { useExtensionUpdates, useConfirmUpdateRequests, useSettingInputRequests, usePluginChoiceRequests, } from './hooks/useExtensionUpdates.js';
...
import { findLastUserItemIndex, isSyntheticHistoryItem, itemsAfterAreOnlySynthetic, realUserPromptTexts, } from './utils/historyUtils.js';
import { MAIN_CONTENT_HEIGHT_RESERVATION } from './utils/layoutUtils.js';
const CTRL_EXIT_PROMPT_DURATION_MS = 1000;
const debugLogger = createDebugLogger('APP_CONTAINER');
export function isRenderModeToggleKey(key) {
    return (keyMatchers[Command.TOGGLE_RENDER_MODE](key) ||
        (key.name === 'm' && key.meta && !key.ctrl && !key.paste));
}
export function getNextRenderMode(current) {
    return current === 'render' ? 'raw' : 'render';
}
export function handleRenderModeToggleKey(key, setRenderMode) {
...
function isCompressionPending(pendingHistoryItems) {
    return pendingHistoryItems.some((item) => item.type === MessageType.COMPRESSION && item.compression.isPending);
}
export function isInputActiveForState({ initError, isProcessing, hasPendingCompression, streamingState, }) {
    return (!initError &&
        (!isProcessing || hasPendingCompression) &&
        (streamingState === StreamingState.Idle ||
            streamingState === StreamingState.Responding));
}
export function shouldDrainMessageQueue({ isConfigInitialized, streamingState, isProcessing, dialogsVisible, messageQueueLength, }) {
...
export function useQueuedSubmissionDrain({ config, isConfigInitialized, streamingState, isProcessing, dialogsVisible, pendingSubmissionCount, getPendingSubmissionCount, popNextSubmission, enqueueGoalTurn, restoreMessages, submitQuery, submissionInFlightRef, submissionSettledRevision, }) {
    ...
    const admissionFailureRef = useRef(null);
    const [queueDrainNonce, setQueueDrainNonce] = useState(0);
    useEffect(() => {
        if (queueDrainingRef.current || submissionInFlightRef.current)
            return;
        ...
        if (admissionFailure) {
            ...
            else if (pendingSubmissionCount <= admissionFailure.pendingSubmissionCount &&
        ...
        const request = submission.kind === 'goal'
            ? submitQuery(submission.continuationContext, SendMessageType.Goal, undefined, {
                goal: submission,
                onAdmissionFailed: () => {
            ...
            : submitQuery(submission.modelText, SendMessageType.UserQuery, undefined, {
                userAdmission: { turnKey: submission.turnKey },
                ...(submission.submittedPrompt === undefined
                    ? {}
                    : { submittedPrompt: submission.submittedPrompt }),
                onAdmissionFailed: () => {
                    restoreMessages([submission.modelText], submission.submittedPrompt);
                    markAdmissionFailed();
                },
            });
...
export function getSpeculativeToolResult(response) {
    const responseRecord = typeof response === 'object' && response
        ? response
        : undefined;
    const hasError = responseRecord !== undefined &&
...
function getResponseCandidateTokens(pendingGeminiHistoryItems) {
    let tokens = 0;
...
function useStableStickyTodos(todos) {
    ...
    const stableTodosRef = useRef(null);
...
export function dedupeNewestFirst(messages) {
    const seen = new Set();
    const result = [];
...
export function mergeStartupWarnings(currentWarnings, nextWarnings) {
...
export const AppContainer = (props) => {
    const { settings, config, initializationResult, initialUseVirtualViewport, repaintViewport, } = props;
    const extensionRefreshState = useMemo(() => props.extensionRefreshState ?? new ExtensionRefreshState(), [props.extensionRefreshState]);
    ...
    extensionManager.setRequestConsent(requestConsentOrFail.bind(null, (description) => requestConsentInteractive(description, addConfirmUpdateExtensionRequest)));
    extensionManager.setRequestChoicePlugin((marketplace) => new Promise((resolve, reject) => {
    ...
    const { extensionsUpdateState, extensionsUpdateStateInternal, dispatchExtensionStateUpdate, } = useExtensionUpdates(extensionManager, historyManager.addItem, config.getWorkingDir());
    const { providerUpdateRequest, dismissProviderUpdate } = useProviderUpdates(settings, config, historyManager.addItem);
    ...
    const [expandedThoughtHeadIds, setExpandedThoughtHeadIds] = useState(() => new Set());
    const toggleThoughtExpanded = useCallback((headId) => {
    ...
    const writeRaw = useCallback((data) => {
        process.stdout.write(data);
    }, []);
    // Terminal notification helpers (constructed directly, not via context)
    const terminal = useMemo(() => buildTerminalNotification(writeRaw), [writeRaw]);
    // Additional hooks moved from App.tsx
    const { stats: sessionStats, startNewSession, seedPromptCount, } = useSessionStats();
    const logger = useLogger(config.storage, sessionStats.sessionId);
    const worktreeSession = useWorktreeSession(config);
    const branchName = useGitBranchName(worktreeSession?.worktreePath ?? config.getTargetDir());
     ...
     */
    const pendingWorktreeNoticeRef = useRef(null);
    // One-shot announcement of the context files (LAILATUL.md / context.fileName)
    ...
    const loadHistoryWithLatchReconciliation = useCallback((newHistory) => {
        rawLoadHistory(newHistory);
        contextFilesAnnouncedRef.current = newHistory.some(isContextFilesAnnouncement);
    }, [rawLoadHistory]);
    const activeWorktree = useMemo(() => worktreeSession
        ? {
        ...
        : null, [worktreeSession]);
    // Layout measurements
    const mainControlsRef = useRef(null);
    const lastTitleRef = useRef(null);
    const [startupWarnings, setStartupWarnings] = useState(() => props.startupWarnings || []);
    ...
    useEffect(() => {
        void loadLowlight().catch((err) => {
            ...
            debugLogger.warn(`Failed to load lowlight chunk; code blocks will render as plain text: ${err instanceof Error ? err.message : String(err)}`);
        });
    }, []);
    ...
    useEffect(() => {
        (async () => {
            ...
            setStartupWarnings((currentWarnings) => mergeStartupWarnings(currentWarnings, config.getWarnings()));
            ...
            const startupWorktreeNotice = config.consumePendingStartupWorktreeNotice();
            if (startupWorktreeNotice) {
                historyManager.addItem({ type: MessageType.INFO, text: startupWorktreeNotice }, Date.now());
            ...
            if (resumedSessionData) {
                ...
                const historyItems = applyCollapsePolicyAndSummary(rawItems, collapseOnResume, collapsePreviewCount);
                ...
                const userTurnCount = resumedSessionData.conversation.messages.filter((m) => m.type === 'user' &&
                    m.subtype !== 'mid_turn_user_message' &&
                    m.subtype !== 'realtime_message').length;
                if (userTurnCount > 0) {
                    seedPromptCount(userTurnCount);
                }
                const recovered = await config.loadPausedBackgroundAgents(config.getSessionId());
                ...
                if (!startupWorktreeNotice) {
                    try {
                        ...
                        if (restored.contextMessage) {
                            ...
                            historyManager.addItem({ type: MessageType.INFO, text: restored.contextMessage }, Date.now());
    ...
    useEffect(() => {
        ...
        const flushNow = () => {
            ...
            return geminiClient.setTools().catch((err) => {
                debugLogger.error(`setTools() batch-flush failed: ${err instanceof Error ? err.message : String(err)}`);
            });
        };
        const scheduleFlush = () => {
            if (flushTimer !== null)
        ...
        const surfaceFailuresOnce = () => {
            ...
            const failedNames = typeof config.getFailedMcpServerNames === 'function'
                ? config.getFailedMcpServerNames()
                : [];
            if (failedNames.length > 0) {
                debugLogger.warn(`MCP server(s) failed to start: ${failedNames.join(', ')}. ` +
        ...
        const finalizeCap = setTimeout(finalizeOnce, STARTUP_PROFILE_FINALIZE_CAP_MS);
        return () => {
    ...
    useEffect(() => {
        const handler = setUpdateHandler(historyManager.addItem, setUpdateInfo, isIdleRef);
    ...
    const restoredSubmissionRef = useRef(null);
    const submittedPromptProvenanceUnavailableRef = useRef(false);
    const setBufferTextRef = useRef(null);
    ...
    const handleBufferChange = useCallback((text) => {
        if (text.length === 0) {
            if (restoredSubmissionRef.current !== null ||
    ...
    const restoredPromptStashTargetsRef = useRef(new Set());
    ...
    useEffect(() => {
        const fetchUserMessages = async () => {
            ...
            const currentSessionUserMessages = realUserPromptTexts(historyManager.history).reverse();
    ...
    const [useTerminalBuffer] = useState(() => initialUseVirtualViewport ??
        shouldUseVirtualViewport(settings.merged.ui?.useTerminalBuffer, config.getScreenReader(), isInteractiveTerminal()));
    ...
    const shrinkRemountTimerRef = useRef(null);
    useEffect(() => {
    ...
    // coverage.
    const wakeRepaint = useMemo(() => buildWakeRepaint({
    ...
    const { isThemeDialogOpen, openThemeDialog, handleThemeSelect, handleThemeHighlight, } = useThemeCommand(settings, setThemeError, historyManager.addItem, initializationResult.themeError, config);
    const { isApprovalModeDialogOpen, openApprovalModeDialog, handleApprovalModeSelect, } = useApprovalModeCommand(settings, config);
    const { isEffortDialogOpen, openEffortDialog, handleEffortSelect } = useEffortCommand(settings, config, historyManager.addItem);
    const auth = useAuthCommand(settings, config, historyManager.addItem, refreshStatic);
    ...
    const { isEditorDialogOpen, openEditorDialog, handleEditorSelect, exitEditorDialog, } = useEditorSettings(settings, setEditorError, historyManager.addItem, config);
    ...
    const openStatusLineDialog = useCallback(() => setStatusLineDialogOpen(true), []);
    const closeStatusLineDialog = useCallback(() => setStatusLineDialogOpen(false), []);
    const [statusLineSettingsVersion, setStatusLineSettingsVersion] = useState(0);
    const [statusLineConfigOverride, setStatusLineConfigOverride] = useState(undefined);
    const notifyStatusLineSettingsChanged = useCallback((newConfig) => {
        setStatusLineConfigOverride(newConfig);
        setStatusLineSettingsVersion((version) => version + 1);
    }, []);
    const { isMemoryDialogOpen, openMemoryDialog, closeMemoryDialog } = useMemoryDialog();
    const { isModelDialogOpen, isFastModelMode, isVoiceModelMode, isVisionModelMode, isCompactionModelMode, isImageModelMode, modelDialogPersistScope, openModelDialog, closeModelDialog, } = useModelCommand();
    ...
    useEffect(() => {
        ...
        if (!chatRecordingService?.setTitleRecordedCallback)
            return;
        ...
        chatRecordingService.setTitleRecordedCallback((customTitle, source, sessionId) => {
    ...
    const clearPendingStateRef = useRef(() => { });
    const clearPendingStateFromRef = useCallback(() => clearPendingStateRef.current(), []);
    const { isResumeDialogOpen, resumeMatchedSessions, openResumeDialog, closeResumeDialog, handleResume, } = useResumeCommand({
        config,
        settings,
        historyManager,
    ...
    const { isDeleteDialogOpen, openDeleteDialog, closeDeleteDialog, handleDelete, handleDeleteMany, } = useDeleteCommand({
        config,
        addItem: historyManager.addItem,
    });
    const [isHelpDialogOpen, setHelpDialogOpen] = useState(false);
    const [activeHelpTab, setHelpTab] = useState('general');
    ...
    const { isSkillsManagerDialogOpen, openSkillsManagerDialog, closeSkillsManagerDialog, } = useSkillsManagerDialog();
    const { isExtensionsManagerDialogOpen, openExtensionsManagerDialog, closeExtensionsManagerDialog, } = useExtensionsManagerDialog();
    ...
    const openRewindSelectorRef = useRef(() => { });
    const cancelOngoingRequestRef = useRef(() => { });
    ...
    const [skillReviewPending, setSkillReviewPending] = useState(null);
    ...
    const skillReviewDismissedTaskIdsRef = useRef(new Set());
    ...
    const closeSkillReviewDialog = useCallback(() => setIsSkillReviewDialogOpen(false), []);
    const acceptPendingSkill = useCallback((skillName) => {
        if (!skillReviewPending)
            return;
        void config
            .getMemoryManager()
    ...
    const rejectPendingSkill = useCallback((skillName) => {
        if (!skillReviewPending)
            return;
        void config
            .getMemoryManager()
    ...
    useEffect(() => {
        ...
        const refresh = () => {
            ...
            const pendingSkills = withPending.metadata['pendingSkills'];
    ...
    const { handleSlashCommand, slashCommands, recentSlashCommands, pendingHistoryItems: pendingSlashCommandHistoryItems, btwItem, setBtwItem, cancelBtw, commandContext, shellConfirmationRequest, confirmationRequest, reloadCommands, } = useSlashCommandProcessor(config, settings, historyManager.history, historyManager.addItem, historyManager.clearItems, loadHistoryWithLatchReconciliation, refreshStatic, toggleVimEnabled, isProcessing, setIsProcessing, isIdleRef, setGeminiMdFileCount, slashCommandActions, extensionsUpdateStateInternal, isConfigInitialized, logger, historyManager.updateItem, setSessionName, extensionRefreshState);
    ...
    const handleWorktreeExit = useCallback(async (choice) => {
        if (choice === 'cancel') {
        ...
        if (choice === 'remove' && activeWorktree) {
            try {
                ...
                // subdir but the worktree lives at `<repoRoot>/.lailatulcoder/worktrees/`,
                ...
                if (owner !== null && owner !== currentSessionId) {
                    const ownerActive = await isSessionRuntimeActive(owner, [
                        ...
                        config
                            .getDebugLogger()
                            .warn(`Worktree owner runtime check failed for ${owner}: ${error}`);
                        return true;
                    });
                    if (ownerActive) {
                        historyManager.addItem({
                            type: MessageType.ERROR,
                            text: `Refusing to remove worktree "${activeWorktree.slug}" — ` +
                ...
                if (!result.success) {
                    historyManager.addItem({
                        type: MessageType.ERROR,
                        text: `Failed to remove worktree "${activeWorktree.slug}": ` +
                ...
                await clearWorktreeSession(config
                    .getSessionService()
                    .getWorktreeSessionPath(config.getSessionId()));
            }
            catch (error) {
                ...
                historyManager.addItem({
                    type: MessageType.ERROR,
                    text: `Worktree removal failed for "${activeWorktree.slug}": ` +
    ...
    const performMemoryRefresh = useCallback(async () => {
        ...
        if (config.isSafeMode()) {
            ...
            config.setConditionalRulesRegistry(new ConditionalRulesRegistry([], config.getWorkingDir()));
        ...
        historyManager.addItem({
            type: MessageType.INFO,
            text: 'Refreshing hierarchical memory (LAILATUL.md or other context files)...',
        }, Date.now());
        try {
            const { memoryContent, fileCount, contextFilePaths, conditionalRules, projectRoot, } = await loadHierarchicalGeminiMemory(config.getWorkingDir(), settings.merged.context?.loadFromIncludeDirectories
                ? config.getWorkspaceContext().getDirectories()
                : [], config.getFileService(), config.getExtensionContextFilePaths(), config.isTrustedFolder(), settings.merged.context?.importFormat || 'tree', // Use setting or default to 'tree'
            config.getContextRuleExcludes(), {
                loadReason: 'refresh',
                onInstructionsLoaded: createInstructionsLoadedCallback(() => config.getHookSystem()),
            });
            config.setUserMemory(memoryContent);
            config.setGeminiMdFileCount(fileCount);
            config.setContextFilePaths(contextFilePaths);
            config.setConditionalRulesRegistry(new ConditionalRulesRegistry(conditionalRules, projectRoot));
            ...
            debugLogger.debug(`[DEBUG] Refreshed memory content in config: ${memoryContent.substring(0, 200)}...`);
        }
        catch (error) {
    ...
    const midTurnDrainRef = useRef(null);
    const midTurnRestoreRef = useRef(null);
    const goalQueueRef = useRef(null);
    const goalReservationSettlementRef = useRef(Promise.resolve());
    ...
    const waitForReservationSettlement = useCallback(() => goalReservationSettlementRef.current, []);
    const { streamingState, submitQuery, initError, pendingHistoryItems: pendingGeminiHistoryItems, clearPendingState, thought, cancelOngoingRequest, preemptGoalTurn, retryLastPrompt, handleApprovalModeChange, activePtyId, loopDetectionConfirmationRequest, pendingToolCalls, streamingResponseLengthRef, isReceivingContent, } = useGeminiStream(config.getGeminiClient(), historyManager.history, historyManager.addItem, config, isConfigInitialized, settings, onDebugMessage, handleSlashCommand, shellModeActive, () => settings.merged.general?.preferredEditor, onAuthError, performMemoryRefresh, modelSwitchedFromQuotaError, setModelSwitchedFromQuotaError, refreshStatic, (info) => cancelHandlerRef.current(info), setEmbeddedShellFocused, terminalWidth, terminalHeight, midTurnDrainRef, logger, availableTerminalHeightRef, terminalWidthRef, midTurnRestoreRef, goalQueueRef);
    ...
    const tipHistory = useMemo(() => (tipsDisabled ? null : getTipHistory()), [tipsDisabled]);
    useContextualTips({
        streamingState,
    ...
    const { dialogOpen: bgTasksDialogOpen, entries: bgTaskEntries, livePanelFocused: bgLivePanelFocused, } = useBackgroundTaskViewState();
    ...
    const prevStreamingStateRef = useRef(StreamingState.Idle);
    const speculationRef = useRef(IDLE_SPECULATION);
    ...
    const { messageQueue, pendingSubmissionCount, addMessage, enqueueGoalTurn, peekNextUserBatchKey, hasQueuedUserMessages, getPendingSubmissionCount, claimGoalTurn, claimDirectUserAdmission, removeGoalTurns, popNextSubmission, popAllMessages, restoreMessages, drainQueue, } = useMessageQueue();
    ...
    const releaseQueuedGoalReservations = useCallback((turnKeys) => {
        let runtime;
        try {
        ...
        const settlement = previousSettlement.then(async () => {
            await Promise.all(turnKeys.map((turnKey) => runtime.releaseTurn(turnKey)));
        });
        goalReservationSettlementRef.current = settlement.catch((error) => {
            debugLogger.warn(`Failed to release queued Goal turns: ${getErrorMessage(error)}`);
        });
    }, [config]);
    ...
    const confirmRequestMap = useRef(new Map()); // requestId → callId
    const confirmCallIdMap = useRef(new Map()); // callId → requestId
    const confirmEmitted = useRef(new Set());
    useEffect(() => {
        if (!dualOutput || !dualOutput.isConnected)
            return;
        for (const tc of pendingToolCalls) {
            if (tc.status === 'awaiting_approval' &&
                ...
                dualOutput.emitPermissionRequest(requestId, tc.request.name, tc.request.callId, tc.request.args, null, buildPermissionSuggestions(tc.confirmationDetails));
            }
        }
    ...
    useEffect(() => {
        if (!remoteInput)
            return;
        remoteInput.setConfirmationHandler((requestId, allowed) => {
            const callId = confirmRequestMap.current.get(requestId);
            if (!callId) {
                dualOutputRef.current?.emitControlError(requestId, 'unknown request_id (already resolved, cancelled, or never issued)');
                return;
            }
            const tc = pendingToolCallsRef.current.find((t) => t.request.callId === callId && t.status === 'awaiting_approval');
            if (!tc) {
                dualOutputRef.current?.emitControlError(requestId, 'tool call is no longer awaiting approval');
                return;
            }
            const waitingTc = tc;
            if (!waitingTc.confirmationDetails?.onConfirm) {
                dualOutputRef.current?.emitControlError(requestId, 'tool call has no onConfirm handler');
                return;
            }
            void waitingTc.confirmationDetails.onConfirm(allowed
    ...
    const handleFinalSubmit = useCallback((submittedValue, options) => {
        const consumesComposerState = options !== undefined;
        const restoredSubmission = consumesComposerState
            ? restoredSubmissionRef.current
            : null;
        const submittedPromptProvenanceUnavailable = consumesComposerState &&
            submittedPromptProvenanceUnavailableRef.current;
        ...
        const provenanceEnabled = !vimEnabled && submittedPromptCandidate !== undefined;
        const trimmedSubmittedPrompt = submittedPromptCandidate?.trim();
        const submittedPrompt = submittedPromptProvenanceUnavailable || !provenanceEnabled
            ? undefined
        ...
        if (!contextFilesAnnouncedRef.current &&
        ...
        const recoveredAgentsNotice = !isSlashCommand(userPromptText) && !isBtwCommand(userPromptText)
        ...
        const workflowTriggerEnabled = config.isWorkflowsEnabled() &&
        ...
        if (streamingState === StreamingState.Responding &&
            ...
            void Promise.resolve(submitQuery(submittedValue, SendMessageType.UserQuery, undefined, {
        ...
        if (spec.status !== 'idle' &&
            ...
            acceptSpeculation(spec, geminiClient)
                .then((result) => {
                logSpeculation(config, new SpeculationEvent({
                    outcome: 'accepted',
                        ...
                        if (msg.role === 'user' && msg.parts) {
                            ...
                            if (hasText) {
                                const text = msg.parts
                                    ...
                                    .join('');
                                if (text) {
                                    historyManager.addItem({ type: 'user', text }, now);
                                }
                            }
                            ...
                            if (textParts) {
                                historyManager.addItem({ type: 'gemini', text: textParts }, now);
                            }
        ...
        if (streamingState === StreamingState.Idle &&
            ...
            void Promise.resolve(submitQuery(submittedValue, SendMessageType.UserQuery, undefined, submittedPrompt === undefined ? undefined : { submittedPrompt })).catch((error) => {
    ...
    const handleArenaModelsSelected = useCallback((models) => {
    ...
    const rawStickyTodos = useMemo(() => getStickyTodos(historyManager.history, pendingHistoryItems), [historyManager.history, pendingHistoryItems]);
    ...
    cancelHandlerRef.current = useCallback((info) => {
        ...
        if (popped !== null) {
            debugLogger.debug('auto-restore bail: queue had items (drained to buffer)');
            return;
        }
        ...
        if (cancelledTurnUserItem == null) {
            debugLogger.debug('auto-restore bail: cancelled turn did not add a user history item');
            return;
        }
        ...
        if (pendingHistoryItems.some((item) => item.type === 'tool_group')) {
            debugLogger.debug('auto-restore bail: tool execution is pending or committed');
            return;
        }
        ...
        if (info?.turnProducedMeaningfulContent) {
            debugLogger.debug('auto-restore: preserving streamed output and restoring prompt text');
            restoreCancelledPrompt();
            return;
        }
        if (pendingHistoryItems.some((item) => !isSyntheticHistoryItem(item))) {
            debugLogger.debug('auto-restore bail: pending stream item has meaningful content');
            return;
        }
        ...
        if (!itemsAfterAreOnlySynthetic(history, lastUserIdx)) {
            debugLogger.debug('auto-restore: preserving committed output and restoring prompt text');
            restoreCancelledPrompt();
            return;
        }
        const lastUserItem = history[lastUserIdx];
        if (lastUserItem.type !== 'user') {
            debugLogger.debug('auto-restore bail: lastUserItem type narrowing failed (unexpected)');
            return;
        }
        ...
        // a cheap sanity belt.
        if (lastUserItem.id !== cancelledTurnUserItem.id ||
            lastUserItem.text !== cancelledTurnUserItem.text) {
            debugLogger.debug('auto-restore bail: lastUserItem identity does not match cancelled-turn user item');
            return;
        }
        debugLogger.debug('auto-restore: rewinding cancelled turn and restoring prompt');
    ...
    useEffect(() => {
        ...
        if (prevStreamingStateRef.current === StreamingState.Idle &&
            streamingState === StreamingState.Responding) {
            suggestionAbortRef.current?.abort();
            setPromptSuggestion(null);
            if (speculationRef.current.status !== 'idle') {
        ...
        if (followupSuggestionsEnabled &&
            ...
            generatePromptSuggestion(config, conversationHistory, ac.signal, {
                ...
                if (result.suggestion) {
                    ...
                    logPromptSuggestion(config, new PromptSuggestionEvent({
                        outcome: 'suppressed',
    ...
    const [startupIdeConnectionStatus, setStartupIdeConnectionStatus] = useState({ state: 'idle' });
    useEffect(() => {
        const getIde = async () => {
            const ideClient = await IdeClient.getInstance();
    ...
    const { showMigrationNudge: shouldShowCommandMigrationNudge, tomlFiles: commandMigrationTomlFiles, setShowMigrationNudge: setShowCommandMigrationNudge, } = useCommandMigration(settings, config.storage);
    ...
    const [renderMode, setRenderMode] = useState(configuredRenderMode === 'raw' ? 'raw' : 'render');
    ...
    const { isMcpApprovalDialogOpen, currentMcpApproval, pendingMcpApprovals, mcpApprovalRemaining, handleMcpApprovalSelect, } = useMcpApproval(config);
    const { needsRestart: ideNeedsRestart, restartReason: ideTrustRestartReason, } = useIdeTrustListener();
    const { isFeedbackDialogOpen, openFeedbackDialog, closeFeedbackDialog, temporaryCloseFeedbackDialog, submitFeedback, } = useFeedbackDialog({
        config,
        settings,
        streamingState,
        history: historyManager.history,
    ...
    const shouldShowStickyTodos = stickyTodos !== null &&
        !dialogsVisible &&
        !isFeedbackDialogOpen &&
        streamingState === StreamingState.Responding;
    const stickyTodoWidth = Math.min(mainAreaWidth, 64);
    const stickyTodoMaxVisibleItems = getStickyTodoMaxVisibleItemsForMode(terminalHeight, useTerminalBuffer);
    const stickyTodosLayoutKey = shouldShowStickyTodos
        ? getStickyTodosLayoutKey(stickyTodos, stickyTodoWidth, stickyTodoMaxVisibleItems)
        : 'hidden';
    ...
    const liveAgentPanelLayoutKey = getLiveAgentPanelLayoutKey(bgTaskEntries, bgLivePanelFocused);
    useLayoutEffect(() => {
        if (!mainControlsRef.current) {
            setControlsHeight((previousHeight) => previousHeight === 0 ? previousHeight : 0);
            return;
        }
        const fullFooterMeasurement = measureElement(mainControlsRef.current);
        setControlsHeight((previousHeight) => previousHeight === fullFooterMeasurement.height
            ? previousHeight
    ...
    const handleRewindConfirm = useCallback(async (userItem, option) => {
        try {
            ...
            if (needsConversation) {
                if (!geminiClient) {
                    if (option === 'conversation') {
                        historyManager.addItem({
                            type: 'error',
                            text: t('Cannot rewind conversation: no active model client.'),
                        }, Date.now());
                        return;
                    }
                    ...
                    apiTruncateIndex = computeApiTruncationIndex(historyManager.history, userItem.id, geminiClient.getHistoryShallow());
                    if (apiTruncateIndex < 0) {
                        historyManager.addItem({
                            type: 'error',
                            text: t('Cannot rewind to a turn that was compressed. Try a more recent turn.'),
                        }, Date.now());
                        if (option === 'both') {
            ...
            let fileRestoreMessage;
            let fileRestoreError;
            let hasRestoreFailure = false;
            if (option === 'code' || option === 'both') {
                const promptId = userItem.promptId;
                if (promptId) {
                    try {
                        const truncateHistory = option === 'both' && !!geminiClient && apiTruncateIndex >= 0;
                    ...
                    hasRestoreFailure = true;
                    fileRestoreError = t('Cannot restore files: this turn was created before file checkpointing was enabled.');
                }
            }
            ...
            if (needsConversation &&
                ...
                const hasSummary = originalHistory.some((h) => h.display?.kind === 'collapse-summary');
                ...
                const truncatedUi = expandCollapsedHistory(originalHistory.filter((h) => h.id < userItem.id));
                clearPendingStateRef.current();
                ...
                historyManager.addItem({
                    type: 'info',
                    text: t('Conversation rewound. Edit your prompt and press Enter to continue.'),
                }, Date.now());
                config.getChatRecordingService()?.rewindRecording(targetTurnIndex, { truncatedCount: effectiveLength - truncatedUi.length }, !hasRestoreFailure
                    ? config
                        .getFileHistoryService()
                        .getSnapshots()
            ...
            if (fileRestoreMessage) {
                historyManager.addItem({ type: 'info', text: fileRestoreMessage }, Date.now());
            }
            if (fileRestoreError) {
                historyManager.addItem({ type: 'error', text: fileRestoreError }, Date.now());
            }
            if (conversationSkippedNoClient) {
                historyManager.addItem({
                    type: 'info',
                    text: t('Code restored, but conversation could not be rewound (no active client).'),
                }, Date.now());
            }
        }
        catch (error) {
    ...
    const handleDoubleEscRewind = useDoublePress(openRewindSelector, (pending) => setRewindEscPending(pending));
    ...
    const handleCommandMigrationComplete = useCallback(async (result) => {
        ...
        if (result.userSelection === 'yes') {
            ...
            try {
                ...
                if (workspaceResult.convertedFiles.length > 0 ||
                ...
                if (userResult.convertedFiles.length > 0 ||
    ...
    const responseCandidateTokens = getResponseCandidateTokens(pendingGeminiHistoryItems);
    const { elapsedTime, currentLoadingPhrase, taskStartTokens, taskStartStreamingChars, } = useLoadingIndicator(streamingState, settings.merged.ui?.customWittyPhrases, responseCandidateTokens, streamingResponseLengthRef.current, hasExecutingTool);
    ...
    useEffect(() => {
        ...
        if (!registry?.setNotificationCallback)
            return;
        registry.setNotificationCallback((entry) => {
            ...
            sendNotification({ message: `Workflow '${name}' ${verb}`, title: 'LailatulCoder Ai' }, terminal, workflowBellEnabled);
        });
    ...
    const handleGlobalKeypress = useCallback((key) => {
        ...
        if (handleRenderModeToggleKey(key, setRenderMode)) {
            ...
            const promotableShells = pendingToolCallsRef.current.filter((tc) => tc.status === 'executing' &&
            ...
            if (promotableShells.length === 1) {
                ...
                debugLogger.debug(`Ctrl+B promote: matched executing shell tool call ${executingShell.request.callId}`);
                executingShell.promoteAbortController.abort({
                    kind: 'background',
                });
                return;
            }
            debugLogger.debug(`Ctrl+B promote: expected exactly one executing shell tool call; falling through ` +
    ...
    useEffect(() => {
        ...
        if (settings.merged.ui?.showStatusInTitle === false) {
            if (lastTitleRef.current !== null) {
                ...
                writeTerminalTitle((value) => process.stdout.write(value), formatSessionWindowTitle(null, folderName));
            }
            return;
        }
        const folderName = basename(config.getTargetDir());
        const title = formatSessionWindowTitle(sessionName, folderName, titleStatusPrefix(streamingState));
    ...
    const renderModeValue = useMemo(() => ({ renderMode, setRenderMode }), [renderMode, setRenderMode]);
    ...
    return (_jsx(VirtualViewportContext.Provider, { value: useTerminalBuffer, children: _jsx(UIStateContext.Provider, { value: uiState, children: _jsx(UIActionsContext.Provider, { value: uiActions, children: _jsx(ConfigContext.Provider, { value: config, children: _jsx(AppContext.Provider, { value: {
                            version: props.version,
                            startupWarnings,
                        }, children: _jsx(ThoughtExpandedProvider, { value: thoughtExpandedValue, children: _jsx(RenderModeProvider, { value: renderModeValue, children: _jsx(TerminalOutputProvider, { value: writeRaw, children: _jsx(ShellFocusContext.Provider, { value: isFocused, children: _jsx(App, {}) }) }) }) }) }) }) }) }) }));
};
