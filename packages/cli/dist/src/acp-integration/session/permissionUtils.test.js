...
describe('permissionUtils', () => {
    ...
    it('accepts only an option that was actually offered', () => {
        ...
        expect(resolvePermissionOutcome({
            outcome: {
                outcome: 'selected',
                optionId: ToolConfirmationOutcome.ProceedOnce,
            },
        }, options)).toBe(ToolConfirmationOutcome.ProceedOnce);
        expect(() => resolvePermissionOutcome({
            outcome: {
                outcome: 'selected',
                optionId: ToolConfirmationOutcome.ProceedAlwaysProject,
            },
        }, options)).toThrow('unoffered option');
    });
    it('aborts permission requests and ignores late settlement', async () => {
        let resolveRequest;
        const client = {
            requestPermission: vi.fn(() => new Promise((resolve) => {
                resolveRequest = resolve;
            })),
        };
        const controller = new AbortController();
        const request = requestPermissionWithAbort(client, { sessionId: 'session', options: [], toolCall: {} }, controller.signal);
        controller.abort();
        await expect(request).rejects.toThrow('aborted');
        resolveRequest?.({});
    });
});
//# sourceMappingURL=permissionUtils.test.js.map