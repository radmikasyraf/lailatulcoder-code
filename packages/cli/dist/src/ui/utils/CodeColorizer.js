...
import { getLowlightInstance, isLowlightCoolingDown, loadLowlight, } from './lowlightLoader.js';
 ...
 */
export function looksLikeDiagramOrArt(line) {
 ...
 */
function ensureLowlightLoading() {
...
function highlightAndRenderLine(line, language, theme, lowlight) {
...
export function colorizeCode(code, language, availableHeight, maxWidth, options = {}) {
    ...
    try {
        ...
        return (_jsx(MaxSizedBox, { maxHeight: availableHeight, maxWidth: maxWidth, additionalHiddenLinesCount: hiddenLinesCount, overflowDirection: "top", children: lines.map((line, index) => {
                ...
                return (_jsxs(Box, { children: [showLineNumbers && (_jsx(Text, { color: activeTheme.colors.Gray, selectable: false, children: `${String(index + firstLineNumber + hiddenLinesCount).padStart(padWidth, ' ')} ` })), _jsx(Text, { color: activeTheme.defaultColor, wrap: "wrap", children: contentToRender })] }, index));
            }) }));
    }
    catch (error) {
        ...
        const padWidth = String(lines.length + firstLineNumber - 1).length;
        return (_jsx(MaxSizedBox, { maxHeight: availableHeight, maxWidth: maxWidth, overflowDirection: "top", children: lines.map((line, index) => (_jsxs(Box, { children: [showLineNumbers && (_jsx(Text, { color: activeTheme.defaultColor, selectable: false, children: `${String(index + firstLineNumber).padStart(padWidth, ' ')} ` })), _jsx(Text, { color: activeTheme.colors.Gray, children: line })] }, index))) }));
    }
}
