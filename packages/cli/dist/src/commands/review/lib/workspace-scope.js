...
import { affectedWorkspaces, isNegationExcluded, reverseDependencyClosure, workspaceDirFor, } from './workspaces.js';
/**
 ...
 */
const LICENSE_LIKE_RE = /(^|\/)(LICEN[CS]ES?|COPYING|NOTICES?)(-[^/.]+)?(\.(md|txt|rst))?$/;
/**
 ...
 */
export function isInertLicense(path) {
 ...
 */
const INERT_OUTSIDE_RE = /^(\.github\/|CHANGELOG\.md$|\.gitignore$|\.gitattributes$|\.editorconfig$|\.idea\/|\.vscode\/)/;
/**
...
export function resolveTestScope(input) {
    ...
    // repo's root and packages/cli share the name `@lailatul-coder/lailatul-coder`, and
    ...
    const caveats = [];
    if (skipped.length > 0) {
        caveats.push(`the workspace graph could not be fully computed: ${skipped.join(', ')} ` +
    ...
    const unmapped = affected.filter((d) => d !== '.' && !scriptsOf.has(d) && !skipped.includes(d));
    ...
    const influential = outside.filter((f) => !isInertLicense(f) &&
        !f.startsWith('docs/') &&
        !INERT_OUTSIDE_RE.test(f) &&
        !isNegationExcluded(f, globs));
    if (unmapped.length > 0) {
        caveats.push(`the workspace globs claim ${unmapped.join(', ')}, but no readable ` +
    ...
    if (influential.length > 0) {
        caveats.push(`${influential.length} changed file(s) sit outside every workspace ` +
    ...
    if (excluded.length > 0) {
        caveats.push(`${excluded.length} changed file(s) sit in negated workspaces (e.g. ` +
    ...
    {
        const rootRuns = input.rootPackage?.scripts.includes('test') && !input.rootTestFansOut
            ? 1
            : 0;
        const testable = packages.filter((p) => p.scripts.includes('test')).length + rootRuns;
        const scoped = workspaces.length;
        if (scoped * 2 > testable) {
            caveats.push(`the diff's reverse-dependency closure covers ${scoped} of ` +
                `${testable} testable ${rootRuns ? 'suites (including the root)' : 'workspaces'} — more than half, so the scoped set is not a meaningful narrowing`);
        }
    }
    if (fanOutCaveat)
        caveats.push(fanOutCaveat);
