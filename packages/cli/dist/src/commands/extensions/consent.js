 ...
 */
export async function requestChoicePluginNonInteractive(marketplace) {
...
export async function requestConsentInteractive(consentDescription, addExtensionUpdateConfirmationRequest) {
    return promptForConsentInteractive(consentDescription + '\n\n' + t('Do you want to continue?'), addExtensionUpdateConfirmationRequest);
}
/**
...
export function extensionConsentString(extensionConfig, commands = [], skills = [], subagents = [], originSource = 'QwenCode') {
    ...
    if (originSource !== 'QwenCode' && originSource !== 'AgentPlugins') {
        output.push(t('You are installing an extension from {{originSource}}. Some features may not work perfectly with LailatulCoder Ai.', { originSource }));
    }
    ...
    if (typeof extensionConfig.description === 'string' &&
        extensionConfig.description) {
        output.push(stripAnsi(extensionConfig.description));
    }
    output.push(t('**Extensions may introduce unexpected behavior. Ensure you have investigated the extension source and trust the author.**'));
    ...
    if (commands && commands.length > 0) {
        output.push(t('This extension will add the following commands: {{commands}}.', {
    ...
    if (extensionConfig.contextFileName) {
        ...
        output.push(t('This extension will append info to your LAILATUL.md context using {{fileName}}', { fileName }));
    }
    if (skills.length > 0) {
...
export const requestConsentOrFail = async (requestConsent, options) => {
    ...
    if (!(await requestConsent(extensionConsent))) {
        throw new Error(t('Installation cancelled for "{{name}}".', {
