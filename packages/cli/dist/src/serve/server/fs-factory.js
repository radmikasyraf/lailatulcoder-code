 ...
 */
export function parseNewFileModePolicy(env = process.env) {
    ...
    if (normalized === 'system')
        return 'system';
    if (normalized === 'owner' || normalized === '0600')
        return 'owner';
    writeStderrLine(`LailatulCoder serve: ignoring invalid ${NEW_FILE_MODE_ENV_VAR}=${raw} ` +
...
export function createDefaultFsAuditEmit() {
    ...
    return (event) => {
        ...
        if (droppedCount === 1 || droppedCount % WARN_EVERY === 0) {
            ...
            writeStderrLine(`LailatulCoder serve: fs audit emit is the default no-op — ${droppedCount} event(s) dropped so far. ` +
...
export function resolveBoundWorkspacesFromIdeEnv(primaryWorkspace, ideWorkspacePath = process.env[IDE_WORKSPACE_PATH_ENV_VAR], includeWorkspace) {
    ...
    try {
        ...
        writeStderrLine(`LailatulCoder serve: failed to canonicalize IDE workspace paths, using primary only: ${err}`);
        return [primary];
    }
    for (const workspace of parseIdeWorkspacePathEnv(ideWorkspacePath)) {
        try {
            ...
            writeStderrLine(`LailatulCoder serve: skipping IDE workspace root that failed to canonicalize: ${workspace} (${err})`);
        }
    }
    if (envCanonicals.length > 0 &&
        !envCanonicals.some((workspace) => isWithinRoot(primary, workspace) || isWithinRoot(workspace, primary))) {
        writeStderrLine('LailatulCoder serve: ignoring stale IDE workspace paths that do not overlap ' +
...
function dropNestedWorkspacesPreservingPrimary(workspaces) {
    ...
    if (filtered.length < workspaces.length) {
        writeStderrLine('LailatulCoder serve: dropping nested IDE workspace roots ' +
