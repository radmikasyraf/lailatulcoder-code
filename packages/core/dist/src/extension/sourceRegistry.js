...
function pluginComponents(plugin) {
...
function pluginLastUpdated(plugin) {
    const record = plugin;
    const value = record['lastUpdated'] ?? record['updatedAt'] ?? record['updated'];
    return typeof value === 'string' ? value : undefined;
}
function pluginInstalls(plugin) {
    const record = plugin;
    const value = record['installs'] ?? record['installCount'] ?? record['downloads'];
 ...
 */
export function parseExtensionSourceType(source) {
...
function isOwnerRepoShorthand(source) {
 ...
 */
function resolveInstallSource(marketplace, plugin) {
    ...
    if (typeof src === 'string') {
        ...
        if (path.isAbsolute(src) || src.startsWith('.') || src.startsWith('~')) {
            debugLogger.warn(`Ignoring local path source "${src}" from remote marketplace "${marketplace.source}".`);
            return plugin.name;
        }
    ...
    if (src && src.source === 'url') {
        ...
        if (typeof src.url === 'string' &&
            (path.isAbsolute(src.url) ||
                src.url.startsWith('.') ||
                src.url.startsWith('~'))) {
            debugLogger.warn(`Ignoring local path source "${src.url}" from remote marketplace "${marketplace.source}".`);
            return plugin.name;
        }
        return src.url;
    }
...
function pluginsFromConfig(marketplace, config, installedNames) {
...
export class SourceRegistryStore {
    ...
    read() {
        ...
        try {
            ...
            process.stderr.write(`[warn] Could not read marketplace registry at ${this.filePath}: ${error instanceof Error ? error.message : String(error)}. Using an empty source list for this session.\n`);
     ...
     */
    add(source) {
        const sources = this.read().filter((existing) => existing.name !== source.name && existing.source !== source.source);
        sources.push(source);
        this.write(sources);
    }
 ...
 */
export async function discoverPlugins(sources, installedNames, networkPolicy) {
    const results = await Promise.all(sources.map(async (marketplace) => {
        try {
            const config = await loadMarketplaceConfigFromSource(marketplace.source, networkPolicy);
            if (!config) {
                debugLogger.debug(`No marketplace config resolved for ${redactUrlCredentials(marketplace.source)}`);
                return [];
            }
            return pluginsFromConfig(marketplace, config, installedNames);
        }
        catch (error) {
            debugLogger.error(`Failed to discover plugins from ${redactUrlCredentials(marketplace.source)}:`, error);
            return [];
        }
    }));
