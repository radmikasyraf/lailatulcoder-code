...
import { registerWorkspaceManagementRoutes, } from './workspace-management.js';
import { NativeDirectoryPickerUnavailableError } from '../native-directory-picker.js';
import { tmpdir } from 'node:os';
import { realpathSync } from 'node:fs';
import { mkdir, mkdtemp, readdir, rm, symlink, writeFile, } from 'node:fs/promises';
import { join } from 'node:path';
import { workspaceRegistrationId, WorkspaceRegistrationStoreCommittedError, WorkspaceRegistrationStoreLimitError, } from '../workspace-registration-store.js';
...
function createMockRegistry(runtimes = []) {
    ...
    return {
        primary: runtimes[0],
        list: () => Object.freeze(runtimes.filter((runtime) => !draining.has(runtime))),
        ...
        getManagedByWorkspaceCwd: (cwd) => byCwd.get(cwd),
        getByWorkspaceId: (id) => {
            const runtime = byId.get(id);
            return runtime && !draining.has(runtime) ? runtime : undefined;
        },
        getManagedByWorkspaceId: (id) => byId.get(id),
        syncRuntimeMetadata: vi.fn((runtime) => {
        ...
        beginDrain: vi.fn((runtime) => {
            if (draining.has(runtime))
                return false;
            draining.add(runtime);
            return true;
        }),
        cancelDrain: vi.fn((runtime) => draining.delete(runtime)),
        commitDrain: vi.fn(),
        completeDrain: vi.fn((runtime) => {
...
function makeRuntime(cwd, overrides = {}) {
    return {
        workspaceId: `id-${cwd}`,
        workspaceCwd: cwd,
        primary: false,
...
function createApp(overrides) {
    ...
    const deps = {
        ...
        createWorkspaceRuntime: vi
            .fn()
            .mockImplementation((cwd) => Promise.resolve(makeRuntime(cwd))),
        ...overrides,
    };
    const handle = registerWorkspaceManagementRoutes(app, deps);
    return { app, deps, handle };
}
function createRemovalController(pendingSessionStarts = 0) {
    return {
        beginDrain: vi.fn(),
        cancelDrain: vi.fn(),
        completeDrain: vi.fn(),
...
describe('owned workspace runtime publication', () => {
    it('allows only the daemon-owned Live runtime to nest under the primary workspace', async () => {
        ...
        const runtime = makeRuntime('/Users/test/Documents/LailatulCoder Ai/Conversations', {
        ...
        await expect(handle.publishOwnedRuntime(runtime.workspaceCwd, 'live-conversation', () => undefined)).resolves.toBe(runtime);
    ...
    it('keeps nested owned runtimes blocked for non-Live provenance', async () => {
        ...
        await expect(handle.publishOwnedRuntime(runtime.workspaceCwd, 'managed-scratch', () => undefined)).rejects.toThrow('nests with an existing workspace');
    ...
    it('does not let the Live root contain an existing workspace', async () => {
        ...
        await expect(handle.publishOwnedRuntime(runtime.workspaceCwd, 'live-conversation', () => undefined)).rejects.toThrow('nests with an existing workspace');
        expect(createWorkspaceRuntime).not.toHaveBeenCalled();
    });
    it('shares registry publication and runtime-added hooks', async () => {
        ...
        await expect(handle.publishOwnedRuntime(runtime.workspaceCwd, 'live-conversation', (candidate) => {
    ...
    it('immediately disposes a candidate rejected before publication', async () => {
        ...
        await expect(handle.publishOwnedRuntime(runtime.workspaceCwd, 'live-conversation', () => {
            throw new Error('ownership rejected');
        })).rejects.toThrow('ownership rejected');
        expect(registry.getManagedByWorkspaceCwd(runtime.workspaceCwd)).toBe(undefined);
    ...
    it('rejects and disposes a primary owned-runtime candidate', async () => {
        ...
        await expect(handle.publishOwnedRuntime(runtime.workspaceCwd, 'live-conversation', () => undefined)).rejects.toThrow('Daemon-owned workspace runtime must not be primary');
    ...
    it('disposes a candidate rejected by final pre-publication validation', async () => {
        ...
        await expect(handle.publishOwnedRuntime(runtime.workspaceCwd, 'live-conversation', validate)).rejects.toThrow('root changed before publication');
    ...
    it('keeps a candidate unpublished and the topology lock free during final validation', async () => {
        ...
        const publication = handle.publishOwnedRuntime(runtime.workspaceCwd, 'live-conversation', validate);
        ...
        expect(registry.getManagedByWorkspaceCwd(runtime.workspaceCwd)).toBeUndefined();
        expect(registry.add).not.toHaveBeenCalled();
...
describe('POST /workspaces', () => {
    ...
    it('reserves the Conversations root and its children but allows its parent', async () => {
        ...
        try {
            ...
            await expect(request(app).post('/workspaces').send({ cwd: parent, persist: false })).resolves.toMatchObject({ status: 201 });
        }
        finally {
    ...
    it('allows scratch creation but protects existing paths in loopback development', async () => {
        ...
        try {
            ...
            const mutate = vi.fn((options) => (_req, res, next) => {
                if (options?.strict) {
    ...
    it('creates a trusted ephemeral runtime with managed provenance', async () => {
        ...
        try {
            const root = prepareManagedScratchRoot(join(parent, 'root'), []);
            const runtimeRemoval = createRemovalController();
            const factory = vi.fn((cwd) => Promise.resolve(makeRuntime(cwd, { trusted: true })));
    ...
    it('re-registers a scratch child as existing without restoring managed trust', async () => {
        ...
        try {
            ...
            const factory = vi.fn((cwd, options) => Promise.resolve(makeRuntime(cwd, {
    ...
    it('preserves a created directory when runtime construction fails', async () => {
        ...
        try {
            ...
            expect(writeStderrLine).toHaveBeenCalledWith(expect.stringContaining(`retained directory: ${join(root.canonicalRoot, retained)}`));
        }
        finally {
            await rm(parent, { recursive: true, force: true });
        }
    ...
    it('reserves capacity before scratch filesystem work can yield', async () => {
        ...
        try {
            const root = prepareManagedScratchRoot(join(parent, 'root'), []);
            const registry = createMockRegistry(Array.from({ length: 24 }, (_, index) => makeRuntime(`/workspace-${index}`)));
            let releaseFactory;
            ...
            const { app } = createApp({
                workspaceRegistry: registry,
                createWorkspaceRuntime: vi.fn(async (cwd) => {
                    await blocked;
    ...
    it('sealing waits for construction and disposes an unregistered runtime once', async () => {
        ...
        try {
            ...
            const factory = vi.fn(async (cwd) => {
                await blocked;
                return { ...runtime, workspaceCwd: cwd };
            });
    ...
    it('disposes a runtime that violates the managed scratch contract', async () => {
        ...
        try {
            ...
            const { app, deps } = createApp({
                workspaceRegistry: createMockRegistry([makeRuntime('/workspace')]),
                createWorkspaceRuntime: vi.fn((cwd) => Promise.resolve(makeRuntime(cwd, { trusted: false }))),
                managedScratchRoot: root,
                runtimeRemoval,
            });
            const res = await request(app)
    ...
    it.skipIf(process.platform === 'win32')('translates a Windows-shaped cwd past the absolute-path guard in a sandbox', async () => {
    ...
    it.skipIf(process.platform === 'win32')('still rejects a Windows-shaped cwd outside a sandbox', async () => {
        vi.stubEnv('SANDBOX', '');
        try {
            const { app } = createApp();
            const res = await request(app)
    ...
    it('does not double-count a runtime while its addition hook is pending', async () => {
        ...
        try {
            const registry = createMockRegistry(Array.from({ length: 23 }, (_, index) => makeRuntime(`/registered-${index}`)));
            let releaseAddition;
            const additionPending = new Promise((resolve) => {
    ...
    it('returns the limit error when a concurrent writer fills the store', async () => {
        const { app } = createApp({
            workspaceRegistrationStore: {
                ...
                add: vi
                    .fn()
                    .mockRejectedValue(new WorkspaceRegistrationStoreLimitError('limit reached')),
            },
        });
        const res = await request(app)
            .post('/workspaces')
    ...
    it('rolls back a newly persisted record when runtime registration fails', async () => {
        ...
        expect(res.status).toBe(500);
        expect(res.body.code).toBe('runtime_creation_failed');
        expect(removeById).toHaveBeenCalledWith(expect.stringMatching(/^[a-f0-9]{16}$/));
...
describe('PATCH /workspaces/:workspace', () => {
    ...
    it('updates every persistent registration alias before the runtime', async () => {
        ...
        const setDisplayNameByIds = vi.fn(async (_ids, _displayName) => {
            expect(runtime.displayName).toBe('Old name');
            return 2;
        });
        const { app } = createApp({
        ...
        const { app } = createApp({
            ...
            workspaceRegistrationStore: {
                setDisplayNameByIds: vi
                    .fn()
                    .mockRejectedValue(new WorkspaceRegistrationStoreCommittedError('release failed')),
            },
        });
        const res = await request(app)
            .patch(`/workspaces/${encodeURIComponent(runtime.workspaceId)}`)
    ...
    it('serializes removal and shutdown behind a pending update', async () => {
        ...
        const removal = await request(app).delete(`/workspaces/${encodeURIComponent(runtime.workspaceId)}`);
...
describe('DELETE /workspaces/:workspace', () => {
    ...
    it('validates force and protects primary and static runtimes', async () => {
        ...
        const forbidden = await request(primaryApp).delete(`/workspaces/${encodeURIComponent(primary.workspaceId)}`);
        ...
        const staticResult = await request(staticApp).delete(`/workspaces/${encodeURIComponent(staticRuntime.workspaceId)}`);
    ...
    it('returns 501 when runtime removal is unavailable', async () => {
        ...
        const res = await request(app).delete(`/workspaces/${encodeURIComponent(runtime.workspaceId)}`);
        expect(res.status).toBe(501);
    ...
    it('rolls every gate back when the final frozen snapshot becomes busy', async () => {
        ...
        expect(acpHandle.beginWorkspaceDrain).toHaveBeenCalledWith(runtime.workspaceId);
        expect(acpHandle.cancelWorkspaceDrain).toHaveBeenCalledWith(runtime.workspaceId);
    ...
    it('force-removes activity, aliases, runtime resources, and registry state', async () => {
        ...
        expect(vi.mocked(deps.workspaceRegistry.commitDrain).mock.invocationCallOrder[0]).toBeLessThan(vi.mocked(runtimeRemoval.disposeRuntime).mock.invocationCallOrder[0]);
        expect(runtimeRemoval.completeDrain).toHaveBeenCalledWith(runtime);
        expect(acpHandle.commitWorkspaceRemoval).toHaveBeenCalledWith(runtime.workspaceId);
        expect(acpHandle.disposeWorkspace).toHaveBeenCalledWith(runtime.workspaceId);
        expect(deps.workspaceRegistry.getManagedByWorkspaceId(runtime.workspaceId)).toBeUndefined();
    });
    it('does not reactivate a runtime when cleanup fails after persistence commits', async () => {
        ...
        const runtimeRemoval = createRemovalController();
        vi.mocked(runtimeRemoval.disposeRuntime).mockRejectedValueOnce(new Error('bridge cleanup failed'));
    ...
    it('continues cleanup when registry drain commit fails after persistence commits', async () => {
        ...
        expect(registry.getManagedByWorkspaceId(runtime.workspaceId)).toBeUndefined();
        expect(writeStderrLine).toHaveBeenCalledWith('qwen serve: failed to commit workspace registry drain: registry commit failed');
    });
    it('accepts a URL-encoded absolute cwd selector', async () => {
        ...
        const res = await request(app).delete(`/workspaces/${encodeURIComponent(runtime.workspaceCwd)}`);
        expect(res.status).toBe(200);
        expect(res.body.workspaceCwd).toBe(runtime.workspaceCwd);
        expect(deps.workspaceRegistry.getManagedByWorkspaceCwd(runtime.workspaceCwd)).toBeUndefined();
    });
    it('canonicalizes a symlink cwd selector before removal', async () => {
        ...
        await symlink(REAL_DIR, selector, process.platform === 'win32' ? 'junction' : 'dir');
        const runtime = makeRuntime(REAL_DIR);
        const { app, deps } = createApp({
            workspaceRegistry: createMockRegistry([runtime]),
            runtimeRemoval: createRemovalController(),
        ...
        try {
            const res = await request(app).delete(`/workspaces/${encodeURIComponent(selector)}`);
            expect(res.status).toBe(200);
            expect(res.body.workspaceCwd).toBe(runtime.workspaceCwd);
            expect(deps.workspaceRegistry.getManagedByWorkspaceCwd(runtime.workspaceCwd)).toBeUndefined();
        }
    ...
    it('reserves the cwd against concurrent remove and add operations', async () => {
        ...
        const firstRemoval = request(app).delete(`/workspaces/${encodeURIComponent(runtime.workspaceId)}`);
        const firstResult = firstRemoval.then((res) => res);
        await vi.waitFor(() => {
            expect(runtimeRemoval.disposeRuntime).toHaveBeenCalledWith(runtime, 'workspace_removed');
        });
        const duplicateRemoval = await request(app).delete(`/workspaces/${encodeURIComponent(runtime.workspaceId)}`);
    ...
    it('waits for an in-flight removal after sealing and rejects new work', async () => {
        ...
        const removal = request(app).delete(`/workspaces/${encodeURIComponent(runtime.workspaceId)}`);
...
describe('persistent workspace registrations', () => {
    it('returns 501 for registration management without a store', async () => {
        ...
        const remove = await request(app).delete('/workspace-registrations/missing');
    ...
    it('keeps legacy Conversations registrations inactive and only forgets the stored record', async () => {
        ...
        const removed = await request(app).delete(`/workspace-registrations/${registrationId}`);
    ...
    it('forgets persistence without unloading an active runtime', async () => {
        ...
        const registrationId = workspaceRegistrationId(REAL_DIR);
        const res = await request(app).delete(`/workspace-registrations/${registrationId}`);
        expect(res.status).toBe(200);
        expect(removeById).toHaveBeenCalledWith(registrationId);
        ...
        const aliasResult = await request(app).delete(`/workspace-registrations/${aliasId}`);
    ...
    it('does not require restart when forgetting an alias of a static runtime', async () => {
        ...
        const res = await request(app).delete(`/workspace-registrations/${aliasId}`);
        expect(res.status).toBe(200);
        expect(res.body).toMatchObject({
            active: true,
            restartRequired: false,
    ...
    it('treats a draining runtime registration as active', async () => {
        ...
        const res = await request(app).delete(`/workspace-registrations/${workspaceRegistrationId(REAL_DIR)}`);
        expect(res.status).toBe(200);
        expect(res.body).toMatchObject({
            active: true,
            restartRequired: true,
    ...
    it('serializes forget and runtime removal for the same workspace', async () => {
        ...
        const runtime = makeRuntime(REAL_DIR);
        let finishForget;
        const forgetting = new Promise((resolve) => {
        ...
        const pendingForget = request(app).delete(`/workspace-registrations/${registrationId}`);
        const forgetResult = pendingForget.then((res) => res);
        await vi.waitFor(() => expect(removeById).toHaveBeenCalledOnce());
        const removalWhileForgetting = await request(app).delete(`/workspaces/${encodeURIComponent(runtime.workspaceId)}`);
        expect(removalWhileForgetting.status).toBe(409);
        expect(removalWhileForgetting.body.code).toBe('workspace_registration_in_progress');
        finishForget();
        expect((await forgetResult).status).toBe(200);
        const pendingRemoval = request(app).delete(`/workspaces/${encodeURIComponent(runtime.workspaceId)}`);
        const removalResult = pendingRemoval.then((res) => res);
        await vi.waitFor(() => expect(removeByIds).toHaveBeenCalledOnce());
        const forgetWhileRemoving = await request(app).delete(`/workspace-registrations/${registrationId}`);
    ...
    it('waits for an inactive forget that is still reading its registration', async () => {
        ...
        const pendingForget = request(app).delete('/workspace-registrations/inactive');
        const forgetResult = pendingForget.then((res) => res);
    ...
    it('finishes a sealed forget operation when registration reading fails', async () => {
        let failRead;
        const reading = new Promise((_resolve, reject) => {
...
describe('GET /workspace-path-suggestions', () => {
    ...
    it('hides dot-directories unless the filter starts with a dot', async () => {
        ...
        const withoutDot = await request(app)
            .get('/workspace-path-suggestions')
            .query({ prefix: `${base}/` });
        expect(withoutDot.body.suggestions.map((s) => s.name)).not.toContain('.hidden');
        const withDot = await request(app)
            .get('/workspace-path-suggestions')
            .query({ prefix: join(base, '.hi') });
        expect(withDot.body.suggestions.map((s) => s.name)).toEqual(['.hidden']);
    });
...
describe('POST /workspace-directory-picker', () => {
    ...
    it('returns 501 when the native picker is unavailable', async () => {
        ...
        expect(writeStderrLine).toHaveBeenCalledWith(expect.stringContaining('native directory picker unavailable'));
    });
    it('returns 500 when the picker fails unexpectedly', async () => {
        ...
        expect(writeStderrLine).toHaveBeenCalledWith(expect.stringContaining('native directory picker failed: boom'));
    });
    it('passes an abort signal to the picker', async () => {
        ...
        expect(pickWorkspaceDirectory).toHaveBeenCalledWith(expect.any(AbortSignal));
    });
    it('does not abort the picker before it can resolve, for the body the Web Shell actually sends', async () => {
        ...
        const pickWorkspaceDirectory = vi.fn((signal) => new Promise((resolve, reject) => {
            setTimeout(() => {
                if (signal?.aborted) {
    ...
    it('still aborts a picker that is genuinely in flight when the client hangs up', async () => {
        let observed;
        const { app } = createApp({
            pickWorkspaceDirectory: vi.fn((signal) => {
                observed = signal;
                return new Promise(() => { });
            }),
        });
...
//# sourceMappingURL=workspace-management.test.js.map