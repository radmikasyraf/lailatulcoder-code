...
import { DEFAULT_MAX_RETRIES, DEFAULT_DASHSCOPE_BASE_URL, DASHSCOPE_PROXY_BASE_URL, resolveRequestTimeout, } from '../constants.js';
 ...
 */
export function selectDashScopeThinkingKnob(model, extraBody, samplingParams, reasoningEffort) {
    if (!isTieredEffortWireModel((model ?? '').toLowerCase())) {
        return undefined;
    }
    const selectFromLayer = (source, layer) => {
    ...
    const selectValueFromLayer = (source, layer) => {
    ...
    const selectOnSwitch = (source, layer) => {
        if (layer?.['enable_thinking'] === true) {
            return { source, field: 'enable_thinking', value: true };
        }
        return undefined;
    };
    const reasoningSelection = reasoningEffort !== undefined
        ? {
            source: 'reasoning',
            field: 'reasoning_effort',
    ...
    if (extraBodySelection?.field === 'enable_thinking' &&
        ...
        return (selectValueFromLayer('samplingParams', samplingParams) ??
            reasoningSelection ??
            extraBodySelection);
    }
 ...
 */
export const DASHSCOPE_REGIONAL_HOSTS = [
...
export class DashScopeOpenAICompatibleProvider extends DefaultOpenAICompatibleProvider {
    ...
    static isDashScopeProvider(contentGeneratorConfig) {
        ...
        if (authType === AuthType.lailatulcoder_OAUTH)
            return true;
        if (!baseUrl)
            return true;
        ...
        const isDashscopeOrigin = hostname !== null &&
            DASHSCOPE_REGIONAL_HOSTS.some((host) => hostname === host || hostname.endsWith('.' + host));
        const isTokenPlanOrigin = hostname !== null &&
        ...
        const isInternalOrigin = hostname !== null &&
        ...
        if (normalizedProxyUrl &&
            ...
            !isAliCloudApiOrigin &&
            !isProxyMatch) {
            debugLogger.debug(`DASHSCOPE_PROXY_BASE_URL is configured but the request baseUrl does not match. DashScope headers/cache control will be skipped.`);
        }
        if (isInternalOrigin) {
            debugLogger.debug(`DashScope provider activated via internal origin: ${hostname}`);
        }
        if (isAliCloudApiOrigin) {
            debugLogger.debug(`DashScope provider activated via alicloudapi origin: ${hostname}`);
        }
        return (isDashscopeOrigin ||
            isTokenPlanOrigin ||
            isInternalOrigin ||
            isAliCloudApiOrigin ||
            isProxyMatch);
    ...
    buildClient() {
        const { apiKey, baseUrl = DEFAULT_DASHSCOPE_BASE_URL, maxRetries = DEFAULT_MAX_RETRIES, } = this.contentGeneratorConfig;
    ...
    buildRequest(request, userPromptId) {
        ...
        const flattenPlainTextForGlm = this.isGlmModel(request.model) &&
        ...
        const isTieredQwenModel = isTieredEffortWireModel(this.resolveWireModel(request.model));
        ...
        if ('reasoning_effort' in requestParams &&
        ...
        const selectedThinkingKnob = isTieredQwenModel
            ? selectDashScopeThinkingKnob(this.resolveWireModel(request.model), extraBody, requestParams, qwenEffortConfig['reasoning_effort'])
            : undefined;
        if (this.isVisionModel(request.model)) {
            ...
            return this.mergeExtraBodyAndResolveKnobs(visionResult, extraBody, request.model, selectedThinkingKnob);
        }
        ...
        return this.mergeExtraBodyAndResolveKnobs(result, extraBody, request.model, selectedThinkingKnob);
    }
    /**
    ...
    mergeExtraBodyAndResolveKnobs(result, extraBody, model, selectedThinkingKnob) {
        ...
        if (selectedThinkingKnob?.field === 'reasoning_effort' &&
        ...
        for (const key of this.dropConflictingThinkingKnobs(model, merged, selectedThinkingKnob)) {
            dropped.add(key);
        }
