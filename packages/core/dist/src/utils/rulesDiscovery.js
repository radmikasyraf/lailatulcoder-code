...
//
// Discovers .lailatulcoder/rules/ files (recursively) with optional YAML frontmatter.
...
/**
 * Discover and load rule files from a single `.lailatulcoder/rules/` directory.
...
export function formatRules(rules, projectRoot) {
    return rules
        ...
        // display paths should match — otherwise Windows shows `.lailatulcoder\rules\foo.md`
        // and Linux shows `.lailatulcoder/rules/foo.md`, which is confusing in diffs/tests.
...
export class ConditionalRulesRegistry {
     ...
     */
    async matchAndConsume(filePath) {
        if (this.compiledRules.length === 0)
            return undefined;
        // Resolve symlinks and check both original and realpath-resolved paths
        const relativePaths = await resolveSymlinkAwareRelativePaths(filePath, this.projectRoot);
...
// ─────────────────────────────────────────────────────────────────────────────
/**
 * Load rules from both global (`~/.lailatulcoder/rules/`) and project-level
 * (`.lailatulcoder/rules/`) directories.
 *
...
export async function loadRules(projectRoot, folderTrust, excludes = []) {
    ...
    const allRules = [];
    // 1. Global rules: <QWEN_HOME or ~/.lailatulcoder>/rules/
    ...
    // 2. Project-level rules: <projectRoot>/.lailatulcoder/rules/  (trusted only)
