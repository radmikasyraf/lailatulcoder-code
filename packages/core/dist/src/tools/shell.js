...
import { getShellAbortReasonKind, isSignalTermination, ShellExecutionService, } from '../services/shellExecutionService.js';
import { statusFilePathFor, } from '../services/backgroundShellRegistry.js';
import stripAnsi from 'strip-ansi';
import { formatMemoryUsage } from '../utils/formatters.js';
import { isSubpaths, makeRelative, shortenPath } from '../utils/paths.js';
import { buildShellExecWarnings, detectSelfKillCommand, getCommandRoot, getCommandRoots, getShellConfiguration, hasShellSubstitution, SHELL_SELF_KILL_REJECTION, splitCommands, stripShellWrapper, } from '../utils/shell-utils.js';
import { parse } from 'shell-quote';
import { createDebugLogger } from '../utils/debugLogger.js';
import { checkPriorRead, StructuredToolError } from './priorReadEnforcement.js';
import { isShellCommandReadOnlyASTInDirectory, extractCommandRules, } from '../utils/shellAstParser.js';
import { applySedSubstitution, parseSedEditCommand, } from '../utils/sedEditParser.js';
...
function getShellOutputThreshold(config) {
 ...
 */
function statusFileGuidance(outputPath) {
    return (`status file: ${statusFilePathFor(outputPath)}\n` +
 ...
 */
function stripTrailingBackgroundAmp(command) {
 ...
 */
function escapeForBashDoubleQuote(s) {
    return s.replace(/[\\"$`]/g, '\\$&');
}
/**
 ...
 */
function findUnquotedCommentStart(s) {
    let inSingle = false;
    let inDouble = false;
    let i = 0;
    while (i < s.length) {
...
function isMatchInside(inner, outer) {
 ...
 */
function pickOuterLastMatch(doubleMatch, singleMatch) {
 ...
 */
function tokeniseSegment(segment) {
    let tokens;
    try {
        ...
        // choice.
        tokens = parse(segment, (key) => '$' + key).filter((t) => typeof t === 'string');
    }
    catch (e) {
        debugLogger.warn(`tokeniseSegment: parse failed for "${segment.slice(0, 80)}": ${e instanceof Error ? e.message : String(e)}`);
        return null;
    }
    let i = 0;
    ...
    while (i < tokens.length) {
        ...
        if (key === null)
            break;
        if (GIT_ENV_SHIFTS_REPO.has(key))
            return null;
        i++;
    }
    ...
    if (tokens[i] === 'sudo' || tokens[i] === 'command' || tokens[i] === 'env') {
        ...
        while (i < tokens.length && tokens[i].startsWith('-')) {
            ...
            const takesValue = (wrapper === 'sudo' && SUDO_FLAGS_WITH_VALUE.has(flag)) ||
...
const PIPELINE_ALLOWED_OPERATORS = new Set([
    '|',
    '|&',
    '<<<',
    '>>',
...
function getExitStatusSegment(command) {
    const segments = splitCommands(command);
    if (segments.length === 1)
        return segments[0];
    try {
        const operators = parse(command, (key) => '$' + key).filter((token) => typeof token === 'object' && token !== null && 'op' in token);
...
function isShellExitError(command, exitCode) {
    if (exitCode === null || exitCode === 0)
        return false;
    if (exitCode >= 2)
        return true;
    const exitStatusSegment = getExitStatusSegment(command);
    if (!exitStatusSegment)
        return true;
 ...
 */
function isShiftCwdFlag(flag, set) {
    if (set.has(flag))
        return true;
    for (const f of set) {
        if (f.startsWith('--') && flag.startsWith(f + '='))
            return true;
        if (f.length === 2 &&
            f.startsWith('-') &&
            flag.startsWith(f) &&
...
// check here.
function isNoopCwdTarget(target) {
    const t = target.trim();
    return t === '.' || t === './';
}
 ...
 */
function gitCommitContext(command) {
    let hasCommit = false;
    ...
    for (const sub of splitCommands(command)) {
        ...
        if (program === 'cd' || program === 'pushd') {
            ...
            if (!hasCommit && cdTargetMayChangeRepo(tokens))
                cwdShifted = true;
            continue;
        }
        if (program === 'popd') {
...
function parseGhInvocation(tokens) {
    let i = 1; // skip 'gh'
    while (i < tokens.length) {
        const t = tokens[i];
 ...
 */
function cdTargetMayChangeRepo(tokens) {
...
function isAmendCommit(command) {
    ...
    for (const sub of splitCommands(command)) {
        ...
        const program = tokens[0];
        if (program === 'cd' || program === 'pushd') {
            if (!cwdShifted && cdTargetMayChangeRepo(tokens))
                cwdShifted = true;
            continue;
        }
        if (program === 'popd') {
        ...
        if (subcommand === 'commit' && !cwdShifted && !changesCwd) {
            return (tokens.includes('--amend') ||
                tokens.some((t) => t.startsWith('--amend=')));
        }
        if (changesCwd && !cwdShifted)
            cwdShifted = true;
    }
    return false;
}
 ...
 */
function findAttributableCommitSegment(command) {
    let cursor = 0;
    let cwdShifted = false;
    for (const sub of splitCommands(command)) {
        ...
        if (start < 0) {
            ...
            debugLogger.warn(`findAttributableCommitSegment: cannot map segment "${sub.slice(0, 60)}" ` +
 ...
 * attribution there.
 */
function findGhPrCreateSegment(command) {
    let cursor = 0;
    for (const sub of splitCommands(command)) {
        const start = command.indexOf(sub, cursor);
        if (start < 0) {
            debugLogger.warn(`findGhPrCreateSegment: cannot map segment "${sub.slice(0, 60)}" ` +
                `back to the original command (likely line-continuation / whitespace mismatch).`);
            continue;
        }
        const end = start + sub.length;
...
export function parseNumstat(numstatOutput) {
    ...
    for (const line of lines) {
        ...
        if (Number.isNaN(adds) || Number.isNaN(dels))
            continue;
 ...
 */
function endStreamThenSettle(stream, origin, shellId, settle) {
    let settled = false;
    let flushTimer = null;
    const runOnce = () => {
        if (settled)
    ...
    try {
        flushTimer = setTimeout(() => {
            debugLogger.warn(`${origin}: output stream flush timed out for ${shellId} after ${OUTPUT_FLUSH_TIMEOUT_MS}ms — transitioning registry without flush confirmation`);
            runOnce();
        ...
        debugLogger.warn(`${origin}: closing output stream on settle threw: ${getErrorMessage(closeErr)}`);
        runOnce();
    }
}
...
function longRunThresholdFor(effectiveTimeoutMs) {
    return Math.max(MIN_LONG_RUN_THRESHOLD_MS, Math.floor(effectiveTimeoutMs / 2));
}
const FOREGROUND_TIMEOUT_WARNING_LEAD_MS = 15_000;
const FOREGROUND_TIMEOUT_WARNING = 'Warning: this command is about to time out. In the interactive TUI, press Ctrl+B to keep it running in the background.';
function foregroundTimeoutWarningDelayFor(effectiveTimeoutMs, elapsedMs) {
...
function appendForegroundTimeoutWarning(output) {
 ...
 * work in every mode.
 */
export function buildLongRunningForegroundHint(elapsedMs) {
    const seconds = Math.round(elapsedMs / 1000);
    return (`Note: this foreground command ran for ${seconds}s. ` +
...
function detectBlockedSleepPatternDetails(command) {
    ...
    const { command: uncommentedCommand, comment } = splitTrailingShellComment(command);
    const trimmed = uncommentedCommand.trim();
    if (!trimmed.startsWith('sleep'))
        return null;
    const afterSleep = trimmed.slice('sleep'.length);
    if (!afterSleep || !/\s/.test(afterSleep[0]))
        return null;
    let index = 0;
    ...
    if (isStandalone &&
        ...
        if (reason === null) {
            return {
                description,
                isStandalone,
                intentionalSleepRejection: 'The intentional-sleep comment was recognized, but the reason is too short; explain why the delay is needed after `intentional-sleep:`.',
            };
        }
        if (secs > MAX_INTENTIONAL_SLEEP_SECONDS) {
            return {
                description,
                isStandalone,
                intentionalSleepRejection: 'The intentional-sleep comment was recognized, but foreground sleeps over 10 minutes are not allowed; use is_background: true or Monitor for longer waits.',
            };
        }
...
function getIntentionalSleepReason(trimmedComment) {
...
function parseSleepDurationToSeconds(token) {
    if (!token)
        return null;
    let index = 0;
    let seenDigit = false;
...
function getSleepSequentialSeparator(suffix) {
    let index = 0;
    ...
    if (!restWithSeparator)
        return { rest: '' };
    if (restWithSeparator.startsWith('&&') ||
...
function splitTrailingShellComment(command, keepCommandsAfterCommentNewline = true) {
    ...
    for (let i = 0; i < command.length; i++) {
        ...
        if (ch === '#' &&
            ...
            return {
                command: newlineIndex === -1 || !keepCommandsAfterCommentNewline
                    ? command.slice(0, i)
                    : command.slice(0, i) + command.slice(newlineIndex),
                comment: newlineIndex === -1
                    ? command.slice(i + 1)
...
function trimTrailingShellComment(command) {
    return splitTrailingShellComment(command, false).command;
}
function hasTopLevelTrailingBackgroundOperator(command) {
...
function getAbortReasonName(signal) {
...
export class ShellToolInvocation extends BaseToolInvocation {
    config;
    preparedSedEdit;
    confirmedSedNewContent;
    sedEditPreviewFailed = false;
    ...
    async checkSedPriorRead(filePath) {
        if (this.config.getFileReadCacheDisabled()) {
            return;
        }
        const priorReadResult = await checkPriorRead(this.config.getFileReadCache(), filePath, 'editing');
        if (!priorReadResult.ok) {
            throw new StructuredToolError(priorReadResult.rawMessage, priorReadResult.type);
        }
    }
    async prepareSedEdit(sedInfo) {
        const filePath = this.resolveSedFilePath(sedInfo.filePath);
        if (fs.lstatSync(filePath).isSymbolicLink()) {
            throw new Error(`sed edit target '${filePath}' is a symlink; falling back to shell execution`);
        }
    ...
    makeSedEditDisplay(edit) {
        const diffStat = getDiffStat(edit.fileName, edit.originalContent, edit.newContent, edit.newContent);
        return {
            fileDiff: createPatchSmart(edit.fileName, edit.originalContent, edit.newContent, 'Current', 'Proposed'),
    ...
    sedEditError(message, type = ToolErrorType.FILE_WRITE_FAILURE) {
        return {
            llmContent: message,
    ...
    sedEditCancelledResult(signal, effectiveTimeout) {
    ...
    waitForSedOperation(operation, signal) {
        if (signal.aborted) {
    ...
    async executeSedEdit(sedInfo, signal, effectiveTimeout) {
        let edit;
        try {
            edit = await this.waitForSedOperation(() => this.prepareSedEdit(sedInfo), signal);
        }
        catch (err) {
            const filePath = this.resolveSedFilePath(sedInfo.filePath);
            ...
            if (err instanceof SedEditSimulationError) {
                ...
                return this.sedEditError(message, ToolErrorType.EDIT_PREPARATION_FAILURE);
            }
            ...
            return this.sedEditError(message, isNodeError(err) && err.code === 'ENOENT'
                ? ToolErrorType.FILE_NOT_FOUND
                : ToolErrorType.READ_CONTENT_FAILURE);
        }
        if (this.preparedSedEdit?.filePath === edit.filePath &&
            this.preparedSedEdit.originalContent !== edit.originalContent) {
            return this.sedEditError(`File changed since sed edit confirmation: ${edit.filePath}. Please re-read the file and retry.`, ToolErrorType.FILE_CHANGED_SINCE_READ);
        }
        if (this.preparedSedEdit?.filePath === edit.filePath &&
        ...
        try {
            ...
            try {
                await this.waitForSedOperation(() => this.config.getFileHistoryService().trackEdit(edit.filePath), signal);
                ...
                debugLogger.warn(`file history trackEdit failed for sed edit ${edit.filePath}: ${getErrorMessage(err)}`);
            ...
            if (!userModifiedSedContent) {
                try {
                    CommitAttributionService.getInstance().recordEdit(edit.filePath, edit.originalContent, edit.newContent);
                }
                catch (err) {
                    debugLogger.warn(`commit attribution recordEdit failed for sed edit ${edit.filePath}: ${getErrorMessage(err)}`);
            ...
            try {
                ...
                debugLogger.warn(`file read cache recordWrite failed for sed edit ${edit.filePath}: ${getErrorMessage(err)}`);
            ...
            if (fileHistoryBackupRecorded) {
                debugLogger.warn(`sed edit write failed after file history backup was recorded for ${edit.filePath}: ${getErrorMessage(err)}`);
            }
    ...
    async getDefaultPermission() {
        ...
        try {
            const isReadOnly = await isShellCommandReadOnlyASTInDirectory(command, this.params.directory || this.config.getTargetDir());
            if (isReadOnly) {
                return 'allow';
            }
        }
    ...
    async getConfirmationDetails(_abortSignal) {
        ...
        if (sedInfo) {
            try {
                ...
                debugLogger.warn(`sed edit preview failed, falling back to exec confirmation: ${getErrorMessage(err)}`);
            }
        }
    ...
    async execute(signal, updateOutput, shellExecutionConfig, setPidCallback, setPromoteAbortControllerCallback, canPromoteForegroundShell) {
        const strippedCommand = stripShellWrapper(this.params.command);
        if (signal.aborted) {
            return {
        ...
        const effectiveTimeout = this.params.timeout ??
        ...
        let timeoutSignalStartedAt = null;
        if (effectiveTimeout) {
        ...
        if (sedInfo && !this.sedEditPreviewFailed) {
            ...
            debugLogger.debug('falling back to shell execution for sed edit without prepared preview', { command: this.params.command });
        }
        // Add co-author to git commit commands and LailatulCoder Ai attribution to
        ...
        // inside a wrapper.
        const processedCommand = this.addAttributionToPR(this.addCoAuthorToGitCommit(this.params.command.trim()));
        ...
        const preHead = commitCtx.attributableInCwd
        ...
        const armTimeoutWarning = () => {
            ...
            const warningDelay = foregroundTimeoutWarningDelayFor(effectiveTimeout, Date.now() - timeoutSignalStartedAt);
        ...
        const onShellOutputEvent = (event) => {
            ...
            switch (event.type) {
                case 'data':
                    ...
                    if (Array.isArray(event.chunk)) {
                        ...
                        totalBytes = event.chunk.reduce((sum, line) => sum +
                            line.reduce((ls, token) => ls + Buffer.byteLength(token.text, 'utf-8'), 0), 0);
                    }
                    ...
                    if (Array.isArray(event.chunk)) {
                        ...
                        const remaining = OUTPUT_UPDATE_INTERVAL_MS - (Date.now() - lastUpdateTime);
        ...
        const postPromote = {
            onData: (event) => {
                ...
                // and inconsistent.
                const rawChunk = typeof event.chunk === 'string'
                    ? event.chunk
                    : event.chunk
                ...
                if (promoteArtifacts.stream) {
                    try {
                        ...
                        debugLogger.warn(`promote: postPromote stream.write failed: ${getErrorMessage(err)}`);
                    }
                }
                    ...
                    debugLogger.debug('promote: dropping post-promote chunk because output stream open failed');
                }
                else {
        ...
        let executionHandle;
        try {
            executionHandle = await ShellExecutionService.execute(commandToExecute, cwd, onShellOutputEvent, combinedSignal, this.config.getShouldUseNodePtyShell(), shellExecutionConfig ?? {}, { postPromote });
        }
        catch (err) {
        ...
        // doesn't exist yet.
        const heartbeatIntervalMs = this.config.getShellHeartbeatIntervalMs() ??
        ...
        if (updateOutput && heartbeatIntervalMs > 0 && !combinedSignal.aborted) {
            heartbeatTimer = setInterval(() => {
                ...
                if (now - idleSince < heartbeatIntervalMs)
                    return;
                updateOutput({
                    type: 'shell_progress',
        ...
        if (result.promoted) {
            const promotedToolResult = await this.handlePromotedForeground(result, cwd, commandToExecute, promoteAbortController, promoteArtifacts);
            return promotedToolResult;
        }
        const abortReasonName = getAbortReasonName(combinedSignal);
        const wasTimeout = result.aborted &&
            effectiveTimeout > 0 &&
            abortReasonName === 'TimeoutError';
        const wasPromoteRefused = result.aborted &&
        ...
        if (commitCtx.attributableInCwd) {
            ...
            attributionWarning = await this.attachCommitAttribution(cwd, preHead, isAmend);
        }
        ...
        const longRunThreshold = effectiveTimeout === 0 ? null : longRunThresholdFor(effectiveTimeout);
        const shouldAppendLongRunHint = longRunThreshold !== null &&
            !result.aborted &&
        ...
        debugLogger.debug(`long-run hint: elapsed=${Math.round(elapsedMs)}ms threshold=${longRunThreshold === null ? 'disabled' : `${longRunThreshold}ms`} ` +
        ...
        if (typeof llmContent === 'string') {
            ...
            const truncatedResult = await truncateToolOutput(this.config, ShellTool.Name, llmContent, 
                ...
                : (!result.aborted && isSignalTermination(result.signal)) ||
                    ...
                    ? {
                        error: {
                            ...
                            message: typeof llmContent === 'string'
                                ? llmContent
                                : returnDisplayMessage,
                            type: ToolErrorType.SHELL_EXECUTE_ERROR,
                        },
     ...
     */
    async handlePromotedForeground(result, cwd, commandToExecute, abortController, promoteArtifacts) {
        ...
        const outputDir = path.join(this.config.storage.getProjectTempDir(), 'background-shells', this.config.getSessionId());
        ...
        if (mkdirError) {
            debugLogger.warn(`promote: mkdirSync(${outputDir}) failed before registry register — killing orphan child: ${mkdirError.message}`);
            const pid = result.pid;
            if (pid !== undefined) {
                if (os.platform() === 'win32') {
                    try {
                        const taskkillChild = childProcess.spawn('taskkill', ['/pid', String(pid), '/f', '/t'], 
        ...
        try {
            ...
            outputStream.on('error', (err) => {
                debugLogger.warn(`promote: output write stream error for ${outputPath}: ${getErrorMessage(err)}`);
            ...
            debugLogger.warn(`promote: failed to open output stream for ${outputPath}: ${getErrorMessage(err)}`);
            ...
            if (promoteArtifacts.buffer.length > 0) {
                debugLogger.warn(`promote: dropping ${promoteArtifacts.buffer.length} buffered post-promote chunks for ${outputPath} (stream open failed before drain)`);
            ...
            try {
                ...
                debugLogger.warn(`promote: snapshot fallback writeFileSync also failed for ${outputPath}: ${getErrorMessage(err2)}`);
            }
        }
                ...
                if (os.platform() === 'win32') {
                    try {
                        ...
                        taskkillChild.on('error', (err) => {
                            debugLogger.warn(`promote: taskkill spawn failed for pid=${pid}: ${err.message}`);
                        });
                    }
                    catch (e) {
                        // childProcess.spawn itself throwing (sync) is rare but possible
                        // (e.g. EMFILE — too many open files) — same recovery.
                        debugLogger.warn(`promote: childProcess.spawn('taskkill') threw for pid=${pid}: ${getErrorMessage(e)}`);
                    }
                }
                else {
                    try {
                        ...
                        process.kill(-pid, 'SIGTERM');
                        await new Promise((res) => setTimeout(res, PROMOTE_CANCEL_SIGKILL_TIMEOUT_MS));
                        try {
                        ...
                        debugLogger.warn(`promote: process.kill on -${pid} threw: ${getErrorMessage(e)}`);
                    }
                }
            }
        ...
        try {
            ...
            debugLogger.warn(`promote: registry.register threw for ${shellId} (pid=${result.pid}) — killing orphan child: ${e instanceof Error ? e.message : String(e)}`);
            try {
                entryAc.abort();
            }
            catch {
        ...
        let postPromoteFinalStatus = null;
        const classifySettle = (info) => {
            ...
            // message.
            if (info.error)
                return { status: 'failed', failMsg: info.error.message };
            ...
            if (info.exitCode === 0)
                return { status: 'completed', failMsg: null };
            ...
            debugLogger.warn(`promote: classifySettle all-null fallback hit for ${shellId} — ` +
        ...
        const transitionRegistry = (info) => {
            ...
            if (cls.status === 'completed') {
                ...
                registry.fail(shellId, cls.failMsg, info.endTime);
            }
        };
        promoteArtifacts.onSettleWired = (info) => {
            ...
            if (stream) {
                while (promoteArtifacts.buffer.length > 0) {
                    try {
                        ...
                        debugLogger.warn(`promote: pre-end buffer drain write failed: ${getErrorMessage(writeErr)}`);
                    }
                }
            }
            promoteArtifacts.stream = null;
            ...
            endStreamThenSettle(stream, 'promote', shellId, () => transitionRegistry(info));
        };
        ...
        debugLogger.debug(`promote: registered ${shellId} (pid=${result.pid}) — outputPath=${outputPath}`);
        return {
            llmContent,
     ...
     */
    async executeBackground(signal, shellExecutionConfig) {
        const strippedCommand = stripShellWrapper(this.params.command);
        ...
        if (gitCommitContext(strippedCommand).hasCommit) {
            return {
                llmContent: 'Refusing to run `git commit` in background mode: AI-attribution notes ' +
                ...
                returnDisplay: 'Refused: `git commit` is not supported in background shell mode.',
            };
        }
        ...
        if (noTrailingAmp !== trimmedOriginal) {
            debugLogger.warn('Stripped trailing & from background shell command — managed path handles backgrounding');
        }
        const processedCommand = this.addAttributionToPR(this.addCoAuthorToGitCommit(noTrailingAmp));
        const cwd = this.params.directory || this.config.getTargetDir();
        ...
        outputStream.on('error', (err) => {
            debugLogger.warn(`background shell ${shellId} output write error: ${err.message}`);
        });
        const startTime = Date.now();
        const registration = {
            shellId,
        ...
        const { result: resultPromise, pid } = await ShellExecutionService.execute(processedCommand, cwd, (event) => {
        ...
        try {
            ...
            debugLogger.warn(`background shell ${shellId} register threw (pid=${pid}) — aborting orphan child: ${e instanceof Error ? e.message : String(e)}`);
            try {
                entryAc.abort();
            }
            catch {
        ...
        void resultPromise.then((result) => {
            ...
            const endTime = Date.now();
            endStreamThenSettle(outputStream, 'background', shellId, () => registry.fail(shellId, getErrorMessage(err), endTime));
        });
     ...
     */
    async countCommitsAfter(cwd, preHead, postHead) {
     ...
     */
    async countCommitsFromRoot(cwd, postHead) {
        return this.runGitCount(cwd, ['rev-list', '--count', postHead]);
    }
    /** Shared helper for the two `rev-list --count` invocations. */
    async runGitCount(cwd, args) {
        return new Promise((resolve) => {
            const child = childProcess.execFile('git', args, { cwd, timeout: 2000, windowsHide: true }, (error, stdout) => {
                if (error) {
                    resolve(0);
                    return;
                }
     ...
     */
    async getGitHead(cwd) {
        return new Promise((resolve) => {
            const child = childProcess.execFile('git', ['rev-parse', 'HEAD'], { cwd, timeout: 2000, windowsHide: true }, (error, stdout) => {
                if (error) {
                    resolve(null);
                    return;
                }
     ...
     */
    async attachCommitAttribution(cwd, preHead, isAmend) {
        ...
        const commitCount = preHead !== null
        ...
        let committedAbsolutePaths = null;
        ...
        try {
            ...
            const stagedInfo = await this.getCommittedFileInfo(cwd, isAmend, postHead, preHead);
            ...
            attributionService.applyCommittedRenames(stagedInfo.renamedFiles, canonicalBase);
            ...
            const committedScope = attributionService.matchCommittedFiles(stagedInfo.files, canonicalBase);
            ...
            committedAbsolutePaths = attributionService.matchCommittedFiles(stagedInfo.files, canonicalBase);
            ...
            const note = attributionService.generateNotePayload(stagedInfo, baseDir, this.config.getModel());
            ...
            if (!notesCommand) {
                debugLogger.warn('AI attribution note too large, skipping git notes attachment');
                warning =
            ...
            const { exitCode, output, timedOut } = await new Promise((resolve) => {
                const child = childProcess.execFile(notesCommand.command, notesCommand.args, { cwd, timeout: 5000, windowsHide: true }, (error, stdout, stderr) => {
                    ...
                    if (error) {
                        ...
                        const errno = error;
                        const isTimeout = errno.code === 'ETIMEDOUT' ||
                            (errno.killed === true && errno.signal === 'SIGTERM');
                        const code = typeof errno.code === 'number'
                            ? errno.code
                            : null;
                        resolve({
                            exitCode: code ?? 1,
            ...
            if (exitCode !== 0) {
                ...
                debugLogger.debug(`Attached AI attribution note: ${note.summary.aiPercent}% AI, ${note.summary.totalFilesTouched} file(s)`);
            ...
            debugLogger.warn(`Failed to attach AI attribution note: ${getErrorMessage(err)}`);
            warning =
     ...
     */
    async getCommittedFileInfo(cwd, isAmend, postHead, preHead) {
        const empty = {
            files: [],
            diffSizes: new Map(),
        ...
        const runGit = async (args) => {
            const handle = await ShellExecutionService.execute(`git ${args}`, cwd, () => { }, AbortSignal.timeout(5000), false, {});
        ...
        try {
            ...
            // correctly.
            const [hasParentOutput, parentShaOutput, repoRootOutput] = await Promise.all([
            ...
            if (parentShaOutput === null) {
                debugLogger.warn('getCommittedFileInfo: log -1 --pretty=%P <postHead> failed; ' +
            ...
            if (!hasParent && !isTrueRootCommit) {
                debugLogger.warn('getCommittedFileInfo: <postHead>~1 unreadable but commit is not ' +
            ...
            if (isAmend) {
                ...
                if (preHead === null) {
                    debugLogger.warn('getCommittedFileInfo: --amend with no preHead; skipping ' +
                ...
                if (preHeadProbe === null || preHeadProbe.length === 0) {
                    debugLogger.warn('getCommittedFileInfo: --amend preHead unresolvable; skipping ' +
                        'attribution note (cannot determine amend delta).');
                    return null;
                }
                diffArgs = {
            ...
            if (nameOutput === null ||
                statusOutput === null ||
                numstatOutput === null) {
                debugLogger.warn('getCommittedFileInfo: one or more diff calls failed; ' +
            ...
            if (diffSizes.size === 0) {
                debugLogger.warn('getCommittedFileInfo: --numstat returned empty while ' +
    ...
    addCoAuthorToGitCommit(command) {
        ...
        const doubleQuotePattern = new RegExp(`(${FLAG_PREFIX})"((?:[^"\\\\]|\\\\.)*)"`, 'g');
        ...
        // `addAttributionToPR`.
        const singleQuotePattern = new RegExp(`(${FLAG_PREFIX})'((?:[^']|'\\\\'')*)'`, 'g');
        ...
        const segment = commentStart >= 0 ? fullSegment.slice(0, commentStart) : fullSegment;
        ...
        if (match) {
            ...
            if (matchStart >= segmentRange.start) {
                return (command.slice(0, matchStart) +
                    replacement +
     ...
     * PR body. Format: "Generated with LailatulCoder Ai (N-shotted by lailatul-coderr)"
     * when at least one user prompt has been recorded since the last commit;
     * otherwise just "Generated with LailatulCoder Ai".
     *
    ...
    addAttributionToPR(command) {
        ...
        const generator = gitCoAuthorSettings.name ?? 'lailatul-coderr';
        const attribution = shots > 0
            ? `\n\nGenerated with LailatulCoder Ai (${shots}-shotted by ${generator})`
            : `\n\nGenerated with LailatulCoder Ai`;
        ...
        const bodyDoublePattern = new RegExp(`(${BODY_FLAG})"((?:[^"\\\\]|\\\\.)*)"`, 'g');
        ...
        const bodySinglePattern = new RegExp(`(${BODY_FLAG})'((?:[^']|'\\\\'')*)'`, 'g');
        ...
        debugLogger.warn('addAttributionToPR: gh pr create detected but no inline ' +
...
function getExecutableBasename(executable) {
    return path.basename(path.win32.basename(executable));
}
function getShellDisplayName({ executable, shell, }) {
    switch (shell) {
        case 'cmd':
            return 'cmd.exe';
        case 'powershell': {
...
function getShellCommandSequencingGuidance({ executable, shell, }) {
    const independentGuidance = '- If the commands are independent and can run in parallel, make multiple run_shell_command tool calls in a single message. For example, if you need to run "git status" and "git diff", send a single message with two run_shell_command tool calls in parallel.';
    switch (shell) {
        case 'bash':
  ...
  - DO NOT use ';' or newlines to separate commands in cmd.exe.`;
        case 'powershell': {
            const executableBasename = getExecutableBasename(executable).toLowerCase();
...
function getCommandDescription() {
    ...
    switch (shellConfiguration.shell) {
        ...
        default: {
            const _exhaustive = shellConfiguration.shell;
            return _exhaustive;
        }
    }
}
export class ShellTool extends BaseDeclarativeTool {
    config;
    static Name = ToolNames.SHELL;
    get maxOutputChars() {
        return getShellOutputThreshold(this.config);
    }
    constructor(config) {
        super(ShellTool.Name, ToolDisplayNames.SHELL, getShellToolDescription(), Kind.Execute, {
            type: 'object',
            properties: {
                command: {
                    type: 'string',
    ...
    validateToolParamValues(params) {
        ...
        if (!params.is_background) {
            ...
            if (sleepPattern !== null) {
                const intentionalSleepGuidance = sleepPattern.intentionalSleepRejection ??
                ...
                return (`Blocked: ${sleepPattern.description}. ` +
