...
export const INSTALL_METADATA_FILENAME = '.lailatulcoder-extension-install.json';
 ...
 */
export function substituteHookVariables(hooks, basePath) {
    if (!hooks)
        return hooks;
    ...
    for (const eventName in clonedHooks) {
        const eventHooks = clonedHooks[eventName];
 ...
 */
export function performVariableReplacement(extensionPath, pluginRoot = extensionPath) {
    ...
    try {
        ...
        for (const file of mdFiles) {
            ...
            try {
                ...
                const updatedContent = content.replace(/\$\{CLAUDE_PLUGIN_ROOT\}/g, () => pluginRoot);
                ...
                // Replace references to ".claude" directory with ".lailatulcoder" in markdown files
                ...
                const updatedMdContent = syntaxUpdatedContent.replace(/(\$\{?HOME\}?\/|~\/)?\.claude(\/|$)/g, '$1.lailatulcoder$2');
                ...
                debugLogger.warn(`Failed to process file ${filePath}: ${error instanceof Error ? error.message : String(error)}`);
            }
        }
    }
    catch (error) {
        debugLogger.warn(`Failed to scan markdown files in extension directory ${extensionPath}: ${error instanceof Error ? error.message : String(error)}`);
    }
    ...
    try {
        ...
        for (const file of scriptFiles) {
            ...
            try {
                ...
                const adaptedScriptContent = updatedScriptContent.replace(/\.message\.content\s*\|\s*map\(select\(\.type\s*==\s*"text"\)\)/g, '.message.parts | map(select(has("text")))');
                // Replace references to ".claude" directory with ".lailatulcoder" in shell scripts
                // Only match path references (e.g., ~/.claude/, $HOME/.claude, ./.claude/)
                // Avoid matching URLs, comments, or string literals containing .claude
                const finalScriptContent = adaptedScriptContent.replace(/(\$\{?HOME\}?\/|~\/)?\.claude(\/|$)/g, '$1.lailatulcoder$2');
                // Only write if content was actually changed
                if (finalScriptContent !== content) {
                    fs.writeFileSync(filePath, finalScriptContent, 'utf8');
                    debugLogger.debug(`Updated transcript format and replaced .claude with .lailatulcoder in shell script: ${filePath}`);
                }
            }
            catch (error) {
                debugLogger.warn(`Failed to process shell script file ${filePath}: ${error instanceof Error ? error.message : String(error)}`);
            }
        }
    }
    catch (error) {
        debugLogger.warn(`Failed to scan shell script files in extension directory ${extensionPath}: ${error instanceof Error ? error.message : String(error)}`);
    }
}
