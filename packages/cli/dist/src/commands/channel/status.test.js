...
const mockDaemonClient = vi.hoisted(() => vi.fn(() => ({ getChannelWorkerControl: mockGetChannelWorkerControl })));
...
async function invokeStatus(argv = {}) {
    const handler = statusCommand.handler;
    if (!handler)
        throw new Error('status handler missing');
    await handler({ _: [], $0: 'qwen', ...argv });
}
beforeEach(() => {
    vi.clearAllMocks();
});
...
describe('statusCommand', () => {
    ...
    it('reads remote daemon manager state without consulting the pidfile', async () => {
        ...
        expect(mockWriteStdoutLine).toHaveBeenCalledWith(expect.stringContaining('enabled'));
        expect(mockWriteStdoutLine).toHaveBeenCalledWith('[Channel] Startup failure (workspace=/work, channel=feishu, phase=connect): connection refused');
    });
    it('reports remote status failures on stderr', async () => {
        ...
        expect(mockWriteStderrLine).toHaveBeenCalledWith(expect.stringContaining('offline'));
        expect(mockReadServiceInfo).not.toHaveBeenCalled();
