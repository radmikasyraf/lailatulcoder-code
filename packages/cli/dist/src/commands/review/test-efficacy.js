...
import { mkdirSync, writeFileSync, readFileSync, rmSync, lstatSync, existsSync, realpathSync, } from 'node:fs';
...
import { discardWorktree, exposeDependencies, redirectedAncestor, sanitizedGitEnv, worktreeCreateFailureDetail, } from './lib/worktree.js';
 ...
 */
const FIXTURE_DIR_RE = /(^|\/)(__fixtures__|__mocks__|__snapshots__|fixtures)\//;
/**
 ...
 */
export function planTestEfficacy(files, workspaceGlobs) {
    ...
    const revert = files
        ...
        .map((f) => f.path);
    const unreachable = tests.filter((t) => !isWorkspaceMember(t, workspaceGlobs));
 ...
 * is new ground.
 */
export function committedSymlinkProbes(tree, probes) {
    if (probes.length === 0)
        return new Set();
 ...
 */
const SAFETY_VERB_RE = /\.(?:clear|delete|reset|abort|removeListener|unref)\(|=\s*\[\]\s*;$|=\s*new\s+(?:Map|Set|WeakMap|WeakSet)(?:<[^=;]*>)?\(\)\s*;$/;
/**
 ...
 */
const NON_STATEMENT_START_RE = /^(?:const|let|var|function|class|interface|type|enum|import|export|return|throw|yield|if|for|while|switch|do|else|try|catch|finally|case|default|break|continue|async|public|private|protected|readonly|static)\b/;
/**
 ...
 */
function skipTemplateOnLine(line, start) {
    let depth = 0;
 ...
 */
function scanLineDelimiters(line) {
    let paren = 0;
    let bracket = 0;
    let brace = 0;
 ...
 */
function scanFileLines(content) {
    const inLiteral = [];
    const codeLines = [];
    let state = 'code';
 ...
 */
function insideClassBody(codeLines, idx) {
    let depth = 0;
    for (let j = idx - 1; j >= 0; j--) {
        ...
        for (let i = code.length - 1; i >= 0; i--) {
            ...
            if (ch === '}')
                ...
                if (depth === 0) {
                    if (/\bclass\b/.test(code.slice(0, i)))
                        return true;
                    for (let k = j - 1; k >= 0; k--) {
 ...
 */
function isRemovableStatement(lines, codeLines, idx) {
    ...
    if (!(codeLines[idx] ?? '').endsWith(';'))
        return false;
    if ((codeLines[idx] ?? '').slice(0, -1).includes(';'))
        return false;
    if (!/^(?:await\s+)?[A-Za-z_$]/.test(t))
        return false;
    if (NON_STATEMENT_START_RE.test(t))
        return false;
    if (insideClassBody(codeLines, idx))
        return false;
 ...
 */
export function replacementMutantsOf(raw, codeLine) {
    if (raw.trim() !== codeLine)
        return null;
    const lead = /^\s*/.exec(raw)[0];
    const done = (operator, edited) => ({ operator, mutated: lead + edited });
    // `a ?? b` with a SIMPLE fallback — an identifier/member/call chain, no
    // operators — so the drop cannot truncate a larger expression.
    const coalesce = /\s\?\?\s+[\w$.]+(?:\((?:[^()]|\([^()]*\))*\))?(?=\s*[;,)\]}]|\s*$)/.exec(codeLine);
    if (coalesce) {
        return done('coalesce', codeLine.slice(0, coalesce.index) +
    ...
    if (term) {
        return done('term-drop', codeLine.slice(0, term.index) +
...
export function selectMutants(files, cap = MAX_MUTANTS) {
    const preferred = [];
    const rest = [];
    const replPreferred = [];
    const replRest = [];
 ...
 */
export function collocatedProbe(file, testPaths) {
 ...
 */
export function heldForRedCollocatedTest(kind, file, probes, greenProbes, baselinePerFile) {
...
const REASON_PHRASE = {
    ...
    'control-failed': 'it read green there, but the positive control failed, so nothing could have turned that run red',
    ...
    'runner-died': 'the probe suite was started there and did not survive (killed at the deadline, or by a signal)',
    ...
    'no-output': 'the runner produced no parseable output there, so nothing at all is known about it',
    ...
    'not-in-results': 'the run produced results there but none for it — which a compile or import error looks like, and so does a path that did not match',
    'no-tests': 'it collected no tests there — the shape a compile or import error in the probe tree takes',
 ...
 */
export function collocatedNotGreenDetail(kind, probe, baselinePerFile) {
 ...
 */
export function hasCollocatedNewTest(file, testPaths) {
 ...
 * revert probe holds.
 */
export function classifyMutantRun(perFile) {
    if (perFile.some((r) => r.verdict === 'gated'))
        return 'killed';
 ...
 */
export function fitsAnotherMutantRun(remainingMs, estimatedRunMs) {
 ...
 *   started.
 */
export function classifyProbeRun(exitCode, stdout, probes, stderr = '') {
    let parsed;
 ...
 * non-hoisted layouts.
 */
export function findVitestBin(worktree, resolveModule = createRequire(join(worktree, 'noop.js')).resolve) {
    let pkgPath;
    try {
 ...
 */
function existsAtRev(cwd, rev, path) {
 ...
 */
export function safeRmWithin(worktree, relPath) {
    ...
    if (parts.includes('..')) {
        throw new Error(`refusing to delete through a parent reference: ${relPath}`);
    }
    ...
    try {
        if (lstatSync(worktree).isSymbolicLink()) {
            throw new Error(`refusing to delete through a symlink: ${relPath} ` +
...
const existsAtBase = (cwd, base, path) => existsAtRev(cwd, base, path);
/**
 ...
 */
export function probeCleanupFailureDetail(probeTree, removeError, sweepStderr) {
...
function restoreProbeTreeTracked(probeTree) {
    ...
    try {
        ...
        if (lstatSync(join(probeTree, '.git')).isFile()) {
            ...
            if (realpathSync(dirname(resolve(gitDir, backpointer))) !==
            ...
            const redirected = redirectedAncestor(dirname(resolve(probeTree)), dirname(realpathSync(commonDir)));
            if (redirected !== null) {
    ...
    const bits = spawnSync('git', ['-c', 'core.fsmonitor=', 'ls-files', '-v', '-z'], {
        cwd: probeTree,
        encoding: 'utf8',
        maxBuffer: 64 * 1024 * 1024,
        env: sanitizedGitEnv(),
 ...
 */
function runProbeSuite(probeTree, probes, deadlineAt, now = Date.now, dependencyRoot = probeTree) {
    const started = now();
    const timeout = deadlineAt !== undefined
    ...
    const r = spawnSync(process.execPath, [findVitestBin(dependencyRoot), 'run', '--reporter=json', ...probes], {
        cwd: probeTree,
        encoding: 'utf8',
        timeout,
    ...
    if (r.signal) {
        throw new ProbeRunFailure(`runner killed by ${r.signal}${r.signal === 'SIGTERM' ? ` (probe timed out after ${Math.round(timeout / 1000)}s)` : ''}`, runnerFailureReason(r));
    }
    return {
        perFile: classifyProbeRun(r.status ?? 1, `${r.stdout ?? ''}`, probes, `${r.stderr ?? ''}`),
        ms: now() - started,
        exposed,
    };
}
 ...
 */
export function splitDiffIntoHunks(diffText) {
    ...
    while (i < lines.length) {
        ...
        let startLine = Number(/^@@ -\d+(?:,\d+)? \+(\d+)/.exec(header)?.[1] ?? '0');
        let offset = 0;
        for (let k = i + 1; k < j; k++) {
 ...
 */
export function selectHunkProbes(files, cap = MAX_HUNK_PROBES) {
    const preferred = [];
    const rest = [];
    for (const f of files) {
        ...
        hunks.forEach((h, index) => {
            ...
            const hunkStart = Number(/^@@ -\d+(?:,\d+)? \+(\d+)/.exec(h.header)?.[1] ?? '0');
            const end = hunkStart + newSideLength(h.header);
            if (f.mutantLines.some((n) => n >= hunkStart && n < end))
                return;
            (f.hasNewTests ? preferred : rest).push({
                file: f.file,
                index,
 ...
 */
function probeTargetEscapes(probeTree, file) {
 ...
 * to remove.
 */
export function runOneHunkProbe(probeTree, hunk, probes, deadlineAt, now = Date.now, dependencyRoot = probeTree) {
    ...
    if (probeTargetEscapes(probeTree, hunk.file)) {
        return {
            ...meta,
            verdict: 'inconclusive',
            detail: 'the probe target resolves through a symlink — the reverse patch and the restore would follow it out of the probe tree, so nothing was neutralised',
        };
    }
    ...
    const attempt = () => {
        try {
            const { perFile } = runProbeSuite(probeTree, probes, deadlineAt, now, dependencyRoot);
            const verdict = classifyMutantRun(perFile);
            const detail = verdict === 'killed'
    ...
    if (relinkedMidRun) {
        throw new Error(`refusing to restore ${hunk.file} through a symlink — the probe tree was relinked mid-run`);
    }
    return result;
}
/**
 ...
 */
export function runControlMutant(probeTree, probeFile, deadlineAt, now = Date.now, dependencyRoot = probeTree) {
    ...
    if (restoreProbeTreeTracked(probeTree) !== null)
        return null;
    let original;
    try {
        original = readFileSync(abs, 'utf8');
    ...
    const attempt = () => {
        try {
            ...
            if (probeTargetEscapes(probeTree, probeFile))
                return null;
            writeFileSync(abs, `${original}\n;import { it as __qcIt, expect as __qcExpect } from 'vitest';\n__qcIt('LailatulCoder-REVIEW-POSITIVE-CONTROL', () => {\n  __qcExpect(1).toBe(2);\n});\n`, 'utf8');
            const { perFile } = runProbeSuite(probeTree, [probeFile], deadlineAt, now, dependencyRoot);
    ...
    if (relinkedMidRun) {
        throw new Error(`refusing to restore ${probeFile} through a symlink — the probe tree was relinked mid-run`);
    }
    return demonstrated;
}
export function runOneMutant(probeTree, mutant, probes, deadlineAt, now = Date.now, dependencyRoot = probeTree) {
    const abs = join(probeTree, mutant.file);
    if (probeTargetEscapes(probeTree, mutant.file)) {
        return {
            ...mutant,
            verdict: 'inconclusive',
            detail: 'the probe target resolves through a symlink — the mutation and the restore would follow it out of the probe tree, so nothing was mutated',
        };
    }
    const stale = restoreProbeTreeTracked(probeTree);
    if (stale !== null) {
    ...
    if ((lines[mutant.line - 1] ?? '').trim() !== mutant.statement) {
        ...
        return {
            ...mutant,
            verdict: 'inconclusive',
            detail: 'the probe tree does not match the selected statement at this line — nothing was mutated',
        };
    }
    // A replacement operator edits the line; the legacy shape deletes it.
    const what = mutant.operator === 'coalesce'
        ...
        try {
            ...
            if (probeTargetEscapes(probeTree, mutant.file)) {
                return {
                    ...mutant,
                    verdict: 'inconclusive',
                    detail: 'the probe target was relinked through a symlink before the mutation was written — nothing was mutated',
                };
            }
            ...
            const detail = verdict === 'killed'
                ...
                : verdict === 'survived'
                    ? `every affected test still PASSED with this statement ${what} — no test fails ${mutant.mutated === undefined
                        ? 'when it is removed'
                        : 'when it changes'}`
    ...
    const result = attempt();
    if (relinkedMidRun) {
        throw new Error(`refusing to restore ${mutant.file} through a symlink — the probe tree was relinked mid-run`);
    }
    return result;
}
/**
 ...
 * other files.
 */
function hunkProbeInputs(worktree, base, headSha, sources, probes, mutants) {
    return sources.map((file) => {
        let diff = '';
        try {
            diff = gitCapture(worktree, '-c', 'core.quotePath=false', 'diff', '--no-color', '--src-prefix=a/', '--dst-prefix=b/', '--no-ext-diff', '--no-textconv', base, headSha, '--', file);
        }
        catch {
            diff = '';
        }
...
async function runTestEfficacy(args) {
    ...
    const rootPkg = JSON.parse(readFileSync(`${worktree}/package.json`, 'utf8'));
    ...
    for (const p of revert) {
        ...
        if (isAbsolute(p) || (norm !== root && !norm.startsWith(root + sep))) {
            throw new Error(`refusing to run: revert path escapes the worktree: ${JSON.stringify(p)}`);
        }
    }
    ...
    const noteDependencyFarm = (exposed) => {
        if (exposed.failed > 0) {
            noteMutants(`dependency farm incomplete: exposed ${exposed.linked}/${exposed.linked + exposed.failed} dependencies into the probe tree (${exposed.failed} failed to link) — probe results may not be evidence`);
        }
    };
    if (probes.length > 0 && revert.length > 0) {
        ...
        // is nested under the repo (`.lailatulcoder/tmp/…-probe`), so Node walks up to the
        ...
        let candidates = [];
        try {
            const mutantFiles = revert.filter((p) => MUTANT_SOURCE_RE.test(p) && !DECLARATION_FILE_RE.test(p));
            if (mutantFiles.length > 0) {
                const added = parseAddedLines(gitCapture(worktree, '-c', 'core.quotePath=false', 'diff', '--unified=0', '--no-color', '--src-prefix=a/', '--dst-prefix=b/', '--no-ext-diff', '--no-textconv', base, headSha, '--', ...mutantFiles));
                const selection = selectMutants(mutantFiles
                ...
                if (selection.derailed.length > 0) {
                    noteMutants(`mutant selection dropped ${selection.derailed.length} file(s) whose literal scan derailed (${selection.derailed.join(', ')}) — a regex literal holding a quote or backtick can do this; their candidates were not probed`);
                }
            }
        }
        catch (e) {
            ...
            noteMutants(`mutant selection failed: ${e instanceof Error ? e.message : String(e)} — no mutants were run`);
            candidates = [];
        }
        ...
        let hunkCandidates = [];
        try {
            const selection = selectHunkProbes(hunkProbeInputs(worktree, base, headSha, revert, probes, candidates));
        ...
        try {
            ...
            const detail = worktreeCreateFailureDetail('probe', e, String(sweep?.stderr ?? ''));
            ...
            for (const c of candidates) {
                mutantResults.push({ ...c, verdict: 'inconclusive', detail });
            }
            for (const { patch: _p, ...h } of hunkCandidates) {
                hunkResults.push({ ...h, verdict: 'inconclusive', detail });
            }
        }
        if (created && probes.length > 0) {
            ...
            if (linked.size > 0) {
                ...
                for (const file of probes) {
                    if (linked.has(file)) {
                        results.push({
                            ...
                            verdict: 'inconclusive',
                            reason: 'not-run',
                            detail: 'committed as a symlink (mode 120000) — vitest would collect it through the link and score mutants against code this tree never mutated, so it was dropped from the probe set',
                        });
                    }
                    else {
                        kept.push(file);
                ...
                probes = kept;
                if (probes.length === 0) {
                    noteMutants('mutants not run: every probe file is committed as a symlink (mode 120000), so no suite can be scored in this tree');
                }
            }
        }
        if (created &&
            ...
            try {
                ...
                const mutantDeadline = startedAt + TOTAL_BUDGET_MS - PROBE_RUN_TIMEOUT_MS;
                ...
                if (staleBaseline !== null) {
                    throw new Error(`the probe tree could not be put back to the commit: ${staleBaseline}`);
                }
                const baseline = runProbeSuite(probeTree, probes, mutantDeadline, now, worktree);
                ...
                if (greenProbes.length === 0) {
                    ...
                    noteMutants('mutants not run: no probe file was green in the unmutated baseline (every file was red or collected nothing), so a red mutant run would prove nothing');
                }
                else {
                    ...
                    if (fitsAnotherMutantRun(mutantDeadline - now(), estimatedRunMs)) {
                        harnessValidated = runControlMutant(probeTree, greenProbes[0], mutantDeadline, now, 
                        ...
                        if (harnessValidated === null) {
                            ...
                            noteMutants(`the positive control could not be set up (${greenProbes[0]} could not be read in the probe tree), so the harness was NOT validated this run — read every survivor below as unconfirmed by a control`);
                        }
                    }
                    if (harnessValidated === false) {
                        ...
                        noteMutants('positive control FAILED (the injected always-failing test did not turn the run red — a dead runner, a collector that skipped it, or a reporter that dropped the failure): nothing here can kill, every would-be survivor is reported inconclusive, and the remaining mutant/hunk window was not spent');
                    }
                    for (const c of harnessValidated === false ? [] : candidates) {
                        ...
                        const heldDetail = heldForRedCollocatedTest('mutant', c.file, allProbes, greenProbes, baseline.perFile);
                        if (heldDetail) {
                            mutantResults.push({
                                ...c,
                                verdict: 'inconclusive',
                        ...
                        mutantResults.push(runOneMutant(probeTree, c, greenProbes, mutantDeadline, now, worktree));
                    }
                    ...
                    for (const h of harnessValidated === false ? [] : hunkCandidates) {
                        ...
                        const heldDetail = heldForRedCollocatedTest('hunk', h.file, allProbes, greenProbes, baseline.perFile);
                        if (heldDetail) {
                        ...
                        hunkResults.push(runOneHunkProbe(probeTree, h, greenProbes, mutantDeadline, now, worktree));
                    }
                }
            }
            catch (e) {
                ...
                for (const { patch: _p, ...h } of hunkCandidates.slice(hunkResults.length)) {
                    hunkResults.push({ ...h, verdict: 'inconclusive', detail });
                }
            }
        }
        ...
        if (created && probes.length > 0) {
            try {
                ...
                if (probeTargetEscapes(probeTree, '.')) {
                    throw new Error('the probe tree no longer resolves to itself (a symlink at its ' +
                ...
                const collectable = probes.filter((p) => !probeTargetEscapes(probeTree, p));
                ...
                if (collectable.length === 0) {
                    throw new Error('every probe was relinked out of the probe tree after the baseline — the revert phase had nothing left it could score');
                }
                for (const p of probes) {
                    if (collectable.includes(p))
                        continue;
                    results.push({
                        file: p,
                        verdict: 'inconclusive',
                        reason: 'not-run',
                        detail: 'the probe resolves through a symlink out of the probe tree — it was relinked after the baseline, so this run could not score it',
                    });
                }
                const revertRun = runProbeSuite(probeTree, collectable, startedAt + TOTAL_BUDGET_MS, now, worktree);
                ...
                const reason = e instanceof ProbeRunFailure ? e.reason : 'not-run';
                results.push(...probes.map((file) => ({
                    file,
                    verdict: 'inconclusive',
                    reason,
                    detail,
                ...
                if (existsSync(probeTree)) {
                    cleanupFailure = probeCleanupFailureDetail(probeTree, removeError, String(discard?.stderr ?? ''));
                }
            }
        }
    }
    ...
    if (harnessValidated === false) {
        const detail = 'the positive control failed (an injected always-failing test stayed green), so a surviving mutant proves nothing about coverage';
    ...
    const count = (v) => mutantResults.filter((m) => m.verdict === v).length;
    const result = {
        unreachable,
        probed: results,
    ...
    writeStdoutLine(`Wrote test-efficacy report to ${out} (${unreachable.length} unreachable, ${results.length} probed, ${mutantResults.length} mutant(s), ${hunkResults.length} hunk probe(s), ${findings.length} finding(s))`);
    ...
    if (mutantsSkippedForBaseline > 0) {
        writeStdoutLine(`  ${mutantsSkippedForBaseline} mutant(s) skipped: no probe file was green in the unmutated baseline`);
    }
    if (mutantsSkippedForBudget > 0) {
        writeStdoutLine(`  ${mutantsSkippedForBudget} mutant(s) skipped: the remaining budget cannot fit another suite run`);
    }
    ...
    if (hunksSkippedForCap > 0) {
        writeStdoutLine(`  ${hunksSkippedForCap} hunk probe(s) skipped: more hunks than the cap of ${MAX_HUNK_PROBES}`);
    }
    if (hunksSkippedForBudget > 0) {
        writeStdoutLine(`  ${hunksSkippedForBudget} hunk probe(s) skipped: the mutants used the window`);
    }
    if (hunksSkippedForBaseline > 0) {
        writeStdoutLine(`  ${hunksSkippedForBaseline} hunk probe(s) skipped: no probe file was green in the unmutated baseline`);
    }
    if (mutantsNote) {
...
export const testEfficacyCommand = {
    command: 'test-efficacy <report>',
    describe: "Check whether the diff's new tests actually gate its new behaviour (unreachable + revert probe + statement-deletion mutants + per-hunk probes)",
    builder: (yargs) => yargs
        .positional('report', {
        type: 'string',
        demandOption: true,
    ...
    handler: async (argv) => {
        await runTestEfficacy(argv);
    },
};
//# sourceMappingURL=test-efficacy.js.map