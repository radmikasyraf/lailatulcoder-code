...
import { forwardApproval, wrapConfirmWithBadge, } from './leaderPermissionBridge.js';
import { TeamEventEmitter, TeamEventType } from './team-events.js';
import { MAX_TEAMMATES, LEADER_NAME } from './types.js';
import { formatAgentId, generateUniqueTeammateName, assignTeammateColor, writeTeamFile, findMemberByName, classifyShutdownResponse, sanitizeName, } from './teamHelpers.js';
import { consumeUnread, sendStructuredMessage, writeMessage, readInbox, getInboxPath, } from './mailbox.js';
import { listTasks, claimTask, onTasksUpdated, unassignTeammateTasks, } from './tasks.js';
...
(function (MessagePriority) {
    MessagePriority[MessagePriority["SHUTDOWN"] = 0] = "SHUTDOWN";
    MessagePriority[MessagePriority["LEADER"] = 1] = "LEADER";
    MessagePriority[MessagePriority["PEER"] = 2] = "PEER";
 ...
 */
const LEADER_ENVELOPE_TAG_RE = new RegExp(`<(\\/?\\s*${LEADER_ENVELOPE_TAG})(?=[\\s>/]|$)`, 'gi');
// ─── TeamManager ────────────────────────────────────────────
export class TeamManager {
    backend;
    teamFile;
    teamEventEmitter = new TeamEventEmitter();
    /**
    ...
    maxTeammates;
    constructor(backend, teamFile, subagentManager, options) {
        ...
        this.taskUpdateUnsubscribe = onTasksUpdated((teamName) => {
            if (teamName === this.teamFile.name) {
                this.fireAndForget('scanIdleAgentsForTasks', this.scanIdleAgentsForTasks());
            }
        });
    }
    // ─── Teammate lifecycle ─────────────────────────────────
    ...
    async spawnTeammate(config) {
        if (this.teamFile.members.length >= this.maxTeammates) {
            throw new Error(`Maximum number of teammates (${this.maxTeammates}) reached.`);
        }
        ...
        const rollback = () => {
            ...
            if (idx !== -1)
                this.teamFile.members.splice(idx, 1);
            ...
            if (agentSpawned) {
                try {
                    ...
                    const errMsg = stopErr instanceof Error ? stopErr.message : String(stopErr);
                    debug.warn(`Failed to stop agent ${agentId} during rollback: ${errMsg}`);
                }
            }
        };
        try {
            ...
            if (config.agentType && this.subagentManager) {
                const subagentConfig = await this.subagentManager.loadSubagent(config.agentType);
                if (!subagentConfig) {
                    throw new Error(`Subagent type "${config.agentType}" not found.`);
                }
                const runtimeCfg = await this.subagentManager.convertToRuntimeConfig(subagentConfig);
                ...
                if (toolConfig) {
                    ...
                    const existing = new Set(toolConfig.tools.map((t) => (typeof t === 'string' ? t : t.name)));
                    ...
                    if (toolConfig.disallowedTools?.length) {
                        toolConfig.disallowedTools = toolConfig.disallowedTools.filter((t) => !teamTools.includes(t));
                    }
                }
            }
            if (config.readOnly) {
            ...
            const addendum = buildTeammatePromptAddendum(name, this.teamFile.name, LEADER_NAME, {
            ...
            await runWithTeammateIdentity(identity, () => this.backend.spawnAgent(spawnConfig));
            agentSpawned = true;
     ...
     */
    async sendMessage(toName, message, from, summary, automatic = false) {
        // Messages addressed to the leader go to leader's mailbox.
        if (toName.toLowerCase() === LEADER_NAME ||
            ...
            const sender = from
                ...
                : undefined;
            const shutdownResponse = sender && !automatic && this._shutdownPending.has(sender.name)
        ...
        if (!queue) {
            ...
            // silently dropped.
            throw new Error(`Teammate "${toName}" is no longer active and cannot ` +
                `receive messages.`);
        }
        if (queue.length >= TeamManager.MAX_PENDING_MESSAGES) {
            ...
            throw new Error(`Teammate "${toName}" has too many pending messages ` +
     ...
     * (except the sender).
     */
    async broadcast(message, fromName) {
        ...
        if (failures.length > 0) {
            debug.warn(`Broadcast: ${failures.length}/${results.length} send(s) failed ` +
    ...
    async requestShutdown(name) {
        ...
        await sendStructuredMessage(this.teamFile.name, member.name, {
            ...
            text: 'The team leader has requested that you shut down. ' +
    ...
    async consumeLeaderInbox() {
        ...
        try {
            ...
            debug.warn(`Leader inbox consume failed (transient), will retry: ${getErrorMessage(err)}`);
            return [];
        }
    }
    /**
     ...
     */
    async quarantineLeaderInbox(err) {
        const inboxPath = getInboxPath(this.teamFile.name, LEADER_NAME);
        debug.warn(`Quarantining corrupt leader inbox at ${inboxPath}: ${getErrorMessage(err)}`);
        try {
            await fsPromises.rename(inboxPath, `${inboxPath}.corrupt-${Date.now()}`);
        }
        catch (renameErr) {
            debug.warn(`Failed to quarantine ${inboxPath}: ${getErrorMessage(renameErr)}`);
        }
        return [];
    }
     ...
     */
    setLeaderMessageCallback(cb) {
        this.leaderMessageCallback = cb
            ? (message, display) => runOutsideAgentContext(() => cb(message, display))
            : null;
    }
    requestPlanApproval(request) {
        const member = findMemberByName(this.teamFile.members, request.teammateName);
        if (!member) {
            return Promise.reject(new Error(`Teammate "${request.teammateName}" not found.`));
        }
        if (!member.planModeRequired) {
            return Promise.reject(new Error(`Teammate "${request.teammateName}" is not configured for plan approval.`));
        }
        const callback = this.leaderMessageCallback;
        if (!callback) {
            return Promise.reject(new Error('No leader message callback is attached for plan approval.'));
        }
        ...
        for (const pending of this.pendingPlanApprovals.values()) {
            if (pending.teammateName === member.name) {
                return Promise.reject(new Error(`Teammate "${member.name}" already has a pending plan approval request.`));
            }
        }
        const requestId = randomBytes(12).toString('hex');
        debug.info(`Created plan approval request ${requestId} for teammate "${member.name}"`);
        return new Promise((resolve, reject) => {
            ...
            if (request.signal) {
                pending.onAbort = () => {
                    this.rejectPlanApprovalRequest(requestId, new Error('Plan approval request aborted.'));
                };
            ...
            try {
                ...
                callback(this.formatPlanApprovalEnvelope(requestId, normalizedRequest), `**${member.name}** requested plan approval`);
            }
            catch (error) {
                this.rejectPlanApprovalRequest(requestId, new Error(`Leader message callback failed: ${getErrorMessage(error)}`));
            }
        });
    }
    resolvePlanApprovalRequest(requestId, decision) {
        const pending = this.pendingPlanApprovals.get(requestId);
        if (!pending) {
            throw new Error(`No pending plan approval request for id "${requestId}".`);
        }
        this.clearPlanApprovalRequest(requestId, pending);
        debug.info(`Resolved plan approval request ${requestId} for teammate "${pending.teammateName}" with action "${decision.action}"`);
     ...
     */
    ensureLeaderInboxPolling() {
        if (this.pollingInterval)
            return;
        this.pollingInterval = setInterval(() => this.fireAndForget('pollLeaderInbox', this.pollLeaderInbox()), 500);
    }
    /**
     * Stop polling the leader inbox.
     */
    stopLeaderInboxPolling() {
        if (this.pollingInterval) {
     ...
     */
    formatLeaderEnvelope(messages) {
        return messages.map((m) => `<${LEADER_ENVELOPE_TAG} from="${m.from}">\n` +
     ...
     */
    static escapeEnvelopeTags(text) {
        return text.replace(LEADER_ENVELOPE_TAG_RE, '&lt;$1');
    }
    formatPlanApprovalEnvelope(requestId, request) {
        const payload = {
        ...
        const escapedJson = JSON.stringify(payload, null, 2).replace(/</g, '\\u003c');
        return [
     ...
     */
    allTeammatesTerminated() {
        for (const member of this.teamFile.members) {
            const agent = this.getAgentFromBackend(member.agentId);
            if (!agent)
                continue;
            if (!isTerminalStatus(agent.getStatus()))
                return false;
        }
        return true;
    }
     ...
     */
    waitForTeammateActivity(timeoutMs = 120_000, signal) {
        return new Promise((resolve) => {
            ...
            const finish = (reason) => {
                ...
                this.teamEventEmitter.off(TeamEventType.ALL_TEAMMATES_TERMINATED, onTerminated);
                resolve(reason);
            };
            ...
            const wrappedCallback = (msg, display) => {
            ...
            this.teamEventEmitter.once(TeamEventType.ALL_TEAMMATES_TERMINATED, onTerminated);
            // Resolve on timeout.
    ...
    buildTeamStatusSummary() {
        ...
        for (const member of this.teamFile.members) {
            ...
            if (isTerminalStatus(status)) {
                ...
                if (lastActivityAgo >= TeamManager.STALL_THRESHOLD_S) {
                    stalled++;
                    lines.push(`  - ${member.name}: STALLED` +
                        ` (no activity for ${lastActivityAgo}s)`);
                }
                else {
                    active++;
                    lines.push(`  - ${member.name}: RUNNING` +
        ...
        if (stalled > 0 && active === 0) {
            parts.push('', 'All remaining teammates are stalled.' +
                ' Proceed with the results you have' +
                ' — write your report now.');
        }
        else {
            parts.push('', 'Do NOT call task_list to check on teammates.' +
    ...
    fireAndForget(label, work) {
        const tracked = work.catch((err) => {
            ...
            try {
                this.leaderMessageCallback?.(`<team_error>Coordination step "${label}" failed: ${msg}</team_error>`, `Team coordination step "${label}" failed`);
            }
            catch (cbErr) {
    ...
    async cleanup() {
        this.stopLeaderInboxPolling();
        this.rejectAllPlanApprovalRequests(new Error('Team was cleaned up before plan approval completed.'));
     ...
     */
    setupEventBridge(agentId, agentName) {
        ...
        if (!agent) {
            ...
            debug.warn(`setupEventBridge: backend has no agent handle for "${agentName}" (${agentId}); it will not receive messages.`);
            return;
        }
        const emitter = agent.getEventEmitter();
        if (!emitter) {
            debug.warn(`setupEventBridge: agent "${agentName}" (${agentId}) has no event emitter; it will not receive messages.`);
            return;
        }
        ...
        const onStatusChange = (event) => {
            ...
            if (event.newStatus === AgentStatus.IDLE) {
                ...
                if (!explicitlyReported && !event.roundCancelledByUser) {
                    this.fireAndForget(`reportFinalAnswer(${agentId})`, this.sendMessage(LEADER_NAME, finalReport ??
                        `${agentName} completed a turn without a model-visible final answer. Check the shared task list or send a follow-up if more detail is needed.`, agentName, `${agentName} completed a turn`, true));
                }
                ...
                this.fireAndForget(`flushNextMessage(${agentId})`, this.flushNextMessage(agentId, agentName));
            }
            if (isTerminalStatus(event.newStatus)) {
                // Release any in_progress tasks back to pending so
                // other teammates can pick them up.
                this.fireAndForget(`unassignTeammateTasks(${agentId})`, unassignTeammateTasks(this.teamFile.name, agentId).then((count) => {
                    if (count > 0) {
                        this.fireAndForget('scanIdleAgentsForTasks', this.scanIdleAgentsForTasks());
                    }
                }));
                ...
                this.rejectPendingPlanApprovalsForTeammate(agentName, new Error(`Teammate "${agentName}" terminated before plan approval completed.`));
            }
        };
        const onToolCall = (_event) => {
            recordActivity();
        };
        const onToolResult = (_event) => {
            recordActivity();
        };
        ...
        const onApproval = (event) => {
            ...
            const badged = wrapConfirmWithBadge(event.confirmationDetails, agentName, event.respond, color);
     ...
     */
    emitTeammateApprovalRequest(agentName, event) {
        const payload = {
        ...
        this.teamEventEmitter.emit(TeamEventType.TEAMMATE_APPROVAL_REQUEST, payload);
    }
     ...
     */
    async flushNextMessage(agentId, agentName) {
        const agent = this.getAgentFromBackend(agentId);
        if (!agent)
            return;
        ...
        if (this._shutdownPending.has(agentName)) {
            const shutdowns = await consumeUnread(this.teamFile.name, agentName, 'shutdown_request');
     ...
     */
    enqueueWithIdentity(agentId, agent, message) {
     ...
     */
    async tryAutoClaimTask(agentId, agentName, pending) {
        const agent = this.getAgentFromBackend(agentId);
        if (!agent)
            return;
        if (agent.getStatus() !== AgentStatus.IDLE)
            return;
        if (findMemberByName(this.teamFile.members, agentName)?.readOnly)
            return;
        if (this._shutdownPending.has(agentName))
            return;
    ...
    buildTaskPrompt(task) {
        ...
        return (`You have been assigned task #${task.id}.\n\n` +
            `${open}\n` +
    ...
    validateTaskOwner(ownerName) {
        ...
        if (!member) {
            return (`Cannot assign to "${ownerName}": no teammate by that name. ` +
                `Spawn the teammate first or choose an existing one.`);
        }
        if (this._shutdownPending.has(member.name)) {
            return (`Cannot assign to "${ownerName}": shutdown is already pending ` +
                `for that teammate.`);
        }
        const agent = this.getAgentFromBackend(member.agentId);
        if (!agent || isTerminalStatus(agent.getStatus())) {
            return (`Cannot assign to "${ownerName}": that teammate is no longer ` +
     ...
     */
    async dispatchAssignedTask(task) {
    ...
    async scanIdleAgentsForTasks() {
        ...
        if (pending.length === 0)
            return;
        await Promise.all(idleMembers.map((member) => this.tryAutoClaimTask(member.agentId, member.name, pending)));
    }
    clearPlanApprovalRequest(requestId, pending) {
        this.pendingPlanApprovals.delete(requestId);
        if (pending.signal && pending.onAbort) {
            pending.signal.removeEventListener('abort', pending.onAbort);
        }
    }
    rejectPlanApprovalRequest(requestId, error) {
        const pending = this.pendingPlanApprovals.get(requestId);
        if (!pending)
            return;
        this.clearPlanApprovalRequest(requestId, pending);
    ...
    rejectPendingPlanApprovalsForTeammate(teammateName, error) {
    ...
    rejectAllPlanApprovalRequests(error) {
        for (const [requestId, pending] of this.pendingPlanApprovals) {
            this.clearPlanApprovalRequest(requestId, pending);
            this.logRejectedPlanApprovalRequest(requestId, pending, error);
            pending.reject(error);
    ...
    logRejectedPlanApprovalRequest(requestId, pending, error) {
        debug.info(`Rejected plan approval request ${requestId} for teammate "${pending.teammateName}": ${error.message}`);
    }
    /**
     * Determine message priority from the sender name.
     */
    getSenderPriority(from) {
        if (!from)
        ...
        if (from === this.teamFile.leadAgentId ||
