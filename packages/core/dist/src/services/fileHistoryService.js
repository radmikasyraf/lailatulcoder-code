...
import { chmod, copyFile, mkdir, open, readFile, stat, unlink, } from 'node:fs/promises';
...
export function serializeSnapshot(s) {
    return {
        ...
        trackedFileBackups: Object.fromEntries(Object.entries(s.trackedFileBackups).map(([path, backup]) => [
            path,
            {
...
export function deserializeSnapshots(arr) {
    return arr.map((s) => ({
        promptId: s.promptId,
...
function resolveBackupPath(backupFileName, sessionId) {
    const baseDir = resolve(Storage.getGlobalQwenDir(), FILE_HISTORY_DIR, sessionId);
...
async function restoreBackup(filePath, backupFileName, sessionId) {
    const backupPath = resolveBackupPath(backupFileName, sessionId);
    let backupStats;
    try {
    ...
    if (result === 'src-missing') {
        debugLogger.error(`FileHistory: Backup file disappeared during restore: ${backupPath}`);
        return false;
    }
    await chmod(filePath, backupStats.mode);
    return true;
}
async function checkOriginFileChanged(originalFile, backupFileName, sessionId, originalStatsHint) {
    const backupPath = resolveBackupPath(backupFileName, sessionId);
...
async function computeDiffStatsForFile(originalFile, backupFileName, sessionId) {
    const filesChanged = [];
    let insertions = 0;
    let deletions = 0;
    try {
 ...
 */
async function readEndpointContent(backup, worktreePath, sessionId) {
 ...
 */
async function readPathWithSizeGuard(path, kind) {
    let fh;
    try {
...
export class FileHistoryService {
    ...
    cwd;
    onSnapshotUpdated;
    constructor(sessionId, enabled, cwd, onSnapshotUpdated) {
    ...
    async validateRestoredSnapshots() {
        ...
        for (let i = 0; i < names.length; i += BATCH_SIZE) {
            ...
            const results = await Promise.all(batch.map(async (name) => {
                ...
                try {
                    ...
                    debugLogger.error(`FileHistory: rejected backupFileName during validation: ${name}: ${e}`);
                    return false;
                }
        ...
        debugLogger.warn(`FileHistory: ${missing.size} restored backup file(s) missing on disk`);
    }
    async trackEdit(filePath) {
        if (!this.enabled)
            return;
     ...
     */
    async makeSnapshot(promptId) {
        if (!this.enabled)
            return;
        const trackedFileBackups = {};
        const mostRecent = this.state.snapshots.at(-1);
        if (mostRecent) {
            await Promise.all(Array.from(this.state.trackedFiles, async (trackingPath) => {
                try {
                    ...
                    if (latestBackup &&
                        ...
                        !(await checkOriginFileChanged(filePath, latestBackup.backupFileName, this.sessionId, fileStats))) {
                    ...
                    trackedFileBackups[trackingPath] = await createBackup(filePath, nextVersion, this.sessionId);
                }
                catch (error) {
                    debugLogger.error(`FileHistory: Failed to backup file ${trackingPath}: ${error}`);
        ...
        debugLogger.debug(`FileHistory: Added snapshot for ${promptId}, tracking ${this.state.trackedFiles.size} files`);
    }
    async rewind(promptId, truncateHistory = true) {
        if (!this.enabled)
        ...
        if (truncateHistory && result.filesFailed.length === 0) {
            ...
            if (targetIdx >= 0) {
                ...
                this.state.trackedFiles = new Set(this.state.snapshots.flatMap((s) => Object.keys(s.trackedFileBackups)));
                await this.cleanupOrphanedBackups(removed);
            }
        }
    ...
    async getDiffStats(promptId) {
        ...
        if (!targetSnapshot)
            return undefined;
        const results = await Promise.all(Array.from(this.state.trackedFiles, async (trackingPath) => {
            try {
                const filePath = this.maybeExpandFilePath(trackingPath);
                ...
                if (backupFileName === undefined)
                    return null;
                const stats = await computeDiffStatsForFile(filePath, backupFileName === null ? undefined : backupFileName, this.sessionId);
                ...
                debugLogger.error(`FileHistory: Error computing diff stats: ${error}`);
                return null;
            }
        }));
        const filesChanged = [];
    ...
    async getTurnDiff(promptId) {
        ...
        const nextSnapshot = targetIdx + 1 < this.state.snapshots.length
        ...
        const candidatePaths = Object.keys(target.trackedFileBackups).sort((a, b) => a.localeCompare(b));
        ...
        if (filesOmitted > 0) {
            debugLogger.warn(`FileHistory: getTurnDiff truncating ${filesOmitted} files for prompt ${promptId} (cap: ${MAX_TURN_DIFF_FILES})`);
        }
        const cappedPaths = candidatePaths.slice(0, MAX_TURN_DIFF_FILES);
        const results = await Promise.all(cappedPaths.map((trackingPath) => this.computeTurnFileDiff(trackingPath, target, nextSnapshot)));
        const files = [];
        let totalAdded = 0;
        let totalRemoved = 0;
        for (const r of results) {
    ...
    async computeTurnFileDiff(trackingPath, before, after) {
        try {
            ...
            debugLogger.error(`FileHistory: computeTurnFileDiff failed for ${trackingPath}: ${e}`);
            return null;
        }
    }
    async computeTurnFileDiffUnsafe(trackingPath, before, after) {
        ...
        const beforeRead = await readEndpointContent(beforeBackup, undefined, this.sessionId);
        ...
        if (beforeRead.kind === 'unreadable') {
            debugLogger.warn(`FileHistory: skipping turn diff for ${trackingPath}: before backup unreadable`);
            return null;
        }
        ...
        if (afterRead.kind === 'unreadable') {
            debugLogger.warn(`FileHistory: skipping turn diff for ${trackingPath}: after ${afterFromWorktree ? 'worktree' : 'backup'} unreadable`);
            return null;
        }
        ...
        const oversized = Buffer.byteLength(beforeContent, 'utf8') > MAX_DIFF_SIZE_BYTES ||
        ...
        const patch = structuredPatch(trackingPath, trackingPath, beforeContent, afterContent, '', '', { context: 3 });
        let linesAdded = 0;
        let linesRemoved = 0;
    ...
    async applySnapshot(targetSnapshot) {
        const filesChanged = [];
        const filesFailed = [];
        for (const trackingPath of this.state.trackedFiles) {
            try {
                ...
                const backupFileName = targetBackup
                    ...
                    : this.getBackupFileNameFirstVersion(trackingPath);
                if (backupFileName === undefined) {
                    debugLogger.error('FileHistory: Error finding the backup file to apply');
                    filesFailed.push(filePath);
                    continue;
                }
                ...
                if (await checkOriginFileChanged(filePath, backupFileName, this.sessionId)) {
                    const restored = await restoreBackup(filePath, backupFileName, this.sessionId);
                    if (restored) {
                        debugLogger.debug(`FileHistory: Restored ${filePath} from ${backupFileName}`);
                        filesChanged.push(filePath);
                    }
                    else {
                        filesFailed.push(filePath);
    ...
    async cleanupOrphanedBackups(removedSnapshots) {
        ...
        await Promise.all(Array.from(toDelete, async (name) => {
            try {
...
//# sourceMappingURL=fileHistoryService.js.map