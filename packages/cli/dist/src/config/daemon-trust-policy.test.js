...
import { evaluateDaemonWorkspaceTrust, readDaemonTrustPolicySnapshot, } from './daemon-trust-policy.js';
...
function installFiles(files) {
    ...
    mockedFs.readFile.mockImplementation(async (filePath) => files[String(filePath)] ?? '');
}
describe('daemon trust policy', () => {
    ...
    it('uses system folder trust over user', async () => {
        ...
        expect(evaluateDaemonWorkspaceTrust(snapshot, '/work/project')).toMatchObject({
            state: 'trusted',
            targetTrusted: true,
            source: 'disabled',
        });
    ...
    it('confirms a missing trusted-folders file before treating it as empty', async () => {
        ...
        expect(mockedFs.lstat.mock.calls.filter(([filePath]) => String(filePath) === '/config/trusted.json')).toHaveLength(2);
    });
    it('retries when trusted folders disappear between stat and read', async () => {
        ...
        expect(mockedFs.readFile.mock.calls.filter(([filePath]) => String(filePath) === '/config/trusted.json')).toHaveLength(2);
    });
    ...
    it('fails closed when system defaults are malformed', async () => {
        ...
        const decision = evaluateDaemonWorkspaceTrust(await readDaemonTrustPolicySnapshot(), '/work');
    ...
    it('resolves relative trusted-folder rules from the daemon working directory', async () => {
        ...
        const decision = evaluateDaemonWorkspaceTrust(await readDaemonTrustPolicySnapshot(), `${process.cwd()}/relative-workspace/project`);
        expect(decision).toMatchObject({
            state: 'trusted',
            targetTrusted: true,
            source: 'file',
    ...
    it('maps an unknown workspace to operationally untrusted', async () => {
        ...
        const decision = evaluateDaemonWorkspaceTrust(await readDaemonTrustPolicySnapshot(), '/work/unknown');
        expect(decision).toMatchObject({
            state: 'unknown',
            targetTrusted: false,
            source: 'none',
    ...
    it('uses the most-specific trust rule for descendant workspaces', async () => {
        ...
        const snapshot = await readDaemonTrustPolicySnapshot();
        expect(evaluateDaemonWorkspaceTrust(snapshot, '/projects/evil/packages/foo')).toMatchObject({
            state: 'untrusted',
            targetTrusted: false,
            source: 'file',
    ...
    it('ignores a trusted-folders error when folder trust is disabled', async () => {
        ...
        expect(evaluateDaemonWorkspaceTrust(await readDaemonTrustPolicySnapshot(), '/work')).toMatchObject({
            state: 'trusted',
            targetTrusted: true,
            source: 'disabled',
        });
    ...
    it('lets IDE trust resolve the primary before a trusted-folders error', async () => {
        ...
        expect(evaluateDaemonWorkspaceTrust(await readDaemonTrustPolicySnapshot(), process.cwd())).toMatchObject({ state: 'trusted', source: 'ide' });
    });
});
//# sourceMappingURL=daemon-trust-policy.test.js.map