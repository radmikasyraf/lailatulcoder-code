...
export const REPEATED_TOOL_FAILURE_REMINDER = 'System: the same tool execution has failed repeatedly for the same classified reason. Do not repeat the same approach. Inspect the returned result, change the approach or required preconditions, or explain the blocker.';
export const REPEATED_TOOL_FAILURE_STOP_MESSAGE = 'Automatic continuation stopped because the same tool execution failure continued after a corrective reminder. New user input is required to continue.';
...
export function createRepeatedToolFailureGuardState() {
    return {
        phase: 'idle',
        failureCount: 0,
        batchCount: 0,
...
export function parseRepeatedToolFailureGuardMode(value) {
...
function resetState(state, enforcementDisabled = state.enforcementDisabled) {
    return {
        phase: 'idle',
        failureCount: 0,
        batchCount: 0,
...
function reset(state, reason, enforcementDisabled = state.enforcementDisabled) {
...
function keysEqual(left, right) {
    return (left?.policyToolName === right.policyToolName &&
        left.executionErrorType === right.executionErrorType);
}
function effectiveMode(mode, enforcementDisabled) {
    return mode === 'enforce' && enforcementDisabled ? 'warn' : mode;
}
export function reduceRepeatedToolFailureGuard(state, input) {
    ...
    const observations = input.batch.observations.filter((observation) => !observation.providerDuplicate);
    if (observations.length === 0) {
        return { kind: 'none', state };
    }
    const eligible = [];
    const successfulPolicyToolNames = new Set();
    ...
    const resetReason = INELIGIBLE_RESET_REASON_PRECEDENCE.find((reason) => resetReasons.has(reason));
    if (resetReason !== undefined) {
        return reset(state, resetReason, enforcementDisabled);
    }
    const matchingFailures = eligible.filter((failure) => !successfulPolicyToolNames.has(failure.policyToolName));
    ...
    if (state.phase === 'warned') {
        ...
        return {
            kind: effectiveMode(input.mode, state.enforcementDisabled) === 'enforce'
                ? 'stop'
                : 'would_stop',
            state: nextState,
        };
    ...
    if (failureCount < REPEATED_TOOL_FAILURE_THRESHOLD ||
        batchCount < REPEATED_TOOL_FAILURE_BATCH_THRESHOLD) {
        return { kind: 'tracked', state: nextState };
    }
    nextState.phase = 'warned';
    return {
        kind: effectiveMode(input.mode, state.enforcementDisabled) === 'shadow'
            ? 'would_warn'
            : 'warn',
        state: nextState,
    };
}
//# sourceMappingURL=repeated-tool-failure-guard.js.map