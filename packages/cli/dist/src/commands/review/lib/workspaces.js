 ...
 */
export function isWorkspaceMember(filePath, workspaceGlobs) {
 ...
 */
export function workspaceDirFor(filePath, workspaceGlobs) {
 ...
 */
export function isNegationExcluded(filePath, workspaceGlobs) {
    if (workspaceDirFor(filePath, workspaceGlobs) !== null)
        return false;
 ...
 */
export function hasUnmodeledWorkspaceGlob(globs) {
    return globs.some((glob) => {
        const g = glob.replace(/^!/, '').replace(/^\.\//, '');
        if (!g.includes('*'))
            return false; // a literal path — fully modeled
...
export function readRootPackage(root) {
    ...
    if (!scripts.includes('build') && !scripts.includes('test'))
        return null;
    const scriptsText = {};
 ...
 */
export function scriptFansOut(text) {
 ...
 */
function workspaceDirCandidates(root, globs) {
    const dirs = new Set();
    for (const glob of globs) {
        if (glob.startsWith('!'))
            continue; // negations are applied by workspaceDirFor
        const g = glob.replace(/^\.\//, '').replace(/\/$/, '');
        if (g.endsWith('/*')) {
            const base = g.slice(0, -2);
            let entries;
...
export function readWorkspacePackages(root) {
    ...
    for (const dir of workspaceDirCandidates(root, globs)) {
        ...
        if (workspaceDirFor(`${dir}/package.json`, globs) !== dir) {
            ...
            if (workspaceDirFor(`${dir}/package.json`, positives) === dir)
                continue;
...
export function affectedWorkspaces(changedFiles, workspaceGlobs) {
    const dirs = new Set();
 ...
 */
export function reverseDependencyClosure(affected, packages) {
 ...
 * does not touch.
 */
export function buildSetFor(affected, packages, alsoBuild = []) {
    const byDir = new Map(packages.map((p) => [p.dir, p]));
    const dependsOn = dependencyEdges(packages);
    ...
    // need it.
    for (const dir of alsoBuild.filter((d) => wanted.has(d)).sort())
        visit(dir);
    for (const dir of [...wanted].sort())
        visit(dir);
    return order;
}
