 ...
 */
export function getToolResponseDisplayText(parts) {
        ...
        if (Array.isArray(fr.parts)) {
            for (const nested of fr.parts) {
                if (nested.inlineData) {
                    segments.push(`<media: ${sanitizeMediaLabel(nested.inlineData.mimeType) ?? 'inline'}>`);
                }
                else if (nested.fileData) {
                    segments.push(`<media: ${sanitizeMediaLabel(nested.fileData.mimeType) ??
