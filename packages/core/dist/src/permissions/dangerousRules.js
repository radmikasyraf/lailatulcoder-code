 ...
 * upstream identifier.
 */
const DANGEROUS_BASH_INTERPRETERS = Object.freeze([
    // Unix shells
    'bash',
    'sh',
    'zsh',
...
function stripWindowsExecutableSuffix(token) {
    return token.endsWith('.exe') ? token.slice(0, -'.exe'.length) : token;
}
function matcherColonIndex(content) {
...
function leadingCommandToken(content) {
    if (/^[a-z]:[\\/]/i.test(content)) {
 ...
 */
const SHELL_LIKE_TOOLS = Object.freeze([
    ToolNames.SHELL,
    ToolNames.MONITOR,
]);
/**
 ...
 */
function isInterpreterToken(rawToken) {
    if (!rawToken)
        return false;
    ...
    return DANGEROUS_BASH_INTERPRETERS.some((interpreter) => stripWindowsExecutableSuffix(interpreter) === normalizedSegment);
}
/**
 ...
 */
export function isDangerousBashRule(rule) {
    if (!SHELL_LIKE_TOOLS.includes(rule.toolName))
        return false;
    if (!rule.specifier || rule.specifier === '*')
        return true;
    ...
    if (!isInterpreterToken(firstToken))
        return false;
    ...
    if (firstToken === content && !hasMatcherColon)
        return true;
 ...
 */
export function isDangerousAgentRule(rule) {
 ...
 * classifier's view.
 */
export function isDangerousSkillRule(rule) {
    return rule.toolName === ToolNames.SKILL;
}
/**
 * Aggregate predicate combining all dangerous-rule categories.
 */
export function isDangerousAllowRule(rule) {
    return (isDangerousBashRule(rule) ||
 ...
 */
export function findDangerousAllowRules(allowRules) {
    return allowRules.filter(isDangerousAllowRule);
}
//# sourceMappingURL=dangerousRules.js.map