...
import { sessionIdContext, getSessionProjectDir, getSessionModel, getSessionModelIdentity, } from './sessionIdContext.js';
import { isShellTracePropagationEnabled, getTraceContext, formatTraceparent, } from '../telemetry/trace-context.js';
/**
...
const unusableCache = new Map();
export function isUnusableScriptEntry(path) {
 ...
 */
function identityDescribes(identity, model) {
    const head = /^(.*)@[0-9a-f]{8}$/.exec(identity)?.[1] ?? identity;
    return head === model;
}
export function getShellContextEnvVars() {
    const env = {};
    ...
    const sessionId = sessionIdContext.getStore() ?? process.env['QWEN_CODE_SESSION_ID'];
    if (sessionId) {
    ...
    const projectDir = (sessionId ? getSessionProjectDir(sessionId) : undefined) ??
    ...
    const modelIdentity = (sessionId ? getSessionModelIdentity(sessionId) : undefined) ??
...
//# sourceMappingURL=shellContextEnv.js.map