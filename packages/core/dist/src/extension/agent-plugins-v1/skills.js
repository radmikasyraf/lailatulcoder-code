...
export async function loadAgentPluginSkills(pluginRoot) {
    const skillsPath = path.join(pluginRoot, 'skills');
    let resolvedSkillsPath;
    try {
        ...
        if (error.code === 'ENOENT')
            return [];
        debugLogger.warn(`Disabling Agent Plugins skills: ${error instanceof Error ? error.message : String(error)}`);
        return [];
    }
    let entries;
    try {
    ...
    for (const entry of entries) {
        ...
        try {
            const resolvedSkillDir = resolveContainedExistingPath(pluginRoot, skillDir);
            if (!fs.statSync(resolvedSkillDir).isDirectory())
                continue;
            const skillManifest = path.join(resolvedSkillDir, 'SKILL.md');
            const resolvedManifest = resolveContainedExistingPath(pluginRoot, skillManifest);
            if (!fs.statSync(resolvedManifest).isFile())
                continue;
            ...
            debugLogger.warn(`Skipping Agent Plugins skill "${entry.name}": ${error instanceof Error ? error.message : String(error)}`);
        }
    }
    return skills;
}
export function parseAgentPluginSkill(content, filePath, directoryName = path.basename(path.dirname(filePath))) {
...
function validateOptionalString(frontmatter, field) {
...
function validateCompatibility(value) {
    ...
    if (typeof value !== 'string' ||
        ...
        throw new Error('Agent Skills compatibility must be a 1-500 character string.');
    }
}
