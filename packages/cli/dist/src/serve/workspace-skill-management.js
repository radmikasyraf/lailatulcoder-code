...
import { SkillManager, Storage } from '@lailatul-coder/lailatul-coder-core';
...
export function validateWorkspaceSkillName(name) {
...
function decodeBase64(value) {
    ...
    if (content.length > MAX_TOTAL_BYTES) {
        skillError('skill_package_too_large', 'Skill package exceeds the allowed size', 413);
    }
    return content;
}
...
function normalizePackageFiles(files) {
    ...
    if (!files.length || files.length > MAX_FILES) {
        skillError('invalid_skill_package', 'Skill package has an invalid file count');
    }
    ...
    if (!normalized.some((file) => file.relativePath === 'SKILL.md')) {
        const roots = new Set(normalized.map((file) => file.relativePath.split('/')[0]));
        if (roots.size !== 1) {
            skillError('skill_manifest_missing', 'Skill package must contain a root SKILL.md');
        }
        const [root] = roots;
    ...
    for (const file of normalized) {
        ...
        if (file.content.length > MAX_FILE_BYTES) {
            skillError('skill_package_too_large', `Skill file is too large: ${file.relativePath}`, 413);
        }
...
function githubRequestError(code, status, resource) {
    ...
    return new WorkspaceSkillManagementError(code, message, status === 404 ? 404 : 502);
}
async function fetchBytes(url, githubToken) {
    ...
    if (!response.ok) {
        throw githubRequestError('github_skill_download_failed', response.status, 'Skill file');
    }
...
async function downloadGitHubDirectory(owner, repo, ref, directory, githubToken, relativeRoot = '', depth = 0, state = {
    ...
    if (!response.ok) {
        throw githubRequestError('github_api_failed', response.status, 'Skill path');
    }
    ...
    for (const item of items) {
        ...
        const relativePath = relativeRoot
            ...
            : itemName;
        if (itemType === 'dir') {
            await downloadGitHubDirectory(owner, repo, ref, itemPath, githubToken, relativePath, depth + 1, state);
        }
            ...
            if (parsedDownloadUrl.protocol !== 'https:' ||
                ...
                skillError('github_skill_download_failed', 'GitHub returned an invalid Skill file URL', 502);
            }
...
async function downloadGitHubDirectoryWithGit(owner, repo, ref, directory) {
    const checkout = await fs.mkdtemp(path.join(os.tmpdir(), 'LailatulCoder-skill-git-'));
    try {
        ...
        if (directory) {
            await execFileAsync('git', ['-C', checkout, 'sparse-checkout', 'set', '--cone', directory], { timeout: 30_000, maxBuffer: 1024 * 1024 });
        }
        else {
            await execFileAsync('git', ['-C', checkout, 'sparse-checkout', 'disable'], { timeout: 30_000, maxBuffer: 1024 * 1024 });
        }
        return await filesFromFolder(path.join(checkout, directory));
    }
    catch (error) {
        if (error instanceof WorkspaceSkillManagementError)
            throw error;
        throw new WorkspaceSkillManagementError('github_skill_download_failed', `Failed to download GitHub Skill: ${error instanceof Error ? error.message : String(error)}`, 502);
    }
    finally {
...
async function downloadGitHubSkill(sourceUrl, githubToken) {
    ...
    if (segments.length < 5 ||
        ...
        skillError('invalid_skill_source', 'GitHub URL must point to a repository SKILL.md file');
    }
    ...
    try {
        return await downloadGitHubDirectory(owner, repo, ref, directory, githubToken);
    }
    catch (error) {
        if (!(error instanceof WorkspaceSkillManagementError) ||
    ...
    return new Promise((resolve, reject) => {
        zipFile.openReadStream(entry, (error, stream) => {
            ...
            stream.on('data', (chunk) => {
                size += chunk.length;
                if (size > MAX_FILE_BYTES) {
                    stream.destroy(new WorkspaceSkillManagementError('skill_package_too_large', `Skill ZIP entry is too large: ${entry.fileName}`, 413));
                }
                else {
                    chunks.push(chunk);
                }
    ...
    return new Promise((resolve, reject) => {
        ...
        zipFile.on('entry', (entry) => {
            ...
            if (files.length >= MAX_FILES) {
                finish(new WorkspaceSkillManagementError('invalid_skill_package', 'Skill ZIP contains too many files'));
                return;
            }
            if (entry.uncompressedSize > MAX_FILE_BYTES) {
                finish(new WorkspaceSkillManagementError('skill_package_too_large', 'Skill ZIP entry exceeds the allowed size', 413));
                return;
            }
            if (totalBytes + entry.uncompressedSize > MAX_TOTAL_BYTES) {
                finish(new WorkspaceSkillManagementError('skill_package_too_large', 'Skill ZIP expands beyond the allowed size', 413));
                return;
            }
...
async function filesFromSource(source, githubToken) {
    try {
        if (source.type === 'github')
            return normalizePackageFiles(await downloadGitHubSkill(source.url, githubToken));
        ...
        if (error instanceof WorkspaceSkillManagementError)
            throw error;
        const code = source.type === 'github'
            ? 'github_skill_download_failed'
            : source.type === 'folder'
                ? 'invalid_skill_folder'
                : 'invalid_skill_package';
        throw new WorkspaceSkillManagementError(code, error instanceof Error ? error.message : String(error), source.type === 'github' ? 502 : 400);
    }
}
async function filesFromFolder(folderPath) {
    ...
    if ((await fs.lstat(folderPath)).isSymbolicLink()) {
        skillError('invalid_skill_folder', 'Skill folder path must not be a symbolic link');
    }
    const root = await fs.realpath(folderPath);
    if (!(await fs.stat(root)).isDirectory()) {
        skillError('invalid_skill_folder', 'Skill folder path must point to a directory');
    }
    const files = [];
    let totalBytes = 0;
    const visit = async (directory, relativeRoot = '', depth = 0) => {
        ...
        for (const entry of await fs.readdir(directory, { withFileTypes: true })) {
            ...
            if (entry.isSymbolicLink()) {
                skillError('unsafe_skill_path', 'Skill folder contains a symbolic link');
            }
            ...
            if (stats.size > MAX_FILE_BYTES) {
                skillError('skill_package_too_large', `Skill file is too large: ${relativePath}`, 413);
            }
            ...
            if (content.length !== stats.size) {
                skillError('invalid_skill_package', `Skill file changed while reading: ${relativePath}`);
            }
...
function skillBaseDir(workspace, scope) {
    return scope === 'workspace'
        ? path.join(workspace, '.lailatulcoder', 'skills')
...
export async function installWorkspaceSkill(workspace, request, githubToken, assertGenerationOpen) {
    assertGenerationOpen?.();
    ...
    const workPrefix = `.${path.basename(baseDir)}-${skillName}`;
    assertGenerationOpen?.();
    const staging = await fs.mkdtemp(path.join(workDir, `${workPrefix}.installing-`));
    ...
    try {
        ...
        let parsed;
        try {
            parsed = new SkillManager({}).parseSkillContent(await fs.readFile(skillFile, 'utf8'), skillFile, request.scope === 'workspace' ? 'project' : 'user');
        }
        catch (error) {
            throw new WorkspaceSkillManagementError('invalid_skill_manifest', error instanceof Error ? error.message : String(error));
        }
        if (parsed.name !== skillName) {
            skillError('skill_name_mismatch', `Skill name "${parsed.name}" does not match requested name "${skillName}"`);
        }
        if (existing) {
            assertGenerationOpen?.();
...
export async function deleteWorkspaceSkill(workspace, scope, skillNameInput, installedPath, assertGenerationOpen) {
    assertGenerationOpen?.();
    ...
    if (skillFile !== path.join(skillDir, 'SKILL.md') ||
        !allowedBaseDirs.includes(baseDir) ||
        path.basename(skillDir) !== skillName) {
        skillError('skill_not_managed', 'Skill is not managed in the requested scope', 409);
    }
    ...
    const parsed = new SkillManager({}).parseSkillContent(await fs.readFile(skillFile, 'utf8'), skillFile, scope === 'workspace' ? 'project' : 'user');
    if (parsed.name !== skillName) {
        skillError('skill_name_mismatch', 'Skill name does not match its installed directory');
    }
    assertGenerationOpen?.();
    await fs.rm(skillDir, { recursive: true, force: true });
    return { skillName, scope, deleted: true };
}
//# sourceMappingURL=workspace-skill-management.js.map