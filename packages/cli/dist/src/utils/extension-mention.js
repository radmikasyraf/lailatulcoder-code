...
import { getErrorMessage, isSubpath, stripTerminalControlSequences, } from '@lailatul-coder/lailatul-coder-core';
...
export function parseExtensionRef(pathName) {
    if (!pathName.startsWith(EXTENSION_REF_PREFIX))
        return null;
...
export function buildExtensionRef(extensionName) {
    return `${EXTENSION_REF_PREFIX}${extensionName}`;
}
export function matchExtensionByRef(name, extensions) {
    const lower = name.toLowerCase();
    return extensions.find((ext) => ext.name.toLowerCase() === lower ||
...
export function getSanitizedExtensionDisplayName(extension) {
    return (sanitizeDisplayText(extension.displayName || extension.name) ||
        extension.name);
}
export function buildExtensionContextText(extension) {
...
export async function buildExtensionMentionContext(extension, options) {
    ...
    const fileReads = await Promise.allSettled(extension.contextFiles.map(async (contextFilePath) => {
        let realPath;
        let realExtPath;
        try {
            ...
            options.onDebugMessage?.(`Skipping unreadable context file: ${contextFilePath}`);
            return null;
        }
        if (!isSubpath(realExtPath, realPath)) {
            options.onDebugMessage?.(`Skipping context file outside extension directory: ${contextFilePath}`);
            return null;
        }
    ...
    for (let i = 0; i < fileReads.length; i++) {
        ...
        if (outcome.status === 'rejected') {
            options.onDebugMessage?.(`Failed to read extension context file ${extension.contextFiles[i]}: ${getErrorMessage(outcome.reason)}`);
            continue;
        }
        ...
        if (remainingBudget <= 0) {
            options.onDebugMessage?.('Extension context budget exhausted, skipping remaining files.');
            break;
        }
        const cap = Math.min(EXTENSION_CONTEXT_FILE_CAP, remainingBudget);
        const cappedContent = content.length > cap
            ? content.slice(0, cap) + '\n... (truncated)'
            : content;
        contextText += `\n\n${cappedContent}`;
...
//# sourceMappingURL=extension-mention.js.map