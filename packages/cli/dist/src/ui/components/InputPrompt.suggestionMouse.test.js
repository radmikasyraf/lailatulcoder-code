...
describe('InputPrompt suggestion mouse routing', () => {
    ...
    it('uses the startup VP decision for suggestion mouse when the raw setting is unset', () => {
        const { unmount } = renderWithProviders(_jsx(VirtualViewportContext.Provider, { value: true, children: _jsx(InputPrompt, { ...props }) }));
        expect(captured.props).not.toBeNull();
    ...
    it('disables suggestion mouse when ui.mouseTracking is false despite VP being on', () => {
        ...
        const settingsNoMouse = new LoadedSettings({ path: '', settings: {}, originalSettings: {} }, { path: '', settings: {}, originalSettings: {} }, { path: '', settings: ui, originalSettings: ui }, { path: '', settings: {}, originalSettings: {} }, true, new Set());
        const { unmount } = renderWithProviders(_jsx(InputPrompt, { ...props }), {
    ...
    it('hovering a suggestion updates the active index on the default source', () => {
        const { unmount } = renderWithProviders(_jsx(InputPrompt, { ...props }));
        act(() => {
            captured.props['onHoverIndex'](1);
        });
        expect(mockCommandCompletion.setActiveSuggestionIndex).toHaveBeenCalledWith(1);
        unmount();
    });
    it('clicking a leaf command auto-submits it (submitOnAccept), matching Enter', () => {
        const { unmount } = renderWithProviders(_jsx(InputPrompt, { ...props }));
        act(() => {
            captured.props['onSelectIndex'](0);
        });
    ...
    it('clicking a non-leaf suggestion accepts without submitting', () => {
        const { unmount } = renderWithProviders(_jsx(InputPrompt, { ...props }));
        act(() => {
            captured.props['onSelectIndex'](1);
        });
    ...
    it('routes hover/select to the command-search source while command search is active', async () => {
        ...
        expect(mockCommandCompletion.setActiveSuggestionIndex).not.toHaveBeenCalled();
...
//# sourceMappingURL=InputPrompt.suggestionMouse.test.js.map