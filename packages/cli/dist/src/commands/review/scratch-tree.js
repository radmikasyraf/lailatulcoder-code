...
import { assertWritableOutPath, inertPath, scratchLabel, scratchWorktreePath, } from './lib/paths.js';
import { shellQuotePath } from './lib/shell-quote.js';
import { discardWorktree, exposeDependencies, redirectedAncestor, sanitizedGitEnv, worktreeCreateFailureDetail, worktreeResidue, } from './lib/worktree.js';
/**
...
function localFilterCommands(worktree) {
    const files = spawnSync('git', ['rev-parse', '--git-common-dir', '--git-dir'], { cwd: worktree, encoding: 'utf8', env: sanitizedGitEnv() });
 ...
 */
function resetScratchTree(tree, headSha, worktree) {
    ...
    try {
        ...
        if (realpathSync(gitOut(tree, 'rev-parse', '--show-toplevel')) !==
            realpathSync(tree)) {
            return false;
        }
        ...
        const commonOf = (dir) => realpathSync(gitOut(dir, 'rev-parse', '--path-format=absolute', '--git-common-dir'));
        if (commonOf(tree) !== commonOf(worktree))
            return false;
        // EVERY ancestor, not just the parent. The first cut lstat'd `dirname(tree)`
        // on the stated premise that `.lailatulcoder/tmp` is the one component above the leaf
        // anything here can replace — which is false one hop higher: a link at
        // `.lailatulcoder` redirects the whole path, and then every check in this gate agrees
        ...
        if (redirectedAncestor(dirname(resolve(tree)), dirname(commonOf(worktree)))) {
            return false;
        }
        const gitdir = realpathSync(gitOut(tree, 'rev-parse', '--path-format=absolute', '--git-dir'));
        if (gitdir === commonOf(worktree))
            return false;
        ...
        if (realpathSync(dirname(resolve(gitdir, backpointer))) !== realpathSync(tree)) {
            return false;
        }
    }
    catch {
...
export function runScratchTree(args) {
    ...
    const unavailable = (note) => ({
        ...
        sharedTreeUnmeasured: 'the command refused before it measured the shared worktree',
        note,
    });
    ...
    if (!label) {
        return unavailable('--label is required, and must keep at least one of `A-Za-z0-9._-` once ' +
    ...
    // argument side.
    try {
        if (realpathSync(gitOut(worktree, 'rev-parse', '--show-toplevel')) !==
            realpathSync(worktree)) {
            return unavailable(`the review worktree ${worktree} is not a git worktree — repository ` +
                'discovery walks up into the enclosing checkout; check its .git file');
        }
    }
    catch (err) {
        return unavailable(`cannot read HEAD in ${worktree}: ${inertPath(err.message)}`);
    }
    let headSha;
    try {
    ...
    if (filters.length > 0) {
        return unavailable(`the repository's local config defines content filter(s) ${filters
            .map(inertPath)
            .join(', ')} — ` +
    ...
    if (existsSync(tree) && resetScratchTree(tree, headSha, worktree)) {
        ...
        return {
            ...
            note: `your scratch tree is at ${shellQuotePath(tree)}, restored to ${headSha.slice(0, 9)} ` +
    ...
    try {
        ...
        return {
            ...
            sharedTreeResidueTotal: residue.total,
            sharedTreeUnmeasured: residue.unmeasured,
            note: `${inertPath(worktreeCreateFailureDetail('scratch', e, String(sweep?.stderr ?? '')))}. ` +
    ...
    return {
        ...
        sharedTreeResidueTotal: residue.total,
        sharedTreeUnmeasured: residue.unmeasured,
        note: `your scratch tree is at ${shellQuotePath(tree)}, checked out at ${headSha.slice(0, 9)}. ` +
 ...
 */
function farmDependencies(tree, worktree, opts = {}) {
...
function dependencyNote(farm) {
    if (farm === null) {
        return (' The review worktree has no `node_modules`, so nothing was linked in and ' +
    ...
    if (farm.linked === 0 && farm.failed === 0) {
        ...
        return (" The review worktree's `node_modules` held nothing linkable, so a JS " +
            'unit harness will not start here — install in the SCRATCH tree if you ' +
            'need one, never in the review worktree.');
    }
    return (` ${farm.linked} dependencies linked in` +
        (farm.failed > 0
...
export const scratchTreeCommand = {
    command: 'scratch-tree',
    describe: "Create this agent's own throwaway worktree at the commit under review, so " +
    ...
    builder: (yargs) => yargs
        ...
        type: 'string',
        demandOption: true,
        describe: 'The PR worktree — the scratch tree is created beside it, at its HEAD',
    })
        .option('label', {
        type: 'string',
        demandOption: true,
        describe: 'What makes this tree yours: pass the record key from your launch ' +
    ...
    handler: (argv) => {
        ...
        try {
            ...
            writeStderrLine(`scratch-tree: ${err.message}`);
...
//# sourceMappingURL=scratch-tree.js.map