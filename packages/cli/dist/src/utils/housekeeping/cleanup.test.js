...
import { OpenAILogger } from '@lailatul-coder/lailatul-coder-core';
...
// flakiness.
function setMtime(dir, mtime) {
    fs.utimesSync(dir, mtime, mtime);
}
function mkSessionDir(root, sessionId, mtime) {
...
describe('cleanupOldFileHistoryBackups', () => {
    ...
    it.skipIf(process.platform === 'win32')('counts errors and continues sweep when one dir cannot be removed', async () => {
        const old = new Date(Date.now() - 60 * MS_PER_DAY);
...
describe('cleanupOldSubagentTranscripts', () => {
    ...
    let cutoff;
    beforeEach(() => {
        projectDir = fs.mkdtempSync(path.join(os.tmpdir(), 'qwen-subagent-cleanup-'));
...
describe('cleanupOldOpenAILogs', () => {
    ...
    function mkLog(name, mtime) {
        const p = path.join(logDir, name);
        fs.writeFileSync(p, '{}');
        fs.utimesSync(p, mtime, mtime);
        return p;
    }
    function openAILogName(timestamp, id = 'a1b2c3d4', suffix) {
    ...
    it('removes logs whose filename date is older than the cutoff, even with a fresh mtime', async () => {
        ...
        const old = mkLog(openAILogName(new Date(Date.now() - 30 * MS_PER_DAY)), new Date());
    ...
    it('preserves old openai-prefixed JSON files not emitted by OpenAILogger', async () => {
        const oldPrefixedFile = mkLog('openai-not-a-date.json', new Date(Date.now() - 30 * MS_PER_DAY));
        const missingId = mkLog('openai-2026-06-01T10-00-00.000Z.json', new Date(Date.now() - 30 * MS_PER_DAY));
        const r = await cleanupOldOpenAILogs({ logDir, cutoffDate: cutoff });
        expect(r).toEqual({ removed: 0, errors: 0, completed: true });
    ...
    it('recognizes the current OpenAILogger filename contract', async () => {
        ...
        const generated = await logger.logInteraction({ model: 'test-model' }, { choices: [] }, undefined, 'side-query:session-title');
        const r = await cleanupOldOpenAILogs({
            logDir,
    ...
    it('uses mtime to disambiguate files dated exactly on the cutoff day', async () => {
        ...
        const olderThanCutoff = mkLog(`openai-${cutoffDay}T00-00-00.000Z-a1b2c3d4.json`, new Date(cutoff.getTime() - MS_PER_HOUR));
        const newerThanCutoff = mkLog(`openai-${cutoffDay}T23-59-59.999Z-b2c3d4e5-subagent-Explore-g2tss0.json`, new Date(cutoff.getTime() + MS_PER_HOUR));
        const r = await cleanupOldOpenAILogs({ logDir, cutoffDate: cutoff });
        expect(r).toEqual({ removed: 1, errors: 0, completed: true });
    ...
    it('ignores non-matching files and directories', async () => {
        ...
        const otherLog = mkLog('openai-logs.txt', new Date(Date.now() - 60 * MS_PER_DAY));
        const dirWithMatchingName = path.join(logDir, 'openai-2026-06-01T00-00-00.000Z-a1b2c3d4.json');
    ...
    it.skipIf(process.platform === 'win32')('rejects when the log directory cannot be scanned', async () => {
        fs.chmodSync(logDir, 0o000);
        try {
            await expect(cleanupOldOpenAILogs({ logDir, cutoffDate: cutoff })).rejects.toMatchObject({ code: 'EACCES' });
        }
        finally {
