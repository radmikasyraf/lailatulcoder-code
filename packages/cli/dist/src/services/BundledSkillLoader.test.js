...
describe('BundledSkillLoader', () => {
    ...
    it('should submit skill body as prompt without args', async () => {
        ...
        const loader = new BundledSkillLoader(mockConfig);
        const commands = await loader.loadCommands(signal);
        const result = await commands[0].action({ invocation: { raw: '/review', args: '' } }, '');
    ...
    describe('invocation arguments', () => {
        ...
        async function invoke(raw, args) {
            mockSkillManager.listSkills.mockResolvedValue([makeSkill()]);
            const loader = new BundledSkillLoader(mockConfig);
            const commands = await loader.loadCommands(signal);
            const result = (await commands[0].action({ invocation: { raw, args } }, args ?? ''));
            return result.content[0].text;
        }
        it('appends the raw invocation when args are provided', async () => {
            const text = await invoke('/review 123', '123');
            expect(text).toContain(makeSkillPrompt('You are an expert code reviewer.'));
        ...
        it('preserves flags and spacing exactly', async () => {
            await invoke('/review 6771 --comment --effort high', '6771 --comment --effort high');
            expect(readFileSync(skillArgsPath('review'), 'utf8')).toBe('6771 --comment --effort high');
        });
    ...
    describe('allowedTools grant', () => {
        it('grants allowedTools as session allow rules when the command runs', async () => {
            ...
            const loader = new BundledSkillLoader(mockConfig);
            const commands = await loader.loadCommands(signal);
            await commands[0].action({ invocation: { raw: '/review', args: '' } }, '');
    ...
    it('should resolve {{model}} template variable in skill body', async () => {
        ...
        mockSkillManager.listSkills.mockResolvedValue([skill]);
        mockConfig.getModel.mockReturnValue('qwen3-coder');
        const loader = new BundledSkillLoader(mockConfig);
        const commands = await loader.loadCommands(signal);
        const result = await commands[0].action({ invocation: { raw: '/review', args: '' } }, '');
        expect(result).toEqual({
    ...
    it('should resolve {{model}} when args are provided', async () => {
        ...
        try {
            const loader = new BundledSkillLoader(mockConfig);
            const commands = await loader.loadCommands(signal);
            result = (await commands[0].action({ invocation: { raw: '/review 123', args: '123' } }, '123'));
        }
        finally {
    ...
    it('should use empty string for {{model}} when getModel returns empty string', async () => {
        ...
        mockSkillManager.listSkills.mockResolvedValue([skill]);
        mockConfig.getModel.mockReturnValue('');
        const loader = new BundledSkillLoader(mockConfig);
        const commands = await loader.loadCommands(signal);
        const result = await commands[0].action({ invocation: { raw: '/review', args: '' } }, '');
        expect(result).toEqual({
    ...
    describe('skills.disabled filter', () => {
        it('omits disabled bundled skills (case-insensitive)', async () => {
            ...
            mockConfig.getDisabledSkillNames.mockReturnValue(new Set(['REVIEW'.toLowerCase()]));
            const loader = new BundledSkillLoader(mockConfig);
            const commands = await loader.loadCommands(signal);
...
//# sourceMappingURL=BundledSkillLoader.test.js.map