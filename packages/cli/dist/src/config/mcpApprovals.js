...
import { getErrorMessage, hashMcpServerConfig, isGatedMcpScope, Storage, atomicWriteFile, } from '@lailatul-coder/lailatul-coder-core';
import stripJsonComments from 'strip-json-comments';
import { writeStderrLine } from '../utils/stdioHelpers.js';
export const MCP_APPROVALS_FILENAME = 'mcpApprovals.json';
export function getMcpApprovalsPath() {
...
export class LoadedMcpApprovals {
     ...
     */
    getState(projectRoot, serverName, config) {
        const record = this.file.config[normalizeProjectRoot(projectRoot)]?.[serverName];
        if (!record) {
            return 'pending';
        }
    ...
    async setState(projectRoot, serverName, config, status) {
...
let loadedMcpApprovals;
/** FOR TESTING ONLY. Resets the in-memory cache. */
export function resetMcpApprovalsForTesting() {
...
export function loadMcpApprovals() {
    ...
    loadedMcpApprovals = new LoadedMcpApprovals({ path: filePath, config }, errors);
 ...
 * project `.mcp.json` and workspace `.lailatulcoder/settings.json` (see
 ...
 */
export function getPendingGatedMcpServers(mcpServers, projectRoot) {
 ...
 */
export function getPromptableMcpServers(mcpServers, projectRoot) {
    const approvals = loadMcpApprovals();
    const promptable = [];
    for (const [name, config] of Object.entries(mcpServers)) {
        if (!isGatedMcpScope(config.scope)) {
