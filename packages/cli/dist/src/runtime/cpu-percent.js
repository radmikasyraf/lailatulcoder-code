 ...
 * phantom spike.
 */
export function computeCpuPercent(prev, cur, elapsedMs, coreCount) {
    if (!prev || !cur || elapsedMs <= 0 || coreCount <= 0)
        return 0;
    const cpuUs = cur.user - prev.user + (cur.system - prev.system);
    return Math.min(100, Math.max(0, ((cpuUs / (elapsedMs * 1000)) * 100) / coreCount));
}
//# sourceMappingURL=cpu-percent.js.map