 ...
 * a null return.
 */
export async function readWorktreeSession(filePath, options = {}) {
    let raw;
    try {
...
export async function isSessionRuntimeActive(sessionId, projectRoots) {
    const roots = uniquePaths((Array.isArray(projectRoots) ? projectRoots : [projectRoots]).map((root) => path.resolve(root)));
    ...
    for (const runtimeBase of runtimeBases) {
        for (const projectRoot of roots) {
            const statusPath = await Storage.runWithRuntimeBaseDir(runtimeBase, undefined, async () => new Storage(projectRoot).getRuntimeStatusPath(sessionId));
...
async function getRuntimeStatusStateInBase(runtimeBase, sessionId) {
    ...
    for (const entry of entries) {
        ...
        const statusPath = path.join(projectsDir, entry.name, 'chats', `${sessionId}.runtime.json`);
        const statusState = await getRuntimeStatusPathState(statusPath, sessionId);
        if (statusState === 'active') {
            return 'active';
        }
...
function shouldSkipRuntimeStatusScanDir(name, parent) {
    ...
    return name === 'worktrees' && path.basename(parent) === '.lailatulcoder';
}
...
export async function restoreWorktreeContext(sidecarPath, onWarn) {
    ...
    // `<originalCwd>/.lailatulcoder/worktrees/`. Schema validation (readWorktreeSession)
    ...
    const expectedParent = path.join(session.originalCwd, '.lailatulcoder', 'worktrees');
