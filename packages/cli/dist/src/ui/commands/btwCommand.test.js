...
const mockGetCacheSafeParams = vi.hoisted(() => vi.fn().mockReturnValue({
    generationConfig: {},
    history: [],
    model: 'test-model',
    version: 1,
...
const mockBuildBtwPrompt = vi.hoisted(() => vi.fn().mockImplementation((q) => `<system-reminder>...\n${q}`));
...
describe('btwCommand', () => {
    ...
    describe('interactive mode', () => {
        ...
        it('should not update btwItem when cancelled via btwAbortControllerRef', async () => {
            mockRunForkedAgent.mockImplementation(() => new Promise((resolve) => setTimeout(() => resolve({
                text: 'late answer',
                usage: { inputTokens: 5, outputTokens: 2, cacheHitTokens: 0 },
            }), 50)));
            await btwCommand.action(mockContext, 'test question');
            expect(mockContext.ui.btwAbortControllerRef.current).toBeInstanceOf(AbortController);
...
//# sourceMappingURL=btwCommand.test.js.map