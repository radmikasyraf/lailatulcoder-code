...
import { QWEN_DIR, realpathNearestExisting, resolvePath, sanitizeCwd, } from '../utils/paths.js';
...
/**
 * Directory name (under the repo's `.lailatulcoder/`) for the team auto-memory layer —
...
export { MEMORY_PROJECT_SCOPES } from './scopes.js';
...
const _autoMemoryRootCache = new Map();
...
const _teamAutoMemoryRootCache = new Map();
...
function resolveWorkspaceProjectScope() {
    ...
    if (normalized === 'workspace')
        return true;
    if (normalized !== '' &&
        !MEMORY_PROJECT_SCOPES.includes(normalized) &&
        !warnedUnknownMemoryProjectScope) {
        warnedUnknownMemoryProjectScope = true;
        // eslint-disable-next-line no-console
        console.warn(`[lailatul-coder] Ignoring unrecognized QWEN_CODE_MEMORY_PROJECT_SCOPE="${raw}"; ` +
            'falling back to "git-root". Expected "git-root" or "workspace".');
    }
    return false;
}
export function getAutoMemoryRoot(projectRoot) {
    ...
    if (useLocalMemory) {
        ...
        result = path.join(memoryBaseDir, 'projects', sanitizeCwd(projectKey), AUTO_MEMORY_DIRNAME);
    }
    _autoMemoryRootCache.set(cacheKey, result);
    return result;
}
/** Clear the memoization caches (for tests that change environment or git layout). */
export function clearAutoMemoryRootCache() {
 ...
 * `<projectRoot>/.lailatulcoder/memory`, so the anchor is the project root; otherwise the
 ...
 * repo-tracked `.lailatulcoder -> /outside`) can't silently relocate the allowed root
 * out of the trusted anchor.
 */
export function getAutoMemoryTrustedAnchor(projectRoot) {
 ...
 */
export function getAutoMemoryProjectStateDir(projectRoot) {
 ...
 */
export function isAutoMemPath(absolutePath, projectRoot) {
...
export function getAutoMemoryIndexPath(projectRoot) {
    return path.join(getAutoMemoryRoot(projectRoot), AUTO_MEMORY_INDEX_FILENAME);
}
export function getAutoMemoryMetadataPath(projectRoot) {
    return path.join(getAutoMemoryProjectStateDir(projectRoot), AUTO_MEMORY_METADATA_FILENAME);
}
export function getAutoMemoryExtractCursorPath(projectRoot) {
    return path.join(getAutoMemoryProjectStateDir(projectRoot), AUTO_MEMORY_EXTRACT_CURSOR_FILENAME);
}
export function getAutoMemoryConsolidationLockPath(projectRoot) {
    return path.join(getAutoMemoryProjectStateDir(projectRoot), AUTO_MEMORY_CONSOLIDATION_LOCK_FILENAME);
}
export function getAutoMemoryTopicFilename(type) {
    return `${type}.md`;
}
export function getAutoMemoryTopicPath(projectRoot, type) {
    return path.join(getAutoMemoryRoot(projectRoot), getAutoMemoryTopicFilename(type));
}
export function getAutoMemoryFilePath(projectRoot, relativePath) {
    return path.join(getAutoMemoryRoot(projectRoot), relativePath);
}
/**
 * Returns the user-level (cross-project) auto-memory root.
 * Lives at `${getMemoryBaseDir()}/memories/` — typically `~/.lailatulcoder/memories/`.
 * Unlike project memory, this is NOT scoped to a git root; it is shared
 * across every project the user works in.
 */
export function getUserAutoMemoryRoot() {
    return path.join(getMemoryBaseDir(), USER_AUTO_MEMORY_DIRNAME);
}
export function getUserAutoMemoryIndexPath() {
    return path.join(getUserAutoMemoryRoot(), AUTO_MEMORY_INDEX_FILENAME);
}
export function getUserAutoMemoryTopicPath(type) {
 ...
 */
export function isUserAutoMemPath(absolutePath) {
    const normalizedPath = path.normalize(absolutePath);
...
/**
 * Returns the team auto-memory root: `<gitRoot>/.lailatulcoder/team-memory/`.
 ...
 */
export function getTeamAutoMemoryRoot(projectRoot) {
...
export function getTeamAutoMemoryIndexPath(projectRoot) {
    return path.join(getTeamAutoMemoryRoot(projectRoot), AUTO_MEMORY_INDEX_FILENAME);
}
/**
 ...
 * path-separator differences and path-traversal edge cases are handled.
 */
export function isTeamAutoMemPath(absolutePath, projectRoot) {
    const normalizedPath = path.normalize(realpathNearestExisting(absolutePath));
    const memRoot = path.normalize(realpathNearestExisting(getTeamAutoMemoryRoot(projectRoot)));
    const rel = path.relative(memRoot, normalizedPath);
    return rel === '' || (!rel.startsWith('..') && !path.isAbsolute(rel));
}
/**
 ...
 */
export function isManagedMemoryPath(filePath, projectRoot, baseDir = projectRoot) {
 ...
 */
export function isAnyAutoMemPath(absolutePath, projectRoot) {
