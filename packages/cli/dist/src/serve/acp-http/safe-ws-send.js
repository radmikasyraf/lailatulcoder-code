 ...
 */
export function safeWsSend(ws, payload, context = 'frame') {
    if (ws.readyState === ws.OPEN) {
        try {
            ...
            if (isServeDebugMode()) {
                try {
                    writeStderrLine(`LailatulCoder serve: failed to send ${context} frame on /acp socket: ${err instanceof Error ? err.message : String(err)}`);
                }
                catch {
    ...
    if (isServeDebugMode()) {
        try {
            writeStderrLine(`LailatulCoder serve: dropped ${context} frame on non-OPEN /acp socket (readyState=${ws.readyState})`);
        }
        catch {
...
//# sourceMappingURL=safe-ws-send.js.map