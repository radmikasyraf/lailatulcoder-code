...
import { ghEnv, resolveGhHost, setGhHost, parseNdjson, gh, ghRaw, ghWithInput, ghWithInputRetried, ensureAuthenticated, isOwnerRepo, } from './gh.js';
...
describe('gh() transient-error retry', () => {
    ...
    it('retries on HTTP 503 and succeeds, logging to stderr', () => {
        ...
        mockExecFileSync
            ...
            throw ghError('No server is currently available to service your request. (HTTP 503)');
        })
    ...
    it('retries up to MAX_RETRIES (2) then throws', () => {
        ...
        expect(() => gh('api', 'repos/o/r/pulls/1')).toThrow(/server is currently unavailable/);
...
describe('ghWithInputRetried() DOES retry (idempotent writes)', () => {
    ...
    it('retries a transient HTTP 500 and succeeds on the second attempt', () => {
        ...
        expect(mockExecFileSync).toHaveBeenCalledTimes(2);
        expect(mockExecFileSync.mock.calls[0][2]).toHaveProperty('input', '{"x":1}');
    });
    it('throws immediately on a NON-transient error', () => {
        mockExecFileSync.mockImplementationOnce(() => {
            throw ghError('HTTP 401: Bad credentials');
        });
        expect(() => ghWithInputRetried('{"x":1}', 'api', '--input', '-')).toThrow(/401/);
        expect(mockExecFileSync).toHaveBeenCalledTimes(1);
    });
});
describe('ghWithInput() does NOT retry (non-idempotent POST)', () => {
    ...
    it('throws immediately on HTTP 500 without retrying', () => {
        ...
        expect(() => ghWithInput('{"body":"review"}', 'api', '--input', '-')).toThrow(/Internal Server Error/);
        expect(mockExecFileSync).toHaveBeenCalledTimes(1);
    });
    it('passes input through on success', () => {
        ...
        expect(mockExecFileSync.mock.calls[0][2]).toHaveProperty('input', '{"body":"review"}');
    });
});
describe('ensureAuthenticated() transient retry', () => {
    ...
    it('throws after one retry when auth genuinely fails', () => {
        ...
        expect(() => ensureAuthenticated()).toThrow('gh CLI is not authenticated. Run `gh auth login` and retry.');
...
describe('ghRaw()', () => {
    ...
    it('returns bytes untouched — no trim, no CRLF rewrite, no utf8 loss', () => {
        ...
        mockExecFileSync.mockReturnValueOnce(Buffer.from(' line one\r\n é\r\n', 'latin1'));
        expect(ghRaw('pr', 'diff', '1')).toBe(' line one\r\n é\r\n');
        expect(mockExecFileSync).toHaveBeenCalledWith('gh', expect.any(Array), expect.objectContaining({ encoding: 'buffer' }));
    });
...
describe('ghRaw() transient retry (buffer stderr)', () => {
    ...
    it('still retries on HTTP 503 when stderr arrives as a Buffer', () => {
        ...
        const err = new Error('gh: No server is currently available (HTTP 503)');
...
//# sourceMappingURL=gh.test.js.map