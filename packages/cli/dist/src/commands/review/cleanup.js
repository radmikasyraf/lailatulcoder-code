...
import { clearReviewWorktreeLease, isReviewLeaseFile, readReviewWorktreeLease, reviewLeaseHeldByAnotherSession, reviewLeasePath, } from '../../services/review-worktree-lease.js';
...
import { worktreePath, probeWorktreePath, baseWorktreePath, scratchWorktreePrefix, reviewBranch, REVIEW_TMP_DIR, tmpFile, tmpPrefix, } from './lib/paths.js';
/**
 * Marker prefix every bot comment in this repo's own automation carries
 * (`<!-- LailatulCoder-review-ack -->`, `<!-- LailatulCoder-pr-precheck:… -->`,
 * `<!-- LailatulCoder-triage:… -->`, …). In CI the review runs under the same bot
 ...
 */
const AUTOMATION_MARKER = '<!-- LailatulCoder-';
/**
 ...
 *
 * `LailatulCoder review submit` is the ONLY sanctioned write in `/review`, and it
 ...
 */
export function findUnsanctionedIssueComments(comments, reviewer, sinceIso) {
    const reviewerLc = reviewer.toLowerCase();
    const relevant = comments.filter((c) => (c.user?.login ?? '').toLowerCase() === reviewerLc &&
        typeof c.created_at === 'string' &&
        !isAutomationComment(c.body));
    return {
        posted: relevant.filter((c) => c.created_at >= sinceIso),
        edited: relevant.filter((c) => c.created_at < sinceIso &&
 ...
 */
export function findUnsanctionedReviews(reviews, reviewer, sinceIso, receiptReviewIds) {
    const reviewerLc = reviewer.toLowerCase();
    return reviews.filter((r) => (r.user?.login ?? '').toLowerCase() === reviewerLc &&
...
function readAuditWindow(target, expectedPrNumber) {
    ...
    try {
        ...
        if (typeof report.prNumber !== 'string' ||
        ...
        const auditSince = typeof report.auditSince === 'string' &&
...
function readSubmitReceipt(target) {
    try {
        return new Set(parseReceiptIds(readFileSync(tmpFile(target, 'submit-receipt.json'), 'utf8')));
    }
    catch {
        return new Set();
    }
...
function auditPrWrites(target, prNumber) {
    const skipNote = (reason) => writeStderrLine(`note: bypass audit skipped (${reason})`);
    ...
    try {
        ...
        const boundary = new Date(Date.parse(window.auditSince) - CLOCK_SKEW_MS).toISOString();
        const comments = ghApiAll(`repos/${window.ownerRepo}/issues/${window.prNumber}/comments?since=${encodeURIComponent(boundary)}&per_page=100`);
        const reviews = ghApiAll(`repos/${window.ownerRepo}/pulls/${window.prNumber}/reviews?per_page=100`).filter((r) => typeof r.submitted_at === 'string' && r.submitted_at >= boundary);
        ...
        const { posted, edited } = findUnsanctionedIssueComments(comments, me, boundary);
        const rogueReviews = findUnsanctionedReviews(reviews, me, boundary, readSubmitReceipt(target));
        const total = posted.length + edited.length + rogueReviews.length;
        if (total === 0)
            return;
        writeStdoutLine(`warning: ${total} write(s) by the reviewing account on ` +
            `${window.ownerRepo}#${window.prNumber} during this review window were not made by ` +
            `\`LailatulCoder review submit\` — the only sanctioned write in /review:`);
        for (const c of posted) {
            writeStdoutLine(`warning:   posted comment ${c.id} at ${c.created_at}${c.html_url ? ` — ${c.html_url}` : ''}`);
        }
        for (const c of edited) {
            writeStdoutLine(`warning:   edited comment ${c.id} at ${c.updated_at}${c.html_url ? ` — ${c.html_url}` : ''}`);
        }
        for (const r of rogueReviews) {
            writeStdoutLine(`warning:   review ${r.id} (${r.state ?? 'UNKNOWN'}) at ${r.submitted_at}${r.html_url ? ` — ${r.html_url}` : ''} — no submit receipt vouches for it`);
        }
        writeStdoutLine(`warning: The likely cause is benign — the user (from another terminal), ` +
            `another workflow, or a bot posting under the same account (${me}) produces ` +
            `exactly this shape. ` +
            `\`/review\` writes to the PR only through \`LailatulCoder review submit\`; a write ` +
 ...
 */
function scratchWorktreesOf(worktree) {
    ...
    try {
        ...
        if (err.code !== 'ENOENT') {
            writeStderrLine(`Failed to read ${parent} for scratch worktrees: ${err.message}`);
    ...
    // The LEAF checks below cannot see an ancestor: a symlink at `.lailatulcoder/tmp`
...
export function runCleanup(target) {
    ...
    if (redirected !== null) {
        writeStderrLine(`Refusing to clean: ${redirected} is a symlink, so every delete under ` +
    ...
    if (prMatch) {
        ...
        if (reviewLeaseHeldByAnotherSession(holder)) {
            writeStdoutLine(`note: skipped cleanup for "${target}" — another review session ` +
        ...
        // carrier), check the PR for writes that bypassed `LailatulCoder review submit`.
        ...
        if (redirectedAfterAudit !== null) {
            writeStderrLine(`Refusing to clean: ${redirectedAfterAudit} became a symlink during ` +
        ...
        if (reviewLeaseHeldByAnotherSession(holderAfterAudit)) {
            writeStdoutLine(`note: skipped cleanup for "${target}" — a review session ` +
        ...
        const report = (label, path) => {
            ...
            if (symlink) {
                try {
                    ...
                    writeStderrLine(`Failed to remove ${label} link ${path}: ${err.message}`);
                    failedAny = true;
                }
                return;
            }
        ...
        try {
            ...
            writeStderrLine(`note: could not remove base lock ${baseWorktreePath(wt)}.lock: ${err.message}`);
        }
    ...
    // --- Per-target side files (under .lailatulcoder/tmp/) -------------------------
    ...
    for (const file of tmpEntries) {
        if (!file.startsWith(prefix) || !file.endsWith('-prompts'))
            continue;
        const planCandidate = join(REVIEW_TMP_DIR, `${file.slice(0, -'-prompts'.length)}.json`);
        if (readBudgetStopUnfenced(planCandidate) !== null ||
    ...
    for (const file of tmpEntries) {
        ...
        if (preserved.has(file)) {
            writeStdoutLine(`Kept ${full}: a review run stopped here without converging — ` +
 ...
 */
function hasPreviousRunRecords(planPath) {
    try {
