...
const gitIn = (cwd, ...args) => execFileSync('git', args, { cwd, encoding: 'utf8', env });
beforeAll(() => {
...
const capture = () => execFileSync('git', [...HOSTILE_CONFIG, ...PINNED_CONFIG, 'diff', '--cached', ...CAPTURE_FLAGS], { cwd: repo, maxBuffer: 1 << 28, env }).toString('utf8');
describe('real git capture', () => {
    ...
    it('reports added ranges that exclude context lines', () => {
        ...
        const plain = parseDiff(capture()).files.find((f) => f.path === 'plain.ts');
    ...
    it('survives diff.suppressBlankEmpty even without the pinned -c', () => {
        ...
        const unpinned = execFileSync('git', [...HOSTILE_CONFIG, 'diff', '--cached', ...CAPTURE_FLAGS], { cwd: repo, maxBuffer: 1 << 28, env }).toString('utf8');
...
//# sourceMappingURL=diff-plan.integration.test.js.map