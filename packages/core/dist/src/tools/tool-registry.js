...
import { matchesAnyServerPattern } from '../config/config.js';
...
export class ToolRegistry {
    ...
    revealedDeferred = new Set();
    config;
    mcpClientManager;
    constructor(config, eventEmitter, sendSdkMcpMessage) {
        this.config = config;
        // options-bag
    ...
    static compareToolsByDeclarationName(a, b) {
     ...
     */
    registerTool(tool) {
        if (this.isToolDisabled(tool.name, tool instanceof DiscoveredMCPTool ? tool.permissionAliases : [])) {
            debugLogger.info(`Tool "${tool.name}" skipped: present in disabledTools set.`);
            return;
        }
        ...
        if (this.isToolDisabled(tool.name, tool instanceof DiscoveredMCPTool ? tool.permissionAliases : [])) {
            debugLogger.info(`Tool "${tool.name}" skipped (post-rename): present in disabledTools set.`);
            return;
        }
    ...
    registerFactory(name, factory) {
        if (this.isToolDisabled(name)) {
            debugLogger.info(`Tool factory "${name}" skipped: present in disabledTools set.`);
            return;
        }
    ...
    async warmAll(options) {
        ...
        if (pending.length === 0)
            return;
        const results = await Promise.allSettled(pending.map((name) => this.ensureTool(name)));
     ...
     */
    copyDiscoveredToolsFrom(source) {
     ...
     */
    preloadDeferredToolsWithinBudget(budgetTokens) {
        const candidates = [];
        let totalChars = 0;
        for (const tool of this.tools.values()) {
            if (!tool.shouldDefer || tool.alwaysLoad)
                continue;
            if (this.config.getVisibleTools().has(tool.name))
                continue;
        ...
        if (candidates.length === 0) {
            debugLogger.debug(`preloadDeferredToolsWithinBudget: no deferrable tools to preload (budget=${budgetTokens} tokens).`);
            return 0;
        }
        if (estimatedTokens > budgetTokens) {
            debugLogger.debug(`preloadDeferredToolsWithinBudget: keeping ${candidates.length} deferred tool(s) behind ToolSearch ` +
        ...
        debugLogger.debug(`preloadDeferredToolsWithinBudget: preloading ${candidates.length} deferred tool(s) ` +
    ...
    getFunctionDeclarationsFiltered(toolNames) {
        if (this.factories.size > 0) {
            debugLogger.warn(`getFunctionDeclarationsFiltered() called with ${this.factories.size} unloaded ` +
    ...
    getAllTools() {
        if (this.factories.size > 0) {
            debugLogger.warn(`getAllTools() called with ${this.factories.size} unloaded tool factories. ` +
    ...
    async readMcpResource(serverName, uri, options) {
